...
contract Barn is Ownable, IERC721Receiver, Pausable {
    ...
    external
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else _addAdventurerToBarn(account, tokenIds[i]);
        }
    }
    ...
    _updateEarnings
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else owed += _claimAdventurerFromBarn(tokenIds[i], unstake);
        }
        //fee transfer
    ...
    {
        if (hunter.getTokenTraits(tokenId).isGuard) {
            uint256 alpha = _alphaForHunter(tokenId);
     ...
     * @return isGuard - whether or not a token is a Adventurer
     */
    function isHunter(uint256 tokenId)
    public
    view
    returns (bool isGuard)
    {
        (isGuard, , , , , , , , , , ,) = hunter.tokenTraits(tokenId);
    }
...
contract Hunter is IHunter, ERC721Enumerable, Ownable, Pausable {
    ...
    {
        uint256 seed = _seed;
        t.isGuard = (seed & 0xFFFF) % 10 != 0;
        t.isFemale = (seed & 0xFFFF) % 2 != 0;
        if (t.isGuard) {
            seed >>= 16;
            t.guardBody = selectTrait(uint16(seed & 0xFFFF), 0);
            seed >>= 16;
            t.guardClothes = selectTrait(uint16(seed & 0xFFFF), 3);
            seed >>= 16;
            t.guardShoes = selectTrait(uint16(seed & 0xFFFF), 1);
            seed >>= 16;
            t.skin = selectTrait(uint16(seed & 0xFFFF), 2);
            seed >>= 16;
