...
contract Traits is Ownable, ITraits {
  ...
  string public baseURI = "https://ipfs.io/ipfs/Qmcoh6g2TnH941HJ8bmKC5R9B5VTBpzRFtjgNk16bvXE2s/";

  IHunter public gardenerAndFarmer;
  ...
  function setGame(address _gardenerAndFarmer) external onlyOwner {
    gardenerAndFarmer = IHunter(_gardenerAndFarmer);
  }
  ...
  {
    IHunter.AvtHtr memory s = gardenerAndFarmer
    ...
    return
      string(
        abi.encodePacked(
          ...
          s.isGuard ? '"Guard"' : s.isFemale ? '"FemalePrisoner"' : '"Prisoner"',
          "}]"
        )
      );
  }
