...
contract Norma is ERC721, Ownable {
  ...
  event fiveTokenIdsEvent(
    uint256[] fiveTokenIds
  );
  ...
  function mint(uint256[] memory ids) public payable {
    ...
    // require(supply.add(_numberOfTokens) <= MAX_SUPPLY, "max supply limit exceeded");

    if (m_IsPublic) {
      require(msg.value == m_PublicMintPrice.mul(_numberOfTokens), "insufficient fund");
    } else {
      require(m_WhiteList[msg.sender], "Not in whitelist");
      require(msg.value >= m_MainMintPrice.mul(_numberOfTokens), "insufficient fund");
    }
  ...
  function gainFiveTokenIds() public returns (uint256[] memory) {
    uint256 count = 0;
    uint256[] memory fiveTokenIds;
    bool incrementable = true;

    for (uint256 id = m_StartTokenId + 1; id <= MAX_SUPPLY; id++) {
      if (count == 5)
        break;
      ...
      if (_exists(id)) {
        ...
        incrementable = false;
        fiveTokenIds[count] = id;
        count++;
      }
    }
    // emit fiveTokenIdsEvent(fiveTokenIds);
    return fiveTokenIds;
  }
