    ...
    ERC2981PerTokenRoyalties
{
    ...
    string private baseURI = "https://gateway.pinata.cloud/ipfs/QmUV2B2RrxAwX4Hx4Y2mr9HVnAVoEgvQVtmfbbqgrcfiFJ/";
    string public baseExtension = ".json";
    uint256 public whiteListSize = 0;
    uint256 public cost = 5 ether;       //5 avax 
    uint256 public maxSupply = 5000;
    uint256 public maxMintsForAddress = 5;
    uint256 private contractRoyalties = 1000; //10%
    
    uint256 public publicTime = 0;
    
    mapping(address => bool) public whiteListAddresses;
    mapping(address => uint8) public addressMints;
    
    constructor() ERC721("ProjectX NFT", "#PXT") {
        publicTime = block.timestamp + 7200;
    }
  ...
  */
    function mint(uint256 _mintAmount) public payable {
        require(block.timestamp > publicTime || whiteListAddresses[msg.sender]==true, "ERROR: You can mint after published.");
        
        uint256 supply = totalSupply();
        uint256 canMintAmount = maxMintsForAddress - addressMints[msg.sender];

        require(
            canMintAmount >= _mintAmount,
            "ERROR: You cannot mint more than MAX_MINTS_FOR_ADDRESS"
        );
        require(
            supply + _mintAmount <= maxSupply,
            "ERROR: Not enough tokens left to mint!"
        );
    ...
    */
    function publicNow() public onlyOwner {
        publicTime = block.timestamp;
    }

    /*
    @function setPublicTime(_publicTime)
    @description - Pauses the contract.
    @param <timestamp> _publicTime
    */
    ...
    */
    function setMaxSupply(uint256 amount) public onlyOwner {
        require(
            amount > maxSupply,
            "ERROR: Max supply have to larger than old value."
        );
        maxSupply = amount;
    }

    /*
    @function addToWhitelistMany(addresses)
    @description - Add a list of addresses to the whitelist
    @param <address[]> addresses - The array of addresses to whitelist
    */
    function addToWhitelistMany(address[] memory addresses) public onlyOwner {
        require(block.timestamp < publicTime, "ERROR: Contract is public!");
        require(
            whiteListSize + addresses.length <= 1000,
            "ERROR: MAX number of whitelist addresses added"
        );

        for (uint256 account = 0; account < addresses.length; account++) {
            addToWhitelistOne(addresses[account]);
        }
    }

    /*
    @function addToWhitelistOne(addr)
    @description - Add an address to the whitelist
    @param <address> addr - The address to whitelist
    */
    function addToWhitelistOne(address addr) public onlyOwner {
        require(block.timestamp < publicTime, "ERROR: Contract is public!");
        require(
            whiteListSize < 1000,
            "ERROR: MAX number of whitelist addresses added"
