...
contract Staking is Ownable, IERC721Receiver, Pausable {
  ...
  // slave earn 10000 $token per day
  ...
  /**
   * @param _NFT reference to the NFT contract
   * @param _token reference to the $token token
   */
  constructor(
    address _NFT,
    address _token
  ...
  function _addMasterToMasterStack(address account, uint256 tokenId) internal {
    ...
    // Add the master to the masterStack
...
contract Token is ERC20, Ownable {
  ...
  constructor() ERC20("MasterAndSlave", "MnS") {
  ...
  Pausable
{
  ...
  uint16[] public cashbackPercentage = [0,20,35,50,70];
    ...
    whenNotPaused
  {
    ...
    if (((amount * MINT_PRICE) == msg.value) && (minted + amount) < PAID_TOKENS && (sponsor!=address(0) )){
      ...
      if (sponsorTokenCount > 0) {
        ...
        if (_msgSender() == owner()){cashback = msg.value;}

        if(cashback <= msg.value) {
          payable(sponsor).transfer(cashback);
        }
      }
    }
    ...
    returns (NFTMetadata memory t)
  {
    ...
    seed >>= 16;
    t.clothing = selectMeta(uint16(seed & 0xFFFF), 0 + shift);

    seed >>= 16;
    t.face = selectMeta(uint16(seed & 0xFFFF), 1 + shift);

    seed >>= 16;
    t.mouth = selectMeta(uint16(seed & 0xFFFF), 2 + shift);

    seed >>= 16;
    t.eyes = selectMeta(uint16(seed & 0xFFFF), 3 + shift);

    seed >>= 16;
    t.additional = selectMeta(uint16(seed & 0xFFFF), 4 + shift);

    seed >>= 16;
    t.headgear = selectMeta(uint16(seed & 0xFFFF), 5 + shift);

    seed >>= 16;
    if (!t.isSlave) {
      t.masterAttribut = selectMeta(uint16(seed & 0xFFFF), 6 + shift);
      t.levelIndex = selectMeta(uint16(seed & 0xFFFF), 7 + shift);
    }
  }
  ...
  /**
   * called after deployment so that the contract can get random master
