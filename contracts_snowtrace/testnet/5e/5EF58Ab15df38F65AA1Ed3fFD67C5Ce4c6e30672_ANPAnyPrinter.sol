...
contract ANPAnyPrinter is Auth {

    // TODO:: update all addresses for AVAX chain

    // TODO:: update with AXAC chain AFP address
    // address public AFPC = 0xb044709ae4162C98dba7dd51130319d4bD28b837; 
    address public AFPTC = 0x77d6B20b22Fab6Bcea922AE676e674Ac7F1FF8E8; 


    // avax-mainnet-udsc address
    address public USDC = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75;
    
    // avax-mainnet-wavax address
    // address public WAVAX = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;

    //avax-testnet-avax address
    address public WAVAX = 0xd00ae08403B9bbb9124bB305C09058E32C39A48c;
    address public WAVT = 0x1D308089a2D1Ced3f1Ce36B1FcaF815b07217be3;

    address public DEAD = 0x000000000000000000000000000000000000dEaD;

    // avax-afpt-pair-testnet address
    address public pair = 0x6c07d16e912D33C1555a202DAe3b0285fdCe3749;

    // avax-pangolin-router-testnet address
    address public routerAddress = 0x2D99ABD9008Dc933ff5c0CD271B88309593aB921;
    ...
    function printToken(address tokenToPrint) external {
        ...
        AFPT(payable(AFPTC)).transfer(msg.sender, 1); //Workaround to claim dividend contractually.
...
contract AFPT is Auth {
  ...
  event AutoLiquify(uint256 _amountWAVAXLiquidity, uint256 _amountToLiquify);
  event BuybackMultiplierActive(uint256 _duration);

  /** ======= ERC20 PARAMS ======= */

  string public constant NAME =  "AFPT";    
  string public constant SYMBOL = "AFPT";        
  uint8 public constant DECIMALS = 9;

  uint256 public TOTAL_SUPPLY = 1_000_000_000_000_000 * (10 ** DECIMALS);
  ...
  address public DIVIDEND_TOKEN_ADDRESS = address(0); 
  ...
  // 3% Goes to the dev team; 
  uint256 devFee = 300;
  // 3% Goes to the investment fund;
  uint256 investmentFee = 300;
  ...
  // Address that gets the marketing fees; 
  ...
  // Investment Fund Address
  address public investmentFeeReceiver = address(0);
  ...
  constructor (
    address _router,    
    address _dividendTokenAddress
  ) {
    DIVIDEND_TOKEN_ADDRESS = _dividendTokenAddress;

    autoLiquidityReceiver = msg.sender; 
    marketingFeeReceiver = msg.sender;
    developerFeeReciver = msg.sender;
    investmentFeeReceiver = msg.sender; 

    // TESTNET Addresses
    // testnet WAVAX: 0x1d308089a2d1ced3f1ce36b1fcaf815b07217be3
    // testnet Pangolin Router: 0x2D99ABD9008Dc933ff5c0CD271B88309593aB921
    // testnet USDC: 0x04a0dc7c7029c647e4279accc64d2906d3db283c
    // testnet LPTEST: 0x3c241d25a3668709d2b539773bb16be3bb9facf1

    // TODO:: Double check mainnet addresses
    // WAVAX : 0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7
    // USDC : 0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e
    // TJ Router : 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
  ...
  function swapBack() internal swapping {
    ...
    uint256 amountWAVAX = address(this).balance.sub(balanceBefore);

    uint256 totalWAVAXFee = totalFee.sub(dynamicLiquidityFee.div(2));

    uint256 amountWAVAXLiquidity = amountWAVAX.mul(dynamicLiquidityFee).div(totalWAVAXFee).div(2);

    // Calculate the amount used for reflection fees; 
    uint256 amountWAVAXReflection = amountWAVAX.mul(reflectionFee).div(totalWAVAXFee);
    // Calculate the amount used for marketing fees: 
    uint256 amountWAVAXMarketing = amountWAVAX.mul(marketingFee).div(totalWAVAXFee);
    // Calculate the amount used for dev fees: 
    uint256 amountWAVAXDev = amountWAVAX.mul(devFee).div(totalWAVAXFee);
    // Calculate the amount used for invesment fees: 
    uint256 amountWAVAXInvestment = amountWAVAX.mul(investmentFee).div(totalWAVAXFee);

    // Send the dividend fees to the distributor; 
    distributor.deposit{value: amountWAVAXReflection}();

    // Send the marketing fees; 
    payable(marketingFeeReceiver).transfer(amountWAVAXMarketing);
    // Send the dev fees; 
    payable(developerFeeReciver).transfer(amountWAVAXDev);
    // Send the dev fees; 
    payable(investmentFeeReceiver).transfer(amountWAVAXInvestment);

    // Handle the liquidity adding; 
    if(amountToLiquify > 0){
        router.addLiquidityETH{value: amountWAVAXLiquidity}(
            address(this),
            amountToLiquify,
            0,
            0,
        ...
        emit AutoLiquify(amountWAVAXLiquidity, amountToLiquify);
    }
  }
  ...
  function setFees(
    ...
    totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_investmentFee);
    feeDenominator = _feeDenominator;
    require(totalFee < feeDenominator/4);
  }

  function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _investmentFeeReceiver) external authorized {
  ...
  // Make contract able to recive WAVAX; 
...
contract DividendDistributor  {
    ...
    IERC20 DIVIDEND_TOKEN_ADDRESS = IERC20(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75); // USDC
    ...
    function deposit() external payable onlyToken {
        uint256 balanceBefore = DIVIDEND_TOKEN_ADDRESS.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WAVAX;
        path[1] = address(DIVIDEND_TOKEN_ADDRESS);
        ...
        uint256 amount = DIVIDEND_TOKEN_ADDRESS.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address _shareholder) internal {
        ...
        if(amount > 0){
            ...
            DIVIDEND_TOKEN_ADDRESS.transfer(_shareholder, amount);
            // Update holderClaims; 
