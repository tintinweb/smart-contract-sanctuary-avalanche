...
contract PocketIndex {
    ...
    constructor() {
        ...
        indexLPToken = IIndexLPToken(0x9E7473dc7B1037671eaB5082b916b2223F63Eb18);
    }
        ...
        for (uint256 i = 0; i < totalBuyers; i++) {
            ...
            for (uint256 j = 0; j < totalAssets; j++) {
                ...
                userBalances[nextBuyers[i]].balances[
                    assets[j].contractAddress
                ] += assetBalanceOfContract
                    .mul(userUsdtBalance)
                    .div(100);
            }
            // Send LP tokens to person
            uint256 userNewLp = newLPTokenToMint.mul(userUsdtBalance).div(newBalance);
    ...
    function disolveWithLP(uint256 amount) public {
        ...
        require(indexLPToken.balanceOf(msg.sender) >= amount, "Must have said balance");
        ...
        // Loop and set asset balances to 0
        for (uint256 i = 0; i < totalAssets; i++) {
            uint256 userTokenSoldBalance = userBalances[msg.sender].balances[assets[i].contractAddress].mul(ratio);
            userBalances[msg.sender].balances[assets[i].contractAddress] -= userTokenSoldBalance;
        }
