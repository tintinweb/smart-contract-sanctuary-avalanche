...
contract BSGGDistributionToken is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    function allocateBSGG(uint _amount, uint _ticketTypeId) public onlyOwner {
    ...
    function addTicketType(
        ...
        require(_lockDuration >= 1 days, "Lock duration too short");
    ...
    function deactivateTicketType(uint _ticketTypeId) public onlyOwner {
        ticketTypes[_ticketTypeId].active = false;
    }

    function activateTicketType(uint _ticketTypeId) public onlyOwner {
    ...
    function redeemTicket(uint _ticketId) public whenNotPaused {
    ...
    function getPendingTokens(uint _ticketId) public view returns (uint stakeAmount, uint rewardAmount) {
        Ticket memory currentTicket = tickets[_ticketId];
        uint halfPeriodTimestamp = currentTicket.lockedToTimestamp - ticketTypes[currentTicket.ticketType].lockDuration / 2;

        if (block.timestamp < currentTicket.lockedToTimestamp){
            stakeAmount = currentTicket.amountLocked * 950000 / 1e6; // -5%
    ...
    // Check pending rewards by the date. Returns 0 in deleted ticket Id
    ...
    function getTicketTypes() external view returns(TicketType[] memory allTicketTypes){
        ...
        for (uint i = 0; i < ticketTypeCounter; i++) allTicketTypes[i] = ticketTypes[i];
    }
    ...
    function withdrawNonReservedBSGG(uint _amount, uint _ticketTypeId) public onlyOwner{
        ...
        BSGGAllocation[_ticketTypeId] = 0;
    }
