...
contract Ownable is IOwnable {
    ...
    function verifyPolicy() internal view {
...
contract BetifyBondDepository is Ownable {
    ...
    event LogSetVestingDiscount(uint indexed index, uint percentage);


    /* ======== STATE VARIABLES ======== */

    address public immutable betify; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints BETIFY when receives principle
    address public immutable DAO; // receives profit share from bond
    address public immutable boosterNFT; // Booster Box NFT address
    ...
    mapping (uint8 => uint16) public vestingDiscount;
    mapping (uint8 => uint256) public debt;
    ...
    struct Bond {
        uint payout; // BETIFY remaining to be paid
        uint pricePaid; // In DAI, for front end viewing
        uint32 lastTime; // Last interaction
        uint32 vesting; // Seconds left to vest
        uint16 nftBoosting; // % of discount user opted using BoosterNFT
    }
    ...
    struct Adjust {
        ...
        uint32 lastTime; // betify when last adjustment made
    }
    ...
    constructor ( 
        ...
        require( _betify != address(0), 'err1' );
        betify = _betify;
        ...
        DAO = _DAO;
        require(_boosterNFT != address(0), 'err5');
        ...
        vestingDiscount[0] = 0;   // NO  Discount
        vestingDiscount[1] = 100; // 10% Discount
        vestingDiscount[2] = 150; // 15% Discount
        vestingDiscount[3] = 200; // 20% Discount
        vestingDiscount[4] = 250; // 25% Discount
        vestingDiscount[5] = 300; // 30% Discount
        vestingDiscount[6] = 500; // 50% Discount
    }
    ...
    /**
     *  @notice set Vesting discount
     *  @param _index NFT index
     *  @param _percentage discount in percentage
     */
    function updateVestingDiscount(
        uint8 _index, 
        uint16 _percentage
    ) external onlyPolicy() {
        require(_index != 0, 'NA');

        vestingDiscount[_index] = _percentage;

        emit LogSetVestingDiscount(_index, _percentage);
    }
    ...
    /**
     *  @notice whitelist zapper address to deposit/reedem on user behalf
     *  @param zapper zapper address
     */
    ...
    /**
     *  @notice remove zapper address from whitelist
     *  @param zapper zapper address
     */
    ...
    function deposit( 
        ...
        require((_nftTokenId == 0 || 
                    IERC721(boosterNFT).ownerOf(_nftTokenId) == msg.sender), 'Invalid NFT Index');
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 BETIFY ( underflow protection )
        ...
        uint8 index = 0;
        if (_nftTokenId > 0) {
            index = IERC721(boosterNFT).getNftType(_nftTokenId);
            IERC721(boosterNFT).transferFrom( msg.sender, burnAddress, _nftTokenId );
        }

        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) BETIFY
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( betify ).safeTransfer( DAO, fee ); 
        }

        debt[index] = debt[index].add(value); 
        ...
        bondInfo[ _depositor ] = Bond({
            ...
            vesting: terms.vestingTerm * (1000 - vestingDiscount[index]) / 1000,
            lastTime: uint32(block.timestamp),
            pricePaid: priceInUSD,
            nftBoosting: vestingDiscount[index]
        });
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( betify ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( betify ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( betify ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( betify ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to BETIFY supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( betify ).totalSupply();
    ...
    function debtDecay() public view returns ( uint decay_ ) {
        ...
        uint vestingTerm = 0;
        for (uint8 i = 0; i <= 6; i++) {
            vestingTerm = terms.vestingTerm * (1000 - vestingDiscount[i]) / 1000;
            decay_ += debt[i].mul(timeSinceLast).div(vestingTerm);
        }
    ...
    /**
     *  @notice update debt for each vesting term
     */
    function updateDecay() internal {
        uint32 timeSinceLast = uint32(block.timestamp).sub32( lastDecay );
        uint256 decay = 0;
        uint vestingTerm = 0;
        for (uint8 i = 0; i <= 6; i++) {
            vestingTerm = terms.vestingTerm * (1000 - vestingDiscount[i]) / 1000;
            decay = debt[i].mul(timeSinceLast).div(vestingTerm);
            if (decay > debt[i]) {
                debt[i] = 0;
            } else {
                debt[i] = debt[i].sub(decay);
            }
        }
    }
    ...
    /**
     *  @notice calculate amount of BETIFY available for claim by depositor
     *  @param _depositor address
   ...
   /* ======= AUXILLIARY ======= */
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or BETIFY) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != betify );
