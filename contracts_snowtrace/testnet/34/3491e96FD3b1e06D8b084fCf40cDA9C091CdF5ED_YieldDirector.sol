...
import {IERC20} from "../interfaces/IERC20.sol";
import {IsMHUB} from "../interfaces/IsMHUB.sol";
import {SafeERC20} from "../libraries/SafeERC20.sol";
import {IYieldDirector} from "../interfaces/IYieldDirector.sol";
import {MetahubAccessControlled, IMetahubAuthority} from "../types/MetahubAccessControlled.sol";

/**
    @title YieldDirector (codename Tyche)
    @notice This contract allows donors to deposit their sMHUB and donate their rebases
            to any address. Donors will be able to withdraw their principal
            sMHUB at any time. Donation recipients can also redeem accrued rebases at any time.
 */
contract YieldDirector is IYieldDirector, MetahubAccessControlled {
    using SafeERC20 for IERC20;

    uint256 private constant MAX_UINT256 = type(uint256).max;

    address public immutable sMHUB;
    ...
    struct DonationInfo {
        ...
        uint256 carry; // Amount of sMHUB accumulated over on deposit/withdraw
    ...
    constructor(address sMhub_, address authority_) MetahubAccessControlled(IMetahubAuthority(authority_)) {
        require(sMhub_ != address(0), "Invalid address for sMHUB");
    ...
    /**
        @notice Deposit sMHUB, records sender address and assign rebases to recipient
        @param amount_ Amount of sMHUB debt issued from donor to recipient
    ...
    */
    function deposit(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sMHUB).safeTransferFrom(msg.sender, address(this), amount_);

        uint256 index = IsMHUB(sMHUB).index();
    ...
    /**
        @notice Withdraw donor's sMHUB from vault and subtracts debt from recipient
     */
    function withdraw(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sMHUB).safeTransfer(msg.sender, amount_);
    ...
    function withdrawAll() external override {
        ...
        uint256 sMhubIndex = IsMHUB(sMHUB).index();
        uint256 total = 0;
        ...
        for (uint256 index = 0; index < donationsLength; index++) {
            ...
            recipient.agnosticDebt = _toAgnostic(recipient.totalDebt + recipient.carry);
            recipient.indexAtLastChange = sMhubIndex;
        ...
        IERC20(sMHUB).safeTransfer(msg.sender, total);

        emit AllWithdrawn(msg.sender, total);
    }

    /**
        @notice Get deposited sMHUB amount for specific recipient
     */
    ...
    /**
        @notice Return total amount of donor's sMHUB deposited
     */
    ...
    /**
        @notice Return total amount of sMHUB donated to recipient since last full withdrawal
     */
    ...
    /**
        @notice Return total amount of sMHUB donated from donor since last full withdrawal
     */
    ...
    /**
        @notice Get redeemable sMHUB balance of a recipient address
     */
    ...
    /**
        @notice Redeem recipient's full donated amount of sMHUB at current index
        @dev Note that a recipient redeeming their vault shares effectively pays back all
             sMHUB debt to donors at the time of redeem. Any future incurred debt will
    ...
    function redeem() external override {
        ...
        recipient.carry = 0;
        recipient.indexAtLastChange = IsMHUB(sMHUB).index();

        IERC20(sMHUB).safeTransfer(msg.sender, redeemable);
    ...
    /**
        @notice Get accumulated sMHUB since last time agnostic value changed.
     */
    ...
    /**
        @notice Convert flat sMHUB value to agnostic value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sMHUB has 9 decimals.
     */
    function _toAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * 1e9) / (IsMHUB(sMHUB).index());
    }

    /**
        @notice Convert agnostic value at current index to flat sMHUB value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sMHUB has 9 decimals.
     */
    function _fromAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * (IsMHUB(sMHUB).index())) / 1e9;
    }

    /**
        @notice Convert flat sMHUB value to agnostic value at a given index value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sMHUB has 9 decimals.
     */
