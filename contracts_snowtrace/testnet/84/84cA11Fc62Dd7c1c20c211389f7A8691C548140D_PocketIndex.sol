...
contract PocketIndex {
    ...
    function bulkBuy() public {
        ...
        if (totalBulkBuys == 0) {
            ...
            uint256 ratio = totalCurrentBalance.div(totalCurrentWorth);
            uint256 denominator = one.sub(ratio);
            uint256 newTotalLP = totalLp.div(denominator);
    ...
    function disolveWithLP(uint256 amount) public {
        ...
        for (uint256 i = 0; i < totalAssets; i++) {
            path[0] = assets[i].contractAddress;
            userBalance = userBalances[msg.sender]
    ...
    function currentLP() public view returns (uint256) {
        return indexLPToken.totalSupply();
    }
    ...
    function ratio() public view returns (uint256) {
        return
            baseTokenAddress.balanceOf(address(this)).mul(10000).div(
                baseTokenAddress.balanceOf(address(this)).add(currentWorth())
            );
    }

    function denominator() public view returns (uint256) {
        return one().sub(ratio());
    }

    function totalLPDivDenominator() public view returns (uint256) {
        return indexLPToken.totalSupply().mul(10000).div(denominator());
    }

    function newLPToMint() public view returns (uint256) {
        return totalLPDivDenominator().sub(indexLPToken.totalSupply());
    }
