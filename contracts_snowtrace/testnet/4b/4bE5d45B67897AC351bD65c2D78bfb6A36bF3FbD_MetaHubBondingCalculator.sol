...
contract MetaHubBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable MHUB;
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(MHUB)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(MHUB), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(MHUB)).decimals())).div(getTotalValue(_pair));
    }
}
