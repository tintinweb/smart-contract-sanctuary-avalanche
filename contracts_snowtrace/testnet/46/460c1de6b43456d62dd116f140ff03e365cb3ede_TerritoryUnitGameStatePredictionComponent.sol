...
contract TerritoryUnitGameStatePredictionComponent {
    ...
    function _addAttacker(uint _poolId, address _contractReference, uint _tokenIdReference, uint _fightEntry, uint _lp, uint[] memory _targetsList, uint _dmgPerSecond) internal returns (uint) {
        ...
        uint[] memory _fullAttackersPool = attackers[_poolId].getWholeArray();
        if (_targetsList.length == 0) // if no list have been provided, create it from current attackers pool
            _targetsList = _fullAttackersPool;
        else
            require(_targetsList.length == _fullAttackersPool.length, "length dismatch");
        ...
        for (uint i=0; i < _attackersUniqueIds.length - 1; i++) {
            ...
            for (uint n=0; n < _attackersUniqueIds.length; n++) {
                if (_data._lps[n] > 0) {
                    ...
                    (_attack, _targetUniqueId) = _data._targetPosition[n] <= attackersIndex[_attackersUniqueIds[n]].targetsList.length-1 ? (true, attackersIndex[_attackersUniqueIds[n]].targetsList[_data._targetPosition[n]]) : (false, 0);
                    ...
                    if (_attack) {
                        ...
                        uint _deltaTimeTargeted = attackersIndex[_attackersUniqueIds[_targetUniqueId]].fightEntry > time ? attackersIndex[_targetUniqueId].fightEntry - time : 0;
                        uint _highestDeltaTime = _deltaTimeAttacker > _deltaTimeTargeted ? _deltaTimeAttacker : _deltaTimeTargeted;

                        if (_data._deathTime[_targetUniqueId] == 0) { // if there is no death time predicted ..
                            _data._deathTime[_targetUniqueId] = cal((_data._lps[_targetUniqueId] / attackersIndex[_attackersUniqueIds[n]].dmgPerSecond) + _highestDeltaTime);
                            _data._cumulatedDamage[_targetUniqueId] += cal(attackersIndex[_attackersUniqueIds[n]].dmgPerSecond);
                        } else {
                            // if the assailled unit death time is under the max figth entry, go to the next attacker
                            if (_highestDeltaTime >= _data._deathTime[_targetUniqueId]) {
                                continue;
                            }
                            _data._cumulatedDamage[_targetUniqueId] += cal(attackersIndex[_attackersUniqueIds[n]].dmgPerSecond);
                            _data._deathTime[_targetUniqueId] = cal(((_data._deathTime[_targetUniqueId] - _deltaTimeAttacker) / _data._cumulatedDamage[_targetUniqueId]) + (_deltaTimeAttacker));
                        }

                        // replace the closest death time by this one if it's the smallest value
                        if (_data._deathTime[_targetUniqueId] < _closestDeathTime || _closestDeathTime == 0) {
                            _closestDeathTime = cal(_data._deathTime[_targetUniqueId]);
                            _closestDeathTimeAttackerIndex = cal(_targetUniqueId);
                        }
            ...
            uint _dmg;
            for (uint n=0; n < _attackersUniqueIds.length; n++) {
                uint _entry = attackersIndex[_attackersUniqueIds[_closestDeathTimeAttackerIndex]].fightEntry;
                if (_entry <= _closestDeathTime) { // if this unit may have received damage (entered the fight before the closest death time added above (and already added to "time"))
                    _dmg = (_closestDeathTime - _entry) * _data._cumulatedDamage[n];
                    cal(_dmg);
                    cal(_closestDeathTime);
                    cal(_entry);
                    cal(_data._cumulatedDamage[n]);
                    if (_dmg < _data._lps[n] && _data._lps[n] > 0) {
            ...
            for (uint n=0; n < _attackersUniqueIds.length; n++) {
                for (uint w=0; w < _attackersUniqueIds.length; w++) {
                    (_attack, _targetUniqueId) = _data._targetPosition[n] <= attackersIndex[_attackersUniqueIds[n]].targetsList.length-1 ? (true, attackersIndex[_attackersUniqueIds[n]].targetsList[_data._targetPosition[n]]) : (false, 0);
                    if (_data._lps[_targetUniqueId] == 0 && _attack) {
