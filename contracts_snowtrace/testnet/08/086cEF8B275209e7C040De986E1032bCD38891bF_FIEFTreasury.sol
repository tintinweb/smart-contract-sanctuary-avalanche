...
contract FIEFTreasury is AccessControl, Signable {
    ...
    function buyTokens(BuyLib.Buy calldata buy) external {
        ...
        require(buy.tokenIds.length > 0, "token_ids_empty");
        ...
        uint256[] memory tokenIds = buy.tokenIds;
        for (uint256 index = 0; index < tokenIds.length; index++) {
            require(!tokenIdSold[tokenIds[index]], "token_id_already_sold");
            tokenIdSold[tokenIds[index]] = true;
            IERC721(nft).transferFrom(multisig, msg.sender, tokenIds[index]);
        }

        emit TokensBought(msg.sender, signer, buy.paymentToken, buy.tokenIds, buy.totalPrice);
    }

    function addPaymentTokens(address[] memory paymentTokensList)
        external
    ...
    function removePaymentTokens(address[] memory paymentTokensList)
        external
        hasRoleAccount(CONFIGURATOR_ROLE, msg.sender)
    {
        require(paymentTokensList.length > 0, "tokens_list_empty");
...
library BuyLib {
    ...
    function getSigner(
        ...
        bytes32 hash = hashRequest(self, sender, treasury, chainId);
