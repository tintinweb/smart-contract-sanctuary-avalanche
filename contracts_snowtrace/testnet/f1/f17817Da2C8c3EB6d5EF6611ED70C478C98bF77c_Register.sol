...
contract Register is IdCard, UserData {
    ...
    function setInfoHash(string memory _infoHash) public {
        _setInfoHash(tokenOf(msg.sender), _infoHash);
    }
...
import "@openzeppelin/contracts/utils/Counters.sol";
import "./ERC721Uncollectible/ERC721Uncollectible.sol";
import "./Peyment.sol";
    ...
    Peyment
{
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        if (tokenId == 0) {return owner();} // token number 0 belows to the DAO.
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        internal
        whenNotPaused
        override(ERC721, ERC721Uncollectible)
    {
...
abstract contract UserData {
    ...
    function _setProfile(
        ...
        emit NewUser(_userId, _username);
    }
    event NewUser(uint256 _userId, string _username);


    function _setInfoHash(uint256 _userId, string memory _infoHash) internal {
        users[_userId].infoHash = _infoHash;
        emit SetInfo(_userId, _infoHash);
    }
    event SetInfo(uint256 userId, string infoHash);
...
import "./IERC721Uncollectible.sol";

/**
 * @title ERC721 Uncollectible Token
 * @dev ERC721 Token that every address can just have one of it.
 */
abstract contract ERC721Uncollectible is IERC721Uncollectible, Context, ERC721 {
    ...
    function tokenOf(address owner) public view returns(uint256) {
        require(ownsToken(owner), "ERC721Uncollectible: this address does not have a token");
        ...
        override
    {
        ...
        require(!ownsToken(to), "ERC721Uncollectible: this address does have a token");
...
abstract contract Peyment is ContDataCall, ERC721Uncollectible {
    ...
    function deposit(uint256 userId, string memory memo) public payable {
        require(msg.sender.isContract(), "sender is not contract");
        _deposit(userId, msg.value);
        emit Deposit(userId, msg.sender, msg.value, memo);
    }
    event Deposit(uint256 userId, address sender, uint256 amount, string memo);
    ...
    function withdraw() public {
        ...
        emit  Withdraw(IdCard, receiver, "Owner, Withdraw all", amount);
    }
    event Withdraw(uint256 IdCard, address receiver, string memo, uint256 value);
    ...
    function _registerPayment(
        ...
        _deposit(referral, refBon);
        _deposit(dapp, dappBon);
        _deposit(DAO, amount.sub(refBon + dappBon));
    }

    /**
     * @dev _Deposit the `amount` in wei to the `addr` in pendeingwithdrawals;
     */
    function _deposit(uint256 userId, uint256 amount) internal {
 ...
 * @dev ERC721 Token that every address can just have one of it.
 */
interface IERC721Uncollectible {

    /**
     * @dev Returns true if `owner` has a token.
     */
    function ownsToken(address owner) external view returns(bool);
 ...
 */
abstract contract ContDataCall {

    IContData ContData;

    /**
     * @dev ContData eternal address on fuji testnet.
     */
    constructor() {
        ContData = IContData(0x50724849ee6F5cF087347A65EbC763c832ce9DE9);
    }
