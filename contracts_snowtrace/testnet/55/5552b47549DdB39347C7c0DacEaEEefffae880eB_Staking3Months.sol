...
contract Staking3Months is Ownable, Pausable {
    ...
    function withdraw(uint256 id) external{
        ...
        require(
            deposits[msg.sender][id].depositedQty > 0,
            "There is nothing to deposit"
        );
        require(
            deposits[msg.sender][id].depositedAt >= deposits[msg.sender][id].depositedAt+(depositTime),
            "Staking period not passed yet"
        );
        require(
            deposits[msg.sender][id].isWithdrawn == false
        );
    ...
    function _stake(address staker, uint256 amount) private _after(stakingStarts) _before(stakingEnds) whenNotPaused {
        
        stakedTotal = stakedTotal+(amount);
        //_stakes[staker] = _stakes[staker]+(remaining);

        deposits[staker].push(
            Deposit(
                block.timestamp,
                amount,
                false
