...
contract BSGGDistributionToken is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    mapping(uint => uint) public BSGGAllocation; // ticketTypeId => BSGG allocation
    mapping(uint => uint) public BSGGAllTimeAllocation; // ticketTypeId => BSGG allocation
    ...
    */
    function allocateBSGG(uint _amount, uint _ticketTypeId) external onlyOwner {
        require(_ticketTypeId < ticketTypeCounter, "Bad ticket type");
        BSGGAllocation[_ticketTypeId] += _amount;
        BSGGAllTimeAllocation[_ticketTypeId] += _amount;
    ...
    function addTicketType(
        ...
        uint _maxLockAmount,
        uint _lockDuration,
        uint _gainMultiplier
    ) external onlyOwner {
        require(_minLockAmount >= 1 ether, "Bad minimum lock amount");
        require(_maxLockAmount >= _minLockAmount, "Max < min");
        require(_lockDuration >= 1 hours, "Lock duration too short");
        require(_gainMultiplier > 1e6, "Gain multiplier lower or equal to base");
        ...
        ticketTypes[ticketTypeCounter].maxLockAmount    = _maxLockAmount;
    ...
    function updateTicketType(
        ...
        currentTicket.maxLockAmount    = _maxLockAmount;
    ...
    function deactivateTicketType(uint _ticketTypeId) external onlyOwner {
        ticketTypes[_ticketTypeId].active = false;
    }

    /// @dev Activates a selected ticket type.
    function activateTicketType(uint _ticketTypeId) external onlyOwner {
    ...
    function buyTicket(
        ...
        require(ticketTypes[_ticketTypeId].active, "Bad ticket");
        require(_amount >= ticketTypes[_ticketTypeId].minLockAmount, "Too small stake amount");
        require(_amount <= ticketTypes[_ticketTypeId].maxLockAmount, "Too big stake amount");
        ...
        tickets[ticketId].amountToGain       = (_amount * ticketTypes[_ticketTypeId].gainMultiplier / 1e6) - _amount;
        ...
        require(tickets[ticketId].amountToGain <= BSGGAllocation[_ticketTypeId], "Sold out");
        BSGGAllocation[_ticketTypeId] -= tickets[ticketId].amountToGain;
    ...
    */
    function redeemTicket(uint _ticketId) external {
        ...
        BSGGAllocation[currentTicket.ticketType] += totalAmountToReAllocate;
    ...
    function getTicketTypes() external view returns(TicketTypeFrontend[] memory allTicketTypes){
        allTicketTypes = new TicketTypeFrontend[](ticketTypeCounter);
        for (uint i = 0; i < ticketTypeCounter; i++) {
            allTicketTypes[i].ticketTypeId          = ticketTypes[i].id;
            allTicketTypes[i].active                = ticketTypes[i].active;
            allTicketTypes[i].minLockAmount         = ticketTypes[i].minLockAmount;
            allTicketTypes[i].maxLockAmount         = ticketTypes[i].maxLockAmount;
            allTicketTypes[i].lockDuration          = ticketTypes[i].lockDuration;
            allTicketTypes[i].gainMultiplier        = ticketTypes[i].gainMultiplier;
            allTicketTypes[i].BSGGAllocation        = BSGGAllocation[i];
            allTicketTypes[i].BSGGAllTimeAllocation = BSGGAllTimeAllocation[i];
        }
    }
    ...
    function triggerFireAlarm(uint code) external onlyOwner{
        ...
        for(uint i = 0; i < ticketTypeCounter; i++){
            ...
            uint withdrawAmount = BSGGAllocation[i] > availableBalance ? availableBalance : BSGGAllocation[i];
    ...
    function emergencyWithdraw() external{
        ...
        for(uint i = 0; i < userTicketsCount; i++){
            ...
            uint availableBalance = BSGG.balanceOf(address(this));
            uint withdrawAmountWanted = tickets[ticketId].amountLocked + getPendingRewards(ticketId);
            uint withdrawAmountPossible = withdrawAmountWanted > availableBalance ? availableBalance : withdrawAmountWanted;
    ...
    function withdrawNonReservedBSGG(uint _amount, uint _ticketTypeId) external onlyOwner{
        uint withdrawAmount = BSGGAllocation[_ticketTypeId] >= _amount ? _amount : BSGGAllocation[_ticketTypeId];
        BSGGAllTimeAllocation[_ticketTypeId] -= withdrawAmount;
        BSGGAllocation[_ticketTypeId] -= withdrawAmount;
        BSGG.transfer(msg.sender, withdrawAmount);
    }
    
    function _beforeTokenTransfer(address from, address to, uint ticketId) internal whenNotPaused override(ERC721, ERC721Enumerable){
