...
contract Barn is Ownable, IERC721Receiver, Pausable {
    ...
    {
        if (hunter.getTokenTraits(tokenId).isPrisoner) {
     ...
     * @return isPrisoner - whether or not a token is a Adventurer
     */
    function isPrisoner(uint256 tokenId)
    public
    view
    returns (bool isPrisoner)
    {
        (isPrisoner, , , , , , , , , , ,) = hunter.tokenTraits(tokenId);
    }
    ...
    function _alphaForHunter(uint256 tokenId) internal view returns (uint8) {
        (, , , , , , , , uint8 guardScore, , ,) = hunter.tokenTraits(tokenId);
        return MAX_ALPHA - guardScore; // alpha index is 0-3
    }
...
contract Hunter is IHunter, ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(
        ...
        AdminWallet = payable(0x692C83F4a08F9CD84Aed500EddA0aA4656096357);
        Multisig = payable(0x692C83F4a08F9CD84Aed500EddA0aA4656096357);
        gem = IGEM(_gem);
