    ...
    WalletObserverImplementationPointer
{
    ...
    constructor(address _factionsManager) ERC20("SpookyVerse", "FEAR") {
        require(
            _factionsManager != address(0),
    ...
    function _setFeesAmount(uint _sellFeesAmount, uint _transferFeesAmount) external onlyOwner {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        } else { // apply transfer fee
            uint _fees = (amount * transferFeesAmount) / 1000;
            _transferAmount = amount - _fees;
            _transfer(_msgSender(), treasury, _fees); // transfer fee to treasury address
        }
        _transfer(_msgSender(), recipient, _transferAmount);
        return true;
    }
