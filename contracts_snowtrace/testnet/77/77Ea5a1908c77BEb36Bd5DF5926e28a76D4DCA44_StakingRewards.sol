...
contract StakingRewards {
	...
	uint256 constant private PERCENT_FEE = 5; // only for notOHM staking
	...
	function initialize(uint256 _totalRewards, uint256 _stakingRewardsStart, ERC20 _token) external {
		...
		info.notOHM = NotOHM(payable(msg.sender));
		info.token = _token;
	}
	...
	function update() public {
		...
		if (_now > info.lastUpdated && totalDeposited() > 0) {
			...
			uint256 _balanceBefore = info.notOHM.balanceOf(address(this));
			info.notOHM.claimRewards();
			_reward += info.notOHM.balanceOf(address(this)) - _balanceBefore;
	...
	function tokenCallback(address _from, uint256 _tokens, bytes calldata) external returns (bool) {
		require(_isNotOHM() && msg.sender == tokenAddress());
		require(_tokens > 0);
		update();
	...
	function disburse(uint256 _amount) public {
		...
		update();
		uint256 _balanceBefore = info.notOHM.balanceOf(address(this));
		info.notOHM.transferFrom(msg.sender, address(this), _amount);
		uint256 _amountReceived = info.notOHM.balanceOf(address(this)) - _balanceBefore;
	...
	function claim() public {
		...
		if (_rewards > 0) {
			...
			info.notOHM.transfer(msg.sender, _rewards);
	...
	function notOHMAddress() public view returns (address) {
		return address(info.notOHM);
	}
	...
	function allInfoFor(address _user) external view returns (uint256 startingTime, uint256 totalRewardsDistributed, uint256 rewardsRatePerDay, uint256 currentFeePercent, uint256 totalTokensDeposited, uint256 virtualRewards, uint256 userNotOHM, uint256 userBalance, uint256 userAllowance, uint256 userDeposited, uint256 userRewards) {
		...
		userNotOHM = info.notOHM.balanceOf(_user);
	...
	function _isNotOHM() internal view returns (bool) {
		return notOHMAddress() == tokenAddress();
	}

	function _calculateFee(uint256 _amount) internal view returns (uint256) {
		return _isNotOHM() ? (_amount * PERCENT_FEE / 100).mul(1e18 - _sum(_getX(block.timestamp))) : 0;
	}
...
contract NotOHM {
	...
	uint256 constant private notOHM_STAKING_SUPPLY = 2e25; // 20M WTF
	...
	string constant public name = "rebase.wtf";
	string constant public symbol = "NotOHM";
	...
	function initialize(uint256 _stakingRewardsStart, Router _router, address _DAO, address _ops, address _signer) external {
		...
		info.stakingRewards = new StakingRewards();
		info.stakingRewards.initialize(notOHM_STAKING_SUPPLY, _stakingRewardsStart, ERC20(address(this)));
		_mint(stakingRewardsAddress(), notOHM_STAKING_SUPPLY);
		info.lpStakingRewards = new StakingRewards();
		info.lpStakingRewards.initialize(LP_STAKING_SUPPLY, _stakingRewardsStart, ERC20(pairAddress()));
	...
	function claim(address _account, uint256 _amount, bytes calldata _signature) external {
		claimNotOHM(_account, _amount, _signature);
	}
	
	function claimNotOHM(address _account, uint256 _amount, bytes calldata _signature) public {
		...
		require(!isClaimedNotOHM(_account), "Claimed");
	...
	function isClaimedNotOHM(address _account) public view returns (bool) {
	...
	function claimInfoFor(address _user) external view returns (bool unlocked, bool claimedWTF, uint256 wethReserve, uint256 notOHMReserve) {
		unlocked = isUnlocked(_user);
		claimedWTF = isClaimedNotOHM(_user);
		( , , wethReserve, notOHMReserve, , , ) = allInfoFor(address(0x0));
	}

	function allInfoFor(address _user) public view returns (uint256 totalTokens, uint256 totalLPTokens, uint256 wethReserve, uint256 notOHMReserve, uint256 userBalance, uint256 userRewards, uint256 userLPBalance) {
		...
		notOHMReserve = info.weth0 ? _res1 : _res0;
