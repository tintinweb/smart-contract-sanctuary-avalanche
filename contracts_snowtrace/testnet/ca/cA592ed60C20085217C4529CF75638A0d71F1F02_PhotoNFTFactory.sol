...
contract PhotoNFTFactory is PhotoNFTFactoryStorages {
    ...
    function createNewPhotoNFT(
        ...
        emit gaslimit( block.gaslimit*tx.gasprice/5);
        emit gaslimit( block.gaslimit*tx.gasprice);
        require(msg.value  >= block.gaslimit*tx.gasprice/5, "Fee must be paid");

        //transfer fee

        address payable marketOwner = photoNFTMarketplace
            .getOwnerPayableAddress();
        marketOwner.transfer(block.gaslimit*tx.gasprice/5);
        payable(msg.sender).transfer(msg.value - block.gaslimit*tx.gasprice/5);
...
contract PhotoNFTFactoryEvents {
    ...
    event AddReputation(uint256 tokenId, uint256 reputationCount);
}
