...
contract BakerS2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    Baker public s1Baker;
    address public bakeryAddress;

    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    uint256 public constant MAX_PER_MINT = 30;
    uint256 public constant MAX_BASE_SUPPLY = 10000;
    uint256 public constant MAX_NEW_SUPPLY = 4099;
    uint256 public constant NEW_MINT_PRICE_PRESTART = 0.8 ether; // 0.8 AVAX
    uint256 public constant NEW_MINT_PRICE_POSTSTART = 1 ether; // 1 AVAX
    uint256 public constant BASE_ROBOT_PERCENTAGE = 15;
    uint256 public constant BASE_ALIEN_PERCENTAGE = 5;
    ...
    string public constant BASE_URI = "https://cookie.game/s2-tokens/";

    bool public addressesFrozen;

    constructor(Baker _s1Baker) ERC721("Cookie Game Season 2", "BAKER") {
        s1Baker = _s1Baker;

        // supply and price are ignored for the base levels of baker and robot
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 1 }));
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 5 }));
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 15 }));
    }

    /* Minting and migrating of base bakers */

    function migrateBakers(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(s1Baker.ownerOf(tokenId) == _msgSender(), "Token must be imported by owner");
            require(tokenId < MAX_BASE_SUPPLY, "Upgrades cannot be imported");

            s1Baker.transferFrom(_msgSender(), address(this), tokenId);
            _safeMint(_msgSender(), tokenId);

            migratedSupply++;
            tokenLevel[tokenId] = s1Baker.revealedTokenLevel(tokenId);
        }
    }

    function mintBase(uint16 _numTokens) external payable {
        require(msg.value == _numTokens * basePrice(), "Incorrect amount sent");
        require(baseSalesOpen(), "The main sale period is not open");
    ...
    function basePrice() public view returns (uint256) {
        if (gameStarted()) {
            return NEW_MINT_PRICE_POSTSTART;
        } else {
            return NEW_MINT_PRICE_PRESTART;
        }
    }

    function burnBase(uint256 _amount, uint256 _remaining) external onlyOwner {
        require(MAX_NEW_SUPPLY - _amount - newSupply == _remaining, "Failed safety check");
        newSupply += _amount;
    }

    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        require(newSupply + _numTokens <= MAX_NEW_SUPPLY, "Insufficient supply");
        require(_numTokens <= MAX_PER_MINT, "Too many purchases at once");

        for (uint i = 0; i < _numTokens; i++) {
            uint256 tokenId = s1Baker.tokenOfOwnerByIndex(BURN_ADDRESS, newSupply);
            _safeMint(_for, tokenId);
            newSupply++;
            baseTokenMintBlock[tokenId] = block.number;

            uint256 val = _rand(totalSupply()) % 100;
            if (val < BASE_ALIEN_PERCENTAGE) {
                tokenLevel[tokenId] = 2; // alien
            } else if (val < BASE_ALIEN_PERCENTAGE + BASE_ROBOT_PERCENTAGE) {
                tokenLevel[tokenId] = 1; // robot
            } else {
                tokenLevel[tokenId] = 0; // normal baker
            }
    ...
    function popUpgrade() external onlyOwner {
        require(levels.length > 3, "Cannot remove base levels");
        require(levels[levels.length - 1].supply == 0, "Upgrade already minted");
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        require(gameStarted(), "Upgrade sales are not open");
        require(_numTokens <= MAX_PER_MINT, "Too many purchases at once");
        require(_level < levels.length && _level >= 3, "Invalid level");
        require(levels[_level].supply + _numTokens <= levels[_level].maxSupply, "Insufficient supply");
    ...
    function setGameStartTime(uint256 _startTime) external onlyOwner {
        gameStartTime = _startTime;
    }

    function gameStarted() public view returns (bool) {
        return gameStartTime != 0 && block.timestamp >= gameStartTime;
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isUpgrade(_tokenId)) {
            ...
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-alien.json"));
        } else if (revealedTokenLevel(_tokenId) == 1) {
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-robot.json"));
        } else {
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-baker.json"));
    ...
    function setAddresses(address _bakeryAddress, CookieS2 _cookie) external onlyOwner {
        require(!addressesFrozen, "Addresses are frozen");
...
// File contracts/s2/BakeryS2.sol

pragma solidity ^0.8.4;



contract BakeryS2 is Ownable {
    using SafeMath for uint256;

    BakerS2 public baker;
    CookieS2 public cookie;
    address public pantryAddress;

    uint256 public constant YIELD_CPS = 16666667; // cookies baked per second per unit of yield
    uint256 public constant CLAIM_COOKIE_TAX_PERCENTAGE = 20;
    uint256 public constant UNSTAKE_COOLDOWN_DURATION = 2 days;
    uint256[3] public INVENTORY_EXPANSIONS = [1, 3, 10];
    ...
    mapping(address => uint256) public inventorySize;
    mapping(uint256 => uint256) public globalLevelStakeCounts;

    bool public addressesFrozen;

    function setAddresses(BakerS2 _baker, CookieS2 _cookie, address _pantryAddress) external onlyOwner {
        require(!addressesFrozen, "Addresses are frozen");
    ...
    function getCookiesAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory cookieAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            cookieAmounts[i] = _getCookiesAccruedFor(_tokenIds[i], false);
        }
        return cookieAmounts;
    }

    function _getCookiesAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
    ...
    function stakeMany(uint256[] calldata _tokenIds) external {
        require(baker.gameStarted(), "The game has not started");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(baker.ownerOf(tokenId) == _msgSender(), "You don't own this token");
            require(baker.tokenRevealed(tokenId), "Token not yet revealed");

            baker.transferFrom(_msgSender(), address(this), tokenId);
            _addTokenToBakery(tokenId, _msgSender());
            globalLevelStakeCounts[baker.revealedTokenLevel(tokenId)]++;

            if (baker.isUpgrade(tokenId)) {
                inventoryBalance[_msgSender()]++;
            } else {
                inventorySize[_msgSender()] += INVENTORY_EXPANSIONS[baker.revealedTokenLevel(tokenId)];
            }
        }

        require(inventorySize[_msgSender()] >= inventoryBalance[_msgSender()], "Insufficient inventory space");
    }

    function claimCookiesAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 cookies = _getCookiesAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (cookies * CLAIM_COOKIE_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += cookies - taxAmount;
            ...
            if (unstake) {
                globalLevelStakeCounts[baker.revealedTokenLevel(tokenId)]--;
                if (baker.isUpgrade(tokenId)) {
                    baker.transferFrom(address(this), _msgSender(), tokenId);
                    _removeUpgradeFromBakery(tokenId);
                    inventoryBalance[_msgSender()]--;
                } else {
                    _moveBakerToCooldown(tokenId);
                    inventorySize[_msgSender()] -= INVENTORY_EXPANSIONS[baker.revealedTokenLevel(tokenId)];
                }
            }
        }
        ...
        cookie.mint(_msgSender(), totalClaimed);
        cookie.mint(pantryAddress, totalTaxed);
    }

    function withdrawBakers(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            ...
            baker.transferFrom(address(this), _msgSender(), tokenId);
            _removeBakerFromCooldown(tokenId);
        }
    }

    function _addTokenToBakery(uint256 _tokenId, address _owner) internal {
    ...
    function _removeUpgradeFromBakery(uint256 _tokenId) internal {
        address owner = stakes[_tokenId].owner;
        delete stakes[_tokenId];
        _removeStakeFromOwnerEnumeration(owner, _tokenId);
    }

    function _moveBakerToCooldown(uint256 _tokenId) internal {
        address owner = stakes[_tokenId].owner;
    ...
    function _removeBakerFromCooldown(uint256 _tokenId) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                tokenId: tokenId,
                level: baker.revealedTokenLevel(tokenId),
                stakeStart: stakes[tokenId].startTimestamp
            });
        }
