...
/// @title Vote Escrow KLO Token - veKLO
/// @author KLO
/// @notice Infinite supply, used to receive extra farming yields and voting power
contract veKLOToken is VeERC20("VeKLOToken", "veKLO"), Ownable {
    /// @dev Creates `_amount` token to `_to`. Must only be called by the owner (veKLOStaking)
    ...
    /// @dev Destroys `_amount` tokens from `_from`. Callable only by the owner (veKLOStaking)
