...
contract AmbrosiaRedeemer is Ownable {
	...
	constructor (
	address _AmbrosiaTreasury,
    address _AMB
	){
	AT = _AmbrosiaTreasury;
    AMB =_AMB;
    enabled=false;
    }
    ...
    function redeem(uint _amount) external{
        ...
        for (uint i=0;i<deemToke.length;i++) {
            ...
            uint balance = IERC20(deemToke[i].token).balanceOf(address(AT));
            uint xamount = portion.mul(balance).mul(1000000-tax).div(1000000000).div(10000000);
            ITreasury(AT).redeem(deemToke[i].token, xamount, msg.sender);
        }
