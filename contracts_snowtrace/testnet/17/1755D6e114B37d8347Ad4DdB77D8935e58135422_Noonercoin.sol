...
contract Noonercoin is ERC20{
   ...
   uint256 public mintingRateNoonerCoin;
   uint256 public mintingRateNoonerWei;
   uint256 public lastMintingTime;
   address private adminAddress;
   bool public isNewCycleStart = false;
   uint8[] private __randomVariable = [150, 175, 200, 225, 250];
   uint8[] private __remainingRandomVariable = [150, 175, 200, 225, 250];
   uint8[] private tempRemainingRandomVariable;
   mapping (uint256 => uint256) private occurenceOfRandomNumber;
   uint256 private weekStartTime = now; 
   mapping (address => uint256) private noonercoin;
   mapping (address => uint256) private noonerwei;
   ...
   uint256 private _cycleTime = 86400;
   uint256 private _fundersAmount;
   uint256 private _randomValue;
   uint256 private randomNumber;
   int private count = 0; 
   uint256 private previousCyclesTotalTokens = 0;
   uint256 private previousCyclesTotalWei = 0;
   uint256 private indexs = 1;
   uint256[] public randomVariableArray;
   uint256[] public previousCyclesBalance;
   uint256[] private previousCyclesWeiBalance;
   uint256 public weiAmountAdded = 0;
   uint256 public weiAmountAddedAtBurn = 0;
   uint256 public signmaValueWei = 0;
   uint256 public currentMintingRateTotalTokens = 0;
   uint256 public totalMintedTokens = 0;
   uint256 public weiToBurned = 0;
   uint256 public totalWeiInAdminAcc = 0;
   uint256[] public previousSigmaValues;
   uint256[] public previousBurnValues;
   bool public stopTheMinting =  false;
   ...
   function mintToken(address add, uint256 currentTime) public returns (bool) {  //admin only
      ...
      if(isNewCycleStart) {
       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            uint256 nowTime = now;
            uint256 totalOccurences = getCurrentOcuurenceTotal();
            ...
            lastMintingTime = currentTime;//block.timestamp;
            ...
            if(timeDiff >= _cycleTime){  
                //a checking conditions if all tokens minted are equal or not
                ...
                uint256 adminBalanceTotal = noonercoin[adminAddress] + transferAmount;
                if(adminBalanceTotal != tokens) {
                    if(adminBalanceTotal > tokens) {
                        uint256 removeDiff = adminBalanceTotal - tokens;
                        noonercoin[adminAddress] = noonercoin[adminAddress] - removeDiff;
                    }
                    if(adminBalanceTotal < tokens) {
                        uint256 addDiff = tokens - adminBalanceTotal;
                        noonercoin[adminAddress] = noonercoin[adminAddress] + addDiff;
                    }
                }  
                ...
                if(totalCycleLeft == 0) {
                    stopTheMinting = true;
                    totalMintedTokens = (noonercoin[adminAddress] + transferAmount -_fundersAmount)*10**18 + noonerwei[adminAddress] + totalWeiBurned;
                }
                else {
    ...
    function burnToken() internal returns(bool){
        ...
        //signmaValueWei = convertWeiToToken(signmaValueWei);
        uint256 adminBalance = noonercoin[adminAddress] + transferAmount;      
        ...
        currentMintingRateTotalTokens = convertWeiToToken(currentMintingRateTotalTokens);
        totalMintedTokens =  (adminBalance-_fundersAmount )*10**18 + noonerwei[adminAddress] + totalWeiBurned; //before adding totalWeiBurned.
        weiToBurned = _totalSupply*10**18 - signmaValueWei - totalMintedTokens - currentMintingRateTotalTokens - totalWeiBurned;
        totalWeiInAdminAcc = (adminBalance-_fundersAmount ) * 10**18 + noonerwei[adminAddress];
        ...
        if(totalWeiInAdminAcc > weiToBurned) {
            weiToBurned = convertWeiToToken(weiToBurned);
            remainingWei = totalWeiInAdminAcc - weiToBurned;
            noonercoin[adminAddress] =  _fundersAmount  + (remainingWei/10**18) - transferAmount;
            uint256 weiAddedInAdminAccount  = remainingWei -  (noonercoin[adminAddress] + transferAmount - _fundersAmount) * 10**18;
            ...
            for(indexs=1;indexs<=1;indexs++) {
                previousCyclesTotalTokens = noonercoin[adminAddress] + transferAmount;
                previousCyclesTotalWei = remainingWei -  (noonercoin[adminAddress] + transferAmount - _fundersAmount ) * 10**18;
    ...
    function changeConfigVariable() public returns (bool){
        require(msg.sender == adminAddress, "Only owner can do this");
        if(stopTheMinting || totalCycleLeft == 0) {
            return false;
        }
        else {
            //a checking conditions if all tokens minted are equal or not
  ...
  function getLastMintingTimeAndStartTimeDifference() public view returns (uint256) {
    ...
    function checkMissingTokens(address add) public view returns (uint256, uint256, uint256, bool) {
      uint256 adminBalance = noonercoin[add] + transferAmount;//admin bal 
      uint256 adminBalanceinWei = noonerwei[add];//admin bal wei
     
      if (lastMintingTime == 0) {
          return (0,0, 0, stopTheMinting);
      }
      if (lastMintingTime != 0) {
        ...
        return (checkDifference, weiVariance, weekStartTime, stopTheMinting); 
       }
    }
    ...
    function getCurrentOcuurenceTotal() public view returns(uint256){
    ...
    function getCurrentBurnValues(address add) public view returns(uint256 sigmaValue, uint256 currentTotalTokens, uint256 totalTokens, uint256 BurnTokens, uint256 totalTokenInAdmin, uint256) {
        uint256 callOfAdminAddress = noonercoin[add];
        return(signmaValueWei, currentMintingRateTotalTokens, totalMintedTokens, weiToBurned, totalWeiInAdminAcc, callOfAdminAddress);
    }
    ...
    function getBurnedValuesArray() public returns(bool) {
        isNewCycleStart = false;
        noonerwei[adminAddress] = 0;
        for(indexs=1;indexs<=1;indexs++) {
            previousCyclesTotalTokens = noonercoin[adminAddress] + transferAmount;
    ...
    function convertWeiToToken(uint256 value) internal pure returns(uint256) {
        uint256 temp = value/10**18;
        uint256 weiFromValue = value - (temp * 10**18);
        uint256 returnValue = (value/10**18 ) * 10**18;
        if(weiFromValue >= (10**18/2)) {
            returnValue = (value/10**18 + 1) * 10**18;
        } 
        return returnValue;
    }
