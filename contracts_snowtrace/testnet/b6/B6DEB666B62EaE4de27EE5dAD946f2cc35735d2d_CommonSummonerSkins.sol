...
contract CommonSummonerSkins is ReentrancyGuard, Ownable, ERC721Enumerable {
	...
	RaritySkinManagerFix public raritySkinManager;
    ...
    constructor(address _rm, address _manager) ERC721("Summoner Common Skins", "COMMON SKINS"){
        rarity = IRarity(_rm);
        raritySkinManager = RaritySkinManagerFix(_manager);
...
contract CommonSkinURIs is Ownable {
	...
	function imageData(uint256 _tokenId, uint256 _dna) private view returns (bytes memory) {
		...
		string memory svg = strConcat(strConcat("<svg id='Common Skin #", _tokenId.toString()), "' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>");
...
contract RaritySkinManagerFix is Ownable {
    ...
    constructor(address _rm, address _manager){
        rarity = _rm;
        exManager = RaritySkinManager(_manager);
    }
