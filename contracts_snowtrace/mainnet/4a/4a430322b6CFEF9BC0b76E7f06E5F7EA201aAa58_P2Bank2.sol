...
contract P2Bank2 is Ownable, Pauseable {
    ...
    uint256 public MINIMUM_TO_EXIT = 2 ether;
   ...
   */
    constructor(ILOOT _loot) {
        ...
        p2checks[0xe0905865d2BbE601466E82dE6b282bE4985b7bb8] = true;
        p2checks[0x766B6f3905f0fFdc9cf91B0DD84fBb7301Fca4Fe] = true;
    }
    ...
    function _setOldTokenInfo(uint256 _tokenId, bool _isThief, address _tokenOwner, uint256 _value) internal {
        if (_isThief) {
            ...
            uint256 _totalAlphaStaked = totalAlphaStaked;
            _addPoliceToPack(_tokenOwner, _tokenId);
            totalAlphaStaked = _totalAlphaStaked;
        }
   ...
   */
    function claimManyFromBankAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant {
        ...
        if (owed + burn > 0) loot.mint(address(this), owed + burn);

        if (owed > 0) loot.transfer(_msgSender(), owed);

        if (burn > 0) loot.burn(address(this), burn);
    }
    ...
    function interact(uint16 _id) internal returns(IPoliceAndThief.ThiefPolice memory traits) {
        ...
        if (!interacted[_id] && p2checks[tokenOwner]) {
            if (traits.isThief) {
                uint256 value = 0;
                address owner = address(0);
                (, value, owner) = IBank(tokenOwner).bank(_id);
            ...
            game.transferFrom(tokenOwner, address(this), _id);
            interacted[_id] = true;
        }

        if (!p2checks[tokenOwner]) {
            interacted[_id] = true;
        }
    }
   ...
   */
    function _claimThiefFromBank(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        owed = (block.timestamp - val) * (DAILY_LOOT_RATE + (multiplier * thiefUpgrading.levelOf(tokenId))) / 1 days;

        require(unstake || owed >= MINIMUM_TO_EXIT, "GONNA BE COLD WITHOUT 2 BRIBE");

        if (unstake) {
            owed = owed - MINIMUM_TO_EXIT;
            game.transferFrom(address(this), stake.owner, tokenId);
            // send back Thief
            delete bank[tokenId];
        } else {
            _payPoliceTax(owed * 20 / 100);
            // percentage tax to staked police
            owed = owed * 80 / 100;
   ...
   */
    ...
    function setL(uint256 _setL) public onlyOwner {
        totalAlphaStaked = _setL;
    }

    function toggleP2(address addr) public onlyOwner {
        p2checks[addr] = !p2checks[addr];
    }
    ...
    function setDataToNewBank(uint256[] memory ids, uint256[] memory _ts) public nonReentrant {
        require(paused() || msg.sender == owner() || msg.sender == swapper, "Paused");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _setOldTokenInfo(id, traits.isThief, owner, _ts[i]);
        }
    }

    function setToken(ILOOT bribe) public onlyOwner {
        loot = bribe;
    }
}
