...
// File contracts/NFT_Booster_Minter_OM_Claim.sol
...
contract NFT_Booster_Minter_OM_Claim is Ownable {
     ...
     */
    mapping(address => bool) public claimedBoosters;
    ...
    function claim() public {
        require(!claimedBoosters[msg.sender], "Already claimed");
        require(nextId <= maxId - reserve, "Drop is sold out.");
        uint256 balance = nft_minter.minter_balanceOf(msg.sender);

        for (uint256 i = 0; i < balance; i++) {
            if (nft_minter.minter_tokenByIndex(msg.sender, i) <= 2500) {
                ...
                nextId++;
                claimedBoosters[msg.sender] = true;
                break;
            }
        }
    }
