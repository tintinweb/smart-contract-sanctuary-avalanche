...
contract AVAXDTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAVAXD }
    ...
    address public sAVAXD;
    uint public sAVAXDQueue; // Delays change to sAVAXD address
    ...
    /**
        @notice allow approved address to deposit an asset for AVAXD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AVAXD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AVAXD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AVAXD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAVAXDERC20( AVAXD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAVAXD ).balanceOf( msg.sender ); // Can only borrow against sAVAXD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AVAXD
        @param _amount uint
     */
    function repayDebtWithAVAXD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAVAXDERC20( AVAXD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, AVAXD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AVAXD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AVAXD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AVAXD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AVAXD decimals
            value_ = _amount.mul( 10 ** IERC20( AVAXD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAVAXD ) { // 9
            sAVAXDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
