// Enjoy TOMB rewards. Only at YOBU.

// SPDX-License-Identifier: MIT
// https://t.me/yobu_avax
...
contract DividendDistributor is IDividendDistributor {
    ...
    IARC20 TOMB = IARC20(0xb84527D59b6Ecb96F433029ECc890D4492C5dCe1); //mainnet
    address WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7; //mainnet
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = TOMB.balanceOf(address(this));
        ...
        uint256 amount = TOMB.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            TOMB.transfer(shareholder, amount);
...
contract yobu is IARC20, Auth {
    using SafeMath for uint256;

    uint256 public constant MASK = type(uint128).max;
    address TOMB = 0xb84527D59b6Ecb96F433029ECc890D4492C5dCe1; //mainnet
    address public WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7; //mainnet

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEAD_NON_CHECKSUM = 0x000000000000000000000000000000000000dEaD;
    address public marketingFeeReceiver=0x4C4bdA644C284569a082255a46fadd9Dd16BfB28;

    string private _name;
    string private _symbol;
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1e11 * (10 ** _decimals); //100Billion
    uint256 public _maxTxAmount = _totalSupply.div(80); //1.25%
    ...
    uint256 sLiquidityFee = 1000;
    uint256 sReflectionFee = 1000;
    uint256 sMarketingFee = 1000;
    uint256 sTotalFee = 3000;
    ...
    constructor (
        ...
        address _wAVAX,
        string memory arc20_name,
        string memory arc20_symbol
    )   Auth(msg.sender) {
        _name = arc20_name;
        _symbol = arc20_symbol;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "Mempool snipers not allowed");
		...
		if(!_isFree[recipient]){ checkTxLimit(sender, amount); }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountAVAXReflection = amountAVAX.mul(sReflectionFee).div(totalAVAXFee);
        uint256 amountAVAXMarketing = amountAVAX.mul(sMarketingFee).div(totalAVAXFee);
    ...
    function Sweep() external onlyOwner {
     ...
     function rescueAnyARC20Tokens(address _tokenAddr, address _to, uint _amount) external authorized {
        IARC20(_tokenAddr).transfer(_to, _amount);
    }
    ...
    function blacklist(address account, bool value) external authorized{
        _isBlacklisted[account] = value;
    }

    function bulkBlacklist(address[] memory accounts, bool value) external authorized{
        for(uint i=0; i < accounts.length; i++){ _isBlacklisted[accounts[i]] = value; } 
    }
    ...
    function setSellFees(uint256 _sLiquidityFee, uint256 _sReflectionFee, uint256 _sMarketingFee, uint256 _feeDenominator) external authorized {
        sLiquidityFee = _sLiquidityFee;
        sReflectionFee = _sReflectionFee;
        sMarketingFee = _sMarketingFee;
        sTotalFee = _sLiquidityFee.add(_sReflectionFee).add(_sMarketingFee);
        feeDenominator = _feeDenominator;
    }
