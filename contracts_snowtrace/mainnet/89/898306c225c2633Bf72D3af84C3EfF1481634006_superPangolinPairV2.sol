...
// File: contracts/superVaultV2/superTokenDebugV2.sol
...
contract superTokenDebugV2 is ERC20,ImportOracle,ReentrancyGuard {
    ...
    address public swapRouter = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    ...
    function swapOnDex(address token0,address token1,uint256 balance)internal{
        if(token0 == token1){
            return;
        }
        ...
        if (token0 == address(0)){
            IJoeRouter01(swapRouter).swapExactAVAXForTokens{value : balance}(minOut,path,address(this),block.timestamp+30);
        }else{
            if (token1 == address(0)){
                IJoeRouter01(swapRouter).swapExactTokensForAVAX(balance,minOut,path,address(this),block.timestamp+30);
            }else{
                IJoeRouter01(swapRouter).swapExactTokensForTokens(balance,minOut,path,address(this),block.timestamp+30);
            }
        }
    }
    ...
    function _setReward(uint256 index,uint8 _reward,bool _bClosed,address _rewardToken,uint256 _sellLimit) internal virtual{
        if(index <rewardInfos.length){
            ...
            if(!_bClosed){
                if(_rewardToken != address(0) && IERC20(_rewardToken).allowance(address(this), swapRouter) == 0){
                    SafeERC20.safeApprove(IERC20(_rewardToken), swapRouter, uint(-1));
                }
            }else{
                if(_rewardToken != address(0)){
                    SafeERC20.safeApprove(IERC20(_rewardToken), swapRouter, 0);
                }
            }
        }else{
...
// File: contracts/interface/IMiniChefV2.sol
...
// File: contracts/superVaultV2/superPangolinPairV2.sol
...
//
// This contract handles swapping to and from superToken.
contract superPangolinPairV2 is superTokenDebugV2 {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _dsOracle,address payable _FeePool,uint256 _poolId)
            superTokenDebugV2(multiSignature,origin0,origin1,_stakeToken,_dsOracle,_FeePool) {
        poolId = _poolId;
    ...
    function emergencyWithdraw()external isAuthorized {
