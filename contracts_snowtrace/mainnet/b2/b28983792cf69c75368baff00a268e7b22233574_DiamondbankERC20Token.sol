...
contract DiamondbankERC20Token is IERC20 {
    ...
    string constant private _name = "DiamondBank";
    string constant private _symbol = "DB";
    ...
    address payable public DiamondbankTreasury = payable(0x34E872CB56C3f69f9Ba3dEEdBc494C235b90beee);
    address payable private rewardsPool = payable(0xc0fe72Ce371fbe0e8fC051db4fdfB06615A02817);
    address payable public DiamondbankAvaxBondDepository = payable(0x9dD7b06e4718B23Bd213A2f6D94f8eF6dDD14bA2);
    address payable public DiamondbankBondingCalculator = payable(0x4184F1221954e6799B7f69CC5FDbB118Da3f4CF0);

    uint256 private _maxTxAmount = (_tTotal * 50) / 100;
    ...
    IERC20 MANSION = IERC20(0xc0fe72Ce371fbe0e8fC051db4fdfB06615A02817);
...
//===============================================================================================================
    ...
    function setWallets(address payable marketing, address payable treasury, address payable rewards) external onlyOwner {
        DiamondbankTreasury = payable(marketing);
        DiamondbankAvaxBondDepository = payable(treasury);
    ...
    function getWhaleFee(address from) public view returns (uint256) {
        ...
        if(MANSION.balanceOf(from) >= 20 &&
            MANSION.balanceOf(from) < 50 ){return vbuy1;}
        if(MANSION.balanceOf(from) >= 50 &&
            MANSION.balanceOf(from) < 100 ){return vbuy1;}
        if(MANSION.balanceOf(from) >= 100) {return vbuy1;}
        else{
            return 0;
        }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if(address(this).balance > 0){
            ...
            DiamondbankAvaxBondDepository.transfer((amountAVAX * _ratios.treasury) / (_ratios.treasury + _ratios.marketing));
            DiamondbankTreasury.transfer(address(this).balance);
        }
    }
