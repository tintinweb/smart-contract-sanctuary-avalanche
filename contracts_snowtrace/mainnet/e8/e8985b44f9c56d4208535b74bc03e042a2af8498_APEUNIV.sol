...
{
    address public immutable APEUNIVManager;

    modifier onlyAPEUNIVManager() {
        address sender = _msgSender();
        require(
            sender == address(APEUNIVManager),
            "Implementations: Not APEUNIVManager"
        );
        _;
    }

    constructor(address _APEUNIVManager) ERC20("APE UNIVERSE", "ApeU") {
        require(
            _APEUNIVManager != address(0),
            "Implementations: APEUNIVManager is not set"
        );
        APEUNIVManager = _APEUNIVManager;
        ...
        onlyAPEUNIVManager
    {
        ...
        // effectively protecting APEUNIVManager from suspicious addresses
        ...
        onlyAPEUNIVManager
    {
        require(
            ...
            "APEUNIV: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyAPEUNIVManager {
        require(
            address(liquidityPoolManager) != address(0),
            "APEUNIV: LiquidityPoolManager is not set"
        );
