...
interface ITRILLERC20 is IERC20Mintable, IERC20 {
...
contract TrillestTreasury is Ownable {
    ...
    ITRILLERC20 public immutable Trillest;
    ...
    constructor (
        ...
        Trillest = ITRILLERC20(_Trillest);
    ...
    /**
        @notice allow approved address to deposit an asset for Trillest
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Trillest needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Trillest.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Trillest for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Trillest.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = sTRILL.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Trillest
        @param _amount uint
     */
    function repayDebtWithTime( uint _amount ) external {
        ...
        Trillest.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Trillest), _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        Trillest.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Trillest.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Trillest valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Trillest decimals
            value_ = _amount.mul( 10 ** Trillest.decimals() ).div( 10 ** IERC20( _token ).decimals() );
