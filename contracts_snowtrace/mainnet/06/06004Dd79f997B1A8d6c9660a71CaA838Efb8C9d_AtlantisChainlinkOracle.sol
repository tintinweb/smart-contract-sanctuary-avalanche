...
contract AtlantisChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(AToken aToken) public view returns (uint) {
       ...
       if (compareStrings(symbol, "aAVAX")) {
    ...
    function setATLPrice(address asset, uint price) external onlyGuardian() {
        emit PricePosted(asset, prices[asset], price, price);
        prices[asset] = price;
    }
    ...
    function setGuardian(address newGuardian) external onlyAdmin() {
        ...
        emit NewAdmin(oldGuardian, newGuardian);
    }
    ...
    modifier onlyGuardian() {
      require(msg.sender == guardian || msg.sender == admin, "only admin or guardian may call");
      _;
    }
}
