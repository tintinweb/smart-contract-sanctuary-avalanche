...
import {Bytes} from "./lib/LibBytes.sol";
...
contract Router is Ownable, IRouter {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < chunks; i++) {
            amountsFormatted[i] = Bytes.bytesToUint256(
                i * 32 + 32,
                _amounts
            );
        }
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < chunks; i++) {
            addressesFormatted[i] = Bytes.bytesToAddress(
                i * 32 + 32,
                _addresses
            );
        }
    ...
    function _findBestPathWithGas(
        ...
        if (_maxSteps>1 && _queries.adapters.length/32<=_maxSteps-2) {
            ...
            for (uint256 i=0; i<TRUSTED_TOKENS.length; i++) {
                ...
                address tokenOut = Bytes.bytesToAddress(newOffer.path.length, newOffer.path);
                uint256 amountOut = Bytes.bytesToUint256(newOffer.amounts.length, newOffer.amounts);
