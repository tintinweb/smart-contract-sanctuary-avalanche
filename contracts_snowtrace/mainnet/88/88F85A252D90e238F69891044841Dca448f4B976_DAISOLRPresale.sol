...
contract DAISOLRPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount DAI deposited by user
        uint256 debt; // total SOLR claimed by user
        bool claimed; // True if a user has claimed SOLR
    }
    ...
    uint256 public price = 1 * 10**(decimalsDAI - 2); // 0.01 DAI per SOLR

    uint256 public maxAllocation = 100 * 10**decimalsDAI; // 100 DAI cap per whitelisted user

    uint256 public totalRaisedDAI; // total DAI raised by sale

    uint256 public totalDebt; // total SOLR owed to users

    bool public started; // true when sale starts

    bool public ended; // true when sale ends

    bool public claimable; // true when SOLR is claimable

    bool public claimAlpha; // true when aSOLR is claimable
    ...
    mapping(address => uint256) public SOLRClaimable; // amount of SOLR claimable by address
    ...
    // @notice lets users claim SOLR
    // @dev send sufficient SOLR before calling
     ...
     *  @param _amount: amount of DAI to deposit to sale
     */
    function deposit(uint256 _amount) external checkIfPaused {
        require(started, 'Sale has not started');
        require(!ended, 'Sale has ended');
        require(whitelistedAdrs[msg.sender] == true, 'msg.sender is not whitelisted user');
        ...
        uint256 payout = _amount.mul(10**decimalsSOLR).div(price); // aSOLR to mint for _amount
        totalDebt = totalDebt.add(payout);

        DAI.safeTransferFrom(msg.sender, address(this), _amount);
            
        IMintable(address(aSOLR)).mint(address(msg.sender), payout);

        emit Deposit(msg.sender, _amount);
    }

    /**
     *  @notice it deposits aSOLR to withdraw SOLR from the sale
     *  @param _amount: amount of aSOLR to deposit to sale
     */
    function withdraw(uint256 _amount) external checkIfPaused {
        require(claimable, 'SOLR is not yet claimable');
        ...
        aSOLR.safeTransferFrom( msg.sender, address(this), _amount );

        IMintable(address(SOLR)).mint(msg.sender, _amount);

        emit Withdraw(address(SOLR), msg.sender, _amount);
    }
    ...
    function addLiquidityAfterPresale(uint256 SOLRTokenAmount) onlyOwner external {
        uint balanceDAI = DAI.balanceOf(address(this));
        IMintable(address(SOLR)).mint(address(this), SOLRTokenAmount);
        SOLR.approve(address(router), SOLRTokenAmount);
        DAI.approve(address(router), balanceDAI);
        router.addLiquidity(address(SOLR), address(DAI), SOLRTokenAmount, balanceDAI, 1, 1, dao, block.timestamp + 1);
    }
