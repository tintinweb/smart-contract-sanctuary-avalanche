...
contract SeleniumToken is ERC20('SELENIUM', 'SE') {
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSeleniumPerShare;    
        uint16 depositFeeBP;
        uint256 lpSupply;
    }
    
    SeleniumToken public immutable selenium;
    
    uint256 public startTime;
    address public feeAdd;
    address public treasuryAdd;
    
    uint256 private immutable SELENIUMMAXSUPPLY;
    uint256 public seleniumPerSec = 0.007 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 seleniumPerSec);

    constructor(SeleniumToken _selenium, uint256 _startTime, address _feeAdd, address _treasuryAdd) {
        selenium = _selenium;
        startTime = _startTime;
        feeAdd = _feeAdd;
        treasuryAdd = _treasuryAdd;
        SELENIUMMAXSUPPLY  = _selenium.maxSupply();
    }
    ...
    function safeSeleniumTransfer(address _to, uint256 _amount) internal {
        uint256 seleniumBalance = selenium.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > seleniumBalance) {
            transferSuccess = selenium.transfer(_to, seleniumBalance);
        } else {
            transferSuccess = selenium.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTerraTransfer: Transfer failed");
    }
    
    function updateEmissionRate(uint256 _seleniumPerSec) external onlyOwner {
        require(_seleniumPerSec >= MIN_PERSEC && _seleniumPerSec <= MAX_PERSEC, "Bad Emissions per sec");
        massUpdatePools();
        seleniumPerSec = _seleniumPerSec;
        emit UpdateEmissionRate(msg.sender, _seleniumPerSec);
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (selenium.totalSupply() >= SELENIUMMAXSUPPLY) { 
            return 0; 
        }
        return _to - _from;
    }
    ...
    function pendingSelenium(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardperSec && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 seleniumReward = (multiplier * seleniumPerSec * pool.allocPoint) / totalAllocPoint;
            accSeleniumPerShare += seleniumReward * 1e18 / pool.lpSupply;
        }
        return ((user.amount * accSeleniumPerShare) / 1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 seleniumReward = (multiplier * seleniumPerSec * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = seleniumReward / 10;
        uint256 SeleniumCalc = selenium.totalSupply() + seleniumReward + toTreasury;
        
      if (SeleniumCalc <= SELENIUMMAXSUPPLY) {
            selenium.mint(treasuryAdd, toTreasury);
            selenium.mint(address(this), seleniumReward);
        } else {
            seleniumReward = SELENIUMMAXSUPPLY - selenium.totalSupply();
            selenium.mint(address(this), seleniumReward);
        }
        
        pool.accSeleniumPerShare += seleniumReward * 1e18 / pool.lpSupply;
        pool.lastRewardperSec = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accSeleniumPerShare) / 1e18) - user.rewardDebt;
        ...
        user.rewardDebt = (user.amount * pool.accSeleniumPerShare) / 1e18;
