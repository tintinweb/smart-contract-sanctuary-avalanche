...
// File: distribution/CreamRewardPool.sol
...
contract CreamRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. CREAMs to distribute in the pool.
        uint256 lastRewardTime; // Last time that CREAMs distribution occurred.
        uint256 accCreamPerShare; // Accumulated Creams per share, times 1e18. See below.
    ...
    uint256[3] public epochCreamPerSecond;
    ...
    constructor(address _cream, uint256 _poolStartTime) public {
        ...
        epochCreamPerSecond[0] = epochTotalRewards[0].div(4 days);
        epochCreamPerSecond[1] = epochTotalRewards[1].div(5 days);

        epochCreamPerSecond[2] = 0;
        operator = msg.sender;
    }
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "CreamRewardPool: caller is not the operator"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        return _toTime.sub(_fromTime).mul(epochCreamPerSecond[0]);
    }
