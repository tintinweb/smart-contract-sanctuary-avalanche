...
// File contracts/CompletionCheckers/FullMintCompletionChecker.sol
...
/**
 * @title FullMintCompletionChecker
 * @author 0xLostArchitect
 *
 * @notice Checks if the LostWorldBase is fully minted
 */
contract FullMintCompletionChecker is UUPSUpgradeableBase, ICompletionChecker {
    /**
     * @notice Initiailization function called from the Proxy pattern.
     */
    ...
    /**
     * @notice Check if the LostWorld at `address_` is fully minted
     *
     * @param address_ - Address of the LostWorld
     * @return bool - Indicating whether the LostWorld has achieved full mint
     */
    function checkCompletion(address address_, bytes calldata)
        external
        view
        returns (bool)
    {
        return ILostWorldBase(address_).isFullyMinted();
    }
}