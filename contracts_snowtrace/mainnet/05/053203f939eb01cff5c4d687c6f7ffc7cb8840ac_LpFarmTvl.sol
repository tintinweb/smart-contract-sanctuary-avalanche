...
contract LpFarmTvl {
    ...
    address public melt = 0x026187BdbC6b751003517bcb30Ac7817D5B766f8;//H2O ADDRESS
    uint256 constant TLVMUL = 10**2;

    constructor(address _curveLpFarmAddress,address _oracle) public {
    ...
    function getTvl()
        ...
        returns (uint256)
    {
        uint256 meltAmount = IERC20(melt).balanceOf(curveLpFarmAddress);
        uint256 meltPrice = IOracle(oracleAddress).getPrice(melt);
        return meltAmount.mul(meltPrice).mul(TLVMUL).mul(2).div(10**36);
    }
