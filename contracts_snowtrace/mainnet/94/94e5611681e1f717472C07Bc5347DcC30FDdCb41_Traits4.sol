...
contract Traits4 is Ownable {
    ...
    constructor(ITraits _old, address _policeAndThief) {
        old = _old;
   ...
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IPoliceAndThief.ThiefPolice memory s = IPoliceAndThief(policeAndThief).getTokenTraits(tokenId);
    ...
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        if (s.isThief) {
            traits = string(abi.encodePacked(
                    attributeForTypeAndValue(traitTypes[1], traitData[0][s.uniform % traitCountForType[0]].name), ',',
                    attributeForTypeAndValue(traitTypes[2], traitData[1][s.hair % traitCountForType[1]].name), ',',
                    attributeForTypeAndValue(traitTypes[3], traitData[2][s.facialHair % traitCountForType[2]].name), ',',
                    attributeForTypeAndValue(traitTypes[4], traitData[3][s.eyes % traitCountForType[3]].name), ',',
                    attributeForTypeAndValue(traitTypes[6], traitData[4][s.accessory % traitCountForType[4]].name), ',',
                    attributeForTypeAndValue(traitTypes[5], traitData[5][s.headgear % traitCountForType[5]].name), ',',
                    attributeForTypeAndValue("Level", toString(uint256(level))), ','
                ));
        } else {
            traits = string(abi.encodePacked(
                    attributeForTypeAndValue(traitTypes[0], traitData[10][s.uniform % traitCountForType[10]].name), ',',
                    attributeForTypeAndValue(traitTypes[2], traitData[11][s.hair % traitCountForType[11]].name), ',',
                    attributeForTypeAndValue(traitTypes[3], traitData[12][s.facialHair % traitCountForType[12]].name), ',',
                    attributeForTypeAndValue(traitTypes[4], traitData[13][s.eyes % traitCountForType[13]].name), ',',
                    attributeForTypeAndValue(traitTypes[6], traitData[14][s.accessory % traitCountForType[14]].name), ',',
                    attributeForTypeAndValue(traitTypes[5], traitData[15][s.alphaIndex % traitCountForType[15]].name), ',',
                    attributeForTypeAndValue(traitTypes[7], traitData[16][s.neckGear % traitCountForType[16]].name), ',',
        ...
        return string(abi.encodePacked(
                ...
                tokenId <= IPoliceAndThief(policeAndThief).getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
   ...
   */
    ...
    function moveData(uint8 id, uint8 startFrom, uint8 till, bool counts) public onlyOwner {
        for(uint8 i = startFrom; i < till; i++) {
            if (bytes(traitData[id][i].name).length != 0) {
                continue;
            }

            (traitData[id][i].name, traitData[id][i].png) = old.traitData(id, i);
        }
