...
contract TrillestBondDepository is Ownable {
    ...
    IERC20 public immutable Trillest; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Trillest when receives principle
    ...
    struct Bond {
        uint payout; // Trillest remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Trillest ( underflow protection )
        ...
        uint balanceBefore = Trillest.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Trillest
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Trillest.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Trillest.balanceOf(address(this)), "Not enough Trillest to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Trillest.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Trillest.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Trillest.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Trillest.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Trillest supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Trillest.totalSupply();
    ...
    /**
     *  @notice calculate amount of Trillest available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Trillest) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Trillest, "NAT" );
