...
import "../../interfaces/moremoney/IVestingStakingRewards.sol";
import "../../interfaces/curve/ICurveSwap.sol";
import "../Common/StratManager2.sol";
import "../Common/FeeManager2.sol";
import "../../utils/StringUtils.sol";

contract StrategyMoreMoneyCurve is StratManager2, FeeManager2 {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IVestingStakingRewards(chef).stake(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IVestingStakingRewards(chef).withdraw(_amount.sub(wantBal));
    ...
    function managerHarvest() external onlyManager {
        IVestingStakingRewards(chef).withdrawVestedReward();
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        IERC20(output).safeTransfer(msg.sender, outputBal);
    }
    ...
    function addLiquidity() internal {
        ...
        if (depositBal > 1000) {
            ...
            ICurveSwap4(pool).add_liquidity(want, amounts, 0);
        }
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 _amount = IVestingStakingRewards(chef).balanceOf(address(this));
        return _amount;
    }
    ...
    function rewardsAvailable() public view returns (uint256) {
        ...
        bytes memory result = Address.functionStaticCall(chef, abi.encodeWithSignature(signature, address(this)));
    ...
    function retireStrat() external {
        ...
        IVestingStakingRewards(chef).exit();
    ...
    function _giveAllowances() internal {
        ...
        IERC20(depositToken).safeApprove(pool, uint256(-1));
    }
...
interface IVestingStakingRewards {
