 ...
 * Uniswap v2 router 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
 * testnet PCS router 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
 */
contract Shared {
    address constant public ROUTER = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
}
...
contract ProjectEuropa is ERC20, Ownable, Shared {
    ...
    address public marketingWalletAddress = 0xdf7b7493C4b7A4aCfc140834886E4BD3a70F84BE;
    address public devWalletAddress = 0xdf7b7493C4b7A4aCfc140834886E4BD3a70F84BE;
    ...
    event AccidentallySentAvaxWithdrawn (address indexed account, uint256 amount);
    ...
    constructor() ERC20("ProjectEuropa", "Orbit") {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Orbit: The router already has that address");
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner{
        require(marketingWalletAddress != address(0), "Orbit: Can't set marketing wallet to the zero address");
        marketingWalletAddress = wallet;
    }
    
    function setDevWallet(address payable wallet) external onlyOwner{
        require(devWalletAddress != address(0), "Orbit: Can't set marketing wallet to the zero address");
    ...
    function setMaxTxPermille(uint256 maxTxPermille) external onlyOwner {
        require (maxTxPermille > 0, ":Orbit Can't set max Tx to 0");
        maxTxAmount = totalSupply() * maxTxPermille / 1000;
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Orbit: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Orbit: Automated market maker pair is already set to that value");
    ...
    // Help users who accidentally send Avax to the contract address - this only removes Avax that has been manually transferred to the contract address
    // Avax that is created as part of the liquidity provision process will be sent to the PCS pair address immediately and so cannot be affected by this action
    function withdrawExcessAvax (address _account) external onlyOwner {
        uint256 contractAvaxBalance = address(this).balance;
        
        if (contractAvaxBalance > 0)
            payable(_account).sendValue(contractAvaxBalance);
        
        emit AccidentallySentAvaxWithdrawn (_account, contractAvaxBalance);
    }
    ...
    function _transfer(
        ...
        require (tradingOpened || isExcludedFromFees[from], "Orbit: Trading paused");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(isBlacklistedUntil[from] < block.timestamp && isBlacklistedUntil[to] < block.timestamp, "Orbit: Blacklisted address");
