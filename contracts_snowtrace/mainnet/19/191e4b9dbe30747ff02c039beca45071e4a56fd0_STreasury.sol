...
abstract contract ReentrancyGuard {
    ...
    // tsnake's contents, replace the bits taken up by the boolean, and then write
...
contract STreasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x375ae4D2650fbD91BaE4aEc13aaEe1f0D88a6375), // SnakeGenesisPool
        address(0x763e65965dA1D1BAA838d55445073723f2041E33) // new SnakeRewardPool
    ];

    // core components
    address public snake;
    address public bsnake;
    address public tsnake;
    ...
    modifier checkOperator {
        require(
            IBasisAsset(snake).operator() == address(this) &&
                ...
                IBasisAsset(tsnake).operator() == address(this) &&
    ...
    function getSnakePrice() public view returns (uint256 snakePrice) {
        try IOracle(snakeOracle).consult(snake, 1e18) returns (uint144 price) {
    ...
    function getSnakeUpdatedPrice() public view returns (uint256 _snakePrice) {
        try IOracle(snakeOracle).twap(snake, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult SNAKE price from the oracle");
        }
    ...
    function getBurnableSnakeLeft() public view returns (uint256 _burnableSnakeLeft) {
        ...
        if (_snakePrice <= snakePriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableSnake = _maxMintableBond.mul(_snakePrice).div(1e17);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_snakePrice > snakePriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalSnake.mul(1e17).div(_rate);
            }
        }
    }
    ...
    function initialize(
        ...
        snakePriceOne = 10**17; // This to allow a PEG of 10 SNAKE per AVAX
    ...
    function buyBonds(uint256 _snakeAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _snakeAmount.mul(_rate).div(1e17);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _snakeAmount = _bondAmount.mul(_rate).div(1e17);
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochSnakePrice > snakePriceCeiling) {
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForMasonry = snakeSupply.mul(_percentage).div(1e17);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = snakeSupply.mul(_percentage).div(1e17);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(bsnake), "bsnake");
        require(address(_token) != address(tsnake), "tsnake");
