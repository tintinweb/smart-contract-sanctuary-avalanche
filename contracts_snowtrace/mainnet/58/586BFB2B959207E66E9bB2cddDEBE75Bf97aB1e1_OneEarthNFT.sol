...
contract OneEarthNFT is ERC721Enumerable, Ownable {    
    ...
    uint [] public tierMaxMint = [0, 1500, 500, 500, 250, 13];      
    ...
    function getRandomNumber() internal view returns (uint256) {
        uint256 random = uint256(keccak256(abi.encodePacked(block.timestamp,gasleft(),msg.sender))) % 100; 
        if(random < 54){
            random = 1;
        } else if(random < 72){
            random = 2;
    ...
    function distributeRewards (uint256 tier) internal returns (uint256 total) {
        ...
        uint tier2Mint = tierMinted[2];
        uint tier3Mint = tierMinted[3];
        uint tier4Mint = tierMinted[4];
        uint tier5Mint = tierMinted[5];
    ...
    function claimReflections (uint256 [] memory tokensIds) external {
        ...
        for(uint256 i; i < tokensIds.length; i++){
            ...
            detailsByTokenId[tokensIds[i]].lastReflection = tokenReflectionRewards;
        }
    ...
    function getReflections(uint256 tokenId) public view returns (uint256 amount){
        amount = tokenReflectionRewards - detailsByTokenId[tokenId].lastReflection;
    }

    function getRewards(uint256 tokenId) public view returns (uint256 amount, uint256 tierRewards){    
    ...
    function getRewardsByTier(uint256 tier) internal view returns (uint256 amount){        
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory){
        ...
        return string(abi.encodePacked(tierBaseUri[tier], tierIndex.toString()));      
    }
    ...
    function setUris(string memory _tier1, string memory _tier2,string memory _tier3,string memory _tier4,string memory _tier5) external onlyOwner{
    ...
    function tokenReflections (uint256 amount) external {
        ...
        if(totalSupply() == 0){
            ...
            tokenReflectionRewards += (amount + nonDeliveredReflections) / totalSupply();
