...
/**
Crypto Invest Capital: $CIC

Tokenomics:
Total supply : 100000000000
5% of each sell/buy goes to existing holders.
3.75% of each sell/buy goes into cross-chain farming/investing to add to the treasury 
and buy back CIC tokens. With community vote, funds will be used for marketing or buy backs
1.25% of each sell/buy goes into the team wallet -  In case the marketing wallet grows too big, 50% of it will also be used as a farming/buyback fund.

Website:
https://www.cryptoinvestcapital.finance

Discord:
https://discord.gg/cryptoinvestcapital

Twitter:
https://twitter.com/CryptoInvestCap
...
contract CryptoInvestCapital is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Crypto Invest Capital";
    ...
    constructor(
        ...
        address payable CICWalletAddress
    ) {
        ...
        joeV2Pair = IJoeFactory(_uniswapV2Router.factory()).createPair(
            address(this),
            _uniswapV2Router.WAVAX()
        );
        IERC20(joeV2Pair).approve(address(joeV2Router), type(uint256).max);
    }
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 tax1 = amount.mul(75).div(100);
        w1.transfer(tax1);
        w2.transfer(amount.sub(tax1));
    }
    ...
    function setCICWallet(address payable _w2) external {
    ...
    function setTeamFee(uint256 team) external {
        ...
        require(
            team + _taxFee <= 12,
            "The sum of team and tax must less than 12%"
        );
        _teamFee = team;
    }
