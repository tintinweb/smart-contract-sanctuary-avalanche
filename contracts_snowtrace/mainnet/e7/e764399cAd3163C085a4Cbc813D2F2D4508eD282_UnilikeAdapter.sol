...
import {IUnilikeFactory} from "../interface/IUnilikeFactory.sol";
import {IUnilikePair} from "../interface/IUnilikePair.sol";
import {IERC20} from "../interface/IERC20.sol";
import {Adapter} from "../Adapter.sol";

import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

import {LibUnilike} from "../lib/LibUnilike.sol";
...
contract UnilikeAdapter is Adapter {
    ...
    uint256 public immutable FEE_COMPLIMENT;
    ...
    constructor(
        ...
        FACTORY = _factory;
        NAME = _name;
        FEE_COMPLIMENT = FEE_DENOMINATOR - _fee;
        ...
        returns (uint256 _amt)
    {
        ...
        address pair = LibUnilike.getPairAddress(FACTORY, _tokenIn, _tokenOut);
        if (pair == address(0)) {
            return 0;
        }

        (uint256 r0, uint256 r1) = LibUnilike.getReserves(pair);
        (uint256 reserveIn, uint256 reserveOut) = _tokenIn < _tokenOut
            ? (r0, r1)
            : (r1, r0);

        if (reserveIn > 0 && reserveOut > 0) {
            _amt = _getAmountOut(_amountIn, reserveIn, reserveOut);
        }
    ...
    function _swap(
        ...
        override
    {
        IUnilikePair pair = LibUnilike.getPair(FACTORY, _tokenIn, _tokenOut);
        (uint256 amount0Out, uint256 amount1Out) = (_tokenIn < _tokenOut)
            ? (uint256(0), _amountOut)
            : (_amountOut, uint256(0));
        IERC20(_tokenIn).safeTransfer(address(pair), _amountIn);
