...
contract tokenSwap is Ownable {
    ...
    function Initialize(address _SwapAddress, address _WETH, uint _SellRate, uint _allowedTaxRate) external onlyOwner {
    ...
    pathOut[1] = _tokenIn;
    if (_tokenIn == WETH) {
        uint256[] memory amountOut = IUniswapV2Router(SwapAddress).swapExactETHForTokens{value: _amountIn}(_amountOutMin, path, address(this), block.timestamp);
        ...
        IUniswapV2Router(SwapAddress).swapExactTokensForETH(sellamount, getamount, pathOut, address(this), block.timestamp);
    } else {
    ...
    if (_StableCoin == WETH) {
        IUniswapV2Router(SwapAddress).swapExactTokensForETH(_amountIn, _amountOutMin, pathOut, address(this), block.timestamp);
    } else {
   ...
   function buySupportingFee(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, uint deadline) external {
    ...
    if (_tokenIn == WETH) {
        ...
        uint256 sellamount = amountOut[1].mul(SellRate).div(10000);
        uint256 getamount = amountOut[0].mul(SellRate).div(10000).mul(unTaxed).div(100);
        IUniswapV2Router(SwapAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(sellamount, getamount, pathOut, address(this), block.timestamp);
    } else {
        uint256[] memory amountOut = IUniswapV2Router(SwapAddress).swapExactTokensForTokens(_amountIn, _amountOutMin, path, address(this), block.timestamp);
        uint256 sellamount = amountOut[1].mul(SellRate).div(10000);
        uint256 getamount = amountOut[0].mul(SellRate).div(10000).mul(unTaxed).div(100);
    ...
    pathOut[1] = _StableCoin;
    if (_StableCoin == WETH) {
        IUniswapV2Router(SwapAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(_amountIn, _amountOutMin, pathOut, address(this), block.timestamp);
    } else {
