...
contract Subscription is ISubscription{
    ...
    bool public _prorated_first_payment;
    uint256 public _prorated_first_payment_amount;
    ...
    event Subscription_Owner_Changed(
        uint256 changed_at,
        address new_payor_address, 
        string newName,
        string newEmail
    );

    constructor(string memory offer , address thepayor, address thepayee, uint256 interval, uint256 amount, string memory name, string memory email, bool prorated_first_payment, uint256 prorated_first_payment_amount){
        ...
        payor_email = email;
        _active = false;
        _prorated_first_payment = prorated_first_payment;
        _prorated_first_payment_amount = prorated_first_payment_amount;
        if(_prorated_first_payment){
                _MUG_owed = MuPrice.get_cost_In_MUG_USD(_prorated_first_payment_amount);
        }else{
    ...
    function changePayor(address newPayor, string memory newName, string memory newEmail) public virtual{
        require(msg.sender == payor, "only payor can change the payors");
        payor = newPayor;
        payor_name = newName;
        payor_email = newEmail;
        emit Subscription_Owner_Changed(block.timestamp, newPayor, newName, newEmail);
    }
...
contract SubscriptionFactory{
    ...
    function createSubscription(string memory offer , address thepayor, address thepayee, uint256 interval, uint256 amount, string memory name, string memory email, bool prorated_first_payment, uint256 prorated_first_payment_amount) public virtual{
    //
        require(msg.sender == _owner, "You're not the owner");
        Subscription s = new Subscription(offer, thepayor, thepayee, interval, amount, name, email, prorated_first_payment, prorated_first_payment_amount);
