...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  // for testnet: 0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8
...
contract SHIVA is ERC20, Ownable {
    ...
    //buy fee varable
    uint256 public sellBTCBRewardsFee = 8;
    ...
    uint256 public selltotalFees = sellBTCBRewardsFee.add(sellliquidityFee).add(sellmarketingFee).add(burnFee);
    ...
    /****************** set sell fee ***************************** */
    function setsellBTCBRewardsFee(uint256 value) external onlyOwner{
        selltotalFees = value.add(sellliquidityFee).add(sellmarketingFee).add(burnFee);
        require(selltotalFees <= 20, "SHIVA::setBTCBRewardsFee: Too high Fees");
        emit BTCBRewardsFeeUpdated(msg.sender, sellBTCBRewardsFee, value);
        sellBTCBRewardsFee = value;
    }

    function setsellLiquidityFee(uint256 value) external onlyOwner{
        selltotalFees = value.add(sellBTCBRewardsFee).add(sellmarketingFee).add(burnFee);
        require(selltotalFees <= 20, "SHIVA::setBTCBRewardsFee: Too high Fees");
        emit LiquidityFeeUpdated(msg.sender, sellliquidityFee, value);
        sellliquidityFee = value;
    }

    function setsellMarketingFee(uint256 value) external onlyOwner{
        selltotalFees = value.add(sellBTCBRewardsFee).add(sellliquidityFee).add(burnFee);
        require(selltotalFees <= 20, "SHIVA::setBTCBRewardsFee: Too high Fees");
        emit MarketingFeeUpdated(msg.sender, sellmarketingFee, value);
        sellmarketingFee = value;
    }

    function setsellBurnFee(uint256 value) external onlyOwner{
        selltotalFees = value.add(sellBTCBRewardsFee).add(sellliquidityFee).add(sellmarketingFee);
        require(selltotalFees <= 20, "SHIVA::setBTCBRewardsFee: Too high Fees");
        emit MarketingFeeUpdated(msg.sender, burnFee, value);
        burnFee = value;
    }
    //////////////////////////////////////////////////////////////////////
    ...
    function SwapEnabled() public view returns (bool) {
        return block.number > swapStartblock;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[to]){//sell
                fees = amount.mul(selltotalFees-burnFee).div(100);
