...
// Note that this pool has no minter key of SNAKE (rewards).
// Instead, the governance will call SNAKE distributeReward method and send reward to this pool at the beginning.
contract SnakeRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SNAKEs to distribute in the pool.
        uint256 lastRewardTime; // Last time that SNAKEs distribution occurred.
    ...
    uint256[3] public epochSnakePerSecond;
    ...
    constructor(address _snake, uint256 _poolStartTime) public {
        ...
        epochSnakePerSecond[0] = epochTotalRewards[0].div(4 days);
        epochSnakePerSecond[1] = epochTotalRewards[1].div(5 days);

        epochSnakePerSecond[2] = 0;
        operator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "SnakeRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SnakeRewardPool: existing pool?");
        }
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accSnakePerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
            totalAllocPoint = totalAllocPoint.add(_allocPoint);
        }
    }

    // Update the given pool's SNAKE allocation point. Can only be called by the owner.
        ...
        for (uint8 epochId = 2; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return _toTime.sub(_fromTime).mul(epochSnakePerSecond[epochId]);
                }

                uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(epochSnakePerSecond[epochId]);
                if (epochId == 1) {
                    return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochSnakePerSecond[0]));
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward.add(epochEndTimes[epochId].sub(_fromTime).mul(epochSnakePerSecond[epochId]));
                    }
                    _generatedReward = _generatedReward.add(epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochSnakePerSecond[epochId]));
                }
                return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochSnakePerSecond[0]));
            }
        }
        return _toTime.sub(_fromTime).mul(epochSnakePerSecond[0]);
    }
