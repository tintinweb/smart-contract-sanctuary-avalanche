...
contract AvaxSSOV is Pausable, ReentrancyGuard, INativeSSOV, ContractWhitelist {
    ...
    /// @notice Total epoch deposits for specific strikes including premiums
    /// @dev mapping (epoch => (strike => deposits))
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochStrikeBalance;

    /// @notice Total epoch deposits across all strikes including premiums
    /// @dev mapping (epoch => deposits)
    ...
    /// @notice Epoch avax balance per strike after accounting for rewards
    /// @dev mapping (epoch => (strike => balance))
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochStrikeAvaxBalance;

    /// @notice Epoch Joe token rewards
    /// @dev mapping (epoch => (strike => balance))
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochStrikeJoeRewards;
    ...
    event EmergencyWithdraw(address sender, uint256 avaxWithdrawn);
    ...
    constructor(
        ...
        require(_joe != address(0), 'E1');
        require(_javax != address(0), 'E1');
        require(_rewardDistributor != address(0), 'E1');
        ...
        addresses['JAVAX'] = _javax;
        addresses['JOE'] = _joe;
        addresses['RewardDistributor'] = _rewardDistributor;
        ...
        returns (bool)
    {
        ...
        emit EmergencyWithdraw(msg.sender, avaxBalance);
    ...
    /// @dev Updates the final epoch avax balances per strike of the vault
    function _updateFinalEpochBalances(bool accountPremiums) internal {
        IJAvax javax = IJAvax(getAddress('JAVAX'));
        IRewardDistributor rewardDistributor = IRewardDistributor(
            getAddress('RewardDistributor')
        );

        // Variable to track avax claimed after withdrawing and removing liquidity
        uint256 avaxClaimed;
        uint256 avaxRewards;
        uint256 joeRewards = IERC20(getAddress('JOE')).balanceOf(address(this));
        rewardDistributor.claimReward(0, payable(address(this)));
        joeRewards =
            IERC20(getAddress('JOE')).balanceOf(address(this)) -
            joeRewards;

        if (javax.balanceOf(address(this)) > 0) {
            avaxClaimed = address(this).balance;
            // Withdraw all tokens from previous epoch
            require(
                javax.redeemNative(javax.balanceOf(address(this))) == 0,
                'Redeem failed'
            );
            avaxClaimed = address(this).balance - avaxClaimed;
            avaxRewards = avaxClaimed - totalEpochBalance[currentEpoch];
        }
        ...
        if (totalEpochDeposits[currentEpoch] > 0) {
            ...
            for (uint256 i = 0; i < strikes.length; i++) {
                ...
                uint256 avaxRewardsPerStrike = (avaxRewards *
                    totalEpochStrikeBalance[currentEpoch][strikes[i]]) /
                    totalEpochBalance[currentEpoch];

                // Update final avax balances for epoch and strike
                totalEpochStrikeAvaxBalance[currentEpoch][strikes[i]] =
                    totalEpochStrikeDeposits[currentEpoch][strikes[i]] +
                    avaxRewardsPerStrike -
                    settlement;
                // update final joe rewards for epoch and strike
                totalEpochStrikeJoeRewards[currentEpoch][strikes[i]] =
                    (joeRewards *
                        totalEpochStrikeBalance[currentEpoch][strikes[i]]) /
                    totalEpochBalance[currentEpoch];

                if (accountPremiums) {
                    totalEpochStrikeAvaxBalance[currentEpoch][
                        strikes[i]
    ...
    function bootstrap() external onlyOwner whenNotPaused returns (bool) {
        ...
        for (uint256 i = 0; i < epochStrikes[nextEpoch].length; i++) {
            ...
            string memory name = concatenate('AVAX-CALL', strike.toString());
    ...
    /**
     * @notice Deposits AVAX into the ssov to mint options in the next epoch for selected strikes
        ...
        returns (bool)
    {
        ...
        _depositAvax(msg.value);
        return true;
    }

    /**
     * @notice Deposit AVAX multiple times into different strike
     * @param strikeIndices Indices of strikes to deposit into
     * @param amounts Amount of AVAX to deposit into each strike index
     * @param user Address of the user to deposit for
    ...
    function depositMultiple(
        ...
        // Check if AVAX amount sent is enough for depositing to each strike
        uint256 totalAmount;
        ...
        _depositAvax(totalAmount);

        return true;
    }

    /**
     * @notice Internal function to handle AVAX deposits
     * @param strikeIndex Index of strike
     * @param amount Amout of AVAX to deposit
     */
    function _deposit(
        uint256 strikeIndex,
        uint256 amount,
    ...
    /**
     * @notice Internal function to handle despositing into Trader Joe's
     * @param amount Amout of AVAX to deposit
     */
    function _depositAvax(uint256 amount) internal {
        // Deposit AVAX to Trader Joe's
        IJAvax(getAddress('JAVAX')).mintNative{value: amount}();
    }
        ...
        returns (uint256, uint256)
    {
        ...
        // Must have transfered enough avax to purchase
    ...
    /**
     * @notice Settle calculates the PnL for the user with the settlement price and withdraws the PnL in AVAX to the user. Will also the burn the option tokens from the user.
     * @param strikeIndex Strike index for current epoch
        ...
        returns (uint256[1] memory)
    {
        ...
        // Calculate amount of AVAX to transfer to user
        uint256 userAvaxAmount = (totalEpochStrikeAvaxBalance[withdrawEpoch][
            strike
        ] * userStrikeDeposits) /
            totalEpochStrikeDeposits[withdrawEpoch][strike];

        uint256 userJoeAmount = (totalEpochStrikeJoeRewards[withdrawEpoch][
            strike
        ] * userStrikeDeposits) /
            totalEpochStrikeDeposits[withdrawEpoch][strike];
        ...
        payable(msg.sender).transfer(userAvaxAmount);
        // Transfer Joe to user
        IERC20(getAddress('JOE')).safeTransfer(msg.sender, userJoeAmount);
    ...
    /// @param price price of AVAX
    /// @param strike strike price of the the AVAX option
    ...
    /**
     * @notice Returns AVAXs price in USD
     */
