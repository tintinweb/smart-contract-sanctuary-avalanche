...
contract TRILLTreasury is Ownable {
    ...
    address public immutable TRILL;
    ...
    address public sTRILL;
    uint public sTRILLQueue; // Delays change to sTRILL address
    ...
    //    isLiquidityToken[ _TRILLDAI ] = true;
    //    liquidityTokens.push( _TRILLDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for TRILL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TRILL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TRILL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TRILL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITRILL( TRILL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTRILL ).balanceOf( msg.sender ); // Can only borrow against sTRILL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TRILL
        @param _amount uint
     */
    function repayDebtWithTRILL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITRILL( TRILL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TRILL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TRILL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TRILL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TRILL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TRILL decimals
            value_ = _amount.mul( 10 ** IERC20( TRILL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sTRILL ) { // 9
            sTRILLQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
