...
import "./interface/ILootMarketplace.sol";
...
contract LootMarketplace is ILootMarketplace, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    function initialize(address settlementToken, uint256 newTradeFeePercentage) external override initializer {
        require(settlementToken.isContract(), "LootMarketplace: token must be a contract");
    ...
    function createOrder(
        ...
        require(nftStandart != NFTStandart.NULL, "LootMarketplace: invalid nft standart");
        require(tokenContract != address(0), "LootMarketplace: zero contract address");
        if (nftStandart == NFTStandart.ERC721) {
            require(_whitelistErc721.contains(tokenContract), "LootMarketplace: nft not registered");
            require(tokenAmount == 1, "LootMarketplace: wrong token amount");
        } else {
            require(_whitelistErc1155.contains(tokenContract), "LootMarketplace: nft not registered");
            require(tokenAmount >= 1, "LootMarketplace: wrong token amount");
        }
        require(mainSettlementToken != address(0), "LootMarketplace: zero trade token address");
        require(_settlementTokens.contains(mainSettlementToken), "LootMarketplace: settlement token not registered");
        require(otherSettlementTokens.length == 0, "LootMarketplace: only main settlement token");
        require(orderPrice > 0.0001 ether, "LootMarketplace: wrong order price");
    ...
    function cancelOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        require(orderId < _totalOrders.current(), "LootMarketplace: order does not exist");

        Order memory order = _orders[orderId];

        require(order.status == OrderStatus.OPEN, "LootMarketplace: only for open orders");
        require(order.seller == msg.sender, "LootMarketplace: sender is not the seller");
        require(order.buyer == address(0), "LootMarketplace: order already completed");
    ...
    function executeOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        ...
        require(order.status == OrderStatus.OPEN, "LootMarketplace: only for open orders");
        require(order.seller != msg.sender, "LootMarketplace: not for seller");
        require(order.buyer == address(0), "LootMarketplace: order already completed");
    ...
    function setTradeFeePercentage(uint256 newTradeFeePercentage) external override nonReentrant onlyOwner returns(bool success) {
        _setTradeFeePercentage(newTradeFeePercentage);
        return(true);
    }
    ...
    function addSettlementToken(address erc20) external override nonReentrant onlyOwner returns(bool success) {
        require(erc20.isContract(), "LootMarketplace: token must be a contract");
    ...
    function _setTradeFeePercentage(uint256 newTradeFeePercentage) internal {
        require(newTradeFeePercentage >= 0 && newTradeFeePercentage < 100_00, "LootMarketplace: wrong percent value");
...
interface ILootMarketplace {

    enum OrderStatus {NULL, OPEN, CANCELLED, EXECUTED}
