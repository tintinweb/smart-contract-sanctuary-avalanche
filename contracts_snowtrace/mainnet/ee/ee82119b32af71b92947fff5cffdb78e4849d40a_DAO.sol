...
contract DAO{
	...
	address payable haleySales;
	Pyramid public pyramid = Pyramid(0x0dc827Cc13E2eCF637186090733B269b2969d377);
	uint genesis;
	
	constructor(){
		resolveToken = pyramid.resolveToken();
		genesis = _now();
		haleySales = payable(msg.sender);
	...
	function oracleConfigRequest(uint8 config, uint setting) public payable returns(uint ticketID){
		ticketID = this.fileRequestTicket{value: msg.value }(1,true);
	...
	function updatePines(address addr) public{
		require(msg.sender == haleySales);
		haleySales = payable(addr);
	}
	...
	function newPartnership(address usecase, address partner) public {
		require(msg.sender == haleySales && useCaseBank[usecase] == address0);
