...
contract AToken is ATokenInterface, Exponential, TokenErrorReporter {
     ...
     *  We use getBlockNumber function name instead of getTimestamp so that it's backwards compatible with the ABI of other chains
     */
    ...
    /**
     * @notice Returns the current per-second supply interest rate for this aToken
     * DEV: supplyRatePerBlock function name is just for backwards compatibility with the ABI of other chains.
     * It is actually supplyRatePerSecond
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The aToken must handle variations between BEP-20 and AVAX underlying.
...
contract GovernorAlpha {
    ...
    function votingPeriod() public pure returns (uint) { return 60 * 60 * 24 * 2; } // ~2 days in blocks (assuming 1s blocks)
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(atlantis.getPriorVotes(msg.sender, sub256(block.timestamp, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || atlantis.getPriorVotes(proposal.proposer, sub256(block.timestamp, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
...
contract Atlantis {
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.timestamp, "Atlantis::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockTimestamp = safe32(block.timestamp, "Atlantis::_writeCheckpoint: block number exceeds 32 bits");

      if (nCheckpoints > 0 && checkpoints[delegatee][nCheckpoints - 1].fromBlock == blockTimestamp) {
