...
/*----------------------------------------------------*/
pragma solidity ^0.8.11;
contract Revelation is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function init(address _oracle) public{
        require( address(ORACLE) == address(0) && msg.sender == itsMarcusJams);
        ORACLE = Oracle(_oracle);
    }
    
    uint genesis;

    address itsMarcusJams;
    address parker = 0xAeA13531Ab3726086A02fD9254ee3dC37c876442;

    constructor(){
        itsMarcusJams = msg.sender;
        genesis = block.timestamp;
        _name = "Revelation";
        _symbol = "Its";
        _mint(itsMarcusJams, 0);
    ...
    function serverSoftwareIsReady() public {
        require(msg.sender == itsMarcusJams && HEAVY_TEXT);
        HEAVY_TEXT = false;
    }
