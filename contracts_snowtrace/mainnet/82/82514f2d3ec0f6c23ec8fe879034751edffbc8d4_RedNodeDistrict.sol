...
contract VaultOwned is Ownable {
  ...
  function setNodeManager( address vault_ ) external onlyOwner() returns ( bool ) {
    _vault = vault_;
...
contract RedNodeDistrict is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    constructor() ERC20("Red Node District", "PLAYMATE", 9) {
    }
    ...
    function _setNodeManagement(uint256 _maxBuyAmount) external onlyOwner() {
      maxBuyAmount = _maxBuyAmount;
    }

    function _setNodeEmission(uint256 _maxBuyAmount) external onlyOwner() {
      maxBuyAmount = _maxBuyAmount;
    }
