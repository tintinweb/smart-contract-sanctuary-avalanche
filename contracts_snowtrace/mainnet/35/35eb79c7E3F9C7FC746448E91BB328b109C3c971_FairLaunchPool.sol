...
contract FairLaunchPool is Ownable {
    ...
    // awards special discount to investor
    function adjustPriceTier(address _address, uint256 _priceTier) external onlyOwner {
    	InvestorInfo storage investor = investorInfoMap[_address];
    	investor.priceTier = _priceTier;
    }
    ...
    function invest(uint256 investAmount) public {
        ...
        uint256 xprice = price;
        
        if (investor.priceTier == 1){
        	xprice = 475;
        } else if (investor.priceTier == 2){
        	xprice = 450;
        } else if (investor.priceTier == 3){
        	xprice = 400;
        }

        //MAG decimals = 9, MIM decimals = 18
        uint256 issueAmount = investAmount * priceQuote / (xprice * 10 ** launchDecimals);
