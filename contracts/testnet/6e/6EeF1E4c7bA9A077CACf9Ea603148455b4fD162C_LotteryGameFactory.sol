...
contract LotteryGameFactory is ILotteryGameFactory, Ownable {
    ...
    /// @param bytecode creating code for the CustomLottery contract
    /// @param salt is a random number used to create an address
    /// @param game struct of info about the game
    /// @param dateTime DateTime contract address
    ...
    function createLottery(
        ...
        address lotteryGameAddress;
        assembly {
            lotteryGameAddress := create2(
                callvalue(),
                add(bytecode, 0x20),
                mload(bytecode),
                salt
    ...
    function deleteLottery(address game) external override onlyOwner {
        ...
        router.cancelKeperAdditionalLottery(game);
...
import "../lotteryGame/IDateTime.sol";
...
interface ILotteryGameFactory {
    ...
    /// @param rewardPool RewardPool contract address
    /// @param dateTime DateTime contract address
    /// @param gameToken LotteryGameToken contract address
    ...
    KeeperCompatibleInterface
{
    ...
    function startGame(uint32 VRFGasLimit_, uint96 amount) external virtual {
        ...
        if (linkToken.balanceOf(address(this)) > MIN_LINK_TOKENS_NEEDDED) {
            router.addFunds(address(this), amount);
        }
