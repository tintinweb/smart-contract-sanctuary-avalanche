...
contract IDO {
    ...
    function deposit(uint256 _amount, address _referral) external {
        ...
        if (
            ...
            uint256 reward = (_amount * REFFERAL_REWARD_RATE) / 10; // Adjusted to USDT decimals
    ...
    function claim(uint256 i) external {
        ...
        uint256 claimable = (depo.amount * rate * 10 ** (btfDecimals - usdtDecimals)) / 10; // Adjusted to BTF decimals
        uint256 reward = (claimable * DEPOSITOR_REWARD_RATE) / 10; // Adjusted to BTF decimals
