...
contract AuctionHouse is ERC721Holder, Ownable {
    ...
    event ListingUpdated(IERC721 indexed token, uint256 id, bytes32 indexed auctionId, address seller, uint256 price);
    ...
    function updateFixedPrice(bytes32 _auction, uint256 _price) external onlyOwner {
        Auction storage a = auctionInfo[_auction];
        require(a.isSold == false, "The item is already sold");
        require(a.auctionType == 0, "You can only edit the price in a fixed price sale");
        a.startPrice = _price;

        emit ListingUpdated(a.token, a.tokenId, _auction, a.seller, _price);
    }
}
