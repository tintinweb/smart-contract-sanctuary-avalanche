...
contract CollectionsRepository is ICollectionsRepository, Ownable {
    ...
    function create(
        ...
        require(bytes(_params.name).length >= 2, "Name<3");

        ONFT721A collection = new ONFT721A(_getSymbolByName(_params.name), _params, _creator, tokenFactory, lzEndpoint);
        userCollections[_creator].push(address(collection));

        emit Created(address(collection), _creator);
    }
    ...
    /**
     * @notice Sets the LayerZero Endpoint.
     *
     * @param _lzEndpoint The address of the Endpoint.
     */
...
import "../interfaces/IONFT721A.sol";
import "./ERC721A.sol";
...
contract ONFT721A is ONFT721Core, IONFT721A, ERC721A, ReentrancyGuard {
    using Strings for uint256;

    event TokenMinted(address collAddr, address owner, uint256 tokenId, uint256 quantity);
    ...
    modifier onlyTokenFactory() {
        require(_msgSender() == tokenFactory);
        _;
    }
    ...
    bytes4 public override assetName;
    address public tokenFactory;
    ...
    constructor(
        ...
        address _lzEndpoint
    ) ERC721A(params.name, _symbol) ONFT721Core(_lzEndpoint) {
    ...
    function mint(address _owner, uint256 _quantity, bytes32[] memory _merkleProof) override external nonReentrant {
        ...
        while (_locked.length > 0) {
            _quantity -= 1;
            transferFrom(address(this), _owner, _locked[_locked.length - 1]);
            _locked.pop();
        ...
        emit TokenMinted(address(this), _owner, (_nextTokenId() - 1), _quantity);
    }

    function _validateMint(uint256 _quantity, address _owner, bytes32[] memory _merkleProof) internal onlyTokenFactory {
        uint256 _newTotalMinted = _totalMintedUnlocked() + _quantity;
    ...
    function setNotRevealedURI(string memory _uri) external isBeforeMint onlyOwner {
        _notRevealedURI = _uri;
    }

    function reveal() external {
        require(_msgSender() == owner() || _msgSender() == TREASURY);
        _notRevealedURI = "";
    }

    function totalMinted() external view override returns (uint256) {
        return _totalMintedUnlocked();
    }
    ...
    function preMintToTeam(uint256 _quantity) external isBeforeMint {
        require(_msgSender() == creator, "!creator");
        if (dropFrom > 0) {
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint _tokenId) internal virtual override {
        _checkApproval(_msgSender(), _tokenId);
        require(ERC721A.ownerOf(_tokenId) == _from, "!own");
        transferFrom(_from, address(this), _tokenId);
    }

    function _creditTo(uint16, address _to, uint _tokenId) internal virtual override {
...
interface IONFT721A {
    ...
    function assetName() external view returns (bytes4);
...
contract ERC721A is IERC721A {
    ...
    function totalSupply() public view virtual override returns (uint256) {
        ...
        unchecked {
            return _currentIndex - _burnCounter - _locked.length - _startTokenId();
        }
    }
    ...
    function _totalMintedUnlocked() internal view virtual returns (uint256) {
        unchecked {
            return _currentIndex - _locked.length - _startTokenId();
        }
    }
    ...
    // Mints difference from _currentIndex to _tokenId to this contract to not change ERC721A behaviour and mint to _to only _tokenId
    function _mintTokenId(address _to, uint256 _tokenId) internal virtual {
        if (_tokenId == _currentIndex) {
            _mint(_to, 1);
            return;
        }
        require(_tokenId > _currentIndex);
        // If difference > 0, Minted tokens (diff) will be now locked and transferred instead of _mint() to the future `minters`
        uint256 _diff = _tokenId - _currentIndex;

        if (_diff > 0) {
            for (uint256 _i = 0; _i < _diff; _i++) {
                _locked.push(_currentIndex + _i);
            }
    ...
    function _mint(address _to, uint256 _quantity) internal virtual {
        uint256 fromTokenId = _currentIndex;
        if (_quantity == 0) revert MintZeroQuantity();

        _beforeTokenTransfers(address(0), _to, fromTokenId, _quantity);
        ...
        unchecked {
            ...
            _packedAddressData[_to] += _quantity * ((1 << _BITPOS_NUMBER_MINTED) | 1);
            ...
            _packedOwnerships[fromTokenId] = _packOwnershipData(
                _to,
                _nextInitializedFlag(_quantity) | _nextExtraData(address(0), _to, 0)
            );

            uint256 toMasked;
            uint256 end = fromTokenId + _quantity;
        ...
        _afterTokenTransfers(address(0), _to, fromTokenId, _quantity);
    }
