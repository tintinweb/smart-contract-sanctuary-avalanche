...
contract Bridge is Ownable, Pausable {
    ...
    constructor(address _WAVAX, address[] memory _signers, uint256 _required) {
        require(isContract(_WAVAX), "not a contract");
    ...
    function depositETH() external payable whenNotPaused {
        ...
        if (msg.value != 0) {
            IWAVAX(payable(WAVAX)).deposit{value: ETHAmount}();
        }
        require(
            IWAVAX(payable(WAVAX)).balanceOf(address(this)) >= ETHAmount,
            "Ethereum not deposited"
        );
        IWAVAX(payable(WAVAX)).transfer(msg.sender, ETHAmount);
        IWAVAX(payable(WAVAX)).transferFrom(
            address(msg.sender),
            address(this),
            ETHAmount
        );
        emit Deposit(msg.sender, WAVAX, ETHAmount);
    }
    ...
    function withdraw(
        ...
        if (token == WAVAX) {
            require(
                IWAVAX(payable(WAVAX)).balanceOf(address(this)) >= amount,
                "insufficient balance"
            );

            if (verify(signatures, message)) {
                txHashs[txhash] = true;
                IWAVAX(payable(WAVAX)).withdraw(amount);
                payable(msg.sender).transfer(address(this).balance);
                emit Withdraw(msg.sender, WAVAX, amount, message);
            } else {
