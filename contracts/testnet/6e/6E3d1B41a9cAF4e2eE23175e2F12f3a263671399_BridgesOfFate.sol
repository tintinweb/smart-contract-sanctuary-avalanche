...
contract BridgesOfFate is Ownable {
    ...
    uint256[11] private _buyBackCurve = [
        0.005 ether,
        0.01 ether,
        0.02 ether,
        0.04 ether,
    ...
    function _calculateBuyBackIn() internal view returns (uint256) {
        if (GameStatusInitialized[_latestGameId].stageNumber > 0) {
            if (GameStatusInitialized[_latestGameId].stageNumber <= _buyBackCurve.length) {
                return _buyBackCurve[GameStatusInitialized[_latestGameId].stageNumber - 1];
            }else if(GameStatusInitialized[_latestGameId].stageNumber > _buyBackCurve.length){
                return _buyBackCurve[_buyBackCurve.length - 1];
            }
        }
        return 0;
    }
