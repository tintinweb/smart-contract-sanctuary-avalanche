...
interface BidexCollection1155 {
    ...
    function lazyMint(string memory ipfsmetadata, address from, address to, uint total, uint supply, uint royal, uint256 id_) external;
}
...
contract BidexTrade is Initializable, OwnableUpgradeable, VerifySignature {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    function orderPlace(
        ...
        if (
            ...
            approveStatus[from][_conAddress] = true;
    ...
    // users[0] - from, users[1] - to, users[2] - contractAddress
    ...
    function lazyMinting(string[] memory ipfsmetadata,address[] memory users,uint256[] memory datas,string memory bidtoken,bytes memory signature) public payable {
        require(verify(users[0], datas[6], ipfsmetadata[3], datas[5], signature) == true ,"Not vaild User");
        require(
            msg.sender == owner() || publicMint == true,
            "Public Mint Not Available"
        );
        uint256[8] memory ldatas;
        ldatas[6] = pBEPent(datas[3], serviceValue).add(datas[3]);
        _tid = _tid.add(1);
        uint256 id_ = _tid.add(block.timestamp);
        if(datas[1] == 721){
            BidexCollection721(Bidex721).lazyMint(ipfsmetadata[0], users[0], users[1], datas[2], id_);
        }
        else{
            BidexCollection1155(Bidex1155).lazyMint(ipfsmetadata[0], users[0], users[1], datas[4], datas[0], datas[2], id_);
            if (datas[6] > 0) {
                orderPlace(id_, datas[6], users[2], users[0], datas[1], "mint");
            }
        }
        if (
            keccak256(abi.encodePacked((bidtoken))) ==
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Bidex721, msg.sender, datas[1], "mint");
            } else {
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Bidex1155, msg.sender, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
