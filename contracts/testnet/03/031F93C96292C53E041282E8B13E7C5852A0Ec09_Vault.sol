...
contract Vault is ERC20, ReentrancyGuard, Pausable {
  ...
  // return 0 for positive delta, return 1 for negative delta
  // function getDelta() public view returns (uint256, uint256) {
  //   (uint256 _tokenAAmt,,,) = IManager(manager).assetInfo(address(this));

  //   (uint256 _tokenADebtAmt,,,) = IManager(manager).debtInfo(address(this));
  //   if (_tokenAAmt > _tokenADebtAmt) {
  //     return ((_tokenAAmt - _tokenADebtAmt) * SAFE_MULTIPLIER / _tokenAAmt, 0);
  //   } else {
  //     return ((_tokenADebtAmt - _tokenAAmt) * SAFE_MULTIPLIER / _tokenAAmt, 1);
  //   }
  // }
