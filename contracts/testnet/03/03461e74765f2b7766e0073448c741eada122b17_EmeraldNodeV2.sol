...
contract EmeraldNodeV2 is EmeraldNodeStorageV2 {
    ...
    function viewBPN()
        public
        view
        returns (uint256 AxavPrice, uint256 EmPrice)
    {
    ...
    function sellNode(uint256 _nodeId) external returns (bool) {
        ...
        require(
            AvaxCoin.balanceOf(bondTreasuryAddress.treasuryAccount()) >=
                _avax_price,
            "EmeraldNodeV1: Not Enough balance in treasury"
        );

        AvaxCoin.transferFrom(
            bondTreasuryAddress.treasuryAccount(),
            address(this),
            _avax_price
        );
...
interface IBondAndTreasury {
     ...
     * @dev Returns the amount of tokens in existence.
     */
    function getEmToUSDPrice() external view returns (int256);

    function treasuryAccount() external view returns (address);

    function getAvaxToUsdPrice() external view returns (int256);
