...
contract V2SwapERC20 {
    ...
    IJoeRouter02 public immutable swapRouter = IJoeRouter02(SWAP_ROUTER);
    address private constant BRIDGE_ADDRESS =
        0x6EA0C45bAC29a2A2769ec14f4BCD959b07780106;
    IBridge public immutable bridge = IBridge(BRIDGE_ADDRESS);
    ...
    function swapExactInputSingle(
        ...
        safeTransferWithApprove(amountIn, address(swapRouter), tokenIn);
    ...
    function swapExactOutputSingle(
        ...
        path[0] = address(tokenIn);
        path[1] = address(tokenOut);
        safeTransferWithApprove(amountInMax, address(swapRouter), tokenIn);
    ...
    function swapExactInputSingleThenBridge(
        uint256 amountIn,
        uint256 amountOutMinimum,
        IERC20 tokenIn,
        IERC20 tokenOut,
        ...
        tokenOut.safeApprove(BRIDGE_ADDRESS, amountOut);
        bridge.depositERC20(
            address(tokenOut),
            amountOut,
            bundle_,
    ...
    function swapExactOutputSingleThenBridge(
        uint256 amountOut,
    ...
    function swapExactOutputMultiHopThenBridge(
        ...
        amountIn = swapExactOutputMultihop(amountOut, amountInMaximum, path);
        address tokenOut = path[path.length - 1];
        IERC20(tokenOut).safeApprove(BRIDGE_ADDRESS, amountOut);
        bridge.depositERC20(
            address(tokenOut),
            amountOut,
            bundle_,
...
interface IBundler {
     ...
     */
    function determineProxyAddress(bytes32 salt_)
        external
        view
        returns (address);
}
