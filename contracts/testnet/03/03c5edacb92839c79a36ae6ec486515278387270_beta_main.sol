...
contract beta_main is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 dec_c= (10**dec);
    uint denom = 100;
    uint private rwSwap;
    uint public nodeAmount;//amount of toekns needed for node purchase
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        teamPoolFee = (teamPoolFee);
        treasuryFee = (treasuryFee);
        rewardsFee = (rewardsFee);
        require(nodeAmount > nodeAmount.mul(teamPoolFee.div(denom)).add(nodeAmount.mul(rewardsFee.div(denom))).add(nodeAmount.mul(treasuryFee.div(denom))), "fees are too high");
        supply = _supply*(10**18);
	emit Transfer(address(0x0), address(this), supply);
    }
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(address(this),teamPool, nodeAmount.mul(teamPoolFee).div(denom));
        super._transfer(address(this),rewardsPool, nodeAmount.mul(rewardsFee).div(denom));
        super._transfer(address(this),treasuryPool, nodeAmount.mul(treasuryFee).div(denom));
   ...
   function get(address account) public view returns(uint256[3] memory){
	...
	if (tier[2] != 0) {
        ...
        uint get = getEm();
        uint256 feeAmount = rewardAmount.mul((cashoutFee.mul(perc).div(10000)));
