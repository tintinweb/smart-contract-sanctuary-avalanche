...
contract StakeRewards is DepositWithdraw {
    ...
    mapping(address => mapping(uint => AccBaseReward)) public accountBaseReward;
    ...
    function setBooster(uint16 boosterId_, BoosterType type_, address contractAddress_, uint amount_, uint boostPercentage_) public onlyRewardDefiner {
        ...
        if (type_ == BoosterType.ERC20Balance || type_ == BoosterType.HoldingNFT) {
            require(contractAddress_ != address(0), "Contract address can't be zero address");
        }
    ...
    function setStakingContract(address contract_) public onlyOwner {
        require(contract_ != address(0), "Contract address can't be zero address");
    ...
    function setRewardDefiner(address rewardDefiner_) public onlyOwner {
        require(rewardDefiner_ != address(0), "Reward definer address can't be zero address");
    ...
    function setRewardConfig(uint rewardId_, bool vestingEnabled_, uint vestingStartDate_, uint vestingDayInterval_, uint vestingPeriodCount_, uint snapshotMin_, uint snapshotMax_) public onlyRewardDefiner stakingContractIsSet {
        RewardConfig memory config = rewardConfig[rewardId_];
        if (config._exist == true && _isRewardClaimed[rewardId_] == true) {
            revert("This rewardId has claimed reward. Can't update");
        }
        ...
        // Check vesting day
        if (vestingEnabled_) {
            require(vestingDayInterval_ > 0, "Vesting day can't be zero");
            require(vestingPeriodCount_ > 0, "Vesting period count can't be zero");
            require(vestingStartDate_ > 0, "Vesting start date can't be zero");
        }
    ...
    // Set & Update reward
    function _setReward(uint rewardId_, uint chainId_, RewardType rewardType_, address contractAddress_, uint amount_) internal {
        Reward memory currentReward = _rewards[rewardId_];
        Reward memory reward = Reward(chainId_, rewardType_, contractAddress_, amount_, 0, false);
        ...
        if (currentReward._exist == true && _isRewardClaimed[rewardId_] == true) {
            revert("Can't set/update claimed reward");
        }
    ...
    function setNativeReward(uint rewardId_, uint chainId_, uint amount_) public onlyRewardDefiner {
        _setReward(rewardId_, chainId_, RewardType.Native, address(0), amount_);
    }

    // Set token reward
    function setTokenReward(uint rewardId_, uint chainId_, address contractAddress_, uint amount_) public onlyRewardDefiner {
        _setReward(rewardId_, chainId_, RewardType.Token, contractAddress_, amount_);
    }
    ...
    function _claimReward(address account_, uint rewardId_, uint vestingIndex_) internal {
        ...
        require(isRewardClaimable(account_, rewardId_, vestingIndex_) == true, "Reward isn't claimable");
        ...
        if (reward.chainId == chainId()) {
            if (reward.rewardType == RewardType.Native) {
                _sendNativeCoin(payable(account_), reward.amount);
            } else if (reward.rewardType == RewardType.Token) {
                _sendToken(account_, reward.contractAddress, reward.amount);
            }
    ...
    // Get un claimed reward count
    function getUnClaimedRewardCount(address account_, uint rewardId_) external view returns (uint) {
        RewardConfig memory config = rewardConfig[rewardId_];
        uint unClaimedCounter = 0;

        for (uint i = 0; i < config.vestingPeriodCount; ++i) {
            AccountReward memory reward = _accountRewards[rewardId_][account_][i];
            if (reward.isClaimed == false && reward._exist == true) {
                unClaimedCounter++;
            }
        }
    ...
    function getAccountRewardDetails(address account_, uint rewardId_, uint snapshotMin_, uint snapshotMax_) public view returns (AccBaseReward memory, CacheAverage memory, uint) {
        AccBaseReward memory baseReward = accountBaseReward[account_][rewardId_];
    ...
    function _prepareRewardVestingPeriods(address account_, uint rewardId_) internal {
        ...
        if (baseReward._exist == false) {
            ...
            accountBaseReward[account_][rewardId_] = baseReward;
        }
