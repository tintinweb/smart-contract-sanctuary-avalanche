...
library PlacedOrderLib {
    ...
    function reset(PlacedOrder storage order)
        internal
        returns (uint256 totalAmount, uint256 totalPaid)
    {
        totalAmount = order.amountAsRegular + order.amountAsWhitelisted;
        totalPaid = order.totalPaidAsRegular + order.totalPaidAsWhitelisted;
...
contract PreSale is Owners, EmergencyMode, IPreSaleBurnable {
    ...
    bool internal openPreOrderToRegular = false;
    bool internal openPreOrderToWhitelist = false;
