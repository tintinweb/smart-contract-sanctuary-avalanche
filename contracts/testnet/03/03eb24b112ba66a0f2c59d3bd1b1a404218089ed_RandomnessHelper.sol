...
contract RandomnessHelper is Test {
  ...
  function randomInRange224(uint224 min, uint224 max) public returns (uint224) {
    require(min < max, "min >= max");
    randomSeed = uint224(uint256(keccak256(abi.encode(randomSeed))));
    return min + uint224(randomSeed % (max - min));
  }
  ...
  function amountTimeBetweenOracleUpdate(uint256 seed, IOracleManagerFixedEpoch oracleManager)
    internal
    view
    ...
    )
  {
    ...
    for (uint256 index = 1; index < maxNumberOfOracleUpdates; index++) {
      ...
      additionalTime = amountTimeBetweenOracleUpdate(
        block.timestamp + additionalTime + randomSeed + index,
        oracleManager
      );
      ...
      bool willOraclePriceTriggerEpochExecution = ((startEpoch + numberOfEpochsToExecute + 1) *
      ...
      if (willOraclePriceTriggerEpochExecution) {
        ...
        epochExecutions[numberOfEpochsToExecute] = TestOracleInfo(
          block.timestamp,
          price,
          currentOracleUpdateIndex
        );
