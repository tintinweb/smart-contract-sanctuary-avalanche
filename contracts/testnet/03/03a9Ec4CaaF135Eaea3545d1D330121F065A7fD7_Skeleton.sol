...
contract Skeleton is ERC721Enumerable, Pausable, Restricted {
    ...
    /**
     * Function for sneaky mint Skeletons / Rangers with Adventurers
     * Costs : 1 Adventurers 
        ...
        notBlacklisted
    {
        ...
        require(
            orders.length + tokenIds.length + (sneakyOrders.length - sneakyOrdersSettled) <= MAX_TOKENS,
            "All tokens minted"
        );
    ...
    function _manageSneakyOrders(uint256 amountToSettle, uint256 amountToPlace, address buyer) internal {
        ...
        while (
            ...
            if (seed < 10) _manageOrders(1, 1, sneakyOrder.buyer);
            unchecked{sneakyOrdersSettled++;}
            emit SneakyOrderSettled(sneakyOrder.buyer, sneakyOrdersSettled, seed < 10);
        }
    ...
    function mintCost(uint256 tokenId) public view returns (uint256, uint256) {
        if (tokenId <= generationsTresholds[0]) return (generationsPriceAVAX[0], generationsPriceGLOW[0]);
        if (tokenId <= generationsTresholds[1]) return (generationsPriceAVAX[1], generationsPriceGLOW[1]);
        if (tokenId <= generationsTresholds[2]) return (generationsPriceAVAX[2], generationsPriceGLOW[2]);
        if (tokenId <= generationsTresholds[3]) return (generationsPriceAVAX[3], generationsPriceGLOW[3]);
        return (generationsPriceAVAX[4], generationsPriceGLOW[4]);
    }
    ...
    function setState(bool _normalMint, bool _burnMint, bool _burnSneakyMint) public onlyController {
        state = State(_normalMint, _burnMint, _burnSneakyMint);
    }
