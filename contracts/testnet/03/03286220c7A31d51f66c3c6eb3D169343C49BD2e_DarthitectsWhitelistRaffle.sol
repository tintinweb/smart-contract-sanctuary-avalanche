...
interface IDarthitectsWhitelistRaffle {
    ...
    /// @param numOfWLSpots Number of Whitelist Spots
    ...
    // RAFFLE PARTICIPENT VIEWS

    /// @notice  returns the raffle enter status of the caller
    /// @param ownerAddress raffle owner address
    /// @param id bytes32 encoded raffle name
    ...
    /// @notice  returns the raffle result of caller
    /// @param ownerAddress raffle owner address
    /// @param id bytes32 encoded raffle name
    ...
    /// @notice  returns the raffle public view
    /// @param ownerAddress raffle owner address
    /// @param id bytes32 encoded raffle name
    function getRaffle(
        address ownerAddress,
    ...
    /// @notice  returns the raffle public status
    /// @param ownerAddress raffle owner address
    /// @param id bytes32 encoded raffle name
...
/// @title Darthitect's Whitelist Raffle
/// @author darthitect
/// @notice With this contract, you can organize a whitelist raffle for your NFT projects
/// @dev almost secure, on-chain randomness is used in this contract to distribute whitelist spots to participants
    ...
    IDarthitectsWhitelistRaffle
{
    ...
    function enter(address ownerAddress, bytes32 id) external override {
        ...
        require(
            payable(msg.sender).balance >= raffle.minReqNativeTokenBalance,
            "insufficient balance"
        );

        if (raffle.minReqTokenBalance > 0) {
            (
                uint256 balance,
                bool success,
                string memory errorMessage
            ) = _getTokenBalance(
                    raffle.requiredTokenType,
                    raffle.requiredTokenAddress,
                    raffle.requiredTokenId
                );
            require(success, errorMessage);
            if (raffle.requiredTokenType == type(IERC20).interfaceId) {
                require(success, "ERC20 balance failed");
                require(
                    balance >= raffle.minReqTokenBalance,
                    "insufficient ERC20"
                );
            } else if (raffle.requiredTokenType == type(IERC721).interfaceId) {
                require(success, "ERC721 balance failed");
                require(
                    balance >= raffle.minReqTokenBalance,
                    "insufficient ERC721"
                );
            } else if (raffle.requiredTokenType == type(IERC1155).interfaceId) {
                require(success, "ERC1155 balance failed");
                require(
                    balance >= raffle.minReqTokenBalance,
    ...
    function getMyRaffle(
        ...
        RaffleView memory _raffleView = RaffleView({
            ...
            minReqNativeTokenBalance: raffle.minReqNativeTokenBalance,
            minReqTokenBalance: raffle.minReqTokenBalance,
            ...
            requiredTokenAddress: raffle.requiredTokenAddress,
            requiredTokenType: raffle.requiredTokenType,
            requiredTokenName: raffle.requiredTokenName,
            requiredTokenId: raffle.requiredTokenId,
            logo: raffle.logo,
    ...
    function getERC20Balance(
        ...
        try tokenContract.balanceOf(msg.sender) returns (uint256 balance) {
            return (balance, true);
        } catch {
            return (0, false);
        }
    ...
    function getERC1155Balance(
        ...
        try tokenContract.balanceOf(msg.sender, tokenId) returns (
            uint256 balance
        ) {
            return (balance, true);
        } catch {
        ...
        returns (uint256 result, bool success, string memory errorMessage)
    {
        if (tokenAddress != address(0x0) && tokenAddress.code.length > 0) {
            if (tokenType == type(IERC20).interfaceId) {
                try this.getERC20Balance(tokenAddress) returns (
                    uint256 _result,
                    bool _success
                ) {
                ...
                try this.getERC721Balance(tokenAddress) returns (
                    uint256 _result,
                    bool _success
                ) {
                ...
                try this.getERC1155Balance(tokenAddress, tokenId) returns (
                    uint256 _result,
                    bool _success
                ) {
