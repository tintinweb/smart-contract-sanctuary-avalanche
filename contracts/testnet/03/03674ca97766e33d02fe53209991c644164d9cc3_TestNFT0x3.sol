...
// File: github/LayerZero-Labs/solidity-examples/contracts/interfaces/ILayerZeroUserApplicationConfig.sol
...
// File: github/LayerZero-Labs/solidity-examples/contracts/interfaces/ILayerZeroEndpoint.sol
...
// File: github/LayerZero-Labs/solidity-examples/contracts/interfaces/ILayerZeroReceiver.sol
...
// File: github/LayerZero-Labs/solidity-examples/contracts/lzApp/LzApp.sol
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress) internal virtual {
        bytes memory trustedRemote = trustedRemoteLookup[_dstChainId];
        uint16 version = 1;
        uint256 destGasAmount = 200000;
        bytes memory _adapterParams = abi.encodePacked(version, destGasAmount);        
        require(trustedRemote.length != 0, "LzSend: destination chain is not a trusted source.");
        lzEndpoint.send{value: msg.value}(_dstChainId, trustedRemote, _payload, _refundAddress, address(0x0), _adapterParams);
    }
...
// File: github/LayerZero-Labs/solidity-examples/contracts/lzApp/NonblockingLzApp.sol
...
// File: github/LayerZero-Labs/solidity-examples/contracts/token/onft/IONFT721.sol
...
interface IONFT721 is IERC721 {
     ...
     */
    function traverseChains(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress) external payable;
...
// File: github/LayerZero-Labs/solidity-examples/contracts/token/onft/ONFT721.sol
...
contract ONFT721 is IONFT721, NonblockingLzApp, ERC721 {
    ...
    function traverseChains(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress) public payable virtual override {
        _send(_msgSender(), _dstChainId, _toAddress, _tokenId, _refundAddress);
    }

    function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, address payable _refundAddress) internal virtual {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ONFT721: send caller is not owner nor approved");
        require(ERC721.ownerOf(_tokenId) == _from, "ONFT721: send from incorrect owner");
        _beforeSend(_from, _dstChainId, _toAddress, _tokenId);

        bytes memory payload = abi.encode(_toAddress, _tokenId);
        _lzSend(_dstChainId, payload, _refundAddress);
...
// File: github/LayerZero-Labs/solidity-examples/contracts/token/onft/extension/UniversalONFT721.sol
...
contract UniversalONFT721 is ONFT721 {
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 2, "ONFT: Max 1 Token per transaction");
        require(nextMintId <= maxMintId, "ONFT: Max Mint limit reached");
        require(minted[msg.sender] + numTokens <= 1, "limit per wallet reached");
...
// File: github/LayerZero-Labs/solidity-examples/contracts/TestNFT0x3.sol
...
contract TestNFT0x3 is UniversalONFT721 {
    constructor(
        address _layerZeroEndpoint,
        uint _startMintId,
        uint _endMintId
    ) UniversalONFT721("TestNFT0x3", "TNFT0x3", _layerZeroEndpoint, _startMintId, _endMintId) {}
}