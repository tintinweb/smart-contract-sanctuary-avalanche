    ...
    Authorizable
{
    ...
    constructor(
        ...
        royaltiesEndTime = block.timestamp + 30 days;
    }
    ...
    modifier wlRequirements(uint8 tier, uint256 quantity, bytes32[] calldata merkleProof){
        ...
        require(quantity <= tierMints[tier]);
    ...
    function isWhitelisted(
        ...
        if (MerkleProof.verify(proof, merkleRootT3, keccak256(abi.encodePacked(user))) && !claimed3[user]) {
            return 3;
        } else if (
            MerkleProof.verify(proof, merkleRootT2, keccak256(abi.encodePacked(user))) && !claimed2[user]
        ) {
            return 2;
        } else if (
            MerkleProof.verify(proof, merkleRootT1, keccak256(abi.encodePacked(user))) && !claimed1[user]
        ) {
            return 1;
        } else return 0;
    }
    ...
    function mint(uint256 quantity, uint256 royalty) external payable publicRequirements(quantity){
    ...
    function tier1Mint(uint256 quantity, uint256 royalty, bytes32[] calldata merkleProof)
        external
        payable wlRequirements(1, quantity, merkleProof)
    {
    ...
    function tier2Mint(uint256 quantity,uint256 royalty, bytes32[] calldata merkleProof)
        external
        payable wlRequirements(2, quantity, merkleProof)
    {
    ...
    function tier3Mint(uint256 quantity,uint256 royalty, bytes32[] calldata merkleProof)
        external
        payable wlRequirements(3, quantity, merkleProof)
    {
