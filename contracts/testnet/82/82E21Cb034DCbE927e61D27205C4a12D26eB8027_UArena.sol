...
contract UArena is IUArena, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  // TODO: This check is not the best. Check also the maps and make sure the Stake is stored. BUT, don't trigger require() which say "Token is not staked" you want to do this gracefully here.
  ...
  /**
   * gets the rank score for a token
   * @param tokenId the ID of the token to get the rank for
   * @return the rank score of the token (5-8)
   */
  ...
  /** 
   * add $BLOOD to claimable pot for the Yakuza
   * @param amount $BLOOD to add to the pot
   */
  ...
  /**
   * adds Fighters and Yakuza to the Arena and Patrol
   * should never be called from another contract!
   * @param tokenIds the IDs of the Fighters and Yakuza to stake
   */
  ...
  /**
   * adds a single Fighter to Arena
   * @param account the address of the staker
   * @param tokenId the ID of the Fighters to add to the Arena
   */
  ...
  /**
   * adds a single Yakuza to Patrol
   * @param account the address of the staker
   * @param tokenId the ID of the Yakuza to add to the Patrol
   */
  ...
  /**
   * realize $BLOOD earnings and optionally unstake tokens from the Arena
   * unstaking requires 2 days of not claiming beforehand - then claim+unstake can be called to to unstake
  ...
  /**
   * realize $BLOOD earnings for a single Fighter and optionally unstake it
   * if not unstaking, pay a 20% tax to the staked Yakuza
   * if unstaking, there is a 50% chance all $BLOOD is stolen
   * @param tokenId the ID of the Fighter to claim earnings from
   * @param unstake whether or not to unstake the Fighters
   * @return owed - the amount of $BLOOD earned
   */
  ...
  /**
   * realize $BLOOD earnings for a single Yakuza and optionally unstake it
   * Yakuza earn $BLOOD proportional to their rank
   * @param tokenId the ID of the Yakuza to claim earnings from
   * @param unstake whether or not to unstake the Yakuza
   * @return owed - the amount of $BLOOD earned
   */
  function _claimYakuza(uint256 tokenId, bool unstake) private returns (uint256 owed) {
    ...
    require(uNFT.ownerOf(tokenId) == address(this), "Arena: Doesn't own token"); // TODO: Is this still needed?
  ...
  // TODO: Test this well!
  /**
   * chooses a random Yakuza thief when a newly minted token is stolen
   * @param seed a random value to choose a Yakuza from
   * @return the owner of the randomly selected Yakuza thief
   */
  ...
  function setPaused(bool paused) external requireContractsSet onlyOwner {
    if (paused) _pause();
    else _unpause();
  }
