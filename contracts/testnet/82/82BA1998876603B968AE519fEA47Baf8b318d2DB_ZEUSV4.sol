...
contract ZEUSV4 is Ownable {
    ...
    function getNodesOfUser(address account, uint32 nodeType)
        public
        view
        returns (NodeEntity[] memory)
    {
        ...
        for (uint256 i = 0; i < 100; i++) {
            (
                ...
                uint256 rewardAvailable,
                uint32 _nodeType
            ) = nodeRewardManager._nodesOfUser(account, i);
            nodes[i] = NodeEntity({
                name: name,
    ...
    function getLastClaimTime(address account, uint256 creationTime, uint32 nodeType) public view returns(uint256) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        return nodeRewardManager.getLastClaimTimeWithCreationTime(account,creationTime);
    }
    ...
    function _cashoutNodeReward(
        ...
        NodeEntity[] memory nodes = getNodesOfUser(account, nodeType);
        ...
        public
    {
        ...
        for (uint256 i = 0; i < claimtimes_.length; i++) {
            bool isDue = _isDue(sender,claimtimes_[i],nodeType);
            (uint256 rewardAmount, uint256 fee) = nodeRewardManager._cashoutNodeReward(
                sender,
                claimtimes_[i]
            );
            if(isDue){
