...
contract Avalant is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    address public avalantBoxesContract;
    address public antGoldContract;
    address public sugaContract;
    ...
    function initialize(
        ...
        address avalantBoxesContractAddr,
        ...
        openForPublic = false;
        mintFeeAmountWL = 800000000000000000;
        mintFeeAmount = 1200000000000000000;
        feesToChangeName = 500000000000000000000; // 500 ANTG
        avalantBoxesContract = avalantBoxesContractAddr;
    ...
    function setContractAddresses(address _antGoldContract, address _sugaContract) public onlyRole(DEFAULT_ADMIN_ROLE) {
        antGoldContract = _antGoldContract;
        sugaContract = _sugaContract;
    }

/*
    function verifyWhitelist(address toVerify) internal returns (bool) {
        // TODO
        return false;
    }
*/
    ...
    function openBox(uint boxTokenId) public {
        AvalantBoxes ab = AvalantBoxes(avalantBoxesContract);
    ...
    function _mint(address to) private {
        ...
        Ant memory newAnt = Ant(nextTokenId, to, 1, string(abi.encodePacked("Ant #", Strings.toString(nextTokenId))));
        allAvalants[nextTokenId] = newAnt;
        _safeMint(to, nextTokenId);
    }

    function pickAnts(uint numberOfAnts) public payable {
        // -- TODO WL
        // require(openForPresale == true, "PRESALE NOT OPEN");
        // TODO HERE WL BUYING AND BOX OPENING
        // bool isWhitelisted = verifyWhitelist(msg.sender);
        // ----
        ...
        uint realMintFees = isWhitelisted ? mintFeeAmountWL : mintFeeAmount;
        uint price = realMintFees * numberOfAnts;
    ...
    function dig(uint _tokenId) public {
        require(msg.sender == sugaContract, "Only suga contract can dig");
        // claim antg todo
    ...
    {
        if (address(antGoldContract) != address(0) && address(from) != address(0)) {
            AntGold ag = AntGold(antGoldContract);
            if (ag.antStaked(tokenId) == true) {
                ag.unstakeAntAndClaim(tokenId);
            }
        }
...
// Todo level up
// Todo cooldown skipping

contract AntGold is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function setSugaAddress(address _sugaAddress) public onlyOwner {
    ...
    function _claimAntGold(uint[] memory tokenIds) internal {
        Avalant a = Avalant(AVALANT_CONTRACT);
        uint totalNewAntg = 0;
        address ownerOfAnt;
        for (uint i = 0; i < tokenIds.length; i++) {
            require(a.ownerOf(tokenIds[i]) == msg.sender || msg.sender == AVALANT_CONTRACT, "Not ur ant");
            ownerOfAnt = a.ownerOf(tokenIds[i]);
    ...
    function _unstakeAnt(uint tokenId) internal {
        Avalant a = Avalant(AVALANT_CONTRACT);
        require(a.ownerOf(tokenId) == msg.sender || msg.sender == AVALANT_CONTRACT, "Not ur ant");
        require(antStaked[tokenId] == true, "Not staked");
        antStaked[tokenId] = false;
    }

    function unstakeAntAndClaim(uint tokenId) public {
    ...
    function unstakeMultipleAndClaim(uint[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            unstakeAntAndClaim(tokenIds[i]);
        }
    }

    function burn(address acc, uint amount) public {
        require(msg.sender == AVALANT_CONTRACT || msg.sender == SUGA_CONTRACT, "Not authorized");
        _burn(acc, amount);
    }
}
