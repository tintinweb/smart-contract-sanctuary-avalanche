/**
 *Submitted for verification at testnet.snowtrace.io on 2023-04-16
*/
...
interface ITreasury is IEpoch {
    function getSncPrice() external view returns (uint256);

    function getSncUpdatedPrice() external view returns (uint256);

    function getSncLockedBalance() external view returns (uint256);

    function getSncCirculatingSupply() external view returns (uint256);

    function getNextExpansionRate() external view returns (uint256);

    function getNextExpansionAmount() external view returns (uint256);

    function previousEpochSncPrice() external view returns (uint256);
...
contract Boardroom is ShareWrapper, OwnableUpgradeSafe, ContractGuard {
    ...
    function setLockUp(uint256 _withdrawLockupEpochs) external onlyOwner {
        require(_withdrawLockupEpochs <= 28, "_withdrawLockupEpochs: out of range"); // <= 2 week
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyOwnerOrTreasury {
        require(_withdrawFee <= 500, "Max withdraw fee is 5%");
    ...
    function canClaimReward() public view returns (bool) {
        ...
        return _treasury.previousEpochSncPrice() >= 1e18 && _treasury.getNextExpansionRate() > 0; // current epoch and next epoch are both expansion
    }
    ...
    function _sacrificeReward() internal updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            IBasisAsset(address(snc)).burn(reward);
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            members[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
            members[msg.sender].rewardEarned = 0;
            snc.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOwnerOrTreasury {
        ...
        snc.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(snc), "snc");
