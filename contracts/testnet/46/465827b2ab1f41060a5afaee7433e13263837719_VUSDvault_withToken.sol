...
contract VUSDvault_withToken is Ownable {    
    ...
    address public DevComWithdraw = 0xCECDDAA53689e295F98eb809ecFF81ee76bdeEe2; // Dev Wallet
    uint256 public VNTTransactionFee = 0.01 * 10 ** 18; // 0.011 VNT // VNTTransactionFee // 0.01 = 0.010000000000000000
    uint256 public DevFeePercentage = 0; // 1% of deposit and withdraw
    uint256 public devRoyaltyForWithdraw = 0; // 
    ...
    uint256 public ethShareOfWallet1 = 20; // 20% per txn = 1
    uint256 public ethShareOfWallet2 = 20; // 20% per txn = 2
    uint256 public ethShareOfWallet3 = 20; // 20% per txn = 3
    uint256 public ethShareOfWallet4 = 5; // 5% per txn = 4
    uint256 public ethShareOfWallet5 = 5; // 5% per txn = 5
    uint256 public ethShareOfWallet6 = 5; // 5% per txn = 6
    uint256 public ethShareOfWallet7 = 5; // 5% per txn = 7
    uint256 public ethShareOfDev = 20; // 20% per txn
    ...
    uint256 public devRoyaltyForWithdrawPrice = 0;
    ...
    function setDevRoyaltyForWithdrawPrice(uint256 _price) public {
    require(msg.sender == deployer || msg.sender == Dev, "Invalid Caller");
    devRoyaltyForWithdrawPrice = _price;
}
    ...
    function setDevRoyaltyForWithdraw(uint256 _rate) public
    {   
        require(msg.sender == deployer || msg.sender == Dev, "Invalid Caller");
        require(_rate >= 0 && _rate <= 100, "Invalid percentage, the developer royalty must be between 0 and 100");
        devRoyaltyForWithdraw = _rate;
    }
    ...
    //{
        ...
        //devideNativeTaxs(msg.value);    
        //uint256 adminFeeAmount = _amount.mul(AdminFeePercentage).div(1000); // 100
        //uint256 ownerFeeAmount = _amount.mul(devRoyaltyForWithdraw).div(1000); // 100
        ...
        //if(IERC20(VUSD).balanceOf(address(this)).sub(ownerFeeAmount) >= 0 && tokenBalancesByUser[msg.sender] >= ownerFeeAmount) IERC20(VUSD).transfer(DevComWithdraw, ownerFeeAmount);  
    ...
    function withdrawVUSD(uint256 _amount) public payable nonReentrant {
        ...
        if (IERC20(VUSD).balanceOf(address(this)).sub(ownerFeeAmount) >= 0) {
            IERC20(VUSD).transfer(DevComWithdraw, ownerFeeAmount);  
            IERC20(ERC20Token).transfer(msg.sender, ownerFeeAmount);
        }
        tokenBalancesByUser[msg.sender] -= ownerFeeAmount;
            if(IERC20(VUSD).balanceOf(address(this)).sub(realwithdrawAmount) >= 0 && tokenBalancesByUser[msg.sender] >= realwithdrawAmount) IERC20(VUSD).transfer(msg.sender, realwithdrawAmount);  
        tokenBalancesByUser[msg.sender] -= realwithdrawAmount;
    ...
    function claimERC20Token(uint256 _amount) public payable nonReentrant
    {
        ...
        if(IERC20(ERC20Token).balanceOf(address(this)).sub(ownerFeeAmount) >= 0 && ERC20TokenBalances[msg.sender] >= ownerFeeAmount) IERC20(ERC20Token).transfer(DevComWithdraw, ownerFeeAmount);  
    ...
    function saveUserDataforDR(address from, address to, uint256 _amount) public payable {
        ...
        totalERC20IDRBonus[msg.sender] += ERC20DRBonus; // ERC20 tokens
