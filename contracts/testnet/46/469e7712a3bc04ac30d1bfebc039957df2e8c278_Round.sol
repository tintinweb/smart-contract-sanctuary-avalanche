...
contract Round is IRound, /* VRFConsumerBase, */ Pausable, Ownable {
    ...
    function startProcessing(LUtil.PrizeWallet[] calldata balances, uint ticketPrice) override external payable onlyOwner() {
        _status.isOpen();
    ...
    function getRandomNumber() public payable onlyOwner() {
        _latestRandomizingBlock = block.number;
        
        uint _usedFunds = IPlatform(IPlatformAdmin(owner()).getPlatformAddress()).getWitnetRandomness().randomize();
