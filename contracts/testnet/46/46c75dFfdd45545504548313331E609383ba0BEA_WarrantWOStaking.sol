...
contract WarrantWOStaking is Ownable {

    using SafeMath for uint256;
    using SafeMath for uint32;
    using SafeERC20 for IERC20;

    address public immutable WarrantWO;
    ...
    constructor ( 
        ...
        uint32 _firstEpochWarrantWO
    ) {
        require( _WarrantWO != address(0) );
        WarrantWO = _WarrantWO;
        ...
        epoch = Epoch({
            ...
            endWarrantWO: _firstEpochWarrantWO,
            distribute: 0
        });
    }
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        IERC20( WarrantWO ).safeTransferFrom( msg.sender, address(this), _amount );
    ...
    function forfeit() external {
        ...
        IERC20( WarrantWO ).safeTransfer( msg.sender, info.deposit );
    }
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        IERC20( WarrantWO ).safeTransfer( msg.sender, _amount );
    }
    ...
    function rebase() public {
        if( epoch.endWarrantWO <= uint32(block.timestamp) ) {

            IMemo( Memories ).rebase( epoch.distribute, epoch.number );

            epoch.endWarrantWO = epoch.endWarrantWO.add32( epoch.length );
            epoch.number++;
    ...
    function contractBalance() public view returns ( uint ) {
        return IERC20( WarrantWO ).balanceOf( address(this) ).add( totalBonus );
    }
