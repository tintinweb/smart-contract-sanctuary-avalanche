    ...
    UUPSUpgradeable
{
    ...
    function batchMint(
        address account_,
        uint256 quantity_,
        Metadata[] memory metadatas_
    ) external onlyRole(MINTER_ROLE) {
        if (quantity_ != metadatas_.length) revert LengthMismatch();
        _batchMint(account_, _idCounter, quantity_, metadatas_);
    }
    ...
    function _batchMint(address account_, uint256 tokenId_, uint256 totalMint_, Metadata[] memory metadatas_) internal {
        for (uint256 i; i < totalMint_; ) {
            unchecked {
                _metadatas[tokenId_] = Metadata({ tokenName: metadatas_[i].tokenName });
                _safeMint(account_, tokenId_);
                emit SetMetadata(account_, tokenId_, metadatas_[i]);
                ++tokenId_;
                ++i;
            }
        }
    ...
    function setMetadata(uint256 tokenId_, Metadata memory metadatas_) external {
        ...
        _metadatas[tokenId_] = Metadata({ tokenName: metadatas_.tokenName });
        emit SetMetadata(user, tokenId_, metadatas_);
    }
...
interface IERC721Full {
    ...
    function batchMint(address account_, uint256 quantity_, Metadata[] memory metadatas_) external;
