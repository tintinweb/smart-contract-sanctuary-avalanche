...
contract CustomisableTokens is ERC721Drop {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    mapping(uint256 => bool) public isCustomised;
    mapping(uint256 => bool) public tokenCustomisationIsLocked;
    mapping(uint256 => string) private customTokenURI;

    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isCustomised[_tokenId]) {
    ...
    function lockTokenCustomisation(uint256 _tokenId) onlyOwner public {
        tokenCustomisationIsLocked[_tokenId] = true;
    }
    
    function setCustomTokenURI(uint256 _tokenId, string memory _tokenURI) onlyOwner public {
        require(!tokenCustomisationIsLocked[_tokenId], "Token customisation is locked");
        require(ownerOf(_tokenId) == msg.sender, "You are not the owner of the token.");
        isCustomised[_tokenId] = true;
        customTokenURI[_tokenId] = _tokenURI;
    }
}