...
contract SwapToken is Operator {
    ...
    modifier onlyAcceptToken(address token) {
        require(acceptTokens[token].isAdded, "SwapToken: not accept this token");
        _;
    }

    modifier onlyOpen() {
        require(isPause == false, "SwapToken: in pause state");
        _;
    }
    ...
    {
        require(amount > 0, "SwapToken: invalid swap amount");
    ...
    function setAcceptTokens(address token, uint256 scale)
        external
        onlyOperator
    {
        acceptTokens[token].isAdded = true;
