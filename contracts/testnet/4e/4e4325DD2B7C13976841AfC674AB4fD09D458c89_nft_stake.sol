...
contract nft_stake {
    ...
    function calc(address _account) public view returns (uint[3] memory) {
        uint256 time = block.timestamp;
        uint va;
    	uint256[3] memory nums = [va,va,va];
    	uint256 num;
	va = 0;
	num = stakes_1[_account].amount;
	    	for (uint i=0; i < num; i++){
			if ((stakingTime_1[_account][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[0] = va;
		...
		num = stakes_2[_account].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_2[_account][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[1] = va;
		...
		num = stakes_3[_account].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_3[_account][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[2] = va;		
