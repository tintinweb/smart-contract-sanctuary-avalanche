    ...
    HasRouter
{
    ...
    IERC20 public MedalToken;

    mapping(address => Asset) public assets;

    uint256 public startDay;
    uint256 public endDay;
    uint256 public discountPercent = 1000; // 10.00% discount
    uint256 public totalVested = 0;
    uint256 public constant SECOND_IN_DAY = 1 days;
    ...
    modifier onlyAsset(address token) {
        require(
            ...
            "RebateROI: token is not a bondable asset"
        );
        _;
    }
    ...
    function bond(
        address token,
        uint256 amount
    ) external onlyAsset(token) onlyOpen notInBlackList(msg.sender) {
        require(amount > 0, "RebateROI: invalid bond amount");

        _verifyEnoughMainToken(token, amount);
        address _sender = msg.sender;
        IERC20(token).safeTransferFrom(_sender, address(this), amount);
        uint256 discountAmount = (amount * (PERCENTAGE + discountPercent)) / PERCENTAGE;
        uint256 amountForLP = discountAmount / 2;
        uint256 sideTokenAmount = (token == address(SideToken))
            ? amountForLP
            : _buySideToken(token, amountForLP);

        uint256 lpTokenAmount = _addLPFromSideToken(sideTokenAmount);
        totalVested += lpTokenAmount;
        IUniswapV2Pair LpToken = IUniswapV2Pair(FACTORY.getPair(address(MainToken), address(SideToken)));

        LpToken.approve(address(MedalToken), lpTokenAmount);
        uint256 mpBefore = MedalToken.balanceOf(address(this));
        ILPNode(address(MedalToken)).deposit(lpTokenAmount);
        uint256 mpAfter = MedalToken.balanceOf(address(this));

        uint256 mpAmount = mpAfter - mpBefore;
        MedalToken.safeTransfer(_sender, mpAmount);
     ...
     * --------------------
     */
    function setMedalToken(IERC20 medalToken) external onlyOperator {
        MedalToken = medalToken;
    }
    ...
    // Set main token
    function setMainToken(IERC20 mainToken) external onlyOperator {
        MainToken = mainToken;
    }

    // Set side token
    function setSideToken(IERC20 sideToken) external onlyOperator {
        SideToken = sideToken;
    }
     ...
     * ------------------
     */
    function _verifyEnoughMainToken(
        address token,
        uint256 amount
    ) internal view {
        uint256 discountAmount = (amount * (PERCENTAGE + discountPercent)) / PERCENTAGE;
        ...
        if (token == address(SideToken)) {
            amountBDesired = amountForLP;
        } else {
            Asset memory asset = assets[token];
            uint256[] memory tokenAmount = ROUTER.getAmountsOut(
                amountForLP,
                asset.path
            );
            amountBDesired = tokenAmount[asset.path.length - 1];
        }

        IUniswapV2Pair LpToken = IUniswapV2Pair(FACTORY.getPair(address(MainToken), address(SideToken)));
        (uint256 reserve0, uint256 reserve1, ) = LpToken.getReserves();

        if (LpToken.token0() == address(SideToken)) {
            uint256 reserve2 = reserve0;
            reserve0 = reserve1;
            reserve1 = reserve2;
        }

        uint256 amountADesired = (amountBDesired * reserve0) / reserve1;
        uint256 mainBalance = MainToken.balanceOf(address(this));

        require(
            mainBalance >= amountADesired,
            "RebateROI: not enough balance"
        );
    }
    ...
    function _getLpAmount(
        ...
        if (address(SideToken) == token0) {
            amountADesired = amountBDesired;
            amountBDesired = (amountADesired * reserve1) / reserve0;
        }

        uint256 liquidityForLpA = (amountADesired * totalSupply) / reserve0;
        uint256 liquidityForLpB = (amountBDesired * totalSupply) / reserve1;

        return
            (liquidityForLpA > liquidityForLpB)
                ? liquidityForLpB
                : liquidityForLpA;
    }

    function _buySideToken(
        address token,
        uint256 amountIn
    ) internal returns (uint256) {
        IERC20(token).safeIncreaseAllowance(address(ROUTER), amountIn);
    ...
    function _addLPFromSideToken(
        uint256 sideTokenAmount
    ) internal returns (uint256) {
        uint256 mainTokenAmount = MainToken.balanceOf(address(this));
        MainToken.safeIncreaseAllowance(address(ROUTER), mainTokenAmount);
        SideToken.safeIncreaseAllowance(address(ROUTER), sideTokenAmount);
     ...
     * --------------
     */
    function getEstimateMedalToken(
        address token,
        uint256 amount
        ...
        uint256 lpAmount = _getLpAmount(amountBDesired);

        return ILPNode(address(MedalToken)).getEstimateMedalToken(lpAmount);
    }
