    ...
    Ownable
{
    ...
    uint256 private _maxRandomizer = 21000;
    ...
    function randomNumber() internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % _maxRandomizer;
    }
    ...
    function internalMint(address to, uint256 amount) internal {
        ...
        for (uint256 i = 1; i <= amount; i++) {  
            ...
            if (tokenID <= 1000 && !won[0] && remaining1 > 0) {  //5.85 chance to win the best prize
                contract1.mint(1,to);
                won[0] = true;
                remaining1 -= 1;
            } else if ( 1000 < tokenID && tokenID <= 4000 && remaining2 > 0) { //17.55 chance to win second best
                contract2.mint(1,to);
                won[1] = true;
                remaining2 -= 1;
            } else if ( 10000 < tokenID && tokenID <= 16000 && remaining3 > 0) { // chance to win third best
                contract3.mint(1,to);
                won[2] = true;
                remaining3 -= 1;
            }else if (19900 < tokenID && tokenID <= 20000 && remaining4 > 0){ // chance to win 1 avax
                payable(to).transfer(1 ether);
                won[3] = true;
                remaining4 -= 1;
            }else if(20000 < tokenID && !avaSharkWon){
