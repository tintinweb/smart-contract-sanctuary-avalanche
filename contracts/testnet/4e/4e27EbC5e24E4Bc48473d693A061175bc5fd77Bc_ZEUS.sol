...
contract NODERewardManagement {
    ...
    function createNode(
        ...
        totalNodesCreated++;
        _addReferral(account, referred_by);
    ...
    function _transferNode(
        ...
        //check if user has a node is available;
        require(newOnwer != address(0), "New owner address is invalid");
        NodeEntity[] storage nodesOfPrev = _nodesOfUser[prevOwner];
        NodeEntity memory node = _getNodeWithCreatime(
            nodesOfPrev,
            _creationTime
        );

        uint256 numberOfNodes = nodesOfPrev.length;
        int256 index = binary_search(
            nodesOfPrev,
            0,
            numberOfNodes,
            _creationTime
        ...
        _burn(validIndex);
        _nodesOfUser[newOnwer].push(node);
        nodeOwners.set(newOnwer, _nodesOfUser[newOnwer].length);
    }
    ...
    function _addReferral(address referrer, address referred_by)
        ...
        onlySentry
    {
        NodeEntity[] memory nodes = _nodesOfUser[referrer];
        NodeEntity[] memory nodes2 = _nodesOfUser[referred_by];
        if (referrer != referred_by && nodes.length <= 1 && nodes2.length > 0) {
            bool alreadyExists = referred_by == _referrals[referrer];
            if (alreadyExists) {
                _addRefferralBonus(referrer);
            } else {
                _referrals[referrer] = referred_by;
                userReferrals[referred_by].push(referrer); //keep track of referrals
                _addRefferralBonus(referrer);
            }
        }
    }
    ...
    function _addRefferralBonus(address referred) public onlySentry {
        // check if referrer has a node .
        //   rewardAvailable
        address referred_by = _referrals[referred];
        NodeEntity[] storage nodes = _nodesOfUser[referred_by];
        if (nodes.length > 0) {
            // get ref amount based on node created
            uint256 amount = referralBonusPercent.mul(nodePrice).div(100);
            referralHistory[referred_by][referred] = amount;
            availableReferralBonus[referred_by] += amount;
        }
    }
    ...
    function _compoundNode(
        ...
        require(created_at > 0, "NODE: CREATIME must be higher than zero");
        NodeEntity[] storage nodes = _nodesOfUser[account];
        uint256 numberOfNodes = nodes.length;
        require(
            numberOfNodes > 0,
        ...
        NodeEntity storage node = _getNodeWithCreatime(nodes, created_at);
        uint256 nodeReward = node.rewardAvailable;
        require(nodeReward > nodePrice, "Not enough to compound");
        node.rewardAvailable -= nodePrice;
        address referred_by = _referrals[account];
        createNode(account, nodeName, referred_by, nodeType);
    }
    ...
    function _getUserNodeTypes(address account)
        external
        view
        returns (string memory)
    {}
...
contract ZEUS is ERC20, PaymentSplitter {
    using SafeMath for uint256;
    ...
    address public futurUsePool = 0x664FEBa636D13f9FE27b959cCef2541590C253dd;
    address public distributionPool =
        0x2B6935FA50384956C5475E17A197F48896EaaBD8;
    IERC20 public token = IERC20(0x764a4A6f3326559b6Ef5F3F186AD1094FA044c43);
    ...
    uint256 public roi;
    uint256 public roiBonusPercent;
    uint256 public roiTaxPercent;
    uint256 public compoundTaxPercent;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        require(token.approve(address(uniswapV2Router), tokenAmount), "Approve failed");
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        require(token.approve(address(uniswapV2Router), tokenAmount), "Approve failed");
        require(token.allowance(address(this), address(uniswapV2Router)) >= tokenAmount, "Allowance insufficient");
    ...
    function createNodeWithTokens(
        ...
        uint256 nodePrice = nodeRewardManager.getNodePriceByType(nodeType);
        require(
        ...
        if (
            ...
            // token.transfer(address(this), futurTokens);
        ...
        for (uint256 i = 0; i < total; i++) {
            string memory name_ = string(
                abi.encodePacked(name, "_", Strings.toString(i + 1))
            );
            nodeRewardManager.createNode(sender, name_, referred_by, nodeType);
        }
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        uint256 rewardAmount = nodeRewardManager._cashoutNodeReward(
            sender,
            blocktime
        );
        require(
        ...
        // distr pool needs to give contract access to funds 
        require(allowance(distributionPool, address(this)) >= rewardAmount, "Distribution allowance required");
        token.transferFrom(distributionPool, address(this), rewardAmount);
    ...
    function transferNode(uint256 _creationTime, address newOnwer) external {
        address sender = _msgSender();
        require(
            !_isBlacklisted[sender] && !_isBlacklisted[newOnwer],
            "NODE CREATION: Blacklisted address"
        );
        nodeRewardManager._transferNode(sender, _creationTime, newOnwer);
    }

    function setCompoundTaxPercent(uint256 value) external onlyOwner {
        compoundTaxPercent = value;
    }

    function setRoi(uint256 value) external onlyOwner {
        roiBonusPercent = value;
    }

    function setRoiBonusPercent(uint256 value) external onlyOwner {
        roi = value;
    }

    function setRoiTaxPercent(uint256 value) external onlyOwner {
        roiTaxPercent = value;
    }
    ...
    function compoundNode(
        ...
        uint256 cmpTax = nodePrice.mul(compoundTaxPercent).div(100);
        uint256 total = nodePrice.add(cmpTax);
        address sender = _msgSender();
        require(
            sender != address(0),
            "NODE CREATION:  creation from the zero address"
        ...
        require(
            token.balanceOf(sender) >= total,
            "NODE CREATION: Balance too low for compounding."
        );
        token.transfer(address(this), total);
        nodeRewardManager._compoundNode(sender, created_at, nodeName, nodeType);
    }
    ...
    function setReferralBonusPercent(uint256 amount) external onlyOwner {
    ...
    function getUserNodeTypes()
        external view
        returns (string memory)
    {
        return nodeRewardManager._getUserNodeTypes(_msgSender());
    }
}