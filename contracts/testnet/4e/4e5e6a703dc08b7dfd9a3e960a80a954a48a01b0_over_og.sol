  ...
  Authorizable
{
  ...
  address stable_add = 0x4E47Cd2a94dB858f599A55A534543ddAc30AFeC2;
  bool stableOn = true;
  ...
  function adds(address add) public view returns(address[2] memory) {
      IUniswapV2Pair _pair = IUniswapV2Pair(add);
      IERC20 swap_token0 = IERC20(_pair.token0());
      IERC20 swap_token1 = IERC20(_pair.token1());
  ...
  function getTokenPrice() public view returns(uint[7] memory)
   {
    ...
    return [_token,_main,boo,now_val,low_val,high_val,doll_pr];// return amount of token0 needed to buy token1
   }
    function getEm() public view returns (uint) {
    	uint[7] memory res = getTokenPrice();
    	uint a = res[2]; // return amount of token0 needed to buy token1
    	return a;
    }
    function getAdd() public view returns (address[2] memory) {
      IUniswapV2Pair _pair = IUniswapV2Pair(swap_add);
      IERC20 swap_token0 = IERC20(_pair.token0());
      IERC20 swap_token1 = IERC20(_pair.token1());
      return [address(swap_token0),address(swap_token1)];
      }
