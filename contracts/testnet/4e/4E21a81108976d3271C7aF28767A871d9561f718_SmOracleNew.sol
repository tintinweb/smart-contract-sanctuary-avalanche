...
contract SmOracleNew is Ownable {
    ...
    mapping(bytes32 => FloorData) private floorData;
    ...
    mapping(bytes32 => LastTwap) public lastTwaps;
    uint256 public counter = 20;
    uint256 public timeInterval = 3600;
    uint256 public totalNumOfInterval = 50;
    uint256 public twapIndex = 2;
    uint256 public capForTxData = 20;
    uint256 public capForTwap = 20;
    bool public isTraitFilter = false;

    event UpdateTransactionData(bytes32 currencyKey, uint256 timestamp, uint256 blockNumbers, bytes32 transactionHashes, uint256 tokenIds, uint256 prices);
    event UpdateFloorData(bytes32 currencyKey, uint256 timestamp, uint256 floorPrices);
    event SetRareTokenId(bytes32 currencyKey, uint256 tokenIds, bool isRare);
    event SetIsTraitFilter(bool isTraitFilter);
    event SetTimeInterval(uint256 timeInterval);
    event SetTotalNumOfInterval(uint256 totalNumOfInterval);
    event SetCounter(uint256 counter);
    event SetCapForTxData(uint256 capForTxData);
    event SetCapForTwap(uint256 capForTwap);
    event SetInitialDailyAveragePrice(bytes32 currencyKey, uint256 initialDailyAveragePrice);
    event SetInitialTwap(bytes32 currencyKeys, uint256 twaps);
    event SetRegressionStats(bytes32 currencyKey);
    event SetAttributeRarityScore(bytes32 currencyKey, int256 attributeRarityScore, uint256 tokenId);

    constructor(bytes32[] memory _currencyKeys, uint256[] memory _twaps) public {
        for (uint256 i = 0; i < _currencyKeys.length; i++) {
            lastTwaps[_currencyKeys[i]].price = _twaps[i];
            lastTwaps[_currencyKeys[i]].isInitialized = true;
        }
    }
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _prices.length; i++) {
            ...
            // include data within certain range of twap
            if (transactionData[_currencyKey].prices.length >= totalNumOfInterval.mul(2).add(30)) {
                uint256 twap = getTwap(_currencyKey);
                if (_prices[i] > twap * (100 + capForTxData) / 100 || _prices[i] < twap * (100 - capForTxData) / 100 ) {
                    continue;
                }
            }
            ...
            if (transactionData[_currencyKey].prices.length >= totalNumOfInterval.mul(2).add(30)) {
                lastTwaps[_currencyKey].price = getTwap(_currencyKey);
            }
            emit UpdateTransactionData(_currencyKey, _timestamp[i], _blockNumbers[i], _transactionHashes[i], _tokenIds[i], _prices[i]);
        }
    }

    function updateFloorData(bytes32 _currencyKey, uint256[] memory _timestamp, uint256[] memory _floorPrices) public {
        require(
            _floorPrices.length == _timestamp.length, "length of 2 input arrays not align"
        );
        for (uint256 i = 0; i < _floorPrices.length; i++) {
            require(block.timestamp >= _timestamp[i], "floor price timestamp cannot be larger than current block timestamp");
            if (floorData[_currencyKey].timestamp.length > 0) {
                require(_timestamp[i] > floorData[_currencyKey].timestamp[ floorData[_currencyKey].timestamp.length - 1 ], "do NOT update old floor price");
            }
            floorData[_currencyKey].timestamp.push(_timestamp[i]);
            floorData[_currencyKey].floorPrices.push(_floorPrices[i]);
            emit UpdateFloorData(_currencyKey, _timestamp[i], _floorPrices[i]);
        }
    }
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            emit SetRareTokenId(_currencyKey, _tokenIds[i], _isRare[i]);
        }
    }

    function setIsTraitFilter(bool _isTraitFilter) external onlyOwner {
        isTraitFilter = _isTraitFilter;
        emit SetIsTraitFilter(_isTraitFilter);
    }

    function setTimeInterval(uint256 _timeInterval) external onlyOwner {
        timeInterval = _timeInterval;
        emit SetTimeInterval(_timeInterval);
    }

    function setTotalNumOfInterval(uint256 _totalNumOfInterval) external onlyOwner {
        totalNumOfInterval = _totalNumOfInterval;
        emit SetTotalNumOfInterval(_totalNumOfInterval);
    }

    function setCounter(uint256 _counter) external onlyOwner {
        counter = _counter;
        emit SetCounter(_counter);
    }

    function setCapForTxData(uint256 _capForTxData) external onlyOwner {
        capForTxData = _capForTxData;
        emit SetCapForTxData(_capForTxData);
    }

    function setCapForTwap(uint256 _capForTwap) external onlyOwner {
        capForTwap = _capForTwap;
        emit SetCapForTwap(_capForTwap);
    }
    ...
    function setInitialDailyAveragePrice(bytes32 _currencyKey, uint256 _initialDailyAveragePrice) external onlyOwner {
        ...
        emit SetInitialDailyAveragePrice(_currencyKey, _initialDailyAveragePrice);
    }

    function setInitialTwap(bytes32[] memory _currencyKeys, uint256[] memory _twaps) external onlyOwner {
        for (uint256 i = 0; i < _currencyKeys.length; i++) {
            require(lastTwaps[_currencyKeys[i]].isInitialized == false, "twap cannot be re-initialized");
            lastTwaps[_currencyKeys[i]].price = _twaps[i];
            lastTwaps[_currencyKeys[i]].isInitialized = true;
            emit SetInitialTwap(_currencyKeys[i], _twaps[i]);
        }
    }
    ...
    function setRegressionStats(bytes32 _currencyKey, int256[] memory _coefficients, int256 _intercept) external onlyOwner {
        ...
        emit SetRegressionStats(_currencyKey);
    }
    ...
    function setAttributeRarityScore(bytes32 _currencyKey, int256[] memory _attributeRarityScore, uint256[] memory _tokenId) external onlyOwner {
        ...
        for (uint256 i = 0; i < _attributeRarityScore.length; i++){
            ...
            emit SetAttributeRarityScore(_currencyKey, _attributeRarityScore[i], _tokenId[j]);
        }
    }
    ...
    function getTwap(bytes32 _currencyKey) public view returns(uint256) {
        if (twapIndex == 1) {
            ...
            require(transactionData[_currencyKey].prices.length >= totalNumOfInterval.mul(2).add(30), "not enough price data for twap");
            uint256 twap = 0;
            ...
            // equal weighting for all average prices
            if (each_interval_price_counter == 0) {
                twap = lastTwaps[_currencyKey].price;
            } else {
                twap = each_interval_price_sum / each_interval_price_counter;
            }
            if (floorData[_currencyKey].floorPrices.length > 0) {
                if (block.timestamp - floorData[_currencyKey].timestamp[ floorData[_currencyKey].timestamp.length - 1 ] > 1800) {
                    return twap;
                } else {
                    uint256 latestFloorPrice = floorData[_currencyKey].floorPrices[ floorData[_currencyKey].floorPrices.length - 1 ];
                    uint256 upperCap = latestFloorPrice * (100 + capForTwap) / 100;
                    uint256 lowerCap = latestFloorPrice * (100 - capForTwap) / 100;
                    if (twap > upperCap ) {
                        return upperCap;
                    } else if (twap < lowerCap) {
                        return lowerCap;
                    } else {
                        return twap;
                    }
    ...
    function getFloorData(bytes32 _currencyKey, uint256 _index) public view returns(uint256, uint256) {
        return (
            floorData[_currencyKey].timestamp[_index],
            floorData[_currencyKey].floorPrices[_index]
        );
    }

    function getTokenInfo(bytes32 _currencyKey, uint256 _tokenId) public view returns(bool, uint256) {
        return ( 
    ...
    function getAttributeRarityScore(bytes32 _currencyKey, uint256 _tokenId, uint256 _index) public view returns(int256) {
        return (
