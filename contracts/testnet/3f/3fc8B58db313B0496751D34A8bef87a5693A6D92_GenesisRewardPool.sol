...
contract GenesisRewardPool is ContractGuard, Ownable {
    ...
    uint256 public mainTokenPerSecond = 0.1475694 ether; // 38250 Token / (72h * 60min * 60s)

    uint256 public constant DAO_FUND_REWARDS = 6750 ether;
    uint256 public mainTokenPerSecondForDaoFund = 0.0260416 ether; // 6750 Token / (72h * 60min * 60s)
    uint256 lastDaoFundRewardTime;

    address public airdropWallet;
    uint256 public airdropTipPercent = 20;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public onlyOneBlock {
        ...
        if (_reward > 0) {
            ...
            uint256 userReward = _reward.sub(airdropTip);
            if (userReward > 0) {
                safeMainTokenTransfer(_sender, userReward);
                emit RewardPaid(_sender, userReward);
            }
        }
    ...
    function safeMainTokenTransfer(address _to, uint256 _amount) internal {
        uint256 _mainTokenBalance = IERC20(mainToken).balanceOf(address(this));
        if (_mainTokenBalance > 0) {
            if (_amount > _mainTokenBalance) {
                IERC20(mainToken).safeTransfer(_to, _mainTokenBalance);
            } else {
                IERC20(mainToken).safeTransfer(_to, _amount);
            }
        }
    }
