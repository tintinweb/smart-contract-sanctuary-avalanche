...
import "./internal/BusinessAddresses.sol";
import "./internal/BaseData.sol";
import "./internal/TransferFee.sol";
import "./internal/Withdrawable.sol";
import "./internal/ERC4907.sol";
import "./internal/Signable.sol";

contract GenesisConvert is
    ERC4907,
    ERC721Burnable,
    Ownable,
    BusinessAddresses,
    ...
    Withdrawable
{
    ...
    event UpgradeNftLevel(address indexed user, uint256 tokenId, uint256 nftLevel);
    ...
    string private constant REVERT_NOT_AN_OWNER = "Manager: caller is not the NFT's owner";
    ...
    string private constant REVERT_INVALID_LEVEL = "NFT: Invalid level";
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        require(_msgSender() == ownerOf(tokenId), REVERT_NOT_AN_OWNER);
        _;
    }
    ...
    constructor() ERC4907("TestGenesisConvert", "TGC") {
        ...
        _setUpgradeFeeToken(0x2F7265b97F6655F78AbBf13168e1FB4749A03bd0);
        _configTypeAndRarityAndLevel(4, 3, 30);
        _configBasicPerformance(1, 1, 10);
        _configBasicPerformance(2, 11, 20);
        _configBasicPerformance(3, 21, 30);
        _setBaseAmount(100 ether);
        _setFloorCoefficient(1);
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(tokenId, abi.encode(meta), timestamp));
    }
    ...
    function convert(
        ...
        bytes32 messageHash = getMessageHash(tokenId, meta, timestamp);
        require(permit(messageHash, timestamp, v, r, s), "Invalid signature");
        _batchMint(account, creator, meta);
    }

    function upgradeLevel(uint256 tokenId) external onlyTokenOwner(tokenId) nonReentrant {
        Metadata storage nftMeta = _metadata[tokenId];
        require(nftMeta.nftLevel < maxLevel, REVERT_INVALID_LEVEL);
        address account = _msgSender();
        uint256 total = getUpgradeFee(nftMeta.nftLevel + 1);
        require(IERC20(upgradeFeeToken).transferFrom(account, feeBeneficiary, total), REVERT_INSUFFICIENT_BALANCE);

        nftMeta.nftLevel += 1;
        emit UpgradeNftLevel(account, tokenId, nftMeta.nftLevel);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, ERC4907) returns (bool) {
...
abstract contract BusinessAddresses is Ownable {
    ...
    function viewBusinessLists(uint256 cursor, uint256 size)
        external
        view
        returns (address[] memory businessAddresses, uint256)
    {
        uint256 length = size;
...
contract BaseData is Ownable {
    event SetUnboxNFTLimit(uint256[] rarities, uint256[] minValues, uint256[] maxValues);
    event SetBaseAmount(uint256 baseAmount);
    event SetUpgradeFeeToken(address token);

    address public upgradeFeeToken;
    uint256 public maxType; // 1 -> maxType
    uint256 public maxRarity; // maxRarity = 3 => rarities[1,2,3] <=> 1: common, 2: uncommon, 3: rare
    uint256 public maxLevel; // 1 -> maxLevel
    uint256 public baseAmount;
    uint256 public floorCoefficient;
    string internal constant REVERT_NULL = "ConfigBaseData: Can not be address 0";
    string internal constant REVERT_INVALID_MAX_LEVEL = "ConfigBaseData: Invalid NFT Level";
    string internal constant REVERT_INVALID_ARRAY = "ConfigBaseData: Array length mismatch";

    struct PerformanceLimit {
        uint256 min;
        uint256 max;
    }

    mapping(uint256 => PerformanceLimit) private basicPerformances;
    ...
    function isValidMeta(
        ...
        if (level < 1 || rarity > maxLevel) return false;
        if (basicPerformance < basicPerformances[rarity].min || basicPerformance > basicPerformances[rarity].max)
            return false;
    ...
    function _setUnboxNFTLimit(
        uint256[] memory rarities,
        uint256[] memory minValues,
        uint256[] memory maxValues
    ) internal {
        require(rarities.length == minValues.length && minValues.length == maxValues.length, REVERT_INVALID_ARRAY);
        for (uint256 i = 0; i < rarities.length; i++) {
            _configBasicPerformance(rarities[i], minValues[i], maxValues[i]);
        }
        emit SetUnboxNFTLimit(rarities, minValues, maxValues);
    }

    function setUnboxNFTLimit(
        uint256[] memory rarities,
        uint256[] memory minValues,
        uint256[] memory maxValues
    ) external onlyOwner {
        _setUnboxNFTLimit(rarities, minValues, maxValues);
    }

    function getUnboxNFTLimitInfo(uint256 rarity)
        public
        view
        returns (
            uint256,
            uint256,
            uint256,
            PerformanceLimit memory
        )
    {
        return (maxType, maxRarity, maxLevel, basicPerformances[rarity]);
    }

    function _configTypeAndRarityAndLevel(
        uint256 _maxType,
        uint256 _maxRarity,
        uint256 _maxLevel
    ) internal {
    ...
    function configTypeAndRarityAndLevel(
        uint256 _maxType,
        uint256 _maxRarity,
        uint256 _maxLevel
    ) external onlyOwner {
        _configTypeAndRarityAndLevel(_maxType, _maxRarity, _maxLevel);
    }

    function _configBasicPerformance(
        uint256 rarity,
        uint256 min,
        uint256 max
    ) internal {
        basicPerformances[rarity] = PerformanceLimit(min, max);
    }

    function _setUpgradeFeeToken(address _token) internal {
        upgradeFeeToken = _token;
        emit SetUpgradeFeeToken(_token);
    }

    function setUpgradeFeeToken(address _token) external onlyOwner {
        require(_token != address(0), REVERT_NULL);
        _setUpgradeFeeToken(_token);
    }

    function _setBaseAmount(uint256 _baseAmount) internal {
        baseAmount = _baseAmount;
    }

    function setBaseAmount(uint256 _baseAmount) external onlyOwner {
        _setBaseAmount(_baseAmount);
    }
    ...
    function _setFloorCoefficient(uint256 _floorCoefficient) internal {
        floorCoefficient = _floorCoefficient;
    }

    function setFloorCoefficient(uint256 _floorCoefficient) external onlyOwner {
        _setFloorCoefficient(_floorCoefficient);
    }

    function getFloorCoefficient() public view returns (uint256) {
        return floorCoefficient;
    }

    function getUpgradeFee(uint256 level) public view returns (uint256) {
        if (level > 2 && level <= maxLevel) return (baseAmount * (floorCoefficient + level - 2)) / 10;
        return 0;
    }
}
