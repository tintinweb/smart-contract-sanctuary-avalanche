...
contract VMShips is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	address private VMNAddr; 
	address private VMCollectionAddress;
	...
	constructor() ERC721("VM Ships", "VMS") {
		paused = true;
		revealed = false;
		allowListings = true;
		...
		communityAddress = 0x4057F6d0C49358B88A318DCb230f7D3140fA78de;
		wavaxAddr = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
		VMNAddr = 0xBbD9786f178e2AEBb4b4329c41A821921ca05339;
		VMCollectionAddress = 0xBbD9786f178e2AEBb4b4329c41A821921ca05339;
	...
	function mint(uint256 amount) public payable nonReentrant {
		...
		uint256 proceeds = 0;
		if (verifyUserForHoldingVMCollection(msg.sender)) {
	...
	function airDropVMOwner(uint256 _lastToken) public onlyOwner {
		...
		require(_lastToken <= 1444, "Last token must be 1444 or less.");

		IERC721 VMContract = IERC721(VMNAddr);
	...
	function  verifyUserForHoldingVMCollection(address _user) public view returns (bool) {
		return IERC721(VMCollectionAddress).balanceOf(_user) > 0 ? true : false;
	}
	...
	function fulfillListing(uint256 tokenID) public payable nonReentrant {
		...
		(bool success1, ) = payable(listing.seller).call{ value: msg.value - (royaltiesValue + salesRewardValue) }("");
    ...
    function setVMNAddr (address _contract) public onlyOwner {
		VMNAddr = _contract;
	}

	function setVMCollectionAddr (address _contract) public onlyOwner {
		VMCollectionAddress = _contract;
	}
	...
	receive() external payable {
		...
		uint256 fraction = msg.value / 2;

		(bool success1, ) = payable(communityAddress).call{ value: fraction }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to community wallet!");

		(bool success3, ) = payable(owner()).call{ value: fraction }("");
