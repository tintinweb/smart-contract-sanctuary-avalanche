...
contract DrawerV2 is Ownable {
  ...
  function getRoundSVG(
    ...
    uint16 y = 720;
    for (; i < 8; i++) {
      svg = string(abi.encodePacked(svg, getTeamHead(x, y), teamNames[i], getLine(i, x, y, scores[i / 2], winScore)));
      x += 180;
    }
    ...
    for (; i < 12; i++) {
      svg = string(
        ...
        abi.encodePacked(svg, getTeamHead(x, y), teamNames[i], getLine(i, x, y, scores[i / 2], winScore))
      );
      x += 360;
    }
    ...
    y = 320;
    for (; i < 14; i++) {
      svg = string(abi.encodePacked(svg, getTeamHead(x, y), teamNames[i], getLine(i, x, y, scores[6], winScore)));
      x += 720;
    }
        ...
        abi.encodePacked(
          ...
          Base64.encode(
            abi.encodePacked(
              ...
              getRoundSVG(size / 2, winner, teamNames, scores),
              '","attributes":',
              attributes,
              "}"
            )
