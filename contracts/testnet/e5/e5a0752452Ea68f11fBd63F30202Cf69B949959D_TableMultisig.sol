...
contract TableMultisig is ERC1155, ReentrancyGuard, Multicall {
    ...
    /*///////////////////////////////////////////////////////////////
														DAO STORAGE
		//////////////////////////////////////////////////////////////*/
    uint256 public constant SHEILD = 0;
    ...
    function propose(
        ...
        if (proposalType == ProposalType.TYPE)
            if (amounts[0] > 10 || amounts[1] > 3 || amounts.length != 2) revert TypeBounds();
        // TODO correct for full members length
        if (proposalType == ProposalType.MINT)
            if (accounts.length > 12) revert MemberLimitExceeded();

        bool selfSponsor;

        // if member or extension is making proposal, include sponsorship
        if (balanceOf[msg.sender][0] != 0 || extensions[msg.sender]) selfSponsor = true;
    ...
    function sponsorProposal(uint256 proposal) public virtual nonReentrant {
        ...
        if (balanceOf[msg.sender][0] == 0) revert NotMember();
    ...
    function _vote(
        ...
        if (balanceOf[signer][0] == 0) revert NotMember();
