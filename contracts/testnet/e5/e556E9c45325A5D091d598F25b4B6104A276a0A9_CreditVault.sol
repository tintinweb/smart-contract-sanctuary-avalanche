...
contract CreditVault is AccessControlEnumerable, Pausable, EIP712 {
    ...
    function _withdraw(
        ...
        if (ercVersion == 1) {
            require(tokenId == 0, "CreditVault: invalid tokenId");
            IERC20(token).transferFrom(superVault, account, amount);
        } else if (ercVersion == 2) {
            require(amount == 1, "CreditVault: invalid amount");
            IERC721(token).safeTransferFrom(superVault, account, tokenId);
        } else if (ercVersion == 3) {
            IERC1155(token).safeTransferFrom(superVault, account, tokenId, amount, "0x");
        }
    }
