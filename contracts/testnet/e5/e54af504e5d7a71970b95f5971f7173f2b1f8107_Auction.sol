...
contract Auction is Ownable {
    ...
    uint256 public biddingIncrementStep;
    uint256 public auctionTime;

    constructor() {
        token = IERC20(0x3d60a6A3DBcAC4d5C2B9a262E24A2F6991B9817f);
        biddingIncrementStep = 50 * (10**token.decimals());
        auctionTime = 30 minutes;
    }

    function setBiddingIncrementStep(uint256 _amount) public onlyOwner {
        biddingIncrementStep = _amount;
    }

    function setAuctionTime(uint256 _time) public onlyOwner {
        biddingIncrementStep = _time;
    }
    ...
    function createAuction(
        ...
        auctions[auctionNonce].endTime = block.timestamp + auctionTime;
        ...
        returns (uint256)
    {
        return
            (auctions[_auctionId].bidCount > 0)
                ? auctions[_auctionId].lastBidPrice + biddingIncrementStep
                : auctions[_auctionId].lastBidPrice;
    }
        ...
        returns (uint256)
    {
        ...
        return ((auctions[_auctionId].lastBidPrice + biddingIncrementStep) -
            auctions[_auctionId].biddingAmounts[
