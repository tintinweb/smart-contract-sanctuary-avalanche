...
contract PXTV3 is ERC20Upgradeable, IBridgeToken {
    // STRINGS
    ...
    function initialize() public virtual initializer {
        ...
        priceFeed = AggregatorV3Interface(0x5498BB86BC934c8D34FDA08E81D444153d0D06aD); // AVAX / USDT
        usdtAddress = 0x57C5c0a1ccAC1A28f9B165d0DdF42feaa0796C25; // USDT PANGOLIN
    ...
    returns (uint256)
    {
        ...
        if (antiWhaleUsdtLimit1 > 0) {
            ...
            uint256 usdtAmount = getAmountOut(
                contractAddress,
                usdtAddress,
                amount
            );
    ...
    function getAmountOutUSD(address add1, address add2, uint256 _amount) public view returns (uint256) {
    ...
    function getAmountOut(address add1, address add2, uint256 _amount) public view returns (uint256) {
        if (address(dexRouter) == address(0)) return 0;
    ...
    function getAmountOut(address add1, address add2, uint256 _amount) public view returns (uint256) {
        if (address(dexRouter) == address(0)) return 0;
        address[] memory path = new address[](3);
        path[0] = address(contractAddress);
        path[1] = add1;
        path[2] = usdtAddress;
        uint256[] memory amountsOut = dexRouter.getAmountsOut(_amount, path);
        return amountsOut[1];
    ...
    */
    ...
    returns (uint256)
    {
        ...
        if (availableChainlinkNetworks[network]) {
            ...
            avaxPrice = estimatedTokensForTokens(
                dexRouter.WAVAX(),
                usdtAddress,
                1000000000000000000 // 1 avax
            );
        }
    ...
    function estimatedTokensForTokens(
        ...
        return
        dexRouter.getAmountsOut(amount, pathTokensForTokens(add1, add2))[
        1
        ];
    }
