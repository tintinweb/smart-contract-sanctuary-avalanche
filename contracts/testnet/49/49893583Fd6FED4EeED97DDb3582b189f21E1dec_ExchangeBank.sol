...
contract ExchangeBank is IExchange, ReentrancyGuard {
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        require(token.allowance(seller, address(this)) >=totalPledgedToken,"please approve exchange to sell");
    ...
    //     ) = _IMarginLoan.getTotalLoanOfPledgedToken(borrower,address(token), seller);
    ...
    //                 remainingAmount = totalAmount - loanAmounts[i];
    ...
    //                 // stableCoin.transferFrom(seller, banks[i], loanAmounts[i]);
    //                 _IMarginLoan.updatePledgeLoan(seller, ids[i], 0,0, 1);
