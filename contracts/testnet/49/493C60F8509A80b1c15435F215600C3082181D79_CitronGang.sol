...
// File contracts/PikaPikAVAX.sol
pragma solidity ^0.8.0;

contract CitronGang is Context, IERC20, Ownable {
    ...
    uint256 public maxTxAmountBuy = _tTotal / 40; // 2,5% of supply
    uint256 public maxTxAmountSell = _tTotal / 40; // 2,5% of supply
    ...
    address payable public shillAddress;

    mapping(address => bool) public isAutomatedMarketMakerPair;

    string private constant _name = "CitronGang";
    string private constant _symbol = "CITRON";
    ...
    feeRatesStruct public buyRates =
        feeRatesStruct({
            ...
            shill: 5, // shill team fee in %
            treasury: 5, // treasury fee in %
            lp: 2, // lp rate in %
            toSwap: 12 // treasury + shill + lp
        });
    ...
    event TreasuryAndDevFeesAdded(uint256 shillFee, uint256 treasuryFee);
    ...
    constructor() {
        ...
        shillAddress = payable(msg.sender);
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[treasuryAddress] = true;
        _isExcludedFromFee[shillAddress] = true;
        _isExcludedFromFee[address(this)] = true;

        _isExcludedFromMaxWallet[owner()] = true;
        _isExcludedFromMaxWallet[treasuryAddress] = true;
        _isExcludedFromMaxWallet[shillAddress] = true;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 AVAXToAddLiquidityWith = (deltaBalance * appliedRates.lp) /
            (denominator - appliedRates.lp);

        // add liquidity
        addLiquidity(tokensToAddLiquidityWith, AVAXToAddLiquidityWith);

        // we give the remaining tax to shill & treasury wallets
        uint256 remainingBalance = address(this).balance;
        uint256 shillFee = (remainingBalance * appliedRates.shill) /
            (denominator - appliedRates.shill);
        uint256 treasuryFee = (remainingBalance * appliedRates.treasury) /
            (denominator - appliedRates.treasury);
        shillAddress.sendValue(shillFee);
    ...
    function setBuyFees(
        ...
        buyRates.shill = _shill;
        buyRates.lp = _lp;
        buyRates.toSwap = _treasury + _shill + _lp;
    }
    ...
    function setSellFees(
        ...
        sellRates.shill = _shill;
        sellRates.lp = _lp;
        sellRates.toSwap = _treasury + _shill + _lp;
    }
    ...
    function setDevAddress(address payable _shillAddress) external onlyOwner {
        shillAddress = _shillAddress;
    }
