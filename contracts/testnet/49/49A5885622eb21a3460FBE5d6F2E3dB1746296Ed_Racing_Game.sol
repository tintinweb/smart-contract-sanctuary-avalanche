...
contract Racing_Game is Ownable {
    ...
    uint256 public pricePerRace = 3 ether; // amount of $SHADY tokens - ticket
    ...
    mapping(uint256 => mapping(address => uint256)) public epochNoOfRacesPerUser; // counter of races per user
    ...
    uint256 public raceSignupDuration = 300;
    uint256 public raceDuration = 60;
    uint256 public delayBeforeAndAfterRacing = 20;
    ...
    function getEpochAddressListLength(uint256 _epochId) external view returns (uint256) {
        return epochAddressList[_epochId].length;
    }

    function getEpochRaceIdsLength(uint256 _epochId) external view returns (uint256) {
    ...
    function isRacingCooldown(address _address) public view returns (bool) {
        if (epochNoOfRacesPerUser[epochId][_address] < epochRacesWithoutCooldown) {
        ...
        if (userLastRacingTimestamp[_address] + raceCooldownDuration > block.timestamp) {
            // User in cooldown
            return true;
        }
    ...
    function setEpochRacesWithoutCooldown(uint256 _epochRacesWithoutCooldown) public onlyOwner {
        epochRacesWithoutCooldown = _epochRacesWithoutCooldown;
    }

    function setEpochCooldownDelay(uint256 _epochCooldownDelay) public onlyOwner {
        epochCooldownDelay = _epochCooldownDelay;
    }

    function setRaceCooldownDuration(uint256 _raceCooldownDuration) public onlyOwner {
        raceCooldownDuration = _raceCooldownDuration;
    }
