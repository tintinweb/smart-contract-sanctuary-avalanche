...
// File: contracts/HAGA.sol

contract HAGA is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
          require(msg.value >= cost * _mintAmount, "Avax send below price");
          require(!paused,"Mint function is paused");
  ...
  function whitelistMint(address _to, uint256 _mintAmount) public payable {
    ...
    uint256 supply = totalSupply(); 
    uint256 whitelist_tokens_per_address = 1; 

    require(!Whitelistpaused,"Whitelist Mint function is paused");
    require(whitelist[_to], "Not in whitelist");
    require(balanceOf(_to) < whitelist_tokens_per_address,"Address already minted the whitelist token");
  ...
  function addWhitelist(address _newEntry) public onlyOwner {
