...
contract TalentScout is ERC20, Auth, AccessControl {
    ...
    constructor(address _dexRouter, address _distributor)
        Auth(msg.sender)
        ERC20(_name, _symbol, _totalSupply)
    {
        ...
        uint256 initialMintAmount = _totalSupply.sub(presaleSupply);
    ...
    function _transferFrom(
        ...
        uint256 amountReceived = shouldTakeFee(sender)
            ? takeFee(feeType, sender, recipient, amount)
            : amount;
    ...
    function getTotalFee(uint256 _id, bool selling)
        public
        view
        returns (uint256)
    {
        if (launchedAt + 1 >= block.number) {
            return fees[_id].feeDenominator.sub(1);
        }
        if (selling) {
            return getMultipliedFee(_id);
        }
    ...
    function takeFee(
        ...
        uint256 feeAmount = amount.mul(getTotalFee(_id, receiver == pair)).div(
            fees[_id].feeDenominator
        );
    ...
    function swapBack(uint256 _id) internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(
            ...
            ? 0
            : fees[_id].liquidityFee;
        uint256 amountToLiquify = swapThreshold
            .mul(dynamicLiquidityFee)
            .div(fees[_id].totalFee)
            .div(2);
        ...
        uint256 totalAVAXFee = fees[_id].totalFee.sub(
        ...
        uint256 amountAVAXReflection = amountAVAX
            .mul(fees[_id].reflectionFee)
            .div(totalAVAXFee);
        uint256 amountAVAXMarketing = amountAVAX
            .mul(fees[_id].marketingFee)
            .div(totalAVAXFee);
        uint256 amountAVAXBurned = amountAVAX.mul(fees[_id].burnFee).div(
            totalAVAXFee
        );
        uint256 amountAVAXRewarded = amountAVAX
            .mul(fees[_id].voteRewardFee)
            .div(totalAVAXFee);
        ...
        assert(
            IWETH(WAVAX).transfer(marketingFeeReceiver, amountAVAXMarketing)
        );
    ...
    function setVoteStakingContract(address _voteStakingContract)
        external
        onlyOwner
    {
    ...
    function setFees(
        ...
        fees[_id].totalFee = _liquidityFee
            .add(_reflectionFee)
            .add(_voteRewardFee)
            .add(_burnFee)
            .add(_marketingFee);
    ...
    function insertPresaleLock(
        ...
        require(
            totalSupply() + totalLockedAmount + _amount <= _totalSupply,
    ...
    function releasePresaleAmount(uint256 lock_id) public {
        ...
        totalLockedAmount -= _locks[msg.sender][lock_id].locked;

        // mark the lock as realeased
        _locks[msg.sender][lock_id].released = true;

        // transfer the tokens to the sender
        _transferFrom(address(this), msg.sender, _locks[msg.sender][lock_id].locked);
