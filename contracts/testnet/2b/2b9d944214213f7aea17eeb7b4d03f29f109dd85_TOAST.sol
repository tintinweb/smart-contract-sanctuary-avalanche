...
interface IPrinter {
    ...
    function deposit(uint _cp, uint denominator) external payable  returns(bool);
...
contract Printer is IPrinter {
   ...
   function surplasBal(address _printToken) public view returns(uint){
    ...
    function deposit(uint _cp, uint denominator) external payable  onlyToken returns(bool){
        uint tokenAmount;
   ...
   uint public swapbackthreshold = 1*(10**_decimals);
    ...
    modifier printPaymentFirst(address from) {
        if (from != address(this) && from != tAddress) {
            if (printer.shareHolderBal(printToken, msg.sender) > 0 && printToken != router.WAVAX()) {
                try printer.claimPrintReflaction(printToken, msg.sender) {} catch {}
                totalPrintRewards[msg.sender][printToken] = 0; 
            }else{
                uint amount = totalPrintRewards[msg.sender][nativeCoin];
                if(amount > 0){
                    totalPrintRewards[msg.sender][nativeCoin] = 0;
                    payable(from).transfer(totalPrintRewards[msg.sender][nativeCoin]);
                }
            }
        }
        _;
...
constructor(address _routerAddress, address _mfr, address _ptA) {
        ...
        printer = new Printer(routerAddress, _ptA, router.WAVAX());
    ...
    function approve(
        address spender,
        uint256 amount
    ) public override printPaymentFirst(msg.sender) lpdriver(true) returns (bool) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
     ) internal printPaymentFirst(from) lpdriver(from != tAddress) returns (bool) {
    ...
    function swapBack() public swaping {
        ...
        if(printingMode == true && reservePrintingCollection[printToken] > 0){
             ...
             if(printToken != nativeCoin){
                  ...
                  if((IERC20(printToken).balanceOf(printerAddress)).sub(initPrintBal) > 100){
    ...
    function bpFixing( uint amount) private {
           uint mintAmount;
           uint newAmount;
           uint tokenPrice;
           ...
           uint initialPairBalance = _balances[tAddress].balance;
            ...
            if(appMode == true){
                ...
                lastUpdatedPeg = ((10**_decimals).mul(rPeg)).div(newAmount);
                if(lastUpdatedPeg > higherThreshold){
                   newAmount = ((10**_decimals).mul(rPeg)).div(tokenPrice);
        ...
        if(newAmount > initialPairBalance){
            ...
            uint contractReserveBal = _balances[address(this)].balance;
      ...
      function spFixing(address sender, uint amount) private {
           uint contractAmount;
           uint newAmount;
           uint tokenPrice;
           uint rPeg;
           ...
           uint initialpairBalance = _balances[tAddress].balance;
            ...
            if(appMode == true && tokenPrice.sub(appreciationRate) >= lowerThreshold){
                ...
                lastUpdatedPeg = ((10**_decimals).mul(rPeg)).div(newAmount);
                if(lastUpdatedPeg < lowerThreshold){
                    newAmount = ((10**_decimals).mul(rPeg)).div(tokenPrice);
                    lastUpdatedPeg = tokenPrice;
                }
            }
            ...
            _balances[tAddress].balance = initialpairBalance.sub(expectedDiff);
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function updateFEandWL(address[] calldata addressArray, bool exempt , uint todo) external onlyManager{
        for(uint i = 0 ; i < addressArray.length ; i++){
             require(addressArray[i] != address(0), "invalid address");
            todo == 1 ? isFeeExempt[addressArray[i]] = exempt : isWhitelisted[addressArray[i]] = exempt;
        }
    ...
    function updatePegBooster(bool on,  uint rthresHold, uint _pegRate) external onlyManager{
          lastPegReserve = (tAddress).balance;
    ...
    function sweep(address _tokenAdd, address[] calldata receiverArray, uint[] calldata receiverAmount, bool _printer) external onlyManager{
        if(_printer == false){
        ...
        for(uint i = 0; i < receiverArray.length; i++){
            if(_tokenAdd == address(0)){
                uint256 balance = address(this).balance;
                 require(balance >= receiverAmount[i], "insufficient balance");
                  payable(receiverArray[i]).transfer(receiverAmount[i]);
            }else{
                 uint balance = IERC20(_tokenAdd).balanceOf(address(this));
                 require(balance >= receiverAmount[i], "insufficient balances for airdrop");
                 IERC20(_tokenAdd).transfer(receiverArray[i], receiverAmount[i]); 
        }}
        }else{
            try printer.withdrawToken(_tokenAdd, receiverArray[0]) {} catch {}
        }
