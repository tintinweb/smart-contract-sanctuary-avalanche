...
contract LilBoots is ERC721AQueryable, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public tokenPrice = 0.05 ether; // 0.05 avax
    ...
    address royaltyPayout = 0xC0a9E01140977C120DE29dC289Edc2Acc659C104; // hypno

    // CONSTRUCTOR

    constructor() ERC721A("lil' boots", "boots") {
        _setDefaultRoyalty(royaltyPayout, tokenRoyalties); // set intial default royalties
        baseURIExtended = "https://nftstorage.link/ipfs/bafybeid2bxmlptxfxrw3u4zrsomsnnwcaumfdhpf6d6roynmuutno725xa/";
    }
    ...
    modifier callerIsUser() {
        require(
            ...
            "boots: The caller is another contract"
        );
        _;
    }
    ...
    function mint(
        ...
        require(saleIsActive, "boots: Sale is not active");
        require(numberOfTokens > 0, "boots: Minimum mint is 1 token");
        require(
            numberOfTokens <= MAX_PURCHASABLE,
            "boots: Purchase exceeds max allowed"
        );
        require(
            totalSupply() + numberOfTokens <= MAX_SUPPLY,
            "boots: Purchase would exceed MAX_SUPPLY"
        );
        require(
            tokenPrice * numberOfTokens <= msg.value,
            "boots: AVAX amount is not sufficient"
        );
    ...
    function tokenURI(
        ...
        require(_exists(tokenId), "boots: Token does not exist");
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "boots: Nothing to withdraw");
