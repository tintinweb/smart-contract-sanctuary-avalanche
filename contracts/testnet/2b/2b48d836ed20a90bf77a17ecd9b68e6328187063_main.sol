...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        Send_it(address(this),treasury, checkTXN(address(this),myLib.doPercentage(nodeAmount,overSeerFee)));
        logFees(myLib.doPercentage(nodeAmount,rewardsFee),myLib.doPercentage(nodeAmount,teamPoolFee),myLib.doPercentage(nodeAmount,treasuryFee),myLib.doPercentage(nodeAmount,overSeerFee),Zero,Zero,Zero,Zero,sender,Zero);
