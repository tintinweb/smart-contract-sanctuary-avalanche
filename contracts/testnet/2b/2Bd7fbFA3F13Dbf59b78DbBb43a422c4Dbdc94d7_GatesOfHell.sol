...
// File contracts/OnChainMetadata.sol

// Made by Daniel Abalde - https://github.com/DanielAbalde/NFT-On-Chain-Metadata
...
// File contracts/ERC721OnChainMetadata.sol
...
// File contracts/GatesOfHell.sol


pragma solidity ^0.8.4;
contract GatesOfHell is ERC721OnChainMetadata, Ownable {
    using Strings for uint256;

    // Variable Initialization
    string public baseURI = "https://nftstorage.link/ipfs/bafybeigs7k6br34py4s6huw7e32sefbnqrvdrkqygmlczpez3t56ks6xii/";
    uint256 public MAX_SUPPLY = 100;
    uint256 public _totalSupply = 0;
    mapping(address => bool) public peopleInHell;

    // Runs on Contract Initialization
    constructor() ERC721OnChainMetadata("Gates of Hell", "GOHELL") {
        _addValue(_contractMetadata, key_contract_name, abi.encode("Gates of Hell"));
        _addValue(_contractMetadata, key_contract_description, abi.encode("Ready to connect with Hell?"));
        _addValue(_contractMetadata, key_contract_seller_fee_basis_points, abi.encode(666));
        _addValue(_contractMetadata, key_contract_fee_recipient, abi.encode(_msgSender()));
    }

    modifier isOpen() {
        require(block.timestamp > 1658803735);
        _;
    }
    ...
    function mint(uint256 quantity) public payable isOpen {
        ...
        require(
            quantity == 1,
            "Gates of Hell: You cannot mint that many at once"
        );
        require(tokenId <= MAX_SUPPLY, "Gates of Hell: Project has sold out");
        require(msg.value == 0.0666 ether, "Gates of Hell: Please send the correct value");
        require(!peopleInHell[_msgSender()], "Gates of Hell: You have minted an nft already.");
        _setValue(tokenId, key_token_name, abi.encode(abi.encodePacked("Gate to Hell #", abi.encode(tokenId))));
        _setValue(tokenId, key_token_description, abi.encode("Want to be possessed by the Devil himself? Mint this no-roadmap collection and give me your soul..."));
        _setValue(tokenId, key_token_image, abi.encode(abi.encodePacked(abi.encodePacked(baseURI, abi.encode(tokenId)), ".png")));
        _safeMint(_msgSender(), tokenId);
        peopleInHell[_msgSender()] = true;
        _totalSupply = _totalSupply + 1;
    }

    function changeDescription(uint256 tokenId, string memory description) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Gates of Hell: You are not the owner of this NFT");
    ...
    function withdrawAvax() external onlyOwner {
        ...
        require(
            balance > 0,
            "Gates of Hell: Not enough AVAX on contract to withdraw"
        );
