    ...
    HasRouter
{
    ...
    function setMainToken(IERC20 mainToken) external onlyOperator {
        _setLpToken(address(SideToken), address(mainToken));
        MainToken = mainToken;
    }

    // Set side token
    function setSideToken(IERC20 sideToken) external onlyOperator {
        _setLpToken(address(MainToken), address(sideToken));
        SideToken = sideToken;
    }
    ...
    function _setLpToken(address _tokenA, address _tokenB) internal {
        LpToken = IUniswapV2Pair(FACTORY.getPair(_tokenA, _tokenB));
    }
