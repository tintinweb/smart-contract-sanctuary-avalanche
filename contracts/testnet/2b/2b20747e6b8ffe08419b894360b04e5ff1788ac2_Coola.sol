...
contract Coola {
    ...
    uint256 public constant INVEST_MIN_AMOUNT = 0.01 ether; // 0.1
    uint256 public constant INVEST_MAX_AMOUNT = 500 ether;
    uint256 public constant CLAIM_MAX_AMOUNT = 0.01 ether; // 200
    uint256[] public REFERRAL_PERCENTS = [60, 30, 10];
    uint256 public constant INVEST_FEE = 50;
    uint256 public constant CLAIM_FEE = 80;
    ...
    constructor(address payable wallet) {
        ...
        startUNIX = block.timestamp.add(999 days);
    ...
    function invest(address referrer, uint8 plan) public payable {
        require(startUNIX < block.timestamp, "Project not launched yet");
        ...
        require(
            ...
            "Maximum 100 deposits from single address"
        );
    ...
    function claim() public {
        User storage user = users[msg.sender];
        require(
            user.cooldownCheckpoint <=
                block.timestamp.sub(CLAIM_COOLDOWN.div(96)),
    ...
    {
        return (totalUsers, totalStaked, address(this).balance);
    }
