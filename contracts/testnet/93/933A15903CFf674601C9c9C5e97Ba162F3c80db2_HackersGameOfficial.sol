...
contract HackersGameOfficial is ERC20, ERC721Holder, HackersGameStruct{
    ...
    function unstakeHacker(uint256 hackerNftID) external {
        ...
        //require(hackerFeeTimePaid[hacker[tokenId].ID] > block.timestamp - 30 days);
    ...
    function unstakeCube(uint256 cubeTokenId, uint256 hackerNftId) external {
        ...
        //require(cubeFeeTimePaid[hacker[tokenId].ID] > block.timestamp - 30 days,"Unpaid Fee");
    ...
    function levelUpCube(uint8 cubePart, uint8 _level, uint256 _cubeNFTId) external payable{
        ...
        DynamicCUBE(address(cubeNFT0)).levelUpCube(cube[_cubeId].ID,_cube_levels[cube[_cubeId].ID][1],_cube_levels[cube[_cubeId].ID][2],_cube_levels[cube[_cubeId].ID][3],_cube_levels[cube[_cubeId].ID][4],_cube_levels[cube[_cubeId].ID][5]);
    }
    ...
    function hacking(uint _hackerNFTId,uint _cubeNFTId, uint _job) external {
        ...
        uint _cubeId = stakedCubeId[_cubeNFTId];
        //require(cubeFeeTimePaid[hacker[_cubeId].ID] > block.timestamp - 30 days,"unpaid cube fee");
       // require(hackerFeeTimePaid[hacker[tokenId].ID] > block.timestamp - 30 days,"unpaid hacker fee");
        ...
        require(cube[_cubeId].owner == msg.sender,"not cube owner");
    ...
    function payFee(uint _cubeOrHacker, uint _Id) external payable { 
        ...
        require(msg.value == fee,"not enouhg AVAX");
        if (_cubeOrHacker == 1){
    ...
    function rentCube(uint _cubeId, uint _hackerId,uint _cubeTimeInDays) external payable{
        require(cubeRentAble[cube[_cubeId].ID] == true);
        require(hacker[_hackerId].owner == msg.sender);
        HACKaddress.transferFrom(msg.sender,cube[_cubeId].owner, cubeRentPrice[_cubeId] * _cubeTimeInDays);
        hacker[_hackerId].cube = cube[_cubeId].ID;
        cubeRented[_cubeId] = true;
        cubeRentAble[_cubeId] = false;
        cubeRentedUntil[_cubeId] = block.timestamp + _cubeTimeInDays * 1 days;
