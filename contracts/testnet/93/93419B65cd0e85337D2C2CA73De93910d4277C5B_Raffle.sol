...
interface ICoupons{
    function getCoupon(
        ...
        bytes32 couponHash
    ) external view returns (bool success, uint8 minPct, uint8 maxPct, uint16 multiplierPct);
}
...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface, IRaffleResults, IPrizeMatrix {
    ...
    function enterRaffle(string memory coupon, string memory partnerID) public payable {
        ...
        // Check coupons
        (bool success, uint8 minPct, uint8 maxPct, uint16 multiplierPct) = couponManager.getCoupon(
            address(this), keccak256(abi.encodePacked(coupon))
        );
        if (success) {
            uint256 startBalancePct = uint16(100 * (address(this).balance - msg.value) / s_targetBalance);
            if (minPct <= startBalancePct && startBalancePct <= maxPct) {
                bonusTickets += (realTickets * multiplierPct) / 100;
            }
        }
    ...
    function fulfillRandomWords(
        ...
        uint unpaidPrize = _getTotalPrize();
        uint winnerId;
        ...
        if (safeTxSuccess) {
            if (s_raffleParams.autoStart) {
                ...
                // copy matrix if autostart enabled
    ...
    function _setRaffleParams(RaffleParams memory raffleParams) private {
        ...
        s_raffleParams.bank = raffleParams.bank;
    ...
    function _getTotalPrize() internal view returns (uint256) {
        return (s_raffleParams.bank * s_raffleParams.prizePct) / 100;
    }
    ...
    function setPrizeMatrix(IPrizeMatrix.PrizeLevel[] memory prizeMatrix) public onlyOwner atMaintenance {
        ...
        if (prizeMatrix.length == 0) {
            ...
            s_prizeMatrix[s_raffleId].push(IPrizeMatrix.PrizeLevel(1, _getTotalPrize(), true));
        } else {
            delete s_prizeMatrix[s_raffleId];
    ...
    function _checkCurrentPrizeMatrix(IPrizeMatrix.PrizeLevel[] memory prizeMatrix) internal view {
        if (s_raffleId > 0 && prizeMatrix.length > 0) {
            ...
            if (matrixBalance != _getTotalPrize()) {
