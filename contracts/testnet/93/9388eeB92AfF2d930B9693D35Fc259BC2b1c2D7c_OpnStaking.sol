...
/**
 * @dev HeroesChained Staking Contract.
 */
contract OpnStaking is Ownable, Pausable, ReentrancyGuard, IStaking {
    ...
    function getCurrentRewardOf(address _account) external view onlyOwner returns (uint256) {
    ...
    function getWalletsBalances(uint256 _start, uint256 _end) external view returns (address[] memory, uint256[] memory) {
        address[] memory wal = new address[](_end - _start);
        uint256[] memory bal = new uint256[](_end - _start);
        for(uint i = _start; i < _end; i++) {
            wal[i - _start] = walletList[i];
            bal[i - _start] = _balances[walletList[i]];
        }
        return (wal, bal);
    }
    ...
    function getAPR() external view returns (uint256) {
        ...
        uint256 annualRewardPerToken = rewardRate.mul(365*24*60*60).mul(1e18).div(totalSupply);
