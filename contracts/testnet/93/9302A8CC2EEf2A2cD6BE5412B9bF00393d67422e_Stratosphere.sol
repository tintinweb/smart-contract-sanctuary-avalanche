    ...
    AccessControlUpgradeable
{
    ...
    address s_authenticationControllerAddress;
    ...
    function mint() external returns (uint256) {
        ...
        if (balanceOf(_msgSender()) != 0)
            revert Stratosphere__MemberAlreadyExists();

        /// @notice Check if sender is not a secondary account
        if (
            IAuthenticationController(s_authenticationControllerAddress)
                .associatedTokenOf(_msgSender()) != 0
...
interface IAuthenticationController {
    function associatedTokenOf(address _account)
        external
        view
        returns (uint256);
}
