...
contract Bakery is Ownable {
    ...
    uint256 public constant YIELD_CPS = 16666666666666667; // cookies baked per second per unit of yield
    ...
    constructor(Baker _baker, Cookie _cookie, address _pantryAddress) {
        baker = _baker;
        cookie = _cookie;
        pantryAddress = _pantryAddress;
    }
    ...
    function stakeMany(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _addBakerToBakery(tokenId, _msgSender());
        }
    }
    ...
    function _addBakerToBakery(uint256 _tokenId, address _owner) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                ...
                accrual: _getCookiesAccruedFor(tokenId, false)
            });
        }
