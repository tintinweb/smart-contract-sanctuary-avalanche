...
contract math2 {
		...
		uint256 public a = getDecimals(5000);
		uint256 public d;
		uint256 public e;
		uint256 public f;
		uint256[] public ls  = doPercentage(5000,28);
		...
		function getDecimals(uint256 _x) internal returns(uint){
			uint i = 0;
			while(_x != 0){
				_x = _x.div(10);
				i++;
		...
		function doPercentage(uint256 _x,uint256 perc) internal returns(uint256[3] memory){
			uint256 percDec = getDecimals(perc);
			uint256 refDec = getDecimals(_x);
			uint256 refDecP = percDec.sub(2);
			uint256 perc = (10**refDecP);
			uint256 _x = _x.mul(10**refDec);
			uint256 _y = _x.div(perc);
			uint256 _z = _y.div(refDec);
			return [_x,_y,_z];
