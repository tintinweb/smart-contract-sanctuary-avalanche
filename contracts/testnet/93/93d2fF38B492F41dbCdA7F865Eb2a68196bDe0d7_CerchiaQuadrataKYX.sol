...
contract CerchiaQuadrataKYX is ICerchiaQuadrataKYX {
	...
	function userCreateNewDealAsBid(
		...
		if (!usesMirroredToken(denomination)) {
			require(isBusinessInvestor(), IS_NOT_BUSINESS_INVESTOR);
		}
	}
	...
	function userUpdateDealToCancel(uint256 dealId) external override {
		ICerchiaDRT(_cerchiaDiamondAddress).userUpdateDealToCancel(msg.sender, dealId);
	}
	...
	function userUpdateDealFromBidToMatched(uint256 dealId) external override {
		IMatchDeal(_cerchiaDiamondAddress).userUpdateDealFromBidToMatched(msg.sender, dealId);
	...
	function userUpdateDealFromAskToMatched(uint256 dealId) external override {
		IMatchDeal(_cerchiaDiamondAddress).userUpdateDealFromAskToMatched(msg.sender, dealId);
	...
	function processContingentSettlement(uint64 timestamp, uint256 dealId) external override {
		IAnyAccountOperationsFacet(_cerchiaDiamondAddress).processContingentSettlement(msg.sender, timestamp, dealId);
	}
	...
	function claimBack(uint256 dealId) external override {
		IClaimBackFacet(_cerchiaDiamondAddress).claimBack(msg.sender, dealId);
	}

	/**
	 * @notice  Checks with Quadrata, if the calling user is a business investor
	 */
	...
	/**
	 * @dev     Users may perform operations if they own our test token (does not have any value, compared with real USDC)
	 * @param   dealId  Deal id
	 * @return  bool  True if user uses a deal placed on the test token
	 */
	...
	/**
	 * @dev     Users may perform operations if they own our test token (does not have any value, compared with real USDC)
	 * @param   denomination  Test token symbol (for example cUSDC)
	 * @return  bool  True if user uses our test token (cUSDC)
	 */
