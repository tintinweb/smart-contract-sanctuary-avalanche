...
abstract contract AToken is ATokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     */
    function initialize(AvatrollerInterface comptroller_,
        ...
        // Set the comptroller
        uint err = _setAvatroller(comptroller_);
     ...
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, ATokenInterface cTokenCollateral) internal nonReentrant {
        accrueInterest();
     ...
     * @param repayAmount The amount of the underlying borrowed asset to repay
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, ATokenInterface cTokenCollateral) internal {
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setAvatroller(AvatrollerInterface newAvatroller) override public returns (uint) {
...
contract Avatroller is AvatrollerV7Storage, AvatrollerInterface, AvatrollerErrorReporter, ExponentialNoError {
    ...
    event CompBorrowSpeedUpdated(AToken indexed aToken, uint newSpeed);

    /// @notice Emitted when a new supply-side COMP speed is calculated for a market
    event CompSupplySpeedUpdated(AToken indexed aToken, uint newSpeed);
    ...
    function liquidateBorrowAllowed(
        ...
        if (isDeprecated(AToken(cTokenBorrowed))) {
     ...
     * @param cTokenBorrowed The address of the borrowed aToken
     * @param cTokenCollateral The address of the collateral aToken
     ...
     */
    function setCompSpeedInternal(AToken aToken, uint supplySpeed, uint borrowSpeed) internal {
        Market storage market = markets[address(aToken)];
    ...
    function updateCompSupplyIndex(address aToken) internal {
        CompMarketState storage supplyState = compSupplyState[aToken];
        uint supplySpeed = compSupplySpeeds[aToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
    ...
    function updateCompBorrowIndex(address aToken, Exp memory marketBorrowIndex) internal {
        CompMarketState storage borrowState = compBorrowState[aToken];
        uint borrowSpeed = compBorrowSpeeds[aToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
    ...
    function distributeSupplierComp(address aToken, address supplier) internal {
        ...
        uint supplierIndex = compSupplierIndex[aToken][supplier];
    ...
    function distributeBorrowerComp(address aToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        uint borrowerIndex = compBorrowerIndex[aToken][borrower];
     ...
     */
    function _setCompSpeeds(AToken[] memory cTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
...
abstract contract ATokenInterface is ATokenStorage {
    ...
    function _setAvatroller(AvatrollerInterface newAvatroller) virtual external returns (uint);
...
abstract contract AErc20Interface is AErc20Storage {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, ATokenInterface cTokenCollateral) virtual external returns (uint);
