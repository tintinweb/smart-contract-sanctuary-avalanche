...
contract OmniseaONFT721Psi is ONFT721Core, IOmniseaONFT721Psi, ERC721Psi, IERC2981Royalties, ReentrancyGuard {
    ...
    address private immutable _lzEndpoint = 0x93f54D755A063cE7bB9e6Ac47Eccc8e33411d706;
    IOmniseaDropsScheduler private immutable _scheduler = IOmniseaDropsScheduler(0xaE382068F1E3BB8c682b56EEE07a03e362898592);
    IOmniseaPaymentsManager private immutable _paymentsManager = IOmniseaPaymentsManager(0xEB492204e0C4267fF5f622245B679424bDA41d23);
    ...
    function mint(address _owner, uint24 _quantity, bytes32[] memory _merkleProof, uint8 _phaseId, uint256 _paid) external override nonReentrant {
        require(msg.sender == dropsManager);
        _validate(_quantity, _owner, _merkleProof, _phaseId);
        _paymentsManager.onPayment(_owner, _paid);
        _scheduler.increasePhaseMintedCount(_owner, _phaseId, _quantity);
        _mint(_owner, _quantity);
    }

    function _validate(uint24 _quantity, address _owner, bytes32[] memory _merkleProof, uint8 _phaseId) view internal {
        require(!_paymentsManager.isFlagged(address(this)));
        require(block.timestamp < endTime);
        uint256 _newTotalMinted = totalMinted() + _quantity;
        if (maxSupply > 0) require(maxSupply >= _newTotalMinted);
        require(isAllowed(_owner, _quantity, _merkleProof, _phaseId), "!isAllowed");
    }
    ...
    function payout() external nonReentrant {
        ...
        _paymentsManager.payout(owner, endTime);
    }

    function refund() external nonReentrant {
        _paymentsManager.refund(msg.sender, endTime);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Psi, ONFT721Core) returns (bool) {
        ...
        || interfaceId == type(IOmniseaONFT721Psi).interfaceId
...
interface IOmniseaPaymentsManager {
    function onPayment(address _payee, uint256 _paid) external;
    function payout(address _recipient, uint256 _endTime) external payable;
