...
contract voyager is ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer{
  ...
  uint private _price;
  uint private _maxMintable = 350;
  string private _customBaseURI;
  uint private _royaltyAmount;
  uint private _maxMintPerTx;
  uint private _maxPerWallet;
  uint private constant START_ID = 350;
  ...
  constructor() ERC721("Voyager", "V") ClampedRandomizer(_maxMintable){
    _customBaseURI = 'ipfs:notareallinkhh';
    _price = 0 * 1000000000000000;  //price in wei 
    _royaltyAmount = 1000;
    _maxMintPerTx = 100; 
    _mintActive = false; 
    _privateSale = false;
  ...
  mapping(address => bool) addressesMinted;
  ...
  function changeMintActive() public onlyOwner {
    _mintActive = !_mintActive;
  }
  function changePrivateSale() public onlyOwner {
  ...
  function setMaxPerTx(uint max) public onlyOwner {
    _maxMintPerTx = max;
  }
  function setMaxPerWallet(uint max) public onlyOwner {
    _maxPerWallet = max;
  }
  function setRoyalty(uint royalty) public onlyOwner {
    _royaltyAmount = royalty;
  }

  function mint(uint quantity) public payable {
    if(_privateSale == true) {require(verifyUser(msg.sender) == true, "not whitelisted, wait until public sale");}
    else if(_privateSale == false){require(hasMinted(msg.sender) == false, "user has already minted");}
    ...
    if(_privateSale){whitelistedAddresses[msg.sender] = false;}
    else{addressesMinted[msg.sender] = true;}
  }
