...
contract MyToken is ERC721, ERC721URIStorage, Pausable, AccessControl {
    ...
    function mintTokens(uint256 qty) public onlyRole(MINTER_ROLE) {
        for (uint i = 0; i < qty; i++){
            safeMint(msg.sender);
        }
    }

    function _setBaseURI(string memory _newBaseURI) public onlyRole(MINTER_ROLE) {
        baseTokenURI = _newBaseURI;
    }

    function _setURISuffix(string memory _newUriSuffix) public onlyRole(MINTER_ROLE) {
