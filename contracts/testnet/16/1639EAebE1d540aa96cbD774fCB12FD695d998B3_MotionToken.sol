    ...
    ReentrancyGuardUpgradeable
{
    ...
    function transfer(
        ...
        if (hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || _whitelist[msg.sender]) {
...
contract MotionVesting {
  ...
  function vestedAmount(address token) public view returns (uint256) {
    ...
    uint256 diff = block.timestamp.sub(_startDate).div(1 weeks);
    return (
      (balance.add(_erc20Released[token]).div(20)).mul(diff < 20 ? diff : 20)
    );
  }
