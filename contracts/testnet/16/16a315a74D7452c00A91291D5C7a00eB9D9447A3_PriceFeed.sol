...
contract PriceFeed is AccessControl, IPriceFeed {
    ...
    // mapping(bytes32 => AggregatorV3Interface) internal priceFeeds;
    ...
    function getPriceInToken(uint256 _amount, bytes32 _currency) external view returns (uint256) {
        uint256 price;
        uint256 exchangeRate;
        if (_currency == bytes32("AVAX")) {
            exchangeRate = getEthPrice();
            price = _amount.mul(10**20).div(exchangeRate);
        } else if(_currency == bytes32("USDC")) {
            exchangeRate = getUsdcPrice();
            price = _amount.mul(exchangeRate).div(10**8);
        }
