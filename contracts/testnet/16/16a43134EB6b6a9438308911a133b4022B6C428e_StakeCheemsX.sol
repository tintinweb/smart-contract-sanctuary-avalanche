...
contract StakeCheemsX {
    ...
    struct DepositInfo {
        ...
        uint256 x;                  // Reward Factor
        uint256 percentage;                  // Reward Percentage
    }
    ...
    /**
    Deposit
     type: 1 - staking with no lock
     type: 2 - 1 month
     type: 3 - 3 months
     type: 4 - 6 month
     type: 5 - 12 months
     type: 6 - Irreversible lock
    */ 
    function deposit(uint256 _amount, uint depositType) public {
        ...
        if (_amount > 0) {
            ...
            if(depositType == 1) {
                user.depositInfo[1].amount += _amount;
                user.depositInfo[1].depositTime = block.timestamp;
                user.depositInfo[1].lockedTime = block.timestamp;
                user.depositInfo[1].percentage = user.depositInfo[1].amount.mul(100).div(user.totalAmount);
            }
            if(depositType == 2) {
                user.depositInfo[2].amount += _amount;
                user.depositInfo[2].depositTime = block.timestamp;
                user.depositInfo[2].lockedTime = block.timestamp + 30 days;
                user.depositInfo[2].percentage = user.depositInfo[2].amount.mul(100).div(user.totalAmount);
            }
            if(depositType == 3) {
                user.depositInfo[3].amount += _amount;
                user.depositInfo[3].depositTime = block.timestamp;
                user.depositInfo[3].lockedTime = block.timestamp + 90 days;
                user.depositInfo[3].percentage = user.depositInfo[3].amount.mul(100).div(user.totalAmount);
            }
            if(depositType == 4) {
                user.depositInfo[4].amount += _amount;
                user.depositInfo[4].depositTime = block.timestamp;
                user.depositInfo[4].lockedTime = block.timestamp + 180 days;
                user.depositInfo[4].percentage = user.depositInfo[4].amount.mul(100).div(user.totalAmount);
            }
            if(depositType == 5) {
                user.depositInfo[5].amount += _amount;
                user.depositInfo[5].depositTime = block.timestamp;
                user.depositInfo[5].lockedTime = block.timestamp + 365 days;
                user.depositInfo[5].percentage = user.depositInfo[5].amount.mul(100).div(user.totalAmount);
            }
            if(depositType == 6) {
                user.depositInfo[6].amount += _amount;
                user.depositInfo[6].depositTime = block.timestamp;
                user.depositInfo[6].percentage = user.depositInfo[6].amount.mul(100).div(user.totalAmount);
                cheemsX.burn(_amount.mul(30).div(100));
                swapTokensForAVAX(_amount.mul(20).div(100));
                swapTokensForUSDC(_amount.mul(20).div(100));
            }
    ...
    function withdraw(uint256 _amount, uint depositType) public {
        ...
        require(user.depositInfo[depositType].amount > _amount, "Invalid withdraw amount");
        
        if (_amount > 0 && canWithdraw(msg.sender, depositType)) {
            cheemsX.transfer(msg.sender, _amount);
            user.depositInfo[depositType].amount = user.depositInfo[depositType].amount.sub(_amount);
    ...
    function canWithdraw(address _user, uint depositType) public view returns (bool) {
        ...
        return block.timestamp > user.depositInfo[depositType].lockedTime && user.depositInfo[depositType].amount > 0;
    }
    ...
    function harvest(uint depositType) public {
        ...
        if(canHarvest(msg.sender, depositType)) {
            if(user.depositInfo[depositType].nextHarvestUntil == 0) {
                uint256 pendingDate = (block.timestamp - user.depositInfo[depositType].depositTime).div(intervaLTime).add(1);
                uint256 dailyPending = rewardPoolBalance.div(distributionPeriod).mul(user.depositInfo[depositType].percentage).div(100).mul(x[depositType]);
                pending = pendingDate.mul(dailyPending);
            } else {
                uint256 pendingDate = (block.timestamp - user.depositInfo[depositType].nextHarvestUntil).div(intervaLTime).add(1);
                uint256 dailyPending = rewardPoolBalance.div(distributionPeriod).mul(user.depositInfo[depositType].percentage).div(100).mul(x[depositType]);
                pending = pendingDate.mul(dailyPending);
            }
        }
        ...
        if (pending > 0) {
            user.depositInfo[depositType].nextHarvestUntil = block.timestamp.add(intervaLTime);
            // send rewards
    ...
    function harvest() public {
        ...
        for(uint depositType = 1; depositType < 6; depositType++) {
            if(canHarvest(msg.sender, depositType)) {
                if(user.depositInfo[depositType].nextHarvestUntil == 0) {
                    uint256 pendingDate = (block.timestamp - user.depositInfo[depositType].depositTime).div(intervaLTime).add(1);
                    uint256 dailyPending = rewardPoolBalance.div(distributionPeriod).mul(user.depositInfo[depositType].percentage).div(100).mul(x[depositType]);
    ...
    function canHarvest(address _user, uint depositType) public view returns (bool) {
        UserInfo storage user = userInfo[_user];
        return  (block.timestamp >= user.depositInfo[depositType].nextHarvestUntil && block.timestamp < user.depositInfo[depositType].lockedTime);
    }
    ...
    function pendingTokenByType(address _user, uint depositType) external view returns (uint256) {
        ...
        if(canHarvest(_user, depositType)) {
            if(user.nextHarvestUntil == 0) {
                uint256 pendingDate = (block.timestamp - user.depositInfo[depositType].depositTime).div(intervaLTime).add(1);
                uint256 dailyPending = rewardPoolBalance.div(distributionPeriod).mul(user.depositInfo[depositType].percentage).div(100).mul(x[depositType]);
                pending = pendingDate.mul(dailyPending);
