...
// File: SalientYachtsSYONE_v07.sol
...
contract SalientYachtsSYONE_v07 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 private constant TEN_YEAR_DEPOSIT       = 1199999999999765395680;
    uint8   private constant NFT_PRICE_DECIMALS     = 18;
    uint256 private constant NFT_MINT_PRICE_USD     = 50 * (10 ** NFT_PRICE_DECIMALS); //mint price fixed at $50
    //uint256 private constant NFT_MINT_PRICE_ETH     = 27000000000000000; //0.027 ETH for mainnet
    uint256 private constant NFT_MINT_PRICE_ETH     = 27000000000000; //0.000027 ETH for testnet
    uint16  private constant SUPPLY_LIMIT           = 20000; //assume Yacht price is $1,000,000 -> we will have 20000 tokens at $50 each
    ...
    bool public useFixedPrice = false;
    ...
    event newStream(uint256 streamId, uint256 tokenId, address recipient, NFTType nftType);
    ...
    constructor(address _rewardContractAddress) ERC721("Salient Yachts", "SYONE") {
        ...
        nftTypeToData[NFTType.Common] = NFTTypeData('bafyreihmx5rptcqfl2uwxdk5kategwa3ygssfmusj2fxlkdldbf7ezc5q4/metadata.json', 
            NFT_MINT_PRICE_ETH * 1, TEN_YEAR_DEPOSIT, 1, true);

        nftTypeToData[NFTType.Standard] = NFTTypeData('bafyreihmx5rptcqfl2uwxdk5kategwa3ygssfmusj2fxlkdldbf7ezc5q4/metadata.json', 
            NFT_MINT_PRICE_ETH * 2, TEN_YEAR_DEPOSIT * 2, 2, true);

        nftTypeToData[NFTType.Rare]   = NFTTypeData('bafyreid46czh77qyamtnlsit4qieaoynu6sjn2klcjfvlwbp3gju2o5mly/metadata.json', 
            NFT_MINT_PRICE_ETH * 20, TEN_YEAR_DEPOSIT * 20, 20, true);

        nftTypeToData[NFTType.Epic]  = NFTTypeData('bafyreig6zlp6b5mdpuw4kgyibae4kgoma6nczqmgqdjvpem54xfrqgsllm/metadata.json', 
            NFT_MINT_PRICE_ETH * 200, TEN_YEAR_DEPOSIT * 200, 200, true);
    }
    ...
    function toggleUseFixedPrice() public onlyOwner returns (bool) {
    ...
    function buyYachtNFT(uint256 numberOfTokens, NFTType nftType, string memory affiliateId) public payable {
        ...
        uint256 currentNFTPrice = uint256(nftTypeData.mintPrice);
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            emit newStream(streamId, tokenId, msg.sender, nftType);
        }

        if (bytes(affiliateId).length > 0) {
            bytes32 affIdHash = keccak256(abi.encodePacked(affiliateId));
    ...
    // function getLatestNFTPrice(NFTType nftType) internal view returns (int256) {
    //     NFTTypeData storage nftTypeData = nftTypeToData[nftType];
    ...
    //     require(price > 0, "Could not retrieve price of ETH/USD");
    ...
    function getAffiliateSales(string memory inAffiliateId) public view returns (NFTSale[] memory) {
        ...
        bytes32 affIdHash = keccak256(abi.encodePacked(inAffiliateId));
