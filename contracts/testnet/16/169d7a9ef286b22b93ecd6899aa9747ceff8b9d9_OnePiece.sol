...
// File: tests/OnePieceNodeManager.sol

pragma solidity ^0.8.10;

contract OnePieceNodeManager is Ownable {
    ...
    struct Node {
      ...
      uint256 OnePieceUpgradeStartTimestamp;
    ...
    uint256[] public _countNodesPerType = [0,0,0,0,0];
    // Datas
    ...
    function _setMaxNodesPerType(uint256 nodeTypeId, uint256 maxNode) external onlyOwner{
            require(nodeTypeId < _nodeTypes.length && nodeTypeId >= 0, "Not valid id of a type of Node");
            _nodeTypes[nodeTypeId].maxNode = maxNode;
    }
    ...
    function _createNode(address user, string memory name, uint256 nodeTypeId) public onlyOwner {
      ...
      _countNodesPerType[nodeTypeId]+=1;
      ...
      usersNodes[user].push(Node({
        ...
        OnePieceUpgradeStartTimestamp: 0,
        availableRewards: 0
      }));
    ...
    function _buyNode(address user, string memory name, uint256 nodeTypeId) external onlyOwner {
      ...
      require(nodeType.buyable, "Cannot buy a non-buyable node");
      require(_countNodesPerType[nodeTypeId]< nodeType.maxNode,"Maximum of nodetype reached");
    ...
    function _upgradeNode(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.upgrading = true;
      node.OnePieceUpgradeStartTimestamp = block.timestamp;
    ...
    function _claimUpgrade(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.OnePieceUpgradeStartTimestamp = 0;

      _countNodesPerType[node.nodeType]-=1;
      _countNodesPerType[_nodeTypes[node.nodeType].typeUpgradeNode]+=1;
    ...
    function _getClaimableUpgrades(address user) external onlyOwner view returns (uint256[] memory) {
      ...
      for (uint256 i = 0; i < nodes.length; i++) {
        ...
        if(node.upgrading) {
          if(block.timestamp >= node.OnePieceUpgradeStartTimestamp + nodeType.timeToUpgrade) {
    ...
    function canClaimUpgrade(Node memory node) private view returns (bool) {
      ...
      return node.upgrading && block.timestamp >= node.OnePieceUpgradeStartTimestamp + nodeType.timeToUpgrade;
    }
    ...
    function _editNodeType(uint256 nodeTypeId, uint256 rewards, uint256 timeToUpgrade, uint256 typeUpgradeNode,
                           uint256 price,uint256 maxNode, bool buyable, bool upgradable, bool bonus) external onlyOwner {
      require(nodeTypeId < _nodeTypes.length && nodeTypeId >= 0, "Not valid id of a type of Node");
...
// File: tests/OnePiece.sol

pragma solidity ^0.8.10;
contract OnePiece is ERC20, Ownable {
    using SafeMath for uint256;

    OnePieceNodeManager private nodeManager;
    ...
    uint256 public _boosterChildAmount = 2000000000;

    // Pools & Wallets
    address public _futurUsePool = 0x35Bc7c5ebf5AF19051d893f0e93b7FB24326Ce7D;
    address public _distributionPool = 0x1321F4867E2530a5Ba5D1C71C3bb5475D4705b68;
    address public _buyNodePool = 0x1321F4867E2530a5Ba5D1C71C3bb5475D4705b68;    
    ...
    constructor() ERC20("OnePieceNodes", "Sunny") {
        nodeManager = new OnePieceNodeManager();

        // Exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(_futurUsePool, true);
        excludeFromFees(_distributionPool, true);
        excludeFromFees(_buyNodePool, true);
    ...
    function buyNode(string memory name, uint256 nodeType) external  {
      ...
      super._transfer(sender, _buyNodePool, price);
 ...
 function setMaxNodesPerType(uint256 nodeTypeId,uint256 maxAmount) external onlyOwner {
      nodeManager._setMaxNodesPerType(nodeTypeId, maxAmount);
    }
    ...
    function getNodes(address user) external view returns (OnePieceNodeManager.Node[] memory) {
      return nodeManager._getNodes(user);
    }

    function getNodeTypes() external onlyOwner view returns (OnePieceNodeManager.NodeType[] memory) {
      return nodeManager._getNodeTypes();
    }

    function getBoosterDatas(address user) external view returns (OnePieceNodeManager.BoosterData memory) {
    ...
    function editNodeType(uint256 nodeTypeId, uint256 rewards, uint256 timeToUpgrade, uint256 typeUpgradeNode,
                           uint256 price,uint256 maxNode, bool buyable, bool upgradable, bool bonus) external onlyOwner {
      nodeManager._editNodeType(nodeTypeId, rewards, timeToUpgrade, typeUpgradeNode, price,maxNode, buyable, upgradable, bonus);
    }

    function addNodeType(uint256 rewards, uint256 timeToUpgrade, uint256 typeUpgradeNode,
                           uint256 price, uint256 maxNode, bool buyable, bool upgradable, bool bonus) external onlyOwner {
      nodeManager._addNodeType(rewards, timeToUpgrade, typeUpgradeNode, price,maxNode, buyable, upgradable, bonus);
    }
