    ...
    IGameFiCoreV2
{
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < totalSupply(); i++) {
            if (IGameFiTokenERC1155(_collection[0].tokenContract).balanceOf(_profileIdToVault[i], 1) > 0) {
                _propertyValueOf[i][0] = bytes32(uint256(1));
            }
            if (IGameFiTokenERC1155(_collection[0].tokenContract).balanceOf(_profileIdToVault[i], 2) > 0) {
                _propertyValueOf[i][0] = bytes32(uint256(2));
            }
            if (IGameFiTokenERC1155(_collection[0].tokenContract).balanceOf(_profileIdToVault[i], 3) > 0) {
                _propertyValueOf[i][0] = bytes32(uint256(3));
            }
            if (IGameFiTokenERC1155(_collection[0].tokenContract).balanceOf(_profileIdToVault[i], 0) > 0) {
                _propertyValueOf[i][0] = bytes32(0);
            }
        }
    }
