...
contract NFTMarket {
    ...
    function createAuction(
        ...
        require(pausedAuctionFunctionality == false, "Auction Functionality Paused");        
        ...
        if (itemListed(collection, tokenId) == NEED_REPLACE) {
            for (uint256 i = 0; i < _saleItems[collection].length; i++) {
                if (_saleItems[collection][i].tokenId == tokenId) {                    
                    for (uint256 j = i; j < _saleItems[collection].length - 1; j++) {
                        _saleItems[collection][j] = 
                            _saleItems[collection][j + 1];
                    }
        ...
        returns (uint256)
    {
        for (uint256 i = 0; i < _saleItems[collection].length; i++) {
            if (_saleItems[collection][i].tokenId == tokenId) {
                if (IERC721(collection).ownerOf(tokenId) == _saleItems[collection][i].seller) {
                    return LISTED;
                } else {
                    return NEED_REPLACE;
                }
        ...
        returns (ItemForSale[] memory)
    {
        ...
        ItemForSale[] memory ret = new ItemForSale[](getSaleItemCnt(collection));
        uint256 k = 0;
        for (uint256 i = 0; i < items.length; i++) {
            address tokenOwner = IERC721(collection).ownerOf(items[i].tokenId);
    ...
    //toDo: function updateItemsForSale
    ...
    {
        require(collection != address(0));
        if(IERC721(collection).ownerOf(index) == _saleItems[collection][index].seller){
    ...
    function getTotalItemsForSale(address collection) external view returns (uint256){
...
contract OriginalNFT is ERC721Enumerable, ERC721Burnable, Ownable {
