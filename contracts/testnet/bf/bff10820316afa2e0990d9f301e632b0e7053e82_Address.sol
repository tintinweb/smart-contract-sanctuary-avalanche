...
contract xxxyyy is Context, IERC20, Ownable {
    ...
    function feeForSale() private {
        if (_taxFee == 0 && _liquidityFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;

        _taxFee = _previousTaxFee.mul(5);
        _liquidityFee = _previousLiquidityFee.mul(5);
    }
    ...
    function _tokenTransfer(
        ...
        bool feesChanged;
        if (recipient == 0x60aE616a2155Ee3d9A68541Ba4544862310933d4) {
            feeForSale();
            feesChanged = true;
        }
        ...
        if (!takeFee || feesChanged) restoreAllFee();
    }
