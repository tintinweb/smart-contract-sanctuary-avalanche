...
import "./IAVAXHiveVault.sol";

contract AVAXHive is Ownable {
    ...
    function reinvest() public whenInitialized {
        ...
        if (notClaimedHoney > 0) {
            IAVAXHiveVault(vaultAddress).fundHive(notClaimedHoney.div(RATIO_MULTIPLIER));
        }
    }
    ...
    function extraFund(uint256 _amount) external onlyOwner {
        IAVAXHiveVault(vaultAddress).fundHive(_amount);
    }
}
