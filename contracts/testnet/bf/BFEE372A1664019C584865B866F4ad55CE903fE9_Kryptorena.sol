...
contract Kryptorena is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function triggerBattle() public {
        ...
        uint256 p2Attack = 2;
        uint256 p2Defend = 5;
        initiateBattle(p1, p2, p1Attack, p1Defend, p2Attack, p2Defend);
    }
    ...
    function updateStats(address _winner, uint256 newAttack, uint256 newDefense) external {
...
contract KryptorenaBattle is VRFConsumerBaseV2, Ownable {
    ...
    function attack() external {
        uint256 id = currentMatch[msg.sender].battleId;
        Battle storage round = battles[id];
        BattleData storage resultData = matchDataPlayerTracker[msg.sender];

        require(resultData.battleStatus != endStatus.ENDED);
        ...
        if (msg.sender == round.player1) {
            console.log("PLAYER 1 ATTACKS WITH 3 PTS");
            round.player1Choice = AttackOrDefense.ATTACK;
        } else {
            console.log("PLAYER 2 ATTACKS WITH 2 PTS");
    ...
    function defend() external {
        uint256 id = currentMatch[msg.sender].battleId;
        Battle storage round = battles[id];
        BattleData storage resultData = matchData[id];
        ...
        if (msg.sender == round.player1) {
            console.log("PLAYER 1 DEFEBDS WITH 1 PTS");

            round.player1Choice = AttackOrDefense.DEFEND;
        } else {
            console.log("PLAYER 2 DEFENDS WITH 5 PTS");
    ...
    function turnAction(address player) private returns (string memory) {
        ...
        if (
            ...
            if (!(int(round.player2DefensePoints) - int(round.player1AttackPoints) > 0)) {
                console.log("OPTION 2");
                int calculateDamage = int(round.player2DefensePoints) -
                    int(round.player1AttackPoints);

                int damage = absoluteValue(calculateDamage, 0);

                round.player2HP = round.player2HP - damage;
            }
            ...
            if (!(int(round.player1DefensePoints) - int(round.player2AttackPoints) > 0)) {
                console.log("OPTION 3");

                int calculateDamage = int(round.player1DefensePoints) -
                    int(round.player2AttackPoints);
                int damage = absoluteValue(calculateDamage, 0);
                round.player1HP = round.player1HP - damage;
            }
        ...
        if (round.player1HP == 0 || round.player2HP == 0 || resultData.turn == MAX_TURNS) {
            ...
            int256 int_HP_DIFFERENCE = absoluteValue(round.player1HP, round.player2HP);
            resultData.hpDifference = uint256(int_HP_DIFFERENCE);
            resultData.battleStatus = endStatus.ENDED;
            endGame(player);
        }
    ...
    function endGame(address player) private {
        ...
        uint256 id = currentMatch[player].battleId;
        Battle storage round = battles[id];
        BattleData storage resultData = matchData[id];
        ...
        if (resultData.player1 == endStatus.WON) {
            ...
            console.log("Requesting VRF for player 1.......");
            requestRandomWord(winner);
        } else if (resultData.player2 == endStatus.WON) {
            console.log("Requesting VRF for player 2.......");

            winner = round.player2;
            requestRandomWord(winner);
        } else {
            console.log("SENT NEW STATS TO LOGIC CONTRACT (DRAW)");
    ...
    function requestRandomWord(address winner) private returns (uint256 requestId) {
        console.log("RANDOM WORD REQUESTING....");
        requestId = i_vrfCoordinator.requestRandomWords(
            i_gasLane,
            i_subscriptionId,
            REQUEST_CONFIRMATIONS,
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        console.log("FULFILLING RANDOM WORD");
        uint256 randomNumber = randomWords[0];

        address winner = s_requestIdToSender[requestId];
        uint256 id = currentMatch[winner].battleId;
        BattleData storage resultData = matchData[id];

        Battle storage round = battles[id];
        uint256 s_HP_DIFFERENCE = resultData.hpDifference;
        ...
        //     console.log("PLAYER 1 WINNER STATS SENT TO LOGIC");
        ...
        // } else {
        //     console.log("PLAYER 2 WINNER STATS SENT TO LOGIC");
