...
contract AvaxSpies is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant ROYALTY = 700; // 7% Percent Royalty
    uint256 public constant MINT_PRICE = 0.01 ether; //PROD_CHANGE to 1 ether
    uint256 public constant MAX_MINT_PER_TX = 10;

    string public baseExtension = ".json";
    string public baseURI = "";
    ISTIK public stikNFT;
    address public royaltyRecipient = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4; // Needs to be updated
    mapping(address => uint256) public mintedWallets;

    constructor(string memory _baseURI, address _stik_address) ERC721A("AVAX Spies", "SPIES") {
        updateBaseURI(_baseURI);
        stikNFT = ISTIK(_stik_address);
    }

    function mint(uint[] memory tokenIds) public payable nonReentrant whenNotPaused {
        if (tokenIds.length == 0) revert AvaxSpies__BelowMinimumLimit();
        if (totalSupply() + tokenIds.length > MAX_SUPPLY) revert AvaxSpies__MaxNFTsMinted();
        if (tokenIds.length > MAX_MINT_PER_TX) revert AvaxSpies__ExceedsMaxMintPerTx();
        if (msg.value < tokenIds.length * MINT_PRICE) revert AvaxSpies__InsufficientFunds();

        for (uint256 i; i < tokenIds.length; ) {
            if (stikNFT.ownerOf(tokenIds[i]) != msg.sender) revert AvaxSpies__NotOwner();
    ...
    function withdrawTokens() external onlyOwner {
        ...
        (bool transferOne, ) = payable(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4).call{value: balance}("");
