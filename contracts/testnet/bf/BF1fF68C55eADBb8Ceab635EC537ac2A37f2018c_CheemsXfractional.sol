...
contract CheemsXfractional is ERC721URIStorage, Ownable {
    ...
    function setRoyalFee (uint _fee) public onlyOwner {
        royaltyFee = _fee;
    }
    ...
    function dealwithRoyalty (uint nftId) private {
        ...
        if(royaltyOption == 0) {
            IERC20(token).transferFrom(_msgSender(), address(this), price[tierId] * royaltyFee * 10 ** IERC20Metadata(token).decimals() / 100 / priceDivisor);
        } else {
            if(minter == _msgSender()) return;
            IERC20(token).transferFrom(_msgSender(), minter, price[tierId] * royaltyFee * 10 ** IERC20Metadata(token).decimals() / 100 / priceDivisor);
        }
    }
