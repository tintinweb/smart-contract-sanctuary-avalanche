...
import "@openzeppelin/contracts/utils/Strings.sol";
import "./KindaRandomRanges.sol";

contract Sausagers is Ownable, ERC721 {
    ...
    modifier onlyOperator() {
        require(operators[msg.sender] == true, "Caller is not operator");
        _;
    }

    event SausagerMinted(uint256 tokenId);

    constructor() ERC721("Sausagers", "MEAT") {
        operators[owner()] = true;
    }

    function mint(address to, uint256 id) external onlyOperator {
        _mint(to, id);
        totalMinted++;
        emit SausagerMinted(id);
    }
...
contract KindaRandomRanges is Ownable {
    struct RandomRange {
        uint256 index;
        uint256 max;
        uint256 offset;
    }
    mapping(uint256 => mapping(uint256 => uint256)) idsByRangeId;

    mapping(uint256 => RandomRange) rangeById;

    constructor() {}

    function generateKindaRandomID(uint256 rangeId, uint256 randomIndex) internal virtual returns (uint256) {
        RandomRange storage range = rangeById[rangeId];
        require(range.max > 0, "Range not configured");

        uint256 remainder = range.max - range.index;
        uint256 available;
        uint256 result;

        if (idsByRangeId[rangeId][remainder - 1] == 0) {
            available = remainder - 1;
        } else {
            available = idsByRangeId[rangeId][remainder - 1];
        }

        if (idsByRangeId[rangeId][randomIndex] == 0) {
            result = randomIndex;
            idsByRangeId[rangeId][randomIndex] = available;
        } else {
            result = idsByRangeId[rangeId][randomIndex];
            idsByRangeId[rangeId][randomIndex] = available;
        }
    ...
    function addRange(uint256 rangeId, uint256 index, uint256 max, uint256 offset) public onlyOwner {
        require(max > index, "max must be greater than starting index");
        rangeById[rangeId] = RandomRange(index, max, offset);
    }
}
