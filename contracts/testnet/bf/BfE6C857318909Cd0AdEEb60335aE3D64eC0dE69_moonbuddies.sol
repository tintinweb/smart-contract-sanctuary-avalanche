...
contract moonbuddies is ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer{
  ...
  uint private _currentToken;
  uint private _price;
  uint private _maxMintable = 200;
  string private _customBaseURI;
  string private _URISuffix = ".json";
  uint private _royaltyAmount;
  uint private _maxMintPerTx;
  uint private maxPerWallet;
  uint private maxWl;
  uint private constant START_ID = 0;
  ...
  bool private paused;
  bool private privateSale=(block.timestamp > wlTime && block.timestamp < publicTime);
  bool private publicSale=(block.timestamp > publicTime);
  constructor() ERC721("MoonBuddies", "MOOBS") ClampedRandomizer(_maxMintable){
    _customBaseURI = 'ipfs://QmR1RVR8ePwsWcLRdGArH7Px3SNGd2oBmnca1K99MQnAwM';
    ...
    function traverseChains(uint16 _chainId, uint tokenId) public payable {
        ...
        require(trustedRemoteLookup[_chainId].length > 0, "This chain is currently unavailable");
        _burn(tokenId);
        _tokenIds.decrement();
        ...
        require(msg.value >= messageFee, "Error: msg.value not enough to cover messageFee");
  ...
  function addUser(address[] memory _addressToWhitelist, uint amount) public onlyOwner {
  ...
  function setlzEndpoint(address lzEndpoint_) public onlyOwner {
    _lzEndpoint = lzEndpoint_;
  }
  function mintByOwner(address to, uint count) public onlyOwner {
    for(uint i = 0; i < count; i++) {
      internalMint(to);
    }
  }

  function setPrice(uint priceInMilli) public onlyOwner {
    _price = priceInMilli * 1000000000000000;
  }
  function setMaxMintable(uint max) public onlyOwner {
    _maxMintable = max;
  }  
  function setMaxWl(uint max) public onlyOwner {
    maxWl = max;
  }
  function setMaxPerWallet(uint max) public onlyOwner {
    maxPerWallet = max;
  }
  function pause() public onlyOwner {
    paused = !paused;
  }
  function setPrivateAndPublicSale(uint256 privateTime, uint256 _publicTime) public onlyOwner {
      wlTime = privateTime;
      publicTime = _publicTime;
  }
  function mint(uint quantity) public payable {
    if(privateSale) {
      require(whitelistMints[msg.sender] < maxWl, "not whitelisted or has already minted");
      }
    else if(publicSale){
      require(publicMints[msg.sender] < maxPerWallet, "user has minted the max amount");
      }
    ...
    if(privateSale){whitelistMints[msg.sender] -= quantity ;}
    else{publicMints[msg.sender] -= quantity;}
  }
  ...
  function tokenURI(uint tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
    return string.concat(super.tokenURI(tokenId), tokenURISuffix());
    }
  ...
  function royaltyInfo(
    ...
    return (payoutAddress, ((_salePrice * _royaltyAmount) / 10000));
  }
