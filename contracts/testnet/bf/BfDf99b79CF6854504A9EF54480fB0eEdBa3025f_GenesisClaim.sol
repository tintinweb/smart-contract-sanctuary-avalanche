...
contract GenesisClaim is Ownable, Pausable, Multicall {
	...
	IClaim internal immutable previous = IClaim(0x0ee1c6dE96FFa69fC84Ab43bb3aCefbF4d29b535); // 43113
	...
	function _purchase(uint256 _nftsToPurchase) internal {
		...
		uint256 reserved = reservedRemaining(msg.sender);
		uint256 decimals = usdc.decimals();

		// * Tried info.reserved == type(uint256).min - working
		if (reserved == type(uint256).min || reserved == 0 || whitelistWindow == false) {
			// TODO --- HERE --- TODO //
			...
			require(_nftsToPurchase <= reserved, "Exceeds reservation!");
		...
		remainingMint = remainingMint.sub(_nftsToPurchase);
