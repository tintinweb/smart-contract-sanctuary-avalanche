...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	address private nonAffiliateWallet;

	//fees
	uint256 public ticketPrice = 0.1 ether;
	uint256 public prizeFee = 5000; //50%
	uint256 public rollOverFee = 2000; //20%
	uint256 public buyBackFee = 1000; //10%
	uint256 public dividentsFee = 1000; //10%
	uint256 public insuranceFee = 500; //5%
	uint256 public affiliateRewardsFee = 500; //5%

	//game settings
	uint256 public newTicketStep = 30 seconds; // every ticket purchased adds this much to the timer
	uint256 public roundInitialTime = 5 minutes;
	uint256 public roundMaxTime = 24 hours; // max time a round
	...
	constructor() {
		multisigWallet = 0xE6c0705c9c3615F820C26Eed226C1A06cd0049F1; // _msgSender();
		devWallet = 0xE6c0705c9c3615F820C26Eed226C1A06cd0049F1; // _msgSender();
		buybackWallet = 0xE6c0705c9c3615F820C26Eed226C1A06cd0049F1; // _msgSender();
		insuranceWallet = 0xE6c0705c9c3615F820C26Eed226C1A06cd0049F1; // _msgSender();
		nonAffiliateWallet = 0xE6c0705c9c3615F820C26Eed226C1A06cd0049F1; // _msgSender();

		//starts a round
		Round memory _round;
		_round.leader = address(0);
		_round.isActive = true;
		_round.endTime = block.timestamp + roundInitialTime;
		rounds[currentRoundIndex] = _round;
	}

	function delBuildNumber0() internal pure {} //etherscan trick
	...
	function buyTicket(address affiliateAddress)
		...
		nonReentrant
	{
		Round memory _currentRound = rounds[currentRoundIndex];
		require(_currentRound.isActive, "round not active");
		require(msg.value == ticketPrice, "invalid price");
		require(_msgSender() != affiliateAddress, "invalid affiliate");

		if (block.timestamp >= _currentRound.endTime) {
			_asyncTransfer(_msgSender(), ticketPrice); //refund the ticket
			distributeEarnings();
			startNextRound();
			return;
		}
		...
		uint256 dividentsAmount = _calcPercentage(ticketPrice, dividentsFee);
		_asyncTransfer(_msgSender(), dividentsAmount);

		Round storage currentRound = rounds[currentRoundIndex];
		currentRound.endTime = currentRound.endTime + newTicketStep;
	...
	function forceStartNextRound() external onlyDev {
		distributeEarnings();
		startNextRound();
	}
	...
	function setGameSettings(
		uint256 _newTicketStep,
		uint256 _roundInitialTime,
		uint256 _roundMaxTime
	) external onlyOwner {
		require(_roundMaxTime > _newTicketStep, "invalid values 1");
		require(_roundInitialTime > _newTicketStep, "invalid values 2");
		require(_roundInitialTime < _roundMaxTime, "invalid values 3");
		newTicketStep = _newTicketStep;
		roundInitialTime = _roundInitialTime;
		roundMaxTime = _roundMaxTime;
	}
	...
	// withdrawal to owner of all funds (used for testing)
	...
	function distributeEarnings() internal {
		...
		uint256 buyBackAmount = _calcPercentage(address(this).balance, buyBackFee); //10%
		uint256 insuranceAmount = _calcPercentage(address(this).balance, insuranceFee); //5% of total earnings
		...
		//transfer buyback to multisig
		payable(buybackWallet).transfer(buyBackAmount);

		//transfer insurance to multisig
		payable(insuranceWallet).transfer(insuranceAmount);
	}
	...
	function startNextRound() internal {
		...
		oldRound.endTime = block.timestamp;
