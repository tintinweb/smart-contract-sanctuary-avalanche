...
contract EVICNodePool is ContractGuard {
    ...
    // todo
    uint256 public constant runningTime = 1 hours; // 21 days
    ...
    constructor(address _polWallet, address _chamToken, uint256 _poolStartTime) {
    ...
    // Update the given pool's ChamToken allocation point. Can only be called by the owner.
    ...
    function claimReward(uint256 _pid) external onlyOneBlock {
        ...
        uint256 _pending = user.amount.mul(pool.accChamTokenPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            require(block.timestamp > poolEndTime, "Not enough time to claim reward");
            safeShareTokenTransfer(_sender, _pending);
            emit RewardPaid(_sender, _pending);
        }
    }

    // Safe ChamToken transfer function, just in case if rounding error causes pool to not have enough ChamToken.
    function safeShareTokenTransfer(address _to, uint256 _amount) internal {
        uint256 _shareTokenBalance = chamToken.balanceOf(address(this));
        if (_shareTokenBalance > 0) {
            if (_amount > _shareTokenBalance) {
                chamToken.safeTransfer(_to, _shareTokenBalance);
            } else {
    ...
    function emergencyWithdraw(uint256 _pid) external onlyOneBlock {
        ...
        uint256 _pending = user.amount.mul(pool.accChamTokenPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeShareTokenTransfer(msg.sender, _pending);
            emit RewardPaid(msg.sender, _pending);
        }

        uint256 amount = user.amount;
        uint256 taxAmount = user.amount.mul(taxRate).div(10000);
        uint256 userAmount = user.amount.sub(taxAmount);
        user.amount = 0;
