...
contract Marketplace is OwnableUpgradeable, PausableUpgradeable, ERC721Holder {
  ...
  modifier onlyExecutor() {
    require(jobExecutor == msg.sender, "Calller is not job executor");
    _;
  }
  ...
  function cancel(bytes32 _itemId) public whenNotPaused {
    ...
    require(item.seller == msg.sender, "Not token owner");
  ...
  function buy(bytes32 _itemId) public whenNotPaused {
    ...
    _executePayment(_itemId, msg.sender);

    item.buyer = msg.sender;
    item.saleStatus = SaleStatus.Sold;

    IERC721(item.nftAddress).transferFrom(item.seller, msg.sender, item.tokenId);
  ...
  function bid(
    ...
    /// the bid must be at least +5% from the last bid price
    require(_price >= (item.topBidPrice + item.topBidPrice.div(1000).mul(50)), "The bid must be at least +5% from the last bid price");
    ...
    if (item.saleType == SaleType.Auction && item.saleStatus == SaleStatus.Open) {
      ...
      if (_price > item.topBidPrice) {
        item.topBidIndex = bidIndex;
        item.topBidPrice = _price;
        item.topBidder = msg.sender;
      }

      emit ItemBid(_itemId, msg.sender);
    }
  }
  ...
  function swap(
    ...
    require(msg.sender == fromTokenOwner, "Not token owner");
    require(
  ...
  function approveSwap(bytes32 _swapId) public whenNotPaused {
    ...
    require(_swap.toUser == msg.sender, "Not token owner");
    require((fromCollection.ownerOf(_swap.fromTokenId) == _swap.fromUser) && (toCollection.ownerOf(_swap.toTokenId) == msg.sender), "Not token owner");
    require(
  ...
  function cancelSwap(bytes32 _swapId) public whenNotPaused {
    Swap storage _swap = swaps[_swapId];
    require(_swap.fromUser == msg.sender, "Not token owner");
   ...
   */
  function _executePayment(bytes32 _itemId, address _sender) internal virtual {
    Item storage item = _items[_itemId];
  ...
  function createCollection(
    ...
    (bool royaltySupported) = IERC2981(_nftContractAddress).supportsInterface(_INTERFACE_ID_ERC2981);
    
    collections[_nftContractAddress] = Collection({
      active: _active,
      royaltySupported: royaltySupported
    });
  ...
  /**
   * @notice Get fee collector details
   * @param _index FeeCollector index
   */
  function getFeeCollector(uint _index) public view onlyOwner returns(FeeCollector memory) {
    if (_index < _feeCollectors.length) {
      FeeCollector storage feeCollector = _feeCollectors[_index];
      return feeCollector;
    } else {
      FeeCollector memory removedFeeCollector;
      return removedFeeCollector;
    }
  }
   ...
   * @param _index FeeCollector index
   */
  function removeFeeCollector(uint _index) public onlyOwner {
    FeeCollector memory removedFeeCollector;
      ...
      if (auctionExpiry.expiresAt < block.timestamp && !auctionExpiry.executed) {
        ...
        if (item.saleStatus == SaleStatus.Open) {
          if (item.bids.length > 0) {
            ...
            IERC721(item.nftAddress).transferFrom(item.seller, item.buyer, item.tokenId);
          ...
          emit AuctionExpiryExecuted(auctionExpiry.itemId, msg.sender);
        }
      }
    }
  }
