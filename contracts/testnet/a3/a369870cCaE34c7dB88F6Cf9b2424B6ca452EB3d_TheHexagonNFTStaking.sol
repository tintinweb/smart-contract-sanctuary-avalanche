...
contract TheHexagonNFTStaking is IERC721Receiver, Ownable, Pausable {
    ...
    mapping(uint256=>Stake) public stakeOfTokenId;
    mapping(address=>uint256[]) public tokenIdOf;
    mapping(uint256=>uint256) public tokenIndexOfId;
    mapping(address=>mapping(uint256=>bool)) public isTokenStaking;
    mapping(uint256=>uint256) public salaryOfMode;
    mapping(uint256=>uint256) public lowPercentOfMode;

    mapping(uint256=>uint256) public stolenOf;
    mapping(uint256=>uint256) public winbackOf;
