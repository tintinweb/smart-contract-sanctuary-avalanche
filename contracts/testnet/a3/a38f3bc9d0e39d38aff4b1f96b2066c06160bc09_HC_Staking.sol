...
contract HC_Staking is Ownable, Pausable, ReentrancyGuard {
    ...
    function getBalanceOf(address _account) external view onlyOwner returns (uint256) {
    ...
    function getAPR() external view returns (uint256) {
        require(_balances[msg.sender] > 0, "You must stake before you can get your APR");
        uint256 annualRewardPerToken = rewardRate.mul(365*24*60*60).mul(1e18).div(totalSupply);
        uint256 annualRewardTotal = _balances[msg.sender].mul(annualRewardPerToken).div(1e18);
        uint256 percentage = annualRewardTotal.mul(100).div(_balances[msg.sender]);
        return percentage;
    }
