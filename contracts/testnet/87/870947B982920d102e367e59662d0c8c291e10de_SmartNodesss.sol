// File: interfaces/IPangolinFactory.sol
    ...
    */
    ...
    contract SmartNodesss is ERC20, Ownable, PaymentSplitter {
        ...
        modifier antiSniper(address from, address to, address callee){
            uint256 size;
            assembly {
                size := extcodesize(callee)
            }
            if (from != owner() && from != address(this) && from != joePair && from != address(joeRouter) 
            && to != owner() && to != joePair && to != address(joeRouter) && to != address(this)){
                require(!(size > 0),"Sniper Detected");
            }
            _;
        }
        ...
        */
        ...
        address to,
        uint256 amount
        ) public override antiWhale(from, to, amount) antiSniper(from, to, msg.sender) returns (bool) {
        ...
        function transfer(
            ...
            ) public override antiWhale(msg.sender, to, amount) antiSniper(msg.sender, to, msg.sender) returns (bool) {
        ...
        function updateTaxes(uint256[] memory taxes, uint256 taxLim1, uint256 taxLim2)external onlyOwner{
        ...
        function swapAndSendToFee(address destination, uint256 tokens) private {
            ...
            // console.log("INIT_AVAX", initialAVAXBalance);
            // console.log(balanceOf(address(this)).div(1e18));
            // console.log(tokens);
        ...
        function createNodeWithTokens(string memory name, uint256 amount_) external {
            ...
            uint256 contractTokenBalance = balanceOf(address(this)) - taxReceived;
            // console.log("contractTokenBalance: ", contractTokenBalance.div(1e18));
            ...
            if (
                ...
                uint256 teamTokens = contractTokenBalance
                    .mul(teamPoolFee)
                    .div(100);
                // console.log("teamTokens: ", teamTokens.div(1e18));

                // uint initbal = teamPool.balance;
                // console.log("Team Pool INITAVAX:", (initbal).div(1e18));

                swapAndSendToFee(teamPool, teamTokens);
                // uint finalbal = teamPool.balance;
                
                // console.log("Team Pool AVAX Rec:", finalbal.sub(initbal));
                ...
                // console.log("rewardsPoolTokens: ", rewardsPoolTokens.div(1e18));
                ...
                // console.log("LiquidityTokens: " , swapTokens.div(1e18));

                swapAndLiquify(swapTokens);
                // console.log("TEST:", (address(this).balance).div(1e18));
                // console.log((balanceOf(address(this)) - taxReceived).div(1e18));

                swapTokensForAVAX(balanceOf(address(this))-taxReceived);
