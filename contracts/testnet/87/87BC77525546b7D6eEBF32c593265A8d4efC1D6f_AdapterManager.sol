...
contract AdapterManager is IAdapterManager, Ownable {
    ...
    function receiveCallFromController(bytes calldata _callArgs)
        external
        override
        onlyUser
        whenNotPaused
    ...
    {
        // Dispatch the action
        return __callOnAdapter(msg.sender, _callArgs);
    }

    function __callOnAdapter(address _controller, bytes memory _callArgs)
        internal
        returns (bytes memory ret)
    {
        (address adapter, bytes4 selector, , , bytes memory adpaterData) = abi
            .decode(_callArgs, (address, bytes4, uint256, bool, bytes));
        ...
        ret = __callOnAdapterInner(_callArgs);

        emit CallOnAdapterExecuted(
            msg.sender,
            _controller,
            adapter,
            selector,
    ...
    function __callOnAdapterInner(bytes memory _callArgs)
        internal
        returns (bytes memory)
    {
        (address adapter, bytes4 selector, , , bytes memory adpaterData) = abi
        ...
        (bool success, bytes memory returnData) = adapter.call(
            abi.encodeWithSelector(selector, adpaterData)
        );
