...
interface ISharedData {
    ...
    struct IDOParams {
        ...
        address _admin;
        address _manager;
        //["100000000000000000000","100000000000000000000000",10000,1000000,1655890547,1655894147,10,"0x89d1a873921dd736B780E3ec61616850B8e93Ac9","0x4c701d94572eCd5464aa95CFD4E6aA70615Aad80","0x4c701d94572eCd5464aa95CFD4E6aA70615Aad80",["0x89d1a873921dd736B780E3ec61616850B8e93Ac9",10],[1655890547,10]]
    }
}
contract IDO is
    Initializable,
    ...
    ISharedData
{
    ...
    function initialize(IDOParams memory params, 
        ...
        for (uint256 index = 0; index < _vestingInfo.length; index++) {
            vestingInfo.push(
                VestingInfo(
                    _vestingInfo[index]._time,
                    _vestingInfo[index]._percent
                )
            );
        }
        ...
        if (isMainTokenAllowed) {
            ...
            for (uint256 index = 0; index < tokensLength; index++) {
                addToken(
                    IERC20(_tokens[index]._tokenAddress),
                    _tokens[index]._presaleRate
                );
            }
        }
    }
    ...
    function claim() public virtual whenNotPaused {
        ...
        TokenInfo storage _tokenInfo = tokenInfo[allowanceToUserInfo[msg.sender].tokenInfoId];
        ...
        uint256 claimAmount = (_tokenInfo.presaleRate *allowanceToUserInfo[msg.sender].deposit) / (1 ether);

        require(
            claimAmount > allowanceToUserInfo[msg.sender].claimedAmount,
            "Clamed all allocation"
        );
        uint256 calculatedClaimAmount = (claimAmount * allowedPercentage) /(1 ether) /100;
        ...
        if (
            IERC20(tokenAddress) ==
            IERC20(0x81C8Fd5B7eAd8958BeAA7766EEc7c40dB26945D5)
        ) {
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        for (
            ...
            require(
                params._vestingInfo[index + 1]._time >=params._vestingInfo[index]._time,
    ...
    function getTokenInfoId(IERC20 _token) public view returns (uint256) {
