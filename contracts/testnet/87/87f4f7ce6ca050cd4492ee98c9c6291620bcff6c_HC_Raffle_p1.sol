...
/**
 * @dev HeroesChained Project base NFT class.
 */
 contract HC_NFT_Base is Ownable {
    using SafeMath for uint256;    

    uint public constant MAX_SUPPLY = 10000;
    uint private totalAllowedNFTCount;

    mapping(address => uint) private addressIndices;
    Info[] private allowedAddresses;
    ...
    modifier onlyAllowedAddresses() {
        require(allowedAddresses.length > 0, "Address is not allowed to mint NFTs.");
        uint256 _index = addressIndices[ msg.sender ];
        require( msg.sender == allowedAddresses[ _index ].allowedAddress, "Address is not allowed to mint NFTs." );
        _;
    }

    function addAllowedAddress(address _address, uint _allowedNFTCount) external onlyOwner {
        require( _address != address(0), "Address cannot be empty." );        
        require( _allowedNFTCount > 0, "NFT count must be greater than 0." );

        uint256 index = addressIndices[ _address ];
        if (allowedAddresses.length > 0) {
            ...
            {
                require(_address != allowedAddresses[0].allowedAddress, "Address already exists.");
            }
        }
        require( _allowedNFTCount.add(totalAllowedNFTCount) <= MAX_SUPPLY, "Total NFT count cannot exceed MAX_SUPPLY." );

        allowedAddresses.push( Info({
                    allowedAddress: _address,
                    initialBalance: _allowedNFTCount,
                    remainingBalance: _allowedNFTCount
        }));

        addressIndices[_address] = allowedAddresses.length.sub(1);
        totalAllowedNFTCount = totalAllowedNFTCount.add(_allowedNFTCount);
    }

    function removeAllowedAddress(address _address) external onlyOwner {
        require(_address != address(0), "Address cannot be empty." );        
        require(allowedAddresses.length > 0, "Address not found.");

        uint256 _index = addressIndices[ _address ];
        require( _address == allowedAddresses[ _index ].allowedAddress, "Address to be removed not found in list." );

        totalAllowedNFTCount = totalAllowedNFTCount.sub( allowedAddresses[ _index ].remainingBalance );

        allowedAddresses[ _index ].allowedAddress = address(0);
        allowedAddresses[ _index ].initialBalance = 0;
        allowedAddresses[ _index ].remainingBalance = 0;

        delete addressIndices[_address];
    }

    function getMintableNFTBalance(address _address) public view returns (uint) {
        require(_address != address(0), "Address cannot be empty." );        
        require(allowedAddresses.length > 0, "Address not found.");

        uint256 _index = addressIndices[ _address ];
        require( _address == allowedAddresses[ _index ].allowedAddress, "Address is not allowed to mint NFTs." );

        return (allowedAddresses[ _index ].remainingBalance);
    } 

    function decreaseNFTAllowance() internal {
        uint256 _index = addressIndices[ msg.sender ];
        allowedAddresses[ _index ].remainingBalance = allowedAddresses[ _index ].remainingBalance.sub(1);
    }

    function getAllowedAddress(uint256 _index) view external onlyOwner returns(Info memory) {
        return allowedAddresses[_index];
    }
} 

/**
 * @dev HeroesChained Project ERC721 token for special MayinHara heroes.
 */
contract HC_MayinHara_NFT is ERC721, Pausable, Ownable, HC_NFT_Base, ReentrancyGuard {
    using SafeMath for uint256;    
    ...
    function renounceOwnership() public view override onlyOwner {
        revert ("Renouncing ownership is prevented.");
    }

    function mint(address to) public nonReentrant whenNotPaused onlyAllowedAddresses {
        require (getMintableNFTBalance(msg.sender) > 0, "No more NFT allowance for minting.");
        require (_tokenIdCounter.current() < MAX_SUPPLY, "MAX_SUPPLY reached. Cannot mint new NFT.");

        _tokenIdCounter.increment();

        uint256 tokenId = _tokenIdCounter.current();

        decreaseNFTAllowance();
...
contract HC_Raffle_p1 is Ownable, ReentrancyGuard, Pausable {
    ...
    /* ========== STATE VARIABLES ========== */
    HC_MayinHara_NFT public nftContractAddress;
    ...
    ) 
    {
        ...
        nftContractAddress = HC_MayinHara_NFT(_nftContractAddress);
    ...
    function drawPhase1(uint256 _drawCount) external onlyOwner returns (bool) {
        ...
        uint256 random;
        address winner;
        uint256 lastIndex = mintableNFTs + _drawCount;

        while (mintableNFTs < lastIndex && participants.length > 0) {
            seed = block.number;
            ...
            participants.removeByIndex(random);
        }
    ...
    function collectTokens(address to) external onlyOwner {
        require(isRaffleCompleted, "Raffle has not ended. Cannot withdraw funds at the moment.");
        uint256 amount = depositTokenAddress.balanceOf(address(this));
    ...
    {
        return depositTokenAddress.balanceOf(address(this));
    }
