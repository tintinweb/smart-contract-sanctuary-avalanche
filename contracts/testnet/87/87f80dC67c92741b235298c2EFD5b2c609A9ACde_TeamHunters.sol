...
contract TeamHunters is ERC721EnumerableV2 {
  ...
  function test(uint256 roundId) public view returns (Round memory) {
    return rounds[roundId];
  }

  function test1(uint256 roundId) public view returns (uint256[] memory teams) {
    teams = rounds[roundId].teams;
  }

  function test2(uint256 roundId) public view returns (uint8[8] memory places) {
    places = rounds[roundId].places;
  }
