...
contract Pyramid {
    ...
    //PARAMETERS
    uint256 public DEFAULT_PRICE = 0.02 ether;
    uint256 public MAX_BLOCKS = 400000;

    uint256 public INCREASE_FACTOR = 1500; // = 15%

    uint256 public OWNER_REBUY_SHARES = 8500; // = 85%

    uint256 public constant BASE = 10000; // = 100%
    
    address payable public pharaon;
    ...
    mapping(uint256 => bool) public built;
    ...
    //Modifiers 
    modifier onlyPharaon ()  {
        require(pharaon == msg.sender, "Not the pharaon");
        _;
    }

    constructor(address _wAVAX) {
        pharaon = payable(msg.sender);
        wAVAX = IWAVAX(_wAVAX);
        feeDistributor = new FeeDistributor(address(this), address(wAVAX));
    }

    function buyBlocks (uint256[] memory _blocks, bytes memory _color) payable public {
        ...
        for (uint i; i < _blocks.length; i++) {
            ...
            //check if there is still enough to afford the block
            uint256 blockPrice = _getPrice(blockId);
            require(remainingAmount >= 0, "Not enough money");
            ...
            if(built[blockId]) {
                ...
                address payable lastOwner = payable(ownerOf[blockId]);
                require(msg.sender != lastOwner, "Cannot buy its own block");
                uint256 ownerShare = blockPrice.mul(OWNER_REBUY_SHARES).div(BASE);
                lastOwner.transfer(ownerShare);
            ...
            remainingAmount = remainingAmount.sub(blockPrice);
        }
    ...
    function setColor (bytes calldata _newColor, uint256[] memory _blocks) public {
        for (uint i; i < _blocks.length; i++) {
            uint256 blockId = _blocks[i];
            require(ownerOf[blockId] == msg.sender, "Not the block owner");

            colorOf[blockId] = _newColor;
        }
    }

    function getTotalPrice (uint256[] memory _blocks) public view returns (uint256) {
        uint256 sum = 0;
        for (uint i; i < _blocks.length; i++) {
            sum = sum.add(_getPrice(_blocks[i]));
        }
    ...
    function setPharaon (address payable _new) onlyPharaon public {
        require(_new != address(0));
        pharaon = _new;
    }

    function pauseRewards() onlyPharaon public {
        feeDistributor.pause();
    }

    function setRewards(bytes32 _merkleRoot) onlyPharaon public {
    ...
    function _getPrice (uint256 _id) internal view returns (uint256) {
        ...
        return built[_id] ? priceOf[_id] : DEFAULT_PRICE;
    }
}
