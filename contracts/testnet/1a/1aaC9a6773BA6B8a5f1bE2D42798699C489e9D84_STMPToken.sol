...
// File contracts/SwipeToMineERC20.sol

// contracts/GLDToken.sol

pragma solidity ^0.8.7;

error ReachedCappedAmount();

contract STMPToken is ERC20PresetMinterPauser {
    uint256 private immutable i_capAmount = 200000000 * 10**18;

    constructor(uint256 initialSupply, string memory coinName)
        ERC20PresetMinterPauser(coinName, "STM")
    {
    ...
    function mint(address to, uint256 amount) public virtual override {
        ...
        // console.log(i_capAmount);
        if ((totalSupply() + amount) > i_capAmount) {
            revert ReachedCappedAmount();
        }
        super.mint(to, amount);
    }

    //overriding so transfer uses contract ass address of "pot of tokens"
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        //address owner = _msgSender();
    ...
    function getCapAmount() public pure returns (uint256) {
        return i_capAmount;
    }
}