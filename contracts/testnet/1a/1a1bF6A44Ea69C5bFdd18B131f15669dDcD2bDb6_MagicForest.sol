...
contract MagicForest is Controllable, IERC721Receiver, Pausable, Restricted {
    ...
    event RangerClaimed(uint256 tokenId, uint256 earned, bool unstaked);
    event SkeletonClaimed(uint256 tokenId, uint256 earned, bool unstaked);
    ...
    /**
     * Stakes one Ranger or Skeleton in the MagicForest
     * @param tokenId Ids of the Ranger or Skeleton to stake
     */
    function stakeOne(uint16 tokenId) external onlyEOA noReentrency {
        _stakeOne(_msgSender(), tokenId);
    }
    ...
    function _stakeOne(address account, uint16 tokenId) internal {
        ...
        if (isRanger(tokenId)) {
            ...
            _stakeSkeleton(account, tokenId);
        }
     ...
     */
    function _stakeSkeleton(address account, uint256 tokenId) internal {
        ...
        _updateEarnings
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (isRanger(tokenIds[i])) {
                owed += _claimRanger(tokenIds[i], unstake);
            } else {
    ...
    function calculateRewards(uint256 tokenId) external view returns (uint256 owed) {
        if (isRanger(tokenId)) {
           return _calculateRangerRewards(tokenId, barn[tokenId].value);
        } else {
            uint8 alpha = _alphaForHunter(tokenId);
            return _calculateSkeletonRewards(alpha, pack[alpha][packIndices[tokenId]].value);
        }
    }

    function _calculateRangerRewards(uint256 tokenId, uint80 stakeValue) internal view returns(uint256) {
    ...
    function _calculateSkeletonRewards(uint8 alpha, uint80 stakeValue) internal view returns(uint256) {
        return alpha * (glowPerAlpha - stakeValue);
    }
     ...
     */
    function _claimRanger(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        Stake memory stake = barn[tokenId];

        require(stake.owner == _msgSender(), "Not your token");
        if (unstake) require(block.timestamp - stake.value < getMinimumToExit(tokenId), "Need to wait some days before unstake");

        owed = _calculateRangerRewards(tokenId, barn[tokenId].value);
    ...
    function _claimHunterFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        require(stake.owner == _msgSender(), "Not your token");
        owed = _calculateSkeletonRewards(alpha, stake.value);
        if (unstake) {
    ...
    function getDailyGlowrate(uint256 tokenId) public view returns(uint256){
        ...
        AttributesPets.Boost memory boostWallet = pets.getWalletBoost(msg.sender);
    ...
    function getLevel(uint256 tokenId) public view returns (uint8 level) {
