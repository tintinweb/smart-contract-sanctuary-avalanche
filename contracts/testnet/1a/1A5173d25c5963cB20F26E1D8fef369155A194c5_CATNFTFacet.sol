...
contract CATNFTFacet is Context {
    ...
    */
	function deployNFT(
		...
		CATERC721(tokenAddress).initialize(diamondStorage.chainId, diamondStorage.wormhole, diamondStorage.finality , totalSupply);
		Ownable(tokenAddress).transferOwnership(owner);
		emit NFTDeployed(owner, tokenAddress, name, symbol, salt);
	}
	
	
	function computeAddress(bytes32 salt, string calldata name, string calldata symbol) internal view returns (address addr) {
