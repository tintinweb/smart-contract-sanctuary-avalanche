...
contract NodeManager is Ownable {
  ...
  uint32 public discountPer10 = 10; // 0.1%
  uint32 public withdrawRate = 0; // 0.00%
  uint32 public transferFee = 1000; // 10%
  uint32 public rewardsPoolFee = 7000; // 70%
  uint32 public treasuryFee = 2000; // 70%
  uint32 public operatorFee = 1000; // 70%
  uint32 public maxCountOfUser = 100; // 0-Infinite
  ...
  function _transferOperatorFee(uint256 feeOperator) public {
    ...
    for (uint32 i = 0; i < operators.length; i++) {
      if (i == operators.length - 1) {
        _transferAVAX(operators[i], feeOperator);
      } else {
        _transferAVAX(operators[i], feeEachOperator);
  ...
  function _transferFee(uint256 amount) private {
    ...
    IERC20(tokenAddress).transferFrom(address(msg.sender), address(this), amount);

    _transferAVAX(treasury, feeTreasury);
  ...
  function _transferAVAX(address recipient, uint256 amount) private {
  ...
  function _claim(uint256 exceptAmount) private {
    ...
    IERC20(tokenAddress).transfer(address(msg.sender), claimableAmount.sub(exceptAmount));
  }
  ...
  function upgrade(
    ...
    if (claimableAmount > 0) {
      rewardsOfUser[msg.sender] = rewardsOfUser[msg.sender].add(claimableAmount);
      rewardsTotal = rewardsTotal.add(claimableAmount);
      IERC20(tokenAddress).transfer(address(msg.sender), claimableAmount);
    }
  ...
  function transfer(
    ...
    if (fee > claimableAmount)
      ...
      IERC20(tokenAddress).transfer(address(msg.sender), claimableAmount.sub(fee));
  ...
  function pay(uint8 count) public {
