...
contract Tree is ITree, ERC721Enumerable, Ownable, Pausable {
    ...
    mapping(uint256 => TreeStr) public tokenTraits;
     ...
     * The first 20% are free to claim, the remaining cost $Leaves
     */
    ...
    function _normal_mint(uint256 amount, bool stake) private onlyWhileClose {
        ...
        if (minted < PAID_TOKENS) {
            ...
            require(
                msg.value == 0,
                "Do not send AVAX, minting is with Leaves now"
            );
        }
    ...
    function core_mint(uint256 amount, bool stake) private {
        ...
        for (uint256 i = 0; i < amount; i++) {
           ...
           // } else {
                Tree_minted += 1;
          //  }
            totalLeavesCost += mintCost(minted); // 0 if we are before 10.000
        }

        //we may want to do that first but w/o reentrancy
        if (totalLeavesCost > 0) leaves.burn(_msgSender(), totalLeavesCost);
     ...
     * the next 20% are 20000 $Leaves
     * the next 40% are 40000 $Leaves
     * the final 20% are 80000 $Leaves
   ...
   /*  function transferFrom(
        ...
        returns (TreeStr memory t)
    {
        ...
        seed >>= 16;
        t.envs = selectTrait(uint16(seed & 0xFFFF), 0);
        seed >>= 16;
        t.shadows = selectTrait(uint16(seed & 0xFFFF), 1);
        seed >>= 16;
        t.barkNumber = selectTrait(uint16(seed & 0xFFFF), 2);
        seed >>= 16;
        t.barkMaterial = selectTrait(uint16(seed & 0xFFFF), 3);
        seed >>= 16;
        t.leavesMaterial = selectTrait(uint16(seed & 0xFFFF), 4);        
    }
