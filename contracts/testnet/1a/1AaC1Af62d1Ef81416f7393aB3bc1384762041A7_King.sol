...
contract King is IKing, ERC721Enumerable, Ownable, Pausable {
    ...
    uint16 public Peasant_minted;
    uint16 public King_minted;
    // Pre mint
    ...
    mapping(uint256 => uint256) public kingIds;
    mapping(uint256 => uint256) public peasantIds;
    ...
    // 0 - 6 are associated with Peasants, 7 - 12 are associated with kings
    uint8[][12] public rarities;
    // list of aliases for Walker's Alias algorithm
    // 0 - 6 are associated with Peasants, 6 - 12 are associated with Kings
    uint8[][12] public aliases;

    // reference to the Field for choosing random King thieves
    IField public field;
    // reference to $CORN for burning on mint
    ICORN public corn;
    // reference to Traits
    ...
    /*
     * mint a token - 90% Peasants, 10% Kings
     * The first 20% are free to claim, the remaining cost $CORN
     */
    ...
    function _normal_mint(uint256 amount, bool stake) private onlyWhileClose {
        ...
        if (minted < PAID_TOKENS) {
            ...
            require(
                msg.value == 0,
                "Do not send AVAX, minting is with CORN now"
            );
        }
    ...
    function core_free_mint(uint256 amount, bool stake) private {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            if (!stake || recipient != _msgSender()) {
                ...
                _safeMint(address(field), minted);
                tokenIds[i] = minted;
            }
            if (tokenTraits[minted].isPeasant) {
                if(Peasant_minted == 9000) {
                    Peasant_minted = 1; 
                    peasantIds[minted] = Peasant_minted;
                } else {
                    Peasant_minted += 1;
                    peasantIds[minted] = Peasant_minted;
                }
        ...
        if (totalRichCost > 0) corn.burn(_msgSender(), totalRichCost);
     ...
     * the next 20% are 20000 $CORN
     * the next 40% are 40000 $CORN
     * the final 20% are 80000 $CORN
     ...
     * the remaining 80% have a 10% chance to be given to a random staked peasant
     * @param seed a random value to select a recipient from
     * @return the address of the recipient (either the minter or the peasant thief's owner)
     */
    function selectRecipient(uint256 seed) internal view returns (address) {
        if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0)
            return _msgSender(); // top 10 bits haven't been used
        address thief = field.randomKingOwner(seed >> 144); // 144 bits reserved for trait selection
    ...
    /**
     * called after deployment so that the contract can get random peasants thieves
...
interface IField {
    function addManyToFieldAndPack(address account, uint16[] memory tokenIds)
        external;

    function randomKingOwner(uint256 seed) external view returns (address);
}
