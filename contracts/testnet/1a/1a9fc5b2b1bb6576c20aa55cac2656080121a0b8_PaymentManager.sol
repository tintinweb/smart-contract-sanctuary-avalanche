...
contract PaymentManager is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    uint256 private failedTotalBalance;
    uint32 public commissionPercentage;
    address payable public companyWallet;
    uint256 public maximumShareholdersLimit;

    event RoyaltyReceived(address sender, address indexed collection, uint256 indexed tokenId, address indexed royaltyReceiver, uint256 amount);
    event PlatformRemoved(address indexed platform);
    event PlatformWhitelisted(address indexed platform);
    ...
    function addPlatform(address _platform) external onlyOwner {
        ...
        emit PlatformWhitelisted(_platform);
    }
    ...
    function payout(
        ...
        uint256 remainder = _price - commission;
        if (commission > 0) {
    ...
    function withdrawFailedCredits() external nonReentrant {
        ...
        failedTotalBalance -= failedTransferCredits[msg.sender];
    ...
    function getFailedTotalBalance() onlyOwner external view returns (uint256) {
        return failedTotalBalance;
    }
