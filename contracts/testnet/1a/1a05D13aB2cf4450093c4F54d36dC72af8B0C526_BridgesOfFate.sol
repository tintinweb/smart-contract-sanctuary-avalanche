...
contract BridgesOfFate  {
    ...
    uint256 private _hybirdNumberProbility;
    ...
    event ParticipateOfPlayerInGame(bytes32 playerId);
 ...
 function _randomHybirdGame() internal view returns (uint256) {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules
    {
        ...
        if((_gameStatus.stageNumber > STAGES && _hybirdGameMap  < _hybirdNumberProbility) && (_dayDifferance(block.timestamp,_gameStatus.startAt) > _gameStatus.lastUpdationDay)){
        ...
        PlayerItem[playerId] = _member;
        GameStatusInitialized[latestGameId] = _gameStatus;
        emit ParticipateOfPlayerInGame(playerId);
    }
