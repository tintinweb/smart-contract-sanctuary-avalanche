...
contract PRVRouter {
    ...
    function convertAndStake(uint256 amount) external {
        ...
        uint amountAfterFee = IPRV(PRV).previewDeposit(amount);
        IERC20(PRV).approve(Staker, amountAfterFee);
        IRollStaker(Staker).depositFor(amountAfterFee, msg.sender);
    }
    ...
    function convertAndStake(uint256 amount, address _receiver) external {
        ...
        uint amountAfterFee = IPRV(PRV).previewDeposit(amount);
        IERC20(PRV).approve(Staker, amountAfterFee);
        IRollStaker(Staker).depositFor(amountAfterFee, _receiver);
    }
