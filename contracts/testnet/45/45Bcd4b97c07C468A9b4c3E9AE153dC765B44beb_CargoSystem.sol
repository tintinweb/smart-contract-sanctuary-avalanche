...
contract CargoSystem {
    ...
    function getCargoData(uint256 _cargoKey)
        public
        view
        returns (Cargo memory)
    {
        return cargoData[_cargoKey];
    }

    function getPersonCargos(uint256 _phone)
        public
        view
    ...
    //mevcut kullanıcının adresini güncelle
    function updatePersonHomeAddress(
        uint256 _phone,
        string memory _homeAddress
    ) public {
    ...
    //mevcut kullanıcının epostasını güncelle
    function updatePersonEposta(uint256 _phone, string memory _eposta) public {
        require(personData[_phone].phone == _phone, "Kayitli hesap yok.");
        Person storage person = personData[_phone];
    ...
    function createCargoNo(uint256 _phone, uint256 _personPrivateKey)
        public
        returns (uint256)
    {
        require(personData[_phone].phone == _phone, "Kayitli hesap yok.");
        require(
            personPrivateKey[_phone] > 0,
            "Kisisel Takip Numarasi bulunamadi."
        );
        uint256 rand = uint256(
            keccak256(abi.encodePacked(_phone, _personPrivateKey))
        );
    ...
    function updateCargoStatus(uint256 _cargoKey, string memory _status)
        public
    {
        require(
            cargoData[_cargoKey].status.length > 0,
            "Kisisel Takip Numarasi bulunamadi."
        );
