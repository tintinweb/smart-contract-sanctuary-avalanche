...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function chainBalanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            ...
            if (collateralByIssuer[_collateralCurrencyKey][account] > 0) {
                ...
                synthrBalance += collateralByIssuer[_collateralCurrencyKey][account]
    ...
    function issueSynthsOnBehalf(address issueForAddress, bytes32 _collateralKey, uint256 _collateralAmount, uint256 _synthToMint) external payable issuanceActive optionalProxy {
        if (_collateralAmount > 0) {
...
contract BaseWrappedSynthr is IERC20, ExternWrappedStateToken, MixinResolver, ISynthetix {
    ...
    constructor(
        ...
        public
        ExternWrappedStateToken(_proxy, TOKEN_NAME, TOKEN_SYMBOL, _totalSupply, DECIMALS, _owner)
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[4] = CONTRACT_LIQUIDATOR;
        addresses[5] = CONTRACT_COLLATERAL_AGGREGATOR;
        addresses[6] = CONTRACT_EXRATES;
        addresses[7] = CONTRACT_LIQUIDATOR_REWARDS_MANAGER;
    }
    ...
    function transfer(address, uint256) external onlyProxyOrInternal systemActive returns (bool) {
        _notImplemented();
        return true;
    }
    ...
    function collateralTransfer(address _to, bytes32 _collateralKey, uint256 _usdAmount) public systemActive optionalProxy returns (bool) {
        ...
        if (collateralCurrency[_collateralKey] == NULL_ADDRESS) {
            ...
            require(IERC20(collateralCurrency[_collateralKey]).balanceOf(address(this)) >= collateralAmount, "Insufficient collateral amount to transfer");
    ...
    function liquidateDelinquentAccount(address account, bytes32 collateralKey) external systemActive optionalProxy returns (bool) {
        ...
        if (totalRedeemed > 0) {
            ...
            bool liquidatorRewardTransferSucceeded = collateralTransfer(address(liquidatorRewards(collateralKey)), collateralKey, stakerRewards);
    ...
    function _mint(address _to, uint256 _synthrAmount, bytes32 _collateralKey) internal returns (bool) {
    ...
    function _burn(address _to, uint256 _synthrAmount, bytes32 _collateralKey) internal returns (bool) {
...
interface ISynthetix {
    ...
    function issueSynthsOnBehalf(address issueForAddress, bytes32 _collateralKey, uint256 _collateralAmount, uint256 _synthToMint) external payable;
...
interface IIssuer {
    ...
    function liquidateAccount(address account, bytes32 collateralKey, bool isSelfLiquidation)
        external
