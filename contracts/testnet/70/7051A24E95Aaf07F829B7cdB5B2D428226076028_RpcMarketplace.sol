...
contract RpcMarketplace is Ownable, ReentrancyGuard {
    ...
    event RpcRemoved(address owner, uint256 tokenId, address tokenAddress);

    modifier onlyRpcOwner(address _tokenAddress, uint256 _tokenId) {
    ...
    modifier isForSale(uint256 _offerId) {
        require(offers[_offerId].isSold == false, "RPC already sold.");
        _;
    }
    ...
    function setOffer(
        ...
        public
        onlyRpcOwner(tokenAddress, tokenId)
    ...
    function removeOffer(uint256 offerId) public {
        ...
        emit RpcRemoved(msg.sender, tokenId, tokenAddress);
    }
    ...
    function buyRpc(uint256 offerId)
        public
        payable
        nonReentrant
        itemExists(offerId)
        isForSale(offerId)
    {
        require(
            ...
            "Payment must be equal to price of the RPC"
        );
        require(
            offers[offerId].seller != msg.sender,
            "Cannot buy your own RPC"
        );
        require(
            activeOffers[offers[offerId].tokenAddress][
                offers[offerId].tokenId
        ...
        emit RpcSold(tokenAddress, msg.sender, price, tokenId, offerId);
    }
