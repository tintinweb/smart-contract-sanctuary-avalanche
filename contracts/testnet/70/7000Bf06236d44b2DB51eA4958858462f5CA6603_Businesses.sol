...
contract Businesses is ERC20, Authorizable {
    using SafeMath for uint256;
    string private TOKEN_NAME = "$BUSINESSES";
    string private TOKEN_SYMBOL = "BUSINESSES";

    address public VMTYCOON_CONTRACT;

    // the base number of $BUSINESSES per vmtycoon (i.e. 0.75 $businesses)
    uint256 public BASE_HOLDER_BUSINESSES = 750000000000000000;

    // the number of $BUSINESSES per vmtycoon per day per level (i.e. 0.25 $businesses /vmtycoon /day /level)
    uint256 public BUSINESSES_PER_DAY_PER_LEVEL = 250000000000000000;

    // how much businesses it costs to skip the cooldown
    uint256 public COOLDOWN_BASE = 100000000000000000000; // base 100
    // how much additional businesses it costs to skip the cooldown per level
    ...
    /**
     * Stores staked vmtycoon fields (=> 152 <= stored in order of size for optimal packing!)
     */
    struct StakedVMTycoonObj {
        // the current level level (0 -> 16,777,216)
        uint24 level;
        // when to calculate businesses from (max 20/02/36812, 11:36:16)
        uint32 sinceTs;
        // for the skipCooldown's cooldown (max 20/02/36812, 11:36:16)
        uint32 lastSkippedTs;
        // how much this vmtycoon has been fed (in whole numbers)
        uint48 dollaAmount;
    ...
    uint16 public totalStakedVMTycoon;

    StakedVMTycoonObj[100001] public stakedVMTycoon;

    // Events

    event Minted(address owner, uint256 businessesAmt);
    event Burned(address owner, uint256 businessesAmt);
    event Staked(uint256 tid, uint256 ts);
    event UnStaked(uint256 tid, uint256 ts);

    // Constructor

    constructor(address _vmtycoonContract) ERC20(TOKEN_NAME, TOKEN_SYMBOL) {
        VMTYCOON_CONTRACT = _vmtycoonContract;
    }

    // "READ" Functions
    // How much is required to be fed to level up per level

    function dollaLevelingRate(uint256 level) public view returns (uint256) {
        // need to divide the level by 100, and make sure the dolla level is at 18 decimals
    ...
    // stake vmtycoon, check if is already staked, get all detail for vmtycoon such as
    function _stake(uint256 tid) internal {
        VM_Tycoon x = VM_Tycoon(VMTYCOON_CONTRACT);

        // verify user is the owner of the vmtycoon...
        require(x.ownerOf(tid) == msg.sender, "NOT OWNER");

        // get calc'd values...
        (, , , , , , , uint256 level) = x.allVMTycoons(tid);
        // if lastSkippedTs is 0 its mean it never have a last skip timestamp
        StakedVMTycoonObj memory c = stakedVMTycoon[tid];
        uint32 ts = uint32(block.timestamp);
        if (stakedVMTycoon[tid].level == 0) {
            // create staked vmtycoon...
            stakedVMTycoon[tid] = StakedVMTycoonObj(
                uint24(level),
                ts,
            ...
            // N.B. could be optimised for multi-stakes - but only saves 0.5c AUD per vmtycoon - not worth it, this is a one time operation.
            totalStakedVMTycoon += 1;
    ...
    /**
     * Calculates the amount of businesses that is claimable from a vmtycoon.
     */
    function claimableView(uint256 tokenId) public view returns (uint256) {
        StakedVMTycoonObj memory c = stakedVMTycoon[tokenId];
        if (c.level > 0) {
            uint256 businessesPerDay = ((BUSINESSES_PER_DAY_PER_LEVEL * (c.level / 100)) +
                BASE_HOLDER_BUSINESSES);
            uint256 deltaSeconds = block.timestamp - c.sinceTs;
            return deltaSeconds * (businessesPerDay / 86400);
        } else {
            return 0;
        }
    }

    function myStakedVMTycoon() public view returns (uint256[] memory) {
        VM_Tycoon x = VM_Tycoon(VMTYCOON_CONTRACT);
        uint256 vmtycoonCount = x.balanceOf(msg.sender);
        uint256[] memory tokenIds = new uint256[](vmtycoonCount);
        uint256 counter = 0;
        for (uint256 i = 0; i < vmtycoonCount; i++) {
            uint256 tokenId = x.tokenOfOwnerByIndex(msg.sender, i);
            StakedVMTycoonObj memory vmtycoon = stakedVMTycoon[tokenId];
            if (vmtycoon.level > 0) {
    ...
    /**
     * Calculates the TOTAL amount of businesses that is claimable from ALL vmtycoons.
     */
    function myClaimableView() public view returns (uint256) {
        VM_Tycoon x = VM_Tycoon(VMTYCOON_CONTRACT);
        uint256 cnt = x.balanceOf(msg.sender);
        require(cnt > 0, "NO VMTYCOON");
    ...
    /**
     * Claims businesses from the provided vmtycoons.
     */
    function _claimBusinesses(uint256[] calldata tokenIds) internal {
        VM_Tycoon x = VM_Tycoon(VMTYCOON_CONTRACT);
        uint256 totalClaimableBusinesses = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(x.ownerOf(tokenIds[i]) == msg.sender, "NOT OWNER");
            StakedVMTycoonObj memory vmtycoon = stakedVMTycoon[tokenIds[i]];
            // we only care about vmtycoon that have been staked (i.e. level > 0) ...
            if (vmtycoon.level > 0) {
                uint256 claimableBusinesses = claimableView(tokenIds[i]);
                if (claimableBusinesses > 0) {
                    totalClaimableBusinesses = totalClaimableBusinesses + claimableBusinesses;
                    // reset since, for the next calc...
                    vmtycoon.sinceTs = uint32(block.timestamp);
                    stakedVMTycoon[tokenIds[i]] = vmtycoon;
                }
            }
        }
        if (totalClaimableBusinesses > 0) {
            _mint(msg.sender, totalClaimableBusinesses);
            emit Minted(msg.sender, totalClaimableBusinesses);
        }
    }
     ...
     * Claims businesses from the provided vmtycoons.
     */
    function claimBusinesses(uint256[] calldata tokenIds) external {
        _claimBusinesses(tokenIds);
    }

    /**
     * Unstakes a vmtycoon. Why you'd call this, I have no idea.
     */
    ...
    /**
     * Unstakes MULTIPLE vmtycoon. Why you'd call this, I have no idea.
     */
    ...
    /**
     * Unstakes MULTIPLE vmtycoon AND claims the businesses.
     */
    function withdrawAllVMTycoonAndClaim(uint256[] calldata tids) external {
        _claimBusinesses(tids);
        _unstakeMultiple(tids);
    }

    /**
     * Public : update the vmtycoon's level level.
     */
     function levelUpVMTycoon(uint256 tid) external {
        StakedVMTycoonObj memory c = stakedVMTycoon[tid];
        require(c.level > 0, "NOT STAKED");

        VM_Tycoon x = VM_Tycoon(VMTYCOON_CONTRACT);
        // NOTE Does it matter if sender is not owner?
        // require(x.ownerOf(vmtycoonId) == msg.sender, "NOT OWNER");

        // check: vmtycoon has eaten enough...
        require(c.dollaAmount >= dollaLevelingRate(c.level), "MORE FOOD REQD");
        // check: cooldown has passed...
        require(block.timestamp >= c.cooldownTs, "COOLDOWN NOT MET");

        // increase level, reset eaten to 0, update next dolla level and cooldown time
        c.level = c.level + 100;
        c.dollaAmount = 0;
        c.cooldownTs = uint32(block.timestamp + cooldownRate(c.level));
        stakedVMTycoon[tid] = c;

        // need to increase overall size
        totalLevel += uint24(100);

        // and update the vmtycoon contract
        x.setLevel(tid, c.level);
    }

    /**
     * Internal: burns the given amount of businesses from the wallet.
     */
    function _burnBusinesses(address sender, uint256 businessesAmount) internal {
        // NOTE do we need to check this before burn?
        require(balanceOf(sender) >= businessesAmount, "NOT ENOUGH BUSINESSES");
        _burn(sender, businessesAmount);
        emit Burned(sender, businessesAmount);
    }

    /**
     * Burns the given amount of businesses from the sender's wallet.
     */
    function burnBusinesses(address sender, uint256 businessesAmount) external onlyAuthorized {
        _burnBusinesses(sender, businessesAmount);
    }

    /**
     * Skips the "levelUp" cooling down period, in return for burning businesses.
     */
     function skipCoolingOff(uint256 tokenId, uint256 businessesAmount) external {
        StakedVMTycoonObj memory vmtycoon = stakedVMTycoon[tokenId];
        require(vmtycoon.level != 0, "NOT STAKED");

        uint32 ts = uint32(block.timestamp);

        // NOTE Does it matter if sender is not owner?
        // VMTycoon instance = VMTycoon(VMTYCOON_CONTRACT);
        // require(instance.ownerOf(vmtycoonId) == msg.sender, "NOT OWNER");

        // check: enough businesses in wallet to pay
        uint256 walletBalance = balanceOf(msg.sender);
        require(walletBalance >= businessesAmount, "NOT ENOUGH businesses IN WALLET");

        // check: provided businesses amount is enough to skip this level
        require(businessesAmount >= checkSkipCoolingOffAmt(vmtycoon.level), "NOT ENOUGH businesses TO SKIP");

        // check: user hasn't skipped cooldown in last 24 hrs
        require((vmtycoon.lastSkippedTs + COOLDOWN_CD_IN_SECS) <= ts, "BLOCKED BY 24HR COOLDOWN");

        // burn businesses
        _burnBusinesses(msg.sender, businessesAmount);

        // disable cooldown
        vmtycoon.cooldownTs = ts;
        // track last time cooldown was skipped (i.e. now)
        vmtycoon.lastSkippedTs = ts;
        stakedVMTycoon[tokenId] = vmtycoon;
    }
    ...
    /**
     *  Giving credis to vmtycoon
     */
    function equipVMTycoon(uint256 tokenId, uint256 dollaAmount)
        external
        onlyAuthorized
    {
        StakedVMTycoonObj memory vmtycoon = stakedVMTycoon[tokenId];
        require(vmtycoon.level > 0, "NOT STAKED");
        require(dollaAmount > 0, "NOTHING TO EQUIP");
        // update the block time as well as claimable
        vmtycoon.dollaAmount = uint48(dollaAmount / 1e18) + vmtycoon.dollaAmount;
        stakedVMTycoon[tokenId] = vmtycoon;
    }
    ...
    function updateSkipCooldownValues(
        ...
        BASE_HOLDER_BUSINESSES = d;
        BUSINESSES_PER_DAY_PER_LEVEL = e;
    }

    // INTRA-CONTRACT: use this function to mint businesses to users
    // this also get called by the DOLLA contract
    function mintBusinesses(address sender, uint256 amount) external onlyAuthorized {
        _mint(sender, amount);
        emit Minted(sender, amount);
    }

    // ADMIN: drop businesses to the given vmtycoon wallet owners (within the vmtycoonId range from->to).
    function airdropToExistingHolder(
        uint256 from,
        uint256 to,
        uint256 amountOfBusinesses
    ) external onlyOwner {
        // mint 100 businesses to every owners
        VM_Tycoon instance = VM_Tycoon(VMTYCOON_CONTRACT);
        for (uint256 i = from; i <= to; i++) {
            address currentOwner = instance.ownerOf(i);
            if (currentOwner != address(0)) {
                _mint(currentOwner, amountOfBusinesses * 1e18);
            }
        }
    }

    // ADMIN: Rebalance user wallet by minting businesses (within the vmtycoonId range from->to).
    // NOTE: This is use when we need to update businesses production
    function rebalanceBusinessesClaimableToUserWallet(uint256 from, uint256 to)
        external
        onlyOwner
    {
        VM_Tycoon instance = VM_Tycoon(VMTYCOON_CONTRACT);
        for (uint256 i = from; i <= to; i++) {
            address currentOwner = instance.ownerOf(i);
            StakedVMTycoonObj memory vmtycoon = stakedVMTycoon[i];
            // we only care about vmtycoon that have been staked (i.e. level > 0) ...
            if (vmtycoon.level > 0) {
                _mint(currentOwner, claimableView(i));
                vmtycoon.sinceTs = uint32(block.timestamp);
                stakedVMTycoon[i] = vmtycoon;
            }
        }
    }
}