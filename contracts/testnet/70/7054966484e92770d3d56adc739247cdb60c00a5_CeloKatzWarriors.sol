...
contract CeloKatzWarriors is ERC721Enumerable, Ownable {
    ...
    event welcomeToCeloKatzWarrior(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("CeloKatzWarrior", "CKW"){
        setBaseURI(baseURI);
    }
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToCeloKatzWarrior(_tokenId);
    }
    ...
    function withdrawFor(address[] memory _addrs, uint[] memory _values) public onlyOwner {
