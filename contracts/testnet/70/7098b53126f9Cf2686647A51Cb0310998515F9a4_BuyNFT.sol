...
contract BuyNFT is Ownable {
    event SetFiat(string[] symbols, address[] _address);
    event SetPrice(address _game, uint256[] _tokenIds, uint256 _Price, uint8 _type);
    ...
    address public ceoAddress = address(0x1C8Bc08469794bd62b6d9C3507a4c8f07bFC93B0);
    address[] public fiat = [address(0), address(0x1c73743870BEE9459c2AE95D97C5DF78ffBED29F)];
    ...
    constructor() {
        ...
        tokensFiat[address(0x2F7265b97F6655F78AbBf13168e1FB4749A03bd0)] = Token("ET", true);
        fiatContract = Fiat(0x87596f78A0Fa11814B907C51e147aDA39905F099);
        buyNFTSub = address(0xC1325a01202f2BEaA345a7D9bcf719A6402D0387);
        ceoAddress = _msgSender();
    }

    modifier onlyCeoAddress() {
        require(_msgSender() == ceoAddress, "BuyNFT: caller is not CEO");
        _;
    }

    modifier onlySub() {
        require(_msgSender() == buyNFTSub, "BuyNFT: caller is not BuyNFTSub");
        _;
    }

    modifier isValidFiatBuy(address _fiat) {
        require(tokensFiat[_fiat].existed, "BuyNFT: Invalid buy token");
        _;
    }

    modifier isOwnerOf(address _game, uint256[] memory _tokenIds) {
        require(_isOwnerOf(_game, _tokenIds), "BuyNFT: caller is not the owner of NFTs");
        _;
    }

    modifier isValidFiat(address[] memory _fiat) {
        require(_isValidFiat(_fiat), "BuyNFT: Invalid fiat");
        _;
    }
    ...
    function _isValidFiat(address[] memory _fiat) private view returns (bool) {
    ...
    function calFee(address _game, string memory _fee, uint256 _price) public view returns (uint256) {
        uint256 amount = _price.mul(games[_game].fees[_fee].percent).div(PERCENT);
        return amount;
    }

    function calPrice(address _game, uint256 _orderId) public view returns (address, uint256,
            address[] memory,
            address,
            bool
        )
    ...
    function _setPrice(
        ...
        require(games[_game].tokenPrice[_orderId].maker == address(0) || games[_game].tokenPrice[_orderId].maker == msg.sender, "BuyNFT: Only owner can sell NFT");
    ...
    function getGameFees(address _game) public view returns (string[] memory, address[] memory, uint256[] memory, uint256) {
    ...
    function setLimitFee(
        ...
        require(_fee >= 0 && _limitFee >= 0, "BuyNFT: Invalid fee");
    ...
    function sellNfts(uint256[] memory _orderIds, address[] memory _game, uint256[][] memory _tokenIds, uint256[] memory _price, address[][] memory _fiats) public {
        require(_orderIds.length == _tokenIds.length, "Orders and tokenIds length miss match");
        for (uint256 i = 0; i < _orderIds.length; i++) {
            require(_isOwnerOf(_game[i], _tokenIds[i]), "BuyNFT: caller is not the owner of NFTs");
            require(_isValidFiat(_fiats[i]), "BuyNFT: Invalid fiat");
            setPriceFee(_orderIds[i], _game[i], _tokenIds[i], _price[i], _fiats[i]);
        }
    }
    ...
    function removePrice(address _game, uint256 _orderId) public {
        require(_msgSender() == games[_game].tokenPrice[_orderId].maker, "BuyNFT: Only owner can remove price");
    ...
    function changeCeo(address _address) public onlyCeoAddress {
        require(_address != address(0), "BuyNFT: Can not be address 0");
        ceoAddress = _address;
    }
    ...
    function setFiat(string[] memory _symbols, address[] memory _addresses) public onlyOwner {
        require(_symbols.length == _addresses.length, "BuyNFT: Fiat length mismatch");
