...
interface ICygnusCollateralControl is ICygnusTerminal {
    ...
    /**
     *  @param hangar18 Address of the cygnus factory
     *  @param underlying Address of underlying collateral token (LP Token address from Joe, Pangolin, etc.)
     *  @param borrowDAITokenA Address of the first cygnus borrow token
     *  @param borrowDAITokenB Address of the second cygnus borrow token
     *  @custom:event Emitted when factory calls initialize function
     */
    event InitializeCollateral(address hangar18, address underlying, address borrowDAITokenA, address borrowDAITokenB);
...
interface ICygnusDeneb {
    ...
    /**
     *  @notice Passing the struct parameters to the collateral contract
     *  @notice Collateral deployments have no constructor, so we can predict addresses for the Factory deployments
     *  @dev This constructor gets removed with `delete`, so values should always be default 0
     */
...
interface ICygnusBorrowControl is ICygnusTerminal {
     ...
     *  @dev We set a minimum here just in case
     */
    ...
    /**
     *  @notice 👽
     *  @param newBaseRate The new base rate for the pool
     */
    function setBaseRate(uint256 newBaseRate) external;

    /**
     *  @notice 👽
     *  @param newFarmMultiplier The new multiplier for the interest rate
     */
...
interface ICygnusAlbireo {
    ...
    /**
     *  @notice Passing the struct parameters to the borrow contracts
     *  @notice Borrow deployments have no constructor so it is easier to predict addresses for the Factory deployments
     *  @dev This constructor gets removed with `delete` refunding some gas, so values should always be default 0
     */
     ...
     *  @param underlying Address of the underlying token representing the borrow token (ie address of DAI)
     *  @param collateral Address of the Cygnus Collateral token
     *  @param albireoIndex Index representing the 2 underlyings of the LP Token (0 or 1)
...
interface ICygnusFactory {
     ...
     */
    error CygnusFactory__BorrowAAlreadyExists(address);

    /**
     *  @custom:error Emitted when borrow token B already exists for this shuttle
     */
    error CygnusFactory__BorrowABlreadyExists(address);
     ...
     *  @custom:event Emitted when setting a new Cygnus Admin
     */
     ...
     *  @custom:event Emitted when setting a new contract which handles cashflow
     */
    event NewVegaTokenManager(address oldVegaContract, address newVegaContract);
     ...
     */
    function pendingNewVegaTokenManager() external view returns (address);
    ...
    /**
     *  @notice Sets a pending admin for Cygnus to approve
     *  @param newCygnusAdmin Address of the requested Cygnus admin
     */
    function setPendingAdmin(address newCygnusAdmin) external;

    /**
     *  @notice Approves the pending admin as admin. Has special privileges to set params in collateral/borrow contracts
     */
    function setNewCygnusAdmin() external;

    /**
     *  @notice Sets the new contract which handles the Cygnus cashflow
     *  @param _newVegaTokenManager The address of the requested contract to be the new Vega Token Manager
     */
    function setNewVegaTokenManager(address _newVegaTokenManager) external;
}
...
/* .        .               .          .         . 🛸                .                            . 
    ...
    ░░███     ███ ░███ ░███ ░███ ░███ ░███ ░███  ░███ ░███  ░░░░███              .             *⠀
     ...
     Contract                        ⠀Interface                                             
        ...
        │     ├ Public                  │     ├ Public                            ⠀       
        │     ├ Private                 │     └ External                        ⠀⠀⠀              
        │     └ Internal               ⠀   └ 4. Non-Constant Functions  
...
contract CygnusTerminal is ICygnusTerminal, Erc20Permit {
    ...
    /**
     *  @custom:library PRBMathUD60x18 For Fixed point 18 decimal math library, imports main library `PRBMath`
     */
    ...
    constructor(
        ...
        (hangar18, underlying, , ) = ICygnusAlbireo(_msgSender()).borrowParameters();
    }
    ...
    /**
     *  @notice 👽
     &  @notice For CygnusCollateralControl and CygnusBorrowControl contracts
     */
    ...
    /**
     *  @notice Updates the total balance of this in terms of its underlying (LP Token or borrow token)
     *  @dev Emits {Sync} event
     */
...
contract CygnusCollateralControl is ICygnusCollateralControl, CygnusTerminal {
     ...
     */
    string private constant COLLATERAL_SYMBOL = "DenebCYG";
    ...
    constructor()
        /**
         *  @param COLLATERAL_NAME Name of the Erc20 Cygnus Collateral token
         *  @param COLLATERAL_SYMBOL Symbol of Erc20 Cygnus Collateral token
         *  @param DECIMALS Decimals of the Erc20 Cygnus Collateral token (always 18)
         */
        CygnusTerminal(COLLATERAL_NAME, COLLATERAL_SYMBOL, DECIMALS)
    {
        // Get pool parameters from deployer
        (, , borrowDAITokenA, borrowDAITokenB) = ICygnusDeneb(_msgSender()).collateralParameters();
    }
     ...
     *  @inheritdoc ICygnusCollateralControl
     */
    function setPriceOracle(ICygnusNebulaOracle newCygnusNebulaOracle) external override cygnusAdmin nonReentrant {
        newCygnusNebulaOracle = ICygnusFactory(hangar18).cygnusNebulaOracle();
...
contract CygnusCollateralModel is ICygnusCollateralModel, CygnusCollateralControl {
    ...
    function accountLiquidityInternal(
        ...
        // 2. (amountTokenA * priceTokenA) / scale
...
/*  ═══════════════════════════════════════════════════════════════════════════════════════════════════════
        ...
        https://cygnusdao.finance - COLLATERAL DEPLOYER                                                           
        ═══════════════════════════════════════════════════════════════════════════════════════════════════════  */
...
/**
 *  @title CygnusDeneb
 *  @notice This is the Collateral Deployer contract which starts the collateral arm of the lending pool.
 *  @dev Contract bytecode wouldn't fit in Factory.
 */
contract CygnusDeneb is ICygnusDeneb, Context {
    /*  ─────────────────────────────────────────────── Public ────────────────────────────────────────────────  */

    /**
     *  @notice These parameters get passed to all Cygnus Collateral contracts and then deleted
     *  @custom:struct factory The address of the Cygnus factory
     *  @custom:struct lpTokenPair The address of the underlying LP Token
     *  @custom:struct borrowDAITokenA The address of the first Cygnus borrow token
     *  @custom:struct borrowDAITokenB The address of the second Cygnus borrow token
     */
    ...
    /**
     *  @dev Must initialize struct as public for Collateral constructor to pick up parameters
    ...
    function deployDeneb(
        ...
        address borrowDAITokenA,
        address borrowDAITokenB
    ) external override isHangar18 returns (address deneb) {
        collateralParameters = CollateralParameters({
            factory: _msgSender(),
            lpTokenPair: lpTokenPair,
            borrowDAITokenA: borrowDAITokenA,
            borrowDAITokenB: borrowDAITokenB
        });

        // Create collateral contract
        deneb = address(new CygnusCollateral{ salt: keccak256(abi.encode(_msgSender(), lpTokenPair)) }());
