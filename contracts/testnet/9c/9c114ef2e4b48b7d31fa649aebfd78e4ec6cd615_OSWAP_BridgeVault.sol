...
// File contracts2/interfaces/IOSWAP_HybridRouter2.sol
...
// File contracts2/OSWAP_BridgeVault.sol
...
contract OSWAP_BridgeVault is ERC20, ReentrancyGuard {
    ...
    event EditOrderRequest(uint256 indexed orderId, Order order, int256 newImbalance);
    ...
    enum OrderStatus{Pending, Executed, RequestCancel, ApprovedCancel, Cancelled, RequestEdit}
    ...
    // cancel should be call on target chain
    // function requestCancelOrder(uint256 orderId) external nonReentrant whenNotPaused {
    //     // Order memory order = orders[orderId];
    //     require(orderOwner[orderId] == msg.sender, "BridgeVault: not from owner");
    //     require(orderStatus[orderId] == OrderStatus.Pending, "BridgeVault: not a pending order");
    //     orderStatus[orderId] = OrderStatus.RequestCancel; //request cancel
    //     // imbalance -= toInt256(order.inAmount);
    //     emit RequestCancelOrder(msg.sender, orderId, imbalance);
    // }
    function withdrawUnexecutedOrder(uint256 orderId) external nonReentrant whenNotPaused {
        Order memory order = orders[orderId];
        require(orderOwner[orderId] == msg.sender,  "BridgeVault: not from owner");
        require(orderStatus[orderId] == OrderStatus.ApprovedCancel, "BridgeVault: cancellation not approved");
        orderStatus[orderId] = OrderStatus.Cancelled;
        imbalance -= toInt256(order.inAmount);
        asset.safeTransfer(msg.sender, order.inAmount);
        emit WithdrawUnexecutedOrder(msg.sender, orderId, imbalance);
    }
    function requestEditOrder(uint256 orderId, Order calldata order) external nonReentrant whenNotPaused {
        require(orderOwner[orderId] == msg.sender, "BridgeVault: not from owner");
        require(orderStatus[orderId] == OrderStatus.Pending, "BridgeVault: not a pending order");
        require(order.inAmount >= orders[orderId].inAmount, "Invalid in amount");

        uint256 excess = order.inAmount - orders[orderId].inAmount;
        if (excess > 0) {
            excess = _transferAssetFrom(msg.sender, excess);

            imbalance += toInt256(excess);
        }
        orderAmendments[orderId].push(order);
        orderStatus[orderId] = OrderStatus.RequestEdit;
        emit EditOrderRequest(orderId, order, imbalance);
    }
    ...
    function requestCancelOrder(uint256 sourceChainId, uint256 orderId) external whenNotPaused {
        ...
        require(swapOrderStatus[_orderId] == OrderStatus.Pending, "BridgeVault: not a pending order");
    ...
    function cancelOrder(bytes[] calldata signatures, uint256 orderId, bool canceledByOrderOwner, uint256 protocolFee) external whenNotPaused {
        ...
        require(orderStatus[orderId] == OrderStatus./*RequestCancel*/Pending, "BridgeVault: cancel not requested");
        ...
        orderStatus[orderId] = OrderStatus.ApprovedCancel;
    ...
    function swap(
        ...
        require(swapOrderStatus[orderId] == OrderStatus.Pending,"BridgeVault: Order already processed");
        ...
        if (pair.length == 0) {
            ...
            IERC20(order.outToken).safeTransfer(order.to, order.minOutAmount);
        } else {
    ...
    function rebalancerDeposit(uint256 assetAmount) external nonReentrant {
        require(msg.sender == configStore.rebalancer(), "not from rebalancer");
    ...
    function rebalancerWithdraw(bytes[] calldata signatures, uint256 assetAmount, uint256 _nonce) external nonReentrant {
        require(msg.sender == configStore.rebalancer(), "not from rebalancer");
        vaultRegistry.verifyMinimumSignaturesAndNonce(msg.sender, signatures, hashWithdrawParams(msg.sender, assetAmount, _nonce), _nonce);
        imbalance -= toInt256(assetAmount);
        asset.safeTransfer(msg.sender, assetAmount);
        emit Rebalance(msg.sender, -toInt256(assetAmount), imbalance);
    }
