...
abstract contract Withdrawable is AdminableUpgradeable {
    ...
    address withdrawAddress = 0xf0675997E24e32EE69db72AC9e43Cf701A168FB3;
    ...
    function withdrawAll() external onlyOwnerOrAdmin {
        ...
        if (fundByTokens && fundToken.balanceOf(address(this)) > 0) {
            fundToken.transfer(withdrawAddress, fundToken.balanceOf(address(this)));
        }
    }
    ...
    function withdrawBalance(uint256 amount) external onlyOwnerOrAdmin {
        ...
        if (fundByTokens) {
            fundToken.transfer(withdrawAddress, amount);
        } else {
    ...
    {
        require(amount > 0, "Withdrawable: amount should be greater than zero");
        IERC20(token).transfer(withdrawAddress, amount);
    }

    /**
    * Set the address to which the funds will be withdrawn
     */
    function setWithdrawAddress(address _withdrawAddress) external onlyOwnerOrAdmin {
        withdrawAddress = _withdrawAddress;
    }

    /**
     * Get the address to which the funds will be withdrawn
     */
