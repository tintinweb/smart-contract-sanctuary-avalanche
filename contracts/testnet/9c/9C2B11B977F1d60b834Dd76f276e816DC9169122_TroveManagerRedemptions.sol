...
contract TroveManagerRedemptions is TroveManagerBase, ITroveManagerRedemptions {
    ...
    uint256 public constant BOOTSTRAP_PERIOD = 14 minutes;

    // constant for how much the borrower gets. Flat fee of 20% of the base fee.
    // 1e15 = 0.5% * 20% * 1e18
    // 1e15 / 1e18 = 1e3 which is what we can divide by directly to get the percentage.
    uint256 public constant BORROWER_FEE_SPLIT = 1e3;
    ...
    function redeemCollateral(
        ...
        uint256 borrowerSplit = totals.totalYUSDToRedeem.div(BORROWER_FEE_SPLIT);
        ...
        _requireYUSDBalanceCoversRedemption(
            ...
            _redeemer,
            totals.totalYUSDToRedeem.add(totals.YUSDfee).sub(borrowerSplit)
        );
    ...
    function redeemCollateralSingle(
        ...
        // Confirm redeemer's balance is less than total YUSD supply
        require(contractsCache.yusdToken.balanceOf(_redeemer) <= totals.totalYUSDSupplyAtStart, "TMR:Redeemer YUSD Bal too high");

        totals.remainingYUSD = _YUSDamount;
        require(_isValidFirstRedemptionHint(contractsCache.sortedTroves, _target), "TMR:Invalid first redemption hint");
        ...
        (singleRedemption.CollLot.tokens, amounts, singleRedemption.troveDebt) = troveManager.getCurrentTroveState(_target);
        ...
        {
            uint256 singleCollUSD = contractsCache.controller.getValueUSD(_collToRedeem, amounts[i]); //Get usd value of only the collateral being redeemed
            ...
            uint tokenAmountToRedeem = singleRedemption.YUSDLot.mul(amounts[i]).div(singleCollUSD);
        ...
        if (singleRedemption.troveDebt == YUSD_GAS_COMPENSATION) {
            ...
            uint256 newRICR = _getRICRColls(newColls(singleRedemption.CollLot.tokens, amounts), singleRedemption.troveDebt);
        ...
        _requireYUSDBalanceCoversRedemption(
            contractsCache.yusdToken,
            _redeemer,
            totals.remainingYUSD.add(totals.YUSDfee).sub(borrowerSplit)
        );
    ...
    function _redeemCollateralFromTrove(
        ...
        uint256[] memory amounts;
        (singleRedemption.CollLot.tokens, amounts, singleRedemption.troveDebt) = troveManager.getCurrentTroveState(_borrower);
        ...
        { 
            ...
            for (uint256 i; i < collsLen; ++i) {
                uint tokenAmountToRedeem = baseLot.mul(amounts[i]).div(totalCollUSD).div(1e18);

                // uint tokenAmountToRedeem = baseLotDivUSD.mul(amounts[i]).div(DECIMAL_PRECISION);
                ...
                if (contractsCache.controller.isWrapped(singleRedemption.CollLot.tokens[i])) {
                    IWAsset(singleRedemption.CollLot.tokens[i]).updateReward(_borrower, address(this), tokenAmountToRedeem);
                }
            }
        }
        ...
        if (newDebt == YUSD_GAS_COMPENSATION) {
            ...
            uint256 newRICR = _computeCR(_getRVC(singleRedemption.CollLot.tokens, finalAmounts), newDebt);
     ...
     * @param _borrowerSplit : The amount which has already been transferred to the borrower
     */
    function _transferAndSplitFee(ContractsCache memory contractsCache, address _redeemer, uint256 _YUSDFee, uint256 _borrowerSplit) internal {
        ...
        // And the rest to fee recipient (sYETI)
        // The borrower split has already been sent to the borrowers, and is a flat 0.5% * 20% of all yusd drawn.
        contractsCache.yusdToken.safeTransferFrom(
            _redeemer,
            YUSDFeeRecipient,
            (_YUSDFee - treasurySplit) - _borrowerSplit
        );
    }
    ...
    function _sendBorrowerFeeSplit(
        ...
        uint256 toSendToBorrower = (_redeemedAmount).div(BORROWER_FEE_SPLIT);
        contractsCache.yusdToken.safeTransferFrom(
            _redeemer,
            address(contractsCache.collSurplusPool),
            toSendToBorrower
