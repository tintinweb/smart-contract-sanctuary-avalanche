...
contract ParamountStaker {
    ...
    uint256 public constant INSURANCE_FEE = 200; // 2% insurance fee
    uint256 public constant REINVEST_FEE = 1000; // 2% insurance fee
    ...
    uint256 public TIME_STEP = 10 minutes;
    ...
    function deposit(address userAddress, address referrer, uint8 plan,uint256 amount) internal {
        require(plan < 6, "Invalid plan");
        User storage user = users[userAddress];

        uint256 fee = amount.mul(PLATFORM_FEE).div(PERCENTS_DIVIDER);
        PLATFORM_WALLET.transfer(fee);
        emit FeePayed(userAddress, fee);

        if (user.referrer == address(0)) {
            if (
                (users[referrer].deposits.length == 0 || referrer == userAddress)
            ) {
    ...
    function withdraw(uint8 plan) public {
        ...
        uint256 rFees = totalAmount.mul(REINVEST_FEE).div(PERCENTS_DIVIDER);
    ...
    function getUserDebt(address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function isDepositActive(address userAddress, uint256 index) public view returns(bool) {
		User storage user = users[userAddress];
