...
import "./ERC20.sol";
import "./interfaces/IDexanceNFTMarketplace.sol";
import "./utils/Address.sol";

contract DexanceNFTLaunchpad is Ownable, ReentrancyGuard {
    ...
    //DexanceNFTMarketplace
    address public dexanceNftMarketplace;
    ...
    constructor(address _dexanceNftMarketplace) {
        require(_dexanceNftMarketplace != address(0),"Zero address");
        require(_dexanceNftMarketplace.isContract(),"Not contract");
        dexanceNftMarketplace = _dexanceNftMarketplace;
    }
    ...
    function addLaunchPad(
        ...
        require(IDexanceNFTMarketplace(dexanceNftMarketplace).isOwnerOfRoot(_rootId,msg.sender),"Bad root token");
    ...
    function setWinners(uint24 launchpadId, address[] memory _winners) public onlyOwner() launchpadNotFound(launchpadId) {
        ...
        require(_winners.length <= IDexanceNFTMarketplace(dexanceNftMarketplace).getMaxFragment(),"Too many fragment");
    ...
    function claim(uint24 launchpadId) external nonReentrant onlyEOA{
        ...
        IDexanceNFTMarketplace(dexanceNftMarketplace).mintFragment(_msgSender(), launchpad.rootIdToken);
    ...
    function setAddressMarketplace(address _dxnMarketplace) public onlyOwner(){
        require(_dxnMarketplace!= address(0),"Zero address");
        require(_dxnMarketplace.isContract(),"Not contract");
        dexanceNftMarketplace = _dxnMarketplace;
    }
    ...
    function deleteLaunchpad(uint24 launchpadId) external onlyOwner() launchpadNotFound(launchpadId){
        ...
        IDexanceNFTMarketplace(dexanceNftMarketplace).burnByLaunchpad(owner(),launchpad.rootIdToken);
        //set disable
        launchpadDisabled[launchpadId] = true;
        // set Emegency
        if(launchpad.startTime < block.timestamp && block.timestamp < launchpad.endTime){
...
interface IDexanceNFTMarketplace {
    // Event =================================================================================
