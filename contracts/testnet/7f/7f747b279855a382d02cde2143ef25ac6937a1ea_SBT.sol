...
interface ISBT {
     ...
     * @return the user type (1 = Auditor, 2 = Platform)
     */
    ...
    /**
     * @dev used to get the name of the token
     * @return the name of the token
     */
    function name() external view returns (string memory);

    /**
     * @dev used to get the symbol of the token
     * @return the symbol of the token
     */
    function symbol() external view returns (string memory);

    /**
     * @dev used to get the owner of a token
     * @param id the token id
     ...
     * 2: Platform
     */
    function mint(address _to, string calldata _uri, uint256 _type) external;
    ...
    /**
     * @dev used to get the implementation address of the proxy
...
contract SBT is Initializable, PausableUpgradeable, UUPSUpgradeable, ISBT {
    /* ========== STORAGE ========== */
    ...
    modifier validateType(uint256 _type) {
        if (_type != 1 && _type != 2) revert InvalidUserType();
        _;
    }
    ...
    modifier onlyRevokeAuthorized(uint256 id) {
        if (contractOwner != msg.sender && ownerOf[id] != msg.sender)
    ...
    modifier onlyOwner() {
        if (contractOwner != msg.sender) revert NotOwner();
        _;
    }
    ...
    function mint(
        ...
        uint256 _type
    ) external validateType(_type) whenNotPaused onlyOwner {
        if (userType[_to] != 0) revert AlreadyMinted();
    ...
    function burn(
        uint256 _id
    ) external whenNotPaused onlyRevokeAuthorized(_id) {
        //solhint-disable-next-line var-name-mixedcase
        address SBTOwner = ownerOf[_id];
