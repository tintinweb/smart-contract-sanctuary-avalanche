...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    function open(
        ...
        require(_opt._optionsQty >= 2 && _opt._optionsQty < 256, "invalid _optionsQty");
    ...
    function claimCreationRewards(uint256 _pairID) external {
        ...
        uint64 publicityEndAt = pairExtra[_pairID].publicityEndAt;
        require(publicityEndAt > 0 && block.timestamp > publicityEndAt , "invalid time");
        ...
        require(innerPair.openAmount > 0, "invalid openAmount");
        IERC20MetadataUpgradeable(betUpDaoToken).safeTransfer(receiver, uint256(innerPair.openAmount) * 10 ** 18);
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint64 publicityEndAt = pairExtra[pairID].publicityEndAt;
            require(publicityEndAt > 0 && block.timestamp > publicityEndAt, "invalid time");
    ...
    */
    function _getReward(
        ...
        if(innerPair.resultID == INVALID_ID) {
            ...
            if(resultOpAmount == 0 || resultOpAmount == innerPair.total) {
                refund = _player.amount;
            }else {
                uint64 publicityEndAt = pairExtra[_pairID].publicityEndAt;
                if(publicityEndAt > 0 && block.timestamp > publicityEndAt) {
                    if(innerPair.resultID == _resultID) {

                        refund = _player.amount +
                            (
                                ( 9 * _player.amountWithBonus * (innerPair.total - playerOp.amount)) /
                                (10 * playerOp.amountWithBonus)
                            );
