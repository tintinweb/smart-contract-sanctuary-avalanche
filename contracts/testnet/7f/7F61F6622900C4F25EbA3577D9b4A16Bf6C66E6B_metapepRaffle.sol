...
contract metapepRaffle is VRFConsumerBaseV2 {
    ...
    event TransferFailed(address winner, uint256 tokenId);
    event EndTimeChanged(uint256 newEndTime);

    // Define Chainlink VRF variables and randomness status
    uint256 public randomResult;
    bool public randomResultReady = false;
    ...
    constructor(
        ...
        endTime = _endTime; // Initialize endTime from constructor parameter
    ...
    function buyTicket(uint256 numberOfTickets) public payable {
        ...
        require(
            ...
            "Ticket purchasing period has ended."
        );
    ...
    function endLottery() public onlyAdmin {
        ...
        lotteryStatus = false;
        emit LotteryEnded();
        requestRandomWords(); // Call requestRandomWords internally
    }
    ...
    function pickWinner() public onlyAdmin {
        ...
        require(randomResultReady, "Random number is not ready");

        uint256 index = randomResult % playerSelector.length;
        address winner = playerSelector[index];

        // Transfer the NFT to the winner
        try ERC721Base(nftContract).transferFrom(admin, winner, tokenId) {
            // Transfer succeeded, reset state variables
            ...
            resetEntryCounts();
            emit Winner(winner);
            randomResultReady = false;
        } catch {
            // Transfer failed, emit an event or revert the transaction
            emit TransferFailed(winner, tokenId);
        }
        ...
        // Reset the entryCounts mapping based on playerSelector
    ...
    function changeEndTime(uint256 newEndTime) public onlyAdmin {
        require(
    ...
    function getBalance() public view returns (uint256) {
        return lotteryToken.balanceOf(address(this)); // Return the contract's token balance
    }

    function withdrawBalance() public onlyAdmin {
        uint256 amount = lotteryToken.balanceOf(address(this));
        require(amount > 0, "No balance to withdraw.");
        require(lotteryToken.transfer(admin, amount), "Transfer failed"); // Transfer tokens to admin
        emit BalanceWithdrawn(amount);
    }

    function changeLotteryToken(IERC20 _newToken) public onlyAdmin {
    ...
    function resetContract() public onlyAdmin {
        require(
            lotteryToken.balanceOf(address(this)) == 0,
            "Contract balance must be zero before reset"
        );
        ...
        resetEntryCounts(); // Reset the entryCounts mapping
    }
}