...
contract JonesGlpLeverageStrategy is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, Governable {
    ...
    function onGlpRedeem(uint256 _glpAmount) external nonReentrant onlyRole(OPERATOR_ROLE) returns (uint256) {
        ...
        uint256 yrtToRedeem = yrt.getSharesForDepositTokens(assetsToRedeem);
        yrt.withdraw(yrtToRedeem);
    ...
    function onGlpDeposit(uint256 _amount, uint256 _mintIncentive) external nonReentrant onlyRole(OPERATOR_ROLE) {
        _borrowGlp(_amount, _mintIncentive);
        // if (leverage() < leverageConfig.target) {
        //     _leverage(_amount);
        // }
        (uint256 underlying, ) = getUnderlyingGlp();
        _rebalance(underlying);
    ...
    function leverage() public view returns (uint256) {
        (uint256 glpTvl, uint totalGlp) = getUnderlyingGlp(); // 18 Decimals
    ...
    /**
     * @notice get user GLP and total glp // BAVA chef comment
     */
    function getUnderlyingGlp() public view returns (uint256, uint256) {
        ...
        if (stableDebt > 0) {
            ...
            return (totalGlpBalance > glpAmount ? totalGlpBalance - glpAmount : 0, totalGlpBalance);
        } else {
            return (totalGlpBalance, totalGlpBalance);
        }
    }
    ...
    function _borrowGlp(uint256 _amount, uint256 _mintIncentive) private {
        glpVault.borrow(_amount);
        
        yrt.deposit(_mintIncentive);
