...
contract ParticleHolder is Manageable {
    ...
    /// @param ids Array containing an array of ids to be bridged per wallet
    function getParticlesFrom(address[] calldata wallets, uint256[][] calldata ids) public onlyManagers {
        for (uint i = 0; i < wallets.length; i++) {
            ...
            for (uint j = 0; j < ids[i].length; j++) {
                tokenContract.safeTransferFrom(wallet, address(this), ids[i][j]);
            }

            emit TokensTransfered(wallet, tokenContract.balanceOf(wallet), ids[i]);
        }
    }
}