...
import {IFlashLoanReceiver} from "../flashclaim/IFlashLoanReceiver.sol";
import {IENSReverseRegistrar} from "../flashclaim/IENSReverseRegistrar.sol";
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    modifier onlyClaimAdmin() {
        require(
            ...
            "BNFT: caller is not the claim admin"
        );
        _;
    }
    ...
    function contractURI() external view override returns (string memory) {
        string memory hexAddress = StringsUpgradeable.toHexString(
            uint256(uint160(address(this))),
            20
        );
        return
            string(
                abi.encodePacked("https://metadata.benddao.xyz/", hexAddress)
            );
    }
    ...
    function setENSName(address registrar, string memory name)
        external
        nonReentrant
        onlyOwner
        returns (bytes32)
