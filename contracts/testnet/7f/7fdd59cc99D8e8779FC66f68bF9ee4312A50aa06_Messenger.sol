...
contract Messenger is IApp, Ownable {
    address public anycallContract;
    address public anycallConfig;
    address public executor;
    uint256 public flag; // flag gas
    ...
    */
    ...
    function sendMessage(uint256 _toChainID, bytes calldata _msg, uint256 _anyFee) external virtual onlyOwner {
        IAnycallProxy(anycallContract).anyCall{value: _anyFee}(
    ...
    function encodeMessage(string calldata _msg) public virtual pure returns(bytes memory encoded, uint256 length) {
        encoded = abi.encode(_msg);
    ...
    function setFlag(uint256 _flag) external virtual onlyOwner {
        flag = _flag;
    }

    function deposit(uint256 _amount) external onlyOwner {
        IAnycallConfig(anycallConfig).deposit{value: _amount}(address(this));
    }

    function withdrawNative() external onlyOwner {
        uint256 execAmount = executionBudget();
        if(execAmount > 0) {
            IAnycallConfig(anycallConfig).withdraw(execAmount);
        }
    ...
    function executionBudget() public view returns(uint256) {
        return IAnycallConfig(anycallConfig).executionBudget(address(this));
    }
