...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.3.2/contracts/utils/structs/EnumerableSet.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.3.2/contracts/security/ReentrancyGuard.sol
...
// File: ERC2981/IERC2981Royalties.sol
...
// File: ERC2981/ERC2981PerTokenRoyalties.sol
...
// added . json on BASE URI, dweb uriinstead of party animal, and string base URI variable concatenate, added whitelist function that works with function claim
// added comments on errors
...
//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.3.2/contracts/token/ERC721/extensions/IERC721Metadata.sol";



//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.3.2/contracts/token/ERC721/ERC721.sol";

//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.3.2/contracts/token/ERC721/extensions/IERC721Enumerable.sol";
...
// import "./@openzeppelin/contracts/utils/introspection/ERC165.sol";
// import "./@openzeppelin/IERC2981Royalties.sol";
// import "./@openzeppelin/ERC2981Base.sol";
// import "./@openzeppelin/ERC2981PerTokenRoyalties.sol";
    ...
    Ownable
{
    ...
    uint256 public constant POST_2778_PRICE = 3.5 ether;
    //     uint256 public constant INITIAL_CLAIM_PRICE = 2 ether;
    // uint256 public constant POST_2778_PRICE = 1.5 ether;
    ...
    // string baseUri = "https://Avavoxs.xyz/id/";
    ...
    uint256 public post2778Extra = 0;
   ...
   string baseUri = "https://dweb.link/ipfs/";

      address private whiteListedAddress1;
  /* *************** */
    /* WhiteList */
    /* *************** */
    ...
    {
        if (index >= 2778) return 0;
        if (redeemedIDs[index]) return 0;
        return getFirstRewardComponent(index) + post2778Extra;
    }
    ...
    function getPrice(uint256 index) internal pure returns (uint256 price) {
        if (index >= 2778) return POST_2778_PRICE;
    ...
    function mintAVOX(address receiver, uint256 nextTokenIndex) internal {
        uint256 nextIndexerId = enoughRandom();
        if (minted <= 15) {
        nextIndexerId = minted - 1;
        }
    ...
    function claim(uint256 n) public payable nonReentrant {
              require(
            canClaim || msg.sender == whiteListedAddress1 ,
            "AvaVox: it's not possible to claim just yet."
        );
        require(
            MAX_MINTABLE - airdropNumber - minted > 0,
            "AvaVox: not enough AvaVox left to mint."
        );
        require(n > 0, "AvaVox: not enough AvaVox left to mint.");
        require(n <= MAX_PER_CLAIM, "Cannot claim more than 10 at a time.");
        if (n > MAX_MINTABLE - airdropNumber - minted) {
            n = MAX_MINTABLE - airdropNumber - minted;
        }
        uint256 total_cost = getNextBatchPrice(n);
        require(msg.value >= total_cost, "Minting cost must be total minting cost or greater.");
        gross += total_cost;
    ...
    function addHonoraries(address[] calldata honoraries) external onlyOwner {
        uint256 honoraryID = numHonoraries + 5555;
    ...
    function redeem(
        ...
        require(
            canClaimAirdrop,
            "AvaVox: it's not possible to claim the airdrop at the moment."
        );
