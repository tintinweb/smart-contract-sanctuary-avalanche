...
contract FollowNFT is LensNFTBase, IFollowNFT {
  ...
  constructor(address hub, MinimalForwarder forwarder) ERC2771ContextExtended(address(forwarder)) {
  ...
  function delegateBySig(
    ...
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
          keccak256(abi.encode(DELEGATE_BY_SIG_TYPEHASH, delegator, delegatee, sigNonces[delegator]++, sig.deadline))
        ),
        delegator,
        sig
      );
