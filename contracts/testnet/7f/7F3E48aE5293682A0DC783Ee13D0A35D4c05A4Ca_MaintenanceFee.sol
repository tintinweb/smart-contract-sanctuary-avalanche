    ...
    AccessControlUpgradeable
{
    ...
    function payMaintenanceFee(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (
                ...
                maintFee[_tokenIds[i]].firstDay = uint64(time);
            } else {
            ...
            maintFee[_tokenIds[i]].feeDue = ((maintFee[_tokenIds[i]].feeDue ==
                ...
                ? uint64(time.add(_paidDays[i].mul(3 minutes)))
     ...
     */
    function getFeeDue(uint256 _tokenId)
        public
        view
        returns (uint64 feeDue)
    {
        feeDue = maintFee[_tokenId].feeDue > uint64(block.timestamp)
						? (maintFee[_tokenId].feeDue - uint64(block.timestamp)) / 3 minutes
						: 0;
    }

		function getPaidDays(uint256 _tokenId) public view returns (uint256 paidDais) {
