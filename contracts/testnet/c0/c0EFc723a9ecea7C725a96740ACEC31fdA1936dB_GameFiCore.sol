...
import "./interface/ITokenWithdraw.sol";
...
import "./interface/token/IGameFiTokenERC20.sol";
import "./interface/token/IGameFiTokenERC721.sol";
import "./interface/token/IGameFiTokenERC1155.sol";
    ...
    IGameFiCore
{
    ...
    modifier onlyAdminOrMinter() {
        require(
            ...
            "GameFiCore: caller is not the admin/minter"
        );
        _;
    }
    ...
    function withdrawToken(TokenStandart standart, Token memory token) external override onlyAdmin nonReentrant {
    ...
    function setPropertyValue(
        ...
        require(targetProperty.isActive, "GameFiCore: inactive property");
    ...
    function createCollection(
        ...
        // TODO
        // сделать общий интерфейс инициализации
    ...
    function mintTokenToWallet(
        ...
        bytes memory data
    ) external override onlyAdminOrMinter nonReentrant {
        require(targetWallet != address(0), "GameFiCore: zero wallet");
        _checkCollectionExistense(collectionToken.collectionId);
        _checkCollectionActivity(collectionToken.collectionId);
    ...
    function _mintToken(
        ...
        Token memory token = Token(collection.tokenContract, collectionToken.tokenId, collectionToken.amount);
    ...
    function _checkCollectionActivity(uint256 collectionId) internal view {
        require(_collection[collectionId].isActive, "GameFiCore: inactive collection");
    }

    function _checkOwnerOf(address target, uint256 profileId) internal view {
        require(target == ownerOf(profileId), "GameFiCore: only for profile owner");
    }

    function _checkSupportsInterface(address target, bytes4 interfaceId) internal view {
        require(IERC165Upgradeable(target).supportsInterface(interfaceId), "GameFiCore: unsupported interface");
    }
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function __ProfileRegistry_init(
        ...
        __ProfileRegistry_init_unchained(
            name_,
            symbol_,
            baseURI_
        );
    ...
    // solhint-disable-next-line func-name-mixedcase
    function __ProfileRegistry_init_unchained(
        string memory name_,
        string memory symbol_,
        string memory baseURI_
    ) internal onlyInitializing {
...
interface ITokenWithdraw is ITokenTypes {
    ...
    event WithdrawToken(address indexed sender, TokenStandart standart, Token token, uint256 timestamp);

    function withdrawToken(TokenStandart standart, Token memory token) external;
}
