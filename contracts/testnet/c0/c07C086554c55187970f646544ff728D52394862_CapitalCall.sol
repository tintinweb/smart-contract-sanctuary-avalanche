...
interface ICapitalCall {
    ...
    function distribution(uint256 _id, address investor,bytes calldata certificate) external;
...
contract CapitalCall is Context, Ownable, ICapitalCall {
    ...
    event RaisedAmountWithdrawn(uint256 indexed id, uint256 amount);
    ...
    function _onlyPropertyAccount(IERC1400RawERC20 _stAddress) private view {
        require(
            _stAddress.propertyAccount() == _msgSender(),
    ...
    modifier onlyAdmin() {
        ...
        require(whitelist.isOwner(_msgSender()) ||  whitelist.isAdmin(_msgSender()) , "Only Owner or Admin is allowed to send");
        _;
    }
        ...
        
    {
        ...
        require(whitelist.isOwner(_msgSender()) ||  whitelist.isAdmin(_msgSender()) || _stAddress.propertyAccount() == _msgSender(), "Only Owner or Admin is allowed to send");
        ...
        external
    {
        ...
        IERC1400RawERC20(address(cc.stAddress)).issue(
            investor,
            token,
            certificate
        );
    ...
    {
        CapitalCallStr storage cc = idToCapitalCall[_id];
        require(cc.stAddress.propertyAccount() == _msgSender(), "only property account");

        uint256 contractBalance = akusd.balanceOf(address(this));
        require(contractBalance > 0,"contractBalance is zero");
        ...
        emit RaisedAmountWithdrawn(_id,cc.amountRaised);
    }


    function extendTime(uint256 _id, uint256 _endDate) 
        external
        override
    ...
    {
        CapitalCallStr storage cc = idToCapitalCall[_id]; 
        require(whitelist.isOwner(_msgSender()) ||  whitelist.isAdmin(_msgSender()) || cc.stAddress.propertyAccount() == _msgSender(), "Only Owner or Admin is allowed to send");

        require(block.timestamp > cc.endDate, "capital call duration not ended");
        require(_endDate > cc.endDate,"new end date should be greater than previous end date");
        require(cc.amountRaised < cc.goal, "can not extend time when goal is reached");
