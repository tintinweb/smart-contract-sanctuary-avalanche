...
contract AuctionHouse is KeeperCompatibleInterface {
    ...
    function getActiveRoomForSeller() external view returns (RoomInfo[] memory roomInfoArray) {

        RoomInfo[] memory sellerRooms = idToRoomArray[msg.sender];
        uint counter = 0;

        for (uint i = 0; i < sellerRooms.length; i++) {
            AuctionRoom currentRoom = AuctionRoom(sellerRooms[i]._room);
            if(currentRoom.isRunning()) {
                counter += 1;
            }
        }

        RoomInfo[] memory activeRooms = new RoomInfo[](counter);

        for (uint i = 0; i < sellerRooms.length; i++) {
            AuctionRoom currentRoom = AuctionRoom(sellerRooms[i]._room);
            if(currentRoom.isRunning()) {
                counter -= 1;
                activeRooms[counter] = sellerRooms[i];
            }
        }

        roomInfoArray = activeRooms;
    }
