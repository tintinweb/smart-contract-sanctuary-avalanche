...
contract TestWhitelistTree {
    ...
    function verifyMerkleProof(uint256 amount1, uint256 amount2, string calldata ipfsCid, bytes32[] calldata proof) external {
        require(
            MerkleProof.verify(proof, whitelistTreeRoot, keccak256(abi.encodePacked(msg.sender, amount1, amount2, ipfsCid)))
        , "merkle proof failed");
        successCount++;
    }
}