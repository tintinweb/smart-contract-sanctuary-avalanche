/**
 *Submitted for verification at testnet.snowtrace.io on 2022-07-17
*/
...
// File contracts/FoundersPass.sol
...
contract FoundersPass is ERC165, IERC721, IERC721Metadata, Ownable{
    ...
    // By default not allow transfers to go through at the beginning
    bool public allTransfersLocked = true;

    mapping ( uint256 => bool ) public transferLocks;
    mapping(address => uint256) public WlAddressToCap;
    mapping ( address => bool ) private mintedAlready;
    mapping ( address => uint256 ) public addrToId;

    uint256 public MAX_CAP = 5500;

    uint256 private _basePrice = 6 ether;
    uint256 private _priceAbove2000 = 8 ether;
    uint256 private _priceAbove3500 = 12 ether;
    uint256 private _priceAbove4500 = 20 ether;
    uint256 private _priceAbove5000 = 35 ether;

    uint8 public mintingPhase;

    event MintHappened(uint256 nextId, uint256 remainingMintCount);
    /**
...
if (msg.sender != owner() && allTransfersLocked) { revert ("TransferIsLockedGlobally"); }
...
if(nextId > 5000) {
return _priceAbove5000;
}
else if(nextId > 4500) {
return _priceAbove4500;
}
else if(nextId > 3500) {
return _priceAbove3500;
}
else if(nextId > 2000) {
return _priceAbove2000;
}
else {
return  _basePrice;
}
...
function mintLive() public payable {
    uint256 supply = totalSupply();

    require(mintingPhase == 2, "Mint is not live");
    require(mintedAlready[msg.sender] == false, "Wallet minted already");
    require(balances[msg.sender] == 0, "Wallet owns this token already");
    ...
    // Emits the nextId it will mint with and the remainig mint count
    emit MintHappened(nextId, (MAX_CAP - (nextId - 1)));
}

function mintOTC() public payable {
    uint256 supply = totalSupply();
    require(mintingPhase == 1, "Founders Pass only");
    require(supply + 1 <= MAX_CAP, "Mint amount exceeds max supply");
    require(mintedAlready[msg.sender] == false, "Wallet minted already");
    //require(incubator_supply + _mintAmount <= INCUBATOR_MAX_SUPPLY, "Exceeds total token supply");
    require(WlAddressToCap[msg.sender] > 0, "Not eligible for allowlist mint");

    uint256 price = currentPrice();

    require(msg.value >= price, "Price is too low");

    //Reduce number of AL allocation
    WlAddressToCap[msg.sender] -= 1;
...
function ownerMintToAddresses(address[] memory _addresses) external onlyOwner {
  ...
  uint256 supply = totalSupply();
  require(supply + _addresses.length <= MAX_CAP, "Mint amount exceeds max supply");

    for (uint i = 0; i < _addresses.length; i++) {
        cheapMint(_addresses[i], 1);
        // Emits the nextId it will mint with and the remainig mint count
        emit MintHappened(nextId, (MAX_CAP - (nextId - 1)));
    }
}

function ownerMintTo(uint256 _mintingAmount,address _mintingTo) external onlyOwner {
  
  uint256 supply = totalSupply();
  require(supply + _mintingAmount <= MAX_CAP, "Mint amount exceeds max supply");

        cheapMint(_mintingTo, _mintingAmount);
...
*/
function cheapMint (address _recipient,uint256 _amount) internal {
...
if (nextId - 1 + _amount > MAX_CAP) { revert ("CapExceeded"); }
  ...
  addrToId[_recipient] = startTokenId;
  mintedAlready[_recipient] = true;
...
*/
function changeMintPhase(uint8 _val) external onlyOwner {
require(_val < 3, "3 phases only: pasued, whitelist and public" );
mintingPhase = _val;
}
...
{
WlAddressToCap[_whitelisted[i]] = 1;
}
}
...
return WlAddressToCap[_whitelistedAddr];
}
