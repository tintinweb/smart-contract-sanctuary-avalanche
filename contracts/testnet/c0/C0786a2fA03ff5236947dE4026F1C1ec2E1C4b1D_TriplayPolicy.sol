...
contract TriplayPolicy is Authorization {
    ...
    address[] public treasuriesToBeAdded;
    address[] public treasuriesToBeRemoved;
    ...
    function executeHighImpactAction() external auth {
        HighImpactAction storage action = pendingHighImpactAction;
        require(action.status == HighImpactActionStatus.Active, "not an active action");
        require(block.timestamp >= action.executionTime, "too early to execute");
        if (action.name == "addTreasury") {
            bytes32 value = action.value;
            address _treasury = address(bytes20(value));
            require(_treasury != address(0), "zero address"); 
            isTreasury[_treasury] = true;
        } else if (action.name == "removeTreasury") {
            isTreasury[address(bytes20(action.value))] = false;
        } else if (action.name == "updateCooldownPeriod") {
            cooldownPeriod = uint256(action.value);
        } else {
            revert("Invalid Action");
        }
        emit ExecuteHighImpactAction(msg.sender, action.name, action.value);
        // delete pendingHighImpactAction;
        action.status = HighImpactActionStatus.Inactive;
    }
    ...
    function verifyTransferToTreasury(address player, address _treasury) external view {
        require(isPermitted[player] == 1, "Action performed by unauthorized address.");
