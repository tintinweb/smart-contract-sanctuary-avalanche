        ...
        if(OverFlowPlayerStatus[playerId]){
            if(_currentUserStage  > 0){
                ...
                if(totalNFTOnTile[_currentUserStage - 1][previousJumpSide] > 0){
                    ...
                    // PlayerItem[PlayerIDAtSpecifyStage[i]].stage
                    // PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide

                    for (uint i = 0; i < PlayerIDAtSpecifyStage.length; i++) {
                        if((PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide) && 
                            (OverFlowPlayerStatus[PlayerIDAtSpecifyStage[i]] != false && 
                            _checkSide(PlayerItem[PlayerIDAtSpecifyStage[i]].stage,PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide) )) {
