...
/// @title For Unstaking old LP pool
/// @dev Only the owner can add user infos
contract StoreOldRewards is Ownable, ReentrancyGuard {
    ...
    uint256 public cntUsers = 0;

    mapping(address => UserInfo) public users;
    mapping(address => bool) public isWhitelistUser;

    constructor(IERC20 _THOR) {
        require(address(_THOR) != address(0), "constructor: _THOR must not be zero address");
        THOR = _THOR;
    }

    function addUserInfoBatch(
        address[] memory userAddresses,
        uint256[] memory amounts,
        uint256[] memory pendingRewards,
        bool[] memory hasPendingBefore13thNovs
    ) external onlyOwner {
        for (uint256 i = 0; i < userAddresses.length; i++) {
            address user = userAddresses[i];
            uint256 amount = amounts[i];
            uint256 pendingReward = pendingRewards[i];
            bool hasPendingBefore13thNov = hasPendingBefore13thNovs[i];
            if (!isWhitelistUser[user]) {
                users[user] = UserInfo(amount, pendingReward, hasPendingBefore13thNov, false);
                isWhitelistUser[user] = true;
                cntUsers++;
                totalAmount += amount;
    ...
    function addUserInfo(
        ...
        require(isWhitelistUser[user] == false, "already whitelist");
        users[user] = UserInfo(amount, pendingReward, hasPendingBefore13thNov, false);
        isWhitelistUser[user] = true;
        cntUsers++;
        totalAmount += amount;
    ...
    function requestReward() external {
        ...
        require(user.paid == false, "you have been paid");
        require(user.pendingReward > 0, "you have empty rewards");
        user.paid = true;
        safeTHORTransfer(msg.sender, user.pendingReward);
    }
