/**
 *Submitted for verification at testnet.snowtrace.io on 2022-09-20
*/
...
{ 
    ...
    function createAffiliateCode() external
    {
        ...
        createNewAffiliateCode();
    }

    function getOrCreateAffiliateCode() external returns (string memory)
    {
        //  require(tx.origin == tx.origin, "Only EOA");
        string memory code = _promoCodes[tx.origin];
        if(_codeOwners[code] != address(0)) { //You have not created an affiliate code yet
            createNewAffiliateCode();
            _getAffiliateCode();
        }
        return code; 
    ...
    function getAffiliateCodeForAddr(address affiliateAddr) public view returns (string memory) 
    {
        string memory code = _promoCodes[affiliateAddr];
         ...
         require(_codeOwners[code] == affiliateAddr, "You are not owner of this affiliate code");
        return code; 
    }
    ...
    function _getAffiliateCode() private view returns (string memory)
    {
        //  require(tx.origin == tx.origin, "Only EOA");
        string memory code = _promoCodes[tx.origin];
         require(_codeOwners[code] != address(0), "You have not created an affiliate code yet");
    ...
    function createNewAffiliateCode() private {
