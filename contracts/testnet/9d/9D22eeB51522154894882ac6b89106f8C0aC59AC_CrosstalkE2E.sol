...
/*
    ...
    - Happy Paths
        ...
        4 Single Request with Ack
        5 Multiple Request with Ack
    - Error Paths
        6 Gas Limit Errors Single Request
        7 Gas Price Errors Single Request
        8 ACK Gas Price Error
        9 ACK Gas Limit Error
        10 Error ACK Single Request
        11 Error ACK
...
contract CrosstalkE2E is AccessControl , ICrossTalkApplication {
    ...
    event CrossTalkE2EReceive( uint64 indexed requestId , uint8 _type );

    event CrossTalkE2EACKReceive( uint64 indexed requestId , uint8 _type , bool _success );

    error CustomRevert( uint64 requestId , uint8 _type);
    ...
    function setDestGasLimit ( uint64 _gasLimit ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        destGasLimit = _gasLimit;
    }

    function setAckGasLimit ( uint64 _gasLimit ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ackGasLimit = _gasLimit;
    }

    function setStringPayload ( string memory _payload ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function testCrossTalk(
        ...
        bytes memory payload = abi.encode(_requestId.current(), reqType ,stringPayload);
        ...
        for (uint i = 0; i < destAddress.length; i++) {
            if ( ( i == 1 ) && ( ( reqType == 2 ) || ( reqType == 3 ) || ( reqType == 5 ) ) ){
                bytes memory payload = abi.encode(_requestId.current(), reqType , true ,stringPayload);
            }else {
                bytes memory payload = abi.encode(_requestId.current(), reqType , false ,stringPayload);
            }
    ...
    function handleRequestFromSource(
        ...
        (uint64 reqID, uint8 reqType , bool error , string memory strPayload) = abi.decode(
            payload,
            (uint64, uint8 , bool , string)
        );
        emit CrossTalkE2EReceive( reqID , reqType );
        if (error) {
            revert CustomRevert( reqID  , reqType);
        }
        if (reqType == 1 ) {
            returnPayload = abi.encode( reqID , reqType , true );
    ...
    function handleCrossTalkAck(
        ...
        (uint64 reqID, uint8 reqType , bool success ) = abi.decode(
            _execData,
            (uint64, uint8, bool)
        );

        emit CrossTalkE2EACKReceive(reqID, reqType , success);
    }
