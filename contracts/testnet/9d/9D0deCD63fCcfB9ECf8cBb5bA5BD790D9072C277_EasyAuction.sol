...
import "./interfaces/Ordermanage.sol";

contract EasyAuction is Ownable, Ordermanage {
    ...
    mapping(uint256 => AuctionData1) public auctionData1;
    ...
    function initiateAuction(
        auctionTokken memory object
    ) public returns (uint256) {
        object._auctioningToken.safeTransferFrom(
            msg.sender,
            address(this),
            object
        ...
        {
            require(
                object._auctionedSellAmount > 0,
                "cannot auction zero tokens"
            );
        }
        {
            require(
                object._minBuyAmount > 0,
                "tokens cannot be auctioned for free"
            );
            require(
                object.minimumBiddingAmountPerOrder > 0,
                "minimumBiddingAmountPerOrder is not allowed to be zero"
            );
            require(
                object.orderCancellationEndDate <= object.auctionEndDate,
                "time periods are not configured correctly"
            );
            require(
                object.auctionEndDate > block.timestamp,
        ...
        auctionData[auctionCounter] = AuctionData(
            object._auctioningToken,
            object._biddingToken,
            object.orderCancellationEndDate,
            object.auctionEndDate,
            IterableOrderedOrderSet.encodeOrder(
                userId,
                object._minBuyAmount,
                object._auctionedSellAmount
            ),
            object.minimumBiddingAmountPerOrder,
            0,
        ...
        auctionData1[auctionCounter] = AuctionData1(
            feeNumerator,
            object.minFundingThreshold
        );
    ...
    function placeSellOrders(
        manageOrder memory object
    ) external atStageOrderPlacement(object.auctionId) returns (uint64 userId) {
        object.orderSubmitter = msg.sender;
        return _placeSellOrders(object);
    }
    ...
    function _placeSellOrders(
    ...
    ) internal returns (uint64 userId) {
        {
            ...
            ) = auctionData[object.auctionId].initialAuctionOrder.decodeOrder();
            require(
                object._minBuyAmounts.mul(buyAmountOfInitialAuctionOrder) <
                    sellAmountOfInitialAuctionOrder.mul(object._sellAmounts),
                "limit price not better than mimimal offer"
            );
        }
        uint256 sumOfSellAmounts = 0;
        userId = getUserId(object.orderSubmitter);
        uint256 minimumBiddingAmountPerOrder = auctionData[object.auctionId]
            .minimumBiddingAmountPerOrder;
        require(
            object._minBuyAmounts > 0,
            "_minBuyAmounts must be greater than 0"
        );
        require(
            object._sellAmounts > minimumBiddingAmountPerOrder,
            "order too small"
        );
        if (
            sellOrders[object.auctionId].insert(
                IterableOrderedOrderSet.encodeOrder(
                    userId,
                    object._minBuyAmounts,
                    object._sellAmounts
                ),
                object._prevSellOrders
            )
        ) {
            sumOfSellAmounts = sumOfSellAmounts.add(object._sellAmounts);
            emit NewSellOrder(
                object.auctionId,
                userId,
                object._minBuyAmounts,
        ...
        auctionData[object.auctionId].biddingToken.safeTransferFrom(
            msg.sender,
            address(this),
            sumOfSellAmounts
        );
    ...
    function cancelSellOrders(
        ...
        bool success = sellOrders[auctionId].removeKeepHistory(_sellOrders);
        if (success) {
            (
                uint64 userIdOfIter,
                uint96 buyAmountOfIter,
                uint96 sellAmountOfIter
            ) = _sellOrders.decodeOrder();
            require(
                userIdOfIter == userId,
    ...
    function precalculateSellAmountSum(
        ...
        for (uint256 i = 0; i < iterationSteps; i++) {
            ...
            (, , uint96 sellAmountOfIter1) = iterOrder.decodeOrder();
            sumBidAmount = sumBidAmount.add(sellAmountOfIter1);
        }
        require(
        ...
        returns (bytes32 clearingOrder)
    {
        ...
        if (auctionData1[auctionId].minFundingThreshold > currentBidSum) {
    ...
    {
        require(
            sellOrders[auctionId].remove(orders),
        ...
        (, uint96 priceNumerator, uint96 priceDenominator) = auction
            .clearingPriceOrder
            .decodeOrder();
        (uint64 userId, , ) = orders.decodeOrder();
        ...
        if (minFundingThresholdNotReached) {
            ...
            if (orders == auction.clearingPriceOrder) {
                ...
                if (orders.smallerThan(auction.clearingPriceOrder)) {
                    sumAuctioningTokenAmount = sumAuctioningTokenAmount.add(
    ...
    function processFeesAndAuctioneerFunds(
        ...
        uint256 feeAmount = fullAuctionedAmount
            .mul(auctionData1[auctionId].feeNumerator)
            .div(FEE_DENOMINATOR);
    ...
    function returnEncodedOrder(
        uint64 userId,
        uint96 buyAmount,
        uint96 sellAmount)public pure returns(bytes32){
            return
            bytes32(
                (uint256(userId) << 192) +
...
library IdToAddressBiMap {
    ...
    function convertIntToUint(int myInt) public pure returns (uint256) {
        return uint256(myInt);
    }
    ...
    function insert(
        ...
        uint value = convertIntToUint(-1);
        require(id != value, "Cannot insert max uint64");
        if (
