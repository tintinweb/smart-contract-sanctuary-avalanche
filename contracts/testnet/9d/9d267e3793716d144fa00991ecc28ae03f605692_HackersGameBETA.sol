...
contract HackersGameBETA is ERC20, ERC721Holder, Ownable{
    IERC721 public hackerNFT;
    IERC721 public cubeNFT;
    ...
    constructor(address _hackerNFT,address _cubeNFT) ERC20("HACK", "HACK") {
        hackerNFT = IERC721(_hackerNFT);
        cubeNFT = IERC721(_cubeNFT);
    ...
    function setCube() private onlyOwner{
        ...
        _cube_rewards[WALLS][1] = 200; // x1.0
        _cube_rewards[WALLS][2] = 210; // x1.1
        _cube_rewards[WALLS][3] = 230; // x1.3
        _cube_rewards[WALLS][4] = 250; // x1.5
        _cube_rewards[WALLS][5] = 300; // x2
        
        _cube_rewards[FLOOR][1] = 200; 
        _cube_rewards[FLOOR][2] = 210; 
        _cube_rewards[FLOOR][3] = 230; 
        _cube_rewards[FLOOR][4] = 250; 
        _cube_rewards[FLOOR][5] = 300; 
        
        _cube_rewards[PC][1] = 1.5 ether;
        _cube_rewards[PC][2] = 3.5 ether;
        _cube_rewards[PC][3] = 7 ether; 
        _cube_rewards[PC][4] = 18 ether;
        _cube_rewards[PC][5] = 45 ether;
        
        _cube_rewards[TABLE][1] = 1.3 ether;
        _cube_rewards[TABLE][2] = 5 ether;
        _cube_rewards[TABLE][3] = 9 ether;
        _cube_rewards[TABLE][4] = 18 ether;
        _cube_rewards[TABLE][5] = 40 ether; 
        
        _cube_rewards[CONSOLE][1] = 1.2 ether; // 1%
        _cube_rewards[CONSOLE][2] = 8 ether; // 1%
        _cube_rewards[CONSOLE][3] = 12 ether; // 1%
        _cube_rewards[CONSOLE][4] = 19 ether; // 1%
        _cube_rewards[CONSOLE][5] = 25 ether; // 1%
    ...
    function stakeHacker(uint256 tokenId) external {
        hackerNFT.safeTransferFrom(msg.sender, address(this), tokenId);
        hacker.push(Hacker("",0,0,1,0, tokenId,2.05 ether,0,0, true,false, msg.sender));
        stakedHackerId[tokenId]++;
    ...
    function unstakeHacker(uint256 tokenId) external {
        ...
        hackerNFT.transferFrom(address(this), msg.sender, hacker[tokenId].ID);
        delete hacker[tokenId];
        ownerOfHackers[msg.sender]--;
    }
    function matchCube(uint8 tokenId, uint8 _hackerId) external {
        ...
        cubeNFT.safeTransferFrom(msg.sender, address(this), tokenId);
    ...
    function unstakeCube(uint256 tokenId, uint256 hackerId) external {
        ...
        cubeNFT.transferFrom(address(this), msg.sender, cube[tokenId].ID);
    ...
    function levelUpHacker(uint256 _hackerId) external payable{
        ...
        hacker[_hackerId].rewards = hacker[_hackerId].rewards + 15/100 *10**18;
        hackerLevel[hacker[_hackerId].ID]++;
    }
    function levelUpCube(uint8 cubePart, uint8 _level, uint256 _cubeId) external payable{
        ...
        DynamicCUBE(address(cubeNFT)).levelUpCube(cube[_cubeId].ID,_cube_levels[cube[_cubeId].ID][1],_cube_levels[cube[_cubeId].ID][2],_cube_levels[cube[_cubeId].ID][3],_cube_levels[cube[_cubeId].ID][4],_cube_levels[cube[_cubeId].ID][5]);
    }
    ...
    function payRansom(uint _hackerId) external payable {
        ...
        require(hacker[_hackerId].kidnappedTime > block.timestamp - 1 days,"late");
    ...
    /*function buyKidnappedHacker(uint _hackerId) external payable {
        ...
        require(hacker[_hackerId].kidnappedTime < block.timestamp - 1 days,"Cannot buy");
        HACKaddress.transferFrom(msg.sender,address(this),hacker[_hackerId].ransom);
