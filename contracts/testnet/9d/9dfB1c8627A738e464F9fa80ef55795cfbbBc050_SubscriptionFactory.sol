...
contract SubscriptionFactory is OwnableUpgradeable {
    ...
    function createSubscriptionManager(string memory name, address token, address treasury) external returns (address) {
        SubscriptionManager subscription = new SubscriptionManager(address(this), name, token, treasury);
    ...
    function getName(uint256 _id) public view returns (string memory) {
        return SubscriptionManager(ids[_id]).name();
    }

    function getActiveSub(uint256 _id) public view returns (uint256) {
        return SubscriptionManager(ids[_id]).getActiveSubscriptionCount();
    }

    function getTokenSymbol(uint256 _id) public view returns (string memory) {
        return SubscriptionManager(ids[_id]).getSymbol();
    }
...
contract SubscriptionManager is Ownable {
    event Payment(address indexed user, uint256 indexed subscrptionType);
