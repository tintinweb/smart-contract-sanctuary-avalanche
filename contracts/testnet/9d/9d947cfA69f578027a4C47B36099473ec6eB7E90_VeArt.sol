...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    // dışarıdan nft stake edip boost verme ama süreli olacak mantığı normal salvor stakelemeye benziyor.
    // bunu öncelikli gibi.

    // avax rewardı verirken aynı zamanda art reward verebilecekmiyiz? zamanla bizdeki art bittikçe
    // kısıcaz.


    // sınırsız nft stake edebilmeli
    // her stake işleminde kümülatif olarak burada ne birikecek power*knowledge(örnek)
    struct UserInfo {
    ...
    mapping(uint256 => address) public stakedSalvors;

    uint256 public lastRewardBalance;
    uint256 public accRewardPerShare;

    uint256 accARTPerShare;
    uint256 rewardArtGenerationRate;
    ...
    function initialize(IERC20Upgradeable _art) public initializer {
        ...
        // set generationRate (veArt per sec per art staked) old value: 3888888888888
        _name = "VeArt";
		_symbol = "VART";
        generationRate = 38888888888888888;
        rewardArtGenerationRate = 1000000000000000000;
        // set maxCap
        maxCap = 100;
        // set art
        art = _art;
    ...
    function setGenerationRate(uint256 _generationRate) external onlyOwner {
        ...
        emit GenerationRateUpdated(_generationRate);
    }
    ...
    */
    ...
    function claimableVeART(address _addr) public view returns (uint256) {
        ...
        uint256 pending = _wmul(user.amount, secondsElapsed * generationRate);
    ...
    * @param _user the address of the user
    */
    function pendingArtRewards(address _user) public view returns (uint256) {
        UserInfo memory user = users[_user];
    ...
    */
    function _claimArtEarnings(address _to, uint256 _amount) internal {
        uint256 totalArtBalance = art.balanceOf(address(this));
