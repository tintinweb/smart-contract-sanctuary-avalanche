...
/* Admin transfer provider: allows transfers only for the admin role */
contract VNXAdminTransferProvider is Ownable, ITransferProvider {
  using SafeMath for uint256;
  
  address private rbacManager;

  constructor(address rbac)
  {
    require(rbac!=address(0), "RBAC address cannot be zero");
    rbacManager = rbac;
  }
  /*
  ...
  */
  function approveTransfer(address from, address to, uint256 value, address spender) external override returns(bool)
  {
    if (_isAdmin(spender)) {
      ...
      emit TransferDeclined(from, to, value);
      return false;
    }
  }
  ...
  */
  function considerTransfer(address from, address to, uint256 value) external pure override returns(bool)
  {
     ...
     */
    function _isAdmin(address _spender) internal view returns (bool) {
      return IRBAC(rbacManager).hasRole(_spender, 0);
    }
