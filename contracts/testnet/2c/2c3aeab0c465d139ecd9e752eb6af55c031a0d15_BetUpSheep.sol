...
contract BetUpSheep is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    function createCard(uint256 _amount, address _receiver, uint8[] memory _fixedItems ) external onlyRole(FREE_MINT_ROLE) {
        ...
        if(!hasRole(FREE_MINT_ROLE, msg.sender)) {
            ...
            uint256 beforeNumberMinted = _numberMinted(receiver);
            ...
            require(_isAvailableMint(receiver, consumeCount, consumeAmount), "condition is not met");
            IBetUpCore(betCoreAddress).recordPlayerConsumption(receiver, consumeCount);
        }else {
            receiver = _receiver;
        }

        _mintWithLevel(_amount, _fixedItems, receiver);
    }
