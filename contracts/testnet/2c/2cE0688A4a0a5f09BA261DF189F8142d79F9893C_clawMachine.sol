...
contract clawMachine is ConfirmedOwner, VRF {
    ...
    function internalMint(address to) internal {
        require(
            minted1() < contract1.maxMintable() ||
            minted2() < contract2.maxMintable() ||
            minted3() < contract3.maxMintable() ||
            !avaSharkWon ||
            avaxWon < 100,
            "no more prizes"
        );
        ...
        if (
            randomNum <= 20 && minted1() < contract1.maxMintable()
        ) {
            contract1.mint(to);
            emit win(1, to);
        } else if (
