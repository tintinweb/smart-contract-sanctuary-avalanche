...
contract BridgesOfFate is Ownable {
    ...
    function _overFlow(bytes32 _playerId,bool _jumpSide) internal returns(bool)  {
        GameStatus storage  _gameStatus = GameStatusInitialized[latestGameId];        
        ...
        // }            
        if(totalNFTOnTile[PlayerItem[_playerId].stage][_jumpSide] <= 0){                
        ...
        if(totalNFTOnTile[PlayerItem[_playerId].stage][_jumpSide] > 0 ){
            totalNFTOnTile[PlayerItem[_playerId].stage][_jumpSide] = totalNFTOnTile[PlayerItem[_playerId].stage][_jumpSide] - 1;
        }  
        return  OverFlowPlayerStatus[_playerId];
    }
    ...
    */
    ...
    function changeStatyStatus(bytes32 playerId) public {
        GameItem storage _member = PlayerItem[playerId];
        _member.ishold =  true;
