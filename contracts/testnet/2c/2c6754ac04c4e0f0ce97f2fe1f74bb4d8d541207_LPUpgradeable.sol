    ...
    PausableUpgradeable
{
     ...
     * Sale settings
     */
    uint256 public airdropPrice;//  10000 test for USDC with 6 decimals
    ...
    IERC20 private weth; // //0x5425890298aed601595a70AB815c96711a31Bc65 - AVAX TESTNET
    ...
    */
    ...
    function validatePhase3Mint(uint256 quantity) public view {
        require(phase3Start <= block.timestamp && block.timestamp <= phase3End, "Phase 3 inactive");
        require(quantity <= 20, "Max 20 at once");
        require(totalSupply() + quantity <= maxSupply, "Exceeds supply");
    }
    ...
    function validateMintEligibility(
        ...
        if (block.timestamp < phase2Start || paused()) return "Mint is paused";

        if (phase2Start <= block.timestamp && block.timestamp < phase3Start) {
            if (!isMintValid(to, proof, phase2Root)) return "Not in phase 2 whitelist";
            ...
            if (block.timestamp > phase3End) return "Phase 3 has ended";
            if (quantity > 20) return "Max 20 at once";
            if (totalSupply() + quantity > maxSupply) return "Exceeds supply";
            return "";
        }
