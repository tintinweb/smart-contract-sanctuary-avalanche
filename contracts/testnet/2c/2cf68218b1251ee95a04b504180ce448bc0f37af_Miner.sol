...
contract Miner is ERC721Enumerable, Ownable, Pausable {
   using Strings for uint256;
    ...
    uint256 public constant MAX_BASE_SUPPLY = 5;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isUpgrade(_tokenId)) {
            ...
            return string(abi.encodePacked(BASE_URI, "json-content/unrevealed.json"));
        } else if (revealedTokenLevel(_tokenId) == 1) {
            return string(abi.encodePacked(BASE_URI, "json-content/", _tokenId.toString(), "-super.json"));
        } else {
            return string(abi.encodePacked(BASE_URI, "json-content/", _tokenId.toString(), "-miner.json"));
        }
    }
