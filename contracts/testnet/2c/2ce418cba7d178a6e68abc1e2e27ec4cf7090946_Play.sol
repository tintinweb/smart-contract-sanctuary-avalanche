...
contract Play is RebaseTKN, GMaster, Utils {
    ...
    */
    function gameStats(uint _gIndex) public onlyOwn() returns(uint, uint, uint){
        ...
        waged[_gIndex]._toDistrib = _distrLos;                                      // This should be += not only +
    ...
    */
    function check(uint _gIndex) public  returns(uint,uint,uint,uint) {
        require(checkIfWon(_gIndex) == true,"You did not win!!!");
        uint _betted = bets[msg.sender][_gIndex]._wage;
        //uint _toDist = waged[_gIndex]._toDistrib; // I can just call this directly waged[_gIndex]._toDistrib
        uint _amtWon = (bets[msg.sender][_gIndex]._wage * waged[_gIndex]._toDistrib) / (10 ** 18);
        ...
        return (_betted, waged[_gIndex]._toDistrib, _amtWon, _toCashOut);
    }
    ...
    */
    function cashOut(uint _gIndex) public returns (bool) {
        ...
        uint fragVal = bets[msg.sender][_gIndex]._toCashOut * _fragXTKN; // need to change name to wageVal
        _fragBal[casino] = _fragBal[casino] - fragVal;
        _fragBal[msg.sender] = _fragBal[msg.sender] + fragVal;
        bets[msg.sender][_gIndex]._toCashOut = 0;
