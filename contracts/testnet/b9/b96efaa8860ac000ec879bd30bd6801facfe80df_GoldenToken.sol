...
contract GoldenToken is Ownable, ERC721 {
    ...
    constructor(string memory name_, string memory symbol_, bytes32 newRoot, address _apaAddress)
        ERC721(name_, symbol_)
    {
        wlMerkleRoot = newRoot;
    ...
    function getGoldenToken(uint256 tokenId, uint256 rarity, bytes32[] memory proof) external {
        if (!MerkleProof.verify(proof, wlMerkleRoot, keccak256(abi.encodePacked(tokenId, rarity)))) {
            revert Unauthorized();
        }
