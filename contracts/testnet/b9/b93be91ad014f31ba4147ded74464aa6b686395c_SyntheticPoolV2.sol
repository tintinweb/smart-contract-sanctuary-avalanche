...
contract SyntheticPoolV2 is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 poolStartDate;                                                                      // Pool trading start timestamp
    uint256 tradingHour;
    ...
    */
    ...
    function _calculateDays(uint256 currentTimestamp) view public returns(uint256, uint256) {
        uint256 numberOfDays = (currentTimestamp - poolStartDate)/ (1 days);
        uint256 startTradingHour = poolStartDate * numberOfDays * (1 days);
        uint256 endTradingHour = startTradingHour + tradingHour;
        return (startTradingHour, endTradingHour);
    }
