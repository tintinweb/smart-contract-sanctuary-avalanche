...
contract Allrounder is IERC20 {
    ...
    address public tfusdAdd = 0x5DAeB69622c3870e5cC1043a4C61ea3A0c49B6E9 ;
...
constructor(address _routerAddress) {
        ...
        _balances[address(this)].balance = LPDriverThreshold;
   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    function _transfer(
        ...
        require(_balances[from].balance >= amount, "insufficient balance");
        uint feeAmount;
    ...
    function _lpDriver(uint desiredA, uint desiredB) private {
            uint gurdianReward;
    ...
    function selfLpDriver(uint _dz, uint _dy) external {
    ...
    function bpFixing( uint amount, uint feeAmount) public {
        ...
        if(expectedPairAmount > initialPairBalance){
            ...
            uint contractReserveBal = (_balances[address(this)].balance).sub(swapRequiredToken);
                ...
                require(totalSupply.add(surplasAmount) <= maxSupply, "max suplly exceed, reduce the tx or mode");
                if(maxSupply.sub(totalSupply) < 100*(10**_decimals)){
                    mode = 2;
                }
                totalSupply = totalSupply.add(surplasAmount);
                }
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function upgradeGurdian(address[] calldata addressArray, bool rank) external onlyManager{
        if(rank == true){
            require(NumberOfGurdian.add(addressArray.length) <= MaxNumberOfGurdian, "exceed totals");
        }else{
            require(NumberOfGurdian > 0);
            require(NumberOfGurdian.sub(addressArray.length) > 0, "over than existing");
        }
        for(uint i = 0; i < addressArray.length; i++){
    ...
    function setRouter(address _routerAdd, address _pegToken) external onlyManager {
        routerAddress = _routerAdd;
        router = IDEXRouter(_routerAdd);
        nativeCoin = router.WAVAX();
        tAddress = IDEXFactory(router.factory()).createPair(nativeCoin, address(this));
        isFeeExempt[routerAddress] = true;
        if(_pegToken != address(0)){
          tAddress = IDEXFactory(router.factory()).createPair(_pegToken, address(this));
        }
        isMaxExempt[tAddress] = true;
        isWhitelisted[tAddress]= true;
    }
