    ...
    PTokenAdmin
{
    function initialize(
        ...
        if (address(middleLayer_) == address(0) ||
            address(eccAddress_) == address(0)) revert AddressExpected();
        if (decimals_ == 0) revert ParamOutOfBounds();

        underlying = underlying_;
        decimals = decimals_;
        middleLayer = IMiddleLayer(middleLayer_);
        ecc = IECC(eccAddress_);
    }
    ...
    */
    function deposit(
        ...
        uint256 actualTransferAmount = _doTransferIn(amount);
        uint256 actualDepositAmount = (actualTransferAmount * 10**18) / exchangeRate;
        ...
        /* We emit a Deposit event, and a Transfer event */
        // emit Transfer(address(this), msg.sender, actualDepositAmount);
    }

    /**
    * @notice Sender withdraws pTokens in exchange for a specified amount of underlying asset
...
abstract contract IPToken {
    ...
    function deposit(uint256 amount, address route) external virtual payable;

    function withdraw(uint256 withdrawAmount, address route) external virtual payable;
