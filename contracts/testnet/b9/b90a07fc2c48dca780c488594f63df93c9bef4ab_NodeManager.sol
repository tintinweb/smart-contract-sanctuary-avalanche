...
contract NodeManager is Ownable, Pausable {
    ...
    function createNode(address sender, address receiver, uint256 nodeValue_, bool withRewards) 
    external whenNotPaused {
        if (sender == receiver) {
            require(isNodeOwner(sender), "Cannot create nodes for self");
        }
        
        //If it is the first node, then create a new node object. Otherwise add to the 
        //existing node's value
        // 2 Cases
        //Sender has no nodes and they are creating for themself
        //Reciever has no node and it is a donation
        if ((!isNodeOwner(sender) && sender == receiver) || (!isNodeOwner(receiver) && sender != receiver)){
            ...
            uint256 nodeGiverCount = nodeGivers.get(sender);
            nodeGiverCount += 1;
            nodeOwners.set(receiver, 1);
            nodeGivers.set(sender, nodeGiverCount);
            ...
            //"Create" a new node by adding the equivalent value to the nodeValue
        ...
        returns (uint256)
    {
        if (isNodeOwner(account)){
            NodeEntity storage node = _nodesOfUser[account][0];
            uint256 lct = node.lastClaimTime;
            uint256 nv = node.nodeValue;
            uint256 rewardsCalculated = _calculateNodeRewards(lct, nv);
            node.rewardsAvailable += rewardsCalculated;
