...
contract LydiaRestrictedStrategyAddTwoSidesOptimal is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "LydiaRestrictedStrategyAddTwoSidesOptimal::onlyWhitelistedWorkers:: bad worker");
    _;
  }
  ...
  function execute(
    ...
    baseToken.safeApprove(address(bento), uint256(-1));
    ...
    ITridentRouter.ExactInputSingleParams memory inputSingleParams = ITridentRouter.ExactInputSingleParams(
      swapAmt,
      lpToken.getAmountOut(abi.encode(path[0], swapAmt)),
      address(lpToken),
      path[0],
      abi.encode(path[0], address(this), true)
    );
    if (swapAmt > 0) router.exactInputSingleWithNativeToken(inputSingleParams);
    ...
    tokenInput[0] = ITridentRouter.TokenInput(baseToken, false, baseToken.myBalance());
    tokenInput[1] = ITridentRouter.TokenInput(farmingToken, false, farmingToken.myBalance());

    router.addLiquidity(tokenInput, address(lpToken), minLPAmount, abi.encode(msg.sender));

    require(
      lpToken.transfer(msg.sender, lpToken.balanceOf(address(this))),
      "LydiaRestrictedStrategyAddTwoSidesOptimal::execute:: failed to transfer LP token to msg.sender"
    );
    // 7. Reset approve to 0 for safety reason
    baseToken.safeApprove(address(bento), 0);
