...
contract SmOracleNew is Ownable {
    ...
    mapping(bytes32 => RegressionStat) public regressionStats;
    ...
    uint256 public initialDailyAveragePrice;
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _prices.length; i++) {
            ...
            console.log("predictedDelta %s, actualDelta %s, deltaAbsDiff %s", uint256(predictedDelta), uint256(actualDelta), deltaAbsDiff);
            if (transactionData[_currencyKey].prices.length >= counter) {
                ...
                console.log("deltaSd %s, deltaMean %s", deltaSd, deltaMean);
    ...
    function setInitialDailyAveragePrice(uint256 _initialDailyAveragePrice) external onlyOwner {
        initialDailyAveragePrice = _initialDailyAveragePrice;
    }
    ...
    function getPredictedDelta(bytes32 _currencyKey, uint256 _tokenId) public view returns(int256) {
        ...
        for (uint256 i = 0; i < regressionStats[_currencyKey].coefficients.length; i++) {
            console.log("i %s, _tokenId %s", i, _tokenId);
            console.log("coefficients %s", uint256(regressionStats[_currencyKey].coefficients[i]));
            console.log("attributeRarityScore %s", uint256(tokenInfo[_currencyKey][_tokenId].attributeRarityScore[i]));
    ...
    function getActualDelta(bytes32 _currencyKey, uint256 _price) public view returns (int256) {
        ...
        console.log("dailyAveragePrice: ", dailyAveragePrice);
    ...
    function getDailyAveragePrice(bytes32 _currencyKey) public view returns(uint256) {
        if (transactionData[_currencyKey].prices.length < counter) {
            return initialDailyAveragePrice;
        }
        uint256 priceSum = 0;
        uint256 actualCounter = 0;
