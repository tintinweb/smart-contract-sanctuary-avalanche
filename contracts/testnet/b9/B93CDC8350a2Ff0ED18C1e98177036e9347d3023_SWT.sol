...
import "./Uniswap/ISWTMultiNetworkRouter.sol";
import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol";
import "./IBridgeToken.sol";
import "./utils.sol";
import "./common/IMarket.sol";
import "./AggregatorV3Interface.sol";
contract SWT is ERC20Upgradeable, IBridgeToken {
    // STRINGS
    ...
    uint256 antiWhaleUsdtLimit1; //antiwhale limits in usdt
    uint256 antiWhaleUsdtLimit2;
    uint256 antiWhaleUsdtLimit3;
    uint256 antiWhaleUsdtLimit1Percent; //antiwhale fees to apply
    ...
    bool public haveAssetBeforeSellCheck;
    ...
    // OBJECTS ----------------------------------------------------------------------------------------------
    ISWTMultiNetworkRouter public dexRouter;
    Fees public _taxRates;
    IMarket public marketplace;
    ...
    function initialize() public virtual initializer {
        __ERC20_init("Sicrux World Token", "SWT");
        _mint(msg.sender, 35000000000000000000000000);
        ...
        antiWhaleUsdtLimit1 = 5000000000000000000000; // 5000 USDT
        antiWhaleUsdtLimit2 = 10000000000000000000000; // 10000 USDT
        antiWhaleUsdtLimit3 = 15000000000000000000000; // 15000 USDT
        ...
        dexRouter = ISWTMultiNetworkRouter(currentRouter);
        ...
        usdtAddress = 0x57C5c0a1ccAC1A28f9B165d0DdF42feaa0796C25; // USDT traderjoe main
    ...
    function updateMarketAddress(address add) public virtual onlyOwner {
        marketAddress = add;
        marketplace = IMarket(marketAddress);
        excludeFromFee(add);
        approve(add, MAX_INT);
    }

    function updateHaveAssetBeforeSellCheck(bool value)
    public
    virtual
    onlyOwner
    {
        haveAssetBeforeSellCheck = value;
    }
    ...
    function _beforeTransferCheck(
        ...
        // user should have assets before sell
        if (haveAssetBeforeSellCheck && automatedMarketMakerPairs[to]) {
            require(marketplace.countOfAssetUser(from) > 0,
                "Insufficient assets count!"
            );
        }
    ...
    returns (uint256)
    {
        ...
        if (antiWhaleUsdtLimit1 > 0) {
            ...
            uint256 usdtAmount = getAmountOutUSD(amount)[1];
...
interface ISWTMultiNetworkRouter is IUniswapV2Router02, IJoeRouter02 {
    // ROUTER V1
    function factory()
        external
        pure
...
function getOutEstimatedTokensForTokens(
    ISWTMultiNetworkRouter dexRouter,
...
interface IMarket {
    function countOfAssetUser(address account) external view returns(uint32);
    function countOfPackUser(address account) external view returns(uint32);
}
