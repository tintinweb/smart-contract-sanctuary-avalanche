 ...
 * @title NFTLoan
 */
contract NFTLoan is NFTLoanGovernance, SigningUtils {
    ...
    function registerLoanSameChain(
        NFTLoanStructs.BorrowerDetail memory borrowerDetails,
        NFTLoanStructs.LenderDetail memory lenderDetails
    ) external {
        // signature reuse check
        require(
            !isSignatureConsumed(lenderDetails.lenderSignature),
            "Signature already used"
        );

        // checking expiry of borrower's signature
        require(
            block.timestamp < lenderDetails.lenderExpiry,
            "Lender's Signature Expired"
        );

        // @todo optimize params passing in the functions by making _truncateAddress function commoned out
        // verifying signature's validity
        require(
            isValidLenderSignature(
                lenderDetails,
                _truncateAddress(lenderDetails.loanErc20Token),
                _truncateAddress(lenderDetails.lender),
                _truncateAddress(borrowerDetails.nftCollateralContract),
                borrowerDetails.nftCollateralId,
                chainId()
            ),
            "Lender signature is invalid"
        );
        // marking signature as used to prevetn reuse
        setSignatureConsumed(lenderDetails.lenderSignature);

        NFTLoanStructs.Offer memory offer = NFTLoanStructs.Offer(
            lenderDetails.loanPrincipalAmount,
            lenderDetails.loanRepaymentAmount,
            chainId(),
            borrowerDetails.nftCollateralId,
            borrowerDetails.nftCollateralContract,
            lenderDetails.loanDuration,
            lenderDetails.loanErc20Token,
            lenderDetails.loanAdminFeeInBasisPoints,
            lenderDetails.lenderChainId,
            lenderDetails.lender
        );
        registerLoan(
            offer,
            borrowerDetails.borrowerSignature,
            borrowerDetails.borrowerExpiry,
            borrowerDetails.borrowerNonce,
            0 // as same chain loan will not use this value
        ); // @todo ask from Yasir bhai about nonce
    }
    ...
    function registerLoan(
        NFTLoanStructs.Offer memory offer,
        bytes memory borrowerSignature,
        uint borrowerExpiry,
        uint32 borrowerNonce,
        uint32 nonce
    ) public returns (uint) {
        // signature reuse check
        require(
            !isSignatureConsumed(borrowerSignature),
            "Signature already used"
        );

        // checking expiry of borrower's signature
        require(block.timestamp < borrowerExpiry, "Borrower Signature Expired");

        // verifying signature's validity

        require(
            isValidBorrowerSignature(
                _truncateAddress(offer.nftCollateralContract),
        ...
        // marking signature as used to prevent reuse
        setSignatureConsumed(borrowerSignature);
        ...
        require(
            IERC721(collateralNFT).ownerOf(offer.nftCollateralId) == msg.sender,
            "Invalid NFT owner"
        );
        ...
        require(
            offer.loanRepaymentAmount >= offer.loanPrincipalAmount,
            "Invalid repayment amount"
        );

        // here now we will start for loan
        return _createLoan(offer, nonce);
    } // end of loan.

    /**
     * In case of cross chain loan. We do get vaa from wormhole and then call this complete create loan function.
     */
    function completeCreateLoan(bytes memory _encodedVAA) external {
        IWormhole wormhole = wormhole();

        (IWormhole.VM memory vm, bool valid, string memory reason) = wormhole
            .parseAndVerifyVM(_encodedVAA);

        require(valid, reason);

        require(verifyEmitterVM(vm), "invalid emitter");

        require(!isMessageConsumed(vm.hash), "Loan already registered");

        setMessageConsumed(vm.hash);

        NFTLoanStructs.Loan memory loan = decodeLoan(vm.payload);

        require(loan.loanChainId == chainId(), "Invalid Laon chain");

        require(
            loan.status == NFTLoanStructs.LoanStatusType.ACTIVE,
            "Loan either repaid or Liquidated"
        );

        setLoan(loan.loanId, loan.nftChainId, loan);

        address tokenContract = _truncateAddress(loan.loanERC20Contract);
        address lender = _truncateAddress(loan.lender);
        address borrower = _truncateAddress(loan.borrower);
        uint adminFee = computeAdminFee(
            loan.loanRepaymentAmount - loan.loanPrincipalAmount,
            loan.loanAdminFeeInBasisPoints
        );
        // transferring tokens to lender with interest
        _transferTokens(tokenContract, adminFee, lender, address(this));
        // transferring to borrower after deducting fees
        _transferTokens(
            tokenContract,
            loan.loanPrincipalAmount - adminFee,
            lender,
            borrower
        );
        emit LoanCreated(loan.loanId);
    } // end of comleteCreateLoan
    ...
    function repayLoan(
        ...
        _repayLoan(loanId, otherChainId, nonce);
    }

    /**
     * In case of cross chain loan repayment. We do get vaa from wormhole and then call this complete repay loan function.
     */
    function completeRepayLoan(bytes memory _encodedVAA) external {
        IWormhole wormhole = wormhole();
        ...
        require(loan.nftChainId == chainId(), "Invalid NFT chain");
        require(
            loan.status == NFTLoanStructs.LoanStatusType.REPAID,
            "Loan Not repaid or Liquidated"
        );

        loan.status = NFTLoanStructs.LoanStatusType.REPAID;
        setLoan(loan.loanId, loan.loanChainId, loan);
        ...
        _transferNFT(
            ...
            address(this),
            _truncateAddress(loan.borrower)
        );
        emit LoanRepayed(loan.loanId);
    } // end of completeRepayLoan
    ...
    function liquidateLoan(
        ...
        _liquidateLoan(loanId, otherChainId, nonce);
    }

    /**
     *
     * In case of cross chain loan liquidation. We do get vaa from wormhole and then call this complete liquidate loan function.
     */
    function completeLiquidateLoan(bytes memory _encodedVAA) external {
        IWormhole wormhole = wormhole();
        ...
        require(
            ...
            "Loan either not active or repaid"
        );
        ...
        _transferNFT(
            ...
            loan.nftCollateralId,
            address(this),
            _truncateAddress(loan.lender)
        );
        emit LoanLiquidated(loan.loanId);
    } // end of completeLiquidateLoan
} // end of class
...
contract NFTLoanGetters is NFTLoanState {
    ...
    function isSignatureConsumed(
        bytes memory _signature
    ) public view returns (bool) {
        return state.consumedSignatures[_signature];
    }
    ...
    function getLoan(
        uint256 loanId,
        uint16 otherChainId
    ) public view returns (NFTLoanStructs.Loan memory) {
        return state.loans[loanId][otherChainId];
    }

    function verifyEmitterVM(
        IWormhole.VM memory vm
    ) internal view returns (bool) {
        return
            state.emitterImplementations[vm.emitterChainId] ==
            vm.emitterAddress;
    }
} // end of class
...
contract NFTLoanHelper is NFTLoanGetters, NFTLoanSetters {
    ...
    function _transferNFT(
        ...
        IERC721(nftContract).transferFrom(_from, _recipient, nftCollateralId);
    }
    ...
    function _transferTokens(
        ...
        IERC20(tokenContract).transferFrom(_from, _recipient, amount);
    }
    ...
    function _createLoan(
        ...
        setLoan(loan.loanId, offer.offerChainId, loan);

        if (offer.offerChainId == chainId()) {
            // it means offer is on same chain.
            address tokenContract = _truncateAddress(offer.loanERC20Contract);
            require(tokenContract != address(0), "Invalid ERC20 token");
            ...
            return
                wormhole().publishMessage{value: 0}(
                    nonce,
                    encodeLoan(loan),
                    finality()
                );
    ...
    function _repayLoan(
        ...
        NFTLoanStructs.Loan memory loan = getLoan(loainId, otherChainId);
        ...
        IERC20(tokenContract).transferFrom(
            msg.sender,
            lender,
            loan.loanRepaymentAmount // @todo ask if whether to deduct fees from lender side too?
        );
        loan.status = NFTLoanStructs.LoanStatusType.REPAID;

        setLoan(loainId, otherChainId, loan);
    ...
    function _liquidateLoan(
        ...
        NFTLoanStructs.Loan memory loan = getLoan(loanId, otherChainId);
        require(loan.loanChainId == chainId(), "Invalid chain");
        require(
            loan.status == NFTLoanStructs.LoanStatusType.ACTIVE,
            "Loan is not active"
        ...
        loan.status = NFTLoanStructs.LoanStatusType.LIQUIDATED;
        setLoan(loanId, otherChainId, loan);
        if (loan.nftChainId == chainId()) {
            // it means offer is on same chain.
            _transferNFT(
                loan.nftCollateralContract,
    ...
    function initializeLoan(
        NFTLoanStructs.Offer memory offer
    ) internal view returns (NFTLoanStructs.Loan memory loan) {
    ...
    function encodeLoan(
        NFTLoanStructs.Loan memory loan
    ...
    ) public pure returns (NFTLoanStructs.Loan memory loan) {
        uint index = 0;
        loan.loanId = encoded.toUint256(index);
        index += 32;

        loan.nftCollateralId = encoded.toUint256(index);
        index += 32;

        loan.nftCollateralContract = encoded.toBytes32(index);
        index += 32;

        loan.nftChainId = encoded.toUint16(index);
        index += 2;

        loan.loanPrincipalAmount = encoded.toUint256(index);
        index += 32;

        loan.loanRepaymentAmount = encoded.toUint256(index);
        index += 32;

        loan.loanERC20Contract = encoded.toBytes32(index);
        index += 32;

        loan.loanStartTime = encoded.toUint256(index);
        index += 32;

        loan.loanDuration = encoded.toUint32(index);
        index += 4;

        loan.loanChainId = encoded.toUint16(index);
        index += 2;

        loan.loanAdminFeeInBasisPoints = encoded.toUint16(index);
        index += 2;

        loan.lender = encoded.toBytes32(index);
        index += 32;

        loan.borrower = encoded.toBytes32(index);
        index += 32;

        loan.status = NFTLoanStructs.LoanStatusType(encoded.toUint8(index));
    }
}
...
contract NFTLoanSetters is NFTLoanState, AccessControl {
    ...
    function _setWormholeContractAddress(
        address payable _wormholeContractAddress
    ) internal {
    ...
    function setSignatureConsumed(bytes memory _signature) internal {
        state.consumedSignatures[_signature] = true;
    }
    ...
    function setLoan(
        ...
        state.loans[loanId][otherChainId] = loan;
    }
...
contract NFTLoanStorage {
    ...
    struct State {
        ...
        uint256 nextOrderId;
        Provider provider;
        // Mapping of loans agaisnt other chain : loanId -> otherChainId -> Loan
        mapping(uint256 => mapping(uint16 => NFTLoanStructs.Loan)) loans;
        ...
        mapping(bytes => bool) consumedSignatures; // to prevent signature reuse
    }
}
...
contract NFTLoanStructs {
    ...
    // struct ERC20Details {
    //     uint loanPrincipalAmount;
    //     address loanERC20Contract;
    // }
    ...
    struct LenderDetail {
        bytes32 loanErc20Token;
        uint loanPrincipalAmount;
        uint loanRepaymentAmount;
        uint16 loanAdminFeeInBasisPoints;
        uint32 loanDuration;
        uint16 lenderChainId;
        uint32 lenderNonce;
        uint lenderExpiry;
        bytes32 lender;
...
// @title  Helper contract for NFTLoan. This contract manages verifying signatures
//         from off-chain Lendup orders.
contract SigningUtils {
    ...
    function isValidBorrowerSignature(
        ...
        uint32 _borrowerNonce,
        uint _borrowerExpiry,
        address _borrower,
        uint16 _chainId,
    ...
    function isValidLenderSignature(
        NFTLoanStructs.LenderDetail memory _lenderDetail,
        address _loanERC20Token,
        address _lender,
        address _nftCollateralContract,
        uint _nftCollateralId,
        uint16 _chainId
    ) public pure returns (bool) {
        if (_lender == address(0)) {
            ...
            bytes32 message = keccak256(
                abi.encodePacked(
                    _loanERC20Token,
                    _lenderDetail.loanPrincipalAmount,
                    _lenderDetail.loanRepaymentAmount,
                    _lenderDetail.loanAdminFeeInBasisPoints,
                    _lenderDetail.loanDuration,
                    _chainId, // as loan is same chain so lender's must have signed with this chainId
                    _lenderDetail.lenderNonce,
                    _lenderDetail.lenderExpiry,
                    _nftCollateralId,
                    _nftCollateralContract,
                    _lender
                )
            ...
            return (messageWithEthSignPrefix.recover(
                _lenderDetail.lenderSignature
            ) == _lender);
        }
    }
}
