...
contract nft_stake {
    ...
    mapping(address => Greens) public greens;
    ...
    function calc() public view returns (uint[3] memory) {
        uint256 time = block.timestamp;
        uint va;
    	uint256[3] memory nums = [va,va,va];
    	uint256 num;
	va = 0;
	num = stakes_1[msg.sender].amount;
	    	for (uint i=0; i < num; i++){
			if ((stakingTime_1[msg.sender][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[0] = va;
		...
		num = stakes_2[msg.sender].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_2[msg.sender][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[1] = va;
		...
		num = stakes_3[msg.sender].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_3[msg.sender][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[2] = va;		
	...
	uint256 time = block.timestamp;
        ...
        uint256[3] memory nar = calc();
        greens[msg.sender].green1 = nar[0];
        greens[msg.sender].green2 = nar[1];
        greens[msg.sender].green3 = nar[2];
