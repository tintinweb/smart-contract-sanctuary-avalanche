...
contract AaveVault is OwnableUpgradeable, PausableUpgradeable, BaseBuildingBlock {
    ...
    * @param aaveProvider_ aave Pool provider address
    * @param wethGateway_ weth network gateway
    * @param rewardsController_ rewards controller address
    */
    function initialize(
        address actionPoolDcRouter_,
    ...
    */
    ...
    /**
     * @notice Sets a an asset already deposit as collateral for a future borrow
     ...
     * @param _data depositor, baseAsset, amount
     */
    ...
    function openPosition(bytes memory _data) external whenNotPaused{
        ...
        // aaveLendingPool.deposit(baseAsset, amount, address(this), 0); // deprecated
     ...
     * @param _data amount Amount of native tokens to deposit
     */
    function submitNativeToken(bytes memory _data) external payable {
        // TODO integrate AAVE WETH GATEWAY
    }
     ...
     */
    function withdrawNativeToken(bytes memory _data) external payable {
    ...
    function borrow(bytes memory _data) external {
        (address borrowAsset, uint256 amount, uint16 borrowRate) = abi.decode(_data,(address,uint256,uint16));
        aaveLendingPool.borrow(borrowAsset, amount, borrowRate, 0, address(this));
    ...
    function repay(bytes memory _data) external {
        (address baseAsset, uint256 amount,uint16 borrowRate) = abi.decode(_data,(address,uint256,uint16));
        IERC20(baseAsset).approve(address(aaveLendingPool), amount);
        aaveLendingPool.repay(baseAsset, amount,  borrowRate, address(this));
    ...
    **/
    function claimAllRewardsOnBehalf(
        ...
        (rewardsList,claimedAmounts) = rewardsController.claimAllRewardsOnBehalf(assets,user,to);
    }

/* VIEW METHODS */

    /**
    * @return weth Native WETH of chain
    */
    function getWETH() public returns(address weth){
        weth = wethGateway.WETH();
    }
    ...
    **/
    function getRewardsData(address asset, address reward) external view returns (
        uint256, uint256, uint256, uint256 ){
            return rewardsController.getRewardsData(asset,reward);
    }
    ...
    **/
    function getUserAccruedRewards(address user, address reward) external view returns (uint256){
        return rewardsController.getUserAccruedRewards(user,reward);
    }
    ...
    */
    function getUserEMode(address user) external view returns (uint256){
            return aaveLendingPool.getUserEMode(user);
    }
...
abstract contract BaseBuildingBlock is SmartLZBase{
    ...
    address public _actionPoolDcRouter;
