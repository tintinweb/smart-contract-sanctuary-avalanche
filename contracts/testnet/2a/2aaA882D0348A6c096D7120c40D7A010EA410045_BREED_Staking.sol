...
contract BREED_Staking is Ownable{
    ...
    mapping(address => mapping(uint256 => uint256)) public stakingTime; // adress > id > time
    ...
    function rewardOfUser(address Add, uint256 Tid) public view returns(uint256) {
        ...
        for(uint256 i = 0 ; i < Tokenid[Add].length ; i++){
            ...
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*72 ether;     
            }
            if(Breed.check_type(Tid) == 2 && Tokenid[Add][i] == Tid)
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*145 ether;     
            }
            if(Breed.check_type(Tid) == 3 && Tokenid[Add][i] == Tid)
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*362 ether;     
            }
            if(Breed.check_type(Tid) == 4 && Tokenid[Add][i] == Tid)
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*725 ether;     
            }
            if(Breed.check_type(Tid) == 5 && Tokenid[Add][i] == Tid)
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*1449 ether;     
            }
            if(Breed.check_type(Tid) == 6 && Tokenid[Add][i] == Tid)
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*7246 ether;     
            }
        }
    return RewardToken;
    }
    ...
    function WithdrawReward(uint256 Tid)  internal {
       ...
       stakingTime[msg.sender][Tid] = block.timestamp;  // updating time for single token id _Sarmad
    ...
    function WithdrawRewardAll()  public {
       ...
       // // updating time for all token ids
       for(uint8 i=0; i<Tokenid[msg.sender].length; i++){
        // mapping address > tokenId > time
