...
contract BridgesOfFate is Ownable  {
    ...
    event ParticipateOfPlayerInGame(bytes32 playerId,uint256 stageGlobal,uint256 randomNumber, bool safeSide);
    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules {
        ...
        if((_member.stage  == STAGES)){
            ...
            if((_jumpSide == true && GameMap[_currentUserStage] >= 50e9) || (_jumpSide == false && GameMap[_currentUserStage] < 50e9)){
                _isWinner =  true;
        ...
        emit ParticipateOfPlayerInGame(playerId,_gameStatus.stageNumber,GameMap[_gameStatus.stageNumber],_checkSide(GameMap[_gameStatus.stageNumber],_jumpSide));
        PlayerItem[playerId] = _member;
        GameStatusInitialized[latestGameId] = _gameStatus;
        emit comment(_isWinner); 

    }
    event comment (bool isWinner);
