...
contract Presale is Initializable {
    ...
    function initialize(address token_addr) public initializer {
       ...
       limitperwallet = 10000000000000000000000;
       presalePrice = 1000000000000;
       paused = false;
    }
    ...
    function calculateMeltForWT(uint256 amount) public view returns (uint256) {
        return (presalePrice.mul(amount));
    }

    function Buy(uint256 _amount) public payable whenNotPaused onlyWhitelisted
    {
        require(limit[msg.sender].add(_amount)<=limitperwallet,"Limit exceeded");
        require(whitelist[msg.sender], "You are not Whitelist!" );
        uint256 amount = calculateMeltForWT(_amount);
    ...
    function WithdrawMelt(uint256 amount) public onlyowner {
