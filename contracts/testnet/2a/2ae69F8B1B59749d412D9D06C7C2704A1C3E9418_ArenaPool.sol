...
contract ArenaPool is Pausable {
    ...
    struct UserInfo {
        ...
        uint256 unlockTimestamp; // withdraw unlock timestamp.
    }
    ...
    struct PoolInfo {
        ...
        uint256 supply; // Amount of LP token deposited for the pool.
    }

    uint256[4] public lockPeriods = [0, 7 days, 30 days, 90 days];
    ...
    function checkPoolDuplicate(IERC20 _token, uint8 _lockMode) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token || poolInfo[pid].lockMode != _lockMode, "ArenaPool: existing pool?");
        }
    }
    ...
    function add(
        ...
        checkPoolDuplicate(_token, _lockMode);
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_sender];
        require(user.unlockTimestamp <= block.timestamp, "withdraw: locked");
    ...
    function _depositFor(
        ...
        pool.supply = pool.supply.add(depositAmount);
        user.unlockTimestamp = block.timestamp + lockPeriods[pool.lockMode];
