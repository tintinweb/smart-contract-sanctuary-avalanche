...
contract ClockAuctionBase {
    ...
    modifier onSale(address contractAddr, uint256 tokenId) {
        require(
            tokenIdToAuctions[contractAddr][tokenId].startedAt > 0,
            "Not On Sale"
        );
        _;
    }
    ...
    modifier onlySeller(address contractAddr, uint256 tokenId) {
        require(
            tokenIdToAuctions[contractAddr][tokenId].seller == msg.sender,
    ...
    function _purchase(
        ...
        if (price > 0) {
            ...
            payable(charityAddr).transfer(price / 100);
            payable(teamAddr).transfer(price / 50);
            ...
            for (i = 0; i < length; ++i) {
                payable(royalty[contractAddr][i].destination).transfer(
                    (price * royalty[contractAddr][i].profit) / 10000
                );
            }
            for (i = 0 ; i < 10000 ; ++i) {
                rewards[seller] += 10;
    ...
    function _bid(
        ...
        _purchase(contractAddr, _tokenId, auction.seller, _bidAmount, price);
    ...
    function _removeAuction(address contractAddr, uint256 _tokenId) internal {
        ...
        for (
            i = 0;
            saleTokenIdsBySeller[auction.seller][contractAddr][i] != _tokenId;
            ++i
        ) {}
    ...
    function _createOffer(
        ...
        offers[contractAddr][tokenId].push(Offer(offerer, price));
...
interface MintRewardOfTokenClaimable {
    function claimMintRewardOfToken(uint256 tokenId) external payable;

    function getMintRewardsOfToken(uint256 tokenId)
        external
        view
        returns (uint256);
}

interface MintRewardsClaimable {
    function claimMintRewards() external payable;
...
interface SalesRewardOfTokenClaimable {
    function claimSalesRewardOfToken(uint256 tokenId) external payable;

    function getSalesRewardsOfToken(uint256 tokenId)
        external
        view
        returns (uint256);
}

interface SalesRewardsClaimable {
    function claimSalesRewards() external payable;
...
interface RewardOfTokenClaimable {
    function claimRewardOfToken(uint256 tokenId) external payable;

    function getRewardsOfToken(uint256 toeknId) external view returns (uint256);
}

interface RewardsClaimable {
    function claimRewards() external payable;

    function getRewardsOfAddress(address _address)
        external
        view
        returns (uint256);
}
...
contract ClockAuction is Pausable, ClockAuctionBase, Ownable {
        ...
        onlyBuyer(contractAddr, tokenId)
    {
        ...
        uint256 price = _bid(contractAddr, tokenId, msg.value);
        _transfer(contractAddr, msg.sender, tokenId);
        // Tell the world!
        emit AuctionSuccessful(contractAddr, tokenId, price, msg.sender);
    }
    ...
    function getCurrentPrice(address contractAddr, uint256 _tokenId)
        ...
        returns (uint256)
    {
        return _currentPrice(tokenIdToAuctions[contractAddr][_tokenId]);
    }
    ...
    {
        require(
            msg.value <= tokenIdToAuctions[contractAddr][tokenId].startingPrice,
            "Make an offer in current price range"
        );
        uint256 offerLength = offers[contractAddr][tokenId].length;
        require(
            offerLength == 0 ||
                offers[contractAddr][tokenId][offerLength - 1].price <
                msg.value,
            "You should create offer on higher price"
        );
    ...
    function claimMintRewardOfToken(address contractAddr, uint256 tokenId)
        external
        payable
    {
        MintRewardOfTokenClaimable(contractAddr).claimMintRewardOfToken(
            tokenId
        );
    }

    function getMintRewardsOfToken(address contractAddr, uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return
            MintRewardOfTokenClaimable(contractAddr).getMintRewardsOfToken(
                tokenId
            );
    }

    function claimMintRewards(address contractAddr) external payable {
        MintRewardsClaimable(contractAddr).claimMintRewards();
    }

    function getMintRewardsOfAddress(address contractAddr, address _address)
        external
        view
        returns (uint256)
    {
        return
            MintRewardsClaimable(contractAddr).getMintRewardsOfAddress(
                _address
            );
    }

    function claimSalesRewardOfToken(address contractAddr, uint256 tokenId)
        external
        payable
    {
        SalesRewardOfTokenClaimable(contractAddr).claimSalesRewardOfToken(
            tokenId
        );
    }

    function getSalesRewardsOfToken(address contractAddr, uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return
            SalesRewardOfTokenClaimable(contractAddr).getSalesRewardsOfToken(
                tokenId
            );
    }

    function claimSalesRewards(address contractAddr) external payable {
        SalesRewardsClaimable(contractAddr).claimSalesRewards();
    }

    function getSalesRewardsOfAddress(address contractAddr, address _address)
        external
        view
        returns (uint256)
    {
        return
            SalesRewardsClaimable(contractAddr).getSalesRewardsOfAddress(
                _address
            );
    }

    function claimRewardOfToken(address contractAddr, uint256 tokenId)
        external
        payable
    {
        RewardOfTokenClaimable(contractAddr).claimRewardOfToken(tokenId);
    }

    function getRewardsOfToken(address contractAddr, uint256 tokenId)
        external
        view
        returns (uint256)
    {
        return RewardOfTokenClaimable(contractAddr).getRewardsOfToken(tokenId);
    }

    function claimRewards(address contractAddr) external payable {
        RewardsClaimable(contractAddr).claimRewards();
    }

    function getRewardsOfAddress(address contractAddr, address _address)
        external
        view
        returns (uint256)
    {
        return RewardsClaimable(contractAddr).getRewardsOfAddress(_address);
    }
}