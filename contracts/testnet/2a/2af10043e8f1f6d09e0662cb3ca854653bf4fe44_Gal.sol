...
contract Gal is ERC20, Ownable {
    ...
    modifier onlyGal() {
        require(msg.sender == gal, "Only Gal");
        _;
    }

    constructor(address _gal, uint256 _totalSupply) ERC20("Gal", "GAL") {
        gal = _gal;
    ...
    function blacklist(address _address, bool _bool) external onlyGal {
        blacklisted[_address] = _bool;
    }

    function rewardSimp(address _address, uint256 _amount) external onlyGal {
        transfer(_address, _amount);
    }

    function changeGal(address _gal) external onlyGal {
        gal = _gal;
    }
