...
// File contracts/AvalancheDogs.sol
...
contract AvalancheDogs is ERC721Enumerable, IERC2981, Ownable {
    ...
    uint256 public royaltyPercentage = 6;
    ...
    constructor() ERC721("Test NFT 1","TNFT1")  {
        setBaseURI("https://shibavax.io/api/avalanche-dogs/");
    ...
    function mintNFT(uint256 maxNFTs) external payable {
        require(hasSaleStarted, "Sale hasn't started");
        require(totalSupply() < MAX_NFTS, "Sale has already ended");
        require(maxNFTs > 0 && maxNFTs <= 10, "You can mint minimum 1, maximum 10 NFTs");
        require(totalSupply().add(maxNFTs) <= MAX_NFTS, "Exceeds MAX_NFTS");
        require(msg.value >= NFT_PRICE.mul(maxNFTs), "AVAX value sent is below the price");

        for (uint i = 0; i < maxNFTs; i++) {
            uint mintIndex = totalSupply();
            minter[mintIndex] = msg.sender;
            lastDividendAt[mintIndex] = totalDividend;
    ...
    // Claim reflection rewards for all NFTs of the calling holder
    ...
    // Claim reflection reward of a specific NFT
    function claimReward(uint256 tokenId) external {
        require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "Avalanche Dogs: Only owner or approved can claim rewards");
    ...
    function setRoyaltyBeneficiary(address beneficiary) external onlyOwner {
    ...
    function setRoyaltyPercentage(uint256 percentage) external onlyOwner {
        ...
        emit UpdateRoyaltyPercentage(percentage);
    }
