/**
 *Submitted for verification at testnet.snowtrace.io on 2022-09-11
*/
...
/// @title Hypatia NFT Smart Contract
/// @author www.github.com/21kkfy
/// @notice Explain to an end user what this does
contract Origin is ERC721, OwnableNR, ReentrancyGuard {
    ...
    uint256 public constant MAX_SUPPLY_WHITELIST = MAX_SUPPLY;
    uint256 public constant MAX_WHITELIST_MINT = 5;
    uint256 public constant MAX_PUBLIC_MINT = 2;
    uint256 public constant MAX_WHITELIST_WALLETS = 999;
    ...
    mapping(uint256 => uint256) private starAmount;
    mapping(uint256 => uint256) public starAmountDEBUG;
    ...
    modifier callerIsUser() {
        require(
            ...
            "<CONTRACT-NAME> :: Cannot be called by a contract"
        );
        _;
    }
    ...
    modifier notPaused() {
        require(!pause, "<CONTRACT-NAME> :: Contract is paused.");
        _;
    }

    // Mainnet JSON:
    constructor() ERC721("<CONTRACT-NAME>", "<CONTRACT-SYMBOL>") {
        debugAddress = address(314);
    }
    ...
    function teamMint() external nonReentrant onlyOwnerAdmin {
        require(!teamMinted, "<CONTRACT-NAME> :: Team already minted.");
        teamMinted = true;
        for (uint256 i = 0; i < 5; i++) {
            _tokenIdCounter.increment();
            uint256 tokenId = _tokenIdCounter.current();
            _safeMint(msg.sender, tokenId);
            /* require(pairStarOfToken(tokenId)); */
            pairStarOfToken(tokenId);
        }
    }
    ...
    function whitelistFreeMint(
        ...
        uint256 _quantity,
        address _tokenIdMerkle
    ) external nonReentrant callerIsUser notPaused {
        require(
            whiteListSale,
            "<CONTRACT-NAME> :: White-list minting is on pause"
        ); /* 
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY_WHITELIST,
            "<CONTRACT-NAME> :: Cannot mint beyond max supply"
        ); */
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
            "<CONTRACT-NAME> :: Cannot mint beyond whitelist max mint!"
        );
        require(
            isEarlyMinter(_tokenIdMerkle, _merkleProof),
            "<CONTRACT-NAME> :: You are not whitelisted."
        );
    ...
    function _withdraw(address wallet, uint256 amount) private {
        ...
        require(success, "<CONTRACT-NAME>: Transfer failed.");
    }
    ...
    /***********************
     * WHITELIST & STAR FUNCTIONS *
     ***********************/
    // Merkle Root and Proof WL
    bytes32 public merkleRootWL;
    bytes32[] public proofWL;
    // Merkle 1 Star and Proof
    bytes32 public merkleRoot1S;
    bytes32[] public proof1S;
    // Merkle 2 Star and Proof
    bytes32 public merkleRoot2S;
    bytes32[] public proof2S;
    // Merkle 3 Star and Proof
    bytes32 public merkleRoot3S;
    bytes32[] public proof3S;
    // Merkle 4 Star and Proof
    bytes32 public merkleRoot4S;
    bytes32[] public proof4S;
    // Merkle 5 Star and Proof
    bytes32 public merkleRoot5S;
    bytes32[] public proof5S;
    bytes32 public debugLeaf;
    ...
    function isEarlyMinter(
        ...
        return MerkleProof.verifyCalldata(_proof, merkleRootWL, leaf);
    }

    event pairedStar(uint256 tokenId, uint256 starAmount);

    function pairStarOfToken(uint256 _tokenId) internal returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_tokenId));
        debugLeaf = leaf;
        if (MerkleProof.verify(proof1S, merkleRoot1S, leaf)) {
            starAmount[_tokenId] = 1;
            starAmountDEBUG[_tokenId] = 1;
            emit pairedStar(_tokenId, 1);
            return true;
        } else if (MerkleProof.verify(proof2S, merkleRoot2S, leaf)) {
            starAmount[_tokenId] = 2;
            starAmountDEBUG[_tokenId] = 2;
            emit pairedStar(_tokenId, 2);
            return true;
        } else if (MerkleProof.verify(proof3S, merkleRoot3S, leaf)) {
            starAmount[_tokenId] = 3;
            starAmountDEBUG[_tokenId] = 3;
            emit pairedStar(_tokenId, 3);
            return true;
        } else if (MerkleProof.verify(proof4S, merkleRoot4S, leaf)) {
            starAmount[_tokenId] = 4;
            starAmountDEBUG[_tokenId] = 4;
            emit pairedStar(_tokenId, 4);
            return true;
        } else if (MerkleProof.verify(proof5S, merkleRoot5S, leaf)) {
            starAmount[_tokenId] = 5;
            starAmountDEBUG[_tokenId] = 5;
            emit pairedStar(_tokenId, 5);
            return true;
        }
        return false;
    }

    bool public debugBool;
    address public debugAddress;

    function debugMerkleProof(
        bytes32[] calldata _proof,
        bytes32 _root,
        address _tokenId) public pure returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(_tokenId));
        //debugLeaf = _leaf;
        //debugBool = MerkleProof.verify(_proof, _root, _leaf);
        //debugAddress = address(1234);
    ...
    function setMerkleRootStars(
        ...
        merkleRoot1S = _root1S;
        merkleRoot2S = _root2S;
        merkleRoot3S = _root3S;
        merkleRoot4S = _root4S;
        merkleRoot5S = _root5S;
    }

    function setMerkleProofStars(
        bytes32[] calldata _proof1S,
        bytes32[] calldata _proof2S,
        bytes32[] calldata _proof3S,
        bytes32[] calldata _proof4S,
        bytes32[] calldata _proof5S
    ) external onlyOwner {
        proof1S = _proof1S;
        proof2S = _proof2S;
        proof3S = _proof3S;
