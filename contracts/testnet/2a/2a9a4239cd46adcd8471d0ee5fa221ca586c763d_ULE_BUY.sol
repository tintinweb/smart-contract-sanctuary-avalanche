...
contract ULE_BUY is Ownable {
    ...
    uint256 public PoolPercentage = 70;
    uint256 public count;
    uint256 public SwapandLiquifyCount = 10;
    address public LpReceiver;
    constructor(IERC20 _ULE_, IPangolinRouter _Router,address _LpReceiver_)
    {
       ULE = _ULE_;
       Router = _Router;
       WETH = Router.WAVAX();
        ...
        ) public payable
    {
        ...
        uint256[] memory returnValues = swapExactAVAXForToken(half,address(ULE));
    ...
    function swapExactAVAXForToken(uint256 value, address token) public payable  returns (uint[] memory amounts )  
    {
        address[] memory path = new address[](2);
        path[0] = Router.WAVAX();
        path[1] = token;
        return Router.swapExactAVAXForTokens{value:value}(
        0, 
        path,
        address(this), 
        block.timestamp
    ...
    {
        Router.addLiquidityAVAX{value:half}(
            address(ULE),
            _amount,
            0,
            0,
