...
{
    event Create(address indexed _to, uint256[] indexed tokenId);
    ...
    function minting(
        ...
        for (uint256 i = 0; i < ipfsmetadata.length; i++) {
            ...
            _royal[id] = royal[i];
            if (price[i] > 0) {
                orderPlace(id, price[i], address(this), "mint");
            }
            tokenId[i] = id;
        }
    ...
    function saleWithToken(
        ...
        if (balanceOf(from, ids[0]).sub(ids[2]) == 0) {
            if (order_place[from][ids[0]].price > 0) {
                delete order_place[from][ids[0]];
            }
        }
        safeTransferFrom(from, msg.sender, ids[0], ids[2], "");
    }
    ...
    function acceptBId(
        ...
        if (balanceOf(msg.sender, ids[0]).sub(ids[2]) == 0) {
            if (order_place[msg.sender][ids[0]].price > 0) {
                delete order_place[msg.sender][ids[0]];
            }
        }
        safeTransferFrom(msg.sender, bidaddr, ids[0], ids[2], "");
    }
