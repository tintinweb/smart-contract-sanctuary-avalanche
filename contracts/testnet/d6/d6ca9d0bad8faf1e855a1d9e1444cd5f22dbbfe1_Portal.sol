...
// File contracts/Portal/utils/DataStoreLib.sol
...
// File contracts/Portal/utils/GeodeUtilsLib.sol
...
library GeodeUtils {
   ...
   * * This change can be giving the control of an id to a user, a new implementation contract, a new Senate etc.
   *
   ...
   * @param _electorTypes only given types can vote @dev must only be used at upgrades @dev can not change a type's voting right status after adding an id to that type
...
// File contracts/interfaces/IgAVAX.sol
...
// File contracts/WithdrawalPool/LPToken.sol
...
// File contracts/Portal/utils/StakeUtilsLib.sol
...
library StakeUtils {
  ...
  uint256 constant ORACLE_DAILY_PERIOD = 1 days;
  ...
  function _isOracleActive(
    ...
    else if (block.timestamp % ORACLE_DAILY_PERIOD <= ORACLE_ACTIVE_PERIOD)
      return true;
    else return false;
  }
   ...
   * @dev checks:
   * 1. Price should be increased & it should not be increased more than DAILY_PRICE_INCREASE_LIMIT
   * 2. All given operators are still allowed to gather fees, means activationExpiration is not ended
   */
  function _sanityCheck(
    ...
    uint256 maxPrice = curPrice +
      (curPrice * self.DAILY_PRICE_INCREASE_LIMIT) /
   ...
   * @param _opIds all ids of all operators who sill collect fees.
  ...
  function reportOracle(
    ...
    _sanityCheck(self, _DATASTORE, _poolId, price, _opIds);
  ...
  function payDebt(
    ...
    uint256 _poolId,
    uint256 _operatorId
  ) public beforeActivationExpiration(_DATASTORE, _poolId, _operatorId) {
    require(
      !_isOracleActive(_DATASTORE, _poolId),
      "StakeUtils: Oracle is active"
    );

    uint256 debt = withdrawalPoolById(_DATASTORE, _poolId).getDebt();
    require(debt > 0, "StakeUtils: no debt found");

    if (debt > msg.value) {
      debt = msg.value;
    }
    // burns
    _buyback(self, _DATASTORE, address(0), _poolId, debt, 0, type(uint256).max);

    if (msg.value > debt) {
      _DATASTORE.writeUintForId(
        _poolId,
        "surplus",
        _DATASTORE.readUintForId(_poolId, "surplus") + (msg.value - debt)
      );
    }

    if (_DATASTORE.readUintForId(_poolId, "pBalance") > debt) {
      _DATASTORE.writeUintForId(
        _poolId,
        "pBalance",
        _DATASTORE.readUintForId(_poolId, "pBalance") - debt
      );
    } else {
      _DATASTORE.writeUintForId(_poolId, "pBalance", 0);
    }
    emit debtPaid(_poolId, _operatorId, debt);
  }
  ...
  function pausePool(DataStoreUtils.DataStore storage _DATASTORE, uint256 _id)
    public
  {
    _DATASTORE.writeBytesForId(_id, "paused", bytes("1")); // meaning true, importantly length > 0
    emit PausedPool(_id);
  }

  function unpausePool(DataStoreUtils.DataStore storage _DATASTORE, uint256 _id)
    public
  {
    _DATASTORE.writeBytesForId(_id, "paused", bytes("")); // meaning false, importantly length = 0
  ...
  function deployWithdrawalPool(
    ...
    address _LPToken = ISwap(WithdrawalPool).initialize(
      ...
      string(
        abi.encodePacked(_DATASTORE.readBytesForId(_id, "name"), "_GeodeLP")
      ),
      self.DEFAULT_A,
      self.DEFAULT_FEE,
  ...
  function stake(
    ...
    require(
      _DATASTORE.readBytesForId(poolId, "paused").length == 0,
...
// File contracts/interfaces/IERC20InterfaceUpgradable.sol
...
// File contracts/Portal/Portal.sol
  ...
  UUPSUpgradeable
{
  ...
  function initialize(
    ...
    STAKEPOOL.DAILY_PRICE_INCREASE_LIMIT = (5 * GEODE.FEE_DENOMINATOR) / 1e3; // 0.5%
  ...
  modifier onlyMaintainer(uint256 _id) {
    require(
  ...
  function setPlanetInterface(
    ...
    address _Interface,
    bool isSet
  ) external virtual override whenNotPaused onlyMaintainer(_id) {
  ...
  /**
   * @notice An Operator is expected to pay for the DEBT of a staking pool, but anyone can call this func
  ...
  function claimSurplus(uint256 planetId)
    ...
    whenNotPaused
    nonReentrant
    onlyMaintainer(DATASTORE.readUintForId(planetId, "activeOperator"))
    returns (bool success)
  {
  ...
  /// @notice when a pool is paused there are NO new funds to be minted, NO surplus.
  function pausePool(uint256 _id)
    external
    virtual
    override
    onlyMaintainer(_id)
  {
    StakeUtils.pausePool(DATASTORE, _id);
  }

  function unpausePool(uint256 _id)
    external
    virtual
    override
    onlyMaintainer(_id)
  {
    StakeUtils.unpausePool(DATASTORE, _id);
  }
