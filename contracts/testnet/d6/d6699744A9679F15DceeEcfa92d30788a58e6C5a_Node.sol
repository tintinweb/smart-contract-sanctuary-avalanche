...
contract Node is ERC721, ERC721Enumerable, Owners {
	...
	modifier onlyHandler() {
		require(msg.sender == handler, "Node: God mode not activated");
		_;
	}

	// external
	function burnBatch(address user, uint[] calldata tokenIds) external onlyHandler {
		for (uint i = 0; i < tokenIds.length; i++) {
			require(ownerOf(tokenIds[i]) == user, "Node: Not nft owner");
	...
	function generateNfts(
		...
		returns(uint[] memory)
	{
		require(!isBlacklisted[user], "Node: Blacklisted address");
		require(openCreateNft, "Node: Not open");
	...
	// external setters
	function setTokenIdToType(uint tokenId, string calldata nodeType) external onlyHandler {	
		...
		override
	{
		require(!isBlacklisted[from] && !isBlacklisted[to], 
			"Node: Blacklisted address");

		if (onlyAuthorized) {
			require(!_isContract(from) || isAuthorized[from],
				"Node: Unauthorized contract");
