...
contract MadVeggies is
    ...
    uint256 public  invitorsfraction = 0.16 ether;
    uint256 public _maxSupply = 1515;
    uint256 public maxTx = 10;
    uint256 private contractRoyalties = 300;
    uint256 public reflectionBalance;
    uint256 public totalDividend;
    uint256 private _juicePrice = 90 ether;
    uint256 public mintLimit;
    uint256 indexerLength = 5554;
    ...
    constructor(
        ...
        setBaseURI(_initBaseURI);
         juice_ADDRESS = IERC20(juiceAddress);
    ...
    function setIndexer(uint256 _new) external onlyOwner {
        indexerLength = _new;
    }
      ...
      function enoughRandom() internal view returns (uint256) {
        uint256 minted = _tokenIdTracker.current();
        uint256 randomNumber = s_randomWords[0];
        if (_maxSupply - minted == 0) return 0;
        return
            uint256(
                keccak256(
                    abi.encodePacked(
      ...
      function _mintAmount(uint amount) private {

      require(_tokenIdTracker.current() + amount <= _maxSupply, "Mad Veggies: not enough Mad Cucumbers left to mint");
      require(_tokenIdTracker.current() + amount <= _maxSupply, "Max Mint amount been reached.");
      for (uint i = 0; i < amount; i++) {
          uint256 tokenId = enoughRandom();
          _mint(msg.sender, tokenId);
          _setTokenRoyalty(tokenId, _owner, contractRoyalties);
        ...
        function toggleJuiceMint(bool _new) public onlyOwner{
            live = _new;
        }

        function mintWithjuice(uint amount) public {
            require(live, "minting with JUICE is paused" );
            uint256 juiceCost = amount * _juicePrice;
            juice_ADDRESS.approve(address(this),juiceCost);
            require(juice_ADDRESS.allowance(_msgSender(), address(this)) >= juiceCost, "Mad Veggies: not enough juice approved");
            require(juice_ADDRESS.balanceOf(_msgSender()) >= juiceCost, "MadVeggies: not enough juice in account");
            juice_ADDRESS.transferFrom(_msgSender(), _owner, juiceCost);
             _mintAmount(amount);
    ...
    function airdrop(address to, uint amount) external onlyOwner {
        ...
        require(_tokenIdTracker.current() + amount <= _maxSupply, "Not enough avax mad veggies left to mint amount.");     
         for(uint i=0; i < amount; i++){
          uint256 tokenId = enoughRandom();
         _mint(to , tokenId);
         _setTokenRoyalty(_tokenIdTracker.current(), _owner, contractRoyalties);
         minter[tokenId] = to;
         lastDividendAt[tokenId] = totalDividend;
         _tokenIdTracker.increment();
        }
    ...
    function setCost(uint256 _newCost) public onlyOwner {
     ...
     function setJuiceCost(uint256 _newCost) public onlyOwner {
        _juicePrice = _newCost;
    }
    ...
    function claimReward(uint256 tokenId) public {
        require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "Mad Veggies: Only owner or approved can claim rewards");
