...
contract JoePair is JoeERC20 {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        console.log("JoePair.mint - feeOn",feeOn);
        uint256 _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can update in _mintFee
        console.log("JoePair.mint - _totalSupply",_totalSupply);
        if (_totalSupply == 0) {
            address migrator = IJoeFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                console.log("JoePair.mint - migrator - liquidity",liquidity);
                require(
                ...
                console.log("JoePair.mint - NOT migrator - liquidity",liquidity);
                console.log("JoePair.mint - NOT migrator - MINIMUM_LIQUIDITY",MINIMUM_LIQUIDITY);
        ...
        console.log("JoePair.mint - liquidity",liquidity);
        require(liquidity > 0, "Joe: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);

        _update(balance0, balance1, _reserve0, _reserve1);
        if (feeOn) kLast = uint256(reserve0).mul(reserve1); // reserve0 and reserve1 are up-to-date
        console.log("JoePair.mint - Mint amount0",amount0);
        console.log("JoePair.mint - Mint amount1",amount1);
...
contract JoeFactory is IJoeFactory {
        ...
        returns (address pair)
    {
        ...
        string memory newString = bytesToString(bytecode);
