...
/*
* @title ERC1155 token for IK NFT's
*
* @author hammm.eth
*/
...
contract IKTEST is AbstractERC1155Factory, NonblockingReceiver, ILayerZeroUserApplicationConfig, PaymentSplitter {
    ...
    event Claimed(uint indexed tokenID, address indexed account, uint numberIdClaimed);

    address[] private addresses = [0x71665B951a9EB87c8e6A8EF29a266fA578a78654];
    ...
    constructor(
    ...
    ) ERC1155("https://www.infinitykeys.io") PaymentSplitter(addresses, splits) {
        name_ = _name;
        symbol_ = _symbol;
        setSecret(_secret);
        setSigner(_signer);
        ...
        addToken(true, false, 0, address(0), "www.hello0.com");
        addToken(true, false, 0, address(0), "www.hello1.com");
    }

    /**
    * @notice adds a new Token
    * 
    * @param _claimable bool whether claim is open for tokenID
    * @param _gated is token gated
    * @param _gatedId tokenId needed for gate
    * @param _gatedAddress address for NFT gating
    * @param _tokenURI URI for specific tokenID
    */
    function addToken(
        bool _claimable,
        bool _gated,
        uint256 _gatedId,
        address _gatedAddress,
    ...
    /**
    * @notice edits existing token
    * 
    * @param _claimable bool whether claim is open for tokenID
    * @param _gated is acheivement NFT gated
    * @param _gatedId tokenId needed for gate
    * @param _gatedAddress address for NFT gating
    * @param _tokenURI URI for specific tokenID
    * @param _tokenId tokenID to edit
    */
    function editToken(
        bool _claimable,
        bool _gated,
    ...
    /**
    * @notice airdrop token to address
    * 
    * @param _tokenID the token id to mint
    * @param _address address to airdrop too 
    */
    function mintTo ( uint256 _tokenID, address _address ) external onlyOwner {
        require( exists(_tokenID), "MintTo: token does not exist" );

        _mint(_address, _tokenID, 1, "");
    }

    /**
    * @notice returns whether or not minting is closed for given token Id
    *
    * @param _tokenID the token id to mint
    */
    ...
    /**
    * @notice claim token tokens
    * 
    * @param _tokenID the token id to purchase
    * @param _signature hash of vital information signed by signer address
    */
    function claim ( uint256 _tokenID, bytes memory _signature ) external payable whenNotPaused {
        ...
        if (tokens[_tokenID].gated) {
            if (tokens[_tokenID].gatedAddress != address(0)) {
                IGateNFT gateNFT = IGateNFT(tokens[_tokenID].gatedAddress);
                require( gateNFT.balanceOf(msg.sender) > 0, "Claim: Must own gated NFT to claim" );
            } else {
                require(balanceOf(msg.sender, tokens[_tokenID].gatedId) > 0, "Claim: Must own gated token to claim");
            }
        }
        ...
        emit Claimed(_tokenID, msg.sender, totalSupply(_tokenID));
    }

    /**
    * @notice check if _address has claimed _tokenId
    * 
    * @param _tokenID the token id check
    * @param _address address to check if claimed
    */
    ...
    /**
    * @notice return total supply for all existing tokens
    */
    ...
    */
    ...
    /**
    * @notice check server verification to prevent contract mints
    * 
    * @param _tokenID the token id to check
    * @param _signature hash of vital information signed by signer address
    */
    function verify( uint256 _tokenID, bytes memory _signature ) private view returns ( bool ) {
    ...
    /**
    * @notice return signer address from signature message
    * 
    * @param _tokenID the token id to check
    * @param _signature hash of vital information signed by signer address
    */
    ...
    /**
    * @notice create hash of information
    * 
    * @param _tokenID to hash
    */
    ...
    /**
    * @notice set the secret used in hash
    * 
    * @param _secret secret used in hash 
    */
    ...
    /**
    * @notice set the signer used to sign the message
    * 
    * @param _signer set the signer to check address of message
    */
    ...
    function traverseChains(
        ...
        require(balanceOf(msg.sender, tokenId) > 0, "Message sender must own the OmnichainNFT.");
...
abstract contract AbstractERC1155Factory is ERC1155Pausable, ERC1155Supply, Ownable {
