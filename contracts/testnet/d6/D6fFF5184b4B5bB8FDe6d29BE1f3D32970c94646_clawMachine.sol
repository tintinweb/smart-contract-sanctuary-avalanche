...
contract clawMachine is ConfirmedOwner, VRF {
    ...
    function setPrizes(address[3] calldata prizes) external onlyOwner{
        contract1 = prize(prizes[0]);
        contract2 = prize(prizes[1]);
        contract3 = prize(prizes[2]);
    }
    ...
    function internalMint(address to) internal {
        require(
            //contract1.totalSupply() < contract1.maxMintable() ||
            //contract2.totalSupply() < contract2.maxMintable() ||
            //contract3.totalSupply() < contract3.maxMintable() ||
            !avaSharkWon ||
            avaxWon < 100,
            "no more prizes"
        );
        uint256 randomNum;
        randomNum = (getRandomness() % 100) + 1;
        if (
            randomNum <= 20 //&& contract1.totalSupply() < contract1.maxMintable()
        ) {
            //contract1.mint(to);
            emit win(1, to);
        } else if (
            ...
            //contract3.mint(to);
            emit win(3, to);
        } else if (60 < randomNum && randomNum <= 80 && !avaSharkWon) {
            //IERC721(avashark).safeTransferFrom(owner(),to,avasharkToken);
            emit win(4, to);
        } else if (80 < randomNum && randomNum <= 100 && avaxWon < 100) {
