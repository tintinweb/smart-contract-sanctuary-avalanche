...
contract Fountain is Initializable, BEP20, Whitelist {
    using SafeMath for uint256;

    string public constant name = "Melt Liquidity Token";
    ...
    function tokenToUsdtInput(uint256 tokens_sold, uint256 min_usdt, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        (uint256 realized_sold, uint256 taxAmount) = token.calculateTransferTaxes(buyer, address(this), tokens_sold);
    ...
    function addLiquidity(uint256 min_liquidity, uint256 max_tokens, uint256 usdt_amount) isNotPaused public returns (uint256) {
        require(max_tokens > 0 && usdt_amount > 0, "Swap#addLiquidity: INVALID_ARGUMENT");
        uint256 total_liquidity = _totalSupply;
        uint256 token_amount = 0;
        ...
        if (total_liquidity > 0) {
            ...
            uint256 usdt_reserve = usdt.balanceOf(address(this)).sub(usdt_amount);
            uint256 token_reserve = token.balanceOf(address(this));
            token_amount = (usdt_amount.mul(token_reserve) / usdt_reserve).add(1);
            uint256 liquidity_minted = usdt_amount.mul(total_liquidity) / usdt_reserve;
            ...
            require(usdt.transferFrom(msg.sender, address(this), usdt_amount),"usdt transfrom error");
            emit onAddLiquidity(msg.sender, usdt_amount, token_amount);
            emit onLiquidity(msg.sender, _balances[msg.sender]);
            emit Transfer(address(0), msg.sender, liquidity_minted);
            return liquidity_minted;
        } else {
            require(usdt_amount >= 1e18, "INVALID_VALUE");
    ...
    function removeLiquidity(uint256 amount, uint256 min_usdt, uint256 min_tokens) onlyWhitelisted public returns (uint256, uint256) {
        ...
        require(usdt.transfer(msg.sender, usdt_amount), "transfer err");
