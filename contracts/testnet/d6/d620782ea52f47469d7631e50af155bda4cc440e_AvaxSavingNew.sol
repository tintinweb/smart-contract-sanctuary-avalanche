...
contract AvaxSavingNew is ReentrancyGuard, Context {
    ...
    function newStake(address referrer, uint8 plan)
        public
        payable
        nonReentrant
    {
    ...
    function claimReward() public nonReentrant {
        ...
        if (stakeRewardCommision > 0) {
            ...
            (level0, level1, level2) = getUserDownlineAddress(msg.sender);
            for (uint256 i = 0; i < level0.length; i++) {
                for (uint256 k = 0; k < users[level0[i]].deposits.length; k++) {
                    if (
                        users[level0[i]].checkpoint <
                        users[level0[i]].deposits[k].finish
                    ) {
                        if (
                            users[level0[i]].deposits[i].status == true &&
                            users[level0[i]].deposits[i].claimedStatus ==
                            false &&
                            users[level0[i]].deposits[i].scrClaimed == false
                        ) {
                            users[level0[i]].deposits[i].scrClaimed = true;
                        }
                    }
                }
            }
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            ...
            if (user.checkpoint < user.deposits[i].finish) {
                if (
                    ...
                    user.deposits[i].claimedStatus == false &&
                    user.deposits[i].scrClaimed == false
                ) {
                    uint256 share = user
                        .deposits[i]
                        .amount
        ...
        )
    {
        ...
        // for (uint256 i = 0; i < level0.length; i++) {
        //     uint256 depositCount = getUserDepositsCount(level0[i]);
        ...
        //         ) = getUserDepositInfo(userAddress, j);
        //         level0SC = level0SC.add(
        //             amount.mul(percent).div(PERCENTS_DIVIDER)
        //         );
        //     }
        // }
        ...
        for (uint256 i = 0; i < level0.length; i++) {
            ...
            level0SC = getUserSCFromDeposit(level0[i], 0);
        }
        for (uint256 i = 0; i < level1.length; i++) {
            level1SRC = level1SRC.add(
                getUserStakeRewardCommision(level1[i])
                    .mul(STAKE_REWARD_COMMISSION_PERCENTS[1])
                    .div(PERCENTS_DIVIDER)
            );

            level1TES = level1TES.add(getUserTotalExestingStake(level1[i]));
            level1SC = getUserSCFromDeposit(level0[i], 1);
            // uint256 depositCount = getUserDepositsCount(level1[i]);
            // for (uint256 j = 0; j < depositCount; j++) {
            //     (
            //         ,
            //         ,
        ...
        for (uint256 i = 0; i < level2.length; i++) {
            level2SRC = level2SRC.add(
                getUserStakeRewardCommision(level2[i])
                    .mul(STAKE_REWARD_COMMISSION_PERCENTS[2])
                    .div(PERCENTS_DIVIDER)
            );

            level2TES = level2TES.add(getUserTotalExestingStake(level2[i]));
            level2SC = getUserSCFromDeposit(level0[i], 2);
            // uint256 depositCount = getUserDepositsCount(level2[i]);
            // for (uint256 j = 0; j < depositCount; j++) {
            //     (
            //         ,
            //         ,
...
NEW OLD
        ...
        )
    {
        ...
        for (uint256 i = 0; i < level0.length; i++) {
            ...
            level0SC = level0SC.add(getUserStakeCommision(level0[i]));
            level0TES = level0TES.add(getUserTotalExestingStake(level0[i]));
        }
        for (uint256 i = 0; i < level1.length; i++) {
            level1SRC = level1SRC.add(
            ...
            level1SC = level1SC.add(getUserStakeCommision(level1[i]));
            level1TES = level1TES.add(getUserTotalExestingStake(level1[i]));
        }
        for (uint256 i = 0; i < level2.length; i++) {
            level2SRC = level2SRC.add(
            ...
            level2SC = level2SC.add(getUserStakeCommision(level2[i]));
            level2TES = level2TES.add(getUserTotalExestingStake(level2[i]));
        }
    }
*/
            ...
            for (
                ...
                if (i == 0) {
                    level0StakedAmount = level0StakedAmount.add(
                        getUserTotalExestingStake(
                            users[userAddress].downline[0][k]
                        )
                    );
                    level0StakeComission = level0StakeComission.add(
                        getUserStakeCommision(users[userAddress].downline[0][k])
                    );

                    uint256 scrNew;
                    // getUserStakeRewardCommision <== getUserAllStakeRewardCommissions
                    scrNew = getUserStakeRewardCommision(
                        users[userAddress].downline[0][k]
                    );
                ...
                if (i == 1) {
                    level1StakedAmount = level1StakedAmount.add(
                        getUserTotalExestingStake(
                            users[userAddress].downline[1][k]
                        )
                    );
                    level1StakeComission = level1StakeComission.add(
                        getUserStakeCommision(users[userAddress].downline[1][k])
                    );
                ...
                if (i == 2) {
                    level2StakedAmount = level2StakedAmount.add(
                        getUserTotalExestingStake(
                            users[userAddress].downline[2][k]
                        )
                    );
                    level2StakeComission = level2StakeComission.add(
                        getUserStakeCommision(users[userAddress].downline[2][k])
                    );
