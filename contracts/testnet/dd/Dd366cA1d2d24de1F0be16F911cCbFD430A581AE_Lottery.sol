...
contract Lottery is Ownable {
    ...
    uint256 roundId = 0;
    uint256 roundStatus = 0; // 0 = pending, 1 = active
    uint256 roundReward = 0; // total round reward in AVAX (distributed amoung all winners)
    uint256 roundNoOfPicks = 5; // number of picks
    uint16 minNumber = 1; // min random number
    uint16 maxNumber = 9; // max random number

    uint256 ticketId = 0; // current ticket id, each address can have multiple tickets
    uint256 ticketPrice = 5 ether; // in shady tokens
    ...
    function startRound(uint256 _roundReward) public onlyOwner {
    ...
    function submitTicket(uint8[] calldata _numbers) public {
        ...
        require(_numbers.length == roundNoOfPicks, 'Wrong number of numbers.');

        // each minter allowed to submit as many free tickets as mints done
        if (roundAddressTickets[roundId][msg.sender].length >= nft_minter.minter_balanceOf(msg.sender)) {
            require(
