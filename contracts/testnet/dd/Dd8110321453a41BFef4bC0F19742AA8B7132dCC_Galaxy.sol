...
/**    PLUTUS NODE FINANCE - 2022
       Web:      https://linktr.ee/plutusnode
 */
        ...
        for (uint256 i = 0; i < total; i++) {
            if (i == 0) {
                _nodesOfUser[account].push(
                    NodeEntity({
                        name: nodeName,
                        creationTime: block.timestamp + i + 1,
...
contract Galaxy is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256[] memory fees,
        uint256 swapAmount
    ) ERC20("Galaxy Node", "GXY") PaymentSplitter(payees, shares) {
    ...
    function createNodeWithTokens(string memory name, uint256 total) public {
        ...
        require(totalUserNodes.add(total) <= 100, "Maximum number of nodes reached");
    ...
    function compoundMultipleNodes(
        ...
        address sender = _msgSender();
        require(blocktimes.length <= numberOfLoops, "Maximum of 10 nodes");
        uint256 totalUserNodes = nodeRewardManager.getUserNodes(sender);
        ...
        while (total >= nodePrice && count <= numberOfLoops) {
            totalUserNodes++;
            total -= nodePrice;
            if(totalUserNodes < 100){
                count++;
            }
        }
        nodeRewardManager.createNode(sender, name, count);
        if (total > 0) {
            nodeRewardManager.creditLastNode(sender, total);
        }
    }
}