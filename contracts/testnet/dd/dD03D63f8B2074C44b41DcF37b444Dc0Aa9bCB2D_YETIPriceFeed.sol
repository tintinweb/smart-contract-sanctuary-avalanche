...
contract YETIPriceFeed is IPriceFeed {
    ...
    function fetchPrice_v() view external override returns (uint256) {
        (uint112 reserve0, uint112 reserve1, uint32 timeStamp) = YETIYUSD.getReserves();
        uint256 price =  uint256(UQ112x112.encode(reserve1).uqdiv(reserve0));
        price = price.div(10**(17));
        return price;
    }
}
