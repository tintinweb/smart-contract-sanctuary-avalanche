...
contract NeFiFeeManager is Ownable{
	...
	uint256 public AVAXFee= 74*(10**12);
	...
	function payFeeTokenSpec(address _account, uint256 _intervals, uint256 _x) payable external onlyGuard{
	   ...
	   //checkFeeTokenSend(_account,total,_x);
	...
	function payFeeToken(address _account,uint256 _intervals, uint256 _x) payable external onlyGuard{
	    uint256 total = nebuLib.doMultiple(_intervals,tokFee);
	    //checkFeeTokenSend(_account,total,101);
	    feeTok.transferFrom(_account, treasury, total);
	    updateFeesPaid(_account,_intervals,_x);
	    if(msg.value > 0){
		payable(_account).transfer(msg.value);
	...
	function payFeeAvax(address _account,uint256 _intervals,uint256 _amount, uint256 _x) payable external onlyGuard{
		uint256 _fee = over.getFee();
		(uint256 _total,uint256 returnBalance) =  nebuLib.getAllMultiple(msg.value,AVAXFee);
		updateFeesPaid(_account,_intervals,_x);
	...
	function INTupdateProtoTotalPayable(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoTotalPayable = nebuLib.getMultiple(feePeriod,nebuLib.getLower(INTgetProtoCollapseDate(_account,_x).sub(block.timestamp),maxFeePayment)).sub(protoowners[_account][_x].protoFutureFees);
	}
	...
	function INTupdateProtoFutureFees(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoFutureFees = nebuLib.getMultiple(feePeriod,INTgetProtoDueElapsed(_account,_x));
	}
	...
	function INTupdateOwed(address _account) internal {
		totalfees[address(this)].futureFees -= totalfees[_account].feesOwed;
