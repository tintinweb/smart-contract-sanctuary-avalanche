...
contract NFTBankFactory is AccessControl {
    ...
    event NewPublicMintCapWasSet(
        uint256 newPublicMintCap,
        uint256 oldPublicMintCap
    );
    ...
    function setPublicMintCap(uint256 _publicMintCap)
        external
        onlyRole(ADMIN_ROLE)
    {
        uint256 oldPublicMintCap = publicMintCap;
        publicMintCap = _publicMintCap;
        emit NewPublicMintCapWasSet(_publicMintCap, oldPublicMintCap);
    }
}
...
contract NFTBankProxy is NFTBankStorageStructure {
    ...
    function initialize(
        ...
        publicMintCap = _variables[1];
        nftImportExpirationTime = _variables[2];
        launchpadFeePercentage = _variables[3];
