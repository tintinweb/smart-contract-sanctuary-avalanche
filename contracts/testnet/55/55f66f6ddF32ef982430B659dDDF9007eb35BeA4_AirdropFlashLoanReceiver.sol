...
import "../bnft/IBNFT.sol";
import "../bnft/IBNFTRegistry.sol";
...
contract AirdropFlashLoanReceiver is
    IFlashLoanReceiver,
    ...
    ERC1155HolderUpgradeable
{
    ...
    function executeOperation(
        ...
        address bnftProxy = IBNFTRegistry(bnftRegistry).getBNFTAddresses(
            nftAsset
        );
        ...
        for (
            ...
            if (vars.airdropTokenTypes[typeIndex] == 1) {
                // ERC20
                vars.airdropBalance = IERC20Upgradeable(
                    vars.airdropTokenAddresses[typeIndex]
                ).balanceOf(address(this));
                if (vars.airdropBalance > 0) {
                    IERC20Upgradeable(vars.airdropTokenAddresses[typeIndex])
                        .transfer(initiator, vars.airdropBalance);
                }
            } else if (vars.airdropTokenTypes[typeIndex] == 2) {
                // ERC721 with Enumerate
                vars.airdropBalance = IERC721Upgradeable(
                    vars.airdropTokenAddresses[typeIndex]
                ).balanceOf(address(this));
                for (uint256 i = 0; i < vars.airdropBalance; i++) {
                    vars.airdropTokenId = IERC721EnumerableUpgradeable(
                        vars.airdropTokenAddresses[typeIndex]
                ...
                // ERC1155
                vars.airdropBalance = IERC1155Upgradeable(
                    vars.airdropTokenAddresses[typeIndex]
                ).balanceOf(address(this), vars.airdropTokenIds[typeIndex]);
                IERC1155Upgradeable(vars.airdropTokenAddresses[typeIndex])
                    .safeTransferFrom(
                        address(this),
                        initiator,
