...
interface INodeManager {
    function getNodesAmount(address _account) external view returns (uint256);
    function getNodesRewards(address _account, uint256 k) external view returns (uint256);
    function cashoutNodeReward(address _account, uint256 k) external;
...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function cashoutAll() external {
        ...
        uint length = nodeManager.getNodesAmount(sender);
        uint256 rewards;
        for(uint i=0;i<length;i++){
        	rewards += nodeManager.getNodesRewards(sender,i);
        }
        uint256[2] memory rew  = takeFee(rewards,_cashoutFee);
