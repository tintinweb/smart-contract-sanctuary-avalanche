...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface, IRaffleResults, IPrizeMatrix {
    ...
    event CheckCouponError(string coupon);
    ...
    function enterRaffle(string memory coupon, string memory partnerID) public payable {
        ...
        if (bytes(coupon).length > 0) {
            try couponManager.getCoupon(address(this), keccak256(abi.encodePacked(coupon)))
            returns (uint8 minPct, uint8 maxPct, uint16 multiplierPct) {
