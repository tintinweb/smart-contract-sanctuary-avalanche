...
contract BridgesOfFate is OwnableUpgradeable {
    ...
    uint256 private constant _ownerRewarsdsPercent = 15;
    ...
    {
        return  uint256(keccak256(abi.encodePacked(block.timestamp + block.gaslimit + ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.number)) +
        ...
        uint256 _randomNumber = gameRandomNumber[_currentUserStage];
           if (_randomNumber <= 0) {
            ...
            if(_seed > 0){
                ...
                _seed = (_seed - ((_seed / 100) * 100));
                pioneerPushBackGameMap[_currentGlobalStage] = _seed;
            }
