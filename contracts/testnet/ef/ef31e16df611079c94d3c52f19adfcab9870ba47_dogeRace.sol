...
// File: DogeMoon/AVAX-new/AVAX-DogeRaceTest.sol
...
contract dogeRace is Ownable, VRFConsumerBaseV2 {
    ...
    */
    ...
    address internal linkAddress = 0x0b9d5D9136855f6FEc3c0993feE6E9CE8a297846;
    address internal vrfCoordinator = 0x2eD832Ba664535e5886b75D64C46EB9a228C2610;
    bytes32 internal keyHash = 0x354d2f95da55398f44b7cff77da56283d9c6c829a4bdf1bbcaf2ad6a4d081f61;
    uint256 public linkThreshold = 1 ether;
    ...
    event RaceCreation(uint256 raceId);
    ...
    uint256 public speedTraitId;
    uint256 public enduranceTraitId;
    uint256 public luckTraitId;
    ...
    constructor(address _dogeAddress, address _biofiAddress, address payable _devWallet, uint256 _entryFee, address _dogeTraitsAddress, uint256 _speedTraitId, uint256 _enduranceTraitId, uint256 _luckTraitId) VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        LINKTOKEN = LinkTokenInterface(linkAddress);

        dogeAddress = _dogeAddress;
        biofiAddress = _biofiAddress;
    ...
    function createRace(uint256 dogeId) public returns(bool success) {
        require(active == true, "Racing is currently disabled");
        require(isDogeBusy[dogeId] == false, "This doge is already participating in a race.");
        require(MoonDoge(dogeAddress).ownerOf(dogeId) == msg.sender, "You are not the owner of this doge.");

        // pull biofi from doge owner
        ...
        emit RaceCreation(raceCounter);
        emit RaceJoined(raceCounter, dogeId);

        return true;
    }
    
    function enterRace(uint256 raceId, uint256 dogeId) public returns(bool success) {
        require(active == true, "Racing is currently disabled");
        require(isDogeBusy[dogeId] == false, "This doge is already participating in a race.");
        require(MoonDoge(dogeAddress).ownerOf(dogeId) == msg.sender, "You are not the owner of this doge.");
        require(races[raceId].creationTime > 0, "This race does not exist.");
        require(races[raceId].isFinished == false, "This race has finished.");
        require(races[raceId].isCancelled == false, "This race was cancelled.");

        
        // pull biofi from doge owner
        
        for(uint256 i = 0; i < 9; i++) {
            if(races[raceCounter].isSlotTaken[i] == false) {
                races[raceCounter].participants[i] = dogeId;
                races[raceCounter].isSlotTaken[i] = true;
                races[raceCounter].participantCount++;
                ...
                if(races[raceCounter].participantCount == 9) {
                    
                    raceIdByRequestId[raceId] = raceId;
                    
                // emit Request(currentRequestId);
                    emit RaceStart(raceId, raceId);
                }
    ...
    function finishRace(uint256 requestId, uint256[] memory randomNumbers) private {
        ...
        for(uint256 i = 0; i < 9; i++) {
            ...
            /*
            bytes2 traits = MoonDoge(dogeAddress).tokenTraits(races[raceId].participants[i]);
            bytes1 breed = traits[0];

            uint256 speedSeed = TraitStorage(dogeTraitsAddress).traitValues(speedTraitId, races[raceId].participants[i]);
            uint256 enduranceSeed = TraitStorage(dogeTraitsAddress).traitValues(enduranceTraitId, races[raceId].participants[i]);
            uint256 luckSeed = TraitStorage(dogeTraitsAddress).traitValues(luckTraitId, races[raceId].participants[i]);
    ...
    function setTraitSettings(uint256 dogeBreedId, uint256 _speedMin, uint256 _speedMax, uint256 _enduranceMin, uint256 _enduranceMax, uint256 _luckMin, uint256 _luckMax) public onlyOwner {
        TraitSettings memory traitSettings = TraitSettings(true, _speedMin, _speedMax, _enduranceMin, _enduranceMax, _luckMin, _luckMax);

        traitSettingsPerDoge[dogeBreedId] = traitSettings;
    }

    function deleteTraitSettings(uint256 dogeBreedId) public onlyOwner {
        TraitSettings memory traitSettings = TraitSettings(false, 0, 0, 0, 0, 0, 0);
    ...
    function getDogeTraits(uint256 dogeBreedId, uint256 speedSeed, uint256 enduranceSeed, uint256 luckSeed) public view returns(DogeTraits memory) {
        require(traitSettingsPerDoge[dogeBreedId].exists == true, "This breed id is not set up.");
    ...
    function adminUpdateSubscriptionSettings(uint32 _callbackGasLimit, uint16 _requestConfirmations) external onlyOwner {
        callbackGasLimit = _callbackGasLimit;
        requestConfirmations = _requestConfirmations;
    }

    function adminSetLinkThreshold(uint256 newThreshold) external onlyOwner {
        linkThreshold = newThreshold;
    }

    function adminSetEntryFee(uint256 newEntryFee) external onlyOwner {
    ...
    function fulfillRandomWords(
        ...
        require(raceIdByRequestId[requestId] != 0, "The requestId is invalid.");
