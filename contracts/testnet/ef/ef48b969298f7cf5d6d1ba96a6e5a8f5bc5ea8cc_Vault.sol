...
contract Router is OwnableUpgradeable {
    ...
    function swapYetiVaultToken(
        ...
        if (_tokenIn == _receiptToken) {
            // Swap _receiptToken for _tokenIn, aka withdraw() that amount. 
            return IYetiVaultToken(_receiptToken).withdraw(_amount); 
        } else {
...
contract Vault is ERC20Upgradeable, Router, ReentrancyGuard {
    using SafeTransferLib for IERC20;

    // Min swap to rid of edge cases with untracked rewards for small deposits. 
    uint256 constant public MIN_SWAP = 1e16;
    uint256 constant public MIN_FIRST_MINT = 1e16; // Require substantial first mint to prevent exploits from rounding errors
    uint256 constant public FIRST_DONATION = 1e12; // Lock in first donation to prevent exploits from rounding errors
    ...
    function depositFor(address _borrower, uint256 _amt) external nonReentrant returns (uint256 receiptTokens) {
        ...
        _triggerDepositAction(_amt);
        _mint(_borrower, receiptTokens);
        emit Deposit(msg.sender, _borrower, _amt, receiptTokens);
    }
    ...
    function redeem(address _to, uint256 _amt) public virtual nonReentrant returns (uint256 underlyingTokens) {
        require(_amt > 0, "0 tokens");
        if (block.timestamp > lastReinvestTime + maxReinvestStale) {
            _compound();
        }
        uint256 amtToReturn = (underlyingPerReceipt() * _amt) / 1e18;
    ...
    function redeemFor(
        ...
        address _from,
        address _to
    ) public nonReentrant returns (uint256 underlyingTokens) {
        require(_amt > 0, "0 tokens");
        if (block.timestamp > lastReinvestTime + maxReinvestStale) {
            _compound();
        }
        ...
        _burn(_from, _amt);
        SafeTransferLib.safeTransfer(underlying, _to, amtToReturn);
        emit Withdraw(msg.sender, _from, _to, amtToReturn, _amt);
    }
    ...
    function balanceOfUnderlying(address _owner) public view returns (uint256) {
        return (balanceOf[_owner] * underlyingPerReceipt()) / 1e18;
    }
    function withdraw(address _to, uint256 _value) public returns (uint256 _shares) {
        _shares = (_value * receiptPerUnderlying()) / 1e18;
        redeem(_to, _shares);
    }
