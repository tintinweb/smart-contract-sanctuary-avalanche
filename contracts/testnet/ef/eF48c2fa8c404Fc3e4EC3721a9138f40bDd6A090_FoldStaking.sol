...
contract FoldStaking is Ownable {
    ...
    /// @notice State for FoldToken, BAYC, MAYC, and Pair Pools
    struct Pool {
     ...
     */
    function depositLP(uint256 _amount, address _recipient) public {
       updatePool();

        // check valid LP token

       Position storage position = addressPosition[_recipient];
       _deposit(position, _amount); // amount should be 10x _amount

       //foldToken.transferFrom(msg.sender, address(this), _amount); - transfer LP token in.
       //set the balance of that LP token for that user
     ...
     * @param _recipient Address the deposit it stored to
     */
    function deposit(uint256 _amount, address _recipient) public {
         updatePool();
     ...
     */
    function withdrawLP(uint256 _amount, address _recipient) public {

        // make sure this is only for valid LP contracts.
        ...
        _withdraw(position, _amount); // amount should be 10x _amount

        //foldToken.transfer(_recipient, _amount); - send LP token.
        //set the balance of that LP token for that user - amount
