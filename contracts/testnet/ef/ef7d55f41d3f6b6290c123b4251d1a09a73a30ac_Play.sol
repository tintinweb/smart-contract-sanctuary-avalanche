...
contract RebaseTKN {
    ...
    uint public constant maxNo = type(uint).max;   // IMMUTABLE
...
contract Utils {
    ...
    */    
    function compare(string memory _choice) public pure returns(uint _result) {
...
contract Play is RebaseTKN, GMaster, Utils {
    ...
    */
    ...
    Returns the bet inforamtions from the user database. 
    */
    function gameStats(uint _gIndex) public returns(uint, uint, uint){
        uint _losTotAmt;
        uint _winTotAmt;
        uint _distrLos;
