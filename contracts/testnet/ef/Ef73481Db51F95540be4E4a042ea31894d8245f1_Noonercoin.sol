...
contract ERC20  {
    ...
    constructor(string memory name_, string memory symbol_, uint256 decimal_, uint256 totalSupply_) public {
        _name = name_;
        _symbol = symbol_;
        _decimal = decimal_;
...
contract Noonercoin is ERC20 {
   ...
   uint256 private lastMintingTime;
   ...
   uint256 public _cycleTime = 72576000;
   ...
   constructor(uint256 totalSupply_, string memory tokenName_, string memory tokenSymbol_,uint256 decimal_, uint256 mintingRateNoonerCoin_, uint256 frequency_, uint256 fundersAmount_) public ERC20(tokenName_, tokenSymbol_, decimal_, totalSupply_){
       _totalSupply = totalSupply_;
       _name = tokenName_;
   ...
   function mintToken(address add, uint256 time) public  returns (bool) {  //admin only
       require(msg.sender == adminAddress, "Only owner can do this");
       require(add == adminAddress, "Only owner can do this");
       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            lastMintingTime = time;//block.timestamp;
    ...
    function burnToken() internal returns(bool){
        ...
        for(uint256 index=1;index<=totalCycleLeft;index++){
            uint256 intValue = getIntegerValue(flag/720*604800, 150**index, index);
            uint256 intDecimalValue = getDecimalValue(flag/720*604800, 150**index, index);
  ...
  function getLastMintingTimeAndStartTimeDifference() public view returns (uint256) {
    ...
    function checkMissingTokens(address add) public view returns (uint256, uint256, uint256, bool) {
      ...
      if (lastMintingTime != 0) {
        ...
        if(weiVariance == adminBalanceinWei || adminBalanceinWei > weiVariance) {
            weiVariance = 0;
        }
