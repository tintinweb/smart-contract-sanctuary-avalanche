...
// File: contracts/interfaces/liquidityMining/IKyberFairLaunchV2.sol
...
// File: contracts/interfaces/liquidityMining/IKyberRewardLockerV2.sol
...
// File: contracts/liquidityMining/GeneratedToken.sol
...
// File: contracts/liquidityMining/KyberFairLaunchV2.sol
    ...
    ReentrancyGuard
{
    ...
    event UpdatePool(
        uint256 indexed pid,
        uint32 indexed endTime,
        uint32 indexed vestingDuration
    );
    event Deposit(
    ...
    function addPool(
        ...
        require(
            rewardTokens.length == _totalRewards.length,
            "add: invalid length"
        );

        require(
            _startTime > _getBlockTime() && _endTime > _startTime,
            "add: invalid times"
        );
    ...
    function updatePool(
        ...
        require(
            _endTime > _getBlockTime() && _endTime > pool.startTime,
    ...
    function _updateUserReward(
        ...
        for (uint256 i = 0; i < rTokensLength; i++) {
            ...
            if (shouldHarvest && _pending > 0) {
                ...
                _lockReward(
                    IERC20Ext(rewardTokens[i]),
                    _to,
                    _lockAmount,
                    poolInfo[_pid].vestingDuration
                );
                emit Harvest(
                    _to,
                    _pid,
     ...
     */
    function _lastAccountedRewardTime(uint256 _pid)
        internal
        view
    ...
    function _isSameVestingDuration(uint256[] calldata _pids)
        private
        view
        returns (bool)
    {
