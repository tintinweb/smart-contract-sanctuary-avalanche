...
contract IKTEST is AbstractERC1155Factory, NonblockingReceiver, ILayerZeroUserApplicationConfig, PaymentSplitter {
    ...
    */
    ...
    function traverseChains(
        ...
        require(balanceOf(msg.sender, tokenId) >= amount, "Message sender must own the OmnichainNFT.");
    ...
    function getEstimatedFees(uint16 _chainId, uint256 tokenId, uint256 amount) public view returns (uint) {
        bytes memory payload = abi.encode(msg.sender, tokenId, amount);
        uint16 version = 1;
        uint gas = 350000;
        bytes memory adapterParams = abi.encodePacked(version, gas);
    ...
    function _LzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override  {
        (address _dstAddress, uint256 tokenId, uint256 amount) = abi.decode(_payload, (address, uint256, uint256));
        _mint(_dstAddress, tokenId, amount, "");
    }
