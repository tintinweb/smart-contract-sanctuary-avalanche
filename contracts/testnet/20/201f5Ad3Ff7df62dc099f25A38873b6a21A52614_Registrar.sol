...
contract Registrar is IRegistrar, SynchronizerApplication, AccessControlUpgradeable, UUPSUpgradeable, Helper {
  ...
  function register(
    ...
    if (_registry.getTldClass(_tld) == TldClass.TldClass.OMNI && _registry.getTldChains(_tld).length > 0) {
      _requestSync(
        ...
        abi.encodeWithSignature("_register(bytes,bytes,address,uint64)", name, tld, owner, expiry)
      );
    }
  }
  ...
  function renew(
    ...
    bytes32 _tld = keccak256(tld);
    if (_registry.getTldClass(_tld) == TldClass.TldClass.OMNI && _registry.getTldChains(_tld).length > 0) {
      _requestSync(payable(_msgSender()), SyncAction.SyncAction.REGISTRAR, _registry.getTldChains(_tld), abi.encodeWithSignature("_renew(bytes,bytes,uint64)", name, tld, expiry));
    }
  }
