...
contract LoadPriceHandler is ReentrancyGuard, Ownable, ILoadPriceHandler {
    ...
    event HealingDollarsPerHPUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    event ResurrectionDollarsUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    event BardSongDollarsUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    event InnRestDollarsUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    event TokenDollarsRewardPerLevelUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    event DCARBondPriceUpdated(
        address indexed sender,
        uint256 indexed newPrice
    );
    ...
    function setTokenPricePerDollar(
        address tokenAddress,
        uint256 amountPerDollar
    ) public {
    ...
    function tokensPerDollar(address tokenAddress)
        public
        view
        returns (uint256)
    {
    ...
    function tokenCostDCARBond(address tokenAddress, uint256 discount)
        ...
        returns (uint256)
    {
        uint256 totalTokenCost = TokensPerDollar[tokenAddress] *
            DCAR_BOND_PRICE;
        uint256 discountInTokens = (((totalTokenCost * (1000 + discount)) /
            1000) - totalTokenCost);
    ...
    function totalCostHeal(uint256 amount) external view returns (uint256) {
        ...
        return
            (costInDollars * TokensPerDollar[DCAU_CONTRACT_ADDRESS]) /
            DOLLAR_DECIMALS;
    }
    ...
    function bardSongCost() external view returns (uint256) {
        return
            (BARD_SONG_DOLLARS * TokensPerDollar[DCAU_CONTRACT_ADDRESS]) /
            DOLLAR_DECIMALS;
    }
    ...
    function innRestCost(uint256 level) external view returns (uint256) {
        ...
        return
            ((INN_REST_DOLLARS + levelCostDollars) *
                TokensPerDollar[DCAU_CONTRACT_ADDRESS]) / DOLLAR_DECIMALS;
    }

    // amount in dollars is 4 decimal digits, 1 dollar is 10000, 50 cents is 5000, 5 cents is 500, 0.5 cents is 50, 0.005 cents is 5
    function costFromStableCost(address tokenAddress, uint256 amountInDollars)
        public
        view
        returns (uint256)
    {
        ...
        return
            (amountInDollars * TokensPerDollar[tokenAddress]) / DOLLAR_DECIMALS;
    }
}
