...
// File: contracts/shields/interfaces/IShields.sol
...
// File: contracts/shields/shieldManager.sol
...
contract ShieldManager is ERC1155, Ownable {
	event ShieldBuilt(
		uint256 tokenId,
		uint16 field,
		uint16 hardware,
	...
	mapping(address => bool) public earlyWhitelist;
	...
	modifier publicMintPaid() {
		require(msg.value == publicMintPrice, 'Shields: invalid mint fee');
		_;
	}

	modifier onlyTokenOwner(uint256 tokenId) {
		require(ERC1155.balanceOf(msg.sender, tokenId) != 0, 'Shields: only owner can build Shield');
		_;
	}
	...
	modifier onlyWhitelist(address to) {
		require(earlyWhitelist[to], 'Shields: Whitelist Only');
		_;
	}
	...
	// 		require(
	// 			tokenId <= makerBadgeThreshold,
	// 			'Shields: Three Shields hardware reserved for Maker Badge'
	// 		);
	// 	}
	// 	_;
	// }
	...
	function setEarlyWhitelist(address user, bool setTo) external onlyOwner publicMintIsNotActive {
		earlyWhitelist[user] = setTo;
	}
	...
	function mintShieldPass(address to, string calldata shieldData) external onlyWhitelist(to) {
		_mint(to, _nextId++, 1, '');
		//console.logUint(_nextId);
		_setURI(shieldData);
	}
	...
	function build(
		...
		uint24[4] memory colors,
		uint256 tokenId
	) external payable publicMintIsActive onlyTokenOwner(tokenId) shieldNotBuilt(tokenId) {
		...
		emit ShieldBuilt(tokenId, field, hardware, frame, colors);
	}

	function setTokenURI(uint256 tokenId, string memory _uri) internal virtual {
	...
	function _exists(uint256 tokenId) internal view virtual returns (bool) {
		return _shields[tokenId].field != 0;
	}
}