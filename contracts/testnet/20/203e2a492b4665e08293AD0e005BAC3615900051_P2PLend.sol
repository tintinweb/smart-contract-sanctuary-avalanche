...
interface IConfigProvider {
    ...
    function lendManager() external view returns (address);

    function loanManager() external view returns (address);

    function loanMaxDuration() external view returns (uint256);
    ...
    function interestBlock() external view returns (uint256);

    function weth() external view returns (address);

    function loanExtraRepayDuration() external view returns (uint256);
}
...
interface IP2PLend {
    ...
    event CancelNonce(address user, uint256 nonce);

    function acceptOffer(DataTypes.AcceptOfferParams memory params) external;

    function acceptLoan(DataTypes.AcceptLoanParams memory params) external;

    function acceptLoanETH(DataTypes.AcceptLoanParams memory params)
        external
        payable;

    function repay(uint256 _loanId, uint256 _amount) external payable;

    function liquidate(uint256 _loanId) external;

    function checkReserveWhitelist(address reserveAsset)
        external
        returns (bool);

    function checkNftWhitelist(address nftAsset) external returns (bool);

    function checkNonceIsUsed(address user, uint256 nonce)
        external
        returns (bool);
    ...
    function addNftCollection(address nftAddress) external;
}
...
interface IP2PLoan {
    ...
    function createLoan(DataTypes.BorrowParams memory params)
        external
        returns (uint256);
    ...
    function totalDebtInReserve(uint256 loanId)
        external
        returns (
            address reserveAsset,
            uint256 totalDebt,
...
library Errors {
    ...
    string public constant LP_DURATION_REACH_LIMITED = "401"; // Loan duration exceeds maximum loan duration
    string public constant LP_DURATION_CAN_NOT_ZERO = "402"; // Loan duration cannot be zero
    string public constant LP_RESERVE_NOT_IN_WHITELIST = "403"; //Currency denomination is not whitelisted to be used by this contract
    string public constant LP_NFT_NOT_IN_WHITELIST = "404"; //NFT collateral contract is not whitelisted to be used by this contract
    string public constant LP_BORROWER_NONCE_INVALID = "405"; //nonce invalid, borrower has either cancelled/begun this loan, or reused this nonce when signing
    string public constant LP_LENDER_NONCE_INVALID = "406"; //nonce invalid, lender has either cancelled/begun this loan, or reused this nonce when signing
    string public constant LP_IS_PAUSED = "407"; //lend is paused
    string public constant LP_LOAN_NOT_ACTIVE = "408"; //Loan has already been repaid or liquidated
    string public constant LP_REPAYER_IS_NOT_OWNER = "409"; //Repayer is not borrower
    string public constant LP_TRANSFER_NFT_FAILED = "410"; //NFT was not successfully transferred
    string public constant LP_LOAN_NOT_OVERDUE_YET = "411"; //Loan is not overdue yet
    // string public constant LP_LOAN_NOT_OVERDUE_YET = "412"; //Loan is not overdue yet
    string public constant LP_NFT_HAS_USED_AS_COLLATERAL = "413";
    string public constant LPL_INVALID_LOAN_STATE = "414";
    string public constant LP_NFT_IS_NOT_USED_AS_COLLATERAL = "415";
    string public constant LP_NOT_CONTRACT = "416";
    string public constant LP_NFT_ALREADY_INITIALIZED = "417"; // 'Nft has already been initialized'
    string public constant LP_LENDER_INVALID_SIGNATURE = "418"; //Invalid lender signature
    string public constant LP_BORROWER_INVALID_SIGNATURE = "419"; //Invalid lender signature
    string public constant LP_AVAILABLE_TO_EXPIRED = "420";
    string public constant LP_INVALID_WETH_ASSET = "421";
    string public constant LP_INVALID_ETH_AMOUNT = "422";
    string public constant LP_BORROWER_INVALID = "423";
    string public constant LP_LENDER_INVALID = "424";
...
import {IP2PLend} from "../../interfaces/IP2PLend.sol";
import {IP2PLoan} from "../../interfaces/IP2PLoan.sol";
...
library BorrowLogic {
    ...
    function executeBorrow(
        ...
        _borrow(addressesProvider, params);
    }

    function _borrow(address provider, DataTypes.BorrowParams memory params)
        internal
    {
        // Sanity check loan values.
        require(
            uint256(params.loanDuration) <=
                IConfigProvider(provider).loanMaxDuration(),
            Errors.LP_DURATION_REACH_LIMITED
        );
        require(
            uint256(params.loanDuration) != 0,
            Errors.LP_DURATION_CAN_NOT_ZERO
        );

        require(
            params.loanAvailableTo >= block.timestamp,
            Errors.LP_AVAILABLE_TO_EXPIRED
        );

        require(
            IP2PLend(IConfigProvider(provider).lendManager())
                .checkReserveWhitelist(params.reserveAsset),
            Errors.LP_RESERVE_NOT_IN_WHITELIST
        );
        require(
            IP2PLend(IConfigProvider(provider).lendManager()).checkNftWhitelist(
                params.nftAsset
            ),
            Errors.LP_NFT_NOT_IN_WHITELIST
        );

        //verify lender signature
        require(
            !IP2PLend(IConfigProvider(provider).lendManager()).checkNonceIsUsed(
                params.lender,
                params.lenderNonce
            ),
            Errors.LP_LENDER_NONCE_INVALID
        );
        if (!params.isAcceptLoan) {
            require(
                params.offerAvailableTo >= block.timestamp,
                Errors.LP_AVAILABLE_TO_EXPIRED
            );
            //borrower accept lender's offer => check lender signature
            params.isNative = false;
            require(
                GenericLogic.isValidLenderSignature(
                    params.loanPrincipalAmount,
                    params.loanDuration,
                    params.loanInterestRate,
                    params.offerAvailableTo,
                    params.nftAsset,
                    params.nftTokenId,
                    params.reserveAsset,
                    params.lender,
                    ...
                    params.lenderSignature
                ),
                Errors.LP_LENDER_INVALID_SIGNATURE
            );
        }
        ...
        require(
            ...
            Errors.LP_BORROWER_NONCE_INVALID
        );
        if (params.isAcceptLoan) {
            //lender accept loan => check borrower signature
            require(
                GenericLogic.isValidBorrowerSignature(
                    params.loanPrincipalAmount,
                    params.loanDuration,
                    params.loanInterestRate,
                    params.loanAvailableTo,
                    params.nftAsset,
                    params.nftTokenId,
                    params.reserveAsset,
                    params.borrower,
                    params.borrowerNonce,
                    params.borrowerSignature
                ),
                Errors.LP_BORROWER_INVALID_SIGNATURE
            );
        }
        uint256 loanId = IP2PLoan(IConfigProvider(provider).loanManager())
            .getCollateralLoanId(params.nftAsset, params.nftTokenId);
        ...
        if (loanId == 0) {
            ...
            loanId = IP2PLoan(IConfigProvider(provider).loanManager())
                .createLoan(params);
        } else {
            revert("not supported");
        }

        if (params.reserveAsset == IConfigProvider(provider).weth()) {
            if (params.isAcceptLoan && params.isNative) {
                require(
                    msg.value == params.loanPrincipalAmount,
                    Errors.LP_INVALID_ETH_AMOUNT
                );
                //transfer eth to borrower
                TransferHelper.safeTransferETH(
                    IConfigProvider(provider).weth(),
                    params.borrower,
                    params.loanPrincipalAmount
                );
            } else {
                //transfer weth from lender to contract
                IERC20Upgradeable(params.reserveAsset).transferFrom(
                    params.lender,
                    address(this),
                    params.loanPrincipalAmount
                );
    ...
    function executeRepay(
        ...
        return _repay(provider, params);
    }

    function _repay(
        address provider,
        DataTypes.ExecuteRepayParams memory params
    ) internal returns (uint256 repayAmount, uint256 feeAmount) {
        ...
        DataTypes.LoanData memory loan = IP2PLoan(
        ...
        uint256 loanMaturityDate = (uint256(loan.loanStartTime)).add(
            uint256(loan.loanDuration).add(
                IConfigProvider(provider).loanExtraRepayDuration()
            )
        );
        require(block.timestamp <= loanMaturityDate, Errors.LP_LOAN_NOT_ACTIVE);
        vars.borrowAmount = loan.loanPrincipalAmount;
        address lender = loan.lender;

        (vars.repayAmount, vars.interestAmount, vars.feeAmount) = GenericLogic
            .calculateInterestInfo(
                GenericLogic.CalculateInterestInfoVars({
                    principalAmount: loan.loanPrincipalAmount,
                    interestRate: loan.loanInterestRate,
                    platformFeeRate: IConfigProvider(provider).platformFeeRate(),
                    loanStartTime: loan.loanStartTime,
                    loanDuration: loan.loanDuration,
                    interestBlock: IConfigProvider(provider).interestBlock()
                })
            );

        require(params.amount >= vars.repayAmount, Errors.VL_INVALID_AMOUNT);
        IP2PLoan(IConfigProvider(provider).loanManager()).repayLoan(
            vars.initiator,
            vars.loanId,
            vars.repayAmount
        );
        if (loan.reserveAsset == IConfigProvider(provider).weth()) {
            require(
                msg.value == vars.repayAmount,
                Errors.LP_INVALID_ETH_AMOUNT
            );
            if (loan.isNative) {
                // Transfer principal-plus-interest-minus-fees (ETH) to lender
                TransferHelper.safeTransferETH(
                    IConfigProvider(provider).weth(),
                    lender,
                    vars.repayAmount.sub(vars.feeAmount)
                );
            } else {
                // Transfer principal-plus-interest-minus-fees (WETH) to lender
                TransferHelper.transferWETHFromETH(
                    IConfigProvider(provider).weth(),
                    lender,
                    vars.repayAmount.sub(vars.feeAmount)
                );
            ...
            TransferHelper.safeTransferETH(
                IConfigProvider(provider).weth(),
                IConfigProvider(provider).platformFeeReceiver(),
                vars.feeAmount
            );
        } else {
    ...
    function executeLiquidate(
        ...
        require(
            ...
            Errors.LP_LOAN_NOT_OVERDUE_YET
        );

        IP2PLoan(IConfigProvider(provider).loanManager()).liquidateLoan(
            loan.lender,
            loan.loanId,
            loan.loanPrincipalAmount
        );

        // transfer erc721 to lender
        IERC721Upgradeable(loan.nftAsset).safeTransferFrom(
            address(this),
            loan.lender,
            loan.nftTokenId
...
import {Constants} from "../helpers/Constant.sol";
import {IConfigProvider} from "../../interfaces/IConfigProvider.sol";
...
library GenericLogic {
    ...
    {
        uint256 durationSoFarInSeconds = 0;
        if (block.timestamp > vars.loanStartTime) {
            durationSoFarInSeconds = block.timestamp - vars.loanStartTime;
        }
        if (vars.interestBlock == 0) {
            vars.interestBlock = 86400; //1day
        }
        uint256 totalBlock = (vars.loanDuration.div(vars.interestBlock)).add(1);
        uint256 sofarBlock = 0;
        uint256 remainBlock = 0;

        if (durationSoFarInSeconds > vars.loanDuration) {
            sofarBlock = totalBlock;
        } else {
            sofarBlock = (durationSoFarInSeconds.div(vars.interestBlock)).add(
                1
            );
            remainBlock = totalBlock - sofarBlock;
        }

        interest = _computeInterestDue(
            vars.principalAmount,
            vars.interestRate,
            sofarBlock,
            remainBlock,
            vars.interestBlock
        );
        platformFee = vars.principalAmount.mul(vars.platformFeeRate).div(10000);
        totalDebt = vars.principalAmount.add(interest).add(platformFee);
        return (totalDebt, interest, platformFee);
    }
    ...
    function _computeInterestDue(
        ...
        //start--------------------repay---------------------end
        //        full interest              1/2 interest
        interest = principalAmount
            .mul(rate)
            .mul(sofarBlock)
            .div(uint256(10000))
            .div(uint256(365 * 86400) / interestBlock);
        interest =
            interest +
            principalAmount
                .mul(rate)
...
library DataTypes {
    ...
    struct BorrowParams {
        ...
        uint256 loanDuration;
        uint256 loanInterestRate;
        uint256 loanAvailableTo;
        uint256 offerAvailableTo;
        address borrower;
        address lender;
        uint256 borrowerNonce;
        uint256 lenderNonce;
...
pragma solidity ^0.8.17;
import {IP2PLend} from "../interfaces/IP2PLend.sol";
import {P2PLendStorage} from "./P2PLendStorage.sol";
import {IConfigProvider} from "../interfaces/IConfigProvider.sol";
    ...
    ERC721HolderUpgradeable
{
    ...
    function acceptOffer(DataTypes.AcceptOfferParams memory params)
        public
        whenNotPaused
        nonReentrant
    {
        require(msg.sender == params.borrower, Errors.LP_BORROWER_INVALID);
        BorrowLogic.executeBorrow(
            ...
            DataTypes.BorrowParams({
                ...
                loanAvailableTo: params.loanAvailableTo,
                offerAvailableTo: params.offerAvailableTo,
                borrower: params.borrower,
                lender: params.lender,
                borrowerNonce: params.borrowerNonce,
                lenderNonce: params.lenderNonce,
                borrowerSignature: params.lenderSignature, //don't use
                lenderSignature: params.lenderSignature,
                isAcceptLoan: false,
                isNative: false
            })
        );
        _setNonceIsUsed(params.borrower, params.borrowerNonce);
        _setNonceIsUsed(params.lender, params.lenderNonce);
    }

    function acceptLoan(DataTypes.AcceptLoanParams memory params)
        public
        whenNotPaused
        nonReentrant
    {
        require(msg.sender == params.lender, Errors.LP_LENDER_INVALID);
        BorrowLogic.executeBorrow(
            address(provider),
            DataTypes.BorrowParams({
                nftAsset: params.nftAsset,
                ...
                loanInterestRate: params.loanInterestRate,
                loanAvailableTo: params.loanAvailableTo,
                offerAvailableTo: 0, //don't use
                borrower: params.borrower,
                lender: params.lender,
                borrowerNonce: params.borrowerNonce,
                lenderNonce: params.lenderNonce,
                borrowerSignature: params.borrowerSignature,
                lenderSignature: params.borrowerSignature, //don't use
                isAcceptLoan: true,
                isNative: false
            })
        );
    ...
    function checkReserveWhitelist(address reserveAsset)
        ...
        returns (bool)
    {
        return reservesWhitelist[reserveAsset];
    }

    function checkNftWhitelist(address nftAsset)
        external
        view
        override
        returns (bool)
    {
        return nftsWhitelist[nftAsset];
    }
    ...
    function checkNonceIsUsed(address user, uint256 nonce)
        ...
        returns (bool)
    {
        return _nonceHasBeenUsedForUser[user][nonce];
    }

    function _setNonceIsUsed(address user, uint256 nonce) internal {
        _nonceHasBeenUsedForUser[user][nonce] = true;
    }
    ...
    function _addReserveToList(address asset) internal {
        require(
            reservesWhitelist[asset] == false,
            Errors.RL_RESERVE_ALREADY_INITIALIZED
        );
        reservesWhitelist[asset] = true;
    }

    function addNftCollection(address nftAddress)
        external
        override
        onlyFactoryConfigurator
    {
        ...
        IP2PLoan(provider.loanManager()).initNft(nftAddress);
    }

    function _addNftToList(address nftAddress) internal {
        require(
            nftsWhitelist[nftAddress] == false,
            Errors.LP_NFT_ALREADY_INITIALIZED
        );
        nftsWhitelist[nftAddress] = true;
    }

    function cancelNonce(uint256 nonce) external {
        _setNonceIsUsed(msg.sender, nonce);
        emit CancelNonce(msg.sender, nonce);
    }
}
...
contract P2PLendStorage {
    mapping(address => bool) public reservesWhitelist;

    mapping(address => bool) public nftsWhitelist;
