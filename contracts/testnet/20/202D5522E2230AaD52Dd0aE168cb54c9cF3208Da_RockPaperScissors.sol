...
abstract contract GameBet is Ownable {
    ...
    function validateBet(uint256 treasuryBalance, uint256 totalRake, uint numGames) internal returns(BetDetails memory) {
        ...
        require(rake >= (bet * totalRake) / 10000); // what is this? check for divison?
...
contract RockPaperScissors is GameBase, GameBet, GameChainlink {
    ...
    event GameStarted(address indexed creator, uint256 indexed gameId, RoundChoices[] choices, address referrer, uint256 betPerRound);
    event RequestedGameWinner(uint256 indexed requestId, uint256 indexed gameToClose);
    event WinnerPicked(uint256 indexed gameId, RoundChoices[] resultChoices, RoundState[] results, uint256 amountSent);
    ...
    function _createGame(
        ...
        emit GameStarted(msg.sender, fi.gameId, fi.choices, fi.referrer, fi.betPerRound);
        return fi.gameId;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        RoundChoices[] memory output = new RoundChoices[](fi.choices.length);
        RoundState[] memory results = new RoundState[](fi.choices.length);
        uint256 betToUser = 0;
        ...
        for (uint i=0; i<fi.choices.length; i++) {
            ...
            output[i] = RoundChoices(roundRandom % 3);
            if (fi.choices[i] == output[i]) {
                results[i] = RoundState.TIE;
                betToUser += fi.betPerRound;
                rakeToUser += fi.rakePerRound;
            } else if ((uint(fi.choices[i]) + 1) % 3 == uint(output[i])) {
                results[i] = RoundState.LOST;
                rakeToKeep += fi.rakePerRound;

            } else {
                results[i] = RoundState.WON;
        ...
        emit WinnerPicked(gameId, output, results, amountToUser);
    }
