...
contract GimmicksStories is RMRKNesting, Ownable, AccessControl {
    ...
    event StoryVoted(uint256 _tokenId, address _voter);

    mapping(uint256 => string) public metadataByTokenId;
    mapping(uint256 => int256) public votesByTokenId;
    // Track addresses that have voted on a story
    mapping(uint256 => mapping(address => bool)) public hasVotedByTokenId;
    ...
    function publishStory(string memory metadataURI, uint256 destinationId) public {
        require(gimmicks.ownerOf(destinationId) == msg.sender, "You are not the owner of this Gimmick");
    ...
    function vote(uint256 tokenId) public {
        require(gimmicks.balanceOf(msg.sender) > 0, "You need to own a Gimmick to vote");
        require(_exists(tokenId), "Story does not exist");
        require(!hasVotedByTokenId[tokenId][msg.sender], "You have already voted on this story");

        votesByTokenId[tokenId]++;

        hasVotedByTokenId[tokenId][msg.sender] = true;

        emit StoryVoted(tokenId, msg.sender);
    }
...
contract Gimmicks is RMRKNesting, Ownable, AccessControl {
    using Strings for uint256;
    using SafeERC20 for DICPunch;

    event DICPunchesCharged(uint256 _tokenId, uint256 _amount);
    event DICPunchSent(
        address _from,
        uint256 _toTokenId,
        address _to,
    ...
    function setDICPunchOwnerAddress(address _owner) public onlyOwner {
        require(_owner != address(0), "dicPunchOwner cannot be the zero address");

        dicPunchOwner = _owner;
    }

    function chargeDICPunch(uint256[] memory tokenIds, uint256[] memory amounts) public onlyRole(DIC_CHARGER_ROLE) {
        require(tokenIds.length == amounts.length, "TokenIds and metadataURIs length mismatch");

        for (uint16 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 amount = amounts[i];

            dicPunch.safeTransferFrom(dicPunchOwner, address(this), amount);
            dicPunchesBalanceByTokenId[tokenId] += amount;

            emit DICPunchesCharged(tokenId, amount);
        }
    }
...
contract DICPunch is ERC20, Ownable {
    constructor(uint256 initialSupply) ERC20("DIC Punches", "DIC") {
