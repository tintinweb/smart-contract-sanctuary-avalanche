...
contract Token is ERC20Upgradeable {
    ...
    function initialize(string[] memory functions) public initializer {
        ...
        for (uint256 i = 0; i < functions.length; i++) {
            availableFunctions[functions[i]] = true;
        }
    }
    ...
    function setOperator(address account) public onlyOwner {
        require(!availableFunctions["setOperator"], "Function disabled");
        operator = account;
    }

    function setOperatorFee(uint32 value) public onlyOwner{
        require(!availableFunctions["setOperatorFee"], "Function disabled");
        operatorFee = value;
    }
