...
contract raffleFactory {
    ...
    string  _name = "The Buny Project: Buny Raffle";
    string  _symbol = "rBUNY";
    string  _animationUrl = "https://ipfs.io/ipfs/Qmd94YsrsK5jPbi675XiwyLwMhcM6HS6MULC7mqQGMnXtW";
    uint256 _editionSize = 12;
    uint256 _royaltyBPS = 500;
    uint256  _prime = 893519510858573286167967681123963591339487568913085725764533;
  ...
  struct Raffle {
    ...
    function createEdition(
        ...
        atContract.increment();
         Raffle memory x = Raffle(_name, newContract, _editionSize, newId);
    raffle.push(x);
    raffleCount ++;
    return newId;
        return newId;
  ...
  function readAllRaffles() public view  returns (Raffle[] memory) {
    Raffle[] memory result = new Raffle[](raffleCount);
...
contract raffleNft is ERC721Upgradeable, IEditionSingleMintable, IERC2981Upgradeable, OwnableUpgradeable {
  using CountersUpgradeable for CountersUpgradeable.Counter;
  ...
  event WinnerPicked(address payable winner, uint prize);
 ...
 // return balance in wei
    ...
    // require total supply to equal edition size. 
    // Call random integer function
    // transferring 80% of raffle to winner
    // transfer 20% to maker
    // push winner into winners circle
    // emit winner address and prize amount
    function pickWinner() external payable returns(uint256){
        require (players.length >= minPlayers);
        require (totalSupply() >= editionSize);
        uint r = random();
  ...
  /**
     ...
     */
  function getURIs() public view returns (string memory, string memory) {
    return (imageUrl, animationUrl);
  }
