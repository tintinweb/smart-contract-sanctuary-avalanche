    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("ALLOY-Testnet", "ALY-TT");
        __Ownable_init();
        ...
        NiozERC20Address = IERC20Upgradeable(
            0xb1c05AEF07bE44A78617028B6553336c1C109616
        );
    ...
    function mintPrinterARC(uint256 epoch_id, uint256 quantity)
        external
        payable
        virtual
        nonReentrant
    {
        require(
            ARFtokenAddress.checkEpochStatus(epoch_id) &&
                ARFtokenAddress.epochDetails(epoch_id).totalQuantityARC >=
                (ARFtokenAddress.epochDetails(epoch_id).quantityCounterARC + quantity) && 
                (epoch_id == 6 || epoch_id == 7 || epoch_id == 8),
            "AlloyERC721: This epoch is closed or printer is out of stock"
        );
        require(
            ARFtokenAddress.epochDetails(epoch_id).avaxPriceARC * quantity == msg.value,
        ...
        if (ARFtokenAddress.epochDetails(epoch_id).NiozPriceARC > 0) {
            NiozERC20Address.safeTransfer(
                msg.sender,
                address(this),
                ARFtokenAddress.epochDetails(epoch_id).NiozPriceARC
            );
        ...
        // ARFtokenAddress.updateCounter(epoch_id, quantity);
