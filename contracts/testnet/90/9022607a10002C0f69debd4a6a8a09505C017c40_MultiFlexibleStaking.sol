...
contract MultiFlexibleStaking is Ownable, Pausable {
    ...
    struct Stake {
        ...
        address owner;
        uint128 unstakedAtBlockNumber;
        uint128 unstakedAtBlockTimestamp;
    }
    ...
    uint256 public cumulativePreviousDebt;
    ...
    function withdraw(uint256 id) public updateReward(id) {
        ...
        if (_stake.unstakedAtBlockNumber == 0 && _stake.earned != 0) {
        ...
        stakes[id].unstakedAtBlockNumber = uint128(block.number);
        stakes[id].unstakedAtBlockTimestamp = uint128(block.timestamp);
    ...
    function _claim(uint256 id, uint256 rewardsPenaltyPercentage) internal {
        ...
        if (totalSupply == 0) {
            ...
            cumulativePreviousDebt = reward > cumulativePreviousDebt
                ? 0
                : cumulativePreviousDebt - reward;
        }
    ...
    function _notifyRewardAmount(uint128 newRewardRate) private {
        ...
        cumulativePreviousDebt = maxPotentialDebt;
        if (totalSupply != 0) {
    ...
    function getTotalRewards() external view returns (uint256) {
        // last_period_debt * (1 - time_left / time_total) + previous_cumulative_debt
        uint80 timeLeft = periodFinish - lastTimeRewardApplicable();
        return
            (((maxPotentialDebt - cumulativePreviousDebt) *
                (rewardsDuration - timeLeft)) / rewardsDuration) + cumulativePreviousDebt;
    }
}