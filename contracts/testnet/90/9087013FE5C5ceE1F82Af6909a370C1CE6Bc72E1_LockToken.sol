...
contract LockToken is OwnableUpgradeable, IERC20 {
    ERC20 public token;
    ...
    function startLocking(
        ...
        startTimeToLock[_startDate].startDate = _startDate;
        startTimeToLock[_startDate].assetAddresses = _assetAddresses;
        startTimeToLock[_startDate].lockingPeriods = _lockingPeriods;
        startTimeToLock[_startDate].amounts = _amounts;
        startTimeToLock[_startDate].receivers = _receivers;
        startTimes.push(_startDate);
        for (uint256 i; i < _amounts.length; i++) {
            totalSupply += _amounts[i];
            token.transferFrom(_assetAddresses[i], address(this), _amounts[i]);
        }
    }
