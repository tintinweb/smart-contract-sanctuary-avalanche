...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    modifier GameEndRules() {
        ...
        require(
            dayDifferance(block.timestamp, lastUpdateTimeStamp) <= 2,
            "Game Ended !"
        );
        require(
            dayDifferance(block.timestamp, _gameStatus.startAt) <=
                (STAGES + THERSHOLD) - 1,
            "Game Achived thershold!"
        );
        _;
    }
        ...
        GameEndRules
    {
        ...
        if (_member.userWalletAddress != address(0)) {
            GameStatus storage _admin = GameStatusInitialized[latestGameId];
            uint256 dayPassedAfterJump = dayDifferance(
        ...
        GameEndRules
    {
        ...
        require(
            dayDifferance(block.timestamp, _member.lastJumpTime) <= 1,
        ...
        GameEndRules
    {
        GameItem memory _member = PlayerItem[playerId];
        GameStatus memory _gameStatus = GameStatusInitialized[latestGameId];
        uint256 currentDay = dayDifferance(
            block.timestamp,
            _gameStatus.startAt
        );
    ...
    function getAll() external view returns (uint256[] memory) {
        ...
        if (_gameStatus.stageNumber > 0) {
            ...
            for (uint256 i = 1; i < _stageNumber; i++) {
                ret[i] = GameMap[i];
            }
        }
        return ret;
    ...
    function LateBuyInFee(uint256 _nftId, uint8 seriesType)
        public
        GameEndRules
    {
        require(seriesType == 1 || seriesType == 2, "Invalid seriseType");
        bytes32 playerId = computeNextPlayerIdForHolder(
            msg.sender,
            _nftId,
            seriesType
        );
    ...
    function bulkLateBuyInFee(
        uint256[] calldata _nftId,
        uint8[] calldata seriesType
    ) external {
        for (uint256 i = 0; i < _nftId.length; i++) {
