...
contract Node {
    ...
    IERC20 public constant TOKEN = IERC20(0x1084Bac18Ec775dFe631f2FDb021D75267A9fb71);

    uint256[] public tierAllocPoints = [150 ether, 0 ether];
    uint256[] public tierAmounts = [50 ether, 0 ether];
    ...
    address public dev;
    address public treasury = 0xc0702Ae0374F83fc3bA71CE2B30A323b09EC19da;
    ...
    function setUser(address _addr, User memory _user) external onlyDev {
        ...
        totalAllocPoints = totalAllocPoints.sub(users[_addr].alloc_points).add(_user.alloc_points);

        users[_addr].total_deposits = _user.total_deposits;
        users[_addr].total_claims = _user.total_claims;
        users[_addr].last_distPoints = _user.last_distPoints;
        users[_addr].alloc_points = _user.alloc_points;
        users[_addr].compounds = _user.compounds;
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        uint256 alloc = tierAllocPoints[nodeTier].mul(numNodes);
        ...
        nodes[_sender][nodeTier] = nodes[_sender][nodeTier].add(numNodes); // add nodes to account
        totalNodes[nodeTier] = totalNodes[nodeTier].add(numNodes); // add nodes to total
    ...
    function _compound(uint256 nodeTier, uint256 numNodes) internal {
        ...
        users[_sender].total_deposits = users[_sender].total_deposits.add(tierPrice); //set user total deposits
        users[_sender].alloc_points = users[_sender].alloc_points.add(alloc).add(bonus); //set user alloc points
