...
contract BorrowerOperations is LiquityBase, IBorrowerOperations, ReentrancyGuardUpgradeable {
    ...
    function _singleLeverUp(
        ...
        if (_isWrapped(_token)) {
            _additionalTokenAmount = _additionalTokenAmount.mul(
                IYetiVaultToken(_token).receiptPerUnderlying()
     ...
     */
    function _singleTransferCollateralIntoActivePool(address _coll, uint256 _amount)
        internal
        returns (uint256)
    {
        if (_isWrapped(_coll)) {
    ...
    function _requireLeverUpEnabled(IYetiController _controller) internal view {
        require(_controller.leverUpEnabled(), "B");
    }
    ...
    /**
     * @notice Function require input correct like length equal, used to save contract size on revert strings
     */
    function _requireInputCorrect(bool inputCorrect) internal pure {
        require(inputCorrect, "3");
    }
     ...
     */
    function _requireSufficientYUSDBalance(IYUSDToken _yusdToken, uint256 _debtRepayment)
        internal
        view
    {
        require(_IERC20TokenBalanceOf(_yusdToken, msg.sender) >= _debtRepayment, "0");
    }
        ...
        pure
    {
        ...
        _requireInputCorrect(_maxFeePercentage <= DECIMAL_PRECISION && (_isRecoveryMode || _maxFeePercentage >= BORROWING_FEE_FLOOR));
    }
    ...
    /**
     * @notice calls controller is wrapped checker. 
     */
    function _isWrapped(address _coll)
        internal view
        returns (bool)
    {
        return controller.isWrapped(_coll);
    }
}
...
contract LiquityBase is ILiquityBase, YetiCustomBase {
    ...
    uint256 internal constant BORROWING_FEE_FLOOR = (DECIMAL_PRECISION / 1000) * 5; // 0.5%

    // Minimum fee paid on redemption, paid in YUSD
    uint256 internal constant REDEMPTION_FEE_FLOOR = (DECIMAL_PRECISION / 1000) * 5; // 0.5%
    ...
    function _getVCColls(newColls memory _colls) internal view returns (uint256 totalVC) {
        totalVC = controller.getValuesVC(_colls.tokens, _colls.amounts);
    }

    function _getRVCColls(newColls memory _colls) internal view returns (uint256 totalRVC) {
        totalRVC = controller.getValuesRVC(_colls.tokens, _colls.amounts);
    }

    function _getUSDColls(newColls memory _colls) internal view returns (uint256 totalUSDValue) {
