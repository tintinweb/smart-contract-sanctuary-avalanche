...
contract SmolMockFuji is Ownable, ERC721Enumerable {
    using Strings for uint256;
    uint nextTokenId;
    string public baseTokenUri;
    uint256 public immutable COLLECTION_SIZE;
    uint public immutable metaDataOffset;
    mapping (uint => uint) public tokenIdtoImageId;

    constructor(string memory name_, string memory symbol_, uint256 _offset, string memory _baseTokenUri, uint256 _collectionSize)ERC721(name_, symbol_)
    {
        baseTokenUri = _baseTokenUri;
        metaDataOffset = _offset;
    ...
    function mint(uint amount, address _user) external {
        for (uint256 index = 0; index < amount; index++) {
            _mint(_user, nextTokenId++);
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        uint metadataId = (tokenId + metaDataOffset) % COLLECTION_SIZE;
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, metadataId.toString())) : "";
    }
