...
contract SingleStakingRewards is StakingRewards, ISingleStakingRewards {
    ...
    constructor(
        ...
        uint16[] memory _withdrawalFeesBps,     // aligned to fee schedule
        uint32[] memory _withdrawalFeeSchedule  // assumes a sorted array
    ) StakingRewards(_stakingTokenAddress, _rewardTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeeSchedule) {
        require(_stakingTokenAddress != address(0), "Staking token must be an ElkDex LP token");
    }

    function compoundSingleStakingRewards() external updateRewards(msg.sender) {
        address stakingTokenAddress = address(stakingToken);
        require(rewardTokenAddresses[stakingTokenAddress], "Cannot compound: Staking token is not one of the rewards tokens.");
...
/* Contract that is used by users to create SingleStakingRewards contracts. It stores each farm as it's created, as well as the current 
...
contract ElkSingleStakeFactory is ISingleStakeFactory, Ownable {
    ...
    // Main function in the contract. Creates a new SingleStakingRewards contract, stores the farm address by creator and the given LP token, and also stores 
    ...
    function createNewSingleStake(
     ...
     public override {
        require(getSingleStake[msg.sender][_stakingTokenAddress] == address(0), 'Elk: FARM_EXISTS'); // single check is sufficient

        bytes memory creationCode = type(SingleStakingRewards).creationCode;
        bytes memory bytecode = abi.encodePacked(creationCode, abi.encode(_stakingTokenAddress, _rewardTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeeSchedule));
        address addr;
    ...
    // This is available in case any updates are made to the SingleStakeManager contract.  Ownership is not changed retroactively, so any created
    // farms will always have the same manager contract.
    function setManager(address managerAddress) external override onlyOwner {
        require(managerAddress != address(0), "managerAddress is the zero address");
    ...
    function _takeFee() private {
        require(feeToken.balanceOf(msg.sender) >= fee, "Creator cannot pay fee");
    ...
    function setFee(uint256 newFee) external onlyOwner {
        require(newFee < maxFee, "Fee cannot be greater than max allowed");
        fee = newFee;
        emit FeeSet(newFee);
    }
    ...
    function _overrideOwnership(address farmAddress) private {
        ...
        SingleStakingRewards rewardsContract = SingleStakingRewards(farmAddress);
        address stakingToken = address(rewardsContract.stakingToken());

        getSingleStake[creatorAddress][stakingToken] = address(0);
        getSingleStake[msg.sender][stakingToken] = farmAddress;
