...
contract TalentScout is IERC20, AccessControl, Ownable {
    ...
    // TODO: Change for mainnet
    IWAVAX public WAVAX = IWAVAX(0xd00ae08403B9bbb9124bB305C09058E32C39A48c);
    ...
    // Taxes
    uint256 taxAmount = 1200;

    uint256 reflectionTax = 6500;
    uint256 talentAwardTax = 2000;
    uint256 voterRewardTax = 1000;
    uint256 lpTax = 200;
    uint256 adminTax = 200;
    uint256 buyBackTax = 100;
    uint256 feeDenominator = 10000;

    mapping(address => bool) isFeeExempt;

    // Holder list implementation
    address[] public holders;
    mapping(address => bool) public _holderList;
    ...
    // check holders and push them in the array
    function addHolder(address _address) internal { 
        if (isFeeExempt[_address] && !_holderList[_address]) {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        unchecked {
            ...
            _balances[ZERO] = _balances[ZERO] + amount;
        }
    ...
    function _transferFrom(
        ...
        // check if sender/receiver is fee exempt
    ...
    function takeFee(uint256 amount, address _sender) internal {
        ...
        collectedReflectionFeeAmount = collectedReflectionFeeAmount.add(reflectionFeeAmount);
        _basicTransfer(_sender, address(this), reflectionFeeAmount);
        ...
        autoBuybackAndBurn(buybackFeeAmount);
    }
    ...
    function swapBackAVAX(uint256 scoutAmount) internal returns(uint256) {
        ...
        uint256 balanceBefore = adminAddress.balance;

        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            swapThreshold.sub(scoutAmount), // amount to swap
            0,
            path,
            adminAddress,
            block.timestamp + 360
        ...
        uint256 amountAVAX = adminAddress.balance.sub(balanceBefore);
        return amountAVAX;
    }

    function autoLiqidify(uint256 liquidityFeeAmount) internal {
        ...
        if (amountToLiquify > 0) {
            adminAddress.call{value: amountAVAXLiquidity}(new bytes(0));
            ...
            emit AutoLiquify(liquidityFeeAmount, amountToLiquify);
        }
    }

    function autoBuybackAndBurn(uint256 buybackFeeAmount) internal {
        uint256 amountAVAX = swapBackAVAX(buybackFeeAmount);
        if (amountAVAX > 0) {
            uint256 balanceBefore = _balances[address(this)];
            buyTokens(amountAVAX, address(this));
            uint256 balanceScout = _balances[address(this)].sub(balanceBefore);
            _burn(address(this), balanceScout);
        }
    }
