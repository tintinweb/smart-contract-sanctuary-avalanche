...
interface Pair {
	...
	function allowance(address, address) external view returns (uint); // console
}
...
contract REBASED {
    ...
    uint256 constant private TOTAL_SUPPLY_CAP = 1000000000 ether; // 1B
    uint256 constant private TRANSFER_FEE_SCALE = 100000; // 100 = 0.1%
    uint256 constant private REBASED_STAKING_SUPPLY = 2e25; // 20M REBASED
    uint256 constant private LP_STAKING_SUPPLY = 4e25; // 40M REBASED
    uint256 constant private TREASURY_SUPPLY = 4e25; // 40M REBASED
    uint256 constant private BASE_UPGRADE_COST = 1e19; // 10 REBASED
    uint256 constant private SERVICE_FEE = 0.6942 ether;
    bool private isInitialized;

    string constant public name = "rebased.wtf";
    string constant public symbol = "REBASED";
    ...
    function initialize(
        uint256 _stakingRewardsStart, StakingRewards _stakingRewards, StakingRewards _lpStakingRewards,
        Router _router, address _DAO, address _ops, address _signer
    ) external {
        ...
        info.pair = Pair(Factory(info.router.factory()).createPair(info.router.WAVAX(), address(this)));
		info.pair.approve(address(_lpStakingRewards), UINT_MAX);
        // info.weth0 = info.pair.token0() == info.router.WETH();
        info.weth0 = info.pair.token0() == info.router.WAVAX();
        info.transferFee = 6942; // 6.9420%
        info.owner = msg.sender;
        _mint(msg.sender, TREASURY_SUPPLY);
        info.stakingRewards = _stakingRewards;
        info.stakingRewards._initialize(REBASED_STAKING_SUPPLY, _stakingRewardsStart, ERC20(address(this)));
        _mint(stakingRewardsAddress(), REBASED_STAKING_SUPPLY);
        info.lpStakingRewards = _lpStakingRewards;
        info.lpStakingRewards._initialize(LP_STAKING_SUPPLY, _stakingRewardsStart, ERC20(pairAddress()));
        _mint(lpStakingRewardsAddress(), LP_STAKING_SUPPLY);
        isInitialized = true;
        _approve(address(this), address(info.router), UINT_MAX);
        _approve(address(this), stakingRewardsAddress(), UINT_MAX);
        _approve(address(this), lpStakingRewardsAddress(), UINT_MAX);
    ...
    function sweep() external {
        if (address(this).balance > 0) {
            (bool success,) = info.ops.call{value: address(this).balance}("");
            require(success);
        }
    }
    ...
    function unlock(address _account, address payable _referrer) external payable {
        ...
        uint256 _halfRemaining = (SERVICE_FEE - _refFee) / 2;
        (bool a,) = info.DAO.call{value: _halfRemaining}("");
        require(a);
        emit ReflinkRewards(info.DAO, _halfRemaining);
        (bool b,) = info.ops.call{value: _halfRemaining}("");
        require(b);
        emit ReflinkRewards(info.ops, _halfRemaining);
        info.users[_account].unlocked = true;
    }
    
    function claim(address _account, uint256 _amount, bytes calldata _signature) external {
        _claimREBASED(_account, _amount, _signature);
        _mint(_account, _amount);
    }
    
    function _claimREBASED(address _account, uint256 _amount, bytes calldata _signature) private {
        ...
        require(recoveredAddr == info.signer, "Invalid signature");
        require(isUnlocked(_account), "Locked");
        require(!isClaimedREBASED(_account), "Claimed");

        info.users[_account].isClaimed = true;
    }

    function claimAndStake(address _account, uint256 _amount, bytes calldata _signature) external {
        _claimREBASED(_account, _amount, _signature);
        _mint(address(this), _amount);
        info.stakingRewards.depositFor(_account, _amount);
    }

    function claimAndAddLiquidityAndStake(address _account, uint256 _amount, bytes calldata _signature) external payable {
		// Claim REBASED
        _claimREBASED(_account, _amount, _signature);
        _mint(address(this), _amount);
		// Add liquidity to REBASED-AVAX pair
		(,, uint lpTokenAmt) = info.router.addLiquidityAVAX{value: address(this).balance}(
            address(this), _amount,
            0, 0, address(this), block.timestamp
        );
		// Stake into LP Staking contract
        info.lpStakingRewards.depositFor(_account, lpTokenAmt);
    }

	/// @dev Will delete on Mainnet deployment // console
	function approveAll() external _onlyOwner { // console
		info.pair.approve(address(info.lpStakingRewards), UINT_MAX); // console
	} // console
 // console
	function getAVAXAmtToPair(uint256 _amount) external view returns (uint) {
		(uint112 reserve0, uint112 reserve1,) = info.pair.getReserves();
		return info.router.quote(_amount, uint(reserve0), uint(reserve1));
	}
    ...
    function burn(address _account, uint256 _tokens) public {
    ...
    function unlockTransfer() external _onlyOwner {
        info.transferUnlocked = true;
    }
    ...
    function setDAO(address _DAO) external _onlyOwner {
        info.DAO = _DAO;
    }

    function setOps(address _ops) external _onlyOwner {
        info.ops = _ops;
    }
    ...
    function _mint(address _account, uint256 _amount) private {
        require(totalSupply() + _amount < TOTAL_SUPPLY_CAP, "Cap exceeded");
    ...
    function _approve(address _owner, address _spender, uint256 _tokens) private returns (bool) {
        info.users[_owner].allowance[_spender] = _tokens;
        emit Approval(_owner, _spender, _tokens);
        return true;
    }
    
    function _transfer(address _from, address _to, uint256 _tokens) private returns (bool) {
        if(!info.transferUnlocked) require(_from == owner(), "locked"); // Anti-snipe

        require(balanceOf(_from) >= _tokens);

        if (info.amountToLiquify > 0 && !info.liqInProgress) _addLiquidity(info.amountToLiquify);
        ...
        if (_fee > 0) {
            uint256 _portionFee = _fee / 5;
            burn(address(this), _portionFee);
            _toStakingRewards(_portionFee);
            _transfer(address(this), info.DAO, _portionFee);
            _transfer(address(this), info.ops, _portionFee);
            // Record amount to add liquidity in next _transfer()
            // Must put last to prevent transfer error
            info.amountToLiquify += _portionFee;
        }
        return true;
    }

    function _addLiquidity(uint256 amount) private {
        info.liqInProgress = true;
        uint256 halfAmt = amount / 2;
        address[] memory path = new address[](2);
        path[0] = address(this);
        // path[1] = router.WETH();
        path[1] = info.router.WAVAX();
        // uint256 WETHAmt = info.router.swapExactTokensForETH(
        uint256 WETHAmt = info.router.swapExactTokensForAVAX(
        // uint256 WETHAmt = info.router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            halfAmt, 0, path, address(this), block.timestamp
        )[1];
        info.router.addLiquidityAVAX{value: WETHAmt}(
            address(this), halfAmt,
            0, 0, info.DAO, block.timestamp
        );
        info.amountToLiquify = 0;
        info.liqInProgress = false;
    }

    receive() external payable {}

    function _toStakingRewards(uint256 _amount) private {
        uint256 _halfAmount = _amount / 2;
        // _transfer(address(this), stakingRewardsAddress(), _halfAmount);
        info.stakingRewards.disburse(_halfAmount);
        // _transfer(address(this), lpStakingRewardsAddress(), _halfAmount);
        info.lpStakingRewards.disburse(_halfAmount);
    }
    ...
    function _isExcludedFromFee(address _from, address _to) internal view returns (bool) {
        return isFromWhitelisted(_from) || isToWhitelisted(_to)
            ...
            || _from == info.owner || _to == info.owner
    ...
    function isTransferUnlocked() external view returns (bool) {
        return info.transferUnlocked;
    }

    function isClaimedREBASED(address _account) public view returns (bool) {
        return info.users[_account].isClaimed;
    }
    ...
    function claimInfoFor(address _user) external view returns (bool unlocked, bool claimedREBASED, uint256 wethReserve, uint256 REBASEDReserve) {
        unlocked = isUnlocked(_user);
        claimedREBASED = isClaimedREBASED(_user);
        ( , , wethReserve, REBASEDReserve, , , ) = allInfoFor(address(0x0));
    }

    function allInfoFor(address _user) public view returns (uint256 totalTokens, uint256 totalLPTokens, uint256 wethReserve, uint256 REBASEDReserve, uint256 userBalance, uint256 userRewards, uint256 userLPBalance) {
        ...
        REBASEDReserve = info.weth0 ? _res1 : _res0;
...
contract StakingRewards {
    ...
    uint256 constant private FLOAT_SCALAR = 2**64;
    uint256 constant private PERCENT_FEE = 5; // only for REBASED staking
    ...
    function _initialize(uint256 _totalRewards, uint256 _stakingRewardsStart, ERC20 _token) external {
        ...
        info.REBASED = REBASED(payable(msg.sender));
        info.token = _token;
    }
    ...
    function update() public {
        ...
        if (_now > info.lastUpdated && totalDeposited() > 0) {
            ...
            uint256 _balanceBefore = info.REBASED.balanceOf(address(this));
            info.REBASED.claimRewards();
            _reward += info.REBASED.balanceOf(address(this)) - _balanceBefore;
    ...
    function tokenCallback(address _from, uint256 _tokens, bytes calldata) external returns (bool) {
        require(_isREBASED() && msg.sender == tokenAddress());
        require(_tokens > 0);
        update();
    ...
    function disburse(uint256 _amount) public {
        ...
        update();
        uint256 _balanceBefore = info.REBASED.balanceOf(address(this));
        info.REBASED.transferFrom(msg.sender, address(this), _amount);
        uint256 _amountReceived = info.REBASED.balanceOf(address(this)) - _balanceBefore;
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            info.users[msg.sender].scaledPayout += int256(_rewards * FLOAT_SCALAR);
            info.REBASED.transfer(msg.sender, _rewards);
    ...
    function REBASEDAddress() public view returns (address) {
        return address(info.REBASED);
    }
    ...
    function allInfoFor(address _user) external view returns (uint256 startingTime, uint256 totalRewardsDistributed, uint256 rewardsRatePerDay, uint256 currentFeePercent, uint256 totalTokensDeposited, uint256 virtualRewards, uint256 userREBASED, uint256 userBalance, uint256 userAllowance, uint256 userDeposited, uint256 userRewards) {
        ...
        userREBASED = info.REBASED.balanceOf(_user);
    ...
    function _isREBASED() internal view returns (bool) {
        return REBASEDAddress() == tokenAddress();
    }

    function _calculateFee(uint256 _amount) internal view returns (uint256) {
        return _isREBASED() ? (_amount * PERCENT_FEE / 100).mul(1e18 - _sum(_getX(block.timestamp))) : 0;
    }
