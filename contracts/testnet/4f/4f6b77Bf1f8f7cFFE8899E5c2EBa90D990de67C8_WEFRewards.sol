...
contract WEFRewards is IRewards, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    // --- Auth ---
    ...
    function distributionApy(address token) public view returns(uint256) {
        return (dMath.rpow(pools[token].rewardRate, YEAR, ONE) - ONE) / 10 ** 7;
    }
//
    ...
    function unrealisedRewards(address token, address usr) public poolInit(token) view returns(uint256) {
        ...
        uint256 rate = dMath.rpow(pools[token].rewardRate, block.timestamp - last, ONE);
