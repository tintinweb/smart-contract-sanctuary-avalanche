...
contract TokenMigrator is Ownable, PermissionsEnumerable {
    ERC721Base public oldToken;
    ERC721Base public newToken;

    constructor(address _oldToken, address _newToken) {
        oldToken = ERC721Base(_oldToken);
        newToken = ERC721Base(_newToken);
        _setupOwner(msg.sender);
    ...
    function migrateToken(uint256 tokenId) internal {
        address tokenOwner = oldToken.ownerOf(tokenId);
        require(
            msg.sender == tokenOwner,
            "TokenMigrator: caller is not the token owner"
        );
        oldToken.transferFrom(tokenOwner, address(this), tokenId);
        newToken.transferFrom(owner(), tokenOwner, tokenId);
    }
