...
contract Sale is SaleToken, Ownable {
    ...
    function availableAllocation(address participant) public view onlyInitialized returns (uint256) {
     ...
     */
    function buyToken(address depositAddress, uint256 amount) external onlyInitialized {
        ...
        uint256 usedAllocationPoint = amountOfSellingTokens.mul(10**decimals()).div(balanceOf(address(this)));
     ...
     * @param amount The number of tokens that are approved
     */
    function buyTokenOnFCFS(address depositAddress, uint256 amount) external onlyInitialized {
    ...
    function getState() external view returns (uint256) {
        return
            !isInitialized ? uint256(States.NOT_INITIALIZED) : (block.timestamp < sale.startTime) ? uint256(States.WAITING_FOR_SALE) : (block.timestamp >= sale.startTime && block.timestamp <= sale.endTime) ? uint256(States.SALE) : (block.timestamp > sale.endTime && block.timestamp < fcfs.startTime)
                ? uint256(States.WAITING_FOR_FCFS)
                : (block.timestamp >= fcfs.startTime && block.timestamp <= fcfs.endTime)
                ? uint256(States.FCFS)
                : uint256(States.CLAIM);
    }
    ...
    function _getAmountOfSellingTokens(address depositAddress, uint256 amount) private view returns (uint256) {
        uint256 depositTokenPriceInUSD = farmLensAddress.getPriceInUsd(depositAddress);

        return amount.mul(depositTokenPriceInUSD).div(tokenPriceInUSD).mul(10**sellingTokenAddress.decimals()).div(IERC20Decimals(depositAddress).decimals());
    }
...
contract SalesFactory is Ownable, ReentrancyGuard {
    ...
    function deploySale(address sellingTokenAddress) external onlyOwner {
        ...
        Sale sale = new Sale(msg.sender, _signatory, address(stakingAddress), address(farmLensAddress), sellingTokenAddress, _appendStrings(sellingTokenSymbol, " AVATA Sale"), _appendStrings("iAVAT-", sellingTokenSymbol));
