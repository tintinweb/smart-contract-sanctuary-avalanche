...
contract poolBet {
  ...
  event ProposeWinner(address indexed winningAddress, string winner, uint poolId);
  ...
  **/
  ...
  function canRecognizeWinner(address proposedWinnerAddress, uint poolId) public view returns(bool) {
    Pool storage currentPool = poolMapping[poolId];
  ...
  function canUndoRecognizeWinner(address proposedWinnerAddress, uint poolId) public view returns(bool) {
    Pool storage currentPool = poolMapping[poolId];
    return !currentPool.isWinnerRecognized
    && proposedWinnerAddress != address(0)
    && currentPool.winner == proposedWinnerAddress
  ...
  function canProposeWinner(uint poolId) public view returns(bool) {
    Pool storage currentPool = poolMapping[poolId];
    return msg.sender == currentPool.moderator
    && currentPool.winner == address(0)
    && !currentPool.isWinnerRecognized
  ...
  function proposeWinner(address proposedWinnerAddress, uint poolId) public virtual onlyModerator(poolId) onlyWhenPoolActive(poolId) {
    ...
    emit ProposeWinner(proposedWinnerAddress, userMapping[proposedWinnerAddress], poolId);
  }
