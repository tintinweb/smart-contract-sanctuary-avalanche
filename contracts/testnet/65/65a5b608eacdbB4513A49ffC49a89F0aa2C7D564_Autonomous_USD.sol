...
contract Autonomous_USD is IERC20 {
    ...
    string constant _name = "Autonomous_USD";
    string constant _symbol = "USDA";
    ...
    uint public lastInflationUpdatedAt;
   ...
   uint higherThreshold = 1005000;
   uint lowerThreshold = 1000000;
   uint appreciationRate = 5;
   bool appreciationMode = true;
...
function _buyFixing(uint amount) private {
    ...
    function _shouldUpgradePrice() public view returns(bool){
        ...
        if(mode == 1){
            sure = block.timestamp < periodThreshold;
        }     
    ...
    function _upgradePrice() private returns(uint){
        ...
        if(mode == 2){
            for(uint i = 0; i < period; i++){
                ...
                isOver = (block.timestamp).sub(lastInflationUpdatedAt) > periodThreshold;
            }
        }
        ...
        if(isOver && mode == 2){
            lastInflationUpdatedAt = block.timestamp;
        }
    ...
    function periodInflation(uint _inflationRate, uint _periodThreshold, uint _upgradingPeriod) external onlyManager{
    ...
    function upgradeAppreciation(uint _upto, uint _downTo, bool on) external onlyManager{
       if(on == true){
              higherThreshold = _upto;
              lowerThreshold = _downTo;
              uint distance = _upto.sub(_downTo);
              require(distance > 1000);
              appreciationRate = distance.div(1000);
              appreciationMode = true;
       }else{
           appreciationMode = false;
       }
