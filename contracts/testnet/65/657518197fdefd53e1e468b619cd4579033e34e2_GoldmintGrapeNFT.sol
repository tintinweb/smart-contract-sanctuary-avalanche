...
contract GoldmintGrapeNFT is Ownable, RoyaltiesAddon, ERC2981 {
    ...
    function mint(uint256 _tokenAmount) public payable saleIsOpen {
        ...
        if(WhitelistSale){
            ...
            require(_tokenAmount == 1, "Whitelist limit per user");
            require(users[wallet].whitelist_minted < 1, "Whitelist mint per user total"); 
            require(msg.value >= whitelistPrice(_tokenAmount), "Value below price");  

            _mintAnElement(wallet, total + 1);   
            users[wallet].whitelist_minted = users[wallet].whitelist_minted.add(1); 
