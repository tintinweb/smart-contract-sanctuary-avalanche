...
interface ITaxable {
    ...
    function setHOSTOracle(address _HOSTOracle) external;
...
contract TaxOfficeV2 is Initializable, OwnableUpgradeable {
    ...
    function setTaxTiersTwap(uint8 _index, uint256 _value) public onlyOwner returns (bool) {
        return ITaxable(HOST).setTaxTiersTwap(_index, _value);
    }

    function setTaxTiersRate(uint8 _index, uint256 _value) public onlyOwner returns (bool) {
    ...
    function excludeAddressFromTax(address _address) external onlyOwner returns (bool) {
    ...
    function includeAddressInTax(address _address) external onlyOwner returns (bool) {
        ...
        )
    {
        ...
        uint256 liquidity;
        (resultAmtHOST, resultAmtEth, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            HOST,
            amtHOST,
            amtHOSTMin,
            amtEthMin,
    ...
    function setTaxableHOSTOracle(address _HOSTOracle) external onlyOwner {
        ITaxable(HOST).setHOSTOracle(_HOSTOracle);
    }

    function transferTaxOffice(address _newTaxOffice) external onlyOwner {
