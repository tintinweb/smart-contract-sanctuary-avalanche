...
/// @notice Manage list of company-debts
...
///         This contract doesn't make actual paying.
///         It only marks debts as paid and return list of sums to controller.
///         Controller make all payments itself.
contract DebtsManager is DebtsManagerStorage {
  ...
  override // IDebtsManager
  {
    ...
    requestsData[requestUid_] = RequestData({
      ...
      , epoch: currentEpoch //TODO this value should be passed here instead requestUid_
    });
...
//      earnedDollars: addUSD(ws.earnedDollars, amountUSD)
//    });
  ...
  external {
    ...
    for (uint i = 0; i < lenDepartments; i = _uncheckedInc(i)) {
      ...
      weekBudgetLimitsForRolesST[departmentUid] = cm.getMaxWeekBudgetForRolesST(departmentUid);
    }
  ...
  function getRequestWorker(RequestUid requestUid_) external view returns (WorkerUid) {
    ...
    return requestsData[requestUid_].worker;
  }
