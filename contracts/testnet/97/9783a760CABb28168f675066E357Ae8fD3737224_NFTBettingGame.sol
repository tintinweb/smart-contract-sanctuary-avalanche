...
contract NFTBettingGame is ERC721, ERC721URIStorage, Ownable {
    ...
    //distribute the reward to top 10 winners
    function distributeRewards() public payable onlyOwner {
        ...
        // 80% of total pot
        uint256 totalRewards = (pot * 80) / 100;
        for (uint256 i = 0; i < 10; i++) {
            if (i >= bettors.length) {
                break;
            }
            // distribute rewards in ascending form
            rewards[i] = (totalRewards * (i + 1)) / 55;
            // transfer rewards to the winner
            payable(bettors[i]).transfer(rewards[i]);
        }

        // keep 20% of the total pot in treasury
        treasuryPool += (pot - totalRewards);

        winner = payable(bettors[0]);
    }

    //it refunds to the betters
