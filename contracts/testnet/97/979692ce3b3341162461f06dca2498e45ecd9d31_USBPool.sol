...
contract USBPool is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function sellUSB(uint256 usPeggedTokenIndex, uint256 inputUSBAmount) external {
        ...
        if(sellingFees > 0) {
            usPegCoin.safeTransfer(reservePool,sellingFees);
        }

        _lockReward(usPegCoin, msg.sender, (usPeggedAmount-sellingFees), vestingDuration);
    ...
    */
    ...
    function provideLiquidity(uint256 usPeggedTokenIndex, uint256 usPeggedAmount) external onlyAuthorized {
        ...
        usPegCoin.safeTransferFrom(msg.sender, address(this), usPeggedAmount);
        systemCoin.mint(msg.sender,usbAmount);
    ...
    function paybackUSB(address account, uint256 usbAmount) external {
        ...
        systemCoin.burn(msg.sender, usbAmount);
    ...
    function addUSPeggedCoin(ISystemCoin _USPeggedCoin) external onlyOwner {
     ...
     *************************************************************/
    function initialize(address _systemCoin, IUSBSwapLocker _swapLocker, uint256 _vestingDuration, address _reservePool, uint256 _devFee) public initializer {
        BIPS_DIVISOR = 10000;
