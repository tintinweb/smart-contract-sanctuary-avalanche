/**
 *Submitted for verification at testnet.snowtrace.io on 2022-11-11
*/
...
contract BEP1155Upgradeable is Initializable, ContextUpgradeable, BEP165Upgradeable, IBEP1155Upgradeable, IBEP1155MetadataURIUpgradeable {
    ...
    function _laztmint(
        ...
        emit TransferSingle(operator, from, account, id, amount);
...
contract Lazy1155 is Initializable,BEP1155Upgradeable, OwnableUpgradeable, BEP1155BurnableUpgradeable {
   event Approve(
    ...
    function initialize() public initializer {
        BEP1155Upgradeable.__BEP1155_init("", "OPENSALE", "OPNS");
        __Ownable_init();
        _tid = 1;
    ...
    function minting(string memory ipfsmetadata, uint256 royal, address from, address to, uint256 nooftoken, uint256 fromtoken) internal returns(uint256){
        _tid = _tid.add(1);
        uint256 id_ = _tid.add(block.timestamp);
        _laztmint(from, to, id_, nooftoken, fromtoken, "");
        _setTokenURI(id_, ipfsmetadata);
        _creator[id_]= from;
        _royal[id_]=royal.mul(1e18);
        return id_;
    }
    function lazyMint(uint256 royal, uint256 nooftoken, uint256 fromtoken, string memory ipfsmetadata, address payable from,uint256 amount, string memory bidtoken, string memory status) public payable{
        uint256 id_ = 0;
        if(keccak256(abi.encodePacked((status))) == keccak256(abi.encodePacked(("Buy")))){
            id_ = minting(ipfsmetadata, royal, from, msg.sender, nooftoken, fromtoken);
            _saleToken(from, id_, amount, bidtoken);
            if(fromtoken > 0){
                _orderPlace(id_, amount.div(nooftoken) ,from);
            }
        }   
        else{
            id_ = minting(ipfsmetadata, royal, msg.sender, from, nooftoken, fromtoken);
            _acceptBId(bidtoken, from, amount, id_);
            if(fromtoken > 0){
                _orderPlace(id_, amount.div(nooftoken) ,msg.sender);
            }
        }
       emit TokenId(msg.sender, id_);
    }
    ...
    function _orderPlace(uint256 tokenId, uint256 _price, address from) internal {
        Order memory order;
    ...
    function orderPlace(uint256 tokenId, uint256 _price) public{
        ...
        _orderPlace(tokenId,_price,msg.sender);
    }
