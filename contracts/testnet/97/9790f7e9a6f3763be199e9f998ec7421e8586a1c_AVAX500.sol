...
// File: interfaces/IJoeFactory.sol
...
// File: interfaces/IJoeRouter02.sol
...
contract AVAX500 is ERC20, Ownable, PaymentSplitter {
	using SafeMath for uint256;

	address public joePair;
	address public joeRouterAddress = 0xf886a74660f50DC81418a19B8570460E6c54feFf; // TraderJoe Router
	...
	constructor(
		address[] memory payees, // This is used to indicate payment to which address
		uint256[] memory shares, // this is indicating how much % those addresses are paid
		address[] memory addresses, // array [TeamPool, rewardsPool, nodeManager]
		uint256[] memory fees, // array [teamPoolFee, rewardsFee, liquidityPoolFee, cashoutFee, rwSwap]
		uint256 swapAmount // how many tokens for 1 avax
	) ERC20('AVAX500', 'A500') PaymentSplitter(payees, shares) {
		require(
		...
		require(fees[0] != 0 && fees[1] != 0 && fees[2] != 0 && fees[3] != 0, 'CONSTR:3');
		teamPoolFee = fees[0];
		rewardsFee = fees[1];
	...
	function createNodeWithTokens(string memory uname, uint256 amount_) external {
		address sender = _msgSender();
		require(bytes(uname).length > 3 && bytes(uname).length < 32, 'NC:1');
		...
		nodeManager.createNode(sender, uname, amount_);
	}
