...
contract Pool is OwnableUpgradeable, ReentrancyGuardUpgradeable, IERC20 {
  ...
  function setDepositIndex(CompoundingIndex depositIndex_) external onlyOwner {
    require(AddressUpgradeable.isContract(address(depositIndex_)) || address(depositIndex_) == address(0), "Must be a contract");
    _depositIndex = depositIndex_;
    if (address(depositIndex_) != address(0)) {
      _updateRates();
    }
  }
  ...
  * @dev borrowIndex_ the address of borrow index
  ...
  function setBorrowIndex(CompoundingIndex borrowIndex_) external onlyOwner {
    require(AddressUpgradeable.isContract(address(borrowIndex_)) || address(borrowIndex_) == address(0), "Must be a contract");
    _borrowIndex = borrowIndex_;
    if (address(borrowIndex_) != address(0)) {
      _updateRates();
    }
  }
