...
contract SmolNothing is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    using Strings for uint256;

    uint256 public constant MAX_MINTABLE = 250; //  250 total for this launchpad
    uint256 public constant MAX_PER_CLAIM = 1;
    uint256 public constant ROYALTY_VALUE = 900; // 100 = 1%, 200 = 2% etc...  10%
    uint256 public constant MAX_EARLY_CLAIM = 1;
    uint256 public constant MAX_PUBLIC_CLAIM = 1;
    uint256 public constant NFT_PRICE = 0.01 ether; // 0.01 avax for this launchpad
    uint public startAt;
...
//
    ...
    function premint(uint256 n, address _adr) public onlyOwner {
        require(n > 0, "Number need to be higher than 0");
        require(n <= MAX_PER_CLAIM, "you can't claim that much at ounce");

        for (uint256 i = 0; i < n; i++) {
            _safeMint(_adr, minted);
            _setTokenRoyalty(minted, royaltyRecipient, ROYALTY_VALUE);
