...
abstract contract BavaBaseGlpVault is JonesOperableVault, PausableUpgradeable {
     ...
     */
    function setStrategyAddress(IJonesGlpLeverageStrategy _strategy) external onlyRole(GOVERNOR_ROLE) {
        strategy = _strategy;
    }

    function setExcessReceiver(address _receiver) external onlyRole(GOVERNOR_ROLE) {
        receiver = _receiver;
    }
...
import {BavaBaseGlpVault} from "./BavaBaseGlpVault.sol";
import {IAggregatorV3} from "../../interfaces/IAggregatorV3.sol";
import {IJonesGlpLeverageStrategy} from "../../interfaces/IJonesGlpLeverageStrategy.sol";
import {IStakedGlp} from "../../interfaces/IStakedGlp.sol";

contract BavaGlpStableVault is
    Initializable,
    UUPSUpgradeable,
    BavaBaseGlpVault
{
    ...
    //     uint256 _assets,
    //     address _receiver
    // ) public override(BavaBaseGlpVault) whenNotPaused returns (uint256) {
    //     return super.deposit(_assets, _receiver);
    // }
    ...
    function redeemExactTokens(
        ...
        require(assets <= previewAssets, "ERC4626: redeem more than preview");
        _withdraw(_msgSender(), receiver, owner, assets, shares);
    ...
    function borrow(
        uint256 _amount
    ) external virtual onlyBorrower whenNotPaused returns (uint256) {
    ...
    function repay(
        uint256 _amount
    ) external virtual onlyBorrower returns (uint256) {
...
abstract contract JonesGovernableVault is AccessControlUpgradeable {
...
abstract contract JonesOperableVault is
    JonesGovernableVault,
    ERC4626Upgradeable
{
    function deposit(
        uint256 _assets,
