...
//slither-disable-next-line locked-ether
contract FatTadpoleNFT is ERC721 {
    address public owner;
    ...
    constructor(string memory _NFT_NAME, string memory _NFT_SYMBOL) ERC721(_NFT_NAME, _NFT_SYMBOL) {
        owner = msg.sender;
    }
    ...
    function addMintAddress(address adr) external onlyOwner {
        mintAddress[adr] = true;
    }

    function removeMintAddress(address adr) external onlyOwner {
        delete mintAddress[adr];
    }
    ...
    function mint(address _receiver) external {
        if (mintAddress[msg.sender] != true) {
            revert Unauthorized();
        }
        unchecked {
            uint256 tokenId = nextTokenID++;
...
// FatTadpole 0x4db672b7d426f0ebe8e2707a10aa6d86c97d69d1
// FattestTadpole 0xeee8332e15876ea6018a0c461b6531f92ad0efea
...
contract FattestTadpole {
    ...
    function claim() external {
        ...
        FatTadpoleNFT fatTadpole = FatTadpoleNFT(FAT_TADPOLE);
        for (uint256 i = 0; i < claimAmount; i++) {
            fatTadpole.mint(msg.sender);
            unchecked {
