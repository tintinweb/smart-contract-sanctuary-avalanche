...
contract BetUpCore is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function isAvailableMint(address _address, uint256 _nextMintedCount) external view returns(bool) {
        ...
        if(condition.count == 0 && condition.amount == 0) {
            return false;
        }

        IBetStruct.PlayerRecord memory player = playerRecords[_address];
        uint256 num = player.cumulative - player.consumption;

        if( num >= condition.count && player.accAmount >= condition.amount * 10 ** 6) {
            return true;
        }
        return false;
    }
