    ...
    IPack
{
    ...
    function setQuackCount(uint256[] memory quackCount) external onlyRoleWithSwitch(minterRole) { 
        if(quackIDsRemaining.length == 0) { 
            quackIDsRemaining = quackCount;
        } else { 
            for( uint i= 0; i < quackCount.length; i += 1) { 
                quackIDsRemaining.push(quackCount[i]);
            }
        }
    }
    ...
    function addToQuackCount(uint256 quackCount) external onlyRoleWithSwitch(minterRole) { 
        ...
        for (uint256 j = 0; j < quackCount; j += 1) {
            quackIDsRemaining.push(quackIDsRemaining.length+j);
        }
    }
