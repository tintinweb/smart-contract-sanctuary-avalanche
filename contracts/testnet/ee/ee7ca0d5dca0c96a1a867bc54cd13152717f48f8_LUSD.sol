...
////// /nix/store/8xb41r4qd0cjb63wcrxf1qmfg88p0961-dss-6fd7de0/src/lusd.sol
...
contract LUSD is LibNote {
    ...
    modifier auth() {
        require(wards[msg.sender] == 1, "Lusd/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string public constant name = "Lusd Stablecoin";
    string public constant symbol = "LUSD";
    string public constant version = "1";
    uint8 public constant decimals = 18;
    uint256 public totalSupply;
    uint256 public dailyLUSDLimit;

    mapping(address => uint256) public balanceOf;
    mapping(address => mapping(address => uint256)) private allowances;
    mapping(address => uint256) public nonces;
    mapping(address => uint256) public lastMintRestart;
    mapping(address => uint256) public lusdMintedToday;
    ...
    constructor(uint256 chainId_) {
        ...
        dailyLUSDLimit = 10000000000000000000000;
    }
    ...
    function transferFrom(
        ...
        require(balanceOf[src] >= wad, "Lusd/insufficient-balance");
        if (src != msg.sender && _allowance(src, msg.sender) != uint256(-1)) {
            require(_allowance(src, msg.sender) >= wad, "Lusd/insufficient-allowance");
    ...
    function adjustDailyLUSDLimit(uint256 _limit) external auth {
        dailyLUSDLimit = _limit;
    }

    function mint(address usr, uint256 wad) external {
        if (wards[msg.sender] == 0) {
            require(
                add(wad, lusdMintedToday[msg.sender]) <= dailyLUSDLimit ||
                    (sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyLUSDLimit),
                "Over daily LUSD Limit"
            );
            if (sub(block.number, lastMintRestart[msg.sender]) >= 6500) {
                lusdMintedToday[msg.sender] = wad;
                lastMintRestart[msg.sender] = block.number;
            } else {
                lusdMintedToday[msg.sender] = add(lusdMintedToday[msg.sender], wad);
            }
        }
    ...
    function burn(address usr, uint256 wad) external {
        require(balanceOf[usr] >= wad, "Lusd/insufficient-balance");
        if (usr != msg.sender && _allowance(usr, msg.sender) != uint256(-1)) {
            require(_allowance(usr, msg.sender) >= wad, "Lusd/insufficient-allowance");
    ...
    function permit(
        ...
        require(holder != address(0), "Lusd/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "Lusd/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "Lusd/permit-expired");
        require(nonce == nonces[holder]++, "Lusd/invalid-nonce");
