...
contract EarnNodeV1 is EarnNodeStorageV1 {
    ...
    function initialize() public initializer {
        ...
        EarnCoin = IERC20(0xffdedD90C4adec23f58982961581dc329c1Aad70);
    ...
    function updateEarnCoinAddress(address _address)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function updateCount(uint256 _num, bool _status)
        external
        onlyOwner
        returns (bool)
    {
        ...
        returns (bool)
    {
        ...
        require(
            nodeNameDetails[order.NodeName] == false,
            "EarnNodeV1: Name is already used"
        );
        ...
        require(
            tierDetails[_tier].rewardRate != 0 &&
                tierDetails[_tier].tierEMnodePrice <=
                EarnCoin.balanceOf(msg.sender),
            "EarnNodeV1: Invalid price or Insufficient balance"
        );

        // Transfer & Approve Section
        EarnCoin.safeTransfer(
            msg.sender,
            address(this),
            tierDetails[_tier].tierEMnodePrice
        );
        EarnCoin.approve(communityAddress, tierDetails[_tier].tierEMnodePrice);
        EarnCoin.safeTransfer(
            address(this),
            communityAddress,
            tierDetails[_tier].tierEMnodePrice
        ...
        )
    {
        ...
        uint256 number = uint256(block.timestamp - LastClaimTime) /
            tierDetails[tierId].intervalTimestamp;
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < ownerNodes[msg.sender].length; i++) {
            ...
            Node storage data = nodeDetails[NodeId];
            data.lastClaimTimestamp += (tierDetails[TierId].intervalTimestamp *
                Number);
        ...
        //Transfer & Approve Section
        EarnCoin.safeTransfer(owner(), address(this), totalReward);
        EarnCoin.approve(msg.sender, totalReward);
        EarnCoin.safeTransfer(address(this), msg.sender, totalReward);
