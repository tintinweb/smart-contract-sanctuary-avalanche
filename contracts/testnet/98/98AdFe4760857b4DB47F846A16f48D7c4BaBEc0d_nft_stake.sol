...
contract nft_stake {
    ...
    function getEm(uint256 _tokenId,uint _amount, address _sender,uint k) internal returns(NodeEntity[] memory){
    	NodeEntity[] storage nodes = ogOfUser[msg.sender];
    	
	
    	
    	if (_tokenId == 1) {
    		Stake_1[] storage id = stakes_1[_sender];
		Stake_1 storage go = id[id.length];
    	if (_tokenId == 2) {
    		Stake_2[] storage id = stakes_2[_sender];
		Stake_2 storage go = id[id.length];
    	}
    	if (_tokenId == 3) {
    		Stake_3[] storage id = stakes_3[_sender];
    		Stake_3 storage go = id[id.length];
    	}
    	if (_tokenId == 4) {
    		Stake_4[] storage id = stakes_4[_sender];
		Stake_4 storage go = id[id.length];
    ...
    function stake(uint256 _tokenId, uint256 _amount) public{
    		getEm( _tokenId,_amount,msg.sender,1);
    	}
    function unStake(uint256 _tokenId, uint256 _amount) public {
    		getEm( _tokenId,_amount,msg.sender,0);
    	}
