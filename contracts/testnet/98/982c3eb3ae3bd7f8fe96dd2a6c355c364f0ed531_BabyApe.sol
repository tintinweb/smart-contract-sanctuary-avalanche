...
contract BabyApe is ERC20, Ownable {
    ...
    function buyNode(string memory name, uint256 nodeType) external  {
      ...
      uint256 contractBabyApeBalance = balanceOf(_distributionPool);
      ...
      if (canProcess && !swapping) {
          ...
          if(_distributionSwapFee > 0) {
            uint256 distributionTokensToSwap = balanceOf(_distributionPool).mul(_distributionSwapFee).div(100);
            swapAndSendToFee(_distributionPool, distributionTokensToSwap);
          }

          super._transfer(_distributionPool, _distributionPool, balanceOf(_distributionPool));

          swapping = false;
      }

      super._transfer(sender, _distributionPool, price);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
      ...
      path[0] = _distributionPool;
      path[1] = uniswapV2Router.WETH();

      _approve(_distributionPool, address(uniswapV2Router), tokenAmount);
    ...
    function swapAndSendToFee(address destination, uint256 tokens) private {
      uint256 initialETHBalance = _distributionPool.balance;

      swapTokensForEth(tokens);
      uint256 newBalance = (_distributionPool.balance).sub(initialETHBalance);
    ...
    function swapAndLiquify(uint256 tokens) public {
      ...
      uint256 initialBalance = _distributionPool.balance;

      // Swaping the half
      swapTokensForEth(half);
      uint256 newBalance = _distributionPool.balance.sub(initialBalance);

      // Approving and adding liquidity
      _approve(_distributionPool, address(uniswapV2Router), tokens);
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        ...
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory())
            .createPair(_distributionPool, uniswapV2Router.WETH());
    }
