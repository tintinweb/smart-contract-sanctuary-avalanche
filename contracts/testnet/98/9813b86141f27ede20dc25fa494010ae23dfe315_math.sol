...
using SafeMath for uint256;
    ...
    rewardsPerHour = doPercentage(one, dailyRewardsPerc);
    (rewardsTMul,dayMultiple1) = getMultiple(last,timeStep,rewardsPerHour);
    dayMultiple_mul = rewardsPerHour.mul(doFraction(dayMultiple1,timeStep));
    dayMultiple_mul1 = doPercentage(rewardsPerHour,doFraction(dayMultiple1,timeStep));
    ...
    function calcReward(uint256 _dailyRewardsPerc,uint256 _timeStep, uint256 _lastClaimTime, uint256 _boost) public view returns (uint256[3] memory){
	    ...
	    uint256 _boost = _rewards_[1];
    	    ...
    	    return [_all.mul(24),_boost.mul(24),_rewards.mul(24)];
    	   }
