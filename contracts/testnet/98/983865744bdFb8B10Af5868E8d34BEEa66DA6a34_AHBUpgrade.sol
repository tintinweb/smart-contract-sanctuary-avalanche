...
contract AHBUpgrade {
    ...
    function hasLimitConditionMet(uint256 _stars) public view returns (bool) {
        ...
        if (_stars == 2) {
            if (threeStarsTracker.current() > 700) {
                isUpgradable = false;
            }
        }

        if (_stars == 3) {
            if (fourStarsTracker.current() > 500) {
                isUpgradable = false;
            }
        }

        if (_stars == 4) {
            if (fiveStarsTracker.current() > 300) {
                isUpgradable = false;
            }
        }

        if (_stars == 5) {
            if (sixStarsTracker.current() > 45) {
                isUpgradable = false;
            }
        }

        if (_stars == 6) {
            if (sevenStarsTracker.current() > 5) {
                isUpgradable = false;
            }
        }
