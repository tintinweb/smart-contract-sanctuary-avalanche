...
contract USBLiquidityPool is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function sellUSB(uint256 usPeggedTokenIndex, uint256 inputUSBAmount) external {
        ...
        require(usPegCoinInfo.swapEnabled == true, "same state");
        // uint256 liquidity = usPegCoinInfo.coinAddress.balanceOf(address(this));
        // require(liquidity >= inputUSBAmount, "Insuffient liq");
        ...
        if(sellingFees > 0) {
            usPegCoinInfo.coinAddress.safeTransfer(reservePool,sellingFees);
        }

        _lockReward(usPegCoinInfo.coinAddress, msg.sender, (usPeggedAmount-sellingFees), usPegCoinInfo.vestingDuration);
    ...
    */
    ...
    function provideLiquidity(uint256 usPeggedTokenIndex, uint256 usPeggedAmount) external onlyAuthorized {
        require(usPeggedAmount > 0, "P.A!=0");
        require(address(swapLocker) != address(0), "0 address");
        ...
        usPegCoin.safeTransfer(address(swapLocker), usPeggedAmount);
        systemCoin.mint(msg.sender,usbAmount);
