    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if(OverFlowPlayerStatus[playerId] == true){
            if(_currentUserStage  > 0){
                ...
                emit Comment2(PlayerIDAtSpecifyStage);
                ...
                // }
                // bytes32[] memory SelectPlayerId; 
                // if((PlayerIDAtSpecifyStage.length > 3) && totalNFTOnTile[_member.stage][previousJumpSide] > 0){
                //     emit Comment3(PlayerIDAtSpecifyStage.length);
