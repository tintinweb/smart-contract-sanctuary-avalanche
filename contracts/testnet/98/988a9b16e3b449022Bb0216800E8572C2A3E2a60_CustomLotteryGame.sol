    ...
    KeeperCompatibleInterface
{
    ...
    constructor(
        ...
        IERC20 gameToken_,
        address executor
    ) VRFConsumerBaseV2(VRF_COORDINATOR) ConvertAvax(LINK_TOKEN){
    ...
    function getGameInfo() external view returns (Game memory){
     ...
     function getLotteries(uint256 index) external view returns (Lottery memory){
        return lotteries[index];
    }

    /// @notice get participation fee for LotteryGameFactory contract
    function getParticipationFee() external view override returns (uint256) {
    ...
    /// @notice set Router contract address
    /// @param router_ router address
    function setRouter(address router_) external onlyOwner {
        router = Router(router_);
        IERC20(LINK).approve(router_, ~uint256(0));
    }
    ...
    /// @notice get time lock if the game will be locked
    /// @return timelock_ time lock for the locked lottery game
    ...
    function startGame(uint32 VRFGasLimit_) external nonReentrant {
        Lottery storage lastLotteryGame = lotteries[lotteries.length - 1];
        ...
        _fundSubscription(vrfRequired);
        
        // IKeeperRegistry(KEEPERS_REGISTRY).addFunds(
        //     chainlinkSettings.keeperId,
        //     keepersRequires
        // );

        router.addFunds(address(this), keepersRequires); 
    ...
    function _handleChainlinkWinnersResponce(bytes memory bytesData) internal {
        ...
        for (uint256 i = 0; i < game.countOfWinners; i++) {
    ...
    /// @dev register game for VRF work
    ...
    /// @dev fund VRF work
    /// @param amount amount of LINK token to pop up VRF with
...
interface ICustomLotteryGame {
    ...
    /// @notice start created game
    /// @param VRFGasLimit_ price for VRF
    function startGame(uint32 VRFGasLimit_) external;
...
contract ConvertAvax{
    ...
    function swapAvaxToLink() internal {
        ...
        PANGOLIN.swapExactAVAXForTokens{value: amountIn}(amountOutMin, path, address(this), block.timestamp + 1 hours);
    }
