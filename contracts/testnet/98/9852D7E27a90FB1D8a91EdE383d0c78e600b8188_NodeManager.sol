...
contract NodeManager is Ownable, Pausable {
    ...
    function queryDuePayment(address _account) public view returns (uint) {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
    		uint j;
    		for(uint i=0;i<nodes.length;i++){
    			if (block.timestamp >= nodes[i].nextDue){
    				j++;
    			}
    		}
    		return j;
    	}
    function queryFuturePayment(address _account) public view returns (uint) {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
    		uint j;
    		for(uint i=0;i<nodes.length;i++){
    			if (nodes[i].nextDue < (block.timestamp + feeCap)){
    				j++;
    			}
    		}
    		return j;
    ...
    function doPayments(address _account,uint256 payments) external {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	for(uint i=0;i<nodes.length;i++){
    		if (block.timestamp >= nodes[i].nextDue && payments != 0){
    ...
    function doFuturePayments(address _account,uint256 payments) external {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	for(uint i=0;i<nodes.length;i++){
    		if (nodes[i].nextDue < (block.timestamp + feeCap)  && payments != 0){
    			NodeEntity storage _node = nodes[i];
    			_node.nextDue += 30 days; 
    			payments -= 1;
    		}
