...
contract USBSwapLocker is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // User addresses data
    mapping(IERC20Upgradeable => address[]) private userAddresses;                  // Pid => userAddresses list
    mapping(IERC20Upgradeable => mapping(address => bool)) private addedAccount;    // Pid => userAddress => addedintoArray(true=added, false=not added)
    ...
    */
    ...
    function lockWithStartTime(IERC20Upgradeable token, address account, uint256 quantity, uint32 vestingDuration) public onlySwapContract(token) {
        ...
        if(addedAccount[token][msg.sender] == false) {
            userAddresses[token].push(msg.sender);
            addedAccount[token][msg.sender] = true;
        }
    ...
    */
    ...
    function getUserAddressesList(IERC20Upgradeable token) external view returns(address [] memory) {
        return userAddresses[token];
    }
