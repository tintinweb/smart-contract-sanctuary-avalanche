...
contract Auction {
    ...
    function lowerReserve(uint _reserve) external returns(uint)

    {
        ...
        directBuyPrice = _reserve; // The new direct buy price
    ...
    event loweredReserve(uint _reserve); // Event for lowering the reserve

    event NewBid(address bidder, uint bid); // A new bid was placed

    event NFTWithdrawn(address withdrawer); // The auction winner withdrawed the token

    event FundsWithdrawn(address withdrawer, uint256 amount); // The auction owner withdrawed the funds
...
// constructor
    ...
    function getOneNFT(address _NFT, uint _tokenId) external view returns(address _info) {
...
// events
    ...
    event collectionAdded(address _collection); // Event for when a collection is added

    event tokenAdded(address _collection, uint _tokenId); // Event for when a token is added

    event auctionCreated(address _auction); // Event for when an auction is created

    event managerFundsWithdrawn(uint _amount); // Event for when funds are withdrawn
