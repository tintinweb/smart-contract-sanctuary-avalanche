...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function getActiveZone() public view onlyOwner returns (uint) {
        return _activeZone;
    }
    ...
    function mintUniqueTokenTo(address account) public {
        ...
        uint amountToken = this.balanceOf(msg.sender);
        uint level = 1;
        uint discount = 0;

        if (whitelistUser.whitelistAddress != address(0) && !mintedAddressList[account]) {
            level = whitelistUser.level;
        }

        if (_activeZone == 1) {
            require(!mintedAddressList[account] && (level >= 1 && level <= 10), "NFKey: Zone rejection");
            discount = 100;
        } else if (_activeZone == 2) {
            require(!mintedAddressList[account] && (level >= 1 && level <= 3), "NFKey: Zone rejection");
            discount = 50;
        } else if (_activeZone == 3) {
            require(level >= 1 && level <= 10, "NFKey: Zone rejection");
            discount = 20;
        } else if (_activeZone == 4) {
            require(level == 1, "NFKey: Zone rejection");
        ...
        uint commission = _commission - (_commission * discount / 100);
