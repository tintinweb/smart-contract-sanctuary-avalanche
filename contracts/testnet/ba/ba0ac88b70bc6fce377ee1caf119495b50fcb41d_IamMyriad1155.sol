...
contract IamMyriad1155 is Initializable,BEP1155Upgradeable, OwnableUpgradeable, BEP1155BurnableUpgradeable, VerifySignature {
   event Approve(
    ...
    uint private saltvalue;
    function initialize() public initializer {
        BEP1155Upgradeable.__BEP1155_init("", "IamMyriad1155", "Myriad1155");
        __Ownable_init();   
        _tid = 1;
    ...
    function editDevAddress(address dev) public onlyOwner {
        devaddress = dev;
    }
    function enableClaim(bool enable) public {
        require(devaddress == msg.sender, "Access denied");
        openClaim = enable;
    }
    function claimNft(address from,uint256 tokenId,string memory _message, uint _nonce, bytes memory signature) public {
        _nonce = _nonce.add(tokenId);
        require(verify(msg.sender, msg.sender ,0, _message, _nonce, signature) == true,"Not vaild User");
        require(nonce[_nonce] == false,"signature already used");
        _setApprovalForAll(from, msg.sender,true);
        saleTokenTransfer(from, tokenId, 1);
        nonce[_nonce] = true;
    }
}