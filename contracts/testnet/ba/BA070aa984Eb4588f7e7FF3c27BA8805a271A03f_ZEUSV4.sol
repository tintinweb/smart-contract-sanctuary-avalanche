...
abstract contract NODERewardManagement {
    struct NodeEntity {
        string name;
    ...
    function getLastClaimTimeWithCreationTime(
        address account,
        uint256 creationTime
        ...
        for (uint256 i = 0; i < nodes.length; i++) {
            if (nodes[i].creationTime == creationTime) {
                node = nodes[i];
                break;
            }
        }
        return node.lastClaimTime;
    }

    function nodePrice() external view virtual returns (uint256);

    function zeusTotal() external view virtual returns (uint256);

    function rewardPerNode() external view virtual returns (uint256);

    function claimTime() external view virtual returns (uint256);

    function gateKeeper() external virtual returns (address);

    function token() external virtual returns (address);

    function totalNodesCreated() external view virtual returns (uint256);

    function totalRewardStaked() external view virtual returns (uint256);
    ...
    function _isNodeOwner(address account) external view virtual returns (bool);
    ...
    function _changeNodePrice(uint256 amount) external virtual;

    function _changeRewardPerNode(uint256 amount) external virtual;

    function _changeClaimTime(uint256 amount) external virtual;

    function _changeGasDistri(uint256 amount) external virtual;

    function gasForDistribution() external view virtual returns (uint256);

    function getUserNodes(address account)
        external
        view
        virtual
        returns (uint256);
...
abstract contract IMasterOfCoin {
    ...
    function getDueDate(string memory nodeId)
        external
        view
        virtual
        returns (uint256);
    ...
    function payFee(string memory nodeId) external virtual;

    function isDelinquent(string memory nodeId)
        external
        view
        virtual
        returns (bool);
...
contract ZEUSV4 is Ownable {
    ...
    IZeus public Zeus = IZeus(0x764a4A6f3326559b6Ef5F3F186AD1094FA044c43);
    ...
    mapping(uint32 => bool) public nodeTierFeeStatus;
    mapping(address => bool) public maiciousUsers;
    ...
    function updateNodeTierFeeStatus(bool status, uint32 nodeType)
        external
        onlyOwner
    {
        nodeTierFeeStatus[nodeType] = status;
    }
    ...
    function _restorNode() external onlyOwner {
    ...
    function restoreNode(
        ...
        uint256 feeThreshold = fee.sub(100000000000000);
    ...
    function getLastClaimTime(
        ...
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        return
            nodeRewardManager.getLastClaimTimeWithCreationTime(
                account,
                creationTime
            );
    }
    ...
    function createNodeWithTokens(
        ...
        // uint256 nodesTotal = nodeRewardManager.getUserNodes(sender);
        string memory nodeId = _getLastNodeId(
            sender,
            nodeType,
            block.timestamp
        );
    ...
    function _getLastNodeId(
        ...
        string memory nodeIdCreationTime = string(
            abi.encodePacked(userId, "_", Strings.toString(_time))
        );
    ...
    function hasUnpaidNode(uint32 nodeType, address account)
        public
        view
        returns (bool)
    {
        if (nodeTierFeeStatus[nodeType]) {
            string memory rewards = getNodesRewards(nodeType);
            bool status = false;
            string[] memory creationTimes = splitWithDelimiter(rewards);
    ...
    function cashoutAll(uint32 nodeType) public {
        address sender = _msgSender();
        bool status = hasUnpaidNode(nodeType, sender);
        require(!status, "Unpaid Node Exists");
        require(
            sender != address(0),
        ...
        if (swapLiquify) {
            uint256 fee_ = rewardAmount.mul(cashoutFee).div(100);
            rewardAmount = rewardAmount.sub(fee_);
            token.transferFrom(distributionPool, taxPool, fee_);
        }
    ...
    function cashoutReward(uint256 blocktime, uint32 nodeType) public {
        ...
        bool dueDate = _isDue(sender, blocktime, nodeType);
        require(!dueDate, "Node Expired");
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        ...
        public
    {
        ...
        for (uint256 i = 0; i < claimtimes.length; i++) {
            if (nodeTierFeeStatus[nodeType]) {
                bool isDue = _isDue(sender, claimtimes[i], nodeType);
                require(!isDue, "Expired node selected");
            }
            (uint256 rewardAmount, uint256 fee) = nodeRewardManager
                ._cashoutNodeReward(sender, claimtimes[i]);
    ...
    function _isDue(
        ...
        if (nodeTierFeeStatus[nodeType]) {
            uint256 dueDate = _getNodeDueDate(account, creationTime, nodeType);
    ...
    function _getRewardAmountOf(
        ...
        if (nodeTierFeeStatus[nodeType] && block.timestamp > dueDate) {
            return 0;
        }
