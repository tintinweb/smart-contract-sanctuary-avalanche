...
contract Pets is ERC721Enumerable, Pausable, Controllable, Restricted {
    ...
    address payable AdminWallet =
        payable(0x9F523A9d191704887Cf667b86d3B6Cd6eBE9D6e9);
    address payable devAddress =
        payable(0xCCf8234c08Cc1D3bF8067216F7b63eeAcb113756);

    event onNFTsOrdered(
        address buyer,
        uint256 indexed amount,
        bool indexed isLimitedMint
    );
        ...
        noReentrency
    {
        ...
        emit onNFTsOrdered(_msgSender(), amount, false);
        storeMintOrder(_msgSender(), amount, false);
    }

    function limitedMint(uint256 amount)
        external
        payable
        whenNotPaused
        onlyEOA
        noReentrency
    {
        ...
        _settleOrders(amount + 1);
        emit onNFTsOrdered(_msgSender(), amount, true);
    ...
    function core_mint(Order memory order) internal {
        ...
        emit onNFTDelivered(order.buyer, ordersSettled, order.isLimitedMint);
    }
    ...
    function dissociatePetFromRanger(uint256 petToken, uint256 rangerToken)
        external
        onlyEOA
        noReentrency
    {
    ...
    function _dissociatePetFromRanger(uint256 petToken, uint256 rangerToken)
        internal
    {
    ...
    function getBoostToken(uint256 tokenId)
        public
        view
