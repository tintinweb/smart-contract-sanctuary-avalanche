...
contract Test is ERC721Upgradeable {
    ...
    address private _buyRoyaltiesAddr; // royality receives for buy 購買版稅接收地址
    uint256 public royaltyPercentage; // royalty based on sales price 賣出版稅%數
    mapping(address => bool) public excludedList; // list of people who dont have to pay fee 無需付費的人員名單

    //GB HOLDERS EARLY ACCESS TOKENS GB持有者提前獲取代幣
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            splitBalance(msg.value/numberOfToken);
        }
    }
...
*/
    ...
    function splitBalance(uint256 amount) private {
        ...
        payable(_adminAddr).transfer(mintingShare1);
    }
