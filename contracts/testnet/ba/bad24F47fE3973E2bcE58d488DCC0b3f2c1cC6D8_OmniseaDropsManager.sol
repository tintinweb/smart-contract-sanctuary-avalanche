...
contract OmniseaDropsManager is NonblockingLzApp, ReentrancyGuard {
    ...
    function mint(MintParams calldata _params) public payable nonReentrant {
        ...
        IONFT721A collection = IONFT721A(_params.coll);
        ...
        if (quantityPrice > 0) {
            ...
            (bool p1,) = payable(_params.coll).call{value : (quantityPrice * (100 - _fee) / 100)}("");
            require(p1, "OmniseaDropsManager: !p1");

            if (_fee > 0) {
                (bool p2,) = payable(_feeManager).call{value : (quantityPrice * _fee / 100)}("");
                require(p2, "OmniseaDropsManager: !p2");
            }
        }
        collection.mint(msg.sender, _params.quantity, _params.merkleProof);
    ...
    function _nonblockingLzReceive(
        ...
        IONFT721A collection = IONFT721A(params.coll);
