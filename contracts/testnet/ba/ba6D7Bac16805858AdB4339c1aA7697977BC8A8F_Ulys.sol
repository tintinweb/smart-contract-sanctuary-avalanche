...
contract Ulys is ERC20Burnable, Operator {
    ...
    event TaxHappened(address recipient);
    ...
    constructor(
        ...
        excludeAddress(address(_routerAddress));
    ...
    function transferFrom(
        ...
        if (taxRate != 0 && recipient == lpAddress && !swapping) {
            swapping = true;
    ...
    function _transferWithTax(
        ...
        emit TaxHappened(recipient);

        _approve(address(this), address(router), amount);
        _approve(sender, address(router), amount);
        IPangolinRouter(router).swapExactTokensForAVAXSupportingFeeOnTransferTokens(taxAmount, 0, path, taxCollectorAddress, block.timestamp + 100);
