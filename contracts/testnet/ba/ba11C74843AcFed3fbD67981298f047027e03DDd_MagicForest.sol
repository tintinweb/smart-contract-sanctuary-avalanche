...
contract MagicForest is IERC721Receiver, Pausable, Restricted {
    ...
    /**
     * Stakes a Ranger
     * @dev Rangers go to rangerStakes
     * @param account the address of the staker
    ...
    function _stakeRanger(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        rangerStakes[tokenId] = RangerStake(tokenId, uint32(block.timestamp), uint64(block.number), account);
        totalRangerStaked++;
    ...
    /**
     * Stakes a Skeleton
     * @dev Skeletons go to skeletonStakes
     * @param account the address of the staker
    ...
    function _claimRanger(
        ...
        if (unstake) {
            ...
            uint256 seed = uint256(keccak256(abi.encodePacked(blockhash(stake.stakeBlockNumber), tokenId)));
        ...
        if (isRanger) {
            ...
            if (stake.tokenId == tokenId) {
                ...
                return _calculateRangerRewards(
                    ...
                    walletToNumberStaked[stake.owner],
    ...
    function getMinimumToExit(uint256 tokenId) public view returns(uint256){
        ...
        RangerStake memory stake = rangerStakes[uint16(tokenId)];
        AttributesPets.Boost memory rangerBoost = pets.getRangerBoost(tokenId);
        AttributesPets.Boost memory walletBoost = pets.getWalletBoost(stake.owner);
    ...
    function timeRemainingBeforeExit(uint256 tokenId) external view returns (uint256) {
        ...
        uint256 lastClaim = rangerStakes[uint16(tokenId)].lastClaim;
