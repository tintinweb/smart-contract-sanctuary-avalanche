...
contract Farm is Ownable, FarmCore {
    ...
    function deposit(
        ...
        uint256 farmPairId = getFarmPairIdByNetwork(_farmPoolId);

        if (_amountFirst > 0 ) {
            _transferTokens(
                farmPairs[farmPairId].firstToken,
        ...
        if (_amountSecond > 0 ) {
            _transferTokens(
                farmPairs[farmPairId].secondToken,
                farmPools[_farmPoolId].depositAddress,
                _amountSecond
            );
        }
        ...
        if (depositCount == 0) {
            ...
            uint256 iterations = depositCount == 0 ? depositCount : depositCount - 1;

            for (uint i = 0; i <= iterations; i++) {
                if (users[_msgSender()].deposits[i].farmPoolId == _farmPoolId && isNew == true) {
                    isNew = false;
    ...
    function syncCrossChainDeposit(
        ...
        if (depositCount == 0) {
            ...
            for (uint i = 0; i <= iterations; i++) {
                if (users[_userAddress].deposits[i].farmPoolId == _farmPoolId && isNew == true) {
                    isNew = false;
