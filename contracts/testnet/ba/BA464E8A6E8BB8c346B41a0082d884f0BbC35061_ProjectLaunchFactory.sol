...
import {Sale} from "./contracts/Sale_V6.sol";
...
contract ProjectLaunchFactory {
  ...
  function createSale(
    ...
    IBEP20(_sale_token).transferFrom(msg.sender, address(this), totalTokens + totalLiquidityTokens);
    IBEP20(_sale_token).approve(address(sale), totalTokens + totalLiquidityTokens);
    IBEP20(_sale_token).transferFrom(address(this), address(sale), totalTokens + totalLiquidityTokens);
...
import "./Sale_V6_structs.sol";

contract Sale is ReentrancyGuard, saleStructs {
    ...
    function init_sale (
        ...
        if(_options[0] >= 4) {
            require(_seed_details[6] == 0 && _seed_details[7] == 0 && _presale_details[6] >= block.timestamp && _community_details[6] >= _presale_details[7], "7");
        } else if (_options[0] < 4){
    ...
    function userWithdrawBaseTokens (uint _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 3 || sale_info[_round].attempt > 0, "Not eligible for withdraw"); // FAILED
