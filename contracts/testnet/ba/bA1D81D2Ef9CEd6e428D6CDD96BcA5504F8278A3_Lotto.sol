...
contract Lotto is ILotto, VRFConsumerBaseV2 {
    ...
    address public constant AMES = 0xb9E05B4C168B56F73940980aE6EF366354357009;
    address public constant ASHARE = 0xFa4b16b0f63F5A6D0651592620D585D308F749A4;
    ...
    modifier onlyOwner(){
        require(
            ...
            'Only Cavern Crawl Owner Can Call'
        );
        _;
    }
    ...
    function register(address user, uint256 amount) external override {
        require(
            msg.sender == Miner,
            'Only Cavern Crawl Can Register Users'
        );
    ...
    function _sendDailyRewards(uint256[] memory random) internal {
        ...
        if (rewardPot > 0) {
            ...
            _distributeRewards(addr, rewardToken, rewardPot);
        }
    ...
    function _sendLargestDailyRewards() internal {
        ...
        if (largestDaily.amount > 0 && largestDaily.depositor != address(0)) {
            ...
            if (rewardPot > 0) {
                ...
                _sendToken(rewardToken, largestDaily.depositor, rewardPot);
            }
        }
    ...
    /**
        Distributes `rewardPot` amongst `recipients` in the token `rToken`
     */
    function _distributeRewards(address[] memory recipients, address rToken, uint rewardPot) internal {
        ...
        for (uint j = 0; j < length;) {
            if (recipients[j] != address(0)) {
                ...
                _sendToken(rToken, recipients[j], amt);
            }
            unchecked{ ++j; }
        }
    }
