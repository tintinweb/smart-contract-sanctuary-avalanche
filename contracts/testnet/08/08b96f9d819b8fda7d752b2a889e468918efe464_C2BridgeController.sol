    ...
    IBridgeController
{
    ...
    function bridge(uint16 _dstChainId, address _to, uint256 _amount) external payable nonReentrant whenNotPaused {
        ...
        bytes memory _adapterParams = _getAdapterParams();
        uint64 _nonce = bridgeProxy.sendTokens{value: msg.value}(_dstChainId, abi.encodePacked(_to), _amount, payable(msg.sender), address(0), _adapterParams);
...
contract C2BridgeController is BaseBridgeController {
    ...
    function _getAdapterParams() internal view override returns (bytes memory) {
        return abi.encodePacked(uint16(2), uint256(200000), uint256(350000000000000), 0x5fDCd1a8D90843A699563820eDdD3613C887E80e);
    }
}
