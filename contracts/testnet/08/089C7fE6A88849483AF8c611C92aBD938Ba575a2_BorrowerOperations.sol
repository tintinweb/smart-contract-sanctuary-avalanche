...
contract BorrowerOperations is LiquityBase, Ownable, IBorrowerOperations, ReentrancyGuard {
    ...
    string constant NAME = "BorrowerOperations";
    ...
    function openTrove(
        ...
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, yusdToken, controller);
    ...
    function _singleLeverUp(address _token, 
        ...
        uint _additionalTokenAmount = _amount.mul(_leverage.sub(DECIMAL_PRECISION)).div(DECIMAL_PRECISION); 
        _additionalYUSDDebt = _getValueUSD(contractsCache.controller, _token, _additionalTokenAmount);
        ...
        uint256 slippageAdjustedValue = _additionalTokenAmount.mul(DECIMAL_PRECISION.sub(_maxSlippage)).div(DECIMAL_PRECISION);
        ...
        _finalTokenAmount = IYetiRouter(router).route(address(this), address(contractsCache.yusdToken), _token, _additionalYUSDDebt, slippageAdjustedValue);
        require(erc20Token.balanceOf(address(contractsCache.activePool)) == balanceBefore.add(_finalTokenAmount), "4");
    }
    ...
    function _openTroveInternal(
        ...
        {
            uint256 variableFee = _getTotalVariableDepositFeeAndUpdate(
                contractsCache.controller,
                _colls, _amounts, vars.entireSystemColl, vars.VC, 0
            );
        ...
        if (vars.isRecoveryMode) {
            ...
            _requireNewTCRisAboveCCR(_getNewTCRFromTroveChange(vars.entireSystemColl, vars.entireSystemDebt, vars.VC, true, vars.compositeDebt, true)); // bools: coll increase, debt increase);
        }
    ...
    /**
     * @notice add collateral to trove
    ...
    /**
     * @notice Adjusts trove with multiple colls in / out
     * @dev Calls _adjustTrove with correct params
     */
    function adjustTrove(
        ...
        // Replaces calls to requireValidCollateral and condenses them into one controller call. 
        {
            ...
            _requireLengthCorrect(collsOutLen == _amountsOut.length && collsInLen == _amountsIn.length);
    ...
    function _adjustTrove(AdjustTrove_Params memory params, ContractsCache memory contractsCache) internal {
        ...
        if (params._isDebtIncrease) {
            _requireValidMaxFeePercentage(params._maxFeePercentage, vars.isRecoveryMode);
            _requireLengthCorrect(params._YUSDChange != 0);
        }
        ...
        if (params._isDebtIncrease) {
            vars.maxFeePercentageFactor = (vars.VCin >= params._YUSDChange) ? vars.VCin : params._YUSDChange;
        } else {
        ...
        (vars.currAssets, vars.currAmounts, vars.debt) = _getCurrentTroveState(contractsCache.troveManager);

        // current VC based on current portfolio and latest prices
        vars.currVC = contractsCache.controller.getValuesVC(vars.currAssets, vars.currAmounts);

        // get new portfolio in trove after changes. Will error if invalid changes:
        // transitioning back to internal func call might be better
        (vars.newAssets, vars.newAmounts) = _subColls( 
            _sumColls(newColls(vars.currAssets, vars.currAmounts), newColls(params._collsIn, params._amountsIn)), 
            params._collsOut, params._amountsOut);
        ...
        if (params._collsIn.length != 0) {
            vars.variableYUSDFee = _getTotalVariableDepositFeeAndUpdate(
                contractsCache.controller,
                params._collsIn,
                params._amountsIn,
            ...
            if (vars.variableYUSDFee != 0) {
                _requireUserAcceptsFee(vars.variableYUSDFee, vars.maxFeePercentageFactor, params._maxFeePercentage);
        ...
        vars.newICR = _computeCR(vars.newVC, // if debt increase, then add net debt change and subtract otherwise.
                params._isDebtIncrease ? 
                    vars.debt.add(vars.netDebtChange) : vars.debt.sub(vars.netDebtChange));
        ...
        if (vars.isRecoveryMode) {
            ...
            _requireNewTCRisAboveCCR(_getNewTCRFromTroveChange(
                vars.entireSystemColl,
        ...
        if (params._collsIn.length != 0) {
            _activePoolReceiveCollateral(contractsCache.activePool, params._collsIn, params._amountsIn);
        }
        if (params._isDebtIncrease) { // if debt increase, increase by both amounts
            vars.newDebt = _increaseTroveDebt(contractsCache.troveManager, vars.netDebtChange.add(vars.variableYUSDFee));
        } else {
            if (vars.netDebtChange > vars.variableYUSDFee) { // if debt decrease, and greater than variable fee, decrease 
                vars.newDebt = contractsCache.troveManager.decreaseTroveDebt(msg.sender, vars.netDebtChange - vars.variableYUSDFee); // already checked no safemath needed
            } else { // otherwise increase by opposite subtraction
                vars.newDebt = _increaseTroveDebt(contractsCache.troveManager, vars.variableYUSDFee - vars.netDebtChange);
            }
        }
        
        _updateTroveCollAndStakesAndTotalStakes(contractsCache.troveManager, vars.newAssets, vars.newAmounts);
    ...
    function _singleUnleverUp(
        ...
        uint valueOfCollateral = _getValueUSD(contractsCache.controller, _token, _amount);
        uint256 slippageAdjustedValue = valueOfCollateral.mul(DECIMAL_PRECISION.sub(_maxSlippage)).div(DECIMAL_PRECISION);
    ...
    function _unleverColls(
        ...
        uint256 balanceBefore = _yusdTokenBalanceOf(contractsCache.yusdToken, msg.sender);
        ...
        for (uint256 i; i < _colls.length; ++i) {
            ...
            if (_maxSlippages[i] != 0) {
                totalYUSDUnlevered = totalYUSDUnlevered.add(_singleUnleverUp(contractsCache, _colls[i], _amounts[i], _maxSlippages[i]));
            } else {
                _sendSingleCollateralUnwrap(contractsCache.activePool, _colls[i], _amounts[i]);
            }
        }
        require(_yusdTokenBalanceOf(contractsCache.yusdToken, msg.sender) == balanceBefore.add(totalYUSDUnlevered), "6");
    }
     ...
     *     of collateral and swap them for YUSD. Use that YUSD and YUSD from borrower's account to papy back 
     *     remaining debt. 
     * @dev Calls _adjustTrove with correct params. nonReentrant
     * @param _collsOut Collateral types to withdraw
     * @param _amountsOut Amounts to withdraw
    ...
    function _closeTrove(
            ...
            if (_isUnlever) {
                ...
                for (uint256 i; i < vars.colls.length; ++i) {
                    ...
                    if (j < _collsOut.length && vars.colls[i] == _collsOut[j]) {
                        totalYUSDUnlevered = totalYUSDUnlevered.add(_singleUnleverUp(contractsCache, _collsOut[j], _amountsOut[j], _maxSlippages[j]));
                    ...
                    if (thisAmount > 0) {
                        _sendSingleCollateralUnwrap(contractsCache.activePool, vars.colls[i], thisAmount);
                    }
                }
                require(_yusdTokenBalanceOf(contractsCache.yusdToken, msg.sender) == balanceBefore.add(totalYUSDUnlevered), "6");
            }
        ...
        _requireSufficientYUSDBalance(contractsCache.yusdToken, msg.sender, vars.debt.sub(YUSD_GAS_COMPENSATION));
        _requireNewTCRisAboveCCR(_getNewTCRFromTroveChange(vars.entireSystemColl, vars.entireSystemDebt, vars.troveVC, false, vars.debt, false));
        ...
        _repayYUSD(contractsCache.activePool, contractsCache.yusdToken, msg.sender, vars.debt.sub(YUSD_GAS_COMPENSATION));
     ...
     * to do all necessary interactions. 
     */
    ...
    function _singleTransferCollateralIntoActivePool(
      ...
      * @return YUSDFee Triggers the borrowing fee
      */
    ...
    function _mintYUSDFeeAndSplit(ContractsCache memory contractsCache, uint256 _YUSDFee) internal {
        (uint256 feeSplit, address yetiTreasury, address YUSDFeeRecipient) = contractsCache.controller.getFeeSplitInformation();
     ...
     */
    function _requireValidTroveCollsLen(IYetiController controller, uint _n) internal view {
     ...
     */
    function _requireValidCollateral(address[] memory _colls, uint256[] memory _amounts, IYetiController controller, bool _deposit) internal view {
        uint256 collsLen = _colls.length;
    ...
    function _requireSufficientYUSDBalance(
        ...
        require(
            _yusdTokenBalanceOf(_yusdToken, _borrower) >= _debtRepayment,
            "26"
        );
    }
    ...
    function _sendCollateralsUnwrap(IActivePool _activePool, address[] memory _collsOut, uint256[] memory _amountsOut) internal {
        _activePool.sendCollateralsUnwrap(msg.sender, msg.sender, _collsOut, _amountsOut);
    }

    function _sendSingleCollateralUnwrap(IActivePool _activePool, address _collOut, uint256 _amountOut) internal {
        _activePool.sendSingleCollateralUnwrap(msg.sender, msg.sender, _collOut, _amountOut);
    }

    function _increaseTroveDebt(ITroveManager _troveManager, uint256 _amount) internal returns (uint256) {
        return _troveManager.increaseTroveDebt(msg.sender, _amount);
    }

    function _updateTroveCollAndStakesAndTotalStakes(ITroveManager _troveManager, address[] memory _colls, uint256[] memory _amounts) internal {
        _troveManager.updateTroveColl(msg.sender, _colls, _amounts);
        _troveManager.updateStakeAndTotalStakes(msg.sender);
    }

    function _activePoolReceiveCollateral(IActivePool _activePool, address[] memory _colls, uint256[] memory _amounts) internal {
        _activePool.receiveCollateral(_colls, _amounts);
    }

    function _getCurrentTroveState(ITroveManager _troveManager) internal view returns (address[] memory, uint256[] memory, uint256) {
        return _troveManager.getCurrentTroveState(msg.sender);
    }

    function _getDefaultRouterAddress(IYetiController _controller, address _token) internal view returns (address) {
        return _controller.getDefaultRouterAddress(_token);
    }

    function _getValueUSD(IYetiController _controller, address _token, uint256 _amount) internal view returns (uint256) {
        return _controller.getValueUSD(_token, _amount);
    }

    // todo 
    function _getTotalVariableDepositFeeAndUpdate(IYetiController controller, address[] memory _colls, uint256[] memory _amounts, uint256 _entireSystemColl, uint256 _VCin, uint256 _VCout) internal returns (uint256) { 
        return controller.getTotalVariableDepositFeeAndUpdate(_colls, _amounts, _entireSystemColl, _VCin, _VCout);
    }

    function _yusdTokenBalanceOf(IYUSDToken _yusdToken, address _owner) internal view returns (uint256) {
        return _yusdToken.balanceOf(_owner);
    }
...
interface IYetiController {
    ...
    function changePriceCurve(address _collateral, address _priceCurve) external;
...
interface IPool is ICollateralReceiver {
    ...
    event DefaultPoolAddressChanged(address _newDefaultPoolAddress);
    event StabilityPoolAddressChanged(address _newStabilityPoolAddress);
    event YetiControllerAddressChanged(address _newYetiControllerAddress);
    event EtherSent(address _to, uint _amount);
