...
contract GenerateBytecode { 
    function createLotteryBytecode()    
     external pure returns(bytes memory){
        return  type(CustomLotteryGame).creationCode;
    }
}
    ...
    KeeperCompatibleInterface
{
    ...
    constructor(
        ...
        require(
            _isPercentageCorrect(game_.countOfWinners, game_.winnersPercentages)
        );
        // TODO: check the consition
        // require(
    ...
    function setGameCallerFeePercents(uint256 _amount) external onlyOwner {
        require(_amount < HUNDRED_PERCENT_WITH_PRECISONS);
    ...
    function setWinnersNumber(
        ...
        require(countOfWinners_ != 0 && countOfWinners_ <= WINNERS_LIMIT);
        require(winnersPercentages_.length == countOfWinners_);

        //check if sum of percentage is 100%
        require(_isPercentageCorrect(countOfWinners_, winnersPercentages_));
    ...
    function startGame(uint32 VRFGasLimit_, uint96 amount) external virtual {
        Lottery storage lastLotteryGame = lotteries[lotteries.length - 1];

        require(lastLotteryGame.isActive && lastLotteryGame.startedAt == 0);

        // router.addFunds(address(this), amount); -- TODO: check line
    ...
    function _handleChainlinkWinnersResponce(bytes memory bytesData) internal {
        ...
        require(lastLotteryGame.startedAt != 0 && lastLotteryGame.isActive);
        uint256 id = lastLotteryGame.id;
