...
contract NodeManager is Initializable {
  ...
  function _transferFee(uint256 amount) private {
    ...
    IERC20Upgradeable(tokenAddress).approve(address(this), amount);
  ...
  function transfer(
    ...
    if (fee > claimableAmount)
      ...
      IERC20Upgradeable(tokenAddress).transferFrom(address(rewardsPoolAddress), address(msg.sender), claimableAmount.sub(fee));
  ...
  function sellNode(uint32 indice) public {
    ...
    if (node.owner != address(0)) {
      ...
      countOfTier[tier.name]--;
      countTotal--;
      IERC20Upgradeable(tokenAddress).transferFrom(address(rewardsPoolAddress), address(node.owner), tier.price.div(100).mul(sellPricePercentage));
    }
  }
  ...
  function setUnpaidNodes() internal {
    uint32 count = 0;
