...
contract math2 {
		...
		uint256 public e;
		uint256 public f;
		uint256[] public ls  = doPercentage(5000,28*(10**16));
		...
		function elimZero(uint256 _y) internal returns(uint256){
		...
		function doPercentage(uint256 _x,uint256 perc) internal returns(uint256[4] memory){
			uint256 percDec = getDecimals(perc);
			uint denom =  20-percDec;
			uint trunc = elimZero(perc);
			uint[3] memory range = [20,denom,trunc];
			uint256 _y = _x.mul(10**range[1]);
			uint256 _z = _y.mul(perc);
			uint256 _w = _z.div(range[1]+2);
			return [_x,_y,_z,_w];
