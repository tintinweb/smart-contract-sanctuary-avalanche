...
// File: bava2/BavaMasterFarmerV2_3.sol
...
contract BavaMasterFarmerV2_3 is Ownable, Authorizable {
    ...
    struct PoolInfo {
        ...
        IPool poolContract;         // Address of autocompound Pool contract.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, IPool _poolContract, bool _withUpdate) external onlyOwner {        
    ...
    function updatePool(uint256 _pid) public {
        ...
        ( , , , uint256 lpSupply, ) = pool.poolContract.poolInfo();       // User lp token total supply not included compound lp reward token
        if (lpSupply == 0) {
    ...
    function poolRewardTokens(uint256 _pid) external view returns (address[] memory rewardToken) {
        PoolInfo memory pool = poolInfo[_pid];
        ( , , address[] memory rewardTokens, ) = pool.poolContract.poolRestakingInfo();       // User lp token total supply not included compound lp reward token
        return rewardTokens;
    }
}