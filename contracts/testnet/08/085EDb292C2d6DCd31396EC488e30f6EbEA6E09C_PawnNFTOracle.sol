    ...
    BlockContext
{
    ...
    function setAssets(
        ...
        for (uint256 i = 0; i < _nftContracts.length; i++) {
            _addAsset(_nftContracts[i], INFTOracle(_nftOracles[i]));
        }
    }

    function addAsset(address _nftContract, INFTOracle _nftOracle)
        external
        onlyOwner
    {
        _addAsset(_nftContract, _nftOracle);
    }

    function _addAsset(address _nftContract, INFTOracle _nftOracle) internal {
        ...
        returns (uint256)
    {
        ...
        uint256 _priceEx = nftOracleMap[_nftContract].getAssetPrice(_nftContract);
