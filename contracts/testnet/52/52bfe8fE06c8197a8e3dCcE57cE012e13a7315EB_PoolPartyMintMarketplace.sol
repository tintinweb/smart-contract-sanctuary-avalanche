...
contract PoolPartyMintMarketplace is Ownable {
  ...
  uint256 public feeMarketplace = 200;
  ...
  constructor(address _PaymentGatewayAddress, address payable _publisherWallet) {
  ...
  function changePublisherWallet(address _newAddr) public onlyOwner {
      require(_newAddr != address(0x0), "cant be 0x0");
      publisherWallet = payable(_newAddr);
  }

  function changeFee(uint256 _newFee) public onlyOwner{
      require(_newFee <= 10000, "invalid fee");
      feeMarketplace = _newFee;
  }

  function setOffer(uint256 price, uint256 tokenId, address tokenAddress, PoolPartyMint _OpenMint) public{
  ...
  function buyArt(uint256 tokenId, address tokenAddress, PoolPartyMint _OpenMint) public payable{
    ...
    _distributeFees(tokenId, offers[offer.offerId].price, offer.seller, _OpenMint);
  ...
  function _computePublisherFee(uint256 price) internal view returns(uint256){
    uint256 publisherFee = price * feeMarketplace / 10000;
    return publisherFee;
  }

  function _distributeFees(uint256 tokenId, uint256 price, address payable seller, PoolPartyMint _OpenMint) internal{
