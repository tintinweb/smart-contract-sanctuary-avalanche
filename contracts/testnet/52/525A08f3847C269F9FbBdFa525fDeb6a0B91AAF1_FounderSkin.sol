...
contract FounderSkin is ERC165, IERC721, IERC721Metadata, Ownable {
  ...
  bool public allTransfersLocked = false;
  ...
  constructor() {
    _name = "TS";
    _symbol = "SKN";
    // _name = "TimeShuffle Founder Skin";
    // _symbol = "FOUNDER SKIN";
    metadataUri = "";
  }

  function name() external view override returns (string memory name_ret) {
    return _name;
  }

  function symbol() external view override returns (string memory symbol_ret) {
    return _symbol;
  }
...
*/
  function approve(address _approved, uint256 _id) external override {
    if (msg.sender != owner() && allTransfersLocked) {
...
*/
  ...
  function airdropToAddresses(address[] memory _addresses) external onlyOwner {
    uint256 supply = totalSupply();
    require(
      supply + _addresses.length <= MAX_CAP,
  ...
  function ownerMintTo(uint256 _mintingAmount, address _mintingTo)
    external
    onlyOwner
  {
    uint256 supply = totalSupply();
    require(
      supply + _mintingAmount <= MAX_CAP,
      "Mint amount exceeds max supply"
    );
...
*/
  function _checkOnERC721Received(
    ...
    if (_to.isContract()) {
      try
        IERC721Receiver(_to).onERC721Received(_msgSender(), _from, _id, _data)
