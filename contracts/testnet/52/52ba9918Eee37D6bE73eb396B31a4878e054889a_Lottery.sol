...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	uint256 public dividendsFee = 1000; //10%
	...
	uint256 private _dividendsDeposits; //dividends deposit storage
	mapping(address => uint256) public withdrawedDividendsTime; //keeps track when dividends were taken
	uint256 public dividendsInterval = 30 days;
		...
		nonReentrant
	{
		...
		uint256 dividendsAmount = _calcPercentage(ticketPrice, dividendsFee); //10%
		_dividendsDeposits = _dividendsDeposits + dividendsAmount;
	...
	//@notice withdrawdividends can be called anytime
	//funds are deposited only after a round ends
	function withdrawdividends() external notContract whenNotPaused nonReentrant {
		require(
			withdrawedDividendsTime[_msgSender()] + dividendsInterval < block.timestamp,
			"already claimed in interval"
		);
		Round storage currentRound = rounds[currentRoundIndex];
		require(currentRound.leader != _msgSender(), "leader cannot withdraw dividends");

		uint256 dividends = getDividendsForAddress(_msgSender());
		withdrawedDividendsTime[_msgSender()] = block.timestamp;
		_asyncTransfer(_msgSender(), dividends);
	}

	//calculates how much dividends an address has
	function getDividendsForAddress(address addr) public view returns (uint256) {
		return (boughtTickets[addr] * _dividendsDeposits) / totalTicketsBought;
	}
	...
	function getWinnerEarnings() public view returns (uint256) {
		return _calcPercentage(address(this).balance - _dividendsDeposits, prizeFee);
	}
	...
	// enable/disable claiming for dividends
	function setdividendsSettings(uint256 intervalInSeconds) public onlyOwner {
		dividendsInterval = intervalInSeconds;
	}
	...
	// withdrawal to owner of some funds (used for testing or stuck dividends)
	...
	function distributeEarnings() internal {
		...
		//substract the dividends deposits
		uint256 balanceToDistribute = address(this).balance;
		balanceToDistribute = balanceToDistribute - _dividendsDeposits;
		...
		//everything else, rollback (including thet dividends deposits)
	}
