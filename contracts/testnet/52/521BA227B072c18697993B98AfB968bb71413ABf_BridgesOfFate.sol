...
contract BridgesOfFate is Ownable {
    ...
    event comment3(uint256 _stage1,uint256 i_index ,uint256 _stage2,bytes32 player);
    event comment4(bytes32 player);
    event comment5(bytes32 player);


    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  {
        //Select index of NFT's Arrays 
        for (uint256 i = 1; i <= playerIds.length; i++) {
            emit comment5(playerIds[i]);
        ...
        if(totalNFTOnTile[GameStatusInitialized[latestGameId].stageNumber][_jumpSides] < playerIds.length){
            playerIndex = generateRandomTicketNumbers(playerIds.length,playerIds.length,totalNFTOnTile[PlayerItem[playerIds[0]].stage + 1][_jumpSides]);
        }
        ...
        //         emit comment3(playerIndex[j], i ,playerIndex[i],playerIds[i]);
                
        //         if(playerIndex[j] > 0 && (i == playerIndex[j]) ){
        //             emit comment4(playerIds[playerIndex[j]]);
        //             _overFlow(playerIds[playerIndex[j]],_currentUserStage,_jumpSides);
        //         }else{
        //             if(totalNFTOnTile[_currentUserStage][_jumpSides] <= 0){                
        //                 OverFlowPlayerStatus[playerIds[i]] =  false;
        //             }
