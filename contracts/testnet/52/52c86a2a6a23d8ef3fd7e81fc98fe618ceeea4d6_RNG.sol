...
contract RNG is VRFConsumer, Ownable {
    uint128 public modulo;
    uint128 public shift;
    uint256 public nextRng;
    mapping (address => bool) public admin;
    error AdminOnly();
    event RandomNumber(uint rng);
    constructor() VRFConsumer(532) {
        modulo = 1467;
        shift = 1;
    }

    function setParameters(uint _modulo, uint _shift) external onlyOwner {
        modulo = uint128(_modulo);
        shift = uint128(_shift);
    }

    function setAdmin(address _admin, bool value) external onlyOwner {
        admin[_admin] = value;
    }

    function getRng() public {
        if(!admin[msg.sender]){
            revert AdminOnly();
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
            nextRng = (randomWords[0] % modulo ) + shift;
            emit RandomNumber(nextRng);
    }
