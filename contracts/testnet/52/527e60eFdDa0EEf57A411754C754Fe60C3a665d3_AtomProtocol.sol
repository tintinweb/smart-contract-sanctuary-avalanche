...
{
    address public immutable atomsManager;
    ...
    constructor(address _atomsManager) ERC20("Atom", "ATM") {
        require(
            _atomsManager != address(0),
            "Implementations: AtomsManager is not set"
        );
        atomsManager = _atomsManager;
        ...
        onlyAtomsManager
    {
        ...
        // effectively protecting AtomsManager from suspicious addresses
        ...
        onlyAtomsManager
    {
        require(
            ...
            "AtomProtocol: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyAtomsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "AtomProtocol: LiquidityPoolManager is not set"
        );
        super._mint(address(liquidityPoolManager), amount);
    }

    function getFreeAtm() public payable
    {
