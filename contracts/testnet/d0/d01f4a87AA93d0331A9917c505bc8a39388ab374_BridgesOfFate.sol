...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    bytes32[] private PlayerAtSpecificStageList;
    ...
    function bulkParticipateInGame(bool[] memory _jumpSides, bytes32[] memory playerIds) public   GameInitialized GameEndRules {
        for (uint256 i = 0; i < _jumpSides.length; i++) {
            participateInGame(_jumpSides[i],playerIds[i]);
        }
    }
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public GameInitialized GameEndRules
    {
        ...
        /*
        * Remove Before final deployement 
        */
    ...
    function getStagesData(uint256 _stage) public  returns (bytes32[] memory) {
          for (uint i = 0; i < allStagesData[_stage].length; i++) {
                PlayerAtSpecificStageList.push(allStagesData[_stage][i]);
          }
        return PlayerAtSpecificStageList;
    }
    
    function getAllStagesData() external view returns(bytes32[] memory) {
        return PlayerAtSpecificStageList;
    }
}
