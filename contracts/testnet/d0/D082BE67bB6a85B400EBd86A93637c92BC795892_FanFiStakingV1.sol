...
contract FanFiStakingV1 is Ownable {
    struct APYScheme {
    ...
    function stake(uint256 _amount, uint256 _schemeID) external {
        ...
        users[msg.sender].totalStakePerScheme[_schemeID] += _amount;
        users[msg.sender].stakes[users[msg.sender].stakeCount] = newStake;
        users[msg.sender].stakeCount += 1;
    ...
    function unstake(uint256 _stakeIndex) external {
        require(
            users[msg.sender].stakeCount > _stakeIndex,
            "FanFiStake: stake index doesn't exist"
        );

        StakeInfo memory stakeInfo = users[msg.sender].stakes[_stakeIndex];
        ...
        } else stakeInfo.status = StakeStatus.IN_REDEMPTION;

        users[msg.sender].totalStake -= stakeInfo.stakeAmount;
    ...
    function redeem(uint256 _stakeIndex) external {
        ...
        require(
            token.balanceOf(address(this)) >=
                stakeInfo.rewards + stakeInfo.stakeAmount,
            "FanFiStake: not enough liquidity"
        );

        totalStake -= stakeInfo.stakeAmount;

        token.transfer(msg.sender, stakeInfo.stakeAmount + stakeInfo.rewards);
    ...
    {
        require(
            users[_userAddress].stakeCount > _stakeIndex,
            "FanFiStake: stake index doesn't exist"
        );
        ...
        returns (uint256 userRewards)
    {
        ...
        StakeInfo memory stakeInfo = users[_userAddress].stakes[_stakeIndex];
        APYScheme memory scheme = apySchemes[stakeInfo.schemeID];
