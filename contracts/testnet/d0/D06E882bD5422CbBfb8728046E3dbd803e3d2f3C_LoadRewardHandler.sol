...
// File: contracts/core/LoadRewardHandler.sol
...
contract LoadRewardHandler is ReentrancyGuard, Ownable, ERC721Holder, IRewardReceiver {
    ...
    function transferTokensNoDistribution(
        address tokenAddress,
        uint256 amount
    ...
    function distributeNestHolderTokens( address tokenAddress, uint256 amount ) internal {
        if( NESTS_STAKED > 0 )
        {
            ...
            for (uint256 index = 0; index < NEST_COUNT; index++) {
                ...
                {
                    uint256 amountOnNest = StakedNests[index+1].pendingRewards[tokenAddress];
                    StakedNests[index+1].pendingRewards[tokenAddress] = amountOnNest + amountPerNest;
    ...
    function claimNestTokens( address tokenAddress, uint256 tokenId ) external nonReentrant {
        ...
        require( nestContract.ownerOf(tokenId) == msg.sender, "REWARD HANDLER: you must own this nft to claim rewards" );
    ...
    function hasRewardsForToken( address tokenAddress ) public view returns(bool) {
        require( tokenAddress != address(0), "INVALID_TOKEN_ADDRESS" );

        return PendingTokenRewards[tokenAddress] > 0;
    }

    function claimTokens(address tokenAddress, uint256 amount, bytes memory signature, string memory nonce) external nonReentrant {
        require( tokenAddress != address(0), "INVALID_TOKEN_ADDRESS" );
        require( amount > 0, "BAD_AMOUNT" );
        require(matchAddresSigner(hashTransaction(msg.sender, tokenAddress, amount, nonce), signature), "DIRECT_CLAIM_DISALLOWED");
    ...
    function hashTransaction(address sender, address tokenAddress, uint256 claimAmount, string memory nonce) private pure returns(bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(sender, tokenAddress, claimAmount, nonce)))
        );
