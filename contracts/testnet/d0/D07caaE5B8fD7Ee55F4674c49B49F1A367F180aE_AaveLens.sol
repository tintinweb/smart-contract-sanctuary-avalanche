...
contract AaveLens is IAaveLens {
     ...
     */
    function getPositionSizesInUSDC(address[] calldata _assets)
        external
        view
        override
        returns (uint256[] memory usdcPositions)
    {
        ...
        for (uint256 i; i < _assets.length; i++) {
            ...
            uint16 assetDecimals = IERC20Metadata(_assets[i]).decimals();

            // AaveV3 oracle - base token decimals across all chains
            uint8 priceDecimals = 8;

            if (assetDecimals == priceDecimals) {
                usdcPositions[i] = (currentATokenBalance * 1e2) * prices[i];
                continue;
            }

            if (assetDecimals > priceDecimals) {
                uint16 decimalDifference = assetDecimals - priceDecimals;
                usdcPositions[i] =
                    (currentATokenBalance *
                        (prices[i] * (10**decimalDifference))) /
                    1e18;
            } else if (assetDecimals < priceDecimals) {
                uint16 decimalDifference = priceDecimals - assetDecimals;
                usdcPositions[i] =
                    ((currentATokenBalance * (10**decimalDifference)) * 1e18) /
                    prices[i];
            }
        }
    }
