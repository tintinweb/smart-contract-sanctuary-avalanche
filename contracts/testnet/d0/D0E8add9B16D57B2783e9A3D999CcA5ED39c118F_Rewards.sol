...
interface IYieldBox is IStakedYieldKey {
    /**
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    event SetRewardPerYieldBox(uint256 oldRewardPerYieldBox, uint256 newRewardPerYieldBox);
    /**
     ...
     * @return YieldKeyLegYieldbox of Pre-Rewards per Yield Key Legendary.
     * @return YieldKeyRegYieldbox of Pre-Rewards per Yield Key Regular.
    ...
    function preReward()
        ...
        returns (
            ...
            uint256[] memory YieldKeyLegYieldbox,
            uint256[] memory YieldKeyRegYieldbox,
            uint64 time
        )
    {
        ...
        uint256 index;
        // console.log("YieldBoxIds.length", YieldBoxIds.length);
        // console.log("getYKAvailableStake:", getYKAvailableStake());
        // console.log("getSKAvailableStake:", getSKAvailableStake());
        if (
            (YieldKey.tokenStaking(_msgSender()).length > 0) ||
            (StakedYieldKey.tokenStaking(_msgSender()).length > 0)
        ) {
            uint256 ykAvailableClaim = (getYKAvailableStake() +
                getSKAvailableStake()) > YieldBoxIds.length
                ? YieldBoxIds.length
                : (getYKAvailableStake() + getSKAvailableStake());
            // console.log("ykAvailableClaim: ", ykAvailableClaim);
            ...
            if (
                ...
                YieldKeyLegYieldbox = new uint256[](ykAvailableClaim);
                for (uint256 i = 0; i < ykAvailableClaim; i++) {
                    if (i > (11 * (1 + index))) {
                        index++;
                    }
                    YieldKeyLegYieldbox[i] = StakedYieldKey.tokenStaking(
                        _msgSender()
                    )[index];
                    (startstake, endstake) = YieldKey
                        .getHistoricalStakedYieldKey(
                            _msgSender(),
                            StakedYieldKey.tokenStaking(_msgSender())[index]
                        );
                    ...
                    // console.log("rewarded[i] Legendary stake: ", rewarded[i]);
                    // console.log("amount[i] Legendary stake:: ", amount[i]);
                }
                ...
                // console.log(" Else If");
                (StakedYKROrdered, order) = getYKROrdered(
                    YieldKey.tokenStaking(_msgSender())
                );
                index = 0;
                rewarded = new uint256[](ykAvailableClaim);
                amount = new uint256[](ykAvailableClaim);
                YieldKeyRegYieldbox = new uint256[](ykAvailableClaim);
                for (uint256 i = 0; i < ykAvailableClaim; i++) {
                    (startstake, endstake) = YieldKey
                        .getHistoricalStakedYieldKey(
                            _msgSender(),
                    ...
                    YieldKeyRegYieldbox[i] = StakedYKROrdered[order[i]];
                    rewarded[i] += getRewardsDays(
                        startstake,
                        endstake,
                        YieldBoxIds[i],
                    ...
                    amount[i] += rewarded[i] * REWARDS_PER_DAYS;
                    // console.log("rewarded[i] Regular stake: ", rewarded[i]);
                    // console.log("amount[i] Regular stake:: ", amount[i]);
                }
            } else {
                index = 0;
                rewarded = new uint256[](ykAvailableClaim);
                amount = new uint256[](ykAvailableClaim);
                YieldKeyLegYieldbox = new uint256[](getSKAvailableStake());
                YieldKeyRegYieldbox = new uint256[](
                    ykAvailableClaim - getSKAvailableStake()
                );
                for (uint256 i = 0; i < getSKAvailableStake(); i++) {
                    ...
                    // console.log("Yield Key Legendary:", YieldKeyLegYieldbox[i]);
                    ...
                    amount[i] += rewarded[i] * REWARDS_PER_DAYS;
                    // console.log(
                    //     "rewarded[i] mixto Legendary stake: ",
                    //     rewarded[i]
                    // );
                    // console.log(
                    //     "amount[i] mixto Legendary stake:: ",
                    //     amount[i]
                    // );
                }
                (StakedYKROrdered, order) = getYKROrdered(
                ...
                for (
                    uint256 i = getSKAvailableStake();
                    i < (ykAvailableClaim - getSKAvailableStake());
                    i++
                ) {
                    (startstake, endstake) = YieldKey
                        .getHistoricalStakedYieldKey(
                    ...
                    amount[i] += rewarded[i] * REWARDS_PER_DAYS;
                    // console.log(
                    //     "rewarded[i] mixto Regular stake: ",
                    //     rewarded[i]
                    // );
                    // console.log("amount[i] mixto Regular stake: ", amount[i]);
                }
            }
        } else {
            ...
            for (uint256 i = 0; i < YieldBoxIds.length; i++) {
                YieldKeyLegYieldbox[i] = 0;
                YieldKeyRegYieldbox[i] = 0;
                if (
                    ((time - YieldBox.yieldBox(YieldBoxIds[i]).createdAt) <=
                        FREE_DAYS) &&
                    (RewardsClaimed[YieldBoxIds[i]].unClaimedDays > 0)
                ) {
                    rewarded[i] = RewardsClaimed[YieldBoxIds[i]].unClaimedDays;
                    amount[i] = mulDiv(
                        rewarded[i],
                        REWARDS_PER_DAYS,
                        REWARDS_PERCENTAGE
        ...
        )
    {
        ...
        (
            ...
            uint256[] memory YieldKeyLegYieldbox2,
            uint256[] memory YieldKeyRegYieldbox2,
            uint64 time2
        ) = preReward();
        time = time2;
        uint256 index;
        if (YieldKey.isStaked(_msgSender(), _YieldKey)) {
            for (uint256 i = 0; i < YieldKeyLegYieldbox2.length; i++) {
                if (YieldKeyLegYieldbox2[i] == _YieldKey) {
                    index++;
                }
            }
            ...
            for (uint256 i = 0; i < YieldKeyRegYieldbox2.length; i++) {
                if (YieldKeyRegYieldbox2[i] == _YieldKey) {
                    index++;
                }
            }
            amount = new uint256[](index);
    ...
    function reward() external nonReentrant {
        ...
        for (uint256 i = 0; i < YieldBoxRewarded.length; i++) {
            if (amount[i] > 0) {
                ...
                RewardsClaimed[YieldBoxRewarded[i]].firstDay = RewardsClaimed[
                    YieldBoxRewarded[i]
                ].firstDay == uint64(0)
                    ? uint64(time)
                    : RewardsClaimed[YieldBoxRewarded[i]].firstDay;
                if (
                    amount[i] / rewarded[i] ==
                    ...
                    RewardsClaimed[YieldBoxRewarded[i]]
                        .unClaimedDays -= rewarded[i];
                }
                if (
                    (RewardsClaimed[YieldBoxRewarded[i]].unClaimedDays == 0) &&
                    (MaintFee.maintFee(YieldBoxRewarded[i]).daysPaid ==
                        LIMITS_DAYS)
                ) {
    ...
    function setValue(uint256 _value) external onlyOwner {
        ...
        REWARDS_PER_DAYS = _value;
        emit SetRewardPerYieldBox(oldValue, _value);
    }
    ...
    function getRewardsDays(
        ...
        rewardDays = stakedDays > RewardsClaimed[YieldBoxIds].unClaimedDays
            ? RewardsClaimed[YieldBoxIds].unClaimedDays
            : stakedDays;
    }
    ...
    /**
     * @dev This Method for Setting the Limits of Days Payable of Maintenence Fee of Yield Box
     * @param _days Days to pay for all Yield Box of the Sender
     */
    function setLimitsDays(uint256 _days) external onlyOwner {
