...
interface IOOPATraits {
    ...
    function mintTrait(uint traitType, address user) external; 
}
...
contract OOPARaffler is Ownable, VRFConsumer, ERC1155Holder {
    ...
    IOOPACredits immutable OOPA_CREDITS;
    IOOPAShoppingBag immutable OOPA_GIFTBOX;
    ...
    event RewardClaimed(uint raffleId);
    ...
    function createRaffle (Raffle memory _raffle, bool _transferRewards) external  returns (uint) {
        ...
        if ((_raffle.raffledGoodsAmount == 0) || (_raffle.pricePerTicket == 0 ) || (_raffle.endDate < _raffle.startDate + 60 ) || (_raffle.maxTicketAvailable == 0 ) || (_raffle.ticketsSold != 0 )){
    ...
    function finalizeRaffle(uint _raffleId) external {
        ...
        // this will also prevent later finalizeRaffle calls
        if (RaffleTickets[_raffleId].length == 0){
            revert InvalidRaffle();
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        address[] memory winners = new address[](raffle.raffledGoodsAmount);
        uint[] memory luckyNumbers = new uint[](raffle.raffledGoodsAmount);


        for (uint i; i < amountOfGoods; ) {

            uint randomNumber = ((randomWords[0] >> (i*4)) % ticketsSold);

            winners[i] =  findWinner(RaffleTickets[raffleID], randomNumber);
        ...
        if (raffle.raffleType == RaffleType.ERC1155) {
            RaffleWinners[raffleID] = RaffleWinnersData(winners,raffle.raffledCollectionERC1155 , 0, raffle.raffledGoodERC1155, RaffleType.ERC1155, 0,raffle.ERC1155BundleRewards);
        }
        else if (raffle.raffleType == RaffleType.ERC721) {
            RaffleWinners[raffleID] = RaffleWinnersData(winners, raffle.raffledCollectionERC721, raffle.tokenIdERC721, 0, RaffleType.ERC721,0,raffle.ERC1155BundleRewards);
        }
        else if (raffle.raffleType == RaffleType.ERC1155BUNDLE) {
            RaffleWinners[raffleID] = RaffleWinnersData(winners,raffle.raffledCollectionERC1155 , 0, 0, RaffleType.ERC1155BUNDLE, 0,raffle.ERC1155BundleRewards);
        }
    ...
    function claimReward(uint _raffleId) external {
        RaffleWinnersData memory winnersData = RaffleWinners[_raffleId];
        if (winnersData.raffleType == RaffleType.ERC1155 ) { // ERC1155
            uint len = winnersData.winners.length;
            for (uint i ; i < len;) {
                if(msg.sender == winnersData.winners[i] )
                {
                    delete winnersData.winners[i];
                    winnersData.rewardsClaimed++;
                    IERC1155(winnersData.raffledCollectionAddress).safeTransferFrom(address(this), msg.sender,winnersData.raffledGoodERC1155,1, "");
                    emit RewardClaimed(_raffleId);
                }
                unchecked {
                    ++i;
                }
            ...
            if(winnersData.rewardsClaimed == len) {
                delete RaffleWinners[_raffleId];
            } else {
                RaffleWinners[_raffleId] = winnersData;
            }
        }
        else if (winnersData.raffleType == RaffleType.ERC1155BUNDLE) { // ERC1155
            uint len = winnersData.winners.length;
            for (uint i ; i < len;) {
                if(msg.sender == winnersData.winners[i] )
                {
                    delete winnersData.winners[i];
                    winnersData.rewardsClaimed++;
                    uint256 rewardsLen = winnersData.ERC1155BundleRewards.length;
                    uint256[] memory amounts = new uint256[](len);
                    for (uint256 j = 0; j < rewardsLen; j++) {
                       amounts[j] = 1; 
                    }
                    IERC1155(winnersData.raffledCollectionAddress).safeBatchTransferFrom(address(this),msg.sender,winnersData.ERC1155BundleRewards,amounts,"");
                    emit RewardClaimed(_raffleId);
                }
                unchecked {
                    ++i;
        ...
        else if (winnersData.raffleType == RaffleType.ERC721) { // ERC721
            if(msg.sender == winnersData.winners[0] ){
                address collection = winnersData.raffledCollectionAddress;
                uint32 tokenId = winnersData.tokenIdERC721;
    ...
    function getRaffleWinners(uint _raffleId) view external returns(address[] memory) {
        return RaffleWinners[_raffleId].winners;
    }
...
abstract contract VRFConsumer is VRFConsumerBaseV2 {
    ...
    constructor(uint64 subscriptionId) VRFConsumerBaseV2(vrfCoordinatorAVAXMOCK) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinatorAVAXMOCK);
