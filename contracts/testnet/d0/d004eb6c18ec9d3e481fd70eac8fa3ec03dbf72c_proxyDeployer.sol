...
contract proxyDeployer is Owned {
    ...
    CollectionDetails[] CollectiondetailList; // All Collection Details

    mapping(address => CollectionDetails) private collectionsList; //Stored Collection Details
    mapping(address => mapping(address => bool)) public whitelistedAddresses; // Whitelisted user Address
    mapping(address => address[]) public whitelistedAddressesList;
    ...
    function deployerFunction(
        ...
        _CollectionDetails.startDate = data[0];
        _CollectionDetails.endDate = data[1];
        ...
        _CollectionDetails.pid = collectionAddressList.length; // Total Collection length
        _CollectionDetails.deleted = false; // maintain collection deleted status
        collectionsList[_collectionAddress] = _CollectionDetails;
        collectionAddressList.push(_collectionAddress);
        CollectiondetailList.push(_CollectionDetails);
        ...
        )
    {
        ...
        bool[] memory _status = new bool[](2);
        data[0] = collectionsList[_collectionAddress].startDate;
        data[1] = collectionsList[_collectionAddress].endDate;
        data[2] = collectionsList[_collectionAddress].price;
        data[3] = collectionsList[_collectionAddress].whiteprice;
        data[4] = collectionsList[_collectionAddress].totalQTY;
        data[5] = collectionsList[_collectionAddress].maxNFT;
        data[6] = collectionsList[_collectionAddress].maxWhiteNFT;
        _Address[0] = collectionsList[_collectionAddress].feeCollectorAddress;
        _Address[1] = collectionsList[_collectionAddress].tokenAddress;
        _status[0] = whitelistedAddresses[_collectionAddress][_whiteAddress];
        _status[1] = collectionsList[_collectionAddress].deleted;
        return (
            collectionsList[_collectionAddress].Currency,
            data,
            _Address,
            _status
        );
    ...
    function editCollections(
        ...
        CollectiondetailList[_collectionId] = collectionsList[
            _collectionAddress
        ];
    }

    function addAndRemoveWhiteList(
        address _collectionAddress,
        address[] memory _whiteAddress,
        address[] memory _blockAddress,
        ...
        if (
            ...
            for (uint256 i = 0; i < _whiteAddress.length; i++) {
                whitelistedAddresses[_collectionAddress][
                    _whiteAddress[i]
                ] = true;
            }
        } else {
            for (uint256 i = 0; i < _blockAddress.length; i++) {
                whitelistedAddresses[_collectionAddress][
                    _blockAddress[i]
                ] = false;
            }
        }
        whitelistedAddressesList[_collectionAddress] = _whiteAddress;
    }

    function removeAndAddCollection(address _collectionAddress, uint256 _id)
        public
        onlyOwner
    {
        if (collectionsList[_collectionAddress].deleted) {
            collectionsList[_collectionAddress].deleted = false;
        } else {
            collectionsList[_collectionAddress].deleted = true;
        }
        CollectiondetailList[_id] = collectionsList[_collectionAddress];
    }
    function getWhiteListAddress(address _collectionAddress) public view returns(address[] memory){
        return whitelistedAddressesList[_collectionAddress];
    }
}