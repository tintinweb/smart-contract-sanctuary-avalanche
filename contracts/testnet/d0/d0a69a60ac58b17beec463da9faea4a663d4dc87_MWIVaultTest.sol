...
contract MWIVaultTest is ERC20Upgradeable, OwnableUpgradeable, 
    ...
    function withdraw(uint share) external nonReentrant {
        ...
        if (!paused()) {
            ...
            uint USDTAmt = withdrawAmt / (PriceLib.getAssetPrice(address(USDT)) * 1e4); // USDT's decimals is 6, price's decimals is 8
    ...
    function getAllPoolInUSD() public view returns (uint) {
        if (paused()) return USDT.balanceOf(address(this)) * PriceLib.getAssetPrice(address(USDT)) * 1e4; // USDT's decimals is 6, price's decimals is 8
