...
contract BaseDeployer is Script {
    ...
    bytes32 internal saltDescriptorImplementation;
...
contract DeployDBD is Script, BaseDeployer {
    ...
    address private descriptorImplementationAddress;
    address private descriptorAddress;
    address private DBDImplementationAddress;
    address private DBDAddress;
    ...
    function computeCreate2(
        bytes32 saltDBDImplementation,
        bytes32 saltDBD,
        bytes32 saltDescriptorImplementation,
        bytes32 saltDescriptor
    ) private {
        DBDImplementationAddress = computeCreate2Address(
            saltDBDImplementation, keccak256(abi.encodePacked(type(DownbadDonkeys).creationCode, "")), CREATE2_FACTORY
        );
        DBDAddress = Clones.predictDeterministicAddress(DBDImplementationAddress, saltDBD, CREATE2_FACTORY);

        descriptorImplementationAddress = computeCreate2Address(
            saltDescriptorImplementation,
            keccak256(abi.encodePacked(type(Descriptor).creationCode, "")),
            CREATE2_FACTORY
        );
        descriptorAddress =
            Clones.predictDeterministicAddress(descriptorImplementationAddress, saltDescriptor, CREATE2_FACTORY);

        console2.log("downbadDonkeysImplementation", DBDImplementationAddress);
        console2.log("downbadDonkeys", DBDAddress);
        console2.log("descriptorImplementation", descriptorImplementationAddress);
    ...
    function deployContractsMainnet() external setEnvDeploy(Cycle.Prod) {
        ...
        saltDescriptor = bytes32(uint256(11));
        saltDescriptorImplementation = bytes32(uint256(12));
        saltDBD = bytes32(uint256(13));
        saltDBDImplementation = bytes32(uint256(14));
    ...
    function createDeployMultichain(Chains[] memory deployForks) private {
        ...
        for (uint256 i; i < deployForks.length;) {
            ...
            computeCreate2(saltDBDImplementation, saltDBD, saltDescriptorImplementation, saltDescriptor);
    ...
    function chainDeployContracts() private broadcast(deployerPrivateKey) {
        DownbadDonkeys downbadDonkeysImplementation = new DownbadDonkeys{salt: saltDBDImplementation}();
        ...
        collectionData = IDownbadDonkeys.CollectionData(
            "DownbadDonkeys", "DBD", address(0), 0, deployCreationData.donkeysCollectionSize, 0, 0, 0
        );
        ...
        Descriptor descriptorImplementation = new Descriptor{salt: saltDescriptorImplementation}();
        address descriptorAddress = Clones.cloneDeterministic(address(descriptorImplementation), saltDescriptor);

        descriptor = Descriptor(descriptorAddress);
        descriptor.initialize(
            downbadDonkeysAddress,
            deployCreationData.donkeysBatchRevealSize,
            deployCreationData.donkeysCollectionSize,
            deployCreationData.isChainWithVRF
        );

        require(
            DBDImplementationAddress == address(downbadDonkeysImplementation), "Address mismatch DBD Implementation"
        );
        require(DBDAddress == address(downbadDonkeys), "Address mismatch DBD");
        require(descriptorImplementationAddress == address(descriptorImplementation), "Address mismatch Descriptor");
        require(descriptorAddress == address(descriptor), "Address mismatch Descriptor");
    }

    function chainSetupContracts() private broadcast(deployerPrivateKey) {
        downbadDonkeys = DownbadDonkeys(DBDAddress);

        downbadDonkeys.setDescriptor(address(descriptorAddress));
        ...
        downbadDonkeys.setTrustedRemote(
            deploySetupData.destChainId, abi.encodePacked(address(DBDAddress), address(DBDAddress))
        );
        ...
        if (deployCreationData.isChainWithVRF) {
            VRFCoordinatorV2Interface vrfCoordinator = VRFCoordinatorV2Interface(deploySetupData.vrfCoordinator);
            uint64 subId = vrfCoordinator.createSubscription();
            vrfCoordinator.addConsumer(subId, descriptorAddress);

            descriptor.setVRF(deploySetupData.vrfCoordinator, deploySetupData.keyhash, subId, 2000000);
            console2.log("VRF set up");
        }
    }
}
