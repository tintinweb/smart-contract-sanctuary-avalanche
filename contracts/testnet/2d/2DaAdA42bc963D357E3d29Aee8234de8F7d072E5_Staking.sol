...
contract Staking is Ownable {
    ...
    function unstake(uint256 id) external {
        (bool found, UserNode memory userStake, uint256 index) = searchUserNode(
            id,
            msg.sender
        );
    ...
    function searchUserNode(uint256 id, address user)
        private
        view
        returns (
            bool,
    ...
    {
        UserNode[] memory userNodes = stakes[user];
    ...
    function ableToGetReward(uint256 id) external view returns (bool) {
        (bool found, UserNode memory userStake, ) = searchUserNode(id, msg.sender);
        require(found, 'User node with id not found.');
    ...
    function getUsers(uint256 begin, uint256 end)
        external
        view
        returns (UserStat[] memory users)
    {
        if (begin == 0) begin = 1;
        if (end > totalNodes) end = totalNodes;
        users = new UserStat[](end - begin + 1);

        for (uint256 i; i < users.length; ++i) {
            users[i] = UserStat(begin + i, nodes.ids[i + begin]);
        }
    }
    ...
    function getUserNode(uint256 id, address user)
        external
        view
        returns (UserNode memory userNode)
    {
        (, userNode, ) = searchUserNode(id, user);
    }
    ...
    function setTotalNodesAmount(uint256 _totalNodes) external onlyOwner {
        totalNodes = _totalNodes;
    }
