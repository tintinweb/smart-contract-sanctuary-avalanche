...
contract UI is  AccessControl, ReentrancyGuard, Pausable {
    ...
    address public splitter; // The safe that stores the BNB made from the fees

    /** CONSTRUCTOR **/

    constructor(address _token, address _nft, address _staking, address _splitter) {
...
/** TOKEN METHODS **/
    ...
    function TOKENsetBuyingFeeDAO(uint256 _buyingFee) external whenNotPaused onlyRole(ADMIN) {
        IToken(token).setBuyingFeeDAO(_buyingFee);
    }

    function TOKENsetSellingFeeClaimedDAO(uint256 _sellingFeeClaimed) external whenNotPaused onlyRole(ADMIN) {
        IToken(token).setSellingFeeClaimedDAO(_sellingFeeClaimed);
    }

    function TOKENsetSellingFeeNonClaimedDAO(uint256 _sellingFeeNonClaimed) external whenNotPaused onlyRole(ADMIN) {
        IToken(token).setSellingFeeNonClaimedDAO(_sellingFeeNonClaimed);
    }

    function TOKENblacklistDAO(address[] calldata _users, bool _state) external whenNotPaused nonReentrant onlyRole(MODERATOR){
        IToken(nft).blacklistDAO(_users, _state);
    }

/** NFT METHODS **/
    
    function NFTaddAirdrops(address[] calldata _accounts, uint[] calldata _values) external whenNotPaused nonReentrant onlyRole(MARKETING) {
        INFT(nft).addAirdrops( _accounts, _values);
    }

    function NFTaddWhiteList(address[] calldata _accounts, uint[] calldata _values) external whenNotPaused nonReentrant onlyRole(MARKETING) {
        INFT(nft).addWhiteList( _accounts, _values);
    }

    function NFTsetPaused(bool _paused) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        INFT(nft).setPaused( _paused);
    }

    function NFTsetMINT_PRICE(uint256 _price) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        INFT(nft).setMINT_PRICE( _price);
    }
    
    function NFTsetWhiteList(bool _OnlyWhiteList) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        INFT(nft).setWhiteList( _OnlyWhiteList);
    }

    function NFTwithdraw() external whenNotPaused nonReentrant onlyRole(ADMIN) {
        INFT(nft).withdraw( );
    }

/** STACKING METHODS **/

    function STACKINGsetPaused(bool _paused) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setPaused( _paused);
    }

    function STACKINGsetMINIMUM_TO_EXIT(uint256 _MINIMUM_TO_EXIT)  external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setMINIMUM_TO_EXIT( _MINIMUM_TO_EXIT);
    }

    function STACKINGsetDAILY_token_RATE(uint256 _DAILY_token_RATE)  external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setDAILY_token_RATE( _DAILY_token_RATE);
    }

    function STACKINGsetToken_CLAIM_TAX_PERCENTAGE(uint256 _token_CLAIM_TAX_PERCENTAGE) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setToken_CLAIM_TAX_PERCENTAGE(_token_CLAIM_TAX_PERCENTAGE);
    }

    function STACKINGsetProbabilityToLoseEvryThing(uint256[] calldata _probabilityToLoseEvryThing)  external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setProbabilityToLoseEvryThing( _probabilityToLoseEvryThing);
    }

    function STACKINGsetClaiming(bool _canClaim) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setClaiming(_canClaim);
    }

/** SPLITTER METHODS **/

    function SPLITTERwithdraw() external whenNotPaused nonReentrant {
...
/** ROLE MANAGEMENT **/
    ...
    // Gives the TAXEStoken role to an address
    function grantADMINRoleDAO(address _ADMIN) external onlyRole(DAO) {
        grantRole(ADMIN, _ADMIN);
    }

    function revokeADMINRoleDAO(address _ADMIN) external onlyRole(DAO) {
    ...
    // Gives the DAO role to an address
...
/** SETTER **/
    ...
    function setNFT(address _nft) external onlyRole(DAO) {
        nft = _nft;
    }

    function setStacking(address _staking) external onlyRole(DAO) {
        staking = _staking;
    }
...
interface IToken {
  ...
  // Bridge
  function bridgeTransfert(address _from,address _to,uint256 _amount) external;
...
interface INFT {
  ...
  function addAirdrops(address[] calldata accounts, uint[] calldata values) external;

  function addWhiteList(address[] calldata accounts, uint[] calldata values) external;
...
interface IStaking {
  ...
  function setToken_CLAIM_TAX_PERCENTAGE(uint256 _token_CLAIM_TAX_PERCENTAGE) external ;

  function setProbabilityToLoseEvryThing(uint256[] calldata _probabilityToLoseEvryThing) external ;
...
contract Staking is AccessControl, IERC721Receiver, Pausable {
  ...
  // slave must have 1 days worth of $token to unstake or else it's too cold
  uint256 public MINIMUM_TO_EXIT = 1 days;

  // masters take a 30% tax on all $token claimed
  uint256 public token_CLAIM_TAX_PERCENTAGE = 30;

  // there will only ever be (roughly) 5 millions $token earned through staking
  ...
  function claimManyFromStaking(uint16[] memory tokenIds, bool unstake)
    ...
    _updateEarnings
  {
    require(msg.sender == tx.origin || tx.origin == address(this), "Only Externally Owned Account");
    require(canClaim, "Claim is not currently possible");
    ...
    returns (uint256 owed)
  {
    ...
    if (block.timestamp - stake.value > MINIMUM_TO_EXIT) {
      ...
      if (unstake) {
        uint256 numberOfDaysFromLastClaim = ((block.timestamp - stake.value) / 1 days);
        address recipient = _msgSender();


        if (random(tokenId) % 100 < probabilityToLoseEvryThing[numberOfDaysFromLastClaim]) {
  ...
  function _remove(address account, uint _tokenId) internal {
    for (uint256 i = 0; i < userNFTStack[account].length; i++) {
      if(userNFTStack[account][i] == _tokenId) {
        userNFTStack[account][i] = userNFTStack[account][userNFTStack[account].length - 1];
  ...
  function getTokensOf(address _user) view external returns(uint16[] memory) {
    return userNFTStack[_user];
  }

  function slaveStackedNumber(address _user) view external returns(uint256) {
  ...
  function masterStackedNumber(address _user) view external returns(uint256) {
    uint16[] memory tokenIds = userNFTStack[_user];
    uint256 totalNumber = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      INFT.NFTMetadata memory s = game.getTokenMetadata(tokenIds[i]);
  ...
  function potusStackedNumber(address _user) view external returns(uint256) {
    uint16[] memory tokenIds = userNFTStack[_user];
    uint256 totalNumber = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      INFT.NFTMetadata memory s = game.getTokenMetadata(tokenIds[i]);
  ...
  function estimatedRevenuesOf(address _user) view external returns(uint) {
    uint16[] memory tokenIds = userNFTStack[_user]; 
  ...
  function randomMasterOwner(uint256 seed) public view returns (address) {
    if (totalLevelStaked == 0) return address(this);
  ...
  function setMINIMUM_TO_EXIT(uint256 _MINIMUM_TO_EXIT) external onlyRole(DAO) {
    MINIMUM_TO_EXIT = _MINIMUM_TO_EXIT;
  }

  function setDAILY_token_RATE(uint256 _DAILY_token_RATE) external onlyRole(DAO) {
    DAILY_token_RATE = _DAILY_token_RATE;
  }

  function setToken_CLAIM_TAX_PERCENTAGE(uint256 _token_CLAIM_TAX_PERCENTAGE) external onlyRole(DAO) {
    token_CLAIM_TAX_PERCENTAGE = _token_CLAIM_TAX_PERCENTAGE;
  }

  function setMAXIMUM_GLOBAL_token(uint256 _MAXIMUM_GLOBAL_token) external onlyRole(DAO) {
    MAXIMUM_GLOBAL_token = _MAXIMUM_GLOBAL_token;
  }
  
  function setProbabilityToLoseEvryThing(uint256[] calldata _probabilityToLoseEvryThing) external onlyRole(DAO) {
  ...
  function setStaking(uint16 _tokenId, address _owner, uint80 _blockTimestamp) external onlyRole(DAO) {
  ...
  function setTotalLevelStaked(uint256 _totalLevelStaked) external onlyRole(DAO) {
    totalLevelStaked = _totalLevelStaked;
  }

  function setUnaccountedRewards(uint256 _unaccountedRewards) external onlyRole(DAO) {
  ...
  function setTotalTokenEarned(uint256 _totalTokenEarned) external onlyRole(DAO) {
    totalTokenEarned = _totalTokenEarned;
  }

  function setTotalSlaveStaked(uint256 _totalSlaveStaked) external onlyRole(DAO) {
    totalSlaveStaked = _totalSlaveStaked;
  }

  function setTotalMasterStaked(uint256 _totalMasterStaked) external onlyRole(DAO) {
    totalMasterStaked = _totalMasterStaked;
  }

  function setTotalPotusStaked(uint256 _totalPotusStaked) external onlyRole(DAO) {
    totalPotusStaked = _totalPotusStaked;
  }

  function setTotalSlaveStolen(uint256 _totalSlaveStolen) external onlyRole(DAO) {
    totalSlaveStolen = _totalSlaveStolen;
  }

  function setLastClaimTimestamp(uint256 _lastClaimTimestamp) external onlyRole(DAO) {
...
// SPDX-License-Identifier: MIT LICENSE
  ...
  /** VARIABLES **/
    ...
    // number of Master staked in the staking 
  ...
  /** CONSTRUCTOR **/
    ...
    */
    function mint(uint256 amount, bool stake, address sponsor) external payable nonReentrant whenNotPaused {
      require(tx.origin == _msgSender(), "Only EOA");
      require(minted + amount <= MAX_TOKENS, "All tokens minted");
      require(amount > 0 && amount <= NFTLimitByTransaction, "Invalid mint amount");

      if (OnlyWhiteList) {
        require(amount <= whiteList[_msgSender()], "You do not have enough WhiteList");
      ...
      if (minted <= PAID_TOKENS) {
        ...
        require(balanceOf(_msgSender()) + staking.getTokensOf(_msgSender()).length + amount <= NFTLimit, "Do not sell your house to buy our NFT please");
      } else {
        require(msg.value == 0);
      }
      ...
      uint256 sponsorTokenCount = balanceOf(sponsor) +  staking.getTokensOf(sponsor).length;
      ...
      if (totalTokenCost > 0) {
        require(token.balanceOf(_msgSender()) < totalTokenCost,"Not enough tokens to pay");
      ...
      if (((amount * MINT_PRICE) == msg.value) && (minted + amount) <= PAID_TOKENS && (sponsor != address(0) )){
        ...
        if (sponsorTokenCount > 0) {
          ...
          if(cashback > 0 && cashback <= msg.value ) {
            ...
            // payable(marketingAgencyWallet).call{value: ((amount * MINT_PRICE) - cashback ) * marketingAgencyShare/100 }("");
          }
        }
      }
    ...
    */
    function generate(uint256 tokenId, uint256 seed) internal returns (NFTMetadata memory t) {
    ...
    */
    function selectMeta(uint16 seed, uint8 metaType) internal view returns (uint8) {
    ...
    */
    function selectMetadata(uint256 seed) internal view returns (NFTMetadata memory t) {
      ...
      if (!t.isSlave) {
        ...
        // if levelindex is between 0 and 4,  then it is a POTUS, shift = 20
        // if  levelindex is superior at 4,  then is a master shift = 10,
        shift = t.levelIndex <= 4 ? 20 : 10;
        t.Shift = shift;
      }
    ...
    function slaveUnstackedNumber(address _user) view external returns(uint256) {
    ...
    function masterUnstackedNumber(address _user) view external returns(uint256) {
      uint256[] memory tokenIds = walletOfOwner(_user);
      uint256 totalNumber = 0;
      uint256 shift;
      for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function potusUnstackedNumber(address _user) view external returns(uint256) {
      uint256[] memory tokenIds = walletOfOwner(_user);
      uint256 totalNumber = 0;
      uint256 shift;
      for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    */
    ...
    function addAirdrops(address[] calldata accounts, uint[] calldata values) public override onlyRole(DAO) {
    ...
    function addWhiteList(address[] calldata accounts, uint[] calldata values) public override onlyRole(DAO) {
      require(accounts.length == values.length, "Accounts != Values");
      for (uint256 i = 0; i < values.length; i++) {
    ...
    */
    ...
    function setNFTLimitByTransaction(uint256 _NFTLimitByTransaction) external onlyRole(DAO) {
    ...
    function setCashbackPercentage(uint16[] calldata _cashbackPercentage) external onlyRole(DAO) {
    ...
    // function setLiquidityPoolAddress(address _liquidityPoolAddress) external onlyRole(DAO) {
    ...
    // function setMarketingAgencyWallet(address _marketingAgencyWallet) external onlyRole(DAO) {
    //   marketingAgencyWallet = _marketingAgencyWallet;
    // }

    // function setMarketingAgencyShare(uint256 _marketingAgencyShare) external onlyRole(DAO) {
    ...
    function setTotalSlaveMinted(uint256 _totalSlaveMinted) external onlyRole(DAO) {
      totalSlaveMinted = _totalSlaveMinted;
    }
    
    function setTotalMasterMinted(uint256 _totalMasterMinted) external onlyRole(DAO) {
      totalMasterMinted = _totalMasterMinted;
    }
    
    function setTotalPotusMinted(uint256 _totalPotusMinted) external onlyRole(DAO) {
...
contract Token is  ERC20, AccessControl, ReentrancyGuard, Pausable {
  ...
  constructor(address _pancakeRouter, address payable _feeSafe) ERC20("TNT", "TNT") {
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    grantRole(DAO, msg.sender);
    ...
    pancakeBucksBnbPair = IUniswapV2Factory(pancakeRouter.factory()).createPair(
      address(this),
      pancakeRouter.WETH()
    );
  ...
  function _beforeTokenTransfer(
    ...
    require(!_isBlacklisted(_from), "You do not have enough BUCKS to sell/send them.");
  ...
  function _transfer(
    ...
    if (swapEnabled) {
      ...
      if (!swapping && !swapLiquify && !swapBNBRewards && safeFeeBalance > minimumSafeFeeBalanceToSwap) {
      ...
      if (!swapping && !swapLiquify && !swapBNBRewards && liquidityFeeBalance > minimumLiquidityFeeBalanceToSwap) {
      ...
      if (!swapping && !swapLiquify && !swapBNBRewards && BNBRewardsFeeBalance > minimumBNBRewardsBalanceToSwap) {
        // Forbid swapping 
        swapBNBRewards = true;
  ...
  function _swapSafeFeeBalance() internal {
    require(safeFeeBalance > minimumSafeFeeBalanceToSwap, "Not enough BUCKS tokens to swap for safe fee");
 ...
 function grantDAORole(address _dao) external onlyRole(DAO) {
  ...
  function blacklistDAO(address[] calldata _users, bool _state) external onlyRole(DAO) {
  ...
  function setMinimumBNBRewardsBalanceToSwap(uint256 _minimumBNBRewardsBalanceToSwap) external onlyRole(DAO) {
  ...
  function setLiquidityFeePercentage(uint256 _liquidityFeePercentage) external onlyRole(DAO) {
    liquidityFeePercentage = _liquidityFeePercentage;
  }


  function setClaimedTokens(address _account, uint256 _amount) external onlyRole(BRIDGE) {
    claimedTokens[_account] = _amount;
  }

  // Bridge from the Network
  function bridgeTransfert(address _from,address _to,uint256 _amount) external onlyRole(BRIDGE) {
    _transfer( _from, _to, _amount);
  }

  function bridgeAddSafeFeeBalance(uint256 _amount) external onlyRole(BRIDGE) {
    safeFeeBalance += _amount; 
  }

  function bridgeAddLiquidityFeeBalance(uint256 _amount) external onlyRole(BRIDGE) {
    liquidityFeeBalance += _amount; 
  }

  function bridgeAddBNBRewardsFeeBalance(uint256 _amount) external onlyRole(BRIDGE) {
    BNBRewardsFeeBalance += _amount; 
  }

  function bridgeAddClaimedTokens(address _user,uint256 _amount) external onlyRole(BRIDGE) {
    claimedTokens[_user] += _amount; 
  }

  function bridgeBlackList(address _user, bool _state) external onlyRole(BRIDGE) {
