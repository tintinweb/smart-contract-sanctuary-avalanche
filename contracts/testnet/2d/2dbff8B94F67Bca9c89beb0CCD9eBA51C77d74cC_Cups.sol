...
/// @title Cups
contract Cups is AccessControlEnumerable, GameManager, VRFManager {
    mapping(uint256 => uint256) public gameByRequestId;

    uint256 public houseBalance;
    uint256 public houseHold;
    uint256 public constant RESERVE_RATIO = 3;
    ...
    /// @param _guess is the cup the caller wants to choose 0,1,2
    function startGame(uint256 _guess) external payable {
        require(active, "startGame: game contract must be active to play");
        require(_guess <= 2, "startGame: only 3 cups to choose, 0,1,2");
        require(
            msg.value <= maximumAnte && msg.value >= minimumAnte,
            "startGame: have to stay within the min and max ante"
        );
        require(
            houseBalance > msg.value * RESERVE_RATIO,
            "startGame: not enough funds in contract to support a winning bet"
        );
        ...
        unchecked {
            houseBalance -= msg.value * RESERVE_RATIO;
            houseHold += msg.value * RESERVE_RATIO;
        }
    ...
    function _getRandomNumber() private {
        ...
        //NOTE: requesting random words from VRF
    ...
    function fulfillRandomWords(
        ...
        uint256 modifiedRandomNumber = (_randomWords[0] % NUMBER_OF_CUPS);
    ...
    function _endGame(uint256 _gameId, uint256 _randomNumber) private {
        ...
        if (gameData[_gameId].guess == _randomNumber) {
            ...
            houseBalance += (gameData[_gameId].ante +
                (gameData[_gameId].ante * RESERVE_RATIO));
        }
    ...
    {
        require(
            _amount <= houseBalance,
            "houseWithdraw: cannot withdraw more then contract balance"
        );
    ...
    function removePlayerFromGame(address _player)
        external
        onlyRole(GAME_MANAGER_ROLE)
    {
    ...
    function voidGame(uint256 _gameId) external onlyRole(GAME_MANAGER_ROLE) {
        ...
        uint256 houseAmount = ante * RESERVE_RATIO;
...
contract VRFManager is IConsumer {
    ...
    function fulfillRandomWords(uint256, uint256[] memory) public virtual {}
...
contract GameManager {
    ...
    uint256 public constant NUMBER_OF_CUPS = 3;
