...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /** 
     *  @notice Pay a payment by USDC Token
     * 
     *  @dev    Only Client can call this function
     * 
     *          Name        Meaning 
     *  @param  paymentId   ID of payment needs to be paid
     *  @param  amount      Amount needs to be paid
     *
     *  Emit event {RequestedPayment}
     */
    function payUsdc(uint256 paymentId, uint256 amount) 
        external 
        onlyValidPayment(paymentId) 
        ...
        nonReentrant  
    {
        ...
        require(
            keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDC"))), 
            "You must pay USDC"
        );
    ...
    function payUsdt(uint256 paymentId, uint256 amount) 
        external 
        onlyValidPayment(paymentId) 
        onlyClient(paymentId) 
        nonReentrant  
    {
        Payment storage payment = payments[paymentId];
        require(
            keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDT"))), 
            "You must pay USDT"
        );
        require(amount > 0, "Amount must be greater than 0");
        require(
    ...
    function approveUsdcTokens(uint256 _amount) public returns(bool){
       usdcToken.approve(address(this), _amount);
       return true;
    }

    function approveUsdtTokens(uint256 _amount) public returns(bool){
       usdtToken.approve(address(this), _amount);
       return true;
    }

    function getAllowanceUsdc() public view returns (uint256) {
       return usdcToken.allowance(_msgSender(), address(this));
    }

    function getAllowanceUsdt() public view returns (uint256) {
    ...
    function acceptUsdcPayment(address from, uint256 _amount) public returns (bool) {
       require(_amount <= getAllowanceUsdc(), "Please approve tokens before transferring");
    ...
    function acceptUsdtPayment(address from, uint256 _amount) public returns (bool) {
       require(_amount <= getAllowanceUsdt(), "Please approve tokens before transferring");
       usdtToken.transferFrom(from, address(this), _amount);
       return true;
    }

    function getContractUsdcTokenBalance() public onlyOwner view returns (uint256){
       return usdcToken.balanceOf(address(this));
    }

    function getContractUsdtTokenBalance() public onlyOwner view returns (uint256){
    ...
    function claimUsdc(uint256 paymentId) 
        external 
        onlyValidPayment(paymentId) 
        onlyBusinessOwner(paymentId) 
        nonReentrant 
    {
        ...
        uint256 amount = payments[paymentId].amount;
        // acceptUsdcPayment(address(this), amount);
        uint256 serviceFee = calculateServiceFee(amount);
        payments[paymentId].status = Status.CLAIMED;
        serviceFeeUsdcTotal += serviceFee;
        usdcToken.transfer(_msgSender(), amount - serviceFee);
        emit Claimed(paymentId, amount - serviceFee);
    }

    function claimUsdt(uint256 paymentId) 
        external 
        onlyValidPayment(paymentId) 
        onlyBusinessOwner(paymentId) 
        nonReentrant 
    ...
    function withdrawServiceFeeUsdc(uint256 _amount, address _fundingReceiver) 
        external 
        onlyOwner 
        onlyValidAddress(_fundingReceiver) 
        nonReentrant
    ...
    function withdrawServiceFeeUsdt(uint256 _amount, address _fundingReceiver) 
        external 
        onlyOwner 
        onlyValidAddress(_fundingReceiver) 
        nonReentrant
        ...
        nonReentrant 
    {
        ...
        emit WithdrawnAmount(paymentId, payment.paidAmount, payment.currency);
    }

    function withdrawAmountUsdc(uint256 paymentId) 
        external 
        onlyClient(paymentId) 
        nonReentrant 
    {
    ...
    function withdrawAmountUsdt(uint256 paymentId) 
        external 
        onlyClient(paymentId) 
        nonReentrant 
    {
