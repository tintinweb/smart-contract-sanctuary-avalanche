...
contract IndexPool is Operator, ERC20, IIndexPool {
  // BONE = 10**18
  ...
  function getTokenRecord(address token) public view returns (Record memory record) {
  ...
  constructor (
    ...
    require(_exitFeeRecipient != address(0), "DexETF: Fee recipient is zero address");
  ...
  function setExitFeeRecipient(address _exitFeeRecipient) public onlyOperator {
    require(_exitFeeRecipient != address(0), "DexETF: Fee recipient is zero address");
    exitFeeRecipient = _exitFeeRecipient;
    emit LOG_EXIT_FEE_RECIPIENT_UPDATED(_exitFeeRecipient);
  }

  function setMinimumBalance(address token, uint256 minimumBalance) public onlyOperator {
    Record storage record = _records[token];
    require(record.bound, "DexETF: Token not bound");
    require(!record.ready, "DexETF: Token already ready");
  ...
  function joinPool(uint256 poolAmountOut, uint256[] memory maxAmountsIn) public {
    address caller = msg.sender;
    uint256 poolTotal = totalSupply();
    uint256 ratio = poolAmountOut / poolTotal;
    require(ratio != 0, "DexETF: Invalid ratio");
    require(maxAmountsIn.length == _tokens.length, "DexETF: Invalid arrays length");
    uint256 _maxPoolTokens = maxPoolTokens;
    if (_maxPoolTokens > 0) require(
    ...
    for (uint256 i = 0; i < maxAmountsIn.length; i++) {
      ...
      uint256 tokenAmountIn = ratio * record.balance;
      require(tokenAmountIn != 0, "DexETF: Token amount in is zero");
      require(tokenAmountIn <= maxAmountsIn[i], "DexETF: Max amount in overflow");
      _updateInputToken(t, record, (realBalance + tokenAmountIn));
  ...
  function exitPool(uint256 poolAmountIn, uint256[] memory minAmountsOut) public {
    address caller = msg.sender;
    require(minAmountsOut.length == _tokens.length, "DexETF: Invalid arrays length");
    uint256 poolTotal = totalSupply();
    uint256 exitFee = poolAmountIn * EXIT_FEE;
    uint256 pAiAfterExitFee = poolAmountIn - exitFee;
    uint256 ratio = pAiAfterExitFee / poolTotal;
    require(ratio != 0, "DexETF: Invalid ratio");
    _transfer(caller, address(this), poolAmountIn);
    _transfer(address(this), exitFeeRecipient, exitFee / 2);
    _burn(caller, pAiAfterExitFee);
    for (uint256 i = 0; i < minAmountsOut.length; i++) {
      address t = _tokens[i];
      Record memory record = _records[t];
      if (record.ready) {
        uint256 tokenAmountOut = ratio * record.balance;
        require(tokenAmountOut != 0, "DexETF: Token amount out is zero");
        require(tokenAmountOut >= minAmountsOut[i], "DexETF: Min amount out overflow");
        _records[t].balance = record.balance - tokenAmountOut;
        emit LOG_EXIT(caller, t, tokenAmountOut);
        _pushUnderlying(t, caller, tokenAmountOut);
      } else {
        require(minAmountsOut[i] == 0, "DexETF: Min amount out overflow");
      }
    }
  }
  ...
  function addTokenAsset(address token, uint256 minimumBalance) public onlyOperator {
  ...
  function _getInputToken(address token) internal view returns (
    Record memory record,
    uint256 realBalance
  ) {
    record = _records[token];
