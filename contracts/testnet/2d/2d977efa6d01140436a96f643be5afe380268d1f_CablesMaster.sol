 ...
 **/
contract CablesEscrow is Ownable {
    ...
    **/
    function isExecutionFacility(address _sender) public view returns (bool) {
        uint length = executionFacilities.length;
        for (uint i = 0; i < length; i++) {
            if (executionFacilities[i] == _sender) {
                return true;
            }
        }
    ...
    **/
    function removeExecutionFacility(address _facility) external onlyOwner {
        uint length = executionFacilities.length;
        for (uint i = 0; i < length; i++) {
            if (executionFacilities[i] == _facility) {
                executionFacilities[i] = executionFacilities[length - 1];
                executionFacilities.pop();
                break;
            }
        }
    }
    ...
    **/
    function addExecutionFacility(address _facility) external onlyOwner {
        executionFacilities.push(_facility);
    }
    ...
    function createEscrow(
        ...
        if (_order.orderType == OrderType.Sell) {
            ...
            if (_order.baseCurrency != AVAX_ADDRESS) {
                ERC20(_order.baseCurrency).transferFrom(_order.creator, address(this), escrowBalance);
            }
        } else {
            ...
            if (_order.quoteCurrency != AVAX_ADDRESS) {
                ERC20(_order.quoteCurrency).transferFrom(_order.creator, address(this), escrowBalance);
            }
        }
    ...
    **/
    function cancelOrder(uint _escrowId) public {
        if (_escrowId >= escrowCount) {
            return;
        }

        Escrow storage escrow = escrows[_escrowId];

        if (escrow.user != msg.sender && msg.sender != master) {
            return;
        }
    ...
    **/
    function exchangeAssets(
        ...
        if (_sellEscrow.balance >= _buyEscrow.order.baseAmount) {
            ...
            transferToUser(baseCurrency, _buyEscrow.user, _buyEscrow.order.baseAmount);
            transferToUser(quoteCurrency, _sellEscrow.user, quoteAmount);
            ...
            transferToUser(baseCurrency, _buyEscrow.user, _sellEscrow.balance);
            transferToUser(quoteCurrency, _sellEscrow.user, quoteAmount);
    ...
    **/
    function partialFill(
        uint _makerEscrowId,
        uint _takerEscrowId
    ) external onlyExecutionFacility {
        ...
        require(
            makerEscrow.order.baseAmount > takerEscrow.order.baseAmount ||
            makerEscrow.order.status == OrderStatus.OpenPartiallyFilled,
            "The baseAmount of maker escrow does not match the partial fill"
        );
        ...
        gasCompensation(makerEscrow, takerEscrow, gasAmountLeft, msg.sender);
    }

    /**
    * @dev fills the order fully
    * @param _makerEscrowId the escrow id of maker order
    * @param _takerEscrowId the escrow id of taker order
    **/
    function fullFill(
        ...
        require(
            makerEscrow.order.baseAmount <= takerEscrow.order.baseAmount &&
                makerEscrow.order.status == OrderStatus.OpenUnfilled,
            "The baseAmount of maker escrow does not match the full fill"
        );
    ...
    **/
    function fill(
        ...
        if (_isLastFill && takerEscrow.order.status == OrderStatus.OpenPartiallyFilled) {
            cancelOrder(_takerEscrowId);
            if (takerEscrow.order.orderType == OrderType.Buy) {
                gasAmountLeft += 13188;
            } else {
                gasAmountLeft += 13200;
            }
        ...
        gasCompensation(makerEscrow, takerEscrow, gasAmountLeft, takerEscrow.order.creator);
    }

    /**
    * @dev internal function that compensations gas
    * @param _makerEscrow the escrow of maker order
    * @param _takerEscrow the escrow of taker order
    ...
    **/
    function gasCompensation(
        Escrow storage _makerEscrow,
        Escrow storage _takerEscrow,
        uint _gasAmountLeft,
        address _user
    ) internal {
        uint compensation;

        if (_makerEscrow.balance == 0) {
            compensation = (_gasAmountLeft - gasleft() + 24195) * tx.gasprice;

            if (_takerEscrow.balance == 0) {
                payable(_takerEscrow.order.creator).transfer(_takerEscrow.order.gasDeposit);
                _takerEscrow.order.gasDeposit = 0;
                compensation -= 28533 * tx.gasprice;
            }

            payable(_makerEscrow.order.creator).transfer(_makerEscrow.order.gasDeposit - compensation);
            _makerEscrow.order.gasDeposit = 0;
        } else {
            compensation = (_gasAmountLeft - gasleft() + 24009) * tx.gasprice;
            payable(_takerEscrow.order.creator).transfer(_takerEscrow.order.gasDeposit - compensation);
            _takerEscrow.order.gasDeposit = 0;
        }
    ...
    **/
    function setBasicGasDeposit(uint _newBasicGasDeposit) external onlyOwner {
    ...
    **/
    function setMaster(address _cablesMaster) external onlyOwner {
    ...
    **/
    function transferToUser(address _currency, address _user, uint256 _amount) internal {
        if (_currency == AVAX_ADDRESS) {
            payable(_user).transfer(_amount);
        } else {
            ERC20(_currency).transfer(_user, _amount);
        }
    }
 ...
 **/
contract CablesMaster {
    CablesEscrow public escrow;
    ...
    **/
    function createOrder(
        ...
        if (_orderType == OrderType.Sell && _baseAsset == AVAX_ADDRESS) {
            ...
            gasDeposit = msg.value - _quoteAmount;
        } else {
    ...
    **/
    function createAndFillTakerBuyOrder(
    ...
    ) external payable onlyExecutionFacility returns(uint) {
        uint gasAmountLeft = gasleft();

        uint length = _makerEscrowsIds.length;
        require(length <= 9, "Too many maker orders to process");

        uint escrowId = createOrder(_baseAmount, 0, _baseAsset, _quoteAsset, _maxPrice, OrderType.Buy, TradeType.Limit, 0);

        uint24[9] memory gasCompensationCorrections = [117589, 185714, 212710, 239704, 266713, 293717, 320721, 347726, 374728];
        uint8[9] memory lastFillGasCompensationCorrections = [0, 52, 40, 43, 47, 47, 42, 39, 36];
        ...
        for (uint i = 0; i < length; i++) {
            escrow.fill(
                _makerEscrowsIds[i],
                escrowId,
                i == length - 1 ? extraGas + lastFillGasCompensationCorrections[length-1] : extraGas,
    ...
    **/
    function createAndFillTakerSellOrder(
        ...
        uint24[9] memory gasCompensationCorrections = [117336, 185745, 212744, 239739, 266748, 293743, 320752, 347765, 374766];
        uint8[9] memory lastFillGasCompensationCorrections = [0, 49, 41, 44, 46, 36, 41, 48, 43];
