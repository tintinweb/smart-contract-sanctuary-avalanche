...
contract Colds {
    ...
    function reinvest(uint8 plan) public payable {
        ...
        User storage user = users[msg.sender];
        require(
            user.claimCheckpoint <= block.timestamp.sub(CLAIM_COOLDOWN),
            "User can only withdraw 1 time per 12 hours"
        );
        ...
        require(totalAmount > 0, "User has no enough rewards to reinvest");
