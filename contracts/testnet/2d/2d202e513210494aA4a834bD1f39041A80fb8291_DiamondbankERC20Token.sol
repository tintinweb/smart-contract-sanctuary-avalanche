...
contract DiamondbankERC20Token is ERC20Permit, VaultOwned {
    ...
    mapping (address => bool) public isIncludedFromFees;
    mapping (address => bool) public isExcludedFromFees;

    uint256 public taxFee = 2;
    uint256 public burnRate = 3;

    event IncludeFromFees(address indexed account, bool isIncluded);
    event InExudeFromFees(address indexed account, bool isExcluded);

    constructor(address _treasury) ERC20("DiamondBank", "DB", 9) {
        treasury = _treasury;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(isIncludedFromFees[to] && !isExcludedFromFees[from]) {
            uint256 taxAmount = amount.mul(taxFee).div(100);
            uint256 burnAmount = amount.mul(burnRate).div(100);

            super._transfer(from, to, amount.sub(taxAmount).sub(burnAmount));
    ...
    function setIncludeFromFees(address account) public onlyOwner {
        isIncludedFromFees[account] = !isIncludedFromFees[account];
        emit IncludeFromFees(account, isIncludedFromFees[account]);
    }

    function setExcludeFromFees(address account) public onlyOwner {
        isExcludedFromFees[account] = !isExcludedFromFees[account];
        emit IncludeFromFees(account, isExcludedFromFees[account]);
    }

    function updateTaxFee(uint256 _newTax) public onlyOwner {
        taxFee = _newTax;
    }
    ...
    function updateTreasuryAddr(address _treasury) public onlyOwner {
        treasury = _treasury;
    }
}