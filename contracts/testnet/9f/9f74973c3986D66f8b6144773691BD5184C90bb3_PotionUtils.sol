    ...
    Pausable
{
   ...
   */
    ...
    // Whether or not the NFT implements EIP-2981 royalty standard
    bool public supportsRoyalties;
   ...
   */
    ...
    function initialize(
        ...
        supportsRoyalties = nft().supportsInterface(type(IERC2981).interfaceId);
    }
        ...
        returns (uint256 inputAmount)
    {
        ...
        emit BuyNFTs(msg.sender, nftIds, inputAmount);
    }
...
import { LSSVMRouter } from "./LSSVMRouter.sol";
import { LSSVMPairETH } from "./LSSVMPairETH.sol";
import { ILSSVMPairRegistry, RegisterdPair } from "./ILSSVMPairRegistry.sol";
import { ICurve } from "./bonding-curves/ICurve.sol";
import { LSSVMPairERC20 } from "./LSSVMPairERC20.sol";
...
contract LSSVMPairFactory is Ownable, ILSSVMPairFactoryLike {
  ...
  ILSSVMPairRegistry public pairRegistry;

  event NewPair(address pairAddress, address poolAddress);
  event NewPool(address pairAddress);
  ...
  event RegistryStatusUpdate(ILSSVMPairRegistry router);
  ...
  function createPairETH(
  ...
  ) external payable returns (LSSVMPairETH pair, PotionPool pool) {
    require(
    ...
    address template;
    try
      IERC165(address(_nft)).supportsInterface(INTERFACE_ID_ERC721_ENUMERABLE)
    ...
    pool = new PotionPool(pair, poolName, poolSymbol);

    _registerPairOrFail(msg.sender, _nft, pair, pool);
  ...
  function createPairERC20(CreateERC20PairParams calldata params)
    external
    returns (LSSVMPairERC20 pair, PotionPool pool)
  {
    require(
  ...
  /**
     ...
     */
  function setRegistry(ILSSVMPairRegistry _pairRegistry) external onlyOwner {
...
contract PotionPool is ERC20 {
  ...
  */
  ...
  // redeems the max amount of fees for the owner's shares.
  // returns the shares burned and fees redeemed.
  function redeemFees(address owner)
    external
    returns (
  ...
  {
    // burn the owner's shares (must happen after calculating fees)
    shares = getRemainderShares(owner);

    uint256[] memory nftIds;
    (nftIds, tokens, fromFees) = redeemShares(shares, owner, owner);

    // // re-deposit nft-fungible pair
    // tokens = tokens.sub(getRequiredDeposit(numNfts));
    ...
    // burn only the shares required to redeem fees
    console.log("redeemRemainder", shares, tokens, fromFees);
  }
  ...
  // TODO remove??
  // redeems the max amount of shares the owner can burn without losing NFTs.
  function getRemainderShares(address owner) public returns (uint256 shares) {
    uint256 ownerBalance = balanceOf[owner];

    // get quantities to redeem
    uint256 numNfts;
    uint256 protocolFee;
    (numNfts, , , ) = getAssetsForShares(ownerBalance);

    // burn the owner's shares (must happen after calculating fees)
    shares = ownerBalance.sub(getSharesForAssets(numNfts));
  }
    ...
    returns (uint256 shares)
  {
    ...
    // totalShares = 100
    // nftBalance = 10       1 nft = 10 shares
    // tokenBalance = 1000   1 nft = 100 tokens; total value 2000

    // depositValue = numNfts * spotPrice * 2
    // totalValue = tokenBalance * 2 + feeBalance

    shares = getRequiredDeposit(numNfts.mul(2).mul(supply)).div(
      tokenBalance.mul(2).add(feeBalance)
    );
  }
    ...
    )
  {
    ...
    (uint256 nftBalance, uint256 tokenBalance) = pair.getBalances();
    uint256 feeBalance = pair.accruedFees();
    uint256 supply = totalSupply; // saves extra SLOAD
    uint256 totalValue = getTotalValue();

    // working ver
    // .mul(nftBalance.mul(tokenBalance.add(feeBalance)).mul(2))
    // numNfts = shares
    //   .mul(nftBalance.mul(tokenBalance.mul(2).add(feeBalance)))
    //   .div(supply.mul(2).mul(tokenBalance));

    // uint256 shareValue = totalValue.mul(shares).div(supply);
    // numNfts = totalValue.mul(shares).mul(nftBalance).div(tokenBalance.mul(2).mul(supply));
    // uint256 shareValue = totalValue.mulDivDown(shares, supply);

    // shares = getRequiredDeposit(numNfts.mul(2).mul(supply)).div(
    //     tokenBalance.mul(2).add(feeBalance)
    // );

    // spotPrice = tokenBalance.div(nftBalance);

    numNfts = totalValue.mulDivDown(nftBalance.mul(shares), supply.mul(tokenBalance.mul(2)));

    uint256 minShares = getSharesForAssets(numNfts);

    console.log("ALLOFTHIS", numNfts, minShares, shares);
    ...
    console.log("ALLOFTHIS2", numNfts, minShares, shares);
    ...
    fromFees = remainder.mul(feeBalance).div(totalValue);
...
struct RegisterdPair {
    address creator;
    address pair;
    address pool;
}

interface ILSSVMPairRegistry {
    function registerPair(address nft, RegisterdPair calldata pair)
        external
        returns (bool);
}
...
contract SmolrDoomers is ERC721URIStorage, Ownable {
  using Counters for Counters.Counter;
  ...
  constructor() ERC721("Smolr Doomers", "SMOLRS") {
...
import { Ownable } from "lib/openzeppelin-contracts/contracts/access/Ownable.sol";
import { RegisterdPair } from "./ILSSVMPairRegistry.sol";

/// @author 10xdegen.
// A Simple implementation of ILSSVMPairRegistry
// Allows on-chain indexing, whitelist, etc for LSSVMPairFactory.
contract LSSVMPairRegistry is Ownable {
  address public factory;

  mapping(address => RegisterdPair) public registeredPairs;
  ...
  modifier onlyFactoryOrOwner() {
    require(
      ...
      "LSSVMPairRegistry: FORBIDDEN"
    );

    _;
  }

    // this function only allows one pair per nft.
    // future implementationss may allow multiple pairs per nft.
  function registerPair(address nft, RegisterdPair calldata pair)
    external
    onlyFactoryOrOwner
    returns (bool)
  {
    if (registeredPairs[nft].pair != address(0)) {
      return false;
    }
    registeredPairs[nft] = pair;
    return true;
  }

    // resets a pair to the zero address to be reused.
    // admin only.
  function unregister(address nft)
    external
    onlyOwner
  {
    registeredPairs[nft].pair = address(0);
  }
}
