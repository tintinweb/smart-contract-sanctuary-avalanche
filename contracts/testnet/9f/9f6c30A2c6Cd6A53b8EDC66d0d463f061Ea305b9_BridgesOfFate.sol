...
contract BridgesOfFate  {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules
    {
        ...
        if((_hybirdGameMap  > 50e9) && (_dayDifferance(block.timestamp,_gameStatus.startAt) > _gameStatus.lastUpdationDay)){
            revert("Hybird Game Ended.");
        }
        if (GameMap[_member.stage + 1] <= 0) {
            if(_gameStatus.stageNumber >= STAGES){
                _hybirdGameMap = _random() * 1e9;
            }
        ...
        if(_checkSide(GameMap[_gameStatus.stageNumber],_jumpSide) && (_member.stage  > STAGES && _hybirdGameMap > 50e9 )){
    ...
    function withdrawWrappedEtherOFCommunity(uint8 withdrawtype) public  
    {
