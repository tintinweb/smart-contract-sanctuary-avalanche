...
interface IGameVRFManager {
    function reqRandomness() external returns (uint256);
}

contract WBCoinFlipManagerV1 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    VRFCoordinatorV2Interface COORDINATOR;
    IERC20 private _tokenContract;
    IGameVRFManager private GameVRFManager;
    address private GameVRFManagerAddress;

    address public TAX_ADDRESS;
    uint256 public TOTAL_TAX_AMOUNT;
    ...
    mapping(address => uint256) public addressWinStreak;

    uint256 public coinFlipIDCounter;
    event CoinFlipped(
    ...
    // Controls
    bool public paused;
    uint32 public checkFlipGasLimit;
    uint256 public minBet;
    uint256 public winTax;
    uint256 public loseTax;
    ...
    function initialize(
        ...
        GameVRFManager = IGameVRFManager(address(vrfManagerAddress));
        GameVRFManagerAddress = address(vrfManagerAddress);

        paused = false;
        coinFlipIDCounter = 500;
        checkFlipGasLimit = 40000;
        minBet = 500000000000000000000; // in wei -> 500 CHRO
        winTax = 100; // 10% will go tax address rest to winner
        loseTax = 0; // 0% will go to tax address rest to this contract
    }

    // MODIFIERS ================
    modifier isGameVRFManager() {
        require(GameVRFManagerAddress == _msgSender(), "You are not allowed");
        _;
    }
    ...
    function coinFlip(uint256 amountCHRO, uint8 choice) external nonReentrant {
        ...
        require(minBet >= amountCHRO, "Less than minimum bet");

        address theBetStarter = _msgSender();
        ...
        _tokenContract.transferFrom(_msgSender(), address(this), amountCHRO);

        reqIdToFlipId[GameVRFManager.reqRandomness()].push(coinFlipID);
        ...
        emit FlipStarted(coinFlipID, _msgSender(), choice, amountCHRO, reward);
        coinFlipIDCounter += 1;
    }

    function settle(uint256 _requestId, uint256[] memory randomNumbers)
        internal
        isGameVRFManager
    {
    ...
    function _checkFlip(uint256 flipId, uint256 _randomNumber) internal {
        ...
        if (isWin) {
            ...
            addressWinStreak[c.betStarter]++;

            // TF REWARD
            if (addressWinStreak[c.betStarter] == 3) {
                winStreakReward = c.bet / 4;
                _tokenContract.transfer(
                    c.betStarter,
                    c.reward + winStreakReward
                );
            } else {
            ...
            addressWinStreak[c.betStarter] = 0;
        }
        ...
        returns (bool, string memory)
    {
        ...
        if (block.number <= c.blockPlaced + 10 + 5)
    ...
    function setVRFManager(address vrfManagerAddress) external onlyOwner {
        GameVRFManager = IGameVRFManager(address(vrfManagerAddress));
        GameVRFManagerAddress = address(vrfManagerAddress);
    }
