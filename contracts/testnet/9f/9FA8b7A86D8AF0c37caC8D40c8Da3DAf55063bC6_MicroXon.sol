...
contract MicroXon is Ownable, Pausable {
    ...
    uint256 public xonCost = 5 * (10 ** 18);

    uint256 public xonCreationFee = 15 * (10 ** 6);

    uint256 public claimRewardFee = 5 * (10 ** 6);

    uint256 public subscriptionFee = 15 * (10 ** 6);
    ...
    event CreatingXonEvent(address indexed userWallet, uint256 indexed amount, uint8 indexed _type, bool payInTokens);
    ...
    event SetXetaTokenEvent(address indexed xetaTokenAddress);
    ...
    constructor(address _usdcToken, address _xetaToken) {
    ...
    function mintXon(uint256 amount, uint256 _xonCreationFee, uint8 _type, bool payInTokens) external whenNotPaused {

        require(_xonCreationFee == xonCreationFee, "Invalid XON creation fee");

        require(amount > 0, "Illegal amount");

        require(xetaToken.balanceOf(msg.sender) >= xonCost * amount, "Insufficient XETA balance");

        require(xetaToken.allowance(msg.sender, address(this)) >= xonCost * amount, "Insufficient XETA allowance");
        ...
        if (payInTokens) {
            ...
            xetaToken.transferFrom(msg.sender, address(xetaToken), xonCost * amount);

        }

        emit CreatingXonEvent(msg.sender, amount, _type, payInTokens);
    ...
    function setXetaToken(address _xetaToken) external onlyOwner {

        xetaToken = IERC20(_xetaToken);

        emit SetXetaTokenEvent(_xetaToken);
