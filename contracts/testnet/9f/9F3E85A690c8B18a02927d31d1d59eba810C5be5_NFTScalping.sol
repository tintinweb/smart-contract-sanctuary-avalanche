...
// File: contracts/interfaces/IRadius.sol
...
// File: contracts/interfaces/IScalperNFT.sol
...
// File: contracts/interfaces/INFTScalping.sol
...
// File: contracts/NFTScalping.sol
    ...
    IERC721Receiver
{
    ...
    uint256 public totalAllocattionLimit;
    ...
    /**
     * @dev Used by owner to vacate tenant in case when rental period expires.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     * - NFT should be on rent
     * - Rental duration must be completed.
     * - _msgSender must be the NFTScalping contract
     * -
     *
     * Emits a {RentalTerminated} event.
     */
    function terminateRental(uint tokenId)
        ...
        onlyOwner
    {
        NFTRentInfo memory _nftInfo = nftRentInfos[tokenId];
        require(_nftInfo.tenant != address(0), "NFTScalping: NFT not on rent");
        require(
            _nftInfo.rentalDuration + _nftInfo.rentStartTime < block.timestamp,
            "NFTScalping: NFt is in active rent duration"
        );

        address tenant = _nftInfo.tenant;
        ...
        totalAllocatedpool -= _nftInfo.scalpingPercentage;
        // set _nftInfo as un rented
        _nftInfo.tenant = address(0);
        _nftInfo.rentalDuration = 0;
        _nftInfo.rentStartTime = 0;
        _nftInfo.rentPaid = 0;
        _nftInfo.scalpingPercentage = 0;
        nftRentInfos[tokenId] = _nftInfo;
    }

    /**
     * @dev Used by owner to mint new NFTs.
     *
     * Requirements:
     *
     * - `_count` must be be greater than zero.
     * -
     */
    function mint(
    ...
    /**
     * @dev Used by owner to set values in RentalLevelInfo for each level
     *
     * Requirements:
     *
     * - `_rentalLevel` must be be of type RentalLevels.
     * - `_priceInAvax` must be be greater than zero.
     * - `_priceInRadius` must be be greater than zero.
     * - `_duration` must be be greater than zero.
     * - `_scalpingPercentage` must be be greater than zero.
     */
    function setRentalInformation(
        RentalLevels _rentalLevel,
        uint256 _priceInAvax,
    ...
    function _setRentalInformation(
        ...
        require(
            _scalpingPercentage > 0,
            "NFTScalping: _scalpingPercentage must be greater than zero"
        );
    ...
    function setTotalAllocationLimit(uint256 _limit)
        ...
        nonReentrant
    {
        require(_limit > 0, "NFTScalping: can not set limit equal to zero");
        require(
            _limit >= totalAllocatedpool,
            "NFTScalping: can not set limit less than current allocation pool"
        );
        totalAllocattionLimit = _limit;
    }

    /**
     * @dev can be used to set Scalper NFT contract address
     */
    function setScalperNFT(address _scalperNFT) external onlyOwner {
        scalperNFT = IScalperNFT(_scalperNFT);
    }

    /**
     * @dev can be used to mint tokens to this contract address
     */
    function mintRadius(uint256 amount) external onlyOwner {
        radiusToken.mint(amount);
    }

    /**
     * @dev can be used to transfer balance amount to owner
     */
    ...
    /**
     * @dev Can be used by tenant to rent an NFT
     *      Can be used on a NFT where rental payment has expired
     *      Rent can be paid in either RADIUS tokens or in ether
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     * - `_rentalLevel must be of Type RentalLevels
     * - if previously rented, Rental duration must be completed.
     * - _msgSender must be the NFTScalping contract
     * - _currency must be of type Currency
     *
     * Emits a {NFTRented} event.
     */
    function rentNFT(
        uint256 tokenId,
        RentalLevels _rentalLevel,
        ...
        NFTRentInfo memory _nftInfo = nftRentInfos[tokenId];
        if (_nftInfo.tenant != address(0)) {
            // if rented previously
            require(
                _nftInfo.rentalDuration + _nftInfo.rentStartTime <
                    block.timestamp,
        ...
        if (_currency == Currency.AVAX) {
            ...
            require(
                allowance >= price,
                "PurchaseNFT: price amount not approved"
            );
        ...
        uint256 rentStartTime = block.timestamp;
        _nftInfo.tokenId = tokenId;
        _nftInfo.tenant = sender;
        _nftInfo.rentStartTime = rentStartTime;
        _nftInfo.rentPaid += price; // needed ?
        _nftInfo.rentalDuration = _requestedRentalLevel.duration;
        _nftInfo.scalpingPercentage = _requestedRentalLevel.scalpingPercentage;
        nftRentInfos[tokenId] = _nftInfo;
    ...
    /**
     * @dev used to transfer rewards to tenant of NFT holder
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    function transferRewards(uint256 tokenId) external virtual override {
        ...
        require(_nftInfo.tenant != address(0), "NFTScalping: NFT not on rent");
        require(
            _nftInfo.nextScalpTime != 0,
            "NFTScalping: rewards transferred maximum number of times"
        );
        require(
            _nftInfo.nextScalpTime <= block.timestamp,
            "NFTScalping: can not transfer rewards before next scalp time"
        );
        uint256 totalRewardsPercentage = 0;
        while (_nftInfo.nextScalpTime <= block.timestamp) {
            totalRewardsPercentage += _nftInfo.scalpingPercentage;
            _nftInfo.nextScalpTime += timeIntervalForRewards;
        }
        if (
            _nftInfo.nextScalpTime >
            _nftInfo.rentalDuration + _nftInfo.rentStartTime
        ) {
            _nftInfo.nextScalpTime = 0;
        }
        uint256 tBalance = radiusToken.balanceOf(address(this));
        uint256 rBalance = radiusToken.reflectionFromToken(tBalance, false);
        uint256 totalRewards = (rBalance * totalRewardsPercentage) /
            DENOMINATOR;
        radiusToken.transfer(_nftInfo.tenant, totalRewards);
        nftRentInfos[tokenId] = _nftInfo;
        emit RewardsTransferred(tokenId, _nftInfo.tenant, totalRewards);
    }

    // ======== View only functions ========

    /**
     * @dev Can be used to check if NFT is currently on rent.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    /**
     * @dev used to get the tenant of rented NFT.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    /**
     * @dev used to get the token ID of rented NFT by tenant and index.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     * - `index` must be less than rentCount for this tenant
     */
    function rentedByIndex(address tenant, uint256 index)
        public
        view
    ...
    /**
     * @dev used to check if this NFT is rentable.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    /**
     * @dev used to get the end time for rental
     *
     * Requirements:
     *
     * - `tokenId` must exist.
        ...
        returns (uint256)
    {
        NFTRentInfo memory _nftInfo = nftRentInfos[tokenId];
        require(_nftInfo.tenant != address(0), "NFTScalping: NFT not on rent");
        return _nftInfo.rentalDuration + _nftInfo.rentStartTime;
    }
