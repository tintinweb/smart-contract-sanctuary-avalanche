...
contract PerpetualTokensFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        returns(bool)
    {
        ...
        for(uint256 i = 0; i < stakeInfo[user].length; i++){
            if(stakeInfo[user][i].epochId == epochId){
        ...
        returns(bool)
    {
        ...
        if(noOfUsers[_epochId] == 1){
            totalAmount = 0;
            for(uint256 i = 0; i < stakeInfo[user].length; i++){
                if(stakeInfo[user][i].epochId == _epochId){
                    totalAmount += stakeInfo[user][i].amount;
                }
            }
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        ...
        // Emit Withdraw event
        emit Withdraw(msg.sender, stakeId, _amount, stake.epochId);
    }
        ...
        internal
    {
        ...
        } else if (stakeFeePercent[_epochId] > 0) {
            // Collect reward fee
