...
ReentrancyGuardUpgradeable
{
    ...
    ICertToken private _aAVAXc;
    address private _router;
    mapping(address => uint256) private _claimed; // in aAVAXc
    mapping(address => uint256) private _depositors; // in aAVAXc
    mapping(address => uint256) private _ceTokenBalances; // in aAVAXc
    /**
     * Modifiers
     */
    modifier onlyRouter() {
    ...
    function initialize(
        ...
        _aAVAXc = ICertToken(aAVAXcAddress);
    }
    // deposit
    ...
    returns (uint256)
    {
        uint256 ratio = _aAVAXc.ratio();
        _aAVAXc.transferFrom(msg.sender, address(this), amount);
        uint256 toMint = (amount * 1e18) / ratio;
        _depositors[account] += amount; // aAVAXc
    ...
    returns (uint256)
    {
        ...
        _aAVAXc.transfer(recipient, availableYields);
    ...
    {
        uint256 ratio = _aAVAXc.ratio();
        return (_ceTokenBalances[account] * ratio) / 1e18; // in aAVAXc
    }
    // yield = deposited*(1-current_ratio/init_ratio) = cetoken.balanceOf*init_ratio-cetoken.balanceOf*current_ratio
    // yield = cetoken.balanceOf*(init_ratio-current_ratio) = amount(in aAVAXc) - amount(in aAVAXc)
