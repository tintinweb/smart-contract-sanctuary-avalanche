...
import {IVotingStrategy} from '../voting/interfaces/IVotingStrategy.sol';
...
contract VotingMachineDataHelper is IVotingMachineDataHelper {
  ...
  function getProposalsData(
    ...
    IVotingStrategy votingStrategy = votingMachine.getVotingStrategy();
    address[] memory votingAssets = IBaseVotingStrategy(address(votingStrategy))
      .getVotingAssetList();

    for (uint256 i = 0; i < initialProposals.length; i++) {
      IVotingMachineWithProofs.ProposalState state = votingMachine
        .getProposalState(initialProposals[i].id);

      if (state > IVotingMachineWithProofs.ProposalState.NotCreated) {
        proposalWithoutVotes = votingMachine.getProposalById(
          initialProposals[i].id
        );

        if (user != address(0)) {
          vote = votingMachine.getUserProposalVote(
            user,
            initialProposals[i].id
          );
        ...
        proposals[i] = Proposal({
          ...
          bridgedVoteInfo: BridgedVoteInfo({
            isBridgedVote: bridgedVote.votingTokens.length > 0,
            support: bridgedVote.support,
            votingTokens: bridgedVote.votingTokens
          }),
          votingAssets: votingAssets,
          hasRequiredRoots: _hasRequiredRoots(
            votingStrategy,
            initialProposals[i].snapshotBlockHash
          )
        });
      } else {
        proposals[i] = _getEmptyProposal(
          initialProposals[i].id,
          initialProposals[i].snapshotBlockHash,
          votingStrategy,
          votingAssets
    ...
    return
      Proposal({
        ...
        bridgedVoteInfo: BridgedVoteInfo({
          isBridgedVote: false,
          support: false,
          votingTokens: new address[](0)
        }),
        votingAssets: votingAssets,
        hasRequiredRoots: _hasRequiredRoots(strategy, snapshotBlockHash)
      });
  }
}
