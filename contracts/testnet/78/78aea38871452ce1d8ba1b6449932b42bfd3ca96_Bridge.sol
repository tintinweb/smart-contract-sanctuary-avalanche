...
contract Bridge {
    ...
    event LogReceive(address indexed recipent, uint amount, uint tokenID, string tokenName);
    ...
    function deposit(uint256 _amount,uint _tokenID) public {
        token = IERC20(supportedTokens[_tokenID].tokenAddress);
        bool tx = token.transferFrom(msg.sender, address(this), _amount);
        require(tx, "Not Confirmed");
    ...
    function recive(uint256 _amount, address _recipent, uint _tokenID) public onlyBridge {
        token = IERC20(supportedTokens[_tokenID].tokenAddress);
        token.transferFrom(address(this), _recipent, _amount);
        emit LogReceive(_recipent, _amount, _tokenID,supportedTokens[_tokenID].name);
    }
    ...
    function ClearERC20(uint256 _amount, address _recipent, uint _tokenID) public onlyOwner {
        token = IERC20(supportedTokens[_tokenID].tokenAddress);
        token.transferFrom(address(this), _recipent, _amount);
        emit LogReceive(_recipent, _amount, _tokenID,supportedTokens[_tokenID].name);
    }
