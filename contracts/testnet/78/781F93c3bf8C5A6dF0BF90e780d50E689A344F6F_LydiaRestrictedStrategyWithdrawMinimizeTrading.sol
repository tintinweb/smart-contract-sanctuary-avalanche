...
contract LydiaRestrictedStrategyWithdrawMinimizeTrading is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(
      ...
      "LydiaRestrictedStrategyWithdrawMinimizeTrading::onlyWhitelistedWorkers:: bad worker"
    );
    _;
  }
  ...
  /// @param _wavax The wrapped BNB token.
  ...
  function execute(
    ...
    require(
      ...
      "LydiaRestrictedStrategyWithdrawMinimizeTrading::execute:: insufficient farming tokens received"
    );
    if (remainingFarmingToken > 0) {
      if (farmingToken == address(wavax)) {
    ...
    _approveTokens(address(lpToken), farmingToken, false);
  }
