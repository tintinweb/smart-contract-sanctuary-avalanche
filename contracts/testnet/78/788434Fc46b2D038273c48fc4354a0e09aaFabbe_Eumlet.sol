...
contract Eumlet is Ownable, ReentrancyGuard {
    ...
    event CancelStream(address user, uint128 id);
     ...
     * @param accounts - addresses of users, who will receive tokens
     * @param amounts - amounts of tokens, accounts[i] will receive amounts[i]
     * @param params - [0] - start, [1] - steps num, [2] - step in sec, [3] - totalAmount
     */
    function multistream(
        ...
        emit Multistream(msg.sender, token, params[0], accounts, ids, _amounts);
    }
        ...
        exist(userToIds[user].streamId, streamId)
    {
        ...
        require(stream.start < block.timestamp, "Stream not start yet");
        stream.paused = true;
