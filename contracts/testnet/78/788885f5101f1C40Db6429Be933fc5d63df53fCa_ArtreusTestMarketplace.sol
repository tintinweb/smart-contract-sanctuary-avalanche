...
contract ArtreusTestMarketplace is ReentrancyGuard, Ownable {
    ...
    mapping(address=>mapping(uint => MarketItem)) public _items;

    mapping(address => mapping(uint => MarketItem)) public listings;
    ...
    function createMarketItem(
        ...
        idToMarketItem[nftContract][itemId] = MarketItem(
    ...
    function buy(address nftContract, uint256 itemId) public payable nonReentrant {
        uint256 price = idToMarketItem[nftContract][itemId].price;
        uint256 tokenId = idToMarketItem[nftContract][itemId].tokenId;
        MarketItem storage item = idToMarketItem[nftContract][itemId];
        ...
        if(msg.value>price){
            uint rem = msg.value - price;
            (bool success,) = payable(msg.sender).call{value:rem}("");
            success?true:false;
        }
        ...
        emit MarketItemSold(
            ...
            nftContract,
            tokenId,
            idToMarketItem[nftContract][itemId].seller,
            idToMarketItem[nftContract][itemId].owner,
            price,
            royaltyAmount,
            royaltyRecipient
        );
    ...
    // Cancel Sale
    function cancelSale(address nftcontract,uint _itemId) public {
        MarketItem storage item = idToMarketItem[nftcontract][_itemId];
    ...
    function getTotalPrice(address nftContract,uint itemId) view public returns(uint) {
        return idToMarketItem[nftContract][itemId].price*(100 + feePercent/100);
    }
    ...
    function fetchMarketItems(address nftContract) public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToMarketItem[nftContract][i + 1].owner == address(0)) {
                uint256 currentId = i + 1;
                MarketItem storage currentItem = idToMarketItem[nftContract][currentId];
    ...
    function fetchMyNFTs(address nftContract) public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[nftContract][i + 1].owner == msg.sender) {
                itemCount += 1;
            }
        }
    ...
    function fetchItemsCreated(address nftContract) public view returns (MarketItem[] memory) {
        uint256 totalItemCount = _itemIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[nftContract][i + 1].seller == msg.sender) {
                itemCount += 1;
            }
        }
