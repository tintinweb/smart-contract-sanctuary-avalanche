...
// File: contracts/BridgingWolfies final/BridgingWolfies.sol
...
/// @title Bridging Wolfies
contract BridgingWolfies is ERC721Metadata {
   ...
   function mint(uint256 counts_)
        external payable {
        require(pendingCount > 0, "Bridging Wolfies: All minted");
        require(counts_ > 0, "Bridging Wolfies: Counts cannot be zero"); 
        require(totalSupply().add(counts_) <= MAX_NFT_SUPPLY,
            "Bridging Wolfies: sale already ended"); 
        require(!paused, "Bridging Wolfies: The contract is paused"); 
        require(MINT_PRICE.mul(counts_) == msg.value,
            "Bridging Wolfies: invalid ether value");
    ...
    function _randomMint(address to_)
        ...
        require(to_ != address(0), "Bridging Wolfies: zero address");

        require(totalSupply() < MAX_NFT_SUPPLY,
            "Bridging Wolfies: max supply reached");
