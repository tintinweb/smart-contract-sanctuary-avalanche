...
contract NodeManager is Ownable, Pausable {
    ...
    function checkInsolvent(address _account) external returns (uint256){
    	...
    	for(uint i=0;i<nodes.length;i++){
    		if (timer = false) {
    			...
    			_node.deathclock = block.timestamp - _node.creationTime;
    			_node.nextDue = _node.nextDueElapsed + block.timestamp;
    		}else{
    			if ((nodes[i].nextDue + 5 days) < block.timestamp  && nodes[i].deleted != true) {
    				NodeEntity storage _node = nodes[i];
    				if (_node.lastClaimTime < _node.nextDue){
    					(uint256 take,uint256 rewards_boost) = boostLib.calcReward(dailyRewardsPerc,timeStep,_node.nextDue,_node.lastClaimTime,0);
    					remains += take;
    					
    				}
    				for(uint j=i;j< nodes.length;j++){
	    				if (j == (nodes.length - 1)){
	    					nodes[j];
	    					delete nodes[j];
				      		nodes.pop();
    ...
    function doPayments(address _account,uint256 payments) external onlyGuard onlyNodeOwner(_account) whenNotPaused  {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	for(uint i=0;i<nodes.length;i++){
    ...
    function doFuturePayments(address _account,uint256 payments) external onlyGuard onlyNodeOwner(_account) whenNotPaused  {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	for(uint i=0;i<nodes.length;i++){
