...
contract Farming is OwnableUpgradeable, ReentrancyGuard {
    ...
    function _harvest(uint256 pendingAvat, uint256 pendingBonus) internal {
        ...
        if (pendingBonus != 0) {
            uint256 senderAmount = pendingBonus * REWARDS_FEE / 1000;

            _safeTransfer(bonusToken, msg.sender, senderAmount);
            _safeTransfer(bonusToken, address(farmingFactory), pendingBonus - senderAmount);
        }
...
abstract contract OwnableTimelock is Context {
     ...
     * Can only be called by the current owner.
     */
    function confirmTransferOwnership(address newOwner) public virtual onlyOwner {
