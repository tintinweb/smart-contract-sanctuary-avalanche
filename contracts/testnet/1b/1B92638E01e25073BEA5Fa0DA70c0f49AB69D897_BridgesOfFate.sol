...
contract BridgesOfFate is Ownable  {
    ...
    uint256 private constant STAGES = 5; 
    uint256 private constant TURN_PERIOD = 300;//10 min // 5 minutes // 3 hours 10800
    ...
    event ParticipateOfPlayerInGame(bytes32 playerId,uint256 stageGlobal,uint256 stagePlayer,uint256 randomNumber, bool safeSide,bool jumpAt);
    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules {
        GameItem storage _member = PlayerItem[playerId]; //memory
        ...
        lastUpdateTimeStamp = block.timestamp;
        uint256 _rn = GameMap[_currentUserStage];
        //Push winner into the Array list 
        if((_member.stage  == STAGES)){
            if (_jumpSide == true && _rn >= 50e9) {
                gameWinners.push(playerId);

            }
            if (_jumpSide == false && _rn < 50e9) {
                gameWinners.push(playerId);

            }
           /*if(_checkSide(GameMap[_currentUserStage],_jumpSide)){
                gameWinners.push(playerId);
           }*/
        }

        bool _test;
        if ((_jumpSide == true && GameMap[_currentUserStage] >= 51e9) || (_jumpSide == false && GameMap[_currentUserStage] < 50e9)) {
            _test =  true;
        }

        emit ParticipateOfPlayerInGame(
            playerId,
            _gameStatus.stageNumber,
            _currentUserStage,GameMap[_currentUserStage],
            _checkSide(GameMap[_currentUserStage],_jumpSide),
            _jumpSide
        );
        PlayerItem[playerId] = _member;
        GameStatusInitialized[latestGameId] = _gameStatus;
        // emit ParticipateOfPlayerInGame(_test,
        //     playerId,
        //     GameStatusInitialized[latestGameId].stageNumber,
        //     PlayerItem[playerId].stage,
        //     GameMap[_currentUserStage],
        //     _checkSide(GameMap[_currentUserStage],PlayerItem[playerId].lastJumpSide),
    ...
    function _checkSide(uint256 stageNumber, bool userSide) public view returns (bool)
    {
        ...
        if ((userSide == true && stage_randomNumber >= 51e9) || (userSide == false && stage_randomNumber < 50e9)) {
            return true;
        }else{
           return false; 
        }
