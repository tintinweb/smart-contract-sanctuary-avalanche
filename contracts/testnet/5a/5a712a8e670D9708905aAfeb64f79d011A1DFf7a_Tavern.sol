...
contract Tavern is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public winPecentage = 25; // 33% Chance of Winning
    ...
    function claimRewardBatch(uint256[] memory _tokenIds) public whenNotPaused {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
    ...
    function daysSinceLastBoost(uint256 _tokenId) public view returns (uint256 intervals) {
        BoostedNFT storage boostedNFT = boostedNFTs[_tokenId];
        intervals = boostedNFT.tokenBoostingCoolDown != 0 &&
            boostedNFT.tokenBoostingCoolDown < block.timestamp
            ? (block.timestamp - boostedNFT.tokenBoostingCoolDown) / boostInterval
            : 0;
    }
    ...
    function calculateRewardBatch(uint256[] memory _tokenIds) public view returns (uint256) {
        uint256 claimableReward = 0;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            BoostedNFT storage boostedNFT = boostedNFTs[_tokenIds[i]];
            if (
