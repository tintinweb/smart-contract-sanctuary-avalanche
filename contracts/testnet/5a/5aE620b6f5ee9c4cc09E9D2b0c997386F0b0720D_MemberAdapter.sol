    ...
    AdapterGuard 
{
    ...
    modifier reentrancyAdapterGuard(address daoAddress) {
        require(_lockedAt[daoAddress] != block.number, "adapter reentrancy guard");
    ...
    struct SetMemberAgreementProposal {
        Foundance.ProposalStatus status;
        Foundance.MemberSetupConfig[] memberSetupConfigArray;
    ...
    function submitSetMemberAgreementProposal(
        ...
        bytes32 proposalId,
        bytes memory data,
        Foundance.MemberSetupConfig[] memory _memberSetupConfigArray,
        ...
        onlyMember(dao)
    {
        Foundance.MemberSetupConfig[] storage memberSetupConfigArray = setMemberAgreementProposal[address(dao)][proposalId].memberSetupConfigArray;
    ...
    function processSetMemberProposal(DaoRegistry dao, bytes32 proposalId)
        external override
        reentrancyAdapterGuard(address(dao))
    {
        ...
        reentrancyAdapterGuard(address(dao))
    {
        ...
        if (proposal.status == Foundance.ProposalStatus.IN_PROGRESS) {
            ...
            for(uint256 i=0; i < proposal.memberSetupConfigArray.length; i++){
                Foundance.MemberSetupConfig memory factoryMemberConfig = proposal.memberSetupConfigArray[i];
                member.setMember(
                    dao,
  ...
  AdapterGuard
{
    ...
    function cancelVote(DaoRegistry dao, bytes32 proposalId
    ) external onlyAdapter(dao) {
        Voting storage vote = voting[address(dao)][votingIndex[address(dao)][proposalId]];
    ...
    {
        Voting memory vote = voting[address(dao)][votingIndex[address(dao)][proposalId]];
        Foundance.VotingConfig memory _votingConfig = votingConfig[address(dao)];
        if (vote.startingTime == 0) {
            return Foundance.VotingState.NOT_STARTED;
        }
        if (
            block.timestamp < vote.startingTime + _votingConfig.votingPeriod
        ) {
            return Foundance.VotingState.IN_PROGRESS;
        }
        if (
            block.timestamp <
            vote.startingTime +
            ...
            return Foundance.VotingState.GRACE_PERIOD;
        }
        if(vote.votingType == Foundance.VotingType.OPTIMISTIC){
            if (vote.nbYes >= vote.nbNo) {
                return Foundance.VotingState.PASS;
            } else {
                return Foundance.VotingState.NOT_PASS;
            }
        }else{
            BankExtension bank = BankExtension(
                dao.getExtensionAddress(DaoLibrary.BANK_EXT)
            ...
            if (vote.nbYes > vote.nbNo) {
                ...
                return Foundance.VotingState.TIE;
            }
        }
    }
...
contract MemberExtension is IExtension {
    ...
    function removeMember(
        DaoRegistry dao,
        address _memberAddress
    ) external hasExtensionAccess(dao, AclFlag.REMOVE_MEMBER) {
    ...
    function getMemberConfig(
        address _memberAddress
    ) public view returns (Foundance.MemberConfig memory) {
        require(
            memberIndex[_memberAddress]>0,
            "member not set"
        );
...
library VotingAdapterLibrary {
    ...
    function _cancelProposal(
        ...
        IVotingAdapter votingAdapter = IVotingAdapter(dao.votingAdapter(proposalId));
        require(address(votingAdapter) != address(0), "votingAdpt::adapter not found");
        votingAdapter.cancelVote(dao, proposalId);
        return Foundance.ProposalStatus.FAILED;
    }
}
...
interface IMemberAdapter {
    ...
    function processSetMemberAgreementProposal(DaoRegistry dao, bytes32 proposalId) external;
...
interface IVotingAdapter {
    ...
    function voteResult(
        DaoRegistry dao, 
        bytes32 proposalId
    ) external view returns (Foundance.VotingState state);
...
contract ERC20Extension is AdapterGuard, IExtension, IERC20 {
    ...
    function _transferInternal(
        ...
        DaoLibrary.potentialNewMember(recipient, dao, bank);
...
contract DynamicEquityExtension is IExtension {
    ...
    function getDynamicEquityMemberEpochAmount(
        ...
        Foundance.DynamicEquityMemberConfig memory _epochMemberConfig = dynamicEquityEpochs[epochConfig.epochLast][_memberAddress];
        if(_epochMemberConfig.memberAddress != address(0)){
            return _getDynamicEquityMemberEpochAmountInternal(
                _epochMemberConfig
            );
        }else{
            return _getDynamicEquityMemberEpochAmountInternal(
                dynamicEquityMemberConfig[dynamicEquityMemberIndex[_memberAddress]-1]
            );
        }
    }
...
contract VestedEquityExtension is IExtension {
    ...
    function getVestedEquityMemberConfig(
        ...
        require(vestedEquityMemberIndex[_member]>0,"_member not set");
...
contract CommunityEquityExtension is IExtension {
    ...
    function removeCommunityEquityMember(
        ...
        bytes32 communityEquityId,
        address _memberAddress
    ) external hasExtensionAccess(dao, AclFlag.REMOVE_COMMUNITY_EQUITY) {
    ...
    function getCommunityEquityEpochConfig(
        ...
        require(communityEquityIndex[communityEquityId]>0,"communityEquity not set");
    ...
    function getCommunityEquityMemberConfig(
        ...
        require(communityEquityMemberIndex[communityEquityId][_memberAddress]>0,"communityEquity not set");
