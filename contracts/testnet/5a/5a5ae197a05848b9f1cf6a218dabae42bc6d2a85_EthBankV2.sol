...
contract EthBankV2 is EthBank, OwnableUpgradeable {
    ...
    function deposit(address token_, uint256 amount_) public override {
        require(!paused, "All transaction is paused");
    ...
    function transfer(
        ...
        require(_accounts[msg.sender], "Sender is not registered");
        require(_accounts[to_], "Receiver is not registered");

        require(amount_ > 0, "Amount should not be zero");
        require(
...
contract EthBank is Initializable {
    ...
    function createAccount() public {
        ...
        require(!_accounts[msg.sender], "Account already created");
    ...
    function deposit(address token_, uint256 amount_) public virtual {
        require(_accounts[msg.sender], "Account is not registered");
        require(amount_ > 0, "Amount should not be zero");
        _balances[msg.sender][token_] += amount_;
    ...
    function withdraw(address token_, uint256 amount_) public virtual {
        require(_accounts[msg.sender], "Account is not registered");
        require(amount_ > 0, "Amount should not be zero");
        require(_balances[msg.sender][token_] >= amount_, "Insufficient amount");
        _balances[msg.sender][token_] -= amount_;
