...
// DSFeeHandler_V2
contract DSFeeHandler is UUPSUpgradeable, OwnableUpgradeable {
    ...
    event NewDosSwapRouter(address indexed sender, address indexed router);
    event NewOperatorAddress(address indexed sender, address indexed operator);
    event NewCakeBurnAddress(
        address indexed sender,
        address indexed burnAddress
    );
    event NewCakeVaultAddress(
        address indexed sender,
        address indexed vaultAddress
    );
    event NewCakeBurnRate(address indexed sender, uint256 cakeBurnRate);

    address public cake;
    IDosRouter02 public dosSwapRouter;
    address public operatorAddress; // address of the operator
    address public cakeBurnAddress;
    address public cakeVaultAddress;
    uint256 public cakeBurnRate; // rate for burn (e.g. 718750 means 71.875%)
    ...
    function initialize(
        address _cake,
        address _dosSwapRouter,
        address _operatorAddress,
        address _cakeBurnAddress,
        address _cakeVaultAddress,
        uint256 _cakeBurnRate,
        address[] memory destinations
    ) external initializer {
        __Ownable_init();
        __UUPSUpgradeable_init();
        cake = _cake;
        dosSwapRouter = IDosRouter02(_dosSwapRouter);
    ...
    function swap(
        ...
        try
            dosSwapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                amountIn,
                amountOutMin,
                path,
                address(this),
     ...
     * @dev Callable by owner/operator
     */
    function sendCake(uint256 amount) external onlyOwnerOrOperator {
        require(amount > 0, "invalid amount");
        uint256 burnAmount = (amount * cakeBurnRate) / RATE_DENOMINATOR;
    ...
    /**
     * @notice Set DosSwapRouter
     * @dev Callable by owner
     */
    function setDosSwapRouter(address _dosSwapRouter) external onlyOwner {
        dosSwapRouter = IDosRouter02(_dosSwapRouter);
        emit NewDosSwapRouter(msg.sender, _dosSwapRouter);
    }
     ...
     * @dev Callable by owner
     */
    function setCakeBurnRate(uint256 _cakeBurnRate) external onlyOwner {
    ...
    function getPairAddress(
        ...
        IDosFactory pcsFactory = IDosFactory(factory);
