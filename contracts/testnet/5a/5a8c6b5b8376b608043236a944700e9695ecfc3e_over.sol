  ...
  Pausable
{
  ...
  function updateStable_ratio(bool newVal) external onlyOwner {
        stable_ratio = newVal; //update stable decimal
    }
  ...
  function getTokenPrice() public view returns(uint[6] memory)
   ...
   {
   bool stable_ratio = stable_ratio;
   bool main_position = main_position;
    ...
    if (main_position = false){
    	uint res0 = Res0.div(10**this_dec); //integer for token0 
    	uint res1 = Res1.div(10**main_dec); //integer for token1 (main)
    	uint now_val = res0.div(res1); //actual ratio
    	}
    if (stable_ratio = true){
    	uint stable_ratio = getStablePrice();
    	uint now_val = res0.div(res1).div(stable_ratio); //new actual ratio
    }
