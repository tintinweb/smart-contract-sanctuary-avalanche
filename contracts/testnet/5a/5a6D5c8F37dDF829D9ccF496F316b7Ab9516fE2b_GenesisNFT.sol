...
import { Beneficiary } from "./internal/Beneficiary.sol";
...
contract GenesisNFT is ERC721Burnable, ERC721Rentable, ERC721URIStorage, ERC721Permit, IGenesisNFT, Beneficiary, Creator, NonFungibleType, Withdrawable, ReentrancyGuard {
    ...
    constructor(string memory name_, string memory symbol_) payable ERC721Rentable(name_, symbol_) Signable(type(GenesisNFT).name, "1") {
        address sender = _msgSender();
        _setBaseURI("https://dev-eatsmile-api.w3w.app/nfts/0x588ec3d5b42b9d7b4b69d4f8040283b36440036d/");
        _setBeneficiary(sender);
        _setCreator(sender);
    ...
    function buyNFT(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
        address beneficiary = _beneficiary.toAddress();
        ...
        if (typeInfo.paymentToken == address(0)) {
            ...
            __transfer(address(0), beneficiary, total);
        } else {
            __transferFrom(typeInfo.paymentToken, account, beneficiary, total);
        }
    ...
    function batchMint(
        ...
        _batchMint(account_, typeNFT_, quantity_);
    }
    ...
    {
        return (_typeInfo[typeNFT].paymentToken, _typeInfo[typeNFT].price, _typeInfo[typeNFT].limit, _typeInfo[typeNFT].unboxQuantity);
    }
...
abstract contract ERC721Burnable is IERC721Burnable, Context, ERC721 {
    /**
