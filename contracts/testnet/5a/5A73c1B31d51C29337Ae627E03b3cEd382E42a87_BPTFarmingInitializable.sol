...
// solhint-disable not-rely-on-time, max-states-count, var-name-mixedcase
...
contract BPTFarmingInitializable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint256) public accRewardBPT;
    ...
    event AddScoreBPT(address indexed user, uint256 amount);
    event SubScoreBPT(address indexed user, uint256 amount);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (userBPT.score > 0 && user.amount > 0) {
            ...
            if (pendingBPT > 0) {
                rewardToken.safeTransfer(address(msg.sender), pendingBPT + accRewardBPT[msg.sender]);
            }
            accRewardBPT[msg.sender] = 0;
        }
    ...
    function addScores(address[] memory target, uint256[] memory value) external {
        ...
        for (uint256 i = 0; i < target.length; i++) {
            UserInfoBPT storage userBPT = userInfoBPT[target[i]];

            if (userBPT.score > 0) {
                accRewardBPT[target[i]] = accRewardBPT[target[i]] + (userBPT.score * accTokenPerShareBPT) / PRECISION_FACTOR - userBPT.rewardDebtBPT;
            }

            userBPT.score += value[i];
    ...
    function pendingRewardBPT(address _user) public view returns (uint256) {
        return(pendingRewardBPTClear(_user) + accRewardBPT[_user]);
    }

    function pendingRewardBPTClear(address _user) public view returns (uint256) {
