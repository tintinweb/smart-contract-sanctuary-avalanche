...
import "./Whitelight.sol";

contract FeeManager is Context, Ownable {
  using SafeMath for uint256;
  using Address for address;

  uint256 public feeRoyalty = 100;
  uint256 public feeLiquidity = 100;
  uint256 public feeStakingPool = 100;
  uint256 public feeBurn = 100;
  uint256 public feeMarketing = 100;
  uint256 public feeDev = 100;

  uint256 public feeTotal;

  // 600

  address payable public royaltyAddress = payable(0xEF0c0cCC6F249352B32290E47023E992f7E7592C);
  address payable public marketingAddress = payable(0x87B511ABA989dd206Af0cF6510b95a11364cD25D);
  address payable public devAddress = payable(0x8d2c6d61d006A32916EE8172Ae493d9681D11670);
  address payable public stakingPoolAddress = payable(0x112d29683867dB060597C0bA425C8E42c1cbfdFc);
  address payable public lpTokensHolderAddress = payable(0x3D2d8991370e34dd34b69B4CA09981d69146dFC1);

  WHITELIGHT public immutable whitelightToken;
  IPangolinRouter public router;

  constructor(WHITELIGHT _whitelightToken, IPangolinRouter _router) {
    whitelightToken = _whitelightToken;
    router = _router;
    updateTotalFee();
  }

  function updateTotalFee() private {
    feeTotal = feeRoyalty.add(feeLiquidity).add(feeMarketing).add(feeStakingPool).add(feeDev).add(feeBurn);
  }
  ...
  function setFeeLiquidity(uint256 fee) external onlyOwner {
    feeLiquidity = fee;
    updateTotalFee();
  }

  function setRoyaltyAddress(address payable newAddress) external onlyOwner {
  ...
  function setDevAddress(address payable newAddress) external onlyOwner {
    devAddress = newAddress;
  }

  function setStakingPoolAddress(address payable newAddress) external onlyOwner {
    stakingPoolAddress = newAddress;
  }

  function setLpTokensHolderAddress(address payable newAddress) external onlyOwner {
    lpTokensHolderAddress = newAddress;
  }

  function setRouterAddress(IPangolinRouter newAddress) external onlyOwner {
    router = newAddress;
  }

  receive() external payable {}

  function getSwapPercentToNative() public view returns (uint256) {
    return feeMarketing.add(feeDev).add(feeLiquidity.div(2));
  }

  function distributeBurnFee(uint256 toSpend) private {
    if (feeBurn > 0) {
      uint256 burnAmount = toSpend.mul(feeBurn).div(feeTotal);
      whitelightToken.burn(burnAmount);
    }
  }

  function distributeRoyaltyFee(uint256 toSpend) private {
    if (feeRoyalty > 0 && royaltyAddress != address(0)) {
      uint256 royaltyAmount = toSpend.mul(feeRoyalty).div(feeTotal);
      whitelightToken.transfer(royaltyAddress, royaltyAmount);
    }
  }

  function distributeStakingPoolFee(uint256 toSpend) private {
    if (feeStakingPool > 0 && stakingPoolAddress != address(0)) {
      uint256 stakingPoolAmount = toSpend.mul(feeStakingPool).div(feeTotal);
      whitelightToken.transfer(stakingPoolAddress, stakingPoolAmount);
    }
  }

  function distributeDevFee(uint256 swapPercentToNative, uint256 nativeToSpend) private {
    if (feeDev > 0 && devAddress != address(0)) {
      uint256 devAmount = nativeToSpend.mul(feeDev).div(swapPercentToNative);
      devAddress.transfer(devAmount);
    }
  }

  function distributeMarketingFee(uint256 swapPercentToNative, uint256 nativeToSpend) private {
    if (feeMarketing > 0 && marketingAddress != address(0)) {
      uint256 marketingAmount = nativeToSpend.mul(feeMarketing).div(swapPercentToNative);
      marketingAddress.transfer(marketingAmount);
    }
  }

  function distributeLiquidityFee(
    uint256 swapPercentToNative,
    uint256 nativeToSpend,
    uint256 toSpend
  ) private {
    if (feeLiquidity > 0 && lpTokensHolderAddress != address(0)) {
      uint256 amountInNative = nativeToSpend.mul(feeLiquidity.div(2)).div(swapPercentToNative);
      uint256 amountInToken = toSpend.mul(feeLiquidity.div(2)).div(feeTotal);

      whitelightToken.approve(address(router), amountInToken);

      router.addLiquidityAVAX{value: amountInNative}(
        address(whitelightToken),
        amountInToken,
        0, // slippage is unavoidable
        0, // slippage is unavoidable
        lpTokensHolderAddress,
        block.timestamp
      );
    }
  }

  function disburseSwapAndLiquifyTokens(uint256 toSpend) public onlyOwner {
    require(toSpend <= whitelightToken.balanceOf(address(this)), "Amount is greater than contract token balance");

    distributeBurnFee(toSpend);
    distributeRoyaltyFee(toSpend);
    distributeStakingPoolFee(toSpend);

    uint256 swapPercentToNative = getSwapPercentToNative();
    uint256 swapTokensToNative = toSpend.mul(swapPercentToNative).div(feeTotal);

    // uint256 initialNativeBalance = address(this).balance;
    swapToNative(swapTokensToNative);
    uint256 boughtNativeAmount = address(this).balance;

    distributeDevFee(swapPercentToNative, boughtNativeAmount);
    distributeMarketingFee(swapPercentToNative, boughtNativeAmount);
    distributeLiquidityFee(swapPercentToNative, boughtNativeAmount, toSpend);
  }

  function swapToNative(uint256 tokenAmount) private {
    address[] memory path = new address[](2);
    path[0] = address(whitelightToken);
    path[1] = router.WAVAX();

    whitelightToken.approve(address(router), tokenAmount);
  ...
  function recoverTokens(
    ...
    require(balance >= amountToRecover, "Not enough tokens");

    token.transfer(recipient, amountToRecover);
  }

  function recoverNative(address payable recipient, uint256 amountToRecover) external onlyOwner {
    require(address(this).balance >= amountToRecover, "Not enough coins");
...
contract WHITELIGHT is Context, IERC20, LockOwnable {
  ...
  function recoverAVAX(
    address payable recipient,
    uint256 amountToRecover,
    uint256 recoverFeePercentage
  ) external onlyOwner {
    require(
      address(this).balance >= amountToRecover,
      "WHITE Liquidity Provider: Not Enough AVAX in contract to recover"
    );
...
contract Presale {
  ...
  uint256 public totalBought; // total bought Avax in wei
  ...
  function calculateAvaxToPresaleToken(address _address, uint256 _amount) public view returns (uint256) {
    ...
    string memory addressBand = allocatedBand[_address];
    if (bytes(addressBand).length == 0) {
      addressBand = DEFAULT_BAND;
    }

    uint256 tokensWithBand = tokens + (tokens * bandsPercentages[addressBand]) / FEE_DENOMINATOR;
  ...
  function getTotalClaimPercentage() private view returns (uint256) {
    ...
    if (startDateClaim > _currentClaimDate) return 0;
    ...
    if (claimIntervalDay > startClaimDay) {
      interval += 1;
    }
    if (day > claimIntervalDay || (day == claimIntervalDay && hour >= claimIntervalHour)) {
      interval += 1;
    }
  ...
  function buy() public payable {
    ...
    require(
      bytes(allocatedBand[msg.sender]).length > 0 || isMainSalePhase,
      "User does not belong to any band (not whitelisted)"
    );
    require(bought[msg.sender] + msg.value <= maxPurchase, "Cannot buy more than max purchase amount");
    require(msg.value >= minBuyAvax, "Value is less than minBuyAvax");
    require(msg.value <= maxBuyAvax, "Value is great than maxBuyAvax");
