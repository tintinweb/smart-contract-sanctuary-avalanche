...
error VapeToken__AlreadyMigrate();
error VapeToken__NoBalance();

// VapeToken with Governance

/// @title VapeTokenV2
/// @notice Main VAPE token contract
contract VapeTokenV2 is LERC20, Ownable {
    /// STORAGE ///
    ///////////////
    uint224 public constant MAX_SUPPLY = 21_000_000 * 1e18;

    // Migration from old VapeToken
    address public oldVapeToken;
    ...
    function migrate() external {
        if (isMigrated[_msgSender()]) {
            revert VapeToken__AlreadyMigrate();
        }
        uint256 balance = IERC20(oldVapeToken).balanceOf(_msgSender());
        if (balance == 0) {
            revert VapeToken__NoBalance();
        }
        isMigrated[_msgSender()] = true;
...
contract LERC20 is ILERC20, ERC20Votes {
