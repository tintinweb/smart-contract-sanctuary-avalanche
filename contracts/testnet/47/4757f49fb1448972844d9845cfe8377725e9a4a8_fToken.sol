...
contract fToken is Initializable
  ...
  function depositToken(
    ...
    // mint and emit event
    token20.mint(msg.sender, toMint * 1 ether);
    emit Transfer(address(0), msg.sender, toMint * 1 ether);
    // burn all tokens on contract
    IERC20Burn(Token).burn(finish);
    return true;
  }

  function depositWToken(
    uint256 amount
  ) external
    virtual
    returns (bool) {
    // Find balance of prior to
    uint256 start = IERC20(WToken).balanceOf(address(this));
    // The transfer
    IERC20(WToken).safeTransferFrom(msg.sender, address(this), amount);
    // Balance of post transfer
    uint256 finish = IERC20(WToken).balanceOf(address(this));
    // The delta (aka to mint)
    uint256 toMint = finish - start;
    // mint and emit event
    token20.mint(msg.sender, toMint);
    emit Transfer(address(0), msg.sender, toMint);
    // now to unwrap all wrapped tokens to tokens
    bool check = IERC20Burn(WToken).withdraw(finish);
    if (check) {
      // burn all tokens on contract
      uint256 toBurn = IERC20(Token).balanceOf(address(this));
      IERC20Burn(Token).burn(toBurn);
    }
    return true;
  }
