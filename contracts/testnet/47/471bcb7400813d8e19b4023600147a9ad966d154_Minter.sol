...
contract Minter is Ownable {
     ...
     */
    function tokenPriceInUSD(address _token, address _lp, uint256 _amount) public view returns ( uint256 _wavaxValue,  uint256 _tokenValue , uint256  _tokenValueInWAVAX, uint256 _tokenValueInUSD ) { 
          _wavaxValue = IERC20(Wavax).balanceOf(_lp).div(10** IERC20(Wavax).decimals()).mul( 10** IERC20(Waceo).decimals());
         _tokenValue = IERC20(_token).balanceOf(_lp).div(10** IERC20(_lp).decimals()).mul( 10** IERC20(Waceo).decimals()); 
