...
contract OmnichainRouter is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable {
    ...
    */
    ...
    function isDirectRoute(string memory dstChainName) public view returns (bool) {
        return chainNameToRoute[dstChainName].length > 0;
    }
    ...
    function send(string memory dstChainName, address dstUA, bytes memory fnData, uint gas, address user, uint256 redirectFee) external override payable {
        ...
        RouteCall memory params = RouteCall(dstChainName, _payload, gas, user, address(0), 0);
        _route(params);
    }
     ...
     */
    function _route(RouteCall memory params) internal {
        Route[] storage routes = chainNameToRoute[params.dstChainName];
        ...
        if (keccak256(bytes(route.provider)) == keccak256(bytes('lz'))) {
            _lzProcess(route.chainId, params, isRedirect);
            return;
        }
        _axProcess(route.dstChain, params, isRedirect);
    }
     ...
     */
    function _lzProcess(uint16 chainId, RouteCall memory params, bool isRedirect) internal {
        bytes memory adapter = _getAdapter(isRedirect ? 0 : params.gas);

        if (params.redirectFee > 0) {
            (uint messageFee,) = lzEndpoint.estimateFees(chainId, address(this), params.payload, false, adapter);
            _validateAndChargeUA(params.srcUA, messageFee);
        ...
        lzEndpoint.send{value : msg.value}(chainId, this.getTrustedRemote(chainId), params.payload, payable(params.user), address(0x0), adapter);
    }
     ...
     */
    function _axProcess(string memory dstChain, RouteCall memory params, bool isRedirect) internal {
        ...
        if (params.redirectFee > 0) {
            ...
            axGasReceiver.payNativeGasForContractCall{value : msg.value}(address(this), dstChain, dstStringAddress, params.payload, params.user);
        }
