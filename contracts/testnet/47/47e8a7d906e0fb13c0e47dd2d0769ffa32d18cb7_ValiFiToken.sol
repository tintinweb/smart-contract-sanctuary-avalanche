...
contract ValiFiToken is Initializable, StabilityFee {
    ...
    function initialize() public initializer {
        ...
        vestingTypes.push(VestingType(50000000000000000000, 50000000000000000000, 30 days, 0, true)); // Unlock 50% in the TGE and another 50% after 1 month
        // Allocation #2 / VestingType # 1, IDO OR Listing Public , (4%) Unlocked the all Token immediatly deploy the Liquidity Pool of Token
        vestingTypes.push(VestingType(0, 100000000000000000000, 0, 0, true)); // 0 Days 100 Percent Unloked
        // Allocation #3 / VestingType # 2, REWARDS of Project (70%), Unlock (4,16)% in the TGE and another (3.91304347826087)% for 23 months
        vestingTypes.push(VestingType(39130434782608700, 10000000000000000000, 30 days, 0, true)); // Unlock (3.91304347826087)% in the TGE and another (3.91304347826087)% for 23 months
        // Allocation #4 / VestingType # 3, Liquidity (10%), Unlock (10)% after 60 months
        vestingTypes.push(VestingType(100000000000000000000, 0, 1800 days, 0, true)); // Unlock (10)% after of 60 months
    ...
    function circulatingSupply() public view returns (uint256 result) {
        ...
        for (uint256 i = 0; i < valifi_wallets.length; i++) {
            if ((valifi_wallets[i] != address(0)) && (result != 0)) {
                result -= balanceOf(valifi_wallets[i]);
            }
        }
...
contract StabilityFee is Vesting {
     ...
     * @return bool
     */
    function isWinner(address _wallet) public view returns (bool) {
        return walletWinners[_wallet] != 0;
    }
     ...
     * @return bool
     */
    function addWinner(address _wallet, uint256 _amount) public returns (bool) {
        if (!isTrader(_wallet)) {
            walletTraded[_wallet] = getTimestamp();
        }
        walletWinners[_wallet] += _amount;
        return true;
    }

    function getRestAmountOfFee(address _wallet, uint256 _amount)
        public
        returns (uint256)
    {
        ...
        if (timeWallet == 0) {
            ...
            transfer(valifi_wallets[1], mulDiv(_amount, 65e16, 1e18));
            // Burn 15% amount of CAN Token to Tranfer to the Wallet
            _burn(msg.sender, mulDiv(_amount, 15e16, 1e18));
            return mulDiv(_amount, 20e16, 1e18);
            ...
            transfer(valifi_wallets[1], mulDiv(_amount, 5e16, 1e18));
            // Burn 5% amount of CAN Token to Tranfer to the Wallet
            _burn(msg.sender, mulDiv(_amount, 5e16, 1e18));
            return mulDiv(_amount, 90e16, 1e18);
        }
    }
}
    ...
    ERC20PermitUpgradeable
{
    ...
    function getReleaseTime() public pure returns (uint256) {
        return 1650542400; // "Thursday, 21 April 2022 12:00:00 GMT"
    }
...
contract CirculatingSupply is OwnableUpgradeable, Antibots {
    ...
    function isValiFiWallet(address _account) public view returns (bool) {
        ...
        uint256 index = valifi_wallets.length;
        for (uint256 i = 0; i < index; i++) {
            if (_account == valifi_wallets[i]) {
                return true;
            }
        }
        return false;
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < index; i++) {
            if (_account == valifi_wallets[i]) {
                valifi_wallets[i] = valifi_wallets[index - 1];
                valifi_wallets.pop();
    ...
    function getValiFiWallets() public view returns (address[] memory) {
        return valifi_wallets;
    }

	/**
     * @dev Getting the all wallets address of VALIFi Foundation Wallets
     */
    function getValiFiWallet(uint256 _index) public view returns (address) {
        return valifi_wallets[_index];
    }
}
