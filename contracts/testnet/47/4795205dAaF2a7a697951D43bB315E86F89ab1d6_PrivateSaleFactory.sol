...
contract PrivateSaleFactory is Ownable {
    ...
    event UserSeizedForSale(string indexed name, address[] users);
    event EmergencyWithdrawnSale(string indexed name);
    ...
    constructor(address _receiverAddress, address _implementation) {
        ...
        // Safety check
        IPrivateSale(implementation).initialize("Implementation", 1e4, 1, 1);
    ...
    function createPrivateSale(
        ...
        IPrivateSale(privateSale).initialize(
            name,
            price,
            maxSupply,
            minTokenAmount
        ...
        emit SaleCreated(privateSale, name, maxSupply, minTokenAmount);
    ...
    function seizeUsers(string calldata name, address[] calldata addresses)
        ...
        onlyOwner
    {
        getPrivateSale[name].seizeUsers(addresses);
        emit UserSeizedForSale(name, addresses);
    }
    ...
    function setImplementation(address _implementation) external onlyOwner {
        ...
        implementation = _implementation;
        // Safety check
        IPrivateSale(implementation).initialize("initialized", 1e4, 1, 1);
        emit ImplementationSet(_implementation);
    }
    ...
    function emergencyWithdraw(string calldata name) external onlyOwner {
        ...
        emit EmergencyWithdrawnSale(name);
    }
}
