...
contract BabyWhales is ERC721URIStorage, Ownable, KindaRandom {
  ...
  constructor(address signer_) ERC721("Baby Whales", "BBW") KindaRandom(333) {
    _customBaseURI = 'https://not-a.real.website/';
    _unrevealedBaseURI = 'https://nfts.bpi.network/bb_whales/';
    _maxMintable = 333; // 333 max supply
    _royaltyAmount = 690; // 6.9% royalties
    _mintActive = false; // start paused
    _onlyWhitelist = true; // start with just WL
    _maxMintPerTx = 1;
    _signer = signer_;
    _revealed = false;
  }
  ...
  function setUnrevealedBaseURI(string memory customBaseURI_) public onlyOwner {
    _customBaseURI = customBaseURI_;
  }
  ...
  function setOnlyWhitelist(bool status) public onlyOwner {
  ...
  function signatureMint(uint256 quantity, bytes32 _salt, uint256 maxGas, bytes memory _signature) public payable {

    bytes32 hash = keccak256(abi.encodePacked(msg.sender, address(this), maxGas, _salt));
    address signer = hash.toEthSignedMessageHash().recover(_signature);
    require(signer == _signer, "Signature verification failed.");

    require(!_used[_salt], "Salt has already been used.");
    ...
    require(tx.gasprice <= (maxGas * (10 ** 9)), "Gas price too high.");

    payable(owner()).transfer(msg.value); // shouldn't be money here, but just in case

    for(uint i = 0; i < quantity; i++) {
      _privateMint(msg.sender, _salt);
    }
  ...
  function _privateMint(address recipient, bytes32 salt) private {
    uint256 randomish = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, salt))) % (_maxMintable - _tokenIds.current());
