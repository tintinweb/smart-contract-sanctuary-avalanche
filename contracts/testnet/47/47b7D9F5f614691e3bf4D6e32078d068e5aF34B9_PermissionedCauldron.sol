...
/// @dev This contract allows contract calls to any contract (except BentoBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract NereusStableCoin is ERC20, BoringOwnable {
    using BoringMath for uint256;
    // ERC20 'variables'
    string public constant symbol = "NUSD";
    string public constant name = "Nereus Stable Coin";
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(to != address(0), "NUSD: no mint to zero address");
    ...
    function burn(uint256 amount) public {
        require(amount <= balanceOf[msg.sender], "NUSD: not enough");
...
contract WXTOracle is IOracle {
    ...
    function _get() internal view returns (uint256) {
        return 1e26 / uint256(wxtOracle.latestAnswer());
    }
...
contract PermissionedCauldron is CauldronV2 {
    ...
    /// @notice The constructor is only used for the initial master contract. Subsequent clones are initialised via `init`.
    constructor(IBentoBoxV1 bentoBox_, IERC20 nxusd_, PermissionManager permissionManager) CauldronV2(bentoBox_, nxusd_, permissionManager) public {
    }
