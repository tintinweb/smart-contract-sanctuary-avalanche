...
contract TokenStakePoolOld is ReentrancyGuard, Ownable, Pausable {
  ...
  */
  modifier whenNotLocked(address _user) {
    ...
    if (unlockTime != 0) {
      require(
        users[_user].unlockTime != 0,
        "Activate unlock for withdrawal and redemption window first"
      );
      require(
        block.timestamp > users[_user].unlockTime,
        "Unlock time still in progress"
      );
      require(
        block.timestamp < users[_user].relockTime,
  ...
  */
  modifier updatePool() {
    if (totalStaked > 0) {
        rewardsPerStakedToken = block.timestamp.sub(lastUpdatedAt)
  ...
  */
  function balanceOf(address _account) external view returns (uint256) {
    return users[_account].stakedAmount;
  }
  ...
  */
  function currentRewardsPerStakedToken() private view returns (uint256) {
    if (totalStaked <= 0) {
      return rewardsDistributionRate;
    }

    return block.timestamp.sub(lastUpdatedAt)
                          .mul(rewardsDistributionRate)
                          .mul(SAFE_MULTIPLIER)
                          .div(totalStaked)
                          .add(rewardsPerStakedToken);
  ...
  */
  function rewardsEarned(address _account) public view returns (uint256) {
    return users[_account].stakedAmount
                          ...
                          .sub(users[_account].rewardsDebt)
                          .add(users[_account].rewardsAccrued)
                          .div(SAFE_MULTIPLIER);
  }
  ...
  */
  function _updateUserRewardsDebt(address _account, uint256 _amount) private {
    users[_account].rewardsDebt = users[_account].rewardsDebt
  ...
  */
  function _updateUserRewardsAccrued(address _account) private {
    if (_account != address(0)) {
      users[_account].rewardsAccrued = users[_account].rewardsAccrued
                                              .add(users[_account].stakedAmount
                                              .mul(rewardsPerStakedToken)
                                              .sub(users[_account].rewardsDebt));
      users[_account].rewardsDebt = 0;
    }
  }
  ...
  */
    ...
    updatePool
  {
    ...
    _updateUserRewardsDebt(msg.sender, users[msg.sender].stakedAmount);
  ...
  */
  function claim() public nonReentrant whenNotPaused whenNotLocked(msg.sender) {
    uint256 rewards = rewardsEarned(msg.sender);

    if (rewards > 0) {
      users[msg.sender].rewardsAccrued = 0;
      users[msg.sender].rewardsRedeemed = users[msg.sender].rewardsRedeemed.add(rewards);

      // Reset user's rewards debt, similar to as if user has just withdrawn and restake all
      users[msg.sender].rewardsDebt = users[msg.sender].stakedAmount
                                              .mul(currentRewardsPerStakedToken());
  ...
  */
  function activateUnlockTime() external whenNotPaused {
    require(
      users[msg.sender].unlockTime < block.timestamp,
      "Unlock time still in progress"
    );

    users[msg.sender].unlockTime = block.timestamp + unlockTime * 1 minutes;
    users[msg.sender].relockTime = block.timestamp + (unlockTime + relockTime) * 1 minutes;
  ...
  * @param _rate      Rate of rewards token distribution per second
  */
