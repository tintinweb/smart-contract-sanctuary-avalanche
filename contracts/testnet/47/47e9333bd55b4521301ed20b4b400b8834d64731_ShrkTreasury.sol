...
contract ShrkTreasury is Ownable {
    ...
    address public sShrk;
    uint public sSHRKQueue; // Delays change to sSHRK address
    ...
    //    isLiquidityToken[ _SHRKDAI ] = true;
    //    liquidityTokens.push( _SHRKDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SHRK
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SHRK needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Shrk ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SHRK for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISHRK( Shrk ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sShrk ).balanceOf( msg.sender ); // Can only borrow against sSHRK held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SHRK
        @param _amount uint
     */
    function repayDebtWithSHRK( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISHRK( Shrk ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Shrk, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Shrk ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Shrk ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SHRK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SHRK decimals
            value_ = _amount.mul( 10 ** IERC20( Shrk ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSHRK ) { // 9
            sSHRKQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
