...
contract TrantorV3 is ERC20Upgradeable {
   ...
   /**
    ...
    function createPairv2(address add1) public onlyOwner() {
        lpPair = IFactoryV2(dexRouter.factory()).createPair(dexRouter.WAVAX(), add1);
        lpPairs[lpPair] = true;
    }
    ...
    */
    ...
    function withdrawStuckTokens(uint256 amount) public onlyOwner {
        require(balanceOf(contractAddress) > 0, "Contract balance is zero");
        if (amount > balanceOf(contractAddress)) {
            amount = balanceOf(contractAddress);
        }
