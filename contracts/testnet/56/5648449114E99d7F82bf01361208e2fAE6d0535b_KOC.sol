...
import "./TournamentNative.sol";
import "./TournamentERC20.sol";
import "@openzeppelin/contracts/token/ERC20/extensions/ERC20Burnable.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract KOC {
    event TournamentAddress(address tournamentAddress);
    function createTournamentERC20(address _token, uint256 _claimPeriod, uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _gladShare, uint256 _burnShare, bool _nonBurnable, bool _deadress, address _treasury) external{
        TournamentERC20 tournamentERC20 = new TournamentERC20(_token, _claimPeriod, _periodSeconds, _roundSeconds, _breakevenSeconds, _ticketSize, _potShare, _gladShare, _burnShare, _nonBurnable, _deadress, _treasury);
        emit TournamentAddress(address(tournamentERC20));
    }
    // function createTournamentNative() external{
    //     TournamentNative tournamentNative = new TournamentNative();
    // }
}
...
contract TournamentNative is Ownable, ReentrancyGuard, ITournament {
    ...
    constructor (uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _gladShare, uint256 _burnShare, bool _nonBurnable, bool _deadress, address _treasury) {
...
contract TournamentERC20 is Ownable, ReentrancyGuard, ITournament {
    address public winner;
    bool public claimed;
    mapping(address => bool) public pgClaimed;
    uint256 public lastTs;
    bool public nonBurnable;
    ...
    constructor (address _token, uint256 _claimPeriod, uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _gladShare, uint256 _burnShare, bool _nonBurnable, bool _deadress, address _treasury) {
    ...
    function buy() external payable nonReentrant {
        ...
        if (BURN_SHARE >  0) {
            if (nonBurnable) {
                rewardToken.transfer(burnAddress, transferSize * BURN_SHARE / 10000);
            } else {
