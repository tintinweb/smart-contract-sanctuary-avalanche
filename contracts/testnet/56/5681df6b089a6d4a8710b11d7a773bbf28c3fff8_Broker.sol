...
contract Broker {
    ...
    function findBookingsByClient(address owner) public view returns (Booking[] memory filteredBookings) {
        Booking[] memory bookingsTemp = new Booking[](nextBookingId);
        uint256 count;
        for (uint256 i = 0; i < nextBookingId; i++) {
            if (bookings[i].client == owner && bookings[i].bookedTill > block.timestamp) {
    ...
    function findBookingsForMiner(address miner) public view returns (Booking[] memory filteredBookings) {
        Booking[] memory bookingsTemp = new Booking[](nextBookingId);
        uint256 count;
        for (uint256 i = 0; i < nextBookingId; i++) {
            if (bookings[i].miner == miner && bookings[i].bookedTill > block.timestamp) {
                bookingsTemp[count] = bookings[i];
                count += 1;
            }
        }
