...
contract ExampleERC6551Account is IERC165, IERC1271, IERC6551Account {
    ...
    function balanceOfERC20(IERC20 token) external view returns (uint256) {
        return token.balanceOf(address(this));
    }

    function transferERC721(IERC721 token, address to, uint256 tokenId) external {
        require(msg.sender == owner(), "Not token owner");
        token.transferFrom(address(this), to, tokenId);
    }

    function approveERC721(IERC721 token, address to, uint256 tokenId) external {
        require(msg.sender == owner(), "Not token owner");
