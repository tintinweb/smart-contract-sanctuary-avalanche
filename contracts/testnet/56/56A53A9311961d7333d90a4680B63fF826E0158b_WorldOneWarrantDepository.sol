...
contract WorldOneWarrantDepository is Ownable {
    ...
    function EditWarrantLot(uint256 _warrantLotIndex, uint256 _price)
        external
        onlyOwner
    {
     ...
     *  @return uint
     */
    function valueCheckOf( uint _amount ) external view returns ( uint ) {
    ...
    function getLatestPrice() public view returns ( uint price_ ) {
        ...
        if (circulatingSupply == 0) { // On first warrant sale, there will be no circulating supply
            ...
            if(warrantLotIndex > 0){
                ...
                price_ = treasuryBalance.div(circulatingSupply).mul(1e4).div(1e11);
            }
        }
    }
