    ...
    ValidationNodesProviderFactoryC2Upgradable
{
    ...
    // 32 ETH on each ETH validation node (25k AVAX on each AVAX validation node)
    uint256 public constant STAKED_BY_NODE = 32 * 10**18;
        ...
        onlyExistingPool(poolId)
    {
        ...
        (, int256 nodeTotalStaked, , , ) = IAggregatorV3(
            stakingPools[poolId].aggregator
        ).latestRoundData();
        uint256 availableToStake = uint256(nodeTotalStaked) -
        ...
        onlyExistingPool(poolId)
    {
        ...
        uint256 amountOwnedPerToken = stakingPool.cumulativeRewardPerStake -
            stakingPool.accountCumulativeRewardPerStake[_msgSender()];
        uint256 claimableAmount = (stakingPool.stakes[_msgSender()] *
            amountOwnedPerToken) / PRECISION;

        payable(_msgSender()).transfer(
            claimableAmount + stakingPool.stakes[_msgSender()]
        );

        stakingPool.stakes[_msgSender()] = 0;
        stakingPool.earned[_msgSender()] = claimableAmount;

        emit Claimed(poolId, _msgSender(), claimableAmount);
    }
    ...
    function stakedByUserPerPool(uint256 poolId, address user)
        public
        view
        returns (uint256)
    {
    ...
    function totalStakedPerPool(uint256 poolId) public view returns (uint256) {
        return stakingPools[poolId].totalStaked;
    }
    ...
    OwnableUpgradeable
{
    ...
    function __ValidationNodesProviderFactoryC2_init(address initialBeacon)
        internal
        initializer
    {
        __Context_init_unchained();
