...
contract GenesisRewardPool is ContractGuard, OwnableUpgradeable {
    ...
    uint256 public constant TOTAL_REWARD_POOL_2_NEXT_PHASE = 22312.5 ether; // 70% of (85% of 37500)
    uint256 public constant TOTAL_REWARD_POOL_1_NEXT_PHASE = 4781.25 ether; // 15% of (85% of 37500)
    uint256 public constant TOTAL_REWARD_POOL_0_NEXT_PHASE = 4781.25 ether; // 15% of (85% of 37500)

    // MAINNET
    uint256 public constant runningTime = 72 hours;
    uint256 public constant TOTAL_REWARDS = 45000 ether;

    uint256 public constant TOTAL_USER_REWARDS = 38250 ether; // (85% of 45000)
    uint256 public constant mainTokenPerSecond = 0.1475694 ether; // 38250 Token / (72h * 60min * 60s)
    uint256 public constant DAO_FUND_REWARDS = 6750 ether;
    uint256 public constant mainTokenPerSecondForDaoFund = 0.0260416 ether; // 6750 Token / (72h * 60min * 60s)
    uint256 lastDaoFundRewardTime;

    address public airdropWallet;
    uint256 public constant airdropTipPercent = 20;

    IERC20Upgradeable public usdcAddress;
    IERC20Upgradeable public wavaxAddress;
    // END MAINNET
    ...
    function __Upgradeable_Init(
        ...
        IERC20Upgradeable _usdcAddress,
        IERC20Upgradeable _wavaxAddress,
        uint256 _poolStartTime) public initializer {
        __Ownable_init();
        require(block.timestamp < _poolStartTime, "late");
        if (_token != address(0)) mainToken = _token;
        totalAllocPoint = 0;
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pendingUser = user.amount.mul(accMainTokenPerShare).div(1e18).sub(user.rewardDebt);
        return pendingUser.add(daoFundReward);
    }
    ...
    function pendingDaoFund(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (IMainToken(mainToken).isDaoFund(_user)) {
            ...
            if (_toTime >= poolEndTime) {
                if (_fromTime >= poolEndTime) return 0;
                if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(mainTokenPerSecondForDaoFund);
                return poolEndTime.sub(_fromTime).mul(mainTokenPerSecondForDaoFund);
            } else {
                if (_toTime <= poolStartTime) return 0;
                if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(mainTokenPerSecondForDaoFund);
                return _toTime.sub(_fromTime).mul(mainTokenPerSecondForDaoFund);
            }
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public onlyOneBlock {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                uint256 airdropTip = _pending.mul(airdropTipPercent).div(100);
                if (airdropTip > 0 && isUsdcOrWavax(pool.token)) {
                    safeMainTokenTransfer(airdropWallet, airdropTip);
                    emit RewardPaid(airdropWallet, airdropTip);
                    _pending = _pending.sub(airdropTip);
                }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public onlyOneBlock {
        ...
        if (_reward > 0) {
            uint256 airdropTip = _reward.mul(airdropTipPercent).div(100);
            if (airdropTip > 0 && isUsdcOrWavax(pool.token)) {
                safeMainTokenTransfer(airdropWallet, airdropTip);
                emit RewardPaid(airdropWallet, airdropTip);
                _reward = _reward.sub(airdropTip);
            }
            
            if (_reward > 0) {
                safeMainTokenTransfer(_sender, _reward);
    ...
    function safeMainTokenTransfer(address _to, uint256 _amount) internal {
        uint256 _mainTokenBalance = IERC20Upgradeable(mainToken).balanceOf(address(this));
        if (_mainTokenBalance > 0) {
            if (_amount > _mainTokenBalance) {
                IERC20Upgradeable(mainToken).safeTransfer(_to, _mainTokenBalance);
            } else {
                IERC20Upgradeable(mainToken).safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function isUsdcOrWavax (IERC20Upgradeable _token) internal view returns (bool) {
        return _token == usdcAddress || _token == wavaxAddress;
    }
}
