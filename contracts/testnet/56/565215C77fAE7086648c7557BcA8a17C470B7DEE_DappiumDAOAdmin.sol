...
abstract contract SimpleGovernor {
   ...
   */
  function quorum(uint256 _proposalId, uint256 blockNumber)
    public
    view
    virtual
    returns (uint256);
  ...
  function propose(
    ...
    if (_proposalType == uint8(ProposalType.Single)) {
      ...
      require(
        _choices.length > 1,
        'Multiple Choice proposal should have at least 2 choices.'
      );
    } else if (_proposalType == uint8(ProposalType.Checkbox)) {
      require(
        _choices.length > 1,
        'Checkbox proposal should have at least 2 choices.'
      );
      require(
        _votingMechanism == uint8(VotingMechanism.Equal),
        'Checkbox type proposal can only have equal voting.'
      );
    } else {
   ...
   */
  function castVote(uint256 proposalId, uint8[] memory support)
    public
    virtual
    returns (uint256)
  {
  ...
  function _castVote(
    ...
    if (proposal.proposalType == uint8(ProposalType.Checkbox)) {
      ...
      require(
        support.length == 1,
        'There can only be 1 choice for this proposal.'
      );
  ...
  function getProposal(uint256 _proposaId)
    public
    view
  ...
  function getProposalList(uint256 _start, uint256 _end)
    external
    view
    returns (ProposalCore[] memory)
  {
    ...
    require(
      _start < _proposalNumber,
      'Start value must be smaller than the total number of proposals'
    );
  ...
  function getStates(uint256 _start, uint256 _end)
    public
    view
...
abstract contract GovernorCounting is SimpleGovernor {
    ...
    returns (bool)
  {
    ...
    return
      quorum(proposalId, proposalSnapshot(proposalId)) <=
  ...
  function _countCheckbox(
    ...
    require(
      ...
      'GovernorCountingExtended: vote already cast'
    );
    proposalvote.hasVoted[account] = true;

    for (uint256 i = 0; i < support.length; i++) {
      if (support[i] == uint8(VoteType.Against)) {
        // against
        proposalvote.againstVotes += weight;
      } else if (proposalCore.choices.length >= support[i]) {
        // for
  ...
  function _countVoteWeighted(
    ...
    if (proposalCore.proposalType == uint8(ProposalType.Single)) {
      ...
      if (support == uint8(VoteType.Against)) {
        // against
        proposalvote.againstVotes += weight;
      } else if (proposalCore.choices.length >= support) {
        // for
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < proposalCore.choices.length; i++) {
    ...
    returns (uint256[] memory, string[] memory)
  {
    ...
    string[] memory votesDescriptions = new string[](
      proposalCore.choices.length
    );
    for (uint256 i = 0; i < proposalCore.choices.length; i++) {
...
abstract contract GovernorVotes is SimpleGovernor {
   ...
   */
  function quorumNumerator(uint256 _proposalId)
    public
    view
    virtual
    returns (uint256)
  ...
  {
    return
      (token.getPastTotalSupply(blockNumber) * quorumNumerator(_proposalId)) /
      quorumDenominator();
  }
}

contract DappiumDAOAdmin is
  SimpleGovernor,
  GovernorCounting,
  GovernorVotes,
  AccessControl
{
  ...
  constructor(IVotes _token, address[] memory _proposers)
  ...
  function addProposers(address[] memory _proposers)
    public
    virtual
  ...
  function removeProposers(address[] memory _proposers)
    external
    virtual
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
