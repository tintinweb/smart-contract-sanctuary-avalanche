...
contract AvaDiceNFT is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage, IERC2981Royalties {
    ...
    constructor(string memory name, string memory symbol, string memory _baseTokenURI, address _royaltyRecipient) ERC721(name, symbol) {
    ...
    function setBaseURI(string memory baseURI) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "requires admin");
    ...
    function setMintFlag(uint8 flag) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "requires admin");
        mintFlag = flag;
    }

    function setClaimFlag(uint8 flag) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "requires admin");
        claimFlag = flag;
    }
    ...
    function addWinner(address[] calldata winners) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "requires admin");
    ...
    function removeWinner(address[] calldata winners) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "requires admin");
        for(uint i = 0; i < winners.length; i++) {
    ...
    function splitBalance(uint256 amount) private {
        ...
        dev = address(0xe650580Ab0B22C253e13257430b1102dB8C27d1E);
        if(devPay < 200 ether) {
