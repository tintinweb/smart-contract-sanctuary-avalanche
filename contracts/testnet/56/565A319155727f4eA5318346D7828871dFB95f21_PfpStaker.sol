...
contract PfpStaker is IPfpStaker, ReentrancyGuard, Ownable, NFTreceiver {
	...
	function getURI() external view returns (string memory) {
		...
		if (IERC721(stake.NFTcontract).supportsInterface(ERC721_METADATA)) {
			return IERC721Metadata(stake.NFTcontract).tokenURI(stake.tokenId);
		} else {
			if (IERC1155(stake.NFTcontract).supportsInterface(ERC1155_METADATA)) {
				return IERC1155MetadataURI(stake.NFTcontract).uri(stake.tokenId);
			}
		}
	}
	...
	function getStakedNFT() external view returns (address NFTContract, uint256 tokenId) {
		return (stakes[msg.sender].NFTcontract, stakes[msg.sender].tokenId);
	}
