...
contract CATERC20 is Context, ERC20, CATERC20Governance, CATERC20Events {
    ...
    function bridgeIn(bytes memory encodedVm) external returns (bytes memory) {
        ...
        require(bytesToAddress(vm.emitterAddress) == address(this) || tokenContracts(vm.emitterChainId) == vm.emitterAddress, "Invalid Emitter");
...
import "./CATRelayerStructs.sol";
import "../libraries/external/BytesLib.sol";
import "../interfaces/INexaERC20.sol";
import "./CATRelayerGovernance.sol";

import "../cat/ERC20/CATERC20.sol";
/**
 * @title CATRelayer
 */
contract CATRelayer is CATRelayerGovernance {
    ...
    /**
    * @notice CATRelayer deploy token contract
    */
    function deployToken(
        ...
        bytes memory contractCode = type(CATERC20).creationCode;
        //create contract code hash
        bytes32 contractCodeHash = keccak256(contractCode);
        address projectedTokenAddress = computeAddress(salt, contractCodeHash, address(this));
        require(!isContract(projectedTokenAddress) , "already address deployed");
        tokenAddress = address(new CATERC20{ salt: salt }(name, symbol, decimals, totalSupply));
        if (chainIdForMinting == chainId()){
            CATERC20(tokenAddress).mint(owner, totalSupply);
        }
        CATERC20(tokenAddress).initialize(chainId(), address(wormhole()), 1 );
        Ownable(tokenAddress).transferOwnership(owner);
    ...
    */
    function initiateTokensDeployment(
        ...
        gasCollector().transfer(msg.value);
    ...
    * @dev this function is allowed to be called by user to start relaying tokens on all selected chain.
    ...
    function initiateBridgeOut(
        ...
        require(msg.value > 0, "invalid value for gas");
        gasCollector().transfer(msg.value);
        IERC20 nexaErc20 = IERC20(tokenAddress);
        nexaErc20.safeTransferFrom(_msgSender(), address(this), amount);
        if (nexaErc20.allowance(address(this), tokenAddress) < amount) {
            nexaErc20.approve(tokenAddress, amount);
        }

        INexaERC20(tokenAddress).bridgeOut(amount, recipientChain, recipient, nonce);
...
import "./CATRelayerState.sol";
import "./CATRelayerStructs.sol";
import "../libraries/external/BytesLib.sol";
import "../interfaces/IWormhole.sol";

/**
 * @title CATRelayerGetters
 */
contract CATRelayerGetters is CATRelayerState {
    using BytesLib for bytes;
    ...
    function gasCollector() public view returns (address payable){
        return state.gasCollector;
    }
...
import "./CATRelayerSetters.sol";
import "./CATRelayerGetters.sol";
import "./CATRelayerState.sol";

contract CATRelayerGovernance is  ERC1967Upgrade, CATRelayerGetters, CATRelayerSetters, CATRelayerEvents   {
    event ContractUpgraded(address indexed oldContract, address indexed newContract);
    ...
    function updateGasCollector (address payable _gasCollector) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not a admin");
        setGasCollector(_gasCollector);
    }
}
...
import "./CATRelayer.sol";


contract CATRelayerImplementation is CATRelayer {
...
/**
 * @title CATRelayerSetters
 */
contract CATRelayerSetters is CATRelayerState, AccessControl {
    using SafeERC20 for IERC20;
    ...
    function setGasCollector(address payable _gasCollector) internal {
        state.gasCollector = _gasCollector;
    }
...
/**
 * @title CATRelayerState
 */
contract CATRelayerState {
    CATRelayerStorage.State state;
} // end of class

/**
* @title CATRelayerEvents
 */

 contract CATRelayerEvents {
...
/**
 * @title CATRelayerStructs
 */
contract CATRelayerStructs {
