...
import "../InfinityPool.sol";



/**
    @dev if inheriting the old contract is chosen, please remember that the code onchain matches with the repository code. 
 */
contract UpgradeInfinityPoolInherit is InfinityPool{

   //constructor() {}

    uint8 public newParamUpdate;
    ...
    function upgradeReadDailyWithdrawalCap(address tokenAddr) public view returns (uint256)
    {
        return dailyWithdrawalCap[tokenAddr];
    }

    function upgradeViewWoParam() public pure returns(uint256) {
        uint256 testnum = 12345;
        return testnum;
    }

    function upgradeTxnWoParam() public  {
        newParamUpdate = 2;
    }

    function upgradeTxnWithParam(uint8 _newParamUpdate) public  {
        newParamUpdate = _newParamUpdate;
    }
