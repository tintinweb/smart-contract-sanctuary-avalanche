...
contract Skeleton is ERC721Enumerable, Controllable, Pausable, Restricted {
    ...
    event onNFTDelivered(
        address recipient,
        uint256 tokenId,
        bool indexed stake
    );

    uint256 public constant MINT_PRICE = 0.2 ether; // 2 ether for real launch
    ...
    address payable AdminWallet =
        payable(0x9F523A9d191704887Cf667b86d3B6Cd6eBE9D6e9); // TO CHECK
    address payable Multisig =
        payable(0x49208f9eEAD9416446cdE53435C6271A0235dDA4); // TO CHECK
    address payable devAddress =
        payable(0xCCf8234c08Cc1D3bF8067216F7b63eeAcb113756); // TO CHECK
    ...
    IHunter hunter = IHunter(address(0xEaB33F781aDA4ee7E91fD63ad87C5Bb47FFb8a83));
     ...
     * @param stake If you'd like to stake them in MagicForest instantly after mint
     */
        ...
        whenNotPaused
    {
        ...
        emit onNFTsOrdered(_msgSender(), amount, stake);
     ...
     * @param stake If you'd like to stake them in MagicForest instantly after mint
     */
    function mintWithNFT(uint256[] memory tokenIds, bool stake)
        external
        whenNotPaused
        onlyEOA
        noReentrency
    ...
    function storeMintOrder(
        ...
        AttributesPets.Boost memory boostWallet = pets.getBoostWallet(
            _msgSender()
        );
    ...
    function settleOrders(uint256 amount)
        external
        whenNotPaused
        onlyEOA
        noReentrency
    ...
    function core_mint(Order memory order) internal returns(uint256){
        ...
        if (recipient != order.buyer)
            emit Stolen(ordersSettled, order.buyer, recipient);

        emit onNFTDelivered(recipient, ordersSettled, order.stake);
    ...
    function _beforeTokenTransfer(
        ...
        if (_to != address(magicForest) && tokenTraits[tokenId].isRanger) {
            pets.beforeRangerTransfer(_msgSender(), tokenId);
        }
    }
    ...
    function upgradeLevel(uint256 tokenId) external onlyEOA noReentrency {
        ...
        SklRgr memory sklrgr = tokenTraits[tokenId];
        require(sklrgr.level < 5, "already at max level (level 5)");
        uint256 gemPrice;
        if (sklrgr.isRanger) {
            gemPrice = 20000 ether * sklrgr.level;
        } else {
            if (sklrgr.level == 1) {
                gemPrice = 50000 ether;
            } else if (sklrgr.level == 2) {
                gemPrice = 75000 ether;
            } else if (sklrgr.level == 3) {
     ...
     */
    function selectRecipient(Order memory order, uint256 seed)
        internal
        view
        returns (address)
    {
    ...
    function selectTraits(uint256 _seed, SklRgr memory old)
        internal
        view
    ...
    function setconstInAdventurers(uint256 _constInAdventurers)
        external
        onlyController
    {
