...
contract Manager is ReentrancyGuard, Pausable, AccessControl {
  ...
  function equityInfo(address _vault) public view returns (uint256, uint256, uint256) {
    ...
    uint256 tokenAEquityValue = Math.max(0, tokenAAssetValue - tokenADebtValue);
    uint256 tokenBEquityValue = Math.max(0, tokenBAssetValue - tokenBDebtValue);
    uint256 totalEquityValue = Math.max(0, tokenBAssetValue - tokenBDebtValue);
