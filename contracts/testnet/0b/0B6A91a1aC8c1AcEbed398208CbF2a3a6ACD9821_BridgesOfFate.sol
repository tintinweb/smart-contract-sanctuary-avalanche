...
contract BridgesOfFate is Ownable {
    ...
    bytes32[] SelectSuccessSidePlayerId; 
    bytes32[] SelectUnholdPlayerId; 

    event RandomNumberComment(uint256 _randomNumber);
    event Comment1(uint256 _rcomment1);
    event Comment2(bytes32[] _rcomment2);
    event Comment3(uint256[] _rcomment3);
    event Comment4(bytes32 selectedPlayer);
    event Comment6(bytes32 overflow);
    event Comment5(bytes32[] inFlow);
        ...
        if(OverFlowPlayerStatus[playerId]){
            if(_currentUserStage  > 0){
                ...
                if(totalNFTOnTile[_currentUserStage - 1][previousJumpSide] > 0){
                    ...
                    //Make Separeted function.

                    // && currentDay == _member.day) && 

                    for (uint i = 0; i < PlayerIDAtSpecifyStage.length; i++) {
                        if(PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide && 
                        (OverFlowPlayerStatus[PlayerIDAtSpecifyStage[i]] == true &&  _checkSide(PlayerItem[PlayerIDAtSpecifyStage[i]].stage,PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide) )) {
                            
                            SelectSuccessSidePlayerId.push(PlayerIDAtSpecifyStage[i]);
                            // emit Comment1(totalNFTOnTile[_currentUserStage - 1][previousJumpSide]);
                        }

                    }

                    emit Comment2(SelectSuccessSidePlayerId);
                    
                    if(SelectSuccessSidePlayerId.length > 3){
                        for (uint256 j = 0; j < SelectSuccessSidePlayerId.length; j++) {
                            if(PlayerItem[SelectSuccessSidePlayerId[j]].ishold == false){
                                SelectUnholdPlayerId.push(SelectSuccessSidePlayerId[j]);
                                OverFlowPlayerStatus[SelectUnholdPlayerId[j]] =  false;
                            }
                        }
                        emit Comment2(SelectUnholdPlayerId);
                        playerIndex = generateRandomTicketNumbers(SelectUnholdPlayerId.length - 1,SelectUnholdPlayerId.length - 1,3);
                        emit Comment3(playerIndex);
                    }else{
                        for (uint256 q = 0; q < SelectSuccessSidePlayerId.length; q++) {
                            emit Comment6(SelectSuccessSidePlayerId[q]);
                            OverFlowPlayerStatus[SelectSuccessSidePlayerId[q]] =  true;
                        }
                    }
                ...
                // emit Comment2(SelectPlayerId);
                if(SelectSuccessSidePlayerId.length > 3){
                    for (uint256 k = 0; k < playerIndex.length; k++) {
                        if(playerIndex[k] > 0){
                            OverFlowPlayerStatus[SelectUnholdPlayerId[playerIndex[k]]] =  true;
                            emit Comment4(SelectUnholdPlayerId[playerIndex[k]]);
                            emit Comment1(_currentUserStage);
                        }
                    }
                }
            }
