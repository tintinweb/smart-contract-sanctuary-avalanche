...
contract Crypto528Marketplace is ReentrancyGuard, Ownable {
    ...
    uint256[] public levelUSDCPrices = [50000000, 250000000, 500000000, 2500000000, 5000000000, 10000000000, 20000000000, 50000000000];
    uint256[] public levelUSDTPrices = [50 ether, 250 ether, 500 ether, 2500 ether, 5000 ether, 10000 ether, 20000 ether, 500000 ether];
    ...
    function mint(
        ...
        } else if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("USDC")))) {
            price[_tokenId] = levelUSDCPrices[_level];
        ...
        } else if  (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("USDT")))) {
            price[_tokenId] = levelUSDTPrices[_level];
            require (IERC20(tokenAddressMap[paymentType]).balanceOf(msg.sender) >= price[_tokenId], "token balance should be greater than the buyAmount");
            require (IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, adminRecoveryAddress, price[_tokenId]));
        }
