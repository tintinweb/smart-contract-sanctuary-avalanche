...
contract OmnichainRouter is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable {
    ...
    constructor(address _lzEndpoint, address _axGateway, address _axGasReceiver) NonblockingLzApp(_lzEndpoint) IAxelarExecutable(address(0))  {
        ...
        _owner = msg.sender;
        chainName = "Avalanche";
        // https://docs.axelar.dev/dev/chain-names
    ...
    function send(string memory dstChainName, address dstUA, bytes memory _fnData, uint gas, address origin, uint256 redirectFee) external override payable {
        bytes memory _payload = abi.encode(dstChainName, dstUA, _fnData, gas, msg.sender, chainName, origin, redirectFee);
        RouteCall memory params = RouteCall(dstChainName, _payload, gas, origin, address(0), 0);
        call(params);
    }

    function fundUA(address ua) external payable {
        uaRedirectBudget[ua] += msg.value;
    }

    function call(RouteCall memory params) internal {
        ...
        if (keccak256(bytes(route.provider)) == keccak256(bytes('lz'))) {
            ...
            uint setGas = isRedirect ? 0 : params.gas;
            bytes memory adapter = _getAdapter(setGas);

            if (params.redirectFee > 0) {
                (uint messageFee,) = lzEndpoint.estimateFees(route.chainId, address(this), params.payload, false, adapter);
                _validateAndChargeUA(params.srcUA, messageFee);
                lzEndpoint.send{value : messageFee}(chId, this.getTrustedRemote(chId), params.payload, payable(params.origin), address(0x0), adapter);
                return;
            }
            lzEndpoint.send{value : msg.value}(chId, this.getTrustedRemote(chId), params.payload, payable(params.origin), address(0x0), adapter);
            return;
        }
        ...
        if (keccak256(bytes(route.provider)) == keccak256(bytes('ax'))) {
            ...
            if (params.redirectFee > 0) {
                ...
                axGasReceiver.payNativeGasForContractCall{value : params.redirectFee}(address(this), route.dstChain, dstStringAddress, params.payload, params.origin);
            } else {
                axGasReceiver.payNativeGasForContractCall{value : msg.value}(address(this), route.dstChain, dstStringAddress, params.payload, params.origin);
            }
            gateway.callContract(route.dstChain, dstStringAddress, params.payload);
        }
    }
    ...
    function handleCall(bytes memory _payload) internal {
        (string memory dstChainName, address dstUA, bytes memory fnData, uint gas, address srcUA, string memory srcChain, address origin, uint256 redirectFee) = abi.decode(_payload, (string, address, bytes, uint, address, string, address, uint256));

        if (keccak256(bytes(dstChainName)) != keccak256(bytes(chainName))) {
            emit Redirected(srcChain, dstChainName);
            RouteCall memory params = RouteCall(dstChainName, _payload, gas, origin, srcUA, redirectFee);
            call(params);
    ...
    function _validateAndChargeUA(address ua, uint256 fee) internal {
        require(address(this).balance >= fee, "ROUTER_NOT_FUNDED");
        require(uaRedirectBudget[ua] >= fee, "UA_NOT_FUNDED");
        uaRedirectBudget[ua] -= fee;
    }
    ...
    function estimateFees(uint16 chainId, bytes memory _payload, uint gas) external view returns (uint) {
        (uint fee,) = lzEndpoint.estimateFees(chainId, address(this), _payload, false, _getAdapter(gas));
        return fee;
    }
