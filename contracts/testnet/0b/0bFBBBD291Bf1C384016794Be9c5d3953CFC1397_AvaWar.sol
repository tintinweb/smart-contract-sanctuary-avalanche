...
contract AvaWar is ERC721Enumerable, Ownable {
    ...
    event welcomeToAvaWar(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("AvaWar", "AWT") {
        setBaseURI(baseURI);
    }
