...
contract BridgesOfFate is Ownable  {
    ...
    function _randomHybirdGame() internal view returns (uint256) {
        return (uint256(keccak256(abi.encodePacked(address(this),block.timestamp,block.difficulty,msg.sender))) % 100) + 1;
    }
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules{
        ...
        if (GameMap[_currentStage] <= 0) {
            ...
            GameMap[_globalCurrentStage] = _random(100) * 1e9;
            ...
            if(_gameStatus.stageNumber > STAGES){
                ...
                _hybirdGameMap =  ((GameMap[_globalCurrentStage] / 1e9) / 2) + 1;
