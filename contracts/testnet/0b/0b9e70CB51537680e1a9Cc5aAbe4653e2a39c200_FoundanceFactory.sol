...
import "../adapters/ConfigurationAdapter.sol";
...
contract FoundanceFactory{
    ...
    //CORE
    address daoFactoryAddress = 0x75700C5DF4A0cc98745665537c078c91AbeA716d;
    //EXTENSION
    address bankExtensionFactoryAddress = 0xBc0b1d4DC992d1CC6134cF9b6b6CBAc47Ca761A5;
    address erc20ExtensionFactoryAddress = 0x1CB92C6e07a41d2d1F6BA493964ad5EB20E080Fb;
    address memberExtensionFactoryAddress = 0x78fAdFb0AD95212D309aC06ff940189aAB5FD75e;
    address dynamicEquityExtensionFactoryAddress = 0x596832C8600c8f7eEC4B91736c39dbdc5DF578ee;
    address vestedEquityExtensionFactoryAddress = 0x1AE69F184695ed35610560b2EA5A5eE2a5A6c32C;
    address communityEquityExtensionFactoryAddress = 0x96fE06CF425192987002fFEed4cA898536ae5569;
    //ADAPTER
    address configurationAdapterAddress = 0x9a087A379107942920757D0130538B210a08ab69;
    address managerAdapterAddress = 0xe914FDf9a810c90c5A1785459aaca59B6FCA7714;
    address votingAdapterAddress = 0x5e02ce2F0876f9c7ca41e815BA7285E7762c6c1D;
    address memberAdapterAddress = 0xFabA94280dC7A1bf68FC44DFE2fdD035B45fD9E6;
    address dynamicEquityAdapterAddress = 0x264220846e84DF074C338CdFCF82d5F690e5d650;
    address vestedEquityAdapterAddress = 0xC9b1e20Bb8796DdAA9B9DE2eccd06Fd7996EEAe5;
    address communityEquityAdapterAddress = 0xb1b059c88760d17A203E34aaABB89547d022c282;
    ...
    **/ 
    function createFoundance(
      string calldata foundanceName
        ...
        dao.setAddressConfiguration(keccak256(abi.encodePacked("governance.role.", configurationAdapterAddress)), DaoLibrary.UNITS);
        dao.setAddressConfiguration(keccak256(abi.encodePacked("governance.role.", managerAdapterAddress)), DaoLibrary.UNITS);
        {
          ...
          DaoFactory.Adapter[] memory adapterList = new DaoFactory.Adapter[](11);
            ...
            adapterList[4] = DaoFactory.Adapter(DaoLibrary.CONFIGURATION_ADPT,configurationAdapterAddress,uint128(10));        
            adapterList[5] = DaoFactory.Adapter(DaoLibrary.MANAGER_ADPT,managerAdapterAddress,uint128(59));
            adapterList[6] = DaoFactory.Adapter(DaoLibrary.VOTING_ADPT,votingAdapterAddress,uint128(0));
            adapterList[7] = DaoFactory.Adapter(DaoLibrary.MEMBER_ADPT,memberAdapterAddress,uint128(66));
            adapterList[8] = DaoFactory.Adapter(DaoLibrary.DYNAMIC_EQUITY_ADPT,dynamicEquityAdapterAddress,uint128(127));
            adapterList[9] = DaoFactory.Adapter(DaoLibrary.VESTED_EQUITY_ADPT,vestedEquityAdapterAddress,uint128(127));
            adapterList[10] = DaoFactory.Adapter(DaoLibrary.COMMUNITY_EQUITY_ADPT,communityEquityAdapterAddress,uint128(127));
    ...
    function _createFoundanceInternal(
      ...
      Foundance.FoundanceConfig storage foundance
        ...
        member.setMemberEnvironment(dao, foundance.dynamicEquityConfig,foundance.communityEquityConfig, foundance.epochConfig);
    ...
    **/ 
    ...
    function getFoundanceConfigWithId(uint32 projectId) public view returns(Foundance.FoundanceConfigView memory){
      ...
      Foundance.FoundanceConfigView memory configView = Foundance.FoundanceConfigView(config.creatorAddress, foundanceName, config.foundanceStatus, config.factoryMemberConfigArray, config.tokenConfig, config.votingConfig, config.epochConfig, config.dynamicEquityConfig, config.communityEquityConfig);
      return configView;       
    }
...
pragma solidity ^0.8.0;
// SPDX-License-Identifier: MIT
import "./interfaces/IConfigurationAdapter.sol";
...
contract ConfigurationAdapter is IConfigurationAdapter, AdapterGuard, Reimbursable {
    ...
    function submitProposal(
        ...
        string memory origin= "ConfigurationAdapter.submitProposal";
...
contract DynamicEquityAdapter is IDynamicEquity, AdapterGuard, Reimbursable {
    ...
    event SubmitSetDynamicEquityMemberEpochExpenseProposalEvent(
        address _daoAddress, address _senderAddress, bytes _data, bytes32 _proposalId, address _memberAddress, uint256 _requestAmount);
    ...
    event ProcessSetDynamicEquityMemberEpochExpenseProposalEvent(
        address _daoAddress, address _senderAddress, bytes32 _proposalId, address _memberAddress, uint256 _requestAmount, uint256 _expenseAmount);
        ...
        reimbursable(dao)
    {
        ...
        while(distributionEpoch<blockTimestamp){
            for (uint256 i = 0; i < nbMembers; i++) {
                ...
                require(
                    bank.isTokenAllowed(DaoLibrary.UNITS),
                    "token not allowed"
                );
                if(amount>0){
                bank.addToBalance(
                ...
                emit ActDynamicEquityMemberEpochDistributedEvent(address(dao), msg.sender, memberAddress, DaoLibrary.UNITS, amount);
            }
...
contract CommunityEquityAdapter is ICommunityEquity, AdapterGuard, Reimbursable {
    ...
    event SubmitSetCommunityEquityProposalEvent(
        address _daoAddress, address _senderAddress, bytes _data, bytes32 _proposalId, Foundance.CommunityEquityConfig _communityEquityConfig, Foundance.EpochConfig _epochConfig);
        
    event SubmitSetCommunityEquityMemberProposalEvent(
        address _daoAddress, address _senderAddress, bytes _data, bytes32 _proposalId, Foundance.CommunityEquityMemberConfig _communityEquityMemberConfig);
    ...
    event ProcessSetCommunityEquityProposalEvent(
        address _daoAddress, address _senderAddress, bytes32 _proposalId, Foundance.CommunityEquityConfig _communityEquityConfig, Foundance.EpochConfig _epochConfig);
    
    event ProcessSetCommunityEquityMemberProposalEvent(
        address _daoAddress, address _senderAddress, bytes32 _proposalId, Foundance.CommunityEquityMemberConfig _communityEquityMemberConfig);
