...
contract AvaFlip {
    ...
    uint256 private maxLimit = 2.2 ether;
    uint256 private minLimit = 0.1 ether;
    uint256 private publicHouseEdge = 2;
    uint256 private nftOwnerHouseEdge = 1;
    ...
    constructor(address _treasuryAddress, address _nftAddress, address _database) payable {
        owner = msg.sender;
        setTresury(_treasuryAddress);
        AvaDiceNFT = IERC721(_nftAddress);
    ...
    function setNFTOwnerHouseEdge(uint256 _houseEdge) external onlyOwner {
        nftOwnerHouseEdge = _houseEdge;
    }
    ...
    function playFlip(
        ...
        console.log("Flip result is", flipResult);
        ...
        if(isNFTOwner(msg.sender)){
            houseEdge = nftOwnerHouseEdge;
        } else {
            houseEdge = publicHouseEdge;
        }

        if(flip == flipResult) {
            uint pay = (msg.value * 100 / (houseEdge + 100)) * 2;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, msg.value, pay, true, flipResult);
            console.log("Win");
            database.updateStatistics(msg.sender, true, msg.value, pay);
        }
        else {
            emit GameResult(msg.sender, msg.value, 0, false, flipResult);
            console.log("Lose");
            database.updateStatistics(msg.sender, false, msg.value, 0);
        }
    }

    function isNFTOwner(address _msgSender) internal view returns (bool) {
        if(AvaDiceNFT.balanceOf(_msgSender) != 0) {
            return true;
        }
        else {
            return false;
