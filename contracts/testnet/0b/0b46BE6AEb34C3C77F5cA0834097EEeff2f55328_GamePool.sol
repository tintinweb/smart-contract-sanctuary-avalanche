...
contract GamePool  is ERC1155Holder, ERC721Holder,Multicall,Context{
    ...
    event WithdrawGameProbeEvent(uint chainId,address player,uint256 id,uint256 amount,uint256 timeStamp);
    event withdrawRoleEvent(uint chainId,address player,uint256 tokenId,uint256 timeStamp);

    event withdrawCytEVent(uint chainId,address player,uint256 amount,uint256 timeStamp);
    event withdrawCoinEvent(uint chainId,address player,uint256 amount,uint256 timeStamp);
   ...
   */
    ...
    */ 
    function withdrawCyt(uint256 amount,bytes memory signature,uint256  currentTimeStamp,uint chainId)external nonReentrant updateProof(currentTimeStamp) {
      require(chainId == block.chainid,"misMatched chinId ");
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,amount,signature,currentTimeStamp,"ERC20_CYT",chainId)==true,"You don't get the proof right");
      IERC20(cyt).transfer(_msgSender(),amount);
      proofMapping[signature] = 1;        
      emit withdrawCytEVent(chainId,_msgSender(),amount,currentTimeStamp);
    }

    /*@dev: player retrieve  coin
     *@param: signature：proof of cyt. currentTimeStamp: timeStamp
    */ 
    function withdrawCoin(uint256 amount,bytes memory signature,uint256 currentTimeStamp,uint chainId) external nonReentrant updateProof(currentTimeStamp){
      require(chainId == block.chainid,"misMatched chinId ");
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,amount,signature,currentTimeStamp,"ERC20_COIN",chainId)==true,"You don't get the proof right");
      IERC20(coin).transfer(_msgSender(),amount);
      proofMapping[signature] = 1;        
      emit withdrawCoinEvent(chainId,_msgSender(),amount,currentTimeStamp);
    }
   ...
   /* @dev:  player retrieve gameProbe
    * @param: ids: the collection to retrieve the id . amounts: The number of ids to retrieve
    */
    function WithdrawGameProbe(address player,uint256  id,uint256  amount,bytes memory signature,uint256 currentTimeStamp,uint chainId) external nonReentrant updateProof(currentTimeStamp){
      require(chainId == block.chainid,"misMatched chinId ");
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,id,amount,signature,currentTimeStamp,"ERC1155_game",chainId)==true,"You don't get the proof right");
       IERC1155(erc1155GameAddress).safeTransferFrom(address(this),player,id,amount,"0x"); 
         proofMapping[signature] = 1;
       emit WithdrawGameProbeEvent(chainId,player,id,amount,block.timestamp);
    }
    ...
    */
    ...
    /*@dev: player retrieves the role
    * @param: tokenId: The player wants to load the Id of the equipment. amout: the amount of erc721
    */
    function withdrawRole(address player,uint256 tokenId,bytes memory signature,uint256 currentTimeStamp,uint chainId) external nonReentrant updateProof(currentTimeStamp){
      require(chainId == block.chainid,"misMatched chinId ");
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,tokenId,signature,currentTimeStamp,"ERC721_Role",block.chainid)==true,"You don't get the proof right");
      IERC721(roleAddress).safeTransferFrom(address(this),player,tokenId);
        proofMapping[signature] = 1;
      emit withdrawRoleEvent(chainId,player,tokenId,block.timestamp);     
    }
   ...
   function checkPermissionForGameUpgrade(uint256[] memory oldIds,uint256[] memory oldAmounts, uint256 newId, uint256 amount,address tokenAddress,uint256[] memory currentTimeStamp,uint256 chainId,bytes[] memory signature) internal view returns(bool){
     ...
     for(uint i =0;i<oldIds.length;i++){
      if(i <= oldIds.length-2){
        if(currentTimeStamp[i]>currentTimeStamp[i+1]){
          return false;
          }
      }
 ...
 /* @dev: upgrade game prop
  ...
  function setCytAddress(address cytAddress_) external  {
      if(msg.sender != owner){
              revert Unauthorized(msg.sender);
       } 
      cyt = cytAddress_;
  ...
  function setCoinAddress(address coinAddress_) external{
      if(msg.sender != owner){
              revert Unauthorized(msg.sender);
       } 
     coin = coinAddress_;
