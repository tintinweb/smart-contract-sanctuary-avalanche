...
contract ProjectLaunchFactory {
  ...
  function createSale(
    ...
    if(_seed_details[0] > 0) {
      totalLiquidityTokens = (totalLiquidityTokens + _seed_details[5]);
      totalTokens = (totalTokens + (_seed_details[4] * _seed_details[1]));
    }
    if(_presale_details[0] > 0) {
      totalLiquidityTokens = (totalLiquidityTokens + _presale_details[5]);
      totalTokens = (totalTokens + (_presale_details[4] * _presale_details[1]));
    }
    if(_community_details[0] > 0) {
      totalLiquidityTokens = (totalLiquidityTokens + _community_details[5]);
      totalTokens = (totalTokens + (_community_details[4] * _community_details[1]));
    }
