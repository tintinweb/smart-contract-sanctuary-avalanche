/**
 *Submitted for verification at testnet.snowtrace.io on 2022-05-15
*/
...
interface IERC721NFTCollection is IERC165Upgradeable {
    ...
    error RequestedPrivateModeMintCountExceedsTotal();
    error RequestedFreeMintCountExceedsTotal();
    error RequestedGuaranteedMintCountExceedsTotal();
    error RequestedRaceMintCountExceedsTotalForUser();
    error RequestedRaceMintCountExceedsTotal();
    error NotEnoughNFTsLeft();
    error CanNotAllocateMoreThanMintCap();
    error UserIsFreezed();
    error UserIsUnfreezed();
    ...
    event MintPriceChanged(uint256 newMintPrice, uint256 oldMintPrice);
    event MintCapChanged(uint256 newMintCap, uint256 oldMintCap);
    event WinnersAreSet(
    ...
    event UserMinted(
        ...
        uint256 mintedFreeSpotsCount,
        uint256 mintedGuaranteedSpotsCount,
        uint256 mintedRaceSpotsCount,
        uint256[] mintedIds
    );
    ...
    function mint(
        ...
        uint256 guaranteedMintCount,
        uint256 raceMintCount,
    ...
    function getTotalMintPrice(
        uint256 guaranteedMintCount,
        uint256 raceMintCount
    ) external view returns (uint256 mintFee);
    ...
    AccessControlUpgradeable
{
    ...
    IERC721NFTCollection.Mode public mode;
...
contract ERC721NFTCollectionProxy is ERC721NFTCollectionStorageStructure {
    ...
    function initialize(
        ...
        launchpadFeeWallet = payable(addresses[1]);
        collectionCreatorAddress = addresses[3];

        mintPrice = variables[0];
        maxMintCount = variables[1];
        mintExpirationTime = variables[2];
        launchpadFeePercentage = variables[3];
        mintCap = variables[4];

        _setupRole(SUPER_ADMIN_ROLE, addresses[4]);
        _setupRole(ADMIN_ROLE, addresses[2]);
        _setupRole(FPL_ROLE, addresses[2]);
        _setupRole(FPL_ROLE, collectionCreatorAddress);
        _setupRole(ADMIN_ROLE, collectionCreatorAddress);
        _setupRole(COLLECTION_CREATOR_ROLE, collectionCreatorAddress);

        _setRoleAdmin(SUPER_ADMIN_ROLE, SUPER_ADMIN_ROLE);
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
        _setRoleAdmin(FPL_ROLE, ADMIN_ROLE);
        _setRoleAdmin(COLLECTION_CREATOR_ROLE, COLLECTION_CREATOR_ROLE);
    }
...
contract ERC721NFTCollectionFactory is AccessControl {
    ...
    bytes32 public COLLECTION_CREATOR_ROLE =
        keccak256("COLLECTION_CREATOR_ROLE");
    ...
    constructor(
        ...
        _setupRole(COLLECTION_CREATOR_ROLE, _nftCollectionFactory);
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
    ...
    function createNFTCollectionProxy(
        ...
        // uint256 launchpadFeePercentage
        // uint256 mintCap
        uint256[5] memory _variables,
        // address marketplaceContract,
        // address launchpadFeeWallet,
        // address admin,
        // address collectionCreator
        address[4] memory _addresses
    ) external onlyRole(COLLECTION_CREATOR_ROLE) returns (address) {
        address[5] memory addresses = [
            _addresses[0],
            _addresses[1],
            _addresses[2],
    ...
    AccessControlUpgradeable
{
    ...
    address[] public winners;
    CountersUpgradeable.Counter internal _tokenIds;
    IERC1155NFTCollection.Mode public mode;
...
contract ERC1155NFTCollectionFactory is AccessControl {
    event NewERC1155NFTCollectionImplWasSet(
        address indexed newERC1155NFTCollectionImpl,
    ...
    constructor(
        address _erc1155NFCollectionImpl,
        address _superAdmin,
        address _nftCollectionFactory
    ) {
        _setupRole(ADMIN_ROLE, _msgSender());
        ...
        if (
            IERC1155NFTCollection(_erc1155NFCollectionImpl).supportsInterface(
                type(IERC1155NFTCollection).interfaceId
            )
        ) {
            address oldERC1155NFTCollectionImpl = erc1155NFTCollectionImpl;
            erc1155NFTCollectionImpl = _erc1155NFCollectionImpl;
            emit NewERC1155NFTCollectionImplWasSet(
                _erc1155NFCollectionImpl,
...
contract NFTCollectionFactory is AccessControl {
    ...
    event LaunchpadFeeWalletChanged(
        address indexed newLaunchpadFeeWallet,
        address indexed oldLaunchpadFeeWallet
    );
    ...
    constructor(
        ...
        admin = _msgSender();
        launchpadFeeWallet = payable(
            ISparkWorldToken(_sparkWorldToken).getFeeWallet()
        );
    ...
    function setMarketPlace(address _marketPlace) public onlyRole(ADMIN_ROLE) {
        address oldMarketPlace = marketplaceContract;
        marketplaceContract = _marketPlace;
        emit MarketPlaceAddressChanged(_marketPlace, oldMarketPlace);
    }

    function setLaunchpadFeeWallet(address newLaunchpadFeeWallet)
        public
        onlyRole(ADMIN_ROLE)
    {
        address oldLaunchpadFeeWallet = launchpadFeeWallet;
        launchpadFeeWallet = payable(newLaunchpadFeeWallet);
        emit LaunchpadFeeWalletChanged(
            newLaunchpadFeeWallet,
            oldLaunchpadFeeWallet
        );
    }
