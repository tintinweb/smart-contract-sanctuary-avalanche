...
{
    address public immutable kingsManager;
    ...
    constructor(address _kingsManager, address _treasury) ERC20("Garden", "SPADE") {
        require(
            _kingsManager != address(0),
            "Implementations: kingsManager is not set"
        );
        kingsManager = _kingsManager;
        ...
        onlyKingsManager
    {
        ...
        // effectively protecting KingsManager from suspicious addresses
        ...
        onlyKingsManager
    {
        require(
            ...
            "Garden: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyKingsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "Garden: LiquidityPoolManager is not set"
        );
    ...
    function _transferTaxOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (!excludedFromFees[sender]) {
            ...
            if (isSPADEsLiquidityPool(recipient)) { // if the recipient address is a liquidity pool, apply sell fee
            ...
            } else if (!isSPADEsLiquidityPool(sender)) { // apply transfer fee if this isn't a transfer from the pool (buy)
    ...
    // return true if the "_recipient" address is a SPADE liquidity pool
    function isSPADEsLiquidityPool(address _recipient) private returns (bool) {
