...
contract Storage is InitializableOwnable, ReentrancyGuard {
    ...
    // ============ Variables for Cross Chain ============
    address public _DOOD_ZOO_;
    ...
    function getQuoteCapitalBalanceOf(address lp)
        public
        view
        returns (uint256)
    {
...
contract Trader is Storage, Pricing, Settlement {
    ...
    function sellBaseTokenTo(
        ...
        return _sellBaseToken(_address, amount, data, true, _isFillingSwap);
    }

    function sellBaseToken(uint256 amount, bytes calldata data)
        external
        returns (uint256)
    {
        return _sellBaseToken(msg.sender, amount, data, false, false);
    }

    function _sellBaseToken(
        address sender,
        uint256 amount,
        bytes calldata data,
        bool _isCrossChain,
        ...
        returns (uint256)
    {
        ...
        // settle assets
        if (_isCrossChain && !_isFillingSwap) {
            _quoteTokenTransferOut(_MAINTAINER_, receiveQuote);
        } else {
            _quoteTokenTransferOut(sender, receiveQuote);
        }
        if (data.length > 0) {
            IDODOCallee(sender).dodoCall(false, amount, receiveQuote, data);
        }
        if (_isFillingSwap) {
            _baseTokenTransferIn(_MAINTAINER_, amount);
        } else {
            _baseTokenTransferIn(sender, amount);
        }
        if (mtFeeQuote != 0) {
        ...
        emit SellBaseToken(sender, amount, receiveQuote);

        return receiveQuote;
    }

    function buyBaseToken(uint256 amount, bytes calldata data)
        external
        returns (uint256)
    {
        return _buyBaseToken(msg.sender, amount, data, false, false);
    }
    ...
    function buyBaseTokenTo(
        ...
        return _buyBaseToken(_address, amount, data, true, _isFillingSwap);
    }
        ...
        returns (uint256)
    {
        ...
        // settle assets
        if (_isCrossChain && !_isFillingSwap) {
            _baseTokenTransferOut(_MAINTAINER_, amount);
        } else {
            _baseTokenTransferOut(sender, amount);
        }
        if (data.length > 0) {
            IDODOCallee(sender).dodoCall(true, amount, payQuote, data);
        }
        if (_isFillingSwap) {
            _quoteTokenTransferIn(_MAINTAINER_, payQuote);
        } else {
            _quoteTokenTransferIn(sender, payQuote);
        }
        ...
        emit BuyBaseToken(sender, amount, payQuote);
...
contract Admin is Storage {
    ...
    function setLiquidityProviderFeeRate(uint256 newLiquidityPorviderFeeRate)
        external
        onlyOwner
    {
        emit UpdateLiquidityProviderFeeRate(
            _LP_FEE_RATE_,
            newLiquidityPorviderFeeRate
        );
        _LP_FEE_RATE_ = newLiquidityPorviderFeeRate;
        _checkDODOParameters();
    }

    function setMaintainerFeeRate(uint256 newMaintainerFeeRate)
        external
        onlyOwner
    {
    ...
    function setGasPriceLimit(uint256 newGasPriceLimit)
        external
        onlySupervisorOrOwner
    {
