...
contract Oceans_Finance is ERC20Detailed, Ownable {
    ...
    string public _name = "Ocean Finance";
    string public _symbol = "OCEANS";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 807 * 10**3 * 10**DECIMALS;

    uint256 public liquidityFee = 30;
    uint256 public treasuryFee = 30;
    uint256 public oceanInsuranceFundFee = 70;
    uint256 public sellFee = 30;
    uint256 public blackholeFee = 20;

    uint256 public totalFee = liquidityFee.add(treasuryFee).add(oceanInsuranceFundFee).add(blackholeFee);
    ...
    //testnet USDC
    address USDC = 0x0f2A5D18aD37395266c0b59e3dfCFe790a0EE8E1;
    ...
    address public oceanInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("Ocean Finance", "OCEANS", uint8(DECIMALS)) Ownable() {
        ...
        oceanInsuranceFundReceiver = 0xe92d0a76B04D3d89fC199060a229D50e78355238;
    ...
    function takeFee(
        ...
        uint256 _oceanInsuranceFundFee = oceanInsuranceFundFee;
        ...
        if (recipient == pair) {
            ...
            _oceanInsuranceFundFee = oceanInsuranceFundFee.add(_sell);
            _liquidityFee = liquidityFee.add(_sell);
        }

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_totalFee);

        uint256 forburn = feeAmount.div(feeDenominator).mul(blackholeFee);

        uint256 subtotal = feeAmount.sub(forburn);
       
        _gonBalances[blackhole] = _gonBalances[blackhole].add(
            gonAmount.div(feeDenominator).mul(blackholeFee)
        );
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(_oceanInsuranceFundFee))
        );
        _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(
            gonAmount.div(feeDenominator).mul(_liquidityFee)
        );

        emit Transfer(sender, blackhole, forburn.div(_gonsPerFragment));
        emit Transfer(sender, address(this), subtotal.div(_gonsPerFragment));
        return gonAmount.sub(subtotal);
    }
    ...
    function addLiquidity() internal swapping {
        ...
        //Testnet Usc with router
        IERC20(USDC).approve(0x2D99ABD9008Dc933ff5c0CD271B88309593aB921, amountUSDCLiquidity);

        //Mainnet USDC With router
    ...
    function swapBack() internal swapping {
        ...
        IERC20(USDC).transfer(treasuryReceiver, amountUSDCToTreasuryAndAIF.mul(treasuryFee).div(treasuryFee.add(oceanInsuranceFundFee)));
        IERC20(USDC).transfer(oceanInsuranceFundReceiver, amountUSDCToTreasuryAndAIF.mul(oceanInsuranceFundFee).div(treasuryFee.add(oceanInsuranceFundFee)));
    }
    ...
    function setFeeReceivers(
        ...
        address _oceanInsuranceFundReceiver,
        address _blackhole
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        oceanInsuranceFundReceiver = _oceanInsuranceFundReceiver;
