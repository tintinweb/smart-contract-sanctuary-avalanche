...
contract SToken is ERC20, Ownable {
    ...
    event LogRebase(uint256 indexed nonce, uint256 totalSupply);
     ...
     * @param nonce The number of new fragment tokens to add into circulation via expansion.
     * @return The total number of fragments after the supply adjustment.
     */
    function rebase(uint256 nonce, uint256 delta) external onlyStaking returns (uint256) {
        if (delta == 0) {
            emit LogRebase(nonce, _totalSupply);
            return _totalSupply;
        }

		uint256 circulatingSupply_ = circulatingSupply();
		uint256 amount = delta * _totalSupply / circulatingSupply_;

        _totalSupply += amount; 

		// MAX SUPPLY cannot be reached - event duration
		/*
	   ...
	   */
		...
		// updated uFragment: _gonsPerFragment lower => balanceOf increases
        _gonsPerFragment = TOTAL_GONS / _totalSupply;
