...
contract CollateralVault is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 public MAX_UINT;                               // Constant uint256 Max number
    uint256 public poolLength;                             // All BRT pools length
    ...
    struct PoolInfo {
        ...
        uint256 collateralRate;         // Collateral ratio (E.g:2e18 => 200%, 3e18 => 300%) assume oracle price is 18 decimals
        ...
        uint256 liquidationPenalty;     // User's penalty rate for position liquidated(E.g:20%)
        bool depositEnabled;            
    }
    ...
    event BorrowSystemCoin(address indexed sender, uint256 amount);
    event RepaySystemCoin(address indexed sender, address indexed account, uint256 amount, uint256 _repayDebt);
    event Exit(address indexed sender, uint256 amount);
    event EmergencyExit(address indexed sender, address indexed account, uint256 amount);
    event Liquidate(address indexed sender, address indexed account, address indexed collateralToken, uint256 debt, uint256 punishment, uint256 amount);

    event SetEmergency(address indexed sender, uint256 emergencyStart);
    event PoolsEnabled(uint256 pid, bool depositEnabled);
    ...
    event UpdateBalance(uint256 indexed pid, address indexed account, uint256 indexed _tokenStockSplitIndex);
    ...
    function initContract(address _liqPool, address _systemCoin, address _oracle, int256 _interestRate, uint256 _feeInterval, bool update) external onlyOwner{
        liqPool = _liqPool;
    ...
    function add(IBRT _collateralToken, uint256 _collateralRate, uint256 _assetCeiling, uint256 _assetFloor, uint256 _liquidationPenalty) external onlyOwner {
        require(address(_collateralToken) != address(0));
    ...
    /****************************************** Collateral CORE FUNCTION ******************************************/
 
    function join(uint256 pid, uint256 amount) whenNotPaused external {
        _join(pid, msg.sender, amount);
    }

    function joinAndBorrow(uint256 pid, uint256 collateralamount, uint256 systemCoinAmount) payable whenNotPaused settleAccount(pid, msg.sender) external {
    ...
    function borrowSystemCoin(uint256 pid, uint256 amount) whenNotPaused external {
        _borrowSystemCoin(pid, msg.sender, amount);
    }

    function exit(uint256 pid, uint256 amount) whenNotPaused settleAccount(pid, msg.sender) external {
        require(checkLiquidate(pid, msg.sender, amount, 0) , "collateral remove overflow!");
    ...
    function emergencyExit(uint256 pid, address account) whenPaused notZeroAddress(account) external {
    ...
    function repaySystemCoin(uint256 pid, uint256 amount) whenNotPaused settleAccount(pid, msg.sender) external {
        if(amount == MAX_UINT) {
            amount = assetInfoMap[pid][msg.sender].assetAndInterest;
        }
        _repaySystemCoin(pid, msg.sender, amount);
    }

    function _join(uint256 pid, address account, uint256 amount) internal {
        PoolInfo storage pool = poolInfo[pid];
        require(pool.depositEnabled == true, "Dep N/A");
        pool.collateralToken.safeTransferFrom(account, address(this), amount);
        collateralBalances[pid][account] = collateralBalances[pid][account] + (amount);
        emit Join(account, account, amount);
    }

    function _borrowSystemCoin(uint256 pid, address account, uint256 amount) settleAccount(pid, msg.sender) internal {
        require(checkLiquidate(pid, account, 0, amount) , "overflow liquidation limit!");
        systemCoin.mint(account,amount);
        addAsset(pid, account, amount);
        emit BorrowSystemCoin(account, amount);
    }
    ...
    function _repaySystemCoin(uint256 pid, address account, uint256 amount) internal{
        ...
        if(amount>_repayDebt){
            require(systemCoin.transferFrom(msg.sender, liqPool, amount-_repayDebt),"systemCoin : transferFrom failed!");       // Interest of loan
            systemCoin.burn(msg.sender,_repayDebt);
        }else{
            systemCoin.burn(msg.sender,amount);
        }
        emit RepaySystemCoin(msg.sender, account, amount, _repayDebt);
    }

    function liquidate(uint256 pid, address account) whenNotPaused settleAccount(pid, account) external onlyAuthorized {
        require(account != address(0),"A!=0");
        require(!checkLiquidate(pid, account, 0, 0) , "liquidation check error!");
        _liquidate(pid, account);
    }

    function _liquidate(uint256 pid, address account) settleAccount(pid, msg.sender) internal {
        PoolInfo storage pool = poolInfo[pid];
        ...
        allDebt += penalty;
        uint256 _payback = allDebt * calDecimals / collateralPrice;
        ...
        emit Liquidate(msg.sender,account,address(pool.collateralToken),allDebt,penalty,_payback);  
    }

    function setLiquidationInfo(uint256 pid, uint256 _liquidationPenalty)external onlyOwner{
        _setLiquidationInfo(pid ,_liquidationPenalty);
    }
    ...
    function _setLiquidationInfo(uint256 pid, uint256 _liquidationPenalty) internal {
        PoolInfo storage pool = poolInfo[pid];
        require(_liquidationPenalty <= 5e17 && (calDecimals+_liquidationPenalty) <= pool.collateralRate,"Collateral Vault : Liquidate setting overflow!");
    ...
    function addAsset(uint256 pid, address account, uint256 amount) internal settleAccount(pid, account) {
        PoolInfo storage pool = poolInfo[pid];
        assetInfoMap[pid][account].originAsset += amount;
        assetInfoMap[pid][account].assetAndInterest += amount;
        pool.totalAssetAmount += amount;
        require(assetInfoMap[pid][account].assetAndInterest >= pool.assetFloor, "Debt is below the limit");
        require(pool.totalAssetAmount <= pool.assetCeiling, "vault debt is overflow");
    ...
    function subAsset(uint256 pid, address account, uint256 amount) internal returns(uint256) {
        ...
        if(assetAndInterest == amount) {
            ...
            _subAsset = originBalance * (amount) /assetAndInterest;
            assetInfoMap[pid][account].assetAndInterest = assetAndInterest - (amount);
            require(assetInfoMap[pid][account].assetAndInterest >= pool.assetFloor, "Debt is below the limit");
            assetInfoMap[pid][account].originAsset = originBalance - (_subAsset);
        } else {
    ...
    // rpower(uint256(1e27+interestRate) , (block.timestamp-latestSettleTime) /interestInterval, rayDecimals);
    ...
    function oracleLpPrice(address asset) internal view returns (bool,uint256){
    ...
    function setPoolLimitation(uint256 pid, uint256 _assetCeiling,uint256 _assetFloor)external onlyOwner {
        PoolInfo storage pool = poolInfo[pid];
        pool.assetCeiling = _assetCeiling;
        pool.assetFloor = _assetFloor;
    ...
    */
    function updatePoolsEnabled(uint256 pid, bool depositEnabled) public onlyOwner {
        PoolInfo storage pool = poolInfo[pid];
