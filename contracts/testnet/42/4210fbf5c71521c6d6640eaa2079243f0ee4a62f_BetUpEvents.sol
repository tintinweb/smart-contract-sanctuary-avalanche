...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    // @dev mapping from `pairID` to `pair's extra info`
    mapping(uint256 => ExtraPair) public pairExtra;

    uint32 public publicitySecond;
    ...
    */
    function open(
        ...
        require(_opt._optionsQty >= 2 && _opt._optionsQty <= 7, "invalid _optionsQty");
        require( _opt._endStackAt > block.timestamp && _opt._resolutionAt > _opt._endStackAt, "invalid time");

        uint256 resolversLen = _resolvers.length;
        require(resolversLen & 1 == 1, "invalid _resolvers");
        require(_opt._chip != address(0) && validChips[_opt._chip] == 1, "invalid _chip");
        if( _opt._resultID > _opt._optionsQty || _opt._resultID == 0) revert InvalidResultID(_opt._resultID);
        if(_opt._amount == 0) revert InvalidAmount(_opt._amount);

        uint64 startAt = uint64(block.timestamp);

        pairs[pairIDCounter].resolutionAt = _opt._resolutionAt;
        pairs[pairIDCounter].creator = sender;
        pairs[pairIDCounter].resolverQty = uint32(resolversLen);
        pairs[pairIDCounter].endStackAt = _opt._endStackAt;
        pairs[pairIDCounter].chip = _opt._chip;
        pairs[pairIDCounter].optionsQty = _opt._optionsQty;
        pairs[pairIDCounter].creationRatio = createRewardsRatio;
        innerPairs[pairIDCounter].openAmount = _openCoefficient;
        innerPairs[pairIDCounter].deadlineAt = _opt._resolutionAt + resolveDeadlineSecond;
        innerPairs[pairIDCounter].startAt = startAt;
        ...
        _stack(pairIDCounter, _opt._amount, _opt._chip, startAt, _opt._endStackAt, _opt._resultID);
    ...
    function close(uint256 _pairID,  uint16 _resultID, bool _isInvalid) external {
        ...
        bytes32 pairResolverID = keccak256(abi.encodePacked(_pairID, msg.sender));
        Resolver memory pairResolver = pairsResolver[pairResolverID];
        require(pairResolver.isResolver && pairResolver.resultID == 0, "settlement denied");

        InnerPair memory innerPair = innerPairs[_pairID];
        require(innerPair.resultID == 0, "resolved");
        require(
            _notOneSide(_pairID, pair.optionsQty) &&
            (block.timestamp > pair.resolutionAt && block.timestamp < innerPair.deadlineAt),
            "operation denied"
        );
        ...
        if((resultCounter[resultCounterID] + 1) * 10e3 >= pair.resolverQty * 2 *  10e3 / 3 ) {
            innerPairs[_pairID].resultID =  __resultID;
            pairExtra[_pairID].publicityEndAt = uint64(block.timestamp) + publicitySecond;
        }
    ...
    function stack(uint256 _pairID, uint16 _resultID, uint256 _amount) external  {
        ...
        if(pair.paused) revert PairPaused(_pairID);
        if( _resultID == 0  || _resultID > pair.optionsQty) revert InvalidResultID(_resultID);

        InnerPair memory innerPair = innerPairs[_pairID];
        require(innerPair.resultID == 0 && block.timestamp < pair.endStackAt, "operation denied");
        if(_amount == 0) revert InvalidAmount(_amount);

        _stack(_pairID, _amount, pair.chip, innerPair.startAt, pair.endStackAt, _resultID);
    }
    ...
    function _stack(
        ...
        address sender = msg.sender;
        IERC20MetadataUpgradeable(_chip).safeTransferFrom(sender, address(this), _amount);

        uint256 bonus = IBetUpCore(betUpCore).calcIncentiveBonus(_startAt, _endStackAt, _amount);
        bytes32 playerKey = keccak256(abi.encodePacked(_pairID, _resultID, sender));
        bytes32 opvKey = keccak256(abi.encodePacked(_pairID,_resultID));
        unchecked {
            players[playerKey].amount += _amount;
            players[playerKey].amountWithBonus += bonus;
            optionsVolume[opvKey].amount += _amount;
            optionsVolume[opvKey].amountWithBonus += bonus;
            innerPairs[_pairID].total +=  _amount;
        }

        IBetUpCore(betUpCore).setPlayerRecord(sender, _amount, IERC20MetadataUpgradeable(_chip).decimals());
    ...
    function claim(uint256 _pairID, uint16 _resultID, uint256 _maxLevel) external  {
        ...
        require(refund > 0 || reward > 0, "invalid claim");
    ...
    function claimCreationRewards(uint256 _pairID) external {
        ...
        InnerPair memory innerPair = innerPairs[_pairID];
        uint256 resultTotalAmount = optionsVolume[keccak256(abi.encodePacked(_pairID, innerPair.resultID))].amount;
        require(_notOneSide(_pairID, pair.optionsQty)
            && (innerPair.resultID > 0 && innerPair.resultID != INVALID_ID)
            && block.timestamp > pairExtra[_pairID].publicityEndAt && resultTotalAmount != 0 , "operation denied");

        address receiver = msg.sender;
        require(pair.creator == receiver, "permission denied");
        require(pair.claimedReward == false, "claimed");
        require(pair.creationRatio > 0, "isInvalid");

        uint256 fee = (innerPair.total - resultTotalAmount) * 10 * pair.creationRatio / 1000000 ;
        pairs[_pairID].claimedReward = true;
        IERC20MetadataUpgradeable(pair.chip).safeTransfer(receiver, fee);
        if(innerPair.openAmount > 0) IERC20MetadataUpgradeable(betUpDaoToken).safeTransfer(receiver, uint256(innerPair.openAmount) * 10 ** 18);
    ...
    function setResultID(uint256 _pairID, uint16 _resultID, bool _isInvalid) external onlyRole(GOVERNANCE_ROLE) {
        ...
        if(
            !_notOneSide(_pairID, pair.optionsQty)
            || (block.timestamp > pairExtra[_pairID].publicityEndAt && innerPairs[_pairID].resultID > 0)
        ) revert OperationDenied(_pairID);
        ...
        if(_isInvalid) {
            ...
            require(_resultID <= pair.optionsQty && _resultID > 0, "Invalid _resultID");
            __resultID = _resultID;
        }

        pairExtra[_pairID].publicityEndAt = uint64(block.timestamp) + publicitySecond;
        innerPairs[_pairID].resultID =  __resultID;
        pairs[_pairID].creationRatio = 0;
        innerPairs[_pairID].openAmount = 0;
        pairExtra[_pairID].adminResolved = 1;
    ...
    function setOpenCoefficient(uint24 _num) external onlyRole(DEFAULT_ADMIN_ROLE) {
        openCoefficient = _num;
    }

    function setResolveDeadlineSecond(uint32 _spam) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint256 resultOpAmount = optionsVolume[keccak256(abi.encodePacked(pairID, innerPair.resultID))].amount;

            require(
                _notOneSide(pairID, pair.optionsQty)
                && (innerPair.resultID > 0 && innerPair.resultID != INVALID_ID)
                && block.timestamp > pairExtra[pairID].publicityEndAt
                && resultOpAmount != 0 ,
                "operation denied"
            );

            uint256 availableAmount = ((innerPair.total - resultOpAmount) * (10000  - pair.creationRatio)) / (10*10000);
    ...
    */
    function _getReward(
        ...
        uint256 resultOpAmount =  optionsVolume[keccak256(abi.encodePacked(_pairID,innerPair.resultID))].amount;
        OptionVolume memory playerOp =  optionsVolume[keccak256(abi.encodePacked(_pairID,_resultID))];

        if(_player.amount == 0) return (refund,reward);

//        if(resultOpAmount == total || (playerOp.amount != innerPair.total && resultOpAmount == 0)){}
        if(!_notOneSide(_pairID, pair.optionsQty)) {
            if(block.timestamp > pair.endStackAt) refund = _player.amount;
//            console.log(1);
        }else {
//            console.log(2);
            //  && pairExtra[_pairID].publicityEndAt == 0
            if(block.timestamp > innerPair.deadlineAt && innerPair.resultID == 0) {
//                console.log(3);
                refund = _player.amount;
            }else if(
                (pairExtra[_pairID].publicityEndAt> 0 && block.timestamp > pairExtra[_pairID].publicityEndAt)
                || ((pairExtra[_pairID].publicityEndAt ==0 && block.timestamp > innerPair.deadlineAt))
            ) {
//                console.log(4);
                if(innerPair.resultID > 0 && innerPair.resultID < INVALID_ID) {
//                    console.log(5);
                    unchecked {
                        if(innerPair.resultID == _resultID) {
//                            console.log(6);
                            refund = _player.amount + ( 9 * _player.amountWithBonus * (innerPair.total - playerOp.amount)) / (10 * playerOp.amountWithBonus);
                            reward = winPer * _player.amount  / (10 ** IERC20MetadataUpgradeable(pairs[_pairID].chip).decimals());
                        }else {
                            if(resultOpAmount == 0) {
//                                console.log(7);
                                refund = _player.amount;
                            }
                            reward = losePer * _player.amount / (10 ** IERC20MetadataUpgradeable(pairs[_pairID].chip).decimals());
                        }
                    }
                }
            }
    ...
    function setPublicitySecond(uint32 _time) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_time > 0, "invalid time");
        publicitySecond = _time;
    }
