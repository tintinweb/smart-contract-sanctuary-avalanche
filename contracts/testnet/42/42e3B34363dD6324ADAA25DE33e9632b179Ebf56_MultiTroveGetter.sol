...
/**
 * YetiController is the contract that controls sytem parameters.
 ...
 * the team to update these. This includes: change the price
...
contract YetiController is Ownable, IYetiController, IBaseOracle, CheckContract {
    ...
    uint256 private yetiFinanceTreasurySplit;
    ...
    function setAddresses(
        ...
        addressesSet = true;
        yetiFinanceTreasurySplit = 2e17; // 20% to treasury by default
    }
    ...
    function deprecateAllCollateral() external override onlyOwner {
        uint len = validCollateral.length;
        for (uint i; i < len; i++) {
     ...
     * Still can interact with it via validCollateral and CollateralParams
     */
    function unDeprecateCollateral(address _collateral) external override exists(_collateral) onlyOneWeekTimelock {
    ...
    function updateMaxCollsInTrove(uint _newMax) external override onlyOwner {
    ...
    {
        checkContract(_priceCurve);
        require(IPriceCurve(_priceCurve).initialized(), "price curve not set");
     ...
     */
    function changeRatios(address _collateral, uint256 _newSafetyRatio, uint256 _newRecoveryRatio)
    external override
    exists(_collateral)
    onlyOneWeekTimelock
    {
        ...
        require(collateralParams[_collateral].safetyRatio <= _newSafetyRatio, "New SR must be >= than previous SR");
    ...
    function setDefaultRouter(address _collateral, address _router) external override onlyOneWeekTimelock exists(_collateral) {
    ...
    function changeYetiFinanceTreasurySplit(uint256 _newSplit) external override onlyOneWeekTimelock {
        require(_newSplit <= 8e17, "split can't be more than 80%"); // 20% goes to the borrower for redemptions
    ...
    function getIndices(address[] memory _colls) external view override returns (uint256[] memory indices) {
     ...
     */
    function checkCollateralListSingle(address[] memory _colls, bool _deposit) external view override {
    ...
    function _checkCollateralListSingle(address[] memory _colls, bool _deposit) internal view {
        ...
        for (uint i; i < len; ++i) {
            ...
            _exists(thisColl);
            require(collateralParams[thisColl].index > prevIndex || i == 0, "Collateral list must be sorted by index");
     ...
     */
    function checkCollateralListDouble(address[] memory _depositColls, address[] memory _withdrawColls) external view override {
        uint256 _depositLen = _depositColls.length;
        uint256 _withdrawLen = _withdrawColls.length;
        if (_depositLen == 0) {
            if (_withdrawLen == 0) { // Both empty, nothing to check
                return;
            } else { // Just withdraw check
                _checkCollateralListSingle(_withdrawColls, false);
                return;
            }
        } if (_withdrawLen == 0) { // Just deposit check
        ...
        while (true) {
            ...
            if (dIndex < wIndex) { // update d coll
                if (d_i == _depositLen) {
                    break;
                }
    ...
    function getValueUSD(address _collateral, uint256 _amount) external view override returns (uint256)
    {
    ...
    function leverUpEnabled() external override view returns (bool) {
        return isLeverUpEnabled;
    }

    function getMaxCollsInTrove() external override view returns (uint) {
     ...
     */
    function getFeeSplitInformation() external override view returns (uint256, address, address) {
    ...
    // Gets total variable fees from all collaterals with entire system collateral, 
    ...
    function getTotalVariableDepositFeeAndUpdate(
        ...
        uint256 _VCin,
        uint256 _VCout
    ) external override returns (uint YUSDFee) {
        require(
            msg.sender == borrowerOperationsAddress,
            "caller must be BO"
        );
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            if (feeBoostrapPeriodEnabled) {
                controllerFee = LiquityMath._min(controllerFee, 1e16); // cap at 1%
            } 
            vars.collateralYUSDFee = vars.collateralInputVC
                .mul(controllerFee).div(1e18);
