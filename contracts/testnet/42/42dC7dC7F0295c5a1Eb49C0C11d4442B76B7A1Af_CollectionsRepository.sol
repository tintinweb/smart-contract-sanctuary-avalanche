...
contract OmniERC721 is IOmniERC721, ERC721URIStorage {
    ...
    /**
     * @notice Sets metadata URIs of the tokens that belong to this collection.
     *
    ...
    function _setMetadataURIs(string[] memory _metadataURIs) internal {
        ...
        totalSupply = _metadataURIs.length;
    }
    ...
    function addMetadataURIs(string[] memory _newMetadataURIs) external {
        require(isEditable(), "!EDITABLE");
        string[] memory _totalMetadataURIs = new string[](metadataURIs.length + _newMetadataURIs.length);

        for (uint256 i = 0; i < metadataURIs.length; i++) {
            _totalMetadataURIs[i] = metadataURIs[i];
        }
        uint256 currentIndex = _totalMetadataURIs.length;

        for (uint256 j = 0; j < _newMetadataURIs.length; j++) {
            _totalMetadataURIs[currentIndex] = _newMetadataURIs[j];
            currentIndex++;
        }

        _setMetadataURIs(_totalMetadataURIs);
    }

    function isEditable() public view returns (bool) {
        return msg.sender == creator && block.timestamp <= (dropFrom > 0 ? dropFrom : (createdAt + 1 hours)) && tokenIds == 0;
    }
}
