...
contract Manager is ReentrancyGuard, Pausable, AccessControl {
  ...
  function equityInfo(address _vault) public view returns (uint256, uint256, uint256, uint256) {
    ...
    uint256 tokenAAmount = vaults[_vault].lpTokenAmount
                                          * reserveA
                                          / IJoePair(lpToken).totalSupply();
    uint256 tokenBAmount = vaults[_vault].lpTokenAmount
                                          * reserveB
                                          / IJoePair(lpToken).totalSupply();
    ...
    uint256 tokenAEquityValue = IJoeRouter02(router).getAmountsOut(tokenAAmount, tokenAToBPath)[1];
    // Get tokenB (assuming to be USDC) value based on ChainLink (~$1.00)
    // uint256 tokenBEquityValue = IOracle(oracle).getPriceFromChainlink(tokenB) * tokenBAmount;
    // Mock it as $1 for now (1e6)
    uint256 tokenBEquityValue = tokenBAmount;

    return (tokenAAmount, tokenBAmount, tokenAEquityValue, tokenBEquityValue);
  }

  // get debtValue of vault
  function debtInfo(address _vault) public view returns (uint256, uint256, uint256, uint256) {
    ...
    tokenAToBPath[0] = tokenA;
    tokenAToBPath[1] = tokenB;
    uint256 tokenADebtValue = IJoeRouter02(router).getAmountsOut(tokenADebtAmount, tokenAToBPath)[1];
    // Get tokenB (assuming to be USDC) value based on ChainLink (~$1.00)
    // uint256 tokenBDebtValue = IOracle(oracle).getPriceFromChainlink(tokenB) * tokenBDebtAmount;
    // Mock it as $1 for now (1e6)
  ...
  function positionValue(address _vault) public view returns (uint256) {
    ...
    (, , uint256 tokenADebtValue, uint256 tokenBDebtValue) = debtInfo(_vault);

    return tokenAEquityValue + tokenADebtValue + tokenBEquityValue + tokenBDebtValue;
  }

  function getPositionInfo(address _vault) public view onlyApprovedVault returns (uint256, uint256, uint256, uint256) {
    (, , uint256 tokenAEquityValue, uint256 tokenBEquityValue) = equityInfo(_vault);
    (uint256 tokenADebtAmount, uint256 tokenBDebtAmount, uint256 tokenADebtValue, uint256 tokenBDebtValue) = debtInfo(_vault);

    uint256 equityValue = tokenAEquityValue + tokenBEquityValue;
    uint256 debtValue = tokenADebtValue + tokenBDebtValue;

    return (equityValue, debtValue, tokenADebtAmount, tokenBDebtAmount);
  }
  ...
  function work(
    ...
    if (_borrowTokenAValue > 0) {
      ILendingPool(tokenALendingPool).borrow(_borrowTokenAValue);
      // vaults[msg.sender].tokenADebtAmount +=
    }
    ...
    if (positionValue(msg.sender) > _targetLpValue) {
      // Calculate repay value difference betweem the two assets
      ...
      // uint256 lpWithdrawAmount = (minimumRepayValue * 2 + repayDifferenceValue) / IOracle(oracle).getPrice(lpToken);

      // Get AVAX value in USDC, and mock USDC to be valued at $1 (1e6) for now
      uint256 _tokenAOneAmountValue = IJoeRouter02(router).getAmountsOut(1_000_000_000_000_000_000, tokenAToBPath)[1];

      uint256 lpWithdrawAmount = (minimumRepayValue * 2 + repayDifferenceValue) / (_tokenAOneAmountValue + 1000000);
      ...
      if (swapTokenAForB) {
        ...
        swapAmountIn = IJoeRouter02(router).getAmountsIn(repayDifferenceValue / 2, tokenAToBPath)[0];
      } else {
    ...
    if (positionValue(msg.sender) < _targetLpValue) {
      ...
      if (_repayTokenAValue > 0) {
        uint256 _tokenARepayAmount = IJoeRouter02(router).getAmountsIn(_repayTokenAValue, tokenAToBPath)[0];
