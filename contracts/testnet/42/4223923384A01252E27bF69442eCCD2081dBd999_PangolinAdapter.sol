...
import {IPangolinFactory} from "../../interfaces/pangolin/IPangolinFactory.sol";
import {IPangolinRouter} from "../../interfaces/pangolin/IPangolinRouter.sol";
import {IPangolinChef} from "../../interfaces/pangolin/IPangolinChef.sol";
import {IStakingRewards} from "../../interfaces/sushi/IStakingRewards.sol";

contract PangolinAdapter is AdapterBase, Basic {
    ...
    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "Pangolin")
    {}

    event PangolinFarmEvent(uint256 pid, address owner, uint256 amount);

    event PangolinUnFarmEvent(uint256 pid, address owner, uint256 amount);
        ...
        onlyAdapterManager
    {
        ...
        address exchangeAddr = IPangolinFactory(router.factory()).getPair(
            tokenA,
            tokenB
        );
    ...
    function depositLpToken(bytes calldata encodedData) external onlyProxy {
        (uint256 pid, address chefAddress, uint256 amount) = abi.decode(
            encodedData,
    ...
    function claim_rewards(bytes calldata encodedData) external onlyProxy {
        (uint256 pid, address chefAddress) = abi.decode(
            encodedData,
            (uint256, address)
        );
...
interface IPangolinChef {
    ...
    // function poolInfos() external view returns (tuple[]);
    ...
    // function updatePool(uint256 pid) external returns (tuple pool);
