...
contract Pets is ERC721Enumerable, Pausable, Restricted {
    ...
    address payable adminAddress = payable(0x9F523A9d191704887Cf667b86d3B6Cd6eBE9D6e9); // TODO
    ...
    event OrderSettled(address indexed buyer, uint256 indexed orderNumber);
    ...
    // [0.5 ether, 0.4 ether, 0.3 ether, 0.2 ether, 0.1 ether] for real launch <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    uint256[5] private generationsPriceAVAX = [5 wei, 4 wei, 3 wei, 2 wei, 1 wei];
    uint256[5] private generationsPriceGEM = [20000 ether, 30000 ether, 30000 ether, 40000 ether, 50000 ether];
    ...
    mapping(uint256 => uint256) public petTokenToRangerToken;
    mapping(uint256 => uint256) public rangerTokenToPetToken;
    mapping(address => uint256) public walletToPetToken;
    mapping(address => uint256) public lastWalletAssociation;
    ...
    mapping(uint256 => LimitedMint) public limitedMints;
    ...
    ) ERC721("pipi", "pipi") { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ("Yield Hunt Pets", "YHPETS") for real launch
        ...
        notBlacklisted
    {
        ...
        uint256 totalGemPrice;
        uint256 avaxPrice;
        uint256 gemPrice;

        for (uint256 i = 1; i <= amount;) {
            (avaxPrice, gemPrice) = mintCost(orders.length + i);
        ...
        notBlacklisted
    {
        ...
        if (limitedMints[index].collabTokenPrice > 0 && !adminMint) {
        ...
        if (limitedMints[index].glowPrice > 0 && !adminMint) {
    ...
    function core_mint(Order memory order) internal {
        ...
        emit OrderSettled(order.buyer, ordersSettled);
    }
    ...
    function associatePetToWallet(uint256 petToken)
        ...
        notBlacklisted
    {
        require(ownerOf(petToken) == _msgSender(),    "This pet is not yours");
    ...
    function _associatePetToWallet(address _from, uint256 petToken) internal {
        ...
        lastWalletAssociation[_from] = block.timestamp;
    ...
    /** 
     * Associates many pets to many rangers, in the order specified
     * @param petTokens Ids of the pets to associate
     * @param rangerTokens Ids of the rangers to associate
     */
    function associateManyPetsToManyRangers(uint256[] calldata petTokens, uint256[] calldata rangerTokens)
        external
        whenNotPaused
        onlyEOA
        noReentrency
        notBlacklisted
    {
        require(petTokens.length == rangerTokens.length, "Mismatched inputs");
        for (uint256 i; i < petTokens.length;) {
            _associatePetToRanger(petTokens[i], rangerTokens[i]);
            unchecked{++i;}
        }
    }
    ...
    function _associatePetToRanger(uint256 petToken, uint256 rangerToken) internal {
        require(skeleton.ownerOf(rangerToken) == _msgSender(), "This ranger is not yours");
        require(ownerOf(petToken) == _msgSender(),             "This pet is not yours");
    ...
    /**
     * Dissociates many pets from the rangers associated
     * @param petTokens Id of the pets you'd like to dissociate
     */
    function dissociateManyFromPets(uint256[] calldata petTokens)
        external
        whenNotPaused
        onlyEOA
        noReentrency
    ...
    {
        for (uint256 i; i < petTokens.length;) {
            require(ownerOf(petTokens[i]) == _msgSender(), "This pet is not yours");
            _dissociateFromPet(petTokens[i]);
            unchecked{++i;}
        }
    }
    ...
    /**
     * Dissociates many rangers given from the pets associated
     * @param rangerTokens Ids of the rangers you'd like to dissociate
     */
    function dissociateManyFromRangers(uint256[] calldata rangerTokens)
        external
        whenNotPaused
        onlyEOA
        noReentrency
        notBlacklisted
    {
        for (uint256 i; i < rangerTokens.length;) {
            require(skeleton.ownerOf(rangerTokens[i]) == _msgSender(), "This ranger is not yours");
            _dissociateFromRanger(rangerTokens[i]);
            unchecked{++i;}
        }
    }
    ...
    function _dissociatePetFromRanger(uint256 petToken, uint256 rangerToken) internal {
        ...
        if (petToken != 0 && rangerToken != 0) 
            emit RangerAssociation(ownerOf(petToken), petToken, rangerToken, false);
    }
    ...
    function mintCost(uint256 tokenId) public view returns (uint256, uint256) {
        ...
        return (generationsPriceAVAX[generation], generationsPriceGEM[generation]);
    }
    ...
    function withdrawToken(address _token) public onlyController {
        ...
        IERC20(_token).transferFrom(address(this), adminAddress, getTokenBalance(_token));
    }
    ...
    function kill() external onlyController { // delete for real launch <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
