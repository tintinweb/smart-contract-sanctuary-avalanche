...
contract Stake is OwnableUpgradeable  {
  ...
  function initialize(address _mintAddress, address _BR34PTokenAddress, address _solisiumTokenAddress, address _vaultAddress) external initializer {
    __Ownable_init();
  ...
  //@dev Default payable is empty since Faucet executes trades and recieves AVAX
  fallback() external payable {
    //Do nothing, AVAX will be sent to contract when selling tokens
  }
  ...
  function setTotalAVAX(uint256 newTotalAVAX) public onlyOwner {
    total_avax = newTotalAVAX;
  }
  ...
  function contractInfo() external view returns(uint256 _total_users, uint256 _total_deposited, uint256 _total_withdraw, uint256 _total_avax, uint256 _total_txs, uint256 _total_airdrops) {
    return (total_users, total_deposited, total_withdraw, total_avax, total_txs, total_airdrops);
  }
  ...
  function MultiSendairdrop(address[] memory _to, uint256 _amount) external 
  {
    ...
    //This can only fail if the balance is insufficient
    require(solisiumToken.transferFrom( _addr,_to[i],_amount ),"SOLS to contract transfer failed; check balance and allowance, airdrop");
        //Make sure _to exists in the system; we increase
    require(users[_to[i]].upline != address(0), "_to not found");
        (_realizedAmount, taxAmount) = solisiumToken.calculateTransferTaxes(_addr, _amount);
        //Fund to deposits (not a transfer)
