...
contract FrogFrensStaking is Ownable, Pausable {
  ...
  */
   ...
   /*
    ...
    @description - Checks when the user last claimed is not withing last 24 hours
  ...
  */
    function unStakeToken(uint256 _tokenId) public returns (bool) {
        ...
        require(block.timestamp >= user.lastDepositTime.add(pool.lockPeriodInDays.mul(1 days)),"Stake still in locked state");
        _claimSpawn(msg.sender);
