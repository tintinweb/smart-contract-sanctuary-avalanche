    ...
    IVe
{
    ...
    /// @notice Platypus Ambassador is able to mint up to 50% of vePTP supply and vote for gauges
    address public platypusAmbassador;
    /// @notice
    uint256 internal platypusAmbassadorMinted;
    ...
    function initializeAmbassador(address _platypusAmbassador) public onlyOwner {
        require(platypusAmbassador == address(0), "Don't change ambassador");
        platypusAmbassador = _platypusAmbassador;
    }
    ...
    /**
     * @notice Platypus Ambassador is able to mint up to 50% of vePTP supply and vote for gauges
     * It's balance should monotonic increasing
     */
    function mintForAmbassador() external onlyOwner {
        require(platypusAmbassador != address(0), 'not initialized');

        // mint half of vePTP total supply (exclude those minted to the ambassador)
        uint256 targetAmount = (totalSupply() - platypusAmbassadorMinted) / 2;
        if (platypusAmbassadorMinted >= targetAmount) return;

        uint256 amountToMint = targetAmount - platypusAmbassadorMinted;

        platypusAmbassadorMinted += amountToMint;
        _mint(platypusAmbassador, amountToMint);
    }

    function hackUsedVote(address addr, uint256 value) external onlyOwner {
        usedVote[addr] = value;
    }
}
