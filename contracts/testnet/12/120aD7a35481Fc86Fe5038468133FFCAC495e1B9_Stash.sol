...
contract Stash is IERC20, Ownable {
  ...
  uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 325 * 10**3 * 10**_decimals;
  ...
  uint256 public totalFee = liquidityFee.add(treasuryFee).add(stashInsuranceFundFee).add(firePitFee);
  ...
  function transferFrom(
    ...
    if (_allowedFragments[from][msg.sender] != MAX_UINT256) {
      _allowedFragments[from][msg.sender] = _allowedFragments[from][msg.sender].sub(value, "INSUFFICIENT_ALLOWANCE");
    }
