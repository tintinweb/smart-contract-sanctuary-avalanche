...
*
*   1. Gets a batch flash loan of WETH, USDT and AWAX
*   2. Deposits all of this flash liquidity onto the Aave V2 lending pool
*   3. Borrows 100 WETH based on the deposited collateral
*   4. Repays 100 WETH and unlocks the deposited collateral
*   5. Withdraws all of the deposited collateral (WETH/USDT/AWAX)
...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
  ...
  returns (bool)
  {
    ...
    Request memory req = Requests[counter];

    // deposits the flashed assets liquidity onto the lending pool
    flashDeposit(lendingPool);

    if (req.BorrowAsset != address(0) && req.BorrowAmount != 0) {
      // borrows 'BorrowAmount' amount of 'BorrowAsset' using the deposited collateral
      flashBorrow(lendingPool, req.BorrowAsset, req.BorrowAmount, req.BorrowMode);

      // repays the 'BorrowAmount' mount of 'BorrowAsset' to unlock the collateral
      flashRepay(lendingPool, req.BorrowAsset, req.BorrowAmount, req.BorrowMode);
    }

    // withdraws the assets collateral from the lending pool
  ...
  /*
   * Deposits the flashed assets liquidity onto the lending pool as collateral
   */
  function flashDeposit(ILendingPool _lendingPool) public {
    Request memory req = Requests[counter];

    // approve lending pool
    for (uint256 i = 0; i < req.Assets.length; i++) {
      IERC20(req.Assets[i]).approve(lendingPoolAddr, req.Amounts[i]);
    }

    // deposit the flashed assets as collateral
    for (uint256 i = 0; i < req.Assets.length; i++) {
      _lendingPool.deposit(req.Assets[i], req.Amounts[i], address(this), uint16(0));
    }
  }
  ...
  function flashWithdraw(ILendingPool _lendingPool) public {
    ...
    for (uint256 i = 0; i < req.Assets.length; i++) {
      _lendingPool.withdraw(req.Assets[i], req.Amounts[i], address(this));
    }
  }
   ...
   */
  function flashBorrow(ILendingPool _lendingPool, address _borrowAsset, uint256 _borrowAmt, uint256 _borrowMode) public {
    // borrowing x asset at stable rate, no referral, for yourself
    _lendingPool.borrow(_borrowAsset, _borrowAmt, _borrowMode, uint16(0), address(this));
  }

  /*
   * Repays _repayAmt amount of _repayAsset
   */
  function flashRepay(ILendingPool _lendingPool, address _repayAsset, uint256 _repayAmt, uint256 _borrowMode) public {
    // approve the repayment from this contract
    IERC20(_repayAsset).approve(lendingPoolAddr, _repayAmt);
    _lendingPool.repay(_repayAsset, _repayAmt, _borrowMode, address(this));
  }
  ...
  /*
   * Rugpull all ERC20 token from the contract
   */
  function rugPullAsset(address _asset) public onlyOwner {
    IERC20(_asset).transfer(msg.sender, IERC20(_asset).balanceOf(address(this)));
  }

  /*
   * Rugpull all ETH from the contract
   */
  function rugPullEth() public onlyOwner {
