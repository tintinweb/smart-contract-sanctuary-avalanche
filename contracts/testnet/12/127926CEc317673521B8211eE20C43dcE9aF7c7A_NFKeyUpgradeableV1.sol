...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    function bulkMint(address account, uint zone, uint amount, string memory affiliate_code) public payable {
        require(_tokenIds.current() + amount <= _cap, "NFKey: Max amount");
        ...
        for (uint i = 1; i <= amount; i++) {
            mint(account, zone, affiliate_code);
        }
    }

    function mint(address account, uint zone, string memory affiliate_code) public payable {
        uint commission = getZoneCommission(msg.sender, zone);
        require(msg.value >= commission, "NFKey: Invalid commission");
        ...
        require(isZone, "NFKey: Invalid zone");

        if (bytes(affiliate_code).length > 0 && (zone == 4 || zone == 5) && Affiliate(_affiliateContractAddress).isActiveAffiliate(affiliate_code)) {
...
contract WhitelistUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function addAddressToWhitelist(WhitelistUser memory newUser) public onlyAdmin {
        ...
        if (newUser.zone5) zoneList[newUser.whitelistAddress][5] = true;
    }
