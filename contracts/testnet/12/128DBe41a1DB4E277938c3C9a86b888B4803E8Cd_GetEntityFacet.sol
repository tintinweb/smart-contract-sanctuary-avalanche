...
contract GetEntityFacet is IGetEntityFacet {
	...
	function isLevelSet(
		...
		uint64 timestamp
	) external view isExactDate(timestamp) isValidBlockTimestamp(timestamp) returns (bool) {
	...
	function getLevel(
		bytes32 configurationId,
		uint64 timestamp
	) external view isExactDate(timestamp) isValidBlockTimestamp(timestamp) returns (int128) {
