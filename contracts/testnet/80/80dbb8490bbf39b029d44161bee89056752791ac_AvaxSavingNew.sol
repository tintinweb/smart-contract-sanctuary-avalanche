...
contract AvaxSavingNew is ReentrancyGuard, Context {
    ...
    function claimReward() public nonReentrant {
        ...
        uint256 unStakedAtAmountFinal = getUserUnStakedAtAmount(msg.sender);

        totalAmount = totalAmount.add(unStakedAtAmountFinal);
        /********/
        for (uint256 i = 0; i < user.deposits.length; i++) {
            //Only active deposits
            if (user.checkpoint < user.deposits[i].finish) {
                user.deposits[i].unStakedAtAmount == 0;
            }
        }
        /********/
        if (stakeCommision > 0) {
        ...
        returns (Deposit[] memory)
    {
        ...
        if (block.timestamp < user.deposits[investmentIndex].finish) {
            uint256 feeAmount;
            uint256 finalAmount;
            feeAmount = user.deposits[investmentIndex].amount.mul(200).div(
                PERCENTS_DIVIDER
            );
            finalAmount = user.deposits[investmentIndex].amount - feeAmount;
            user.deposits[investmentIndex].status = false;

            // user.deposits[investmentIndex].claimedStatus = true;
            // user.deposits[investmentIndex].depositState = 1;
            _donator.transfer(feeAmount);
        ...
        user.deposits[investmentIndex].unStakedAtAmount = user
            .deposits[investmentIndex]
            .unStakedAtAmount
            .add(getUserStakeReward(msg.sender));
    ...
    function newUnStake2(uint256 investmentIndex)
        public
        nonReentrant
        returns (Deposit[] memory)
    {
        ...
        if (block.timestamp < user.deposits[investmentIndex].finish) {
            uint256 fee;
            uint256 finalAmount;
            fee = user.deposits[investmentIndex].amount.mul(200).div(
                PERCENTS_DIVIDER
            );
            finalAmount = user.deposits[investmentIndex].amount - fee;
            _donator.transfer(fee);
            user.deposits[investmentIndex].status = false;
            payable(msg.sender).transfer(finalAmount);
            emit UnStaked(
        ...
        )
    {
        ...
        total = myStakingRewards.add(myAffliateStakeComission).add(diff).add(
            myProofShareReward
        );
        total = total.add(getUserUnStakedAtAmount(userAddress));
    }
