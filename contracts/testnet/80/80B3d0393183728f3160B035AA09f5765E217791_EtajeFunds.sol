...
error EtajeFunds__TransferFailed();

contract EtajeFunds is BaseRelayRecipient {
    string data;
     ...
     *James Lucky was here 2022/12/06
     **/
    constructor(address _trustedForwarder) {
        _setTrustedForwarder(_trustedForwarder);
        data = "Hello Metatransaction";
        owner = msg.sender;
        s_stakingToken = IERC20(0x4fC4Fd531dA58966b5fE2f7e18605020f4F7b6Ed);
    }
    ...
    function depositFunds(uint256 amount) external
    {
        ...
        s_stakingToken.approve(address(this), amount);
        bool success = s_stakingToken.transferFrom(msg.sender, address(this), amount);
        tokenOwned[msg.sender] += amount;
        if (!success) {
            revert EtajeFunds__TransferFailed();
        }

    }
     function sendOutFunds(uint256 amount, address receiver) external {
        // s_totalBUSD -= amount;
        require(tokenOwned[msg.sender] >= amount, "Insufficient USDT balance");
        tokenOwned[msg.sender] -= amount;
        bool success = s_stakingToken.transfer(receiver, amount);
        if (!success) {
            revert EtajeFunds__TransferFailed();
        }
    }
