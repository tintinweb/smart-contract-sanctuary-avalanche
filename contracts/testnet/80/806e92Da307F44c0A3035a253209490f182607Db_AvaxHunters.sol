...
contract AvaxHunters is ERC721Enumerable, Whitelister {
  ...
  modifier onlyState(uint8 flags) {
    require(states & flags > 0, "State invalid");
    _;
  }
  ...
  function mintWithTicket() external payable onlyState(3) {
  ...
  function mint(uint256 amount) internal {
    ...
    for (uint256 i = 1; i <= amount; i++) {
      uint256 tokenId = mintTicker + i;
      data().registerHunter(tokenId, seed(tokenId));
      _mint(to, tokenId);
    }
    mintTicker += amount;
  }
  ...
  function useBreed(
    ...
    uint256 breedId,
    uint256 breedHash
  ) external withinWhitelist onlyState(4) {
    if (breedId > 0) {
  ...
  function useHunter(uint256 hunterId, uint32 hunterRate) external withinWhitelist {
    data().setRate(hunterId, hunterRate);
  }

  function useHunt(address account, uint256[] calldata hunters) external withinWhitelist returns (uint256 energy) {
    for (uint16 i = 0; i < hunters.length; i++) {
