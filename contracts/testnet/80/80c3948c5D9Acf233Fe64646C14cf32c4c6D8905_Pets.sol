...
contract Pets is ERC721Enumerable, Pausable, Restricted {
    ...
    * @param petToken The id of the pet you'd like to associate to the wallet
    ...
    function dissociateManyFromPets(uint256[] calldata petTokens)
        ...
        notBlacklisted
    {
        for (uint8 i = 0; i < petTokens.length; i++) {
            require(ownerOf(petTokens[i]) == _msgSender(), "This pet isn't yours");
    ...
    function dissociateManyFromRangers(uint256[] calldata rangerTokens)
        ...
        notBlacklisted
    {
        for (uint8 i = 0; i < rangerTokens.length; i++) {
            require(skeleton.ownerOf(rangerTokens[i]) == _msgSender(), "This ranger isn't yours");
