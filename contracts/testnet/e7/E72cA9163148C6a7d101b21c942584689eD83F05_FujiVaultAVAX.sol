...
// File contracts/interfaces/IVaultControl.sol
...
// File contracts/abstracts/vault/VaultBaseUpgradeable.sol
...
interface IVault {
  ...
  */
  ...
  function depositOnBehalf(address user, uint256 _collateralAmount) external payable;
  ...
  function paybackOnBehalf(address user, int256 _repayAmount) external payable;
...
// File contracts/interfaces/IFujiERC1155.sol
...
// File contracts/interfaces/IProvider.sol
...
// File contracts/fuji/libraries/LibUniversalERC20UpgradeableAVAX.sol
...
// File contracts/fuji/FujiVaultAVAX.sol
...
contract FujiVaultAVAX is VaultBaseUpgradeable, ReentrancyGuardUpgradeable, IVault {
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using LibUniversalERC20UpgradeableAVAX for IERC20Upgradeable;
  ...
  */
  ...
  function depositAndBorrow(uint256 _collateralAmount, uint256 _borrowAmount) external payable {
    updateF1155Balances();
    _internalDeposit(msg.sender, _collateralAmount);
    _internalBorrow(msg.sender, _borrowAmount);
  }
  ...
  function paybackAndWithdraw(int256 _paybackAmount, int256 _collateralAmount) external payable {
    updateF1155Balances();
    _internalPayback(msg.sender, _paybackAmount);
    _internalWithdraw(msg.sender, _collateralAmount);
  }
  ...
  function depositOnBehalf(address user, uint256 _collateralAmount) public payable override {
    updateF1155Balances();
    _internalDeposit(user, _collateralAmount);
  }
  ...
  function withdraw(int256 _withdrawAmount) public override nonReentrant {
    updateF1155Balances();
    _internalWithdraw(msg.sender, _withdrawAmount);
  }
  ...
  function payback(int256 _repayAmount) public payable override {
    updateF1155Balances();
    _internalPayback(msg.sender, _repayAmount);
  }

  function paybackOnBehalf(address user, int256 _repayAmount) public payable override {
    updateF1155Balances();
    _internalPayback(user, _repayAmount);
  }
  /**
  ...
  * See {deposit}
  */
  function _internalDeposit(address _user, uint256 _collateralAmount) internal {
    if (vAssets.collateralAsset == AVAX) {
    ...
    IFujiERC1155(fujiERC1155).mint(_user, vAssets.collateralID, _collateralAmount);

    emit Deposit(_user, vAssets.collateralAsset, _collateralAmount);
  }
  ...
  * See {withdraw}
  */
  function _internalWithdraw(address _user, int256 _withdrawAmount) internal {
    ...
    uint256 neededCollateral = getNeededCollateralFor(
      IFujiERC1155(fujiERC1155).balanceOf(_user, vAssets.borrowID),
      true
    );
    ...
    IFujiERC1155(fujiERC1155).burn(_user, vAssets.collateralID, amountToWithdraw);

    // Delegate Call Withdraw to current provider
    _withdraw(amountToWithdraw, address(activeProvider));

    // Transer Assets to User
    IERC20Upgradeable(vAssets.collateralAsset).univTransfer(payable(_user), amountToWithdraw);

    emit Withdraw(_user, vAssets.collateralAsset, amountToWithdraw);
  }
  ...
  * See {borrow}
  */
  function _internalBorrow(address _user, uint256 _borrowAmount) internal {
    uint256 providedCollateral = IFujiERC1155(fujiERC1155).balanceOf(
      _user,
      vAssets.collateralID
    );
    ...
    uint256 userFee = (debtPrincipal *
      (block.timestamp - _userFeeTimestamps[_user]) *
      protocolFee.a) /
      protocolFee.b /
      ONE_YEAR;

    _userFeeTimestamps[_user] =
      block.timestamp -
    ...
    // Debt Management
    IFujiERC1155(fujiERC1155).mint(_user, vAssets.borrowID, _borrowAmount);

    // Delegate Call Borrow to current provider
    _borrow(_borrowAmount, address(activeProvider));

    // Transer Assets to User
    IERC20Upgradeable(vAssets.borrowAsset).univTransfer(payable(_user), _borrowAmount);

    emit Borrow(_user, vAssets.borrowAsset, _borrowAmount);
  }
  ...
  * See {payback}
  */
  function _internalPayback(address _user, int256 _repayAmount) internal {
    uint256 debtBalance = IFujiERC1155(fujiERC1155).balanceOf(_user, vAssets.borrowID);
    uint256 userFee = _userProtocolFee(_user, debtBalance);
    ...
    // Debt Management
    IFujiERC1155(fujiERC1155).burn(_user, vAssets.borrowID, amountToPayback - userFee);

    // Update protocol fees
    _userFeeTimestamps[_user] = block.timestamp;
    remainingProtocolFee += userFee;

    emit Payback(_user, vAssets.borrowAsset, debtBalance);
  }
}