...
import "./lib/StabilityFee.sol";

/**
 * @title ERC20 Token of ValiFi, with all inherited features from Open Zeppelin's Library, and custom features
 * @dev All Method to permit handle the Vesting Process of ValiFi token
 * @custom:a Alfredo Lopez / Marketingcycle / ValiFi
 */
contract ValiFiToken is Initializable, StabilityFee {
    using AddressUpgradeable for address;
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    // Max Total Supply 5 million
    uint256 private constant MAXTOTALSUPPLY =
        5_000_000 * (uint256(10)**uint256(18));

    function initialize() public initializer {
        __ERC20_init("ValiFi Token", "VALI");
        __Ownable_init();
        __Pausable_init_unchained();
        __ERC20Permit_init("ValiFi Token");

        //mint all Token
        mint(MAXTOTALSUPPLY);

        // Allocation of Token ERC20, based on Whitepaper
        // Allocation #1 / VestingType #0, Presale 1 / Private Sale (1%), Unlock 50% in the TGE and another 50% after 1 month
        vestingTypes.push(VestingType(50e18, 50e18, 30 days, 0, true)); // Unlock 50% in the TGE and another 50% after 1 month
        // Allocation #2 / VestingType # 1, IDO OR Listing Public , (4%) Unlocked the all Token immediatly deploy the Liquidity Pool of Token
        vestingTypes.push(VestingType(0, 100e18, 0, 0, true)); // 0 Days 100 Percent Unloked
        // Allocation #3 / VestingType # 2, REWARDS of Project (70%), Unlock (4,16)% in the TGE and another (3.91304347826087)% for 23 months
        vestingTypes.push(VestingType(39130434782608700, 10e18, 30 days, 0, true)); // Unlock (3.91304347826087)% in the TGE and another (3.91304347826087)% for 23 months
        // Allocation #4 / VestingType # 3, Liquidity (10%), Unlock (10)% after 60 months
        vestingTypes.push(VestingType(100e18, 0, 1800 days, 0, true)); // Unlock (10)% after of 60 months
        // Allocation #5 / VestingType # 3, Reserve of Project (15%), lock for 3 month and Start to Unlock (8,3)% for 12 months
        vestingTypes.push(VestingType(83333333333333333, 0, 90 days, 0, true)); // lock for 3 month and Start to Unlock (8,3)% for 12 months
    }
    ...
    /**
     * @dev Circulating Supply Method for Calculated based on Wallets of VALIFi Foundation
     */
    function circulatingSupply() public view returns (uint256 result) {
        result = totalSupply().sub(balanceOf(owner()));
        for (uint256 i = 0; i < valifiWallets.length; i++) {
            if ((valifiWallets[i] != address(0)) && (result != 0)) {
                result -= balanceOf(valifiWallets[i]);
            }
        }
     ...
     * @param sender Address of Sender of the token
     ...
     * @param amount Amount Token to mint
     *
     * See {ERC20-_mint}.
     *
     * Requirements:
    ...
    function _beforeTokenTransfer(
        ...
        address recipient,
        uint256 amount
    ) internal virtual override notBlacklisted(sender) {
        require(
            !isBlacklisted(recipient),
            "ERC20 VALIFi: recipient account is blacklisted"
        );
        // Permit the Owner execute token transfer/mint/burn while paused contract
        if (_msgSender() != owner()) {
            require(
                !paused(),
                "ERC20 VALIFi: token transfer/mint/burn while paused"
            );
        }
        require(
            canTransfer(sender, amount),
            "ERC20 VALIFi: Wait for vesting day!"
        );
    ...
    function _transfer(
        ...
        if (isTransferDisabled()) {
            ...
            if (
                ...
                (sender != owner()) &&
                (recipient != owner()) &&
                (!isValiFiWallet(sender)) &&
                (!isValiFiWallet(recipient))
            ) {
                ...
                super._transfer(
                    sender,
                    recipient,
                    getRestAmountOfFee(sender, amount)
                );
            } else if (isWinner(sender)) {
                // Add sender in list of Winners in any events of ValiFi Protocol
                if (walletWinners[sender] >= amount) {
                    walletWinners[sender] = walletWinners[sender].sub(amount);
                    super._transfer(sender, recipient, amount);
                } else {
                    super._transfer(sender, recipient, walletWinners[sender]);
                    super._transfer(
                        sender,
                        recipient,
                        getRestAmountOfFee(
                            sender,
                            (amount - walletWinners[sender])
                        )
                    );
                }
            } else {
...
contract StabilityFee is Vesting {
    // Mapping of Wallet to Trade with ValiFi Token
    // Address Wallet / uint256 Timestamp
    mapping(address => uint256) internal walletTraded;
    // Address Wallet / uint256 Amount of ValiFi Token permit to trade
    mapping(address => uint256) internal walletWinners;
     ...
     * @return bool
     */
    function isTrader(address wallet) public view returns (bool) {
        return walletTraded[wallet] != 0;
    }
    ...
    function isWinner(address wallet) public view returns (bool) {
        return walletWinners[wallet] != 0;
    }

    /**
     * @dev Allows to add a Trader to the list of Traders
     * @param wallet Address of the User
     * @param amount uint256 Amount of CAN Token permit to trade
     * @return bool
     */
    function addWinner(address wallet, uint256 amount) public onlyOwner() returns (bool) {
        if (!isTrader(wallet)) {
            walletTraded[wallet] = getTimestamp();
        }
        walletWinners[wallet] += amount;
        return true;
    }

    function getRestAmountOfFee(address wallet, uint256 amount)
        internal
        returns (uint256)
    {
        uint256 time = getTimestamp();
        uint256 timeWallet = walletTraded[wallet];
        if (timeWallet == 0) {
            // Add wallet i register of wallet that traded with ValiFI Token
            walletTraded[wallet] = time;
            // Transfer 65% amount of Reward Holders Pool of ValiFI Token
            transfer(valifiWallets[1], mulDiv(amount, 65e16, 1e18));
            // Burn 15% amount of CAN Token to Tranfer to the Wallet
            _burn(msg.sender, mulDiv(amount, 15e16, 1e18));
            return mulDiv(amount, 20e16, 1e18);
            ...
            // Transfer 5% amount of Reward Holders Pool of ValiFI Token
            transfer(valifiWallets[1], mulDiv(amount, 5e16, 1e18));
            // Burn 5% amount of CAN Token to Tranfer to the Wallet
            _burn(msg.sender, mulDiv(amount, 5e16, 1e18));
            return mulDiv(amount, 90e16, 1e18);
        }
    }
}
    ...
    ERC20PermitUpgradeable
{
    ...
    // MApping of FrozenWallets
    mapping(address => bool) public frozenWalletMap;
    ...
    function getReleaseTime() public pure returns (uint256) {
        return 1655298000; // "Thursday, 21 April 2022 12:00:00 GMT"
    }
    ...
    function addAllocations(
        ...
        for (uint256 i = 0; i < addressesLength; i++) {
            ...
            require(
                !isBlacklisted(_address),
                "ERC20 VALIFi: recipient account is blacklisted"
            );
            require(
                totalAmounts[i] != 0,
                "ERC20 VALIFi: total amount token is zero"
            );
            total = total.add(totalAmounts[i]);
        }

        if (total > balanceOf(msg.sender)) {
            revert("ERC20 VALIFi: insufficient funds");
        }
    ...
    function addFrozenWallet(
        ...
        // Add wallet to frozen wallets map
        frozenWalletMap[wallet] = true;
        ...
        returns (uint256 months)
    {
        ...
        months = diff.div(30 days).add(1).sub(monthDelay);
        ...
        returns (uint256 transferableAmount)
    {
        ...
        if (
            ...
            (frozenWallets[sender].monthlyAmount !=
                frozenWallets[sender].totalAmount)
        ) {
        ...
        uint256 monthlyTransferableAmount = frozenWallets[sender]
            .monthlyAmount
            .mul(months);
        transferableAmount = monthlyTransferableAmount.add(
     ...
     */
    function getRestAmount(address sender)
        public
        view
 ...
 * @dev Allows update the wallets of VALIFi Foundation by Owner
 * @custom:a Alfredo Lopez / Marketingcycle / ValiFi
 */
contract CirculatingSupply is OwnableUpgradeable, Antibots {
    /**
	 * @dev Array of Wallets of ValiFi Foundation
	 ...
	 */
    address[] internal valifiWallets;

    event InValiFiWallet(address indexed _account);
    event OutValiFiWallet(address indexed _account);

    /**
     * @dev function to verify if the address exist in ValiFiWallet or not
     * @param _account The address to check
     */
    function isValiFiWallet(address _account) public view returns (bool) {
        if (_account == address(0)) {
            return false;
        }
        uint256 index = valifiWallets.length;
        for (uint256 i = 0; i < index; i++) {
            if (_account == valifiWallets[i]) {
                return true;
            }
        }
        return false;
    ...
    /**
     * @dev Include the wallet in the wallets address of VALIFi Foundation Wallets
     * @param _account The address to include
     */
    function addValiFiWallet(address _account)
        public
        validAddress(_account)
        onlyOwner
        returns (bool)
    {
        require(!isValiFiWallet(_account), "ERC20 VALIFi: wallet is already");
        valifiWallets.push(_account);
        emit InValiFiWallet(_account);
        return true;
    }

    /**
     * @dev Exclude the wallet in the wallets address of VALIFi Foundation Wallets
     * @param _account The address to exclude
     */
    function dropValiFiWallet(address _account)
        public
        validAddress(_account)
        onlyOwner
        returns (bool)
    {
        require(isValiFiWallet(_account), "ERC20 VALIFi: Wallet don't exist");
        uint256 index = valifiWallets.length;
        for (uint256 i = 0; i < index; i++) {
            if (_account == valifiWallets[i]) {
                valifiWallets[i] = valifiWallets[index - 1];
                valifiWallets.pop();
                emit OutValiFiWallet(_account);
                return true;
            }
        }
        return false;
    ...
    /**
     * @dev Getting the all wallets address of VALIFi Foundation Wallets
     */
    function getValiFiWallets() public view returns (address[] memory) {
        return valifiWallets;
    }

	/**
     * @dev Getting the all wallets address of VALIFi Foundation Wallets
     */
    function getValiFiWallet(uint256 index) public view returns (address) {
        return valifiWallets[index];
    }
}
...
contract Antibots is OwnableUpgradeable, Blacklistable {
    ...
    function isTransferDisabled() internal view returns (bool) {
        ...
        return (!burnBeforeBlockNumberDisabled &&
            (block.number < burnBeforeBlockNumber));
    }
    ...
    function disableBurnBeforeBlockNumber() public onlyOwner {
        burnBeforeBlockNumber = uint256(0);
        burnBeforeBlockNumberDisabled = true;
        emit DisableDefenseAntiBots(
            block.number,
     ...
     */
    function getBurnBeforeBlockNumber()
        public
        view
        onlyOwner
        returns (uint256)
     ...
     * @param _account The address to include
     */
    function addWhiteListed(address _account)
        public
        validAddress(_account)
        onlyOwner
        returns (bool)
    {
        require(!isWhiteListed(_account), "ERC20 VALIFi: wallet is already");
        whitelisted[_account] = true;
				whitelistedWallets.push(_account);
     ...
     * @param _account The address to exclude
     */
    function dropWhiteListed(address _account)
        public
        validAddress(_account)
        onlyOwner
        returns (bool)
    {
        require(isWhiteListed(_account), "ERC20 VALIFi: Wallet don't exist");
				whitelisted[_account] = false;
        uint256 index = whitelistedWallets.length;
        for (uint256 i = 0; i < index; i++) {
            if (_account == whitelistedWallets[i]) {
                whitelistedWallets[i] = whitelistedWallets[index - 1];
...
contract Blacklistable is OwnableUpgradeable {
    // Index Address
    address[] private blacklistedWallets;
    // Mapping blacklisted Address
    mapping(address => bool) private blacklisted;
    // Events when add or drop a blacklistedWallets in the blacklisted mapping
    ...
    modifier validAddress(address to) {
        require(to != address(0), "ERC20 VALIFi: Not Add Zero Address");
        /* solcov ignore next */
        _;
    }
        ...
        onlyOwner
    {
        ...
        blacklistedWallets.push(_account);
				...
				returns (bool)
    {
        ...
        blacklisted[_account] = false;
				uint256 index = blacklistedWallets.length;
        for (uint256 i = 0; i < index; i++) {
            if (_account == blacklistedWallets[i]) {
                blacklistedWallets[i] = blacklistedWallets[index - 1];
