...
// Reentrant guard
   ...
   // disable all functions with the modifier disable, this can not be undone
    ...
    function addNativeToken(address _address, bool value) public onlyOwner {
    nativeTokens[_address] = value;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public isHuman nonReentrant {
        ...
        updatePool(_pid);
        if (nativeTokens[address(pool.token)] == false) {
    ...
    function setCakeTokenFee(uint256 cakeFee) public onlyOwner {
    require(cakeFee <= 10000 || cakeFee > 7999, "Max fee 20%");
    cakeTokenFee = cakeFee;
    }
