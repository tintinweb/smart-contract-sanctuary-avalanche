    ...
    ERC1155BurnableUpgradeable
{
    ...
    mapping(uint256 => uint256) public _dropCount;
    mapping(uint256 => uint256) public _rdropCount;
    ...
    //     );
    //     require(_subscribeNFT[ids[0]].subPrice == msg.value,"Fee mismatch");
    //     if(_dropCount[ids[0]] <= _rdropCount[ids[0]]){
    //          _saleToken(from, ids, bidtoken);
    //     }
    //     else{
    //         _rdropCount[ids[0]] = _rdropCount[ids[0]].add(1);
    //     }
    //     payable(owner()).transfer(msg.value);
    //     _subscription(ids[0],msg.sender);
    ...
    // ids[0] - tokenId, ids[1] - amount, ids[2] -  nooftoken
    function saleToken(string memory bidtoken,
    address payable from,
     uint256[] memory ids)
        public
        payable
    {
        ...
        if(_dropCount[ids[0]] <= _rdropCount[ids[0]]){
            require(order_place[from][ids[0]].price == ids[1],"Order Mismatch");
            _saleToken(from, ids, "coin");
            // payable(owner()).transfer(_subscribeNFT[ids[0]].subPrice);
        }
        else{
            if(from == owner()){
                _rdropCount[ids[0]] = _rdropCount[ids[0]].add(1);
        ...
        if (_unblockBalances[ids[0]][msg.sender] == 0) {
            ...
            _subUser.tokenId = ids[0];
            _subUser.subPrice = _subscribeNFT[ids[0]].subPrice;
            _subUser.subStartPeriod = block.timestamp;
            _subUser.subEndPeriod = block.timestamp.add(_subscribeNFT[ids[0]].subPeriod.mul(1 days));
            userSubscrptiondata[msg.sender][ids[0]] = _subUser;
        }
        // else{
    ...
    function acceptBId(
        ...
        if(_dropCount[ids[0]] <= _rdropCount[ids[0]]){
                require(order_place[msg.sender][ids[0]].price == ids[1],"Order Mismatch");
    ...
    function _userSubscrptiondata(uint256 tokenId) internal {
        ...
        _subscription(tokenId,msg.sender);
    ...
    function _subscription(uint256 tokenId, address from) internal {
         userSubscrptiondata[from][tokenId].tokenId = tokenId;
         userSubscrptiondata[from][tokenId].subStartPeriod = block.timestamp;
         userSubscrptiondata[from][tokenId].subEndPeriod = block.timestamp.add(
            _subscribeNFT[tokenId].subPeriod.mul(1 days)
        );
    }
