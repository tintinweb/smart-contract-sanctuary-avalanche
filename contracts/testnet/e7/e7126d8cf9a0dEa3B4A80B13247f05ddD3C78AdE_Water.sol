...
contract Water is IWater, ERC4626, Ownable, Pausable, Constant {
    ...
    uint128 private depositFeeBPS; 
    uint128 private withdrawFeeBPS;
    ...
    function repayDebt(uint256 _amount) external onlyBartenderOrLiquor {
        if (_amount > _totalDebt) _totalDebt = 0;
        else {
            _totalDebt -= _amount;
        }
    ...
    function previewWithdraw(uint256 _assets) public view override returns (uint256) {
        uint256 fees = _assets.mulDiv(depositFeeBPS, MAX_BPS);
    ...
    function previewRedeem(uint256 _shares) public view override returns (uint256) {
        uint256 fees = _shares.mulDiv(withdrawFeeBPS, MAX_BPS);
    ...
    function _transferRecipientFee(bytes32 _params, uint256 _assets) internal returns (uint256) {
        ...
        if (_params == "deposit") {
            if (depositFeeBPS != 0) {
                fees = _assets.mulDiv(depositFeeBPS, MAX_BPS);
        ...
        if (_params == "withdraw") {
            if (withdrawFeeBPS != 0) {
                uint256 shares = _convertToShares(withdrawFeeBPS, Math.Rounding.Up);
