...
contract Forwarder is OwnableUpgradeable, IForwarder {
    ...
    function updateWhitelisted(address sender, bool isWhitelisted) external onlyOwner {
        whitelisted[sender] = isWhitelisted;
    }
    ...
    function execute(
        ...
        bytes calldata suffixData,
        bytes calldata sig
    ) external payable override onlyWhitelisted returns (bool success, bytes memory ret) {
        verify(req, domainSeparator, requestTypeHash, suffixData, sig);
        _updateNonce(req);
