    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if(OverFlowPlayerStatus[playerId] == true){
            if(_currentUserStage  > 0){
                ...
                bytes32[] memory SelectSuccessSidePlayerId; 
                if(totalNFTOnTile[_currentUserStage - 1][previousJumpSide] > 0){
                    ...
                    for (uint i = 0; i < PlayerIDAtSpecifyStage.length; i++) {
                        if(PlayerItem[PlayerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide) {
                            ...
                            // SelectSuccessSidePlayerId[i] == PlayerIDAtSpecifyStage[i];
                        }
                    }
                    emit Comment2(SelectSuccessSidePlayerId);

                    // for (uint256 i = 0; i < PlayerIDAtSpecifyStage.length; i++) {
                    //     if(PlayerItem[PlayerIDAtSpecifyStage[i]].ishold == false){
                    //         SelectPlayerId[i] = PlayerIDAtSpecifyStage[i];
                    //     }
                    // }
                    // emit Comment2(SelectPlayerId);
                //     playerIndex = generateRandomTicketNumbers(SelectPlayerId.length - 1,SelectPlayerId.length - 1,totalNFTOnTile[_member.stage][previousJumpSide]);
                }
                // emit Comment2(SelectPlayerId);
