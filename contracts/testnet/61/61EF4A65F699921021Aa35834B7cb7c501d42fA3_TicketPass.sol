  ...
  MixinPurchase
{
  ...
  /**
   * @dev Update the key status to checkedIn.
  ...
  /**
   * @dev check the key for the given keyOwner.
   * @notice this deletes ownership info and expire the key, but doesnt 'burn' it
   */
  function _checkIn(uint _tokenId) internal {
    ...
    // emit event
    emit CheckIn(_tokenId, keyOwner, msg.sender, _keys[_tokenId].expirationTimestamp);
