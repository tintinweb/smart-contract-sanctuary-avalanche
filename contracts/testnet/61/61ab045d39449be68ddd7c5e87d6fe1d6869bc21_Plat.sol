...
contract Plat is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the PLAT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("PLAT", "PLAT") {
        // Mints 1 PLAT to contract creator for initial pool setup
    ...
    function _getPlatPrice() internal view returns (uint256 _platPrice) {
        try IOracle(platOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Plat: failed to fetch PLAT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _platPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_platPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPlatOracle(address _platOracle) public onlyOperatorOrTaxOffice {
        require(_platOracle != address(0), "oracle address cannot be 0 address");
        platOracle = _platOracle;
    }
    ...
    /**
     * @notice Operator mints PLAT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PLAT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPlatPrice = _getPlatPrice();
            currentTaxRate = _updateTaxRate(currentPlatPrice);
            if (currentPlatPrice < burnThreshold) {
                burnTax = true;
            }
        }
