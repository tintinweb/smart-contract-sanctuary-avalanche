...
contract Liquifier is Ownable {
    ...
    function execute() public {
        ...
        if (tokenBalance > 0) {
            (address token0,) = sortTokens(IRouter(router).WAVAX(), address(token));
            (uint112 reserve0, uint112 reserve1, ) = pair.getReserves();
            (uint112 reserveA, uint112 reserveB) = IRouter(router).WAVAX() == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
