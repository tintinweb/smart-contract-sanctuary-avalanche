...
// Payees:
// ["0x6Dc984c9bEd938F139CF4C89709c732EF34B3048", "0x9626C9F9d3d6ed3932A13A3AD9EFa593Dc7523c8"]
// shares:
// [30,70]
// addresses:
// ["0x6Dc984c9bEd938F139CF4C89709c732EF34B3048","0xdFB4ee2A067a0D2a6719df23e54ec13B094B66c2","0xFc411B82C6c96E7d4D1938cfC438D0e0cFCFcdB5","0x42a76E053375DEe98CC20664071eE0a51C6cd708","0x262157114E6897C8ebC2Ce8333613d600d479d40"]
// balances:
...
contract Marvel is ERC20, Ownable, PaymentSplitter {
    ...
    constructor(
        ...
        address[5] memory addresses= [0x6Dc984c9bEd938F139CF4C89709c732EF34B3048,0xdFB4ee2A067a0D2a6719df23e54ec13B094B66c2,0xFc411B82C6c96E7d4D1938cfC438D0e0cFCFcdB5,0x42a76E053375DEe98CC20664071eE0a51C6cd708,0x262157114E6897C8ebC2Ce8333613d600d479d40];
        uint256[5] memory balances = [uint256(5000000), 5000000, 5000000, 5000000, 5000000];
        uint256[5] memory fees = [uint256(2),60,10,10,2];
        uint256 swapAmount =30000000000000000000;
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        //     blocktime
        // );
        uint256 rewardAmount = rewardManager._cashoutNodeReward(sender, blocktime);
        require(
            rewardAmount > 0,
    ...
    function cashoutNodeTypeReward(uint256 typeNum) public {
        address sender = _msgSender();
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        require(
        ...
        // uint256 rewardAmount = rewardManager._getNodeTypeRewardAmountOf(
        //     sender,
        //     typeNum
        // );
        uint256 rewardAmount = rewardManager._cashoutNodeTypeReward(sender, typeNum);
        require(
            rewardAmount > 0,
            "CSHT: You don't have enough reward to cash out"
        );
        ...
        super._transfer(distributionPool, sender, rewardAmount);
        // rewardManager._cashoutNodeTypeReward(sender, typeNum);
    }
    ...
    function cashoutAll() public {
        ...
        uint256 rewardAmount = rewardManager._cashoutAllNodesReward(sender);
        require(
            rewardAmount > 0,
    ...
    function payAll() payable public {
        address sender = _msgSender();
        require(msg.value > 0, "Pay: No Fund");
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        require(
            sender != futurUsePool && sender != distributionPool,
        ...
        uint256 remainedEther = rewardManager._payAllNodesFee(sender, msg.value);
        payable(sender).transfer(remainedEther);
    }

    function payNodeTypeFee(uint256 typeNum) payable public {
        address sender = _msgSender();
        require(msg.value > 0, "Pay: No Fund");
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        ...
        uint256 remainedEther = rewardManager._payNodeTypeFees(sender, typeNum, msg.value);
        payable(sender).transfer(remainedEther);
    }

    function payNodeFee(uint256 _creationTime) payable public {
        address sender = _msgSender();
        require(msg.value > 0, "Pay: No Fund");
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        ...
        uint256 remainedEther = rewardManager._payNodeFee(sender, _creationTime, msg.value);
        payable(sender).transfer(remainedEther);
    }
    ...
    function getRewardAmount(address account) public returns (uint256) {
        require(account != address(0), "SENDER CAN'T BE ZERO");
        require(rewardManager._isNodeOwner(account), "NO NODE OWNER");
        return rewardManager._getRewardAmountOf(account);
    }

    // Get Node price
    function getNodeType(uint256 typeNum) public view returns (uint256 nodePrice, uint256 rewardPerDay, uint256 totalRewardReleased, uint256 fee) {
        return rewardManager.getNodeType(typeNum);
    }

    function addNewNodeType(uint256 typeNum, uint256 nodePrice, uint256 rewardPerDay, uint256 fee) public onlyOwner {
        rewardManager.addNewNodeType(typeNum, nodePrice, rewardPerDay, fee);
    }
    ...
    function changeNodeFee(uint256 typeNum, uint256 fee) public onlyOwner {
        rewardManager.changeNodeFee(typeNum, fee);
    }
    ...
    function getNodesInfo(address account) public view returns (string memory) {
        require(account != address(0), "SENDER CAN'T BE ZERO");
        require(rewardManager._isNodeOwner(account), "NO NODE OWNER");
        return rewardManager._getNodesInfo(account);
    }

    function getNodesCreatime(address account) public view returns (string memory) {
        require(account != address(0), "SENDER CAN'T BE ZERO");
        require(rewardManager._isNodeOwner(account), "NO NODE OWNER");
        return rewardManager._getNodesCreationTime(account);
    }

    function getNodesLastClaims(address account) public view returns (string memory) {
        require(account != address(0), "SENDER CAN'T BE ZERO");
        require(rewardManager._isNodeOwner(account), "NO NODE OWNER");
        return rewardManager._getNodesLastClaimTime(account);
    }
