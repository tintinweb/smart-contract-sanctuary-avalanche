...
contract ShmoneyPandaGiveaway is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256[]) public winnersByGiveaway;
    ...
    function selectRandomWinners(uint256 numberOfDraws) external onlyAdmin {
        ...
        // If requested more than the number of winners left, correct and only select up to the max number of winners
        if (currentWinnerCount + numberOfDraws > totalNumberOfWinners) {
            numberOfDraws = totalNumberOfWinners - currentWinnerCount;
        }
        ...
        for(uint256 i; i < numberOfDraws; i++) {
            uint256 randomTokenIndex = uint256(keccak256(abi.encode(randomNumber, i))) % totalSupply;
            ...
            if (_winnerIndices[randomTokenIndex] != giveawayId) {
                ...
                winnersByGiveaway[giveawayId].push(tokenId);
            }
        }
    ...
    function claimableAmount(address claimAddress) public view returns (uint256) {
        uint256 currentGiveawayId = _giveawayCount;
        uint256 currentTimestamp = uint256(block.timestamp);
        uint256 claimTotal;
