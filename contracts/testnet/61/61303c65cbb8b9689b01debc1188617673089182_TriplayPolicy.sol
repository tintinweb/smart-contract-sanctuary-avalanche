...
contract TriplayPolicy is Authorization {
    ...
    uint256 public cooldownPeriod; //Amount of time to wait for critical actions to take effect (treasury or cooldownPeriod change)
    CriticalAction[] public criticalActions;
    ...
    event SetCooldownPeriod(address indexed sender, uint256 value);
    ...
    function requestCriticalAction(bytes32 name, bytes32 value) external auth {
        ...
        emit RequestCriticalAction(msg.sender, criticalActionCount, name, value);
        ++criticalActionCount;
    }
    ...
    function executeCriticalAction(uint256 id) external auth {
        ...
        if (criticalAction.name == "addTreasury") {
            ...
            cooldownPeriod = uint256(criticalAction.value);
