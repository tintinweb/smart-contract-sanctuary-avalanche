    ...
    ReentrancyGuard
{
    ...
    uint256 public constant RPC = 10000;
    uint256 public PRICE;
    address private devAddress;

    event RpcMinted(address minter, uint256 id, string URI);

    constructor(
        uint256 mintPrice,
        string memory _uriBase,
        string memory _uriExt
    ) ERC721("Rockn Punks Club", "RPC") {
        setURIBase(_uriBase);
        setURIExt(_uriExt);
        setPrice(mintPrice);
        pause(true);
    ...
    modifier saleIsOpen() {
        require(_totalSupply() <= RPC, "All Minted");
        if (_msgSender() != owner()) {
    ...
    function mint(uint256 amount) public payable saleIsOpen {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            emit RpcMinted(_to, tokenId, URI);
        }
    }
    ...
    function withdrawRpcBusiness() public payable nonReentrant {
