...
contract TalentScoutPresale is ReentrancyGuard, Ownable {
    ...
    /* INTERNAL */
        ...
        function saleLock(address _account, uint256 _scoutToLock) private {
            ...
            uint256 m6Release = (_scoutToLock.mul(1000)).div(10000);
            // 10% released after 240 days
            uint256 m8Release = (_scoutToLock.mul(1000)).div(10000);
            // 10% released after 270 days
            uint256 m9Release = (_scoutToLock.mul(1000)).div(10000);
            // 10% released after 300 days
            uint256 m10Release = (_scoutToLock.mul(1000)).div(10000);
            // 10% released after 330 days
            uint256 m11Release = (_scoutToLock.mul(1000)).div(10000);
            // 10% released after 1 year
            uint256 m12Release = (_scoutToLock.mul(1000)).div(10000);
            // 20% released after 15 months
            uint256 m15Release = (_scoutToLock.mul(2000)).div(10000);
            // 20% released after 18 months
            uint256 m18Release = (_scoutToLock.mul(2000)).div(10000);
