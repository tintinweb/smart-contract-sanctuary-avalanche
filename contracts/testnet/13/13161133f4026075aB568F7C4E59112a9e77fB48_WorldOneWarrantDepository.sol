...
contract WorldOneWarrantDepository is Ownable {
    ...
    mapping(address => uint256[]) private depositorWarrantId; // stores warrant ids for depositors

    uint256 public warrantLotIndex = 0;
    uint256 private warrantId = 1;
    ...
    function deposit(
        ...
        warrantInfo[_depositor][warrantId] = Warrant({
            payout: warrantInfo[_depositor][warrantId].payout.add(_warrantAmount),
            warrantLotID: uint32(warrantLotIndex - 1),
            purchasedAt: uint32(block.timestamp),
            pricePaid: priceInUSD
        });
        depositorWarrantId[_depositor].push(warrantId);
     ...
     *  @return uint
     */
    function redeem(address _recipient, uint256 _warrantId) external returns (uint256) {
        require(msg.sender == _recipient, "NA");
        Warrant memory info = warrantInfo[_recipient][_warrantId];
        require(
        ...
        delete warrantInfo[_recipient][_warrantId]; // delete user info
        delete depositorWarrantId[_recipient]; //delete user warrant id
    ...
    /**
     *  @notice get user warrant ids
     *  @return depositorWarrantId uint[]
     */
    function getWarrantIds(address _depositor) public view returns (uint256[] memory) {
        return depositorWarrantId[_depositor];
    }
