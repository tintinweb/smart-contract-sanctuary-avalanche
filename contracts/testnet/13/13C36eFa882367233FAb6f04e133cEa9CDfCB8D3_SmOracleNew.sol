...
contract SmOracleNew is Ownable {
    ...
    mapping(bytes32 => uint256) public lastTwaps;
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _prices.length; i++) {
            ...
            if (transactionData[_currencyKey].prices.length >= totalNumOfInterval.mul(2).add(30)) {
                lastTwaps[_currencyKey] = getTwap(_currencyKey);
            }
        }
    }
    ...
    function updateFloorData(bytes32 _currencyKey, uint256[] memory _timestamp, uint256[] memory _floorPrices) public {
        ...
        for (uint256 i = 0; i < _floorPrices.length; i++) {
            require(block.timestamp >= _timestamp[i], "floor price timestamp cannot exceed block timestamp in the future");
