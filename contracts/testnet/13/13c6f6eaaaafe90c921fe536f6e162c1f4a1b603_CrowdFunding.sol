...
contract CrowdFunding {
    ...
    constructor(IERC1155 _nftContract, uint256 _requiredNFT) {
    ...
    function createCampaign(string memory _title, string memory _description, uint256 _target, uint256 _deadline, string memory _image) public returns (uint256) {
        require(nftContract.balanceOf(msg.sender, requiredNFT) > 0, "Must hold the required NFT to create a campaign.");
        ...
        nftContract.safeTransferFrom(msg.sender, address(this), requiredNFT, 1, "");
    ...
    function transferTokens(address _to, uint256 _tokenId, uint256 _amount) public {
        require(msg.sender == address(nftContract), "Only the token contract can call this function.");

        nftContract.safeTransferFrom(address(this), _to, _tokenId, _amount, "");

        emit TokensTransferred(_to, _tokenId, _amount);
    }

    function getCampaignDonation(uint256 _id, address _donor) public view returns (uint256) {
    ...
    function checkCampaignStatus(uint256 _id) public returns (CampaignStatus) {
        Campaign storage campaign = campaigns[_id];
