...
contract Controller is Ownable {
    ...
    function getWinIndex(
        ...
        require(
            ...
            "error pool.userNum and poolMax"
        );
        require(
            round.prizeBlock <= block.number,
            "error pool.prizeBlock and block.number"
        );
        if (round.winer != address(0)) {
            return (round.prizeIndex, round.winer);
        }
        bytes32 blockHash = blockhash(round.prizeBlock);
        uint256 hashNum = uint256(blockHash);
        uint256 prizeIndex = hashNum % poolMax;
        return (prizeIndex, round.users[prizeIndex]);
    }
    ...
    function prizeRound(
        ...
        (uint256 prizeIndex, address winer) = getWinIndex(
            poolPrice,
            poolMax,
            roundIndex
        );
