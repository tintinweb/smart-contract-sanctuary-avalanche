...
contract Norma is ERC721, Ownable {
  ...
  uint256 private m_MainMintPrice = 3.5 ether; // 3.5 AVAX
  uint256 private m_PublicMintPrice = 6 ether; // 6 AVAX
  ...
  function mint(uint256 _numberOfTokens) public payable {
    ...
    if (m_IsPublic) {
      require(msg.value == m_PublicMintPrice.mul(_numberOfTokens), "insufficient funds");
    } else {
      require(m_WhiteList[msg.sender], "account not in whitelist");
      require(msg.value >= m_MainMintPrice.mul(_numberOfTokens), "insufficient funds");
    }

    _safeMintMultiple(msg.sender, _numberOfTokens);
  }

  function custom_mint(uint256[] memory ids) public payable {
    require(m_IsMintable, "must be active");
  ...
  function getFiveTokenIds() public returns (uint256[] memory) {
    uint256 count = 0;
    uint256[] memory fiveTokenId;
  ...
  function setMintPrice(uint256 _mainMintPrice) external onlyOwner {
