...
contract XCoin is ERC20Interface {
    ...
    uint constant token_price = 0.0000032 ether;
    ...
    payable
    {
        require(msg.value >= numberOfTokens * token_price, "Not enough bnbs sent");
        _mint(numberOfTokens);
    }

    function _mint(uint256 tokenAmount) internal {
        balances[msg.sender] = tokenAmount * 10**uint(decimals);
