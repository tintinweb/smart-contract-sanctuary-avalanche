...
contract CygnusAlbireo is ICygnusAlbireo {
     ...
     */
    function deployAlbireo(address lpTokenPair, string memory borrowToken) external override returns (address albireo) {
        albireo = address(new CygnusBorrow{ salt: keccak256(abi.encode(lpTokenPair, borrowToken)) }());
    }
}