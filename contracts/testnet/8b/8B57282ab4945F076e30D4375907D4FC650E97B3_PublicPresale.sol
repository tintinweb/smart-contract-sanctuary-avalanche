...
contract PublicPresale is Owned {
    ...
    function buy(uint256 paidAmount) public {
        ...
        require(isPresaleOpen, "Presale is not open yet");
        require(tokenPaid > minTokLimit, "You need to sell at least some min amount");
        require(_paidTotal[msg.sender] + tokenPaid <= maxTokLimit);

        IERC20 token = IERC20(presaleTokenAddr);
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= tokenPaid, "Check the token allowance");
