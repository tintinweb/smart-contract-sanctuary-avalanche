...
contract NodeManagerV1 is Initializable {
  ...
  function _claim(uint256 exceptAmount) private {
    ...
    require(!_isBlacklisted[msg.sender],"Blacklisted");
    bool isCompounding = exceptAmount > 0 ? true : false;
    // save the exceptAmount to be able to subtract in claimeableAmount calc and dont lose the value
    int256 amountToPay = int256(exceptAmount);
    ...
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == msg.sender && (node.limitedTime + payInterval) > uint32(block.timestamp)) {
          ...
          // sub the claimeableAmount for this node to the amount to pay in the compound, if amountToPay is
          // zero the compound is already payed
          if (isCompounding) {
            amountToPay -= int256(claimableAmount);
            
            if (amountToPay <= 0) {
              i = uint32(nodeIndice.length);
              // if amountToPay is less than 0 we need to return this pxt with a subtract in node.claimedTime (seconds)
              if (amountToPay < 0) {
                // calc the amount of rewards per seconds of this tier
                uint256 rewardPerSecond = (((tier.rewardsPerTime) * multiplier) / 1 ether) / tier.claimInterval;
                // calc the amount of seconds to accumulate the claimeable amount to return
                uint32 secondsToRetun = uint32(uint256(-amountToPay / int256(rewardPerSecond)));
                // sub the seconds to this node claimedTime to return the claimeable amount
                node.claimedTime -= secondsToRetun;
              }
            }
          }
        }
   ...
   // if is compounding we create node, dont charge fee and keep remaining amount in rewards
    if (isCompounding) {
      ...
      if (claimFee > 0) {
        ...
        IERC20(tokenAddress).transferFrom(address(msg.sender), address(operationsPoolAddress), feeAmount);
      } 
