...
contract FarmManager is Ownable {
    ...
    //Utility function that allows the farm to remove a reward token. Cannot be called while the farm is active. 
    function removeRewardToken(address farmAddress, uint8 _index) external checkOwnership(farmAddress) {
        IFarmingRewards(farmAddress).removeRewardToken(_index);
    }
