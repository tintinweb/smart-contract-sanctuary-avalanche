...
contract HackersGameStruct is Ownable{
    ...
    mapping(uint256 => bool) public hackerStakedAlready;
    ...
    mapping(uint256 => uint256) public cubeRentedUntil;
    mapping(uint256 => address) public cubeRenter;
    mapping(uint256 => uint256) public cubeRentedWith;
    ...
    constructor () {
        ...
        legendaryHacker[170] = true;
        legendaryHacker[215] = true;
        legendaryHacker[351] = true;
        legendaryHacker[404] = true;
        legendaryHacker[469] = true;
        legendaryHacker[470] = true;
        legendaryHacker[471] = true;
        legendaryHacker[472] = true;
        legendaryHacker[53] = true;
        legendaryHacker[84] = true;
...
contract HackersGameOfficial is ERC20, ERC721Holder, HackersGameStruct{
    ...
    Cube[] public cube;
        ...
        function stakeHacker(uint256 tokenId, uint _gen) external {
        if (_gen == 0)
        {
            hackerNFT0.safeTransferFrom(msg.sender, address(this), tokenId);
            gen0Hacker[tokenId] = true;
            if (!hackerStakedAlready[tokenId]){
                hackerFeeTimePaid[tokenId] = block.timestamp;
            }
        }
        if (_gen == 1)
        {
            hackerNFT1.safeTransferFrom(msg.sender, address(this), tokenId);
        }
    ...
    function unstakeHacker(uint256 hackerNftID, uint gen) external {
        uint tokenId = stakedHackerId[hackerNftID];
        require(hacker[tokenId].transferable == true,"Not transferable");
        require(hacker[tokenId].owner == msg.sender, "not owner");
        //require(hackerFeeTimePaid[hackerNftID] > block.timestamp - 30 days,"unpaid fee");
        if (gen == 0)
        {
        ...
        if (gen == 1)
        {
            hackerNFT1.transferFrom(address(this), msg.sender, hacker[tokenId].ID);
        }
    ...
    function matchCube(uint8 tokenId, uint8 hackerNftId) external {
        uint _hackerId = stakedHackerId[hackerNftId];
        require(hacker[_hackerId].owner == msg.sender, "Not owner"); // checks if owns hacker
        require(hacker[_hackerId].cube == 0,"Already has Cube");
    ...
    function stakeSoloCube(uint8 cubeNFTID) external
    {
        cubeNFT0.safeTransferFrom(msg.sender, address(this), cubeNFTID);
        cube.push(Cube( 0, cubeNFTID, true, msg.sender));
        if (cubeStakedAlready[cubeNFTID] == false){ //checks if cube was staked before to not erase the levels
        _cube_levels[cubeNFTID][WALLS] = 1;
        _cube_levels[cubeNFTID][FLOOR] = 1;
        _cube_levels[cubeNFTID][PC] = 1;
        _cube_levels[cubeNFTID][TABLE] = 1;
        _cube_levels[cubeNFTID][CONSOLE] = 1;
        }
        cubeStakedAlready[cubeNFTID] = true;
        stakedCubes++;
        stakedCubeId[cubeNFTID] = stakedCubes;
        walletOfCubeIds[msg.sender].push(cubeNFTID);
        ownerCubeCount[msg.sender]++;
    }
    
    function unstakeCube(uint256 cubeTokenId, uint256 hackerNftId) external {
        uint tokenId = stakedCubeId[cubeTokenId];
        uint hackerId = stakedHackerId[hackerNftId];
        //require(cubeFeeTimePaid[cubeTokenId] > block.timestamp - 30 days,"Unpaid Fee");
    ...
    function levelUpHacker(uint256 hackerNftId) external payable{
        uint _hackerId = stakedHackerId[hackerNftId];
    ...
    function levelUpCube(uint8 cubePart, uint8 _level, uint256 _cubeNFTId) external payable{
        uint _cubeId = stakedCubeId[_cubeNFTId];
        require(_level <= 5,"Invalid level up");
        require(cubePart <= 5 && cubePart >= 1);
        require(_level > _cube_levels[_cubeNFTId][cubePart],"Level is higher");
        require(cube[_cubeId].owner == msg.sender,"Not owner");
        uint256 need_hack = _cube_upgrade_price[cubePart][_level];
        _cube_levels[_cubeNFTId][cubePart] = _level;
        HACKaddress.transferFrom(msg.sender,address(this),need_hack);
        DynamicCUBE(address(cubeNFT0)).levelUpCube(_cubeNFTId,_cube_levels[_cubeNFTId][1],_cube_levels[_cubeNFTId][2],_cube_levels[_cubeNFTId][3],_cube_levels[_cubeNFTId][4],_cube_levels[_cubeNFTId][5]);
    }

    function hackChest(uint _hackerNFTId, uint _cubeNFTId)external payable returns(uint){
        uint _hackerId = stakedHackerId[_hackerNFTId];
        uint _cubeId = stakedCubeId[_cubeNFTId];
    ...
    function hacking(uint _hackerNFTId,uint _cubeNFTId, uint _job) external {
        uint tokenId = stakedHackerId[_hackerNFTId];
        uint _cubeId = stakedCubeId[_cubeNFTId];
        if(cubeRented[_cubeNFTId])
        {
            require(cubeRentedUntil[_cubeNFTId] < block.timestamp);
        }
        
        require(cubeFeeTimePaid[_cubeNFTId] > block.timestamp - 30 days,"unpaid cube fee");
        require(hackerFeeTimePaid[_hackerNFTId] > block.timestamp - 30 days,"unpaid hacker fee");
        require(hacker[tokenId].kidnapped == false, " is kidnapped");
        require(_job >= 1 && _job <= 4, "Job does not exist");
        require(hacker[tokenId].hackingJob == 0, "Currently hacking");
        require(hacker[tokenId].owner == msg.sender,"not hacker owner");
        require(hacker[tokenId].cube == _cubeNFTId,"Not matched with Cube");
    ...
    function  finishHacking(uint _hackerNFTId, uint _cubeNFTId, uint _job) external {
        uint hackerId = stakedHackerId[_hackerNFTId];
        uint _cube = stakedCubeId[_cubeNFTId];
     ...
     uint randNonce = 0;
        ...
        function cancelHacking(uint _hackerNFTId, uint _cubeNFTId,uint _job) external {
        uint _hackerId = stakedHackerId[_hackerNFTId];
        uint _cube = stakedCubeId[_cubeNFTId];
    ...
    function payRansom(uint _hackerNFTId) external payable {
        uint _hackerId = stakedHackerId[_hackerNFTId];
        require(hacker[_hackerId].owner == msg.sender,"Not owner");
    ...
    /* function buyKidnappedHacker(uint _hackerNFTID) external payable {
        uint _hackerId = stakedHackerId[_hackerNFTID];
        ...
        HACKaddress.transferFrom(msg.sender,address(this),hacker[_hackerId].ransom);
        hackerNFT0.transferFrom(address(this),msg.sender, _hackerNFTID);
    ...
    function payFee(uint _cubeOrHacker, uint _Id) external payable { 
        ...
        require(msg.value >= fee,"not enough AVAX");
        address payable _to = payable(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4);
        _to.transfer(msg.value);
        if (_cubeOrHacker == 1){
            
            if (hackerFeeTimePaid[_Id] > block.timestamp - 30 days){
                hackerFeeTimePaid[_Id] += 30 days;
            }
            hackerFeeTimePaid[_Id] = block.timestamp;
        }
        if (_cubeOrHacker == 2){
            
            if (cubeFeeTimePaid[_Id] > block.timestamp - 30 days){
                cubeFeeTimePaid[_Id] += 30 days;
            }
            cubeFeeTimePaid[_Id] = block.timestamp;
        }

        
    }

    function lendCube(uint _CubeNFT, uint _price) external{
        uint _Id = stakedCubeId[_CubeNFT];
        require(cubeFeeTimePaid[_CubeNFT] > block.timestamp - 30 days,"unpaid fee");
        ...
        cubeRentAble[_CubeNFT] = true;
        cubeRentPrice[_CubeNFT] = _price * 10**18;
    }

    function rentCube(uint _cubeNFTId, uint _hackerNFTId,uint _cubeTimeInDays) external payable{
        uint _hackerId = stakedHackerId[_hackerNFTId];
        uint _cubeId = stakedCubeId[_cubeNFTId];
        require(hacker[_hackerId].cube == 0);
        require(cubeRentAble[_cubeNFTId] == true);
        require(hacker[_hackerId].owner == msg.sender);
        HACKaddress.transferFrom(msg.sender,cube[_cubeId].owner, cubeRentPrice[_cubeNFTId] * _cubeTimeInDays);
        hacker[_hackerId].cube = _cubeNFTId;
        cubeRented[_cubeNFTId] = true;
        cubeRentAble[_cubeNFTId] = false;
        cubeRentedWith[_cubeNFTId] = _hackerNFTId;
        cubeRentedUntil[_cubeNFTId] = block.timestamp + _cubeTimeInDays * 1 days;
        cubeRenter[_cubeNFTId] = msg.sender;

    }

    function cancelCubeLending(uint _cubeId) public {
        require(cubeRentedUntil[_cubeId] < block.timestamp,"Still rented");
        cubeRentAble[_cubeId] = false;
        cubeRented[_cubeId] = false;
        cubeRentPrice[_cubeId] = 0;
        cubeRentedUntil[_cubeId] = 0;
        hacker[cubeRentedWith[_cubeId]].cube = 0;
