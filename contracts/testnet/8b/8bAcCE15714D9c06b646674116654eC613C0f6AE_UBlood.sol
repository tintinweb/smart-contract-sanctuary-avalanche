...
contract UBlood is IUBlood, ERC20, Ownable {
  ...
  /**
   * Store when the origin has last written changes to the BLOOD contract
   */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public virtual override(ERC20, IUBlood) disallowIfSameBlock returns (bool) {
    ...
    // If the entity invoking this transfer is an admin (i.e. Game, Arena)
    ...
    // If it's not an admin entity (i.e. Game, Arena)
    // The entity will need to be given permission to transfer these funds
    // For instance, someone can't just make a contract and siphon $BLOOD from every account
