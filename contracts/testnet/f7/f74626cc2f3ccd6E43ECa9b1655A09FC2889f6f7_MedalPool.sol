...
contract MedalPool is ERC20, Operator {
    ...
    event EmergencyWithdraw(address _to, address _token, uint256 _amount);
    ...
    function setStartTime(uint256 _startTime) public onlyOperator {
        require(startTime > block.timestamp, "MedalPool: POOL_STARTED");
        require(_startTime > block.timestamp, "MedalPool: TIME_HAS_PASSED");
        startTime = _startTime;
    ...
    function setPolFeePercent(uint256 _percent) external onlyOperator {
        require(_percent <= 400, "MedalPool: OUT_OF_RANGE");
    ...
    function setInstantFeePercent(uint256 _percent) external onlyOperator {
        require(_percent <= 600, "MedalPool: OUT_OF_RANGE");
    ...
    function setRewardPercent(uint256 _percent) external onlyOperator {
        require(_percent >= 150 && _percent <= 300, "MedalPool: OUT_OF_RANGE");
    ...
    function deposit(uint256 _amount) external {
        address _sender = msg.sender;
        require(_amount > 0, "MedalPool: ZERO_AMOUNT");
        ...
        returns (uint256)
    {
        ...
        return
            totalClaims.add(rewards) > _maxPayout
                ? _maxPayout.sub(totalClaims).mul(claimFeePercent).div(PERCENTAGE)
                : rewards.mul(claimFeePercent).div(PERCENTAGE);
    }
    ...
    function getDistributeReward() public view returns (uint256) {
        if (lastDeliveryTime < block.timestamp) {
            ...
            uint256 rewardPerSecond = poolBalance
                .mul(rewardPercent)
                .div(PERCENTAGE)
                .div(rollingDuration);
            uint256 rewards = secondsPassed.mul(rewardPerSecond);
    ...
    function getBalancePool() public view returns (uint256) {
        require(
            ...
            "MedalPool: Pool balance should greater than reward"
        );
    ...
    function emergencyWithdraw(address _token, uint256 _amount)
        ...
        onlyOperator
    {
        IERC20(_token).safeTransfer(pol, _amount);
        emit EmergencyWithdraw(pol, _token, _amount);
    }
}
