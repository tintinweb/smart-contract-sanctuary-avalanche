    ...
    IPack
{
    ...
    function addToQuackCount(uint256 quackStart, uint256 quackEnd) external onlyRoleWithSwitch(minterRole) { 
        for (uint256 j = quackStart; j < quackEnd; j += 1) {
        ...
        if (lastQuackIndex > 0) {
            for(uint p =0; p < _amountToOpen; p += 1) {
                ...
                if(targetQuack <= lastQuackIndex) {
                    quackIDsToGiveOut[p] = quackIDsRemaining[targetQuack];
                    quackIDsRemaining[targetQuack] = quackIDsRemaining[lastQuackIndex];
                    //quackIDsRemaining.pop();
                    lastQuackIndex -= 1;
                    quacksAddress.safeTransferFrom(address(this), msg.sender, quackIDsToGiveOut[p]);
                }
            }
        }
