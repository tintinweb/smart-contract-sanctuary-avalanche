    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _token) public virtual initializer {
        _initializeEIP712("WagerBetCustodial", "1");
        __Ownable_init();
    ...
    function withdrawEtherByPlayer(
        ...
        checkOwnerApproval(
            abi.encode(msg.sender, amount, deadline),
            sigR,
            sigS,
            sigV
        );
    ...
    function withdrawWBETByPlayer(
        ...
        _withdrawWBET(msg.sender, amount);
    }

    function withdrawEtherToBulkPlayers(
        address payable[] calldata players,
        uint256[] calldata amounts
    ) external onlyOwner {
        require(
            players.length == amounts.length,
            "Custodial: Mismatch in No of Elements of players and Amounts"
        );
        for (uint256 i = 0; i < players.length; i++) {
            _withdrawEther(players[i], amounts[i]);
        }
    }

    function withdrawWBETToBulkPlayers(
        ...
        for (uint256 i = 0; i < players.length; i++) {
            _withdrawWBET(players[i], amounts[i]);
        }
    }
    ...
    function _withdrawEther(address player, uint256 amount) internal {
        ...
        emit Withdraw(0, player, amount);
    }

    function _withdrawWBET(address player, uint256 amount) internal {
        token.transfer(player, amount);
        emit Withdraw(1, player, amount);
    }
...
contract NativeOwnerApproval is EIP712Base, OwnableUpgradeable {
    ...
    function getNonce(address player) public view returns (uint256 nonce) {
        nonce = nonces[player];
    }

    function checkOwnerApproval(
        bytes memory playerData,
        bytes32 sigR,
        bytes32 sigS,
        uint8 sigV
    ) internal {
        ...
        // increase nonce for player (to avoid re-use)
    ...
    function hashOwnerApproval(OwnerApproval memory ownerApprovalTx) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(OWNER_APPROVAL_TYPEHASH, ownerApprovalTx.nonce, ownerApprovalTx.from, keccak256(ownerApprovalTx.playerData))
            );
    }
