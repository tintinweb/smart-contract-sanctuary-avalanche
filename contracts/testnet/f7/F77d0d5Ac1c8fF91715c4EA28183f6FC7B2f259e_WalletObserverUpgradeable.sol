...
import "../implementations/output/ParticleImplementationPointerUpgradeable.sol";
import "../implementations/output/LiquidityPoolManagerImplementationPointerUpgradeable.sol";
import "../implementations/output/ParticlesManagerImplementationPointerUpgradeable.sol";

contract WalletObserverUpgradeable is
    Initializable,
    OwnableUpgradeable,
    OwnerRecoveryUpgradeable,
    ParticleImplementationPointerUpgradeable,
    LiquidityPoolManagerImplementationPointerUpgradeable,
    ParticlesManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        setTimeframeExpiresAfter(12 hours);
        setTimeframeQuotaIn(200_000_000 * (10**18));
        setTimeframeQuotaOut((5_000_000 / 10) * (10**18));

        // Limit token to 1% of the supply per wallet (we don't count rewards)
        // 1% of 42 000 000 000 = 4 200 000
    ...
    function changeParticleImplementation(address newImplementation)
        public
        virtual
        override(ParticleImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeParticleImplementation(newImplementation);
        excludeFromObserver(particle.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(particlesManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to particlesManager, use the Observatory to create a particle (https://univ.money/observatory)"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= particle.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (particle.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return particle.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the Particle team to know if
...
import "../../contracts/interfaces/IParticle.sol";

abstract contract ParticleImplementationPointerUpgradeable is OwnableUpgradeable {
    IParticle internal particle;

    event UpdateParticle(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyParticle() {
        require(
            address(particle) != address(0),
            "Implementations: Particle is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(particle),
            "Implementations: Not Particle"
        );
        _;
    }

    function getParticleImplementation() public view returns (address) {
        return address(particle);
    }
    ...
    function changeParticleImplementation(address newImplementation)
        ...
        onlyOwner
    {
        address oldImplementation = address(particle);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "Particle: You can only set 0x0 or a contract address as a new implementation"
        );
        particle = IParticle(newImplementation);
        emit UpdateParticle(oldImplementation, newImplementation);
    }
...
import "../../contracts/interfaces/IParticlesManager.sol";

abstract contract ParticlesManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IParticlesManager internal particlesManager;

    event UpdateParticlesManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyParticlesManager() {
        require(
            address(particlesManager) != address(0),
            "Implementations: ParticlesManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(particlesManager),
            "Implementations: Not ParticlesManager"
        );
        _;
    }

    function getParticlesManagerImplementation() public view returns (address) {
        return address(particlesManager);
    }

    function changeParticlesManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(particlesManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "ParticlesManager: You can only set 0x0 or a contract address as a new implementation"
        );
        particlesManager = IParticlesManager(newImplementation);
        emit UpdateParticlesManager(oldImplementation, newImplementation);
    }
