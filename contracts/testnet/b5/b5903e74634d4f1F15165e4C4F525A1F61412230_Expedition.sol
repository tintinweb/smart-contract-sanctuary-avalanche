...
contract Expedition is Ownable {
  ...
  function participate(uint256 _id, uint256[] calldata _tokenIds) public {
  ...
  function reinforceAttack(uint256 _id, uint256 _tokenId) external {
    require(_id < totalCampaigns, "campaign does not exist");
    ...
    campaign.attackPoint += nft.getAttack(_tokenId) + nft.getEndurance(_tokenId);
    if (nft.getLevel(_tokenId) < campaign.tier * 10) {
      campaign.attackPoint -= penaltyAmount;
    }
  }

  function reinforceDefense(uint256 _id, uint256 _tokenId) external {
    require(_id < totalCampaigns, "campaign does not exist");
    ...
    require(campaign.startTimestamp + CAMPAIGN_DURATION > block.timestamp, "campaign has been ended");
    require(campaign.ambusher == msg.sender, "invalid ambusher");
    require(campaign.defenseNFTs.length + 1 == campaign.attackNFTs.length, "invalid turn");
    ...
    nft.transferFrom(msg.sender, address(this), _tokenId);
    campaign.defensePoint += nft.getAttack(_tokenId) + nft.getEndurance(_tokenId);
    if (nft.getLevel(_tokenId) < campaign.tier * 10) {
      campaign.defensePoint -= penaltyAmount;
    }
  }

  function checkCampaignerWin (uint256 _id) private view returns (bool) {
    Campaign storage campaign = campaigns[_id];
    
    uint256[] storage attackNFTs = campaign.attackNFTs;
    uint256[] storage defenseNFTs = campaign.defenseNFTs;
    uint256 attackPoints = 0;
    uint256 defensePoints = 0;
    uint256 i = 0;
    while(i < attackNFTs.length) {
      attackPoints = attackPoints + nft.getAttack(attackNFTs[i]) + nft.getEndurance(attackNFTs[i]);
      i++;
    }
    i = 0;
    while(i < defenseNFTs.length) {
      defensePoints = defensePoints + nft.getDefense(defenseNFTs[i]) + nft.getEndurance(defenseNFTs[i]);
      i++;
    }
    return defensePoints >= attackPoints;
  }

  function finishCampaign(uint256 _id) external {
    Campaign storage campaign = campaigns[_id];
    ...
    if (isDefenseWinner) {
      honToken.safeTransferFrom(address(this), winner, honFee[aveLevel]);
      hrmToken.safeTransferFrom(address(this), winner, hrmFee[aveLevel]);
    } else {
      honToken.safeTransferFrom(address(this), winner, honFee[aveLevel] * 70 / 100);
      hrmToken.safeTransferFrom(address(this), winner, hrmFee[aveLevel] * 70 / 100);
      honToken.safeTransferFrom(address(this), loser, honFee[aveLevel] * 30 / 100);
      hrmToken.safeTransferFrom(address(this), loser, hrmFee[aveLevel] * 30 / 100);
    }
  ...
  function createAndParticipateCampaign(uint256 _tier, uint256[] calldata _tokenIds) external {
  ...
  function unlockAttackNFTs(uint256 _campaignId) external {
    ...
    require(campaign.reinforceTimestamps[reinforceAttackLength * 2 - 1] > block.timestamp + 1 hours, "!try later");

    for (uint256 i = 0; i < campaign.defenseNFTs.length; i++) {
      require(nft.ownerOf(campaign.defenseNFTs[i]) == msg.sender, "!token owner");
