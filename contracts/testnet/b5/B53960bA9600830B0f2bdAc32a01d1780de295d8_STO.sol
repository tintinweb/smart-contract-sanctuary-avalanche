...
using SafeMath for uint256;
  ...
  //  address public ownerWallet;
        ...
        RefundableCrowdsale(_goal)
    {
        ...
        // admin = _msgSender();
        developerToken = (_propertyValue/_rate) - (_cap + _debtTokens);
        goalPercent = _goal;
    ...
    */
    ...
    function getRemainingTokens() public view returns (uint256 _remainingTokens) {
        require(goalReached(),"Goal is not reached yet");
            IERC1400RawERC20 _securityToken = IERC1400RawERC20(
                address(token())
            );
            //total supply returns number of tokens
    ...
    function mintDevsTokens(address _propertyOwner,uint256 _token,bytes memory certificate) public onlyAdmin{
            require(_token <= developerToken,"STO: Not enough developer Tokens left");
              IERC1400RawERC20 _securityToken = IERC1400RawERC20(
                address(token())
            );
             _securityToken.issue(
                _propertyOwner, 
                _token, // Add those tokens as well which are left other than cap
                certificate
            );
            developerToken -= _token; 
     }
    
    function mintRemainingTokens(address _propertyOwner,uint256 _noOfTokens ,bytes memory _certificate) public onlyAdmin{
        IERC1400RawERC20 _securityToken = IERC1400RawERC20(
                address(token())
            );
        ...
        require(_noOfTokens <= remainingTokens,"not enough tokens");

        _securityToken.issue(
                _propertyOwner, 
                _noOfTokens, // Add those tokens as well which are left other than cap
                _certificate
            );
