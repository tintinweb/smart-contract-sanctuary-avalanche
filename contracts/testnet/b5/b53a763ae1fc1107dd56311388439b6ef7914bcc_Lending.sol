...
contract Lending {
    ...
    event RepaySuccess(address borrower, uint receipt_sn, uint amount, address lender, uint income, uint changed_order, uint order_balance, uint chainid, uint c_sn, address source, address token, uint c_frozen);
    ...
    function repay(uint receipt_sn) external payable {
        ...
        emit RepaySuccess(msg.sender, receipt_sn, msg.value, receipt.lender, income, latest_order, order_balance, receipt.chainid, receipt.c_sn, collateral.source, collateral.token, collateral.frozen);
    }
    ...
    function getConfigure() external view returns(uint, uint, uint, uint, uint, uint, uint) {
         return (min_ir, max_ir, penalty_ir, penalty_days, cycle, deadline, commission_rate);
    }
