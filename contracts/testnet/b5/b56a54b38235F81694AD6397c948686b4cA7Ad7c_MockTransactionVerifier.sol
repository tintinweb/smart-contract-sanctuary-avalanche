...
contract MockTransactionVerifier is Ownable {
    ...
    function validateReceiptProof(bytes memory proofBlob, bytes32[] memory merkleProof) public view returns (
        ...
        (result, index, chainId, blockHash, t) = _validateReceiptProof(proofBlob, merkleProof);
        state = t.state;
        gasUsed = t.gasUsed;
        bloom = t.bloom;
        logs = t.logs;
    ...
    function setBlockUpdater(uint16 chainId, address blockUpdaterAddress) external onlyOwner {
        blockUpdaters[chainId] = IBlockUpdater(blockUpdaterAddress);
    }

    function _validateReceiptProof(bytes memory proofBlob, bytes32[] memory merkleProof) internal view returns (
        uint8 result,
        uint256 index,
        uint16 chainId,
        bytes32 blockHash,
...
interface IBlockUpdater {
    function checkBlockHash(bytes32 blockHash,bytes32 receiptsRoot, bytes32[] calldata merkleProof)
        external
        view
        returns (bool);
}
