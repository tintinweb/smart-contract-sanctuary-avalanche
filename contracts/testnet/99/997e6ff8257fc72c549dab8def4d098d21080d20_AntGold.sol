...
contract AntGold is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable {
    ...
    // ant staked from timestam
    mapping(uint => uint) private antStakedFromTime;
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    event Minted(address owner, uint antGold);
    ...
    function claimableView(uint256 tokenId) public view returns (uint) {
        ...
        // get colonyStage from avalant contract
        uint colonyStage = 1;
        if (antStaked[tokenId] == false) {
            ...
            uint deltaSeconds = block.timestamp - antStakedFromTime[tokenId];
    ...
    function _claimAntGold(uint[] memory tokenIds) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            require(a.ownerOf(tokenIds[i]) == msg.sender, "NOT OWNER");
            uint claimableAntGold = claimableView(tokenIds[i]);
            if (claimableAntGold > 0) {
                totalNewAntg += claimableAntGold;
                antStakedFromTime[tokenIds[i]] = uint(block.timestamp);
            }
        }
        if (totalNewAntg > 0) {
            _mint(msg.sender, totalNewAntg);
            emit Minted(msg.sender, totalNewAntg);
        }
    }
...
// How do we give antg at the beginning to everyone ?