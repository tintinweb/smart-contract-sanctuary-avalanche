...
contract DexillaExchangeV2 is AccessControl, Pausable {
    using SafeERC20 for ERC20;

    bytes32 public constant FEE_COLLECTOR_ROLE = keccak256("FEE_COLLECTOR_ROLE");

    uint256 public tradeFee = 10; // 0.1%
    uint256 public totalBaseFee = 0;
    uint256 public totalQuoteFee = 0;

    uint8 private immutable BASE_TOKEN_DECIMALS;
    uint8 private immutable QUOTE_TOKEN_DECIMALS;

    ERC20 public baseToken;
    ERC20 public quoteToken; // always USD token

    mapping(address => mapping(uint256 => uint256)) public bids; // owner, price, quantity
    mapping(address => mapping(uint256 => uint256)) public asks; // owner, price, quantity

    event OrderCreated(address indexed maker, uint8 side, uint256 price, uint256 quantity);
    event OrderExecuted(
    ...
    event OrderSizeAdjusted(address indexed maker, uint8 side, uint256 price, uint256 quantity);
    event OrderCanceled(address indexed maker, uint8 side, uint256 price, uint256 quantity);
    event FeeWithdrawn(address indexed owner, uint256 baseFee, uint256 quoteFee);
    event TradeFeeAdjusted(uint256 tradeFee);

    constructor(address _baseToken, address _quoteToken, address feeCollector, uint256 _tradeFee) {
    ...
    // 0 = bid, 1 = ask
    function createOrder(uint8 side, uint256 price, uint256 quantity) public whenNotPaused {
        require(side == 0 || side == 1, "Invalid side");
        require(price > 0, "Invalid price");
        require(quantity > 0, "Invalid quantity");

        if (side == 0) {
            uint256 quoteAmount = multiplyWithDecimal(
                quantity,
                BASE_TOKEN_DECIMALS,
                price,
    ...
    function executeOrder(address[] memory makers, uint8 side, uint256 price, uint256 quantity) public whenNotPaused {
        require(makers.length <= 10, "Too many makers");
        require(side == 0 || side == 1, "Invalid side");
        require(quantity > 0, "Invalid quantity");

        uint256 remainningQuantity = quantity;
        for (uint256 i = 0; i < makers.length; i++) {
            require(makers[i] != address(0), "Invalid maker");
            if (side == 0) {
                uint256 makerQuantity = asks[makers[i]][price];
                ...
                uint256 makerTransferQuantity = multiplyWithDecimal(
                    transferQuantity,
                    BASE_TOKEN_DECIMALS,
                    price,
                    QUOTE_TOKEN_DECIMALS,
                ...
                quoteToken.safeTransferFrom(msg.sender, makers[i], makerTransferQuantity);
                uint256 fee = (transferQuantity * tradeFee) / 10000;
                totalBaseFee += fee;
                uint256 remainningToTaker = transferQuantity - fee;
                baseToken.safeTransfer(msg.sender, remainningToTaker);
                ...
                uint256 makerQuantity = bids[makers[i]][price];
                if (makerQuantity == 0) continue;
                uint256 transferQuantity;
                if (makerQuantity >= remainningQuantity) {
                    transferQuantity = remainningQuantity;
                ...
                baseToken.safeTransferFrom(msg.sender, makers[i], transferQuantity); // transfer base from taker to maker
                uint256 quantityWithoutFee = multiplyWithDecimal(
                    transferQuantity,
                    BASE_TOKEN_DECIMALS,
                    price,
                    QUOTE_TOKEN_DECIMALS,
                ...
                uint256 fee = (quantityWithoutFee * tradeFee) / 10000;
                totalQuoteFee += fee;
                uint256 remainningToTaker = quantityWithoutFee - fee;
                quoteToken.safeTransfer(msg.sender, remainningToTaker); // transfer usd from contract to taker
    ...
    function adjustOrderSize(uint8 side, uint256 price, uint256 desiredQuantity) public whenNotPaused {
        require(side == 0 || side == 1, "Invalid side");
        ...
        if (side == 0) {
            ...
            if (oldQuantity > desiredQuantity) {
                uint256 _quantity = multiplyWithDecimal(
                ...
                quoteToken.safeTransfer(msg.sender, _quantity);
                ...
                quoteToken.safeTransferFrom(msg.sender, address(this), _quantity);
            }
            bids[msg.sender][price] = desiredQuantity;
        } else {
            oldQuantity = asks[msg.sender][price];
            if (oldQuantity > desiredQuantity) {
                baseToken.safeTransfer(msg.sender, oldQuantity - desiredQuantity);
            } else if (oldQuantity < desiredQuantity) {
                baseToken.safeTransferFrom(msg.sender, address(this), desiredQuantity - oldQuantity);
            }
    ...
    function cancelOrder(uint8 side, uint256 price) public {
        require(side == 0 || side == 1, "Invalid side");
        require(price > 0, "Invalid price");
    ...
    function withdrawFee() external onlyRole(FEE_COLLECTOR_ROLE) {
        baseToken.safeTransfer(msg.sender, totalBaseFee);
        quoteToken.safeTransfer(msg.sender, totalQuoteFee);
