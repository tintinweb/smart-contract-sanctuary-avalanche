...
contract Lottery is Ownable {
     ...
     */
    function startLottery(uint tickets, uint price) public onlyOwner {
        if ((tickets <= 1) || (price == 0)) {
            revert();
        }
        numTickets = tickets;
        ticketPrice = price;
     ...
     */
    function playLottery1() public {
        // revert in case user already has bought a ticket OR,
        // value sent is less than the ticket price OR,
        // gameStatus is false.
        if ((playerAddresses[msg.sender]) || (!gameStatus)) {
            revert();
        }
        // IERC20(0xA3f742E790ce255CfEf42282A0dA123d3A99436F).approve(msg.sender, ticketPrice);
        IERC20(0xA3f742E790ce255CfEf42282A0dA123d3A99436F).transferFrom(msg.sender, address(this), ticketPrice);
        availTickets = availTickets - 1;
        players[++counter] = msg.sender;
        winningAmount += ticketPrice;
    ...
    function getWinner() internal {
        ...
        payable(winnerAddress).transfer(winningAmount.div(100).mul(98));
        payable(creator).transfer(winningAmount.div(100).mul(2));
    }
