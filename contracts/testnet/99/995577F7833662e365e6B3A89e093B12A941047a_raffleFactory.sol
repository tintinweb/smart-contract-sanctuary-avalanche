  ...
  struct Collection {
    ...
    function createEdition(
        ...
        atContract.increment();
         Collection memory x = Collection(_name, newContract, _editionSize, newId,  _minPlayers, _salePrice);
    collection.push(x);
    collectionCount ++;
    return newId;
        return newId;
...
contract RaffleNft is ERC721Upgradeable, IEditionSingleMintable,  OwnableUpgradeable {
  ...
  function totalSupply() public view returns (uint256) {
   ...
   function setIsActive(bool _state) public onlyOwner {
    require(salePrice > 0, "Define sale price before starting raffle");
    require(minPlayers > 0, "Define minimum players before starting raffle");
    isActive = _state;
  }
 ...
 // return balance in wei
    ...
    function houseFee() public view returns(uint) {
        return address(this).balance * 19 / 100;
    }
