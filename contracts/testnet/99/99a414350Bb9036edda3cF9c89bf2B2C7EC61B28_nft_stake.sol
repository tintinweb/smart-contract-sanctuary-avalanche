...
contract nft_stake {
    ...
    function calc_now(address _accounts) public view returns (uint[3] memory) {
    	...
    	uint256 num;
	va = 0;
	num = stakes_1[_accounts].amount;
	    	for (uint i=0; i < num; i++){
			if ((stakingTime_1[_accounts][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[0] = va;
		...
		num = stakes_2[_accounts].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_2[_accounts][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[1] = va;
		...
		num = stakes_3[_accounts].amount;
		for (uint i=0; i < num; i++){
			if ((stakingTime_3[_accounts][i] - time) > 24*60*60) {
				va += 1;
				}
		}
		nums[2] = va;		
