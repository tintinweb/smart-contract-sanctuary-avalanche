...
contract ArfERC721V1 is ERC721Upgradeable, OwnableUpgradeable {
    ...
    function whitelistAddressesEggnite(address[] calldata _address, bool status)
        external
        virtual
        onlyOwner
    {
        for (uint256 i = 0; i < _address.length; i++) {
            whitelistedAddressEggnite[_address[i]] = status;
        }

        emit WhitelistedAddressEggnite(_address, status);
    }
    ...
    function Joepegs(uint256 _token_id) external virtual {
        ...
        (bool success, bytes memory data) = address(JoepegsTokenAddress).call(
            abi.encodeWithSignature("burn(uint256)", _token_id)
        );
    ...
    function mintEgginite() external virtual {
        ...
        emit Egginte_Minted(msg.sender, 4, id, tokenCounter);
    }
    ...
    function mintARC() external virtual returns (uint256) {
        require(
            checkEpochStatus[3] &&
        ...
        emit ARCminted(msg.sender, tokenCounter, 3);
        return tokenCounter;
    }
