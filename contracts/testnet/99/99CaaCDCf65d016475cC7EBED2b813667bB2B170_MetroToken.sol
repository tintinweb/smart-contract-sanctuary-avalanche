...
contract MetroToken is ERC20, ERC20Burnable, ERC20Snapshot, TokenWithdrawable {
    constructor() ERC20("MetroToken", "METRO") {
        _mint(msg.sender, 1e28);
    }

    /**
     * @dev snapshot and return snapshot id
     */
...
contract TokenWithdrawable is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    event ReceiveETH(address from, uint256 amount);
    event WithdrawETH(address recipient, uint256 amount);
    event WithdrawToken(IERC20 token, address recipient, uint256 amount);
     ...
     * @param amount amount token to withdraw
     */
    function withdrawETH(address payable recipient, uint256 amount) external nonReentrant onlyOwner {
