...
contract MarginLoan is IMarginLoan {
    ...
    function requestLoan(
        ...
        //  uint256 previousLoan = getLoan(_tokenAddress);
         (uint256 previousLoan,) =  getLoan(msg.sender,_tokenAddress);
         uint256 newLoan = previousLoan.add(_loanAmount);
        // uint256 limitOfloan =  _loanlimit(_tokenAddress, loanPercentage);
         require(
            newLoan <= _loanlimit(_tokenAddress, loanPercentage),
        ...
        // );
        // uint256 tokens = getLoan(msg.sender, _tokenAddress);
        // uint256 newLoanTokens = tokens + noOfTokens;
        // uint256 newLoanTokens = IERCtoken.balanceOf(msg.sender);
        ...
        // newMarginLoan.noOfTokens = newLoanTokens;
        newMarginLoan.loanLimit = _loanlimit(_tokenAddress, loanPercentage);
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < loanLength; i++) {
            MarginLoan storage newMarginLoan = marginLoan[msg.sender][i];
            if (
        ...
        returns (uint256[] memory ids, uint256[] memory loans)
    {
        ...
        // uint256[] memory ids = new uint256[](newLoanLength);j
    ...
    //     override
    //     view
    //     returns (uint256[] memory noOfTokens, uint256[] memory loanAmounts ,uint256[] memory ids)
    // {
    //     uint8 newLoanLength = getLoansLength(_user, _token);
    //     uint256[] memory noOfTokens = new uint256[](newLoanLength);
    //     uint256[] memory loanAmounts = new uint256[](newLoanLength);
    ...
    //      (uint256 previousLoan,) =  getLoan(_tokenAddress);
    //     uint256 limitOfloan =  _loanlimit(_tokenAddress, _loanPercentage);
    //     //  _noOfTokens += tokens;
    //     newMarginLoan.bank = _bank;
    //     newMarginLoan.loanAmount = _loanAmount;
    //     newMarginLoan.interestRate = _interestRate;
    ...
    //     newMarginLoan.loanLimit = limitOfloan;
    //     newMarginLoan.noOfTokens = _user.balance;
    //     newMarginLoan.loanPercentage = _loanPercentage;
    ...
    function _loanlimit(address _tokenAddress, uint256 _loanPercentage) internal view returns (uint256){
        require(
            Address.isContract(_tokenAddress),
            "required erc1400 token address"
        );
        ...
        uint256 balanceOfuser = IERCtoken.balanceOf(msg.sender); 

        uint loanLimit = (balanceOfuser.mul(1000)).mul(_loanPercentage).div(100);
...
interface IMarginLoan {
   ...
   function getTotalNoOfTokens(address _user, address _token)
    ...
    // function _loanlimit(
    //     address _tokenAddress,
    //     uint256 _loanPercentage) external returns (uint256);
}
