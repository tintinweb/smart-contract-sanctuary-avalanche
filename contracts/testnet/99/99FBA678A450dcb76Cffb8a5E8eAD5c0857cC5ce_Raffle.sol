    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        for(uint i; i < _randomWords.length; i++){
            if(i < numberOfWinners/2){
                ...
                firstArr[i] = _randomWords[i] % (firstMintersCount-j);
                ...
                if(mintersCount >= firstMintersCap){
                    ...
                    lastArr[i] = _randomWords[i] % (lastMintersCount-j);
                    lastMintersWinners.push(lastMinters[lastArr[i]]);
                }
            }
        }
