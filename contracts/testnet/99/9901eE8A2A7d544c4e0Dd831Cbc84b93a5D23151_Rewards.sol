    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @param _yieldKey Yield Key of Yield Box
     * @param _type Type of Yield Box
		 * @param _daysClaimed Days Claimed for All Yield Box Associated with Yield Key
     */
    function rewardPerYK(uint256 _yieldKey, uint8 _type, uint256 _daysClaimed) external nonReentrant {
        (
            uint256[] memory amount,
            uint256[] memory rewarded,
            uint256[] memory YieldBoxRewarded,
        ...
        for (uint256 i = 0; i < amount.length; i++) {
					if (rewarded[i] > _daysClaimed) {
						amount[i] = mulDiv(amount[i], _daysClaimed, rewarded[i]); // Adjust amount to the days claimed
					}
          total += amount[i];
        }
