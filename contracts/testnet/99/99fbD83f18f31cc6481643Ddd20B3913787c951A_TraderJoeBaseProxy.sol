...
contract TraderJoeBaseProxy is AccessControlEnumerable, Pausable {
    ...
    /// @param _value Value in tokens decimals to withdraw. This is not wei, every token has own decimals number
    /// @dev Maybe be better to change the function to withdraw all funds
    ...
    function swapExactTokensForTokens(
        ...
        traderJoe.swapExactTokensForTokens(_amountIn, _amountOutMin, _pairBinSteps, _tokenPath, _to, _deadline);
    }
    ...
    function swapExactTokensForAVAX(
        ...
        IERC20(_tokenPath[0]).safeTransferFrom(msg.sender, address(this), _amountIn);
        IERC20(_tokenPath[0]).approve(address(traderJoe), maxApproval);
        traderJoe.swapExactTokensForAVAX(_amountIn, _amountOutMinAVAX, _pairBinSteps, _tokenPath, payable(_to), _deadline);
    }
