...
contract DiamondHand is Ownable, ReentrancyGuard {
    ...
    /// @notice The DXN ERC-20 contract.
    ...
    uint256 MAX_APY = 100;
    uint256 private constant ACC_DXN_PRECISION = 1e12; // 6 digit after comma
    ...
    /// @param _DXN the DXN Token
    /// @param _TREASURY the DXN treasury for reward
    constructor(address _DXN, address _TREASURY) public {
        DXN = IERC20(_DXN);
        TREASURY = _TREASURY;
        poolSetting[0] = PoolSetting({duration: 5 minutes, apy: 10});
        poolSetting[1] = PoolSetting({duration: 10 minutes, apy: 30});
        poolSetting[2] = PoolSetting({duration: 15 minutes, apy: 60});
    }
        ...
        onlyOwner
    {
        require(
            ...
            "DiamondHand::updatePoolApySetting bad apy"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "DiamondHand::updatePoolDurationSetting bad duration"
        );
    ...
    function updateTreasury(address _treasury) external onlyOwner {
        require(
            ...
            "DiamondHand::updateTreasury bad address"
        );
    ...
    /// @notice View function to see pending DXNs on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user address of user
    function pendingDXN(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        PoolInfo memory pool = poolInfo[_pid];
        require(pool.owner == _user, "DiamondHand::pendingDXN not owner");
        uint256 _pendingDXN = 0;
        uint256 stakeTokenSupply = DXN.balanceOf(address(this));
        if (stakeTokenSupply != 0) {
            ...
            if (pass > 0) {
                uint256 multiplier = pass.mul(ACC_DXN_PRECISION).div(
                    pool.duration
                );
                _pendingDXN = multiplier.mul(
                    pool.rewardDebt.div(ACC_DXN_PRECISION)
                );
            }
        }
        return _pendingDXN;
    ...
    function addPool(
        ...
        require(apy > 0 && apy <= MAX_APY, "DiamondHand::addPool bad apy");
        uint256 reward = _amount.mul(apy).div(100).mul(ACC_DXN_PRECISION);
        poolInfo.push(
            PoolInfo({
                amount: _amount,
                rewardDebt: reward,
    ...
    /// @notice Deposit DXN to DiamondHand.
    /// @param _amount The amount of DXN
    ...
    {
        // Validation
        require(msg.sender != address(0), "DiamondHand::deposit:: bad address");
        require(_amount > 0, "DiamondHand::deposit:: bad _amount");
        require(_poolSetting >= 0, "DiamondHand::addPool bad _poolSetting");
        require(
            msg.sender != address(TREASURY),
            "DiamondHand::deposit:: treasury excluded"
        );
        // Interactions
        DXN.safeTransferFrom(address(msg.sender), address(this), _amount);
        addPool(msg.sender, _amount, _poolSetting);
        emit Deposit(msg.sender, _amount);
    }

    /// @notice Withdraw DXN from Diamond Hand.
    /// @param _for Receiver
    /// @param pid The index of the pool. See `poolInfo`.
    function withdraw(address _for, uint256 pid) external nonReentrant {
        PoolInfo storage pool = poolInfo[pid];
        require(!pool.claimed, "DiamondHand::withdraw:: already claimed");
        require(pool.owner == msg.sender, "DiamondHand::withdraw:: only owner");
        require(pool.depositedAt != 0, "DiamondHand::withdraw:: time invalid");
        require(
            (pool.depositedAt + pool.duration) < block.timestamp,
            "DiamondHand::withdraw:: time invalid"
        );
        // Effects
        uint256 reward = pool.rewardDebt.div(ACC_DXN_PRECISION);
        ...
        allClaimed++;
        // Interactions
        DXN.safeTransferFrom(address(TREASURY), msg.sender, reward);
        DXN.safeTransfer(msg.sender, amount);
        emit Withdraw(msg.sender, pid, reward, _for);
    }

    /// @notice Withdraw all valid pool from Diamond Hand.
    /// @param _for Receiver
        ...
        nonReentrant
    {
        ...
        require(plength > 0, "DiamondHand::withdrawAll:: bad pids array");
        uint256 l = 0;
        ...
        require(
            ...
            "DiamondHand::withdrawAll:: no valid pool"
        );
        // Effects
        uint256 reward = 0;
        uint256 amount = 0;
    ...
    /// @notice emergencyWithdraw DXN that somehow got stuck
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        uint256 amount = DXN.balanceOf(address(this));
        DXN.safeTransfer(msg.sender, amount);
