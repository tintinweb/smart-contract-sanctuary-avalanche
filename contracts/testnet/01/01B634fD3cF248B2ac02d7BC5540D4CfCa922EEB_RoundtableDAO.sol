...
contract RoundtableDAO is TableGovernance, ReentrancyGuard, Multicall, NFTreceiver, IERC1271 {
	/// ----------------------------------------------------------------------------------------
	///							EVENTS
	/// ----------------------------------------------------------------------------------------
		...
		if (didProposalPass) {
			...
			unchecked {
				...
				if (prop.proposalType == ProposalType.ALLOW_CONTRACT_SIG) {
					...
					// We start from 1 since account[0] will be the marketplace address which we are approving
					for (uint256 i = 1; i < prop.accounts.length; i++) {
						contractSignatureAllowance[prop.accounts[0]][bytes32(prop.payloads[i])] = 1;
					}
				}
