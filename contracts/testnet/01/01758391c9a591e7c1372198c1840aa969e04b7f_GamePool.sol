...
contract GamePool is IGamePool, SignableStakes, Constants, UUPSUpgradableTemplate, IAttributablePlayer {
  ...
  function harvest(
    ...
    require(
      isSignedByValidator(0, hashHarvesting(_msgSender(), amount, deadline, randomNonce, opId), signature0),
      "GamePool: invalid signature0"
    );
    require(
      isSignedByValidator(1, hashHarvesting(_msgSender(), amount, deadline, randomNonce, opId), signature1),
      "GamePool: invalid signature1"
    );
    _saveSignatureAsUsed(signature0);
    _saveSignatureAsUsed(signature1);
  ...
  function isSignatureUsed(bytes calldata signature) external view returns (bool) {
    bytes31 key = bytes31(keccak256(abi.encodePacked(signature)));
    return _usedSignatures[key] != 0;
  }
