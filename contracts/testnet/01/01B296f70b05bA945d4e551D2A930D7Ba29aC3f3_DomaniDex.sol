...
contract DomaniDex is IDomaniDex, Ownable, ReentrancyGuard {
    ...
    returns (uint256 inputAmountUsed)
  {
    ...
    for (uint256 i = 0; i < componentsNumber; i++) {
      if (address(_inputDexParams.swapToken) == components[i]) {
        remainingInputAmount = remainingInputAmount - notionalUnits[i];
        ...
        remainingInputAmount = remainingInputAmount - returnValues.inputAmount;
      }
    ...
    inputAmountUsed = startingAmount - remainingInputAmount;
    ...
    returns (uint256 outputAmountReceived)
  {
    ...
    _inputDexParams.fund.safeApprove(address(basicIssuanceModule), _inputDexParams.fundQuantity);
    ...
    for (uint256 i = 0; i < componentsNumber; i++) {
      if (address(_inputDexParams.swapToken) == components[i]) {
        outputAmountReceived = outputAmountReceived + notionalUnits[i];
        ...
        outputAmountReceived = outputAmountReceived + returnValues.outputAmount;
      }
    }
