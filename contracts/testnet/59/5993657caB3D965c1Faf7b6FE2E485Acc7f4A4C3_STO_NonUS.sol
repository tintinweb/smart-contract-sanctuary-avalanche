...
import "../../token/SecurityToken-NonUS/ERC20/IERC1400RawERC20.sol"; // This is to use transfer function
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    constructor(
        ...
        _rate = __rate;
        _token = __token;
        _wallet = __token.propertyOwners();
        stableCoin_ = __stableCoin;
    ...
    function tokenFallback(
        ...
        require(whitelist.isUserForeign(beneficiary),"STO50");
    ...
    function _deliverTokens(address beneficiary, uint256 tokenAmount,bytes memory data) internal virtual {
        ...
        // uint256 totalTokens = (tokenAmount / _rate ) * _token2.granularity();
        // _token2.issue(_token2.reserveWallet(), totalTokens,data);
    ...
    function _forwardTokens(address receiver, uint256 stableAmount) internal {
        bool isTransfer = stableCoin_.transfer(receiver, stableAmount);
        require(isTransfer);
    }
    /**
    ...
    function changeSTableCoinAddress(IERC20 newStableCoin_) public onlyAdmin {
     stableCoin_ = newStableCoin_;
        emit ChangeStableCoinAddress(msg.sender,address(stableCoin_), address(newStableCoin_) );
    }
     ...
     */
    function updateUSSTOAddress(address _USSTO) internal returns (bool) {
        stoUSAddress = _USSTO;
        return true;
    }
}
...
import "../validation/TimedCrowdsale.sol";
import "../../../token/SecurityToken-NonUS/ERC20/IERC1400RawERC20.sol";
...
abstract contract PostGoalCrowdsale is TimedCrowdsale, RefundableCrowdsale {
     ...
     */
    function withdrawTokens(address beneficiary) public onlyAdmin {
        require(goalReached(), "STO21");
        require(whitelist.isAdmin(msg.sender),"only Admin");
...
abstract contract CappedCrowdsale is Crowdsale {
   ...
   event AddDebtToken(address sender,uint256 debtTokens);
    ...
    function _preValidatePurchase(
        ...
        require((_securityToken.totalSupply() + (targetTokens) + (debtTokens_)) <= _cap, 
        "STO34"
        );
    }
...
interface ISTO{
   function rate() external view returns (uint256);
    ...
    function stoUSAddress() external view;
...
import "../NFTwhitelist/IAkruNFTWhitelist.sol";
// This is to import ERC20 Coin
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
// This is to use transfer function
//import "../token/SecurityToken-NonUS/ERC20/IERC1400RawERC20.sol";
// This is import Crowd Sale
import "./crowdsale/Crowdsale.sol";
// Instead of sending tokens from admin wallet mint them and create new tokens
import "./crowdsale/emission/MintedCrowdsale.sol";
// Maxium amount of TKUSD that can be raised
import "./crowdsale/validation/CappedCrowdsale.sol";
// Timed CrowdSale Start and End Date
import "./crowdsale/validation/TimedCrowdsale.sol";
// Replace this with your own whitelist
import "./crowdsale/distribution/PostGoalCrowdsale.sol";
// To refund payment if goal never achives
...
import "./ISTO.sol";
  ...
  * Code Detail used in require statment of STO
     ...
     * STO49: Crowdsale: Token should be divisible with US rate after GoalReached
     * STO50: Crowdsale: Only Foriegn investor can invest
     * STO51: Crowdsale: TotalSupply of NonUS are not multiple of ratio of US side
     * STO52: Should be Divisible of No of Token on Non US
  */
contract STO_NonUS is
    Crowdsale,
    MintedCrowdsale,
    CappedCrowdsale,
    TimedCrowdsale,
    ...
    PostGoalCrowdsale
{
   ...
   event SetNewUSSTOAddress(address sender, address stoUSAddress);
   event TokenBatch(address sender,uint256 batchSize,uint256 noOfToken);
    ...
    modifier onlyAdmin() override(Crowdsale ,TimedCrowdsale){
        require(Crowdsale.whitelist.isAdmin(msg.sender), "STO0");
        _;
    }
    ...
    */
        ...
        override(CappedCrowdsale, TimedCrowdsale, Crowdsale)
    {
        ...
        require(_targetTokens % batchIs == 0,"STO52");
        require(
    ...
    function _processPurchase(
        ...
        ISTO stoUS = ISTO(stoUSAddress);
        uint256 tokenMultiple = stoUS.rate()/rate();
        address reserveWalletUS = IERC1400RawERC20(address(stoUS.token())).reserveWallet();
        uint256 totalSupplyNonUS = IERC1400RawERC20(address(token())).totalSupply();
         uint256 totalSupplyUS = IERC1400RawERC20(address(stoUS.token())).totalSupply();
        if (stage == CrowdsaleStage.ICO)
        {
            _deliverTokens(_beneficiary, _tokenAmount, _data);
            uint256 totalTokens = _tokenAmount/(stoUS.rate()/rate());
            uint256 totalTokenNonUS = (totalSupplyNonUS + _tokenAmount)/tokenMultiple;
            require(totalTokenNonUS >= totalSupplyUS, "STO51");
            unchecked {
            totalTokens = totalTokenNonUS - totalSupplyUS ;
            }
            if(totalTokens!=0){
            address reserveWalletNonUS = IERC1400RawERC20(address(stoUS.token())).reserveWallet();
            IERC1400RawERC20(address(stoUS.token())).issue(reserveWalletNonUS,totalTokens,"0x00");
            }
        }
        else {
             ...
             if(totalTokens!=0){
             stoUS.callPostGoalProcessPurchase(reserveWalletUS,  totalTokens, "0x00");
             }
        }
    }
    /**
     ...
     * @param whitelist_ address of the new whitelist
     */
    function changeWhitelist(
        IAkruNFTWhitelist whitelist_
    ) public onlyAdmin returns (bool) {
        emit ChangeWhitelist(msg.sender, address(whitelist_), address(whitelist));
    ...
    function updateGoal(uint256 newGoal) external returns (uint256) {
        require(whitelist.isAdmin(_msgSender()) ,"STO5" );
        ...
        returns (uint256 _remainingTokens)
    {
        ...
        uint256 _finalTotalSupply = (propertyValue / (initialRate)) - (debtTokens);
     ...
     */
    function setUSSTOAddress(address UsSTO) public onlyAdmin returns(bool success)
    {
        require(!goalReached(), "STO14");
        require(!hasClosed(),"STO6");
         emit SetNewUSSTOAddress(msg.sender, UsSTO);
        return super.updateUSSTOAddress(UsSTO);
    }
    /**
    ...
    /**
     * @dev This function make token Batching If someone purchase token 
     * then restrict to purchase minumum token equal to batch and multiple of batch
     * @param batchSize size of minimum token want to sale during STO
     */
    function tokenBatch(uint batchSize) public onlyAdmin{
        uint256 noOfToken = ISTO(stoUSAddress).rate()/_rate;
        require(noOfToken % batchSize == 0,"STO52");
        batchIs = batchSize;
        emit TokenBatch(msg.sender, batchSize,noOfToken);
    }
    /**
