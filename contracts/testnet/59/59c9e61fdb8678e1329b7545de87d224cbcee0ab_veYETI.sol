...
interface IEmitter {
    ...
    function getReward(address _user) external returns (uint);
}
...
contract veYETI is IveYETI {
    ...
    */
    ...
    event UserUpdate(address _user, bool _isStakeIncrease, uint _stakeAmount);
    ...
    */ 
    function update(RewarderUpdate[] memory _yetiAdjustments) external {
        ...
        _update(_yetiAdjustments, msg.sender);
    }
     ...
     */
    function _handleStaking(UserInfo storage userInfo, address _stakeFrom, bool _isIncreaseStake, uint _amount) internal {
        if (_amount > 0) {

            if (_isIncreaseStake) {
                // pull in YETI tokens to stake
                require(yetiToken.transferFrom(_stakeFrom, address(this), _amount));
    ...
    function _update(RewarderUpdate[] memory _yetiAdjustments, address _stakeFrom) internal {
        ...
        _handleStaking(userInfo, _stakeFrom, _isStakeIncrease, _stakeAmount);
    ...
    // claim earned veYETI rewards 
    // the claimed rewards will be provided to
    // _rewarder address
    function claimRewards(address _rewarder) external {
        _requireValidCaller();

        uint256 stakeAmount = emitter.getReward(msg.sender);
        RewarderUpdate[] memory yetiAdjustments = new RewarderUpdate[](1);
        yetiAdjustments[0] = RewarderUpdate(_rewarder, stakeAmount, true);

        _update(yetiAdjustments, address(emitter));
    }
