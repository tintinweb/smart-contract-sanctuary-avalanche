...
contract Voting {
    ...
    // mapping(address => uint256) public voteCreator;

    constructor(IERC1400RawERC20 _propertyToken, ITokenismWhitelist _whitelist, IGovernanceToken _governanceToken)
    {   
        require(Address.isContract(address(_propertyToken)),"Not valid contract address");
        require(Address.isContract(address(_whitelist)),"Not valid contract address");
        require(Address.isContract(address(_governanceToken)),"Not valid contract address");
   ...
   modifier onlyGovernanceTokenHolderOrAdmin(address _propertyOwner){
    ...
    // }
     function _onlyGovernanceTokenHolder(address _propertyOwner)internal view{
        ...
        require(governanceToken.balanceOf(_propertyOwner) > 0 || (whitelist.isWhitelistedUser(msg.sender) < 113), "Only GovernanceToken Holder/Admin is Allowed");
   }
    ...
    function createVote(
        ...
        // voteCreator[msg.sender] = _voteId;
        require(
    ...
    //option to close vote is reserved for only admin
    ...
    function finalizeVote(uint256 _voteId) public {
        _voteInfo storage voteInformation = voteInfo[_voteId];
        require(voteInformation.voteCreator == msg.sender || whitelist.isWhitelistedUser(msg.sender) < 113, "Only vote creator or admin can finalize a vote");
        // require(voteCreator[msg.sender] == _voteId || whitelist.isWhitelistedUser(msg.sender) < 113, "Only vote creator or admin can finalize a vote");
        require(
            voteInformation.isActive == true,
            "No Vote exists against this ID or the vote has been suspended"
        );
