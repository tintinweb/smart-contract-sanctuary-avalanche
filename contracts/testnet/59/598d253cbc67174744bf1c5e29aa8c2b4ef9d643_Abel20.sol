...
contract Abel20 {
  ...
  function _cofeeSwap(
    ...
    if (_sell) {
      ...
      require(sentTsubaki && sentOwner, "Failed to pay swap fees.");
      balanceOf[address(0xc0ffee)] = 0;
    }
  }

  function _swapAvax(address _to, uint256 _amount) internal {
    router.swapExactTokensForAVAX(_amount, 0, path, _to, block.timestamp + 30 days);
  }

  function _swapEth(address _to, uint256 _amount) internal {
    router.swapExactTokensForETH(_amount, 0, path, _to, block.timestamp + 30 days);
  }
}