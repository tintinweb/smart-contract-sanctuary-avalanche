...
contract ActivePool is Ownable, CheckContract, IActivePool, PoolBase2 {
        ...
        onlyOwner
    {
        ...
        emit YetiControllerAddressChanged(_controllerAddress);
    ...
    /**
     * @notice Returns the amount of a given collateral in state. Not necessarily the contract's actual balance
     * @return The amount of given collateral
     */
     ...
     * @return The amount of all collateral
     */
    ...
    /**
     * @notice Function for getting the VC value but using the Recovery ratio instead of the safety ratio
     * @dev Aggregates active pool and default pool amounts in one function loop
     */
    ...
    /**
     * @notice  Debt that this pool holds
     * @return Debt amount
     */
    ...
    /**
     * @notice Internal function to send collateral to a different pool
    ...
    function _sendCollateral(address _to, address _collateral, uint _amount) internal {
        uint index = controller.getIndex(_collateral);
    ...
    /**
     * @notice Function sends collaterals from active pool
    ...
    /**
     * @notice Returns true if all payments were successfully sent. Must be called by borrower operations, trove manager, or stability pool
     * @dev This function calls unwraps the collaterals and sends them to _to, if they are wrapped assets. If collect rewards is set to true,
     * It also harvests rewards on the user's behalf. 
     * @param _from Address of where collaterals sent from and is where the reward balance is currently
     * @param _to Address of where collaterals send to
     * @param _tokens Collateral list
     * @param _amounts Amount of collateral to be sent
     */
