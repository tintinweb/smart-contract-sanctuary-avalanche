...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        onlyValidAddress(client) 
    {
        ...
        for (uint256 i = 0; i < amountPerInstallment.length; i++) {
            sum += amountPerInstallment[i];
        }
        require(
            _msgSender() != client, 
            "Business Owner and Client can not be same"
        );
        require(sum == amount, "Amount must be equal to sum of installment");
        lastPaymentId++;
        ...
        nonReentrant
    {
        Payment storage payment = payments[paymentId];
        require(
            payment.status == Status.PAYING || payment.status == Status.PAID, 
            "You can not cancel this payment"
        );
        require(
     ...
     */
    function clientWithdraw(uint256 paymentId) 
        external 
        onlyClient(paymentId) 
        nonReentrant 
    {
        ...
        emit ClientWithdrawn(paymentId, withdrableAmount, payment.paymentToken);
    }
