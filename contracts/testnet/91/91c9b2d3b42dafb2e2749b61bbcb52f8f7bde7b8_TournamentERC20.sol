...
contract TournamentERC20 is Ownable, ITournament {
    ...
    constructor ( address _token, uint256 _initialPeriodSeconds, address _teamAddress, address _sidePot, uint256 _periodSeconds, uint256 _decaySeconds, uint256 _decayStopSeconds, uint256 _TotalBurn, uint256 _TotalWinnings, uint256 _ticketSize, uint256 _potShare, address _treasury) {
    ...
    function buy() external payable {
        require(msg.value == 0, "This is for AVAX, don't send ERC20 Tokens");
        ...
        if (SIDEPOTSHARE >  0) {
            rewardToken.transferFrom(msg.sender, address(SIDEPOT), ticketPrice * (OWNER_SHARE / 10000));
            
        }
         if (TEAM_SHARE >  0) {
            rewardToken.transferFrom(msg.sender, address(teamaddress), ticketPrice * (OWNER_SHARE / 10000));
