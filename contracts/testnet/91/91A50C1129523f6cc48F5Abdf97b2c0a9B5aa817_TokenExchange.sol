...
contract TokenExchange is Ownable, Pausable {
    ...
    IERC20Metadata public base; // A-NRG
    IERC20Metadata public quote; // AAA (Agarta token)
    ...
    function calculateOutTokens(uint256 _value) public view returns (uint256) {
        return
            (_value * price_divisor * uint256(10**uint256(base.decimals()))) /
            (price * uint256(10**uint256(quote.decimals())));
    }

    function calculateInputTokens(uint256 _value)
        public
        view
        returns (uint256)
    {
        return
            (_value * price * uint256(10**uint256(base.decimals()))) /
            (price_divisor * uint256(10**uint256(quote.decimals())));
    }
    ...
    function buy(uint256 _value) external whenNotPaused {
        ...
        require(
            base.balanceOf(address(this)) >= outAmount,
