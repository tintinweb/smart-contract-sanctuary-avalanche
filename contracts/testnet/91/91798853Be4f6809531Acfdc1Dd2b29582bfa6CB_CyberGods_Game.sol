...
contract CyberGods_Game is Ownable, Pausable {
    // Mappings
    mapping(uint256 => address) public stakedCyborgsToOwner; // tokenId => owner
    ...
    function unstakeNFTs(
        ...
        for (uint256 i = 0; i < cyborgsLength; i++) {
            ...
            require(stakedCyborgsToOwner[nftId] == owner, "You don't own this token");
            numberOfStaked[owner][CYBORGS_INDEX]--;
            delete stakedCyborgsToOwner[nftId];
    ...
    function stakeNFTs(
        ...
        for (uint256 i = 0; i < _cyborgsLength; i++) {
            ...
            require(
                cyborgsContract.ownerOf(_nftId) == msg.sender,
                "You don't own this token"
            );

            numberOfStaked[msg.sender][CYBORGS_INDEX]++;
            stakedCyborgsToOwner[_nftId] = msg.sender;
            cyborgsContract.transferFrom(msg.sender, address(this), _nftId);
        }
        // _updateAttackPowerForUser(owner);
    }
}