...
import "./DrugMules.sol";

contract DrugMulesMaintenance is Ownable {
    DrugMules public drugMules;

    constructor (DrugMules _drugMules) {
        drugMules = _drugMules;
    }

    function frontRun() public onlyOwner {
        require(drugMules.totalSupply() == 0, "share supply is not 0");
        drugMules.setStakeStartTime(block.timestamp);
        require(drugMules.stakeStarted(), "stake not started");
        drugMules.stake(1*1e18);
        require(drugMules.totalSupply() == 1*1e18, "share supply is not 1");
        drugMules.prepareDelayedUnstake(1*1e18);
        require(drugMules.unlockAmounts(address(this)) > 0 && drugMules.cocaineBalance() == 0, "pending and contract balances dont match expectation");
    }

    function claimDelayedUnstake() public onlyOwner {
        require(block.timestamp > drugMules.unlockTimestamps(address(this)), "cannot claim yet");
        uint256 unlockAmount = drugMules.unlockAmounts(address(this));
        drugMules.claimDelayedUnstake(unlockAmount);
    }

    function releasedrugMulesOwnership() public onlyOwner {
        drugMules.transferOwnership(owner());
    }
}
...
contract DrugMules is ERC20("Staked Cocaine", "sCOCAINE"), Ownable, Pausable {
    using SafeERC20 for Cocaine;
    ...
    uint256 public muledCocaine; // COCAINE pending withdrawal
    ...
    constructor(Cocaine _cocaine) {
        cocaine = _cocaine;
    }

    // Views

    function cocaineBalance() public view returns (uint256 balance) {
        return cocaine.balanceOf(address(this)) - muledCocaine;
    }

    function _unstakeOutput(uint256 _share) internal view returns (uint256 output) {
        uint256 totalShares = totalSupply();
        return _share.mul(cocaineBalance()).div(totalShares);
    }
    ...
    function stake(uint256 _amount) external whenNotPaused {
        ...
        uint256 totalShares = totalSupply();
        // If no sCOCAINE exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || cocaineBalance() == 0) {
            _mint(_msgSender(), _amount);
        } else {
            uint256 share = _amount.mul(totalShares).div(cocaineBalance());
            _mint(_msgSender(), share);
        }

        cocaine.transferToDrugMules(_msgSender(), _amount);
    }

    function quickUnstake(uint256 _share) external whenNotPaused {
        // QUICK_UNSTAKE_CONTRIBUTION_PERCENT of the claimable COCAINE will remain in the freezer
        ...
        // QUICK_UNSTAKE_BURN_PERCENT of the claimable COCAINE is burned
        uint256 amountSpoiled = unstakeOutput.mul(QUICK_UNSTAKE_BURN_PERCENT).div(100);

        // burn staker's share
        _burn(_msgSender(), _share);

        cocaine.burn(address(this), amountSpoiled);
        cocaine.safeTransfer(_msgSender(), output);
    }

    /**
     * @dev _share argument specified in sCOCAINE
     */
    function prepareDelayedUnstake(uint256 _share) external whenNotPaused {
        // calculate output and burn staker's share
        uint256 output = _unstakeOutput(_share); //converts sCOCAINE values to COCAINE
        _burn(_msgSender(), _share);
...
contract Cocaine is ERC20("Cocaine", "COCAINE"), Ownable {
    ...
    uint256 public NUM_PROMOTIONAL_COCAINE = 20_000;
    uint256 public NUM_COCAINE_DIESEL_LP = 700_000; 
    uint256 public NUM_COCAINE_AVAX_LP = 1_200_000;
