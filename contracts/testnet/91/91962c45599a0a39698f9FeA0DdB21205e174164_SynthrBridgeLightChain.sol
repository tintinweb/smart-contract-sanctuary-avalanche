...
abstract contract BaseBridge is NonblockingLzApp {
    ...
    uint16 internal constant PT_APPEND = 8;
    uint16 internal constant PT_VEST = 9;
...
contract SynthrBridgeLightChain is BaseBridge {
    ...
    function initialize(address __synthrAddressResolver, uint16 __mainChainId) external onlyOwner {
    ...
    function sendDepositCollateral(
        ...
        _broadcast(lzPayload, PT_DEPOSIT_COLLATERAL, _mainChainId);
    }
    ...
    function sendExchange(
        ...
        _broadcast(lzPayload, PT_EXCHANGE, dstChainId);

        if (dstChainId != _mainChainId) {
            _broadcast(lzPayload, PT_EXCHANGE, _mainChainId);
        }
    }
