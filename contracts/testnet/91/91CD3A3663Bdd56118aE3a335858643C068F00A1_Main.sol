...
contract Main is Ownable, CloneFactory {
    ...
    event TradingNFT(bytes32 _key, uint256 amount, uint256 price, uint256 royaltyAmount, uint256 income, address maker, address taker);
    ...
    function putOnSale(
        ...
        if(collectionOwner != address(0)) {         // But when the original contract owner will come and verify and set the roatlity fee...
    ...
    function refundFromFNFT(bytes32 _key) external isBlackList {
        require(listInfo[_key].maker == address(0), "Main:cant refund");
        uint price;
        uint amount;
        for(uint i = 0; i < 10; i++) {
    ...
    function buyNow(bytes32 _key, uint _amount) external isBlackList {
        ...
        require(listInfo[_key].maker != msg.sender && listInfo[_key].maker != address(0), "Main:IV maker");
    ...
    function _trading (bytes32 _key, uint _amount, uint _price, address user, bool isBuyNow) private {
        if(_detect(listInfo[_key].collectionId) == ContractType.ERC721) {
            ...
            try IERC1155(listInfo[_key].collectionId).getUserInfo(listInfo[_key].tokenId) returns(uint8, uint8, uint8 nftType, uint, address) {
                require(nftType != 3, "Main:cant trade");
    ...
    function _exchangeDefaultNFT(bytes32 _key, uint amount, uint price, bool isFNFT, address user, bool isBuyNow) private {
        ...
        if(listInfo[_key].admin != address(0) && listInfo[_key].admin != address(this) && 100 > listInfo[_key].royaltyShare) {
        ...
        emit TradingNFT(_key, amount, price, royaltyAmount, income, listInfo[_key].maker, user);
    }
