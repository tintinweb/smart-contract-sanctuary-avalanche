...
contract ElkFarmFactory is IElkFarmFactory, Ownable {
    ...
    function setManager(address _managerAddress) external override onlyOwner {
        require(_managerAddress != address(0), "0 addr");
    ...
    function _takeFee() private {
        require(
            ...
            "Not enough balance to pay contract creation fee"
        );
    ...
    function setFee(uint256 _newFee) external onlyOwner {
        require(_newFee < maxFee, "Fee > max");
        fee = _newFee;
        emit FeeSet(_newFee);
    }
    ...
    /// @dev This function is available to Elk in case any "Scam" or nefarious farms are created using the contract. Elk will be able to stop the offending farm and allow users to recover funds.
