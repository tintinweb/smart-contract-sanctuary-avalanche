...
contract Manager is ReentrancyGuard, Pausable, AccessControl {
  ...
  // ) external {
  //   // TODO: IN DEV ONLY
  //   IERC20(tokenA).safeTransferFrom(msg.sender, address(this), _tokenAAmount);
  //   IERC20(tokenB).safeTransferFrom(msg.sender, address(this), _tokenBAmount);
  //   // ---
  ...
  //   ITokenStakePool(stakePool).stake(IERC20(lpToken).balanceOf(address(this)));
  // }

  // function withdraw() external {
  //   ITokenStakePool(stakePool).withdraw(vaults[msg.sender].lpTokenAmount);
  // }
  ...
  function work(
    ...
    if (positionValue(msg.sender) < _targetLpValue) {
      ...
      IERC20(rewardToken).approve(stakePool, lpTokensMinted);
  ...
  function compound() public {
    ...
    if (rewardTokenBalance > 0) {
      ...
      // TODO: we can reduce the code here to hardcode as tokenB since we only swap to TokenB->Rewards
      address[] memory swapPathForOptimalDeposit = new address[](2);
  ...
  // function compound1() public returns (uint256, bool) {
  //   uint256 rewardTokenBalance = IERC20(rewardToken).balanceOf(address(this));
  ...
  //     return (optimalSwapAmount, isReversed);
  //   }

  //   return (0, false);
  // }

  // function compound2(uint256 optimalSwapAmount) public {
  //   address[] memory swapPathForOptimalDeposit = new address[](2);
  //   swapPathForOptimalDeposit[0] = tokenB;
  //   swapPathForOptimalDeposit[1] = tokenA;
  ...
  // function compound3() public returns (uint256) {
  //   IERC20(tokenA).approve(router, IERC20(tokenA).balanceOf(address(this)));
  //   IERC20(tokenB).approve(router, IERC20(tokenB).balanceOf(address(this)));
  ...
  // function compound4(uint256 lpTokensMinted) public {
  //   IERC20(lpToken).approve(stakePool, lpTokensMinted);
  //   ITokenStakePool(stakePool).stake(lpTokensMinted);
  // }
