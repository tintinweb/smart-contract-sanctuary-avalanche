...
contract NIT is INIT, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "NIT: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "NIT: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __NIT_v2_init_unchained(_router);
    }

    function __NIT_v2_init_unchained(address _router) internal initializer {
        _name = "Node Investment Trust";
        _symbol = "NIT";
        _decimals = 9;
    ...
    function __NIT_tiers_init() internal initializer {
    ...
    preventBlacklisted(_account, "NIT: Selected account is in blacklist")
    {
        require(_account != address(0), "NIT: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "NIT: Invalid address");
        require(_accountsTier[_account] > 0, "NIT: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "NIT: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "NIT: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "NIT: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "NIT: Owner address is blacklisted")
    preventBlacklisted(spender, "NIT: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "NIT: Address is blacklisted")
    preventBlacklisted(from, "NIT: From address is blacklisted")
    preventBlacklisted(to, "NIT: To address is blacklisted")
    {
    ...
    function migrate(address account, uint256 amount) external override {
        require(migration != address(0), "Node Investment Trust: Migration is not started");
        require(_msgSender() == migration, "Node Investment Trust: Not Allowed");
