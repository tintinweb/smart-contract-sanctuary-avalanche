...
contract Zap is OwnableUpgradeable {
    ...
    function zapOutToken(address _from, uint amount, address _to, address _recipient) external {
        ...
        uint256 amountA;
        uint256 amountB;
        (amountA, amountB) = ROUTER.removeLiquidity(token0, token1, amount, 0, 0, msg.sender, block.timestamp);
        if (token0 != _to) {
        ...
        IERC20Upgradeable(_to).safeTransfer(_recipient, amountA.add(amountB));
    }
