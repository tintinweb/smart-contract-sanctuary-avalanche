        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                if (contractTokenBalance > 0) {
                    ...
                    uint256 otherHaflfForLiquidty = amountForLiquidity -
                        halfForLiquidity;
                    uint256 marketingAmount = contractTokenBalance
                        .mul(marketingFee)
                        .div(Fee);
                    uint256 amountToSwap = marketingAmount.add(
                        halfForLiquidity
                    );
                    ...
                    uint256 marketingTax = marketingFee.mul(100).div((liquidityFee.div(2)).add(marketingFee));
                    marketingTax = contractETHBalance.mul(marketingTax).div(10000);
                    if (marketingTax > 0) {
                        sendETHToFee(marketingTax);
                    }

                    uint256 amountToLiquify = (liquidityFee.div(2)).mul(100).div((liquidityFee.div(2)).add(marketingFee));
                    amountToLiquify = contractETHBalance.mul(amountToLiquify).div(10000);
                    if (amountToLiquify > 0) {
                        addLiquidityETH(amountToLiquify,otherHaflfForLiquidty);
                    }
                }
            }
        }
