...
// File contracts/cygnus-core/interfaces/ICygnusNebula.sol

//goo: Unlicensed
pragma solidity >=0.8.4;

/// @title ICygnusNebula Interface for Oracle
interface ICygnusNebula is IDexPair {
    /*  ══════════════════════════════════════════════════════════════════════════════════ 
            1. CUSTOM ERRORS
        ══════════════════════════════════════════════════════════════════════════════════  */
...
interface ICygnusCollateralControl is ICygnusTerminal {
     ...
     */
    event NewPriceOracle(ICygnusNebula newPriceOracle);
     ...
     */
    function setPriceOracle(ICygnusNebula newPriceOracle) external;
...
interface ICygnusBorrowTracker is ICygnusBorrowInterest, ICygnusBorrowApprove {
    ...
    /**
     *  @notice Accrues interest rate and updates borrow rate.
     */
...
.        ░█▀▀█ █──█ █▀▀▀ █▀▀▄ █──█ █▀▀        *         .            
  *      ░█─── █▄▄█ █─▀█ █──█ █──█ ▀▀█             .             . 
    ...
    /// @param lpTokenPair Address of LP Token
    /// @param poolIndex Index of lending pool
    function deployAlbireo(address lpTokenPair, uint8 poolIndex)
        external
        returns (address albireo);
}
...
contract CygnusFactory is ICygnusFactory {
     ...
     *  @inheritdoc ICygnusFactory
     */
    ICygnusNebula public override cygnusNebulaOracle;
    ...
    constructor(
        ...
        ICygnusNebula _cygnusNebulaOracle
    ) {
        admin = _cygnusAdmin;
    ...
    function createBorrowTokenA(address lpTokenPair) external override returns (address borrowDAITokenA) {
        ...
        // Deploy collateral
        borrowDAITokenA = collateralDeployer.deployDeneb(lpTokenPair, address(this));
    ...
    function createBorrowTokenB(address lpTokenPair) external override returns (address borrowDAITokenB) {
        ...
        // Deploy collateral
        borrowDAITokenB = collateralDeployer.deployDeneb(lpTokenPair, address(this));
...
// File contracts/cygnus-core/CygnusNebula.sol
...
abstract contract CygnusNebula is ICygnusNebula {
    /*           .        .          .     .          .           *    .   
            .      .                           . 
        ░█▀▀█ █──█ █▀▀▀ █▀▀▄ █──█ █▀▀       . .         .            
        ░█─── █▄▄█ █─▀█ █──█ █──█ ▀▀█                          . 
    ...
    function initializeCygnusNebula(address lpTokenPair) external override {
        ...
        // Update current price cumulative for Token A and Token B
        ...
        // Udate last update slot for Token A and Token B
...
the benefit of the public at large and to the detriment of our heirs and successors.
We intend this dedication to be an overt act of relinquishment in perpetuity of all present
and future rights to this software under copyright law.

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT
NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND
NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS BE LIABLE FOR ANY CLAIM, DAMAGES OR
...
/* .        .               .          .                          .                            . 
     ...
     Deposit LP Token, borrow stablecoins. Contract then converts the stables to more LP Token and 
     puts back in pool, accruing interest and compounding on any interaction.

     Farmers may leverage up to 8x their LP Token value putting their LP Token as collateral, but may suffer
     loss from liquidations if loan goes above debt ratio.
     ...
     In normal mode users can lend/borrow from any supported pair that is approved by CygnusDAO. Lenders
     earn from interest rates and a % of yield rewards and borrowers earn from leveraging their LP Tokens.

     multi-player on the other hand are pools that only whitelisted addresses can join and anyone can deploy.
     In these pools there is only 1 borrower and there can be many lenders. There are no interest rates between
     parties. In shuttles users can choose whichever assets they want to collateralize/farm as long as
     it's in a supported exchange.
    ...
    These contracts are a fork of Impermax/Tarot with some small differences.
 ...
 *  @notice Contract used to mint Collateral and Borrow tokens and sets Factory and Admin rights.
 *  @dev UniswapV2ERC20 implementation with some minimal edits.
 */
contract CygnusTerminal is ICygnusTerminal, Erc20Permit {
     ...
     *  @param name_ Erc20 name of both LP Tokens.
     *  @param symbol_ Erc20 symbol of both LP Tokens.
     *  @param decimals_ decimals of both LP Tokens (always 18).
     */
    constructor(
        string memory name_,
        string memory symbol_,
    ...
    /**
     *  @notice Checks if caller is Factory. 🛸
    ...
    function exchangeRate() public view virtual override returns (uint256) {
        // Saves an SLOAD if non-zero
        uint256 _totalSupply = totalSupply;

        // If there is no supply for this token return initial (1e18), else totalBalance * SCALE / totalSupply
 ...
 *  @title CygnusCollateralControl
 *  @author CygnusDAO
 *  @notice Factory initializes Collateral Arm (Deneb)
 *  @notice Main contract for controlling collateral settings like liquidation incentive/fee and debt ratios
 */
contract CygnusCollateralControl is ICygnusCollateralControl, CygnusTerminal {
    ...
    /**
     *  @notice Name of the Erc20 collateral token.
     *  @dev Passed in constructor and assigned to Erc20 public `name`.
     */
    string private constant COLLATERAL_NAME = "Cygnus: Collateral";

    /**
     *  @notice Symbol of the Erc20 collateral token.
     *  @dev Passed in constructor and assigned to Erc20 public `symbol`.
     */
    string private constant COLLATERAL_SYMBOL = "Deneb";

    /**
     *  @notice Decimals of the Erc20 collateral token.
     ...
     *  @inheritdoc ICygnusCollateralControl
     */
    address public override borrowDAITokenA;

    /**
     *  @inheritdoc ICygnusCollateralControl
     */
    address public override borrowDAITokenB;
     ...
     *  @inheritdoc ICygnusCollateralControl
     */
    uint256 public override debtRatio = 0.805e18;
     ...
     *  @inheritdoc ICygnusCollateralControl
     */
    uint256 public constant override LIQUIDATION_INCENTIVE_MIN = 1.01e18;

    /**
     *  @inheritdoc ICygnusCollateralControl
     */
    uint256 public constant override LIQUIDATION_INCENTIVE_MAX = 1.50e18;
    ...
    /**
     *  @notice Creates Collateral arm.
     */
    constructor()
        /**
         *  @param COLLATERAL_NAME Name of the Erc20 Collateral token
         *  @param COLLATERAL_SYMBOL Symbol of Erc20 Collateral token
         *  @param DECIMALS Decimals of the Erc20 Collateral token (always 18)
         */
        CygnusTerminal(COLLATERAL_NAME, COLLATERAL_SYMBOL, DECIMALS)
    {}
    ...
    /**
     *  @notice Cygnus Admins can update values for liq incentive, debt ratio and liq fee within min/max ranges only.
    ...
    function initializeCollateral(
        ...
        address _borrowDAITokenA,
        address _borrowDAITokenB
    ) external override cygnusHangar18 {
        // initialize price oracle
        cygnusNebulaOracle = ICygnusFactory(hangar18).cygnusNebulaOracle();

        // Assign contract of the underlying LP Token
        underlying = _underlying;

        // Assign address of Borrow Token A
        borrowDAITokenA = _borrowDAITokenA;

        // Assign address of Borrow Token B
        borrowDAITokenB = _borrowDAITokenB;

        /// @custom:event InitializeCollateral
        emit InitializeCollateral(underlying, borrowDAITokenA, borrowDAITokenB);
    }
     ...
     *  @inheritdoc ICygnusCollateralControl
     */
    function setPriceOracle(ICygnusNebula newCygnusNebulaOracle) external override cygnusAdmin nonReentrant {
        // update price oracle
        cygnusNebulaOracle = newCygnusNebulaOracle;

        /// @custom:event newDebtRatio
        emit NewPriceOracle(newCygnusNebulaOracle);
    }
    ...
    function setDebtRatio(uint256 newDebtRatio) external override cygnusAdmin nonReentrant {
        ...
        // It is, update debt ratio
        debtRatio = newDebtRatio;

        /// @custom:event newDebtRatio
        emit NewDebtRatio(newDebtRatio);
    }
...
contract CygnusCollateralModel is ICygnusCollateralModel, CygnusCollateralControl {
    ...
    function accountLiquidityInternal(
        ...
        // 1. Amount of Token A * Price of token A and divide by scale
        uint256 totalTokenA = amountTokenA.mul(priceTokenA);

        // 2. Amount of Token B * Price of token B and divide by scale
        uint256 totalTokenB = amountTokenB.mul(priceTokenB);

        // 3. Factor in the current debt ratio for this market
        uint256 totalPosition = (totalTokenA + totalTokenB).mul(debtRatio);

        // 4. Multiply by the current liquidation incentive for this market
    ...
    function getAccountLiquidity(
        ...
        // This is the account's total collateral deposited * present exchange rate
        uint256 amountCollateral = balanceOf(borrower).mul(exchangeRate());

        // Calculate user's liquidity or shortfall
    ...
    function getTimeWeightedPrices() public override returns (uint256 priceTokenA, uint256 priceTokenB) {
        // 1. Get the uint224 price of LP Token
        (
        ...
        // 3. Total supply of the collateral
...
contract CygnusCollateral is ICygnusCollateral, CygnusCollateralModel {
    ...
    /**
     *  @custom:library UQ112x112 For oracle and reserves.
     */
    using UQ112x112 for uint224;
     ...
     *  @param value Value amount of loan.
     */
    function transferInternal(
        address from,
        address to,
    ...
    function tokensUnlocked(address from, uint256 value) public override returns (bool) {
        ...
        // Calculate final balance against the underlying's exchange rate
        uint256 amountCollateral = PRBMath.mulDivFixedPoint(finalBalance, exchangeRate());

        // Borrow balance of Token A, calls BorrowDAITokenA contract
        uint256 amountTokenA = ICygnusBorrowTracker(borrowDAITokenA).getBorrowBalance(from);

        // Borrow balance of Token B, calls BorrowDAITokenB contract
    ...
    function accountLiquidity(address borrower) public override returns (uint256 liquidity, uint256 shortfall) {
        // Calculate liquidity internally
    ...
    function canBorrow(
        ...
        // saves an SLOAD if passes next condition
    ...
    /**
     *  @notice This should only be called by Albireo contracts.
     *  @inheritdoc ICygnusCollateral
     */
    function seizeDeneb(
        address liquidator,
        address borrower,
        uint256 repayAmount
    ) external override nonReentrant returns (uint256 denebAmount) {
        // @custom:error Avoid liquidating yourself :(
        ...
        if (_msgSender() != borrowDAITokenA || _msgSender() != borrowDAITokenB) {
        ...
        // prettier-ignore
        // if calling from DAI, get the weighted price of DAI, else from token B
        ...
        denebAmount = (PRBMath.mulDivFixedPoint(repayAmount, liquidationIncentive) * denebPrice) / exchangeRate();
        ...
        emit Transfer(borrower, liquidator, denebAmount);
    }
    ...
    function redeemDeneb(
        ...
        uint256 declaredRedeemTokens = PRBMath.mulDiv(redeemAmount, 1e18, exchangeRate()) + 1;

        /// @custom:error Avoid insufficient
        if (redeemAmount < declaredRedeemTokens) {
            revert CygnusCollateral__InsufficientRedeemAmount(declaredRedeemTokens);
        }

        // burn tokens and emit a `Burn` event.
        burnInternal(address(this), redeemTokens);

        /// @custom:event RedeemCollateral
        emit RedeemCollateral(_msgSender(), redeemer, redeemAmount, redeemTokens);
    }
}
...
/// @title CygnusDeneb
/// @author CygnusDAO ([email protected])
/// @notice Deployer for collateral contracts
contract CygnusDeneb is ICygnusDeneb {
    /*           .        .          .     .          .           *    .   
            .      .                            
.        ░█▀▀█ █──█ █▀▀▀ █▀▀▄ █──█ █▀▀        *         .            
  *      ░█─── █▄▄█ █─▀█ █──█ █──█ ▀▀█             .             . 
    ...
    function deployDeneb(address lpTokenPair, address factory) external override returns (address deneb) {
        deneb = address(new CygnusCollateral{ salt: keccak256(abi.encode(lpTokenPair, factory)) }());
    }
}