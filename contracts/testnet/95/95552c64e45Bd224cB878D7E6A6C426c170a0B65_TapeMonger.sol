...
// A intégrer : require(block.timestamp >= 1640995200, "Not yet available");
...
contract TapeMonger is ERC721URIStorage, Ownable, ERC2981 {
    ...
    constructor() ERC721("TAPEMONGER", "TAPMONGER") {
        _customBaseURI = 'https://bafybeih5hacl7h4m4lxcisey4uypptxi7ibjdbf2mqkgr3ke3ltntpriay.ipfs.nftstorage.link/';
        _price = 0 ether; // 0 AVAX mint price
        _maxMintable = 69; // 30 max supply
        _royaltyAmount = 690; // 8% royalties
...
**/
    function mintPublic(uint256 quantity) public payable {
        require(block.timestamp >= 1665428400, "Not yet available"); //21h
  ...
  function mintWL(uint256 quantity) public payable isWhitelisted(msg.sender) {
        require(block.timestamp >= 1665406800, "Not yet available"); //15h 
        require(balanceOf(msg.sender) == 0, "Max Mint per wallet reached");
        require(quantity <= _maxMintPerTx, "Cannot mint that many at once.");
        require(msg.value >= (_price * quantity), "Not enough AVAX sent.");
