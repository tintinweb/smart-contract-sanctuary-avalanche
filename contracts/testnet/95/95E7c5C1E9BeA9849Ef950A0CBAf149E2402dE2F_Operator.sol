...
abstract contract AuthorizedReceiver is AuthorizedReceiverInterface {
    ...
    * @param senders The addresses of the authorized FrensOracle node
    */
    ...
    * @param sender The address of the FrensOracle node
...
abstract contract FPTokenReceiver {
    /**
    * @notice Called when FPT is sent to the contract via `transferAndCall`
    * @dev The data payload's first 2 words will be overwritten by the `sender` and `amount`
    * values to ensure correctness. Calls oracleRequest.
    * @param sender Address of the sender
    * @param amount Amount of FPT sent (specified in wei)
    ...
    */
    function onTokenTransfer(
        ...
        uint256 amount,
        bytes memory data
    ) public validateFromFPT permittedFunctionsForFPT(data) {
        assembly {
    ...
    function getFrensProtocolToken() public view virtual returns (address);
    ...
    */
    ...
    /**
    * @dev Reverts if not sent from the FPT token
    */
    modifier validateFromFPT() {
        require(msg.sender == getFrensProtocolToken(), "Must use FPT token");
        _;
    }
    ...
    */
    modifier permittedFunctionsForFPT(bytes memory data) {
        bytes4 funcSelector;
        assembly {
            // solhint-disable-next-line avoid-low-level-calls
...
interface FrensProtocolRequestInterface {
...
interface OperatorInterface is FrensProtocolRequestInterface, OracleInterface {
...
interface WithdrawalInterface {
    /**
    * @notice transfer FPT held by the contract belonging to msg.sender to
    * another address
    * @param recipient is the address to send the FPT to
    * @param amount is the amount of FPT to send
    */
    function withdraw(address recipient, uint256 amount) external;

    /**
    * @notice query the available amount of FPT to withdraw by msg.sender
    */
    function withdrawable() external view returns (uint256);
}
...
/**
 * @title The FrensProtocol Operator contract
 * @notice Node operators can deploy this contract to fulfill requests sent to them
 */
contract Operator is AuthorizedReceiver, ConfirmedOwner, FPTokenReceiver, OperatorInterface, WithdrawalInterface {
    using Address for address;
    using SafeMathFrensProtocol for uint256;
    ...
    FPTokenInterface internal immutable fpToken;
    ...
    /**
    * @notice Deploy with the address of the FPT token
    * @dev Sets the FPToken address for the imported FPTokenInterface
    * @param fpt The address of the FPT token
    * @param owner The address of the owner
    */
    constructor(address fpt, address owner) ConfirmedOwner(owner) {
        fpToken = FPTokenInterface(fpt); // external but already deployed and unalterable
    }
    ...
    */
    ...
    /**
    * @notice Creates the FrensProtocol request. This is a backwards compatible API
    ...
    * @param specId The Query Specification ID
    ...
    */
    function oracleRequest(
        ...
        uint256 dataVersion,
        bytes calldata data
    ) external override validateFromFPT {
    ...
    /**
    * @notice Creates the FrensProtocol request
    * @dev Stores the hash of the params as the on-chain commitment for the request.
    * Emits OracleRequest event for the FrensOracle node to detect.
    ...
    */
    ...
    /**
    * @notice Called by the FrensOracle node to fulfill requests
    ...
    */
    ...
    /**
    * @notice Called by the FrensOracle node to fulfill requests with multi-word support
    * @dev Given params must hash back to the commitment stored from `oracleRequest`.
    * Will call the callback address' callback function without bubbling up error
    * checking in a `require` so that the node can get paid.
    * @param requestId The fulfillment request ID that must match the requester's
    ...
    */
    ...
    /**
    * @notice Allows the node operator to withdraw earned FPT to a given address
    * @dev The owner of the contract can be another wallet and does not have to be a FrensOracle node
    * @param recipient The address to send the FPT token to
    ...
    */
    ...
    {
        assert(fpToken.transfer(recipient, amount));
    }

    /**
    * @notice Displays the amount of FPT that is available for the node operator to withdraw
    * @dev We use `ONE_FOR_CONSISTENT_GAS_COST` in place of 0 in storage
    * @return The amount of withdrawable FPT on the contract
    */
    function withdrawable() external view override(OracleInterface, WithdrawalInterface) returns (uint256) {
        //return _fundsAvailable();
        return fpToken.balanceOf(address(this)).sub(ONE_FOR_CONSISTENT_GAS_COST);
    }
    ...
    */
    function ownerForward(address to, bytes calldata data) external onlyOwner validateNotToFPT(to) {
    ...
    /**
    * @notice Interact with other FPTokenReceiver contracts by calling transferAndCall
    ...
    */
    function ownerTransferAndCall(
        ...
        return fpToken.transferAndCall(to, value, data);
    }
    ...
    * Will transfer the FPT sent for the request back to the recipient address.
    ...
    */
    function cancelOracleRequest(
        ...
        fpToken.transfer(msg.sender, payment);
    }
    ...
    */
    ...
    /**
    * @notice Returns the address of the FPT token
    * @dev This is the public implementation for FrensProtocolTokenAddress, which is
    * an internal method of the FrensProtocolClient contract
    */
    function getFrensProtocolToken() public view override returns (address) {
        return address(fpToken);
    }
    ...
    */
    function _verifyAndProcessOracleRequest(
        ...
        uint256 nonce,
        uint256 dataVersion
    ) private validateNotToFPT(callbackAddress) returns (bytes32 requestId, uint256 expiration) {
    ...
    */
    ...
    /**
    * @notice Returns the FPT available in this contract, not locked in escrow
    * @return uint256 FPT tokens available
    */
    function _fundsAvailable() private view returns (uint256) {
        uint256 inEscrow = s_tokensInEscrow.sub(ONE_FOR_CONSISTENT_GAS_COST);
        return fpToken.balanceOf(address(this)).sub(inEscrow);
    }
    ...
    */
    ...
    /**
    * @dev Reverts if the callback address is the FPT token
    * @param to The callback address
    */
    modifier validateNotToFPT(address to) {
        require(to != address(fpToken), "Cannot call to FPT");
        _;
    }
