...
contract Multiworking is Ownable {
    ...
    function mintPublic(
      ...
      uint256 amount
        ...
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.mintPublic(amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function transferFrom(
      ...
      address from, address to, uint256 tokenId
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, from, to, tokenId));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.transferFrom(from, to, tokenId);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
