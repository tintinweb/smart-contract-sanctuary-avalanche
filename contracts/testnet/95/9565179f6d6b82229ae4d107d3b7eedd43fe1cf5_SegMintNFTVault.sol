...
contract SegMintNFTVault is ERC721Holder, ERC1155Holder, ISegMintVault, MultiSigStruct {
    //////////////////////
    ////    Fields    ////
    //////////////////////
    ...
    // Mapping from contractAddress => TokenID => Fractioning Info
    mapping(address => mapping(uint256 => LOCKINGINFO))
        private _lockers;
    ...
    modifier notLockedNFT(address contractAddress, uint256 tokenId) {
        ...
        require(
            _lockerOf(contractAddress, tokenId) == address(0),
            "NFT is locked! Status : Locked!"
        );
        _;
    }
    ...
    function approveLocker(
        BatchNFTsStruct[] memory _data,
        address approvedLockerAddress
    )   
        public 
        onlyOwner 
        notNullAddress(approvedLockerAddress, "Fractioner address")
    {
        require(
            msg.sender != approvedLockerAddress,
            "NFT VAULT: locker approval to owner wallet address!"
        );
        for(uint256 i=0; i<_data.length; i++){
            // contract address for this batch
            address contractAddress = _data[i].contractAddress;
            // token Ids for this batch
            uint256[] memory tokenIds = _data[i].tokenIds;

            for(uint256 j=0; j<tokenIds.length; j++){
                // tokenId
                uint256 tokenId = tokenIds[j];
                _approveLocker(approvedLockerAddress, contractAddress, tokenId);
            }
        }
    }
        ...
        notLockedNFT(contractAddress, tokenId)
    {
        ...
        require(
            ...
            "TokenId is already approved for a locker"
        );

        // update approved locker address
        _lockersApproval[contractAddress][tokenId] = approvedLockerAddress;
    }
    ...
    // batch import NFTs 
    function batchImportNFTs(BatchNFTsStruct[] memory importData) public onlyOwner {
        for(uint256 i=0; i<importData.length; i++){
            // contract address for this batch
            address contractAddress = importData[i].contractAddress;
            // token Ids for this batch
            uint256[] memory tokenIds = importData[i].tokenIds;
    ...
    function batchLockNFTs(BatchNFTsStruct[] memory _lockData)  public {
        for(uint256 i = 0; i< _lockData.length; i++) {
            address contractAddress = _lockData[i].contractAddress;
            for(uint256 j = 0; j<_lockData[i].tokenIds.length; j++) {
                uint256 tokenId = _lockData[i].tokenIds[j];
                require(_lockersApproval[contractAddress][tokenId] == msg.sender, "Sender is not the approved locker");
                // update _locker
                _lockers[contractAddress][tokenId].lockerAddress = msg.sender;
                _lockers[contractAddress][tokenId].lockingDate = block.timestamp;
                
                // update lockerApproval to address(0)
                _lockersApproval[contractAddress][tokenId] = address(0);
            }
        }
        emit BatchLockEvent(msg.sender, _lockData, block.timestamp);
    ...
    function batchUnlockNFTs(BatchNFTsStruct[] memory _lockData)  public {
        for(uint256 i = 0; i< _lockData.length; i++) {
            ...
            for(uint256 j = 0; j<_lockData[i].tokenIds.length; j++) {
                uint256 tokenId = _lockData[i].tokenIds[j];
                require(_lockers[contractAddress][tokenId].lockerAddress == msg.sender, "Sender is not the locker");
                // update _locker
                _lockers[contractAddress][tokenId].lockerAddress = address(0);
                _lockers[contractAddress][tokenId].lockingDate = 0;
            }
            emit BatchLockEvent(msg.sender, _lockData, block.timestamp);
        }        
    }
    ...
    {
        // return
        return _lockerOf(contractAddress, tokenId) != address(0);
    }
    ...
    // _locker of
    function _lockerOf(address contractAddress, uint256 tokenId)
        internal
        view
        virtual
        returns (address)
    {
        return _lockers[contractAddress][tokenId].lockerAddress;
    }
    // approve fractioner
