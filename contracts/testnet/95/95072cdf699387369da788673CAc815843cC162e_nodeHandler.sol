...
contract nodeHandler is Ownable, Pausable {
    ...
    uint256 public boostRewardsPerNode = 1425;
    ...
    modifier onlyGuard() {
        require(owner() == _msgSender() || token == _msgSender() || presale == _msgSender() || airdrop == _msgSender(), "NOT_GUARD");
        _;
    }
    ...
    function _calculateNodeRewards(uint _lastClaimTime, uint256 amount_) private view returns (uint256 rewards) {
        ...
        uint256 rewardPerDay = amount_.mul(avRewardsPerNode).div(10000);
    ...
    // Are we cashing out existing rewards here ? since we are updating the lastClaimTime
        ...
        returns (string memory)
    {
        ...
        require(nodes.length > 0, "You need to have atleast 1 Node");
    ...
    function updateReward(uint8[] memory newVal) external onlyOwner {
        require(newVal.length == 2, "Wrong length");
        boostRewardsPerNode = newVal[0];
        rewardPerNode = newVal[1];
    }


    function updateBoostTime(uint256 _time)external onlyOwner{
        boostEndTime = block.timestamp + _time * 3600;
    }
