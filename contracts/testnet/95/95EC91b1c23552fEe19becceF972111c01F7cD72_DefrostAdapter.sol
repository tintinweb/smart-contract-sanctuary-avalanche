...
import {ICollateralVault} from "../../interfaces/defrost/ICollateralVault.sol";
import {ISmeltSavingsFarm} from "../../interfaces/defrost/ISmeltSavingsFarm.sol";

contract DefrostAdapter is AdapterBase, Basic {

    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "Defrost")
    {}

    /// @dev add collateral
    function join(bytes calldata encodedData) external onlyProxy {
    ...
    /// @dev mint H2O
    function mintSystemCoin(bytes calldata encodedData) external onlyProxy {
        (address stakePoolAddr, uint256 amount) = abi.decode(
            encodedData,
            (address, uint256)
        );
    ...
    function joinAndMint(bytes calldata encodedData) external onlyProxy {
        (
    ...
    function joinAndMintAVAX(bytes calldata encodedData) external onlyProxy {
        (
            address stakePoolAddr,
            uint256 collateralAmount,
            uint256 systemCoinAmount
    ...
    /// @dev repay H2O
    function repaySystemCoin(bytes calldata encodedData) external onlyProxy {
        (address stakePoolAddr, uint256 amount) = abi.decode(
            encodedData,
            (address, uint256)
        );
...
interface ICollateralVault {
    function assetCeiling() external view returns (uint256);

    function assetFloor() external view returns (uint256);

    function assetInfoMap(address)
        external
        view
        returns (
            uint256 originAsset,
    ...
    function collateralBalances(address) external view returns (uint256);

    function collateralRate() external view returns (uint256);

    function collateralToken() external view returns (address);

    function emergencyExit(address account) external;

    function emergencyStart() external view returns (uint256);
    ...
    function getMaxMintAmount(address account, uint256 newAddCollateral)
        external
        view
        returns (uint256);

    function getMultiSignatureAddress() external view returns (address);

    function getOracleAddress() external view returns (address);

    function initContract(
        int256 _stabilityFee,
    ...
    function joinAndMint(uint256 collateralamount, uint256 systemCoinAmount)
        external
        payable;
    ...
    function setPoolLimitation(uint256 _assetCeiling, uint256 _assetFloor)
        external;

    function setStabilityFee(int256 stabilityFee, uint256 feeInterval) external;

    function systemCoin() external view returns (address);

    function totalAssetAmount() external view returns (uint256);

    function transferOrigin(address _oldOrigin, address _newOrigin) external;
    ...
    function txOrigin()
        external
        view
        returns (address _origin0, address _origin1);
...
interface ISmeltSavingsFarm {
    function allPendingReward(address _account)
        external
        view
    ...
    function getMeltAmount(uint256 _smeltAmount)
        external
        view
        returns (uint256);
    ...
    function h2o() external view returns (address);
    ...
    function melt() external view returns (address);
    ...
    function setFarmFlag(bool flag) external;

    function setFarmTime(uint256 _startime, uint256 _endtime) external;
    ...
    function setMineRate(uint256 _reward, uint256 _duration) external;

    function smelt() external view returns (address);

    function tokenFarm() external view returns (address);

    function totalAssetAmount() external view returns (uint256);

    function totalStaked() external view returns (uint256);

    function totalStakedFor(address _account) external view returns (uint256);
    ...
    function withdraw(uint256 _smeltAmount) external;
}
