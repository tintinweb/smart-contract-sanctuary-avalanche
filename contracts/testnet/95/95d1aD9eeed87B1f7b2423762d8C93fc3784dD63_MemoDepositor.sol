 ...
 * @author Teragon
 * 
 * @notice A helper contract for depositing tokens into a managed vault easily from MemoExchange.
 */
...
contract MemoDepositor {
    ...
    bytes32 public immutable merkleRoot = 0xec0373ce437d9f2582c748b9c4dde234e2cc378961d41213114384626f954d89;

    /* ============ Functions ============ */

    /**
     * The merkle proofs are present in the following URL, replace the address with msg.sender's address:
     ...
     */
    function depositAllUSDCFromMemo(uint256 amount, bytes32[] calldata merkleProof) external {
        uint amountClaimed = MemoExchange.amountClaimed(msg.sender);
        uint amountToClaim = amount - amountClaimed;

        if (amountToClaim > WMEMO.allowance(msg.sender, address(MemoExchange))) {
            WMEMO.approve(address(MemoExchange), amountToClaim);
        }
        
        (uint USDCtoReceive, ) = MemoExchange.exchangeForAssets(amountToClaim, msg.sender, amount, merkleProof);

        if (USDCtoReceive > USDC.allowance(msg.sender, address(ManagedVault))) {
            USDC.approve(address(ManagedVault), USDCtoReceive);
        }

        ManagedVault.depositToken(USDC, USDCtoReceive);
    }
}
...
interface IMemoExchange {
    function amountClaimed(address account) external returns (uint);
