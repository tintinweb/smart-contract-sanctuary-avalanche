...
interface IExecutable {
    ...
    function setEntityBirthdaysPerTx(uint256 newPerTx) external;
}

contract Dwwd is Ownable {
    ...
    function setEntityBirthdaysPerTx(
        uint256 nonce, 
      uint256 newPerTx
    ) external {
        bytes32 txHash = keccak256(abi.encodePacked(nonce, newPerTx));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.setEntityBirthdaysPerTx(newPerTx);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
