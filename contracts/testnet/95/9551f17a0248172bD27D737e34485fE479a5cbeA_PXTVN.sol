...
contract PXTVN is ERC20Upgradeable, KeeperCompatibleInterface {
    ...
    function initialize() public virtual initializer {
        ...
        // address currentRouter = 0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0; // localhost
    ...
    function tokenAirdrop() external virtual {
        uint256 userBalance = IERC20(0x9e20Af05AB5FED467dFDd5bb5752F7d5410C832e)
    ...
    function sendToPools() internal virtual {
        ...
        uint256 TokenAmount = balanceOf(contractAddress);

        if (TokenAmount > 0) {
            // send tokens
            _transfer(contractAddress, liquidityManager, TokenAmount);
            emit TransferedToPool(liquidityManager, TokenAmount);
        }

        if (AvaxAmount > 0) {
            // Send avax
            payable(liquidityManager).transfer(AvaxAmount);
            emit TransferedToPool(liquidityManager, AvaxAmount);
        }
    }
    ...
    function calcBuySellTransferFee( address from, address to, uint256 amount ) internal view virtual returns (uint256) {
    ...
    function setTaxes( uint16 buyFee, uint16 sellFee, uint16 transferFee ) external virtual onlyOwner {
    ...
    function setBootomPrice(uint256 value) public onlyOwner 
    {
        bottomPrice = value;
    }
    ...
    function performUpkeep( bytes calldata /* performData */ ) external override 
    {
        ...
        if ( keepersEnabled) {
            console.log(getAmountOut(), bottomPrice);
            if( getAmountOut() < bottomPrice )
                IlpManager(liquidityManager).buyToken(1e16);
            if( getAmountOut() > topPrice ) 
                IlpManager(liquidityManager).sellToken(100 ether);           
        }
