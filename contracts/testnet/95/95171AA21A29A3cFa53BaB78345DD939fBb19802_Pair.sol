...
contract Pair is IPair, Initializable {
    ...
    function initialize(address _token0, address _token1, bool _stable) external initializer {
        ...
        if (_stable) {
            name = string(abi.encodePacked("Stable sAMM - ", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol()));
            symbol = string(abi.encodePacked("ssAMM-", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol()));
        } else {
            name = string(
                abi.encodePacked("Volatile sAMM - ", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol())
            );
            symbol = string(abi.encodePacked("vsAMM-", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol()));
        }
