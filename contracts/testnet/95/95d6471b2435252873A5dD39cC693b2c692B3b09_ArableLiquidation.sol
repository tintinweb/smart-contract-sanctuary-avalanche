...
contract ArableLiquidation is Ownable {
     ...
     */
    uint private _liquidationRate;
    uint private _immediateLiquidationRate;
    uint private _liquidationDelay;
    uint private _liquidationPenalty;
    ...
    constructor (address addressRegistry) {
        ...
        setImmediateLiquidationRate(1.2 ether); // 120%
        setLiquidationPenalty(1.1 ether); // 110%
        setLiquidationDelay(1800); // 30min - TODO: update to 3 days for mainnet
    }
    ...
    modifier onlyFlaggable(address user) {
        ...
        require(calculateLiquidationRate(user) >= _liquidationRate, "User has enough collateral");
        _;
    }
    ...
    function isFlaggable(address user) public view returns (bool) {
        ...
        // Calculate if user passed the threshold to be flagged
        if(calculateLiquidationRate(user) < _liquidationRate) {
            return false;
        }
        return true;
    }
    ...
    /**
     * @notice function to return the user's liquidation rate
     * this function is used to calculate if user is flaggable or not
     *
     * @param user address of the user
     *
     * @return uint liquidation rate in decimals
     */
    function calculateLiquidationRate(address user) public view returns (uint) {
     ...
     * and if the `userLiquidationRate_` is not very high,
    ...
    function liquidate(address user) public {
        ...
        uint userLiquidationRate_ = calculateLiquidationRate(user);
        // Check threshold
        require(userLiquidationRate_ >= _liquidationRate, "User cant be liquidated");
        // Liquidate immediately if user is above the threshold
        if(userLiquidationRate_ >= _immediateLiquidationRate) {
