...
contract BridgesOfFate is Ownable {
    ...
    function generateRandomTicketNumbers(uint256 _lotteryCount,uint256 _range,uint256 _length) public view returns (uint8[6] memory) {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            // Generate a new number while it is a duplicate, up to 5 times (to prevent errors and infinite loops)
            while (!readyToAdd && retry <= maxRetry) {
                uint256 _packedNumber = uint256(keccak256(abi.encodePacked(msg.sender,block.timestamp,i,retry,_lotteryCount)));
                generatedNumber = (_packedNumber % _range) + 1;
                bool isDuplicate = false;
    ...
    function participateInGame(bool _jumpSide,bytes32 playerId) public GameEndRules NarrowingGameEndRules {
        ...
        if (GameMap[_currentUserStage] <= 0) {
            ...
            // uint256 _packedNumber = uint256(keccak256(abi.encodePacked(block.timestamp, address(this), msg.sender))) % 100;
            // if(_packedNumber > 0){
            //     _packedNumber  =  _packedNumber * 1e9; 
            // }else{
            //     _packedNumber  =  1 * 1e9;
            // }
            GameMap[_globalStageNumber] = _packedNumber[0];
            ...
            _narrowingGameMap = _packedNumber[1];
    ...
    function _selectPlayerAndSetCapacityOnTile(uint256 _currentPlayerStage,uint256 currentDay,bool _jumpSide) internal {
        ...
        if (_currentPlayerStage > 0) {
            ...
            if (_selectJumpedSidePlayersId.length > _titleCapacityAgainestStage) {
                ...
                _randomSelectPlayerIndex = generateRandomTicketNumbers(_selectUnholdPlayerId.length - 1,_selectUnholdPlayerId.length - 1,_titleCapacityAgainestStage);
