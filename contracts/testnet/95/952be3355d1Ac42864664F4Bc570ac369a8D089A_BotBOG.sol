...
contract BotBOG is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string private baseURI;
  uint256 public maxSupply;
  uint256 private _offset = 1; // to make tokenId start from 1

  constructor(
    string memory _initBaseURI,
    uint256 _amount
  ) ERC721("Battle of the Blockchains OG", "BotBOG") {
    setBaseURI(_initBaseURI);
    setMaxSupply(_amount);
  }

  // public
  function mintTo(uint256[] calldata quantity, address[] calldata recipient) public payable onlyOwner {
    require(
    ...
    require(
      supply + totalQuantity <= maxSupply + _offset,
      "Mint exceeds supply"
    );
    require(totalQuantity > 0, "Mint must be more than 1");
