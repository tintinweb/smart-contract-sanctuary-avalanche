...
contract Ekokraft is Initializable, UUPSUpgradeable, OwnableUpgradeable, SignatureUtils, ReentrancyGuardUpgradeable{
   /**
   ...
   */
   ...
   address public signer; // signer address who is responsible for signing the unlock transaction 
   ...
   function burn(
      ...
      // Burn token
      require(IERC20Ekokaft(curToken_).allowance(msg.sender, address(this)) >= amount_, "Mint: Transfer amount exceeds allowance");
      IERC20Ekokaft(curToken_).burnFrom(
         msg.sender,
         amount_
      );
 ...
 */
interface IERC20Ekokaft is IERC20Upgradeable{

    function mint(address account, uint256 amount) external;

    function burnFrom(address aaccount, uint256 amount) external;
