...
/*  ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
    ...
    ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
     ...
     loss from liquidations if the debt ratio goes above limit allowed.
...
contract CygnusTerminal is ICygnusTerminal, Erc20Permit {
     ...
     */
    uint64 internal constant initialExchangeRate = 1e18;
    ...
    /**
     *  @notice Updates the total balance storage var in terms of its underlying (LP Token or borrow token)
    ...
    function exchangeRate() public virtual override returns (uint256) {
        ...
        // If there is no supply for this token return initial, else totalBalance * 1e18 / totalSupply
    ...
    /**
     *  @notice This low level function should only be called by `Altair` contract
     *  @inheritdoc ICygnusTerminal
     */
 ...
 *  @author CygnusDAO
 *  @notice Factory initializes Borrow Arm (Albireo)
 *  @dev Main contract for controlling borrow settings like interest base rate, kink utilization, etc.
 */
contract CygnusBorrowControl is ICygnusBorrowControl, CygnusTerminal {
     ...
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public constant override CYGNUS_BORROW_FEE = 0.001e18;
     ...
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public override kink = 0.85e18;
     ...
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public constant override baseRateMax = 0.10e18;

    /**
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public constant override kinkUtilizationRateMin = 0.50e18;

    /**
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public constant override kinkUtilizationRateMax = 0.95e18;

    /**
     *  @inheritdoc ICygnusBorrowControl
     */
    uint256 public constant override reserveFactorMax = 0.20e18;
    ...
    /**
     *  @notice Creates the Borrow arm of the pool and passes to CygnusTerminal the Cygnus borrow token
     */
    constructor()
        /**
         *  @param borrowName The name of ERC20 borrow token
         *  @param borrowSymbol The symbol of ERC20 borrow token
         *  @param borrowDecimals Decimals used (always 18).
         */
        CygnusTerminal(borrowName, borrowSymbol, borrowDecimals)
    {
        // Get important addresses from borrow deployer
        (hangar18, underlying, collateral) = ICygnusAlbireo(_msgSender()).borrowParameters();

        // Match initial exchange rate
        exchangeRateStored = initialExchangeRate;
    ...
    function setCygnusBorrowTracker(address newBorrowTracker) external override cygnusAdmin nonReentrant {
        ...
        emit NewCygnusBorrowTracker(newBorrowTracker);
    }
    ...
    function setReserveFactor(uint256 newReserveFactor) external override cygnusAdmin nonReentrant {
        ...
        validRange(0, reserveFactorMax, newReserveFactor);
    ...
    function setKinkUtilizationRate(uint256 newKinkUtilizationRate) external override cygnusAdmin nonReentrant {
        ...
        validRange(kinkUtilizationRateMin, kinkUtilizationRateMax, newKinkUtilizationRate);

        // It is, update kink utilization rate
...
contract CygnusBorrowInterest is ICygnusBorrowInterest, CygnusBorrowControl {
     ...
     */
    uint256 public constant override secondsPerYear = 31536000;
    ...
    function getBorrowRate(
        ...
        // If utilization > kink return normal rate + kink rate
        else {
    ...
    function updateJumpRateModelInternal(
        ...
        validRange(0, baseRateMax, baseRatePerYear);

        // Internal parameter check for the Kink point to not be below minimum or above maximum allowed.
        validRange(kinkUtilizationRateMin, kinkUtilizationRateMax, kink_);
        ...
        multiplierPerSecond = PRBMath.mulDiv(multiplierPerYear, 1e18, (secondsPerYear * kink_));
...
contract CygnusBorrowTracker is ICygnusBorrowTracker, CygnusBorrowInterest, CygnusBorrowApprove {
    ...
    function getBorrowBalance(address borrower) public view override returns (uint256) {
        /// @custom:error Avoid zero address
        ...
        // If interestIndex = 0 then borrowBalance is 0, return 0 instead of fail division by 0
...
contract CygnusBorrow is ICygnusBorrow, CygnusBorrowTracker {
    ...
    function mintBorrowInternal(uint256 _exchangeRate, uint256 _totalSupply) internal returns (uint256) {
        ...
        if (_exchangeRate > _exchangeRateLast) {
            uint256 _exchangeRateNew = _exchangeRate - ((_exchangeRate - _exchangeRateLast).mul(reserveFactor));
    ...
    /**
     *  @notice This low level function should only be called from `Altair` contract only.
    ...
    function sync() external override nonReentrant accrue {}
}


// File contracts/cygnus-core/CygnusAlbireo.sol
 ...
 *  @title CygnusAlbireo
 *  @author CygnusDAO
 *  @notice This is the Borrow Deployer contract which starts both borrow arms of the lending pool
 */
contract CygnusAlbireo is ICygnusAlbireo, Context {
    /*  ─────────────────────────────────────────────── Public ────────────────────────────────────────────────  */
    ...
    function deployAlbireo(
        ...
        address collateral,
        uint8 albireoIndex
    ) external override returns (address albireo) {
        // Assign important addresses to pass to borrow contracts
        borrowParameters = BorrowParameters({ factory: _msgSender(), underlying: underlying, collateral: collateral });

        // Create Borrow contract
        albireo = address(new CygnusBorrow{ salt: keccak256(abi.encode(collateral, _msgSender(), albireoIndex)) }());
