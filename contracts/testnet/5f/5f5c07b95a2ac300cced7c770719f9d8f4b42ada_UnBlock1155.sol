    ...
    ERC1155BurnableUpgradeable
{
    ...
    function minting(
        ...
        for (uint256 i = 0; i < ipfsmetadata.length; i++) {
            id = id.add(1);
            _creator[id] = address(this);
            _mint(address(this), id, supply[i], "");
            _setTokenURI(id, ipfsmetadata[i]);
            _setTokenRoyalty(id, address(this), uint96(royal[i].div(1e16)));
            _royal[id] = royal[i];
            if (price[i] > 0) {
    ...
    function nftSubscription(uint256 tokenId) public payable {
        ...
        userSubscrption memory _subUser;
        _subUser.tokenId = tokenId;
        _subUser.subPrice = _subscribeNFT[tokenId].subPrice;
        _subUser.subStartPeriod = block.timestamp;
        _subUser.subEndPeriod = block.timestamp.add(
