...
bool private _inSwapAndLiquify;
    ...
    function initialize() public initializer {
        ...
        //setExcludedFromFee(address(this));
        //setExcludedFromFee(operationPoolAddress);
        //setExcludedFromFee(tresuryPoolAddress);
        ...
        IJoeRouter02 _dexRouter = IJoeRouter02(0x2D99ABD9008Dc933ff5c0CD271B88309593aB921); // fuji pangoling
        dexRouter = _dexRouter;
        swapTokenAddress = dexRouter.WAVAX();
        //lpPair = IUniswapV2Factory(_dexRouter.factory()).createPair(_dexRouter.WAVAX(),address(this));
        //lpPairs[lpPair] = true;
    ...
    function initialMint() public onlyOwner {
        require(!alreadyMinted, "Tokens already minted!");
        _mint(msg.sender, 1000000e18);
        alreadyMinted = true;
    }
    */

    function createPair(address add1, address add2) external onlyOwner {
	...
	function setRatios(uint16 _operationRatio, uint16 _treasuryRatio) external onlyOwner {
        require((_operationRatio + _treasuryRatio) <= _ratios.total, "MAX RATIOS EXCEEDED 100%");
   ...
   /**
    ...
    function checkAntiBot(address from, address to) internal {
        // blacklist
        require(
    ...
    function calculateWhaleFee(uint256 amount) public view returns (uint256) {
        ...
        uint256 avaxAmount = getOutEstimatedTokensForTokens(address(this), swapTokenAddress, amount);
    ...
    function calcBuySellTransferFee(address from, address to, uint256 amount) internal view returns (uint256) {
        ...
        if (from == lpPair) {
            ...
            //totalFeePercent += antiWhaleAmount;
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        // swap
        swapTokensForAVAX(numTokensToSwap);

         // transfer to team wallets
    ...
    function _checkliquidity(address from, address to) private {
    ...
    */
    ...
    function withdrawStuckTokens1(uint256 amount) public onlyOwner {
        require(balanceOf(address(this)) > 0, "Contract balance is zero");
        if (amount > balanceOf(address(this))) {
            amount = balanceOf(address(this));
        }
