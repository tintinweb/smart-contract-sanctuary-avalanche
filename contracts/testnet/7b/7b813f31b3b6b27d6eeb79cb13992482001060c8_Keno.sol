...
/// Keno; select up to 10 numbers from 0-39, payout based on number of matches and number of selections.
contract Keno is GameBase, GameBet, GameChainlink {
    enum RoundState {
        UNKNOWN,
        LOST,
        TIE,
    ...
    mapping(uint256 => bool[40]) public choicesByGameId;

    /// Multiplier for each number of matches.
    mapping(uint8 => uint256[]) public matchMultipliers;

    /// Contract can be funded.
    receive() external payable {}

    /// Constructor populates generic Chainlink values.
    /// Also sets static multipliers for winning.
    constructor(
        ...
        matchMultipliers[1] = [40, 275];
        matchMultipliers[2] = [0, 190, 450];
        matchMultipliers[3] = [0, 100, 310, 1040];
        matchMultipliers[4] = [0, 80, 180, 500, 2250];
        matchMultipliers[5] = [0, 25, 140, 410, 1650, 3600];
        matchMultipliers[6] = [0, 0, 100, 368, 700, 1650, 4000];
        matchMultipliers[7] = [0, 0, 47, 300, 450, 1400, 3100, 6000];
        matchMultipliers[8] = [0, 0, 0, 220, 400, 1300, 2200, 5500, 7000];
        matchMultipliers[9] = [0, 0, 0, 155, 300, 800, 1500, 4400, 6000, 8500];
        matchMultipliers[10] = [0, 0, 0, 140, 225, 450, 800, 1700, 5000, 8000, 10000];
    }

    /// Create a new game.
    function playGame(
        uint8 numRounds,
        bool[40] memory userChoices
    ) external payable isEOA isNotPaused isNotBlacklist returns (uint256 gameId) {
        uint8 numSelected = countSelected(userChoices);
        require(numSelected >= 1, "Must select at least one value");
        require(numSelected <= 10, "Can make 10 selections maximum");

        IRakeDistributor rakeDistributor = IRakeDistributor(rakeDistributorAddress);
        BetDetails memory details = validateBetWithPayout(
            treasuryAddress.balance,
            rakeDistributor.getTotalRake(),
            numRounds,
            matchMultipliers[numSelected][numSelected],
            100
        );
        ...
        choicesByGameId[fi.gameId] = userChoices;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        bool[40] memory values = choicesByGameId[gameId];
        uint8 numSelected = countSelected(values);
        uint8 numRounds = fi.numRounds;

        RoundInfo[] memory results = new RoundInfo[](fi.numRounds);
        uint256 amountToUser = 0;

        for (uint i = 0; i < numRounds; ++i) {
            uint8[10] memory selected = generateNumbers(randomWords[0], i);
            results[i].selected = selected;

            uint256 numMatched = 0;
            for (uint j = 0; j < 10; ++j) {
                if (values[selected[j]]) {
                    ++numMatched;
                }
            }

            uint256 multiplier = matchMultipliers[numSelected][numMatched];
            if (multiplier > 100) {
                results[i].state = RoundState.WON;
            } else if (multiplier < 100) {
                results[i].state = RoundState.LOST;
            } else {
                results[i].state = RoundState.TIE;
            }

            uint256 payout = fi.betPerRound *  multiplier / 100;
            results[i].sent = payout;
            amountToUser += payout;
        }
        ...
        emit WinnerPicked(gameId, fi.subgame, results);
    }
    ...
    /// Given an array of keno choices, determine how many of them the user selected (by setting value to true).
    function countSelected(bool[40] memory userChoices) public pure returns (uint8) {
        uint8 numSelected = 0;
    ...
    /// Provided two seed values (the chainlink random value and the round number) uses a Fisher-Yates shuffle
    /// to generate an array of 10 random numbers from 0-39.
    function generateNumbers(uint256 seed1, uint256 seed2) internal pure returns(uint8[10] memory) {
        uint8[40] memory numbers;
        for(uint8 i = 0; i < 40; i++) {
            numbers[i] = i;
        }

        uint8[10] memory randomNumbers;
        for(uint8 i = 0; i < 10; ++i) {
            uint8 randomIndex = uint8(uint256(keccak256(abi.encodePacked(seed1, seed2, i))) % (40 - i)) + i;
...
abstract contract GameBet is Ownable {
    ...
    function validateBet(uint256 treasuryBalance, uint256 totalRake, uint numRounds) internal returns(BetDetails memory) {
    ...
    function validateBetWithPayout(uint256 treasuryBalance, uint256 totalRake, uint256 numRounds,
        ...
        uint256 payoutPerRound = betPerRound * betNumerator / betDenominator;
        require(payoutPerRound - betPerRound <= (treasuryBalance / maxBetDivisor), "Gamble less, king");
