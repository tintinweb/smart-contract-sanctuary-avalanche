...
contract LnExchangeSystem is LnAdminUpgradeable, LnAddressCache {
    ...
    bytes32 private constant sETH_KEY = "sETH";
    ...
    function _exchange(
        ...
        if (exitPositionOnly) {
            require(destKey == sETH_KEY, "LnExchangeSystem: can only exit position");
        }
    ...
    function _settle(uint256 pendingExchangeEntryId, address settler) private {
        ...
        uint feeForPoolInETH = mPrices.exchange(exchangeEntry.toCurrency, fee, mPrices.sETH());
        ...
        if (foundationFeeHolder == address(0)) {
            ...
            if (splitRatio == 0) {
                ...
                foundationSplit = feeForPoolInETH.multiplyDecimal(splitRatio);
                feeForPoolInETH = feeForPoolInETH.sub(foundationSplit);
            }
        }

        ILnAsset sETH =
            ILnAsset(mAssets.getAddressWithRequire(mPrices.sETH(), "LnExchangeSystem: failed to get sETH address"));

        if (feeForPoolInETH > 0) sETH.mint(mRewardSys, feeForPoolInETH);
        if (foundationSplit > 0) sETH.mint(foundationFeeHolder, foundationSplit);
        ...
        emit PendingExchangeSettled(exchangeEntry.id, settler, destRecived, feeForPoolInETH, foundationSplit);
    }
