...
import '../interfaces/IRoundtableFactory.sol';
...
contract PfpStaker is IPfpStaker, ReentrancyGuard, Owned, NFTreceiver {
	...
	address public roundtableFactory;
	...
	constructor(address shieldContract_, address roundtableFactory_) Owned(msg.sender) {
		shieldContract = shieldContract_;

		roundtableFactory = roundtableFactory_;
	...
	// Set Roundtable Factory - this address can set stakers directly. This happens on multisig creation.
	function setRoundtableFactory(address roundtableFactory_) external onlyOwner {
		roundtableFactory = roundtableFactory_;
	}

	// Update shieldContract
	function setShieldContract(address shieldContract_) external onlyOwner {
		shieldContract = shieldContract_;
		enabledPfpContracts[shieldContract_] = true;
	}

	// Restrict the NFT collections that can be used as PFPs
	function setRestrictedContracts() external onlyOwner {
		restrictedContracts = !restrictedContracts;
	}

	// For adding allowed NFT collections
	...
	// Stake initial shield - direct from factory
	function stakeInitialShield(address recipient, uint256 shieldId) external {
		if (msg.sender != roundtableFactory) revert Unauthorised();
	...
	// Stake NFT to use as group pfp
	function stakeNFT(
		address staker,
		address NFTContract_,
		uint256 tokenId
	) external {
		// If input NFTContract_ is address(0), then use the shield contract.
		// This saves us having to know the shield contract address in the multisig.
		address NFTContract = NFTContract_ == address(0) ? shieldContract : NFTContract_;
		...
		if (IERC721(NFTContract).supportsInterface(ERC721_METADATA)) {
			...
			IERC721(NFTContract).safeTransferFrom(staker, address(this), tokenId);
			...
			if (IERC1155(NFTContract).supportsInterface(ERC1155_METADATA)) {
				...
				IERC1155(NFTContract).safeTransferFrom(staker, address(this), tokenId, 1, '');
				stakes[staker] = StakedPFP(NFTContract, tokenId);
	...
	function getURI() external view returns (string memory nftURI) {
		StakedPFP memory stake = stakes[msg.sender];
	...
	// Return token staked by address
...
/// @notice Roundtable Factory Interface.
