...
contract CoinFlip is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        emit Outcome(Game.COIN_FLIP, result, msg.sender, value);

        delete _pendingCommits[msg.sender][nonce];

        if (result) {
            (bool ok2, ) = payable(msg.sender).call{ value: value }('');
            require(ok2, 'failed to send payout'); // execute the payout
        } else {
            // We avoid keeping all funds in the contract, so we keep sending it to the house address
            (bool ok, ) = payable(houseAddress).call{
                value: (
                    address(this).balance >= liquidityThreshold
                        ? msg.value
                        : fee
                )
            }('');
