...
contract RealEstateMarketplace is ERC721URIStorage, Ownable {
    ...
    event PropertyMinted(uint256 indexed propertyId, string name, uint256 price, address indexed owner);
    event PropertyTransferred(uint256 indexed propertyId, address indexed from, address indexed to);

    constructor() ERC721("Real Estate Marketplace", "REM") {}

    function registerLandlord(string memory _name, string memory _profile,string memory phone) external {
        landlords[msg.sender] = Landlord(_name, _profile,phone);
    }

    function mintProperty(string memory _name, uint256 _price, string memory _tokenURI) external {
        uint256 propertyId = propertyIdCounter++;
        properties.push(Property(propertyId, _name, _price, msg.sender, true));
        _safeMint(msg.sender, propertyId);
        _setTokenURI(propertyId, _tokenURI);
        emit PropertyMinted(propertyId, _name, _price, msg.sender);
    }
    ...
    function sellProperty(uint256 _propertyId, uint256 _price) external {
        Property storage property = properties[_propertyId];
        require(msg.sender == property.owner, "You are not the owner of the property");
