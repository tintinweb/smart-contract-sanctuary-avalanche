...
contract MembershipCardV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    // @dev [1000,3000,9000,35000,136500,408000,1210000, 3653000,11538000]
    uint64[] public upgradeAmounts;
    ...
    function getMaxLevel(address _address) external view returns(uint256){
        return _query(playersSnapshot[_address].snapshot);
    }
    ...
    function remainingMintAmount(address _sender) external view returns(uint256) {
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        ...
        IBet.PlayerRecord memory pr = IBet(betContract).playerRecords(_sender);
        require(_getAvailableAmount(_sender, pr) >= _amount, "condition is not met");
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        IERC20BurnAndMintUpgradeable(daoTokenAddress).safeTransferFrom(owner, address(this), perSpendAmount);
        IERC20BurnAndMintUpgradeable(daoTokenAddress).burn(perSpendAmount);
...
//    function JUST_FOR_TEST(uint256 _tokenId, uint256 _afterLevel, uint256 _rareSerial) external {
//        address owner = msg.sender;
//        Property storage pp = _properties[_tokenId];
//        uint256 _beforeLevel = pp.level;
//        uint256 perSpendAmount = _getLevelUpAmount(_beforeLevel, _afterLevel);
...
//        _properties[_tokenId].level = _afterLevel;
//
//        (uint256 _serial,uint256 _newLevelRareItemAmount) = _updateSerial(_beforeLevel, _afterLevel, _rareSerial, _tokenId);
//        _properties[_tokenId].serial[_afterLevel] = _serial;
//        levelConfig[_afterLevel].rareItemLimitAmount = _newLevelRareItemAmount;
//        emit UpgradeCardEvent(owner, _tokenId, _beforeLevel, _afterLevel);
//    }
    ...
    function updateBetAddress(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateUpgradeAmount(uint256 _ID, uint64 _baseUpgrade) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_baseUpgrade < 0, "require _baseUpgrade");
        upgradeAmounts[_ID-1] = _baseUpgrade;
    }
    ...
    /*
        ...
        result = min(n1, n2);
    */
    function _calTotalMintOf(IBet.PlayerRecord memory _pr) private pure returns(uint256){
    ...
    function _query(uint256 x) private pure returns(uint256) {
        if(x == 0) {
            return 0;
        }
    ...
    function _getAvailableAmount(address _sender,  IBet.PlayerRecord memory _pr) internal view returns(uint256) {
        ...
        uint256 totalAmount = _calTotalMintOf(_pr);
    ...
    function initialize(
        ...
        for(uint256 i = 0; i < _upgradeAmounts.length; i++) {
            uint64 amount = _upgradeAmounts[i];
            require(amount > 0, "Invalid _upgradeAmounts");
            upgradeAmounts.push(amount);
        }
...
interface IBetStruct {
    struct PairsCRound {
        ...
        uint256 rTotalBonus; // the total number of rise direction bonus
        uint256 fTotalBonus; // the total number of fall direction bonus
    }
