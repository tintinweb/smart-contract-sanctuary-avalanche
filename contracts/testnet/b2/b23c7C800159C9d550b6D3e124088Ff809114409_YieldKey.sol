 ...
 */
contract YieldKey is ERC721Enumerable, ERC721Burnable, Claimable, Pausable, Random {
    ...
    uint32 public publicSaleStartTime = 1651410000; // Sunday, 1 May 2022 13:00:00
    ...
    function mint(address _to, uint256 _mintAmount) external payable returns (uint256[] memory) {
        ...
        require(_batchSize > uint256(0), "ERC721 VALIFi: can't create any YieldBox pls Try Later or Wait a new Batches");


        uint256 supply = totalSupply();
        require(!paused(), "VE: Minting paused");
        require(_mintAmount > 0, "VE: Invalid amount");
        require(supply + _mintAmount <= maxSupply, "VE: mint exceeds supply");
        require(cost * _mintAmount <= msg.value, "VE: Insufficient AVAX");
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
			...
			_amountOfKey[i - 1] = amountKeys[supply + i];
	...
	/**
	 * @dev Method for set the Cost of the Yield Key
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721Enumerable) notBlacklisted(from) {
		require(!paused(), "VE: can't create any YieldBox while paused");
		require(!isBlacklisted(to), "VE: can't transfer to blacklisted address");
...
contract Claimable is Blacklistable {
     ...
     * @param _to address of the tokens receiver.
     */
    function _claimErc1155Tokens(address _token, address _to, uint256 _id) private {
