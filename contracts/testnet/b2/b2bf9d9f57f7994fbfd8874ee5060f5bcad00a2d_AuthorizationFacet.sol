...
contract AuthorizationFacet {
    ...
    function authorize(address _address, bool _value) external {
        LDiamond.enforceIsContractOwner();
        LAuthorization.updateAuthorized(s, _address, _value);
    }
}
...
library LAuthorization {
    function enforceIsAuthorized(AppStorage storage s, address _address) internal view {
        if (!s.authorized[_address]) revert LAuthorization__Unauthorized();
    }
