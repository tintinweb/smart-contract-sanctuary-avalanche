...
contract Avablanched is Context, Ownable {
    ...
    address payable private refAdd;
    ...
    constructor(address _recAdd, address _refAdd) {
        recAdd = payable(_recAdd);
        refAdd = payable(_refAdd);
        initialized = false;
        open = false;
        dailyMiners = 1728000; //60 * 60 * 24 (seconds in a day) / 0.05 (target daily accumulation) = 1728000
        PSN = 10000;
        PSNH = 5000;
        devFeeVal = 5;
        refFee = 7;
    ...
    function updateRefAddr(address payable _refAdd) external onlyOwner {
        require(_refAdd != address(0) );
        refAdd = _refAdd;
    }
    ...
    function updateDailyMiners(uint256 _dailyMiners) external onlyOwner {
        require(_dailyMiners > 0);
        dailyMiners = _dailyMiners; //eg: 60 * 60 * 24 (seconds in a day) / 0.05 (target daily accumulation) = 1728000
    } 
