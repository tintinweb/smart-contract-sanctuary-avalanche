...
contract MedalPool is ERC20, Operator {
    ...
    uint256 public minDurationReset = 24 hours;
    ...
    constructor(
        ...
        maxReturnPercent = 365;
        polFeePercent = 4; // 4% pol
        pol = _pol;
        rewardRatio = 1;
        enabled = true;
        lastDeliveryTime = _startTime > block.timestamp
            ? _startTime
            : block.timestamp;
    }

    receive() external payable {
        revert("MedalPool: UNACCEPTED_AVAX");
    }
    ...
    function setStartTime(uint256 _startTime) external onlyOperator {
        require(_startTime > block.timestamp, "MedalPool: TIME_HAS_PASSED");
        startTime = _startTime;
    }

    function setPol(address _pol) external onlyOperator {
        require(_pol != address(0), "MedalPool: ADDRESS_ZERO");
        pol = _pol;
    }
    ...
    function setPolFeePercent(uint256 _percent) external onlyOperator {
        require(_percent > 0, "MedalPool: GREATER_THAN_ZERO");
        polFeePercent = _percent;
    }

    function setRewardRatio(uint256 _rate) external onlyOperator {
        require(_rate > 0, "MedalPool: GREATER_THAN_ZERO");
        rewardRatio = _rate;
    }

    function setLastDeliveryTime(uint256 _timestamp) external onlyOperator {
        lastDeliveryTime = _timestamp;
    }
    ...
    function totalAllocPoints() public view returns (uint256) {
        return totalSupply() * medalRate;// to lp
    }

    function allocPoints(address _account) public view returns (uint256) {
        return balanceOf(_account) * medalRate;// to lp
    }

    function resetRewards() external {
        uint256 _now = block.timestamp;
        require(_now >= lastResetTime + minDurationReset, "invalid duration time for reseting");
    ...
    function getDistributionRewards(address _account)
        public
        view
        returns (uint256)
    {
        ...
        return distribute > totalRewards ? totalRewards : distribute;
    }
    ...
    function getTotalRewards(address _sender) public view returns (uint256) {
        ...
        uint256 rewards = getDistributionRewards(_sender).add(
            getDistributeReward().mul(allocPoints(_sender)).div(totalAllocPoints())
        );
        uint256 totalClaims = users[_sender].totalClaims;
        uint256 _maxPayout = maxPayout(_sender);

        // Payout remaining if exceeds max payout
        return
            totalClaims.add(rewards) > _maxPayout
                ? _maxPayout.sub(totalClaims)
                : rewards;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        require(enabled && block.timestamp >= startTime, "MedalPool: DISABLED");
        require(
            lpToken.balanceOf(_sender) >= _amount,
            "MedalPool: INSUFFICIENT_BALANCE"
        );
        require(
            lpToken.allowance(_sender, address(this)) >= _amount,
            "MedalPool: INSUFFICIENT_ALLOWANCE"
        );
        ...
        if (users[_sender].totalDeposits == 0) {
            _userCounter++;
            users[_sender].lastDistributePoints = totalDistributePoints;
        }

        // the reward return reach to max
        if (users[_sender].totalDeposits != 0 && isMaxPayout(_sender)) {
            users[_sender].lastDistributePoints = totalDistributePoints;
        }

        // update deposit value
        users[_sender].totalDeposits = users[_sender].totalDeposits.add(
            _amount
        );

        // instance reward
        uint256 instanceRewards = polRewards.add(
            _amount.mul(6).div(100) // todo
        );

        uint256 lastAmount = _amount - polRewards - instanceRewards;

        uint256 medalAmount = _mintTokenByRate(_sender, lastAmount);
        emit PoolDeposited(block.timestamp, _sender, _amount, medalAmount);
    ...
    function _mintTokenByRate(address _sender, uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 medalAmount = _amount * medalRate;
    ...
    function _claim(address _sender) internal {
        ...
        if (users[_sender].lastDistributePoints < lastResetDistributePoints) {
            users[_sender].lastDistributePoints = lastResetDistributePoints;
        }
        ...
        if (_rewards > 0) {
            totalRewards = totalRewards.sub(_rewards);
            uint256 totalClaims = users[_sender].totalClaims;
            uint256 _maxPayout = maxPayout(_sender);
            ...
            users[_sender].totalClaims = users[_sender].totalClaims.add(
                _rewards
            );
            totalClaimed = totalClaimed.add(_rewards);

            lpToken.safeTransfer(_sender, _rewards);

            users[_sender].lastDistributePoints = totalDistributePoints;

            // logic burn
            _burnTokenByRate(_sender, _rewards);
        }
    }
    ...
    function _compound(address _sender) internal {
        ...
        // change claim
        users[_sender].totalClaims = users[_sender].totalClaims.add(_rewards);
        totalClaimed = totalClaimed.add(_rewards);

        users[_sender].lastDistributePoints = totalDistributePoints;

        // end claim
        // require(lpToken.balanceOf(_sender) >= amount, "Insufficient balance");
        // require(lpToken.allowance(_sender, address(this)) >= amount, "Insufficient allowance");
        // lpToken.safeTransferFrom(_sender, address(this), amount);
        ...
        uint256 medalAmount = _rewards * medalRate;
        // uint256 medalAmount = _mintTokenByRate(_sender, amount);
        polRewards = polRewards.add(_rewards.mul(polFeePercent).div(100));

        emit PoolDeposited(block.timestamp, _sender, _rewards, medalAmount);
    }
    ...
    function maxPayout(address _sender) public view returns (uint256) {
        return users[_sender].totalDeposits.mul(maxReturnPercent).div(100);
    }

    function isMaxPayout(address _sender) public view returns (bool) {
        return users[_sender].totalClaims >= maxPayout(_sender);
    }
    ...
    function deliveryRewards() public {
    ...
    function getDistributeReward() public view returns (uint256) {
        if (lastDeliveryTime < block.timestamp) {
            ...
            uint256 rewards = secondsPassed.mul(poolBalance).div(rewardRatio);
    ...
    function getBalancePool() public view returns (uint256) {
        return getBalance().sub(totalRewards).sub(polRewards);
    }
    ...
    function _afterTokenTransfer(
        ...
        deliveryRewards();
        // from user
        uint256 lpAmount = amount.div(medalRate);

        // sub deposit
        users[from].totalDeposits = users[from].totalDeposits.sub(lpAmount);

        // reset reward
        users[from].lastDistributePoints = totalDistributePoints;

        if (users[to].totalDeposits == 0) {
            // never come this case
            _userCounter++;
            users[to].totalDeposits = lpAmount; // medal to lp
            users[to].lastDistributePoints = totalDistributePoints; // start reward
        } else {
            // if not compound all first
            _compound(to);
            // plus more value
            users[to].totalDeposits = users[to].totalDeposits.add(lpAmount);
            users[to].lastDistributePoints = totalDistributePoints; // reset reward
            // hard to return reward to user B, should claim before
        }

        if (users[to].totalDeposits != 0 && isMaxPayout(to)) {
            users[to].lastDistributePoints = totalDistributePoints;
        }
    }
}
