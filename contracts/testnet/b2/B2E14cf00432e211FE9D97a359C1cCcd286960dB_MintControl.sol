...
contract MintControl is Ownable, Pausable {
	 ...
	 */
	function callForMint(string memory label, uint16 nftToMint) public whenNotPaused {
		...
		if (nftToMint > 1){
			for (uint16 i = nftToMint; i > 0; i--) {
				ICFContainer(info.location).safeMint(msg.sender, uint16(info.idCounter.current()));
	...
	function callForMint2(string memory label, uint16 nftToMint) public whenNotPaused {
		...
		if (nftToMint > 1){
			for (uint16 i = nftToMint; i > 0; i--) {
				info.location.safeMint(msg.sender, info.counter);
				// ICFContainer(info.location).safeMint(msg.sender, uint16(info.idCounter.current()));
				// containers[label].idCounter.increment();
				info.counter++;
			}
		...
		containers[label].counter = info.counter;
	}

	function callForMint3(string memory label, uint16 nftToMint) public whenNotPaused {
		Container memory info = containers[label];
		require(info.price > 0, string(abi.encodePacked('Container with label ', label, ' does not exist!')));
		require(_checkSupply(label) >= nftToMint, "Not enough supply remaining.");

		uint16 startId = info.location.totalSupply();
		if(startId > 0){startId--;}

		info.location.safeMint2(msg.sender, nftToMint, startId);

		containers[label].counter += nftToMint;
	}
	// ================================ VIEW FUNCTIONS ================================ //
	...
	function _createEntry(
		...
		containers[_containerLabel] = Container({ label: _containerLabel, location: ICFContainer(_location), price: _price, maxNfts: _maxNfts, idCounter: newCounter, counter: 0 });
	}
