...
contract RedestaMain {
    ...
    function donateProject(uint _projectID, uint amount) public {
        require(ourToken.balanceOf(msg.sender) >= amount );
        ourToken.approve(msg.sender, amount );
        ourToken.transferFrom(msg.sender, address(this), amount );

        allProjects[_projectID].status.current += amount;

        profiles[msg.sender].donateAmount += amount ;
        profiles[msg.sender].supporting.push(_projectID);
        rewardPool += amount ; // reward pool--------------------------
    ...
    function withdrawRequest(uint _id) public returns(uint) {
        ...
        if(block.timestamp < allProjects[_id].requestInfo.endtimestamp){
            ...
            ourToken.transfer(allProjects[_id].projectOwner, allProjects[_id].requestInfo.amount);
