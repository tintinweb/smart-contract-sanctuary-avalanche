    ...
    HasRouter
{
        ...
        notInBlackList(msg.sender)
    {
        ...
        uint256 sideTokenAmount;
        sideTokenAmount = (token == address(SideToken))
            ? amountForLP
        ...
        uint256 mpBefore = IERC20(medalToken).balanceOf(address(this));
        ILPNode(medalToken).deposit(lpTokenAmount);
        uint256 mpAfter = IERC20(medalToken).balanceOf(address(this));

        uint256 mpAmount = mpAfter - mpBefore;
        IERC20(medalToken).transfer(_sender, mpAmount);
     ...
     * --------------------
     */
    function setLPNode(address _medalToken) external onlyOperator {
     ...
     * ------------------
     */
    function _verifyEnoughMainToken(address token, uint256 amount)
        internal
        view
    {
        uint256 discountAmount = (amount * (100 + discountPercent)) / 100;
    ...
    function _getLpAmount(
        ...
        uint256 liquidityForLpA = (_amountADesired * totalSupply) / reserve0;
        uint256 liquidityForLpB = (_amountBDesired * totalSupply) / reserve1;
    ...
    function _buySideToken(address token, uint256 amountIn)
        internal
        returns (uint256)
    {
    ...
    function _addLPFromSideToken(uint256 sideTokenAmount)
        internal
        returns (uint256)
    {
        uint256 mainBalance = MainToken.balanceOf(address(this));
     ...
     * --------------
     */
    function getEstimateMedalToken(address token, uint256 amount)
        external
        view
        returns (uint256)
    {
        ...
        uint256 lpAmount = _getLpAmount(_tokenA, _tokenB, _amountBDesired);

        return ILPNode(medalToken).getEstimateMedalToken(lpAmount);
    }
