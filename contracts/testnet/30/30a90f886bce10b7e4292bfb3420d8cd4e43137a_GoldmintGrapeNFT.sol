...
contract GoldmintGrapeNFT is Ownable, RoyaltiesAddon, ERC2981 {
    ...
    uint256 public STARTTIME = 1658076653;
    uint256 public WHITELISTPRICE = 1.2 ether;
    ...
    constructor(string memory baseURI) ERC721("Goldmint x Grape VIP Collection", "Goldmint x Grape"){
        setBaseURI(baseURI);
    }
    ...
    function mint(uint256 _tokenAmount) public payable saleIsOpen {
        ...
        require(users[wallet].total_minted < 3, "Max User Limit");

        if(WHITELIST){
            require(verifyUser(wallet), "Not Whitelisted");
            require(msg.value >= whitelistPrice(_tokenAmount), "Value below price");
            for(uint8 i = 1; i <= _tokenAmount; i++){
                _mintAnElement(wallet, total + i);
                users[wallet].total_minted = users[wallet].total_minted.add(1);
            }
        }else{
    ...
    function whitelistPrice(uint256 _count) public view returns (uint256) {
    ...
    function setPause(bool _pause) public onlyAuth{
        PAUSE = _pause;
        emit PauseEvent(PAUSE);
    }

    function setWhitelist(bool _whitelist) public onlyAuth{
