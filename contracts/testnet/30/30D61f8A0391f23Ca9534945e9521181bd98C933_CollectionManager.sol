...
contract CollectionManager is CollectionEnumerable, ICollectionManager {
    ...
    modifier onlyOwnerOf(uint256 collectionId) {
        require(msg.sender == collectionToOwner[collectionId], "no creator");
        _;
    }
    ...
    function createNFT(
        ...
        if (nftCollection.nftType == uint8(NftType.ERC721)) {
            ...
            uint256 newTokenId = mockERC721.mint(msg.sender, nftURI);
            ...
            uint256 newTokenId = mockERC1155.mint(msg.sender, nftURI, amountForErc1155);
            emit TokenMinted(newTokenId, nftURI);
        }
    }
    ...
    function fetchCollectionsByOwner() public view override returns (NftCollection[] memory) {
        uint256 totalCount = collectionIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalCount; i++) {
            if (idToNftCollection[i + 1].owner == msg.sender) {
                itemCount += 1;
            }
        }
    ...
    function transferOwner(uint256 collectionId, address newOwner) external override onlyOwnerOf(collectionId) {
        ...
        collectionToOwner[collectionId] = newOwner;
        _emitCollectionUpdated(collectionId);
    }
}
...
interface ICollectionManager {
    ...
    function fetchCollectionsByOwner() external returns (NftCollection[] memory);
