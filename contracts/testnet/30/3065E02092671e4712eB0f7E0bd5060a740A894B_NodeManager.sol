...
abstract contract Pausable is Context {
     ...
     *
     * Requirements:internal view virtual
     *
     * - The contract must not be paused.
     */
...
abstract contract nft_stake is Context {
    ...
    function calc_now(address _accounts) public view returns (uint[3] memory) {}
...
contract NodeManager is Ownable, Pausable {
    ...
    address stake = 0xa4B79d03f1BBA5123d1636F659bF3268371ee4Cc;
    nft_stake _stake =  nft_stake(stake);
    ...
    function _calculateBoost(address _account,uint elapsedTime_,uint256 amount_) internal view returns (uint256) {
        uint256[3] memory Tier = _stake.calc_now(_account);
    ...
    function updateNftRewardsBoostPercs(uint8[] calldata newVal) external onlyOwner {
     ...
     function updateNFTPool(address newVal) external onlyOwner {
        stake = newVal;
        nft_stake _stake =  nft_stake(stake);
    }
