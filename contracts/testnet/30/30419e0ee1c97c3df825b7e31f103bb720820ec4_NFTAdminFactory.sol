...
// File @confluxfans/contracts/utils/[email protected]
...
// File @confluxfans/contracts/InternalContracts/[email protected]
...
// File @confluxfans/contracts/token/CRC721/extensions/[email protected]
...
abstract contract NFTRole is ERC721, AccessControlEnumerable {
    ...
    function grantRoleBatch(bytes32[] memory roles, address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
...
// File contracts/NFTMintable.sol
...
abstract contract NFTMetadata is NFTRole {
    ...
    function setTokenURI(uint256 tokenId, string memory uri) public onlyMetadataRole {
...
// File contracts/NFTPausable.sol
    ...
    InternalContractsHandler
{
    constructor(
        ...
        string memory tokenPlaceholderURI
    ) ERC721(name, symbol) NFTMetadata(tokenPlaceholderURI) {
    }
    ...
    function _burn(uint256 tokenId) internal virtual override(ERC721, NFTMetadata) {
...
abstract contract NFTFactoryBase is AccessControlEnumerable, InternalContractsHandler {

    bytes32 public constant DEPLOYER_ROLE = keccak256("DEPLOYER_ROLE");

    mapping (address => bool) _deployed;
    ...
    modifier onlyDeployerRole() {
        require(hasRole(DEPLOYER_ROLE, _msgSender()), "NFTFactory: DEPLOYER_ROLE required");
        _;
    }
    ...
    function _nftDeployed(NFT nft) internal returns (address) {
        nft.grantRole(DEFAULT_ADMIN_ROLE, _msgSender());

        address addr = address(nft);
        _deployed[addr] = true;
        emit NFTDeployed(addr);
...
contract NFTFactory is NFTFactoryBase {

    function deployNFT(
        string memory name,
        string memory symbol,
        string memory tokenPlaceholderURI
    ) public onlyDeployerRole returns (address) {
        NFT nft = new NFT(name, symbol, tokenPlaceholderURI);
        return _nftDeployed(nft);
    }

}


// File contracts/admin/NFTAdmin.sol

// SLI: MIT
pragma solidity ^0.8.0;
contract NFTAdmin is NFT {
    ...
    constructor(
        ...
        string memory symbol,
        string memory tokenPlaceholderURI
    ) NFT(name, symbol, tokenPlaceholderURI) {
    ...
    modifier onlyBurnerRole() {
        require(hasRole(BURNER_ROLE, _msgSender()), "NFTAdmin: BURNER_ROLE required");
        _;
    }

    function burnByAdmin(uint256 tokenId) public onlyBurnerRole {
        _burn(tokenId);
    }

    function burnBatchByAdmin(uint256[] memory tokenIds) public onlyBurnerRole {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _burn(tokenIds[i]);
        }
    }
...
// File contracts/admin/NFTAdminFactory.sol

// SLI: MIT
pragma solidity ^0.8.0;
contract NFTAdminFactory is NFTFactoryBase {
    ...
    function deployNFT(
        ...
        NFTAdmin nft = new NFTAdmin(name, symbol, tokenPlaceholderURI);
        return _nftDeployed(nft);
    }
