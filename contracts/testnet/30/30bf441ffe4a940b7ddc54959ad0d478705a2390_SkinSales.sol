...
import { DefimonsSkins } from "./DefimonsSkins.sol";

contract SkinSales is AccessControl {
    ...
    event SkinBought(uint256 indexed saleId, address to);
    ...
    modifier onlyActive(uint256 saleId_) {
        require(saleId_ < _sales.length, "SkinSales: Sale not found");
        require(_sales[saleId_].active, "SkinSales: This sale is not active yet");
    ...
    modifier onlyInactive(uint256 saleId_) {
        require(saleId_ < _sales.length, "SkinSales: Sale not found");
        require(!_sales[saleId_].active, "SkinSales: This sale is currently active");
    ...
    /**
     * @dev Struct that defines a Skin Sale.
     * @param tokenId       The tokenId of the Skin sold in this Sale.
     ...
     * @param skinAmount    The total Skin amount to be sold.
     * @param skinPrice     The price at which each Skin will be sold.
    ...
    /**
     * @param skinsAddress_ The address of DefimonsSkins contract.
     * @param adminAddress_ The address to be granted the DEFAULT_ADMIN_ROLE to.
     */
    constructor(address skinsAddress_, address adminAddress_) {
        skins = DefimonsSkins(skinsAddress_);
    ...
    /**
     * @dev Public buy function.
     * To be called by end users.
     * Mints 1 Skin from the Sale with saleId_ to to_.
     * Sale must be activated beforehand.
     * @param saleId_   The index of the _sales array indicating which sale to buy from.
     * @param to_       The address to mint the Skin to.
     */
    function buy(uint256 saleId_, address to_) public payable {
        require(saleId_ < _sales.length, "SkinSales: Sale not found");
        require(to_ != address(0), "SkinSales: Buyer cannot be address 0");
        Sale memory sale_ = _sales[saleId_];

        require(sale_.active, "SkinSales: This sale is not active yet");

        if (sale_.limitedTime) {
            require(block.timestamp > sale_.startTime, "SkinSales, This sale hasn't started yet");
            require(block.timestamp < sale_.endTime, "SkinSales: This sale is already over");
        }

        require(msg.value == sale_.skinPrice, "SkinSales: Incorrect value sent for this sale");
        require(sale_.bought < sale_.skinAmount, "SkinSales: No more skins available for this sale");

        _sales[saleId_].bought += 1;

        emit SkinBought(saleId_, to_);

        skins.mint(to_, sale_.tokenId, 1, "");
    }
    ...
    /**
     * @dev Adds a Sale to the _sales array.
     * A newly added Sale is always deactivated.
     * For parameter documentation see {SkinSales.Sale}.
     */
    function addSale(
        uint192 tokenId_,
        bool limitedTime_,
        ...
        _validateSaleParams(tokenId_, limitedTime_, startTime_, endTime_, skinAmount_, skinPrice_);
        ...
        Sale memory sale_ = Sale({
            ...
            endTime: endTime_,
            skinAmount: skinAmount_,
            skinPrice: skinPrice_
        });

        _sales.push(sale_);

        emit SaleCreated(_sales.length - 1, tokenId_, limitedTime_, startTime_, endTime_, skinAmount_, skinPrice_);
    }

    /**
     * @dev Edits a Sale.
     * @param saleId_   The index of the _sales array where the Sale to edit is stored.
     * For parameter documentation see {SkinSale.Sale}
     */
    function editSale(
        uint256 saleId_,
        uint192 tokenId_,
        ...
        sale.skinAmount = skinAmount_;
        sale.skinPrice = skinPrice_;

        emit SaleEdited(saleId_, tokenId_, limitedTime_, startTime_, endTime_, skinAmount_, skinPrice_);
    }

    /**
     * @dev Activates a Sale by setting active to true.
     * @param saleId_   The index of the _sales array where the Sale to activate is stored.
     */
    function activateSale(uint256 saleId_) public onlyRole(DEFAULT_ADMIN_ROLE) onlyInactive(saleId_) {
        require(saleId_ < _sales.length, "SkinSales: Sale not found");
    ...
    /**
     * @dev Deactivates a Sale by setting active to false.
     * @param saleId_   The index of the _sales array where the Sale to deactivate is stored.
     */
    function deactivateSale(uint256 saleId_) public onlyRole(DEFAULT_ADMIN_ROLE) onlyActive(saleId_) {
        require(saleId_ < _sales.length, "SkinSales: Sale not found");
    ...
    /**
     * @dev Function to withdraw the payment for the sold skins.
     */
    function withdrawPayment(address to_, uint256 amount_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(to_ != address(0), "SkinSales: Cannot send funds to address 0");
        require(amount_ <= address(this).balance, "SkinSales: Not enough funds in Sales contract");

        payable(to_).transfer(amount_);
    }

    /**
     * @dev Function to withdraw any ERC20s wrongly sent to contract.
     */
    ...
    /**
     * @dev Getter for a specific Sale's parameters.
     * @param saleId_ The index of the _sales array where the Sale to get is stored.
     */
    ...
    /**
     * @dev Getter for the number of added Sales.
     */
    ...
    /**
     * @dev Checks if the Sale with given saleId_ is active.
     */
    ...
    /**
     * @dev Getter for the amount of Skins that have yet to be sold in a given Sale.
     */
    function getAvailableAmount(uint256 saleId_) public view returns (uint64) {
        Sale memory sale_ = _sales[saleId_];
        return uint64(sale_.skinAmount - sale_.bought);
    }
    ...
    /**
     * @dev Internal function used to validate Sale parameters sent as arguments.
     * Used in addSale() and editSale().
     */
    function _validateSaleParams(
        ...
        require(skins.exists(tokenId_), "SkinSales: Invalid token ID");

        if (limitedTime_) {
            require(startTime_ > 0, "SkinSales: Limited time sale cannot have zero start time");
            require(endTime_ > startTime_, "SkinSales: End time must be greater than start time");
        }

        require(skinAmount_ > 0, "SkinSales: Skin amount must be greater than zero");
        require(skinPrice_ > 0, "SkinSales: Skin price must be greater than zero");
    }
}
...
contract DefimonsSkins is AccessControl, ONFT1155 {
    ...
    event SkinCreated(uint256 tokenId, uint256 initialSupply);
    ...
    // Supply cap of the token with given ID
    ...
    /**
     * @dev Creates a new skin with Max Supply = initialSupply_
     * @param tokenId_          Token ID to use for this skin
     * @param initialSupply_    Total supply for this skin
     */
    function createSkin(uint256 tokenId_, uint256 initialSupply_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(maxSupply[tokenId_] == 0, "DefimonsSkins: Skin already exists");
        require(initialSupply_ > 0, "DefimonsSkins: Invalid initial supply for token creation");

        maxSupply[tokenId_] = initialSupply_;

        emit SkinCreated(tokenId_, initialSupply_);
    }

    /**
     * @dev Updates the Max Supply of the token with the given ID.
     * newMaxSupply_ cannot be greater than the Total Supply of a token
     */
    function updateMaxSupply(uint256 tokenId_, uint256 newMaxSupply_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(exists(tokenId_), "DefimonsSkins: Invalid token ID");
        require(newMaxSupply_ > 0, "DefimonsSkins: Max supply cannot be zero");
        require(
            newMaxSupply_ >= totalSupply[tokenId_],
            "DefimonsSkins: Max supply must not be lesser than total supply"
        );
     ...
     * Required to call mint() function.
     */
    ...
    function mint(
        ...
        require(
            ...
            "DefimonsSkins: Mint surpasses the max supply of this skin"
        );
    ...
    /**
     * @dev Indicates whether a skin exist with a given id or not.
     */
    ...
    /**
     * @dev Getter for the available supply of token with given tokenId_.
     * @return Amount of tokens that have been created but have not yet been minted.
     */
