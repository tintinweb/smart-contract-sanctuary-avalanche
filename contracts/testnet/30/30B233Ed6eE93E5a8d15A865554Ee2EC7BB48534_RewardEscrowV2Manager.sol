...
contract RewardEscrowV2Manager is Owned, MixinSystemSettings {
    ...
    mapping(bytes32 => IRewardEscrowV2Aggregator) public aggregators;
    ...
    function totalBalanceOf(address account) external view returns (uint256 _totalBalanceOf) {
        for (uint256 ii = 0; ii < availableAggregators.length; ii++) {
            IRewardEscrowV2Aggregator aggregator = aggregators[availableAggregators[ii]];
            uint256 balance = aggregator.balanceOf(account);
    ...
    function totalBalanceOfByKey(bytes32 currencyKey, address account) external view returns (uint256 _totalBalanceOf) {
        IRewardEscrowV2Aggregator aggregator = aggregators[currencyKey];
        uint256 balance = aggregator.balanceOf(account);
        (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
    ...
    function totalEscrowedBalance() external view returns (uint256 _totalEscrowedBalance) {
        for (uint256 ii = 0; ii < availableAggregators.length; ii++) {
            IRewardEscrowV2Aggregator aggregator = aggregators[availableAggregators[ii]];
            uint256 balance = aggregator.totalEscrowedBalance();
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableAggregators[ii]);
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    function addAggregator(bytes32 currencyKey, IRewardEscrowV2Aggregator aggregator) external onlyOwner returns (bool) {
        if (aggregators[currencyKey] != IRewardEscrowV2Aggregator(0)) {
    ...
    function removeAggregator(bytes32 currencyKey) public onlyOwner returns (bool) {
        IRewardEscrowV2Aggregator oldAggregator = aggregators[currencyKey];
        for (uint256 ii = 0; ii < availableAggregators.length; ii++) {
        ...
        aggregators[currencyKey] = IRewardEscrowV2Aggregator(0);
    ...
    event AddAggregator(bytes32 collateralKey, IRewardEscrowV2Aggregator aggregator);
    event RemoveAggregator(bytes32 collateralKey, IRewardEscrowV2Aggregator oldAggregator);
