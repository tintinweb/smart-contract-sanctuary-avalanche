...
contract MXV4 is OFTV2, ReentrancyGuard {
    ...
    constructor(
            ...
            // uint8 _totalFees,
            // address _feesWalletAddress,
            // address _routerAddress
        ) payable OFTV2("MXV4", "MXV4", _sharedDecimals, _layerZeroEndpoint) {
    ...
    function _transfer(
        ...
        require(from != address(0), "M8Token: transfer from the zero address");
        require(to != address(0), "M8Token: transfer to the zero address");
        require(controller != address(0), "M8Token: missing controller set");
        ...
        if (
            ...
            (uint256 estimatedAmount, ) = IM8Controller(controller).estimateTransferFees(from, to, amount);

            if (estimatedAmount < amount) {
                super._transfer(from, controller, amount);
                uint256 updatedAmount = IM8Controller(controller).transferFees(from, to, amount);
    ...
    function _transfer(
        ...
        require(
            ...
            "MXV4: amount larger than allowed"
        );
        ...
        // uint256 contractTokenBalance = balanceOf(address(this));s
    ...
    */
    ...
    function updateController(address _newController)
        ...
        nonReentrant
    {
        require(_newController != address(0), 'M8Token: no zero address for controller');
