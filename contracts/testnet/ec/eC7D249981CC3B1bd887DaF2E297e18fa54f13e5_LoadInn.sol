...
contract LoadInn is ReentrancyGuard, Ownable {
    ...
    function bardSong(
        ...
        require(
            matchAddresSigner(
                hashTransactionSong(msg.sender, tokenId, songId, nonce),
                signature
            ),
            "DIRECT_SONG_DISALLOWED"
        );
    ...
    function rest(
        uint256 level,
        uint256 tokenId,
        string memory restId,
        bytes memory signature,
        string memory nonce
    ) external {
        require(
            matchAddresSigner(
                hashTransactionRest(msg.sender, level, tokenId, restId, nonce),
                signature
            ),
    ...
    function hashTransactionRest(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(
                    abi.encodePacked(sender, level, tokenId, actionId, nonce)
                )
            )
        );
