...
contract Flesh is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        if (!hasRole(FREEZER_ROLE, account)) {
            if (balanceOf(account) == 0 || _timers[account] == 0) {
    ...
    function withdrawRFLSH() external whenNotPaused {
        ...
        _burn(msg.sender, _rottenFleshToWithdraw);

        _rottenFlesh.mint(msg.sender, _rottenFleshToWithdraw);
    ...
    function _burn(address account, uint256 amount) internal virtual override {
        require(
            super.balanceOf(account) >= amount,
            "FLSH: burn amount exceeds balance"
        );

        // Reset rotting timer if balance comes to 0
        if (super.balanceOf(account) == amount) {
            _timers[account] = 0;
        }
