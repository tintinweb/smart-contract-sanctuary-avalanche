...
contract HintHelpers is LiquityBase, Ownable, CheckContract {
        ...
        )
    {
        ...
        while (currentTroveuser != address(0) && sortedTrovesCached.getOldBoostedRICR(currentTroveuser) < MCR) {
    ...
    */
        ...
        returns (address hintAddress, uint diff, uint latestRandomSeed)
    {
        ...
        diff = YetiMath._getAbsoluteDifference(_CR, sortedTroves.getOldBoostedRICR(hintAddress));
        ...
        while (i < _numTrials) {
            ...
            uint currentRICR = sortedTroves.getOldBoostedRICR(currentAddress);

            // check if abs(current - CR) > abs(closest - CR), and update closest if current is closer
            uint currentDiff = YetiMath._getAbsoluteDifference(currentRICR, _CR);
