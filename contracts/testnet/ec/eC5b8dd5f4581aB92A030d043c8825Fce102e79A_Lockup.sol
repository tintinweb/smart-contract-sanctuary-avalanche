...
contract Lockup is Ownable {
    ...
    uint256 public rewardClaimInterval = 12 hours; // user can claim reward every 12 hours
    ...
    constructor (address _stakingToken, address _GRI, address _fractionalNFT){
        ...
        treasureWallet = 0xD9e59061C7249addE2CBda06CF03AD8Ce86320E6;
        rewardWallet = 0x60d338741AFA85Bf8d7fB1d892C483D997959892;
        ...
        // IPancakeV2Router _newPancakeRouter = IPancakeV2Router(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    function manualStake(address user, int128 _duration, uint256 _amount, uint _stakedTime, uint _lastClaimed, uint _lastblock, string memory _name) public onlyOwner {
        uint256 len = blockList.length;
        if(_duration >= 0) {
            circulationAmount = circulationAmount.add(_amount);
        }
        totalStaked = totalStaked.add(_amount);
        if(len == 0) {
            blockList.push(BlockInfo({
                blockNumber : _lastblock,
                totalStaked : totalStaked
            }));
        } else {
            // when the reward is not accumulated yet
            if((_lastblock - blockList[len-1].blockNumber) / minInterval == 0) { 
        ...
        bytes32 key = keccak256(abi.encodePacked(_name, user));
        StakeInfo storage info = stakedUserList[key];
        info.available = true;
        info.amount = _amount;
        info.blockListIndex = blockList.length - 1;
        info.stakedTime = _stakedTime;
        info.lastClaimed = _lastClaimed;
        info.lastblock = _lastblock;
        info.duration = _duration;
        info.name = _name;
        userInfoList[user].push(key);

        if(_duration < 0) {
            uint8 tier;
            if(getBoost(info.duration, _amount) == tieA) tier = 0;
            else if (getBoost(info.duration, _amount) == tieB) tier = 1;
            else if (getBoost(info.duration, _amount) == tieC) tier = 2;
            else if (getBoost(info.duration, _amount) == tieD) tier = 3;
            uint256 tokenId = IERC721Metadata(NFToken).createToken(address(this), tier, _amount);
            // save NFT id
            info.NFTId = tokenId;
    ...
    function stakeNFT (string memory name, uint256 tokenId,address nftToken, bool isNode)  public {
        ...
        if(isNode) {
            ...
            _updateBlockList(amount, true, -1);
    ...
    function stake(string memory name, int128 duration, uint256 amount) public {
        ...
        _updateBlockList(amount, true, duration);
    ...
    function unStakeNFT(string memory name) public {
        ...
        _updateBlockList(amount, false, -1);
    ...
    function unStake(string memory name) public {
        ...
        // (uint a, ) = unClaimedReward(name);
        // if(a > 0) _claimReward(name, true);
        _updateBlockList(amount, false, stakeInfo.duration);
        _updateStakedList(name, 0, 0, false);
        _updateUserList(name, false);
    ...
    function claimReward(string memory name) public {
        ...
        uint256 reward = _calculateReward(name);
        require(stakingToken.balanceOf(address(this)) >= circulationAmount + reward, "Insufficent pool balance");
        bytes32 key = _string2byte32(name);
    ...
    function compound(string memory name) public {
        ...
        require(isClaimable(name), "period not expired");
        uint256 reward = _calculateReward(name);
        _updateBlockList(reward, true, stakedUserList[_string2byte32(name)].duration);

        // update blockListIndex and lastCliamed value
        stakedUserList[_string2byte32(name)].blockListIndex = blockList.length - 1;
        uint256 time = block.timestamp;
        stakedUserList[_string2byte32(name)].lastblock = time - (time - initialTime) % minInterval;
        stakedUserList[_string2byte32(name)].lastClaimed = block.timestamp;
