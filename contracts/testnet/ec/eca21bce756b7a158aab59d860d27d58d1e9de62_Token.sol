...
contract Token is ERC20, Ownable {
    ...
    uint256 public sellTax = 44; // 44%
    uint256 public buyTax = 0; // 0%
    uint256 public maxPerWallet = 10_000_000 ether; // 10,000,000 tokens
    uint256 public maxPerTransaction = 5_000_000 ether; // 5,000,000 tokens
    bool public isTradingOpen = false; // Trading status

    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isBlacklisted;

    constructor (address _uniswapV2Pair) ERC20("Test", "TESTT") {
    ...
    function setMaxPerTransaction(uint256 newMaxPerTransaction) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        require(isTradingOpen || sender == owner(), "Trading is not open yet");
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "Blacklisted address");

        if (sender != owner() && recipient != owner()) { // Exclude owner from max per wallet
            uint256 recipientBalance = balanceOf(recipient);
            require(recipientBalance + amount <= maxPerWallet, "Transfer amount exceeds max per wallet");
            require(amount <= maxPerTransaction, "Transfer amount exceeds max per transaction");
        }
        ...
        if (_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]) {
            ...
            if (sender == uniswapV2Pair) { // if this is a buy
                ...
                super._transfer(sender, address(0x272A88dF9249223c2418E102521daF660C19e70B), taxAmount); // burn the tax amount
