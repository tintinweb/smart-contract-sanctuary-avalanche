...
contract DrawerV2 is Ownable {
  ...
  function getTeamName(string memory name) internal pure returns (string memory teamName) {
    teamName = string(
      abi.encodePacked(
        '<foreignObject width="320" height="320"><div xmlns="http://www.w3.org/1999/xhtml" style="text-align:center;color:red;font-size:500%;position:relative;left:50%;top:50%;transform:translate(-50%,-50%);">',
        name,
        "</div></foreignObject>"
      )
    );
