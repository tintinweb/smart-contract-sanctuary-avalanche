...
contract Faucet {
    mapping(address => uint256) public _lastRequest;
    ...
    function checkRequest(address user) public view returns (bool valid) {
        valid = _lastRequest[user] + _requestInterval <= block.timestamp;
        return valid;
    }
}