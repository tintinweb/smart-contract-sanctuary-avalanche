...
import { IEatereum } from "./interfaces/IEatereum.sol";
import { Lockable } from "./internal/Lockable.sol";
import { MinterRole } from "./internal/MinterRole.sol";
import { BusinessAddresses } from "./internal/BusinessAddresses.sol";
...
contract Eatereum is IEatereum, ERC20, Pausable, Ownable, ERC20Permit, Lockable, MinterRole, BusinessAddresses {
    using ECDSA for bytes32;
    using Helper for bytes32;
    using BitMaps for BitMaps.BitMap;

    constructor() ERC20("Eatereum", "EAT") ERC20Permit("Eatereum") {
        _setMinterStatus(0x64470E5F5DD38e497194BbcAF8Daa7CA578926F6, true);
    }
    ...
    function acceptBusinessAddresses(address[] calldata businessAddresses_) external onlyOwner {
        _acceptBusinessAddresses(businessAddresses_);
    }

    function cancelBusinessAddresses(address[] calldata businessAddresses_) external onlyOwner {
    ...
    function burn(
        ...
        uint256 burnId = burnId_.toUint256();
        bytes32 messageHash = keccak256(abi.encodePacked(burnId_, amount_, deadline_));
        bytes32 digest = messageHash.toEthSignedMessageHash();
        address tokenOwner = digest.recover(v_, r_, s_);

        if (!_inBusinessList(_msgSender())) revert BusinessAddresses__NotAuthorized();
        if (_isUsed.get(burnId)) revert IEatereum__AlreadyBurnt();
        _isUsed.setTo(burnId, true);
        _burn(tokenOwner, amount_);
        emit Burn(burnId_, tokenOwner, amount_);
    }
    ...
    function __notExpired(uint256 deadline_) private view {
        if (block.timestamp > deadline_) revert IEatereum__Expired();
    }
}
...
interface IEatereum {
    error IEatereum__AlreadyBurnt();
    error IEatereum__Expired();

    event Burn(bytes32 burnId, address owner, uint256 amount);
...
error MinterRole__NotAuthorized();

abstract contract MinterRole is Context {
    using Helper for address;
    using BitMaps for BitMaps.BitMap;

    BitMaps.BitMap private _isMinter;

    modifier onlyMinter() {
        address sender = _msgSender();
        if (!_isMinter.get(sender.toUint256())) revert MinterRole__NotAuthorized();
        _;
    }

    function isMinter(address account) external view returns (bool) {
        return _isMinter.get(account.toUint256());
    }

    function addMinter(address account) external onlyMinter {
        _setMinterStatus(account, true);
    }

    function renounceMinter() external {
        _setMinterStatus(_msgSender(), false);
    }

    function _setMinterStatus(address account_, bool status_) internal {
        _isMinter.setTo(account_.toUint256(), status_);
    }
}
