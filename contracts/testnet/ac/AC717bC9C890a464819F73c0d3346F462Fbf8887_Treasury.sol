...
contract Treasury is MetaVaultAC {
    ...
    function queue(MANAGING _managing, address _address) external onlyGovernor returns (bool) {
        ...
        if (_managing == MANAGING.RESERVEDEPOSITOR) {
            // 0
            reserveDepositorQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.RESERVESPENDER) {
            // 1
            reserveSpenderQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.RESERVETOKEN) {
            // 2
            reserveTokenQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.RESERVEMANAGER) {
            // 3
            ReserveManagerQueue[_address] = block.timestamp.add(blocksNeededForQueue.mul(2));
        } else if (_managing == MANAGING.LIQUIDITYDEPOSITOR) {
            // 4
            LiquidityDepositorQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.LIQUIDITYTOKEN) {
            // 5
            LiquidityTokenQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.LIQUIDITYMANAGER) {
            // 6
            LiquidityManagerQueue[_address] = block.timestamp.add(blocksNeededForQueue.mul(2));
        } else if (_managing == MANAGING.DEBTOR) {
            // 7
            debtorQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.REWARDMANAGER) {
            // 8
            rewardManagerQueue[_address] = block.timestamp.add(blocksNeededForQueue);
        } else if (_managing == MANAGING.SMVD) {
            // 9
            sMVDQueue = block.timestamp.add(blocksNeededForQueue);
        } else return false;
