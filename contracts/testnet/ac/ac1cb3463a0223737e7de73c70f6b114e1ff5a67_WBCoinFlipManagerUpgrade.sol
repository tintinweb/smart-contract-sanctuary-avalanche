    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct AddressTaxReceiver {
        address receiver;
        uint256 portion;
    }

    AddressTaxReceiver[] public addressTaxReceiverArray;
    uint256 private portions;
    mapping(address => bool) private addressTaxReceiverMapping;
    ...
    modifier isPaused() {
        require(paused, "Need to be paused");
        _;
    }
    ...
    function _payLoseTax(uint256 _amount) internal {
        for (uint256 i = 0; i < addressTaxReceiverArray.length; i++) {
            uint256 tfAmount = (_amount * addressTaxReceiverArray[i].portion) /
                1000;
            _tokenContract.transfer(
                addressTaxReceiverArray[i].receiver,
                tfAmount
            );
        }
    }

    function _payWinTax(uint256 _amount) internal {
        require(
    ...
    function addTaxReceiver(address newReceiver, uint256 portion)
        external
        onlyOwner
        isPaused
    {
        require(portions + portion <= 1000, "Can't exceed 100% total portions");
        addressTaxReceiverArray.push(AddressTaxReceiver(newReceiver, portion));
        addressTaxReceiverMapping[newReceiver] = true;
        portions += portion;
    }

    function removeTaxReceiver(address toRemove) external onlyOwner isPaused {
        addressTaxReceiverMapping[toRemove] = false;
        for (uint256 i = 0; i < addressTaxReceiverArray.length; i++) {
            if (address(addressTaxReceiverArray[i].receiver) == toRemove) {
                portions -= addressTaxReceiverArray[i].portion;

                addressTaxReceiverArray[i] = addressTaxReceiverArray[
                    addressTaxReceiverArray.length - 1
                ];
                addressTaxReceiverArray.pop();
                return;
            }
        }
    }
