...
contract RaffleContract is Ownable, IERC721Receiver {
    ...
    function createRaffleUSDC(
        ...
        unclaimedPool += rafflePrizeAmount_;
    ...
    function raffleById(uint256 id) public view returns (Raffle memory) {
        require(id <= _raffleId.current() && id > 0, 'Raffle id does not exist');
        return raffles[id];
    }
    ...
    function cancelRaffle(uint256 id) public onlyOwner {
        ...
        if (raffle.prizeType == PrizeTypes.Tokens) {
            ...
            unclaimedPool -= raffle.rafflePrize;
        } else {
            IERC721(NFT).safeTransferFrom(
                address(this),
                owner(),
        ...
        returns (Raffle[] memory)
    {
        ...
        require(totalRaffleHistory > 0, 'No cancelled or completed raffles exist');
    ...
    function claim(uint256 id) public {
        ...
        require(raffle.isPaid == PaidStatus.No, 'Raffle prize already paid out');
        if (raffle.prizeType == PrizeTypes.Tokens) {
    ...
    function getUnclaimedAmount() public view onlyOwner returns (uint256) {
        return unclaimedPool;
    }

    function withdrawUSDC() external onlyOwner {
        require(IERC20(USDC).transfer(owner(), address(this).balance - unclaimedPool));
    }
