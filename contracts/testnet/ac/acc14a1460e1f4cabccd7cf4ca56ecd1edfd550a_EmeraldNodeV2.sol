...
contract EmeraldNodeV2 is EmeraldNodeStorageV2 {
    ...
    function updateEmeraldCoinAddress(address _address)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function updateAvaxAddress(address _address)
        external
        onlyOwner
        returns (bool)
    {
        AvaxCoin = IERC20(_address);
        return true;
    }
    ...
    function updateBondTresuryAddress(address _address)
        external
        onlyOwner
        returns (bool)
    {
        bondTreasuryAddress = IBondAndTreasury(_address);
        return true;
    }
    ...
    function setTierReward(
        ...
        uint256[] calldata _nodePrice,
        uint256[] calldata _nodeAvaxPrice,
    ...
    function updateNodeIds(uint256[] calldata _ids)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function updateCount(uint256 _num, uint256 _num2)
        external
        onlyOwner
        returns (bool)
    {
        count = _num;
        IcoNodeCounter = _num2;
        return true;
    }
    ...
    function whiteListAddresses(address[] calldata _addresses, bool _status)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            whiteListAddress[_addresses[i]] = _status;
        }
    }
    ...
    function buyNode(Order memory order, bytes memory signature)
        external
        payable
        ...
        returns (bool)
    {
        ...
        uint256 nodeId;
        require(
            owner() == ECDSAUpgradeable.recover(order.key, signature),
            "EmeraldNodeV1: ERROR"
        );
        require(
            referenceDetails[msg.sender][order.ReferenceId] == false,
            "EmeraldNodeV1: ReferenceId is already used"
        );

        if (
            whiteListAddress[msg.sender] &&
            addressCount[msg.sender] < count &&
            nodeCounter <= nodeIds[1]
        ) {
            _tier = 1;
            ...
            IcoWhitelistData[msg.sender].user == msg.sender &&
            IcoWhitelistData[msg.sender].allowedNode >
            IcoWhitelistData[msg.sender].createdNode &&
            IcoNodeCounter <= nodeIds[7]
        ) {
            _tier = 7;
            addressCount[msg.sender] += 1;
            nodeId = IcoNodeCounter;
            IcoWhitelistData[msg.sender].createdNode += 1;
            IcoNodeCounter += 1;
        } else {
            for (uint256 i = 0; i < 5; i++) {
                if (
                    nodeIds[i] < RegularNodeCounter &&
                    nodeIds[i + 1] >= RegularNodeCounter
                ) {
                    _tier = i + 1;
                ...
                if (
                    nodeIds[5] < RegularNodeCounter &&
                    nodeIds[6] >= RegularNodeCounter
                ) {
                    _tier = 6;
                    nodeId = RegularNodeCounter;
                    RegularNodeCounter += 1;
        ...
        require(
            updatedTierDetails[_tier].rewardRate != 0 &&
                updatedTierDetails[_tier].tierEMnodePrice <=
                EmeraldCoin.balanceOf(msg.sender) &&
                updatedTierDetails[_tier].tierAVAXnodePrice == msg.value,
            "EmeraldERC20V1: Invalid price or Insufficient balance"
        );

        // Transfer & Approve Section
        EmeraldCoin.safeTransfer(
            msg.sender,
            address(this),
            updatedTierDetails[_tier].tierEMnodePrice
        );
    ...
    function viewTotalReward(address _address)
        public
        view
        returns (
    ...
    function viewNodeDetails(uint256 _nodeId)
        public
        view
        returns (
            uint256 NodeId,
        ...
        )
    {
        ...
        uint256 number = 0;
        if (nodeDetails[_nodeId].nodeOwner != address(0)) {
            number =
                uint256(block.timestamp - LastClaimTime) /
                updatedTierDetails[tierId].intervalTimestamp;
            uint256 rewardAmt = uint256(
                updatedTierDetails[tierId].rewardRate * number
            );
    ...
    function claimRewards() external isBlacklistedAddress returns (bool) {
        require(
            ownerNodes[msg.sender].length != 0,
            "EmeraldERC20V1: You do not owned any node"
        );
        ...
        for (uint256 i = 0; i < ownerNodes[msg.sender].length; i++) {
            ...
            Node storage data = nodeDetails[NodeId];
            data.lastClaimTimestamp += (updatedTierDetails[TierId]
        ...
        require(
            totalReward != 0,
            "EmeraldNodeV1: Zero reward cannot be claimed"
        );

        //Transfer & Approve Section
        EmeraldCoin.safeTransfer(owner(), address(this), totalReward);
        EmeraldCoin.approve(msg.sender, totalReward);
        EmeraldCoin.safeTransfer(address(this), msg.sender, totalReward);
        ...
        )
    {
        (
            ...
            uint256 RewardToBeClaimed,
            uint256 RewardAlreadyClaimed,
            uint256 TotalNodeReward, /*uint256 Number*/
    ...
    function viewAllDetails(address _user)
        public
        view
        returns (
            address NodeOwnerAddress,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < number; i++) {
            ...
            if (nodeOwner != address(0)) {
                NodeIds[j] = _node;
                TierIds[j] = tierId;
                RewardToBeClaimeds[j] = rewardToBeClaimed;
                RewardAlreadyClaimeds[j] = rewardAlreadyClaimed;
                TotalNodeRewards[j] = totalNodeReward;
                j++;
            }
        }
    }
        ...
        returns (bool)
    {
        require(
            ...
            "EmeraldNodeV1: Status is already updated"
        );
        blacklistAddresses[_address] = _status;
        return _status;
    }
    ...
    function IcoWhiteListAddresses(IcoWhitelistAddress[] calldata _addresses)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            IcoWhitelistAddress storage data = IcoWhitelistData[
                _addresses[i].user
            ];
            data.user = _addresses[i].user;
            data.allowedNode = _addresses[i].allowedNode;
        }
        return true;
    }

    // remove ICO whitlist
    function removeIcoWhiteListAddresses(address[] calldata _addresses)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            IcoWhitelistAddress storage data = IcoWhitelistData[_addresses[i]];
            data.user = address(0);
            data.allowedNode = 0;
        }
        return true;
    ...
    {
        TreasuryPriceInUSD = bondTreasuryAddress.checkTreasuryValueInUSD();
    }

    function updateBPNPercentage(uint256 _avax_percent, uint256 _em_percent)
        external
        onlyOwner
    {
        require(
            _avax_percent + _em_percent == 100,
            "EmeraldNodeV1: Total must be 100 percentage"
        );
        AvaxPercentage = _avax_percent;
        EMPercentage = _em_percent;
    }

    function totalNodes() public view returns (uint256 TotalNodes) {
        uint256 icoNodes = IcoNodeCounter - nodeIds[6]; // 0
        uint256 regularNodes = RegularNodeCounter - nodeIds[1]; // 25
        uint256 whitelistNodes = nodeCounter; // 2

        TotalNodes =
            (icoNodes + regularNodes + whitelistNodes) -
            (deletedNodeCounter + 3);
    }

    // sell node
    function sellNode(uint256 _nodeId) external returns (bool) {
        require(
            nodeDetails[_nodeId].nodeOwner == msg.sender,
            "EmeraldNodeV1: You are not owner"
        );

        nodeDetails[_nodeId].nodeOwner = address(0);
        // for (uint256 i = 0; i < ownerNodes[msg.sender].length; i++) {
        //     if (ownerNodes[msg.sender][i] == _nodeId) {
        //         delete ownerNodes[msg.sender][i];
        //     }
        // }

        deletedNodeCounter += 1;

        uint256 bpn = bondTreasuryAddress.checkTreasuryValueInUSD() /
            totalNodes(); // divide by 10**8

        uint256 _avax_value = feeCalculation(AvaxPercentage, bpn); // change price to value
        uint256 _em_value = bpn - _avax_value;

        uint256 _avax_price = (_avax_value * (10**18)) /
            uint256(bondTreasuryAddress.getAvaxToUsdPrice());

        AvaxCoin.transferFrom(owner(), address(this), _avax_price);
        AvaxCoin.transfer(msg.sender, _avax_price);

        uint256 _em_price = (_em_value * (10**18)) /
            uint256(bondTreasuryAddress.getEmToUSDPrice());
        EmeraldCoin.safeTransfer(owner(), address(this), _em_price);
        EmeraldCoin.approve(msg.sender, _em_price);
        EmeraldCoin.safeTransfer(address(this), msg.sender, _em_price);

        emit nodeBurned(_nodeId, msg.sender, _avax_price, _em_price);
 ...
 * @dev Interface of the ERC20 standard as defined in the EIP.
 */
interface IBondAndTreasury{
    /**
     * @dev Returns the amount of tokens in existence.
     */
     function getEmToUSDPrice() external returns (int256);

     function treasuryAccount() external returns(address);

     function getAvaxToUsdPrice() external returns (int256);

     function checkTreasuryValueInUSD() external view returns(uint256 TreasuryPriceInUSD);
...
contract EmeraldNodeStorageV2 is Initializable, OwnableUpgradeable{
    ...
    struct IcoWhitelistAddress{
        address user;
        uint256 allowedNode;
        uint256 createdNode;
    }

    // [[0x7226818A761b9D20A995F5Ff7360c543E7C6e347,2,0],[0x7226818A761b9D20A995F5Ff7360c543E7C6e347,2,0]]

    mapping(address => IcoWhitelistAddress) public IcoWhitelistData;
    
    IBondAndTreasury public bondTreasuryAddress;

    uint256 public AvaxPercentage;
    uint256 public EMPercentage;

    uint256 public deletedNodeCounter;

    IERC20 public AvaxCoin;
    ...
    */
    ...
    event nodeBurned(uint256 nodeId, address nodeOwner, uint256 avaxPrice, uint256 emPrice);
}
