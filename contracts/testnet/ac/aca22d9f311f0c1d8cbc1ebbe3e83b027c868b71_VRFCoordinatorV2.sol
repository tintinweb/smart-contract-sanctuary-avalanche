...
contract VRF {
  ...
  function hashToCurve(uint256[2] memory pk, uint256 input) public view returns (uint256[2] memory rv) {
  ...
  ERC677ReceiverInterface
{
  ...
  function hashToCurveView(uint256[2] memory pk, uint256 input) public view returns (uint256[2] memory rv) {
    rv = newCandidateSecp256k1Point(abi.encodePacked(HASH_TO_CURVE_HASH_PREFIX, pk, input));
    while (!isOnCurve(rv)) {
      rv = newCandidateSecp256k1Point(abi.encodePacked(rv[0]));
    }
