...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256[4] memory _fees =[myLib.doPercentage(nodeAmount,rewardsFee),myLib.doPercentage(nodeAmount,teamPoolFee),myLib.doPercentage(nodeAmount,treasuryFee),myLib.doPercentage(nodeAmount,overSeerFee)];
