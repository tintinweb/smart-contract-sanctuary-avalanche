    ...
    Ownable
{
    ...
    constructor()
        ERC721A("teste", "tetets")
    {
        _customBaseURI = "no";
        _price = 0.05 ether;
        _royaltyAmount = 10000;
        paused = true;
    ...
    function mintByOwner(address to, uint256 count) public onlyOwner {
        require(totalSupply() + count <= _maxMintable, "minting more than the max supply");
            _mint(to, count);
    }
    ...
    function mint(uint256 quantity) public payable {  
        require(msg.value >= (_price * quantity), "Not enough DOGE sent");
        require(totalSupply() + quantity <= _maxMintable, "minting more than the max supply");
        require(!paused, "minting not active.");

        payable(payoutAddress1).transfer(msg.value * 20/100);
        payable(payoutAddress2).transfer(msg.value * 80/100);
