...
contract AvaxSavingNew is ReentrancyGuard, Context {
    ...
    */
    ...
    function newGetUserReferalDetails(address userAddress)
        public
        view
        returns (
            uint256 level0SRC,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < level1.length; i++) {
            level1SRC = level1SRC.add(
                getUserStakeRewardCommision(level0[i])
                    .mul(STAKE_REWARD_COMMISSION_PERCENTS[1])
                    .div(PERCENTS_DIVIDER)
            );
            level1SC = level1SC.add(getUserStakeCommision(level0[i]));
            level1TES = level1TES.add(getUserTotalExestingStake(level0[i]));
        }
        for (uint256 i = 0; i < level2.length; i++) {
            ...
            level2SC = level2SC.add(getUserStakeCommision(level0[i]));
            level2TES = level2TES.add(getUserTotalExestingStake(level0[i]));
        }
    }
