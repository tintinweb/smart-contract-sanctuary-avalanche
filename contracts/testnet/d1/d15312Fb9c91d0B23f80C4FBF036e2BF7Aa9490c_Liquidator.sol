...
contract Liquidator is ILiquidator, Multicall, Withdrawals {
    ...
    // pool => swapper
    mapping(address => address) swappers;
    address immutable bentoBox;
    address immutable repayToken;

    constructor(address _bentoBox, address _repayToken, address _masterContract) {
        bentoBox = _bentoBox;
    ...
    function depositToBentoBox(uint256 amount) public payable onlyManager {
        IBentoBox(bentoBox).deposit(IERC20(repayToken), address(this), address(this), amount, uint256(0));
    }

    function setSwapper(address _pool, address _swapper) public onlyOwner {
        swappers[_pool] = _swapper;
    }

    function _isSolvent(IPool _pool, address _user, uint256 _exchangeRate) internal view returns (bool) {
        ...
        return
            IBentoBox(bentoBox).toAmount(
                ...
                collateralShare.mul(EXCHANGE_RATE_PRECISION / COLLATERIZATION_RATE_PRECISION).mul(_pool.COLLATERIZATION_RATE()),
                false
            ) >=
    ...
    function getBatchUserData(IPool _pool, address[] calldata _users) override external view returns (UserData[] memory) {
    ...
    function balanceOf(address _user, address _token) override public view returns (uint) {
    ...
    onlyManager
    {
        require(IERC20(repayToken).approve(_pool, type(uint256).max), "Approval error");

        IPool(_pool).liquidate(_users, _maxBorrowParts, address(this), ISwapper(swappers[_pool]));
    }
}
...
interface ILiquidator {
  ...
  function getBatchUserData(IPool _pool, address[] calldata users) external view returns (UserData[] memory);
...
interface IOracle {
    ...
    function get(bytes calldata data) external view returns (bool success, uint256 rate);
