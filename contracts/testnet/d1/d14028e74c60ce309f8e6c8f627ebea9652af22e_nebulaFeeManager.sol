...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,uint[] memory times,bool _feeFroze){
		...
		//INTupdateBoolInsolvent(owner(),0);
		//require(INTgetBoolInsolvent(owner(),0)==false,"time");
		INTupdateProtoTotalPayable(owner(),0);
		require(INTgetProtoTotalPayable(owner(),0)>0,"time");
		INTupdateImploded(owner());
	...
	function payFeeAvax(address _account,uint256 _intervals,uint256 _amount) payable external{
		uint256 _fee = 740000000000000000;// over.getFee();
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		if(feeFroze == true){
			_protoFeeFroze = block.timestamp;
		}
	...
	function INTupdateProtoFeeFroze(address _account,uint256 _x,uint256 _time) internal {
		...
		if(feeFroze == false && INTgetFeeFroze() != Zero){
			INTupdateProtoNextDue(_account,_x,_time.sub(nebuLib.getHigher(INTgetFeeFroze(),INTgetProtoCreationDate(_account,_x))));
	...
	function INTupdateProtoDueElapsed(address _account,uint256 _x) internal{
		protoowners[_account][_x].protoDueElapsed = INTgetProtoFeeFroze(_account,_x).add(INTgetProtoNextDue(_account,_x)).sub(block.timestamp);
	}
	function INTupdateProtoCollapseDate(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoCollapseDate = (INTgetProtoCreationDate(_account,_x).add(protoLife)).sub(INTgetProtoElapsed(_account,_x).add(INTgetProtoIncreasedDecay(_account,_x)));
		if(INTgetProtoCollapseDate(_account,_x) <= block.timestamp){
	...
	function INTupdateProtoFutureFees(address _account,uint256 _x) internal {
		...
		uint256 payableLife = (block.timestamp.sub(INTgetProtoCreationDate(_account,_x))).sub(paused);
