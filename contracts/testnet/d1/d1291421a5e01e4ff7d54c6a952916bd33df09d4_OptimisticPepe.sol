...
contract OptimisticPepe is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public constant cexShare = 60000000000000 * 10**18; // Cex Listing, MEXC Binance
    ...
    function claim(bytes32[] calldata merkleProof, address referral) external payable nonReentrant {
    require(!claimed[msg.sender], "Already claimed");

    uint256 tokensToMint = claimAmount;

    if (msg.value > 0) {
        uint256 additionalTokens = msg.value * ethToTokenRate;
        tokensToMint += additionalTokens;
    }

    require(totalClaimed + tokensToMint <= opAirdrop, "Max supply reached");
    ...
    if (referral != address(0)) {
        uint256 referralBonus = tokensToMint * 10 / 100;
        _mint(referral, referralBonus);
    }
