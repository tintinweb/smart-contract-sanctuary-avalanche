...
contract AvalancheSideBridge is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event TokensReturned(address indexed requester, bytes32 indexed sideDepositHash, uint amount, uint timestamp , uint256 chainId);
   ...
   _disableInitializers();
    ...
    function bridgeTokens (address _requester, uint _bridgedAmount, bytes32 _mainDepositHash) verifyInitialization onlyGateway  external {
        sideToken.mint(_requester,_bridgedAmount);
         totalMintedAmount = totalMintedAmount + _bridgedAmount ;
        emit TokensBridged(_requester, _mainDepositHash, _bridgedAmount, block.timestamp);
    }
    function returnTokens (address _requester, uint256 _chainId , uint _bridgedAmount, bytes32 _sideDepositHash ) verifyInitialization  external payable  {
        ...
        totalBurnedAmount = totalBurnedAmount + _bridgedAmount ;
        emit TokensReturned(_requester, _sideDepositHash, _bridgedAmount, block.timestamp ,_chainId);
    }
