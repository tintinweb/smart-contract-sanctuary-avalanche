...
contract Review is Context, Ownable{
    ...
    function review(uint256 reviewId, address storeContract, address nftAddress, uint256 nftId, uint stars) public{      
        ...
        if(isOwner){
            ...
            IERC20Mintable(rewardToken).mint(account, reward);
        }
        if(isRenter){ 
            ...
            IERC20Mintable(rewardToken).mint(account, reward.sub(ownerReward));
        }   

        IERC20Mintable(rewardToken).mint(storeContract, stars * 1e18);
        IStakingFactory(factoryContract).notifyRewardAmount(storeContract, stars * 1e18);
