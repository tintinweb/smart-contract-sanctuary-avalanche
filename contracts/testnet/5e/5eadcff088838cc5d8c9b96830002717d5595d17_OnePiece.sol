...
contract OnePieceNodeManager is Ownable {
    ...
    uint256[] public _upgradePrice = [0, 0, 0];
    // Datas
    ...
    function _setUpgradePrice(uint256 nodeTypeId, uint256 price) external onlyOwner{
        require(nodeTypeId < _nodeTypes.length && nodeTypeId >= 0, "Not valid id of a type of Node");
        _upgradePrice[nodeTypeId] = price;
    }

    function _getUpgradePrice() external view returns(uint256[] memory){
    ...
    function _upgradeNode(address user, uint256 id) external onlyOwner returns(uint256) {
        Node[] storage nodes = usersNodes[user];
      ...
      NodeType storage nodeType = _nodeTypes[node.nodeType];
        ...
        return _upgradePrice[node.nodeType];
    }
      ...
      uint256 result = 0;
        for (uint256 i = 0; i < nodes.length; i++) {
            ...
            if (!node.upgrading && nodeType.upgradable) {
                node.upgrading = true;
                node.OnePieceUpgradeStartTimestamp = block.timestamp;
                result += _upgradePrice[node.nodeType];
            }
        }
...
contract OnePiece is ERC20, Ownable {
    ...
    function upgradeNode(uint256 id) external returns(bool) {
      ...
      uint256 fee = nodeManager._upgradeNode(sender, id);
        if (fee != 0) {
            require(balanceOf(sender) >= fee,
                "You have not the balance to buy this node");
            super._transfer(sender, _buyNodePool, fee);
        }
    ...
    function upgradeAllNodes() external returns(uint256) {
      ...
      uint256 fee = nodeManager._upgradeAllNodes(sender);
        if (fee != 0) {
            require(balanceOf(sender) >= fee,
                "You have not the balance to buy this node");
    ...
    function setUpgradePrice(uint256 nodeTypeId, uint256 amount) external {
        nodeManager._setUpgradePrice(nodeTypeId, amount);
    }

    function getUpgradePrice() external view returns(uint256[] memory) {
        return nodeManager._getUpgradePrice();
    }
