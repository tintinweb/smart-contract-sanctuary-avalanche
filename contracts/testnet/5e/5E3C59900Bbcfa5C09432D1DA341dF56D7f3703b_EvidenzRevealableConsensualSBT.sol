...
abstract contract CustomTemplate is Ownable {
    ...
    constructor(Template memory template_) {
        _template = template_;
    }

    function getTemplateId() external view returns (string memory) {
        return _template.id;
    }

    function getTemplateName() public view returns (string memory) {
        return _template.name;
    }

    function setTemplate(string memory newTemplate) external onlyOwner {
    ...
    function setMetadata(string memory newMetadata) external onlyOwner {
        _template.metadata = newMetadata;
    }

    function getMetadata() external view returns (string memory) {
        return _template.metadata;
    }

    function setTemplateReader(
        Environment.Endpoint memory newReader
    ) external onlyOwner {
        _template.reader = newReader;
    }

    function getTemplateReader()
        public
        view
        returns (Environment.Endpoint memory)
    {
        return _template.reader;
    }

    function setTemplateToolbox(
        Environment.Endpoint memory newToolbox
    ) external onlyOwner {
        _template.toolbox = newToolbox;
    }

    function getTemplateToolbox()
        public
        view
        returns (Environment.Endpoint memory)
    {
        return _template.toolbox;
    }
}
...
abstract contract Evidenz is ERC721, Ownable, CustomTemplate {
    ...
    Issuer internal immutable issuer;
    ...
    constructor(
        Issuer.Attributes memory issuer_,
        Template memory template
    ) CustomTemplate(template) {
        issuer = new Issuer(issuer_);
    }
    ...
    function _mint(
        ...
        _safeMint(to, tokenId);
        _data[tokenId] = data;
        _hashedPinCodes[tokenId] = hashedPinCode;
    ...
    function _getImage(
        ...
        return getTemplateToolbox().buildURL(publicKeys[tokenId]);
    }

    function _getExternalUrl(
        uint256 tokenId
    ) internal view virtual returns (string memory) {
        return getTemplateReader().buildURL(publicKeys[tokenId]);
    }
    ...
    function _requirePinCode(
        ...
        bytes32 hashedPinCode = keccak256(abi.encodePacked(pinCode));
        require(
            _hashedPinCodes[tokenId] == hashedPinCode,
    ...
    function _requireClaimed(uint256 tokenId) internal view {
        require(ownerOf(tokenId) != owner(), 'Evidenz: token must be claimed');
    }
    ...
    function setIssuer(Issuer.Attributes memory issuer_) external onlyOwner {
        issuer.setAttributes(issuer_);
    }
}
...
abstract contract EvidenzRevealable is Evidenz {
    using Strings for uint256;
    ...
    constructor(
        Issuer.Attributes memory issuer_,
        CustomTemplate.Template memory template,
        string memory defaultImage
    ) Evidenz(issuer_, template) {
    ...
    function _reveal(uint256 tokenId, string memory publicKey) internal {
        _requireMinted(tokenId);
        require(
            _hashedPublicKeys[tokenId] ==
                keccak256(abi.encodePacked(publicKey)),
            'Evidenz: invalid public key'
        );
        require(!isRevealed(tokenId), 'Evidenz: revealed already');
    ...
    function _requireOwned(uint256 tokenId) internal view {
        require(
            ...
            'Evidenz: caller not token owner'
        );
    }
    ...
    function _getImage(
        ...
        if (isRevealed(tokenId)) return super._getImage(tokenId);
        else return string(getDefaultImage());
    }

    function _getExternalUrl(
        uint256 tokenId
    ) internal view virtual override returns (string memory) {
        if (isRevealed(tokenId)) return super._getExternalUrl(tokenId);
        else return issuer.url();
    }
    ...
    function setDefaultImage(string memory newDefaultImage) external onlyOwner {
...
contract Issuer is Ownable {
    Attributes private _attributes;
    ...
    constructor(Attributes memory attributes) {
        _attributes = attributes;
    }

    function setAttributes(Attributes memory newAttributes) external onlyOwner {
        _attributes.name = newAttributes.name;
        _attributes.url = newAttributes.url;
    }

    function url() external view returns (string memory) {
        return _attributes.url;
    }
...
abstract contract ERC721Renamable is ERC721, Ownable {
...
import {EvidenzRevealable} from '../domain/EvidenzRevealable.sol';
import {Issuer} from '../domain/Issuer.sol';
import {CustomTemplate} from '../domain/CustomTemplate.sol';
    ...
    EvidenzRevealable
{
    constructor(
        ...
        ERC5484Burnable(name_, symbol, burnAuth)
        EvidenzRevealable(issuer_, template, defaultImage)
        ERC721Renamable(name_)
    {}
    ...
    function reveal(uint256 tokenId, string memory publicKey) external {
        _requireMinted(tokenId);
        _requireClaimed(tokenId);
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC721, ERC5484) returns (bool) {
    ...
    function tokenURI(
        uint256 tokenId
    ) public view virtual override(ERC721, Evidenz) returns (string memory) {
    ...
    function _burn(
        uint256 tokenId
    ) internal virtual override(ERC721, EvidenzRevealable) {
        return EvidenzRevealable._burn(tokenId);
    }
}
...
library Environment {
    ...
    function buildURL(
        ...
        string memory url = string(
            abi.encodePacked(endpoint.url, '/', publicKey)
        );
