...

{
    ...
    uint256 private maxMintable = 1100;
    string private customBaseURI;
    string private unrevealedURI;
    string private URISuffix = ".json";
    uint256 private royaltyAmount;
    uint256 private maxPerWallet;
    uint256[] private tierMints = [0, 10, 5, 2];
    uint256[] private tierPrice = [999*10**10, 400, 250, 0];
    ...
    constructor()
        ERC721A("TieredMintTest", "TIER")
        clampedVRF(431, maxMintable, uint32(maxMintable/REVEAL_BATCH_SIZE))
    {
        customBaseURI = "revealed";
        unrevealedURI = "unrevealed";
        price = 10 * 10**15; //price in milli
        royaltyAmount = 500;
        maxPerWallet = 100;
        tierMints = [0, 10, 5, 2];
        tierPrice = [999*10**10, 400, 250, 0];
        paused = true;
        wlTime = 0;
        publicTime = 1;
    }
    ...
    mapping(address => uint256) public wlTier;
    mapping(uint256 => address) royaltyReceiver;
    function addWl(address[] calldata addresses, uint256 tier) external onlyOwner{
        for (uint i = 0; i <addresses.length; i++){
            wlTier[addresses[i]] = tier;
        }
    }
    ...
    function mint(uint256 quantity) external payable{
        ...
        require(msg.value >= price, "incorrect amount of avax sent");
    ...
    function tier1Mint(uint256 quantity) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(wlTier[msg.sender] == 1, "wrong wl tier");
        require(msg.value >= tierPrice[1], "incorrect amount of avax sent");
        require(quantity > REVEAL_BATCH_SIZE, "this is gonna break the internet");
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
    ...
    function tier2Mint(uint256 quantity) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(wlTier[msg.sender] == 2, "wrong wl tier");
        require(msg.value >= tierPrice[2], "incorrect amount of avax sent");
        require(quantity > REVEAL_BATCH_SIZE, "this is gonna break the internet");
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
    ...
    function tier3Mint(uint256 quantity) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(wlTier[msg.sender] == 3, "wrong wl tier");
        require(msg.value >= tierPrice[3], "incorrect amount of avax sent");
        require(quantity > REVEAL_BATCH_SIZE, "this is gonna break the internet");
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
...
abstract contract BatchReveal {
    ...
    uint constant public REVEAL_BATCH_SIZE = 5;
