...
contract DcRouterV1 is DcRouter {

    function bridgeFromOwner(
 ...
 */
contract DcRouter is NonBlockingBaseApp, UUPSUpgradeable {
    /// deCommas address of Building block contract for lost funds
    address private _deCommasTreasurer;

    // @dev pending deposits for strategy per user. strategyId => user => amount
    mapping(uint256 => mapping(address => uint256)) public pendingDeposits;
    ...
    function initialize(
        ...
        __Ownable_init();
        __LzAppUpgradeable_init(_nativeLZEndpoint);
        _deCommasTreasurer = _deCommasTreasurerAddress;
        _nativeChainId = _nativeId;
        lzEndpoint = ILayerZeroEndpointUpgradeable(_nativeLZEndpoint);
    ...
    function deposit(uint256 _strategyId, uint256 _stableAmount) external {
        ...
        pendingDeposits[_strategyId][_msgSender()] = _stableAmount;
    ...
    function initiateWithdraw(uint256 _strategyId, uint256 _stableAmount)
        external
    {
        _pendingWithdrawn[_msgSender()][_strategyId] = _stableAmount;
    ...
    function getPendingTokensToWithdraw(address _user, uint256 _strategyId)
        ...
        returns (uint256)
    {
        return _pendingWithdrawn[_user][_strategyId];
    }
    ...
    function transferDeposits(bytes memory _payload) public onlySelf {
        ...
        (
            ...
            bytes4 destSelector
                ...
                _payload,
                (address[], address, address, uint256, uint16, bytes4)
            );
        uint256[] memory amounts = new uint256[](users.length);

        for (uint256 i = 0; i < users.length; i++) {
            uint256 pendingAmount = pendingDeposits[strategyId][users[i]];
            total += pendingAmount;
            amounts[i] = pendingAmount;
        }
        ...
        if (total > 0) {
            _bridge(
                ...
                destinationToken,
                0, //msg.value
                abi.encodeWithSelector(destSelector, abi.encode(users, amounts))
            );
        }
    }
    ...
    function approveWithdraw(
        ...
        uint256 stableWithdraw = (_pendingWithdrawn[_user][_strategyId] *
            _stableDeTokenPrice) / 1e18;
        require(
            _pendingWithdrawn[_user][_strategyId] != 0,
        ...
        returns (bool)
    {
        ...
        emit CancelWithdrawn(
            _user,
            _strategyId,
            _pendingWithdrawn[_user][_strategyId]
        );
        return true;
    }
