...
contract NitroPFP is ERC721, Ownable, SignatureChecker {
    ...
    event MintNitroPFP(address indexed to, uint256 tokenid);
    ...
    function bulkMintPFPs(
        Pass memory pass,
        uint256 nonce,
        uint256 price,
        uint256[] memory tokenIds
    ) public payable checker(pass, nonce, price, tokenIds) {
        require(msg.value >= price, "Low Fee");
        uint excess = msg.value - price;
        _mintedAddresses[msg.sender][nonce] = true;

        for (uint i = 0; i < tokenIds.length; i++) {
            _safeMint(msg.sender, tokenIds[i]);
            emit MintNitroPFP(msg.sender, tokenIds[i]);
        }
...
abstract contract SignatureChecker {
    ...
    function _isVerifiedPass(
        ...
        // console.log(signer, _passSigner);
    ...
    modifier checker(
        ...
        require(!_mintedAddresses[msg.sender][nonce], "Invalid nonce");
        _;
    }
