...
import "./MadSkullzSteps.sol";

/// @custom:security-contact [email protected]
contract MadSkullz is MadSkullzSteps, ERC721, ERC721Enumerable, ERC2981 {
    ...
    constructor(string memory baseTokenURI, address royaltyRecipient)
        ERC721("MadSkullz", "MADSKULLZ")
    {
        ...
        _setDefaultRoyalty(royaltyRecipient, uint96(ROYALTIES_VALUE));
    }
    ...
    function randomMint(address receiver, uint256 nextTokenIndex) internal {
        uint256 nextIndexerId = MAX_SUPPLY - totalSupply() == 0
            ? 0
            : uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function holderMint(uint256 amount) external payable {
    ...
    function whitelistMint(uint256 amount) external payable {
        require(
            _isWhitelist() || _isHolder(),
            "MadSkullz: You are not whitelisted"
        );
        require(
    ...
    function _mint(uint256 amount) internal {
        ...
        uint256 totalCost = startingPrice *
            amount -
        ...
        returns (string memory)
    {
        ...
        if (bytes(_baseUri).length > 0) {
            if (_reveals[tokenId] || _autoRevealed) {
...
import "./MadSkullzRoles.sol";

/// @custom:security-contact [email protected]
contract MadSkullzSteps is MadSkullzRoles {
    enum ContractSteps {
        CLOSED,
        OPEN_HOLDER,
        OPEN_WHITELIST,
...
/// @custom:security-contact [email protected]
contract MadSkullzRoles is AccessControl {
