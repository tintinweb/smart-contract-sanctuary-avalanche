...
contract BusinessAddresses {
    event BusinessNew(address acount);
    event BusinessCancel(address acount);

    mapping(address => bool) internal _isBusiness;

    function inBusinessList(address _address) external view returns (bool) {
        return _isBusiness[_address];
    }

    function _acceptBusinessAddress(address _address) internal {
        _isBusiness[_address] = true;
        emit BusinessNew(_address);
    }

    function _cancelBusinessAddress(address _address) internal {
        _isBusiness[_address] = false;
...
contract StakingRewards is ReentrancyGuard, Ownable, BusinessAddresses {
    using SafeMath for uint256;
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwnerOrBusiness updateReward(address(0)) {
    ...
    function cancelBusinessAddresses(address _address) external onlyOwner {
    ...
    modifier onlyOwnerOrBusiness() {
        ...
        require(owner() == sender || _isBusiness[sender], "Authorized: caller is not the owner or business contract");
        _;
    }
