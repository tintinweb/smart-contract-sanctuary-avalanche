...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	function delBuildNumber19() internal pure {} //etherscan trick
		...
		nonReentrant
	{
		...
		if (boughtTickets[_msgSender()] < royaltiesTicketsThreshold) {
			underRoyaltiesTickets[_msgSender()]++;
		}

		if (underRoyaltiesTickets[_msgSender()] >= royaltiesTicketsThreshold) {
	...
	function withdrawRoyalties() external notContract whenNotPaused nonReentrant {
		...
		Round storage currentRound = rounds[currentRoundIndex];
		require(currentRound.leader != _msgSender(), "leader cannot withdraw royalties");
	...
	// withdrawal to owner of some funds (used for testing or stuck royalties)
