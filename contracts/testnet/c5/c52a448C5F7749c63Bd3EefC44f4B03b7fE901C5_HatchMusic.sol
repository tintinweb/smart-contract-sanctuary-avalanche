...
contract HatchMusic is ERC1155, Ownable, ReentrancyGuard {
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < _trackIds.length; ++i) {
            ...
            uint256 trackNetCost = trackCost -
                ((trackCost * hatchMintingFee) / 10000);
            for (
                uint256 j = 0;
                j < track[_trackIds[i]].artistsWallets.length;
                ++j
            ) {
                uint256 artistsShare = (trackNetCost *
                    track[_trackIds[i]].artistsShares[j]) / 10000;
                artistBalance[
                    track[_trackIds[i]].artistsWallets[j]
                ] += artistsShare;
            }
        }
    ...
    function createTrackPack(
        ...
        require(
            _artistsWallets.length <= maxArtistsPerTrack,
            "Too many artists"
        );
        require(
        ...
        require(
            MerkleProof.verify(_merkleProof, roots[_rootId], leaf),
    ...
    function createTrackInternal(
        ...
        for (uint256 i = 0; i < _trackNames.length; ++i) {
            ...
            emit URI(_uris[i], currentTrackId);
            emit TracksCreated(
