...
contract Marketplace is Ownable {
  ...
  event DeployedItemForSale(ERC721 collection, uint256 id, uint256 tokenId, address minter, uint256 newPrice);
  event ItemSold(ERC721 collection, uint256 id, uint256 tokenId, address minter, address buyer, uint256 price);
  event UpdateTokenPrice(ERC721 collection, uint256 id, uint256 tokenId, uint256 newPrice);
  event UpdateTokenIsForSale(ERC721 collection, uint256 id, uint256 tokenId, bool isForSale);
  ...
  function updateTokenPrice(ERC721 collection_, uint256 id_, uint256 newPrice_)
    ItemExists(collection_, id_)
    public {
      uint256 _tokenId = itemsOnMarket[collection_][id_].tokenId;
      ...
      require(newPrice_ > 0, "price must be bigger than 0.");

      itemsOnMarket[collection_][id_].price = newPrice_;
      emit UpdateTokenPrice(collection_, id_, _tokenId, newPrice_);
  }

  function toggleItemForSale(ERC721 collection_, uint256 id_, bool isForSale_)
    ItemExists(collection_, id_)
    public {
      bool  _isForSale = itemsOnMarket[collection_][id_].isForSale;
      uint256 _tokenId = itemsOnMarket[collection_][id_].tokenId;
      address _ownerOfItem = collection_.ownerOf(_tokenId);

      require(_isForSale != isForSale_, "You don't feel to need update for sale.");
      require(_ownerOfItem == msg.sender, "Sender does not own the item.");

      itemsOnMarket[collection_][id_].isForSale = isForSale_;
      emit UpdateTokenIsForSale(collection_, id_, _tokenId, isForSale_);
  }
