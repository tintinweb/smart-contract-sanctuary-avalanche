    ...
    ReentrancyGuard
{
    ...
    event nftMinted(uint256 indexed quantity, address user, uint256 timestamp);
    ...
    function batchMintTo(
        ...
        if (isWhitelisted(msg.sender)) {
            _batchMintMetadata(nextTokenIdToMint(), _quantity, _baseURI); // more than 1 metadata is minting
            _safeMint(_to, _quantity, _data); // _safeMint ERC721A token standard method
            emit nftMinted(_quantity, msg.sender, block.timestamp); // log to build mint stats
        } else {
            require(false, "Not authorized to mint.");
        }
    }
