...
contract AsgardianBullsGiveaway is Ownable {
    ...
    mapping(uint256 => bool) public _legendaryBulls;
    mapping(uint256 => bool) public _blacklistedBulls;
    ...
    constructor(address _asgardianBullsTokenContract) {
        asgardianBullsTokenContract = IERC721Enumerable(_asgardianBullsTokenContract);

        legendaryBullTokenIds.push(1050);
        legendaryBullTokenIds.push(2315);
        legendaryBullTokenIds.push(3805);
        legendaryBullTokenIds.push(4475);
        legendaryBullTokenIds.push(5395);
        legendaryBullTokenIds.push(7115);
        legendaryBullTokenIds.push(7802);

        for (uint256 i; i < legendaryBullTokenIds.length; i++) {
            _legendaryBulls[legendaryBullTokenIds[i]] = true;
        }
    ...
    function selectRandomWinners(uint256 numberOfDraws) external onlyAdmin {
        ...
        for (uint256 i; i < numberOfDraws; i++) {
            ...
            if(bullWinDetail.tokenId == 0) {
                ...
                // ex. 10000 - 5 * 1000 = 5000 so 50% win rate assuming 5 wins and a winRateDropPerWin of 1000
                ...
                if(won) {
                    updateBullWinDetail(tokenId, isReset);
                } else {
                    continue;
                }
            }
            ...
            addBullWinner(totalWinnerCount, giveawayId, tokenId, bullWinnerAddress);
    ...
    function getWinPercentageForToken(uint256 tokenId) public view returns (uint256) {
    ...
    function addNewBullWinDetail(uint256 tokenId) internal {
        bullWinDetail = BullWinDetail(tokenId, 1, latestWinResetBlock);
    ...
    function updateBullWinDetail(uint256 tokenId, bool reset) internal {
        bullWinDetail = bullWinDetails[tokenId];
