    ...
    UUPSProxiable
{
    ...
    function swapEnoughQuoteAmount(IERC20 _quoteToken, Decimal.decimal memory _requiredQuoteAmount)
        internal
    {
        ...
        if (_requiredQuoteAmount.toUint() > 0) {
            Decimal.decimal memory requiredIfnxAmount = exchange.getOutputPrice(
                ifnxToken,
                _quoteToken,
                _requiredQuoteAmount
            );
    ...
    function getOrderedQuoteTokens(IERC20 _exceptionQuoteToken)
        internal
        view
        returns (IERC20[] memory orderedTokens)
    {
        ...
        for (uint256 i = 0; i < getQuoteTokenLength(); i++) {
            ...
            Decimal.decimal memory currentIfnxValue = exchange.getInputPrice(
                currentToken,
                ifnxToken,
            ...
            for (uint256 j = i; j > 0; j--) {
                Decimal.decimal memory subsetIfnxValue = exchange.getInputPrice(
                    tokens[j - 1],
                    ifnxToken,
    ...
    function proxiableUUID() public pure override returns (bytes32) {
        return keccak256("infinix-finance.InsuranceFund");
    }
