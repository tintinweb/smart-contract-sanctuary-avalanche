...
contract RaffleContract is Ownable {
    ...
    function createRaffle(
        ...
        if (prizeType_ == PrizeTypes.Tokens) {
            require(
            ...
            require(
                IERC721(NFT).getApproved(rafflePrize_) == address(this),
                'Contract not approved'
            );
            require(
                IERC721(NFT).ownerOf(rafflePrize_) == msg.sender,
                'Only owner'
            );
            IERC721(NFT).safeTransferFrom(owner(), address(this), rafflePrize_);
        }
        _raffleId.increment();
