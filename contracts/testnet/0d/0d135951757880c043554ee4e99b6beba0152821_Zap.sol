...
contract Zap is OwnableUpgradeable {
    ...
    /* ========== INITIALIZER ========== */
    function initialize(address _router, address _usdcAddress) external initializer {
        __Ownable_init();
    ...
    function zapOutToken(address _from, uint amount, address _to) external {
        ...
        IERC20Upgradeable(_to).safeTransfer(msg.sender, amountA.add(amountB));
    }
    ...
    function estimateZapOut(
        ...
        if (token0 == _tokenOut) {
            uint256 token0Swap = _estimateSwap(token1, reserve1, token0);
            return reserve0.add(token0Swap).mul(amount).div(totalSupply);
        } else if (token1 == _tokenOut) {
            uint256 token1Swap = _estimateSwap(token0, reserve0, token1);
            return reserve1.add(token1Swap).mul(amount).div(totalSupply);
        } else {
            return 0;
        }
    }
    ...
    function _dustDistribution(uint256 token0, uint256 token1, uint256 amountToken0, uint256 amountToken1, address native, address token, address recipient) private {
        ...
        if (nativeDust > 0) {
            IERC20Upgradeable(native).safeTransfer(recipient, nativeDust);
        }
        if (tokenDust > 0) {
            IERC20Upgradeable(token).safeTransfer(recipient, tokenDust);
        }
    }
