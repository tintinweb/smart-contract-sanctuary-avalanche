...
contract ConfigBaseData is Ownable{
    event SetUnboxNFTLimit(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel, uint256[]  rarities, uint256[]  minValues, uint256[]  maxValues);
    event SetBaseCoinAmount(uint256 baseCoin);
    event SetUpgradeFeeToken(address token);

    uint256 maxType; // 1 -> maxType
    uint256 maxRarity; // maxRarity = 3 => rarities[1,2,3] <=> 1: common, 2: uncommon, 3: rare
    uint256 maxLevel; // 1 -> maxLevel
    uint256 baseCoin;
    ...
    // string internal constant REVERT_ZERO_VALUE = "ConfigBaseData: Value can not be 0";
    ...
    mapping(uint256 => PerformanceLimit) basicPerformances;
    // level => magnification
    mapping(uint256 => uint256)  upgradeMagnifications;
    ...
    function _setUnboxNFTLimit(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel, uint256[] memory rarities, uint256[] memory minValues, uint256[] memory maxValues) internal{
        ...
        emit SetUnboxNFTLimit( _maxType, _maxRarity, _maxLevel, rarities,  minValues,  maxValues);
    }

    function setUnboxNFTLimit(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel, uint256[] memory rarities, uint256[] memory minValues, uint256[] memory maxValues) external onlyOwner{
        _setUnboxNFTLimit(_maxType, _maxRarity, _maxLevel, rarities, minValues, maxValues);
    }

    function getUnboxNFTLimitInfo(uint256 rarity) public view returns(uint256, uint256, uint256, PerformanceLimit memory){
    ...
    function _setBaseCoinAmount(uint256 _baseCoin) internal{
        baseCoin = _baseCoin;
    } 

    function setBaseCoinAmount(uint256 _baseCoin) external onlyOwner{
        _setBaseCoinAmount(_baseCoin);
    } 

    function getBaseCoinAmount() public view returns(uint256){
        return baseCoin;
    } 

    // max magnification = 3.8 ==> input: _maxMagnification = 38
    function _setUpgradeMagnifications(uint256 _maxMagnification) internal{
       for(uint256 i = maxLevel; i >= 2; i--){
           upgradeMagnifications[i] = _maxMagnification--;
       }
    } 

    function setUpgradeMagnifications(uint256 _maxMagnification) external onlyOwner{
        _setUpgradeMagnifications(_maxMagnification);
    }

    function getUpgradeFee(uint256 level) public view returns(uint256){
        return upgradeMagnifications[level] * baseCoin / 10;
    }
}
...
contract TestGenesisConvert is ERC4907, ERC721Burnable, BusinessAddresses, Signer, TransferFee, ReentrancyGuard, Withdrawable, ConfigBaseData, NonFungibleType {
    ...
    constructor() ERC4907("TestGenesisConvert", "TGC") {
        ...
        _setBaseCoinAmount(100);
        _setUpgradeMagnifications(38);
    }
