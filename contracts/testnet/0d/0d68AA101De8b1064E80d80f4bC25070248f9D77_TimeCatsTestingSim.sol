...
*/
contract TimeCatsTestingSim is
    ERC721Enumerable,
    Ownable,
    Multisig(msg.sender),
...
contract Multisig is Ownable {
    ...
    function containsSignatory(address signatorAddress) public view returns (bool, uint) {
    ...
    function removeSignatory(address signatorToRemove) public onlyOwner {
        require(_signatoryAddresses.length > 1, "ERROR: There must at least one removable signatory");
        require(signatorToRemove != _primarySignatory, "ERROR: Cannot remove owner");
        (bool isContained, uint at) = containsSignatory(signatorToRemove);
        require(isContained, "ERROR: address must be signator");
        _signatoryAddresses[at] = _signatoryAddresses[_signatoryAddresses.length - 1];
