...
contract YETIPriceFeed is IPriceFeed {
    ...
    JoePair public YETIYUSD;
    constructor(address LPToken) public {
        YETIYUSD = JoePair(LPToken);
    }

    function fetchPrice_v() view external override returns (uint256) {
        IERC20 t0 = IERC20(YETIYUSD.token0());
        IERC20 t1 = IERC20(YETIYUSD.token1());
        (uint112 r0, uint112 r1, ) = YETIYUSD.getReserves();
        uint8 t0Dec = t0.decimals();
        uint8 t1Dec = t1.decimals();
        uint256 price;
        if (t0Dec >= t1Dec) {
            price =  uint256(UQ112x112.encode(r0).uqdiv(r1 * (10 ** (t0Dec - t1Dec))));
            price = price.mul(10 ** 18).div(10 ** t0Dec);
        } else {
            price =  uint256(UQ112x112.encode(r0* (10 ** (t1Dec - t0Dec))).uqdiv(r1));
            price = price.mul(10 ** 18).div(10 ** t1Dec);
        }
        return price;
    }
}
