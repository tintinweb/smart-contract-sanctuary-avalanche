...
contract Marketplace is CollectionEnumerable, ReentrancyGuard, Ownable {
    ...
    // address thorV2Address = 0xb529782800e4a0feac1A01aE410E99E90c4C28AB; //testnet : 0xb529782800e4a0feac1A01aE410E99E90c4C28AB //mainnet : 0x8F47416CaE600bccF9530E9F3aeaA06bdD1Caa79
    ERC20 thorToken; // = ERC20(thorV2Address);
    ...
    function updateListingFee(uint256 _price) public onlyOwner {
        listingFee = _price;
    }
    ...
    function createMarketItem(
        ...
        NftCollection memory collection = collectionManager.fetchByCollectionId(
            collectionId
        );
        require(
            collection.status == uint8(CollectionStatus.LISTED),
        ...
        if (paymentMethod == uint8(PaymentMethod.AVAX)) {
            if (nftType == uint8(NftType.ERC721)) {
                ...
                require(
                    ...
                    "Listing fee must be equal to listing fee of ERC721"
                );
                ...
                require(
                    INodeRewardManagementNFT(nftContract).ownerOf(tokenId) ==
                        msg.sender,
                    "You are not owner of this token"
                );
                require(
                    msg.value == listingFee,
                    "Listing fee must be equal to listing fee of ThorNodeNFT"
                );
            }
        ...
        if (nftType == uint8(NftType.ERC721)) {
            ...
            INodeRewardManagementNFT(nftContract).transferFrom(
                msg.sender,
                address(this),
                tokenId
            );
    ...
    function _splitPaymentWithRoyalties(
        ...
        if (hasRoyalty) {
            ...
            ) = royaltyManagerMainnet.getRoyalty(nftContract, price);
     ...
     */
    function createMarketSale(uint256 marketItemId, uint256 saleAmountOfErc1155)
        public
        payable
        nonReentrant
    {
        ...
        if (paymentMethod == uint8(PaymentMethod.AVAX)) {
            ...
            if (nftType == uint8(NftType.ERC1155)) {
                require(
                    saleAmountOfErc1155 <= listAmount,
                    "Sale amount must be less than listed amount in ERC1155"
                );
            }
        }
        ...
        if (nftType == uint8(NftType.ERC721)) {
            ...
            if (paymentMethod == uint8(PaymentMethod.AVAX)) {
                ...
                thorToken.transfer(
                    owner(),
                    saleAmountOfErc1155.mul(listingFee)
                );
            }
        } else if (nftType == uint8(NftType.ThorNodeNFT)) {
            INodeRewardManagementNFT(nftContract).transferFrom(
...
contract CollectionManager is CollectionEnumerable {
    ...
    modifier onlyOwnerOf(uint256 collectionId_) {
        require(msg.sender == collectionToOwner[collectionId_], "no owner");
        _;
    }
     ...
     * @param status_- 0: LISTED, 1: BANNED, 2: REMOVED, 3: OTHERS
     *
     * returns null
     */
    function createCollection(
    ...
    ) external onlyNftType(nftType_) {
        require(bytes(name_).length > 0, "name should be not empty");
        require(bytes(symbol_).length > 0, "symbol should be not empty");
        require(bytes(uri_).length > 0, "uri should be not empty");

        collectionIds.increment();
        uint256 collectionId = collectionIds.current();

        if (nftType_ == uint8(NftType.ERC721)) {
            MockERC721 contractAddress = new MockERC721(name_, symbol_, uri_);
        ...
        } else if (nftType_ == uint8(NftType.ERC1155)) {
            MockERC1155 contractAddress = new MockERC1155(name_, symbol_, uri_);
            idToNftCollection[collectionId] = NftCollection(
                name_,
                symbol_,
                uri_,
        ...
        } else if (nftType_ == uint8(NftType.ThorNodeNFT)) {
            idToNftCollection[collectionId] = NftCollection(
                name_,
                symbol_,
                uri_,
        ...
        emit CollectionCreated(collectionId, nftType_);
     ...
     * mint Collection
     *
     * @param collectionId_ - 1, 2, 3...
     * @param value - 0(case ERC721), amount(case ERC1155)
     *
     * returns null
     */
    function mint(uint256 collectionId_, uint256 value)
        external
        onlyOwnerOf(collectionId_)
    {
        require(collectionId_ <= collectionIds.current());
        NftCollection memory nftCollection = idToNftCollection[collectionId_];
        if (nftCollection.nftType == uint8(NftType.ERC721)) {
            MockERC721 mockERC721 = MockERC721(nftCollection.nftContract);
            mockERC721.mint(msg.sender);
        } else if (nftCollection.nftType == uint8(NftType.ERC1155)) {
            MockERC1155 mockERC1155 = MockERC1155(nftCollection.nftContract);
            mockERC1155.mint(msg.sender, value);
        }
    }

    function fetchByCollectionId(uint256 collectionId_)
        public
        view
        returns (NftCollection memory)
    {
        require(collectionId_ <= collectionIds.current());
        return idToNftCollection[collectionId_];
    }

    function fetchMyCollections() public view returns (NftCollection[] memory) {
        uint256 itemCount = collectionIds.current();
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i + 1]._owner == msg.sender) {
                currentIndex += 1;
            }
        }

        NftCollection[] memory items = new NftCollection[](currentIndex);
        currentIndex = 0;
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i + 1]._owner == msg.sender) {
                items[currentIndex] = idToNftCollection[i + 1];
                currentIndex += 1;
            }
        }
        return items;
    ...
    function fetchCollectionsByOwner(address target_)
        public
        view
        returns (NftCollection[] memory)
    {
        require(target_ != address(0), "address to the zero address");
        uint256 itemCount = collectionIds.current();
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i + 1]._owner == target_) {
                currentIndex += 1;
            }
        }
     ...
     * update metadata
     */
    function updateName(uint256 collectionId_, string memory name_)
        external
        onlyOwnerOf(collectionId_)
    {
        require(collectionId_ <= collectionIds.current());
        require(bytes(name_).length > 0, "name should be not empty");
        idToNftCollection[collectionId_].name = name_;
    }

    function updateSymbol(uint256 collectionId_, string memory symbol_)
        external
        onlyOwnerOf(collectionId_)
    {
        require(collectionId_ <= collectionIds.current());
        require(bytes(symbol_).length > 0, "symbol should be not empty");
        idToNftCollection[collectionId_].symbol = symbol_;
    }

    function updateTokenUri(uint256 collectionId_, string memory tokenuri_)
        external
        onlyOwnerOf(collectionId_)
    {
        require(collectionId_ <= collectionIds.current());
        require(bytes(tokenuri_).length > 0, "token uri should be not empty");
        idToNftCollection[collectionId_].uri = tokenuri_;
    }

    function updateCollectionStatus(uint256 collectionId_, uint8 status)
        external
        onlyCollectionStatus(status)
    {
        require(collectionId_ <= collectionIds.current());
        idToNftCollection[collectionId_].status = status;
    }
}
...
contract MockERC721 is ERC721URIStorage {
    ...
    function mint(address owner) public returns (uint256) {
        ...
        string memory newTokenUri = string(
            abi.encodePacked(_tokenUri, Strings.toString(newItemId))
        );
        _mint(owner, newItemId);
        _setTokenURI(newItemId, newTokenUri);
        emit TokenMinted(newItemId, newTokenUri);
        return newItemId;
    }
}
