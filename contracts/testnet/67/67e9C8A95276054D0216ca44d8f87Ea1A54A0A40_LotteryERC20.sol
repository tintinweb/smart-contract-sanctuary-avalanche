...
contract LotteryERC20 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public royaltiesTicketsThreshold = 20; //how many tickets you should buy before you get royalties
    ...
    function setCurrentPhase() internal {
        if ((ticketsBoughtThisRound > phaseMaxTickets[1]) && (phase < 2)) {
            phase = 2;
            return;
        }
        if ((ticketsBoughtThisRound > phaseMaxTickets[2]) && (phase < 3)) {
            phase = 3;
            return;
        }
        if ((ticketsBoughtThisRound > phaseMaxTickets[3]) && (phase < 4)) {
            phase = 4;
            return;
        }
