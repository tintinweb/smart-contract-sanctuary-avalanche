...

{
    ...
    uint256 public maxMintable = 3000;
    ...
    bool public paused;
    bytes32 immutable public merkleRootT1;
    bytes32 immutable public merkleRootT2;
    bytes32 immutable public merkleRootT3;
    constructor(bytes32 _merkleRootT1, bytes32 _merkleRootT2, bytes32 _merkleRootT3)
        ERC721A("testies", "testes")
        VRF(431, 100)
    {
        customBaseURI = "revealed";
        unrevealedURI = "unrevealed";
        price = 0.01 ether; //price in milli avax (10**-3)
        royaltyAmount = 500;
    ...
    function setTierMints(uint256 tier1,uint256 tier2,uint256 tier3)public onlyOwner{
      tierMints = [0,tier1,tier2,tier3];
    }
    function setTierPrice(uint256 tier1,uint256 tier2,uint256 tier3)public onlyOwner{
    ...
    function maxValues(uint256 maxMintable_)
        public
        onlyOwner
    {
    ...
    function mint(uint256 quantity) external payable{
        ...
        require(quantity < REVEAL_BATCH_SIZE, "this is gonna break the internet");
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
        }
    }
    function tier1Mint(uint256 quantity, bytes32[] calldata merkleProof) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(msg.value >= tierPrice[1] * quantity, "incorrect amount of avax sent");
        require(quantity < REVEAL_BATCH_SIZE, "this is gonna break the internet");
        require(MerkleProof.verify(merkleProof, merkleRootT1, toBytes32(msg.sender)) == true, "invalid merkle proof");
        require(claimed[msg.sender] + quantity <= tierMints[1]);
    ...
    function tier2Mint(uint256 quantity, bytes32[] calldata merkleProof) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(msg.value >= tierPrice[2] * quantity, "incorrect amount of avax sent");
        require(quantity < REVEAL_BATCH_SIZE, "this is gonna break the internet");
        require(MerkleProof.verify(merkleProof, merkleRootT2, toBytes32(msg.sender)) == true, "invalid merkle proof");
        require(claimed[msg.sender] + quantity <= tierMints[2]);
        claimed[msg.sender] += quantity;
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
    ...
    function tier3Mint(uint256 quantity, bytes32[] calldata merkleProof) external payable{
        require(privateSale(), "private sale hasn't started yet");
        require(!paused, "minting is paused");
        require(msg.value >= tierPrice[3] * quantity, "incorrect amount of avax sent");
        require(quantity < REVEAL_BATCH_SIZE, "this is gonna break the internet");
        require(MerkleProof.verify(merkleProof, merkleRootT3, toBytes32(msg.sender)) == true, "invalid merkle proof");
        require(claimed[msg.sender] + quantity <= tierMints[3]);
        claimed[msg.sender] += quantity;
        _mint(msg.sender, quantity);
        if(totalSupply() >= (lastTokenRevealed + REVEAL_BATCH_SIZE)){
            setBatchSeed(getRandomness());
        ...
        returns (string memory)
    {
        if(tokenId >= lastTokenRevealed){
            ...
            return string.concat(customBaseURI, getShuffledTokenId(tokenId).toString(), URISuffix);
    }
    }
    ...
    {
        return (royaltyReceiver[_tokenId], ((_salePrice * royaltyAmount) / 10000));
    }
