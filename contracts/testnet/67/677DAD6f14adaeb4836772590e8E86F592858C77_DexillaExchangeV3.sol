...
contract DexillaExchangeV3 is AccessControl, Pausable, ReentrancyGuard, Multicall, SelfPermit {
    ...
    function _adjustOrderSize(uint8 side, uint price, uint desiredQuantity, bool receiveNative) private {
    ...
    function _findTotalAmount(address[] memory makers, uint8 side, uint price) private view returns (uint) {
        uint makerQuantity;
        for (uint i; i < makers.length; ) {
            if (side == 0) {
                makerQuantity += asks[makers[i]][price];
            } else {
                makerQuantity += bids[makers[i]][price];
            }
        ...
        return makerQuantity;
    }

    function _executeOrder(address[] memory makers, uint8 side, uint price, uint quantity, bool receiveNative) private {
    ...
    function _transferFrom(address token, address from, address to, uint amount) private {
        ...
        if (msg.value > 0) {
            _sum += amount;
            require(_sum <= msg.value, "Incorrect amount");
            if (token == weth) {
