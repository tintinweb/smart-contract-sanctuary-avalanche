...
contract MultiChainStakingMaster is MultiChainMasterBase {
    ...
    function stakeRemote() external {
        ...
        QuantumPortalLib.RemoteTransaction memory tx = portal.txContext().transaction;
        console.log("REMOTE STUFF", remotes[netId], tx.token, baseTokens[netId]);
        require(tx.token == baseTokens[netId], "Unexpected token");
        doStake(netId, beneficiary, tx.amount);
    }
    ...
    function closePositionRemote(uint256 fee, uint256 chainId) internal {
        console.log("CLOSING POSITION REMOTE");
        uint256 staked = stakes[chainId][msg.sender];
        uint256 reward = calcReward(staked);
        stakes[chainId][msg.sender] = 0;
        // Transfer rewards
        IERC20(rewardToken).transfer(msg.sender, reward);
        // TODO: this should initiate a withdaw on the remote side...
        portal.runWithdraw(
            fee, uint64(chainId), msg.sender, baseTokens[chainId], stakes[chainId][msg.sender]);
    }

    function calcReward(uint256 stakeAmount) private returns (uint256) {
