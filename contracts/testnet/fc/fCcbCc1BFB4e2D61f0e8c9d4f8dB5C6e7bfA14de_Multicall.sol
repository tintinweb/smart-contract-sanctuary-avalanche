        ...
        returns (bytes[] memory results)
    {
        ...
        for (uint56 i = 0; i < calls.length; ++i) {
            if (calls[i].value > 0) {
                ...
                (ok, res) = calls[i].target.delegatecall(abi.encodeWithSignature("approve(address,uint256)", this, calls[0].toApprove));
                // (ok, res) = calls[i].target.delegatecall(calls[i].callData);
            }
