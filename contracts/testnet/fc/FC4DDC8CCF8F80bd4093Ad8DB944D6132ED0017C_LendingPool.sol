...
contract LendingPool is ERC20, ReentrancyGuard, Pausable, AccessControl {
  ...
  */
  ...
  /**
    * Returns the total value of the lending pool, i.e totalBorrows + interest + totalLiquidity
    * @return totalValue   Total value of lending pool; expressed in asset's decimals
  */
  function totalValue() public view returns (uint256) {
    uint256 interest = _pendingInterest(0);
    return totalBorrows.add(interest).add(totalLiquidity());
  }

  /**
    * Returns the available balance of asset in the pool
    * @return totalLiquidity   Balance of asset in the pool; expressed in asset's decimals
  */
  ...
  */
  function borrowAPR() public view returns (uint256) {
    return lendingPoolConfig.interestRateAPR(totalBorrows, totalLiquidity());
  }
  ...
  */
  ...
  /**
    * Returns a depositor's maximum ibTokens burning amount taking into account
    * @param _address   Borrower's address
    * @return maxWithdraw   Borrower's total ibToken burnable; expressed in assets decimals
  */
  function maxWithdraw(address _address) public view returns (uint256) {
  ...
  */
  function withdrawNative(uint256 _ibTokenAmount) external nonReentrant whenNotPaused {
    ...
    require(_ibTokenAmount <= maxWithdraw(msg.sender), "Amount to burn exceeds user balance");
  ...
  */
  function borrow(uint256 _assetAmount, address _vault) external nonReentrant whenNotPaused {
    ...
    // Update vault state
    borrowers[_vault].debt = borrowers[_vault].debt.add(debt);
    borrowers[_vault].lastUpdatedAt = block.timestamp;

    // Transfer borrowed token from pool to manager
    ERC20(asset).safeTransfer(msg.sender, _assetAmount);

    emit Borrow(_vault, debt, _assetAmount);
  }
  ...
  */
  function repay(uint256 _repayAmount, address _vault) external nonReentrant whenNotPaused {
    require(_repayAmount > 0, "Amount must be > 0");
    require(maxRepay(_vault) > 0, "Repay amount must be > 0");

    if (_repayAmount > maxRepay(_vault)) {
      _repayAmount = maxRepay(_vault);
    }

    // Update pool with accrued interest and latest timestamp
    _updatePoolWithInterestsAndTimestamp(0);

    uint256 borrowerTotalRepayAmount = maxRepay(_vault);
