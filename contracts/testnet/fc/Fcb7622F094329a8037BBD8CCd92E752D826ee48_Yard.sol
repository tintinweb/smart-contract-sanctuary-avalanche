...
contract Yard is ERC20 {
    ...
    uint256 public veBOOSTER_MULTIPLIER = 1;
    uint256 public veFEED_FARMING_FACTOR = 20; // fish to feed ratio
    ...
    function _upsertFishStaking(
        ...
        // since its an upsert, we took out old egg and add new amount
        uint256 previousveFish = vefish.amount;
        // update stake
        vefish.user = user;
        vefish.amount = amount;
        vefish.since = uint32(block.timestamp);

        vefishStakeHolders[user] = vefish;
    ...
    function staking(uint256 amount) external {
        ...
        function vestaking(uint256 amount) external {
        require(amount > 0, "NEED EGG");

        Fish fishContract = Fish(FISH_CONTRACT);
        uint256 available = fishContract.balanceOf(msg.sender);
        require( available >= amount, "NOT ENOUGH FISH");
        veFishStake memory existingveFish = vefishStakeHolders[msg.sender];
        if (existingveFish.amount > 0) {
            // already have previous egg staked
            // need to calculate claimable
            uint256 projection = veclaimableView(msg.sender);
            // mint feed to wallet
            _mint(msg.sender, projection);
            //emit Minted(msg.sender, amount);
            _upsertveFishStaking(msg.sender, existingveFish.amount + (amount));
        } else {
            // no egg staked just update staking
    ...
    function claimableView(address user) public view returns (uint256) {
        ...
        function veclaimableView(address user) public view returns (uint256) {
        veFishStake memory vefish = vefishStakeHolders[user];
        require(vefish.user != address(0), "NOT STAKED For veclaimableView");
        // need to add 10000000000 to factor for decimal
        return
            ((vefish.amount * veFEED_FARMING_FACTOR) *
                (((block.timestamp - vefish.since) * 10000000000) / 86400) *
                veBOOSTER_MULTIPLIER) /
            10000000000;
    }
    ...
    function unstakeFish(uint256 amount) external {
        ...
        require(fish.user != address(0), "NOT STAKED for unstakeFish");
        require(amount <= fish.amount, "OVERDRAWN For unstakeFish");
        ...
        require(vefish.user != address(0), "NOT STAKED for unstakeveFish");
        require(amount <= vefish.amount, "OVERDRAWN FOR unstakeveFish");

        Fish fishContract = Fish(FISH_CONTRACT);
        // uint256 projection = claimableView(msg.sender);
        _upsertveFishStaking(msg.sender, vefish.amount - amount);
        // Need to burn 1/12 when withdrawing (breakage fee)
    ...
    function _removeUserFromFishEnumeration(address user) private {
       ...
       function _removeUserFromveFishEnumeration(address user) private {
        ...
        _allveFishStakeHolders[currentUserIndex] = lastUser; // Move the last token to the slot of the to-delete token
        _allveFishStakeHoldersIndex[lastUser] = currentUserIndex; // Update the moved token's index
     ...
     */
    function withdrawAllFishAndClaimFeed() external {
        FishStake memory fish = fishStakeHolders[msg.sender];

        // NOTE does this ever happen?
        require(fish.user != address(0), "NOT STAKED for withdrawAllFishAndClaimFeed");
    ...
    function _unstakingFish(address user) internal {
        ...
        function _unstakingveFish(address user) internal {
        veFishStake memory vefish = vefishStakeHolders[user];
        // NOTE when whould address be zero?
        require(vefish.user != address(0), "EMPTY ADDRESS");
        totalveFishStaked = totalveFishStaked - vefish.amount;
    ...
    function getFishHolderByIndex(uint256 index) internal view returns (address){
      ...
      function getveFishHolderByIndex(uint256 index) internal view returns (address){
        return _allveFishStakeHolders[index];
    }
    ...
    function rebalanceStakingPool(uint256 from, uint256 to) external  {
        ...
        function rebalanceveStakingPool(uint256 from, uint256 to) external  {
        // for each holder of staked Fish...
        for (uint256 i = from; i <= to; i++) {
            address holderAddress = getveFishHolderByIndex(i);

            // check how much feed is claimable...
            uint256 pendingClaim = veclaimableView(holderAddress);
            veFishStake memory vefish = vefishStakeHolders[holderAddress];
            ...
            // pdates user's amount of staked eggs to the given value. Resets the "since" timestamp.
            _upsertveFishStaking(holderAddress, vefish.amount);
        }
    }
}
