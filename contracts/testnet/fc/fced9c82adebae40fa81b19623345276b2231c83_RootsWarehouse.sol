...
import {IRootsWarehouse} from './interfaces/IRootsWarehouse.sol';
import {StateProofVerifier} from './libs/StateProofVerifier.sol';
import {RLPReader} from './libs/RLPReader.sol';

/**
 * @dev Stores account state roots and allows proving against them
 */
contract RootsWarehouse is IRootsWarehouse {
  using RLPReader for bytes;
  using RLPReader for RLPReader.RLPItem;

  string public constant ERROR_UNPROCESSED_STORAGE_ROOT =
    'ERROR_UNPROCESSED_STORAGE_ROOT';

  // account address => (block hash => Account state root hash)
  mapping(address => mapping(bytes32 => bytes32)) internal _storageRoots;

  mapping(bytes32 => uint128) internal _exchangeRates;
  ...
  function getTokenExchangeRate(bytes32 blockHash)
    public
    view
    returns (uint128)
  {
    return _exchangeRates[blockHash];
  }
  ...
  function registerExchangeRate(
    ...
    StateProofVerifier.SlotValue memory exchangeRateSlotValue = getStorage(
      asset,
      blockHash,
    ...
    (, uint128 exchangeRate, ) = abi.decode(
      abi.encode(exchangeRateSlotValue.value),
      (uint120, uint128, bool)
    );

    _exchangeRates[blockHash] = exchangeRate;
  }
}
