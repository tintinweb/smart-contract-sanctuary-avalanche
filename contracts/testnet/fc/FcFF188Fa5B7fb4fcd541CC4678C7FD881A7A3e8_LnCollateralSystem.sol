...
contract LnCollateralSystem is LnAdminUpgradeable, PausableUpgradeable, LnAddressCache {
    ...
    bytes32 public constant Currency_SAM = "SAM";
     ...
     * `getFreeCollateralInETH()` instead. This function is not removed since it's still
     * used by `LnBuildBurnSystem`.
     */
    function MaxRedeemableInETH(address _user) public view returns (uint256) {
        return getFreeCollateralInETH(_user);
    }

    function getFreeCollateralInETH(address user) public view returns (uint256) {
        uint256 totalCollateralInETH = GetUserTotalCollateralInETH(user);

        (uint256 debtBalance, ) = debtSystem.GetUserDebtBalanceInETH(user);
        if (debtBalance == 0) {
        ...
        if (totalCollateralInETH < minCollateral) {
            return 0;
        }

        return totalCollateralInETH.sub(minCollateral);
    }

    function maxRedeemableSam(address user) public view returns (uint256) {
        (uint256 debtBalance, ) = debtSystem.GetUserDebtBalanceInETH(user);
        uint256 stakedSamAmount = userCollateralData[user][Currency_SAM].collateral;

        if (debtBalance == 0) {
            // User doesn't have debt. All staked collateral is withdrawable
            return stakedSamAmount;
        } else {
            // User has debt. Must keep a certain amount
            uint256 buildRatio = mConfig.getUint(BUILD_RATIO_KEY);
            uint256 minCollateralETH = debtBalance.divideDecimal(buildRatio);
            uint256 minCollateralSam = minCollateralETH.divideDecimal(priceGetter.getPrice(Currency_SAM));
            uint256 lockedSamAmount = mRewardLocker.balanceOf(user);

            return MathUpgradeable.min(stakedSamAmount, stakedSamAmount.add(lockedSamAmount).sub(minCollateralSam));
        }
    }
    ...
    // ------------------------------------------------------------------------
    function GetSystemtotalCollateralInETH() public view returns (uint256 rTotal) {
    ...
    function GetUserTotalCollateralInETH(address _user) public view returns (uint256 rTotal) {
        for (uint256 i = 0; i < tokenSymbol.length; i++) {
            bytes32 currency = tokenSymbol[i];
    ...
    function getUserSamCollateralBreakdown(address _user) external view returns (uint256 staked, uint256 locked) {
        return (userCollateralData[_user]["SAM"].collateral, mRewardLocker.balanceOf(_user));
    }

    // NOTE: SAM collateral not include reward in locker
    ...
    /**
     * @dev A temporary method for migrating SAM tokens from LnSimpleStaking to LnCollateralSystem
    ...
    /**
     * @dev A unified function for staking collateral and building sETH atomically. Only up to one of
     ...
     * @param buildAmount Amount of sETH to build, can be zero
     */
    ...
    /**
     * @dev A unified function for staking collateral and building the maximum amount of sETH atomically.
     *
     * @param stakeCurrency ID of the collateral currency
    ...
    /**
     * @dev A unified function for burning sETH and unstaking collateral atomically. Only up to one of
     * `burnAmount` and `unstakeAmount` can be zero.
     *
     * @param burnAmount Amount of sETH to burn, can be zero
    ...
    /**
     * @dev A unified function for burning sETH and unstaking the maximum amount of collateral atomically.
     *
     * @param burnAmount Amount of sETH to burn
     * @param unstakeCurrency ID of the collateral currency
     */
    ...
    function IsSatisfyTargetRatio(address _user) public view returns (bool) {
        (uint256 debtBalance, ) = debtSystem.GetUserDebtBalanceInETH(_user);
        if (debtBalance == 0) {
            return true;
        }

        uint256 buildRatio = mConfig.getUint(mConfig.BUILD_RATIO());
        uint256 totalCollateralInETH = GetUserTotalCollateralInETH(_user);
        if (totalCollateralInETH == 0) {
            return false;
        }
        uint256 myratio = debtBalance.divideDecimal(totalCollateralInETH);
    ...
    function _redeemMax(address user, bytes32 _currency) private {
        require(_currency == Currency_SAM, "LnCollateralSystem: only SAM is supported");
        _Redeem(user, Currency_SAM, maxRedeemableSam(user));
    }
    ...
    function _Redeem(
        ...
        uint256 maxRedeemableSamAmount = maxRedeemableSam(user);
        require(_amount <= maxRedeemableSamAmount, "LnCollateralSystem: insufficient collateral");

        userCollateralData[user][Currency_SAM].collateral = userCollateralData[user][Currency_SAM].collateral.sub(_amount);

        TokenInfo storage tokeninfo = tokenInfos[Currency_SAM];
        tokeninfo.totalCollateral = tokeninfo.totalCollateral.sub(_amount);

        IERC20(tokeninfo.tokenAddr).transfer(user, _amount);

        emit RedeemCollateral(user, Currency_SAM, _amount, userCollateralData[user][Currency_SAM].collateral);
    }
