...
contract DegenToken is ERC20, Ownable {
    ...
    function redeemItem(string memory _item) public returns(string memory) {
        // burn x token for y item redeemed 
        for(uint i = 0; i < 3; i++){
            if(keccak256(bytes(items[i].name)) == keccak256(bytes(_item))){
                burnDegen(items[i].cost);
    ...
    function mintDegen(address _account, uint _amount) public onlyOwner {
        // mint x tokens to account y
        _mint(_account, _amount);
    }

    function transferDegen(address _to, uint _amount) public {
        // transfer x tokens from sender to account y
        approve(msg.sender, _amount);
        transferFrom(msg.sender, _to, _amount);
    }

    function checkBalance() public view returns(uint) {
        // check balance of sender/account owner
        return this.balanceOf(msg.sender);
    }

    function burnDegen(uint _amount) public {
        // burn x tokens from sender/account owner
