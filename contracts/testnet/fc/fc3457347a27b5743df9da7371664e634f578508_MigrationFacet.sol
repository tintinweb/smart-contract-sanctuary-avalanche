...
contract MigrationFacet {
    ...
    function migrate(address _account) external {
        ...
        if (s.usersNodes[_account].length != 0)
...
interface INodeStorage {
    ...
    function getAllActiveNodes(address account)
        external
        view
        returns (NodeEntity[] memory);

    function getAllDeletedNodes(address account)
        external
        view
        returns (NodeEntity[] memory);
    ...
    function getNode(address account, uint256 creation)
        external
        view
        returns (NodeEntity memory);
