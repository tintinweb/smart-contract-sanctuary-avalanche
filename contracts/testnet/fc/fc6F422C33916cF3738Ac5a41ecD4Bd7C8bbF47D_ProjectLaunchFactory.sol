...
library TransferHelper {
    function safeApprove(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), '42');
    }
    function safeTransfer(address token, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), '43');
    }
    function safeTransferFrom(address token, address from, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), '44');
    }
    // 19
    function safeTransferBaseToken(address token, address payable to, uint value, bool isERC20) internal {
        if (!isERC20) {
            ...
            (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
            require(success && (data.length == 0 || abi.decode(data, (bool))), '45');
        }
    }
}
interface IBEP20 {
...
contract Sale is ReentrancyGuard {
    ...
    struct SaleDetails {
        ...
        uint canceled;
        uint hasSeedRound;
        uint hasPresaleRound;
        uint hasCommunityRound;
        uint256 listRate;
        uint256 saleOption;
        uint256 hasVesting;
        uint256 launchFee;
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "46");
        _;
    }
    modifier IsSaleWhitelisted(uint _round) {
        require(sale_info[_round].sale_type == SaleType.SALEWHITELIST, "47");
        _;
    }
    constructor(
        address owner_,
    ...
    function init_sale (
        ...
        require(saleSetting == 0, "48");
        require(a != address(0), "49");
        sale_details.sale_token = address(a);
    ...
    function setSaleDetails(uint _sale_round, uint256[] memory _sale_input) private {
        ...
        saleInfo.attempt = 1;
        saleInfo.liquidityTokens = ((saleInfo.hardcap * saleInfo.mult) / 100) * ((sale_details.listRate * saleInfo.liquidity)/100);
    }
    ...
    function remainingBurn(uint _round) public onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "50"); // 27
                require(sale_info[_round].hardcap * sale_info[_round].token_rate >= sale_status[_round].sale_sold_amount, "51");
    ...
    function userDeposit (uint _round) public payable nonReentrant {
        require(iBlacklist(blacklistContract).getBlacklist(_address, _contract) == 0, '10');
        if(sale_info[_round].start > 0) {
            if((block.timestamp >= sale_info[_round].start && block.timestamp <= sale_info[_round].end)) {
                if(sale_info[_round].sale_type == SaleType.SALEWHITELIST) {
                    require(saleWhitelistInfo[msg.sender] == 1, "11");
                }
                require(saleStatusCheck(_round) == 1, "12");
                require(sale_info[_round].raise_min <= msg.value, "13");
                require(sale_info[_round].raise_max >= msg.value, "14");
                ...
                require(tokensSold > 0, "15");
                require(sale_status[_round].sale_raised_amount * sale_info[_round].token_rate <= IBEP20(sale_details.sale_token).balanceOf(address(this)), "16");
    ...
    function userWithdrawTokens (uint _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "19"); // 6
                BuyerInfo storage buyer = buyers[_round][msg.sender];
                SaleStatus storage saleStatus = sale_status[_round];
                uint256 remaintoken = saleStatus.sale_sold_amount - saleStatus.sale_token_withdraw;
                require(remaintoken >= buyer.sale, "20");
                if(sale_details.hasVesting == 0) {
                    require(block.timestamp >= sale_info[_round].end + sale_lock_delay, "21"); // 7
                    ...
                    require(block.timestamp >= sale_vesting[_round].TGE, "22");
    ...
    function ownerWithdrawTokens (uint _round) private onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 3, "23"); // 10
    ...
    function userWithdrawBaseTokens (uint _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 3 || sale_info[_round].attempt > 0, "24"); // 14
                // 15
                BuyerInfo storage buyer = buyers[_round][msg.sender];
                SaleStatus storage saleStatus = sale_status[_round];
                uint256 remainingBaseBalance = address(this).balance;
                require(remainingBaseBalance >= buyer.sale_base, "25");
    ...
    function purchaseICOCoin (uint _round) public nonReentrant onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "26"); // 18
                address payable receiver = payable(msg.sender);
    ...
    function changeDateAdmin(uint _round, uint256 _start, uint256 _end) public  {
        require(msg.sender == admin, '30');
        require(_start > block.timestamp, "31");
        require(_end > _start, "32");
    ...
    function changeVestingAdmin(uint _round, uint256[] memory _vesting) public  {
        require(msg.sender == admin, '33');
        require(sale_vesting[_round].TGE > sale_info[_round].end, "34");
        require(sale_vesting[_round].cliff > sale_info[_round].end, "35");
        require(sale_vesting[_round].cliff >= sale_vesting[_round].TGE, "36");
    ...
    function restartRound(uint _round, uint256 _start, uint256 _end) public payable onlyOwner  {
        require(block.timestamp > sale_info[_round].end, "37");
        require(saleStatusCheck(_round) == 3, "38");
        require(_start > block.timestamp, "39");
        require(_end > _start, "40");
        require(_round == 3 || (_end < sale_info[_round + (1)].start), "42");
