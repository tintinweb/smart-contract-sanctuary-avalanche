...
contract AaveVault is OwnableUpgradeable, PausableUpgradeable, BaseBuildingBlock {
    ...
    event BorrowEvent(address borrowAsset,address baseAsset, uint256 amount);
    ...
    * @param wrappedNativeAsset_ wrapped Native Asset
    ...
    function setCollateral(address collateralAsset) external returns (bool){
                aaveLendingPool.setUserUseReserveAsCollateral(collateralAsset, true);
                return true;
    }

    function deposit(bytes memory _data) external whenNotPaused {
     ...
     */
    function openPosition(bytes memory _data) external whenNotPaused{
     ...
     */
    function submitNativeToken(address /*baseAsset*/, uint256 amount, bool /*positionType*/) external payable {
     ...
     */
    function withdrawNativeToken(bytes memory _data) external payable onlyActionPoolDcRouter {
    ...
    function borrow(bytes memory _data) external {
        (address collateralAsset,address borrowAsset, uint256 amount) = abi.decode(_data,(address,address,uint256));
        require(IERC20(collateralAsset).balanceOf(address(this)) >= amount,'AAVE BB : Insufficient collateral asset amount in vault');
        aaveLendingPool.borrow(borrowAsset, amount, 1, 0, address(this));
        emit BorrowEvent(collateralAsset,borrowAsset,amount);
    }
    ...
    function getUserAccountData(address user) public view
      ...
      uint256 healthFactor
           ...
           (totalCollateralBase,
            ...
            ltv,
            healthFactor) = aaveLendingPool.getUserAccountData(user);
    }
