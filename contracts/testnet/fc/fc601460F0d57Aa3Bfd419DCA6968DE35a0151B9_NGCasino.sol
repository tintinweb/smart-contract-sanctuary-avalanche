...
import {UpgradeRequirements, UpgradeTechRequirementMetaData, MSSpinWheelRewards, LotteryStats} from "../NGTypes.sol";
...
contract NGCasino is WithStorage {
    ...
    function _getUpgradeTimeCasino(uint16 level, uint256 upgradeTimeInitialSeconds, uint256 upgradeTimePerLevelFactor ) private view returns (uint256){
    ...
    function upgradeCasino(uint256 tokenId) public notPaused resourceAddresses {
        ...
        hero.casinoUpgradeTime = block.timestamp + _getUpgradeTimeCasino(hero.casinoLevel + 1, 800, 1600);
     ...
     function lottery(uint256 tokenId) public notPaused resourceAddresses  {
        ...
        require(hero.casinoLevel > 0, "Casino is still locked");
        require(block.number != hero.lastBlock, "Can not do it in the same block");
        require(getTimeLeft() > 0, "cannot add more tickets during processing time");
     ...
     function lotteryStats() public view notPaused resourceAddresses returns(LotteryStats memory lotteryStats) {
        ...
        lotteryStats.timeLeft = getTimeLeft();
     }
     ...
     function getTimeLeft() public view notPaused resourceAddresses returns (uint256) {
        ...
        uint256 startTime = (gs().currentDayOfEvents * 86400);
     ...
     function processWinners() public notPaused resourceAddresses onlyAPI {
         uint256 daysSince1970 = block.timestamp / 86400;
