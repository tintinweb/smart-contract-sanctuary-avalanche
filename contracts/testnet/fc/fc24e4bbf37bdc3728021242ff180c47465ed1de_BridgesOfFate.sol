...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    uint256 private constant TURN_PERIOD = 300; //Now we are use 1 minute //86400; // 24 HOURS
    ...
    function dayDifferance(uint256 timeStampTo, uint256 timeStampFrom) public pure returns (uint256)
    {
    ...
    {
        uint256 stageRandomNumber = RandomNumber[stageNumber]; //Extra
        ...
        GameEndRules
    {
        ...
        if (_member.userWalletAddress != address(0)) {
            bool checkUserSide = checkSide(_member.stage, _member.jumpSide); //extra
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public GameInitialized GameEndRules
    {
        ...
        if (_member.stage != 0) {
            require((_member.jumpSide == true && RandomNumber[_member.stage] >= 50e9) || (_member.jumpSide == false && RandomNumber[_member.stage] < 50e9), "You are Failed");
        }
