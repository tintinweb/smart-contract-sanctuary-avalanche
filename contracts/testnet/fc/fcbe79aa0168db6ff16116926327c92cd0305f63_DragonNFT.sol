...
contract DragonNFT is ERC721 {
    ...
    uint256 public dragonsLength;
    uint256 public dragonMaxAttributeValue;
    ...
    mapping(uint256 => string) public dragonsNames;
    ...
    function setDragonMaxAttributeValue(uint256 _dragonMaxAttributeValue)
        external
        onlyOwner
    {
        dragonMaxAttributeValue = _dragonMaxAttributeValue;
    }
    ...
    function getDragonWithData(string[] calldata _keys, uint256 _tokenId)
        external
        view
        returns (Dragon memory dragon, bytes32[] memory arrData)
    {
        dragon = dragons[_tokenId];
    ...
    function rebirth(uint256 _tokenId) external {
        Dragon memory dragon = dragons[_tokenId];

        if (ownerOf[_tokenId] != msg.sender) revert Unauthorized();
        if (dragon.level < 100) revert OnlyLvL100();

        uint256 _dragonMaxAttributeValue = dragonMaxAttributeValue;

        unchecked {
            if (dragon.strength < _dragonMaxAttributeValue) {
                dragons[_tokenId].strength = uint8(dragon.strength + 1);
            }

            if (dragon.intelligence < _dragonMaxAttributeValue) {
                dragons[_tokenId].intelligence = uint8(dragon.intelligence + 1);
            }

            if (dragon.agility < _dragonMaxAttributeValue) {
                dragons[_tokenId].agility = uint8(dragon.agility + 1);
            }

            if (dragon.vitality < _dragonMaxAttributeValue) {
                dragons[_tokenId].vitality = uint8(dragon.vitality + 1);
            }

            if (dragon.fertility < _dragonMaxAttributeValue) {
                dragons[_tokenId].fertility = uint8(dragon.fertility + 1);
            }

            ++dragons[_tokenId].rebirths;
        }

        dragons[_tokenId].level = 1;
    ...
    function changeDragonName(uint256 tokenId, string calldata _newName)
        external
        onlyZone
        returns (uint256)
    {
        ...
        // Free previous name
        takenNames[keccak256(bytes(dragonsNames[tokenId]))] = false;

        // Reserve name
        takenNames[nameHash] = true;
        dragonsNames[tokenId] = _newName;
    ...
    function generate(
        ...
        uint256 randCap
    ) internal pure returns (Dragon memory) {
        unchecked {
            return
                Dragon({
                    strength: uint8(
    ...
    function _mintDragons(uint256 numberOfMints, uint256 preTotalDragons)
        internal
    {
        ...
        unchecked {
            _indexerLength = MAX_SUPPLY - preTotalDragons;
        }
        ...
        for (uint256 i; i < numberOfMints; ) {
            ...
            // Mint Dragon and generate its attributes
            _mint(msg.sender, tokenId);

            if (tokenId >= LEGENDARY_ID_START) {
                dragons[tokenId] = generate(seed, 5, 6);
            } else {
                dragons[tokenId] = generate(seed, 1, 10);
            }
            unchecked {
                ++i;
            }
    ...
    function _handleMint(uint256 numberOfMints) internal {
        ...
        unchecked {
            uint256 totalDragons = dragonsLength + numberOfMints;

            if (
                numberOfMints > MAX_PER_ADDRESS ||
                totalDragons > (MAX_SUPPLY - reserved)
            ) revert MintLimit();

            _mintDragons(numberOfMints, totalDragons - numberOfMints);
            dragonsLength = totalDragons;
        }
    }
    ...
    function getDragonName(uint256 tokenId)
        public
        view
        returns (string memory name)
    {
        name = dragonsNames[tokenId];

        if (bytes(name).length == 0) {
            name = string(bytes.concat("dragon #", bytes(_toString(tokenId))));
        }
    }

    function _getTraits(Dragon memory dragon)
        internal
        pure
        ...
        return
            string(
                bytes.concat(
                    ...
                    bytes(_toString(dragon.rebirths)),
                    "},",
                    '{"trait_type": "strength", "value": ',
                    bytes(_toString(dragon.strength)),
                    "},",
                    '{"trait_type": "agility", "value": ',
                    bytes(_toString(dragon.agility)),
                    "},",
                    '{"trait_type": "vitality", "value": ',
                    bytes(_toString(dragon.vitality)),
                    "},",
                    '{"trait_type": "intelligence", "value": ',
                    bytes(_toString(dragon.intelligence)),
                    "},",
                    '{"trait_type": "fertility", "value": ',
                    bytes(_toString(dragon.fertility)),
                    "}"
                )
            );
    }
        ...
        returns (string memory)
    {
        ...
        if (dragon.level == 0) revert InvalidTokenID();

        return
            string(
                bytes.concat(
                    '{"name":"',
                    bytes(getDragonName(tokenId)),
                    '", "description":"Dragon", "attributes":[',
                    '{"trait_type": "level", "value": ',
                    bytes(_toString(dragon.level)),
                    "},",
                    bytes(_getTraits(dragon)),
                    "],",
                    '"image":"',
                    bytes(imageURL),
    ...
    {
        //slither-disable-next-line incorrect-equality
        if (dragons[tokenId].level == 0) revert InvalidTokenID();
