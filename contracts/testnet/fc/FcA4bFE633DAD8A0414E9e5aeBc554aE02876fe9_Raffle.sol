...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    address payable private immutable i_safeAddress;
    // Lottery variables
    ...
    event RaffleEnter(address indexed player, uint256 timestamp, uint ticketsSold);
    event WinnerPicked(address indexed player, uint256 indexOfWinner, uint256 prize, uint256 ownerIncome);
    event CheckUpkeepCall(address indexed keeper, uint256 timePassed, bool isCheckTime);
    event CheckUpkeepNeeded(address indexed keeper, uint256 timestamp, RaffleState raffleState, bool upkeepNeeded);
    event RequestRandomWords(bytes32 gasLane, uint64 subId, uint16 minBlocks, uint32 callbackGasLimit, uint32 numWords);
    event ChangeState(RaffleState prevRaffleState, RaffleState newRaffleState, uint tag);
    ...
    function enterRaffle() public payable {
        ...
        for (uint ticketId = 0; ticketId < ticketsSold; ticketId++) {
            ...
            if (s_tickets.length + 1 > s_targetNumberOfTickets) {
                emit ChangeState(s_raffleState, RaffleState.DRAW_PENDING, 143);
        ...
        emit RaffleEnter(msg.sender, block.timestamp, ticketsSold);
    }
    ...
    {
        bool isCheckTime = getTimePassed() > s_raffleParams.interval;
        emit CheckUpkeepCall(msg.sender, getTimePassed(), isCheckTime);
        return (isCheckTime, upkeepData);
    }

    function checkUpkeepNeeded() public returns (bool upkeepNeeded)
    {
        ...
        if (upkeepNeeded) {
            emit ChangeState(s_raffleState, RaffleState.DRAW_PENDING, 185);
            s_raffleState = RaffleState.DRAW_PENDING;
        }
        emit CheckUpkeepNeeded(msg.sender, block.timestamp, s_raffleState, upkeepNeeded);
    }
    ...
    function performUpkeep(
        ...
        s_lastTimeStamp = block.timestamp;
        emit ChangeState(s_raffleState, RaffleState.DRAW, 207);
    ...
    function fulfillRandomWords(
        ...
        if (winnerTxSuccess) {
            ...
            (bool safeTxSuccess, ) = i_safeAddress.call{value: fee}("");
            if (safeTxSuccess) {
                if (s_raffleParams.autoStart) {
                    emit ChangeState(s_raffleState, RaffleState.OPEN, 252);
                    s_raffleState = RaffleState.OPEN;
//                    s_lastTimeStamp = block.timestamp;
                } else {
                    emit ChangeState(s_raffleState, RaffleState.MAINTENANCE, 256);
                    s_raffleState = RaffleState.MAINTENANCE;
                }
            } else {
                emit ChangeState(s_raffleState, RaffleState.MAINTENANCE, 260);
                s_raffleState = RaffleState.MAINTENANCE;
            }
        } else {
            emit ChangeState(s_raffleState, RaffleState.MAINTENANCE, 264);
            s_raffleState = RaffleState.MAINTENANCE;
        }
        emit WinnerPicked(recentWinner, indexOfWinner, prize, fee);
    }

    /** Getter Functions */
    function getRaffleSafeAddress() public view returns (address) {
        return i_safeAddress;
    }
    ...
    function setRaffleMaintenance() public onlyOwner {
        emit ChangeState(s_raffleState, RaffleState.MAINTENANCE, 349);
        s_raffleState = RaffleState.MAINTENANCE;
    }

    function setRaffleOpen() public onlyOwner atMaintenance {
        emit ChangeState(s_raffleState, RaffleState.OPEN, 354);
        s_raffleState = RaffleState.OPEN;
    ...
    function resetRaffle() public onlyOwner atMaintenance {
        ...
        // Send funds to safe
        (bool success, ) = i_safeAddress.call{value: address(this).balance}("");
        if (success) {
            s_tickets = new address payable[](0);
            s_raffle_id += 1;
        } else {
