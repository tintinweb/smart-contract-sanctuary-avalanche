...
contract BeefyAdapter is AdapterBase, Basic {
    using SafeERC20 for IERC20;

    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "Beefy")
    {}
    ...
    function deposit(bytes calldata encodedData) external onlyAdapterManager {
        (
            address tokenAddr,
            address mooTokenAddr,
            uint256 tokenAmount,
            address owner
        ) = abi.decode(encodedData, (address, address, uint256, address));
        pullAndApprove(tokenAddr, owner, mooTokenAddr, tokenAmount);
        ...
        mooToken.transfer(owner, amountDiff);
        emit BeefyDepositEvent(tokenAddr, mooTokenAddr, tokenAmount, owner);
    }
    ...
    function withdraw(bytes calldata encodedData) external onlyAdapterManager {
        ...
        pullAndApprove(mooTokenAddr, owner, mooTokenAddr, mooTokenAmount);
        ...
        require(amountDiff >= 0, "amount error");
        token.safeTransfer(owner, amountDiff);
        emit BeefyWithdrawEvent(tokenAddr, mooTokenAddr, mooTokenAmount, owner);
    }
}
