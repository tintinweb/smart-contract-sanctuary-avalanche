...
// SPDX-License-Identifier: MIT And Apache
    ...
    OwnableUpgradeable
{
    ...
    // State variable to store the URI of the contract metadata
    uint256 public maxSupply;
    address public deployer;
    string private _baseURIextended;
    string private _contractURI;

    ERC721BurnableUpgradeable public oldPumpskinContract;
    ...
    function initialize(
        uint256 _maxSupply,
        address oldPumpskinAddress,
        string memory baseURI_
    ) public initializer {
        __ERC721_init("PumpskinV2", "PUMPSKNV2");
        ...
        oldPumpskinContract = ERC721BurnableUpgradeable(oldPumpskinAddress);
        deployer = msg.sender;
    ...
    function mint(address to, uint256 tokenId) public onlyRole(MINTER_ROLE) {
        require(tokenId <= maxSupply, "more than max");
        _mint(to, tokenId);
    }
    ...
    function migrateV1Token(uint256 tokenId) public {
        require(tokenId <= maxSupply, "more than max");
        oldPumpskinContract.burn(tokenId);
        _mint(msg.sender, tokenId);
    }

    function migrateV1Tokens(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            migrateV1Token(tokenIds[i]);
        }
    }
}