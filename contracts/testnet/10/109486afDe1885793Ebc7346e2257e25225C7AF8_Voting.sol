...
contract Voting {
   ...
   modifier onlyPropertyOwnerOrAdmin() {
    ...
    function castVote(uint256 _voteId, uint256 _option) public onlyAkru {
        _voteInfo storage voteInformation = voteInfo[_voteId];
    ...
    function updateWhitelistContract(ITokenismWhitelist _newWhitelistAddress)
        public
        onlyAdmin
    {
        ...
        onlyPropertyOwnerOrAdmin
    {
        ...
        require(
            (voteInformation.numOfOptions > _newDefaultOption &&
                _newDefaultOption >= 0),
            "Seletcted default option does not exist"
        );
