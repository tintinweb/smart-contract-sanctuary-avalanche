...
contract NFTStaking is Ownable, IERC721Receiver, ReentrancyGuard {
   using SafeMath for uint256;
    ...
    function add(uint256 _totalReward, address _nftAddress, bool _withUpdate,
        ...
        poolCount ++;
        PoolInfo memory pool = poolInfo[poolCount];
    ...
    function transferRewards(address[] memory users, uint256[] memory amounts) external onlyOwner {
        require(users.length == amounts.length, "Incorrect Length");
        for (uint256 i; i < users.length; i++) {
            transferredRewards[users[i]] = amounts[i];
        }
    }
    ...
    function migratePool(uint256 _pid, uint256 _endBlock, uint256 _addTotalTokens) external validatePoolByPid(_pid) onlyOwner {
        ...
        uint256 newReward = _addTotalTokens / (_endBlock - block.number);
        poolCount ++;
        ...
        address user;
        uint256 tokenId;
        for (uint256 i; i < oldPool.users.length && gasUsed < 5000000; i++) {
            user = oldPool.users[i];
            // Transfer User

            newPool.users.push(user);
            userInfo[poolCount][user].inStaking = true;
            userInfo[poolCount][user].tokenIds = userInfo[_pid][user].tokenIds;

            // Transfer NFT
            for (uint256 x; x < userInfo[_pid][user].tokenIds.length; x ++) {
                tokenId = userInfo[_pid][user].tokenIds[x];
                nftInfo[poolCount][tokenId].amount = nftInfo[_pid][tokenId].amount;
                nftInfo[poolCount][tokenId].owner = nftInfo[_pid][tokenId].owner;
                nftInfo[poolCount][tokenId].rewardDebt = nftInfo[poolCount][tokenId].amount.mul(newPool.accTokenPerShare).div(ACC_TOKEN_PRECISION);
            }
    ...
    function contMigration(uint256 _pid) external validatePoolByPid(_pid) onlyOwner {
        ...
        require(poolInfo[_pid].emergCount < poolInfo[_pid].users.length, "Migration has finished");
        updatePool(_pid);
        uint256 gasLeft = gasleft();
        uint256 gasUsed;
        PoolInfo storage newPool = poolInfo[poolInfo[_pid].migratedTo];
        PoolInfo storage oldPool = poolInfo[_pid];
        address user;
        uint256 tokenId;
        for (uint256 i = oldPool.emergCount + 1; i < oldPool.users.length && gasUsed < 5000000; i++) {
            user = oldPool.users[i];
            // Transfer User

            newPool.users.push(user);
            userInfo[oldPool.migratedTo][user].inStaking = true;
            userInfo[oldPool.migratedTo][user].tokenIds = userInfo[_pid][user].tokenIds;

            // Transfer NFT
            for (uint256 x; x < userInfo[_pid][user].tokenIds.length; x ++) {
                tokenId = userInfo[_pid][user].tokenIds[x];
                nftInfo[oldPool.migratedTo][tokenId].amount = nftInfo[_pid][tokenId].amount;
                nftInfo[oldPool.migratedTo][tokenId].owner = nftInfo[_pid][tokenId].owner;
                nftInfo[oldPool.migratedTo][tokenId].rewardDebt = nftInfo[oldPool.migratedTo][tokenId].amount.mul(newPool.accTokenPerShare).div(ACC_TOKEN_PRECISION);
            }
            oldPool.emergCount = i;
            gasUsed += gasLeft - gasleft();
            gasLeft = gasleft();
