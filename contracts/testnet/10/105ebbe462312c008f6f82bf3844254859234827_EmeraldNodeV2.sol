...
contract EmeraldNodeV2 is EmeraldNodeStorageV2 {
    ...
    */
    ...
    function updateEmeraldCoinAddress2(address _address) external onlyOwner returns(bool){
        EmeraldCoin = IERC20(_address);
        return true;
    }
    ...
    */
    ...
    function checkTreasuryValueInUSD() public returns(uint256 TreasuryPriceInUSD){
        uint256 price = uint256(bondTreasuryAddress.getEmToUSDPrice());

        TreasuryPriceInUSD = (price * EmeraldCoin.balanceOf(bondTreasuryAddress.treasuryAccount())) / (10**18);
    }
    ...
    function checkEMPriceInUSD() public returns(int256){
        return bondTreasuryAddress.getEmToUSDPrice();
    }
    ...
    function sellNode(uint256 _nodeId) external returns(bool){
       ...
       uint256 bpn = checkTreasuryValueInUSD() / nodes;

       uint256 _avax_price = feeCalculation(AvaxPercentage, bpn);
       uint256 _em_price = bpn - _avax_price;

       uint256 price = _avax_price*(10**8) / uint256(bondTreasuryAddress.getAvaxToUsdPrice());

       AvaxCoin.transferFrom(owner(), address(this), price);
       payable(msg.sender).transfer(price);

       uint256 price1 = _em_price * (10**8) / uint256(checkEMPriceInUSD());
       EmeraldCoin.safeTransfer(owner(), address(this), price1);
       EmeraldCoin.approve(msg.sender, price1);
       EmeraldCoin.safeTransfer(address(this), msg.sender, price);
