...
contract SampleMintingChain is OnMintingChain {
    ...
    ) OnMintingChain(name_, symbol_, MaxTokenId_, gateway_, gasReceiver_) {
        owner = msg.sender;
    }
...
abstract contract OnMintingChain is ERC721, IERC721Receiver, AxelarExecutable {
    ...
    constructor(
        ...
        address gateway_,
        address gasReceiver_
    ) ERC721(name_, symbol_) AxelarExecutable(gateway_) {
    ...
    function _execute(
        ...
        (address recipient, address refundAddress) = abi.decode(payload, (address, address));
        sourceChain = sourceChain_;
        receiveCrossChain(recipient, refundAddress);
    }

    function receiveCrossChain(address recipient, address refundAddress) internal returns (bool) {
    ...
    function mint(address recipient) internal {
        ...
        IERC20(feeTokenForNFT).safeTransferFrom(msg.sender, address(this), feeInTokenForNFT);
