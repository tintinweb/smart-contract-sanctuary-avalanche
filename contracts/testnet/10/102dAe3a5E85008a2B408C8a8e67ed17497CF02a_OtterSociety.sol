...
pragma solidity ^0.8.0;
/* startTokenId => 0  REGULAR ERC721A CAN BE USED */
 ...
 */
/// @title Otter Society ERC721A contract.
/// @author Crypto Spectres - Developer Team
/* JSON ipfs hash: QmXUWMjeKwST84uHJPLAQxahZtkcJuKnz6qFvELaCMSQQM */
contract OtterSociety is ERC721A, OwnableNR, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public constant MAX_WHITELIST_WALLETS = 200;
    ...
    address payable warthuWallet;
    address payable reedWallet;
    address payable rushCWallet;
    address payable skipperWallet;
    ...
    //TODO: When everything is minted, automatically reveal
    ...
    {
        require(whiteListSale, "Otter Society :: Minting is on Pause");
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY,
    ...
    function teamMint() external onlyOwnerAdmin {
    ...
    function addUser(address _addressToWhitelist) external onlyOwnerAdmin {
        /* Require whitelist address to have at least 1 AVAX of balance. */
        require(_addressToWhitelist.balance >= 1 ether);
    ...
    function withdraw() external payable onlyOwnerAdmin {
        ...
        /* 20% goes to robbo */
        _withdraw(robboWallet, (fullBalance * 20) / 100);

        /* 20% goes to Warthu */
        _withdraw(warthuWallet, (fullBalance * 20) / 100);
        /* 10% goes to Skipper */
        _withdraw(skipperWallet, (fullBalance * 10) / 100);
        /* 5% goes to Reed */
        _withdraw(reedWallet, (fullBalance * 5) / 100);
        /* 5% goes to RushCutter */
        _withdraw(rushCWallet, (fullBalance * 5) / 100);

        /* 40% goes to Marketing & Development */
        _withdraw(marketDevWallet, (fullBalance * 40) / 100);
    }
