...
/**
  * @title Avastorm's InterestRateModel Interface
  * @author Avastorm
  */
...
/**
 * @title Avastorm's AToken Contract
...
/**
 * @title Avastorm's AErc20 Contract
...
contract AErc20 is AToken, AErc20Interface {
    ...
    /**
    * @notice Admin call to delegate the votes of the ALT-like underlying
    * @param altLikeDelegatee The address to delegate votes to
    * @dev ATokens whose underlying are not AltLike should revert here
    */
    function _delegateAltLikeTo(address altLikeDelegatee) external {
        require(msg.sender == admin, "only the admin may set the alt-like delegate");
        AltLike(underlying).delegate(altLikeDelegatee);
    }
}
...
// File: contracts/AvastormAggregatorPriceOracle.sol
...
contract AvastormAggregatorPriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(AToken aToken) public view returns (uint) {
        if (compareStrings(aToken.symbol(), "aAVAX")) {
            if (address(getFeed(aToken.symbol())) != address(0)) {
                return getChainlinkPrice(getFeed(aToken.symbol()));
            } else {
                IStdReference.ReferenceData memory data = ref.getReferenceData("AVAX", "USD");
                return data.rate;
            }
        }else if (compareStrings(aToken.symbol(), "ALT")) {
