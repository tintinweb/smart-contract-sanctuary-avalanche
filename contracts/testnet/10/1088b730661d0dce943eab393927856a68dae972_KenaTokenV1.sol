 ...
 * Peer-To-Peer Community Fee     1%
 * On-Ramp Community Fee (Buy)    5%
 * Off-Ramp Community Fee (Sell)  7%
    ...
    OwnableUpgradeable
{
    ...
    uint256 internal constant PEER_TO_PEER_COMUNITY_FEE = 10;
    // 5% RFI as on-ramp Fee (Buy)
    uint256 internal constant ON_RAMP_COMMUNITY_FEE = 50;
    // 7% RFI as off-Ramp Fee (Sell)
    uint256 internal constant OFF_RAMP_COMMUNITY_FEE = 70;
    ...
    event LiquidityPoolEvent(address account, string message, bool isRegister);
    ...
    */ 
     ...
     */
    function registerLiquidityPool(address _lp, bool _isRegister) public onlyManager {
        if(_isRegister) {
            require(!isLiquidityPool[_lp], "Liquidity pool address already registered.");
            _exclude(_lp);
        } else {
            require(isLiquidityPool[_lp], "Liquidity pool address already removed.");
            _include(_lp);
        }
        isLiquidityPool[_lp] = _isRegister;
        // emit events as well
        emit LiquidityPoolEvent(_lp, "LP Registered", _isRegister);
    }
    ...
    function _addFees() internal {
        // Peer-To-Peer Comunity Fee as RFI (Only on transfers. Not on Buy or Sell)
        _addFee(
            FeeType.PeerToPeerNetworkFee,
            PEER_TO_PEER_COMUNITY_FEE,
            address(this)
        );
        // On-Ramp Community Fee as RFI. During Buy.
        _addFee(FeeType.OnRampNetworkFee, ON_RAMP_COMMUNITY_FEE, address(this));
        // Off-Ramp Comunity Fee as RFI. During Sell.
        _addFee(FeeType.OffRampNetworkFee, OFF_RAMP_COMMUNITY_FEE, address(this));
        // Whale Penalty distrubuted to Community (This triggers only for higher sale numbers)
        _addFee(
...
abstract contract ERC20Vestable is EconomicProtection, KenaTokenomics {
        ...
        returns (uint256 amountAvailable)
    {
        ...
        uint256 notVested = _getNotVestedAmount(grantHolder, onDay);
        return totalTokens.sub(notVested);
    }
...
abstract contract KenaTokenGrant is ERC20Vestable {
    ...
    function grantUniformVestingTokens(
        ...
        returns (bool ok)
    {
        require(totalAmount <= balanceOf(_msgSender()), "Insufficient funds.");
...
contract KenaTokenV1 is KenaTokenGrant {
    ...
    function _getSumOfFees(
        ...
        if (_isWhaleSale) {
            return (OFF_RAMP_COMMUNITY_FEE +
                BURN_FEE +
                ANTI_WHALE_SALE_PENALTY_FEE);
        } else if (_isSale) {
            return (OFF_RAMP_COMMUNITY_FEE + BURN_FEE);
        } else if (_isBuy) {
            return ON_RAMP_COMMUNITY_FEE;
        } else if (_isPeerToPeer) {
            return PEER_TO_PEER_COMUNITY_FEE;
        } else {
            return PEER_TO_PEER_COMUNITY_FEE; // Default
        }
    }
