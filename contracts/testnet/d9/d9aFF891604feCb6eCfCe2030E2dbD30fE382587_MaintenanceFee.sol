        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (
                ...
                maintFee[_tokenIds[i]].firstDay = uint64(block.timestamp);
						} else if (maintFee[_tokenIds[i]].unClaimedDays <=
                    Rewards.RewardsClaimed(_tokenIds[i]).claimedDays) {
								revert("Inconsistence Rewards Claimed");
            } else {
								maintFee[_tokenIds[i]].firstDay = uint64(block.timestamp - ((maintFee[_tokenIds[i]].unClaimedDays -
                    Rewards.RewardsClaimed(_tokenIds[i]).claimedDays).mul(3 minutes)));
						}
            maintFee[_tokenIds[i]].unClaimedDays += _paidDays[i];
            if (_type == 12) {
                if (
