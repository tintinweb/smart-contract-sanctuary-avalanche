...
contract FactoryStorage {
    ...
    address public implAPIConsumer;
    address public usdc;
    address public usdt;
    address public dai;
    ...
    mapping(address => address) public fundContract;
    ...
    FactoryStorage
{
    ...
    function setImplementation(
        ...
        implFund = _implAuthorithyFund;
    }

    function setStableCoins(address _usdc, address _usdt, address _dai) public onlyRole(Operator_Role){
        usdc = _usdc;
        usdt = _usdt;
        dai = _dai;
    }

    function getStableCoin(uint8 _key) public view returns(address){
        if(_key == 0){
            return dai;
        }
        else if(_key == 1){
    ...
    function createToken(
        ...
        uint16[] memory _countries,
        address _issuer,
        address _transferAgent,
        address _tokenizationAgent
    ...
    function _initCompliance(
        ...
        address _issuer,
        address _transferAgent,
        address _tokenizeAgent
    ) internal {
        (address compliance, address proxy) = _initProxy(
            identityRegistry,
            _name,
            _symbol,
            _decimals,
        ...
        (bool success, ) = compliance.call(
            abi.encodeWithSelector(0xc5ce23c2, address(this))
        );
        require(success, "Compliance Agent Factory Failed");
        success = false;
        (success, ) = compliance.call(
            abi.encodeWithSelector(0xb6184ccd, _countries)
        );
        ...
        success = false;
        (success, ) = proxy.call(
            abi.encodeWithSelector(0xb0314599, _transferAgent)
        );
        require(success, "Adding Transfer Agent Failed");
        success = false;
        (success, ) = proxy.call(
            abi.encodeWithSelector(0xadccbc3c, _tokenizeAgent)
        );
    ...
    function _initProxy(
        ...
        uint256 _holdReleaseTimestamp,
        uint256 _tokenLimitPerUser
    ) internal returns (address compliance, address proxy) {
    ...
    function _initIdentityRegistry(address identityRegistry) internal {
        ...
        address trustedIssuersRegistry = _clone(
        ...
        success = false;
        (success, ) = claimTopicsRegistry.call(
            abi.encodeWithSelector(0xe1c7392a)
        );
        require(success, "claimTopicsRegistry init failure");
        success = false;
        (success, ) = trustedIssuersRegistry.call(
            abi.encodeWithSelector(0xe1c7392a)
        );
    ...
    function _deployIdentity() internal returns (address) {
        ...
        (success, ) = identityProxy.call(
            abi.encodeWithSelector(0x19ab453c, address(this))
        );
        require(success, "Identity Intiatialization Failed");
        success = false;
    ...
    function deployAdminIdentity(
        ...
        (bool success, ) = adminIdentityProxy.call(
            abi.encodeWithSelector(0x3659cfe6, implAuthorityAdminIdentity)
        );
        require(success, " Upgrade failed");
        success = false;

        (success, ) = adminIdentityProxy.call(
            abi.encodeWithSelector(
                0x39bd0430,
                address(this),
                executionBuffer,
        ...
        (success, ) = adminIdentityProxy.call(
            abi.encodeWithSelector(
                0x1d381240,
                keccak256(abi.encode(tokenizationAgent)),
                2,
                1
            )
        );
        ...
        emit multiSig(adminIdentityProxy, mintApvl, burnApvl, tokenizationAgent, _issuer, _token);
        return adminIdentityProxy;
    }

    // --------------------------Deploying Fund for Token-------------------------------------
    ...
    function deployFund(
        ...
        uint256 _targetAUM,
        uint128 _NAVLaunchPrice,
        string memory _mappingValue,
        string memory _NAVEndPoint
    ) public onlyRole(Operator_Role) returns (address) {
        string memory fundName = _fundName;
        address token = _token;
        address fundManagerAddress = fmAddress;
        uint8 assetType = _assetType;
        string memory issuerName = _issuerName;
        uint256 targetAUM = _targetAUM;
        uint128 NAVLaunchPrice = _NAVLaunchPrice;
        string memory mappingValue = _mappingValue;
        string memory NAVEndPoint = _NAVEndPoint;

        address FundProxy = address(new ProxyV1());
        (bool success, ) = FundProxy.call(
            abi.encodeWithSelector(0x3659cfe6, implFund)
        );
        require(success, "Fund Upgrade failed");
        success = false;

        (success, ) = FundProxy.call(
            abi.encodeWithSelector(
                0x203cbe6a,
                token,
                fundName,
        ...
        require(success, "Fund Initialization Failed");
        success = false;

        (success, ) = FundProxy.call(
            abi.encodeWithSelector(0x84e79842, fundManagerAddress)
        );
        require(success, "Fund Manager as Agent Failed");
        success = false;

        // address newConsumer = address(new APIConsumer(_fundName[1]));

        // (success,) = FundProxy.call(abi.encodeWithSelector(0x63a579d5, newConsumer));

        fundContract[token] = FundProxy;

        IToken token1 = IToken(token);
        token1.setFund(FundProxy);

        bytes32[] memory role = new bytes32[](2);
        role[0] = Minter_Role;
        role[1] = Burner_Role;

        setRolesOnToken(FundProxy, role, token);
    ...
    function whitelistUser(
        ...
        require(
            iR.isAgentOnIdentityRegistry(msg.sender),
            "Not an Agent on Token Identity Registry"
        );
        IIdentityRegistry(iR).registerIdentity(
            _userAddress,
            IIdentity(_userIdentity),
            _countryCode
        );
    ...
    function deployAndWhitelist(
        ...
        if (userIdentityId[_userAddress] == 0) {
            ...
            address userIdentity = userIdentities[
                userIdentityId[_userAddress] - 1
            ];
            whitelistUser(_token, _userAddress, _CountryCode, userIdentity);
        }
    }
    ...
    function batchDeployAndWhitelist(
        ...
        require(
            _userAddress.length == _countrycodes.length,
    ...
    function revokeSubAdminOnToken(
        address _userAddress,
        address _token
    ) external onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function addTokenizationAgentOnToken(
        address _agent,
        address _token
    ) public onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function revokeTokenizationAgentOnToken(
        address _agent,
        address _token
    ) external onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function addTransferAgentOnToken(
        address _agent,
        address _token
    ) public onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function removeTransferAgentOnToken(
        address _agent,
        address _token
    ) external onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function updateReleaseTimestampForToken(
        address _token,
        uint256 _timestamp
    ) external onlyRole(Operator_Role) {
        IToken token = IToken(_token);
    ...
    function updateHolderLimitForToken(
        address _token,
        uint256 _newHolderLimit
    ) external onlyRole(Operator_Role) {
        IToken token = IToken(_token);
        ICompliance compliance = token.compliance();
    ...
    function addSubAdminOnIdentityRegistry(
        address _userAddress,
        address _identityRegistry
    ) external onlyRole(Operator_Role) {
    ...
    function revokeSubAdminOnIdentityRegistry(
        address _SubAdminAddress,
        address _identityRegistry
    ) external onlyRole(Operator_Role) {
        IIdentityRegistry iR = IIdentityRegistry(_identityRegistry);
    ...
    function addSubAdminOnCompliance(
        address _userAddress,
        address _compliance
    ) external onlyRole(Operator_Role) {
    ...
    function revokeSubAdminOnCompliance(
        address _SubOwner,
        address _compliance
    ) external onlyRole(Operator_Role) {
        ICompliance compliance = ICompliance(_compliance);
    ...
    function addAdminIdentitySigner(
        address _signer,
        address _token
    ) public onlyRole(Operator_Role) {
        address multiSig = getAdminIdentityMinter(_token);
        IAdminIdentityInit(multiSig).addKey(
            keccak256(abi.encode(_signer)),
            2,
            1
        );
    ...
    function removeMultiSigSigner(
        ...
        address multiSig = getAdminIdentityMinter(_token);
        IAdminIdentityInit(multiSig).removeKey(
            keccak256(abi.encode(_signer)),
            2
        );
    }

    function removeMultiSigBurnSigner(
        address _signer,
        address _token
    ) public onlyRole(Operator_Role) {
        address multiSig = getAdminIdentityMinter(_token);
    ...
    function setMintRequirementOnMultiSig(
        uint256 _sigReq,
        address _token
    ) public onlyRole(Operator_Role) {
        address multiSig = getAdminIdentityMinter(_token);
    ...
    function setBurnRequirementOnMultiSig(
        uint256 _sigReq,
        address _token
    ) public onlyRole(Operator_Role) {
        address multiSig = getAdminIdentityMinter(_token);
    ...
    ) public onlyRole(Operator_Role) returns (bool success, bytes memory data) {
        (success, data) = _to.call{value: _value}(_data);
        emit adminCallResponse(msg.sender, _to, success, data);
    }

    function adminCallSequence(
        address[] calldata _to,
        uint256[] calldata _value,
        bytes[] calldata _data
    ) public returns (bytes[] memory) {
        require(
            _to.length == _value.length && _value.length == _data.length,
            "LibAdminCall: check arrays"
        );
        bool success;
        bytes memory data;
