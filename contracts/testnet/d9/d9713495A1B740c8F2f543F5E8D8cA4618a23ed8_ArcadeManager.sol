...
contract ArcadeManager {
    ...
    function swapBONKToBAM(address _user, uint _amount) external {
        ...
        if (_user != address(0)) {
            userBONKToBAMReserve[_user] = totalAmount;
        }
    }

    function clearAllUserReserve(address _user) external onlyVIP {
        userBONKToBAMReserve[_user] = 0;
    }
