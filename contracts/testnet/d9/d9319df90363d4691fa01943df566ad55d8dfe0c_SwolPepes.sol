    ...
    ClampedRandomizer
{
    ...
    address  public royaltyAddress = 0x10e642E61fC8646327DF18E922FACFcD2168A8bB;
    ...
    uint256 public maxPerPerson = 4;
    uint256 public price = 100000000000000000; //fix price on go-live
    uint256 public whiteListAPrice = 120000000000000000; //fix price on go-live
    uint256 public whiteListBPrice = 150000000000000000; //fix price on go-live
    ...
    bool public whitelistedAOnly = true;
    bool public whitelistedBOnly = false;
    bool public publicSale = false;

    uint256 public listAAllocation = 150;
    uint256 public listBAllocation = 250;


    mapping(address => WhiteListedA) public whiteListedA;
    struct WhiteListedA {
        uint256 count; 
        bool isWhiteListedA;
    }    

    mapping(address => WhiteListedB) public whiteListedB;
    struct WhiteListedB {
        uint256 count; 
        bool isWhiteListedB;
    }    
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("SwolPepes", "SWPP") ClampedRandomizer(maxSupply) {     
...
function mint(uint256 amount) public payable whenNotPaused {
    ...
    if (whitelistedAOnly == true && whiteListedA[msg.sender].isWhiteListedA) {
        require(amount > 0 && amount <= maxPerTx, "Error: max per tx limit");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: max per address limit");
        require(msg.value == whiteListAPrice * amount, "Error: invalid price");
        require(supply + amount - 1 < listAAllocation, "Error: Over allocation. Wait for public sale.");
        require(whiteListedA[msg.sender].count >= amount, "Error: you are not whitelisted or amount is higher than limit");
        mintPrice = whiteListAPrice;
    } else if (whitelistedBOnly == true && whiteListedB[msg.sender].isWhiteListedB) {
        require(amount > 0 && amount <= maxPerTx, "Error: max per tx limit");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: max per address limit");
        require(msg.value == whiteListBPrice * amount, "Error: invalid price");
        require(supply + amount - 1 < listBAllocation, "Error: Over allocation. Wait for public sale.");
        require(whiteListedB[msg.sender].count >= amount, "Error: you are not whitelisted or amount is higher than limit");
        mintPrice = whiteListBPrice;
    } 
    ...
    for (uint256 i = 0; i < amount; i++) {
        // Perform specific mint based on whitelist
        if (mintPrice == whiteListAPrice) {
            internalMint(msg.sender);
        } else if (mintPrice == whiteListBPrice) {
            internalMint(msg.sender);
        } else {
            internalMint(msg.sender);
        }
    ...
    if (mintPrice == whiteListAPrice) {
    whiteListedA[msg.sender].count -= amount;
    }   
    else if (mintPrice == whiteListBPrice) {
    whiteListedB[msg.sender].count -= amount;
    }
...
function whiteListA(address[] memory _addressList, uint256 count) external onlyOwner {
    require(_addressList.length > 0, "Error: list is empty");

    for (uint256 i = 0; i < _addressList.length; i++) {
        require(_addressList[i] != address(0), "Address cannot be 0.");
        whiteListedA[_addressList[i]].count = count;

        WhiteListedA storage entry = whiteListedA[_addressList[i]];
        entry.count = count;
        entry.isWhiteListedA = true;
    }
}

function whiteListB(address[] memory _addressList, uint256 count) external onlyOwner {
    require(_addressList.length > 0, "Error: list is empty");

    for (uint256 i = 0; i < _addressList.length; i++) {
        require(_addressList[i] != address(0), "Address cannot be 0.");
        whiteListedB[_addressList[i]].count = count;

        WhiteListedB storage entry = whiteListedB[_addressList[i]];
        entry.count = count;
        entry.isWhiteListedB = true;
    }
}


function removeWhiteListA(address[] memory _addressList) external onlyOwner {
    require(_addressList.length > 0, "Error: list is empty");
    for (uint256 i = 0; i < _addressList.length; i++) {
        whiteListedA[_addressList[i]].count = 0;
    }
}

function removeWhiteListB(address[] memory _addressList) external onlyOwner {
    require(_addressList.length > 0, "Error: list is empty");
    for (uint256 i = 0; i < _addressList.length; i++) {
        whiteListedB[_addressList[i]].count = 0;
    }
}

    function whitelistAOn() external onlyOwner {
        whitelistedAOnly = true;
    }

    function updateToWhitelistB() external onlyOwner {
    whitelistedBOnly = true;
    whitelistedAOnly = false;
    }


    function disableAllWhitelists() external onlyOwner {
        whitelistedAOnly = false;
        whitelistedBOnly = false;
    }

    function updateToPublicSale() external onlyOwner {
    whitelistedBOnly = false;
    whitelistedAOnly = false;
    publicSale = true;
    }
