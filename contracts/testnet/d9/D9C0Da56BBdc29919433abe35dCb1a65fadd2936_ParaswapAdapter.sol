...
contract ParaswapAdapter is AdapterBase, Basic {
    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "Paraswap")
    {}

    address public constant AugustusSwapper =
        0xDEF171Fe48CF0115B1d80b88dc8eAB59176FEe57;

    function swap(bytes calldata encodedData) external onlyProxy {
        (bytes memory callArgs, uint256 amountETH) = abi.decode(
            encodedData,
            (bytes, uint256)
        );
