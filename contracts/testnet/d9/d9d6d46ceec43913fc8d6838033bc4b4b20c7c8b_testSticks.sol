...
contract testSticks is ReentrancyGuard, Ownable{
    ...
    address public scepterTreasuryAddr = 0x4a55c1181B4aeC55cF8e71377e8518E742F9Ae72;
    ...
    **/
    ...
    function cashOutScepter(uint256 amountSCPtoSell, uint256 timeChosenLocked, string memory _stableChosen) public payable{
       ...
       /* require(
        ...
        uint256 usdAmt; 
        usdAmt = decMul18(this.getSellPrice() , sptrAmt) / (10**(18-stableERC20Info[_stableChosen].tokenDecimals));
        tokenStable.transfer(msg.sender, usdAmt);
        ...
        tokensSoldXDays[dInArray] += sptrAmt;
        circulatingSupplyXDays[dInArray] -= sptrAmt;
        //TODO: 
    ...
    function buyScepter(uint256 amountSCPtoBuy, string memory _stableChosen) public nonReentrant{
       ...
       // require(tokenStable.balanceOf(msg.sender) > amountSCPtoBuy, "You dont have that amount!");
        ...
        uint256 usdAmtToPay;
        usdAmtToPay = decMul18(sptrAmt, this.getBuyPrice()) / (10**(18-stableERC20Info[_stableChosen].tokenDecimals));

        //Transfer USDC to WI from trader
        _safeTransferFrom(tokenStable, msg.sender, address(this), usdAmtToPay);
      
        SPTR.mint(msg.sender, sptrAmt);
        WAND.mint(address(this), sptrAmt); //TODO: Activate and test

        //Keeping track of tokens bought per day
        uint256 dInArray =(block.timestamp- timeLaunched)/86400; 
        tokensBoughtXDays[dInArray] += sptrAmt;
        circulatingSupplyXDays[dInArray] += sptrAmt;
        ...
        emit sceptersBought(msg.sender, sptrAmt);
    }
