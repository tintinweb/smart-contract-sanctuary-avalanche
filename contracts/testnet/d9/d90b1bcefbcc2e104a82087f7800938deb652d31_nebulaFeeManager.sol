...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,uint256[] memory times,bool _feeFroze){
		...
		uint256[5] memory ls = [feePeriod,gracePeriod,protoLife,maxFeePayment,maxPayPeriods];
		for(uint i=0;i<5;i++){
		...
		INTaddProto(owner(),"asdfasdfdsdsaf");
		INTcollapse(owner(),0);
	...
	_protoowners.push(PROTOOWNERS({name:_name,protoCreationDate:INTgetCurrentTime(_account),protoElapsed:Zero,protoCollapseDate:INTgetCurrentTime(_account).add(protoLife),protoNextDue:feePeriod.add(block.timestamp),protoDueElapsed:feePeriod,protoFutureFees:Zero,protoIncreasedDecay:Zero,protoFeeFroze:_protoFeeFroze,protoTotalPayable:nebuLib.doMultiple(feePeriod,maxFeePayment.sub(feePeriod))}));
