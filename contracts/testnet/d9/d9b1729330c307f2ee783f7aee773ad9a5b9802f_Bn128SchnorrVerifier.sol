...
contract Bn128 {
    ...
    /// @dev Order is the number of elements in both G? and G?: 36u?+36u3+18u2+6u+1.
...
contract Bn128SchnorrVerifier is Bn128 {
    ...
    function h(bytes32 m, uint256 a, uint256 b) public pure returns (uint256) {        
        return uint256(sha256(abi.encode(m, a, b)));     
    }
    ...
    function cadd(uint256 ax, uint256 ay, uint256 bx, uint256 by) public view returns (uint256, uint256) {
		...
		function myFunc(bytes32 b) public view returns (uint256) {
            return uint256(b);
        }

        function MyFun2(bytes32 m, uint256 a, uint256 b) public pure returns (bytes) {
        return abi.encode(m, a, b);
    }

     function MyHash(bytes m) public pure returns (bytes32) {
        return sha256(m);
    }				
