...
contract DefaultHoldingPeriod is Compliance, Ownable {
    ...
    uint8 public HOLD_FAIL_CODE = 4;

    /// @dev specify revert messages for custom compliance modules
    string public HOLD_FAIL_ERROR = "ILLEGAL_TRANSFER_DURING_HOLDING_PERIOD";
    ...
    /// @dev Mapping of hold release timestamp of tokens
    mapping(address => uint) private _holdRelease;
     ...
     */
    function setTokenHold(address _token, uint256 _holdEnd) external onlyOwner {
        require(_tokensBound[_token], "UNBOUND TOKEN");
        _holdRelease[_token] = _holdEnd;
    }
    ...
    function detectTransferRestriction(address _from, address _to, uint256 _value) public override view returns (uint8 restrictionCode) {
        ...
        if (block.timestamp < _holdRelease[msg.sender] && !IToken(msg.sender).isAgent(_from)) {
        ...
        if (!IToken(msg.sender).isVerified(_to)) {
