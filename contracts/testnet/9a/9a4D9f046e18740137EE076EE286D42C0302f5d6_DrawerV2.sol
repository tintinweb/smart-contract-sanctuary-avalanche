...
contract DrawerV2 is Ownable {
  ...
  function getRoundSVG(
    ...
    svg = string(
      abi.encodePacked(
        '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="woh-round" width="100%" height="100%" version="1.1" viewBox="0 0 1440 960"><image x="0" y="0" width="100%" xlink:href="',
        BG1,
    ...
    uint16 y = 720;
    for (; i < 8; i++) {
      svg = string(abi.encodePacked(getTeamHead(x, y), teamImages[i], getLine(i, x, y)));
      x += 180;
    }
    ...
    for (; i < 12; i++) {
      base = i - 8;
      svg = string(abi.encodePacked(getTeamHead(x, y), teamImages[i], getLineAndScore(base, x, y, scores[base], size)));
      x += 360;
    }
  ...
  function drawRound(
    ...
    string memory attributes = string(
      abi.encodePacked(
        '[{"display_type":"number","trait_type":"Round","value":"',
        toString(roundId),
        ...
        abi.encodePacked(
          ...
          Base64.encode(
            abi.encodePacked(
              ...
              getRoundSVG(size, winner, teamImages, scores),
              '","attributes":',
              attributes,
              "}"
            )
