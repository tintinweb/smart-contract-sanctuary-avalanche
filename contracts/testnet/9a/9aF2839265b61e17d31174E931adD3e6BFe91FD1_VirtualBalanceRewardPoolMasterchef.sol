...
contract VirtualBalanceRewardPoolMasterchef is
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        assert(address(masterchef) == address(0x0));
    ...
    function totalSupply() public view returns (uint256) {
        return masterchef.totalDeposits(poolId);
    }

    function balanceOf(address account) public view returns (uint256) {
        return masterchef.balanceOf(poolId, account);
    }
    ...
    function snapshot(address _account)
        external
        updateReward(_account)
        returns (bool)
    {
        require(msg.sender == address(masterchef), "!authorized");
        return true;
    }
