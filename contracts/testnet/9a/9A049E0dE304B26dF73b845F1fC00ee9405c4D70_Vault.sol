...
contract Vault is ERC20, AccessControl, ReentrancyGuard, Pausable {
  ...
  function deposit(uint256 _depositAmt, uint256 _minSharesReceive) external payable nonReentrant {
    // IManager(manager).compound();

    ERC20(tokenA).safeTransferFrom(msg.sender, address(this), _depositAmt);
    ERC20(tokenA).safeTransfer(manager, _depositAmt);

    uint256 _lpAmt = IManager(manager).lpInfo();
    ...
    _checkAndMint(_depositAmt, _minSharesReceive, _lpAmt);
  }
  ...
  function _checkAndMint(uint256 _depositAmt,
    ...
    uint256 _shares = IManager(manager).lpInfo() - _lpAmt;

    // calculate shares to users
    uint256 _sharesToUser = _valueToShares(_shares, _lpAmt);
  ...
  function _valueToShares(uint256 _shares, uint256 _lpAmt) internal view returns (uint256) {
    uint256 _sharesSupply = totalSupply() /*+ pendingManagementFee()*/;
    if (_sharesSupply == 0) return _shares;
    return _shares * _sharesSupply / _lpAmt;
  }
...
interface IManager {
  function lpInfo() external view returns (uint256);
