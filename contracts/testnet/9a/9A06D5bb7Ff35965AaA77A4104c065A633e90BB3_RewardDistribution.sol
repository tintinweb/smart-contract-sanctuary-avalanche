    ...
    KeeperCompatibleInterface
{
    ...
    uint256 private constant DECIMALS18 = 1 ether;
    /// @notice 100% with precisions 100% -> 100 * 10^18
    ...
    /// @notice last time of the check whether there are valid governance rewards for distribution
    uint256 public lastTimeGovRewardCheck;
    /// @notice conditions of interval and week day number what point
    /// if gov reward should be checked
    CheckGovParams public timeGovConditions;
    ...
    /// @dev total voting power defined for the specific proposal
    ...
    /// @dev voting power amount for the proposal
    ...
    /// @dev last valid checked proposal index for the governance rewards distribution
    LastProposalIndexInfo private indexOfLastCheckedProposal;

    /// @dev point if proposal with id 0 where used
    ...
    constructor(
        ...
        loopClaimLimit = 1000;
        timeGovConditions.interval = 7 days;
        timeGovConditions.day = 0;
    ...
    function setWeekDays(uint8 day_) external onlyAllowed {
        require(day_ >= 0 && day_ < 7, "INVALID_VALUE");
        timeGovConditions.day = day_;
    }
    ...
    function setIntervalForCheck(uint256 interval_) external onlyAllowed {
        ...
        timeGovConditions.interval = interval_;
    }

     /**
     * @dev setting address to game factory contract
     * @param gameFactory_ address of LotteryGameFactory contract
     */
    function setGameFactory(address gameFactory_)
        external
        onlyOwner
    {
        require(gameFactory_ != address(0), "ZERO_ADDRESS");
        gameFactory = gameFactory_;
    }
    ...
    function claimCommunityReward() external override {
        ...
        require(
            lastClaimedDistribution[user].communityReward <
                rewardDistributionInfo.length,
            "NOTHING_TO_CLAIM"
        );
        uint256 from = lastClaimedDistribution[user].communityReward;
        ...
        lastClaimedDistribution[user].communityReward = to;
    ...
    /// @return lottoRewards amount of lotto rewards
    /// @return gameRewards amount of game rewards
    ...
    {
        (lottoRewards, gameRewards) = _availableReward(
            user,
            rewardDistributionInfo.length,
            lastClaimedDistribution[user].communityReward,
            lastClaimedDistribution[user].isCommNullIndexUsed,
            false
        );
    }
        ...
        returns (uint256 lottoRewards, uint256 gameRewards)
    {
        (lottoRewards, gameRewards) = _availableReward(
            user,
            govRewardsInfo.length,
            lastClaimedDistribution[user].governanceReward,
            lastClaimedDistribution[user].isGovNullIndexUsed,
            true
        );
    }

    /// @dev Called by Chainlink Keepers to check if work needs to be done
    /// @notice Runs off-chain at every block to determine
    /// if the performUpkeep function should be called on-chain
    ...
    {
        upkeepNeeded = _checkDate(block.timestamp);
    }
    ...
    function _checkGovernanceReward() internal {
        ...
        if (
            ...
            bool condition = indexOfLastCheckedProposal.prevCountOfProposals !=
                0 &&
                !isNullIdPropActive;
            uint256 proposalCount = totalCountOfProposals -
                indexOfLastCheckedProposal.index;

            uint256 countOfNewProposals = condition
                ? proposalCount - 1
                : proposalCount;
    ...
    function claimGovernanceReward() external override {
        ...
        uint256 from = lastClaimedDistribution[user].governanceReward;
        ...
        lastClaimedDistribution[user].governanceReward = to;

        if (to >= 0) {
            if (!lastClaimedDistribution[user].isGovNullIndexUsed)
                lastClaimedDistribution[user].isGovNullIndexUsed = true;
        }
    }

    /// @dev Add new community reward distribution portion
    /// @param distributionInfo structure of <CommunityReward> type
        ...
        for (uint256 i = from; i <= to; i++) {
            ...
            for (
                ...
                if (
                    ...
                    (uint256 lotto, uint256 game) = _governanceRewardCondition(
                        govRewards,
                        i,
                        user,
                        propIndex
    ...
    function _communityRewardCondition(
        ...
        uint256 totalRewardSupply = gameInfo.isMainLottoToken
            ? totalLottoSupply
            : totalGameSupply;
        uint256 totalUsersHoldingsExludingRewardsAmount = gameInfo
            .totalUsersHoldings - totalRewardSupply;
    ...
    /// @notice calculate available reward based on the distribution type
    /// @param user address need check rewards for
    /// @param length total length of distribution
    /// @param lastRewardIndex index of last claimed distribution of the 'user'
    /// @param isNullIndex point whether null index distribution was claimed by 'user'
    /// @param isGovReward point whether is governance reward type or not (Community)
    /// @return lottoRewards amount of lotto rewards
    /// @return gameRewards amount of game rewards
    function _availableReward(
        ...
        bool isNullIndex,
        bool isGovReward
    ) private view returns (uint256 lottoRewards, uint256 gameRewards) {
        require(length != 0, "INVALID_ITEMS");
        require(lastRewardIndex < length, "NOTHING_TO_CLAIM");
        uint256 from = lastRewardIndex;
        if (isNullIndex) {
            from++;
        }
        uint256 to = length - 1;
        (lottoRewards, gameRewards) = isGovReward
            ? _availableGovernanceReward(from, to, user)
            : _availableCommunityReward(from, to, user);
    }
        ...
        returns (uint256 lottoAmount, uint256 gameAmount)
    {
        ...
        if (extraPoolValues.lottoAmount != 0) {
            uint256 extraLotto = (extraPoolValues.lottoAmount *
                10 *
                DECIMALS18) / ONE_HUNDRED_PERCENT_WITH_PRECISIONS;
            extraPool.lottoAmount -= extraLotto;
            lottoAmount += extraLotto;
        }

        if (extraPoolValues.gameAmount != 0) {
            uint256 extraGame = (extraPoolValues.gameAmount * 10 * DECIMALS18) /
                ONE_HUNDRED_PERCENT_WITH_PRECISIONS;
    ...
    function _checkDate(uint256 timestamp) private view returns (bool) {
        return
            (timestamp >=
                timeGovConditions.interval + lastTimeGovRewardCheck) &&
            (dateTime.getWeekday(timestamp) == timeGovConditions.day);
    }
}
...
contract GovernancePool {
    ...
    ///@dev checked if the msg.sender is the lottery game contract
 ...
 */
 ...
 interface IFacroty{
    ...
    function autorizedLotteryGames(address lottery) external view returns(LotteryGameInfo memory);
 }
...
contract DateTime {
    ...
    mapping (address => TimelockInfo) private timelock; // additional lottery timelock, needed for FE
    IFacroty public immutable factory;

    modifier onlyAutorizedLotteryGame(address lottery){
        require(factory.autorizedLotteryGames(lottery).isAuthorized, "NOT_AUTORIZED");
        _;
    }
    constructor(address factory_){
        factory = IFacroty(factory_);
    }
    ...
    function setTimelock(
        address lottery, 
        uint8[] memory daysUnlocked,
        uint8[] memory hoursStartUnlock,
        uint256[] memory unlockDurations 
    ) external onlyAutorizedLotteryGame(msg.sender){
        timelock[lottery] = TimelockInfo(
            daysUnlocked,
            hoursStartUnlock,
            unlockDurations
        );
    ...
    function getTimelock(address lottery)
        external
        view
        returns(TimelockInfo memory timelock_)
    {
        return timelock[lottery];
    }
}
