...
contract Rental is PaymentTokens, Withdrawable {
    ...
    mapping(address => mapping(uint256 => bool)) private _isExisted;
    ...
    modifier isValidRentingNFT(address nftAddress, uint256 tokenId) {
        require(!_isExisted[nftAddress][tokenId], REVERT_ALREADY_LISTED);
    ...
    function rent(uint256 orderId) external payable isNotRenting(orderId) {
        ...
        _isExisted[rental.nftAddress][rental.nftId] = false;
    ...
    function _setListRental(
        ...
        require(_rentalInfo[orderId].renter == address(0) , REVERT_ALREADY_RENTING);
        _isExisted[nftAddress][tokenId] = true;
    ...
    function listRental(
        ...
        uint256 price,
        uint256 period
    ) external isOwnerOf(nftAddress, tokenId) isValidRentingNFT(nftAddress, tokenId) isValidRate(ownerRewardRate) isValidPeriod(period) isValidPaymentToken(paymentToken) {
