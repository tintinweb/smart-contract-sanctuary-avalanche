...
contract StrattonPresale is Ownable {
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 pBWDPurchaseAmount = _amount.mul(1e18).div(priceDAI);

        require(
            (IpBWD(address(pBWD)).totalSupply() + pBWDPurchaseAmount) <=
                maxSupply,
            "Purchase amount more than available pBWD supply"
        );

        UserInfo storage user = userInfo[msg.sender];

        require(
            userpBWDCap >= user.pBWDPurchased.add(pBWDPurchaseAmount),
        ...
        user.pBWDPurchased = user.pBWDPurchased.add(pBWDPurchaseAmount);
        ...
        returns (uint256)
    {
        ...
        return userpBWDCap.sub(user.pBWDPurchased);
    }

    function getRemainingPresaleTokens() external view returns (uint256) {
        if (IpBWD(address(pBWD)).totalSupply() >= maxSupply) {
            return 0;
        }
        return maxSupply - IpBWD(address(pBWD)).totalSupply();
    }

    function whenLambo(address _user) external view returns (string memory) {
        return "Soon\u2122";
    }
}