...
contract BridgesOfFate is Ownable {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if(totalNFTOnTile[_currentUserStage][_jumpSide] <= 0){                
           _selectPlayerAndMakeCapacityOnTile(_currentUserStage,currentDay,_jumpSide);
        }
    ...
    event selectJumpAtSidePlayerIdEvent(bytes32[] _jumpAtSidePlayerId);

   function _selectSuccessSidePlayerId(bytes32[] memory _playerIDAtSpecifyStage, bool _jumpAtSide,uint256 currentDay) public  returns(bytes32[]  memory){ 
        ...
        emit selectJumpAtSidePlayerIdEvent(SelectSuccessSidePlayerId);
    ...
    */
    ...
    function _selectPlayerAndMakeCapacityOnTile(uint256 _currentPlayerStage,uint256 currentDay,bool _jumpSide) internal  { 
        ...
        if(_currentPlayerStage  > 0){
            ...
            // currentJumpSide = GameMap[_currentPlayerStage] >= 50e9;

            _selectSuccessSidePlayerIdList =  _selectSuccessSidePlayerId(PlayerIDAtSpecifyStage,_jumpSide,currentDay);
            //If tile has 3 or more nft's on jumped side then use random selection function. 

            if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentPlayerStage]){
                ...
                _randomSelectPlayerIndexPreviousStage = generateRandomTicketNumbers(SelectUnholdPlayerId.length - 1 ,SelectUnholdPlayerId.length - 1,TitleCapacityAgainestStage[_currentPlayerStage]);
            }
              ...
              if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentPlayerStage]){
                ...
                if(TitleCapacityAgainestStage[_currentPlayerStage]  > 0 && _setCapacityPreviouseStage > 0){
                    totalNFTOnTile[_currentPlayerStage][currentJumpSide] = TitleCapacityAgainestStage[_currentPlayerStage] - _setCapacityPreviouseStage;
                }else{
                    totalNFTOnTile[_currentPlayerStage][currentJumpSide] = totalNFTOnTile[_currentPlayerStage][currentJumpSide];
                }
            }            
        }
