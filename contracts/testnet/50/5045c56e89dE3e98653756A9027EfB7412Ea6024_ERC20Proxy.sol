...
contract ERC20Proxy {
    ...
    constructor(address implementation_, address admin_) {
        _upgradeToAndCall(implementation_, false);
        _changeAdmin(admin_);
    }
    ...
    function _upgradeToAndCall(address newImplementation, bool forceCall) internal {
        _upgradeTo(newImplementation);
        if (msg.data.length > 0 || forceCall) {
            _delegate(newImplementation);
        }
    }
