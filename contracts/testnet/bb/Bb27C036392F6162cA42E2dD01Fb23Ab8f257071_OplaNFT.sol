...
contract OplaNFT is ERC721A, Ownable, ILayerZeroReceiver  {
    uint256 gas = 350000;
    ...
    uint256 private publicPrice = 0.001 ether;
    address private seedbagContractAddress;
    ...
    constructor(address _endpoint) ERC721A("Optimistic Plants", "OPLA") {
        endpoint = ILayerZeroEndpoint(_endpoint);
    }

    // Mint

    function mintWithSeedbag() external {
        address addr = msg.sender;
        require(sellingStep == Step.PrivateSale, "Private sale is not active");
        IPass pass = IPass(
            seedbagContractAddress
        );
        require(
            pass.balanceOf(addr, 0) >= 1,
            "You need at least one pass to mint"
        );
        require(totalSupply() + 1 <= MAX_SUPPLY, "Purchase would exceed max tokens");
        pass.burn(addr, 1);
        _safeMint(addr, 1);
    }
    ...
    function setseedbagContractAddress(address _seedbagContractAddress)
        external
        onlyOwner
    {
        seedbagContractAddress = _seedbagContractAddress;
    }

    function getseedbagContractAddress() external view returns (address) {
        return seedbagContractAddress;
    }

    function getSeedbagAmount(address addr) external view returns (uint256){
                IPass pass = IPass(
            seedbagContractAddress
        );
        return pass.balanceOf(addr, 0);
    }
