...
contract InvestPool {
    ...
    GeekzPass public geekzPassAddress;
    GamePool public gamePool;

    address public tokenAddress; // TUS address
    address public mainPoolAddress;
    ...
    function invest(uint _amount) public {
        ...
        gamePool = GamePool(mainPoolAddress);
        gamePool.invest(poolNum, msg.sender, _amount);
    }

    function mintGeekzPass() public {
        // check if status is deployed
        require(status == PoolStatus.DEPLOYED, "Not DEPLOYED");

        // check if user invested in this pool
        require(balances[msg.sender] > 0, "Not Invested");

        geekzPassAddress.mintPass(
            poolGameId,
            poolNum,
            msg.sender,
            balances[msg.sender]
        );
        // uint _gameId, uint _poolNum, address to, uint _invest
    }

    function getInvestorAddress() public view returns (address[] memory) {
        return investors;
    }

    function setReceiverAddress(address _receiverAddress) external onlyOwner {
        console.log("Set receiverAddress to '%s'", _receiverAddress);
        receiverAddress = _receiverAddress;
    }

    function setGeekzPass(address _gp) external onlyOwner {
        geekzPassAddress = GeekzPass(_gp);
    }
    ...
    function finalize() external onlyOwner {
        ...
        // add total investment to GamePool
        gamePool = GamePool(mainPoolAddress);
        gamePool.poolClosed(poolNum, total);
    }

    function setMainPoolAddress(address _mainPoolAddress) external onlyOwner {
        console.log("Set mainPoolAddress to '%s'", _mainPoolAddress);
        mainPoolAddress = _mainPoolAddress;
    }

    function setTokenAddress(address _tokenAddress) external onlyOwner {
        console.log("Set tokenAddress to '%s'", _tokenAddress);
        tokenAddress = _tokenAddress;
    }

    function setPoolGame(string memory _poolGame) external onlyOwner {
        console.log("Changing poolGame from '%s' to '%s'", poolGame, _poolGame);
        poolGame = _poolGame;
    }

    function setPoolTime(uint _poolStart, uint _poolEnd) external onlyOwner {
        console.log(
            "set pool time open from '%s' to '%s'",
            _poolStart,
            _poolEnd
        );
        poolStart = _poolStart;
    ...
    function setPoolCap(uint _poolSoftcap, uint _poolHardcap)
        external
        onlyOwner
    {
        console.log(
            "set pool cap soft: '%s' hard: '%s'",
            _poolSoftcap,
            _poolHardcap
        );
    ...
    function setMinInvest(uint _minInvest) external onlyOwner {
        console.log("set minInvest '%s'", _minInvest);
        minInvest = _minInvest;
    }
}
...
contract GeekzPass is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("GeekzPass", "GPASS") {}

    function _baseURI() internal pure override returns (string memory) {
        return "https://gpass.geekzwolf.com/";
    }
    ...
    function mintPass(
        ...
        for (uint i = 0; i < investors.length; i++) {
            // check if to is on the list and gameId + investment + poolNum matched
            if(msg.sender == investors[i] && !minted[i]){
                ...
                // after mint, don't forget to set false the state of user + gameid + investment + poolNum
                minted[i] = true;
            }
        }
