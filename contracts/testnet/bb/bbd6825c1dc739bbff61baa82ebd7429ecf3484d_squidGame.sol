...
contract squidGame is ERC721Enumerable, Ownable {
    ...
    function enteryFee() public payable {
...
_member.feeStatus = true;
 uint256 seriesOneBalance = seriesOne.balanceOf(msg.sender);
        ...
        if (seriesOneBalance > 0 && seriesTwoBalance > 0) {
            deposit();
        } else if (seriesTwoBalance > 0) {
            deposit();
        }
    ...
    */
    function partitionInGame(bool _chooes_side) public after24Hours GameInitialized {
        GameMember memory _member = Player[msg.sender];
        start_period = block.timestamp;
        require(STAGES >= _member.level, "Reached maximum");
        if (randomNumber <= 0) {
