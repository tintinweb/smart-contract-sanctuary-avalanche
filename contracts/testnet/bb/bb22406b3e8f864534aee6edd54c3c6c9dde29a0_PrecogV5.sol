/**
 *Submitted for verification at testnet.snowtrace.io on 2022-06-24
*/
...
contract PrecogV5 is IPrecogV5, ReentrancyGuard {
    ...
    function _processHandleIncreaseInvestment(address _token, address _account, uint _amount) internal returns (uint _depositAmount){
        ...
        if(precogStorage.getInvestmentsOf(_token, _account).length == 0) {
            ...
            IPrecogStorage.Cycle memory _futureTradingCycle = precogInternal.getTradingCycleByTimestamp(_token, _nextFundingTime);
            ...
            if(_futureTradingCycle.id >= _lastInvestmentOf.idChanged) {
                ...
                IWithdrawalRegister.Register memory _register = _getWithdrawalRegisterInstance().getRegister(_token, _account);
                if(_amount < _register.amount) {
                    precogInternal.increaseInvestment(
                        _token, 
                        _account, 
                        _amount,
                    ...
                    _getWithdrawalRegisterInstance().modifyRegister(
                        ...
                        IWithdrawalRegister.Register({
                            amount: _register.amount - _amount,
                            deadline: _register.deadline
                        })
                    );
                    
                } else if(_amount == _register.amount) {
                    ...
                    for(uint i = _futureTradingCycle.id + 1; i <= _lastInvestmentOf.idChanged; i++) {
                        _futureTradingCycle = precogInternal.getTradingCycleByTimestamp(_token, _futureTradingCycle.endTime);
                        uint _unit =  _depositAmount * (_futureTradingCycle.endTime - _futureTradingCycle.startTime);
                        uint _newUnit = precogStorage.getTotalUnitsTradingCycle(_token, i) + _unit;
                        precogStorage.updateTotalUnitsTradingCycle(_token, i, _newUnit);
                        if(_lastInvestmentOf.isWhitelist) {
                            uint _newUnitForWhitelist = precogStorage.getTotalUnitsForWhitelistTradingCycle(_token, i) + _unit;
                            precogStorage.updateTotalUnitsForWhitelistTradingCycle(_token, i, _newUnitForWhitelist);
                        }
                    }
                }
            }
    ...
    function deposit(address token, uint amount) external override isExistingToken(token) nonReentrant {
        require(amount >= _getCoreInstance().minFunding(token), "PrecogV5: Amount must be greater than min funding amount");
        ...
        require(amountInvestment <= _getCoreInstance().maxFunding(token), "PrecogV5: You can not deposit greater than max funding amount");
        ...
        require(precogStorage.getLiquidity(token) <=  _getCoreInstance().maxFundingPool(token), "PrecogV5: Liquidity amount must be less than limitation of pool");
    ...
    function _processHandleDecreaseInvestment(address _token, uint _amount, bool _isFirstRequestWithdrawal) internal returns (uint _remainingAmount) {
        ...
        if (_isFirstRequestWithdrawal) {
            ...
            if (_getWithdrawalRegisterInstance().isInDeadline(_token, msg.sender)) {
                _nextDefundingTime = uint48(_getWithdrawalRegisterInstance().getRegister(_token, msg.sender).deadline);
            } else {
    ...
    function requestWithdrawal(address token, uint amount) external override isExistingToken(token) nonReentrant {
        require(amount >= _getCoreInstance().minDefunding(token), "PrecogV5: Amount must be greater than min defunding amount");
        precogInternal.updateCurrentTradingCycle(token, false, 0);
        ...
        require(remainingAmount == 0 || remainingAmount >= _getCoreInstance().minFunding(token), "PrecogV5: Remaining amount must be equal to zero or greater than min funding amount");
    ...
    function turnOnWhitelist(address token, address account) external {
    ...
    function turnOffWhitelist(address token, address account) external {
