...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    function mintUniqueTokenTo(address account, string memory affiliate_code) public payable {
        ...
        if (bytes(affiliate_code).length > 0 && (_activeZone == 4 || _activeZone == 5)) {
            uint affiliateDiscount = Affiliate(_affiliateContractAddress).getAffiliateDiscount();
            ...
            if (zoneDiscount < affiliateDiscount) {
                ...
                uint affiliatePaybackPercent = Affiliate(_affiliateContractAddress).getAffiliatePayback();
...
contract Affiliate is Pausable, Ownable {
    address private _owner;
    uint256 public affiliateDiscount;
    uint256 public affiliatePaybackPercent;
    mapping(address => bool) private adminAddressList;

    constructor() {
        affiliateDiscount = 10;
        affiliatePaybackPercent = 15;
        adminAddressList[0xCeaf7780c54bc6815A8d5c3E10fdc965d0F26762] = true;
        adminAddressList[0x9CB52e780db0Ce66B2c4d8193C3986B5E7114336] = true;
        adminAddressList[0xbDe951E26aae4F39e20628724f58293A4E6457D4] = true;
        adminAddressList[msg.sender] = true;
    ...
    function getAffiliateDiscount() public returns(uint256) {
        return affiliateDiscount;
    }

    function getAffiliatePayback() public returns(uint256) {
        return affiliatePaybackPercent;
    }
    ...
    function changeAffiliateDiscount (uint newDiscount) public onlyAdmin() {
        require(newDiscount > 0 && newDiscount < 100, "NFKey: wrong discount value");
        affiliateDiscount = newDiscount;
    }

    function changeAffiliatePayback (uint newPayback) public onlyAdmin() {
        require(newPayback > 0 && newPayback < 100, "NFKey: wrong payback value");
        affiliatePaybackPercent = newPayback;
    }
