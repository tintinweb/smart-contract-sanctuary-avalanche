...
contract SyntheticPoolV2 is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 internal poolTradingHourStartDate;              // Pool trading start timestamp(match with stock market hour)
    ...
    function initContract(address _reservePool, address _systemCoin, address _USBPool, uint256 _minSystemCoinTxAmount , uint256 _txFee, uint256 _tradingHourStartDate) external onlyOwner{
        ...
        txFee = _txFee;
        poolTradingHourStartDate = _tradingHourStartDate;
    }
    ...
    */
    ...
    function getTradingHourStartDate() external view returns (uint256) {
        return poolTradingHourStartDate;
    }
    
    // For future restrited trading hour usage
    function _calculateDays(uint256 pid, uint256 currentTimestamp) view public returns(uint256, uint256) {
        PoolInfo memory pool = poolInfo[pid];
        uint256 numberOfDays = (currentTimestamp - poolTradingHourStartDate)/ (1 days);
        uint256 openingTime = poolTradingHourStartDate * numberOfDays * (1 days);
    ...
    */
    function updateStartDate(uint256 newStartTimeStamp) external onlyOwner {
        require(newStartTimeStamp > poolTradingHourStartDate, "< current startDate");
        poolTradingHourStartDate = newStartTimeStamp;
    }
