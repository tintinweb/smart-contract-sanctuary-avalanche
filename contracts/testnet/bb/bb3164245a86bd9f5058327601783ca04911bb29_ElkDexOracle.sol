...
contract ElkDexOracle {
    ...
    constructor(address weth_, address factory_, uint windowSize_, uint8 granularity_) public {
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) public view returns (uint amountOut) {
        ...
        address pair = ElkLibrary.pairFor(factory, tokenIn, tokenOut);
        ...
        (uint price0Cumulative, uint price1Cumulative,) = ElkOracleLibrary.currentCumulativePrices(pair);
        (address token0,) = ElkLibrary.sortTokens(tokenIn, tokenOut);
    ...
    function consultWeth(address tokenIn, uint amountIn) external view returns (uint amountOut) {
        return consult(tokenIn, amountIn, weth);
    }
}