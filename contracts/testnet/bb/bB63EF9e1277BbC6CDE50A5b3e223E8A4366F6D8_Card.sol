...
contract Card is ExtendERC721, ICard {
    ...
    // 0: female, 1: male
    mapping(uint256 => uint8) public genderOfCards;
    ...
    constructor() ExtendERC721("Dogeon Card", "DOGEC") {
    ...
    ) external override(ICard) allowMint returns (uint256) {
        uint256 newCardId = increment();

        _mint(_to, newCardId);
        _setMetadata(newCardId, _metadataId);
        genderOfCards[newCardId] = _gender;

        emit CardCreated(_to, newCardId, _gender, _metadataId);

        return newCardId;
    }

    function setMintRole(address _to) external onlyOwner {
        _setupRole(MINT_ROLE, _to);
    }
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i != length; i++) {
            uint256 cardId = ids[i];

            super.safeTransferFrom(from, to, cardId);
        }
    }

    function getCardGender(uint256 id) external view override returns (uint8) {
        require(_exists(id), "ExtendERC721: query for nonexistent token");
        return genderOfCards[id];
    }
...
interface ICard {
    function mint(address _to, uint8 _gender, string memory _metadataId) external returns(uint256);
    function getCardGender(uint256 id) external view returns (uint8 genes);
...
import "../swap/IERC721Swappable.sol";
