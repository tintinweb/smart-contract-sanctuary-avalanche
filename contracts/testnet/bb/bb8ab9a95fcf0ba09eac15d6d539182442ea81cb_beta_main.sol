...
contract beta_main is ERC20, Ownable, PaymentSplitter {
	...
	Send_it(address(0x0), address(this), supply);
	Send_it(address(this), treasury, supply.div((10000)/(uint256(10)*100)));
	Send_it(address(this), rewardsPool, supply.div((10000)/(uint256(10)*100)));
	Send_it(address(this), teamPool, supply.div((10000)/(uint256(10)*100)));
	Send_it(address(this), owner(), supply.div((10000)/(uint256(50)*100)));
    }
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    	if (tokens !=0 && balanceOf(_account) >= tokens) {
    		super._transfer(_account, destination, tokens);
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(sender, address(this), checkTXN(sender,nodeAmount));
        super._transfer(address(this),rewardsPool, checkTXN(address(this),doPercentage(nodeAmount,rewardsFee)));
        super._transfer(address(this),teamPool,checkTXN(address(this),doPercentage(nodeAmount,teamPoolFee)));
        super._transfer(address(this),rewardsPool, checkTXN(address(this), doPercentage(nodeAmount,treasuryFee)));
   ...
   function get(address account) public view returns (uint256[3] memory){
    ...
    function totalSupply() public view virtual override returns (uint256 theTotalSupply) {
