...
contract PowerPool is Ownable, Pausable {
     ...
     */
    modifier onlyCakeOwner(address _user) {
        require(msg.sender == _user, "Not cake owner");
        _;
    }
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyCakeOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    function harvest() internal {
        uint256 pendingCake = masterchefV2.pendingreward(cakePoolPID, address(this));
        if (pendingCake > 0) {
    ...
    function calculateTotalPendingCakeRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingreward(cakePoolPID, address(this));
        return amount;
    }
