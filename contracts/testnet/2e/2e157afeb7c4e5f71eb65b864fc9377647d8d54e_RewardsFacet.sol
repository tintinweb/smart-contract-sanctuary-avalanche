...
contract RewardsFacet {
    ...
    /// @notice Get 'dailySplit' value in 'AppStorage'
    function dailySplit() external view returns (uint256) {
        return s.dailySplit;
    }
    ...
    /// @param _id Node ID
    function calculateRewards(address _account, uint256 _id) external {
        // IF NOT NODE OWNER REVERT
        Node storage node = s.userNodes[_account][_id];

        node.rewardDebt = s.dailySplit * node.amount;
    }
}
