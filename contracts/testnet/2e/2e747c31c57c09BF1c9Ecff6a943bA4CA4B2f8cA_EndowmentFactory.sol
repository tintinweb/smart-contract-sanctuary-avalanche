...
contract EndowmentFactory {
    /// @notice array to store all endowments created
    Endowment[] public endowments;

    /**
     * @notice mapping to keep track of all the endowment address created by a user
     */
    mapping(address => address[]) public userToEndowment;

     /**
     * @notice mapping to keep track of all the endowment address created by a endowment owner
     */
    mapping(address => address[]) public ownerToEndowment;

    /**
     * @notice event to emit when a new endowment is created
     * @param owner address of endowment owner
     * @param endowmentNumber Number of endowment
     * @param creator address of the creator
     */
    event EndowmentCreated(address indexed owner, uint indexed endowmentNumber, address creator);

    /**
     * @notice function to create new endowment 
     * @param _owner address of endowment owner
     */
    function createEndowment(address _owner) public returns (address) {
        Endowment endowment = new Endowment(_owner);
        endowments.push(endowment);
        userToEndowment[msg.sender].push(address(endowment));
        ownerToEndowment[_owner].push(address(endowment));
        emit EndowmentCreated(_owner, endowments.length, msg.sender);
        return address(endowment);
    }

    /**
     * @notice function to return total number of endowments created
     */
    function totalNoOfEndowments() public view returns (uint256) {
        return endowments.length;
    }

    /**
     * @notice function to get all the endowments created
     */
    function getEndowments() public view returns(Endowment[] memory){
        return endowments;
    }
}
