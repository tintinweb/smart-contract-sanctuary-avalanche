...
contract PrivateSale is Ownable, Initializable {
    ...
    function deposit(uint256 amount) external {
        ...
        rewards[msg.sender] += SellingAmount;
    ...
    function claim() external {
        ...
        uint256 SellingAmount = rewards[msg.sender];
        require(SellingAmount != 0, "no claim reward");

        uint256 contractBalance = tokenSelling.balanceOf(address(this));
        require(contractBalance >= SellingAmount, "no available Selling");

        rewards[msg.sender] = 0;
        tokenSelling.transfer(msg.sender, SellingAmount);

        emit Claim(msg.sender, SellingAmount);
    }
