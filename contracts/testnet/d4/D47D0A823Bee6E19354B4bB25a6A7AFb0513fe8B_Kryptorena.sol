...
/**
 * @title Kryptorena Battle contract
 * @notice This is the battle contract. This contract works in tangent with the game logic contract.
 * Whenever a user starts a battle from the lobby in the Kryptorena contract it will trigger this contract to
 * initiate the battle with the users stats. Initially, this contract will have to be linked with
 * the Kryptorena contract through the initialize() function. Once two players have
 ...
 * attack and defense values will then be sent to the Kryptorena contract (through endGame()) for
 * player stat update. It should be noted that multiple matches can be hosted simultaneously
 * using this contract.
 */
...
contract KryptorenaBattle is VRFConsumerBaseV2, Ownable {
    ...
    struct BattleData {
        endStatus player1Status;
        endStatus player2Status;
        uint256 turn;
    ...
    function attack() public {
        ...
        require(round.player1HP > 0 && round.player2HP > 0, "The game has ended.");
    ...
    function endGame(address player) private {
        ...
        if (round.player1HP > round.player2HP) {
            resultData.player1Status = endStatus.WON;
            resultData.player2Status = endStatus.LOST;
        } else if (round.player1HP == round.player2HP) {
            resultData.player2Status = resultData.player1Status = endStatus.DRAW;
        } else {
            resultData.player1Status = endStatus.LOST;
            resultData.player2Status = endStatus.WON;
        }
        address winner;
        if (resultData.player1Status == endStatus.WON) {
            winner = round.player1;
            requestRandomWord(winner);
        } else if (resultData.player2Status == endStatus.WON) {
