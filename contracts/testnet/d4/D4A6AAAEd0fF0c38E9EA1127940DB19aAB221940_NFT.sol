...
contract NFT is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 private immutable MAX_SUPPLY = 10;
    uint256 private immutable MAX_COMMUNITY = 7;
    uint256 private immutable MAX_ULTIMATE = 3;
    uint256 private immutable MAX_SALE_COMMUNITY = 5;
    uint256 private immutable MAX_SALE_ULTIMATE = 2;
    uint256 private immutable MAX_MINT_PRIVATE = 3;

    uint256 private immutable community_price = 0.05 ether;
    uint256 private immutable ultimate_price = 0.06 ether;
    ...
    mapping(address => bool) private unlimitedMinted;

    string private baseURI_;
    string private baseExtension_;

    constructor(address _sys) ERC721("Test NFT", "TESTNFT") {
        sys = _sys;
    }

    /// @dev For testing purposes

    function startU() external onlyOwner {
        unlimited_pass_mint_start = block.timestamp;
        unlimited_pass_mint_end = block.timestamp + 1000;
        private_mint_start = block.timestamp + 1100;
        private_mint_end = block.timestamp + 2000;
        public_mint_start = block.timestamp + 2010;
        public_mint_nex = block.timestamp + 2500;
    }

    function startPriv() external onlyOwner {
        unlimited_pass_mint_start = block.timestamp - 100;
        unlimited_pass_mint_end = block.timestamp - 5;
        private_mint_start = block.timestamp;
        private_mint_end = block.timestamp + 1000;
        public_mint_start = block.timestamp + 1010;
        public_mint_nex = block.timestamp + 1500;
    }

    function startPub() external onlyOwner {
        unlimited_pass_mint_start = block.timestamp - 100;
        unlimited_pass_mint_end = block.timestamp - 95;
        private_mint_start = block.timestamp - 50;
        private_mint_end = block.timestamp - 5;
        public_mint_start = block.timestamp;
        public_mint_nex = block.timestamp + 1000;
    }

    function startNex() external onlyOwner {
        require(block.timestamp > public_mint_start);
        public_mint_nex = block.timestamp;
    }
    ...
    function getAllUltimateTokens() external view virtual returns (uint256[] memory tokenIds) {
        tokenIds = new uint256[](ultimate_supply);
        uint256 counter;
        for (uint256 i = 1; i <= supply; i++) {
            if (tokenInfo[i].rarity == 1) {
                tokenIds[counter] = i;
                counter ++;
            }
        }
    ...
    function getAllCommunityTokens() external view virtual returns (uint256[] memory tokenIds) {
        tokenIds = new uint256[](community_supply);
        uint256 counter;
        for (uint256 i = 1; i <= supply; i++) {
            if (tokenInfo[i].rarity == 0) {
                tokenIds[counter] = i;
                counter ++;
            }
        }
    ...
    function mintUltimate(bytes memory sig) external payable nonReentrant {
        ...
        require(!unlimitedMinted[_msgSender()], "You have already minted in this round");
        unlimitedMinted[_msgSender()] = true;
        ultimate_supply += 1;
        supply += 1;
