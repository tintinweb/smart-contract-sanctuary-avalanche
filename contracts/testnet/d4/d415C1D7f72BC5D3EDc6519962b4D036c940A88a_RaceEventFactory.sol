...
interface IRewardManager {
    ...
    function claimRewards(uint256 position) external;
    ...
    ERC1155Holder
{
    ...
    /// As winner, claim rewards for the won position.
    /// @param position to claim rewards for.
    function claimRewards(uint256 position) external override nonReentrant {
...
contract Race is IRace, Context, Ownable, ReentrancyGuard, TokenWithdrawer {
    ...
    /// @param position to claim rewards for.
    function claimRewards(uint256 position) external {
        rewardManager.claimRewards(position);
    }
    ...
    /*
    function setPositionResults(address[] calldata results) external {
        return rewardManager.setPositionResults(raceID, results);
    }
    */
}
