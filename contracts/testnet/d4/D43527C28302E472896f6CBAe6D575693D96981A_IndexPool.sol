...
contract IndexPool is ERC20Upgradeable, IIndexPool {
  // BConst
  ...
  function configure(
    ...
    require(_controller == zero, "DexETF: Already configured");
    require(controller != zero, "DexETF: Controller is zero address");
    require(
      exitFeeRecipient != zero
      && exitFeeRecipientAdditional != zero,
      "DexETF: Fee recipient is zero address"
    );
    require(uniswapV2oracle != zero, "DexETF: Oracle is zero address");
    require(uniswapV2router != zero, "DexETF: Router is zero address");
  ...
  function setExitFeeRecipient(address exitFeeRecipient_, bool additional) external override _control_ returns (bool) {
    require(exitFeeRecipient_ != address(0), "DexETF: Fee recipient is zero address");
    if (additional) {
  ...
  function joinPool(
    ...
    if (maxPoolTokens > 0) require(
      (poolTotal + poolAmountOut) <= maxPoolTokens,
  ...
  function _bind(address token, uint256 minimumBalance) internal {
  ...
  modifier _viewlock_() {
    require(!_mutex, "DexETF: Locked view");
    _;
  }

  modifier _control_ {
    require(msg.sender == _controller, "DexETF: Caller is not controller");
    _;
  }

  modifier _public_ {
    require(_publicSwap, "DexETF: Pool is not public");
    _;
  }
}
