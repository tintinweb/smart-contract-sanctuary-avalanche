...
contract RequestFactory {
    ...
    function _createCreationBasketRequest(
        ...
        Request memory creationBasketRequest = Request(
            ...
            abi.encode(_proposer, _contract, _amount, _to, _basketType),
            false
        );
    ...
    function _createRedemptionBasketRequest(
        ...
        Request memory redemptionBasketRequest = Request(
            ...
            abi.encode(_proposer, _contract, _amount, _to),
            false
        );
...
contract MultiSigV1 is SignedWallet, RequestFactory, RuleFactory {
    ...
    event CreationBasket(address proposer, uint256 amount, address to, string basketType);
    event RedemptionBasket (address proposer,  uint256 amount, address to);
        ...
        notExecuted(_idx)
    {
        ...
        if (requestType == RequestType.CREATION_BASKET) {
            (address _proposer, address _contract, uint256 _amount, address _to, string memory _basketType) = abi.decode(
                data,
                (address, address, uint256, address, string)
            );
            GoldTokenInterface instance = GoldTokenInterface(_contract);
            instance.creationBasket(_amount, _to, _basketType);
            emit CreationBasket(_proposer, _amount, _to, _basketType);
        } else if (requestType == RequestType.REDEMPTION_BASKET){
            (address _proposer, address _contract, uint256 _amount, address _to) = abi.decode(
                data,
                (address, address, uint256, address)
            );
            GoldTokenInterface instance = GoldTokenInterface(_contract);
            instance.redemptionBasket(_amount, _to);
            emit RedemptionBasket(_proposer, _amount, _to);
        } else {
    ...
    function creationBasket (
        ...
        if (_amount > _creationLimit){
            ...
            GoldTokenInterface instance = GoldTokenInterface(_contract);
            instance.creationBasket(_amount, _to, _basketType);
            emit CreationBasket(msg.sender, _amount, _to, _basketType);
        }
    }
    ...
    function redemptionBasket(
        ...
        if (_amount > _redemptionLimit){
            ...
            GoldTokenInterface instance = GoldTokenInterface(_contract);
            instance.redemptionBasket(_amount, _to);
            emit RedemptionBasket(msg.sender, _amount, _to);
        }
    }
