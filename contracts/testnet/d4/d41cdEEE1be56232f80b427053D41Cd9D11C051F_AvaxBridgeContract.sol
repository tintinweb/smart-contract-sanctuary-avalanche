...
contract AvaxBridgeContract is Ownable {
    ...
    WrappedToken public awHBAR;
    address payable public system = payable(0xB0ED63C8395453F7AcF7528bF63C5adFb6CcEc44);
    address public governor = address(0xfD22068Bf2C956bD6f4ab812F11354FB7d7dea35);
    address public bridgeFeesAddress = address(0xB0ED63C8395453F7AcF7528bF63C5adFb6CcEc44);
    ...
    constructor() {
        ...
        WrappedToken _awHBAR = new WrappedToken(
            "Apex wrapped HBAR",
            "awHBAR",
            18
        );
    ...
    function setProcessFees(uint256 targetChainID, uint256 processFees)
        external
        onlyOwner
    {
        _processFees[targetChainID] = processFees;
        emit ProcessFeesUpdated(targetChainID, processFees);
    }
    
    function getProcessFees(uint256 targetChainID) external view returns(uint256){
        return _processFees[targetChainID];
    }
    ...
    function calculateFees(uint256 amount) public view returns (uint256) {
        return amount.mul(_bridgeFee) / 1000;
    }
    ...
    function bridge(
        ...
        require(
            msg.value >= _processFees[_targetChain],
            "Insufficient Process Fee"
        );
        awHBAR.burnFrom(msg.sender, _amount);
    ...
    function bridgeBack(
        ...
        if(serviceFee > 0) {
            awHBAR.mint(bridgeFeesAddress, serviceFee);
        }
        awHBAR.mint(_receiver, transferAmount);
