...
contract arb is Ownable{
    ...
    function addUser(address userAddress, uint256 userShare) external view onlyOwner{
        users memory newUser;
        newUser.userAddress = userAddress;
        newUser.userShare = userShare;
        //return user.push(addUser)-1;
        //return true;
    }
    ...
    function totalShare(uint256 total) public {


    }
    
    function swapper(address[] memory path , address pair, uint256 amount, uint256 amount_outMin, uint256 reserve0Min, uint256 reserve1Min) external onlyOwner() {
