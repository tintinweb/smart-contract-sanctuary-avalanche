...
contract NodeManager is Ownable, Pausable {
   ...
   function returnSoonestDue(address _account) internal returns(uint ){
    ...
    function compileVars(address _account) internal {
    ...
    function getNodeReward(address _account) external returns (uint256) {
        compileVars(_account);
