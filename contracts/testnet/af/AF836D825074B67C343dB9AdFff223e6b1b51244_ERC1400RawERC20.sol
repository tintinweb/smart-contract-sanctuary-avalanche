...
contract CertificateController {
        // event Log(address msgSender,address signer,bytes32 _hash,bytes4 funcId,address to,uint256 amount);
    ...
    function _checkCert(bytes memory data) internal view returns (bool) {
        ...
        if (v == 27 || v == 28) {
            ...
            // Check if certificate match expected transactions parameters
                // emit Log(msg.sender,_signer,_hash,funcId,to,uint256(amount));
