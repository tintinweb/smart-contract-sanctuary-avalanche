...
contract BLV_IDO is Ownable {
    ...
    */
    ...
    function buyBUSDtoUSDT(uint256 _tokenAmount) public {
        ...
        require(amount.add(BUSDFee) <= BUSD.balanceOf(tx.origin),"User must have minimum 10 busd!");
        uint256 per = BUSD_to_USDT.div(fixedValue);
        uint256 tokens = amount.mul(per);
        require (tokens <= USDT.balanceOf(address(this)),"Contract Not Have Enough USDT");
        BUSD.transferFrom(tx.origin,address(this), _tokenAmount); 
        USDT.transfer(tx.origin, tokens);
    ...
    function buyUSDTtoBUSD(uint256 _tokenAmount) public {
        ...
        require(amount > 0, "invalid..");
        require(amount.add(USDTFee) <= USDT.balanceOf(tx.origin),"User must have minimum 10 busd!");
        uint256 per = ((fixedValue).mul(1 ether)).div(BUSD_to_USDT);
        uint256 tokens = (amount.mul(per)).div(1 ether);
        require(tokens > 0, "less value");
        require (tokens <= BUSD.balanceOf(address(this)),"Contract Not Have Enough USDB");
        USDT.transferFrom(tx.origin,address(this), _tokenAmount); 
        BUSD.transfer(tx.origin, tokens);
    ...
    function buyBLVtoUSDT(uint256 _tokenAmount) public {
        ...
        require(amount > 0, "invalid..");
        require(amount.add(BLVFee) <= BLV.balanceOf(tx.origin),"User must have minimum 10 busd!");
        ...
        BLV.transferFrom(tx.origin,address(this), _tokenAmount); 
        USDT.transfer(tx.origin, tokens);
