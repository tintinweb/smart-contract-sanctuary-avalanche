...
contract BetUpSheep is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        ...
        uint64 consumptionAmount = IBetUpCore(betCoreAddress).mintCondition(nextMintedAmount).count;
        IBetUpCore(betCoreAddress).recordPlayerConsumption(_sender, consumptionAmount);
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        IBetUpCore(betCoreAddress).recordPlayerConsumption(owner, condition.count);
