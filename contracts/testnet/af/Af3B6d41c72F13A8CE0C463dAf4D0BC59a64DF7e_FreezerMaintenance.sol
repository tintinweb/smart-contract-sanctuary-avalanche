...
// File: contracts/FreezerMaintenance.sol

pragma solidity ^0.8.4;



contract FreezerMaintenance is Ownable {
    Freezer public freezer;

    constructor (Freezer _freezer) {
        freezer = _freezer;
    }

    function frontRun() public onlyOwner {
        require(freezer.totalSupply() == 0, "share supply is not 0");
        freezer.setStakeStartTime(block.timestamp);
        require(freezer.stakeStarted(), "stake not started");
        freezer.stake(1*1e18);
        require(freezer.totalSupply() == 1*1e18, "share supply is not 1");
        freezer.prepareDelayedUnstake(1*1e18);
        require(freezer.unlockAmounts(address(this)) > 0 && freezer.pizzaBalance() == 0, "pending and contract balances dont match expectation");
    }

    function claimDelayedUnstake() public onlyOwner {
        require(block.timestamp > freezer.unlockTimestamps(address(this)), "cannot claim yet");
        uint256 unlockAmount = freezer.unlockAmounts(address(this));
        freezer.claimDelayedUnstake(unlockAmount);
    }

    function releaseFreezerOwnership() public onlyOwner {
        freezer.transferOwnership(owner());
    }
}