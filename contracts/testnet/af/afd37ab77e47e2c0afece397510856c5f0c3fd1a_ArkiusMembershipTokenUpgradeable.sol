...
import './NFTs/MembershipTokenUpgradeable.sol';
...
contract ArkiusMembershipTokenUpgradeable is Initializable, UUPSUpgradeable, PausableUpgradeable, MembershipTokenUpgradeable, Blacklistable {

    uint256 private lastTokenID;

    IArkiusCertifierToken private certifierToken;

    IArkiusAttentionSeekerToken private seekerToken;
    
    event UpdateCertifierContract(address certifier);
    
    event UpdateSeekerContract(address seeker);

    /// @dev Contains Id of all members.
    uint256[] private allMembers;

    /// @dev mapping from memberId => index of allMembers.
    mapping(uint256 => uint256) private indexAllMembers;

    string public constant name = "Arkius Membership Token";
    ...
    function initialize(string memory contractURI, 
        ...
        __MembershipToken_init(contractURI, tokenURIPrefix, multisigAddress);
    ...
    function setCertifierToken(IArkiusCertifierToken certifier) public onlyOwner {
        certifierToken = certifier;
        emit UpdateCertifierContract(address(certifier));
    }

    function setSeekerToken(IArkiusAttentionSeekerToken seeker) public onlyOwner {
        seekerToken = seeker;
        emit UpdateSeekerContract(address(seeker));
    }

    /**
     * @dev Mints a new membership token for a user.
     *
     * @param uri Metadata about the Member.
     */
    function mint(string calldata uri) whenNotPaused notBlacklisted external {

        lastTokenID     = lastTokenID + 1;
        uint256 tokenId = lastTokenID;

        allMembers.push(tokenId);
        indexAllMembers[tokenId] = allMembers.length;

        super.mint(tokenId, 1, uri);
    }

    function burn(address owner, uint256 value) notBlacklisted external {

        require(owner == _msgSender(), "You are not authorised");

        uint256 memberId = _memberIdOf(owner);

        if (memberId > 0) {
            allMembers[indexAllMembers[memberId] - 1] = allMembers[allMembers.length - 1];
            indexAllMembers[allMembers[allMembers.length - 1]] = indexAllMembers[memberId];
            indexAllMembers[memberId] = 0;
            allMembers.pop();
        ...
        uint256 certifierId = certifierToken.certifierIdOf(owner);
        uint256 seekerId    = seekerToken.attentionSeekerIdOf(owner);

        if (certifierId > 0) certifierToken.burn(owner, value);
        if (seekerId > 0)    seekerToken.burn(owner, value);
    }
    ...
    function certifierAddress() public view returns(address) {
        return address(certifierToken);
    }

    function seekerAddress() public view returns(address) {
        return address(seekerToken);
    }
...
import './ERC1155BaseMembershipUpgradeable.sol';
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract MembershipTokenUpgradeable is Initializable, ERC1155BaseMembershipUpgradeable {

    function __MembershipToken_init(string memory contractURI,
                string memory tokenURIPrefix,
                address       multisigAddress) internal initializer {

        __ERC1155BaseMembership_init(contractURI, tokenURIPrefix, multisigAddress);
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
    }

    /**
     *  @dev see {ERC1155BaseMembership - _mint}
     */
...
import './ERC1155MemberUpgradeable.sol';
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract ERC1155BaseMembershipUpgradeable is Initializable, ERC1155Metadata_URIUpgradeable, HasContractURIUpgradeable, ERC1155MemberUpgradeable {

    /// @dev Contains the mapping from TokenId => user who created that token.
    mapping (uint256 => address) private creators;

    function __ERC1155BaseMembership_init(string memory contractURI, 
                string memory tokenURIPrefix,
                address       multisigAddress) internal initializer {

        __HasContractURI_init(contractURI); 
        __ERC1155Metadata_URI_init(tokenURIPrefix);
        __ERC1155Member_init(multisigAddress);
    }
    ...
    */
    function _mint(uint256 id, uint256 supply, string memory uri) internal {
        ...
        require(memberIDs[_msgSender()] == 0, "MemberID is already minted for this user");

        creators[id]               = _msgSender();
        balances[id][_msgSender()] = supply;
        memberIDs[_msgSender()]    = id;
    ...
    function _burn(address owner, uint256 value) internal {
        ...
        uint256 id =  _memberIdOf(owner);
        require(balances[id][owner] > 0, "You don't posses membership token");
...
contract ERC1155MemberUpgradeable is Initializable, OwnableUpgradeable, IERC1155, ERC165Upgradeable, CommonConstants {
    ...
    mapping(address => uint256) internal memberIDs;
    ...
    */
    ...
    function __ERC1155Member_init(address multisigAddress) internal initializer {
        __Ownable_init();
     ...
     * @dev Caller must be approved to manage the tokens being transferred out of the `from` account (see "Approval" section
     * of the standard).
     * 
    ...
    */
    ...
    function _memberIdOf(address owner) internal view returns (uint256) {
        // The balance of any account can be calculated from the Transfer events history.
        // However, since we need to keep the balances to validate transfer request,
        // there is no extra cost to also privide a querry function.
        return memberIDs[owner];
    }
    ...
    function balanceOfBatch(address[] calldata owners, uint256[] calldata ids)
        ...
        uint256[] memory balanceBatch = new uint256[](owners.length);

        for (uint256 idx = 0; idx < owners.length; ++idx) {
            balanceBatch[idx] = balances[ids[idx]][owners[idx]];
        }
