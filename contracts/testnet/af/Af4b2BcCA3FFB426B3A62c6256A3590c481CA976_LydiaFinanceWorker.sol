...
/// @title LydiaFinanceWorker is a reinvest-optimized Worker which deposit LP into MasterChefV2 introduced in the LydiaFinance's MasterChefV2 migration
contract LydiaFinanceWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  ...
  /// @notice Configuration varaibles for LydiaFinance
  uint256 public fee;
  ...
  IMasterChefLyd public masterChefV2;
  uint256 public pendingLyd;

  struct WorkerInit {
    address operator;
    address baseToken;
    IMasterChefLyd masterChef;
    ITridentRouter router;
    uint256 pid;
    IStrategy addStrat;
    IStrategy liqStrat;
    uint256 reinvestBountyBps;
  ...
  function initialize(WorkerInit calldata data) external initializer {
    ...
    wNative = data.wNative;
    masterChefV2 = data.masterChef;
    router = data.router;
    console.log("operator %s , wnative %s , masterChef %s ,", operator, wNative, address(masterChefV2));

    // 3. Assign tokens state variables
    baseToken = data.baseToken;
    pid = data.pid;
    (IERC20 _lpToken, , , ) = masterChefV2.poolInfo(data.pid);
    lpToken = IPool(address(_lpToken));
    address token0 = lpToken.token0();
    address token1 = lpToken.token1();

    farmingToken = token0 == baseToken ? token1 : token0;
    lyd = address(masterChefV2.LYD());

    // 4. Assign critical strategy contracts
    addStrat = data.addStrat;
    liqStrat = data.liqStrat;
    okStrats[address(addStrat)] = true;
    okStrats[address(liqStrat)] = true;

    // 5. Assign Re-invest parameters
    treasuryAccount = data.treasuryAccount;
    treasuryBountyBps = data.reinvestBountyBps;
    maxReinvestBountyBps = 900;
    setReinvestConfig(data.reinvestBountyBps, data.reinvestThreshold, data.reinvestPath);

    poolConfig = data.poolConfig;

    // 6. Set LydiaFinance swap fees
    fee = lpToken.swapFee();
    feeDenom = 10000;
  ...
  /// @param _reinvestThreshold - The threshold to be reinvested if pendingLyd pass over.
  function _reinvest(
    ...
    uint256 reward = pendingLyd;
    if (reward <= _reinvestThreshold) return;

    // 2. Approve tokens
    lyd.safeApprove(address(router), uint256(-1));
    ...
    if (bounty > 0) {
      ...
      lyd.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty));
    }
    ...
    // 7. Reset approval
    lyd.safeApprove(address(router), 0);
  ...
  /// @param _beneficialVaultBounty - The amount of lyd to be swapped to BTOKEN & send back to the Vault.
  /// @param _callerBalance - The balance that is owned by the msg.sender within the execution scope.
  ...
  function actualBaseTokenBalance() internal view returns (uint256) {
    if (baseToken == lyd) {
      return baseToken.myBalance().sub(pendingLyd).sub(buybackAmount);
    }
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      ...
      // 3. Deposit balance to LydiaFinance and update pendingLyd
  ...
  function setReinvestConfig(
    ...
    require(
      _reinvestPath[0] == lyd && _reinvestPath[_reinvestPath.length - 1] == baseToken,
      "LydiaFinanceWorker::setReinvestConfig:: _reinvestPath must start with lyd, end with BTOKEN"
    );
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    ...
    require(
      _rewardPath[0] == lyd && _rewardPath[_rewardPath.length - 1] == beneficialVault.token(),
      "LydiaFinanceWorker::setRewardPath:: rewardPath must start with lyd and end with beneficialVault token"
    );
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      _rewardPath[0] == lyd && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "LydiaFinanceWorker::setBeneficialVaultConfig:: rewardPath must start with lyd, end with beneficialVault token"
    );
  ...
  /// @dev Deposit lp to masterChef and update pendingLyd
  /// @param _balance - Lp amount to deposit.
  function _masterChefV2Deposit(uint256 _balance) internal {
    uint256 _lydBefore = lyd.balanceOf(address(this));
    masterChefV2.deposit(pid, _balance);
    pendingLyd = pendingLyd.add(lyd.balanceOf(address(this)).sub(_lydBefore));
  }

  /// @dev Withdraw lp from masterChef and update pendingLyd
