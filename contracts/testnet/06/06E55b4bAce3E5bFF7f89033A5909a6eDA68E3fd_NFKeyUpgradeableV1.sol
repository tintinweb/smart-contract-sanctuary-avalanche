...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    address private _affiliateContractAddress;
    uint private _smarterToLevelUp;
    address private _wallet;
    uint private _activeZone;
    ...
    function initialize(address newWhitelistContractAddress, address newSmarterCoinContractAddress, address newAffiliateContractAddress) initializer public {
        __Ownable_init();
        ...
        _affiliateContractAddress = newAffiliateContractAddress;
    ...
    function getIPFS(uint level) public view returns (string memory) {
        return ipfsURIList[level];
    }

    function setIPFS(uint level, string memory uri) public onlyOwner {
        ipfsURIList[level] = uri;
    }
    ...
    function getActiveZone() public view returns (uint) {
        return _activeZone;
    }

    function setActiveZone(uint value) public onlyOwner {
        _activeZone = value;
    }
    ...
    function upgradeKey(uint tokenID) public {
        ...
        string memory tokenUri = getIPFS(level + 1);
    ...
    function mintUniqueTokenTo(address account, string memory affiliate_code) public payable {
        uint level = WhitelistUpgradeableV1(_whitelistContractAddress).getAddressToWhitelist(account);
        if (mintedAddressList[account] || level == 0) level = 1;

        uint zone = WhitelistUpgradeableV1(_whitelistContractAddress).getWhitelistZone(account);

        require(_activeZone >= 5 || _activeZone >= zone, "NFKey: You need to be whitelisted");
        if (zone == 1) require(!mintedAddressList[account] && (level >= 1 && level <= 10), "NFKey: Zone 1 rejection");
        else if (zone == 2) require(!mintedAddressList[account] && (level >= 1 && level <= 3), "NFKey: Zone 2 rejection");
        else if (zone == 3) require(level >= 1 && level <= 10, "NFKey: Zone 3 rejection");
        else if (zone == 4) require(level == 1, "NFKey: Zone 4 rejection");
        else require(level == 1, "NFKey: Zone 5 rejection");

        if (bytes(affiliate_code).length > 0 && (_activeZone == 4 || _activeZone == 5) && Affiliate(_affiliateContractAddress).isActiveAffiliate(affiliate_code)) {
            uint affiliateDiscount = Affiliate(_affiliateContractAddress).getAffiliateDiscount(affiliate_code);
            uint zoneDiscount = getZoneDiscount(msg.sender, zone);

            if (zoneDiscount < affiliateDiscount) {
                address affiliateWallet = Affiliate(_affiliateContractAddress).getAffilationWalletByCode(affiliate_code);
                uint affiliatePaybackPercent = Affiliate(_affiliateContractAddress).getAffiliatePayback(affiliate_code);

                uint affiliateComission = _commission - (_commission *  affiliateDiscount / 100);
                uint affiliatePayback = affiliateComission * affiliatePaybackPercent / 100;
                payable(_wallet).transfer(affiliateComission);
                payable(affiliateWallet).transfer(affiliatePayback);
            } else {
                uint commission1 = getZoneCommission(msg.sender, zone);
                payable(_wallet).transfer(commission1);
            }
        } else {
            uint commission = getZoneCommission(msg.sender, zone);
            payable(_wallet).transfer(commission);
        }

        _tokenIds.increment();
        uint tokenID = _tokenIds.current();
        string memory tokenUri = getIPFS(level);
        ...
        WhitelistUpgradeableV1(_whitelistContractAddress).removeWhitelistZone(msg.sender, zone);
    }
}
...
contract WhitelistUpgradeableV1 is Initializable {
    ...
    // WHITELIST ZONE
    function addWhitelistZones(WhitelistUserZone[] memory userZoneList) public onlyAdmin {
        for (uint i = 0; i < userZoneList.length; i++) {
            addWhitelistZone(userZoneList[i]);
        }
    }

    function removeWhitelistZones(WhitelistUserZone[] memory userZoneList) public onlyAdmin {
        for (uint i = 0; i < userZoneList.length; i++) {
            removeWhitelistZone(userZoneList[i].whitelistAddress, userZoneList[i].zone);
        }
    }

    function getWhitelistZone(address account) public view returns (uint) {
        if (zoneList[account][1]) return 1;
        else if (zoneList[account][2]) return 2;
        else if (zoneList[account][3]) return 3;
        else if (zoneList[account][4]) return 4;
        else return 5;
    }

    function addWhitelistZone(WhitelistUserZone memory newUserZone) public onlyAdmin {
        zoneList[newUserZone.whitelistAddress][newUserZone.zone] = true;
    }
...
contract NFKeyStakingUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function initialize(address newNfkeyContractAddress, address newTresrCoinContractAddress, address newBonusRewardAddressContract) public initializer {
        __Ownable_init();
    ...
    function calcUnlockCost(uint tokenID, uint time) public view returns (uint) {
        ...
        return chestTierDays * level * 125 * chestTier * 25 * 10 ** 13;
    }
...
contract Affiliate is Pausable, Ownable {
    ...
    AffiliateUser[] private affiliateUsers;
    ...
    function deactivateAffiliate(address wallet) public onlyAdmin() {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (address(affiliateUsers[i].wallet) == address(wallet)) {
                affiliateUsers[i].isActive = false;
            }
        }
    }

    function activateAffiliate(address wallet) public onlyAdmin() {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (address(affiliateUsers[i].wallet) == address(wallet)) {
                affiliateUsers[i].isActive = true;
            }
        }
    }
    ...
    function getAffiliateDiscount(string memory affilation_code) public view returns(uint256 discount) {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (keccak256(abi.encode(affiliateUsers[i].code)) == keccak256(abi.encode(affilation_code))) {
                return affiliateUsers[i].discount;
            }
        }
    }

    function isActiveAffiliate(string memory code) public view returns(bool status) {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (keccak256(abi.encode(affiliateUsers[i].code)) == keccak256(abi.encode(code))) {
                return affiliateUsers[i].isActive;
            }
        }
    }
    ...
    function getAffiliatePayback(string memory affilation_code) public view returns(uint256 payback) {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (keccak256(abi.encode(affiliateUsers[i].code)) == keccak256(abi.encode(affilation_code))) {
                return affiliateUsers[i].payback;
            }
        }
    }
    ...
    function addAffiliations(AffiliateUser[] memory newAffiliations) public onlyAdmin() {
        for (uint i = 0; i < newAffiliations.length; i++) {
            affiliateUsers.push(newAffiliations[i]);
        }
    }

    function changeAffiliateDiscount (address wallet, uint newDiscount) public onlyAdmin() {
        require(newDiscount > 0 && newDiscount < 100, "NFKey: wrong discount value");
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (address(affiliateUsers[i].wallet) == address(wallet)) {
                affiliateUsers[i].discount = newDiscount;
            }
        }
    }

    function changeAffiliatePayback (address wallet, uint newPayback) public onlyAdmin() {
        require(newPayback > 0 && newPayback < 100, "NFKey: wrong payback value");
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (address(affiliateUsers[i].wallet) == address(wallet)) {
                affiliateUsers[i].payback = newPayback;
            }
        }
    }
    ...
    function getAffilationWalletByCode(string memory affilation_code) public view returns(address affiliate_wallet) {
        for (uint i = 0; i < affiliateUsers.length; i++) {
            if (keccak256(abi.encode(affiliateUsers[i].code)) == keccak256(abi.encode(affilation_code))) {
                return affiliateUsers[i].wallet;
            }
        }
    }
}
