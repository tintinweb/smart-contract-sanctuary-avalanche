...
contract AvaxGeisha is ERC721, Ownable {
    ...
    constructor() ERC721("Avax geisha", "AG") {
        tokenPrimaryPrice = 1 ether;
    ...
    function mint(uint256 mintAmount_) payable external {
        ...
        // require(startMintTime <= block.timestamp, 'You have to left until 2021.10.22 20:00 UTC');
    ...
    function mintFree(uint256 mintAmount_) onlyOwner external {
        // check if this function caller is not an zero address account
        require(msg.sender != address(0), "can't mint from address(0)");
        ...
        uint256 _reflection = tokenPrimaryPrice.mul(reflection).div(PERCENT100);
        uint256 _tempCounter = nftCounter;

        for (uint i = 0; i < mintAmount_ - 1; i++) {
            _tempCounter ++;
            totalDividend += (_reflection / _tempCounter);
        }
