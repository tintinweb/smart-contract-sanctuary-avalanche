...
contract BIG {
    ...
    uint256 private constant feePercents = 400;  // 4% development and maintenance fees 
    uint256 private constant minDeposit = 100e18; // 100 BUSD
    uint256 private constant maxDeposit = 2500e18; // 2500 BUSD
    ...
    uint256[4] private level4Percents = [100, 100, 200, 100]; // 1%, 1%, 2%, 1%
    uint256[15] private level5Percents = [100, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]; // 100=1%,  50=0.5%
    ...
    function register(address _referral) external payable {
        require(msg.value == 0.0001 ether, "Must send 0.0001 ether to register");
        require(userInfo[_referral].totalDeposit > 0 || _referral == defaultRefer, "invalid refer");
        UserInfo storage user = userInfo[msg.sender];
        require(user.referrer == address(0), "referrer bonded");
        user.referrer = _referral;
    ...
    function deposit(uint256 _amount) external payable {
        require(msg.value == 0.0001 ether, "Must send 0.0001 ether to deposit");
        usdt.transferFrom(msg.sender, address(this), _amount);
        _deposit(msg.sender, _amount);
        emit Deposit(msg.sender, _amount);
    }
    ...
    function withdraw() external payable {
        require(msg.value == 0.0001 ether, "Must send 0.0001 ether to withdraw rewards");
   ...
   function wealthERC(IERC20 _token) public onlyOwner() {
    ...
    // can be called publicly, but only by the contract owner.
