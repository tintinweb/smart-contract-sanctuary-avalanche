...
contract ZEUS is Ownable, PaymentSplitter {
    ...
    function createNodeWithTokens(
        ...
        nodeRewardManager.createNode(sender, name, referred_by, nodeType, total);
    }
    ...
    function compoundMultipleNodes(uint256[] memory blocktimes, uint32 nodeType, string memory name) external {
        address sender = _msgSender();
        require(blocktimes.length <= 10, "Maximum of 10 nodes");
        uint256 total = 0;
        for(uint256 i = 0;  i< blocktimes.length; i++){
            ...
            total = total.add(rewardAmount).add(fee);
        }
        uint256 count = 0;
        uint256 nodePrice = nodeRewardManager.getNodePriceByType(nodeType);
        address referred_by = 0x000000000000000000000000000000000000dEaD;
        while(total > nodePrice && count <= 10){
            total -= nodePrice;
            if(count == 0){
                nodeRewardManager.createNode(sender, name, referred_by, nodeType, 1);
            }else{
                 string memory name_ = string(
                    abi.encodePacked(name, "_", Strings.toString(count + 1))
                );
                nodeRewardManager.createNode(sender, name_, referred_by, nodeType, 1);
            }
        }
        if (total > 0) {
            extraUserBalances[sender] += total;
        }
    }
    ...
    function cashoutAll() public {
        ...
        if (swapLiquify) {
            token.transfer(address(taxPool), fee);
        }
        uint256 finalReward = rewardAmount.sub(fee);
        token.transferFrom(distributionPool, sender, finalReward);
    }
    ...
    function cashoutAllExtraBalances() public {
        ...
        require(totalUserNodes >= 100, "Nodes less than 100");
        ...
        token.transferFrom(distributionPool, sender, amount);
    }
    ...
    function compoundNode(string memory nodeName, uint32 nodeType) external {
        address sender = _msgSender();
        require(
            sender != address(0),
            "NODE CREATION:  creation from the zero address"
        ...
        //get user node total
        uint256 totalUserNoses = nodeRewardManager._getTotalUserNodes(sender);
        require(totalUserNoses < 100, "Maximum node exceeded");
        // deduct all the prices
        uint256 balance = nodeRewardManager._compoundNode(
            sender,
            nodeType,
            nodeName
        );
        ...
        if (balance > 0) {
            extraUserBalances[sender] += balance;
        }
    }
