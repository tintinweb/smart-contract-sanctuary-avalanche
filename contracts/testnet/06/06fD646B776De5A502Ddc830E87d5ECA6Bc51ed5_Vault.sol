...
contract Vault is ERC20, ReentrancyGuard, Pausable {
  ...
  function sharesToValue(uint256 _sharesToWithdraw) public view returns (uint256) {
    ...
    return _sharesToWithdraw * getEquity() / _shareSupply;
  }

  function valueToShares(uint256 _value) public view returns (uint256) {
    return _valueToShares(_value, getEquity());
  }

  function getEquity() public view returns (uint256) {
    (uint256 totalEquityValue) = IManager(config.getManagerAddress()).equityInfo(address(this));
  ...
  function getAsset() public view returns (uint256) {
    ...
    ) = IManager(config.getManagerAddress()).assetInfo(address(this));
    return _tokenAAsset + _tokenBAsset;
  }

  function getDebt() public view returns (uint256) {
    (,, uint256 _tokenADebt, uint256 _tokenBDebt) = IManager(config.getManagerAddress()).debtInfo(address(this));
    return _tokenADebt + _tokenBDebt;
  }

  function getLeverage() public view returns (uint256) {
    // leverage = asset / equity
    return getAsset() * SAFE_MULTIPLIER / getEquity();
  }
  ...
  // TEST
  function test(uint256 _depositAmt, uint256 _minSharesReceive) external nonReentrant returns (uint256, uint256, uint256, uint256) {
    uint256 _currentEquity = getEquity();

    _transferTokenToVault(tokenA, _depositAmt);
    ERC20(tokenA).safeTransfer(config.getManagerAddress(), _depositAmt);

    uint256 _depositValue = priceOracle.getAmountsOut(_depositAmt, tokenA, tokenB, config.getPairAddress());
    uint256 _equityValue = getEquity();
    ...
    return(_depositValue, _equityValue, _targetLpValue, _targetDebtValue);
  }

  // TEST
  function test2(uint256 _depositAmt,
    uint256 _minSharesReceive,
    uint256 _equityBefore
    ) public returns (uint256) {

    // calculate equity change
    uint256 _value = getEquity() - _equityBefore;
  ...
  function deposit(uint256 _depositAmt, uint256 _minSharesReceive) external nonReentrant {
    uint256 _currentEquity = getEquity();
    ...
    _checkAndMint(_depositAmt, _minSharesReceive, _currentEquity);
  }

  function withdraw(uint256 _sharesAmt, uint256 _minTokenAReceive) external nonReentrant {
    require(_sharesAmt > 0, "Quantity must be > 0");
    require(_sharesAmt <= balanceOf(msg.sender), "Amount to burn exceeds user balance");
    ...
    uint256 _equityValue = getEquity();
    uint256 _targetLpValue = (_equityValue - _withdrawValue)
                             * config.getTargetLeverage()
                             / SAFE_MULTIPLIER;

    uint256 _targetDebtValue = (_equityValue - _withdrawValue)
                               * (config.getTargetLeverage() - 1e18)
                               / SAFE_MULTIPLIER;
    ...
    _checkAndTransfer(_minTokenAReceive, _withdrawValue, _currentEquity);
  }

  /* ========== INTERNAL FUNCTIONS ========== */

  function _transferTokenToVault(address _token, uint256 _depositAmt) public {
    /* NOTE for future native token deposits */
  ...
  function _calculate(uint256 _targetDebtValue) public view returns (uint256,
   uint256, uint256, uint256) {

    (,,uint256 _tokenADebt,
    uint256 _tokenBDebt) = IManager(config.getManagerAddress()).debtInfo(address(this));
  ...
  function _execute(uint256 _depositOrWithdraw,
    ...
    IManager(config.getManagerAddress()).work(
      address(this),
      _targetLpValue,
      _borrowA,
      _borrowB,
  ...
  function _moreCalc(uint256 canBorrowValue,
    ...
    uint256 repayB
        ...
        if (borrowA > 0 && borrowB > 0) {
          ...
          _borrowA = borrowA == 0 ? 0 : borrowRepayRatio * canBorrowValue / SAFE_MULTIPLIER;
          _borrowB = borrowB == 0 ? 0 : borrowRepayRatio * canBorrowValue / SAFE_MULTIPLIER;
          _repayA = repayA == 0 ? 0 : canRepayValue * SAFE_MULTIPLIER / borrowRepayRatio;
          _repayB = repayB == 0 ? 0 : canRepayValue * SAFE_MULTIPLIER / borrowRepayRatio;
        }
  ...
  function _checkAndMint(uint256 _depositAmt,
    ...
    emit Deposit(msg.sender, _depositAmt,_sharesToUser);
  }
  ...
  function _valueToShares(uint256 _value, uint256 _currentEquity) public view returns (uint256) {
  ...
  function _getVaultBalance() public view returns (VaultBalance memory) {
    return
      VaultBalance({
  ...
  // function pendingManagementFee() internal view returns (uint256) {
  ...
  function rebalance() external {
    uint256 _equityBefore = getEquity();
    uint256 _debtBefore = getDebt();
    ...
    uint256 _equityAfter = getEquity();
    uint256 _debtAfter = getDebt();
...
interface IManager {
  function assetInfo(address _vault) external view returns (uint256, uint256, uint256, uint256);

  function debtInfo(address _vault) external view returns (uint256, uint256, uint256, uint256);

  function equityInfo(address _vault) external view returns (uint256);
