...
contract OracleSwap {
    ...
    function swap(
        ...
        uint basePrice = convertToUint(currentBasePrice, 18);
        uint quotePrice = convertToUint(currentQuotePrice, 18);
        ...
        if (isBuy) {
            ...
            // Transfer to the user his amount without the fees (the fees stay in the pool so the liquidity providers' tokens owned increase)
    ...
    function arbitrate(
        ...
        uint imbalanced = isImbalanced(pythUpdateData);
    ...
    function isImbalanced(
        ...
        uint targetPrice = ((basePrice * DECIMALS) / quotePrice); // Compute the real price of the pool

        int delta = int(poolPrice()) - int(targetPrice);
        uint spread = delta > 0
    ...
    function getPoolShare(address provider) public view returns (uint) {
        return (liquidityOwned[provider] * DECIMALS) / baseBalance();
    }
