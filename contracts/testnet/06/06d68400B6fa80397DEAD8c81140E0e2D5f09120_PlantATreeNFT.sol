    ...
    IERC721Receiver
{
    ...
    uint256 public _price_Public = 0.25 ether;
    uint256 public _maxMintable = 500;
    string private _customBaseURI;
    uint256 public _royaltyAmount = 1000; //10%
    uint256 public _maxMintPerTx = 3;
    ...
    mapping(address => uint256[]) public tokensOwner;
    uint256 private link;
    address PATNFT_Address = address(this);
    address PlantATree_Address;


    ///////// WL /////////
    uint256 public _price_WL = 0.25 ether;
    mapping(address => bool) public whiteListed;
    bool public isWL = false;

    /// airdrop ///
    uint256 public _maxSupply = 600;
    ...
    // function setBaseUnRevealURI(string memory unrevealdURI_) public onlyOwner {
    //     _unrevealdURI = unrevealdURI_;
    // }
    ...
    function mint(uint256 quantity) public payable {
        ...
        //check if the tokens claimed so you can't mint again
        uint256[] memory tokens = getNFTTokens();
        uint256 tokensAmount = tokens.length;
        bool canMint = true;
        ...
        require(canMint == true, "You can't mint again with this wallet because you already claimed your minted tokens. You must mint with a different wallet");
    ...
    function splitFunds(uint256 quantity) private {
        uint256 mintValue = msg.value * quantity;
        uint256 amountA = (mintValue * 75) / 1000; //7.5%
        uint256 amountB = (mintValue * 75) / 1000;
    ...
    function withdraw() external onlyOwner {
        require(PATNFT_Address.balance > 0, "Nothing to withdraw");
        payable(_msgSender()).transfer(PATNFT_Address.balance);
    }
    ...
    ////// Stake then Claim
    function stakeAndClaim() public {
        //require(claimedWallets[msg.sender] == false, "This wallet already claimed the Avax" );
        uint256[] memory tokens = getNFTTokens();
        uint256 tokensAmount = tokens.length;
        require(tokensAmount > 0, "You don't have NFT tokens to claim");
        ...
        for (uint256 i = 0; i < tokensAmount; i++) {
            ...
            if (claimedTokens[tokenId] != true) {
                canClaim = true;
                //approve(PATNFT_Address, tokenId);
                safeTransferFrom(_msgSender(), PATNFT_Address, tokenId);
                claimedTokens[tokenId] = true;
           }
        }

        require(canClaim == true, "NFT Tokens Already Claimed");

        //save the owner of te NFT tokens on staking
        if (tokensOwner[_msgSender()].length == 0) {
            tokensOwner[_msgSender()] = tokens;
        } else {
            for (uint256 i = 0; i < tokensAmount; i++) {
                tokensOwner[_msgSender()].push(tokens[i]);
            }
        }

        require(PATNFT_Address.balance > 0, "No Balance to Claim");
        uint256 treesAvax = (_price * 40) / 100; //40%
        uint256 avaxValue = treesAvax * tokensAmount;
        payable(msg.sender).transfer(avaxValue);
    }

    function unstake(uint256 _link) public {
        require(_link == link, "Can't unstake NFT tokens" );
        uint256 tokensAmount = tokensOwner[_msgSender()].length;
        require(tokensAmount > 0, "You don't have tokens to Unstake");
        uint256 trees = PlantATree(PlantATree_Address).getTreesSinceLastPlant(msg.sender);
        require(trees > 0 , "You have to plant trees to unstake your NFT tokens.");
        //transfer the tokens to the owner
        uint256[] storage tokens = tokensOwner[_msgSender()];
        for (uint256 i = 0; i < tokensAmount; i++) {
            uint256 tokenId = tokens[i];
            //approve(PATNFT_Address, tokenId);
            IERC721(PATNFT_Address).safeTransferFrom(
                PATNFT_Address,
                _msgSender(),
                tokenId
            );
        }
        //delete the staked tokens
        delete tokensOwner[_msgSender()];
    }

    function ownerOfNFTToken(uint256 _tokenId) public view returns (address) {
        return ERC721(PATNFT_Address).ownerOf(_tokenId);
    }

    //get All NFTs of Owner
    function getNFTTokens() public view returns (uint256[] memory) {
        // uint256[] storage tokens;
        uint256 amount = ERC721.balanceOf(_msgSender());
        uint256[] memory tokens = new uint256[](amount);
        for (uint256 i = 0; i < amount; i++) {
            tokens[i] = ERC721Enumerable(PATNFT_Address).tokenOfOwnerByIndex(
                _msgSender(),
                i
            );
        }
    ...
    //get staked tokens for owner
    function getStakedTokens(address staker)
        public
        view
        returns (uint256[] memory)
    {
        return tokensOwner[staker];
    }
