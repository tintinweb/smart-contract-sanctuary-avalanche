...
/*
    ...
    - Happy Paths
        ...
        4 Multiple Request Non Atomic
        5 Single Request with Ack
        6 Multiple Request with Ack
    - Error Paths
        7 Gas Limit Errors Single Request
        8 Gas Price Errors Single Request
        9 ACK Gas Price Error
        10 ACK Gas Limit Error
        11 Error ACK Single Request
        12 Error ACK
...
contract CrosstalkE2E is AccessControl , ICrossTalkApplication {
    ...
    function handleRequestFromSource(
        ...
        (uint64 reqID, uint8 reqType ,string memory strPayload) = abi.decode(
            payload,
            (uint64, uint8 , string)
        );
        if (reqType == 1 ) {
