...
contract SapphireNode is ERC721Enumerable, Ownable{
    ...
    uint256 constant GATE_1 = 120;
    uint256 constant GATE_2 = 240;
    ...
    function nearestGate() private view returns(uint256) {
        uint256 gateReturn;
        if (supply.current() < GATE_1) {
            gateReturn = GATE_1;
        } else if (supply.current() > GATE_1 && supply.current() < GATE_2) {
    ...
    function mintNode() public payable {
        ...
        require(balanceOf(msg.sender) <= maxNodesPerWallet, "Cannot mint more than `maxNodePerWallet` nodes");
        require(supply.current() <= maxTotalSapphireNodes, "No more sapphire nodes to mint");
        uint256 nodePrice = sapphireNodePrice * DECIMAL_MULTIPLIER;
        require(
           DAIToken.balanceOf(msg.sender) > nodePrice,
           "Insuffucient DAI balance"
        );

        DAIToken.transferFrom(msg.sender, address(this), nodePrice);
        supply.increment();
