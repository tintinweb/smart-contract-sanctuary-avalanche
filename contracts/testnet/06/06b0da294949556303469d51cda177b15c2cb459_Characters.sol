...
contract Characters is ERC721Enumerable, Ownable, Pausable{
    ...
    function claimMints(uint32[] memory orders) external whenNotPaused {
        for (uint32 i; i < orders.length; i++) {
            ...
            require(random.viewOwner(orderId) == msg.sender, "You did not place this order, nor got aprooved to claim it");
            

            if (random.seedReady(orderId)) {
    ...
    function _assignTrait(uint seed) private pure returns (uint8 number) { 
        // NOTE ➡️ Assigns a number between 0 - 3 depending on the last number of seed
        uint8 num = 25;
        for (uint8 i = 0; i < 4; i++) {
            if (num > (seed % 100)) return i;
            else num += 25; 
        }
    }
}
