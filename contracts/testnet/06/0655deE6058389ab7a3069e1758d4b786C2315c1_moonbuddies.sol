...
contract moonbuddies is ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer{
  ...
  function setPrice(uint priceInMilli, uint wlPriceInMilli) public onlyOwner {
  ...
  function publicSale() public view returns (bool){
    bool publicS = (block.timestamp > publicTime);
    return publicS;
  }
  function privateSale() public view returns (bool){
    bool privateS = (block.timestamp > wlTime && block.timestamp < publicTime);
    return privateS;
  }
  function mint(uint quantity) public payable {
    if(privateSale()) {
      require(quantity < whitelistMints[msg.sender], "not whitelisted or has already minted");
    ...
    else if(publicSale()){publicMints[msg.sender] += quantity;}
  }
