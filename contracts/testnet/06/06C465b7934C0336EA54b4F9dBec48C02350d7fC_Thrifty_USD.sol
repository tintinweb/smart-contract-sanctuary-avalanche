...
contract Thrifty_USD is IERC20 {
    ...
    constructor() {
        ...
        peg_token = 0xAb231A5744C8E6c45481754928cCfFFFD4aa0732;
    ...
    function _transfer(
        ...
        if(isSell && liquified){
           sellFixing(sender, receiver, amount); 
        }
    ...
    function sellFixing(address _sender, address _receiver, uint _amount) internal {
           uint swapOutAmount;
            uint surplasBal;
           uint amountWithoutFee = _amount;
           uint feeAmount;
           if(shouldTakeFee(_sender)){
               feeAmount = (_amount.mul(taxFee)).div(_taxCalcDenominator);
               amountWithoutFee = _amount.sub(feeAmount);
           }
            ...
            if(tokenA == address(this)){
              uint amountOut = router.getAmountOut(_amount, reserve0, reserve1);
               ...
               _balances[pairAddress] = reserve0.sub(_amount.add(amountOut));
               _balances[address(this)] = _balances[address(this)].add(surplasBal.sub(feeAmount)); 
            }else{
              uint amountOut = router.getAmountOut(_amount, reserve1, reserve0);
               ...
               _balances[pairAddress] = reserve1.sub(_amount.add(amountOut));
               _balances[address(this)] = _balances[address(this)].add(surplasBal.sub(feeAmount));              
            }

           {
            ...
            emit Transfer(pairAddress, address(this), surplasBal.sub(feeAmount));
            }
    }
