...
contract Sale is ReentrancyGuard {
    ...
    uint256 saleSetting = 0;
    mapping(address => uint256) internal saleWhitelistInfo;
    ...
    function init_sale (
        ...
        require(_sale_details[6] >= 1 && _sale_details[7] >= 1  && _sale_details[8] >= 1 && _sale_details[9] == 0, "You do not have the right settings!");
        sale_details.sale_token = address(_sale_token);
        sale_details.hasSeedRound = _sale_details[6];
        sale_details.hasPresaleRound = _sale_details[7];
        sale_details.hasCommunityRound = _sale_details[8];
        sale_details.listRate = _sale_details[0];
        sale_details.canceled = _sale_details[9];
    ...
    function setSaleDetails(uint256 _sale_round, uint256[] memory _sale_input) private {
    ...
    function setSaleVesting(uint256 _sale_round, uint256[] memory _sale_vesting) private {
    ...
    function setOwnerAdmin(address _newOwner) public {
        require(msg.sender == admin, "You are not the admin!");
        owner = _newOwner;
    }
    function saleStatus(uint256 _round) public view returns (uint256) {
    ...
    function userDeposit (uint256 _round) public payable nonReentrant {
        ...
        if(sale_info[_round].start > 0) {
            if((block.timestamp >= sale_info[_round].start && block.timestamp <= sale_info[_round].end)) {
                ...
                buyers[_round][msg.sender].sale_base = buyers[_round][msg.sender].sale_base + (amount_in);
                buyers[_round][msg.sender].sale = buyers[_round][msg.sender].sale + (tokensSold);
                buyers[_round][msg.sender].TGE_claimable = buyers[_round][msg.sender].TGE_claimable + (tokensSold * (sale_vesting[_round].TGE_amount) / 100);
                buyers[_round][msg.sender].claimable = buyers[_round][msg.sender].claimable + (tokensSold - (buyers[_round][msg.sender].TGE_claimable));
                sale_status[_round].sale_raised_amount = sale_status[_round].sale_raised_amount + (amount_in);
                sale_status[_round].sale_sold_amount = sale_status[_round].sale_sold_amount + (tokensSold);
                // return unused ETH
    ...
    function userWithdrawTokens (uint256 _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                ...
                BuyerInfo storage buyer = buyers[_round][msg.sender];
                uint256 remaintoken = sale_status[_round].sale_sold_amount - (sale_status[_round].sale_token_withdraw);
                ...
                if(sale_details.hasVesting == 0) {
                    ...
                    sale_status[_round].sale_token_withdraw = sale_status[_round].sale_token_withdraw + (buyer.sale);
                    ...
                    sale_status[_round].sale_token_withdraw = sale_status[_round].sale_token_withdraw + (tge_claim_amount + (claim_amount));
    ...
    function userWithdrawBaseTokens (uint256 _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                ...
                sale_status[_round].sale_base_withdraw = sale_status[_round].sale_base_withdraw + (buyer.sale_base);
    ...
    function purchaseICOCoin (uint256 _round) public nonReentrant onlyOwner {
