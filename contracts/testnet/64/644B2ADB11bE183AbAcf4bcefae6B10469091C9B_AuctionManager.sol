...
// File: contracts/remixAuction.sol
...
contract AuctionManager {
    ...
    mapping(address => uint) private collections; // collections

    mapping(address => Collection) private NFTcollections; // collection ids
...
// constructor
    ...
    function createAuction(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external returns (bool){
        ...
        if (collections[_nftAddress] == 0){

            collectionID += 1;

            collections[_nftAddress] = collectionID;

            NFTcollections[_nftAddress] = Collection(new uint[](0),new address[](0));

        }

        collectionID = collections[_nftAddress];

        uint auctionId = _auctionIdCounter; // get the current value of the counter

        // check if the collection already has the token

        if (NFTcollections[_nftAddress].ids.length == 0){

            NFTcollections[_nftAddress].ids.push(_tokenId);

        }

        else{

            for (uint i = 0; i < NFTcollections[_nftAddress].ids.length; i++){

                if (NFTcollections[_nftAddress].ids[i] == _tokenId){
        ...
        NFTcollections[_nftAddress].colAuctions.push(address(auction));
    ...
    function getOneNFT(address _NFT, uint _tokenId) public view returns(            
        ...
        for (uint i = 0; i < collectionID; i++){

            if (NFTcollections[_NFT].ids[i] != _tokenId){
                ...
                // get auction info for token id 

                _tkauction = Auction(NFTcollections[_NFT].colAuctions[i]);  
        ...
        return (
            ...
            _tkauction.startPrice(),

            uint(_tkauction.getAuctionState())));

    }

   // get all for sale for collection

    function collectionGetAllForSale(address _NFT) external view returns(info[] memory _auctionsList) {

        // get auctions for all token ids

        info[] memory _auctionInfo = new info[](NFTcollections[_NFT].ids.length);

        for (uint i = 0; i < NFTcollections[_NFT].ids.length; i++){

            _auctionInfo[i] = getOneNFT(_NFT, NFTcollections[_NFT].ids[i]);
...

    {
        ...
        return ( // return the arrays
            ...
            Auction(_addy).directBuyPrice(),

            Auction(_addy).creator(),

            Auction(_addy).maxBid(),

            Auction(_addy).tokenId(),

            Auction(_addy).endTime(),

            Auction(_addy).startPrice(),

            uint(Auction(_addy).getAuctionState())
