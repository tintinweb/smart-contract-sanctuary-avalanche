...
// swapp awax with any token
   function swapAVAXForExactTokens(address[] calldata _path)
        external
        payable
        returns (uint[] memory amounts){
    ...
    function swapTokensForExactTokens(uint _amountIn, address[] calldata _path)
        external
        returns (uint[] memory amounts){
        ...
        uint[] memory calculations = getAmountsOut(_amountIn,_path);
        amounts = router.swapTokensForExactTokens(_amountIn,calculations[2],_path,msg.sender,block.timestamp + 3 minutes);
        uint tax = calculateTax(calculations[1]);
        Dai.transfer(msg.sender,calculations[1]-tax);
        Dai.transfer(vault,tax);
        }
