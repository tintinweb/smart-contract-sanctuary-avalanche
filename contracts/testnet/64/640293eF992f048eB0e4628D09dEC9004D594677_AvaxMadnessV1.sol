...
contract AvaxMadnessV1 {
	...
	uint256 constant public MAX_HOLD_PERCENT = 15; // 1.5 Max Holding Bonus
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(14, 10)); // 1% per day for 14 days
        plans.push(Plan(21, 13)); // 1.3% per day for 21 days
        plans.push(Plan(28, 16)); // 1.6% per day for 28 days
		plans.push(Plan(14, 20)); // 2% per day for 14 days (at the end)
        plans.push(Plan(21, 26)); // 2.6% per day for 21 days (at the end)
        plans.push(Plan(28, 34)); // 3.4% per day for 28 days (at the end)
	}
    ...
    function forceWithdraw(uint256 index) public {
        ...
        require(user.deposits[index].plan > 1, "unable to withdraw deposit");
