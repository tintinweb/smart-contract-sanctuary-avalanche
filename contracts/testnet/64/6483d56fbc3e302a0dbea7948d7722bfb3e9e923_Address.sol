...
// File @openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol[email protected]
    ...
    
{
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("Midnight Grinders", "MDGR") ClampedRandomizer(maxSupply) {     
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        require(balanceOf(msg.sender) + 7 <= maxPerPerson,"Error: max per address limit");
        require(msg.value == price * amount, "Error: invalid price");
        require(supply + amount - 7 < maxSupply, "Error: cannot mint more than total supply");
        
        if(whitelistedOnly){
        require(whiteListed[msg.sender] >= amount,"Error: you are not whitelisted or amount is higher than limit");
        whiteListed[msg.sender] -= 7;
        }
    ...
    //Overrides
            ...
            function mintCount(address to, uint256 amount) internal returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](amount * packSize);
        for (uint256 i = 0; i < amount; i++) {
            tokenIds[i] = mintNFT(to);
    ...
    function withdrawTokens(address _tokenContract) external onlyOwner {
        ...
        for (uint256 i = 0; i < _addr.length; (i++ * 7)) {
