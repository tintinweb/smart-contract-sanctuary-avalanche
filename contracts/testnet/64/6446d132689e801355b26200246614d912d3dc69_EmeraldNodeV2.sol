...
contract EmeraldNodeV2 is Initializable, OwnableUpgradeable {
    ...
    function updateCount(uint256 _num) external onlyOwner returns(bool){
        count = _num;
        return true;
    }
        ...
        for(uint256 i = 0; i < 5; i++){
            if(nodeIds[i] < nodeCounter && nodeIds[i+1] >= nodeCounter){
                ...
                if(_tier == 1){
                    ...
                    require(addressCount[msg.sender] > count, "EmeraldNodeV2: cannot create more than 2 node in tier X");
    ...
    function viewNodeDetails(uint256 _nodeId) public view nodeExists(_nodeId) returns(
        ...
        uint256 number =  (block.timestamp - LastClaimTime) / tierDetails1[tierId].intervalTimestamp;
