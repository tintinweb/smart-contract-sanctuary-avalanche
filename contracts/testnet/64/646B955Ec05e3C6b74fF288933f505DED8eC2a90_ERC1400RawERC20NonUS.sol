...
contract CertificateController {
    ...
    bool public certificateControllerActivated;
    IAkruNFTWhitelist iwhitelist;
    ...
    modifier onlyAKRUAdmin()  {
        require(iwhitelist.isOwner(msg.sender) ||
          iwhitelist.isAdmin(msg.sender) ||
          iwhitelist.isSubSuperAdmin(msg.sender)
    ...
    modifier isValidCertificate(bytes memory data) {
        if (
            ...
            (iwhitelist.isWhitelistedUser(msg.sender) > 132)
        ) {
            require(_checkCert(data), "54"); 
            checkNonce[msg.sender] += 1;
        }
        _;
    }
    constructor(address certificateSigner_, bool activated,IAkruNFTWhitelist whitelist) {
        certificateSigners[certificateSigner_] = true;
        certificateControllerActivated = activated;
    ...
    function setCertificateSigner(
        ...
        certificateSigners[operator] = authorized;
    }
    /**
    ...
    function toggleCertificateController(bool isActive) public onlyAKRUAdmin {
        certificateControllerActivated = isActive;
    }
...
import "../ERC20/IERC1400RawERC20.sol";
...
contract ERC1400Dividends {
    ...
    address[]  investors;
    uint256 public taxAmount = 30;
    address private taxAddress;
    bool public taxFlag = false;
    mapping(address => bool) dividends; 
    mapping (address => uint256) public exchangeBalance;
    ...
    modifier onlyAdmin() virtual {
     ...
     modifier onlyAkruUser(){
        require(whitelist_.isWhitelistedUser(msg.sender) <= 200, 
                "ST18");
        _;
    }
    /**
    ...
    function distributeDividends(
        ...
        if(taxFlag)
         {     
          uint256 _feePercentage = (_dividends * taxAmount)/100;
          _dividends = _dividends - _feePercentage;
          bool isTaxTransfer = IERC20(_token).transfer(taxAddress, _feePercentage);
          require(isTaxTransfer);
         // emit feeTransfered(feeAddress,_feePercentage );//update
         }
        for (uint256 i = 0; i < investors.length; i++) {
           if(investors[i] != reserveWalletNonUS){
    ...
    function newMarginLoanContract(IMarginLoan _ImarginLoan) public onlyAdmin{
      ...
      marginLoan_ = _ImarginLoan;
         ...
         /**
     * @dev get alldata of the user on which distribution taken place
...
function getAllData() public view onlyAdmin returns(address[] memory _investors, uint256[] memory _balances) {
  ...
  // Change set fees 
   function setNonUSSTOFeeInfo(address _taxAddress,uint256 _tax,bool taxStatus) public onlyAdmin{
        taxAddress = _taxAddress;
        taxAmount = _tax;
        taxFlag = taxStatus;
    }

    function enableTax(bool taxStatus) public onlyAdmin {
        taxFlag = taxStatus;
    }
}
    ...
    ERC1400Dividends
{
    ...
    function _issue(
        ...
        require(whitelistStatus <= 200, "ST14");
        require(_totalSupply + (value) <= _cap, "ST22");
        _totalSupply = _totalSupply + (value);
        _balances[to] = _balances[to] + (value);
        addInvestor(to);
     ...
     */
    function setCap(uint256 propertyCap) public {   
        require(whitelist_.isWhitelistedUser(tx.origin) <= 201,"ST0");
 ...
 * @author AKRU's Dev team
 */
contract ERC1400RawERC20NonUS is IERC20, ERC1400RawIssuable {
    ...
    modifier onlyAdmin() override(ERC1400Raw) {
        require(
            whitelist_.isWhitelistedUser(msg.sender) < 132,
            "ST0"
        );
        _;
    }
