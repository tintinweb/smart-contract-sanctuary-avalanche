...
contract RaffleContract is Ownable, IERC721Receiver {
    ...
    function createRaffleNFT(
        ...
        require(
            IERC721(NFT).getApproved(rafflePrizeTokenId_) == address(this),
            'Contract not approved'
        );
        require(
        ...
        returns (Raffle[] memory)
    {
        ...
        require(totalRaffleHistory > 0, 'No cancelled and completed exist');
        uint256 correctLimit;
        ...
        for (uint256 i = raffleCount; i > 0 || currentIndex < limit; i--) {
            if (
                raffles[i + 1].status == RaffleStatus.Cancelled ||
                raffles[i + 1].status == RaffleStatus.Completed
            ) {
                uint256 currentId = i;
                Raffle storage currentRaffle = raffles[currentId];
    ...
    function purchaseTicket(uint256 id, uint256 quantity) public {
        require(id <= _raffleId.current() && id > 0, 'Raffle id not exist');
        Raffle storage raffle = raffles[id];
        require(block.timestamp < raffle.endIn, 'Raffle is ended');
    ...
    function claim(uint256 id) public {
        ...
        require(raffle.isPaid == PaidStatus.No, 'You are already paid');
        if (raffle.prizeType == PrizeTypes.Tokens) {
