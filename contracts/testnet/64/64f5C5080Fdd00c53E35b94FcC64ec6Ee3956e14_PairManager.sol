...
contract PairManager is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function setRTotal(uint256 _id, uint64 _counter, uint256 _amount) public onlyRole(PAIRCR_MANAGER) {
        pairsCR[_id][_counter].rTotal = _amount;
    }

    function setFTotal(uint256 _id, uint64 _counter, uint256 _amount) public onlyRole(PAIRCR_MANAGER) {
    ...
    function batchGrantRole(bytes32[] memory _roles, address[] memory _accounts ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_roles.length == _accounts.length, "Invalid length");
