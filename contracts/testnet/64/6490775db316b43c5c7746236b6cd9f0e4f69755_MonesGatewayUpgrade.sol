...
// File contracts/Gateway/VerifierUpgradeable.sol
...
// File contracts/Gateway/MonesGateway.sol
...
//
    ...
    function setupCooldownDuration(uint256 newCooldown) external onlyOwner {
        cooldownDuration = newCooldown;
        emit CooldownUpdated(newCooldown);
    }

    function depositSingle(address nftAddress, uint256 nftId, uint256 amount, bytes memory data) public {
    ...
    function depositBatch(address[] memory nftAddress, uint256[] memory nftId, uint256[] memory amount, bytes memory data) public {
    ...
    function claimERC20(address tokenAddress, uint256 amount, uint256 nonce, bytes memory signature) external {
    ...
    function withdrawERC721(address tokenAddress, uint256 tokenId, uint256 nonce, bytes memory signature) external {
    ...
    function upgradeAndWithdrawERC721(address tokenAddress, uint256 tokenId, uint256 heroId, uint8 rarity, uint8 star, uint8 level, uint8 enhancement, uint256 nonce, bytes memory signature) external {
    ...
    function claimHero(address tokenAddress, uint256 heroId, uint256 nonce, bytes memory signature) external {
    ...
    function claimEquipment(address tokenAddress, uint256 equipmentId, uint256 nonce, bytes memory signature) external {
    ...
    function withdrawERC1155(address tokenAddress, uint256 tokenId, uint256 tokenAmount, uint256 nonce, bytes memory signature) external {
    ...
    function claimMaterial(address tokenAddress, uint256 tokenId, uint256 tokenAmount, uint256 nonce, bytes memory signature) external {
        require(verifyERC1155(msg.sender, tokenAddress, tokenId, tokenAmount, IS_NEW, nonce, signature), "MonesGW: Wrong signature!");
        require(!isExec[msg.sender][nonce], "MonesGW: Duplicate nonce!");
        require(lastMove[msg.sender] + cooldownDuration < block.timestamp, "MonesGW: Insufficient cooldown!");
        lastMove[msg.sender] = block.timestamp;
