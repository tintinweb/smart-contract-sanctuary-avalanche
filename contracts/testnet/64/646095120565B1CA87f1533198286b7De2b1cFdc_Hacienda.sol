...
contract Hacienda is HaciendaProgression, ReentrancyGuard {
    ...
    // Constants
    uint256 public constant YIELD_PPS = 16666666666666667; // cocaine produced per second per unit of yield
    ...
    mapping(address => uint256[2]) private numberOfFarmers; // address => [number of regular farmers, number of master farmers]
    ...
    function _getMasterFarmerNumber(address _owner) internal view returns (uint256) {
     ...
     */
     function restingTimeCalculation(uint256 _farmerType, uint256 _masterFarmerType, uint256 _fatigue) public pure returns (uint256) {
        uint256 maxTime = 43200; //12*60*60
        if( _farmerType == _masterFarmerType){
            maxTime = maxTime / 2; // master farmers rest half of the time of regular farmers
        }
    ...
    function getRestingTime(uint256 _tokenId, bool checkOwnership) public view returns (uint256) {
        ...
        return restingTimeCalculation(farmer.getType(_tokenId), farmer.SICARIO_TYPE(), getFatigueAccruedForFarmer(_tokenId, false));
    }
    ...
    function getCocaineAccruedForManyFarmers(uint256[] calldata _tokenIds) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            output[i] = _getCocaineAccruedForFarmer(_tokenIds[i], false);
        }
        return output;
    }
     ...
     */
     function cocaineAccruedCalculation(uint256 _initialCocaine, uint256 _deltaTime, uint256 _ppm, uint256 _modifier, uint256 _fatigue, uint256 _fatiguePerMinute) public pure returns (uint256) {
    ...
    function _getCocaineAccruedForFarmer(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
        StakedFarmer memory stakedFarmer = stakedFarmers[_tokenId];
        ...
        uint256 ppm = farmer.getYield(_tokenId);
        ...
        uint256 masterFarmerSkillModifier = getSicarioSkillModifier(owner, _getMasterFarmerNumber(owner));

        uint256 delta = endTimestamp - stakedFarmer.startTimestamp;

        return cocaineAccruedCalculation(farmerCocaine[_tokenId], delta, ppm, masterFarmerSkillModifier, farmerFatigueLastUpdate, getFatiguePerMinuteWithModifier(owner));
    }
    ...
    function _updateState(address _owner) internal {
        ...
        for (uint256 i = 0; i < farmerBalance; i++) {
            ...
            if (stakedFarmer.staked && block.timestamp > stakedFarmer.startTimestamp) {
                farmerCocaine[tokenId] = _getCocaineAccruedForFarmer(tokenId, false);
    ...
    function _claimCocaine(address _owner) internal {
        ...
        for (uint256 i = 0; i < farmerBalance; i++) {
            ...
            totalClaimed += _getCocaineAccruedForFarmer(farmerId, true); // also checks that msg.sender owns this token
        ...
        uint256 taxAmountFreezer = totalClaimed * (CLAIM_COCAINE_CONTRIBUTION_PERCENTAGE - drugMulesSkillModifier) / 100;
        ...
        cocaine.mint(drugMulesAddress, taxAmountFreezer);
    }
    ...
    function unstakeFarmersAndUpgrades(uint256[] calldata _farmerIds, uint256[] calldata _upgradeIds) public nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < _farmerIds.length; i++) { //unstake farmers
            ...
            if(farmer.getType(farmerId) == farmer.SICARIO_TYPE()){
    ...
    function reStakeRestedFarmers(uint256[] calldata _farmerIds) public nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < _farmerIds.length; i++) { //stakes farmer
            ...
            if(farmer.getType(_farmerId) == farmer.SICARIO_TYPE()){
                numberOfFarmers[owner][1]++; 
            } else {
                numberOfFarmers[owner][0]++; 
            }
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = StakedFarmerInfo({
                ...
                cocaine: _getCocaineAccruedForFarmer(farmerId, false),
...
contract Upgrade is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(Cocaine _cocaine, Diesel _diesel, string memory _BASE_URI) ERC721("Cartel Game Farmer Tools", "COCAINE-GAME-FARMER-TOOL") {
        ...
        levels[0] = Level({ supply: 0, maxSupply: 2500, priceCocaine: 3000 * 1e18, priceDiesel: 50 * 1e18, yield: 1 });
        levels[1] = Level({ supply: 0, maxSupply: 2200, priceCocaine: 10000 * 1e18, priceDiesel: 80 * 1e18, yield: 3 });
        levels[2] = Level({ supply: 0, maxSupply: 2000, priceCocaine: 20000 * 1e18, priceDiesel: 110 * 1e18, yield: 5 });
        currentLevelIndex = 2;
    }
...
contract HaciendaProgression is Context, Ownable, Pausable {
    ...
    // Constants
    uint256[20] public DIESEL_LEVELS = [0, 100 * 1e18, 250 * 1e18, 450 * 1e18, 700 * 1e18, 1000 * 1e18, 1350 * 1e18, 1750 * 1e18, 2200 * 1e18, 2700 * 1e18, 3250 * 1e18, 3850 * 1e18, 4500 * 1e18, 5200 * 1e18, 5950 * 1e18, 6750 * 1e18, 7600 * 1e18, 8500 * 1e18, 9450 * 1e18, 10450 * 1e18];
    ...
    uint256 public constant SICARIO_ID = 3;
    ...
    */
    ...
    /**
    * Returns the multiplier for $COCAINE production based on the number of sicarios and the skill points spent
    */
    function getSicarioSkillModifier(address _owner, uint256 _sicarioNumber) public view returns (uint256) {
        uint256 sicarioSkill = skillsLearned[_owner][SICARIO_ID];

        if(sicarioSkill == 2 && _sicarioNumber >= 5){
            return 110;
        } else if (sicarioSkill >= 1 && _sicarioNumber >= 2){
            return 103;
        } else {
            return 100;
        }
    ...
    */
    function resetSkills() external whenNotPaused {
        ...
        skillsLearned[_msgSender()][SICARIO_ID] = 0;
...
contract Farmer is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public constant WHITELIST_FARMERS = 1050; 
    uint256 public constant FARMERS_PER_COCAINE_MINT_LEVEL = 10000;

    uint256 public constant MAXIMUM_MINTS_PER_WHITELIST_ADDRESS = 3;
    ...
    uint256 public constant SICARIO_TYPE = 2;

    uint256 public constant FARMER_YIELD = 1;
    uint256 public constant SICARIO_YIELD = 3;
    ...
    uint256 public currentCOCAINEMintCost = 20_000 * 1e18;
    ...
    constructor(Cocaine _cocaine, address _farmerTypeOracleAddress, string memory _BASE_URI) ERC721("Cartel Game Farmers", "CARTEL-GAME-FARMER") {
        ...
        typeYields[SICARIO_TYPE] = SICARIO_YIELD;
    }
    ...
    function setFarmerType(uint256 tokenId, uint256 farmerType) external {
        ...
        require(farmerType == FARMER_TYPE || farmerType == SICARIO_TYPE, "invalid farmer type");
    ...
    function mintPromotional(uint256 qty, uint256 farmerType, address target) external onlyOwner {
        ...
        require(farmerType == FARMER_TYPE || farmerType == SICARIO_TYPE, "invalid chef type");
    ...
    function mintFarmerWithCOCAINE(uint256 qty) external whenNotPaused {
        ...
        if(farmersMintedWithCOCAINE <= FARMERS_PER_COCAINE_MINT_LEVEL && farmersMintedWithCOCAINE + qty > FARMERS_PER_COCAINE_MINT_LEVEL) {
            currentCOCAINEMintCost = currentCOCAINEMintCost * 2;
        }
