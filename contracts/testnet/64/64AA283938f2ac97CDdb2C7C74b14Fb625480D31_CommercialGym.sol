...
//
contract CommercialGym is Ownable {
    using SafeMath for uint256;

    error IsPaused();
    error InvalidInput();

    uint256 public gainsPerSecond = 1157407400000000;
    bool public paused;
    GymJunkie public gymJunkie;
    Gains public gains;
    
    mapping(address => uint256) public unclaimedGains;
    mapping(address => uint256) public stakedNFTSCount;
    mapping(address => mapping(uint256 => uint256)) public stakedNFTS;
    mapping(uint256 => uint256) public stakedNFTSIndex;
    mapping(address => uint256) public lastUpdate;

    constructor(GymJunkie _gymJunkie, Gains _gains) {
        gymJunkie = _gymJunkie;
        gains = _gains;
    }

    // WRITE FUNCTIONS

    function stakeGymJunkies(uint256[] calldata tokenIds) external {
        _isNotPaused();
        _countOutstandingGains(msg.sender);

        if (tokenIds.length == 0) revert InvalidInput();

        for(uint i=0; i < tokenIds.length; i++) {
            _stakeNFTToUser(msg.sender, tokenIds[i]);
            gymJunkie.transferFrom(msg.sender, address(this), tokenIds[i]);
        }
    }

    function unstakeGymJunkies(uint256[] calldata tokenIds) external {
        _isNotPaused();
        _countOutstandingGains(msg.sender);
        ...
        for(uint i=0; i < tokenIds.length; i++) {
            _unstakeNFTFromUser(msg.sender, tokenIds[i]);
            gymJunkie.transferFrom(address(this), msg.sender, tokenIds[i]);
        }
    }
    ...
    function claimGains() external {
        _isNotPaused();
        _countOutstandingGains(msg.sender);
        uint256 amount = getClaimableGains(msg.sender);
        unclaimedGains[msg.sender] = 0;
        gains.mint(msg.sender, amount);
    }

    // VIEW FUNCTIONS

    function getClaimableGains(address _user) public view returns (uint256) {
        return _getUncountedgains(_user) + unclaimedGains[_user];
    }

    function getUserStakedNFTS(address _user) public view returns (uint256[] memory myNFTS) {
        uint256 amount = stakedNFTSCount[_user];
        myNFTS = new uint256[](amount);

        for(uint i=0; i < amount; i++) {
            myNFTS[i] = tokenOfOwnerByIndex(_user, i);
        }
    }

    // INTERNAL FUNCTIONS

    function _countOutstandingGains(address _user) private {
        unclaimedGains[_user] += _getUncountedgains(_user);
        lastUpdate[_user] = block.timestamp;
    }

    function _getUncountedgains(address _user) private view returns (uint256) {
        return (block.timestamp - lastUpdate[_user])*stakedNFTSCount[_user]*gainsPerSecond;
    }

    function tokenOfOwnerByIndex(address _owner, uint256 _index) private view returns (uint256) {
        require(_index < stakedNFTSCount[_owner], "owner index out of bounds");
        return stakedNFTS[_owner][_index];
    }

    function _stakeNFTToUser(address _to, uint256 _tokenId) private {
        uint256 length = stakedNFTSCount[_to];
        stakedNFTS[_to][length] = _tokenId;
        stakedNFTSIndex[_tokenId] = length;
        ++stakedNFTSCount[_to];
    }

    function _unstakeNFTFromUser(address _from, uint256 _tokenId) private {
        // To prevent a gap in from's tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).

        uint256 lastTokenIndex = stakedNFTSCount[_from] - 1;
        uint256 tokenIndex = stakedNFTSIndex[_tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = stakedNFTS[_from][lastTokenIndex];

            stakedNFTS[_from][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            stakedNFTSIndex[lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete stakedNFTSIndex[_tokenId];
        delete stakedNFTS[_from][lastTokenIndex];
        --stakedNFTSCount[_from];
    }
    ...
    function setgainsPerSecond(uint256 _gainsPerSecond) external onlyOwner {
