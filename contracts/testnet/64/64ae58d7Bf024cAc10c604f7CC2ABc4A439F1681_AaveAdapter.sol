...
contract AaveAdapter is AdapterBase, Basic, Ownable {
        ...
        onlyAdapterManager
    {
        ...
        if (token == avaxAddr) {
            ...
            pullAndApprove(token, account, aavePoolAddr, amount);
        ...
        onlyAdapterManager
    {
        ...
        if (token == avaxAddr) {
            ...
            IWAVAXGateway wavaxGateway = IWAVAXGateway(wavaxGatewayAddr);
            wavaxGateway.withdrawETH(aavePoolAddr, amount, account);
        } else {
            pullAndApprove(atoken, account, aavePoolAddr, amount);
    ...
    function borrow(bytes calldata encodedData) external onlyProxy {
        (address token, uint256 amount, uint256 rateMode) = abi.decode(
            encodedData,
    ...
    function borrowTokens(bytes calldata encodedData) external onlyProxy {
        (address[] memory token, uint256[] memory amount) = abi.decode(
            encodedData,
            (address[], uint256[])
        );
    ...
    function payback(bytes calldata encodedData) external onlyProxy {
        (address tokenAddr, uint256 amount, uint256 rateMode) = abi.decode(
            encodedData,
            (address, uint256, uint256)
        );
    ...
    function getReward(bytes calldata encodedData) external onlyProxy {
        (address[] memory assertAddress, uint256 amount) = abi.decode(
            encodedData,
            (address[], uint256)
        );
    ...
    function exchangeDebt(bytes calldata encodedData) external onlyProxy {
        (address user, address account, address collateralPool) = abi.decode(
            encodedData,
