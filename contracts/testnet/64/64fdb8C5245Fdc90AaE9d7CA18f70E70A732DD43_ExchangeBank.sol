...
contract ExchangeBank is IExchange, ReentrancyGuard {
    ...
    address borrower;   // loanee of pledged loan
    ...
    uint256 totalLoanAmount;   // total loan of user
    ...
    function sellTokens(uint256 _price) nonReentrant public onlyAdminSeller {
        ...
        require(
            _price > 0 && _tokensAmount > 0,
            "Price and Quantity must be greater than zero"
        );
        price = _price;
        totalQty = _tokensAmount;
        remainingQty = _tokensAmount;
        ...
        emit TokensSold(seller, address(token), _tokensAmount, _price);
    }
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        if(remainingLoanAmount < buyerPay){
          ...
          uint256 borrowerReceive = buyerPay - remainingLoanAmount;
          stableCoin.transferFrom(_wallet, seller, remainingLoanAmount);
    ...
    */
    ...
    function acceptCounter(address _buyerWallet) nonReentrant public {
        ...
        if(remainingLoanAmount < totalAmount){
            ...
            uint256 borrowerReceive = totalAmount - remainingLoanAmount;
            stableCoin.transfer(seller, remainingLoanAmount);
...
//             uint256[] memory interestRates,
//             uint256[] memory createdAts
//         ) = _IMarginLoan.getTotalLoanOfToken(loanee, token);


//         (uint256[] memory ids,uint256[] memory loans) = _IMarginLoan
//             .getTotalNoOfTokens(loanee, token);
//         uint256 loanAmountWithInterset;
//         uint256 len = banks.length;
//         if (len > 0) {
//             for (uint256 i = 0; i < len;) {
//                 //if (loans[i] <= totalAmount) {
//                     if(banks[i] == sellerBank){
                    
//                      loanAmountWithInterset = loanAmounts[i] + ((loanAmounts[i] * interestRates[i] * 90/ 360 )/100);
//                     remainingAmount = totalAmount -loanAmountWithInterset;
                    
//                     emit Log(totalAmount,remainingAmount,len,loanAmountWithInterset,"first");
                    
//                     stableCoin.transferFrom(buyer, sellerBank, loanAmountWithInterset);
//                     _IMarginLoan.updateLoan(loanee, ids[i], 0, 1);
//                     totalAmount  = remainingAmount;
...
//                     // stableCoin.transferFrom(buyer, sellerBank, loanAmountWithInterset);
//                     stableCoin.transferFrom(buyer, sellerBank,loanAmounts[i]);
//                     _IMarginLoan.updateLoan(loanee, ids[i], 0, 1);
//                     totalAmount  =remainingAmount ;
//                 } else if (loanAmounts[i] > totalAmount) {
               
//                     uint256 remainingLoan = loanAmounts[i] - ;
//                     emit Log(remainingLoan,totalAmount,loanAmounts[i],len,"second");

                     
//                     stableCoin.transferFrom(buyer, sellerBank, totalAmount);
