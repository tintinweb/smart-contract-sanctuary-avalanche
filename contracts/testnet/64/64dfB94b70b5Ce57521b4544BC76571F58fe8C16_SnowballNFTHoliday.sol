...
contract SnowballNFTHoliday is ERC721, Ownable{
    address unclaimedNFT = 0x7d6B14A3ef0eB821f346FD446ebd56cF0575B7Aa;
    ...
    function mintAndBurn(uint256 _qty) public {
        require(_qty > 0, "!qty");

        address msgSender = _msgSender();
        uint256 userBalance = IERC721(unclaimedNFT).balanceOf(msgSender);

        require(userBalance > 0, "Unclaimed NFT not found");
        require(userBalance > _qty, "Not enough NFTs");

        uint256[] memory userTokenList = this.tokensOfOwner(msgSender);

        uint256 index;
        for (index = 0; index < _qty; index++) {
            require(_isApprovedOrOwner(msgSender, userTokenList[index]), "ERC721: transfer caller is not owner nor approved");

            IERC721(unclaimedNFT).transferFrom(msgSender, address(0), userTokenList[index]);
