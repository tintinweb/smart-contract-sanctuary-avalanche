...
contract Racing_Game is Ownable {
    ...
    uint8 public minRacers = 5; // min number of unique addresses to participate in race
    ...
    uint256 public pointsToWinEpoch; // number of points, user needs to collect to win epoch
    ...
    uint256 public raceSignupDuration = 600; // 10 min - how long we wait for other participants (note: if all places are filled, than game should immediately start)
    uint256 public raceDuration = 60; // 1 min - duration of racing (raceStatus = 2)
    uint256 public delayBeforeAndAfterRacing = 30;
    ...
    function startEpoch(
        ...
        pointsToWinEpoch = _pointsToWinEpoch;
    ...
    function signUp() public {
        ...
        uint16 quarter1Speed = semirandomNumber(
            raceDetails[raceId].numberOfRacers
        );
        uint16 quarter2Speed = semirandomNumber(block.timestamp);
        uint16 quarter3Speed = semirandomNumber(
        ...
        uint8 salt = 0;
        do {
            quarter4Speed = semirandomNumber(salt); // it's enough if we recalc only one quarter to affect totalSpeed
            totalSpeed =
                quarter1Speed +
                quarter2Speed +
                quarter3Speed +
        ...
        raceAddressDetailsList[raceId][msg.sender] = Participant({
            quarter1Speed: quarter1Speed,
            quarter2Speed: quarter2Speed,
            quarter3Speed: quarter3Speed,
            quarter4Speed: quarter4Speed,
    ...
    function semirandomNumber(uint256 randomKey) private view returns (uint16) {
        ...
        uint16 offset = raceSpeedMIN;
        uint16 scope = raceSpeedMAX - raceSpeedMIN;
        assembly {
    ...
    function rewardWinnerAndStartNew() public {
        ...
        for (uint256 i = 0; i < raceAddressList[raceId].length; i++) {
            ...
            if (epochPoints[epochId][participant] > epochMaxPoints) {
                ...
                // TODO: add check if we have multiple maxPoints, in that case force another race round
            }
        }

        (address first, address second, address third) = _getRaceWinner(raceId);

        uint256 shadyBal = shady.balanceOf(address(this));

        // return ticket-price to winner
        shady.transferFrom(address(this), first, pricePerRace);

        // return half ticket-price (pricePerRace) to 2nd place
        shady.transferFrom(address(this), second, pricePerRace / 2);

        // collect fee
        uint256 taxAmount = (shadyBal * CLAIM_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up
        shady.transferFrom(address(this), devAddress, taxAmount);

        // burn remaining tokens
        shady.transferFrom(
            address(this),
            addressDEAD,
            shady.balanceOf(address(this))
        );

        // Check if epoch over
        if (epochMaxPoints >= pointsToWinEpoch) {
    ...
    function setPointsToWinEpoch(uint256 _pointsToWinEpoch) public onlyOwner {
        pointsToWinEpoch = _pointsToWinEpoch;
    }
