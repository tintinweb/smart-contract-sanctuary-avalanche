...
// File contracts/interfaces/IFPLPool.sol
...
interface IFPLPool is IERC165Upgradeable {
    ...
    event SparkPrizeWithdrawn(address indexed user, uint256 prize);
    ...
    error CanNotWithdrawSparkWorldTokens();
    ...
    function getPrize(address user) external view returns (uint256 prize);
...
// File contracts/interfaces/IPrivateMode.sol
...
// File contracts/libraries/PriceConsumer.sol
...
// File contracts/FPL/FPLPoolStorageStructure.sol
...
contract FPLPoolStorageStructure is AccessControlUpgradeable {
    ...
    uint256 public sparkPrizeAmount;
    ...
    mapping(address => StakeWithPrediction) public stakes;
...
// File contracts/FPL/FPLPoolProxy.sol
...
contract FPLPoolProxy is FPLPoolStorageStructure {
    ...
    error DoesNotSupportIERC721NFTCollectionOrBank();
    error DoesNotSupportIERC1155NFTCollectionOrBank();
    ...
    function initialize(
        ...
        if (_variables[6] == ERC721Token)
            if (
                IERC721NFTCollection(_nftCollection).supportsInterface(
            ...
            else revert DoesNotSupportIERC721NFTCollectionOrBank();
        else if (_variables[6] == ERC1155Token)
            if (
                IERC1155NFTCollection(_nftCollection).supportsInterface(
            ...
            else revert DoesNotSupportIERC1155NFTCollectionOrBank();
        else revert NotASupportedNFTCollection();

        nftType = _variables[6];
...
// File contracts/FPL/FPLFactory.sol

pragma solidity 0.8.11;

/**
 * @dev this is the contract for creating FPLs based on a NFT collection and whitelisting and minting those NFTs
 * by the pool winners.
 */
...
contract FPLFactory is AccessControl {
    ...
    constructor(
        ...
        _setupRole(POOL_CREATOR_ROLE, _msgSender());
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
    ...
    function createFPLPoolProxy(
        ...
        rewardManager.addPool(fplPoolProxyAddr);

        return fplPoolProxyAddr;
    }

    /**
     * @dev Call this in case you want to use a new StakingPoolImplementation from now on
