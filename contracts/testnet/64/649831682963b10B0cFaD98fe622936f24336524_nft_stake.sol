...
contract nft_stake {
    ...
    function removeArr(uint256[] memory _arr, uint rem) public returns(uint256[] memory){
    	uint j;
        while (j < rem) {
            delete _arr[0];
            j++;
    ...
    function stake(uint256 _tokenId, uint256 _amount) public {
    	if (_tokenId == 1) {
        	...
        	stamps = stakes_1[msg.sender].timestamp;
        	stamps.push(time);
        	stakes_1[msg.sender].timestamp = stamps;
        }
        if (_tokenId == 2) {
        	stakes_2[msg.sender].tokenId = 2;
        	stakes_2[msg.sender].amount += _amount;
        	stamps = stakes_2[msg.sender].timestamp;
        	stamps.push(time);
        	stakes_2[msg.sender].timestamp = stamps;
        }
        if (_tokenId == 3) {
        	stakes_3[msg.sender].tokenId = 3;
        	stakes_3[msg.sender].amount += _amount;
        	stamps = stakes_3[msg.sender].timestamp;
        	stamps.push(time);
        	stakes_3[msg.sender].timestamp = stamps;
        }
    ...
    function unstake(uint256 _tokenId, uint8 _amount) public {
	    if (_tokenId == 1) {
			...
			stakes_1[msg.sender].amount -= _amount;
			stamps = stakes_1[msg.sender].timestamp;
			stamps = removeArr(stamps,_amount);
			stakes_1[msg.sender].timestamp = stamps;
		}
		if (_tokenId == 2) {
			stakes_2[msg.sender].tokenId = 2;
