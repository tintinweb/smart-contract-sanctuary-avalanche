...
contract ZEUS is Ownable, PaymentSplitter {
    ...
    function cashoutAllNodesByType(address account, uint32 nodeType) public {
        address sender = _msgSender();
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        require(
        ...
        (uint256 total, uint256 fee) = nodeRewardManager
            ._cashoutAllNodesByTypeReward(account, nodeType);
        require(total > 0, "CSHT: You don't have enough reward to cash out");
        // distr pool needs to give contract access to funds
        token.transferFrom(distributionPool, address(this), total);
    ...
    function compoundNode(
        ...
        uint256 fee = nodeRewardManager._compoundNode(
            _msgSender(),
            nodeName,
            total,
            nodeType
