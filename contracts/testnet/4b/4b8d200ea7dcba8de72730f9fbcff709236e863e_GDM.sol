...
contract GDM {
    ...
    constructor(
        ...
        LDiamond.setContractOwner(_gdmOwner);
        IGDMCut.FacetCut[] memory cut = new IGDMCut.FacetCut[](1);

        bytes4[] memory functionSelectors = new bytes4[](1);

        functionSelectors[0] = IGDMCut.diamondCut.selector;

        cut[0] = IGDMCut.FacetCut({
            facetAddress: _diamondCutFacet,
            action: IGDMCut.FacetCutAction.Add,
    ...
    fallback() external payable {
        ...
        if (facet == address(0)) revert GDM__InexistentFunction();
