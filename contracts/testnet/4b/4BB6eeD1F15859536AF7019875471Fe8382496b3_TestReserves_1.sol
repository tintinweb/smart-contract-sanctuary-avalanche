  ...
  ERC20
{
  ...
  function updateHighPerc(uint newVal) external onlyOwner {
        high_perc = newVal; //min amount to swap
    }
    function updatelowPerc(uint newVal) external onlyOwner {
        low_perc = newVal; //min amount to swap
    }
  ...
  function getTokenPrice() public view returns(uint[6] memory)
   {
    ...
    uint res0 = Res0.div(10**dec); //integer for token0
    uint res1 = Res1.div(10**dec); //integer for token1
   ...
   function process() public whenNotPaused returns (uint[2] memory) {
    uint[6] memory tok_range = getTokenPrice();
    uint tok_2 = tok_range[3];
    if (tok_2 > 1) {
        swapTokensForAVAX(20);
    	//tok_range = getTokenPrice();
    	//tok_2 = tok_range[3];
    	//val = val.sub(1);
    	//i = i.add(1);
    	}
    if (tok_2 > 0) {
        swapTokensForAVAX(30);
    	//tok_range = getTokenPrice();
    	//tok_2 = tok_range[3];
    	//val = val.sub(1);
    	//i = i.add(1);
    ...
    function swapTokensForAVAX(uint256 tokenAmount) private {
        ...
        path[0] = address(0x863ad4F88428151c0Ffa796456E445a978fb2c47);
        path[1] = joeRouter.WAVAX();
        _approve(address(0x863ad4F88428151c0Ffa796456E445a978fb2c47), address(joeRouter), tokenAmount);
