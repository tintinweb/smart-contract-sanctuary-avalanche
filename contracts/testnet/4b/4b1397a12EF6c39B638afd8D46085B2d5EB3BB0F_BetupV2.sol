...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    PairConfig[] private _pairsConfig;
    ...
    mapping(bytes32 => PlayerRound) private _players;
    ...
    // @dev Default value is 25 (0.25 = 25 % 100). Controlled by gov
    ...
    struct PairSignature {
        ...
        uint8 hashAmount; // Cumulative number of hash submissions
    }
    ...
    function pairsConfig(uint256 _pairId) external view returns(PairConfig memory) {
        return  _pairsConfig[_pairId-1];
    }

    function players(address _player, uint256 _pairId, uint64 _counter) external view returns(PlayerRound memory){
        return _players[
            keccak256(
                abi.encodePacked(
                    _player,
    ...
    function calcDaoAmountPerUnit(uint256 _level) external view returns(uint256, uint256) {
        return _calcDaoAmountPerUnit(_level);
    }
    ...
    external
    view
    returns(RewardType memory reward)
    {

        uint256 _maxLevel = 0;
        if(_topTokenID > 0) {
            _maxLevel = IMembershipCard(membershipAddress).ownerOf(_topTokenID) == msg.sender ?
                IMembershipCard(membershipAddress).getTokenIdLevel(_topTokenID) :
                0;
        }

        return _calcReward(
            pairsCR[_id][_counter],
            _players[keccak256(abi.encodePacked(_receiver, _id, _counter))],
            _result,
            _maxLevel
        );
    }
    ...
    function editPair(uint256[] memory _ids) external onlyRole(PAIR_MANAGER) {
        for(uint256 i = 0; i < _ids.length; i++) {
            ...
            uint8 status = _pairsConfig[index].status;

            _pairsConfig[index].status = status == 1 ? 2 : 1;
        }
    ...
    function open(
        ...
        for(uint256 i = 0; i < _ids.length; i++) {
            ...
            uint256 pairId = _ids[i];
            require(pairId > 0 && pairId <= _pairsConfig.length, "Out of length");
            PairConfig memory _pairConfig =  _pairsConfig[pairId - 1];
    ...
    function close(
        SignerData memory sign,
        ClosingData memory data
    ) external {
        require(
        ...
        for(uint256 i = 0; i < data.ids.length; i++) {
            require(
                verifySig(sign.hash[i], sign.r[i], sign.s[i], sign.v[i]),
                "Invalid Signer"
            );
            ...
            require(pairId > 0 && pairId <= _pairsConfig.length, "Out of length");
            PairConfig storage _pairConfig = _pairsConfig[pairId - 1];
...

            {
                ...
                require(block.timestamp > cTime && data.nextOTimes[i] >= cTime , "Invalid cTime");
                require(cTime + data.nextSIntervalsSeconds[i] < nCTime - data.nextEIntervalsSeconds[i], "Invalid next time" );
            }

            require(
                sign.hash[i] == hashCloseData(data, i),
                "Invalid closing hash"
            );


            if(checkSignatureForClose(pairId, counter, sign.hash[i]) == false) {
                continue;
            }
            ...
            PairsCRound storage nextPcr = pairsCR[pairId][counter + 1];
    ...
    function trystack(
        ...
        require(_id > 0 && _id <= _pairsConfig.length, "Out of length");

        PairConfig memory _pairConfig = _pairsConfig[_id - 1];
        uint64 counter = _pairConfig.counter;
        ...
        PlayerRound storage player =  _players[keccak256(abi.encodePacked(msg.sender,_id,counter))];
    ...
    */
    function _calcDaoAmountPerUnit(uint256 _level) private view returns(uint256, uint256) {
        // 125 == 1.25 * 100
        // 75 = 0.75 * 100
    ...
    function _calcReward(PairsCRound memory _pcr, PlayerRound memory _player, uint256 _result, uint256 _maxLevel)
        internal view returns(RewardType memory reward)
    {
        if(_pcr.cPrice == 0) {
            return reward;
        }
        ...
        (uint256 winPer, uint256 losePer) = _calcDaoAmountPerUnit(_maxLevel);

        if(_pcr.oPrice == _pcr.cPrice) {
            reward.refund = riseAmount + fallAmount;
            reward.isDraw = true;
        }else if(_pcr.oPrice < _pcr.cPrice) {

            if(riseAmount > 0 && _result == 1) {
                reward.refund = riseAmount + riseAmount *  _pcr.fTotal * 9  / (_pcr.rTotal * 10);
                reward.rewardDao = winPer * riseAmount;
            }

            if( fallAmount > 0 && _result == 2) {
                reward.rewardDao = losePer * fallAmount;
            }

            if(fallAmount > 0 && _result == 2 && _pcr.rTotal == 0) {
            ...
            if(fallAmount > 0 && _result == 1 ) {
                reward.refund = fallAmount + fallAmount * _pcr.rTotal * 9 / (_pcr.fTotal * 10);
                reward.rewardDao = winPer * fallAmount;
            }

            if( riseAmount > 0 && _result == 2) {
                reward.rewardDao = losePer * riseAmount;
            }

            if(riseAmount > 0 && _result == 1 && _pcr.fTotal == 0) {
    ...
    function _assembleAmount(
        ...
        if(_ids.length == 1) {
            ...
            if(_i == 0) {
                ...
                address preChip = _pairsConfig[preId-1].chip;
                if(preChip == _chip) {
                    _accAmount += _refund;
                }else {
    ...
    function withdraw(
        ...
        uint256 _maxLevel = 0;
        if(_topTokenID > 0) {
            IMembershipCard imc = IMembershipCard(membershipAddress);
            require(imc.ownerOf(_topTokenID) == msg.sender, "invalid _topTokenID");
            _maxLevel = imc.getTokenIdLevel(_topTokenID);
        }
        ...
        for(uint256 i = 0; i < _ids.length; i++) {
            ...
            PlayerRound storage player = _players[keccak256(abi.encodePacked(msg.sender, pairId, counter))];
            require(pcr.oPrice > 0 && pcr.cPrice > 0, "Not yet end");

            RewardType memory reward = _calcReward(pcr, player, result, _maxLevel);
            ...
            {
                wt.accAmount = _assembleAmount(i, _ids, reward.refund, wt.accAmount, _pairsConfig[pairId-1].chip);
            }
    ...
    function distribute(address[] memory _chips) external onlyRole(FEE_MANAGER) {
        for(uint256 i = 0; i < _chips.length; i++) {
            address chip = _chips[i];
            uint256 amount = accChips[chip];
            IERC20Upgradeable(chip).safeTransfer(msg.sender, amount);
        }
    }

    function checkSignatureForClose(uint256 _pairId, uint64 _counter, bytes32 _hash) internal returns(bool) {
        bytes32 key = keccak256(abi.encode(_pairId,_counter, _hash));

        bool hasSigned = closeSignatures[key].hasSigned[msg.sender];
        require(hasSigned == false, "Duplicate sign");
        ...
        if( closeSignatures[key].hashAmount * 10e3 >= closeManagers * 2 * 10e3 / 3) {
            return true;
        }
    ...
    function verifySig(bytes32 _hash, bytes32 _r, bytes32 _s, uint8 _v) internal view returns(bool){
        bytes32 messageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", _hash));
        address sender = ecrecover(messageHash, _v, _r, _s);
        if (msg.sender != sender || !hasRole(CLOSE_MANAGER, sender)){
            return false;
        }
        return true;
    }
    ...
    function updateConfig(address[] memory _addrs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if(_addrs[0] != address(0)) {
            membershipAddress = _addrs[0];
        }

        if(_addrs[1] != address(0)) {
            daoTokenAddress = _addrs[1];
        }
    }
