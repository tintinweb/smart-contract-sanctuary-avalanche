...
contract LofiLeopards is ERC721URIStorage, Ownable, KindaRandom {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

  uint256 private _legendaryTokenIds;
  uint256 private _legendaryTokenCount;
  ...
  constructor() ERC721("LoFi Leopards", "LOFI") KindaRandom(2453) {
    _customBaseURI = "https://nfts.bpi.network/lofi_leopards/";
    _maxMintable = 2453; // supply of generated NFTs
    _royaltyAmount = 750; // 7.5% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 10;
    _price = 0.3 ether;
    _artist =  0x73AF430b3Ba5aBB336E835Ede31702d7330b3169;
    _startTime = 1661383800;
    _legendaryTokenIds = 2454;
    _legendaryTokenCount = 0;
  ...
  function legendaryAirdrop() public onlyOwner {
    require(_legendaryTokenIds <= 2512);
    _mint(_artist, _legendaryTokenIds);
    _legendaryTokenIds += 1;
    _legendaryTokenCount += 1;
  }

  function legendaryTokenCount() public view returns (uint256) {
    return _legendaryTokenCount;
  }

  function airdrop(address[] memory _accounts) public onlyOwner {
    for (uint256 index = 0; index < _accounts.length; index++) {
      _privateMint(_accounts[index]);
    }
  }
  ...
  function totalSupply() public view returns (uint256) {
    return _tokenIds.current() + _legendaryTokenCount;
  }
