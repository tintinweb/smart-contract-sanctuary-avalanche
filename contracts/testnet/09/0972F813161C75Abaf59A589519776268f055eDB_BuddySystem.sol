...
contract BuddySystem is Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        function updateBuddyFromPresales(address[] memory buddy, address _buddies) onlyOwner public  {
        for (uint256 i = 0; i < buddy.length; i++) {
        updateBuddyFromPresale(buddy[i], _buddies);
        }
    }
