...
// File: contracts/ThortlesNFTClub.sol
...
contract ThortlesNFTClub is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public whiteListedCost = 2.5 ether;
...
//test
  ...
  mapping(address => uint) private ThortlesNFTsPreSale;
  ...
  /*
    ...
    @description - Mints _mintAmount of Thortles NFTs for sender address.
    @param <uint256> _mintAmount - The number of Thortles NFTs to mint.
  */
    function mint(uint256 _mintAmount) public payable {
    require(!paused, "The contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "You need to mint at least 1 Thortle NFT");
    require(_mintAmount <= maxMintAmount, "Max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "Max Thortles NFT limit exceeded");

    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(msg.sender), "Sorry, address is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender]; 
            require(ownerMintedCount + _mintAmount <= maxMintAmountForWhitelist, "Max Thortles NFT per whitelisted address exceeded");
            whitelistCount = ThortlesNFTsPreSale[msg.sender] += _mintAmount;
        } else {
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(_mintAmount <= maxMintAmount, "Max Thortles NFT per transaction exceeded");
            require(ownerMintedCount + _mintAmount <= maxWalletAmount + whitelistCount, "Max Thortles NFT per wallet exceeded");
        }

        if (dynamicCost) {
            require(msg.value >= updateCost(onlyWhitelisted) * _mintAmount, "Insufficient funds. Please add more AVAX to whitelist address.");   
        } else {
            require(msg.value >= cost * _mintAmount, "Insufficient funds. Please add more AVAX to address.");
        }
    }
    
    for (uint256 i = 1; i <= _mintAmount; i++) {
        uint256 sply = totalSupply();
        uint256 tokenId = sply + 1;
  ...
  */
    ...
    function manualGiveawayActivation() public onlyOwner {
        giveawayActive = true;
        giveawayDates.push(block.timestamp + (epochDay * giveawayIntervalDays));
        drawNumber++;
    }
...
//   }
  ...
  */
    function getAllWhiteListedAddresses() public view returns (address[] memory) {
  ...
  */
  function setCost(uint256 newCost) public onlyOwner {
    ...
    /*
    @function setWhiteListCost(newWLCost)
    @description - Sets the cost per NFT for whitelist
  */
  function setWhiteListCost(uint256 newWLCost) public onlyOwner {
      whiteListedCost = newWLCost;
  }

   /*
    @function setMaxWalletAmount(newMaxWalletAmount)
    @description - Sets max wallet amount
  */
  ...
  /*
    @function setMaxMintAmount(newmaxMintAmount)
  ...
  */
  function setMaxMintAmount(uint256 newmaxMintAmount) public onlyOwner {
   ...
   /*
    @function setNftPerAddressLimit(limit)
    @description - Sets the number of NFTs a whitelisted address can mint
  */
  function setMaxMintAmountForWhitelist(uint256 limit) public onlyOwner {
    maxMintAmountForWhitelist = limit;
  }
  ...
  */
  function pause(bool state) public onlyOwner {
    ...
    /*
    @function pauseWithdrawals(state)
    @description - Pauses withdrawal (True/False)
  */
  function pauseWithdrawals(bool state) public onlyOwner {
    withdrawPaused = state;
  }

  /*
    @function dynamicCostState(state)
    @description - Turns dynamic costs on or off (True/False)
  */
  function dynamicCostState(bool state) public onlyOwner {
      dynamicCost = state;
  }

  /*
    @function setOnlyWhitelisted(state)
  ...
  */
  ...
  /*
    @function addWhitelistUsers(usersToAdd)
    @description - Add user address to whitelist for priority minting
  */
//   function addWhitelistUsers(address[] calldata usersToAdd) public onlyOwner {
//     delete whitelistedAddresses;
//     whitelistedAddresses = usersToAdd;
//   }
...
/*
    ...
    */
    ...
    /*
      @function setGiveawayState(state)
    ...
    */
    ...
    /*
      @function withDrawToInvest()
      @description - Sends contract balance to wallet for staking.
    */
      function withDrawToInvest() public payable onlyOwner {
        require(!paused, "ERROR: Contract paused!");
        require(!withdrawPaused, "ERROR:  Withdrawals currently paused!");
