...
{
    mapping(address => ContractInfo) public contractAddressMapping;
    mapping(bytes32 => ContractInfo) public contractNameMapping;
    ...
    function bridgeNFT(
        ...
        require(
            contractAddressMapping[_srcToken].status &&
                contractAddressMapping[_srcToken].isERC721,
        ...
        bytes32 keyMapping = keccak256(
            abi.encodePacked(contractAddressMapping[_srcToken].name)
        );
    ...
    function _nonblockingLzReceive(
        ...
        require(
            contractNameMapping[keyMapping].status,
            "Contract-not-supported"
        );

        IERC721Monsterra(contractNameMapping[keyMapping].token).mint(
            to,
            data,
            "",
            0 // Type: not use
    ...
    function addContractSupport(
        ...
        require(
            !contractAddressMapping[_contractAddress].status,
            "Contract-added"
        );
        ContractInfo memory contractInfo;
        contractInfo.token = _contractAddress;
        contractInfo.name = _name;
        contractInfo.isERC721 = _isERC721;
        contractInfo.isMintBurn = _isMintBurn;
        contractInfo.status = true;
        contractAddressMapping[_contractAddress] = contractInfo;
        bytes32 keyMapping = keccak256(abi.encodePacked(_name));
        contractNameMapping[keyMapping] = contractInfo;
    }

    function removeContractSupport(
        address _contractAddress
    ) external onlyOwner {
        contractAddressMapping[_contractAddress].status = false;
        bytes32 keyMapping = keccak256(
            abi.encodePacked(contractAddressMapping[_contractAddress].name)
        );
        contractNameMapping[keyMapping].status = false;
    }

    function estimateFeeBridgeNFT(
        uint16 _dstChainId,
        address _srcToken,
        uint256 _tokenId,
        bool _useZro
