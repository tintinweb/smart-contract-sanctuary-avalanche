...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    uint256 private constant TURN_PERIOD = 60; //Now we are use 5 minute //86400; // 24 HOURS
    uint256 private constant THERSHOLD = 800; //5
    uint256 private constant SERIES_TWO_FEE = 0.7 ether;   //0.01
    ...
    function entryFeeForSeriesOne(bytes32  _playerId,uint256 _nftId) public GameEndRules
    {
        bytes32 playerId = _computeNextPlayerIdForHolder(msg.sender, _nftId, 1);
    ...
    function bulkEntryFeeForSeriesOne(bytes32[] memory _playerId,uint256[] calldata _nftId) external {
        for (uint256 i = 0; i < _nftId.length; i++) {
            entryFeeForSeriesOne(_playerId[i],_nftId[i]);
        }
    }
    ...
    function entryFeeSeriesTwo(bytes32  _playerId,uint256 _nftId) public GameEndRules
    {
        bytes32 playerId = _computeNextPlayerIdForHolder(msg.sender, _nftId, 2);
        require(playerId == _playerId,"Player ID not match");
          if(isExist(playerId)){
            participatePlayerList.push(playerId);    
        }
    ...
    function bulkEntryFeeSeriesTwo(bytes32[] memory _playerId,uint256[] calldata _nftId) external {
        for (uint256 i = 0; i < _nftId.length; i++) {
            entryFeeSeriesTwo(_playerId[i],_nftId[i]);
        }
    }
    ...
    function gameWinnerLength() external view returns(uint256){
