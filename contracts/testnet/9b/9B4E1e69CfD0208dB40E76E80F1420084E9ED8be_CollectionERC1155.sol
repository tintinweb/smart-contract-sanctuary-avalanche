...
ERC1155URIStorageUpgradeable {
    ...
    function initialize1155(string memory _name, string memory _symbol, address receiver, uint256 amount, string memory tokenUri, bytes32 msgHash) initializer public {
        ...
        mint(receiver, 1, amount, tokenUri, "", msgHash);
    }
    ...
    function mint(address account, uint256 id, uint256 amount, string memory tokenURI, bytes memory data, bytes32 msgHash)
        public
        onlyOwner
    {
        _setURI(id, tokenURI);
        _mint(account, id, amount, data);
        royaltyHash[id] = RoyaltyData(account, msgHash);
    }

    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, string[] memory tokenURIs, bytes memory data, bytes32[] memory msgHash)
        public
        onlyOwner
    {
        for(uint256 i=0; i< tokenURIs.length; i++){
            _setURI(ids[i], tokenURIs[i]);
            royaltyHash[ids[i]] = RoyaltyData(to, msgHash[i]);
        }
