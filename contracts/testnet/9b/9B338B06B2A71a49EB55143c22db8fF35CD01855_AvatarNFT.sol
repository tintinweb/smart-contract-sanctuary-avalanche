...
// 
contract AvatarNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _baseUri, address _randomizer, uint256 _price, uint256 _maxLimit) ERC721("AVATAR-BKWZ", "AVATAR") {
        baseUri = _baseUri;
    ...
    function mint() internal returns (uint256) {
        ...
        require(_tokenIds.current() <= 1000, "You can't mint");
        ...
        // _setTokenURI(newNftId, newAvatarId.toString());
    ...
    function mintAvatar(uint256 amount) external payable {
        require(msg.value >= price * amount, "avax value is less than price");
        require((super.balanceOf(msg.sender) + amount) <= maxLimit || msg.sender == owner(), "You can't mint more than max limit");
    ...
    function ownedAvatars(address purchaser) public view returns (avatarDetail[] memory) {
    ...
    function setPriceByOwner(uint256 _price) external onlyOwner {
        price = _price;
    }
}