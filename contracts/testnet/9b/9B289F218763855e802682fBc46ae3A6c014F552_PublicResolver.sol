...
interface IMintHook {
    function mintHook(uint256 tokenId, uint256 newPrice, uint256 expirationDate) external;
}

contract Main is ERC721, Ownable {
    address private harbergerAddress;
    address private rentAddress;
    ...
    modifier fromHarberger() {
        require(msg.sender == harbergerAddress, "Only Harberger can call this function");
        _;
    }

    modifier fromRent() {
        require(msg.sender == rentAddress, "Only Rent can call this function");
        _;
    }

    modifier fromMint() {
        require(msg.sender == mintAddress, "Only Mint can call this function");
        _;
    }
    ...
    function setHarbergerAddress(address _harbergerAddress) external onlyOwner {
        harbergerAddress = _harbergerAddress;
    }

    function setRentAddress(address _rentAddress) external onlyOwner {
    ...
    function mint(address to, uint256 tokenId, uint256 newPrice, uint256 expirationDate) external fromMint {
        if (isHarberger(tokenId)) {
            IMintHook(harbergerAddress).mintHook(tokenId, newPrice, expirationDate);
        } else {
            IMintHook(rentAddress).mintHook(tokenId, 0, 0);
        }

        resolvers[tokenId] = publicResolver;
        _safeMint(to, tokenId);
    }
    ...
    function getResolver(uint256 tokenId) external view returns (address) {
        if (isHarberger(tokenId)) {
            require(!IExpiry(harbergerAddress).expired(tokenId), "Token is expired");
        } else {
            require(!IExpiry(rentAddress).expired(tokenId), "Token is expired");
        }
    ...
    function forcedTransfer(address from, address to, uint256 tokenId) external fromHarberger {
        _transfer(from, to, tokenId);
    }
...
import {Main as main} from "./Main.sol";
...
contract PublicResolver is IPublicResolver {
    /*//////////////////////////////////////////////////////////////
                              BASE RESOLVER
    //////////////////////////////////////////////////////////////*/
    main public immutable harberger;
    ...
    constructor(address _harbergerAddress) {
        harberger = main(_harbergerAddress);
    }
    ...
    mapping(uint256 => mapping(uint256 => bytes)) public addresses;
    ...
    mapping(uint256 => PublicKey) public publicKeys;
