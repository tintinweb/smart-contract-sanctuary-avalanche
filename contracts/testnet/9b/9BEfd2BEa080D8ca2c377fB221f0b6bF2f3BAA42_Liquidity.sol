...
interface ISwap {
     ...
     */
    function avaxToTokenSwapInput(uint256 min_tokens) external payable returns (uint256);
     ...
     */
    function avaxToTokenSwapOutput(uint256 tokens_bought) external payable returns (uint256);
     ...
     */
    function tokenToAvaxSwapInput(uint256 tokens_sold, uint256 min_avax) external returns (uint256);
     ...
     */
    function tokenToAvaxSwapOutput(uint256 avax_bought, uint256 max_tokens) external returns (uint256);
     ...
     */
    function getAvaxToTokenInputPrice(uint256 avax_sold) external view returns (uint256);
     ...
     */
    function getAvaxToTokenOutputPrice(uint256 tokens_bought) external view returns (uint256);
     ...
     */
    function getTokenToAvaxInputPrice(uint256 tokens_sold) external view returns (uint256);
     ...
     */
    function getTokenToAvaxOutputPrice(uint256 avax_bought) external view returns (uint256) ;
    ...
    function getAvaxToLiquidityInputPrice(uint256 avax_sold) external view returns (uint256);
     ...
     */
    function removeLiquidity(uint256 amount, uint256 min_avax, uint256 min_tokens) external returns (uint256, uint256);
}
...
contract Liquidity {
    ...
    uint8 constant internal solisiumFee = 50;
    ...
    /// @dev converts AVAX into liquidity and buys
    function buy() public payable returns (uint256){

        require(msg.value >= 1e16, "min buy is 0.01 AVAX");
        ...
        uint tokens = sellAvax(balance / 2);

        //the secret sauce for adding liquidity properly
        uint avaxAmount = SafeMath.min(swap.getTokenToAvaxInputPrice(tokens), address(this).balance);

        //If you don't get avaxAmount from the contract you will have pain
        uint liquidAmount = swap.addLiquidity.value(avaxAmount)(1, tokens);
    ...
    function() public payable  {
        //DO NOTHING!!! Swap will send AVAX to us!!!
    }
    ...
    function reinvest() public onlyStronghands returns (uint) {
        ...
        uint avaxAmount = calculateLiquidityToAvax(_dividends);

        // fire event
        emit onReinvestment(_customerAddress, avaxAmount, _tokens, now);

        //Stats
        stats[_customerAddress].reinvested = SafeMath.add(stats[_customerAddress].reinvested, avaxAmount);
    ...
    function withdraw() public onlyStronghands returns (uint) {
        ...
        //remove liquidity and sell the tokens for AVAX
        (uint avaxAmount, uint tokenAmount) = swap.removeLiquidity(_dividends,1,1);
        avaxAmount = avaxAmount.add(sellTokens(tokenAmount));

        // lambo delivery service
        _customerAddress.transfer(avaxAmount);

        totalWithdrawn += avaxAmount;

        //stats
        stats[_customerAddress].withdrawn = SafeMath.add(stats[_customerAddress].withdrawn, avaxAmount);
        stats[_customerAddress].xWithdrawn += 1;
        totalTxs += 1;
        totalClaims += _dividends;

        // fire event
        emit onWithdraw(_customerAddress, avaxAmount, now);
    ...
    /// @dev Retrieve the token balance of any single address.
    function avaxBalance(address _customerAddress) public view returns (uint256) {
    ...
    function sellTokens(uint256 amount) internal returns (uint256) {
        approveSwap();
        return swap.tokenToAvaxSwapInput(amount,1);
    }

    function sellAvax(uint256 amount) internal returns (uint256){
        return swap.avaxToTokenSwapInput.value(amount)(1);
    }

    function calculateLiquidityToAvax(uint256 _amount) public view returns (uint256) {
        if (_amount > 0){
            (uint avaxAmount, uint tokenAmount) = swap.getLiquidityToReserveInputPrice(_amount);
            return avaxAmount.add(swap.getTokenToAvaxInputPrice(tokenAmount));
        } else {
            return 0;
        }
    }

    function calculateTaxedAvaxToTokenLiquidity(uint256 _amount) public view returns (uint256) {
        if (_amount > 0){
            uint amount = swap.getAvaxToLiquidityInputPrice(_amount.div(2));
    ...
    function calculateTaxedLiquidityToAvax(uint256 _amount) public view returns (uint256){
        if (_amount > 0){
    ...
    function sweep() public returns (uint256){
        ...
        if (balanceOriginTokens >= 1e18  && tokenSupply_ > 0){
            ...
            //We need to start with AVAX so we can safely split and add liquidity; also collect AVAX dust in the contract
            uint balanceAvax = sellTokens(halfTokens);
            uint balanceTokens = collateralBalance();
            //the secret sauce for adding liquidity properly
            uint avaxAmount = SafeMath.min(swap.getTokenToAvaxInputPrice(balanceTokens), balanceAvax);

            //If you don't get avaxAmount from the contract you will have pain
            uint liquidAmount = swap.addLiquidity.value(avaxAmount)(1, balanceTokens);
            ...
            //Add the new liquidity to solisium dividends;
    ...
    /// @dev Calculate daily estimate of swap tokens awarded in AVAX
    function dailyEstimateAvax(address _customerAddress) public view returns (uint256){
        if (tokenSupply_ > 0){
            ...
            (uint avaxAmount, uint tokenAmount) = swap.getLiquidityToReserveInputPrice(estimate);
            return avaxAmount.add(swap.getTokenToAvaxInputPrice(tokenAmount));
        } else {
            return 0;
        }
    ...
    /// @dev Distribute undividend in and out fees across solisium pools and instant divs
    function allocateFees(uint fee) private {
        uint _share = fee.div(100);
        uint _solisium = _share.mul(solisiumFee);           //40 --> 50
        uint _instant = _share.mul(instantFee);     //40 --> 20
        uint _lock = fee.safeSub(_solisium + _instant); //20 --> 30

        if (tokenSupply_ > 0) {
            //Apply divs
            profitPerShare_ = SafeMath.add(profitPerShare_, (_instant * magnitude) / tokenSupply_);
        }
        //Add to dividend solisium pools
        dividendBalance += _solisium;
    ...
    // @dev Distribute solisium pools
    function distribute() private {

        // @Bb updates balance data of contract
        if (now.safeSub(lastBalance_) > balanceInterval && totalTokenBalance() > 0) {
            (uint avaxAmount, uint tokenAmount) = swap.getLiquidityToReserveInputPrice(totalTokenBalance());
            emit onBalance(avaxAmount, tokenAmount, now);
            lastBalance_ = now;
        }
    ...
    function purchaseTokens(address _customerAddress, uint256 _incomingtokens) internal returns (uint256) {
        ...
        uint256 avaxAmount = calculateLiquidityToAvax(_incomingtokens); //total avax worth of lp token

        // fire event
        emit onTokenPurchase(_customerAddress, avaxAmount, _amountOfTokens, now);
        ...
        //solisium and buybacks; instant requires being called after supply is updated
        allocateFees(_undividedDividends);
        ...
        stats[_customerAddress].invested += avaxAmount;
