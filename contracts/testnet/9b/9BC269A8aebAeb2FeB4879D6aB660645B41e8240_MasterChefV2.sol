...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///     the TOKEN rewards to all the PantokenSwap products.
    ...
    uint256 public tokenRateToBurn = 643750000000;
    ...
    /// @param _MASTER_CHEF The PantokenSwap MCV1 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of TOKEN.
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn TOKEN reward from current block in MCV1 pool
    ...
    /// only for TOKEN distributions within PantokenSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
    ...
    /// @notice Harvests TOKEN from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough TOKEN. If not, harvest from MCV1.
