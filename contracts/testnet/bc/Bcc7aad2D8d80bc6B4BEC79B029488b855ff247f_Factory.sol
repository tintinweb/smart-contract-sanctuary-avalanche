...
contract Factory is Ownable, KeeperCompatibleInterface {
    ...
    */
    function unstake(uint256 nectarAmount, bool wantToClaim) external {
        ...
        nectar.transfer(address(this), nectarAmount);
    ...
    */
    function _claim(address _owner) internal {
        require(stakingByAddress[_owner].claimableBalance > 0, "You cannot claim rewards if you have none");
        uint amountToClaim = stakingByAddress[_owner].claimableBalance;
        stakingByAddress[_owner].alreadyWithdrawn += amountToClaim;
        stakingByAddress[_owner].claimableBalance = 0;
        honey.mintForStaking(_owner, amountToClaim);
        emit Claimed(_owner, amountToClaim);
    }
