...
contract Blacksmith is FeeCollector, VRFConsumerV2Ownable, CoalConsumer {
    Blueprint public immutable BlueprintContract;
    GearNFT public immutable GearNFTContract;
    RawMaterialNFT public immutable RawMaterialNftContract;
    ...
    mapping(uint256 => PlayerRequest) public Requests;
    ...
    event GearInfoSet(uint256 gearNFTId, bytes32 gearInfo);
    event CraftingStarted(uint256 blueprintId, uint256 landTokenId, uint256 slotId);
    ...
    constructor(
        ...
        address _gearNFTAddress,
        uint64 _subscriptionId
    )
        FeeCollector(_smolApaLandAddress, _feeAggregator, _landRegistery, ForgeType)
        ...
        CoalConsumer(_rawMaterialNftContract)
    {
        ...
        GearNFTContract = GearNFT(_gearNFTAddress);
    }

    function emergencyWithdraw (address _address, uint256 _gearNFTId ) external onlyOwner {
        GearNFTContract.transferFrom(address(this), _address, _gearNFTId);
    }
    ...
    function startCrafting(uint256 _blueprintId, uint256 _landTokenId, uint256 _slotId) external payable {
        ...
        if (_blueprintId < COMMON_SCHEMATIC) {
            // prevent mixing up with alchemy scematics which would create weird NFTS
        ...
        JobsDictionary[msg.sender][jobId] = Order(uint32(completedTimeStamp), uint16(_blueprintId), 0, false);
        emit CraftingStarted(_blueprintId, _landTokenId, _slotId);
    }

    function repairGear(uint256 _gearNFTId, uint256 _landTokenId, uint256 _slotId) external payable{
        (uint256 level, uint256 landType) = LandRegistryContract.LandInfoRegistry(_landTokenId);
        if (landType != LandType) {
            revert WrongLand();
        }
        ...
        GearNFTContract.transferFrom(msg.sender, address(this), _gearNFTId);
        slotInfo[_landTokenId][_slotId] = completedTimeStamp; // update the timeslot
        handleFees(_landTokenId);
        uint256 jobId = _landTokenId * 100 + _slotId;

        bytes32 gearInfo = GearNFTContract.GearInfo(_gearNFTId);
        uint256 gearType = uint16(gearInfo[1] | (bytes2(gearInfo[2]) >> 8));
        bytes32 repairInfo = GearNFTContract.RepairInfo(gearType);
        (uint256[] memory types, uint256[] memory amounts) = getMaterialsFromRecipe(repairInfo);
        RawMaterialNftContract.batchBurnRawMaterial(types, amounts, msg.sender);
        JobsDictionary[msg.sender][jobId] = Order(uint32(completedTimeStamp), uint16(0), uint200(_gearNFTId),true);
    }
    ...
    function initiateClaim(uint256 _landTokenId, uint256 _slotId) external {
        uint256 jobId = _landTokenId * 100 + _slotId;
        ...
        if (order.forgeRepair) {
            bytes32 gearInfo = GearNFTContract.GearInfo(order.gearId);
            uint8 repairAmount = uint8(gearInfo[3]) / 2; // max durability / 2
            bytes1 newDurability = bytes1(repairAmount + uint8(gearInfo[4]) % uint8(gearInfo[3])); // current durability + 50 percent
            gearInfo = gearInfo & 0xFFFFFFFF00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; 
            gearInfo |= (bytes32(newDurability) >> (8 * 4));
            GearNFTContract.setGearInfo(order.gearId, gearInfo);
            GearNFTContract.transferFrom(address(this), msg.sender, order.gearId);
            emit GearInfoSet(order.gearId, gearInfo);
        }
        else{
            uint256 s_requestId =
            COORDINATOR.requestRandomWords(keyHash, s_subscriptionId, requestConfirmations, callbackGasLimit, numWords);
            Requests[s_requestId].player = msg.sender;
            Requests[s_requestId].blueprintId = order.blueprintId;
            Requests[s_requestId].jobId = uint32(jobId);
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        PlayerRequest memory request = Requests[requestId];
        GearNFTContract.mintGear(request.blueprintId, request.player, randomWords[0]);
        emit Claimed(request.player, request.jobId);
    }
    ...
    function getRequiredMaterials(uint256 _blueprintId)
        ...
        returns (uint256[] memory types, uint256[] memory amounts)
    {
        uint256 blueprintIndex = _blueprintId % 200; // example 2206 -> index = 6
        uint256 blueprintCategory = _blueprintId - blueprintIndex; //
        bytes32 recipe = BlueprintContract.blueprints(blueprintCategory, blueprintIndex - 1); // -1 is necessary because blueprints 
