...
contract HauntedWitchesLZ is Context, ERC721Enumerable, ERC721URIStorage, Ownable, NonblockingReceiver{
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTracker;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI, uint mintPrice, uint max, address wallet, address _lzEndpoint) ERC721(name, symbol) {
      _baseTokenURI = baseTokenURI;
      _price = mintPrice;
      _max = max;
      _wallet = wallet;
  ...
  function setMax(uint max) external onlyOwner {
    _max = max;
  }

  function setMint(bool openMint, bool openWhitelistMint) external onlyOwner {
    _openMint = openMint;
    _openWhitelistMint = openWhitelistMint;
  }
  ...
  function whitelistUser(address user, uint nbspot) public onlyOwner {
    whitelist[user] = nbspot;
  }
...
contract HauntedWitches is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer {
    ...
    // CONSTANTS
    uint public constant MAX_SUPPLY = 750;
    ...
    constructor(
        uint _startID,
        address _lzEndpoint
    ) ERC721("Haunted Witches", "HW") ClampedRandomizer(maxSupply) {
        startID = _startID;
        endpoint = ILayerZeroEndpoint(_lzEndpoint);
        _pause();
    }
