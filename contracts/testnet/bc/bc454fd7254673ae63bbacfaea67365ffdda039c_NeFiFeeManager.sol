...
contract NeFiFeeManager is Ownable{
	...
	function payFeeTokenSpec(address _account, uint256 _intervals, uint256 _x) payable external {
	    ...
	    uint256 total = nebuLib.getMultiple(_intervals,tokFee);
	...
	function checkAvaxSend(address _account,uint256 _sent,uint256 _intervals) internal returns (uint256,uint256,uint256){
		...
		require(nebuLib.addressInList(protoOwners,_account) == true,"you are not in the protolist"); 
		(uint256 unusedIntervals,uint256 _intervals,uint256 newFee,uint256 balanceReturn) = nebuLib.doAllMultiple(_intervals,_sent, 74000000);
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		_protoowners.push(PROTOOWNERS({
			...
			protoFeeFroze:Zero,
			protoTotalPayable:nebuLib.getMultiple(feePeriod,maxFeePayment.sub(feePeriod))
		}));		
