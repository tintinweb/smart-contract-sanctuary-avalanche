...
contract Marketplace is GameFiat, Withdrawable {
    ...
    function calFee(address game, string memory fee, uint256 price) public view returns (uint256) {
        return price.mul(games[game].fees[fee].percent).div(PERCENT);
    }

    function calPrice(address game, uint256 orderId) public view returns (
        address,
        uint256,
        address[] memory,
        address,
    ...
    function _resetPrice(address game, uint256 orderId) private {
        Price storage _price = games[game].tokenPrice[orderId];
        _price.maker = address(0);
        _price.price = 0;
        _price.buyByFiat = address(0);
        _price.isBuy = false;
        games[game].tokenPrice[orderId] = _price;
        emit ResetPrice(game, orderId);
    }

    function removePrice(address game, uint256 orderId) public onlyMaker(game, orderId) {
