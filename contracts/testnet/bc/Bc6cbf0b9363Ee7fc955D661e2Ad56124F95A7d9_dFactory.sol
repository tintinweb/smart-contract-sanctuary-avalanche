...
contract dFactory {
  ...
  function createEdition(
    ...
    Collection memory x = Collection(_name, newContract, newId, msg.sender);
    collection.push(x);
    collectionCount ++;
    return newId;
  }
...
contract erc721Dynamic is ERC721Upgradeable, IEditionSingleMintable, IERC2981Upgradeable, OwnableUpgradeable {
  ...
  event WhitelistStatus(bool paused, uint time);  

  event MaxMintChanged(uint256 maxMint);
  event PublicMintEnabled(bool enabled);
  CountersUpgradeable.Counter public atTokenId;
...
constructor(
 ...
 function enablePublicMint() public onlyOwner {
        ...
        emit PublicMintEnabled(publicMintActive);
    }
 ...
 function disablePublicMint() public onlyOwner {
  ...
  function publicPurchase() external payable returns (uint256) {
    require(!paused, "Down the rabbit hole. Contract is paused!");
    require(publicMintActive,"Public mint not enabled" );
    require(atTokenId.current() <= (editionSize - maxMint), "Max mint limit per wallet reached!");
