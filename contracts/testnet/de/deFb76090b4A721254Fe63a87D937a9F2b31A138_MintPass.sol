...
*/
contract MintPass is ERC165, IERC721, IERC721Metadata, Ownable{
    ...
    uint256 private _basePrice = 0.1 ether;
    uint256 private _priceAbove2000 = 0.2 ether;
    uint256 private _priceAbove3500 = 0.3 ether;
    uint256 private _priceAbove4500 = 0.4 ether;
    uint256 private _priceAbove5000 = 0.5 ether;

    // Minting paused: 0, WL minting: 1, Public minting: 2
  ...
  function symbol() external override view returns (string memory symbol_ret){
    ...
    */
    ...
    function mintPublic() public payable {
      ...
      require(mintingPhase == 2, "Contract is not in public mint mode");
    ...
    function mintWhitelist() public payable {
      uint256 supply = totalSupply();
      require(mintingPhase == 1, "Contract is not in whitelist mint mode");
      require(supply + 1 <= MAX_CAP, "Mint amount exceeds max supply");
      ...
      if(nextId > 5000) {
        price = _priceAbove5000;
      }
      else if(nextId > 4500) {
        price = _priceAbove4500;
      }
      else if(nextId > 3500) {
        price = _priceAbove3500;
      }
      else if(nextId > 2000) {
        price = _priceAbove2000;
      }
      else {
        price = _basePrice;
      }
    ...
    function ownerBatchMint(
      ...
      //Reduce number of allocation
      cheapMint(msg.sender, _mintingAmount);
