...
interface I_YieldKey is IERC721Upgradeable {
...
interface I_StakedYieldKey is I_YieldKey {
...
interface I_RegularYieldKey is I_YieldKey {
...
interface I_Router {
    function getAmountsOut(uint256 amount, address[2] memory _pair)
        external
        view
    ...
    ReentrancyGuardUpgradeable
{
    ...
    I_YieldKey private ValiFiNFTv3;
    I_StakedYieldKey private LegYieldKey;
    I_StakedYieldKey private RegYieldKey;
    I_RegularYieldKey private YieldKey;
    I_Presale private Presale1;
    I_Presale private Presale2;
    I_PresaleRefund private Presale3;
    I_Router private router;
    // Role Reward claimer
    ...
    /** @dev USD-pegged ERC20 stable coin our current liquidity pool pair values */
    ...
    /** @dev Weighted value Between VaLiFi and StableCoin */
    uint256 private Sval; // StableCoin Weighted Value
    ...
    function initialize(
        ...
        ValiFiNFTv3 = I_YieldKey(_configAddresses[2]);
        YieldKey = I_RegularYieldKey(_configAddresses[3]);
        LegYieldKey = I_StakedYieldKey(_configAddresses[4]);
        RegYieldKey = I_StakedYieldKey(_configAddresses[5]);
        Presale1 = I_Presale(_configAddresses[6]);
        Presale2 = I_Presale(_configAddresses[7]);
        Presale3 = I_PresaleRefund(_configAddresses[8]);
        router = I_Router(_configAddresses[9]);
    ...
    modifier claimer(address _claimer) {
        if (
            ...
            revert("Your are not elegible to claim YieldBoxes");
        }
        _;
    }
    ...
    /**
     * @dev Function to claim the YieldBoxes purchased by the user during the Presale Events
     ...
     */
    function create(address _currency) external nonReentrant whenNotPaused {
        require(
            TOTAL_SUPPLY >= totalSupply().add(uint256(1)),
            "ERC721 ValiFi: Can't mint YieldBoxes, maximum supply exceeded"
        );
        require(
        ...
        require(
            ...
            "ERC721 ValiFi: No available slots to create YieldBoxes, maximum capacity amount reached"
        );

        IERC20Upgradeable _currency2 = IERC20Upgradeable(_currency);
        uint256 _stableamount = mulDiv(Sval, _getVaLiFiPriceinStable(), 100);
    ...
    /**
     * @dev This function is to verify if a token address is valid to create YieldBoxes.
    ...
    /**
     * @dev This method allows to set the Token Base URI.
     */
    ...
    /**
     * @dev This function allows to set the USD-pegged stable currency and ValiFi token values to create YieldBoxes
     ...
     * @param _Sval Weighted value at 100% in USD-pegged stable currency to create YieldBoxes
     * @param _Vval Weighted value at 100% in ValiFi Token to create YieldBoxes
     */
    function setTokenValue(
        ...
        require(
            _value[0] >= 0 && // Weighted value at 100%, for treasury wallet in stable coin
                _value[1] >= 0 && // Weighted value at 100%, for liquidity wallet in stable coin
                _value[2] >= 0 && // Weighted value at 100%, for rewards wallet in ValiFi coin
                _value[3] >= 0 && // Weighted value at 100%, for liquidity wallet in ValiFi token
                _value[4] >= 0 && // Weighted value at 100%, for payroll wallet in ValiFi token
                _value[0] + _value[1] <= 100 && // Sum of all values in stable coin must be equal or less than 100
                _value[2] + _value[3] + _value[4] <= 100 && // Sum of all values in ValiFi coin must be equal or less than 100
								_Sval + _Vval <= 100, // Sum Weighted value between stable coin and ValiFi coin must be equal or less than 100
            "ERC721 ValiFi: Can't set value, value must be greater than 0 and sum of all values of VaLiFi and Stable Coin must be less than or equal to 100"
        );
    ...
    /**
     * @dev This method allows to set the config addresses used in the ecosystem
    ...
    /**
     * @dev This method allows to set the ERC20 ValiFi Token address
    ...
    /**
     * @dev This method allows to set rewards as claimed
        ...
        returns (uint256 result)
    {
        ...
        return
            (ValiFiNFTv3.balanceOf(_userAddress) * 12) +
    ...
    /**
     * @dev Method to Get Price of VALiFi in USD-pegged stable coin baed on current liquidity pool pair values
     * @return price Price in USD-pegged stable coin
     */

    function _getVaLiFiPriceinStable() private view returns (uint256 price) {
        uint256[2] memory amountsOut = router.getAmountsOut(
            (1 * 10**18),
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !paused(),
            "ERC721Pausable: can't create YieldBox while paused"
        );
        require(
            !isBlacklisted(to),
