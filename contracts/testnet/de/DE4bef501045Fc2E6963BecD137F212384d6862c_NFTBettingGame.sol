...
contract NFTBettingGame is ERC721, Ownable {
    ...
    uint256 public constant BETTING_FEE = 0.1 ether;
    ...
    uint256 public constant TREASURY_FEE_PERCENTAGE = 10; // 5% of total pool amount is kept in treasury pool
    ...
    function pickWinner() public onlyOwner {
        ...
        uint256[] memory topWinners = new uint256[](10);
        uint256 i;
        uint256 j;
        uint256 maxAmount;
        for (i = 0; i < 10; i++) {
            for (j = 0; j < tokenIdList.length; j++) {
                address bettor = ownerOf(tokenIdList[j]);
                uint256 amount = betAmounts[tokenIdList[j]][bettor];
                if (
                    amount > maxAmount && !contains(topWinners, tokenIdList[j])
                ) {
                    maxAmount = amount;
                    topWinners[i] = tokenIdList[j];
                    uint256 index = random() % tokenIdList.length;
                    winner = payable(ownerOf(tokenIdList[index]));
                    winners[winner].push(tokenIdList[index]);
                }
            }
            if (maxAmount == 0) {
                break;
        ...
        // Transfer the NFTs to the top 10 winners
        // for (i = 0; i < 10; i++) {
        //     if (topWinners[i] == 0) {
        //         break;
        //     }
        //     address topWinner = ownerOf(topWinners[i]);
        //     for (j = 0; j < winners[topWinner].length; j++) {
        //         if (winners[topWinner][j] == topWinners[i]) {
        //             _transfer(topWinner, msg.sender, topWinners[i]);
        //             break;
        //         }
        //     }
        // }
    ...
    function getRandomAddresses() public view returns (address[] memory) {
        require(
            addressesArray.length >= 10,
            "Not enough addresses in the array"
        );

        address[] memory randomAddresses = new address[](10);

        uint256[10] memory usedIndexes;
