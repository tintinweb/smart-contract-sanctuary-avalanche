...
contract MarginLoan is IMarginLoan {
    ...
    //     returns (bool)
    // {
    //     require(stableCoin.balanceOf(_user)>= _amount, "Margin Loan: Insufficient balance to pay loan");
    //     MarginLoan storage newMarginLoan = marginLoan[_user][_id];
        
        
    //     stableCoin.transferFrom(_user, newMarginLoan.bank, newMarginLoan.loanAmount);
       
    //     newMarginLoan.loanAmount -= _amount;
    //     newMarginLoan.status = LoanStatus.COMPLETE;
    //     emit UpdateLoan(_user, _id, newMarginLoan.status);
    //     emit PayLoan(_user, _id,_amount,_bank, newMarginLoan.status);
    //     return true;
    // }
    function completeLoan(address _user, uint256 _id)
        public
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < loanLength; i++) {
            ...
            return (loans,loanAmount,tokensAmount);
    ...
    // function payLoan(address _bank, uint256 _amount, uint256 _loanId) public override returns (uint256){
       
    //     require(stableCoin.balanceOf(msg.sender)>= _amount, "MarginLoan: Insufficient balance");
        
    //     stableCoin.transferFrom(msg.sender, _bank, _amount);

    //     MarginLoan storage newMarginLoan = marginLoan[msg.sender][_loanId];
