...
contract OmniFree is Ownable, ERC721A, NonblockingReceiver {
    ...
    string public unRevealedURI = "https://ipfs.io/ipfs/";
    ...
    constructor(string memory baseURI_, address _layerZeroEndpoint, uint256 _nextTokenId, uint256 _maxTokenId, uint256 _price) ERC721A("OmniFREE", "OF") { 
        _owner = msg.sender;
    ...
    function mint(address _to, uint256 _count) external payable {
        ...
        require(supply + _count <= MAX_NETWORK_MINT, "Exceeds maximum supply");
        require(_count > 0, "Minimum 1 NFT has to be minted per transaction");

        if (msg.sender != owner() && (nextTokenId + _count) > freeMintAmount) {
