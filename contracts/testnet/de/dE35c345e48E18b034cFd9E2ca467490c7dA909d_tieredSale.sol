    ...
    ClampedRandomizer
{
    ...
    uint256[] private tierPrice = [0, 400, 250, 0];
    ...
    function tieredMint(uint256 quantity, uint256 tier) public payable {
        require(wlTier[msg.sender] == tier, "wrong tier");
        require(privateSale(), "private sale hasn't started");
        require(publicMints[msg.sender] + quantity <= tierMints[tier], "minted max");
        require(msg.value >= (tierPrice[tier] * quantity * 10**15), "Not enough AVAX");
        publicMints[msg.sender] += quantity;
        require(totalSupply() + quantity < _maxMintable, "minting more than max supply");
        require(!paused, "not active.");
