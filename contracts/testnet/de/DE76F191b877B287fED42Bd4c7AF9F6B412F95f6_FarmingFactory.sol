...
contract Farming is OwnableUpgradeable {
    ...
    uint256 public ATPS_PRECISION = 1e36;
        ...
        )
    {
        ...
        if (address(avatToken) != address(0)) {
            ...
            if (block.timestamp > lastRewardTimestamp && lpSupply != 0) {
                uint256 reward = farmingFactory.countRewardAmount(lastRewardTimestamp, block.timestamp);
                accTokenPerShare = accTokenPerShare.add(reward.mul(ATPS_PRECISION).div(lpSupply));
            }
    ...
    function _harvest() internal {
        ...
        if (pendingBonus != 0) {
            bonusToken.safeTransfer(msg.sender, pendingBonus);
        }
...
contract FarmingFactory is Ownable {
    ...
    event DeployFarming(address indexed farmingAddress, address indexed lpToken, address indexed bonusToken, uint256 bonusTokenPerSec, uint256 startTimestamp);
    event UpdateDistributionV2Address(address indexed newAddress);
    event MintTokens(address indexed to, uint256 amount);

    constructor(
        address avatToken_,
        address distributionV2_
    ) {
    ...
    function deployFarming(
        ...
        Farming(clone).initialize(msg.sender, address(avatToken), lpToken_, bonusToken_, bonusTokenPerSec_, startTimestamp_);
        ...
        emit DeployFarming(clone, lpToken_, bonusToken_, bonusTokenPerSec_, startTimestamp_);
        return clone;
    }
    ...
    function countRewardAmount(uint256 start_, uint256 end_) external view returns (uint256) {
        ...
        uint farmingReward = reward.mul(farmingAllocationPoint[msg.sender]).div(totalFarmingAllocationPoint);
