...
// TODO: Trim the name before allow to add 

contract RoostrNameService is Ownable {
    address public EGG_CONTRACT; // egg contract
    address public ROOSTR_CONTRACT; // roostr nft contract
    
    // rns variables
    uint256 public RNS_BASE_PRICE = 999999000000000000000000; // number of egg required to change the name
    ...
    // events
    event RNSChange(uint256 tokenId, string previousName, string name);
    event PersonalityChange(uint256 tokenId, string lore, string flavour);

    constructor(address _eggContract, address _roostrContract) {
    ...
    function rnsNameChange(uint256 tokenId, string memory name) external {
        ...
        string memory currentRoostrName = x.rns(tokenId);
        bytes memory byteName = bytes(currentRoostrName);
        if (byteName.length > 0) {
            // assume Roostr have already been name
            bytes32 oldName = keccak256(
                abi.encodePacked(_toLower(currentRoostrName))
            );
            delete rnsAlreadyRegisted[oldName];
        }
        // register the unique Roostr name
        rnsAlreadyRegisted[hashedInput] = true;
        // burn the egg
        eggContract.burnEggs(msg.sender, RNS_BASE_PRICE);
        // and update the Roostr contract
        x.changeName(tokenId, name);
        emit RNSChange(tokenId, currentRoostrName, name);
    }

    // changing the Roostr Personality
    ...
    // update rns and personality in one function
    // this function work out if rns is empty or personality is empty so calculate the right price
    function nameAndPersonalityChange(
        ...
        if (keccak256(abi.encodePacked("")) != hashedName) {
            price += RNS_BASE_PRICE;
            // validate name
            isRNSNameAvailable(name);
            setName = true;
        }
        ...
        if (setName) {
            ...
            rnsAlreadyRegisted[hashedName] = true;
            // and update the Roostr contract
            x.changeName(tokenId, name);
            emit RNSChange(tokenId, currentRoostrName, name);
        }
...
contract Roostr is ERC721Upgradeable {
    address private _owner;
    ...
    event NameChange(uint256 roostrNum, string name);
    ...
    // add roostr naming service
    mapping(uint256 => string) public rns;
