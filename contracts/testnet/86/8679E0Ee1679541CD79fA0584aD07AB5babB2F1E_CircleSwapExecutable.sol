...
contract CircleSwapExecutable is IAxelarForecallable, Ownable {
    ...
    function _tradeDest(
        ...
        (bool _success, bytes memory _result) = router.call(data);
        if (!_success) {
            return (false);
        }
        ...
        if (wrappedNativeTokenOutput > 0) {
            uint256[] memory amounts = abi.decode(_result, (uint256[]));
            uint256 wrappedNativeAmount = amounts[amounts.length - 1];
            ...
            bool sent = fallbackRecipient.send(wrappedNativeAmount);
            return (sent);
        }
        return (true);
    }
