/**
 *Submitted for verification at testnet.snowtrace.io on 2023-01-08
*/
...
// File contracts/token/ERC20/AdminERC20.sol

pragma solidity ^0.8.0;


contract AdminERC20 is Ownable, ERC20 {
    string public _name;
...
// File contracts/token/ERC20/NativeTokenRamp.sol

pragma solidity 0.8.13;

abstract contract ReadyERC20 is AdminERC20 {
  constructor(address _token) AdminERC20(ERC20(_token).name(),ERC20(_token).symbol()){}
}
...
abstract contract TokenPool is PoolInterface, Ownable {
   ...
   */
  function _validateOnRamp(address check) internal view {
    if (!isOnRamp(check)) revert PermissionsError();
  }
  /**
   * @notice Checks whether the msg.sender is either the owner, or a permissioned offRamp on this contract
   * @dev Reverts with a PermissionsError if check fails
   */
  function _validateOffRamp(address check) internal view {
    if (!isOffRamp(check)) revert PermissionsError();
  }

  /**
   * @notice Check permissions and limits of a lock or burn
   */
  modifier assertLockOrBurn(address check) {
    _validateOnRamp(check);
    _;
  }
   ...
   * @notice Check permissions and limits of a lock or burn
   */
  modifier assertReleaseOrMint(address check) {
    _validateOffRamp(check);
    _;
  }
}
 ...
 */
interface OnRamp is OnRampData {
...
interface OffRamp is OffRampData {
  /**
...
abstract contract Allowlist {
  ...
  event SetAllow(address indexed sender, bool value, bytes data);

  bool internal allowFlag;

  mapping(bytes => bool) internal allowlist;

  function _setAllow(bool value, bytes memory data) internal {
    allowlist[data] = value;
    emit SetAllow(msg.sender, value, data);
  }

  function _validateAllow(bytes memory data) internal view {
    if (allowFlag && !allowlist[data]) revert AllowError();
  }

  function _setAllowFlag(bool value) internal {
    allowFlag = value;
  }
   ...
   */
  modifier assertAllow(bytes memory check) {
    _validateAllow(check);
    _;
  }
}

abstract contract TokenLimiter {
  error ExceedsTokenLimit(uint256 currentLimit, uint256 requested);

  event SetLimit(address indexed sender, uint256 limit);

  uint256 internal currentLimit;

  function _setLimit(uint256 limit) internal {
    currentLimit = limit;
    emit SetLimit(msg.sender, limit);
  }
  ...
  function _validateLimit(uint256 limit) internal view {
    if( !(limit <= currentLimit) ) revert ExceedsTokenLimit(currentLimit, limit);
  }

  /**
   * @notice Check permissions
   */
  modifier assertLimit(uint256 limit) {
    _validateLimit(limit);
    _;
  }
}

contract NativeTokenRamp is TokenPool, OffRampData, OnRampData, Allowlist, TokenLimiter, ReadyERC20 {
  error NotEnoughBalance();

  event Moved(address indexed sender, uint256 amount);
  event Locked(address indexed sender, uint256 amount);
  event ccipReceived(address indexed sender, address indexed recipient, uint256 amount);
  event Transport(address sender, uint256 amount, uint256 destChainId);
  ...
  constructor(address token) TokenPool(IERC20(token)) ReadyERC20(token) {}

  /**
   * @notice Transports a token to another chain
   * @param amount Amount to send
   * @param destAddress destination address for the amount
   * @param onRamp is the message router to be used
   * @param gasLimit used in CCIP to limit gas usage
   */
  function transport(
      uint256 amount,
      address destUser, 
      ...
      uint256 gasLimit) 
        external 
          assertLockOrBurn(onRamp) 
          assertAllow(abi.encode(destChainId))
          assertAllow(abi.encode(destAddress))
          assertLimit(amount)
    {
    ...
    Payload memory data = Payload(destUser, amount);

    EVM2AnySubscriptionMessage memory message = EVM2AnySubscriptionMessage(
    ...
    OnRamp(onRamp).ccipSend(destChainId, message);
    emit Transport(msg.sender, amount, destChainId);
  }

  /**
   * @notice CCIP Releases tokens from the pool to the recipient
   * @param message Any2EVMMessage struct containing amount and recipient
   */
  function ccipReceive(Any2EVMMessage calldata message) external assertReleaseOrMint(msg.sender) {
    Payload memory data = abi.decode(message.data, (Payload) );
    if(getToken().balanceOf(address(this)) < data.amount) {
      // mint intermediary token.
      _mint(data.receiver, data.amount);
    } else {
      getToken().transfer(data.receiver, data.amount);
    }
    emit ccipReceived(msg.sender, data.receiver, data.amount);
  }

  function completeTransport(address destUser, uint256 amount) external {
    if(getToken().balanceOf(address(this)) >= amount || balanceOf(destUser) < amount){
      revert NotEnoughBalance();
    }
    getToken().transfer(destUser, amount);
    _burn(destUser, amount);
  }
  ...
  function setAllowFlag(bool value) external onlyOwner {
    _setAllowFlag(value);
  }

  function setAllow(bool value, bytes memory data) external onlyOwner {
    _setAllow(value, data);
  }
}