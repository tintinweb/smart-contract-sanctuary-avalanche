...
contract HatchFactory is Ownable {
    uint256 public cost = 0.1 ether;

    bool public paused = false;
    bool inside;

    address factory = 0x9d4B301cd3CcC03a555F91E86D6d0e9626908BA0;
    ...
    function hatchLimited(
        ...
        tokenAddress.transferOwnership(msg.sender);
        emit NewHatchLimited(name, symbol, tokenAddress, msg.sender);
    }

    function hatchUnlimited(string memory name, string memory symbol)
        external
        payable
    {
        require(paused, "Contract is paused");
        require(msg.value >= cost);
        HatchUnlimited tokenAddress = new HatchUnlimited(name, symbol);
        tokenAddress.transferOwnership(msg.sender);
        emit NewHatchUnlimited(name, symbol, tokenAddress, msg.sender);
    }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        sendEth(factory, address(this).balance);
        inside = false;
    }
}
...
contract HatchUnlimited is ERC721URIStorage, Ownable {
    ...
    function hatch(string[] memory tokenUriList) external onlyOwner {
...
contract HatchLimited is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    address factory = 0x234FD60E54E6580fb6951f3ce974aA57E1bEd3dE;
    ...
    function hatch(uint256 _mintAmount) public payable nonReentrant {
        require(saleActive, "Presale isn't active");
        require(
            _mintAmount > 0 && _mintAmount <= maxMintAmount,
            "Too many NFTs to mint"
        );
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply, "Not enough NFTs available");
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        sendEth(factory, percent * 1);
        sendEth(address(this), address(this).balance);
        inside = false;
    }
}
