...
interface Random {
    function randomDays(uint256 _daysMax) external view returns (uint256);
    function randomPerc(uint256 _percMax) external view returns (uint256);
}
...
contract TheWheelOfReturns is Ownable {
    ...
    uint256 private constant PERC_MAX_FOR_RANDOM = 4;  // to generate random number from 5 - 8

    uint256 private constant TOTAL_PERC = 1000;
    uint256 private constant PRIMARY_BENIFICIARY_PERC = 100; //10%
    ...
    constructor(address payable _primaryBenificiaryAddress,address _randomContractAddress) {
        require(
    ...
    function _invest(
        ...
        uint256 day = randomContract.randomDays(DAYS_MAX_FOR_RANDOM);
        uint256 perc = randomContract.randomPerc(PERC_MAX_FOR_RANDOM).mul(10);
