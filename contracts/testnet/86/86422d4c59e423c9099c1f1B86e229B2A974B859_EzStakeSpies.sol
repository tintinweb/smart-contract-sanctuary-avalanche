...
contract EzStakeSpies is Ownable, ReentrancyGuard {
    IERC721 NFT;
    IEspionage Espionage;
    ...
    constructor(address _nft, address _reward, uint _firstRate) {
        NFT = IERC721(_nft);
        Espionage = IEspionage(_reward);
        rate.push(_firstRate);
    ...
    function stake(uint[] memory tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            tokenInfo[tokenIds[i]] = stakeInfo(
                msg.sender,
                block.timestamp,
                userStaked[msg.sender].length,
                rate.length - 1
            );
    ...
    function unStake(uint[] memory tokenIds) external nonReentrant {
    ...
    function claimRewards(uint[] memory tokenIds) public {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            tokenInfo[tokenIds[i]].lastClaim =
                block.timestamp -
                ((block.timestamp - tokenInfo[tokenIds[i]].lastClaim) %
                    duration);
            tokenInfo[tokenIds[i]].rateIndex = rate.length - 1;
        }
        Espionage.mint(msg.sender, amount);
    }

    function getRewards(uint _id) public view returns (uint) {
        stakeInfo storage userInfo = tokenInfo[_id];
        ...
        for (uint i = userInfo.rateIndex; i < rate.length; i++) {
            if (userInfo.lastClaim < time[i]) {
                if (collected == 0) {
                    collected +=
                        ((time[i] - userInfo.lastClaim) / duration) *
                        rate[i - 1];
                } else {
                    collected +=
                        ((time[i] - time[i - 1]) / duration) *
                        rate[i - 1];
                }
            }
            currentTime = i;
        ...
        if (collected == 0) {
            collected +=
                ((block.timestamp - userInfo.lastClaim) / duration) *
                rate[currentTime];
        } else {
            collected +=
                ((block.timestamp - time[currentTime]) / duration) *
                rate[currentTime];
        }
        return collected;
    }
    ...
    function popToken(uint tokenId) private {
        uint lastToken = userStaked[msg.sender][
            userStaked[msg.sender].length - 1
        ];
    ...
    function setNFT(address _newNftAddress) external onlyOwner {
        NFT = IERC721(_newNftAddress);
    }

    function setRewardToken(address _rewardTokenAddress) external onlyOwner {
        Espionage = IEspionage(_rewardTokenAddress);
    }

    function retrieveRewardToken(address _to) external onlyOwner {
        Espionage.transfer(_to, Espionage.balanceOf(address(this)));
    }
}