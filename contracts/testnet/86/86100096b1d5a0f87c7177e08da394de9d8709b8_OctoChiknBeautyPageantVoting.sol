...
// File: Octo/Test8OctosEggVoter.sol
...
interface IvotePostrs {//voters mint postrs with each new candidate voted for
...
contract OctoChiknBeautyPageantVoting is Ownable, ReentrancyGuard 
{
  ...
  address public EggContract = 0x252e1137a18900C72fEC17B88825D32d08b2A4b4; //Currently set to testnet
  ...
  address public EggBurnPartner = 0x676C7ddcEC36edf7c1290CeEed3AeE696a8f57c1; //Currently set to testnet
  ...
  address public votePostrsAddress;//contract address of our external postr nft smart contract
...
*/
  ...
  function testVoteForOneMinute() public onlyOwner {
    startofVotingTime = block.timestamp;
    endofVotingTime = block.timestamp+60;
  }

  function testVoteForOnehour() public onlyOwner {
    startofVotingTime = block.timestamp;
...
////main vote code:
  ...
  public timeNotStarted timeIsOver nonReentrant returns(bool) {
    ...
    if (addressTotalVotesForIdNumber[msg.sender][_candidateIDNumber] == 0){//if they are a new voter for that ID number then they get a sticker for voting
      IvotePostrs(votePostrsAddress).voterMint(msg.sender, IDStringToNumberedIDs[_candidateIDName]);//send voter wallet and who they voted for of 1-32 contestants
    } 
