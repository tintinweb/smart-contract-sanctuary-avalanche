...
abstract contract feeManager is Context {
    ...
    function reconcileFees(address _account,uint256 _x) virtual external;
    function reconcileAccountFees(address _account) virtual external;
...
contract NeFiFeeManager is Ownable{
	...
	modifier whiteOnly() {require(NeFiLib.addressInList(blacklist,msg.sender)== false, "sorry, i hope you had time to think about what you did"); _;}
	modifier managerOnly() {require(NeFiLib.addressInList(Managers,msg.sender)== true,"manager access only"); _;}
	...
	//toFeeMGR------------------------------------------------------------------------------------------------------------------
	//SpecFeesToken
	function sendFeeTokenSpec(uint256 _intervals, uint256 _x) payable whiteOnly() external{
		//preliminaries
    	...
    	//SpecFeesAvax
    	function payFeeAvaxSpec(uint256 _intervals,uint _x) payable  whiteOnly() external{
    		//preliminaries
    	...
    	//FeesToken
	function sendFeeToken(uint256 _intervals) payable whiteOnly() external{
    		//preliminaries
    		require(feeMGRpayOff == false, "sorry feeMGR pay if off currently");
    		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		address _account = msg.sender;
	...
	//FeesAvax
    	function sendFeeAvax(uint256 _intervals) payable whiteOnly() external{
    		//preliminaries
    		require(feeMGRpayOff == false, "sorry feeMGR pay if off currently");
    		require(avaxOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
    		address _account = msg.sender;
    	...
    	//InHouse------------------------------------------------------------------------------------------------------------------
	//SpecFeesToken
	function ProtoFeesTokenSpec(uint256 _intervals, uint256 _x) payable whiteOnly() external{
		//preliminaries
		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		address _account = msg.sender;
    		//calculation
	...
	//FeesAvaxSpec
	function protoFeesAvaxSpec(uint256 _intervals,uint256 _x) payable whiteOnly() external{
		//preliminaries
		require(tokOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
		address _account = msg.sender;
    		uint256 sent = msg.value;
		 //calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(NeFiLib.getLower(getTotalPayable(_account),_intervals),msg.value,over.getFee());
		//verification
		require(_nIntervals != 0 && needed > 0,"you do not hold enough to cover this claim");
		require(_nIntervals <= _intervals,"you do not hold enough to cover this claim");
	        //bookKeeping
	...
	//FeesToken
	function protoFeesToken(uint256 _intervals) payable whiteOnly() external{
		//preliminaries
		require(tokOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
    		address _account = msg.sender;
	...
	//FeesAvax
	function protoFeesAvax(uint256 _intervals) payable whiteOnly() external{
		//preliminaries
		require(tokOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
		address _account = msg.sender;
		 //calculation
	...
	//ProtoClaim  ------------------------------------------------------------------------------------------------------------------------
	//ProtoClaimToken
    	function createProtoFeeTok(string memory _name) payable whiteOnly() external{
    		//preliminaries
    		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		address _account = msg.sender;
	...
	//ProtoClaimAvax
	function createProtoAvax(string memory _name) payable whiteOnly() external {
		//preliminaries
		require(avaxOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
    		address _account = msg.sender;	
    		uint256 _intervals = 1;
	...
	function getTotalPayableSpec(address _account,uint256 _x) internal returns(uint256){
		if(feeMGR.getProtoownersLength(_account) != 0 && feeMGR.getProtoownersLength(_account) ==_x){
			feeMGR.reconcileFees(_account,_x);
		}
		uint256 fees = feeMGR.getProtoTotalPayable(_account,_x);
		if(fees == 0 && feeMGR.getProtoownersLength(_account)==0){
			return 12;
		}else{
			return fees;
		}
	...
	function getTotalPayable(address _account) internal returns(uint256){
		if(feeMGR.getTotalPayable(_account) != 0){
			feeMGR.reconcileAccountFees(_account);
		}
		uint256 fees = feeMGR.getTotalPayable(_account);
		if(fees == 0 && feeMGR.getProtoownersLength(_account)==0){
			return 12;
		}else{
			return fees;
	...
	function removeManagers(address newVal) external managerOnly() {
    		if(NeFiLib.addressInList(Managers,newVal) ==true && newVal != owner()){
    	...
    	function removeFromBlack(address newVal) external managerOnly() {
    		if(NeFiLib.addressInList(blacklist,newVal) ==true){
    			uint _i = NeFiLib.isInList(blacklist,newVal);
    			uint len = blacklist.length-1;
    			blacklist.push();
    			for(uint i=_i;i<len;i++){
    				uint _i_ = i +1;
    				blacklist[i] = blacklist[_i_];
    			}
    			blacklist.pop();
        	}
    	}
    	function addtoblacklistPrice(address _account) external onlyOwner(){
    		if(NeFiLib.addressInList(blacklist,_account) ==false && _account != owner()){
