...
interface IPortal {
  ...
  event ProviderError(CrossChainProvider.CrossChainProvider provider, bytes reason);
...
interface ISynchronizer {
  event SynchronizerApplicationError(SyncAction.SyncAction action, bytes reason);
  event PortalError(SyncAction.SyncAction action, bytes reason);
