    ...
    ERC2981Upgradeable
{
    ...
    /// @notice maximum amount of different stamps
    ...
    /// @param stampIds List of Stamps lists to mint per address
    function seedAllowlist(
        address[] calldata _addresses,
        uint256[][] calldata stampIds
    ) external override onlyOwner {
        uint256 addressesLength = _addresses.length;
        console.log("addressesLength = %s", addressesLength);
        ...
        for (uint256 i; i < addressesLength; i++) {
            ...
            for (uint256 j; j < amount; j++ ){
                ...
                emit PreMint(_addresses[i], stampIds[i][j], j);
                console.log("i = %s, j = %s, stampIds[i][j] = %s", i, j, stampIds[i][j]);
            }
        }
        console.log("%s %s",stampAllowlist[_addresses[0]].at(0),stampAllowlist[_addresses[0]].at(1));
        console.log("%s %s %s",stampAllowlist[_addresses[0]].at(2),stampAllowlist[_addresses[0]].at(3),stampAllowlist[_addresses[0]].at(4));
    ...
    function devMint(uint256 _quantity)
        ...
        whenNotPaused
    {
        revert Launchpeg__OnlyAllowlistMint();
    }
        ...
        atPhase(Phase.Allowlist)
    {
        ...
        console.log("inside allowlist mint");
        ...
        // console.log("0-%s 1-%s 2-%s",stampsList[0],stampsList[1],stampsList[2]);
        // console.log("3-%s 4-%s",stampsList[3],stampsList[4]);
    ...
    function burn(uint256 _id, uint256 _amount) external {
        if (balanceOf(_msgSender(), uint8(_id)) == 0) {
            revert Launchpeg__ZeroTokenBalance(uint8(_id));
        }
        if (balanceOf(_msgSender(), uint8(_id)) < _amount) {
            revert Launchpeg__AmountExceedsBalance(balanceOf(_msgSender(), uint8(_id)));
        }

        _burn(_msgSender(), uint8(_id), _amount);
    }
    ...
    // ) private {
    //     uint256[] memory senderStamps = allowlist[_sender];
    //     if (senderStamps.length != _quantity) revert Launchpeg__InvalidQuantity();
    //     for (uint256 i; i < _quantity; ++i) {
    //         _mint(_sender, senderStamps[i], _maxBatchSize, "");
    //     }
    // }
    ...
    function _numberMinted(address owner) internal view returns (uint256){
        ...
        for (uint256 i; i < stampsAmount; i++){
            numberOwned += IBaseLaunchpeg(address(this)).balanceOf(owner, i);
        }
        return numberOwned;
        }
...
error Launchpeg__ZeroTokenBalance(uint8 id);
