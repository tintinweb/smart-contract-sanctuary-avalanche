...
// File contracts/Interfaces/IiMIM.sol

pragma solidity ^0.8.0;

interface IiMIM {
    event Deposit(address user, uint256 _smay);
    event Withdraw(address, uint256 _shares, uint256 _smay);
...
// File contracts/iMIM.sol

pragma solidity ^0.8.0;

contract iMIM is ERC20("iMIM", "iMIM"), ERC20Permit("iMIM"), IiMIM, Ownable {
    ...
    function getShareValue() external view override returns (uint256) {
        return totalSupply() > 0
            ? 1e18 * mim.balanceOf(address(this)) / totalSupply()
            : 1e18;
    }

    function deposit(uint256 _amount) public override {
        uint256 totalMIM = mim.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        // if user is first depositer, mint _amount of xMIM
        if (totalShares == 0 || totalMIM == 0) {
            _mint(msg.sender, _amount);
        } else {
            // loss of precision if totalMIM is significantly greater than totalShares
            // seeding the pool with decent amount of MIM prevents this
            uint256 myShare = _amount * totalShares / totalMIM;
    ...
    function depositWithPermit(uint256 _amount, Permit calldata permit) external override {
        IERC20Permit(address(mim)).permit(
            permit.owner,
            permit.spender,
            permit.amount,
            permit.deadline,
    ...
    function withdraw(uint256 _share) external override {
        uint256 totalShares = totalSupply();
        uint256 shareInMIM = _share * mim.balanceOf(address(this)) / totalShares;
        _burn(msg.sender, _share);
        mim.safeTransfer(msg.sender, shareInMIM);
        emit Withdraw(msg.sender, _share, shareInMIM);
    }
    ...
    function collect(IERC20 _token) external override onlyOwner {
        if (totalSupply() > 0) {
            require(_token != mim, "iMIM: cannot collect MIM");
        }
        uint256 balance = _token.balanceOf(address(this));
        require(balance > 0, "iMIM: _token balance is 0");
