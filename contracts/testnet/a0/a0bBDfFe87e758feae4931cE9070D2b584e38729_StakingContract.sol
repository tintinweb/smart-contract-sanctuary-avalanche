...
contract StakingContract is Ownable {
    ...
    constructor(address _rTokenAddress, address _piggyTokenAddress) {
        RewardTokenPerBlock = 5 ether;
        rtoken = _rTokenAddress;
        piggyAddress = _piggyTokenAddress;
    }
    ...
    function changePiggyTokenAddress(address _piggyTokenAddress) public onlyOwner {
        piggyAddress = _piggyTokenAddress;
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721(piggyAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(piggyAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(piggyAddress).ownerOf(tokenIds[i]) != address(this)) continue;
            ...
            if(stakingAmount[msg.sender] > 0)
                stakingAmount[msg.sender] = stakingAmount[msg.sender] - 1;
            IERC721(piggyAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
