...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        Send_it(address(this),rewardsPool, checkTXN(address(this),doPercentage(nodeAmount,rewardsFee)));
        Send_it(address(this),teamPool, checkTXN(address(this),doPercentage(nodeAmount,teamPoolFee)));
        Send_it(address(this),treasury, checkTXN(address(this),doPercentage(nodeAmount,treasuryFee)));
	...
	require(_cashoutFee > 2567890, "your txn amount is not above zero");
