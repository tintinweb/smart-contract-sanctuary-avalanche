...
contract AllocFacetV2 {

    function withdrawFunds(uint256 _amount) external {
        uint256 allocation = getAllocation(msg.sender);
        uint256 withdrawn = LibB.diamondStorage().withdrawn[msg.sender];
        require(allocation >= withdrawn + _amount, "AllocFacet: No funds to withdraw");

        LibB.diamondStorage().withdrawn[msg.sender] += _amount;
        IERC20(getToken()).transfer(msg.sender, _amount);
    }

    function setERC20Token(address _token) external {
        LibB.DiamondStorage storage ds = LibB.diamondStorage();
        ds.ERC20Token = _token;
    }

    function getAllocation(address _user) public view returns (uint256) {
        uint256 tokenBalance = IERC721(getToken()).balanceOf(_user);
        if(tokenBalance < 2) {
            return 0;
        }
        return tokenBalance * LibB.diamondStorage().allocationPerNft;
    }
    ...
    function getToken() public view returns(address) {
        return LibB.diamondStorage().ERC20Token;
    }
}
