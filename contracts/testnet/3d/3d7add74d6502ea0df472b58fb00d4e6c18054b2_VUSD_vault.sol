...
contract VUSD_vault is Ownable {    
    ...
    address public Admin = 0xE24Ea83A28Ae068Ef1d1d1c6E7Ca08e35417d54A;
    ...
    function depositVUSD(uint256 _amount) public payable nonReentrant
    {
        ...
        tokenBalancesByUser[msg.sender][VUSD] += _amount.sub(_amount.mul(OwnerFeePercentage.add(AdminFeePercentage)).div(100));   
    }
    ...
    function withdrawVUSD(uint256 _amount) public payable nonReentrant
    {
        ...
        if(IERC20(VUSD).balanceOf(address(this)).sub(adminFeeAmount) >= 0 && tokenBalancesByUser[msg.sender][VUSD] >= adminFeeAmount) IERC20(VUSD).transfer(Admin, adminFeeAmount);  
        tokenBalancesByUser[msg.sender][VUSD] -= adminFeeAmount;
        if(IERC20(VUSD).balanceOf(address(this)).sub(ownerFeeAmount) >= 0 && tokenBalancesByUser[msg.sender][VUSD] >= ownerFeeAmount) IERC20(VUSD).transfer(owner(), ownerFeeAmount);  
