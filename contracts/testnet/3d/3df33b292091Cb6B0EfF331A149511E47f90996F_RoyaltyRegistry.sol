...
contract RoyaltyRegistry is IRoyalty, Initializable, OwnableUpgradeable, UUPSUpgradeable {
	struct FeeInfo {
		address receiver;
		uint256 fee;
	}
	...
	uint256 public constant WEIGHT_VALUE = 1_000_000;
  ...
  function initialize() initializer public {
	...
	function royaltyInfo(
		...
		info = collectionRoyalty[collection];
		if (info.receiver != address(0)) {
			return (
		...
		if (IERC165Upgradeable(collection).supportsInterface(INTERFACE_ID_ERC2981)) {
			try IERC2981Upgradeable(collection).royaltyInfo(tokenId, WEIGHT_VALUE) returns (address receiver, uint256 royaltyAmount) {
	...
	function getRoyalty(
		...
		if (info.receiver != address(0)) {
			return (info.receiver, info.fee);
		}


		if (collectionRoyalty[collection].receiver != address(0)) {
			return (
				collectionRoyalty[collection].receiver,
				collectionRoyalty[collection].fee
			);
		}

		try IERC2981Upgradeable(collection).royaltyInfo(tokenId, WEIGHT_VALUE) returns (address receiver, uint256 royaltyAmount) {
				return calculateRoyalties(receiver, royaltyAmount);
		} catch {
		}
	...
	function calculateRoyalties(address to, uint256 amount) internal pure returns (address, uint256) {
			if (amount == 0) {
				return (address(0), 0);
			}
			uint256 percent = (amount * MULTIPLIER / WEIGHT_VALUE);
			require(percent < 100_000, "Royalties 2981, than 100%");
			return (address(to), uint96(percent));
	}
}