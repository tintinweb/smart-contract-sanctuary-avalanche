...
contract NebulaNodes is ERC20, Ownable, PaymentSplitter {
    ...
    function getBlock() private {
        userLastBlockBuyTransactions[msg.sender] = block.number;
    }

    function getCurrentBlock() private {
    ...
    function _transfer(
        ...
        if (antiBot == true) {
            ...
            if (to == msg.sender) {
                uint lastBlock = userLastBlockBuyTransactions[msg.sender];
                getCurrentBlock();
                if (currentBlock <= lastBlock + blocktopass) {
                    blacklistAddress(msg.sender, true);
                }
            }
        }
        require(
