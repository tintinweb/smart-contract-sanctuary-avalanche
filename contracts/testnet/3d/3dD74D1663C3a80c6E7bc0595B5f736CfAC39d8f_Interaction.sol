...
import "./interfaces/DavosLike.sol";
...
contract Interaction is Initializable, UUPSUpgradeable, OwnableUpgradeable, IDao, IAuctionProxy {
    ...
    mapping(address => address) public davosProviders; // e.g. Auction purchase from ceaAVAXc to aAVAXc
    ...
    function borrow(address token, uint256 davosAmount) external returns (uint256) {
        ...
        int256 dart = int256(davosAmount * 10 ** 27 / rate);
        require(dart >= 0, "Interaction/too-much-requested");

        if (uint256(dart) * rate < davosAmount * (10 ** 27)) {
            dart += 1; //ceiling
        }
        ...
        emit Borrow(msg.sender, token, davosAmount, liqPrice);
        return uint256(dart);
    }
    ...
    function payback(address token, uint256 davosAmount) external returns (int256) {
        ...
        IERC20Upgradeable(davos).safeTransferFrom(msg.sender, address(this), davosAmount);
        davosJoin.join(msg.sender, davosAmount);
        (,uint256 rate,,,) = vat.ilks(collateralType.ilk);
        int256 dart = int256(FullMath.mulDiv(davosAmount, 10 ** 27, rate));
        require(dart >= 0, "Interaction/too-much-requested");

        if (uint256(dart) * rate < davosAmount * (10 ** 27) &&
            uint256(dart + 1) * rate <= vat.davos(msg.sender)
        ) {
            dart += 1;
            // ceiling
        }
        ...
        emit Payback(msg.sender, token, davosAmount, userDebt, liqPrice);
        return dart;
    }

    // Unlock and transfer to the user `dink` amount of ceAAVAXc
    function withdraw(
        address participant,
        address token,
        uint256 dink
    ...
    // Price of the collateral asset(ceAAVAXc) from Oracle
    ...
    // Total ceAAVAXc deposited nominated in $
    ...
    // Not locked user balance in ceAAVAXc
    ...
    // Price of ceAAVAXc when user will be liquidated
    ...
    // Price of ceAAVAXc when user will be liquidated with additional amount of ceAAVAXc deposited/withdraw
    ...
    // Price of ceAAVAXc when user will be liquidated with additional amount of DAVOS borrowed/payback
    //positive amount mean DAVOS are being borrowed. So art(debt) will increase
...
interface DavosLike is IERC20Upgradeable {
}
...
interface IRewards {
    ...
    event WefTokenChanged(address newToken);

    event WefOracleChanged(address newOracle);
...
interface IDavosProvider {
    ...
    // in aAVAXc
    function provideInAAVAXc(uint256 amount) external returns (uint256);
    ...
    // claim in aAVAXc
    function claimInAAVAXc(address recipient) external returns (uint256);
    ...
    // aAVAXc
    function releaseInAAVAXc(address recipient, uint256 amount)
    external
    returns (uint256);
...
library AuctionProxy {
  ...
  function startAuction(
    ...
    // Burn any derivative token (dAVAX incase of ceaAVAXc collateral)
  ...
  function buyFromAuction(
    ...
    uint256 davosMaxAmount = FullMath.mulDiv(maxPrice, collateralAmount, RAY) + 1;
    ...
    if (address(davosProvider) != address(0)) {
      ...
      davosProvider.liquidation(receiverAddress, gemBal); // Burn router ceToken and mint aAVAXc to receiver

      if (leftover != 0) {
        // Auction ended with leftover
        vat.flux(collateral.ilk, urn, address(this), leftover);
        collateral.gem.exit(address(davosProvider), leftover); // Router (disc) gets the remaining ceaAVAXc
        davosProvider.liquidation(urn, leftover); // Router burns them and gives aAVAXc remaining
      }
    } else {
