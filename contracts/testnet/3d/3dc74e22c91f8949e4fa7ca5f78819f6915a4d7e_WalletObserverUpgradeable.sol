...
PlanetsManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        setTimeframeQuotaIn(15_000_000 * (10**18));
        setTimeframeQuotaOut((15_000_000 / 10) * (10**18));
    ...
    returns (bool)
    {
        ...
        /*require(
            ...
            "WalletObserverUpgradeable: Cannot send directly tokens to dealersManager, use MedellinCity to create a planet (https://app.medellincity.cash)"
        );*/
        require(
        ...
        if (!isExcludedFromObserver(from)) {
            ...
            if (isSellOrLiquidityAdd) {
                ...
                _outTokens[timeframeCurrent][from] = uint256(getRemainingTransfersOutWithSellAllowance(from));
            } else {
                require(
    ...
    returns (int256)
    {
        uint256 remainingTransfersOutWithSellAllowance = (timeframeQuotaOut
        + getBoughtTokensOf(account))
        - _outTokens[timeframeCurrent][account];

        if (remainingTransfersOutWithSellAllowance > timeframeQuotaOut) {
            return int256(timeframeQuotaOut);
        } else {
            return int256(remainingTransfersOutWithSellAllowance);
        }
    }
