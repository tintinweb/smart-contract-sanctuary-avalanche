...
// File contracts/NovusContract.sol

pragma solidity ^0.8.19;
// NovusContract.sol
// SPDX-License-Identifier: MIT
contract NovusContract {
    struct NovusAddress {
        string name;
        address theAddress;
    }
  ...
  struct DistributionType {
    ...
    function addNovusAddress(
        string memory name,
        address newAddress
    ) public onlyAdmin onlyvalidAddress(newAddress) returns (bool) {
        require(
            !isValidNovusAddress[newAddress],
            "That Novus address has already been added to the list"
        );
    ...
    function addAdmin(
        address newAdmin
    ) public onlyAdmin onlyValidNovusAddresses(newAdmin) returns (bool) {
    ...
    function removeAdmin(address oldAdmin) public onlyAdmin returns (bool) {
        require(
            validNovusAddress.length > 1,
            "Admins list should have at least 1 admin. You can not remove them"
        );
        require(
            isAdmin[address(oldAdmin)],
            "That wallet is not actually an admin"
        );
        require(
            oldAdmin != msg.sender,
            "You can not remove yourself from the admin list"
        );
    ...
    function removeNovusAddress(
        address oldAddress
    ) public virtual onlyAdmin onlyValidNovusAddresses(oldAddress) returns (bool) {
        require(
            oldAddress != msg.sender,
            "You can not remove yourself from the Novus addresses list"
        );
        require(
            !isAdmin[oldAddress],
            "Fist remove that address from the admin list"
        );
        ...
        for (uint i = 0; i < validNovusAddress.length; i++) {
            if (validNovusAddress[i] == address(oldAddress)) {
                validNovusAddress[i] = validNovusAddress[
                    validNovusAddress.length - 1
                ];
    ...
    modifier onlyAdmin() {
        require(
            isAdmin[msg.sender] || msg.sender == deployer,
    ...
    modifier onlyValidNovusAddresses(address someAddress) {
        require(
            isValidNovusAddress[address(someAddress)],
            "That wallet is not actually a valid Novus address"
        );
        _;
    }
    ...
    modifier onlyvalidAddress(address someAddress) {
        require(
            ...
            "Novus Address must be a valid address"
        );
        _;
    }
}
...
contract NovusDistribution is 
  NovusContract, ReentrancyGuard {
  ...
  constructor(string memory _version, uint _creationDate) 
  NovusContract(_version, _creationDate) {
    super.addNovusAddress("Novus Minting", address(this));
  }

  function removeNovusAddress(address oldAddress) public override onlyAdmin onlyValidNovusAddresses(oldAddress) returns (bool) {
    require(!isDistributionAddress[oldAddress], "You can not remove a distribution address");
    super.removeNovusAddress(oldAddress);
    return true;
  }
  ...
  function setDistribution(uint distributionTypeId,uint sequence,string[] memory names,uint[] memory amount,address from,address[] memory to) public onlyAdmin onlyValidDistributionTypes(distributionTypeId) returns (Distribution memory) {
  ...
  function distributeNOVOInSequence(uint distributionTypeId, uint sequence)  private onlyValidDistributionTypes(distributionTypeId) {
  ...
  // function mintNOVO(address to, uint minted_amount, uint fee) public payable onlyAdmin returns (bool, bytes memory) {
  //   require(msg.value > 0, 'Invalid Deposited amount');
  //   require(msg.value == minted_amount + fee, "Deposited amount is not equal to the minted_amount + fee");
  //   (bool sent, bytes memory data) = _transferTo(address(to), minted_amount);
  //   string memory dTypeName = "NOVO Minting";
  //   DistributionType memory _dType = getDistributionTypeByName(dTypeName);
  //   addToDistributionBalance(_dType.id, fee);
  //   return (sent, data);
  // }

  // function depositToDistribution(uint distributionType) public payable{
  //   require(msg.value > 0, 'Invalid Deposited amount');
  //   addToDistributionBalance(distributionType ,msg.value );
  // }
