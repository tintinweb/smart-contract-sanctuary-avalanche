...
interface iCoupon {
  ...
  function getCouponActive(address _address) external view returns (uint256 _couponActive);
}
...
contract ProjectLaunchFactory {
  ...
  address public couponContract = 0xE039660A6221BC3a98B7228f2bc116d885860A18;
  ...
  function createSale(
    ...
    uint256[10] memory _seed_details,
    uint32[5] memory _seed_vesting,
    uint256[10] memory _presale_details,
    uint32[5] memory _presale_vesting,
    uint256[10] memory _community_details,
    ...
    _sale_details[6] = iCoupon(couponContract).getCouponDiscount(_address);

    if (iCoupon(couponContract).getCouponActive(_address) == 1) {
      _seed_details[8] = round1Mult[_sale_details[0]] + ((10000 - round1Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
      _seed_details[9] = round1MultT[_sale_details[0]] + ((10000 - round1Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
      _presale_details[8] = round2Mult[_sale_details[0]] + ((10000 - round2Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
      _presale_details[9] = round2MultT[_sale_details[0]] + ((10000 - round2Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
      _community_details[8] = round3Mult[_sale_details[0]] + ((10000 - round3Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
      _community_details[9] = round3MultT[_sale_details[0]] + ((10000 - round3Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address) / 100) ;
    } else {
      _seed_details[8] = round1Mult[_sale_details[0]];
      _seed_details[9] = round1MultT[_sale_details[0]];
      _presale_details[8] = round2Mult[_sale_details[0]];
      _presale_details[9] = round2MultT[_sale_details[0]];
      _community_details[8] = round3Mult[_sale_details[0]];
      _community_details[9] = round3MultT[_sale_details[0]];
    }
    ...
    if (iCoupon(couponContract).getCouponActive(_address) == 1) {
      payable(feeTo).transfer((launchFee[_sale_details[0]])*(100-((iCoupon(couponContract).getCouponDiscount(_address)))/100));
    } else {
...
contract Sale is ReentrancyGuard {
    ...
    function init_sale (
        ...
        if(_sale_details[0] >= 4) {
            require(_seed_details[6] == 0 && _seed_details[7] == 0 && _presale_details[6] >= block.timestamp && _community_details[6] >= _presale_details[7] && (_presale_details[7] > _presale_details[6] || _community_details[7] > _community_details[6]), "Dates not valid");
        } else if (_sale_details[0] < 4){
            require(_seed_details[6] >= block.timestamp && _seed_details[7] > _seed_details[6] && _presale_details[6] >= _seed_details[7] && _presale_details[7] > _presale_details[6] && _community_details[6] >= _presale_details[7] && _community_details[7] > _community_details[6], "Dates not valid");
        }
    ...
    function setSaleDetails(uint256 _sale_round, uint256[10] memory _sale_input) private {
    ...
    function purchaseICOCoin (uint256 _round) public nonReentrant {
        ...
        ERC20(sale_details.sale_token).transferFrom(address(this), address(feeAddress), (address(this).balance - (address(this).balance * sale_info[_round].multT) / 10000));
    }
