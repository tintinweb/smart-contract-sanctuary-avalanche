...
/**
 * @title MultiFlexibleStaking
 * @author gotbit
 * @dev Contract for staking tokens in order to earn rewards. Any user can make multiple stakes. Reward earn period is practically unlimited.
 */
...
contract MultiFlexibleStaking is Ownable, Pausable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;

    struct Stake {
        uint256 id;
        uint128 stakingPeriod;
        uint128 timastamp;
        uint128 amount;
        uint128 earned;
    ...
    uint80 public rewardsDuration = 100 * 365 days; // 100 years
    ...
    uint256 public constant MIN_REWARDS_DURATION = 100 * 365 days; // 100 years

    mapping(uint256 => Stake) public stakes;
    // TODO stake history
    mapping(address => ClosedStake[]) public userInactiveStakes;
    ...
    function stake(
        uint128 amount,
        uint128 stakingPeriod
    ) external whenNotPaused updateReward(globalId) {
    ...
    /// @dev Allows to calculate principal and rewards penalties (nominated in percents) for a specific stake
    /// @param id Stake id
        ...
        returns (uint256 principalPenaltyPercentage, uint256 rewardPenaltyPercentage)
    {
        ...
        uint256 actualStakingTime = block.timestamp - _stake.timastamp;

        if (actualStakingTime <= _stake.stakingPeriod) {
            // EMERGENCY UNSTAKE
            if (actualStakingTime < (2 * _stake.stakingPeriod) / 10) {
                // 0 - 20 % hold time
                principalPenaltyPercentage = 80 * ACCURACY;
                rewardPenaltyPercentage = 100 * ACCURACY;
            } else if (actualStakingTime < (4 * _stake.stakingPeriod) / 10) {
                // 20 - 40 % hold time
                principalPenaltyPercentage = 40 * ACCURACY;
                rewardPenaltyPercentage = 100 * ACCURACY;
            } else if (actualStakingTime < (5 * _stake.stakingPeriod) / 10) {
                // 40 - 50 % hold time
                rewardPenaltyPercentage = 100 * ACCURACY;
            } else if (actualStakingTime < (7 * _stake.stakingPeriod) / 10) {
                // 50 - 70 % hold time
        ...
        } else if (actualStakingTime > _stake.stakingPeriod + RECEIVE_PERIOD) {
            // LATE UNSTAKE
            // principal penalty = 0

            uint256 extraPeriod = actualStakingTime - _stake.stakingPeriod;
            rewardPenaltyPercentage = (extraPeriod * 100 * ACCURACY) / actualStakingTime;
        }
        // else SAFE UNSTAKE (principal fee = 0, rewards fee = 0)

        return (principalPenaltyPercentage, rewardPenaltyPercentage);
    }

    /// @dev Allows user to withdraw staked tokens + claim earned rewards - penalties
    /// @param id Stake id
    function withdraw(uint256 id) public updateReward(id) {
        Stake memory _stake = stakes[id];
        uint256 amount = _stake.amount;
        require(amount > 0, 'Cannot withdraw 0');
        require(userById[id] == msg.sender, 'Can`t be called not by stake owner');
        ...
        userInactiveStakes[msg.sender].push(ClosedStake(stakes[id], block.number));
        stakes[id].amount = 0;

        emit Withdrawn(msg.sender, id, amount);

        if (_stake.earned > 0) {
            _claim(id, rewardPenaltyPercentage);
        }
        ...
        stakingToken.safeTransfer(
            msg.sender,
            (amount * (100 * ACCURACY - principalPenaltyPercentage)) / (100 * ACCURACY)
        );
        stakingToken.safeTransfer(
            penaltyWallet,
            (amount * principalPenaltyPercentage) / (100 * ACCURACY)
        );
    }

    /// @dev Allows user to claim his rewards - penalties
    /// @param id Stake id
    function claim(uint256 id) external updateReward(id) {
        require(userById[id] == msg.sender, 'Can`t be called not by stake owner');
        (, uint256 rewardPenaltyPercentage) = calculatePenalties(id);
        _claim(id, rewardPenaltyPercentage);
    }

    /// @dev Allows user to claim all his rewards
    /// @param id Stake id
    /// @param rewardsPenaltyPercentage rewards penalty, which will be transfered to penalty wallet (nominated in percents)
    function _claim(uint256 id, uint256 rewardsPenaltyPercentage) internal {
        ...
        emit RewardPaid(
            msg.sender,
            id,
            (reward * (100 * ACCURACY - rewardsPenaltyPercentage)) / (100 * ACCURACY)
        );
        ...
        rewardToken.safeTransfer(
            penaltyWallet,
            (reward * rewardsPenaltyPercentage) / (100 * ACCURACY)
        );
    }
    ...
    function notifyRewardAmount(
        ...
        require(
            rewardRate != 0,
            'Rewards too low to be distributed over the current rewards duration'
        );
    ...
    function earned(uint256 id) public view returns (uint128) {
        return
            uint128(
                (stakes[id].amount *
                    (rewardPerToken() - stakes[id].userRewardPerTokenPaid)) /
                    ACCURACY +
                    stakes[id].earned
            );
    }
    ...
    /// @dev Returns the APR for a specific stake position
    /// @param id stake id
    ...
    function getAPR(uint256 id) external view returns (uint256) {
        ...
        uint256 _tvl = totalSupply < 10_000 * decimals ? 10_000 * decimals : totalSupply;
        uint128 userStakeAmount = stakes[id].amount;
        return
            (uint256(rewardRate) * ACCURACY * 365 days * 100) / (_tvl * userStakeAmount); // earned user value in tokens for one second including share (1e18 for accuracy)
    }
    ...
    function getUserInactiveStakesSlice(address user, uint256 startIndex, uint256 length) external view returns(ClosedStake[] memory) {
        uint256 len = userInactiveStakes[user].length;
        require(startIndex + length <= len, 'Invalid startIndex + length');
        ClosedStake[] memory userStakes = new ClosedStake[](length);
        uint256 userIndex;
        for (uint256 i = startIndex; i < startIndex + length; ++i) {
            userStakes[userIndex] = userInactiveStakes[user][i];
            ++userIndex;
        }
        return userStakes;
    }
