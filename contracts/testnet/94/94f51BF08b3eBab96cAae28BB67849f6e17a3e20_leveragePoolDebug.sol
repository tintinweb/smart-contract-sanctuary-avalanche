...
contract leveragePool is leverageData,timeLockSetting {
    ...
    function sellLeverage(uint256 amountUnderlying,uint256 slipRate,uint256 deadLine) ensure(deadLine) external {
        _sellLeverage(msg.sender,amountUnderlying,slipRate);
    }
    function sell(address to,uint256 slipRate,uint256 deadLine) ensure(deadLine) external {
        _sell(msg.sender,to,slipRate);
    }
    /**
