...
contract DrawerV2 is Ownable {
  ...
  uint256 constant TEAM_3 = 0x00ae007e00540052; // x = 82 y = 84 x2 = 126 y2 = 174
  uint256 constant TEAM_5 = 0x00a8005c00580030; // x = 48 y = 88 x2 =  92 y2 = 168
  uint256 constant TEAM_7 = 0x00a50036005B0011; // x = 17 y = 91 x2 =  54 y2 = 165
  ...
  function draw(
    ...
    bytes memory merged = abi.encodePacked(
  ...
  function drawTeam(
    ...
    bytes memory attributes = abi.encodePacked(
      '[{"display_type":"number","trait_type":"Size","value":"',
      toString(size),
      '"}]'
    );
    ...
    for (uint256 i = 0; i < size; i++) {
      ...
      merged = abi.encodePacked(merged, '<svg viewBox="0 0 64 64" width="64" height="64" x="', x, '" y="', y, '">');
      for (uint256 j = 0; j < 7; j++) {
        merged = abi.encodePacked(merged, traits[j].getContent(isMale, pieces[6 - j]));
      }
      merged = abi.encodePacked(merged, "</svg>");
    }
    merged = abi.encodePacked(
      merged,
