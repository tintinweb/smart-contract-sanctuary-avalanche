...
// File contracts/PublicPoolsVesting.sol
...
contract VestingPublicPools {
    ...
    function _vestingSchedule(uint256 totalAllocation, uint256 timestamp) internal view virtual returns (uint256) {
        // require(timestamp >= _nextRelease, "Time not reached");
        // TODO: year computation
        uint256 year = timestamp.sub(start()).div(duration()) + 1;
        if (year > 4) {
            return totalAllocation;
        } else {
            return totalAllocation.mul(year).div(4);
        }
    }
}