...
contract Treasury is ContractGuard {
    ...
    event StakeFunded(uint256 timestamp, uint256 seigniorage);
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(token).operator() == address(this) &&
                ...
                Operator(stake).operator() == address(this),
    ...
    function initialize(
        ...
        tokenPriceOne = 10**18; // This is to allow a PEG of 1 TOKEN per AVAX
        ...
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for stake
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn TOKEN and mint tBOND)
    ...
    function setStake(address _stake) external onlyOperator {
        stake = _stake;
    }
    ...
    function _sendToStake(uint256 _amount) internal {
        ...
        if (team1FundSharedPercent > 0) {
            ...
            IERC20(token).transfer(team1Fund, _team1FundSharedAmount);
        ...
        IERC20(token).safeApprove(stake, 0);
        IERC20(token).safeApprove(stake, _amount);
        IStake(stake).allocateSeigniorage(_amount);
        emit StakeFunded(block.timestamp, _amount);
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToStake(tokenSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochTokenPrice > tokenPriceCeiling) {
                // Expansion ($TOKEN Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bond).totalSupply();
                uint256 _percentage = previousEpochTokenPrice.sub(tokenPriceOne);
                uint256 _savedForBond;
                uint256 _savedForStake;
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForStake = tokenSupply.mul(_percentage).div(1e18);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = tokenSupply.mul(_percentage).div(1e18);
                    _savedForStake = _seigniorage.mul(seigniorageExpansionFloorPercent).div(10000);
                    _savedForBond = _seigniorage.sub(_savedForStake);
                    if (mintingFactorForPayingDebt > 0) {
                        _savedForBond = _savedForBond.mul(mintingFactorForPayingDebt).div(10000);
                    }
                }
                if (_savedForStake > 0) {
                    _sendToStake(_savedForStake);
                }
                if (_savedForBond > 0) {
    ...
    function stakeSetOperator(address _operator) external onlyOperator {
        IStake(stake).setOperator(_operator);
    }

    function stakeSetLockUp(uint256 _withdrawLockupEpochs, uint256 _rewardLockupEpochs) external onlyOperator {
        IStake(stake).setLockUp(_withdrawLockupEpochs, _rewardLockupEpochs);
    }

    function stakeAllocateSeigniorage(uint256 amount) external onlyOperator {
        IStake(stake).allocateSeigniorage(amount);
    }

    function stakeGovernanceRecoverUnsupported(
        address _token,
        uint256 _amount,
        address _to
    ) external onlyOperator {
        IStake(stake).governanceRecoverUnsupported(_token, _amount, _to);
    }
}
