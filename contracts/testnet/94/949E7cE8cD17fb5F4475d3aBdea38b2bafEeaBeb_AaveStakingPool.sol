...
contract AaveStakingPool is
    AdapterBase,
    Basic,
    AaveStakingCollERC20,
    Ownable,
    ReentrancyGuard
{
    ...
    function initialize() external onlyOwner {
        IERC20(stakeToken).safeApprove(aaveAdapter, type(uint256).max);
    }
    ...
    function flashLoan(address user, uint256 amount)
        external
        onlyAdapter
        nonReentrant
    {
        IERC20(stakeToken).safeTransfer(user, amount);
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        uint256 mintAmount = uBalanceIn.mul(totalSupply).div(
