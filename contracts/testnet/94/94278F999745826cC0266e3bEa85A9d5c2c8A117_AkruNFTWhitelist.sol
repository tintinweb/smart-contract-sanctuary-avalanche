...
{  
    ...
    */
    ...
    function addManageralRole(address user, uint256 nftId, ROLES role) public {
        require(role != ROLES.superAdmin, "Whitelisting: SuperAdmin Not added");
    ...
    function removeManageralRole(address user, ROLES role) public {
        require(role != ROLES.superAdmin,"Whitelisting: SuperAdmin Not Removed" );
        if (      
    ...
    function renounceSuperAdmin(address newSuperAdmin) external {
        onlySuperAdmin();
        require(NFT.ownerOf(11) == msg.sender,"OnlySuper Admin could Do");
         NFT.transferOwnerShip(_msgSender(), newSuperAdmin, 11);
        roleInfo[11] = userRole({
            roleId: 11,
            roleName: ROLES.superAdmin,
            userAddress: newSuperAdmin,
            idPrefix: getRoleIdPrefix(ROLES.superAdmin),
            valid: true
        });
     ...
     * @return role status
     */
    function isMediaManager(address _wallet) public view returns (bool) {
...
contract AkruNFTWhitelist is AkruNFTAdminWhitelist
{
    ...
    function addWhitelistUser(
        ...
        require(getIdPrefix(nftId) > 14,"User-Whitelist: Manager Couldn't Add hiher roles");
        ...
        emit AddWhitelistUser(_wallet, role, nftId, _accredationExpiry,_accredationVerified,_kycVerified);
    }
    ...
    function checkUserRole(
        ...
        for (uint i = 0; i < nftIds.length; ) {
            uint256 idPrefix = getIdPrefix(nftIds[i]);
            if (rolePrefix == idPrefix) {
                if(accreditationCheck && role == ROLES.user_USA  && msg.sig == 0x7e9ad37f){
 ...
 */
    function updateUserType(address user, bool status) public {
        onlyAdmin();
        WhitelistInfo storage userD =  whitelistUsers[user];
        require(userD.valid,"Not a Valid User");
        userD.isPremium = status;
