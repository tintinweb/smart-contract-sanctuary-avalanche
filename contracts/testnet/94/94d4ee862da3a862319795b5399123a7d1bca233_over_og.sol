  ...
  function getTokenPrice() public view returns(uint[7] memory)
   {
    ...
    uint[7] memory res = [main,now_val,low_val,high_val,doll_pr,val,boo];
    return res; // return amount of token0 needed to buy token1
   }
    function getEm() public returns (uint) {
    	uint[7] memory resem = getTokenPrice();// return amount of token0 needed to buy token1
    	uint a = resem[2];
    	return a;
    }
}