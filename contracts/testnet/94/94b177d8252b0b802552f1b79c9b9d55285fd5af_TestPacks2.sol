    ...
    
{
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        require(amount > 0 && (amount * packSize) <= maxPerTx,"Error: max par tx limit");
        ...
        require(supply + (amount * packSize) - 1 < maxSupply, "Error: cannot mint more than total supply");
        if(whitelistedOnly)
    ...
    //Overrides
        ...
        for (uint256 i = 0; i < (amount * packSize); i++) {
            mintTokenInternal((amount * packSize), to);
        }
    }
