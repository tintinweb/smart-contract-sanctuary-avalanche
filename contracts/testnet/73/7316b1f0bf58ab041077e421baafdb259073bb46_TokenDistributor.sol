...
contract TokenDistributor is ReentrancyGuard, Ownable {
    ...
    function burnAndBridge(address recipient, uint256 tokenAmount) external payable {
        ...
        IERC20(token).burn(recipient, tokenAmount);
        ...
        emit TokensBurned(recipient, tokenAmount);
        emit unbridgeProcessed(recipient, tokenAmount, nonceUnbridged);
        nonceUnbridged++;
    }

    function bridgeAndMint(address recipient, uint256 tokenAmount, uint256 ethAmount) external payable onlyOwner nonReentrant {
        require(!isPaused, "bridge is paused");
        require(msg.sender == address(uint160(msg.sender)), "Recipient is not an EOA");
        require(tokenAmount > 0, "Invalid token amount");
        ...
        emit bridgeProcessed(recipient, tokenAmount, nonceBridged, ethAmount);
