...
contract LydiaRestrictedStrategyAddBaseTokenOnly is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "LydiaRestrictedStrategyAddBaseTokenOnly::onlyWhitelistedWorkers:: bad worker");
    _;
  }

  /// @dev Create a new add Token only strategy instance.
  /// @param _router The PancakeSwap Router smart contract.
  function initialize(ITridentRouter _router) external initializer {
  ...
  function execute(
    ...
    address baseToken = IWorker(msg.sender).baseToken();
    address farmingToken = IWorker(msg.sender).farmingToken();
    IPool lpToken = IPool(address(IWorker(msg.sender).lpToken()));
    uint256 swapFee = lpToken.swapFee();
    // 2. Approve router to do their stuffs
    _approveTokens(baseToken, farmingToken, true);
    ...
    // Constants come from
    // 2-fee = 2-0.0025 = 19975
    // 4(1-fee) = 4*9975*10000 = 399000000, where fee = 0.0025 and 10,000 is a way to avoid floating point
    // 19975^2 = 399000625
    // 9975*2 = 19950
    uint256 aIn = AlpacaMath
      .sqrt(rIn.mul(balance.mul(40000 * (10000 - swapFee)).add(rIn.mul((20000 - swapFee)**2))))
      .sub(rIn.mul(20000 - swapFee)) / (10000 - swapFee).mul(2);

    ITridentRouter.ExactInputSingleParams memory inputSingleParams = ITridentRouter.ExactInputSingleParams(
      aIn,
      lpToken.getAmountOut(abi.encode(baseToken, aIn)),
      address(lpToken),
      baseToken,
      abi.encode(baseToken, address(this), true)
    );
    router.exactInputSingleWithNativeToken(inputSingleParams);

    ITridentRouter.TokenInput[] memory baseTokenInput = new ITridentRouter.TokenInput[](2);
    baseTokenInput[0] = ITridentRouter.TokenInput(baseToken, true, baseToken.myBalance());
    baseTokenInput[1] = ITridentRouter.TokenInput(farmingToken, true, farmingToken.myBalance());

    router.addLiquidity(baseTokenInput, address(lpToken), minLPAmount, abi.encode(msg.sender));

    // 6. Reset approval for safety reason
    _approveTokens(baseToken, farmingToken, false);
  }
  ...
  function _approveTokens(
    ...
    if (approved) {
      approveAmount = uint256(-1);
    }

    token1.safeApprove(router.bento(), approveAmount);
    token2.safeApprove(router.bento(), approveAmount);
    IBentoBoxMinimal(router.bento()).setMasterContractApproval(
      address(this),
      address(router),
      approved,
      0,
