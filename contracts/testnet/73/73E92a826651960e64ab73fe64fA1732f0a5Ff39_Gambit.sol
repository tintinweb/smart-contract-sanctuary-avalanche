...
contract Gambit {
    ...
    function claimWin(string memory _matchId) external {
        ...
        if (matchesBets[_matchId].betsByAddress[msg.sender].isMatchWinner && matchesBets[_matchId].betsByAddress[msg.sender].isPerfectScoreWinner){
            ...
            uint _betAmount = matchesBets[_matchId].betsByAddress[msg.sender].amount / 1e18;
            emit LogEvent(msg.sender, "betAmountPS", _betAmount, "NA", false);

            uint _matchWinnerAvax = matchesBets[_matchId].matchWinnersAvax / 1e18;
            emit LogEvent(msg.sender, "_matchWinnerAvax", _matchWinnerAvax, "NA", false);
            
            uint _winnersPot = matchesBets[_matchId].winnersPot / 1e18;
            emit LogEvent(msg.sender, "_winnersPot", _winnersPot, "NA", false);

            uint _factor = _betAmount/_matchWinnerAvax;
            emit LogEvent(msg.sender, "_factor", _factor, "NA", false);
            
            uint _prizeMatchWinner = _factor*_winnersPot;
            ...
            uint _factorPS = _betAmount/_perfectScoreWinnersAvax;
            
            uint _perfectScoreWinnerPrize = _factorPS*_perfectScorePot;
