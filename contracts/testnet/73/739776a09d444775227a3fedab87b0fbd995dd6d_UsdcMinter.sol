...
contract UsdcMinter is UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function mintUsdcToReserveHandler(bytes memory message, bytes calldata attestation)
        external
        onlyRole(RELAYER_ROLE)
    {
        ...
        emit UsdcMinted(success, balBefore - balAfter);
    }
}
...
interface IMessageTransmitter {
    function receiveMessage(bytes memory _message, bytes calldata _attestation) external returns (bool);
}
