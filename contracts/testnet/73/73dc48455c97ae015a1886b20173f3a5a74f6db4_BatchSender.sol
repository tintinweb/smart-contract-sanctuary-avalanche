...
contract BatchSender {
    ...
    function withdrawnFund(address payable destination) external payable onlyAdmin {
        require(address(this).balance > 0);

        destination.transfer(address(this).balance);
    }

    function batchEth(address payable[] memory accounts) external payable onlyAdmin {
    ...
    function batchToken(address payable[] memory accounts) external payable onlyAdmin {
