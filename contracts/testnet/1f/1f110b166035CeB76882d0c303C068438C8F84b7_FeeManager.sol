...
contract FeeManager is Context, Ownable {
  ...
  constructor(WHITELIGHT _whitelightToken, IUniswapV2Router02 _router) {
  ...
  function setRouterAddress(IUniswapV2Router02 newAddress) external onlyOwner {
    router = newAddress;
  }
  ...
  function distributeRoyaltyFee(uint256 toSpend) private {
    if (feeRoyalty > 0 && royaltyAddress != address(0)) {
      uint256 royaltyKapexAmount = toSpend.mul(feeRoyalty).div(feeTotal);
      whitelightToken.transfer(royaltyAddress, royaltyKapexAmount);
    }
  }
  ...
  function distributeLiquidityFee(
    ...
    if (feeLiquidity > 0 && lpTokensHolderAddress != address(0)) {
      ...
      uint256 amountInKapex = toSpend.mul(feeLiquidity.div(2)).div(feeTotal);

      whitelightToken.approve(address(router), amountInKapex);

      router.addLiquidityETH{value: amountInNative}(
  ...
  function disburseSwapAndLiquifyTokens(uint256 toSpend) public onlyOwner {
    require(toSpend <= whitelightToken.balanceOf(address(this)), "Amount is greater than contract kapex balance");
