...
contract PublicSale {
  ...
  uint256 public constant INVEST_MAX_AMOUNT = 1000 * 10**18; // 1000 MIM
  ...
  function invest(uint256 _amount) public {
    ...
    require(
      IERC20(farmingToken).balanceOf(address(this)) > 1000 * 10**9,
      "sold out"
    );
    require(
      IERC20(baseToken).allowance(msg.sender, address(this)) >= _amount,
      "not approved"
    );
    require(_amount <= INVEST_MAX_AMOUNT, "max overflow");
    IERC20(baseToken).transferFrom(msg.sender, address(this), _amount);
    IERC20(baseToken).transfer(owner, _amount);
