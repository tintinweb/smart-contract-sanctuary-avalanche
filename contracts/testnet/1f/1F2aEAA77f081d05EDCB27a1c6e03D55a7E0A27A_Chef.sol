...
contract Chef is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public constant CHEF_PRICE_WHITELIST = 0.1 ether;
    uint256 public constant CHEF_PRICE_AVAX = 0.1 ether;

    uint256 public constant CHEFS_PER_SUSHI_MINT_LEVEL = 5000;
    ...
    uint256 public constant CHEF_TYPE = 0;
    uint256 public constant SHINOBI_TYPE = 1;

    uint256 public constant CHEF_YIELD = 1;
    uint256 public constant SHINOBI_YIELD = 3;
    ...
    // external contracts
    Sushi public sushi;
    ChefMetadata public meta;
    address public sushishopAddress;
    address public bossfightAddress;
    ...
    mapping(uint256 => uint256[5]) public tokenDna; // maps tokenId to its type

    // mint tracking
    uint256 public chefsMintedWithAVAX;
    uint256 public chefsMintedWithSUSHI;
    uint256 public chefsMintedWhitelist;
    uint256 public chefsMintedPromotional;
    uint256 public chefsPulled;
    uint256 public chefsMinted;

    // mint control bools
    bool public startTimeWhitelist;
    bool public startTimeAVAX;
    bool public startTimeSUSHI;
    ...
    /**
     * requires sushi, chefType oracle address
     * sushi: for liquidity bootstrapping and spending on chefs
     * chefTypeOracleAddress: external chef generator uses secure RNG
     */
    constructor(Sushi _sushi, address _chefTypeOracleAddress, address _ChefMetadata) ERC721("Sushi Game Chefs", "SUSHI-GAME-CHEF") {
    ...
    function getDna(uint256 _tokenId) public view returns (uint256[5] memory) {
        require (_exists(_tokenId), "token does not exist");
        return tokenDna[_tokenId];
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require (_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        return meta.getMetadata(tokenId, getType(tokenId), getDna(tokenId));
    }

    // override

    function isApprovedForAll(address _owner, address _operator) override(ERC721, IERC721) public view returns (bool) {
        // pizzeria must be able to stake and unstake
        if (sushishopAddress != address(0) && _operator == sushishopAddress) return true;
        return super.isApprovedForAll(_owner, _operator);
    }

    // ADMIN

    function setSushishopAddress(address _sushishopAddress) external onlyOwner {
        sushishopAddress = _sushishopAddress;
    }

    function setBossfightAddress(address _bossfightAddress) external onlyOwner {
        bossfightAddress = _bossfightAddress;
    }
    ...
    function setStartTimeWhitelist(bool _start) external onlyOwner {
        startTimeWhitelist = _start;
    }

    function setStartTimeAVAX(bool _start) external onlyOwner {
        startTimeAVAX = _start;
    }

    function setstartTimeSUSHI(bool _start) external onlyOwner {
        startTimeSUSHI = _start;
    }
     ...
     * - The external automation is open source and a fork of https://github.com/pizzacapital/pizza_game_generator
     ...
     */
    function setChefType(uint256 tokenId, uint256 chefType, uint256[5] calldata _dna) external {
        ...
        require(chefType == CHEF_TYPE || chefType == SHINOBI_TYPE, "invalid chef type");

        tokenTypes[tokenId] = chefType;
        tokenDna[tokenId] = _dna;
        chefsPulled += 1;
    ...
    function mintPromotional(uint256 qty, address target) external onlyOwner {
        ...
        for (uint256 i = 0; i < qty; i++) {
            ...
            chefsMinted += 1;
            _createChef(target, chefsMinted);
        }
    }
    ...
    function mintWhitelist(bytes32[] calldata _merkleProof, uint256 qty) external payable whenNotPaused {
        ...
        require(qty > 0 && qty <= MAXIMUM_MINTS_PER_WHITELIST_ADDRESS, "quantity must be between 1 and 5");
    ...
    function mintChefWithAVAX(uint256 qty) external payable whenNotPaused {
        ...
        require ((chefsMintedWithAVAX + qty) <= (NUM_GEN0_CHEFS - chefsMintedWhitelist), "you can't mint that many right now");
     ...
     * @dev GEN1 minting 
     */
    function mintChefWithSUSHI(uint256 qty) external whenNotPaused {
        require (mintingStartedSUSHI(), "cannot mint right now");
        require (qty > 0 && qty <= 10, "quantity must be between 1 and 10");
        require ((chefsMintedWithSUSHI + qty) <= NUM_GEN1_CHEFS, "you can't mint that many right now");
        ...
        if(chefsMintedWithSUSHI <= CHEFS_PER_SUSHI_MINT_LEVEL && chefsMintedWithSUSHI + qty > CHEFS_PER_SUSHI_MINT_LEVEL) {
            currentSUSHIMintCost = currentSUSHIMintCost * 2;
        }

        chefsMintedWithSUSHI += qty;
...
contract Sushi is ERC20("Sushi", "SUSHI"), Ownable {
    ...
    uint256 public constant NUM_PROMOTIONAL_SUSHI = 500_000;
    uint256 public constant NUM_SUSHI_SAKE_LP = 20_000_000;

    uint256 public NUM_SUSHI_AVAX_LP = 32_000_000;
    ...
    bool public avaxLPSushiMinted = false;
    ...
    function mintAvaxLPPizza() external onlyOwner {
        require(!avaxLPSushiMinted, "avax sushi LP has already been minted");
        avaxLPSushiMinted = true;
        _mint(owner(), NUM_SUSHI_AVAX_LP * ONE_SUSHI);
    }
    ...
    function setNumPizzaAvaxLp(uint256 _numPizzaAvaxLp) external onlyOwner {
        NUM_SUSHI_AVAX_LP = _numPizzaAvaxLp;
    }

    // external

    function mint(address _to, uint256 _amount) external {
        require(sushishopAddress != address(0) && chefAddress != address(0) && freezerAddress != address(0) && upgradeAddress != address(0), "missing initial requirements");
        require(_msgSender() == sushishopAddress,"msgsender does not have permission");
        _mint(_to, _amount);
    }
...
/**
* Keep NFT data on chain so it can be altered by some game events
* or by some boss special powers...
*/
contract ChefMetadata is Ownable {
    using Strings for uint256;

    string uri;
    string[2] types = ['Chef', 'Shinobi'];
    string[2] typesDesc = ['Chef', 'Shinobi'];
    string[6] categoryNames = ["Kimono", "Skin", "Face", "Hat", "Sushi"];
    uint256[6] categoryCount = [0,0,0,0,0];
    mapping(uint256=>mapping(uint256=>string)) public categoryData;

    constructor(string memory _uri) {
        uri = _uri;
    }

    function addTraits(uint256 _category, string[] calldata _names) public onlyOwner {
        categoryCount[_category] = uint256(_names.length);
    ...
    function getImage(uint256 _typeId, uint256[5] memory _dna) public view returns (string memory) {
    ...
    function compileAttributes(
        uint256[5] memory dna
    ) public view returns (string memory) {
        return string(
            abi.encodePacked(
                attributeForTypeAndValue(
                    categoryNames[0],
                    categoryData[0][dna[0]]
                ),
                ",",
                attributeForTypeAndValue(
                    categoryNames[1],
                    categoryData[1][dna[1]]
                ),
                ",",
                attributeForTypeAndValue(
                    categoryNames[2],
                    categoryData[2][dna[2]]
                ),
                ",",
                attributeForTypeAndValue(
                    categoryNames[3],
                    categoryData[3][dna[3]]
                ),
                ",",
                attributeForTypeAndValue(
                    categoryNames[4],
                    categoryData[4][dna[4]]
                )
            )
        );
    }
    ...
    function getMetaImg(uint256 _typeId, uint256[5] calldata _dna) public view returns (string memory) {
        /*
        uint256[4] memory traits;
        uint256[4] memory dna;
        ...
        */
        return getImage(_typeId, _dna);
    }
    
    function getMetadata(uint256 _tokenId, uint256 _typeId, uint256[5] calldata _dna) public view returns (string memory) {
                        ...
                        bytes(
                            abi.encodePacked(
                                '{',
                                    ...
                                    '"description": "', typesDesc[_typeId],'",',
                                    '"attributes": [', compileAttributes(_dna), 
                                    '],"image": "', getImage(_typeId, _dna),'"',
                                '}'
                            )
                        )
                    )
