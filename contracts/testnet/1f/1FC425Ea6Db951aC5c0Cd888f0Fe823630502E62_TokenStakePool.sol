...
contract TokenStakePool is ReentrancyGuard, Ownable, Pausable {
  ...
  /**
  * Modifier to check if pool is active
  * @param _id  Unique id of stake pool
  */
  modifier onlyActive(uint256 _id) {
    require(pools[_id].active == true, "Pool is not active");
    _;
  }
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view onlyActive(_id) returns (uint256) {
    return (userPools[_id][_user].stakedAmount * currentRewardPerStakedToken(_id)
                                                 - userPools[_id][_user].rewardDebt
                                                 + userPools[_id][_user].rewardAccrued)
                                                 / SAFE_MULTIPLIER;
  }
  ...
  /**
  Update stake pool's reward per staked token value
  * @param _id  Unique id of stake pool
  */
  ...
  */
  function _updateUserRewardDebt(uint256 _id, address _user, uint256 _amount) private {
    userPools[_id][_user].rewardDebt = userPools[_id][_user].rewardDebt
  ...
  */
  function _updateUserRewardAccrued(uint256 _id, address _user) private {
    if (_user != address(0)) {
      userPools[_id][_user].rewardAccrued = userPools[_id][_user].rewardAccrued
                                               + (userPools[_id][_user].stakedAmount
                                               * pools[_id].rewardPerStakedToken
                                               - userPools[_id][_user].rewardDebt);
      userPools[_id][_user].rewardDebt = 0;
    }
  }
  ...
  */
  function claim(uint256 _id) public nonReentrant whenNotPaused onlyActive(_id) whenNotLocked(_id, msg.sender) {
    ...
    require(rewards > 0, "Cannot claim 0 reward");
    require(pools[_id].totalReward >= rewards, "Rewards deposited in stake pool less than rewards claimable");
    ...
    IERC20(pools[_id].rewardToken).safeTransfer(msg.sender, rewards);
  ...
  * @param _id  Unique id of stake pool
  */
  function exit(uint256 _id) external onlyActive(_id) {
    claim(_id);
  ...
  */
  ...
  /**
  * Deposit more reward tokens to a stake pool
  ...
  */
  function withdrawRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner onlyActive(_id) {
    ...
    require(pools[_id].totalReward > 0, "Cannot withdraw when stake pool has no reward tokens deposited");
    require(_amount <= pools[_id].totalReward, "Cannot withdraw more reward tokens than deposited in stake pool");

    IERC20(pools[_id].rewardToken).safeTransfer(msg.sender, _amount);
    pools[_id].totalReward += _amount;
  }

  /**
  * Update a stake pool's active status
  * @param _id      Unique id of pool
