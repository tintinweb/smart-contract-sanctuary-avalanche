...
// this a test
contract BSCBlockUpdater is Initializable, OwnableUpgradeable,Merkle {
    ...
    mapping(bytes32 => uint256) public validatorSetHash;

    mapping(bytes32 => BlockInfo) public blockInfos;

    function initialize(address circuitVerifierAddress) public initializer {
    ...
    function importNextValidatorSet(
        ...
        if (genesisBlockNumber != 0) {
            require(canonical[canonical.length - 1] == parsedInput.preValidatorSetHash, "invalid preValidatorSetHash");
        }
        require(_verifyMerkleRoot(hashList, parsedInput.currentMerkleRoot), "invalid hashList");

        // set genesisBlockNumber
        if (genesisBlockNumber == 0) {
            genesisBlockNumber = parsedInput.lastBlockNumber - BATCH_SIZE + 1;
            canonical.push(parsedInput.preValidatorSetHash);
        }
        validatorSetHash[parsedInput.currentValidatorSetHash] = canonical.length;
        canonical.push(parsedInput.currentValidatorSetHash);
    ...
    function importBlock(
        ...
        require(genesisBlockNumber != 0, "uninitialized");
        require(circuitVerifier.verifyProof(a, b, c, inputs), "invalid proof");
        ParsedInput memory parsedInput = _parseInput(inputs);
        uint256 currentValidatorIndex = validatorSetHash[parsedInput.currentValidatorSetHash];
        uint256 preValidatorIndex = validatorSetHash[parsedInput.preValidatorSetHash];
        require(canonical[currentValidatorIndex] == parsedInput.currentValidatorSetHash, "invalid proof");
        require(canonical[preValidatorIndex] == parsedInput.preValidatorSetHash, "invalid proof");
