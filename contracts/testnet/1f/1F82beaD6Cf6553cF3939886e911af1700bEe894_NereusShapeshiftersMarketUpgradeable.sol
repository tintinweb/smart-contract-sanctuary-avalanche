...
contract NereusShapeshiftersMarketUpgradeable is
 OwnableUpgradeable,
 ...
 INereusShapeshiftersMarket {
    ...
    uint256 private _availableTokensAmount;
    ...
    mapping(uint256 => mapping(address => uint256)) public _saleWavesClaimed;

    /// @notice Contract initializer
    function initialize(address nftContractAddress)
        public
        initializer
    {
    ...
    modifier isSalesAllowed(uint256 amount) {
        ...
        require(_availableTokensAmount >= amount, "NereusShapeshiftersMarket: requested amount exceed NFTs balance on contract");
        _;
    }
    ...
    * Allows to add new sale wave
     */
    function addSaleWave(SaleWaveTokenPrice[] calldata wavePrices, uint256 claimAllowance) external onlyOwner returns (uint256) {
        require(claimAllowance > 0, "NereusShapeshiftersMarket: claim allowance should be more than 0");

        uint256 saleWaveId = _saleWavesCounter;
    
        addSalesWaveTokenPrices(saleWaveId, wavePrices);
    
        _saleWaves[saleWaveId] = SaleWave(false, "", claimAllowance, true);
        _saleWavesCounter++;
    ...
    * Allows to add new sale wave
    ...
    function addWhitelistableSaleWave(
        ...
        _saleWaves[saleWaveId] = SaleWave(true, merkleRoot, claimAllowance, true);
        _saleWavesCounter++;
    ...
    * should: specified token to be not allowed
     */
    function allowERC20BuyToken(address erc20BuyToken) external onlyOwner {
    ...
    * should: specified token to be already allowed
     */
    function disallowERC20BuyToken(address erc20BuyToken) external onlyOwner {
        require(erc20BuyToken != address(0), "NereusShapeshiftersMarket: invalid address specified");
    ...
    * if whitelist wave should: address to be whitelisted, address requested amount does not exceed allowance per wave
    ...
    {
        require(!isContract(_msgSender()), "NereusShapeshiftersMarket: msg sender can not be smart contract");
        require(_saleWaves[_currentSaleWave].claimAllowance 
        ...
        for(uint256 i = 0; i < amount; i++) {
            buySingleItem(erc20BuyToken, currentWaveCost);
        }
        
        _saleWavesClaimed[_currentSaleWave][_msgSender()] += amount;
    }

    function buySingleItem(address erc20BuyToken, uint256 currentWaveCost) private {
        uint256 randomValue = getRandom(_availableTokensAmount);
        uint256 tokenId = tokenAt(randomValue);
    ...
    * Withdraw specified ERC20 token balance from contract address to owner
     */
    function withdrawERC20Balance(address erc20BuyToken) external onlyOwner nonReentrant {
        require(_allowedErc20Tokens[erc20BuyToken], "NereusShapeshiftersMarket: specified token does not exists");
    ...
    * Tracks only NFTs sent from specified nft contract address
    ...
    function onERC721Received(
        ...
        if(_msgSender() == _nftContractAddress) {
            _allowedTokenIds[_availableTokensAmount++] = _tokenId;
