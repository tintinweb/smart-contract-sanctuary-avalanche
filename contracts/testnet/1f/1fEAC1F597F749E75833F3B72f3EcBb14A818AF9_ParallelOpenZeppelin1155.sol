...
contract ParallelOpenZeppelin1155 is ONFT1155 {

    string uriPrefix;
    mapping(uint256 => string) uris;

    constructor(string memory _uriPrefix, address _lzEndpoint) ONFT1155("", _lzEndpoint) {
        uriPrefix = _uriPrefix;
    }

    function uri(uint256 _tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(uriPrefix, uris[_tokenId]));
    }

    function setTokenURI(uint256 _tokenId, string memory newuri) internal virtual {
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
        ...
        (bytes memory toAddressBytes, uint[] memory tokenIds, uint[] memory amounts, string[] memory _uris) = abi.decode(_payload, (bytes, uint[], uint[], string[]));
        address toAddress;
        assembly {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            setTokenURI(tokenIds[i], _uris[i]);
        }
