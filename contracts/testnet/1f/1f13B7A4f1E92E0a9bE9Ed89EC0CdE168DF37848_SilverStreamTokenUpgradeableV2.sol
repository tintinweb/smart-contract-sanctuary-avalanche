...
contract SilverStreamTokenUpgradeableV2 is Initializable, ERC20Upgradeable, AccessControlEnumerableUpgradeable {
    ...
    address public USDC;
    uint256 public constant USDC_MULTIPLIER = 102;
    uint256 public constant PERCENTAGE_FACTOR = 100;

    bool public CALL_OPTION_EXECUTED;

    event RoleProposed(uint256 proposal, address account, bytes32 role, bool grant);
    event CallOptionTriggered(uint256 usdcFunds);
    event RedemptionExecuted(address caller, uint256 tokenBalance, uint256 usdcBalance);
    /*
    function initialize(address _owner, address _usdc) public initializer {
    ...
    */
    ...
    function triggerCallOption() external onlyRole(OWNER_ROLE) {
        require(!CALL_OPTION_EXECUTED, "Error: Call option already executed");

        address caller = _msgSender();
        uint256 currentSupply = totalSupply();
        uint256 usdcAmount = currentSupply * USDC_MULTIPLIER * (10 ** IERC20MetadataUpgradeable(USDC).decimals()) / (PERCENTAGE_FACTOR * (10 ** decimals()));

        CALL_OPTION_EXECUTED = true;

        IERC20Upgradeable(USDC).safeTransferFrom(caller, address(this), usdcAmount);

        emit CallOptionTriggered(usdcAmount);
    }

    function redeemTokensForUSDC() external {
        require(CALL_OPTION_EXECUTED, "Error: Call option has not been executed yet");

        address caller = _msgSender();
        uint256 callerBalance = balanceOf(caller);
        uint256 usdcBalance = callerBalance * USDC_MULTIPLIER * (10 ** IERC20MetadataUpgradeable(USDC).decimals()) / (PERCENTAGE_FACTOR * (10 ** decimals()));

        _spendAllowance(address(this), caller, callerBalance);
        _burn(caller, callerBalance);

        IERC20Upgradeable(USDC).safeTransfer(caller, usdcBalance);

        emit RedemptionExecuted(caller, callerBalance, usdcBalance);
    }
