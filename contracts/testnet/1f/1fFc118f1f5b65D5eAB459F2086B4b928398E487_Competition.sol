...
interface IHalon {
    ...
    function getAmountOutUSD(uint256) external view returns (uint256);
}
...
contract Competition is ERC20Upgradeable {
    ...
    function initialize(address tokenAddress) public initializer {
        ...
        COORDINATOR = VRFCoordinatorV2Interface(0x2eD832Ba664535e5886b75D64C46EB9a228C2610); // Avax Testnet
        subscriptionId = 72;
        keyHash = 0x354d2f95da55398f44b7cff77da56283d9c6c829a4bdf1bbcaf2ad6a4d081f61;
    }

    // To receive AVAZ to buy tiket with AVAX 
    ...
    function buy(uint256 id, uint32 count, address tokenSent) payable public {
        ...
        if(price > 0){
            ...
            if(tokenSent==address(0)) {
                uint256 avaxAmount = IHalon(token).getAmountOut() * price / 1e18;
                console.log("avaxAmount", msg.value,avaxAmount);

                require(msg.value >= avaxAmount, "Insufficient AVAX");
            }else{
                if(tokenSent==usdcToken){
                    price = IHalon(token).getAmountOutUSD(price) / 1e12;
                }
                console.log(IERC20(tokenSent).balanceOf(address(msg.sender)));
                require(IERC20(tokenSent).balanceOf(address(msg.sender)) >= price,
                    "Buy: Insufficent balance."
                );
                IERC20(tokenSent).transferFrom(
                    address(msg.sender),
                    address(this),
                    price
                );
    ...
    function setUSDCAddress(address _usdcToken) public forOwner{
