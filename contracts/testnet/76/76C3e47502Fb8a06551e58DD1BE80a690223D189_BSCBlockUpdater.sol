...
contract BSCBlockUpdater is Initializable, OwnableUpgradeable {
    ...
    //    event ImportValidator(uint256 indexed epoch, bytes32 preValidatorSetHash,bytes32 validatorSetHash,uint256 lastBlockNumber);
    event ImportValidator(uint256 indexed epoch, bytes32 preValidatorSetHash, bytes32 validatorSetHash, bytes32[16] blockHash, uint256 lastBlockNumber);
    event ImportBlock(uint256 indexed epoch,bytes32[16] blockHash, uint256 lastBlockNumber);
    ...
    function importNextValidatorSet(
        ...
        //        uint256 u4 = gasleft();
        //        uint256 setValidatorSetHashGas = u3 - u4;

        bytes32[16] memory blockHash = _addBlock(parsedInput.lastBlockNumber, hashList);
        //        uint256 u5 = gasleft();
        //        uint256 setBlockInfoGas = u4 - u5;
        //emit GasLog(parseAndVerifyGas, verifyMerkleRootGas, verifyProofGas, setValidatorSetHashGas, setBlockInfoGas);
        emit ImportValidator(epoch, parsedInput.preValidatorSetHash, parsedInput.currentValidatorSetHash, blockHash, parsedInput.lastBlockNumber);
    }
    ...
    function importBlock(
        ...
        lastBlockNumber = parsedInput.lastBlockNumber;
        bytes32[16] memory blockHash = _addBlock(parsedInput.lastBlockNumber, hashList);
        emit ImportBlock(epoch,blockHash, parsedInput.lastBlockNumber);
    }
    ...
    function checkBlockHash(bytes32 blockHash, bytes32 receiptsRoot) external view returns (bool) {
        bytes32 _receiptsRoot = blockInfos[blockHash];
        if (_receiptsRoot != bytes32(0) && _receiptsRoot == receiptsRoot) {
            return true;
        }
        return false;
    }
    ...
    function _addBlock(uint256 lastBlockNumber, bytes32[32] calldata hashList) internal returns (bytes32[16] memory blockHash){
        for (uint16 i = 0; i < BATCH_SIZE; i++) {
            blockInfos[hashList[i]] = hashList[i + BATCH_SIZE];
            blockHash[i] = hashList[i];
        }
    }
