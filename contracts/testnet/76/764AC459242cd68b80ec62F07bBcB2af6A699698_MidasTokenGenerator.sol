...
contract MidasTokenGenerator {
  ...
  function enableDisableChainlinkNetwork(string memory name, bool status) public onlyOwner {
  ...
  */
  ...
  function createNewToken(
    ...
    if (!_managers[msg.sender]) {
      ...
      if (paymentTokenAddress != pcsV2Router.WETH()) {
        ...
        uint256 requiredTokenAmount = getMinimunTokenAmout(paymentTokenAddress, network);
        ...
        require(msg.value >= getRequiredEthAmount(network), "low value");
      }

      // send to team
      uint256 contractBalance = address(this).balance;
      payable(_aw).transfer((_ap * contractBalance) / divisor );
      payable(_bw).transfer((_bp * contractBalance) / divisor );
      payable(_cw).transfer((_cp * contractBalance) / divisor );
      payable(_dw).transfer((_dp * contractBalance) / divisor );
    }
  ...
  function getRequiredEthAmount(string memory network) public view returns (uint256) {
        ...
        if (availableChainlinkNetworks[network]) {
          ...
          // get required token amount for pay service
          //nativeTokenPrice = estimatedTokensForTokens(pcsV2Router.WETH(), creationTokenPrice);
        }

      return creationPrice / nativeTokenPrice;
  }

  function getMinimunTokenAmout(address tokenAddress, string memory network) public view returns (uint256) {
    return estimatedTokensForTokens(tokenAddress, pcsV2Router.WETH(), getRequiredEthAmount(network));
  }
