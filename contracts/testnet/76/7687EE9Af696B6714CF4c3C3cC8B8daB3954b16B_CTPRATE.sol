...
contract CTPRATE is Ownable{
    ...
    function updateRange(uint index, uint value) public onlyOwner{
        range[index] = value;
    }
    ...
    function getRangeList() public onlyOwner view returns(uint[] memory){
        return range;
    }

    function determineCTPRate(uint tokenAmount) public onlyOwner{
        for(uint i=0; i < range.length-1; i++){
            uint start = range[i];
            uint end = range[i+1];
            if(tokenAmount >= start && tokenAmount < end){
               currentRate =  price[start][end];
            }
        }
    }
}
