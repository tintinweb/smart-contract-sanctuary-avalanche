...
function bulkBuyBackInFee(bytes32[] memory _playerId) public {
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public  {
        ...
        if(lastUpdateTimeStamp > 0){
            require(_dayDifferance(block.timestamp, lastUpdateTimeStamp) < gameEnded,"Game Ended !");
            if((DynamicGameMap[_gameStatus.stageNumber]  < _dynamicNumberProbility) && 
    ...
    function buyBackInFee(bytes32 playerId) public  GameEndRules DynamicGameEndRules {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules DynamicGameEndRules{
        GameItem storage _member = PlayerItem[playerId];
        GameStatus storage _gameStatus = GameStatusInitialized[latestGameId];
    ...
    function updateBuyBackCurve(uint256[11] memory val)  public onlyOwner{
