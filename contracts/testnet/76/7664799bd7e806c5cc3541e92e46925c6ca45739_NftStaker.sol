...
contract NftStaker is Ownable {
    ...
    function unstake(uint256[] memory _tokenIds) public {
        ...
        if (stakedAmount[msg.sender] == 0) {
            for (uint256 x = 0; x < stakers.length; x++) {
                if (stakers[x] == msg.sender) {
    ...
    function distributeStakingPoints() public onlyOwner() {
        for (uint256 i = 0; i < stakers.length; i++) {
            stakingPoints[stakers[i]] = stakingPoints[stakers[i]] + stakedAmount[stakers[i]];
        }
    }
