...
contract Vault is ERC20, ReentrancyGuard, Pausable {
  ...
  function getEquity() public view returns (uint256) {
    (,
     ,
     uint256 _tokenAEquity,
     uint256 _tokenBEquity
    ) = IManager(config.getManagerAddress()).equityInfo(address(this));
    return _tokenAEquity + _tokenBEquity;
  }
  ...
  function _transferTokenToVault(address _token, uint256 _depositAmt) internal {
...
interface IManager {
  ...
  function equityInfo(address _vault) external view returns (uint256, uint256, uint256, uint256);
