    ...
    Ownable
{
    ...
    uint256 public serviceFee = 25 * 10**17;
    ...
    function viewOfferingNFT(address _tokenContract, uint256 _tokenId)
        public
        view
        returns (CreateOffering[] memory)
    {
        uint totalOffersCount = _offeringIds.current();
        uint currentIndex = 0;
        CreateOffering[] memory items = new CreateOffering[](totalOffersCount);
        for (uint i = 0; i < totalOffersCount; i++) {
            if (offerersData[i + 1].nftAddress == _tokenContract && offerersData[i + 1].tokenId == _tokenId) {
                uint currentId = i + 1;
                CreateOffering storage currentItem = offerersData[currentId];
                items[currentIndex] = currentItem;
                currentIndex += 1;
    ...
    function acceptOffer(uint _nonce) external nonReentrant {
        ...
        if (nft.supportsInterface(0xd9b67a26) == true) {
            ...
            // TO DO: Add Service Fee and royalty fee parts
            payable(msg.sender).transfer(currentOffer.bidPrice);
            delete (offerersData[_nonce]);
