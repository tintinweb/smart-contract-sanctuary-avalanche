...
contract NODERewardManagement {
    ...
    mapping(address => uint256[]) private _nodeIdsOfUser;
    mapping(address => mapping(uint256 => uint256))private _nodesCountOfUserByType;
    mapping(uint256 => NodeEntity) private nodeEntityById;

    uint256[] public nodePrice = [1250*10**15, 6250*10**15, 12500*10**15];
    uint256[] public rewardPerNode = [8*10**15, 50*10**15, 144*10**15];
    uint256[] public monthlyMaintenanceFee = [1399,2799,5599];  //100X USDT
    uint256[] public ROT_PERIOD = [156, 125, 87]; //days
    uint256 public TotalNodeTypes = 3;
    uint256 public graceDays = 5 days;
    uint256 public MAX_NODES_PER_USER = 100;
    ...
    function _getTotalNodesCount(address account) external view onlySentry returns (uint256){
        return _nodeIdsOfUser[account].length;
    }

    function _getNodeIdByIndex(address account, uint256 index) external view onlySentry returns(uint256){
        return _nodeIdsOfUser[account][index];
    }

    function _getNodesCountByType(address account, uint256 nodeType) external view onlySentry returns(uint256){
        return _nodesCountOfUserByType[account][nodeType];
    }

    function createNode(address account, string memory nodeName, uint256 nodeType_) external onlySentry {
        require(_nodeIdsOfUser[account].length <= MAX_NODES_PER_USER , "max node limit exceed");
        require(
        ...
        _nodeIdsOfUser[account].push(totalNodesCreated);
        nodeEntityById[totalNodesCreated]=node;
        totalNodes[nodeType_]++;
        totalNodesCreated++;
        _nodesCountOfUserByType[account][nodeType_]++;
    }
    ...
    returns (bool)
    {
        uint256[] memory nodeIds = _nodeIdsOfUser[account];
        for (uint256 i = 0; i < nodeIds.length; i++) {
            if (keccak256(bytes(nodeEntityById[nodeIds[i]].name)) == keccak256(bytes(nodeName))) {
                return false;
            }
        }
        return true;
    ...
    function getNodeWithId(uint256 id) external view returns (NodeEntity memory) {
        return nodeEntityById[id];
    }

    function pendingNodeReward(address account, uint256 id) external view onlySentry
    returns (uint256)
    {
        bool find = false;
        if ( (id < totalNodesCreated) && !Isexpired(id) ){
            uint256[] memory nodeIds = _nodeIdsOfUser[account];
            uint256 nodesCount = nodeIds.length;
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            uint256 id = nodeIds[i];
            NodeEntity memory node = nodeEntityById[id];
            if (Isexpired(id)){
                continue;
            }
            if (node.nodeType == nodeType){
                rewardNode = (block.timestamp - node.lastClaimTime).mul(rewardPerNode[node.nodeType]).div(1 days);
    ...
    external onlySentry
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            uint256 id = nodeIds[i];
            if (nodeEntityById[id].nodeType == nodeType){
                nodeEntityById[id].lastClaimTime = block.timestamp;
            }
        }
    }
    ...
    returns (string memory)
    {
        ...
        string memory names = nodeEntityById[nodeIds[0]].name;
        string memory separator = "#";
        for (uint256 i = 1; i < nodesCount; i++) {
            _node = nodeEntityById[nodeIds[i]];
    ...
    returns (string memory)
    {
        ...
        uint256 nodesCount = nodeIds.length;
        NodeEntity memory _node;
        string memory _monthlyPayTimes = uint2str(nodeEntityById[nodeIds[0]].monthlyPayTime);
        string memory separator = "#";
        for (uint256 i = 1; i < nodesCount; i++) {
            _node = nodeEntityById[nodeIds[i]];
    ...
    returns (string memory)
    {
       ...
       uint256[] memory nodeIds = _nodeIdsOfUser[account];
        uint256 nodesCount = nodeIds.length;
        NodeEntity memory _node;
        string memory _lastClaimTimes = uint2str(nodeEntityById[nodeIds[0]].lastClaimTime);
        string memory separator = "#";
        for (uint256 i = 1; i < nodesCount; i++) {
            _node = nodeEntityById[nodeIds[i]];
    ...
    function setMonthlyMaintenanceFee(uint256[] memory value) external onlySentry{
    ...
    function isNodeOwner(address account) private view returns (bool) {
        return _nodeIdsOfUser[account].length > 0;
    }
    ...
    function _isGodUser(address account) external view returns (bool){
        for(uint256 i = 0; i < TotalNodeTypes; i++){
            if (_nodesCountOfUserByType[account][i] == 0){ return false;}
        }
        return true;
    }
    ...
    function _isWithinROTPeriod(uint256 id) external view returns (bool){
        NodeEntity memory node = nodeEntityById[id];
        return block.timestamp < (node.creationTime + ROT_PERIOD[node.nodeType] * 1 days);
    }
...
contract CAP is ERC20, Ownable {
    ...
    // address public usdtTokenAddress = 0xc7198437980c041c805A1EDcbA50c1Ce5db95118;
    address public usdtTokenAddress = 0xEdDEB2ff49830f3aa30Fee2F7FaBC5136845304a;
    ...
    uint256 public ROT_PERIOD_FEE_MUL=1;
    ...
    function updateROT_PERIOD_FEE_MUL(uint256 val) public onlyOwner {
        ROT_PERIOD_FEE_MUL = val;
    }
    ...
    function updateUSDTTokenAddress(address addr) public onlyOwner{
        usdtTokenAddress = addr;
    }
    ...
    function getAVAXForUSDT(uint usdtAmount100x) public view returns (uint) {
        ...
        return uniswapV2Router.getAmountsOut(usdtAmount100x * 10 ** 4, path)[1];
    }
    ...
    function setSwapLiquify(bool val) public onlyOwner{
        swapLiquify = val;
    }
    ...
    function claimReward(uint256 id) public {
        ...
        if (nodeRewardManager._isWithinROTPeriod(id)){
            feeMultiplier = ROT_PERIOD_FEE_MUL;
        }
    ...
    function claimAll() public {
        ...
        uint256 nodesCount = nodeRewardManager._getTotalNodesCount(sender);

        for (uint256 i = 0; i < nodesCount; i++){
            uint256 id = nodeRewardManager._getNodeIdByIndex(sender, i);
            rewardAmountofNode = nodeRewardManager.pendingNodeReward(sender, id);
    ...
    function getNodesCountByType(address account, uint256 nodeType) public view returns(uint256){
       return nodeRewardManager._getNodesCountByType(account, nodeType);
    }

    function getNodeIdByIndex(address account, uint256 index) public view returns(uint256){
        return nodeRewardManager._getNodeIdByIndex(account, index);
    }

    function getNodesIds(address account) public view returns (uint256[] memory)
    {
        return nodeRewardManager._getNodesIds(account);
    }
    ...
    function setMonthlyMaintenanceFee(uint256[] memory value) public onlyOwner{
        nodeRewardManager.setMonthlyMaintenanceFee(value);
    }

    function isGodUser(address account) public view returns (bool){
        return nodeRewardManager._isGodUser(account);
    }
}
...
contract MasterChef is Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        totalFarmed= totalFarmed.add(_amount);
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount>0){
            ...
            totalFarmed= totalFarmed.sub(_amount);
        }
