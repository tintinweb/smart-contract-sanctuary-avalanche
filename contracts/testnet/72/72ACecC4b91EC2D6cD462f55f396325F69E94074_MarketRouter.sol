...
contract MarketCore is MarketPojo{
    ...
    mapping(address => mapping(bytes=> bool)) private effectiveSignature;
    ...
    function calldataWithFee(bytes memory CallData) internal view returns(bytes memory callDataA,bytes memory callDataB){
        ...
        callDataA = abi.encodeWithSelector(SELECTOR,from,feeTo,valueA);  //这里的to是收税员的地址
        callDataB = abi.encodeWithSelector(SELECTOR,from,to,valueB);
        return(callDataA,callDataB);
    }
        
    function decodeCalldataErc20(bytes memory data) internal view returns(address from,address to,uint256 value){
    ...
    function commitOrder(  
        ...
        effectiveSignature[msg.sender][signature] = true;
    ...
    function orderMatch_(bytes memory hashOrders,bytes memory singers, bytes memory callDatas,bytes memory callTarget, bytes memory signatures,bytes memory orderData) internal {
        ...
        require(effectiveSignature[firstsinger][firstSignature] == true,"Invalid signature");
        ...
        effectiveSignature[firstsinger][firstSignature] == false;
