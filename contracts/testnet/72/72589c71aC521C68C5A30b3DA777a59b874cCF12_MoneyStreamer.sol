        ...
        returns (Types.Stream[] memory streamsByRecipient)
    {
        ...
        for (uint256 i = 1; i < nextStreamId; i++) {
            if (streams[i].recipient == recipient) {
                iteratingStreams[i].deposit = streams[i].deposit;
                iteratingStreams[i].ratePerSecond = streams[i].ratePerSecond;
                iteratingStreams[i].remainingBalance = streams[i]
                    .remainingBalance;
                iteratingStreams[i].startTime = streams[i].startTime;
                iteratingStreams[i].stopTime = streams[i].stopTime;
                iteratingStreams[i].recipient = streams[i].recipient;
                iteratingStreams[i].sender = streams[i].sender;
                iteratingStreams[i].tokenAddress = streams[i].tokenAddress;
                iteratingStreams[i].isActive = streams[i].isActive;
                iteratingStreams[i].isCanceled = streams[i].isCanceled;
                iteratingStreams[i].isFinished = streams[i].isFinished;
            }
        }
