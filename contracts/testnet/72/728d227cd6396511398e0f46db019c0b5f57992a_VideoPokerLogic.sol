...
contract VideoPokerLogic {
  ...
  function replace(uint256 cards_, uint change_, uint256 _oldRandomness, uint256 _newRandomness) public pure returns (uint256, uint256, uint[5] memory, uint[5] memory, uint[5] memory) {
    ...
    if ((change_ & 1) != 0) {
      ...
      cards_ = (cards_ & MASK_CARD_4) | cardValues[0];
    }
    if ((change_ & 2) != 0) {
      cardValues[1] = nextCard(builder_);
      cards_ = (cards_ & MASK_CARD_3) | (cardValues[1] << 6);
    }
    if ((change_ & 4) != 0) {
    ...
    if ((change_ & 8) != 0) {
      ...
      cards_ = (cards_ & MASK_CARD_1) | (cardValues[3] << 18);
    }
    if ((change_ & 16) != 0) {
      cardValues[4] = nextCard(builder_);
      cards_ = (cards_ & MASK_CARD_0) | (cardValues[4] << 24);
    }
