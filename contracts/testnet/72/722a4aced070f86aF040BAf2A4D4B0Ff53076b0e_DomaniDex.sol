...
contract DomaniDex is IDomaniDex, Ownable, ReentrancyGuard {
  ...
  string public constant EXACT_INPUT_SIG =
    "swapExactInput(bytes,(uint256,uint256,bytes,bool,uint256,address))";

  string public constant EXACT_OUTPUT_SIG =
  ...
  function sweepToken(IERC20 token, address payable recipient)
    external
    nonReentrant
    returns (uint256)
  {
    bool isETH = address(token) == DomaniDexConstants.NATIVE_ADDR;
    uint256 balance = isETH ? address(this).balance : token.balanceOf(address(this));
    if (balance > 0) {
      if (isETH) {
  ...
  function getRequiredComponents(
    ...
    uint256 _quantity,
    bool isIssue
  ) public view onlyValidFund(_fund) returns (address[] memory, uint256[] memory) {
    address[] memory components = _fund.getComponents();

    uint256[] memory notionalUnits = new uint256[](components.length);

    for (uint256 i = 0; i < components.length; i++) {
      uint256 singleUnit = _fund.getDefaultPositionRealUnit(components[i]).toUint256();
      notionalUnits[i] = isIssue
