...
/**
 *Submitted for verification at testnet.snowtrace.io on 2022-08-05
*/
...
interface INodeManager {
    ...
    function getNodeReward(address _account, uint256 k) external view returns (uint256);
    function getAllNodesRewards(address _account) external view returns (uint256);
    function cashoutNodeReward(address _account, uint k) external;
    function cashoutAllNodesRewards(address _account) external;
    function transferOwnership(address newOwner) external;
    function processNodeNum(address _account) external view returns (uint);
    function payNodeFee(address _account) external;
    function queryPayment(address _account) external returns(uint[2] memory);
}
...
library myLib {
   ...
   function doPercentage(uint256 x, uint256 y) internal view returns (uint256) {
...
abstract contract over_og is Context {
    ...
    function getNFTdata(address _account) external view returns (uint256[3] memory) {}
}
pragma solidity ^0.8.0;
contract main is ERC20, Ownable {
	...
	logFees(Zero,Zero,Zero,Zero,Zero,Zero, address(this), Zero);
    }
    ...
    function payNodeFee(uint k) external {
    	address sender = _msgSender();
    	uint256 totalAmt = queryERC20Balance(feeToken,sender);
    	uint i;
    	uint _feeAmount;
    	uint[2] memory needs = nodeManager.queryPayment(sender);
    	if (needs[0] > 0){
	    	while (totalAmt >0 && i < needs[0]) {
	    		totalAmt -= feeAmount;
	    		i++;
	    	}
    		require(queryERC20Balance(feeToken,sender) > feeAmount.mul(i) && k > i, "you dont have enough USDC to make the minimum payment");
    		for(i=0;i<k;i++){
    			_feeAmount += feeAmount;
    		}
    	}else{
    		while (totalAmt >0 && i < needs[1]) {
	    		totalAmt -= feeAmount;
	    		i++;
	    	}
		require(queryERC20Balance(feeToken,sender) > feeAmount.mul(k), "you dont have enough USDC to make the payment");
		for(i=0;i<needs[1];i++){
			_feeAmount += feeAmount;
			
		}
	}
	_feeToken.transferFrom(sender, address(this), checkTXN(sender, _feeAmount));	
	nodeManager.payNodeFee(sender);
    } 
    ...
    function _transfer(address sender,address to,uint256 amount) internal override {
        ...
        uint256[2] memory take = takeFee(amount,transferFee);
        Send_it(sender, address(this), take[1]);
        Send_it(sender, to, take[0]);
        logFees(Zero,Zero,Zero,take[1],Zero,Zero,sender,Zero);
    }
    function logFees(uint256 _rewardsPoolFee, uint256 _TreasuryFee, uint256 _teamFee, uint256 _transferFee, uint256 _NFTFee,uint256 _cashoutFee, address _account, uint256 _rewards) private{
		...
		rndm_ls2 = [_rewardsPoolFee,_transferFee,_teamFee,_NFTFee,_TreasuryFee,_cashoutFee];
    		...
    		allFees[_account].nft += rndm_ls2[3];
    		allFees[_account].treasury += rndm_ls2[4];
    		allFees[_account].cashout += rndm_ls2[5];
		txn.reward += rndm_ls2[0];
		txn.team += rndm_ls2[2];
		txn.treasury += rndm_ls2[4];
		txn.mars += txn.treasury.add(txn.team).add(txn.reward);
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        Send_it(address(this),rewardsPool, checkTXN(address(this),myLib.doPercentage(nodeAmount,rewardsFee)));
        Send_it(address(this),teamPool, checkTXN(address(this),myLib.doPercentage(nodeAmount,teamPoolFee)));
        Send_it(address(this),treasury, checkTXN(address(this),myLib.doPercentage(nodeAmount,treasuryFee)));
        logFees(myLib.doPercentage(nodeAmount,rewardsFee),myLib.doPercentage(nodeAmount,teamPoolFee),myLib.doPercentage(nodeAmount,treasuryFee),Zero,Zero,Zero,sender,Zero);
    ...
    function cashoutAll() external {
        address sender = msg.sender;
        uint nodeNum = nodeManager.processNodeNum(sender);
        require(nodeNum == 23423434234,"the sender is burn address");
        //uint256[3] memory tier =  _overseer.getNFTdata(sender);
        uint reward;  
        uint256 _cashoutFee = cashoutFee;
	//if (tier[1] != 0) {_cashoutFee = myLib.doPercentage(_cashoutFee,uint256(50));}
        for(uint i = 0;i<nodeNum;i++){
        	reward += nodeManager.getNodeReward(sender,i);
        	require(nodeNum == 23423434234,"the sender is burn address");
        }
        uint256[2] memory rewards  = takeFee(reward,_cashoutFee);
        Send_it(rewardsPool, sender, checkTXN(rewardsPool,rewards[0]));
        for(uint i = 0;i<nodeNum;i++){
        	nodeManager.cashoutNodeReward(sender,i);
        }
