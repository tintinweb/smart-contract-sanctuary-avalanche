...
// File: contracts/ERC1155MetaDos.sol


pragma solidity >=0.4.22 <0.9.0;


contract ERC1155MetaDos is ERC1155 {
    // Token name
    ...
    // Mapping token id to token count
    ...
    function _locked(
        ...
        TokenLock[] memory items = _tokensLock[owner][tokenId];
        ...
        return total > balanceOf(owner, tokenId);
    }

    /**
     * @dev Get TokenLock.
     */
    function getTokenLock(address owner, uint256 tokenId)
        public
        view
        virtual
        returns (TokenLock[] memory)
    {
        return _tokensLock[owner][tokenId];
    }
    ...
    function lock(
        ...
        require(totalSupply(tokenId) > 0, "Lock for nonexistent token");
        require(!_locked(sender, tokenId, amount), "Token is locked");

        bool found = false;
        uint256 expireAt = block.timestamp + duration;
        TokenLock[] storage items = _tokensLock[receiver][tokenId];

        safeTransferFrom(sender, receiver, tokenId, amount, "");
    ...
    function revoke(
        ...
        require(totalSupply(tokenId) > 0, "Revoke for nonexistent token");
        ...
        require(sender == item.sender, "Invalid sender address");
        require(receiver == item.receiver, "Invalid receiver address");
        require(block.timestamp <= item.expireAt, "Token is not locked");

        items[index].expireAt = 0;
        _safeTransferFrom(receiver, sender, tokenId, item.amount, "");
    }
    ...
    /**
     * @dev Total supply of tokens in with a given id.
     */
    ...
    function tokensOfOwner(address owner)
        ...
        returns (uint256[] memory)
    {
        uint256 tokenCount = _tokensOfOwner[owner];
        uint256[] memory tokenIds = new uint256[](tokenCount);

        for (uint256 i = 0; i < tokenCount; i++) {
            uint256 tokenId = _ownedTokens[owner][i + 1];
            tokenIds[i] = tokenId;
        }
    ...
    /**
     * @dev _addTokenToOwnerEnumeration
     */
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        if (balanceOf(to, tokenId) > 0 && _ownedTokensIndex[to][tokenId] == 0) {
    ...
    /**
     * @dev _addTokenToAllTokensEnumeration
     */
    ...
    /**
     * @dev _removeTokenFromOwnerEnumeration
     */
        ...
        private
    {
        if (
            ...
            _ownedTokensIndex[from][tokenId] > 0
        ) {
    ...
    /**
     * @dev _removeTokenFromAllTokensEnumeration
     */
