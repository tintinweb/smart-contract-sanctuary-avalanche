...
contract ETFZapper is BNum {
    ...
    function joinMulti(
        ...
        uint poolAmountOut
    ) external returns (uint amountInTotal) {
        address[] memory tokens = IETFToken(ETFToken).getCurrentTokens();
        require(
            tokens.length == intermediaries.length,
            "DexETF: Invalid arrays length"
        );
        uint[] memory amountsToPool = new uint[](tokens.length);
        uint ratio = bdiv(poolAmountOut, IERC20(ETFToken).totalSupply());
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            uint usedBalance = IETFToken(ETFToken).getUsedBalance(tokens[i]);
            uint amountOut = bmul(ratio, usedBalance);
            if (swap) {
                uint[] memory amounts = _swap(amountOut, path, address(this));
    ...
    function _getPathForSwap(
        ...
        address tokenOut
    ) public view returns (bool swap, address[] memory path) {
        if (tokenIn == tokenOut) return (swap, path);
        // path.push(tokenIn);
        // if (intermediate != address(0)) path.push(intermediate);
        // path.push(tokenOut);
        // swap = true;
        path[0] = tokenIn;
    ...
    function getMaxAmountForJoinMulti(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (swap) {
                ...
                amountInMax = badd(amountInMax, 1);
            } else {
                amountInMax = badd(amountInMax, 1);
            }
        }
    ...
    function getMaxAmountForExitMulti(
        ...
        uint256 ratio = pAiAfterExitFee / IERC20(ETFToken).totalSupply();
    ...
    function _getSwapAmountsForJoin(
        ...
        uint256 amountToPool = ratio * usedBalance;
        if (tokenIn == tokenOut) {
        ...
        amounts = router.getAmountsIn(amountToPool, path);
        swap = true;
    }
    ...
    function _getSwapAmountsForExit(
        ...
        uint256 amountOutPool = ratio * usedBalance;

        if (tokenIn == tokenOut) {
            amounts[0] = amountOutPool;
            return (amounts, swap, path);
        }
        ...
        amounts = router.getAmountsOut(amountOutPool, path);
        // amounts = UniswapV2Library.getAmountsOut(factory, amountOutPool, path);
        swap = true;
    }
}
