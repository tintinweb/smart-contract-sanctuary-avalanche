    ...
    ReentrancyGuardUpgradeable
{
        ...
        )
    {
        ...
        require(
            ...
            "ERC721 ValiFi: No YieldBox currently with rewards"
        );
        require(
        ...
        )
    {
        ...
        for (uint256 i = 0; i < YieldBoxIds.length; i++) {
            ...
            if (
                ...
                rewarded[i] = (MINIMAL_REWARDS_DAYS / 1 minutes) > RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed ? (MINIMAL_REWARDS_DAYS / 1 minutes) - RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed : 0;
                amount[i] = mulDiv(
                    rewarded[i],
                    REWARDS_PER_DAYS,
                    MINIMAL_REWARDS
        ...
        nonReentrant
    {
        for (uint256 i = 0; i < _yieldKey.length; i++) {
            ...
            require(
                ...
                "ERC721 ValiFi: Not enough ValiFi tokens in pool"
            );
            ...
            require(success_treasury, "ERC721 ValiFi: Rewards claim failed");
        ...
        nonReentrant
    {
        ...
        require(
            success_treasury,
            "ERC721 ValiFi: Minimal rewards claim failed"
        );
        for (uint256 i = 0; i < YieldBoxRewarded.length; i++) {
            if (amount[i] > 0) {
                emit RewardsPaid(
