...
/**
 * @dev Dagora is marketplace develop by Coin98, allow user to listing and sale NFT
 */
contract Dagora is Ownable {
    ...
    constructor(address _paymentAddress) {
        ...
        registerPackage(address(0),250,0);
        paymentAddress = _paymentAddress;
    }

    modifier isUnuseSignature(bytes memory _signature) {
        require(!_signatureUseds[_signature], "Dagora: Invalid signature format");
        _;
    }

    modifier isValidPackage(address _package) {
        require(_packageInfos[_package].flag, "Dagora: Package not active");
        _;
    }

    modifier isValidTime(uint256 _time) {
        require(block.timestamp <= _time, "Dagora: Time over");
        _;
    }

    modifier onlyAdmin() {
        require(_admins[msg.sender], "Dagora: Only admin can call");
        _;
    }
    ...
    function registerPackage(address _token, uint256 _marketFee, uint256 _claimFee) public onlyOwner() {
        PackageInfo storage packageInfo = _packageInfos[_token];

        packageInfo.flag = true;
        packageInfo.martketFee = _marketFee;
        packageInfo.claimFee = _claimFee;
    }

    function configurePackage(address _token, uint256 _marketFee, uint256 _claimFee) public onlyOwner() {
        PackageInfo storage packageInfo = _packageInfos[_token];
        require(packageInfo.flag,"Dagora: Package not active");
    ...
    function _validateNFT(IERC721 _token, address _owner, uint256 _id, bool _checkApprove) internal view returns (bool) {
        if (_token.ownerOf(_id) != _owner) {
            return false;
        }
    ...
    function _disableSignature(bytes memory _signature) internal {
        _signatureUseds[_signature] = true;
    }

    function _subFee(uint256 _amount, uint256 _claimFee, uint256 _marketFee) internal pure returns(uint256) {
    ...
    function _buyItem(address[] memory _tokens, uint256[] memory _ids, address _seller, uint256 _amount, address _saleToken) internal {
        if (_saleToken == address(0)) {
            require(msg.value >= _amount, "Dagora: Not enough payment");
        } else {
            IERC20(_saleToken).safeTransferFrom(msg.sender, address(this), _amount);
        }

        // transfer meta
        _safeTransaferFrom(_tokens, _ids, _seller, msg.sender);

        uint256 paymentAmount = _subFee(_amount, _packageInfos[_saleToken].claimFee, _packageInfos[_saleToken].martketFee);

        if (_saleToken == address(0)) {
            payable(_seller).transfer(paymentAmount);
        } else {
            IERC20(_saleToken).safeTransfer(_seller, paymentAmount);
        }
    }

    function _safeTransaferFrom(address[] memory _tokenAddresses, uint256[] memory _tokenIds, address _from, address _to) private {
    ...
    function cancel(address[] calldata metaAddress, uint256[] calldata metaUInt, bytes memory _signature) public {
        for (uint256 i = 3; i < metaAddress.length; i++) {
            ...
            require(_validateNFT(meta, msg.sender, metaUInt[i + 1], false), "Dagora: Invalid NFT");
        }
        require(_verifySignature(metaAddress, metaUInt, _signature, msg.sender), "Dagora: Signature not match");
    ...
    // @param metaAddress define with a list of address below [sale by token, seller, buyer, ...erc721 meta token]
    // @param metaUInt define with a list of Uint below [amount , time, duration, nonce, ...token id]
    function buy(address[] calldata metaAddress, uint256[] calldata metaUInt, bytes memory _signature) public payable isUnuseSignature(_signature) isValidPackage(metaAddress[0]) isValidTime(metaUInt[1] + metaUInt[2]) {
        require(_verifySignature(metaAddress, metaUInt, _signature, metaAddress[1]), "Dagora: Signature not match");
        require(metaAddress.length > 4, "Dagora: Invalid Input");
        require(metaAddress.length == metaUInt.length + 1, "Dagora: Invalid Input");

        address[] memory tokenSaleList = new address[](metaAddress.length - 3);
        uint256[] memory tokenIdList = new uint256[](metaUInt.length - 4);

        for (uint256 i = 0; i < metaAddress.length - 3; i++) {
            tokenSaleList[i] = metaAddress[i + 3];
            tokenIdList[i] = metaUInt[i + 4];
        }

        _buyItem(tokenSaleList, tokenIdList, metaAddress[1], metaUInt[0], metaAddress[0]);
        _disableSignature(_signature);

        emit Buy(msg.sender, metaAddress, metaUInt);
    }


    function listing(address _saleByToken, address[] calldata _tokenAddresses, uint256[] calldata _tokenIds, uint256 _startPrice, uint256 _expiresAt) public isValidTime(_expiresAt) {
        require (_tokenAddresses.length == _tokenIds.length, "Dagora: Invalid Input");
        require(_saleByToken != address(0), "Dagora: Native token not support");
        for (uint i = 0; i < _tokenAddresses.length; i++) {
            IERC721 erc721 = IERC721(_tokenAddresses[i]);
            require(_validateNFT(erc721, msg.sender, _tokenIds[i], true));
        }

        bytes32 nftHash = keccak256(abi.encodePacked(_tokenAddresses, _tokenIds));

        require(_listingInfos[nftHash].seller == address(0), "Dagora: List item already listed on dagora");
        ...
        _safeTransaferFrom(_tokenAddresses, _tokenIds, msg.sender, address(this));
    ...
    function cancelListing(address[] calldata _tokenAddresses, uint256[] calldata _tokenIds) public {
        ...
        require(_listingInfos[nftHash].seller == msg.sender, "Dagora: Sender does not owner");

        _deleteListingInfo(nftHash);
        _safeTransaferFrom(_tokenAddresses, _tokenIds, address(this), msg.sender);
    ...
    function endBid(address[] calldata _tokenAddresses, uint256[] calldata _tokenIds, uint256 _amount, address _buyer, bytes memory _signature, uint _nonce) public onlyAdmin isUnuseSignature(_signature) {
        ...
        require(listingInfo.seller != address(0), "Dagora: Listing does not exist");
        require(listingInfo.startPrice < _amount, "Dagora: Amount should larger than startPrice");
        require(listingInfo.expiresAt < block.timestamp, "Dagora: Bid does not end");

        bytes32 messageHash = keccak256(abi.encodePacked(nftHash, _amount, message, _nonce));
        require(_verifySignature(messageHash, _signature, _buyer), "Dagora: Invalid Signature");

        IERC20 saleByToken = IERC20(listingInfo.saleByToken);
        saleByToken.safeTransferFrom(_buyer, address(this), _amount);

        PackageInfo memory package = _packageInfos[listingInfo.saleByToken];
        uint256 paymentAmount = _subFee(_amount, package.claimFee, package.martketFee);

        saleByToken.safeTransfer(listingInfo.seller, paymentAmount);
        _deleteListingInfo(nftHash);
        _disableSignature(_signature);

        _safeTransaferFrom(_tokenAddresses, _tokenIds, address(this), _buyer);

        emit EndBid(_buyer, nftHash, _amount);
    }

    function addAdmins(address[] calldata _ads) public onlyOwner {
        for(uint256 i = 0; i < _ads.length; i++) {
            _admins[_ads[i]] = true;
        }
    }

    function deleteAdmins(address[] calldata _ads) public onlyOwner {
        for(uint256 i = 0; i < _ads.length; i++) {
            _admins[_ads[i]] = false;
        }
    }
    ...
    function getListingInfo(bytes32 _nftHash) public view returns(ListingInfo memory) {
        return _listingInfos[_nftHash];
    }

    function getPackageInfo(address _token) public view returns(PackageInfo memory) {
        return _packageInfos[_token];
    }
