...
import {MockBatchReveal} from "./MockBatchReveal.sol";

contract MockLaunchpeg is ERC721, MockBatchReveal {
    using Strings for uint256;

    constructor(string memory _name, string memory _symbol, uint256 _revealBatchSize, uint256 _collectionSize)
        ERC721("ERC721Mock", "ERC721Mock")
        MockBatchReveal(_revealBatchSize, _collectionSize)
    {}
...
contract MockBatchReveal is Ownable2Step {
    struct Range {
        int128 start;
        int128 end;
    }
    ...
    function seedBatchToSeed(uint256[] calldata randomSeeds) external onlyOwner {
