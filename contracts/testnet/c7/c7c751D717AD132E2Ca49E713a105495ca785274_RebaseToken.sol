...
contract RebaseToken is ERC20Detailed, Ownable, ReentrancyGuard, MinterRole {
    ...
    uint256 private constant MAX_REBASE_FREQUENCY = 14400;
    ...
    address public liquidityReceiver =
        0x0aCD97149364F40ff70FD2ad744fAcB5286fE101;
    address public treasuryReceiver =
        0x0aCD97149364F40ff70FD2ad744fAcB5286fE101;
    address public riskFreeValueReceiver =
        0x0aCD97149364F40ff70FD2ad744fAcB5286fE101;

    address public usdtToken = 0x82DCEC6aa3c8BFE2C96d40d8805EE0dA15708643; //mainnet
    ...
    constructor() ERC20Detailed("Rebase", "MDR", uint8(DECIMALS)) {
        router = IJoeRouter(0x2D99ABD9008Dc933ff5c0CD271B88309593aB921); //mainnet
        ...
        // address pairDai = IJoeFactory(router.factory()).createPair(
        //     usdtToken,
        //     address(this)
        //   );

        setAutomatedMarketMakerPair(pair, true);
        //  setAutomatedMarketMakerPair(pairDai, true);

        IERC20(usdtToken).approve(address(router), type(uint256).max);
        // IERC20(usdtToken).approve(address(pairDai), type(uint256).max);
        ...
        // _allowedFragments[address(this)][pairDai] = type(uint256).max;
    ...
    function mint(address recipient, uint256 amount) external onlyMinter {
        ...
        _gonBalances[recipient] = _gonBalances[recipient].add(amount.mul(_gonsPerFragment));
    }
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _gonBalances[account] = accountBalance.sub(amount.mul(_gonsPerFragment));
    ...
    function getRewardYieldDenominator() public view returns (uint256) {
        return rewardYieldDenominator;
    }

    function getRewardYield() public view returns (uint256) {
        return rewardYield;
    }
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        if (isLiquidityInAvax) {
            ...
            _swapTokensForDai(half, address(this));

            uint256 newBalance = IERC20(usdtToken).balanceOf(address(this)).sub(
                initialBalance
            );

            _addLiquidityDai(otherHalf, newBalance);

            emit SwapAndLiquifyDai(half, newBalance, otherHalf);
        }
    }
    ...
    function _addLiquidityDai(uint256 tokenAmount, uint256 daiAmount) private {
        router.addLiquidity(
            address(this),
            usdtToken,
            tokenAmount,
    ...
    function _swapTokensForDai(uint256 tokenAmount, address receiver) private {
        ...
        if (_value) {
            ...
            for (uint256 i = 0; i < _markerPairs.length; i++) {
                if (_markerPairs[i] == _pair) {
                    _markerPairs[i] = _markerPairs[_markerPairs.length.sub(1)];
                    _markerPairs.pop();
                    break;
                }
            }
    ...
    function setIsLiquidityInAvax(bool _value) external onlyOwner {
        ...
        emit SetIsLiquidityInAvax(_value);
    }
    ...
    event SetIsLiquidityInAvax(bool _value);
