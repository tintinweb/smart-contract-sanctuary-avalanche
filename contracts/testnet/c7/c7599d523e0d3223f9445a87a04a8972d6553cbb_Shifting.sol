...
contract Shifting is AccessControlledAndUpgradeable {
  ...
  // TODO: find a way to re-use the same function from the MarketTieredLeverage contract (less code duplication)
  ...
  function _getSynthPrice(
    ...
    uint32 currentExecutedEpoch = IMarketTieredLeverage(market)
      .epochInfo()
      .latestExecutedEpochIndex;

    uint256 price = IMarketTieredLeverage(market).syntheticToken_priceSnapshot(
  ...
  function _validateShiftOrder(
    ...
    address token = IMarketTieredLeverage(_marketTo).pools(_poolTypeTo, _poolTierTo).token;

    require(token != address(0), "to pool does not exist");
    require(
      _getAmountInPaymentToken(
        _marketFrom,
        _poolTypeFrom,
        _poolTierFrom,
  ...
  function shiftOrder(
    ...
    address token = MarketExtended(address(_marketFrom)).getMiaAddress(
      _poolTypeFrom,
      _poolTierFrom
    );
    ...
    if (_poolTypeFrom == IMarketCommon.PoolType.LONG) {
      MarketTieredLeverage(_marketFrom).redeemLong(_poolTierFrom, _amountOfSyntheticToken);
    } else if (_poolTypeFrom == IMarketCommon.PoolType.SHORT) {
      MarketTieredLeverage(_marketFrom).redeemShort(_poolTierFrom, _amountOfSyntheticToken);
    }

    uint32 previousEpochIndex = IMarketTieredLeverage(_marketFrom).epochInfo().previousEpochIndex;
    ...
    )
  {
    for (uint32 index = 0; index < validMarketArray.length; index++) {
      ...
      uint32 latestExecutedEpochIndex = IMarketTieredLeverage(market)
        .epochInfo()
        .latestExecutedEpochIndex;
  ...
  function shouldExecuteShiftOrder()
    external
    view
    returns (bool _canExec, bytes memory execPayload)
  {
    (
      bool canExec,
      address market,
      uint256 executeUpUntilAndIncludingThisIndex
    ) = _shouldExecuteShiftOrder();
  ...
  function _executeShiftOrder(address _marketFrom, uint256 _executeUpUntilAndIncludingThisIndex)
    internal
  {
    // First claim all oustanding DAI.
    MarketTieredLeverage(_marketFrom).executeOutstandingEpochSettlementsUser(address(this));
    ...
    for (
      ...
      uint256 syntheticToken_price = IMarketTieredLeverage(_shiftOrder.marketFrom)
  ...
  function performUpkeep(bytes calldata dataForUpkeep) external {
    (address marketFrom, uint256 executeUpUntilAndIncludingThisIndex) = abi.decode(
      dataForUpkeep,
      (address, uint256)
    );
  ...
  MarketTieredLeverageGetters
{
  ...
  modifier systemStateUpdated() {
    ...
    _updateSystemStateInternal(type(uint256).max);
    _;
  }
