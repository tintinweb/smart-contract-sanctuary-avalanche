...
import {ISorbettiere} from "../../interfaces/abracadabra/ISorbettiere.sol";
import {IBentoBoxV1} from "../../interfaces/abracadabra/IBentoBoxV1.sol";
import {ICauldronV2} from "../../interfaces/abracadabra/ICauldronV2.sol";

contract AbracadabraAdapter is AdapterBase, Basic {
    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "Abracadabra")
    {}
    ...
    function deposit(bytes calldata encodedData) external onlyProxy {
        (uint256 tokenType, uint256 amount) = abi.decode(
            encodedData,
            (uint256, uint256)
        );
    ...
    function harvest(bytes calldata encodedData) external onlyProxy {
        uint256 tokenType = abi.decode(encodedData, (uint256));
        ISorbettiere(sorbettiereAddr).claimOwnership();
    }

    function setMasterContractApproval(bytes calldata encodedData)
        external
        onlyProxy
    {
        (address boxAddr, address masterContract) = abi.decode(
            encodedData,
            (address, address)
        );
    ...
    function cook(bytes calldata encodedData) external onlyProxy {
        (
            uint8[] memory actions,
            uint256[] memory values,
            bytes[] memory datas,
            address cauldronAddr
        ) = abi.decode(encodedData, (uint8[], uint256[], bytes[], address));
        uint256 totalValue;
...
interface IBentoBoxV1 {
    ...
    function batch(bytes[] calldata calls, bool revertOnFail)
        external
        returns (bool[] calldata successes, bytes[] calldata results);
