...
contract EVICSellToken is Ownable, ContractGuard {
    ...
    constructor(
        address _polWallet,
        address _router,
        address _treasuryOracle,
        address _wetheToken, 
        address _evicToken
    ) {
        require(_polWallet != address(0), "!_polWallet");
        require(_router != address(0), "!_router");
        require(_treasuryOracle != address(0), "!_treasuryOracle");
        ...
        treasuryOracle = _treasuryOracle;
    ...
    modifier lock() {
        require(unlocked == 1, 'EVICSellToken: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    modifier onlyAdmin() {
        require(admin == msg.sender, "EVICSellToken: caller is not the admin");
        _;
    }

    function getTreasuryTwapPrice() public view returns (uint256) {
        try IOracle(treasuryOracle).twap(address(evicToken), 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("EVICSellToken: failed to twap evicToken price from the treasury oracle");
        }
    }

    function sellEvicTokenByWethe(
        uint256 _evicAmount,
        uint256 _minAmountWetheOut
    ) external onlyOneBlock lock {
        require(enabled, "Contract selling is disabled");
        uint256 maxAmountSell = evicToken.rebaseSupply().mul(maximumAmountSellPercent).div(10000);
        ...
        uint256 treasuryTwapPrice = getTreasuryTwapPrice();
        if (pegValue >= treasuryTwapPrice) {
            // tax enable
            uint256 taxAmount = 0;
            uint256 currentTaxRate = evicToken.calculateTaxRate(treasuryTwapPrice);
            if (currentTaxRate > 0) {
