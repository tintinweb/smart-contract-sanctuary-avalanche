...
contract ERC20PoolReward is RewardStrategy, Ownable, Pausable {
    ...
    function _reward(uint weight) internal view returns (uint _amount) {
      ...
      if (_lastUpdate < _end) {
        _accPerShare += tokenPerSecond * (_end - _lastUpdate) / stakingContract.totalWeight();
      }
...
contract Staking is Ownable, Pausable {
    ...
    function addRewardContract(address _reward) external onlyOwner {
      // Check the address is unique and a valid reward contract
