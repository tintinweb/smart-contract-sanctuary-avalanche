...
// File: contracts/talentstaking.sol
...
contract TalentStaking is Ownable {
   ...
   function setVoterStaking(address _voterStaking) external onlyOwner{
    ...
    // function getTalentsData()public {
    ...
    function stake(uint256 types, string memory uri, string memory category, string memory subCategory, string memory subChildCategory, string memory uid)public{ // types 1 = 30 days types 2 = 60 days types 3 = 90 days 
      ...
      require (bytes(uid).length == 8);
       ...
       if(types == 1){
       userInfo[msg.sender].expiry=block.timestamp+30 days;
       }

       else if(types == 2){
       userInfo[msg.sender].expiry=block.timestamp+60 days;
       }
       else if(types == 3){
       userInfo[msg.sender].expiry=block.timestamp+90 days;
       }
       userInfo[msg.sender].uri = uri;
       userInfo[msg.sender].category = category; 
       userInfo[msg.sender].subCategory = subCategory;
       userInfo[msg.sender].subChildCategory = subChildCategory;
       userInfo[msg.sender].uid = uid;
       nextStaking[msg.sender] = userInfo[msg.sender].expiry + DELAY;  // confirm if 30 days from staking date of from expiry of that staking
       voterStaking.updateNextStakeTimestamp(msg.sender);
    }

    function unstake()public{
       require(userInfo[msg.sender].expiry <= block.timestamp, "Need To Wait to unstake tokens");
