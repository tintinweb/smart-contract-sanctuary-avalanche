...
contract Manager is ReentrancyGuard, Pausable, AccessControl {
  ...
  function equityInfo(address _vault) public view returns (uint256, uint256, uint256) {
    ( , , uint256 tokenAAssetValue, uint256 tokenBAssetValue) = assetInfo(_vault);
    ( , , uint256 tokenADebtValue, uint256 tokenBDebtValue) = debtInfo(_vault);

    uint256 tokenAEquityValue = tokenAAssetValue - tokenADebtValue;
    uint256 tokenBEquityValue = tokenBAssetValue - tokenBDebtValue;
    uint256 totalEquityValue = tokenBAssetValue - tokenBDebtValue;

    return (tokenAEquityValue, tokenBEquityValue, totalEquityValue);
  }

  function currentAssetValue(address _vault) public view returns (uint256) {
    (, , uint256 tokenAAssetValue, uint256 tokenBAssetValue) = assetInfo(_vault);
  ...
  //     ,
  //     ,
  //     uint256 tokenAAssetValue,
  //     uint256 tokenBAssetValue
  //   ) = assetInfo(_vault);
  ...
  //   return (
  //     tokenAAssetValue + tokenBAssetValue,
  ...
  function work(
    ...
    if (_borrowTokenAValue > 0) {
      ...
      ILendingPool(tokenALendingPool).borrow(tokenAAmountToBorrow, _vault);
    }

    if (_borrowTokenBValue > 0) {
      ILendingPool(tokenBLendingPool).borrow(_borrowTokenBValue, _vault);
    }

    // Probably withdrawal flow
    // If current position value is more than target position value, we reduce liquidity and repay debt
    if (currentAssetValue(_vault) > _targetAssetValue) {
      ...
      if (lpWithdrawAmount > 0) {
        ...
        vaults[_vault].lpTokenAmount -= lpWithdrawAmount;
      }
      ...
      ILendingPool(tokenALendingPool).repay(IERC20(tokenA).balanceOf(address(this)), _vault);

      IERC20(tokenB).approve(tokenBLendingPool, IERC20(tokenB).balanceOf(address(this)));
      ILendingPool(tokenBLendingPool).repay(IERC20(tokenB).balanceOf(address(this)), _vault);
      }
    }
    ...
    if (currentAssetValue(_vault) < _targetAssetValue) {
      ...
      if (_repayTokenAValue > 0) {
        ...
        ILendingPool(tokenALendingPool).repay(_tokenARepayAmount, _vault);
      }
      ...
      if (_repayTokenBValue > 0) {
        ...
        ILendingPool(tokenBLendingPool).repay(_repayTokenBValue, _vault);
      }
      ...
      vaults[_vault].lpTokenAmount += lpTokensMinted;
    ...
    IERC20(tokenA).safeTransfer(_vault, IERC20(tokenA).balanceOf(address(this)));
    IERC20(tokenB).safeTransfer(_vault, IERC20(tokenB).balanceOf(address(this)));
    }
  }
  ...
  function compound(address _vault) public {
    ...
    if (rewardTokenBalance > 0) {
      ...
      IERC20(rewardToken).approve(router, rewardTokenBalance);
      IJoeRouter02(router).swapExactTokensForTokens(
        rewardTokenBalance,
        0,
      ...
      (uint256 optimalSwapAmount, ) = optimalDeposit(
        0,
        IERC20(tokenB).balanceOf(address(this)),
        reserveA,
        reserveB,
