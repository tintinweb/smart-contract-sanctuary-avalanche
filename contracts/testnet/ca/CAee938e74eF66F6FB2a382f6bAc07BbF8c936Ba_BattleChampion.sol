    ...
    ReentrancyGuard
{
    ...
    uint256 public publicPrice = 2 ether;
    ...
    mapping(uint256 => Rarities) championRarities; // tokenId => Rarities

    constructor(
        string memory name_,
        string memory symbol_,
        uint256 maxSupply_
    ) ERC721Base(name_, symbol_, maxSupply_) {}
    ...
    function setBaseExtension(string calldata _baseExtension)
        ...
        onlyOwner
    {
        _setBaseExtension(_baseExtension);
    }
    ...
    function setTokenRarities(
        ...
        require(
            ...
            "tokenIds length doesn't match rarities length"
        );

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            championRarities[_tokenIds[i]] = _rarities[i];
        }
    }
    ...
    */
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < recipient.length; ++i) {
            _mint(recipient[i], quantity[i], 0);
        }
    }

    // Get Data
    function checkTokenRarity(uint256 tokenId)
        external
        view
        returns (Rarities)
    {
        return championRarities[tokenId];
    }
}
...
contract ERC721Base is ERC721A {
    ...
    mapping(uint256 => uint256) internal mintedPrices;
    ...
    function _setUnrevealURI(string calldata newURI) internal {
        unrevealURI = newURI;
    }

    function _setIsReveal(bool _isReveal) internal {
        isReveal = _isReveal;
    }

    function _setBaseExtension(string calldata _baseExtension) internal {
    ...
    function _mint(
        ...
        for (uint256 i = 1; i <= amount; i++) {
            uint256 tokenID = i + supply;
            mintedPrices[tokenID] = price;
        }
...
contract Signed is Ownable {
    ...
    function createHash(string memory _data, uint256 _nonce)
        internal
        view
        returns (bytes32)
    {
        return keccak256(abi.encode(address(this), msg.sender, _nonce, _data));
    }
    ...
    function verifySignature(
        ...
        address extracted = getSigner(createHash(data, nonce), signature);
        require(isAuthorizedSigner(extracted), "Signature verification failed");
    }
}
