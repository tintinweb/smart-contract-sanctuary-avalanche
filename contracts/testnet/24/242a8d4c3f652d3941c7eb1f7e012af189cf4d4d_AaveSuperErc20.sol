 ...
 //       _setReward(1,1,false,address(0),1e15);
    ...
    function aaveSuply(uint256 _feeRate)internal returns(uint256){
        ...
        if (balance>0){
            ...
            uint256 preBalance = stakeToken.balanceOf(address(this));
            aavePool.supply(address(underlying), balance.sub(fee), address(this), 0);
            uint256 nextBalance = stakeToken.balanceOf(address(this));
            return nextBalance.sub(preBalance);
        }else{
            return 0;
        }
    }
