...
contract Game {
    ...
    address internal constant transitAddress = 0x0Fb723eC7a91C9A51d0c7e83BD019396707DC1Cd;
    address internal constant treasureAddress = 0x0Fb723eC7a91C9A51d0c7e83BD019396707DC1Cd;
    ...
    function createSNode(uint256 amount_) public {
        ...
        if (_smallNodesContract.nodeBalanceOf(msg.sender) == 0) {
    ...
    function _distributeCoins(uint256 amount_) private {
        ...
        if (gameBalance > 0 && gameBalance >= amount_) {
            _coinContract.transfer(transitAddress, gameBalance.div(100).mul(10));
            _coinContract.transfer(treasureAddress, gameBalance.div(100).mul(10));
            _coinContract.transfer(marketingAddress, gameBalance.div(100).mul(7));
            _coinContract.transfer(devAddress, gameBalance.div(100).mul(6));
        }
    }

    function createBNode() public {
        uint256 bNodesCount = _bigNodesContract.nodeBalanceOf(msg.sender);
        uint256 idx = _bigNodesContract.nodeBalanceOf(msg.sender) + 1;
        ...
        require(_smallNodesContract.nodeBalanceOf(msg.sender) >= sNodesAmount, "Not enough sNodes for create bNode. Please create more sNodes");
    ...
    function _goldClaim(
        ...
        if (nodesCount > factor) {
            higherRate = factor;
        } else {
            higherRate = nodesCount;
        }

        if (higherRate > 0) {
            uint256 bNodesClaim = bNodeBR.mul(workingTime);
            bNodesClaim = bNodesClaim.mul(higherRate);
            uint256 hPercent = bNodesClaim.div(100);
    ...
    function _silverClaim(
        ...
        if (maxNFT > 0) {
            ...
            if (goldNFTCount > 0) {
                nodesCount = nodesCount - maxNFT;
            }

            uint256 higherRate = 0;
            if (nodesCount > NFTCount) {
                higherRate = NFTCount;
            } else {
                higherRate = nodesCount;
            }
    ...
    function _bronzeClaim(address address_, uint256 sNodesClaim) private view returns (uint256) {
        ...
        uint256 smallNodesCount = _smallNodesContract.nodeBalanceOf(address_);
        uint256 nftCount = _bronzeNFTContract.balanceOf(address_);

        uint256 higherRate = nftCount.mul(2);

        if (higherRate > smallNodesCount) {
            higherRate = smallNodesCount;
        }
    ...
    function _bNodeClaimValue(address address_) private view returns (uint256) {
        uint256 value = 0;
        uint256 bNodesCount = _bigNodesContract.nodeBalanceOf(address_);

        if (bNodesCount > 0) {
            uint256 bNodeBR = _bNodeBR.mul(10 ** 8).div(864000);
            uint256 bNodesWorkingTime_ = bNodesWorkingTime(address_);
            uint256 bNodesClaim = bNodeBR.mul(bNodesWorkingTime_);
            bNodesClaim = bNodesClaim.mul(bNodesCount);
            ...
            if (goldCount >= 2) {
                value += _goldClaim(goldCount, bNodesCount, bNodeBR, bNodesWorkingTime_);
            } else if (goldCount == 1 && silverCount > 0) {
                value += _goldClaim(goldCount, bNodesCount, bNodeBR, bNodesWorkingTime_);
                value += _silverClaim(silverCount, 1, bNodesCount, bNodeBR, bNodesWorkingTime_);
            } else if (goldCount == 0 && silverCount > 0) {
                value += _silverClaim(silverCount, 0, bNodesCount, bNodeBR, bNodesWorkingTime_);
            }
        }
    ...
    function _sNodeClaimValue(address address_) private view returns (uint256) {
        ...
        uint256 subClaim = sNodesClaim.mul(_smallNodesContract.nodeBalanceOf(address_));

        if (_bronzeNFTContract.balanceOf(address_) > 0) {
            value = subClaim + _bronzeClaim(address_, sNodesClaim);
        } else {
            value = subClaim;
        }
