...
contract DividendTracker is Ownable, IERC20 {
    address public constant dai = address(0xfcC62Dc696DFfF2aB736b3dc42E6Ec499dA56aa2);  // DAI.e address
    ...
    constructor(address _tokenAddress, address _uniswapRouter) {
        minTokenBalanceForDividends = 1 * (10**16);
...
contract Digits is Ownable, IERC20 {
    ...
    function setMaxTxBPS(uint256 bps) external onlyOwner {
        require(bps >= 49 && bps <= 500, "BPS must be between 49 and 500");
        maxTxBPS = bps;
    ...
    function setMaxWalletBPS(uint256 bps) external onlyOwner {
        require(
            bps >= 100 && bps <= 500,
            "BPS must be between 100 and 500"
        );
        maxWalletBPS = bps;
