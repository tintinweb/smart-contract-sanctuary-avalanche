...
contract VShareRewardPool  is Initializable, OwnableUpgradeable  {
    ...
    // Deposit LP tokens.
    function deposit(uint256 _pid, uint256 _amount) public isHuman {
    ...
    // Withdraw LP tokens.
    function withdraw(uint256 _pid, uint256 _amount) public isHuman {
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_sender];
    ...
    function emergencyWithdraw(uint256 _pid) public isHuman {
        PoolInfo storage pool = poolInfo[_pid];
