...
contract TeamHunters is ERC721EnumerableV2 {
  ...
  uint256 public constant PVP_FEE = 12_500_000_000; // 12.5k $orb
  ...
  mapping(uint256 => uint256) public lastRound; // team_id -> round_id
  ...
  function enterRoundInternal(
    ...
    orb.transferFrom(account, roundToken, PVP_FEE);

    require(rounds[lastRound[teamId]].roundEndedAt < timestamp);
  ...
  function enterRound(uint16 size, uint256 teamId) external {
    address account = msg.sender;
