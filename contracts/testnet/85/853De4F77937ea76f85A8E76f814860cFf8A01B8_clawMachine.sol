    ...
    ClampedRandomizer
{
    ...
    uint256 private _maxRandomizer = 10000;
    ...
    constructor()
        ERC721("Claw Machine Prizes", "CMP")
    ...
    function maxMintable(uint256 maxMintable_) public onlyOwner {
    ...
    function withdraw(uint256 amountInMilli) public onlyOwner {
    ...
    function mint(uint256 quantity) public payable {
        ...
        payable(msg.sender).transfer(msg.value - _price*quantity); 
    ...
    function internalMint(address to, uint256 amount) internal {
        for (uint256 i = 1; i <= amount; i++) {
            ...
            bool won;
            if (tokenID <= _maxMintable) {
                tokenIds[tokenIds.length] = tokenID;
                _safeMint(to, tokenID);
                won = true;
            }
            if (won) {
                emit win(msg.sender, tokenIds.length);
            } else {
