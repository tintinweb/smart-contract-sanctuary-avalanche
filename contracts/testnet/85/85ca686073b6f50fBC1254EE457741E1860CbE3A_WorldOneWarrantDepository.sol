...
contract WorldOneWarrantDepository is Ownable {
    ...
    IPangolinFactory public dexFactory; // Factory address to get market price

    mapping(address => mapping(uint256 => Warrant)) public warrantInfo; // stores warrant information for depositors
    mapping(address => uint256[]) private _depositorWarrantIds; // stores warrant ids for depositors

    uint256 public warrantLotIndex = 0;
    uint8 constant public conversionRate = 1;

    uint32 constant MAX_PAYOUT_IN_PERCENTAGE = 100000; // in thousandths of a %. i.e. 500 = 0.5%
    uint32 constant MIN_VESTING_TERM = 300;//129600; // in seconds. i.e. 1 day = 86400 seconds
    ...
    function initializeWarrantLot(
        ...
        emit WarrantPriceChanged(warrantPrice());
    }
    ...
    function deposit(
        ...
        uint256 _amount = _warrantAmount.mul(nativePrice).div(10**WorldOne.decimals());
       
        require(_warrantAmount >= 10_000_000, "Warrant too small"); // must be > 0.01 WorldOne ( underflow protection )
        require(
            currentWarrantLot().consumed.add(_warrantAmount) <=
                currentWarrantLot().totalCapacity,
            "Exceeding maximum allowed purchase in current warrant lot"
        );

        uint256 fee = _warrantAmount.mul(currentWarrantLot().fee) / 100_00;

        principle.safeTransferFrom(msg.sender, address(this), _amount);
        principle.approve(address(treasury), 10**WorldOne.decimals());

        treasury.deposit(_warrantAmount.mul(10**WorldOne.decimals()), address(principle), fee);
        ...
        uint warrantId_ = 1;
        uint lastIndex = _depositorWarrantIds[_depositor].length - 1;
        _depositorWarrantIds[_depositor].length == 0 ? warrantId_ : warrantId_ += _depositorWarrantIds[_depositor][lastIndex];
        warrantInfo[_depositor][warrantId_] = Warrant({
            payout: warrantInfo[_depositor][warrantId_].payout.add(_warrantAmount),
            warrantLotID: uint32(warrantLotIndex - 1),
            purchasedAt: uint32(block.timestamp),
            pricePaid: nativePrice
        });
        _depositorWarrantIds[_depositor].push(warrantId_);
        ...
        warrantLots[warrantLotIndex - 1] = WarrantLot({
            ...
            consumed: currentWarrantLot().consumed.add(_warrantAmount),
    ...
    /**
     *  @notice update warrant lot
     *  @param _warrantLotIndex uint
     *  @param _discount uint
     *  @param _price uint
     *  @param _totalCapacity uint
     *  @param _maxPayout uint32
     *  @param _status bool
     */
    ...
    function updateWarrantLot(
        ...
        if (warrantLotIndex > 0) {
            require(
            _warrantLotIndex == warrantLotIndex,
            "Only update current warrant lot"
            );
            require(
                currentWarrantLot().consumed !=
                    currentWarrantLot().totalCapacity && _totalCapacity.mul(10**WorldOne.decimals()) > currentWarrantLot().consumed,
                "Warrant lot already in progress"
            );
        }
     ...
     *  @param warrantId_ uint
     *  @return uint
     */
    function redeem(address _recipient, uint256 warrantId_) external returns (uint256) {
        require(msg.sender == _recipient, "NA");
        Warrant memory info = warrantInfo[_recipient][warrantId_];
        require(
            canAvailableClaim(_recipient,warrantId_),
            "Cannot redeem"
        );
        delete warrantInfo[_recipient][warrantId_]; // delete user info
        for(uint i = 0; i < _depositorWarrantIds[_recipient].length; i++) { // delete user warrant id
            if(_depositorWarrantIds[_recipient][i] == warrantId_) {
                while ( i < _depositorWarrantIds[_recipient].length - 1) {
                    _depositorWarrantIds[_recipient][i] = _depositorWarrantIds[_recipient][i+1];
                    i++;
                }
                _depositorWarrantIds[_recipient].pop();
            }
        }
        emit WarrantRedeemed(_recipient, info.payout, 0, warrantId_); // emit warrant data
     ...
     */
    function setDexFactory(address _factory)
        external onlyOwner
    {
        dexFactory = IPangolinFactory(_factory);
    }
    ...
    function payoutFor(uint256 _value) public view returns (uint256) {
        return
            FixedPoint.fraction(_value.mul(conversionRate), warrantPrice()).decode112with18() /
            1e16;
    }
    ...
    function getLatestPrice() public view returns (uint256 price_) {
        ...
        uint256 treasuryBalance = treasury.getTotalReserves().mul(1e9); 
    ...
    /**
     *  @notice check currently user can claim or not
     *  @return bool
     */
    function canAvailableClaim(address _depositor, uint256 warrantId_) public view returns (bool) {
        Warrant memory info = warrantInfo[_depositor][warrantId_];
        if(info.purchasedAt == 0){
            return false;
        }
        return block.timestamp >= info.purchasedAt.add32(warrantLots[info.warrantLotID].vestingTerm);
    }

    /**
     *  @notice get users warrant ids
     *  @return _depositorWarrantIds uint[]
     */
    function getWarrantIds(address _depositor) public view returns (uint256[] memory) {
        return _depositorWarrantIds[_depositor];
    }

     /**
     *  @notice get what will be next warrant id for deposit
     *  @return uint
     */
    function getLatestDepositingWarrantId(address _depositor) public view returns (uint256) {
        uint256 idsLength = _depositorWarrantIds[_depositor].length;
        if (idsLength == 0) {
            return 1;
        }
        return _depositorWarrantIds[_depositor][idsLength - 1] + 1;
    }
