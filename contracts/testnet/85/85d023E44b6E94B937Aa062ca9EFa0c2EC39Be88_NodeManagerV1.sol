...
contract NodeManagerV1 is Initializable {
  ...
  function setFunctionAvailable(string memory functionName, bool value) public onlyOwner() {
      require(keccak256(abi.encodePacked(functionName)) != keccak256(abi.encodePacked("setFunctionAvailable")), "Cant disabled");
  ...
  function _transferFee(uint256 amount) private {
    ...
    IERC20(tokenAddress).transferFrom(address(msg.sender), address(rewardsPoolAddress), amount.mul(rewardsPoolFee).div(10000));
    IERC20(tokenAddress).transferFrom(address(msg.sender), address(operationsPoolAddress), amount.mul(operationsPoolFee).div(10000));
  }
  ...
  function transfer(
    ...
    if (claimableAmount > 0) {
      rewardsOfUser[from] = rewardsOfUser[from].add(claimableAmount);
      rewardsTotal = rewardsTotal.add(claimableAmount);
      IERC20(tokenAddress).transferFrom(address(rewardsPoolAddress), address(from), claimableAmount);
    }
  ...
  function sellNode(uint32 indice) public {
    ...
    if (node.owner != address(0)) {
      ...
      countOfTier[tier.name]--;
      countTotal--;
      IERC20(tokenAddress).transferFrom(address(rewardsPoolAddress), address(node.owner), tier.price.div(100).mul(sellPricePercent));
    }
  }
  ...
  // function payWithNFT(uint8 count, uint256[] memory selected) public {
  //   require(!availableFunctions["pay"], "Disabled");
  //   require(count <= selected.length, "Invalid number of nodes");
  //   for (uint32 i = 0; i < selected.length; i++) {
  //     uint256 nodeIndex = selected[i];
  //     Node storage node = nodesTotal[nodeIndex];
  //     if (node.owner == msg.sender) {
  //       node.limitedTime += 12 * 30 days;
  //     }
  //   }
  // }
  ...
  function migrateFromOldVersion() public {
    require(!availableFunctions["migrateFromOldVersion"], "Disabled");
