...
contract TokenPresale {
    ...
    constructor(address tokenAddress, address bonusTokenAddress, address usdtAddress) {
        ...
        _avaxPriceFeed = AggregatorV3Interface(0x0A77230d17318075983913bC2145DB16C7366156);
    }
    ...
    function buyWithUSDT(uint256 amount) public notLocked {
        ...
        require(usdt.balanceOf(msg.sender) >= totalPrice, "Insufficient USDC balance");
        require(token.balanceOf(address(this)) >= amount, "Insufficient tokens in contract");
        
        require(usdt.transferFrom(msg.sender, address(this), totalPrice), "USDC transfer failed");
        require(token.transfer(msg.sender, amount), "Token transfer failed");
        // require(bonusToken.transfer(msg.sender, bonusAmount), "Bonus Token transfer failed");
    ...
    function buyWithAvax(uint256 amount) external payable notLocked {
        ...
        uint256 totalPrice = amount * _tokenPrice;
        uint256 bonusAmount = amount / 100 * _bonusPercent;
        uint256 currentAvaxPrice = getLatestAvaxUsdPrice();
        uint256 requiredAvax = totalPrice / currentAvaxPrice;
        
        require(msg.value >= requiredAvax, "Insufficient AVAX balance");
        require(token.balanceOf(address(this)) >= amount, "Insufficient tokens in contract");
        // require(bonusToken.balanceOf(address(this)) >= bonusAmount, "Insufficient bonus tokens in contract");
