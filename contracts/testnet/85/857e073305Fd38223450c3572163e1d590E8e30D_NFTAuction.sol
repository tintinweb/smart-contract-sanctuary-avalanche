...
contract NFTAuction {
        ...
        returns (address)
    {
        address nftHighestBidder = nftContractAuctions[_nftContractAddress][_tokenId]
            .nftHighestBidder;
        require(nftHighestBidder != address(0), "NFT not deposited");

        return nftHighestBidder;
    }
    function getHighestBid(address _nftContractAddress , uint256 _tokenId) external view returns (uint128) {
        uint128 nftHighestBid = nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid;

        return nftHighestBid;
    }
