...
abstract contract TriplayVault is EIP712, Authorization, ReentrancyGuard, Pausable {
    ...
    TriplayPolicy public immutable policy;
    ...
    function addTreasury(address _treasury) external onlyOwner {
        require(_treasury != address(0), "zero address"); 
        isTreasury[_treasury] = true;
        emit AddTreasury(msg.sender, _treasury);
    }

    function removeTreasury(address _treasury) external onlyOwner {
...
contract TriplayERC20Vault is TriplayVault {
    ...
    constructor(
        ...
        address _treasury,
        TriplayPolicy _policy
    ) TriplayVault(_treasury, _policy) {
        token = _token;
    }
    ...
    function withdraw(
        ...
        policy.verifyWithdrawERC20(player, amount, signatures, paramsHash);
    ...
    function cancelWithdrawal(
        ...
        policy.verifyCancelWithdrawal(msg.sender, signatures, paramsHash);
        requestsStatus[requestId] = RequestStatus.Canceled;
        emit CancelWithdrawal(msg.sender, requestId, amount);
    }

    function transferToTreasury(address treasury, uint256 amount) external whenNotFullShutDown auth {
        require(amount > 0, "amount = 0");
        require(isTreasury[treasury], "not a treasury");
