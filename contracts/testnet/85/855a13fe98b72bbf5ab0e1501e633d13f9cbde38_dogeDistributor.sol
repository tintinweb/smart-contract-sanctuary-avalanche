...
// File: DogeMoon/AVAX-new/AVAX-DogePurchase.sol
...
contract dogeDistributor is Ownable, VRFConsumerBaseV2 {
    ...
    */
    ...
    mapping (uint256 => address) public eggHatcher;
    // requestId => isFulfilled address
    mapping (uint256 => bool) public requestFulfilled;
    
    mapping (address => bool) public allowedExternalMinters;
    
    constructor(address _dogeAddress, address payable _devWallet, uint256 _dogePrice) VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        LINKTOKEN = LinkTokenInterface(linkAddress);

        dogeAddress = _dogeAddress;
        devWallet = _devWallet;
        dogePrice = _dogePrice;

        createNewSubscription();
    }
    
    function buyDoges(uint32 amount) payable public {
        require(active == true, "The incubator is currently disabled");
        require(amount > 0, "Why would you want zero Doges?");
        require(amount <= availableDoges.length, "You can't purchase more Doges than available.");
        require(amount <= 10, "You can only mint 10 doges at a time");
        require(msg.value == dogePrice * amount, "You need to send the exact price");
    ...
    function adminBuyDoges(address _receiver, uint32 amount) public onlyOwner {
        require(amount <= availableDoges.length, "You can't purchase more Doges than available.");
        require(amount > 0, "Why would you want zero Doges?");
    ...
    // Allows external whitelisted contract to mint doges for free (mint cost / egg nft should be taken from the user in the external contract)
    function externalDogeMint(address _receiver, uint32 amount) external {
        require(allowedExternalMinters[msg.sender] == true, "You are not an allowed minter");
        require(amount <= availableDoges.length, "You can't purchase more Doges than available.");
        require(amount > 0, "Why would you want zero Doges?");
    ...
    function finishHatchingEgg(uint256 requestId, uint256 randomNumber) private {
        ...
        MoonDoge(dogeAddress).mint(hatcher, pickedDoge);
    ...
    function setAllowedMinter(
        ...
        allowedExternalMinters[_minter] = _newState;
    }
    ...
    function adminSetDogePrice(uint256 newDogePrice) external onlyOwner {
    ...
    function fulfillRandomWords(
        ...
        for(uint256 i=0; i<randomWords.length; i++) {
            finishHatchingEgg(requestId, randomWords[i]);
        }
    }
