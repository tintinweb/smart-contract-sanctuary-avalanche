...
interface ITraitNft is IERC1155,IERC4907 {
	...
	function batchTraitMint(address, uint256[] memory, Trait[] calldata) external returns(bool);
...
contract MainNft is ERC4907, Ownable {
    ...
    uint256 private PRICE_PER_TRAIT = 0.001 ether;
	...
	*/
    function mintNFT(uint256 avatarId,string memory avatarUri,bytes calldata signature,Trait[] calldata traits) external payable {
        ...
        ITraitNft(IMarketplace(marketplaceAddress).traitContract()).batchTraitMint(msg.sender,traitIds,traits);
    }
