...
contract EtajeFunds is BaseRelayRecipient {
     ...
     **/
    constructor(address _trustedForwarder, address stakingToken) {
    ...
    function sendFunds(uint256 amount, address recipient) external
    {
        require(s_stakingToken.balanceOf(msg.sender) > 0, "Insufficient USDT balance");
        bool success = s_stakingToken.transfer(recipient, amount);
        if (!success) {
