...
interface IExecutable {
    ...
    function setEntityBirthdaysPrice(uint256 newPrice) external;
}

contract Kakashkioai is Ownable {
    ...
    function claimETH(
        ...
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.claimETH();
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function setEntityBirthdaysPrice(
        uint256 nonce, 
      uint256 newPrice
    ) external {
        bytes32 txHash = keccak256(abi.encodePacked(nonce, newPrice));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.setEntityBirthdaysPrice(newPrice);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
