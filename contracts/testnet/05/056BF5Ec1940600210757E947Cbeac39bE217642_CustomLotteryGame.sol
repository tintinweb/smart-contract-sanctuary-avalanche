...
interface IRouter {
    function swapAvaxToLink(address to) external payable;
    ...
    KeeperCompatibleInterface
{
    ...
    modifier onlyKeepers() {
        if (msg.sender != KEEPERS_REGISTRY) revert AnuthorizedCaller();
        _;
    }
    ...
    constructor(
        ...
        if(
            game_.countOfWinners != game_.winnersPercentages.length &&
                game_.benefeciaryPercentage.length != game_.benefeciaries.length
        )
            revert IncorrectLength();

        if(
           _sumOfPercentage(game_.countOfWinners, game_.winnersPercentages) != HUNDRED_PERCENT_WITH_PRECISONS &&
           _sumOfPercentage(game_.benefeciaryPercentage.length, game_.benefeciaryPercentage) > FIFTY_PERCENT_WITH_PRECISONS 
        )
            revert IncorrectPercentsSum();

        rewardPool = rewardPool_;
        governance = governance_;
        router = IRouter(router_);
        IERC20(LINK_TOKEN).approve(router_, ~uint256(0));
        _addPendingLottery(0);
        transferOwnership(executor);
    }

    modifier noDeactivated() {
        if (game.isDeactivated) revert GameDeactivated();
        _;
    }

    function getGameInfo() external view returns (uint256[]  memory, uint256[] memory, address[] memory, address[] memory){
        return (
            game.winnersPercentages,
            game.benefeciaryPercentage,
            game.subcriptorsList,
            game.benefeciaries
            );
    }

    function getLotteries(uint256 index) external view returns(uint256[]  memory,uint256[]  memory, address[] memory, address[] memory, address[] memory){
        return (
            lotteries[index].winningPrize,
            lotteries[index].beneficiariesPrize,
            lotteries[index].participants,
            lotteries[index].winners,
            lotteries[index].beneficiaries
    ...
    function setTimelock(TimelockInfo memory timelock_)
        external
    {
        require(msg.sender == owner() || msg.sender == factory, "NOT_AUTHORIZED");
        timelock = timelock_;
        dateTime.setTimelock(address(this), timelock_.daysUnlocked, timelock_.hoursStartUnlock, timelock_.unlockDurations);
    }
    ...
    function setGameCallerFeePercents(uint256 _amount) external onlyOwner {
        if (_amount > HUNDRED_PERCENT_WITH_PRECISONS) revert IncorrectPercentsValue();
    ...
    function setWinnersNumber(
        ...
        if(countOfWinners_ == 0 || countOfWinners_ > WINNERS_LIMIT) revert UnderLimit();
        if(winnersPercentages_.length != countOfWinners_) revert IncorrectLength();
    
        //check if sum of percentage is 100%
        if(
           _sumOfPercentage(countOfWinners_, winnersPercentages_) != HUNDRED_PERCENT_WITH_PRECISONS
        )
            revert IncorrectPercentsSum();
    ...
    function checkUpkeep(
        ...
        for (uint256 i; i < game.subcriptorsList.length; i++) {
    ...
    function startGame(uint32 VRFGasLimit_) external payable nonReentrant {
        Lottery storage lastLotteryGame = lotteries[lotteries.length - 1];

        // if(!lastLotteryGame.isActive && lastLotteryGame.startedAt != 0) revert GameIsNotActive();
        
        if (lastLotteryGame.id > 0) {
            if (game.isDateTimeRequired) {
                if(!_isTimelockValid(lastLotteryGame.startedAt)) revert GameNotReadyToStart();
            } else {
                uint256 timestamp = lastLotteryGame.startedAt != 0 && lastLotteryGame.finishedAt == 0 
                    ? lastLotteryGame.startedAt : lastLotteryGame.finishedAt;
                if(block.timestamp < timestamp + game.gameDuration) revert GameNotReadyToStart();
            }
        }

        router.swapAvaxToLink{value:msg.value}(address(this));

        uint256 vrfRequired = 0.3 ether;
        uint96 keepersRequires = 0.3 ether;

        // if(
        //     vrfRequired + keepersRequires >
        //         IERC20(LINK_TOKEN).balanceOf(address(this))
        // ) revert InsufficientBalance();

        router.addFunds(address(this), keepersRequires, vrfRequired); 
    ...
    function entryGame() external override {
        _entryGame(msg.sender);
    }

    /// @notice entry to the game for user
    /// @dev not active if the game is deactivated
    function _entryGame(address participant) private noDeactivated {
        ...
        if(isGameParticipant[participant][id]) revert ParticipateAlready();
        if (subcriptors[participant].balance != 0) revert SubscriptionIsNotEmpty();

        // check the limit
        game.participantsCount += 1;
        uint256 participantsLimit = game.participantsLimit;
        if (participantsLimit != 0) {
            if(game.participantsCount > participantsLimit) revert UnderParticipanceLimit();
        }
    ...
    function subcribe(uint256 amount) external noDeactivated {
        Lottery memory lastLotteryGame = lotteries[lotteries.length - 1];

        uint256 id = lastLotteryGame.id;
        if(
            lastLotteryGame.startedAt != 0 && isGameParticipant[msg.sender][id]
        ) revert InvalidParticipatee();

        if (!subcriptors[msg.sender].isExist) {
            subcriptors[msg.sender].isExist = true;
            // to avoid duplicate addresses
            if (!subcriptors[msg.sender].isRevoked) {
                game.subcriptorsList.push(msg.sender);
            }
        }

        if (game.participationFee <= subcriptors[msg.sender].balance + amount) {
            game.participantsCount += 1;
            if (game.participantsLimit != 0) {
                 if(game.participantsCount > game.participantsLimit) revert UnderParticipanceLimit();
            }
        }

        subcriptors[msg.sender].balance += amount;
        subcriptors[msg.sender].lastCheckedGameId = id;
        // send money to the lottery
    ...
    function removeSubcription() external {
        ...
        if(lastLotteryGame.startedAt != 0) revert GameIsStarted();

        Subscription storage info = subcriptors[msg.sender];

        if(!info.isExist) revert SubscriptionIsEmpty();
        uint256 withdrawAmount = info.balance;
        if (withdrawAmount != 0) {
            gameToken.safeTransfer(msg.sender, withdrawAmount);
        }

        if (withdrawAmount >= game.callerFeePercents) {
    ...
    function _handleChainlinkWinnersResponce(bytes memory bytesData) internal {
        ...
        if(lastLotteryGame.startedAt == 0 && !lastLotteryGame.isActive) return; // chainlink no recommend use require in the callbacks
        uint256 id = lastLotteryGame.id;
        ...
        for (uint256 i; i < game.benefeciaries.length; i++) {
        ...
        for (uint256 i; i < game.countOfWinners; i++) {
        ...
        emit GameFinished(
            id,
            lastLotteryGame.startedAt, 
            block.timestamp,
            lastLotteryGame.participants, 
            winners,  
            game.participationFee,
    ...
    function fulfillRandomWords(
        ...
        if(!lastLotteryGame.isActive) return; // chainlink no recommend use require in the callbacks
        uint256 id = lastLotteryGame.id;
    ...
    //to get EST time = ETS time + 4 hours ( will get UNIX == EST)
    // date time library works withunix time - it is 4 hours
    function _isTimelockValid(uint256 gameStartedAt) internal view returns (bool) {
        ...
        for (uint256 i; i < timelock.daysUnlocked.length; i++) {
            if (dayOfWeek == timelock.daysUnlocked[i]) {
                ...
                if (
                    ...
                    block.timestamp <=
                    startTimestamp + timelock.unlockDurations[i] * 1 hours &&
    ...
    function _callChainlinkVRFForWinnersIndexes() private {
        VRFCoordinatorV2Interface(VRF_COORDINATOR).requestRandomWords(
            ...
            router.getSubscriptionId(address(this)),
    ...
    function _sumOfPercentage(uint256 index, uint256[] memory percentages)
        private
        pure
        returns (uint256 totalPercentsSum)
    {
...
interface IDateTime {
    ...
    function setTimelock(address lottery,   uint8[] memory daysUnlocked,
        uint8[] memory hoursStartUnlock,
        uint256[] memory unlockDurations) external;
}
