  ...
  VotingMachineWithProofs
{
  ...
  function receiveCrossChainMessage(
    ...
    if (_votingStrategy.hasAvailableRoots(blockHash)) {
...
abstract contract VotingMachineWithProofs is IVotingMachineWithProofs, Ownable {
  ...
  function createVote(uint256 proposalId) public returns (uint256) {
    ...
    require(
      _votingStrategy.hasAvailableRoots(voteConfig.l1ProposalBlockHash),
...
interface IL2VotingStrategy {
  ...
  function hasAvailableRoots(bytes32 blockHash) external view returns (bool);
}
