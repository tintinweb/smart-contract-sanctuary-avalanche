...
contract RewardEscrowV2Manager is Owned, MixinSystemSettings {
    ...
    bytes32 private constant CONTRACT_REWARDESCROWV2_AGGREGATOR = "RewardEscrowV2Aggregator";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[2] = CONTRACT_REWARDESCROWV2_AGGREGATOR;
    ...
    function rewardEscrowV2Aggregator() internal view returns (IRewardEscrowV2Aggregator) {
        return IRewardEscrowV2Aggregator(requireAndGetAddress(CONTRACT_REWARDESCROWV2_AGGREGATOR));
    }
    ...
    function totalBalanceOf(address account) external view returns (uint256 _totalBalanceOf) {
        ...
        for (uint256 ii = 0; ii < availableCollaterals.length; ii++) {
            uint256 balance = rewardEscrowV2Aggregator().balanceOf(availableCollaterals[ii], account);
    ...
    function totalBalanceOfByKey(bytes32 currencyKey, address account) external view returns (uint256 _totalBalanceOf) {
        uint256 balance = rewardEscrowV2Aggregator().balanceOf(currencyKey, account);
        (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
    ...
    function totalEscrowedBalance() external view returns (uint256 _totalEscrowedBalance) {
        bytes32[] memory availableCollaterals = synthetix().getAvailableCollaterals();
        for (uint256 ii = 0; ii < availableCollaterals.length; ii++) {
            uint256 balance = rewardEscrowV2Aggregator().totalEscrowedBalance(availableCollaterals[ii]);
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableCollaterals[ii]);
    ...
    function totalEscrowedBalanceByKey(bytes32 currencyKey) external view returns (uint256 _totalEscrowedBalance) {
        uint256 balance = rewardEscrowV2Aggregator().totalEscrowedBalance(currencyKey);
        (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
    ...
    function addRewardEscrows(bytes32 collateralKey, IRewardEscrowV2 rewardEscrow) external onlyOwner returns (bool) {
        if (rewardEscrows[collateralKey] != IRewardEscrowV2(0)) {
    ...
    function removeRewardEscrows(bytes32 collateralKey) public onlyOwner returns (bool) {
        ...
        rewardEscrows[collateralKey] = IRewardEscrowV2(0);
    ...
    function sendAppend(
        ...
        address account,
        uint256 amount
    ) external onlyRewardEscrow(currencyKey) {
        rewardEscrowV2Aggregator().sendAppend(currencyKey, account, amount);

        emit AppendMsgToAggregator(currencyKey, account, amount);
    }
    ...
    function sendVest(
        ...
        rewardEscrowV2Aggregator().sendVest(currencyKey, account, amount);

        emit VestMsgToAggregator(currencyKey, account, amount);
    }

    /* ========== MODIFIER FUNCTIONS ========== */
    modifier onlyRewardEscrow(bytes32 collateralKey) {
        require(msg.sender == address(rewardEscrows[collateralKey]), "Only rewardEscrowV2 can call this function");
        _;
    }

    /* ========== EVENTS ========== */
    event AppendMsgToAggregator(bytes32 collateralKey, address indexed account, uint256 amount);
    event VestMsgToAggregator(bytes32 collateralKey, address indexed account, uint256 amount);
