...
interface IONFT721 is IERC721 {
     ...
     */
    function send(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress, bytes calldata _adapterParams) external payable;
...
contract ONFT721 is IONFT721, NonblockingLzApp, ERC721 {
    ...
    function send(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress, bytes calldata _adapterParams) public payable virtual override {
        _send(_msgSender(), _dstChainId, _toAddress, _tokenId, _refundAddress, address(0x0), _adapterParams);
    }
