...
contract RoyaltyChecker is IRoyaltyChecker, AccessControlEnumerable {
    ...
    event UnpaidFeeSetted(uint256[] sbtIds, address[] collections, uint256[] ids, TxData[] txs);
    ...
    function setUnpaidFee(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (_txsHashes[sbtIds_[i]].contains(txs_[i].hash)) continue;
            if (_sbtId[txs_[i].seller][collections_[i]][ids_[i]] > 0) {
                _collectionDetails[sbtIds_[i]] = Collection(txs_[i].token, ids_[i]);
            }
            _txsHashes[sbtIds_[i]].add(txs_[i].hash);
            _txsData[sbtIds_[i]].push(txs_[i]);
            _unpaidTotalFee[sbtIds_[i]][txs_[i].token] += txs_[i].fee;
            _paymentTokens[sbtIds_[i]].add(txs_[i].token);
        }
        emit UnpaidFeeSetted(sbtIds_, collections_, ids_, txs_);
        return true;
    }

    function closeUnpaidPayments(uint256[] memory sbtIds) external onlyRole(OPERATOR_ROLE) returns (bool) {
        require(sbtIds.length > 0, "SBT ids is not positive");
        for (uint256 i = 0; i < sbtIds.length; i++) {
            if (_txsData[sbtIds[i]].length == 0) continue;
            delete _txsHashes[sbtIds[i]];
            delete _txsData[sbtIds[i]];
            delete _collectionDetails[sbtIds[i]];
            delete _txsHashes[sbtIds[i]];
            for (uint256 j = 0; j < _paymentTokens[sbtIds[i]].length(); j++) {
                delete _unpaidTotalFee[sbtIds[i]][_paymentTokens[sbtIds[i]].at(j)];
            }
            delete _paymentTokens[sbtIds[i]];
        }
        emit UnpaidFeeClosed(sbtIds);
        return true;
    }
}
