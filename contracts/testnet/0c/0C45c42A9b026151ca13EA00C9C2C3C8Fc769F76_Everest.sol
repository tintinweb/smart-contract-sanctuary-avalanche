...
// OpenZeppelin Contracts (last updated v4.5.0) (token/ERC20/IERC20.sol)d
...
pragma solidity ^0.8.0;
contract Everest is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the EVEREST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("EVEREST", "EVEREST") {
        // Mints 1 EVEREST to contract creator for initial pool setup
    ...
    function _getEverestPrice() internal view returns (uint256 _everestPrice) {
        try IOracle(everestOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Everest: failed to fetch EVEREST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _everestPrice) internal returns (uint256 newTaxRate) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_everestPrice >= taxTiersTwaps[tierId]) {
    ...
    function setEverestOracle(address _everestOracle) public onlyOperatorOrTaxOffice {
        require(_everestOracle != address(0), "oracle address cannot be 0 address");
        everestOracle = _everestOracle;
    }
    ...
    /**
     * @notice Operator mints EVEREST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EVEREST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEverestPrice = _getEverestPrice();
            currentTaxRate = _updateTaxRate(currentEverestPrice);
            if (currentEverestPrice < burnThreshold) {
                burnTax = true;
            }
        }
