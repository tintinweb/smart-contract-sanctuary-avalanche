...
// constructor
    ...
    function createAuction(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external returns (bool){
        ...
        tokensListed[_nftAddress][collectionSize[_nftAddress]] = _tokenId;
   ...
   // get all for sale for collection
    ...
    function collectionGetAllForSale(address _NFT) external view returns(address[] memory _info) {
        ...
        for (uint i = 0; i < collectionSize[_NFT]; i++) { // for each auction
            ...
            _info[i] = NFTcollections[_NFT][tokensListed[_NFT][i]]; // add the address of the auction to the array  
