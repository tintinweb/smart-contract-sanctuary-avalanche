...
interface IUniversalExchange {
     ...
     *  E43: Order Id is already Exist order ID should be unique
     */
    ...
    ReentrancyGuard
{
   ...
   // Add for seperation of US and Non US Fee Status
    ...
    function _onlyAdmin() private view {
        require(whitelist.isWhitelistedUser(msg.sender) < 132, "E1");
    }
    ...
    function sellTokens(
        ...
        require(!isOrderExist[orderId],"E43");
        ...
        listedTokens[orderId] = ListTokens({
            seller: msg.sender,
            token: token,
            price: price,
            quantity: quantity,
...
//*********************************************************************************************
        if(whitelist.isUserUSA(msg.sender) && feeStatusUS ){
            feeAmount = (totalAmount * feePercentageUS) / 100 ether;
            totalAmount = totalAmount +feeAmount;
        }else if(whitelist.isUserForeign(msg.sender) && feeStatusNonUS){
    ...
    function nullOffer(uint256 id) private {
        ...
        for (uint256 i = 0; i < len; ) {
            ...
            if( counteroffers.quantity  > listing.remainingQty + listing.tokenInHold && (counteroffers.status == Status.Created||
                counteroffers.status == Status.Countered))
             {
    ...
    /**
     * @dev  single token status update
