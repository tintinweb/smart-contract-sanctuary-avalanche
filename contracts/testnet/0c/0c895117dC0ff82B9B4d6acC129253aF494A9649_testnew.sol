...
contract testnew is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	constructor(string memory _initUnrevealedURI, address _initArtistAddress, address _initaiswalletAddress) ERC721("test new", "TTN") {
		paused = true;
		revealed = false;
		allowListings = true;
	...
	function mint(uint256 amount) public payable nonReentrant {
		...
		(bool success1, ) = payable(aiswalletAddress).call{ value: proceeds_10percent * 7 }("");
	...
	function withdraw() public onlyOwner {
		...
		(bool success1, ) = payable(aiswalletAddress).call{ value: balance_10percent * 7 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to aiswallet wallet!");
	...
	function royaltyInfo(uint256, uint256 value) external view returns(address receiver, uint256 royaltyAmount) {
		...
		royaltyAmount = value * (royalties + artistRewards) / PERCENTAGE_MULTIPLIER;
	}
	...
	function fulfillListing(uint256 tokenID) public payable nonReentrant {
		...
		(bool success1, ) = payable(listing.seller).call{ value: msg.value - (royaltiesValue + artistRewardValue) }("");
		require(success1, "AVAX Transaction: Failed to transfer funds");

		(bool success2, ) = payable(artistAddress).call{ value: artistRewardValue }("");
		require(success2, "AVAX Transaction: Failed to transfer funds to artist address");

		(bool success3, ) = payable(owner()).call{ value: royaltiesValue }("");
		require(success3, "AVAX Transaction: Failed to transfer funds to owner address");
	}
    ...
    function setWithdrawAddresses(address newaiswalletAddress, address newArtistAddress) public onlyOwner {
	...
	function withdrawWavax() public onlyOwner { 
        ...
        wavaxContract.transfer(aiswalletAddress, balance_fraction * 333);
	...
	receive() external payable {
		...
		(bool success1, ) = payable(aiswalletAddress).call{ value: fraction * 333 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to aiswallet wallet!");
