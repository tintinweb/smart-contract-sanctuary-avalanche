...
contract OOPARaffler is Ownable, VRFConsumer, ERC1155Holder {
    ...
    enum RaffleType{ERC721 , ERC1155, WHITELIST}
    ...
    function createRaffle (Raffle memory _raffle, bool _transferRewards) external onlyOwner returns (uint) {
        if ((_raffle.raffledGoodsAmount == 0) || (_raffle.pricePerTicket == 0 ) || (_raffle.endDate < _raffle.startDate + 3 hours ) || (_raffle.maxTicketAvailable == 0 ) || (_raffle.ticketsSold != 0 )){
            revert InvalidRaffle();
        }

        if (_raffle.raffleType != RaffleType.ERC1155 && _raffle.raffleType != RaffleType.ERC721 && _raffle.raffleType != RaffleType.WHITELIST ) {
            revert InvalidRaffle();
        }
    ...
    function handleTransferRewards(Raffle memory _raffle) internal {
        if(_raffle.raffleType == RaffleType.ERC721) {
            ...
            if(_raffle.raffledCollectionERC1155 == address(OOPA_TRAITS)) {
                ...
                for(uint i; i < _raffle.raffledGoodsAmount;) {
                    OOPA_TRAITS.mintTrait(_raffle.raffledGoodERC1155,address(this));
                    unchecked {
                        ++i;
                    }
                }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        if (raffle.raffleType == RaffleType.ERC1155) {
            RaffleWinners[raffleID] = RaffleWinnersData(winners,raffle.raffledCollectionERC1155 , 0, raffle.raffledGoodERC1155, 0);
        }
        else if (raffle.raffleType == RaffleType.ERC721) {
            RaffleWinners[raffleID] = RaffleWinnersData(winners, raffle.raffledCollectionERC721, raffle.tokenIdERC721, 0, 0);
        }
    ...
    function claimReward(uint _raffleId) external {
        ...
        if (winnersData.raffledCollectionAddress == address(0)) { // ERC1155
