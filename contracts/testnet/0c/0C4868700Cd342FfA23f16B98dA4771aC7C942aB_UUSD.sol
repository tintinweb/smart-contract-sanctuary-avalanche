...
contract UUSD is Ownable, Pausable, Blacklistable, ERC20 {
    ...
    address private constant FACTORY_CONTRACT = address(0x43A8a15DB97932B697Edcb69815c83695f743220); //Testnet address
    address private UNISWAP_V2_ROUTER = address(0xdBd2D0363A01df3E686B379a8BB9Bb5bE34C3AAb); //Testnet Address
    ...
    */
    ...
    function settleAmount() external returns(uint) {
        uint usdc_balance = IERC20(usdcToken).balanceOf(address(this));
        uint _amountIn = usdc_balance.div(3);
    ...
    // //     usdcToken.transferFrom(vault,address(this),usdc_balance.div(3));
        IERC20(usdcToken).approve(UNISWAP_V2_ROUTER,_amountIn);
        uint[] memory result = uniswapRouter.swapExactTokensForTokens(_amountIn, 0, path, address(this), deadline); 
        return result[1]; 
    }    
    ...
    // //     usdcToken.transferFrom(vault,address(this),usdc_balance.div(3));
    //     usdcToken.approve(UNISWAP_V2_ROUTER,settleBalance);
    //     uniswapRouter.swapExactTokensForTokens(settleBalance, 0, path, vault, deadline); 
