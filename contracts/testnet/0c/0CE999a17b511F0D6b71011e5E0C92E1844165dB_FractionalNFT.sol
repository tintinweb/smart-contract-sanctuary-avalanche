...
contract FractionalNFT is ERC721, Ownable {
    ...
    function mint(uint256 tokenId, uint256 totalSupply) public onlyOwner {
...
contract FractionalToken is ERC20, Ownable {
    ...
    modifier onlyVault() {
        require(
            vault == msg.sender,
            "You are not allowed to declare the result"
        );
        _;
    }

    constructor() ERC20("Orebits.au", "ORBS") {
    ...
    // Function to set the maximum claim amount (callable only by the owner)
    // function setClaimAmount(address _claimAddress, uint256 _claimAmount) external onlyVault {
    //     claimableAmounts[_claimAddress] += _claimAmount;
    // }

    // // Function for users to claim tokens
    // function claim(address to) external returns(bool) {
    //     // Calculate the remaining amount the user is eligible to claim
    //     require(claimableAmounts[to] > 0, "Nothing left to claim");
    //     require(totalSupply() + claimableAmounts[msg.sender] <= reserves, "Reserves overflow error");
    //     _mint(to, claimableAmounts[msg.sender] );
