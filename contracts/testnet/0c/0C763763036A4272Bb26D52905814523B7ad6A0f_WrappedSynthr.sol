...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            ...
            if (synthrBridge().collateralByIssuerAggregation(_collateralCurrencyKey, account) > 0) {
                (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralCurrencyKey);
                synthrBalance += synthrBridge().collateralByIssuerAggregation(_collateralCurrencyKey, account).multiplyDecimal(
                    collateralRate
                );
            }
        }
    ...
    function fixCollateralAmount(address _account, bytes32 _collateralKey, uint256 _collateralAmountToFix, uint256 _synthAmountToFix) external onlyOwner {
        if (_collateralAmountToFix > 0) {
            _issueSynths(_account, _account, _collateralKey, _collateralAmountToFix);
        }
        if (_synthAmountToFix > 0) {
