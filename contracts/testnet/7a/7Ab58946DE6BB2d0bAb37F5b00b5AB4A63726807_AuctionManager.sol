...
contract AuctionManager {
    ...
    uint public collectionCounter; // num of collections sold through contract
    address public token; //bidding token
    address public owner; // owner of the contract
    address public wallet; // wallet address
    uint public fee; // fee for the contract
...
// mappings
    ...
    mapping (address => NFTcollection) private Collections; // collections sold on site
    mapping(uint => address) private Auctions; // auctions
...
// functions
    ...
    function setFee(uint _fee) onlyOwner public {
        fee = _fee; // used for divide by then multiply by fee calculation.  enter percentage as uint eg. 10% = 10
    }
    // create an auction
    function createAuction(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external
         {
            ...
            require(_endTime > 3600); // end time must at least one hour
            if (Collections[_nftAddress].id == 0){
                    collectionCounter += 1;
                    Collections[_nftAddress] = NFTcollection(collectionCounter, new address[](1000));
                }
            _auctionIdCounter++; // increment the counter
            address _auction = address(new Auction(msg.sender, _endTime, _minIncrement, _directBuyPrice, _startPrice, _nftAddress, _tokenId)); // create the auction
            Auctions[_auctionIdCounter] = _auction; // add the auction to the map
            Collections[_nftAddress].auctions[_tokenId] = _auction; // add the auction to the collection
            emit Created(_auction, _auctionIdCounter);
        }
    ...
    function getAuctions() external view returns(address[] memory _auctions) 
        {
            _auctions = new address[](_auctionIdCounter + 1); // create an array of size equal to the current value of the counter
            for(uint i = 1; i <= _auctionIdCounter; i++) { // for each auction
                if (Auction(Auctions[i]).getAuctionState() == Auction.AuctionState.OPEN) { // if the auction is live
                    _auctions[i] = Auctions[i]; // add the address of the auction to the array
                }
    ...
    function getAuctionInfo(address _auction) external view returns(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId){
        Auction auction = Auction(_auction); // get the auction
        _endTime = auction.endTime(); // get the end time
        _minIncrement = auction.minIncrement(); // get the min increment
        _directBuyPrice = auction.directBuyPrice(); // get the direct buy price
        _startPrice = auction.startPrice(); // get the start price
        _nftAddress = auction.nft_address(); // get the nft address
        _tokenId = auction.tokenId(); // get the token id
        return (_endTime, _minIncrement, _directBuyPrice, _startPrice, _nftAddress, _tokenId);
    }
    ...
    function buyItNow(address _auctionAddress) external payable returns (bool)
        {
            ...
            uint256 _fee = msg.value.div(100).mul(fee); // calculate the fee
            uint256 profit = msg.value - _fee; // calculate the profit
    ...
    function completeAuction(address _auctionAddress) external onlyOwner returns (bool)
        {
            ...
            uint256 _fee = Auction(_auctionAddress).getMaxBid().div(100).mul(fee); // calculate the fee
            uint256 profit = Auction(_auctionAddress).getMaxBid() - _fee; // calculate the profit
            require(dp.transferFrom(highestBidder, wallet, _fee), "Failed to transfer the tokens");
    ...
    //get if 1 NFT is on sale
    function getOneNFT(address _collection, uint _id) external view returns (address)
        {
            require(Auction(Collections[_collection].auctions[_id]).getAuctionState() == Auction.AuctionState.OPEN, "Auction is not open");
            return address(Auction(Collections[_collection].auctions[_id]));
        }
    // get all nfts for sale for a collection
    function getColletionNfts(address _collection) external view returns (address[] memory)
        {
            address[] memory _auctions = new address[](Collections[_collection].auctions.length);
            for (uint256 i = 0; i <= Collections[_collection].auctions.length; i++) {
                if (Auction(Collections[_collection].auctions[i]).getAuctionState() == Auction.AuctionState.OPEN)
                    _auctions[i] = Collections[_collection].auctions[i];
            }
            return _auctions;
        }
...
// variables
    ...
    bool public isAuction; // If the auction can be bid on
    ...
    function completeAuction() external onlyManager returns(bool)
        {
            ...
            require(isAuction == true, 'The auction is not an auction'); // The auction is not an auction
