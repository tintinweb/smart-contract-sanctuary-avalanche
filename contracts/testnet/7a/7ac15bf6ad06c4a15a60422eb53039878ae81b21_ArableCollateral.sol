...
contract ArableCollateral is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function removeSupportedCollateral(address token) public onlyOwner returns (bool) {
        ...
        for (uint256 i = _collateralAssetData[token].index; i < _supportedCollaterals.length - 1; i++) {
     ...
     * @return bool - to enable calls from other contracts
     */
    function burn(address beneficiary, uint256 amount) public nonReentrant returns (bool) {
