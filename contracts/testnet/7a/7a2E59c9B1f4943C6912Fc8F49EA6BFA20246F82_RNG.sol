...
contract RNG {
    ...
    function randomWinner(uint256 _eventId) public returns (uint256[] memory) {
        ...
        uint256 lenghtCheck;
        if (
            choosedEvent.numberOfWinners > choosedEvent.excludedNumbers.length
        ) {
            lenghtCheck =
                choosedEvent.numberOfWinners -
                choosedEvent.excludedNumbers.length;
        } else {
            lenghtCheck =
                choosedEvent.excludedNumbers.length -
                choosedEvent.numberOfWinners;
        ...
        if (choosedEvent.range > lenghtCheck) {
            numbersLength = choosedEvent.numberOfWinners;
        } else if (choosedEvent.numberOfWinners < (choosedEvent.range - choosedEvent.excludedNumbers.length)) {
