...
contract WBENFT is WhitelistEquipment, ERC721, ERC721Enumerable {
    ...
    /**
     * @notice Initiate Equipment as product and whitelist, for their qty is immutable
    ...
    ) ERC721("Wyndblast Equipment NFT", "WBENFT") {
    ...
    /**
     * @notice The following functions call burn standard erc721 burn method
 ...
 */
contract WhitelistEquipment is Ownable {
    uint256 public constant EQUIPMENT_GENESIS_SUPPLY = 4500;
     ...
     * @return consumedQty
     */
    function getWhitelistMemberData(
        string memory _group,
        address _account
    ) public view returns (uint256 reservedQty, uint256 consumedQty) {
    ...
    function addWhitelist(
        ...
        if (_itemType == Item.Forging) {
            require(
                totalQty + _itemQty <= EQUIPMENT_GENESIS_SUPPLY,
    ...
    function setQuantity(
        ...
        if (_itemType == Item.Forging) {
            require(
                (totalQty - itemQty) + _qty <= EQUIPMENT_GENESIS_SUPPLY,
                "New total must not exceed 4500"
            );
        }
    ...
    function isWhitelisted(
        string memory _group,
        address _member
    ) public view isAddress(_member) returns (bool) {
        WhitelistData storage _whitelist = _whitelists[_group];
    ...
    function consume(
        ...
        // Dont need to consume if _itemType > 0
        if (_itemType != Item.Forging) {
