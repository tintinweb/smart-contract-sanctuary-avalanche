...
contract NodeManager is Ownable, Pausable {
    ...
    uint256 public cashoutFee_perc = 100;
    uint256 public rewardsBoost_perc = 100;
    ...
    uint256[] private _boostRewardPerc = [100,125,150,175];
    ...
    function _calculateNodeRewards(address account, uint _lastClaimTime,uint i, uint256 amount_,uint256[3] memory tier) private view returns (uint256[2] memory) {
	...
	uint _boost = 0;
        ...
        uint256 rew = ((rewardPerDay.mul(10000).div(1440) * (elapsedTime_ / 1 minutes)) / 10000) * (_boostRewardPerc[_boost]).div(100);
        uint256 rew_ = rew - ((rewardPerDay.mul(10000).div(1440) * (elapsedTime_ / 1 minutes)) / 10000);
        return [rew,rew_];
    }
        ...
        returns (uint256[2] memory)
    {
        ...
        uint256 rewardsBoost = 0;
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            uint256[2] memory rew = _calculateNodeRewards(account, _node.lastClaimTime,i, _node.amount,tier);
            rewardsTotal += rew[0];
            rewardsBoost += rew[1];
        }
        return [rewardsTotal,rewardsBoost];
    }
