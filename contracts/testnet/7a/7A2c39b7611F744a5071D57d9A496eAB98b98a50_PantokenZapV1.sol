...
import "./IPantokenRouter01.sol";

interface IPantokenRouter02 is IPantokenRouter01 {
...
import {Ownable} from "../openzeppelin/contractsV4.8.1/access/Ownable.sol";
import {IERC20} from "../openzeppelin/contractsV4.8.1/token/ERC20/IERC20.sol";
import {SafeERC20} from "../openzeppelin/contractsV4.8.1/token/ERC20/utils/SafeERC20.sol";
import {ReentrancyGuard} from "../openzeppelin/contractsV4.8.1/security/ReentrancyGuard.sol";

import {IPantokenPair} from "./interfaces/IPantokenPair.sol";
import {IPantokenRouter02} from "./interfaces/IPantokenRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by PantokenSwap teams.
 */
contract PantokenZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // PantokenRouter interface
    IPantokenRouter02 public pantokenRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from PantokenSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address PantokenRouter
    address private pantokenRouterAddress;
     ...
     * @param _pantokenRouter: address of the PantokenRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WETHAddress,
        address _pantokenRouter,
        uint256 _maxZapReverseRatio
    ) {
        WETHAddress = _WETHAddress;
        WETH = IWETH(_WETHAddress);
        pantokenRouterAddress = _pantokenRouter;
        pantokenRouter = IPantokenRouter02(_pantokenRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. TOKENNAME/ETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. TOKENNAME) to receive in the intermediary swap (e.g. ETH --> TOKENNAME)
     */
     ...
     * @param _lpToken: LP token address (e.g. TOKENNAME/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. TOKENNAME) in the intermediary swap (e.g. BUSD --> TOKENNAME)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. TOKENNAME/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. TOKENNAME --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. TOKENNAME/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. TOKENNAME or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. TOKENNAME) in the intermediary swap (e.g. BUSD --> TOKENNAME)
     */
    ...
    {
        address token0 = IPantokenPair(_lpToken).token0();
        address token1 = IPantokenPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IPantokenPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = pantokenRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = pantokenRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IPantokenPair(_lpToken).token0() || _token0ToZap == IPantokenPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IPantokenPair(_lpToken).token0() || _token1ToZap == IPantokenPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IPantokenPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IPantokenPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IPantokenPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = pantokenRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = pantokenRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IPantokenPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "PantokenRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "PantokenRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        pantokenRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), pantokenRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(pantokenRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = pantokenRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = pantokenRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = pantokenRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = pantokenRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = pantokenRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
