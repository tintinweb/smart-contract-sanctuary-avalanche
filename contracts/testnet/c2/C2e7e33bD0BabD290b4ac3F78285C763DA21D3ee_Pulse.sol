...
contract Pulse {
    ...
    function reimbourseInvestors(bytes32 _projectId) public onlyAdmin {
        ...
        for(uint256 i = 0; i < projectInvestors[_projectId].length; i++) {
            address investor = projectInvestors[_projectId][i];
            uint256 totalInvestment = 0;
            for(uint256 j = 0; j < investorProjectInvestments[investor][_projectId].length; j++) {
                totalInvestment += investorProjectInvestments[investor][_projectId][j].amount;
            }
            uint256 amountToReimburse = (totalInvestment / projects[_projectId].raised) * projects[_projectId].raised;
            IERC20(usdtTokenAddress).transfer(investor, amountToReimburse);
        }
