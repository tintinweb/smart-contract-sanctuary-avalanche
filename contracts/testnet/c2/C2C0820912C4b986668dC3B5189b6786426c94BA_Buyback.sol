...
contract Buyback is IBuyback, AccessControl {
    using SafeERC20 for IERC20;

    IQZoneNFT immutable nftContract;
    ...
    mapping(address => mapping(uint256 => address)) private _userPurchaseToken;

    constructor(address admin, address _penaltyWallet, address _nftContract) {
    ...
    function purchase(uint256 id) external {
        ...
        require(props.price > 0, 'bad token id');

        if (props.singleton)
            require(nftContract.balanceOf(msg.sender, id) == 0, 'Singleton NFT');

        IERC20 stable = IERC20(props.token);

        nftContract.mintInternal(msg.sender, id, 1);

        _userPurchaseTimestamp[msg.sender][id] = block.timestamp;

        if (props.buybackGarantee) {
          _userPurchasePrice[msg.sender][id] = props.price;
          _userPurchaseToken[msg.sender][id] = props.token;
        }

        emit Purchase(msg.sender, id, block.timestamp);

        if (props.buybackGarantee) {
            stable.safeTransferFrom(msg.sender, address(this), props.price);
        } else if (props.burnOnBuy) {
            stable.safeTransferFrom(msg.sender, address(0xdead), props.price);
        } else {
            stable.safeTransferFrom(msg.sender, penaltyWallet, props.price);
        }
    }
    ...
    function buyback(uint256 id) external {
        ...
        address token = _userPurchaseToken[msg.sender][id];
        _transferTokensWithPenalty(IERC20(token), msg.sender, price, penalty);
    }

    /// @dev Returns amount of stable user will get after buyback
    /// @param id of purchased nft (only one nft id for one user)
 ...
 * @title IBuyback
 * @author gotbit
 * @dev Buying IQZoneNFTs and refunding them ("buying back").
 */
