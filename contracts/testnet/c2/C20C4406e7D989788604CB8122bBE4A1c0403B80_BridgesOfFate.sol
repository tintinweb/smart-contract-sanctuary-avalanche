...
contract BridgesOfFate is Ownable {
    ...
    mapping(uint256 => mapping(bool => bytes32[])) public _playerArray; //change
    // mapping(uint256 => mapping(bool => bytes32[])) public _selectUnholdPlayerId; //change
    ...
    function _deletePlayerIDForSpecifyStageForMapping(uint256 _stage,bool  _side, bytes32 _playerId) internal {
        for (uint i = 0; i < _playerArray[_stage][_side].length; i++) {
            if(_playerArray[_stage][_side][i] == _playerId){
                delete _playerArray[_stage][_side][i];
            }
        }
    }
    ...
    function generateRandomTicketNumbers(uint256 _lotteryCount,uint256 _range,uint256 _length) public view returns (uint8[6] memory) {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            while (!readyToAdd && retry <= maxRetry) {
                ...
                // generatedNumber = uint256ToArray(uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, i, retry, _lotteryCount))),_range);
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        // _playerArray[_member.stage][_jumpSide].push(playerId);
        // _deletePlayerIDForSpecifyStageForMapping(_member.stage,_member.lastJumpSide,playerId); 
    ...
    function _selectJumpedSidePlayersMapping(bytes32[] memory _playerIDAtSpecifyStage,uint256 currentDay) public  returns(bytes32[]  memory){ 
        ...
        for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
            if((OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == false && currentDay == PlayerItem[_playerIDAtSpecifyStage[i]].day )
                ||
                 (OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == true)
                )
                
            {
                if(PlayerItem[_playerIDAtSpecifyStage[i]].stage >= 1 && PlayerItem[_playerIDAtSpecifyStage[i]].ishold == false){
                    // _selectSuccessSidePlayerId.push(_playerIDAtSpecifyStage[i]);
                        // _selectUnholdPlayerId[_member.stage][_jumpSide].push(playerId);
                        _selectUnholdPlayerId.push(_playerIDAtSpecifyStage[i]);
                        OverFlowPlayerStatus[_selectUnholdPlayerId[i]] =  false;
    ...
    */
    ...
    function _selectPlayerAndSetCapacityOnTile(uint256 _currentPlayerStage,uint256 currentDay,bool _jumpSide) internal  { 
        ...
        //     bytes32[] memory _PlayerIDAtSpecifyStage  =  _playerArray[_currentPlayerStage][_jumpSide]
            ...
            // for (uint i = 0; i < _PlayerIDAtSpecifyStage.length; i++) {
                ...
                // {
                //     OverFlowPlayerStatus[_PlayerIDAtSpecifyStage[i]] =  false;
                //     if(PlayerItem[_PlayerIDAtSpecifyStage[i]].stage >= 1 && PlayerItem[_PlayerIDAtSpecifyStage[i]].ishold == false){
                //         // _selectSuccessSidePlayerId.push(_playerIDAtSpecifyStage[i]);
                //             _selectUnholdPlayerId[_PlayerIDAtSpecifyStage[i].stage][_jumpSide].push(_PlayerIDAtSpecifyStage[i]);
        ...
        delete _uniqueRN;
        // delete _selectUnclearholdPlayerId; 
