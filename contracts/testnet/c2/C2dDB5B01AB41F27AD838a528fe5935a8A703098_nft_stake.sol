...
contract nft_stake {
    ...
    mapping(address => uint256[]) public green_time; 
    mapping(address => uint256[]) public amts;
    constructor() {
    ...
    function calc() public {
    	...
    	uint j;
		j = 0;
	    	while (j < stakes_1[msg.sender].amount) {
			if ((stakingTime_1[msg.sender][j] - time) > 24*60*60) {
				va += 1;
				}
			j++;
		}
		...
		va = 0;
		j = 0;
		while (j < stakes_2[msg.sender].amount) {
			if ((stakingTime_1[msg.sender][j] - time) > 24*60*60) {
				va += 1;
				}
			j++;
		...
		va = 0;
		j = 0;
		while (j < stakes_3[msg.sender].amount) {
			if ((stakingTime_1[msg.sender][j] - time) > 24*60*60) {
				va += 1;
				}
			j++;
    ...
    function whileTime(uint256[] memory _arr, uint256 _amount) public view returns (uint) {
    ...
    function checkTier(address _account) public view returns (uint256[3] memory) {
    	uint256[3] memory ls = [uint256(0),uint256(0),uint256(0)];
    	uint256 _amount_1 = stakes_1[_account].amount;
    	uint256 _amount_2 = stakes_2[_account].amount;
    	uint256 _amount_3 = stakes_3[_account].amount;
    	uint256[] memory arr_1;
    	if (_amount_1 >0){
    		arr_1 = stakingTime_1[_account];
    		ls[0] = whileTime(arr_1,_amount_1);
    	}
    	if (_amount_2 > 0){
    		arr_1 = stakingTime_2[_account];
    		ls[1] = whileTime(arr_1,_amount_2);
    	}
    	if (_amount_3 > 0){
    		arr_1 = stakingTime_3[_account];
    		ls[2] = whileTime(arr_1,_amount_3);
    	}
    	return ls;
    }
    function unstake(uint256 _tokenId, uint8 _amount) public {
     ...
     function CheckEm(address _account) external view returns (uint256[3] memory) {
        return checkTier(_account);
    }
     function onERC1155Received(
        address operator,
        address from,
