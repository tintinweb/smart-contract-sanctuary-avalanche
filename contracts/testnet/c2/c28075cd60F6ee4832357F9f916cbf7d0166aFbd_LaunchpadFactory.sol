    ...
    WithClaimer
{
    ...
    function internalBuyTokens(
        ...
        userWeight = MerkleProof.verify(merkleProof, merkleRoot, node)
            ? userWeight
            : 0;
        uint256 maxAllocation = checkAccountAllowedToBuy(userWeight);
    ...
    WithLimits
{
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 weight = levelsOpen
            ? maxSell + userWeight.mul(rate).div(1e18)
            : userWeight.mul(rate).div(1e18);
        uint256 allocation = calculatePurchaseAmountUSD(weight);
