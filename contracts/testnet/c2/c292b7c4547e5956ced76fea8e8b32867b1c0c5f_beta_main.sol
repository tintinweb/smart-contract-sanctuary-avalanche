...
contract beta_main is ERC20, Ownable {
	...
	//payable(treasury).transfer(supply.div((10000)/(uint256(10)*100)));
	//payable(rewardsPool).transfer( supply.div((10000)/(uint256(10)*100)));
	//payable(teamPool).transfer(supply.div((10000)/(uint256(10)*100)));
    ...
    function sendFees(uint256 _rewardsPoolFee, uint256 _TreasuryFee, uint256 _teamFee, uint256 _transferFee, uint256 _NFTFee,uint256 _cashoutFee, address _account, uint256 _rewards) private {
		...
		if (_rewards != 0) {
			Send_it(address(this),_account, uint256(_rewards));
			}
		allfees_ = allfees_ - _rewardsPoolFee;
		Send_it(address(this),Mars, uint256(allfees_));
	}	
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function checkTXN(address _account) internal view returns (uint256){
        ...
        require(balanceOf(_account) >= nodeAmount,"you do not have enough to complete the Node purchase");
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        //Send_it(sender, address(this), checkTXN(sender));
        //Send_it(address(this),rewardsPool, checkTXN(address(this),rewardsFee));
        //Send_it(address(this),teamPool,checkTXN(address(this),nodeAmount.div((10000)/(teamPoolFee*100))));
        //Send_it(address(this),rewardsPool, checkTXN(address(this), nodeAmount.div((10000)/(treasuryFee*100))));
        //emit Transfer(address(this),rewardsPool, checkTXN(address(this),rewardsFee));
        //emit Transfer(address(this),teamPool,checkTXN(address(this),nodeAmount.div((10000)/(teamPoolFee*100))));
        //emit Transfer(address(this),rewardsPool, checkTXN(address(this), nodeAmount.div((10000)/(treasuryFee*100))));
    ...
    function cashoutAll() external {
        ...
        tier =  _overseer.getNFTdata(sender);
        uint _perc = 100;
	if (tier[0] != 0){
		tier[0] = tier[0].mul(5);
	}
	if (tier[1] != 0) {
		...
		_perc = cashoutFee.div((10000)/(cashoutFee*100));
	}
	if (tier[2] != 0) {
        ...
        uint256 feeAmount = rewardAmount.mul(cashoutFee.div((10000)/(50*100)));
   ...
   //function get(address account) public returns (uint256[] memory){
  // 	
  // 	return tier;
   //}
