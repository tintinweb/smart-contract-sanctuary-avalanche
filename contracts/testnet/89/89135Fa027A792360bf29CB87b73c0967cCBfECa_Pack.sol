    ...
    IPack
{
    ...
    uint256[] public quackIDsRemaining;
    IERC721 public quacksAddress; 
    ...
    constructor(
     ...
     address _quackAddr
        ...
        quacksAddress = IERC721(_quackAddr);
    ...
    function addToQuackCount(uint quackCount) external onlyRoleWithSwitch(minterRole) { 
        require(quackCount + quackIDsRemaining.length <= 400, "!Len");
        for (uint j = quackIDsRemaining.length; j < quackIDsRemaining.length + quackCount; j++) {
            quackIDsRemaining.push(j);
        }
    }
    ...
    function openPack(uint256 _packId, uint256 _amountToOpen) external returns (Token[] memory, uint256) {
       ...
       (Token[] memory rewardUnits, uint256 random) = getRewardUnits(_packId, _amountToOpen, pack.amountDistributedPerOpen, pack);
        ...
        uint256 targetID = 999999999;
        if (quackIDsRemaining.length > 0) {
            uint256 randomVal = uint256(keccak256(abi.encode(random, 999999999)));
            uint256 targetQuack = randomVal % quackIDsRemaining.length;
            if(targetQuack <= quackIDsRemaining.length-1) {
                targetID = quackIDsRemaining[targetQuack];
                quackIDsRemaining[targetQuack] = quackIDsRemaining[quackIDsRemaining.length-1];
                quackIDsRemaining.pop();
                quacksAddress.safeTransferFrom(address(this), msg.sender, targetID);
            }
        }

        emit PackOpened(_packId, opener, _amountToOpen, rewardUnits, targetID);

        return (rewardUnits, targetID);
    }
    ...
    ) internal returns (Token[] memory rewardUnits, uint256 random) {
...
interface IPack is ITokenBundle {
     ...
     */
    function openPack(uint256 packId, uint256 amountToOpen) external returns (Token[] memory, uint256);
}
