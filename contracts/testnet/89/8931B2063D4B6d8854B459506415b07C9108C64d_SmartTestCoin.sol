...
contract SmartTestCoin is ERC20 {
    ...
    uint256 private _presaleTotal = 200000;
    uint256 private _gameTotal = 1000000;

    uint256 private _outcomePoolFee = 5;
    uint256 private _incomePoolFee = 25;
    uint256 private _poolAddressFee = 10;
    uint256 private _legendaryAddressFee = 5;
    uint256 private _epicAddressFee = 2;
    ...
    function setEpicAddress(address address_) public onlyOwner {
        _epicAddress = address_;
    }

    function setLegendaryAddress(address address_) public onlyOwner {
        _legendaryAddress = address_;
    }
    ...
    function setPresaleContract(address address_) public onlyOwner {
        _presaleAddress = address_;
    }
    ...
    constructor() ERC20("TEST COIN V7", "$TCV V7") {
        _coinOwner = msg.sender;
        uint256 reserved = _presaleSupply().add(_gameSupply());
        uint256 initialSupply = _totalSupply().sub(_teamSupply().add(_investorSupply()).add(reserved));
    ...
    function fillPresaleContract() public onlyOwner {
        require(_presaleAddress != address(0), "Set presale contract address.");
        _mint(_presaleAddress, _presaleSupply());
    }

    function fillGameContract() public onlyOwner {
        require(_gameAddress != address(0), "Set game contract address.");
        _mint(_gameAddress, _gameSupply());
    }
    ...
    function _gameSupply() private view returns (uint256) {
        return _gameTotal.mul(10 ** decimals());
    }

    function _presaleSupply() private view returns (uint256) {
        return _presaleTotal.mul(10 ** decimals());
    }
    ...
    function _distributeFee(address sender, uint256 amount) private {
        ...
        if (sender != _poolAddress) {
            _transfer(sender, _poolAddress, percent.mul(_poolAddressFee));
        }

        _transfer(sender, _epicAddress, percent.mul(_epicAddressFee));
        _transfer(sender, _legendaryAddress, percent.mul(_legendaryAddressFee));
        ...
        _transfer(sender, _gameAddress, percent.mul(_gameAddressFee));
    ...
    function _customTransfer(address sender, address recipient, uint256 amount) private {
        require(_allowActions == true, "Actions not allowed.");
        uint256 actualAmount;
        if (sender == _poolAddress && recipient != _poolAddress) {
