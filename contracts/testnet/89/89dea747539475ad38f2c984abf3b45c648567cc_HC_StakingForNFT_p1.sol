...
/**
 * @dev Staking Rewards Interface for HeroesChained.
 */
interface IStakingRewards {

    function getTicketCount(address _user) external returns (uint256);
    function updateTicketCount(address _user) external;
}

/**
 * @dev HeroesChained Staking Contract for NFTs part1.
 */
contract HC_StakingForNFT_p1 is Ownable, Pausable, ReentrancyGuard, IStakingRewards {
    ...
    uint256 public ticketPerTokenStored;

    uint256 public stakeStartDate;
    uint256 public ticketStartDate;
    uint256 public ticketEndDate;
    uint256 public stakeEndDate;

    uint256 public minStake;
    uint256 public maxStake;
    address public stakingBank;
    uint256 public dailyTicketPrize;
    uint256 public ticketRate;

    mapping(address => uint256) public userRewardPerTokenPaid;
    mapping(address => uint256) public userTicketPerTokenPaid;
    ...
    constructor(
        ...
        require(_ticketStartDate < _ticketEndDate, "Ticket start date must be before ticket end date");
        require(_dailyTicketPrize > 0, "Daily ticket prize must be greater than 0");
        ...
        ticketStartDate = _ticketStartDate;
        ...
        dailyTicketPrize = _dailyTicketPrize;
	    uint l_1e18 = 1e18;
        ticketRate = (l_1e18.div(dailyTicketPrize)).div(60*60*24);
    }
    ...
    function firstTimeTicketApplicable() private view returns (uint256) {
        return lastUpdateTime < ticketStartDate ? ticketStartDate : lastUpdateTime;
    }
    ...
    function ticketPerToken() private view returns (uint256) {
        if (totalSupply == 0) {
            return ticketPerTokenStored;
        }
        if (block.timestamp < ticketStartDate || block.timestamp > ticketEndDate) {
            return ticketPerTokenStored;
        }
        return 
            ticketPerTokenStored.add(
                lastTimeRewardApplicable().sub( firstTimeTicketApplicable() ).mul(ticketRate)
            );
    }
    ...
    function earnedTickets(address account) private view returns (uint256) {
        return _balances[account].mul(  ticketPerToken().sub( userTicketPerTokenPaid[account] )  ).div( 1e18 ).add( tickets[account] );
    }

    modifier updateReward(address account) {
        rewardPerTokenStored = rewardPerToken();
        ticketPerTokenStored = ticketPerToken();

        lastUpdateTime = lastTimeRewardApplicable();

        rewards[account] = earned(account);
        tickets[account] = earnedTickets(account);

        userRewardPerTokenPaid[account] = rewardPerTokenStored;
        userTicketPerTokenPaid[account] = ticketPerTokenStored;
        _;
    }
    ...
    function updateTicketCount(address _user) external override updateReward(_user) {
    }

    function getTicketCount(address _user) public view override returns (uint256) {
