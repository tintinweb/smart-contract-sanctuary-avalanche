...
import "../interfaces/vela-exchange/IVault.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract VelaTokenFarmMock {
    // Info of each user.
    struct UserInfo {
    ...
    constructor(address _mockVlpAddress) {
        vlp = IERC20(_mockVlpAddress);
    }

    function deposit(uint256 _pid, uint256 _amount) external {
        UserInfo storage user = userInfo[msg.sender];
        vlp.transferFrom(msg.sender, address(this), _amount);
        user.amount += _amount;
    }

    function withdraw(uint256 _pid, uint256 _amount) external {
        UserInfo storage user = userInfo[msg.sender];
        vlp.transfer(msg.sender, _amount);
        user.amount -= _amount;
    }
}