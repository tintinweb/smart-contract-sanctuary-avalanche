...
/// @title IQZoneStaking
/// @author gotbit
contract IQZoneStaking is Ownable, IIqStaking2 {
    ...
    function setClasses(Class[] calldata newClasses) external onlyOwner {
        ...
        unchecked {
            for (uint256 i; i < newClasses.length; ++i) {
    ...
    function _share(address user) internal view returns (uint256) {
        ...
        return stake_.startTimestamp == 0 ? 0 :
          (stake_.amount * stake_.class.rate * 1 ether) / weightedTotalSupply;
    }
    ...
    function userShares(
        uint256 offset,
        uint256 size
    ) external view returns (UserSharesOutput[] memory result) {
        require(offset + size <= stakers.length, 'out of bounds');
        result = new UserSharesOutput[](size);

        for (uint256 i; i < size; ) {
            address user = stakers[offset + i];

            result[i] = UserSharesOutput({user: user, share: _share(user)});
            unchecked {
                ++i;
            }
        }
