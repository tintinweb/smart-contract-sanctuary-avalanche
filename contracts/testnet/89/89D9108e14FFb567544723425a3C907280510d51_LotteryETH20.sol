...
contract LotteryETH20 is Ownable, Pausable, ReentrancyGuard {
	...
	//busd
	address public BUSD = 0x20BA324d0DDcA357e1523c95bdB516A780e77398;

	//fees
	uint256 public ticketPrice = 0.01 ether; //all prices are in BUSD
	...
	function delBuildNumber2() internal pure {} //etherscan trick
	 ...
	 */
	function buyTickets(address affiliateAddress) external notContract whenNotPaused nonReentrant {
		...
		if (affiliateAddress != address(0)) {
			require(
				IERC20(BUSD).transferFrom(msg.sender, affiliateAddress, affiliateReward),
				"failed transfer affiliate reward"
			);
		} else {
			//if the user didn't set an affiliate, transfer the amount to nonAffiliateWallet
			require(
				IERC20(BUSD).transferFrom(msg.sender, nonAffiliateWallet, affiliateReward),
		...
		require(
			IERC20(BUSD).transferFrom(msg.sender, insuranceBuyBackWallet, insuranceBuyBackAmount),
			"failed transfer to insuranceBuyBackWallet"
		);

		//rest of the BUSD goes to the contract
		uint256 remainderBUSD = ticketPrice -
			affiliateReward -
			royaltiesAmount -
	...
	function withdrawRoyalties() external notContract whenNotPaused nonReentrant {
		...
		require(
			IERC20(BUSD).transferFrom(msg.sender, _msgSender(), royalties),
			"failed transfer nonAffiliateWallet reward"
		);
	}
	...
	function distributeEarnings() internal {
		...
		require(
			IERC20(BUSD).transferFrom(address(this), multisigWallet, winnerAmount),
