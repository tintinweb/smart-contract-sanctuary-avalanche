...
contract BunyRaffleNft is ERC721Upgradeable, IEditionSingleMintable,  OwnableUpgradeable {
  ...
  event Winnerpicked(address winner, uint256 winningNumber, uint256 endTime, uint256 prize);
...
function pickWinner() public  returns (uint256, address payable) {
  ...
  emit Winnerpicked(Winner, winningNumber, endTime, prize);
  return (winningNumber,Winner);
}

    function claimPrize() public {
    address payable tokenOwner = payable(ownerOf(winningNumber));
  ...
  function _mintEditions(address[] memory recipients) internal returns (uint256) {
    ...
    AddressUpgradeable.sendValue(payable(owner()), address(this).balance * 10 / 100);
  }
