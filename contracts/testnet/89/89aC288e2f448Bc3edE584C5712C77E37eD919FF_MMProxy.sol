  ...
  SP1Storage
{
    ...
    internal
  {
    ...
    _setupRole(OWNER_ROLE, guardian);
    _setupRole(DELEGATION_ADMIN_ROLE, guardian);
    _setupRole(BORROWER_ROLE, guardian);
    _setupRole(EXCHANGE_OPERATOR_ROLE, guardian);
...
abstract contract SP2Exchange is SP1Balances {
    ...
    // @notice Address to help to bridge usdt to marginX.
    address public MM_ACCOUNT;
        ...
        returns (uint256)
    {
        ...
        // Transfer token to marker maker account
        TOKEN.safeTransfer(address(MM_ACCOUNT), quantizedAmount);
        ...
        emit DepositedToMM(msg.sender, recipient, address(TOKEN), targetIBC, tokenAmount);
        return tokenAmount;
    }
  ...
  SP2Exchange
{
   ...
   *
   * @param  mmAccount   The FX-EVM market maker which token will be sent to before bridge to marginX
   */
  function setMMContract(
      ...
      onlyRole(OWNER_ROLE)
  {
      require(mmAccount != address(0),'Address 0');
