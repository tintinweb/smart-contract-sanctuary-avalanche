...
interface ISignatureVerifier {
    function verifyListing(DataTypes.Listing calldata listing) external view returns (bool);
    function verifyOffer(DataTypes.Offer calldata offer) external view returns (bool);
}
...
contract PawnPeer is Ownable, IERC721Receiver {
    ...
    uint256 public platformFee; // 2.5% default
    uint256 public minDailyInterest;
    uint256 public maxDailyInterest;

    mapping(address => uint256) public userMinNonce;
    mapping(address => mapping(uint256 => bool)) private nonceExecutedOrCancelled;

    uint256 public nextDealId = 1;
    mapping(uint256 => DataTypes.Deal) public idToDeal;

    mapping(address => CollectionDiscount) public collectionToDiscount;
    ...
    event CancelAllOrdersAndOffers(uint256 minNonce);
    event CancelMultipleOrdersAndOffers(uint256[] nonceList);
    event DiscountUpdated(CollectionDiscount discount);
    ...
    constructor(address _wavaxAddress, address _paymentReceiver) {
        ...
        minAskPrice = 1 ether / 10; // 0.1 avax
        minLockTime = 7 days;
        platformFee = 250; // 1.5%
    }
    ...
    function enableDiscount(address collection, uint256 discountRate) external onlyOwner {
        collectionToDiscount[collection] = CollectionDiscount({
            collection: collection,
            discountRate: discountRate,
            enabled: true
        });
        emit DiscountUpdated(collectionToDiscount[collection]);
    }

    function disableDiscount(address collection) external onlyOwner {
        if (collectionToDiscount[collection].enabled) {
            collectionToDiscount[collection].enabled = false;
            emit DiscountUpdated(collectionToDiscount[collection]);
        }
    }
    ...
    function setSignatureVerifierAddress(address _signatureVerifier) external onlyOwner {
        signatureVerifierAddress = _signatureVerifier;
    }
    ...
    function setMinAskPrice(uint256 _minAskPrice) external onlyOwner {
    ...
    function acceptDeal(DataTypes.Listing calldata listing) external payable {
        validateListingAndRevert(listing);
        address transferFromAddress = address(this);

        if (msg.value == 0) {
            // Transfer wavax from user
            transferFromAddress = msg.sender;
        } else {
            // Convert paid avax to wavax and transfer from contract
            if (msg.value < listing.askPrice) {
                revert InsufficientPayment();
            }
            IWAVAX(wavaxAddress).deposit{value: msg.value}();
        }

        INFTShipper(nftShipperAddress)
            .transferNFT(listing.collection, listing.maker, address(this), listing.tokenId, listing.amount);

        nonceExecutedOrCancelled[listing.maker][listing.nonce] = true;
        createDealAndTransferFunds(listing.collection, listing.tokenId, listing.amount,
            listing.maxLockTime,listing.maker, msg.sender, listing.askPrice, transferFromAddress, listing.dailyInterest);
    }

    function acceptOffer(DataTypes.Offer calldata offer, uint256 tokenId) external {
        ...
        nonceExecutedOrCancelled[offer.taker][offer.nonce] = true;
        createDealAndTransferFunds(offer.collection, tokenId, offer.amount,
            offer.maxLockTime, msg.sender, offer.taker, offer.bidPrice, offer.taker, offer.dailyInterest);
    }

    function createDealAndTransferFunds(address collection, uint256 tokenId, uint256 amount, uint256 maxLockTime,
        address maker, address taker, uint256 dealPrice, address transferFundsFrom, uint256 dailyInterest) internal {
        if (dealPrice < minAskPrice) {
            revert InvalidPrice();
        }
        if (dailyInterest < minDailyInterest || dailyInterest > maxDailyInterest) {
            revert OutOfBounds();
        }
        ...
        unchecked {
            makerReceived = dealPrice - calculatePlatformFee(dealPrice, collection);
            platformReceived = dealPrice - makerReceived;
        }
        IERC20 wavax = IERC20(wavaxAddress);
    ...
    function closeDeal(uint256 dealId) external payable {
        ...
        if (deal.expiresTs < block.timestamp) {
            ...
            IWAVAX(wavaxAddress).deposit{value: msg.value}();
            IERC20(wavaxAddress).safeTransferFrom(address(this), deal.taker, msg.value);
    ...
    function forceCloseDeal(uint256 dealId) external onlyOwner {
        DataTypes.Deal storage deal = idToDeal[dealId];
        if (deal.acceptedTs == 0 || deal.closed) {
            revert InvalidDeal();
        }
    ...
    function validateListingAndRevert(DataTypes.Listing calldata listing) internal view {
        if (!ISignatureVerifier(signatureVerifierAddress).verifyListing(listing)) {
            revert InvalidSignature();
        }
        if (listing.expiresAt < block.timestamp || listing.nonce < userMinNonce[listing.maker] || nonceExecutedOrCancelled[listing.maker][listing.nonce]) {
            revert OrderExpired();
        }
    }

    function validateOfferAndRevert(DataTypes.Offer calldata offer) internal view {
        if (!ISignatureVerifier(signatureVerifierAddress).verifyOffer(offer)) {
            revert InvalidSignature();
        }
        if (offer.expiresAt < block.timestamp || offer.nonce < userMinNonce[offer.taker] || nonceExecutedOrCancelled[offer.taker][offer.nonce]) {
            revert OrderExpired();
        }
    }

    function cancelAllOrdersAndOffers(uint256 minNonce) external {
        if (minNonce < userMinNonce[msg.sender] || minNonce > userMinNonce[msg.sender] + 500000) {
            revert OutOfBounds();
        }
        userMinNonce[msg.sender] = minNonce;
        emit CancelAllOrdersAndOffers(minNonce);
    }

    function cancelMultipleOrdersAndOffers(uint256[] calldata nonceList) external {
        ...
        for (uint256 i = 0; i < nonceList.length; i++) {
            ...
            nonceExecutedOrCancelled[msg.sender][nonceList[i]] = true;
        }
        emit CancelMultipleOrdersAndOffers(nonceList);
    }
    ...
    function calculatePlatformFee(uint256 price, address collection) public view returns (uint256) {
        unchecked {
            uint256 fee = price * platformFee / (100 * percentageMultiplier);
            return applyDiscount(collection, fee);
        }
    }

    function applyDiscount(address collection, uint256 fee) view internal returns (uint256) {
        CollectionDiscount memory discount = collectionToDiscount[collection];
        if (!discount.enabled) {
            return fee;
        }
        unchecked {
            return fee - ((fee * discount.discountRate) / (100 * percentageMultiplier));
        }
    }
