...
/* Errors */
    ...
    error Raffle__DomainNameTooLong();
...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    mapping(uint32 => string[]) private s_partnerDomains;
    ...
    event ChangeCampaign(bytes32 keyHash);
    ...
    constructor(
        ...
        s_partnerDomains[0] = new string [](0);
        // Init data for 1st session
        s_tickets[s_raffleId] = new address payable [](0);
        s_partnerDomains[s_raffleId] = new string [](0);
    }

    function enterRaffle(string memory coupon, string memory domain) public payable {
        if (bytes(domain).length > 256) {
            revert Raffle__DomainNameTooLong();
        }
        ...
        uint32 bonusTickets = 0;

        // Update partner balance
        if (bytes(domain).length > 0) {
            if (s_partnerBalance[s_raffleId][domain] == 0) {
                s_partnerDomains[s_raffleId].push(domain);
            }
            s_partnerBalance[s_raffleId][domain] += msg.value - overbooking;
        }
        ...
        if (s_campaigns[s_raffleId].length > 0) {
            ...
            for (uint i=0; i < s_campaigns[s_raffleId].length; i++) {
                if (
                    s_campaigns[s_raffleId][i].keyHash == couponHash &&
                    s_campaigns[s_raffleId][i].minPct <= startBalancePct &&
                    startBalancePct <= s_campaigns[s_raffleId][i].maxPct
                ) {
                    bonusTickets += (realTickets * s_campaigns[s_raffleId][i].multiplierPct) / 100;
                }
            }
        }
        ...
        emit RaffleEnter(msg.sender, s_raffleState, realTickets, bonusTickets, domain);
    }
    ...
    function fulfillRandomWords(
        ...
        // Update raffle stats
        s_results[s_raffleId] = RaffleResult(block.timestamp, recentWinner, indexOfWinner, prize);
        // Copy campaigns if auto_start enabled
        if (s_raffleParams.autoStart) {
            copyCampaigns(s_raffleId, s_raffleId + 1);
        }
        ...
        emit WinnerPicked(recentWinner, indexOfWinner, prize, fee, s_raffleState);
    }
    ...
    function getRaffleResultByRaffleId(uint32 raffleId) public view returns (RaffleResult memory) {
    ...
    function getCampaigns(uint32 raffleId) public view returns (Campaign[] memory) {
        return s_campaigns[raffleId];
    }
    ...
    function getPartnerDomains(uint32 raffleId) public view returns (string[] memory) {
        return s_partnerDomains[raffleId];
    }

    function getPartnerBalance(uint32 raffleId, string memory domain) public view returns (uint256) {
        require(bytes(domain).length < 256, "Domain name too long");
        return s_partnerBalance[raffleId][domain];
    }
    ...
    function setCampaign(uint32 raffleId, Campaign memory campaign) public onlyOwner{
        ...
        if (s_campaigns[raffleId].length > 0) {
            for (uint i=0; i < s_campaigns[raffleId].length; i++) {
                if (s_campaigns[raffleId][i].keyHash == campaign.keyHash) {
                    found = true;
                    id = i;
                    break;
                }
        ...
        if (found) {
            s_campaigns[raffleId][id] = campaign;
        } else {
            s_campaigns[raffleId].push(campaign);
        }
        emit ChangeCampaign(campaign.keyHash);
    }

    function deleteCampaign(uint32 raffleId, string memory key) public onlyOwner {
        ...
        if (s_campaigns[raffleId].length > 0) {
            ...
            if (found) {
                for (uint i = id; i < s_campaigns[raffleId].length - 1; i++){
                    s_campaigns[raffleId][i] = s_campaigns[raffleId][i + 1];
                }
                s_campaigns[raffleId].pop();
            }
        }
        emit ChangeCampaign(keyHash);
    }

    function copyCampaignsAdmin(uint32 fromRaffleId, uint32 toRaffleId) public onlyOwner {
        copyCampaigns(fromRaffleId, toRaffleId);
    }

    function copyCampaigns(uint32 fromRaffleId, uint32 toRaffleId) internal {
        if (s_campaigns[fromRaffleId].length > 0) {
            for (uint i = 0; i < s_campaigns[fromRaffleId].length; i++) {
                s_campaigns[toRaffleId].push(s_campaigns[fromRaffleId][i]);
            }
        }
    }
