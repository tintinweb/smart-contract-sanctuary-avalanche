...
ReentrancyGuardUpgradeable
{
    ...
    ICertToken private _collateralToken; // (default dAVAX)
    ...
    function provideInAAVAXc(uint256 amount)
    external
    override
    nonReentrant
    returns (uint256 value)
    {
        value = _ceRouter.depositAAVAXcFrom(msg.sender, amount);
        // deposit ceToken as collateral
        _provideCollateral(msg.sender, value);
        emit Deposit(msg.sender, value);
        return value;
     ...
     */
    // claim in aAVAXc
    function claimInAAVAXc(address recipient)
    external
    override
    nonReentrant
    onlyOperator
    ...
    {
        _withdrawCollateral(msg.sender, amount);
        value = _ceRouter.withdrawAAVAXc(recipient, amount);
    ...
    nonReentrant
    {
        _ceRouter.withdrawAAVAXc(recipient, amount);
    }
