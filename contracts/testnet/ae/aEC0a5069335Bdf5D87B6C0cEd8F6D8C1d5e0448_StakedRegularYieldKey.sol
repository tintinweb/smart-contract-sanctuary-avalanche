...
interface IRegularYK is IValiFiYK {
...
/**
 * @title staking regular Yield Key Contract / NFT Smart Contract ERC721
 * @dev Implementation of the staking regular Yield Key NFT, based on ERC900.
 * @custom:a ValiFi
 */
contract StakedRegularYieldKey is
    ...
    PausableUpgradeable
{
    ...
    // Instance of ValiFi Regular YieldKey
    IRegularYK public valifiRYK;
    IMaintFee public maintFee;
    // Mapping of Staked Regular YieldKeys
    ...
    /**
     * @dev Mapping for storing Staked Regular YieldKeys
    ...
    /**
     * @dev Event when a Regular YieldKey is Staked
     * @param user Address of the Regular YieldKey NFT holder
     * @param tokenIds Token Ids of the Regular YieldKeys
     * @param capacityAmount Capacity Amount per YieldKey NFT
     */
    ...
    /**
     * @dev Event when a Regular YieldKey is UnStaked
     * @param user Address of the Regular YieldKey NFT holder
     * @param tokenIds Token Ids of the Regular YieldKeys
     * @param capacityAmount Capacity Amount per YieldKey NFT
     */
    ...
    function initialize(
        address regularYieldKey,
        string memory _initURI
    ) public initializer {
        __ERC721_init_unchained("Staked Regular YieldKey", "RYKSTK");
        __Ownable_init();
        __Pausable_init();
        valifiRYK = IRegularYK(regularYieldKey);
        setBaseURI(_initURI);
    }
    ...
    /**
     * @dev This method checks if the YieldKey is Staked
     * @param wallet address of Regular YieldKey holder
     * @param _tokenId Token Id of Regular YieldKey
     */
    ...
    /**
     * @dev This method allows to stake Regular YieldKeys
     * @param wallet address of Regular YieldKey holder
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                valifiRYK.ownerOf(tokenIds[i]) == _msgSender(),
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            valifiRYK.safeTransferFrom(wallet, address(this), tokenIds[i]);
            ...
            stakedYieldKeys[wallet][tokenIds[i]].endstake.push(uint64(0));
						_capacityAmount[i] = valifiRYK.capacityAmount(tokenIds[i]);
        }
        emit StakedYieldKey(wallet, tokenIds, _capacityAmount);
    }

    /**
     * @dev This method allows to unStake Regular YieldKeys
     * @param wallet address of Regular YieldKey holder
        ...
        whenNotPaused
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _burn(tokenIds[i]);
            valifiRYK.safeTransferFrom(address(this), wallet, tokenIds[i]);
            // Reset Maintenance Fee
            maintFee.setResetMaintenanceFee(tokenIds[i], valifiRYK.capacityAmount(tokenIds[i]));
						_capacityAmount[i] = valifiRYK.capacityAmount(tokenIds[i]);
        }
    ...
    /**
     * @dev Get History of Staked Regular YieldKey
     * @param wallet address of Regular YieldKey
     * @param tokenId Token Id of Regular YieldKey
     */
    ...
    /**
     * @dev Get Regular YieldKey Tokens owned by a wallet address
     * @param _wallet Wallet address of the owner
     * @return tokenIds Array of token Ids of Regular YieldKeys owned by the wallet
     */
    ...
    /**
     * @dev Get Staked Regular YieldKey Tokens owned by a wallet address
     * @param _owner address of tokens holder
     * @return stakedTokenIds tokenIds Array of Staked Regular YieldKeys owned by wallet address
     */
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !paused(),
            "ERC721 ValiFi: can't stake/unstake or transfer any Regular YieldKey while paused"
        );
        if (from != address(0)) {
            require(
