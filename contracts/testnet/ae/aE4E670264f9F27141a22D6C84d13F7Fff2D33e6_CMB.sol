...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private constant DAY_TO_SECOND = 86400;
    uint256 private constant RIGHT_TO_CANCEL = 3;
    ...
    event Paid(uint256 indexed paymentId, uint256 amount, string currency);
    event PaidAll(uint256 indexed paymentId);
    event ConfirmedToRelease(uint256 indexed paymentId);
    event Claimed(uint256 indexed paymentId, uint256 amount);
    event WithdrawnServiceFee(uint256 amount, address indexed fundingReceiver, string token);
    event Cancelled(uint256 indexed paymentId);
    event WithdrawnAmount(uint256 indexed paymentId, uint256 amount, string currency);
     ...
     */
    function initialize(address _owner, address _usdcToken, address _usdtToken) public initializer {
        OwnableUpgradeable.__Ownable_init();
        transferOwnership(_owner);
        serviceFeePercent = 15 * WEIGHT_DECIMAL / 10;
        usdcToken = IERC20(address(_usdcToken));
        usdtToken = IERC20(address(_usdtToken));
    }
     ...
     *  @param  currency    Type of token that needs to be paid (Example: USDT, USDC)
     *
     *  Emit event {Paid}
     */
    function payToken(uint256 paymentId, uint256 amount, string memory currency) 
        external 
        onlyValidPayment(paymentId) 
        ...
        nonReentrant  
    {
        ...
        string memory token = payment.currency;
        ...
        require(
            keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked((payment.currency))),
            "You need to pay exact currency"
        );

        if (keccak256(abi.encodePacked((currency))) == keccak256(abi.encodePacked(("USDC")))) {
            acceptUsdcPayment(payment.client, amount);
        } else if (keccak256(abi.encodePacked((currency))) == keccak256(abi.encodePacked(("USDT")))) {
            acceptUsdtPayment(payment.client, amount);
        }
        ...
        if (
            ...
            emit PaidAll(paymentId);
        } else {
            payment.status = Status.PAYING;
            emit Paid(paymentId, payment.paidAmount, payment.currency);
        }
    }
        ...
        nonReentrant  
    {
        Payment storage payment = payments[paymentId];
        require(
            keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("AVAX"))), 
            "You must pay AVAX"
        );
    ...
    /** 
     *  @notice Get balance of by type of token
     * 
     *  @dev    Anyone can call this function.
     * 
     *          Name        Meaning 
     *  @param  token       Type of token that want to get balance (Example: USDT, USDC)
     *
     *  Emit event {Paid}
     */
    function getTokenBalance(string memory token) public view returns (uint256 balance) {
        if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDC")))) {
            return usdcToken.balanceOf(_msgSender());
        } else if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDT")))) {
            return usdtToken.balanceOf(_msgSender());
        }
    } 

    /** 
     *  @notice Get allowance by type of token
     * 
     *  @dev    Anyone can call this function.
     * 
     *          Name        Meaning 
     ...
     *
     */
    function getAllowance(string memory token) public view returns (uint256 allowance) {
        if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDC")))) {
            return usdcToken.allowance(_msgSender(), address(this));
        } else if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDT")))) {
            return usdtToken.allowance(_msgSender(), address(this));
        }
    }

    function acceptUsdcPayment(address from, uint256 _amount) public returns (bool) {
       require(_amount <= getAllowance("USDC"), "Please approve tokens before transferring");
       usdcToken.transferFrom(from, address(this), _amount);
       return true;
    }

    function acceptUsdtPayment(address from, uint256 _amount) public returns (bool) {
       require(_amount <= getAllowance("USDT"), "Please approve tokens before transferring");
       usdtToken.transferFrom(from, address(this), _amount);
       return true;
    }

    /** 
     *  @notice Get balance of contract by type of token
     * 
     *  @dev    Only owner can call this function. 
     * 
     *          Name        Meaning 
     ...
     *
     */
    function getContractTokenBalance(string memory token) public onlyOwner view returns (uint256 balance) {
        if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDC")))) {
        ...
        nonReentrant 
    {
        ...
        emit Claimed(paymentId, amount - serviceFee);
    }

    /** 
     *  @notice Business Owner claim payment paid by other token (Example: USDC, USDT)
     * 
     *  @dev    Only Business Owner can call this function. 
     * 
     *          Name        Meaning 
     *  @param  paymentId   ID of payment that needs to be claimed
     *
     *  Emit event {Claimed}
     */
    function claimToken(uint256 paymentId) 
        external 
        onlyValidPayment(paymentId) 
        onlyBusinessOwner(paymentId) 
        nonReentrant 
    {
        ...
        serviceFeeUsdtTotal += serviceFee;

        if (keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDC")))) {
            usdcToken.transfer(_msgSender(), amount - serviceFee);
        } else if (keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDT")))) {
            usdtToken.transfer(_msgSender(), amount - serviceFee);
        }
        ...
        nonReentrant
    {
        ...
        emit WithdrawnServiceFee(_amount, _fundingReceiver, "AVAX");
    }

    function withdrawServiceFeeToken(uint256 _amount, address _fundingReceiver, string memory _token)
        external 
        onlyOwner 
        onlyValidAddress(_fundingReceiver) 
        nonReentrant
    {
        ...
        if (keccak256(abi.encodePacked((_token))) == keccak256(abi.encodePacked(("USDC")))) {
            serviceFeeUsdcTotal -= _amount;
            usdcToken.transfer(_fundingReceiver, _amount);
        } else if (keccak256(abi.encodePacked((_token))) == keccak256(abi.encodePacked(("USDT")))) {
            serviceFeeUsdtTotal -= _amount;
            usdtToken.transfer(_fundingReceiver, _amount);
        }
        // serviceFeeUsdtTotal -= _amount;
        // usdtToken.transfer(_fundingReceiver, _amount);
        emit WithdrawnServiceFee(_amount, _fundingReceiver, _token);
    }
        ...
        onlyClient(paymentId) 
    {
        ...
        if (today - payment.firstPaidDate <= DAY_TO_SECOND * RIGHT_TO_CANCEL) {
            payment.status = Status.CANCELLED;
            emit Cancelled(paymentId);
        } else {
            revert("This contract only be cancelled within 3 days of initialing payment");
        }
    }
        ...
        nonReentrant 
    {
        Payment storage payment = payments[paymentId];
        require(
            block.timestamp >= payment.voidDate || payment.status == Status.CANCELLED, 
            "This contract is not reaches the void date or not cancelled by client"
        );
        require(payment.paidAmount > 0, "This payment has not been paid");
        uint256 paidAmount = payment.paidAmount;
        payment.status = Status.CANCELLED;
        payment.paidAmount = 0;
        payable(payments[paymentId].client).transfer(payment.paidAmount);
        emit WithdrawnAmount(paymentId, paidAmount, payment.currency);
    }

    /** 
     *  @notice Client withdraws unused money by other token when the contract reaches the void date
     * 
     *  @dev    Only Client can call this function
     * 
     *          Name                Meaning 
     *  @param  paymentId           ID of payment that client want to withdraw money
     */
    function withdrawAmountToken(uint256 paymentId) 
        external 
        onlyClient(paymentId) 
        nonReentrant 
    {
        ...
        require(
            currentTime >= payment.voidDate || payment.status == Status.CANCELLED, 
            "This contract is not reaches the void date or not cancelled by client"
        );
        uint256 paidAmount = payment.paidAmount;
        if (keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDC")))) {
            usdcToken.transfer(_msgSender(), payment.paidAmount);
            payment.paidAmount = 0;
        } else if (keccak256(abi.encodePacked((payment.currency))) == keccak256(abi.encodePacked(("USDT")))) {
            usdtToken.transfer(_msgSender(), payment.paidAmount);
            payment.paidAmount = 0;
        }
