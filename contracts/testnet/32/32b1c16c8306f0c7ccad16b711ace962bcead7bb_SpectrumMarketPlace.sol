    ...
    Ownable
{
    ...
    uint256 public serviceFee = 0.00001 ether;
    ...
    function cancelOffer(bytes32 _offerId) external nonReentrant {
        require(msg.sender == offeringRegistry[_offerId].offerer, "You are not allowed to cancel the offer");
        payable(offeringRegistry[_offerId].offerer).transfer(offeringRegistry[_offerId].bidPrice);
        delete (offeringRegistry[_offerId]);
        emit OfferingCancelled(_offerId);
    }


    function operatorCancelOffer(bytes32 _offerId) external onlyOperator nonReentrant {
        payable(offeringRegistry[_offerId].offerer).transfer(offeringRegistry[_offerId].bidPrice);
        delete (offeringRegistry[_offerId]);
        emit OfferingCancelled(_offerId);
    }
    ...
    function acceptOffer(bytes32 _offeringId) external nonReentrant {
        CreateOffering storage currentOffer = offeringRegistry[_offeringId];
