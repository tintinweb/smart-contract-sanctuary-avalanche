...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    //     console.log("amount stable coin before------->",stableCoins);
       ...
       uint256 tokens = _getTokenAmount(stableCoins);
        ...
        //console.log("in tokenFallback in NonUS");
    ...
    function _deliverTokens(address beneficiary, uint256 tokenAmount,bytes memory data) internal virtual {
       // console.log("in _deliverTokens in NonUS 207");
...
abstract contract PostGoalCrowdsale is TimedCrowdsale, RefundableCrowdsale {
    ...
    function _processPurchase(
        ...
        if (_balances[beneficiary] == 0) {
            ...
            //console.log("here in postgoal in Non_US");
...
interface ISTO{
   function rate() external view returns (uint256);
    ...
    function STO_UsAddress() external view;
    ...
    PostGoalCrowdsale
{
   ...
   event setNewUSSTOAddress(address sender, address USSTOAddress);
    /**
     * 
    ...
    */
        ...
        override(CappedCrowdsale, TimedCrowdsale, Crowdsale)
    {
        ...
        // ) {
        //     require(
        //        keccak256(abi.encodePacked(whitelist.getUserType(_beneficiary))) == keccak256(abi.encodePacked("USA Premium User")),
        //         //whitelist.getUserType(_beneficiary) == "USA Premium User",
        //         "STO9"
        //     );
        // }
    ...
    function _processPurchase(
        ...
        if (stage == CrowdsaleStage.ICO)
        {
            ...
            console.log("mod _tokenAmount",_tokenAmount);
            console.log("mod--------->",((_tokenAmount / tokenMultiple) % tokenMultiple == 0) );
           // require(((_tokenAmount / tokenMultiple) % tokenMultiple == 0) ,"STO49");
            _deliverTokens(_beneficiary, _tokenAmount, _data);
            uint256 totalTokens = _tokenAmount/(stoUS.rate()/rate());
            console.log("totalTokens in 1 US",totalTokens);
            IERC1400RawERC20(address(stoUS.token())).issue(reserveWalletUS,totalTokens,"0x00");
        }
        else {
            //console.log("here in _processPurchase in PreICO");
             super._processPurchase(_beneficiary, _tokenAmount, _data);
             console.log("**totalSupplyNonUS**",totalSupplyNonUS);
             console.log("**_tokenAmount**",_tokenAmount);
             console.log("**tokenMultiple**",tokenMultiple);
             uint256 totalTokenNonUS = (totalSupplyNonUS + _tokenAmount)/tokenMultiple;
             console.log("totalTokenNonUS------------",totalTokenNonUS);
             console.log("totalSupplyUS------------",totalSupplyUS);
             uint256 totalTokens;
             unchecked {
                 totalTokens = totalTokenNonUS - totalSupplyUS ;
             }
            console.log("totalTokens in NON US --------------",totalTokens);
    ...
    function updateGoal(uint256 newGoal) external returns (uint256) {
        ...
        //console.log("updateGoal in NonUS",newGoal);
        goalPercent = newGoal;
     ...
     */
    function setUSSTOAddress(address UsSTO) public returns(bool success)
    {
        require(!goalReached(), "STO14");
        require(!hasClosed(),"STO6");
