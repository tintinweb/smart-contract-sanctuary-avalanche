...
contract PlayerRewardsContract {
    ...
    function addWhiteList(address user,uint256 amount) public onlyOwner{
        ...
        //starting 8.10 17:30
        lastUnlockTime[user] = 1665208800;
    ...
    function getDaysUnlocked(uint8 daysPast, address _receiver) internal{
        ...
        //tokens for 1 day
		uint256 newTokens = ownedBFG[_receiver] * 69 / 1000000;
    ...
    function Withdraw() public {
        ...
        //3 months cliff
		if(vestingCycles == 0){
			require(block.timestamp > constUnlockTime + 250 minutes , "Too early for unlocking tokens");
			constUnlockTime = constUnlockTime + 250 minutes;
            vestingCycles ++;
            return;
		}
		...
		if(vestingCycles > 0){
            ...
            if(lastUnlockTime[msg.sender] == 1665208800){
                lastUnlockTime[msg.sender] = 1665208800 + 250 minutes;
            }
			require(block.timestamp > lastUnlockTime[msg.sender] + 1 minutes, "Too early for unlock");
			//need to calculate days
			//uint8 daysPast = uint8((block.timestamp - lastUnlockTime) / 60 / 60 / 24);
			uint8 daysPast = uint8((block.timestamp - lastUnlockTime[msg.sender]) / 60);
