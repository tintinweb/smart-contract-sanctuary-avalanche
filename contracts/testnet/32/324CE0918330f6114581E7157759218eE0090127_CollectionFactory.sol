...
import {MintParams, Asset, MintDecodedPayload} from "../structs/erc721/ERC721Structs.sol";
import "../interfaces/IONFT721A.sol";
import "../interfaces/ICollectionsRepository.sol";

contract CollectionFactory is NonblockingLzApp, ReentrancyGuard {
    ...
    modifier whenNotPaused(uint16 _chainId) {
        require(!globalPaused && !pausedChains[_chainId], "paused");
        _;
    }
    ...
    ) public payable whenNotPaused(_params.dstChainId) nonReentrant {
        if (_params.dstChainId == chainId) {
            _collectionsRepository.create(_params, msg.sender);

            return;
        }

        _lzSend(_params.dstChainId, _encodePayload(_params), payable(msg.sender), address(0x0), _adapterParams, msg.value);
    }
    ...
    function _encodePayload(
        ...
        return abi.encode(_params, msg.sender);
    }
    ...
    // uint64 = 8 bytes
    // uint256 = 32 bytes
    function _decodeNonEVMPayload(bytes memory _payload) internal pure returns (CreateParams memory, address) {
        // require(_payload.length == 167, "TokenBridge: invalid payload length");

        PacketType packetType = PacketType(uint8(_payload[0]));
        require(packetType == PacketType.RECEIVE_FROM_APTOS, "!packet type");

        uint16 dstChainId;
        bytes11 name; // TODO: Arbitrary string - we have to know how long in separate param?
        bytes11 uri; // TODO: Possible 2 CID versions length - we have to know how long in separate param?
        uint256 price;
        bytes4 assetName;
        uint256 from;
        uint256 to;
        bytes11 tokensURI; // TODO: Possible 2 CID versions length - we have to know how long in separate param?
        uint256 maxSupply;
        bool isZeroIndexed;
        address creator;
        ...
        assembly {
            dstChainId := mload(add(_payload, 3))
        }

        return (CreateParams(dstChainId, "", "", price, assetName, from, to, "", maxSupply, isZeroIndexed), creator);
    }

    function _decodeEVMPayload(bytes memory _payload) internal pure returns (CreateParams memory params, address creator) {
        return abi.decode(_payload, (CreateParams, address));
    }
}
