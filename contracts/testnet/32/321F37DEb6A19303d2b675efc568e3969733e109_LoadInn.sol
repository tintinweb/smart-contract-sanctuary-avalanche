...
// File: contracts/game/LoadInn.sol
...
contract LoadInn is ReentrancyGuard, Ownable {
    ...
    constructor( address _priceHandler, address _rewardReceiver, address _dcauToken ) {
        PRICE_HANDLER_CONTRACT = _priceHandler;
        REWARD_RECEIVER_CONTRACT = _rewardReceiver;
        DCAU_TOKEN_CONTRACT = _dcauToken;
    }

    function bardSong( uint256 tokenId, string memory songId, bytes memory signature, string memory nonce ) external {
        require(matchAddresSigner(hashTransactionSong(msg.sender, tokenId, songId, nonce), signature), "DIRECT_SONG_DISALLOWED");
        require(tokenId > 0, "MUST_HAVE_QUANTITY");
        ...
        dcauContract.transferFrom(msg.sender, REWARD_RECEIVER_CONTRACT, costForSong);
        rewardReceiver.receiveTokens(DCAU_TOKEN_CONTRACT, costForSong);

        emit BardSong(msg.sender, tokenId, songId);
    }

    function rest( uint256 level, uint256 tokenId, string memory restId, bytes memory signature, string memory nonce ) external {
        require(matchAddresSigner(hashTransactionRest(msg.sender, level, tokenId, restId, nonce), signature), "DIRECT_REST_DISALLOWED");
        require(tokenId > 0, "MUST_HAVE_QUANTITY");
        require(!_usedNonces[nonce], "HASH_USED");
        ...
        dcauContract.transferFrom(msg.sender, REWARD_RECEIVER_CONTRACT, costToRes);
    ...
    function hashTransactionSong(address sender, uint256 tokenId, string memory actionId, string memory nonce) private pure returns(bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(sender, tokenId, actionId, nonce)))
        );
     ...
     // verify whether hash matches against tampering;
    function hashTransactionRest(address sender, uint256 level, uint256 tokenId, string memory actionId, string memory nonce) private pure returns(bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(sender, level, tokenId, actionId, nonce)))
        );
