...
contract VUSDvault_withToken is Ownable {    
    ...
    uint256 public ERC20TokenPrice = 20 * 10 ** 18; // ERC20 Token Price
    ...
    function withdrawVUSD(uint256 _amount) public payable nonReentrant {
        ...
        uint256 ownerFeeAmount = _amount.mul(forERC20Convert).mul(ERC20TokenPrice).div(1000); // 100
        ...
        if (IERC20(VUSD).balanceOf(address(this)).sub(ownerFeeAmount) >= 0) {
            ...
            IERC20(ERC20Token).transfer(msg.sender, amountInERC20Token);
        }
