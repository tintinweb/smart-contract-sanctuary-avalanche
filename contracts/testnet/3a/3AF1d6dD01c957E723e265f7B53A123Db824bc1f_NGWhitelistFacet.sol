...
contract NGWhitelistFacet is WithStorage {
    ...
    // total people on Whitelist - Depricated - can use AdminStats instead
    function getWLCount() public view onlyAdmin returns (uint256) {
        return ws().allowedAccountsArray.length;
    }

    // Disable whitelist - mainally used for testing or if we dont sale all of WL
    function changeWhitelistStatus(bool enabled) public onlyAdmin {
        ws().enabled = enabled;
    }
    
    // Check if person is whitelisted by address
    function isWhitelisted(address _addr) public view onlyAdmin returns (bool) {
        return ws().allowedAccounts[_addr];
    }

    // Check if self is whitelisted
