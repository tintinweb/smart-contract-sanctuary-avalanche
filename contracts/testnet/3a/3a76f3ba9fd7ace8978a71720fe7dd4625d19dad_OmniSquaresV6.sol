...
contract UniversalONFT is ONFT {
    ...
    uint constant public maxTokenPurchase = 3;
    ...
    function mint(uint256 numberOfTokens) external payable {
        require(nextMintId <= maxMintId, "OmniSquares: Max Mint limit reached");
        require(
          numberOfTokens > 0 && numberOfTokens <= maxTokenPurchase,
          "Can mint 3 tokens at a time"
        );
        require(
          msg.value >= tokenPrice.mul(numberOfTokens),
          "Ether value sent is not correct"
        );
        payable(msg.sender).call{value: tokenPrice.mul(numberOfTokens)};
...
contract OmniSquaresV6 is UniversalONFT {
