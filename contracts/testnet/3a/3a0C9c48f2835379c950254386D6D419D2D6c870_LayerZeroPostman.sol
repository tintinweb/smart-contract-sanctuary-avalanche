...
contract LayerZeroPostman is ILayerZeroReceiver, ILayerZeroUserApplicationConfig, IPostman, Ownable {
	...
	function deliverMessage(
		...
		emit MessageOnTheWay(_dstChainId, chains[_dstChainId], _msg, _dstVautAddress, _messageType);
	}
	...
	function lzReceive(
		...
		(Message memory _msg, address _dstVaultAddress, messageType _messageType) = abi.decode(
			_payload,
			(Message, address, messageType)
		);
