...
contract MultiFlexibleStaking is Ownable, Pausable {
    ...
    function stake(
        ...
        if (totalSupply == 0 && uint80(block.timestamp) < periodFinish) {
            maxPotentialDebt = (periodFinish - uint80(block.timestamp)) * uint256(rewardRate);
    ...
    function _claim(uint256 id, uint256 rewardsPenaltyPercentage) internal {
        ...
        if (totalSupply == 0) {
            ...
            cumulativePreviousDebt = reward > cumulativePreviousDebt ? 0 : cumulativePreviousDebt - reward;
        }
    ...
    function getUserInactiveStakesSlice(address user, uint256 startIndex, uint256 length) external view returns(Stake[] memory) {
    ...
    function getTotalRewards() external view returns(uint256) {
        ...
        return ((maxPotentialDebt - cumulativePreviousDebt) * (rewardsDuration - timeLeft) / rewardsDuration) + cumulativePreviousDebt;
    }
}