...
contract nebulaFeeManager is Ownable{
	...
	function INTupdateProtoFutureFees(address _account,uint256 _x) internal {
		...
		if(nebuLib.safeDiv(feePeriod,INTgetProtoDueElapsed(_account,_x))>0){
			INTupdateFutureFees(_account,true,nebuLib.safeDiv(feePeriod,INTgetProtoDueElapsed(_account,_x)).sub(1));
			INTupdateFutureFees(_account,false,bef);
		}	
	}
