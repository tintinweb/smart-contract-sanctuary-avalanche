    ...
    ERC1155URIStorageUpgradeable
{
    ...
    uint256 public totalQuantityMinted;
    ...
    function mintBatch(
        ...
        totalQuantityMinted += tokenIds.length;
    ...
    function mintBatchAndTransfer(
        ...
        for (uint256 i = 0; i < uris.length; i++) {
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _mint(to[i], tokenId, amounts[i], data);
            _setURI(tokenId, uris[i]);
        }
        totalQuantityMinted += uris.length;
    }

    function mintBatchAndTransfer2(
        address[] memory to,
        uint256[][] memory amounts,
        string[][] memory uris,
        bytes memory data
        ...
        for (uint256 i = 0; i < uris.length; i++) {
            for (uint256 j = 0; j < uris[i].length; j++) {
                ...
                tokenIds.push(tokenId);
                _tokenIdCounter.increment();
                _setURI(tokenId, uris[i][j]);
            }
            _mintBatch(to[i], tokenIds, amounts[i], data);
            totalQuantityMinted += tokenIds.length;
            delete tokenIds;
        }
    }
