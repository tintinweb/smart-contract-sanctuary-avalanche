...
contract FarmToken is ERC20("FarmToken", "FTOK"), Ownable {
...
// MasterChef is the master of the farm token. He can make tokens and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number that distribution occurred
    ...
    uint256 public tokensPerBlock = 1 ether;
    // maximum emission rate
    uint256 public MAX_EMISSION_RATE = 5 ether;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 reward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        farmToken.mint(address(this), reward);
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        uint256 pending = 0;
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(
                user.rewardDebt
            );
            if (pending > 0) {
                safeTokenTransfer(msg.sender, pending);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > balance) {
            transferSuccess = farmToken.transfer(_to, balance);
        } else {
    ...
    function setFeeAddress(address _feeAddress) public onlyOwner {
        require(_feeAddress != address(0x0), "!non zero");
        feeAddress = _feeAddress;
    ...
    function setEmissionRate(uint256 _emissionRate) public onlyOwner {
        require(_emissionRate <= MAX_EMISSION_RATE, "!max emission rate");
