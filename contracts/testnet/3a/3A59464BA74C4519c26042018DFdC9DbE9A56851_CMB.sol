...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        onlyValidAddress(client)
    {
        ...
        require(
            expiredDate > block.timestamp + RIGHT_TO_CANCEL, 
        ...
        onlyClient(paymentId)
    {
        require(
            payments[paymentId].status == Status.BO_CONFIRMED, 
            "Services are not provided by business owner"
        );
        ...
        nonReentrant
    {
        ...
        emit OwnerWithdrawn(paymentId, withdrableAmount, payment.paymentToken);
    }

    /**    
     *  @notice Return/Transfer money to client or business based on real life situation owner when payment is expired 
     * 
     *  @dev    Only Owner (CMB) can call this function
     * 
     *          Name                Meaning 
     *  @param  paymentId           ID of payment that client want to withdraw money
     */
    function handleExpiredPayment(uint256 paymentId)
        external
        onlyOwner
        ...
        nonReentrant
    {
        ...
        require(
            ...
            payment.status == Status.PAID ||
            payment.status == Status.CLIENT_CONFIRMED, 
            "Can not handle expired payment"
        );
        if (payment.status == Status.PAYING || payment.status == Status.PAID) {
            _makePayment(payment.client, payment.paymentToken, payment.paidAmount);
            emit ReturnedMoney(paymentId, payment.client, payment.paidAmount);
        } else if (payment.status == Status.CLIENT_CONFIRMED) {
            uint256 excessAmount = payment.paidAmount - payment.amount;
            _makePayment(payment.bo, payment.paymentToken, payment.amount);
            _makePayment(owner(), payment.paymentToken, excessAmount);
            emit ReturnedMoney(paymentId, payment.bo, payment.amount);
        }
    }

    /** 
     *  @notice Owner (CMB) sees something wrong and decide to return money to client or business when payment is expired
     * 
     *  @dev    Only Owner (CMB) can call this function
     * 
     *          Name                Meaning 
        ...
        nonReentrant
    {
        ...
        if (receiver == payment.bo) {
            ...
            _makePayment(payment.bo, payment.paymentToken, payment.amount);
            _makePayment(owner(), payment.paymentToken, excessAmount);
            emit ReturnedMoney(paymentId, receiver, payment.amount);
        } else {
            _makePayment(payment.client, payment.paymentToken, payment.paidAmount);
