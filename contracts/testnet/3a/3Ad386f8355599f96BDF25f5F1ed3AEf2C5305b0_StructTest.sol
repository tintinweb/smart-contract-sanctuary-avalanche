...
import "../interfaces/IYieldFarmNeutralManager.sol";
...
contract StructTest is ERC20, AccessControl, ReentrancyGuard, Pausable {
  ...
  // Conversion factor for tokens with less than 18 decimals
  uint256 public tokenAConversionFactor;
  uint256 public tokenBConversionFactor;
  IChainLinkOracle public priceOracle;
  IVaultConfig public vaultConfig;
  ...
  // uint256 upperDebtRatio;
  // uint256 lowerDebtRatio;
  ...
  constructor (
    ...
    // IVaultConfig _vaultConfig
    VaultConfig memory _vaultConfigs
  ) ERC20(_name, _symbol) {
    tokenA = _tokenA;
    tokenB = _tokenB;
    lpToken = _lpToken;
    // tokenAConversionFactor = _to18ConversionFactor(_tokenA);
    // tokenBConversionFactor = _to18ConversionFactor(_tokenB);

    priceOracle = _priceOracle;
    // vaultConfig = _vaultConfig;

    vaultConfigs = _vaultConfigs;
  ...
  //   (uint256 _tokenAAssetAmt, uint256 _tokenBAssetAmt) = IYieldFarmNeutralManager(manager).assetInfo();

  //   return getTokenAValue(_tokenAAssetAmt) + getTokenBValue(_tokenBAssetAmt);
  // }

  // function getDebtValue() public view returns (uint256, uint256) {
  //   (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt) = IYieldFarmNeutralManager(manager).debtInfo();
  //   return (getTokenAValue(_tokenADebtAmt), getTokenBValue(_tokenBDebtAmt));
  // }
  ...
  //   return (
  //     getTokenAValue(_tokenAAssetAmt) +
  //     getTokenBValue(_tokenBAssetAmt) -
  //     getTokenAValue(_tokenADebtAmt) -
  //     getTokenBValue(_tokenBDebtAmt)
  //   );
  // }

  // function getLpAmt() public view returns (uint256) {
  //   return IYieldFarmNeutralManager(manager).lpTokenAmount();
  // }
  ...
  //   (uint256 _tokenAAmt,) = IYieldFarmNeutralManager(manager).assetInfo();
  //   (uint256 _tokenADebtAmt,) = IYieldFarmNeutralManager(manager).debtInfo();
  ...
  //   return _valueToShares(getTokenBValue(_tokenBAmt), getEquityValue());
  // }
  ...
  //   return (totalSupply() * vaultConfig.managementFeePerSec() / SAFE_MULTIPLIER * _secondsFromLastCollection) / SAFE_MULTIPLIER;
  // }

  // /* ========== MUTATIVE FUNCTIONS ========== */

  // function deposit(uint256 _tokenBAmt, uint256 _minSharesReceive) external collectMgmtFee nonReentrant {
  //   IYieldFarmNeutralManager(manager).compound();

  //   uint256 equityBefore = getEquityValue();

  //   ERC20(tokenB).safeTransferFrom(msg.sender, address(this), _tokenBAmt);
  //   ERC20(tokenB).safeTransfer(manager, _tokenBAmt);

  //   // depositValue expressed in 1e6
  //   uint256 depositValue = _tokenBAmt * priceOracle.consult(tokenB) / SAFE_MULTIPLIER;

  //   // expressed in 1e18
  //   uint256 borrowTokenAAmt = depositValue // 1e6
  //     * ((vaultConfig.getTargetLeverage() - 1e18)/SAFE_MULTIPLIER) // * 2 1e6
  //     * vaultConfig.getTokenADebtRatio() // * .75 1e6 x 1e18
  //     / priceOracle.consult(tokenA) // / price / 1e18 -> 1e6
  //     * (10 ** ERC20(tokenA).decimals()) / 1e6; // multiply by tokenA decimals (18), divide by 1e6 from previous price

  //   uint256 borrowTokenBAmt = depositValue
  //     * ((vaultConfig.getTargetLeverage() - 1e18)/SAFE_MULTIPLIER)
  //     * vaultConfig.getTokenBDebtRatio()
  //     / priceOracle.consult(tokenB)
  //     * (10 ** ERC20(tokenB).decimals()) / 1e6; // multiply by tokenB decimals (1e6), divide by 1e6

  //   IYieldFarmNeutralManager(manager).work(
  ...
  //   _checkAndMint(_tokenBAmt, _minSharesReceive, equityBefore);
  // }

  // function withdraw(uint256 _shareAmt, uint256 _minTokenBReceive) external collectMgmtFee nonReentrant {
  //   IYieldFarmNeutralManager(manager).compound();
  //   uint256 equityBefore = getEquityValue();
  ...
  //   uint256 shareRatio = _shareAmt * SAFE_MULTIPLIER / (totalSupply() + pendingManagementFee());

  //   _burn(msg.sender, _shareAmt);

  //   // do calculations
  //   (uint256 tokenADebtAmt, uint256 tokenBDebtAmt) = IYieldFarmNeutralManager(manager).debtInfo();
  //   uint256 lpAmt = shareRatio * IYieldFarmNeutralManager(manager).lpTokenAmount() / SAFE_MULTIPLIER;
  ...
  //   _checkAndTransfer(_minTokenBReceive, equityBefore);
  // }

  // function compound() external collectMgmtFee {
  //   IYieldFarmNeutralManager(manager).compound();
  ...
  // function getTokenAValue(uint256 _amt) internal view returns (uint256) {
  //  return _amt * tokenAConversionFactor * priceOracle.consult(tokenA) / SAFE_MULTIPLIER;
  // }

  // function getTokenBValue(uint256 _amt) internal view returns (uint256) {
  //  return _amt * tokenBConversionFactor * priceOracle.consult(tokenB) / SAFE_MULTIPLIER;
  // }

  // function _checkAndMint(uint256 _tokenBAmt, uint256 _minSharesReceive, uint256 _equityBefore) internal {
  ...
  // function _checkAndTransfer(uint256 _minTokenBReceive, uint256 _equityBefore) internal {
  //   uint256 _withdrawAmt = ERC20(tokenB).balanceOf(address(this));

  //   if (_withdrawAmt < _minTokenBReceive) {
  //     revert InsufficientTokenAReceived(tokenB, _minTokenBReceive, _withdrawAmt);
  //   }
  ...
  //   _mint(vaultConfig.managementFeeTreasury(), pendingManagementFee());
  ...
  //   require(_decimals <= 18, 'decimals are more than 18');
  ...
  // function updateManager(address _manager) external onlyRole(OWNER_ROLE) {
  //   manager = _manager;
  // }
  ...
  // ) external collectMgmtFee onlyRole(KEEPER_ROLE) {

  //   // TODO add some checks on the arguments passed
  //   IYieldFarmNeutralManager(manager).work(_action, _lpAmt, _borrowTokenAAmt, _borrowTokenBAmt, _repayTokenAAmt, _repayTokenBAmt);
  // }
...
interface IYieldFarmNeutralManager {
