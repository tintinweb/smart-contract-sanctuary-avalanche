...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    function open(
        ...
        if(!hasRole(OPEN_ROLE, sender)) {
            ...
            IBetUpDAOToken(betUpDaoToken).transferFrom(
                sender,
                address(this),
    ...
    function stack(uint256 _pairID, uint16 _resultID, uint256 _amount) external  {
        ...
        players[playerKey].amountWithBonus += IBetUpCore(betUpCore)
                    .calcIncentiveBonus(innerPair.startAt, pair.endStackAt, _amount);
    ...
    function claimCreationRewards(uint256 _pairID) external {
        ...
        uint256 op = optionsVolume[keccak256(abi.encodePacked(_pairID, innerPair.resultID))];
        uint256 fee = (innerPair.total - op) * 10 * pair.creationRatio/1000000 ;
        ...
        if(innerPair.openAmount > 0) IERC20MetadataUpgradeable(betUpDaoToken).safeTransfer(receiver, innerPair.openAmount * 10**18);
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint256 others = (innerPair.total - optionsVolume[keccak256(abi.encodePacked(pairID, innerPair.resultID))]);
    ...
    */
    function _getReward(
        ...
        uint256 totalAmount = innerPair.total;
        ...
        if(innerPair.resultID == INVALID_ID) {
            ...
            if(innerPair.resultID == _resultID) {
                uint256 _profit = (9 * _player.amountWithBonus *(totalAmount - op)) / (10 * op);
                refund = _player.amount +  _profit;
            ...
            block.timestamp > pair.endStackAt && (totalAmount - op == 0)
        )  {
            refund = _player.amount;
        }
