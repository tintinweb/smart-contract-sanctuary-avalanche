...
/// @title VaultDeployer
/// @notice A contract for deploying vault contracts
contract VaultDeployer is Owned(msg.sender), IVaultDeployer {
    using VaultLib for address;

    /// @dev The address of the message router used to share messages between chains and contracts
    address internal immutable router;

    /// @inheritdoc IVaultDeployer
    Params public override params;

    /// @notice Constructs a new VaultDeployer instance
    /// @param _router The address of the router used to trade assets
    ...
    function deploy(address index, address asset) external returns (address vault) {
        params = Params(asset, router, owner);
        vault = address(new Vault{salt: keccak256(abi.encode(index, asset))}());
        delete params;
    }

    /// @inheritdoc IVaultDeployer
    function vaultOf(address index, address asset) external view returns (address) {
        return address(this).vaultOf(index, asset);
    }
}
...
library VaultLib {
    /// @dev keccak256(type(Vault).creationCode)
    bytes32 public constant VAULT_INIT_CODE_HASH = keccak256(type(Vault).creationCode);

    /// @dev Returns encoded `Vault` for given asset, not guaranteed to exist
    /// @param asset Address of asset
    /// @return Address of `Vault`
    function vaultOf(address vaultDeployer, address index, address asset) internal pure returns (address) {
        return Create2.computeAddress(keccak256(abi.encode(index, asset)), VAULT_INIT_CODE_HASH, vaultDeployer);
    }
}
