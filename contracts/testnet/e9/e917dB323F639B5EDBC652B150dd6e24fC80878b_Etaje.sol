...
error Etaje__TransferFailed();

contract Etaje is BaseRelayRecipient {
    string data;
     ...
     *zoe was here 2022/12/06
     **/
    constructor(address _trustedForwarder, address token) {
    ...
    function transferFunds(address receiver, uint256 amount) external
    {
       bool success = s_stakingToken.transfer(receiver, amount);
        if (!success) {
            revert Etaje__TransferFailed();
        }
    }
