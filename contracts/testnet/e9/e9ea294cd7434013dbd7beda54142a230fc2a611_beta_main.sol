...
contract beta_main is ERC20, Ownable, PaymentSplitter {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(address(this),teamPool, nodeAmount.div((10000)/(rewardsFee*100)));
        super._transfer(address(this),rewardsPool, nodeAmount.div((10000)/(treasuryFee*100)));
        super._transfer(address(this),treasury, nodeAmount.div((10000)/(teamPoolFee*100)));
   ...
   function get(address account) public view returns(uint256[3] memory){
	...
	if (tier[2] != 0) {
        ...
        uint get = getEm();
        uint256 feeAmount = rewardAmount.mul((cashoutFee.mul(_perc).div(100)));
