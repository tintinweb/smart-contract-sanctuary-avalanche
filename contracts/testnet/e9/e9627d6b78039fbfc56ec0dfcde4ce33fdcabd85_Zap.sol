    ...
    function zapInToken(
        ...
        if (isLP(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                _swapUSDCToLP(_to, usdcAmount, msg.sender);
            }
        } else {
    ...
    function estimateZapIn(
        ...
        if (isLP(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                uint token0Amt = sellAmount;
                uint token1Amt = _estimateSwap(_from, amount.sub(sellAmount), other);
                ...
                uint256 token1Amt = _estimateSwap(USDC, amount.sub(sellAmount), token1);
                return (token0Amt, token1Amt);
            }
        } else {
            return (0, 0);
