...
contract GeSAct20v1 is Context, ERC20CregistryBase3, ERC20Whitelisting, ERC20Pausable {
    ...
    function _hasRole(uint256 roleTokenId, address account)
        internal
        view
        virtual
        override(ERC20CregistryBase3, PermissionsAware)
        returns (bool)
    {
...
contract GeSActFactory is Context, PermissionsOverwriter, Ownable {
    ...
    function _addToken(address token) internal {
        ...
        emit TokenAdded(_msgSender(), token);
    }

    function addToken(address token) external onlyDeployer {
        _addToken(token);
    }
...
contract PermissionsOverwriter is PermissionsAware, PermissionErrors {
...
contract ERC20CregistryBase3 is ERC20Burnable, PropertiesAware, PermissionsOverwriter, ISPVContract {
    ...
    function _hasRole(uint256 roleTokenId, address account)
        ...
        view
        virtual
        override(PermissionsOverwriter, PermissionsAware)
        returns (bool)
    {
        return super._hasRole(roleTokenId, account);
    }
