...
contract Expedition is Ownable {
  ...
  function reinforceAttack(uint256 _id, uint256 _tokenId) external {
    ...
    // create a ref
    Campaign storage campaign = campaigns[_id];
    require(nft.getLevel(_tokenId) >= campaign.tier * 10, "invalid nft level");
  ...
  function isMakerWinner (uint256 _id) private view returns (bool) {
    Campaign storage campaign = campaigns[_id];
  ...
  function finishCampaign(uint256 _id) external {
    ...
    bool isDefenseWinner = isMakerWinner(_id);
    uint256 aveLevel = 0;
    if (isDefenseWinner) {
    ...
    if (isDefenseWinner) {
      ...
      honToken.safeTransferFrom(address(this), winner, honFee[aveLevel] * 75 / 100);
      hrmToken.safeTransferFrom(address(this), winner, hrmFee[aveLevel] * 75 / 100);
      honToken.safeTransferFrom(address(this), loser, honFee[aveLevel] * 25 / 100);
      hrmToken.safeTransferFrom(address(this), loser, hrmFee[aveLevel] * 25 / 100);
    }

    for (uint256 i = 0; i < campaign.attackNFTs.length; i++) {
      nft.transferFrom(address(this), campaign.maker, campaign.attackNFTs[i]);
    }
    for (uint256 i = 0; i < campaign.defenseNFTs.length; i++) {
      nft.transferFrom(address(this), campaign.ambusher, campaign.defenseNFTs[i]);
    }
  }
  ...
  function createSingleCampaign(uint256 _tier) internal {
    ...
    campaigns.push(
      Campaign({
        startTimestamp: block.timestamp, tier: _tier, maker: address(0), ambusher: address(0), area: random(3, 0),
