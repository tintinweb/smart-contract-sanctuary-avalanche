...
contract FraudsterVsCommissioner is IFraudsterVsCommissioner, ERC721Enumerable, Ownable, Pauseable {
    ...
    mapping(uint256 => uint256) public commissionerIds;
    mapping(uint256 => uint256) public fraudsterIds;
    ...
    // reference to the Bank for choosing random Commissioner thieves
    IBank public bank;
   ...
   */
    ...
    function getClassId(uint256 tokenId) external view override returns(uint256){
        if (tokenTraits[tokenId].isFraudster) {
            return fraudsterIds[tokenId];
        } else {
            return commissionerIds[tokenId];
        }
    }
   ...
   */
     function generate(uint256 tokenId, uint256 seed)
        internal
        returns (FraudsterCommissioner memory t)
    {
        t = selectTraits(seed);
