...
contract Chats is Ownable, ReentrancyGuard {
    ...
    function finishChat(bytes32 _id) external nonReentrant {
        ...
        uint[] memory unclaimedFeeResult = getUnclaimedFee(chat.id);
        uint totalFee = unclaimedFeeResult[0];
        uint duration = unclaimedFeeResult[1];
    ...
    function getUnclaimedFee(bytes32 _id) public view returns (uint[] memory) {
        Chat storage chat = chatsMapping[_id];
        uint start;
        uint end;
        ...
        uint[] memory unclaimedFeeResult = new uint[](2);
        unclaimedFeeResult[0] = chat.fee * (end - start) / 3600;
        unclaimedFeeResult[1] = end - start;
        return unclaimedFeeResult;
    }
}
