...
contract ExchangeBank is IExchange, ReentrancyGuard {
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        ///@dev Transfer Fee to Tokenism address fee collection address
        require(
        ...
        if(remainingLoanAmount <= buyerPay){
          ...
          console.log("balace of pledged tokens ====>",remainingPledgedToken );
        if(remainingPledgedToken > 0){
         ...
         console.log("balace of pledged tokens after====>",remainingPledgedToken );
         ...
         console.log("balace of pledged tokens 2nd====>",remainingPledgedToken );
        }

        // ///@dev transfer remaining pledged token after loan payment to borrower;
        //   if(remainingPledgedToken > 0  && remainingLoanAmount == 0) {
        //     token.transferFrom(seller, borrower, remainingPledgedToken);
        //   }
    ...
    function acceptCounter(address _buyerWallet) nonReentrant public {
        ...
        if(remainingLoanAmount < totalAmount){
            ...
            console.log("pledged token ===>", remainingPledgedToken);
        if(remainingPledgedToken > 0){
            uint256 pedgeTokenBalance = remainingPledgedToken;
            remainingPledgedToken =  0;
            token.transferFrom(seller, borrower, pedgeTokenBalance);
            }
         console.log("pledged token after ===>", remainingPledgedToken);
        }
        else if(remainingLoanAmount >= totalAmount){
            remainingLoanAmount -= totalAmount;
            stableCoin.transfer(seller, totalAmount);
            
            console.log("pledged token 2nd ===>", remainingPledgedToken);
        }
    ...
    function setPledgeTokenSaleLimit() public   {
