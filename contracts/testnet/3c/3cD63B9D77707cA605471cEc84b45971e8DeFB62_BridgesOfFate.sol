 ...
 * _calculateBuyBackIn (in test process)
 * Only first jumped pionner in test process
 * Under test
 * Game 5
 * Noted
...
contract BridgesOfFate is OwnableUpgradeable {
    ...
    uint256 private constant TURN_PERIOD = 600;  
    ...
    mapping(uint256 => uint256) private pioneerPushBackBankingGameMap; //private
    ...
    function _pushBackBankingGameEndRules() internal view returns (bool) {
        if((pioneerPushBackBankingGameMap[gameStatusInitialized[_latestGameId].stageNumber] > 0) && 
        (pioneerPushBackBankingGameMap[gameStatusInitialized[_latestGameId].stageNumber]  < _pioneerPushBackBankingNumberProbility) && 
        ...
        {
            require(_isPioneerPushBackEnd == true ,"Push Back Bank Game Ended.");
        }
        return true;
    }
    ...
    function _calculateBuyBackIn() public view returns (uint256) {
        GameStatus memory _gameStatus = gameStatusInitialized[_latestGameId];
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public {
        ...
        if (_member.userWalletAddress != address(0)) {
            ...
            require(_dayDifferance(_member.lastJumpTime,_gameStatus.startAt) + 1 < _dayDifferance(block.timestamp, _gameStatus.startAt),"Buyback is useful only one time in 24 hours");
                ...
                // _member.isPushBack =  true; // in restart case
        } 
    ...
    function switchSide(bytes32 playerId) external  GameEndRules  PushBackBankingGameEndRules{
        ...
        // require(pioneerPushBackBankingGameMap[gameStatusInitialized[_latestGameId].stageNumber] < _pioneerPushBackBankingNumberProbility, "Reached maximum");
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules PushBackBankingGameEndRules{
        ...
        if (_randomNumber <= 0) {
            ...
            // uint8[2] memory _packedNumber = generateRandomNumbersForGame(2);
            // gameRandomNumber[_currentGlobalStage] = _packedNumber[0];
            /*
            * Test
            * In progress
            */
            ...
            */
            ...
            _seed = (_seed - ((_seed / 100) * 100));
            pioneerPushBackBankingGameMap[_currentGlobalStage] = _seed;
            // ------ END -----
            _pioneerPushBackBankingNumberProbility =  _pioneerPushBackBankingNumberProbility + 1;
            //only first jumped pionner
    ...
    function restartGame(uint256 _startAT) public onlyOwner {
        ...
        //Restart Game Again NEW CHEC K THE 
