...
/*
This Contract is for simulations on DEVNET!
*/
contract LottoCatsTestingSim is
    ERC721Enumerable,
    Ownable,
    Multisig(msg.sender),
    ERC2981PerTokenRoyalties
{
    ...
    uint256 public lotteryIntervalDays = 60;
    uint256 public epochDay = 1;
    ...
    function selectAWinner(
        ...
        if ((it < 5) && alreadyWon) {
            ...
            for (uint256 i = 0; i < maxSupply; i++) {
                address winnerAddressIt = ERC721.ownerOf(i + 1);
                if (_winners[winnerAddressIt] != lotteryDate) {
                    win.date = lotteryDate;
                    win.winner = winnerAddressIt;
                    win.tokenId = i;
                    winnerLog[drawNumber].push(win);

                    _winners[winnerAddressIt] = lotteryDate;
                    return;
                }
            }
            return;
...
contract Multisig is Ownable {
    ...
    function signTx() public {
        (bool isContained, ) = containsSignatory(msg.sender);
        require(isContained, "ERROR: You are not a signatory");
    ...
    function containsSignatory(address signatorAddress)
        public
        view
        returns (bool, uint256)
    {
        for (uint256 i = 0; i < _signatoryAddresses.length; i++) {
            if (_signatoryAddresses[i] == signatorAddress) {
                return (true, i);
            }
        }
        return (false, 0);
    ...
    function addSignatory(address newSignatorAddress) public onlyOwner {
        (bool isContained, ) = containsSignatory(newSignatorAddress);
        require(!isContained, "ERROR: address must not already be a signator");
        _signatoryAddresses.push(newSignatorAddress);
    }

    function removeSignatory(address signatorToRemove) public onlyOwner {
        require(
            _signatoryAddresses.length > 1,
            "ERROR: There must at least one removable signatory"
        );
        require(
            signatorToRemove != _primarySignatory,
            "ERROR: Cannot remove owner"
        );
        (bool isContained, uint256 at) = containsSignatory(signatorToRemove);
        require(isContained, "ERROR: address must be signator");
        _signatoryAddresses[at] = _signatoryAddresses[
            _signatoryAddresses.length - 1
        ];
        _signatoryAddresses.pop();
    }
