...
contract NFTShipper is RestrictedExecutable, INFTShipper, TokenIdentifier {
    using Tokens for Tokens.TokenType;
    ...
    function transferNFT(address collection, address from,
        ...
        Tokens.TokenType tokenType = getTokenType(collection);
        if (tokenType == Tokens.TokenType.ERC721) {
            transferERC721(collection, from, to, tokenId, amount);
        } else if (tokenType == Tokens.TokenType.ERC1155) {
            transferERC721(collection, from, to, tokenId, amount);
        } else {
    ...
    function transferERC721(address collection, address from,
        address to, uint256 tokenId, uint256 amount) internal {
        IERC721(collection).safeTransferFrom(from, to, tokenId);
    }

    function transferERC1155(address collection, address from,
        address to, uint256 tokenId, uint256 amount) internal {
...
contract TokenIdentifier is ITokenIdentifier, IERC165 {
    ...
    function getTokenType(address nftAddress) public view returns (Tokens.TokenType) {
        if (isERC721(nftAddress)) {
            return Tokens.TokenType.ERC721;
        } else if (isERC1155(nftAddress)) {
            return Tokens.TokenType.ERC1155;
        }
        return Tokens.TokenType.UNSUPPORTED;
    }
