...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    // whitelist addresses
    mapping(address => bool) _globalWhitelist;
    mapping(address => bool) _transferWhitelist;
    mapping(address => bool) _CRWhitelist;
    // fee supervisor
    ...
    // set minTransferAmount
    event setMinTransferAmountEvent(
        address indexed feeSupervisor,
        uint256 minTransferAmount,
        uint256 indexed timestamp
    );

    // set minCreationAmount
    event setMinCreationAmountEvent(
        address indexed feeSupervisor,
        uint256 minCreationAmount,
        uint256 indexed timestamp
    );

    // set minRedemptionAmount
    event setMinRedemptionAmountEvent(
        address indexed feeSupervisor,
    ...
    function transfer(address to_, uint256 amount_)
        ...
        returns (bool)
    {
        // require amount > _minTransferAmount
        require(
            amount_ > _minTransferAmount,
            'Amount should be greater than minimum amount to transfer!'
        );
        ...
        if (_globalWhitelist[msg.sender] || _transferWhitelist[msg.sender]) {
            // transfer amount from sender to to address
            _transfer(owner_, to_, amount_);
        } else {
            uint256 tranferFee = (amount_ * _transferFee) / (10**_feeDecimals);
            // transfer fee to feeRecipient
            _transfer(owner_, _feeRecipient, tranferFee);
            // transfer amount from sender to to address
            _transfer(owner_, to_, amount_ - tranferFee);
        }
    ...
    function transferFrom(
        ...
        if (_globalWhitelist[from_] || _transferWhitelist[from_]) {
            _transfer(from_, to_, amount_);
        } else {
            uint256 tranferFee = (amount_ * _transferFee) / (10**_feeDecimals);
            // transfer fee to feeRecipient
            _transfer(from_, _feeRecipient, tranferFee);
            _transfer(from_, to_, amount_ - tranferFee);
        }
        return true;
    }
    ...
    // set the minimum tranfer amount
    function setMinTransferAmount(uint256 minTransferAmount_)
        public
        onlyFeeSupervisor
        returns (bool)
    {
        require(
            minTransferAmount_ > 0,
            'Minimum Transfer Amount should be greater than zero.'
        );
        _minTransferAmount = minTransferAmount_;
        emit setMinTransferAmountEvent(
            msg.sender,
            minTransferAmount_,
            block.timestamp
        );
    ...
    // set the minimum creation amount
    function setMinCreationAmount(uint256 minCreationAmount_)
        public
        onlyFeeSupervisor
        returns (bool)
    {
        require(
            minCreationAmount_ > 0,
            'Minimum Creation Amount should be greater than zero.'
        );
        _minCreationAmount = minCreationAmount_;
        emit setMinCreationAmountEvent(
            msg.sender,
            minCreationAmount_,
            block.timestamp
        );
    ...
    // set the minimum redemption amount
    function setMinRedemptionAmount(uint256 minRedemptionAmount_)
        public
        onlyFeeSupervisor
        returns (bool)
    {
        require(
            minRedemptionAmount_ > 0,
            'Minimum Redepmtion Amount should be greater than zero.'
        );
        _minRedemptionAmount = minRedemptionAmount_;
        emit setMinRedemptionAmountEvent(
            msg.sender,
            minRedemptionAmount_,
            block.timestamp
        );
        return true;
    }
    ...
    function setTransferFee(uint256 transferFee_)
        public
        onlyFeeSupervisor
        returns (bool)
    {
        require(transferFee_ != 0, 'Transfer Fee can not be zero');
        _transferFee = transferFee_;
        emit setTransferFeeEvent(msg.sender, transferFee_, block.timestamp);
        return true;
    }
    ...
    function setCreationFee(uint256 creationFee_)
        ...
        returns (bool)
    {
        require(creationFee_ != 0, 'Creation Fee can not be zero');
        _creationFee = creationFee_;
        emit setCreationFeeEvent(msg.sender, creationFee_, block.timestamp);
        return true;
    }

    // set the Redemption Fee
    function setRedemptionFee(uint256 redemptionFee_)
        public
        onlyFeeSupervisor
        returns (bool)
    {
        require(redemptionFee_ != 0, 'Redemption Fee can not be zero');
        _redemptionFee = redemptionFee_;
        emit setRedemptionFeeEvent(msg.sender, redemptionFee_, block.timestamp);
        return true;
    }
    ...
    function increaseTokenSupply(
        ...
        require(
            ...
            'Amount should be greater than minimum amount to create.'
        );
        if (
            keccak256(abi.encodePacked((creationType))) ==
            keccak256(abi.encodePacked(('cash')))
        ) {
            _mint(toAddress, amount_);
        ...
        if (
            ...
            if (_globalWhitelist[toAddress] || _CRWhitelist[toAddress]) {
                _mint(toAddress, amount_);
                emit increaseTokenSupplyEvent(
                    msg.sender,
                    toAddress,
                ...
                uint256 mintFee = (amount_ * _creationFee) /
                    (10**(2 + _feeDecimals));
                _mint(_feeRecipient, mintFee);
                _mint(toAddress, amount_ - mintFee);
                emit increaseTokenSupplyEvent(
                    msg.sender,
                    toAddress,
                    mintFee,
        ...
        returns (bool)
    {
        require(
            ...
            'Amount should be greater than minimum amount to burn.'
        );
        require(
            amount_ <= _totalSupply,
            'Burn amount is greater than total supply'
        );
        if (_globalWhitelist[msg.sender] || _CRWhitelist[msg.sender]) {
            _burn(msg.sender, amount_);
            emit decreaseTokenSupplyEvent(msg.sender, amount_, block.timestamp);
        } else {
            uint256 redemptionFee = (amount_ * _redemptionFee) /
                (10**(2 + _feeDecimals));
            _transfer(msg.sender, _feeRecipient, redemptionFee);
            _burn(msg.sender, amount_ - redemptionFee);
            emit decreaseTokenSupplyEvent(
                msg.sender,
                amount_ - redemptionFee,
                block.timestamp
            );
        }
        return true;
    ...
    {
        _globalWhitelist[account_] = true;
    ...
    function appendToTransferWhitelist(address account_)
        ...
        notNullAddress(account_)
    {
        _transferWhitelist[account_] = true;
    ...
    function removeFromWhitelist(address account_)
        ...
        notNullAddress(account_)
    {
        _globalWhitelist[account_] = false;
    ...
    // remove account from tranfer whitelist
    function removeFromTranferWhitelist(address account_)
        public
        onlyWhitelistSupervisor
        notNullAddress(account_)
    {
        _transferWhitelist[account_] = false;
