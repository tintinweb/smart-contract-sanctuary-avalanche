...
interface INodeManager {
    ...
    function compileVars(address _account,uint256[3] memory tier) external returns (uint);
    ...
    function cashoutNodeReward(address _account, uint i) external;
...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function check_compile() public returns (uint){
    	    address sender = msg.sender;
            uint256[3] memory tier = [uint256(0),uint256(0),uint256(0)];
            uint nodeNum = nodeManager.compileVars(sender,tier);
            return nodeNum;
           }
