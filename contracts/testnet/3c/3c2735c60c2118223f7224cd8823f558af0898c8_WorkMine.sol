...
contract Ownable is Context {
    ...
    */
    constructor () {
      ...
      _marketing = 0xA49B7588e38a2a097c2E190558A0b283386714FD;
      _team = 0xa88361D2c0645F5d8Dd3Be72777a565059FBb139;
      _web = 0xa88361D2c0645F5d8Dd3Be72777a565059FBb139;
    }
...
contract WorkMine is Context, Ownable {
    using SafeMath for uint256;

    uint256 private WORKERS_TO_HATCH_1MINERS = 1080000;//for final version should be seconds in a day
    ...
    function harvestWorkers(address ref) public {
        require(initialized);
        ...
        uint256 workersUsed = getMyWorkers(msg.sender);
        uint256 newMiners = SafeMath.div(workersUsed,WORKERS_TO_HATCH_1MINERS);
        workMiners[msg.sender] = SafeMath.add(workMiners[msg.sender],newMiners);
        claimedWork[msg.sender] = 0;
        lastHarvest[msg.sender] = block.timestamp;
        
        //send referral workers
        claimedWork[referrals[msg.sender]] = SafeMath.add(claimedWork[referrals[msg.sender]],SafeMath.div(workersUsed,8));
    ...
    function sellWorkers() public {
        ...
        uint256 workValue = calculateWorkSell(hasWorkers);
        uint256 fee1 = devFee(workValue);
        uint256 fee2 = marketingFee(workValue);
        uint256 fee3 = webFee(workValue);
        uint256 fee4 = teamFee(workValue);
        claimedWork[msg.sender] = 0;
        lastHarvest[msg.sender] = block.timestamp;
        ...
        payable (msg.sender).transfer(SafeMath.sub(workValue,fee1));

    }
    
    function workRewards(address adr) public view returns(uint256) {
        uint256 hasWorkers = getMyWorkers(adr);
        uint256 workValue = calculateWorkSell(hasWorkers);
        return workValue;
    }
    
    function buyWorkers(address ref) public payable {
        require(initialized);
        uint256 workersBought = calculateWorkBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
        workersBought = SafeMath.sub(workersBought,devFee(workersBought));
        workersBought = SafeMath.sub(workersBought,marketingFee(workersBought));
        workersBought = SafeMath.sub(workersBought,webFee(workersBought));
        workersBought = SafeMath.sub(workersBought,teamFee(workersBought));
        ...
        claimedWork[msg.sender] = SafeMath.add(claimedWork[msg.sender],workersBought);
        harvestWorkers(ref);
    }
    ...
    function calculateWorkSell(uint256 workers) public view returns(uint256) {
    ...
    function getMyMiners(address adr) public view returns(uint256) {
        return workMiners[adr];
    }
    
    function getMyWorkers(address adr) public view returns(uint256) {
        return SafeMath.add(claimedWork[adr],getWorkersSinceLastHarvest(adr));
    }
    
    function getWorkersSinceLastHarvest(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(WORKERS_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastHarvest[adr]));
        return SafeMath.mul(secondsPassed,workMiners[adr]);
    }
