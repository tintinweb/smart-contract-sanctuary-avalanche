...
contract DAOfundMultisig {
    ...
    constructor(address projectOwner_, address teamMember_, address daoMember_) {
            ...
            founder[projectOwner] = true;
            approver[teamMember] = true;
            approver[daoMember] = true;
    }
	...
	Info private info;
	event Approval(address indexed owner, address indexed projectOwner, uint256 tokens);
	...
	function approve(uint256 _amount, IERC20 token_, address _receiver) external returns (bool) {
        ...
        require(approver[msg.sender], "You are not a approver");	   
		info.users[msg.sender].allowance[projectOwner] = _amount;
        tokenToWithdraw = token_;
        receiver = _receiver;
		emit Approval(msg.sender, projectOwner, _amount);
		return true;
	}
    ...
    function setNewDaoMember(address daoMember_) external returns (bool) {
        require(founder[msg.sender], "You are not the daoMember"); 
        daoMember = daoMember_;
        return true;
	}

    function setNewTeamMember(address teamMember_) external returns (bool) {
        require(founder[msg.sender], "You are not the teamMember"); 
        teamMember = teamMember_;
        return true;
	}
	...
	function transferTokens(uint256 _tokens) external returns (bool) {
		...
		info.users[teamMember].allowance[msg.sender] -= _tokens;
		info.users[daoMember].allowance[msg.sender] -= _tokens;
    ...
    function resetToken() external returns (bool) {
        require(founder[msg.sender] || approver[msg.sender], "You are neither the founder or approver");  
        tokenToWithdraw = deadAddressToken;
        return true;
    }
