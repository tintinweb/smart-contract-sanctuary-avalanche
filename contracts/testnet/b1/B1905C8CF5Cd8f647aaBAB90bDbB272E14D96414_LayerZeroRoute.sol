    ...
    IRoute
{
    ...
    ) external onlyLZ() onlySrc(__translate(_srcChainId), _fromAddress) {
        middleLayer.mreceive(
            __translate(_srcChainId),
            _payload
        );
    }
...
abstract contract LayerZeroAdmin is LayerZeroModifiers {
    function addSrc(uint256 srcChain, address _newSrcAddr) public onlyOwner() {
