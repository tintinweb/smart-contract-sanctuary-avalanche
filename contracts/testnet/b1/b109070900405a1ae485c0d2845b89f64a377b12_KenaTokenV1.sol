...
abstract contract Manageable is Initializable {
 ...
 * Peer To Peer Network Fee     3%
 * On-Ramp Network Fee (buy)    5%
 * Off-Ramp Network Fee (sale)  7%
 * Off-Ramp Burn (sale)         3%
 * Transaction Cap              0.5% of total supply
 * Wallet Cap                   4% of total supply
 * Anti-Whale Sale trigger      0.10% of total supply
 * Anti-Whale Sale Penalty      5%
 * Bull-Sell Latency            1 hour
 * Sell-Spread Latency          Start at 15 mins and doubles
    ...
    UUPSUpgradeable
{
    ...
    string internal constant SYMBOL = "KENA.57"; // CHANGE ME
    ...
    // --------------------- Economic Protection ------------------- //
    // 3% RFI as Transaction Fee (For peer-to-peer)
    uint256 internal constant PEER_TO_PEER_NETWORK_FEE = 30;
    // 5% RFI as on-ramp Fee
    uint256 internal constant ON_RAMP_NETWORK_FEE = 50;
    // 7% RFI as off-Ramp Fee
    uint256 internal constant OFF_RAMP_NETWORK_FEE = 70;
    // 3% off-ramp Burn as Inflation Protection
    uint256 internal constant BURN_FEE = 30;
    // Anti-Whale penalty for large dumps additonal 5% (total 15% off-ramp Fee)
    ...
    // Anti-Whale Penalty sale trigger above 0.1% of total supply
     ...
     * Probability of finding the key for this is ridiculuslly infinitesimal.
     * Also, even if that probability materializes, this address is locked completely
     * So the key holder CANNOT operate this account for transfers or allowance.
     * This is a completely safe Vanity burn that burns all coins for eternity.
     */
...
abstract contract AntiScam is Manageable {
    ...
    modifier whenTransferEscrowCriteriaMet(address sender, address recipient) {
        require(block.timestamp > _allowancesCoolDownPeriod[sender][recipient], "AntiScam: Escrow period not yet expired.");
        _;
    }
...
// File imports/KenaNetworkBase.sol
...
/**
 * @dev KenaNetworkBase is the main template.
 * This is used for Fee redistribution to the community.
 * Used to enable Scam and Economic Protection
 * Inherits the Kena Tokenomics
 */

abstract contract KenaNetworkBase is
    KenaTokenomics,
    Presaleable,
    AntiScam
{
    ...
    function initialize() virtual override (KenaTokenomics, Manageable) public initializer {
        super.initialize();
    ...
    function _getTransactionType(address sender, address recipient, uint256 amount)
        internal
        view
        returns (bool , bool , bool , bool)
    {
        ...
        if(isLiquidityPool[sender]) { // Check to see if this is a Buy
            _isBuy = true;
            _isPeerToPeer = false;
            _isSale = false;
        } else if(isLiquidityPool[recipient]) { // Check to see if this is a Sell
            _isSale = true;
            _isBuy = false;
            _isPeerToPeer = false;
    ...
    //  */
    // function reflectionFromToken(uint256 tAmount, bool deductTransferFee, bool _isWhale)
    //     external
    //     view
    //     returns (uint256)
    // {
    ...
    function _approve(
        ...
        if (spender != owner() && spender != manager()) {
            ...
            _allowancesCoolDownPeriod[accountHolder][spender] = block.timestamp;
        }
     ...
     */
    function _getSumOfFees(address sender, address recipient, uint256 amount) internal view virtual returns (uint256);
     ...
     */
    function _takeTransactionFees(address sender, address recipient, uint256 amount, uint256 currentRate)
        internal
        virtual;
}
...
abstract contract GrantorRole is Initializable, OwnableUpgradeable {
    ...
    modifier onlyGrantorOrSelf(address account) {
        require(isGrantor(_msgSender()) || _msgSender() == account, "onlyGrantorOrSelf");
        _;
    }
...
abstract contract EconomicProtection is Manageable {
    ...
    function isLiquidityPoolAddress(address account)
        internal
        virtual
        returns (bool);
    ...
    modifier whenEconomicProtectionCriteriaMet(
        ...
        if (sender == manager() || recipient == manager()) {
            // DO NOT apply buy/sell restriction criteria to owner.
        } else {
            if (isLiquidityPoolAddress(sender)) {
            ...
            } else if (isLiquidityPoolAddress(recipient)) {
                ...
                if (
                    ...
                    if (transactionCoolDownPeriod[sender][
                        ...
                        (
                            ...
                            uint256 minute,
                            uint256 second
                        ) = timeLeft(transactionCoolDownPeriod[sender][
                            TransactionType.SELL
 ...
 *     for value, such as with tokens that have been purchased in an IDO.
 ...
 */
abstract contract ERC20Vestable is
    EconomicProtection,
    KenaNetworkBase,
    GrantorRole,
    IERC20Vestable
{
    ...
    function initialize() virtual override (KenaNetworkBase, GrantorRole, Manageable) public initializer {
        super.initialize();
    }
    ...
    modifier whenTokenomicsCriteriaMet(
        ...
        require(
            sender != address(0) &&
                recipient != address(0) &&
                sender != address(BURN_ADDRESS),
            "Transfer to/from the zero/burn address"
        );
        ...
        if (!isInPresale) {
            if (
                ...
                revert("Tokens exceeding 0.25% of total Kena supply.");
            }
            if (
        ...
        returns (bool ok)
    {
        ...
        require(
            _msgSender() == owner() || _msgSender() == grant.grantor,
            "not allowed"
        );
    ...
    function transferOwnership(address newOwner)
        public
        override (OwnableUpgradeable, GrantorRole)
        onlyOwner
    {
        super.transferOwnership(newOwner);
    }
    ...
    function renounceOwnership()
        public
        view
        override(GrantorRole, OwnableUpgradeable)
        onlyOwner
    {
        require(false, "forbidden");
    }
...
abstract contract KenaTokenGrant is ERC20Vestable {
    ...
    returns (bool ok)
    {
        ...
        return _grantVestingTokens(beneficiary, totalAmount, vestingAmount, startDay, _msgSender(), _msgSender());
    }
...
contract KenaTokenV1 is 
    ...
    function _takeTransactionFees(
        ...
        for (uint256 index = 0; index < feesCount; index++) {
            (FeeType name, uint256 value, address feeRecipient, ) = _getFee(
                index
            );
