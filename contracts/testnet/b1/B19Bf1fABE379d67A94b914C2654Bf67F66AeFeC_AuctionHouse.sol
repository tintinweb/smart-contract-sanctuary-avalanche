...
contract AuctionRoom {
    ...
    function start() public {
        ...
        require(isApproved(), "Room contract not approved");
        started = true;
...
contract AuctionHouse is KeeperCompatibleInterface {
    ...
    function checkUpkeep(bytes calldata checkData) external view override 
        ...
        for (uint i = 0; i < rooms.length; i++) {
            ...
            if (!currentRoom.started() && currentRoom.startTime() <= block.timestamp + 60 seconds && currentRoom.isApproved()) {
                countToBeStarted += 1;
            } else if (!currentRoom.ended() && block.timestamp >= currentRoom.endTime() + 60 seconds && currentRoom.isApproved()) {
                countToBeEnded += 1;
            }
        }
