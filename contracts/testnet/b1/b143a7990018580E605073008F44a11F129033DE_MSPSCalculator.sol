...
contract MSPSCalculator is Ownable {
  ...
  function calculateRewardsMeta(address from, uint[] memory nfts) external view returns (uint256) {
    uint256 total = 0;
  ...
  function existsInArray(uint[] memory _array, uint num) public view returns (bool) {
...
function valueInArray(uint[] memory _array, uint[] memory values, uint num) public view returns (uint) {
  for (uint i = 0; i < _array.length; i++) {
      if (_array[i] == num) {
          return values[i];
      }
...
function indexInArray(uint[] memory _array, uint num) public view returns (uint) {
  for (uint i = 0; i < _array.length; i++) {
      if (_array[i] == num) {
          return i;
      }
...
function nextIndexArray(uint[] memory _array) public view returns (uint) {
  for (uint i = 0; i < _array.length; i++) {
      if (_array[i] == 0) {
          return i;
      }
