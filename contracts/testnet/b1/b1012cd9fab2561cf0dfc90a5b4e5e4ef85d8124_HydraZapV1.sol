...
// File: contracts/interfaces/IHydraRouter01.sol
...
// File: contracts/interfaces/IHydraRouter02.sol
...
// File: contracts/interfaces/IHydraPair.sol
...
// File: contracts/HydraZapV1.sol

pragma solidity ^0.8.4;

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by HydraSwap teams.
 */
contract HydraZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped AVAX (WAVAX)
    IWETH public WAVAX;

    // HydraRouter interface
    IHydraRouter02 public hydraRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from HydraSwap)
    ...
    // Address HydraRouter
    address private hydraRouterAddress;
    ...
    {
        address token0 = IHydraPair(_lpToken).token0();
        address token1 = IHydraPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IHydraPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = hydraRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = hydraRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IHydraPair(_lpToken).token0() || _token0ToZap == IHydraPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IHydraPair(_lpToken).token0() || _token1ToZap == IHydraPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IHydraPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IHydraPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IHydraPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = hydraRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = hydraRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IHydraPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "HydraRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "HydraRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        hydraRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), hydraRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(hydraRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = hydraRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = hydraRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = hydraRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = hydraRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = hydraRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
