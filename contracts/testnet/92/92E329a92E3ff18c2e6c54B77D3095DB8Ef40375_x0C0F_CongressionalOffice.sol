...
// File: contracts/SyntheticNFT.sol
...
/**
 * https://github.com/shadowkite/council-of-frogs/blob/master/contracts/CongressionalOffice.sol
 */
contract x0C0F_CongressionalOffice is
    ERC721Enumerable,
    Ownable,
    NonblockingReceiver
{
    ...
    /**
     * We can use same bridger address for any chain
     * Will cause invalid signer if signiture by bridger for other chain
     */
    ...
    mapping(bytes32 => mapping(uint8 => bool)) public handledMessages;
    ...
    /* BCH Bridge */
    function projectTraverseChains(uint16 _target, uint256[] calldata _tokenId)
        public
    {
        require(
            projectTraverseEnabled != false,
            'Project traverse route disabled'
        );

        uint256 tokenIdLength = _tokenId.length;
        for (uint256 i; i < tokenIdLength; i++) {
            require(msg.sender == ownerOf(_tokenId[i]), 'Not owner');
            _burn(_tokenId[i]);
            emit BridgeRequest(msg.sender, _target, _tokenId[i]);
        }
    }

    function projectReceive(
        bytes32 _msgId,
        uint8 index, // if we sepearate input tx to multiple
        address _receiver,
        uint256[] calldata _tokenId,
        bytes calldata _signiture
    ) public {
        require(!handledMessages[_msgId][index], 'Already handled');

        require(
            getSigner(
                // prettier-ignore
                keccak256(abi.encodePacked(_msgId, index, chainId, _receiver, _tokenId)),
                _signiture
            ) == bridger,
            'Invalid signature'
        );
        ...
        uint256 tokenIdLength = _tokenId.length;
        for (uint256 i; i < tokenIdLength; i++) {
            _mint(_receiver, _tokenId[i]);
        }

        handledMessages[_msgId][index] = true;
    }

    /* Layer Zero */
    function traverseChains(uint16 _dstChainId, uint256 _tokenId)
        public
        payable
    {
    ...
    /** Signiture */
    function getSigner(bytes32 hashes, bytes memory signature)
        public
        pure
        returns (address)
    {
