...
using SafeMath for uint256;
    ...
    uint256 public boostMultiplier = 100;
    uint256[] public rewards_ = addFee(rewardsPerDay, boostMultiplier);
    uint256 public rewards1 = rewards_[0]+rewards_[1];
    uint256 public boost1 = rewards_[1];
   uint256[] public rewards_2 = addFee(rewardsPerDay2, boostMultiplier);
    ...
    function getMultiple(uint x,uint y) public view returns (uint){
    	uint i;
    	while(y >0){
    		y = y - x;
    		i++;
    ...
    }    function updatelast(uint8 newVal) external{
        last = newVal; // NFT tier_3
    }    function updateboostMultiplier(uint8 newVal) external{
        boostMultiplier = newVal; // NFT tier_3
    }    
}