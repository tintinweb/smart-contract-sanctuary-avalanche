...
contract Defire is Auth {
    ...
    function updatePersonalMatrix(
        ...
        if (ref.refLevels[1].length < 3) {
            ...
            updateDownline(_referrer, user.id, 1, _pkg, 3);
        }

        else if (ref.refLevels[3].length < 27) {
            updateUpline(_referrer, user.id, _pkg, 3);
            updateDownline(_referrer, user.id, 2, _pkg, 9);
        }

        else if (ref.refLevels[4].length < 81) {
            updateUpline(_referrer, user.id, _pkg, 4);
            updateDownline(_referrer, user.id, 3, _pkg, 27);
        }

        else if (ref.refLevels[5].length < 243) {
            updateUpline(_referrer, user.id, _pkg, 5);
            updateDownline(_referrer, user.id, 4, _pkg, 81);
        }

    }

    function updateUpline(address _referrer, uint256 _userId, uint8 _pkg, uint8 _from) internal {
        ...
        for (uint8 i = _from ; i <= 5 ; i++) {
                ...
                users[upline].personalMatrix[_pkg].refBonus += pkgPrice[_pkg].mul(refPercent).div(
                    percentDivider
                );
    ...
    function updateDownline(address _referrer, uint256 _userId, uint8 _pkg, uint8 _lvl, uint8 _length) internal {
        for (uint8 i=0 ; i < _length ; i++){
            ...
            if(users[downline].personalMatrix[_pkg].refLevels[1].length < 3){
                ...
                users[downline].personalMatrix[_pkg].refBonus += pkgPrice[_pkg].mul(refPercent).div(
                    percentDivider
                );
                break;
            }
