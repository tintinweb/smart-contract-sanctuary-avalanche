...
contract Competition is ERC20Upgradeable, VRFConsumerBaseV2{
    ...
    constructor() VRFConsumerBaseV2(vrfCoordinator) initializer {}
    ...
    function requestRandomWords() external forOwner {
    ...
    function createNewSubscription() public forOwner {
    ...
    function topUpSubscription(uint256 amount) external forOwner {
        LINKTOKEN.transferAndCall(address(COORDINATOR), amount, abi.encode(s_subscriptionId));
    }

    // Add a consumer contract to the subscription.
    function addConsumer(address consumerAddress) external forOwner {
        COORDINATOR.addConsumer(s_subscriptionId, consumerAddress);
    }

    // Remove a consumer contract from the subscription.
    function removeConsumer(address consumerAddress) external forOwner {
        COORDINATOR.removeConsumer(s_subscriptionId, consumerAddress);
    }

    // Cancel the subscription and send the remaining LINK to a wallet address.
    function cancelSubscription(address receivingWallet) external forOwner {
    ...
    // 1000000000000000000 = 1 LINK
    function withdraw(uint256 amount, address to) external forOwner {
