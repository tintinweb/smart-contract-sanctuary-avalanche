...
contract StabilityPool is PoolBase, ReentrancyGuardUpgradeable, IStabilityPool {
    ...
    mapping (address => newColls) pendingCollGains;
     ...
     * @param _reinvest Bool whether or not to reinvest the YUSD received from the swap
     */
    function claimRewardsSwap(uint256 _yusdMinAmountTotal, address[] calldata _swapAssets, bool _reinvest)
        external
        override
        nonReentrant
        returns (uint256 amountFromSwap)
    {
        ...
        amountFromSwap = _sendGainsToDepositorSwap(assets, amounts, _swapAssets);
        require(amountFromSwap >= _yusdMinAmountTotal, "SP:Insufficient YUSD Transferred");
        if (_reinvest) {
            _reinvestAfterClaim(amountFromSwap);
        }
    }

    /** 
     * @notice function to reinvest YUSD after selling collateral rewards to YUSD
     * @param _amount of YUSD to reinvest
     */
    function _reinvestAfterClaim(uint256 _amount) internal {
        _requireNonZeroAmount(_amount);

        uint256 compoundedYUSDDeposit = getCompoundedYUSDDeposit(msg.sender);

        // Pulls YUSD into the pool, updates totalYUSDDeposits variable for the stability pool
        // and throws an event
        _sendYUSDtoStabilityPool(msg.sender, _amount);
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        (address[] memory collateralsFromNewGains, uint256[] memory amountsFromNewGains) = _calculateNewGains(initialDeposit, snapshots);
        // Add pending gains to the current gains
        return (collateralsFromNewGains, _leftSumColls(newColls(collateralsFromNewGains, amountsFromNewGains), pendingCollGains[_depositor].tokens, pendingCollGains[_depositor].amounts));
    }
    ...
    function _sendGainsToDepositor(
        ...
        newColls memory tempPendingCollGains;
     ...
     *  Resets pendingCollGains to whatever was not swapped during this function. 
     * @param _rewardAssets array of address
     * @param _amounts array of uint256 amounts, includes pending collaterals since that was added in during previous steps
     * @param _swapAssets addresses of swap assets that should match up with the reward assets. If there exists a collateral 
     *  in _swapAssets, but not in _rewardAssets, then all after that will be skipped due to bad formatting. 
     */
    function _sendGainsToDepositorSwap(address[] memory _rewardAssets, uint256[] memory _amounts, address[] calldata _swapAssets)
        internal
        returns (uint256 totalYUSD)
    {
        uint256 assetsLen = _rewardAssets.length;
        require(assetsLen == _amounts.length, "SP:Length mismatch");
        IYUSDToken yusdTokenCached = yusdToken;
        uint256 balanceBefore = yusdTokenCached.balanceOf(msg.sender);
        // If length 0 then just swap all. 
        if (_swapAssets.length == 0) {
            for (uint256 i; i < assetsLen; ++i) {
                ...
                totalYUSD = totalYUSD.add(_callRouterSwap(asset, amount, address(yusdTokenCached)));
            } 
        } else {
            // Check that in the same order as the whitelist, so that the lists will collide. 
            controller.checkCollateralListSingle(_swapAssets, false);
            // Create tempPendingCollGains to add to it later 
            newColls memory tempPendingCollGains;
            // Tokens will always be all whitelisted
            tempPendingCollGains.tokens = _rewardAssets;
            tempPendingCollGains.amounts = new uint256[](assetsLen);
            uint256 j;
            uint256 i;
            // If j runs out, or we get to the rest of the list, means we are done.
            for (; i < assetsLen && j < _swapAssets.length; ++i) {
                uint256 amount = _amounts[i];
                address asset = _rewardAssets[i];
                if (amount == 0) {
                    continue;
                }
                // If is not swap asset, then skip it, and set amounts to 0 so that in the subColls call later it is not accounted
                // for in the trove still. 
                // If j has passed the reward assets length, then that means we are done with the ones to swap, so 
                // just transfer it normally. 
                if (j >= _swapAssets.length || asset != _swapAssets[j]) {
                    // Add to pendingCollGains list since it was not swapped here. Saved for next time.
                    tempPendingCollGains.amounts[i] = amount;
                    // Set amounts to 0 so it is not removed from total coll yet. 
                    _amounts[i] = 0;
                } else {
                    totalYUSD = totalYUSD.add(_callRouterSwap(asset, amount, address(yusdTokenCached)));
                    // Increment j since a swap asset has been seen, and unroute that through the router. 
                    ++j;
                }
            }
            // If we reached end of j but not end of i, that means there are more pending colls. 
            for (; i < assetsLen; ++i) {
                    // Add to pendingCollGains list since it was not swapped here. Saved for next time.
                    tempPendingCollGains.amounts[i] = _amounts[i];
                    // Set amounts to 0 so it is not removed from total coll yet. 
                    _amounts[i] = 0;
            }
            
            // Set to new pending colls 
            pendingCollGains[msg.sender] = tempPendingCollGains;
        ...
        totalColl.amounts = _leftSubColls(totalColl, _rewardAssets, _amounts);
    }

    /** 
     * Calls the router swap to get YUSD back from swapping out collateral assets. 
     */
    function _callRouterSwap(address _asset, uint256 _amount, address _yusdToken) internal returns (uint256) {
        address router = controller.getDefaultRouterAddress(_asset);
        ...
        IERC20(_asset).safeTransfer(router, _amount);
        return 
