...
contract CBTower is Ownable, ICBTower {
    ...
    function _getWithdrawableUSDT(address _user) internal view returns (uint256 amount) {
        ...
        if (user.lastUpdateFloor < currentFloor) {
            ...
            amount = (user.lastAmount * 10**18 - user.lastAmount*( (user.reinvestRatio * 10**18 / 1000) ** difference ) / 10**18);
        } else {
            amount = 0;
        }
    }
    ...
    function _initiateNewFloor() internal {
        ...
        for (uint256 i = 500; i < 1001; i++) {
            if (floorReinvestAmounts[currentFloor - 1][i] > 0) {
                uint256 _amount = floorReinvestAmounts[currentFloor - 1][i] * i / 1000;
                floorReinvestAmounts[currentFloor][i] = _amount;
