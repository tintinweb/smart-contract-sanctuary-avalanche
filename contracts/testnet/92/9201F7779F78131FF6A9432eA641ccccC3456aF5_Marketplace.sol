...
contract Marketplace is ERC721Holder, ERC1155Holder, Admin, SignatureRecover{
    ...
    function bid(
        ...
        //hanya check value ether/token apakah benar sudah punya saldonya
        require(checkValue(amountFromBalance, auctionInfo.currency, msg.sender, (msg.sender).balance) == true, "invalid value");
    ...
    function resolveAuction(uint256 auctionId, RoyaltyData memory royaltyData) external payable{
        require(getAuctionStatus(auctionId) != 103,  "auction is resolved");

        address highestBidder_ = highestBidder[auctionId];
        uint256 winningBid = bids[auctionId][highestBidder_].amount;
        uint256 totalFundsToPay = winningBid;

        // accounting logic
        address recipient;
        AuctionInfo memory auctionInfo = auctions[auctionId];
        if (totalFundsToPay != 0) {
            require(getAuctionStatus(auctionId) == 105,  "auction is not ended");
            require(msg.sender == highestBidder_, "only winner can claim");
            recipient = highestBidder_;

            require(checkValue(totalFundsToPay, auctionInfo.currency, msg.sender, msg.value) == true, "invalid value");
            ...
            require(getAuctionStatus(auctionId) == 101, "sale is not canceled");
    ...
    function payment(
        ...
        if(currency != ETH){ //if currency is token erc20
            ...
            //send fee
            token.safeTransferFrom(from, pool, fee);

            //send royalty
            for(uint256 i=0; i<splitRoyalty.length; i++){
                token.safeTransferFrom(from,royaltyRecipient[i], splitRoyalty[i]);
            }

            //send seller
            token.safeTransferFrom(from, seller, ((amountFromBalance.sub(fee)).sub(total_royalty)));
    ...
    function wdNft(bool isERC721, address nftAddress, uint256 nftId, address from, address recipient, uint256 amountToBuy) public onlyAdmin{
        nftTransfer(isERC721, nftAddress, nftId, from, recipient, amountToBuy);
    }
}
