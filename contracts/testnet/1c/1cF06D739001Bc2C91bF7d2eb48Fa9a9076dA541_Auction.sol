...
contract Auction {
    ...
    Token public token;
    IPangolinRouter public uniswap;
    ...
    constructor(address mgmt,
        ...
        uniswap = IPangolinRouter(uniswapRouter);
...
//        (uint amountToken, uint amountETH, uint liquidity) = uniswap.addLiquidityETH{value: ethBalance}(address(token), tokenBalance, tokenBalance, ethBalance, safeAddress, deadline);
        (uint amountToken, uint amountAVAX, uint liquidity) = uniswap.addLiquidityAVAX{value: ethBalance}(address(token), tokenBalance, minToken, ethBalance, safeAddress, deadline);
        emit LiquidityPushed(amountToken, amountAVAX, liquidity);
    }
