...
contract slotMachine is ConfirmedOwner, VRFConsumerBaseV2 {
    ...
    event slots(uint256 slot1, uint256 slot2, uint256 slot3);
    ...
    function play() external payable {
        ...
        require(maxPlays > 0, "machine is closed");
        emit slots(
            getRequestStatus(lastRequestId, currentWord)%9,
            getRequestStatus(lastRequestId, currentWord + 1)%9,
            getRequestStatus(lastRequestId, currentWord + 2)%9
        );
        currentWord += 3;
    }
    ...
    function getRequestStatus(uint256 _requestId, uint256 word)
        internal
        view
        returns (uint256 randomWords)
    {
        ...
        return (request.randomWords[word]);
    }
}
