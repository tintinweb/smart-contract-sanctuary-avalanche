...
contract BridgedToken is ERC20BurnableUpgradeable, IBridgedToken, OwnableUpgradeable, ChainIdHolding {
  ...
  function claimBridgedToken(
    ...
    bytes32 _claimId = keccak256(
      abi.encode(_originToken, _to, _amount, _chainIdsIndex, _txHash, name(), symbol(), decimals())
    );
...
import '../interfaces/IBridgedToken.sol';
import '../lib/ChainIdHolding.sol';
import '../lib/InuXBase.sol';
import './BridgedToken.sol';
import './Governable.sol';

contract InuXBridge is InuXBase, ReentrancyGuardUpgradeable, BlackholePrevention, Governable, ChainIdHolding {
  using SafeMathUpgradeable for uint256;
  ...
  event SetFeeToken(uint256 chainId, address token, uint256 fee, uint256 timestamp);

  function initialize(uint256[] memory _chainIds) public initializer {
    __InuXBase_initialize();
  ...
  function setFeeReceiver(address payable _feeReceiver) external onlyGovernance {
  ...
  function setFeeForTokens(
    ...
    require(chainIds.length == originTokens.length && originTokens.length == fees.length, '!input');
    uint256 tokenCount = originTokens.length;
    for (uint256 i = 0; i < tokenCount; ++i) {
      feeForTokens[chainIds[i]][originTokens[i]] = fees[i];
      emit SetFeeToken(chainIds[i], originTokens[i], fees[i], block.timestamp);
    }
  }
  ...
  function requestBridge(
    ...
    if (!isBridgedToken(_tokenAddress)) {
      ...
      uint256 feePercent = defaultFeePercentage == 0 ? DEFAULT_FEE_PERCENTAGE : defaultFeePercentage;
      uint256 forUser = (_amount * (DEFAULT_FEE_DIVISOR - feePercent)) / DEFAULT_FEE_DIVISOR;
      ...
      emit RequestBridge(_tokenAddress, _toAddr, forUser, chainId, chainId, _toChainId, index);
      ++index;
  ...
  function claimToken(
    ...
    bytes32 _claimId = keccak256(
      abi.encode(_originToken, _toAddr, _amount, _chainIdsIndex, _txHash, _name, _symbol, _decimals)
    );
    require(!alreadyClaims[_claimId], 'already claim');
    alreadyClaims[_claimId] = true;
    ...
    if (_originToken == NATIVE_TOKEN_ADDRESS) {
      if (_chainIdsIndex[0] != chainId) {
        ...
        if (tokenMap[_chainIdsIndex[0]][_originToken] == address(0)) {
          BridgedToken bt = new BridgedToken();
          bt.initialize(_originToken, _chainIdsIndex[0], _name, _symbol, _decimals);
          tokenMap[_chainIdsIndex[0]][_originToken] = address(bt);
          tokenMapReverse[address(bt)] = TokenInfo({addr: _originToken, chainId: _chainIdsIndex[0]});
          bridgeTokens[address(bt)] = true;
        }
        _mintTokenForUser(_amount, _originToken, _chainIdsIndex, _txHash, _toAddr);
        emit ClaimToken(
          _originToken,
          _toAddr,
          _amount,
        ...
        _transferOriginToken(_amount, _originToken, _chainIdsIndex, _toAddr);
  ...
  function _mintTokenForUser(
    ...
    IBridgedToken(tokenMap[_chainIdsIndex[0]][_originToken]).claimBridgedToken(
      _originToken,
      _toAddr,
      _amount,
      _chainIdsIndex,
  ...
  function _transferOriginToken(
    ...
    //claiming original token
    (uint256 forUser, uint256 forFee) = getAmountsToSend(_amount, _originToken, _chainIdsIndex[0]);
  ...
  function safeTransferOut(address _token, address _toAddr, uint256 _forUser, uint256 _forFee) internal {
    if (_token == NATIVE_TOKEN_ADDRESS) {
      ...
      require(balBefore.sub(erc20.balanceOf(address(this))) == _forUser + _forFee, '!transfer to');
    }
  }
  ...
  function getAmountsToSend(
    ...
    uint256 feePercent = defaultFeePercentage == 0 ? DEFAULT_FEE_PERCENTAGE : defaultFeePercentage;
    forUser = (amount * (DEFAULT_FEE_DIVISOR - feePercent)) / DEFAULT_FEE_DIVISOR;
  ...
  function getFeeInfo(address token) external view returns (uint256 feeAmount, uint256 feePercent) {
    feePercent = defaultFeePercentage == 0 ? DEFAULT_FEE_PERCENTAGE : defaultFeePercentage;
  ...
  function getFeeRecipientAddress() internal view returns (address payable ret) {
...
contract InuXBase is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  function __InuXBase_initialize() internal initializer {
    __Ownable_init();
  }
