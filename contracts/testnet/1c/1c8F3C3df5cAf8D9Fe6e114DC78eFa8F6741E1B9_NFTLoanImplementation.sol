...
contract NFTLoan is NFTLoanGovernance, SigningUtils {
    ...
    function registerLoanSameChain(
        ...
        uint16 _chainId; // @todo just get chain id here and verify it with lender's chainId
    ...
    function registerLoan(
        ...
        // @todo also there remove the offer.nftChainId from the below check and finalize chainId() implemnatiaon
    ...
    function completeRepayLoan(bytes memory _encodedVAA) external {
        ...
        //@todo consumption of VAA to be done
        NFTLoanStructs.Loan memory loan = decodeLoan(vm.payload);
...
contract NFTLoanHelper is NFTLoanGetters, NFTLoanSetters {
    ...
    function _createLoan(
        ...
        // @todo change this offer.offerChainId to chainId() after asking from Yasir bhai
        if (offer.offerChainId == offer.offerChainId) {
            // it means offer is on same chain.
    ...
    function _repayLoan(
        ...
        // @todo also there remove the offer.nftChainId from the below check and finalize chainId() implemnatiaon
        require(loan.loanChainId == loan.nftChainId, "Invalid chain");
        require(
            loan.status == NFTLoanStructs.LoanStatusType.ACTIVE,
            "Loan is not active"
        );
        ...
        if (loan.nftChainId == loan.loanChainId) {
            // it means offer is on same chain.
            _transferNFT(
                loan.nftCollateralContract,
                loan.nftCollateralId,
