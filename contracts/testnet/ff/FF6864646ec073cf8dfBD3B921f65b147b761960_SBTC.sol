...
contract SBTC is BaseERC1155, IRoyaltyChecker {
    ...
    event UnpaidFeeSetted(address[] collections, uint256[] ids, TxData[] txs);
    event UnpaidFeeClosed(uint256[] sbtIds);
    ...
    function setUnpaidFee(
        ...
        uint256 length = tokenURI_.length;
        require(
            length == collections_.length && length == ids_.length && length == txs_.length,
            "Invalid params length"
        );
        for (uint256 i = 0; i < length; i++) {
            require(collections_[i] != address(0), "Collection is zero address");
        ...
        emit UnpaidFeeSetted(collections_, ids_, txs_);
        return true;
    }

    function closeUnpaidPayments(uint256[] memory sbtIds_) external payable onlyRole(OPERATOR_ROLE) returns (bool) {
        require(sbtIds_.length > 0, "Invalid params length");
        for (uint256 i = 0; i < sbtIds_.length; i++) {
            if (_txsData[sbtIds_[i]].length == 0) continue;
            IRoyaltyChecker.TxData memory txData_ = _txsData[sbtIds_[i]][0];
            for (uint256 j = 0; j < _paymentTokens[sbtIds_[i]].length(); j++) {
                ...
                if (token_ == address(0)) {
                    require(msg.value == fee_, "Value is not positive");
...
interface IRoyaltyChecker {
    ...
    function closeUnpaidPayments(uint256[] memory sbtIds) external payable returns (bool);
}
