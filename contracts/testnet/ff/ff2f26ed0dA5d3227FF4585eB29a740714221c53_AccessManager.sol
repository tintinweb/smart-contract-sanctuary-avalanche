...
contract AccessManager is ERC1155, IERC2981, IAccessManager, Ownable {
  ...
  //////////////////////////////////////////////////////////////*/
	...
	constructor(
		...
		//
		// TODO
		// reconsider the initial items. need placeholders for shield WL levels
		// - ability to mint
		// -

		// Add placeholder items for the WL passes
		for (itemCount; itemCount < 7; ) {
			// Add the basic access pass on creation
	...
	function linkShieldManager(address shieldManagerAddress) external onlyOwner {
	...
	function mintAndDrop(
		...
		for (uint256 i = 0; i < receivers.length; ) {
			...
			// can not overflow on human timescales
			unchecked {
				++i;
			}
		}
	...
	function toggleItemWhitelist(address user, uint256 itemId) external {
		...
		// TODO consider below check in case of non items (ie. just whitelist placeholders)
	...
	function whitelistMintItem(uint256 itemId) external {
		...
		if (itemId <= GOLD) {
			...
			// if (balanceOf[msg.sender][BASIC] == 0) {
			// 	_mint(msg.sender, BASIC, 1, '');

			// 	// supply will not overflow on human timescale
			// 	unchecked {
			// 		++itemList[BASIC].currentSupply;
			// 	}
			// }
		}
	...
	function mintItem(uint256 itemId, address member) external payable {
		// prevents unregestered user minting any item but an access pass (non access passes have id > GOLD)
		//if (itemId > GOLD && balanceOf[member][BASIC] == 0) revert Unauthorised();

		if (itemList[itemId].live == false && !roundtableWhitelist[member][itemId])
		...
		if (itemId <= GOLD) {
			...
			// if (itemId != BASIC && balanceOf[member][BASIC] == 0) {
			// 	_mint(member, BASIC, 1, '');
			// 	++itemList[BASIC].currentSupply;
			// }
		}
	...
	function discountedPrice(uint256 itemId, address member) public returns (uint256 price) {
		// if memberLevel >= level for this item -> this covers gold level
