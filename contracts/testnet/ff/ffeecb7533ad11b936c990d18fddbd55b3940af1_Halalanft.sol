...
/// @title Halalanft
/// @notice This is the main contract of Halalanft, NFT with Islamic DeFi got explored
contract Halalanft is
  ERC165Storage,
  ERC721A,
  ERC721AQueryable,
  Ownable2Step,
  ...
  ReentrancyGuard
{
  ...
  // max supply cap
  uint256 private constant MAX_SUPPLY = 3001;
  uint256 private constant MAX_PRESALE = 2;
  uint256 private constant MAX_PUBLIC = 11;

  address private constant TREASURY = address(0xB85885977D5daf4D6768AFC670C14a5FC209b8E9);
  address private constant FILANTROFI = address(0x6A07F68a60eB5890947A5af40F3eA928447d5527);
  address private constant MARKOPS = address(0x42EF5475Eb5951B92cE28dccECB720654623fCb2);
  address private constant TEAMS = address(0x742a1b678a34F9E79Ff4468968FFed99676ad3dc);
  ...
  constructor(address _usdc, address _feed) ERC721A("Halalanft", "HNFT") {
    _usdcToken = IERC20(_usdc);
    _priceFeed = AggregatorV3Interface(_feed);
    _usdPrice = 239 * 10 ** _priceFeed.decimals(); // 239 USD by default
  ...
  function publicMint(uint256 _amount) external payable {
    ...
    require(_amount < MAX_PUBLIC, "public: cannot minted more than 10");
  ...
  function presale(uint256 _amount, bytes32[] calldata _merkleProof) external payable {
    require(msg.sender == tx.origin, "presale: bot is not allowed");
    require(_verifyProof(msg.sender, _merkleProof), "presale: address is not in the whitelist");
    require(_getAux(msg.sender) < 3, "presale: address cannot minted anymore");
  ...
  function _distribute() private {
    ...
    uint256 forTreasury = currentBalance * 3 / 5;
    uint256 forFilantrofi = currentBalance / 10;
    uint256 forMarkops = currentBalance * 3 / 20;
    uint256 forTeams = currentBalance * 3 / 20;

    _usdcToken.transfer(TREASURY, forTreasury);
    _usdcToken.transfer(FILANTROFI, forFilantrofi);
    _usdcToken.transfer(MARKOPS, forMarkops);
    _usdcToken.transfer(TEAMS, forTeams);
  }
