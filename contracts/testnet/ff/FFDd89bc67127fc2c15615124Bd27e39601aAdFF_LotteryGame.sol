    ...
    KeeperCompatible
{
    ...
    bool public isDateTimeRequired;
    uint256[] public winnersPercentages;
    ...
    constructor(
        ...
        timelock = timelock_;
        IERC20(LINK).approve(KEEPERS_REGISTRY, ~uint256(0));
    }
    ...
    function setParticipationFee(uint256 _participationFee) public onlyOwner {
        ...
        require(
            _participationFee <=
                participationFee + (participationFee * 2_500) / 10_000,
            "INVALID_ENTRY_FEE"
        );
        ...
        nonReentrant
    {
        ...
        uint96 keepersRequires = 0.1 ether;
    ...
    function cancelSubscription(address to_) external onlyOwner {
        VRFCoordinatorV2Interface(VRF_COORDINATOR)
            .cancelSubscription(chainlinkSettings.subscriptionId, to_);
    ...
    function _isTimelockValid() private view returns (bool) {
        ...
        for (uint256 i; i < timelock.daysUnlocked.length; i++) {
            if (dayOfWeek == timelock.daysUnlocked[i]) {
                uint8 day = contracts.dateTime.getDay(block.timestamp);
