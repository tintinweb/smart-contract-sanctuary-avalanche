...
contract Lottery is Ownable, ReentrancyGuard {
    ...
    /// @dev number of max lottery participant length
    uint256 public maxLotteryParticipantLen;
     ...
     * @param _maxLotteryParticipantLen number of max lottery participants
     */
    constructor(
        address _payToken,
        uint256 _minAmount,
        ...
        uint256 _maxLotteryParticipantLen
    ) {
        ...
        maxLotteryParticipantLen = _maxLotteryParticipantLen;
     ...
     */
    function enterLottery(uint256 _lotteryId, uint256 _amount)
        public
        nonReentrant
    {
        require(
        ...
        require(
            lotteries[_lotteryId].players.length < maxLotteryParticipantLen,
    ...
    /**
     * @notice Update minimum amount of payToken to enter lottery
     * @param _minAmount     amount of minAmount
     */
    function updateMinPayTokenAmount(uint256 _minAmount) external onlyOwner {
        minAmount = _minAmount;
    }
    ...
    /**
     * @notice Update number of max lottery participant length
     * @param _maxLotteryParticipantLen  number of max lottery participant length
     */
    function updateMaxLotteryParticipantLen(uint256 _maxLotteryParticipantLen)
        external
        onlyOwner
    {
        require(
            _maxLotteryParticipantLen > 0,
            "Error: treasury address is zero"
        );

        if (lotteries[currentLotteryId].status == LOTTERY_STATE.ACTIVE) {
            require(
                _maxLotteryParticipantLen > maxLotteryParticipantLen,
                "Error: can't reduce number of participants when lottery is active"
            );
        }
