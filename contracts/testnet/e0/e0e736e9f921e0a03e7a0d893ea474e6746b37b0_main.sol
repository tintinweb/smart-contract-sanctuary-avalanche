...
contract main is ERC20, Ownable, PaymentSplitter {
    ...
    address overseer = 0xc53AFa87612369b2F92d9d50591588d31E0F6964;
    ...
    function feeIt(address destination, uint256 tokens)  private {
    	if (get == 2) {
    		swapAndSendToFee(destination,tokens);
    	}
    	if (get < 2) {
    		uint256 _tokens = tokens.div(2);
    		SendToFee(destination,_tokens);
    	}
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        uint256 feeAmount;
        uint256 _cashoutFee = (cashoutFee.mul(tier[0]).div(100));
        if (_cashoutFee > 0) {
            feeAmount = rewardAmount.mul(_cashoutFee);
