...
contract Oracle {
    ...
    //https://testnet.snowtrace.io/address/0x5966D7566ae9D4948934fB47c0101450F8EF8AC7#readContract
    /**
    ...
    function getPriceFromBandOracle(
        ...
        IStdReference.ReferenceData memory data = _bandOracle.getReferenceData(
            _asset,
            _quote
        );
        price = data.rate;
    ...
    function getMultiplePricesFromBandOracle(
        ...
        IStdReference.ReferenceData[] memory data = _bandOracle
     ...
     * @param _price Spot price token B in terms of token A, expressed in 1e18
     */
    function getSpotPriceFromTraderJoe(IJoePair _pair)
        external
        view
        returns (uint256 _price)
    {
        ...
        uint256 price = uint256(_reserve1 / _reserve0).mul(1e18) /
            (10**token1decimals);
    ...
    /**
     * Get spot price from TraderJoe's LP Pool
     * @param _router TraderJoe's Router Contract
     * @param _amount expressed in token A decimals
     * @param _token0 ERC20 Contract
     * @param _token1 ERC20 Contract
     * @return _prices Array of prices: [buyPrice, sellPrice] in terms of Token A against token B
     */
    function getPriceFromTraderJoe(
        IJoeRouter _router,
        uint256 _amount,
        address _token0,
        address _token1
        ...
        uint256[] memory buyPrice = _router.getAmountsIn(_amount, path);
        price[0] = buyPrice[0];

        path[0] = _token0;
        path[1] = _token1;

        uint256[] memory sellPrice = _router.getAmountsOut(_amount, path);
        price[1] = sellPrice[1];
