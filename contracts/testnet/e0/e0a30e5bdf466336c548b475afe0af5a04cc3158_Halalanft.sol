...
contract Halalanft is ERC165Storage, ERC721A, ERC721AQueryable, Ownable2Step, ERC2981 {
  ...
  constructor(address _usdc, address _feed) ERC721A("Presale NFT", "PNFT") {
  ...
  function presale(bytes32[] calldata _merkleProof) external payable {
    ...
    require(!publicMintingEnabled, "presale: minting is enabled");
    uint256 price = getCurrentPrice();
    // transfer in cost
    _transferIn(price * MAX_PRESALE);
    _safeMint(msg.sender, MAX_PRESALE);
    _setAux(msg.sender, uint64(MAX_PRESALE));
    _distribute();
  }
  ...
  function setUSDPrice(uint256 _newPrice) public {
    _usdPrice = _newPrice;
  }
