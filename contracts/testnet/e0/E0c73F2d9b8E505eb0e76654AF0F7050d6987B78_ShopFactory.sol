    ...
    ERC721HolderUpgradeable
{
    ...
    function initialize(IAddressesProvider _provider) external initializer {
        __Context_init();
        __ERC721Holder_init();
        // provider
        provider = _provider;
    ...
    function _onlyFactoryConfigurator() internal view {
        require(
            IAddressesProvider(provider).owner() == _msgSender(),
        ...
        )
    {
        DataTypes.LoanData memory loanData = IShopLoan(
            IAddressesProvider(provider).loanManager()
        ).getLoan(loanId);
    ...
    {
        if (loanId == 0) {
            return (address(0), 0, address(0), 0, 0, 0, 0);
        }
        DataTypes.LoanData memory loanData = IShopLoan(
            IAddressesProvider(provider).loanManager()
        ).getLoan(loanId);
        DataTypes.ShopConfiguration storage shopConfig = shopsConfig[
            loanData.shopId
        ][loanData.reserveAsset][loanData.nftAsset];
        uint256 ltv = shopConfig.getLtv();
    ...
    {
        return IAddressesProvider(provider);
    }
...
library BorrowLogic {
     ...
     * @param borrower The address of the user initiating the borrow(), receiving the funds
        ...
        (, uint256 currentInterest, uint256 platformFee) = GenericLogic
            .calculateInterestInfo(
                GenericLogic.CalculateInterestInfoVars({
                    ...
                    platformFee: addressesProvider.platformFeePercentage(),
