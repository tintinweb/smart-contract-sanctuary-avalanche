...
contract InfinityToken is IInfinityToken, Initializable, InfinityTokenERC1155, InfinityTokenERC721 {
    ...
    function withdraw(
        ...
        require(_coinIds.length == _amounts.length, "ids and amounts length mismatch");
        for(uint i=0;i<_coinIds.length;i++){
			if(ERC721Validator.isERC721(address(uint160(_coinIds[i])))){
                _withdrawERC721(clientAddress,_coinIds[i],_amounts[i]);
			}else{
                _burn(clientAddress, _coinIds[i], _amounts[i]);
            }
        }
    }

    // TODO audit recommendation: possible gas cost saving rewrites
    function updateBalance(
        ...
        for(uint j = 0; j < tokenUpdates.length; j++){
			...
			// if(ERC721Validator.isERC721(address(uint160(tokenUpdates[j].tokenId)))){
                _depositERC721(clientAddress, tokenUpdates[j].tokenId, tokenUpdates[j].amount);
			} else {
                _update(clientAddress, tokenUpdates[j].tokenId, tokenUpdates[j].amount, tokenUpdates[j].priceIndex);
            }
        }
        emit UpdateBatch(clientAddress, tokenUpdates);
    }

    function updateFixedRatePosition(address clientAddress, IInfinityToken.FixedRatePosition[] calldata _fixedRatePositionUpdates) external override onlyPoolOrOwner {
        uint256[] memory frIds = new uint256[](_fixedRatePositionUpdates.length);
        for(uint j = 0; j < _fixedRatePositionUpdates.length; j++){
            _fixedRatePositions[_nextFrId].tokenId = _fixedRatePositionUpdates[j].tokenId;
            _fixedRatePositions[_nextFrId].rate = _fixedRatePositionUpdates[j].rate;
            _fixedRatePositions[_nextFrId].maturityTimestamp = _fixedRatePositionUpdates[j].maturityTimestamp;
            _fixedRatePositions[_nextFrId].amount = _fixedRatePositionUpdates[j].amount;
            _balances[_fixedRatePositionUpdates[j].tokenId][clientAddress].amount += 1;
            ownersNFT[_fixedRatePositionUpdates[j].tokenId][_nextFrId] = clientAddress;
            frIds[j] = _nextFrId;
            _nextFrId++;
        }
        emit UpdateFrBatch(clientAddress, _fixedRatePositionUpdates, frIds);
	}
    ...
    function _beforeTokenTransfer(
        ...
        bytes memory data
    ) internal override onlyPoolOrOwner {
    ...
    function _ifUserTokenExistsERC721(
        ...
        exists = ownersNFT[tokenAddress][tokenId]==account;
    }
    function _depositERC721(
        ...
        require(ownersNFT[tokenAddress][tokenId]==address(0), "ERC721 already owned by another user");
        address operator = _msgSender();
        _balances[tokenAddress][account].amount += 1; // standard balance update
        ownersNFT[tokenAddress][tokenId] = account; // save tokenId to user
        emit TransferSingle(operator, address(0), account, tokenAddress, tokenId);
        _doSafeTransferAcceptanceCheck(operator, address(0), account, tokenAddress, tokenId, "");
    }
    function _withdrawERC721(
        ...
        require(ownersNFT[tokenAddress][tokenId]==address(account), "ERC721: already belongs to another address");
        _balances[tokenAddress][account].amount -= 1; // standard balance update
        ownersNFT[tokenAddress][tokenId] = address(0);
...
import "./dependencies/openzeppelin/contracts/token/ERC1155/IERC1155Receiver.sol";
import "./dependencies/openzeppelin/contracts/token/ERC1155/extensions/IERC1155MetadataURI.sol";
import "./dependencies/openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol";
import "./dependencies/openzeppelin/contracts/utils/introspection/ERC165.sol";
...
contract InfinityTokenERC1155 is ERC165, IERC1155, IERC1155MetadataURI, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyPoolOrOwner() {
        require(address(pool) == _msgSender()||owner() == _msgSender(), "caller is not pool nor owner");
        _;
    }
    ...
    function balanceOf(address account, uint256 id) public view virtual override returns (uint256) {
        ...
        // uint160 = 160 bits represents 20-byte address while 21-byte address represents interest bearing token
        bool isInterestBearing = id > type(uint160).max;
        if (isInterestBearing && _balances[id][account].priceIndex != 0) {
            uint64 poolPriceIndex = pool.priceIndex(id);
            // simplify from _balances[id][account].amount * (1 + (poolPriceIndex - _balances[id][account].priceIndex) / _balances[id][account].priceIndex);
            return _balances[id][account].amount * poolPriceIndex / _balances[id][account].priceIndex;
        } else {
    ...
    event UpdateBatch(
        address indexed to,
        IInfinityPool.TokenUpdate[] tokenUpdates
    );

    event UpdateFrBatch(
        address indexed to,
        IInfinityToken.FixedRatePosition[] _fixedRatePositionUpdates,
        uint256[] frIds
    );
    ...
    function _update(
        ...
        require(to != address(0), "ERC1155: mint to the zero address");
        _balances[id][to].amount = amount;
        if(priceIndex!=0) _balances[id][to].priceIndex = priceIndex;
    }
}
...
contract InfinityTokenERC721 {
    ...
    mapping(uint => mapping(uint => address)) public ownersNFT;

    // frId => FixedRatePositions
    mapping(uint256 => IInfinityToken.FixedRatePosition) internal _fixedRatePositions;
    uint256 internal _nextFrId; 

    function fixedRatePositions(uint256 _frId) external view returns(IInfinityToken.FixedRatePosition memory) {
        return _fixedRatePositions[_frId];
    }
