...
abstract contract Pausable is Context {
    ...
    modifier whenNotPaused() {
        require(!paused(), "Marketplace pausable: paused");
        _;
    }

    modifier whenPaused() {
        require(paused(), "Marketplace pausable: not paused");
        _;
    }
...
contract MarketplaceAuction is Pausable {
    ...
    function bid(uint auctionID) public whenNotPaused() payable {
        require(auctions.length > auctionID, "Marketplace bid: invalid auctionID");
        ...
        require(tempAmount <= ethAmountSent, "Marketplace bid: invalid bid amount");
        if (bidsLength > 0) require(payable(lastBid.from).send(lastBid.amount), "Marketplace bid: invalid transaction");
    ...
    function bidToken(uint auctionID, uint value) public whenNotPaused() {
        require(auctions.length > auctionID, "Marketplace bidToken: invalid auctionID");
        ...
        require(tempAmount <= ethAmountSent, "Marketplace bidToken: invalid bid amount");
        if (bidsLength > 0) require(IERC20(myAuction.tokenAddress).transfer(lastBid.from, lastBid.amount), "Marketplace bidToken: invalid transaction");
        require(IERC20(myAuction.tokenAddress).transferFrom(msg.sender, myAuction.auctionAddress, value), "Marketplace bidToken: invalid transaction");
    ...
    function finalizeAuction(uint auctionID) public whenNotPaused() {
        require(auctions.length > auctionID, "Marketplace finalizeAuction: invalid auctionID");
        ...
        if (bidsLength == 0) {
            ...
            if (!myAuctionPayableStatus.paidRoyalty) {
                if (myAuction.isToken) require(IERC20(myAuction.tokenAddress).transfer(receiver, royaltyAmount), "Marketplace finalizeAuction: invalid royalty transaction");
                else require(payable(receiver).send(royaltyAmount), "Marketplace finalizeAuction: invalid royalty transaction");
            }
    ...
    function isFinalizedAuction(uint auctionID) public view returns (bool) {
        require(auctions.length > auctionID, "Marketplace Auction: invalid auctionID");
        Auction memory myAuction = auctions[auctionID];
    ...
    modifier isOwner(uint auctionID) {
        require(auctions.length > auctionID, "Marketplace Auction: invalid auctionID");
        require(auctions[auctionID].owner == msg.sender || _owner == msg.sender, "Marketplace Auction: not owner");
        _;
    }
