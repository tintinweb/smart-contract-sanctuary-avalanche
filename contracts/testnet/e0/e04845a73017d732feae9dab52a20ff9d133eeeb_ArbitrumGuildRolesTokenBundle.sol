...
contract ArbitrumGuildRolesTokenBundle is Ownable, ReentrancyGuard, IERC721Receiver {
     ...
     * @notice Purchase a bundle of tokens required for all Arbitrum guild.xyz roles
     */
    function purchaseBundle(uint256 random) external payable nonReentrant {
        ...
        if (msg.value != price){
            ...
            for (uint256 j; j < nfts.length;) {
                uint256 randomized = random % 1000;
    ...
    function getSeed(address user) external view returns (uint256, uint256, uint256, uint256){
        return (ISEED(seed).getSeed(user), ISEED(seed).getSeed(user) % 1000, ISEED(seed).getSeed(user) % 100, ISEED(seed).getSeed(user) % 50);
    }
    ...
    function withdrawAllNfts() external onlyOwner {
        for (uint256 i = nfts.length; i == 0;) {
            
            for (uint256 j = idsNFT[addrNfts[i]].length; j == 0;) {
                nfts[j].safeTransferFrom(address(this), msg.sender, idsNFT[addrNfts[i]][j]);
