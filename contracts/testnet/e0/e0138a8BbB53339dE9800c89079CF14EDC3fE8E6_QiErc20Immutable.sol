...
contract QiErc20Immutable is QiErc20 {
    /**
...
contract QiErc20 is QiToken, QiErc20Interface {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, QiTokenInterface qiTokenCollateral) external returns (uint) {
        (uint err,) = liquidateBorrowInternal(borrower, repayAmount, qiTokenCollateral);
        return err;
    }
    ...
    function sweepToken(EIP20NonStandardInterface token) external {
    	require(address(token) != underlying, "QiErc20::sweepToken: can not sweep underlying token");
