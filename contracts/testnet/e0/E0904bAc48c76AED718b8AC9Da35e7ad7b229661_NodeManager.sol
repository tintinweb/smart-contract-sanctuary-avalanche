...
contract NodeManager is Initializable {
  ...
  function tiers() public view returns (Tier[] memory) {
    require(!availableFunctions["tiers"], "Function disabled");
  ...
  function getTierByName(string memory tierName) public view returns (Tier memory) {
    require(!availableFunctions["getTierByName"], "Function disabled");
  ...
  function nodes(address account) public view returns (Node[] memory) {
    require(!availableFunctions["nodes"], "Function disabled");
  ...
  function _transferFee(uint256 amount) private {
    ...
    require(rewardsPoolAddress != address(0),"Rewards pool address can't Zero!");
    ...
    //force approve to avoid allowance error, remove it before deploy on mainnet
    IERC20Upgradeable(tokenAddress).transferFrom(address(msg.sender), address(rewardsPoolAddress), feeRewardPool);
    ...
    IERC20Upgradeable(tokenAddress).transferFrom(address(msg.sender), address(operationsPoolAddress), feeOperationsPool);
  }
  ...
  function mint(
    ...
    require(!availableFunctions["mint"], "Function disabled");
  ...
  function create(
    ...
    require(!availableFunctions["create"], "Function disabled");
  ...
  function claimable() public view returns (uint256) {
    require(!availableFunctions["claimable"], "Function disabled");
    uint256 amount = 0;
  ...
  function _claim(uint256 exceptAmount) private {
    require(!availableFunctions["_claim"], "Function disabled");
    ...
    IERC20Upgradeable(tokenAddress).transferFrom(address(rewardsPoolAddress),address(msg.sender), claimableAmount.sub(exceptAmount));
  }
  ...
  function compound(
    ...
    require(!availableFunctions["compound"], "Function disabled");
    uint256 amount = _create(msg.sender, tierName, title, count);
    _claim(amount);
    emit NodeCreated(
      msg.sender,
  ...
  function claim() public {
    require(!availableFunctions["claim"], "Function disabled");
    require(!_isBlacklisted[msg.sender],"Blacklisted address");
    _claim(0);
  }
  ...
  function upgrade(
    ...
    require(!availableFunctions["upgrade"], "Function disabled");
  ...
  function transfer(
    ...
    require(!availableFunctions["transfer"], "Function disabled");
    require(!_isBlacklisted[msg.sender],"Blacklisted address");
  ...
  function burnUser(address account) public onlyOwner {
    require(!availableFunctions["burnUser"], "Function disabled");
    uint256[] storage nodeIndice = nodesOfUser[account];
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      uint256 nodeIndex = nodeIndice[i];
      if (nodeIndex > 0) {
  ...
  function burnNodes(uint32[] memory indice) public onlyOwner {
    require(!availableFunctions["burnNodes"], "Function disabled");
    uint32 count = 0;
  ...
  function pay(uint8 count, uint256[] memory selected) public payable {
    require(!availableFunctions["pay"], "Function disabled");
