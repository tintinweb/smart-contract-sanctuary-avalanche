...
contract MultipleDepositsSplashpads {
   ...
   IERC20 public lpTokenContractAddress;
    ...
    function calculateRewards(uint256 _deposits, uint256 _atTime, uint256 _claimTime) private pure returns(uint256 _rewards){
        ...
        if(_timeElasped > 0){
            _rewards = _deposits * _timeElasped / 10 seconds * 1 / 100;
        }
        if(_timeElasped >= 1 * _bracket){
            _rewards += _deposits * (_timeElasped - _bracket) / 10 seconds * 2 / 100 - _deposits * (_timeElasped - _bracket) / 10 seconds * 1 / 100;
        }
        if(_timeElasped >= 2 * _bracket){
            _rewards += _deposits * (_timeElasped - 2 * _bracket) / 10 seconds * 3 / 100 - _deposits * (_timeElasped - 2 * _bracket) / 10 seconds * 2 / 100;
        }
    }
