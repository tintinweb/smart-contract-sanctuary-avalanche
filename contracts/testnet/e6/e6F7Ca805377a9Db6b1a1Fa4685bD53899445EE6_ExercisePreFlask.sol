...
interface ICirculatingFLASK{
    function FLASKCirculatingSupply() external view returns ( uint );
}

contract ExercisePreFlask is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    address public immutable pFLASK;
    address public immutable FLASK;
    address public immutable DAI;
    address public immutable treasury;
    address public immutable circulatingFLASKContract;
    ...
    constructor( address _pFLASK, address _flask, address _dai, address _treasury, address _circulatingFLASKContract ) {
        require( _pFLASK != address(0) );
        pFLASK = _pFLASK;
        require( _flask != address(0) );
        FLASK = _flask;
        require( _dai != address(0) );
        DAI = _dai;
        require( _treasury != address(0) );
        treasury = _treasury;
        require( _circulatingFLASKContract != address(0) );
        circulatingFLASKContract = _circulatingFLASKContract;
    }
    ...
    // Allows wallet to redeem pFLASK for FLASK
    function exercise( uint _amount ) external returns ( bool ) {
        ...
        IPreFlask( pFLASK ).burnFrom( msg.sender, _amount );

        IERC20( DAI ).approve( treasury, _amount );
        uint FLASKToSend = ITreasury( treasury ).deposit( _amount, DAI, 0 );

        terms[ msg.sender ].claimed = info.claimed.add( _amount );

        IERC20( FLASK ).safeTransfer( msg.sender, FLASKToSend );
    ...
    function redeemable( Term memory _info ) internal view returns ( uint ) {
        return ( ICirculatingFLASK( circulatingFLASKContract ).FLASKCirculatingSupply().mul( _info.percent ).mul( 1000 ) ).sub( _info.claimed );
    }
