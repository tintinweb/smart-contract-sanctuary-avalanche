...
contract CyberGods_Game is Ownable, Pausable {
    struct Player {
        // Troops
        uint256[4] warriorsByTier; // [0] cyborgs [1] mechas [2] beasts [3] demigods
        uint256[4] defensesByTier; // [0] fences [1] militians [2] protectors [3] zeus lightnings
        uint256[2] upgradesByTier; // [0] hydras
        uint256 attackPower; // represents the attack power given by warriors
        uint256 defensePower; // represents the defense power given by defenses
        uint256 shares; // Current shares (attackPower + defensePower). Shares go back to 0 once ROI is reached
        uint256 tier; // current tier based on the shares, on ranks[]
        // Buildings
        uint256[4] buildings; // [0] excavator [1] garage [2] radar [3] robbery school
        // Stats
        uint256[4] defensesStats; // [0] = count of defenses, [1] = won defenses, [2] = lost defenses, [3] = total avax lose
        uint256[4] attacksStats; // [0] = count of attacks, [1] = won attacks, [2] = lost attacks, [3] = total avax won
        // Shares & Rewards
        uint256 totalLostSinceLastClaim; // amount of avax lost to fights since last claim
        uint256 totalWonSinceLastClaim; // amount of avax won from fights since last claim
        uint256 totalClaimed; // total avax claimed ever
        uint256 lastClaimTime; // Time of last claim
        uint256 lastClaimEpoch; // epoch when last claim
        uint256 nextClaimTime; // Time when claim unlocks
        uint256 attackUnlocksTime; // Time for your next attack
        bool isROI; // Indicates if player has ROId
    }

    struct EpochSnapshot {
        uint256 timestamp; // timestamp of the snapshot
        uint256 totalShares; // total shares at snapshot
        uint256 totalRewards; // rewards to distribute this epoch
    }

    // Events
    event AttackResult(address winner, address loser, address engager); // Emited when a player attacks another one

    // Constants
    uint256 private constant EXCAVATOR_ID = 0;
    uint256 private constant GARAGE_ID = 1;
    ...
    uint256[2] public upgradesAvaxPriceTiers = [5 ether, 5 ether];

    // Supply of Troops & Upgrades
    uint256[4] public warriorsMaxSupplyTiers = [10000, 7500, 5000, 2500];
    uint256[4] public defensesMaxSupplyTiers = [10000, 7500, 5000, 2500];
    ...
    // In hours, how often can a user claim
    ...
    uint256[6] private garageLevelValues = [
        ...
        2 minutes,
        1 minutes
    ]; // In hours, how long it takes to rest NFTs
    uint256[2] private radarLevelValues = [0, 1]; // can see or not player's upgrades
    uint256[4] private robberySchoolLevelValues = [10, 30, 40, 50]; // In %, amount you steal from an enemy for a won attack

    uint256[3] private excavatorLevelPrices = [0.1 ether, 0.1 ether, 0.1 ether];
    ...
    uint256[1] private radarLevelPrices = [0.1 ether];
    uint256[3] private robberySchoolLevelPrices = [1 ether, 1 ether, 1 ether];

    // Mappings
    mapping(address => Player) private players; // Map of Players
    address[] private playersAddress; // Array of address, used for quickly looping over accounts when retrieving players in a tier
    mapping(uint256 => uint256) private playersInTiers; // mapping of player count per tier
    mapping(address => string) public playersToUsername; // mapping of players to their username
    mapping(uint256 => EpochSnapshot) public epochSnapshots; // mapping of indexes to snapshots

    // No modifier
    uint256 public totalPlayers = 0; // Total Players in Game
    uint256 public totalShares = 0; // Total Shares of all Players
    uint256 public currentEpoch = 0; // Current Epoch
    uint256 public nextEpochTime = 0; // Next Epoch Time
    uint256 public totalRewardsPending = 0; // Total Unclaimed rewards
    uint256 public totalRewardsClaimed = 0; // Total Claimed rewards
    uint256 public totalRewards = 0; // Total rewards distributed since Epoch 1

    // Editable
    uint256 public changeUsernamePrice = 250000000000000000; // Cost to change username in game
    uint256 public maxROI = 150; // % of deposits of a player to ROI
    uint256 public maxDailyAvaxPerShare = 7500000000000000; // even if pool can sustain a higher APR, limit to this amount of Avax per share per day
    uint256 public runwayInDays = 90; // when pool cannot sustain 0.0075 avax per day anymore, the APR is calculated on a 90 days runway
    uint256 public epochsPerDay = 8; // number of epochs per day
    ...
    uint256 public adesBloodBoost = 2; // The boost provided to each Demi Gods by Ades Blood upgrade

    // Contracts
    address public feesManagerAddress; // Fees Manager Contract Address
    ...
    function adminSetAdesBloodBoost(uint256 _boost) external onlyOwner {
        adesBloodBoost = _boost;
    }

    // Force changes username of a player
    // _address: the player's address
    // _username: the username
    function adminChangeUsername(address _address, string memory _username)
        external
        onlyOwner
    {
    ...
    // Get the count of players in a tier
    // _tier: the tier value
    function getPlayerCountInTier(uint256 _tier)
        external
        view
        returns (uint256)
    {
        return playersInTiers[_tier];
    }
    ...
    // Getters
    // Get ranks range
    function getRanks() external view returns (uint256[] memory) {
        return ranks;
    }

    // Get Username of a player
    // _playerAddress: the player address
    function getUsername(address _playerAddress)
        external
        view
        returns (string memory)
    {
        return playersToUsername[_playerAddress];
    }
    ...
    function getPlayerInformation() external view returns (Player memory) {
        return players[msg.sender];
    }

    // Get the caller's warriors
    function getPlayerWarriors() external view returns (uint256[4] memory) {
        return players[msg.sender].warriorsByTier;
    }

    // Get the caller's defenses
    function getPlayerDefenses() external view returns (uint256[4] memory) {
        return players[msg.sender].defensesByTier;
    }

    // Get the caller's upgrades
    function getPlayerUpgrades() external view returns (uint256[2] memory) {
        return players[msg.sender].upgradesByTier;
    }
    ...
    function getAttackUnlocksTime(address _player)
        ...
        returns (uint256)
    {
        return players[_player].attackUnlocksTime;
    }
    ...
    // Return a player's buildings
    // _player: the player address
    function getBuildingsForPlayer(address _player)
        public
        view
        returns (uint256[4] memory)
    {
        return players[_player].buildings;
    }

    // Process an epoch and calculate the rewards to be distributed
    function adminProcessEpoch() external whenNotPaused onlyOwner {
        uint256 avaxInPoolMinusRewards = getPoolBalanceMinusRewards();
        ...
        totalRewardsPending += totalRewardsThisEpoch;
        totalRewards += totalRewardsThisEpoch;

        nextEpochTime = block.timestamp + (86400 / epochsPerDay);
        ...
        whenNotPaused
    {
        ...
        if (tempEmptyStringTest.length == 0) {
            delete playersToUsername[msg.sender];
        } else {
            playersToUsername[msg.sender] = _username;
        }
    }
    ...
    function buildingUpgrade(uint256 _buildingId, uint256 _toLevel)
        external
        payable
        whenNotPaused
    {
        ...
        uint256 currentBuildingLevel = players[msg.sender].buildings[
            _buildingId
        ];
        ...
        players[msg.sender].buildings[_buildingId] = _toLevel;
        // (bool sent, ) = payable(feesManagerAddress).call{value: msg.value}("");
        ...
        returns (PlayerInTier[] memory)
    {
        ...
        for (uint256 i = 0; i < playersAddress.length; i++) {
            address playerAddress = playersAddress[i];
            // Verify that the player isn't the caller and that the player is in the same tier

            if (
                (players[playerAddress].tier >= _minRange &&
                    players[playerAddress].tier <= _maxRange)
            ) {
                PlayerInTier memory pit = PlayerInTier(
                    playerAddress,
                    playersToUsername[playerAddress],
                    getPlayerRewards(playerAddress),
                    players[playerAddress].defensePower,
                    players[msg.sender].buildings[2] > 0
                        ? players[playerAddress].upgradesByTier[0]
                        : 0
                );
                playersInTier[y] = pit;
                y++;
    ...
    function attackPlayer(address _playerAddress) external whenNotPaused {
        require(
            players[msg.sender].attackUnlocksTime < block.timestamp,
            "Cannot attack yet"
        );

        require(msg.sender != _playerAddress, "Cannot attack this player");

        uint256 attackPower = players[msg.sender].attackPower;
        uint256 defensePower = players[_playerAddress].defensePower;

        // Verify if the defender has Hydras
        if (players[_playerAddress].upgradesByTier[0] > 0) {
            defensePower += (players[_playerAddress].upgradesByTier[0] *
                (upgradesAvaxPriceTiers[0] / MULTIPLIER));
        }

        // Verify if the attacker has a blood of ares && Demi Gods to give extra attack power to demi gods
        if (
            players[msg.sender].upgradesByTier[1] > 0 &&
            players[msg.sender].warriorsByTier[3] > 0
        ) {
            attackPower += (players[msg.sender].warriorsByTier[3] *
                adesBloodBoost);
        }

        players[msg.sender].attacksStats[0]++;
        // Set next attack timer based on caller's Garage level
        players[msg.sender].attackUnlocksTime =
            block.timestamp +
            garageLevelValues[players[msg.sender].buildings[1]];

        players[_playerAddress].defensesStats[0]++;
        if (attackPower > defensePower) {
            // Calculate how much rewards the attacker gets based on its Robbery School level
            uint256 defenserRewards = (getPlayerRewards(_playerAddress) *
                robberySchoolLevelValues[players[msg.sender].buildings[3]]) /
                100;

            players[msg.sender].totalWonSinceLastClaim += defenserRewards;
            players[msg.sender].attacksStats[1]++;
            players[msg.sender].attacksStats[3] += defenserRewards;

            players[_playerAddress].totalLostSinceLastClaim += defenserRewards;
            players[_playerAddress].defensesStats[2]++;
            players[_playerAddress].defensesStats[3] += defenserRewards;
            emit AttackResult(msg.sender, _playerAddress, msg.sender);
        } else {
            players[msg.sender].attacksStats[2]++;
            players[_playerAddress].defensesStats[1]++;
            emit AttackResult(_playerAddress, msg.sender, msg.sender);
        }
    }

    // Set the tier of a player
    // _toPlayer: the player address
    // isNewPlayer: defines if the player is new to the game
    function _setTier(address _toPlayer, bool isNewPlayer) private {
        // If the player isn't new
        // Decrement the count of players in the player's tier
        if (!isNewPlayer) {
            playersInTiers[players[_toPlayer].tier]--;
        }

        uint256 playerShare = players[_toPlayer].shares;
    ...
    function _recruitRequires(
        ...
        if (_type == 0) {
            ...
            require(
                players[_toPlayer].upgradesByTier[_tier] + _qty <=
    ...
    function adminGiftRecruitTroops(
        ...
        _recruitRequires(_type, _tier, _qty, _toPlayer, true);

        if (_type == 0) {
            _recruitWarriors(_tier, _qty, _toPlayer);
        } else if (_type == 1) {
            _recruitDefenses(_tier, _qty, _toPlayer);
        } else if (_type == 2) {
            _recruitUpgrades(_tier, _qty, _toPlayer);
        }
    }
    ...
    function recruitTroops(
        ...
        _recruitRequires(_type, _tier, _qty, msg.sender, false);
    ...
    function _recruitWarriors(
        ...
        warriorsInUseByTiers[_tier] += _qty;
        players[_toPlayer].warriorsByTier[_tier] += _qty;
        uint256 additionalAttackPower = (warriorsAvaxPriceTiers[_tier] * _qty) /
            MULTIPLIER;

        bool isNewPlayer = players[_toPlayer].shares == 0;
        players[_toPlayer].attackPower += additionalAttackPower;
        players[_toPlayer].shares += additionalAttackPower;
        // If the buyer has ROId
        // Reset its ROI value
        // and adds back its shares to the total shares
        if (players[_toPlayer].isROI) {
            players[_toPlayer].isROI = false;
            totalShares += (players[_toPlayer].shares + additionalAttackPower);
        } else {
            totalShares += additionalAttackPower;
        }

        // if new player
        // creates a new player, adds the address to the list of players
        if (isNewPlayer) {
            playersAddress.push(_toPlayer);
            totalPlayers++;
            players[_toPlayer].lastClaimEpoch = currentEpoch;
            players[_toPlayer].attackUnlocksTime = block.timestamp;
            _setTier(_toPlayer, true);
        } else {
            _setTier(_toPlayer, false);
        }
    }
    ...
    function _recruitDefenses(
        ...
        defensesInUseByTiers[_tier] += _qty;
        players[_toPlayer].defensesByTier[_tier] += _qty;
        uint256 additionalDefensePower = (defensesAvaxPriceTiers[_tier] *
            _qty) / MULTIPLIER;

        bool isNewPlayer = players[_toPlayer].shares == 0;
        players[_toPlayer].defensePower += additionalDefensePower;
        players[_toPlayer].shares += additionalDefensePower;
        // If the buyer has ROId
        // Reset its ROI value
        // and adds back its shares to the total shares
        if (players[_toPlayer].isROI) {
            players[_toPlayer].isROI = false;
            totalShares += (players[_toPlayer].shares + additionalDefensePower);
        } else {
            totalShares += additionalDefensePower;
        }
    ...
    function _recruitUpgrades(
        ...
        // if buying an hydra
        if (_tier == 0) {
            players[_toPlayer].shares +=
                (upgradesAvaxPriceTiers[_tier] * _qty) /
                MULTIPLIER;
            ...
            if (players[_toPlayer].isROI) {
                ...
                totalShares +=
                    players[_toPlayer].shares +
                    ((upgradesAvaxPriceTiers[_tier] * _qty) / MULTIPLIER);
            } else {
                totalShares +=
                    (upgradesAvaxPriceTiers[_tier] * _qty) /
                    MULTIPLIER;
    ...
    function getPlayerRewards(address _player) public view returns (uint256) {
        ...
        if (players[_player].isROI == true) {
            return
                players[_player].totalWonSinceLastClaim -
                players[_player].totalLostSinceLastClaim;
        }

        uint256 totalRewardsPerShare = 0;
        for (
            uint256 i = players[_player].lastClaimEpoch;
            i < currentEpoch;
            i++
        ) {
            totalRewardsPerShare +=
                epochSnapshots[i].totalRewards /
                epochSnapshots[i].totalShares;
        }
        return
            (players[_player].shares * totalRewardsPerShare) +
            players[_player].totalWonSinceLastClaim -
            players[_player].totalLostSinceLastClaim;
    }
    ...
    function _claim(address _player) internal {
        ...
        if (
            ...
            players[msg.sender].nextClaimTime < block.timestamp
        ) {
            uint256 rewards = getPlayerRewards(_player);
            if (rewards > getPoolBalanceMinusRewards()) {
                rewards = getPoolBalanceMinusRewards();
            }

            // Verify if the user is going over ROI on this claim
            if (
                players[_player].shares != 0 &&
                (rewards + players[_player].totalClaimed >
                    ((players[_player].shares * maxROI * MULTIPLIER) / 100))
            ) {
                players[_player].isROI = true;
                // decrement totalShares so player is out of calculation in rewards of each epoch
                totalShares -= players[_player].shares;
                // forces rewards to be what the user can claim to reach max ROI
                rewards =
                    ((players[_player].shares * maxROI * MULTIPLIER) / 100) -
                    players[_player].totalClaimed;
            }
            ...
            players[_player].nextClaimTime =
                block.timestamp +
                excavatorLevelValues[players[_player].buildings[0]];
            players[_player].lastClaimEpoch = currentEpoch;
    ...
    function claimRewards() external whenNotPaused {
        ...
        require(players[msg.sender].shares > 0, "No share");
