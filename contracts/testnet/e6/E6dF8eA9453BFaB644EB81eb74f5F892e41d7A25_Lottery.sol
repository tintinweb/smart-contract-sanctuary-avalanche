...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	using Queue for Queue.Address32Deque;
	...
	uint256 public maxTicketsToBuy = 10;

	Queue.Address32Deque private leadersQueue;
	...
	uint256 public royaltiesTicketsThreshold = 10; //how many tickets you should buy before you get royalties
	...
	event TicketsBought(address indexed user, uint256 quantity);
	...
	/**
		...
		@param qty how many tickets you want to buy
	 */
		...
		nonReentrant
	{
		...
		require(msg.value == ticketPrice * qty, "invalid price");
		require(_msgSender() != affiliateAddress, "invalid affiliate");

		uint256 affiliateReward = _calcPercentage(ticketPrice * qty, affiliateRewardsFee);
		...
		uint256 royaltiesAmount = _calcPercentage(ticketPrice * qty, royaltiesFee); //10%
		_royaltiesDeposits = _royaltiesDeposits + royaltiesAmount;

		uint256 insuranceBuyBackAmount = _calcPercentage(ticketPrice * qty, insuranceBuyBackFee); //15%
		...
		if (underRoyaltiesTickets[_msgSender()] + qty >= royaltiesTicketsThreshold * phase) {
		...
		if (boughtTickets[_msgSender()] + qty < royaltiesTicketsThreshold * phase) {
			underRoyaltiesTickets[_msgSender()] = underRoyaltiesTickets[_msgSender()] + qty;
		}
		...
		if (block.timestamp + qty * newTicketStep[phase] < roundMaxTime[phase]) {
			currentRound.endTime = currentRound.endTime + qty * newTicketStep[phase];
		}

		//pop the queue
		for (uint256 i = 0; i < qty; i++) {
			if (!leadersQueue.empty()) {
				leadersQueue.popBack();
			}
		}

		//add the buyer to the leaderQueue
		for (uint256 i = 0; i < qty; i++) {
			pushFront(_msgSender());
		}

		emit TicketsBought(_msgSender(), qty);
	}
	...
	function setTicketPrice(uint256 maxTickets, uint256 newPrice) external onlyOwner {
	...
	function pushFront(address value) internal {
		leadersQueue.pushFront(value);
	}

	//pops an item from the queue
	function popBack() internal returns (address) {
		address value = leadersQueue.popBack();
		return value;
	}

	//clears the queue
	function clearLeadersQueue() internal {
		leadersQueue.clear();
	}
	...
	function startNextRound() internal {
		...
		roundMaxTime[1] = block.timestamp + roundMaxTime[1];
		roundMaxTime[2] = block.timestamp + roundMaxTime[2];
		roundMaxTime[3] = block.timestamp + roundMaxTime[3];
		roundMaxTime[4] = block.timestamp + roundMaxTime[4];
		roundMaxTime[5] = block.timestamp + roundMaxTime[5];
		roundMaxTime[6] = block.timestamp + roundMaxTime[6];
	}
...
// SPDX-License-Identifier: MIT
// based on OpenZeppelin Contracts (last updated v4.6.0) (utils/structs/DoubleEndedQueue.sol)
pragma solidity 0.8.14;
...
library Queue {
	 ...
	 */
	function popBack(Address32Deque storage deque) internal returns (address value) {
	 ...
	 */
	function pushFront(Address32Deque storage deque, address value) internal {
		int128 frontIndex;
		unchecked {
	 ...
	 * Reverts with `Empty` if the queue is empty.
	 */
	function popFront(Address32Deque storage deque) internal returns (address value) {
		if (empty(deque)) revert Empty();
	 ...
	 */
	function clear(Address32Deque storage deque) internal {
		deque._begin = 0;
		deque._end = 0;
	}
	 ...
	 */
	function empty(Address32Deque storage deque) internal view returns (bool) {
	 ...
	 * Reverts with `Empty` if the queue is empty.
	 */
	function back(Address32Deque storage deque) internal view returns (address value) {
		if (empty(deque)) revert Empty();
		int128 backIndex;
		unchecked {
			backIndex = deque._end - 1;
