...
contract beefstake is ERC1155Holder {
    ...
    function getem(address _account) public view returns (uint256[4] memory) {
    	
  	uint256 timeElapsed = block.timestamp - stakesMapping[_account].timestamp;
        uint256 bal1 = NFT.balanceOf(_account, uint(1));
        uint256 bal2 = NFT.balanceOf(_account, uint(2));
        uint256 bal3 = NFT.balanceOf(_account, uint(3));
        return [bal1,bal2,bal3,timeElapsed];
    }
