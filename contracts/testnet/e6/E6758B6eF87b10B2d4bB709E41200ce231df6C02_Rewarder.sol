...
contract Rewarder is Ownable {
    ...
    mapping(string => uint256) private countryIndex;

    address public REWARD_TOKEN = 0x7B69177de7C6C2939386030F424b51c920ca6b29;
    address public SIGNER_WALLET = 0x977CdF0aCb3d1307C1daf2ddcf3f1c94c5Fb85dA;
    ...
    function finalizePrizeMultiOwner(Prize[] memory _prizes) public onlyOwner {
        for (uint i = 0; i < _prizes.length; i++) {
            Prize memory prize = _prizes[i];
            ...
            reward.country = prize.country;
            reward.byOwner = true;
            reward.amountClaimable = prize.amount;
    ...
    function claimPrizeOwnerTo(address _recipient, address _to) public onlyOwner {
        (
            uint256 amountMint,
            uint256 amountBalance,
            uint256 amountTotal,
        ...
        if (amountMintClaimable > 0) {
            IRewardToken rt = IRewardToken(REWARD_TOKEN);
            rt.mint(_to, amountMintClaimable);
        ...
        if (amountBalanceClaimable > 0) {
            IERC20 rt = IERC20(REWARD_TOKEN);
            rt.safeTransfer(_to, amountBalanceClaimable);
    ...
    function addCountries(Country[] memory _countries) public onlyOwner {
        for (uint i = 0; i < _countries.length; i++) {
            string memory code = _countries[i].code;
            uint256 index = countryIndex[code];

            if (index == 0) {
                countryIndex[code] = COUNTRIES.length + 1;
                COUNTRIES.push(
                    Country(code, _countries[i].ratio)
                );
            } else {
                COUNTRIES[index - 1].ratio = _countries[i].ratio;
            }
        }
    }
    ...
    function getNextRewardPeriod()
        public
        view
        returns (NextRewardPeriod memory)
    {
        return NEXTREWARDPERIOD;
    }

    function recoverTokenBalance() public onlyOwner {
        IERC20 token = IERC20(REWARD_TOKEN);
        uint256 balance = token.balanceOf(address(this));

        require(
            balance > 0,
            "Rewarder contract does not have recoverable amount."
        );
