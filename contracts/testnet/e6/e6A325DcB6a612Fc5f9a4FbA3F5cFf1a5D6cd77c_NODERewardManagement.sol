...
contract NODERewardManagement {
    ...
    function createNodes(
        ...
        uint userNewNodeCount = nodeNames.length;
        NodeEntity[] storage nodesList = _nodesOfUser[account];

        for (uint256 i = 0; i < userNewNodeCount; i++) {
            nodesList.push(NodeEntity({
                name: nodeNames[i],
        ...
        _nodesOfUser[account] = nodesList;

        nodeOwners.set(account, nodesList.length);
