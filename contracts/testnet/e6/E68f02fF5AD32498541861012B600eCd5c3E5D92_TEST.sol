...
interface Cashier {
    ...
    function gibAvax(address hobo) external;
...
contract TEST is IERC20 {
    ...
    struct TaxWallets {
        ...
        address payable veTreasury;
        address payable dev;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xDC6a06a689a0DCf8C3B49B65B209271914b54E5b),
        veTreasury: payable(0xDC6a06a689a0DCf8C3B49B65B209271914b54E5b),
        dev: payable(0xDC6a06a689a0DCf8C3B49B65B209271914b54E5b)
        });
    ...
    constructor () payable {
        ...
        if (block.chainid == 56) {
            ...
            dexRouter = IRouter02(0x2D99ABD9008Dc933ff5c0CD271B88309593aB921);
        } else {
            revert();
        }
...
//===============================================================================================================
    ...
    function setRatios(uint16 rewards, uint16 liquidity, uint16 marketing, uint16 vep, uint16 dev) external onlyOwner {
        _ratios.rewards = rewards;
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.veTreasury = vep;
        _ratios.dev = dev;
        _ratios.total = rewards + liquidity + marketing + vep + dev;
    }

    function setWallets(address payable marketing, address payable vep) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.veTreasury = payable(vep);
    }
    ...
    function gibAvax() external {
        reflector.gibAvax(msg.sender);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal swapping {
        ...
        uint256 vepBalance = (amtBalance * ratios.veTreasury) / ratios.total;
        uint256 devBalance = (amtBalance * ratios.dev) / ratios.total;
        uint256 marketingBalance = amtBalance - (rewardsBalance + vepBalance + devBalance);

        try reflector.load{value: rewardsBalance}() {} catch {}

        if(ratios.veTreasury > 0){
            _taxWallets.veTreasury.transfer(vepBalance);
        }
        if(ratios.dev > 0){
