...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "MY TEST TOKEN";
    string private _symbol = "MTT";
    uint8 private _decimals = 18;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 5000 * 10**6 * 10 ** uint256(_decimals);
    ...
    function _distributeFee(uint256 rFee, uint256 rBurn, uint256 rTreasury, uint256 tFee, uint256 tBurn, uint256 tTreasury) private {
        _rTotal -= (rFee + rBurn);
        _tFeeTotal += tFee;
        _tTotal -= tBurn;
        _tBurnTotal += tBurn;
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tBurn - tTreasury;
        return TValuesStruct(tTransferAmount, tFee, tBurn, tTreasury);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tTreasury, uint256 currentRate) private view returns (RValuesStruct memory) {
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rBurn = tBurn * currentRate;
        uint256 rTreasury = tTreasury * currentRate;
        uint256 rTransferAmount = rAmount - rFee - rBurn - rTreasury;
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rTreasury);
    }
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, treasury fee
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rFee, vs.rBurn, vs.rTreasury, vs.tFee, vs.tBurn, vs.tTreasury);
    ...
    //Call this function after the presale
    ...
    //Call this function before the presale
