 ...
 */
contract MultiwalletContractUpgradeable is Initializable, UUPSUpgradeable, OwnableUpgradeable, Blacklistable {

    //Enum for RecipientType as in Marketplace.
    ...
    mapping(RecipientType => mapping(address => uint256)) private _multiwallet;

    /// @dev mapping from Recipient Type => Recipient's Address => Recipient's claimable balance.
    mapping(RecipientType => mapping(address => uint256)) private _claim;
     ...
     */
    function initialize(IERC20 arkiusTokenAddress, address blacklistContractAddress, address multisigAddress) public initializer {
     ...
     */
    function multiwallet(address mAddress, RecipientType mType, uint256 amount) external onlyAllowed notBlacklisted {
     ...
     */
    function multiwalletTransfer(address fromAddress, RecipientType fromType, address toAddress, RecipientType toType, uint256 amount) external onlyAllowed notBlacklisted {
     ...
     */
    function deMultiwallet(address mAddress, RecipientType mType, uint256 amount) external onlyAllowed notBlacklisted {
        _multiwallet[mType][mAddress] = _multiwallet[mType][mAddress] - amount;
     ...
     * @param amount   Amount to be deMultiwalleted(made claimable).
     */
    function unMultiwallet(address mAddress, RecipientType mType, uint256 amount) external onlyAllowed notBlacklisted {
        _multiwallet[mType][mAddress] = _multiwallet[mType][mAddress] - amount;
     ...
     */
    function claim(RecipientType recipientType) external notBlacklisted {
    ...
    function multiwalletBalance(RecipientType recipient, address add) public view returns(uint256) {
        return _multiwallet[recipient][add];
    }

    function claimableBalance(RecipientType recipient, address add) public view returns(uint256) {
