...
import "./ERC20WithTaxUpgradeable.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract ETFToken is ERC20WithTaxUpgradeable, IETFToken, BNum {
    uint256 public constant MIN_BALANCE = 1e6;
    uint256 public constant INIT_POOL_SUPPLY = 1e18;

    uint256 public minBoundTokens;
    uint256 public maxBoundTokens;
    ...
    event TokenAdded(address indexed token, uint256 minimumBalance);
    event TokenReady(address indexed token);

    function __ETFToken__init(
        string memory _name,
        ...
        require(
            ...
            "ETFToken: Fee recipient is zero address"
        );
        minBoundTokens = 2;
        maxBoundTokens = 10;
        exitFee = 1e16;
    ...
    function initialize(
        ...
        require(tokens.length == 0, "ETFToken: Already initialized");
        uint256 len = _tokens.length;
        require(len >= minBoundTokens, "ETFToken: Min bound tokens overflow");
        require(len <= maxBoundTokens, "ETFToken: Max bound tokens overflow");
        require(balances.length == len, "ETFToken: Invalid arrays length");
        for (uint256 i = 0; i < len; i++) {
            address token = _tokens[i];
            uint256 balance = balances[i];
            require(balance >= MIN_BALANCE, "ETFToken: Min balance overflow");
        ...
        _mint(tokenProvider, INIT_POOL_SUPPLY);
    }

    function addTokenAsset(address token, uint256 minimumBalance, uint256 balance)
        public
        onlyOperator
    {
        require(!records[token].bound, "ETFToken: Token already bound");
        require(
            minimumBalance >= MIN_BALANCE,
            "ETFToken: Min balance overflow"
        );
        records[token] = Record({
            bound: true,
            ready: false,
            index: uint8(tokens.length),
            balance: balance
        });
        tokens.push(token);
        _pullUnderlying(token, msg.sender, balance);
        minimumBalances[token] = minimumBalance;
        emit TokenAdded(token, minimumBalance);
    }
    ...
    function removeTokenAsset(address token) public onlyOperator {
        ...
        if (index != last) {
            tokens[index] = tokens[last];
            records[tokens[index]].index = uint8(index);
        }
        tokens.pop();
        records[token] = Record({
            bound: false,
    ...
    function setMinBoundTokens(uint256 _minBoundTokens) public onlyOperator {
        require(
            _minBoundTokens > 1,
            "ETFToken: Min bound token must greater than 2"
        );
        minBoundTokens = _minBoundTokens;
    }

    function setMaxBoundTokens(uint256 _maxBoundTokens) public onlyOperator {
        require(
            _maxBoundTokens >= minBoundTokens,
            "ETFToken: Max bound token must greater than min bound tokens"
        );
        maxBoundTokens = _maxBoundTokens;
    }

    function setExitFee(uint256 _exitFee) public onlyOperator {
        require(_exitFee >= 1e16, "ETFToken: exitFee >= 1%");
        require(_exitFee <= 5 * 1e16, "ETFToken: exitFee <= 5%");
        exitFee = _exitFee;
    }
    ...
    function setMinimumBalance(address token, uint256 minimumBalance)
        ...
        onlyOperator
    {
        Record storage record = records[token];
        require(record.bound, "ETFToken: Token not bound");
        require(!record.ready, "ETFToken: Token already ready");
        minimumBalances[token] = minimumBalance;
    }
        ...
        public
    {
        ...
        for (uint256 i = 0; i < maxAmountsIn.length; i++) {
            ...
            require(tokenAmountIn != 0, "ETFToken: Token amount in is zero");
            require(
                tokenAmountIn <= maxAmountsIn[i],
                "ETFToken: Max amount in overflow"
            );
            _updateInputToken(t, record, badd(realBalance, tokenAmountIn));
            emit JoinPool(caller, t, tokenAmountIn);
            _pullUnderlying(t, caller, tokenAmountIn);
        }
        _mint(caller, poolAmountOut);
    }
        ...
        public
    {
        ...
        uint256 pAiAfterExitFee = bsub(poolAmountIn, _exitFee);
        ...
        for (uint256 i = 0; i < minAmountsOut.length; i++) {
            address t = tokens[i];
            Record memory record = records[t];
            if (record.ready) {
                uint256 tokenAmountOut = bmul(ratio, record.balance);
                require(
                    tokenAmountOut != 0,
                    "ETFToken: Token amount out is zero"
                );
                require(
                    tokenAmountOut >= minAmountsOut[i],
                    "ETFToken: Min amount out overflow"
                );
                records[t].balance = bsub(record.balance, tokenAmountOut);
                emit ExitPool(caller, t, tokenAmountOut, _exitFee);
        ...
        returns (Record memory record, uint256 realBalance)
    {
        ...
        if (!record.ready) {
            record.balance = minimumBalances[token];
        }
    }
}
 ...
 */
contract ERC20WithTaxUpgradeable is OperatorUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    // Tax
	function setTaxRate(uint256 _value) external onlyOperator {
        require(_value <= TAX_MULTIPLIER, "ERC20: tax rate over 100%");
        taxRate = _value;
    }

	function setTaxWallet(address _wallet) external onlyOperator {
        taxWallet = _wallet;
    }
	...
	function excludeTaxAddress(address _address) external onlyOperator {
        excludedTaxAddresses[_address] = true;
    }

    function includeTaxAddress(address _address) external onlyOperator {
	...
	function _transfer(
		...
		if (taxRate > 0) {
			...
			if(
                ...
                taxAmount = amount * taxRate / TAX_MULTIPLIER;
			}

			if(taxAmount > 0)
			{
				amount = amount - taxAmount;
				_transferBase(from, taxWallet, taxAmount);
			}
		}
