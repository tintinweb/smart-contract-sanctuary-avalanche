...
contract TrantorV2 is ERC20Upgradeable {
    ...
    address public tresuryPoolAddress;
    ...
    function updateSwapTokenAddress(address value) public onlyOwner{
        swapTokenAddress = value;
    }
   ...
   /**
     ...
     * Can only be called by the current operator.
     */
    function setPairAddress(address add1, address add2) public onlyOwner() {
        require(!availableFunctions["setPairAddress"], "Function disabled");
        lpPairs[uniswapV2Pair] = false;
        uniswapV2Pair = IUniswapV2Factory(dexRouter.factory()).getPair(add1, add2);
     ...
     * Can only be called by the current operator.
     */
    function setPairAddressV2(address add1) public onlyOwner() {
        require(!availableFunctions["setPairAddress"], "Function disabled");
        lpPairs[uniswapV2Pair] = false;
        uniswapV2Pair = address(add1);
        lpPairs[uniswapV2Pair] = true;
        emit uniswapV2PairUpdated(msg.sender, address(dexRouter), uniswapV2Pair);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from != owner && to != address(dexRouter) && to != address(uniswapV2Pair)) {
            if (to != address(dexRouter) && to != address(uniswapV2Pair) && !isExcludedFromFee[to]) {
