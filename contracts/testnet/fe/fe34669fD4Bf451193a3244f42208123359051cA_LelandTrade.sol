...
contract LelandTrade is ERC721Holder, Ownable, ILelandTrade {
    ...
    function upgradeCollection(
        ...
        _checkCollection(_tokenIds, _duplicateMode);

        for (uint256 i = 0; i < length; i++) {
            IERC721(lelandNFT).safeTransferFrom(sender, DEAD, _tokenIds[i]);
        }

        uint256[] memory tokenIds = depositedTokenIdsPerRarity[originRarity + 1]
            .values();
        for (uint256 i = 0; i < upgradeCollectionCnt; i++) {
            uint256 tokenId = tokenIds[i];
            depositedTokenIdsPerRarity[originRarity + 1].remove(tokenId);
            IERC721(lelandNFT).safeTransferFrom(address(this), sender, tokenId);
        }
    ...
    function _checkCollection(
        ...
        uint256 originRarity = collectionInfo[_tokenIds[0]].rarityType;
        for (uint256 i = 0; i < length; i++) {
            require(
