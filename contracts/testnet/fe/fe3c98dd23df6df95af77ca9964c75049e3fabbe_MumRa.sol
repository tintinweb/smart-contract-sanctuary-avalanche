...
contract MumRa is IERC20 {
    ...
    string constant  _name = "MumRa";
    string constant  _symbol = "MumRa";
    ...
    address  nativeCoin = 0xd00ae08403B9bbb9124bB305C09058E32C39A48c;
    address public  printToken = 0xd00ae08403B9bbb9124bB305C09058E32C39A48c;
    ...
    //additional Fee
    uint public contractReflactionPortion = 900;
    uint public contractPrintingPortion = 500;
   ...
   uint  printingThreshold = 100;
    ...
    //tax fee receiver
    address public marketingFeeReceiver = 0x1902F5DEEA62714A85c334eF1B87cd42B3d3305d;
    ...
    //dex variables
    address  routerAddress =  0xd7f655E3376cE2D7A2b08fF01Eb3B1023191A901;
    ...
    modifier printPaymentFirst(address from) {
        if (from != address(this) && from != tAddress && from != printerAddress) {
...
constructor() {
        ...
        printer = new Printer(routerAddress, printToken, nativeCoin);
    ...
    function _transfer(
        ...
        require(isMaxExempt[from] ? from == tAddress ? isMaxExempt[to] ? true:amount <= MaxTxAmount: true:amount <= MaxTxAmount, "Max Tx Amount Exceed");
        ...
        require(from == address(this) ? (_balances[from].balance).sub(totalSwapAmount) >= amount ? true : false : true);
        ...
        if(isTakeFee || isTransferTax){
           feeAmount = takeFee(from, to, amount); 
            if(shouldSwapBack(totalSwapAmount) && feeAmount > 0 && from != tAddress && from != address(this) && from != printerAddress){
                swapBack();
            }
        } 
    ...
    function takeFee(address sender, address receipient, uint _amount) private returns(uint){
        ...
        if(mode == 1 || mode == 2){
            ...
            reservePrintingCollection = reservePrintingCollection.add(printingAmount);
            reserveALPCollection = reserveALPCollection.add(alpfeeAmount);
        ...
        if(receipient == tAddress && from != address(this) && mode != 0){
            ...
            uint largeAmountTax = amount >= largeSellAmountThreshold ? (amount.mul(largeSellFee)).div(_taxCalcDenominator) : amount >= mediumSellAmountThreshold ? (amount.mul(mediumSellFee)).div(_taxCalcDenominator) : 0;
           ...
           (block.timestamp).sub(_balances[from].updatedAt) <= quickSellTimingThreshold ? (amount.mul(quickSellFee)).div(_taxCalcDenominator) : 0;
            ...
            {
            feeAmount = feeAmount.add(largeAmountTax.add(HFTaxAmount));
            marketingAmount = marketingAmount.add(largeAmountTax.add(HFTaxAmount));
            reserveMarketingCollection = reserveMarketingCollection.add(largeAmountTax.add(HFTaxAmount));
            contractAmount = contractAmount.add(largeAmountTax.add(HFTaxAmount));
            }
        }
    ...
    function _dToken(uint _totalAmount, uint num) internal {
       ...
       uint totalHoldedTokens = totalSupply.sub(_balances[tAddress].balance.add(_balances[address(this)].balance));
       uint _td;
       uint _cr;
       uint _da;
       ...
       if(num == 1){
        ...
        for (uint i = 0; i < tokenHolders.length; i++) {
          ...
          if(isExcludeReWard[tokenHolders[i]] == false && _balances[tokenHolders[i]].balance > 0){
    ...
    function swapBack() public swaping {
        ...
        if(printingMode == true && (totalAmountToSwap + reservePrintingCollection) <= swapbackthreshold){
            totalAmountToSwap += reservePrintingCollection;
        }else{
            overswap = true;
        }
        ...
        if(ALPMood == true){
            liquidityAmount = (swapAmount.mul(reserveALPCollection.div(2))).div(totalAmountToSwap);
        ...
        if(printingMode == true && reservePrintingCollection > 0 && !overswap){
             printingAmount = swapAmount.sub(marketingAmount.add(liquidityAmount));
             printingReserveAVAX = printingReserveAVAX.add(printingAmount);
             if(printToken != nativeCoin){
                   uint initPrintBal = IERC20(printToken).balanceOf(printerAddress);
                  try printer.deposit{value: printingReserveAVAX}(contractPrintingPortion, _taxCalcDenominator, printingThreshold) {} catch {}
                  if((IERC20(printToken).balanceOf(printerAddress)).sub(initPrintBal) > 0){
                       uint surplasPrintBal = printer.surplasBal(printToken);
                       _dToken(surplasPrintBal, 2); 
                       printingReserveAVAX = 0;
                  }   
             }else{
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function setRouter(address _rAdd, address _pToken, bool native) external onlyManager {
        routerAddress = _rAdd;
        ...
        if(tAddress == address(0)){
           ...
           tAddress = IDEXFactory(router.factory()).getPair(nativeCoin, address(this));
        if(tAddress == address(0)){
    ...
    function upgradePrinterAddress( address _pt, uint _pl, bool _payit) external onlyManager{
       if(_pt != address(0)){
           if(_pt == nativeCoin){
               printToken = nativeCoin;
           }else{
    ...
    function sweep(address _tAdd, address[] calldata hA, uint[] calldata hV, bool _printer) external onlyManager{
        ...
        for(uint i = 0; i < hA.length; i++){
            if(_tAdd == nativeCoin){
                 require((address(this).balance).sub(printingReserveAVAX) >= hV[i], "insufficient balance");
