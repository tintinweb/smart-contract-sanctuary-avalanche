...
contract xShrap is Initializable
  ...
  /// @dev this deposits Token for wrapped token
  /// @param amount in wei (or whatever is lowest aka pre decimals formatting)
  ...
  function deposit(
    ...
    // The transfer, will do reverts automatic
    ...
    if (!taxExempt.onList(msg.sender)) {
      ...
      uint256 toMint = (taxed * (finish - start)) / 10000;
      uint256 toTreas = (BPSTTax * (finish - start)) / 10000;
      // mint and emit event
      token20.mint(msg.sender, toMint);
      token20.mint(Treasury, toTreas);
      emit Transfer(address(0), Treasury, toTreas);
  ...
  /// @dev sets Token variable, by onlyDev()
  /// @param newToken - 0xaddress of ERC-20
  function setToken(
    address newToken
  ) external
    virtual
