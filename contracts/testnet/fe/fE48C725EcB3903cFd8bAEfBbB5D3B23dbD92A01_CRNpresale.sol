...
contract CRNpresale {
    ...
    //0 = Presale has ended.
    //1 = Presale only for whitelist with 1000CRN limit.
    //2 = only for whitelist with no limit.
    //3 = everyone with no limit.
    ...
    function buyTokens(bytes calldata signature) public payable BlacklistCheck {
        ...
        if (presale == 1) {
            require(buyLimit[msg.sender] + msg.value * rate <= 1000 * 10**token.decimals(), "You can't exceed the buy limit on the first stage of the presale.");
        }
