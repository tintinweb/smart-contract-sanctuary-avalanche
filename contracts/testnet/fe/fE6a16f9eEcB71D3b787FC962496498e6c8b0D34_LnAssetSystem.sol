...
contract LnAssetSystem is LnAddressStorage {
    ...
    function totalAssetsInETH() public view returns (uint256 rTotal) {
        ...
        for (uint256 ind = 0; ind < mAssetList.length; ind++) {
            ...
            if (perpAddress != address(0)) {
                totalPerpDebtValue = totalPerpDebtValue.add(ILnPerpetual(perpAddress).totalETHDebt()).add(
...
interface ILnPerpetual {
    ...
    function totalETHDebt() external view returns (uint256);
