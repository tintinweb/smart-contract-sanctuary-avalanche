...
// File contracts/presale/PreFlaskSales.sol
...
contract PreFlaskSales is Ownable {
    using SafeMath for uint256;
    ...
    IERC20 public pFlask;

    address private _saleProceedsAddress;

    uint256 public pFlaskPrice; // 0.01 DAI per pFlask (eg: if 1 pFlask = 0.01 DAI)
    ...
    function initialize(
        address pFlask_,
        address dai_,
        uint256 pFlaskPrice_,
        address saleProceedsAddress_
    ) external onlyOwner {
        require(!initialized);
        pFlask = IERC20(pFlask_);
        dai = IERC20(dai_);
        pFlaskPrice = pFlaskPrice_;
        _saleProceedsAddress = saleProceedsAddress_;
        initialized = true;
    }

    function setPreFlaskPrice(uint256 newPreFlaskPrice_)
        external
        onlyOwner
        returns (uint256)
    {
        pFlaskPrice = newPreFlaskPrice_;
        return pFlaskPrice;
    }
    ...
    function _calculateAmountPurchased(uint256 amountPaid_)
        ...
        returns (uint256)
    {
        return amountPaid_.mul(pFlaskPrice);
        // Previously, we returned 1:1 but now we want to return
        // for 1:100 instead of 1:1
        // return amountPaid_.mul(pFlaskPrice).mul(100);
    }

    function buyPreFlask(uint256 amountPaid_) external returns (bool) {
        require(approvedBuyers[msg.sender], 'Buyer not approved.');
        uint256 pFlaskAmountPurchased_ = _calculateAmountPurchased(amountPaid_);
        dai.safeTransferFrom(msg.sender, _saleProceedsAddress, amountPaid_);
        pFlask.safeTransfer(msg.sender, pFlaskAmountPurchased_);
        return true;
    }
