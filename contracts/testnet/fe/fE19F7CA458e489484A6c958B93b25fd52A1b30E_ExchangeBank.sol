...
contract ExchangeBank is IExchange, ReentrancyGuard {
    ...
    // uint256 public totalLoanAmount;        ///@dev  total loan of user
    // uint256 public remainingLoanAmount;    ///@dev remaining loan of user
     uint256 public totalPledgedToken;        ///@dev  total pledged token of user
    uint256 public remainingPledgedToken;    ///@dev remaining pledge token after listing on exchange
    ...
    function sellTokens(uint256 _price) nonReentrant public onlyAdminSeller {
        
        
        require(token.allowance(seller, address(this)) >=remainingPledgedToken,"The bank exchange should be approved to sell tokens");
        require(
             remainingPledgedToken >= 0,
            "Sell token Amount must be greater than zero"
        );
        require(
            _price > 0 && remainingPledgedToken > 0,
            "Price and Quantity must be greater than zero"
        );

        price = _price;
        totalQty = remainingPledgedToken;
        remainingQty = remainingPledgedToken;
        sellPlaced = true;
        ...
        emit TokensSold(seller, address(token), remainingPledgedToken, _price);
    }
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        remainingPledgedToken = remainingPledgedToken.sub(_quantity);
        console.log("remainingQty==>",remainingQty );
        
        adjustLoan(_quantity,buyerPay, _wallet, borrower, seller );
    ...
    function acceptCounter(address _buyerWallet) nonReentrant public {
        ...
        if (msg.sender == seller || (_whitelist.isAdmin(msg.sender) && StringUtils.equal(counterOffers[_buyerWallet].role, "buyer"))) {
            ...
            adjustLoanCounter(_quantity,totalAmount, _wallet, borrower, seller );
            ...
            console.log("exchange balance ==>", stableCoin.balanceOf(address(this)));
    ...
    ///@notice  it returns the loan amount including interest amount and number of pledged tokens of borrower.
    ///@notice loan from bank selling tokens against listed property.
    ///@dev it internally calls a method from margin loan.

    function getUserPledgedLoan() public view returns (uint256, uint256, uint256) {

      (,uint256 _loanAmount,uint256 _tokensAmount, uint256 _interestAmount) = _IMarginLoan.getPledgedLoan( borrower , address(token), seller);
    
        return (_loanAmount,_tokensAmount, _interestAmount);
    ...
    function setPledgeTokenSaleLimit() public onlyAdmin  {
       ...
       (uint256 _loanAmount,uint256 _tokensAmount, uint256 _interestAmount) =  getUserPledgedLoan();
    ...
    function adjustLoan(uint256 quantity,uint256 totalAmount, address _wallet, address _borrower, address _seller ) internal {

        (uint256[] memory ids,uint256[] memory loans) = _IMarginLoan
            .getTotalNoOfPledgeTokens(borrower,address(token),seller);
        
        uint256 len = loans.length;
        if (len > 0) {
            for (uint256 i = 0; i < len;) {
                if (loans[i] <= totalAmount) {
                    console.log("loans==> 1st",loans[i]);
                    remainingAmount = totalAmount - loans[i];
                    stableCoin.transferFrom(_wallet, _seller, loans[i]);
                    _IMarginLoan.updatePledgeLoan(_borrower, ids[i],0, 0, 1);
                    (,uint256 loansAmount,,) = _IMarginLoan.getPledgedLoan(_borrower, address(token), seller);
                    
                    if(loansAmount <= remainingAmount){
                    uint256 exTokenBalance = remainingQty;
                    remainingQty =  0;
                    remainingPledgedToken = 0;
                    stableCoin.transferFrom(_wallet, borrower, remainingAmount);
                    token.transfer(_borrower, exTokenBalance);
                    }
                    
                    totalAmount  = remainingAmount;
                } else if (loans[i] > totalAmount) {
                    console.log("loans 2nd==>",loans[i]);
                    uint256 remainingLoan = loans[i] - totalAmount;
                    stableCoin.transferFrom(_wallet, _seller, totalAmount);
                  
                    _IMarginLoan.updatePledgeLoan(
                        _borrower,
                        ids[i],
                        remainingLoan,
                        quantity,
    ...
    function adjustLoanCounter(uint256 quantity,uint256 totalAmount, address _wallet, address _borrower, address _seller ) internal {
        ...
        if (len > 0) {
            for (uint256 i = 0; i < len;) {
                if (loans[i] <= totalAmount) {
                    console.log("loans==> 1st",loans[i]);
                    remainingAmount = totalAmount - loans[i];
                    stableCoin.transfer(_seller, loans[i]);
                    _IMarginLoan.updatePledgeLoan(_borrower, ids[i],0, 0, 1);
                    (,uint256 loansAmount,,) = _IMarginLoan.getPledgedLoan(_borrower, address(token), seller);
                    ...
                    remainingQty =  0;
                    remainingPledgedToken = 0;
                    stableCoin.transfer(borrower, remainingAmount);
                    token.transfer(_borrower, exTokenBalance);
                    }
                    ...
                    console.log("loans 2nd==>",loans[i]);
                    uint256 remainingLoan = loans[i] - totalAmount;
                    stableCoin.transfer(_seller, totalAmount);
