...
interface IERC721 is IERC165 {
     ...
     */
    event Transfer(address indexed from, address indexed to, bytes indexed tokenId);

    /**
     * @dev Emitted when `owner` enables `approved` to manage the `tokenId` token.
     */
    event Approval(address indexed owner, address indexed approved, bytes indexed tokenId);
     ...
     * - `tokenId` must exist.
     */
    function ownerOf(bytes memory tokenId) external view returns (address owner);
   ...
   /* function safeTransferFrom(
     ...
     */
    function approve(address to, bytes memory tokenId) external;
     ...
     * - `tokenId` must exist.
     */
    function getApproved(bytes memory tokenId) external view returns (address operator);
...
interface IERC721Metadata is IERC721 {
     ...
     */
    function tokenURI(bytes memory tokenId) external view returns (string memory);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function ownerOf(bytes memory tokenId) public view virtual override returns (address) {
     ...
     */
    function tokenURI(bytes memory tokenId) public view virtual override returns (string memory) {
     ...
     */
    function approve(address to, bytes memory tokenId) public virtual override {
     ...
     */
    function getApproved(bytes memory tokenId) public view virtual override returns (address) {
   ...
   /* function safeTransferFrom(
     ...
     */
    function _exists(bytes memory tokenId) internal view virtual returns (bool) {
     ...
     * - `tokenId` must exist.
     */
    function _isApprovedOrOwner(address spender, bytes memory tokenId) internal view virtual returns (bool) {
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, bytes memory tokenId) internal virtual {
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, bytes memory tokenId) internal virtual {
     ...
     * Emits a {Transfer} event.
     */
    function _burn(bytes memory tokenId) internal virtual {
        address owner = ERC721.ownerOf(tokenId);
     ...
     * Emits an {Approval} event.
     */
    function _approve(address to, bytes memory tokenId) internal virtual {
...
contract socialMedia is Ownable, ERC721{
    ...
    constructor ()ERC721("socialMedia", "SMA"){
    ...
    event postCreated (address indexed sender, string indexed post, uint256 time, bytes postId);
    event postDeleted(bytes indexed tokenId);
    ...
    function createPost(string calldata post) external {
        ...
        emit postCreated (to, post, time, newtokenId);   
    }
    ...
    function deletePost(bytes calldata tokenId)external{
        _burn(tokenId);
