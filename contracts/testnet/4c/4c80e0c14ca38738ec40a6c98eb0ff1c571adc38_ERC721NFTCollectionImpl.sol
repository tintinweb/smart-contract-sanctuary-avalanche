...
contract ERC721NFTCollectionImpl is
    ...
    INFTCollection
{
    ...
    function setWhitelistedUsers(
        ...
        for (uint256 i = 0; i < allocations.length; i++) {
            ...
            if (totalNumberOfSpots > mintCap)
                revert CanNotAllocateMoreThanMintCap();
    ...
    function mint(
        ...
        if (mode == Mode.Private) {
            if (getNumberOfTotalRemainingNFTs() < privateModeMintCount)
                revert RequestedPrivateModeMintCountExceedsTotal();

            totalMintFee = privateModeMintCount * mintPrice;

            (extraPaid, launchpadFee) = _tokenPurchase(totalMintFee);

            mintedIds = _createToken(_msgSender(), privateModeMintCount);
        } else if (mode == Mode.Whitelist) {
            if (whitelistMintStartDate + mintExpirationTime < block.timestamp)
                revert MintingHasEnded();
            ...
            if (getNumberOfTotalRemainingRaceNFTs() < raceMintCount)
                revert RequestedRaceMintCountExceedsTotal();

            uint256 totalMintCount = freeMintCount +
                guaranteedMintCount +
                raceMintCount;
            if (getNumberOfTotalRemainingNFTs() < totalMintCount)
                revert NotEnoughNFTsLeft();

            totalMintFee = getTotalMintPrice(
                guaranteedMintCount,
                raceMintCount
            );
            (extraPaid, launchpadFee) = _tokenPurchase(totalMintFee);
            ...
            mintedIds = _createToken(_msgSender(), totalMintCount);
        } else if (mode == Mode.Public) {
            if (getNumberOfTotalRemainingNFTs() < publicModeMintCount)
                revert RequestedPublicModeMintCountExceedsTotal();
            if (
                getTotalNFTsMinted(_msgSender()) + publicModeMintCount > mintCap
            ) {
                revert CanNotMintMoreThanMintCap();
            }

            totalMintFee = publicModeMintCount * mintPrice;

            (extraPaid, launchpadFee) = _tokenPurchase(totalMintFee);

            mintedIds = _createToken(_msgSender(), publicModeMintCount);
        ...
        emit UserMinted(
            ...
            launchpadFee,
            _spots,
            privateModeMintCount,
            publicModeMintCount,
            mintedIds
        );
    }

    function _createToken(address user, uint256 amount)
        internal
    ...
    function _tokenPurchase(uint256 totalMintFee)
        internal
        returns (uint256 extraPaid, uint256 launchpadFee)
    {
        if (totalMintFee > msg.value)
            revert InvalidAmount({sent: msg.value, minRequired: totalMintFee});

        extraPaid = msg.value > totalMintFee ? msg.value - totalMintFee : 0;

        launchpadFee = totalMintFee.mulBP(launchpadFeePercentage);
    ...
    function getTotalMintPrice(
        uint256 guaranteedMintCount,
        uint256 raceMintCount
    ) public view returns (uint256 mintFee) {
        // ToDo: get discounts into account, not MVP
        return (guaranteedMintCount + raceMintCount) * mintPrice;
    }
        ...
        onlyRole(COLLECTION_CREATOR_ROLE)
    {
        ...
        emit MintPriceChanged(_mintPrice, oldMintPrice);
    }
    ...
    function setMintCap(uint256 _mintCap) external onlyRole(ADMIN_ROLE) {
        ...
        mintCap = _mintCap;
        emit MintCapChanged(_mintCap, oldMintCap);
    }
    ...
    function getNumberOfTotalRemainingNFTs() public view returns (uint256) {
        return maxMintCount - _tokenIds.current();
    }
    ...
    function getPublicMintCap(address user) external view returns (uint256) {
        return mintCap - getTotalNFTsMinted(user);
    }
