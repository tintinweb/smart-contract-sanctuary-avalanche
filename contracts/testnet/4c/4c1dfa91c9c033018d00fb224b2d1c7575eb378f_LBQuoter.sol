...
contract LBQuoter {
     ...
     */
    constructor(address factoryV1, address factoryV2, address routerV2) {
        ...
        returns (Quote memory quote)
    {
        ...
        for (uint256 i; i < swapLength; i++) {
            // Fetch swap for V1
            quote.pairs[i] = ILydiaFactory(_factoryV1).getPair(route[i], route[i + 1]);
            ...
            if (quote.pairs[i] != address(0) && quote.amounts[i] > 0) {
                ...
                if (reserveIn > 0 && reserveOut > 0) {
                    quote.amounts[i + 1] = LydiaLibrary.getAmountOut(quote.amounts[i], reserveIn, reserveOut).safe128();
                    quote.virtualAmountsWithoutSlippage[i + 1] = LydiaLibrary.quote(
        ...
        returns (Quote memory quote)
    {
        ...
        for (uint256 i = swapLength; i > 0; i--) {
            // Fetch swap for V1
            quote.pairs[i - 1] = ILydiaFactory(_factoryV1).getPair(route[i - 1], route[i]);
            if (quote.pairs[i - 1] != address(0) && quote.amounts[i] > 0) {
                ...
                if (reserveIn > 0 && reserveOut > quote.amounts[i]) {
                    quote.amounts[i - 1] = LydiaLibrary.getAmountIn(quote.amounts[i], reserveIn, reserveOut).safe128();
                    quote.virtualAmountsWithoutSlippage[i - 1] = (
                        LydiaLibrary.quote(quote.virtualAmountsWithoutSlippage[i] * 1000, reserveOut * 997, reserveIn)
                            + 1
                    ).safe128();
