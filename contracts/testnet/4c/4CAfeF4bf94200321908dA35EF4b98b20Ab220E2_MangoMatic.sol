...
contract MangoMatic is Context, Ownable {
    ...
    function sendRef(address ref, bool isSend) private {
        require(initialized);
    ...
    function harvestSeeds() public {
        ...
        uint256 feeDev = SafeMath.div(SafeMath.mul(seedValue, 4), 100);
        claimedSeeds[msg.sender] = reInvestAmount;
        lastPlanted[msg.sender] = block.timestamp;
        marketSeeds = SafeMath.add(marketSeeds,hasSeeds);
        claimedSeeds[recAddr] += reInvestAmount;
        ownerAddr.transfer(feeDev);
        payable (msg.sender).transfer(SafeMath.sub(seedValue,feeDev));
    }
    ...
    function plantSeeds(address ref) public payable {
        ...
        seedsBought = SafeMath.sub(seedsBought,SafeMath.add(feeDev, feeOwner));
