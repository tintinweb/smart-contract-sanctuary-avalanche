...
import "../interfaces/IGenericBridge.sol";
    ...
    ChainIdHolding
{
    ...
    IGenericBridge public genericBridge;

    function initialize(address _genericBridge) public initializer {
        __DTOUpgradeableBase_initialize();
        __ChainIdHolding_init();
        genericBridge = IGenericBridge(_genericBridge);
    }
    ...
    function safeTransferOut(
        ...
        if (_token == NATIVE_TOKEN_ADDRESS) {
            ...
            require(
                balBefore.sub(erc20.balanceOf(address(this))) == _forUser,
                "!transfer to"
            );
        }
    }
    ...
    function addLiquidity(
        ...
        IERC20Upgradeable(_tokenOnThisChain).safeApprove(
            address(genericBridge),
            _amount
        );
        genericBridge.addLiquidity(
            _originToken,
            _originChainId,
            _tokenOnThisChain,
            _amount,
    ...
    function removeLiquidity(
        ...
        address _bridgeToken = genericBridge.tokenMap(
            _originChainId,
            _originToken
        );
        safeTransferIn(_bridgeToken, msg.sender, _amount);
        IERC20Upgradeable(_bridgeToken).safeApprove(
            address(genericBridge),
            _amount
        );
        genericBridge.removeLiquidity(
            _originToken,
            _originChainId,
            _tokenOnThisChain,
            _amount,
    ...
    function requestBridge(
        ...
        if (!genericBridge.isBridgeToken(_tokenAddress)) {
            //first add liquidity to get bridge token
            IGenericBridge.TokenInfo memory originTokenInfo = genericBridge
                .crossChainMappingReverse(_tokenAddress);
            if (originTokenInfo.addr == address(0)) {
                //not multichain token, just bridge normal
                genericBridge.requestBridge(
                    _tokenAddress,
                    _toAddr,
                    _amount,
                    _toChainId
                ...
                //check if multi chain token
                address _tokenOnThisChain = genericBridge.crossChainMapping(
                    originTokenInfo.chainId,
                    originTokenInfo.addr
                );
                {
                    genericBridge.addLiquidity(
                        originTokenInfo.addr,
                    ...
                    address bridgeToken = genericBridge.tokenMap(
                        originTokenInfo.chainId,
                        originTokenInfo.addr
                    );
                    require(
                        IERC20Upgradeable(bridgeToken).balanceOf(
                            address(this)
                        ) >= _amount,
                        "invalid bridge token amount receive"
                    );

                    IERC20Upgradeable(bridgeToken).safeApprove(
                        address(genericBridge),
                        _amount
                    );
...
interface IGenericBridge {
    ...
    function alreadyClaims(bytes32) external view returns (bool);

    function bridgeApprovers(uint256) external view returns (address);

    function approverMap(address) external view returns (bool);

    function supportedChainIds(uint256) external view returns (bool);

    function tokenMapList(address, uint256) external view returns (uint256);

    function tokenMapSupportCheck(address, uint256)
        external
        view
        returns (bool);

    function tokenMap(uint256, address) external view returns (address);

    function tokenMapReverse(address) external view returns (TokenInfo memory);

    function bridgeTokens(address) external view returns (bool);

    function claimFee() external view returns (uint256);

    function index() external view returns (uint256);

    function minApprovers() external view returns (uint256);

    function feeReceiver() external view returns (address payable);

    function defaultFeePercentage() external view returns (uint256);

    function DEFAULT_FEE_DIVISOR() external view returns (uint256);

    function DEFAULT_FEE_PERCENTAGE() external view returns (uint256);

    function crossChainMapping(uint256, address)
        external
        view
        returns (address);
    function crossChainMappingReverse(address) external view returns (TokenInfo memory);

    function crossChainIndex() external view returns (uint256);
