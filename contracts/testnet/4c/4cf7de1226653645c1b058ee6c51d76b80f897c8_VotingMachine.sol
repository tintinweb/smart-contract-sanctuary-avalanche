...
import {VotingMachineWithProofs, IRootsWarehouse, IL2VotingStrategy, IVotingMachineWithProofs} from './VotingMachineWithProofs.sol';
  ...
  VotingMachineWithProofs
{
  ...
  constructor(
    ...
    IL2VotingStrategy votingStrategy
  ...
  function receiveCrossChainMessage(
    ...
    try _votingStrategy.hasRequiredRoots(blockHash) {
...
import {IVotingMachineWithProofs, IRootsWarehouse, CrossChainUtils} from './interfaces/IVotingMachineWithProofs.sol';
import {IL2VotingStrategy} from './interfaces/IL2VotingStrategy.sol';
...
abstract contract VotingMachineWithProofs is IVotingMachineWithProofs, Ownable {
  ...
  IL2VotingStrategy internal _votingStrategy;

  mapping(uint256 => ProposalVoteConfiguration) _proposalsVoteConfiguration;

  constructor(
    IRootsWarehouse newRootsWarehouse,
    IL2VotingStrategy newVotingStrategy
  ) Ownable() {
  ...
  function createVote(uint256 proposalId) public returns (uint256) {
    ...
    uint48 startTime = _getCurrentTimeRef();
    uint48 endTime = startTime + voteConfig.votingDuration;
  ...
  /// @inheritdoc IVotingMachineWithProofs
  function setVotingStrategy(IL2VotingStrategy newVotingStrategy)
    external
    onlyOwner
  {
    _setVotingStrategy(newVotingStrategy);
  ...
  function _setVotingStrategy(IL2VotingStrategy newVotingStrategy) internal {
  ...
  function _getCurrentTimeRef() internal view returns (uint48) {
