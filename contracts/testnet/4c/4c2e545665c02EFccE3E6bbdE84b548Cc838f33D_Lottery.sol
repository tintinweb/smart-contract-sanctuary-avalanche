...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
		...
		nonReentrant
	{
		...
		if (block.timestamp + newTicketStep[phase] < roundMaxTime[phase]) {
	...
	function startNextRound() internal {
		...
		//sets all round max times to start from current time
		roundMaxTime[1] = block.timestamp + roundMaxTime[1];
		roundMaxTime[2] = roundMaxTime[1] + roundMaxTime[2];
		roundMaxTime[3] = roundMaxTime[2] + roundMaxTime[3];
		roundMaxTime[4] = roundMaxTime[3] + roundMaxTime[4];
		roundMaxTime[5] = roundMaxTime[4] + roundMaxTime[5];
		roundMaxTime[6] = roundMaxTime[5] + roundMaxTime[6];
	}
