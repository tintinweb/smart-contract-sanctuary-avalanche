...
contract StakingLp is Ownable, Destructible, Pausable {
    struct User {
        uint256 payouts;
   ...
   function getPercentage(address _addr)internal view returns(uint256){
       uint256 percentage=(block.timestamp - users[_addr].withdrawTime) / 10 seconds;
       if(percentage<3){
           return percentage;
       }else{
           return 3;
    ...
    function payoutOf(address _addr) view internal returns(uint256 payout) {
            payout = ((users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 10 seconds) / 100)) - users[_addr].deposit_payouts;
    }
    function getWaterReward(address _addr)view external returns(uint256){
        ...
        return waterToUsd(reward);
            ...
            reward=reward*getPercentage(_addr)-users[_addr].deposit_amount/100;
            return waterToUsd(reward);
        }else if(getPercentage(_addr)==3){
            reward= reward*getPercentage(_addr)-users[_addr].deposit_amount*3/100;
            return waterToUsd(reward);
        }
    }
