...
interface IStakingPool {
    ...
    enum PenaltyType {
        CLAIM_A_HALF_OF_REWARD,
        CAN_NOT_CLAIM_REWARD
    }
    ...
    event SetMinimumStakeAmount(uint value);
    event SetPenaltyType(PenaltyType value);
}
...
contract StakingPool is IStakingPool, ReentrancyGuardUpgradeable, AuthorizerUpgradeable, PausableUpgradeable {
    ...
    function stake(uint amount) public nonReentrant whenNotPaused {
        ...
        Staking memory staking = userStaking[user];
        Pool memory poolInfo = pool;
        require(amount >= minimumStakeAmount, "Stake amount invalid");
        require(amount + staking.amount <= poolInfo.quota, "Cannot stake over quota");
        uint fee = (amount * poolInfo.stakeFeeRate) / HUNDRED_PERCENT;
        amount -= fee;

        // Process stake from each RewardController
        for (uint i = 0; i < poolInfo.rewardControllers.length; i++) {
            IRewardController rewardController = poolInfo.rewardControllers[i];
            IERC20 rewardToken = rewardController.rewardToken();

            // Get stake amount for RewardController
            ...
            // Handle stake from RewardController
            rewardController.stake(user, stakeAmount);
        }

        // Update storage data
        _handleCountStakers(staking.amount, staking.amount + amount);
        userStaking[user] = Staking({
            amount: staking.amount + amount,
            deadline: block.timestamp + poolInfo.stakingPeriod
        });
        totalStaked += amount;
    ...
    function unstake(uint amount) public nonReentrant {
        ...
        Staking memory staking = userStaking[user];
        Pool memory poolInfo = pool;
        require(amount <= staking.amount, "Cannot unstake more than staked");
        uint fee = (amount * poolInfo.unstakeFeeRate) / HUNDRED_PERCENT;
        bool earlyUnstake = block.timestamp < staking.deadline;

        // Process unstake for each RewardController
        for (uint rewardId; rewardId < poolInfo.rewardControllers.length; rewardId++) {
            IRewardController rewardController = poolInfo.rewardControllers[rewardId];
            IERC20 rewardToken = rewardController.rewardToken();

            // Get unstake amount for RewardController
            uint unstakeAmount = rewardController.rewardType() == IRewardController.Type.APR
                ? priceFeed.getAmountOut(amount, address(stakeToken), address(rewardToken))
                : amount;

            // Handle unstake and verify reward from RewardController
            rewardController.unstake(user, unstakeAmount);
            uint rewardAmount = rewardController.verifyRewards(user);

            // Handle penalty for early unstake
            if (earlyUnstake) {
                uint penaltyAmount = (rewardAmount * poolInfo.penaltyRate) / HUNDRED_PERCENT;
    ...
    function claimReward(uint rewardId, uint amount) external nonReentrant {
        ...
        address user = msg.sender;
        Staking memory staking = userStaking[user];
        bool earlyClaimReward = block.timestamp < staking.deadline;

        // Verify reward from RewardController
        uint reward = rewardController.verifyRewards(user);

        // Handle penalty for early claim reward
        if (earlyClaimReward) {
            uint penaltyReward = _handlePenalty(reward);
            reward -= penaltyReward;
            rewardToken.safeTransfer(treasury, penaltyReward);
        }
    ...
    function setMinimumStakeAmount(uint value) external onlyRole(ADMIN_ROLE) {
        ...
        emit SetMinimumStakeAmount(value);
    }

    function setPenaltyType(PenaltyType value) external onlyRole(ADMIN_ROLE) {
        penaltyType = value;
        emit SetPenaltyType(value);
    }
    ...
    function _handlePenalty(uint _reward) internal view returns (uint _penaltyReward) {
        Pool memory _pool = pool;
        if (penaltyType == PenaltyType.CAN_NOT_CLAIM_REWARD) {
            revert("You can not claim reward now");
        } else if (penaltyType == PenaltyType.CLAIM_A_HALF_OF_REWARD) {
            _penaltyReward = (_reward * _pool.penaltyRate) / HUNDRED_PERCENT;
            return _penaltyReward;
        }
    }
}