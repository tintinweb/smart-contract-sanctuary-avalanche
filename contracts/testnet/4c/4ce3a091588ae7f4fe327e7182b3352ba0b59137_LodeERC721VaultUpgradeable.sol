...
contract LodeERC721VaultUpgradeable is Initializable, ERC721URIStorageUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    struct EnumerableERC20BackedTokens {
        address[] _tokens;
    ...
    EnumerableERC20BackedTokens internal backedTokens;

    CountersUpgradeable.Counter public mintingId;
    mapping(uint256 => MintingInfo) public mintingInfo; // from vault tokenId to minting information.
    ...
    /*********** ENUMERABLE ADDRESS STRUCT METHODS ***********/
    function addToken(address _token) external onlyRole(OWNER_ROLE) {
        require(_token != address(0), "Error: Token cannot be 0x0");

        backedTokens._tokens.push(_token);
        backedTokens._indexes[_token] = backedTokens._tokens.length;

        emit TokenAdded(_token);
    }

    function containsToken(address _token) public view returns (bool) {
        return backedTokens._indexes[_token] != 0;
    }

    function removeToken(address _token) external onlyRole(OWNER_ROLE) {
        uint256 tokenIndex = backedTokens._indexes[_token];

        require(tokenIndex != 0, "Error: Token not included in Enumerable list");

        uint256 toDeleteIndex = tokenIndex - 1;
        uint256 lastIndex = backedTokens._tokens.length - 1;

        if (toDeleteIndex != lastIndex) {
            address lastToken = backedTokens._tokens[lastIndex];
            backedTokens._tokens[toDeleteIndex] = lastToken;
            backedTokens._indexes[lastToken] = tokenIndex; 
        }

        backedTokens._tokens.pop();
        delete backedTokens._indexes[_token];

        emit TokenRemoved(_token);
    }

    /*********** ERC721 ROLE METHODS ***********/
    function mint(address token, address to, uint256 erc20Amount, string memory tokenURI) external onlyRole(MINTER_ROLE) {
        require(containsToken(token) == true, "Error: Token not included in Enumerable list");
        require(erc20Amount != 0, "Error: Null value");
        
        uint256 tokenId = mintingId.current();

        _safeMint(to, tokenId);
        _setTokenURI(tokenId, tokenURI);
        
        mintingInfo[tokenId] = MintingInfo(token, erc20Amount);
    ...
    function burn(uint256 tokenId) external onlyRole(BURNER_ROLE) {
        ...
        delete mintingInfo[tokenId];
    }

    /*********** GETTER METHODS ***********/
    function getERC20BackedTokens() external view returns(address[] memory) {
        return backedTokens._tokens;
    }

    function getBackedTokenIndex(address _token) external view returns(uint256) {
        return backedTokens._indexes[_token];
    }

    function getBackedToken(uint256 tokenId) external view returns (address) {
        return mintingInfo[tokenId]._token;
    }

    function getERC20Equivalence(uint256 tokenId) external view returns (uint256) {
        return mintingInfo[tokenId]._equivalence;
    ...
    /*********** AUXILIARY METHODS ***********/
    function supportsInterface(bytes4 interfaceId) public view override (AccessControlEnumerableUpgradeable, ERC721Upgradeable) returns (bool) {
