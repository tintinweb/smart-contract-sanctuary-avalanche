...
interface IMaintFee {
    ...
    /** @dev Method to Get Array the YieldBox without YieldBox Associeate */
    ...
    ReentrancyGuardUpgradeable
{
        ...
        )
    {
        ...
        for (uint256 i = 0; i < YieldBoxIds.length; i++) {
            ...
            if (
                ...
                (((time - YieldBox.yieldBox(YieldBoxIds[i]).createdAt) /
                    1 days) > RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed)
            ) {
                rewarded[i] =
                    ((time - YieldBox.yieldBox(YieldBoxIds[i]).createdAt) /
                        1 days) -
     ...
     * @param _yieldKey Array of Yield Key of Yield Box
     * @param _type Array of Type of Yield Box
     */
        ...
        for (uint256 i = 0; i < _yieldKey.length; i++) {
            ...
            for (uint256 j = 0; j < YieldBoxRewarded.length; j++) {
                if (amount[j] > 0) {
                    ...
                    MaintFee.maintFee(YieldBoxRewarded[j]).firstDay = MaintFee
                        .maintFee(YieldBoxRewarded[j])
                        .firstDay == uint64(0)
                        ? uint64(time)
                        : MaintFee.maintFee(YieldBoxRewarded[j]).firstDay;
    ...
    /**
     * @dev This Method to calculate of Minimal Rewards per Sender.
     * @param _wallet Wallet of the User Owner of the Yield Boxes without Yield Key in Staking
     * @param _YieldBoxIds Yield Boxes Ids of the User Owner of the Yield Boxes without Yield Key in Staking
     */
    function minimalRewardPerYK(address _wallet, uint256[] memory _YieldBoxIds)
        external
        nonReentrant
    {
        (
        ...
        require(success_treasury, "ERC721 VALIFi: Can't pay Minimal Rewards");
        for (uint256 i = 0; i < YieldBoxRewarded.length; i++) {
            if (amount[i] > 0) {
                emit RewardsPaid(
                    _msgSender(),
