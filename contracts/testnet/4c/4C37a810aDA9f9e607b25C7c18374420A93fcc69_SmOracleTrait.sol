...
contract SmOracleTrait is Ownable {
    ...
    struct TokenInfo {
        ...
        int256[] attributeRarityScore; // input value = actual value * 1e4
    }
    struct RegressionStats {
        // cannot accept decimal !!!!!!!!!!!!!!!!!!
        int256[] coefficients; // input value = actual value * 1e9
    ...
    mapping(bytes32 => RegressionStats) public regressionStats;
    mapping(bytes32 => DeltaStats) private deltaStats;
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _prices.length; i++) {
            ...
            // trait filter
            {
                int256 predictedDelta = getPredictedDelta(_currencyKey, i);
            ...
            priceInfo[_currencyKey].timestamp.push(_timestamp[i]);
    ...
    function setAttributeRarityScore(bytes32 _currencyKey, int256[] memory _attributeRarityScore, uint256[] memory _tokenId) external onlyOwner {
        ...
        for (uint256 i = 0; i < _attributeRarityScore.length; i++){
            ...
            if ( (i + 1) % regressionStats[_currencyKey].coefficients.length == 0) {
                j++;
            }
        }
    }
    ...
    function getActualDelta(bytes32 _currencyKey, uint256 _price) public view returns (int256) {
        ...
        return ( int256(_price) - int256(dailyAveragePrice) ) / int256(dailyAveragePrice);
    }
    ...
    function getDailyAveragePrice(bytes32 _currencyKey) public view returns(uint256) {
        ...
        for (uint256 i = priceInfo[_currencyKey].prices.length - 1; i >= 0; i--) {
            if (block.timestamp - priceInfo[_currencyKey].timestamp[i] > 3600 * 24) {
                break;
            } else {
                // only include data within 2 SD
                if (priceInfo[_currencyKey].prices[i] >= mean) {
    ...
    function getStandardDeviationAndMeanWithin24hr(bytes32 _currencyKey, uint256 _index) public view returns(uint256, uint256) {
        ...
        if (_index == 1) {
            datasetArray = priceInfo[_currencyKey].prices;
            timestampArray = priceInfo[_currencyKey].timestamp;
        } else if (_index == 2) {
