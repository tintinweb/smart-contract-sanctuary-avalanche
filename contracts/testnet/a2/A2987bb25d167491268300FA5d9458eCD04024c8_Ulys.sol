...
contract Ulys is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the day 2-5 ULYS-WETH LP -> ULYS pool
    uint256 public constant INITIAL_ULYS_POOL_DISTRIBUTION = 800 ether;
    ...
    uint256[] public taxTiersRates = [2000, 1900, 1800, 1700, 1600, 1500, 1500, 1500, 1500, 1400, 900, 400, 200, 100]; // First is 20% Last is 1%
    ...
    event TaxHappened(address sender, address _msgSender, address recipient, uint256 taxAmount);

    event TransferFromHappened(address sender, address _msgSender, address recipient);

    event TransferHappened(address sender, address _msgSender, address recipient, uint256 taxAmount);
    ...
    /**
     * @notice Constructs the ULYS ERC-20 contract.
     */
    constructor(
        uint256 _taxRate,
        address _taxCollectorAddress,
        address _routerAddress
    ) public ERC20("ulysse.finance", "ULYS") {
        // Mints 1 ULYS to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));
        excludeAddress(msg.sender);
        excludeAddress(_taxCollectorAddress);
        excludeAddress(_routerAddress);
        excludeAddress(address(0));
        excludeAddress(address(0xE4A575550C2b460d2307b82dCd7aFe84AD1484dd));

        router = IPangolinRouter(_routerAddress);
    ...
    function _getUlysPrice() internal view returns (uint256 _UlysPrice) {
        try IOracle(ulysOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ulysse: failed to fetch ULYS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ulysPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ulysPrice >= taxTiersTwaps[tierId]) {
    ...
    function setUlysOracle(address _ulysOracle) public onlyOperatorOrTaxOffice {
        require(_ulysOracle != address(0), "oracle address cannot be 0 address");
        ulysOracle = _ulysOracle;
    }
    ...
    function setLP(address _lpAddress) public onlyTaxOffice {
        require(_lpAddress != address(0), "lp address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ULYS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ULYS to mint to
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        emit TransferHappened(msg.sender, _msgSender(), recipient, amount);
    ...
    function transferFrom(
        ...
        emit TransferFromHappened(sender, _msgSender(), recipient);

        if (swapping == true || taxRate == 0 || excludedAddresses[sender]) {
    ...
    function _transferWithTax(
        ...
        uint256 taxAmount = amount.mul(400).div(10000); // MEANS 4%
        // require(this.transferFrom(msg.sender, address(this), taxAmount), "transferFromTax Failed");
        require(this.approve(address(router), taxAmount), "approveTax Failed");
        require(swapping == true || taxRate == 0 || excludedAddresses[sender], "Check Tax Conditions");
        ...
        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(taxAmount, 0, path, taxCollectorAddress, block.timestamp);
        emit TaxHappened(sender, _msgSender(), recipient, taxAmount);
     ...
     */
    function distributeReward(address _ulysPool) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_ulysPool != address(0), "!_ulysPool");
        rewardPoolDistributed = true;
        _mint(_ulysPool, INITIAL_ULYS_POOL_DISTRIBUTION);
    }
