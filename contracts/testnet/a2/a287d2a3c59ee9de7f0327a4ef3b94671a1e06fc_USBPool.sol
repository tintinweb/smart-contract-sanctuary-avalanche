...
contract USBPool is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    ISystemCoin internal USPeggedCoin;
    IUSBSwapLocker public swapLocker;
    ...
    function buyUSB(uint256 inputUSPeggedAmount) external {
        require(inputUSPeggedAmount > 0, "P.A!=0");
        uint256 usbDecimal = systemCoin.decimals();
        uint256 usPeggedDecimal = USPeggedCoin.decimals();
        ...
        USPeggedCoin.safeTransferFrom(msg.sender, address(this), inputUSPeggedAmount);
        if(buyingFees > 0) {
            USPeggedCoin.safeTransfer(reservePool,buyingFees);
        }
    ...
    function sellUSB(uint256 inputUSBAmount) external {
        ...
        uint256 sellingFees = usPeggedAmount * DEV_FEE_BIPS / BIPS_DIVISOR;

        systemCoin.burn(msg.sender, inputUSBAmount);
        if(sellingFees > 0) {
            USPeggedCoin.safeTransfer(reservePool,sellingFees);
        }
        _lockReward(USPeggedCoin, msg.sender, (usPeggedAmount-sellingFees), vestingDuration);
    ...
    function paybackUSB(address account, uint256 usPeggedAmount) external {
        require(usPeggedAmount > 0, "A!=0");
        ...
        USPeggedCoin.safeTransferFrom(msg.sender,address(this),usPeggedAmount);
        debt[account] -= usbAmount;

        emit Payback(msg.sender, account, usPeggedAmount);
    }

    function provideLiquidity(uint256 usPeggedAmount) external onlyAuthorized {
        require(usPeggedAmount > 0, "P.A!=0");
        uint256 usbDecimal = systemCoin.decimals();
        uint256 usPeggedDecimal = USPeggedCoin.decimals();
    ...
    */
    ...
    function getUSPeggedCoin() external view returns (address) {
        return address(USPeggedCoin);
    }
     ...
     *************************************************************/
    function initialize(address _systemCoin, address _USPeggedCoin, IUSBSwapLocker _swapLocker, uint256 _vestingDuration, address _reservePool, uint256 _devFee) public initializer {
        BIPS_DIVISOR = 10000;
        systemCoin = ISystemCoin(_systemCoin);
        USPeggedCoin = ISystemCoin(_USPeggedCoin);
        reservePool = _reservePool;
        DEV_FEE_BIPS = _devFee;
        vestingDuration = _vestingDuration;
        IERC20Upgradeable(_USPeggedCoin).safeApprove(address(_swapLocker), type(uint256).max);
