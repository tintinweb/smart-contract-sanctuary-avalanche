...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    enum Status { REQUESTING, PAID, CONFIRMED, CLAIMED, CANCELLED }
        ...
        nonReentrant  
    {
        ...
        emit Paid(paymentId, amount, payment.currency);
    }
        ...
        nonReentrant  
    {
        ...
        emit Paid(paymentId, msg.value, payment.currency);
    }
     ...
     *
     */
    function getAllowance(string memory token) public view returns (uint256) {
        if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDC")))) {
     ...
     *
     */
    function getContractTokenBalance(string memory token) public onlyOwner view returns (uint256) {
        if (keccak256(abi.encodePacked((token))) == keccak256(abi.encodePacked(("USDC")))) {
