...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    function entryFeeForSeriesOne(uint256 _nftId) public GameInitialized GameEndRules
    {
        ...
        if (_member.userWalletAddress != address(0)) {
            ...
            require(dayPassedAfterJump + 1 < dayDifferance(block.timestamp, _admin.startAt),"You Can only use Buy back in 24 hours");
            _member.day = 0;
            _member.stage = 0;
            _member.startAt = 0;
    ...
    function withdrawWrappedEtherOFCommunity(uint8 withdrawtype) public onlyOwner nonReentrant 
    {
        ...
        if (withdrawtype == 0) {
            //owner
            require(ownerbalances[Contracts[0]] > 0,"Insufficient Owner Balance");
            require(Contracts[0] == msg.sender, "Only Owner use this");
            _withdraw(ownerbalances[msg.sender]);
        } else if (withdrawtype == 1) {
            //vault
            require(vaultbalances[Contracts[1]] > 0,"Insufficient Vault Balance");
