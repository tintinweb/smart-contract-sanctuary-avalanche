...
contract FullFeatureToken is ERC20, ERC20Burnable, ERC20Pausable, Ownable {
  ...
  modifier checkWhitelist(address sender, address recipient) {
  ...
  function blackList(address addr) external onlyOwner whenNotPaused {
    ...
    if (configProps._isWhitelistEnabled && whitelist[addr]) {
  ...
  function transferFrom(
    ...
    override
    whenNotPaused
    checkBlacklist(from, to)
    checkWhitelist(from, to)
    returns (bool)
  {
    if (configProps._isMaxAmountOfTokensSet) {
      if (balanceOf(to) + amount > maxTokenAmountPerAddress) {
   ...
   */
  function _addManyToWhitelist(address[] memory addresses) internal {
   ...
   */
  function _removeFromWhitelist(address[] memory addresses) internal {
    for (uint256 i = 0; i < addresses.length; ) {
