...
using SafeMath for uint256;
    ...
    all_new_0 = calcReward(lastClaimTime,boostMultiplier[0]);
    all_new_1 = calcReward(lastClaimTime,boostMultiplier[1]);
    all_new_2 = calcReward(lastClaimTime,boostMultiplier[2]);
    all_new_3 = calcReward(lastClaimTime,boostMultiplier[3]);
    }

    function calcReward(uint256 _lastClaimTime, uint256 _boost) public view returns (uint256[3] memory){
	    uint256 elapsed = block.timestamp - _lastClaimTime;
	    uint256 _rewardsPerDay = doPercentage(one, dailyRewardsPerc);
