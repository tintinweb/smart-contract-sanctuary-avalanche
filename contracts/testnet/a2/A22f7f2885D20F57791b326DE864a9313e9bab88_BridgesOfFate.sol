...
contract BridgesOfFate is Ownable {
    ...
    event ParticipateOfNewPlayerInLateBuyBackIn(
        bytes32 playerId,
        uint256 moveAtStage,
        uint256 amount
    );
    ...
    function _gameEndRules() internal view returns (bool) {
        ...
        require(
            (_gameStatus.startAt > 0 && block.timestamp >= _gameStatus.startAt),
            "Game start after intialized time."
        );
        if (lastUpdateTimeStamp > 0) {
            require(
                _dayDifferance(block.timestamp, lastUpdateTimeStamp) <=
                    gameEnded,
                "Game Ended ...!"
            );
        }
    ...
    function _narrowingGameEndRules() internal view returns (bool) {
        if (
            (_narrowingGameMap > 0) &&
            (_narrowingGameMap < _NarrowingNumberProbility) &&
            (_dayDifferance(
                block.timestamp,
                GameStatusInitialized[_latestGameId].startAt
            ) > GameStatusInitialized[_latestGameId].lastUpdationDay)
        ) {
    ...
    function uint256ToArray(
        uint256 value,
        uint256 _range
    ) internal pure returns (uint256) {
        uint256[] memory array = new uint256[](3); // Assuming you want an array of size 32
    ...
    function _random(uint256 _range) internal view returns (uint256) {
        uint256 _packedNumber = uint256(
            keccak256(
                abi.encodePacked(block.timestamp, address(this), msg.sender)
            )
        );
        uint256 _number = uint256ToArray(_packedNumber, _range);
        _number = _number + 1;
        return (_number * 1e9);
    }

    function _balanceOfUser(
        address _accountOf
    ) internal view returns (uint256) {
        return token.balanceOf(_accountOf);
    }

    function _randomNarrowingGame(
        uint256 _range
    ) internal view returns (uint256) {
        uint256 _packedNumber = uint256(
            keccak256(
    ...
    function _calculateBuyBackIn() internal view returns (uint256) {
        if (GameStatusInitialized[_latestGameId].stageNumber > 0) {
            if (
                GameStatusInitialized[_latestGameId].stageNumber <=
                buyBackCurve.length
            ) {
                return
                    buyBackCurve[
                        GameStatusInitialized[_latestGameId].stageNumber - 1
                    ];
            } else if (
                GameStatusInitialized[_latestGameId].stageNumber >
                buyBackCurve.length
    ...
    function _deletePlayerIDForSpecifyStage(
        uint256 _stage,
        bytes32 _playerId
    ) internal {
        uint256 _index = _findIndex(_playerId, getStagesData(_stage));
        removeSecondListItem(_index, _stage);
        // bytes32[] storage _stagesData = allStagesData[_stage];
        // delete _stagesData[_index];
    }
    ...
    function _checkSide(
        ...
        if (
            (userSide == false && stage_randomNumber < 50e9) ||
            (userSide == true && stage_randomNumber >= 50e9)
        ) {
            return true;
        } else {
            return false;
    ...
    function _computeNextPlayerIdForHolder(
        address holder,
        uint256 _nftId,
        uint8 _seriesIndex
    ...
    function _randomFortileCapacity(
        uint256 _range
    ) internal view returns (uint) {
        uint randomnumber = uint256ToArray(
            uint(
                keccak256(
    ...
    function _overFlow(
        ...
        if (totalNFTOnTile[jumpAtStage][_jumpSide] > 0) {
            totalNFTOnTile[jumpAtStage][_jumpSide] =
                totalNFTOnTile[jumpAtStage][_jumpSide] -
                1;
        }
        ...
        if (
            jumpAtStage >= 2 &&
            titleCapacityAgainestStage[jumpAtStage - 1] >=
            totalNFTOnTile[jumpAtStage - 1][isSafeSide]
        ) {
            totalNFTOnTile[jumpAtStage - 1][isSafeSide] =
                totalNFTOnTile[jumpAtStage - 1][isSafeSide] +
                1;
    ...
    function isExistForRandomSide(
        uint256[] memory _playerIDNumberList,
        uint256 _playerIDindex
    ...
    function entryFeeSeries(
        bytes32 _playerId,
        uint256 _nftId,
        uint8 _seriesType
    ) public NarrowingGameEndRules {
        ...
        bytes32 playerId = _computeNextPlayerIdForHolder(
            msg.sender,
            _nftId,
            _seriesType
        );
        ...
        if (
            ...
            _checkSide(_member.stage, _member.lastJumpSide)
        ) {
        ...
        if (_member.userWalletAddress != address(0)) {
            require(
                _dayDifferance(_member.lastJumpTime, _gameStatus.startAt) + 1 <
                    _dayDifferance(block.timestamp, _gameStatus.startAt),
                "Buyback is useful only one time in 24 hours"
            );
            _member.lastJumpTime = _member.startAt = _member.stage = _member
                .day = 0;
        ...
        if (_seriesType == 1) {
            ...
            require(
                _balanceOfUser(msg.sender) >= SERIES_TWO_FEE,
    ...
    function bulkEntryFeeSeries(
        bytes32[] calldata _playerId,
    ...
    function changeCommunityOwnerWA(
        address[2] calldata communityOwnerWA
    ) external onlyOwner {
    ...
    function buyBackInFee(
        bytes32 playerId
    ) public GameEndRules NarrowingGameEndRules {
        uint256 buyBackFee = _calculateBuyBackIn();
        require(
            _balanceOfUser(msg.sender) >= buyBackFee,
            "Insufficient balance"
        );
        GameItem storage _member = PlayerItem[playerId]; //memory
        require(
            (_member.userWalletAddress != address(0)) &&
                (_member.userWalletAddress == msg.sender),
            "Only Player Trigger"
        );
        require(
            _dayDifferance(block.timestamp, _member.lastJumpTime) <= 1,
        ...
        if (
            _member.stage - 1 >= 1 &&
            totalNFTOnTile[_member.stage - 1][_member.lastJumpSide] == 0
        ) {
    ...
    function switchSide(
        bytes32 playerId
    ) external GameEndRules NarrowingGameEndRules {
        GameItem storage _member = PlayerItem[playerId];
        ...
        require(
            ...
            "Switch tile time is over."
        );
        _member.lastJumpSide = _member.lastJumpSide == true ? false : true;
        _member.ishold = false;
        _overFlow(
            playerId,
            PlayerItem[playerId].stage,
            PlayerItem[playerId].lastJumpSide
        );
        lastUpdateTimeStamp = block.timestamp;
    }
    ...
    function participateInGame(
        ...
        uint256 currentDay = _dayDifferance(
            block.timestamp,
            _gameStatus.startAt
        );
        ...
        if (GameMap[_currentUserStage] <= 0) {
            ...
            // 4444
            GameMap[_globalStageNumber] = _random(100);
            // GameMap[_globalStageNumber] = _randomcustom;
        ...
        if (totalNFTOnTile[_currentUserStage][_jumpSide] <= 0) {
            _selectPlayerAndSetCapacityOnTile(
                _currentUserStage,
                currentDay,
                _jumpSide
            );
        ...
        if (
            (_narrowingGameMap < _NarrowingNumberProbility) &&
            (_gameStatus.stageNumber == _currentUserStage)
        ) {
            if (
                _checkSide(_gameStatus.stageNumber, _jumpSide) &&
                overFlowPlayerStatus[playerId]
            ) {
                //If player successfull side but capacity over
    ...
    function bulkParticipateInGame(
        bool _jumpSides,
        bytes32[] calldata playerIds
    ) external GameEndRules {
    ...
    function _selectJumpedSidePlayers(
        bytes32[] memory _playerIDAtSpecifyStage,
        bool _jumpAtSide,
        uint256 currentDay
        ...
        for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
            ...
            */
            if (_playerIDAtSpecifyStage[i] != bytes32(0x00)) {
                if (
                    ((PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide ==
                        _jumpAtSide) &&
                        (overFlowPlayerStatus[_playerIDAtSpecifyStage[i]] ==
                            false &&
                            currentDay ==
                            PlayerItem[_playerIDAtSpecifyStage[i]].day)) ||
                    (
                        (PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide ==
                            _jumpAtSide &&
                            overFlowPlayerStatus[_playerIDAtSpecifyStage[i]] ==
                    ...
                    if (PlayerItem[_playerIDAtSpecifyStage[i]].stage >= 1) {
                        _selectSuccessSidePlayerId.push(
                            _playerIDAtSpecifyStage[i]
                        );
                    }
                }
            }
    ...
    function LateBuyInFee(
        ...
        if (
            overFlowPlayerStatus[playerId] == true &&
            _checkSide(_member.stage, _member.lastJumpSide) == true
        ) {
        ...
        if (totalNFTOnTile[_member.stage][_member.lastJumpSide] > 0) {
            totalNFTOnTile[_member.stage][_member.lastJumpSide] =
                totalNFTOnTile[_member.stage][_member.lastJumpSide] -
                1;
            overFlowPlayerStatus[playerId] = true;
        }
        ...
        allStagesData[_member.stage].push(playerId);
        require(
            _balanceOfUser(msg.sender) >= totalAmount,
            "Insufficient balance"
        );
        token.safeTransferFrom(msg.sender, address(this), totalAmount);
        emit ParticipateOfNewPlayerInLateBuyBackIn(
            playerId,
            _gameStatus.stageNumber - 1,
            totalAmount
        );
    }
    ...
    function _selectPlayerAndSetCapacityOnTile(
        uint256 _currentPlayerStage,
        uint256 currentDay,
        bool _jumpSide
    ) internal {
        ...
        if (_currentPlayerStage > 0) {
            uint256 _titleCapacityAgainestStage = titleCapacityAgainestStage[
                _currentPlayerStage
            ];
            bytes32[] memory _playersIDAtSpecifyStage = getStagesData(
                _currentPlayerStage
            );
            // Set (3) into the variable

            _selectJumpedSidePlayersId = _selectJumpedSidePlayers(
                _playersIDAtSpecifyStage,
                _jumpSide,
                currentDay
            );
            //If generated Capacity is less tile capacity on jumped side then use random selection function.
            if (
                _selectJumpedSidePlayersId.length > _titleCapacityAgainestStage
            ) {
                // Randomlly Select NFT's ANY three maxiumn or minmmun two NFT"S
                for (
                    uint256 j = 0;
                    j < _selectJumpedSidePlayersId.length;
                    j++
                ) {
                    if (
                        PlayerItem[_selectJumpedSidePlayersId[j]].ishold ==
                        false
                    ) {
                        // Expect Hold and Mute All NFT" select
                        _selectUnholdPlayerId.push(
                            _selectJumpedSidePlayersId[j]
                        );
                ...
                // _randomSelectPlayerIndex = generateRandomTicketNumbers(
                //     _selectUnholdPlayerId.length - 1,
                //     _selectUnholdPlayerId.length - 1,
                //     _titleCapacityAgainestStage
                // );
                _randomSelectPlayerIndex = [3, 1, 2, 0, 0, 0];
            }
            ...
            if (
                ...
                for (uint256 k = 0; k < _randomSelectPlayerIndex.length; k++) {
                    ...
                    if (_randomSelectPlayer > 0) {
                        if (
                            isExistForRandomSide(_uniqueRN, _randomSelectPlayer)
                        ) {
                            _setTileCapacity = _setTileCapacity + 1;
                            _uniqueRN.push(_randomSelectPlayer);
                            bytes32 _selected = _selectUnholdPlayerId[
                                _randomSelectPlayer
                            ];
                ...
                if (_titleCapacityAgainestStage > 0 && _setTileCapacity > 0) {
                    ...
                    totalNFTOnTile[_currentPlayerStage][_jumpSide] =
                        _titleCapacityAgainestStage -
                        _setTileCapacity;
                } else {
                    emit setCapacityEvent(
                        totalNFTOnTile[_currentPlayerStage][_jumpSide],
                        2
                    );
                    totalNFTOnTile[_currentPlayerStage][
                        _jumpSide
                    ] = totalNFTOnTile[_currentPlayerStage][_jumpSide];
                }
            }
    ...
    function _distributionReward() internal {
        ...
        require(
            isNarrowingGameEnded == false,
            " Distribution time should not start before reaching final stage."
        );
        if (GameStatusInitialized[_latestGameId].isDistribution) {
            // 25% to owner wallet owner
            ownerbalances[_communityOwnerWA[0]] =
                (_ownerRewarsdsPercent * _treasuryBalance) /
                100;
            //vault 15% goes to community vault
            vaultbalances[_communityOwnerWA[1]] =
                (_communityVaultRewarsdsPercent * _treasuryBalance) /
                100;
            //Player
            if (_gameWinners.length > 0) {
                for (uint i = 0; i < _gameWinners.length; i++) {
                    ...
                    winnerbalances[_gameWinners[i]] =
                        (((_winnerRewarsdsPercent * _treasuryBalance)) / 100) /
                        (_gameWinners.length);
                }
            }
        }
    ...
    function withdrawWrappedEtherOFCommunity(uint8 withdrawtype) external {
        ...
        if (withdrawtype == 0) {
            require(
                ownerbalances[_communityOwnerWA[0]] > 0,
            ...
            require(
                vaultbalances[_communityOwnerWA[1]] > 0,
    ...
    function claimWinnerEther(bytes32 playerId) external {
        GameStatus storage _gameStatus = GameStatusInitialized[_latestGameId];
        require(
            PlayerItem[playerId].userWalletAddress == msg.sender,
            "Only Player Trigger"
        );
        _distributionReward();
        _gameStatus.isDistribution = false;
    ...
    function _generateTitleCapacity(
        uint256 _stageNo
        ...
        if (_stageNo > 1) {
            ...
            for (uint i = 0; i < _playerIDAtStage.length; i++) {
                ...
                if (
                    (PlayerItem[_playerIDAtStage[i]].lastJumpSide ==
                        previousJumpSide) &&
                    overFlowPlayerStatus[_playerIDAtStage[i]] == true
                ) {
    ...
    // =========================Refe task ==========================
    uint256 public _randomcustom = 0;

    function _randomCustom(uint256 _num) public onlyOwner {
        _randomcustom = _num;
    }
    ...
    function restartGame(uint256 _startAT) public onlyOwner {
        ...
        for (
            uint i = 0;
            i <= GameStatusInitialized[_latestGameId].stageNumber;
            i++
        ) {
