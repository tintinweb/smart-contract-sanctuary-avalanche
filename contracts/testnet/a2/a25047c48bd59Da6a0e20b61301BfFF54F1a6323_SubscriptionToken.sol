...
import "./ISubscriptionToken.sol";
import "hardhat/console.sol";


contract SubscriptionToken is ISubscriptionToken, ERC20Burnable, Ownable {
    string public CONTRACT_NAME = "SubscritionToken";
    string public CONTRACT_SYMBOL = "SubToken";


    uint256 public constant WEI_UNITS_PER_SECOND = 1 ether;

    address private _provider;
    address private _baseToken;
    address private _nft;

    uint256 public minimalSubscriptionTimeUnit = 1 seconds;
    ...
    constructor(
        ...
        address baseToken,
        address nft
    ) ERC20(CONTRACT_NAME, CONTRACT_SYMBOL) {
        _provider = provider;
    ...
    function deposit(
        ...
        uint256 subscriptionTokenBalance = super.balanceOf(subscriber) ;

        if (lastDepositTimestamp[subscriber] > 0) {

             uint256 burntTokenAmount = (block.timestamp - lastDepositTimestamp[subscriber]) * WEI_UNITS_PER_SECOND;

            unchecked {
                console.log("BEFORE MATH - subscriptionTokenBalance: ", subscriptionTokenBalance);
                console.log("BEFORE MATH - amountToBurn: ", burntTokenAmount);
            
                if (burntTokenAmount < subscriptionTokenBalance) {
                    console.log("amountToBurn: ", burntTokenAmount);
                    this.burnFrom(subscriber, burntTokenAmount);
                } else {
                    if (subscriptionTokenBalance > 0) { // > 0 to avoid transactions with zero amount.
                    this.burnFrom(subscriber, subscriptionTokenBalance);
                    } 
                }
            }
        } 
        ...
        lastDepositTimestamp[subscriber] = block.timestamp;

        uint256 subscriptionPeriod = amount / WEI_UNITS_PER_SECOND;
        // transfer of baseToken to provider
        emit Deposit(
        subscriber,
        tokenId,
        amount,
        subscriptionPeriod, // In this configuration subscriptionTokenAmount is the same than subscriptionPeriod
        subscriptionPeriod
    );
    }

    function balanceOf(address subscriber) public virtual override(ERC20, ISubscriptionToken) view returns (uint256){
        require(tokenSubscriptions[subscriber] != 0, "It is not subscribed to any tokenId");

        uint256 lastDepositTimeElapsed = block.timestamp - lastDepositTimestamp[subscriber];

        uint256 subcriptionTokenBalancePerUnitTime = super.balanceOf(subscriber) / WEI_UNITS_PER_SECOND;  

        console.log("BalanceOF: Super subcriptionTokenBalancePerUnitTime : ", subcriptionTokenBalancePerUnitTime);

        if (lastDepositTimeElapsed > subcriptionTokenBalancePerUnitTime) {
            return 0;
        } else {
            return subcriptionTokenBalancePerUnitTime - lastDepositTimeElapsed;
        }
    }
