...
contract DeepSquare is ERC20, Ownable {
    ...
    /**
     * @notice Transfer DPS to multiple accounts atomically.
     * @dev This function is useful when running airdrops. The same security policy as transfer applies.
     * Maximum recipients number is correlated with the gas used.
     * Requires that the recipients number is equals to the amounts number.
     * @param recipients The recipient accounts.
    ...
    function transferBatch(address[] memory recipients, uint256[] memory amounts) public returns (bool) {
        require(recipients.length != 0, "DeepSquare: recipient length is zero");
        require(recipients.length == amounts.length, "DeepSquare: arguments size mismatch");
