...
contract SalvorCollection is ERC721, ERC721Enumerable, ERC2981, ReentrancyGuard, Ownable {
    ...
    bool public isPublicSaleActivated;
    bool public isWlSaleActivated;
    ...
    */
    ...
    constructor(address _receiver) ERC721("test", "TST") {
        maxCap = 10000;
        // 0.3 ether
        publicPrice = 300000000000000000;
    ...
    * @param _operatorContract The address of the admin contract.
    */
    function setOperatorContract(address _operatorContract) external onlyOwner {
    ...
    */
    function tooglePublicSale(bool _isPublicSaleActivated) external onlyOwner {
        isPublicSaleActivated = _isPublicSaleActivated;
    }
    ...
    */
    function toogleWlSale(bool _isWlSaleActivated) external onlyOwner {
        isWlSaleActivated = _isWlSaleActivated;
    }
    ...
    */
    function setWhitelistInfo(uint8 part, uint256 _maxCap, uint256 _price) external onlyOwner {
        uint256 totalMaxCap;
        for (uint8 i = 0; i < 2; i++) {
            totalMaxCap += wls[part].maxCap;
        }
        require(totalMaxCap <= maxCap, "Total maximum capacity exceeded");
    ...
    */
    function wlMint(uint8 _part, uint256 _amount) external payable isEOA nonReentrant {
        require(isWlSaleActivated, "Whitelist sale has not been started yet");
        ...
        if (_part == 0) {
            ...
            uint256 mintAmount = users[_part][msg.sender].mintAmount + users[_part - 1][msg.sender].mintAmount;
            require((mintAmount + _amount) <= allowedAmount, "Allowed amount has been reached for this user");
        }
    ...
    */
    function publicMint(uint256 _amount) external isEOA payable {
        require(isPublicSaleActivated, "Public sale has not been started yet");
        require(_amount <= 20, "Exceeded maximum buy limit of 20 tokens per transaction");
        require((totalSupply() + _amount) <= maxCap, "Maximum token supply reached");
        require(msg.value >= _amount * publicPrice, "Insufficient payment amount");
