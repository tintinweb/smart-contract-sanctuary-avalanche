...
/// @notice Manage the list of epochs.
///         Collect requests to pay salary for the current epoch
///         Collect approvals and update approve-statuses of the requests on the fly
///         When the current epoch ended, all approved requests are converted to company-debts.
contract RequestsManager is RequestsManagerStorage {
  ...
  function createRequest(
    ...
    if (bytes(descriptionUrl_).length == 0) {
      revert ErrorEmptyString();
    }
    if (countHours_ >= MAX_ALLOWED_HOURS) {
      revert ErrorTooManyHours(countHours_, MAX_ALLOWED_HOURS);
    }
    if (bytes(descriptionUrl_).length >= MAX_URL_LENGTH) {
      revert ErrorTooLongString(bytes(descriptionUrl_).length, MAX_URL_LENGTH);
    }
  ...
  , string memory explanation_
    ...
    (RequestStatus requestStatus, uint32 countPositive, uint32 countNegative)
      = decodeRequestStatusValue(requestsStatusValues[requestUid]);

    if (requestStatus == RequestStatus.Unknown_0) {
      revert ErrorUnknownRequest(requestUid);
    }
    ...
    if (lenExplanation >= MAX_EXPLANATION_LENGTH) {
      revert ErrorTooLongString(lenExplanation, MAX_EXPLANATION_LENGTH);
    }

    IController c = IController(_controller());
    WorkerUid workerUid = IDebtsManager(c.debtsManager()).getRequestWorker(requestUid);
    if (!IApprovalsManager(c.approvalsManager()).isApprover(msg.sender, workerUid)) {
      revert ErrorNotApprover(msg.sender, workerUid);
    }

    ApprovalUid approvalUid = _getApprovalUid(msg.sender, requestUid);
    ...
    if (prevApprovedValue == APPROVAL_UNKNOWN
      ...
      requestApprovals[requestUid].push(msg.sender);
      if (approveValue_) {
        countPositive++;
      } else {
        countNegative++;
    ...
    _refreshRequestStatus(requestUid, requestStatus, countPositive, countNegative);
  }
  ...
  function _refreshRequestStatus(RequestUid requestUid_
    ...
    RoleUid role = dm.getRequestRole(requestUid_);
...
import "../../interfaces/IRequestsManager.sol";
import "../../interfaces/IRequestsManagerBase.sol";
...
, IRequestsManager
{
  ...
  // TODO: currently this mapping is not used in the code
