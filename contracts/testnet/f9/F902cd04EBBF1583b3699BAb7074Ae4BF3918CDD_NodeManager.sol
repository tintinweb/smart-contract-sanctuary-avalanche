...
contract NodeManager is Ownable, Pausable {
    ...
    function getNodesRewards(address _account, uint256 _time, uint256 k) external view returns (uint256) {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	uint256 lastClaim = nodes[k].lastClaimTime;
    	uint256 rewards = boostLib.calcReward(dailyRewardsPerc,timeStep,_time,lastClaim,boostMultiplier[0]);
    	return rewards;
   	}
