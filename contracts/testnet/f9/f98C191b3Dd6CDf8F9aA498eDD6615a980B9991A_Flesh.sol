...
contract Flesh is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    uint96 public rottingRatio = 1000;
    ...
    constructor(address rottenFlesh) ERC20("Flesh", "FLSH") {
    ...
    function withdrawRFLSH() external whenNotPaused {
        ...
        emit WithdrewRottenFlesh(
            block.timestamp,
            msg.sender,
            rottenFleshToWithdraw(msg.sender)
        );
    }

    /// @notice Mints the amount of rFLSH a user is eligible to.
    /// This method also refreshes user's true FLSH balance
    function _withdrawRFLSH(address account) internal {
        uint256 _rottenFleshToWithdraw = rottenFleshToWithdraw(account);
        ...
        if (
            !hasRole(FREEZER_ROLE, msg.sender) &&
            // equivalent of balanceOf(msg.sender) in this case
            // but super.balanceOf(msg.sender) method is more efficient
            super.balanceOf(msg.sender) > 0
        ) {
            _timers[msg.sender] = block.timestamp;
        } else {
        ...
        returns (uint256 balance)
    {
        ...
        if (!hasRole(FREEZER_ROLE, account)) {
            if (_timers[account] > 0) {
                ...
                uint256 timeElapsed = Math.max(
                    0,
                    block.timestamp -
                        (_timers[account] -
                            (_timers[account] % rottingInterval))
                ) / rottingInterval;

                // Balance ponderated by elapsed time and given rotting ratio
                balance =
                    (super.balanceOf(account) *
                        ((uint256(_percentageDenominator() - rottingRatio)) **
                            timeElapsed)) /
                    (uint256(_percentageDenominator())**timeElapsed);
            }
        }
    }
    ...
    function setRottenFlesh(address rottenFlesh)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _rottenFlesh = IRottenFlesh(rottenFlesh);
    ...
    function setRottingRatio(uint96 _rottingRatio)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        rottingRatio = _rottingRatio;
    }
