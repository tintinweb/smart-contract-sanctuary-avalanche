...
contract AdapterManager is IAdapterManager, Ownable {
    ...
    mapping(address => bool) private suspendPermissions;
    ...
    modifier onlyUser() {
        require(
            ...
            "Only the onepiece user can call this function"
        );
        _;
    }
    ...
    ) external payable override onlyUser whenNotPaused returns (bytes memory) {
        require(
            adapterIsRegistered(adapter),
            "callOnAdapter: Adapter is not registered"
        );
        return __callOnAdapter(msg.sender, adapter, selector, callArgs);
    }
        ...
        onlyTimelock
    {
        require(
            _adapters.length > 0,
            "registerAdapters: _adapters cannot be empty"
        );
        for (uint256 i; i < _adapters.length; i++) {
            require(
                _adapters[i] != address(0),
                "registerAdapters: Adapter cannot be empty"
            );
            require(
            ...
            emit AdapterRegistered(
                _adapters[i],
                IAdapter(_adapters[i]).identifier()
            );
        }
    }
        ...
        onlyTimelock
    {
        ...
        for (uint256 i; i < _adapters.length; i++) {
            require(
                _adapters[i] != address(0),
                "unregisterAdapters: Adapter cannot be empty"
            );

            require(
                adapterIsRegistered(_adapters[i]),
                "unregisterAdapters: Adapter is not exist"
            );
    ...
    function adapterIsRegistered(address _adapter)
        public
        view
        override
