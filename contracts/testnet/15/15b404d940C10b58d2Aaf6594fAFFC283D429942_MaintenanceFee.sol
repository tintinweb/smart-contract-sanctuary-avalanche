    ...
    AccessControlUpgradeable
{
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (
                ...
                maintFee[_tokenIds[i]].firstDay = uint64(
                    (block.timestamp).sub(maintFee[_tokenIds[i]].unClaimedDays >
                    Rewards.RewardsClaimed(_tokenIds[i]).claimedDays
                    ? (
                        maintFee[_tokenIds[i]].unClaimedDays.sub(
                            Rewards.RewardsClaimed(_tokenIds[i]).claimedDays
                        )
                    ).mul(3 minutes)
                    : 0)
            ...
            maintFee[_tokenIds[i]].feeDue = ((maintFee[_tokenIds[i]].feeDue ==
                ...
                : uint64(uint256(maintFee[_tokenIds[i]].feeDue).add(_paidDays[i].mul(3 minutes)));
            maintFee[_tokenIds[i]].unClaimedDays += _paidDays[i];
            if (_type == 12) {
                if (
                    !isYieldBoxInArray(
    ...
    function getFeeDue(uint256 _tokenId) public view returns (uint64 feeDue) {
        feeDue = maintFee[_tokenId].feeDue > uint64(block.timestamp)
            ? uint64((maintFee[_tokenId].feeDue - uint64(block.timestamp)) / 3 minutes)
            : 0;
    }
