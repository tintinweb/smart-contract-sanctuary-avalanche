...
contract FeeConverter is BaseComponent, IFeeConverter {
    ...
    /**
     * @notice Sends the redistributed token balance to the receiver.
     */
    function redistribute() external override onlyDelegateCall {
        uint256 balance = _REDISTRIBUTED_TOKEN.balanceOf(address(this));
        if (balance == 0) revert FeeConverter__InsufficientRedistributedTokenBalance();

        _REDISTRIBUTED_TOKEN.safeTransfer(_RECEIVER, balance);

        emit Redistributed(_RECEIVER, balance);
    }
    ...
    function _swap(address executor, IOneInchRouter.SwapDescription calldata desc, bytes calldata data) private {
        if (desc.dstToken != _REDISTRIBUTED_TOKEN) revert FeeConverter__InvalidSrcToken();
        if (desc.dstReceiver != address(this)) revert FeeConverter__InvalidReceiver();
        ...
        (uint256 returnAmount, uint256 spentAmount) = _ONE_INCH_ROUTER.swap(executor, desc, "", data);

        emit Swap(address(desc.srcToken), address(desc.dstToken), spentAmount, returnAmount);
    }
...
interface IFeeConverter is IBaseComponent {
    ...
    error FeeConverter__InvalidSrcToken();
    error FeeConverter__ZeroAmount();
    error FeeConverter__InsufficientRedistributedTokenBalance();

    event Swap(address indexed srcToken, address indexed dstToken, uint256 amount, uint256 returnAmount);

    event Redistributed(address indexed redistributedToken, uint256 amount);
