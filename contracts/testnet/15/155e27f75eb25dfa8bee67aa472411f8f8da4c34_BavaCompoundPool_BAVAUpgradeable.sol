...
contract BavaCompoundPool_BAVAUpgradeable is Initializable, UUPSUpgradeable, BRTERC20Upgradeable {
    ...
    function setAllowances(uint256 _amount) external onlyOwner {  
        ...
        if (address(router) != address(0)) {
            ...
            IERC20(IPair(address(vaultInfo.lpToken)).token0()).approve(address(router), _amount);
            IERC20(IPair(address(vaultInfo.lpToken)).token1()).approve(address(router), _amount);
