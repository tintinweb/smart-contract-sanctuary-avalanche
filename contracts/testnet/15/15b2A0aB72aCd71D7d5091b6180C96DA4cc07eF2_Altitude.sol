...
/******************************************/
    ...
    function swap(uint16 _dstChainId, address _dstToken, uint256 _amount, bytes memory _to) public payable {
        ...
        executeLayerZero(TYPE_SWAP_REMOTE, cp, swapAmount, _to);
    ...
    function addLiquidity(uint16 _dstChainId, address _dstToken, uint256 _amount) public payable {
        ...
        executeLayerZero(TYPE_ADD_LIQUIDITY, cp, _amount, "");
    ...
    function removeLiquidityLocal(uint16 _dstChainId, address _dstToken, uint256 _amount) public payable {
        ...
        cp.localLiquidity -= _amount;
        IERC20(cp.srcToken).transfer(msg.sender, _amount);

        executeLayerZero(TYPE_REMOVE_LIQUIDITY, cp, _amount, "");
    ...
    function executeLayerZero(uint8 functionType, ChainPath storage cp, uint256 _amount, bytes memory _to) internal {
        ...
        if (functionType == TYPE_SWAP_REMOTE) {
            ...
            adapterParams = getAndCheckGasFee(TYPE_REDEEM_LIQUIDITY, cp.dstChainId, payload);
        }
