...
contract CreditVault is AccessControlEnumerable, Pausable, EIP712 {
    ...
    constructor(string memory name, string memory version) EIP712(name, version) {
        ...
        _grantRole(TRUST_SIGNER_ROLE, _msgSender());
    ...
    function deposit(
        ...
        if (token == address(0)) return deposit();
    ...
    function _deposit(
        ...
        require(ercVersion > 0, "Vault: token not supported");

        if (ercVersion == 1) {
            require(tokenId == 0, "Vault: invalid tokenId");
            IERC20(token).transferFrom(_msgSender(), address(this), amount);
        } else if (ercVersion == 2) {
            require(amount == 1, "Vault: invalid amount");
            IERC721(token).transferFrom(_msgSender(), address(this), tokenId);
        } else if (ercVersion == 3) {
            IERC1155(token).safeTransferFrom(_msgSender(), address(this), tokenId, amount, "0x");
        }
    }
    ...
    function withdraw(
        ...
        require(account != address(0), "Vault: invalid account");
        require(block.timestamp <= expiry, "Vault: signature expired");
        require(nonce == _useNonce(account), "Vault: invalid nonce");
        require(validateTrustSig(account, token, tokenId, amount, nonce, expiry, signature), "Vault: invalid signature");

        if (token == address(0)) return _withdraw(account, amount, nonce);
        _withdraw(account, token, tokenId, amount);
    ...
    function _withdraw(
        ...
        emit Withdraw(account, address(0), 0, amount, nonce);
    }
    ...
    function _withdraw(
        ...
        if (ercVersion == 1) {
            ...
            if (currentBalance < amount) {
                ...
                IERC20Mintable(token).mint(address(this), mintAmount);
            }
            ...
            IERC1155(token).safeTransferFrom(address(this), account, tokenId, amount, "0x");
        }
    }
    ...
    function addToken(address token, uint8 ercVersion) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Vault: only admin can add token");
