...
// Type imports
import {Hero, UpgradeTechType, Tech, Resources} from "../NGTypes.sol";
import {UpgradeRequirements, UpgradeTechRequirementMetaData} from "../NGTypes.sol";
...
contract NGLibrary is WithStorage {
    ...
    // function _checkAndSpendResourcesDelegate(Hero memory hero, UpgradeRequirements memory upgradeRequirements) private {
    //     (bool success, bytes memory result) = address(this).delegatecall(
    //         abi.encodeWithSignature("CheckAndSpendResources(Hero,UpgradeRequirements)", hero, upgradeRequirements)
    //     );
    ...
    function _getUpgradeTimeTech(uint16 level, uint256 upgradeTimeInitialSeconds, uint256 upgradeTimePerLevelFactor ) private view returns (uint256){
    ...
    function upgradeLibrary(uint256 tokenId) public notPaused resourceAddresses {
        ...
        hero.libraryUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.libraryLevel + 1, 200, 800);
    ...
    function upgradeTech(uint256 tokenId, UpgradeTechType upgradeTechType) public notPaused resourceAddresses {
        ...
        if (upgradeTechType == UpgradeTechType.CashMax) {
            ...
            _checkAndSpendResourcesTech(hero, upgradeRequirementMetaData.upgradeRequirements);
            hero.techUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.tech.cashMax, 100, 700);
        }
        if (upgradeTechType == UpgradeTechType.WoodMax) {
            ...
            _checkAndSpendResourcesTech(hero, upgradeRequirementMetaData.upgradeRequirements);
            hero.techUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.tech.woodMax, 100, 700);
        }
        if (upgradeTechType == UpgradeTechType.StoneMax) {
            ...
            _checkAndSpendResourcesTech(hero, upgradeRequirementMetaData.upgradeRequirements);
            hero.techUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.tech.stoneMax, 100, 700);
        }
        if (upgradeTechType == UpgradeTechType.SteelMax) {
            ...
            _checkAndSpendResourcesTech(hero, upgradeRequirementMetaData.upgradeRequirements);
            hero.techUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.tech.steelMax, 100, 700);
        }
        if (upgradeTechType == UpgradeTechType.OilMax) {
            ...
            _checkAndSpendResourcesTech(hero, upgradeRequirementMetaData.upgradeRequirements);
            hero.techUpgradeTime = block.timestamp + _getUpgradeTimeTech(hero.tech.oilMax, 100, 700);
        }
...
import {Tech, UpgradeTechType} from "../NGTypes.sol";
...
// contract NGHeroFacet is WithStorage, ERC721 {
    ...
    function forceUnstakeHero(address _address) public onlyAdmin {
        // remove id from staked id
        gs().stakedTokenId[_address] = uint256(0);
    }
}
...
// Type imports
import {UpgradeRequirements, HeroRequirements, BuildingRequirements, ResourceRequirements, UpgradeTechRequirementMetaData} from "../NGTypes.sol";
...
contract NGResourcesFacet is WithStorage {
    ...
    function getAllResourcesStaked(uint256 tokenId) public view notPaused resourceAddresses returns(Resources memory resources) {
        ...
        // CASH
        uint256 cashRes = (block.timestamp - hero.cashResource.lastCollectedTime) * hero.cashResource.perHour / 3600;
        
        resources.cashPending = cashRes + hero.cashResource.unclaimedAmount;
        resources.cashPerHour = hero.cashResource.perHour;   
        ...
        // WOOD
        uint256 woodRes = (block.timestamp - hero.woodResource.lastCollectedTime) * hero.woodResource.perHour / 3600;

        resources.woodPending = woodRes + hero.woodResource.unclaimedAmount;
        resources.woodPerHour = hero.woodResource.perHour;
        ...
        // STONE
        uint256 stoneRes = (block.timestamp - hero.stoneResource.lastCollectedTime) * hero.stoneResource.perHour / 3600;

        resources.stonePending = stoneRes + hero.stoneResource.unclaimedAmount;
        resources.stonePerHour = hero.stoneResource.perHour;
        ...
        // STEEL
        uint256 steelRes = (block.timestamp - hero.steelResource.lastCollectedTime) * hero.steelResource.perHour / 3600;

        resources.steelPending = steelRes + hero.steelResource.unclaimedAmount;
        resources.steelPerHour = hero.steelResource.perHour;
        ...
        // OIL
        uint256 oilRes = (block.timestamp - hero.oilResource.lastCollectedTime) * hero.oilResource.perHour / 3600;

        resources.oilPending = oilRes + hero.oilResource.unclaimedAmount;
        resources.oilPerHour = hero.oilResource.perHour;
