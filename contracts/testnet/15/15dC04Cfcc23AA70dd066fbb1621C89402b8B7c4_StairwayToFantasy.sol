...
// Written by Daniel Abalde @ https://github.com/DanielAbalde/NFT-On-Chain-Metadata/blob/master/contracts/OnChainMetadata.sol
...
// Written by Daniel Abalde @ https://github.com/DanielAbalde/NFT-On-Chain-Metadata/blob/master/contracts/ERC721OnChainMetadata.sol
...
// File contracts/stairwaytofantasy.sol

pragma solidity ^0.8.12;
pragma solidity ^0.8.12;

contract StairwayToFantasy is ERC721OnChainMetadata, Ownable, ERC2981 {
    using Strings for uint256;

    // Variable Initialization
    string public baseURI =
        "https://nftstorage.link/ipfs/bafybeigs7k6br34py4s6huw7e32sefbnqrvdrkqygmlczpez3t56ks6xii/";
    uint256 public MAX_SUPPLY = 100;
    uint256 private _totalSupply = 0;
    uint256 public publicMintTime = 1690953753;
    uint256 public whitelistMintTime = 1659416685;
    mapping(address => bool) public whitelistedAddresses;
    mapping(address => bool) public hasMinted;

    // Runs on Contract Initialization
    constructor() ERC721OnChainMetadata("Stairway To Fantasy", "FANTASY") {
        _setValue(
            1,
            key_token_name,
            abi.encode(
                string.concat("Stairway to Fantasy #", Strings.toString(1))
            )
        );
        _setValue(
            1,
            ...
            abi.encode(
                "Things are not what they look like. We live in a world of fantasy..."
            )
        );
        _setValue(
            1,
            ...
            abi.encode(
                string.concat(
                    string.concat(baseURI, Strings.toString(1)),
                    ".png"
                )
            )
        );
    ...
    modifier isOpen() {
        require(block.timestamp > whitelistMintTime); // 5pm utc
        _;
    }
    ...
    function addWhitelist(address[] memory addresses) public onlyOwner {
        for (uint256 index = 0; index < addresses.length; index++) {
            whitelistedAddresses[addresses[index]] = true;
        }
    }
    ...
    function mint(uint256 quantity) public payable isOpen {
        ...
        if (!whitelistedAddresses[msg.sender]) {
            require(
                ...
                "Stairway to Fantasy: Please send the correct value"
            );
            require(
                block.timestamp > publicMintTime,
                "Stairway to Fantasy: Mint hasn't started yet."
            );
        } else {
            require(
                msg.value == 0.1 ether,
        ...
        require(
            quantity == 1,
            "Stairway to Fantasy: You cannot mint that many at once"
        );
        require(
            tokenId <= MAX_SUPPLY,
            "Stairway to Fantasy: Project has sold out"
        );
        require(
            !hasMinted[_msgSender()],
            "Stairway to Fantasy: You have minted an nft already."
        );
        _setValue(
            tokenId,
            key_token_name,
            abi.encode(
                string.concat("Stairway to Fantasy #", Strings.toString(tokenId))
            )
        );
        _setValue(
            tokenId,
    ...
    function changeDescription(uint256 tokenId, string memory description)
        public
    {
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "Stairway to Fantasy: You are not the owner of this NFT"
        );
    ...
    function withdrawAvax() external onlyOwner {
        ...
        require(
            balance > 0,
            "Stairway to Fantasy: Not enough AVAX on contract to withdraw"
        );
