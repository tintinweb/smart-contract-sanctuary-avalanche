...
/**
 * @title  A liquid staking derivative for ARV
 * @author alexintosh
 * @notice Tokens are stakingManager in perpetuity, no coming back
 *         PRV is a tradeable ERC20 token with ERC20 permit functionality for off-chain approvals
 *         Users can convert AUXO -> PRV, but not the other way around.
 * @dev    Depositing into Auxo will send tokens to a separate Staking Manager contract
 *         This will convert the AUXO to ARV and generate rewards for all PRV holders, which
 *         can be claimed at defined intervals by PRV holders.
 *         Rewards are not managed directly in this contract.
 *         A governor address is set which has certain administrative permissions.
 *         Once deployed, the staking manager must be configured,
 *         then the setup function must be called to initialize the PRV contract.
 */
contract PRV is ERC20, ERC20Permit, IPRVEvents {
    ...
    /**
     * @notice AUXO deposited is converted to ARV -
     *         This generates rewards which are distributed to PRV holders.
     *         The STAKING_MANANGER is the contract that holds accumulated ARV
     *         on behalf of PRV holders.
     */
    address public immutable STAKING_MANAGER;
    ...
    /**
     * @notice the _auxo and _stakingManager params cannot be changed after deployment
     *         and only the governor can change the governor parameter.
     */
    constructor(address _auxo, address _stakingManager, address _governor, uint256 _entryFee, address _feeBeneficiary)
        ERC20("Auxo Passive Reward Vault", "PRV")
        ERC20Permit("Auxo Passive Reward Vault")
    {
        require(_stakingManager != address(0), "SM:ZERO_ADDR");
        require(_auxo != address(0), "AUXO:ZERO_ADDR");
        require(_governor != address(0), "GV:ZERO_ADDR");
        require(_entryFee <= MAX_ENTRY_FEE, "FEE TOO BIG");
        ...
        emit EntryFeeSet(_entryFee);
        emit FeeBeneficiarySet(_feeBeneficiary);
    }

    /**
     * @notice initializes the PRV by minting the first PRV & instantiating a staking position with the staking manager.
     * @dev    We mint the first PRV here to msg.sender.
    ...
    function setup() external {
        IERC20(AUXO).safeTransferFrom(msg.sender, STAKING_MANAGER, 1 ether);
        IStakingManager(STAKING_MANAGER).stake();
    ...
    function depositFor(address _account, uint256 _amount) external {
        ...
        _depositAndStake(_account, amountMinFee);
    }
    ...
    function depositForWithSignature(
        ...
        // approve PRV to spend my AUXO and send to staking manager
        IERC20Permit(AUXO).permit(msg.sender, address(this), _amount, _deadline, v, r, s);
        uint256 amountMinFee = _chargeFee(_amount);
        _depositAndStake(_account, amountMinFee);
    }
    ...
    function _calcFee(uint256 _amount) internal view returns (uint256, uint256) {
        ...
        if (entryFee == 0 || feeBeneficiary == address(0)) return (_amount, 0);

        uint256 feeAmount = _amount * entryFee / (10 ** 18);
        uint256 amountMinFee = _amount - feeAmount;
        return (amountMinFee, feeAmount);
    }

    /**
     *  @dev    calculates the entry fee in auxo and sends to the fee beneficiary.
    ...
    function _chargeFee(uint256 _amount) internal returns (uint256) {
        ...
        // charge fee already checks for the fee beneficiary being set
     ...
     *      The staking manager will then deposit the Auxo into the ARV locker and mint PRV.
     */
    function _depositAndStake(address _account, uint256 _amount) internal {
        IERC20(AUXO).safeTransferFrom(msg.sender, STAKING_MANAGER, _amount);
        // Since it's increasing min amount does not matter
        IStakingManager(STAKING_MANAGER).increase();
        _mint(_account, _amount);
    }

    /// ====== VIEWS ======

    /// @return the amount of AUXO that will be minted for the given amount of AUXO, minus any entry fees.s
    ...
    /**
     * @notice This function sets the entry fee for the contract. Only the governor can call this function.
     * @param  _fee uint256 value of the entry fee, bounded at 10%.
     */
