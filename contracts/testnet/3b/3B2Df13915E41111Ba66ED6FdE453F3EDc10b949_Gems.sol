...
contract Gems is ERC1155, Ownable, Withdrawable {
  ...
  // event MintMany(address indexed owner, uint256 amount, uint256 price, uint256[] tokenIds);
  ...
  function mintMany(uint256 amount) public payable {
    ...
    safeMintMany(amount);
    // uint256[] memory mintedTokens = safeMintMany(amount);

    // emit MintMany(msg.sender, amount, totalPrice, mintedTokens);
  }
  ...
  function safeMintMany(uint256 amount) internal {
    ...
    // uint256[] storage mintedTokenIds;
