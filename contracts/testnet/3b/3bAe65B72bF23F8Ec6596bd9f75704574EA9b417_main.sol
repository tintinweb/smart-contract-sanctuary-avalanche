...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address _destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256[4] memory _fees = [myLib.doPercentage(nodeAmount,rewardsFee),myLib.doPercentage(nodeAmount,treasuryFee),myLib.doPercentage(nodeAmount,teamPoolFee),nodeAmount.sub(myLib.doPercentage(nodeAmount,teamPoolFee)).sub(myLib.doPercentage(nodeAmount,treasuryFee)).sub(myLib.doPercentage(nodeAmount,rewardsFee))];
