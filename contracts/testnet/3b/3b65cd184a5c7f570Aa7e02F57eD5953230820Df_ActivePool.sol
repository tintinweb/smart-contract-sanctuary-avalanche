...
contract ActivePool is Ownable, CheckContract, IActivePool, PoolBase2 {
        ...
        returns (uint256 totalVC, uint256 totalVCforTCR)
    {
        ...
        (totalVC, totalVCforTCR) = controller.getValuesVCforTCR(poolColl.tokens, summedAmounts);
    }
    ...
    function sendCollateralsUnwrap(
        ...
        for (uint256 i; i < tokensLen; ++i) {
            if (isWrapped[i]) {
                ...
                IYetiVaultToken(_tokens[i]).withdrawFor(_amounts[i], address(this), _to);
            } else {
    ...
    function sendSingleCollateralUnwrap(
        ...
        if (controller.isWrapped(_token)) {
            // Unwraps for original owner. _amounts[i] is in terms of the receipt token, and
            // the user will receive back the underlying based on the current exchange rate. 
            IYetiVaultToken(_token).withdrawFor(_amount, address(this), _to);
        } else {
            _sendCollateral(_to, _token, _amount, controller.getIndex(_token)); // reverts if send fails
        }
    }
