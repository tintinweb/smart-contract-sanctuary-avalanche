...
contract YieldKey is ERC721Enumerable, ERC721Burnable, Pausable, Random {
    ...
    /** @dev Mapping of Yield Box are minted in the Each Phase*/
    mapping(address => uint256) public totalMinted;
    /** @dev Amount of Token IDs Minted */
    uint256 public totalMintCnt = 0;
    /** @dev Address Whiteliested */
    mapping(address => bool) public whitelist;
    /** @dev Mapping for storage of Activation Capacity per token*/
    ...
    /** @dev USDC Token Compatible Native */
    address public constant USDC_ADDRESS =
        address(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E);
    /** @dev USDC.e Test Token Compatible with Avalanche Testnet */
    ...
    constructor(
		...
		modifier onlyAdmin() {
        require(_administrators[msg.sender], "Only administrator can call this function");
        _;
    }
     ...
     */
    function addToWhitelist(address _address) external validAddress(_address) onlyAdmin() {
    ...
    function mint(
        ...
        address _to,
        uint256 _mintAmount
    ) external validAddress(_stable) whenNotPaused returns (uint256[] memory) {
        ...
        // IERC20Metadata _stableToken = IERC20Metadata(_stable);
        require(
            IERC20Metadata(_stable).balanceOf(_msgSender()) >=
                amountCoin[_stable].mul(_mintAmount) *
                    (10**IERC20Metadata(_stable).decimals()),
            "ERC721 ValiFi: Insufficient Funds"
        );
        // Transfer the Stable Coin to Create the Yield Key
        bool success_treasury = IERC20Metadata(_stable).transferFrom(
            _msgSender(),
            treasuryAddress,
            amountCoin[_stable].mul(_mintAmount) *
                (10**IERC20Metadata(_stable).decimals())
        ...
        require(
            success_treasury,
            "ERC721 ValiFi: Can't create Yield-Key, funds transfer failed"
        );

        uint256[] memory _amountOfKey = new uint256[](_mintAmount);
        for (uint256 i = 0; i < _mintAmount; i++) {
            _safeMint(_to, supply + i);
            capacityAmount[supply + i] = getCapacityAmount();
            _amountOfKey[i] = capacityAmount[supply + i];
    ...
    {
        uint256[] memory tokenIds = tokenHoldings(_owner);
     ...
     * See {ERC721Pausable}.
     */
    function MintPause(bool status) public onlyAdmin() {
        if (status) {
            mintPause = true;
        } else {
            mintPause = false;
        ...
        whenNotPaused
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            AmountKeys[i] = capacityAmount[tokenIds[i]];
        }
