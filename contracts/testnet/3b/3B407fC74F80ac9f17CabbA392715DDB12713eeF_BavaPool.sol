...
// File: bava2/IBavaMasterFarm.sol
...
contract BavaPool is BRTERC20 {
    ...
    event Liquidate(address indexed userAccount, uint256 amount, uint256 depositTokenAmount);
    ...
    function initPool(IERC20 _lpToken, IMiniChef _stakingPglContract, uint256 _restakingFarmID, IERC20 _rewardToken, IERC20[] memory _bonusRewardTokens) external onlyOwner {        
    ...
    function setAllowancesRouter(uint256 _amount) external onlyOwner {   
        ...
        if (address(router) != address(0)) {
            ...
            IERC20(IPair(address(pool.lpToken)).token0()).approve(address(router), _amount);
            IERC20(IPair(address(pool.lpToken)).token1()).approve(address(router), _amount);
    ...
    function setBavaMasterFarm(IBAVAMasterFarm _BavaMasterFarm) external onlyOwner {
        BavaMasterFarm = _BavaMasterFarm;
    }
  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    ...
    // Liquidate user collateral when 
    function liquidateCollateral(address userAccount, uint256 amount) public onlyAuthorized {
        PoolInfo storage pool = poolInfo;
        ...
        user.rewardDebt = user.receiptAmount * (accBavaPerShare) / (1e12);
        pool.depositAmount -= depositTokenAmount;
        pool.lpToken.safeTransfer(address(liqaddr), depositTokenAmount);

        emit Liquidate(userAccount, amount, depositTokenAmount);
    }
    ...
    function emergencyWithdrawDepositTokens(bool disableDeposits) external onlyOwner {
        ...
        if (pool.deposits_enabled == true && disableDeposits == true) {
            updateDepositsEnabled(bavaPid, false);
        }
    }
    ...
    function getSharesForDepositTokens(uint amount) public view returns (uint) {
        PoolInfo storage pool = poolInfo;
        if (totalSupply()*pool.depositAmount == 0) {
            return amount;
        }
        return amount*totalSupply()/pool.depositAmount;
    }
    ...
    function getDepositTokensForShares(uint amount) public view returns (uint) {
        ...
        return amount*pool.depositAmount/totalSupply();
    }
    ...
    function pendingReinvestReward() public view returns (uint256 pending, uint256[] memory) {
        PoolRestakingInfo storage poolRestaking = poolRestakingInfo;
        ...
        return (pendingRewardAmount, pendingBonusToken);
    }

    /**
    * Autocompound strategy - Swap all reward tokens to WAVAX and swap half/half WAVAX token to both LP  token0 & token1, Add liquidity to LP token
    */
    ...
    function _convertRewardIntoWAVAX() public returns (uint256) {
        uint pathLength = 2;
        ...
        router.swapExactTokensForTokens(rewardBal, amountOutToken, path, address(this), block.timestamp+300);
        ...
        //     router.swapExactTokensForTokens(rewardBal, bonusAmountOutToken, path, address(this), block.timestamp+300);
        // }
        avaxAmount = WAVAX.balanceOf(address(this));
        return avaxAmount;
    }

    function _convertWAVAXTokenToDepositToken(uint256 amount) public returns (uint256) {
        require(amount > 0, "Amount < 0");
        PoolInfo storage pool = poolInfo;
        ...
        if (path0[0] != path0[path0Length - 1]) {
            ...
            router.swapExactTokensForTokens(amountIn, amountOutToken0, path0, address(this), block.timestamp+300);
        }
        ...
        if (path1[0] != path1[path1Length - 1]) {
            ...
            router.swapExactTokensForTokens(amountIn, amountOutToken1, path1, address(this), block.timestamp+300);
        }
