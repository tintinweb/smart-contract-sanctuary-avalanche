...
contract NodeManager is Ownable, Pausable {
    ...
    uint256[] public rewardboost = [125,150,175];
    event NodeCreated(
    ...
    constructor(
        ...
        minPrice = _minPrice;
        nodeAmount = nodeAmount * (10**18); //amount to putchase node
    }
    // Private methods
    ...
    function checkTier(address sendit) private view returns(uint[5] memory){
	...
	uint bal_3 = IERC721(tier_3).balanceOf(sendit);
    	return [rewardsBoost_perc,cashoutFee_perc,bal_1,bal_2,bal_3];
    	}
    function _calculateNodeRewards(address account,uint _lastClaimTime, uint256 amount_, uint256 num) private view returns (uint256 rewards) {
        
    	uint256[5] memory tiers = checkTier(account);
        uint256 elapsedTime_ = (block.timestamp - _lastClaimTime);
        uint256 rewardPerDay = amount_.mul(rewardPerNode).div(100);
        uint256 boostMultiplier;
        uint256 t_num_1 = 5*tiers[2];
        uint256 t_num_2 = t_num_1.add(5*tiers[3]);
        uint256 t_num_3 = t_num_2.add(5*tiers[4]);
        
        if (num <= t_num_1){
        	boostMultiplier = (rewardboost[0]).div(100);
        }
        if (t_num_2 >= num && num > t_num_1){
        	boostMultiplier = (rewardboost[1]).div(100);
        }
        if ( t_num_3 >= num && num> t_num_2){
        	boostMultiplier = (rewardboost[2]).div(100);
        }
        ...
        returns (uint256)
    {
        ...
        return _calculateNodeRewards(account,node.lastClaimTime, node.amount,0);
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            rewardsTotal += _calculateNodeRewards(account,_node.lastClaimTime, _node.amount,i);
        }
        return rewardsTotal;
    }
