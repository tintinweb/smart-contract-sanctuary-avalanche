...
contract StableCoin is ERC20PresetMinterPauser{
    ...
    function transferWithData(
        ...
        uint256 amount,
        bytes memory data
    ) public onlyTokenism(msg.sender, account) returns (bool success) {
        msgSender = msg.sender;

        if (!userExist[account]) {
            // Use to maintain user list
            userExist[account] = true;
            contributorAddresses.push(account);
        }
        ...
        if (Address.isContract(account)) {
            ...
            receiver.tokenFallback(_msgSender(), amount, data);
            //console.log("in stable coin");
        } else _transfer(_msgSender(), account, amount);
        return true;
    }
    ...
    function transfer(
        ...
        public
        override
        onlyTokenism(msg.sender, account)
        returns (bool success)
    {
        ...
        if (Address.isContract(account)) {
            IERC223Recipient receiver = IERC223Recipient(account);
            _transfer(_msgSender(), account, amount);
            receiver.tokenFallback(_msgSender(), amount, empty);
        } else _transfer(_msgSender(), account, amount);
        return true;
    }
   /**
    ...
    function mint(address account, uint256 amount)
        ...
        onlyAdmin
    {
        // require(isWhitelisted(account), "Cannot send tokens outside Tokenism");
        uint256 code = whitelist.isWhitelistedUser(account);
        require(
            code <= 201,
    ...
    /**
     * @dev changeWhitelist update the whitelist address
     * @param whitelisted  new whitelist address
     */
    function changeWhitelist(ITokenismWhitelist whitelisted)
        public
        onlyAdmin
        returns (bool)
    {
    ...
    /**
     * @dev getAllInvestor To Destroy Stable Coin Get All STable Coin Holder
     */
        ...
        returns (address[] memory, uint256[] memory)
    {
     ...
     /**
     * @dev closeStableCoin Destruct Stable Coin Contract Address
     */
