...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => DQPoolItem)) dqPoolItem;
    ...
    */
    ...
    function stakeNFTToBoostVeART(address _collection, uint256[] calldata _tokenIds) external payable whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            dqPoolItem[_collection][_tokenIds[i]].owner = msg.sender;
            dqPoolItem[_collection][_tokenIds[i]].endedAt = block.timestamp + dqPools[_collection].withdrawDuration;
            emit DqStake(msg.sender, _collection, _tokenIds[i], dqPoolItem[_collection][_tokenIds[i]].endedAt);
        }
    ...
    function withdrawNFTFromBoostingPool(address _collection, uint256[] calldata _tokenIds) external whenNotPaused nonReentrant {
        ...
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
            require(dqPoolItem[_collection][_tokenIds[i]].owner == msg.sender, "The provided NFT does not belong to the sender");
            require(dqPoolItem[_collection][_tokenIds[i]].endedAt <= block.timestamp, "The provided NFT has not yet expired, and cannot be withdrawn from the boosting pool.");

            ISalvorMini(_collection).safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
            delete dqPoolItem[_collection][_tokenIds[i]];
