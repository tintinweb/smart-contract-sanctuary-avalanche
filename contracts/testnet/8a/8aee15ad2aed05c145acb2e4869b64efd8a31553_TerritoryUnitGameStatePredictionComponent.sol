...
contract TerritoryUnitGameStatePredictionComponent {
    ...
    constructor () {
        ...
        _addAttacker(0, address(0), 0, 100, 1000, 3);
        _addAttacker(0, address(0), 0, 200, 100, 4);
        _addAttacker(0, address(0), 0, 300, 150, 5);
    }
    ...
    function _removeAttacker(uint _poolId, address _contractReference, uint _tokenIdReference) public {
        ...
        if (_unitCurrentlyAttacked != 0) {
            ...
            attackersIndex[_uniqueId].lp -= 0;
            attackersIndex[_unitCurrentlyAttacked].lp -= 0;
    ...
    // current unit who the unit "_uniqueId" is attacking at timestamp "_timestamp"
    function getUnitAttackedForUniqueIdAndTimestamp(uint _poolId, uint _uniqueId, uint _timestamp) public returns (uint attackingUnit) {
        // if the unit is currently fighting
        if (attackersIndex[_uniqueId].fightEntry <= _timestamp && (attackersIndex[_uniqueId].predictedDeathTime > _timestamp || attackersIndex[_uniqueId].predictedDeathTime == 0)) {
            uint winningUnit = cal(getWinningUnitFromTimestamp(_poolId, _timestamp)); // get the current possesor of the fight pool at "_timestamp"
            ...
            if (winningUnit == _uniqueId) { // if "uniqueId" unit is possesing the pool, it can only be attacking the next attacker to arrive
                ...
                for (uint i=_id; i < _areaFightPools.length - 1; i++) {
                    if (cal(attackersIndex[_areaFightPools[_id+1]].fightEntry) <= _timestamp && cal(attackersIndex[_areaFightPools[_id+1]].predictedDeathTime) > _timestamp) { // if the next attacker is fighting, it's the current unit attacked ..
                        attackingUnit = cal(_areaFightPools[_id+1]);
                        break;
                    }
                }
            } else { // else, it is just in fight with the current pool possesor
                attackingUnit = cal(winningUnit);
            }
        }
    }
    ...
    function _update(uint _poolId) public {
        ...
        for (uint i=1; i < _attackersUniqueIds.length; i++) {
            ...
            if (deathScoreB > deathScoreA) { // Attacker B win
                ...
                winningUnitUniqueId = 0; // nobody got the pool
                winningUnitLp = 0;
            }
