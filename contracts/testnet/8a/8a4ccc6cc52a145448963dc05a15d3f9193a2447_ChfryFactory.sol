...
/** 
 *  SourceUnit: /Users/sg99xxml/projects/chfry-protocol-internal/contracts/avalanche/ChfryFactory.sol
*/
...
contract ChfryFactory is UpgradableProduct, ReentrancyGuard {
	...
	function setPools(Pool[] memory pools) external requireImpl {
		require(pools.length >= poolSize, 'pool length less than previous');
		...
		require(totalWeight == TOTAL_WEIGHT , 'weight mismatch');

		for(uint256 i = 0; i < pools.length; i++){
			Pool memory pool = pools[i];
			Pool memory newPool = Pool(pool.name, pool.pool, pool.weight, poolInfo[pool.name].minted);
			poolInfo[pool.name] = newPool;
			poolSize++;
		}
	}


	function poolMint(bytes32 poolName_) external nonReentrant returns (uint256) {
