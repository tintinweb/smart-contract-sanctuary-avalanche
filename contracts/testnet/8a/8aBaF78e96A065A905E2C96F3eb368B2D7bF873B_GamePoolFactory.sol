...
contract GamePoolFactory is Ownable {
    ...
    function createGamePool(string memory _gameName, address _tokenAddress)
        public
        onlyOwner
    {
        gameCounter += 1;
        GamePool newPool = new GamePool(
            msg.sender,
            _gameName,
            gameCounter,
            _tokenAddress
    ...
    function getData()
        external
        view
        returns (string[] memory, GamePool[] memory)
    {
        return (gameList, gamePoolAddresses);
    }
}
...
contract GamePool is Ownable {
    ...
    uint256 public rewardCounter = 10;
    ...
    function setGeekzPass(address _geekzPassAddress) public onlyOwner {
    ...
    function setGeekzPassData(
        ...
        geekzPass = GeekzPass(geekzPassAddress);
        geekzPass.setGeekzPassData(_gameId, _poolNum, _investors, _investments);
    }

    function fillRewardEth(uint weekNum) public payable onlyOwner {
        RewardLog storage rewardDetail = rewardLogs[rewardCounter];
        rewardDetail.weekNum = weekNum;
        rewardDetail.amount = msg.value;
        rewardCounter += 1;
    ...
    function getSharePercent() public {
        ...
        for (uint i = 0; i < totalNFT; i++) {
            ...
            if (nftLastClaimedWeek[tokenId] != rewardCounter - 1) {
                (, , uint _amount) = geekzPass.getTokenData(tokenId);
                investorAmount += _amount;
            }
        }

        uint share = (investorAmount * 100) /
            totalPool;
    ...
    function claimAll() public returns (uint) {
        ...
        // loop nft yg dia punya
        // count total amount
        // cek if claimedweek < currentweek
        // set claimedweek = currentweek
        // calculate share (totalamount/totalinvest) * reward
        for (uint i = 0; i < totalNFT; i++) {
            uint tokenId = geekzPass.tokenOfOwnerByIndex(investor, i);
            if (nftLastClaimedWeek[tokenId] != rewardCounter - 1) {
                (, , uint _amount) = geekzPass.getTokenData(tokenId);
                ...
                nftLastClaimedWeek[tokenId] = rewardCounter - 1;
            }
        }

        uint share = (investorAmount * rewardLogs[rewardCounter - 1].amount) /
            totalPool;
        ...
        if (tokenAddress != address(0)) {
            IERC20 token = IERC20(tokenAddress);
            token.approve(address(this), share);
...
contract InvestPool is Ownable {
    ...
    receive() external payable {
        require(tokenAddress == address(0), "ERC20 Token invest assets");
        invest(msg.value);
    }
    ...
    function invest(uint _amount) public {
        ...
        // check if amount > min
        require(_amount >= minInvest, "Investment Too Small");

        // check if has the token
        if (tokenAddress != address(0)) {
            IERC20 token = IERC20(tokenAddress);
            require(
    ...
    function mintGeekzPass() external {
        require(status == PoolStatus.DEPLOYED, "Not DEPLOYED");
        require(geekzPassAddress != address(0), "GeekzPass not set");
        require(balances[msg.sender] > 0, "Not Invested");
    ...
    function getInvested() public view returns (uint) {
    ...
    function updateGamePool() external onlyOwner {
        require(status == PoolStatus.DEPLOYED, "Pool Status not Deployed");

        gamePool = GamePool(gamePoolAddress);
        gamePool.poolClosed(poolNum, total);
        ...
        gamePool.setGeekzPassData(poolGameId, poolNum, investors, investments);
    }
