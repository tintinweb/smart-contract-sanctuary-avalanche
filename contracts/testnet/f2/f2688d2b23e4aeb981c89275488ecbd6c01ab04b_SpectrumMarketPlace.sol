    ...
    Ownable
{
        ...
        nonReentrant
    {
        ...
        uint256 fee;
        uint256 userReceipt = 25 * 10**17;

        if (serviceFee > 0 && _serviceWallet != address(0)) {
            fee = msg.value * serviceFee / ROYALTY_MAX;
            userReceipt += fee;
            Address.sendValue(payable(_serviceWallet), fee);
        }

        idToListedToken[tokenId].RoyaltyPercentage = msg.value - idToListedToken[tokenId].RoyaltyPercentage;
        Address.sendValue(payable(idToListedToken[tokenId].owner), msg.value - idToListedToken[tokenId].RoyaltyPercentage);
