...
contract RNG {
    ...
    function addEvent(
        ...
        require(_range > _numbersToExclude.length, "Event: Range must be greater then numbers to exclude");
    ...
    function randomWinner(uint256 _eventId) public returns (uint256[] memory) {
        ...
        if(choosedEvent.range > (choosedEvent.numberOfWinners - choosedEvent.excludedNumbers.length)) {
            numbersLength = choosedEvent.numberOfWinners;
        }
        else {
            numbersLength = choosedEvent.range;
        }
        ...
        do {
            ...
            if(choosedEvent.numbers[randomNumber] == false && choosedEvent.checkNumberExcluded[randomNumber] != true) {
