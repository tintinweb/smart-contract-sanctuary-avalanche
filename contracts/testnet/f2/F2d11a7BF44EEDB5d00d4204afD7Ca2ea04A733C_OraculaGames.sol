...
contract OraculaGames is Context, Ownable {
    ...
    function claimReward(uint256 _eventId) external verifyEvent(_eventId) returns (uint256[] memory) {
        ...
        for (uint256 x; x < arrayOfEventTokens.length; x++) {
            payableInToken[x] += ((eventFinances[_eventId][arrayOfEventTokens[x]].total*(denominator - eventInfo[_eventId].oraculaFee + eventInfo[_eventId].creatorFee)/denominator - eventFinances[_eventId][arrayOfEventTokens[x]].totalForOutcome[eventInfo[_eventId].realOutcome]) + share) / 10**18;
    ...
    function setEventOutcome(uint256 _eventId, uint256 _realOutcome) external onlyOracula verifyEvent(_eventId) {
        ...
        if (eventInfo[_eventId].oraculaFee > 0 || eventInfo[_eventId].creatorFee > 0) {
            ...
            for (uint256 x; x < arrayOfEventTokens.length; x++) {
                feeInTokens = (eventFinances[_eventId][arrayOfEventTokens[x]].total - eventFinances[_eventId][arrayOfEventTokens[x]].totalForOutcome[contractId]) * feeSum / denominator;
