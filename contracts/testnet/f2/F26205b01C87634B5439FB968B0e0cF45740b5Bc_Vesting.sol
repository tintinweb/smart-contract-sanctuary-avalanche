...
contract Vesting {
    ...
    /**
     * @dev Get current vesting time (current block time)
     */
    function getCurrentVestingTime() public view returns (uint) {
    ...
    function getVestingEndTime() public view returns(uint256) {
        ...
        // /// for test net 
        for (uint i = 0; i < 12; i++) {
            if (block.timestamp > getTGETime().add(claimPeriodTime.mul(2 days).mul(i)) && block.timestamp <= getTGETime().add(claimPeriodTime.mul(2 days).mul(i.add(1)))) {
                VestingEndTime = getTGETime().add(claimPeriodTime.mul(2 days).mul(i.add(1)));
                break;
            } else if(block.timestamp > getTGETime().add(claimPeriodTime.mul(2 days).mul(11))) {
                VestingEndTime = getTGETime().add(claimPeriodTime.mul(2 days).mul(11));
                break;
            }
        }
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        for (uint i = 0; i < 12; i++) {
            if (block.timestamp > getTGETime().add(claimPeriodTime.mul(2 days).mul(i)) && block.timestamp <= getTGETime().add(claimPeriodTime.mul(2 days).mul(i.add(1)))) {
                vestedPercent = TGE_UNLOCK.add(UNLOCK_UNIT.mul(i));
                break;
            } else if(block.timestamp > getTGETime().add(claimPeriodTime.mul(2 days).mul(12))) {
                vestedPercent = 1000;
                break;
            }  
        }
