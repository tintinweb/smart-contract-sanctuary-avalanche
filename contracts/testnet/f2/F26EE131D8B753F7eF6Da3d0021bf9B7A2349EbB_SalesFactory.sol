...
contract Sale is SaleToken, Ownable {
    ...
    function takeStakingAllocation(address to) external onlyInitialized {
        require(block.timestamp >= allocation.startTime, "Sale::takeStakingAllocation: Allocation has not started");
        require(block.timestamp <= allocation.endTime, "Sale::takeStakingAllocation: Allocation ended, so sorry");
    ...
    function buyToken(address depositAddress, uint256 amount) external onlyInitialized {
        ...
        require(amount > 0, "Sale::buyToken: Amount must be positive value");
        ...
        uint256 usedAllocationPoint = amountOfSellingTokens.mul(balanceOf(address(msg.sender))).div(sellingTokenAddress.balanceOf(address(this)));
    ...
    function buyTokenOnFCFS(address depositAddress, uint256 amount) external onlyInitialized {
        ...
        require(amount > 0, "Sale::buyTokenOnFCFS: Amount must be positive value");
    ...
    function _getAmountOfSellingTokens(address depositAddress, uint256 amount) private view returns (uint256) {
        ...
        return amount.mul(depositTokenPriceInUSD).div(tokenPriceInUSD).mul(10**sellingTokenAddress.decimals()).div(10**IERC20Decimals(depositAddress).decimals());
    }
