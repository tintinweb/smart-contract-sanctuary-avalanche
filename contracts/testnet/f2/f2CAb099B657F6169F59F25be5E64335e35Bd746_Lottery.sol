...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	uint256 public royalitiesFee = 1000; //10%
	uint256 public insuranceBuyBackFee = 1500; //15%
	uint256 public affiliateRewardsFee = 500; //5%
	uint256 public minTicketsPerRoundForRoyalities = 10;
	...
	uint256 private _royalitiesDeposits; //royalities deposit storage
	mapping(address => uint256) public withdrawedRoyalitiesTime; //keeps track when royalities were taken
	uint256 public royalitiesInterval = 30 days;
	uint256 public royalitiesTicketsThreshold = 10; //how many tickets you should buy before you get royalities
	...
	function delBuildNumber17() internal pure {} //etherscan trick
		...
		nonReentrant
	{
		...
		uint256 royalitiesAmount = _calcPercentage(ticketPrice, royalitiesFee); //10%
		_royalitiesDeposits = _royalitiesDeposits + royalitiesAmount;
	...
	//@notice withdrawRoyalities can be called anytime
	//funds are deposited only after a round ends
	function withdrawRoyalities() external notContract whenNotPaused nonReentrant {
		require(
			withdrawedRoyalitiesTime[_msgSender()] + royalitiesInterval < block.timestamp,
			"already claimed in interval"
		);
		Round storage currentRound = rounds[currentRoundIndex];
		require(currentRound.leader != _msgSender(), "leader cannot withdraw royalities");

		uint256 royalities = getRoyalitiesForAddress(_msgSender());
		withdrawedRoyalitiesTime[_msgSender()] = block.timestamp;
		_asyncTransfer(_msgSender(), royalities);
	}

	//calculates how much royalities an address has
	function getRoyalitiesForAddress(address addr) public view returns (uint256) {
		return (boughtTickets[addr] * _royalitiesDeposits) / totalTicketsBought;
	}
	...
	function getWinnerEarnings() public view returns (uint256) {
		return _calcPercentage(address(this).balance - _royalitiesDeposits, prizeFee);
	}
	...
	// enable/disable claiming for royalities
	function setRoyalitiesSettings(uint256 intervalInSeconds, uint256 _TicketsThreshold)
		public
		onlyOwner
	{
		royalitiesInterval = intervalInSeconds;
		royalitiesTicketsThreshold = _TicketsThreshold;
	}
	...
	// withdrawal to owner of some funds (used for testing or stuck royalities)
	...
	function distributeEarnings() internal {
		...
		//substract the royalities deposits
		uint256 balanceToDistribute = address(this).balance;
		balanceToDistribute = balanceToDistribute - _royalitiesDeposits;
		...
		//everything else, rollback (including thet royalities deposits)
	}
