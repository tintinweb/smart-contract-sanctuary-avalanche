...
contract MagicForest is IERC721Receiver, Pausable, Restricted {
    ...
    uint8 public constant MAX_VIRTUAL_ALPHA = 16;
    ...
    mapping(address => uint16[]) skeletonsByOwner;
    mapping(uint16 => uint16) skeletonsByOwnerIndices;
    ...
    function _getVirtualAlpha(
        ...
        return MAX_ALPHA - alphaIndex + alphaAugmentation + alphaFromLevel; // alpha index is 0-3
    }
    ...
    function _updateClaimingSession(uint32 claimingTimestamp) internal {
        if (claimingTimestamp - sessionBegining > sessionDuration) {
            ...
            if (claimingTimestamp - sessionBegining > 2 * sessionDuration) {
                lastSessionClaimNumber = (sessionBegining == 0) ? claimingFeeValues[2] : 0;
            } else {
