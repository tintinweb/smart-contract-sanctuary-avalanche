...
import "./CrossableToken.sol";
import "./lzApp/NonblockingLzApp.sol";

contract MondayBridge is NonblockingLzApp {

    CrossableToken public crossToken;
    ...
    function setToken(address _crossToken) onlyOwner external {
        crossToken = CrossableToken(_crossToken);
    }

    function sendCrossChain(uint16 _dstChainId, uint _amount) public payable {
        require(crossToken.balanceOf(msg.sender) >= _amount,"not enuf balance");
        (uint256 messageFee, ) = estimateSendFee(_dstChainId, _amount);

        require( msg.value >= messageFee,"not enough messageFee");

        crossToken.burn(msg.sender, _amount);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
        ...
        crossToken.mint(toAddress, amount);
...
contract CrossableToken is ERC20,Ownable {
    address public trustedBridge;

    constructor(address _trustedBridge) ERC20("Crossable Token", "CROSS") {
    ...
    function mint(address _account,uint _amount) external {
        require(msg.sender == trustedBridge, "only trusted bridge");
