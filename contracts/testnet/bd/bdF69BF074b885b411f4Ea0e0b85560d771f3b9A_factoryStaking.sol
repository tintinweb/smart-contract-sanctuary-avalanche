...
contract factoryStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public BASE_SUPPLY_RATE = 1;
    uint256 public SUPPLY_PER_DAY_PER_LVL = 10; 
    ...
    function myClaimableView(address user) public view returns(uint256 claimable){
        ...
        if(stakedSince[user]!= 0){
            for(uint i; i<=MAX_FACTORY_LEVEL;i++)
                total += (((block.timestamp - stakedSince[user])* factoryToken.balanceOf(user, i) * i * ((SUPPLY_PER_DAY_PER_LVL + BASE_SUPPLY_RATE)* 1 ether/86400)));
        return total;
        } else return 0;
    }
    function claimSupply() public{
        ...
        stakedSince[msg.sender] = block.timestamp;
        supplyToken.mint(msg.sender, claimable);
