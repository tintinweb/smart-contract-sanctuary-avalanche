...
string constant PERMISSION_MANAGER_NAME = "PERMISSION_MANAGER";
...
contract DAOParameterStorage is IDAOParameterStorage, Initializable, AbstractDependant {
    ...
    error DAOStorage__ParameterNotFound(string parameterName);
    ...
    function __DAOParameterStorage_init(
        string calldata resource_,
    ...
    function checkPermission(
        ...
        if (!restrictedOnly && votingType == VotingType.NON_RESTRICTED) {
            return true;
        }

        return
            _permissionManager.hasPermission(member_, DAO_PARAMETER_STORAGE_RESOURCE, permission_);
    }
    ...
    function setDAOParameters(
        ...
        for (uint256 i = 0; i < parameters_.length; i++) {
            _parameters.add(parameters_[i]);
        }
    }

    function changeDAOParameter(
        Parameter calldata parameter_
    ) external override onlyUpdatePermission {
        _checkParameterExistence(parameter_.name);

        _parameters.change(parameter_);
    }

    function changeDAOParameters(
        Parameter[] calldata parameters_
    ) external override onlyUpdatePermission {
        for (uint256 i = 0; i < parameters_.length; i++) {
            _checkParameterExistence(parameters_[i].name);

            _parameters.change(parameters_[i]);
        }
    }
    ...
    function removeDAOParameters(
        ...
        for (uint256 i = 0; i < parameterNames_.length; i++) {
            _checkParameterExistence(parameterNames_[i]);

            _parameters.remove(parameterNames_[i]);
        }
    }
    ...
    function _checkParameterExistence(string memory parameterName_) private view {
        if (!_parameters.contains(parameterName_)) {
            revert DAOStorage__ParameterNotFound(parameterName_);
        }
    }

    function _requirePermission(string memory permission_) private view {
        require(
            _permissionManager.hasPermission(
                msg.sender,
                DAO_PARAMETER_STORAGE_RESOURCE,
                permission_
            ),
            "DAOParameterStorage: access denied"
        );
    }
}
...
contract PermissionManager is IPermissionManager, RBAC {
    ...
    function hasStoragePermissionByGroup(
        address account_,
        string calldata mainResource_,
        string calldata componentResource_,
        string calldata permission_
    ) external view returns (bool) {
        if (!hasPermission(account_, mainResource_, permission_)) return false;
        return hasPermission(account_, componentResource_, permission_);
    }
}
...
interface IDAOParameterStorage is IDAORestriction {
    struct ConstructorParams {
        address daoRegistry;
    }
    ...
    function changeDAOParameter(Parameter calldata parameter_) external;

    function changeDAOParameters(Parameter[] calldata parameters_) external;
