...
contract BavaCompoundVault_PngUpgradeable is Initializable, UUPSUpgradeable, BRTERC20Upgradeable {
    ...
    function initVault(IERC20Upgradeable _lpToken, IStakingRewards _stakingPglContract, IERC20Upgradeable _rewardToken, IERC20Upgradeable[] memory _bonusRewardTokens, IRouter _router, uint256 _MIN_TOKENS_TO_REINVEST, uint256 _DEV_FEE_BIPS, uint256 _REINVEST_REWARD_BIPS) external onlyOwner {        
        require(address(_lpToken) != address(0), "0Addr");
        require(address(_stakingPglContract) != address(0), "0Addr");

        vaultInfo.lpToken = _lpToken;
        vaultInfo.pngStakingContract = _stakingPglContract;        
    ...
    function setAllowances(uint256 _amount) external onlyOwner {       
        if (address(vaultInfo.pngStakingContract) != address(0)) {
            vaultInfo.lpToken.approve(address(vaultInfo.pngStakingContract), _amount);
        }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(vaultInfo.restaking_enabled == true) {
            ...
            vaultInfo.pngStakingContract.stake(restakeAmount);
        }
  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    function withdraw(uint256 _amount, bool harvestReinvestReward) public {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function reinvest() external {
        ...
        if(vaultInfo.restaking_enabled == true) {
            vaultInfo.pngStakingContract.stake(liquidity);
        }
    }
    ...
    function _withdrawDepositTokens(uint amount) private {
        require(amount > 0, "#<0");
        uint256 depositAmount = vaultInfo.pngStakingContract.balanceOf(address(this));
        
        if (depositAmount > 0) {
            if(depositAmount >= amount) {
                vaultInfo.pngStakingContract.withdraw(amount);
            } else {
                vaultInfo.pngStakingContract.withdraw(depositAmount);
            }
        }
    }
    ...
    function _getReinvestReward() private {
        uint256 pendingRewardAmount = vaultInfo.pngStakingContract.earned(address(this));
        if(pendingRewardAmount > 0) {
            vaultInfo.pngStakingContract.getReward();
        }
    }
    ...
    function emergencyWithdrawDepositTokens(bool disableDeposits) external onlyOwner {
        vaultInfo.pngStakingContract.exit();
        setFeeBips(0,0);
    ...
    function setVaultInfo(IStakingRewards _stakingPglContract, IERC20Upgradeable _rewardToken, IERC20Upgradeable[] memory _bonusRewardTokens, bool _withUpdate) external onlyOwner {
        require(address(_stakingPglContract) != address(0) , "0Addr");
        if (_withUpdate) {
            updatePool();
        }
    ...
    ****************************************/
    ...
    function _convertRewardIntoWAVAX() private returns (uint) {
        ...
        if (rewardAmount > 0) {
            ...
            uint256 devFee = avaxAmount*(DEV_FEE_BIPS)/(DEV_FEE_BIPS + REINVEST_REWARD_BIPS);
            if (devFee > 0) {
