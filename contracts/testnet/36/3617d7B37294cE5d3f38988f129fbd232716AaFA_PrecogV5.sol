...
interface IPrecogCore {
    /**
     * @dev Structure about role configurations for specific functions in Precog and PrecogCore contract
     * NOTE Explainations of params in struct:
     * - `admin` is admin address that can call configuration funcitons in Precog and PrecogCore contract
     * - `middleware` is middleware address that can call trading functions in Precog contract
     * - `exchanges` is exchange address that Precog can interact with it to get profit 
     */
...
// File: contracts\precog\abstracts\APrecogV5.sol

abstract contract APrecogV5 is IPrecogV5 {
        ...
        returns (Cycle memory _currentTradingCycleByTimestamp)
    {
        ...
        _currentTradingCycleByTimestamp = _lastTradingCycle;
    }
     ...
     * @return _lastInvestmentOf is the last Investment of user
     */
    function _getLastInvestmentOf(address _token, address _account)
        internal
        view
        returns (Investment memory _lastInvestmentOf)
    {
        uint _lastInvestmentIndex = investmentsOf[_token][_account].length - 1;
        _lastInvestmentOf = investmentsOf[_token][_account][_lastInvestmentIndex];
    }
    ...
    function _updateCurrentTradingCycle(
        ...
        while (uint48(block.timestamp) >= _lastTradingCycle.endTime) {
            ...
            if (!isUpdateUnitTradingCycle[_token][_lastTradingCycle.id]) {
    ...
    /**
     * @dev Calculates and updates investment amount, unit, trading cycle id at current investment or push new investment
     * @param _token is token address
     * @param _account is account address
    ...
    function _increaseInvestment(
        ...
        {
            _futureTradingCycle = _getTradingCycleByTimestamp(_token, _timestamp);
        }

        if (!accountTradingInfo[_token][_account].isNotFirstIncreaseInvestment) {
        ...
        if (!isUpdateUnitTradingCycle[_token][_futureTradingCycle.id]) {
    ...
    {
        (uint _availableWithdrawalAmount, , ,) = _availableDepositedAmount(_token, _account);
        _isBeforeInvestmentCycle = _availableWithdrawalAmount > 0;
    }
    ...
    function _updateDepositInfo(
        ...
        if (block.timestamp >= _accountTradingInfo.depositedTimestampOf) {
            ...
            _accountTradingInfo.depositedTimestampOf = _getLastInvestmentOf(_token, _account).timestamp;
        } else {
            _accountTradingInfo.availableAmount += _amount;
        }
    ...
    function _availableDepositedAmount(address _token, address _account)
        internal
        view
        returns (uint _amount, uint _fundingStartTimeOf, uint _lastFundingStartTime, uint _nextFundingStartTime)
    {
        AccountTradingInfo memory _accountTradingInfo = accountTradingInfo[_token][_account];
        _amount = _accountTradingInfo.depositedTimestampOf > block.timestamp
            ? _accountTradingInfo.availableAmount
            : 0;
        _fundingStartTimeOf = _accountTradingInfo.depositedTimestampOf;
        (_lastFundingStartTime, _nextFundingStartTime) = core.getCurrentFundingCycle(_token);
    }
    ...
    function _withdrawBeforeFundingTime(
        ...
        (uint _availableWithdrawalAmount, , ,) = _availableDepositedAmount(_token, _account);
        require(_availableWithdrawalAmount >= _amount && IERC20(_token).balanceOf(address(this)) >= _amount, "8");
        uint _remainingAmount = _decreaseInvestmentWhenWithdraw(_token, _account, _amount);
        require(
            _remainingAmount == 0 ||
                (_remainingAmount >= core.minFunding(_token) && _amount >= core.minDefunding(_token)), "7"
        );
...
pragma solidity ^0.8.2;
contract PrecogV5 is APrecogV5 {
    using SafeERC20 for IERC20;
    ...
    function getLastInvestmentOf(address token, address account)
        ...
        returns (Investment memory lastInvestmentOf)
    {
        lastInvestmentOf = _getLastInvestmentOf(token, account);
    }
    ...
    function increaseInvestment(
        ...
        _increaseInvestment(token, account, amount, uint48(block.timestamp));
    }
    ...
    function decreaseInvestment(
        ...
        require(msg.sender == tokenConvert[token]);
        _decreaseInvestment(token, account, amount, uint48(block.timestamp));
    }
    ...
    {
        (amount, fundingStartTimeOf, lastFundingStartTime, nextFundingStartTime) = _availableDepositedAmount(token, account);
    }
    ...
    function deposit(address token, uint amount) external override {
        ...
        _increaseInvestment(
            ...
            msg.sender, 
            actualDepositAmount, 
            PrecogV5Library._nextFundingTime(
                core.getCyclesChangedInfo(token).fundingApplyTime,
                core.getCyclesChangedInfo(token).fundingDuration
        ));
    ...
    function requestWithdrawal(address token, uint amount) external override {
        ...
        IPrecogCore.CyclesChangedInfo memory _cycleChangedInfo = core.getCyclesChangedInfo(token);
        if (isFirstRequestWithdrawal) {
            if (withdrawalRegister.isInDeadline(token, msg.sender)) {
                nextDefundingTime = uint48(withdrawalRegister.getRegister(token, msg.sender).deadline);
            } else {
        ...
        uint remainingAmount = _decreaseInvestment(token, msg.sender, amount, nextDefundingTime);
    ...
    {
        isBeforeInvestmentCycle = _isBeforeFundingTime(token, account);
    }
