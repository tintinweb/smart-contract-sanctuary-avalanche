...
contract Erc20PoolEscrow is IERC20PoolEscrow, Ownable {
    ...
    /// @notice Burns amount from user wallet
    /// @param amount Amount of tokens to be burnt
    function burnAmountFromWallet(address spender, uint256 amount)
    override 
    public {
        ...
        _escrowBalance[address(0)] += amount;
    ...
    /// @notice Burns amount from user wallet
    /// @param amount Amount of tokens to be burnt
    function burnAmountFromBalance(address spender, uint256 amount)
    override 
    public {
    ...
    public {
        require(address(_token) != address(0), "Erc20PoolEscrow : token isn't initialized");
        require(_escrowBalance[_msgSender()] > 0, "Erc20PoolEscrow : address doesn't have any token in balance");
