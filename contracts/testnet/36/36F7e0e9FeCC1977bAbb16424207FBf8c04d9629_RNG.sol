...
contract RNG {
    ...
    event randomWinnerNumber(uint256 eventId, uint256[] newWinners);
    event allWinners(uint256 eventId, uint256[] winners);
        ...
        returns (uint256[] memory)
    {
        ...
        require(
            choosedEvent.numbersArray.length < choosedEvent.numberOfWinners,
            "Event: Max Winners reached"
        );
        ...
        emit randomWinnerNumber(_eventId, newWinners);
        return (newWinners);
    }

    // Get winners
    function getAllWinners(uint256 _eventId) public returns (uint256[] memory) {
        require(
        ...
        require(
            events[_eventId].status == true,
            "Event: Not all winners decided"
        );

        emit allWinners(_eventId, events[_eventId].numbersArray);
    ...
    // Get winners
    function checkAllWinners(uint256 _eventId)
        public
        view
        returns (uint256[] memory)
    {
