...
contract GamePool  is ERC1155Holder, ERC721Holder,Multicall,Context{
   ...
   */
    ...
    */ 
    function withdrawCyt(uint256 amount,bytes memory signature,uint256  currentTimeStamp)external nonReentrant updateProof(currentTimeStamp) {
     ...
     *@param: signature：proof of cyt. currentTimeStamp: timeStamp
    */ 
    function withdrawCoin(uint256 amount,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
    ...
    */ 
    function withdrawErc20(address tokenAddress,uint256 amount,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,amount,signature,currentTimeStamp,Proof.addrToStr(tokenAddress))==true,"You don't get the proof right");
    ...
    */
    function WithdrawGameProbe(address player,uint256  id,uint256  amount,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,id,amount,signature,currentTimeStamp,"ERC1155_game")==true,"You don't get the proof right");
    ...
    * @param: ids: the collection to retrieve the id . amounts: The number of ids to retrieve
    */
    function WithdrawErc1155(address tokenAddress,address player,uint256  id,uint256  amount,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,id,amount,signature,currentTimeStamp,Proof.addrToStr(tokenAddress))==true,"You don't get the proof right");
    ...
    */
    function withdrawRole(address player,uint256 tokenId,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
    ...
    * @param: tokenId: The player wants to load the Id of the equipment. amout: the amount of erc721
    */
    function withdrawErc721(address tokenAddress,address player,uint256 tokenId,bytes memory signature,uint256 currentTimeStamp) external nonReentrant updateProof(currentTimeStamp){
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,tokenId,signature,currentTimeStamp,Proof.addrToStr(tokenAddress))==true,"You don't get the proof right");
 ...
 function bytesToStr(bytes memory data) internal pure returns (string memory) {
  ...
  //Convert parameter types from address to string
    ...
    function getHash(address from,string memory newId,string memory timestamp,string memory tokenName) internal pure returns (bytes32 result){  
     ...
     function getHash(address from,string memory newId, string memory amount, string memory timestamp,string memory tokenName) internal pure returns (bytes32 result){  
        return ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(strConcat(addrToStr(from),newId,amount,timestamp,tokenName))));
     }  
   ...
   *  @param: newId: the id of bd. signature: The signature of the server. timestamp: The timestamp at the time the proof was generated
   */
    function checkPermissions(address signer,uint256 id,uint256 amount, bytes memory signature,uint256 timestamp,string memory tokenName) internal view returns(bool) {
        return SignatureChecker.isValidSignatureNow(signer,getHash(msg.sender,Strings.toString(id),Strings.toString(amount),Strings.toString(timestamp),tokenName),signature);
    }
}
