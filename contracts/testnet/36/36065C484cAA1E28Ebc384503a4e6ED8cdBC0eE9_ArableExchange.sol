    ...
    EIP712Upgradeable
{
    ...
    bytes32 public constant SET_PRICE_METHOD = keccak256("SetPrice(address token,uint256 price,uint256 timestamp)");
    ...
    function initialize(address addressRegistry_) external initializer {
        ...
        __EIP712_init_unchained("Arable", "1.0.0");
    ...
    function setPriceMessage(
        ...
        return keccak256(abi.encode(SET_PRICE_METHOD, token, price, timestamp));
    }

    function signer(bytes32 message, bytes memory signature) public view returns (address _signer) {
    ...
    function _verifySignature(
        ...
        if (token != arUSD) {
            address _signer = signer(setPriceMessage(token, price, timestamp), signature);
            require(oracleContract.isAllowedProvider(_signer), "Invalid signer");
        }
    }
