...
contract NFTStaking is Ownable,ERC721Holder {
    ...
    uint256 constant token = 0.1 ether;
    ...
    bool initialised;
    Collectible private nft;
    RewardToken private rewardsToken;
    constructor(address nftContractAddress,address rewardsTokenContractAddress ) 
    {
        nft = Collectible(nftContractAddress);
        rewardsToken = RewardToken(rewardsTokenContractAddress);
    }
    ...
    ///@notice Allows reward tokens to be claimd
    event ClaimableStatusUpdated(bool status);

    ///@notice Emergency unstake tokens without rewards
    event EmergencyUnstake(address indexed user, uint256 tokenId);

    function initStaking(/* Collectible _nft,RewardToken _rewardsToken */) public onlyOwner {
    ...
    function setTokenClaimble(bool _enabled) public onlyOwner{
        //needs access control
    ...
    function unstake ( uint256 _tokenId) public {
        return _unstake (msg.sender,_tokenId);
    }
    function _unstake ( address _user, uint256 _tokenId )internal{
        require(
            tokenOwner[_tokenId] == _user,
            "NFT Staking System : user must be the owner of the staked out"
        );
    ...
    function claimableReward(address _user) public view returns(uint256 _amount){
...
contract RewardToken is ERC20,ERC20Burnable,AccessControl{
    ...
    constructor() ERC20 ("TRI_TOKEN","TRI"){
    ...
    function setMinterRole(address to) public onlyRole(DEFAULT_ADMIN_ROLE){
        _grantRole(MINTER_ROLE, address(to));
    }
     function setAdminRole(address to) public onlyRole(DEFAULT_ADMIN_ROLE){
        _grantRole(DEFAULT_ADMIN_ROLE, address(to));
...
contract Collectible is  ERC721Enumerable, Ownable,AccessControl,ERC721Holder {
  ...
  address public nftStakingContractaddress= address(0x00000000000000000000000000000000);
  ...
  function setStakingContractAddress(address _contractAddress) public onlyOwner{
    nftStakingContractaddress=_contractAddress;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      _safeMint(address(this), itemId);
      // setApprovalForAll(address(this), true);
      _transfer(address(this), msg.sender, itemId);
      setApprovalForAll(address(nftStakingContractaddress), true);
      setApprovalForAll(address(this), true);
      if (itemId > 1) {
