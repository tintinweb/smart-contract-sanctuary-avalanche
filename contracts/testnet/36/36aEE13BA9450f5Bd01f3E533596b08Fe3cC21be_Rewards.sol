    ...
    ReentrancyGuardUpgradeable
{
        ...
        for (uint256 i = 0; i < YieldBoxIds.length; i++) {
            ...
            if (
                ...
                (((time - YieldBox.yieldBox(YieldBoxIds[i]).createdAt).div(
                    3 minutes)) > RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed)
            ) {
                rewarded[i] =
                    ((time - YieldBox.yieldBox(YieldBoxIds[i]).createdAt).div(3 minutes)) -
                ...
                rewarded[i] = (MINIMAL_REWARDS_DAYS.div(3 minutes)) >
                    RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed
                    ? (MINIMAL_REWARDS_DAYS.div(3 minutes)) -
                        RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed
                    : 0;
                amount[i] = mulDiv(
                    rewarded[i],
    ...
    function setValue(uint256[4] memory _value) external onlyOwner {
        ...
        LIMITS_DAYS = _value[2] * 3 minutes;
        MINIMAL_REWARDS_DAYS = _value[3] * 3 minutes;
    ...
    function getStakedDays(
        ...
        for (uint256 i = 0; i < startStaked.length; i++) {
            if ((value < startStaked[i]) && (value < endStaked[i])) {
                amountDays += (endStaked[i] - startStaked[i]).div(3 minutes);
            } else if ((value >= startStaked[i]) && (value < endStaked[i])) {
                amountDays += (endStaked[i] - value).div(3 minutes);
            }
        }
    }
}
