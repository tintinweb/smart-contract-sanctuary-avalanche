...
contract JonesGlpLeverageStrategy is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, Governable {
    ...
    function onGlpRedeem(uint256 _share, uint256 _amount) external nonReentrant onlyRole(OPERATOR_ROLE) returns (uint256) {
        // if (_amount > getUnderlyingGlp()) {
        //     revert NotEnoughUnderlyingGlp();
        // }

        uint256 glpRedeemRetentionAmount = glpRedeemRetention(_amount);
        uint256 assetsToRedeem = _amount - glpRedeemRetentionAmount;

        assetsToRedeem = yrt.getSharesForDepositTokens(assetsToRedeem);

        glpVault.redeemExactTokens(_share, assetsToRedeem, msg.sender, address(this));
    ...
    function onStableRedeem(uint256 _amount, uint256 _amountAfterRetention) external onlyRole(OPERATOR_ROLE) returns (uint256) {
        ...
        stable.transfer(address(glpStableVault), stableAmount);
    ...
    function claimGlpRewards() internal nonReentrant returns(uint256, uint256) {
        ...
        (uint256 stableRewards, uint256 glpRewards) = distributor.splitRewards(rewards, currentLeverage, utilization());
    ...
    function leverage() public view returns (uint256) {
        ...
        uint256 glpYRT = glpVault.totalUnderlyingAssets();
        uint256 yyGlpBalance = yrt.getDepositTokensForShares(glpYRT);
        uint256 totalGlpBalance = currentBalance + yyGlpBalance;

        // uint256 glpTvl = getUnderlyingGlp(); // 18 Decimals
        
        uint256 glpTvl;     // 18 Decimals
        ...
        if (stableDebt > 0) {
            ...
            glpTvl = (totalGlpBalance > glpAmount ? totalGlpBalance - glpAmount : 0);
        } else {
            glpTvl = totalGlpBalance;
        }
        ...
        return ((totalGlpBalance * BASIS_POINTS) / glpTvl); // 12 Decimals;
    }

    /**
     * @notice get user GLP exclude borrowed glp // BAVA chef comment
     */
    function getUnderlyingGlp() public view returns (uint256) {
        uint256 currentBalance = glp.balanceOf(address(this));
        uint256 yrtAsset = glpVault.totalUnderlyingAssets();
        uint256 vaultGlpBalance = yrt.getDepositTokensForShares(yrtAsset);
        uint256 totalGlpBalance = currentBalance + vaultGlpBalance;
        ...
        if (stableDebt > 0) {
            (uint256 glpAmount,) = _getRequiredGlpAmount(stableDebt + 2);
            return totalGlpBalance > glpAmount ? totalGlpBalance - glpAmount : 0;
        } else {
            return totalGlpBalance;
        }
    }
     ...
     */
    function unwind() external onlyRole(KEEPER_ROLE) {
    ...
    function _compound() private {
        ...
        if (glpRewards > 0) {
            ...
            yrt.depositFor(address(glpVault),glpRewards);
