...
contract MarketRouter is MarketCore{
    ...
    function commitOrder(  
        ...
        MarketCore.commitOrder(Order(maker,salt,listingTime,expirationTime),assetData,callData,orderData,signature);  
    }
    ...
    function orderMatch(bytes memory hashOrders,bytes memory singers, bytes memory callDatas,bytes memory callTarget, bytes memory signatures,bytes memory orderData) external  {
...
contract MarketCore is MarketPojo{
    ...
    function hashOrder(Order memory order) internal pure returns(bytes memory){
        return abi.encode(order.maker,order.salt,order.listingTime,order.expirationTime);
    }
    ...
    //验证用户上架前是否存在该资产
    function assetValidation() external view returns(bool) {
        
    }

    //order中的maker 应该跟calldata的to相同
    function commitOrder(  
        Order memory order,
        bytes memory assetData,  //要检查订单参数
        bytes memory callData, //tokenAddress, from ,to
        bytes memory orderData,
    ...
    function orderMatch_(bytes memory hashOrders,bytes memory singers, bytes memory callDatas,bytes memory callTarget, bytes memory signatures,bytes memory orderData) internal {
            ...
            (bytes32 firstOrderHash, bytes32  secondOrderHash) = abi.decode(hashOrders, (bytes32, bytes32));  //hashorders要sha3
            (bytes memory firstCallData, bytes memory secondCallData) = abi.decode(callDatas, (bytes, bytes));


            bytes32 firstSignatureHash = Proof.hashSignature(firstOrderHash,Proof.getBehindBytes(secondCallData),orderData,secondCallTarget);
            bytes32 seccondSignatureHash = Proof.hashSignature(secondOrderHash,Proof.getBehindBytes(firstCallData),orderData,firstcallTarget);

            require(orderState[firstsinger][firstOrderHash]==true,"order status incorrect ");
            ...
            emit orderMatchEvent(firstsinger,secondsinger,firstOrderHash,secondOrderHash,block.timestamp);
    }
