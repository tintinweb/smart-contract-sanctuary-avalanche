...
contract MintV2 is ERC721Enumerable, Ownable, RoyaltiesImpl, Pausable {
    ...
    whenNotPaused
    {
        uint256 supply = nftSwappable + totalSupply() ;
    ...
    public
    {
        ...
        require(nft.balanceOf(msg.sender) > 0, "You do not have any nft in your wallet");

        for(uint8 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < 2500, "Unallocated Seam");
            if (_findElement(tokenIds[i],msg.sender)) {
                console.log("nft mint %s", tokenIds[i]);
                _mint(msg.sender,tokenIds[i]);
                setRoyalties(tokenIds[i],royaltiesAddressPayable,500);
                tokenIdSwap.push(tokenIds[i]);
                nftSwappable -= 1;
                trsr.mint(msg.sender,5000 * 10**18);
            }
        }
    }
    ...
    onlyOwner
    {
        ...
        for(uint8 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < 2500, "Unallocated Seam");
            if (_findElement(tokenIds[i],owner)) {
                _mint(owner,tokenIds[i]);
                setRoyalties(tokenIds[i],royaltiesAddressPayable,500);
                tokenIdSwap.push(tokenIds[i]);
                nftSwappable -= 1;
                trsr.mint(owner,5000 * 10**18);
            }
        }
    }

    function _findElement(uint value, address owner)
    internal
    returns (bool)
    {
        uint[] memory tokenIdsOld = nft.walletOfOwner(owner);
        for(uint i=0;i < tokenIdsOld.length; i++) {
            if (tokenIdsOld[i] == value) {
                return true;
            }
        }
        return false;
    ...
    function mintCost(uint256 tokenId)
    internal
    view
    returns (uint256)
    {
        if (tokenId <= paidTokens) return 0;
        if (tokenId <= maxSupply - paidTokens) return priceToBuyTRSR;
    }

    function getCategorySeam(uint tokenId)
    public
    view
    returns(uint)
    {
        if (tokenId <= paidTokens) return 1;
        if (tokenId <= maxSupply - paidTokens) return 2;
    }
    ...
    function setPaidTokens (uint _newPaidTokens)
    public
    onlyOwner
    {
        paidTokens = _newPaidTokens;
    }
    ...
    function setPriceToBuyTRSR (uint newPrice)
    public
    onlyOwner
    {
