...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function trystack(uint256 _id, uint256 _amount, uint8 _direction) external {
        ...
        require(pcr.oPrice > 0 && pcr.cPrice == 0, "Invalid pair");
         ...
         require(
             block.timestamp > pcr.oTime + pcr.sInterval &&
             block.timestamp < pcr.cTime - pcr.eInterval ,
             "Timeout"
         );
        ...
        unchecked {
            ...
            if(_direction == 1) {
                IPairManager(pairManagerAddress).setRTotal(_id, counter, pcr.rTotal + _amount);
                player.riseAmount += _amount;
            }else {
                IPairManager(pairManagerAddress).setFTotal(_id, counter, pcr.fTotal + _amount);
    ...
    function updateRewardCoefficient(uint256 _rewardCoefficient) external onlyRole(REWARD_MANAGER)  {
        ...
        uint256 _historyCf =  rewardCoefficient;
    ...
    */
    function _calcDaoAmountPerUnit(uint256 _level) private view returns(uint256, uint256) {
        ...
        uint256 number = (100 + rewardCoefficient * (_level - 1)) * 1e18;
        return (
            125 * number  / (100 * 100),
            75 * number  / (100 * 100)
        );
...
interface IPairManager {
    ...
    function setRTotal(uint256 _id, uint64 _counter, uint256 _amount) external;

    function setFTotal(uint256 _id, uint64 _counter, uint256 _amount) external;
