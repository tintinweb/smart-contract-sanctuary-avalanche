...
contract NodeManagerV1 {
  ...
  constructor(address[] memory addresses) {
    ...
    payInterval = 600; // todo in mainnet 30 days
  }
  ...
  function setPayInterval(uint32 value) public onlyOwner {
    //require(payInterval != value,"Same value!");
    payInterval = value;
  }
  ...
  function setOperationsPoolAddress(address account) public onlyOwner {
    //equire(operationsPoolAddress != account, "Same account!");
  ...
  function unpaidNodes() public payable returns (Node[] memory) {
    for (uint32 i = 0; i < nodesTotal.length; i++) {
      Node storage node = nodesTotal[i];
