...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    event updateFeeManagementContractAddressEvent(
        address ownerAccount,
        address indexed previousFeeManagementContractAddress,
    ...
    // only Fee Management
    modifier onlyFeeManagement() {
        require(
            msg.sender == _FeeManagementContractAddress,
            "Sender is not the Fee Management contract address!"
        );
        _;
    }
        ...
        returns (bool)
    {
        ...
        address owner_ = _msgSender();
        _approve(owner_, spender_, allowance(owner_, spender_).add(addedValue_));
        return true;
    }
    ...
    // update Fee Management Contract Address
    function updateFeeManagementContractAddress(address FeeManagementContractAddress_)
        public
        onlyOwner
        notNullAddress(FeeManagementContractAddress_)
    {
        ...
        // update Fee Managment Contract Address
        ...
        returns (bool)
    {
        if (
            ...
            require(
                amount_ > 
                _FeeManagement.getMinCreationAmount(),
                "Amount should be greater than min creation amount!"
            );

            // creation fee amount
            uint256 creationFeeAmount = (amount_.mul(_FeeManagement.getCreationFee())) /
            ...
            _mint(_FeeManagement.getTreasurer(), creationFeeAmount);
        ...
        returns (bool)
    {
        ...
        if (
            ...
            uint256 redemptionFeeAmount = (amount_.mul(_FeeManagement.getRedemptionFee())) /
                (10**_FeeManagement.getFeeDecimals());

            // transfer
            _transfer(senderAddress_, _FeeManagement.getTreasurer(), redemptionFeeAmount);
    ...
    function withdrawContractTokens() external  {
        ...
        if(balance_ > 0 ){
            _transfer(address(this), _FeeManagement.getTreasurer(), balance_);
        }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 transferFeeAmout = (amount_.mul(_FeeManagement.getTransferFee())).div(10**_FeeManagement.getFeeDecimals());
            ...
            unchecked {
                ...
                _balances[_FeeManagement.getTreasurer()] = _balances[_FeeManagement.getTreasurer()].add(transferFeeAmout);

                // add the rest amount to receiver address
                _balances[to_] = _balances[to_].add(amount_.sub(transferFeeAmout));
            }

            // update holders
            // add treasuer as holder if not already in the list
            if(!_holdersStatus[_FeeManagement.getTreasurer()]){
                _appendToHolders(_FeeManagement.getTreasurer());
            }
            ...
            emit Transfer(from_, _FeeManagement.getTreasurer(), transferFeeAmout);
...
interface FeeManagementInterface {
    // update Owner
    function updateOwner(address newOwner_) external;
    ...
    // update Treasurer
    function updateTreasurer(address newTreasurer_) external;
    ...
    function getWhitelistManager() external view returns (address);
    ...
    function isTransferWhitelisted(address account_) external view returns (bool);
    ...
    function getCreationRedemptionWhitelist() external view returns (address[] memory);
...
contract FeeManagement {
    ...
    function setFeeDecimals(uint256 feeDecimals_) public onlyFeeManager {
        ...
        // update min transfer amount
        // _globalMinTransferAmount = (10**_feeDecimals) / _transferFee;
        ...
        // update min creation amount
        // _globalMinCreationAmount = (10**_feeDecimals) / _creationFee;
        ...
        // update min redemption amount
        // _globalMinRedemptionAmount = (10**_feeDecimals) / _redemptionFee;
