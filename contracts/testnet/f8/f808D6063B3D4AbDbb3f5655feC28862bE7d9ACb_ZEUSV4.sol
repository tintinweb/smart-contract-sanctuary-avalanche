...
abstract contract IMasterOfCoin {
     address public collector;
    function getDueDate(string memory nodeId) virtual external view returns (uint256);
    ...
    function payFee(string memory nodeId) virtual external;

    function isDelinquent(string memory nodeId) virtual external view returns (bool);
...
contract ZEUSV4 is Ownable {
    ...
    constructor() {
        ...
        collector = payable(0x67dD4EA99CE6453f28DA3b08d0257063189121e6);
    }
    ...
    function _isDelinquent(string memory nodeId) public view returns(bool){
        return masterOfCoin.isDelinquent(nodeId);
    }

   function updateCollector(address payable collector_) external onlyOwner {
   ...
   function withdraw() public {
    ...
    function restoreNode(
        ...
        require(msg.value >= fee, "Fee is insufficient");
        // uint256 avaxFee = fee / 10**18;
        bool isDelinquent = _isDelinquent(nodeId);
        if (isDelinquent) {
