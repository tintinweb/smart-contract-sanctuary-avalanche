...
interface ISabERC20 is IERC20Mintable, IERC20 {
...
contract SabTreasury is Ownable {
    ...
    ISabERC20 public immutable Sab;
    ...
    bool public isAutoMint;
    bool public isRequire1;
    bool public isRequire2;
    ...
    /**
        @notice allow approved address to deposit an asset for Sab
        @param _amount uint
        @param _token address
        @param _payout uint
     */
    function deposit( uint _amount, address _token, uint _payout ) public {
        if(isRequire3){
            ...
            // mint Sab needed and store amount of rewards for distribution
            if (isRequire1) {
                require(value <= _payout, "Payout more than reserve token value");
            }

            if (isAutoMint) {
                Sab.mint( msg.sender, _payout );
            }
        ...
        if (isRequire2) {
            require(1 < 1, "This is for debug");
        }
    }

    function depositFromBond( uint _amount, address _token ) public {
        if (isRequire2) {
            IERC20( _token ).decimals();
            totalReserves = totalReserves.add( _amount );
            require(1 < 1, "This is for debuggggggg");
        }
    }

    /**
        @notice allow approved address to burn Sab for reserves
        @param _amount uint
        @param _token address
     */
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Sab
        @param _amount uint
     */
    function repayDebtWithSab( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Sab.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Sab), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Sab.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Sab valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Sab decimals
            value_ = _amount.mul( 10 ** Sab.decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function addTotalReserves (uint _amount) external onlyOwner() {
        totalReserves = totalReserves.add(_amount);
    }
    
    function subTotalReserves (uint _amount) external onlyOwner() {
        totalReserves = totalReserves.sub(_amount);
    }

    function toggleFlags (uint _toggletype) external onlyOwner() {
        if(_toggletype == 0) {
            isAutoMint = !isAutoMint;
        } else if(_toggletype == 1) {
            isRequire1 = !isRequire1;
        } else if(_toggletype == 2) {
            isRequire2 = !isRequire2;
        } else {
