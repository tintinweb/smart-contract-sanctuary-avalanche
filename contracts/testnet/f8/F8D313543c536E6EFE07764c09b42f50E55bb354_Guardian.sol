...
contract Guardian is Storage{
    ...
    event GuardinshipTransferred(
        ...
        address indexed newGuardianAddress
    );
    ...
    event GuardianRemovalQueued(address indexed guardianAddress);

    /// @notice emit when guardian removed
    event GuardianRemoved(
        address indexed oldGuardianAddress,
        address indexed newGuardianAddress
    );

    // /**
    //  * @notice Sets guardian addresses and threshold
    //  * @param _guardianAddresses - array of guardian address
    //  * @param _threshold - number of guardians required for guardian duties
    //  */
    // constructor(address[] memory _guardianAddresses, uint256 _threshold) {
    //     require(_threshold <= _guardianAddresses.length, "threshold too high");

    //     for (uint256 i = 0; i < _guardianAddresses.length; i++) {
    //         require(!isGuardian[_guardianAddresses[i]], "duplicate guardian");
    //         isGuardian[_guardianAddresses[i]] = true;
    //     }
    ...
    /// @param _guardianAddresses addresses of guardians to be made
    /// @param _threshold minumum approval
    function initGuardian(
        address[] memory _guardianAddresses,
        uint256 _threshold
    ) external {
        console.log(
            "Hello World in Guardian------------------------------------------"
        );
        require(!guardianInitialized, "Contract already initialized");
        guardianInitialized = true;
        require(_threshold <= _guardianAddresses.length, "threshold too high");
        for (uint256 i = 0; i < _guardianAddresses.length; i++) {
            require(!isGuardian[_guardianAddresses[i]], "duplicate guardian");
            isGuardian[_guardianAddresses[i]] = true;
            guardians.push(_guardianAddresses[i]);
        }
        threshold = _threshold;
        owner = msg.sender;
    }

    function isWListed(address _address) public view returns (bool) {
        console.log("Yes this guy is hereeeeeeeeeee", isGuardian[_address]);
     ...
     * @param newGuardianAddress - address of the new guardian
     */
    function transferGuardianship(address newGuardianAddress)
        external
        onlyGuardian
        notInRecovery
    {
        ...
        require(
            guardianAddressToRemovalTimestamp[msg.sender] == 0,
            "guardian queueud for removal, cannot transfer guardianship"
        );
        isGuardian[msg.sender] = false;
        isGuardian[newGuardianAddress] = true;
        emit GuardinshipTransferred(msg.sender, newGuardianAddress);
    }

    /**
     * @notice Allows the owner to queue a guardian for removal
     * @param guardianAddress - address of the guardian to queue
     */
    function initiateGuardianRemoval(address guardianAddress)
        external
        onlyOwner
    {
        // verify that the address actually corresponds to a guardian
        require(isGuardian[guardianAddress], "not a guardian");

        // removal delay fixed at 3 days
        guardianAddressToRemovalTimestamp[guardianAddress] =
            block.timestamp +
            3 days;
        emit GuardianRemovalQueued(guardianAddress);
    }
     ...
     * @param oldGuardianAddress - address of the guardian to remove
     * @param newGuardianAddress - new guardian address to replace the old guardian
     */
    function executeGuardianRemoval(
        ...
        require(
            guardianAddressToRemovalTimestamp[oldGuardianAddress] > 0,
            "guardian isn't queued for removal"
        );
        require(
            guardianAddressToRemovalTimestamp[oldGuardianAddress] <=
                block.timestamp,
        ...
        isGuardian[oldGuardianAddress] = false;
        isGuardian[newGuardianAddress] = true;
        emit GuardianRemoved(oldGuardianAddress, newGuardianAddress);
    }

    /**
     * @notice Allows the owner to cancel the removal of a guardian
     * @param guardianAddress - address of the guardian queued for removal
     */
    function cancelGuardianRemoval(address guardianAddress) external onlyOwner {
        guardianAddressToRemovalTimestamp[guardianAddress] = 0;
    }
}
