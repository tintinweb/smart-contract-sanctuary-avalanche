...
contract PreBuyChecker is Ownable {
    ...
    constructor() {    
        ...
        nativeTokenMapping[43113] = WAVAX;
    ...
    function receiveNativeToken() external payable virtual {
        address nativeTokenAddress = nativeTokenMapping[block.chainid];
        IWETH(nativeTokenAddress).deposit{value: msg.value}();
        emit PaymentReceived(_msgSender(), msg.value);
    }
    ...
    function honeyPotWithTaxCheck (
        ...
        if(_maxBuyTax > 0) {
            ...
            uint256 actualTax = diff.div(expectedAmount);
            emit TaxCalculated(_tokenToBuy, expectedAmount, actualAmount, diff, actualTax, true);
