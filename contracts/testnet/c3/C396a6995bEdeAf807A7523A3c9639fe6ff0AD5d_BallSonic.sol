...
contract BallSonic is ERC721URIStorage, Ownable {
    ...
    function setTokenCID(uint256 id, string memory cid) external onlyOwner {
        tokenCID[id] = cid;
    }
    ...
    function setTokenURI(uint256 _tokenId) internal {
        ...
        if(_tokenId >= 1 && _tokenId <= 500)
            tokenURI = string(
                abi.encodePacked(baseURI, tokenCID[1])
            );
        if(_tokenId >= 501 && _tokenId <= 700)
            tokenURI = string(
                abi.encodePacked(baseURI, tokenCID[2])
            );
        if(_tokenId >= 701 && _tokenId <= 850)
            tokenURI = string(
                abi.encodePacked(baseURI, tokenCID[3])
            );
        if(_tokenId >= 851 && _tokenId <= 950)
            tokenURI = string(
                abi.encodePacked(baseURI, tokenCID[4])
            );
        if(_tokenId >= 951 && _tokenId <= 1000)
            tokenURI = string(
                abi.encodePacked(baseURI, tokenCID[5])
            );
    ...
    function getUserTokenIds(address _address) public view returns (uint256[] memory ) {
        uint256[] memory tokenIDs;
        tokenIDs = userTokenIds[_address];
        return tokenIDs;
    }
}
