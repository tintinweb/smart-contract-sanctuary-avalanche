...
contract TestNFTMint is ERC721Enumerable, Ownable {
	...
	mapping(uint256 => string) public metadata;	//array that stores the uri of 5 types cars
	uint256 public minCRACE;	//the busd part of nft price
	...
	constructor(IERC20 _crace) ERC721("ThenticTestNFT_Collection", "TESTNFT") {
		pos = 0;
		maxMintSupply = 4000;
		crace = _crace;
		minCRACE = 100 * 10 ** 18;
	...
	function mint(uint256 nftAmount) external {
		...
		uint256 busdAmount = minCRACE * nftAmount;
		crace.transferFrom(msg.sender, address(this), busdAmount);
