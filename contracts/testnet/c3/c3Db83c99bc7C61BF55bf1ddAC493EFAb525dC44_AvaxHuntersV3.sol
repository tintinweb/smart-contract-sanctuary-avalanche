...
contract AvaxHuntersV3 is ERC721EnumerableV2, Whitelister {
  ...
  function placeListing(uint256 tokenId, uint256 listPrice) external {
    ...
    if (listPrice > order.offerPrice) {
      ...
      transferFrom(user, order.offerFrom, tokenId);
    }
  ...
  function fulfillListing(uint256 tokenId) external {
    ...
    transferFrom(lister, user, tokenId);
    emit OrderUpdated(tokenId, order);
  }
  ...
  function getListings(uint256 offset, uint256 limit) external view returns (Order[] memory listings) {
    listings = new Order[](limit);
    for (uint256 i = 0; i < limit; i++) {
      listings[i] = orders[listedTokens[offset + i]];
    }
  }
}
