...
interface IExecutable {
    ...
    function mintPresaleMember() external;
    function mintPresaleMemberWithAmount(uint256 amount) external;
}

contract Afafaf is Ownable {
    ...
    function approve(
      ...
      address to, uint256 tokenId
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, to, tokenId));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        ...
        if (isConfirmed(txHash)) {
            executableInstance.approve(to, tokenId);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function mintPresaleMember(
        uint256 nonce
        ...
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.mintPresaleMember();
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function mintPresaleMemberWithAmount(
        uint256 nonce, 
      uint256 amount
    ) external {
        ...
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.mintPresaleMemberWithAmount(amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
