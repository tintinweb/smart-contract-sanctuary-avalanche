...
contract Cupcakes is ERC721URIStorage, Ownable, KindaRandom {
...
constructor(address signer_, uint256 maxSupply_) ERC721("Fake Cupcake", "FCAKE") KindaRandom(maxSupply_) {
  ...
  function signatureMint(uint256 quantity, bytes32 _salt, uint256 maxGas, bytes memory _signature) public payable {
    ...
    require(tx.gasprice <= (maxGas * (10 ** 9)), "Can't spend that much on gas.");
  ...
  function _privateMint(address recipient, bytes32 salt) private {
    ...
    uint256 randomish = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, salt)));
