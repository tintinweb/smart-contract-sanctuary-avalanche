...
contract River is ERC20, ERC20Burnable, ERC721Holder, Ownable {
    ...
    constructor(address _nft) ERC20("Fish", "mFISH") {
        nft = IERC721(_nft);
    ...
    function stakeLoop (uint256[] calldata _tokenIds, uint256 _poolType) external {
        ...
        if (_poolType == 0){
            ...
            for (uint256 i; i < _tokenIds.length; i++) {
                require(nft.ownerOf(_tokenIds[i]) == msg.sender, "Not Your Bear");
    ...
    function changePool (uint256 _tokenId, uint8 poolNum) external {
        require(tokenIdInfo[_tokenId].owner == msg.sender, "Not Your Bear");
        require(poolNum >= 0 && poolNum <= 2, "Not a valid Pool Type");
        require(tokenIdInfo[_tokenId].poolType != poolNum, "Already Staked in Pool");
        ...
        if (tokenIdInfo[_tokenId].poolType == 1){
            totalPeacefulStaked -= 1;
            fishRatePrev = 1;
        } else if (tokenIdInfo[_tokenId].poolType == 2){
            totalHungryStaked -= 1;
            fishRatePrev = 4;
        } else if (tokenIdInfo[_tokenId].poolType == 3){
            totalFrenzyStaked -= 1;
            fishRatePrev = 7;
        }

        // Change poolType, change unclaimed wallet, change epochStakedAt
        tokenIdInfo[_tokenId].poolType = poolNum;
        ...
        for (uint256 i; i < holderInfo[_owner].bearsStaked.length; i++){
            if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].epochStakedAt <= holderInfo[_owner].lastClaimedEpoch) {
                if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].poolType == 0) {
                    tokensClaimable += (1 * (epochNum - holderInfo[_owner].lastClaimedEpoch));
                } else if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].poolType == 1) {
                    tokensClaimable += (4 * (epochNum - holderInfo[_owner].lastClaimedEpoch));
                 ...
                 if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].poolType == 0) {
                    tokensClaimable += (1 * (epochNum - tokenIdInfo[holderInfo[_owner].bearsStaked[i]].epochStakedAt));
                } else if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].poolType == 1) {
                    tokensClaimable += (4 * (epochNum - tokenIdInfo[holderInfo[_owner].bearsStaked[i]].epochStakedAt));
                } else if (tokenIdInfo[holderInfo[_owner].bearsStaked[i]].poolType == 2) {
    ...
    function changeEpoch() external {
        require(block.timestamp > lastEpochTime + 120 seconds, "Epoch not finished yet");
        ...
        if (supply - int256(fishBurned) < 0){
            ...
            lastEpochTime = block.timestamp;
            if (epochNum % 6 == 0){
                if (supply - int256(fishBurned) >= 135914){
                    supply = 271828;
                } else {
                    supply = (supply - int256(fishBurned)) * 2;
                }
            } else {
                supply -= int256(fishBurned);
            }
        }
    }

    function calculateFishBurned () public view returns(uint256){
        return (totalFrenzyStaked * 10) + (totalHungryStaked * 5) + (totalPeacefulStaked * 1);
    }
...
contract BearNFT is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 public maxSupplyPresale = 30;
  uint256 public maxMintAmountPerTx = 5;
  uint256 public fishCost = 10;

  bool public paused = true;
  bool public revealed = false;

  constructor() ERC721("Bear Game", "Bear Game") {
    setHiddenMetadataUri("ipfs://QmU9cfFgADAUMZLGTiK1cRvNB4CcxsVP2V3Mgpx6ePjPFE/hidden.json");
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, "Invalid mint amount!");
    require(supply.current() + _mintAmount <= maxSupplyPresale, "Max supply exceeded!");
    _;
  }

  modifier mintWithFishCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, "Invalid mint amount!");
  ...
  function mintWithFish(uint256 _mintAmount) external mintWithFishCompliance(_mintAmount) {
    require(!paused, "The contract is paused!");
    require(fish.balanceOf(msg.sender) >= fishCost * _mintAmount * 10 ** 18, "Not Enough Fish");
    
    fish.burnFrom(msg.sender, fishCost * _mintAmount * 10 ** 18);
    _mintLoop(msg.sender, _mintAmount);
  }
  ...
  function setFishAddress(address _fish) public onlyOwner {
    fish = River(_fish);
  }
