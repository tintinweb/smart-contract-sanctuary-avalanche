 ...
 */
contract ERC721Controller is IERC721, IERC721Metadata {
...
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import {LibEtherClock} from "../contracts/Libraries/LibEtherClock.sol";
...
library LibEtherClock {
    ...
    bytes32 internal constant STORAGE_SLOT =
        keccak256("EtherClock.contracts.Libraries.LibEtherClock");
    ...
    function buildMetadata(uint256 tokenId) internal view returns (string memory) {
        string memory temp = clockFrame(tokenId);
    ...
    function buildDigits(uint256 num, uint256 digit) internal view returns (string memory content) {
        ...
        for (uint256 index = 0; index < digitsBuild_.length; index++) {
            content = string(abi.encodePacked(content, _storage().svgContent[num][digitsBuild_[index]]));
        }
    }
    ...
    */
    function getDigitsBuild (uint256 digit) internal view returns (uint256[] memory digitsArr) {
        digitsArr = _storage().digitsBuild[digit];
    }
