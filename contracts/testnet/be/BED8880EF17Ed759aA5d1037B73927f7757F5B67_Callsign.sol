...
import "./ICallsign.sol";

/// @title Shrapnel Callsign Contract (Callsign.sol)
/// @author benhooper.eth
contract Callsign is
  ICallsign,
  ERC721,
  ERC721Burnable,
  IERC2981,
  AccessControl
{
  ...
  // role used by backend to mint callsigns
  ...
  /**
   * Function that validates callsign only contains:
   * A-Z a-z 0-9 + _ . -
   *
   * And only have a length of 1 - 16
   * If it does have a length of 1 - it can not be characters + _ . -
   *
   * Works by searching each byte value for it's corresponding ascii code
   *
   * Returns the string as a bytes32 value if valid - bytes32(0) otherwise
   */
  function _validateCallsign(string memory _callsignToValidate)
    internal
    pure
    returns (bytes32)
  {
    bytes32 _callsign = _stringToBytes32(_callsignToValidate);
    bytes32 _empty = bytes32(0);
    if (_callsign != _empty) {
      // check it's length isn't > 16
      if (_callsign << (16 * 8) == _empty) {
        // if it's single character callsign can't be special character
        if (_callsign[1] == 0x00) {
          if (
            !((_callsign[0] >= 0x30 && _callsign[0] <= 0x39) || //9-0
              (_callsign[0] >= 0x41 && _callsign[0] <= 0x5A) || //A-Z
              (_callsign[0] >= 0x61 && _callsign[0] <= 0x7A)) //a-z
          ) {
            return _empty;
          }
          return _callsign;
        }
        // now just needs to be A-Z a-z 0-9 . + - _
        for (uint256 i; i < _callsign.length; i++) {
          bytes1 char = _callsign[i];
          if (
          ...
          if (char == 0x00) {
            // now check the rest of the callsign after the first 0x00 found - if it matches, break the loop early and return true
            if (_callsign << ((i) * 8) == bytes32(0)) {
              return _callsign;
            }
          }
        }
  ...
  /// @notice The function that mints callsign tokens, can only be called by uses with MINTER_ROLE assigned
  function mint(
    address _to,
    address _origin,
    string memory _callsign
  ) public override onlyRole(MINTER_ROLE) {
    bytes32 converted = _validateCallsign(_callsign);

    // _validateCallsign returns bytes32(0) - it means it's an invalid callsign
    if (converted == bytes32(0)) {
      revert InvalidCallsign();
    }

    // now we convert to uppercase
    bytes32 upperCase = _upperBytes(converted);

    if (callsignStorage[upperCase].creator != address(0)) {
      revert ApologiesAlreadyTaken();
    }

    uint256 currentTokenId = uint256(converted);

    callsignStorage[upperCase].creator = _to;
    callsignStorage[upperCase].origin = _origin;
    callsignStorage[upperCase].callsign = _callsign;
    callsignStorage[upperCase].createdAt = block.timestamp;

    emit Mint(_to, _callsign, currentTokenId, callsignStorage[upperCase]);
  ...
  {
    // token status is ACTIVE or IN_REVIEW - return real tokenURI, else return the placeholder STATUS URI
    // for respective status
    if (
...
/// @title Shrapnel Callsign Contract Interface (ICallsign.sol)
/// @author benhooper.eth
...
interface ICallsign {
  ...
  /// ERRORS
  // error thrown when duplicate callsign mint is tried
  error ApologiesAlreadyTaken();

  // error thrown when callsign doesn't match regex
  error InvalidCallsign();

  // error thrown when an address with no minting privilleges tries to mint
  error NotMinter();
  ...
  // event emitted when mint is succesfully called
  event Mint(
    address _recipient,
    string _callsign,
    uint256 _tokenId,
    CallsignStorage _callsignStorage
  );
  ...
  // spoof ownership transferred event
  event OwnershipTransferred(address oldOwner, address newOwner);

  /// METHODS
  /**
   * Minting function - creates a callsign token
   * @param _to The address to receive the token
   * @param _origin The origin address of the contract
   * @param _callsign The callsign value, which fits the following spec:
   *
