...
import "./utils/NODERewardManagement.sol";
...
contract Bifrost is Ownable, PaymentSplitter {
    ...
    function claimReward(
        ...
        rewardAmount -= _applyCashOutFee(rewardAmount, tier.cashOutFee);
        nodeRewardManager._cashoutNodeReward(sender, createdAt);
...
contract NODERewardManagement {
    ...
    modifier onlySentry() {
        console.log(msg.sender, token, msg.sender == token,  "Owner or Bifrost contract");
        emit CheckValidate(msg.sender, token, msg.sender == token);
