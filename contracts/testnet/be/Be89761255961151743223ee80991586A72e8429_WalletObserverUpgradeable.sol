 ...
 * @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 *
 *    Web:     https://aviators.finance
 */

// Read more about Smart Daily Limits @ https://sdl.aviators.finance
...
import "../implementations/output/AircraftsManagerImplementationPointerUpgradeable.sol";
...
contract WalletObserverUpgradeable is
    ...
    AircraftsManagerImplementationPointerUpgradeable
{
    ...
    function changeAviatorsImplementation(address newImplementation)
        public
        virtual
        override(AviatorsImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeAviatorsImplementation(newImplementation);
        excludeFromObserver(aviators.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(aircraftsManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to aicraftsManager, use the Control Tower to create an aircraft (https://aviators.finance/controltower)"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= aviators.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (aviators.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return aviators.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the Aviators team to know if
...
import "../../contracts/interfaces/IAircraftsManager.sol";

abstract contract AircraftsManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IAircraftsManager internal aircraftsManager;

    event UpdateAircraftsManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyAircraftsManager() {
        require(
            address(aircraftsManager) != address(0),
            "Implementations: AircraftsManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(aircraftsManager),
            "Implementations: Not AircraftsManager"
        );
        _;
    }

    function getAircraftsManagerImplementation() public view returns (address) {
        return address(aircraftsManager);
    }

    function changeAircraftsManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(aircraftsManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "AircraftsManager: You can only set 0x0 or a contract address as a new implementation"
        );
        aircraftsManager = IAircraftsManager(newImplementation);
        emit UpdateAircraftsManager(oldImplementation, newImplementation);
    }
