...
contract TraderJoeRouterFunctions {
    ...
    function swapExactTokensForTokens(uint256 _amountIn, uint256 _amountOutMin, Components.Path memory path, address _to, uint256 _deadline) external  returns(uint256 amountOut){
        return amountOut;
    }
...
contract CounterCaller {
   ...
   function mockPath() public pure returns (Components.Path memory) {
        Components.Path memory path;
        path.pairBinSteps = new uint256[](1);
        path.pairBinSteps[0] = 20;
        path.versions = new uint8[](1);
        path.versions[0] = 0;
        path.tokenPath = new address[](2);
        path.tokenPath[0] = address(0xd00ae08403B9bbb9124bB305C09058E32C39A48c);
        path.tokenPath[1] = address(0x477Fd10Db0D80eAFb773cF623B258313C3739413);
        return path;
    }

   

        function swapExactTokensForTokens(uint256 amountIn, uint256 amountOutMin,uint256 deadline) public returns (uint256){
       Components.Path memory path = mockPath();
        address _to = address(this);
        return myTraderJoeRouter.swapExactTokensForTokens(amountIn, amountOutMin, path,  _to, deadline);
    }
