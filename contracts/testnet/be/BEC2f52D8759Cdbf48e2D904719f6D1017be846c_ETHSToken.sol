...
contract ETHSToken is IBEP20, Auth {
    ...
    // TraderJoe router
    address public constant ROUTER = 0xadcBe444619dE95aeDD82245d0B422288b27C895;  //debugging
    ...
    mapping(address => bool) public isFeeExempt;  // debugging
    mapping(address => bool) public isTxLimitExempt; // debugging
    mapping(address => bool) public isMaxWalletExempt; // debugging

    mapping(address => uint256) public localVault; // debugging
    mapping(address => uint256) public localVaultCooldowns; // debugging

    mapping(address => bool) public lpPairs;  // debugging
    ...
    uint256 public sniperTimestampThreshold;// debugging private

    uint256 public launchTaxTimestamp;  // debugging private
    uint256 public launchTaxEndTimestamp; // debugging private

    mapping(address => bool)public  blacklisted; // debugging 
    ...
    function _approve(address sender, address spender, uint256 amount) public { // debugging private
    ...
    function shouldSwapBack() public view returns (bool) { // debugging internal
    ...
    function swapBack() public swapping { // debugging - private
        lastSwap = block.timestamp;
        
        uint256 swapAmount = swapThreshold;
        uint256 amountToLiquify = ((swapAmount * liquidityFee) / totalFee) / 2;  // half tokens and half AVAX for adding liquidity...
        ...
        // if(amountAVAXReflection > 0) {  debugging
    ...
    function setInSwap(bool _inSwap) external { // debugging onlyVaultDistributor
        inSwap = _inSwap;
    }
