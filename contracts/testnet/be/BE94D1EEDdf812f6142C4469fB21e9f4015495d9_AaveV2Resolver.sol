...
interface AaveIncentivesInterface {
    ...
    function getAssetData(address asset) external view returns (uint128 emissionPerSecond,uint128 lastUpdateTimestamp, uint256 index);
}
...
contract AaveHelpers is DSMath {
        ...
        returns (AaveTokenData memory aaveTokenData)
    {
        ...
        aaveTokenData.totalATokenSupply = aaveTokenData.totalSupply;
        aaveTokenData.decimal = TokenInterface(token).decimals();

        (,aaveTokenData.aEmissionPerSecond,) = AaveIncentivesInterface(
            getAaveIncentivesAddress()
        ).getAssetData(aaveTokenData.aTokenAddress);

        (,aaveTokenData.vEmissionPerSecond,) = AaveIncentivesInterface(
            getAaveIncentivesAddress()
        ).getAssetData(aaveTokenData.variableDebtTokenAddress);

        (,aaveTokenData.sEmissionPerSecond,) = AaveIncentivesInterface(
            getAaveIncentivesAddress()
        ).getAssetData(aaveTokenData.stableDebtTokenAddress);
    }
    ...
    function getTokenData(
        ...
        aaveTokenData.totalCurrentVariableDebt = aaveTokenData.totalVariableDebt;
