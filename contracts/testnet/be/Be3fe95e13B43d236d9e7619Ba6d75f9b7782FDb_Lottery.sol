...
interface ILotteryDerivedState is ILotteryStructs {
    ...
    /// @notice Get the lottery winner by lottery id
    /// @return The winner address for the given lottery id
    function getLotteryWinnerByLotteryId(uint256 id) external view returns (address);
}
...
contract Lottery is ILottery, Ownable, Pausable, ReentrancyGuard, VRFConsumerBaseV2, PullPayment {
    ...
    /// @inheritdoc ILotteryDerivedState
    function getLotteryWinnerByLotteryId(uint256 _id) external view override returns (address) {
        return lotteriesWinner[_id];
    }
