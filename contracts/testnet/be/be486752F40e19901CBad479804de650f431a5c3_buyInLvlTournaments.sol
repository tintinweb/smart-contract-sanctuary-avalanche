...
contract buyInLvlTournaments is FrensProtocolClient {
    ...
    constructor(address _fpt, uint _maxPlayers ) {
        // 0x5E20E033d579091888b276C885eEB76cAB2a0A55
        ...
        //buyInLvl[0] = 0.1 ether;
        //buyInLvl[1] = 0.2 ether;
        //buyInLvl[2] = 0.5 ether;
        //buyInLvl[3] = 1 ether;
        //buyInLvl[4] = 2 ether;
        //buyInLvl[5] = 5 ether;
    ...
    mapping (address => mapping(uint => PlayerData)) public sessionsPlayerData;

    mapping(address => mapping(uint => bool)) public sessionIsRegistered;
    ...
    function register(uint _id, string memory _buyInLvl) external payable returns(bool) {
        ...
        require(!sessionIsRegistered[msg.sender][_id], "Wallet already registered for the session.");
        ...
        return sessionIsRegistered[msg.sender][_id] = true;
    }
    ...
    function requestRankingReward(
        ...
        string memory _pathUint1,
        string memory _pathUint2,
        string memory _pathString1, 
        string memory _pathAddress 
    ) public {

        require(!sessionsPlayerData[address(msg.sender)][_id].isRewarded, "Player already rewarded for that session!");
        require(sessionIsRegistered[msg.sender][_id], "Player not registered for the session.");

        string memory _jsonext = ".json";
        // @notice concatenate the address of the player and the path in the json.
        string memory _urlSession = concatenate(_baseUrl, _sessionId);
        ...
        get2Uint2StringRequest(
            ...
            _pathUint1, // The API path of the uint1 data
            _pathUint2, // The API path of the uint2 data
            _pathString1, // The API path of the string1 data
    ...
    function achievedRequest(bytes32 _requestId, uint256 _uint1, uint256 _uint2, string calldata _string1, string calldata _address) external recordAchievedRequest(_requestId)
    {
        ...
        rank = _uint1;
        buyLvl = _string1;
        sessionsPlayerData[address(addr)][id].rank = _uint1;

        require(sessionsPlayerData[address(addr)][id].rank <= sessions[buyLvl][id].payoutPlayers, "Player not in the Payout rankings !");
        ...
        sessionsPlayerData[address(addr)][id].isRewarded = true;
    }
