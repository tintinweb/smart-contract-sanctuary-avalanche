...
contract KapaDexBank is Ownable {
	...
	uint256 constant public DEV_FEE = 300;
	uint256 constant public PERCENT_STEP = 10;
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public PLANPER_DIVIDER = 10000;
	uint256 constant public	WITHDRAW_COOLDOWN = 7 days; // claim 7 time per day
	...
	function invest(address referrer) public payable {
		...
		uint256 fee1 = msg.value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		uint256 fee2 = msg.value.mul(DEV_FEE).div(PERCENTS_DIVIDER);
		uint256 fee = fee1.add(fee2);
		commissionWallet.transfer(fee1);
		devWallet.transfer(fee2);
	...
	function withdraw() public {
		...
		uint256 fee1 = totalAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		uint256 fee2 = totalAmount.mul(DEV_FEE).div(PERCENTS_DIVIDER);
		uint256 fee = fee1.add(fee2);
