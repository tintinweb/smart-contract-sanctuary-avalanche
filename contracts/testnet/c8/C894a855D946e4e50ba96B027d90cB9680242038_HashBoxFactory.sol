...
contract HashBoxBeta {
        ...
        onlyOwner
    {
        ...
        addressToIndex[_address] = _ipfsCount;
        nameToIndex[_name] = _ipfsCount;
        _ipfsCount++;
    }
    ...
    function removeContactByName(string calldata name) public onlyOwner {
        ...
        require(removeIndex < _ipfsCount, "Index is out of range");
    ...
    {
        Contact[] memory result = new Contact[](_ipfsCount);
        for (uint256 i = 0; i < _ipfsCount; i++) {
            result[i] = contacts[i];
        }
        return result;
    }
    ...
    function readTotalContacts()
        ...
        view
        onlyOwner
        returns (uint256 ipfsCount)
    {
        ipfsCount = _ipfsCount;
        return ipfsCount;
    }
    ...
    // Leaving these two functions in case of accidental transfer of money into contract
...
contract HashBoxFactory {
    ...
    mapping(address => HashBoxBeta) private addressBooks;
    ...
    {
        HashBoxBeta newBook = new HashBoxBeta(msg.sender);
