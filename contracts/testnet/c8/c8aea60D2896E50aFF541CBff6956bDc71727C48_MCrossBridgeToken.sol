...
contract MCrossBridgeToken is Ownable, IMCrossBridgeToken {
    ...
    // Internal Function
    function _mintAxelar(uint256 amount, bytes calldata _payload) internal {
        string memory destinationChain;
        address minter;

        (destinationChain, minter) = abi.decode(_payload, (string, address));

        bytes memory payload = abi.encode(minter, amount);
        ...
        require(
            bytes(destinaionChain[destinationChain]).length > 0,
        ...
        gasReceiver.payNativeGasForContractCallWithToken{value: msg.value}(
            address(this),
            destinationChain,
            destinaionChain[destinationChain],
            payload,
            "WETH",
            amount,
            minter
    ...
    function _bridge(uint256 amount, bytes calldata payload) internal override {
        require(msg.sender == address(controller), "Only Controller Call");
        _mintAxelar(amount, payload);
    }
}
