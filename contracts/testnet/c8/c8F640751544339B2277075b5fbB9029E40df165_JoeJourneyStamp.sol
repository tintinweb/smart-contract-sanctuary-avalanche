...
contract JoeJourneyStamp is BaseLaunchpeg1155, IJoeJourneyStamp {
    ...
    uint256 public override stampsAmount;
        ...
        atPhase(Phase.Allowlist)
    {
        ...
        uint256 _stampsAmount = stampsAmount + 1;
        ...
        uint256 j;
        for (uint256 i; i < _stampsAmount; ++i) {
            amountsList[j] = 1;
            if (_checkIfStampAllowlisted(i, allowlistedStamps)) {
                stampsList[j] = i;
                stampsList[j++] = i;
                allowlistedStamps = _removeStamp(j, allowlistedStamps);
                if (j == _quantity) {
                    break;
                }
            }
    ...
    function balanceOf(address _user) external view override returns (uint256) {
        ...
        uint256 _stampsAmount = stampsAmount;
        for (uint256 i; i < _stampsAmount; i++) {
    ...
    function _allowlist(address user) internal view returns (uint256) {
        ...
        for (uint256 i = 1; i < _stampsAmount + 1; i++) {
    ...
    {
        newStamps = currentStamps | bytes32(2**stamp);
    }
    ...
    function _removeStamp(uint256 stamp, bytes32 currentStamps)
        ...
        returns (bytes32 newStamps)
    {
        bytes32 removedStamp = bytes32(type(uint256).max - 2**stamp);
        newStamps = removedStamp & currentStamps;
    }
    ...
    function _checkIfStampAllowlisted(uint256 stamp, bytes32 userStamps)
        ...
        returns (bool)
    {
        return (uint256(userStamps) >> stamp) & 1 == 1;
    }
