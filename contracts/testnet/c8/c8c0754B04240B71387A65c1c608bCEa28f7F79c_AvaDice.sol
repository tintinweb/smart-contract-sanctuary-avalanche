...
interface IRoll {
    function roll() external returns (uint);

    function flip() external returns (uint);
}
// File: contracts/IStrongbox.sol
...
contract AvaDice {
    ...
    event GameResult(address player, uint playedAmount, uint wonAmount, bool result, uint winningNumber);

    address private owner;
    uint256 private maxLimit = 2 ether;
    uint256 private minLimit = 0.1 ether;
    uint256 private houseEdge = 2;
    ...
    function setVRF(address _vrfAddress) public onlyOwner {
        vrfAddress = _vrfAddress;
        vrf = IRoll(vrfAddress);
    }
    ...
    function setOnChain() external onlyOwner {
        isOnchain = !isOnchain;
    }
    ...
    function playDice(
        ...
        if(dice1 == roll || dice2 == roll || dice3 == roll) {
            uint pay = msg.value * 2 * (100 - houseEdge) / 100;
            console.log(pay);
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, msg.value, pay, true, roll);
            console.log("Win");
        }
        else {
            emit GameResult(msg.sender, msg.value, 0, false, roll);
            console.log("Lose");
        }
    }
