...
contract NPC is Context, IERC20, Ownable {
    ...
    address public LpReceiver =
        payable(0xd8E80326977d85ad7323D2b62194742dc73283dc);

    mapping(address => uint256) private cooldown;

    address payable private _developmentWalletAddress =
        payable(0x4e1A5e7a59c0E59935c33b918119d6c35a663011);
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                if (contractTokenBalance > 0) {
                    ...
                    uint256 amountToSwap = Fee.sub(halfLP);

                    uint256 actualAmountToSwap =  balanceOf(address(this)).mul(amountToSwap).div(Fee);
                    uint256 halfLPToken = balanceOf(address(this)).mul(halfLP).div(Fee);

                    uint256 beforeETHBalance = address(this).balance;
                    swapTokensForEth(actualAmountToSwap);
                    uint256 afterETHBalance = address(this).balance.sub(beforeETHBalance);

                    uint256 marketingShare = marketingFee.div(amountToSwap).mul(100);
                    uint256 marketingTax = afterETHBalance.mul(marketingShare).div(100);
                    

                    if (marketingTax> 0) {
                        sendETHToFee(marketingTax);
                    }

                    uint256 liquidityShare = halfLP.div(amountToSwap).mul(100);
                    uint256 amountToLiquify = afterETHBalance.mul(liquidityShare).div(100);
    ...
    function _takeTeam(uint256 tTeam) private {
        ...
        uint256 afterBalance = balanceOf(address(this)) - beforeBalance;
         emit Transfer(msg.sender, address(this), afterBalance);
