...
contract factoryStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    struct stakedFactoryStruct{
        uint256[6] amounts;
        uint256 stakedSince;
    }

    mapping(address => stakedFactoryStruct) public stakedDetails;
    ...
    function stake(uint256 level, uint256 amount) public{
        ...
        stakedFactoryStruct memory details = stakedDetails[msg.sender];
        details.amounts[level] = amount;
        details.stakedSince = block.timestamp;
        stakedDetails[msg.sender] = details;
    ...
    function myClaimableView(address user) public view returns(uint256){
        uint256 total;
        for(uint i=1; i<=5;i++){
            total += (((stakedDetails[user].stakedSince - block.timestamp)* factoryToken.balanceOf(user, i) * SUPPLY_PER_DAY_PER_LVL + BASE_SUPPLY_RATE)/86400);
        }
        return total;
    }
    function claimSupply() public{
        ...
        stakedFactoryStruct memory details = stakedDetails[msg.sender];
        for(uint i=1;i<=factoryToken.MAX_FACTORY_LEVEL();i++){
            details.amounts[i] = factoryToken.balanceOf(msg.sender, i);
        }
        details.stakedSince = block.timestamp;
        stakedDetails[msg.sender] = details;
