...
contract LaunchpadProxy is ILaunchpad2, Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        IManager.Project memory proj = IManager(msg.sender).project2(projectId_);
        ...
        projectToken = IERC20Metadata(proj.token);
        projectWallet = proj.wallet;
    ...
    function setVestingContract(address vestingContract_)
        external
    ...
    function invest(
        ...
        console.log('invest', block.timestamp, startTime_);
    ...
    function _userShare(address user) internal view returns (uint256) {
        return (invested[user] * _oneUsd()) / raised;
    }
    ...
    function payBackOnPrice(address investor) external onlyRole(MANAGER_ROLE) {
        ...
        uint256 claimedInUsd = (IVesting(vestingContract).claimed(investor) * price_) /
            _oneUsd();
    ...
    function payBack(address investor) external onlyRole(MANAGER_ROLE) {
        ...
        uint256 claimedInUsd = (IVesting(vestingContract).claimed(investor) * price) /
            _oneUsd();
        uint256 amount = (raisedCapped() * _userShare(investor)) /
            _oneUsd() -
            claimedInUsd;
    ...
    function setRoundData(Launch memory launchData_) external onlyRole(MANAGER_ROLE) {
        require(startTime > block.timestamp, 'round has already started');
        hardCap = launchData_.hardCap;
        softCap = launchData_.softCap;
        maxVentureAllocation = launchData_.maxVentureAllocation;
    ...
    function updateProjectData() external onlyRole(MANAGER_ROLE) {
        IManager.Project memory proj = IManager(msg.sender).project2(projectId);
        projectToken = IERC20Metadata(proj.token);
        projectWallet = proj.wallet;
    }
    ...
    function setStableToken(address stableToken_) external onlyRole(MANAGER_ROLE) {
        require(startTime > block.timestamp, 'round has already started');
        stableToken = IERC20Metadata(stableToken_);
    }
}
