...
interface IVault {
    ...
    // function withdrawFees(address _token, address _receiver_distu, address _receiver_anoc) external returns (uint256);
    function addFeeManager(address _feeManager) external ;
    function removeFeeManager(address _feeManager) external;
    function withdrawFeesFM(address _token, address _receiver_distu) external returns (uint256);
    ...
    function getTokenFundingRate(address _token) external view returns (uint256);
    function getTokenPoolAmount(address _token) external view returns  (uint256);
}

contract OrderBookDemo is ReentrancyGuard{
