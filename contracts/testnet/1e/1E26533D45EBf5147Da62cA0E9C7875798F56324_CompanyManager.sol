...
contract CompanyManager is CompanyManagerStorage {
  ...
  , CountApprovals[] memory countApprovals_
    ...
    for (uint16 i = 0; i < newCountRoles; i = _uncheckedInc16(i)) {
      ...
      uint lenName = bytes(names_[i]).length;
      if (lenName == 0) {
        revert ErrorEmptyString();
      }
      if (lenName >= NAME_LEN_LIMIT) {
        revert ErrorTooLongString(lenName, NAME_LEN_LIMIT);
      }
  ...
  , string calldata departmentTitle
    ...
    if (bytes(departmentTitle).length >= NAME_LEN_LIMIT) {
      revert ErrorTooLongString(bytes(departmentTitle).length, NAME_LEN_LIMIT);
    }
    if (bytes(departmentTitle).length == 0) {
      revert ErrorEmptyString();
    }
  ...
  function setDepartmentOptionApproveLowByHigh(DepartmentUid departmentUid, bool value) external override {
    onlyGovernance();

    if (!_equals(departmentsData[departmentUid].uid, departmentUid)) {
      revert ErrorUnknownDepartment(departmentUid);
    }

    departmentOptions[departmentUid] = setDepartmentOption(
  ...
  returns
  (Worker memory) {
    uint lenName = bytes(name_).length;
    if (lenName == 0) {
      revert ErrorEmptyString();
    }
  ...
  function setWeekBudget(AmountST amountST, address salaryToken_) external override {
    onlyGovernance();

    weekBudgetST = amountST;
    if (salaryToken_ != address(0)) {
      salaryToken = salaryToken_;
    }
  }
  ...
  returns (
    ...
    AmountST departmentWeekBudgetST = AmountST.wrap(
      AmountST.unwrap(weekBudgetST) * departmentShares[departmentUid] / TOTAL_SUM_SHARES
    );
  ...
  ///         #SCB-45: week budget is auto adjusted to available amount at the start of epoch.
  ...
  override
  returns (
    DepartmentUid[] memory
  , AmountST[] memory
  , address
  ) {
    AmountST budgetST = weekBudgetST;
    address destSalaryToken = salaryToken;
    ...
    AmountST availableAmountST = AmountST.wrap(pm.balance(destSalaryToken));
    ...
    AmountST[] memory outAmountsST = new AmountST[](lenDepartments);

    for (uint i = 0; i < lenDepartments; i = _uncheckedInc(i)) {
      outAmountsST[i] = AmountST.wrap(
        AmountST.unwrap(budgetST) * departmentShares[departmentUidsShares[i]] / TOTAL_SUM_SHARES
      );
    }
    return (departmentUidsShares, outAmountsST, destSalaryToken);
  }
  ...
  // *****************************************************
  // *************** Helper functions for AmountST/USD ***
  // *****************************************************
  ...
  function isDepartmentHead(address wallet, WorkerUid workerUid) external view override returns (bool){
    return _isDepartmentHead(wallet, workerUid);
  }

  function _isDepartmentHead(address wallet, WorkerUid workerUid) internal view returns (bool) {
    return !_equalTo(heads[wallet], 0) && _equals(heads[wallet], workerToDepartment[workerUid]);
  }

  function _onlyGovernanceOrDepartmentHead(WorkerUid workerUid) internal view {
    if (! _isGovernance(msg.sender)) {
      if (!_isDepartmentHead(msg.sender, workerUid)) {
...
import "../../interfaces/ICompanyManager.sol";
import "../../interfaces/ICompanyManagerBase.sol";
...
, ICompanyManager {
  ...
  uint constant public TOTAL_SUM_SHARES = 100; //TODO: what value we should use here?
