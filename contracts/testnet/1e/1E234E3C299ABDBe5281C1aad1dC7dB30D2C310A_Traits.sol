...
contract Traits is Ownable, ITraits {
    ...
    string commissionerBody;
    string fraudsterBody;
    ...
    IFraudsterVsCommissioner public fraudsterVsCommissioner;
    ...
    function setGame(address _fraudsterVsCommissioner) external onlyOwner {
        fraudsterVsCommissioner = IFraudsterVsCommissioner(_fraudsterVsCommissioner);
    }

    function uploadBodies(string calldata _commissioner, string calldata _fraudster) external onlyOwner {
        commissionerBody = _commissioner;
        fraudsterBody = _fraudster;
    }
   ...
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IFraudsterVsCommissioner.FraudsterCommissioner memory s = fraudsterVsCommissioner.getTokenTraits(tokenId);
        uint8 shift = s.isFraudster ? 0 : 10;

        string memory svgString = string(abi.encodePacked(
                s.isFraudster ? draw(fraudsterBody) : draw(commissionerBody),
                ...
                s.isFraudster ? drawTrait(traitData[5 + shift][s.headgear % traitCountForType[5 + shift]]) : drawTrait(traitData[5 + shift][s.alphaIndex]),
                !s.isFraudster ? drawTrait(traitData[6 + shift][s.neckGear % traitCountForType[6 + shift]]) : ''
            ));

        return string(abi.encodePacked(
                '<svg id="fraudsterVsCommissioner" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                svgString,
                "</svg>"
            ));
    }
   ...
   */
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(
                ...
                tokenId <= fraudsterVsCommissioner.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
   ...
   */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
                ...
                s.isFraudster ? 'Fraudster #' : 'Commissioner #',
                tokenId.toString(),
                '", "description": "FraudsterVsCommissioner Game is a new generation play-to-earn NFT game on Avalanche that incorporates probability-based derivatives alongside NFTs. Through a vast array of choices and decision-making possibilities, FraudsterVsCommissioner Game promises to instil excitement and curiosity amongst the community as every individual adopts different strategies to do better than one another and to come out on top. The real question is, are you #TeamFraudster or #TeamCommissioner? Choose wisely or watch the other get rich!", "image": "data:image/svg+xml;base64,',
