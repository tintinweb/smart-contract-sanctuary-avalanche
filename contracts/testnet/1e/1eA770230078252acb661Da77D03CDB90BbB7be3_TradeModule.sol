...
contract TradeModule is ModuleBase, ReentrancyGuard {
        ...
        returns (TradeInfo memory)
    {
        ...
        tradeInfo.totalSendQuantity = _sendQuantity; //Position.getDefaultTotalNotional(tradeInfo.setTotalSupply, _sendQuantity);

        tradeInfo.totalMinReceiveQuantity = _minReceiveQuantity; //Position.getDefaultTotalNotional(tradeInfo.setTotalSupply, _minReceiveQuantity);
    ...
    function _validatePreTradeData(TradeInfo memory _tradeInfo, uint256 _sendQuantity) internal view {
        ...
        require(
            IERC20(_tradeInfo.sendToken).balanceOf(address(_tradeInfo.setToken)) >= _sendQuantity,
    ...
    function _updateSetTokenPositions(TradeInfo memory _tradeInfo) internal returns (uint256, uint256) {
        uint256 currentSendTokenBalance = IERC20(_tradeInfo.sendToken).balanceOf(address(_tradeInfo.setToken));

        uint256 currentReceiveTokenBalance = IERC20(_tradeInfo.receiveToken).balanceOf(address(_tradeInfo.setToken));
