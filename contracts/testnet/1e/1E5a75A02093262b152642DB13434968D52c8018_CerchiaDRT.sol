...
contract CerchiaDRT is RoleBased, ICerchiaDRT {
	...
	function userCreateNewDealAsBid(
		...
		require(
			IERC20(s._tokens[denomination]).transferFrom(callerAddress, address(this), premium),
			LibStructStorage.ERROR_TRANSFERRING_TOKEN
		);
	}
	...
	function userCreateNewDealAsAsk(
		...
		// Transfer token from user to contract
		require(
			IERC20(s._tokens[denomination]).transferFrom(callerAddress, address(this), notional - premium),
			LibStructStorage.ERROR_TRANSFERRING_TOKEN
		);
	}
