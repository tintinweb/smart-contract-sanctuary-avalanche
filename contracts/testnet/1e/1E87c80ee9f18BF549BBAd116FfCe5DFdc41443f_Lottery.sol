...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	event TickestBought(uint256 qty, address indexed user);
		...
		nonReentrant
	{
		...
		if ((currentRound.endTime - block.timestamp + qty * newTicketStep[phase]) > roundMaxTime[phase]) {
			currentRound.endTime = block.timestamp + qty * roundMaxTime[phase];
		} else {
			currentRound.endTime = currentRound.endTime + qty * newTicketStep[phase];
		}

		currentRound.leader = _msgSender();
		emit TickestBought(qty, _msgSender());
	}
