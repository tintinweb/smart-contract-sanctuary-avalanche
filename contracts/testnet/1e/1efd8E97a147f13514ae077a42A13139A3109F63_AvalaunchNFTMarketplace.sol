...
interface IAvalaunchNFTMarketplace {
    ...
    event AuctionParametersSet(uint256 timeExtensionPerBid);
    ...
    event AskCreated(uint256 indexed itemId, address indexed from);
...
contract AvalaunchNFTMarketplace is IAvalaunchNFTMarketplace, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public timeExtensionPerBid;

    // Message delivered on successful NFT sell
    bytes public constant MSG = bytes("Your item got sold! Greetings from Avalaunch team :)");

    // Modifiers
    modifier onlyExisting(uint256 itemId) {
    ...
    // Internal modifier logic (for purpose of gas saving)
    ...
    function _onlyExistingAuction(uint256 _itemId) private view {
        if (auctionItems.length <= _itemId) {
            revert AvalaunchNFTMarketplace__InvalidItemId();
        }
    }
     ...
     * @param _timeExtensionPerBid - time for which auction is extended when someone makes an offer
     */
    function initialize(
        address _owner,
        uint256 _feePrecision,
        uint256 _feePercent,
        uint256 _timeExtensionPerBid
        ...
        _setAuctionParameters(_timeExtensionPerBid);
    }
    ...
    function _setFeeParameters(uint256 _feePrecision, uint256 _feePercent) private {
        if (_feePrecision < _feePercent * 5) {
            // 20% maximum fee take
     ...
     * @dev for more information look at _setAuctionParameters
     */
    function setAuctionParameters(uint256 _timeExtensionPerBid) external onlyOwner {
        _setAuctionParameters(_timeExtensionPerBid);
    }
     ...
     * @param _timeExtensionPerBid - time for which auction is extended when someone makes an offer
     */
    function _setAuctionParameters(uint256 _timeExtensionPerBid) private {
        if (_timeExtensionPerBid > 1 days) {
            revert AvalaunchNFTMarketplace__InvalidBiddingParameters();
        }
        timeExtensionPerBid = _timeExtensionPerBid;
        emit AuctionParametersSet(_timeExtensionPerBid);
    }
    ...
    function addItem(address itemAddress, uint256 tokenId, uint256 price, address buyer) public nonReentrant {
        ...
        // Emit event
        emit MarketplaceItemCreated(items.length - 1, itemAddress, tokenId, msg.sender, price);
    }
     ...
     */
    function removeItem(uint256 _itemId) external nonReentrant onlyExisting(_itemId) {
     ...
     */
    function buyItem(uint256 _itemId) external payable nonReentrant onlyExisting(_itemId) {
        // Retreive item from storage
        Item storage item = items[_itemId];
        // Check msg.value
        if (!item.available) {
        ...
        safeFundTransfer(item.seller, msg.value - fee, true);
        // Mark item as unavailable
        item.available = false;
     ...
     * @param _itemId - id of item/nft user wants to buy
     */
    function createAsk(uint256 _itemId) external payable onlyExisting(_itemId) {
        // Check msg value
        if (msg.value == 0) {
            revert AvalaunchNFTMarketplace__InvalidMessageValue();
        }
        ...
        ask.value = msg.value;
        // Emit an event
        emit AskCreated(_itemId, msg.sender);
    }
     ...
     */
    function removeAsk(uint256 _itemId) external {
        // Retrieve ask
        Ask storage ask = asks[_itemId][msg.sender];
        // Check ask activity
        if (!ask.active) {
     ...
     */
    function acceptAsk(uint256 _itemId, address _asker) external nonReentrant onlyExisting(_itemId) {
        ...
        safeFundTransfer(item.seller, ask.value - fee, true);
        // Transfer item to the buyer
     ...
     */
    function addAuctionItem(address itemAddress, uint256 tokenId, uint256 startingPrice) external {
        // Check price
        if (startingPrice == 0) {
            revert AvalaunchNFTMarketplace__ZeroValue();
        }
        // Create item
        auctionItems.push(
            AuctionItem(itemAddress, tokenId, payable(msg.sender), payable(address(0)), startingPrice, 0, 0, true)
        );
        // Transfer item from user to marketplace
        IERC721Upgradeable(itemAddress).transferFrom(msg.sender, address(this), tokenId);
        // Emit event
     ...
     */
    function bid(uint256 _itemId) external payable onlyExistingAuction(_itemId) {
        // Retrieve item
        ...
        // Check msg value
        if ((item.highestBid == 0 && msg.value < item.startingPrice) || msg.value <= item.highestBid) {
            revert AvalaunchNFTMarketplace__InvalidMessageValue();
        }
        ...
        item.endTime = block.timestamp + timeExtensionPerBid;
        // Emit an event
     ...
     * @param _itemId - id of auctioned marketplace item
     */
    function claimAuctionItem(uint256 _itemId) external nonReentrant onlyExistingAuction(_itemId) {
        // Retrieve item
        AuctionItem storage item = auctionItems[_itemId];
        // Check if auction is in progress
        if (block.timestamp < item.endTime || item.endTime == 0) {
            revert AvalaunchNFTMarketplace__AuctionStillInProgress();
        }
        ...
        // Forward AVAX to seller
        safeFundTransfer(item.seller, item.highestBid - fee, true);
        // Transfer item to the buyer
     ...
     * @param _msg - if msg should be sent
     */
    function safeFundTransfer(address _to, uint256 _amount, bool _msg) private {
        (bool success, ) = _to.call{value: _amount}(_msg ? MSG : new bytes(0x00));
        if (!success) {
    ...
    function withdrawFees(address receiver) external onlyOwner {
        ...
        // Transfer funds
        safeFundTransfer(receiver, _fees, false);
        // Emit event
