...
contract NFT is Ownable, ERC165, IERC721, IERC721Metadata, ERC2981 {
    ...
    string private baseExtension = ".jpg";
    ...
    function getAllTokensOwnedByUser(address owner) public view virtual returns (uint256[] memory) {
    ...
    function mint(uint256 amount) public payable {
        ...
        require(msg.value >= amount*mintPrice, "Not enough payed");
        require(totalSupply() + amount < maxSupply, "Total supply would be exceeded");
        for (uint256 i; i < amount; i++) {
            _safeMint(_msgSender(), _allTokens.length);
        }
        if (msg.value > amount*mintPrice) {
            payable(_msgSender()).transfer(msg.value - amount*mintPrice);
        }
    }
