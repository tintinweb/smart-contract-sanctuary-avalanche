...
import "./IGamePool.sol";
import "./InvestPool.sol";
import "./GeekzPass.sol";

contract GamePool is IGamePool, Ownable, ReentrancyGuard {
    ...
    bool public openInvestment = false;
    mapping(uint => bool) poolClosedStatus;
    mapping(uint => uint) nftLastClaimedWeek;
    mapping(uint => uint) unclaimedReward;
    ...
    mapping(uint => InvestLog) public investLog;
    ...
    constructor(
        ...
        // rewardInjector[_ownerAddress] = true;
    ...
    function createPool(
        ...
        uint _poolHardcap,
        uint _minInvest
    ) external override onlyOwner geekzPassSetted {
        poolCounter += 1;
    ...
    function setGeekzPass(address _geekzPassAddress) external override onlyOwner {
    ...
    function poolClosed(uint _investPoolId, uint _investedAmount)
        external
        byInvestPool
    {
        poolClosedStatus[_investPoolId] = true;
        totalPool += _investedAmount;
        InvestLog memory iv = InvestLog(_investPoolId, _investedAmount);
        investLog[_investPoolId] = iv;
    ...
    function setNftClaimed(uint _tokenId) external byInvestPool {
        nftLastClaimedWeek[_tokenId] = rewardCounter;
    }
    ...
    function setGeekzPassData(
    ...
    ) external byInvestPool geekzPassSetted {
        ...
        PoolData storage poolData = investData[_poolNum];
        poolData.poolId = _poolNum;
    ...
    function fillReward(uint _amount, uint _weekNum) public {
        require(rewardInjector[msg.sender]);
        require(tokenAddress != address(0));
        require(treasuryAddress != address(0));
        require(scholarRewardAddress != address(0));
        require(reinvestAddress != address(0));
        require(_amount > 0);
        ...
        // uint toInvestors = _amount - (toTreasury + toScholar + toReinvest);
        ...
        // if needed loop of invest pool distribute share toInvestor
        // distributeRewardToPools(toInvestors);
    }
    ...
    // function distributeRewardToPools(uint _amount) internal {
    //     // loop for
    //     for (uint i; i < investPools.length; i++) {
    //         IERC20 token = IERC20(tokenAddress);
    //         uint share = (investLog[i].amount * _amount) / totalPool;
    //         token.transfer(address(investPools[i]), share);
    //     }
    // }
    ...
    function deployTokenToGame(uint _amount, address _receiverAddress)
        ...
        onlyOwner
    {
        require(availableForDeployment >= _amount);
    ...
    // better to fetch using NFT hold, not by invest
    // using investData state var only for mint usage.
        ...
        returns (uint[] memory, uint[] memory)
    {
        ...
        for (uint i; i < poolCounter; i++) {
            PoolData storage poolData = investData[i];
            poolId[i] = poolData.poolId;
            amounts[i] = poolData.investments[msg.sender];
        }
        ...
        returns (uint unclaimed, uint totalNFT)
    {
        ...
        GeekzPass _geekzPass = GeekzPass(geekzPassAddress);
        totalNFT = _geekzPass.balanceOf(investor);
        uint investorAmount = 0;
    ...
    function claimAll() public nonReentrant geekzPassSetted returns (uint) {
        (uint unclaimed, uint totalNFT) = getUnclaimed();
        require(unclaimed > 0 && totalNFT > 0);

        GeekzPass _geekzPass = GeekzPass(geekzPassAddress);
        for (uint i = 0; i < totalNFT; i++) {
            uint tokenId = _geekzPass.tokenOfOwnerByIndex(msg.sender, i);

            // // withdraw all investpool reward to gamepool
            // // get investor data
            // (, uint _poolNum, ) = _geekzPass.getTokenData(tokenId);
            // InvestPool curPool = InvestPool(investPools[_poolNum]);
            // curPool.retractRewardToGamePool(msg.sender);
    ...
    //     IERC20 token = IERC20(tokenAddress);
    //     token.transfer(investor, unclaimed);
...
interface IGamePool {
    ...
    function setGeekzPass(address _geekzPassAddress) external;
}
...
contract InvestPool is Ownable {
    ...
    function invest(uint _amount) public {
        ...
        // check if time valid
        require(block.timestamp <= poolEnd, "Not in Investing period");

        // require(
        //     block.timestamp >= poolStart && block.timestamp <= poolEnd,
        //     "Not in Investing period"
        // );
    ...
    function mintGeekzPass() external {
        ...
        _gamePool.setNftClaimed(tokenId);
    }
    ...
    // function getInvestorAmount(address adr)
    //     public
    //     view
    //     onlyOwner
    //     returns (uint)
    ...
    function claimReward() external {
        IERC20 token = IERC20(tokenAddress);
        (uint unclaimed, uint totalNFT) = getAvailableClaim(msg.sender);
        require(unclaimed > 0);
    ...
    function getAvailableClaim(address investor)
        public
        view
        returns (uint unclaimed, uint totalNFT)
    {
    ...
    function retractRewardToGamePool(address investor) external {
        (uint unclaimed, uint totalNFT) = getAvailableClaim(investor);
        IERC20 token = IERC20(tokenAddress);
        GeekzPass _geekzPass = GeekzPass(geekzPassAddress);
        GamePool _gamePool = GamePool(gamePoolAddress);
        for (uint i = 0; i < totalNFT; i++) {
        ...
        token.transfer(gamePoolAddress, unclaimed);
    }
    ...
    function finalize() external onlyOwner {
        ...
        GamePool _gamePool = GamePool(gamePoolAddress);
        _gamePool.poolClosed(poolNum, totalTransfer);
        ...
        _gamePool.setGeekzPassData(poolGameId, poolNum, investors, investments);
    }
