...
import "../RawMaterialBase.sol";
import "../../Utils/Constants.sol";

contract Garden is RawMaterialBase, Constants {
    constructor(
...
abstract contract RawMaterialBase is VRFConsumerV2Ownable {
    uint256 public protocolFee = 0.02 ether;
    uint256 public immutable LandType;
    IERC721 public immutable SmolApaLandNFTContract;
    address payable public feeAggregator;
    LandRegistry public immutable LandRegistryContract;
    RawMaterialNFT public immutable RawMaterialNftContract;

    // LandTokenId => (slot => Job)
    mapping(uint256 => mapping(uint256 => Job)) Slots;
    mapping(uint256 => Request) public PlayerRequest;

    // ResourceType => (Level => Chance)
    mapping(uint256 => mapping(uint256 => uint256)) public OutcomeDictionary;
    ...
    event GatherRequest(uint256 tokenId, uint256 targetResource, uint256 slotId);
    event Claimed(uint256 tokenId, uint256 slotId, uint rng);
    event OutcomeDictionarySet(uint256 targetResource, uint256 level, uint256 treshold);
    ...
    constructor(
        ...
        address _landRegistery,
        uint256 _landtype
    ) VRFConsumerV2Ownable(_subscriptionId) {
    ...
    function setOutcomeDictionary(uint256 _targetResource, uint256 _level, uint256 _treshold) external onlyOwner {
        OutcomeDictionary[_targetResource][_level] = _treshold;
        emit OutcomeDictionarySet(_targetResource, _level, _treshold);
    }

    function withdrawProtocolFee() external {
        feeAggregator.transfer(address(this).balance);
    }

    function setGameEngineParametes(uint256 _protocol_fee, address _feeAggregator) external onlyOwner {
        protocolFee = _protocol_fee;
        feeAggregator = payable(_feeAggregator);
    }
    ...
    function gatherRawMaterials(uint256 _landTokenId, uint256 _targetResource, uint256 _slotId)
        external
        payable
        onlyLandOwner(_landTokenId)
    {
        ...
        // second slot reserved for max lvl
        if (_slotId > 0 && level < 10) {
            revert WrongSlot();
        }
        // slot greater than last slot or the slot is already in use
        if (_slotId > 1 || Slots[_landTokenId][_slotId].completedTimeStamp != 0) {
            revert WrongSlot();
        }
        // target resource doesnt exist
        if (OutcomeDictionary[_targetResource][level] == 0) {
            revert WrongResource();
        }
        if (msg.value < protocolFee) {
            revert CostNotCovered();
        }
        Slots[_landTokenId][_slotId] = Job(
            uint64(block.timestamp + determineGatheringDuration(level, _slotId)), uint32(_targetResource), uint32(level)
        );
        emit GatherRequest(_landTokenId, _targetResource, _slotId);
    }

    function initiateClaim(uint256 _slotId, uint256 _landTokenId) external payable onlyLandOwner(_landTokenId) {
        Job memory job = Slots[_landTokenId][_slotId];
        ...
        PlayerRequest[s_requestId] = Request(msg.sender, uint128(_slotId), uint32(_landTokenId));
    }

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        Request storage request = PlayerRequest[requestId];
        Job storage job = Slots[request.landTokenId][request.slotId];

        uint256 targetResource = job.targetResource;
        if (determineOutcome(randomWords[0], targetResource, job.level)) {
            RawMaterialNftContract.mintRawMaterial(
                targetResource, RawMaterialNftContract.AmountProduced(targetResource) + 1, request.player
            );
        }
        emit Claimed(request.landTokenId, request.slotId,randomWords[0]);
        delete PlayerRequest[requestId];
        delete Slots[request.landTokenId][request.slotId];
    }

    function determineOutcome(uint256 seed, uint256 _targetResource, uint256 level) internal view returns (bool) {
        uint256 limit = 1000;
        uint256 roll = seed % limit;
        uint256 chance = OutcomeDictionary[_targetResource][level];
        return (roll < chance);
    }

    function determineGatheringDuration(uint256 _level, uint256 _slotId) public pure returns (uint256 x) {
        uint256 baseTime = 2 hours; // TODO 2 hours for testing, revert to the original value 
        uint256 duration;
        if (_level == 1) {
            duration = baseTime;
        } else if (_level == 2) {
            ...
            duration = (baseTime - 30 minutes);
        } else if (_level == 4) {
            duration = (baseTime - 45 minutes);
        } else if (_level == 5) {
            duration = (baseTime - 60 minutes);
        } else if (_level == 6) {
            duration = (baseTime - 75 minutes);
        } else if (_level == 7) {
            duration = (baseTime - 90 minutes);
        } else if (_level == 8) {
            duration = (baseTime - 105 minutes);
        } else if (_level == 9) {
            duration = (baseTime - 120 minutes);
        } else {
            duration = (baseTime - 150 minutes);
        }
        if (_slotId == 1) {
            duration *= 2;
        }
