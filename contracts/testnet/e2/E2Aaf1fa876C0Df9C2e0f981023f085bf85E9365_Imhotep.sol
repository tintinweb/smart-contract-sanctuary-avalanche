...
contract ERC20 is IERC20 {
    ...
    function availableSupply() public view returns (uint256) {
        return _limitSupply-(_totalSupply);
    }    
...
contract Imhotep is Token {
    ...
    constructor() public {
        ...
        DEV = payable(0xfE2d3A24699CB7dC2bd0fe002816798Fd5639993);
        MAR = payable(0x394440CDB9055aA047a1A2bD151dCF6Bed839c5f);
    ...
    function stakeAVAX(address referrer,  uint256 _amount) public payable {
		...
		uint fee = _amount*ADV_FEE/PERCENT_DIVIDER;       // calculate fees on _amount and not msg.value
		...
		if (user.referrer == address(0) && msg.sender != ADMIN) {
			...
			for (uint256 i = 0; i < REF_BONUSES.length; i++) {
				if (upline != address(0)) {
					...
					if (i == 0) {
					    users[upline].bonAirdrop = users[upline].bonAirdrop+1;
					}
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < REF_BONUSES.length; i++) {
				...
				uint256 amount = _amount*REF_BONUSES[i]/PERCENT_DIVIDER;
        ...
        user.sM.totalStaked = user.sM.totalStaked+_amount;
        totalAVAXStaked = totalAVAXStaked+_amount;
    }
    ...
    function stakeToken(uint tokenAmount) public {
        ...
        totalTokenStaked = totalTokenStaked+tokenAmount; 
    } 
    
    function unStakeToken() public {
        User storage user = users[msg.sender];
        require(block.timestamp > user.sT.lastStakeTime+TIME_TO_UNSTAKE);
        updateStakeToken_IP(msg.sender);
    ...
    function updateStakeAVAX_IP(address _addr) private {
        ...
        if(amount > 0) {
            user.sM.unClaimedTokens = user.sM.unClaimedTokens+amount;
    ...
    function getStakeAVAX_IP(address _addr) view private returns(uint256 value) {
        ...
        if(fr < to) {
            value = user.sM.totalStaked*(to - fr)*Tarif/TIME_STEP/PERCENT_DIVIDER;
        } else {
            value = 0;
        }
        return value;
    ...
    function updateStakeToken_IP(address _addr) private {
        ...
        if(amount > 0) {
            user.sT.unClaimedTokens = user.sT.unClaimedTokens+amount;
            user.sT.checkpoint = block.timestamp;
        }
    } 
    ...
    function getStakeToken_IP(address _addr) view private returns(uint256 value) {
        ...
        uint256 to = block.timestamp;
        if(fr < to) {
            value = user.sT.totalStaked*(to - fr)*(Tarif)/(TIME_STEP)/(PERCENT_DIVIDER);
        } else {
            value = 0;
        }
        return value;
    ...
    function sellToken(uint tokenAmount) public {
        ...
        require(sold[getCurrentDay()]+tokenAmount <= SELL_LIMIT, "Daily Sell Limit exceed");
        sold[getCurrentDay()] = sold[getCurrentDay()]+tokenAmount;
    ...
    function claimAirdrop() public {
        ...
        require(block.timestamp > users[msg.sender].lastAirdrop+NEXT_AIRDROP);
    ...
    function getUserUnclaimedTokens_M(address _addr) public view returns(uint value) {
        User storage user = users[_addr];
        return getStakeAVAX_IP(_addr)+(user.sM.unClaimedTokens); 
    }
    
    function getUserUnclaimedTokens_T(address _addr) public view returns(uint value) {
        User storage user = users[_addr];
        return getStakeToken_IP(_addr)+(user.sT.unClaimedTokens); 
    }  
    ...
    function getUserTimeToNextAirdrop(address _addr) public view returns (uint) {
        return minZero(users[_addr].lastAirdrop+(NEXT_AIRDROP), block.timestamp);
    } 
    ...
    function getUserAirdropReqInv(address _addr) public view returns (uint) {
        uint ca = users[_addr].countAirdrop+(1); 
        return ca*(AIRDROP_VALUE); 
    }       
	...
	function getAPY_M() public view returns (uint) {
		return AVAX_DAILYPROFIT*(365)/(10);
	}
	
	function getAPY_T() public view returns (uint) {
		return TOKEN_DAILYPROFIT*(365)/(10);
	}	
	...
	function getUserTimeToUnstake(address _addr) public view returns (uint) {
		return  minZero(users[_addr].sT.lastStakeTime+(TIME_TO_UNSTAKE), block.timestamp);
	}	
	
    function getTokenPrice() public view returns(uint) {
        uint d1 = getContractAVAXBalance()*(PRICE_DIVIDER);
        uint d2 = availableSupply()+(1);
        return d1/(d2);
    } 

    function AVAXToToken(uint AVAXAmount) public view returns(uint) {
        return AVAXAmount*(PRICE_DIVIDER)/(getTokenPrice());
    }

    function tokenToAVAX(uint tokenAmount) public view returns(uint) {
        return tokenAmount*(getTokenPrice())/(PRICE_DIVIDER);
    } 	
    ...
    function getTimeToNextDay() public view returns (uint) {
        ...
        uint g = getCurrentDay()*(TIME_STEP);
        return g+(TIME_STEP)-(t);
    }  
