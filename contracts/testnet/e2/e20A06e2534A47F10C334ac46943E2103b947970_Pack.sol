    ...
    IPack
{
    ...
    uint256[400] public quackIDsRemaining;
    IERC721 public quacksAddress; 
    uint private lastQuackIndex = 399;
    ...
    function addToQuackCount(uint256 quackCount) external onlyRoleWithSwitch(minterRole) { 
        uint256 startingLength = quackIDsRemaining.length;
        for (uint256 j = 0; j < quackCount; j += 1) {
            quackIDsRemaining[startingLength+j] = startingLength+j;
        }
    }
    ...
    function openPack(uint256 _packId, uint256 _amountToOpen) external returns (Token[] memory, uint256[] memory) {
       ...
       (Token[] memory rewardUnits, uint256 random) = getRewardUnits(_packId, _amountToOpen, pack.amountDistributedPerOpen, pack);
        ...
        uint256[] memory quackIDsToGiveOut = new uint256[](_amountToOpen);
        if (lastQuackIndex > 0) {
            for(uint p =0; p < _amountToOpen; p += 1) {
                uint256 randomVal = uint256(keccak256(abi.encode(random, 9999+p)));
                uint256 targetQuack = randomVal % lastQuackIndex;
                if(targetQuack <= lastQuackIndex) {
                    uint256 tID = quackIDsRemaining[targetQuack];
                    quackIDsToGiveOut[p] = tID;
                    quackIDsRemaining[targetQuack] = quackIDsRemaining[lastQuackIndex];
                    //quackIDsRemaining.pop();
                    lastQuackIndex -= 1;
                    quacksAddress.safeTransferFrom(address(this), msg.sender, tID);
                }
            }
        }
        emit PackOpened(_packId, opener, _amountToOpen, rewardUnits, quackIDsToGiveOut);
        return (rewardUnits, quackIDsToGiveOut);
    }
...
interface IPack is ITokenBundle {
     ...
     */
    function openPack(uint256 packId, uint256 amountToOpen) external returns (Token[] memory, uint256[] memory);
}
