...
// SPDX-License-Identifier: MIT
    ...
    */
    ...
    contract SnowballNFTHolidayHat is ERC721, Ownable{
        using Counters for Counters.Counter;
        Counters.Counter public _tokenIds;

        string public constant URI = "ipfs://bafybeic56pyf5vehkfhpbh3jitkxbvp75hbcxeca224qw2u34lcedkhacm";
        uint public constant PRICE = 1000000000000000000; // 1 AVAX
        uint public constant SALE_DURATION = 86400; // 24 hours in seconds
        
        uint public _sale_start_time;

        address public _feeAddress = 0x294aB3200ef36200db84C4128b7f1b4eec71E38a; // Snowball Treasury
        address payable _feeReceiver;
        mapping (address => uint8) private _mintCounts;

        constructor() ERC721("Snowball NFT Holiday Hat", "SNOBNFTHOLIDAYHAT") {
            _feeReceiver = payable(_feeAddress);
        }

        function saleLive() public view returns (bool) {
            bool sale = _sale_start_time != 0 && _sale_start_time <= block.timestamp && _sale_start_time + SALE_DURATION >= block.timestamp;
        ...
        function mint(address to) public payable{
            require(_sale_start_time > 0, "sale has not yet begun!");
            require(_sale_start_time <= block.timestamp, "sale has not yet begun!");
            require( _sale_start_time + SALE_DURATION >= block.timestamp, "sale has ended!");
            require(msg.value >= PRICE, "must pay the full price!");
        ...
        function startSale(uint time) public onlyOwner {
            _sale_start_time = time;
        }
    }