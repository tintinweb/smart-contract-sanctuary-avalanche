...
contract AudicityNodePlatform is OwnableUpgradeable {
    ...
    address payable operationalAddress;

    uint256 public halvingNodeAmount;
    uint256 public halvingAmount;
    uint256 normalPeriodForFeePayment;
    uint256 noticePeriodForFeePayment;
    uint256 deletePeriodForFeePayment;
    ...
    struct FounderInfo {
        uint256 isFounder;
        uint256 isDistributed;
    }

    mapping(address => FounderInfo) public founders;
    ...
    function initialize(
        ...
        //Founders addresses
        FounderInfo memory founderInfo;
        founderInfo.isDistributed = 0;
        founderInfo.isFounder = 1;
        founders[0x0354Dbe0a74A14841bA21e563e01750da286Cb76] = founderInfo;
        founders[0x0416a61D61fF8E065aC07BE9762b76929Df85B92] = founderInfo;
        founders[0xb36cA14fc81aB39434Dd00f652153a4B9FC92c80] = founderInfo;
        founders[0x2E229aE988c33C81769d4EdeEC8A8385a7957b16] = founderInfo;
        rewardPoolAddress = 0x0000000000000000000000000000000000000002;
        operationalAddress = payable(0xe254f3186888512Ea3A9B72B91A640c3A2F051CD);
        ...
        nodeUsdMonthlyFee = 12000000;
        nodePurchaseLimit = 10;

        halvingNodeAmount = _halvingNodeAmount;

        normalPeriodForFeePayment = 2592000;
        noticePeriodForFeePayment = 5184000;
        deletePeriodForFeePayment = 7776000;
    ...
    function _purchaseNode(uint256 amount, bool compounded) internal{
        ...
        if (totalActiveNodeCount.add(amount) > halvingNodeAmount) {
            if (halvingAmount.mod(2) == 1) {
                ...
                if (!compounded)
                    payment = payment.add(nodeCost.mul(pendingNodesFromOldHalving));
                _setupNewNodes(
                    msg.sender,
        ...
        if (!compounded) {
            uint256 rewardTokens = (payment.mul(975)).div(1000);
            uint256 liquidityTokens = payment.sub(rewardTokens);
            require(
            ...
            uint256 beforeLiquidityRadix = reserveA;
            uint256 beforeLiquidityAvax = reserveB;
            ...
            uint256 afterLiquidityRadix = reserveA;
            uint256 afterLiquidityAvax = reserveB;
        }
    }
    ...
    function _addNodesToAddress(address nodeOwnerAddress, uint256 id, bool status, bool compounded) internal
    {
        ...
        info.operationalUntilTimestamp = block.timestamp.add(normalPeriodForFeePayment);
    ...
    function _getPayAmountForMonthlyCharge(uint256 lastMoment) internal view returns (uint256)
    {
        ...
        for (index = 0; index < 3; index++)
            checkPoints[index] = lastMoment.add(index.mul(normalPeriodForFeePayment));
    ...
    function _updateNodeFee(uint256 nodeId, address user, uint256 advancedPayMonths) internal returns (uint256)
    {
        ...
        if (operationalUntilTimestamp >= block.timestamp) {
            if (
                (operationalUntilTimestamp.add(advancedPayMonths.mul(normalPeriodForFeePayment))).sub(block.timestamp) > deletePeriodForFeePayment
            ) return 0;
            nodeInfo.operationalUntilTimestamp = nodeInfo.operationalUntilTimestamp.add(normalPeriodForFeePayment.mul(advancedPayMonths));
            return nodeUsdMonthlyFee.mul(advancedPayMonths);
        }
        uint256 timeElapsed = block.timestamp.sub(operationalUntilTimestamp);
        if (timeElapsed > noticePeriodForFeePayment) {
            nodeInfo.operationalUntilTimestamp = nodeInfo.operationalUntilTimestamp.add(noticePeriodForFeePayment.add(normalPeriodForFeePayment.mul(advancedPayMonths)));
        } else if (timeElapsed > normalPeriodForFeePayment) {
            nodeInfo.operationalUntilTimestamp = nodeInfo.operationalUntilTimestamp.add(normalPeriodForFeePayment.add(normalPeriodForFeePayment.mul(advancedPayMonths)));
        } else {
            nodeInfo.operationalUntilTimestamp = nodeInfo.operationalUntilTimestamp.add(normalPeriodForFeePayment.mul(advancedPayMonths));
        }
    ...
    {
        require(
            advancedPayMonths < 4 && advancedPayMonths >= 1,
            "Minimum of one month payment"
        );
        address user = msg.sender;
        uint256 usdPayAmount = _updateNodeFee(nodeId, user, advancedPayMonths);
        uint avaxPrice = getAvaxUsdLatestPrice();
        uint256 avaxPayAmount = usdPayAmount * 10 ** 18 /avaxPrice;
        require(msg.value >= avaxPayAmount, "Wrong payment");
        uint256 returnAmt = msg.value - avaxPayAmount;
        operationalAddress.transfer(avaxPayAmount);
        if (returnAmt > 0) {
    ...
    function payAllAvailableNodeFees() external payable {
        ...
        uint256 avaxPayAmount = usdPayAmount * 10 ** 18 /getAvaxUsdLatestPrice();
        require(msg.value >= avaxPayAmount, "Wrong payment");
        uint256 returnAmt = msg.value - avaxPayAmount;
        operationalAddress.transfer(avaxPayAmount);
        if (returnAmt > 0) {
    ...
    function getNodeClaimableDistribution(uint256 nodeId) public view returns (uint256)
    {
        ...
        return _getDistributionByNode(nodeInfo.lastClaimTimestamp);
    }

    function getNodeUnClaimableDistribution(uint256 nodeId) public view returns (uint256)
    {
        NodeInfo memory nodeInfo = nodes[nodeId];
        if (nodeInfo.isDeleted) return 0;
        if (nodeInfo.isDistributionAvailable && nodeInfo.nextAvailableClaimTimestamp <= block.timestamp) return 0;
        return _getDistributionByNode(nodeInfo.lastClaimTimestamp);
    }

    function _getDistributionByNode(uint256 lastTime) internal view returns (uint256)
    {
    ...
    function _claimNodeDistribution(uint256 nodeId, address user) internal returns (uint256)
    {
        NodeInfo storage nodeInfo = nodes[nodeId];
        console.log("next available claim time", nodeInfo.nextAvailableClaimTimestamp);
        ...
        uint256 reward = _getDistributionByNode(nodeInfo.lastClaimTimestamp);
        nodeInfo.lastClaimTimestamp = block.timestamp;
        nodeInfo.nextAvailableClaimTimestamp = block.timestamp + nodeClaimLimit;
        return reward;
    }

    function claimSpecificNodeDistribution(uint256 nodeId) public {
        IERC20Upgradeable(communityTokenAddress).transfer(msg.sender, _claimNodeDistribution(nodeId, msg.sender));
    }
    ...
    function claimAllDistributions() public {
        ...
        for (uint256 index = 0; index < nodeIds.length; index++) {
            ...
            reward += _claimNodeDistribution(nodeIds[index], user);
        }
        IERC20Upgradeable(communityTokenAddress).transfer(user, reward);
    }
    ...
    function compoundDistributions(uint256 nodeAmount) public {
        ...
        for (uint256 index = 0; index < nodeIds.length; index++) {
            reward += _claimNodeDistribution(nodeIds[index], user);
            if (reward >= compoundRequiredPayment) {
                break;
            }
        }
        if (reward < compoundRequiredPayment) revert();
        reward -= compoundRequiredPayment;
        _purchaseNode(nodeAmount, true);
        IERC20Upgradeable(communityTokenAddress).transfer(user, reward);
    }

    function convertPreSaleToNodes(address[] calldata users, uint256[] calldata nodeCounts) external onlyOwner
    {
        require(users.length == nodeCounts.length,"Missmatch");
        for (uint256 index = 0; index < users.length; index++) {
            _setupNewNodes(users[index], nodeCounts[index], 0, true);
        }
    }
    ...
    function distributeFounderNodes(address founder) external onlyOwner
    {
        FounderInfo storage founderInfo = founders[founder];
        require(founderInfo.isFounder == 1, "Not founder");
        require(founderInfo.isDistributed == 0, "Already distributed");
        _setupNewNodes(founder, nodeCap, 0, true);
        founderInfo.isDistributed = 1;
    }
