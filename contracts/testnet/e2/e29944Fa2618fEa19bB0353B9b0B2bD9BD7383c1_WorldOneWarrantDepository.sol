...
contract WorldOneWarrantDepository is Ownable {
    ...
    function deposit(
        ...
        warrantInfo[_depositor][_warrantId] = Warrant({
            payout: warrantInfo[_depositor][_warrantId].payout.add(_warrantAmount),
            warrantLotID: uint32(warrantLotIndex - 1),
            purchasedAt: uint32(block.timestamp),
            pricePaid: priceInUSD
        });
        _depositorWarrantIds[_depositor].push(_warrantId);
    ...
    function updateWarrantLot(
        ...
        if (warrantLotIndex > 0) {
            require(
                currentWarrantLot().consumed !=
                    currentWarrantLot().totalCapacity && _totalCapacity > currentWarrantLot().consumed,
                "Warrant lot already in progress"
            );
        }
    ...
    function redeem(address _recipient, uint256 warrantId_) external returns (uint256) {
        ...
        for(uint256 i = 0; i < _depositorWarrantIds[_recipient].length; i++) {
            if(_depositorWarrantIds[_recipient][i] == warrantId_){
                delete _depositorWarrantIds[_recipient][i]; //delete user warrant id
            }
        }
     ...
     *  @return bool
     */
    function canAvailableClaim(address _depositor, uint256 _warrantId) public view returns (bool) {
        Warrant memory info = warrantInfo[_depositor][_warrantId];
