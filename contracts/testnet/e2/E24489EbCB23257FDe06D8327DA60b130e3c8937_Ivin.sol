...
interface IExecutable {
    ...
    function setMaxTokensPerWallet(uint256 _amount) external;
    function unpause() external;
    function withdrawTokens() external;
}

contract Ivin is Ownable {
    ...
    function setMaxTokensPerWallet(
      ...
      uint256 _amount
        ...
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.setMaxTokensPerWallet(_amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function unpause(
