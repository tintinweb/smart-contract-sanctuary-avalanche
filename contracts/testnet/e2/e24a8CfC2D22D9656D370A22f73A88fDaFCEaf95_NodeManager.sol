...
contract NodeManager is Ownable, Pausable {
   ...
   function returnSoonestDue(address _account) internal returns(uint ){
    ...
    function compileVars(address _account,uint256[3] memory tier) external onlyGuard whenNotPaused returns (uint){
    ...
    function cashoutNodeReward(address _account, uint i, uint256 rewards) external onlyGuard onlyNodeOwner(_account) whenNotPaused {
        NodeEntity[] storage _nodes = _nodesOfUser[_account];
        NodeEntity storage _node = _nodes[i];
