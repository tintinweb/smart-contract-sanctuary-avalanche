    ...
    function withdraw(uint256 vaultNumber) public { //A chaque fois verifier que le vault existe et est applicable
        ...
        if(userInfo[vaultNumber][msg.sender].withdrawTime > block.timestamp){     
            ...
            userInfo[vaultNumber][msg.sender].amountStaked = 666;
             uint256 reward =  toWithdraw * vaultInfo[vaultNumber].classicYieldOnDuration*100 * (1+(vaultInfo[vaultNumber].additionnalReward/100)) ; // Calculate how much to send to user //Ajouter l'apy du titano
             if (reward > 0) {
