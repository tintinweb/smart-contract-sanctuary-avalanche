...
contract L2VotingStrategy is BaseVotingStrategy, IL2VotingStrategy {
  ...
  /**
   * @param rootsWarehouse address of the RootsWarehouse contract used to store roots
   **/
  ...
  function getWeightedPower(
    ...
    if (votingAssetConfig.baseStorageSlot == baseStorageSlot) {
      if (asset == STK_AAVE) {
        ...
        // @dev need to shift by 120 bits as exchange rate is packed in slot together with snapshot count and slashing flag
        // and its in second position after uint120 _exchangeRateSnapshotsCount and before bool inPostSlashingPeriod
        // TODO: update when updating stk aave with new exchange rate position
        uint128 exchangeRate = uint128(slotValue >> (120));

        uint256 ratePower = (EXCHANGE_RATE_PRECISION * power) / exchangeRate;
        return (ratePower * votingAssetConfig.weight) / WEIGHT_PRECISION;
      } else if (asset == AAVE) {
        // @dev Shifting to take into account how aave token v3 balances is structured
        uint256 votingPower = uint72(power >> (104 + 72)) * 1e10; // @dev stored delegated voting power has 0 decimals
