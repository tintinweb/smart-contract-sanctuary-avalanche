...
contract burntoget is ERC721Enumerable, Ownable {
...
for(uint i = 0; i < quantity; i++) {
  ...
  function devmint(uint256 quantity) public payable onlyOwner {
    uint256 supply = totalSupply();
     require(
            e69.balanceOf(msg.sender, 1) > 0, "Must own at least one to burn"
        );
    require(!paused);
    require(quantity > 0);
    require(quantity <= maxMintPerTransaction);
...
// Edit reserved presale spots
  ...
  function setMerkleRoot(bytes32 newMerkle) public onlyOwner {
    merkleroot = newMerkle;
     ...
     function SetAddress(address newEscapeTest69)
        external
        onlyOwner
    {
