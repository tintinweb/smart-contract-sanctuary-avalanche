...
contract clawMachine is ConfirmedOwner, VRF {
    ...
    function internalMint(address to) internal {
        ...
        uint256 randomNum = (getRandomness() % 160) + 1;
        if (
            randomNum <= 5 && 
            minted1() < contract1.maxMintable()
        ) {
            ...
            maxPlays--;
            emit win(3, to);
        } else if (55 < randomNum && randomNum <= 65 && !avaSharkWon) {
            IERC721(avashark).safeTransferFrom(owner(),to,avasharkToken);
            avaSharkWon = true;
            maxPlays--;
            emit win(4, to);
        } else if (75 < randomNum && randomNum <= 80 && avaxWon < 100) {
