...
*/
contract Glamour is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Supply used to launch in LP
    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 2010 ether;
    ...
    /**
     * @notice Constructs the  Glamour ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20(" Glamour", " Glamour") {
        // Mints 1  Glamour to contract creator for initial pool setup
    ...
    function _getGlamourPrice() internal view returns (uint256 _GlamourPrice) {
        try IOracle(GlamourOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Glamour: failed to fetch  Glamour price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _GlamourPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_GlamourPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGlamourOracle(address _GlamourOracle) public onlyOperatorOrTaxOffice {
        require(_GlamourOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints  Glamour to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of  Glamour to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGlamourPrice = _getGlamourPrice();
            currentTaxRate = _updateTaxRate(currentGlamourPrice);
            if (currentGlamourPrice < burnThreshold) {
                burnTax = true;
            }
        }
