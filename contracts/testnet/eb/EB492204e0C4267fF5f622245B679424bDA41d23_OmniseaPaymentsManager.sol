...
contract OmniseaPaymentsManager is IOmniseaPaymentsManager, ReentrancyGuard {
    ...
    function onPayment(address _payee, uint256 _paid) external override {
        require(_paid > 0, "!paid");
        paid[msg.sender] += _paid;
        paidBy[msg.sender][_payee] += _paid;
    }

    function payout(address _recipient, uint256 _endTime) external payable override nonReentrant {
        require(block.timestamp >= _endTime + 14 days);
        require(!flagged[msg.sender]);
        uint256 feeAmount;

        if (_fee > 0) {
            feeAmount = (msg.value * _fee) / 100;

            (bool p1,) = payable(_feeManager).call{value : feeAmount}("");
            require(p1);
        }
        (bool p2,) = payable(_recipient).call{value : (msg.value - feeAmount)}("");
        require(p2);
    }
