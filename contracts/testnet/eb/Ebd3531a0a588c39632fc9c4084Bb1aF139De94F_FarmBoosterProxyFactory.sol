...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _tokenToken: the address of the token token
     */
    function initialize(
        address _admin,
    ...
    function harvestToken() internal {
        uint256 tokenBalance = tokenToken.balanceOf(address(this));
        if (tokenBalance > 0) {
            tokenToken.safeTransfer(msg.sender, tokenBalance);
        }
    }
}
...
contract FarmBoosterProxyFactory {
    ...
    address public immutable tokenToken;
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, tokenToken);
