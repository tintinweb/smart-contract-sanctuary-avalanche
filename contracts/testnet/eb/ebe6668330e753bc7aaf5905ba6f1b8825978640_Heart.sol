...
// File contracts/interfaces/IPangolinFactory.sol
...
interface INodeManager {
    ...
    function createNode(address account, address receiver, uint256 amount, bool withRewards) external;
    function getAllNodesRewards(address account) external view returns (uint256);
    function getFirstNodeValue(address account) external view returns (uint256);
    ...
    function getAllNodesCount(address account) external returns (uint256);
    function cashoutNodeReward(address account) external;
}
...
contract Heart is ERC20, Ownable, PaymentSplitter {
    ...
    event createDebugEvent(
        address indexed orgSenderAddress,
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Heart Nodes::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "Heart Nodes::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
    ...
    function updatNodePrice(uint256 newVal) external onlyOwner {
        nodePrice = newVal;
    }
    ...
    function updateSellTax(uint16 value) external onlyOwner {
        require(value <= 1000, "Sell Tax too high");
        sellTax = value;
    }
    ...
    function getNodePrice() public view returns (uint256) {
        return nodeManager.getNodePrice();
    }

    function getFirstNodeTotalValue(address account) external view returns (uint256){
        return nodeManager.getFirstNodeValue(account);
    }
    ...
    function _transfer(
        ...
        if (from == joePair && buyTax != 0) {
            ...
            require(nodeManager.isNodeOwner(from), "Create at least one Node before selling!");
            uint256 taxAmount = amount * sellTax/10000;
            if (nodeManager.isNodeOwner(from)){
                uint256 nodeCount = nodeManager.getNodeNumberOf(from);

                //Sell tax is based off how many nodes you have
                if (nodeCount == 1){
    ...
    //Takes in a whole number of BEAT tokens, splits in half to AVAX, keeps the other half in BEAT
    //Adds both sides to the LP and then emits the SwapAndLiquify event
    ...
    function createNodeWithTokens(address receiver, bool withRewards) external {
        address sender = _msgSender();
        // console.log("createNodeWithTokens() - Are they a node owner? %s", nodeManager.isNodeOwner(sender));
        // emit createDebugEvent(_msgSender(), sender, receiver, withRewards);
        // uint256 nodePrice = nodeManager.getNodePrice();
        // console.log("createNodeWithTokens() - Balance of sender %s is %s", sender, balanceOf(sender));
        // console.log("createNodeWithTokens() - withRewards %s", withRewards);


        //Check if this creation is using external funds or if the user's existing rewards will be used
        // If they are using rewards we have to 
        // if (withRewards && sender == receiver){
        //     //don't need the hack i think. it doesn't matter  if the sender tries to use their rewards and create
        //     //for someone else. their loss.
        //     // weird hack to make sure that when creating a node with rewards that the sender is also the receiver.
        //     // sender = receiver;
        //     require(
        //         nodeManager.getAllNodesRewards(sender) >= nodePrice,
        //         "NoCr:Not enough rewards"
        //     );
        //     canCreate = true;
            
        //     // console.log("createNodeWithTokens() - Sender %s", sender);
        // } else if (!withRewards){
        //     require(
        //         balanceOf(sender) >= nodePrice,
        //         "NoCr:Balance too low"
        //     );
        //     canCreate = true;
        // }
        ...
        //If they are creating for themself, check first if they own a node already 
        //This is a project requirement
        if (sender == receiver) {
            require(nodeManager.isNodeOwner(sender), "Cannot create nodes for self");
        }
        
        require(
            sender != address(0),
            "NODE CREATION: creation tried from the zero address"
        );
        require(!isBlacklisted[sender], "NODE CREATION: Blacklisted address");
        require(
            sender != teamPool && sender != rewardsPool,
            "NODE CREATION: teamPool address and rewardsPool address cannot create nodes"
        );
        
        ///#@!#[email protected]#[email protected]#[email protected]#!#@ removed the '!' from `swapping`
        ///Defaults
        ///swapLiquifiyEnabled = true
        ///swapping = false
        ...
        if (
            ...
            swapping = true;
            //Actually transfer the tokens from the sender to the contract address
            super._transfer(sender, address(this), nodePrice);

            uint256 teamTokens = nodePrice
                .mul(teamPoolFee)
                .div(100);

            swapAndSendToFee(teamPool, teamTokens);

            uint256 rewardsPoolTokens = nodePrice
                .mul(rewardsFee)
                .div(100);

            //Part of the rewards pool will be swapped into AVAX here
            uint256 rewardsTokenstoSwap = rewardsPoolTokens.mul(rwSwap).div(
                100
            );
            
            //Swap the $BEAT token for AVAX before sending to the rewardsPools
            swapAndSendToFee(rewardsPool, rewardsTokenstoSwap);

            //Transfer the reward tokens that were not swapped now over to the rewards pool
            super._transfer(
                address(this),
                rewardsPool,
            ...
            //The amount to be put into the LP (needs 50/50 split between the tokens)
            uint256 swapTokens = nodePrice.mul(liquidityPoolFee).div(
                100
            );
            
            //Take the LP fee amount of tokens and add to the LP 
            swapAndLiquify(swapTokens);

            //Take ALL tokens left inside the contract wallet and swap to AVAX
            //WHY?!?! Gas fees later?
        ...
        //All swapping/fee taking is done. Now create node
        require(canCreate, "NCC: Not permitted to create nodes");
        nodeManager.createNode(sender, receiver, nodePrice, withRewards);
        canCreate = false;
    }

    // 

    function getNodeCount(address account) external returns (uint256) {
        uint256 count = nodeManager.getAllNodesCount(account);
        return count;
    }

    function getAllNodesRewards(address account) public view returns (uint256) {
        uint256 pendingRewards = nodeManager.getAllNodesRewards(account);
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        require(
            rewardAmount > 0,
            "CASHOUT: Not enough rewards to cashout (not > 0)"
        );

        /////!!!!!!!!!!!*[email protected]*!*@#*[email protected]*#[email protected]#* Remove 'swapping'.
        // TODO
        if (swapLiquifyEnabled && swapping) {
            uint256 feeAmount;
            if (cashoutFee > 0) {
        ...
        console.log("cashoutReward() - cashoutReward sender address %s", sender);
        // nodeManager.delegatecall(abi.encodeWithSignature("cashoutNodeReward(address account, uint256 _creationTime)", sender, blocktime));
        nodeManager.cashoutNodeReward(sender);
        totalClaimed += rewardAmount;
        emit Cashout(sender, rewardAmount);
    ...
    //     //calculate how much the reward is IN THIS MOMENT
    //     uint256 rewardAmount = nodeManager.getAllNodesRewards(sender);
    //     require(
    //         rewardAmount > 0,
    //         "CASHOUT:8"
    ...
    //     //Take the house cut off of the reward 
    ...
    function createNewNodeWithRewards() external {
        address sender = _msgSender();
        console.log("createNewNodeWithRewards() - sender address: %s", sender);
        require(
            sender != address(0),
            "COMP:1"
        );
        ...
        uint256 totalNodeRewardsPending = nodeManager.getAllNodesRewards(sender);
        require(
            totalNodeRewardsPending > 10,
            "CNN: Reward amount not enough to create new node"
        );
        ...
        if (
            ...
            uint256 feeAmount;
            if (cashoutFee > 0) {
                feeAmount = totalNodeRewardsPending.mul(cashoutFee).div(100);
                swapAndSendToFee(rewardsPool, feeAmount);
            }
            totalNodeRewardsPending -= feeAmount;
            super._transfer(rewardsPool, sender, totalNodeRewardsPending);
        }
        //name param stays empty. we don't care about a name when a node already exists
        this.createNodeWithTokens(sender, true);
