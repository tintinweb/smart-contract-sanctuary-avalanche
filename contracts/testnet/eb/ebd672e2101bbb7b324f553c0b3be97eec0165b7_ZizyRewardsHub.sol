...
// File contracts/competition/ZizyRewardsHub.sol
...
contract ZizyRewardsHub is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
    ...
    event RewardClaimed(RewardType rewardType, address rewardAddress, uint amount, uint tokenId);
    event RewardClaimedOnDiffChain(RewardType rewardType, address rewardAddress, uint amount, uint tokenId);
    ...
    function claimAirdropReward(uint airdropId_) external nonReentrant {
        ...
        if (rew.chainId != chainId()) {
            ...
            emit RewardClaimedOnDiffChain(rew.rewardType, rew.rewardAddress, rew.amount, rew.tokenId);
        } else {
            ...
            emit RewardClaimed(rew.rewardType, rew.rewardAddress, rew.amount, rew.tokenId);
        }
    }
