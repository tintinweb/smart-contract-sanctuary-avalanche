...
contract LiquidationHelper is LiquidationCallback {
    ...
    function getLiquidationBonus(uint256[] calldata _auctionIds) external view returns (int256[] memory bonuses) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            uint256 potentialOutcome = _quoteToUsp(address(token), collateralAmount, isLpToken);
            bonuses[i] = int256(potentialOutcome) - int256(uint256(uspAmount));
        }
    }

    function _quoteToUsp(
        address _token,
        uint256 _amount,
        bool _isLpToken
    ) internal view returns (uint256 potentialOutcome) {
        if (_isLpToken) {
            // TODO: shall we use `withdrawFromOtherAsset`?
            address underlyingToken = IAsset((_token)).underlyingToken();
            (uint256 withdrewAmount, , ) = mainPool.quotePotentialWithdraw(underlyingToken, _amount);
            try mainPool.quotePotentialSwap(underlyingToken, address(usp), withdrewAmount) returns (
                uint256 potentialOutcome_,
                uint256
            ) {
                potentialOutcome = potentialOutcome_;
            } catch (bytes memory) {}
        } else {
            try mainPool.quotePotentialSwap((_token), address(usp), _amount) returns (
                uint256 potentialOutcome_,
                uint256
            ) {
                potentialOutcome = potentialOutcome_;
