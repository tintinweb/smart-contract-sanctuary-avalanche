...
// Note that this pool has no minter key of champ (rewards).
// Instead, the governance will call champ distributeReward method and send reward to this pool at the beginning.
contract RebateAndRewardPool is ContractGuard, Operator {
    ...
    struct PoolTokenInfo {
        ...
        uint256 lastRewardTime; // Last time that champ distribution occurs.
        uint256 accChampPerShare; // Accumulated champ per share, times 1e18. See below.
    ...
    // The time when champ mining starts and ends.
    ...
    uint256 public immutable champPerSecond;
    ...
    constructor(
        ...
        if (_champ != address(0)) champ = IERC20(_champ);
        poolStartTime = _poolStartTime;
        poolEndTime = poolStartTime.add(runningTime);

        champPerSecond = TOTAL_REWARDS.div(runningTime);
    }
    ...
    // Update the given pool's champ allocation point. Can only be called by the owner.
    function set(
        uint256 _pid, 
        uint256 _allocPoint,
        uint256 _vestingDuration
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(champPerSecond);
            return poolEndTime.sub(_fromTime).mul(champPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(champPerSecond);
            return _toTime.sub(_fromTime).mul(champPerSecond);
        }
    }
    ...
    // View function to see pending Champ on frontend.
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        PoolTokenInfo memory poolToken = poolTokens[address(pool.token)];

        uint256 accChampPerShare = poolToken.accChampPerShare;
        ...
        if (block.timestamp > poolToken.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _champReward = _generatedReward.mul(poolToken.allocPoint).div(totalAllocPoint);
            accChampPerShare = accChampPerShare.add(_champReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accChampPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            ...
            //Add (rewards/tokensInPool) to the accumulated Champ per Share. Based in ether - hence .div(1e18) later on.
            poolToken.accChampPerShare = poolToken.accChampPerShare.add(_champReward.mul(1e18).div(tokenSupply));
        }
        poolToken.lastRewardTime = block.timestamp;
    }
    ...
    function _deposit(
        ...
        uint256 _pending = user.amount.mul(poolToken.accChampPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeChampTransfer(_recipient, _pending);
        ...
        user.rewardDebt = user.amount.mul(poolToken.accChampPerShare).div(1e18);
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address _sender) internal onlyOneBlock {
        ...
        uint256 _pending = user.amount.mul(poolToken.accChampPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeChampTransfer(_sender, _pending);
            emit RewardPaid(_sender, _pending);
        }
        
        //Handle withdrawal vesting logic.
        _handleWithdrawVesting(_pid, _amount, _sender);

        emit Withdraw(_sender, _pid, _amount);
    }

    function _handleWithdrawVesting(
        uint256 _pid, 
        uint256 _amount, 
        address _sender
    ) internal {
    ...
    // Safe champ transfer function, just in case if rounding error causes pool to not have enough champ.
    function safeChampTransfer(address _to, uint256 _amount) internal {
        uint256 _champBal = champ.balanceOf(address(this));
        if (_champBal > 0) {
            if (_amount > _champBal) {
                champ.safeTransfer(_to, _champBal);
            } else {
                champ.safeTransfer(_to, _amount);
            }
        }
    }
