    ...
    */
    ...
    function _finalization(uint256[] memory _noOfTokens,address[] memory _propertyOwners,bytes[] memory _certificates) internal override{
        ...
        if (goalReached()) {
            ...
            _securityToken.mintDevsTokensToPropertyOwners(_noOfTokens,_propertyOwners,_certificates);
    ...
    function getRemainingTokensBeforeFinalized() public view returns (uint256 _remainingTokens) {
        if (goalReached()) {
            IERC1400RawERC20 _securityToken = IERC1400RawERC20(
                address(token())
            );
...
abstract contract FinalizableCrowdsale is TimedCrowdsale {
     ...
     */
    function finalize(uint256[] memory _noOfTokens,address[] memory _propertyOwners ,bytes[] memory _certificates) public {
        require(!_finalized, "FinalizableCrowdsale: already finalized");
        require(hasClosed(), "FinalizableCrowdsale: not closed");

        _finalized = true;

       _finalization(_noOfTokens,_propertyOwners,_certificates);
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    function changeSTableCoinAddress(IERC20 stableCoin) public onlyAdmin {
        _stableCoin = stableCoin;
    }
    ...
    function changePropertyAccountAddress(address payable newOwner)
        public
        onlyAdmin
    {
        _wallet = newOwner;
     ...
     */
    function changeSTORate(uint256 rate) internal returns (uint256) {
        _rate = rate;
        return _rate;
    }
}
