...
//function updatePrice(uint newPrice) public returns (uint) {
  //      price = newPrice;
    //    return price;
    //}
    
function buy() public payable whenNotPaused returns (bool){
        require(_msgSender() != address(0), "Null address cant buy token");
        //calculate tokens value against receiving amount 
        uint _tokenValue = msg.value*(price);
        require(_tokenValue <= tokenForSale(), "remaing token less than buying value");
        ...
        //fire token purchased event
        emit tokenPurchased(_msgSender(), price, _tokenValue);

        //return true;

         //update price
        uint newPrice = tokenForSale()/400000000000000000000;
