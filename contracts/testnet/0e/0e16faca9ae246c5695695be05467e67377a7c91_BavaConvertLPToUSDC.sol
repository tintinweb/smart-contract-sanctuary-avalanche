...
import "./interface/IDMMRouter.sol";
...
contract BavaConvertLPToUSDC is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event LiquidateLP(IERC20Upgradeable indexed lpToken, address indexed account, uint256 lpAmount, uint256 USDAmount);
    ...
    function setWavaxAllowance(address _router, uint256 _amount) external onlyOwner {
        require(_router != address(0),"A!=0");
        WAVAX.approve(_router, _amount);
    }
    ...
    function setlpTokenAllowancesRouter(uint256 lpTokenId, uint256 _amount) external onlyOwner {   
        ...
        if (address(lpTokenPair.router) != address(0)) {
            IERC20Upgradeable(IPair(address(lpTokenPair.lpToken)).token0()).approve(address(lpTokenPair.router), _amount);
            IERC20Upgradeable(IPair(address(lpTokenPair.lpToken)).token1()).approve(address(lpTokenPair.router), _amount);
            IERC20Upgradeable(address(lpTokenPair.lpToken)).approve(address(lpTokenPair.router), _amount);
        }
    }

    function addLPToken(IERC20Upgradeable _lpToken , IRouter _router) external onlyOwner {        
        require(address(_lpToken) != address(0), "0Addr");
        require(address(_router) != address(0), "0Addr2");
    ...
    function _liquidateLpToken(uint256 lpTokenId) private {
        ...
        uint256 lpAmount = IERC20Upgradeable(lpTokenPair.lpToken).balanceOf(address(this));
        if(lpAmount > 0) {
            ...
            (uint amountA, uint amountB) = lpTokenPair.router.removeLiquidity(IPair(address(lpTokenPair.lpToken)).token0(), IPair(address(lpTokenPair.lpToken)).token1(), lpAmount, amount0, amount1, address(this), block.timestamp+600);

            uint liquidateAmountA = _convertTokentoUSDCE(lpTokenId, amountA, 0);
            uint liquidateAmountB = _convertTokentoUSDCE(lpTokenId, amountB, 1);
    ...
    function _convertTokentoUSDCE(uint256 lpTokenId, uint amount, uint token) private returns (uint) {
        LPTokenInfo memory lpTokenPair = lpTokenInfo[lpTokenId];
        address oriToken;
        if (token == 0) {
        ...
        if (oriToken == address(USDCE)) {
            ...
            amountUSDCE = _convertExactTokentoToken(lpTokenId, path, amount);
        }
        return amountUSDCE;
    }
