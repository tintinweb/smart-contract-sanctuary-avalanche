...
contract AaveV3Adapter is AdapterBase, Basic, Ownable {
        ...
        onlyAdapterManager
    {
        ...
        IPoolV3 aave = IPoolV3(aaveV3PoolAddr);
    ...
    function borrow(bytes calldata encodedData) external onlyProxy {
        (address token, uint256 amount, address account, uint256 rateMode) = abi
        ...
        if (token == avaxAddr) {
            ...
            IPoolV3 aave = IPoolV3(aaveV3PoolAddr);
            aave.borrow(token, amount, rateMode, referralCode, account);
        }
    }

    function borrowTokens(bytes calldata encodedData) external onlyProxy {
        (address account, address[] memory token, uint256[] memory amount) = abi
            .decode(encodedData, (address, address[], uint256[]));

        IPoolV3 aave = IPoolV3(aaveV3PoolAddr);
        uint256 rateMode = 2;
        for (uint256 i = 0; i < token.length; i++) {
            aave.borrow(token[i], amount[i], rateMode, referralCode, account);
        }
    }
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {IScaledBalanceTokenV3} from "./IScaledBalanceTokenV3.sol";
 ...
 **/
interface IATokenV3 is IERC20, IScaledBalanceTokenV3 {
    /**
...
import {IPoolAddressesProviderV3} from "./IPoolAddressesProviderV3.sol";
import {DataTypes} from "./library/types/DataTypesV3.sol";
 ...
 **/
interface IScaledBalanceTokenV3 {
    /**
