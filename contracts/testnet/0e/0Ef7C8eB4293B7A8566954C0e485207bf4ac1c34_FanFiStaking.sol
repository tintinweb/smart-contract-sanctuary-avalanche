...
contract FanFiStaking is Ownable {
    ...
    constructor(address _tokenAddress) Ownable() {
        ...
        _setApyScheme(1, 2500, 30, 100000, 333333333, 6944444);
        _setApyScheme(2, 3500, 60, 50000, 142857143, 4166666);
        _setApyScheme(3, 6500, 90, 10000, 51282051, 2777777);
    }
    ...
    function _setApyScheme(
        ...
        if (!apySchemes[_schemeID].exist) {
            ...
            apySchemes[_schemeID].lockingPeriod = _lockingPeriod * 1 days;
    ...
    function stake(uint256 _amount, uint256 _schemeID) external {
        ...
        users[msg.sender].totalStakePerScheme[_schemeID] += _amount;
        users[msg.sender].stakes[_schemeID].push(newStake);
    ...
    function unstake(uint256 _schemeID, uint256 _stakeIndex) external {
        APYScheme memory scheme = apySchemes[_schemeID];
        require(scheme.exist, "FanFiStake: scheme doesn't exist");
        StakeInfo[] memory stakes = users[msg.sender].stakes[_schemeID];
        require(
        ...
        if (
            ...
            uint256 userRewards = (stakeInfo.stakeAmount *
                scheme.stakingYield) /
                (PERCENTAGE_DENOMINATOR * 365 * stakedDays);
            stakeInfo.rewards = userRewards;
        } else stakeInfo.status = StakeStatus.UNSTAKED;

        users[msg.sender].totalStakePerScheme[_schemeID] -= stakeInfo
            .stakeAmount;
        users[msg.sender].stakes[_schemeID][_stakeIndex] = stakeInfo;

        apySchemes[_schemeID].currentStake -= stakeInfo.stakeAmount;
        ...
        emit Unstake(msg.sender, stakeInfo.stakeAmount, _schemeID);
    }

    function redeem(uint256 _schemeID, uint256 _stakeIndex) external {
        APYScheme memory scheme = apySchemes[_schemeID];
        require(scheme.exist, "FanFiStake: scheme doesn't exist");
        StakeInfo[] memory stakes = users[msg.sender].stakes[_schemeID];
        require(
        ...
        require(
            block.timestamp - stakeInfo.unstakeTimestamp >= 48 hours,
            "FanFiStake: wait 48 hours after unstake"
        );
        require(
        ...
        apySchemes[_schemeID].currentYield -= stakeInfo.rewards;
        ...
        emit Redeem(msg.sender, stakeInfo.rewards, _schemeID);
    }
    ...
    function getUserStakesInfo(address _userAddress, uint256 _schemeID)
        external
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        return (
            users[_userAddress].stakes[_schemeID].length,
            users[_userAddress].totalStakePerScheme[_schemeID],
            users[_userAddress].stakes[_schemeID]
        );
    }
    ...
    function getUserStakeInfo(
        ...
        require(
            users[_userAddress].stakes[_schemeID].length > _stakeIndex,
            "FanFiStake: stake index doesn't exist"
        );

        return users[_userAddress].stakes[_schemeID][_stakeIndex];
    }
    ...
    function getUserReward(
        ...
        APYScheme memory scheme = apySchemes[_schemeID];
        require(scheme.exist, "FanFiStake: scheme doesn't exist");
        StakeInfo[] memory stakes = users[_userAddress].stakes[_schemeID];
        require(
            stakes.length > _stakeIndex,
            "FanFiStake: stake index doesn't exist"
        );
        ...
        if (stakeInfo.status == StakeStatus.ACTIVE) {
            uint256 stakedDays = (block.timestamp - stakeInfo.stakeTimestamp) /
                1 days;
            userRewards =
                (stakeInfo.stakeAmount * scheme.stakingYield) /
                (PERCENTAGE_DENOMINATOR * 365 * stakedDays);
