...
contract Custodian is StorageStateful, Ownable, ICustodian {
    ...
    event MsgSender(address indexed sender); //Ong
    ...
    function lock(
        ...
        uint256 _value,
        bytes32 _memo
    ) external onlyWhitelistedToken(_typeid) {
        (, , address tokenAddress, ) = _storage.search(_typeid);
        IToken token = IToken(tokenAddress);
        require(msg.sender == _from, "Token: lock from incorrect user");
        require(
    ...
    function unlock(
        ...
        address _to,
        uint256 _value
    ) external onlyWhitelistedToken(_typeid) onlyOwner {
        (, , address tokenAddress, ) = _storage.search(_typeid);
        IToken token = IToken(tokenAddress);
    ...
    function getAsset(uint256 _typeid)
        external
        view
        returns (
            uint256,
        ...
        )
    {
        (
            ...
            string memory partnerIssuer
        ) = _storage.search(_typeid);
    ...
    function getLockedFunds(address _tokenAddress)
        external
        view
        returns (uint256)
    {
    ...
    function getTypeIds() external view returns (uint256[] memory assetIds) {
        assetIds = _storage.getAssetIds();
    }
    //Ong End
}
