...
// File: contracts/ERC1155MTradable.sol
 ...
 */
contract ERC1155MTradable is ERC1155, Ownable {
...
error IsNotOperator(address senderAddress);
error NotOwner();
error PriceMustBeAboveZero();
error PriceNotMet(address nftAddress, uint256 tokenId, uint256 price);
error ItemNotForSale(address nftAddress, uint256 tokenId);
error NotListed(address nftAddress, uint256 tokenId);
error NotPlatformCreated(address nftAddress, uint256 tokenId);

contract SpectrumMarketPlace is
    ERC1155Receiver,
    IERC721Receiver,
    ReentrancyGuard,
    Ownable
{
    ...
    address private _serviceWallet = owner();
    
    ERC1155MTradable mkNFT;


    uint256 public serviceFee = 25 * 10**17; // 2.5 %
    uint256 constant ROYALTY_MAX = 100 * 10**18; // 10%

    constructor(address _nftAddress, address _operator) {
        mkNFTaddress = _nftAddress;
        operator = _operator;
        mkNFT = ERC1155MTradable(_nftAddress);
    }
    ...
    struct CreateOffering {
        uint nonce;
        bytes32 generatedABI; // Remove after testing
    ...
    event OfferingCancelled(uint indexed nonce);
    event OfferingClosed(uint indexed nonce, address indexed buyer);
    /**  EVENTS START */
    ...
    modifier onlyOperator() {
        if (msg.sender != operator) {
            revert IsNotOperator(msg.sender);
        }
        _;
    }

    /**  MODIFIERS END */
    function updateServiceFee(uint256 _listPrice) external onlyOwner {
    ...
    function createListedToken(uint256 tokenId) private {
        ...
        idToListedToken[tokenId] = CreateToken(
            mkNFTaddress,
            tokenId,
            payable(msg.sender),
            0,
    ...
    function listItem(
        ...
        if (!createdByMarketpalce) {
            ...
            idToListedToken[tokenId].RoyaltyAddress = royaltyAddress;
            idToListedToken[tokenId].RoyaltyPercentage = royaltyPercentage;
            idToListedToken[tokenId].startDate = startDate;
            idToListedToken[tokenId].endDate = endDate;
        }

        emit ItemListed(msg.sender, nftAddress, tokenId, price);
    }

    function executeSale(address nftAddress, uint256 tokenId)
        external
        payable
    ...
    {
        require(
            block.number < idToListedToken[tokenId].endDate,
            "Listing has ended"
        );
        if (msg.value < idToListedToken[tokenId].price) {
            revert PriceNotMet(
                nftAddress,
                tokenId,
        ...
        if (serviceFee > 0 && _serviceWallet != address(0)) {
            fee = msg.value * serviceFee / ROYALTY_MAX;
            userReceipt += fee;
            (bool success, ) = payable(_serviceWallet).call{value: fee}("");
            require(success, "Transfer failed.");
        }

        if(idToListedToken[tokenId].RoyaltyPercentage > 0 && idToListedToken[tokenId].RoyaltyAddress != address(0)){
            fee = msg.value * idToListedToken[tokenId].RoyaltyPercentage / ROYALTY_MAX;
            if(fee > 0) {
                userReceipt += fee;
                (bool isRoyaltySent, ) = payable(idToListedToken[tokenId].RoyaltyAddress).call{value: fee}("");
        ...
        if(userReceipt > 0) {
            (bool isSuccess, ) = payable(idToListedToken[tokenId].owner).call{value: userReceipt}("");
            require(isSuccess, "Transfer failed.");
        }

        // Removing all existed offers and refunding money
        for (uint i = 0; i < _offeringIds.current(); i++) {
            if (offerersData[i + 1].nftAddress == nftAddress && offerersData[i + 1].tokenId == tokenId) {
                uint currentId = i + 1;
                CreateOffering storage currentItem = offerersData[currentId];
                payable(offeringRegistry[currentItem.generatedABI].offerer).transfer(offeringRegistry[currentItem.generatedABI].bidPrice);
                delete (offeringRegistry[currentItem.generatedABI]);
            }
        }

        // TODO: Delete ListedTokenItem from memory after back end implementation
        idToListedToken[tokenId].owner = address(msg.sender);
        idToListedToken[tokenId].price = 0;
        idToListedToken[tokenId].currentlyListed = false;
        idToListedToken[tokenId].startDate = 0;
        idToListedToken[tokenId].endDate = 0;

        emit ItemBought(msg.sender,nftAddress,tokenId,idToListedToken[tokenId].price);
    }

    function operatorCancellation(address nftAddress, uint256 tokenId)
        external
        isListed(nftAddress, tokenId)
        onlyOperator
        nonReentrant
    {
        // To Do: Change logic to removing data from memory after back end sync
        idToListedToken[tokenId].price = 0;
        idToListedToken[tokenId].currentlyListed = false;
    ...
    function getUserOfferings(address _userAddress) public view returns (CreateOffering[] memory) {
        ...
        for (uint i = 0; i < totalOffersCount; i++) {
            if (offerersData[i + 1].offerer == _userAddress) {
                itemCount += 1;
            }
        }
    ...
    function cancelOffer(uint _nonce) external nonReentrant {
        require(msg.sender == offerersData[_nonce].offerer, "You are not allowed to cancel the offer");
        payable(offerersData[_nonce].offerer).transfer(offerersData[_nonce].bidPrice);
        delete (offerersData[_nonce]);
        emit OfferingCancelled(_nonce);
    }


    function operatorCancelOffer(uint _nonce) external onlyOperator nonReentrant {
        payable(offerersData[_nonce].offerer).transfer(offerersData[_nonce].bidPrice);
        delete (offerersData[_nonce]);
        emit OfferingCancelled(_nonce);
    }
    ...
    function acceptOffer(uint _nonce, address _token) external nonReentrant {
        ...
        require(block.number < currentOffer.endBlock, "Offer Time Exceeds");
        require(!currentOffer.closed, "Offer cancelled");
        require(
        ...
        IERC1155 nft = IERC1155(currentOffer.nftAddress);
        if (nft.supportsInterface(0xd9b67a26) == true) {
             require(
                nft.balanceOf(msg.sender, currentOffer.tokenId) != 0,
        ...
        if (serviceFee > 0 && _serviceWallet != address(0)) {
            ...
            userReceipt += fee;
            token.transferFrom(payable(currentOffer.offerer), _serviceWallet, fee);
        }

        if(idToListedToken[currentOffer.tokenId].RoyaltyPercentage > 0 && idToListedToken[currentOffer.tokenId].RoyaltyAddress != address(0)){
            fee = currentOffer.bidPrice * idToListedToken[currentOffer.tokenId].RoyaltyPercentage / ROYALTY_MAX;
            if(fee > 0) {
                userReceipt += fee;
                token.transferFrom(payable(currentOffer.offerer), payable(idToListedToken[currentOffer.tokenId].RoyaltyAddress), fee);
            }
        }

        require(currentOffer.bidPrice >= userReceipt, 'invalid royalty or service fee');
        userReceipt = currentOffer.bidPrice - userReceipt;

        if(userReceipt > 0) {
            token.transferFrom(payable(currentOffer.offerer), payable(msg.sender), userReceipt);
        }

        // TODO: After back-end implementation Delete from memory after transfer 
        idToListedToken[currentOffer.tokenId].owner = address(msg.sender);
        idToListedToken[currentOffer.tokenId].price = 0;
        idToListedToken[currentOffer.tokenId].currentlyListed = false;
        idToListedToken[currentOffer.tokenId].startDate = 0;
        idToListedToken[currentOffer.tokenId].endDate = 0;
        delete (offerersData[_nonce]);
