...
// File: contracts/talent-scout.sol
...
interface IVoting{
 function weeklyvote() external returns(uint256);
}
...
interface ITalentAwardDistributor {
    function setShare(address shareholder, uint256 amount) external;
    function depositRewardAmount(uint256 _amount) external;
}
...
contract TalentScout is IERC20, AccessControl, Ownable {
    ...
    uint256 public initialBurn = 5_000_000_000_000 * (10**_decimals); // 50.00%
    uint256 public founderSupply = 285_000_000_000 * (10**_decimals); // 2.85%
    uint256 public presaleSupply = 10_000_000_000 * (10**_decimals); // 0.10%
    uint256 public idoSupply = 15_000_000_000 * (10**_decimals); // 0.15%
    uint256 public initialLiquidity = 4_565_000_000_000 * (10**_decimals); // 45.65%
    uint256 public marketingSupply = 100_000_000_000 * (10**_decimals); // 1.00%
    uint256 public developmentSupply = 25_000_000_000 * (10**_decimals); // 0.25%

    // Taxes
    uint256 public taxAmount = 1200;

    uint256 public reflectionTax = 6000;
    uint256 public talentAwardTax = 2000;
    uint256 public voterRewardTax = 1000;
    uint256 public lpTax = 400;
    uint256 public adminTax = 300;
    uint256 public buyBackTax = 300;
    uint256 public feeDenominator = 10000;

    mapping(address => bool) isFeeExempt;
    mapping (address => bool) isReflectionExempt;
    mapping(address => bool) isTalentAwardExempt;
    ...
    ITalentAwardDistributor public talentAwardDistributor;
    IVoterRewardDistributor public voterRewardDistributor;
    ...
    function _transferFrom(
        ...
        uint256 feeAmount = amount.mul(taxAmount).div(feeDenominator);// 12% of transaction amount
        ...
        if(!isReflectionExempt[sender]) { 
            try reflectionDistributor.setShare(sender, _balances[sender]) {} catch {} // updating share in sender's reflectionDistributor
        }
        if(!isReflectionExempt[recipient]) { 
            try reflectionDistributor.setShare(recipient, _balances[recipient]) {} catch {} // updating share in recipient's reflectionDistributor
        }
    ...
    function takeFee(uint256 amount, address _sender) internal {
        // Reflection Fee
        uint256 reflectionFeeAmount = amount.mul(reflectionTax).div(feeDenominator);//65% of 12% tax
        _depositReflection(reflectionFeeAmount);
        ...
        _depositTalentAwardReward(talentAwardFeeAmount); 

        // Voter Rewards Fee
        uint256 voterRewardFeeAmount = amount.mul(voterRewardTax).div(feeDenominator); //10% of the voter fee
        _depositVoterReward(voterRewardFeeAmount);
    ...
    function _depositTalentAwardReward(uint256 talentAwardFeeAmount) internal swapping {
        _basicTransfer(msg.sender, address(talentAwardDistributor), talentAwardFeeAmount); //sending portion of tax dedicated for talent awards to talentAwardsFeeReceiver
        talentAwardDistributor.depositRewardAmount(talentAwardFeeAmount); // call depositRewardAmount on talentAwardsFeeReceiver contract
    }

    function _depositVoterReward(uint256 voterRewardFeeAmount) internal swapping {
        // require(_timestamps[account] + DELAY <= block.timestamp, "Need To Wait Atleast 7 days to get the reward");
        uint256 zero = 0;
        require(voting.weeklyvote() != zero, "Need Atleast 1 vote to get the reward");
        _basicTransfer(msg.sender, address(voterRewardDistributor), voterRewardFeeAmount);
        voterRewardDistributor.depositRewardAmount(voterRewardFeeAmount);
    }
    ...
    function setVoteRewardDistributor(address _distributor) public onlyOwner {
        require(_distributor != address(0), '!nonzero');
        if (address(voterRewardDistributor) != address(0)) {
            isFeeExempt[address(voterRewardDistributor)] = false;
        }
        voterRewardDistributor = IVoterRewardDistributor(_distributor);
        isFeeExempt[_distributor] = true;
    }
