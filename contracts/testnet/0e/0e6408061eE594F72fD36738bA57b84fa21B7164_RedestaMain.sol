...
contract RedestaMain  {
    ...
    mapping(uint => Projects) idToProject;
    ...
    function deployProject(
        ...
        profiles[msg.sender].projects.push(projectID);
        projectID++;
    }
    ...
    function likeComment(uint _comID, uint _projectID, uint _comIndex) public {
        ...
        profiles[msg.sender].liked.push(_comID);
    }

    function dislikeComment(uint _comID, uint _projectID, uint _comIndex) public {
        idToComments[_projectID][_comIndex].likeCount++;
        profiles[msg.sender].disliked.push(_comID);
    }
    ...
    function donateProject(uint _projectID, uint amount) public {
        ...
        idToProject[_projectID].status.current += amount * (100-cutPercentage) / 100;

        profiles[msg.sender].donateAmount += amount * decimals;
        profiles[msg.sender].supporting.push(_projectID);
        rewardPool += amount * decimals * cutPercentage / 100;
        
        //Miktar threshhold'dan büyük mü ve alınmış nft sayısı maxsupply'dan küçük mü ve kullanıcı bu projenin nftsien sahip mi
        if(amount >= idToProject[_projectID].nftinfo.nftThreshold && idToProject[_projectID].nftinfo.owned < idToProject[_projectID].nftinfo.maxSupply && addrOwnedNFTbool[msg.sender][_projectID] == false){
            //mint komutu buraya
            idToProject[_projectID].nftinfo.owned++;
            addrOwnedNFTbool[msg.sender][_projectID] = true;
        } else {
            revert("Else error message.");
        }
    }
    ...
    function changeBio(string memory _bio) public {
        profiles[msg.sender].bio = _bio;
    }

    function getCategories(uint _id) public view returns(uint[] memory){
        return idToProject[_id].category;
    }

    function getSupporting(address _address) public view returns(uint[] memory){
        return profiles[_address].supporting;
    }

    function deployTwoTestProjects(uint[] memory _array) public {
        deployProject("roadmapaddress", "projectimgaddress", "title", "description", _array, 100, 500, 399000, "nftipfs2", "shortname");
        deployProject("roadmap2", "projectadd2", "title22", "desc22", _array, 200, 600, 799000, "nftipfs2", "shprtname2");
    }

    function vote(uint _id) public {
        if(addrOwnedNFTbool[msg.sender][_id] == true){
            revert("Select one of the other projects");
        } else {
