...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     *  After the last treatment is completed                   CLAIMED
     */
    enum Status { REQUESTING, PAID, PROCESSING, CANCELLED, FINISHED }
        ...
        onlyValidAddress(client)
    {
        ...
        treatments[lastPaymentId][payments[lastPaymentId].lastTreatmentId] = Treatment(
        ...
        nonReentrant 
    {
        Payment storage payment = payments[paymentId];
        Treatment storage treatment = treatments[paymentId][treatmentId];
        require(payment.status == Status.PAID, "This payment hasn't been paid yet");
        require(
            treatment.status == TreatmentStatus.CLIENT_CONFIRMED,
        ...
        nonReentrant 
    {
        Payment storage payment = payments[paymentId];
        require(payment.status == Status.PAID, "This payment hasn't been paid yet");
        for (uint256 i = 0; i < payment.numberOfInstallment; i++) {
            if (treatments[paymentId][i].status == TreatmentStatus.CLIENT_CONFIRMED) {
                treatments[paymentId][i].status = TreatmentStatus.CLAIMED;
                _claimPayment(i, payment.bo, treatmentId);
            }
        }
    }
    ...
    {
        Payment storage payment = payments[paymentId];
        require(payment.status == Status.PAID, "Can not withdraw");
        require(
        ...
        emit ClientWithdrawn(paymentId, payment.amount, payment.paymentToken);
    }
        ...
        nonReentrant
    {
        ...
        require(payment.amount > 0, "Nothing to withdraw");

        payment.status = Status.CANCELLED;
        _makePayment(_msgSender(), payment.paymentToken, payment.amount);

        emit OwnerWithdrawn(paymentId, payment.amount, payment.paymentToken);
    }
        ...
        nonReentrant
    {
        ...
        _makePayment(payment.client, payment.paymentToken, payment.amount);
