...
interface IAlphaNFTPowerLevelStorage {
    function getPowerLimitsByNFTType(string memory tokenType) external view returns(uint256);
}

contract AlphaNFTPowerLevelManager is BaseAttributeManager
{

    uint256 _minPowerIncrease = 10;
    uint256 _powerMultiplier = 1100;

    address _powerLevelStorage;
    ...
    function getRequiredERC20ToLevel(
        ...
        returns(uint256)
    {
        uint256 currentPower = IALPHANFT(_nftContract).getPowerLevelByTokenId(tokenId);
        
        // TODO:: determine power level multiplier vs current power and min power to level
        uint256 requiredERC20ToLevel = _powerMultiplier * power * currentPower;
    ...
    function increasePowerLevel(
        ...
        nonReentrant 
    {
        string memory tokenType = IALPHANFT(_nftContract).getNFTTypeByTokenId(tokenId);
        
        uint256 requiredERC20ToLevel = getRequiredERC20ToLevel(tokenId, power);        
        
        uint256 maxPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getPowerLimitsByNFTType(tokenType);

        require(IERC20(_erc20Token).balanceOf(msg.sender) >= requiredERC20ToLevel, "Insufficient balance to increase power.");
        require(IALPHANFT(_nftContract).getPowerLevelByTokenId(tokenId) <= maxPower - power, "Max Power Would Be Exceeded.");
        
        IERC20(_erc20Token).transferFrom(msg.sender, _feeReceiver, power);
        
        IALPHANFT(_nftContract).increasePowerLevel(power, tokenId);
    }
    ...
    function decreasePowerLevel(
        ...
        requireAdmin(msg.sender) 
    {
        IALPHANFT(_nftContract).decreasePowerLevel(power, tokenId);
    }
    ...
    function resetPowerLevel(
        ...
        nonReentrant 
    {
        IALPHANFT(_nftContract).resetPowerLevel(tokenId);
    }

    function setPowerMultiplier(
...
interface IALPHANFT {
  ...
  function getNFTTypeByTokenId (uint256 tokenId) external view returns ( string memory );
