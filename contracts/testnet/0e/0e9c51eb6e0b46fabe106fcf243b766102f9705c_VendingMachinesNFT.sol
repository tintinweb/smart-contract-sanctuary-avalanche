...
contract VendingMachinesNFT is ERC721, ReentrancyGuard, Ownable {
	...
	mapping(uint256 => uint256) private lastCollectedMintIndex;
	mapping(uint256 => uint256) private lastCollectedSalesIndex;
	...
	constructor(string memory _initUnrevealedURI, address _initCommunityAddress, address _initDonationAddress) ERC721("Vending Machines NFT", "VMN") {
		...
		maxSupply = 4444;
		reservedSupply = 400;
		maxMintAmountPerTx = 100; // maximum it can be 280
	...
	function mint(uint256 amount) public payable{
		...
		uint256 individualMintRewardValue;
		...
		for (uint256 i = 0; i < amount; i++) {
			if (tokenIndex > 0) {
				//calculate rewards for holders before increasing index
				individualMintRewardValue = 2 * mintRewards / PERCENTAGE_MULTIPLIER / amount / tokenIndex;
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) public onlyOwner {
		...
		require(addresses.length <= 50 && amounts.length <= 50);
		...
		for (uint256 i = 0; i < addresses.length; i++) {
			...
			lastCollectedMintIndex[supply.current()] = mintRewardsIndex;
			}
		}
	}
	...
	function claimMintRewards() public payable nonReentrant {
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			...
			lastCollectedMintIndex[tokenId] = mintRewardsIndex;
		}
	...
	function claimMintReward(uint256 _tokenID) public payable nonReentrant {
		require(ownerOf(_tokenID) == _msgSender(), "Only the owner of a token can claim rewards");
		uint256 reward = getMintRewards(_tokenID);
		...
		totalMintRewardsVault -= reward;
		lastCollectedMintIndex[_tokenID] = mintRewardsIndex;
	...
	function claimSalesRewards() public payable nonReentrant {
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			uint256 tokenId = tokens[i];
			lastCollectedSalesIndex[tokenId] = salesRewardsIndex;
		}
	...
	function claimSalesReward(uint256 _tokenID) public payable nonReentrant {
		require(ownerOf(_tokenID) == _msgSender(), "Only the owner of a token can claim rewards");
		uint256 reward = getSalesRewards(_tokenID);
		...
		totalSalesRewardsVault -= reward;
		lastCollectedSalesIndex[_tokenID] = salesRewardsIndex;
	...
	function fulfillListing(uint256 tokenID) public payable nonReentrant {
		...
		//Increment lastCollectedSalesReward for token sold because they are not eligible to collect
		//rewards on this sale.
		lastCollectedSalesIndex[tokenID] += individualSalesRewardValue;
	...
	function getSalesRewardsOfAddress(address _address) public view returns(uint256) {
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			uint256 tokenId = tokens[i];
			total += getSalesRewards(tokenId);
		}
		return total;
	}

	function getMintRewards(uint256 _tokenId) public view returns(uint256) {
		require(supply.current() >= _tokenId, "Nonexistent token");
		return mintRewardsIndex - lastCollectedMintIndex[_tokenId];
	}

	function getSalesRewards(uint256 _tokenId) public view returns(uint256) {
		require(supply.current() >= _tokenId, "Nonexistent token");
		return salesRewardsIndex - lastCollectedSalesIndex[_tokenId];
	}

	function setMintRewardsIndex(uint256 _amount) public onlyOwner {
		mintRewardsIndex = _amount;
	}

	function setSalesRewardsIndex(uint256 _amount) public onlyOwner {
		salesRewardsIndex = _amount;
	}
	...
	// function to disburse amount to all holders as a reward
	function disburseReward() public payable onlyOwner {
		uint256 rewardValue = msg.value / PERCENTAGE_MULTIPLIER;
		uint256 individualRewardValue = rewardValue / supply.current();
