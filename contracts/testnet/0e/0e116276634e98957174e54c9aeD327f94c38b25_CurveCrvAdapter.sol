...
import {ICurveAPoolForUseOnAvalanche} from "../../../interfaces/curve/ICurveAPoolForUseOnAvalanche.sol";
import {ICurveLpToken} from "../../../interfaces/curve/ICurveLpToken.sol";
import {CurveBasic} from "./CurveBasic.sol";

contract CurveCrvAdapter is CurveBasic {
    using SafeERC20 for IERC20;

    constructor(address _adapterManager)
        CurveBasic(_adapterManager, "CurveCrvSwap")
    {}
    ...
    // event CurveCrvExchangeUnderlyingEvent(
    //     address tokenFrom,
    //     address tokenTo,
    //     uint256 amountFrom,
    //     uint256 amountTo,
    ...
    function exchange(bytes calldata encodedData) external onlyAdapterManager {
        ...
        ICurveAPoolForUseOnAvalanche router = ICurveAPoolForUseOnAvalanche(
            addresses[3]
        );
    ...
    function addLiquidity(bytes calldata encodedData)
        ...
        onlyAdapterManager
    {
        /// @param addresses addresses of [dai.e(or avdai), usdc.e(or avusdc), usdt.e(or avusdt), lp, proxy, router]
        ...
        (
            ...
            bool useUnderlying
        ) = abi.decode(encodedData, (address[], uint256[3], uint256, bool));
        pullAndApprove(addresses[0], addresses[4], addresses[5], amountsIn[0]);
        pullAndApprove(addresses[1], addresses[4], addresses[5], amountsIn[1]);
        pullAndApprove(addresses[2], addresses[4], addresses[5], amountsIn[2]);
        ICurveAPoolForUseOnAvalanche router = ICurveAPoolForUseOnAvalanche(
            addresses[5]
        );
        ...
        ICurveLpToken lp = ICurveLpToken(addresses[3]);
        lp.transfer(addresses[4], giveBack);
        ...
        onlyAdapterManager
    {
        ...
        (
            ...
            bool useUnderlying
        ) = abi.decode(encodedData, (address[], uint256, uint256[3], bool));
        ICurveAPoolForUseOnAvalanche router = ICurveAPoolForUseOnAvalanche(
            addresses[5]
        );

        pullAndApprove(addresses[3], addresses[4], addresses[5], removeAmount);

        uint256[3] memory giveBack = router.remove_liquidity(
            removeAmount,
            minAmounts,
            useUnderlying
        );
        IERC20(addresses[0]).safeTransfer(addresses[4], giveBack[0]);
        IERC20(addresses[1]).safeTransfer(addresses[4], giveBack[1]);
        IERC20(addresses[2]).safeTransfer(addresses[4], giveBack[2]);
        ...
        onlyAdapterManager
    {
        ...
        pullAndApprove(addresses[3], addresses[4], addresses[5], lpAmount);
        ...
        if (tokenNumber == 0) {
            ...
            toToken = IERC20(addresses[2]);
            amounts = [0, 0, giveBack];
        }
        toToken.safeTransfer(addresses[4], giveBack);
...
interface ICurveAPoolForUseOnAvalanche {
