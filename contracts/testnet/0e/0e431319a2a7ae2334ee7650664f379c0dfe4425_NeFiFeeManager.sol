...
library NeFiLib{
	...
	uint256 constant Zero = 0;
		...
		function addressInList(address[] memory _list, address _account) internal returns (bool){
			for(uint i=0;i<_list.length;i++){
				if(_account == _list[i]){
					return true;
				}
...
contract NeFiFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,bool _feeFroze){
		_dropManager = _addresses[0];
		dropMGR = dropManager(_dropManager);
		_protoManager = _addresses[1];
	...
	function payFeeTokenSpec(address _account, uint256 _amount,uint256 _intervals, uint256 _x) payable external   onlyGuard(){
    	    _intervals = NeFiLib.getLower(INTgetProtoTotalPayable(_account,_x),_intervals);
    	    require(_intervals >0, "either you opted not to pay any fees at this time or have none left to pay");
    	    //calculation
            (uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(_intervals,feeTok.balanceOf(_account),tokFee);
	    checkFeeTokenSend(_account,needed,_intervals);
	    updateFeesPaidSpec(_account,_nIntervals,_x);
	    feeTok.transferFrom(_account, treasury, needed);
	    if(msg.value > 0){
		payable(_account).transfer(msg.value);
	    }
	}
	function payFeeToken(address _account,uint256 _amount,uint256 _intervals) payable external   onlyGuard(){
    	   _intervals = NeFiLib.getLower(INTgetTotalPayable(_account),_intervals);
	    ...
	    checkFeeTokenSend(_account,needed,_intervals);
	    feeTok.transferFrom(_account, treasury, needed);
	    updateFeesPaid(_account,_nIntervals);
	    if(msg.value > 0){
		payable(_account).transfer(msg.value);
	    }
	}
	function payFeeAvax(address _account,uint256 _amount,uint256 _intervals) payable external   onlyGuard(){
    		_intervals = NeFiLib.getLower(INTgetTotalPayable(_account),_intervals);
    		require(_intervals >0, "Doesnt Look Like you opted to pay any fees at this time");
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(_intervals,msg.value,over.getFee());
		//verification
		updateFeesPaid(_account,_nIntervals);
		checkFeeTokenSend(_account,needed,_intervals);
	...
	function payFeeAvaxSpec(address _account,uint256 _amount,uint256 _intervals,uint _x) payable external   onlyGuard(){
    		_intervals = NeFiLib.getLower(INTgetProtoTotalPayable(_account,_x),_intervals);
    		require(_intervals >0, "either you opted not to pay any fees at this time or have none left to pay");
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(_intervals,feeTok.balanceOf(_account),tokFee);
	...
	function checkFeeTokenSend(address _account,uint256 needed,uint256 _intervals) internal returns (uint256,uint256,uint256){
		require(NeFiLib.addressInList(protoOwners,_account) == true,"you are not in the protolist"); 
		checkFeeTokenBalanceOfUser(_account,needed);
		require(_intervals != 0 && needed > 0,"you do not hold enough to cover this claim");
		require(_intervals <= _intervals,"you do not hold enough to cover this claim");
		require(_intervals>= needed,"you do not hold enough to cover this claim");
	...
	function checkAvaxSend(address _account,uint256 _sent,uint256 _intervals) internal returns (uint256,uint256,uint256){
		require(NeFiLib.addressInList(protoOwners,_account) == true,"you are not in the protolist"); 
	   	require(totalfees[_account].totalPayable>=_intervals,"you have paid more fees than you are able to for this proto"); 
		(uint256 unusedIntervals,uint256 _intervals,uint256 newFee,uint256 balanceReturn) = NeFiLib.doAllMultiple(_intervals,_sent,over.getFee());
		require(_sent >= newFee,"you have not sent enough to pay a fee");
		return (_intervals,newFee,balanceReturn);
	}
	function recieverAvax(address _account,uint256 _amount) payable external   onlyGuard(){
	...
	function recieverFeeTok(address _account,uint256 _amount) payable external   onlyGuard(){
	...
	function updateRefund(address _account,uint256 _amount,bool _bool,bool _isAvax) external   onlyGuard(){
		if(_isAvax == true){
			if(_bool == true){
	...
	function reconcileFees() external   onlyGuard(){
		for(uint i = 0;i<protoOwners.length;i++){
			INTupdateTotalpays(protoOwners[i]);
		}
	}
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		_protoowners.push(PROTOOWNERS({
			...
			protoFeeFroze:Zero,
			protoTotalPayable:NeFiLib.getMultiple(feePeriod,maxFeePayment)
		}));		
	...
	function INTupdateProtoTotalPayable(address _account,uint256 _x) internal {
		uint256 next = INTgetProtoNextDue(_account,_x);
		uint256 _time =block.timestamp;
		if(totalfees[_account].futureFees >= protoowners[_account][_x].protoFutureFees){
			totalfees[_account].futureFees -= protoowners[_account][_x].protoFutureFees;
		}else{
			totalfees[_account].futureFees = Zero;
		}if(totalfees[_account].totalPayable >= protoowners[_account][_x].protoTotalPayable){
			totalfees[_account].totalPayable -= protoowners[_account][_x].protoTotalPayable;
		}else{
		...
		if(next<feePeriod){
			...
			protoowners[_account][_x].protoFutureFees = 0;
			protoowners[_account][_x].protoTotalPayable = maxPayPeriods;
		}else{
			uint256 rem;
			protoowners[_account][_x].protoDueElapsed =(next.add(INTgetProtoFeeFroze(_account,_x)).sub(_time));
			protoowners[_account][_x].protoFutureFees = NeFiLib.getMultiple(feePeriod,protoowners[_account][_x].protoDueElapsed);
			uint256 totalPayableElapsed = maxFeePayment.sub(protoowners[_account][_x].protoFutureFees);
			uint256 futureElapsed = INTgetProtoCollapseDate(_account,_x).sub(INTgetProtoNextDue(_account,_x));
			protoowners[_account][_x].protoTotalPayable= NeFiLib.getMultiple(feePeriod,NeFiLib.getLower(futureElapsed,totalPayableElapsed));
		}
		totalfees[_account].totalPayable += protoowners[_account][_x].protoTotalPayable;
		totalfees[_account].futureFees += protoowners[_account][_x].protoFutureFees;
	}
	...
	function INTupdateTotalpays(address _account) internal{
		if(totalfees[_account].futureFees >= totalfees[_account].futureFees){
			totalfees[address(this)].futureFees -= totalfees[_account].futureFees;
		}else{
			totalfees[address(this)].futureFees = Zero;
		}if(totalfees[address(this)].totalPayable >= totalfees[_account].totalPayable){
			totalfees[address(this)].totalPayable -= totalfees[_account].totalPayable;
		}else{
			totalfees[address(this)].totalPayable = Zero;
		}
		PROTOOWNERS[] storage protos = protoowners[_account];
		for(uint j=0;j<protos.length;j++){
			INTupdateProtoTotalPayable(_account,j);
			INTupdateProtoTotalPayable(_account,j);
		}
		totalfees[address(this)].totalPayable +=totalfees[_account].totalPayable;
		totalfees[address(this)].futureFees += totalfees[_account].futureFees;
	...
	function getPeriodInfo() external view returns(uint256,uint256,uint256,uint256,uint256){
		return (gracePeriod,protoLife,maxFeePayment,maxPayPeriods,feePeriod);
	}
	//get Struct
	...
	//only Owner Functions
    	...
    	function changeFeeTokenPrice(address _account) external onlyOwner(){
    	 	feeToken = _account;
    		feeTok = IERC20(feeToken);
        }
