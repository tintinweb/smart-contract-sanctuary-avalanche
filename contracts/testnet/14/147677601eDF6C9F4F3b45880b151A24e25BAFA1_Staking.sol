...
contract Staking is Ownable {
    ...
    function Stake() public payable {
        ...
        StakerInfo storage _user = _stakerInfo[_lotteryCounter][msg.sender];

        if (_user.isStaker == true) {
    ...
    function withdraw_(uint256 lotteryCounter_) public {
        ...
        require(
            _stakerInfo[lotteryCounter_][msg.sender].withdrawStatus == false,
            "you are not Eligible to withdraw"
        );

        uint256 _amount = _stakerInfo[lotteryCounter_][msg.sender].Totalstaked;

        _stakerInfo[lotteryCounter_][msg.sender].Totalstaked = 0;
        _stakerInfo[lotteryCounter_][msg.sender].withdrawStatus = true;
        _stakerInfo[lotteryCounter_][msg.sender].isStaker = false;
