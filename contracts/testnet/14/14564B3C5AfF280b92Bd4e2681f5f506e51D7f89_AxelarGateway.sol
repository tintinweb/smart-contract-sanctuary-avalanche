...
contract AxelarGateway is IAxelarGateway, AdminMultisigBase {
    ...
    /// @dev Returns the current `getAdminEpoch`.
    function getAdminEpoch() external view override returns (uint256) {
        return _adminEpoch();
    }

    /// @dev Returns the admin threshold for a given `getAdminEpoch`.
    function adminThreshold(uint256 epoch) external view override returns (uint256) {
        return _getAdminThreshold(epoch);
    }

    /// @dev Returns the array of admins within a given `getAdminEpoch`.
