...
abstract contract nebuFormulas is Context {
	function EXTisInList(address[] memory _list, address _account) external virtual view returns (uint256);
	function EXTgetDecimals(uint256 _x) external virtual view returns (uint256);
	function EXTelimZero(uint256 _y) external virtual view returns(uint256);
	function EXTdecPercentage(uint256 _x,uint256 perc) external virtual view returns(uint,uint256,uint256);
...
contract NeFiFeeManager is Ownable{
	...
	constructor(address[] memory addresses){
		Guard = addresses[0];
		dropMGR = dropManager(addresses[1]);
	...
	function INTreconcileFees() internal {
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		_protoowners.push(PROTOOWNERS({
			...
			protoFeeFroze:Zero,
			protoTotalPayable:nebuLib.doMultiple(feePeriod,maxFeePayment.sub(feePeriod))
		}));		
	...
	function pauseFees(bool _x) external onlyOwner {
   	 ...
   	 function changefeeToken(address _account) external  onlyOwner() {
    		feeTok = IERC20(_account);
   	 }
        ...
        function changeDropManager(address _account) external onlyOwner() {
    		dropManager dropMGR = dropManager(_account);
        }
        ...
        function changeBoostFeeToken(address _account) external  onlyOwner(){
    		feeTok = IERC20(_account);
        }
