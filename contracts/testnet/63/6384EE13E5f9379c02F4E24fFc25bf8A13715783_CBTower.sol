...
interface ICBTower {
    function getCurrentFloor() external view returns (uint8);
}

contract CBTower is Ownable, ReentrancyGuard, ICBTower {
    ...
    // testnet - 0x1f71EC59777aA15F11ff86bDd7F8244263753e31
    /// BSC USDT - 0x55d398326f99059fF775485246999027B3197955
    IERC20 public USDT = IERC20(0x1f71EC59777aA15F11ff86bDd7F8244263753e31);


    struct UserInfo {
        uint256 reinvestRatio;
        uint256 joinTime;
        uint8 lastUpdateFloor;
        // Referrals
        address level1;
        address level2;
        address level3;
    ...
    uint8 public currentFloor;
    mapping (uint8 => FloorInfo) public floorInfo;
    ...
    event FloorFinished(uint8 floor);
    ...
    constructor(address _bct, address _bbt, address _bwt) {
        ...
        BWT = _bwt;
        currentFloor = 0;
        floorInfo[currentFloor].startDate = block.timestamp;
        floorInfo[currentFloor].endDate = block.timestamp + getBuildTime();
        floorInfo[currentFloor].collectionAmount = getCollectionAmount();
    }

    /// @dev Public Functions

    function invest(address _referral, uint256 amount) public nonReentrant {
    ...
    function getCurrentFloor() public override view returns (uint8) {
        return currentFloor;
    }
    ...
    function initiateNewFloor() public {
        ...
        floor.startDate = block.timestamp;
        floor.endDate = block.timestamp + getBuildTime();
        floor.collectionAmount = getCollectionAmount();
    ...
    function _invest(uint256 amount) internal returns (uint256 left){
        FloorInfo storage floor = floorInfo[currentFloor];
        UserInfo storage user = userInfo[_msgSender()];
