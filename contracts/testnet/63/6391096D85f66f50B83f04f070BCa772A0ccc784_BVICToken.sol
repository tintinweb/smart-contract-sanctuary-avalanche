...
contract BVICToken is ERC20, Operator {
    using SafeMath for uint256;

    // Initial distribution for the first 3 days genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 2.25 ether;

    // Have the rewards been distributed to the pools
    bool public rewardPoolDistributed;
	
	uint256 public constant INITIAL_FRAGMENTS_SUPPLY = 0.0002 ether;
    ...
    constructor(address _polWallet, address _btcAddress, address _router) ERC20("Bitcoin Victory", "BVIC") {
		require(_polWallet != address(0), "!_polWallet");
		require(_btcAddress != address(0), "!_btcAddress");
		...
		taxTiersTwaps = [0, 8e7, 9e7, 1e8]; // twap with btc.b
		taxTiersRates = [2000, 1000, 500, 0];
		taxRateAfterRebase = 3000; // 30%
		timeTaxAfterRebase = 24 hours;

		taxOffice = msg.sender;
		IUniswapV2Router _dexRouter = IUniswapV2Router(_router);
		address dexPair = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _btcAddress);
        setMarketLpPairs(dexPair, true);
		// Mints token to contract creator for initial pool setup
