...
contract SingleStakeManager is ISingleStakeManager, Ownable {
    ...
    function setFarmFactory(address _factoryAddress) external onlyOwner {
        require(
            ...
            "SingleStakeManager: ZERO_ADDRESS"
        );
        stakeFactory = ISingleStakeFactory(_factoryAddress);
    ...
    /**
     * @dev Any reward tokens are sent to the SingleStakingRewards contract when this function is called.
    ...
    function startEmission(
        ...
        ISingleStakingRewards singleStake = ISingleStakingRewards(_singleStakeAddress);
        // Transfer rewards
        for (uint i = 0; i < _rewards.length; ++i) {
            IERC20(singleStake.rewardTokens(i)).safeTransferFrom(msg.sender, _singleStakeAddress, _rewards[i]);
        }
        // Start emissions
        singleStake.startEmission(
            _rewards,
            _duration
        );
    }
     ...
     */
    function multiClaim(address[] memory _farms) external {
        require(_farms.length < 30, "SingleStakeManager: TOO_MANY_FARMS");
