...
contract PresaleFlatten {
  ...
  //////////
  // Getters
  function calculateAvaxToPresaleToken(uint256 _amount) public view returns (uint256) {
    ...
    uint256 tokensWithBand = tokens + (tokens * bandsPercentages[allocatedBand[msg.sender]]) / FEE_DENOMINATOR;
    return (tokensWithBand);
  }

  function getAvailableTokenToClaim() public view returns (uint256) {
    uint256 totalToken = calculateAvaxToPresaleToken(bought[msg.sender]);
    return ((totalToken * getTotalClaimPercentage()) / FEE_DENOMINATOR) - totalClaimToken[msg.sender];
  }
  ...
  function getTotalClaimPercentage() private view returns (uint256) {
    ...
    uint256 interval = BokkyPooBahsDateTimeLibrary.diffMonths(startDateClaim, block.timestamp);
  ...
  function claim(uint256 requestedAmount) public {
    ...
    uint256 remainingToken = calculateAvaxToPresaleToken(bought[msg.sender]) - totalClaimToken[msg.sender];
