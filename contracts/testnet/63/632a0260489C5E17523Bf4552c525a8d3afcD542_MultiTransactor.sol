...
import "./interface/IMultiTransactor.sol";

contract MultiTransactor is AccessControlEnumerable, IMultiTransactor {
    ...
    function setAdmin(address account, bool value)
        public
        ...
        returns (bool success)
    {
        if (value) {
            require(
                ...
                "MultiTransactor: account already has admin role"
            );
            grantRole(DEFAULT_ADMIN_ROLE, account);
        } else {
            require(
                hasRole(DEFAULT_ADMIN_ROLE, account),
                "MultiTransactor: account doesn't have admin role"
            );
    ...
    function setTransactor(address account, bool value)
        public
        ...
        returns (bool success)
    {
        if (value) {
            require(
                !hasRole(TRANSACTOR_ROLE, account),
                "MultiTransactor: account already has transactor role"
            );
            grantRole(TRANSACTOR_ROLE, account);
        } else {
            require(
                hasRole(TRANSACTOR_ROLE, account),
                "MultiTransactor: account doesn't have transactor role"
            );
    ...
    function sendTx(address to, bytes memory data)
        external
        onlyRole(TRANSACTOR_ROLE)
        ...
        returns (bytes[] memory)
    {
        require(
            ...
            "MultiTransactor: to.length != data.length"
        );
...
interface IMultiTransactor {
    ...
    function setAdmin(address target, bool value)
        external
        returns (bool success);

    function setTransactor(address target, bool value)
        external
        returns (bool success);
