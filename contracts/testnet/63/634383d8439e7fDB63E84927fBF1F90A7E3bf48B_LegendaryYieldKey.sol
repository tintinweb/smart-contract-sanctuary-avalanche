    ...
    Claimable
{
    ...
    function initialize(
        ...
        maxMintPerWallet = 5;
        totalMintCnt = 0;
        whitelistStartTime = 1642987840;
        preSaleStartTime = 1643587185;
        publicSaleStartTime = 1643673585;
        paused = true;
        mintPause = true;
        upgradePaused = true;
    }
    ...
    function setWhiteListStartTime(uint32 timestamp) public onlyOwner {
        whitelistStartTime = timestamp;
    }

    function setPreSaleStartTime(uint32 timestamp) public onlyOwner {
    ...
    function addToWhitelist(address _address) public onlyAdmin {
        require(whitelistIsActive(), "Whitelist period is not active");
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "Mint quantity exceeds maximum allowed per wallet"
        );
        require(
            _mintAmount + totalMinted[_msgSender()] <= maxMintPerWallet,
            "Mint quantity plus already minted exceeds maximum allowed per wallet"
        );
        require(
            supply + _mintAmount <= TOTAL_SUPPLY,
            "Mint quantity exceeds maximum supply"
        );
        require(cost * _mintAmount <= msg.value, "Insufficient AVAX");
    ...
    // Allows users to upgrade their v2 tokens
    function mintV2toV3()
        external
        ...
        nonReentrant
    {
        ...
        require(!upgradePaused, "Upgrade to v3 paused");
        require(isApproved, "Contract is not approved for all");
        require(ownedNFTs.length > 0, "Invalid amount");
        totalMinted[_msgSender()] += ownedNFTs.length;
        for (uint256 i = 0; i < ownedNFTs.length; i++) {
            _safeMint(_msgSender(), ownedNFTs[i]);
            ValiFiNFTv2.burn(ownedNFTs[i]);
        }
        totalMintCnt += ownedNFTs.length;
        emit UpgradeFromV2ToV3(_msgSender(), ownedNFTs);
    }

    // Allows users to upgrade their v1 tokens
    function mintV1toV3()
        external
        validAddress(_msgSender())
        nonReentrant
    {
        ...
        totalMintCnt += ownedNFTs.length;
        emit UpgradeFromV1ToV3(_msgSender(), ownedNFTs);
    }

    //  Used to find the tokens that a wallet holds.
    ...
    function pauseUpgrade(bool _state) public onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(
                ...
                "Transfer is currently not allowed"
            );
            require(
