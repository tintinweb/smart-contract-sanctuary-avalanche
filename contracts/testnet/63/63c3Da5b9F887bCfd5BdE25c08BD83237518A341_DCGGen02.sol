...
contract DCGGen02 is ERC721, IERC2981, ReentrancyGuard, Ownable {
  using Counters for Counters.Counter;

  constructor(string memory customBaseURI_, address accessTokenAddress_)
    ERC721("DCG Gen 0", "DCGGEN0")
  {
  ...
  function mint() public nonReentrant {
    ...
    require(accessToken.balanceOf(msg.sender) > 0, "Access token not owned. You need to get the keys first.");

    require(totalSupply() < MAX_SUPPLY, "Exceeds max supply");

    require(_isWhiteListOnly, "only available for approved users. You need to get the keys and register for whitelisting on DCG discord.");
    require(_whiteList[msg.sender], "You do not have the key required to mint.");
    _whiteListAllocated[msg.sender] -= 1;
