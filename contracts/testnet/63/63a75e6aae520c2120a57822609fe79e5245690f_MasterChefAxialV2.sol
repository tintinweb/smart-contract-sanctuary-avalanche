...
// File contracts/AxialToken.sol


pragma solidity 0.6.12;


// AxialToken with Governance.
contract AxialToken is ERC20("AxialToken", "AXIAL"), Ownable {
    /// @dev Total number of tokens
    uint256 public maxSupply = 365_000_000e18; // 365 million Axial

    /// @dev Creates `_amount` token to `_to`. Must only be called by the owner (MasterAxial).
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) <= maxSupply, "AXIAL::mint: cannot exceed max supply");
        _mint(_to, _amount);
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "AXIAL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "AXIAL::delegateBySig: invalid nonce");
        require(now <= expiry, "AXIAL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "AXIAL::getPriorVotes: not yet determined");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying AXIALs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "AXIAL::_writeCheckpoint: block number exceeds 32 bits");
...
// File contracts/MasterChefAxialV2.sol
...
// MasterChefAxial is a boss. He says "go f your blocks lego boy, I'm gonna use timestamp instead".
...
// will be transferred to a governance smart contract once AXIAL is sufficiently
// distributed and the community can show to govern itself.
//
// With thanks to the Lydia Finance team.
//
// Godspeed and may the 10x be with you.
contract MasterChefAxialV2 is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of AXIALs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAxialPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAxialPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    // AXIAL tokens created per second.
    uint256 public axialPerSec;
    // Percentage of pool  that goto the devs.
    ...
    // The timestamp when AXIAL mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 _axialPerSec);
    ...
    // Update the given pool's AXIAL allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
    ...
    // View function to see pending AXIALs on frontend.
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 axialReward = multiplier.mul(axialPerSec).mul(pool.allocPoint).div(totalAllocPoint).mul(lpPercent).div(
                1000
            );
            accAxialPerShare = accAxialPerShare.add(axialReward.mul(1e12).div(lpSupply));
        }
        pendingAxial = user.amount.mul(accAxialPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 axialReward = multiplier.mul(axialPerSec).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 lpPercent = 1000 - devPercent - treasuryPercent - investorPercent;
        axial.mint(devAddr, axialReward.mul(devPercent).div(1000));
        axial.mint(treasuryAddr, axialReward.mul(treasuryPercent).div(1000));
        axial.mint(investorAddr, axialReward.mul(investorPercent).div(1000));
        axial.mint(address(this), axialReward.mul(lpPercent).div(1000));
        pool.accAxialPerShare = pool.accAxialPerShare.add(axialReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000));
        pool.lastRewardTimestamp = block.timestamp;
        emit UpdatePool(_pid, pool.lastRewardTimestamp, lpSupply, pool.accAxialPerShare);
    }

    // Deposit LP tokens to MasterChef for AXIAL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            // Harvest AXIAL
            uint256 pending = user.amount.mul(pool.accAxialPerShare).div(1e12).sub(user.rewardDebt);
            safeAxialTransfer(msg.sender, pending);
            emit Harvest(msg.sender, _pid, pending);
        }
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accAxialPerShare).div(1e12);

        IRewarder rewarder = poolInfo[_pid].rewarder;
        if (address(rewarder) != address(0)) {
            rewarder.onAxialReward(msg.sender, user.amount);
        }
    ...
    // Safe axial transfer function, just in case if rounding error causes pool to not have enough AXIALs.
    ...
    function updateEmissionRate(uint256 _axialPerSec) public onlyOwner {
        massUpdatePools();
        axialPerSec = _axialPerSec;
        emit UpdateEmissionRate(msg.sender, _axialPerSec);
    }
}