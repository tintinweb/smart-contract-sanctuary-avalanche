...
contract BridgesOfFate is Ownable {
    ...
    event comment3(uint256 _stage1, uint256 _stage2);
    // event comment2(uint256 _stage123);
    // event comment4(uint256[] _stage7);
    event comment5(uint256 matchtest);
    event comment6(uint256 matchtest1);

    uint256[] public bulkRandomNFTSelect;
    ...
    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  {
        ...
        // emit comment2(PlayerItem[playerIds[0]].stage + 1);
        if(totalNFTOnTile[GameStatusInitialized[latestGameId].stageNumber][_jumpSides] < playerIds.length){
         bulkRandomNFTSelect =  playerIndex = generateRandomTicketNumbers(playerIds.length,playerIds.length,totalNFTOnTile[PlayerItem[playerIds[0]].stage + 1][_jumpSides]);
        }
        // emit comment4(playerIndex);

        for (uint256 i = 0; i < playerIds.length; i++) {
                    emit comment5(playerIndex[i]);
            for (uint j = 0; j < playerIndex.length; j++) {
                // if((playerIndex[j] > 0) && (playerIndex[j] == i)){
                    // OverFlowPlayerStatus[playerIds[i]] =  true;
                    if((playerIndex[j] > 0 && playerIndex[i] > 0) && (playerIndex[j] == playerIndex[i]) ){
                        emit comment6(playerIndex[j]);
                        emit comment3(playerIndex[j] , playerIndex[i]);
