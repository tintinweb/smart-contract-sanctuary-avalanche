...
contract Marketplace is AccessControl {
  ...
  IERC721 public weirdFrensContract;
  ...
  constructor(
    ...
    weirdFrensContract = IERC721(weirdFrensNFT);
  ...
  function createOffer(
    ...
    //   abi.encodePacked(block.timestamp, msg.sender, _tokenId, _price)
    // );
  ...
  function acceptOffer(uint256 _tokenId) public payable {
    ...
    require(
      weirdFrensContract.ownerOf(_tokenId) == offer.seller,
      'Seller is no longer the owner of the token'
    );
    require(
    ...
    (bool success, ) = seller.call{value: (msg.value * 95) / 100}('');
    require(success, 'Transfer failed');

    if (weirdFrensContract.getApproved(_tokenId) != address(this)) {
      weirdFrensContract.approve(address(this), _tokenId);
    }
  ...
  function adminWithdraw() external payable onlyTeam {
