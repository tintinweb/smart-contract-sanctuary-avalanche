        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                if (contractTokenBalance > 0) {
                    ...
                    uint256 marketingShare = marketingFee.mul(100).div(amountToSwap);
                    uint256 marketingTax = afterETHBalance.mul(marketingShare).div(10000);

                    if (marketingTax > 0) {
                        sendETHToFee(marketingTax);
                    }

                    uint256 liquidityShare = halfLP.mul(100).div(amountToSwap);
                    uint256 amountToLiquify = afterETHBalance.mul(liquidityShare).div(10000);
