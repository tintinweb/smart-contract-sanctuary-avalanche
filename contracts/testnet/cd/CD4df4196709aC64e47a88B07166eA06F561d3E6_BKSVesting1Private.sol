...
import "./lib/BKSVesting.sol";

// solhint-disable not-rely-on-time

contract BKSVesting1Private is BKSVesting {
    constructor(address _token, bytes32 _mercleRoot, uint256 _tgeTimestamp) {
        require(_token != address(0), "BKSVesting1Private: zero token address");
        require(_mercleRoot != bytes32(0), "BKSVesting1Private: zero mercle root");
        require(_tgeTimestamp >= block.timestamp, "BKSVesting1Private: wrong TGE timestamp");
        ...
        // rounds settings
        // (75000 + 77100 * 12) / 1e6 = 1.0002 ==> 1.0
        categories[CategoryNames.SALE_CATEGORY] = CategoryType({
            totalSteps: 12,
            stepTime: 1 hours,
            tgePercent: 75000,
            stepStageDelay: 1 hours,
            stepPercent: 77100
        });
...
abstract contract BKSVesting {
    ...
    function claim(uint256 _category, uint256 _amount, bytes32[] calldata _merkleProof) external returns(uint256 _claimResult) {
        require(inited, "BKSVesting: not inited");
        require(_verify(msg.sender, _category, _amount, _merkleProof), "BKSVesting: invalid proof or wrong data");
        require(CategoryNames(_category) != CategoryNames.EMPTY, "BKSVesting: invalid category");
        require(_amount > 0, "BKSVesting: invalid amount");
        require(block.timestamp >= tgeTimestamp, "BKSVesting: TGE has not started yet");
        ...
        require(reward > 0, "BKSVesting: no tokens to claim");
        ...
        if (rewarded.add(reward) > _amount) {
            resultReward = _amount.sub(rewarded, "BKSVesting: no tokens to claim (security check)");
        } else {
            resultReward = reward;
        }
