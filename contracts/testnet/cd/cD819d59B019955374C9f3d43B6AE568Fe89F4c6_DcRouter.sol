...
contract DcRouter is NonBlockingNonUpgradableBaseApp {
    ...
    bytes4 internal constant receiveDepositsSelector =
        bytes4(keccak256("receiveDeposit(bytes)"));
    ...
    function transferDeposits(bytes memory _payload) public onlySelf {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            uint256 pendingUserDeposit = pendingDeposits[strategyId][users[i]]; //save gas on sload
            pendingTotal += pendingUserDeposit;
            pendingStrategyDeposits[strategyId] -= pendingUserDeposit;
            _updateUserShares(strategyTvl, users[i], pendingUserDeposit);
