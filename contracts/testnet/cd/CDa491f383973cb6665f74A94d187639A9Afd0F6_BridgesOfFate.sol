...
contract BridgesOfFate is Ownable {
    ...
    uint256 private _dynamicNumberProbility;
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public GameEndRules
    {
        ...
        bytes32 playerId;
        if(_seriesType == 1 ){
            playerId = computeNextPlayerIdForHolder(msg.sender, _nftId, 1);
        }else if( _seriesType == 2){
            playerId = computeNextPlayerIdForHolder(msg.sender, _nftId, 2);
        }
 ...
 function bulkEntryFeeSeries(bytes32[] memory _playerId,uint256[] calldata _nftId, uint8 seriesType) external {
    ...
    function buyBackInFee(bytes32 playerId) external  GameEndRules
    {
        ...
        require(balanceOfUser(msg.sender) >= buyBackFee,"Insufficient balance");
        GameItem memory _member = PlayerItem[playerId];
    ...
    function switchSide(bytes32 playerId) public  GameEndRules {
        ...
        require(_dynamicGameMap < _dynamicNumberProbility, "Reached maximum");
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules
    {
        ...
        require(_member.feeStatus == true, "Please Pay Entry Fee.");
        require(_dynamicGameMap < 50e9,"Dynamic Game Ended.");
        ...
        if(_checkSide(GameMap[_gameStatus.stageNumber],_jumpSide) && (_dynamicGameMap > 50e9 )){
    ...
    function computeNextPlayerIdForHolder(address holder,uint256 _nftId,uint8 _seriesIndex) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(holder, _nftId, _seriesIndex));
    }

    function balanceOfUser(address _accountOf) internal view returns (uint256) {
    ...
    function LateBuyInFee(uint256 _nftId, uint8 seriesType) public GameEndRules
    {
        ...
        if (seriesType == 2) {
            ...
            require(balanceOfUser(msg.sender) >= buyBackFee + SERIES_TWO_FEE,"Insufficient balance");
    ...
    function withdrawWrappedEtherOFCommunity(uint8 withdrawtype) public onlyOwner  
    {
    ...
    function claimWinnerEther(bytes32 playerId) public  {
