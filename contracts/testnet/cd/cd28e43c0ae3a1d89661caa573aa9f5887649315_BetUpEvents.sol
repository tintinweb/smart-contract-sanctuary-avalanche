...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    bytes32 public constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");
    bytes32 public constant OPEN_COEFFICIENT = keccak256("OPEN_COEFFICIENT_ROLE");

    uint256 public openCoefficient;
    uint256 public resolveDeadlineSecond;
    // @dev the reward fee for creator
    uint256 public createFee;
    uint256 public arbitrationSpan;
    ...
    mapping(bytes32 => uint256) public players;

    // @dev mapping from `bytes32(pairID,_resultID)` to `amount`
    mapping(bytes32 => uint256) public optionsVolume;


    struct Pair {
        uint256 total;              // @dev the total number of the stacked amount
        uint64 resolutionAt;        // @dev the time of start to resolute the event
        address creator;            // @dev the account which created the event
        uint32 resolverQty;         // @dev the number of the resolvers
        ...
        uint8 resultID;             // @dev resultID means the order number of options' item and it start from 1
        bool paused;                // @dev pause claim
        uint256 withdrewAmount;     // @dev the number of creationFee which has been withdrew
        uint256 availableAmount;    // @dev the number of fee which will be withdrew by vault
    }
    ...
    function getRewardOf(
        ...
        uint256 playerAmount = players[playerID];
        uint256 op = optionsVolume[keccak256(abi.encodePacked(_pairID,_resultID))];
    ...
    */
    function open(
        ...
        require(IBetUpDAOToken(betUpDaoToken).balanceOf(msg.sender) >= openCoefficient, "substandard conditions");
        require(_optionsQty >= 2, "invalid _optionsQty");
        ...
        pairs[pairIDCounter].creator = msg.sender;
    ...
    function close(uint256 _pairID,  uint8 _resultID) external whenNotPaused {
        Pair memory pair = pairs[_pairID];
        ...
        require(pair.resultID == 0, "pair resolved");
        require(_resultID > 0 && _resultID <= pair.optionsQty, "invalid _resultID" );
        require(block.timestamp > pair.resolutionAt && block.timestamp < pair.resolutionAt + resolveDeadlineSecond, "outdated");

        bytes32 resultCounterID = keccak256(abi.encodePacked(_pairID, _resultID));
        ...
        if(nextResultCount * 10e3 >= pair.resolverQty * 2 *  10e3 / 3 ) {
            _close(_pairID, _resultID, pair);
        }

        pairsResolver[pairResolverID].resultID = _resultID;
        resultCounter[resultCounterID]++;

        emit CloseEvent(_pairID, _resultID);
    }

    function stack(uint256 _pairID, uint8 _resultID, uint256 _amount) external whenNotPaused {
        Pair memory pair = pairs[_pairID];
        ...
        players[keccak256(abi.encodePacked(_pairID,_resultID,sender))] += _amount;
        optionsVolume[keccak256(abi.encodePacked(_pairID,_resultID))] += _amount;
        pairs[_pairID].total +=  _amount;

        IBetUpCore(betUpCore).setPlayerRecord(sender, _amount);

        emit StackEvent(_pairID, _resultID, _amount);
    }

    function claim(uint256 _pairID, uint8 _resultID, uint256 _maxLevel) external whenNotPaused {
        Pair memory pair = pairs[_pairID];
        if (pair.optionsQty == 0) revert InvalidPairID(_pairID);
        if(pair.paused) revert PairPaused(_pairID);
        ...
        uint256 playerAmount = players[playerID];
        if (playerAmount == 0 ) revert InvalidResultID(_resultID);
    ...
    function withdrawFee(uint256 _pairID) external whenNotPaused {
        Pair memory pair = pairs[_pairID];
        address receiver = msg.sender;

        if(pair.creator != receiver) revert InvalidPairID(_pairID);
        require(pair.resultID > 0, "ongoing");
        if(pair.paused) revert PairPaused(_pairID);
        require(pair.withdrewAmount == 0, "withdrew");

        uint256 op = optionsVolume[keccak256(abi.encodePacked(_pairID,pair.resultID))];
        uint256 fee = createFee == 0 ?  0 : (pair.total - op) * 10 * createFee/1000000 ;

        pairs[_pairID].withdrewAmount = fee;
        if (fee > 0) IERC20MetadataUpgradeable(pair.chip).safeTransfer(receiver, fee);

        emit WithdrawFeeEvent(_pairID, fee);
    }

    // @dev Support for two-digit decimals
    function setCreationFee(uint256 _createFee ) external onlyRole(GOVERNANCE_ROLE) {
        require(_createFee <= 10000, "invalid _createFee");
        uint256 _prior = createFee;
        createFee = _createFee;
        
        emit EditCreationFee(_prior, _createFee);
    }

    function setResultID(uint256 _pairID, uint8 _resultID) external  onlyRole(GOVERNANCE_ROLE) {
        Pair memory pair = pairs[_pairID];
    ...
    function _close(uint256 _pairID, uint8 _resultID, Pair memory  pair) private {
        pairs[_pairID].resultID =  _resultID;
        pairs[_pairID].availableAmount +=  pair.total * (10000 - createFee) / (10 * 10000);
    }
    ...
    function setOpenCoefficient(uint256 _num) external onlyRole(OPEN_COEFFICIENT) {
        openCoefficient = _num;
    }

    function setResolveDeadlineSecond(uint256 _spam) external onlyRole(OPEN_COEFFICIENT) {
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint256 availableAmount = pair.availableAmount;
            require(availableAmount > 0 , "Invalid pair");
            pairs[pairID].availableAmount = 0;
    ...
    */
    function _getReward(
        ...
        uint256 totalAmount = pairs[_pairID].total;
        uint256 deadlineAt = pair.resolutionAt + resolveDeadlineSecond;
        uint256 _chipDecimal = IERC20MetadataUpgradeable(pairs[_pairID].chip).decimals();
        console.log(1, refund, reward);
        if(pair.resultID > 0 ) {
            console.log(2, refund, reward);

            if(pair.resultID == _resultID) {
                uint256 _profit = (9 * playerAmount *(totalAmount - op)) / (10 * op);
                refund = playerAmount +  _profit;
                reward = winPer * playerAmount  / 10 ** _chipDecimal;
                console.log(refund, reward);
//                console.log(refund, reward);
//                console.log(winPer);
//                console.log(playerAmount);
//                console.log(_chipDecimal);
//                console.log(10 ** _chipDecimal);
            }else {
                reward = losePer * playerAmount / 10 ** _chipDecimal;
                console.log(3, refund, reward);
            }

//            37500000000000000000000000000000

        }else if(
            block.timestamp > deadlineAt && pair.resultID == 0 ||
            block.timestamp > pair.endStackAt && (pair.total - op == 0)
        )  {
            refund = playerAmount;
            console.log(4, refund, reward);
        }

        console.log( block.timestamp > pair.endStackAt);
        console.log( pair.total);
        console.log( op);

        console.log(5, refund, reward);
    ...
    function initialize(
        ...
        // @Dev initialize the number of createFee which could be set by community
        createFee = 0;
...
interface IBetStruct {
    ...
    struct PlayerRecord {
        uint256 amount;    // @dev the number of transaction amount
...
interface  IBetUpCore {
    ...
    function setPlayerRecord(address, uint256) external;
