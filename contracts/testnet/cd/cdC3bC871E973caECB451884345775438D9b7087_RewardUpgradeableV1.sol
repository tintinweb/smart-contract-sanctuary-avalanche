...
contract RewardUpgradeableV1 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint private _claimedKeyLevelBonusReward;
    uint private _claimedVeTresrBonusReward;
    uint private _claimedJlpSmrtBonusReward;
    uint private _claimedJlpTresrBonusReward;
    ...
    function claimBonus(uint[] memory tokenIDList) public {
        ...
        if (_veTresrAddressContract != address(0)) {
            ...
            if (_claimedVeTresrBonusReward <= reward) {
                amountReward += reward - _claimedVeTresrBonusReward;
                _claimedVeTresrBonusReward += reward;
            }
        }

        if (_jlpSmrtAddressContract != address(0)) {
            uint reward = LpStaking(_jlpSmrtAddressContract).getBonusReward(msg.sender, block.timestamp) * _jlpSmrtBonusReward / 1000;

            if (_claimedJlpSmrtBonusReward <= reward) {
                amountReward += reward - _claimedJlpSmrtBonusReward;
                _claimedJlpSmrtBonusReward += reward;
            }
        }

        if (_jlpTresrAddressContract != address(0)) {
            uint reward = LpStaking(_jlpTresrAddressContract).getBonusReward(msg.sender, block.timestamp) * _jlpTresrBonusReward / 1000;

            if (_claimedJlpTresrBonusReward <= reward) {
                amountReward += reward - _claimedJlpTresrBonusReward;
                _claimedJlpTresrBonusReward += reward;
            }
        }

        if (_keyLevelAddressContract != address(0)) {
            ...
            if (_claimedKeyLevelBonusReward <= reward) {
                amountReward += reward - _claimedKeyLevelBonusReward;
                _claimedKeyLevelBonusReward += reward;
            }
        }
    ...
    function getVeTresrBonusReward(uint time) external view returns (uint) {
        ...
        if (_claimedVeTresrBonusReward >= reward) return 0;
        return reward - _claimedVeTresrBonusReward;
    }
    ...
    function getJlpSmrtBonusReward(uint time) external view returns (uint) {
        ...
        uint reward = LpStaking(_jlpSmrtAddressContract).getBonusReward(msg.sender, time) * _jlpSmrtBonusReward / 1000;

        if (_claimedJlpSmrtBonusReward >= reward) return 0;
        return reward - _claimedJlpSmrtBonusReward;
    }

    function getJlpSmrtBonusRewardPerSecond() external view returns (uint) {
        require(_jlpSmrtAddressContract != address(0), "RewardUpgradeableV1: invalid address");

        return LpStaking(_jlpSmrtAddressContract).getBonusRewardPerSecond(msg.sender) * _jlpSmrtBonusReward / 1000;
    }
    ...
    function getJlpTresrBonusReward(uint time) external view returns (uint) {
        ...
        uint reward = LpStaking(_jlpTresrAddressContract).getBonusReward(msg.sender, time) * _jlpTresrBonusReward / 1000;

        if (_claimedJlpTresrBonusReward >= reward) return 0;
        return reward - _claimedJlpTresrBonusReward;
    }

    function getJlpTresrBonusRewardPerSecond() external view returns (uint) {
        require(_jlpTresrAddressContract != address(0), "RewardUpgradeableV1: invalid address");

        return LpStaking(_jlpTresrAddressContract).getBonusRewardPerSecond(msg.sender) * _jlpTresrBonusReward / 1000;
    }
...
contract TresrStakingUpgradeableV1 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint) private stakedList;
    mapping(address => Log[]) private stakedLogList;
    ...
    modifier onlyBonusRewardContract() {
        require(_bonusRewardAddressContract == msg.sender, "TresrStaking: caller is not BonusRewardContract");
        _;
    }
    ...
    function getStaked() public view returns (uint) {
        return stakedList[msg.sender];
    }

    function getReward(uint time) public view returns (uint) {
        uint rewardSum = 0;
        uint length = stakedLogList[msg.sender].length;

        if (length == 0) return rewardSum;

        for (uint i = 0; i <= length - 1; i++) {
            if (stakedLogList[msg.sender][i].endDate == 0) {
                if (stakedLogList[msg.sender][i].startDate >= time) continue;

                rewardSum += (time - stakedLogList[msg.sender][i].startDate) * stakedLogList[msg.sender][i].amount * _rewardConst / 100 / 3600;
            } else {
                rewardSum += (stakedLogList[msg.sender][i].endDate - stakedLogList[msg.sender][i].startDate) * stakedLogList[msg.sender][i].amount * _rewardConst / 100 / 3600;
            }
        }
    ...
    function getRewardPerSecond() public view returns (uint) {
        ...
        else return stakedLogList[msg.sender][length - 1].amount * _rewardConst / 100 / 3600;
    }

    function getRewardMax() public view returns (uint) {
        return stakedList[msg.sender] * 100;
    }

    function zeroingLog(address account) private {
        uint length = stakedLogList[account].length;

        if (length > 0) {
            for (uint i = 0; i <= length - 1; i++) {
                stakedLogList[account][i].amount = 0;
            }
        }
    }

    function addLog(address account, uint amount) private {
        uint length = stakedLogList[account].length;

        if (length == 0) {
            stakedLogList[account].push(Log(block.timestamp, 0, amount));
        } else {
            stakedLogList[account][length - 1].endDate = block.timestamp;
            stakedLogList[account].push(Log(block.timestamp, 0, amount));
        }
    }
    ...
    function stake(uint amount) external moreThanZero(amount) {
        ...
        stakedList[msg.sender] += amount;
        _stakedAll += amount;
        addLog(msg.sender, stakedList[msg.sender]);
    }

    function unstake(uint amount) external moreThanZero(amount) {
        require(stakedList[msg.sender] >= amount, "TresrStaking: staked must be more than amount");
        require(stakedLogList[msg.sender].length > 0, "TresrStaking: no logs");

        IERC20Upgradeable(_tokenAddressContract).transfer(msg.sender, amount);

        stakedList[msg.sender] -= amount;
        _stakedAll -= amount;
        zeroingLog(msg.sender);
    }
    ...
    function getVeTresrBonusReward(address account, uint time) public view onlyBonusRewardContract returns (uint) {
        ...
        for (uint i = 0; i <= length - 1; i++) {
            if (stakedLogList[account][i].endDate == 0) {
                if (stakedLogList[account][i].startDate >= time) continue;

                rewardSum += (time - stakedLogList[account][i].startDate) * stakedLogList[account][i].amount * _rewardConst / 100 / 3600;
            } else {
                rewardSum += (stakedLogList[account][i].endDate - stakedLogList[account][i].startDate) * stakedLogList[account][i].amount * _rewardConst / 100 / 3600;
            }
        }
    ...
    function getVeTresrBonusRewardPerSecond(address account) public view onlyBonusRewardContract returns (uint) {
        ...
        if (length == 0) return 0;
        else return stakedLogList[account][length - 1].amount * _rewardConst / 100 / 3600;
    }
}
...
contract NFKeyStakingUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    mapping(uint => uint) private tokenRewardsReleasedList;
    ...
    function initialize(address newNfkeyContractAddress, address newWhitelistContractAddress, address newTresrCoinContractAddress, address newBonusRewardAddressContract) public initializer {
        __Ownable_init();
    ...
    function addLog(uint tokenID) private {
        WhitelistUpgradeableV1 whitelistContract = WhitelistUpgradeableV1(_whitelistContractAddress);

        uint chestTier = getChestTier(tokenID, block.timestamp);
        uint length = tokenLogList[tokenID].length;
        uint level = whitelistContract.getKeyTier(tokenID);
    ...
    function calcUnlockCost(uint tokenID, uint time) public view returns (uint) {
        ...
        uint keyLevel = whitelistContract.getKeyTier(tokenID);
        uint chestTier = getChestTier(tokenID, time);
        uint chestTierDays = 8 - chestTier;

        return chestTierDays * keyLevel * 125 * chestTier * 25 * 10 ** 13;
    }

    function calcBaseProbToOpen(uint tokenID, uint time) public view returns (uint) {
        uint chestTier = getChestTier(tokenID, time);

        if (chestTier == 0) return 0;
        return (1000 - (125000 * chestTier / 1000));
    }
    ...
    function updateTreasureRewardTime(uint tokenID) private {
        ...
        // treasureRewardAvailableList[tokenID] = block.timestamp + (chestTierDays * 24 * 60 * 60); // days
        treasureRewardAvailableList[tokenID] = block.timestamp + (chestTierDays * 60);
    }
    ...
    function stake(uint tokenID) external {
        ...
        if (getChestTier(tokenID, block.timestamp) == 0) setChestTier(tokenID, 1);
    ...
    function openChest(uint tokenID) external {
        ...
        uint probToOpen = calcBaseProbToOpen(tokenID, block.timestamp);
        uint timeToTreasureUnlock = getTimeToTreasureUnlock(tokenID);
        bool answer = false;

        if ((randomCount * 10) < probToOpen) {
            if (block.timestamp < timeToTreasureUnlock) setChestTier(tokenID, chestTier + 1);
            answer = true;
        }
...
contract LpStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 lastStakeTime; // the time at which user staked their tokens
    }
    ...
    constructor(IERC20 _stakingToken, address newBonusRewardAddressContract) {
        _bonusRewardAddressContract = newBonusRewardAddressContract;
        token = _stakingToken;
    ...
    function getBonusReward(address _account, uint _time) public view onlyBonusRewardContract returns (uint) {
        ...
        for (uint i = 0; i <= length - 1; i++) {
            if (logList[_account][i].endDate == 0) {
                ...
                rewardSum += (_time - logList[_account][i].startDate) * logList[_account][i].amount / 3600;
            } else {
                rewardSum += (logList[_account][i].endDate - logList[_account][i].startDate) * logList[_account][i].amount / 3600;
            }
        }
    ...
    function getBonusRewardPerSecond(address _account) public view onlyBonusRewardContract returns (uint) {
        ...
        if (length == 0) return 0;
        else return logList[_account][length - 1].amount / 3600;
    }
}
...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    ...
    mapping(address => bool) private mintedAddressList;
    mapping(uint => string) private ipfsURIList;
    mapping(address => uint) private availableUpgradeTimestampList;
    mapping(address => bool) private adminAddressList;

    function initialize(address newWhitelistContractAddress, address newSmarterCoinContractAddress) initializer public {
        __Ownable_init();
        ...
        ipfsURIList[1] = "https://ipfs.moralis.io:2053/ipfs/QmVmT7nRx9yp6AEUBMyuKJEAx8UowpaGicpabsh8QcGgQA";
        ipfsURIList[2] = "https://ipfs.moralis.io:2053/ipfs/QmcxQKUN1wNaxCvrd7eB4DiftBiWTf4ZPeBhfqe8ZirgcJ";
        ipfsURIList[3] = "https://ipfs.moralis.io:2053/ipfs/QmYJREAafP6qTddDLuRpLQVyJjneWAwqob9uWJPdVyA8aR";
        ipfsURIList[4] = "https://ipfs.moralis.io:2053/ipfs/QmbX1WB8eXzgSLgLtY6ThhJXb1fiSxFfbQL15YYiKeZMGN";
        ipfsURIList[5] = "https://ipfs.moralis.io:2053/ipfs/QmaCK3S1aMi27QUzaE6v9KTM2CU86u9DxhmdBtaTtQ2f4g";
        ipfsURIList[6] = "https://ipfs.moralis.io:2053/ipfs/QmP85sUqSYHDkEoKmcCEWD4JUVozEVvAny5p126XDFs1QR";
        ipfsURIList[7] = "https://ipfs.moralis.io:2053/ipfs/QmQeKUZ7ZP9u2eCtNy5mqkwDX5kSFQncFf6aTU5mSGo1Um";
        ipfsURIList[8] = "https://ipfs.moralis.io:2053/ipfs/QmdUx5L36YkR4DbhHLVyRpk56YRZvKomJFvKUjC49xw6b1";
        ipfsURIList[9] = "https://ipfs.moralis.io:2053/ipfs/QmcoMe6FnBzWzMhrYLD5ucsN21m4v413HaYzHE6GZggkHK";
        ipfsURIList[10] = "https://ipfs.moralis.io:2053/ipfs/QmYkzxAFtNE6NgbwHJM1DFuPNjwK9WYPFW8giStzn3eKB5";
    }
    ...
    function isMinted(address account) public view returns (bool){
        return mintedAddressList[account];
    }

    function getNfkeyStakingContractAddress() public view onlyAdmin returns (address) {
        return _nfkeyStakingContractAddress;
    }

    function setNfkeyStakingContractAddress(address newAddress) public onlyAdmin {
    ...
    function _burn(uint tokenID) internal virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) {
        return super._burn(tokenID);
    }

    function tokenURI(uint tokenID) public view virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) returns (string memory) {
        return ERC721URIStorageUpgradeable.tokenURI(tokenID);
    }

    function _setTokenURI(uint tokenID, string memory tokenUri) internal virtual override(ERC721URIStorageUpgradeable) {
    ...
    function setUpgradeDelay(address account, uint currentTier) internal {
        if (currentTier <= 3) {
            availableUpgradeTimestampList[account] = block.timestamp + 30;
            //availableUpgradeTimestampList[_address] = block.timestamp + (current_tier * 60 * 60);
        } else {
            availableUpgradeTimestampList[account] = block.timestamp + 30;
            //availableUpgradeTimestampList[_address] = block.timestamp + ((current_tier + 8) * 60 * 60);
        }
    }
    ...
    function upgradeKey(uint tokenID) public {
        ...
        uint currentTier = whitelistContract.verifyUser(msg.sender);
        require(currentTier < 10, "You already have the maximum tier level");

        string memory tokenUri = getIPFS(currentTier + 1);
        smarterCoinContract.transferFrom(address(msg.sender), address(this), ((currentTier + 1) ** 2) * _smarterToLevelUp * 10 ** 18);

        bool isStaked = nfkeyStakingContract.isTokenStaked(tokenID);
        if (isStaked == false) {
            ...
            ERC721Upgradeable._burn(tokenID);
            _totalSupply = _totalSupply - 1;
            super._mint(_nfkeyStakingContractAddress, tokenID);
        }

        _setTokenURI(tokenID, tokenUri);
        _totalSupply = _totalSupply + 1;

        WhitelistUpgradeableV1.WhitelistUser memory user = WhitelistUpgradeableV1.WhitelistUser(msg.sender, currentTier + 1);

        whitelistContract.addAddressToWhitelist(user);
        whitelistContract.setKeyTier(tokenID, currentTier + 1);
        nfkeyStakingContract.proxyAddLog(tokenID);
        setUpgradeDelay(msg.sender, currentTier + 1);
    }

    function mintUniqueTokenTo(address _to) public {
        WhitelistUpgradeableV1 whitelistContract = WhitelistUpgradeableV1(_whitelistContractAddress);

        uint tier = whitelistContract.verifyUser(msg.sender);
        require(tier > 0, "Whitelist: You need to be whitelisted");
        require(this.balanceOf(msg.sender) <= 1, "Already have an NFKey");
        require(!mintedAddressList[msg.sender], "Already minted by your address");

        if (tier > 0 && this.balanceOf(msg.sender) <= 1 && !mintedAddressList[msg.sender]) {
            string memory tokenUri = getIPFS(tier);
            uint256 mintIndex = totalSupply();

            mintedAddressList[msg.sender] = true;
            super._mint(_to, mintIndex);
            _setTokenURI(mintIndex, tokenUri);
            _totalSupply = _totalSupply + 1;
            whitelistContract.setKeyTier(mintIndex, tier);
            setUpgradeDelay(msg.sender, tier);
        }
    }
}
...
contract WhitelistUpgradeableV1 is Initializable {
    ...
    mapping(address => bool) private adminAddressList;
    mapping(address => uint) private whitelistedAddressList;
    mapping(uint => uint) private keyTierList;
    ...
    function setKeyTier(uint tokenID, uint tier) public onlyAdmin {
        keyTierList[tokenID] = tier;
    }

    function getKeyTier(uint tokenID) public view returns (uint) {
        return keyTierList[tokenID];
    }

    modifier isWhitelisted(address account) {
        require(whitelistedAddressList[account] > 0, "Whitelist: You need to be whitelisted");
        _;
    }
    ...
    function addAddressToWhitelist(WhitelistUser memory newUser) public onlyAdmin {
        whitelistedAddressList[newUser.whitelistAddress] = newUser.tier;
    }

    function removeAddressFromWhitelist(address account) public onlyAdmin {
        whitelistedAddressList[account] = 0;
    }

    function verifyUser(address _whitelistedAddress) public view returns (uint) {
        return whitelistedAddressList[_whitelistedAddress];
    }
}
