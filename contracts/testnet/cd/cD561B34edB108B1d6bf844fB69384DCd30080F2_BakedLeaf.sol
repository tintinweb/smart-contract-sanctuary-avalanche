...
contract BakedLeaf is Context, Ownable {
    using SafeMath for uint256;

    uint256 private SEEDS_TO_HATCH_1MINERS = 1080000;//for final version should be seconds in a day
    ...
    function getMySeeds(address adr) public view returns(uint256) {
        return SafeMath.add(claimedSeeds[adr],getSeedsSinceLastHatch(adr));
    }
    
    function getSeedsSinceLastHatch(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(SEEDS_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastHatch[adr]));
