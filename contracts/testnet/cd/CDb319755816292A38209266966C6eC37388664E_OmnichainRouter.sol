...
// Omnisea omnichain-router Contracts v0.0.1
...
contract OmnichainRouter is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable {
    ...
    event Redirected(string originChain, string toChain);
    ...
    constructor(address _lzEndpoint, address _axGateway, address _axGasReceiver) NonblockingLzApp(_lzEndpoint) IAxelarExecutable(address(0))  {
        ...
        _owner = msg.sender;
        chainName = "Avalanche"; // https://docs.axelar.dev/dev/chain-names
    ...
    function testInfo(string memory dstChainName, uint gas, address origin) external payable {
        bytes memory fnData = abi.encode(dstChainName);
        bytes memory _payload = abi.encode("Info", fnData);
        call(dstChainName, _payload, gas, origin, false);
    }
    ...
    function call(string memory dstChainName, bytes memory _payload, uint gas, address origin, bool isRedirected) internal {
        Route[] storage routes = chainNameToRoute[dstChainName];
        bool isRedirect = routes.length == 0;
        if (isRedirect) {
            emit Redirect(chainName, dstChainName);
        }
        Route storage route = isRedirect ? chainNameToRoute[redirectChain][0] : routes[0];
        emit Redirect("aaaa", "bbbb");

        if (keccak256(bytes(route.provider)) == keccak256(bytes('lz'))) {
            uint16 chId = route.chainId;
            uint setGas = isRedirect ? 0 : gas;
            bytes memory adapter = _getAdapter(setGas);

            if (isRedirected) {
                (uint messageFee, ) = lzEndpoint.estimateFees(route.chainId, address(this), _payload, false, adapter);
                // TODO: Check if Router is funded by calling UA
                require(address(this).balance >= messageFee, "NOT_FUNDED");
                lzEndpoint.send{value : messageFee}(chId, this.getTrustedRemote(chId), _payload, payable(origin), address(0x0), adapter);
                return;
            }
            emit Redirect("ddd", "eee");
            lzEndpoint.send{value : msg.value}(chId, this.getTrustedRemote(chId), _payload, payable(origin), address(0x0), adapter);
            return;
        }
//
//        if (keccak256(bytes(route.provider)) == keccak256(bytes('ax'))) {
//            string memory dstStringAddress = isRedirect ? axRemotes[redirectChain].toString() : axRemotes[dstChainName].toString();
//
//            if (isRedirected) {
//                axGasReceiver.payNativeGasForContractCall{value: 100000000000000000}(address(this), route.dstChain, dstStringAddress, _payload, origin);
//            } else {
...
//        require(isAxRemote(srcChain, srcAddressString.toAddress()), 'NOT_AX_REMOTE');
    ...
    function handleCall(bytes memory _payload) internal {
        ...
        if (keccak256(bytes(dstChainName)) != keccak256(bytes(chainName))) {
            ...
            call(dstChainName, _payload, gas, origin, true);
