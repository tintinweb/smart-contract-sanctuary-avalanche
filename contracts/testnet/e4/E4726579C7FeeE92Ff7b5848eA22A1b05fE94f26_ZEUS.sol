...
contract ZEUS is Ownable, PaymentSplitter {
    ...
    address tokenAddress = 0x6e4FB24AB7F7f1F1d7BF1821e61dC1703625aAA5;
    ...
    function compoundMultipleNodes(
        uint256[] memory blocktimes,
        uint32 nodeType,
        string memory name
    ) external {
        address sender = _msgSender();
        require(blocktimes.length <= loopAmount, "Maximum of 10 nodes");
        uint256 total = 0;
        ...
        while (total >= nodePrice && count <= loopAmount) {
            total -= nodePrice;
            count++;
        }
        nodeRewardManager.createNode(
