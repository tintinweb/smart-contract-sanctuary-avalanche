...
contract Rental is Withdrawable {
    ...
    mapping(address => mapping(uint256 => RentalInfo)) private _rentalInfo;
    ...
    modifier onlyMaker(address account, uint256 orderId) {
        require(_msgSender() == _rentalInfo[account][orderId].owner, REVERT_NOT_A_MAKER);
        _;
    }
    ...
    modifier isNotRenting(address acount, uint256 orderId) {
        require(_rentalInfo[acount][orderId].renter == address(0), REVERT_ALREADY_RENTING);
        _;
    }
    ...
    function getRentalInfo(address lender, uint256 orderId) external view returns(RentalInfo memory) {
        return _rentalInfo[lender][orderId];
    }

    function rent(address lender, uint256 orderId) external payable isNotRenting(lender, orderId) {
        address account = _msgSender();
        RentalInfo storage rental = _rentalInfo[lender][orderId];
    ...
    function _setListRental(
        ...
        address account = _msgSender();
        require(_rentalInfo[account][orderId].owner == address(0) || _rentalInfo[account][orderId].owner == account, REVERT_NOT_A_OWNER_NFTS);
        require(_rentalInfo[account][orderId].renter == address(0) , REVERT_ALREADY_RENTING);
        _rentalInfo[account][orderId] = RentalInfo(nftAddress, tokenId, account, ownerRewardRate, address(0), paymentToken, price, period, 0);
    ...
    function listRental(
        ...
        uint256 price,
        uint256 period
    ) external isOwnerOf(nftAddress, tokenId) isValidRentingNFT(nftAddress, tokenId) isValidRate(ownerRewardRate) isValidPeriod(period) {
        require(IERC721(nftAddress).isApprovedForAll(_msgSender(), address(this)), REVERT_APPROVE_NFTS);
        _setListRental(block.timestamp, nftAddress, tokenId, ownerRewardRate, paymentToken, price, period);
    }

    function _resetRental(address account, uint256 orderId) private {
        RentalInfo storage rental = _rentalInfo[account][orderId];
    ...
    function removeRental(uint256 orderId) external onlyMaker(_msgSender(), orderId) isNotRenting(_msgSender(), orderId) {
        _resetRental(_msgSender(), orderId);
    }
}