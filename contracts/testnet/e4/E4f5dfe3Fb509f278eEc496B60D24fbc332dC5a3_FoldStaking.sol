...
/**
 * @title Fold Staking Contract
 * @notice Stake FoldToken across four different pools that release hourly rewards
 */
contract FoldStaking is Ownable {
    ...
    mapping (address => mapping(address => uint256)) public LPStakes;
    mapping (address => mapping(address => uint256)) public LPStakeDeposits;
    ...
    /// @notice Internal FoldToken amount for distributing staking reward claims
    IERC20 public foldToken;
    uint256 private constant FOLD_TOKEN_PRECISION = 1e18;
    uint256 private constant LP_TOKEN_PRECISION = 1e18;
    ...
    modifier onlyAuthorised() {
        if(!authorisedContracts[msg.sender]){
    ...
    /**
     * @notice Construct a new FoldTokenStaking instance
     */
    constructor() {}
    ...
    /**
     * @notice Deposit FoldToken to the FoldToken Pool
     * @param _amount Amount in FoldToken
     * @param _lpAddress Address of desired LP token to deposit
     */
    function depositLP(uint256 _amount, address _lpAddress) public {
       ...
       if(allowedLPs[_lpAddress]){
            ...
            // Connect to the LP
            ILP lpPool = ILP(_lpAddress);
            (uint112 _reserve0, uint112 _reserve1,) = lpPool.getReserves();

            // Get the FOLD supply in the LP
            uint256 lpFldSize;
            if(lpPool.token0() == address(foldToken)){
                lpFldSize = _reserve0;
            }
            else if(lpPool.token1() == address(foldToken)){
                lpFldSize = _reserve1;
            }
            ...
            // Calculate share of FLD in LP
            uint256 lpFldValue = lpFldSize * _amount / totalLPSupply; // Amount of FOLD tokens are worth


            uint stakeAmount = (lpFldValue/FOLD_TOKEN_PRECISION) * 10; // LP Bonus 10x
            _deposit(position, stakeAmount);

            IERC20(_lpAddress).transferFrom(msg.sender, address(this), _amount); 
            LPStakes[_lpAddress][msg.sender] += _amount;
            LPStakeDeposits[_lpAddress][msg.sender] += lpFldValue;

            emit Deposit(msg.sender, stakeAmount, msg.sender);
       }else{
        revert("LP Token Not Whitelisted");
       }
     ...
     * @notice Deposit FoldToken to the FoldToken Pool
     * @param _amount Amount in FoldToken
     * @param _recipient Address the deposit is stored for
     */
    function deposit(uint256 _amount, address _recipient) public onlyAuthorised {
        

        updatePool();

       Position storage position = addressPosition[_recipient];
       _deposit(position, _amount);
     ...
     */
    function claimFoldToken(address _recipient) public {
        updatePool();

        Position storage position = addressPosition[msg.sender];
        uint256 rewardsToBeClaimed = _claim(position, _recipient);
    ...
    /**
     * @notice Withdraw staked FoldToken from the FoldToken pool.  Performs an automatic claim as part of the withdraw process.
     * @param _amount Amount of FoldToken
     * @param _lpAddress Address of desired LP token to withdraw
     */
    function withdrawLP(uint256 _amount, address _lpAddress) public {

        // make sure balance of LP for user is >= amount

        updatePool();

        if(allowedLPs[_lpAddress]){

            if (_amount > LPStakes[_lpAddress][msg.sender]){
                revert("Not enough balance for that LP Token");
            }

            Position storage position = addressPosition[msg.sender];

            // Total staking shares for this LP Token
            uint256 totalLPDeposit = LPStakeDeposits[_lpAddress][msg.sender];
        
            // Total LP Tokens for this LP Token
            uint256 totalLPTokens = LPStakes[_lpAddress][msg.sender];

            
            // Calculate share of LP Tokens being withdrawn
            uint256 lpShare = _amount / totalLPTokens;
            uint256 depositedPositionValue = totalLPDeposit * lpShare; // Amount of staking shares this _amount is worth


            // Get the correct stakeAmount at time of deposit!
            uint stakeAmount = (depositedPositionValue/FOLD_TOKEN_PRECISION) * 10; 

            _withdraw(position, stakeAmount); // Withdraw stake value


            // If withdrawing everything, withdraw claim too.
            if (_amount == LPStakes[_lpAddress][msg.sender]) {
                uint256 rewardsToBeClaimed = _claim(position, msg.sender);
                emit ClaimRewards(msg.sender, rewardsToBeClaimed, msg.sender);
            }

            // Transfer the LP tokens back
            IERC20(_lpAddress).transfer(msg.sender, _amount);
            LPStakes[_lpAddress][msg.sender] -= _amount;
            LPStakeDeposits[_lpAddress][msg.sender] -= depositedPositionValue;

            emit Withdraw(msg.sender, _amount, msg.sender);
        }
    }

    function withdraw(uint256 _amount, address _recipient) public onlyAuthorised {
     ...
     * @param _amount Total amount of FoldToken to be distributed over the range
    ...
    function addTimeRange(
        ...
        uint256 _endTimeStamp) external onlyOwner
    {
    ...
    function getTimeRangeBy(uint256 _index) public view returns (TimeRange memory) {
        return pools[0].timeRanges[_index];
    }
     ...
     * @return uint256 The amount of FoldToken rewards to be distributed by pool for a given time range
    ...
    function updatePool() public {
        ...
        (uint256 rewards, uint256 index) = rewardsBy(pool.lastRewardedTimestampHour, previousTimestampHour);
        if (pool.lastRewardsRangeIndex != index) {
            pool.lastRewardsRangeIndex = index.toUint16();
        }
        pool.accumulatedRewardsPerShare = (pool.accumulatedRewardsPerShare + (rewards * FOLD_TOKEN_PRECISION) / pool.stakedAmount).toUint96();
        pool.lastRewardedTimestampHour = previousTimestampHour > lastTimestampHour ? lastTimestampHour : previousTimestampHour;

        emit UpdatePool(pool.lastRewardedTimestampHour, pool.stakedAmount, pool.accumulatedRewardsPerShare);
    }
    ...
    /**
     * @notice Fetches a DashboardStake for the FoldToken pool
     * @return dashboardStake A dashboardStake struct
     * @param _address An Ethereum address
     */
    function getStake(address _address) public view returns (DashboardStake memory) {
        uint256 deposited = addressPosition[_address].stakedAmount;
        uint256 unclaimed = deposited > 0 ? this.pendingRewards(_address) : 0;
        uint256 rewards24Hrs = deposited > 0 ? _estimate24HourRewards(_address) : 0;

        return DashboardStake(deposited, unclaimed, rewards24Hrs);
    }



    function _estimate24HourRewards(address _address) private view returns (uint256) {
        Pool memory pool = pools[0];
    ...
    /**
     * @notice Fetches the current amount of claimable rewards for a given position from the pool.
    ...
    function pendingRewards(address _address) external view returns (uint256) {
        ...
        (uint256 rewardsSinceLastCalculated,) = rewardsBy(pool.lastRewardedTimestampHour, getPreviousTimestampHour());
        ...
        if (block.timestamp > pool.lastRewardedTimestampHour + SECONDS_PER_HOUR && pool.stakedAmount != 0) {
            accumulatedRewardsPerShare = accumulatedRewardsPerShare + rewardsSinceLastCalculated * FOLD_TOKEN_PRECISION / pool.stakedAmount;
        }
        return ((position.stakedAmount * accumulatedRewardsPerShare).toInt256() - position.rewardsDebt).toUint256() / FOLD_TOKEN_PRECISION;
    }
    ...
    // Used when someone deposits LP tokens to the contract only.
    function _deposit(Position storage _position, uint256 _amount) private {
        Pool storage pool = pools[0];
    ...
    // Used to only claim their earned rewards
    ...
    // Used to withdraw LP tokens from the contract only.
    ...
    *
    */
    function setContracts(address _address2652, address _addressFold, address _address91) external onlyOwner {
        
        foldToken = IERC20(_addressFold);
        authorisedContracts[_address2652] = true;
        authorisedContracts[_addressFold] = true;
        authorisedContracts[_address91] = true;
    }

    function isValidLP(address _lpAddress) public view returns (bool isValid){
        return allowedLPs[_lpAddress];
    }

    function addValidLP(address _lpAddress) external onlyOwner {
        allowedLPs[_lpAddress] = true;
        delete addressPosition[_lpAddress];
        updatePool();
    }

    function removeValidLP(address _lpAddress) external onlyOwner {
        allowedLPs[_lpAddress] = false;
    }
