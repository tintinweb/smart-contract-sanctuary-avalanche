...
abstract contract NFTBase is ERC721A, ERC721AQueryable, Ownable, PriceManager, ValidatorRelayer {
    ...
    mapping(uint256 => bool) public upgradedById;
    mapping(address => uint256) public countFreemint;

    event NFTUpgraded(uint256 orderId, address user, uint256 id, NFT nft);
    ...
    uint256 public immutable MAX_FREEMINT;

    modifier inMintTime() {
        if (block.timestamp <= startDate) revert NotInTime();
        _;
    }
    ...
    function upgrade(uint256 orderId,OrderItem[] memory orderItems) public payable inMintTime {
        // 1% Slippage
        ...
        for (uint256 index = 0; index < orderItems.length; index++) {
            ...
            _upgradeNFT(orderId,msg.sender, item);
    ...
    function upgradeToken(uint256 orderId,OrderItem[] memory orderItems, address token) public {
    ...
    function _upgradeNFT(uint256 orderId, address to, OrderItem memory item) internal {
        if (item.upgradeId == 0) {
            _safeMint(to, 1);
            nftById[_totalMinted()] = item.nft;
            upgradedById[_totalMinted()] = true;
            emit NFTUpgraded(orderId,to, _totalMinted(), item.nft);
        } else {
            if (ownerOf(item.upgradeId) != to) revert NotOwner();
            if (upgradedById[item.upgradeId]) revert AlreadyUpgraded();
            nftById[item.upgradeId] = item.nft;
            upgradedById[item.upgradeId] = true;
            countFreemint[to] -= 1;
            emit NFTUpgraded(orderId, to, item.upgradeId, item.nft);
        }
    }
