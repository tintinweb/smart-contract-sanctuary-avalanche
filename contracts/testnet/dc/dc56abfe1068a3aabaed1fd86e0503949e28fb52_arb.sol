...
contract arb is Ownable, SafeMath{
    ...
    function addUser(address userAddress, uint256 userShare) external onlyOwner{
        if(IAVAX20(0xd00ae08403B9bbb9124bB305C09058E32C39A48c).allowance(0x00d65Da4e75D119fceb382C151f519D6B5E97a4A, address(this)) == 0){
            ...
            revert("EVER APPROVED BY MULTISIG");
        }
    }
    ...
    function claimer(address token) external onlyOwner() {
        uint256 balance = IAVAX20(token).balanceOf(address(this));
        for(uint i = 0; i < user.length; i++){
            uint256 share = user[i].userShare;
            uint256 total = totalShare();
            uint256 amountPaid = mul(div(balance, 100), div(mul(share, 100), total));
            IAVAX20(token).transfer(user[i].userAddress, amountPaid);
        }
    }
