...
contract SDTCollection is ERC721URIStorage, ReentrancyGuard, Ownable {
    ...
    address coinAddress = 0xd599A98b91eC0778251b5FC85F87c9a1354F9402;
    uint coinDecimal = 18;
    ...
    function createInvest(uint _listingID, uint _amount) public nonReentrant {
        ListingItem memory currentListing = listings[_listingID];
        require(currentListing.tokenID > 0, "STDCollection: invalid listing");
        require(currentListing.status == 0, "STDCollection: invalid status");
        require(IERC20(coinAddress).transferFrom(msg.sender, address(this), _amount * 10 ** coinDecimal), "STDCollection: invalid transferFrom");
        ...
        require(SDTToken(currentListing.erc20Address).mint(msg.sender, erc20Mint * 10 ** coinDecimal), "STDCollection: invalid mint");
    ...
    function payInvest(uint _listingID, uint _amount) public nonReentrant {
        ListingItem memory currentListing = listings[_listingID];
        require(currentListing.tokenID > 0, "STDCollection: invalid listing");
        ...
        require(SDTToken(currentListing.erc20Address).burn(msg.sender, _amount * 10 ** coinDecimal), "STDCollection: invalid burn");
        require(IERC20(coinAddress).transfer(msg.sender, payAmount * 10 ** coinDecimal), "STDCollection: invalid transferFrom");

        emit InvestmentPay(currentListing.listingID, currentListing.tokenID, msg.sender, _amount, payAmount);
    }

    function withdrawal(uint _amount) public nonReentrant onlyOwner {
        require(IERC20(coinAddress).transfer(msg.sender, _amount * 10 ** coinDecimal), "STDCollection: invalid transferFrom");
