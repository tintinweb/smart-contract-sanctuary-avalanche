...
contract Pyramid {
    ...
    uint256 public MAX_BLOCKS = 460539;
    ...
    event UpdateColor (uint256 indexed _id, bytes _newColor);
    ...
    function buildBlocks (uint256[] memory _blocks, bytes memory _color) payable public {
        ...
        for (uint i; i < _blocks.length; i++) {
			...
			if (lastOwner != msg.sender) { //else it's just a repaint
				...
				require(remainingAmount >= blockPrice, "Not enough money");
                ...
                emit UpdateColor(blockId, _color);
            }
