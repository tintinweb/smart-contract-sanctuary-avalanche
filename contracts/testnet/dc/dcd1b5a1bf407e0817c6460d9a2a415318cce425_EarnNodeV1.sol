...
import "./EarnNodeStorageV1.sol";

contract EarnNodeV1 is EarnNodeStorageV1 {
    ...
    */
    ...
    function initialize() public initializer {
        ...
        EarnCoin = IERC20(0x2E8cC44053361e3487e4660635FDC37684e04217);
        nodeIds = [0, 5, 10, 15, 20, 25];
        
        count = 2;
        communityAddress = 0x25fC99eF8C2bE73c303f7e54A0c2Da423E48326b; 
        RegularNodeCounter = nodeIds[1] + 1;        
    }
    ...
    modifier isBlacklistedAddress() {
        require(
            ...
            "EarnNodeV1: You are blacklisted"
        );
        _;
    }

    /**
     * @dev updates the Earn token address
     *  
     * @param _address address of Earn token
     * Requirements:
     * - only owner can update value.
    */

    function updateEarnCoinAddress(address _address) external onlyOwner returns(bool){
        EarnCoin = IERC20(_address);
        return true;
    }
   ...
   function setTierReward(uint num, uint256[] calldata _tier, uint256[] calldata _nodePrice, uint256[] calldata _nodeAvaxPrice, uint256[] calldata _rate, uint256[] calldata _interval) onlyOwner external returns(bool){
        ...
        for(uint i = 0; i < num; i++){
            Tier storage data = tierDetails[_tier[i]];
    ...
    */
     ...
     * @param _status updates node creation status.
     *
     * Requirements:
     * - only owner can update value.
    */

    function updateCount(uint256 _num, bool _status) external onlyOwner returns(bool){
        count = _num;
        nodeStatus = _status;
        return true;
    }
    ...
    */
     ...
     *  
     * Requirements:
     * - must have nodePrice Earn token in users wallet.
     * 
     * Returns
     * - boolean.
     *
    ...
    */
    ...
    function buyNode(Order memory order, bytes memory signature) isBlacklistedAddress nonReentrant external returns(bool){
        uint256 _tier;
        uint256 nodeId;
        require(owner() == ECDSAUpgradeable.recover(order.key, signature) && nodeStatus == true, "EarnNodeV1: ERROR");
        require(nameDetails[msg.sender][order.NodeName] == false, "EarnNodeV1: Name is already used");
        ...
        require(tierDetails[_tier].rewardRate != 0 && tierDetails[_tier].tierEMnodePrice <= EarnCoin.balanceOf(msg.sender), "EarnNodeV1: Invalid price or Insufficient balance");

        EarnCoin.safeTransfer(msg.sender, communityAddress, tierDetails[_tier].tierEMnodePrice);
        ...
        emit nodeCreated(nodeId, _tier, msg.sender, order.NodeName);

        nameDetails[msg.sender][order.NodeName] = true;
    ...
    */
    ...
    function viewNodeDetails(uint256 _nodeId) public view returns(
        ...
        uint256 number =  (block.timestamp - LastClaimTime) / tierDetails[tierId].intervalTimestamp;
        uint256 rewardAmt = tierDetails[tierId].rewardRate * number;

        RewardToBeClaimed = rewardAmt;
        
        number =  (block.timestamp - NodeCreatedAt) / tierDetails[tierId].intervalTimestamp;
        rewardAmt = tierDetails[tierId].rewardRate * number;

        RewardAlreadyClaimed = rewardAmt - RewardToBeClaimed;
        TotalNodeReward = rewardAmt;
        Number = number;
    }
    ...
    */
    ...
    function claimRewards() isBlacklistedAddress nonReentrant external returns(bool){
        require(ownerNodes[msg.sender].length != 0, "EarnNodeV1: You do not owned any node");
        ...
        for(uint256 i = 0; i < ownerNodes[msg.sender].length; i++){
            ...
            Node storage data = nodeDetails[NodeId];
            data.lastClaimTimestamp = data.lastClaimTimestamp + ( tierDetails[TierId].intervalTimestamp * Number);

        }

        require(totalReward != 0, "EarnNodeV1: Zero reward cannot be claimed");

        EarnCoin.safeTransfer(owner(), msg.sender, totalReward);
...
contract EarnNodeStorageV1 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => Node) internal nodeDetails;
    ...
    mapping(address => mapping(string => bool)) public nameDetails;
    ...
    */
    event nodeCreated(uint256 nodeId, uint256 tierId, address nodeOwner, string nodeName);
