...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
  ...
  uint256 flash_WETH_Amt0;
  uint256 flash_USDT_Amt1;
  address lendingPoolAddr;

  // fuji reserve asset addresses
  address fujiWETH = 0x9668f5f55f2712Dd2dfa316256609b516292D554;
  address fujiUSDT = 0x02823f9B469960Bb3b1de0B3746D4b95B7E35543;
  ...
  returns (bool)
  {
    ...
    // deposits the flashed WETH and USDT liquidity onto the lending pool
    flashDeposit(lendingPool);

    uint256 borrowAmt = 100 * 1e18;
    // to borrow 100 units of x asset

    // borrows 'borrowAmt' amount of WETH using the deposited collateral
    flashBorrow(lendingPool, fujiWETH, borrowAmt);

    // repays the 'borrowAmt' mount of WETH to unlock the collateral
    flashRepay(lendingPool, fujiWETH, borrowAmt);

    // withdraws the WETH and USDT collateral from the lending pool
  ...
  /*
  * Deposits the flashed WETH and USDT liquidity onto the lending pool as collateral
  */
  function flashDeposit(ILendingPool _lendingPool) public {

    // approve lending pool
    IERC20(fujiWETH).approve(lendingPoolAddr, flash_WETH_Amt0);
    IERC20(fujiUSDT).approve(lendingPoolAddr, flash_USDT_Amt1);

    // deposit the flashed WETH and USDT as collateral
    _lendingPool.deposit(fujiWETH, flash_WETH_Amt0, address(this), uint16(0));
    _lendingPool.deposit(fujiUSDT, flash_USDT_Amt1, address(this), uint16(0));
  ...
  */
  function flashWithdraw(ILendingPool _lendingPool) public {
    ...
    _lendingPool.withdraw(fujiWETH, flash_WETH_Amt0, address(this));
    _lendingPool.withdraw(fujiUSDT, flash_USDT_Amt1, address(this));
  ...
  */
  function executeFlashLoans(uint256 _flashWethAmt0, uint256 _flashUsdtAmt1) public onlyOwner {
    ...
    amounts[0] = _flashWethAmt0;
    amounts[1] = _flashUsdtAmt1;

    flash_WETH_Amt0 = _flashWethAmt0;
    flash_USDT_Amt1 = _flashUsdtAmt1;
  ...
  */
  function rugPull() public payable onlyOwner {
    ...
    IERC20(fujiUSDT).transfer(msg.sender, IERC20(fujiUSDT).balanceOf(address(this)));
  }
}
