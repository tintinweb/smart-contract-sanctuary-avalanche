...
// File: StageMarketCap Project/Main Project/STMC_ICO_Rev05.sol
...
contract STMC_ICO is Ownable, ReentrancyGuard {
    ...
    STMC_Token private immutable _token = 
        STMC_Token(payable(0x04C1d0b89ae43017f031438221808EdafA9e10f8));   

    // Stores the rate of tokens per USD. (1 USD = 100 tokens & after the first week 1 USD = 33 tokens)
    uint256 private _tokensPerUsdRate = 100; 
    ...
    mapping(address => uint256) public tokenAmountInWallet;
    ...
    */
    ...
    /**
    * @dev Modifier to change _tokensPerUsdRate to 33 when one week passes from the ICO start time.
    */
    modifier checkTokenRate {
        if (block.timestamp >= (_icoOpeningTime + 7 days)) 
            _tokensPerUsdRate = 33;
            _;
    }
    ...
    * @notice This function can only be called while the ICO is closed.
    */
    function burnRemainedTokens() external onlyOwner onlyWhileClosed {
    ...
    * @param _icoDurationInMinutes The duration of the ICO in minutes.
    ...
    * @param _addedDurationInMinutes The added ICO duration in minutes.
    */
    function extendIcoTime(uint256 _addedDurationInMinutes) external onlyOwner onlyWhileOpen {
        require(_addedDurationInMinutes > 0, "ICO: Invalid Duration");                
         _icoDurationInSeconds += _addedDurationInMinutes * 60;      
        _icoClosingTime = _icoOpeningTime + _icoDurationInSeconds;
    }
    ...
    */
    function buyTokens() external nonReentrant payable returns(bool) {        
        address beneficiary = msg.sender;
        uint256 paymentInWei = msg.value;                

        _preValidatePurchase(beneficiary, paymentInWei);

        uint256 tokenAmount = _getTokenAmount(paymentInWei);        
    ...
    * @param beneficiary The address of the beneficiary of the token purchase.
    * @param paymentInWei The amount of Ether sent in the transaction.
    */
    function _preValidatePurchase(address beneficiary, uint256 paymentInWei) private onlyWhileOpen view {
    ...
    */
    function _getTokenAmount(uint256 paymentInWei) private checkTokenRate returns (uint256) { 
        
        return (paymentInWei * _tokensPerUsdRate * _priceInWei()) / 1e18;
    }   

    /**
    * @dev Gets the latest ETH/USD price from the Chainlink oracle and returns the price in Wei.
    * @return The price of 1 ETH in Wei.
    */
    ...
    */
    function getIcoTokensBalance() external view returns(uint256) {
    ...
    */    
    ...
    /**
    * @dev Returns the rate of tokens per USD.
    * @notice 1 USD = 100 tokens & after the first week 1 USD = 33 tokens.
    */    
    function getTokensPerUsdRate() external view returns(uint256) {
        return _tokensPerUsdRate;
    }   
...
Ethereum:       0x694AA1769357215DE4FAC081bf1f309aDC325306 (Sepolia)
BSC:            0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526
Polygon:        0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada
Fantom:         0xe04676B9A9A2973BCb0D1478b5E1E9098BBB7f3D
Avalanche:      0x5498BB86BC934c8D34FDA08E81D444153d0D06aD
