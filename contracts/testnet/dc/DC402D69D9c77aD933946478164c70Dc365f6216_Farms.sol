...
import "../interfaces/mocks/IesREWARD.sol";
...
contract Farms is ReentrancyGuard, Ownable, Pausable {
  ...
  uint256 private totalFarms;
  // Address for Yield Booster plugin for multiplier
  address public yieldBooster;
  // TODO: Temp multiplier
  uint256 public tempMultiplier = 1e18;
  // Contract address for the reward token
  address public rewardToken;
  // Contract address for the esreward token
  address public esRewardToken;

  /* ========== CONSTANTS ========== */

  uint256 private constant SAFE_MULTIPLIER = 1e18;
  uint256 private constant MAX_REWARDS_SPLIT = 100 * 1e18; // 100%
  ...
  struct Farm {
    ...
    uint256 totalStaked;
    // Total amount of points allocated in this farm
    uint256 totalPoints;
    // Total amount of reward tokens deposited in this farm
    uint256 totalReward;
    // % split of total rewards to be given in esRewards; 100% = 1e18
    uint256 esRewardsSplit;
  ...
  struct Position {
    // Amount of tokens staked by user in a farm
    ...
    // Amount of points allocated by user in a farm
  ...
  event Withdraw(uint256 indexed farmId, address indexed user, address token, uint256 amount);
  event Unlock(uint256 indexed farmId, address indexed user);
  ...
  event UpdateRewardDistributionRate(uint256 indexed farmId, uint256 _rate);
  event UpdateEsRewardSplit(uint256 indexed farmId, uint256 _esRewardsSplit);
  // event UpdateUnlockTime(uint256 indexed farmId, uint256 unlockTime);
  // event UpdateRelockTime(uint256 indexed farmId, uint256 relockTime);
  event UpdateYieldBooster(address indexed caller, uint256 timestamp);

  /* ========== CONSTRUCTOR ========== */

  constructor(address _rewardToken, address _esRewardToken) {
    rewardToken = _rewardToken;
    esRewardToken = _esRewardToken;

    IERC20(rewardToken).approve(esRewardToken, type(uint256).max);
  }
  ...
  /**
  * Modifier to check if farm is active
  ...
  */
  modifier whenNotLocked(uint256 _id, address _user) {
    ...
    // if (farms[_id].unlockTime != 0) {
    //   require(
    //     positions[_id][_user].unlockTime != 0,
    //     "Activate unlock for withdrawal and redemption window first"
    //   );
    //   require(
    //     block.timestamp > positions[_id][_user].unlockTime,
    //     "Unlock time still in progress"
    //   );
    //   require(
    //     block.timestamp < positions[_id][_user].relockTime,
  ...
  */
  function currentRewardPerStakedToken(uint256 _id) private view returns (uint256) {
    ...
    if (farm.totalStaked <= 0) {
      return farm.rewardDistributionRate;
    }

    return (block.timestamp - farm.lastUpdatedAt)
      * farm.rewardDistributionRate
      * SAFE_MULTIPLIER
      / farm.totalStaked
      + farm.rewardPerStakedToken;
  }
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view onlyActive(_id) returns (uint256) {
    Position storage position = positions[_id][_user];

    return (position.stakedAmount * currentRewardPerStakedToken(_id)
      * getRewardMultiplier() // TODO: mock multiplier for now
      / SAFE_MULTIPLIER // TODO: mock multiplier for now
      - position.rewardDebt)
      // + position.rewardAccrued)
      / SAFE_MULTIPLIER;
  }

  /**
  * Returns the reward multiplier of a user's position
  ...
  */
  function _updateFarm(uint256 _id) internal {
    ...
    if (farm.totalStaked > 0) {
      farm.rewardPerStakedToken = (block.timestamp - farm.lastUpdatedAt)
        * farm.rewardDistributionRate
        * SAFE_MULTIPLIER
        / farm.totalStaked
        + farm.rewardPerStakedToken;
    ...
    farm.lastUpdatedAt = block.timestamp;
  ...
  */
  function _updateUserRewardDebt(uint256 _id, address _user, uint256 _amount) private {
    Position storage position = positions[_id][_user];

    position.rewardDebt = position.rewardDebt
      + (_amount * farms[_id].rewardPerStakedToken);
  }
  ...
  * @param _user  Address of a user
  */
  // function _updateUserRewardAccrued(uint256 _id, address _user) private {
  //   require(_user != address(0), "Invalid address");

  //   Position memory position = positions[_id][_user];

  //   position.rewardAccrued = position.rewardAccrued
  //     + (position.stakedAmount
  //     * farms[_id].rewardPerStakedToken
  //     - position.rewardDebt);

  //   position.rewardDebt = 0;

  //   positions[_id][_user] = position;
  // }
  ...
  */
  function stake(uint256 _id, uint256 _amount) external nonReentrant whenNotPaused onlyActive(_id) {
  ...
  */
    ...
    whenNotLocked(_id, msg.sender)
  {
    ...
    require(position.stakedAmount >= _amount, "Cannot withdraw more than staked");

    _updateFarm(_id);
    // _updateUserRewardAccrued(_id, msg.sender);
    position.rewardDebt = 0;

    position.stakedAmount = position.stakedAmount - _amount;
    _updateUserRewardDebt(_id, msg.sender, position.stakedAmount);

    Farm storage farm = farms[_id];
    farm.totalStaked = farm.totalStaked - _amount;
    IERC20(farm.stakedToken).safeTransfer(msg.sender, _amount);

    emit Withdraw(_id, msg.sender, farm.stakedToken, _amount);
  }
  ...
  * @param _id  Unique id of farm
  */
  function claim(uint256 _id) public nonReentrant whenNotPaused onlyActive(_id) whenNotLocked(_id, msg.sender) {
    uint256 rewards = rewardsEarned(_id, msg.sender);
    Farm storage farm = farms[_id];

    require(farm.totalReward >= rewards, "Rewards deposited in farm less than rewards claimable");
    require(rewards > 0, "No rewards to claim");

    Position storage position = positions[_id][msg.sender];

    // position.rewardAccrued = 0;
    position.rewardRedeemed = position.rewardRedeemed + rewards;
    positions[_id][msg.sender] = position;

    farm.totalReward -= rewards;
    // Reset user's rewards debt, similar to as if user has just withdrawn and restake all
    // TODO: to check this reward debt section..
    position.rewardDebt = position.stakedAmount * currentRewardPerStakedToken(_id);

    // TODO: esRewardsSplitCheck
    if (farm.esRewardsSplit > 0) {
      uint256 esRewards = rewards * farm.esRewardsSplit / SAFE_MULTIPLIER;
      uint256 nonesRewards = rewards - esRewards;

      IERC20(rewardToken).safeTransfer(msg.sender, nonesRewards);
      IesREWARD(esRewardToken).stake(address(this), esRewards);
      IERC20(esRewardToken).safeTransfer(msg.sender, esRewards);
    } else {
      IERC20(rewardToken).safeTransfer(msg.sender, rewards);
    }

    emit Claim(_id, msg.sender, rewardToken, rewards);
  }
  ...
  * @param _id  Unique id of farm
  */
  // function activateUnlockTime(uint256 _id) external whenNotPaused onlyActive(_id) {
  //   Position memory position = positions[_id][msg.sender];
  //   require(
  //     position.unlockTime < block.timestamp,
  //     "Unlock time still in progress"
  //   );

  //   position.unlockTime = block.timestamp + farms[_id].unlockTime * 1 seconds;
  //   position.relockTime = block.timestamp + (farms[_id].unlockTime + farms[_id].relockTime) * 1 seconds;
  ...
  * @param _esRewardsSplit  % split of total rewards to be given in esRewards
  */
  function createFarm(
    bool _active,
    address _stakedToken,
    // address _rewardToken,
    // address _esRewardToken,
    uint256 _rewardDistributionRate,
    uint256 _esRewardsSplit
    // uint256 _unlockTime,
    // uint256 _relockTime
  ) external onlyOwner {
    Farm memory farm = Farm({
      ...
      esRewardsSplit: _esRewardsSplit,
  ...
  */
  function depositRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner onlyActive(_id) {
    require(_amount > 0, "Cannot deposit 0 amount");

    Farm storage farm = farms[_id];
    IERC20(rewardToken).safeTransferFrom(msg.sender, address(this), _amount);
    farm.totalReward += _amount;
  }
  ...
  * @param _amount  Amount of reward tokens to deposit; in reward token's decimals
  */
  function withdrawRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner onlyActive(_id) {
    Farm storage farm = farms[_id];
    require(_amount > 0, "Cannot withdraw 0 amount");
    require(farm.totalReward > 0, "Cannot withdraw when farm has no reward tokens deposited");
    require(_amount <= farm.totalReward, "Cannot withdraw more reward tokens than deposited in farm");
  ...
  */
  function updateActive(uint256 _id, bool _active) external onlyOwner onlyActive(_id) {
  ...
  */
  function updateRewardDistributionRate(uint256 _id, uint256 _rate) external onlyOwner onlyActive(_id) {
    farms[_id].rewardDistributionRate = _rate;
    _updateFarm(_id);
    emit UpdateRewardDistributionRate(_id, _rate);
  }

  /**
  * Update the rewards split % of total rewards to esRewards
  * @param _id  Unique id of farm
  * @param _esRewardsSplit  Rewards split % of total rewards to esRewards
  */
  function updateEsRewardsSplit(uint256 _id, uint256 _esRewardsSplit) external onlyOwner onlyActive(_id) {
    farms[_id].esRewardsSplit = _esRewardsSplit;
    _updateFarm(_id);
    emit UpdateEsRewardSplit(_id, _esRewardsSplit);
  }
  ...
  // */
  // function updateUnlockTime(uint256 _id, uint256 _seconds) external onlyOwner onlyActive(_id) {
  //   farms[_id].unlockTime = _seconds;
  //   emit UpdateUnlockTime(_id, _seconds);
  // }
  ...
  // */
  // function updateRelockTime(uint256 _id, uint256 _seconds) external onlyOwner onlyActive(_id) {
  //   farms[_id].relockTime = _seconds;
  //   emit UpdateRelockTime(_id, _seconds);
  // }
  ...
  */
  ...
  // TODO TEMP
  function _updateTempMultiplier(uint256 _tempMultiplier) external onlyOwner {
...
import "../../../contracts/interfaces/mocks/IesREWARD.sol";

abstract contract $IesREWARD is IesREWARD {
...
import "../../../contracts/interfaces/plugins/IYieldBooster.sol";

abstract contract $IYieldBooster is IYieldBooster {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../contracts/staking/Farms.sol";

contract $Farms is Farms {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _rewardToken, address _esRewardToken) Farms(_rewardToken, _esRewardToken) {}

    function $_updateFarm(uint256 _id) external {
        super._updateFarm(_id);
    }
