    ...
    CommonModifiers
{
    ...
    function seize(
        ...
        if (!ecc.preProcessingValidation(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0600, params, metadata));
            return;
        }

        if (!ecc.flagMsgValidated(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0601, params, metadata));
            return;
        }

        if (accountTokens[params.borrower] < params.seizeTokens) {
            // TODO: This should probably be handled more gracefully with a fallback to master
            // to mint back the PUSD the liquidator burned...
            emit MessageFailed(abi.encode(0x0602, params, metadata));
            return;
        }
    ...
    function completeRedeem(
        ...
        if (!ecc.preProcessingValidation(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0200, params, metadata));
            return;
        }

        if (!ecc.flagMsgValidated(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0201, params, metadata));
            return;
        }
        ...
        */
        if (_totalSupply < params.redeemAmount) {
            emit MessageFailed(abi.encode(0x0202, params, metadata));
            return;
        }

        if (accountTokens[params.user] < params.redeemAmount) {
            emit MessageFailed(abi.encode(0x0203, params, metadata));
            return;
        }

        if (_getCashPrior() < params.redeemAmount) {
            emit MessageFailed(abi.encode(0x0204, params, metadata));
            return;
        }

        // TODO: make sure we cannot exploit this by having an exchange rate
        // delta in redeem and complete redeem functions
    ...
    function completeTransfer(
        ...
        if (!ecc.preProcessingValidation(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0800, params, metadata));
            return;
        }

        if (!ecc.flagMsgValidated(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0801, params, metadata));
            return;
        }
        ...
        if (startingAllowance < params.tokens) {
            console.log("reverting");
            console.logBytes(abi.encode(0x0802, params, metadata));
            emit MessageFailed(abi.encode(0x0802, params, metadata));
            return;
        }

        if (accountTokens[params.src] < params.tokens) {
            emit MessageFailed(abi.encode(0x0803, params, metadata));
            return;
        }
