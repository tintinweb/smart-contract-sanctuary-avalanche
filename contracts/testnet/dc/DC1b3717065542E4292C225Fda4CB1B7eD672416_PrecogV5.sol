...
contract PrecogV5 is IPrecogV5 {
    ...
    function _updateCurrentTradingCycle(
        ...
        uint48 _duration;
        uint48 _tradingCycleDuration = core.getCycleConfiguration().tradingCycle;
    ...
    function removeLiquidityPool(address token) external override onlyAdmin _isExistingToken(token) {
    ...
    function takeInvestment(address token) external override onlyMiddleware _isExistingToken(token) {
        IERC20(token).safeTransfer(msg.sender, IERC20(token).balanceOf(address(this)) - liquidity[token] / 10);
    }
    ...
    function _buyPCOG(
        ...
        uint _feeTrading = (_profitAmount * core.getFeeConfiguration().tradingFee) / 10**core.feeDecimalBase();
            _tryTransfer(_token, address(core), _feeTrading);
            if (IERC20(_token).allowance(address(this), address(middlewareExchange)) < _profitAmount) {
                _tryAppove(_token, address(middlewareExchange));
            }
        ...
        _isExistingToken(token)
    {
        ...
        _updateCurrentTradingCycle(token, false, 0);
        _tryTransferFrom(token, msg.sender, address(this), profitAmount);
        ...
        returns (Cycle memory _currentTradingCycle)
    {
        ...
        while (uint48(_timestamp) >= _lastTradingCycle.endTime) {
            ...
            if (_lastTradingCycle.endTime < _tradingApplyTime) {
                _newCycleEndTime = _newCycleStartTime + _lastTradingCycle.endTime - _lastTradingCycle.startTime;
            } else {
                _newCycleEndTime = _newCycleStartTime + _tradingCycle;
            }
            _lastTradingCycle = (
    ...
    function _increaseInvestment(
        ...
        if (_investmentsOf.length > 0) {
            ...
            if (_lastInvestmentOf.idChanged < _futureTradingCycle.id) {
                investmentsOf[_token][_account].push(Investment(_lastInvestmentOf.amount + _amount, 
                    (_futureTradingCycle.endTime - _futureTradingCycle.startTime) + _unit,
                    _nextFundingTime,
                    _futureTradingCycle.id));
            } else {
                investmentsOf[_token][_account][_investmentsOf.length - 1] = Investment(
            ...
            investmentsOf[_token][_account].push(Investment(
                _amount,
                _unit,
                _nextFundingTime,
                _futureTradingCycle.id
    ...
    function _decreaseInvestment(
        ...
        require(!isBeforeFundingTime(_token, _account), "5");
        Investment[] memory _investmentsOf = investmentsOf[_token][_account];
        ...
        Investment memory _lastInvestmentOf = _investmentsOf[_investmentsOf.length - 1];
        remainingAmount = _lastInvestmentOf.amount - _amount;
        ...
        if (_lastInvestmentOf.idChanged < _futureTradingCycle.id) {
            investmentsOf[_token][_account].push(Investment(
            _lastInvestmentOf.amount - _amount,
            _lastInvestmentOf.amount * (_futureTradingCycle.endTime - _futureTradingCycle.startTime) - _unit,
    ...
    function deposit(address token, uint amount) external override {
        require(amount >= core.minFunding(token), "1");
        _updateCurrentTradingCycle(token, false, 0);
        ...
        _tryTransferFrom(token, msg.sender, address(this), actualDepositAmount);
        _tryTransferFrom(token, msg.sender, address(core), feeDeposit);
    ...
    function takeProfit(address to, address token) external override _isExistingToken(token) {
        ...
        _tryTransfer(core.PCOG(), to, profitOfAccount);
    ...
    function _withdrawBeforeFundingTime(
        ...
        require(availableWithdrawalAmount >= _amount && IERC20(_token).balanceOf(address(this)) >= _amount, "4");
        uint remainingAmount = _decreaseInvestmentWhenWithdraw(_token, _account, _amount);
        require(
            remainingAmount == 0 ||
                (remainingAmount >= core.minFunding(_token) && _amount >= core.minDefunding(_token)), "3");
        _tryTransfer(_token, _to, _amount - _fee);
        _tryTransfer(_token, address(core), _fee);   
    }
    ...
    function _tryTransfer(address _token, address _to, uint _amount) internal {
        IERC20(_token).safeTransfer(_to, _amount);
    }

    function _tryTransferFrom(address _token, address _from, address _to, uint _amount) internal {
        IERC20(_token).safeTransferFrom(_from, _to, _amount);
    }

    function _tryAppove(address _token, address _to) internal {
        IERC20(_token).safeApprove(_to, 2**255);
    }
