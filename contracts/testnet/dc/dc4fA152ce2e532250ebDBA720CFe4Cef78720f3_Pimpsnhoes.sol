...
contract Pimpsnhoes is IPimps, ERC721Enumerable, Ownable, Pauseable {
    ...
    // reference to the Bank for choosing random pimp thieves
    IBank public bank;
   ...
   */
    function selectRecipient(uint256 seed) internal view returns (address) {
        ...
        address hoe = bank.randomPimp(seed >> 144);
...
contract Bank is Ownable, IERC721Receiver, Pauseable {
    ...
    // wolves take a 20% tax on all $BANDS claimed
    ...
    // number of Hoe staked in the Bank
   ...
   */
    ...
    /**
     * adds Hoe and pimps to the Bank and Pack
   ...
   */
    function addManyToBankAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isHoe(tokenIds[i]))
                _addHoeToBank(account, tokenIds[i]);
            else
    ...
    /**
     * adds a single Hoe to the Bank
     * @param account the address of the staker
   * @param tokenId the ID of the Hoe to add to the Bank
   */
    function _addHoeToBank(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
        bank[tokenId] = Stake({
        owner : account,
   ...
   */
    ...
    /**
     * realize $BANDS earnings and optionally unstake tokens from the Bank / Pack
   ...
   */
    function claimManyFromBankAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused _updateEarnings {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (isHoe(tokenIds[i]))
                owed += _claimHoeFromBank(tokenIds[i], unstake);
            else
   ...
   */
    function _claimHoeFromBank(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
   ...
   */
    function _paypimpTax(uint256 amount) internal {
        if (totalpimpStaked == 0) {// if there's no staked wolves
            unaccountedRewards += amount;
            // keep track of $BANDS due to wolves
            return;
        }
