...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable napaToken;
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        napaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        napaToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $napa leaderboard threshold
     * @param _bidLimit: minimal $napa committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $napa for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(napaToken), "Recover: Cannot be napa token");
