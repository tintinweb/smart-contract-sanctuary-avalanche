...
contract DrawPool is ERC721, Ownable {
    ...
    ERC721(_name, _symbol) {
        ...
        _feeWallet = 0x054209B79a64E7618EB4092841a7363E8Eb35DA4; // test tres
        _tokenURI;
    }
    ...
    function claimPrize(uint256 _ticketNumber) public {
        require(
            drawCount == ticketNumber - 1,
            "Can't claim prize before all draw !!"
        );
        require(
        ...
        // Split the prize between the msg.sender and the fee wallet
        uint256 prizeAmount = ticketBuyToken.balanceOf(address(this));
        uint256 feeAmount = prizeAmount / 10; // 10% fee
        uint256 winnerAmount = prizeAmount - feeAmount;

        ticketBuyToken.transfer(msg.sender, winnerAmount);
        ticketBuyToken.transfer(_feeWallet, feeAmount);
