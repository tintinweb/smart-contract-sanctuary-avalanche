...
import "./interface/IERC721Salvor.sol";

contract MarketplaceV1 is Initializable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint32 public maximumMinPricePercentageForDutch;
    uint32 public defaultAuctionBidPeriod;

    // events
    event Cancel(bytes32 hash, address maker);
    ...
    function fund() external payable paymentAccepted {
        balances[msg.sender] = balances[msg.sender].add(msg.value);
    }

    function withdraw(uint256 _amount) external nonReentrant priceGreaterThanZero(_amount) {
        uint256 existingBalance = balances[msg.sender];
        require(existingBalance >= _amount, "Balance is insufficient for a withdrawal");
        require(existingBalance.sub(_amount) >= offerTotalAmounts[msg.sender], "can't withdraw, have actively offers");
    ...
    isNotCancelled(LibOrder.hashKey(order))
    {
        ...
        address existingNftOwner = IERC721Salvor(order.maker).ownerOf(order.tokenId);
        require(existingNftOwner != address(0), "nftContract and tokenId don't match");

        if (order.isOffer) {
            offers[order.maker][order.tokenId][msg.sender] = 0;
            offerTotalAmounts[msg.sender] = offerTotalAmounts[msg.sender].sub(order.price);
        }

        fills[orderKeyHash] = true;
    }

    function make(LibOrder.Order memory order, bytes memory signature)
    external
    payable
    whenNotPaused
    nonReentrant
    onlySigner(_validate(order, signature))
    isNotCancelled(LibOrder.hashKey(order))
    priceGreaterThanZero(order.price)
    {
        address existingNftOwner = IERC721Salvor(order.maker).ownerOf(order.tokenId);
        require(existingNftOwner != address(0), "nftContract and tokenId don't match");
        balances[msg.sender] = balances[msg.sender].add(msg.value);
        if (order.isOffer) {
            ...
            require(offers[order.maker][order.tokenId][msg.sender] == 0, "you have already an offer");

            require(balances[msg.sender] >= order.price, "Insufficient funds to make an offer");

            offers[order.maker][order.tokenId][msg.sender] = order.price;
            offerTotalAmounts[msg.sender] = offerTotalAmounts[msg.sender].add(order.price);
        }
    }

    function _validate(LibOrder.Order memory order, bytes memory signature) internal view returns (address) {
        bytes32 hash = LibOrder.hash(order);
        return _hashTypedDataV4(hash).recover(signature);
    }

    function redeem(LibOrder.Order calldata order, bytes memory signature)
    external
    payable
    whenNotPaused
    nonReentrant
    ...
    returns (uint256)
    {
        ...
        // buyer or seller can send amount to their balance in redeem action.
        balances[msg.sender] = balances[msg.sender].add(msg.value);
        ...
        if (order.isOffer) {
            ...
            require(tokenId > 0, "cannot redeem the coupon to make an offer (need to mint first)");
            require(offers[order.maker][tokenId][signer] == order.price, "there is no active offer coupon to redeem");
            buyer = signer;
            seller = sender;
            offers[order.maker][tokenId][signer] = 0;
        } else {
            buyer = sender;
            seller = signer;
            if (order.tokenId > 0) {
                ...
                tokenId = IERC721Salvor(order.maker).mintFromMarketplace(order.tokenUri);
            }
        }
        require(IERC721Salvor(order.maker).ownerOf(tokenId) == seller, "cannot redeem the coupon, seller has not the nft");
        // make sure that the redeemer is paying enough to cover the buyer's cost
        require(balances[buyer] >= order.price, "Insufficient funds to redeem");
        balances[buyer] = balances[buyer].sub(order.price);
        offerTotalAmounts[buyer] = offerTotalAmounts[buyer].sub(order.price);

        IERC721Salvor(order.maker).safeTransferFrom(seller, buyer, tokenId);
        (address royaltyReceiver, uint256 royaltyAmount) = IERC721Salvor(order.maker).royaltyInfo(tokenId, order.price);
        if (royaltyReceiver != seller) {
            payable(royaltyReceiver).transfer(royaltyAmount);
            seller.transfer(order.price.sub(royaltyAmount));
        } else {
            seller.transfer(order.price);
        }
        return tokenId;
    }
    ...
    function createDutchAuction(address _nftContractAddress, uint256 _tokenId, uint64 _duration, uint64 _dropInterval, uint128 _startPrice, uint128 _endPrice)
    external
    isAuctionNotStartedByOwner(_nftContractAddress, _tokenId, true)
    startPriceDoesNotExceedLimit(_startPrice, _endPrice)
    priceGreaterThanZero(_startPrice)
    {
        dutchAuctions[_nftContractAddress][_tokenId].startTime = block.timestamp; // ileriki tarih verebilsin
        ...
        dutchAuctions[_nftContractAddress][_tokenId].nftSeller = msg.sender;

        _transferNftToAuctionContract(_nftContractAddress, _tokenId, true);
    }
    ...
    paymentAccepted
    {
        require(msg.sender != dutchAuctions[_nftContractAddress][_tokenId].nftSeller, "Owner cannot bid on own NFT");
        uint256 amount = getDutchPrice(_nftContractAddress, _tokenId);
        require(msg.value >= amount, "Insufficient payment");
        address _nftSeller = dutchAuctions[_nftContractAddress][_tokenId].nftSeller;

        // if higher than expected then what we will do?
        payable(_nftSeller).transfer(amount);

        IERC721Salvor(_nftContractAddress).safeTransferFrom(address(this), msg.sender, _tokenId);

        _resetDutchAuction(_nftContractAddress, _tokenId);
    }

    // returns the current dutch price by calculating the steps
    // startTime gelmeden satış gerçekleşebilir
    function getDutchPrice(address _nftContractAddress, uint256 _tokenId) public view returns (uint256) {
        DutchAuction memory dutchAuction = dutchAuctions[_nftContractAddress][_tokenId];
        if (block.timestamp < dutchAuction.startTime) {
            return dutchAuction.startPrice;
        }

        if ((block.timestamp - dutchAuction.startTime) >= dutchAuction.duration) {
            return dutchAuction.endPrice;
        } else {
            uint256 dropsPerStep = (dutchAuction.startPrice - dutchAuction.endPrice) / (dutchAuction.duration / dutchAuction.dropInterval);
            uint256 steps = (block.timestamp - dutchAuction.startTime) / dutchAuction.dropInterval;
            return dutchAuction.startPrice - (steps * dropsPerStep);
        }
    }
    ...
    function _resetDutchAuction(address _nftContractAddress, uint256 _tokenId) internal {
        ...
        dutchAuctions[_nftContractAddress][_tokenId].nftSeller = address(0);
    }

    /*
     * The maximum endPrice(if set) must be 20% of the startPrice.
     */
    modifier startPriceDoesNotExceedLimit(uint128 _startPrice, uint128 _endPrice) {
        require(_endPrice == 0 || _getPortionOfBid(_startPrice, maximumMinPricePercentageForDutch) >=_endPrice, "x% of startPrice >= endPrice");
        _;
    }

    // If there is no any bid then the owner of nft can update the minimum price
    function updateMinimumPrice(address _nftContractAddress, uint256 _tokenId, uint128 _newMinPrice)
    external
    onlyNftSeller(_nftContractAddress, _tokenId)
    bidNotMade(_nftContractAddress, _tokenId)
    priceGreaterThanZero(_newMinPrice)
    minPriceDoesNotExceedLimit(auctions[_nftContractAddress][_tokenId].buyNowPrice, _newMinPrice)
    {
    ...
    // If there is no any bid then the owner of nft can update the buyNow price
    function updateBuyNowPrice(address _nftContractAddress, uint256 _tokenId, uint128 _newBuyNowPrice)
    external
    onlyNftSeller(_nftContractAddress, _tokenId)
    bidNotMade(_nftContractAddress, _tokenId)
    priceGreaterThanZero(_newBuyNowPrice)
    minPriceDoesNotExceedLimit(_newBuyNowPrice, auctions[_nftContractAddress][_tokenId].minPrice)
    {
    ...
    // When the auction is complete, there is only one thing left to do, which is to "settle" the auction,
    ...
    function _reversePreviousBidAndUpdateHighestBid(address _nftContractAddress, uint256 _tokenId) internal {
        address prevNftHighestBidder = auctions[_nftContractAddress][_tokenId].nftHighestBidder;
        uint256 prevNftHighestBid = auctions[_nftContractAddress][_tokenId].nftHighestBid;

        _updateHighestBid(_nftContractAddress, _tokenId);

        if (prevNftHighestBidder != address(0)) {
            payable(prevNftHighestBidder).transfer(prevNftHighestBid);
        }
    }
     ...
     ******************************************************************/
    function _updateHighestBid(address _nftContractAddress, uint256 _tokenId) internal {
        auctions[_nftContractAddress][_tokenId].nftHighestBid = uint128(msg.value);
    ...
    function createNftAuction(address _nftContractAddress, uint256 _tokenId, uint128 _minPrice, uint128 _buyNowPrice)
    external
    isAuctionNotStartedByOwner(_nftContractAddress, _tokenId, false)
    ...
    priceGreaterThanZero(_minPrice)
    {
        ...
        _transferNftToAuctionContract(_nftContractAddress, _tokenId, false);
    }

    // sale if the buyNowPrice is met or set auction period if 5 minutes near the end of auction.
    ...
    function _updateAuctionEnd(address _nftContractAddress, uint256 _tokenId) internal {
        //the auction end is always set to auction end + the bid period
        auctions[_nftContractAddress][_tokenId].auctionEnd += defaultAuctionBidPeriod;
    }

    // check the auction end is higher than now + defaultAuctionBidPeriod
    function _isAuctionCloseToEnd(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        return auctions[_nftContractAddress][_tokenId].auctionEnd > (uint64(block.timestamp) + defaultAuctionBidPeriod);
    }
    ...
    function _isBuyNowPriceMet(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        ...
        return buyNowPrice > 0 && auctions[_nftContractAddress][_tokenId].nftHighestBid >= buyNowPrice;
    }
    ...
    function _isMinimumBidMade(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        ...
        return minPrice > 0 && (auctions[_nftContractAddress][_tokenId].nftHighestBid >= minPrice);
    }
    ...
    function _doesBidMeetBidRequirements(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        ...
        uint256 bidIncreaseAmount = (auctions[_nftContractAddress][_tokenId].nftHighestBid * (10000 + defaultBidIncreasePercentage)) / 10000;
    ...
    function _isASale(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        return (auctions[_nftContractAddress][_tokenId].buyNowPrice > 0 && auctions[_nftContractAddress][_tokenId].minPrice == 0);
    }

    function _isAuctionOngoing(address _nftContractAddress, uint256 _tokenId) internal view returns (bool) {
        uint64 auctionEndTimestamp = auctions[_nftContractAddress][_tokenId].auctionEnd;
    ...
    function _transferNftToAuctionContract(address _nftContractAddress, uint256 _tokenId, bool _isDutch) internal {
        address _nftSeller = _isDutch ? dutchAuctions[_nftContractAddress][_tokenId].nftSeller : auctions[_nftContractAddress][_tokenId].nftSeller;
        if (IERC721Salvor(_nftContractAddress).ownerOf(_tokenId) == _nftSeller) {
            IERC721Salvor(_nftContractAddress).safeTransferFrom(_nftSeller, address(this), _tokenId);
            require(
            ...
            require(
                IERC721Salvor(_nftContractAddress).ownerOf(_tokenId) == address(this),
    ...
    function _transferNftAndPaySeller(address _nftContractAddress, uint256 _tokenId) internal {
        ...
        payable(_nftSeller).transfer(_nftHighestBid);

        IERC721Salvor(_nftContractAddress).safeTransferFrom(address(this), _nftHighestBidder, _tokenId);
    ...
    modifier isAuctionNotStartedByOwner(address _nftContractAddress, uint256 _tokenId, bool _isDutch) {
        address nftSeller = _isDutch ? dutchAuctions[_nftContractAddress][_tokenId].nftSeller : auctions[_nftContractAddress][_tokenId].nftSeller;
        require(nftSeller != msg.sender, "Auction already started by owner");
        if (nftSeller != address(0)) {
            require(msg.sender == IERC721Salvor(_nftContractAddress).ownerOf(_tokenId), "Sender doesn't own NFT");
            if (_isDutch) {
                _resetDutchAuction(_nftContractAddress, _tokenId);
            } else {
                _resetAuction(_nftContractAddress, _tokenId);
    ...
    modifier onlyNftSeller(address _nftContractAddress, uint256 _tokenId) {
        require(msg.sender == auctions[_nftContractAddress][_tokenId].nftSeller, "Only nft seller");
        _;
    }
...
library LibOrder {
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(address maker,string salt,uint256 tokenId,uint256 price,uint256 start,uint256 end,uint96 percentageBasisPoints,string tokenUri,bool isOffer)"
    );
...
interface IERC721Salvor is IERC721 {
    function owner() external view returns (address);
    function mintFromMarketplace(string memory _tokenUri) external returns (uint256);
