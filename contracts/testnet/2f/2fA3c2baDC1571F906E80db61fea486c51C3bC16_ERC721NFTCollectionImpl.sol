    ...
    INFTCollection
{
    ...
    /**
     * @dev called immidiatly after the whitelistMintStartDate + mintExpirationTime is passed
     */
    function setPublicMode() public onlyRole(ADMIN_ROLE) {
        ...
        publicModeMaxMintCount = _tokenIds.current() + publicMintTotalCount;
        mode = Mode.Public;
        emit PublicModeIsOn();
    }
    ...
    function mint(
        ...
        if (mode == Mode.Private) {
            ...
            if (getNumberOfTotalRemainingPublicNFTs() < publicModeMintCount)
                revert RequestedPublicModeMintCountExceedsTotal();
            if (
                publicModeNFTsMintedCount[_msgSender()] + publicModeMintCount >
                publicMintCap
            ) {
                revert CanNotMintMoreThanPublicMintCap();
            }

            totalMintFee = publicModeMintCount * publicMintPrice;
        ...
        returns (uint256 extraPaid, uint256 launchpadFee)
    {
        ...
        (success, ) = collectionCreatorAddress.call{
            value: totalMintFee - launchpadFee
        }("");
        if (!success) revert TransferNotSuccessful();
    }
    ...
    function getNumberOfTotalRemainingPublicNFTs()
        ...
        returns (uint256)
    {
        return publicModeMaxMintCount - _tokenIds.current();
    }
