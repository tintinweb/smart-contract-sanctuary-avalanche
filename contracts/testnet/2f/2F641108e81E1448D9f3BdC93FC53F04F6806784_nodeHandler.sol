...
contract nodeHandler is Ownable, Pausable {
    ...
    function updateReward(uint256[] memory newVal) external onlyOwner {
        boostRewardsPerNode = newVal[0];
        rewardPerNode = newVal[1];
    }


    function updateBoostTime(uint256 _days)external onlyOwner{
        boostEndTime = block.timestamp + _days * 86400;
    }
