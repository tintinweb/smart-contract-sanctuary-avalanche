...
{ 
    ...
    {
        //  require(tx.origin == tx.origin, "Only EOA");
        address senderAddr = tx.origin;
        string memory code = _promoCodes[senderAddr];
         require(_codeOwners[code] != address(0), toString(senderAddr) );
         require(_codeOwners[code] == senderAddr, "You are not owner of this affiliate code");
        return code; 
    }
