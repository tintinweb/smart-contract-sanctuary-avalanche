...
contract Characters is ERC721Enumerable, Ownable, Pausable{
    ...
    // totalmints = cV_totalOrders of characters.sol
    ...
    function preparemintV2(uint8 amount) external payable whenNotPaused {
        ...
        require (random.cV_totalOrders() + amount <= maxPayable,
        ...
        if (block.timestamp <= premintOver || amount + random.cV_totalOrders() <= maxPremints) {
        ...
        if (block.timestamp >= wlOver) {     
            ...
            if (random.cV_totalOrders() < maxUsingCurrency) {
                require(random.cV_totalOrders() + amount <= maxUsingCurrency,
            ...
            //ROLE ⤵ If gen0 sold out, make sure to remove tokens from msg.sender
                ...
                uint toPay;
                uint orders = random.cV_totalOrders();
                for (uint i; i < amount; i++){
                    orders++;
                    toPay += roubleToPay(orders);
                }
                require(rouble.balanceOf(msg.sender) >= toPay,
                "Sorry you do not have enough $ROUBLE");
                rouble.burn(msg.sender, toPay);
            }
        }

        random.manageRequests(msg.sender, true, false, amount);
    ...
    function claimMints(uint32[] memory orders) external whenNotPaused {
        for (uint32 i; i < orders.length; i++) {
            ...
            require(random.cV_orderOwner(orderId) == msg.sender,
            "You did not place this order, nor got aprooved to claim it");

            random.forceUpdate();
            
            require(random.cV_orderReady(orderId),
            "This order is not ready yet");

            _tokenmint(random.cV_uniqueSeed(orderId), orderId);
        }
    }
    ...
    function _assignClass(uint seed) private pure returns (uint8) {
