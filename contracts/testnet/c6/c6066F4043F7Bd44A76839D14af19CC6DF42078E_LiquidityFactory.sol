...
contract LiquidityFactory {
    ...
    uint public constant PRECISION = 1000000000;
...
//            stats.unpausedBlock = 0;
    ...
    function pulseSum(uint coeff, uint wavelength) public pure returns (uint) {
        ...
        return coeff.times(wavelength.times(wavelength.plus(1))).times(wavelength.times(2).plus(1)) / (6 * PRECISION);
    }
    ...
    function setConfig(
        ...
        pulse.pulseConstant = pulse.pulseAmplitudeWei.times(PRECISION) / pulse.pulseWavelengthBlocks.times(pulse.pulseWavelengthBlocks);
