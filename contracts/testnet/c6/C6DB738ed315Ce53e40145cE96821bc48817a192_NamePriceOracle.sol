...
contract NamePriceOracle is AggregatorV3Interface, Ownable {
...
interface IResolver {
    ...
    function setlinks(uint256 source, uint256[] calldata targets, uint256[] calldata values ) external;

}

contract NameRegistry is ERC721Upgradeable, AccessControlUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    event FlagUpdated(uint256 flags);
    event NewSubdomain(string name, uint256 tokenId, uint256 subtokenId, address to);
    ...
    mapping(uint256 => Record) metatable;
    ...
    function initialize() initializer public {
      __ERC721_init("NameRegistry", "NAME");
    ...
    modifier onlyManager {
        require(hasRole(MANAGER_ROLE, _msgSender()), "caller is not MANAGER");
        _;
    }

    function setFlags(uint256 _flags) public onlyAdmin {
        FLAGS = _flags;
        emit FlagUpdated(FLAGS);
    }
    ...
    modifier authorised(uint256 tokenId) {
        require(hasRole(MANAGER_ROLE, _msgSender()) || _isApprovedOrOwner(_msgSender(), tokenId), "not owner nor approved");
        _;
    }
    ...
    function mint(address addr, uint256 tokenId) external returns (uint256) {
    ...
    function mintSubdomain(address addr, uint256 tokenId, string calldata name) public virtual writable authorised(tokenId) returns (uint256) {
        ...
        _safeMint(addr, subtokenId);

        metatable[subtokenId].parent = tokenId;
        metatable[subtokenId].origin = metatable[tokenId].origin;

        emit NewSubdomain(name, tokenId, subtokenId, addr);
        return subtokenId;
    }

    function mintMultiSubdomain(address[] calldata addr, uint256 tokenId, string[] calldata name) public virtual authorised(tokenId) returns (uint256[] memory tokenIds) {
        // tokenIds = new uint256[](names.length); // todo : needed?
        for (uint256 i = 0; i < addr.length; i+=1) {
            tokenIds[i] = mintSubdomain(addr[i], tokenId, name[i]);
        }
    }

    function register(string calldata name, address addr, uint64 duration, uint256 baseNode) public onlyManager returns(uint256) {
        uint256 tokenId = mintSubdomain(addr, baseNode, name);
        require(available(tokenId), "tokenId not available");

        uint256 exp = block.timestamp + duration;
        metatable[tokenId].expire = uint64(exp);
        metatable[tokenId].origin = tokenId;
        // metatable[tokenId].parent = tokenId; // todo : remove this line, also mind this at migration

        return tokenId;
    }

    function renew(uint256 id, uint64 duration) public onlyManager returns(uint256) {
        require(metatable[id].origin == id, "not renewable");
        require(duration > 0, "duration too small");
        metatable[id].expire += duration;
        return metatable[id].expire;
    }
    ...
    function burn(uint256 tokenId) public virtual {
        require(
            ...
            || isApprovedOrOwner(_msgSender(), metatable[tokenId].origin), "not owner nor approved");

        _burn(tokenId);

        metatable[tokenId].expire = 0;
        metatable[tokenId].origin = 0;
    }
    ...
    function expired(uint256 tokenId) public view returns(bool) {
        return metatable[metatable[tokenId].origin].expire + GRACE_PERIOD < block.timestamp;
    }

    // todo : remove this
    function available(uint256 tokenId) public view returns(bool) {
        return metatable[tokenId].origin == 0;
    }

    function expire(uint256 tokenId) public view returns(uint256) {
        return uint256(metatable[tokenId].expire);
    }

    function origin(uint256 tokenId) public view returns(uint256) {
        return metatable[tokenId].origin;
    }

    function parent(uint256 tokenId) public view returns(uint256) {
        return metatable[tokenId].parent;
    }
    ...
    event MetadataUpdated(uint256[] tokenIds, Record[] data);

    function setMetadataBatch(uint256[] calldata tokenIds, Record[] calldata data) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "caller is not a admin");
        ...
        for (uint256 i = 0; i < tokenIds.length; i+=1) {
            ...
            metatable[tokenId].origin = data[i].origin;
            metatable[tokenId].parent = data[i].parent;
            metatable[tokenId].expire = data[i].expire;
        }
        emit MetadataUpdated(tokenIds, data);
    }
    ...
    function getName(address addr) public view returns (uint256 tokenId) {
    ...
    // *USE WITH CAUTION*
    // 1. resolve NFT token to NameRegistry domain is different from resolve wallet/contract to NameRegistry domain
    // 2. NFT token is transferable, wallet is not.
    // 3. when sell NFT token, which show as NameRegistry domain name, *NON-UNIQUE* mapping of
    //    NFT token to NameRegistry domain will cause problem
    // 4. for example, map 2 different value NFT token to same NameRegistry domain
    // 5. when sell NFT, only NameRegistry domain is shown, buyer will not able to distinguish which NFT token
    //    is on sale.
    function setNftName(
        address nftAddr,
        uint256 nftTokenId,
        uint256 tokenId
    ) external writable authorised(tokenId) {
    ...
    function getNftName(address nftAddr, uint256 nftTokenId) public view returns (uint256) {
    ...
    function _get(string calldata key, uint256 tokenId) private view returns (string memory) {
        return _getByHash(uint256(keccak256(abi.encodePacked(key))), tokenId);
    }

    function _getByHash(uint256 keyHash, uint256 tokenId) private view returns (string memory) {
...
contract NameController is AccessControl {
    ...
    event NameRegistered(string name, uint256 node, address owner, uint256 cost, uint256 expire);
    event NameRenewed(string name, uint256 node, uint256 cost, uint256 expire);

    constructor(NameRegistry _nameRegistry, uint256 _min_length, address _treasury, uint256 _baseNode, address _priceFeed, uint256[] memory _basePrices, uint256[] memory _rentPrices) {
        nameRegistry = _nameRegistry;

        MIN_LENGTH = _min_length;
        TREASURY = _treasury;
        MIN_DURATION = 60 days;
        BASE_NODE = _baseNode;
        FLAGS = 3;
    ...
    function nameRegisterMany(string[] calldata names, address addr, uint64 duration) public payable returns(uint256[] memory tokenIds) {
        // tokenIds = new uint256[](names.length); // todo : needed?
        for (uint256 i = 0; i < names.length; i+=1) {
            tokenIds[i] = nameRegister(names[i], addr, duration);
        }
    }

    function nameRegister(string calldata name, address addr, uint64 duration) public payable open returns(uint256) {
        require(bytes(name).length >= MIN_LENGTH, "name too short");
        require(duration >= MIN_DURATION, "duration too small");

        uint256 cost = registerPrice(name, duration);
        require(msg.value >= cost, "insufficient fee");

        uint256 tokenId = nameRegistry.register(name, addr, duration, BASE_NODE);
        ...
        emit NameRegistered(name, tokenId, addr, cost, nameRegistry.expire(tokenId));

        return tokenId;
    }

    function nameRegisterExtended(string calldata name, address addr, uint64 duration, uint256 data, uint256[] calldata keyHashes, string[] calldata values) public payable open returns(uint256) {
        uint256 tokenId = nameRegister(name, addr, duration);

        if (keyHashes.length > 0) {
          nameRegistry.setManyByHash(keyHashes, values, tokenId);
        }

        if (data & 1 == 1) {
            nameRegistry.setName(addr, tokenId);
        }

        return tokenId;
    }

    function nameRegisterByManager(string calldata name, address addr, uint64 duration, uint256 data, uint256[] calldata keyHashes, string[] calldata values) public live onlyManager returns(uint256) {
        uint256 tokenId = nameRegistry.register(name, addr, duration, BASE_NODE);
        ...
        emit NameRegistered(name, tokenId, addr, 0, nameRegistry.expire(tokenId));

        return tokenId;
    }

    function renew(string calldata name, uint64 duration) external payable open {
        bytes32 label = keccak256(bytes(name));
        bytes32 subnode = keccak256(abi.encodePacked(BASE_NODE, label));
        uint256 tokenId = uint256(subnode);
        uint256 expireAt = nameRegistry.renew(tokenId, duration);

        uint256 cost = renewPrice(name, duration);
        require(msg.value >= cost, "insufficient fee");

        emit NameRenewed(name, tokenId, cost, expireAt);
    ...
    function renewByManager(string calldata name, uint64 duration) external live onlyManager {
        bytes32 label = keccak256(bytes(name));
        bytes32 subnode = keccak256(abi.encodePacked(BASE_NODE, label));
        uint256 tokenId = uint256(subnode);
        uint256 expireAt = nameRegistry.renew(tokenId, duration);

        emit NameRenewed(name, tokenId, 0, expireAt);
    }
    ...
    function getPrices() view public returns(uint256[] memory, uint256[] memory) {
        return (basePrices, rentPrices);
    }

    function setPrices(uint256[] memory _basePrices, uint256[] memory _rentPrices) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "caller is not a admin");
    ...
    function registerPrice(string memory name, uint64 duration) view public returns(uint256) {
        uint256 price = uint256(getTokenPrice());
        return basePrice(name) * (10 ** 26) / (price) + rentPrice(name, duration) * (10 ** 26) / (price) / (86400*365);
    }

    function renewPrice(string memory name, uint64 duration) view public returns(uint256) {
        uint256 price = uint256(getTokenPrice());
        return rentPrice(name, duration) * (10 ** 26) / (price) / (86400*365);
    }

    function basePrice(string memory name) view public returns(uint256) {
    ...
    function rentPrice(string memory name, uint64 duration) view public returns(uint256) {
        uint256 len = bytes(name).length;
        if(len > rentPrices.length) {
            len = rentPrices.length;
        }
        uint256 price = rentPrices[len - 1] * (duration);
