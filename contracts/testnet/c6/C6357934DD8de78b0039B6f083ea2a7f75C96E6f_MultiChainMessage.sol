...
contract MultiChainMessage is ConfirmedOwner {
    ...
    modifier onlyExecutor() {
        require(
            msg.sender == CallProxy(anycallcontract).executor(),
            "onlyExecutor"
        );
        _;
    }
    ...
    function checkCall(
        ...
        IAnycallConfig config = IAnycallConfig(
            CallProxy(anycallcontract).config()
        );
        bytes memory data = abi.encode(_message);
        (_appID, _srcFees) = config.checkCall(
            owneraddress,
            data,
            _toChainID,
            0
    ...
    function anyExecute(
        ...
        (address from, uint256 fromChainId, ) = CallProxy(anycallcontract)
            .context();
        ...
        returns (address, address, uint256, uint256)
    {
        ...
        return (
            CallProxy(anycallcontract).executor(),
            from,
            fromChainId,
            nonce
        );
