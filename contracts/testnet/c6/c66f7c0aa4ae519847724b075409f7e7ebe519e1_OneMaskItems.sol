...
contract Pack {
    ...
    struct PackPurchase {
        ...
        uint8 count;
        uint32 commitBlock;
        //much more random bits than we need (absolute minimum of 13)
        uint32 randomness;
        address owner;
    }
    ...
    event Purchase(PackPurchase pack);
    event PackClaimed(uint256 indexed packId);
    event RandomnessRevealed(address indexed revealer, uint24 indexed packId, uint32 randomness);
    event Recommit(uint256 indexed packId, uint32 blockNumber);
    ...
    function packsPurchase(
        ...
        PackPurchase memory purchase = PackPurchase(id, amount, uint32(block.number + 1), 0, to);
    ...
    function claimPurchase(
        ...
        uint8 nftsInPack = p.count;
        (uint256[] memory _items, uint256[] memory _itemLayers) = getPackItems(randomness, nftsInPack);
    ...
    function recommitPurchase(
        ...
        emit Recommit(id, p.commitBlock);
    }

    function predictPurchase(
        uint id
    ) external view returns(uint256[] memory _items, uint256[] memory _itemLayers) {
        PackPurchase memory p = purchases[id];
        
        if (p.randomness == 0 && block.number > p.commitBlock && block.number < p.commitBlock + 256) {
            p.randomness = uint32(uint(keccak256(abi.encodePacked(blockhash(p.commitBlock), uint24(id)))));
        }
        require(p.randomness != 0, "Pack Not Revealed");

        return getPackItems(p.randomness, p.count);
    }

    function getPackItems(
        uint32 randomness,
        uint8 nftsInPack
    ) internal view returns(uint256[] memory _items, uint256[] memory _itemLayers) {
        _items = new uint256[](nftsInPack * ITEMS_PER_PACK);
        _itemLayers = new uint256[](nftsInPack * ITEMS_PER_PACK);

        bytes[ITEMS_PER_PACK] memory aliasPointersBytes = getAliasPointersBytes();

        for (uint k = 0; k < nftsInPack; k++) {
            for (uint j = 0; j < ITEMS_PER_PACK; j++) {
                ...
                _items[indexToSet] = AliasPacked.getRandomIndex(aliasPointersBytes[j], random);
                _itemLayers[indexToSet] = j;
            }
        }
    }
...
contract Auction {
    ...
    function buy(uint32 amnt) public payable { unchecked {
        ...
        emit Mint(msg.sender, amnt, newPrice);
    }}
    ...
    event Mint(address indexed to, uint32 amount, uint256 price);
...
contract OneMaskNFT is ERC721, Ownable, RolesAuthority {
    ...
    mapping (uint256 => Blue) public blues;
    ...
    ) ERC721("OneMask", "BLUE") RolesAuthority(_owner, Authority(address(this))) {
    ...
    event Unequip(uint256 indexed tokenId, uint256 itemId);
    event Equip  (uint256 indexed tokenId, uint256 itemId);
    
    /// @notice Permissioned mint, "to" also receives a pack containing items
    function mint(
        address to,
        uint256 amnt
    ) public requiresAuth {
        require(amnt <= 255 && amnt > 0, "MINT_AMNT_GT_255");
    ...
    /**
      * @notice Unequips an item from a Blue and sends it to the caller
      * @param tokenId Blue id to unequip from
    ...
    function unequipItem(
        ...
        uint16 _layer = itemContract.getItem(itemId).layer;
        uint48 currentItems = blues[tokenId].items;
        uint16 currentItem = getBluesItem(currentItems, _layer);

        require(currentItem != 0, "Slot already empty");
        require(currentItem == itemId, "Blue does not have this item");

        //First remove the item from the blue
        blues[tokenId].items = setBluesItem(currentItems, _layer, 0);
    ...
    /**
      * @notice Equips an item to a Blue from the caller's wallet
      * @param tokenId Blue id to equip to
      * @param itemId Item id to equip
      * @dev Must have approved this contract to spend item
      */
    function equipItem(
        uint256 tokenId,
        uint16 itemId
        ...
        blues[tokenId].items = setBluesItem(currentItems, _layer, itemId);
    ...
    /// @notice Checks if blue has itemId equipped
    /// @return boolean if equipped or not
    function isItemEquipped(
        ...
        return getBluesItem(currentItems, _layer) != 0;
    }
    ...
    function setBluesItem(
        ...
        uint16 itemLayer,
        uint16 itemId
    ) internal pure returns(uint48 newItems) {
        //Essentially puts the itemId into the blueItems bit struct
        assembly {
            let itemShift := mul(itemLayer, 16)
            newItems := xor(shl(itemShift, 0xFFFF), 0xFFFFFFFFFFFF)
            newItems := and(blueItems, newItems)
            newItems := or(newItems, shl(itemShift, itemId))
        }
        // return uint48(
        //     (0xFFFFFFFFFFFF ^ (0xFFFF << (itemLayer * 16)))
        //         & blueItems | 
        //         (uint48(itemId) << (itemLayer*16)));
    }
    ...
    function getBluesItem(
        ...
        assembly {
            item := shr(mul(itemLayer, 16), blueItems)
        }
    }
    ...
    /**
      ...
      */
    function itemsFromBlue(
        uint256 id
    ) public view returns(uint16[TRAIT_COUNT] memory blueItems) {
        uint48 bitItems = blues[id].items;
        for (uint16 i; i < TRAIT_COUNT; i++) {
            blueItems[i] = getBluesItem(bitItems, i);
        }
    }
    ...
    //     uint256[] calldata ids
    // ) external view returns(uint16[TRAIT_COUNT][] memory bluesData) {
    //     uint len = ids.length;
    //     bluesData = new uint16[TRAIT_COUNT][](len);
    //     for(uint i=0;i<len;i++) {
    //         bluesData[i] = itemsFromBlue(ids[i]);
    //     }
    // }
...
contract OneMaskItems is ERC1155 {
    ...
    event NewItem(Item item, uint32 itemId);
    ...
    */
    ...
    function mintBatch(uint256[] calldata traits, uint256[] calldata layers, address to) external { unchecked {
        require(msg.sender == address(nftContract.packContract()));
        uint len = layers.length;
        require(len == traits.length);
        ...
        for (uint i; i<len; ++i) {
            (uint id, uint32 newTokenId) = findOrGenItem(uint16(layers[i]), uint16(traits[i]), tokenId);
            
            ++balanceOf[to][id];
            ++items[uint32(id)].supply;
            tokenId = newTokenId;
            ids[i] = id;
            amounts[i] = 1;
    ...
    function findOrGenItem(uint16 layer, uint16 trait, uint32 _tempId) internal returns(uint, uint32) {
    ...
    function genItem(uint16 layer, uint16 trait, bytes32 itemHash, uint32 _tempId) internal {
        ...
        emit NewItem(newItem, _tempId);
    }

    function getItemHash(uint16 layer, uint16 trait) public pure returns(bytes32) {
    ...
    */
    ...
    function getItemFromTrait(uint16 _layer, uint16 _trait) external view returns (uint) {
        return hashes[getItemHash(_layer, _trait)];
    }
    ...
    function svgFromId(uint _tokenId) public view returns(string memory) {
        require(items[uint32(_tokenId)].supply != 0, "Does not exist");
