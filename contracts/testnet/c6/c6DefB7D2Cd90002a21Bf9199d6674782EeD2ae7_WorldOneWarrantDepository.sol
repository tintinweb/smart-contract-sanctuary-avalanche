...
contract WorldOneWarrantDepository is Ownable {
    ...
    function initializeWarrantLot( 
        ...
        WarrantLot memory warrantLot = WarrantLot ({
            ...
            totalCapacity: _totalCapacity.mul( 10**Time.decimals() ),
            consumed: 0,
            fee: _fee,
    ...
    function EditWarrantLot(uint256 _warrantLotIndexs, uint256 _price)
        external
        onlyOwner
    {
    ...
    function allCost() public view returns (uint) {
        return remainingAvailable().mul( 10**principle.decimals() ).mul( warrantPrice() ).div( 10**Time.decimals() ) / 100;
    }
