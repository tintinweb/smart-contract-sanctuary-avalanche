...
contract GenesisStaking is Ownable {
    ...
    uint256 public immutable claimsEndAt;

    ////////////////
    /// CONSTANS ///
    ////////////////

    uint256 constant VAPE_TO_DISTRIBUTE = 420000 * 1e18;
    ...
    modifier claimsEnabled() {
        if (block.timestamp < claimsStartAt || block.timestamp > claimsEndAt) revert GenesisStaking__ClaimsDisabled();
    ...
    constructor(address _vpnd, address _vape, address _stratosphere) {
        ...
        depositsStartAt = deployment;
        depositsEndAt = deployment + 6 hours;
        claimsStartAt = depositsEndAt;
        claimsEndAt = depositsEndAt + 4 hours;
    ...
    function claim() external claimsEnabled {
        ...
        unchecked {
            s_vpndAccountBalance[msg.sender] -= vpndAmount;
        }
    ...
    /// @param _to Recipient
    /// @dev Only owner can call this function, after all the dates have passed
    function retrieve(address _to) external onlyOwner {
        if (_to == address(0)) revert GenesisStaking__InvalidAddress();
        if (block.timestamp < claimsEndAt) revert GenesisStaking__RetrievalsDisabled();
