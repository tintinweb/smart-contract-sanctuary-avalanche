...
import "../implementations/output/AtomProtocolImplementationPointerUpgradeable.sol";
import "../implementations/output/LiquidityPoolManagerImplementationPointerUpgradeable.sol";
import "../implementations/output/AtomsManagerImplementationPointerUpgradeable.sol";

contract WalletObserverUpgradeable is
    Initializable,
    OwnableUpgradeable,
    OwnerRecoveryUpgradeable,
    AtomProtocolImplementationPointerUpgradeable,
    LiquidityPoolManagerImplementationPointerUpgradeable,
    AtomsManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        setTimeframeQuotaIn(200_000_000 * (10**18));
        setTimeframeQuotaOut((90_000_000 / 10) * (10**18));
    ...
    function changeAtomProtocolImplementation(address newImplementation)
        public
        virtual
        override(AtomProtocolImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeAtomProtocolImplementation(newImplementation);
        excludeFromObserver(atomProtocol.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(atomsManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to atomsManager, use the Observatory to create a atom (https://protocol-atom.com/matter)"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= atomProtocol.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (atomProtocol.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return atomProtocol.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the AtomProtocol team to know if
...
import "../../contracts/interfaces/IAtomProtocol.sol";

abstract contract AtomProtocolImplementationPointerUpgradeable is OwnableUpgradeable {
    IAtomProtocol internal atomProtocol;

    event UpdateAtomProtocol(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyAtomProtocol() {
        require(
            address(atomProtocol) != address(0),
            "Implementations: AtomProtocol is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(atomProtocol),
            "Implementations: Not AtomProtocol"
        );
        _;
    }

    function getAtomProtocolImplementation() public view returns (address) {
        return address(atomProtocol);
    }
    ...
    function changeAtomProtocolImplementation(address newImplementation)
        ...
        onlyOwner
    {
        address oldImplementation = address(atomProtocol);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "AtomProtocol: You can only set 0x0 or a contract address as a new implementation"
        );
        atomProtocol = IAtomProtocol(newImplementation);
        emit UpdateAtomProtocol(oldImplementation, newImplementation);
    }
...
import "../../contracts/interfaces/IAtomsManager.sol";

abstract contract AtomsManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IAtomsManager internal atomsManager;

    event UpdateAtomsManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyAtomsManager() {
        require(
            address(atomsManager) != address(0),
            "Implementations: AtomsManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(atomsManager),
            "Implementations: Not AtomsManager"
        );
        _;
    }

    function getAtomsManagerImplementation() public view returns (address) {
        return address(atomsManager);
    }

    function changeAtomsManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(atomsManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "AtomsManager: You can only set 0x0 or a contract address as a new implementation"
        );
        atomsManager = IAtomsManager(newImplementation);
        emit UpdateAtomsManager(oldImplementation, newImplementation);
    }
...
interface IAtomProtocol is IERC20 {
