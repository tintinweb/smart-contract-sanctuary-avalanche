 ...
 * _Available since v3.1._
 */
contract BEP1155 is Context, BEP165, IBEP1155, IBEP1155MetadataURI, IBEP2981 {
    ...
    /**
     * @dev See {IBEP1155MetadataURI-name}.
     */
    ...
    /**
     * @dev See {IBEP1155MetadataURI-symbol}.
     */
    ...
    function _listMint(
        ...
        if(amount == listQty){
            ...
            _balances[id][from] += listQty;
            _balances[id][to] += (amount - listQty);
            emit TransferSingle(operator, address(0), to, id, _balances[id][to]);
            emit TransferSingle(operator, address(0), from, id, listQty);
        }
    ...
    function _setDefaultRoyalty(address receiver, uint96 feeNumerator) internal virtual {
        require(feeNumerator <= _feeDenominator(), "BEP2981: royalty fee will exceed salePrice");
        require(receiver != address(0), "BEP2981: invalid receiver");
    ...
    function _setTokenRoyalty(
        ...
        require(feeNumerator <= _feeDenominator(), "BEP2981: royalty fee will exceed salePrice");
        require(receiver != address(0), "BEP2981: Invalid parameters");
...
contract Edaface1155 is Owned, BEP1155 {
    event Create(
        address indexed _to,
    ...
    ) BEP1155("", name_, symbol_) Owned(_ownerAddress) {}
    
    function mint(string memory ipfsmetadata, address to, uint[] memory total, uint royal, uint256 id_, bool _list) external onlyOwner{
        require(msg.sender == owner, "Public Mint Not Available");
        if(_list){
            _listMint(owner, to, id_, total[0], total[1], "");
        }
        else{
            _mint(to, id_, total[0], "");
        }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ipfsmetadata.length; i++) {
            ...
            uint256 _tid = _tokenid.add(block.timestamp);
            _creator[_tid] = from;
            _mint(from, _tid, supply[i], "");
            _setTokenURI(_tid, ipfsmetadata[i]);
            _royal[_tid] = royal;
            emit Create(from, _tid);
        }
    }
    function mintListBatch(
        ...
        require(msg.sender == owner, "Public Mint Not Available");
        for (uint256 i = 0; i < ipfsmetadata.length; i++) {
            _creator[_tokenid[i]] = from;
            _listMint(owner, from, _tokenid[i], supply[i], listQty[i], "");
            _setTokenURI(_tokenid[i], ipfsmetadata[i]);
            _royal[_tokenid[i]] = royal;
            emit Create(from, _tokenid[i]);
        }
    }
    ...
    function setRoyaltyPBEPentage(uint96 _royPer) external onlyOwner{	
        _royaltyPer = _royPer;	
    }
    function adminBurn(uint256 tokenId, uint256 amount, address from) external onlyOwner{
