...
contract SmOracleNew is Ownable {
    ...
    uint256 public capThreshold = 20;
    ...
    constructor(bytes32[] memory _currencyKeys, uint256[] memory _twaps) public {
        for (uint256 i = 0; i < _currencyKeys.length; i++) {
            lastTwap[_currencyKeys[i]] = _twaps[i];
        }
    }
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i = 0; i < _prices.length; i++) {
            ...
            if (transactionData[_currencyKey].prices.length >= totalNumOfInterval) {
                uint256 twap = getTwap(_currencyKey);
                if (_prices[i] > twap * (100 + capThreshold) / 100 || _prices[i] < twap * (100 - capThreshold) / 100 ) {
                    continue;
                }
            }
            ...
            if (transactionData[_currencyKey].prices.length >= totalNumOfInterval) {
                lastTwap[_currencyKey] = getTwap(_currencyKey);
            }
        }
    }
    ...
    function setCapThreshold(uint256 _capThreshold) external onlyOwner {
        capThreshold = _capThreshold;
    }
