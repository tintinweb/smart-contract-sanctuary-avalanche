...
contract AvaxBunnies is ERC721Enumerable, PaymentSplitter, Ownable {
  using Strings for uint256;
  ...
  uint public pricePresale = 0.75 ether;
  uint public priceSale = 1 ether;
  ...
  address[] private _team = [
        0x3eef9664461A69176bee40CD9fcB0DDb1FdCA1A2
    ];
    uint[] private _teamShares = [
        100
    ];
  constructor( bytes32 _merkleRoot, string memory _theBaseURI) ERC721('Avax Bunnie$', 'AB') PaymentSplitter(_team, _teamShares){
  ...
  function presaleMint(address _account, bytes32[] calldata _proof) external payable {
        require(sellingStep == Steps.Presale, "Not started yet.");
        require(nftsPerWallet[_account] < 1, "1 NFT on the Presale");
  ...
  function saleMint(uint256 _ammount) external payable {
        ...
        require(sellingStep == Steps.Sale, "Not started yet.");
        require(msg.value >= price * _ammount, "Not enought funds.");
        require(_ammount <= max_mint_allowed, "Can't mint more than 3");
        require(numberNftSold + _ammount <= MAX_SUPPLY, "");
...
function setUpSale() external onlyOwner {
        require(sellingStep == Steps.Presale, "Presale first");
