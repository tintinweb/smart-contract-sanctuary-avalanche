...
interface IAlphaNFTPowerLevelStorage {
    function getStartingPowerByNFTType(uint256 tokenType) external view returns(uint256);
}
    ...
    ReentrancyGuard 
{
    ...
    Counters.Counter public totalApex;
    Counters.Counter public totalRonin;
    Counters.Counter public totalAlpha;
    Counters.Counter public totalBase;
    
    uint256 public total;    
    uint256 public _ownerShipTransferCooldown; 

    address public _powerLevelStorage;
    ...
    MaxQuantityByNFTType maxQuantityByNFTType;
    ...
    constructor(
        address powerLevelStorage
    ) ERC721("Alpha Shares", "AlphaShares") {
        
        total = 10000; // maxTokens;
        
        _powerLevelStorage = powerLevelStorage;
    ...
    function init() internal {
        ...
        // data mapping for NFTTypes to match from power level controller and storage
        nftTypes = NFTTypes({
            APEX: 4,
            RONIN: 3,
            ALPHA: 2,
    ...
    /**
    *   @dev safely mint ONLY the maximum number of each type of NFT
     */

    function createApexNFT(
        address _to
        )
        external
        minterOnly
        lessThanTotalMintableByLevel(totalApex, maxQuantityByNFTType.APEX)
        returns (uint)  
    {
        totalApex.increment();
        uint256 startingPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getStartingPowerByNFTType(nftTypes.APEX);
        return createToken(_to, nftTypes.APEX, startingPower);
    }

    function createRoninNFT(
        address _to
        )
        external
        minterOnly
        lessThanTotalMintableByLevel(totalRonin, maxQuantityByNFTType.RONIN)
        returns (uint)  
    {
        totalRonin.increment();
        uint256 startingPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getStartingPowerByNFTType(nftTypes.RONIN);
        return createToken(_to, nftTypes.RONIN, startingPower);
    }

    function createAlphaNFT(
        address _to
        )
        external
        minterOnly
        lessThanTotalMintableByLevel(totalAlpha, maxQuantityByNFTType.ALPHA)
        returns (uint)  
    {
        totalAlpha.increment();

        uint256 startingPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getStartingPowerByNFTType(nftTypes.ALPHA);
        return createToken(_to, nftTypes.ALPHA, startingPower);
    }
    ...
    function createBaseNFT(
        ...
        external
        minterOnly
        lessThanTotalMintableByLevel(totalBase, maxQuantityByNFTType.BASE)
        returns (uint)  
    {
        totalBase.increment();

        uint256 startingPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getStartingPowerByNFTType(nftTypes.BASE);
        return createToken(_to, nftTypes.BASE, startingPower);
    }
    ...
    * @dev toggle earning profit - will be toggled off during auction
    ...
    function getNFTTypeTokenId(
        ...
        returns(uint256)
    {
        return alphaNodes[tokenId].nftType;
    }
    ...
    function getPowerLevelByTokenId(
        ...
        returns(uint256)
    {
        return alphaNodes[tokenId].currentPower;
    }
    ...
    * Called by Node NFT Power manager contract;
    ...
    {
        uint256 previousPowerLevel = alphaNodes[tokenId].currentPower;
        uint256 nftType = alphaNodes[tokenId].nftType;
        
        alphaNodes[tokenId].currentPower = IAlphaNFTPowerLevelStorage(_powerLevelStorage).getStartingPowerByNFTType(nftType);
        
        setLastUpdated(tokenId);
        
        emit powerChanged(tokenId, previousPowerLevel, alphaNodes[tokenId].currentPower, block.timestamp);
    }
    ...
    * @dev  Change transfer cooldown for single token by id
    ...
    /**
    * @dev  Set PowerLevelStorage address
     */
    function setPowerLevelStorage(
        address powerLevelStorage
        )
        external
        nonReentrant 
    ...
    * @dev Modifiers
    ...
    modifier lessThanTotalMintableByLevel(Counters.Counter storage levelCounter, uint256 max) {
        require(levelCounter.current() < max, "maximum number of level four NFTs created");
        _;
    }
