...
contract BridgesOfFate is Ownable {
    ...
    bool private _isNarrowingGameEnded = false;
    ...
    function _computeNextPlayerIdForHolder(address holder,uint256 _nftId,uint8 _seriesIndex) internal pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            randomNumber  =  (_packedNumber - ((_packedNumber / max) * max));
        ...
        function rand() internal view returns(uint256) {
            uint256 seed = uint256(keccak256(abi.encodePacked(block.timestamp + block.difficulty + ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.number)) +
    ...
    function participateInGame(bool _jumpSide,bytes32 playerId) public GameEndRules NarrowingGameEndRules {
        ...
        if (gameRandomNumber[_currentUserStage] <= 0) {
            ...
            gameRandomNumber[_globalStageNumber] = rand();
            ...
            _narrowinggameRandomNumber = rand();
