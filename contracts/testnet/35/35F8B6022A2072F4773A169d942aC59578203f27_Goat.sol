...
contract Goat is ERC721AQueryableGoat, BaseTokenURI {
  ...
  function buy(
    ...
    require(
      isSignatureValid(owner, _receiver, _tokenId, _price, buyNonce[_tokenId], _signature),
      "Goat: invalid signature"
    );
    ...
    if (isTokenNotNested[_tokenId] == false) isAllowNestingTransfer = true;
    safeTransferFrom(owner, _receiver, _tokenId);
    if (isTokenNotNested[_tokenId] == false) isAllowNestingTransfer = false;
  ...
  function _beforeTokenTransfers(
    ...
    for (uint256 i = tokenId; i < (tokenId + quantity); ++i) {
      require(from == address(0) || isTokenNotNested[i] || isAllowNestingTransfer, "Goat: token is nesting");
    }
  }
