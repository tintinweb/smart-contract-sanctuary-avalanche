...
contract ERC721Token {
    ...
    constructor(string memory _name, string memory _symbol, string memory _desc, address _owner) {
        require(
            bytes(_name).length > 0 &&
            bytes(_symbol).length > 0 &&
            bytes(_desc).length > 0,
        ...
        bytes memory byteCode = abi.encodePacked(type(ERC721Proxy).creationCode, abi.encode(_owner));
        address addr;
        assembly {
...
contract ERC721ByteCodeGenerator {
    ...
    function generate(
        ...
        return abi.encodePacked(type(ERC721Token).creationCode, abi.encode(_name, _symbol, _desc, owner));
    }
