...
contract NODERewardManagement {
    ...
    function _getNodesRewardAvailableByType(address account, uint32 nodeType)
        public
        view
        returns (uint256)
    {}
    ...
    function _getUserReferralCount(address account)
        ...
        returns (uint256)
    {}
    function _getNodeTypeCounts() external view returns(uint256[4] memory){
...
contract ZEUS is Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    address public futurUsePool = 0x2dC375d8c2c36ce78409A520ec96F917243DFfF5;
    ...
    address public taxPool = 0x2dC375d8c2c36ce78409A520ec96F917243DFfF5;
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        if (swapLiquify) {
            token.transfer(taxPool, fee);
        }
        token.transferFrom(distributionPool, sender, rewardAmount);
    }

    function cashoutAllNodesByType(uint256[] memory claimtimes) public {
        address sender = _msgSender();
        require(sender != address(0), "CSHT:  creation from the zero address");
        require(!_isBlacklisted[sender], "MANIA CSHT: Blacklisted address");
        require(
        ...
        if (swapLiquify) {
            token.transfer(taxPool, feeTotal);
        }
        token.transferFrom(distributionPool, sender, total);
    }
    ...
    function compoundNode(
        ...
        address sender = _msgSender();
        uint256 available = nodeRewardManager._getNodesRewardAvailableByType(sender,nodeType_);
        require(available > 0, "Balance too low");
        uint256 nodePrice = nodeRewardManager.getNodePriceByType(nodeType_);
        require(available > nodePrice, "Balance lower than node price");
        require(
            bytes(nodeName).length > 3 && bytes(nodeName).length < 32,
            "NODE CREATION: NAME SIZE INVALID"
        );
        bool isNameAvailable = nodeRewardManager.isNameAvailable(sender, nodeName);
        require(isNameAvailable, "Node name exits");
        uint256 total = available.div(nodePrice);
        uint256 leftOver = available.sub(total.mul(nodePrice));
        string memory name = nodeName;
        uint32 nodeType = nodeType_;
        nodeRewardManager._cashoutAllNodesByTypeReward(sender, nodeType);
         for (uint256 i = 0; i < total; i++) {
            address dead_ = 0x000000000000000000000000000000000000dEaD;
            if (i == 0) {
                nodeRewardManager.createNode(sender, name, dead_, nodeType);
            } else {
                string memory name_ = string(
                    abi.encodePacked(name, "_", Strings.toString(i - 1))
                );
               nodeRewardManager.createNode(sender, name_, dead_, nodeType);
            }
        }

        if(leftOver > 0){
            token.transferFrom(distributionPool, sender,leftOver);
        }
    }
    ...
    function getUserNodeTypes() external view returns (string memory) {
    ...
    function getNodeTypeCounts() external view returns(uint256[4] memory){
        return nodeRewardManager._getNodeTypeCounts();
    }
}