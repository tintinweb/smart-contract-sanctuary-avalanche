...
contract Authentication is Ownable, IAuthentication {
    ...
    mapping(uint => mapping(address => bool)) public whitelistedProjects;
    ...
    function isOwnerOfNFT(address _account, uint _chainId, address _projectAddress, uint _tokenId) external view override returns (bool) {
        return _account == ownersOfNFTs[_chainId][_projectAddress][_tokenId];
    }

    function setMultipleOwnersOfNFTs(OwnershipUpdate[] memory _newOwnerships) external onlyOwner {
        require(_newOwnerships.length <= 500, "Too many NFTs to set owners");
        ...
        for (uint i = 0; i < _newOwnerships.length; i++) {
            ...
            address from = ownersOfNFTs[_newOwnerships[i].chainId][_newOwnerships[i].projectAddress][_newOwnerships[i].tokenId];
            ...
            transfers[i] = Transfer({from: from, to: _newOwnerships[i].newOwner, chainId: _newOwnerships[i].chainId, projectAddress: _newOwnerships[i].projectAddress, tokenId: _newOwnerships[i].tokenId});
        }
    ...
    function changeStatusOfWhitelistedProject(uint _chainId, address _projectAddress, bool _status) public onlyOwner {
        whitelistedProjects[_chainId][_projectAddress] = _status;
    }

    // Returns if a project address is whitelisted or not.
    function isProjectWhitelisted(uint _chainId, address _projectAddress) external view override returns (bool isWhitelisted) {
        isWhitelisted = whitelistedProjects[_chainId][_projectAddress];
    }
}