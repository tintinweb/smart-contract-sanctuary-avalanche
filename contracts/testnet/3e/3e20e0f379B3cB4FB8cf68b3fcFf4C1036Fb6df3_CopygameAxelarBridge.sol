...
contract CopygameAxelarBridge is IAxelarExecutable, ICopygameBridge, Ownable {
  ...
  function _bridgeAxelarWithToken(address from, string memory destinationChain, string memory destinationAddress, bytes calldata payload, string calldata symbol, uint256 amount) internal {
  ...
  function _bridge(uint256 chainId, address from, bytes calldata payload) override internal {
  ...
  function _bridgeWithToken(uint256 chainId, address from, bytes calldata payload, string calldata symbol, uint256 amount) override internal {
    require(msg.sender == address(controller), "F");
    _bridgeAxelar(from, siblings[chainId].chainName, siblings[chainId].bridgeAddress, payload);
  }
  ...
  function _executeWithToken(
    ...
    controller.creditMaster(chainId, originalLeader, IERC20(gateway.tokenAddresses(tokenSymbol)), amount);
  }
}
...
abstract contract ICopygameBridge {
  ...
  function _bridge(uint256 chainId, address from, bytes calldata payload) virtual internal;
  function bridge(uint256 chainId, address from, bytes calldata payload) public payable onlyController {
    _bridge(chainId, from, payload);
  }

  function _bridgeWithToken(uint256 chainId, address from, bytes calldata payload, string calldata symbol, uint256 amount) virtual internal;
  function bridgeWithToken(uint256 chainId, address from, bytes calldata payload, string calldata symbol, uint256 amount) public payable onlyController {
    _bridgeWithToken(chainId, from, payload, symbol, amount);
  }
}
...
contract CopygameBridgePlugin is Ownable {
  ...
  function creditMaster(
    ...
    tokenAddress.safeTransferFrom(msg.sender, address(mirroredLeader), amount);
  ...
  function enable(address bridge, uint256 chainId, CopygameLeader leader) public payable onlyWhitelistedBridge(chainId, bridge) {
    ...
    ICopygameBridge(bridge).bridge{value: msg.value}(chainId, msg.sender, abi.encodeWithSelector(
  ...
  function tokenBridge(address bridge, uint256 chainId, CopygameLeader leader, string calldata symbol, uint256 amount) public payable onlyWhitelistedBridge(chainId, bridge) {
    ...
    ICopygameBridge(bridge).bridgeWithToken{value: msg.value}(chainId, msg.sender, abi.encode(originalChainId, originalLeader), symbol, amount);
