...
contract AaveYieldV3 is IYield{
    // Aave referral code (still need to research on that)
    uint16 constant REFERRAL_CODE = 0;

    // Aave instance we use to swap Inbound Token to interest bearing aUSDC
    IPoolAddressesProvider public immutable poolAddressesProvider;

    // Lending pool address
    ILendingPoolV3 public immutable lendingPool;

    // Address of the stable coin or inbound token or underlaying token
    ...
    /**
     * @notice modifier to check that the input amount can't be zero
     ...
     */
    constructor(address _poolAddressesProvider, address _stableCoin, address _dataProvider) {
        ...
        lendingPool = ILendingPoolV3(IPoolAddressesProvider(_poolAddressesProvider).getPool());
         ...
         */
        function receiveFromUser(uint _amount) public notZeroAmount(_amount){
        ...
        function deposit(uint _amount) public notZeroAmount(_amount){
            if (getContractStableBalace() < _amount) {
                revert NOT_ENOUGH_STABLE_BALANCE();
            }
        // IERC20(stableCoin).transferFrom(msg.sender, address(this), _amount);
        IERC20(stableCoin).approve(address(lendingPool), _amount);
        lendingPool.supply(address(stableCoin), _amount, address(this), REFERRAL_CODE);
        // deposit === supply in aave v3
        }
         ...
         */
        function withdraw(uint _amount) public notZeroAmount(_amount){
         ...
         */
        function transferTo(uint _amount, address _receiver) public notZeroAddress(_receiver) notZeroAmount(_amount){
            bool success = IERC20(stableCoin).transfer(_receiver, _amount);
            if (!success) {
    ...
    */
     ...
     */
    function getStableCoinBalance(address _user) public view returns(uint){
     ...
     */
    function getContractStableBalace() public view returns(uint){
