/**
 *Submitted for verification at testnet.snowtrace.io on 2023-01-28
*/
...
interface IStakingRewards is IStakingFee {
    ...
    function removeRewardToken(uint8 index) external;
...
contract Staking is ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        require(_stakingTokenAddress != address(0), "Invalid staking token addr");
     ...
     * @_deadline permit deadline parameter
     * @_v permit v parameter
     * @_r permit r parameter
     * @_s permit s parameter
    ...
    function stake(uint256 _amount) public nonReentrant returns(uint256) {
        ...
        require(_amount > 0, "Cannot stake 0"); // Check after the hook
        totalSupply += _amount;
    ...
    function withdraw(uint256 _amount) public nonReentrant returns(uint256) {
        ...
        require(_amount > 0 && _amount <= balances[msg.sender], "Cannot withdraw > account balance");  // Check after the hook
        totalSupply -= _amount;
        balances[msg.sender] -= _amount;
     ...
     * @_account staker address
     * @_amount amount being staken
     ...
     * @_account withdrawer address
     * @_amount amount being withdrawn
     ...
     * @_tokenAddress address of the token being recovered
     * @_recipient recovery address
     * @_amount amount being withdrawn
     */
...
contract StakingFee is Staking {
     ...
     * @_withdrawalAmount amount to withdraw
     ...
     * @_recipient fee recovery address
     */
     ...
     * @_depositFeeBps deposit fee in basis points
     * @_withdrawalFeeBps withdrawal fees in basis points
     * @_withdrawalFeeSchedule withdrawal fees schedule
     */
     ...
     * @_recipient recovery address
     * @_amount amount being recovered
     */
...
/**
 * Contract implementing simple ERC20 token staking functionality with staking rewards deposit/withdrawal fees.
 */
contract StakingRewards is StakingFee {
    ...
    constructor(
        ...
        require(_rewardTokenAddresses.length > 0, "There must be > 0 reward tokens");
    ...
    function startEmission(uint256[] memory _rewards, uint256 _duration) public virtual nonReentrant onlyOwner whenNotEmitting updateRewards(address(0)) {
        ...
        require(_rewards.length == rewardTokens.length, "Reward amounts has too few or too many values");
    ...
    function recoverLeftoverReward(address _tokenAddress, address _recipient) external onlyOwner whenNotEmitting {
        require(totalSupply == 0, "Can't recover reward if tokens are staked");
    ...
    function removeRewardToken(uint8 _index) external onlyOwner whenNotEmitting {
        _removeRewardToken(_index);
    }
    ...
    function _addRewardToken(address _tokenAddress) private {
        require(rewardTokens.length < 10, "Can't have > 10 rewards tokens");
        require(_tokenAddress != address(0), "Reward token address is 0");
        require(!rewardTokenAddresses[_tokenAddress], "Reward tokens must be different");
        rewardTokens.push(IERC20(_tokenAddress));
        rewardTokenAddresses[_tokenAddress];
    }

    function _removeRewardToken(uint8 index) private {
        require(index < rewardTokens.length, "Index > token array");

        for(uint8 i = index; i < rewardTokens.length - 1; i++) {
    ...
    function _beforeSetFees() internal virtual override {
        require(block.timestamp > periodFinish, "Can't set fees when emitting.");
    ...
    modifier whenEmitting() {
        require(block.timestamp <= periodFinish, "Rewards are not emitting");
        _;
    }

    modifier whenNotEmitting() {
        require(block.timestamp > periodFinish, "Rewards are emitting");
        _;
    }
...
contract FarmingRewards is StakingRewards {
    ...
    constructor(
        ...
        require(lpToken.factory() == oracle.factory(), "Only LPs on ElkDex are supported");
        require(lpToken.token0() == _coverageTokenAddress || lpToken.token1() == _coverageTokenAddress, "Coverage token must a token in the LP");
        coverageTokenAddress = _coverageTokenAddress;
        coverageAmount = _coverageAmount;
        require(_coverageVestingDuration >= 24 * 3600, "Coverage duration must be at least 1 day");
        require(_coverageVestingDuration <= rewardsDuration, "Coverage duration cannot be longer than rewards duration");
    ...
    function getCoverage(address _recipient) public nonReentrant updateCoverage(_recipient) {
        require(msg.sender == owner() || msg.sender == _recipient, "Only owner or recipient can claim coverage");
    ...
    function startEmission(uint256[] memory _rewards, uint256 _coverage, uint256 _duration) public onlyOwner updateCoverage(address(0)) {
        ...
        require(coverageVestingDuration <= rewardsDuration, "Coverage duration cannot be longer than rewards duration");  // must check again
    ...
    function recoverLeftoverCoverage(address _recipient) public virtual onlyOwner whenNotEmitting {
        require(totalSupply == 0, "Can't recover coverage if tokens are staked");
