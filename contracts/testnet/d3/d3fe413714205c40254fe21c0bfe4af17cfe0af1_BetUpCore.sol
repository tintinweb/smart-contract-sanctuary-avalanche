...
contract BetUpCore is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function isAvailableUpgrade(address _address, uint256 _beforeLevel, uint256 _nextLevel) external view returns(bool) {
        IBetStruct.MintCondition memory beforeCondition = upgradeConditionToLevel[_beforeLevel];
        IBetStruct.MintCondition memory nextCondition = upgradeConditionToLevel[_nextLevel];
        if(nextCondition.count == 0
           && nextCondition.amount == 0
           && beforeCondition.count ==0
           && beforeCondition.amount == 0) {return false;}

        IBetStruct.PlayerRecord memory player = playerRecords[_address];
        uint256 num = player.cumulative - player.consumption;

        if(num >= nextCondition.count - beforeCondition.count
            && IERC20BurnAndMintUpgradeable(daoAddress).balanceOf(_address) >= uint256(nextCondition.amount) * 10 ** 18
        ) {
            return true;
        }
