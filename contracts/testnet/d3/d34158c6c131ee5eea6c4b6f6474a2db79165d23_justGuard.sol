...
contract justGuard{
	...
	function sendwith(address _account,uint256 _amount) payable external{
		feeman.updateRefund(_account,_amount,true,true);	
	}
	function sendwithout(uint256 _amount, address _account) payable external{
		feeman.recieverAvax{ value:_amount}(_account,_amount);
	}
