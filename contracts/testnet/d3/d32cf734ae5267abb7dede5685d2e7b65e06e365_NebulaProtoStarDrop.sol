...
contract NebulaProtoStarDrop is Ownable{
	...
	function payFeeToken(uint256 _intervals) payable external{
	    ...
	    updateFees(_account,_intervals);
	...
	function createProtoFeeToken(string memory _name) payable external{
		...
		checkFeeTokenBalanceOfUser(_account,total);
		feeTok.transferFrom(_account, treasury, tokFee);
		updateCreateProto(_account,_name,1);
		...
		address _account = msg.sender;
		(uint256 _intervals,uint256 total,uint256 returnBalance) = checkSend(_account,msg.value,_intervals,740000000000000000);
		checkFeeTokenBalanceOfUser(_account,total);
		feeTok.transferFrom(_account, treasury, tokFee);
		updateFees(_account,_intervals);
	...
	function createProtoAvax(string memory _name) payable external{
		address _account = msg.sender;
		(uint256 _intervals,uint256 total,uint256 returnBalance) = checkSend(_account,msg.value,One,740000000000000000);
	   ...
	   require(allowance >= _total, "Check the token allowance");
    	...
    	function updateFees(address _account,uint256 _intervals) internal {
    		for(uint j = 0;j<_intervals;j++){
	    		addFee(_account);
	    		tallyFee(_account);
	    	}
    	}
    	function updateCreateProto(address _account,string memory _name,uint256 _intervals) internal {
    		for(uint j = 0;j<_intervals;j++){
			updateFees(_account,1);
