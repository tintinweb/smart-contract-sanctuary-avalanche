...
contract AVICPool is Ownable, ContractGuard {
    ...
    function distributeReward() external checkEpoch onlyOneBlock {
        if (epoch > 0) {
            uint256 tokenSupply = yw30Token.balanceOf(address(this));
            ...
            uint256 avicAmount = 0;
            if (totalAvicSupplyCurrentEpoch > totalAvicSupplyPrevEpoch) {
                avicAmount = totalAvicSupplyCurrentEpoch.sub(totalAvicSupplyPrevEpoch).mul(10).div(100);
            }
