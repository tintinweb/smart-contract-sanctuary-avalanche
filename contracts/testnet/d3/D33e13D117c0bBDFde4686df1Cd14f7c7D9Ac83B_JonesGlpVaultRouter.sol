...
contract JonesGlpVaultRouter is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, Governable, UUPSUpgradeable {
    ...
    function depositGlp(uint256 _assets, address _sender, bool _compound, bool _rebalance) external whenNotPaused returns (uint256) {
        ...
        (uint256 vaultShares, uint256 glpMintIncentive) = _depositGlp(_sender, _assets);

        if(_rebalance) {
            _strategy.onGlpDeposit(_assets, glpMintIncentive);        // borrow stable from vault to strategy
        }
        ...
        returns (uint256, uint256)
    {
        ...
        return (vaultShares, glpMintIncentives);
    }
