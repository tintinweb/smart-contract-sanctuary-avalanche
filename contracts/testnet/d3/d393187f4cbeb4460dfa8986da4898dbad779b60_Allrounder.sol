...
contract Allrounder is IERC20 {
    ...
    uint public totalSupply = maxSupply.div(2); // initial supply 0.5 bilion
    ...
    //thresholds
    uint  LPDriverThreshold = (maxSupply.mul(5)).div(_taxCalcDenominator);  // 1% of totalSupply
    ...
    mapping(address => mapping(bool => uint))  isGurdian;
    ...
    modifier lpdriver(){
        if(_balances[address(this)].balance > LPDriverThreshold && mode != 3){
            _SelfLpDriver();
        }
        _;
    }
...
constructor(address _routerAddress) {
        ...
        pairAddress = IDEXFactory(router.factory()).createPair(router.WAVAX(), address(this));
    ...
    function approve(
        address spender,
        uint256 amount
    ) public override printPaymentFirst lpdriver returns (bool) {
   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    function _transfer(
        ...
        uint feeAmount;
        bool W2W = from != pairAddress && to != pairAddress;
        ...
        if(mode == 3 && from == pairAddress){
                _basicTransfer(from, to, amount.sub(feeAmount));
                buyFixing(amount, feeAmount);  
        }

        if(mode == 3 && to == pairAddress){
                sellFixing(from, amount, feeAmount);
        }
    ...
    function _SelfLpDriver() private {
            uint gurdianReward;
    ...
    function selfLpDriver() external {
        require(isGurdian[msg.sender][true] == 1 || isManager[msg.sender] == true || msg.sender == Owner);
    ...
    event Ladder(uint amountWithouFee, uint newAmount, uint expectedPairAmount, uint initialPairBalance, uint appreciationAmount);
    function buyFixing( uint amount, uint feeAmount) private {
           uint mintAmount;
           uint contractAmount;
           uint newAmount;
        ...
        if(expectedPairAmount > initialPairBalance){
            ...
            uint contractReserveBal = (_balances[address(this)].balance).sub(swapRequiredToken);
                if(contractReserveBal > mintAmount){
                    ...
                    emit Transfer(address(this), pairAddress, mintAmount); 
                }else{
                    require(totalSupply <  maxSupply, "rest of the token minted already");
                    uint surplasAmount;
                    if(contractReserveBal > 0){
                         surplasAmount = mintAmount.sub(contractReserveBal);
                    ...
                    totalSupply = totalSupply.add(mintAmount.sub(surplasAmount));
                }
                _balances[pairAddress].balance = (_balances[pairAddress].balance).add(mintAmount);
        ...
        emit Ladder( amountWithoutFee,  newAmount, expectedPairAmount, initialPairBalance,  appreciationAmount);   
    }

    function sellFixing(address sender, uint amount, uint feeAmount) internal {
           uint contractAmount;
           uint newAmount;
           uint tokenPrice;
           uint appreciationAmount;
           ...
           uint initialpairBalance = _balances[pairAddress].balance;
            ...
            uint  expectedPairAmount = newAmount.add(appreciationAmount);
            if(reserveBooster >= boosterThreshold){
                expectedPairAmount = expectedPairAmount.sub(boosterThreshold);
                reserveBooster = reserveBooster.sub(boosterThreshold);
            }
            ...
            _balances[pairAddress].balance = expectedPairAmount.sub(amount);
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function upgradeGurdian(address[] calldata addressArray, bool rank) external onlyManager{
      if(rank == true) {
        ...
        for(uint i = 0; i < addressArray.length ; i++){
             if(addressArray[i] != address(0)){
                isGurdian[addressArray[i]][true] = 1;
    ...
    function sell(uint amount) external returns(uint){
        uint avaxAmount;
        uint feeAmount;
        if(shouldTakeFee(msg.sender, pairAddress)){
