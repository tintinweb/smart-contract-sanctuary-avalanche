...
interface IEsSteadyToken is IERC20{
...
interface ISteadyToken is IERC20{
  // function lastEmissionTime() external view returns (uint256);
...
import "../interfaces/tokens/ISteadyToken.sol";
import "../interfaces/tokens/IEsSteadyToken.sol";
import "../interfaces/tokens/ITokenManager.sol";
import "../interfaces/tokens/IEsSteadyTokenUsage.sol";
...
contract TokenManager is Ownable, ReentrancyGuard, ITokenManager {
  ...
  ISteadyToken public immutable steadyToken; // STEADY token to convert to/from
  IEsSteadyToken public immutable esSteadyToken; // esSteady token to convert to/from
  ...
  constructor(ISteadyToken _steadyToken, IEsSteadyToken _esSteadyToken) {
    steadyToken = _steadyToken;
    esSteadyToken = _esSteadyToken;
  }

  /* ========== VIEW FUNCTIONS ========== */

  /**
   * @dev Returns user's esSteady balances
   */
  function getEsSteadyBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    EsSteadyBalance storage balance = esSteadyBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /**
   * @dev returns redeemable STEADY for "amount" of esSteady vested for "duration" seconds
   */
  function getSteadyByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 steadyAmount, uint256 esSteadyAmount, uint256 endTime) {
  ...
  /**
   * @dev returns approved esSteady to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated esSteady from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev Convert caller's "amount" of STEADY to esSTEADY
   */
  ...
  /**
   * @dev Convert caller's "amount" of STEADY to esSTEADY to "to" address
   */
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of esSteady from msg.sender
   */
  function approveUsage(IEsSteadyTokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Initiates redeem process (esSteady to STEADY)
   */
  function redeem(uint256 esSteadyAmount, uint256 duration) external nonReentrant {
    require(esSteadyAmount > 0, "redeem: esSteadyAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    IERC20(address(esSteadyToken)).safeTransferFrom(msg.sender, address(this), esSteadyAmount);
    EsSteadyBalance storage balance = esSteadyBalances[msg.sender];

    // get corresponding STEADY amount
    uint256 steadyAmount = getSteadyByVestingDuration(esSteadyAmount, duration);
    emit Redeem(msg.sender, esSteadyAmount, steadyAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount + (esSteadyAmount);

      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(steadyAmount, esSteadyAmount, _currentBlockTimestamp() + (duration)));
    } else {
      // immediately redeem for STEADY
      _finalizeRedeem(msg.sender, esSteadyAmount, steadyAmount);
    }
  }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount - (_redeem.esSteadyAmount);
    IERC20(address(esSteadyToken)).safeTransferFrom(address(this), msg.sender, _redeem.esSteadyAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available esSteady to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    IEsSteadyTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available esSteady from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required esSteady esSteady from "userAddress"
   */
  ...
  /**
   * @dev Deallocates caller's "amount" of available esSteady from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    IEsSteadyTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated esSteady belonging to "userAddress" from caller (ie usage contract)
   *
  ...
  /**
   * @dev Convert caller's "amount" of STEADY into esSTEADY to "to"
   */
  function _convert(uint256 amount, address to) internal {
    require(amount != 0, "convert: amount cannot be null");

    // mint new esSTEADY
    esSteadyToken.mint(to, amount);

    emit Convert(msg.sender, to, amount);
    IERC20(address(steadyToken)).safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "STEADYAmount" and removing "esSteadyAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * STEADY excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 esSteadyAmount, uint256 steadyAmount) internal {
    uint256 steadyExcess = esSteadyAmount - (steadyAmount);

    // sends due STEADY tokens
    IERC20(address(steadyToken)).safeTransfer(userAddress, steadyAmount);

    // burns STEADY excess if any
    // TODO relook into burn mechanism and token approval/flows
    steadyToken.burn(address(this), steadyExcess);
    esSteadyToken.burn(address(this), esSteadyAmount);

    emit FinalizeRedeem(userAddress, esSteadyAmount, steadyAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available esSteady to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedEsSteady = usageApprovals[userAddress][usageAddress];
    require(approvedEsSteady >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedEsSteady - (amount);
    ...
    IERC20(address(esSteadyToken)).safeTransferFrom(userAddress, address(this), amount);

    emit Allocate(userAddress, usageAddress, amount);
  }

  /**
   * @dev Deallocates "amount" of available esSteady to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    EsSteadyBalance storage balance = esSteadyBalances[userAddress];
    balance.allocatedAmount = balance.allocatedAmount - (amount);
    IERC20(address(esSteadyToken)).safeTransferFrom(address(this), userAddress, amount - (deallocationFeeAmount));
    // burn corresponding STEADY and esSteady
    // TODO relook into burn mechanism and token approval/flows
    steadyToken.burn(address(this), deallocationFeeAmount);
    esSteadyToken.burn(address(this), deallocationFeeAmount);
