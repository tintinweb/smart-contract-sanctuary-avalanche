...
contract BondingTreasuryV1 is OwnableUpgradeable, BondingTreasuryStorageV1 {
    ...
    {
        uint256 EmPrice = (uint256(getEmToUSDPrice()) * EmeraldToken.balanceOf(treasuryAccount)) / (10**18);
        uint256 AvaxPrice = ((uint256(getAvaxToUsdPrice()) * treasuryAccount.balance)) / (10**18);
        uint256 WAvaxPrice = ((uint256(getAvaxToUsdPrice()) * WrappedAvax.balanceOf(treasuryAccount))) / (10**18);
        uint256 WUsdtPrice = ((uint256(getUsdtToUsdPrice()) * WrappedUsdt.balanceOf(treasuryAccount)));
