...
contract NGGameFacet is WithStorage {
    ...
    modifier onlyAdminOrCore() {
        require(
            ...
            "Only the Core or Admin addresses can access."
        );
        _;
    }
    ...
    function getGameStateWhitelistTier1() public view onlyAdmin returns(uint16 level) {
        return gameConstants().WHITELIST_TIER_1;
    }

    function getGameStateNFTSaleMode() public view onlyAdmin returns(NFTCurrentSales mode) {
        return gameConstants().NFT_SALE_MODE;
    }

    function setGameStateNFTSaleMode(NFTCurrentSales _mode) public onlyAdmin {
        gameConstants().NFT_SALE_MODE = _mode;
    }
