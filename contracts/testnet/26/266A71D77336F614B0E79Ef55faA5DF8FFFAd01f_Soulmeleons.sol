 ...
 */
contract ERC721R is Context, ERC165, IERC721, IERC721Metadata, ERC2981 {
...
contract Soulmeleons is ERC721R, Ownable {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC721R) returns (bool) {
        return ERC721R.supportsInterface(interfaceId) || ERC2981.supportsInterface(interfaceId);
    }
    ...
    function tokenURI(
        uint256 tokenId
    ) public view virtual override(ERC721R) returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI_ = _baseURI();
        return bytes(baseURI_).length != 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : prerevealURI;
    }

    /**
    * @notice Mints tokens to the sender during public sale.
    * @param _potions    Array of tokens to burn
    */
    function burnAll(uint256[] calldata _potions) external {
        require(mintLive, "Minting is not live yet");
        require(_potions.length <= 20, "Cannot mint more than 10");

        for (uint i = 0; i < _potions.length; i++) {
            IERC721(potions).transferFrom(msg.sender, dead, _potions[i]);
        }
        _mintRandom(msg.sender, _potions.length);
    }
    ...
    function setMint(bool _mintLive) external onlyOwner {
        mintLive = _mintLive;
    }
}
