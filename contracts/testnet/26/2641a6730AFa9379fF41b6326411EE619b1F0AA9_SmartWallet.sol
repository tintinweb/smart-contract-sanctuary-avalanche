...
contract CallManager is Storage {
    // modifier onlyOwner() {
...
contract Implementation is ModulesManager, CallManager {
    ...
    // TODO: to be removed, for testing to get the guardians
    function getGuardians() public returns (address[] memory) {
        (bool success, bytes memory data) = isGuardianModuleEnabled()
            .delegatecall(abi.encodeWithSignature("guardians()"));
        require(success, "Failed to getGuardians");
...
contract ModulesManager is Storage {
