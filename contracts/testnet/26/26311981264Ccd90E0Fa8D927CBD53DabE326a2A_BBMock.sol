...
//import "../buildingBlocks/BaseBuildingBlockUpgradable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
import "../buildingBlocks/BaseBuildingBlockUpgradable.sol";

contract BBMock is  BaseBuildingBlockUpgradable,  UUPSUpgradeable {
    ...
    function initialize(bytes memory data) public initializer {
        (address endpoint, address actionPool, uint16 actionPoolNativeId) = abi.decode(data, (address, address, uint16));
        __Ownable_init();
...
import "../SmartLZBaseUpgradable.sol";

abstract contract BaseBuildingBlockUpgradable is SmartLZBaseUpgradable {
    ...
    function _nonblockingLzReceive(
        ...
        //require(_msgSender() == address(lzEndpoint), "BBB:invalid endpoint caller");
    ...
    function approve(address baseAsset, address spender, uint256 amount) public onlySelf() {
        IERC20(baseAsset).approve(spender, amount);
    }


    function backTokensToNative(address _token, uint256 amount) public onlySelf() {
        IERC20(_token).transfer(nativeRouter, amount);
        emit TransferredToNativeRouter(_token, amount);
    }


    function nativeBridge(address nativeStableToken,
                        uint256 stableAmount,
                        uint16 receiverLZId,
                        address receiverAddress,
                        address destinationStableToken) external  payable {
        require(_msgSender() == _actionPool, "BBB:sender isn't actionPool");
        _bridge(nativeStableToken, stableAmount, receiverLZId, receiverAddress, destinationStableToken);
    }
...
import "./integrations/layerzero/lzApp/NonblockingLzAppUpgradeable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

abstract contract SmartLZBaseUpgradable is NonblockingLzAppUpgradeable {
    ...
    modifier onlySelf() {
        require(msg.sender == address(this), "SmartLZBaseUpgradable:Only self call");
        _;
    }
    ...
    function _bridge(address nativeStableToken,
        uint256 stableAmount,
        uint16 receiverLZId,
        address receiverAddress,
        address destinationStableToken) internal {
