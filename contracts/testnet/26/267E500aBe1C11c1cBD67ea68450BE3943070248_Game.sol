...
contract Game is Withdrawable {
    ...
    function lastClaimTime(address address_) public view returns (uint256) {
        if (_sNodesClaimTime[address_] > 0) {
            return _sNodesClaimTime[address_];
        }
    ...
    function sNodesCount() public view returns (uint256) {
        return _sNodesCount;
    }

    function bNodesCount() public view returns (uint256) {
        return _bNodesCount;
    }
    ...
    function createBNode() public {
        uint256 bNC = _bigNodesContract.nodeBalanceOf(msg.sender);
    ...
    function withdrawBalance(uint256 amount_, bool all) public {
        if (all == false) {
            require(amount_ > 0, "Please increase value for withdraw");
        }
    ...
    function bNodesWorkingTime(address address_) public view returns (uint256) {
        ...
        if (_bigNodesContract.nodeBalanceOf(msg.sender) == 0) {
            return 0;
        }
    ...
    function _goldClaim(
        ...
        if (countNodes > factor) {
            higherRate = factor;
        } else {
            higherRate = countNodes;
        }
    ...
    function _silverClaim(
        ...
        if (maxNFT > 0) {
            ...
            if (goldNFTCount > 0) {
                countNodes = countNodes - maxNFT;
            }

            uint256 higherRate = 0;
            if (countNodes > NFTCount) {
                higherRate = NFTCount;
            } else {
                higherRate = countNodes;
            }
    ...
    function _bNodeClaimValue(address address_) private view returns (uint256) {
        uint256 value = 0;
        uint256 bNC = _bigNodesContract.nodeBalanceOf(address_);
        ...
        if (bNC > 0) {
            ...
            bNodesClaim = bNodesClaim.mul(bNC);
            ...
            if (goldCount >= 2) {
                value += _goldClaim(goldCount, bNC, bNodeBR, bNodesWorkingTime_);
            } else if (goldCount == 1 && silverCount > 0) {
                value += _goldClaim(goldCount, bNC, bNodeBR, bNodesWorkingTime_);
                value += _silverClaim(silverCount, 1, bNC, bNodeBR, bNodesWorkingTime_);
            } else if (goldCount == 0 && silverCount > 0) {
                value += _silverClaim(silverCount, 0, bNC, bNodeBR, bNodesWorkingTime_);
            }
        }
    ...
    function claimValue(address address_) public view returns (uint256) {
        ...
        value += _sNodeClaimValue(address_);

        value += _bNodeClaimValue(address_);
