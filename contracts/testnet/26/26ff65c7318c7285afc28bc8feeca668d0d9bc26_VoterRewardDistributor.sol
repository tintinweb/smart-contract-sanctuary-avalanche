...
contract VoterRewardDistributor is IVoterRewardDistributor {
    ...
    */
    ...
    function addVoteStaker(address voteStaker) internal {
        require(voting.balanceOf(voteStaker) > 0);
        voterIndexes[voteStaker] = Voters.length;
        Voters.push(voteStaker);
    }

    function removeVoteStaker(address voteStaker) internal {
    ...
    function claimVoterReward(address voter) external {
        ...
        require(voterIndexes[voter] < Voters.length, "Voter not found");
        require(voting.balanceOf(voter) > 0, "Voter not found");
        require(voting.balanceOf(voter) >= individualvoterReward, "Voter not found");
