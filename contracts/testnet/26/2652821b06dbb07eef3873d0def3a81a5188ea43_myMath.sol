...
contract myMath{
	using SafeMath for uint256;
		...
		function EXTdecPercentage(uint256 _x,uint256 perc) external view returns(uint256[3] memory){
			decPercentage(_x,perc);
		}	
		function decPercentage(uint256 _x,uint256 perc) internal view returns(uint256[3] memory){
