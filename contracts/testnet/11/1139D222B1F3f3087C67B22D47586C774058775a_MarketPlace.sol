...
contract MarketPlace is Owners, ReentrancyGuard {
	event NewOffer(
	...
	function setMinPrices(
		...
		uint[] calldata _offerPrices,
		uint[] calldata _auctionPrices	
	) 
		external 
		onlyOwners 
	{
		require(_names.length == _offerPrices.length, "MarketPlace: Length mismatch");
		require(_names.length == _auctionPrices.length, "MarketPlace: Length mismatch");
			
		INft(_nft).tokenIdsToType(0); // pseudo check token standard
	...
	function deleteMapPrice(address _nft, string[] calldata names) external onlyOwners {
	...
	function sellOfferItem(address _nft, uint _tokenId, uint _price) external nonReentrant {
		...
		require(!isBlacklistedNft[_nft], "MarketPlace: Not authorized contract");
		require(openOffer, "MarketPlace: Not open");

		if (nftPrices.inserted[_nft]) {
			string memory name = INft(_nft).tokenIdsToType(_tokenId);

			MapPrices storage mapPrices = nftPrices.values[_nft];
			require(mapPrices.inserted[name], "MarketPlace: Contact core team");
			require(mapPrices.values[name].offerPrice <= _price, 
					"MarketPlace: Price lower than min price");

			if (onlyAttribute[_nft]) {
				string memory attribute = INft(_nft).getAttribute(_tokenId);
				require(bytes(attribute).length > 0, 
						"MarketPlace: Only special Nft");
			}
		}
	...
	function purchaseOfferItem(address _nft, uint _tokenId) external nonReentrant {
		require(nft.inserted[_nft], "MarketPlace: Nft contract is not setup");
		require(nft.values[_nft].mapOffer.inserted[_tokenId], "MarketPlace: Item doesnt exist");

		Offer memory offer = nft.values[_nft].mapOffer.values[_tokenId];

		require(offer.owner != msg.sender, "MarketPlace: You cannot buy your own nft");

		uint feePrice = offer.price * fee / 10000;

		IERC20(token).transferFrom(msg.sender, offer.owner, offer.price - feePrice);
		if (feePrice > 0)
			IERC20(token).transferFrom(msg.sender, swapper, feePrice);
	...
	function recoverOfferItem(address _nft, uint _tokenId) external nonReentrant {
		...
		require(offer.owner == msg.sender || isOwner[msg.sender], "MarketPlace: Unauthorized");
	...
	{
		require(!isBlacklistedNft[_nft], "MarketPlace: Not authorized contract");
		require(openAuction, "MarketPlace: Not open");
	...
	function purchaseAuctionItem(address _nft, uint _tokenId, uint _currentPrice) external nonReentrant {
		require(nft.inserted[_nft], "MarketPlace: Nft contract is not setup");
		require(nft.values[_nft].mapAuction.inserted[_tokenId], "MarketPlace: Item doesnt exist");

		Auction memory auction = nft.values[_nft].mapAuction.values[_tokenId];

		require(auction.owner != msg.sender, "MarketPlace: You cannot buy your own nft");
		require(block.timestamp < auction.end, "MarketPlace: Auction already finished");
		require(_currentPrice > auction.currentPrice, 
				"MarketPlace: New price must be bigger than current one");

		if (auction.owner != auction.nextOwner)
			IERC20(token).transfer(auction.nextOwner, auction.currentPrice);
		IERC20(token).transferFrom(msg.sender, address(this), _currentPrice);
	...
	function recoverAuctionItem(address _nft, uint _tokenId) external nonReentrant {
		...
		require(auction.nextOwner == msg.sender || isOwner[msg.sender], "MarketPlace: Unauthorized");
		require(block.timestamp >= auction.end, "MarketPlace: Auction not finished yet");

		if (auction.owner != auction.nextOwner) {
			uint feePrice = auction.currentPrice * fee / 10000;

			IERC20(token).transfer(auction.owner, auction.currentPrice - feePrice);
			if (feePrice > 0)
				IERC20(token).transfer(swapper, feePrice);
		}
	...
	function setFee(uint _new) external onlySuperOwner {
		fee = _new;
	}
	...
	function setOnlyAttribute(address _addr, bool _new) external onlySuperOwner {
