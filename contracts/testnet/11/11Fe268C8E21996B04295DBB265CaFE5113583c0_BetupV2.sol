...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    public view returns(IBetStruct.RewardType memory reward)
    {
        if(_pcr.cPrice == 0) {
            return reward;
        }
        ...
        if(_maxLevel == 0) {
            require(
                IMembershipCard(membershipAddress).getSnapshot(_account) == 0,
                "Invalid _maxLevel");
        }else {
            require(
                IMembershipCard(membershipAddress).getSnapshot(_account) == 2 ** (256 - _maxLevel),
                "Invalid _maxLevel");
        }
    ...
    function _assembleAmount(
        ...
        if(_ids.length == 1 && _refund  > 0) {
            ...
            if(_i == 0) {
                ...
                address preChip = IPairManager(pairManagerAddress).pairsConfig(preId).chip;
                if(preChip == _chip) {
                    _accAmount += _refund;
                }else {
