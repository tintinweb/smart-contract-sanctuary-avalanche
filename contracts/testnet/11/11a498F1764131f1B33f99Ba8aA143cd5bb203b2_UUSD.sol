...
contract UUSD is Ownable, Pausable, Blacklistable, ERC20 {
    ...
    address private constant FACTORY_CONTRACT = address(0x37DB6251a1950DbBBabE9e7DCAb8A9aA7fEB02B6); //Testnet address
    address private UNISWAP_V2_ROUTER = address(0x4e6740Ed427f61912a95eA766B1e093a97a187eF); //Testnet Address
    ...
    */
    ...
    //BUY UUSD using any Stable coin which is whitelisted in the Contract
    //BUY UUSD using Ethereum
    //BUY UUSD using any Token (ERC20)
    function buy(address _token,uint256 _amount) external payable {
        
        //For stable coins we are minting same amount of tokens
        //Stable coins to be whitelisted
        if(isWhitelisted(_token)){ 
            ERC20(_token).transferFrom(msg.sender, vault, _amount); // address(this) --> vault
            _mint(msg.sender, _amount);
        }
        else{ // For Ether, we are converting Eth to USDC 
            if(msg.value > 0){
               ...
               // uint256 amount = swappedTokenAmount[1].div(1000000)
    ...
    function swap(address _swapToken,uint256 _amountIn) internal returns(uint){
        ...
        uint deadline = block.timestamp + 250;
        address[] memory path = new address[](2);
        address pair1 = uniswapFactory.getPair(_swapToken, address(usdcToken)); //check pair with USDC
        address pair2 = uniswapFactory.getPair(_swapToken, address(usdtToken)); //check pair with USDT
        if(pair1 != address(0)){
    ...
    function settleAmount() external onlyOwner {
        uint256 usdc_balance = usdcToken.balanceOf(vault);
        uint deadline = block.timestamp + 250;
        ...
        path[1] = address(daiToken);
        usdcToken.transferFrom(vault,address(this),usdc_balance.div(3));
        usdcToken.approve(UNISWAP_V2_ROUTER,usdc_balance.div(3));
         uniswapRouter.swapTokensForExactTokens(usdc_balance.div(3), 0, path, vault, deadline); 
        

        path[2] = address(usdcToken);
        path[3] = address(usdtToken);
        usdcToken.transferFrom(vault,address(this),usdc_balance.div(3));
        usdcToken.approve(UNISWAP_V2_ROUTER,usdc_balance.div(3));
        uniswapRouter.swapTokensForExactTokens(usdc_balance.div(3), 0, path, vault, deadline); 
        ...
        usdcToken.approve(UNISWAP_V2_ROUTER, usdcToken.balanceOf(vault));
        usdtToken.approve(UNISWAP_V2_ROUTER, usdtToken.balanceOf(vault));
        daiToken.approve(UNISWAP_V2_ROUTER, daiToken.balanceOf(vault));
    ...
    function removeLiquidity(address _tokenA, address _tokenB,uint256 _percentage) public onlyOwner {
        ...
        uint liquidity = (IERC20(pair).balanceOf(vault)).mul(_percentage).div(100);
