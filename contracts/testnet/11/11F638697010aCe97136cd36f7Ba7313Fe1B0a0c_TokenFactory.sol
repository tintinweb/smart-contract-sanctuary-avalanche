...
contract TokenFactory {
    ...
    bytes32 salt;
    //  = 0x4920616d20546f6b656e20666f7220616e204173736574000000000000000000;
    ...
    function deployIdentity(address _initialManagementKey, bool _isLibrary) public returns (address) {
        // require(tx.origin == msg.sender,"Only owner can call");
        // address _identityLogic = _clone(identityImpl,salt);
        address identityProxy = address(new ProxyFinal());
        // address identityProxy = _clone(proxyImpl,salt1);
        // console.log("logic: ",_identityLogic);
        // console.log("proxy: ",proxyImpl);
        // console.log("I am in Identity",identityProxy);
        // console.log("owner",IToken(identityProxy).proxyOwner());
        // // (bool success1,) = _identity.call(abi.encodeWithSelector(0xe1c7392a));
        // // require(success1,"init failed");
        // (bool success,) = identityProxy.call(abi.encodeWithSelector(0xe1c7392a));
    ...
    function _initProxy(address identityRegistry,string memory _name,string memory _symbol,uint8 _decimals,uint _cap, string memory mappingValue) internal{
        // address token = _clone(tokenImpl,salt);
        address compliance = _clone(complianceImpl,salt);
        address proxy = address(new ProxyFinal());
        // address proxy = _clone(proxyImpl,salt);
        address identity = deployIdentity(proxy,false);
        // (bool success,) = proxy.call(abi.encodeWithSelector(0xe1c7392a));
        // require(success,"init failed");
        // success = false;
...
contract ProxyFinal{
    ...
    function upgradeTo(address newImplementation) public onlyProxyOwner {
        console.log("yo");
        console.log("ok",newImplementation);
    ...
    function _upgradeTo(address newImplementation) internal {
        ...
        console.log("-=======IDHAR======->",currentImplementation);
        console.log("-=======UDHAR======->",newImplementation);
        require(currentImplementation != newImplementation, 'OwnedUpgradeabilityProxy: INVALID');
        console.log("ye nhi hai");
