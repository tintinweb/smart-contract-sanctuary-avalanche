...
contract Avaxer {
	...
	constructor(address payable devWallet, address payable commissionWallet, uint256 startDate) public {
		require(!isContract(devWallet));
		require(!isContract(commissionWallet));
		require(startDate > 0);
		devWallet = devWallet;
	...
	function invest(address referrer, uint8 plan) public payable {
        require(block.timestamp > startUNIX, "Contract is not started yet");
	...
	function withdraw() public {
		...
		totalAmount = totalAmount.sub(devFee);
		emit FeePayed(msg.sender, devFee);
		
		uint256 projectFee = totalAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		
		commissionWallet.transfer(projectFee);
		totalAmount = totalAmount.sub(projectFee);
