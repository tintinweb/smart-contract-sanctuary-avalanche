...
contract CopperProxyV2 is Ownable {
    ...
    address public constant VAULT =
        address(0x6e51BC579946291c48673eff5A39Db0Cab727279);
     ...
     */
    function getBPTTokenBalance(address pool)
        external
        view
        returns (uint256 bptBalance)
    {
     ...
     */
    function getRecipientShareBPS(address recipientAddress)
        external
        view
        returns (uint256 shareSize)
    {
     ...
     */
    function createLBP(PoolConfig memory poolConfig)
        external
        returns (address)
    {
        // 1: deposit tokens and approve vault
        require(
            poolConfig.tokens.length == 2,
            "Copper LBPs must have exactly two tokens"
        );
        require(
            poolConfig.tokens[0] != poolConfig.tokens[1],
            "LBP tokens must be unique"
        );
        require(
            poolConfig.startTime > block.timestamp,
            "LBP start time must be in the future"
        );
        require(
        ...
        bool msgSenderIsNotBlocked = IBlocklist(blockListAddress).isNotBlocked(
            msg.sender
        );
     ...
     */
    function setSwapEnabled(address pool, bool swapEnabled)
        external
        onlyPoolOwner(pool)
    {
     ...
     */
    function transferPoolOwnership(address pool, address newOwner)
        external
        onlyPoolOwner(pool)
    {
        require(blockListAddress != address(0), "no blocklist address set");
        bool newOwnerIsNotBlocked = IBlocklist(blockListAddress).isNotBlocked(
            msg.sender
        );
     ...
     */
    function _calcBPTokenToBurn(address pool, uint256 maxBPTTokenOut)
        internal
        view
        returns (uint256)
    {
        ...
        require(
            maxBPTTokenOut <= bptBalance,
            "Specifed BPT out amount out exceeds owner balance"
        );
    ...
    function exitPool(
        ...
        // 1. Get pool data
        bytes32 poolId = LBP(pool).getPoolId();
        (address[] memory poolTokens, uint256[] memory balances, ) = Vault(
            VAULT
        ).getPoolTokens(poolId);
        require(
            poolTokens.length == minAmountsOut.length,
        ...
        (, uint256[] memory balancesAfterExit, ) = Vault(VAULT).getPoolTokens(
            poolId
        );
    ...
    function _distributeTokens(
        ...
        if (fundTokenFromPool > poolData.fundTokenInputAmount) {
            uint256 totalPlatformAccessFeeAmount = ((fundTokenFromPool -
                poolData.fundTokenInputAmount) * platformAccessFeeBPS) /
                _TEN_THOUSAND_BPS;
            // Fund amount after substracting the fee
            remainingFundBalance =
                fundTokenFromPool -
                totalPlatformAccessFeeAmount;

            if (isStandardFee == true) {
                _distributePlatformAccessFee(
                    pool,
                    fundToken,
                    totalPlatformAccessFeeAmount
                );
    ...
    function _distributePlatformAccessFee(
        ...
        for (uint256 i = 0; i < recipientsLength; i++) {
            ...
            uint256 proportionalAmount = (totalFeeAmount *
                _feeRecipientsBPS[recipientAddress]) / _TEN_THOUSAND_BPS;
    ...
    function updateRecipients(
        ...
        require(
            recipientShareBPS.length > 0,
            "recipientShareBPS must have values"
        );
        require(
        ...
        for (uint256 i = 0; i < arraysLength; i++) {
            require(
                recipientShareBPS[i] > uint256(0),
                "Share BPS size must be greater than 0"
            );
            bool recipientIsNotBlocked = IBlocklist(blockListAddress)
                .isNotBlocked(recipients[i]);
        ...
        require(
            _recipientAddresses.length() == recipientShareBPS.length,
            "Fee recipient address must be unique"
        );
        // emit event
    ...
    function updateBlocklistAddress(address contractAddress)
        external
        onlyOwner
    {
