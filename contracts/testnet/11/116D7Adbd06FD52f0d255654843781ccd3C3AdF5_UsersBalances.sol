...
contract UsersBalances is ReentrancyGuard, AccessControl, SignerController {
  ...
  function addBalance(uint256 profileId, uint256 _amount) public {
    ...
    emit Events.BalanceAdded(profileId, _amount, block.timestamp);
  }
...
library Events {
   ...
   * @param readModule The address of the read module.
   * @param readModuleReturnData The data returned from the read module at initialization. This is abi
   * encoded and totally depends on the read module chosen.
   * @param timestamp The current block timestamp.
   */
  event CommentCreated(
    ...
    /**
   * @dev Emitted when a "reaction" is published.
   *
   * @param profileId The profile's token ID.
   * @param pubId The new publication's ID.
   * @param contentURI The URI mapped to this new publication.
   ...
   * @param to The profile ID the balance is added for.
   * @param app The application that moves the balance.
...
contract LensHub is LensNFTBase, VersionedInitializable, LensMultiState, LensHubStorage, ILensHub {
  ...
  /// @inheritdoc ILensHub
  function react(DataTypes.ReactionData calldata vars) external override whenPublishingEnabled returns (uint256) {
    _validateCallerIsProfileOwnerOrDispatcher(vars.profileId);
    return _createReaction(vars);
  }
  ...
  function getPubType(uint256 profileId, uint256 pubId) external view override returns (DataTypes.PubType) {
    if (pubId == 0 || _profileById[profileId].pubCount < pubId) {
      ...
      // Check for contentURI to distinguish between mirror and reaction publications
    } else if (_pubByIdByProfile[profileId][pubId].collectModule == address(0) && bytes(_pubByIdByProfile[profileId][pubId].contentURI).length == bytes('').length) {
      return DataTypes.PubType.Mirror;
    } else if (_pubByIdByProfile[profileId][pubId].collectModule == address(0)) {
      return DataTypes.PubType.Reaction;
  ...
  function _createReaction(DataTypes.ReactionData memory vars) internal returns (uint256) {
    unchecked {
      uint256 pubId = ++_profileById[vars.profileId].pubCount;
      PublishingLogic.createReaction(vars, pubId, _profileById, _pubByIdByProfile);
      return pubId;
    }
  }
...
library PublishingLogic {
  ...
  function createComment(
    ...
    _emitCommentCreated(vars, pubId, collectModuleReturnData, referenceModuleReturnData, readModuleReturnData);
  }
  ...
  function createMirror(
   ...
   /**
   * @notice Creates a reaction publication mapped to the given profile.
   *
   * @param vars The CommentData struct to use to create the comment.
   * @param pubId The publication ID to associate with this publication.
   * @param _profileById The storage reference to the mapping of profile structs by IDs.
  ...
  function createReaction(
    DataTypes.ReactionData memory vars,
    uint256 pubId,
    mapping(uint256 => DataTypes.ProfileStruct) storage _profileById,
    mapping(uint256 => mapping(uint256 => DataTypes.PublicationStruct)) storage _pubByIdByProfile
  ) external {
    ...
    // Ensure the pointed publication is not the comment being created
    if (vars.profileId == vars.profileIdPointed && vars.pubIdPointed == pubId) revert Errors.CannotReactOnSelf();
    ...
    address refModule = _pubByIdByProfile[vars.profileIdPointed][vars.pubIdPointed].referenceModule;
    if (refModule != address(0)) {
      IReferenceModule(refModule).processReaction(
        vars.profileId,
        vars.profileIdPointed,
        vars.pubIdPointed,
        vars.referenceModuleData
    ...
    // Prevents a stack too deep error
    _emitReactionCreated(vars, pubId);
  }
  ...
  function _emitCommentCreated(
    ...
    bytes memory readModuleReturnData
  ) private {
    emit Events.CommentCreated(
      ...
      referenceModuleReturnData,
      vars.readModule,
      readModuleReturnData,
      block.timestamp
    );
  }

  function _emitReactionCreated(
    DataTypes.ReactionData memory vars,
    uint256 pubId
  ) private {
    emit Events.ReactionCreated(
      vars.profileId,
      pubId,
      vars.contentURI,
      vars.profileIdPointed,
...
interface IReferenceModule {
  ...
  /**
   * @notice Processes a reaction referencing a given publication. This can only be called by the hub.
   *
   * @param profileId The token ID of the profile associated with the publication being published.
   * @param profileIdPointed The profile ID of the profile associated the publication being referenced.
   * @param pubIdPointed The publication ID of the publication being referenced.
