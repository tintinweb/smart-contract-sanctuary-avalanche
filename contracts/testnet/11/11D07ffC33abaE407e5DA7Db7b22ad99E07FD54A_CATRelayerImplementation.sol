...
contract CATRelayer is CATRelayerGovernance {
    ...
    */
    function deployToken(
        ...
        address projectedTokenAddress = computeAddress(salt, name, symbol, decimals, totalSupply);
    ...
    function computeAddress(bytes32 salt,             string calldata name,
                                                      string calldata symbol,
                                                      uint8 decimals,
                                                      uint256 totalSupply ) public view returns (address addr) {
        bytes memory byteCodeForContract = type(CATERC20).creationCode;

        bytes memory contractCode = abi.encodePacked(byteCodeForContract, abi.encode(name, symbol, decimals, totalSupply));
