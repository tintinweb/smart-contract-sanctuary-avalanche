...
contract VotingRegistry is IFactoryDAORegistry, AbstractPoolContractsRegistry {
    string public constant VOTING_REGISTRY_RESOURCE = "VOTING_REGISTRY_RESOURCE";

    string public constant VOTING_FACTORY_DEP = "VOTING_FACTORY";

    string public constant VOTING_NAME = "VOTING";

    PermissionManager internal _permissionManager;

    address internal _votingFactory;
    ...
    modifier onlyVotingFactory() {
        require(_votingFactory == msg.sender, "VotingRegistry: caller is not a factory");
        _;
    }
    ...
    function setDependencies(address registryAddress_, bytes calldata data_) public override {
        ...
        _votingFactory = registry_.getContract(VOTING_FACTORY_DEP);
    }
    ...
    function addProxyPool(string calldata name_, address poolAddress_) external onlyVotingFactory {
        _addProxyPool(name_, poolAddress_);
    }

    function _requirePermission(string memory permission_) internal view {
        require(
            _permissionManager.hasPermission(msg.sender, VOTING_REGISTRY_RESOURCE, permission_),
            "VotingRegistry: access denied"
        );
    }
}
