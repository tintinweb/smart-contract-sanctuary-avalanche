...
abstract contract feeManager is Context {
    ...
    function recieverFeeTok(address _account,uint256 _amount,bool _bool,bool _isAvax) payable virtual external;
...
contract NebulaProtoStarDrop is Ownable{
	...
	constructor(){
		...
		feeTok = IERC20(0x5dAB77B75bf1400952c6F960D4Ea69F3Ee730bb0);
		Managers.push(owner());
	...
	function payFeeTokenSpec(uint256 _intervals,uint256 _x) payable external{
	    ...
	    feeMGR.recieverFeeTok(_account,total,true,true);
	...
	function createProtoFeeToken(string memory _name) payable external{
		...
		feeMGR.recieverFeeTok(_account,tokFee,true,true);
		payable(_account).transfer(msg.value);
	}
	function payAvaxFeeSpec(uint256 _intervals,uint256 _x) payable external{
		...
		feeMGR.updateRefund(_account,tokFee,true,true);
		payable(_feeManager).transfer(740000000000000000);
		feeMGR.recieverAvax(_account,sent-740000000000000000);
	}
	function claimProtoAvaxS(string memory _name) payable external{
    		require(avaxOff == false,"sorry Avax fees have been haulted");
    			uint256 sent = msg.value;
		address _account = msg.sender;
    		theBigCheck(_account,_name);
		...
		feeMGR.recieverAvax{ value:sent}(_account,sendback);
	}
		...
		function theBigCheck(address _account,string memory _name)public {
