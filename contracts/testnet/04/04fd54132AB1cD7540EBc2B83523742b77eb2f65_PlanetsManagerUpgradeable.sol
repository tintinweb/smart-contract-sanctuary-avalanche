    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    uint256[3] public tierLevelThreshold;
    uint32[3] public tierMultiplier;
    ...
    function initialize() external initializer {
        ...
        // Initialize contract
        changeRewardPerDay(11574); // 4% per day
    ...
    function createPlanetWithTokens(
        ...
        //fonction if pour déterminer le tier selon planetvalue

        if (planetValue >= tierLevelThreshold[2]) {
            logTier(tierMultiplier[2], int256(planetValue)); //enregistrement dans tier storage
            ...
            _planets[newPlanetId] = PlanetEntity({
                ...
                rewardMult: tierMultiplier[2],
                planetValue: planetValue,
                totalClaimed: 0,
                exists: true
        });
        } else if (planetValue >= tierLevelThreshold[2]) {
            logTier(tierMultiplier[1], int256(planetValue));
            ...
            _planets[newPlanetId] = PlanetEntity({
                ...
                creationTime: currentTime,
                lastProcessingTimestamp: currentTime,
                rewardMult: tierMultiplier[1],
                planetValue: planetValue,
                totalClaimed: 0,
                exists: true
        });
        } else {
            logTier(tierMultiplier[0], int256(planetValue));
        ...
        returns (uint256)
    {
        ...
        if (planet.rewardMult != tierMultiplier[0]) {
            logTier(planet.rewardMult, -int256(planet.planetValue));
            logTier(tierMultiplier[0], int256(planet.planetValue));
        }
    ...
    function _calculateRewardsFromValue(
        ...
        uint256 _rewardMult,
        uint256 _timeRewards, //nombre de block contenu dans 24h
        uint256 _rewardPerDay
    ) private pure returns (uint256) {
        uint256 rewards = (_timeRewards * _rewardPerDay) / 1000000; //
        uint256 rewardsMultiplicated = (rewards * _rewardMult) / 100000; //tier du note 
    ...
    function setTierThreshold(
        uint256[3] memory _tierLevel,
        uint32[3] memory _tierMult
    ) public onlyOwner {
        require(
            _tierLevel.length == 3,
            "Planets: newTierLevels length has to be 3"
        );
        require(
            _tierMult.length == 3,
            "Planets: newTierSlopes length has to be 3"
        );
        tierLevelThreshold = _tierLevel;
        tierMultiplier = _tierMult;
    }
