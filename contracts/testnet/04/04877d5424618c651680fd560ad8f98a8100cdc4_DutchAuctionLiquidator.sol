...
contract DutchAuctionLiquidator is YSSModuleExtended {
  ...
  CDPModule public cdpModule;

  // collateralTypeId => collateralTypeParams
  mapping(uint256 => CTypeParams) public cTypeParamsMapping;
  ...
  modifier notDone(uint256 auctionId) {
    require(!auctions[auctionId].done, "YSS: Auction done");
    _;
  }
  ...
  function liquidate(
    uint256 vaultId
  ) external onlyWhitelist returns (bool successful) {
  ...
  function claim(
    ...
    require(!isExpired(auctionId), "YSS: Auction expired");

    uint256 price = getPrice(auctionId);
    require(price <= maxPrice, "YSS: price > maxPrice");
    ...
    balanceSheet.addSurplus(int256(price));
    auction.done = true;
  ...
  function resetAuction(
    ...
    require(isExpired(auctionId), "YSS: Auction not expired");
...
/// @notice Manages the creation and maintenance of collateralized debt pools
/// (CDPs)
contract CDPModule is YSSModuleExtended {
  ...
  // collateralTypeId => (borrower => isAllowed)
  mapping(uint256 => mapping(address => bool)) allowedBorrowers;

  event Borrow(uint256 vaultId, uint256 amount);
  event Repay(uint256 vaultId, uint256 amount);
  event AddCollateral(uint256 vaultId, uint256 amount);
  event RemoveCollateral(uint256 vaultId, uint256 amount);
  event CreateVault(
    uint256 collateralTypeId,
    uint256 collateralAmount,
    uint256 loanAmount
  );
  event Liquidate(uint256 vaultId);

  modifier onlyVaultOwner(uint256 vaultId) {
    require(vaults[vaultId].owner == msg.sender, "YSS: Must be vault owner");
    _;
  }

  modifier notLiquidated(uint256 vaultId) {
    require(!vaults[vaultId].isLiquidated, "YSS: Vault already liquidated");
    _;
  }
  ...
  function borrow(
    ...
    require(getTotalDebt(cTypeId)
      <= cType.debtCeiling, "YSS: Debt ceiling exceeded");
    stablecoin.mint(msg.sender, amount);

    emit Borrow(vaultId, amount);
  }
  ...
  function addCollateral(
    ...
    getCollateralType(vaultId).token.transferFrom(
      msg.sender,
      address(this),
      amount
    );
    ...
    emit AddCollateral(vaultId, amount);
  }
  ...
  function removeCollateral(
    ...
    vaults[vaultId].collateralAmount -= amount;
    getCollateralType(vaultId).totalCollateral -= amount;
    require(!underCollateralized(vaultId), "YSS: Vault undercollateralized");
    getCollateralType(vaultId).token.transfer(msg.sender, amount);

    emit RemoveCollateral(vaultId, amount);
  }
  ...
  function createVault(
    ...
    require(loanAmount > 0, "YSS: loanAmount == 0");
    Vault memory vault;
    ...
    borrow(vaultId, loanAmount);

    emit CreateVault(collateralTypeId, collateralAmount, loanAmount);
  }
  ...
  function liquidate(uint256 vaultId) notLiquidated(vaultId) external {
    ...
    require(underCollateralized(vaultId), "YSS: Vault not undercollateralized");
    emit Liquidate(vaultId);
    ...
    revert("YSS: No liquidator accepted the liquidation");
  }

  /// @notice Used by whitelist contracts/EOAs to transfer tokens out
  function transfer(
    IERC20 token,
    address to,
    uint256 amount
  ...
  function clearVault(uint256 vaultId) external onlyWhitelist {
    Vault storage vault = vaults[vaultId];
    updateInterest(vault.collateralTypeId);
    CollateralType storage cType = getCollateralType(vaultId);
    ...
    balanceSheet.addDeficit(int256(getDebt(vaultId)));
    setDebt(vaultId, 0);
  }
  ...
  function addCollateralType(
    ...
    bool enabled,
    bool allowListEnabled
  ) external onlyWhitelist returns (uint256 collateralTypeId) {
    ...
    return collateralTypes.length - 1;
  }

  /// @notice Sets a collateral type's parameters
  function setCollateralTypeParams(
    uint256 collateralTypeId,
    uint256 debtFloor,
    uint256 debtCeiling,
    uint256 collateralRatio,
    ...
    newCollateralType.enabled = enabled;
  ...
  function getCollateralToken(
    uint256 vaultId
  ) public view returns (IERC20 collateralToken) {
  ...
  /// @notice Returns the price of the vault's collateral
  ...
  function getCollateralValue(uint256 vaultId) public view returns (uint256) {
    uint256 price = getCollateralPrice(vaultId);
    return vaults[vaultId].collateralAmount.mul(price);
  }
  ...
  function requireValidDebtAmount(
    ...
    require(validDebtAmount(vaultId, amount), "YSS: Invalid debt amount");
  }

  /// @notice Verify that the debt amount is not greater than 0 and lower than
  /// the debt floor simultaneously. Also verifies the vault is not
  /// undercollateralized.
...
abstract contract YSSModule {
  YSS public stablecoin;
  
  modifier onlyWhitelist() {
    require(stablecoin.whitelist(msg.sender));
    _;
  }
...
contract BalanceSheetModule is YSSModule {
  ...
  event AddSurplus(int256 amount);
  event AddDeficit(int256 amount);
  event SetSurplus(int256 amount);

  constructor(YSS _stablecoin) YSSModule(_stablecoin) {}

  /// @notice Registers a protocol surplus
  function addSurplus(int256 amount) external onlyWhitelist {
    totalSurplus += amount;
    emit AddSurplus(amount);
  }

  /// @notice Registers a protocol deficit
  function addDeficit(int256 amount) external onlyWhitelist {
    totalSurplus -= amount;
    emit AddDeficit(amount);
  }

  /// @notice Sets the protocol surplus
  function setSurplus(int256 amount) external onlyWhitelist {
    totalSurplus = amount;
    emit SetSurplus(amount);
  }
}
...
// @notice Liquidates undercollateralized CDPs. Can be combined with other
// liquidators.
...
// @notice YSS stablecoin.
contract YSS is ModularToken {
  constructor() ModularToken(0, "Yama Settlement Standard", "YSS") {}
}
