...
contract BetUpCore is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function isAvailableMint(address _address, uint256 _nextMintedCount) external view returns(bool) {
        IBetStruct.MintCondition memory condition = mintCondition[_nextMintedCount];

        if( playerRecords[_address].amount >= condition.count
            && playerRecords[_address].accAmount >= condition.amount * 10 ** 6) {
            return true;
        }
        return false;
    }

    function isAvailableUpgrade(address _address, uint256 _nextLevel) external view returns(bool) {
        IBetStruct.MintCondition memory condition = upgradeConditionToLevel[_nextLevel];

        if(playerRecords[_address].amount >= condition.count
            && IERC20BurnAndMintUpgradeable(daoAddress).balanceOf(_address) >= condition.amount
        ) {
            return true;
        }
  ...
  */
    function calcDaoAmountPerUnit(uint256 _level) external view returns(uint256, uint256) {
        ...
        uint256 win =  _level == 1 ? 1e18 : 125  * number / 100;
        return (win, 50 * number / 100);
    }

    function updateRewardCoefficient(uint256 _ID, uint256 _rewardCoefficient) external onlyRole(REWARD_MANAGER)  {
        require(_rewardCoefficient > 0, "Invalid _rewardCoefficient");
        uint256 _historyCf = 0;
    ...
    function setPlayerRecord(address _player, uint256 _amount, uint256 _decimal) external onlyRole(RECORD_MANAGER) {
        playerRecords[_player].amount++;

        uint256 __amount = _decimal > 6 ? _amount / 10 ** (_decimal-6) : _amount;
        playerRecords[_player].accAmount += __amount;
    }

    function decreasePlayerAmount(address _player, uint256 _minusAmount) external onlyRole(RECORD_MANAGER) {
        require(playerRecords[_player].accAmount >= _minusAmount * 10 ** 6, "out out amount");
        playerRecords[_player].amount -= _minusAmount * 10 ** 6;
    }
