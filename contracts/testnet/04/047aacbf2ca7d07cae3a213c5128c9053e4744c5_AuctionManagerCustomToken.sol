...
contract AuctionManagerCustomToken is Ownable {
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        address[] memory addrs = new address[](_bids.length);
        uint256[] memory bidPrice = new uint256[](_bids.length);
        for (uint256 i = 0; i < _bids.length; i++) {
            addrs[i] = _bids[i].sender;
            bidPrice[i] = _bids[i].bid;
        }
    ...
    function placeBid(address _nftContract, uint256 _tokenId, uint256 _bidValue) external returns(bool) {
        ...
        require(_bidValue >= _auction.maxBid + _auction.minIncrement, "The bid must be higher or equal to the current bid + the minimum increment"); // The bid must be higher than the current bid + the minimum increment
    ...
    function exchangeAssets(address _nftContract, uint256 _tokenId) public returns(bool) {
        ...
        emit WithdrawToken(_auction.maxBidder); // Emit a withdraw token event
        ...
        emit WithdrawFunds(msg.sender, _auction.maxBid); // Emit a withdraw funds event
    ...
    function cancelAuction(address _nftContract, uint256 _tokenId) external returns(bool){ // Cancel the auction
        ...
        require(getAuctionState(_auction.auctionId) == AuctionState.OPEN, "Auction is alreay closed."); // The auction must be open
