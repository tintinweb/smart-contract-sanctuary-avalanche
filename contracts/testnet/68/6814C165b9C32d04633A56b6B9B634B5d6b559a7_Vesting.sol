...
contract Vesting {
    ...
    function getVestingEndTime() public view returns(uint256) {
        ...
        // /// for test net 
        for (uint i = 0; i < 12; i++) {
            if (block.timestamp > getTGETime().add(claimPeriodTime.mul(12 hours).mul(i)) && block.timestamp <= getTGETime().add(claimPeriodTime.mul(12 hours).mul(i.add(1)))) {
                VestingEndTime = getTGETime().add(claimPeriodTime.mul(12 hours).mul(i.add(1)));
                break;
            } else if(block.timestamp > getTGETime().add(claimPeriodTime.mul(12 hours).mul(12))) {
                VestingEndTime = getTGETime().add(claimPeriodTime.mul(12 hours).mul(11));
                break;
            }
        }
