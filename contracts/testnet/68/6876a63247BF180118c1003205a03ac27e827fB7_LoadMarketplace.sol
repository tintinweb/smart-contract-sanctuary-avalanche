...
// File: contracts/marketplace/LoadMarketplace.sol
...
contract LoadMarketplace is ReentrancyGuard, Ownable, ERC721Holder, ERC1155Holder {
    event TokenOnSale(address seller, address nftContract, uint256 nftId, uint256 price, uint256 category, uint256 saleIndex, bool isERC1155, uint256 quantity);
    ...
    function saleNFT(
        ...
        saleNFTList[_saleId] = SaleNFT(_tokenId, _price, msg.sender, _category, _tokenContract, _isERC1155, _quantity);
        onSaleNftAmount++;

        saleIds.increment();
        emit TokenOnSale(msg.sender, _tokenContract, _tokenId, _price, _category, _saleId, _isERC1155, _quantity);
    }
    ...
    function removeNFT(uint256 index) external nonReentrant {
        ...
        if (!saleNFTList[index].isERC1155) {
            ...
            nftTokenContract.safeTransferFrom(address(this), msg.sender, saleNFTList[index].tokenId, saleNFTList[index].quantity, "");
        }
    ...
    function buyNFT(uint256 index) external nonReentrant {
        ...
        if (!saleNFTList[index].isERC1155) {
            ...
            if (commissionAmount > 0) {
                dcauContract.transferFrom(msg.sender, REWARDER_CONTRACT, commissionAmount);
                rewardReceiverContract.receiveTokens(DCAU_TOKEN_CONTRACT, commissionAmount);
            }

            dcauContract.transferFrom( msg.sender, onSaleNft.seller, sellerAmount); 
        } else {
            ...
            if (commissionAmount > 0) {
                dcarContract.transferFrom(msg.sender, REWARDER_CONTRACT, commissionAmount);
                rewardReceiverContract.receiveTokens(DCAR_TOKEN_CONTRACT, commissionAmount);
            }

            dcarContract.transferFrom( msg.sender, onSaleNft.seller, sellerAmount); 
        }
