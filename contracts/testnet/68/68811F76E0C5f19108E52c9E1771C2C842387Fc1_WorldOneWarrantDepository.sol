...
contract WorldOneWarrantDepository is Ownable {
    ...
    function redeem(address _recipient, uint256 _warrantId) external returns (uint256) {
        ...
        for(uint256 i = 0; i < depositorWarrantId[_recipient].length; i++){
            if(depositorWarrantId[_recipient][i] == _warrantId){
                delete depositorWarrantId[_recipient][i]; //delete user warrant id
            }
        }
        emit WarrantRedeemed(_recipient, info.payout, 0, _warrantId); // emit warrant data
