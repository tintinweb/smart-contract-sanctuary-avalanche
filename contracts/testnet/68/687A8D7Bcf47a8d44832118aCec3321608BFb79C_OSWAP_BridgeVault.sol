...
contract OSWAP_BridgeVault is ERC20, ReentrancyGuard {
    ...
    event Swap(uint256 indexed orderId, address indexed sender, uint256[] signers, address owner, uint256 amendment, Order order, uint256 outAmount, int256 newImbalance, uint256 newLpAssetBalance);
    ...
    function swap(
        ...
        emit Swap(_orderId, msg.sender, signers, _owner, amendment, order, amount, imbalance, lpAssetBalance);
    }
    ...
    function rebalancerWithdraw(bytes[] calldata signatures, uint256 assetAmount, uint256 _nonce) external nonReentrant {
        ...
        uint256 price = assetPriceAgainstGovToken(govTokenOracle, assetTokenOracle);
        (uint256[] memory signers, /*uint256 totalStake*/) = _verifyStakedValue(signatures, hashWithdrawParams(rebalancer, assetAmount, _nonce), assetAmount * 1e18 / price);
        imbalance -= toInt256(assetAmount);
