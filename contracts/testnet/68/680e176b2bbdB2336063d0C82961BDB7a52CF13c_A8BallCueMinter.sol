...
ReentrancyGuardUpgradeable, StandardAccessControl  {
    ...
    function _randDesignId(uint256 collectionId, uint256[] memory designIds) internal view returns (uint256) {
        ...
        // get total remaining to calculate full randomness
        uint256 total = 0;
        for (uint256 i = 0; i < designIds.length; i++) {
            total += _collections[collectionId].designs[designIds[i]].remaining;
        }

        uint256 result = rand % total;
        total = 0;
        for (uint256 i = 0; i < designIds.length; i++) {
            uint256 max = total + _collections[collectionId].designs[designIds[i]].remaining;
            if (result <= (max - 1)) {
