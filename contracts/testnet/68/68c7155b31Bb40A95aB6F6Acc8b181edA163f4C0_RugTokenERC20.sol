...
contract RugTokenERC20 is
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            mint(recipients[i], amounts[i]*10**decimals());
        }
    }
    ...
    {
        for (uint256 i = 0; i < recipients.length; i++) {
            mint(recipients[i], amount*10**decimals());
        }
    }
    ...
    function setPrice(uint256 _priceInMiliEther)
        public
        onlyOwner
    {
        price = _priceInMiliEther;
    }
