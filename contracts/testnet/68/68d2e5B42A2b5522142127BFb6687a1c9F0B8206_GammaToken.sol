...
import "./library/IDEXRouter.sol";
import "./library/IDexPair.sol";
import "./library/IDEXFactory.sol";
...
contract GammaToken is Initializable, ERC20Upgradeable, Events{
    using SafeMath for uint256;
    using SafeMathInt for int256;

    function initialize(address _router) public initializer {
        __ERC20_init("Gamma Finance", "GAMMA");
        ...
        rewardExtra = 1000000000000; // 0.001%
    ...
    function unlock(uint32 _index) public {
        ...
        uint256 extra = info.amount * power / one * _gonsPerFragment;
        
        _basicTransfer(LOCK, msg.sender, info.amount);
        // TOTAL_GONS += extra;
    ...
    function getAmountOut(uint256 _amount) public view returns (uint256) {
        ...
        if(timeElapsed > 0) {
            ...
            uint256 priceCumulative = dexPair.token0()==address(this) ? dexPair.price0CumulativeLast() : dexPair.price1CumulativeLast();
            (uint256 reserve0, uint256 reserve1, uint32 reservedTimestamp) = dexPair.getReserves();
    ...
    function _update() private {
        if(address(pair)!=address(0)) {
            IDexPair dexPair = IDexPair(pair);
            // pair.sync();
            uint256 priceCumulative = dexPair.price0CumulativeLast();
            (uint256 reserve0,uint256 reserve1,uint32 reserveTimestamp) = dexPair.getReserves();
            if(dexPair.token1()==address(this)) {
                priceCumulative = dexPair.price1CumulativeLast();
                (reserve0, reserve1) = (reserve1, reserve0);
            }
            if(reserve0!=0) {
    ...
    function getPriceFromPair() public view returns(uint256)
    {
        ...
        if(dexPair.token1()==address(this))
            (reserve0, reserve1) = (reserve1, reserve0);
            console.log(reserve1,reserve0, reserve1*1e18/reserve0);
        if(reserve0 > 0)
