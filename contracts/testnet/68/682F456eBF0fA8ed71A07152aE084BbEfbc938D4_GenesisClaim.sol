 ...
 * 				the genesis and investor contracts by Zeus of OlympusDAO
 */
...
contract GenesisClaim is Ownable, Pausable, Multicall {
	...
	struct Term {
		...
		uint256 spent; // USDC terms (6 decimals)
		uint256 purchased; // Purchased NFT's
		uint256 reserved; // NFT's Reserved for purchase
	}
	...
	string internal presaleNFTUri = 'ipfs://QmaywyNvqooKciut3jByMw1WKd9bY6r2CQiCZCTkRtGHKF';
	...
	// IClaim internal immutable previous = IClaim(0x3F019b6DA55c77099aD7C6f7DA725bA4B5BBA916); // 43114
	IClaim internal immutable previous = IClaim(0xaf01fbfE11A4ff321D814f323AB639bE88c3e015); // 43113
	...
	function _purchase(uint256 _nftsToPurchase) internal {
		...
		uint256 usdc_total;
		uint256 purchasable = purchasableFor(msg.sender);
		uint256 decimals = usdc.decimals();

		if (purchasable == 0 || whitelistWindow == false) {
			// If public user or if whitelist window closed.
			...
			if (info.credit > 0){
				// Reservations, plus credit
				if (requested_total > info.credit) {
					// All nfts after credited nfts revert to public price
					require((requested_total.sub(info.credit)) == 0, "Claim NFTs credited before making a purchase!");

					// Zero out credit 
					terms[msg.sender].credit = 0;
					usdc_total = requested_total.sub(info.credit);
				} else if (requested_total <= info.credit) {
					usdc_total = 0;
					terms[msg.sender].credit = info.credit.sub(requested_total); 
				}
			} else {
				// Has terms with reserved nfts, no credit
				usdc_total = _nftsToPurchase.mul(info.price);
			}
		...
		// Process usdc transfer and nft mint
		...
		uint256 allocationModifier = 0;
		if(info.price == publicMint){
			allocationModifier = usdc_total.div(publicMint);
		} else {
			allocationModifier = usdc_total.div(info.price);
		}

		// Update contract state
		remainingMint = remainingMint.sub(allocationModifier);
	...
	*/
	function endWhitelist() external onlyOwner {
		...
		remainingMint = SafeMath.sub(totalReserved, totalPurchased);
	}
	...
	*/
	...
	function setTerms(
		...
		if (_reserved <= _purchased) { 
			subtractamount = _purchased;
		} else {
			subtractamount = _reserved;
		}

		remainingMint = remainingMint.sub(subtractamount);
...
interface IERC20plus is IERC20, IERC20Metadata {}
