...
contract Main is Ownable, CloneFactory {
    ...
    event FindKey(bytes32 indexed Key);
    ...
    function creatCollection(string memory collectionMetadata, uint256 size) external payable isBlackList {
        if (msg.sender != owner()) require(msg.value == IRedeemAndFee(redeemAndFee).flatFee(), "Main: insufficient flat fee");
    ...
    function putOnSale(
        ...
        if(user != msg.sender)
            require(IRedeemAndFee(redeemAndFee).AbleToViewALLPrivateMetadata(msg.sender), "MarketFactory:not angel");
        if(user != _detectOwner(collectionId))
            require(msg.value == IRedeemAndFee(redeemAndFee).flatFee(), "MarketFactory: insufficient flat fee");
        ...
        if(_type == FNFT_TYPE.Increament)
            require(endPrice <= 20 && endPrice >= 10, "Main:invalid rate");
        ...
        if(setRoyaltyFee) {
            if (_detect(collectionId) == ContractType.ERC721) 
                require(_detectOwner(collectionId) == user, "Main:ERC1155-no permission for royaltyFee");
            else if(_detect(collectionId) == ContractType.ERC1155)
                require(IERC1155(collectionId).balanceOf(user, tokenId) >= amount, "Main:1155-not owner");
            ...
            if (_detect(collectionId) == ContractType.ERC721) {
                ...
                _putonSaleFor1155(_key, collectionId, tokenId);
            }
            if(msg.sender != user) {
                listInfo[_key].royaltyShare = 50;
                listInfo[_key].admin = msg.sender;
            }
        }
        if(msg.value > 0)
    ...
    function cancelList (bytes32 _key) external isBlackList {
        ...
        if(listInfo[_key].isFNFT == true) {
            require(buyerInfo[_key][9].buyer == address(0), "Main:already finished sale");
    ...
    function refundFromFNFT(bytes32 _key) external isBlackList {
        require(listInfo[_key].maker == address(0), "Main:can't refund");
        uint price;
        uint amount;
        for(uint i = 0; i < 10; i++) {
        ...
        require(price > 0, "Main:invalid user");
    ...
    function withDrawFromFNFT(bytes32 _key) external isBlackList {
        require(listInfo[_key].maker == msg.sender, "Main:not owner");
        require(buyerInfo[_key][9].buyer != address(0), "Main:not finished sale");
        uint price;
        for (uint i = 0; i < 10; i++) {
            price += buyerInfo[_key][i].price;
        }
    ...
    function buyNow(bytes32 _key, uint _amount) external isBlackList {
        ...
        require(listInfo[_key].maker != msg.sender && listInfo[_key].maker != address(0), "Main:invalid maker");
