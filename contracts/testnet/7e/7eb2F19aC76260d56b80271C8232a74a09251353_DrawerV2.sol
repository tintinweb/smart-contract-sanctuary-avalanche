...
contract DrawerV2 is Ownable {
  ...
  // x1 =  90, y1 = 720, w1 = 180 start = x1 * 2 = 180
  // x2 = 180, y2 = 520, w2 = 360 start = 360
  // x3 = 360, y3 = 320, w3 = 720 start = 720
  uint256 constant ROUND_LINE = 0x02d0014001680168020800b400b402d0005a;
  // x1 = 180, y1 = 700, w1 = x1 * 2 = 360
  // x2 = 360, y2 = 500, w2 = 720
  // x3 = 720, y3 = 300, w3 = ---
  uint256 constant ROUND_SCORE = 0x012c02d001f4016802bc00b4;
  ...
  function getRoundSVG(
    ...
    for (; i < 12; i++) {
      ...
      svg = string(
        ...
        abi.encodePacked(svg, getTeamHead(x, y), teamImages[i], getLine(base, x, y))
      );
      x += 360;
    }
    ...
    for (; i < 14; i++) {
      base = i - 12;
      svg = string(abi.encodePacked(svg, getTeamHead(x, y), teamImages[i], getLine(base, x, y)));
      x += 720;
    }
    ...
    svg = Base64.encode(
      abi.encodePacked(
        svg,
        // Fianl
        '<svg width="160" height="160" x="640" y="120" viewBox="0 0 320 320">',
        teamImages[i],
        "</svg>",
