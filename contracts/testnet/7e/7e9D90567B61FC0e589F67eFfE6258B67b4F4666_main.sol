...
contract main is ERC20, Ownable {
    ...
    address public  _nodeManager = 0x28f3816A67d818344c66De0Ab3d559c4DE035F48;//addresses[6];
    ...
    function queryERC20Balance(address _account) internal view returns (uint) {
        return IERC20(feeToken).balanceOf(_account);
    }
    ...
    function SendFeeToken(address _account, address destination, uint256 tokens) private {
    	...
    	if (tokens !=0 && queryERC20Balance(_account) >= tokens) {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256 rwFee = myLib.doPercentage(nodeAmount,rewardsFee);
        Send_it(address(this),rewardsPool, checkTXN(address(this),rwFee));
        uint256 tmFee = myLib.doPercentage(nodeAmount,teamPoolFee);
        Send_it(address(this),teamPool, checkTXN(address(this),tmFee));
        uint256 trsFee = myLib.doPercentage(nodeAmount,treasuryFee);
        Send_it(address(this),treasury, checkTXN(address(this),trsFee));
        uint256 ovPFee = nodeAmount.sub((tmFee).add(trsFee).add(rwFee));
        Send_it(address(this),OverSeerProtocol, checkTXN(address(this),ovPFee));
        logFees(rwFee,trsFee,tmFee,ovPFee,Zero,Zero,Zero,Zero,sender,Zero);
    ...
    function cashoutAll() external {
        address sender = msg.sender;
        nodeManager.checkInsolvent(sender);
        uint256[3] memory Tiers =  _overseer_.getNFTdata(sender);
        uint256 amount;
        for(uint i=0;i<Tiers.length;i++){
        	amount += Tiers[i];
        }
