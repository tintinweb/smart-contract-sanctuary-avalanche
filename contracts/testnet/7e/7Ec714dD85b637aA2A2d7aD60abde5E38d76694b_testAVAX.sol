...
contract testAVAX is IStargateReceiver {
    using SafeMath for uint;
    address public stargateRouter = 0x13093E05Eb890dfA6DacecBdE51d24DabAb2Faa1; // an IStargateRouter instance
    address public ammRouter = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; // an IUniswapV2Router02 instance
    ...
    function swapNativeForNative(
        ...
        // using the amm router, swap native into the Stargate pool token, sending the output token to this contract
        {
            ...
            uint[] memory amounts = IUniswapV2Router02(ammRouter)
    ...
    function swapUSDCforNative(
        ...
        require(amount > 0, 'error: swap() requires qty > 0');

        // encode payload data to send to destination contract, which it will handle with sgReceive()
        bytes memory data = abi.encode(deadline, amountOut, to);

        // this contract calls stargate swap()
        IERC20(bridgeToken).transferFrom(msg.sender, address(this), amount);
        IERC20(bridgeToken).approve(address(stargateRouter), amount);
