...
contract Lottery is ILottery, Ownable, Pausable, ReentrancyGuard, VRFConsumerBaseV2, PullPayment {
    ...
    function _fulfillRandomWords() external {
    ...
    function _isReadyForDraw() private view returns (bool) {
        ...
        bool hasEnoughPlayers = (this.getTotalPlayers() >= minPlayers);
        bool maxPlayersReached = (this.getTotalPlayers() == maxPlayers);
    ...
    function _isReadyToAcceptPlayer(address player_) private view returns (bool) {
        ...
        bool isNotMaxPlayers = (this.getTotalPlayers() + 1 <= maxPlayers);
    ...
    function _isReadyForClose() private view returns (bool) {
        ...
        bool allPlayersPaid = payoutsExecuted == this.getTotalPlayers();
    ...
    function _draw() private {
        ...
        leftOverPayoutPerPlayer = (jackpot - winnerPayout) / (this.getTotalPlayers() - 1);
    ...
    function _computePayouts(uint256 startIndex) private view returns (address[] memory, uint256[] memory) {
        ...
        uint256 payoutsLeft = this.getTotalPlayers() - startIndex;
    ...
    function _executePayouts(address[] memory _addresses, uint256[] memory _payouts) private {
        ...
        if (payoutsExecuted == this.getTotalPlayers()) {
            _close();
        }
    }
