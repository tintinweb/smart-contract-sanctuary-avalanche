...
contract GlpAdapter is Initializable, Governable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function zapToGlp(address _token, uint256 _amount, bool _compound, bool _rebalance)
        external
        nonReentrant
        validToken(_token)
        returns (uint256)
    ...
    function zapToGlp2(address _token, address _receiver, uint256 _amount, bool _compound, bool _rebalance)
        external
        nonReentrant
        validToken(_token)
        returns (uint256)
    {
        ...
        glp.approve(address(vaultRouter), mintedGlp);
        uint256 receipts = vaultRouter.depositGlp(mintedGlp, _receiver, _compound, _rebalance);
