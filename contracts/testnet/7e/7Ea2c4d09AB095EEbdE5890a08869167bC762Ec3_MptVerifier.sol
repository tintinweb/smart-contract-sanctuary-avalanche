...
contract MptVerifier {
    ...
    event GasLog(uint256 decodeProofBlobGas, uint256 validateMPTProofGas, uint256 decodeReceiptGas);
    ...
    function validateMPT(bytes memory proofBlob) external returns (Receipt memory receipt){
        // decode rlpProof
        uint256 g1 = gasleft();
        Proof memory proof = decodeProofBlob(proofBlob);
        uint256 g2 = gasleft();
        uint256 decodeProofBlobGas = g1 - g2;

        // validate mptProof
        bytes memory rlpTx = validateMPTProof(proof.receiptsHash, proof.mptKey, proof.stack);
        require(rlpTx.length != 0, "invalid proof");
        uint256 g3 = gasleft();
        uint256 validateMPTProofGas = g2 - g3;

        // convert receipt result
        receipt = decodeReceipt(proof.receiptsHash, rlpTx);
        uint256 g4 = gasleft();
        uint256 decodeReceiptGas = g3 - g4;

        emit GasLog(decodeProofBlobGas, validateMPTProofGas, decodeReceiptGas);
