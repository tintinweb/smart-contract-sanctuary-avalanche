...
contract BridgesOfFate is Ownable {
    ...
    function _pushBackBankingGameEndRules() internal view returns (bool) {
       if((PioneerPushBackGameMap[GameStatusInitialized[latestGameId].stageNumber] > 0) && 
        (PioneerPushBackGameMap[GameStatusInitialized[latestGameId].stageNumber]  < _pioneerPushBackNumberProbility) && 
    ...
    function switchSide(bytes32 playerId) external  GameEndRules  PushBackBankingGameEndRules{
        ...
        require(PioneerPushBackGameMap[GameStatusInitialized[latestGameId].stageNumber] < _pioneerPushBackNumberProbility, "Reached maximum");
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules PushBackBankingGameEndRules{
        ...
        if(_currentUserStage == _gameStatus.stageNumber) {
            if(_checkSide(_gameStatus.stageNumber,_jumpSide) && (PioneerPushBackGameMap[_gameStatus.stageNumber] > _pioneerPushBackNumberProbility )){
    ...
    function pushBack(bytes32 playerfrom,bytes32 playerto) external GameEndRules PushBackBankingGameEndRules  {
        ...
        _memberto.lastJumpSide = GameMap[_memberto.stage] >= 50;

        //  if (GameMap[GameStatusInitialized[latestGameId].stageNumber - 1] >= 50) {
