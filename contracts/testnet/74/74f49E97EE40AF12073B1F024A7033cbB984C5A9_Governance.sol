...
contract Governance is Dcarb {
    ...
    // executed only once
    function initialize(address dcarbAddr, address escrowAddr) external {
    ...
    function propose(
        ...
        newProposal.parameters.activeDuration = block.timestamp - 1;
        newProposal.parameters.queueDuration = block.timestamp - 1;
        ...
        newProposal.escrowId = escrow.openCarbonEscrow{value: msg.value}(
            _projectID,
            _vintage,
            _tCO2,
            _endPeriod,
        ...
        emit ProposalCreated(
            ...
            description,
            title,
            newProposal.createTime,
            newProposal.escrowId
        );
    ...
    function execute(uint256 proposalId, bytes32 _escrowID) external payable {
    ...
    function cancelProposal(uint256 proposalId, bytes32 _escrowID) external {
        require(
    ...
    function getProposalList()
        external
        view
        returns (ProposalToReturn[] memory)
    {
        ProposalToReturn[] memory ptr = new ProposalToReturn[](lastProposalId);
        for (uint256 i = 1; i <= lastProposalId; i++) {
            Proposal storage proposal = proposals[i];
            ptr[i].id = proposal.id;
            ptr[i].proposer = proposal.proposer;
            ptr[i].description = proposal.description;
            ptr[i].title = proposal.title;
            ptr[i].targets = proposal.targets;
            ptr[i].values = proposal.values;
            ptr[i].signatures = proposal.signatures;
            ptr[i].calldatas = proposal.calldatas;
            ptr[i].createTime = proposal.createTime;
            ptr[i].eta = proposal.eta;
            ptr[i].forVotes = proposal.forVotes;
            ptr[i].againstVotes = proposal.againstVotes;
            ptr[i].escrowId = proposal.escrowId;
            ptr[i].canceled = proposal.canceled;
            ptr[i].executed = proposal.executed;
            ptr[i].parameters = proposal.parameters;
        }
        return ptr;
    }
    ...
    function getUserVotesAndProposal(address user)
        external
        view
        returns (UserStats memory)
    {
        ...
        for (uint256 i = 0; i < lastProposalId; i++) {
            ...
            if (proposal.proposer == user) {
                proposalCount += 1;
                votes += proposal.receipts[user].votes;
                console.log(
                    "lastProposalId:",
                    lastProposalId,
                    proposal.receipts[user].votes
                );
            }
        }

        userStats.votes = votes;
        userStats.proposals = proposalCount;
    ...
    function abrogationProposal_cancelVote(uint256 proposalId) external {
        require(
            0 < proposalId && proposalId <= lastProposalId,
            "invalid proposal id"
        );
...
interface IEscrow {
    ...
    function totalRewards() external returns (uint256 _totalRewars);
}
...
abstract contract Parameters {
    uint256 public warmUpDuration = 0 days;
    uint256 public activeDuration = 0 days;
    uint256 public queueDuration = 0 days;
    uint256 public gracePeriodDuration = 0 days;

    uint256 public acceptanceThreshold = 99;
    uint256 public minQuorum = 40;

    uint256 constant ACTIVATION_THRESHOLD = 0;
    uint256 constant PROPOSAL_MAX_ACTIONS = 0;
    ...
    function setWarmUpDuration(uint256 period) external onlyDAO {
        warmUpDuration = period;
    }

    function setActiveDuration(uint256 period) external onlyDAO {
        require(period >= 4 hours, "period must be > 0");
        activeDuration = period;
    }

    function setQueueDuration(uint256 period) external onlyDAO {
        queueDuration = period;
    }

    function setGracePeriodDuration(uint256 period) external onlyDAO {
        require(period >= 4 hours, "period must be > 0");
        gracePeriodDuration = period;
    }

    function setAcceptanceThreshold(uint256 threshold) external onlyDAO {
    ...
    function setMinQuorum(uint256 quorum) external onlyDAO {
