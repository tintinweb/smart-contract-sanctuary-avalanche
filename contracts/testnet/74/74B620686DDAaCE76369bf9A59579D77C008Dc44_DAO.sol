...
contract DAO {
    ...
    event Voted(address voter, address pollingOwner, uint256 yesCount, uint256 noCount);
    event VoteCanceled(
    ...
    function Vote(uint256 pollingId, bool answer) public {
        ...
        emit Voted(msg.sender, polling.owner, polling.yesCount, polling.noCount);
    }
    ...
    function CancelVote(uint256 pollingId) public {
        ...
        s_voteRegistry[msg.sender][pollingId] = true;
        emit VoteCanceled(msg.sender, polling.owner, polling.yesCount, polling.noCount);
    }
                    ...
                    if (polling.pollingType == PollingType.ADD_MEMBER) {
                        ...
                        for (uint256 j = 0; j < polling.addressData.length; j += 1) {
                            (bool sent, ) = polling.addressData[j].call{
                                value: uint256(polling.fundData[j])
                            }("");
                        }
                    }
                    polling.pollingState = PollingState.Accepted;
                } else {
