...
contract TestReserves_1 is Ownable{
  using SafeMath for uint;
  using SafeMath for uint8;
  mapping(address => bool) public isBlacklisted;
  uint8 num = 30;
  uint perc = num.div(100);
  uint val = 21;
  uint boo = 0;
  function updatePerc(uint8 newVal) external onlyOwner {
        perc = newVal.div(100); //min amount to swap
    }
  ...
  function getTokenPrice() public view returns(uint[7] memory)
   {
    ...
    uint low_val = val.sub(val.mul(perc));
    uint high_val = val.add(val.mul(perc));
    if (now_val > high_val) {
      uint boo = 1;
    }
    if (now_val < low_val) {
    ...
    return [res0,res1,boo,now_val,low_val,high_val,val]; // return amount of token0 needed to buy token1
   }
