...
contract NFKeyStakingUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    address private _randomizerContractAddress;
    uint private _baseReward;
    uint private _chestLevelConst;
    uint constant CHEST_TIMER = 60;
    ...
    mapping(uint => Reward) private stakedRewards;
    ...
    function initialize(address newNfkeyContractAddress, address newTresrCoinContractAddress, address newBonusRewardAddressContract, address newTresrStakingContractAddress, address newRandomizerContractAddress) public initializer {
        __Ownable_init();
        ...
        _randomizerContractAddress = newRandomizerContractAddress;

        _baseReward = 125;
        _chestLevelConst = 250;
    }

    modifier onlyNfkeyContract() {
        require(_nfkeyContractAddress == msg.sender, "NFkeyStaking: caller is not NFkeyContract");
        _;
    }
    ...
    function updateTreasureRewardTime(uint tokenID) private {
        ...
        treasureRewardAvailableList[tokenID] = block.timestamp + (timeCountDown * CHEST_TIMER);
    }

    function getTimeToTreasureUnlock(uint tokenID, uint time) public view returns (uint) {
        uint chestTier = chestTierList[tokenID];
        uint timeToTreasureUnlock = treasureRewardAvailableList[tokenID];

        if (chestTier > 1 && time > timeToTreasureUnlock) {
            uint timeDiff = time - timeToTreasureUnlock;

            chestTier = chestTier - 1;

            for (uint i = chestTier; i > 0; i--) {
                uint timeCountDownTmp = calcTimeCountDown(i) * CHEST_TIMER;

                timeToTreasureUnlock = timeToTreasureUnlock + timeCountDownTmp;

                if (timeDiff <= timeCountDownTmp) break;

                timeDiff = timeDiff - timeCountDownTmp;
                chestTier = chestTier - 1;
            }
        }

        return timeToTreasureUnlock;
    }
    ...
    function getChestTier(uint tokenID, uint time) public view returns (uint) {
        uint chestTier = chestTierList[tokenID];
        uint timeToTreasureUnlock = treasureRewardAvailableList[tokenID];
    ...
    function getClaimed(uint tokenID) public view returns (uint) {
        return stakedRewards[tokenID].claimed;
    }

    function proxyAddLog(uint tokenID) public onlyNfkeyContract {
        addLog(tokenID);
    }
    ...
    function addLog(uint tokenID) private {
        ...
        if (isStakedList[tokenID] == true) {
            stakedRewards[tokenID].rewards = calcRewards(tokenID, block.timestamp);
            stakedRewards[tokenID].lastDate = block.timestamp;
            stakedRewards[tokenID].lastTier = chestTier;
        }
    }

    function calcReward(uint level, uint chestTier) public view returns (uint) {
        return level * chestTier * _baseReward * 10 ** 18 / 1000 / 24 / 60 / 60;
    }
    ...
    function calcRewards(uint tokenID, uint time) public view returns (uint) {
        ...
        if (isStakedList[tokenID] == false) return 0;

        uint rewardSum = stakedRewards[tokenID].rewards;

        if (stakedRewards[tokenID].lastDate != 0) {
            if (chestTier >= stakedRewards[tokenID].lastTier) {
                rewardSum += (time - stakedRewards[tokenID].lastDate) * calcReward(level, chestTier);
            } else {
                uint timeDiff = time - stakedRewards[tokenID].lastDate;

                if (timeDiff > 0) {
                    for (uint i = stakedRewards[tokenID].lastTier; i > chestTier; i--) {
                        uint timeCountDownTmp = calcTimeCountDown(i) * CHEST_TIMER;

                        if (timeDiff <= timeCountDownTmp) break;

                        timeDiff = timeDiff - timeCountDownTmp;
                        rewardSum += timeCountDownTmp * calcReward(level, i);
                    }

                    rewardSum += timeDiff * calcReward(level, chestTier);
                }
            }
        }
    ...
    function calcRewardsPerSecond(uint tokenID, uint time) public view returns (uint) {
        ...
        if (isStakedList[tokenID] == false) return 0;
        return calcReward(level, chestTier);
    }
    ...
    function calcUnlockCost(uint tokenID, uint time) public view returns (uint) {
        ...
        if (chestTier >= 7) chestTier = 6;

        uint chestTierDays = 8 - (chestTier + 1);

        if (chestTier == 0) return 0;
        return chestTierDays * level * (chestTier + 1) * 125 * _chestLevelConst * 10 ** 12;
    }
    ...
    function bulkStake(uint[] memory tokenIDList) external {
    ...
    function unstake(uint256 tokenID) public {
        ...
        setChestTier(tokenID, 0);
        stakedRewards[tokenID].lastDate = 0;
        stakedRewards[tokenID].rewards = 0;
        stakedRewards[tokenID].lastTier = 0;

        emit Unstaked(msg.sender, tokenID);
    }

    function openChest(uint tokenID) external payable {
        TresrCoinUpgradeableV1 tresrCoinContract = TresrCoinUpgradeableV1(_tresrCoinContractAddress);
        NFKeyUpgradeableV1 nfkeyContract = NFKeyUpgradeableV1(_nfkeyContractAddress);
        Randomizer randomizerContract = Randomizer(_randomizerContractAddress);

        require(nfkeyContract.ownerOf(tokenID) == msg.sender, "NFkeyStaking: caller is not owner");
        require(msg.value >= 10 ** 16, "NFkeyStaking: invalid commission");

        payable(address(owner())).transfer(msg.value);
        randomizerContract.requestRandomWords();

        uint chestTier = getChestTier(tokenID, block.timestamp);
        uint unlockCost = calcUnlockCost(tokenID, block.timestamp);

        if (unlockCost > 0) tresrCoinContract.burn(msg.sender, unlockCost);

        uint randomCount = randomizerContract.getRandomNumber();
        uint probToOpen = calcBaseProbToOpen(tokenID, block.timestamp, msg.sender);
        uint timeToTreasureUnlock = getTimeToTreasureUnlock(tokenID, block.timestamp);
        bool answer = false;

        if ((randomCount * 100) < probToOpen) {
            if (chestTier == 0 || block.timestamp < timeToTreasureUnlock) {
                if (chestTier < 7) setChestTier(tokenID, chestTier + 1);
            }
        ...
        if (answer) updateTreasureRewardTime(tokenID);
    ...
    function setRewardsReleased(uint amount, uint tokenID) public onlyBonusRewardContract {
        ...
        if (stakedRewards[tokenID].rewards < amount) return;

        stakedRewards[tokenID].rewards -= amount;
        stakedRewards[tokenID].claimed += amount;
    }
    ...
    function calcBonusRewards(uint tokenID, uint time) public view returns (uint) {
        NFKeyUpgradeableV1 nfkeyContract = NFKeyUpgradeableV1(_nfkeyContractAddress);
        ...
        uint rewardSum = stakedRewards[tokenID].rewards + stakedRewards[tokenID].claimed;
    ...
    function getBonusReward(uint[] memory tokenIDList, uint time) public view returns (uint) {
        ...
        for (uint i = 0; i <= length - 1; i++) {
            rewardAll += calcBonusRewards(tokenIDList[i], time);
        }
...
contract Randomizer is VRFConsumerBaseV2 {
    ...
    // _vrfCoordinator = 0x2eD832Ba664535e5886b75D64C46EB9a228C2610
    // _subscriptionId = 478
...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    uint public burnedSmarter;

    mapping(uint => mapping(uint => string)) private ipfsURIList;
    mapping(uint => uint) private availableUpgradeTimestampList;
    mapping(uint => uint) private startUpgradeTimestampList;
    mapping(uint => uint) private levelList;
    mapping(uint => uint) private zoneList;

    event Minted(address owner, address nftAddress, uint tokenID, uint amount);
    ...
    function getUpgradeDelay(uint tokenID) public view returns (uint) {
        return availableUpgradeTimestampList[tokenID];
    }

    function getStartUpgradeDelay(uint tokenID) public view returns (uint) {
        return startUpgradeTimestampList[tokenID];
    }

    function setUpgradeDelay(uint tokenID, uint level) internal {
        if (level <= 3) {
            // availableUpgradeTimestampList[account] = block.timestamp + (level * 60 * 60);
            availableUpgradeTimestampList[tokenID] = block.timestamp + (level * 60);
        } else {
            // availableUpgradeTimestampList[tokenID] = block.timestamp + ((480 + (level * 60)));
            availableUpgradeTimestampList[tokenID] = block.timestamp + ((480 + (level)));
        }

        startUpgradeTimestampList[tokenID] = block.timestamp;
    }
    ...
    function bulkUpgradeKey(uint[] memory tokenIDList) public {
        for (uint i = 0; i < tokenIDList.length; i++) {
            upgradeKey(tokenIDList[i]);
        }
    }
    ...
    function upgradeKey(uint tokenID) public {
        ...
        require(block.timestamp >= availableUpgradeTimestampList[tokenID], "NFKey: upgrade not available yet");

        uint level = getLevel(tokenID);
        uint zone = getZone(tokenID);
        uint smarterAmount = getAmountUpgradeKey(level);

        //smarterCoinContract.transferFrom(address(msg.sender), address(0), smarterAmount);
        smarterCoinContract.burn(address(msg.sender), smarterAmount);
        burnedSmarter += smarterAmount;
        ...
        setUpgradeDelay(tokenID, level + 1);
    ...
    function bulkMint(address account, uint zone, uint amount) public payable {
        ...
        uint level = WhitelistUpgradeableV1(_whitelistContractAddress).getAddressToWhitelist(msg.sender);
        if (level == 0 && zone == 5) level = 1;
        require(level >= 1, "NFKey: You need to be whitelisted");

        bool isZone = WhitelistUpgradeableV1(_whitelistContractAddress).getWhitelistZone(msg.sender, zone);
        require(isZone || zone == 5, "NFKey: Invalid zone");

        for (uint i = 1; i <= amount; i++) {
            mint(msg.sender, zone, level);
        }

        if (zone == 1 || zone == 2) WhitelistUpgradeableV1(_whitelistContractAddress).removeWhitelistZone(msg.sender, zone);
    }

    function mint(address account, uint zone, uint level) internal {
        _tokenIds.increment();
        uint tokenID = _tokenIds.current();

        _mint(account, tokenID);
        _setTokenURI(tokenID, getIPFS(level, zone));

        setLevel(tokenID, level);
        setZone(tokenID, zone);
        setUpgradeDelay(tokenID, level);

        uint commission = getZoneCommission(account, zone);
        emit Minted(account, address(this), tokenID, commission);
    }
}
...
contract TresrCoinUpgradeableV1 is ERC20Upgradeable, OwnableUpgradeable {
    uint public burned;
    uint public poolAllocation;
    uint public bonusPoolAllocation;
    ...
    function initialize() initializer public {
        ...
        //225,522,750
        poolAllocation = 225522750 * 10 ** 18;

        //79,029,112
        bonusPoolAllocation = 79029112 * 10 ** 18;
    ...
    // typeAllocation == 1 -> pool allocation | typeAllocation == 2 -> bonus pool allocation
    function mint(address account, uint amount, uint typeAllocation) public onlyAdmin {
        if (typeAllocation == 1) {
            require(poolAllocation > 0, "TresrCoin: zero pool allocation");

            uint allocation = amount;
            if (amount > poolAllocation) allocation = poolAllocation;

            poolAllocation -= allocation;
            _mint(account, allocation);
        } else if (typeAllocation == 2) {
            require(bonusPoolAllocation > 0, "TresrCoin: zero bonus pool allocation");

            uint allocation = amount;
            if (amount > bonusPoolAllocation) allocation = bonusPoolAllocation;

            bonusPoolAllocation -= allocation;
            _mint(account, allocation);
        }
    }
...
contract SmarterCoinUpgradeableV1 is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function faucet(address _to) public {
        _mint(address(_to), 1000000 * 10 ** 18);
    }
