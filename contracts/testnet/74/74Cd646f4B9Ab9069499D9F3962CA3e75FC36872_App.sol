...
contract App {
    ...
    function register(uint256 _player) public view returns (uint256) {
        return _player.addBricks(20).setAirdropDay(block.timestamp / 86400 + 1);
    }

    function update(uint256 _player) public view returns (uint256) {
    ...
    function allowTransfer(uint256 _command) public {
    ...
    function sendFee(uint256 _bricks) public {
        if (msg.sender != owner) {
