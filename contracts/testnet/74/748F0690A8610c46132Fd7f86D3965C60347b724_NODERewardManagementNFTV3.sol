...
contract NODERewardManagementNFTV3 is
    IMasterOfCoinV3,
    ERC721
{
    ...
    struct NodeEntity {
        address owner;
        string name;
        uint256 nodePos;    // position in nodesOfUser array
        uint256 tokenId;
    ...
    function _createNodeForAddress(
        ...
        // _nodesOfUser[_owner].push(totalNodesCreated);
        _tokenIds.increment();
        // _userTokenIdToNodeIndex[_owner].push(_tokenIds.current());
        _tokensOfUser[_owner].push(_tokenIds.current());
        _userTokenIdToNodeIndex[_owner][_tokenIds.current()] = nodeListIdx + 1;

        _nodeList.push(
            NodeEntity({
                owner: _owner,
                name: name_,
                nodePos: _tokensOfUser[_owner].length - 1,
        ...
        _tokenIdToNodeListIndex[_tokenIds.current()] = nodeListIdx + 1;
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        require(_tokensOfUser[_owner].length > 0, "no nodes to burn");
        ...
        // index of tokensOfUser array to burn
        uint256 tokensOfUserIndex = _nodeList[nodeListIndex].nodePos;

        // tokenId of last token in tokensOfUser array
        uint256 lastTokenId = _tokensOfUser[_owner][_tokensOfUser[_owner].length - 1];
        uint lastTokenIdNodeIndex = _userTokenIdToNodeIndex[_owner][lastTokenId] - 1;
        // nodePos of last node will change in nodeList after swapping _tokensOfUser
        _nodeList[lastTokenIdNodeIndex].nodePos = tokensOfUserIndex;

        // update tokenId -> nodeListIndex mapping for the node to swap with
        _tokenIdToNodeListIndex[_nodeList[_nodeList.length - 1].tokenId] = _tokenIdToNodeListIndex[tokenId];
        // removing the tokenId -> nodeListIndex mapping of the burnt token / node
        delete _tokenIdToNodeListIndex[tokenId];

        // swapping with last element of _nodeList
        _nodeList[nodeListIndex] = _nodeList[_nodeList.length - 1];
        _nodeList.pop();


        // uint256 tokensOfUserIndex = _userTokenIdToNodeIndex[_owner][tokenId] - 1;
        _tokensOfUser[_owner][tokensOfUserIndex] = _tokensOfUser[_owner][_tokensOfUser[_owner].length - 1];
        _tokensOfUser[_owner].pop();

        delete _userTokenIdToNodeIndex[_owner][tokenId];
    ...
    function _calculateReward(
        ...
        //     node.creationTime
        // );
        if (_compoundTime[node.owner] > lastClaim) {
            lastClaim = _compoundTime[node.owner];
        }
    ...
    function _isNameAvailable(
        ...
        for (uint256 i = 0; i < tokensOfUser.length; i++) {
            ...
            if (
                keccak256(bytes(_nodeList[nodeListIdx].name)) ==
    ...
    function _setOwner(address _owner) private {
        if (nodeOwners[_owner]) {
            return;
        }

        nodeOwners[_owner] = true;
        allNodeOwners.push(_owner);
    }
    ...
    function _nodeIndexToId(
        uint256 index
    ) private view returns (string memory) {
        NodeEntity memory node = _nodeList[index];
        return _getNodeId(node.name, node.creationTime, tierName, node.owner);
    }

    function _cashOutReward(uint256 index) private returns (uint256) {
        uint256 nodeReward = _calculateReward(index);
        // always update last claim time
        _nodeList[index].lastClaimTime = block.timestamp;
        if (_nodeList[index].tempBoosterDaysCounter > 0) {
            _nodeList[index].tempBoosterDaysCounter--;
        }
        return nodeReward;
    }
    ...
    function getRewardAmountOf(
        ...
        return _calculateReward(nodeIndex);
    }
    ...
    function _getRewardAmountOf(
        ...
        for (uint256 i = 0; i < tokensOfUser.length; i++) {
            uint nodeListIdx = _userTokenIdToNodeIndex[account][tokensOfUser[i]] - 1;
            totalReward += _calculateReward(nodeListIdx);
        }
        return totalReward;
    }
    ...
    function cashOutReward(
        address account,
        uint256 index
    ) external onlySentry indexAvailable(index) returns (uint256) {
        NodeEntity storage node = _nodeList[index];
        require(account == node.owner, "Not node owner");
        emit Cashout(account, index);
        return _cashOutReward(index);
    }
    ...
    function cashOutAllReward(
        ...
        for (uint256 i = 0; i < tokensOfUser.length; i++) {
            ...
            totalReward += _cashOutReward(nodeListIdx);
        }
    ...
    function getNodeId(
        uint256 index
    ) external view indexAvailable(index) returns (string memory) {
        return _nodeIndexToId(index);
    }
    ...
    function renameNode(
        ...
        string memory _name,
        uint256 index
    ) external onlySentry indexAvailable(index) {
        require(account == _nodeList[index].owner, "Not node owner");
        require(_isNameAvailable(account, _name), "CN:Name not available");
        _nodeList[index].name = _name;
    }

    function _isNodeOwner(address account) public view returns (bool) {
        return _tokensOfUser[account].length > 0;
    }
    ...
    function isDelinquent(
        ...
        return _isDelinquent(nodeIndex);
    }
    ...
    function getFee(
        uint256 nodeIndex
    ) public view indexAvailable(nodeIndex) returns (uint256) {
    ...
    function _payFee(uint256 nodeIndex) internal indexAvailable(nodeIndex) {
        ...
        emit PaidFee(_nodeIndexToId(nodeIndex), _nodeList[nodeIndex].dueDate);
    }

    function payFee(uint256 nodeIndex) external payable override {
        require(msg.value >= _getFee(), "Amount less than fee");
        payable(collector).transfer(msg.value);
        _payFee(nodeIndex);
    }

    function payFeeBifrost(uint256 nodeIndex) external onlySentry {
        _payFee(nodeIndex);
    }

    function payFees(uint256[] memory nodeIndexes) external payable override {
        require(
            msg.value >= nodeIndexes.length.mul(_getFee()),
            "Amount less than fee"
        );
        payable(collector).transfer(msg.value);
        for (uint256 i = 0; i < nodeIndexes.length; i++) {
            _payFee(nodeIndexes[i]);
        }
    }

    function restoreNode(
        uint256 nodeIndex
    ) external payable override indexAvailable(nodeIndex) {
    ...
    function balanceOf(
        ...
        balance = _tokensOfUser[owner].length;
    }

    function ownerOf(
        uint256 tokenId
    ) public view virtual override returns (address _owner) {
        _owner = _nodeList[_tokenIdToNodeListIndex[tokenId] - 1].owner;
    }
    ...
    function _transfer(
        ...
        uint256 _nodeListIndex = _tokenIdToNodeListIndex[tokenId] - 1;
        uint256 _tokensOfUserIndex = _nodeList[_nodeListIndex].nodePos;

        _tokensOfUser[from][_tokensOfUserIndex] = _tokensOfUser[from][
            _tokensOfUser[from].length - 1
        ];
        _tokensOfUser[from].pop();

        _tokensOfUser[to].push(_nodeListIndex);
        _nodeList[_nodeListIndex].owner = to;
        _nodeList[_nodeListIndex].nodePos = _tokensOfUser[to].length - 1;
        _setOwner(to);
    ...
    function approve(
        ...
        address owner = _nodeList[tokenId].owner;
        require(to != owner, "NRMV3: approval to current owner");
        require(
    ...
    function tokenOfOwnerByIndex(
        ...
        require(index >=0 && index < _tokensOfUser[_owner].length, 'index out of bound');
    ...
    function allNodesOfUser(address _owner) public view returns (uint[] memory)  {
        uint[] memory nodesOfUser = new uint[](_tokensOfUser[_owner].length);
        for(uint i=0; i<_tokensOfUser[_owner].length; i++) {
            nodesOfUser[i] = _userTokenIdToNodeIndex[_owner][_tokensOfUser[_owner][i]] - 1;
        }
        return nodesOfUser;
    }
}
...
interface IMasterOfCoinV3 {
    ...
    event RestoreNode(string nodeId, uint256 dueDate);
