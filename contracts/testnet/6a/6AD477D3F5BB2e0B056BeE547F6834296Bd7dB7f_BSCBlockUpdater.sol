...
contract BSCBlockUpdater is Initializable, OwnableUpgradeable, Merkle {
    ...
    function importNextValidatorSet(
        ...
        if (genesisBlockNumber != 0) {
            require(parsedInput.preValidatorSetHash == validatorSetHash[epoch--], "invalid preValidatorSetHash");
        }
        ...
        if (genesisBlockNumber == 0) {
            ...
            validatorSetHash[epoch--] = parsedInput.preValidatorSetHash;
        }
    ...
    function importBlock(
        ...
        uint256 epoch = parsedInput.lastBlockNumber / 200;
        require(parsedInput.preValidatorSetHash == validatorSetHash[epoch--], "invalid preValidatorSetHash");
        require(parsedInput.currentValidatorSetHash == validatorSetHash[epoch], "invalid currentValidatorSetHash");
        require(circuitVerifier.verifyProof(a, b, c, inputs), "invalid proof");
        lastBlockNumber = parsedInput.lastBlockNumber;
        for (uint16 i = 0; i < BATCH_SIZE; i++) {
            uint256 blockNumber = parsedInput.lastBlockNumber - BATCH_SIZE + i + 1;
