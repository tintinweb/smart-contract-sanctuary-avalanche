...
contract SmolChest is ERC1155, Ownable, Constants {
    uint256 immutable TOTAL_SUPPLY = 200;
    uint128 public mintedCount;
    uint128 public mintOpenTime;
    bytes32 public wlMerkleRoot;

    mapping(address => uint256) public wlRedeemed;
    mapping(address => bool) public burnAllowlist;
    ...
    function setWlDetails(uint128 _claimOpenTime, bytes32 _wlMerkleRoot) external onlyOwner {
        mintOpenTime = _claimOpenTime;
        wlMerkleRoot = _wlMerkleRoot;
    }

    function mintSmolChest(uint64 numberOfMints, uint256 totalGiven, bytes32[] memory proof) external {
        if (wlRedeemed[msg.sender] + numberOfMints > totalGiven) {
            revert AlreadyClaimed();
        }
        if (block.timestamp < mintOpenTime) {
            revert TooSoon();
        }
        if (numberOfMints + mintedCount > TOTAL_SUPPLY) {
            revert Unauthorized();
        }

        if (!MerkleProof.verify(proof, wlMerkleRoot, keccak256(abi.encodePacked(msg.sender, totalGiven)))) {
            revert Unauthorized();
        }

        wlRedeemed[msg.sender] += numberOfMints;
        mintedCount += numberOfMints;
        _mint(msg.sender, AIRDROP_CHEST_NFT_TYPE, numberOfMints, bytes(""));
    }

    function uri(uint256 _tokenid) public view override returns (string memory) {
        return string(abi.encodePacked(ERC1155.uri(0), Strings.toString(_tokenid)));
    }

    function extendBurnerAddress(address _burner) external onlyOwner {
        burnAllowlist[_burner] = true;
    }

    function burnSmolChest(uint256 _amount, address _user) external {
        if ((_user != msg.sender) && (!burnAllowlist[msg.sender])) {
            revert Unauthorized();
        }
        _burn(_user, AIRDROP_CHEST_NFT_TYPE, _amount);
    }
}
...
contract Constants {
    ...
    uint256 public constant EPIC_FORMULA = 2400;
    uint256 public constant UNIQUE_FORMULA = 2600;
    uint256 public constant LEGENDARY_FORMULA = 2800;
    uint256 public constant COMMON_SCHEMATIC = 3000;
    uint256 public constant RARE_SCHEMATIC = 3200;
    uint256 public constant EPIC_SCHEMATIC = 3400;
    uint256 public constant UNIQUE_SCHEMATIC = 3600;
    uint256 public constant LEGENDARY_SCHEMATIC = 3800;
    // Potions
    uint256 public constant HEALING_POTION = 2201;

    uint256 public constant LEVEL_1_RECIPE = 4000;
    uint256 public constant LEVEL_2_RECIPE = 4100;
    uint256 public constant LEVEL_3_RECIPE = 4200;
    uint256 public constant LEVEL_4_RECIPE = 4300;
    uint256 public constant LEVEL_5_RECIPE = 4400;
    uint256 public constant LEVEL_6_RECIPE = 4500;
    uint256 public constant LEVEL_7_RECIPE = 4600;
    uint256 public constant LEVEL_8_RECIPE = 4700;
    uint256 public constant LEVEL_9_RECIPE = 4800;
    uint256 public constant LEVEL_10_RECIPE = 4900;

    uint256 public constant LEVEL_1_FOOD = LEVEL_1_RECIPE / 100;
    uint256 public constant LEVEL_2_FOOD = LEVEL_2_RECIPE / 100;
    uint256 public constant LEVEL_3_FOOD = LEVEL_3_RECIPE / 100;
    uint256 public constant LEVEL_4_FOOD = LEVEL_4_RECIPE / 100;
    uint256 public constant LEVEL_5_FOOD = LEVEL_5_RECIPE / 100;
    uint256 public constant LEVEL_6_FOOD = LEVEL_6_RECIPE / 100;
    uint256 public constant LEVEL_7_FOOD = LEVEL_7_RECIPE / 100;
    uint256 public constant LEVEL_8_FOOD = LEVEL_8_RECIPE / 100;
    uint256 public constant LEVEL_9_FOOD = LEVEL_9_RECIPE / 100;
    uint256 public constant LEVEL_10_FOOD = LEVEL_10_RECIPE / 100;
}
