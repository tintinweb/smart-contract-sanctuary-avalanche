...
contract GLXLuckyWheel is Ownable {
    ...
    function claim(
        ...
        require(block.timestamp <= deadline, "GLXLuckyWheel: expired deadline");

        _claimIds[claimId] = true;

        bytes32 structHash = keccak256(abi.encode(claimId, receiver, rewardType, amount, deadline));
        ...
        if (rewardType == REWARD_TYPE_TOKEN) {
            ...
            _item.safeTransferFrom(address(this), receiver, _wargearNormalBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_BLUEPRINT_WARGEAR_RARE) {
            _item.safeTransferFrom(address(this), receiver, _wargearRareBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_BLUEPRINT_WARGEAR_MYTHICAL) {
            _item.safeTransferFrom(address(this), receiver, _wargearMythicalBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_BLUEPRINT_SHIP_NORMAL) {
            _item.safeTransferFrom(address(this), receiver, _shipNormalBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_BLUEPRINT_SHIP_RARE) {
            _item.safeTransferFrom(address(this), receiver, _shipRareBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_BLUEPRINT_SHIP_MYTHICAL) {
            _item.safeTransferFrom(address(this), receiver, _shipMythicalBpId, amount, "");
        } else if (rewardType == REWARD_TYPE_SHIP_SPECIAL_ATROPOS) {
            _ship.safeTransferFrom(address(this), receiver, _atroposShipId);
        } else if (rewardType == REWARD_TYPE_SHIP_SPECIAL_LACHESIS) {
            _ship.safeTransferFrom(address(this), receiver, _lachesisShipId);
        } else if (rewardType == REWARD_TYPE_SHIP_SPECIAL_MORTA) {
            _ship.safeTransferFrom(address(this), receiver, _mortaShipId);
        }

        emit RewardClaimed(claimId, receiver);
    }
