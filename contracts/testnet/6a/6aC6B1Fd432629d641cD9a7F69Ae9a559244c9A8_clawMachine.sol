...
contract clawMachine is
    Ownable,
    ClampedRandomizer
{
    uint256 private _price;
    uint256 private _maxRandomizer = 100000;
    uint256 private _maxPerTx = 10;
    bool public paused;
    uint256 public remaining1 =1000;
    uint256 public remaining2 =3000;
    uint256 public remaining3 =6000;
    uint256 public remaining4 =100;
    ...
    ClampedRandomizer(_maxRandomizer)
    {
        contract1 = prize(_prize1);
        contract2 = prize(_prize2);
        contract3 = prize(_prize3);
        contract4 = prize(_prize4);
        
        _price = 100 * 10**15; //price in milli
        paused = false;
    }

    event win(bool[5] prize, address user);
    ...
    function withdraw(uint256 amountInMilli) external onlyOwner {
        ...
        emit withdrawn(amountInMilli * 1000);
    }

    function deposit() external payable {
        emit deposited(msg.value / 10**18);
    }

    function play(uint256 quantity, uint _prize) external payable {

        require(!paused, "game is paused");
        require(msg.value == (_price * quantity), "incorrect amount of AVAX sent");
        require(address(this).balance > 1 ether, "not enough avax in the contract");
        require(quantity <= _maxPerTx, "more than max per tx");
        require(quantity > 0, "quantity can't be 0, what are you trying to do?" );
        internalMint(msg.sender, quantity, _prize);

    }
    

    function internalMint(address to, uint256 amount, uint _prize) internal {
        require(_prize > 0 && _prize < 6 );
        bool[5] memory won;
        uint256 tokenID;
        for (uint256 i = 1; i <= amount; i++) {  
            tokenID = _genClampedNonce() + 1;
            if (tokenID <= 5000 && !won[0] && remaining1 > 0 && _prize == 1) {  //5.85 chance to win the best prize
                require(remaining1 > 0, "there's no more of this prize");
                contract1.mint(1,to);
                won[0] = true;
                remaining1 -= 1;
            } else if ( 5000 < tokenID && tokenID <= 15000 && remaining2 > 0 && _prize == 2) { //17.55 chance to win second best
                require(remaining2 > 0, "there's no more of this prize");
                contract2.mint(1,to);
                won[1] = true;
                remaining2 -= 1;
            } else if ( 15000 < tokenID && tokenID <= 30000 && remaining3 > 0 && _prize == 3) { // chance to win third best
                require(remaining3 > 0, "there's no more of this prize");
                contract3.mint(1,to);
                won[2] = true;
                remaining3 -= 1;
            }else if (30000 < tokenID && tokenID <= 30100 && remaining4 > 0 && _prize == 4){ // chance to win 1 avax
                require(remaining4 > 0, "there's no more of this prize");
                payable(to).transfer(1 ether);
                won[3] = true;
                remaining4 -= 1;
            }else if(95000 < tokenID && _prize == 5){
                require(!avaSharkWon , "there's no more of this prize");
                contract4.safeTransferFrom(0x8654FE925701a6cb66C1cBA2786eF4d3a68ad6fE, to, 1);
                won[4] = true;
                avaSharkWon = true;
            }
            
        }
        emit win(won, msg.sender);
    ...
    ClampedRandomizer
{
    ...
    function mint(uint256 quantity, address to) public onlyAuthorized{
        require(totalSupply() + quantity <= _maxMintable, "minting has ended");
