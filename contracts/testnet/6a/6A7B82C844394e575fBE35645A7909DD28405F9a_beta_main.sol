...
contract beta_main is ERC20, Ownable, PaymentSplitter {
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        cashoutFee = fees[3];
        rwSwap = fees[4];
        require(node_amount > 0, "CONSTR:7");
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        require(balanceOf(sender) >= amount_,"you do not have enough to complete the Node purchase");
        uint get = getEm();
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 teamTokens = contractTokenBalance.mul((teamPoolFee).mul(tier).div(100));
        uint256 rewardsPoolTokens = contractTokenBalance.mul((rewardsFee).div(100));
        if (get == 2 && !swapping && sender != owner() && !automatedMarketMakerPairs[sender] && swapLiquifyEnabled) {
            	swapping = true;
           ...
           super._transfer(address(this),teamPool,teamTokens);
           super._transfer(sender, address(this), amount_);
           nodeManager.createNode(sender, name, amount_);
        }
