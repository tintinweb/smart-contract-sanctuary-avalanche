...
interface IMarketplace {
    ...
    function getPurchaseInfoOrigin(
        bytes32 projectId,
        uint256 subscriptionSeconds,
        uint32 originDomainId,
        uint256 purchaseId
...
contract RemoteMarketplace is Ownable {
    ...
    function buyOrigin(bytes32 projectId, uint256 subscriptionSeconds) public {
        buyForOrigin(projectId, subscriptionSeconds, msg.sender);
    }
    ...
    function buyForOrigin(bytes32 projectId, uint256 subscriptionSeconds, address subscriber) public {
        uint256 purchaseId = purchaseCount + 1;
        purchaseCount = purchaseId;
        purchases[purchaseId] = PurchaseRequest(projectId, msg.sender, subscriber, subscriptionSeconds);
        _queryProjectOrigin(projectId, subscriptionSeconds, purchaseId);
    }
    ...
    function _queryProject(bytes32 projectId, uint256 subscriptionSeconds, uint256 purchaseId) public { // TODO: make private
        queryRouter.query(
            destinationDomainId,
            Call({to: recipientAddress, data: abi.encodeCall(IMarketplace.getPurchaseInfo, (projectId, subscriptionSeconds, purchaseId))}),
    ...
    function _queryProjectOrigin(bytes32 projectId, uint256 subscriptionSeconds, uint256 purchaseId) public { // TODO: make private
        queryRouter.query(
            destinationDomainId,
            Call({to: recipientAddress, data: abi.encodeCall(IMarketplace.getPurchaseInfoOrigin, (projectId, subscriptionSeconds, originDomainId, purchaseId))}),
            abi.encodePacked(this.handleQueryProjectResult.selector)
        );
        emit ProjectQuerySent(destinationDomainId, recipientAddress, projectId, subscriptionSeconds, purchaseId);
    }
