...
contract MetahubTreasury is MetahubAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for MHUB
    ...
    function deposit(
        ...
        // mint MHUB needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        MHUB.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn MHUB for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        MHUB.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new MHUB using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        MHUB.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or MHUB from the treasury, using sMHUB as collateral. This might allow an
     * sMHUB holder to provide MHUB liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(MHUB)) {
            require(permissions[STATUS.MHUBDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sMHUB.changeDebt(value, msg.sender, true);
        require(sMHUB.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(MHUB)) {
            MHUB.mint(msg.sender, value);
            mhubDebt = mhubDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sMHUB.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with MHUB
     * @param _amount uint256
     */
    function repayDebtWithMHUB(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.MHUBDEBTOR][msg.sender],
            notApproved
        );
        MHUB.burnFrom(msg.sender, _amount);
        sMHUB.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        mhubDebt = mhubDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(MHUB), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SMHUB) {
            sMHUB = IsMHUB(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SMHUB) {
            // 9
            sMHUB = IsMHUB(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(MHUB.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns MHUB valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(MHUB)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return MHUB.totalSupply() - mhubDebt;
    }
}
