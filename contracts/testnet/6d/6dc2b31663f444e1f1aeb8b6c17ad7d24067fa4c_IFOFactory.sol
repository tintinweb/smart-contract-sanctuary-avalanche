...
contract IFOFactory is IJoeIFOIFactory, Ownable2Step {
     ...
     * @param fundsReceiverTimelock: The timestamp when raised funds are transferable
    ...
    function createIFO(
        ...
        if (startTime <= block.timestamp) revert JoeIFOFactory__StartTimeHigherThanCurrent();
        if (fundsReceiverTimelock < endTime) revert JoeIFOFactory__FundRaiserHigherThenEndTime();
        ...
        JoeIFOInitializable(clone).initialize(raisingToken, offeringToken, startTime, endTime, fundsReceiverTimelock, adminAddress, fundRaiserAddress);
