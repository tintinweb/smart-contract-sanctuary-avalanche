...
contract SimpleProxySender is NonblockingLzApp {

    event Creation();
    event SendingMessage(uint16 dstChainId, bytes dstAddress, bytes payload);

    uint8 internal constant TYPE_TRANSFER = 1;

    constructor(address _lzEndpoint) NonblockingLzApp(_lzEndpoint) {
        emit Creation();
    }

    function sendMessage(uint16 _dstChainId, uint8 _type, address _address, uint128 _value, bytes memory _adapterParams) public payable {
        _lzSend(_dstChainId, abi.encode(TYPE_TRANSFER, _address, _value), payable(tx.origin), address(0x0), _adapterParams);
    }

    function transferValue(uint16 _dstChainId, address _address, uint128 _value) public payable {
        sendMessage(_dstChainId, TYPE_TRANSFER, _address, _value, "");
    }
