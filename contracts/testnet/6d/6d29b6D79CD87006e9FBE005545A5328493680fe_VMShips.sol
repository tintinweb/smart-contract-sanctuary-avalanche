...
\$$\  $$  |$$\$$\$$ $$ |      \$$$$$$\  $$$$$$$$ |  $$ |  $$$$$$$  |\$$$$$$\        $$ $$\$$ |$$$$$\       $$ |   
 \$$\$$  / $$ \$$$  $$ |       \____$$\ $$  __$$ |  $$ |  $$  ____/  \____$$\       $$ \$$$$ |$$  __|      $$ |   
  \$$$  /  $$ |\$  /$$ |      $$\   $$ |$$ |  $$ |  $$ |  $$ |      $$\   $$ |      $$ |\$$$ |$$ |         $$ |   
   \$  /   $$ | \_/ $$ |      \$$$$$$  |$$ |  $$ |$$$$$$\ $$ |      \$$$$$$  |      $$ | \$$ |$$ |         $$ |   
    \_/    \__|     \__|       \______/ \__|  \__|\______|\__|       \______/       \__|  \__|\__|         \__|   
*/
...
contract VMShips is Initializable, ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
	...
	uint256 public priceForPublicSale;
	uint256 public priceForWL;
	uint256 public maxSupply;
	uint256 public reservedSupply;
	uint256 public nextVMAirdrop;
	uint256 public mintedForVMGroup;
	...
	function initialize() public initializer {
		__ERC721_init("VM Ships", "VMS");
		__ERC721Enumerable_init();
		__Ownable_init();
		paused = true;
		revealed = false;

		priceForPublicSale = 1 ether;
		priceForWL = 0.88 ether;
		maxSupply = 10000;
		reservedSupply = 2000;
		...
		communityAddress = 0x9033606977d8E088fc744362Ee896fbE92479099;
		wavaxAddress = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
		vmnAddress = 0xAb21520fd304ff92c249b368EdE2D08b9d625E20; //replace in mainnet

		wavaxContract = IERC20(wavaxAddress);
	...
	function mint(uint256 amount) public payable nonReentrant {
		...
		uint256 proceeds = 0;
		if (verifyUserForWhiteListCollection(msg.sender)) {
			proceeds = priceForWL * amount;
		} else { 
			proceeds = priceForPublicSale * amount;
		}
	...
	function mintForVMGroup() external onlyOwner {
		for (uint256 i = 10001; i <= 10013; i++) {
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) public onlyOwner {
		...
		for (uint256 i = 0; i < addresses.length; i++) {
			...
			for (uint256 j = 0; j < amount; j++) {
				...
				uint256 tokenID = supply.current() - mintedForVMGroup;
	...
	function airDropVMOwner(uint256 _lastToken) public onlyOwner {
		...
		require(_lastToken <= 2000, "Last token must be 2000 or less.");

		IERC721 VMContract = IERC721(vmnAddress);
	...
	function verifyUserForWhiteListCollection(address _user) public view returns (bool) {
		for(uint8 i = 0;  i < WhiteListCollections.length; i++) {
			if (IERC721(WhiteListCollections[i]).balanceOf(_user) > 0)
				return true;
		}
		return false;
	}
	...
	function withdraw() public onlyOwner {
		...
		payable(communityAddress).transfer(balance_10percent * 7);
	...
	function addWhitelistCollection(address[] memory _list) external onlyOwner {
		for (uint256 i = 0; i < _list.length; i++) {
			WhiteListCollections.push(_list[i]);
		}
	}

	function removeWhitelistCollection(address[] memory _list) external onlyOwner {
		uint256 len = WhiteListCollections.length;
		require(len > 0);
		for (uint256 i = 0; i < _list.length; i++) {
			for (uint256 j = 0; j < len; j++) {
				if (_list[i] == WhiteListCollections[j]) {
					WhiteListCollections[j] = WhiteListCollections[len - 1];
					WhiteListCollections.pop();
					len--;
				}
			}
		}
	...
	function setWhiteListCollections(address[4] memory _list) public onlyOwner {
		WhiteListCollections = _list;
	}
	...
	function setPriceForWL(uint256 newPrice) public onlyOwner {
		priceForWL = newPrice;
	}

	function setPriceForPublicSale(uint256 newPrice) public onlyOwner {
		priceForPublicSale = newPrice;
	}
	...
	function setWithdrawAddresses(address newCommunityAddress) public onlyOwner {
		communityAddress = newCommunityAddress;
	}

	function setWavaxAddr (address _contract) public onlyOwner {
		wavaxAddress = _contract;
		wavaxContract = IERC20(wavaxAddress);
	}
	...
	function _mintLoop(address to, uint256 amount) internal {
		for (uint256 i = 0; i < amount; i++) {
			...
			_safeMint(to, supply.current() - mintedForVMGroup);
		}
	}
	...
	function withdrawWavax() public onlyOwner { 
		...
		uint256 balance_fraction = balance / 2;

        wavaxContract.transfer(communityAddress, balance_fraction);
		wavaxContract.transfer(owner(), balance_fraction);
    }
}
