...
contract BuyNFT is GameFiat, Withdrawable {
    ...
    string public constant REVERT_NOT_A_CEO = "BuyNFT: caller is not CEO";
    string public constant REVERT_NOT_A_OWNER_NFTS = "BuyNFT: caller is not the owner of NFTs or NFTs are being rented";
    string public constant REVERT_NOT_A_MAKER = "BuyNFT: caller is not a maker";
    string public constant REVERT_NOT_A_SUB_MARKET = "BuyNFT: caller is not SubMarketplace";
    string public constant REVERT_INVALID_FIAT = "BuyNFT: Invalid fiat";
    string public constant REVERT_INVALID_FEE = "BuyNFT: Invalid fee";
    ...
    function _isOwnerOf(address _game, uint256[] memory _tokenIds) private view returns (bool) {
        ...
        IERC4907 erc4907Address = IERC4907(_game);
        for (uint256 i = 0; i < length; i++) {
            if ((erc721Address.ownerOf(_tokenIds[i]) != account) || (erc4907Address.userExpires(_tokenIds[i]) > block.timestamp)) {
                flag = false;
                break;
            }
        }
