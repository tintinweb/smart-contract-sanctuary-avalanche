...
/**
 * @title IPantokenProfile
 */
interface IPantokenProfile {
...
import {IERC20Metadata} from "../openzeppelin/contractsV4.8.1/token/ERC20/extensions/IERC20Metadata.sol";
...
contract SmartChefFactory is Ownable {
     ...
     * @param _pantokenProfile: Pantoken Profile address
     * @param _pantokenProfileIsRequested: Pantoken Profile is requested
     * @param _pantokenProfileThresholdPoints: Pantoken Profile need threshold points
    ...
    function deployPool(
        ...
        address _pantokenProfile,
        bool _pantokenProfileIsRequested,
        uint256 _pantokenProfileThresholdPoints,
        address _admin
    ) external onlyOwner {
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_pantokenProfile, _pantokenProfileIsRequested, _pantokenProfileThresholdPoints)
        );
...
import {IPantokenProfile} from "./interfaces/IPantokenProfile.sol";
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Pantoken profile
    IPantokenProfile public immutable pantokenProfile;

    // Pantoken Profile is requested
    bool public pantokenProfileIsRequested;

    // Pantoken Profile points threshold
    uint256 public pantokenProfileThresholdPoints;
    ...
    constructor(
        ...
        IPantokenProfile(_pantokenProfile).getTeamProfile(1);
        pantokenProfile = IPantokenProfile(_pantokenProfile);

        // if pantokenProfile is requested
        pantokenProfileIsRequested = _pantokenProfileIsRequested;

        // pantokenProfile threshold points when profile & points are requested
        pantokenProfileThresholdPoints = _pantokenProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!pantokenProfileIsRequested && pantokenProfileThresholdPoints == 0) ||
                pantokenProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (pantokenProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = pantokenProfile.getUserProfile(msg.sender);
        }

        require(
            pantokenProfileThresholdPoints == 0 || numberUserPoints >= pantokenProfileThresholdPoints,
        ...
        onlyOwner
    {
        ...
        pantokenProfileIsRequested = _isRequested;
        pantokenProfileThresholdPoints = _thresholdPoints;
