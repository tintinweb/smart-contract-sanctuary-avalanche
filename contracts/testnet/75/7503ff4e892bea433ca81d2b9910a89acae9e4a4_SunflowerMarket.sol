...
contract SunflowerMarket is INFTMarket {
    ...
    /// @param askID askIDs
    /// asks on.
    function acceptAskToFarm(bytes32[] calldata askID, address[] calldata landAddr) external payable override {
        uint256 totalPrice = 0;
        for (uint256 i = 0; i < askID.length; i++) {
            Ask memory ask = asks[askID[i]];
            require(ask.exists, REVERT_ASK_DOES_NOT_EXIST);
    ...
    function getBid() internal returns (uint256) {}
