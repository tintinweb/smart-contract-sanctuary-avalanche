...
contract BridgesOfFate is Ownable {
    ...
    event remainCapacityElseEvent(uint256 _capacity);
     ...
     function _selectSuccessSidePlayerId(bytes32[] memory _playerIDAtSpecifyStage, bool currentJumpSide,uint256 currentDay) public  returns(bytes32[]  memory){
        ...
        for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
            if(
                PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide == currentJumpSide  // current Jumped 
                &&
                (OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == false && currentDay == PlayerItem[_playerIDAtSpecifyStage[i]].day )
                ||
                (PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide == currentJumpSide && OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == true))
            {
                ...
                for (uint256 k = 0; k < _randomSelectPlayerIndexPreviousStage.length; k++) {
                    // if(_randomSelectPlayerIndexPreviousStage[k] > 0){
                        ...
                        // if(SelectUnholdPlayerId.length > 0){
                            emit holdCapacityBytesEvent(SelectUnholdPlayerId[k]);
                        ...
                        //         }
                        //     bool _overFlowPlayer = OverFlowPlayerStatus[SelectUnholdPlayerId[_randomSelectPlayerIndexPreviousStage[k]]];
                            emit holdCapacityBytesBoolEvent(OverFlowPlayerStatus[SelectUnholdPlayerId[_randomSelectPlayerIndexPreviousStage[k]]]);
                        // } 
                    // }
                }
