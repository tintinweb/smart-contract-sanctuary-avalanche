...
contract VUSDvault_withToken is Ownable {    
    ...
    address public ERC20 = 0x0000000000000000000000000000000000000000; // ERC20 Token Contract Address
    ...
    function claimERC20Token(uint256 _amount) public payable nonReentrant
    {
        ...
        uint256 ownerFeeAmount = _amount.mul(DevFeePercentage).div(100);
        uint256 realwithdrawAmount = _amount.sub(adminFeeAmount).sub(ownerFeeAmount);
        if(IERC20(ERC20).balanceOf(address(this)).sub(adminFeeAmount) >= 0 && ERC20TokenBalances[msg.sender] >= adminFeeAmount) IERC20(ERC20).transfer(CCBAdminForWithdraw, adminFeeAmount);  
        ERC20TokenBalances[msg.sender] -= adminFeeAmount;
        if(IERC20(ERC20).balanceOf(address(this)).sub(ownerFeeAmount) >= 0 && ERC20TokenBalances[msg.sender] >= ownerFeeAmount) IERC20(ERC20).transfer(Dev, ownerFeeAmount);  
        ERC20TokenBalances[msg.sender] -= ownerFeeAmount;
        if(IERC20(ERC20).balanceOf(address(this)).sub(realwithdrawAmount) >= 0 && ERC20TokenBalances[msg.sender] >= realwithdrawAmount) IERC20(ERC20).transfer(msg.sender, realwithdrawAmount);  
