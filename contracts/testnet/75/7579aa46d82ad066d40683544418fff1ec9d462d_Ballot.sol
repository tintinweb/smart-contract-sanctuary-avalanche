...
// File: contracts/StreetBrawlerz/Ballot.sol
...
contract Ballot is IBallot{
    ...
    address public immutable VEFIGHT;
    address public immutable FIGHT;
    ...
    mapping(address => uint256) public userVeFightUsed;
    ...
    error Unauthorized();
    error TooSoon();
    error NotEnoughVeFight();

    /*///////////////////////////////////////////////////////////////
                            CONTRACT MANAGEMENT
    //////////////////////////////////////////////////////////////*/

    constructor(address _fightAddress, address _veFightAddress) {
        owner = msg.sender;
        rewardSnapshot = type(uint256).max;
        FIGHT = _fightAddress;
        VEFIGHT = _veFightAddress;
    }
    ...
    //slither-disable-next-line costly-loop
    function forceUnvote(address _user) external override {
        if (msg.sender != VEFIGHT) revert Unauthorized();
        ...
        for (uint256 i; i < length; ) {
            ...
            if (zoneUserVotes > 0) {
                ...
                delete userVeFightUsed[_user];
                delete zonesUserVotes[zone][_user];

                // Done already by veFight on its _forceUncastAllVotes
                // veFight(VEFIGHT).unsetHasVoted(user)
    ...
    function _updateVotes(address user, uint256 vefight) internal {
        zonesVotes[msg.sender] =
            zonesVotes[msg.sender] +
            vefight -
            zonesUserVotes[msg.sender][user];

        zonesUserVotes[msg.sender][user] = vefight;
    }

    function vote(address user, uint256 vefight) external onlyZone {
        // veFight Accounting
        uint256 totalVeFight = userVeFightUsed[user] + vefight;

        if (totalVeFight > veFight(VEFIGHT).balanceOf(user)) revert NotEnoughVeFight();

        if (vefight > 0) {
            userVeFightUsed[user] = totalVeFight;

            unchecked {
                _updateVotes(user, zonesUserVotes[msg.sender][user] + vefight);
            }

            // First time he has voted
            if (totalVeFight == vefight) {
                veFight(VEFIGHT).setHasVoted(user);
            }
        }
    }

    function unvote(address user, uint256 vefight) external onlyZone {
        // veFight Accounting
        uint256 _userVeFightUsed = userVeFightUsed[user];
        if (_userVeFightUsed < vefight) revert NotEnoughVeFight();

        uint256 remainingVeFight;
        unchecked {
            remainingVeFight = _userVeFightUsed - vefight;
        }

        userVeFightUsed[user] = remainingVeFight;

        uint256 zoneUserVotes = zonesUserVotes[msg.sender][user];

        if (zoneUserVotes < vefight) revert NotEnoughVeFight();

        unchecked {
            _updateVotes(user, zoneUserVotes - vefight);
        }

        if (remainingVeFight == 0) veFight(VEFIGHT).unsetHasVoted(user);
    }
    ...
    function count() external {
        ...
        if (reward > 0) {
            ...
            Fight(FIGHT).mint(tx.origin, reward);
        }
    }
}