...
contract NODERewardManagement {
    ...
    // mapping referral => refferred => amount
    ...
    uint256 public referralBonusPercent = 0;
    ...
    function createNode(address account, string memory nodeName, address referred_by)
        public
        onlySentry
    {
        require(
    ...
    function getLastNodeCreationTime(address account) external view returns(uint256){
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 lastNodeCreationTime = nodes[nodes.length - 1].creationTime;
        return lastNodeCreationTime;
    }
    ...
    function _transferNode(address prevOwner, uint256 _creationTime, address newOnwer) external onlySentry{
        ...
        int256 index = binary_search(nodesOfPrev, 0, numberOfNodes, _creationTime);
        uint256 validIndex;
        if (index >= 0) {
            validIndex = uint256(index);
        }
        // delete node from previous owner first to avoid re-entrancy
        delete _nodesOfUser[prevOwner][validIndex];
        _nodesOfUser[newOnwer].push(node);
        
    }

    function _addReferral(address referrer, address referred_by) public onlySentry{
    ...
    function getReferrals(address referred_by) public view onlySentry returns(address[] memory) {
        return userReferrals[referred_by];
    }

    function getReferralBonus(address account) public view onlySentry returns(uint256) {
        return availableReferralBonus[account];
    }

    function withdrawReferralBonus(address account) public onlySentry returns(uint256) {
    ...
    function _compoundNode(address account, uint256 created_at, string memory nodeName) external onlySentry{
        ...
        createNode(account, nodeName, referred_by);
...
contract ZEUS is Ownable {
    ...
    address public distributionPool = 0x2B6935FA50384956C5475E17A197F48896EaaBD8;
    address public token = 0x4156F18bF7C1ef04248632C66Aa119De152D8f2E;
    ...
    function createNodeWithTokens(string memory name, address referred_by) public {
        require(
        ...
        require(!_isBlacklisted[sender] && !_isBlacklisted[referred_by], "NODE CREATION: Blacklisted address");
        require(
        ...
        require(
            IERC20(token).balanceOf(sender) >= nodePrice,
        ...
        IERC20(token).transfer(address(this), nodePrice);
        nodeRewardManager.createNode(sender, name, referred_by);
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        if (swapLiquify) {
            ...
            if (block.timestamp > blocktime + roi) {
                uint256 bonus = rewardAmount.mul(roiBonusPercent).div(100);
                rewardAmount += bonus;
            }
            rewardAmount -= feeAmount;
        }
        IERC20(token).transferFrom(distributionPool, sender, rewardAmount);
    ...
    function cashoutAll() public {
        ...
        if (swapLiquify) {
            ...
            uint256 lastNodeCreationTime = nodeRewardManager
                .getLastNodeCreationTime(sender);
            if (block.timestamp > lastNodeCreationTime + roi) {
                uint256 bonus = rewardAmount.mul(roiBonusPercent).div(100);
                rewardAmount += bonus;
            }
            rewardAmount -= feeAmount;
    ...
    function transferNode(uint256 _creationTime, address newOnwer) external {
        address sender = _msgSender();
         require(!_isBlacklisted[sender] && !_isBlacklisted[newOnwer], "NODE CREATION: Blacklisted address");
    ...
    function compoundNode(uint256 created_at, string memory nodeName) external {
        uint256 nodePrice = nodeRewardManager.nodePrice();
        ...
        require(
            IERC20(token).balanceOf(sender) >= total,
            "NODE CREATION: Balance too low for compounding."
        );
        IERC20(token).transfer(address(this), total);
        nodeRewardManager._compoundNode(sender, created_at, nodeName);
    }

    function withdrawReferralBonus() external {
        nodeRewardManager.withdrawReferralBonus(_msgSender());
    }
