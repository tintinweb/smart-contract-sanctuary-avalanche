...
contract MarketCore is MarketPojo{
    ...
    mapping(bytes32 => uint256) public orderState;
    mapping(bytes32 => bytes) public oderInfo;
 ...
 function toEthSignedMessageHash(bytes32 hash) external pure returns (bytes32) {
  ...
  //  }
    ...
    // }
     ...
     bytes32  public orderhash2;
     bytes32 public  signatureHash2;

    function getOrderHash2() external view returns(bytes32){
      return orderhash2;
    } 

    function getsignatureHash2() external view returns(bytes32){
  ...
  // 比如 firstCallData: 把erc721资产转走  secondCallData:  把erc20资产转走
    ...
    function orderMatch(bytes32 firstOrderHash,address firstsinger,address secondCallTarget,bytes memory secondcallData, bytes memory signatures,bytes memory orderData) external {
            ...
            // bytes32 seccondSignatureHash = hashSignature(secondOrderHash,Proof.getBehindBytes(firstcallData),firstcallTarget);
