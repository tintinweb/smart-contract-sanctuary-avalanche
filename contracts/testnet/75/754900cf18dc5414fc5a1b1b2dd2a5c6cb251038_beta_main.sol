...
contract beta_main is ERC20, Ownable, PaymentSplitter {
    ...
    function checkTier(address sendit) public returns(uint[5] memory){
    ...
    function updateTreasury(address payable newVal) external onlyOwner {
        treasury = newVal; // team pool address
    }
    ...
    function swapAVAXForTokens(uint256 AVAXAmount) private {
        ...
        path[1] = address(this);
        _approve(address(this), address(joeRouter), AVAXAmount);
        joeRouter.swapExactAVAXForTokensSupportingFeeOnTransferTokens(
    ...
    function createNodeWithTokens(string memory name) external {
        uint[5] memory tier =  checkTier(_msgSender());
        ...
        uint256 teamTokens = contractTokenBalance.mul((teamPoolFee).mul(tier[1]).div(100));
        uint256 rewardsPoolTokens = contractTokenBalance.mul((rewardsFee).mul(tier[1]).div(100));
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        rewardAmount = (rewardAmount.mul(tier[0]).div(100));
        require(rewardAmount > 0,"your reward amount is not above zero");
        uint get = getEm();
        uint256 feeAmount = rewardAmount.mul((cashoutFee.mul(tier[1]).div(100)));
        if (feeAmount > 0) {
		if (get == 2){ 
