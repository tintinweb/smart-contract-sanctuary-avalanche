...
import { INAVIssuanceHook } from "@setprotocol/set-protocol-v2/contracts/interfaces/INAVIssuanceHook.sol";
...
/**
 * @title CustomOracleDexIssuanceModule
 * @author Set Protocol
 *
 ...
 */
contract DexIssueModuleV1 is ModuleBase, ReentrancyGuard {
    ...
    struct ActionInfo {
        ...
        uint256 finalSetTokenQuantity;                 // Final set token quantity after trade
        uint256 finalRedeemQuantity;                   // Final redeem quantity after trade
    }
    ...
    uint256 constant internal ONE_HUNDRED_PERCENT = 1000000000000000000;
    /* ============ Constructor ============ */
        ...
        onlyValidAndInitializedSet(_setToken)
    {
        ...
        issueInfo = _makeIssueDexSwaps(_setToken, IERC20(_reserveAsset), issueInfo);
        ...
        onlyValidAndInitializedSet(_setToken)
    {
        ...
        redeemInfo = _makeReedemDexSwaps(_setToken, IERC20(_reserveAsset), redeemInfo);

        (redeemInfo.protocolFees, redeemInfo.managerFee, redeemInfo.netFlowQuantity) = _getFees(
            _setToken,
            redeemInfo.finalRedeemQuantity,
    ...
    /**
     * SET MANAGER ONLY. Edit the trade module
     *
     * @param _setToken                     Instance of the SetToken
     * @param _tradeModule                  TradeModule address
     */
    function editTradeModule(ISetToken _setToken, address _tradeModule) external onlyManagerAndValidSet(_setToken) {
        require(_tradeModule != address(0), "Trade module must not be 0 address");

        navIssuanceSettings[_setToken].tradeModule = _tradeModule;
    }
        ...
        returns (bool)
    {
        ...
        if (
            ...
            uint256 existingUnit = ERC20(_reserveAsset).balanceOf(address(_setToken)); //_setToken.getDefaultPositionRealUnit(_reserveAsset).toUint256();

            return existingUnit >= expectedRedeemQuantity;
        }
    }
    ...
    * DEX Swap
    */
    function _makeIssueDexSwaps(ISetToken _setToken, IERC20 _reserveAsset, ActionInfo memory _issueInfo) internal returns (ActionInfo memory) {
        (
            address[] memory components,
            uint256[] memory componentQuantities
        ) = getRequiredComponentUnitsForIssue(_setToken, _issueInfo.setTokenQuantity);
        uint256 SWAP_INSURANCE_PERCENT = 100000000000000000;

        uint256[] memory setComponentValuation = _getSetValuer(_setToken).calculateComponentsValuation(components, componentQuantities, address(_reserveAsset));
        uint256[] memory finalAssets = new uint256[](components.length);


        // For each position, buy the required underlying from the Uniswap
        for (uint256 i = 0; i < components.length; i++) {
            // Trade the component to the SetToken
            address[] memory path = new address[](2);
            path[0] = address(_reserveAsset);
            path[1] = components[i];
            bytes memory _data = this.getUniswapExchangeData(path, true);
            uint256 additional = componentQuantities[i].preciseMul(SWAP_INSURANCE_PERCENT);
            uint256 componentMinValue = componentQuantities[i].sub(additional);
            finalAssets[i] = ITradeModule(navIssuanceSettings[_setToken].tradeModule).trade(_setToken, "uniswap", address(_reserveAsset), setComponentValuation[i], components[i], componentMinValue, _data);
        }

        uint256 minPercent = ONE_HUNDRED_PERCENT;
        for (uint256 i = 0; i < finalAssets.length; i++) {

          uint256 percent = finalAssets[i].preciseDiv(componentQuantities[i]);
          if (percent < minPercent) {
            minPercent = percent;
          }
        }

        _issueInfo.finalSetTokenQuantity = _issueInfo.setTokenQuantity.preciseMul(minPercent);
        return _issueInfo;
    }

    struct DexIssue {
      uint256[] setComponentValuation;
      address[] components;
    ...
    * DEX Swap
    */
    function _makeReedemDexSwaps(ISetToken _setToken, IERC20 _reserveAsset, ActionInfo memory _issueInfo) internal returns (ActionInfo memory) {
        DexIssue memory dexIs;
        (
            dexIs.components,
            dexIs.componentQuantities
        ) = getRequiredComponentUnitsForIssue(_setToken, _issueInfo.setTokenQuantity);

        uint256 SWAP_INSURANCE_PERCENT = 100000000000000000;

        dexIs.setComponentValuation = _getSetValuer(_setToken).calculateComponentsValuation(dexIs.components, dexIs.componentQuantities, address(_reserveAsset));

        uint256 finalQuantity = 0;
        // For each position, buy the required underlying from the Uniswap
        for (uint256 i = 0; i < dexIs.components.length; i++) {
            // Trade the component to the SetToken
            address[] memory path = new address[](2);
            path[0] = dexIs.components[i];
            path[1] = address(_reserveAsset);
            bytes memory _data = this.getUniswapExchangeData(path, true);
            uint256 additional = dexIs.setComponentValuation[i].preciseMul(SWAP_INSURANCE_PERCENT);
            uint256 _sendQuantity = dexIs.setComponentValuation[i].sub(additional);
            finalQuantity += ITradeModule(navIssuanceSettings[_setToken].tradeModule).trade(_setToken, "uniswap", dexIs.components[i], dexIs.componentQuantities[i], address(_reserveAsset), _sendQuantity, _data);
        }

        _issueInfo.finalRedeemQuantity = finalQuantity;
        return _issueInfo;
    }
        ...
        internal
    {
        ...
        _setToken.mint(_to, _issueInfo.finalSetTokenQuantity);
        ...
        emit SetTokenNAVIssued(
            ...
            _issueInfo.finalSetTokenQuantity,
        ...
        returns (uint256)
    {
        ...
        return normalizedTotalReserveQuantityNetFees.preciseDiv(setTokenValuation);
    }
    ...
    {
        //uint256 existingUnit = _setToken.getDefaultPositionRealUnit(_reserveAsset).toUint256();
        uint256 totalExistingUnits = ERC20(_reserveAsset).balanceOf(address(_setToken)); //_setToken.getDefaultPositionRealUnit(_reserveAsset).toUint256();
    ...
    function _getSetValuer(ISetToken _setToken) internal view returns (ISetValuer) {
        ...
        return address(customValuer) == address(0) ? ISetValuer(address(controller.getSetValuer())) : customValuer;
    }
}
...
interface ITradeModule {
    /**
     * Executes a trade on a supported DEX. Only callable by the SetToken's module.
