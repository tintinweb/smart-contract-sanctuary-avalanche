...
contract LendingPool is ERC20, ReentrancyGuard, Pausable, AccessControl {
  ...
  */
  function _mintShares(uint256 _assetAmount) internal returns (uint256) {
    ...
    uint256 shares = previousTotalValue == 0 ? _assetAmount.mul(conversionFactor) : _assetAmount.mul(conversionFactor).mul(totalSupply()).div(previousTotalValue);
  ...
  */
  function _burnShares(uint256 _sharesAmount) internal returns (uint256) {
    ...
    uint256 withdrawAmount = totalShares == 0 ? 0 : _sharesAmount.div(conversionFactor).mul(totalValue()).div(totalShares);
