...
contract Espionage is ERC20, Ownable, Pausable {
    ...
    function mint(address _to, uint256 _amount) external whenNotPaused {
        if (_to == address(0) || StakeAddress == address(0)) revert Espionage_ZeroAddress();

        if (_msgSender() != StakeAddress && ExtensionAddresses[_msgSender()] == false) revert Espionage_Unauthorized();
    ...
    function burn(address _from, uint256 _amount) external whenNotPaused {
        if (_from == address(0) || StakeAddress == address(0)) revert Espionage_ZeroAddress();
    ...
    function RemoveExtensionAddress(address _address) external onlyOwner {
        ...
        if (ExtensionAddresses[_address] == false) revert Espionage_NotExtensionAddress();
