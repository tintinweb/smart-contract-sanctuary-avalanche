...
contract VVSToken is ERC20, Ownable, Whitelisted {
    using SafeMath for uint256;

    string private constant TOKEN_NAME = "VVS Token";
    string private constant TOKEN_SYMBOL = "VVS";
    uint8 private constant DECIMALS = 18;

    // Total Supply  210,000,000 VVS

    uint256 private TOTAL_SUPPLY_LIMIT = 210000000 * (10**DECIMALS);
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        ...
        require( total+amount <= TOTAL_SUPPLY_LIMIT, "Total supply limited");
        _mint(to, amount);
    }
    ...
    function setTotalSupplyLimit(uint256 _limit) public onlyOwner {
        uint256 total = totalSupply();
        require(total <= _limit, "New limit must be greater than or equal to current total supply");
        TOTAL_SUPPLY_LIMIT = _limit;
    }

    function getTotalSupplyLimit() public view returns(uint256 _totalSupplyLimit){
        _totalSupplyLimit = TOTAL_SUPPLY_LIMIT;
    }
