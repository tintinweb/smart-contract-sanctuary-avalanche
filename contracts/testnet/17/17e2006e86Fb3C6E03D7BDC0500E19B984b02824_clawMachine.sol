...
contract clawMachine is ConfirmedOwner, VRFConsumerBaseV2, IERC721Receiver {
    ...
    event win(uint256 prize, address user);
    ...
    function withdrawERC721(address addy, uint256 tokenId) external onlyOwner {
        IERC721(addy).safeTransferFrom(
            address(this),
            msg.sender,
            tokenId
        );
    ...
    function internalMint(address to) internal {
        ...
        if (
            ...
            IERC721(avashark).safeTransferFrom(
                address(this),
                to,
                avasharkToken
            );
