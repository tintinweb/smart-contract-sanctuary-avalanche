...
contract BenqiAdapter is AdapterBase, Basic, Ownable {
    ...
    event BenqiBorrow(address qiToken, uint256 amount, address owner);
    event BenqiRepay(address qiToken, uint256 amount, address owner);
    event BenqiDeposit(address token, uint256 amount, address owner);
    event BenqiWithDraw(address token, uint256 amount, address owner);
    ...
    function deposit(bytes calldata encodedData) external onlyProxy {
        (address tokenAddr, address qitokenAddr_, uint256 tokenAmount) = abi
    ...
    function withdraw(bytes calldata encodedData) external onlyAdapterManager {
        (
            address tokenAddr,
            address qitokenAddr_,
            uint256 qiTokenAmount,
            address owner
        ...
        require(qiTokenAddr != address(0), "Token invalid.");
        pullAndApprove(qiTokenAddr, owner, qiTokenAddr, qiTokenAmount);
        uint256 amountDiff;
        if (tokenAddr == avaxAddr) {
        ...
        emit BenqiWithDraw(tokenAddr, qiTokenAmount, owner);
    }
    ...
    function borrow(bytes calldata encodedData) external onlyProxy {
        (address tokenAddr, address qitokenAddr_, uint256 amount) = abi.decode(
            encodedData,
        ...
        )
    {
        ...
        address[] memory assets = benqi.getAssetsIn(user);
    ...
    function GetUserLiquidateValue(address user) public view returns (uint256) {
        IComptroller benqi = IComptroller(unitrollerAddr);
        IPriceOracle oracle = IPriceOracle(benqi.oracle());
        uint256 sumCollateral;
        address[] memory assets = benqi.getAssetsIn(user);
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < reservesList.length; i++) {
            uint256 tokenBalance = IERC20(reservesList[i]).balanceOf(user);
            if (tokenBalance > 0) {
...
interface IBenqiAdapter {
    ...
    function BorrowRepayEncoder(
        address token,
        address qitoken,
        uint256 amount
    ) external;

    function DepositEncoder(address qitoken, uint256 amount) external;

    function EnterMarkets(address[] memory qitoken) external;
