...
// File contracts/SmayHouseV2.sol

pragma solidity ^0.8.0;



// SmayHouseV2 provides multi-token rewards for the farms of Stake Smay
// This contract is forked from Popsicle.finance which is a fork of SushiSwap's MasterChef Contract
// It intakes one token and allows the user to farm another token. Due to the crosschain nature of Stake Smay we've swapped reward per block
...
contract SmayHouse is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SMAY
    ...
    struct PoolInfo {
        ...
        uint256[] AllocPoints; // How many allocation points assigned to this pool. SMAY to distribute per second.
    }
    ...
    // Owner can retreive excess/unclaimed SMAY 7 days after endtime
    // Owner can NOT withdraw any token other than SMAY
    ...
    // Changes Smay token reward per second. Use this function to moderate the `lockup amount`. Essentially this function changes the amount of the reward
    ...
    // Deposit staking tokens to SmayHouse for rewards allocation.
    ...
    // Withdraw staked tokens from SmayHouse.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(
            user.amount >= _amount,
            "SmayHouse: amount to withdraw is greater than amount available"
        );
        updatePool(_pid);
        for (uint256 i = 0; i < RewardTokens.length; i++) {
            uint256 pending =
