...
contract ConsumableItem is ERC1155, Ownable {
    using SafeMath for uint256;

    // Consumable Item types
    uint256[] public types;

    // amount of Consumable Items minted by their IDs
    uint256[] public minted;

    // Consumable Items prices in USD (item's ID => price, price example: $4.99 = 499 * 10^16)
    mapping(uint256 => uint256) public pricesUSD;

    // amount of Consumable Items minted individually (address => item's ID => amount)
    mapping(address => mapping(uint256 => uint256)) public mintedByPlayer;

    // if Consumable Item used within the game (address => item's ID => used or not)
    mapping(address => mapping(uint256 => bool)) public usedByPlayer;
    ...
    // input must be normal price * 100, example: $4.99 = 499 * 10^16
    function addItem(uint256 mintLimit, uint256 priceUSD) external onlyOwner {
        uint256 id = types.length;
        types.push(id);
        minted.push(mintLimit);
        pricesUSD[id] = priceUSD * 10**16; // it should be 16!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
