    ...
    ReentrancyGuardUpgradeable
{
    ...
    event NestBought(address user, uint256 price);
    mapping(address => int8) private _addressToNestAmount;

    address private jobExecutor;

    event EmergencyTransferCHRO(
        address caller,
        address receiver,
        uint256 amount
    );

    event BreedItemMinted(
        bytes32 childId,
    ...
    function initialize() public initializer {
        _bCollection = IERC721b(
            address(0x237cCBb942861D9Cd6192598e5071B14C1730cAF)
        );
    }
    ...
    modifier onlyExecutor() {
        require(
            jobExecutor == _msgSender() || owner() == _msgSender(),
            "Caller is not job executor"
        );
        _;
    }
    ...
    function _safeRemove(
        ...
        uint256[] memory newArray = _removeElement(
            _tokenHolder[_address][_holderPlace],
            _tokenId
        );
        _tokenHolder[_address][_holderPlace] = newArray;
    }
     ...
     */
    function setReward(address _address, uint256 _reward) public onlyExecutor {
    ...
    /**
     * @notice Claimable Status
     * @param _address address to be looked
     * @return array [amount, last claimed time]
     */
    function claimableStatus(address _address)
        public
        view
        returns (uint256[2] memory)
    {
        return [_addressCHROReward[_address], _addressClaimedTime[_address]];
    }
    ...
    function safeDispatch(
        ...
        _safeRemove(_holderPlace, _tokenId, _address);
     ...
     */
    function safeTransferCHRO(address _address, uint256 _amount)
        public
        onlyExecutor
    {
        _tokenContract.transfer(_address, _amount);

        emit EmergencyTransferCHRO(_msgSender(), _address, _amount);
    }
    ...
    /**
     * @notice Breed and Train
     * @param _cost uint256 cost of breeding
     * @param _parent1 uint256 parent 1 token id
     * @param _parent2 uint256 parent 2 token id
     * @param _nestId string string of Nest ID
     * @param _elementalStone string Elemental Stone used
     */
    function payBreed(
        uint256 _cost,
        uint256 _parent1,
        uint256 _parent2,
        string memory _nestId,
        string memory _elementalStone
    ) public {
        //  check balance
        require(
            _tokenContract.balanceOf(_msgSender()) >= _cost,
            "Not enough tokens"
        );
        require(
            _tokenContract.allowance(_msgSender(), address(this)) >= _cost,
            "Not enough allowance"
        );

        validateBreed(_parent1, _parent2);

        // transfer CHRO to this contract as P2E wallet
        _tokenContract.transferFrom(_msgSender(), address(this), _cost);
    ...
    function validateBreed(uint256 parent1, uint256 parent2) public view {
        require(
            _breedCounts[parent1] < 3 && _breedCounts[parent2] < 3,
            "Max breed count of parent"
        );
        require(
            _tokenOwners[parent1] == _msgSender() &&
                _tokenOwners[parent2] == _msgSender(),
            "Not token owner"
        );
    }
    ...
    /**
     * @notice Breed and Train
     * @param _parents uint256 array of parent ids
     * @param _tokenURI string token uri for child
     * @param _childId string childId
     * @param _holdEnum uint256 either 2 or 3
     * @dev TESTING PURPOSE, token URI shouldnt be set as params
     */
    function breed(
        ...
        require(_parents.length == 2, "Requires 2 nft");
        require(_parents[0] != _parents[1], "Identical tokens");

        for (uint256 i = 0; i < 2; i++) {
            /// both sould be held correctly and owned by sender
            if (_holdEnum == 2) {
                require(
                ...
                require(
                    _heldTokens[_parents[i]] == HolderPlace.Training,
                    "Invalid place of token"
                );
            } else {
        ...
        validateBreed(_parents[0], _parents[1]);

        /// mint to NextGen collection
        uint256 childTokenId = _bCollection.mint(
            _msgSender(),
        ...
        emit BreedItemCreated(_parents[0], _parents[1], childTokenId);
        emit BreedMinted(_childId, childTokenId, _tokenURI, _msgSender());
    }
