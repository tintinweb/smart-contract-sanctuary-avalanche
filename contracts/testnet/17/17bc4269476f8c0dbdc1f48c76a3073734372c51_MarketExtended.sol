...
import "../MarketTieredLeverage.sol";
import "../MarketExtended.sol";

contract MarketExtended is MarketExtendedCore, IMarketExtended {
  constructor(address _paymentToken, IRegistry _longShort) MarketExtendedCore(_paymentToken, _longShort) {}
  ...
  /// @notice Purely a convenience function to get the pool token token address. Used in testing.
  function getMiaAddress(IMarketCommon.PoolType poolType, uint256 index) external view returns (address) {
  ...
  /// @notice Returns batched redeem amount in pool token token for even numbered epochs.
  /// @param poolType indicates either Long or Short pool type
  /// @param pool index of the pool on the market side
  ...
  /// @notice Returns batched redeem amount in pool token token for odd numbered epochs.
  /// @param poolType indicates either Long or Short pool type
  /// @param pool index of the pool on the market side
...
contract Market is MarketTieredLeverageCore, IMarketTieredLeverage {
  ...
  /// @notice Returns the redeem action in pool token tokens of provided user for the given poolType and poolTier.
  /// @dev Action amounts have a fixed 18 decimals.
  /// @param user Address of the user.
  /// @param poolType an enum representing the type of poolTier for eg. LONG or SHORT.
  /// @param poolTier The index of the pool in the side.
  ...
  /// @notice Returns the price of the pool token token given poolType and poolTier.
  /// @dev Prices have a fixed 18 decimals.
  /// @param epoch Number of epoch that has been executed.
  /// @param poolType an enum representing the type of poolTier for eg. LONG or SHORT.
  /// @param poolTier The index of the pool in the side.
  /// @return poolToken_priceSnapshot Price of the pool token tokens in the pool.
...
contract MarketTieredLeverageLogic is AccessControlledAndUpgradeableModifiers, IMarketCommon, IMarketTieredLeverageCore, MarketStorage {
  ...
  IRegistry immutable _registry; // original core contract
  address immutable _gems;
  address public immutable paymentToken;

  constructor(address _paymentToken, IRegistry registry) {
    if (_paymentToken == address(0)) revert InvalidAddress({invalidAddress: _paymentToken});
    if (address(registry) == address(0)) revert InvalidAddress({invalidAddress: address(registry)});

    paymentToken = _paymentToken;

    _registry = registry; // original core contract
    _gems = registry.gems();
  }
  ...
  /// @param fundingRateMultiplier_e18 A scalar base e18 for the funding rate.
  ...
  function _calculateFundingAmount(
    ...
    */
    fundingAmount = ((overbalancedValue - underbalancedValue) * fundingRateMultiplier_e18 * oracleManager.EPOCH_LENGTH()) / SECONDS_IN_A_YEAR_e18;
  }

  /// @notice Returns the effective liquidity shift between long and short sides due to observed price change.
  /// @dev gets the value transfer from short to long (positive is a gain for long, negative is a gain for short) - Look at the `getEffectiveValueChangeReferenceImplementation` function for an un-optimized and easier to read version of this function.
  ...
  function _getEffectiveValueChange(
    uint256 effectiveValueLong,
    uint256 effectiveValueShort,
    int256 previousPrice,
    int256 currentPrice
  ) internal view virtual returns (int256 effectiveValueChange) {
    ...
    if (priceMovement > maxPercentChange) {
      priceMovement = maxPercentChange;
    } else if (priceMovement < -maxPercentChange) {
      priceMovement = -maxPercentChange;
    }

    effectiveValueChange = (priceMovement * int256(Math.min(effectiveValueShort, effectiveValueLong))) / 1e18;

    //  slow drip interest funding payment here.
    if (effectiveValueLong < effectiveValueShort) {
      effectiveValueChange += int256(_calculateFundingAmount(fundingRateMultiplier_e18, effectiveValueShort, effectiveValueLong));
    } else {
      effectiveValueChange -= int256(_calculateFundingAmount(fundingRateMultiplier_e18, effectiveValueLong, effectiveValueShort));
    }

    // funding should instead to be done in pools and sent to tranche.
  }

  /// @notice Reblances the pool given the epoch execution information and can also perform batched actions from the epoch.
  /// @param epochPrice The oracle price for the epoch.
  /// @param epochIndex The index for the epoch being processed.
  /// @param totalEffectiveLiquidityPoolType effective liquidity array for long & short sides
  /// @param effectiveValueChange movement of effective liquidity between the sides
  function _rebalancePoolsAndExecuteBatchedActions(
    int256 epochPrice, // NOTE: this argument is ONLY used for the event - might be possible to optimize
    uint32 epochIndex,
    uint128[2] memory totalEffectiveLiquidityPoolType,
    int256 effectiveValueChange
  ) internal returns (uint128[2] memory nextTotalEffectiveLiquidityPoolType) {
    PoolState[] memory poolStates = new PoolState[](_totalNumberOfPools);
    ...
    for (uint8 poolType = uint8(IMarketCommon.PoolType.SHORT); poolType <= uint8(IMarketCommon.PoolType.FLOAT); poolType++) {
      for (uint8 poolTier = 0; poolTier < _numberOfPoolsOfType[poolType]; poolTier++) {
        int256 poolValue = int256(pools[IMarketCommon.PoolType(poolType)][poolTier].value);
        IMarketCommon.PoolFixedConfig memory poolFixedConfig = pools[IMarketCommon.PoolType(poolType)][poolTier].fixedConfig;

        // calculate funding and pay it here. Long and Short pools pay it. Float pool recieves it.

        int256 effectiveLiquidityForPoolTier_1e18 = (poolValue * poolFixedConfig.leverage);

        poolValue +=
          (effectiveLiquidityForPoolTier_1e18 * effectiveValueChange) /
          (
            int256(
              uint256(
                totalEffectiveLiquidityPoolType[uint256(poolFixedConfig.leverage > 0 ? IMarketCommon.PoolType.LONG : IMarketCommon.PoolType.SHORT)]
              ) * 1e18
            )
          );
        ...
        poolValue += _processAllBatchedEpochActions(epochIndex, IMarketCommon.PoolType(poolType), poolTier, price, poolFixedConfig.token);

        if (IMarketCommon.PoolType(poolType) == IMarketCommon.PoolType.LONG) {
          nextTotalEffectiveLiquidityPoolType[uint256(IMarketCommon.PoolType(poolType))] += uint128(
            (uint256(poolValue) * uint256(int256(poolFixedConfig.leverage))) / 1e18
          ); // this isn't needed to be written for the floating tranche
        } else if (IMarketCommon.PoolType(poolType) == IMarketCommon.PoolType.SHORT) {
          nextTotalEffectiveLiquidityPoolType[uint256(IMarketCommon.PoolType(poolType))] += uint128(
            (uint256(poolValue) * uint256(int256(-poolFixedConfig.leverage))) / 1e18
          ); // this isn't needed to be written for the floating tranche
        }

        pools[IMarketCommon.PoolType(poolType)][poolTier].value = uint256(poolValue);

        poolToken_priceSnapshot[epochIndex][IMarketCommon.PoolType(poolType)][poolTier] = price;

        poolStates[currentPoolStateIndex] = PoolState({
          poolId: packPoolId(IMarketCommon.PoolType(poolType), poolTier),
          tokenSupply: tokenSupply,
          value: uint256(poolValue)
        });
        currentPoolStateIndex++;
      }
    }

    if (_numberOfPoolsOfType[uint8(IMarketCommon.PoolType.FLOAT)] != 0) {
      uint256 shortEffectiveLiquidity = nextTotalEffectiveLiquidityPoolType[uint256(IMarketCommon.PoolType.SHORT)];
      uint256 longEffectiveLiquidity = nextTotalEffectiveLiquidityPoolType[uint256(IMarketCommon.PoolType.LONG)];
      // now we need to set the floating tranche and adjust the totalEffectiveLiquidity accordingly.
      int256 newLeverage = ((int256(shortEffectiveLiquidity) - int256(longEffectiveLiquidity)) * 1e18) /
        int256(pools[IMarketCommon.PoolType.FLOAT][0].value);

      if (newLeverage > 5e18) {
        newLeverage = 5e18;
      } else if (newLeverage < -5e18) {
        newLeverage = -5e18;
      }

      // set of its long or short... and its leverage
      pools[IMarketCommon.PoolType.FLOAT][0].fixedConfig.leverage = int96(newLeverage);

      // this dirty thing now adjust the next total effective liquidity to be correct.
      nextTotalEffectiveLiquidityPoolType[uint256(newLeverage > 0 ? IMarketCommon.PoolType.LONG : IMarketCommon.PoolType.SHORT)] += uint128(
        (uint256(pools[IMarketCommon.PoolType.FLOAT][0].value) * uint256(int256(newLeverage > 0 ? newLeverage : -newLeverage))) / 1e18
      );
    }

    emit SystemUpdateInfo(epochIndex, uint256(epochPrice), poolStates);
  }
  ...
  function updateSystemStateUsingValidatedOracleRoundIds(uint80[] memory oracleRoundIdsToExecute) external checkMarketNotDeprecated {
    ...
    // TODO: rather just inline `epochPrices.length` (check gas before and after for curiosity)
    ...
    for (uint256 i = 0; i < numberOfEpochsToExecute; i++) {
      int256 effectiveValueChange = _getEffectiveValueChange(
      ...
      totalEffectiveLiquidityPoolType = _rebalancePoolsAndExecuteBatchedActions(
        epochPrices[i],
        epochInfo.latestExecutedEpochIndex + 1 + uint32(i),
    ...
    EpochInfo memory _epochInfo = EpochInfo({
      latestExecutedEpochIndex: epochInfo.latestExecutedEpochIndex + uint32(numberOfEpochsToExecute),
  ...
  {
    // Due to _getAmountPoolToken we must have amount * 1e18 > poolTokenPriceInPaymentTokens
    //   otherwise we get 0
    ...
    // TODO: sholud this function not return a uint32 (rather than a uint256) since it is always a timestamp?
    ...
    emit Deposit(packPoolId(poolType, uint8(poolTier)), amount, user, currentEpoch);
  }
  ...
  function mintFloatPool(uint112 amount) external {
    _mint(amount, msg.sender, IMarketCommon.PoolType.FLOAT, 0);
  }
  ...
  function mintFloatPoolFor(uint112 amount, address user) external override {
    _mint(amount, user, IMarketCommon.PoolType.FLOAT, 0);
  }
  ...
  function _redeem(
    ...
    IMarketCommon.UserAction memory userAction = userAction_redeemPoolToken[user][poolType][poolTier];
  ...
  function redeemFloatPool(uint112 amount) external {
    _redeem(amount, msg.sender, IMarketCommon.PoolType.SHORT, 0);
  }
  ...
  function settlePoolUserRedeems(
    ...
    if (userAction.amount > 0 && userAction.correspondingEpoch <= epochInfo.latestExecutedEpochIndex) {
      ...
      emit ExecuteEpochSettlementRedeemUser(packPoolId(poolType, uint8(poolTier)), user, epochInfo.latestExecutedEpochIndex);
    }
  }
  ...
  /// @notice Either mints or burns pool token token supply.
  /// @param poolToken Address of the pool token token.
  ...
  /// @param poolType an enum representing the type of poolTier for eg. LONG or SHORT.
  /// @param poolTier leveraged poolTier index.
  /// @param price Price of the pool token token.
  function _processAllBatchedEpochActions(
    ...
    IMarketCommon.Pool storage pool = pools[poolType][poolTier];

    IMarketCommon.BatchedActions memory batch = pool.batchedAmount[associatedEpochIndex & 1];
    ...
    if (batch.paymentToken_deposit > 0 || batch.poolToken_redeem > 0) {
      ...
      pool.batchedAmount[associatedEpochIndex & 1] = IMarketCommon.BatchedActions(0, 0);
  ...
  function _deprecateMarket() internal {
    ...
    _rebalancePoolsAndExecuteBatchedActions(0, epochInfo.latestExecutedEpochIndex + 1, effectiveLiquidityForPoolType, 0);
    _rebalancePoolsAndExecuteBatchedActions(0, epochInfo.latestExecutedEpochIndex + 2, effectiveLiquidityForPoolType, 0);
  ...
  /// @dev 10 days should be enough time for the team to make an informed desision on how to handle this error.
  ...
  /// @notice Allows users to exit the market after it has been deprecated
  function exitDeprecatedMarket(address user) public {
  ...
  function exitDeprecatedMarket() external {
    exitDeprecatedMarket(msg.sender);
  }
}
...
contract MarketExtendedCore is AccessControlledAndUpgradeableModifiers, MarketStorage, IMarketExtendedCore {
  ...
  // Currently unused.
  event TierAdded(IMarketExtended.SinglePoolInitInfo newTier, uint256 initialSeed);
  ...
  function get_total_value_realized_in_market() external view returns (uint256 totalValueRealizedInMarket) {
    for (uint8 poolType = uint8(IMarketCommon.PoolType.SHORT); poolType <= uint8(IMarketCommon.PoolType.LONG); poolType++) {
      ...
      for (uint256 poolTier = 0; poolTier < maxPoolTier; poolTier++) {
        totalValueRealizedInMarket += pools[IMarketCommon.PoolType(poolType)][poolTier].value;
      }
    }
  }

  modifier registryOnly() {
    require(msg.sender == address(registry), "Not LongShort");
    _;
  }
    ...
    returns (bool initializationSuccess)
  {
    ...
    // maxPercentChange = 1999e14; // Ie default max percentage change is 19.99% (for the 5x FLOAT tier)
  ...
  function changeMarketFundingRateMultiplier(uint256 _fundingRateMultiplier_e18) external adminOnly {
    ...
    emit ConfigChange(ConfigChangeType.fundingRateMultiplier, previousFundingRateMultiplier_e18, fundingRateMultiplier_e18);
  }
  ...
  function _addPoolToExistingMarket(
    ...
    require(_numberOfPoolsOfType[uint256(initPool.poolType)] < 8, "Maximum number of pools for side exceeded");
    require(seederAndAdmin != address(0));
    require(initPool.token != address(0), "Pool token address incorrect");
    require(
      (initPool.leverage >= 1e18 && initPool.leverage <= 5e18) || initPool.poolType == IMarketCommon.PoolType.FLOAT,
      "Pool leverage out of bounds"
    );

    IMarketExtended.SinglePoolInitInfo memory poolInfo = initPool;
    ...
    uint256 tierPriceMovementThresholdAbsolute = IMarketCommon.PoolType.FLOAT == initPool.poolType ? 0.1999e18 : (1e36 / poolInfo.leverage) - 1e14;
    ...
    IMarketCommon.Pool storage pool = pools[initPool.poolType][_numberOfPoolsOfType[uint256(initPool.poolType)]];
    // TODO: pass in leverage as a uint96 type and confirm last type.
    if (IMarketCommon.PoolType.FLOAT == initPool.poolType) {}
    pool.fixedConfig = IMarketCommon.PoolFixedConfig(
      initPool.token,
      (initPool.poolType == IMarketCommon.PoolType.SHORT ? -int96(initPool.leverage) : int96(initPool.leverage))
    );
    ...
    if (initPool.poolType != IMarketCommon.PoolType.FLOAT)
...
contract MarketStorage is IMarketCommon {
  ...
  IMarketCommon.EpochInfo internal epochInfo;
...
interface IMarketTieredLeverageCore is IAccessControlledAndUpgradeable {
  ...
  event Deposit(uint8 indexed poolId, uint112 depositAdded, address indexed user, uint32 indexed epoch);

  event Redeem(uint8 indexed poolId, uint256 synthRedeemed, address indexed user, uint32 indexed epoch);

  // TODO URGENT: think of edge-case where this is in EWT. Maybe just handled by the backend.
  event ExecuteEpochSettlementMintUser(uint8 indexed poolId, address indexed user, uint32 indexed epochSettledUntil);

  event ExecuteEpochSettlementRedeemUser(uint8 indexed poolId, address indexed user, uint32 indexed epochSettledUntil);
  ...
  event SystemUpdateInfo(uint32 indexed epoch, uint256 underlyingAssetPrice, PoolState[] poolStates);
...
interface IMarketTieredLeverageView {
  ...
  // TODO: create functions for get_pool_leverage
  // TODO: create functions for get_pool_batchedActions
  // TODO: create functions for get_pool_token
...
interface IMarketTieredLeverage is IMarketTieredLeverageCore, IMarketTieredLeverageView {}
