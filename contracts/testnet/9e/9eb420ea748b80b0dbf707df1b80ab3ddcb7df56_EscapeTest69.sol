...
contract EscapeTest69 is ERC1155, Ownable {
  ...
  uint public MAX_QTY_for_EDITION = 10;
  //sets the max tokens for current range mint function
  uint public MAX_QTY_for_RANGE = 10;
  //sets price for single edition mint function
  uint public PRICE_EDITION_MINT_PRICE = 0.005 ether;
  //sets price for range function
  uint public PRICE_RANGE_MINT_PRICE = 0.005 ether;
...
//sets the highest token allowed in range
  ...
  uint public CURRENT_EDITION = 1;
//sets the max per wallet tracked using Edition Mints Num 1-10 wallet tracker spots
  uint public MAX_PER_WALLET_EDITION_MINT = 3;
//sets the current wallet tracking memory slot lists used
  uint public CURRENT_WALLET_TRACK_LIST = 1;

  uint public MAX_PER_WALLET_RANGE_MINT = 3;

  uint public CURRENT_WALLET_TRACK_LIST_RANGE = 1;
  ...
  //if not using all slots- user in chronical order (1, 2, etc) then set the others to the SAME tokens & turn off the other supplys to 0 or match it
  uint public E1 = 1;
  uint public E2 = 2;
  uint public E3 = 3;
  uint public E4 = 4;
...
//sets price of each 10 current IDs minting
  ...
  // 0: Closed
  // 1: Open 
  uint256 public SALE_STATUS_MINT_EDITION_SALE = 0;
  uint256 public SALE_STATUS_MINT_RANGE_SALE = 0;
  uint256 public SALE_STATUS_MINT_MULTI_SALE = 0;
  uint256 public SALE_STATUS_GATED_EDITION_SALE = 0;
  uint256 public SALE_STATUS_GATED_RANGE_SALE = 0;


  string private _contractUri = "ipfs://";

  string public name = "EscapeTest69";
  string public symbol = "ETEST69";

 mapping (address => uint256) public FLEX_ALLOWED_PER_WALLET_EDITION_MINT;
...
//10 wallet tracking sales - then move to manual toggles (costs gas) to load the specific arrays
  ...
  constructor() public ERC1155("ipfs://QmdALFtSP96bt99Kze54AbZxokk2XXFzsVcZmfwbgHjP26/{id}.json") {}
...
//allows minting of only 1 specific edition at a set max supply- turn all other sales off and only use this for specific exact control
  function mintEDITION(uint EditionNum, uint amount) public payable {
    require(SALE_STATUS_MINT_EDITION_SALE == 1, "edition mint is not open");
...
//allows gating of the single edition mint function
  function GATEDmintEDITION(uint EditionNum, uint amount,bytes32[] calldata MerkleProofEdition) public payable {
    require(SALE_STATUS_GATED_EDITION_SALE == 1, "edition mint is not open");
  ...
  function mintRANGE(uint EditionNum, uint amount) public payable {
    require(SALE_STATUS_MINT_RANGE_SALE == 1, "range edition mint is not open");
   ...
   function GATEDmintRANGE(uint EditionNum, uint amount,bytes32[] calldata MerkleProofRange) public payable {
    require(SALE_STATUS_GATED_RANGE_SALE == 1, "range edition mint is not open");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
  ...
  function mintCLAIMfromAvail(uint EditionNum, uint amount) public payable {
    require(SALE_STATUS_MINT_MULTI_SALE == 1, "multi sale from available mint is not open");
  ...
  function _internalMint(uint EditionNum, uint amount) internal {
    ...
    if (EditionNum >= 0) {
      checkPayment(PRICE_EDITION_MINT_PRICE * amount);
...
function _checkEdition(uint256 amount, uint EditionNum) internal {
    require(EditionNum == CURRENT_EDITION, "wrong edition");
    require(amount > 0 && amount <= MAX_QTY_for_EDITION, "more than edition max allowed");
    MAX_QTY_for_EDITION -= amount;
    _checkCURRENT_WALLET_TRACK_LIST(amount);
  
  }

function _checkCURRENT_WALLET_TRACK_LIST(uint256 amount) internal {
  if (CURRENT_WALLET_TRACK_LIST == 1){
    NumMinted1[msg.sender] = NumMinted1[msg.sender] + amount;
        require(NumMinted1[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 2){
    NumMinted2[msg.sender] = NumMinted2[msg.sender] + amount;
        require(NumMinted2[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 3){
    NumMinted3[msg.sender] = NumMinted3[msg.sender] + amount;
        require(NumMinted3[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 4){
    NumMinted4[msg.sender] = NumMinted4[msg.sender] + amount;
        require(NumMinted4[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 5){
    NumMinted5[msg.sender] = NumMinted5[msg.sender] + amount;
        require(NumMinted5[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 6){
    NumMinted6[msg.sender] = NumMinted6[msg.sender] + amount;
        require(NumMinted6[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 7){
    NumMinted7[msg.sender] = NumMinted7[msg.sender] + amount;
        require(NumMinted7[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 8){
    NumMinted8[msg.sender] = NumMinted8[msg.sender] + amount;
        require(NumMinted8[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 9){
    NumMinted9[msg.sender] = NumMinted9[msg.sender] + amount;
        require(NumMinted9[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  else if (CURRENT_WALLET_TRACK_LIST == 10){
    NumMinted10[msg.sender] = NumMinted10[msg.sender] + amount;
        require(NumMinted10[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance");
  }
  //choose 0-this allows use of the ADJUSTABLE FLEX max per wallet adjustable qtys different PER WALLET address 
  //Reset NumMinted_FLEX_editions to zero before re-using
  else if (CURRENT_WALLET_TRACK_LIST == 0){
    NumMinted_FLEX_Edition[msg.sender] = NumMinted_FLEX_Edition[msg.sender] + amount;
    require(NumMinted_FLEX_Edition[msg.sender] <= FLEX_ALLOWED_PER_WALLET_EDITION_MINT[msg.sender], "Cannot mint more than your allowance")
  ;}
  //choose 69-this allows use of the ADJUSTABLE FLEX max per wallet - standard across all wallets
  ////Reset NumMinted_FLEX_editions to zero before re-using
  else if (CURRENT_WALLET_TRACK_LIST == 69){
    NumMinted_FLEX_Edition[msg.sender] = NumMinted_FLEX_Edition[msg.sender] + amount;
    require(NumMinted_FLEX_Edition[msg.sender] <= MAX_PER_WALLET_EDITION_MINT, "Cannot mint more than your allowance")
  ;}
  else {
  ...
  function _checkEditionRange(uint256 amount, uint EditionNum) internal {
    ...
    require(amount > 0 && amount <= MAX_QTY_for_RANGE, "more than edition range allowed");
  ...
  function _checkCURRENT_WALLET_TRACK_LIST_RANGE(uint256 amount) internal {
    if (CURRENT_WALLET_TRACK_LIST_RANGE == 0){
    NumMinted_FLEX_RANGE[msg.sender] = NumMinted_FLEX_RANGE[msg.sender] + amount;
    require(NumMinted_FLEX_RANGE[msg.sender] <= FLEX_ALLOWED_PER_WALLET_RANGE_MINT[msg.sender], "Cannot mint more than your allowance")
  ;}
  //choose 69-this allows use of the ADJUSTABLE FLEX max per wallet - standard across all wallets
  ////Reset NumMinted_FLEX_ranges to zero before re-using
  else if (CURRENT_WALLET_TRACK_LIST_RANGE == 69){
    NumMinted_FLEX_RANGE[msg.sender] = NumMinted_FLEX_RANGE[msg.sender] + amount;
    require(NumMinted_FLEX_RANGE[msg.sender] <= MAX_PER_WALLET_RANGE_MINT, "Cannot mint more than your allowance")
  ;}
  else {
      revert("Invalid Wallet Tracker");
    }
  ...
  function setSALE_STATUS_MINT_EDITION_SALE(uint newStateEditionSale) public onlyOwner {
    require(newStateEditionSale >= 0 && newStateEditionSale <= 1, "Invalid state");
    SALE_STATUS_MINT_EDITION_SALE = newStateEditionSale;
  }
  ...
  function set_PRICE_EDITION_MINT_PRICE(uint newEditionPrice) public onlyOwner {
    PRICE_EDITION_MINT_PRICE = newEditionPrice;
  }

  function setPRICE_RANGE_MINT_PRICE(uint newPRICE_RANGE_MINT_PRICE) public onlyOwner {
  ...
  function set_MAX_QTY_for_EDITION_Toggle(uint newMAXeditionQTY) public onlyOwner {
    MAX_QTY_for_EDITION = newMAXeditionQTY;
  }
  ...
  function set_MAX_QTY_for_RANGE_Toggle(uint newMAXrangeQty) public onlyOwner {
    MAX_QTY_for_RANGE = newMAXrangeQty;
  }

  function setCURRENT_EDITION(uint newCURRENT_Edition) public onlyOwner {
    CURRENT_EDITION = newCURRENT_Edition;
  }

  function setMAX_PER_WALLET_EDITION_MINT(uint newMAX_PER_WALLET_EDITION_MINT) public onlyOwner {
    MAX_PER_WALLET_EDITION_MINT = newMAX_PER_WALLET_EDITION_MINT;
  }
  ...
  function setMerkle1(bytes32 newMerkle1) public onlyOwner {
    ...
    function setCURRENT_WALLET_TRACK_LIST(uint newCURRENT_WALLET_TRACK_LIST) public onlyOwner {
    CURRENT_WALLET_TRACK_LIST = newCURRENT_WALLET_TRACK_LIST;
    }

     function setSALE_STATUS_GATED_EDITION_SALE(uint newStateGatedEditionSale) public onlyOwner {
    require(newStateGatedEditionSale >= 0 && newStateGatedEditionSale <= 1, "Invalid state");
    SALE_STATUS_GATED_EDITION_SALE = newStateGatedEditionSale;
  }

   function setSALE_STATUS_GATED_RANGE_SALE(uint newStateGatedRangeSale) public onlyOwner {
    require(newStateGatedRangeSale >= 0 && newStateGatedRangeSale <= 1, "Invalid state");
    SALE_STATUS_GATED_RANGE_SALE = newStateGatedRangeSale;
  }

  function setSALE_STATUS_MINT_RANGE_SALE(uint newStateRangeSale) public onlyOwner {
    require(newStateRangeSale >= 0 && newStateRangeSale <= 1, "Invalid state");
    SALE_STATUS_MINT_RANGE_SALE = newStateRangeSale;
  }

  function setSALE_STATUS_MINT_MULTI_SALE(uint newStateMultiSale) public onlyOwner {
    require(newStateMultiSale >= 0 && newStateMultiSale <= 1, "Invalid state");
    SALE_STATUS_MINT_MULTI_SALE = newStateMultiSale;
  }

  function editFLEX_ALLOWED_PER_WALLET_EDITION_MINT(address[] memory _a, uint256[] memory _amount) public onlyOwner {
    for(uint256 i; i < _a.length; i++){
    FLEX_ALLOWED_PER_WALLET_EDITION_MINT[_a[i]] = _amount[i];
    }
  }
  ...
  function burnTokenfor(uint256 typeId, address burnTokenAddress)
    ...
    function SetBurnerEscapeContractAddress(address BurnerEscapeContractAddress)
        external
        onlyOwner
    {
