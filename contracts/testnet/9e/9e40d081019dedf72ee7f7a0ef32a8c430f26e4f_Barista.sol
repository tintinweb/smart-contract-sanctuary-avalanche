/**
 *Submitted for verification at testnet.snowtrace.io on 2022-07-15
*/
...
contract Coffee is ERC20("Coffee", "COFFEE"), Ownable {
    address shopAddress;
    address baristaAddress;
    ...
    function setBaristaAddress(address _baristaAddress) external onlyOwner {
        require(address(baristaAddress) == address(0), "Barista address already set");
        baristaAddress = _baristaAddress;
    }
    ...
    function burn(address _from, uint256 _amount) external {
        require(_msgSender() == baristaAddress, "Only the Barista contract can burn");
        _burn(_from, _amount);
    }
...
contract Barista is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant BASE_MINT_PRICE = 0.1 ether; 
    uint256 public constant PRESALE_MINT_PRICE = 0.6 ether;  
    uint256 public constant BASE_FEMALE_PERCENTAGE = 15;
    ...
    uint256 public femaleSupply;
    uint256 public maleSupply;

    mapping(uint256 => bool) private isFemale;
    ...
    constructor() ERC721("Coffee Barista", "COFFEE-BARISTA") {
        // supply and price are ignored for the base levels
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 1 })); // regular barista
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            if (_rand(totalSupply()) % 100 < BASE_ALIEN_PERCENTAGE) {
                ...
                tokenLevel[tokenId] = 0; // normal barista
            } 

            uint256 seed = randomizer.randomSeed(tokenId);

            if ((seed & 0xFFFF) % BASE_FEMALE_PERCENTAGE != 0) {
                isFemale[tokenId] = false;
                maleSupply++;
            } else {
                isFemale[tokenId] = true;
                femaleSupply++;
            }
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(coffee.balanceOf(msg.sender) >= totalCost, "Insufficient COFFEE balance");
        coffee.burn(msg.sender, totalCost);
    ...
    function revealedTokenIsFemale(uint256 _tokenId) public view returns (bool) {
        require(tokenRevealed(_tokenId), "Token unrevealed");

        return isFemale[_tokenId];
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if (!tokenRevealed(_tokenId)) {
            return string(abi.encodePacked("unrevealed"));
        } else if (revealedTokenIsFemale(_tokenId)){
            return string(abi.encodePacked("female"));
        } else {
            return string(abi.encodePacked("male"));
        }
    ...
    function batchedBaristasOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (BaristaInfo[] memory) {
        if (_offset >= balanceOf(_owner)) {
            return new BaristaInfo[](0);
        }
        ...
        BaristaInfo[] memory baristas = new BaristaInfo[](outputSize);
    ...
    function setCoffee(Coffee _coffee) external onlyOwner {
        coffee = _coffee;
    }
...
contract Shop is Ownable {
    ...
    Barista public barista;
    Coffee public coffee;
    address public pantryAddress;

    uint256 public constant YIELD_CPS = 16666666666666667; // coffees baked per second per unit of yield
    uint256 public constant CLAIM_COFFEE_TAX_PERCENTAGE = 20;
    ...
    constructor(Barista _barista, Coffee _coffee, address _pantryAddress) {
        barista = _barista;
        coffee = _coffee;
        pantryAddress = _pantryAddress;
    }
    ...
    function getCoffeesAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory coffeeAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            coffeeAmounts[i] = _getCoffeesAccruedFor(_tokenIds[i], false);
        }
        return coffeeAmounts;
    }

    function _getCoffeesAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
        ...
        return (block.timestamp - stake.startTimestamp) * barista.tokenYield(_tokenId) * YIELD_CPS;
    }

    /* Mutators */

    function stakeMany(uint256[] calldata _tokenIds) external {
        require(barista.gameStarted(), "The game has not started");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(barista.ownerOf(tokenId) == _msgSender(), "You don't own this token");
            require(barista.tokenRevealed(tokenId), "Token not yet revealed");

            barista.transferFrom(_msgSender(), address(this), tokenId);
            _addBaristaToShop(tokenId, _msgSender());
        }
    }

    function claimCoffeesAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 coffees = _getCoffeesAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (coffees * CLAIM_COFFEE_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += coffees - taxAmount;
            totalTaxed += taxAmount;
            stakes[tokenId].startTimestamp = block.timestamp;

            if (unstake) {
                _moveBaristaToCooldown(tokenId);
            }
        }

        coffee.mint(_msgSender(), totalClaimed);
        coffee.mint(pantryAddress, totalTaxed);
    }

    function withdrawBaristas(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            ...
            barista.transferFrom(address(this), _msgSender(), tokenId);
            _removeBaristaFromCooldown(tokenId);
        }
    }

    function _addBaristaToShop(uint256 _tokenId, address _owner) internal {
    ...
    function _moveBaristaToCooldown(uint256 _tokenId) internal {
    ...
    function _removeBaristaFromCooldown(uint256 _tokenId) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                tokenId: tokenId,
                level: barista.revealedTokenLevel(tokenId),
                accrual: _getCoffeesAccruedFor(tokenId, false)
            });
        }
