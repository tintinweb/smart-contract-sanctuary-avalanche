...
contract MembershipNFT is ERC721Burnable, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public treasureFee = 5;

    mapping(uint => MembershipType) membershipTypes;

    // Enable/disable mint, transfer
    ...
    // Public mint
    function _mintPublic() public payable nonReentrant {
        require(isMintActive, "Mint inactiave");
        require(balanceOf(msg.sender) < maxPerAccount, "account already have enough amount");
        require(_tokenIdCounter.current() < maxSupply, "Max supply exceeded");
        require(msg.value >= price, "Not enough ether");

        payable(treasureAddress).transfer(msg.value.mul(treasureFee).div(100));

        _mint(msg.sender);
        membershipTypes[_tokenIdCounter.current()] = MembershipType.PAID;
        emit Mint(msg.sender, _tokenIdCounter.current());
    }

    // Whitelisted wallet can only mint 1 during whitelist
    function mintFreesale(bytes32[] calldata proof) public {
        require(isMintActive, "Mint inactiave");
        require(balanceOf(msg.sender) < maxPerAccount, "Account already have enough amount");
        require(freeSupply.current() < freeMaxSupply, "Free sale ended");
        require(_tokenIdCounter.current() < maxSupply, "Max supply exceeded");
        require(
            MerkleProof.verify(
                proof,
        ...
        _mint(msg.sender);
        membershipTypes[_tokenIdCounter.current()] = MembershipType.FREE;
        freeSupply.increment();
        emit Mint(msg.sender, _tokenIdCounter.current());
    }
    ...
    function tokenURI(uint tokenId) public view override returns (string memory){
        ...
        return string(abi.encodePacked(baseURI(), membershipTypes[tokenId] == MembershipType.FREE ? "free" : "paid", "/", uint2str(tokenId)));
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(_exists(tokenId)) {
            require(isTransferActive, "Transfer inactive");
            require(balanceOf(to) < maxPerAccount, "Receiver already have enough amount");
        }
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function setFreeMaxSupply(uint256 _freeMaxSupply) public onlyOwner {
    ...
    function setTreasureFee(uint256 _treasureFee) public onlyOwner {
