...
// File contracts/NFT/ERC721/ERC721NFTCollectionImpl.sol
    ...
    INFTCollection
{
    ...
    function mint(
        ...
        if (mode == Mode.Private) {
            ...
            if (getNumberOfRemainingFreeSpots(_msgSender()) < freeMintCount)
                revert RequestedFreeMintCountExceedsTotal();
            if (
                getNumberOfRemainingGuaranteedSpots(_msgSender()) <
                guaranteedMintCount
            ) revert RequestedGuaranteedMintCountExceedsTotal();
            if (getNumberOfRemainingRaceSpots(_msgSender()) < raceMintCount)
                revert RequestedRaceMintCountExceedsTotalForUser();
            ...
            for (uint256 i = 0; i < user.spots.length; i++) {
                if (free <= freeMintCount && user.spots[i] == freeSpot) {
                    whitelistedUsers[_msgSender()].mintedSpots[i] = true;
                    free++;
                } else if (
                    // todo: change for discounts, not MVP
                    guaranteed <= guaranteedMintCount &&
                    user.spots[i] == guaranteedWithNoDiscountSpot
                ) {
                    whitelistedUsers[_msgSender()].mintedSpots[i] = true;
                    guaranteed++;
                } else if (race <= raceMintCount && user.spots[i] == raceSpot) {
                    whitelistedUsers[_msgSender()].mintedSpots[i] = true;
                    race++;
                    totalRaceMinted++;
                }
