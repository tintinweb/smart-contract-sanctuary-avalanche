...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    function open(
        ...
        uint256 _openCoefficient = 0;
        if(!hasRole(OPEN_ROLE, sender)) {
            IBetUpDAOToken(betUpDaoToken).transferFrom(sender, address(this), openCoefficient);
        ...
        pairs[pairIDCounter].openAmount = _openCoefficient;
    ...
    function close(uint256 _pairID,  uint16 _resultID, bool _isInvalid) external {
        ...
        if(nextResultCount * 10e3 >= pair.resolverQty * 2 *  10e3 / 3 ) {
            pairs[_pairID].resultID =  __resultID;
        }
    ...
    function claimCreationRewards(uint256 _pairID) external {
        ...
        if(pair.paused) revert PairPaused(_pairID);
        require(pair.resultID > 0 && pair.resultID < 2**16-2, "ongoing");
        ...
        uint256 fee = (pair.total - op) * 10 * pair.creationRatio/1000000 ;

        require(fee > 0, "No rewards available");

        pairs[_pairID].claimedReward = true;

//        console.log(pair.total);
//        console.log(op);
//        console.log(pair.creationRatio);
//        console.log(fee);
//        console.log(pair.openAmount);
        IERC20MetadataUpgradeable(pair.chip).safeTransfer(receiver, fee);
        if(pair.openAmount > 0) IERC20MetadataUpgradeable(betUpDaoToken).safeTransfer(receiver, pair.openAmount);
    ...
    function setResultID(uint256 _pairID, uint16 _resultID) external  onlyRole(GOVERNANCE_ROLE) {
        Pair memory pair = pairs[_pairID];
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint256 op = optionsVolume[keccak256(abi.encodePacked(pairID, pair.resultID))];
            uint256 availableAmount = pair.total - op - ((pair.total - op) * 10 * pair.creationRatio/1000000) ;
...
//        console.log(1, refund, reward);
        if(pair.resultID == 2**16-1) {
            refund = playerAmount;
        }else if(pair.resultID > 0 ) {
