...
import { GemUtils } from "./GemUtils.sol";
import "hardhat/console.sol";

// TODO: re-add external actor to allow for external crafting ??

contract Gems is ERC1155, Ownable, Withdrawable {
  uint256 public mintCap = 1000; // TODO: Set before release
  uint256 public mintPrice = 0.01 ether; // TODO: Set before release
  uint256 public limitPerTransaction = 100; // TODO: Set before release
  ...
  uint256 public constant NUM_COLORS = 7;

  uint256[4] public levelWeights = [95, 88, 77, 66];

  bool public isSaleActive = true; // TODO:
  bool public isCraftingActive = false;
  ...
  event MintMany(address indexed owner, uint256 amount, uint256 price);
  event MintSingle(address indexed owner, uint256 tokenId, uint256 amount);

  function setFreeMinters(address[] memory _freeMinters) public onlyOwner {
    for (uint i = 0; i < _freeMinters.length; i++) {
      freeMinters[_freeMinters[i]] = true;
    }
  }

  function mintFree() public payable {
    require(isSaleActive, "SALE_CLOSED");
    require(freeMinters[msg.sender], "NOT_ALLOWED");
    freeMinters[msg.sender] = false;
    safeMintMany(freeMintAmount, 0);
  }
  ...
  function mintMany(uint256 amount) public payable {
    ...
    safeMintMany(amount, totalPrice);
  }

  function safeMintMany(uint256 amount, uint256 totalPrice) internal {
    uint256 newAmount = amountMinted + amount;
    require(newAmount <= mintCap, "CAP_REACHED");

    for (uint256 i = 0; i < amount; i++) {
      uint256 level = GemUtils.getWeightedLevel(block.number, i + 1, levelWeights);
      uint256 color = GemUtils.getRandomColor(block.number, i + 2, NUM_COLORS);
      uint256 tokenId = GemUtils.getTokenId(color, level);

      console.log(level, color);
      _mint(msg.sender, tokenId, 1, "");
      emit MintSingle(msg.sender, tokenId, 1);

      amountMinted++;
    }

    emit MintMany(msg.sender, amount, totalPrice);
  }

  // FIXME: REMOVE
  function mintGemTEST(
    uint256 color,
    uint256 level,
    uint256 amount
  ) public {
  ...
  function combineLevels(uint256 tokenId, uint256 amount) public {
    console.log('- upgradeLevel', tokenId, amount);
    require(isCraftingActive, "CRAFT_OFF");

    uint256 nextLevel = getGemDetails(tokenId).level + 1;
    require(nextLevel <= LEVEL_MAX, "MAX_LVL");

    _burn(msg.sender, tokenId, amount * upgradeGemCost);
    _mint(
      msg.sender,
      GemUtils.getTokenId(
        getGemDetails(tokenId).color,
        nextLevel
      ),
      amount,
      ""
  ...
  function combineColors(
    ...
    // TODO: triple check this function's safety
    require(isCraftingActive, "CRAFT_OFF");

    for (uint256 i = 0; i < NUM_COLORS; i++) {
      _burn(msg.sender, GemUtils.getTokenId(i + 1, level), amount);
    }

    _mint(
      msg.sender,
      GemUtils.getTokenId(8, level),
      amount,
      ""
    );
  }
  ...
  function getGemDetails(uint256 tokenId) public pure returns (Gem memory) {
    uint256 color = tokenId / 10;
    uint256 level = (tokenId - (10 * color));
    return Gem(color, level);
  }
  ...
  function toggleCrafting() public onlyOwner {
    require(!rulesLocked, "RULES_LOCKED");
...
library GetMetadata {
  function getMetadata(
    ...
    (string memory colorCode, string memory colorName) = getColor(color);
    string memory shapeName = getShape(level);
    ...
    if (tokenId == 0) {
      ...
      if (color == 8 && level == 5) {
        description = 'The rarest of all gems.';
      } else if (level == 5) {
        description = 'A rare gem obtained only by the worthy.';
      } else if (color == 8) {
        description = 'They say all the colours are in there.';
      } else {
...
library GemUtils {
  function getTokenId(uint256 color, uint256 level)
    internal
    pure
    returns (uint256)
  {
    return (10 * color) + (1 * level);
  }
