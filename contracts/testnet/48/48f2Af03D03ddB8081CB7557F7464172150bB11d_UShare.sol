...
contract UShare is ERC20Burnable, Operator {
    ...
    // TOTAL MAX SUPPLY = 74 436 USHAREs
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 63936 ether;
    ...
    ) public ERC20("USHARE", "USHARE") {
        _mint(msg.sender, 1 ether); // mint 1 ULYS Share for initial pools deployment
    ...
    function setLP(address _lpAddress) public onlyOperator {
    ...
    function _transferWithTax(
        ...
        if (!excludedAddresses[sender] && !excludedAddresses[recipient]) {
            if (sender == lpAddress || recipient == lpAddress) {
                uint256 taxAmount = amount.mul(200).div(10000);
