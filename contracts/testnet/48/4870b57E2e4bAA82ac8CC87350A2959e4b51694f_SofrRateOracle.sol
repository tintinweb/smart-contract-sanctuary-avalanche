...
contract SofrRateOracle is BaseRateOracle, IRedstoneRateOracle {
    ...
    {
        (, int256 sofrIndex, uint256 sofrIndexTimestamp, , ) = priceFeed.latestRoundData();
        if (sofrIndex <= 0) {
    ...
    {
        timeIn360dayYearsWad = timeInSecondsAsWad.div(SECONDS_IN_360DAY_YEAR_IN_WAD);
    }
        ...
        returns (uint256 apyWad)
    {
        ...
        uint256 timeIn360dayYearsWad = accrualFact360dayYearWad(
            timeInYearsWad.mul(
                FixedAndVariableMath.SECONDS_IN_YEAR_IN_WAD
            )
        );
    ...
    ) public pure override returns (uint256 rateValueRay)
    {
        ...
        uint256 factorInWad = apyFromBeforeOrAtToAtOrAfterWad.mul(timeIn360DayYearsWad) + ONE_IN_WAD;
