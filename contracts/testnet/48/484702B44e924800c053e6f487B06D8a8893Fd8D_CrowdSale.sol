...
abstract contract Crowdsale is Initializable {
    ...
    function _forwardFunds(
        ...
        _USDT.safeTransferFrom(_beneficiary, address(this), _USDTTokenAmount);
    }
     ...
     */
    function _changeSaleToken(IERC20Upgradeable newToken) internal virtual {
        token = newToken;
    }
    ...
    PausableUpgradeable
{
     ...
     */
    function finalize(
        address[] memory _bonusAddress,
        uint256[] memory _aocToken,
        uint256 _usdtValue
    ) public onlyOwner whenNotPaused {
        require(!isFinalized, "Already Finalized");
        require(hasClosed(), "Crowdsale is not yet closed");

        finalBonus(_bonusAddress, _aocToken, _usdtValue);
        finalization();
        emit Finalized();
...
contract CrowdSale is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable {
    ...
    uint256 public minPurchaseInUSDT;
    uint256 public maxPurchaseInUSDT;
    ...
    mapping(address => mapping(uint256 => uint256))
        public _purchasedUSDTAmount;
    mapping(address => uint256) public userEIB; //usdt bonus 
    // mapping(address => mapping(uint256 => uint256)) public userUSDT;
    // mapping(address => mapping(uint256 => uint256)) public userTokenBalance;

    event SetPurchaseLimit(
        uint256 minAmountPerTransaction,
        uint256 maxAmountPerUser
    );
    event UpdateWhitelistingStatus(bool enable);
    event SetReceiveToken(address token);
    ...
    function initialize(
        ...
        _USDT = IERC20Upgradeable(0x1EBB80529EA58933e6F2867A5A29D8c3b2E2Dbb5); // USDT Token

        maxPurchaseInUSDT = 20000 * 10 ** 18;
        minPurchaseInUSDT = 100 * 10 ** 18;
    ...
    function setReceiveTokens(address USDTToken) external onlyOwner {
        ...
        emit SetReceiveToken(USDTToken);
    }
    ...
    function buyToken(
        uint256 _USDTTokenAmount,
        address[] memory EIBAddress,
        uint256[] memory EIBPercentage,
        uint256[] memory phase
    ) external onlyWhileOpen whenNotPaused {
        ...
        uint256 USDTTokenAmount_ = _USDTTokenAmount * 10 ** 18;

        require(
            minPurchaseInUSDT <= USDTTokenAmount_,
            "Need minimum purchase limit"
        );
        require(
            (_purchasedUSDTAmount[msg.sender][round] + USDTTokenAmount_) <=
                maxPurchaseInUSDT,
            "Purchase limit exceed"
        );

        require(
            _USDT.balanceOf(msg.sender) >= USDTTokenAmount_,
            "Insufficient USDT"
        );
        ...
        uint256 token_amount = _getTokenAmount(USDTTokenAmount_);

        if (
            EIBAddress.length >= 1 &&
            EIBAddress.length == EIBPercentage.length &&
            EIBAddress.length == phase.length
        ) {
            for (uint256 i = 0; i < EIBAddress.length; i++) {
                if (phase[i] != 0) {
                    uint256 calculatePercentage = (USDTTokenAmount_ *
                        EIBPercentage[i]) / 100;
                    userEIB[EIBAddress[i]] += calculatePercentage;
                }
            }
        }

        token_amount += (token_amount * bonusPercentage) / 100;

        vestingToken.addTokenGrant(msg.sender, token_amount, vestingMonths, 1);

        _purchasedUSDTAmount[msg.sender][round] += USDTTokenAmount_;
        // userTokenBalance[msg.sender][round] += token_amount;
        token.transfer(vestingAddress, token_amount);
    }
    ...
    function newCrowdSaleRound(
        ...
        uint256 _maxPurchaseLimitInUSDT
    ) external virtual onlyOwner whenNotPaused {
        require(isFinalized, "Crowdsale is not yet closed");
        require(hasClosed(), "Crowdsale is not yet closed");
        require(roundRate > 0, "Rate: Amount cannot be 0");
        require(
            _maxPurchaseLimitInUSDT > minPurchaseInUSDT,
            "Maximum purchase limit should be more than minPurchaseInUSDT"
        );
        maxPurchaseInUSDT = _maxPurchaseLimitInUSDT;
     ...
     */
    function setPurchaseLimitInUSDT(
        uint256 minAmountPerTransaction,
        uint256 maxAmountPerUser
    ) external onlyOwner whenNotPaused {
        minPurchaseInUSDT = minAmountPerTransaction;
        maxPurchaseInUSDT = maxAmountPerUser;
        emit SetPurchaseLimit(minAmountPerTransaction, maxAmountPerUser);
    }
  ...
  */
    function withdrawAnyToken(
        ...
        require(_to != address(0), "Address cant be zero address");
        ...
        tokenContract.safeTransfer(_to, amount);
        emit TokenFromContractTransferred(_tokenAddress, _to, amount);
    }
    ...
    function setVestingAddress(
        address _vestingAddress, VestingVault _vestingVault
    ) external onlyOwner onlyWhileOpen whenNotPaused {
        vestingToken = _vestingVault;
     ...
     * @param _bonusPercentage to set the value of bonus percentage
     */
    function updateBonusPercentage(uint256 _bonusPercentage) external {
        bonusPercentage = _bonusPercentage;
    }

    /**
     * @dev to send bonus of user
     * @param _bonusAddress, _aoc and usdtValue to send the bonus
     */
    function finalBonus(
        address[] memory _bonusAddress,
        uint256[] memory _aoc,
        ...
        if (round == 3) {
            uint256 totalAOC = 0;
            if (
                _bonusAddress.length <= 1 && _bonusAddress.length == _aoc.length
            ) {
                for (uint256 i = 0; i < _bonusAddress.length; i++) {
                    // userTokenBalance[_bonusAddress[i]][round] += _aoc[i];
                    userEIB[_bonusAddress[i]] += usdtValue;
                    totalAOC += _aoc[i];
                ...
                token.transfer(vestingAddress, totalAOC);
            }
        }
    }

    /**
     * @dev to withdraw EIB balance
     */

    function withdrawEIBBalance() external {
        uint256 _userEIB = userEIB[msg.sender];
        require(_userEIB > (5 * 10 ** 18), "EIB balance is less than 6");
        _USDT.safeTransfer(msg.sender, _userEIB);
        userEIB[msg.sender] = 0;
    }
}
...
contract VestingVault is Initializable, OwnableUpgradeable, UUPSUpgradeable  {
    ...
    function claimVestedTokens() external {
        ...
        require(startTime > 0,"Vesting period is not started");
    ...
    function calculateGrantClaim(address _recipient) public view returns (uint256, uint256) {
        ...
        uint256 elapsedMonths = currentTime().sub(startTime).div(intervalTime).add(1); 
    ...
    function remainingToken(address _recipient) view external returns(uint256){
        Grant storage tokenGrant = tokenGrants[_recipient];
        return tokenGrant.amount.sub(tokenGrant.totalClaimed);
    }

    function StartTime() external {
        require(msg.sender == crowdsale_address, "The address cannot set start time");
