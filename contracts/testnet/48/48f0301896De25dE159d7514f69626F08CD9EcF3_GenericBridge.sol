    ...
    ChainIdHolding
{
    ...
    function requestBridge(
        ...
        if (!isBridgeToken(_tokenAddress)) {
            ...
            // check minimum fee
            (forUser, forFee) = validateMinFee(_tokenAddress, _amount, forFee);
    ...
    function _transferOriginToken(
        ...
        (forUser, forFee) = validateMinFee(_originToken, _amount, forFee);
    ...
    function getFeePercent() public view returns (uint256) {
        return defaultFeePercentage == 0
                ? DEFAULT_FEE_PERCENTAGE
                : defaultFeePercentage;
    }

    function validateMinFee(address originTokenAddress, uint256 amount, uint256 _forFee) public view returns (uint256 forUser, uint256 forFee) {
        forFee = _forFee;
        // check minimum fee
        if (feeForTokens[chainId][originTokenAddress] > forFee) {
            forFee = feeForTokens[chainId][originTokenAddress];
            require(amount > forFee, "amount too low");
        }
