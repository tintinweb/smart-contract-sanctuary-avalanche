...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        onlyClient(paymentId) 
    {
        ...
        if (
            currentTime >= payment.firstPaidDate &&
            currentTime - payment.firstPaidDate <= DAY_TO_SECOND * RIGHT_TO_CANCEL
        ) {
        ...
        nonReentrant 
    {
        Payment storage payment = payments[paymentId];
        require(
            block.timestamp > payment.voidDate &&
            block.timestamp - payment.voidDate <= DAY_TO_SECOND * 90,
            "You can only withdraw money within 90 days of void date"
        );
        require(
            payment.paidAmount > 0, 
            "This payment has not been paid or you withdrawn all money in this payment"
        );
        payment.status = Status.CANCELLED;
        payment.paidAmount = 0;
        ...
        nonReentrant 
    {
        ...
        uint256 currentTime = block.timestamp;
        require(
            currentTime > payment.voidDate &&
            currentTime - payment.voidDate <= DAY_TO_SECOND * 90,
            "You can only withdraw money within 90 days of void date"
        );
