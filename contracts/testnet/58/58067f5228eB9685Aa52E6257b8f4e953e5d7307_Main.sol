...
contract Main is Ownable, CloneFactory {
    ...
    function buyNow(bytes32 _key, uint _amount) external {
        ...
        _trading(_key, _amount, listInfo[_key].price);
    }

    function _trading (bytes32 _key, uint _amount, uint _price) private {
        if(_detect(listInfo[_key].collectionId) == ContractType.ERC721) {
            ...
            _exchangeDefaultNFT(_key, _amount, _price, false);
            ...
            try IERC1155(listInfo[_key].collectionId).getUserInfo(listInfo[_key].tokenId) returns(uint8, uint8, uint8 nftType, uint, address) {
                ...
                if(nftType == 0 || nftType == 2 || nftType == 4) {      // default NFT or Tier0 NFT or PRIVATE NFT
                    ...
                    _exchangeDefaultNFT(_key, amount, _price, false);
    ...
    function _exchangeDefaultNFT(bytes32 _key, uint amount, uint price, bool isFNFT) private {
        ...
        if(!isFNFT) {
            if(_detect(listInfo[_key].collectionId) == ContractType.ERC721) {
                ...
                IERC1155(listInfo[_key].collectionId).safeTransferFrom(listInfo[_key].maker, msg.sender, listInfo[_key].tokenId, amount, "");
            }
        }
    }
    ...
    function releaseMilestone(bytes32 _key) external {
        ...
        require(sNftInfo[_key].releaseId+1 <= sNftInfo[_key].auctionInfo.length, "Main: exceed milestone");
