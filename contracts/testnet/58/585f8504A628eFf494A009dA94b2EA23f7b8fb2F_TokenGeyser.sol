...
contract TokenGeyser is IStaking, Ownable
{
  ...
  constructor(IERC20 stakingToken, IERC20 distributionToken, uint maxUnlockSchedules, uint startBonus_, uint bonusPeriodSec_, uint initialSharesPerToken) public
  {
    ...
    require(startBonus_ <= 10 ** BONUS_DECIMALS, "Geyser: bonus too high");
    // if no period is desired, set startBonus = 100% & bonusPeriod to small val like 1sec.
    require(bonusPeriodSec_ != 0, "Geyser: bonus period 0");
    require(initialSharesPerToken > 0, "Geyser: 0");
  ...
  function unlockTokens() public returns (uint)
  {
    ...
    if (unlockedTokens > 0)
    {
      require(_lockedPool.transfer(address(_unlockedPool), unlockedTokens), "Geyser: tx out of locked pool err");
  ...
  {
    require(unlockSchedules.length < _maxUnlockSchedules, "Geyser: reached max unlock schedules");
    ...
    require(_lockedPool.token().transferFrom(msg.sender, address(_lockedPool), amount), "Geyser: tx into locked pool err");
  ...
  {
    require(amount > 0, "Geyser: stake amt is 0");
    require(beneficiary != address(0), "Geyser: ben is 0 addr");
    require(totalStakingShares == 0 || totalStaked() > 0, "Geyser: !valid state, staking shares but no tokens");
    ...
    require(mintedStakingShares > 0, "Geyser: Stake too small");
    ...
    require(_stakingPool.token().transferFrom(staker, address(_stakingPool), amount), "Geyser: tx into staking pool failed");
  ...
  function _unstake(uint amount) private returns (uint)
  {
    ...
    require(now > initStakeTimestamp.add(7 days), "Geyser: in cooldown");

    updateAccounting();

    require(amount > 0, "Geyser: unstake amt 0");
    require(totalStakedFor(msg.sender) >= amount, "Geyser: unstake amt > total user stake");

    uint stakingSharesToBurn = totalStakingShares.mul(amount).div(totalStaked());

    require(stakingSharesToBurn > 0, "Geyser: unstake too small");
    ...
    require(_stakingPool.transfer(owner(), unstakeFee), "Geyser: err tx fee");

    require(_stakingPool.transfer(msg.sender, amount.sub(unstakeFee)), "Geyser: tx out of staking pool err");
    require(_unlockedPool.transfer(msg.sender, rewardAmount), "Geyser: tx out of unlocked pool err");

    emit Unstaked(msg.sender, amount, totalStakedFor(msg.sender), "");
    emit TokensClaimed(msg.sender, rewardAmount);

    require(totalStakingShares == 0 || totalStaked() > 0, "Geyser: Err unstake. Staking shares but no tokens");
