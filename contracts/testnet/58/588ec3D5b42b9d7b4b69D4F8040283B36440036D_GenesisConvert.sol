    ...
    Withdrawable
{
    ...
    string private constant REVERT_CONTRACT_NOT_ALLOWED =
        "Management: Contract not allowed";
    string private constant REVERT_NOT_IN_BUSINESS =
        "Manager: caller is not in the business list";
    string private constant REVERT_NOT_AN_OWNER =
        "Manager: caller is not the NFT's owner";
    ...
    function getMessageHash(
        ...
        return
            keccak256(abi.encodePacked(tokenId, abi.encode(meta), timestamp));
    }
    ...
    function safeMint(
        ...
        _metadata[tokenId] = Metadata(creator, meta.typeNFT, meta.rarity);
        return tokenId;
    }
    ...
    function convert(
        ...
        (address creator, uint256 typeNFT) = Genesis(_genesisNFT).metadata(
            tokenId
        );
        (, , , uint256 unboxQuantity) = Genesis(_genesisNFT).getTypeNFT(
            typeNFT
        );
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            !inBusinessList(msg.sender) &&
            feeAmount > 0
        ) {
            require(
                IERC20(feeToken).transferFrom(from, feeBeneficiary, feeAmount),
                REVERT_NOT_IN_BUSINESS
            );
        }
...
interface Genesis {
    function metadata(uint256 tokenId)
        external
        view
        returns (address creator, uint256 typeNFT);
...
abstract contract BusinessAddresses is Ownable {
    ...
    string internal constant REVERT_ADDRESS_ALREADY_ACCEPTED =
        "BusinessAddresses: Address already accepted";
    string internal constant REVERT_ADDRESS_NOT_ACCEPTED =
        "BusinessAddresses: Address not accepted";
    ...
    function acceptBusinessAddresses(address[] memory _businessAddresses)
        external
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            require(
                !_businessAddressSet.contains(_businessAddresses[i]),
                REVERT_ADDRESS_ALREADY_ACCEPTED
            );
            _acceptBusinessAddress(_businessAddresses[i]);
        }
    }

    function cancelBusinessAddresses(address[] memory _businessAddresses)
        external
        onlyOwner
    {
        uint256 length = _businessAddresses.length;
    ...
    {
        uint256 length = size;
        if (length > _businessAddressSet.length() - cursor)
            length = _businessAddressSet.length() - cursor;
...
contract BaseData is Ownable {
    event SetUnboxNFTLimit(uint256[] rarities);

    uint256 public maxRarity; // maxRarity = 3
    string internal constant REVERT_NULL =
        "ConfigBaseData: Can not be address 0";

    function isValidMeta(uint256 rarity) public view returns (bool) {
        if (rarity < 1 || rarity > maxRarity) return false;

        return true;
    }

    function _configRarity(uint256 _maxRarity) internal {
        maxRarity = _maxRarity;
    }

    function configRarity(uint256 _maxRarity) external onlyOwner {
        _configRarity(_maxRarity);
    }
}
...
abstract contract Withdrawable is Ownable {
    string internal constant REVERT_TRANSFER_FAILED =
        "Withdrawable: AVAX_TRANSFER_FAILED";
...
abstract contract ERC4907 is ERC721, IERC4907 {
    string private constant REVERT_NOT_HAVE_POSSESSION =
        "ERC4907: transfer caller is not owner nor approved";
    string private constant REVERT_NOT_VALID_TRANSFER =
        "ERC4907: Cannot transfer NFTs that are in rented";
...
contract Signable is Context {
    ...
    function _configSigner(address newSigner) internal {
        require(
            newSigner != address(0),
            "Signer: new signer is the zero address"
        );
        _signer = newSigner;
