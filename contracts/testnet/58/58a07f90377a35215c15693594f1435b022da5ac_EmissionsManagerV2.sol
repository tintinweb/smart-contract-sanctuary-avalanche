...
contract EmissionsManagerV2 is EmissionsManager {
    ...
    uint256 s_liquidMiningAllocation;

    uint256 s_vapeMintedToLiquidMining;

    uint256 s_mintedForSeason;
    ...
    function mintLiquidMining() external onlyLiquidMining {
        ...
        uint256 currentSeason = s_liquidMining.getCurrentSeason();
        if(s_mintedForSeason == currentSeason) revert EmissionsManagerV2__AlreadyMintedForSeason();
        uint256 seasonalAllocation = s_liquidMining.rewardTokenToDistribute(currentSeason);
        ...
        s_mintedForSeason = s_liquidMining.getCurrentSeason();
...
interface ILiquidMining {
    function rewardTokenToDistribute(uint256 seasonId) external returns(uint256);
    function getCurrentSeason() external returns(uint256);
}
...
contract EmissionsManager is Initializable, OwnableUpgradeable {
    ...
    function setVape(address _vape) external onlyOwner {
        s_vape = IVape(_vape);
    }
}
