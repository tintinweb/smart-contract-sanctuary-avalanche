...
contract GamePool  is ERC1155Holder, ERC721Holder,Multicall,Context{
 ...
 /* @dev: upgrade game prop
   ...
   */
  function gameUpgrade(uint256 id, uint256 value, uint256 newId, uint256 amount,address tokenAddress,uint256  currentTimeStamp,uint256 chainId,bytes memory signature) external nonReentrant updateProof(currentTimeStamp) {
        require(chainId == block.chainid,"misMatched chinId ");
        require(proofMapping[signature] == 0,"Proof has expired");
        require(Proof.checkPermissions(id,value,newId,amount,tokenAddress,currentTimeStamp,chainId,signer,signature)==true,"You don't get the proof right");
        IErc1155Asset(tokenAddress).burn(address(this),id,value);
        IErc1155Asset(tokenAddress).mint(address(this),newId,amount,"0x");
        emit gameUpgradeEvent(block.chainid,msg.sender,id,value,newId,amount);
   }
