  ...
  Pausable
{
 ...
 //}
  ...
  function getTokenPrice(uint val) public view returns(uint[7] memory)
   {
    IUniswapV2Pair pair = IUniswapV2Pair(address(0x4E47Cd2a94dB858f599A55A534543ddAc30AFeC2));
    (uint Res0, uint Res1,) = pair.getReserves();
    //IERC20 token1 = IERC20(pair.token1()); // function `token1()`
    ...
    return [res0,res1,boo,now_val,low_val,high_val,i]; // return amount of token0 needed to buy token1
   }
   function process() external onlyAuthorized whenNotPaused returns (uint i) {
    uint[7] memory tok_range = getTokenPrice(val);
    uint tok_2 = tok_range[3];
    //uint high = high_it();
    //uint low = low_it();
    while (tok_2 > 1) {
    	tok_range = getTokenPrice(val);
