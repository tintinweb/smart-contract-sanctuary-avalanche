...
contract LinearVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    // Number of user that was checked out
    uint256 public noOfUsersChecked;
    // Total rewards unlocked
    uint256 totalRewardsUnlocked;
    // Total rewards locked
    ...
    {
        require(
            start >= noOfUsersChecked,
            "Start can not be below index of last checked user"
        );
        require(
            end > start,
            "End must be bigger then start index"
        );
        require(
            end <= noOfUsers,
            "End must be lower than number of users"
        );
        ...
        if(noOfUsersChecked == noOfUsers){
            uint256 _totalRewardsUnlocked = totalRewardsUnlocked;
            uint256 _totalRewardsLocked = totalRewardsLocked;
            uint256 _totalWithdrawn = totalWithdrawn;

            totalRewardsUnlocked = 0;
            totalRewardsLocked = 0;
            totalWithdrawn = 0;
            noOfUsersChecked = 0;

            _totalRewardsUnlocked = totalRewards.sub(_totalRewardsUnlocked);
            return (_totalRewardsLocked, _totalRewardsUnlocked, _totalWithdrawn);
        }
