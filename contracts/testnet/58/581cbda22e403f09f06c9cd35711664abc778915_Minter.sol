...
contract Minter is Ownable {
    ...
    /**
     *  @notice converts WACEO price to USDC value
     *  @return _waceoInWAVAX uint
     */
    function waceoPriceInUSD111() public view returns ( uint256 _waceoInWAVAX, uint256 _wavaxInUSD, uint256 _div ) {
        _waceoInWAVAX = waceoPriceInWAVAX();
        _wavaxInUSD =  wavaxPriceInUSD();
        _div =  _waceoInWAVAX.mul(_wavaxInUSD);
    }
     ...
     *  @return price_ uint
     */
    function  waceoPriceInWAVAX111() public view returns ( uint256 price_, uint256 _wavaxValue, uint256 _waceoValue ) {
          _wavaxValue = IERC20(Wavax).balanceOf(WAVAX_WACEO_LP);
     ...
     *  @return price_ uint
     */
    function  wavaxPriceInUSD111() public view returns ( uint256 price_, uint256 _wavaxValue, uint256 _stableValue) {
        _wavaxValue = IERC20(Wavax).balanceOf(WAVAX_STABLE_LP);
        _stableValue = IERC20(Stable).balanceOf(WAVAX_STABLE_LP);
        price_ = _stableValue.div(_wavaxValue);
    }
}