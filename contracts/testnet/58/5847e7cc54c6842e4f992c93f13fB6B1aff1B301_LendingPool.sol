...
contract LendingPool is ERC20, ReentrancyGuard, Pausable, AccessControl {
  ...
  */
  function totalValue() public view returns (uint256) {
    ...
    return totalBorrows + interest + totalLiquidity();
  }
  ...
  */
  function totalLiquidity() public view returns (uint256) {
    return ERC20(asset).balanceOf(address(this)) - poolReserves;
  }
  ...
  */
  function borrow(uint256 _assetAmount, address _vault) external nonReentrant whenNotPaused onlyApproved {
    ...
    uint256 debt = totalBorrows == 0 ? _assetAmount : _assetAmount * totalBorrowDebt / totalBorrows;

    // Update pool state
    totalBorrows = totalBorrows + _assetAmount;
    totalBorrowDebt = totalBorrowDebt + debt;

    // Update vault state
    borrowers[_vault].debt = borrowers[_vault].debt + debt;
  ...
  */
  function repay(uint256 _repayAmount, address _vault) external nonReentrant whenNotPaused onlyApproved {
  ...
  */
  function _mintShares(uint256 _assetAmount) internal returns (uint256) {
    ...
    uint256 shares = previousTotalValue == 0 ? _assetAmount * conversionFactor : _assetAmount * totalSupply() / previousTotalValue;
  ...
  */
  function _pendingInterest(uint256 _value) internal view returns (uint256) {
    if (block.timestamp > lastUpdatedAt) {
      ...
      uint256 floating = totalLiquidity() == 0 ? 0 : totalLiquidity() - _value;
  ...
  */
  function approveBorrower(address _borrower) external onlyRole(OWNER_ROLE) {
  ...
  /**
    * Refuse address to borrow from this pool
  */
  function refuseBorrower(address _borrower) external onlyRole(OWNER_ROLE) {
    require(borrowers[_borrower].approved == true, "Borrower already refused");
