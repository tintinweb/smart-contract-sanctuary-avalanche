...
contract NFTBettingGame is ERC721, ERC721URIStorage, Ownable {
        ...
        returns (address payable[3] memory)
    {
        ...
        // Find the top 3 bettors and their amounts
        ...
        for (uint256 i = 0; i < tokenIdList.length; i++) {
            ...
            // Check if the token is owned by the contract before processing its bet amounts
        ...
        // Calculate the rewards for each of the top 3 bettors
        uint256 rewardPool = (pot * 8) / 10; // 80% of total pot
        ...
        // Add 20% of the remaining pool amount to the treasury pool
        uint256 treasuryAmount = (remainingPool * 2) / 10;
        treasuryPool += treasuryAmount;

        // Transfer the remaining pool amount to the winner
        winner = payable(ownerOf(tokenIdList[random() % tokenIdList.length]));
        winner.transfer(remainingPool - treasuryAmount);
