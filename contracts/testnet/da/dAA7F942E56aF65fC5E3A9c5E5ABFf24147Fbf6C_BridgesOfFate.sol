...
contract BridgesOfFate is Ownable  {
    ...
    uint256 private constant _latestGameId = 1;
    ...
    function _checkSide(uint256 stageNumber, bool userSide) internal view returns (bool)
    {
        ...
        bool random_stage_status = GameMap[stageNumber] >= 50e9;
        if ((userSide == false && random_stage_status) || (userSide == true && random_stage_status)) {
            return true;
        }else {
            return false;
        }
    ...
    function _distributionReward() internal {
        ...
        require(GameStatusInitialized[_latestGameId].stageNumber == STAGES," Distribution time should not start before reaching final stage.");
    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public   {
        GameItem storage _member = PlayerItem[playerId]; //memory
        GameStatus storage  _gameStatus = GameStatusInitialized[_latestGameId];  //memory
        ...
        bool _status = GameMap[_member.stage - 1] >= 50e9;
        
        if (_member.stage != 0) {
            require((_member.lastJumpSide == true && _status ) || (_member.lastJumpSide == false && _status ), "You are Failed" );
        }
        ...
        if((_member.stage  == STAGES) && _checkSide(_gameStatus.stageNumber,_jumpSide) ){
