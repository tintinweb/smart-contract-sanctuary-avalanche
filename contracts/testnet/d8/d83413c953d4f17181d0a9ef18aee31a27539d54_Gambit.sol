...
contract Gambit {
    ...
    function viewPots() external view onlyOwner returns (global memory){
        require(msg.sender == owner, "Only Owner function");
        return globalV;
    }

    function viewMatch(string memory _matchId) external view onlyOwner returns (Match memory){
        require(msg.sender == owner, "Only Owner function");
        return matches[_matchId];   
    }
    
    function viewBet(string memory _matchId, address _address) external view onlyOwner returns (Bet memory){
        require(msg.sender == owner, "Only Owner function");
        return matchesBets[_matchId].betsByAddress[_address];   
    }
}