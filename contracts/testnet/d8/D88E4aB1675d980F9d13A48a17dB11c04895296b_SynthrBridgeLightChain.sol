...
contract SynthrBridgeLightChain is BaseBridge {
    using BytesLib for bytes;

    uint16 private _mainChainId;

    constructor(address _lzEndpoint) BaseBridge(_lzEndpoint) {}

    modifier onlySynthr() {
        require(
            msg.sender == getAddressFromResolver("WrappedSynthr") || msg.sender == getAddressFromResolver("Issuer") || msg.sender == getAddressFromResolver("Exchanger"),
    ...
    function initialize(address __synthrAddressResolver, uint16 __mainChainId, uint16 __selfLZChainId) external onlyOwner {
    ...
    function sendDepositCollateral(
        ...
        _broadcast(account, lzPayload, PT_DEPOSIT_COLLATERAL, _mainChainId);
    }
    ...
    function sendBurn(
        ...
        _broadcast(account, lzPayload, PT_BURN_SYNTH, _mainChainId);
    }
    ...
    function sendExchange(
        ...
        _broadcast(account, lzPayload, PT_EXCHANGE, dstChainId);
        // }

        if (dstChainId != _mainChainId) {
            _broadcast(account, lzPayload, PT_EXCHANGE, _mainChainId);
        }
    }
    ...
    function sendBridgeSyToken(
        ...
        require(dstChainId != 0 && dstChainId != _selfLZChainId, "Can not bridge tokens on the same chain");

        bytes memory lzPayload = abi.encode(PT_BRIDGE_SYNTH, abi.encodePacked(account), synthKey, amount, dstChainId);

        _broadcast(account, lzPayload, PT_BRIDGE_SYNTH, dstChainId);

        // // if (dstChainId != _mainChainId) {
        //     _broadcast(account, lzPayload, PT_BRIDGE_SYNTH, _mainChainId);
        // // }
    }
}