...
interface INodeManager {
    ...
    function compileVars(address _account) external;
...
contract main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function cashoutAll() internal {
        address sender = msg.sender;
        uint256 _cashoutFee = 50;
        uint256 rew = nodeManager.getNodeReward(sender);
        //uint256[2] memory rewards  = takeFee(,_cashoutFee);
        Send_it(rewardsPool, sender, checkTXN(rewardsPool,rew));
        logFees(Zero,Zero,Zero,Zero,Zero,Zero,sender,rew);
