...
import "./Endian.sol";
import "./interfaces/IBtcLightClient.sol";

// BtcLightClient lets you prove that a Bitcoin transaction executed, on Ethereum. It
// does this by running an on-chain light client.
//
// Anyone can submit block headers to BtcLightClient. The contract verifies
...
// script, the BtcLightClient contract always reports the current canonical Bitcoin
// chain.
contract BtcLightClient is IBtcLightClient {
    /**
    ...
    function submit(uint256 blockHeight, bytes calldata blockHeaders) public {
        ...
        // (BtcLightClient does not support a 2-week reorg)
    ...
    function getTarget(bytes calldata blockHeader) public pure returns (uint256) {
        bytes32 bits = bytes32(blockHeader[72 : 76]);
        return getTarget(bits);
    }
