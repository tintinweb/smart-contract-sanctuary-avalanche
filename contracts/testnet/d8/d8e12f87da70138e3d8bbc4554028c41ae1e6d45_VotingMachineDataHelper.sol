  ...
  function getProposalsData(IVotingMachineWithProofs votingMachine, uint256[] calldata proposalIds, address user)
    ...
    returns (Proposal[] memory)
    {
      ...
      for (uint256 i = 0; i < proposalIds.length; i++) {
        IVotingMachineWithProofs.ProposalState state = votingMachine.getProposalState(proposalIds[i]);
        ...
        if (state > IVotingMachineWithProofs.ProposalState.NotCreated) {
          ...
          proposals[i] = Proposal({
            ...
            votedInfo: VotedInfo({ support: vote.support, votingPower: vote.votingPower }),
            votingAssets: proposalWithoutVotes.strategy.getVotingAssetList()
          });
        } else {
          proposals[i] = Proposal({
            ...
            votingAssets: votingMachine.getVotingStrategy().getVotingAssetList()
          });
        }
      }
