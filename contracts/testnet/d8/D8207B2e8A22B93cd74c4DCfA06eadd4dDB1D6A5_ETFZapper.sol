...
contract ETFZapper is BNum {
    ...
    function joinSingle(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            IERC20(tokens[i]).safeApprove(ETFToken, amountsToPool[i]);
        }
        IETFToken(ETFToken).joinPool(poolAmountOut, amountsToPool);
        IERC20(ETFToken).safeTransfer(msg.sender, poolAmountOut);
    ...
    function exitSingle(
        ...
        uint256 exitFee = btoi(bmul(poolAmountIn, IETFToken(ETFToken).EXIT_FEE()));
        ...
        IERC20(ETFToken).safeTransferFrom(
            msg.sender,
            address(this),
            poolAmountIn
        );
        ...
        IERC20(tokenOut).safeTransfer(msg.sender, amountOutTotal);
    }
