/**
 *Submitted for verification at testnet.snowtrace.io on 2022-06-02
*/

pragma solidity 0.8.6;



contract Escrow {

    address owner;
    string dumbPassword = "";
    uint public ethers;


    event sending(address _from, address _to, uint256 amount); 

    function start() public {
        owner = msg.sender;
    }

    function release(string memory password, address receiver) public {
        require(keccak256(abi.encodePacked(password)) == keccak256(abi.encodePacked(dumbPassword)));
        emit sending(owner, receiver, ethers);
        ethers -= ethers;
    }

    function escrow(uint256 amount, string memory password) public {
        ethers += amount;
        dumbPassword = password;
    }

    function getBalance() public view returns (uint256) {
        return ethers;
    }

    function getOwner() public view returns (address) {
        return owner;
    }

    function setPassword(string memory password) public {
        dumbPassword = password;
    }

    receive() external payable {}
}