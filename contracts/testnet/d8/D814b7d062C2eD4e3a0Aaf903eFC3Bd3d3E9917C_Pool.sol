...
contract Pool is Ownable, ReentrancyGuard {
    ...
    function transferToTreasury(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            //50% avax - 50% vavax
            uint256 _wethToSwap = _amount / 2;
            uint256 _wethTransfer = _amount - _wethToSwap;

            WethUtils.weth.safeIncreaseAllowance(address(swapStrategy), _wethToSwap);
            swapStrategy.swapTransfer(_wethToSwap, treasury);
            WethUtils.weth.safeTransfer(treasury, _wethTransfer);
        }
    }
...
interface ISwapStrategy {
    ...
    function swapTransfer(uint256 _wethToSwap, address _treasury) external;
}
...
library WethUtils {
    ...
    IWETH public constant weth = IWETH(0xd00ae08403B9bbb9124bB305C09058E32C39A48c); //WAVAX
