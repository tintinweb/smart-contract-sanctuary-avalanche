...
contract AstroNodeManager is Ownable, Pausable {
    ...
    function setPlatformState(bool _isLive) public {
        require(msg.sender == treasuryAddress, 'Can only be used by Astronodes Treasury');
        isLive = _isLive;
    }
    ...
    function setMarketingShare(uint256 _marketingShare) public {
        require(msg.sender == treasuryAddress, 'Can only be used by Astronodes Treasury');
        marketingShare = _marketingShare;
    }

    function setClaimTax(uint256 _claimTaxNode) public onlyOwner {
    ...
    //Acts as a percentage base. if nodeBase = 100 and dailyInterest = 7 then nodeBase * dailyInterest should give us
    //107 which is what we need to calculate 7%
    ...
    function getAsnoBalance() external view returns (uint256) {
	    return asnoToken.balanceOf(lpPairAddress);
    }


    function getPrice() public view returns (uint256) {
        uint256 asnoTokenBalance = asnoToken.balanceOf(lpPairAddress);
        uint256 avaxBalance = avax.balanceOf(lpPairAddress);
        require(asnoTokenBalance > 0, "divison by zero error");
        uint256 price = avaxBalance * 1e30 / asnoTokenBalance;
        return price;
    }

       //Set Addresses
    function setTokenAddr(address tokenAddress) public {
        require(msg.sender == treasuryAddress, 'Can only be used by AstroNode Treasury');
        asnoToken = IERC20(tokenAddress);
    }

    function setLpPairAddr(address pairAddress) public {
        require(msg.sender == treasuryAddress, 'Can only be used by AstroNode Treasury');
    ...
    function buyNode(uint256 _amount) external {  
        ...
        require(nodesOwned > 1, "You must be gifted an AstroNode first");
        require(nodesOwned < 101, "Max AstroNodes Owned");
        Astronaut memory astronaut;
        if(astronauts[msg.sender].exists){
            astronaut = astronauts[msg.sender];
        } else {
            astronaut = Astronaut(true, 0, 0, 0, 0, 0);
            astronautsAddresses.push(msg.sender);
        }
        uint256 transactionTotal = nodeCost * _amount;
        uint256 toMarketing = transactionTotal / 10 * marketingShare;
        uint256 toTreasury = transactionTotal / 10 * treasuryShare;
        uint256 toPool = transactionTotal - toMarketing - toTreasury;
        _transferFrom(asnoToken, msg.sender, address(this), toPool);
        _transferFrom(asnoToken, msg.sender, address(treasuryAddress), toTreasury);
        _transferFrom(asnoToken, msg.sender, address(marketingAddress), toMarketing);
    ...
    function awardNode(address _address, uint256 _amount) external {
        require(isLive, "Platform is offline");
        require(_amount < 100, "Cannot donate that many AstroNodes");
        ...
        astronauts[_address].lastUpdate = block.timestamp;
    }
    ...
    function compoundNode() public {
        ...
        require(pendingClaims>nodeCost, "Not enough pending ASNO to compound");
    ...
    function updateClaims(address _address) internal {
        ...
        if (timerFrom > 0)
            astronauts[_address].claimsNodes += astronauts[_address].nodes * nodeBase * dailyInterest * (time - timerFrom) / 8640000;
            astronauts[_address].claimsDonated += astronauts[_address].donatedNodes * nodeBase * dailyInterest * (time - timerFrom) / 8640000;
            astronauts[_address].lastUpdate = time;
    }

    function getTotalClaimable() public view returns (uint256) {
        uint256 time = block.timestamp;
        uint256 pendingRewards = astronauts[msg.sender].nodes * nodeBase * dailyInterest * (time - astronauts[msg.sender].lastUpdate) / 8640000;
        uint256 pendingDonatedRewards = astronauts[msg.sender].donatedNodes * nodeBase * dailyInterest * (time - astronauts[msg.sender].lastUpdate) / 8640000;
        uint256 pending = pendingRewards + pendingDonatedRewards;
        return astronauts[msg.sender].claimsNodes + astronauts[msg.sender].claimsDonated + pending;
	}
    ...
    function claim() external payable {
        // make sure msg.sender is sender
		...
		if (reward > 0) {
            ...
            _transfer(asnoToken, msg.sender, toAstronaut);
            _transfer(asnoToken, address(treasuryAddress), toTreasury);
		}
	}
}