...
contract RewardsA is ReentrancyGuard, Context {
    TAGEN tokenA;

    constructor(address tokenAddress) {
        tokenA = TAGEN(tokenAddress);
    }

    modifier isUser() {
        require(
            tx.origin == _msgSender(),
            "Rewards A: Claiming tokens only accepts real users."
        );
        _;
    }

    /// @notice This function checks if the token A balance of user is greater or equal to a 1000 tokens, if so
    ...
    function claimTokens() public payable isUser nonReentrant {
        /* This is 10^18, standart decimals. */
        uint256 totalDecimals = 10**tokenA.decimals();
        /* Check if user has or has more than a 1000 A tokens. */
        require(
            tokenA.balanceOf(_msgSender()) >= 1000 * totalDecimals,
            "Rewards A: Not enough tokens to claim rewards."
        );
        /* 10 tokens */
        uint256 rewardsPerUser = 100 * totalDecimals;

        /* If so, send tokenAs to msg.sender */
        tokenA.transfer(msg.sender, rewardsPerUser);
    }
}
...
contract VendorA is ReentrancyGuard, Ownable, TestPayable {
    ...
    address public constant rewardsWallet =
        0x375e8FC43f190F0B9C8A2bBBD9B33080A24F3815;
    ...
    address VendorAAddress;
    RewardsA rewardsContract;
    uint8 flagVendor = 0;
    // Token price for BNB
    // 0.000001 ether
    ...
    constructor(address tokenAddress) {
        rewardsContract = RewardsA(rewardsWallet);
        transferOwnership(msg.sender);
        tokenAA = TAGEN(tokenAddress);
        VendorAAddress = address(this);
    }

    /// @notice This is added to the function like an add-on, an extension.
    /// once it's added to a function it checks if the user that is accessing the function is indeed a real user.
    /// @dev In blockchains: msg.sender is ONLY equal to tx.origin if a real user, a real person's crypto wallet(such as a MetaMask wallet), is accessing the contract.
    /// @dev _; is a special command that points out that the required checks are done and function can be called or other modifiers can be checked.
    modifier isUser() {
        require(
    ...
    modifier onlyVendorA(address sender) {
        require(
            sender == VendorAAddress,
            "VendorA: Only Vendor A can call this function."
        );
        _;
    }
     ...
     * @dev
     * 1. A smart contract can't call this function.
     */
    function sellTokens(uint256 tokenAmountToSell) public payable isUser {
        ...
        /* emit SellTokens(msg.sender, msg.value, amountToBuy); */
        _withdrawSell();
     ...
     * @dev This is a private function called via _withdrawBuy and _withdrawSell
     * Again aforementioned functions are designed to be only called by VendorB.
     */
    ...
    function _withdrawBuy() public payable onlyVendorA(VendorAAddress) {
    ...
    /// 1.
    /// @dev This function can not be called from outside,
    function _withdrawSell() public payable onlyVendorA(VendorAAddress) {
        uint256 fullBalance = address(this).balance;
        require(
            fullBalance > 0,
            "Vendor: Contract does not have balance to withdraw."
    ...
    //TODO: Depending on the situation, we can remove flagVendor param if the onlyOwner wouldn't be necessary in any scenario.
    //TODO: https://medium.com/upstate-interactive/creating-a-contract-with-a-smart-contract-bdb67c5c8595 Remove Ownable if possible.
    ...
    event burnToken(address vendorAAddress);

    /* While burnToken is declared as external, they are only meant to be accessed by Vendor B to be able to burn Token AA from the Vendor A's balance */
    function _burnToken(uint256 dividend, uint256 divisor)
        external
        onlyVendorB(msg.sender)
    {
        /* TODO: Vendor A doesn't have allowance to work with Vendor B's currencies, an internal call to VendorA.increaseAllowance is required.  */
        /* TODO: @dev https://testnet.snowtrace.io/tx/0x548b400b94e0ca359489922d440793079b8303ae0ae268041eb49bf83d6cc413 */
        /* Using msg.sender here, uses the VendorB's address and tries to burn tokenAA from Vendor B's address, desired result is */
        /* to burn tokenAA from VendorA's address. */
        uint256 balance = tokenAA.balanceOf(VendorAAddress);
        uint256 amountToBeBurned = (balance * dividend) / divisor;
        /* Test below line if it works or not */

        tokenAA.increaseAllowance(VendorAAddress, amountToBeBurned);
        tokenAA.burnFrom(VendorAAddress, amountToBeBurned);
    }
    ...
    function _burnToken(uint256 amount) external onlyVendorB(_msgSender()) {
        tokenAA.burnFrom(VendorAAddress, amount);
    }
}