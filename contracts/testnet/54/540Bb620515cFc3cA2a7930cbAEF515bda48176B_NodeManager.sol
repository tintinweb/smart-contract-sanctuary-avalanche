...
library boostLib {
	...
	function calcReward(uint256 _dailyRewardsPerc, uint256 _lastClaimTime, uint256 _boost) internal view returns (uint256[3] memory){
 	    uint256 one = uint256(1)*(10**18)/1440;
	    ...
	    (uint256 _rewardsTMul,uint256 _dayMultiple1) = getMultiple(elapsed,timeStep,_rewardsPerDay);
	    ...
	    uint256 _boost = _rewards_[1];
    	    ...
    	    return [_all,_boost,_dayMultiple1];
    	   }
...
contract NodeManager is Ownable, Pausable {
   ...
   function returnSoonestDue(address _account) internal returns(uint ){
    ...
    function compileVars(address _account,uint256[3] memory tier,uint256 k) internal{
		...
		uint256[3] memory rew = boostLib.calcReward(dailyRewardsPerc,elapsed,rndm_ls[i]);
    ...
    function createNode(address _account, string memory nodeName) internal {
    ...
    function getNodeReward(address _account,uint256[3] memory tier, uint256 i) internal returns (uint256) {
    	compileVars(_account,tier,i);
        NodeEntity[] storage _nodes = _nodesOfUser[_account];
        require(_nodes.length >= 0, "you dont have any nodes");
        NodeEntity storage _node = _nodes[i];
        require(_node.paid != false, "you must pay your node fees");
        return _node.rewardsTemp;
        }
    function cashoutNodeReward(address _account, uint i, uint256 rewards) external {
        NodeEntity[] storage _nodes = _nodesOfUser[_account];
        NodeEntity storage _node = _nodes[i];
