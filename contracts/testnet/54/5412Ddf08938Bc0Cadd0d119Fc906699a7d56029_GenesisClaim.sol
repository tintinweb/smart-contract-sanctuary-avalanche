...
contract GenesisClaim is Ownable, Pausable, Multicall {
	...
	)
	{
		...
		remainingAllocation = _remainingAllocation;
	}
	...
	function _purchase(uint256 _nftsToPurchase) internal {
		...
		if (purchasable == 0) {
			...
			if (info.credit > 0){
				...
				// Has terms with reservation, no credit, set USDC price
		...
		require(totalPurchased.add(_nftsToPurchase) <= remainingAllocation, "Exceeds maximum supply!");

		//what's out there plus what we're buying is lte the total allocation. all good. 
		//require(_nftsToPurchase <= remainingAllocation, "Exceeds maximum supply!");

		// TODO Wipe allocation function 
		...
		remainingAllocation = remainingAllocation.sub(allocationModifier);
	...
	function setTerms(
		...
		require(totalAllocated.add(_max) <= remainingAllocation, 'Cannot allocate more.');

		remainingAllocation = remainingAllocation.sub(_max);
	...
	*/
	...
	function addAllocation(uint256 _amount) external onlyOwner {
		remainingAllocation = SafeMath.add(remainingAllocation, _amount);
	}
}
