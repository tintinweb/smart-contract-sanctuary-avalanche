...
contract WorldOneWarrantDepository is Ownable {
    ...
    function deposit(
        ...
        //uint _amount = _warrantAmount.mul(priceInUSD);//100000000000*18000000000000000000
        uint _amount = _warrantAmount.mul(10**principle.decimals())
                        .mul(warrantPrice())
        ...
        //warrant ampunt/price calculated here and all remain same
        ...
        warrantInfo[_depositor] = Warrant({
            payout: warrantInfo[_depositor].payout.add(_warrantAmount),
    ...
    // function transferAmount() public onlyOwner {
    //     //principle.safeTransferFrom(address(this),msg.sender ,_amount);
    //     if(remainingAvailable() <= 10_000_000){
    //         send(owner, remainingAvailable());
    //     }
    // }
