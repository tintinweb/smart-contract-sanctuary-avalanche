...
library PropertyTypeLib {
     ...
     * @param _propertyTypeIndex Index of property type
     */
    function setMothlyRent(
        propertyTypes storage self,
        uint256 _monthlyRent,
        uint256 _propertyTypeIndex
    ) internal propertyTypeExists(self, _propertyTypeIndex) {
        self.array[_propertyTypeIndex].monthlyRent = _monthlyRent;
        emit PropertyTypeUpdated(self.array.length - 1, msg.sender);
    }
...
library PropertyLib {
    ...
    function remove(properties storage self, uint256 tokenId) internal {
    ...
    function isRentCleared(properties storage self, uint256 tokenId)
        internal
        view
        returns (bool)
    {
        if (
            self.get(tokenId).lastRentDeposited + monthtime >= block.timestamp
        ) {
            return true;
        }
        return false;
    ...
    {
        if (
            self.get(tokenId).lastRentDeposited +
                monthtime +
                self.rentDueAllowed <
            block.timestamp
        ) {
    ...
    // function to get time till rent is cleared.
    function getRentClearedTime(properties storage self, uint256 tokenId)
        internal
        view
        propertyExists(self, tokenId)
        returns (uint256)
    {
        return self.get(tokenId).lastRentDeposited + monthtime;
    }
    ...
    function acceptRent(
        ...
        // update the rent by months.
        self.array[tokenId].lastRentDeposited += (monthtime * months);
    }

    // Method to get upcoming due date for rent payment.
    function getUpcomingRentDue(properties storage self, uint256 tokenId)
        internal
        view
        propertyExists(self, tokenId)
        returns (uint256)
    ...
    function setRentDueAllowed(properties storage self, uint256 _newValue)
        internal
    {
        self.rentDueAllowed = _newValue;
    }
    ...
    function calculateReward(
        ...
        while (rewardProcessedTill <= block.timestamp) {
            ...
            // Stop calculation if reached today's time or rent cleared duration.
            if (
                rewardProcessedTill > block.timestamp ||
                rewardProcessedTill > self.getRentClearedTime(tokenId)
            ) {
                break;
            }
    ...
    function getClaimFeePercentrage(properties storage self,uint256 tokenId, uint256 reward, uint precisionValue, uint _totalSupply)
        public
        view
        returns (uint256)
    {
        // Flat 50% if property is locked(rent not paid even in due dates.)
...
contract AvaxToDollerUtils {
    address avaxToDaiPair;

    // Method to initialize the pair contract.
    function _initAvaxToDollerUtils(address _pair) internal{
        avaxToDaiPair = _pair;
    }

    // Function to get converted avax price in doller
    function getAvaxInDoller(uint256 amount) public view returns (uint256) {

        // Get pair contract
        IUniswapV2Pair pair = IUniswapV2Pair(avaxToDaiPair);

        // Get decimal for second to
        uint256 token1Decimals = TokenIERC20(pair.token1()).decimals();
        (uint256 Res0, uint256 Res1, ) = pair.getReserves();
        return ((amount * (Res0 * (10**token1Decimals))) /
            (Res1 * (10**token1Decimals))); // return amount of token0 needed to buy token1
    }

    // Function to convert Doller in Avax
    function getDollerInAvax(uint256 amount) view public returns (uint) {
        
        // Get pair contract
        IUniswapV2Pair pair = IUniswapV2Pair(avaxToDaiPair);

        // Get decimal for second to
        uint256 token0Decimals = TokenIERC20(pair.token0()).decimals();
        (uint256 Res0, uint256 Res1, ) = pair.getReserves();
        return ((amount * (Res1 * (10**token0Decimals))) /
            (Res0 * (10**token0Decimals))); // return amount of token0 needed to buy token1
    }

    // Public method to see the Pair contract between AVAX and doller.
    function getPair() view public returns (address) {
        return avaxToDaiPair;
    }
}
...
library PoolLib {
    ...
    function setPool(
        ...
        require(
            _treasuryPoolShare + _NEIBRPoolShare + _rewardPoolShare ==
        ...
        require(
            _treasuryTaxPoolShare + _NEIBRTaxPoolShare + _rewardTaxPoolShare ==
                100 * self.precisionValue,
    ...
    function setPrecisionValue(pool storage self, uint256 _newValue) internal {
    ...
    function distributeInPoolsAtTax(pool storage self) internal {
    ...
    // Set neighbour pool
    function setNeighbour(pool storage self, address _newAddress) internal {
...
// import "./interfaces/ITheFurniture.sol";
    ...
    AvaxToDollerUtils
{
    ...
    //************************Property Rent Variables**************************
    // Admin methods
    function setRentDueAllowed(uint256 _newValue) public onlyOwner {
        properties.setRentDueAllowed(_newValue);
    }
    ...
    function updateProperyTypeMothlyRent(
        ...
        propertyTypes.setDailyReward(_monthlyRent, _propertyTypeIndex);
    }
    ...
    {
        // create property
        uint256 tokenId = properties.create(_propertyTypeIndex, _name);
        // Mint property
    ...
    {
        return
            getDollerInAvax(propertyTypes.get(_propertyTypeIndex).monthlyRent);
    }

    // Method to check if the rent is cleared.
    function isRentCleared(uint256 tokenId)
        public
        view
        propertyExists(tokenId)
        returns (bool)
    {
        return properties.isRentCleared(tokenId);
    }
    ...
    // function to get time till rent is cleared.
    function getRentClearedTime(uint256 tokenId)
        public
        view
        propertyExists(tokenId)
        returns (uint256)
    {
        return properties.getRentClearedTime(tokenId);
    }

    // Method to accept rent
    function acceptRent(uint256 tokenId, uint256 months)
        public
        payable
    ...
    {
        require(
            msg.value >= (getRentInAVAX(tokenId) * months),
            "The Property: Insufficient rent supplied. "
        );

        // update the rent by months.
        properties.acceptRent(tokenId, months);
        ...
        emit RentCleared(
            tokenId,
            ownerOf(tokenId),
            properties.get(tokenId).lastRentDeposited,
            msg.value
        );
    }
    ...
    function getUpcomingRentDue(uint256 tokenId)
        ...
        returns (uint256)
    {
        return properties.getUpcomingRentDue(tokenId);
    }
    ...
    //     // Check if reward calculation is already started yet.
    //     require(
    //         block.timestamp >= rewardCalculationTime,
    //         "TheProperty: Reward calculation not started yet."
    //     );

    //     PropertyLib.property memory _property = properties.get(tokenId);

    //     // Get all furnitureAPYs for this property from last time reward calculated
    //     uint256[] memory rewardBoosts = new uint256[](
    //         _property.furnitureIndices.values.length
    //     );
    //     uint256[] memory tillTime = new uint256[](
    //         _property.furnitureIndices.values.length
    //     );

    //     // Starting time for last reward calculation.
    //     uint256 rewardProcessedTill = _property.lastRewardCalculated <=
    //         rewardCalculationTime
    //         ? rewardCalculationTime
    //         : _property.lastRewardCalculated;
    ...
    //         );
    //         rewardBoosts[i] = _APY;
    //         tillTime[i] = _nextAPYUpdate;
    //     }

    //     // Variables to store reward sum.
    //     uint256 rewardSum = 0;
    //     (uint256 _dailyReward, uint256 _nextUpdate) = propertyTypes.getDailyRewardFrom(
    //         _property.propertyTypeIndex,
    //         rewardProcessedTill
    //     );

    //     // max reward per day.
    //     uint256 maxRewardPerDay = propertyTypes
    //         .get(_property.propertyTypeIndex)
    //         .maxDailyReward;
    ...
    //             // Update the boost if required.
    //             if (rewardProcessedTill > tillTime[i]) {
    //                 (uint256 _APY, uint256 _nextAPYUpdate) = _furniture
    //                     .getAPYFrom(
    //                         _property.furnitureIndices.values[i],
    //                         rewardProcessedTill
    ...
    //             // Break if reached till total reward.
    //             if (todaysReward > maxRewardPerDay) {
    //                 todaysReward = maxRewardPerDay;
    //                 break;
    //             }
    //         }
    //         // Update today's reward
    ...
    //         if (
    //             rewardProcessedTill > block.timestamp ||
    //             rewardProcessedTill > getRentClearedTime(tokenId)
    //         ) {
    //             break;
    //         }
    ...
    //         // update daily reward.
    //         if (_nextUpdate < rewardProcessedTill) {
    //             (_dailyReward, _nextUpdate) = propertyTypes.getDailyRewardFrom(
    //                 _property.propertyTypeIndex,
    //                 rewardProcessedTill
    //             );
    //         }
    ...
    function claimReward(uint256 tokenId) public propertyExists(tokenId) {
        // Get reward calculated till rent is cleared or current time.
        (uint256 reward, uint256 processedTill) = calculateReward(tokenId);
    ...
    function payByReward(uint256 tokenId, uint256 amount) external {
        ...
        // send the reward
        ITheNeighbours _neighbour = ITheNeighbours(neighbour);
        _neighbour.transfer(msg.sender, amount);
    ...
    function initialize(string memory _URIPrefix, address _pair)
        public
        initializer
    {
        ...
        // Set MonthTime
        // monthtime = 30 days;
        properties.setRentDueAllowed(30 days);
