...
contract NODERewardManagement {
    ...
    function _getNodeLastClaimTime (address account, uint256 creationTime) 
        external
        view 
        returns (uint256)
    {
        ...
        return _node.lastClaimTime;
    }

    function _getNodeLastClaimTime (address account)
        external
        view
        returns (uint256[] memory)
    {
        ...
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        uint256[] memory lastClaimTime = new uint256[](nodesCount);
        
        for (uint256 i = 0; i < nodesCount; i++) {
            lastClaimTime[i] = nodes[i].lastClaimTime;
        }
        
        return lastClaimTime;
    }

    function _checkValidateAndCalculateFee (bool[] memory validate, uint256[] memory cashoutFeeTax, uint256 cashoutFee) external view returns (uint256 Fee){
        uint256 count = 0;
        for (uint256 i = 0; i < validate.length - 1; i++ )if(validate[i]) { Fee += cashoutFeeTax[i] ; count ++ ;}
        if(validate[4]){ Fee += cashoutFee; count ++; }
        return Fee.div(count);
    }
}
