...
// File contracts/MerkleLinearVesting.sol
...
contract MerkleLinearVesting is OwnableUpgradeable, ReentrancyGuardUpgradeable {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // Last hash in array of hashes
    ...
    // If user is withdrawing first time
    mapping(address => bool) public withdrawFirstTime;
    ...
    event ActivateFarm(bool indexed isActove);
    event EndTimeSet(uint256 indexed endTime);
    event UserRemoved(
    ...
    /**
     * @notice function is setting initial state of farm
     *
     * @param _root - hash of top node in tree
     * @param _vestedToken - address of token that is vested
     * @param _startTime - unix timestamp of start time
     * @param _endTime - unix timestamp of end time
     * @param _earlyClaimAvailablePer - claim percentage after start
     * @param _beforeStartClaimPer - claim percentage before start
     * @param _initialFundPercent - initial fund percent
     * @param _totalRewards - total amount of user rewards
        ...
        initializer
    {
        ...
        require(
            _totalRewards > 0,
            "Total rewards can not be equal or under 0"
        );
        require(
        ...
        totalRewardsPendingBalance = totalRewards;
    ...
    /**
     * @notice Returns true if a `leaf` can be proved,
     * to be a part of a Merkle tree, defined by `root`.
     * For this, a `proof` must be provided, containing
     ...
     *
     * @param proof - array of hashes of all nodes in tree
     * @param root - hash of last node in tree
     * @param leaf - hash of user
     */
    function _verify(
    ...
    /**
     * @notice function is setting merkle root
     *
        ...
        external
    {
        ...
        require(
            additionalFunds >= 0,
            "Total budget can not be below 0"
        );
        require(
            !isActive,
            "cannot change merkle root while farm is active"
        );
        ...
        onlyOwner
    {
        ...
        require(
            isUserOnFarm(proof, address(user), amount),
            "User is not part of farm"
        );

        if(!withdrawFirstTime[address(user)]) {
            withdrawFirstTime[address(user)] = true;
            totalUserRewards[address(user)] = amount;
        }
        ...
        totalRewardsPendingBalance = totalRewardsPendingBalance
            .sub(removedUserPendingBalance);
        // remove him
        totalUserPayouts[user] = totalUserRewards[user];
        isRemoved[user] = true;
        ...
        onlyOwner
    {
        if(!partialFunding){
            ...
            uint256 missingPendingBalance = totalRewardsPendingBalance
                .sub(vestedToken.balanceOf(address(this)));
            if(missingPendingBalance < minNeededAmount){
                minNeededAmount = missingPendingBalance;
            }
            ...
            uint256 fundAmount = safeTransferAndGetAmountDiff(
                address(msg.sender),
                address(this),
                _amount
            );
    ...
    /**
     * @notice function is doing a safe transfer and calculate,
     * the actual amount that passed. to avoid deflationary tokens errors
     *
     * @param _from - the from address for the transfer
     * @param _to - the recipient of the transfer,
     * @param _amount - amount to transfer
     *
     * @return amount that was transferred to contract
     */
    function safeTransferAndGetAmountDiff(
        address _from,
        address _to,
        uint _amount
    )
        ...
        returns(uint256)
    {
        ...
        uint256 balanceBefore = vestedToken.balanceOf(_to);
        ...
        return vestedToken.balanceOf(_to) - balanceBefore;
    }

    /**
     * @notice function is checking if user is on farm
     *
     * @param proof - array of hashes of all nodes in tree
     * @param amount - address of user + amount of user
     * @param user - address of user + amount of user
     *
     * @return bool if user is on farm or not
     */
    function isUserOnFarm(
        bytes32[] memory proof,
        address user,
        uint256 amount
    )
    ...
    /**
     * @notice function is returning code of these parameters into bytes
     *
     * @param user - address of user
     * @param amount - amount of user
     *
     ...
     *
     * @return return percent based on time
     */
    ...
    function getInfoOfUser(
        ...
        returns(uint256, uint256, uint256, uint256)
    {
        uint256 userTotalRewards = amount;
        ...
        if(userTotalRewards == userTotalPayouts){
            ...
            if(_vestingHasStarted()){
                amountEarned = _withdrawCalculation(user, amount, frontEnd);
            }
            else{
    ...
    function _withdrawCalculation(
        ...
        returns (uint256)
    {
        uint256 _earned = earned(address(user), amount);
     ...
     *
     * @return if farm has enough money to pay
     */
    ...
    /**
     * @notice function returns if user is fullly payed out
     *
     * @param user - the user address
     *
     * @return if user is paid or not
     */
    function _isUserPayedOut(
        address user
    )
        internal
        view
        returns(bool)
    {
        if(totalUserRewards[user] == 0){
            return false;
        }
        else{
            return totalUserPayouts[user] >= totalUserRewards[user];
        }
    }
    ...
    /**
     * @notice function that initialize merkle user rewards,
     * when he interact with contract for the first time
     *
     * @param _user - the user address
     * @param _totalRewards - the total reward amount
     */
    function initMerkleUser(
        address _user,
        uint256 _totalRewards
    )
        internal
    {
        if(!withdrawFirstTime[_user]) {
            withdrawFirstTime[_user] = true;
            totalUserRewards[_user] = _totalRewards;
        }
    }
     ...
     *
     * @param proof - array of hashes
     * @param _amount - initial amount of user
     *
     */
        ...
        nonReentrant
    {
        ...
        require(
            !_isUserPayedOut(address(msg.sender)),
            "User has been paid out"
        );
        require(
            isUserOnFarm(proof, address(msg.sender), _amount),
            "User is not part of farm"
        );
        //update user total rewards
        initMerkleUser(address(msg.sender), _amount);
     ...
     *
     * @param proof - array of hashes
     * @param _amount - initial amount of reward of users
     */
    function withdraw(
        bytes32[] memory proof,
        uint256 _amount
        ...
        nonReentrant
    {
        ...
        require(
            _amount > 0,
            "Users reward amount can not be 0 or below"
        );
        require(
            isUserOnFarm(proof, address(msg.sender), _amount),
            "User is not part of farm"
        ...
        uint256 rewardAmount = _withdrawCalculation(address(msg.sender), _amount, false);
