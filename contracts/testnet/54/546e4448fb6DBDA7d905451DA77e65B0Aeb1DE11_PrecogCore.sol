...
// File: contracts\precog-core\PrecogCore.sol
...
contract PrecogCore is IPrecogCore {
    ...
    modifier onlyPrecog() {
        require(msg.sender == precog, "PrecogCore: NOT_PRECOG");
        _;
    }
    ...
    constructor(
        ...
        cycleConfiguration = CycleConfiguration(604800, 86400, 172800, 86400);
    }
    ...
    function setCoreConfiguration(CoreConfiguration memory _coreConfiguration) external onlyAdmin {
        emit SetCoreConfiguration(
            coreConfiguration.admin,
            _coreConfiguration.admin,
            _coreConfiguration.middleware,
            _coreConfiguration.exchange
        );
    ...
    function _updateAdjustments() internal {
        ...
        for (uint i = 0; i < pairs.length; i++) {
            ...
            IPrecogV5.Cycle memory _futureTradingcycle = IPrecogV5(precog).getTradingCycle(
                pairs[i].token,
                _nextTradingTime
            );
            ...
            cyclesChangedInfo[pairs[i].token] = CyclesChangedInfo(
                _cycleChangedInfo.tradingApplyTime,
                _nextFundingTime,
                _nextDefundingTime,
                _cycleChangedInfo.fundingDuration,
                _cycleChangedInfo.firstDefundingDuration,
