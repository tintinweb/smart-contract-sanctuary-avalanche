...
contract LockToken is Initializable, OwnableUpgradeSafe, PausableUpgradeSafe {
    ...
    */
    ...
    returns (uint256 _id)
    {
        ...
        uint256 minRequiredFeeInEth = getFeesInETH(_tokenAddress);
        if( minRequiredFeeInEth > 0 ) {
            uint256 minRequiredFeeForMultiple = minRequiredFeeInEth.mul(_amounts.length);
            bool feesBelowMinRequired = msg.value < minRequiredFeeForMultiple;

            uint256 feeDiff = (feesBelowMinRequired) ? 
                SafeMath.sub(minRequiredFeeForMultiple, msg.value) : 
                SafeMath.sub(msg.value, minRequiredFeeForMultiple);
            
            if( feesBelowMinRequired ) {
                uint256 feeSlippagePercentage = feeDiff.mul(100).div(minRequiredFeeForMultiple);
                //will allow if diff is less than 5%
                require(feeSlippagePercentage <= 5, "Fee Not Met");
            }

            (bool success,) = companyWallet.call.value(feesBelowMinRequired ? msg.value : minRequiredFeeForMultiple)("");
            require(success, "Fee transfer failed");
            /* refund difference. */
            if (!feesBelowMinRequired && feeDiff > 0) {
                (bool refundSuccess,) = _msgSender().call.value(feeDiff)("");
