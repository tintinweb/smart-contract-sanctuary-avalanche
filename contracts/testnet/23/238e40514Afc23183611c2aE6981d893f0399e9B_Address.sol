...
// File: contracts/AvarobotsAnimals.sol
...
contract AvarobotsAnimals is ERC721Enumerable, Ownable {
    ...
    mapping(address => FirstGenHolders) public firstGenHolder;
    address[] firstGenHoldersAddr;

    address public constant devAddress = 0xEBef16315fBf47B1528755869C125281266A79BD;

    bool public saleIsActive = false;
    bool public privateSaleIsActive = true;

    event congratsForYourAvarobotsAnimalsNFT(uint256 indexed id);

    constructor() ERC721('AVAROBOTS Animals', 'AVAROBOTS Animals') {
    ...
    function addFirstGenHolder(address[] memory addrs) onlyOwner public returns(bool success) {
        firstGenHoldersAddr = addrs;
        for(uint i = 0; i < firstGenHoldersAddr.length; i++) {
            addAddressTo1stGenHolder(firstGenHoldersAddr[i]);
        }
        success = true;
    }
    ...
    function mintToken(address _to, uint numberOfTokens) public payable {
          ...
          if(privateSaleIsActive) {
              ...
              require(whitelist[_to].hasMinted.add(numberOfTokens) <= maxMintWhitelist, "Can only mint 1 while whitelisted");
              require(whitelist[_to].hasMinted <= maxMintWhitelist, "Can only mint 1 while whitelisted");
          ...
          for(uint i = 0; i < numberOfTokens; i++) {
              ...
              emit congratsForYourAvarobotsAnimalsNFT(mintIndex);
          }
    }

    function claimToken(address _to, uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(totalSupply().add(numberOfTokens) <= MAX_ELEMENTS, "Exceeds max supply");
        require(privateSaleIsActive, "Presale must be active to claim");
        require(numberOfTokens <= maxMint1stGenHolder, "Above max tx count");
        require(is1stGenHolder(_to), "Is not holder");
        require(firstGenHolder[_to].hasMinted.add(numberOfTokens) <= maxMint1stGenHolder, "Can only claim 1");
        require(firstGenHolder[_to].hasMinted <= maxMint1stGenHolder, "Can only claim 1");
        firstGenHolder[_to].hasMinted = firstGenHolder[_to].hasMinted.add(numberOfTokens);
    ...
    function addAddressTo1stGenHolder(address addr) onlyOwner public returns(bool success) {
        require(!is1stGenHolder(addr), "Already Added");
        firstGenHolder[addr].addr = addr;
        firstGenHolder[addr].hasMinted = 0;
        success = true;
    }
    ...
    function is1stGenHolder(address addr) public view returns (bool isWhiteListed) {
        return firstGenHolder[addr].addr == addr;
    }
