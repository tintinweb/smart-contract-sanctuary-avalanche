...
library LiquidateLogic {
    ...
    function executeAuction(
        IConfigProvider addressesProvider,
        ...
        vars.loanAddress = addressesProvider.loanManager();
        vars.reserveOracle = addressesProvider.reserveOracle();
        vars.nftOracle = addressesProvider.nftOracle();
        ...
        if (loanData.state == DataTypes.LoanState.Active) {
            ...
            vars.auctionEndTimestamp =
                ...
                addressesProvider.auctionDuration();
            require(
            ...
            vars.minBidDelta = vars.totalDebt.percentMul(
                addressesProvider.minBidDeltaPercentage()
            );
            require(
        ...
        )
    {
        ...
        vars.poolLoan = addressesProvider.loanManager();
        vars.reserveOracle = addressesProvider.reserveOracle();
        vars.nftOracle = addressesProvider.nftOracle();
        ...
        vars.redeemEndTimestamp = (loanData.bidStartTimestamp +
            addressesProvider.redeemDuration());
        require(
        ...
        vars.minRepayAmount = vars.borrowAmount.percentMul(
            addressesProvider.redeemThreshold()
        );
        require(
        ...
        if (fee > 0) {
            ...
            IERC20Upgradeable(loanData.reserveAsset).safeTransferFrom(
                vars.initiator,
                addressesProvider.platformFeeReceiver(),
                fee
            );
        }
    ...
    function executeLiquidate(
        ...
        IShopLoan(addressesProvider.loanManager()).liquidateLoan(
            loanData.bidderAddress,
            vars.loanId,
            vars.borrowAmount
        );
        ...
        if (vars.feeAmount > 0) {
            if (addressesProvider.platformFeeReceiver() != address(this)) {
                IERC20Upgradeable(loanData.reserveAsset).safeTransfer(
                    addressesProvider.platformFeeReceiver(),
                    vars.feeAmount
                );
...
library BorrowLogic {
    ...
    {
        return _repay(addressesProvider, reservesData, params);
    }
        ...
        )
    {
        ...
        vars.initiator = params.initiator;
        vars.loanId = params.loanId;
        vars.loanManager = addressesProvider.loanManager();
        ...
        (, , uint256 currentInterest, uint256 platformFee) = GenericLogic
            .calculateInterestInfo(
                GenericLogic.CalculateInterestInfoVars({
                    ...
                    platformFeeRate: addressesProvider.platformFeePercentage(),
                    interestDuration: addressesProvider.interestDuration()
                })
            );
