...
contract StakingCompound is Ownable, Pausable, ReentrancyGuard, IStaking, KeeperCompatibleInterface {
    ...
    {
        require(
            block.timestamp >= stakeStartDate,
    ...
    function autoCompound() public {
        ...
        for(uint i = start; i < start + iter; i++) {
            _autoCompound(walletList[i]);
        }
    }

    function _autoCompound(address _staker) internal updateReward(_staker) {
        uint256 reward = earned(_staker);
        if (reward > 0) {
            rewards[_staker] = 0;
            rewardsToken.safeTransferFrom(stakingBank, address(this), reward);

            totalSupply = totalSupply.add(reward);
            _balances[_staker] = _balances[_staker].add(reward);
            emit AutoCompounded(_staker, reward);
        }
    }
    ...
    function getCurrentRewardOf(address _account)
        external
        view
        onlyOwner
        returns (uint256)
