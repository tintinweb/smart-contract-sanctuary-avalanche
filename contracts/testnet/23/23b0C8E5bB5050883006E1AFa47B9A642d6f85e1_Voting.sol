...
contract Voting {
    ...
    modifier onlyPropertyOwnerOrAdmin() {
        require(
            ...
            "You must be a Property Owner or Admin to access this function"
        );
        _;
    }
    ...
    modifier onlyAkru() {
        require(
            whitelist.isWhitelistedUser(msg.sender) < 201,
            "Please verify your Accredation and/or KYC status before Voting"
        );
        _;
    }
    ...
    function deleteVote(uint256 _voteId) public onlyAdmin {}
    ...
    function getVoteTally(uint256 _voteId)
        ...
        returns (
            string memory _voteTopic,
            string[] memory _options,
            uint256[] memory _totaledVoteCounts
        )
    {
        _voteInfo memory voteInformation = voteInfo[_voteId];
        require(
