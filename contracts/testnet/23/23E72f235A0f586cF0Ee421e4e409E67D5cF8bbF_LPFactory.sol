...
contract LPERC20 is ILPERC20 {
    ...
    string public constant name = "AVATA Pair";
    string public constant symbol = "AVATLP";
    ...
    function transferFrom(
        ...
        if (allowance[from][msg.sender] != UINT_MAX) {
    ...
    function permit(
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, "LP: INVALID_SIGNATURE");
...
contract LPFactory is ILPFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, "LP: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "LP: ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "LP: PAIR_EXISTS"); // single check is sufficient
        bytes memory bytecode = type(LPPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        LPPair(pair).initialize(token0, token1);
...
import "./interfaces/ILPFactory.sol";
import "./interfaces/ILPCallee.sol";

contract LPPair is LPERC20 {
    using SafeMath for uint256;
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "LP: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(
        ...
        require(balance0 <= UINT_MAX && balance1 <= UINT_MAX, "LP: OVERFLOW");
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0) ILPCallee(to).lpCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        ...
        require(amount0In > 0 || amount1In > 0, "LP: INSUFFICIENT_INPUT_AMOUNT");
        {
            ...
            require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(1000**2), "LP: K");
        }
