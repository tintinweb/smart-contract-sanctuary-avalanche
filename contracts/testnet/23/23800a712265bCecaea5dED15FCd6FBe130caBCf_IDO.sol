...
contract IDO is Ownable {
    ...
    uint256 public LOCKUP_PERIOD = 5 * 60; // 5 minutes
    ...
    function registerReferral(address user, address _referral) external onlyOwner {
        require(
            _referral != user,
            "Referrer cannot be the same as the user"
        );
        require(
    ...
    function withdrawWrongToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(usdt) && _token != address(btf), "Cannot withdraw USDT or BTF");
