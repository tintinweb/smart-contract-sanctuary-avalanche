...
contract UsdcDepositAndBurn is Initializable, UUPSUpgradeable, AccessControlUpgradeable, IUsdcDepositAndBurn {
    ...
    // keccak256(chainType, keccak256(abi.encode(destChainId))) => DestDetails
    mapping(bytes32 => DestDetails) public chainIdToDestDetails;

    function initialize(address _voyagerDepositHandler, address _reserveHandler) external initializer {
    ...
    function setDestDetails(DestDetails[] memory destDetails) external override onlyRole(RESOURCE_SETTER) {
        for (uint256 i = 0; i < destDetails.length; i++) {
            bytes32 chainIdBytes = getChainIdBytes(destDetails[i].chainType, destDetails[i].chainId);
            ...
            chainIdToDestDetails[chainIdBytes] = DestDetails(
                destDetails[i].chainType,
    ...
    function depositAndBurnUsdc(bytes32 destChainIdBytes, uint256 amount) external override returns (uint64) {
        ITokenMessenger _tokenMessenger = tokenMessenger;
        require(address(_tokenMessenger) != address(0) && usdc != address(0), "usdc or circle bridge not set");

        DestDetails memory destDetails = chainIdToDestDetails[destChainIdBytes];
        require(destDetails.reserveHandlerAddress != address(0), "dest chain not configured");
        address _usdc = usdc;

        IERC20Upgradeable(_usdc).safeTransferFrom(reserveHandler, address(this), amount);
