    ...
    Pausable
{
    ...
    function claim(uint256 amount) public whenNotPaused {
        require(step >= ContractSteps.OPEN_HOLDER, "MadSkullz: 1001");
        require(amount <= remainingFreemints(), "MadSkullz: 1002");
    ...
    function holderMint(uint256 amount) external payable whenNotPaused {
        require(isHolder(), "MadSkullz: 1003");
        require(step >= ContractSteps.OPEN_HOLDER, "MadSkullz: 1001");
        _mint(amount);
    }
    ...
    function whitelistMint(uint256 amount) external payable whenNotPaused {
        ...
        require(step >= ContractSteps.OPEN_WHITELIST, "MadSkullz: 1001");
        _mint(amount);
    }

    function publicMint(uint256 amount) external payable whenNotPaused {
        require(step >= ContractSteps.OPEN_PUBLIC, "MadSkullz: 1001");
        _mint(amount);
    }

    function _mint(uint256 amount) internal whenNotPaused {
        uint256 totalCost = price(amount);
        require(msg.value >= totalCost, "MadSkullz: 1005");
        require(
            MAX_SUPPLY - FREEMINT_AMOUNT - totalSupply() - amount > 0,
            "MadSkullz: 1006"
        );
        require(amount <= MAX_MINTABLE_PER_TX, "MadSkullz: 1007");
        require(amount > 0, "MadSkullz: 1008");
    ...
    function reveal(uint256 tokenId) external whenNotPaused returns (uint256) {
        require(_exists(tokenId), "MadSkullz: 1009");
        require(ownerOf(tokenId) == _msgSender(), "MadSkullz: 1010");
        require(!(reveals[tokenId]), "MadSkullz: 1011");
    ...
    function autoReveal() external onlyRole(ADMIN_ROLE) {
        require(step >= ContractSteps.SOLDOUT, "MadSkullz: 1012");
        require(!autoRevealed, "MadSkullz: 1013");
    ...
    {
        require(_exists(tokenId), "MadSkullz: 1014");
        require(step >= ContractSteps.SOLDOUT, "MadSkullz: 1015");
    ...
    function earnXp(uint256 tokenId, uint256 xp)
        ...
        onlyRole(XP_ROLE)
    {
        require(_exists(tokenId), "MadSkullz: 1016");
    ...
    function resetXp(uint256 tokenId) public whenNotPaused onlyRole(XP_ROLE) {
        require(_exists(tokenId), "MadSkullz: 1017");
    ...
    {
        require(_exists(tokenId), "MadSkullz: 1018");
    ...
    function setFreemints(
        ...
        require(beneficiaries.length == amounts.length, "MadSkullz: 1019");
