/**
 *Submitted for verification at testnet.snowtrace.io on 2023-06-01
*/
...
contract MultiSig is MultiSigStruct {
  ...
  mapping(uint256 => mapping(address => bool)) private _signerProposalApprovers;

  mapping(uint256 => mapping(address => bool)) private _signerProposalRevokers;
  ...
  event ProposalExecuted(uint256 indexed id, uint256 indexed timestamp);

  event ProposalRevoked(uint256 indexed id, uint256 indexed timestamp);
  ...
  function proposeMinSignatureChange(
    uint256 newMinSignature
  ) public onlySignatories {
    require(newMinSignature > 0, "Multi-Sig: Invalid minimum signature value!");
    require(
      _minSignatures != newMinSignature,
      "Multi-Sig: Minimum Signatories already set"
    );
    require(
      newMinSignature <= _signatories.length,
      "Min SIgnature Cannot exceed signatories"
    );
    ...
    if(_signatories.length == 1 || _minSignatures == 1){
        _minSignatureProposals[proposalID].ISAPPROVED = true;
        _minSignatures = newMinSignature;
    }
    // Emit event
  ...
  function approveMinSignatureProposal(
    uint256 ProposalID_
  ) public onlySignatories {
    // proposal info
    MinSignatureProposal storage proposal = _minSignatureProposals[ProposalID_];
    ...
    require(proposal.PROPOSER != address(0), "Multi-Sig: Not valid proposal!");
  ...
  function revokeMinSignatureProposal(
    uint256 ProposalID_
  ) public onlySignatories {
    // proposal info
    MinSignatureProposal storage proposal = _minSignatureProposals[ProposalID_];
    ...
    // require sender have not already approved the proposal
    require(
      !_minSignatureProposalRevokers[ProposalID_][msg.sender],
      "Multi-Sig: Proposal is already revoked by signer!"
    );
  ...
  function addSigner(
    address newSigner_,
    SignerGroup _signerGroup,
    uint256 expiration
  ) public onlySignatories notNullAddress(newSigner_) {
    // require account not be a signer
    require(!_isSigner[newSigner_], "Multi-Sig: Signer address already added!");
  ...
  function removeSigner(
    ...
    SignerGroup _signerGroup,
    uint256 expiration
  ) public onlySignatories notNullAddress(signer_) {
    // require address be a signer
    require(_isSigner[signer_], "Multi-Sig: Signer address not found!");
    require(
      _signatories.length - 1 >= _minSignatures,
      "Min Signatures should be less than the number of signers"
    );
  ...
  // approve signer proposal
  function approveSignerProposal(
    ...
    if (_signerProposalRevokers[signerProposalID_][msg.sender] == true) {
      // update proposal approved by sender status
      _signerProposalRevokers[signerProposalID_][msg.sender] = false;
      proposal.REVOKEDCOUNT--;
    }
    ...
    if (proposal.APPROVEDCOUNT >= _minSignatures) {
      if (proposal.UPDATETYPE == SignerProposalType.ADD_SIGNER) {
        ...
        // emit event
        emit SignerAdded(msg.sender, proposal.PROPOSER, block.timestamp);
      } else {
        // remove signer
      ...
      proposal.ISAPPROVED = true;
      emit ProposalExecuted(signerProposalID_, block.timestamp);
    }
  }

  // revoke signer proposal
  function revokeSignerProposal(
    uint256 signerProposalID_
  ) public onlySignatories {
    // proposal info
    SignerProposal storage proposal = _signerProposals[signerProposalID_];
    ...
    // require sender have not already approved the proposal
    require(
      !_signerProposalRevokers[signerProposalID_][msg.sender],
      "Multi-Sig: Proposal is already revoked by this signer!"
    );

    if (_signerProposalApprovers[signerProposalID_][msg.sender] == true) {
      // update proposal approved by sender status
      _signerProposalApprovers[signerProposalID_][msg.sender] = false;
      proposal.APPROVEDCOUNT--;
    }

    _signerProposalRevokers[signerProposalID_][msg.sender] = true;
    proposal.REVOKEDCOUNT++;
    ...
    if (proposal.REVOKEDCOUNT >= _minSignatures) {
      proposal.ISREVOKED = true;
      emit ProposalRevoked(signerProposalID_, block.timestamp);
    }
  }
  ...
  function createLockProposal(
    ...
    for (uint256 i = 0; i < lockData_.length; i++) {
      ...
      for (uint256 j = 0; j < data.tokenIds.length; j++) {
        ...
        require(data.tokenIds.length != 0, "Multi-Sig: Invalid token ID!");
        require(
  ...
  function approveLockorUnlockProposal(
    ...
    if (
      _lockOrUnlockProposalRevokers[lockorUnlockProposalID_][msg.sender] == true
    ) {
      _lockOrUnlockProposalRevokers[lockorUnlockProposalID_][
        msg.sender
      ] = false;
      proposal.REVOKEDCOUNT--;
    }
  ...
  function revokeLockorUnlockProposal(
    uint256 lockorUnlockProposalID_
  ) public onlySignatories {
    // proposal info
    LockOrUnlockProposal storage proposal = _lockOrUnlockProposals[
    ...
    // require sender not have approved the proposal
    require(
      !_lockOrUnlockProposalRevokers[lockorUnlockProposalID_][msg.sender],
      "Multi-Sig: Already Revoked by this signer!"
    );
  ...
  function getSignerProposalCount() public view returns (uint256) {
  ...
  //  mapping(uint256 => SignerProposal) private _signerProposals;
  function getSignerProposalDetail(
    uint256 signerProposalID_
  ) public view returns (SignerProposal memory) {
  ...
  function isMinSignatureProposalApprover(
    ...
    return _minSignatureProposalApprovers[MinSignatureProposalID_][account_];
  }
  ...
  function getLockOrUnlockProposalDetail(
    uint256 lockOrUnlockProposalID_
  ) public view returns (LockOrUnlockProposal memory) {
  ...
  function getBatchLockInfo(
    uint256 lockOrUnlockProposalID_
  ) public view returns (BatchNFTsStruct[] memory) {
    return _batchLockInfo[lockOrUnlockProposalID_];
  }

  // get batch unlock info
  function getBatchUnlockInfo(
    uint256 lockOrUnlockProposalID_
  ) public view returns (BatchNFTsStruct[] memory) {
  ...
  // is signer proposal revoker

  function isSignerProposalRevoker(
    uint256 signerProposalID_,
    address account_
  ) public view returns (bool) {
    return _signerProposalApprovers[signerProposalID_][account_];
  ...
  // is Min Signature proposal revoker

  function isMinSignatureProposalRevoker(
    uint256 MinSignatureProposalID_,
    address account_
  ) public view returns (bool) {
    return _minSignatureProposalRevokers[MinSignatureProposalID_][account_];
  }

  //-----------------------------------------------------------------------------

  // is lock or unlock proposal revoker

  function isLockOrUnlockProposalRevoker(
    uint256 lockOrUnlockProposalID_,
    address account_
  ) public view returns (bool) {
    return _lockOrUnlockProposalRevokers[lockOrUnlockProposalID_][account_];
  }
...
contract SegMint_MultiSig_Factory {
  ...
  function updateOwnerAddress(address newOwnerAddress) public onlyOwner {
    require(
      ...
      "MultiSigFactory: New owner address cannot be the zero address!"
    );
   ...
   */
  function setSegMintKYCAddress(
    address SegMintKYCContractAddress_
  )
    public
    onlyOwner
    notNullAddress(SegMintKYCContractAddress_, "SegMint KYC Address")
  {
    ...
    SegMintKYCContractInterface = SegMintKYCInterface(
   ...
   */
  function restrictSegMintMultiSigAddress(
   ...
   * @return An array of SegMint MultiSig contract addresses.
   */
  function getSegMintMultiSigDeployedAddressByDeployer(
    address deployer
    ...
    if (_isSegMintMultiSig[MultiSigAddress_]) {
      for (uint256 i = 0; i < _deployedSegMintMultiSigList.length; i++) {
        if (_deployedSegMintMultiSigList[i] == MultiSigAddress_) {
          _deployedSegMintMultiSigList[i] = _deployedSegMintMultiSigList[
