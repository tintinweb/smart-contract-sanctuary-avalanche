 ...
 * - Support boosted pool. The dialutingRepartition can be different from that of MasterPlatypusV4
 */
contract BoostedMultiRewarderPerSec is IBoostedMultiRewarder, Ownable, ReentrancyGuard {
    ...
    uint16 public dialutingRepartition; // base: 1000
    ...
    constructor(
        ...
        require(_dialutingRepartition <= 1000, '_dialutingRepartition > 1000');
        require(
        ...
        if (block.timestamp > lastRewardTimestamp && lpSupply > 0) {
            for (uint256 i; i < length; ++i) {
                ...
                pool.accTokenPerShare += toUint128(
                    (tokenReward * ACC_TOKEN_PRECISION * dialutingRepartition) / lpSupply / 1000
                );
                if (sumOfFactors > 0) {
                    pool.accTokenPerFactorShare += toUint128(
                        (tokenReward * ACC_TOKEN_PRECISION * (1000 - dialutingRepartition)) / sumOfFactors / 1000
                    );
                }
            }
            ...
            if (block.timestamp > lastRewardTimestamp && lpSupply > 0) {
                ...
                if (sumOfFactors > 0) {
                    accTokenPerFactorShare +=
                        (tokenReward * ACC_TOKEN_PRECISION * (1000 - dialutingRepartition)) /
                        sumOfFactors /
                        1000;
                }
            }
