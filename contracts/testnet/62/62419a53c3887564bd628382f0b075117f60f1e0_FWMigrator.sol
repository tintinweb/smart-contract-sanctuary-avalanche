...
/// @notice A simple contract to migrate Frozen Walrus to Mammoth
...
contract FWMigrator is ReentrancyGuard, Context, Ownable{
    ...
    IERC20 public tokenV1; //address of the old token
    IERC20 public tokenV2; //address of the new token
    uint256 public rate; // 1 token V1 ---> 1 * rate token V2

    bool public migrationStarted;

    /// @notice Emits event every time someone migrates
    event MigrateToMMTH(address addr, uint256 amount);
    ...
    function migrateToMMTH(uint256 v1amount) public nonReentrant(){
        require(migrationStarted == true, 'Migration not started yet');
        uint256 amount = v1amount;
        uint256 userV1Balance = tokenV1.balanceOf(msg.sender);
        require(userV1Balance >= amount, 'You must hold FW tokens to migrate');
        uint256 amtToMigrate = (v1amount/1e18) * rate;
        require(tokenV2.balanceOf(address(this)) >= amtToMigrate, 'No enough MMTH liquidity');
        tokenV1.transferFrom(msg.sender, deadWallet, amount);
        tokenV2.transfer(msg.sender, amtToMigrate);
        emit MigrateToMMTH(msg.sender, amtToMigrate);
    }
