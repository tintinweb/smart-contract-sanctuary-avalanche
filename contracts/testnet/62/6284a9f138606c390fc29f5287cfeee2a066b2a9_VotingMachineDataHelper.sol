    ...
    for (uint256 i = 0; i < initialProposals.length; i++) {
      ...
      if (state > IVotingMachineWithProofs.ProposalState.NotCreated) {
        ...
        proposals[i] = Proposal({
          ...
          strategy: addresses.votingStrategy,
          dataWarehouse: addresses.dataWarehouse,
          votingAssets: IBaseVotingStrategy(address(addresses.votingStrategy))
            .getVotingAssetList(),
        ...
        proposals[i] = _getEmptyProposal(
          ...
          IBaseVotingStrategy(address(addresses.votingStrategy))
            .getVotingAssetList()
        );
      }
    }
