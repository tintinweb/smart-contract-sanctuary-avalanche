...
contract GothStake is ERC20("GOTH Bits", "bGOTH"), Ownable, ReentrancyGuard {
    ...
    GothTokenV2 public _goth;

    // Fee
    uint256 private _enterFeeMod = 4e8;
    uint256 private _leaveFee = 1e16;

    // Minting
    uint256 private _mintRateConstant = 317097919837640000000;
    uint256 private _startTimeStamp;
    uint256 private _lastMintTimeStamp;
    uint256 private _decayRate = 31705770450;

    mapping(address => uint256) _lastStake;

    event SetFeeMod (uint256 newFeeMod);
    ...
    event MintGoth (uint256 mintRate, uint256 amountMinted);

    constructor(GothTokenV2 goth_) {
        _goth = goth_;
        _startTimeStamp = block.timestamp;
        _lastMintTimeStamp = block.timestamp;
    }

    function feeMod () public view returns (uint256)
    {
        return _enterFeeMod;
    }

    function setFeeMod(uint256 newFeeMod) public onlyOwner
    {
        require(newFeeMod >= 2e8, "setFeeMod: fee mod too low");
        _enterFeeMod = newFeeMod;
        emit SetFeeMod(newFeeMod);
    }

    function leaveFee () public view returns (uint256)
    {
        return _leaveFee;
    }
    ...
    {
        require(newLeaveFee <= 3e16, "setLeaveFee: fee too high");
        _leaveFee = newLeaveFee;
        emit SetLeaveFee(newLeaveFee);
    }

    function calculateMintRate () public view returns (uint256)
    {
        uint256 secondsElapsed = block.timestamp.sub(_startTimeStamp);      
        return _mintRateConstant.sub(_decayRate.mul(secondsElapsed));
    }

    function mintGOTH () internal 
    {
        uint256 rate = calculateMintRate();
        uint256 secondsElapsed = block.timestamp - _lastMintTimeStamp;
        uint256 toMint = rate.mul(secondsElapsed);

        if (_goth.totalSupply() <= _goth.maxSupply().sub(toMint))
        {
            _goth.mint(address(this), toMint);
            _lastMintTimeStamp = block.timestamp;
        }
        
        emit MintGoth(rate, toMint);
    }

    function enter(uint256 _amount) public nonReentrant payable
    {
        require(msg.value >= _amount.div(_enterFeeMod), "enter: supplied fee too little");
        ...
        {
            uint256 toMint = _amount.mul(totalShares).div(totalGoth);
            _mint(msg.sender, toMint);
        }

        _goth.transferFrom(msg.sender, address(this), _amount);

        _lastStake[msg.sender] = balanceOf(msg.sender).mul(_goth.balanceOf(address(this))).div(totalShares);

        emit Enter(msg.sender, _amount);
    }

    function leave(uint256 _share) public nonReentrant payable
    {
        require(msg.value >= _leaveFee, "leave: supplied fee too little");

        mintGOTH();

        uint256 totalShares = totalSupply();
        uint256 toSend = _share.mul(_goth.balanceOf(address(this))).div(totalShares);

        _burn(msg.sender, _share);
        _goth.transfer(msg.sender, toSend);
    ...
    function gather (address account) public nonReentrant 
    {
        require(_lastStake[account] > 0, "gather: nothing staked");
        uint256 totalShares = totalSupply();
        uint256 totalStaked = balanceOf(msg.sender).mul(_goth.balanceOf(address(this))).div(totalShares);
        uint256 difference = totalStaked.sub(_lastStake[account]);
        _goth.transfer(account, difference);
        _lastStake[account] = totalStaked;
    } 
