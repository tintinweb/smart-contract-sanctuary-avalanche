...
import "@daoxy/os/contracts/acl/IACLOracle.sol";
import "@daoxy/os/contracts/apps/DaoxyApp.sol";
import "@daoxy/os/contracts/apps/disputable/IAgreement.sol";
import "@daoxy/os/contracts/apps/disputable/DisputableDaoxyApp.sol";
import "@daoxy/os/contracts/common/ConversionHelpers.sol";
import "@daoxy/os/contracts/common/SafeERC20.sol";
import "@daoxy/os/contracts/lib/math/SafeMath.sol";
import "@daoxy/os/contracts/lib/math/SafeMath64.sol";
import "@daoxy/os/contracts/lib/token/ERC20.sol";

import "@daoxy/staking/interfaces/0.4/IStaking.sol";
import "@daoxy/staking/interfaces/0.4/IStakingFactory.sol";
import "@daoxy/staking/interfaces/0.4/ILockManager.sol";

import "./arbitration/IArbitrable.sol";
import "./arbitration/IDaoxyAppFeesCashier.sol";


contract Agreement is IArbitrable, ILockManager, IAgreement, IACLOracle, DaoxyApp {
    ...
    // Note that Daoxy Court treats the possible outcomes as arbitrary numbers, leaving the Arbitrable (us) to define how to understand them.
    // Some outcomes [0, 1, and 2] are reserved by Daoxy Court: "missing", "leaked", and "refused", respectively.
    // This Arbitrable introduces the concept of the challenger/submitter (a binary outcome) as 3/4.
    // Note that Daoxy Court emits the lowest outcome in the event of a tie, and so for us, we prefer the challenger.
    ...
    event AppFeesCashierSynced(IDaoxyAppFeesCashier newAppFeesCashier);
    ...
    struct Setting {
        ...
        IDaoxyAppFeesCashier daoxyAppFeesCashier; // Fees cashier to deposit action fees (linked to the selected arbitrator)
        string title;
        bytes content;
    }
    ...
    struct Action {
        DisputableDaoxyApp disputable;     // Disputable app that created the action
    ...
    */
    function syncAppFeesCashier() external {
        ...
        IDaoxyAppFeesCashier newAppFeesCashier = _getArbitratorFeesCashier(setting.arbitrator);
        IDaoxyAppFeesCashier currentAppFeesCashier = setting.daoxyAppFeesCashier;

        // Sync the app fees cashier only if there was one set before and it's different from the arbitrator's current one
        if (currentAppFeesCashier != IDaoxyAppFeesCashier(0) && currentAppFeesCashier != newAppFeesCashier) {
            setting.daoxyAppFeesCashier = newAppFeesCashier;
    ...
    */
        ...
        auth(MANAGE_DISPUTABLE_ROLE)
    {
        ...
        DisputableDaoxyApp disputable = DisputableDaoxyApp(_disputableAddress);
    ...
    */
    function changeCollateralRequirement(
        DisputableDaoxyApp _disputable,
        ERC20 _collateralToken,
        uint64 _challengeDuration,
        uint256 _actionAmount,
        uint256 _challengeAmount
    ...
    */
    function newAction(uint256 _disputableActionId, bytes _context, address _submitter) external returns (uint256) {
        ...
        DisputableDaoxyApp disputable = DisputableDaoxyApp(msg.sender);
    ...
    */
    function challengeAction(uint256 _actionId, uint256 _settlementOffer, bool _finishedEvidence, bytes _context) external {
        ...
        (DisputableDaoxyApp disputable, CollateralRequirement storage requirement) = _getDisputableInfoFor(action);
    ...
    * @return daoxyAppFeesCashier Address of the fees cashier to deposit action fees (linked to the selected arbitrator)
    ...
    */
    function getSetting(uint256 _settingId)
        external
        view
        returns (IArbitrator arbitrator, IDaoxyAppFeesCashier daoxyAppFeesCashier, string title, bytes content)
    {
        Setting storage setting = _getSetting(_settingId);
        arbitrator = setting.arbitrator;
        daoxyAppFeesCashier = setting.daoxyAppFeesCashier;
        title = setting.title;
    ...
    */
    ...
    /**
    * @dev Disable built-in DaoxyApp token recovery escape hatch.
    ...
    */
    function _newSetting(IArbitrator _arbitrator, bool _setAppFeesCashier, string _title, bytes _content) internal {
        ...
        setting.daoxyAppFeesCashier = _setAppFeesCashier ? _getArbitratorFeesCashier(_arbitrator) : IDaoxyAppFeesCashier(0);
    ...
    * @param _setting Agreement setting instance, used to get Daoxy App Fees Cashier
    ...
    */
    function _payAppFees(Setting storage _setting, DisputableDaoxyApp _disputable, address _submitter, uint256 _actionId) internal {
        // Get fees
        IDaoxyAppFeesCashier daoxyAppFeesCashier = _setting.daoxyAppFeesCashier;
        if (daoxyAppFeesCashier == IDaoxyAppFeesCashier(0)) {
            return;
        }

        bytes32 appId = _disputable.appId();
        (ERC20 token, uint256 amount) = daoxyAppFeesCashier.getAppFee(appId);
        ...
        _approveFor(token, address(daoxyAppFeesCashier), amount);

        // Pay fees
        daoxyAppFeesCashier.payAppFees(appId, abi.encodePacked(_actionId));
    }
    ...
    */
    function _rejectAction(uint256 _actionId, Action storage _action, uint256 _challengeId, Challenge storage _challenge) internal {
        ...
        (DisputableDaoxyApp disputable, CollateralRequirement storage requirement) = _getDisputableInfoFor(_action);
    ...
    */
    function _getArbitratorFeesCashier(IArbitrator _arbitrator) internal view returns (IDaoxyAppFeesCashier) {
        (address cashier,,) = _arbitrator.getSubscriptionFees(address(this));
        return IDaoxyAppFeesCashier(cashier);
    }
    ...
    */
    function _getDisputableInfoFor(Action storage _action)
        internal
        view
        returns (DisputableDaoxyApp disputable, CollateralRequirement storage requirement)
    {
    ...
    */
    function _canClose(Action storage _action) internal view returns (bool) {
        ...
        DisputableDaoxyApp disputable = _action.disputable;
        // Assume that the Disputable app does not need to be checked if it's the one asking us to close an action
        return DisputableDaoxyApp(msg.sender) == disputable || disputable.canClose(_action.disputableActionId);
    }
    ...
    */
    function _canPerformChallenge(DisputableDaoxyApp _disputable, address _challenger) internal view returns (bool) {
...
import "@daoxy/os/contracts/lib/standards/ERC165.sol";
...
/**
* @title DaoxyAppFeesCashier interface
* @dev This interface is derived from the `IArbitrator`'s subscriptions module.
*      since we are using different solidity versions.
*/
interface IDaoxyAppFeesCashier {
    /**
    * @dev Emitted when an IDaoxyAppFeesCashier instance sets a new fee for an app
    ...
    */
    ...
    /**
    * @dev Emitted when an IDaoxyAppFeesCashier instance unsets an app fee
    * @param appId App identifier
    */
    event AppFeeUnset(bytes32 indexed appId);

    /**
    * @dev Emitted when an IDaoxyAppFeesCashier instance receives a payment for an app
...
// Contracts inheriting from DaoxyApp are, by default, immediately petrified upon deployment so
// that they can never be initialized.
// Unless overriden, this behaviour enforces those contracts to be usable only behind an AppProxy.
// ReentrancyGuard, EVMScriptRunner, and ACLSyntaxSugar are not directly used by this contract, but
// are included so that they are automatically usable by subclassing contracts
contract DaoxyApp is ERC165, AppStorage, Autopetrified, VaultRecoverable, ReentrancyGuard, EVMScriptRunner, ACLSyntaxSugar {
...
contract DisputableDaoxyApp is IDisputable, DaoxyApp {
    /* Validation errors */
    ...
    // bytes32 internal constant AGREEMENT_POSITION = keccak256("daoxyOS.appStorage.agreement");
