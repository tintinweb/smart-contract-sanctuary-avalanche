...
contract LightsExpress is Ownable, Pausable, ReentrancyGuard {
    ...
    function leaveNorthernLightsExpress() external {
        require(stake[_msgSender()].stake > 0, "You didn't deposit yet");
        require(!left[_msgSender()], "This wallet address has already left NorthernLights.express");
    ...
    function withdrawCapital() external {
        ...
        require(block.timestamp - stake[_msgSender()].timestamp > 22 days, "Error: Withdrawing ALL balance (Deposit + Rewards) is available 22 days after Leaving NorthernLights.express - Read FAQ");
    ...
    function clauneckEntityTradingBot (uint256 amount) external onlyOwner {
