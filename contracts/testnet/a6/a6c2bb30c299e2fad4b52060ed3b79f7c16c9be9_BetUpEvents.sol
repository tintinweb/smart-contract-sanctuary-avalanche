    ...
    */
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            if (innerPair.total - op == innerPair.total || innerPair.total - op == 0) revert InvalidPairID(pairID);
            uint256 others = (innerPair.total - op);

            uint256 availableAmount = (others * (10000  - pair.creationRatio)) / (10*10000);
