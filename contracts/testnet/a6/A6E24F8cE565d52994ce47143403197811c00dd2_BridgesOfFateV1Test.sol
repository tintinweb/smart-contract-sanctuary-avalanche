    ...
    */
        ...
        After24Hours(playerId)
    {
        ...
        if (_gameStatus.GameLevel == 1) {
            ...
            if (randomNumberPreviouseStages(playerId) > 0) {
                randomNumber = randomNumberPreviouseStages(playerId);
            }
    ...
    /*
     * This func for the previous stage users.
     * previous use get the least stage erandom number and side
     */
     function randomNumberPreviouseStages(bytes32 playerId)
        public
        view
        returns (uint256)
    {
        GameMember memory _member = Player[playerId];
        if(RandomNumberAt.length > 0){
            return RandomNumberAt[_member.stage + 1];
        }else{
            return 0;
        }
    }
