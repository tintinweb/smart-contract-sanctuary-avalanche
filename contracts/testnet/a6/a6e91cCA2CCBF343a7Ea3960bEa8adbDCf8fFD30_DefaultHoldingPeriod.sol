...
contract DefaultHoldingPeriod is ICompliance, Ownable {

    IToken public token;
    IIdentityRegistry public identityRegistry;
    uint256 private _holdEnd;

    /// @dev start error codes at 4 (1-3 is reserved by default, see Compliance.sol) 
    uint8 public constant SUCCESS_CODE = 0;
    uint8 public constant HOLD_FAIL_CODE = 4;

    /// @dev specify revert messages for custom compliance modules
    string public constant SUCCESS_MESSAGE = "SUCCESS";
    string public constant HOLD_FAIL_ERROR = "ILLEGAL_TRANSFER_DURING_HOLDING_PERIOD";
    string public constant COMPLIANCE_ERROR = "ERROR UNKNOWN";
    ...
    /// @dev Modifier for only bound token functionalities
    modifier onlyToken() {
        require(_tokensBound[msg.sender], "Unbound token");
        _;
    }
    ...
    function canTransfer(
      ...
      if(checkTransferCompliance(_from, _to, _value) == SUCCESS_CODE) {
        return true;
      }
      return false;
    }
    ...
    /**
     *  @dev set hold release time of token (unix timestamp)
     */
    function setTokenHold(address _token, uint256 _holdEndTime) external onlyOwner {
        require(_tokensBound[_token], "UNBOUND TOKEN");
        _holdEnd = _holdEndTime;
    }
     ...
     * 
     * @dev any transfer restriction checks that are not native to the token contract
     *      should be defined and verified here
     */
    function checkTransferCompliance(address _from, address _to, uint256 _value) public override view returns(uint8){
        if(block.timestamp < _holdEnd && token.isAgent(_from)) {
            return HOLD_FAIL_CODE;
        }
        return SUCCESS_CODE;
    }

    /** 
     * @notice  public facing function to check any specific restriction messages that may be related to this contract
     ...
     */
    function restrictionMessage(uint8 _restrictionCode) external override pure returns(string memory) {
        if(_restrictionCode == HOLD_FAIL_CODE) {
            return HOLD_FAIL_ERROR;
        }
