...
/**
 * @notice Defines how to interact with a Whitelister checker.
 *
 * @dev Any consumer should depend only on this interface,
 * rather than the full contract implementation.
 * In turn, any supplier should implement this interface to enforce
 * adhering to the interface.
 * See Interface Segregation Principle for more details on this pattern
...
interface IWhitelistChecker {
    /**
     * @notice Returns `true` if the `whitelistedAddr` address has been
     * already whitelisted by this contract.
     * Otherwise it returns `false`.
     *
     * @dev Each contract implementor is freely to choose how to whitelist addresses.
     */
    function isWhitelisted(address whitelistedAddr) external view returns (bool);
}
...
import "../../interfaces/IWhitelistChecker.sol";
    ...
    IERC4626
{
    ...
    string public constant RECEIPT_TOKEN_NAME = "USDRE";
    string public constant RECEIPT_TOKEN_SYMBOL = "USDRE";
    ...
    IWhitelistChecker private _whitelistChecker;
    ...
    function initialize(
        ...
        uint256 maxDeposit_,
        uint256 _sharePrice,
        IWhitelistChecker whitelistChecker_
        ...
        _whitelistChecker = whitelistChecker_;
    ...
    function updateSharePrice(uint256 _sharePrice) external override whenNotPaused onlyAdmin {
        sharePrice = _sharePrice;
        emit SharePriceUpdated(sharePrice);
    }

    function getWhitelistChecker() external view returns (IWhitelistChecker) {
        return _whitelistChecker;
    }

    function setWhitelistChecker(IWhitelistChecker whitelistChecker) public whenNotPaused {
        require(address(whitelistChecker) != address(0), "MemberDeposit: require whitelist checker");
    ...
    function _deposit(
        ...
        address receiver,
        bool _isManager
    ) internal nonReentrant onlyWhitelisted returns (uint256 sharesToMint) {
    ...
    function _previewDeposit(uint256 assets) internal view returns (uint256 sharesToMint) {
        ...
        uint256 depositFee = (sharesToMint * depositFeeNominator) / DEPOSIT_FEE_DENOMINATOR;
        // sharesToMint = (sharesToMint * (DEPOSIT_FEE_DENOMINATOR - depositFeeNominator)) / DEPOSIT_FEE_DENOMINATOR;
        // console.log("sharesToMint: %s", sharesToMint - depositFee);
        sharesToMint = sharesToMint - depositFee;
    }
    ...
    function _convertToShares(uint256 assets) internal view returns (uint256 shares) {
        // console.log("price: %s", sharePrice);
        // console.log("assets: %s", assets);
    ...
    function _scheduleRedeem(uint256 shares) internal {
        // console.log("_scheduleRedeem: %s", shares);
    ...
    modifier onlyWhitelisted() {
        require(_whitelistChecker.isWhitelisted(msg.sender), "MemberDeposit: Only whitelisted");
        _;
    }
}
