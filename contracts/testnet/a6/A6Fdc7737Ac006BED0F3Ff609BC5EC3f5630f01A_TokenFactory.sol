...
contract TokenFactory is IOmniApp {
    ...
    function mintToken(MintParams calldata params) public payable {
        ...
        if (params.mintPrice > 0) {
            payOnMint(params.mintPrice, msg.sender, address(this), true);
        }
        bytes memory payload = _getMintPayload(params.coll, params.mintPrice, params.creator);
    ...
    function pay(address rec, address cre, uint256 price, bool minted) private {
        ...
        IERC20 erc = IERC20(assets[0]);
        // TODO: Set asset per collection and get from it
        if (minted) {
