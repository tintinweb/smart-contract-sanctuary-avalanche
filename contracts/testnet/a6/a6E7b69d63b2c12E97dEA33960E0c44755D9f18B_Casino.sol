...
contract Casino is Ownable {
    ...
    uint256 public claimTaxGl = 20;
    uint256 public claimTaxBond = 20;
    ...
    struct Casino {
        ...
        uint256 ticket;
        uint256 bondTicket;
        uint256 claimsTicket;
        uint256 claimsBond;
        uint256 lastUpdate;
    }

    //mappings
    mapping(address => Casino) public casino;

    //constructor
    constructor (
        address _gl, //address of a standard erc20 to use in the platform
    ...
    function getGlBalance() external view returns (uint256) {
	    return gl.balanceOf(pair);
    }
    ...
    function getPrice() public view returns (uint256) {
        uint256 glBalance = gl.balanceOf(pair);
        uint256 usdcBalance = usdc.balanceOf(pair);
        require(glBalance > 0, "divison by zero error");
        uint256 price = usdcBalance * 1e30 / glBalance;
        return price;
    }
    ...
    function setGlTax(uint256 _claimTaxGl) public onlyOwner {
        claimTaxGl = _claimTaxGl;
    }
    ...
    function buyNode(uint256 _amount) external payable {  
        ...
        uint256 nodesOwned = casino[msg.sender].ticket + casino[msg.sender].bondTicket + _amount;
        require(nodesOwned < 101, "Max Cows Owned");
        Casino memory farmer;
        if(casino[msg.sender].exists){
            farmer = casino[msg.sender];
        } else {
            farmer = Casino(true, 0, 0, 0, 0, 0);
            farmersAddresses.push(msg.sender);
        }
        uint256 transactionTotal = nodeCost * _amount;
        uint256 toTreasury = transactionTotal / 100 * treasuryShare;
        uint256 toPool = transactionTotal - toTreasury;
        _transferFrom(gl, msg.sender, address(this), toPool);
        _transferFrom(gl, msg.sender, address(treasury), toTreasury);
        casino[msg.sender] = farmer;
        updateClaims(msg.sender);
        casino[msg.sender].ticket += _amount;
        totalNodes += _amount;
    }
    ...
    function bondNode(uint256 _amount) external payable {
        ...
        casino[msg.sender] = farmer;
        updateClaims(msg.sender);
        casino[msg.sender].bondTicket += _amount;
        totalNodes += _amount;
    }

    function awardNode(address _address, uint256 _amount) public onlyOwner {
        uint256 nodesOwned = casino[_address].ticket + casino[_address].bondTicket + _amount;
        require(nodesOwned < 101, "Max Cows Owned");
        Casino memory farmer;
        if(casino[_address].exists){
            farmer = casino[_address];
        } else {
            farmer = Casino(true, 0, 0, 0, 0, 0);
            farmersAddresses.push(_address);
        }
        casino[_address] = farmer;
        updateClaims(_address);
        casino[_address].ticket += _amount;
        totalNodes += _amount;
        casino[_address].lastUpdate = block.timestamp;
    }

    function compoundNode() public {
        uint256 pendingClaims = getTotalClaimable();
        uint256 nodesOwned = casino[msg.sender].ticket + casino[msg.sender].bondTicket;
        require(pendingClaims>nodeCost, "Not enough pending Golden Lion to compound");
        require(nodesOwned < 101, "Max Cows Owned");
        updateClaims(msg.sender);
        if (casino[msg.sender].claimsTicket > nodeCost) {
            casino[msg.sender].claimsTicket -= nodeCost;
            casino[msg.sender].ticket++;
        } else {
            uint256 difference = nodeCost - casino[msg.sender].claimsTicket;
            casino[msg.sender].claimsTicket = 0;
            casino[msg.sender].claimsBond -= difference;
            casino[msg.sender].bondTicket++;
        }
        totalNodes++;
    }
    ...
    function updateClaims(address _address) internal {
        ...
        uint256 timerFrom = casino[_address].lastUpdate;
        if (timerFrom > 0)
            casino[_address].claimsTicket += casino[_address].ticket * nodeBase * dailyInterest * (time - timerFrom) / 8640000;
            casino[_address].claimsBond += casino[_address].bondTicket * nodeBase * dailyInterest * (time - timerFrom) / 8640000;
            casino[_address].lastUpdate = time;
    }

    function getTotalClaimable() public view returns (uint256) {
        uint256 time = block.timestamp;
        uint256 pendingGL = casino[msg.sender].ticket * nodeBase * dailyInterest * (time - casino[msg.sender].lastUpdate) / 8640000;
        uint256 pendingBond = casino[msg.sender].bondTicket * nodeBase * dailyInterest * (time - casino[msg.sender].lastUpdate) / 8640000;
        uint256 pending = pendingGL + pendingBond;
        return casino[msg.sender].claimsTicket + casino[msg.sender].claimsBond + pending;
	}
    ...
    function getTaxEstimate() external view returns (uint256) {
        ...
        uint256 pendingGL = casino[msg.sender].ticket * nodeBase * dailyInterest * (time - casino[msg.sender].lastUpdate) / 8640000;
        uint256 pendingBond = casino[msg.sender].bondTicket * nodeBase * dailyInterest * (time - casino[msg.sender].lastUpdate) / 8640000;
        uint256 claimableMilk = pendingGL + casino[msg.sender].claimsTicket;
        uint256 claimableBond = pendingBond + casino[msg.sender].claimsBond;
        uint256 taxGL = claimableMilk / 100 * claimTaxGl;
        uint256 taxBond = claimableBond / 100 * claimTaxBond;
        return taxGL + taxBond;
	}

    function calculateTax() public returns (uint256) {
        updateClaims(msg.sender);
        uint256 taxGL = casino[msg.sender].claimsTicket / 100 * claimTaxGl;
        uint256 taxBond = casino[msg.sender].claimsBond / 100 * claimTaxBond;
        uint256 tax = taxGL + taxBond;
        return tax;
    }
    ...
    function claim() external payable {
        ...
        require(casino[msg.sender].exists, "sender must be registered in the casino to claim yields");

        updateClaims(msg.sender);
        uint256 tax = calculateTax();
		uint256 reward = casino[msg.sender].claimsTicket + casino[msg.sender].claimsBond;
        uint256 toTreasury = tax;
        uint256 toFarmer = reward - tax;
		if (reward > 0) {
            casino[msg.sender].claimsTicket = 0;		
            casino[msg.sender].claimsBond = 0;
            _transfer(gl, msg.sender, toFarmer);
            _transfer(gl, address(treasury), toTreasury);
		}
	}
    ...
    function getOwnedNodes() external view returns (uint256) {
        uint256 ownedNodes = casino[msg.sender].ticket + casino[msg.sender].bondTicket;
        return ownedNodes;
    }
    ...
    function getGlClaimTax() external view returns (uint256) {
        return claimTaxGl;
    }
