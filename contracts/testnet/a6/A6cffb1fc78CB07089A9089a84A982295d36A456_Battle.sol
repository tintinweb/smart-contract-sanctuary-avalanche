...
// , VRFConsumerBaseV2
 {
    ...
    uint256[7] public registerAmount = [
        5 * 1e18,
        12 * 1e18,
        20 * 1e18,
        30 * 1e18,
    ...
    BattleStatus public versusBattleStatus;
    BattleStatus public royalBattleStatus;

    /// Head to Head Match Registers
    uint256[6][2] public versusFighters;
    /// Battle Royale Registers
    uint256[16] public royaleFighters;
    // Versus Battle Results
    bool[7][6] public versusRoomRoundResult; // True -> Player 1 win , False - Player 2 win
    bool[7] public versusRoomResult; // True -> Player 1 win , False - Player 2 win

    /// Battle Royale Spectators
    // mapping(uint256 => mapping(address => SpectatorOverview)) public spectators;
    SpectatorOverview[][] public spectators;
    ...
    function registerVersusBattle(uint256 _roomId, uint256 _tokenId) external {
        ...
        require(
            luchador.doesOwnLuchador(msg.sender, _tokenId),
            "Not owned this NFT"
        );
        ...
        require(
            luchador.luchadoresWeight(_tokenId) >= _roomId * 10 + 60 &&
                luchador.luchadoresWeight(_tokenId) <= _roomId * 10 + 70,
            "Not owned this NFT"
        );
        // Check if 2 positions are already registered
        require(versusFighters[_roomId][1] == 0, "Fully Registered");

        versusBattleStatus == BattleStatus.Started;
    ...
    function fightVersusBattle() public onlyOwner {
        ...
        for (uint8 i = 0; i < versusFighters.length; i++) {
            require(
                versusFighters[i][0] != 0 && versusFighters[i][1] != 1,
                "Not Fully Registered"
            );
        }

        versusBattleStatus == BattleStatus.Registered;
        ...
        uint8 _versusRoomResult;
        for (roomId = 0; roomId < versusFighters.length; roomId++) {
            randomNumber[0] = getRandomNumber();
            randomNumber[1] = getRandomNumber();
            for (roundId = 0; roundId < 7; roundId++) {
                if (randomNumber[0] == 20 || randomNumber[1] == 1) {
                    versusRoomRoundResult[roomId][roundId] = true;
                    _versusRoomResult += 1;
                } else if (randomNumber[0] == 1 || randomNumber[1] == 20) {
                    versusRoomRoundResult[roomId][roundId] = false;
                    _versusRoomResult -= 1;
                } else {
                    for (playerId = 0; playerId < 2; playerId++) {
                        playerAttributes[playerId] = luchador
                            .getAllLuchadorAttributes(
                                versusFighters[roomId][playerId]
                            );
                        roundMax[playerId] =
                            playerAttributes[playerId][0] *
                            playerAttributes[playerId][1] *
                            playerAttributes[playerId][2] *
                            randomNumber[0];
                    }
                ...
                if (_versusRoomResult > 0) versusRoomResult[roomId] = true;
            }
        }
        versusBattleStatus == BattleStatus.Finished;
        earnSpectaterWager();
    }
    ...
    function earnSpectaterWager() internal {
        ...
        for (uint8 roomId = 0; roomId < 6; roomId++) {
            for (
                uint8 spectatorId = 0;
                spectatorId < spectators[roomId].length;
                spectatorId++
            ) {
                if (
                    spectators[roomId][spectatorId].bet ==
                    versusRoomResult[roomId]
                ) {
                    uint256 spectatorPercent;
                    if (spectators[roomId][spectatorId].bet)
                        spectatorPercent =
                            spectators[roomId][spectatorId].betAmount /
                            wagerTotalAmount[roomId][1];
                    else
                        spectatorPercent =
                            spectators[roomId][spectatorId].betAmount /
                            wagerTotalAmount[roomId][0];
                    nbondToken.transfer(
                        spectators[roomId][spectatorId].account,
                        ((100 - FEE_INTEREST) *
                            spectatorPercent *
                            prizeAmount[roomId]) / 10000
                    );
                }
            }
        }
     ...
     */
    function setRegisterAmount(uint256 _roomId, uint256 _amount)
        external
        onlyOwner
    {
     ...
     */
    function setPrizeAmount(uint256 _roomId, uint256 _amount)
        external
        onlyOwner
    {
...
contract Luchador is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function mint(uint256 amount) external {
        ...
        require(
            ...
            "Not enough NFTs left to be minted"
        );
        require(
            amount <= unmintedTokenURIs.length,
            "Not enough unminted token URIs"
        );
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 selectedTokenURIIndex = uint256(
                keccak256(abi.encodePacked(block.timestamp + i))
            ) % unmintedTokenURIs.length;

            string memory selectedTokenURI = unmintedTokenURIs[
                selectedTokenURIIndex
            ];

            //We can't avoid executing those transactions for the last item so gas estimates are reliable
            string memory lastUnmintedTokenURI = unmintedTokenURIs[
                unmintedTokenURIs.length - 1
            ];
    ...
    function multisigMint(uint256 amount) external onlyOwner {
        require(
            amount <= multisigMaxMintCount,
            "Amount greater than remaining mint count"
        );
        require(
            unmintedTokenURIs.length >= amount,
            "Not enough unminted token URIs to fulfil multisig allocation"
        );
    ...
    function setTrainer(uint256 trainer, address trainerAddress)
        external
        onlyOwner
    {
    ...
    function addLuchadorWin(uint256 luchadorId) external {
        require(
            msg.sender == fightingContract,
            "Only the fighting contract can call this"
        );
    ...
    function doesOwnLuchador(address owner, uint256 luchadorId)
        public
        view
        returns (bool)
    {
    ...
    function setLuchadorName(uint256 luchadorId, string memory _luchadorName)
        external
    {
        require(
            doesOwnLuchador(msg.sender, luchadorId),
            "Luchador doesn't exist yet or sender not luchador owner"
        );
    ...
    function setLuchadorBio(uint256 luchadorId, string memory _luchadorBio)
        external
    {
        require(
            doesOwnLuchador(msg.sender, luchadorId),
    ...
    function getAllTrainers()
        public
        view
        returns (address[ATTRIBUTE_COUNT] memory)
    {
        return trainers;
    }

    function getAllLuchadorAttributes(uint256 luchadorId)
        public
        view
        returns (uint256[ATTRIBUTE_COUNT] memory)
    {
    ...
    function getLuchadoresData() public view returns (LuchadorData[] memory) {
        ...
        for (uint256 i = 1; i < _totalSupply + 1; i++) {
            ...
            luchadores[i - 1]
                .currentTrainerOwner = luchadoresCurrentTrainerOwner[i];
            luchadores[i - 1].locationStartTime = luchadoresLocationStartTime[
                i
            ];
    ...
    function _deleteSale(uint256 luchadorId) internal {
        ...
        uint256 lastLuchadorForSaleId = luchadoresForSale[
            luchadoresForSale.length - 1
        ];
    ...
    function buyLuchador(uint256 luchadorId, uint256 expectedPrice) external {
        ...
        require(
            ...
            "Buyer is already the luchador owner"
        );

        uint256 price = luchadoresPrice[luchadorId];
        require(
            price == expectedPrice,
            "Expected and actual price didn't match"
        );
    ...
    function sendToTrainer(
        ...
        require(
            ITrainer(trainers[trainer]).checkAllowed(trainerOwner, msg.sender),
            "Sender not allowed on trainer"
        );
    ...
    function canRemoveLuchadorFromTrainer(
        uint256 luchadorId,
        uint256 currentTrainer
        ...
        return
            ITrainer(trainers[currentTrainer]).isLuchadorInTrainer(
                luchadorId,
                msg.sender
            );
    }
    ...
    function removeFromTrainer(uint256 luchadorId) public {
        ...
        require(
            canRemoveLuchadorFromTrainer(luchadorId, currentTrainer),
            "Sender not allowed to remove luchador"
        );

        address trainerOwner = luchadoresCurrentTrainerOwner[luchadorId];
        ITrainer(trainers[currentTrainer]).removeLuchador(
            trainerOwner,
            luchadorId
        );

        uint256 elapsedSeconds = block.timestamp -
            luchadoresLocationStartTime[luchadorId];
        uint256 pointsEarned = elapsedSeconds *
            ((luchadorBaseWeight + luchadoresWeight[luchadorId]) /
                luchadorWeightModifier);

        pointsEarned *=
            ITrainer(trainers[currentTrainer]).trainersLevel(trainerOwner)**2;
