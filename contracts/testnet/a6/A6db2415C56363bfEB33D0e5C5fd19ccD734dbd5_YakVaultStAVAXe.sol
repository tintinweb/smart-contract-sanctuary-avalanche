...
// import "./interfaces/perp-vaults/IAction.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/perp-vaults/IWETH.sol";
...
contract YakVaultStAVAXe is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  /// @dev strategies that build up this vault
  ...
  /**
   * @notice allows anyone to close out the previous round by calling "closePositions" on all strategies.
   * @dev this does the following:
   * 1. calls closePositions on all the strategies withdraw the money from all the strategies
   * 2. pay all the fees
  ...
  function closePositions() public onlyLocked unlockState {
    // calls closePositions on all the strategies and transfers the assets back into the vault
    _closeAndWithdraw();
  ...
  /**
   * @notice distributes funds to each strategy and locks the vault
   */
  ...
  /**
   * @notice net assets controlled by this vault, which is effective balance + debts of strategies
   */
  ...
  /**
   * @notice iterrate through each strategy, close position and withdraw funds
   */
  function _closeAndWithdraw() internal {
    for (uint8 i = 0; i < strategies.length; i = i + 1) {
      ...
      // IAction(strategies[i]).closePosition();

      _closePositionAndUnlock(strategies[i]);

      // // 2. withdraw assets
      // uint256 strategyBalance = IERC20(asset).balanceOf(strategies[i]);
      // if (strategyBalance > 0) IERC20(asset).safeTransferFrom(strategies[i], address(this), strategyBalance);
    }
  }

  /**
   * @notice distributes the effective balance to different strategies
  ...
  function _distribute(uint256[] memory _percentages) internal nonReentrant {
    ...
    for (uint8 i = 0; i < strategies.length; i = i + 1) {
      ...
      if (newAmount > 0) {
        ...
        // IAction(strategies[i]).rolloverPosition();
        _rollOverAndLock();
      }
    }
   ...
   * @dev this function is called after withdrawing from strategy contracts and does the following:
