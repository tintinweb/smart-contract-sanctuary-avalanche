...
contract RangoCCTPFacet is IRango, ReentrancyGuard, IRangoCCTP {
    ...
    function initCCTP(address _messenger, address _transmitter) external {
        LibDiamond.enforceIsContractOwner();
        addCCTPBridgesInternal(_messenger, _transmitter);
    }
        ...
        internal
    {
        ...
        require(
            s.tokenMessenger == request.tokenMessengerAddress, 
    ...
    function ReceiveMessageForMint(
        ...
        CCTPStorage storage s = getCCTPStorage();
        require(
            s.messageTransmitter == transmitter, 
    ...
    function addCCTPBridgesInternal(address tokenMessenger, address messageTransmitter) private {
        CCTPStorage storage s = getCCTPStorage();
        require(tokenMessenger != address(0), "invalid address cctp!");
        require(messageTransmitter != address(0), "invalid address cctp!");
        s.tokenMessenger = tokenMessenger;
        s.messageTransmitter = messageTransmitter;
