...
contract Vault is ERC20, ReentrancyGuard, Pausable {
  ...
  function test1() external nonReentrant {
    IManager(manager).compound(address(this));
  }

  function test2(uint256 _depositAmt, uint256 _minSharesReceive) external nonReentrant returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    return (_depositValue, _currentEquity, _equityValue, _targetLpValue, _targetDebtValue);
  }
