...
contract Edaface721 is BEP721URIStorage {
    event Create(
        address indexed _to,
        uint256 indexed tokenId
    );
    ...
    function mint(string memory ipfsmetadata, address to, uint royal, uint256 id_, bool _listStatus) external {
    ...
    function mintListBatch(string[] memory ipfsmetadata, address to, uint256[] memory count, uint256 royal) external {
        require(msg.sender == owner, "Public Mint Not Available");
        for (uint256 i = 0; i < ipfsmetadata.length; i++) {
            _safeMint(owner, count[i]);
            _setTokenURI(count[i], ipfsmetadata[i]);
            _creator[count[i]] = to;
            _royal[count[i]]=royal;
            emit Create(msg.sender, count[i]);
        }
    }
