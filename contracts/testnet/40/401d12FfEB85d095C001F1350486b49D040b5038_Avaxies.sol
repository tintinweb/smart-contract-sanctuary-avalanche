...
contract Avaxies is ERC721Enumerable, Ownable {
  ...
  function setTraitNames(uint256 _index, string[] memory _names) public onlyOwner {
    require(_index < NUM_TRAITS, "Out of trait index");

    bytes32[] memory names_ = new bytes32[](_names.length);
    for (uint256 i = 0; i < _names.length; i++) {
      names_[i] = stringToBytes32(_names[i]);
    }
    traitNames[_index] = names_;
  }
