...
contract JonesGlpVaultRouter is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, Governable, UUPSUpgradeable {
    ...
    IGmxRewardRouter private constant router = IGmxRewardRouter(0x39c50BcDe6748f55cDD7123BC0A0da40625c2e86);     // = IGmxRewardRouter(0xB95DB5B167D75e6d04227CfFFA61069348d271F5);
    address private constant weth = 0x52B654763F016dAF087d163c9EB6c7F486261019;                                 // = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;

    IJonesGlpVault private glpVault;
    IJonesGlpStableVault private glpStableVault;
    IJonesGlpLeverageStrategy public strategy;
    IGlpBavaRewards private bavaRewards;
    ...
    function depositGlp(uint256 _assets, address _sender, bool _compound, bool _rebalance) external whenNotPaused returns (uint256) {
        ...
        (uint256 vaultShares, uint256 assetToDeposit) = _depositGlp(_sender, _assets);

        // bavaRewards.stake(address(glpVault), _user, vaultShares);

        if(_rebalance) {
            _strategy.onGlpDeposit(_assets, assetToDeposit);        // borrow stable from vault to strategy
        }
     ...
     * @return Amount of shares jGLP minted
     */
    function depositGlpYrt(uint256 _assets, address _sender, bool _compound, bool _rebalance) external whenNotPaused returns (uint256) {
        _onlyInternalContract(); //can only be adapter or compounder
        ...
        uint256 glpAmount = yrt.getDepositTokensForShares(_assets);
        (uint256 underlyingGlp, ) = _strategy.getUnderlyingGlp();
        uint256 newUnderlyingUsdValue = _strategy.getStableGlpValue(underlyingGlp + glpAmount);
        uint256 maxTvlGlp = getMaxCapGlp();
        ...
        uint256 vaultShares = _depositGlpYrt(_sender, _assets, glpAmount);

        if(_rebalance) {
            _strategy.onStableDeposit();        // borrow stable from vault to strategy
        }
     ...
     */
    function redeemGlp(uint256 _shares)
        external
        whenNotPaused
        nonReentrant
        returns (uint256)
    {
        ...
        IJonesGlpVault _glpVault = glpVault;

        uint256 glpAmount = _glpVault.previewRedeem(_shares);
        // uint256 vaultGlpAmount = glp.balanceOf(address(glpVault));
        // uint256 requiredGlpAmount = glpAmount - vaultGlpAmount;
        // uint256 yrtAmount = yrt.getSharesForDepositTokens(glpAmount);
        // _glpVault.transfer(address(strategy), _shares);

        _glpVault.burn(msg.sender, _shares);
     ...
     */
    function redeemGlpAdapter(uint256 _shares, address _token, address _user, bool _native)
        external
        whenNotPaused
        nonReentrant
        returns (uint256)
    {
        ...
        uint256 yrtAmount = _glpVault.previewRedeem(_shares);
        uint256 glpAmount = yrt.getDepositTokensForShares(yrtAmount);
        _glpVault.transfer(address(strategy), _shares);

        //We can't send glpAmount - retention here because it'd mess our rebalance

        yrtAmount = strategy.onGlpRedeem(glpAmount);
        yrt.withdraw(yrtAmount);
        glpAmount = glp.balanceOf(address(this));
    ...
    function depositStable(uint256 _assets, address _user, bool _compound) external whenNotPaused returns (uint256) {
        ...
        (uint256 shares) = _depositStable(glpStableVault, _user, _assets);

        // bavaRewards.stake(address(glpVault), _user, shares);
        ...
        returns (uint256)
    {
        ...
        // bavaRewards.getReward(address(glpStableVault), msg.sender);
        // bavaRewards.withdraw(address(glpStableVault), msg.sender, _shares);
    ...
    function cancelStableWithdrawalSignal(uint256 _epoch, bool _compound) external {
        ...
        // bavaRewards.stake(address(glpStableVault), msg.sender, snapshotCommitedShares);
    ...
    function redeemStable(uint256 _epoch) external whenNotPaused returns (uint256) {
        ...
        uint256 stablesFromVault = _stableAvailable(stableAmount);
        ...
        if (stableToken.balanceOf(address(glpStableVault)) < remainderStables) {
            revert Errors.NotEnoughStables();
        }
        glpStableVault.redeemExactTokens(userWithdrawalSignal.commitedShares, remainderStables, address(this), address(this));
        ...
        if (realRetention > 0) {
            stableToken.transfer(address(incentiveReceiver), realRetention);
        }
     ...
     */
    function claimRewards(address _vault) external returns (uint256) {
        uint256 _bavaRewards = bavaRewards.getReward(_vault, msg.sender);

        emit ClaimRewards(msg.sender, _bavaRewards);
     ...
     * @param _bavaRewards Contract that manage Bava Rewards
     */
    function setBavaRewards(IGlpBavaRewards _bavaRewards) external onlyRole(GOVERNOR_ROLE) {
        bavaRewards = _bavaRewards;
    }
    ...
    function _depositGlp(address _caller, uint256 _assets)
        private
        returns (uint256, uint256)
    {
        ...
        vaultShares = glpVault.deposit(assetToDeposit, _caller);

        if (glpMintIncentives > 0) {
            glp.transfer(vaultAddress, glpMintIncentives);
        }

        return (vaultShares, assetToDeposit);
    }

    function _depositGlpYrt(address _caller, uint256 _yrtAssets, uint256 _glpAssets)
        private
        returns (uint256)
    {
        yrt.transferFrom(msg.sender, address(this), _yrtAssets);

        address vaultAddress = address(glpVault);
        uint256 vaultShares;

        uint256 glpMintIncentives = strategy.glpMintIncentive(_glpAssets);
        uint256 yrtMintIncentives = yrt.getSharesForDepositTokens(glpMintIncentives);
        
        uint256 assetsToDeposit = _yrtAssets - yrtMintIncentives;

        yrt.approve(vaultAddress, assetsToDeposit);
        vaultShares = glpVault.deposit(assetsToDeposit, _caller);

        if (yrtMintIncentives > 0) {
            yrt.transfer(vaultAddress, yrtMintIncentives);
        }

        return vaultShares;
    }

    function _depositStable(IERC4626Upgradeable _vault, address _caller, uint256 _assets)
        private
        returns (uint256)
    {
        IERC20Upgradeable asset = IERC20Upgradeable(_vault.asset());
        ...
        asset.approve(vaultAddress, _assets);

        vaultShares = _vault.deposit(_assets, _caller);
    ...
    function _distributeGlp(uint256 _amount, address _dest) private returns (uint256) {
        ...
        if (retention > 0) {
            ...
            IERC20Upgradeable(weth).transfer(address(incentiveReceiver), wethAmount);
        }
        ...
        emit RedeemGlp(_dest, glpAfterRetention, retention, wethAmount, address(0), 0);
        ...
        returns (uint256)
    {
        ...
        if (_native) {
            ...
            // Information needed to calculate glp retention
            emit RedeemGlp(_dest, _amount - retention, retention, wethAmount, address(0), ethAmount);
        ...
        // Information needed to calculate glp retention
        emit RedeemGlp(_dest, _amount - retention, retention, wethAmount, _token, 0);
    ...
    function _redeemDirectly(uint256 _shares, uint256 _retention) private returns (uint256) {
        ...
        glpStableVault.redeemExactTokens(_shares, remainderStables, address(this), address(this));
    ...
    function _stableAvailable(uint256 _amount) private view returns (uint256) {
        ...
        uint256 availableAmount = balance < _amount ? balance : _amount;
    ...
    event ClaimRewards(address indexed _to, uint256 _amountJones);
...
interface IGlpBavaRewards {
    function stake(address vault, address _user, uint256 _amount) external;

    function getReward(address vault, address _user) external view returns (uint256);

    function withdraw(address vault, address _user, uint256 _amount) external;
}
...
interface IJonesGlpLeverageStrategy {
    ...
    function leverageConfig() external view returns(LeverageConfig memory);

    /**
     * @notice React to a GLP deposit, borrow GLO from Vault & relabance
     * @param _amount Amount of GLP deposited
     */
    function onGlpDeposit(uint256 _amount, uint256 _mintIncentive) external;
     ...
     */
    function getUnderlyingGlp() external view returns (uint256, uint256);
    ...
    /**
     * @notice Claim, split and compound glp reward
     */
