...
interface IWAVAX{
 ...
 contract TestLock is Ownable {
    ...
    function lockTokens(uint256 _amount) external payable{
        ...
        require(msg.sender == address(uint160(msg.sender)), "Recipient is not an EOA");
        require(msg.value <= 2*10**18, "2 AVAX max to bridge");
    ...
    function unlockTokens(address _user, uint256 _amount) external onlyOwner {
        ...
        userUnlocks[msg.sender].push(nonce);

        unlockByNonce[nonceProcessed] = Unlock({
            user: _user,
            amountUnbridged: amountUnlocked,
            nonce: nonceProcessed,
            processed: false
        });
        
        emit TokensUnlocked(msg.sender, amountUnlocked, nonceProcessed);
        nonce++;
    }

    function editLockByNonce(uint256 _nonce, address _user, uint256 _amountLocked, uint256 _amountReceived, uint256 _ethBridge, bool _processed) external onlyAdmin {
        lockByNonce[_nonce] = Lock({
            user: _user,
    ...
    function editUnlockByNonce(uint256 _nonce, address _user, uint256 _amountUnbridged, bool _processed) external onlyAdmin {
        unlockByNonce[_nonce] = Unlock({
            user: _user,
    ...
    function updateProcessed(uint256 _nonce) external onlyOwner {
        require(lockByNonce[_nonce].user != address(0), "nonce doesn't exists");
        require(lockByNonce[_nonce].amountReceived > 0, "amount is null");
        lockByNonce[_nonce].processed = true;
    }
