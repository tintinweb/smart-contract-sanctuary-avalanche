...
contract BridgesOfFate is Ownable, ReentrancyGuard {
    ...
    function _updatePlayer(bytes32 _playerId) internal {
        GameItem storage _member = PlayerItem[_playerId];
        _member.day = 0;    
        _member.stage = 0;
        _member.startAt = 0;
    ...
    {
        uint256 stageRandomNumber = GameMap[stageNumber]; 
    ...
    function entryFeeForSeriesOne(uint256 _nftId) public GameInitialized GameEndRules
    {
        ...
        if (_member.userWalletAddress != address(0)) {
            ...
            require(dayDifferance(block.timestamp, _admin.startAt) > _member.day, "Alreday In Game");
            require(checkSide(_member.stage, _member.lastJumpSide) == false, "Already In Game");
            require(dayDifferance(_member.lastJumpTime,_admin.startAt) + 1 < dayDifferance(block.timestamp, _admin.startAt),"You Can only use Buy back in 24 hours");
    ...
    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  GameInitialized GameEndRules {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public GameInitialized GameEndRules
    {
        ...
        if((_gameStatus.stageNumber != STAGES) && (dayDifferance(block.timestamp,_gameStatus.startAt) > _gameStatus.lastUpdationDay)){
    ...
    function restartGame(uint256 _nftId, uint8 seriesType) external {
        ...
        require(dayDifferance(block.timestamp, GameStatusInitialized[latestGameId].startAt) > _member.day, "Alreday In Game");
        require(checkSide(_member.stage, _member.lastJumpSide) == false, "Already In Game");
        _updatePlayer(playerId);
        _member.nftId = _nftId;
        _member.userWalletAddress = msg.sender;
