...
contract NFKeyStakingUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function getChestTier(uint tokenID, uint _time) public view returns (uint) {
        ...
        if (chestTier > 0 && _time > timeToTreasureUnlock) return chestTier - 1;
        return chestTier;
    }
    ...
    function calcBaseProbToOpenByTier(uint tier, uint level) public pure returns (uint) {
        if (tier == 0) {
            if (level > 10) return 875;
            else return 1000;
        }
        return (1000 - (1250 * tier / 10));
    }

    function calcBaseProbToOpen(uint tokenID, uint time) public view returns (uint) {
        return calcBaseProbToOpenByTier(getChestTier(tokenID, time), NFKeyUpgradeableV1(_nfkeyContractAddress).getLevel(tokenID));
    }
    ...
    function updateTreasureRewardTime(uint tokenID) private {
        treasureRewardAvailableList[tokenID] = block.timestamp + (calcTimeCountDown(getChestTier(tokenID, block.timestamp)) * 60 * 60);
    }
...
contract WhitelistUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function removeWhitelistZone(address account, uint zone) public onlyAdmin {
        if (zone == 1 || zone == 2) zoneList[account][zone] = false;
    }
}
