...
contract BEEGART is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC2981, ClampedRandomizer {
    constructor() 
        ERC721("BEEG ART", "BA") ClampedRandomizer(maxSupply) {
    ...
    uint maxSupply = 100;
    uint maxPerTx = 1;
    uint maxPerNonTeam = 1;
    uint price = 0;
    uint public whitelistTime= 1671112800; // Dez 15 2022 24:00:00 GMT+0000
    uint public publicTime= 1671113400; //Dez 16 2022 24:30:00 GMT+0000
    address public royaltyAddress=0x0961754A17b0F97A16244FA19e7Df80D139525Ad;
    uint96 public royalty=969;
    string public baseURI = "";
    string public unrevealedURI = "ipfs://QmRtL6UeG8QcvBepUTZXDbCK8BfuP9XWoD65SeVGekzKkH/hidden.json";
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        //mint start
        require(block.timestamp >= whitelistTime, "Its not time to go Beeg Art yet");
        
        //supply limits
        uint supply = totalSupply();
        require(supply + amount <= maxSupply, "No more Beeg Art left for you");

        //transaction limits
        require(amount > 0,"you want to mint more than just air right?");
        require(amount <= maxPerTx, "Thats too many Beeg Arts at once");

        //mint limits
        require(whiteListedCount[msg.sender] < maxPerNonTeam, "You can have only 1 Beeg Art");
        ...
        {
            require(whiteListed[msg.sender] ==true, "Silly Beeg Art are for WL only right now");
        ...
        if (block.timestamp >= publicTime)
        {
            require(publicCount[msg.sender] < maxPerNonTeam, "You can have only 1 Beeg Art");   
        }
    ...
    {
        uint supply = totalSupply();
        require(supply + 1 <= maxSupply, "No more Beeg Art left for you");
        uint256 tokenId=_genClampedNonce()+1;//+1 since we dont want to deal with token id 0
        _isMinted[tokenId]=true;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(tokenId.toString(), ".json")));
