...
contract MultiRewarderPerSec is IMultiRewarder, Ownable, ReentrancyGuard {
    ...
    function setRewardRate(uint256 _tokenId, uint96 _tokenPerSec) external onlyOperatorOrOwner {
        ...
        poolInfos[_tokenId].tokenPerSec = _tokenPerSec;

        emit RewardRateUpdated(address(poolInfos[_tokenId].rewardToken), oldRate, _tokenPerSec);
    }
    ...
    function onPtpReward(
        ...
        for (uint256 i; i < length; ++i) {
            PoolInfo storage pool = poolInfos[i];
            UserInfo storage user = userInfos[i][_user];
