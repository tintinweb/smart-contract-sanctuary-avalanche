...
contract Ownable is Context {
    ...
    function transferOwnership(address payable adr) public onlyOwner {
        emit OwnershipTransferred(address(_owner), adr);
        authorizations[adr] = true;
        _owner = adr;
    }
...
contract Post is ERC721A, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    {
        uint256 count = this.balanceOf(account);
        uint256[] memory _ids = new uint256[](count);

        for(uint256 i=0 ; i<count ; i++) {
            _ids[i] = this.tokenOfOwnerByIndex(account, i);
        }
