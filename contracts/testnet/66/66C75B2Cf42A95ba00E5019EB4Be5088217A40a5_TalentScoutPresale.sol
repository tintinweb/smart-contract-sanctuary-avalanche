...
// File: contracts/TalentScoutPresale.sol
...
contract TalentScoutPresale is ReentrancyGuard, Ownable {
    ...
    uint256 public supply = 100_000_000_000 ether;
    uint256 public distributed;

    uint256 public salePriceUsd = 12500000000000;

    mapping(address => uint256) public toRefund;
    mapping(address => uint256) public toSale;
    ...
    constructor(
        ...
        require(_talentScout != address(0), "!nonzero");
        require(
    ...
    function salePriceAvax() public view returns(uint256) {
      ...
      uint256 priceInAvax = uint256(avaxPriceUsd).mul(1e10).mul(salePriceUsd).div(1e18);
      return priceInAvax;
    }
    ...
    function buy() public payable nonReentrant {
        ...
        toSale[msg.sender] = toSale[msg.sender].add(tokensToBuy);
    ...
    function saleLock(address _account, uint256 _scoutToLock) private {
        ...
        uint256 immediatelyReleased = _scoutToLock / 10; // * 10 / 100 = / 10

        // 15% released after 3 months
        uint256 m3Release = (_scoutToLock * 15) / 100;

        // 25% released after 9 months
        uint256 m9Release = (_scoutToLock * 25) / 100;

        // 25% released after 15 months
        uint256 m15Release = (_scoutToLock * 25) / 100;

        // 25% released after 21 months
        uint256 m21Release = _scoutToLock -
            (immediatelyReleased + m3Release + m9Release + m15Release);

        talentScout.insertLock(_account, immediatelyReleased, 0);
        talentScout.insertLock(_account, m3Release, 90 days);
        talentScout.insertLock(_account, m9Release, 270 days);
        talentScout.insertLock(_account, m15Release, 450 days);
        talentScout.insertLock(_account, m21Release, 630 days);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (newMinted >= supply) {
            ...
            exceedingEther = etherUsed.add(
                (exceedingTokens.mul(1e18).div(salePrice))
            );
