...
contract DutchAuctionMarketplace is Initializable, ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // events
    event DutchAuctionWithdrawn(address indexed sender, address indexed collection, uint256 indexed tokenId);
    event DutchAuctionMadeBid(address indexed sender, address indexed collection, uint256 indexed tokenId, uint256 value, uint256 amount);
    ...
    function initialize() public initializer {
        ...
        minimumDropInterval = 60; // 1 minute
    }
    ...
    external whenNotPaused isAuctionNotStartedByOwner(_nftContractAddress, _tokenId)
    ...
    function _configureAuction(
        ...
        uint128 _endPrice,
        uint64 _startTime,
        LibShareholder.Shareholder[] memory _shareholders) internal {
        _setShareholders(_nftContractAddress, _tokenId, _shareholders);
        dutchAuctions[_nftContractAddress][_tokenId].startTime = _startTime > 0 ? _startTime : uint64(block.timestamp);
    ...
    paymentAccepted
    {
        address seller = ownerOfNFT(_nftContractAddress, _tokenId);
        ...
        if (msg.value > amount) {
            transferBidSafely(msg.sender, msg.value - amount);
        }
        ...
        emit DutchAuctionMadeBid(msg.sender, _nftContractAddress, _tokenId, msg.value, amount);
    }
    ...
    onlyNftSeller(_nftContractAddress, _tokenId)
    {
        ...
        emit DutchAuctionWithdrawn(msg.sender, _nftContractAddress, _tokenId);
    }
    ...
    function _setShareholders(address _nftContractAddress, uint256 _tokenId, LibShareholder.Shareholder[] memory _shareholders) internal {
        ...
        dutchAuctions[_nftContractAddress][_tokenId].shareholderSize = j + 1;
    }
