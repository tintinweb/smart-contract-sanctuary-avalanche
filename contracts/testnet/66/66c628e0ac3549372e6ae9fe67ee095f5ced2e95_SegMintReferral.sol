...
contract SegMintReferral is Ownable {
    ...
    mapping (address => bool) private _hasUserCreatedReferral;
    ...
    function hasUserCreatedReferral(address account) external view returns (bool) {
        return _hasUserCreatedReferral[account];
    }
    ...
    function createReferralId(string memory id) external {
        ...
        require(!_hasUserCreatedReferral[msg.sender], "You have already created a referral");
        require(!getIdRegistered(toLowerCase(id)), "Id Already Registered");
        idRegistered[toLowerCase(id)] = true;
        _hasUserCreatedReferral[msg.sender] = true;
    ...
    function sendTokens(address[] memory to, address[] memory from) external onlyOwner {
        for (uint i = 0; i < to.length; i ++) {
            ...
            _SGLT.transfer(to[i], referralBonus);
        }
    }
