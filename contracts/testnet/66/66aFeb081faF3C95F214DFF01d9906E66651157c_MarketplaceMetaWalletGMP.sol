...
contract MarketplaceMetaWalletGMP is ReentrancyGuard, IAxelarExecutable, Ownable {
  ...
  constructor(address _master, address _gateway, address _gasReceiver, address _WETH) IAxelarExecutable(_gateway) {
    ...
    whitelisted[keccak256(bytes(addressthis))] = true;
    weth = IWETH(_WETH);
  }

  function addWhitelist(string memory addr) public onlyOwner {
    whitelisted[keccak256(bytes(addr))] = true;
  }
  ...
  function _executeWithToken(
    ...
    require(whitelisted[keccak256(bytes(sourceAddress))], "Fake");
