...
/**
 * @title BasicIssuanceWithFeeModule
 * @author Domani Protocol
 *
 * Module that enables issuance and redemption functionality on a DomaniToken with issue/redeem fees. This is a module that is
 * required to bring the totalSupply of a Set above 0.
 */
contract BasicIssuanceWithFeeModule is ModuleBase, ReentrancyGuard {
  using Invoke for IDomani;
  using Position for IDomani.Position;
  using Position for IDomani;
  ...
  struct FeeState {
    ...
    uint64 issueFeePercentage;
    uint64 redeemFeePercentage;
  }
  ...
  event FeeSettled(address indexed _setToken, uint256 _managerFee, uint256 _protocolFee);
  event IssueRedeemFeeUpdated(
    address indexed _setToken,
    uint64 _issueFeePercentage,
    uint64 _redeemFeePercentage
  );
  event FeeRecipientUpdated(address indexed _setToken, address _newFeeRecipient);

  /* ============ Constants ============ */

  uint256 private constant PROTOCOL_ISSUE_FEE_INDEX = 0;
  uint256 private constant PROTOCOL_REDEEM_FEE_INDEX = 1;

  /* ============ State Variables ============ */

  // Mapping of SetToken to Issuance hook configurations
  mapping(IDomani => IManagerIssuanceHook) public managerIssuanceHook;

  // Mapping of SetToken to issue/redeem fees
  mapping(IDomani => FeeState) public feeStates;
  ...
  function issue(
    ...
    // Calculate fees
    uint256 feeQuantity = _quantity.preciseMul(_issueFeePercentage(_setToken));
    userQuantity = _quantity.sub(feeQuantity);

    // Mint the SetToken
    _setToken.mint(_to, userQuantity);

    if (feeQuantity > 0) {
      (managerFee, protocolFee) = _settleManagerAndProtocolFee(_setToken, feeQuantity, true);
    }

    emit SetTokenIssued(address(_setToken), msg.sender, _to, hookContract, userQuantity);
    emit FeeSettled(address(_setToken), managerFee, protocolFee);
  }
  ...
  function redeem(
    ...
    uint256 protocolFee;
    {
      ...
      _setToken.safeTransferFrom(msg.sender, address(this), _quantity);

      // Calculate fees
      uint256 feeQuantity = _quantity.preciseMul(_redeemFeePercentage(_setToken));
      userQuantity = _quantity.sub(feeQuantity);

      // Burn the SetToken - ERC20's internal burn already checks that the user has enough balance
      _setToken.burn(address(this), userQuantity);

      if (feeQuantity > 0) {
        (managerFee, protocolFee) = _settleManagerAndProtocolFee(_setToken, feeQuantity, false);
      }
    }
    ...
    for (uint256 i = 0; i < components.length; i++) {
      ...
      uint256 componentQuantity = userQuantity.preciseMul(unit);
    ...
    emit SetTokenRedeemed(address(_setToken), msg.sender, _to, userQuantity);
    emit FeeSettled(address(_setToken), managerFee, protocolFee);
  }

  /**
   * Initializes this module to the SetToken with issuance-related hooks and issue/redeem fees. Only callable by the SetToken's manager.
  ...
  function initialize(
    ...
    FeeState calldata _settings
  ) external onlySetManager(_setToken, msg.sender) onlyValidAndPendingSet(_setToken) {
    ...
    _checkFees(_settings.issueFeePercentage, _settings.redeemFeePercentage);
  ...
  /**
   * Edit issue/redeem fees of the fund. Only callable by the SetToken's manager.
   *
   * @param _setToken             Instance of the SetToken to issue
   * @param _issueFeePercentage             Issue fee of the fund
   * @param _redeemFeePercentage         Redeem fee of the fund
   */
  function updatesFees(
    IDomani _setToken,
    uint64 _issueFeePercentage,
    uint64 _redeemFeePercentage
  ) external onlySetManager(_setToken, msg.sender) onlyValidAndInitializedSet(_setToken) {
    _checkFees(_issueFeePercentage, _redeemFeePercentage);

    feeStates[_setToken].issueFeePercentage = _issueFeePercentage;
    feeStates[_setToken].redeemFeePercentage = _redeemFeePercentage;

    emit IssueRedeemFeeUpdated(address(_setToken), _issueFeePercentage, _redeemFeePercentage);
  }
   ...
   */
  function updateFeeRecipient(IDomani _setToken, address _newFeeRecipient)
    external
  ...
  function removeModule() external override {
    revert("The BasicIssuanceWithFeeModule module cannot be removed");
  }
  ...
  /**
   * Mints or transfer sets to both the manager and the protocol. Protocol takes a percentage fee of the total amount of Sets
   * minted or transferred to the manager.
   *
   * @param   _setToken               SetToken instance
   * @param   _feeQuantity            Amount of Sets to be minted as fees
   * @param   _isIssue                True if issue, false if redeem
   * @return  uint256                 Amount of Sets to manager as fee
   * @return  uint256                 Amount of Sets to protocol as fee
   */
  function _settleManagerAndProtocolFee(
    IDomani _setToken,
    uint256 _feeQuantity,
    bool _isIssue
    ...
    uint256 protocolFee = controller.getModuleFee(
      address(this),
      _isIssue ? PROTOCOL_ISSUE_FEE_INDEX : PROTOCOL_REDEEM_FEE_INDEX
    );
    ...
    _isIssue
      ...
      : _setToken.safeTransfer(_feeRecipient(_setToken), managerFeeAmount);

    if (protocolFeeAmount > 0) {
      _isIssue
        ? _setToken.mint(protocolFeeRecipient, protocolFeeAmount)
        : _setToken.safeTransfer(protocolFeeRecipient, protocolFeeAmount);
    }

    return (managerFeeAmount, protocolFeeAmount);
  }

  function _feeRecipient(IDomani _set) internal view returns (address) {
    return feeStates[_set].feeRecipient;
  }

  function _issueFeePercentage(IDomani _set) internal view returns (uint256) {
    return feeStates[_set].issueFeePercentage;
  }

  function _redeemFeePercentage(IDomani _set) internal view returns (uint256) {
    return feeStates[_set].redeemFeePercentage;
  }

  /**
   * Check fees are below 100%
   */
  function _checkFees(uint64 _issueFeePercentage, uint64 _redeemFeePercentage) internal pure {
    require(_issueFeePercentage < PreciseUnitMath.preciseUnit(), "Issue fee must be < 100%.");
    require(_redeemFeePercentage < PreciseUnitMath.preciseUnit(), "Redeem fee must be < 100%.");
  }
}