...
contract ArtMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    // mapping of addresses to UserInfo structs, which holds information about each user in the pool
    ...
    uint256 private constant ACC_TOKEN_PRECISION = 1e24;
    ...
    event InitialFloorPriceUpdated(address indexed collection, uint256 initialFloorPrice);
    event EndRewardPeriod(address indexed lpAddress, uint256 tokenId);
    event PaymentManagerSet(address indexed paymentManager);
    event RewardGenerationRateUpdated(address indexed lpAddress, uint256 rewardGenerationRate);
    event FailedArtBalanceWithdrawed(address indexed lpAddress, address indexed user, uint256 amount);
    ...
    */
    ...
    /**
	* @notice allows owner to update the reward generation rate. 
    * @param _lpAddress the address of the liquidity pool
    */
    function updateRewardGenerationRate(address _lpAddress, uint256 _rewardGenerationRate) external onlyOwner {
        _updatePool(_lpAddress);
        pools[_lpAddress].rewardGenerationRate = _rewardGenerationRate;
        emit RewardGenerationRateUpdated(_lpAddress, _rewardGenerationRate);
    }
    ...
    */
    function harvest(address _lpAddress) external {
        _updatePool(_lpAddress);
        uint256 previousRewardDebt = users[_lpAddress][msg.sender].rewardDebt;
        ...
        uint256 pending = ((users[_lpAddress][msg.sender].rewardableNFTCount * pools[_lpAddress].accARTPerShare) / ACC_TOKEN_PRECISION) - previousRewardDebt;

        emit Harvest(msg.sender, _lpAddress, pending, true);
    ...
    */
    ...
    /**
    * @notice allows the owner of the smart contract to update the initial floor price of an NFT in a specific liquidity pool.
    * @param _lpAddress the address of the liquidity pool
    * @param _initialFloorPrice the new initial floor price
    */
    function updateInitialFloorPrice(address _lpAddress, uint256 _initialFloorPrice) external onlyOwner {
        require(lpCollections.contains(_lpAddress), "The provided liquidity pool does not exist");

        pools[_lpAddress].initialFloorPrice = _initialFloorPrice;
        emit InitialFloorPriceUpdated(_lpAddress, _initialFloorPrice);
    }
    ...
    * @param _floorPriceThresholdNodeCount the floor price threshold node count
    ...
    */
    function depositNFT(
    ...
    ) external priceAccepted(_price) whenNotPaused nonReentrant {
        require(lpCollections.contains(_lpAddress), "The provided liquidity pool does not exist");
        ...
        if (pool.activeNodeCount > 0 && pool.activeNodeCount >= pool.floorPriceThresholdNodeCount) {
    ...
    * @param _user the address of the user
    */
    function pendingRewards(address _lpAddress, address _user)
    external
    view
    returns (uint256)
    {
    ...
    */
    function withdrawNFT(address _lpAddress, uint256 _tokenId) external whenNotPaused nonReentrant {
        ...
        require(block.timestamp - listedNFT.startedAt > pools[_lpAddress].lockDuration, "The minimum withdrawal duration has not been met, the NFT cannot be withdrawn yet.");
        ...
        if (listedNFT.isRewardable) {
            users[_lpAddress][msg.sender].rewardableNFTCount = user.rewardableNFTCount - 1;
    ...
    */
    function buyNFT(address _lpAddress, uint256 _tokenId) external payable nonReentrant whenNotPaused {
        ...
        IPaymentManager(paymentManager).payout{ value: listedNFT.price }(
            ...
            _tokenId,
            _shareholders,
            pools[_lpAddress].commissionPercentage
        );
    ...
    function withdrawFailedARTBalance(address _lpAddress, address _receiver) external {
        uint256 amount = users[_lpAddress][_receiver].failedBalance;
        if (amount > 0) {
            art.safeTransfer(_receiver, amount);
            emit FailedArtBalanceWithdrawed(_lpAddress, _receiver, amount);
        }
    }
    ...
    */
    function endRewardPeriod(address _lpAddress, uint256 _tokenId) public whenNotPaused {
        ...
        PoolInfo memory pool = pools[_lpAddress];
        require(block.timestamp - listedNFT.startedAt > pool.lockDuration, "The NFT's reward period has not yet expired");
    ...
    * @param _tokenId NFT's tokenId
    */
    function _getShareholders(address _lpAddress, uint256 _tokenId)
    internal
    view
