...
interface IGame {
    function receiveRandom(uint _reqId, uint _randWords) external;
}
    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    {
        (, int nativeUsdcFeed, , , ) = AggregatorV3Interface(
            nativeUsdcDatafeedContract
        ).latestRoundData();
        return (requestUsdcPrice * 10 ** 18) / uint(nativeUsdcFeed);
    }
    ...
    function requestRandomWords(
        ...
        if (gameMatchIdToReqId[_msgSender()][_matchId] != 0) {
            uint previousReqId = gameMatchIdToReqId[_msgSender()][_matchId];
            require(reqIdToRandom[previousReqId] == 0);
        }
    ...
    function fulfillRandomWords(
        ...
        // IGame(gameContract).receiveRandom(matchId, _randWords[0]);
    }
