...
abstract contract feeManager is Context {
    ...
    function updateRefund(address _account,uint256 _amount,bool _bool) virtual  external;
   }
...
contract NebulaProtoStarDrop is Ownable{
	...
	function payFeeToken(uint256 _intervals) payable external{
	    ...
	    theBigCheck(msg.sender,total,total,"crrrt");
	...
	function createProtoFeeToken(string memory _name) payable external{
		address _account = msg.sender;
		theBigCheck(msg.sender,tokFee,tokFee,_name);
		uint256 allowance = feeTok.allowance(_account, treasury);
	...
	function payFeesAvax(uint256 _intervals) payable external{
		...
		uint256 total = nebuLib.safeMuls(740000000000000000,_intervals);
		uint256 sendback = sent-total;
		theBigCheck(msg.sender,nebuLib.safeMuls(740000000000000000,_intervals),msg.value,"crrrt");
		updateClaimed(_account);
		feeMGR.reciever{ value: fee}(_account,fee);
		if(sendback > 0){
		...
		theBigCheck(msg.sender,fee,msg.value,_name);
		feeMGR.addProto(_account,_name);
		feeMGR.reciever{ value: fee}(_account,fee);
		if(sendback > 0){
			payable(_account).transfer(sendback);
	...
	function theBigCheck(address _account,uint256 _fee,uint256 _sent,string memory _name) public{
		uint256 sendback;
		...
		getRemaining(_account);
    		if(isFreeClaim(_account)==true){
    			sendback =_sent;
    			feeMGR.updateRefund(_account,sendback,true);
    		}else{
    			require(_sent >= _fee,"you have not sent enough to cover this claim");
    			sendback =_sent-_fee;
