...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, uint _nativeFee) internal virtual {
        ...
        console.log("desct chain id", _dstChainId);
        console.log("trustedRemote", trustedRemote.length);
...
abstract contract BaseAggregator is Ownable {
    ...
    uint256 internal LZ_FEE = 0.2 ether;
...
contract OmniCounter is BaseAggregator, NonblockingLzApp {
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory, uint64, bytes memory _payload) internal override {
        ...
        if (packetType == PT_FIRST) {
            console.log("packet type is first", _srcChainId);
            
            counter += 1;

            console.log("packet type is first", counter);
            ...
            _lzSend(_srcChainId, lzPayload, payable(address(this)), address(0x0), bytes(""), LZ_FEE);
             ...
             console.log("second message was sent");

        } else if (packetType == PT_SECOND) {
            console.log("packet type is second");
