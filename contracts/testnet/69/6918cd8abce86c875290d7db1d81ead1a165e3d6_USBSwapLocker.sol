...
contract USBSwapLocker is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function lockWithStartTime(IERC20Upgradeable token, address account, uint256 quantity, uint32 vestingDuration) public onlySwapContract(token) {
        ...
        if (schedulesLength > 0) {
            ...
            if (lastSchedule.startTime == block.timestamp && lastSchedule.endTime == endTime) {
            lastSchedule.quantity = uint128(uint256(lastSchedule.quantity) + (quantity));
    ...
    */
    ...
    function getRewardContractsPerToken(IERC20Upgradeable token) external view returns (address[] memory swapContracts) {
    ...
    function getSwapContractsPerToken(IERC20Upgradeable token) external view returns (bytes32[] memory) {
        return swapContractsPerToken[token]._inner._values;
    }
