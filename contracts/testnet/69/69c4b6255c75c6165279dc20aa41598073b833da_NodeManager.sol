...
contract NodeManager is Ownable, Pausable {
    ...
    function _calculateNodeRewards(uint _lastClaimTime, uint256 NodePrice) private view returns (uint256 rewards) {
    ...
    function createNode(address account, string memory nodeName) external onlyGuard whenNotPaused {
        ...
        emit NodeCreated(NodePrice, account, block.timestamp);
        totalNodesCreated++;
        totalStaked += NodePrice;
    }
    
    function giftNode(address account, string memory nodeName) external onlyGuard whenNotPaused {
        require(
            _isNameAvailable(account, nodeName),
            "Name not available"
        );
