...
contract SavvyLGE is Ownable {
    ...
    // termsPerMode:: vesting terms for each VestMode
    // pricesPerMode:: base price - 1 unit ($AVAX) per share
    ...
    constructor(
        ...
        require(_vestModes.length == _numVestMode && _numVestMode > 0, "invalid vest mode length");
        numVestMode = _numVestMode;
    ...
    // @dev function that applies NFT term & Lockup term to the user's purchase
    ...
    function _buy(
        ...
        if (NFT != address(0)) {
            ...
            if (!alloc.activated) {
                activate(NFT, nftTokenIndex, amount);
            } else {
                require(
    ...
    function claim() external returns (bool) {
        ...
        uint256 _totalOwed = (totalSvy * terms[msg.sender].shares) /
            shareSupply;
        uint256 perSecond = _totalOwed / terms[msg.sender].term;
        ...
        )
    {
        ...
        perShare =
            (((pricePerShare * licenses[NFT].price) / BASIS_POINTS) *
                vestModes[mode].price) /
            BASIS_POINTS;
    ...
    function _updateTerms(uint256 _shares, uint256 _term)
        internal
        returns (bool)
    {
    ...
    function findWeightedAverage(
        ...
        if (oldValue == 0) {
            ...
            uint256 a = (weightedNew * weightNew) / 1e27;
            uint256 b = (weightedOld * weightOld) / 1e27;
            return (a + b);
        }
    }
    ...
    function upgradeSvy(address newSvy) public onlyOwner {
        require(block.timestamp <= end, "LGE has ended");
        svy = IERC20(newSvy);
    }
    ...
    function updateVestMode(uint256 mode, VestMode memory vestMode) public onlyOwner {
        require(mode < numVestMode, "invalid vest mode");
    ...
    function updateVestModes(uint256 _numVestMode, VestMode[] memory _vestModes) public onlyOwner {
        require(_vestModes.length == _numVestMode && _numVestMode > 0, "invalid vest mode length");
        numVestMode = _numVestMode;
