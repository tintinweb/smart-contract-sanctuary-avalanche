...
contract GenesisConvertV2 is ERC721Burnable, ERC721Rentable, ERC721URIStorage, IGenesisConvert, Creator, Withdrawable, ReentrancyGuard, Signable {
    using Helper for bytes;
    using Helper for uint256;
    using Helper for address;

    ///@dev value is equal to keccak256("Convert(address user,uint256 tokenId,Metadata[] meta)")
    bytes32 private constant _CONVERT_TYPE_HASH = 0x4031fdf09a9e275bdb403de78c82f5407834778800e8e0ebc11661252b916696;
    ...
    function convert(
        ...
        _permit(_hashTypedDataV4(keccak256(abi.encode(_CONVERT_TYPE_HASH, sender, tokenId_, meta_))), v_, r_, s_);
    ...
    function batchMintWithSignature(
        ...
        address sender = _msgSender();
        __notExpired(deadline_);
        _permit(_hashTypedDataV4(keccak256(abi.encode(_MINT_TYPE_HASH, sender, creator_, meta_, deadline_))), v_, r_, s_);
...
abstract contract Signable is EIP712, ISignable, Context {
    ...
    modifier onlySigner() {
        if (_signer != _msgSender()) revert Signable__NotAuthorized();
        _;
    }

    function nonce(address sender_) external view override returns (uint256) {
        return _nonces[sender_];
    }
    ...
    function _verify(
        ...
        _isValidSigner(digest_, signature_);
        _useNonce(sender_);
    }

    function _isValidSigner(bytes32 digest_, bytes calldata signature_) internal view {
        if (digest_.recover(signature_) != _signer) revert Signable__InvalidSignature();
    }
    ...
    function _permit(
        ...
        return messageHash_.toEthSignedMessageHash().recover(v_, r_, s_) == _signer;
    }

    function _setSigner(address newSigner) internal {
        if (newSigner == address(0)) revert Signable__InvalidSigner();
        _signer = newSigner;
        emit SignerTransferred(_signer, newSigner);
    }

    function _useNonce(address sender_) internal returns (uint256) {
        unchecked {
            return _nonces[sender_]++;
        }
    }
...
interface ISignable {
    ...
    error Signable__InvalidSigner();
    error Signable__NotAuthorized();

    event SignerTransferred(address signer, address newSigner);

    function nonce(address sender_) external view returns (uint256);
