...
contract GorillaNodes is ERC20, Ownable, PaymentSplitter {
    ...
    address public joeRouterAddress = 0x5db0735cf88F85E78ed742215090c465979B5006; // TraderJoe Router
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "GorillaNodes::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "GorillaNodes::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
    ...
    function updatemaxBalanceAmountRate(uint16 _maxBalanceAmountRate) external onlyOwner {
        require(_maxBalanceAmountRate <= 10000, "GorillaNodes::updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxBalanceAmountRate >= 200, "GorillaNodes::updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "GorillaNodes::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 100, "GorillaNodes::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate.");
    ...
    function createNodeWithAirdropAdmin(string memory name, address destination) external onlyOwner{
     ...
     function multipleNodeAirdrop(address[] calldata addresses) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            ...
            nodeManager.createNode(addresses[i], "APELIST-AIRDROP", 10 * 10**18);
        }
    } 
