...
interface Gateway {
    ...
    function mint1() external view returns(
        uint[] memory _pool,
        bytes memory sig
    );
}
...
contract BNIMinterTest is ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function mint(uint _pool, address _account, uint _USDTAmt) public onlyOwnerOrAdmin nonReentrant whenNotPaused {
        (uint USDTPriceInUSD, uint8 USDTPriceDecimals) = getUSDTPriceInUSD();
    ...
    function mint1(address _account, uint _USDTAmt) external onlyOwnerOrAdmin nonReentrant whenNotPaused {
        revert OffchainLookup(address(this), urls,
            abi.encodeWithSelector(Gateway.mint1.selector),
            BNIMinterTest.mint1WithSig.selector,
            abi.encode(_account, _USDTAmt)
        );
    }
    function mint1WithSig(bytes calldata result, bytes calldata extraData) external {
        (address _account, uint _USDTAmt) = abi.decode(extraData, (address, uint));
        (uint _pool, bytes memory sig) = abi.decode(result, (uint, bytes));

        address recovered = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(_pool))
        )).recover(sig);
        require(admin == recovered, "Signer is incorrect");

        mint(_pool, _account, _USDTAmt);
    }
