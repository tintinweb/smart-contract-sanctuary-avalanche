...
contract Pyramid {
    ...
    function buildBlocks (uint256[] memory _blocks, bytes memory _color) payable public {
        ...
        for (uint i; i < _blocks.length; i++) {
			...
			if (lastOwner != msg.sender) { //else it's just a repaint
                ...
                require (msg.value.sub(pyramidShare) >= blockPrice, "Not enough money");
                
				ownerOf[blockId] = msg.sender;
				priceOf[blockId] = blockPrice.add(blockPrice.mul(1500).div(BASE)); //increase by 15%

				if(lastOwner != address(0)) {
					//transfer to last owner

					uint256 ownerShare = blockPrice.mul(9000).div(BASE); //owner gets 90%
    ...
    function _getPrice (uint256 _id) internal view returns (uint256) {
        ...
        return ownerOf[_id] != address(0) ? priceOf[_id] : DEFAULT_PRICE;
    }
