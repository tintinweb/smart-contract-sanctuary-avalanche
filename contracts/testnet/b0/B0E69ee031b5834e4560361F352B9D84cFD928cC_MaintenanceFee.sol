    ...
    AccessControlUpgradeable
{
    ...
    function payMaintenanceFee(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            maintFee[_tokenIds[i]].unClaimedDays += _paidDays[i];
            if (_type == 12) {
							if (!isYieldBoxInArray(_tokenIds[i], _yieldBoxYieldKeyLeg[_yieldKey])) {
                _yieldBoxYieldKeyLeg[_yieldKey].push(_tokenIds[i]);
							}
            } else if (_type == 8 || _type == 6 || _type == 4) {
              if (!isYieldBoxInArray(_tokenIds[i], _yieldBoxYieldKeyReg[_yieldKey])) {
    ...
    {
        uint256[] memory _tokenIds = YieldBox.tokenHoldings(_walletAddress);
        uint256 index;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (
		 ...
		 */
		function isYieldBoxInArray(uint256 _tokenId, uint256[] memory _yieldKeyArray)
				public
				pure
				returns (bool)
		{
