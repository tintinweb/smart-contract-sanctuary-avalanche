    ...
    IERC4626
{
    ...
    function scheduleEmergencyWithdraw(uint256[] memory depositIds) external override whenNotPaused {
        ...
        for (uint256 i = 0; i < wInfos.length; i++) {
    ...
    function _deposit(uint256 assets, address receiver) internal nonReentrant returns (uint256 sharesToMint) {
    ...
    function _totalAssets() internal view returns (uint256) {
        return totalAssetsInQSRP + IERC20Upgradeable(_asset).balanceOf(address(this));
    }
        ...
        returns (WithdrawInfo[] memory)
    {
        ...
        for (uint256 i = 0; i < depositIds.length; i++) {
            ...
            if (!dInfo.emergencyWithdrawn) {
                ...
                //todo add formula to the readme
...
interface IMembersPool{
    ...
    event Redeem(address indexed beneficiary, uint256 assets);
