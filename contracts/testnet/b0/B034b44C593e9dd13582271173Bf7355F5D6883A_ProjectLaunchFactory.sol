...
contract ProjectLaunchFactory {
  ...
  address public blacklistContract = 0x6143873664f756fd5Aa5C8849fE18cf218548a52;
  address couponContract = 0x0D24c25D4cCA0e0dF3a8337e0f269fBE8c62A584;
  ...
  mapping(address => commonData) public _commonData;
  ...
  function createSale(
    ...
    common_data.totalTokens = 0;
    common_data.totalLiquidityTokens = 0;
    common_data._sale_token = sale_token;
    ...
    require(address(sale)>=0x0000000000000000000000000000000000000000, "Sale address not set...");
    ...
    if(_seed_details[0] > 0) {
      common_data.totalLiquidityTokens = (common_data.totalLiquidityTokens + _seed_details[5]);
      common_data.totalTokens = (common_data.totalTokens + (_seed_details[4] * _seed_details[1]));
    }
    if(_presale_details[0] > 0) {
      common_data.totalLiquidityTokens = (common_data.totalLiquidityTokens + _presale_details[5]);
      common_data.totalTokens = (common_data.totalTokens + (_presale_details[4] * _presale_details[1]));
    }
    if(_community_details[0] > 0) {
      common_data.totalLiquidityTokens = (common_data.totalLiquidityTokens + _community_details[5]);
      common_data.totalTokens = (common_data.totalTokens + (_community_details[4] * _community_details[1]));
    }
    IBEP20(common_data._sale_token).transferFrom(msg.sender,address(this),(common_data.totalTokens+common_data.totalLiquidityTokens));

    require(address(sale) > 0x0000000000000000000000000000000000000000, "Sale not created...");
    return address(sale);

  }

  function _tokenTransfer(address _sale, address _sale_token) public onlyOwner {
    IBEP20(_sale_token).transferFrom(address(this),address(_sale),IBEP20(_sale_token).balanceOf(address(this)));
  }
  ...
  function getBlacklist(address _Address, address _Contract) public view returns (uint) {
    if (iBlacklist(blacklistContract).getBlacklist(_Address, _Contract) == 1) {
      return (iBlacklist(blacklistContract).getBlacklist(_Address, _Contract));
    } else {
      return 0;
    }
  }

  function getCoupon(address _Address) public view returns (uint) {
    if (iCoupon(couponContract).getCouponActive(_Address) == 1) {
      return iCoupon(couponContract).getCouponDiscount(_Address);
    } else {
      return 0;
    }
  }
...
contract Sale is ReentrancyGuard {
    ...
    address owner;
    address public admin = 0x9F3544aB2669A8B4D7938A77114B1ad89D41ffF2;
    address public blacklistContract = 0x6143873664f756fd5Aa5C8849fE18cf218548a52;
    address public auditorContract = 0x47f207693635BD32df24ae3D2f4B067fB4FD815C;
    address _address = msg.sender;
    address _contract = address(this);
    ...
    function init_sale (
        ...
        sale_details.saleOption = _options[1];
