...
contract FlipV3 is VRFConsumerBaseV2, Ownable {
    ...
    event WinnerPicked(uint256 indexed flipId, CoinSide[] winningChoice, uint256 gambleAmountPerFlip);
    ...
    uint256 public minGambleAmount = .5 ether;

    /// @notice maximum single flip amount is treasury divided by this
    uint256 public maxGambleDivisor = 20;
    ...
    uint256 public maxConsecutiveFlips = 10;

    /// @notice flip amounts must be a multiple of this
    uint256 public flipMultiple = .01 ether;
    ...
    function _createFlip(
        ...
        require(gambleAmount <= (treasuryAddress.balance / maxGambleDivisor), "Gamble less, king");

        uint numFlips = choices.length;
        require(numFlips > 0, "Gamble at least once, pleb");
        require(numFlips <= maxConsecutiveFlips, "Gamble fewer times, king");
        ...
        require(gambleAmountPerFlip % flipMultiple == 0, "Gamble the right amount per flip, pleb");
    ...
    function setMaxConsecutiveFlips(uint256 maxConsecutiveFlips_) external onlyOwner {
        maxConsecutiveFlips = maxConsecutiveFlips_;
    }
