...
contract NFTBridge is NFTBridgeGovernance, ILayerZeroReceiver, ReentrancyGuard {
    using BytesLib for bytes;

    event TransferNFT(uint64 indexed nonce, address token, uint256 tokenID, uint16 recipientChain, address sender, address recipient);
    ...
    function estimateFee(address _token, uint256 _tokenID, uint16 _recipientChain, bytes32 _recipient) public view returns (uint256 fee) {
        bytes memory payload = _transferPayload(_token, _tokenID, _recipientChain, _recipient);
        (fee,) = lzEndpoint().estimateFees(_recipientChain, address(this), payload, false, bytes(""));
    }

    function transferNFT(address _token, uint256 _tokenID, uint16 _recipientChain, bytes32 _recipient) public payable nonReentrant returns (uint64 nonce) {
        bytes memory payload = _transferPayload(_token, _tokenID, _recipientChain, _recipient);

        IERC721(_token).safeTransferFrom(msg.sender, address(this), _tokenID);
        if (isWrappedAsset(_token)) {
            NFTImplementation(_token).burn(_tokenID);
        }
        nonce = _logTransfer(payload, _recipientChain, msg.value);
        emit TransferNFT(nonce, _token, _tokenID, _recipientChain, msg.sender, _truncateAddress(_recipient));
    }

    function _transferPayload(address _token, uint256 _tokenID, uint16 _recipientChain, bytes32 _recipient) internal view returns (bytes memory payload) {
        uint16 tokenChain;
        bytes32 tokenAddress;
        if (isWrappedAsset(_token)) {
            tokenChain = NFTImplementation(_token).chainId();
            tokenAddress = NFTImplementation(_token).nativeContract();
        } else {
            tokenChain = chainId();
        ...
        {
            (,bytes memory queriedSymbol) = _token.staticcall(abi.encodeWithSignature("symbol()"));
            (,bytes memory queriedName) = _token.staticcall(abi.encodeWithSignature("name()"));
            symbolString = abi.decode(queriedSymbol, (string));
            nameString = abi.decode(queriedName, (string));
            (,bytes memory queriedURI) = _token.staticcall(abi.encodeWithSignature("tokenURI(uint256)", _tokenID));
        ...
        NFTBridgeStructs.Transfer memory transfer = NFTBridgeStructs.Transfer({
    ...
    function _logTransfer(bytes memory _payload, uint16 _recipientChain, uint256 _callValue) internal returns (uint64 nonce) {
        bytes memory trustedRemote = trustedRemoteLookup(_recipientChain);
        require(trustedRemote.length != 0, "destination chain is not a trusted source");

        lzEndpoint().send{value : _callValue}(
            _recipientChain,
            trustedRemote,
            _payload,
            payable(msg.sender),
        ...
        nonce = lzEndpoint().getOutboundNonce(_recipientChain, address(this));
    }

    function lzReceive(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce, bytes calldata _payload) external nonReentrant override {
        require(msg.sender == address(lzEndpoint()), "invalid endpoint caller");
        bytes memory trustedRemote = trustedRemoteLookup(_srcChainId);
        // if will still block the message pathway from (srcChainId, srcAddress). should not receive message from untrusted remote.
        require(_srcAddress.length == trustedRemote.length && trustedRemote.length > 0 && keccak256(_srcAddress) == keccak256(trustedRemote), "invalid emitter");
        _completeTransfer(_srcChainId, _nonce, _payload);
    }

    function _completeTransfer(uint16 _srcChainId, uint64 _nonce, bytes calldata _payload) internal {
        NFTBridgeStructs.Transfer memory transfer = _parseTransfer(_payload);
        ...
        emit ReceiveNFT(_nonce, _truncateAddress(transfer.tokenAddress), address(transferToken), transfer.tokenID, transfer.tokenChain, _srcChainId, transferRecipient);
    }

    // Creates a wrapped asset using AssetMeta
    function _createWrapped(uint16 _tokenChain, bytes32 _tokenAddress, bytes32 _name, bytes32 _symbol) internal returns (address token) {
        require(_tokenChain != chainId(), "can only wrap tokens from foreign chains");
        require(wrappedAsset(_tokenChain, _tokenAddress) == address(0), "wrapped asset already exists");

        // initialize the NFTImplementation
        bytes memory initialisationArgs = abi.encodeWithSelector(
            NFTImplementation.initialize.selector,
            _bytes32ToString(_name),
            _bytes32ToString(_symbol),
            address(this),
            _tokenChain,
            _tokenAddress
        );
    ...
    function _encodeTransfer(NFTBridgeStructs.Transfer memory _transfer) internal pure returns (bytes memory encoded) {
        encoded = abi.encodePacked(
            ...
            _transfer.name,
            _transfer.tokenID,
            uint8(bytes(_transfer.uri).length),
            _transfer.uri,
            _transfer.to,
            _transfer.toChain
        );
    ...
    function _parseTransfer(bytes memory _encoded) internal pure returns (NFTBridgeStructs.Transfer memory transfer) {
        uint index = 0;

        uint8 payloadID = _encoded.toUint8(index);
        index += 1;

        require(payloadID == 1, "invalid Transfer");

        transfer.tokenAddress = _encoded.toBytes32(index);
        index += 32;
        ...
        index += 1;
        transfer.uri = string(_encoded.slice(index, _encoded.length - index - 34));

        // From here we read backwards due malformatted package
        index = _encoded.length;

        index -= 2;
        transfer.toChain = _encoded.toUint16(index);

        index -= 32;
        transfer.to = _encoded.toBytes32(index);

        require(_encoded.length == index, "invalid Transfer");
    }
     ...
     */
    function _truncateAddress(bytes32 _b) internal pure returns (address) {
        require(bytes12(_b) == 0, "invalid EVM address");
    ...
    function onERC721Received(
        ...
        require(_operator == address(this), "can only bridge tokens via transferNFT method");
...
contract NFTBridgeGetters is NFTBridgeState {
    function lzEndpoint() public view returns (ILayerZeroEndpoint) {
        return ILayerZeroEndpoint(_state.lzEndpoint);
    }
    ...
    function wrappedAsset(uint16 _tokenChainId, bytes32 _tokenAddress) public view returns (address){
        return _state.wrappedAssets[_tokenChainId][_tokenAddress];
    }

    function trustedRemoteLookup(uint16 _chainId) public view returns (bytes memory){
        return _state.trustedRemoteLookup[_chainId];
    }
    ...
    function isWrappedAsset(address _token) public view returns (bool){
        return _state.isWrappedAsset[_token];
    }

    function getConfig(uint16 _version, uint16 _dstChainId, uint _configType) external view returns (bytes memory) {
        return lzEndpoint().getConfig(_version, _dstChainId, address(this), _configType);
    }
    ...
    function getReceiveVersion() external view returns (uint16) {
        return lzEndpoint().getReceiveVersion(address(this));
    }
}
...
contract NFTBridgeGovernance is NFTBridgeGetters, NFTBridgeSetters, OwnableUpgradeable {
...
contract NFTBridgeImplementation is NFTBridge {
    ...
    function initialize(uint16 _chainId, address _lzEndpoint, address _tokenImplementation) initializer public virtual {
        __Ownable_init();
...
contract NFTBridgeSetters is NFTBridgeState {
    function _setChainId(uint16 _chainId) internal {
        _state.chainId = _chainId;
    }

    function _setTrustedRemote(uint16 _chainId, bytes calldata _path) internal {
        _state.trustedRemoteLookup[_chainId] = _path;
    }

    function _setTrustedRemoteAddress(uint16 _chainId, bytes calldata _remoteAddress) internal {
        _state.trustedRemoteLookup[_chainId] = abi.encodePacked(_remoteAddress, address(this));
    }

    function _setTokenImplementation(address _tokenImplementation) internal {
        _state.tokenImplementation = _tokenImplementation;
    }

    function _setLzEndpoint(address _LzEndpoint) internal {
        _state.lzEndpoint = payable(_LzEndpoint);
    }

    function _setWrappedAsset(uint16 _tokenChainId, bytes32 _tokenAddress, address _wrapper) internal {
        _state.wrappedAssets[_tokenChainId][_tokenAddress] = _wrapper;
        _state.isWrappedAsset[_wrapper] = true;
    }
