...
contract ZOARouterV1 is Initializable, TokenHelper, Router, ITrustedForwarder, IZOARouterV1 {
    ...
    function setup(
        ...
        uint256 avatarShopId_,
        uint256 usernamePropertyId_
    ) external onlyCoreAdminOrOperator {
        _avatarPropertyId = avatarPropertyId_;
        _avatarShopId = avatarShopId_;
    ...
    function createProfile(
        ...
        // buy an avatar
        {
            IGameFiShopV1.Shop memory shopDetails = IGameFiShopV1(gameFiShops()).shopDetails(_avatarShopId);
            // if (shopDetails) {

            // }
            require(shopDetails.tokenInStandart == TokenStandart.ERC1155, "ZOARouterV1: wrong token standart");
            _tokenTransferFrom(shopDetails.tokenInStandart, shopDetails.tokenInOffer, msg.sender, profileVault);
            bytes memory buyAvatarCall = abi.encodeWithSelector(IGameFiShopV1.buyToken.selector, _avatarShopId);
            IGameFiProfileVaultV2(profileVault).call(gameFiShops(), buyAvatarCall, 0);
        }

        // set avatar property
        {
            IGameFiCoreV2.Property memory avatarPropertyDetails = IGameFiCoreV2(gameFiShops()).propertyDetails(
                _avatarPropertyId
            );
            ...
            IGameFiCoreV2(gameFiCore()).setPropertyValue(profileId, _avatarPropertyId, bytes32(username), "0x");
        }

        // set username property
        {
            IGameFiCoreV2.Property memory usernamePropertyDetails = IGameFiCoreV2(gameFiShops()).propertyDetails(
                _usernamePropertyId
            );
