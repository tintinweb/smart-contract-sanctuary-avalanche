...
contract Royalties is Ownable {
    ...
    uint256 public developerPercentage = 8;

    uint256 public claimedDeveloperAmount = 0;
    uint256 public claimedOwnerAmount = 0;
    uint256 public ownerReserved;
    uint256 public developerReserved;
    ...
    function getTotalCollected() public view returns (uint256) {
        ...
        return balance + claimedDeveloperAmount + claimedOwnerAmount;
    }
    ...
    function getBalance() public view returns (uint256) {
        ...
        return (getTotalCollected() * _royatlyRate) / 100 - (claimedDeveloperAmount + claimedOwnerAmount);
    }
    ...
    function getDeveloperBalance() external view returns (uint256) {
        ...
        uint256 normalizedBalance = balance - (developerReserved + ownerReserved);

        uint256 developerShare = (normalizedBalance * developerPercentage) / 100;
    ...
    function getOwnerBalance() external view returns (uint256) {
        uint256 balance = getBalance();
        uint256 normalizedBalance = balance - (developerReserved + ownerReserved);
        uint256 ownerShare = (normalizedBalance * (100 - developerPercentage)) / 100;
    ...
    function claimOwner() external onlyOwner {
        ...
        uint256 developerShare = (normalizedBalance * developerPercentage) / 100;
        uint256 ownerShare = normalizedBalance - developerShare;
        ...
        claimedOwnerAmount += amountToSend;
    ...
    function claimDeveloper() external {
        ...
        claimedDeveloperAmount += amountToSend;
