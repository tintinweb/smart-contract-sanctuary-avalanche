...
import "./BaseAttributeManager.sol";

interface INFTStakingManager {
    function increaseTotalPowerLevelStakedByAddress(uint256 tokenId, uint256 amount, address staker) external returns(bool);
}

contract AlphaNFTPowerLevelManager is 
BaseAttributeManager
{

    uint256 _minPowerIncrease = 1; // 1 of 10000
    
    address _nftStakingManagerContract;
    
    struct StartingPowerLevels {
        uint256 BASE;
        uint256 ALPHA;
        uint256 RONIN;
        uint256 APEX;
    ...
    mapping(TokenType => uint256) public powerLevelMultipliersByTokenType;

    StartingPowerLevels public startingPowerLevels;
    PowerLimits public powerLimits;

    event powerChanged(uint256 tokenId, uint256 prevPower, uint256 newPower, uint256 updatedAt);
    
    constructor() {
        powerLevelMultipliersByTokenType[TokenType.BASE] = 11000;
        powerLevelMultipliersByTokenType[TokenType.ALPHA] = 15000;
        powerLevelMultipliersByTokenType[TokenType.RONIN] = 20000;
        powerLevelMultipliersByTokenType[TokenType.APEX] = 35000;
    }

    /**
    *   @dev registerNewToken - called by minter after token creation
    *   registers the token on the mapping, sets the starting power level according
    *
    *   @param tokenId - uint256 of the new token
    *   @param nftType - enum ID of the nftType
    *
    * */
    function registerNewToken(
        ...
        nonReentrant
    {
        tokenRegistry[tokenId] = Token({
            nftType: nftType,
            powerLevel: getStartingPowerByNFTType(uint256(nftType))
        });
    }

    function setPowerMultiplerByTokenType(
        TokenType tokenType,
        uint256 multiplier
        )
        external 
        ...
        nonReentrant
    {
        powerLevelMultipliersByTokenType[tokenType] = multiplier;
    }

    function getTotalUpgradaleByERC20(
        uint256 tokenId,
        uint256 amountERC20
        )
        public
        ...
        returns(uint256)
    {
        TokenType tokenType = tokenRegistry[tokenId].nftType;
        uint256 currentPower = tokenRegistry[tokenId].powerLevel;
        uint256 multiplier = powerLevelMultipliersByTokenType[TokenType(tokenType)];

        uint256 upgradableAmount = amountERC20 / currentPower / multiplier;

        return upgradableAmount;
    }

    function getRequiredERC20ToLevel(
        uint256 tokenId,
        uint256 power
        )
        public
        ...
        returns(uint256)
    {
        ...
        uint256 requiredERC20ToLevel = powerLevelMultipliersByTokenType[tokenType] * power * currentPower;        
        
        return requiredERC20ToLevel;
    }
        ...
        nonReentrant 
    {
        ...
        uint256 requiredERC20ToLevel = getRequiredERC20ToLevel(tokenId, power);        
        
        uint256 maxPower = getPowerLimitsByNFTType(uint8(tokenType));

        require(IERC20(_erc20Token).balanceOf(msg.sender) >= requiredERC20ToLevel, "Insufficient balance to increase power.");
        require(getPowerLevelByTokenId(tokenId) <= maxPower - power, "Max Power Would Be Exceeded.");
        
        IERC20(_erc20Token).transferFrom(msg.sender, _feeReceiver, requiredERC20ToLevel);
        
        tokenRegistry[tokenId].powerLevel = tokenRegistry[tokenId].powerLevel + power;

        INFTStakingManager(_nftStakingManagerContract).increaseTotalPowerLevelStakedByAddress(tokenId, power, msg.sender);
    }

    function getPowerLevelByTokenId(
        uint256 tokenId
        )
        public
        view
        returns(uint256)
    {
        return tokenRegistry[tokenId].powerLevel;
    }
    ...
    function resetPowerLevel(
        ...
        nonReentrant 
    {
        TokenType nftType = tokenRegistry[tokenId].nftType;
        tokenRegistry[tokenId].powerLevel = getStartingPowerByNFTType(uint256(nftType));
    }

    function setMinPowerIncrease(
        uint256 minPowerIncrease
        )
        external
        requireAdmin(msg.sender)
        nonReentrant
    {
        _minPowerIncrease = minPowerIncrease;
    }
    ...
    function setStartingPower(
        ...
        nonReentrant
    {
        startingPowerLevels = StartingPowerLevels({
            BASE: _BASE,
            ALPHA: _ALPHA,
            RONIN: _RONIN,
            APEX: _APEX
    ...
    function getStartingPowerByNFTType(
        uint256 tokenType
        )
        public
        view
        returns(uint256)
    {
        ...
        if (tokenType == 1) {
            startingPower = startingPowerLevels.BASE;
        }

        if (tokenType == 2) {
            startingPower = startingPowerLevels.ALPHA;
        }

        if (tokenType == 3) {
            startingPower = startingPowerLevels.RONIN;
        }

        if (tokenType == 4) {
            startingPower = startingPowerLevels.APEX;
        }

        return startingPower;
    }

    function getPowerLimitsByNFTType(
        uint8 tokenType
        )
        public
        view
        returns(uint256)
    {
        ...
        if (tokenType == 1) {
            powerLimit = powerLimits.BASE;
        }

        if (tokenType == 2) {
            powerLimit = powerLimits.ALPHA;
        }

        if (tokenType == 3) {
            powerLimit = powerLimits.RONIN;
        }

        if (tokenType == 4) {
            powerLimit = powerLimits.APEX;
        }
...
abstract contract BaseAttributeManager is 
  AccessControl,
  ReentrancyGuard {
    ...
    function setMinter(
        address shareCreatorAddress
        )
        external
        requireAdmin(msg.sender)
        nonReentrant
    {
        grantRole(MINTER_ROLE, shareCreatorAddress);
    }
    ...
    modifier requireMinter(address sender) {
