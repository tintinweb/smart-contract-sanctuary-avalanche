...
contract ImplementationV1 is Ownable {
  ERC_721_STORAGE S; 
  uint256 platform_fee = 3;
  address payment_receiver = 0x9458Dac1C82b933Cc3BAC408FC627A59920c2b0B;
  ...
  function _erc721_buy_from_market_split_payment_royalty_handler(
    ...
    address creator
        ...
        if(S.contract_to_storage[ _contract_add ].id_to_is_primary_sale[ _token_id ]) {
                ...
                payable(payment_receiver).transfer(contractOwnerFee);
            }
...
function set_platform_fee(uint256 _new_fee)public onlyOwner()
{
   platform_fee = _new_fee;
}

function get_payment_receiver() public view returns(address)
{
    return payment_receiver;
}

function set_payment_receiver(address new_receiver) public onlyOwner()
{
    payment_receiver = new_receiver;
}
