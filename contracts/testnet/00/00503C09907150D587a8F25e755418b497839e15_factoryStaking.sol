...
contract factoryStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    function myClaimableView(address user, uint level) public view returns(uint256){
        uint diff = stakedSince[user] - block.timestamp;
        uint balance = factoryToken.balanceOf(user, level);
        uint256 total = ((diff * balance * (SUPPLY_PER_DAY_PER_LVL + BASE_SUPPLY_RATE))/86400);

        return total;
    }
    function claimSupply(uint level) public{
        uint256 claimable = myClaimableView(msg.sender, level);
        stakedTokens[msg.sender][level] = factoryToken.balanceOf(msg.sender, level);
        stakedSince[msg.sender] = block.timestamp;
