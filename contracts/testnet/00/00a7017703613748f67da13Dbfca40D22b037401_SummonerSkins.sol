...
contract SummonerSkins is ReentrancyGuard, Ownable, ERC721Enumerable {
    ...
    constructor(address _rm) ERC721("Summoner Rare Skins", "RARE SKINS"){
		startDate = block.timestamp;
        rarity = IRarity(_rm);
		skinURIs = new SkinURIs(maxSupply);
		raritySkinManager = new RaritySkinManager(_rm);
...
contract SkinURIs is Ownable {
	...
	function imageData(uint256 _tokenId, uint256 _dna) private view returns (bytes memory) {
        ...
        string memory svg = strConcat(strConcat("<svg id='Rare Skin #", _tokenId.toString()), "' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>");
        string memory skin = strConcat(strConcat("<path fill='#", art[7][dna[7]]), "' d='M16 11v-1h1V9h-1V5h-1V4h-1V3h-4v1H9v1H8v5h1v1h1v4H8v3h2v1H9v5h1 5 1v-1h-1v-5h1v-1h1v-2h-2v-4h1z'/><path d='M12 7h1v1h-1zm3 0h1v1h-1z' fill='#fff'/><g fill='#");
        ...
        string memory alt_color = strConcat(strConcat(strConcat(art[6][dna[6]], "'>"), art[8][class]), "</g><g fill='#");
		string memory base_color = strConcat(strConcat(strConcat(art[5][dna[5]], "'>"), art[9][class]), "</g><g opacity='.23'>");
		string memory shadow_part = strConcat(strConcat(art[10][class], "</g>"), art[11][class]); // base part
		return abi.encodePacked(
            ...
            base_color,
            shadow_part,
            strConcat(accessoriesSvg(dna), "</svg>")
		);
	}
...
/**
 *Submitted for verification at FtmScan.com on 2021-10-18
*/
