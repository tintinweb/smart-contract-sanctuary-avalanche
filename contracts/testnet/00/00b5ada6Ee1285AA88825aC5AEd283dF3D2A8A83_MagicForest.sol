...
contract MagicForest is IERC721Receiver, Pausable, Restricted {
    ...
    function getClaimingFee() public view returns (uint256) {
        ...
        else if (block.timestamp - sessionBegining > 1 hours) return claimingFeeValues[0];
        else {
            for (uint8 i = 0; i < 5; i++) {
    ...
    function _updateClaimingSession(uint32 claimingTimestamp) internal {
        if (claimingTimestamp - sessionBegining > 1 hours) {
            ...
            lastSessionClaimNumber = (claimingTimestamp - sessionBegining > 2 hours) ? 0 : sessionClaimNumber;
    ...
    function setClaimingFees(uint256[] calldata values, uint256[] calldata tresholds) external onlyController {
