...
contract PrivateSale is Ownable {
    ...
    function buy(uint256 amount, string memory token) public returns (uint256 result) {
        uint256 decimal = 0;
        if(keccak256(abi.encodePacked(token)) == keccak256(abi.encodePacked("USDT"))) decimal = IERC20(USDT).decimals();
        if(keccak256(abi.encodePacked(token)) == keccak256(abi.encodePacked("USDC"))) decimal = IERC20(USDC).decimals();
        if(keccak256(abi.encodePacked(token)) == keccak256(abi.encodePacked("USDCE"))) decimal = IERC20(USDCE).decimals();
        if(keccak256(abi.encodePacked(token)) == keccak256(abi.encodePacked("BUSD"))) decimal = IERC20(BUSD).decimals();
        if(keccak256(abi.encodePacked(token)) == keccak256(abi.encodePacked("DAI"))) decimal = IERC20(DAI).decimals();

        uint256 multiplier = IERC20(XDX).decimals() - decimal;
        ...
        require(result * rate * multiplier / 100 < balance(), "Insufficient withdrawal amount.");
        ...
        IERC20(XDX).transfer(msg.sender, result * rate * multiplier / 100);
        emit Buy(msg.sender, token, result, result * rate * multiplier / 100);
        return result * rate * multiplier / 100;
    }
