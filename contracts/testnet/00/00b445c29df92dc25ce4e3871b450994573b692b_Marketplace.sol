...
import "../Collectible/INFTCollectible.sol";
import "../PaymentManager/IPaymentManager.sol";
import "./lib/LibOrder.sol";
import "../libs/LibShareholder.sol";
import "../Royalty/LibRoyalty.sol";

contract Marketplace is Initializable, EIP712Upgradeable, ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // events
    event Fund(address indexed sender, uint256 value);
    event Withdraw(address indexed sender, uint256 balance, uint256 amount);
    event Cancel(address indexed sender, address indexed collection, uint256 tokenId, bytes32 hash, string salt);
    event MakeOffer(address indexed sender, address indexed collection, uint256 tokenId, uint256 amount);
    event CancelOffer(address indexed sender, address indexed collection, uint256 tokenId);
    event AcceptOffer(address indexed sender, address indexed collection, uint256 tokenId, address indexed _buyer, uint256 amount);
    event Redeem(address indexed sender, address indexed collection, uint256 tokenId, uint256 value, string salt);
    ...
    function fund() external payable nonReentrant paymentAccepted {
    ...
    function withdraw(uint256 _amount) external nonReentrant priceGreaterThanZero(_amount) {
        ...
        require((existingBalance - _amount) >= offerTotalAmounts[msg.sender], "can't withdraw, have actively offers");
        biddingWallets[msg.sender] -= _amount;

        payable(msg.sender).transfer(_amount);
        emit Withdraw(msg.sender, existingBalance, _amount);
    }
    ...
    priceGreaterThanZero(_amount)
    {
        ...
        require(offers[_nftContractAddress][_tokenId][msg.sender] == 0, "you have already an offer");

        offers[_nftContractAddress][_tokenId][msg.sender] = _amount;
        offerTotalAmounts[msg.sender] += _amount;

        emit MakeOffer(msg.sender, _nftContractAddress, _tokenId, _amount);
    }
    ...
    nonReentrant
    {
        ...
        emit CancelOffer(msg.sender, _nftContractAddress, _tokenId);
    }
    ...
    function acceptOffer(address _nftContractAddress, uint256 _tokenId, address _buyer, LibShareholder.Shareholder[] memory shareholders) external whenNotPaused nonReentrant {
        ...
        require(IERC721Upgradeable(_nftContractAddress).ownerOf(_tokenId) == _buyer, "nft should be transferred to buyer");
        _payout(payable(msg.sender), _nftContractAddress, _tokenId, bid, shareholders);

        emit AcceptOffer(msg.sender, _nftContractAddress, _tokenId, _buyer, bid);
    }
    ...
    returns (uint256)
    {
        ...
        if (order.tokenId > 0) {
            ...
            LibRoyalty.Royalty[] memory royalties = new LibRoyalty.Royalty[](order.royalties.length);
            for (uint256 i = 0; i < order.royalties.length; i++) {
                LibRoyalty.Royalty memory royalty;
                royalty.account = order.royalties[i].account;
                royalty.value = order.royalties[i].value;
                royalties[i] = royalty;
            }
            tokenId = INFTCollectible(order.nftContractAddress).mint(order.tokenUri, royalties);
        }
        ...
        require(IERC721Upgradeable(order.nftContractAddress).ownerOf(tokenId) == buyer, "nft should be transferred buyer");

        _payout(seller, order.nftContractAddress, tokenId, order.price, order.shareholders);
        if (msg.value > order.price) {
            buyer.transfer(msg.value - order.price);
        }
        emit Redeem(msg.sender, order.nftContractAddress, tokenId, msg.value, order.salt);
        return tokenId;
    }
    ...
    isNotCancelled(LibOrder.hashKey(order))
    {
        ...
        emit Cancel(msg.sender, order.nftContractAddress, order.tokenId, orderKeyHash, order.salt);
    }
    ...
    function _payout(address payable _seller, address _nftContractAddress, uint256 _tokenId, uint256 _price, LibShareholder.Shareholder[] memory _shareholders) internal {
        IPaymentManager(paymentManager).payout{ value: _price }(_seller, _nftContractAddress, _tokenId, _price, _shareholders);
    }

    function _payoutx(address payable _seller, address _nftContractAddress, uint256 _tokenId, uint256 _price, LibShareholder.Shareholder[] memory _shareholders) external payable {
        IPaymentManager(paymentManager).payout{ value: _price }(_seller, _nftContractAddress, _tokenId, _price, _shareholders);
    }

    function validate(LibOrder.Order memory order, bytes memory signature) external view returns (address) {
        return _validate(order, signature);
    }
...
library LibOrder {
    ...
    bytes constant royaltyItemTypeString = abi.encodePacked(
        "Royalty(",
            "address account,",
            "uint96 value",
        ")"
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        abi.encodePacked(orderTypeString, royaltyItemTypeString, shareholderItemTypeString)
    );

    bytes32 constant ROYALTY_TYPEHASH = keccak256(
        royaltyItemTypeString
    );
    ...
    function hash(Order memory order) internal pure returns (bytes32) {
        bytes32[] memory royaltyHashes = new bytes32[](order.royalties.length);
        for (uint256 i = 0; i < order.royalties.length; i++) {
            royaltyHashes[i] = _hashRoyaltyItem(order.royalties[i]);
        }
    ...
    function _hashRoyaltyItem(Royalty memory royalty) internal pure returns (bytes32) {
        return keccak256(abi.encode(
                ROYALTY_TYPEHASH,
                royalty.account,
                royalty.value
