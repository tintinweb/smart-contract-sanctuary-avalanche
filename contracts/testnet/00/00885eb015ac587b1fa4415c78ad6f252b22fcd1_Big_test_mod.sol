...
contract Big_test_mod is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 public node_amount = 10000000000000000000;
    uint8 public swap_perc = 3;
    ...
    function updateSwapPerc(uint8 newVal) external onlyOwner {
        swap_perc = newVal;
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        uint256 perc = tokens.div(swap_perc);
        uint256 otherperc = tokens.sub(perc);
        uint256 initialBalance = address(this).balance;
        swapTokensForAVAX(perc);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(otherperc, newBalance);
        emit SwapAndLiquify(perc, newBalance, otherperc);
    }
