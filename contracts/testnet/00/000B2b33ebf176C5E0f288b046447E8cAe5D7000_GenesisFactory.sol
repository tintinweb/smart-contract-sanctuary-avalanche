...
contract GenesisFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function _deployTreasuryProxy(TreasuryParams calldata _treasuryParams) internal returns (address) {
    ERC1967ProxyInitializable treasuryProxy = _deployProxy(
  ...
  function _deployNftProxy(BondNftParams calldata _bondNftParams) internal returns (address) {
    ERC1967ProxyInitializable nftProxy = _deployProxy(
  ...
  function _deployBondProxy(
    ...
    ERC1967ProxyInitializable bondProxy = _deployProxy(
      bondImplementation,
      abi.encodeWithSelector(
  ...
  ) internal returns (ERC1967ProxyInitializable) {
    return new ERC1967ProxyInitializable(_implementationAddress, _params);
  }
  ...
  function setBondImplementation(address _bondImplementation) external onlyOwner {
    bondImplementation = _bondImplementation;
  }

  function setBondNftImplementation(address _bondNftImplementation) external onlyOwner {
    bondNftImplementation = _bondNftImplementation;
  }

  function setTreasuryImplementation(address _treasuryImplementation) external onlyOwner {
    treasuryImplementation = _treasuryImplementation;
  }
...
contract GenesisBond is Initializable, PolicyUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event BondCreated(uint256 deposit, uint256 payout, uint256 expires, uint256 indexed bondId);
    event BondRedeemed(uint256 indexed bondId, address indexed recipient, uint256 payout, uint256 remaining);
    event BondPriceChanged(uint256 internalPrice, uint256 debtRatio);
    event BondInitialized(uint256 controlVariable, uint256 vestingTerm, uint256 minimumPrice, uint256 maxPayout, uint256 maxDebt, uint256 maxTotalPayout, uint256 initialDebt, uint256 lastDecay);
    ...
    EnumerableSetUpgradeable.UintSet private bondIssuedIds;
    address public feeReceiver;
    ...
    mapping(uint256 => Bond) public bondInfo; // stores bond information for nfts
    ...
    struct Terms {
        uint256 controlVariable; // scaling variable for price. times 1e18
    ...
    */
    ...
    /**
     *  @notice change address of feeReceiver
     *  @param _feeReceiver uint
     */
    function changeFeeReceiver(address _feeReceiver) external onlyPolicy {
        feeReceiver = _feeReceiver;
    ...
    function deposit(
        ...
        uint256 bondId = bondNft.mint(_depositor, address(this));
        ...
        emit BondCreated(_amount, payout, block.timestamp.add(terms.vestingTerm), bondId);
     ...
     *  @return uint
     */
    function redeem(uint256 _bondId) public returns (uint256) {
        ...
        uint256 percentVested = percentVestedFor(_bondId); // (seconds since last interaction / vesting term remaining)
     ...
     *  @param _bondIds uint256[]
     */
    function batchRedeem(uint256[] calldata _bondIds) external returns (uint256 payout) { 
    ...
    */
    ...
    function _bondPrice() internal returns (uint256 price_) {
        price_ = terms.controlVariable.mul(debtRatio()).div(1e18);
    ...
    function trueBondPrice() public view returns (uint256 price_) {
        price_ = bondPrice().mul(1e6).div(1e6 - currentFee());
    }
    ...
    function payoutFor(uint256 _value) public view returns (uint256 _payout, uint256 _fee) {
        if(feeInPayout) {
            ...
            _payout = customTreasury.valueOfToken(address(principalToken), _value.sub(_fee)) * 1e18 / bondPrice();
        }
    }
     ...
     */
    function percentVestedFor(uint256 _bondId) public view returns (uint256 percentVested_) {
     ...
     */
    function pendingPayoutFor(uint256 _bondId)
        external
        view
        returns (uint256 pendingPayout_)
    {
        uint256 percentVested = percentVestedFor(_bondId);
        uint256 payout = bondInfo[_bondId].payout;
     ...
     *
     */
    function pendingVesting(uint256 _bondId) external view returns (uint256 vestingSeconds_) {
        uint256 vesting = bondInfo[_bondId].vesting;
        uint256 percentVested = percentVestedFor(_bondId);
    ...
    function allIssuedBondIds() external view returns (uint256[] memory) {
