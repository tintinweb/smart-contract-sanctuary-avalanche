...
contract EtfTreasury is ContractGuard, Initializable {
    ...
    uint256 public constant TOTAL_DEV_REWARD_AOEA = 10000 ether; 
    uint256 public constant TOTAL_DAO_REWARD_AOEA = 20000 ether;
    uint256 public aoeaTokenPerSecondForUser;
    uint256 public aoeaTokenPerSecondForDev;
    uint256 public aoeaTokenPerSecondForDao;
    ...
    event SetPegToSendEtfReward(uint256 oldValue, uint256 newValue);
    ...
    event SetMaxEtfMint(uint256 oldValue, uint256 newValue);
    ...
    function initialize(
        ...
        aoeaTokenPerSecondForDev = TOTAL_DEV_REWARD_AOEA.div(additionalRewardRunningTime);
        aoeaTokenPerSecondForDao = TOTAL_DAO_REWARD_AOEA.div(additionalRewardRunningTime);
        initialized = true;
        operator = msg.sender;
    ...
    function addBoardroom(address _boardroom, uint256 _allocPoint, uint256 _additionalRewardAllocPoint) external onlyOperator {
        ...
        IERC20Upgradeable(aoeaToken).safeApprove(_boardroom, TOTAL_USER_REWARD_AOEA.add(TOTAL_DEV_REWARD_AOEA).add(TOTAL_DAO_REWARD_AOEA));
    ...
    function _approveTokenIfNeeded(address token) private {
        if (IERC20(token).allowance(address(this), address(ROUTER)) == 0) {
            IERC20Upgradeable(token).safeApprove(address(ROUTER), type(uint256).max);
        }
    } 
