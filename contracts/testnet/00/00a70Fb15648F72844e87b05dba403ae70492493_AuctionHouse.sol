...
contract AuctionHouse is ERC721Holder, Ownable {
    ...
    event CreateAuction(IERC721 indexed token, uint256 id, bytes32 indexed auctionId, address seller);
    event CancelAuction(IERC721 indexed token, uint256 id, bytes32 indexed auctionId, address seller);
    event Bid(IERC721 indexed token, uint256 id, bytes32 indexed auctionId, address bidder, uint256 bidPrice);
    event Claim(IERC721 indexed token, uint256 id, bytes32 indexed auctionId, address seller, address taker, uint256 price);
    ...
    function _createAuction(
        ...
        emit CreateAuction(_token, _id, auctionId, msg.sender);
    }
    ...
    function cancelAuction(bytes32 _auction) external {
        ...
        require(a.lastBidPrice == 0, "Cannot cancel the auction since one or more bids already exist"); // for EA. but even in DA, FP, seller can withdraw their token with this func.
    ...
    function bid(bytes32 _auction) payable external {
        ...
        if (lastBidPrice != 0) {
            require(msg.value >= lastBidPrice + (lastBidPrice / 100), "bid must be at least 1% higher than the last bid");  // 1%
        } else {
            require(msg.value >= a.startPrice && msg.value > 0, "bid must be at least 1% higher than the start price");
        }
    ...
    // both seller and taker can call this func in English Auction. Probably the taker (last bidder) might call this func.
