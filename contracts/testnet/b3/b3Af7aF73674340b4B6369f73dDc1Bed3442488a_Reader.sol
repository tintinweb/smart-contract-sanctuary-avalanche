...
contract Oracle is RoleModule {
    ...
    function _setPrices(
        ...
        for (uint256 i = 0; i < params.tokens.length; i++) {
            ...
            console.log("precision %s", cache.info.precision);
            ...
            for (uint256 j = 0; j < signers.length; j++) {
                ...
                console.log("  cache.minPrices[%s], %s", j, cache.minPrices[j]);
                cache.maxPrices[j] = OracleUtils.getUncompactedPrice(params.compactedMaxPrices, cache.priceIndex);
                console.log("  cache.maxPrices[%s], %s", j, cache.maxPrices[j]);
            ...
            for (uint256 j = 0; j < signers.length; j++) {
                ...
                console.log("  loop C minPriceIndex %s", cache.minPriceIndex);
                cache.maxPriceIndex = OracleUtils.getUncompactedPriceIndex(params.compactedMaxPricesIndexes, cache.signatureIndex);
                console.log("  loop C maxPriceIndex %s", cache.maxPriceIndex);

                console.log("  minPrices len %s", cache.minPrices.length);
                console.log("  cache.minPrices[cache.minPriceIndex]", cache.minPrices[cache.minPriceIndex]);
                console.log("  cache.maxPrices[cache.maxPriceIndex]", cache.maxPrices[cache.maxPriceIndex]);
                console.log("  singatures len %s cache.signatureIndex %s", params.signatures.length, cache.signatureIndex);
                console.log("  signers len %s", signers.length);
