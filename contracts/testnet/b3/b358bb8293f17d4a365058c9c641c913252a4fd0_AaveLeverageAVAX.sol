...
// File: contracts\superToken\leverageSuperToken.sol
...
abstract contract leverageSuperToken is superTokenInterface{
    ...
    */
    function _setInterestRate(uint64 _interestrate) internal{
        interestRate = _interestrate;
        defaultRate = _interestrate;
    }
    function _interestInflation(uint64 inflation)internal{
        uint256 _totalSupply = totalUnderlying();
        if(_totalSupply > 0){
            uint256 balance = availableBalance();
            if(balance*100<_totalSupply){
                interestRate = interestRate*inflation/1e8;
            }else{
    ...
    function borrowLimit(address account)public view returns (uint256){
        return loanAccountMap[account].add(availableBalance());
    }
    function borrow(uint256 amount) public isAuthorized returns(uint256) {
        addLoan(msg.sender,amount);
        uint256 repayAmount = amount.mul(calDecimals.sub(interestRate))/calDecimals;
        lendTo(msg.sender,repayAmount);
        emit Borrow(msg.sender, address(underlying),repayAmount,amount);
        emit Interest(msg.sender, address(underlying),amount.sub(repayAmount));
        return repayAmount;
    }
    function borrowAndInterest(uint256 amount) public isAuthorized{
        //l1*r + (l0-l1) = -amount
        uint256 newLoan = loanAccountMap[msg.sender].add(amount).mul(calDecimals)/(calDecimals.sub(interestRate));
        setLoan(msg.sender,newLoan);
        uint256 interest = newLoan.mul(interestRate).div(calDecimals);
        lendTo(msg.sender,amount);
        emit Borrow(msg.sender,address(underlying),amount,newLoan);
        emit Interest(msg.sender,address(underlying),interest);
    }
    function repay(uint256 amount,bool bAll) public payable isAuthorized {
        repayFrom(msg.sender,amount);
        if(!bAll){
            subLoan(msg.sender, amount);
        }else{
            subLoan(msg.sender,loanAccountMap[msg.sender]);
        }
        emit Repay(msg.sender,address(underlying),amount,loanAccountMap[msg.sender]);
    }
    function repayAndInterest(uint256 amount) public payable isAuthorized{
        repayFrom(msg.sender,amount);
        //l1*r + (l0-l1) = amount
        uint256 newLoan = loanAccountMap[msg.sender].sub(amount).mul(calDecimals)/(calDecimals.sub(interestRate));
        setLoan(msg.sender,newLoan);
        uint256 interest = newLoan.mul(interestRate).div(calDecimals);
        emit Repay(msg.sender,address(underlying),amount,newLoan);
        emit Interest(msg.sender,address(underlying),interest);
    }
    function addLoan(address account,uint256 amount) internal {
        loanAccountMap[account] = loanAccountMap[account].add(amount);
        totalLoan = totalLoan.add(amount);
    }
    function subLoan(address account,uint256 amount) internal {
        loanAccountMap[account] = loanAccountMap[account].sub(amount);
        totalLoan = totalLoan.sub(amount);
    }
    function setLoan(address account,uint256 amount) internal {
        totalLoan = totalLoan.sub(loanAccountMap[account]).add(amount);
        loanAccountMap[account] = amount;
    }
}

// File: contracts\superToken\AAVELeverageToken\AaveLeverageAVAX.sol
...
//
// This contract handles swapping to and from superToken.
contract AaveLeverageAVAX is AaveSuperAVAX,leverageSuperToken {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _wavax,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        AaveSuperAVAX(multiSignature,origin0,origin1,_stakeToken,_wavax,_swapHelper,_FeePool)
        leverageSuperToken(leverageFactory) {
    ...
    function totalUnderlying()public virtual override(AaveSuperToken,superTokenInterface) view returns (uint256){
        return availableBalance().add(totalLoan);
    }
}