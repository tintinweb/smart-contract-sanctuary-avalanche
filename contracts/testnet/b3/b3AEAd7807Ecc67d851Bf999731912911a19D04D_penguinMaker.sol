...
import "./penguinpacks.sol";
import "./penguin.sol";
contract penguinMaker is Ownable, Pausable{
    TestPacks2 penguinPacks;
    penguin penguinNft;
    using Strings for uint;
    event Mint(uint[] traits, uint tokenId);
    constructor(){
    }
    
    function setContracts(address packs, address penguins) public onlyOwner {
        penguinPacks = TestPacks2(packs);
        penguinNft = penguin(penguins);
    }

    function mint(uint256[] memory traits) public whenNotPaused{
        require(traits[0]<=500, "invalid trait 0");
        require(500 < traits[1] && traits[1] <= 1000, "invalid trait 1");
        require(1000 < traits[2] && traits[1] <= 1500, "invalid trait 2");
        require(1500 < traits[3] && traits[1] <= 2000, "invalid trait 3");
        require(2000 < traits[4] && traits[1] <= 2500, "invalid trait 4");
        require(2500 < traits[5] && traits[1] <= 3000, "invalid trait 5");
        require(3000 < traits[6] && traits[1] <= 3500, "invalid trait 6");
        for(uint i=0; i<traits.length;i++){
            require(penguinPacks.ownerOf(traits[i]) == msg.sender, string.concat("not the owner of trait " , i.toString()));   
            penguinPacks.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, traits[i]);
        }
        penguinNft.mint(msg.sender);
        emit Mint(traits, penguinNft.totalSupply());
    }
...

{
    ...
    address  public royaltyAddress = 0x36959ff843a686e81ec799c3598A9213c82D7DB3;



    string public baseURI ="ipfs://QmX3q2Gf8d6hVjDshbzeJQVq1KhG1245SrrQ8f7Qqm41ab/";
    string public baseExtension = ".json";


    // VARIABLES
    uint256 public maxSupply = 3500;
    uint256 public maxPerTx = 42;
    uint256 public maxPerPerson = 42;
    uint256 public price = 0 ether;
    uint256 public packSize = 7;
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("Test Pack", "TEST") ClampedRandomizer(maxSupply) {
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        for (uint256 i = 0; i < (amount * packSize); i++) {
    ...
    {
        return string.concat(super.tokenURI(tokenId), baseExtension);
    }
    ...
    //Overrides
       ...
       function mintTokenInternal(uint256 amount, address to) internal {
    ...
    function withdrawTokens(address _tokenContract) external onlyOwner {
        ...
        function airdropToken(address[] memory _addr, uint256 amount) public onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {
            airdropTokenInternal((amount * packSize),_addr[i]);
        }
    }
    ...
    Authorizable
{
    ...
    string public baseURI = 'https://potluck-server-jcnmfxprma-uc.a.run.app/metadata/';
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("Potluck Penguins", "PP") {
        require(_devList.length == _fees.length, "Error: invalid data");
        uint256 totalFee = 0;
        for(uint8 i = 0; i <_devList.length; i++) {
            devList.push(_devList[i]);
