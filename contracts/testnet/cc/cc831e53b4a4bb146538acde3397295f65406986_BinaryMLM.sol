...
contract BinaryMLM {
    ...
    // Amount of the joining fee in wei
    uint256 public joiningFee = 10000000000000000000;

    // Amount of the referral bonus in wei
    uint256 public referralBonus = 100000000000000000;

    // Amount of the pairing bonus in wei
    uint256 public pairingBonus = 200000000000000000;

    // Join function to add a new member to the network
    function join(address payable sponsor) public payable {
        // Check if the joining fee has been paid
        require(msg.value >= joiningFee, "Joining fee not paid");
        ...
        // Pay the referral bonus to the sponsor
        sponsor.transfer(referralBonus);

        // Check if the sponsor has a full team
        if (sponsorInfo.leftNode != 0 && sponsorInfo.rightNode != 0) {
            // Pay the pairing bonus to the sponsor
            sponsor.transfer(pairingBonus);
        }
    }
}