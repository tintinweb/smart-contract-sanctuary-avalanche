...
contract NFTStaking is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _nftAddress, address _rewardTokenAddress, uint256 _rewardTokenPerDay) public initializer {
        __Ownable_init();
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721Upgradeable(nftAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721Upgradeable(nftAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721Upgradeable(nftAddress).ownerOf(tokenIds[i]) != address(this)) continue;

            uint256 temp = pendingReward(msg.sender, tokenIds[i]);
            pending = pending.add(temp);
            
            removeFromStakingInfo(tokenIds[i]);
            IERC721Upgradeable(nftAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
    ...
    function getUnstakedItems(address _user) public view returns(uint256[] memory) {
        uint256 ownerTokenCount = IERC721Upgradeable(nftAddress).balanceOf(_user);

		uint256[] memory ownedTokenIDs = new uint256[](ownerTokenCount);

		for (uint256 i = 0; i < ownerTokenCount; i++) {
            uint256 tokenId = IERC721EnumerableUpgradeable(nftAddress).tokenOfOwnerByIndex(_user, i);
