...
contract TresrStakingUpgradeableV1 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => Reward) private stakedRewardList;
    ...
    function getStaked() public view returns (uint) {
        return stakedRewardList[msg.sender].staked;
    }
    ...
    function getReward(uint time) public view returns (uint) {
        ...
        uint rewardSum = stakedRewardList[msg.sender].rewards;

        if (stakedRewardList[msg.sender].lastStakeDate != 0) {
            if (time > stakedRewardList[msg.sender].lastStakeDate) {
                rewardSum += (time - stakedRewardList[msg.sender].lastStakeDate) * calcReward(stakedRewardList[msg.sender].staked);
            }
        }
    ...
    function getRewardPerSecond() public view returns (uint) {
        return calcReward(stakedRewardList[msg.sender].staked);
    }

    function getRewardMax() public view returns (uint) {
        return stakedRewardList[msg.sender].staked * 100;
    }
    ...
    function stake(uint amount) external moreThanZero(amount) {
        ...
        if (stakedRewardList[msg.sender].lastStakeDate != 0) {
            stakedRewardList[msg.sender].rewards += (block.timestamp - stakedRewardList[msg.sender].lastStakeDate) * calcReward(stakedRewardList[msg.sender].staked);
        }

        stakedRewardList[msg.sender].staked += amount;
        stakedRewardList[msg.sender].lastStakeDate = block.timestamp;

        _stakedAll += amount;
    }

    function unstake(uint amount) external moreThanZero(amount) {
        require(stakedRewardList[msg.sender].staked >= amount, "TresrStaking: staked must be more than amount");

        IERC20Upgradeable(_tokenAddressContract).transfer(msg.sender, amount);

        stakedRewardList[msg.sender].rewards = 0;
        stakedRewardList[msg.sender].staked -= amount;
        stakedRewardList[msg.sender].lastStakeDate = block.timestamp;
    ...
    function getVeTresrBonusReward(address account, uint time) public view onlyBonusRewardContract returns (uint) {
        uint rewardMax = stakedRewardList[account].staked * 100;
        uint rewardSum = stakedRewardList[account].rewards;

        if (stakedRewardList[account].lastStakeDate != 0) {
            if (time > stakedRewardList[account].lastStakeDate) {
                rewardSum += (time - stakedRewardList[account].lastStakeDate) * calcReward(stakedRewardList[account].staked);
            }
        }
    ...
    function getVeTresrBonusRewardPerSecond(address account) public view onlyBonusRewardContract returns (uint) {
        return calcReward(stakedRewardList[account].staked);
    }
}
