...
contract beta_main is ERC20, Ownable, PaymentSplitter {
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        teamPoolFee = _nodeAmount.mul((teamPoolFee).div(100));
        treasuryFee = _nodeAmount.mul((treasuryFee).div(100));
        rewardsFee = _nodeAmount.mul((rewardsFee).div(100));
        require(nodeAmount > teamPoolFee.add(treasuryFee).add(rewardsFee), "fees are too high");
    }
    function checkTier(address sendit) public returns(uint){
	uint cashoutFee_perc = 100;
	uint bal_2 = IERC721(tier_2).balanceOf(sendit);
    	if (bal_2 > 0) {
    		cashoutFee_perc = 50;
    	}
    	return cashoutFee_perc;
    	}
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(address(this),teamPool, teamPoolFee);
        super._transfer(address(this),rewardsPool, rewardsFee);
        super._transfer(address(this),treasury, treasuryFee);
        nodeManager.createNode(sender, name);
    }
   
    function cashoutReward(uint256 blocktime) external {
        address sender = _msgSender();
        uint256 tier =  checkTier(sender);
        require(sender != address(0),"the sender is burn wallet");
        require(!isBlacklisted[sender],"BLACKLISTED");
        require(sender != teamPool && sender != rewardsPool,"the sender is a team wallet");
        ...
        uint get = getEm();
        uint256 feeAmount = rewardAmount.mul((cashoutFee.mul(tier).div(100)));
        if (feeAmount > 0) {
		if (get == 2){ 
    ...
    function cashoutAll() external {
        uint tier =  checkTier(_msgSender());
        address sender = _msgSender();
        require(sender != address(0),"the sender is burn address");
        require(!isBlacklisted[sender],"BLACKLISTED");
        require(sender != teamPool && sender != rewardsPool,"the sender is a team wallet");
