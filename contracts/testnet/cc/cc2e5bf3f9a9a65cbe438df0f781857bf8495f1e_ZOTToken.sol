...
contract ZOTToken is ERC20 {
    ...
    constructor() ERC20("ZOTToken ", "ZOT") {
    ...
    function getAllPendingTransaction() public view onlyOwner returns(uint[] memory) 
    {
    ...
    function createPendingTransaction(address _to, uint _amount, uint Due_Date) public onlyOwner returns(uint)
    {
        ...
        ownerTransactions[id] = OwnerTransaction(_to, _amount, "pending", block.timestamp + (Due_Date*60*60*24));
    ...
    function approvePendingTransaction(uint _id) onlyOwner public
    {
        uint id = _id;
