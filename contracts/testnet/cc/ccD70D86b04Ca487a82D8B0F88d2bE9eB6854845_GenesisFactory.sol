...
contract GenesisFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  // deployer => bill id => bill info
  mapping(address => mapping(uint => BillInfo)) public billInfo;
  // deployer => bills deployed
  mapping(address => uint) public billIds;
  ...
  function initialize(
    ...
    genesisTreasury = _genesisTreasury;
  ...
  function deploy(
    ...
    address nftProxy = _deployNftProxy(_billNftParams);
    address billProxy = _deployBillProxy(_billParams, treasuryProxy, nftProxy);

    _registerBill(billProxy, nftProxy, treasuryProxy);

    emit Deploy(billProxy, nftProxy, treasuryProxy);
  }
  ...
  function _deployNftProxy(BillNftParams calldata _billNftParams) internal returns (address) {
  ...
  function _deployBillProxy(
    ...
    ERC1967ProxyInitializable billProxy = _deployProxy(
      billImplementation,
      abi.encodeWithSelector(
    ...
    BillNft(_nftProxy).addMinter(address(billProxy));

    return address(billProxy);
  }
  ...
  function _registerBill(address billProxy, address nftProxy, address treasuryProxy) internal {
    billInfo[msg.sender][billIds[msg.sender]] = BillInfo({
      bill: billProxy,
      nft: nftProxy,
      treasury: treasuryProxy
    });

    billIds[msg.sender] += 1;
  }

  function setGenesisTreasury(address _genesisTreasury) external onlyOwner {
    genesisTreasury = _genesisTreasury;
  }
...
contract GenesisBill is Initializable, PolicyUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        require(_config[4] != address(0), "billNft cannot be zero");
        billNft = IBillNFT(_config[4]);

        require(_config[5] != address(0), "policy cannot be zero");
        initPolicy(_config[5]);
    ...
    */
    function batchSetBillTerms(Terms memory _terms) external onlyPolicy {
...
contract BillNft is IVestingNFT, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
      ...
      address proxyOwner
        ...
        // Allows proxy owner set GenesisBill contract as minter
        // This role is revoked during the proxy deployment after GenesisBill role is set
