...
contract Vesting is Ownable {
  ...
  function release() public {
    uint256 lockTimestamp = _getLockTimestamp(uint256(block.timestamp));
    require(lockTimestamp > 0, "Can't release CHRO");
    Account storage account = _accounts[msg.sender];

    uint256 _vestedAmount = _vestingSchedule(account.balance.add(account.released), lockTimestamp).sub(account.released);
  ...
  function _getLockTimestamp(uint256 _timestamp) internal view virtual returns (uint256) {
    ...
    uint256 lastTime = start().add(duration());
