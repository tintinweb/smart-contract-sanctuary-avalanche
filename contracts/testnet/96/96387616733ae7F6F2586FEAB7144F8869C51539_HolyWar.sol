...
contract HolyWar is AccessControlEnumerable, GameManager, VRFManager {
    ...
    function startHolyWar() external payable {
        ...
        require(active, "startHolyWar: game contract must be active to play");
    ...
    function goToWar() external payable {
        ...
        require(
            msg.value <= gameData[gameId].ante * 5,
            "goToWar: _sideBet cannot be more then 5 times the ante"
        );

        holdFundsForGotoWar(msg.value);

        gameData[gameId].sidebet += msg.value;
    ...
    function payout(PayoutArgs memory _payoutArgs) internal {
        require(
    ...
    function holdFundsForGotoWar(uint256 _sideBet) internal {
