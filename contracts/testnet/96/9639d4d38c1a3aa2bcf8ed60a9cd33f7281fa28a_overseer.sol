...
abstract contract nft_stake is Context {
    ...
    function getEm(address _account) external view returns (uint[3] memory) {}
  ...
  Authorizable
{
  ...
  nft_stake public _stake;
  bool stableOn = true;
  constructor(address[] memory swaps){
	  swap_add = swaps[0];
	  stable_swap_add = swaps[1];
	  token_add = swaps[2];
	  main_add = swaps[3];
	  stable_add = swaps[4];
	  _stake = nft_stake(0x99a414350Bb9036edda3cF9c89bf2B2C7EC61B28);
 ...
 function updateStake(address newVal) external onlyOwner {
        stake = newVal; //true: token0 = stable && token1 = main
  ...
  function getStablePrice() public view returns (uint)
   ...
   {
   	uint256[3] memory Tier = _stake.getEm(_account);
   	return Tier;
   }
