...
contract ERC20DividendReward is Ownable, RewardStrategy {
    ...
    function updateWeight(address _investor, uint oldWeight, uint oldTotalWeight, uint newWeight) external override {
    ...
    function distribute(string memory originOfTheFunds, uint _amount) external payable onlyOwner {
...
contract Staking is Ownable {
    ...
    uint public totalStacked; // Used for GUI
    uint public totalWeight; // Used by reward pools

    IERC20 public stakedToken;
    RewardStrategy[] public rewards;
    ...
    function removeRewardContract(address _reward) external onlyOwner {
      // Todo : Allow to remove a contract to save gas
    }
    ...
    function stake(uint amount, bool locked) external {
      ...
      uint newWeight;
      if (locked) {
        newWeight = 3 * user.staked;
        user.lockedUntil = block.timestamp + 30 days;
      } else {
