...
import {IJToken} from "../../interfaces/traderJoe/IJoeLens/IJToken.sol";
import {IMasterChefJoeV2} from "../../interfaces/traderJoe/IMasterChefJoeV2.sol";
import {IJoeBar} from "../../interfaces/traderJoe/IJoeBar.sol";
import {IJoeFactory} from "../../interfaces/traderJoe/IJoeFactory.sol";
import {IJoeRouter02} from "../../interfaces/traderJoe/IJoeRouter02.sol";

contract TraderJoeAdapter is AdapterBase, Basic {
    using SafeERC20 for IERC20;

    address public constant routerAddr =
        0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    IJoeRouter02 internal router = IJoeRouter02(routerAddr);

    address public constant joeBarAddr =
        0x57319d41F71E81F3c65F2a47CA4e001EbAFd4F33;

    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "TraderJoe")
    {}
    ...
    event TraderJoeStake(address owner, uint256 stakeAmount);
    event TraderJoeUnStake(address owner, uint256 stakeAmount);
        ...
        onlyAdapterManager
    {
        ...
        address exchangeAddr = IJoeFactory(router.factory()).getPair(
            tokenA,
            tokenB
        );
        ...
        onlyAdapterManager
    {
        ...
        (uint256 _amountToken, uint256 _amountAVAX, uint256 _liquidity) = router
            .addLiquidityAVAX{value: amountAVAXDesired}(
            tokenAddr,
            amountTokenDesired,
            amountTokenMin,
            amountAVAXMin,
    ...
    function depositLpToken(bytes calldata encodedData) external onlyProxy {
        (address masterChefAddr, uint256 pid, uint256 amount) = abi.decode(
            encodedData,
    ...
    function enter(bytes calldata encodedData) external onlyProxy {
        uint256 amount = abi.decode(encodedData, (uint256));
        IJoeBar(joeBarAddr).enter(amount);
        emit TraderJoeStake(address(this), amount);
    }

    function leave(bytes calldata encodedData) external onlyProxy {
        uint256 amount = abi.decode(encodedData, (uint256));
        IJoeBar(joeBarAddr).leave(amount);
        emit TraderJoeUnStake(address(this), amount);
    }
}
