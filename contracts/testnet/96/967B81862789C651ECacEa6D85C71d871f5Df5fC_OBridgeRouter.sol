...
contract OBridgeRouter is Initializable, OwnableUpgradeable {
    ...
    function mintOToken(address token, address to, uint256 amount) external onlyOwner returns (bool) {
        IOBridgeERC20(token).mint(to, amount);
        return true;
    }

    function burnOToken(address token, address from, uint256 amount) external onlyOwner returns (bool) {
