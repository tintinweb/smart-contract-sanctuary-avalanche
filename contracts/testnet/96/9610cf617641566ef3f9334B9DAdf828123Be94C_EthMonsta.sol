    ...
    ReentrancyGuardUpgradeable
{
    ...
    event TaxesUpdated(address indexed owner, uint256 sellTax);
    ...
    function initialize(address router, uint256 _sellTax) public initializer {
        __ERC20_init("ETH Monsta", "MONSTE");
    ...
    function setTaxes(uint256 _sellTax) external onlyOwner {
        ...
        sellTax = _sellTax;
        emit TaxesUpdated(_msgSender(), _sellTax);
    }
        ...
        whenNotPaused
    {
        ...
        // sells
        if (to == pair && !_inSwapAndLiquify && sellTax > 0) {
            // take taxes
            if (!isExcludedFromFee[from]) {
                uint256 fee = (amount * sellTax) / 1e4;
                amount -= fee;
                //super._transfer(from, address(this), fee);
                //_swapTokensForEth(fee);
            }
        }
        super._transfer(from, to, amount);
    }
