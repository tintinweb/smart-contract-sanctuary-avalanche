...
contract Expedition is Ownable {
    ...
    function SetmaxStakeLimit(uint256 _maxStakeLimit) external onlyOwner {
        maxStakeLimit = _maxStakeLimit;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        require(_amount <= maxStakeLimit, "hitting maxStakeLimit");
        PoolInfo storage pool = poolInfo[_pid];
...
contract cToken is ERC20, Ownable {
    ...
    modifier onlyMiners() {
        require(
            owner() == _msgSender() || miners[_msgSender()] == true,
