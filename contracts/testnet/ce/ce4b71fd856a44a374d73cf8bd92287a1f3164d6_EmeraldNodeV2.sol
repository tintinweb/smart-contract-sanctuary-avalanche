...
contract EmeraldNodeV2 is EmeraldNodeStorageV2 {
    ...
    */
    ...
    function updateAvaxAddress(address _address) external onlyOwner returns(bool){
    ...
    */
    ...
    function updateBondTresuryAddress(address _address) external onlyOwner returns(bool){
    ...
    */
    ...
    function updateCount(uint256 _num, uint256 _num2) external onlyOwner returns(bool){
        count = _num;
    ...
    */
    ...
    function whiteListAddresses(address[] calldata _addresses, bool _status) external onlyOwner {
    ...
    */
    ...
    function buyNode(Order memory order, bytes memory signature) payable isBlacklistedAddress external returns(bool){
        ...
        if(whiteListAddress[msg.sender] && addressCount[msg.sender] < count && nodeCounter <= nodeIds[1]){
            ...
            nodeId = nodeCounter;
            nodeCounter += 1;
        }else if(IcoWhitelistData[msg.sender].user == msg.sender && IcoWhitelistData[msg.sender].allowedNode > IcoWhitelistData[msg.sender].createdNode && IcoNodeCounter <= nodeIds[7]){
            _tier = 7;
            addressCount[msg.sender] += 1;
            ...
            for(uint256 i = 0; i < 5; i++){
                ...
                if(nodeIds[5] < RegularNodeCounter && nodeIds[6] >= RegularNodeCounter){
                    _tier = 6;
                    nodeId = RegularNodeCounter;
                    RegularNodeCounter += 1;
                    break;
        ...
        EmeraldCoin.safeTransfer(msg.sender, address(this), updatedTierDetails[_tier].tierEMnodePrice);
        EmeraldCoin.approve(communityAddress,  updatedTierDetails[_tier].tierEMnodePrice);
        EmeraldCoin.safeTransfer(address(this), communityAddress,  updatedTierDetails[_tier].tierEMnodePrice);
    ...
    */
    ...
    function viewNodeDetails(uint256 _nodeId) public view returns(
        ...
        if(nodeDetails[_nodeId].nodeOwner != address(0)){
            number =  uint256(block.timestamp - LastClaimTime) / updatedTierDetails[tierId].intervalTimestamp;
    ...
    */
    ...
    function IcoWhiteListAddresses(IcoWhitelistAddress[] calldata _addresses) external onlyOwner returns(bool){
        for(uint i = 0; i < _addresses.length; i++){
            IcoWhitelistAddress storage data = IcoWhitelistData[_addresses[i].user];
    ...
    // remove ICO whitlist
    function removeIcoWhiteListAddresses(address[] calldata _addresses) external onlyOwner returns(bool){
        for(uint i = 0; i < _addresses.length; i++){
    ...
    function checkTreasuryValueInUSD() public view returns(uint256 TreasuryPriceInUSD){
        TreasuryPriceInUSD = bondTreasuryAddress.checkTreasuryValueInUSD();
    }

    function updateBPNPercentage(uint256 _avax_percent, uint256 _em_percent) external onlyOwner{
        require(_avax_percent + _em_percent == 100, "EmeraldNodeV1: Total must be 100 percentage");
    ...
    function sellNode(uint256 _nodeId) external returns(bool){
       require(nodeDetails[_nodeId].nodeOwner == msg.sender, "EmeraldNodeV1: You are not owner");
       ...
       uint256 nodes = (IcoNodeCounter + RegularNodeCounter + nodeCounter) - deletedNodeCounter;
       ...
       uint256 _avax_price = _avax_value *(10**18)/ uint256(bondTreasuryAddress.getAvaxToUsdPrice());

       AvaxCoin.transferFrom(owner(), address(this), _avax_price);
       AvaxCoin.transfer(msg.sender, _avax_price);

       uint256 _em_price = _em_value *(10**18)/ uint256(bondTreasuryAddress.getEmToUSDPrice());
