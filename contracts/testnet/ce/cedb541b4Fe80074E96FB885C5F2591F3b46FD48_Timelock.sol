...
contract Timelock is ITimelock {
  ...
  function initXlxManager() external onlyAdmin {
    IXlxManager _xlxManager = IXlxManager(xlxManager);

    IMintable xlx = IMintable(_xlxManager.xlx());
    xlx.setMinter(xlxManager, true);

    IUSDG usdg = IUSDG(_xlxManager.usdg());
    usdg.addVault(xlxManager);

    IVault vault = _xlxManager.vault();
    vault.setManager(xlxManager, true);
  }
  ...
  function initRewardRouter() external onlyAdmin {
    ...
    IHandlerTarget(xlxManager).setHandler(rewardRouter, true);
  }
  ...
  function setShouldToggleIsLeverageEnabled(bool _shouldToggleIsLeverageEnabled)
    external
    onlyHandlerAndAbove
  {
  ...
  function setInPrivateLiquidationMode(address _vault, bool _inPrivateLiquidationMode)
    external
    onlyAdmin
  {
  ...
  function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode)
    external
    onlyAdmin
  {
