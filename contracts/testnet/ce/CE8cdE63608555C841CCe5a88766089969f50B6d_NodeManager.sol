...
contract NodeManager is Ownable, Pausable {
   ...
   function returnSoonestDue(address _account) internal returns(uint ){
    ...
    function compileVars(address _account,uint256[3] memory tier) external onlyGuard whenNotPaused returns (uint256){
		...
		_total.dailyRewards = Zero;
	    	...
	    	for (i = 0; i < tier[i]; i++) {
    			tier[i] = tier[i].mul(5);
    			for (uint j = rndm_ls.length; j < rndm_ls.length+tier[i]; j++) {
    				rndm_ls[j] = _boostRewardPerc[i];
    				}
    			}
	    	for (i = 0; i < nodes.length; i++) {
			    NodeEntity storage _node = nodes[i];
			    require(_node.boost != 1231223123123,"i suck!");
			    ...
			    if (paid == true){
        		    ...
        		    uint256[2] memory rew = takeFee(((rewardPerDay.mul(10000).div(1440) * ((elapsed -(block.timestamp - nextDue))  / 1 minutes)) / 10000),rndm_ls[j]);
