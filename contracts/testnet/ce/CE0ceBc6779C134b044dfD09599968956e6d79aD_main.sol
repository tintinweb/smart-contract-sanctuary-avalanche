...
contract main is ERC20, Ownable {
    ...
    address _overseer = 0xf541273f8369Bcf3b992402B00AbCB9957E58FEe;//addresses[4];
   ...
   function Send_it(address _account, address _destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256[4] memory _fees = [myLib.doPercentage(nodeAmount,rewardsFee),myLib.doPercentage(nodeAmount,treasuryFee),myLib.doPercentage(nodeAmount,teamPoolFee),myLib.doPercentage(nodeAmount,OverSeerProtocolFee)];
   ...
   function cashoutAll() external {
        ...
        (uint256[3] memory Tiers, uint256 amount) =  _overseer_.getGreensAmount(sender);
        ...
        for(uint i=0;i<length;i++){
        	...
        	if (amount > 0){
        		...
        		(uint256 _time ,uint256 _boost) = _overseer_.getCurrGreens(msg.sender,j,c);
        		count += 1;
        		if (count == 5) {
        			c += 1; 
        			count = Zero;
        	...
        	rewards += nodeManager.getNodesRewards(sender,time,i,_boost,_time);
        }
