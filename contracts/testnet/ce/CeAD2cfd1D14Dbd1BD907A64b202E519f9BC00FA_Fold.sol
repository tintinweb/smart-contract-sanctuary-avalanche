...
contract Fold is ERC20, EnefteOwnership {
    ...
    * @param _amount amount to mint
    */
    function mintStakingShare(address _stakingContract, uint _amount) external onlyOwner notLocked {
        uint256 realAmount = _amount*FOLD_TOKEN_PRECISION;
        if(realAmount + totalSupply() > MAX_SUPPLY){
            revert("Tokens all allocated");
        }
        _mint(_stakingContract, realAmount);
    }
    ...
    */
    ...
    //
    //
    //  Add in transfer on $fld to add 1x deposit/withdraw, exclude contracts and 0x0.
    //
    //
    //
