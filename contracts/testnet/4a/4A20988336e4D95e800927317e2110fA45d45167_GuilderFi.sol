...
contract GuilderFi is IGuilderFi, IERC20, Ownable {
    ...
    function setLrfAddress (address _address) external override onlyOwner {
        lrf = ILiquidityReliefFund(_address);
        initSubContract(_address);
    }

    function setAutoLiquidityAddress (address _address) external override onlyOwner {
        autoLiquidityEngine = IAutoLiquidityEngine(_address);
        initSubContract(_address);
    }        

    function setSafeExitFundAddress (address _address) external override onlyOwner {
        safeExitFund = ISafeExitFund(_address);
        initSubContract(_address);
    }
    
    function setPreSaleAddress (address _address) external override onlyOwner {
