...
contract BLV_IDO is Ownable {
    ...
    */
    ...
    // function getValues(uint256 _tokenAmount) public view returns(uint256, uint256){
    //     uint256 amount = _tokenAmount.sub(BLVFee);
    //     uint256 per = ((fixedValue).mul(1 ether)).div(BUSD_to_BLV);
    //     uint256 tokens = (amount.mul(per)).div(1 ether);

    //     // uint256 amountsss. =
    //     uint256 perrrr = BUSD_to_BLV.div(fixedValue);
    //     uint256 amountsss = amount.mul(perrrr);
    //     return (tokens, amountsss);
    // }
