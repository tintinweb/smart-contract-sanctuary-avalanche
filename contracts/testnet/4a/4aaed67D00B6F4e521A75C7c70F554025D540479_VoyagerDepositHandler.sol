...
library Setter {
    ...
    function setUsdcBurnableAndMintable(
        ...
        // Array length mismatch
        if (_destChainID.length != _destChainType.length && _destChainID.length != _setTrue.length) {
            revert VoyagerError(2);
        }
    ...
    function handleRequestFromRouter(
        ...
        if (keccak256(abi.encodePacked(sender)) != keccak256(abi.encodePacked(routerBridge))) {
            // only Voyager middleware
    ...
    IApplication
{
    ...
    function _isGateway() private view {
        ...
        if (msg.sender != address(gatewayContract)) {
    ...
    function setUsdcBurnableAndMintable(
        ...
        Setter.setUsdcBurnableAndMintable(_isUsdcBurnableMintable, _destChainType, _destChainID, _setTrue);
        // require(
        //     _destChainID.length == _destChainType.length && _destChainID.length == _setTrue.length,
        //     "Array length mismatch"
        // );
    ...
    function decodeArbitraryData(
        ...
        ) = abi.decode(arbitraryData, (bytes, bytes, bytes, uint256, uint256));
    }
    ...
    function sendRequestToRouter(bytes memory payload, uint256 value) internal {
        gatewayContract.requestToRouter{ value: value }(0, "0x", payload, routerBridge, 1000000, "0x");
    }

    function checkBurnableAndLock(address token, address user, uint256 amount) internal {
        if (!_burnList[token]) {
            _reserve.lockERC20(token, user, address(_reserve), amount);
        } else {
    ...
    function transferNativeToken(
        ...
        convertToWeth(srcTokenAmount, msg.sender);
        ...
        sendRequestToRouter(payload, msg.value.sub(srcTokenAmount));
    }
    ...
    function transferReserveToken(
        ...
        if (_isUsdcBurnableMintable[destChainIdBytes] && _usdc == srcTokenAddress) {
            // Burn USDC
        ...
        sendRequestToRouter(payload, msg.value);
    }
    ...
    function depositReserveToken(
        ...
        if (isSourceNative) {
            ...
            convertToWeth(swapDetails.srcStableTokenAmount, msg.sender);
        }
        ...
        sendRequestToRouter(payload, msg.value.sub(swapDetails.srcStableTokenAmount));
    }
    ...
    function depositReserveTokenAndExecute(
        ...
        if (isSourceNative) {
            ...
            convertToWeth(swapDetails.srcStableTokenAmount, swapDetails.depositor);
        }
    ...
    function depositNonReserveToken(
        ...
        if (isSourceNative) {
            ...
            convertToWeth(swapDetails.srcTokenAmount, msg.sender);
        }
        ...
        sendRequestToRouter(payload, msg.value.sub(swapDetails.srcTokenAmount));
    }
    ...
    function depositNonReserveTokenAndExecute(
        ...
        if (isSourceNative) {
            ...
            convertToWeth(swapDetails.srcTokenAmount, swapDetails.depositor);
        }
    ...
    function handleRequestFromRouter(
        ...
        Setter.handleRequestFromRouter(_executionRevertCompleted, _burnList, _reserve, routerBridge, sender, payload);
        // require(
        //     keccak256(abi.encodePacked(sender)) == keccak256(abi.encodePacked(routerBridge)),
        //     "only Voyager middleware"
        // );
