...
contract ServiceMarket is Ownable, CloneFactory {
    ...
    mapping(bytes32 => mapping(uint => EscrowInfo)) public escrowList;
    mapping(bytes32 => uint) public escrowLength;
    ...
    function confirmSNFT(bytes32 _key, address user, bool prefund) external isBlackList {
        require(sNftInfo[_key].confirmUser == address(0), "Main:already assigned");
        ...
        if(msg.sender == sNftInfo[_key].maker) {
            ...
            for(uint i = 0; i < auctionInfoList.length; i++) {
                if(user == auctionInfoList[i].taker) {
                    milestoneLen = auctionInfoList[i].price.length;
                    for(uint j = 0; j < milestoneLen; j++) {
                        takerPrice += auctionInfoList[i].price[j];
                        makerPrice += sNftInfo[_key].price[j];
                        escrowList[_key][j] = EscrowInfo(auctionInfoList[i].price[j], EscrowStateType.ES_PROCESS);
                    }
                    escrowLength[_key] = milestoneLen;
            ...
            if (prefund == true) {
                ...
                for (uint k = 0; k < milestoneLen; k ++) {
                    escrowList[_key][k].state = EscrowStateType.ES_FUNDED;
                }
            }
    ...
    function fundMilestion(bytes32 _key, uint _index) external isBlackList {
        require(msg.sender == sNftInfo[_key].maker, "Main:not maker");
        IERC20(sNftInfo[_key].coin).safeTransferFrom(msg.sender, address(this), escrowList[_key][_index].price);
        escrowList[_key][_index].state = EscrowStateType.ES_FUNDED;
    }

    function releaseMilestone(bytes32 _key, uint _index) external isBlackList {
        require(msg.sender == sNftInfo[_key].maker, "Main:not maker");
        escrowList[_key][_index].state = EscrowStateType.ES_RELEASED;
        sNftInfo[_key].releaseId ++;
    }

    function takeMilestone(bytes32 _key, uint _index) external isBlackList {
        require(msg.sender == sNftInfo[_key].confirmUser, "Main:not taker");
        require(escrowList[_key][_index].state == EscrowStateType.ES_RELEASED, "Service: node release");
        IERC20(sNftInfo[_key].coin).safeTransfer(msg.sender, escrowList[_key][_index].price * 97 / 100);
        IERC20(sNftInfo[_key].coin).safeTransfer(treasury, escrowList[_key][_index].price * 3 / 100);
        escrowList[_key][_index].state = EscrowStateType.ES_CLAIMED;
    }

    function completeProject(bytes32 _key) external isBlackList {
        IERC1155(sNftInfo[_key].collectionId).safeTransferFrom(sNftInfo[_key].maker, sNftInfo[_key].confirmUser, sNftInfo[_key].tokenId, 1, "");
    }

    function openDisputEscrow(bytes32 _key, uint _index) external isBlackList {
        escrowList[_key][_index].state = EscrowStateType.ES_DISPUTE_OPEN;
    }

    function declineDisputEscrow(bytes32 _key, uint _index) external {
        escrowList[_key][_index].state = EscrowStateType.ES_DISPUTE_DECLINED;
    }

    function solveDisputEscrow(bytes32 _key, uint _index, uint256 percent) external {
        uint amount = escrowList[_key][_index].price;
        uint refundAmout = amount * percent / 10000;
        uint releaseAmout = amount - refundAmout;
        IERC20(sNftInfo[_key].coin).safeTransfer(sNftInfo[_key].confirmUser, releaseAmout);
        IERC20(sNftInfo[_key].coin).safeTransfer(sNftInfo[_key].maker, refundAmout);
        escrowList[_key][_index].state = EscrowStateType.ES_DISPUTE_SOLVED;
    }
