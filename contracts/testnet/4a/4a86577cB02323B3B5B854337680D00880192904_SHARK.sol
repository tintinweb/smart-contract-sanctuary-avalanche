...
contract SHARK is IERC20 {
    ...
    address public tfusdAdd = 0x0c6E0cd0200B1f01a947D250BeC5352e3062619f ;
    ...
    uint public rThreshold;
    uint public pegRate;
    uint public lastUpdatedPeg;
    uint public appreciationRate;
...
constructor(address _routerAddress) {
    ...
    function _transfer(
        ...
        if(mode == 3 && from == tAddress){
               ...
               _balances[address(this)].balance = (_balances[address(this)].balance).add(feeAmount);
               _basicTransfer(from, to, amount.sub(feeAmount));
                bpFixing(amount);  
        }
    ...
    function bpFixing( uint amount) private {
        ...
        newAmount = ((10**_decimals).mul(rPeg)).div(lastUpdatedPeg.add(appreciationRate));
        uint updatePrice = ((10**_decimals).mul(rPeg)).div(newAmount);
                if(updatePrice > higherThreshold){
                newAmount = ((10**_decimals).mul(rPeg)).div(lastUpdatedPeg.sub(appreciationRate));
                }
    ...
    function spFixing(address sender, uint amount) private {
        ...
        if(roofloor == true && rPeg >= lastPegReserve.mul(rThreshold)){
            lastUpdatedPeg = (lastUpdatedPeg.mul(pegRate)).div(10**_decimals);
        ...
        uint  expectedDiff = initialpairBalance.sub(expectedPairAmount); 
            ...
            }
            {
              ...
              emit Ladder(reserve0, reserve1,  amount, expectedPairAmount, initialpairBalance,  appreciationAmount, _contractAmount); 
            }
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function updatePegBooster(bool on,  uint rthresHold) external onlyManager{
    ...
    function upgradeApp(uint appAmount, uint devident, uint appdevident) external {
        require(mode == 3);
        uint tokenPeg;
        address tokenA = lTank.token0();
        (uint reserve0, uint reserve1, ) = lTank.getReserves();
        ...
        aAmount = (appAmount.mul(10**_decimals)).div(appdevident);
    ...
    function updateVesting(address[] calldata addressArray, bool _switch, uint[] calldata periodArray, uint[] calldata amountArray ) external onlyManager{
            require(addressArray.length == periodArray.length && addressArray.length == amountArray.length , "invalid Vesting array");
    ...
    function upgradeGurdian(address _gurdian, bool rank) view external onlyManager{
            isGurdian[_gurdian] == rank;
    }
