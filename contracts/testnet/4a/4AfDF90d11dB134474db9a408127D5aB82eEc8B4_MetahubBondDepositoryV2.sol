...
/// @title Metahub Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract MetahubBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        IMetahubAuthority _authority,
        IERC20 _mhub,
        IgMHUB _gmhub,
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _mhub, _gmhub, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _mhub.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gMHUB due
        ...
        )
    {
         ...
         * price = quote tokens : mhub (i.e. 42069 DAI : MHUB)
         *
         * 1e18 = MHUB decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of MHUB that the market can sell
        ...
        // purchased, and how much MHUB has been sold
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // mhub decimals (9) + price decimals (9)
             ...
             * is 10,000 MHUB. max payout would be 1,000 MHUB (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in MHUB or quote), initial price / MHUB (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         *
         * 1e18 = mhub decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 MHUB, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 MHUB.
         */
        ...
        emit CreateMarket(id_, address(mhub), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in MHUB decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of MHUB to be paid in MHUB decimals
     *
     * @dev 1e18 = mhub decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in MHUB decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in MHUB decimals
     */
...
interface IBondDepository {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in MHUB (false, default)
    ...
    struct Terms {
        ...
        uint64 maxDebt; // 9 decimal debt maximum in MHUB
    }
...
interface INoteKeeper {
    ...
    struct Note {
        uint256 payout; // gMHUB remaining to be paid
    ...
    function redeemAll(address _user, bool _sendgMHUB) external returns (uint256);
...
abstract contract FrontEndRewarder is MetahubAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable mhub; // reward token

    constructor(IMetahubAuthority _authority, IERC20 _mhub) MetahubAccessControlled(_authority) {
        mhub = _mhub;
    }
    ...
    function getReward() external {
        ...
        mhub.transfer(msg.sender, reward);
    }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgMHUB internal immutable gMHUB;
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _mhub) {
        gMHUB = _gmhub;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of MHUB due to the user
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({
                payout: gMHUB.balanceTo(_payout),
        ...
        // note that only the payout gets staked (front end rewards are in MHUB)
     ...
     * @param _sendgMHUB    send payout as gMHUB or sMHUB
     * @return payout_     sum of payout sent, in gMHUB
     */
    function redeem(
        address _user,
        uint256[] memory _indexes,
        ...
        if (_sendgMHUB) {
            gMHUB.transfer(_user, payout_); // send payout as gMHUB
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as sMHUB
        }
    }
     ...
     * @param _sendgMHUB    send payout as gMHUB or sMHUB
     * @return             sum of payout sent, in gMHUB
     */
    function redeemAll(address _user, bool _sendgMHUB) external override returns (uint256) {
        return redeem(_user, indexesFor(_user), _sendgMHUB);
    }
     ...
     * @return payout_     the payout due, in gMHUB
