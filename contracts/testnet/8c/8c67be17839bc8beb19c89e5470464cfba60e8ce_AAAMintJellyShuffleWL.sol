...
// File: contract written by 0xJelle. This is a modified campfire mint w royalties base contract, with wl from joepegs contract, and extention style from hashlips, shuffle reveal using witnet
//Could add more functions still such as:
//Customize name and symbol after launch;
//Customize address for royalties and mint fees;
//multiple wl phases with their own allowlist seeded;

pragma solidity ^0.8.0;


interface Ierc20Token {//generic ability to transfer out tokens accidentally sent into the contract
...
contract AAAMintJellyShuffleWL is ERC721URIStorage, Ownable, Pausable, ReentrancyGuard {
  using Counters for Counters.Counter;
  using Strings for uint256;
  Counters.Counter private _tokenIds;

  uint256 public latestRandomizingBlock;//checks if we have started the shuffle yet
  uint public randomUint;//random number gathered for the shuffle reveal
  IWitnetRandomness witnet;

  string public baseExtension = ".json"; //adds .json to the end of uri links
  bool public mintActive = true;//redundant mint pause feature, starts unpaused here
  bool public botsCanMint = true;//lets other smart contracts mint from our smart contract, starts letting bots mint here
  ...
  uint256 public devFeePercent = 500; //sends this percentage to the devFeeCollector address


    /// @notice The address to which the dev fees on the sale will be sent
  address public devFeeCollector = 0xD74F8a84b7b622Ad0536f9D99D56175c3dAB8a63; //0xJelle smart contract dev address
  ...
  uint256 public phaseNow;//1 is wl, 2 is public phase, 0 or 3+ is paused, 
  uint256 public pricePublic;//price for public phase mint
  uint256 public priceWL;//price for wl phase mint
  uint256 public maxSupply;//max nfts in the collection when all minted out
  string public baseURI;//baseURI link https or ipfs
  string public unrevealedURI;//baseURI link https or ipfs
  uint256 public royaltyPercent;//royalties precentage in basis points
  uint256 public maxMintPerTx;//max amount can mint per each transaction

  bytes4 public constant _INTERFACE_ID_ERC2981 = 0x2a55205a;//royalties interface


      /// @notice The amount of NFTs each allowed address can mint during the allowlist mint
  mapping(address => uint256) public allowlist;// allowlist mapping of addresses to number of nfts allowed to mint
  ...
  event MintWL(address indexed sender, uint quantity, uint priceWL, uint totalAfterMint);
  event MintPublic(address indexed sender, uint quantity, uint pricePublic, uint totalAfterMint);




//initialize with constructor here before deploy
  constructor() ERC721("ChiknOGMeme", "COGM") {//set your name and symbol here before deploying, there is no way to change it later in this contract)
    baseURI = 'https://gateway.pinata.cloud/ipfs/QmScwmkHdHZkozpHx1Egd3CamYfheGzVLWZVmXvZzzMRoD/';//set your baseuri link to folder here, be sure to include the / at the end
    unrevealedURI = "ipfs://QmXAm27KYg6uM5SeeXnnwM1v9LHVK16KQRCJdSCFw6B3UP"; //set your unrevealed json
    pricePublic = 2 ether; //  AVAX mint price Public
    priceWL = 2 ether; //  AVAX mint price WL
    maxSupply = 100; //  max supply for this NFT collection
    royaltyPercent = 600; // 6% royalties to contract owner, number is in basis pts
    maxMintPerTx = 5; //5 minted at a time max
    phaseNow = 1; //starts in WL phase since we have the timers
    WLSaleStartTime = 1685804000; //https://www.unixtimestamp.com/ timestamp time, must be set to a future time
    publicSaleStartTime = 1685805000; //https://www.unixtimestamp.com/ timestamp time, must be set to a future time
    witnet = IWitnetRandomness(address(0xfa9F6dda6E5451EC12Dbb4da0E1910c549646538));//can only be set here in the constructor
...
//newest functions added for shuffle reveal


  modifier timeNotStarted{
    if (phaseNow == 2){
        require (publicSaleStartTime>0, "Public Mint start time is not yet set");
        require(block.timestamp>publicSaleStartTime, "Public Minting has not started yet.");
        _;}
    else if (phaseNow == 1){
        require (WLSaleStartTime>0, "WL Mint start time is not yet set");
        require(block.timestamp>WLSaleStartTime, "WL Minting has not started yet.");
        _;}
  }

  function setWLMintTime(uint _WLstartTime) public onlyOwner {
    WLSaleStartTime = _WLstartTime;
  }



  function setWLMintTimeToNow() public onlyOwner {
      WLSaleStartTime = (block.timestamp+1);
  }

  function setPublicMintTimeToNow() public onlyOwner {
      publicSaleStartTime = (block.timestamp+1);
  }
  
  function setPublicMintTime(uint _PublicstartTime) public onlyOwner {
    publicSaleStartTime = _PublicstartTime;
  }
...
//Added pause transfer function from Openzepplin contract wizard
  ...
  function _beforeTokenTransfer(address from, address to, uint256 _tokenId)
    internal
    whenNotPaused //halts token transfers if paused
    override
  {
  ...
  function setMaxSupply(uint _maxSupply) public onlyOwner{ //nft max supply 
    maxSupply=_maxSupply;
  }


  function timeUntilWLMint() public view returns(uint){
    require (WLSaleStartTime>=block.timestamp, "WL Mint time already started");
    require (WLSaleStartTime>0, "WL Mint time not yet set");
    return(WLSaleStartTime-block.timestamp);
  }

  function timeUntilPublicMint() public view returns(uint){
    require (publicSaleStartTime>=block.timestamp, "Public Mint time already started");
    require (publicSaleStartTime>0, "Public Mint time not yet set");
...
//onlyOwner setter functions
  function setPricePublic(uint _pricePublic) public onlyOwner {
    pricePublic = _pricePublic;
  }

  function setPriceWL(uint _priceWL) public onlyOwner {
    priceWL = _priceWL;
  }
  ...
  function setroyaltyPercentInBasisPts(uint _royaltyPercent) public onlyOwner {
  ...
  function setmaxMintPerTx(uint _maxMintPerTx) public onlyOwner {
    ...
    function seedAllowlist(address[] calldata _addresses,uint256[] calldata _numNfts) external onlyOwner {//WL function
  ...
  function setPhaseNow(uint _phaseNow) public onlyOwner {
    phaseNow = _phaseNow;
  }
  ...
  function devMint(uint256 quantity, address devMintReceiver) public onlyOwner{//dev infinite free mints send any amount to anyone
    for(uint i = 0; i < quantity; i++) {
      _privateMint(devMintReceiver);
    }
  }
    ...
    /// @notice Mint NFTs 
    /// @param amount Quantity of NFTs to mint  
  function mint(uint256 amount) public payable {//campfire interface for minting through their marketplace website
    mintJelly(amount);
  }

//phased shuffled mints
  function mintJelly(uint256 quantity) public payable timeNotStarted nonReentrant{//wl and public mint in one
    require(mintActive, "Minting is not active.");
    if (totalSupply() + quantity > maxSupply) {revert ("Launchpeg__MaxSupplyReached()");}
    require(phaseNow == 1 || phaseNow == 2, "Phase is not yet set to WL or Public mint phase");//require phaseNow set to 1 (wl) or 2 (public)
    if (botsCanMint == false){
        require(msg.sender == tx.origin, "No smart contract bots allowed.");
    }
    require(quantity <= maxMintPerTx, "Cannot mint that many at once.");//max mint per transaction check
    if (phaseNow == 1) {//wl phase check
        require(msg.value >= (priceWL * quantity), "Not enough AVAX sent.");//check if mint fees paid
        if (quantity > allowlist[msg.sender]) {//if minting more than allowed by wl
            revert ("You can't mint more than your allowlist allotment");//revert
        }
        allowlist[msg.sender] -= quantity;//remove one from wl mapping
    }
    else {
        require(msg.value >= (pricePublic * quantity), "Not enough AVAX sent.");//check if mint fees paid not enough
        require(msg.value <= (pricePublic * quantity), "Too much AVAX sent.");//check if mint fees paid too much
    }
    payable(owner()).transfer(msg.value);//send avax mint fee to contract owner
    for(uint i = 0; i < quantity; i++) {//loop through amount to mint
        _privateMint(msg.sender);//go to our private mint function
    }

    if (phaseNow == 1){
        emit MintWL(msg.sender, quantity, priceWL, _tokenIds.current());
    }
    else if (phaseNow == 2) {emit MintPublic(msg.sender, quantity, pricePublic, _tokenIds.current());}
  }

  function _privateMint(address recipient) private {//all the mint functions get routed to here
    _tokenIds.increment();//count how many minted
    require(_tokenIds.current() <= maxSupply, "Project is finished minting. Max amount already minted.");//maxSupply check
    uint256 newItemId = _tokenIds.current();//name ready for new token
    _mint(recipient, newItemId);//mint new token
...
//read functions
  ...
  function _baseURI() internal view virtual override returns (string memory) {//token uri baseURI
    return baseURI;
  }

  function totalSupply() public view returns (uint256) {//current supply of nfts minted
    return _tokenIds.current();
  }

  function royaltyInfo(//required calculator for royalties
    uint256 _tokenId, //do not remove this variable, markets need it to calculate and send the royalties, will break contract to remove this
    uint256 _salePrice
  ) external view returns (address receiver, uint256 _royaltyPercent) {
    return (owner(), ((_salePrice * _royaltyPercent) / 10000));
  }

  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721) returns (bool) {//royalties interface checks reply
...
//extra  features below
  ...
  function donateAvax(uint256 quantity) public payable {
    require(msg.value == (quantity * 1000000000000000000), "Wrong amount of Avax sent");//check if mint fees paid
  }

////fallback function and generic transfer functions in case of an accidental deposit into contract wallet
  ...
  function withdrawAllAVAX(address _to)
        ...
        nonReentrant
  {
        require(0 < address(this).balance, "No Avax in the contract to withdraw");// check if there is any avax to withdraw
