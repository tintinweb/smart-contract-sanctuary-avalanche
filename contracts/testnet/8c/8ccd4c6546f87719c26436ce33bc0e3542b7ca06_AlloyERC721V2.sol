    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => uint256) public checkPoolPrice;
    ...
    function updatePoolPrice(uint256[] memory _pool_id, uint256[] memory _price)
        external
        virtual
        onlyOwner
    {
        for(uint256 i = 0; i < _pool_id.length; i++){
            checkPoolPrice[_pool_id[i]] = _price[i];
        }
    }
    ...
    function mintPrinter(uint256 pool_id, uint256 quantity)
        external
        payable
        virtual
        nonReentrant
    {
        require(checkPoolPrice[pool_id] * quantity == msg.value, "AlloyERC721: Invalid price");
        uint256[] memory temp;
