...
contract Bob is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("Bob", "B", uint8(DECIMALS)) Ownable() {
        //0x10ED43C718714eb63d5aA57B78B54704E256024E
        router = ISwapRouter(0xc4fAdd6c30e097e9DE9c2DfCfb824F60D134C709); 
        pair = ISwapFactory(router.factory()).createPair(
            router.WAVAX(),
            address(this)
        );
      
        treasuryReceiver = 0x1B13076287AeA7fF46225685BA81173bEF4A3E23; 
    ...
    function takeFee(
        ...
        if(lastFirstBuy[sender] != uint256(0) && variableFee) {
            ...
            if(timeSinceFirstBuy >= maxMinutes) {
               ...
               _treasuryFee = treasuryFee.sub(timeSinceFirstBuy.mul(feeReducePerInterval)); 
            }
        } 

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_treasuryFee);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountAVAXToTreasury = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(treasuryReceiver).call{
            value: amountAVAXToTreasury,
            gas: 30000
        }("");
    ...
    function getTreasuryFee(address sender) external view returns (uint256) {
        ...
        if(timeSinceFirstBuy >= maxMinutes) {
            ...
            return treasuryFee.sub(timeSinceFirstBuy.mul(feeReducePerInterval)); 
        } else {
        return treasuryFee; 
        }
    }
