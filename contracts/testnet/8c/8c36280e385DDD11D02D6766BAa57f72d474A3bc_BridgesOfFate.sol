...
contract BridgesOfFate is Ownable {
    ...
    bytes32[] private _totalNFTlength;
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public  NarrowingGameEndRules {
        ...
        // require(OverFlowPlayerStatus[playerId] == true && _checkSide(_member.stage, _member.lastJumpSide) == true, "No Need to pay");
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        bool isSafeSide = GameMap[_member.stage] >= 50e9;
        if (_member.stage != 0) {
            require(_member.lastJumpSide == isSafeSide, "You are Failed"); 
        }
        ...
        if (GameMap[_currentUserStage] <= 0) {
            ...
            // if(_gameStatus.stageNumber > 1){
            //     bool  previousJumpSide = GameMap[_gameStatus.stageNumber - 1 ] >= 50e9;
            //     bytes32[] memory _playerIDAtSpecifyStage = getStagesData(_gameStatus.stageNumber);
            //         for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
            //             if((PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide ) && OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == true)
            //             {
            //                 _totalNFTlength.push(_playerIDAtSpecifyStage[i]);
            //             }
            //         }
            // }else{
            //   _totalNFTlength = getStagesData(_gameStatus.stageNumber);
            // }

            // uint256 newCapacity = _totalNFTlength.length * 2;
            ...
            // emit RandomNumberComment(_narrowingGameMap);
             ...
             delete _totalNFTlength;
        }

        allStagesData[_currentUserStage].push(playerId);
        _deletePlayerIDForSpecifyStage(_member.stage,playerId);
        // emit currentIndexEvent(getStagesData(_currentUserStage));
        ...
        /**selectPreviousStageNFTAndCapacity
         *     if(totalNFTOnTile[_currentUserStage][_jumpSide] <= 0){                
           selectPreviousStageNFTAndCapacity(_currentUserStage,currentDay);
            // OverFlowPlayerStatus[playerId] =  false;
        }
        if(totalNFTOnTile[_currentUserStage][_jumpSide] > 0 ){
            totalNFTOnTile[_currentUserStage][_jumpSide]--;
        }
        // bool isSafeSidePrevious = GameMap[_currentUserStage - 1] >= 50e9;

        if(_currentUserStage >= 2  && TitleCapacityAgainestStage[_currentUserStage - 1] >= totalNFTOnTile[_currentUserStage - 1][isSafeSide ]){
            totalNFTOnTile[_currentUserStage - 1][isSafeSide ]++;
        }  
        */
        ...
        // lastUpdateTimeStamp = block.timestamp;
        lastUpdateTimeStamp = _member.startAt =_member.lastJumpTime = block.timestamp;
    ...
    event CurrentStageEvent(uint256 _cs); //Correct.
    event currentIndexEvent(bytes32 _sSidePalyer);
    event SelectSuccessSidePlayerIdEvent(bytes32[] _sSidePalyer);
    event holdCapacityBytesEvent(bytes32[] _sSidePalyer);
    event holdCapacityBytesBoolEvent(bool _sSidePalyer);
    event SelectSuccessSidePlayerIdLenghtEvent(uint256  _sSidePalyerLength);
    event RandomIndexEvent(uint256  _index);
   ...
   function _selectSuccessSidePlayerId(bytes32[] memory _playerIDAtSpecifyStage, bool previousJumpSide,uint256 currentDay) public  returns(bytes32[]  memory){
        ...
        for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
            if((PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide  || OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == true) || OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == false)
            // if((
            //     (PlayerItem[_playerIDAtSpecifyStage[i]].lastJumpSide == previousJumpSide ) 
            //     &&
    ...
    function selectPreviousStageNFTAndCapacity(uint256 _currentUserStage,uint256 currentDay) internal  {
        ...
        */
        ...
        if(_currentUserStage  > 0){
            ...
            currentJumpSide = GameMap[_currentUserStage] >= 50e9;

            _selectSuccessSidePlayerIdList =  _selectSuccessSidePlayerId(PlayerIDAtSpecifyStage,currentJumpSide,currentDay);
            ...
            if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage]){
                emit SelectSuccessSidePlayerIdLenghtEvent(_selectSuccessSidePlayerIdList.length + 1);
                ...
                for (uint256 j = 0; j < _selectSuccessSidePlayerIdList.length; j++) {
                    ...
                    emit holdCapacityBytesBoolEvent(OverFlowPlayerStatus[SelectUnholdPlayerId[j]]);
                }
                ...
                _randomSelectPlayerIndexPreviousStage = generateRandomTicketNumbers(SelectUnholdPlayerId.length - 1 ,SelectUnholdPlayerId.length - 1,TitleCapacityAgainestStage[_currentUserStage]);
            }
              ...
              if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage]){
                ...
                for (uint256 k = 0; k < _randomSelectPlayerIndexPreviousStage.length; k++) { //clear
                    if(_randomSelectPlayerIndexPreviousStage[k] > 0){
                        _setCapacityPreviouseStage =  _setCapacityPreviouseStage + 1;
                //         if(SelectUnholdPlayerId.length > 0){
                            // emit holdCapacityBytesEvent(SelectUnholdPlayerId);
                            emit currentIndexEvent(SelectUnholdPlayerId[_randomSelectPlayerIndexPreviousStage[k]]);
                                if(isExistForRandomSide(_uniqueRN,_randomSelectPlayerIndexPreviousStage[k])){
                                    ...
                                    emit RandomIndexEvent(_randomSelectPlayerIndexPreviousStage[k]); // clear
                                }
                            emit holdCapacityBytesBoolEvent(OverFlowPlayerStatus[SelectUnholdPlayerId[_randomSelectPlayerIndexPreviousStage[k]]]);
                //         } 
                    }
                }
                ...
                if(TitleCapacityAgainestStage[_currentUserStage]  > 0 && _setCapacityPreviouseStage > 0){
                    ...
                    totalNFTOnTile[_currentUserStage][currentJumpSide] = TitleCapacityAgainestStage[_currentUserStage] - _setCapacityPreviouseStage;
                }else{
                    emit remainCapacityElseEvent(totalNFTOnTile[_currentUserStage][currentJumpSide]);
                    totalNFTOnTile[_currentUserStage][currentJumpSide] = totalNFTOnTile[_currentUserStage][currentJumpSide];
                }
            }            
        }
