...
contract Stake is ERC20{
    ...
    string public name = "stake Test Token";
...
//  uint256 public TotalStakedTokens;
    ...
    function approveStake(uint256 _StakeAmount)public returns(uint256 _approved) {
        token.approve(sPool, _StakeAmount);
        return _approved;
    }
    ...
    function approveSwapSToken(uint256 _amountSTokens)public returns(uint256 _approved) {
        token.approve(sPool, _amountSTokens);
        return _approved;
    }
    ...
    function stakeTKN10(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        ...
        lockTime10[msg.sender] = block.timestamp.add(600); //(600 = 10 min),One month, amount in seconds
    ...
    function reStakeTKN10(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        require(stakingOn, "Staking not active right now!");
        require(lockTime10[msg.sender] > 0 && lockTime10[msg.sender] < block.timestamp, "You can't reStake now!");
        uint256 rewards;         
        lockTime10[msg.sender] = block.timestamp.add(600); //(600 = 10 min),One month, amount in seconds
        rewards = stakeRewards10[msg.sender].add(stakeRewards10[msg.sender].div(20)); //5% rewards add to staked amount
        TotalRewordsHolded = TotalRewordsHolded.sub(stakeRewards10[msg.sender]);
        _sTokenAmount = _stakeAmount;
        return _sTokenAmount;
    }
...
//30 days staking
    function stakeTKN30(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        ...
        lockTime30[msg.sender] = block.timestamp.add(1800); //(1800 = 30 min),One month, amount in seconds
    ...
    function reStakeTKN30(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        require(stakingOn, "Staking not active right now!");
        require(lockTime30[msg.sender] > 0 && lockTime30[msg.sender] < block.timestamp, "You can't reStake now!");
        uint256 rewards;         
        lockTime30[msg.sender] = block.timestamp.add(1800); //(1800 = 30 min),One month, amount in seconds
        rewards = stakeRewards30[msg.sender].add(stakeRewards30[msg.sender].div(5)); //20% rewards add to staked amount
        TotalRewordsHolded = TotalRewordsHolded.sub(stakeRewards30[msg.sender]);
        _sTokenAmount = _stakeAmount;
        return _sTokenAmount;
    }
...
//60 days staking
    function stakeTKN60(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        ...
        lockTime60[msg.sender] = block.timestamp.add(3600); //(3600 = 60 min),One month, amount in seconds
    ...
    function reStakeTKN60(uint256 _stakeAmount) public returns(uint256 _sTokenAmount) {
        require(stakingOn, "Staking not active right now!");
        require(lockTime60[msg.sender] > 0 && lockTime60[msg.sender] < block.timestamp, "You can't reStake now!");
        uint256 rewards;         
        lockTime60[msg.sender] = block.timestamp.add(3600); //(3600 = 60 min),One month, amount in seconds
        rewards = stakeRewards60[msg.sender].add(stakeRewards60[msg.sender].div(2)); //50% rewards add to staked amount
        TotalRewordsHolded = TotalRewordsHolded.sub(stakeRewards60[msg.sender]);
        _sTokenAmount = _stakeAmount;
        return _sTokenAmount;
    }
    ...
    function swapSTokenToToken(uint256 _swapAmount)public returns(uint256 _swapped) {
        //require(_stakeAmount <= token.allowance(stakerAddress, sPool), "Please approve Staking amount!");
        ...
        approveSwapSToken(_swapAmount);
        token.transferFrom(sPool, msg.sender, _swapAmount);
        TotalRewordsHolded = TotalRewordsHolded.sub(_swapAmount);
        _swapped = _swapAmount;
        return _swapped;
    }
    //approveUnstake(_UnStakeAmount);
    //token.transferFrom(sPool, msg.sender, _UnStakeAmount);
}