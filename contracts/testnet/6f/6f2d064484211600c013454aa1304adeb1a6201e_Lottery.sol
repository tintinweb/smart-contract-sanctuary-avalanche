...
contract GameUpgradeable is IGame, PrizePoolUpgradeable {
     ...
     */
    function createNewRound(uint ticketPrice) public payable whenNotPaused onlyRole(LPlatform.adminRole) {
        IPlatform platform = IPlatform(getPlatformAddress());
        if (_rounds.length() > 0) {
            ...
            if (round.getStatus() == LUtil.RoundStatus.OPEN) {
                ...
                LUtil.PrizeWallet[] memory balances = walletBalances();
                round.startProcessing{value: msg.value}(balances, ticketPrice);
    ...
    function createPreLastRoundNumbers() external payable whenNotPaused onlyRole(LPlatform.adminRole) {
