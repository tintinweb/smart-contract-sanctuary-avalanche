...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    event shrimpRemoved(address owner, uint256 tokenId, address tokenAddress);

    modifier onlyShrimpOwner(address _tokenAddress, uint256 _tokenId) {
    ...
    modifier isForSale(uint256 _offerId) {
        require(offers[_offerId].isSold == false, "Shrimp already sold.");
        _;
    }
    ...
    function setOffer(
        ...
        public
        onlyShrimpOwner(tokenAddress, tokenId)
    ...
    function removeOffer(uint256 offerId) public {
        ...
        emit shrimpRemoved(msg.sender, tokenId, tokenAddress);
    }
    ...
    function buyShrimp(uint256 offerId)
        public
        payable
        nonReentrant
        itemExists(offerId)
        isForSale(offerId)
    {
        require(
            ...
            "Payment must be equal to price of the shrimp"
        );
        require(
            offers[offerId].seller != msg.sender,
            "Cannot buy your own shrimp"
        );
        require(
            activeOffers[offers[offerId].tokenAddress][
                offers[offerId].tokenId
        ...
        emit shrimpSold(tokenAddress, msg.sender, price, tokenId, offerId);
    }
