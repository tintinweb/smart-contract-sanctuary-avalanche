    ...
    Authorizable
{
    ...
    function isWhitelisted(
        ...
        if (MerkleProof.verify(proof, merkleRootT3, leaf) && !claimed[user]) {
            return 3;
        } else if (
            MerkleProof.verify(proof, merkleRootT2, leaf) && !claimed[user]
        ) {
            return 2;
        } else if (
            MerkleProof.verify(proof, merkleRootT1, leaf) && !claimed[user]
        ) {
            return 1;
        } else return 0;
    }
    ...
    function mintByOwner(address[] calldata to, uint256[] calldata counts) public onlyOwner {
        ...
        for (uint256 j = 0; j < to.length; j++) {
            ...
            for (uint256 i = totalSupply(); i < counts[j] + totalSupply(); i++) {
                royaltyReceiver[i] = msg.sender;
            }
