...
RevokableDefaultOperatorFilterer
{
    ...
    bool public isMintable; //v
    bool public isBurnable; //v

    bool public isSale; //v

    uint256 public maxSupply; //v
    uint256 public reservedSupply; //v

    uint256 public salePrice; //v

    address public proxyRegistryAddress; //v
    address public signer; //v

    address public poolValue; //v

    string public baseExtension = ""; //v
    string private __baseURI; //v

    event Minted(uint256 amount, uint256 value, address indexed receiver, uint8 indexed mintType, address indexed sender, uint256 timestamp);
    ...
    //7
    function setPoolValue(address _poolValue) external onlyOwner {
        poolValue = _poolValue;
    }
    ...
    //9
    function setIsSale(bool _isSale) external onlyAdmin{
        isSale = _isSale;
    }

    //10
    function setReservedSupply(uint256 _reservedSupply) external onlyAdmin{
        reservedSupply = _reservedSupply;
    }
    ...
    function mintToken(uint256 _amount) external payable{
        require(isMintable == true, "is not mintable");
        require(isSale == true, "is not for sale");
        require(msg.value == (_amount.mul(salePrice)), "wrong ether value");
        require(totalSupply() + reservedSupply + _amount <= maxSupply, "tokens supply reached limit");
        
        _mintNFT(msg.sender, _amount);

        emit Minted(_amount, msg.value, msg.sender, 1, msg.sender, block.timestamp);
    }

    function mintReserved(address[] memory _to, uint256[] memory _amount) external onlyOwner{
        require(_to.length == _amount.length, "must same length");

        uint reserved = reservedSupply;
        for(uint i=0; i<_to.length; i++){
            require(_amount[i] <= reserved ,"reserved supply reached limit");
            _mintNFT(_to[i],_amount[i]);
            reserved-= _amount[i];
            emit Minted(_amount[i], 0, _to[i], 2, msg.sender, block.timestamp);
        }
    ...
    function _beforeTokenTransfers(
        ...
        if(isBurnable == false ){ //if burn is false cannot to zero address
            require(to != address(0), "burnable by owner is not active");
        }
        else if(isBurnable == true && to == address(0)){
            require(isAdmin(from) == true, "only admin can burn token");
        }
