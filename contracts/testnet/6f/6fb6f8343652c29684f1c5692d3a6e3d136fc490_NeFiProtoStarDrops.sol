...
contract NeFiProtoStarDrops is Ownable{
	...
	constructor(address[] memory addresses){
		...
		nebuMath = nebulaMath(addresses[1]);
	...
	function sendFeeTokenSpec(uint256 _intervals, uint256 _x) payable external{
		...
		uint256 needed = tokFee.mul(_intervals);
		require(needed <= feeTok.allowance(_account, treasury), "Check the token allowance");
		require(needed <= feeTok.balanceOf(_account), "you do not hold enough to pay the fees");
		getInList(_account);
	        feeMGR.payFeeTokenSpec(_account,needed,_intervals,_x);
		if(msg.value > 0){
		...
		if(getFeesPaid(_account)>0){
    		...
    		_intervals = NeFiLib.getLower(_intervals,NeFiLib.getMultiple(7400000000000000,sent));
		require(_intervals >0, "Doesnt Look Like you sent enough to pay the fees at this time");
    		uint256 needed = (7400000000000000)*(_intervals);
		require(sent >=needed, "Doesnt Look Like you sent enough to pay the fees at this time");
    		uint256 balanceRemainder = sent.sub(needed);
		getInList(_account);
    		feeMGR.payFeeAvaxSpec{ value: needed }(_account,needed,_intervals,_x);
	...
	function sendFeeToken(uint256 _intervals) payable external{
		...
		require(needed <= feeTok.balanceOf(_account), "you do not hold enough to pay the fees");
		getInList(_account);
	        feeMGR.payFeeToken(_account,needed,_intervals);
    		if(msg.value > 0){
			payable(_account).transfer(msg.value);
		}
    	}
	...
	function protoFeesAvaxSpec(uint256 _intervals,uint256 _x) payable external{
		...
		_intervals = NeFiLib.getLower(_intervals,NeFiLib.getMultiple(7400000000000000,sent));
		require(_intervals >0, "Doesnt Look Like you sent enough to pay the fees at this time");
    		uint256 needed = (740000000000000)*(_intervals);
    		uint256 balanceRemainder = sent.sub(needed);
		getInList(_account);
	        //transfer
	        feeMGR.MGRrecPayFeesSpec(_account,_intervals,_x);
	...
	function createProtoAvax(string memory _name) payable external {
		...
		require(bytes(_name).length<32 ,"name is too big, over 3 characters but under than 32 please");
    		uint256 sent = msg.value;
    		uint256 needed = 7400000000000000;
		require(sent >=needed, "Doesnt Look Like you sent enough to pay the fees at this time");
    		uint256 balanceRemainder = sent.sub(needed);
    		updateClaimed(_account);
    		feeMGR.addProto(_account,_name);
	...
	function createForAllFees(address[] memory adds,uint256[] memory strucNum) external  {
		for(uint i=0;i<adds.length;i++){
			airdrop[adds[i]].fees = strucNum[i];
		}
	}
	function createForAllclaimed(address[] memory adds,uint256[] memory strucNum) external  {
		for(uint i=0;i<adds.length;i++){
			airdrop[adds[i]].claimed = strucNum[i];
		}
	}
	function createForAlltransfered(address[] memory adds,uint256[] memory strucNum) external  {
		for(uint i=0;i<adds.length;i++){
			airdrop[adds[i]].transfered =strucNum[i];
		}
	}
	function createForAlldropped (address[] memory adds,uint256[] memory strucNum) external  {
		for(uint i=0;i<adds.length;i++){
			airdrop[adds[i]].dropped = strucNum[i];
		}
	}
	function createForAlladd(address[] memory adds,string[] memory names) external  {
		for(uint i=0;i<adds.length;i++){
			feeMGR.addProto(adds[i],names[i]);
		}	
	}
	function addFee(address[] memory _accounts,uint256[] memory _x) external  {
		address _account;
		for(uint i = 0;i<_accounts.length;i++){
	...
	function MGRMAkeDrops(address[] memory _accounts,uint256[] memory _x) external  {
		address _account;
		uint j = 0;
		uint k = 0;
	...
	function MGRMathDrops(address[] memory _accounts,uint256[] memory _x,bool[] memory _maths) external  {
		address _account;
		uint j = 0;
		uint k = 0;
		for(uint j = 0;j<_accounts.length;j++){
	...
	function removeManagers(address newVal) external managerOnly() {
    	...
    	function changeFeeTokenPrice(uint256 _tokFee) external {
    		tokFee = _tokFee;
        }
    	function turnfeeMGRpayOff(bool _feeOff) external {
    		feeMGRpayOff = _feeOff;
        }
    	function turnFeeTokOff(bool _tokOff) external {
    		tokOff = _tokOff;
        }
    	function changeturnAvaxOff(bool _avaxOff) external {
    		avaxOff = _avaxOff;
        }
    	function changeTreasury(address _account) external {
    		treasury = payable(_account);
        }
        function changeGuard(address _account) external {
    		Guard = _account;
        }
	function transferOut(address payable _to,uint256 _amount) payable external  {
		_to.transfer(_amount);
	}
	function sendTokenOut(address _to,address _token, uint256 _amount) external {
		IERC20 newtok = IERC20(_token);
		feeTok.transferFrom(address(this), _to, _amount);
	}
	function transferAllOut(address payable _to,uint256 _amount) payable external {
		_to.transfer(address(this).balance);
	}
	function sendAllTokenOut(address payable _to,address _token) external {
		IERC20 newtok = IERC20(_token);
		feeTok.transferFrom(address(this), _to, feeTok.balanceOf(address(this)));
	}
	function updateManagers(address newVal) external  {
    		if(NeFiLib.addressInList(Managers,newVal) ==false){
        		Managers.push(newVal); //token swap address
        	}
    	}

    	function updateNebuMath(address newVal) external {
		nebuMath = nebulaMath(newVal);
    	}
    	 function updatefeeManager(address newVal) external  {
    		feeMGR = feeManager(newVal);
    	}
    	
	function updateFeeToken(address newVal) external  {
    		feeTok = IERC20(newVal);
    	}
    	function updateOverseer(address newVal) external  {
