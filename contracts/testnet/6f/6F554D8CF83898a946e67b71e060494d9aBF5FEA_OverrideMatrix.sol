...
contract OverrideMatrix is AccessControl {
    ...
    function _newLocation(address _account, uint256 _newGrade) internal {
        ...
        accountInfoList[_account].MEOReward = accountInfoList[_account].MEOReward.add(price.mul(1e18).div(1e6).mul(baseNewGradeRewardRate).div(rate));
    ...
    function _should6AutoNewGrant(address _account, uint256 _grade) internal {
        ...
        if (accountInfoList[_account].currentMaxGrade >= _grade.add(1)) {
            ...
            if (
                ...
                if (!accountInfoList[_account].isPauseAutoNewGrant[_grade]) {
                    accountInfoList[_account].MEOReward = accountInfoList[_account].MEOReward.add(currentPrice(_grade.add(1)).mul(1e18).div(1e6).mul(baseNewAutoRewardRate).div(rate));
                    perAutoTimes++;
    ...
    function _should6AutoReVote(address _account, uint256 _grade) internal {
        ...
        if (
            ...
            if (!accountInfoList[_account].isPauseAutoReVote[_grade]) {
                accountInfoList[_account].METReward = accountInfoList[_account].METReward.add(currentPrice(_grade).mul(1e18).div(1e6).mul(baseNewAutoRewardRate).div(rate));
                perAutoTimes++;
