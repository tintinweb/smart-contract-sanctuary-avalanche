...
/** @dev Interface of Yield Key Legendary V3 */
    ...
    AccessControlUpgradeable
{
    ...
    /**
     *@dev Mapping of YieldBox and YieldKey Legendary
     * Yield Key ---> Yield Box
     */
    mapping(uint256 => uint256[]) internal _yieldBoxYieldKeyLeg;
    /**
     *@dev Mapping of YieldBox and YieldKey Regular
     * Yield Key ---> Yield Box
     */
    mapping(uint256 => uint256[]) internal _yieldBoxYieldKeyReg;
    /**@dev Mapping of ERC20 token for create Yield Key */
    mapping(address => uint256) public amountCoin;
    // Array of Maintenance Fee Price per Type of Yield Key
    mapping(uint8 => uint256) public pricePerType; // [0] Legendary, [1] Epic, [2] Rare, [3] Common
    // Add Interface for Call Rewards Smart Contract Methods
    ...
    /**
     * @dev Event when change the Limits per Day payable per YieldBox
     * @param oldLimitsDaysPerYieldBox old value of Limits per Day payable per YieldBox
     * @param newLimitsDaysPerYieldBox new value of Limits per Day payable per YieldBox
     */
    event SetLimitsDays(
    ...
    /**
     * @dev Event when Reset Maintenance Fee of Yield Box when unStake Yield Key
     * @param tokenId Token Id of Yield Box
     * @param yieldKey Token Id of Yield Key
     * @param yieldKeyType Type of Yield Key
     */
    ...
    function payMaintenanceFee(
        ...
        require(
            getAvailableSlots(_yieldKey, _type) >= _tokenIds.length,
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                ValidFreeRewardsClaimed(_tokenIds[i]),
                "At least one Yield Box have Free Rewards pending to Claim"
            );
            require(
            ...
            require(
                ...
                "At least one of the Yield Boxes is not Owner"
            );
            require(
                maintFee[_tokenIds[i]].unClaimedDays < LIMITS_DAYS,
        ...
        uint256 totalPrice = getPricePerType(_stable, _type).mul(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            maintFee[_tokenIds[i]].firstDay = maintFee[_tokenIds[i]].firstDay ==
                uint64(0)
                ? uint64(block.timestamp)
                : maintFee[_tokenIds[i]].firstDay;
            maintFee[_tokenIds[i]].unClaimedDays += _paidDays[i];
            if (_type == 12) {
                if (
                    !isYieldBoxInArray(
    ...
    /**
     * @dev Getting Price per Type
     * @param _stable Token Address of Stable Coin and Token ValiFI to create Yield Box
     * @param _type Type of Yield Box
     * @return index in array of price per type
     */
    ...
    /**
     * @dev Method for Getting the Available Slot per Yield Key
     * @param _yieldKey Yield Key
     * @return Available Slot per Yield Key
     */
    ...
    /**
     * @dev Method to Get Payable Days
     * @param _tokenId Token Ids of Yield Box
     */
    ...
    /**
     * @dev This Method permit set the LIMITS_DAYS payable for Yield Box
    ...
    function setValue(uint256[2] memory _value) external onlyOwner {
        ...
        LIMITS_DAYS = _value[0] * 1 minutes;
        MINIMAL_REWARDS_DAYS = _value[1] * 1 minutes;
        emit SetLimitsDays(oldValue, _value[0]);
    }

    /**
     * @dev This Method permit to set Reset Maintanance Fee.
     * @param _yieldKey Token Id of YieldKey
     */
        ...
        onlyRole(ROLE_RESET_MAINTENANCE_FEE)
    {
        uint256 amount;
        if (_yieldKeyType == 12) {
            amount = _yieldBoxYieldKeyLeg[_yieldKey].length;
            uint256[] memory tokenIds = _yieldBoxYieldKeyLeg[_yieldKey];
            for (uint256 i = 0; i < tokenIds.length; i++) {
                maintFee[tokenIds[i]].status = false;
                maintFee[tokenIds[i]].lastOwner = address(0);
                maintFee[tokenIds[i]].firstDay = uint64(0);
                maintFee[tokenIds[i]].unClaimedDays = 0;
                _yieldBoxYieldKeyLeg[_yieldKey][i] = _yieldBoxYieldKeyLeg[
                    _yieldKey
                ][amount - 1];
                _yieldBoxYieldKeyLeg[_yieldKey].pop();
                maintFee[tokenIds[i]].yieldKey = 0;
                maintFee[tokenIds[i]].yieldKeyType = 0;
                emit ResetMaintenanceFee(tokenIds[i], _yieldKey, _yieldKeyType);
            }
        } else if (
            _yieldKeyType == 8 || _yieldKeyType == 6 || _yieldKeyType == 4
        ) {
            amount = _yieldBoxYieldKeyReg[_yieldKey].length;
            ...
            for (uint256 i = 0; i < tokenIds.length; i++) {
                ...
                maintFee[tokenIds[i]].firstDay = uint64(0);
                maintFee[tokenIds[i]].unClaimedDays = 0;
                _yieldBoxYieldKeyReg[_yieldKey][i] = _yieldBoxYieldKeyReg[
                    _yieldKey
                ][amount - 1];
                _yieldBoxYieldKeyReg[_yieldKey].pop();
                maintFee[tokenIds[i]].yieldKey = 0;
                maintFee[tokenIds[i]].yieldKeyType = 0;
                emit ResetMaintenanceFee(tokenIds[i], _yieldKey, _yieldKeyType);
            }
        }
        emit ResetMaintenanceFee(amount, _yieldKey, _yieldKeyType);
    }
    ...
    /**
     * @dev Method of Yield Box without Yield Key associate per Wallet
     * @param _walletAddress Owner of Yield Box
     */
        ...
        returns (uint256[] memory yieldBoxWithoutYieldKey)
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (
                ...
                (block.timestamp - YieldBox.yieldBox(_tokenIds[i]).createdAt) <=
    ...
    /**
     * @dev Method of Yield Box to Validate is Yield Box exist in the Array of Yield Keys Legendary or Regular
    ...
    /**
     * @dev Set Rewards  Smart Contract
    ...
    /**
     * @dev Verify is Yield Box Claim all Free Rewards Available
     * @param _tokenId Token Id of Yield Box
     */
    function ValidFreeRewardsClaimed(uint256 _tokenId)
        public
        view
        returns (bool)
    {
        return
            (block.timestamp - YieldBox.yieldBox(_tokenId).createdAt) /
                1 minutes <=
            Rewards.RewardsClaimed(_tokenId).freeDaysClaimed;
    }

    /**
     * @dev Method to Setting the Maintenance Fee Price per Type of YieldKey
     * @param _value Array of Value in Decimal per Type of Yield Key Ordered (12, 8, 6, 4)
     */
    function setPriceMaintFee(uint8[4] memory _value) external onlyOwner {
        require(
            _value[0] > 0 && // Yield Key Legendary
                _value[1] > 0 && // Yield Key Regular Epic
                _value[2] > 0 && // Yield Key Regular Rare
                _value[3] > 0, // Yield Key Regular Common
            "ERC721 VALIFi: Can't set value, value must be greater than 0"
        );
