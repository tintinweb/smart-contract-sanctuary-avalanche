...
contract BridgesOfFate is Ownable {
    ...
    uint256 private constant TURN_PERIOD = 180;
    ...
    // Replace Above variavle Place    
    uint256 private _packedNumber = 0;

    function generateRandomArray(uint256 size, uint256 min, uint256 max) public  returns (uint256[] memory) {
        uint256[] memory randomArray = new uint256[](size);
        uint256 randomNumber;
        for (uint256 i = 0; i < size; i++) {
            // Comment Due to Gas issues
            // bytes32 blockHash = blockhash(block.number - i - 1);         
            randomNumber = uint256(_packedNumber) % (max - min + 1) + min;
            randomArray[i] = randomNumber;
            _packedNumber = _packedNumber - 1;
        }
    ...
    function participateInGame(bool _jumpSide,bytes32 playerId) public GameEndRules NarrowingGameEndRules {
        ...
        if (gameRandomNumber[_currentUserStage] <= 0) {
            ...
            // uint256 _packedNumberNarrowinggameRandomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, address(this), msg.sender))) % 100;
    ...
    function _selectPlayerAndSetCapacityOnTile(uint256 _currentPlayerStage,uint256 currentDay,bool _jumpSide) internal {
        ...
        if (_currentPlayerStage > 0) {
            ...
            if (_selectJumpedSidePlayersId.length > _titleCapacityAgainestStage) {
                ...
                _randomSelectPlayerIndex = generateRandomArray(_titleCapacityAgainestStage, 1,_selectUnholdPlayerId.length - 1);
    ...
    function _generateTitleCapacity(uint256 _stageNo) internal returns (uint256) {
        if (_stageNo > 1) {
            ...
            bool previousJumpSide = gameRandomNumber[_stageNo] >= 50; // Side Stage-#-1
