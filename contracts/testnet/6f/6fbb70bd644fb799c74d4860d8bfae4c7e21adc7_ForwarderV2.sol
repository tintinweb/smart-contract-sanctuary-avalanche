...
contract ForwarderV2 is Initializable , AccessControlUpgradeable {
    ...
    function initialize(address _admin, address _flusher) initializer public {
    ...
    // Execute a token transfer for specific erc20 token from this contract to specified destination address
    function flushTokens(address tokenContractAddress, address destination) external onlyRole(FLUSH_ROLE) {
        ...
        instance.safeTransfer(destination, forwarderBalance);
    ...
    function flushETH(address destination) external onlyRole(FLUSH_ROLE){
        (bool success, ) = destination.call{value: address(this).balance}("");
        require(success, "Couldn't flush ETH!");
        emit FlushedETH(destination, address(this).balance);
    }
}