...
interface Gateway {
  function getAllPoolsInUSD() external view returns(uint[] memory _allPoolInUSDs, bytes memory sig);
}
...
contract BNIMinterTest is ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function getAllPoolInUSD1() public view returns (uint) {
        revert OffchainLookup(
            address(this),
            urls,
            abi.encodeWithSelector(Gateway.getAllPoolsInUSD.selector),
    ...
    function getPricePerFullShare(uint[] memory _allPoolInUSDs) external view returns (uint) {
    ...
    /// @notice The length of array is based on network count. And the lengths should be same on the arraies.
    function getAPR(uint[] memory _allPoolInUSDs, uint[] memory _APRs) external pure returns (uint) {
        uint networkCnt = _allPoolInUSDs.length;
