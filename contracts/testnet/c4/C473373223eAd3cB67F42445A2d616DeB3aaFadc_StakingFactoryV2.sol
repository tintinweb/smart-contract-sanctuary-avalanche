...
contract StakingFactoryV2 is RewardsToken {
    event NewInstance(bytes32 storeId, address instance);
    ...
    function _decode(bytes calldata data) internal pure returns (address user, bytes32 storeId) {
        (user, storeId) = abi.decode(data, (address, bytes32));
    }
    ...
    function cloneDeterministic(
        ...
        (address user, bytes32 storeId) = _decode(initData);
        require(user == msg.sender, "Proxy not allow");
        require(MerkleProof.verify(proof, root, storeId), "You are not eligible");
    ...
    function _initAndEmit(bytes32 storeId, address instance, address rewardsToken, address stakingToken, uint256 current, uint256 storeNFTType) private {
        IStakingRewards(instance).initialize(rewardsToken, stakingToken);
        _clones[current] = instance; 
        _metadata[storeId] = Metadata(instance, storeNFTType);
        length.increment();
