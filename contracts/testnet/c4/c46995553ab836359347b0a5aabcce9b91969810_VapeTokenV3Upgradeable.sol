...
import "./lossless/LERC20Upgradeable.sol";

error VapeTokenV3Upgradeable__NotOwner();
error VapeTokenV3Upgradeable__NotMinter();
error VapeTokenV3Upgradeable__SupplyLimit();
error VapeTokenV3Upgradeable__InvalidInput();
error VapeTokenV3Upgradeable__AddressZero();

contract VapeTokenV3Upgradeable is LERC20Upgradeable {
    ...
    modifier onlyOwner() {
        if (msg.sender != owner) revert VapeTokenV3Upgradeable__NotOwner();
        _;
    }
     ...
     * @param _recoveryAdmin Address of recovery admin
     * @param _timelockPeriod Timelock period
     * @param _lossless Address of Lossless protocol
    ...
    function initialize(
        ...
        __LERC20_init(_name, _symbol, _admin, _recoveryAdmin, _timelockPeriod, _lossless);
    ...
    /*
     * @notice Burns `_amount` tokens from the caller
     */
    function burn(uint256 amount) external lssBurn(msg.sender, amount) {
        _burn(msg.sender, amount);
    }

    /*
     * @notice Mint tokens to user
     * @param user Address of user to mint tokens to
     * @param amount Amount of tokens to mint
     * @dev Uses Lossless protocol protection
     */
    function mint(address _user, uint256 _amount) external lssMint(_user, _amount) {
        if (msg.sender != minter) revert VapeTokenV3Upgradeable__NotMinter();
        if (totalSupply() + _amount > MAX_SUPPLY) revert VapeTokenV3Upgradeable__SupplyLimit();

        _mint(_user, _amount);
    }

    /*
     * @notice Mint tokens to multiple users
     * @param users Array of addresses of users to mint tokens to
     * @param amount Array of amounts of tokens to mint
     * @dev Does NOT use Lossless protocol protection
     */
    function batchMint(address[] calldata _users, uint256[] calldata _amount) external onlyOwner {
        if (_users.length != _amount.length) revert VapeTokenV3Upgradeable__InvalidInput();
        for (uint256 i; i < _users.length; ) {
            _mint(_users[i], _amount[i]);

            unchecked {
                ++i;
            }
        }
        if (totalSupply() > MAX_SUPPLY) revert VapeTokenV3Upgradeable__SupplyLimit();
    }

    /*
     * @notice Transfer ownership of the contract to `_newOwner` address
     */

    function transferOwnership(address _newOwner) external onlyOwner {
        if (_newOwner == address(0)) revert VapeTokenV3Upgradeable__AddressZero();
        owner = _newOwner;
    }

    /*
     * @notice Set new minter to `_newMinter` address
     */
    function setMinter(address _newMinter) external onlyOwner {
        if (_newMinter == address(0)) revert VapeTokenV3Upgradeable__AddressZero();
        minter = _newMinter;
    }

    function setLossLess(bool _value) external onlyOwner {
        isLosslessOn = _value;
    }
}
...
error LERC20Upgradeable__NotAdmin();
error LERC20Upgradeable__NotRecoveryAdmin();
error LERC20Upgradeable__NotCandidate();
error LERC20Upgradeable__OnlyLossless();
error LERC20Upgradeable__SameAddress();
error LERC20Upgradeable__DecreaseAllowanceBelowZero();
error LERC20Upgradeable__InvalidTimestamp();
error LERC20Upgradeable__TimelockInProgress();
error LERC20Upgradeable__AlreadyOn();
error LERC20Upgradeable__AlreadyOff();
error LERC20Upgradeable__InvalidKey();

contract LERC20Upgradeable is ILERC20, ERC20VotesUpgradeable {
    /*
     * STATE VARIABLES
     */
    ...
    function __LERC20_init(
        ...
        __LERC20_init_unchained(_admin, _recoveryAdmin, _timelockPeriod, _lossless);
        __ERC20Votes_init();
    ...
    function __LERC20_init_unchained(
        ...
        lossless = ILssController(_lossless);
        isLosslessOn = true;
    }
    ...
    modifier lssDecreaseAllowance(address spender, uint256 subtractedValue) {
        if (isLosslessOn) {
            lossless.beforeDecreaseAllowance(msg.sender, spender, subtractedValue);
        }
        _;
    }
    ...
    modifier onlyRecoveryAdmin() {
        if (msg.sender != recoveryAdmin) {
            revert LERC20Upgradeable__NotRecoveryAdmin();
        }
        _;
    }
    ...
    function transferOutBlacklistedFunds(address[] calldata from) external override {
        if (msg.sender != address(lossless)) revert LERC20Upgradeable__OnlyLossless();
    ...
    function setLosslessAdmin(address newAdmin) external override onlyRecoveryAdmin {
        if (newAdmin == admin) revert LERC20Upgradeable__SameAddress();

        emit NewAdmin(newAdmin);
        admin = newAdmin;
    }

    function transferRecoveryAdminOwnership(address candidate, bytes32 keyHash) external override onlyRecoveryAdmin {
    ...
    function acceptRecoveryAdminOwnership(bytes memory key) external override {
        if (msg.sender != recoveryAdminCandidate) revert LERC20Upgradeable__NotCandidate();
        if (keccak256(key) != recoveryAdminKeyHash) revert LERC20Upgradeable__InvalidKey();
    ...
    function proposeLosslessTurnOff() external override onlyRecoveryAdmin {
        if (losslessTurnOffTimestamp != 0) revert LERC20Upgradeable__InvalidTimestamp();
        if (isLosslessOn == false) revert LERC20Upgradeable__AlreadyOff();
    ...
    function executeLosslessTurnOff() external override onlyRecoveryAdmin {
        if (losslessTurnOffTimestamp == 0) revert LERC20Upgradeable__InvalidTimestamp();
        if (losslessTurnOffTimestamp > block.timestamp) revert LERC20Upgradeable__TimelockInProgress();
        isLosslessOn = false;
    ...
    function executeLosslessTurnOn() external override onlyRecoveryAdmin {
        if (isLosslessOn) revert LERC20Upgradeable__AlreadyOn();
        losslessTurnOffTimestamp = 0;
        isLosslessOn = true;
        emit LosslessOn();
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) public virtual override lssTransferFrom(from, to, amount) returns (bool) {
        ...
        returns (bool)
    {
        ...
        if (currentAllowance < subtractedValue) {
            revert LERC20Upgradeable__DecreaseAllowanceBelowZero();
        }
        unchecked {
