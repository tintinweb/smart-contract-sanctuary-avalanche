...
contract DegenToken is ERC20, Ownable {
    ...
    function mintDGN(address to, uint amount) public onlyOwner {
        ...
        increaseAllowance(to, actual(amount));
    }

    // Transferring tokens
    function transferDGN(address to, uint amount) public {
    ...
    function redeemDGN(uint item) public returns(string memory s) {
        if (item == 1) {
            require(balanceOf(msg.sender) >= 50, "Not enough balance to redeem random costume.");
            transfer(owner(), actual(50));
            return "Redeemed random costume!";
        } else if (item == 2) {
            require(balanceOf(msg.sender) >= 200, "Not enough balance to redeem special weapon.");
            transfer(owner(), actual(200));
            return "Redeemed special weapon!";
        }
    }

    // Checking token balance
    function getDGNBalance() public view returns(uint) {
        return balanceOf(msg.sender) / 10**decimals();
    }

    // Burning tokens
    function burnDGN(uint amount) public {
