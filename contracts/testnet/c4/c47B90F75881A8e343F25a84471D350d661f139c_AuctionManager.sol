...
// functions
    ...
    function createAuction(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external
         {
            ...
            require(_endTime > 59 minutes); // end time must at least one hour
            if (Collections[_nftAddress].id == 0){
                    collectionCounter += 1;
                    Collections[_nftAddress] = NFTcollection(collectionCounter, new address[](10000));
                }
            ...
            Collections[_nftAddress].auctions[_tokenId] = address(auction); // add the auction to the collection
            emit Created(address(auction), auctionId);
        }
    ...
    function getOneNFT(address _collection, uint _id) external view returns (address)
        {
            require(Collections[_collection].auctions[_id] != 0x0000000000000000000000000000000000000000, "No auction found");
