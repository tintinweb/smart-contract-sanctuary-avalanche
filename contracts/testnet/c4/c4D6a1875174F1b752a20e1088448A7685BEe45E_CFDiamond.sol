...
contract CFDiamond is UniqueStorage, IDiamondCut {

    constructor(string memory _pName, address _coinAddr, address _stAddr, uint16 _pOFeePt, uint256 _bufferPeriod, uint256 _conversionPeriod, uint256 _minTh, address _treasuryAddr, address _analyticMathAddr) UniqueStorage(keccak256(abi.encodePacked(_pName, msg.sender, block.timestamp))) {
        LibCFDiamond.setContractOwner(_storagePos, msg.sender);
        LibCFDiamond.setCrowdfunding(_storagePos, _pName, _coinAddr, _stAddr, _pOFeePt, _bufferPeriod, _conversionPeriod, _minTh, _treasuryAddr);
        LibCFDiamond.setAnalyticMath(_storagePos, _analyticMathAddr);
    }
    ...
    function diamondCut(
        ...
        LibCFDiamond.enforceIsContractOwner(_storagePos);
        LibCFDiamond.diamondCut(_storagePos, _diamondCut, _init, _calldata);
    }
...
library LibCFDiamond {
    ...
    struct DiamondStorage {
        ...
        // TODO Percent Rate => PC Release Amount
        // Weekly Release Rates in increasing order (week_index => PC Release Amount)
        mapping(uint256 => uint256) weeklyReleaseAmt;
        // Total Week Claimable PT Amount in Conversion Period
        uint256 releasablePCAmt;
        // Week Index in Conversion Period
        uint256 curWeek;
        // Unclaimed PC Amount
        uint256 claimedPCAmt;
        // Unclaimed PC Amount
        uint256 claimedPTAmt;
    ...
    */
    ...
    function isOngoingCF(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.startTimestamp > 0, "CF: NOT_STARTED");
        require(ds.tpTimestamp == 0 || block.timestamp < ds.tpTimestamp.add(ds.bufferPeriod).add(ds.conversionPeriod), "CF: NOT_ONGOING");
    }

    // Crowdfunding is active when project owner sends enough project coins to funding contracts
    function isActiveCF(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.allocatedPCAmt > 0, "CF: CF_NOT_ACTIVE");
    }

    // Ensure actions can only happen when funding threshold is reached
    function isReachedToTh(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
    ...
    function isBufferPeriod(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.tpTimestamp > 0, "CF: NOT_REACHED_MIN_TP");
        require(block.timestamp >= ds.tpTimestamp && block.timestamp < ds.tpTimestamp.add(ds.bufferPeriod), "CF: NOT_IN_BP");
    }

    // Ensure actions can only happen while crowdfund is ongoing in CP
    function isConversionPeriod(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.tpTimestamp > 0, "CF: NOT_REACHED_MIN_TP");
        require(block.timestamp >= ds.tpTimestamp.add(ds.bufferPeriod) && block.timestamp < ds.tpTimestamp.add(ds.bufferPeriod).add(ds.conversionPeriod), "CF: NOT_IN_CP");
    }

    // Ensure actions can only happen while crowdfund is going after Buffer Time Period
    function isAfterBP(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.tpTimestamp > 0, "CF: NOT_REACHED_MIN_TP");
        require(block.timestamp >= ds.tpTimestamp.add(ds.bufferPeriod), "CF: NOT_ENDED_BP");
    }

    // Ensure actions can only happen when funding threshold is not reached
    function noFunds(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.totalStableCoinAmt == 0, "CF: ANY_FUNDS");
    }

    // Ensure actions can only happen when funding is ended
    function isEndOfCF(bytes32 storagePos) internal view {
        DiamondStorage storage ds = diamondStorage(storagePos);
        require(ds.tpTimestamp > 0, "CF: NOT_REACHED_MIN_TP");
        require(block.timestamp >= ds.tpTimestamp.add(ds.bufferPeriod).add(ds.conversionPeriod), "CF: NOT_ENDED");
    }

    // Ensure actions can only happen when Bond Investment is activated
    function isActiveBondInv(bytes32 storagePos) internal view {
        require(diamondStorage(storagePos).bondToken != address(0), "CF: BI_NOT_ACTIVATED");
    }
    ...
    function setCrowdfunding(bytes32 storagePos, string memory _pName, address _coinAddr, address _stAddr, uint16 _pOFeePt, uint256 _bfTime, uint256 _conversionPeriod, uint256 _minTh, address _treasuryAddr) internal {
        ...
        LibCFDiamond.DiamondStorage storage ds = LibCFDiamond.diamondStorage(storagePos);
    ...
    // Set Bonding Curve
    function setAnalyticMath(bytes32 storagePos, address _analyticMath) internal {
    ...
    function _applyFee(bytes32 storagePos, uint256 _coinAmt) internal returns (uint256 totalFee) {
        DiamondStorage storage ds = diamondStorage(storagePos);
    ...
    function getReleasablePCAmt(bytes32 storagePos, uint256 _curWeek) internal view returns (uint256){
        DiamondStorage storage ds = diamondStorage(storagePos);

        // Next Week Claimable PT Amount In Conversion Period
        if(_curWeek != ds.curWeek){
            // Get Release PT Amount of Current Week From Release Rate Array
            uint256 addPCAmt = ds.releasablePCAmt == 0 ? ds.weeklyReleaseAmt[0] : 0;
            for(uint256 i = ds.curWeek + 1; i <= _curWeek; i++){
                addPCAmt = addPCAmt.add(ds.weeklyReleaseAmt[i]);
            }
            //console.log('weekly claimable', addPCAmt, ds.releasablePCAmt);
            // Get Releasable PT Amount Of Current Week
            return ds.releasablePCAmt.add(addPCAmt);

        } else if(_curWeek == 0 && ds.releasablePCAmt == 0) {
            // First Week
            return ds.weeklyReleaseAmt[0];
        }

        // Default
        return ds.releasablePCAmt;
    }

    // Generate Random PC release per week
    function generatePCRelease(bytes32 storagePos, uint256[] memory rates, uint256 total) internal {
        DiamondStorage storage ds = diamondStorage(storagePos);
        for(uint256 i = 0; i < rates.length ;i++){
            // Set Weekly Releasable PC Percent
            ds.weeklyReleaseAmt[i] = rates[i] * ds.allocatedPCAmt / total;
        }
    }
}
...
library FundingUtils {
    ...
    function getWeeksInCP(uint256 conversionPeriod) internal pure returns(uint256) {
        // 604800 = 7 * 24 * 3600 (seconds of a week)
        uint256 w = conversionPeriod / 604800;
        if(conversionPeriod > w * 604800){
            return w + 1;
        }
        return w;
    }

    function getCurWeek(uint256 curCPTime) internal pure returns(uint256) {
        return curCPTime / 604800;
    }

    // @dev get rand number between 10000000 ~ 20000000
    // @param nonce random number nonce
    function random(bytes32 hash, uint256 nonce) internal pure returns (uint256) {
        return 1e7 + uint256(keccak256(abi.encodePacked(hash, nonce))) % 1e7;
    }

    // @dev selection sort uint256 array in increasing order
...
contract UniqueStorage {
    ...
    function getDiamondStorage() internal view returns (LibCFDiamond.DiamondStorage storage) {
        return LibCFDiamond.diamondStorage(_storagePos);
    }
}
