...
contract Origin is ERC721, OwnableNR, ReentrancyGuard {
    ...
    /// @notice This variable should iterate
    /// through the amount of NFTs that will be sold in the initial sale.
    Counters.Counter private _initSaleIdCounter;

    /// @notice This variable should iterate
    /// through the amount of NFTs that will be made by using the merge functionality.
    /// This counter should basically only return NFTs with 5 stars.
    /// TODO: This should start from 2450(?)
    Counters.Counter private _mergeIdCounter;
    ...
    uint256 public constant PUBLIC_SALE_PRICE = 0.006 ether;
    ...
    modifier callerIsUser() {
        require(
            ...
            "Hypatia :: Cannot be called by a contract"
        );
        _;
    }
    ...
    modifier notPaused() {
        require(!pause, "Hypatia :: Contract is paused.");
        _;
    }

    // Mainnet JSON:
    constructor() ERC721("Hypatia", "<CONTRACT-SYMBOL>") {}
    ...
    function mint(uint256 _quantity)
        ...
        notPaused
    {
        require(publicSale, "Hypatia :: Not Yet Active.");
        require(
            (_initSaleIdCounter.current() + _quantity) <= MAX_SUPPLY,
            "Hypatia :: Beyond Max Supply"
        );
        require(
            (totalPublicMint[msg.sender] + _quantity) <= MAX_PUBLIC_MINT,
            "Hypatia :: Minted maximum amount."
        );
        require(
            msg.value >= (PUBLIC_SALE_PRICE * _quantity),
            "Hypatia :: Not enough AVAX. "
        );
        for (uint256 i = 0; i < _quantity; i++) {
            _safeMint(_msgSender(), _initSaleIdCounter.current());
            starAssign(_initSaleIdCounter.current());
            _initSaleIdCounter.increment();
        }
    ...
    function whitelistMint(uint256 _quantity, bytes32[] calldata _proof)
        ...
        notPaused
    {
        require(whiteListSale, "Hypatia :: White-list minting is on pause");
        require(
            isEarlyMinter(_msgSender(), _proof),
            "Hypatia :: You are not whitelisted."
        );
        require(
            (_initSaleIdCounter.current() + _quantity) <= MAX_SUPPLY_WHITELIST,
            "Hypatia :: Cannot mint beyond max supply"
        );
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
            "Hypatia :: Cannot mint beyond whitelist max mint!"
        );
        require(
            msg.value >= (WHITELIST_SALE_PRICE * _quantity),
            "Hypatia :: Payment is below the price"
        );
        for (uint256 i = 0; i < _quantity; i++) {
            _safeMint(_msgSender(), _initSaleIdCounter.current());
            starAssign(_initSaleIdCounter.current());
    ...
    function teamMint() external nonReentrant onlyOwnerAdmin {
        require(!teamMinted, "Hypatia :: Team already minted.");
        teamMinted = true;
        for (uint256 i = 0; i < TEAM_MINT_AMOUNT; i++) {
            _initSaleIdCounter.increment();
            uint256 tokenId = _initSaleIdCounter.current();
            _safeMint(msg.sender, tokenId);
            require(starAssign(tokenId));
        }
    }

    /// @notice Explain to an end user what this does
    /// @dev
    /// @param _tokenA and _tokenB are the tokens that will be merged into _tokenC.
    function merge(uint256 _tokenA, uint256 _tokenB)
        external
        nonReentrant
        callerIsUser
        notPaused
    {
        // Simulating the scenario, where one of the token has 3 stars
        // and the other one has 2 stars.
        starAmount[_tokenA] = 3;
        starAmount[_tokenB] = 2;
        require(ownerOf(_tokenA) == _msgSender(),"Hypatia :: You are not the owner of _tokenA");
        require(ownerOf(_tokenB) == _msgSender(),"Hypatia :: You are not the owner of _tokenB");
        require(starAmount[_tokenA] + starAmount[_tokenB] == 5,"Hypatia :: Token A and Token B must add up to a total of five stars.");
        _burn(_tokenA);
        _burn(_tokenB);
        _mergeIdCounter.increment();
        uint256 tokenC = _mergeIdCounter.current();
        // TODO: change this hardcoded value.
        _mint(_msgSender(), 2499);
        require(starAssign(2499));
    }

    function starAssign(uint256 _tokenId) internal returns (bool) {
        if (_tokenId >= 0 && _tokenId <= 1250) {
            starAmount[_tokenId] = 1;
            return true;
        } else if (_tokenId >= 1251 && _tokenId <= 2050) {
            starAmount[_tokenId] = 2;
            return true;
        } else if (_tokenId >= 2051 && _tokenId <= 2350) {
            starAmount[_tokenId] = 3;
            return true;
        } else if (_tokenId >= 2351 && _tokenId <= 2450) {
            starAmount[_tokenId] = 4;
            return true;
        } else if (_tokenId >= 2451 && _tokenId <= 2500) {
    ...
    function _withdraw(address wallet, uint256 amount) private {
        ...
        require(success, "Hypatia: Transfer failed.");
    }
    ...
    function isEarlyMinter(
        ...
        return MerkleProof.verify(_proof, merkleRootWL, leaf);
    }
