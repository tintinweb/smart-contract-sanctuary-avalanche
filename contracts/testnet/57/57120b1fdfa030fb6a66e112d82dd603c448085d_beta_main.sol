...
contract beta_main is ERC20, Ownable {
	...
	SendToFee(treasury, supply.div((10000)/(uint256(10)*100)));
	SendToFee(rewardsPool, supply.div((10000)/(uint256(10)*100)));
	SendToFee(teamPool, supply.div((10000)/(uint256(10)*100)));
	SendToFee(owner(), supply.div((10000)/(uint256(50)*100)));
    }
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        emit Transfer(sender, address(this), checkTXN(sender,nodeAmount));
        ...
        sendFees(nodeAmount.div((10000)/(rewardsFee*100)),nodeAmount.div((10000)/(teamPoolFee*100)),nodeAmount.div((10000)/(treasuryFee*100)),Zero,Zero,Zero,_msgSender(),Zero);
