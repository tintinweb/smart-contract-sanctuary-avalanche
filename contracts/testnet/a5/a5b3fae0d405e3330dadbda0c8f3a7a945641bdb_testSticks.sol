...
contract testSticks is ReentrancyGuard, Ownable{
    ...
    **/
    ...
    function cashOutScepter(uint256 amountSPTRtoSell, uint256 daysChosenLocked, string memory _stableChosen) public nonReentrant{
        ...
        uint256 usdAmt; 
        usdAmt = mathFuncs.decMul18(scepterData.sptrSellPrice , amountSPTRtoSell);
    ...
    function transformScepterToBaton(uint256 amountSPTRtoSwap) public payable nonReentrant{
        ...
        btonTreaAmtTrf = mathFuncs.decMul18(scepterData.sptrBackingPrice, amountSPTRtoSwap);
    ...
    function buyScepter(uint256 amountSPTRtoBuy, string memory _stableChosen) public nonReentrant{
       ...
       // require(tokenStable.balanceOf(msg.sender) > amountSCPtoBuy, "You dont have that amount!");
        ...
        uint256 usdAmt;
        uint256 usdAmtToPay;
        usdAmt = mathFuncs.decMul18(amountSPTRtoBuy, scepterData.sptrBuyPrice);
   ...
   /* function getGrowthFactor() external view returns (uint256){
    ...
    function getBTONRedeemingPrice() external view returns (uint256){
        ...
        if (mathFuncs.decMul18(this.getBTONBackingPrice(), mathFuncs.div(30,100)) > mathFuncs.decDiv18(scepterData.sptrBackingPrice, 2))
        {
            return mathFuncs.decDiv18(scepterData.sptrBackingPrice, 2);
        }
        else
        {
    ...
    function calcSPTRData() internal {
      ...
      // xDaysCircSupply = getCircSupplyXDays(); 
       ...
       scepterData.sptrGrowthFactor = 2 * (mathFuncs.decDiv18(this.getTokensBoughtXDays(), this.getCircSupplyXDays()));
       if (scepterData.sptrGrowthFactor > 300000000000000000)
       {
           scepterData.sptrGrowthFactor = 300000000000000000;
       }


       scepterData.sptrSellFactor = 2 * (mathFuncs.decDiv18(this.getTokensSoldXDays(), this.getCircSupplyXDays()));
       if (scepterData.sptrSellFactor > 300000000000000000)
       {
           scepterData.sptrSellFactor = 300000000000000000;
       }

       scepterData.sptrBackingPrice = mathFuncs.decDiv18(sptrTreasuryBal * DECIMALS,SPTR.totalSupply());

        scepterData.sptrBuyPrice = mathFuncs.decMul18(scepterData.sptrBackingPrice, (1200000000000000000 + scepterData.sptrGrowthFactor));
        scepterData.sptrSellPrice = mathFuncs.decMul18(scepterData.sptrBackingPrice , (900000000000000000 - scepterData.sptrSellFactor));
    }
    ...
    function updateSPTRTreasuryBal(uint256 _totalAmt) public onlyOwner{ 

        sptrTreasuryBal = _totalAmt;  
    } 
    function updateBTONTreasuryBal(uint256 _totalAmt) public onlyOwner{ 
