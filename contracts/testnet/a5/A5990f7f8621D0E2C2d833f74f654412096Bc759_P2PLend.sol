...
interface IP2PLend {
    ...
    function borrow(DataTypes.BorrowParams memory params) external payable;
...
library BorrowLogic {
        ...
        internal
    {
        ...
        if (params.reserveAsset == IConfigProvider(provider).weth()) {
            if (params.isOfferNow && params.isNative) {
                require(
    ...
    ERC721HolderUpgradeable
{
    ...
    function borrow(DataTypes.BorrowParams memory params)
        public
        payable
        whenNotPaused
        nonReentrant
    {
        BorrowLogic.executeBorrow(address(provider), params);
