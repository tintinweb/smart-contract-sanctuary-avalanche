    ...
    IERC721Metadata
{
    ...
    function tokenURI(uint256 nodeIndex)
        public
        view
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                tierName,
                                '","image":"',
                                bytes(_nodeList[nodeIndex].image).length > 0 ? _nodeList[nodeIndex].image: placeholderImage,
                                '"',
                                ',"attributes":[',
                                '{',
    ...
    function _createNodeForAddress(
        ...
        _setOwner(owner);
        emit Minted(totalNodesCreated - 1);
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _nodesOfUser[owner][tokenId] = _nodesOfUser[owner][_nodesOfUser[owner].length-1];
        _nodesOfUser[owner].pop();
        delete _nodeList[tokenId];
        totalNodesCreated--;
    ...
    function _calculateReward(uint256 nodeIndex)
        private
        view
        returns (uint256)
    {
        ...
        if (_nodeList[nodeIndex].tempBooster > 0 &&
            _nodeList[nodeIndex].tempBoosterDaysCounter > 0) {
            tempBoostedRewards = totalRewards * ((1000 + _nodeList[nodeIndex].tempBooster) / 1000);
        }
        if (_nodeList[nodeIndex].fixedBooster > 0) {
            fixedBoostedRewards = totalRewards * ((1000 + _nodeList[nodeIndex].fixedBooster) / 1000);
        }
    ...
    function _nodeIndexToId(uint256 index)
        private
        view
        returns (string memory)
    {
    ...
    function boostNode(uint256 nodeIndex, uint256 tempBooster, uint256 tempBoosterDays, uint256 fixedBoosterBasis) external indexAvailable(nodeIndex) onlySentry {
    ...
    function burn(uint256 tokenId) external indexAvailable(tokenId) {
        require(_msgSender() == ownerOf(tokenId), 'not authorized to burn this node');
        _burn(tokenId);
        emit Burned(tokenId);
    }

    function getRewardAmountOf(uint256 nodeIndex)
        external
        view
        returns (uint256)
    {
    ...
    function cashOutReward(address account, uint256 index)
        external
        onlySentry
    ...
    // for each node
    function cashOutAllReward(address _owner)
        external
        onlySentry
        returns (uint256)
    {
    ...
    function getNodeId(uint256 index)
        external
        view
        indexAvailable(index)
        returns (string memory)
    ...
    function _changeNodeCashOutFee(uint256 newNodeCashOutFee)
        external
        onlySentry
    {
    ...
    function setMultiSentry(address[] memory addresses, bool isPermitted)
        external
        onlySentry
    {
    ...
    function _calcRestoreFee(uint256 nodeIndex)
        internal
        view
        returns (uint256)
    {
    ...
    function _getThorPrice(uint256 _amountIn)
        internal
        view
    ...
    function isDelinquent(uint256 nodeIndex)
        external
        view
        override
        returns (bool)
    ...
    function getFee(uint256 nodeIndex)
        public
        view
        indexAvailable(nodeIndex)
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        _nodesOfUser[from][tokenId] = _nodesOfUser[from][
            _nodesOfUser[from].length - 1
        ];
        _nodesOfUser[from].pop();
        _nodesOfUser[to].push(tokenId);
        _nodeList[tokenId].owner = to;
        _setOwner(to);
