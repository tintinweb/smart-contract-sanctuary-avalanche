...
// File: contracts/talentscout1.sol
...
contract TalentScout is IERC20,IERC20Metadata, Auth {
  ...
  address BUSD = 0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664; // use USDC for now
  ...
  uint256 purchaseTax = 1200;
  uint256 sellTax = 1500;
  uint256 transferTax = 1500;
  uint256 lpTax = 200;
  ...
  function _transfer(
   ...
   function checkHolderList(address _address) internal{
    ...
    if (!_holderList[_address]) {
    _holderList[_address] = true;
    holders.push(_address);
    }
    }

    function distributedReflectionAmount() public onlyOwner {
        // uint counter;
        
        
        
        // uint256 reflectionFees = 6500;
        // uint256 purchaseTax = 1200;
        //  uint256 feeDenominator = 10000;
        //  for(uint i = 0; i<holders.length; i++){
            //    uint256 bal = balanceOf(holders[i]);
            // uint circulatingSupply +=  balanceOf(holders[i]);
            // uint amount = reflectionTax.div(feeDenominator).mul(fees[_id].totalFee);
            //    uint amount =  reflectionTax.div(feeDenominator).mul((purchaseTax.div(feeDenominator)).mul(bal));
                 
                //  uint amount = balanceOf(feeAddress).mul(reflectionFees).div(feeDenominator);
                // _transferFrom(msg.sender, holders[i], amount);
        // }
        ...
        for(uint i = 0; i < holders.length; i++){
            ...
            uint distributedamt = (bal.div(getCirculatingSupply())).mul(100); // percentage of amt to be distributed 
            
            uint reflectionamt = ((reflectionTax).div(feeDenominator)).mul(distributedamt);
            
            uint reflectionFeeAmount = bal.mul(reflectionamt);

            _transferFrom(msg.sender, holders[i], reflectionFeeAmount);
        }
