...
contract Token is ERC20Upgradeable {
    ...
    uint32 public buyBackFee;      // 3000 => 30%
    ...
    //         swapAndLiquify();
    //     }
    //     //if (transferTaxRate == 0 || isExcludedFromFee[from] || isExcludedFromFee[to]) { removed due current transferTaxRate == 0
    ...
    function _transfer(address from, address to, uint256 amount)  internal virtual override {
        ...
        bool takeFeeEnabled = true;

        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            takeFeeEnabled = false;
        }

        _finalizeTransfer(from, to, amount, takeFeeEnabled);
    }

    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFeeEnabled) internal returns (bool) {
        ...
        _tokenTransfer(from, to, amount, takeFeeEnabled);
        return true;
    }
    ...
    function takeBuySellTransferFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (totalFee > 0) {
            uint256 feeAmount = (amount * totalFee) / 10000;
            _takeFees(from, feeAmount);
    ...
    // function _transferBothExcluded(address sender, address recipient, uint256 amount) private returns (bool) {
    //     super._transfer(sender, recipient, amount);
    //     emit Transfer(sender, recipient, amount);
    //     return true;
    // }

    function _takeFees(address sender, uint256 amount) private {
    ...
    function removeAllFee() private {
        if (buyBackFee == 0) return;
        _previousTaxFee = buyBackFee;
        buyBackFee = 0;
    }

    function restoreAllFee() private {
        buyBackFee = _previousTaxFee;
    }
   ...
   /**
     ...
     * Can only be called by the current operator.
     */
    function setPairAddress(address lpPair) public onlyOwner() { 
        uniswapV2Pair = address(lpPair);
