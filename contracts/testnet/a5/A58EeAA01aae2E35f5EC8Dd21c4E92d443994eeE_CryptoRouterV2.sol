...
abstract contract BaseRouter is Pausable, EIP712, EndPoint, AccessControlEnumerable {
    ...
    event ComplexOpSet(bytes32 oop, bool registered);
     ...
     *
     * @param complexOps_ array of complex operation hashes and registered flags.
     */
    function registerComplexOp(ComplexOp[] memory complexOps_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i; i < length; ++i) {
            ops[complexOps_[i].oop] = complexOps_[i].registered;
            emit ComplexOpSet(complexOps_[i].oop, complexOps_[i].registered);
        }
    }
    ...
    function _execute(uint256 cPos, string[] calldata operations, bytes[] calldata params) internal virtual whenNotPaused returns (
        bytes32 nextRequestId,
        uint64 chainIdTo,
        ExecutionResult result,
        uint8 lastOp
