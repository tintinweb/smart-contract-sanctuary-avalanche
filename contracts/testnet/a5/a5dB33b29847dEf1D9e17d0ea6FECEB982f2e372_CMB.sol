...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event WithdrawnAmount(uint256 indexed paymentId);
    ...
    function payNative(uint256 paymentId)
        external 
        payable 
        onlyValidPayment(paymentId) 
        onlyClient(paymentId) 
        ...
        nonReentrant 
    {
        ...
        payment.status = Status.CANCELLED;
        payable(payments[paymentId].client).transfer(payments[paymentId].paidAmount);
        emit WithdrawnAmount(paymentId);
    }
}
