...
contract ERC1155Nft is ERC1155Supply, Ownable {
     ...
     * @dev Get TokenLock.
     */
    function tokensLock(address owner, uint256 id)
        public
        view
        virtual
    ...
    function revoke(
        ...
        TokenLock memory item = items[index];

        require(sender == item.sender, "Invalid sender");
        require(receiver == item.receiver, "Invalid receiver");
        require(block.timestamp <= item.expireAt, "Token is not locked");

        items[index].expireAt = 0;
        _safeTransferFrom(receiver, sender, id, item.amount, "");
    }
