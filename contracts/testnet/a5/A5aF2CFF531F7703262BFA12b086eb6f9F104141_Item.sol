...
contract Item is ERC1155, Ownable, ReentrancyGuard {
    ...
    AggregatorV3Interface public _priceOracle;
    ...
    /**
     * @dev Returns whether the `id` type item is paused.
     *
     * Requirements:
     *
     * - `id` must be less than the length of `itemTypes`.
     */
    function getItemPaused(uint256 id) external view returns (bool) {
        require(id < itemTypes.length, "item doesn't exist");

        return items[id].paused;
    }
