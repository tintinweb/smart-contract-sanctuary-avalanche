    ...
    function uploadAttributes(
        ...
        for (uint256 i = 0; i < petIds.length; i++) {
            ...
            if (attributes.length == petIds[i]) {
                attributes.push(Attributes(names[i], pngs[i], rarities[i], boostNames[i]));
            } else {
                attributes[petIds[i]] = Attributes(names[i], pngs[i], rarities[i], boostNames[i]);
            }
        }
    }
