...
// File: contracts/Awea.sol

pragma solidity ^0.8.4;

contract WaifuZ is Ownable, ERC721, NonblockingReceiver {
    using Strings for uint256;
    ...
    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("Waifu Zero Layer", "WAIFUZ")
    {
        _owner = msg.sender;
    ...
    function mint(uint8 _amount) external payable {
        require(_amount < 3, "ERROR: Max 2 NFTs per transaction");
        require(
            nextTokenId + _amount <= MAX_MINT_AVALANCHE,
            "ERROR: Mint exceeds supply"
        );
    ...
    function mintForAddress(uint8 _amount, address _receiver) external onlyOwner {
        require(_amount < 3, "ERROR: Max 2 NFTs per transaction");
        require(
            nextTokenId + _amount <= MAX_MINT_AVALANCHE,
            "ERROR: Mint exceeds supply"
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "ERROR: URI query for nonexistent token"
        );
    ...
    function withdraw(uint256 _amount) external onlyOwner {
        (bool sent, ) = payable(_owner).call{value: _amount}("");
        require(sent, "ERROR: Failed to withdraw Ether");
    }
