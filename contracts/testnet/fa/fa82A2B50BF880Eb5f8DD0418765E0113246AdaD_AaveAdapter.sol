...
import {IAaveStakingPool} from "./IAaveStakingPool.sol";
import {IVariableDebtToken} from "../../../interfaces/aave/IVariableDebtToken.sol";
import {IAaveOracle} from "../../../interfaces/aave/IAaveOracle.sol";

contract AaveAdapter is AdapterBase, Basic {
    using SafeERC20 for IERC20;

    using UserConfiguration for DataTypes.UserConfigurationMap;

    event AaveDeposit(address token, uint256 amount, address owner);
    event AaveWithDraw(address token, uint256 amount, address owner);
    event AaveBorrow(
        address token,
        uint256 amount,
        address owner,
    ...
    constructor(address _adapterManager)
        AdapterBase(_adapterManager, "AaveV2")
    {}

    /**
     * @dev Aave Lending Pool Provider
     */
    address public constant wavaxVtokenAddr =
    ...
    IAaveLendingPoolAddressProvider internal constant aaveProvider =
        IAaveLendingPoolAddressProvider(aaveProviderAddr);

    /// @dev Aave Protocol Data Provider
    address public constant aaveDataAddr =
        0x65285E9dfab318f57051ab2b139ccCf232945451;
    IAaveProtocolDataProvider internal constant aaveData =
        IAaveProtocolDataProvider(aaveDataAddr);
    ...
    function deposit(bytes calldata encodedData) external onlyAdapterManager {
        ...
        address aavePoolAddr = aaveProvider.getLendingPool();
        IAaveLendingPool aave = IAaveLendingPool(aavePoolAddr);
        ...
        if (isEth) {
            ...
            pullAndApprove(token, owner, aavePoolAddr, amount);
            aave.deposit(token, amount, owner, referralCode);
        }
        emit AaveDeposit(token, amount, owner);
    }
    ...
    function setCollateral(bytes calldata encodedData) external onlyProxy {
        ...
        IAaveLendingPool aave = IAaveLendingPool(aaveProvider.getLendingPool());
    ...
    function withdraw(bytes calldata encodedData) external onlyAdapterManager {
        ...
        if (isEth) {
            ...
            IWAVAXGateway wavaxGateway = IWAVAXGateway(wavaxGatewayAddr);
            wavaxGateway.withdrawETH(aavePoolAddr, amount, owner);
        } else {
            pullAndApprove(atoken, owner, aavePoolAddr, amount);
            aave.withdraw(token, amount, owner);
        }
        emit AaveWithDraw(token, amount, owner);
    }
    ...
    function borrow(bytes calldata encodedData) external onlyProxy {
        ...
        if (isEth) {
            ...
            aave.borrow(token, amount, rateMode, referralCode, owner);
        }
        emit AaveBorrow(token, amount, owner, rateMode);
    }

    function approveDelegation(bytes calldata encodedData) external onlyProxy {
        uint256 amount = abi.decode(encodedData, (uint256));
        IVariableDebtToken(wavaxVtokenAddr).approveDelegation(
            wavaxGatewayAddr,
            amount
        );
    }
    ...
    function borrowTokens(bytes calldata encodedData) external onlyProxy {
        ...
        for (uint256 i = 0; i < token.length; i++) {
            aave.borrow(token[i], amount[i], rateMode, referralCode, owner);
        }
    }
    ...
    function payback(bytes calldata encodedData) external onlyProxy {
        ...
        address aaveAddr = aaveProvider.getLendingPool();
        if (isEth) {
            IWAVAXGateway wavaxGateway = IWAVAXGateway(wavaxGatewayAddr);
            ...
            IAaveLendingPool(aaveAddr).repay(
                tokenAddr,
                amount,
                rateMode,
                borrower
        ...
        emit AaveRepay(tokenAddr, amount, borrower, rateMode);
    }

    function getReward(bytes calldata encodedData) external onlyProxy {
        address claimAddress;
        uint256 amount;
        address[] memory assertAddress;
        address to;
        (claimAddress, assertAddress, amount, to) = abi.decode(
            encodedData,
            (address, address[], uint256, address)
        );

        IAaveIncentivesController(claimAddress).claimRewards(
            assertAddress,
            amount,
            to
        );
        emit AaveClaim(claimAddress, amount);
    }

    function positionTransfer(bytes calldata encodedData)
        external
        onlyAdapterManager
    {
        (, address proxyAddr, address collateralPool) = abi.decode(
            encodedData,
            (address, address, address)
        );
        IAaveStakingPool pool = IAaveStakingPool(collateralPool);
        uint256 amountBefore = pool.getAmount();
        uint256 amountLoan = 10e18;
        pool.flashLoan(proxyAddr, amountLoan);
        string memory callFunc = "exchangeDebt(bytes)";
        toCallback(proxyAddr, callFunc, encodedData);
        uint256 amountDiff = pool.getAmount() - amountBefore;
        require(amountDiff == 0, "amount is not true!");
    }

    function exchangeDebt(bytes calldata encodedData) external onlyProxy {
        (address user, address owner, address collateralPool) = abi.decode(
            encodedData,
            (address, address, address)
        );
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (debtAmount != 0) {
                ...
                IERC20(tokens[i]).safeApprove(address(aave), debtAmount);
        ...
        IAaveStakingPool pool = IAaveStakingPool(collateralPool);
        IAToken(pool.getToken()).approve(
            collateralPool,
            pool.getUserLoan(owner)
        );
        pool.repay(owner);
    }
        ...
        )
    {
        ...
        address[] memory reservesList = aave.getReservesList();
        ...
        returns (uint256)
    {
        ...
        address[] memory reservesList = aave.getReservesList();
        uint256 assetValue;
        IAaveOracle oracle = IAaveOracle(aaveOracleAddr);
        for (uint256 i = 0; i < reservesList.length; i++) {
            DataTypes.ReserveData memory asset = aave.getReserveData(
                reservesList[i]
            );
            uint256 price = oracle.getAssetPrice(reservesList[i]);
            IAToken atoken = IAToken(asset.aTokenAddress);
            assetValue +=
                (price * atoken.balanceOf(account)) /
                (10**atoken.decimals());
        }
        return (assetValue);
    }
...
interface IAaveStakingPool {
    function flashLoan(address user, uint256 amount) external;
