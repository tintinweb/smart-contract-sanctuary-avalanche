...
contract proxyDeployer is Owned {
    ...
    function getCollectionDatas(address _collectionAddress, address _whiteAddress)
        public
        view
        ...
        bool[] memory)
    {
        ...
        return (collectionsList[_collectionAddress].Currency, data, _Address, _status);
    }
    ...
    function editCollections(
        ...
        CollectiondetailList[_collectionId] = collectionsList[_collectionAddress];
    }
    function addAndRemoveWhiteList(address _collectionAddress, address[] memory _whiteAddress, string memory _status) public onlyOwner {
        bool update = true;
        if (
        ...
        for (uint i=0; i<_whiteAddress.length; i++) {
            whitelistedAddresses[_collectionAddress][_whiteAddress[i]] = update;
        }
    }
    function removeAndAddCollection(address _collectionAddress, uint256 _id) public onlyOwner {
