...
contract AnnuSea {
    ...
    modifier onlyNotSeller(uint256 tokenId) {
        require(items[tokenId].seller != msg.sender, "can't buy self");
        _;
    }
    modifier inTime(uint256 tokenId) {
        require(block.timestamp < items[tokenId].endAt, "time out");
        _;
    }
    modifier checkState(uint256 tokenId, ItemState state) {
        require(items[tokenId].state == state, "StateError");
        _;
    }
    ...
    function buy(uint256 tokenId)
        public
        payable
        checkState(tokenId, ItemState.Created)
        onlyNotSeller(tokenId)
        inTime(tokenId)
    {
        //檢查錢夠不夠
    ...
    function sellDown(uint256 tokenId)
        public
        checkState(tokenId, ItemState.Created)
        onlySeller(tokenId)
    {
    ...
    function getAllItem() public view returns (Item[] memory) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (items[tokenIds[i]].state == ItemState.Created) {
                _items[count] = items[tokenIds[i]];
                count++;
            }
        }
        return _items;
