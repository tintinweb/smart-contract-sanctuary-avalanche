...
import "./libraries/LevelUtils.sol";
...
*/
contract MembershipCardV2 is Initializable, UUPSUpgradeable,AccessControlUpgradeable, ERC721AUpgradeable  {
    using SafeERC20Upgradeable for IERC20BurnAndMintUpgradeable;
    using StringsUpgradeable for uint256;
    using StringsUpgradeable for uint224;

    // @dev The address of DAO token contract
    address public daoTokenAddress;
    // @dev The address of betUp contract
    ...
    function calcUpgradeAmount(uint256 _beforeLevel, uint256 _afterLevel) external view returns(uint256) {
        return _getLevelUpAmount(_beforeLevel, _afterLevel);
    }
    ...
    function getSnapshot(address _address) external view returns(uint256){
        return _playersSnapshot[_address].snapshot;
    }

    function getMaxLevel(address _address) external view returns(uint256){
        return LevelUtils.query(_playersSnapshot[_address].snapshot);
    }

    function getLevelAmounts(address _address) external view returns(uint256[] memory){
        uint256 snapshot = _playersSnapshot[_address].snapshot;
        uint256 maxLevel = LevelUtils.query(snapshot);

        uint256[] memory levelAmounts = new uint256[](maxLevel);

        for(uint256 i = 0; i < maxLevel; i++) {
            uint256 level = i + 1;
            levelAmounts[i] = _playersSnapshot[_address].amount[level];
        }

        return levelAmounts;
    }

    function getSerialWith(uint256 _tokenId) external view returns(PropertiesType[] memory) {
    ...
    function calcLeftMintOf(address _sender) external view returns(uint256) {
        ...
        if(pr.amount < 30 || pr.accAmount < 2000*10**6) {
            return 0;
        }

        uint256 totalMintAmount = calTotalMintOf(pr);
        uint256 mintedAmount = _numberMinted(_sender);

        uint amount = 0;
        if(totalMintAmount >= mintLimit) {
            amount = mintLimit - mintedAmount;
        }else {
            amount = totalMintAmount - mintedAmount;
        }
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        require(_fixedItems.length < levelConfig[1].categoryAmount, "invalid _fixedItems length");
        LevelConfig memory lc = levelConfig[1];
        ...
        for(uint256 i; i < 3; i++) {
            ...
            require(index <= itemAmount && index > 0, "invalid fixed number" );
        }

        address _sender = msg.sender;
        IBet.PlayerRecord memory pr = IBet(betContract).playerRecords(_sender);
        require(pr.amount >= 30 && pr.accAmount >= 2000*10**6, "condition is not met");

        uint256 __numberMinted = _numberMinted(_sender);
        require(
            calTotalMintOf(pr) - __numberMinted >= _amount &&
            _amount <= mintLimit - __numberMinted,
            "invalid _amount"
        );
        ...
        for(uint256 i = 0; i < _amount; i++) {
            ...
            (
                uint256 _serial,
                uint256 _newLevelRareItemAccAmount
            ) = _setSerials(1, _fixedItems, random);

            _properties[_tokenId].serial[1] = _serial;
            levelConfig[1].rareItemAccAmount = _newLevelRareItemAccAmount;
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        uint256 perSpendAmount = _getLevelUpAmount(_beforeLevel, _afterLevel);
        IERC20BurnAndMintUpgradeable(daoTokenAddress).burn(owner, perSpendAmount);
        ...
        uint256 _serial,
        uint256 _newLevelRareItemAccAmount
        ) = _updateSerial(_beforeLevel, _afterLevel, random, _tokenId);

        _properties[_tokenId].serial[_afterLevel] = _serial;
        levelConfig[_afterLevel].rareItemAccAmount = _newLevelRareItemAccAmount;
    ...
    function updateItemAmount(uint256 _level, uint256 _index, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        LevelConfig memory lc = levelConfig[_level];
        require(_index < lc.categoryAmount, "Invalid _index");
    ...
    */
    function calTotalMintOf(IBet.PlayerRecord memory _pr) private pure returns(uint256){
        ...
        uint256 base = 2000 * 10 ** 6;
        uint256 n2 = (2 * _pr.accAmount - base) /base;
        uint result = n1;
        if(n1 > n2) {
            result = n2;
        }
      ...
      calculate total consumed dao token amount `acc`
        For i = low; i < high; i++;
           ...
           acc += [1+(_n-1)] x(_n-1) / 2 * baseUpgrade
                = baseUpgrade * n(n - 1) / 2
  */
    function _getLevelUpAmount(uint256 _beforeLevel, uint256 _afterLevel) internal view returns(uint256) {
        uint256 acc = 0;
    ...
    function _setSerials(
        ...
        uint256 _newLevelRareItemAccAmount = 0;
        uint256 _serial;

        for(uint256 i = 0 ; i < lc.categoryAmount; i++) {
            if(i < 3) {
                _serial = _serial << 8 | _fixedItems[i];
            }else {
                (uint256 _accAmount, uint256 _itemRandomNumber) = _genSerial(lc, i, _random);
                _serial = _serial << 8 | uint256(_itemRandomNumber);
                _newLevelRareItemAccAmount = _newLevelRareItemAccAmount << 8 | _accAmount;
            }

        }
        return (_serial, _newLevelRareItemAccAmount);
    }
    ...
    function _updateSerial(
        ...
        uint256 _random,
        uint256 _tokenId
    ) internal view returns(uint256 _newLevelRareItemAccAmount, uint256 _serial){
        ...
        for(uint256 i = 0; i < alc.categoryAmount; i++) {
            ...
            if(i < 3) {
                base = 8 * (levelConfig[_beforeLevel].categoryAmount - (i+1));
                itemRandomNumber = (_properties[_tokenId].serial[_beforeLevel] >> base) & 0xff;
            }else {
                (uint256 _accAmount, uint256 _itemRandomNumber) = _genSerial(alc, i, _random);
                accAmount = _accAmount;
                itemRandomNumber = _itemRandomNumber;
            }

            _serial = _serial << 8 | uint256(itemRandomNumber);
            _newLevelRareItemAccAmount = _newLevelRareItemAccAmount << 8 | accAmount;
        }
    ...
    function _genSerial(LevelConfig memory lc, uint256 i, uint256 _random ) private view returns(uint256, uint256 ){
        ...
        uint256 limitAccAmount = lc.rareItemAccAmount >>  base & 0xff;
        uint256 _itemRandomNumber = (_random >> i) % itemAmount + 1;

        uint256 _accAmount = limitAccAmount;

        if(_itemRandomNumber == 1 && itemLimitAmount > 0) {
            if(_checkRate(_random)  && limitAccAmount < itemLimitAmount) {
                _accAmount = limitAccAmount + 1;
            }else {
        ...
        return (_accAmount, _itemRandomNumber);
    }
    ...
    function _afterTokenUpgraded(
        ...
        address owner = msg.sender;
        _playersSnapshot[owner].amount[_beforeLevel]--;
        _playersSnapshot[owner].amount[_afterLevel]++;

        uint256 snapshot = _playersSnapshot[owner].snapshot;
        if(_playersSnapshot[owner].amount[_beforeLevel] == 0) {
            snapshot = snapshot ^( 1 << 256 - _beforeLevel);
        }

        if(_playersSnapshot[owner].amount[_afterLevel] == 1) {
            snapshot = snapshot ^( 1 << 256 - _afterLevel );
        }

        _playersSnapshot[owner].snapshot = snapshot;
    }
    ...
    function _afterTokenTransfers(
        ...
        for(uint256 i = 0; i < quantity; i++) {
            ...
            uint256 level = _properties[tokenId].level;
            uint256 snapshot;

            // mint
            if(from == address(0) && to != address(0)) {
                snapshot = _playersSnapshot[to].snapshot;
                bool hasBit = LevelUtils.getBit(snapshot, level);

                if(hasBit == false) {
                    _playersSnapshot[to].snapshot = snapshot | (1 << 256 - level);
                }
                _playersSnapshot[to].amount[level]++;

            // transfer
            }else if(from !=address(0) && to != address(0)) {
                // from
                if(_playersSnapshot[from].amount[level] - 1 == 0) {
                    _playersSnapshot[from].snapshot = _playersSnapshot[from].snapshot ^ (1 << 256 - level);
                }
                _playersSnapshot[from].amount[level]--;

                // to
                if(_playersSnapshot[to].amount[level] == 0) {
                    _playersSnapshot[to].snapshot = _playersSnapshot[to].snapshot | (1<< 256 - level);
                }
                _playersSnapshot[to].amount[level]++;
    ...
    function initialize(
        ...
        __ERC721A_init("Betting Sheep", "BETS");
