...
// File: nodeHandler.sol
//  import "hardhat/console.sol";
pragma solidity ^0.8.4;

contract nodeHandler is Ownable, Pausable {
    ...
    mapping(address => uint256) public nodesPerUser;

    address public presale;
    address public token;
    uint8 public rewardPerNode;
    uint8 public boostRewardsPerNode = 20;
    uint256 public minPrice;
    uint256 public boostEndTime = (block.timestamp) + (7 * 24 * 3600);

    uint256 public totalNodesCreated = 0;
    uint256 public totalStaked = 0;
    uint256 public totalClaimed = 0;

    uint8[] private _boostMultipliers = [100, 100, 100];
    ...
    modifier onlyGuard() {
        require(owner() == _msgSender() || token == _msgSender() || presale == _msgSender(), "NOT_GUARD");
        _;
    }
    ...
    function _calculateNodeRewards(uint _lastClaimTime, uint256 amount_) private view returns (uint256 rewards) {
        ...
        uint256 avRewardsPerNode = _getAvgRewards(_lastClaimTime);
        uint256 rewardPerDay = amount_.mul(avRewardsPerNode).div(100);
        return (((rewardPerDay.mul(10000).div(1440) * (elapsedTime_ / 1 minutes)) / 10000) * boostMultiplier).div(1e36);
    }
    ...
    function _getAvgRewards(uint256 _lastClaimTime)internal view returns(uint256){

        if(block.timestamp < boostEndTime){
            // console.log("Here 1");
            return (boostRewardsPerNode).mul(1e36);
        }
        else if(_lastClaimTime >= boostEndTime){
            // console.log("Here 2");
            return (rewardPerNode).mul(1e36);
        }
        else{
            // console.log("Here 3");
            return (((block.timestamp - boostEndTime) * rewardPerNode) + ((boostEndTime - _lastClaimTime) * boostRewardsPerNode)).mul(1e36).div(block.timestamp - _lastClaimTime);
        }
    }
    ...
    function createNode(address account, string memory nodeName, uint256 amount_) external onlyGuard whenNotPaused {
        ...
        totalNodesCreated = totalNodesCreated.add(1);
        totalStaked += amount_;
        nodesPerUser[account] = nodesPerUser[account].add(1);
    }
    ...
    function updateReward(uint8[] memory newVal) external onlyOwner {
        boostRewardsPerNode = newVal[0];
        rewardPerNode = newVal[1];
    }
    ...
    // function getNodesPerUser(address account)external view returns (uint256){
    //     return nodesPerUser[account];
    // }
}