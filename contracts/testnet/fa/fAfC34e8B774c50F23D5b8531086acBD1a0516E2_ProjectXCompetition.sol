...
contract ProjectXCompetition {
    ...
    address public token = 0xd821fE4915988b193FE1d4081E0cdf7bD9322EFd;
    ...
    function create(uint32 countTotal, int256 priceForGuest, int256 priceForMember, uint32 maxPerPerson, address prizeToken, uint256 prizeAmount) public forOwner returns (uint256) {
        ...
        require(priceForGuest > priceForMember, "Create: Invalid Price.");
        ...
        if(prizeAmount > 0) {
            winningPrizes[idNew] = WinningPrize({
                token: prizeToken,
                amount: prizeAmount
            });
        }
    ...
    function update(uint256 id, uint32 countTotal, int256 priceForGuest, int256 priceForMember, uint32 maxPerPerson, address prizeToken, uint256 prizeAmount) public forOwner {
        ...
        require(priceForGuest > priceForMember, "Update: Invalid Price.");
        ...
        if(prizeAmount > 0 || winningPrize.amount > 0) {            
            winningPrize.token = prizeToken;
            winningPrize.amount = prizeAmount;
        }
    }
