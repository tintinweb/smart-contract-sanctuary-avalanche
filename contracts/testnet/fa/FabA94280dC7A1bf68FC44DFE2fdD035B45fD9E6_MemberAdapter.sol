...
contract MemberAdapter is IMemberAdapter, Reimbursable, MemberGuard, AdapterGuard {
    ...
    event SubmitSetMemberAgreementProposalEvent(
        address _daoAddress, address _addressSender, bytes _data, bytes32 proposalId);
    ...
    event ProcessSetMemberAgreementProposalEvent(
        address _daoAddress, address _addressSender, bytes32 proposalId);
    ...
    function submitSetMemberAgreementProposal(
        ...
        string memory origin = "MemberAdapter.submitSetMemberAgreementProposal";
        uint256 length = _factoryMemberConfigArray.length;
        ...
        setMemberAgreementProposal[address(dao)][proposalId].status = Foundance.ProposalStatus.NOT_STARTED;
        setMemberAgreementProposal[address(dao)][proposalId].votingConfig = _votingConfig;
        setMemberAgreementProposal[address(dao)][proposalId].epochConfig = _epochConfig;
        setMemberAgreementProposal[address(dao)][proposalId].dynamicEquityConfig = _dynamicEquityConfig;
        setMemberAgreementProposal[address(dao)][proposalId].communityEquityConfig = _communityEquityConfig;
        VotingAdapterLibrary._submitProposal(dao, proposalId, data, origin);
        ...
        reimbursable(dao)
    {
        ...
        if (proposal.status == Foundance.ProposalStatus.IN_PROGRESS) {
            ...
            MemberExtension member = MemberExtension(dao.getExtensionAddress(DaoLibrary.MEMBER_EXT));
            member.setMemberEnvironment(dao,proposal.dynamicEquityConfig,proposal.communityEquityConfig,proposal.epochConfig);
