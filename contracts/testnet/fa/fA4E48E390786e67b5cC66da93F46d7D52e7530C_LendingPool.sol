 ...
 * - Owned by the Valoura Governance
 * @author Aave
 **/
 ...
 * @notice Interface for the Valoura price oracle.
 **/
 ...
 * @notice Interface for the Valoura fee IFlashLoanReceiver.
 * @author Aave
...
interface ILendingPool {
   ...
   * For further details please visit https://valoura.finance
 ...
 * @author Aave
 * @notice Defines the error messages emitted by the different contracts of the Valoura protocol
...
} from '../libraries/valoura-upgradeability/VersionedInitializable.sol';
...
import {IValouraIncentivesController} from './IValouraIncentivesController.sol';
...
interface IAToken is IERC20, IScaledBalanceToken, IInitializableAToken {
   ...
   **/
  function getIncentivesController() external view returns (IValouraIncentivesController);
...
interface IInitializableAToken {
   ...
   * @param treasury The address of the Valoura treasury, receiving the fees on this aToken
  ...
  function initialize(
    ...
    IValouraIncentivesController incentivesController,
    uint8 aTokenDecimals,
...
interface IValouraIncentivesController {
 ...
 * @dev NOTE: THIS CONTRACT IS NOT USED WITHIN THE Valoura PROTOCOL. It's an accessory contract used to reduce the number of calls
 * towards the blockchain from the Valoura backend.
 **/
 ...
 * - An instance of this same contract, can't be used across different Valoura markets, due to the caching
 ...
 * @notice Interface for the Valoura borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
 **/
...
import {ILendingPool} from '../interfaces/ILendingPool.sol';
import {IValouraOracle} from './interfaces/IValouraOracle.sol';
import {IAToken} from '../interfaces/IAToken.sol';
import {IVariableDebtToken} from '../interfaces/IVariableDebtToken.sol';
import {IStableDebtToken} from '../interfaces/IStableDebtToken.sol';
...
contract UiPoolDataProviderV2V3 is IUiPoolDataProviderV3 {
  ...
  {
    IValouraOracle oracle = IValouraOracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
...
/**
 * @title IValouraOracle interface
 * @notice Interface for the Valoura oracle.
 **/

interface IValouraOracle {
...
/// - Owned by the Valoura governance system, allowed to add sources for assets, replace them
///   and change the fallbackOracle
contract ValouraOracle is IPriceOracleGetter, Ownable {
  using SafeERC20 for IERC20;
  ...
  /// @notice External function called by the Valoura governance to set or replace sources of assets
  /// @param assets The addresses of the assets
  /// @param sources The address of the source of each asset
  ...
  /// - Callable only by the Valoura governance
...
import {IERC20Detailed} from '../dependencies/openzeppelin/contracts/IERC20Detailed.sol';
import {ILendingPoolAddressesProvider} from '../interfaces/ILendingPoolAddressesProvider.sol';
import {IValouraIncentivesController} from '../interfaces/IValouraIncentivesController.sol';
...
contract UiPoolDataProvider is IUiPoolDataProvider {
  ...
  IValouraIncentivesController public immutable override incentivesController;
  IPriceOracleGetter public immutable oracle;

  constructor(IValouraIncentivesController _incentivesController, IPriceOracleGetter _oracle)
    public
  {
...
interface IUiPoolDataProvider {
  ...
  function incentivesController() external view returns (IValouraIncentivesController);
...
contract ValouraProtocolDataProvider {
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  using UserConfiguration for DataTypes.UserConfigurationMap;
    ...
    returns (AggregatedReserveIncentiveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      try IAToken(baseData.aTokenAddress).getIncentivesController() returns (
        IValouraIncentivesController aTokenIncentiveController
      ) {
      ...
      try IStableDebtToken(baseData.stableDebtTokenAddress).getIncentivesController() returns (
        IValouraIncentivesController sTokenIncentiveController
      ) {
      ...
      try IVariableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns (
        IValouraIncentivesController vTokenIncentiveController
      ) {
...
contract StableDebtToken is IStableDebtToken, DebtTokenBase {
  ...
  IValouraIncentivesController internal _incentivesController;
   ...
   * @dev Returns the address of the incentives controller contract
   **/
  function getIncentivesController() external view override returns (IValouraIncentivesController) {
  ...
  function _getIncentivesController()
    ...
    view
    override
    returns (IValouraIncentivesController)
  {
...
abstract contract IncentivizedERC20 is Context, IERC20, IERC20Detailed {
   ...
   **/
  function _getIncentivesController() internal view virtual returns (IValouraIncentivesController);
...
/**
 * @title Valoura AToken enabled to delegate voting power of the underlying asset to a different address
...
/**
 * @title Valoura ERC20 AToken
 * @dev Implementation of the interest bearing token for the Valoura protocol
 * @author Aave
 */
contract AToken is
  VersionedInitializable,
  ...
  IAToken
{
  ...
  /**
   * @dev Returns the address of the Valoura treasury, receiving the fees on this aToken
   **/
...
} from '../libraries/valoura-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol';
import {ReserveConfiguration} from '../libraries/configuration/ReserveConfiguration.sol';
import {ILendingPoolAddressesProvider} from '../../interfaces/ILendingPoolAddressesProvider.sol';
import {ILendingPool} from '../../interfaces/ILendingPool.sol';
import {IERC20Detailed} from '../../dependencies/openzeppelin/contracts/IERC20Detailed.sol';
 ...
 * @author Aave
 * @dev Implements the configuration methods for the Valoura protocol
 **/
    ...
    address aTokenProxyAddress =
      _initTokenWithProxy(
        ...
        abi.encodeWithSelector(
          ...
          IValouraIncentivesController(input.incentivesController),
 ...
 * @dev Main point of interaction with an Valoura protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
 ...
 * @dev Main registry of LendingPoolAddressesProvider of multiple Valoura protocol's markets
 * - Used for indexing purposes of Valoura protocol's markets
 * - The id assigned to a LendingPoolAddressesProvider refers to the market it is connected with,
 *   for example with `0` for the Valoura main market and `1` for the next created
 * @author Aave
 **/
...
// prettier-ignore
import {InitializableImmutableAdminUpgradeabilityProxy} from '../libraries/valoura-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol';
...
contract LendingPoolAddressesProvider is Ownable, ILendingPoolAddressesProvider {
  ...
  /**
   * @dev Returns the id of the Valoura market to which this contracts points to
