...
contract FEGexPRO is LPToken, FMath {
    ...
    address public Main = 0x9C4fc5FAD6BAd0bA3b4F83C96C93726f0F2Ea603;
    ...
    function setMaxBuySellRatio(uint256 sellmax, uint256 buymax) public {
        ...
        require (buymax >= 100e18 && buymax <= tob, "min 100 BNB, max 100% of liquidity");
        ...
        returns(uint256 tokenAmountOut)
    {
        ...
        if(io > 0 && tokenAmountIn >= 2e17 && hold >= 2e19){
        sendRebate();
        }
        ...
        returns(uint256 tokenAmountOut)
    {
        ...
        uint256 hold = IERC20(FEG).balanceOf(address(msg.sender));
        if(address(this).balance > 0 && hold >= 2e19 && tokenAmountOut <= 2e18){
        sendRebate();
        }
