...
contract MagicForest is IERC721Receiver, Pausable, Restricted {
    ...
    uint256 public sessionClaimNumber = 100;
    uint256 public claimingFee = 0.02 ether;
        ...
        _updateEarnings
    {
        ...
        require(msg.value >= tokenIds.length * claimingFee, "You didn't pay tax");
    ...
    function calculateRewards(uint16 tokenId) external view returns (uint256) {
        ...
        if (isRanger) {
            ...
            if (stake.tokenId == tokenId) {
                ...
                return _calculateRangerRewards(level, stake.lastClaim, walletBoost, rangerBoost, lastGlowTransfer, numberStaked, lastWalletAssociation);
            } else {
                return 0;
            }
        } else {
    ...
    function _getMinimumToExit(uint8 unstakeCooldownAugmentation, uint8 globalUnstakeCooldownAugmentation) internal pure returns(uint256) {
    ...
    function _chanceToGetStolen(uint8 unstakeStealAugmentation, uint8 unstakeStealReduction, uint8 globalUnstakeStealAugmentation) internal pure returns(uint8) {
        return 50 + unstakeStealAugmentation - unstakeStealReduction + globalUnstakeStealAugmentation;
    }

    function _isStolen(uint256 tokenId, uint8 unstakeStealAugmentation, uint8 unstakeStealReduction, uint8 globalUnstakeStealAugmentation) internal view returns (bool) {
    ...
    function _updateClaimingSession(uint32 claimingTimestamp) internal {
        if (claimingTimestamp - sessionBegining > 1 hours) {
            if (sessionClaimNumber < 10) {
                claimingFee = 0.00 ether;
            } else if (sessionClaimNumber < 100) {
                claimingFee = 0.01 ether;
            } else if (sessionClaimNumber < 1000) {
                claimingFee = 0.02 ether;
            } else if (sessionClaimNumber < 10000) {
                claimingFee = 0.03 ether;
            } else if (sessionClaimNumber < 100000) {
                claimingFee = 0.04 ether;
            } else {
                claimingFee = 0.05 ether;
            }
            sessionBegining = (claimingTimestamp / 1 hours) * 1 hours;
