...
import "./EggV2.sol";
import "./AuthorizableV2.sol";

import "hardhat/console.sol";

contract ChiknFarm is ERC721Upgradeable, AuthorizeUpgradeable {
    address private EGG_CONTRACT; // egg contract
    // Royalty
    address private _royaltyAdd; // royality receiver
    uint256 public royaltyPer; // royalty based on sales price
    ...
    // map id to farmland obj
    mapping(uint256 => Farmland) public lands;
    ...
    //================================= Events ================================= //
    event SaleToggle(uint256 farmId, bool isForSale, uint256 price);
    event PurchaseEvent(
        uint256 farmId,
        address from,
        address to,
    ...
    event IncreaseSize(uint256 farmId, uint24 size);
    event IncreaseFertility(uint256 farmId, uint24 fertility);

    // Initialize

    function initialize(
        address _royalityAdd,
        uint256 _royaltyPer,
        uint256 _fee,
    ...
    function mint() public {
        ...
        require(openPub == true, "not open");
        require(totalSupply() + 1 <= maxSupply, "max supply");
        // pay for minting cost using egg
        EggV2 egg = EggV2(EGG_CONTRACT);
        uint256 available = egg.balanceOf(msg.sender);
        require(available >= fee, "Not enough egg");
        // burn the egg
        egg.burnEggs(msg.sender, fee);
        _tokenIds.increment();
        uint24 newItemId = uint24(_tokenIds.current());
        _safeMint(msg.sender, newItemId);
        // add the token id to the lands
        lands[newItemId] = Farmland(
            1, // size
            1, // fertility level
            0, // no of transfer
            msg.sender, // mintedBy
            msg.sender, // current owner
            0, // prevPrice
            fee, // current price
            false // for sale
        );
    ...
    function setPriceForSale(
        ...
        Farmland memory land = lands[_tokenId];
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        require(msg.value >= land.price, "Not enough money");
        // token should be for sale
        require(land.forSale);
        land.prevPrice = land.price;
        lands[_tokenId] = land;
        _payRoyalty(ownerOf(_tokenId), msg.sender);
        _transfer(ownerOf(_tokenId), msg.sender, _tokenId);
        emit PurchaseEvent(_tokenId, land.owner, msg.sender, land.price);
    }
    ...
    // share function to pay royalty to artist
    function _payRoyalty(address from, address to) internal {
        if (msg.value > 0) {
            console.log("Transfer From where msg.value > 0", msg.value);
            uint256 royalty = (msg.value * royaltyPer) / 100;
            (bool success1, ) = payable(_royaltyAdd).call{value: royalty}("");
            require(success1);
            (bool success2, ) = payable(from).call{value: msg.value - royalty}(
                ""
            );
            require(success2);
            emit PayRoyalty(from, to, msg.value, royalty);
        }
    }

    // overwrite with transferFrom
    function transferFrom(
        address from,
        address to,
        uint256 tokenId
    ) public payable override(ERC721Upgradeable) {
        require(
    ...
    // overwrite with safeTransferFrom
    ...
    function _beforeTokenTransfer(
        ...
        Farmland memory land = lands[tokenId];
        land.owner = to;
        land.numTrans += 1;
        land.forSale = false;
        lands[tokenId] = land;
    ...
    // farmiing and upgrade function
    // function incSize(uint256 _tokenId) external onlyAuthorized {
    //     Farmland memory land = lands[_tokenId];
    //     land.size += 1;
    //     lands[_tokenId] = land;
    //     emit IncreaseSize(_tokenId, land.size);
    // }

    // function incFertility(uint256 _tokenId) external onlyAuthorized {
    //     Farmland memory land = lands[_tokenId];
    //     land.fertility += 1;
    //     lands[_tokenId] = land;
    //     emit IncreaseFertility(_tokenId, land.fertility);
    // }
    ...
    function airdrop(address[] calldata addrs) external onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) {
            _tokenIds.increment();
            uint24 newItemId = uint24(_tokenIds.current());
            _safeMint(addrs[i], newItemId);
            Farmland memory land = Farmland(
                1, // size
                1, // fertility level
                0, // no of transfer
                address(0), // mintedBy
                addrs[i], // current owner
                0, // prevPrice
                0, // current price
                false // for sale
            );
...
contract AuthorizeUpgradeable is OwnableUpgradeable {

    function __AuthOwnable_init() internal initializer {
        __Ownable_init();
    }
