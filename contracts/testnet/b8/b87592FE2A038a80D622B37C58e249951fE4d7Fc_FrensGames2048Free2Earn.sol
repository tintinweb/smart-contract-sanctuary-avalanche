...
// File: FrensGames/FrensProtocol/Frens.sol
...
contract FrensProtocolClient {
    ...
    ) internal returns (bytes32 requestId) 
    {
        ...
        Frens.Request memory request = buildRequest(Frens.stringToBytes32(_queryId), address(this), _acheviedRequestFunc);
        
        string memory _jsonext = ".json";
        // @notice concatenate the address of the player and the path in the json.
        string memory _api = Frens.concatenate(_urltofetch, _addressPlayer);
        string memory _urlApi = Frens.concatenate(_api, _jsonext);
    ...
    function patchUintRequest(
        ...
        Frens.Request memory request = buildRequest(Frens.stringToBytes32(_queryId), address(this), callbackFunc);
        
        string memory _jsontext = ".json";
        // @notice concatenate the address of the player and the path in the json.
        string memory _api = Frens.concatenate(_urltofetch, _addressPlayer);
        string memory _urlApi = Frens.concatenate(_api, _jsontext);

        string memory _openBrackets = "{\"";
        string memory _resetData = "\":0}";

        string memory _bodytemp = Frens.concatenate(_openBrackets, _pathUint);
        string memory _body = Frens.concatenate(_bodytemp, _resetData);
...
contract FrensGames2048Free2Earn is FrensProtocolClient {
    ...
    */
    function requestPlayerScoreData(
        ...
        address addrPlayer = Frens.toAddress(_addressPlayer);
    ...
    function patchFulfill(bytes32 _requestId, uint256 _score, string calldata _address) external recordAchievedRequest(_requestId) {
        address addrPlayer = Frens.toAddress(_address);
