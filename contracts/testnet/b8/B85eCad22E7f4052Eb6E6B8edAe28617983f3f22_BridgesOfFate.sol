...
contract BridgesOfFate is Ownable  {
    ...
    uint256 private constant TURN_PERIOD = 10800; // 3 hours 
    ...
    function _deletePlayerIDForSpecifyStage(uint256 _stage, bytes32 _playerId) internal {
        _removeForList(_findIndex(_playerId,allStagesData[_stage]));  
    ...
    function getStagesData(uint256 _stage) external view  returns (bytes32[] memory) {
    ...
    function _findIndex(bytes32 _fa,bytes32[] memory _playerList)  internal pure returns(uint){
        uint index;
