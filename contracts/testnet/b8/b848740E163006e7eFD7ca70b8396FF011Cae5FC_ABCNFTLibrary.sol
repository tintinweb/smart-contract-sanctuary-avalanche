...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/utils/Counters.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/token/ERC721/extensions/ERC721Burnable.sol
...
contract ABCNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor() ERC721("ABCNFT Token", "ABCNT") {}
...
// File: contracts/ABCNFTLibrary.sol
    ...
    contract ABCNFTLibrary is ReentrancyGuard, Ownable {
        ...
        // The AVAX needed above the current bid for a new bid to be valid; 0.1 AVAX.
        ...
        uint256 public libraryFeeForAVAX = 70;
        uint256 public libraryFeeForToken = 70;
        uint256 public royaltyFee = 5;
        uint256 public mintPrice = 19;
        ...
        // buys/sells of NFTs through the library, 5% tax goes stakingpool
        address public stakingPool;
        // buys/sells of NFTs through the library, 2% tax goes development address
        ...
        constructor(
            ...
            address abcPaymentAddress_,
            address abcTokenAddress_,
            ...
            payoutAddressMap["ABC"] = abcPaymentAddress_;
            tokenAddressMap["ABC"] = abcTokenAddress_;
        ...
        function openTrade(
            ...
            ABCNFT(nftContract).transferFrom(
                msg.sender,
                address(this),
                _id
            );
        ...
        function closeTrade(uint256 _id) external {
            ...
            ABCNFT(nftContract).transferFrom(address(this), msg.sender, _id);
            listedMap[_id] = false;
        ...
        function giveaway(
            ...
            if (listedMap[_id] == false) {
                ABCNFT(nftContract).transferFrom(msg.sender, _to, _id);
            } else {
                require(ownerMap[_id] == msg.sender, "sender is not owner");
                ABCNFT(nftContract).transferFrom(address(this), _to, _id);
                listedMap[_id] == false;
            }
            ownerMap[_id] = _to;
        ...
        function mint(uint256 number) public payable{
            ...
            for (uint256 i = 0; i < number; i++) {
                ...
                ABCNFT(nftContract).mint(msg.sender);
                _tokenIds.increment();
        ...
        function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
            ...
            require(
                msg.sender != ABCNFT(nftContract).ownerOf(_id),
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("AVAX")))) {
                require(msg.value >= salePrice[_id], "msg.value should be equal to the buyAmount");
                uint256 _commissionValue = salePrice[_id].mul(libraryFeeForAVAX).div(1000);
                uint256 _sellerValue = salePrice[_id].sub(_commissionValue);
                attemptAVAXTransfer(payable(_previousOwner), _sellerValue);
                attemptAVAXTransfer(payable(adminRecoveryAddress), _commissionValue.mul(5).div(7)); // 5% tax goes charity wallet
                attemptAVAXTransfer(payable(developmentAddress), _commissionValue.mul(2).div(7)); // 2% goes development address
            } else {
                ...
                uint256 _commissionValue = salePrice[_id].mul(libraryFeeForToken).div(1000);
                uint256 _sellerValue = salePrice[_id].sub(_commissionValue);
        ...
        {
            require(
                msg.sender == ABCNFT(nftContract).ownerOf(_tokenId),
                "Error, you are not the owner"
            );
            ...
            auctionNotExpired(tokenId)
        {
            ...
            if (
                keccak256(abi.encodePacked((paymentType))) ==
                keccak256(abi.encodePacked(("AVAX")))
            ) {
            ...
            if (auctions[tokenId].amount == 0) {
                ...
                if (
                    ...
                    attemptAVAXTransfer(payable(auctions[tokenId].bidder), auctions[tokenId].amount);
                } else {
                    require(
            ...
            // to see whAVAXer we're near the auctions end and should extend the auction.
            ...
            onlyCreatorOrWinner(tokenId)
        {
            ...
            if (winner == address(0)) {
                ABCNFT(nftContract).transferFrom(address(this), Creator, tokenId);
                ownerMap[tokenId] = Creator;
            } else {
                ABCNFT(nftContract).transferFrom(address(this), winner, tokenId);
                if (
                    keccak256(abi.encodePacked((paymentType))) ==
                    keccak256(abi.encodePacked(("AVAX")))
                ) {
                    uint256 _commissionValue = amount.mul(libraryFeeForAVAX).div(
                        1000
                    );
                    attemptAVAXTransfer(payable(Creator), amount.sub(_commissionValue));
                    attemptAVAXTransfer(payable(adminRecoveryAddress), _commissionValue.mul(5).div(7));
                    attemptAVAXTransfer(payable(developmentAddress), _commissionValue.mul(2).div(7));
                } else {
                    uint256 _commissionValue = amount.mul(libraryFeeForToken).div(
                        1000
                    );
                    require(
                        IERC20(tokenAddressMap[paymentType]).transfer(
        ...
        // Allows the admin to transfer any AVAX from this contract to the recovery address.
            ...
            returns (bool success)
        {
            ...
            success = attemptAVAXTransfer(payable(adminRecoveryAddress), amount);
        }
        ...
        // Sending AVAX is not guaranteed complete, and the mAVAXod used here will return false if
        // it fails. For example, a contract can block AVAX transfer, or might use
        ...
        function attemptAVAXTransfer(address payable to, uint256 value)
            private
            returns (bool)
        {
            // Here increase the gas limit a reasonable amount above the default, and try
            // to send AVAX to the recipient.
        ...
        function setLibraryFeeForAVAX(uint256 _newLibraryFeeForAVAX)
            external
            onlyOwner
        {
            require(_newLibraryFeeForAVAX > 1, "Invalid LibraryFee For AVAX");
            libraryFeeForAVAX = _newLibraryFeeForAVAX;
        }

        function setLibraryFeeForToken(uint256 _newLibraryFeeForToken)
            external
            onlyOwner
        {
            require(_newLibraryFeeForToken > 1, "Invalid LibraryFee For Token");
            libraryFeeForToken = _newLibraryFeeForToken;
        }
