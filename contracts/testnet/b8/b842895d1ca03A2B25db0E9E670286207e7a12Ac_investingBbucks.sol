...
contract investingBbucks is Ownable, Authorizable, ReentrancyGuard, ERC1155{
    ...
    Biz public STAKING_BBBS_CONTRACT;
    ...
    constructor(
    ...
    ) ERC1155("ipfs://bafybeidt7huapeubdc4bmcptrdtmue7jjwk7fojossj2amnoalx6ptptj4/{id}"){
        ...
        STAKING_BBBS_CONTRACT = Biz(stakingBadbizbonesContract);
    }

    function mintFactory(uint256 amount, uint256 factoryLevel) public{
        require(factoryLevel > 1 && factoryLevel < 6, "invalid level");
        //BBUCKS_CONTRACT.transferFrom(msg.sender, address(0), amount * factoryLevel*2 * FACTORY_PRICE);
        _mint(msg.sender, factoryLevel,amount,"");
    }
    function rankUpFactory(uint256 factoryLevel) public{
        require(factoryLevel > 1 && factoryLevel < 5, "invalid level or already max level");
        //BBUCKS_CONTRACT.transferFrom(msg.sender, address(0), (factoryLevel+1)*2 * FACTORY_PRICE);
        _burn(msg.sender, factoryLevel, 1);
        _mint(msg.sender, factoryLevel+1,1,"");
    }
...
contract Biz is Ownable, Pausable, ReentrancyGuard {
    //reminder
    // uint8 (0 - 255)
    // uint16 (0 - 65535)
    // uint24 (0 - 16,777,216)
