...
import "./interface/IProfileVault.sol";
import "./interface/IGameFiShops.sol";
import "./interface/tokens/IGameFiTokenERC20.sol";
import "./interface/tokens/IGameFiTokenERC721.sol";
import "./interface/tokens/IGameFiTokenERC1155.sol";

// TODO сделать паузу
// TODO сделать съем денег пропертями

/**
 * @dev Main GameFi infrastructure contract
 */
contract GameFiCore is
    Initializable,
    ...
    IGameFiCore
{
    ...
    modifier onlyAdminOrOperator() {
        require(
            ...
            "GameFiCore: caller is not the admin/operator"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(profileVaultImpl_ != address(0), "GameFiCore: zero address of profile implementation");

        __AccessControlEnumerable_init();
        __ReentrancyGuard_init();
        __ProfileRegistry_init(string(abi.encodePacked(name_, " profile")), symbol_, baseURI_);
    ...
    // function withdrawToken(Token memory token, TokenStandart standart) external override onlyAdmin returns (bool success) {
    //     _tokenTransferFrom(token, standart, address(this), msg.sender);

    //     emit WithdrawToken({sender: msg.sender, token: token, standart: standart, timestamp: block.timestamp});
    ...
    function isMinter(address target) external view override returns (bool) {
        return hasRole(MINTER_ROLE, target);
    }
    ...
    function setProfileVaultImpl(address profileVaultImpl_) external override onlyAdmin returns (bool success) {
        require(AddressUpgradeable.isContract(profileVaultImpl_), "GameFiCore: impl must be a contract");

        _profileVaultImpl = profileVaultImpl_;

        emit SetProfileVaultImpl({
            sender: _msgSender(),
            profileVaultImpl: profileVaultImpl_,
        ...
        returns (uint256 profileId, address profileVault)
    {
        ...
        IProfileVault(newProfileVault).initialize(address(this));
    ...
    function unlockProfile(uint256 profileId) external override onlyAdminOrOperator returns (bool success) {
        _checkProfileById(profileId);
        _checkIsNotLocked(profileId);
    ...
    function profileVaultToId(address profileAddress) external view override returns (uint256) {
        ...
        _checkProfileById(profileId);
        require(_profileIdToVault[profileId] == profileAddress, "GameFiCore: nonexistent profile");
        return profileId;
    }
    ...
    function editProperty(uint256 propertyId, Property memory propertySettings) external returns (bool success) {
    ...
    function setPropertyValue(
        ...
        if (targetProperty.accessType == PropertyAccessType.OPERATOR_ONLY) {
            ...
            _checkRole(
                OPERATOR_ROLE,
                ECDSAUpgradeable.recover(keccak256(abi.encodePacked(profileId, propertyId, newValue)), signature)
            );
        }
    ...
    function createCollection(
        CollectionSettings memory collectionSettings,
        uint256 salt,
        string memory name_,
        string memory symbol_,
        ...
        require(collectionSettings.implementation != address(0), "Gamecore: zero implementation address");
        uint256 id = _totalCollections.current();
        address collectionInstance = ClonesUpgradeable.cloneDeterministic(
            collectionSettings.implementation,
            bytes32(salt)
        );

        _collection[id] = Collection({settings: collectionSettings, tokenContract: collectionInstance});

        _totalCollections.increment();

        if (collectionSettings.tokenStandart == TokenStandart.ERC20) {
            IGameFiTokenERC20(collectionInstance).initialize(name_, symbol_, contractUri, data);
            _checkSupportsInterface(collectionInstance, type(IGameFiTokenERC20).interfaceId);
        } else if (collectionSettings.tokenStandart == TokenStandart.ERC721) {
            IGameFiTokenERC721(collectionInstance).initialize(name_, symbol_, tokenUri, contractUri, data);
            _checkSupportsInterface(collectionInstance, type(IGameFiTokenERC721).interfaceId);
        } else {
            IGameFiTokenERC1155(collectionInstance).initialize(name_, symbol_, tokenUri, contractUri, data);
    ...
    function setCollectionActivity(uint256 collectionId, bool isActive) external returns (bool success) {
        _checkCollection(collectionId);

        emit SetCollectionActivity({
            sender: _msgSender(),
            collectionId: collectionId,
            oldStatus: _collection[collectionId].settings.isActive,
            newStatus: isActive,
            timestamp: block.timestamp
        });

        _collection[collectionId].settings.isActive = isActive;

        return true;
    }

    function callToCollection(uint256 collectionId, bytes memory data)
        external
        override
        onlyAdmin
        nonReentrant
    ...
    function mintTokenToProfile(
        uint256 profileId,
        CollectionToken memory token,
        bytes memory data
    ) external override onlyAdminOrOperator nonReentrant returns (bool success) {
        _checkProfileById(profileId);
        _checkIsLocked(profileId);

        address profileAddress = _profileIdToVault[profileId];

        _mintToken(profileAddress, token, data);
    ...
    function mintTokenToWallet(
        ...
        CollectionToken memory token,
        bytes memory data
    ) external override canMint nonReentrant returns (bool success) {
        require(targetWallet != address(0), "GameFiCore: zero wallet address");

        _mintToken(targetWallet, token, data);
    ...
    function burnTokenFromProfile(
        ...
        _checkProfileById(profileId);
        _checkIsLocked(profileId);
        _checkCollection(token.collectionId);

        Collection memory targetCollection = _collection[token.collectionId];

        address profileAddress = _profileIdToVault[profileId];
        if (targetCollection.settings.tokenStandart == TokenStandart.ERC20) {
            require(token.tokenId == 0, "Gamecore: only for zero token id");
            require(token.amount > 0, "Gamecore: zero erc20 token amount");
            IGameFiTokenERC20(targetCollection.tokenContract).burn(profileAddress, token.amount, data);
        } else if (targetCollection.settings.tokenStandart == TokenStandart.ERC721) {
            require(token.amount == 1, "Gamecore: wrong erc721 token amount");
            require(
                IERC721Upgradeable(targetCollection.tokenContract).ownerOf(token.tokenId) ==
                    _profileIdToVault[profileId],
                "Gamecore: wrong token owner"
            );
            IGameFiTokenERC721(targetCollection.tokenContract).burn(token.tokenId, data);
        } else {
            require(token.amount > 0, "Gamecore: zero erc1155 token amount");
            IGameFiTokenERC1155(targetCollection.tokenContract).burn(profileAddress, token.tokenId, token.amount, data);
        }
    ...
    function _mintToken(
        ...
        if (targetCollection.settings.tokenStandart == TokenStandart.ERC20) {
            require(token.tokenId == 0, "Gamecore: only for zero token id");
            require(token.amount > 0, "Gamecore: zero erc20 token amount");
            IGameFiTokenERC20(targetCollection.tokenContract).mint(target, token.amount, data);
        } else if (targetCollection.settings.tokenStandart == TokenStandart.ERC721) {
            require(token.tokenId == 0, "Gamecore: only for zero token id");
            require(token.amount == 1, "Gamecore: wrong token amount");
            IGameFiTokenERC721(targetCollection.tokenContract).mint(target, data);
        } else {
            require(token.amount > 0, "Gamecore: zero erc1155 token amount");
            IGameFiTokenERC1155(targetCollection.tokenContract).mint(target, token.tokenId, token.amount, data);
        }
    }
    ...
    function _checkProperty(uint256 propertyId) internal view {
        require(propertyId < _totalProperties.current(), "GameFiCore: nonexistent property");
    }
    ...
    function _checkOwnerOf(address target, uint256 profileId) internal view {
        require(target == ownerOf(profileId), "GameFiCore: only for token owner");
    }

    function _checkSupportsInterface(address target, bytes4 interfaceId) internal view {
        require(
            IERC165Upgradeable(target).supportsInterface(interfaceId),
            "GameFiCore: contract does not support this interface"
        );
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ProfileRegistry) {
...
abstract contract ProfileRegistry is
    Initializable,
    ERC721Upgradeable,
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function __ProfileRegistry_init(
        string memory name_,
        string memory symbol_,
        string memory baseURI_
...
import "../interface/types/ICustomTypes.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";

abstract contract TransferHelper is ICustomTypes {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event WithdrawToken(address indexed sender, Token token, TokenStandart standart, uint256 timestamp);

    function withdrawToken(Token memory token, TokenStandart standart) external virtual returns (bool success);
    ...
    function _tokenTransferFrom(
        ...
        if (standart == TokenStandart.ERC20) {
            require(token.tokenId == 0, "TokenTransfer: only for zero token id");
            require(token.amount > 0, "TokenTransfer: zero erc20 token amount");
            IERC20Upgradeable(token.tokenContract).safeTransferFrom(from, to, token.amount);
        } else if (standart == TokenStandart.ERC721) {
            require(token.tokenId == 0, "TokenTransfer: only for zero token id");
            require(token.amount == 1, "TokenTransfer: wrong token amount");
...
import "./gameficore/IProfile.sol";
import "./gameficore/IProperty.sol";
import "./gameficore/ICollection.sol";
import "./gameficore/IToken.sol";

interface IGameFiCore is IProfile, IProperty, ICollection, IToken {
    function initialize(
        string memory name_,
        string memory symbol_,
        string memory baseURI_,
...
import "./types/ICustomTypes.sol";

interface IGameFiShops is ICustomTypes {
    enum ShopStatus {
        NULL,
        OPEN,
        CLOSED
    ...
    function editShop(uint256 shopId, Shop memory shop) external returns (bool success);

    function buyToken(uint256 shopId) external returns (bool success);
...
interface IGameFiTokenERC721 is IERC721Upgradeable {
    function initialize(
        string memory name_,
        string memory symbol_,
        string memory contractUri_,
    ...
    function mint(address to, bytes memory data) external returns (bool);
...
interface IProfile {
    ...
    function setProfileVaultImpl(address profileVaultImpl_) external returns (bool success);
...
interface IProperty is ICustomTypes {
    ...
    struct Property {
        ...
        PropertyChangeFeeType changeFeeType;
        bool isActive;
        string metadataURI;
    }
...
interface ICollection is ICustomTypes {
    ...
    function callToCollection(uint256 collectionId, bytes memory data) external returns (bytes memory result);
...
interface IToken is ICustomTypes {
