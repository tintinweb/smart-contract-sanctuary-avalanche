...
contract NeFiProtoStarDrops is Ownable{
	...
	uint256 public AVAXprice = 1*(10**17);
	address oldDrop;
	address Guard;
	...
	constructor(address[] memory addresses){
		...
		feeMGR = feeManager(addresses[1]);
		
		nebuMath = nebulaMath(addresses[2]);
	...
	function sendFeeTokenSpec(uint256 _intervals, uint256 _x) payable external{
		...
		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		...
    		uint256 _intervals = nebuLib.getLower(getTotalPayableSpec(_account,_x),_intervals);
    	...
    	function payFeeAvaxSpec(uint256 _intervals,uint _x) payable external{
    		...
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(_intervals,msg.value,AVAXprice);
		//verification
	...
	function protoFeesAvaxSpec(uint256 _intervals,uint256 _x) payable external{
		...
		require(_intervals >0, "Doesnt Look Like you opted to pay any fees at this time");
		 ...
		 //calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(nebuLib.getLower(getTotalPayable(_account),_intervals),msg.value,AVAXprice);
		//verification
		require(_nIntervals != 0 && needed > 0,"you do not hold enough to cover this claim");
		require(_nIntervals <= _intervals,"you do not hold enough to cover this claim");
	        //bookKeeping
	...
	function updateManagers(address newVal) external onlyOwner() {
    	...
    	function changeAvaxPrice(uint256 _value) external onlyOwner(){
    		AVAXprice = _value;
    	}
