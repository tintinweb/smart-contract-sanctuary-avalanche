...
contract FarmsV2 is ReentrancyGuard, Ownable, Pausable {
  ...
  struct Position {
    ...
    uint256 rewardsDebt;
    // Variable for tracking rewards accrued by position but not yet claimed
  ...
  */
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0 || position.rewardsAccrued > 0) {
      Farm storage farm = farms[_id];
      ...
      position.rewardsDebt = position.stakedAmount * currentRewardPerStakedToken(_id)
                           * getRewardMultiplier(_id, msg.sender)
                           / SAFE_MULTIPLIER;
      position.rewardsAccrued = position.rewardsAccrued + rewards;

      if (farm.totalRewards >= position.rewardsAccrued) {
        if (farm.esSteadySplit > 0) {
          uint256 esSteadyAmount = position.rewardsAccrued * farm.esSteadySplit / SAFE_MULTIPLIER;
          uint256 steadyAmount = position.rewardsAccrued - esSteadyAmount;

          IERC20(steady).safeTransfer(msg.sender, steadyAmount);
          ITokenManager(tokenManager).convertTo(esSteadyAmount, msg.sender);
          } else {
          IERC20(steady).safeTransfer(msg.sender, position.rewardsAccrued);
        }

        position.rewardsRedeemed = position.rewardsRedeemed + position.rewardsAccrued;
        farm.totalRewards -= position.rewardsAccrued;
  ...
  */
  function updateMaxMultiplier(uint256 _id, uint256 _maxMultiplier) external onlyOwner {
    require(_maxMultiplier >= SAFE_MULTIPLIER, "Reward split greater than maximum");
