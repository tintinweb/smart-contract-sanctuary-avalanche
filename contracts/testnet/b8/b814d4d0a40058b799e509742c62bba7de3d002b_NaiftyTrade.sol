...
contract NaiftyTrade is Initializable, OwnableUpgradeable {
    ...
    function _saleToken(
        ...
        address create;
        if(naifty721    ==  _conAddr || naifty1155    ==  _conAddr){
        if (ids[3] == 721) {
    ...
    function SetRoyalty(uint96 _royPer) public onlyOwner{
        NaiftyCollection721(naifty721).setRoyaltyPercentage(_royPer);
        NaiftyCollection1155(naifty1155).setRoyaltyPercentage(_royPer);
    }
