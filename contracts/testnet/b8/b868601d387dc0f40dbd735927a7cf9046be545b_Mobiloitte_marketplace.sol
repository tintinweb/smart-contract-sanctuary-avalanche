...
contract Mobiloitte_marketplace is ReentrancyGuard {
    ...
    function depositEscrow(address seller) public payable {
        beneficiary = seller;
        funder = msg.sender;
    }

     
  function withdrawalEscrow( ) public payable {
   require(address(this).balance != 0,"0 Escrow Balance");
 ...
 function getBalanceEscrow() public view returns (uint) {
    ...
    function purchaseItem(uint _itemId) external payable  nonReentrant{
        ...
        require( item.price != 1 ether, "not enough ether to cover item price ");
        require(!item.sold, "item already sold");
        
        depositEscrow(item.seller);
