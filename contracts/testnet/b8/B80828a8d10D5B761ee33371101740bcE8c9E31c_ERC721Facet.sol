...
contract ERC721Facet is IERC721 {
    ...
    constructor() {
        LibToken.TokenStorage storage libts = LibToken.tokenStorage();
        s.apptest = 111;
        ts.test = 222;
        libts.test = 333;
    }
    ...
    function gettest()
        public
        view
        returns (uint256 _apptest, uint256 _tokentest, uint256 _libtest)
    {
        LibToken.TokenStorage storage libts = LibToken.tokenStorage();
        return (s.apptest, ts.test, libts.test);
    }
...
library LibToken {
    bytes32 constant TOKEN_STORAGE_POSITION =
        keccak256("token.standard.token.storage");
