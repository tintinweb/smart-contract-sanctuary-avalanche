...
import './interfaces/IM8Controller.sol';
import './interfaces/IRC20.sol';
import "./router/IUniswapV2Factory.sol";
import "./router/IUniswapV2Router02.sol";
import "../contracts-upgradable/lzApp/NonblockingLzAppUpgradeable.sol";
import "hardhat/console.sol";


contract MXControllerV4 is Initializable, NonblockingLzAppUpgradeable, Proxied, IM8Controller {
    ...
    address public m8Address;
    address public rewardToken;
    uint256 public totalFees;
    address public feesWalletAddress;

    uint256 public maxAmountPerBuy;
    ...
    function initialize(address _lzEndpoint, address _m8Address, address _rewardToken, uint8 _totalFees, address _feesWalletAddress) public initializer {
        __MXControllerUpgradeable_init(_lzEndpoint);

        inFee = false;
        m8Address = _m8Address;
        maxAmountPerBuy = IERC20(_m8Address).totalSupply().div(100); // 1%
    ...
    function __MXControllerUpgradeable_init(address _lzEndpoint) internal onlyInitializing {
        __Ownable_init();
        __NonblockingLzAppUpgradeable_init(_lzEndpoint);
        __MXControllerUpgradeable_init_unchained(_lzEndpoint);
    }

    function __MXControllerUpgradeable_init_unchained(address _lzEndpoint) internal onlyInitializing {}

    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory) internal override {}

    function setUniswapV2Router(address routerAddress) external payable onlyOwner {
        console.log("=> ", routerAddress);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
        console.log("==>", _uniswapV2Router.WETH());
    ...
    function setRewardToken(address value) external onlyOwner {
        rewardToken = value;
    }

    function setTotalFees(uint256 value) external onlyOwner {
        totalFees = value;
    }

    function setFeesWallet(address payable wallet) external onlyOwner {
        feesWalletAddress = wallet;
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "M8Controller: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;

        emit SetAutomatedMarketMakerPair(pair, value);
    }

    function setMaxAmountPerBuy(uint256 _maxAmountPerBuy) external onlyOwner {
        maxAmountPerBuy = _maxAmountPerBuy;
    }

    function transferFees(address from, address to, uint256 amount) external override returns (uint256 newAmount) {
        console.log('from', from);
        console.log('to', to);
        console.log('amount', amount);
        console.log('=========');
        require(from != address(0), "M8Controller: transfer from the zero address");
        require(to != address(0), "M8Controler: transfer to the zero address");

        bool isPairTx = automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from];

        require(
            !isPairTx || // just a simple wallet to wallet transaction
            from == owner() ||
            to == owner() ||
            !automatedMarketMakerPairs[from] ||
            (amount <= maxAmountPerBuy),
            "M8Controller: amount larger than allowed"
        );
        ...
        safeTransfer(m8Address, feesWalletAddress, _feesAmount);
        ...
        )
    {
        require(
            amount > 0,
            'M8Controller::estimateTransferFees no amount set'
        );

       bool isPairTx = automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from];

        // if pair is not involved or if from/to is neither contract nor owner, don't take fees
        if (
            !isPairTx ||
            from == address(this) ||
            to == address(this) ||
        ...
        _feeAmount = amount.mul(totalFees).div(100);

        newAmount = amount - _feeAmount;

        return (newAmount, _feeAmount);
    }
    ...
    function safeTransfer(
        ...
        require(
            ...
            'M8Controller::safeTransfer TRANSFER_FAILED'
        );
    }
}
