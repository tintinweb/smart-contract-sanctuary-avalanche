...
contract Manager is ReentrancyGuard, Pausable, AccessControl {
  ...
  function equityInfo(address _vault) public view returns (uint256) {
    ...
    uint256 totalEquityValue = (tokenAAssetValue + tokenBAssetValue) -
  ...
  function work(
    ...
    if (currentAssetValue(_vault) > _targetAssetValue) {
      ...
      if (lpWithdrawAmount > 0) {
        ...
        if (lpWithdrawAmount > vaults[_vault].lpTokenAmount) {
          lpWithdrawAmount = vaults[_vault].lpTokenAmount;
        }
    ...
    else if (currentAssetValue(_vault) < _targetAssetValue) {
