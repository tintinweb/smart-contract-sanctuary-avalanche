...
contract BridgesOfFate is Ownable {
    ...
    uint256 private constant TURN_PERIOD = 420; // 5 minutes // 3 hours 10800
    ...
    // function pop(bytes32 playerIds) public  {}
    
    // function reaminCurrentTileCapacit(uint256 jumpAtStage,bool _jumpSides) public  returns(bytes32[] memory) {
    //     bytes32[] memory _playerIDAtSpecifyStage  =  getStagesData(jumpAtStage);
    //     //Get All Player specify Successfull side .

    //     if(_playerIDAtSpecifyStage.length > 0){
    //         for (uint i = 0; i < _playerIDAtSpecifyStage.length; i++) {
    //             if(OverFlowPlayerStatus[_playerIDAtSpecifyStage[i]] == true && _checkSide(PlayerItem[_playerIDAtSpecifyStage[i]].stage,_jumpSides))
    //             {
    //                 titleCapacityPlayerId.push(_playerIDAtSpecifyStage[i]);
    //             }
    //         }
    //     }

    //     return titleCapacityPlayerId;
     
    // }
    event _overFlowCapacityBefore(uint256 _overflow1);
    event _overFlowCapacityAfer(uint256 _overflow1);
    ...
    function _overFlow(bytes32 _playerId,uint256 jumpAtStage,bool _jumpSide) internal returns(bool)  {
        ...
        // emit _overFlowCapacityBefore(totalNFTOnTile[jumpAtStage][_jumpSide] );
        if(totalNFTOnTile[jumpAtStage][_jumpSide] > 0 ){
            totalNFTOnTile[jumpAtStage][_jumpSide] = totalNFTOnTile[jumpAtStage][_jumpSide] - 1;
        }
        // emit  _overFlowCapacityAfer(totalNFTOnTile[jumpAtStage][_jumpSide] );
        bool isSafeSide;
    ...
    event SelectSuccessSidePlayerIdEvent(bytes32[] _successSide);
    event SelectUnholdPlayerIdEvent(bytes32[] _successSideunhold);
    event StableBackAfterJumpedOfPlayer(bytes32 StableBackNfts);
    event setCapacityOfPreviousStageGreatertThenThree(uint256 _setCapacityOfPreviousStageGreatertThenThree);
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if(OverFlowPlayerStatus[playerId]){
             bytes32[] memory _selectSuccessSidePlayerIdList; 
            if(_currentUserStage  > 0){
                bytes32[] memory PlayerIDAtSpecifyStage  =  getStagesData(_currentUserStage - 1);
                ...
                if(totalNFTOnTile[_currentUserStage - 1][previousJumpSide] > 0){
                   ...
                   _selectSuccessSidePlayerIdList =  _selectSuccessSidePlayerId(PlayerIDAtSpecifyStage,previousJumpSide,currentDay);
                    ...
                    if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage -1]){
                        for (uint256 j = 0; j < _selectSuccessSidePlayerIdList.length; j++) {
                            if(PlayerItem[_selectSuccessSidePlayerIdList[j]].ishold == false){
                                // if(isExistForRandomSide(SelectUnholdPlayerId,_selectSuccessSidePlayerIdList[j])){
                                //     _setCapacity =  _setCapacity + 1;
                        ...
                        emit SelectUnholdPlayerIdEvent(SelectUnholdPlayerId);
                        playerIndex = generateRandomTicketNumbers(SelectUnholdPlayerId.length - 1,SelectUnholdPlayerId.length - 1,TitleCapacityAgainestStage[_currentUserStage -1]);
                    }else{
                        for (uint256 q = 0; q < _selectSuccessSidePlayerIdList.length; q++) {
                            OverFlowPlayerStatus[_selectSuccessSidePlayerIdList[q]] =  true;
                        }
                        //Remaing Capacity of the Tile

                       if(TitleCapacityAgainestStage[_currentUserStage -1] > 0){
                            totalNFTOnTile[_currentUserStage - 1][previousJumpSide] = TitleCapacityAgainestStage[_currentUserStage -1] - _selectSuccessSidePlayerIdList.length;
                        }
                ...
                uint256 _setCapacity =  0;
                
                if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage -1]){
                    for (uint256 k = 0; k < playerIndex.length; k++) {
                        if(playerIndex[k] > 0){
                            if(isExistForRandomSide(_uniqueRN,playerIndex[k])){
                                _setCapacity =  _setCapacity + 1;
                                _uniqueRN.push(playerIndex[k]);
                            }
                            OverFlowPlayerStatus[SelectUnholdPlayerId[playerIndex[k]]] =  true;
                            emit StableBackAfterJumpedOfPlayer(SelectUnholdPlayerId[playerIndex[k]]);
                        }
                    }

                    if(TitleCapacityAgainestStage[_currentUserStage -1]  > 0 && _setCapacity > 0){
                        totalNFTOnTile[_currentUserStage - 1][previousJumpSide] = TitleCapacityAgainestStage[_currentUserStage -1] - _setCapacity;
                    }else{
                        totalNFTOnTile[_currentUserStage - 1][previousJumpSide] = totalNFTOnTile[_currentUserStage - 1][previousJumpSide];
                    }
                    delete _uniqueRN;
                    emit setCapacityOfPreviousStageGreatertThenThree(_setCapacity);
                }
            }
        }
    ...
    function LateBuyInFee(bytes32  _playerId,uint256 _nftId, uint8 seriesType) external GameEndRules NarrowingGameEndRules{
        ...
        || ((OverFlowPlayerStatus[playerId] == true && _checkSide(_member.stage, _member.lastJumpSide) == false))){
            ...
            emit comment1(totalNFTOnTile[_member.stage][_member.lastJumpSide]);
            ...
            if(totalNFTOnTile[_member.stage][_member.lastJumpSide] > 0){
                ...
                emit comment2(totalNFTOnTile[_member.stage][_member.lastJumpSide]);
                OverFlowPlayerStatus[playerId] =  true;
            }else{
                OverFlowPlayerStatus[playerId] =  false;
            }
            emit comment3(OverFlowPlayerStatus[playerId]);
    ...
    event comment1(uint256 _val);
    event comment2(uint256 _val);
    event comment3(bool _val);
    ...
    event SetCapacityOfPreviousStage(uint256 lastJumpStage);
    
    function bulkParticipate(bool _jumpSides, bytes32[] memory playerIds) internal  {

        GameStatus storage  _gameStatus = GameStatusInitialized[latestGameId];        
        uint256 _globalStageAt  = _gameStatus.stageNumber + 1;
     
        if ((TitleCapacityAgainestStage[PlayerItem[playerIds[0]].stage + 1] <= 0)) {
            // uint256 _titleCapacity =  _randomFortileCapacity(11 - _globalStageAt);
            if((11 - _globalStageAt) > 1){
                // totalNFTOnTile[_globalStageAt][true]  =   _randomFortileCapacity(11 - _globalStageAt);
                totalNFTOnTile[_globalStageAt][true]  =   3;
                // totalNFTOnTile[_globalStageAt][false] =   _randomFortileCapacity(11 - _globalStageAt);
                totalNFTOnTile[_globalStageAt][false] =   3;
                TitleCapacityAgainestStage[_globalStageAt] = 3;
            }else{

                totalNFTOnTile[_globalStageAt][true]  =  3;// 5;
                totalNFTOnTile[_globalStageAt][false] =  3;// 5;
                TitleCapacityAgainestStage[_globalStageAt] = 3; //5
            }
        }
        ...
        if(titleCapacity < playerIds.length){
            ...
            if(titleCapacity > 0){
                ...
                if(PlayerCapacity.length > 0){
                    ...
                    _range = TitleCapacityAgainestStage[PlayerItem[playerIds[0]].stage];
                }
            }
            _randomSelectPlayerIndexNextStage = generateRandomTicketNumbers(playerIds.length - 1,playerIds.length - 1,_range);

            for (uint256 i = 0; i < playerIds.length; i++) {

                GameItem storage _member = PlayerItem[playerIds[i]];
                currentDay = _dayDifferance(block.timestamp,_gameStatus.startAt);
                // require(_member.ishold == false, "Player in hold position.");
                require(_member.userWalletAddress == msg.sender,"Only Player Trigger");
                require(_member.feeStatus == true, "Please Pay Entry Fee.");
                if (_member.startAt == 0 && _member.lastJumpTime == 0) {
                ...
                // require(OverFlowPlayerStatus[playerIds[i]] == true,"Drop down due to overflow.");   // change Issue uncommetn
                
                _currentUserStage = _member.stage + 1;
                if (GameMap[_currentUserStage] <= 0) {
                    uint256 _globalStageNumber  = _gameStatus.stageNumber + 1;
                    // uint256 _titleCapacity =  _randomFortileCapacity(11 - _globalStageNumber);
                    if((11 > _globalStageNumber ) && (11 - _globalStageNumber) > 1){
                ...
                // _overFlow(playerIds[i],_currentUserStage,_jumpSides);
        ...
        emit _overFlowCapacityBefore(totalNFTOnTile[_currentUserStage][_jumpSides] );
        // if(totalNFTOnTile[_currentUserStage][_jumpSides] > 0 ){
        //     totalNFTOnTile[_currentUserStage][_jumpSides] = totalNFTOnTile[_currentUserStage][_jumpSides] - 1;
        // }
        emit  _overFlowCapacityAfer(totalNFTOnTile[_currentUserStage][_jumpSides] );
        ...
        if(_currentUserStage >= 2  && TitleCapacityAgainestStage[_currentUserStage - 1] >= totalNFTOnTile[_currentUserStage - 1][isSafeSide]){
            totalNFTOnTile[_currentUserStage - 1][isSafeSide] = totalNFTOnTile[_currentUserStage - 1][isSafeSide] + 1;
                ...
                */
                ...
                if(OverFlowPlayerStatus[playerIds[i]]){
                    bytes32[] memory _selectSuccessSidePlayerIdList; 
                    if(_currentUserStage  > 0){
                        bytes32[] memory PlayerIDAtSpecifyStage  =  getStagesData(_currentUserStage - 1);
                        // Set (3) into the variable
                        ...
                        if(totalNFTOnTile[_currentUserStage - 1][previousJumpSide] > 0){
                            ...
                            if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage -1]){
                                ...
                                _randomSelectPlayerIndexPreviousStage = generateRandomTicketNumbers(SelectUnholdPlayerId.length - 1,SelectUnholdPlayerId.length - 1,TitleCapacityAgainestStage[_currentUserStage -1]);
                            }else{
                                for (uint256 q = 0; q < _selectSuccessSidePlayerIdList.length; q++) {
                                    OverFlowPlayerStatus[_selectSuccessSidePlayerIdList[q]] =  true;
                                }
                        ...
                        if(_selectSuccessSidePlayerIdList.length > TitleCapacityAgainestStage[_currentUserStage -1]){
                            for (uint256 k = 0; k < _randomSelectPlayerIndexPreviousStage.length; k++) {
                                if(_randomSelectPlayerIndexPreviousStage[k] > 0){
                                    ...
                                    emit StableBackAfterJumpedOfPlayer(SelectUnholdPlayerId[_randomSelectPlayerIndexPreviousStage[k]]);
                                }
                            }

                            if(TitleCapacityAgainestStage[_currentUserStage -1]  > 0 && _setCapacityPreviouseStage > 0){
                                totalNFTOnTile[_currentUserStage - 1][previousJumpSide] = TitleCapacityAgainestStage[_currentUserStage -1] - _setCapacityPreviouseStage;
                                emit SetCapacityOfPreviousStage(_setCapacityPreviouseStage);
                                emit setCapacityOfPreviousStageGreatertThenThree(_setCapacityPreviouseStage);
                            }else{
                                totalNFTOnTile[_currentUserStage - 1][previousJumpSide] = totalNFTOnTile[_currentUserStage - 1][previousJumpSide];
                            }
                            delete _uniqueRN;
                ...
                */
                ...
                emit BulkParticipateOfPlayerInGame(playerIds[i],_currentUserStage);
    
            }


            uint256 _setCapacityofNextStage =  0;


            if(titleCapacity < playerIds.length){
                for (uint256 i = 0; i < _randomSelectPlayerIndexNextStage.length; i++) {
                    if(_randomSelectPlayerIndexNextStage[i] > 0){
                        if(isExistForRandomSide(_uniqueRN,_randomSelectPlayerIndexNextStage[i])){
                            _setCapacityofNextStage =  _setCapacityofNextStage + 1;
                            _uniqueRN.push(_randomSelectPlayerIndexNextStage[i]);
                            OverFlowPlayerStatus[playerIds[_randomSelectPlayerIndexNextStage[i]]] =  true;
                ...
                //set the Tile Capacity of the Next Or Jumped Stages 

                if( totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides] > 0){
                    emit SetBulkCapacityofNextStage(_setCapacityofNextStage);
                    totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides] = totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides]  - _setCapacityofNextStage;
                }else{
                    if(TitleCapacityAgainestStage[PlayerItem[playerIds[0]].stage]  > 0  && _setCapacityofNextStage > 0){
                        uint256 _remainCapacity = _setCapacityofNextStage + totalNFTOnTile[_currentUserStage][_jumpSides];
                        emit SetBulkCapacityofNextStage1(_remainCapacity);
                        emit SetBulkCapacityofNextStage2( totalNFTOnTile[_currentUserStage][_jumpSides]);
                        // uint256 _remainCapacity = _setCapacityofNextStage + totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides];
                        totalNFTOnTile[_currentUserStage][_jumpSides] = TitleCapacityAgainestStage[_currentUserStage] - _remainCapacity;
                        // totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides] = totalNFTOnTile[PlayerItem[playerIds[0]].stage][_jumpSides] - _setCapacityofNextStage;
                        
                        
                        emit SetBulkCapacityofNextStage3(_currentUserStage);
                        emit SetBulkCapacityofNextStage4(_setCapacityofNextStage);
                        emit SetBulkCapacityofNextStage5(_randomSelectPlayerIndexNextStage);
    ...
    event SetBulkCapacityofNextStage(uint256 _capacity);
    event SetBulkCapacityofNextStage1(uint256 _capacity1);
    event SetBulkCapacityofNextStage2(uint256 _capacity2);
    event SetBulkCapacityofNextStage3(uint256 _capacity3);
    event SetBulkCapacityofNextStage4(uint256 _capacity4);
    event SetBulkCapacityofNextStage5(uint256[] _capacity4);
