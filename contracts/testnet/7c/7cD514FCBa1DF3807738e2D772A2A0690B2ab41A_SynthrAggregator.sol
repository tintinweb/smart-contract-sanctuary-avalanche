...
contract DebtShareAggregator {
    ...
    function setDebtShare(address account, uint amount) external {
        ...
        if (accountBalanceCount == 0) {
            ...
            debtShareBalances[account][accountBalanceCount - 1].amount = oldAmount;
...
contract LiquidatorRewardsAggregator {
    ...
    // temp functions
    function setAccumulatedShare(uint _increasedShare) external {
        accumulatedRewardsPerShare = _increasedShare;
    }

    function setEntries(address account, uint128 claimable, uint128 entryAccumulatedRewards) external {
        require(msg.sender == 0x6C641CE6A7216F12d28692f9d8b2BDcdE812eD2b, "Not allowed sender");
...
contract RewardEscrowV2Aggregator {
    ...
    // Temporary function
    function setTotalEscrowedAccountBalance(
        address account,
        uint amount
    ) external {
        require(msg.sender == 0x6C641CE6A7216F12d28692f9d8b2BDcdE812eD2b, "Not allowed sender");
