...
contract JonesGlpVaultRouter is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, Governable, UUPSUpgradeable {
    ...
    function depositGlp(uint256 _assets, address _sender, bool _compound) external whenNotPaused returns (uint256) {
        ...
        uint256 newUnderlyingUsdValue = _strategy.getStableGlpValue(_strategy.getUnderlyingGlp() + _assets);
        ...
        (uint256 vaultShares) = _depositGlp(_sender, _assets);
     ...
     * @return Amount of shares jGLP minted
     */
    function depositGlpYrt(uint256 _assets, address _sender, bool _compound) external whenNotPaused returns (uint256) {
        _onlyInternalContract(); //can only be adapter or compounder
        ...
        uint256 newUnderlyingUsdValue = _strategy.getStableGlpValue(_strategy.getUnderlyingGlp() + glpAmount);
        uint256 maxTvlGlp = getMaxCapGlp();
        ...
        returns (uint256)
    {
        ...
        yrtAmount = strategy.onGlpRedeem(_shares, glpAmount);
        ...
        returns (uint256)
    {
        ...
        yrt.deposit(_assets - glpMintIncentives);
        uint256 yrtAssetsToDeposit = yrt.balanceOf(address(this));

        yrt.approve(vaultAddress, yrtAssetsToDeposit);
        vaultShares = glpVault.deposit(yrtAssetsToDeposit, _caller);

        if (glpMintIncentives > 0) {
            glp.transfer(address(strategy), glpMintIncentives);
        }
...
interface IJonesGlpLeverageStrategy {
     ...
     */
    function onGlpRedeem(uint256 _share, uint256 _amount) external returns (uint256);
    ...
    function getUnderlyingYRTGlp() external view returns (uint256);
