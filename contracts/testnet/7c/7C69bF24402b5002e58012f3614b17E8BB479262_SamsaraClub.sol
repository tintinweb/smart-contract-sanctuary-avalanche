...
contract SamsaraClub is ERC20Upgradeable, LnAdminUpgradeable {
    using SafeMathUpgradeable for uint256;

    uint256 public constant MAX_SUPPLY = 10000000000e18;

    function __SamsaraClub_init(address _admin) public initializer {
        __ERC20_init("Samsara Token", "SAM");
        __LnAdminUpgradeable_init(_admin);
    }

    function mint(address account, uint256 amount) external onlyAdmin {
        require(totalSupply().add(amount) <= MAX_SUPPLY, "SamsaraClub: max supply exceeded");
