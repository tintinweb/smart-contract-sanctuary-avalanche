...
contract CoinFlip is VRFConsumerBaseV2, Ownable {
    ...
    struct WalletInfo {
        bool active;
        address wallet;
        mapping(uint256 =>uint256) allTimeWins;
        mapping(uint256 =>uint256) allTimeLoses;
        mapping(uint256 =>uint256) allTimeWinnings;
        mapping(uint256 =>uint256) allTimeLosings;
    ...
    event WinnerPicked(uint256 indexed flipId, CoinSide[] winningChoices, uint256 gambleAmountPerFlip, address player);
    ...
    uint256[] public unfinishedGames;
    ...
    constructor(
        ...
        // add in avax
        activeCoins[0] = TokenInfo(0, address(0), .5 ether, 20, 10, .01 ether, true);
    }
    ...
    function _createFlip(
        ...
        if(id != 0) { 
            ...
            balance = token.balanceOf(treasuryAddress);
        } else { 
            amount = msg.value;
            balance = treasuryAddress.balance;
        }
        ...
        require(gambleAmount <= (balance / tokenInfo.maxGambleDivisor), "Gamble less, king");
        ...
        wallets[msg.sender].nonFinishedGameIds.push(currentFlipId);
        ...
        currentFlipId += choices.length - 1;
         ...
         _addGameIntoGlobalIteratons(fi.flipId);
    ...
    function _addGameIntoGlobalIteratons(uint256 gameId) internal {
        uint256 nextIndex = unfinishedGames.length;
        unfinishedGames.push(gameId);
        unfinishedGames[gameId] = nextIndex;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        if (winnings > 0) {
            ...
            _internalHandleWonGame(requestId, output);
        } else { 
            _internalHandleLostGame(requestId, output);
        }

        fi.state = FlipState.CLOSED;
        emit WinnerPicked(flipId, output, fi.gambleAmountPerFlip, fi.creator);
    }

    
    
    function _internalHandleWonGame(uint256 gameId, CoinSide[] memory results) internal {
        FlipInfo memory game = flipById[gameId];
        WalletInfo storage walletInfo = wallets[game.creator];
        flipById[gameId].result = results;

        walletInfo.allTimeWinnings[game.id] = walletInfo.allTimeWinnings[game.id] + game.gambleAmountPerFlip * game.choices.length;
        walletInfo.allTimeWins[game.id] = walletInfo.allTimeWins[game.id] + 1;

        _updateGameInIterations(gameId);
    }

    function _internalHandleLostGame(uint256 gameId, CoinSide[] memory results)
        internal
    {
        FlipInfo memory game = flipById[gameId];
        WalletInfo storage walletInfo = wallets[game.creator];
        ...
        walletInfo.allTimeLosings[game.id] = walletInfo.allTimeLosings[game.id] + game.gambleAmountPerFlip * game.choices.length;
        walletInfo.allTimeLoses[game.id] = walletInfo.allTimeLoses[game.id] + 1;
        _updateGameInIterations(gameId);
    }
    ...
    function _updateGameOnWalletInfoIterations(uint256 gameId) internal {
        ...
        uint256 toBeRemovedIndex = walletInfo.nonFinishedGameIds[gameId];
    ...
    function _updateGameOnGlobalIterations(uint256 gameId) internal {
        ...
        uint256 lastIndex = unfinishedGames.length - 1;

        // Swap toBeRemoved with the item on last index of an array and update indexes
        if (toBeRemovedIndex != lastIndex) {
            uint256 lastIndexGameId = unfinishedGames[lastIndex];
            unfinishedGames[toBeRemovedIndex] = lastIndexGameId;
            unfinishedGames[lastIndex] = gameId;
    ...
    function getGamePlayerWallet(uint256 gameId) public view returns (address) {
        return flipById[gameId].creator;
    }
    ...
    function getFinishedGames() public view returns (FlipInfo[] memory) {
        uint256 startIndex = finishedGames.length > 25 ? finishedGames.length - 25 : 0;
        uint256 endIndex = finishedGames.length - 1;

        FlipInfo[] memory games = new FlipInfo[](endIndex - startIndex + 1);
        for (uint256 i = 0; i < games.length; i++) {
            uint256 gameId = finishedGames[endIndex - i];
            games[i] = flipById[gameId];
        }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            arr[i] = walletInfo.finishedGameIds[walletInfo.finishedGameIds.length - 1 - i];
        }
    ...
    function addToken(address tokenAddress, uint256 minGamble, uint256 maxGambleDivisor, uint256 maxConsecutiveFlips, uint256 flipMultiple) public onlyOwner {
        activeCoins[nextTokenID] = TokenInfo(nextTokenID, tokenAddress, minGamble, maxGambleDivisor, maxConsecutiveFlips, flipMultiple, true);
        IRakeDistributor rakeDistributor = IRakeDistributor(rakeDistributorAddress);
        ITreasury treasury = ITreasury(treasuryAddress);
        rakeDistributor.activateToken(nextTokenID, tokenAddress);
        treasury.activateToken(nextTokenID, tokenAddress);
        nextTokenID++;
    }
    ...
    function getTreasuaryBalance(uint16 id) public view returns (uint256) { 
        TokenInfo memory tokenInfo = activeCoins[id];
        if(id != 0) { 
            require(tokenInfo.active, "Token not active");
            IERC20 token = IERC20(tokenInfo.tokenAddress);
            return token.balanceOf(treasuryAddress);
        } else { 
            return treasuryAddress.balance;
        }
    }
