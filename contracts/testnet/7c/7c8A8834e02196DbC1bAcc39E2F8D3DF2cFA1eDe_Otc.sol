...
contract Otc is Pausable, Ownable {
    ...
    uint256 public fee = 1 * (10 ** (FEE_DECIMALS - 2)); // Equals to 1%
    ...
    event SetFeeEvent(uint256 indexed fee);
    ...
    constructor(
        ...
        address[] memory listingWallets,
        uint256[] memory initialAmounts,
        uint256[] memory createdAts
    ) {
        require(listingWallets.length == initialAmounts.length, "Illegal initialAmounts length");
        require(listingWallets.length == createdAts.length, "Illegal createdAts length");

        tokenAddress = IERC20(tokenAddress_);
        stableCoinAddress = IERC20(stableCoinAddress_);
        price = initialPrice;

        for (uint i = 0; i < listingWallets.length; i++) {
            addListingToList(listingWallets[i], initialAmounts[i], createdAts[i]);
        }
    }
    ...
    function list(uint256 amountXeta) external whenNotPaused {
        ...
        addListingToList(msg.sender, amountXeta, block.timestamp);
    ...
    function buy(address wallet, uint256 amountXeta) external whenNotPaused {
        require(
            stableCoinAddress.balanceOf(msg.sender) >= amountXeta * price / (10 ** xetaDecimals) * fee / (10 ** FEE_DECIMALS),
            "Insufficient stable coin balance"
        );
        require(
            stableCoinAddress.allowance(msg.sender, address(this)) >= amountXeta * price / (10 ** xetaDecimals) * fee / (10 ** FEE_DECIMALS),
            "Insufficient stable coin allowance"
        );
        ...
        while (amountXetaRemains > 0 && currentId != 0) {
            ...
            res = stableCoinAddress.transferFrom(
                msg.sender,
                listings[currentId].wallet,
                xetaToTransfer * price / (10 ** xetaDecimals)
            );
            require(res, "Error while transferring stable coin");
            res = stableCoinAddress.transferFrom(
                msg.sender,
                address(this),
                xetaToTransfer * price / (10 ** xetaDecimals) * fee / (10 ** FEE_DECIMALS)
            );
            require(res, "Error while transferring stable coin");
            feeTotal += xetaToTransfer * price / (10 ** xetaDecimals) * fee / (10 ** FEE_DECIMALS);
    ...
    function setFee(uint256 newFee) external onlyOwner {
        require(newFee < 10 ** FEE_DECIMALS, "Illegal fee");
        fee = newFee;
        emit SetFeeEvent(newFee);
    }

    /// @notice Withdraws all tokens that were collected for fees
    /// @dev Resets feeTotal
    /// @param recipient The address of the recipient who receives tokens that were collected for fees
    function withdrawFees(address recipient) external onlyOwner {
        bool res = stableCoinAddress.transfer(recipient, feeTotal);
        require(res, "Error transferring tokens");
    ...
    /// @notice Adds listing to the double-linked list
    /// @dev Does not emit ListEvent
    function addListingToList(address wallet, uint256 amount, uint256 createdAt) internal {
        listings[listingsIndex] = Listing(
            {
                wallet: wallet,
                ...
                createdAt: createdAt,
                nextListingId: 0,
                curListingId: listingsIndex,
                prevListingId: lastListingId
            }
        );
        listings[lastListingId].nextListingId = listingsIndex;
        lastListingId = listingsIndex;
        if (firstListingId == 0) {
            firstListingId = listingsIndex;
        }

        listingsNumber++;
        listingsIndex++;
        totalXetaListed += amount;
    }
