...
contract STAKINGBANK is Ownable, Pauseable {
    ...
    address public pot;
    address public cloves;
    address public gold;

     // reference to the GOLD POT NFT contract
    IPOT POT = IPOT(pot);
    //reference to cloves NFT contract
    ICLOV CLOVES = ICLOV(cloves);
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            POT.transferFrom(_msgSender(), address(this), tokenIds[i]);
        ...
        returns (uint256 owed)
    {
        ...
        if(owed < MINIMUM_TO_CLAIM && LosingPOTrisk == true){ 
            ...
            POT.transferFrom(address(this), Treasury , tokenId);
            ...
            emit PotLost(tokenId, stake.owner);
                ...
                if (unstake) {
                    // send back POT
                    POT.transferFrom(address(this), stake.owner, tokenId);
                    delete POTbank[tokenId];
                    staker.nPOT--;
                } else {
                // reset stake time
        ...
        nonReentrant
    {
        ...
        require(!paused() || msg.sender == owner(), "Paused");
        Stakeinfo memory staker = SInfo[_msgSender()];
        require( (staker.nCLOV + tokenIds.length)< MAX_CLOVE_STAKED , "To many CLOVES staked");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (tokenIds[i] == 0) {
                continue;
            }
        ...
        
    {
        ...
        require(clove.owner == _msgSender(), "SWIPER, NO SWIPING");
        CLOVES.transferFrom(address(this), clove.owner, tokenId);
        // send back PCLOV
   ...
   function getPotRewardInfo(uint256 tokenid)  public view returns (address owner, uint256 stakingStart, uint256 nPOT, uint256 ClovesBonus, uint256 owed) {
    ...
    function setToken(address _gold) public onlyOwner {
        gold = _gold;
    }

    function setPOT(address _pot) public onlyOwner {
        pot = _pot;
    }

    function setCLOV(address _clove) public onlyOwner {
        cloves  = _clove;
    }
