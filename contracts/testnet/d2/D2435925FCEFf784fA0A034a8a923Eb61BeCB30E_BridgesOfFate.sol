...
contract BridgesOfFate is Ownable {
    ...
    function _random(uint256 _range) internal view returns (uint256) {
        return uint256ToArray(uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender))),_range) + 1;
    }
    ...
    function _randomNarrowingGame(uint256 _range) internal view returns (uint256) {
        return uint256ToArray(uint256(keccak256(abi.encodePacked(address(this),block.timestamp,block.difficulty,msg.sender,address(0)))),_range) + 1;
    }
