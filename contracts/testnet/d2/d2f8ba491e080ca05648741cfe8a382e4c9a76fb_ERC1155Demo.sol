...
contract ERC1155Demo is Initializable
  ...
  function setFactory(
    ...
    onlyDev() {
    _revokeMinterRole(factory);
    factory = _factory;
    _addMinterRole(_factory);
  }
 ...
 * @notice This specification defines a standard way to allow controlled NFTs’ metadata updates
 *         along predefined formulas. Updates of the original metadata are restricted and
 *         defined by a set of recipes and the sequence and results of these recipes are
 *         deterministic and fully verifiable with on-chain metadata updates event. The proposal
 *         depends on and extends the EIP-721 and EIP-1155.
 * @custom:source https://eips.ethereum.org/EIPS/eip-5185
 * @custom:change-log MIT -> Apache-2.0
 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title:  EIP-4906: EIP-721 Metadata Update Extension
 * @author: Anders, Lance, Shrug, Nathan
 * @notice This standard is an extension of EIP-721. It adds a MetadataUpdate event to 
 *         EIP-721 tokens.
 * @custom:source https://eips.ethereum.org/EIPS/eip-4096
 * @custom:change-log MIT -> Apache-2.0
...
import "./eip/5185/IERC5185.sol";
import "./eip/4906/IERC4906.sol";
...
abstract contract Max1155UUPS2981 is Initializable
                                   , IRoles
                                   , IERC1155
                                   , IERC1155MetadataURI
  ...
  modifier onlyMinter() {
    if (contractRoles.has(MINTER, msg.sender) || contractRoles.has(ADMIN, msg.sender)) {
      _;
    } else {
    revert Unauthorized();
    }
  ...
  function _revokeMinterRole(
    address operator
  ) internal {
    contractRoles.remove(MINTER, operator);
  }
  ...
  function _updateURI(
    ...
    virtual {
    token1155.setURI(update);
    emit MetadataUpdates(update);
  ...
  function getSupplyOf(
    ...
    returns (uint256) {
    return token1155.tokenSupply(id);
  }
