...
contract NFTMarketplace is Ownable, ReentrancyGuard {
    ...
    event CollectionNew(address indexed collection, uint256 businessFee, uint256 creatorFee);
    event CollectionUpdate(address indexed collection, uint256 businessFee, uint256 creatorFee);
    ...
    EnumerableSet.AddressSet private _paymentTokenAddressSet;
    mapping(address => Collection) private _collections; 
    mapping(address => mapping(address => EnumerableSet.UintSet)) private _orderIdsOfSellerForCollection; // seller -> collection -> orderId
    mapping(address => EnumerableSet.UintSet) private _orderIds; 
    mapping(address => mapping(uint256 => Order)) private _orderDetails;  // collection -> orderId -> detail
    ...
    modifier onlyCeo() {
        require(msg.sender == ceoAddress, "Management: Not ceo");
        _;
    }

    constructor(address _ceoAddress) {
        require(_ceoAddress != address(0), "Operations: Ceo address cannot be zero");
        ceoAddress = _ceoAddress;
        _paymentTokenAddressSet.add(address(0));
    }

    function acceptPaymentToken(address _token) external onlyOwner {
        require(!_paymentTokenAddressSet.contains(_token), "Payment: Token already accepted");
    ...
    function cancelPaymentToken(address _token) external onlyOwner {
        require(_paymentTokenAddressSet.contains(_token), "Payment: Token not accepted");
    ...
    function addCollection(
        ...
        require(_businessFee.add(_creatorFee) <= TOTAL_MAX_FEE, "Operations: Sum of fee must inferior to TOTAL_MAX_FEE");
        ...
        emit CollectionNew(_collection, _businessFee, _creatorFee);
    }
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(_collection, _businessFee, _creatorFee);
    }
    ...
    function viewCollections(uint256 cursor, uint256 size) external view returns (
    ...
    function createOrder(
        ...
        require(_collectionAddressSet.contains(_collection), "Collection: Not for listing");
        require(_canTokenBeListed(_collection, _tokenIds), "Order: tokenIds not eligible");
        require(!_orderIds[_collection].contains(_orderId), "Order: OrderId already listed");
    ...
    function modifyOrder(
        ...
        require(_orderDetails[_collection][_orderId].seller == msg.sender && _orderDetails[_collection][_orderId].buyer == address(0), "Order: Already buy");
    ...
    function cancelOrder(address _collection, uint256 _orderId) external nonReentrant {
        require(_orderIdsOfSellerForCollection[msg.sender][_collection].contains(_orderId), "Order: Order not listed");
    ...
    function _buy(
        ...
        require(order.price > 0, "Buy: Invalid order");
        require(order.buyer == address(0), "Buy: Already sold");
        ...
        if (order.paymentToken == address(0)) {
            require(msg.value >= order.price, "Buy: Insufficient balance");
