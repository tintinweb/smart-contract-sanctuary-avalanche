...
contract StakingRewards is RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    // TODO: This reward duration should match the vesting. Define if it should be here or not.
    ...
    function withdraw(uint256 amount) public nonReentrant isWithdrawAllowed checkWithdrawalRequest updateReward(msg.sender) {
    ...
    function getReward() public nonReentrant isWithdrawAllowed checkWithdrawalRequest updateReward(msg.sender) {
    ...
    modifier isWithdrawAllowed() {
        require(block.timestamp >= (initializedAt + freezeTime), "Withdraw is not allowed.");
        _;
    }

    modifier checkWithdrawalRequest() {
        require(block.timestamp >= (withdrawalRequests[msg.sender] + cooldown), "Cooldown time pending.");
        _;
    }
