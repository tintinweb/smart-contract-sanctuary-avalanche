...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,uint256[] memory times,bool _feeFroze){
		...
		INTcollapse(owner(),0);
		INTaddProto(owner(),"asdfasdfdssadadsf");
	}
	...
	_protoowners.push(PROTOOWNERS({name:_name,protoCreationDate:INTgetCurrentTime(_account),protoElapsed:Zero,protoCollapseDate:INTgetCurrentTime(_account).add(protoLife),protoNextDue:feePeriod,protoDueElapsed:Zero,protoFutureFees:uint256(11),protoIncreasedDecay:Zero,protoFeeFroze:_protoFeeFroze,protoTotalPayable:nebuLib.doMultiple(maxFeePayment.sub(feePeriod),feePeriod)}));
