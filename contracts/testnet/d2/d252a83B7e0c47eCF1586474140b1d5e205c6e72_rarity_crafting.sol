...
contract rarity_crafting is ERC721Enumerable {
    ...
    ICraftingMaterials public _craft_i;
    IGold public _gold;
    ISkills public _skills;
    ICBaseRandom public _random;
    ICItemsGoods public _goods;
    ICItemsArmor public _armor;
    ICItemsWeapons public _weapons;
    IXproxy public _xp;
    ...
    constructor(address _rm_, address _attr_, address _craft_, address _gold_, address _skills_, address _random_, address _goods_, address _armor_, address _weapons_, address _xpProxyAddress) {
		_rm = IRarity(_rm_);
        _attr = IAttributes(_attr_);
        _craft_i = ICraftingMaterials(_craft_);
        _gold = IGold(_gold_);
        _skills = ISkills(_skills_);
        _random = ICBaseRandom(_random_);
        _goods = ICItemsGoods(_goods_);
        _armor = ICItemsArmor(_armor_);
        _weapons = ICItemsWeapons(_weapons_);
        _xp = IXproxy(_xpProxyAddress);
    ...
    function get_weapon_dc(uint _item_id) public view returns (uint dc) {
        ICItemsWeapons.weapon memory _weapon = _weapons.item_by_id(_item_id);
    ...
    function get_item_cost(uint _base_type, uint _item_type) public view returns (uint cost) {
        if (_base_type == 1) {
            ...
            ICItemsWeapons.weapon memory _weapon = _weapons.item_by_id(_item_type);
            cost = _weapon.cost;
        }
    }
    ...
    function get_token_uri_weapon(uint _item) public view returns (string memory output) {
        item memory _data = items[_item];
        {
            ICItemsWeapons.weapon memory _weapon = _weapons.item_by_id(_data.item_type);
            output = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
            output = string(abi.encodePacked(output, "category ", get_type(_data.base_type), '</text><text x="10" y="40" class="base">'));
