...
contract ArtMarketplace is Ownable, ReentrancyGuard, ERC721Holder, Pausable {
    ...
    */
	...
	event Deposit(address indexed user, address indexed lpAddress, uint256 tokenId);
	event Withdraw(address indexed user, address indexed lpAddress, uint256 amount);
	event Buy(address indexed user, address indexed lpAddress, uint256 tokenId, uint256 amount);
	...
	function harvest(address _lpAddress) external {
		require(pools[_lpAddress].users[msg.sender].rewardableNftCount > 0, "there is no rewardable nft");
...
* @notice Allows the owner to change maximumRoyaltyReceiversLimit.
    ...
    */
	...
	function depositNft(address _lpAddress, uint256 _tokenId, uint256 _price, uint64 _freeIndex, uint64 _previousIndex) external priceAccepted(_price) whenNotPaused nonReentrant {
		...
		emit Deposit(msg.sender, _lpAddress, _tokenId);
	}
	...
	function massEndRewardPeriod(address[] calldata _lpAddresses, uint256[] calldata _tokenIds) whenNotPaused onlyOwner external {
	...
	function endRewardPeriod(address _lpAddress, uint256 _tokenId) onlyOwner public onlyOwner whenNotPaused {
