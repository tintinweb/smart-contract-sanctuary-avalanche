...
contract _OTC_platform   is OwnableUpgradeable{
    ...
    // Get all Counter IDs for OTCID
     mapping(address=>mapping(uint256=>uint256[])) public userCounterIds;
    // accepted IDs
   ...
   // set Burn fee for not funded offers
    ...
    function counterOffer(address offerToken, uint256 offerAmount,address requestToken, uint256 requestAmount,uint256 otcId)external {
        ...
        uint256 _Ids=counterIds[otcId]+1;
         counterIds[otcId]++;      
        _counterOffer storage  _counter=_counterDetails[otcId][_Ids];
        ...
        if(isAccepted[otcId][acceptedID[otcId]]){
            address accepter=_counterDetails[otcId][acceptedID[otcId]].accepter;
             isAcceptedIds[accepter][otcId].push(_Ids);
        }
        isCounter[otcId]=true;
        _counter._requestToken=requestToken;
        _counter._requestAmount=requestAmount;
        address _accepter=_counterDetails[otcId][_Ids-1].accepter;
        if(msg.sender!=creator){
       ...
       _counter.creator=creator;
        ...
        pendingTaxes[address(token)]+=platformTokenFee;
        userCounterIds[msg.sender][otcId].push(_Ids);
  ...
  // function to claim all counter offered funds of particular Offer
    function claimOfferFunds(uint256 otcId)external{   
       require(keccak256(bytes(getOffers[otcId]._status))==keccak256(bytes("Active")) ,"Not Allowed");
       uint256[] storage _Ids=userCounterIds[msg.sender][otcId];
       for(uint256 i=0;i<_Ids.length;i++){
           require(_counterDetails[otcId][_Ids[i]].accepter==msg.sender,"Not Valid User");
            removeCounterId(otcId,_Ids[i]);
            delete _counterDetails[otcId][_Ids[i]];
            userCounterIds[msg.sender][_Ids[i]];
       }
    //    token.transfer(_user,platformTokenFee);
    }
    ...
    function acceptCounterOffer(uint256 otcID, uint256 _counterId)external {
        ...
        if(isPrivateOffer[otcID]){
          delete _counterDetails[otcID][_counterId];
          removeCounterId(otcID,_counterId);    
          uint256[] memory _ids=getAllCounterIds(otcID);
          for(uint256 i=0;i<_ids.length;i++){
              delete _counterDetails[otcID][_ids[i]];
              removeCounterId(otcID,_ids[i]);
          }
        }else if(isAccepted[otcID][_counterId]){
          uint256[] memory __ids=isAcceptedIds[_offer._Accepter][otcID];
            for(uint256 i=0;i<__ids.length;i++){
             delete _counterDetails[otcID][__ids[i]];
              removeCounterId(otcID,__ids[i]);
            }
        }
        else{
          delete _counterDetails[otcID][_counterId];
