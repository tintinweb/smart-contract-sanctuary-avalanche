...
contract Liquifier_Flattener is Ownable {
    ...
    function buyTokensAndBurn(uint256 amount) private lock {
        address[] memory path = new address[](2);
        path[0] = IRouter(router).WAVAX();
        path[1] = address(token);

        IRouter(router).swapExactAVAXForTokens{value: amount}(0, path, address(0), block.timestamp.add(300));
    }
