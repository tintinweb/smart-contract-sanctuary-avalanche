...
contract BlockStarStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() public initializer {
        ...
        token = IERC20Upgradeable(0x03E193f5Cb753b788f36D31ce5dAf899c301359F);
        started = true;
    ...
    function getUserTotalDepositeAmount(address _userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (
                ...
                total +=
                    (((userdata[i].amount * userdata[i].returnPer) / 10000) /
                        365) *
                    userdata[i].lockupDuration;
            }
        }
        return total;
    }
    ...
    function withdraw() external nonReentrant {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (
                ...
                uint256 reward = (((amount * userdata[i].returnPer) / 10000) /
                    365) * userdata[i].lockupDuration;
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        require(
            token.balanceOf(address(this)) > avalible,
        ...
        for (uint256 i = 0; i < len; i++) {
            if (userdata[i].amount > 0 && !userdata[i].claimed) {
                uint256 fees = (userdata[i].amount * emergencyWithdrawFess) /
                    10000;
                uint256 amount = userdata[i].amount;
