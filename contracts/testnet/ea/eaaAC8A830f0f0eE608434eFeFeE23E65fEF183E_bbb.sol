...

{
    ...
    uint256 private maxMintable = 3000;
        ...
        VRF(431, 100)
    {
        ...
        price = 10 * 10**15; //price in milli avax (10**-3)
        royaltyAmount = 500;
        tierMints = [0, 10, 5, 2]; //don't mind my stupid use of arrays i like having the indexes start from 1 
        tierPrice = [999*10**10, 150 * 10**15, 100 * 10**15, 0]; //same as previous array
        paused = true;
        wlTime = 0;
        publicTime = 1;
    }
...
contract VRF is ConfirmedOwner, VRFConsumerBaseV2 {
    ...
    function getRandomness() internal returns (uint256 randomWord) {
        ...
        if (currentRngIndex < numWords - 1) {
            currentRngIndex++;
            if (currentRequestIndex == numWords - 2){
                requestRandomWords();
            } 
        } else {
            currentRngIndex = 0;
