...
contract PreSale is Ownable {
    ...
    function buy(address _account, uint256 _amount) external payable {
        ...
        require(block.timestamp - launchTime > 1 days, "Not allowed presale");

        if (block.timestamp - launchTime < 30 days) {
            require(totalSellAmount + _amount <= IERC20(tokenAddress).totalSupply() * 25 / 100, "Allowed amount is not enough");
        } else if (block.timestamp - launchTime < 60 days) {
            require(totalSellAmount + _amount <= IERC20(tokenAddress).totalSupply() * 50 / 100, "Allowed amount is not enough");
        } else if (block.timestamp - launchTime < 90 days) {
            require(totalSellAmount + _amount <= IERC20(tokenAddress).totalSupply() * 75 / 100, "Allowed amount is not enough");
        } else {
            require(totalSellAmount + _amount <= IERC20(tokenAddress).totalSupply(), "Allowed amount is not enough");
        }
