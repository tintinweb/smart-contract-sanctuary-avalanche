...
contract AstroNodeManager is Ownable, Pausable {
    ...
    function compoundNode() public {
        ...
        } else if (astronauts[msg.sender].claimsDonated >= nodeCost) {
            astronauts[msg.sender].claimsDonated -= nodeCost;
            astronauts[msg.sender].nodes++;
            totalNodes++;
            totalNodesCompounded++;
        } else {
            if (astronauts[msg.sender].claimsNodes > astronauts[msg.sender].claimsDonated ){
                uint256 deductFromNodeRewards = astronauts[msg.sender].claimsNodes / nodeCost;
                uint256 remainderToDeduct = nodeCost - deductFromNodeRewards;
                astronauts[msg.sender].claimsNodes -= deductFromNodeRewards;
                astronauts[msg.sender].claimsDonated -= remainderToDeduct;
            } else {
                uint256 deductFromDonatedRewards = astronauts[msg.sender].claimsDonated / nodeCost;
                uint256 remainderToDeduct = nodeCost - deductFromDonatedRewards;
                astronauts[msg.sender].claimsDonated -= deductFromDonatedRewards;
                astronauts[msg.sender].claimsNodes -= remainderToDeduct;
            }
