...
contract CryptoShrimpsNFT is ERC721Enumerable, IERC2981, Ownable, ReentrancyGuard {
  ...
  uint256 public constant TOKEN_LIMIT = 7520;
  uint256 public constant PRESALE_MINT_LIMIT = 1000;
  ...
  uint256 public saleState = 0; // 0 = closed, 1 = presale, 2 = sale
  bool public burnActive = false;
  string public baseURI = "ipfs://QmT31DtGjtjmGs3cuaAW4Jwc14Qi4UThwSFfrUUAvcwPoa";

  // Price
  uint256 public presaleMintPrice = 1.2 ether; // 1.2 AVAX
  uint256 public mintPrice = 1.4 ether; // 1.4 AVAX
  ...
  // Wallet locations
  address royaltyWallet = address(this); // initializing to be contract, can be updated
  uint256 public royaltyFee = 5000;  // 5%

  constructor() ERC721("Cryptoshrimps", "SHRIMP") {}
  ...
  function isPresaleEligible(address minter) external view returns(bool) {
  ...
  function mintTokens(uint256 numTokens) external payable nonReentrant {
    ...
    require(numTokens > 0 && numTokens <= MAX_MINT_QUANTITY, "You can only mint 1 to 10 tokens at a time");

    if (saleState == 2) {
      // Open sale
      require(totalSupply() + numTokens <= TOKEN_LIMIT, "Not enough Cryptoshrimps tokens left.");
      uint256 totalPrice = mintPrice * numTokens;
      require(msg.value >= totalPrice, "Value sent is below the price");
    } else if (saleState == 1) {
      // Presale
      require(totalSupply() + numTokens <= PRESALE_MINT_LIMIT, "Not enough presale mint tokens available");
      require(_presaleAllowance[msg.sender] > 0, "You are not eligible to presale mint");
      require(_presaleAllowance[msg.sender] - numTokens >= 0, "You cannot mint that many tokens at this time");
      _presaleAllowance[msg.sender] -= numTokens;
      uint256 totalPresalePrice = presaleMintPrice * numTokens;
      require(msg.value >= totalPresalePrice, "Value sent is below the price");
      }
  ...
  /**
   * Owner only
   * @param addresses = addresses that can participate in presale
   * @param amount = number of tokens that addresses can mint in presale
   */
  ...
  function setRoyaltyWallet(address _wallet) external onlyOwner {
      _setRoyaltyWallet(_wallet);
  }
  ...
  function setPresaleMintPrice(uint256 newPresaleMintPrice) external onlyOwner {
    presaleMintPrice = newPresaleMintPrice;
  }
  ...
  function withdraw() external onlyOwner {
      // OWNER WITHDRAWS ALL FUNDS FROM SMART CONTRACT WALLET HERE
      // ENTER WALLETS THAT GET A % OF SUPPLY AND DISTRIBUTE ACCORDINGLY
