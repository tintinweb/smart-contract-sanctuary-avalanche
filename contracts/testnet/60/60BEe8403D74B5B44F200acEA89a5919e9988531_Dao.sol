...
contract Dao is ReentrancyGuard, Ownable {
    ...
    // This variable is incremented everytime a new proposal is added.
    uint256 public numOfProposals;

    /// @notice Holds all proposals made in the DAO.
        ...
        onlyHighestStakers()
    {
        if(numOfProposals>0){
            for (uint i; i<numOfProposals;i++){
                ...
                if (chaosProposal.proposer == msg.sender){
                    if (chaosProposal.livePeriod < block.timestamp){
                        revert("Proposer has already a pending proposal");
                    }
                }
            }
        }
