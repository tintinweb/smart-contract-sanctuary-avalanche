...
contract CollateralVault is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function add(IBRT _collateralToken, uint256 _collateralRate, uint256 _assetCeiling, uint256 _assetFloor, uint256 _liquidationPenalty) external onlyOwner {
        ...
        require(_liquidationPenalty <= 5e17 && (calDecimals+_liquidationPenalty) <= _collateralRate,"Collateral Vault : Liquidate setting overflow!");
    ...
    function join(uint256 pid, address account, uint256 amount) whenNotPaused settleAccount(pid, account) external {
        _join(pid, account, amount);
    }

    function joinAndBorrow(uint256 pid, uint256 collateralamount, uint256 systemCoinAmount) whenNotPaused settleAccount(pid, msg.sender) external {
    ...
    function borrowSystemCoin(uint256 pid, address account, uint256 amount) whenNotPaused settleAccount(pid, msg.sender) external {
        _borrowSystemCoin(pid, account, amount);
    }

    function exit(uint256 pid, address account, uint256 amount) whenNotPaused settleAccount(pid, msg.sender) external {
    ...
    function repaySystemCoin(uint256 pid, address account, uint256 amount) whenNotPaused settleAccount(pid, account) external {
        if(amount == MAX_UINT) {
    ...
    function _borrowSystemCoin(uint256 pid, address account, uint256 amount) internal {
        require(checkLiquidate(pid, msg.sender, 0, amount) , "overflow liquidation limit!");
    ...
    /*********************************** View FUNCTIONS for external usafe ***********************************/
