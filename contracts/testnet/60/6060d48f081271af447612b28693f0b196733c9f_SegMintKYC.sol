...
interface SegMintKYCInterface {
    ...
    function authorizeAddress(address account_, string memory userLocation_)
        external;
    ...
    // get geo location
    function getUserLocation(address account_)
        external
        view
        returns (string memory);
...
// SegMint KYC
contract SegMintKYC is SegMintKYCInterface {
    ///////////////////////
    ////   Libraries   ////
    ///////////////////////
    ...
    // Location
    mapping(address => string) private _userLocation;

    // authorized account info
    mapping(address => AUTHORIZEDACCOUNTINFO) private _authorizedAccountsInfo;

    // Struct for Authorize Addresses Function
    struct authorizeAddressesStruct {
        address account;
        string userLocation;
    }
    ...
    event setKYCManagerEvent(
        address indexed OwnerAddress,
        address previousKYCManager,
        address indexed newKYCManager,
        uint256 indexed timestamp
    );
    ...
    function authorizeAddress(address account_, string memory userLocation_)
        ...
        onlyKYCManager
    {
        _authorize(account_, userLocation_);
    }

    // add addresses to the authorized addresses
    function authorizeAddresses(authorizeAddressesStruct[] memory authorizeData_) public onlyKYCManager {
        require(authorizeData_.length > 0, "SegMint KYC: Null Authorize Data");
        for(uint256 i = 0; i < authorizeData_.length; i++){
            _authorize(authorizeData_[i].account, authorizeData_[i].userLocation);
        }
    }
    ...
    function unAuthorizeAddress(address account_)
        ...
        onlyKYCManager
    {
        _unauthorize(account_);
    }

    // remove addresses fro mthe authorized addresses
    function unAuthorizeAddresses(address[] memory accounts_)
        public
        onlyKYCManager
    {
        for(uint256 i = 0; i < accounts_.length; i++){
            _unauthorize(accounts_[i]);
        }
    }
    ...
    // private function to authorize address and save the user location
    function _authorize(address account_, string memory userLocation_) private notNullAddress(account_) {
        
        // require correct geo location
        require(
            Strings.equal("US", userLocation_) ||
                Strings.equal("XUS", userLocation_),
            "SegMink KYC: Invalid User Location!"
        );
        ...
        // set user location
        _userLocation[account_] = userLocation_;

        // emit event
        emit authorizeAddressEvent(msg.sender, account_, block.timestamp);
    }

    // private function to remove account from authorize address and set authorize status to false
    function _unauthorize(address account_) private notNullAddress(account_) {
        ...
        _authorizedAccountsInfo[account_].unauthorizeTimestamp = block.timestamp;
