...
contract buddyStaking is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable {
    ...
    string _URISuffix;
    uint public RATE_PER_DAY = 24 * 10**18;
    uint public MAX_CAP = 240 * 10**18;
    ...
    constructor(address _moonbuddies, address _token) ERC721("StakedMBuddy", "SMB") {
        moonbuddies = IERC721(_moonbuddies);
        token = candy(_token);
    }
    ...
    {
        require (user == moonbuddies.ownerOf(tokenId), "User must be owner.");
        moonbuddies.transferFrom(user, address(this), tokenId);
    ...
    function withdraw(uint256 tokenId, address user) internal
    {
        ...
        claim(user, tokenId);
        moonbuddies.transferFrom(address(this), user, tokenId);
        burn(tokenId);
...
contract candy is ERC20, ERC20Burnable, Ownable, Authorizable {
