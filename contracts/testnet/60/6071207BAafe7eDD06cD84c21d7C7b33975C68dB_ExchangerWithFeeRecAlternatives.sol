...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[8] = CONTRACT_CIRCUIT_BREAKER;
        newAddresses[9] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[10] = CONTRACT_SYNTHR_BRIDGE;
        newAddresses[11] = CONTRACT_SYNTHETIX_DEBTSHARE;
    ...
    function exchange(
        ...
        uint256 fee;
        if (args.fromAccount != from) {
            require(delegateApprovals().canExchangeFor(args.fromAccount, from), "Not approved to act on behalf");
        }
    ...
    function _settleAndCalcSourceAmountRemaining(
        ...
        (reclaimed, refunded, numEntriesSettled) = _internalSettle(from, sourceCurrencyKey, false);
    ...
    function _exchange(ExchangeArgs memory args) internal returns (uint256 amountReceived, uint256 fee) {
        ...
        // in these currencies
        // _updateSNXIssuedDebtOnExchange([args.sourceCurrencyKey, args.destCurrencyKey], [entry.sourceRate, entry.destinationRate]);
    ...
    function _convert(ExchangeArgs memory args) internal // uint256 sourceAmountAfterSettlement,
    // uint256 amountReceived,
    // uint256 fee,
    // uint256 reclaimed,
    // uint256 refunded
    {
        ...
        synthrBridge().sendExchange(args);
    }
...
contract ExchangerWithFeeRecAlternatives is Exchanger {
    ...
    //     exchangeFeeRate = _feeRateForAtomicExchange(
    //         sourceCurrencyKey,
    //         destinationCurrencyKey
    //     );
    // }
    ...
    function _exchangeAtomically(ExchangeArgs memory _args) internal returns (uint256 amountReceived, uint256 fee) {
        ExchangeArgs memory args = _args;
        _ensureCanExchange(args.sourceCurrencyKey, args.sourceAmount, args.destCurrencyKey);
        require(!exchangeRates().synthTooVolatileForAtomicExchange(args.sourceCurrencyKey), "Src synth too volatile");
        require(!exchangeRates().synthTooVolatileForAtomicExchange(args.destCurrencyKey), "Dest synth too volatile");
        ...
        // // exchange volume.
        // _updateSNXIssuedDebtOnExchange([args.sourceCurrencyKey, args.destCurrencyKey], [systemSourceRate, systemDestinationRate]);
