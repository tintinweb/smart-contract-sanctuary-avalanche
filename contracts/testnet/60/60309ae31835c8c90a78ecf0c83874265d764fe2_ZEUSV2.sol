...
contract ZEUSV2 is Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    function cashoutAllExtraBalances(uint32 nodeType) public {
        address sender = _msgSender();
        require(
            sender != address(0),
            "MANIA CSHT:  creation from the zero address"
    ...
    function getNodeNumberOf(address account, uint32 nodeType) public view returns (uint256) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
    ...
    function changeNodePrice(uint256 newNodePrice, uint32 nodeType) public onlyOwner {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
    ...
    function changeRewardPerNode(uint256 newPrice, uint32 nodeType) public onlyOwner {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
    ...
    function changeClaimTime(uint256 newTime,uint32 nodeType) public onlyOwner {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
    ...
    function changeGasDistri(uint256 newGasDistri,uint32 nodeType) public onlyOwner {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
    ...
    function getNodesNames(uint32 nodeType) public view returns (string memory) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
    ...
    function getNodesCreatime(uint32 nodeType) public view returns (string memory) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
    ...
    function getNodesRewards(uint32 nodeType) public view returns (string memory) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
    ...
    function getNodesLastClaims(uint32 nodeType) public view returns (string memory) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
    ...
    function getTotalStakedReward(uint32 nodeType) public view returns (uint256) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        return nodeRewardManager.totalRewardStaked();
    }

    function getTotalCreatedNodes(uint32 nodeType) public view returns (uint256) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        return nodeRewardManager.totalNodesCreated();
    }

    function transferSelectedToWallet(uint256[] memory blocktimes,uint32 nodeType) external {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        address sender = _msgSender();
        uint256 total = 0;
        for (uint256 i = 0; i < blocktimes.length; i++) {
            ...
            total.add(rewardAmount).add(fee);
        }
        walletBalances[sender] = total;
    }
    ...
    function compoundNode(
        ...
        uint256 nodePrice = nodeRewardManager.nodePrice();
        require(walletBalances[sender] >= nodePrice, "Insufficient Fund");
        uint256 r_total = total.mul(nodePrice);
        require(r_total >= walletBalances[sender], "Insuffcient Fund");
        walletBalances[sender] -= r_total;
    ...
    function getReferrals(uint32 nodeType) external view returns (address[] memory) {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        return nodeRewardManager.getReferrals(_msgSender());
    }

    function setReferralBonusPercent(uint256 amount,uint32 nodeType) external onlyOwner {
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
        nodeRewardManager.setReferralBonus(amount);
    }

    function setNodeManagement(address nodeManagement, uint32 nodeType) external onlyOwner {
        if (nodeType == 0) {
