...
library boostLib {
    ...
    function calcReward(uint256 _dailyRewardsPerc,uint256 _timeStep,uint256 _timestamp, uint256 _lastClaimTime, uint256 _boost_) internal pure returns (uint256[3] memory){
            uint256 _one_ = 1;
	    ...
	    uint256 _boost = _rewards_[1];
    	    ...
    	    return [_all,_boost_,_dayMultiple1];
    	   }
...
contract NodeManager is Ownable, Pausable {
	...
	for (uint256 j = 0; j < nodes.length; j++) {
		...
		uint256 elapsed = time - lastClaim;
        	uint256[3] memory rew = boostLib.calcReward(dailyRewardsPerc,timeStep,time,lastClaim,boostMultiplier[0]);    //takeFee(((rewardPerDay.mul(10000).div(1440) * ((elapsed -(block.timestamp - nextDue))  / 1 minutes)) / 10000),75)
