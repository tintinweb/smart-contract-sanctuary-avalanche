...
contract Lockup is Ownable {
    ...
    constructor (){
        ...
        StakeNFT = IERC721Metadata(0xa14B79789E55caBeb417A89120fcAF35d8F089e1);
    ...
    function unStakeNFT(string memory name) public {
        ...
        require(stakedUserList[_string2byte32(name)].NFTStakingId != 0, "Invalid operator");
    ...
    function getUserNFT(address user) public view returns(uint256 length, IERC721Metadata.NFTInfo[] memory NFTList, string[] memory uris) {
    ...
    function getUserNFT(string[] memory nameList) public view returns(UserNFTInfo[] memory userInfo) {
        for (uint256 i = 0; i < nameList.length; i++) {
            ...
            StakeInfo memory stakeInfo = stakedUserList[_string2byte32(nameList[i])];
            (IERC721Metadata.NFTInfo memory NFTList, string memory uri) = NFToken.getUserNFTInfoByTokenId(stakeInfo.NFTId);
    ...
    // function getUserStakedNFT(address userAddr) public view returns(uint256[] memory tokenIds, string[] memory uris) {
    //     bytes32[] memory keys = userInfoList[userAddr];
    //     uint256 j = 0;
    //     for (uint256 i = 0; i < keys.length; i++) {
    //         StakeInfo memory info = stakedUserList[keys[i]];
    //         if(info.NFTStakingId > 0) {
    //             tokenIds[j] = info.NFTStakingId;
    //             uris[j++] = StakeNFT.tokenURI(info.NFTStakingId);
    //         }
    //     }
