// SETTING
// 1. ADD VAULT
// 2. PAUSE AND UNPAUSE OF GET REWARD
// 3. PAUSE AND UNPAUSE OF VAULT ID
// 4. VAULTEXISTS(ID)
// 5. UPDATE VAULT
// 6. DEPOSIT
// 7. WITHDRAW
...
// 9. emergencyWithdraw
// 10. GET REWARDS
// 11. POOL VAULT INFO : STATUS WITH UPDATE POOL
// 12. UPDATE REWARD : STAKE, WITHDRAW, GET REWARD
    ...
    ERC1155Holder
{
    ...
    mapping(address => mapping(uint256 => bool)) public StakingExist; // vaultaddress, epochEnd, stakingexist
    ...
    modifier updateReward(address _vault, uint256 epochEnd, address account) {
        ...
        accountUser.reward = accountUser.reward.add(
                                accountUser.balance
        ...
        accountUser.lastTime = lastTimeRewardApplicable(epochEnd);
        _;
    }
    ...
    mapping(address => Count) private count;
    modifier updateCount(address account, uint256 amount){
        count[account].step = count[account].step.add(amount);
        count[account].createdTime = block.timestamp;
        _;
    }

    function nextCount(uint256 amount) external updateCount(msg.sender, amount) {
        count[msg.sender].count = count[msg.sender].count.add(1);
    }

    function getCount(address account) external view returns (Count memory){
        return count[account];
    }
     /* ========== END TEST MODIFIERS ========== */

    modifier marketExists(address vault, uint256 id) {
        if(StakingExist[vault][id] != true)
    ...
    function getReward(address _vault, uint256 epochEnd)
            ...
            updateReward( _vault, epochEnd, msg.sender)
    {   
        User memory UserInfor = user[msg.sender][_vault][epochEnd];
    ...
    function createStakingRewards(address _vaultFactory, uint256 _marketIndex, uint256 _epochEnd, uint256 _hedgeApr, uint256 _riskApr) external onlyOwner {
...
contract TimeLock {
    ...
    uint256 public constant MIN_DELAY = 20 minutes; //7 days
    uint256 public constant MAX_DELAY = 100 minutes; // 30 days
    uint256 public constant GRACE_PERIOD = 40 minutes; // 14 days
