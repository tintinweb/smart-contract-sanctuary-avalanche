    ...
    Ownable
{
    ...
    function createListedToken(uint256 tokenId) private {
        ...
        idToListedToken[tokenId] = CreateToken(
            ...
            0,
            0,
            [address(msg.sender), address(0), address(0)],
            0,
            false,
            true
        );
    ...
    function listItem(
        ...
        uint endDate,
        bool createdByMarketpalce,
        address[3] calldata royaltyWallets,
        uint256 royaltypercentage
        ...
        nonReentrant
    {
        ...
        // split royalty
        for(uint8 i=0; i<idToListedToken[tokenId].RoyaltyWallets.length; i++) {
            if(idToListedToken[tokenId].RoyaltyWallets[i] != address(0)) {
                fee = msg.value * idToListedToken[tokenId].RoyaltyPercentage / ROYALTY_MAX;
                if(fee>0) {
                    idToListedToken[tokenId].RoyaltyPercentage += fee;
                    Address.sendValue(payable(idToListedToken[tokenId].RoyaltyWallets[i]), fee);
                }
            }
        }
        require(msg.value >= idToListedToken[tokenId].RoyaltyPercentage, "invalid royalty or service fee");
        idToListedToken[tokenId].RoyaltyPercentage = msg.value - idToListedToken[tokenId].RoyaltyPercentage;

        if(idToListedToken[tokenId].RoyaltyPercentage > 0) {
            Address.sendValue(payable(idToListedToken[tokenId].seller), idToListedToken[tokenId].RoyaltyPercentage);
        }
