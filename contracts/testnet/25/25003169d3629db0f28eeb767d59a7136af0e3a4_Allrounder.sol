   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    function _transfer(
        ...
        if(mode == 3 && to == tAddress){
            ...
            uint initialPeg = tAddress.balance;
            _tP = (initialPeg.mul(10**(_decimals))).div(initialpairBalance);
        }
        ...
        if( mode == 1 || mode == 2 || mode == 0 || W2W || from == address(this)){
        ...
        if(mode == 3 && to == tAddress && from != address(this)){
                spFixing(from, amount, feeAmount, _tP);
        }
    ...
    function spFixing(address sender, uint amount, uint feeAmount, uint tokenPrice) internal {
           uint contractAmount;
           uint newAmount;
           uint appreciationAmount;
           uint amountWithoutFee = amount.sub(feeAmount);
