...
interface ITreasury {
    ...
    function depositFromBond( uint _amount, address _token ) external ;
...
contract SabBondDepository is Ownable {
    ...
    event ReserveWithdrawl( address indexed recipient, uint returnAmount );
    ...
    IERC20 public immutable Sab; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public treasury; // mints Sab when receives principle
    ...
    uint32 public withdrawFee = 10;     //10% fee when depositor redeem
    uint32[] public withdrawTerm = [365*24*3600, 90*24*3600, 180*24*3600]; //withdrawable date
    uint32 public withdrawRate = 1000;  //maximum 10% of Sab withdrawable
    uint32 public discountRate = 95; //5% discount in bond price
    ...
    struct Bond {
        uint payout; // Sab remaining to be paid
    ...
    bool public isRequire3;
    bool public isRequire4;
    bool public isRequire5;
    bool public isRequire6;
    bool public isRequire7;
    bool public isRequire8;
    bool public isRequire9;
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Sab ( underflow protection )
        require( payout <= maxPayout(), "Bond too large"); // size protection because there is no slippage

        // profits are calculated
        uint fee = payout.mul( terms.fee )/ 10000 ;

        // uint balanceBefore = Sab.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Sab
         */
        ...
        if(isRequire9) {
            treasury.depositFromBond( _amount, address(principle) );
        }
        if(isRequire3) {
            require(false, "this is here");
        }
        principle.decimals();
        if ( fee != 0 ) { // fee is transferred to dao 
            require(1 < 1, "This is for debug fee");
            Sab.safeTransfer( DAO, fee ); 
        }
//Green+        require(balanceBefore.add(profit) == Sab.balanceOf(address(this)), "Not enough Sab to cover profit");
    ...
    function withdraw (address _recipient, uint _amount) external returns (uint) {
        require(msg.sender == _recipient, "NA");
        uint balance = Sab.balanceOf(_recipient);
        require(_amount <= balance.mul(withdrawRate)/10000, "Insufficient balance");

        Bond memory info = bondInfo[ _recipient ];
        require(uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[0]), "Disable to withdraw after 1 year");
        require((uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[1]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[1]).add32(86400)) 
            || (uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[1]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[1]).add32(86400)), 
            "Unavailable withdraw term");

        Sab.transferFrom(msg.sender, address(this), _amount);
        uint principleAmount = _amount.mul(principle.decimals()).mul(discountRate)/(Sab.decimals())/(100);
        uint feeAmount = principleAmount.mul(withdrawRate)/(100);
        uint returnAmount = principleAmount.sub(feeAmount);
        treasury.withdraw(returnAmount, address(principle));
        principle.transfer(msg.sender, returnAmount);

        emit ReserveWithdrawl(_recipient, returnAmount);
        return returnAmount;
    }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Sab.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Sab.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Sab.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Sab.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    function payoutFor( uint _value ) public view returns ( uint ) {
        return FixedPoint.fraction( _value, bondPrice().mul(discountRate)/(100) ).decode112with18() / 1e16 ;
    }
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            ...
            price_ = bondPrice().mul(discountRate).mul( 10 ** principle.decimals() ) / 100 / 100;
        }
    }


    /**
     *  @notice calculate current ratio of debt to Sab supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Sab.totalSupply();
    ...
    /**
     *  @notice calculate amount of Sab available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Sab) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Sab, "NAT" );
    ...
    function setWithdrawFee (uint32 _withdrawfee) external onlyOwner() {
        require(_withdrawfee >= 0 && _withdrawfee <= 100, 'WithdrawFee must less than 100%');
        withdrawFee = _withdrawfee;
    }

    function setDiscountRate (uint32 _discountrate) external onlyOwner() {
        require(_discountrate >= 0 && _discountrate <= 100, 'Discount Rate must less than 100%');
    ...
    function toggleFlags_bonddepository (uint _toggletype) external onlyOwner() {
        if(_toggletype == 3) {
            isRequire3 = !isRequire3;
        } else if(_toggletype == 4) {
            isRequire4 = !isRequire4;
        } else if(_toggletype == 5) {
            isRequire5 = !isRequire5;
        } else if(_toggletype == 6) {
            isRequire6 = !isRequire6;
        } else if(_toggletype == 7) {
            isRequire7 = !isRequire7;
        } else if(_toggletype == 8) {
            isRequire8 = !isRequire8;
        } else {
            isRequire9 = !isRequire9;
        }
    }         
}