...
contract BetUpSheep is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        ...
        uint256 nextMintedAmount = _numberMinted(_sender) + 1;
        require(nextMintedAmount <= mintLimit, "out of mintLimit");
        require(IBetUpCore(betContract).isAvailableMint(_sender, nextMintedAmount), "condition is not met");
        uint16 minusAmount = IBetUpCore(betCoreAddress).mintCondition(nextMintedAmount).amount;
        IBetUpCore(betCoreAddress).decreasePlayerAmount(_sender, minusAmount);
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        require(IBetUpCore(betCoreAddress).isAvailableUpgrade(owner, _afterLevel), "condition is not met");

        IBetStruct.MintCondition memory  condition = IBetUpCore(betCoreAddress).upgradeConditionToLevel(_afterLevel);

        uint256 perSpendAmount = condition.amount * 10 ** 18;
        ...
        IBetUpCore(betCoreAddress).decreasePlayerAmount(owner, condition.count);
...
interface IBetStruct {
    ...
    struct MintCondition {
        ...
        uint16 amount; // @dev the number of dao (amount * 10 ^ 18) for upgradeCondition
...
interface  IBetUpCore {
    ...
    function isAvailableMint(address _address, uint256 _mintedCount) external view returns(bool);
    function decreasePlayerAmount(address _player, uint256 _minusAmount) external;
