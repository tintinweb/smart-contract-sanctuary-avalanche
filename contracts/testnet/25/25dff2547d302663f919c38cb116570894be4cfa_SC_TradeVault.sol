...
contract SC_TradeVault is ReentrancyGuard/*, ERC721Holder*/ {
    ...
    event RemoveLiquidity(address indexed owner, IERC20 indexed srcToken, uint256 amount, uint256 balance);
    event Swap(address indexed owner, address indexed bot, bytes32 indexed id, uint256 inAmount, uint256 outAmount, uint256 srcTokenBalance, uint256 outTokenBalance);
    event VoidOrder(address indexed owner, address indexed bot, uint256 indexed nonce);
    ...
    function redeemFund(IERC20 token) external {
        if (token == feeToken) {
            uint256 amount = feeToken.balanceOf(address(this)) - tokenBalances[feeToken] - totalLpFeeBalance;
