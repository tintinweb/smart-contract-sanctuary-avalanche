...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router) {
        router = _router != address(0) ? IDEXRouter(_router) : IDEXRouter(0xd7f655E3376cE2D7A2b08fF01Eb3B1023191A901);
        _token = msg.sender;
    }
...
contract Test8 is IERC20, Auth {
    using SafeMath for uint256;

    string constant _name = "TEST8";
    ...
    uint256 public swapPercentMax = _totalSupply / 100;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[recipient]) {
            try distributor.setShare(recipient, _balances[recipient]) {
            } 
    ...
    function getTotalBuyFee(bool selling) public view returns (uint256) {
    ...
    function getTotalTransferFee(bool selling) public view returns (uint256) {
        if(launchedAt + 1 >= block.number) {
    ...
    function setTransFees(uint256 _transferReflectionFee, uint256 _transferBurnFee, uint256 _transferMarketingFee, uint256 _transferDevFee) external authorized {
        ...
        require(totalTransferFee < feeDenominator / 10);
    }
