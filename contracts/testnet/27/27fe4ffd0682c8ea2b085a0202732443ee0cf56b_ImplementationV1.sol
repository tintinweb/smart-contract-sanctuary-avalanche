...
contract ImplementationV1 { // ---> Transfering And Approving NFT Tokens Via MarketPlace <---
    ...
    function addContractAddress(
        ...
        require(allMarketContracts[_contract] == true, "this address is not a valid contract address.");
        require(validateUserContracts[msg.sender][_contract] == false, "This contract already exist.");

        address[] storage addrs = userAddedContracts[msg.sender];
        addrs.push(_contract);

        validateUserContracts[msg.sender][_contract] = true;
    }
    ...
    function addOwnedContractAddress(
        address _ownedContract
    ) external {
        require(IERC721(_ownedContract).ownerAddress() == msg.sender, "You are not the owner!");
        require(validateUserOwnedContracts[msg.sender][_ownedContract] == false, "Contract already added.");

        address[] storage contracts = userOwnedContracts[msg.sender];
        contracts.push(_ownedContract);

        validateUserOwnedContracts[msg.sender][_ownedContract] = true;
    }
