...
contract Allrounder is IERC20 {
    ...
    //thresholds
    uint public LPDriverThreshold = (maxSupply.mul(5)).div(_taxCalcDenominator);  // 1% of totalSupply
    uint public MaxWalletAmount = (maxSupply.mul(10)).div(_taxCalcDenominator);
    uint public MaxTxAmount = (MaxWalletAmount.mul(200)).div(_taxCalcDenominator);
    uint public largeSellAmountThreshold = maxSupply.div(1000); // 0.1% of totalSupply
    uint public mediumSellAmountThreshold = maxSupply.div(2000); // 0.05% of totalsupply
    ...
    modifier lpdriver(){
        if(_balances[address(this)].balance > LPDriverThreshold){
            _SelfLpDriver();
        }
        _;
    }
   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    //peg fixing
    function buyFixing( uint amount) private {
           uint amountIn;
           uint tokenPrice;
           uint mintAmount;
           uint newAmount;
            ...
            require(maxSupply.sub(totalSupply) >= mintAmount, "max supply exceed reduce the tx amount");
