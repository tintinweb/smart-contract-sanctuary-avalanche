...
contract MarketCore is MarketPojo{
    ...
    function hashOrder(address maker,
        uint256 salt,
        uint listingTime,
        uint expirationTime) internal pure returns(bytes32){
        return keccak256(abi.encode(maker,salt,listingTime,expirationTime));
    }
    ...
    function commitOrder(  
        ...
        bytes32 signatureHash = Proof.hashSignature(orderHash,Proof.getBehindBytes(data),orderData,target);
    ...
    function orderMatch(bytes memory hashOrders,bytes memory singers, bytes memory callDatas,bytes memory callTarget, bytes memory signatures,bytes memory orderData) external {
            ...
            (bytes32 firstOrderHash, bytes32  secondOrderHash) = abi.decode(hashOrders, (bytes32, bytes32));
    ...
    function decode(bytes memory data) external view returns(address a, uint256 b, uint256 c,uint256 d){
        return abi.decode(data,(address,uint256,uint,uint));
    }
