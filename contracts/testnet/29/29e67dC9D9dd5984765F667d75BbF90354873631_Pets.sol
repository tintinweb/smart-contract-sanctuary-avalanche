...
contract Pets is ERC721Enumerable, Pausable, Restricted {
    ...
    uint16[5] generationsTresholds = [10000, 20000, 30000, 40000, 50000];
    // [0.5 ether, 0.4 ether, 0.3 ether, 0.2 ether, 0.1 ether] for real launch <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    uint256[5] generationsPriceAVAX = [5 wei, 4 wei, 3 wei, 2 wei, 1 wei];
    uint256[5] generationsPriceGEM = [20000 ether, 30000 ether, 30000 ether, 40000 ether, 50000 ether];
        ...
        notBlacklisted
    {
        ...
        unchecked{limitedMints[index].remaining -= amount;}
    ...
    * @param petToken The id of the pet you'd like to associate to the wallet
    ...
    function mintCost(uint256 tokenId) public view returns (uint256, uint256) {
        if (tokenId <= generationsTresholds[0]) return (generationsPriceAVAX[0], generationsPriceGEM[0]);
        if (tokenId <= generationsTresholds[1]) return (generationsPriceAVAX[1], generationsPriceGEM[1]);
        if (tokenId <= generationsTresholds[2]) return (generationsPriceAVAX[2], generationsPriceGEM[2]);
        if (tokenId <= generationsTresholds[3]) return (generationsPriceAVAX[3], generationsPriceGEM[3]);
        return (generationsPriceAVAX[4], generationsPriceGEM[4]);
    }
