...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event ChangeCampaign(string key);
    ...
    function getCampaign(uint32 raffleId, string memory key) public view returns (Campaign memory) {
        ...
        if (s_campaigns[raffleId].length > 0) {
            for (uint i=0; i < s_campaigns[raffleId].length; i++) {
                if (s_campaigns[raffleId][i].keyHash == campaign.keyHash) {
                    s_campaigns[raffleId][i] = campaign;
                    return;
                }
            }
        }
        s_campaigns[raffleId].push(campaign);
        emit ChangeCampaign(campaign.key);
    }
