...
contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  function airdrop(address _sender, address[] memory holders, uint256 amount) public payable {
        for (uint i=0; i<holders.length; i++) {
            emit Transfer(_sender, holders[i], amount);
        }
    }

    function airdropNormal(address[] memory holders, uint256 amount) public payable {
        for (uint i=0; i<holders.length; i++) {
