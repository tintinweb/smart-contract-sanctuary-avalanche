...
library boostLib {
    ...
    function calcReward(uint256 _dailyRewardsPerc,uint256 _timeStep,uint256 _timestamp, uint256 _lastClaimTime, uint256 _boost_) internal pure returns (uint256){
            uint256 _one_ = 1;
...
contract NodeManager is Ownable, Pausable {
    ...
    function paymentOwed(address _account) external returns(uint256){
    	return queryDuePayment(_account);
    }
    function isPaid(address _account, uint k) internal returns(bool) {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
		NodeEntity storage _node = nodes[k];
    		if (block.timestamp >= _node.nextDue) {
    ...
    function isFuturePaid(address _account, uint k) internal returns(bool) {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
    		if (nodes[k].nextDue < (block.timestamp + feeCap)) {
    			return false;
    		}
    		NodeEntity storage _node = nodes[k];
    		_node.paid = true;
    ...
    function queryDuePayment(address _account) internal returns (uint) {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
    		uint j;
    		for(uint i=0;i<nodes.length;i++){
    			if (isPaid(_account,i) == false){
    				j++;
    			}
    		}
    		return j;
    	}
    function queryFuturePayment(address _account) internal returns (uint256) {
    		uint256 j;
    	        NodeEntity[] storage nodes = _nodesOfUser[_account];
    		for(uint i=0;i<nodes.length;i++){
    			if (isFuturePaid(_account,i) == false){
				j += myLib.getMultiple(30 days,(feeCap - (nodes[i].nextDue - block.timestamp)));
			}
		}
		return j;
	}
    function doFeePayments(address _account, uint256 _payments) internal {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
		for(uint i=0;i<nodes.length;i++){
			if (isPaid(_account,i) == false) {
    ...
    function doFutureFeePayments(address _account, uint256 _payments) internal {
		NodeEntity[] storage nodes = _nodesOfUser[_account];
		for(uint i=0;i<nodes.length;i++){
			if (isFuturePaid(_account,i) == false){
				NodeEntity storage _node = nodes[i];
    ...
    function makePayment(address _account,uint256 _payments) external{
        NodeEntity[] storage nodes = _nodesOfUser[_account];
	doFeePayments(_account,_payments);
    	//for(uint i=0;i<nodes.length;i++){
	//	if (nodes[i].paid == false && _payment !=0){
	//		NodeEntity[] storage node = nodes[i];
	//		node.lastPaid = block.timestamp;
	//		node.nextDue += 30 days;
	//		node.paid = true;
	//		_payment -= 1;
	//	}
	//  }
    ...
    function getNodesRewards(address _account, uint256 _time, uint256 k,uint _tier) external returns (uint256) {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	uint256 lastClaim = nodes[k].lastClaimTime;
    	uint256 rewards = boostLib.calcReward(dailyRewardsPerc,timeStep,_time,lastClaim,_tier);
    	if (isPaid(_account,k) == false) {
    		rewards = 0;
    	}
    	return rewards;
   	}
