...
contract EmeraldNodeV2 is Initializable, OwnableUpgradeable {
    ...
    function updateCount(uint256 _num, uint256 _counter) external onlyOwner returns(bool){
        count = _num;
        RegularNodeCounter = _counter;
        return true;
    }
    ...
    function buyNode(string memory _referenceId, string memory _nodeName) external returns(bool){
        ...
        if(whiteListAddress[msg.sender] && addressCount[msg.sender] < count && nodeCounter <= nodeIds[1]){
            ...
            for(uint256 i = 0; i < 5; i++){
                if((nodeIds[i] < RegularNodeCounter && nodeIds[i+1] >= RegularNodeCounter) || nodeIds[5] < RegularNodeCounter){
                    _tier = i+1;
        ...
        EmeraldCoin.safeTransfer(msg.sender, owner(), tierDetails1[_tier].tierNodePrice);
        ...
        emit nodeCreated(nodeId, _tier, _referenceId, msg.sender, _nodeName);
