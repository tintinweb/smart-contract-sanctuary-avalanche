...
import "../interfaces/IHybrid.sol";


contract MigrateV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Meta;
    using SafeERC20 for IHybrid;

    event Converted(address indexed user, uint256 hybridAmount, uint256 v2Amount);
    event DeadlineUpdated(uint256 deadline);
    
    IHybrid public immutable hybrid;
    IERC20Meta public immutable hybridv2;
    uint256 public deadline;
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;

    constructor(address _hybrid, address _hybridv2, uint256 _deadline) {
        require(_hybrid != address(0), "Cannot be zero address");
        require(_hybridv2 != address(0), "Cannot be zero address");
        require(_deadline > 0, "Deadline = 0");
        hybrid = IHybrid(_hybrid);
    ...
    function getRate() public view returns (uint256) {
        uint256 hybridv2Amount = hybridv2.balanceOf(address(this));
        uint256 hybridSupply = hybrid.getCirculatingSupply();
        return hybridv2Amount.mul(1e18).div(hybridSupply);
    }
