...
contract SNCOracle is IEpoch, OwnableUpgradeSafe {
    using FixedPoint for *;
    ...
    modifier checkEpoch {
        ...
        require(epochPegPrice[_epoch] == 0 || block.timestamp >= nextEpochPoint(), "SNCOracle: not opened yet");
        _;
    }
    ...
    function initialize(
        ...
        require(reserve0 != 0 && reserve1 != 0, "SNCOracle: NO_RESERVES"); // ensure that there's liquidity in the pair
    ...
    function consult(address _token, uint256 _amountIn) public view returns (uint144 _amountOut) {
        ...
        if (_token == _token0) {
            ...
            require(_token == _token1, "SNCOracle: INVALID_TOKEN");
