    ...
    Authorizable
{
    ...
    ) ERC721A("Bad Biz Bones", "BBB") VRF(431, 100) {
    ...
    function isWhitelisted(
        ...
        if (
            MerkleProof.verify(proof, merkleRootT1, leaf) &&
            !claimed[user]
        ) {
            return 1;
        } else if (
            MerkleProof.verify(proof, merkleRootT2, leaf) &&
            !claimed[user]
        ) {
            return 2;
        } else if (
            MerkleProof.verify(proof, merkleRootT3, leaf) &&
            !claimed[user]
        ) {
            return 3;
        } else return 0;
