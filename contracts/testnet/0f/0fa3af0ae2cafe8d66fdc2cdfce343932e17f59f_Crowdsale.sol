...
// File: crowsaleLatestversion.sol
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    function checkBalance(uint _amount) private view returns(bool ) {
        return Dai.balanceOf(msg.sender) >= _amount;
    }

function checkApproval(uint _amount) private view returns(bool ) {
        return Dai.allowance(msg.sender,address(this)) >= _amount;
    }
    function _preValidatePurchase(address beneficiary, uint256 Amount) internal view {
        require(beneficiary != address(0), "Crowdsale: beneficiary is the zero address");
        require(Amount != 0, "Crowdsale: Dai amount cant be zero");
    ...
    /**
     * @dev Determines how DAi is stored/forwarded on purchases.
     */
    function _forwardFunds(uint _amount) internal {
        Dai.transferFrom(msg.sender,_wallet,_amount);
    }
}