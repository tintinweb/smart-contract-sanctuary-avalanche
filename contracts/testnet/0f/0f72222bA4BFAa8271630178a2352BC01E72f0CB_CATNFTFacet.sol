...
contract CATNFTFacet is Context {
	...
	* @dev this function is allowed to be called by user to start relaying tokens on all selected chain.
	...
	function initiateBridgeOutNFT(
		...
		nexaErc721.safeTransferFrom(_msgSender(), address(this), tokenId);
		if (!nexaErc721.isApprovedForAll(address(this), tokenAddress)) {
			nexaErc721.setApprovalForAll(tokenAddress, true);
		}
