...
contract Farm is Ownable, FarmCore {
    ...
    function deposit(
        ...
        require(farmServices[farmPairs[farmPools[_farmPoolId].farmPairId].farmServiceId].isActive, "FarmContract: No active Farm Services");
        require(farmServices[farmPairs[farmPools[_farmPoolId].farmPairId].farmServiceId].network == nativeNetwork, "FarmContract: Wrong network");
        ...
        if (isNew) {
            users[_msgSender()].deposits[depositCount] = Deposit(_amountFirst, _amountSecond, block.timestamp, _farmPoolId);
    ...
    function syncCrossChainDeposit(
        ...
        if (isNew) {
            users[_userAddress].deposits[depositCount] = Deposit(_amountFirst, _amountSecond, block.timestamp, _farmPoolId);
