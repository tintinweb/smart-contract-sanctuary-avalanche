...
contract nebulaFeeManager is Ownable{
	...
	_protoowners.push(PROTOOWNERS({name:_name,protoCreationDate:INTgetCurrentTime(_account),protoElapsed:Zero,protoCollapseDate:INTgetCurrentTime(_account).add(protoLife),protoNextDue:feePeriod,protoDueElapsed:Zero,protoFutureFees:uint256(11),protoIncreasedDecay:Zero,protoFeeFroze:_protoFeeFroze,protoTotalPayable:nebuLib.doMultiple(feePeriod,maxFeePayment.sub(feePeriod))}));
