...
// File @openzeppelin/contracts/utils/[email protected]4.7.2
...
contract NodeManager is Ownable, Pausable {
    ...
    function _calculateNodeRewards(uint _lastClaimTime, uint256 amount_,uint256[3] memory tier) public view returns (uint256 rewards) {
        uint256 elapsedTime_ = (block.timestamp - _lastClaimTime);
        uint256 boostMultiplier = _calculateBoost(elapsedTime_,validIndex,tier).div(100);
    ...
    function _calculateBoost(uint elapsedTime_, uint256 i, uint256[3] memory tier) public view returns (uint256) {
	...
	uint256[3] memory _tier = tier;
	uint _boost = 0;
        if (tier[0] >= i && tier[0] != 0) {
           _boost = 1;
            }
        else if (tier[1] >= i && tier[1] != 0) {
           _boost = 2;
           }
       else if (tier[2] >= i && tier[2] != 0) {
          _boost = 3;
           }
    	return _boost;
    ...
    function getNodeReward(address account, uint256 _creationTime,uint256[3] memory tier)
        ...
        returns (uint256)
    {
        ...
        return _calculateNodeRewards( node.lastClaimTime, node.amount,tier);
    }

    function getAllNodesRewards(address account,uint256[3] memory tier)
        external
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            rewardsTotal += _calculateNodeRewards( _node.lastClaimTime, _node.amount,tier);
        }
        return rewardsTotal;
    }
