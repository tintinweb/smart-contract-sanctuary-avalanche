...
contract ArableCollateral is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function restrictDeposits(address[] calldata tokens, bool[] calldata restricts) external onlyOwner {
        require(tokens.length == restricts.length, "Invalid");
        for (uint256 index = 0; index < tokens.length; index++) {
            depositRestricted[tokens[index]] = restricts[index];
        }
    }
