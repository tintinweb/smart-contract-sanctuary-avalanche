...
contract VUSDvault_withToken is Ownable {    
    ...
    uint256 public ERC20TokenPrice = 25 * 10 ** 18; // ERC20 Token Price
    ...
    {   
        require(msg.sender == deployer || msg.sender == Dev, "Invalid Caller");
        require(_rate >= 0 && _rate <= 1000, "Invalid percentage, the developer royalty must be between 0 and 100");
    ...
    function withdrawVUSD(uint256 _amount) public payable nonReentrant {
        ...
        uint256 ownerFeeAmount = _amount.mul(forERC20Convert).div(100); // 100
        ...
        uint256 setPrice = ERC20TokenPrice;
        if (IERC20(VUSD).balanceOf(address(this)).sub(ownerFeeAmount) >= 0) {
            uint256 amountInERC20Token = ownerFeeAmount.div(setPrice);
