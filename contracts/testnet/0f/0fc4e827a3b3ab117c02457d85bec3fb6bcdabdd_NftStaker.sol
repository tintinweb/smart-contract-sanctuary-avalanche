...
contract NftStaker {
    ...
    function stake(uint256[] memory _tokenIds) public {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            // staked[msg.sender].push(_tokenIds[i]);
        }
    }
    ...
    function orderedArray(uint index) public {
        for(uint i = index; i < stakedTokens[msg.sender].length-1; i++) {
            stakedTokens[msg.sender][i] = stakedTokens[msg.sender][i+1];
            }
        stakedTokens[msg.sender].pop();
    }

    // function updateStakedTokens() internal {
    //     delete stakedTokens[msg.sender];
    //     for (uint256 i = 0; i < staked[msg.sender].length; i++) {
    //         if (staked[msg.sender][i] != 0) {
    //             stakedTokens[msg.sender].push(staked[msg.sender][i]);
    //         }
    //     }
    // }
