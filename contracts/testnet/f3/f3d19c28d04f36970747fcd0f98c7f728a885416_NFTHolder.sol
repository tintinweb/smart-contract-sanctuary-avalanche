...
contract NFTHolder is Ownable, ReentrancyGuard, IERC721Receiver, INFTHolder {
    ...
    modifier isSalesAllowed(uint256 amount) {
        require(_saleWaves[_currentSaleWave].active, "NFTHolder: sales not active at the moment");
        require(amount > 0, "NFTHolder: specified amount should be more than zero");
        require(_allowedTokenIdsCounter >= _allowedTokenIdsTransferTracker + amount, "NFTHolder: requested amount exceed NFTs balance on contract");
        _;
    }

    modifier allowedERC20BuyToken(address erc20BuyToken) {
        require(_allowedErc20Tokens[erc20BuyToken], "NFTHolder: specified ERC20 is not allowed");
        _;
    }
    ...
    * Takes current wave price as purchase price
    ...
    modifier buyPriceCompliance(address erc20BuyToken, uint256 amount) {
        ...
        require(
            ...
            "NFTHolder: insufficient ERC20 token balance"
        );
        _;
    }
    ...
    * Allows to insert wave prices into sales wave
    ...
    function addSalesWaveTokenPrices(uint256 salesWaveId, SaleWaveTokenPrice[] calldata wavePrices) public onlyOwner {
        for(uint256 i = 0; i < wavePrices.length; i++) {
            require(_allowedErc20Tokens[wavePrices[i].token], "NFTHolder: specified token is not allowed.");
            require(wavePrices[i].price > 0, "NFTHolder: specified token price should be more than 0.");
    ...
    * Allows to add new sale wave
    ...
    function addWhitelistableSaleWave(
        ...
        require(whitelistClaimAllowance > 0, "NFTHolder: whitelist claim allowance should be more than 0");
    ...
    * should: receive existing and active sale wave
    ...
    function setCurrentSaleWave(uint256 saleWaveId) public onlyOwner {
        require(_saleWaves[saleWaveId].active, "NFTHolder: specified sale wave does not exists or not active");
    ...
    * should: specified token to be not allowed
    ...
    function allowERC20BuyToken(address erc20BuyToken) public onlyOwner {
        require(erc20BuyToken != address(0), "NFTHolder: invalid address specified");
        require(!_allowedErc20Tokens[erc20BuyToken], "NFTHolder: specified token already added");
    ...
    * should: specified token to be already allowed
    ...
    function disallowERC20BuyToken(address erc20BuyToken) public onlyOwner {
        require(erc20BuyToken != address(0), "NFTHolder: invalid address specified");
        require(_allowedErc20Tokens[erc20BuyToken], "NFTHolder: specified token does not exists");
    ...
    * if whitelist wave should: address to be whitelisted, address requested amount does not exceed allowance per wave
        ...
        buyPriceCompliance(erc20BuyToken, amount)
    {
        if(_saleWaves[_currentSaleWave].isWhitelistable) {
            ...
            require(MerkleProof.verify(_merkleProof, _saleWaves[_currentSaleWave].merkleRoot, leaf), "NFTHolder: address is not whitelisted.");

            uint256 requestedAmount = _saleWavesWhitelistClaimed[_currentSaleWave][_msgSender()] + amount;
            require(_saleWaves[_currentSaleWave].whitelistClaimAllowance >= requestedAmount, "NFTHolder: wave whitelist amount per address exceeded.");
        }
