...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => DQUserInfo)) dqOwnerInfo;
    ...
    */
    ...
    function setDQRarityLevels(address _collection, uint256[] calldata _tokenIds, uint256[] calldata _rarityLevels) external onlyOwner {
    ...
    */
    ...
    function setDQConfiguration(address _collection, uint256 _price, uint256 _duration, uint256 _withdrawDuration) external onlyOwner {
        dqPools[_collection].price = _price;
    ...
    function getBoostedGenerationRate(address _addr) external view returns (uint256) {
        if ((users[_addr].lastRelease + boostDuration[_addr]) > block.timestamp) {
            if ((users[_addr].lastRelease + dqBoostDuration[_addr]) > block.timestamp) {
                return veARTgenerationRate * 4 + veARTgenerationRate * 2;
            } else {
    ...
    */
    ...
    function stakeNFTToBoostVeART(address _collection, uint256[] calldata _tokenIds) external payable whenNotPaused nonReentrant {
        // ensures that the call is not made from a smart contract, unless it is on the whitelist.
        _assertNotContract(msg.sender);
        uint256 len = _tokenIds.length;
        uint256 price;
        ...
        for (uint256 i; i < len; ++i) {
            ...
            dqBoostDuration[msg.sender] += dqRarityLevels[_collection][_tokenIds[i]] * 3600;
            dqOwnerInfo[_collection][_tokenIds[i]].owner = msg.sender;
            dqOwnerInfo[_collection][_tokenIds[i]].endedAt = block.timestamp + dqPools[_collection].withdrawDuration;
            emit DqStake(msg.sender, _collection, _tokenIds[i], dqOwnerInfo[_collection][_tokenIds[i]].endedAt);
        }
        require(msg.value >= price * (100 * balanceOf(msg.sender)) / _totalSupply, "Insufficient payment provided for staking the NFT(s).");
    }

    function withdrawNFTFromBoostingPool(address _collection, uint256[] calldata _tokenIds) external whenNotPaused nonReentrant {
        _assertNotContract(msg.sender);
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
            require(dqOwnerInfo[_collection][_tokenIds[i]].owner == msg.sender, "The provided NFT does not belong to the sender");
            require(dqOwnerInfo[_collection][_tokenIds[i]].endedAt <= block.timestamp, "The provided NFT has not yet expired, and cannot be withdrawn from the boosting pool.");

            ISalvorMini(_collection).safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
            delete dqOwnerInfo[_collection][_tokenIds[i]];
    ...
    */
    ...
    /**
	* @dev Allows the contract owner to claim all earnings for a specific user's account in case of an emergency.
    * @param _receiver The address of the user whose earnings will be claimed.
    */
    function emergencyClaimEarnings(address _receiver) external onlyOwner {
        _claimAllEarnings(_receiver);
    }
