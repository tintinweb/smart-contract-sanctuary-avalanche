...
contract Token is ERC20, Owners {
	...
	modifier isAuthorized(address addr) {
		require(!isBlacklisted[addr], "Token: Blacklisted");
		_;
	}

	function mintDistribution(uint amount) external onlySuperOwner {
		_mint(distributionPool, amount);
	}

	// Setters
	function setDistribution(address _new) external onlySuperOwner {
	...
	function setTradingOpen(bool _value) external onlyOwners {
		tradingOpen = _value;
	}
		...
		isAuthorized(to)
	{
		if (!isOwner[from] && !isOwner[to])
			require(tradingOpen, "Token: Trading not open");
