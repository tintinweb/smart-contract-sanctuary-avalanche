...
contract ArkiusGovernanceToken is Ownable, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount, bool unlock) internal {
        ...
        if ( !unlock && ( arkiusValidated[recipient] || arkiusValidated[sender] )) {
            balances[sender]    -= amount;
            balances[recipient] += amount;

            if ( senderLockedBalance < amount ) {
                lockedBalances[sender]     = 0;
                lockedBalances[recipient] += senderLockedBalance;
            }
            else {
        ...
        //If it is a normal transaction, users can only transfer unlocked tokens.
            ...
            uint256 senderUnlockedBalance = senderBalance - lockedBalances[sender];
            require( amount <= senderUnlockedBalance, "ERC20: Not enough unlocked balance to transfer.");
