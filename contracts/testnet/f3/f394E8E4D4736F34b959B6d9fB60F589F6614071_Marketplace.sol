...
contract Marketplace is CollectionEnumerable, ReentrancyGuard, Ownable {
        ...
        if (paymentMethod == uint8(PaymentMethod.AVAX)) {
            if (nftType == uint8(NftType.ERC721)) {
                ...
                require(
                    ...
                    "Listing fee must be equal to listing fee in ERC721"
                );
            } else if (nftType == uint8(NftType.ERC1155)) {
                require(
                    msg.value == amount.mul(listingFee),
                    "Listing fee must be equal to listing fee in ERC1155"
                );
                require(
                    amount <=
                        IERC1155(nftContract).balanceOf(msg.sender, tokenId),
                    "Amount must not be greater than balance in ERC1155"
                );
            }
     ...
     */
    function cancelMarketItem(address nftContract, uint256 marketItemId)
        public
        nonReentrant
    {
    ...
    function createMarketSale(
        ...
        if (nftType == uint8(NftType.ERC721)) {
            ...
            if (listAmount - saleAmount > 0) {
                ...
                idToMarketItem[marketItemId].amount = saleAmount;
                idToMarketItem[marketItemId].sold = true;
                _itemsSold.increment();
            }

            if (paymentMethod == uint8(PaymentMethod.AVAX)) {
                payable(owner()).transfer(saleAmount.mul(listingFee));
            } else if (paymentMethod == uint8(PaymentMethod.ERC20)) {
                thorToken.transfer(owner(), saleAmount.mul(listingFee));
            }
        }
        idToMarketItem[marketItemId].owner = payable(msg.sender);
