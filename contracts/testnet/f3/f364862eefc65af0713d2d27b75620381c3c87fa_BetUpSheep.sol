...
contract BetUpSheep is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        require(IBetUpCore(betCoreAddress).isAvailableUpgrade(owner, _beforeLevel, _afterLevel), "condition is not met");

        IBetStruct.MintCondition memory  afterCondition = IBetUpCore(betCoreAddress).upgradeConditionToLevel(_afterLevel);

        uint256 perSpendAmount = uint256(afterCondition.amount) * 10 ** 18;
        ...
        IBetUpCore(betCoreAddress).recordPlayerConsumption(owner,
            afterCondition.count - IBetUpCore(betCoreAddress).upgradeConditionToLevel(_beforeLevel).count
        );
