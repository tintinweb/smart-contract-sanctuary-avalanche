...
contract Allrounder is IERC20 {
    ...
    uint public totalSupply = maxSupply.sub(LPDriverThreshold.add(MaxTxAmount)); // initial supply 0.9 bilion
...
constructor(address _routerAddress) {
   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    function swapBack() private swaping {
        ...
        if(expectedPairAmount > initialPairBalance){
            ...
            uint contractReserveBal = (_balances[address(this)].balance).sub(swapRequiredToken);
                if(contractReserveBal > mintAmount){
                    ...
                    supplyAmount = surplasAmount;
                    if(totalSupply.add(surplasAmount) > maxSupply){
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function uDriver(uint _driverFee, uint pegBoosterThreshold) external onlyManager{
        LPDriverThreshold = (maxSupply.mul(_driverFee)).div(_taxCalcDenominator);
        boosterThreshold = pegBoosterThreshold.mul(10**_decimals);
    ...
    function printerView() external  view returns(address , address){
        address _native = printer.nativeCoin();
        address _printToken = printer.printTokenAddress();
        return (_native, _printToken);
    }
