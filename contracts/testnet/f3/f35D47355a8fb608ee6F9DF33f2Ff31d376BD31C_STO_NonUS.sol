...
contract Crowdsale is Context, ReentrancyGuard {
   ...
   event nonUSFeeInfoSet(address sender,address _feeAddress,uint256 _fees,bool _feeStatus);
   event feeTransfered(address _feeAddress, uint256 _feeAmount );
   event feeEnabled(address _setterAddress, bool _feeStatus );
    ...
    function tokenFallback(
        ...
        emit TokensPurchased(_msgSender(), beneficiary, stableCoins, targetTokens);
         ...
         if(feeFlag)//if fee flag set we calculate fee and deduct it from the stable coin amount and transfer to fee address
         {
         console.log("feeflag",feeFlag);
          uint256 _feePercentage = (stableCoins * feeAmount)/100;
          console.log("fee percentage", _feePercentage);
         stableCoins = stableCoins - ( _feePercentage * 1 ether);
         console.log("stable amount transfered after fee",stableCoins);
          bool isTransfer = stableCoin_.transfer(feeAddress, _feePercentage);
          require(isTransfer);
          emit feeTransfered(feeAddress,_feePercentage );//update
         }//the rest of the stable coins are transfered 
    ...
    function updateUSSTOAddress(address _USSTO) internal returns (bool) {
      ...
      // Change set fees 
   function setNonUSSTOFeeInfo(address _feeAddress,uint256 _fee,bool feeStatus) public onlyAdmin{
        feeAddress = _feeAddress;
        feeAmount = _fee;
        feeFlag = feeStatus;
        emit nonUSFeeInfoSet(msg.sender,feeAddress,feeAmount,feeFlag);
    }

    function enableFee(bool feeStatus) public onlyAdmin {
        feeFlag = feeStatus;
        emit feeEnabled(msg.sender, feeStatus);
    }
...
interface ISTO{
   function rate() external view returns (uint256);
    ...
    function feeFlag() external returns(bool success);
}
    ...
    PostGoalCrowdsale
{
    ...
    */
     ...
     * @param newGoal new agoal of the STO
     */
