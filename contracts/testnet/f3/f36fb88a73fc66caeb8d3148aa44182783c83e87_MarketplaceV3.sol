...
contract MarketplaceV3 is Initializable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) public listings;
    ...
    function cancel(LibOrder.Order memory order, bytes memory signature) public whenNotPaused nonReentrant {
        address payable signer = payable(_validate(order, signature));
        bytes32 orderKeyHash = LibOrder.hashKey(order);
        require(msg.sender == signer, "not a maker");
        require(fills[orderKeyHash] != true, "order has already cancelled");
        require(order.maker != address(0), "collection address must be set");
        require(order.tokenId > 0, "tokenId must be set");
        address existingNftOwner = IERC721Salvor(order.maker).ownerOf(order.tokenId);
        require(existingNftOwner != address(0), "collection and tokenId don't be matched");

        if (order.isOffer) {
            uint256 amount = offers[msg.sender][order.maker][order.tokenId];
            offers[msg.sender][order.maker][order.tokenId] = 0;
            signer.transfer(amount);
        } else {
            uint256 price = listings[msg.sender][order.maker][order.tokenId];
            require(price > 0, "there is no any cancellable listing");
            listings[msg.sender][order.maker][order.tokenId] = 0;
            IERC721Salvor(order.maker).safeTransferFrom(address(this), signer, order.tokenId);
        }

        fills[orderKeyHash] = true;
    }

    function make(LibOrder.Order memory order, bytes memory signature) public whenNotPaused nonReentrant payable {
        address payable signer = payable(_validate(order, signature));
        bytes32 orderKeyHash = LibOrder.hashKey(order);
        require(msg.sender == signer, "not a maker");
        require(fills[orderKeyHash] != true, "order has already cancelled");
        require(order.maker != address(0), "collection address must be set");
        require(order.tokenId > 0, "tokenId must be set");
        require(order.price > 0, "price must be set");
        address existingNftOwner = IERC721Salvor(order.maker).ownerOf(order.tokenId);
        require(existingNftOwner != address(0), "collection and tokenId don't be matched");

        if (order.isOffer) {
            require(existingNftOwner != msg.sender, "could not offer to own nft");
            require(offers[msg.sender][order.maker][order.tokenId] == 0, "you have already an offer (code:01)");
            require(msg.value >= order.price, "Insufficient funds");

            offers[msg.sender][order.maker][order.tokenId] = order.price;
            address payable receiver = payable(address(this));
            receiver.transfer(order.price);
        } else {
            require(listings[msg.sender][order.maker][order.tokenId] == 0, "you have already a listing (code:01)");
            require(existingNftOwner == msg.sender, "nft does not belongs to seller");
            IERC721Salvor(order.maker).safeTransferFrom(msg.sender, address(this), order.tokenId);
            listings[msg.sender][order.maker][order.tokenId] = order.price;
        }
    }

    function showSigner(LibOrder.Order memory order, bytes memory signature) public view returns (address) {
        return _validate(order, signature);
    }

    function extractValue(LibOrder.Order memory order, bytes memory signature) public view returns (uint256) {
        address signer = _validate(order, signature);
        if (order.isOffer) {
            return offers[signer][order.maker][order.tokenId];
        } else {
            return listings[signer][order.maker][order.tokenId];
        }
    }
    ...
    function redeem(LibOrder.Order calldata order, bytes memory signature) public whenNotPaused nonReentrant payable returns (uint256) {
        ...
        uint256 tokenId;
        if (order.isOffer) {
            require(offers[signer][order.maker][order.tokenId] > 0, "there is no active offer coupon to redeem");
            require(order.tokenId > 0, "cannot redeem the coupon to make an offer (need to mint first)");
            buyer = signer;
            seller = sender;
            ownedBalance = offers[signer][order.maker][order.tokenId];
            offers[signer][order.maker][order.tokenId] = 0;
            if (listings[sender][order.maker][order.tokenId] > 0) {
                listings[sender][order.maker][order.tokenId] = 0;
            }
            IERC721Salvor(order.maker).safeTransferFrom(seller, buyer, order.tokenId);
        } else {
            require(listings[signer][order.maker][order.tokenId] > 0, "there is no active listing coupon to redeem");
            buyer = sender;
            seller = signer;
            ownedBalance = msg.value;
            if (order.tokenId > 0) {
                tokenId = order.tokenId;
                listings[signer][order.maker][order.tokenId] = 0;
                IERC721Salvor(order.maker).safeTransferFrom(address(this), buyer, order.tokenId);
            } else {
        ...
        require(ownedBalance >= order.price, "Insufficient funds to redeem");
    ...
    function withdrawAuction(address _nftContractAddress, uint256 _tokenId) external {
        ...
        require(IERC721Salvor(_nftContractAddress).ownerOf(_tokenId) == msg.sender, "Not NFT owner");
    ...
    function _setupSale(address _nftContractAddress, uint256 _tokenId, uint128 _buyNowPrice) internal {
    ...
    function createSale(address _nftContractAddress, uint256 _tokenId, uint128 _buyNowPrice)
    external
    isAuctionNotStartedByOwner(_nftContractAddress, _tokenId)
    priceGreaterThanZero(_buyNowPrice)
    {
        //min price = 0
        _setupSale(_nftContractAddress, _tokenId, _buyNowPrice);

        // emit SaleCreated(_nftContractAddress, _tokenId, msg.sender, _buyNowPrice);

        //check if buyNowPrice is meet and conclude sale
        if (_isABidMade(_nftContractAddress, _tokenId) && _isBuyNowPriceMet(_nftContractAddress, _tokenId)) {
            _transferNftToAuctionContract(_nftContractAddress, _tokenId);
            _transferNftAndPaySeller(_nftContractAddress, _tokenId);
        }
    }

    function makeBid(address _nftContractAddress, uint256 _tokenId) external payable auctionOngoing(_nftContractAddress, _tokenId) {
    ...
    function _createNewNftAuction(address _nftContractAddress, uint256 _tokenId, uint128 _minPrice, uint128 _buyNowPrice) internal minPriceDoesNotExceedLimit(_buyNowPrice, _minPrice) {
     ...
     */
    function createDefaultNftAuction(address _nftContractAddress, uint256 _tokenId, uint128 _minPrice, uint128 _buyNowPrice)
    external
    isAuctionNotStartedByOwner(_nftContractAddress, _tokenId)
    priceGreaterThanZero(_minPrice)
    {
        _createNewNftAuction(_nftContractAddress, _tokenId, _minPrice, _buyNowPrice);
    }

    //this is the time that the auction lasts until another bid occurs --> _auctionBidPeriod
    function createNewNftAuction(address _nftContractAddress, uint256 _tokenId, uint128 _minPrice, uint128 _buyNowPrice, uint32 _auctionBidPeriod, uint32 _bidIncreasePercentage)
    external
    isAuctionNotStartedByOwner(_nftContractAddress, _tokenId)
    priceGreaterThanZero(_minPrice)
    ...
    function _updateOngoingAuction(address _nftContractAddress, uint256 _tokenId) internal {
        if (_isBuyNowPriceMet(_nftContractAddress, _tokenId)) {
            _transferNftToAuctionContract(_nftContractAddress, _tokenId);
            _transferNftAndPaySeller(_nftContractAddress, _tokenId);
        } else if (_isMinimumBidMade(_nftContractAddress, _tokenId)) {
            _transferNftToAuctionContract(_nftContractAddress, _tokenId);
            _updateAuctionEnd(_nftContractAddress, _tokenId);
        }
    }
 ...
 */
    ...
    function _transferNftAndPaySeller(address _nftContractAddress, uint256 _tokenId) internal {
        ...
        _payout(_nftSeller, _nftHighestBid);
