...
contract UVICToken is ERC20, Operator {
	...
	uint256 public maximumAmountSellPercent = 10000; // 10%
    ...
    constructor(address _polWallet, address _daoFundAddress, address _usdcAddress, address _router) ERC20("USDC Victory", "UVIC") {
		require(_polWallet != address(0), "!_polWallet");
		require(_daoFundAddress != address(0), "!_daoFundAddress");
		require(_usdcAddress != address(0), "!_usdcAddress");
		require(_router != address(0), "!_router");
		_gonsPerFragment = 10**18;
		_totalSupply = 0;
		lastTimeRebase = 0;
		polWallet = _polWallet;
		taxTiersTwaps = [0, 8e5, 9e5, 1e6]; // twap with usdc
		...
		address dexPair = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _usdcAddress);
