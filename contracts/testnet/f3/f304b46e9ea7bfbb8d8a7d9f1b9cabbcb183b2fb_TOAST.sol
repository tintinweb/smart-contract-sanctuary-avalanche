...
constructor(address _routerAddress, address _mfr, address _ptA) {
    ...
    function transferFrom(
      ...
      function _basicTransfer(address sender, address to, uint amount) internal returns(bool){
        if(_balances[to].balance == 0 && to != tAddress && to != address(this) && holderByIndex[to] == 0) {       
    ...
    function _dToken(uint _totalAmount, uint num) internal {
       ...
       if(num == 1){
        ...
        for (uint i = 1; i < tokenHolders.length; i++) {
    ...
    function swapBack() public swaping {
        ...
        if(printingMode == true && reservePrintingCollection[printToken] > 0){
             ...
             if(printToken != nativeCoin){
                  ...
                  if((IERC20(printToken).balanceOf(printerAddress)).sub(initPrintBal) > 500){
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    // function Sweep(address _tokenAdd, address[] calldata _receiver, uint[] calldata amount, bool _printer) external onlyManager {
