...
contract SquidSwapExecutable is IAxelarExecutable, Ownable {
    ...
    function _refund(
        ...
        IERC20(tokenAddress).transfer(refundAddress, amount);
    }

    function _trade(bytes memory tradeData) internal returns (bool success) {
        (
            address tokenIn,
            uint256 amountIn,
            address router,
        ...
        if (tokenIn == address(0)) {
            if (amountIn == 0) revert InvalidTrade();
            if (amountIn > msg.value) revert InsufficientInput();
            ...
            if (amountIn == 0) {
                amountIn = allowance + balance;
            }
            if (balance + allowance < amountIn) revert InsufficientInput();
            if (balance < amountIn) {
    ...
    function tradeSendTrade(
        ...
        if (!_trade(tradeData1)) revert("TRADE_FAILED");
    ...
    function _receiveTrade(
        ...
        address token = gateway.tokenAddresses(symbol);
        if (token != tokenIn || (amountIn != 0 && amountIn != amount)) {
