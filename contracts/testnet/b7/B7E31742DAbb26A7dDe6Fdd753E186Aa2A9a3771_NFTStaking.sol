...
contract NFTStaking is Ownable, IERC721Receiver {
   using SafeMath for uint256;
    ...
    function deposit(uint256 _pid, uint256 _tokenId) external validatePoolByPid(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _tokenId) external validatePoolByPid(_pid) payable {
    ...
    function harvest(uint256 _pid, uint256 _tokenId) public payable validatePoolByPid(_pid) returns (bool success) {
        PoolInfo storage pool = poolInfo[_pid];
        NFTInfo storage nft = nftInfo[_pid][_tokenId];
        updatePool(_pid);
    ...
    function changePoolInfo(uint256 _pid, uint256 _totalAddReward, uint256 _endBlock, bool _withUpdate) external validatePoolByPid(_pid) onlyOwner {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.endBlock > block.timestamp);
        ...
        if (_endBlock > ZERO) {
            require(_endBlock >= block.timestamp);
