...
contract Staking is Ownable, IERC721Receiver, Pausable {
  ...
  function claimAll()
    external
  {
    require(_msgSender() == tx.origin, "Only Externally Owned Account");
    require(canClaim, "Claim is not currently possible");
    this.claimManyFromStaking(this.getTokensOf(_msgSender()),false);
  }
    ...
    external
  {
    require(_msgSender() == tx.origin, "Only Externally Owned Account");
    require(canClaim, "Claim is not currently possible");
    this.claimManyFromStaking(this.getTokensOf(_msgSender()),true);
  }
