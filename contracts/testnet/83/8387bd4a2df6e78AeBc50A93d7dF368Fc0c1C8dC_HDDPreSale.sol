...
contract HDDPreSale is Ownable, ReentrancyGuard {
    ...
    uint256 public constant HARD_CAP = 1_000 * 10**HDD_DECIMAL; // hardcap 1,000 HDD

    uint256 public priceToken = 5; // 5 USDC
    uint256 public minSpend = 50 * 10**BUYING_TOKEN_DECIMAL; // min: 50 USDC
    uint256 public maxSpend = 1000 * 10**BUYING_TOKEN_DECIMAL; // max: 1,000 USDC
    uint256 public startTime;
    uint256 public endTime;

    // Total HDD token user bought
    ...
    function buy(uint256 _amount) public nonReentrant {
        ...
        require(
            userDeposited[_msgSender()] + _amount >= minSpend,
            "Below minimum amount"
        );
        require(
            userDeposited[_msgSender()] + _amount <= maxSpend,
            "You have reached maximum spend amount per user"
        );

        uint256 tokenQuantity = (_amount / priceToken) *
            (HDD_DECIMAL - BUYING_TOKEN_DECIMAL);
        require(
            totalTokenSold + tokenQuantity <= HARD_CAP,
            "Token private sale hardcap reached"
        );
    ...
    function claim() external nonReentrant {
        ...
        for (
            ...
            if (block.timestamp >= timestamp) {
                tokenQuantity +=
                    (userBoughtAmount * claimablePercents[timestamp]) /
                    100;
