...
contract TerritoryUnitGameStatePredictionComponent {
    ...
    constructor () {
        ...
        _addAttacker(0, address(0), 0, 100, 200, 4);
        _addAttacker(0, address(0), 0, 200, 150, 3);
        _addAttacker(0, address(0), 0, 250, 150, 3);
        _addAttacker(0, address(0), 0, 105, 150, 3);
        _addAttacker(0, address(0), 0, 150, 150, 3);
    }
    ...
    function getWinningUnitFromTimestamp(uint _poolId, uint _timestamp) public view returns (uint) {
        ...
        for (uint n=0; n < _areaFightPools.length; n++) {
            if (attackersIndex[_areaFightPools[n]].predictedDeathTime < lastTimestamp && attackersIndex[_areaFightPools[n+1]].predictedDeathTime >= _timestamp) {
    ...
    function _update(uint _poolId) public returns (uint) {
        ...
        for (uint i=0; i < _attackersUniqueIds.length; i++) {
            ...
            deathScoreA = cal(finalPoolPossesorLp / attackersIndex[_attackersUniqueIds[i]].dmg);
            deathScoreB = cal(_lps[i] / attackersIndex[finalPoolPossesor[_poolId]].dmg);

            time = cal(attackersIndex[_attackersUniqueIds[i]].fightEntry);
            ...
            if (deathScoreB > deathScoreA) { // Attacker B win
                ...
                attackersIndex[finalPoolPossesor[_poolId]].killedByUniqueId = cal(_attackersUniqueIds[i]);

                _lps[i] = cal(((DECIMALS - ((deathScoreA * DECIMALS) / (deathScoreB))) * _lps[i]) / DECIMALS);

                finalPoolPossesor[_poolId] = cal(_attackersUniqueIds[i]); // update the final pool possesor (at this moment)
                finalPoolPossesorLp = cal(_lps[i]);
                ...
                attackersIndex[_attackersUniqueIds[i]].killedByUniqueId = cal(finalPoolPossesor[_poolId]);

                finalPoolPossesorLp = cal(((DECIMALS - ((deathScoreB * DECIMALS) / (deathScoreA))) * finalPoolPossesorLp) / DECIMALS);
            } else { // both loose
                attackersIndex[finalPoolPossesor[_poolId]].predictedDeathTime = time; // store the predicted death time value
                attackersIndex[finalPoolPossesor[_poolId]].killedByUniqueId = cal(_attackersUniqueIds[i]);
