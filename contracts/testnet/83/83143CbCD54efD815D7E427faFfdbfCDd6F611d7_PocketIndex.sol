...
contract PocketIndex {
    ...
    constructor() {
        ...
        indexLPToken = IIndexLPToken(
            0x9E7473dc7B1037671eaB5082b916b2223F63Eb18
        );
    }
    ...
    function bulkBuy() public {
        ...
        // Total new balance
        uint256 newBalance = baseTokenAddress.balanceOf(address(this));
        ...
        if (totalBulkBuys == 0) {
            ...
            for (uint256 j = 0; j < totalAssets; j++) {
                ...
                path[0] = address(assets[j].contractAddress);
                path[1] = address(baseTokenAddress);
                uint256[] memory result = router.getAmountsOut(
                    currentBalances[j],
                    path
                );
                currentWorth += result[1];
            }

            // Get current total lp supply
            uint256 currentLPSupply = indexLPToken.totalSupply();

            // Calculate how much new total LP should be
            // 1. new usdt and current usdt ratio
            uint256 ratio = currentWorth.div(newBalance);
            uint256 one = 1000000000000000000;
            uint256 difference = one.sub(ratio);
            uint256 newTotalLPSupply = currentLPSupply.div(difference);
            uint256 newLPTokenToMint = newTotalLPSupply.sub(currentLPSupply);
            indexLPToken.mint(newLPTokenToMint);
            ...
            for (uint256 i = 0; i < totalBuyers; i++) {
                uint256 userUsdtBalance = userBalances[nextBuyers[i]]
                    .usdtBalance;
                // Each asset
                for (uint256 j = 0; j < totalAssets; j++) {
                    ...
                    userBalances[nextBuyers[i]].balances[
                            assets[j].contractAddress
                        ] += assetBalanceOfContract.mul(userUsdtBalance).div(
                        100
                    );
                }
                // Send LP tokens to person
                uint256 userNewLp = newLPTokenToMint.mul(userUsdtBalance).div(
                    newBalance
                );
                indexLPToken.transfer(nextBuyers[i], userNewLp);
                // Empty usdt balance
                userBalances[nextBuyers[i]].usdtBalance = 0;
            }
        } else {
            // Note current balances of all assets
            indexLPToken.mint(newBalance);
            uint256 totalAssets = assets.length;
            ...
            for (uint256 i = 0; i < totalBuyers; i++) {
                ...
                // Send LP tokens to person
                uint256 userNewLp = newBalance.mul(userUsdtBalance).div(
                    newBalance
                );
                indexLPToken.transfer(nextBuyers[i], userNewLp);
                // Empty usdt balance
    ...
    function disolvePosition() public {
        ...
        for (uint256 i = 0; i < totalAssets; i++) {
            ...
            path[1] = address(baseTokenAddress);
            uint256 userBalance = userBalances[msg.sender].balances[
        ...
        baseTokenAddress.transfer(
            msg.sender,
            userUSDTBalance.sub(_managementFee).sub(_performanceFee)
        );
    ...
    function disolveWithLP(uint256 amount) public {
        ...
        uint256 ratio = amount.div(totalLp);
        ...
        // Loop and set asset balances to 0
        for (uint256 i = 0; i < totalAssets; i++) {
            uint256 userTokenSoldBalance = userBalances[msg.sender]
                .balances[assets[i].contractAddress]
                .mul(ratio);
            userBalances[msg.sender].balances[
                assets[i].contractAddress
            ] -= userTokenSoldBalance;
        }
