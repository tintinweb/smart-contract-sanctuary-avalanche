...
contract Token is Ownable, Pausable, ReentrancyGuard, ERC20 {
    ...
    /// If user withdraw before x hours after last deposit, he will pay 20% of fees
    uint256 public withdrawFees = 20;
    // 24h
    uint256 public feesDuration = 86400;
    ...
    function deposit(uint256 _amount) public whenNotPaused {
        ...
        if (_amount > 0) {
            ...
            addHighestStaker(msg.sender, finalDepositAmount);
    ...
    function withdraw(uint256 _amount) public whenNotPaused nonReentrant {
        ...
        if (_amount > 0) {
            ...
            myPool.lpToken.approve(address(msg.sender), user.amount);
    ...
    function removeHighestStaker(address _user) internal {
        ...
        for (uint i;i<arr.length;i++){
            if (arr[i].wallet == _user){
                arr[i].wallet = address(0);
                arr[i].amountStaked = 0;
                quickSort(0, arr.length-1);
                return;
            }
        }
