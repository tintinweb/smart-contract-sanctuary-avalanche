...
contract FarmBooster is Ownable {
    ...
    /// @notice Token pool BOOST_WEIGHT precision
    ...
    /// @param _token TOKEN token contract address.
    ...
    /// @notice Checks if the msg.sender is the token pool.
    modifier onlyTokenPool() {
        require(msg.sender == TOKEN_POOL, "onlyTokenPool: Not token pool");
        _;
    }
    ...
    /// @notice Tokenpool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in token pool.
    /// @param _lockedDuration user locked duration in token pool.
    /// @param _totalLockedAmount Total locked token amount in token pool.
    /// @param _maxLockDuration maximum locked duration in token pool.
    function onTokenPoolUpdate(
        ...
        emit UpdateTokenPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice token pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IERC20(TOKEN).balanceOf(TOKEN_POOL);

        uint256 totalLockedAmount = ITokenPool(TOKEN_POOL).totalLockedAmount();

        uint256 pricePerFullShare = ITokenPool(TOKEN_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((ITokenPool(TOKEN_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = ITokenPool(TOKEN_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = ITokenPool(TOKEN_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration token pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = ITokenPool(TOKEN_POOL)
            .userInfo(_user);
