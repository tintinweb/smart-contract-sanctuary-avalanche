...
import "./BondingTreasuryStorageV1.sol";

contract BondingTreasuryV2 is OwnableUpgradeable, BondingTreasuryStorageV1 {
    ...
    function bond(uint256 _amount) external virtual returns (bool) {
        require(
            EmeraldToken.balanceOf(msg.sender) >= _amount * (10**18) &&
                _amount * (10**18) <= bondMaxAmount,
        ...
        // minting token _amount*(10**18) to treasury account.
        EmeraldToken.mintSupply(treasuryAccount, _amount);
    ...
    function withdraw(uint256 _bondId) external virtual returns (bool) {
        ...
        EmeraldToken.approve(msg.sender, BondDetailsById[_bondId].amount);
        EmeraldToken.safeTransfer(
            address(this),
            msg.sender,
            BondDetailsById[_bondId].amount
        );
    ...
    function withdrawAll() external virtual returns (bool) {
        ...
        EmeraldToken.approve(msg.sender, tempAmt);
        EmeraldToken.safeTransfer(address(this), msg.sender, tempAmt);
    ...
    function buyDiscountTokens(uint256 _amount)
        external
        payable
        virtual
        returns (bool){
        ...
        require(
            RemainingAmount >= _amount * (10**18),
            "BondingTreasuryV1: Invalid amount"
        );

        DiscountDetails memory _data = DiscountDetails(
            msg.sender,
            (ClaimedAmount + _amount * (10**18)),
            block.timestamp
        );

        DiscountData[msg.sender] = _data;
        ...
        require(price == msg.value, "BondingTreasuryV1: Invalid price");

        payable(owner()).transfer(msg.value);

        lockCounter += 1;
        LockDetails memory data = LockDetails(
            lockCounter,
            msg.sender,
            _amount * (10**18),
            false,
        ...
        LockData[lockCounter] = data;
        lockIds[msg.sender].push(lockCounter);

        EmeraldToken.safeTransfer(owner(), address(this), _amount*(10**18));

        emit LockUp(lockCounter, msg.sender, _amount*(10**18), block.timestamp, block.timestamp + 600);
    ...
    function claim() external virtual returns(bool){
        uint256 _amount;
        for(uint256 i = 0; i < lockIds[msg.sender].length; i++){
            if(LockData[i].endedAt <= block.timestamp && !LockData[i].claimed){
                _amount += LockData[i].amount;
                LockData[i].claimed = true;
                tempId.push(LockData[i].id);
            }
        }
        require(_amount > 0 && lockIds[msg.sender].length > 0, "BondingTreasuryV1: No amount to claim");
        
        EmeraldToken.approve(msg.sender, _amount);
        EmeraldToken.safeTransfer(address(this), msg.sender, _amount);

        emit Claim(tempId, msg.sender, _amount);
        delete tempId;
    ...
    function viewUserDiscountedDetails(address _user)
        ...
        for (uint256 i = 0; i < UserBondDetails[_user].length; i++) {
            ...
            if (BondDetailsById[data.bondId].endedAt <= block.timestamp) {
                TotalAmount += (data.amount / bondPrice)* 10;
            }
        }
        ClaimedAmount = DiscountData[_user].amountClaimed;
        RemainingAmount = TotalAmount - ClaimedAmount;
    }

    function lockAmountForUser(address _user) external view returns(uint256 Amount){
        for(uint256 i = 0; i < lockIds[_user].length; i++){
            if(LockData[i].endedAt <= block.timestamp && !LockData[i].claimed){
                Amount += LockData[i].amount;
            }
        }
    }
...
contract BondingTreasuryStorageV1 is OwnableUpgradeable {
    ...
    mapping(address => DiscountDetails) public DiscountData;
    ...
    mapping(uint256 => LockDetails) public LockData;
     ...
     * @dev Emitted when bond is claimed by user.
     */
    event LockUp(uint256 LockId, address User, uint256 Amount, uint256 StartTimestap, uint256 EndTimstamp); 

    /**
     * @dev Emitted when bond is claimed by user.
     */
    event Claim(uint256[] LockIds, address User, uint256 Amount);
