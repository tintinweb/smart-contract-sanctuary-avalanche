 ...
 */
contract SingleStaking is ReentrancyGuard, Pausable, Multicall, Ownable2Step {
    ...
    function getInfo()
        external
        view
        returns (uint256 __totalSupply, Token memory _stakingToken, RewardToken[] memory _rewardTokens)
    {
        ...
        for (uint i; i < rewardTokens.length; i++) {
            _rewardTokens[i] = RewardToken({
                ...
                lastTimeRewardApplicable: lastTimeRewardApplicable(rewardTokens[i]),
