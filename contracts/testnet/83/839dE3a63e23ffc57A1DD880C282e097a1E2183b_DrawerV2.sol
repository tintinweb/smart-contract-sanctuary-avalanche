...
contract DrawerV2 is Ownable {
  ...
  function getTeamName(string memory name) internal pure returns (string memory teamName) {
    teamName = string(
      abi.encodePacked(
        ...
        '<text x="160" y="160" lengthAdjust="spacing" textLength="260" font-size="28px">',
        name,
        // "</div></foreignObject>"
        "</text>"
      )
  ...
  function getRoundSVG(
    ...
    for (; i < 14; i++) {
      svg = string(
        abi.encodePacked(svg, getTeamHead(x, y), getTeamName(teamNames[i]), getLine(i, x, y, scores[6], winScore))
      );
      x += 720;
    }
