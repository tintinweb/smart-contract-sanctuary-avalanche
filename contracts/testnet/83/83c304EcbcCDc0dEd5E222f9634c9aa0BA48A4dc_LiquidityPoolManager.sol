...
import "./implementations/GardenImplementationPointer.sol";

contract LiquidityPoolManager is
    Ownable,
    OwnerRecovery,
    GardenImplementationPointer
{
    ...
    address[] public feeAddresses = [
        address(0x123), // Treasury investments (65%)
        // 35% / 4 :
        address(0x456), // Dev 1 (8.75%)
        address(0x789), // Dev 2 (8.75%)
        address(0xaaa), // Dev 3 (8.75%)
        address(0xbbb) // Dev 4 (8.75%)
    ];
    uint16[] public feePercentages = [6500, 875, 875, 875];
        ...
        if (
            ...
            for (uint256 i = 0; i < feeAddresses.length; i++) {
                if ((feeAddresses.length - 1) == i) {
                    ...
                    uint256 calculatedFee = (totalLP * feePercentages[i]) / 10000; // fees percentage with 2 decimals
    ...
    function getFeePercentages() external view returns (uint16[] memory) {
...
abstract contract GardenImplementationPointer is Ownable {
