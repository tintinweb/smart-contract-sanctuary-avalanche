...
contract ArableManager is Ownable {
    ...
    function registerSynth(
        ...
        // TODO: token initial supply is not calculated on totalDebt and it should be cleared up
        ArableSynth synth = 
            new ArableSynth(tokenName, tokenSymbol, tokenInitialSupply, owner(), farming, exchange, collateral);
    ...
    function onAssetPriceChange(address asset_, uint256 oldPrice, uint256 newPrice) external {
        ...
        if (oldPrice < newPrice) {
            collateral.addToDebt(totalSupply * (newPrice - oldPrice));
        } else if (oldPrice > newPrice) {
            collateral.removeFromDebt(totalSupply * (oldPrice - newPrice));
        }
    }
}
