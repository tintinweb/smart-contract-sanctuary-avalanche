  ...
  VotingMachineWithProofs
{
  ...
  /// @dev address of the L1 VotingPortal contract
  address internal _l1VotingPortal;

  /**
   * @param crossChainManager address of the CrossChainManager contract deployed on current chain. This contract
   ...
   * @param rootsWarehouse address of the new RootsWarehouse contract
  ...
  function receiveCrossChainMessage(
    ...
    (IVotingPortal.MessageType messageType, bytes memory message) = abi.decode(
      messageWithType,
      (IVotingPortal.MessageType, bytes)
    );

    if (messageType == IVotingPortal.MessageType.Proposal) {
      (uint256 proposalId, bytes32 blockHash, uint24 votingDuration) = abi
        .decode(message, (uint256, bytes32, uint24));
      ...
      ) = abi.decode(message, (uint256, address, bool, address[]));
...
abstract contract VotingMachineWithProofs is IVotingMachineWithProofs, Ownable {
  ...
  /// @dev (proposalId => proposal information) stores the information of the proposals
  mapping(uint256 => Proposal) internal _proposals;

  /// @dev address of the rootsWarehouse
  IRootsWarehouse internal _rootsWarehouse;

  /// @dev address of the voting strategy
  IL2VotingStrategy internal _votingStrategy;

  /// @dev (proposalId => proposal vote configuration) stores the configuration for voting on each proposal
  mapping(uint256 => ProposalVoteConfiguration)
    internal _proposalsVoteConfiguration;

  /// @dev (voter => proposalId => voteInfo) stores the information for the bridged votes
  ...
  // TODO: provably needs better naming
  /// @inheritdoc IVotingMachineWithProofs
  function submitBridgedVote(
    ...
    require(
      ...
      'NO_BRIDGED_VOTE_TOKENS'
    );

    // check that the proofs are of the voter assets
    for (uint256 i; i < votingBalanceProofs.length; i++) {
      for (uint256 j; j < bridgedVote.votingTokens.length; j++) {
        require(
          votingBalanceProofs[i].underlyingAsset == bridgedVote.votingTokens[j],
          'EXPECTED_DIFFERENT_PROOFS'
        );
      }
    }
  ...
  function closeVote(uint256 proposalId) external {
    ...
    proposal.votingEndedTimestamp = _getCurrentTimeRef();
    ...
    returns (ProposalWithoutVotes memory)
  {
    ...
    ProposalWithoutVotes memory proposalWithoutVotes = ProposalWithoutVotes({
      ...
      votingEndedTimestamp: proposal.votingEndedTimestamp,
  ...
  function _submitVote(
    ...
    for (uint256 i = 0; i < votingBalanceProofs.length; i++) {
      balanceVotingPower = _rootsWarehouse.getStorage(
        ...
        slotOfAddressUint256Mapping(voter, votingBalanceProofs[i].slot),
  ...
  function _setRootsWarehouse(IRootsWarehouse newRootsWarehouse) internal {
    ...
    emit RootsWarehouseUpdated(address(newRootsWarehouse));
  }
  ...
  /**
   * @notice method to get the timestamp of a block casted to uint48
...
interface IVotingMachine is IBaseReceiverPortal {
  ...
  /**
   * @notice method to get the address of the CrossChainManager contract deployed on current chain
   * @return the CrossChainManager contract address
   **/
