...
abstract contract NonFungibleType is Ownable {
    ...
    mapping(uint256 => TypeNonFungible) private _typeNonFungible;
    mapping(uint256 => uint256) private _sold;

    function getTypeNFT(uint256 typeNFT) public view returns (TypeNonFungible memory) {
        return _typeNonFungible[typeNFT];
    }

    function getSold(uint256 typeNFT) public view returns (uint256) {
        return _sold[typeNFT];
    }

    function _setSold(uint256 _typeNFT, uint256 _quantity) internal {
        _sold[_typeNFT] = _sold[_typeNFT].add(_quantity);
        require(_sold[_typeNFT] <= _typeNonFungible[_typeNFT].limit, "NonFungibleType: Exceeded limit");
    }

    function setTypeNFT(uint256 _typeNFT, address _paymentToken, uint256 _price, uint256 _limit, uint256 _unboxQuantity) external onlyOwner {
        _setType(_typeNFT, _paymentToken, _price, _limit, _unboxQuantity);
    }

    function _setType(uint256 _typeNFT, address _paymentToken, uint256 _price, uint256 _limit, uint256 _unboxQuantity) internal {
        require(_sold[_typeNFT] <= _limit, "NonFungibleType: Exceeded limit");
        TypeNonFungible storage typeNonFungible = _typeNonFungible[_typeNFT];
        typeNonFungible.paymentToken = _paymentToken;
        typeNonFungible.price = _price;
        typeNonFungible.limit = _limit;
        typeNonFungible.unboxQuantity = _unboxQuantity;
        emit SetTypeNFT(_typeNFT, _paymentToken, _price, _limit, _unboxQuantity);
    }
}
...
contract TestGenesis is ERC4907, ERC721Burnable, Ownable, BusinessAddresses, Creator, NonFungibleType, TransferFee, ReentrancyGuard, Withdrawable {
    ...
    string private constant REVERT_INVALID_TYPE = "Genesis: Invalid type";
    string private constant REVERT_INSUFFICIENT_BALANCE = "Genesis: Insufficient balance";
    ...
    constructor() ERC4907("TestGenesis", "TG") {
