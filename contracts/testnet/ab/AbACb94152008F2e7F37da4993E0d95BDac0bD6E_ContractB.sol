...
interface SmeltContract {
    function extract(address, uint256, uint256) external returns (uint256);
    function getBurningRecord(address, uint256) external returns (uint256);
}

contract ContractB is ERC1155Drop, Permissions {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    mapping(address => bool) smelterWhitelist;
    ...
    constructor(
        ...
        address _royaltyRecipient,
        uint128 _royaltyBps,
        address _setupPrimarySaleRecipient
    ) ERC1155Drop(_name, _symbol, _royaltyRecipient, _royaltyBps, _setupPrimarySaleRecipient) {
    ...
    function smeltMint(address _smelterAddress, uint256 _id, uint _amount, bytes memory data) 
        public
    {
        require(smelterWhitelist[_smelterAddress], "NOT_IN_WHITELIST");

        SmeltContract smeltContract = SmeltContract(_smelterAddress);
        uint256 recordedBurn = smeltContract.getBurningRecord(msg.sender, _id);
        require(_amount <= recordedBurn, "Insufficient ore burnt");

        uint256 extractedAmount = smeltContract.extract(msg.sender, _id, _amount);
        
        _mint(msg.sender, _id, extractedAmount, data);
    }
    ...
    external onlyOwner
    {
        for (uint i = 0; i < toAddAddresses.length; i++) {
            smelterWhitelist[toAddAddresses[i]] = true;
        }
    }
    ...
    external onlyOwner
    {
        for (uint i = 0; i < toRemoveAddresses.length; i++) {
            delete smelterWhitelist[toRemoveAddresses[i]];
        }
    }
