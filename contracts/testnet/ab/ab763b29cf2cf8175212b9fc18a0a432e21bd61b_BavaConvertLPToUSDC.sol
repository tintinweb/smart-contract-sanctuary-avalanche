...
contract BavaConvertLPToUSDC is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function _liquidateLpToken(uint256 lpTokenId) private {
        ...
        if(lpAmount > 0) {
            uint balance0 = IERC20(IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token0()).balanceOf(address(lpTokenInfo[lpTokenLength].lpToken));
            uint balance1 = IERC20(IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token1()).balanceOf(address(lpTokenInfo[lpTokenLength].lpToken));

            uint _totalSupply = IPair(address(lpTokenInfo[lpTokenLength].lpToken)).totalSupply();     // gas savings, must be defined here since totalSupply can update in _mintFee
            ...
            (uint amountA, uint amountB) = lpTokenPair.router.removeLiquidity(IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token0(), IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token1(), lpAmount, amount0, amount1, address(this), block.timestamp+600);
            ...
            emit LiquidateLP(lpTokenInfo[lpTokenLength].lpToken, msg.sender, lpAmount, (liquidateAmountA + liquidateAmountB));
        }
    }
    ...
    function _convertTokentoUSDCE(uint256 lpTokenId, uint amount, uint token) private returns (uint) {
        address oriToken;
        if (token == 0) {
            oriToken = IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token0();
        } else if (token == 1) {
            oriToken = IPair(address(lpTokenInfo[lpTokenLength].lpToken)).token1();
        }
        // swap tokenA to USDC
        uint amountUSDCE;
