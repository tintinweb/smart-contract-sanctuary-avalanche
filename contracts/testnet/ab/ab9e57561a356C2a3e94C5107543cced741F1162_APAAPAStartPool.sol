...
contract APAAPAStartPool is
    Ownable,
    ReentrancyGuard,
    ERC20("APA APA Start Pool", "APA-APA")
{
    using SafeMath for uint256;
     ...
     * @notice Withdraw staked tokens and collect reward tokens
     * @param _amount: amount to withdraw (in rewardToken)
     * @param _caller: add 1 more param to call this function from other contract
     */
    function withdraw2(address _caller,uint256 _amount) external nonReentrant {
        UserInfo storage user = userInfo[_caller];
        // only check for Unstake token
         if (_amount > 0) {
            require(unStakingBlock <= block.number, "Unstaking has not started");     
         }
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            if (pending > 0) {
                // ERC20(rewardTokens[i]).transfer(address(msg.sender), pending);
                safeERC20Transfer(ERC20(rewardTokens[i]), address(_caller),pending);
            }
        }
        // If _amount > 0 ==> Unstake Token
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            _burn(address(_caller), _amount);
            _amount = collectFee(_amount, user);
            ERC20(stakedToken).transfer(address(_caller), _amount);
            //_burn(address(msg.sender),_amount);
        }
        // Harvest
        for (uint256 i = 0; i < rewardTokens.length; i++) {
