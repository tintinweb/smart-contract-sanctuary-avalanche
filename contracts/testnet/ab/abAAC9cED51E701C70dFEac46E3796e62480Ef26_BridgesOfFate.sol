    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules {
        ...
        PlayerItem[playerId].stage,
            GameMap[PlayerItem[playerId].stage],
            _checkSide(GameMap[_currentUserStage],PlayerItem[playerId].lastJumpSide),PlayerItem[playerId].lastJumpSide);

        // emit comment(GameMap[_currentUserStage],_checkSide(GameMap[_currentUserStage],_jumpSide),_jumpSide); 

   
    }
    // event comment (uint256 _randomNumber,bool Greater,bool lesser);
