...
contract Swapper {
    ...
    function swapUSDTforAVAX(uint256 _amount) public {
    ...
    function swapUSDTtoLP(uint256 _amount) external {
        ...
        splashToken.approve(address(PangolinRouter), _tokenAmount);
    ...
    function swapLPToUSDT(uint256 _amount) external {
        ...
        LPToken.approve(address(PangolinRouter), _amount);
        removeLP(_amount);
        splashToken.approve(address(PangolinRouter),splashToken.balanceOf(address(this)));
        swapSplashToUSDT(splashToken.balanceOf(address(this)));
        // splashToken.transfer(msg.sender, splashToken.balanceOf(address(this)));
    ...
    function swapLPToAVAX(uint256 _amount) private {
        ...
        splashToken.approve(address(PangolinRouter),splashToken.balanceOf(address(this)));
        swapSplashToUSDT(splashToken.balanceOf(address(this)));
        USDT.approve(address(PangolinRouter), USDT.balanceOf(address(this)));
        swapUSDTforAVAX(USDT.balanceOf(address(this)));
        sendAvaxToWell();
    }
    function swapUSDTBalanceToAvax() external {
        USDT.approve(address(PangolinRouter), USDT.balanceOf(address(this))+1);
        swapUSDTforAVAX(USDT.balanceOf(address(this)));
    }
    ...
    function sendAvaxToWell() private {
        ...
        assert(success);
        splashToken.transfer(theWell, splashToken.balanceOf(address(this)));
    }
    ...
    function swapSplashToUSDT(uint256 amount) internal {
        // SWAP split % of ETH input to token
        address[] memory path = new address[](2);
    ...
    function removeLP(uint256 _LPAmount) internal {
...
//theWellAddress : 0xdE314A2a782F27B6F42479d3fBE824395bC5be38
