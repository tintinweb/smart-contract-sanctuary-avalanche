...
contract RoundtableFactory is Multicall, Owned {
	event DAOdeployed(
  ...
  //////////////////////////////////////////////////////////////*/
	...
	constructor(
		address deployer,
		address payable roundtableMaster_,
...
contract RoundtableDAO is TableGovernance, ReentrancyGuard, Multicall, NFTreceiver {
	...
	function init(
		...
		proposalVoteTypes[ProposalType.PFP] = VoteType(0);
	}
	...
	function propose(
		...
		if (proposalType == ProposalType.MEMBER_THRESHOLD)
			if (amounts[0] < 1 || amounts[0] > 100) revert MemberLimitExceeded();
		...
		if (proposalType == ProposalType.TYPE)
			if (amounts[0] > 11 || amounts[1] > 4 || amounts.length != 2) revert TypeBounds();
	...
	function _vote(address signer, uint256 proposal) internal virtual {
		...
		if (
			proposalVoteTypes[prop.proposalType] == VoteType.MEMBER ||
			proposalVoteTypes[prop.proposalType] == VoteType.UNANIMOUS ||
			proposalVoteTypes[prop.proposalType] == VoteType.N_MINUS_ONE
		) weight = 1;
		else {
		...
		returns (bool didProposalPass, bytes[] memory results)
	{
		...
		if (didProposalPass) {
			...
			unchecked {
				...
				// Set proposal state, reduce active proposal count, and delete proposal now that it has been processed
			...
			// Only delete and update the proposal settings if there are not enough votes AND the time limit has passed - this prevents deleting proposals unfairly
			if (block.timestamp > prop.creationTime + votingPeriod + gracePeriod) {
	...
	function _mint(
		...
		if (id == TOKEN) {
			if (delegationExtension != address(0)) {
				...
				); // 1 indicates type 1 in delegator extension, this sets vote balance for minting and burning
			}
			totalSupply += amount;
		}
