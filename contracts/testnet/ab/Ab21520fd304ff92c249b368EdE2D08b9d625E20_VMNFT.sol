...
contract VMNFT is ReentrancyGuard, Ownable, ERC721Enumerable {
	...
	constructor(address _communityAddress, address _donationAddress, address _wavaxAddress, address _swapAddress) ERC721("Vending Machines NFT", "VMNFT") {
		paused = true;
		revealed = false;
		allowListings = true;
	...
	function claimMintRewardOfToken(uint256 tokenID) public nonReentrant {
		address ownerAddress = ownerOf(tokenID);
		require(_msgSender() == ownerAddress, "Only the owner of a token can claim rewards");
		...
		(bool success, ) = payable(ownerAddress).call{ value: reward }("");
		require(success, "AVAX Transaction: Failed to transfer funds");


		emit ClaimedRewards(ownerAddress, reward, 1);
	}

	function claimSalesRewards() public nonReentrant {
	...
	function claimSalesRewardOfToken(uint256 tokenID) public nonReentrant {
		address ownerAddress = ownerOf(tokenID);
		require(_msgSender() == ownerAddress, "Only the owner of a token can claim rewards");
		...
		emit ClaimedRewards(ownerAddress, reward, 2);
	}
