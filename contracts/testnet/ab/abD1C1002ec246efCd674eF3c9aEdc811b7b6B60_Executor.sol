...
contract Executor is ProposalValidator, IExecutor {
        ...
        )
    {
        ...
        // checkExecutionPeriod(executionPeriod_);
    ...
    /**
     * @notice Function enabling pending admin to become admin
    ...
    function setExecutionPeriod(uint256 _executionPeriod) public onlyTimelock {
        checkExecutionPeriod(_executionPeriod);
        executionPeriod = _executionPeriod;
    }

    /**
     * @notice Set propositionThreshold
     * @dev can be changed only via proposal
     * @dev value setted as tokens amount, by default in will be 1000 LOTTO tokens
    ...
    function setPropositionThreshold(uint256 _propositionThreshold)
        ...
        onlyTimelock
    {
        checkPropositionalTrasehold(_propositionThreshold);
    ...
    function setMinimumQuorum(uint256 _minimumQuorum) public onlyTimelock {
        checkMinimumQuorum(_minimumQuorum);
    ...
    function setVoteDifferential(uint256 _voteDifferential)
        ...
        onlyTimelock
    {
        checkVoteDifferential(_voteDifferential);
    ...
    function setVotingDuration(uint256 _votingDuration) public onlyTimelock {
        checkVoteDuration(_votingDuration);
     ...
     * @notice Function, called by Governance, that cancels a transaction, returns the callData executed
    ...
    function checkExecutionPeriod(uint256 executionPeriod_) internal pure {
        // Execution period - min 1w max unlim
        require(executionPeriod_ >= ONE_WEEK, "INVALID_EXECUTION_PERIOD");
    }
}
...
contract ProposalValidator is IProposalValidator {
    /// @notice propositionThreshold
    uint256 public propositionThreshold;
    /// @notice votingDuration
    uint256 public votingDuration;
    /// @notice voteDifferential
    uint256 public voteDifferential;
    /// @notice minimumQuorum
    uint256 public minimumQuorum;
    /// @notice lotto token contract
    ILotteryToken public lottoToken;
    /// @notice 1 week min value for vote duration
    uint256 public constant ONE_WEEK = 1 weeks;
    /// @notice 2 weeks max value for vote duration
    uint256 public constant TWO_WEEKS = 2 weeks;
    /// @notice total amount of LOTTO tokens
    uint256 private constant TOTAL_ALLOCATED = 2_000_000_000;
    /// @notice ONE_HUNDRED_WITH_PRECISION
    uint256 private constant ONE_HUNDRED_WITH_PRECISION =
        100_000_000_000_000_000_000; // Equivalent to 100%, but scaled for precision
     ...
     * - In ONE_HUNDRED_WITH_PRECISION units
     * @param lottoToken_ LottoToken address
     **/
    constructor(
        ...
        lottoToken = ILotteryToken(lottoToken_);

        // checkVoteDuration(_votingDuration);
        checkVoteDifferential(_voteDifferential);

        setPercentagePropositionalTrasehold(_propositionThreshold);
        votingDuration = _votingDuration;
        voteDifferential = _voteDifferential;
        minimumQuorum = _minimumQuorum;
    }
    ...
    function setPercentagePropositionalTrasehold(uint256 propositionThreshold_)
        internal
    {
        propositionThreshold =
            (propositionThreshold_ * ONE_HUNDRED_WITH_PRECISION) /
            lottoToken.getTotalSupply();
    }

    function checkPropositionalTrasehold(uint256 propositionThreshold_)
        internal view
    {
        uint256 maxGrowthRateLimit = propositionThreshold +
            ((propositionThreshold * 25_000_000_000_000_000_000) /
                ONE_HUNDRED_WITH_PRECISION);
        require(
            propositionThreshold_ <= propositionThreshold + maxGrowthRateLimit,
            "INVALID_PROPOSITIONAL_THRASEHOLD"
        );
    }

    function checkMinimumQuorum(uint256 minimumQuorum_) internal view {
        // Minimum quorum - 5% - minimum number, growth rate limit at 25%, and max 15%
        uint256 maxIncrease = minimumQuorum + minimumQuorum / 4;
        require(
            minimumQuorum_ >= 500 &&
                minimumQuorum_ <= 1500 &&
                minimumQuorum_ <= maxIncrease,
            "INVALID_MIN_QUORUM"
        );
    }

    function checkVoteDuration(uint256 votingDuration_) internal pure {
        // Vote duration - min 1w max 2w
        require(
            votingDuration_ >= ONE_WEEK && votingDuration_ <= TWO_WEEKS,
            "INVALID_VOTE_DURATION"
        );
    }

    function checkVoteDifferential(uint256 voteDifferential_) internal pure {
        // Vote differential - 0.5 - 15%
        require(
            voteDifferential_ >= 50 && voteDifferential_ <= 1500,
            "INVALID_VOTE_DIFFERENTIAL"
        );
    }
}
