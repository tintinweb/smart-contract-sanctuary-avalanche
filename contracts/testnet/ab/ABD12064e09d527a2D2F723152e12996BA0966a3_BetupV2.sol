...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    // @dev the level of reward coefficient :[0.25, 0.5, 1,2, 5, 10, 20, 40,80,200]
    // @dev must smaller than 30000
    uint16[] public rewardCoefficient;
    ...
    event UpdateRewardCoefficientEvent(
        uint16 before,
        uint16 last
    );
    ...
    function getRewardOf(
        ...
        IBetStruct.PlayerRound memory playerR = _players[keccak256(abi.encodePacked(_receiver, _id, _counter))];

        return _calcReward(
            IPairManager(pairManagerAddress).pairsCR(_id, _counter),
            playerR,
            _result,
            _receiver,
    ...
    function calcIncentiveBonus(uint64 _startTime, uint64 _endTime, uint256 _amount) internal view returns(uint256 bonus) {
    ...
    function trystack(uint256 _id, uint256 _amount, uint8 _direction) external {
        ...
        unchecked {
            uint256 bonus = calcIncentiveBonus(startStackTime, endStackTime, _amount);
    ...
    function updateRewardCoefficient(uint256 _ID, uint16 _rewardCoefficient) external onlyRole(REWARD_MANAGER)  {
        require(_rewardCoefficient > 0, "Invalid _rewardCoefficient");
        uint16 _historyCf = 0;
        uint256 len = rewardCoefficient.length;
        if(_ID <= len) {
            _historyCf =  rewardCoefficient[_ID];
            rewardCoefficient[_ID]  = _rewardCoefficient;
        }else if(_ID == len+1 ) {
            rewardCoefficient.push(_rewardCoefficient);
        }

        emit UpdateRewardCoefficientEvent(_historyCf, _rewardCoefficient);
    }

    function updateConfig(address[] memory _addrs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_addrs.length == 2, "length not match");
    ...
    */
    function _calcDaoAmountPerUnit(uint256 _level) private view returns(uint256, uint256) {
        ...
        uint256 number = (100 + rewardCoefficient[_level-1] * (_level - 1)) * 1e18;
        return (
            125 * number  / (100 * 100),
            75 * number  / (100 * 100)
        );
    ...
    private view returns(IBetStruct.RewardType memory reward)
    {
        ...
        uint256 snapshot = IMembershipCard(membershipAddress).playersSnapshot(_account);

        if(_maxLevel == 0) {
            require(
                IMembershipCard(membershipAddress).getLastIndexBits(snapshot) == 0,
                "Invalid _maxLevel");
        }else {
            require(
                IMembershipCard(membershipAddress).getLastIndexBits(snapshot) == 2 ** (256 - _maxLevel),
                "Invalid _maxLevel");
        }
    ...
    function _assembleAmount(
        uint256 _i,
        uint256[] memory _ids,
        uint256 _currentRefund,
        uint256 _accRecord,
        address _currentChip
    ) internal returns(uint256) {
        uint256 _accAmount = _accRecord;
        if(_ids.length == 1 && _currentRefund  > 0) {
            IERC20MetadataUpgradeable(_currentChip).safeTransfer(msg.sender, _currentRefund);

        }else {
            if(_i == 0) {
                _accAmount += _currentRefund;
            }else {
                address preChip = IPairManager(pairManagerAddress).pairsConfig(_ids[_i-1]).chip;
                if(preChip == _currentChip) {
                    _accAmount += _currentRefund;
                    if(_i == _ids.length - 1) { IERC20MetadataUpgradeable(_currentChip).safeTransfer(msg.sender, _accAmount);}

                }else {
                    uint256 _preAmount = _accAmount;
                    _accAmount = _currentRefund;
                    if(_preAmount  > 0) {
                        IERC20MetadataUpgradeable(_currentChip).safeTransfer(msg.sender, _preAmount);
                    }
                }
            }
        }
    ...
    function initialize(
        ...
        uint16[] memory _rewardCoefficient
    ) external initializer {
        require(_membershipAddress != address(0), "Invalid address");
