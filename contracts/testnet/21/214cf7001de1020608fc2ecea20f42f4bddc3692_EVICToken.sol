...
contract EVICToken is ERC20Upgradeable, OwnableUpgradeable {
	...
	uint256 private constant timeTaxAfterRebase = 24 hours;
	...
	event TransferOperator(address indexed account, address newOperator);
    ...
    function __Upgradeable_Init(address _polWallet) external initializer {
		...
		__ERC20_init("Ethereum Victory", "EVIC");
		taxTiersTwaps = [0, 8e17, 9e17, 1e18];
    	taxTiersRates = [2000, 1000, 500, 0];
		taxRateAfterRebase = 2000; // 20%
    ...
    function _transferOperator(address newOperator_) internal {
		...
		emit TransferOperator(msg.sender, newOperator_);
    }
    ...
    */
    ...
    function transferFrom(
		...
		if (autoCalculateTax) {
			...
			currentTaxRate = calculateTaxRate(currentTokenPrice);
		}
		if (currentTaxRate == 0 || excludedTaxAddresses[sender]) {
            _transfer(sender, recipient, amount);
        } else {
            _transferWithTax(sender, recipient, amount, currentTaxRate);
        }
	...
	function _transferWithTax(
        ...
        uint256 taxAmount = amount.mul(currentTaxRate).div(10000);
	...
	function setTaxTiersTwap(uint8 _index, uint256 _value) external onlyOwner returns (bool) {
    ...
    function setTaxTiersRate(uint8 _index, uint256 _value) external onlyOwner returns (bool) {
        require(_index >= 0, "Index has to be higher than 0");
	...
	function setTaxRateAfterRebase(uint256 _value) external onlyOwner returns (bool) {
		emit SetTaxRateAfterRebase(taxRateAfterRebase, _value);
        taxRateAfterRebase = _value;
        return true;
    }

	function excludeTaxAddress(address _address) external onlyOwner returns (bool) {
        require(!excludedTaxAddresses[_address], "Address can't be excluded");
        excludedTaxAddresses[_address] = true;
        return true;
    }

    function includeTaxAddress(address _address) external onlyOwner returns (bool) {
	...
	function isAddressExcluded(address _address) external view returns (bool) {
        return excludedTaxAddresses[_address];
    }
