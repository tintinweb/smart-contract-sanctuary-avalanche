    ...
    IERC721Metadata
{
    ...
    mapping(uint256 => uint256) private _tokenIdToNodeListIndex;
    // mapping(address => uint256) public lastCashoutAllTime;
    ...
    //     uint256 nodeIndex = _tokenIdToNodeListIndex[tokenId];
    //     if(_nodeList.length > 0)
    //     require(_tokenIdToNodeListIndex[tokenId] > 0, "NODE: Index Error");
    //     _;
    // }
    ...
    function _createNodeForAddress(
        ...
        emit CreateNode(owner, name_, creationTime, lastClaimTime);
        totalNodesCreated = _nodeList.length;
        _nodesOfUser[owner].push(totalNodesCreated);
        _tokenIds.increment();
        _nodeList.push(
            NodeEntity({
                owner: owner,
                name: name_,
                nodePos: _nodesOfUser[owner].length - 1,
        ...
        _tokenIdToNodeListIndex[_tokenIds.current()] = totalNodesCreated;
        _setOwner(owner);
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        require(_nodesOfUser[owner].length > 0, "no nodes to burn");
        ...
        uint256 nodeListIndex = _tokenIdToNodeListIndex[tokenId];
        uint256 nodesOfUserIndex = _nodeList[nodeListIndex].nodePos;

        _nodesOfUser[owner][nodesOfUserIndex] = _nodesOfUser[owner][
            _nodesOfUser[owner].length - 1
        ];
        _nodesOfUser[owner].pop();
        delete _nodeList[nodeListIndex];
    ...
    function _isNameAvailable(
        ...
        for (uint256 i = 0; i < nodesIndex.length; i++) {
            if (
                keccak256(bytes(_nodeList[nodesIndex[i]].name)) ==
    ...
    function _setOwner(address owner) private {
        ...
        nodeOwners[owner] = true;
        allNodeOwners.push(owner);
    }
    ...
    function createNodeForAddress(
        ...
        require(_isNameAvailable(owner, _name), "CN:Name not available");
        return
    ...
    function burn(uint256 tokenId) external indexAvailable(tokenId) {
        require(
    ...
    function _getRewardAmountOf(
        ...
        uint256[] memory nodesIndex = _nodesOfUser[account];
        for (uint256 i = 0; i < nodesIndex.length; i++) {
            totalReward += _calculateReward(nodesIndex[i]);
        }
        return totalReward;
    }
    ...
    function cashOutAllReward(
        ...
        for (uint256 i = 0; i < nodesIndex.length; i++) {
            totalReward += _cashOutReward(nodesIndex[i]);
        }
    ...
    function balanceOf(
        ...
        balance = _nodesOfUser[owner].length;
    }

    function ownerOf(
        uint256 tokenId
    ) public view override returns (address owner) {
        owner = _nodeList[_tokenIdToNodeListIndex[tokenId]].owner;
    }
    ...
    function safeTransferFrom(
        ...
        uint256 tokenId,
        bytes calldata data
    ) external virtual override indexAvailable(tokenId) {
        require(
    ...
    function transferFrom(
        ...
        // uint nodeListIndex = _tokenIdToNodeListIndex[tokenId];
        // _nodeList[nodeListIndex].owner 
        // // require()
        require(
    ...
    function _transfer(
        ...
        uint256 _nodeListIndex = _tokenIdToNodeListIndex[tokenId];
        uint256 _nodesOfUserIndex = _nodeList[_nodeListIndex].nodePos;

        _nodesOfUser[from][_nodesOfUserIndex] = _nodesOfUser[from][
            _nodesOfUser[from].length - 1
        ];
        _nodesOfUser[from].pop();
        _nodesOfUser[to].push(_nodeListIndex);
        _nodeList[_nodeListIndex].owner = to;
        _nodeList[_nodeListIndex].nodePos = _nodesOfUser[to].length - 1;
        _setOwner(to);
    ...
    function approve(
        address to,
        uint256 tokenId
    ) external override indexAvailable(tokenId) {
    ...
    function getApproved(
        uint256 tokenId
    ) public view override indexAvailable(tokenId) returns (address operator) {
    ...
    function tokenOfOwnerByIndex(
        ...
        require(index >=0 && index < _nodesOfUser[owner].length, 'index out of bound');
        return _nodesOfUser[owner][index];
    }

    function tokenByIndex(
        uint256 index
    ) external view override indexAvailable(index) returns (uint256) {
