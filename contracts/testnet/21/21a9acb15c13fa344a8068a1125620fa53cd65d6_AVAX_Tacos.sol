...
contract AVAX_Tacos {
    ...
    function ADDSALSA_ProfitDeposit() public payable{
        ...
        uint256 amount = msg.value;
        ADDSALSA_REWARDAMOUNT.add(amount);
    }
    ...
    function sellCrops() public{
        ...
        if (addSalsaReward>0 && block.timestamp.sub(user.lastWithdrawTime)>1 days){
            uint256 fee = addSalsaReward.div(200);
            payable(owner).transfer(fee);
            payable(msg.sender).transfer(addSalsaReward.sub(fee));
        }
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
	 ...
	 uint256 _totalWithdrawn, uint256 _referralEggRewards, uint256 _dailyCompoundBonus, uint256 _lastWithdrawTime, uint256 _addSalsaDeposit) {
