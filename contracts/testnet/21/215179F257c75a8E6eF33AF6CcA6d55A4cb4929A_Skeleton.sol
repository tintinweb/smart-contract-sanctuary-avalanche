...
contract Skeleton is ERC721Enumerable, Controllable, Pausable, Restricted {
    ...
    event onOrderSettled(address indexed recipient, uint256 indexed tokenId);
     ...
     */
    function mint(uint8 amount) external payable noReentrency whenNotPaused {
        ...
        storeMintOrder(_msgSender(), amount, seed);
    }
     ...
     */
    function mintWithNFT(uint256[] memory tokenIds) external whenNotPaused onlyEOA noReentrency {
    ...
    function storeMintOrder(address buyer, uint256 amount, uint256 seed) private {
        withdrawMoney();
    ...
    function core_mint(Order memory order) internal returns(uint256){
        ...
        emit onOrderSettled(recipient, ordersSettled);
