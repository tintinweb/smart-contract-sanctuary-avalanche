    ...
    ReentrancyGuard 
{
    ...
    constructor() ERC721("ALPHANodeNFT", "ALPHANODE") {
        ...
        _ownerShipTransferCooldown = 1 days;  //ownerShipTransferCooldown * 1 minutes;
    }
    ...
    * Token Minting.
    ...
    ) public minterOnly lessThanTotal returns (uint) {
        
        uint256 newItemId = _tokenIds.length;
        _tokenIds.push(newItemId);
    ...
    * Called by Node NFT Power manager contract;
     */
    function increasePowerLevel(uint256 power, uint256 tokenId) public powerControlOnly nonReentrant {
    ...
    function decreasePowerLevel(uint256 power, uint256 tokenId) public powerControlOnly nonReentrant {
    ...
    function resetPowerLevel(uint256 tokenId) public powerControlOnly nonReentrant {
        uint256 previousPowerLevel = alphaNodes[tokenId].currentPower;
    ...
    function updateName(string memory name, uint256 tokenId) public attributeManagerOnly nonReentrant {
    ...
    function updateTotalPaid(uint256 amount, uint256 tokenId) public attributeManagerOnly nonReentrant returns(uint256) {
    ...
    function updateIsEarning(bool earningStatus, uint256 tokenId) public powerControlOnly nonReentrant {
    ...
    *   Marketplace only function, auto sets to not earning if isForSale is true
     */
    function setIsForSale(bool isForSale, uint256 tokenId) public marketPlaceOnly nonReentrant {
    ...
    * Superadmin Only
    ...
    function setBaseURI(string memory baseURI_) external superAdminOnly nonReentrant {
        baseURI = baseURI_;
    }

    function changeMinter(address newAddress) public superAdminOnly nonReentrant {
    ...
    function changeAttributeManager(address newAddress) public superAdminOnly nonReentrant {
    ...
    function changePowerManager(address newAddress) public superAdminOnly nonReentrant {
    ...
    function changeMarketplaceAddress(address newAddress) public superAdminOnly nonReentrant {
    ...
    *   Change global transfer cooldown
     */
    function setGlobalOwnerShipTransferCooldown(uint256 numMinutes) public superAdminOnly nonReentrant {
    ...
    *   Change transfer cooldown for single token by id
     */
    function setOwnerShipTransferCooldownByTokenId(uint256 numMinutes, uint256 tokenId) public nonReentrant superAdminOnly tokenIDInRange(tokenId) {
        require(numMinutes > 1, "Number of minutes must be greater than 1");
    ...
    * Private Utils
    ...
    modifier lessThanTotal() {
        require(_tokenIds.length - 1 < total, "Can not mint any more of this series.");
        _;
    }

    modifier tokenIDInRange(uint256 tokenId) {
        require(tokenId <= total, "TokenId index is out of range.");
        _;
    }    
...
contract AlphaNFTAttributeManagement is AlphaNFTAuthorizations {
    ...
    /**
    *  Add, edit, remove Vanity attributes by token id
     */
