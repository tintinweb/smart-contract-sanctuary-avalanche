...
contract AvaxSavingNew is ReentrancyGuard, Context {
        ...
        returns (Deposit[] memory)
    {
        ...
        if (user.deposits[investmentIndex].finish > block.timestamp) {
    ...
    {
        (, , , totalCommunity) = getUserDownline(userAddress);
        ...
        for (uint256 i = 0; i < 3; i++) {
            for (
                ...
                stakingCommission = stakingCommission.add(
                ...
                stakingRewardCommission = stakingRewardCommission + scrNew;
                claimableCommision = claimableCommision.add(
                    (getUserClaimable(users[userAddress].downline[i][k]))
                );
                totalComission = stakingCommission.add(claimableCommision);
            }
        }
    }
        ...
        returns (uint256)
    {
        return
            getUserStakeCommision(userAddress)
                ...
                .add(users[userAddress].claimableProofShare);
    }
