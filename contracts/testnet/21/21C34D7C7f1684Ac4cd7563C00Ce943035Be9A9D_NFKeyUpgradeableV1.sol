...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function setLevel(uint tokenID, uint value) public onlyAdmin {
    ...
    function getZoneCommission(address account) public view returns (uint) {
        uint discount = 0;
        uint amountToken = this.balanceOf(account);

        if (_activeZone == 1) discount = 100;
        else if (_activeZone == 2) discount = 50;
        else if (_activeZone == 3) discount = 20;
        else if (_activeZone == 4) {
    ...
    function mintUniqueTokenTo(address account) public payable {
        WhitelistUpgradeableV1.WhitelistUser memory whitelistUser = WhitelistUpgradeableV1(_whitelistContractAddress).getAddressToWhitelist(account);

        require(_activeZone >= 5 || (_activeZone >= whitelistUser.zone && whitelistUser.whitelistAddress != address(0)), "NFKey: You need to be whitelisted");

        uint level = 1;
        if (whitelistUser.whitelistAddress != address(0) && !mintedAddressList[account]) level = whitelistUser.level;

        if (_activeZone == 1) require(!mintedAddressList[account] && (level >= 1 && level <= 10), "NFKey: Zone rejection");
        else if (_activeZone == 2) require(!mintedAddressList[account] && (level >= 1 && level <= 3), "NFKey: Zone rejection");
        else if (_activeZone == 3) require(level >= 1 && level <= 10, "NFKey: Zone rejection");
        else if (_activeZone == 4) require(level == 1, "NFKey: Zone rejection");
        else require(level == 1, "NFKey: Zone rejection");

        uint commission = getZoneCommission(msg.sender);
        if (commission > 0) payable(_wallet).transfer(commission);
...
contract WhitelistUpgradeableV1 is Initializable {
    ...
    mapping(address => bool) private adminAddressList;
    mapping(address => WhitelistUser) private whitelistedAddressList;
    ...
    function setZone(uint tokenID, uint value) public onlyAdmin {
    ...
    function getAddressToWhitelist(address account) public view returns (WhitelistUser memory) {
        return whitelistedAddressList[account];
    }

    function addAddressToWhitelist(WhitelistUser memory newUser) public onlyAdmin {
        whitelistedAddressList[newUser.whitelistAddress] = newUser;
    }

    function removeAddressFromWhitelist(address account) public onlyAdmin {
        whitelistedAddressList[account].whitelistAddress = address(0);
        whitelistedAddressList[account].level = 0;
        whitelistedAddressList[account].zone = 0;
    }
}
