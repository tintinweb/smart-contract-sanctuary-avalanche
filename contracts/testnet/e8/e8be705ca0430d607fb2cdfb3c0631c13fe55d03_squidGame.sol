...
contract squidGame is ERC721Enumerable, Ownable {
    ...
    uint256 private constant TURN_PERIOD = 50; //86400; // 24 HOURS
    ...
    constructor(
        ...
        seriesOne = IERC721(sOne);
        seriesTwo = IERC721(sTwo);
    }
    ...
    mapping(address => GameMember) public Player;
    mapping(uint256 => Admin) private adminInitialized;
    ...
    function initialize() public onlyOwner {
        ...
        _admin.startAt = block.timestamp;
        _admin.hybirdLevel = false;
        adminInitialized[adminCounter] = _admin;
        emit initialized(adminCounter, block.timestamp);
    }

    /*
        0 -- Indicate left side
        1 -- Indicate right side
    */
    function partitionInGame(bool _chooes_side)
    public
    after24Hours
    Resume
    GameInitialized
    {
        uint256 seriesOneBalance = seriesOne.balanceOf(msg.sender);
        uint256 seriesTwoBalance = seriesTwo.balanceOf(msg.sender);
        uint256 day_ = dayDifferance();
        //86400;
        GameMember memory _member = Player[msg.sender];
        if (seriesOneBalance > 0) {
            if (randomNumber <= 0) {
            ...
            if (day_ > 0) _member.day = day_;
            if (_member.stage > STAGES) _member.level += 1;
            // Here we use deposit function
            _member.stage += 1;
            _member.level = 1;
            _member.startAt = block.timestamp;
            _member.status = true;
            Player[msg.sender] = _member;
        }
    }
    ...
    function checkProgres() public returns (string memory, uint256) {
        uint256 period_differance = dayDifferance();
    ...
    function calculateBuyBackIn() public returns (uint256) {
        ...
        uint256 days_ = (block.timestamp.sub(_admin.startAt)).div(86400);
    ...
    modifier after24Hours() {
        GameMember memory _member = Player[msg.sender];
        uint256 calTimer = block.timestamp - _member.startAt;
        // Jump after 24 hours.
        require(calTimer >= 86400, "Jump after 1 mintues.");
        _;
    }

    modifier Resume() {
        GameMember memory _member = Player[msg.sender];
        require(_member.status, "Player is Resume Status.");
        _;
    }
    ...
    modifier GameInitialized() {
        ...
        Admin memory _admin = adminInitialized[adminCounter];
        require(
            block.timestamp >= _admin.startAt,
