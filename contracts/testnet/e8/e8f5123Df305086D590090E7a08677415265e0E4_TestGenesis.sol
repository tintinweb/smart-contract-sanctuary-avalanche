...
abstract contract NonFungibleType is Ownable {
    event SetTypeNFT(uint256 typeNFT, address paymentToken, uint256 price, uint256 limit);
    ...
    mapping(uint256 => TypeNonFungible) internal _typeNonFungile;
    ...
    function _setSold(uint256 _typeNFT, uint256 _quantity) internal {
        ...
        require(_sold[_typeNFT] <= _typeNonFungile[_typeNFT].limit, "NonFungibleType: Exceeded limit");
    }

    function setTypeNFT(uint256 _typeNFT, address _paymentToken, uint256 _price, uint256 _limit) external onlyOwner {
        _setType(_typeNFT, _paymentToken, _price, _limit);
    }

    function _setType(uint256 _typeNFT, address _paymentToken, uint256 _price, uint256 _limit) internal {
        ...
        emit SetTypeNFT(_typeNFT, _paymentToken, _price, _limit);
    }
}
...
contract TestGenesis is ERC721, ERC721Burnable, Ownable, BusinessAddresses, Creator, NonFungibleType, TransferFee, ReentrancyGuard, Withdrawable {
    ...
    constructor() ERC721("TestGenesis", "TG") {
        ...
        _setType(1, 0x0000000000000000000000000000000000000000, 100000000000000000, 500);
