...
contract SmashMarblesArenaV3 is SmashMarblesStorageV1 {
    using StringsUpgradeable for uint256;

    uint256 public marbleForArenaClaim;
    uint256 public claimedMarbleCount;
    uint public whitelistAddressCount;
    // event AirDropArena(address[] users, uint256[] tokenIds);
    ...
    /**
     * @dev change marble limit that available for mint and claim
     *
     * @param _marbleSaleCount amount of marbles that use in whitelist and public sale
     *
     * Requirement : -
     *              only contract owner can call
     */
    function changeMarblesCount(
        uint256 _marbleSaleCount,
        uint256 _marbleForArenaClaim
    ) external virtual onlyOwner {
        maxNFTs = _marbleSaleCount;
        marbleForArenaClaim = _marbleForArenaClaim;
    }
        ...
        isblacklistedAddresses
    {
        ...
        require(
            (noOfTokens + tokenCounter - claimedMarbleCount - 500) <= maxNFTs,
    ...
    function claimNft(uint256 _id) external virtual isblacklistedAddresses {
        ...
        require(
            claimedMarbleCount < marbleForArenaClaim,
        ...
        onlyOwner
    {
        ...
        delete tokenArray;
        require(
            (_addresses.length + tokenCounter - claimedMarbleCount - 500) <=
                maxNFTs,
    ...
    // function airDropArena(address[] calldata _addresses)
    //     external
    //     virtual
    //     onlyOwner
    // {
    ...
    //         rewardClaimed[ArenaCounter] = _data;
    //     }
    //     emit AirDropArena(tempAddress, tokenArray);
    // }
