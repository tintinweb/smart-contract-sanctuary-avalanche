...
interface ITournament {
    ...
    function SIDEPOTSIZE() external view returns (uint256); // Total amount of sidepot token
...
contract TournamentERC20 is Ownable, ITournament {
    ...
    uint256 public SIDEPOTSIZE;
    ERC20Burnable public rewardToken;
    ERC20Burnable public SIDEPOT;
    uint256 finalPotSize;

    constructor ( address _token, address _sidePot, uint256 _initialPeriodSeconds, uint256 _periodSeconds, uint256 _decaySeconds, uint256 _decayStopSeconds, uint256 _TotalBurn, uint256 _TotalWinnings, uint256 _ticketSize, uint256 _potShare, uint256 _burnShare, address _treasury) {
        ...
        SIDEPOT = ERC20Burnable(_sidePot);
    }
    ...
    function _getSidePot() internal view returns (uint256) {
        return SIDEPOT.balanceOf(address(this));
    }
    ...
    function setInitialPeriod(uint256 _INITIALPERIOD) external onlyOwner {
    ...
    function buy() external payable {
        ...
        SIDEPOTSIZE = SIDEPOT.balanceOf(address(this));
    ...
    function claimWinner() external {
        ...
        SIDEPOT.transfer(winner, SIDEPOTSIZE);
