...
contract Lockup is Ownable {
    ...
    function getBoost(int128 duration, uint256 amount) internal view returns (uint8) {
        if(duration < 0 && amount < 100 * 10 ** 7 * 10 ** 18) return 0;
        else if (duration < 0 && amount < 500 * 10 ** 7 * 10 ** 18  && amount >= 100 * 10 ** 7 * 10 ** 18) return tieA;      // irreversable
        else if (duration < 0 && amount <= 1000 * 10 ** 7 * 10 ** 18) return tieB;      // irreversable
        else if (duration < 0 && amount <= 5000 * 10 ** 7 * 10 ** 18) return tieC;      // irreversable
        else if (duration < 0 && amount > 5000 * 10 ** 7 * 10 ** 18) return tieD;      // irreversable
    ...
    function _dealWithIrreversibleAmount(uint256 amount, string memory name) private {
        require(amount >= 100 * 10 ** 7 * 10 ** 18, "invalid amount for node");
        // for saving gas fee
        swapBack(amount);
        bytes32 key = _string2byte32(name);
        StakeInfo storage info = stakedUserList[key];
