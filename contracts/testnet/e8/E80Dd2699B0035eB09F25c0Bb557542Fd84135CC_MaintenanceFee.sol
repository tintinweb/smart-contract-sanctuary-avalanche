    ...
    AccessControlUpgradeable
{
    ...
    function initialize(
        ...
        REWARDS_PER_DAYS = 1 * 10**17; // 0.1 ValiFi Tokens per Day
    ...
    function payMaintenanceFee(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                maintFee[_tokenIds[i]].unClaimedDays + _paidDays[i] <= LIMITS_DAYS,
    ...
    /**
     * @dev This Method permit set the Principal Variable of Rewards like REWARDS_PER_DAYS, REWARDS_PERCENTAGE, LIMITS_DAYS
     * @param _value Array Value of REWARDS_PER_DAYS, REWARDS_PERCENTAGE, LIMITS_DAYS
     */
    function setValue(uint256[3] memory _value) external onlyOwner {
        require(
            _value[0] > 10**15 &&
                _value[1] >= 100 &&
                _value[2] >= 100,
        ...
        REWARDS_PERCENTAGE = uint8(_value[1]);
     ...
     */
    function setResetMaintenanceFee(uint256 tokenId)
        external
        onlyRole(ROLE_RESET_MAINTENANCE_FEE)
    {
        uint256 amount;
        maintFee[tokenId].status = false;
        maintFee[tokenId].lastOwner = _msgSender();
        maintFee[tokenId].unClaimedDays += 0;
        uint256 yieldKeyType = maintFee[tokenId].yieldKeyType;
        uint256 yieldKey = maintFee[tokenId].yieldKey;
        if (yieldKeyType == 12) {
            amount = _yieldBoxYieldKeyLeg[yieldKey].length;
            for (uint256 i = 0; i < amount; i++) {
                if (_yieldBoxYieldKeyLeg[yieldKey][i] == tokenId) {
                    _yieldBoxYieldKeyLeg[yieldKey][i] = _yieldBoxYieldKeyLeg[
                        yieldKey
                    ][amount - 1];
                    _yieldBoxYieldKeyLeg[yieldKey].pop();
                }
            }
        } else if (
            yieldKeyType == 8 || yieldKeyType == 6 || yieldKeyType == 4
        ) {
            amount = _yieldBoxYieldKeyReg[yieldKey].length;
            for (uint256 i = 0; i < amount; i++) {
                if (_yieldBoxYieldKeyReg[yieldKey][i] == tokenId) {
                    _yieldBoxYieldKeyReg[yieldKey][i] = _yieldBoxYieldKeyReg[
                        yieldKey
                    ][amount - 1];
                    _yieldBoxYieldKeyReg[yieldKey].pop();
                }
            }
        }
        emit ResetMaintenanceFee(tokenId, yieldKeyType, yieldKey);
        maintFee[tokenId].yieldKey = 0;
        maintFee[tokenId].yieldKeyType = 0;
    }
     ...
     * Yield Key ---> Yield Box
     */
		function yieldBoxYieldKeyLeg(uint256 _yieldKey) public view returns (uint256[] memory) {
     ...
     * Yield Key ---> Yield Box
     */
		function yieldBoxYieldKeyReg(uint256 _yieldKey) public view returns (uint256[] memory) {
