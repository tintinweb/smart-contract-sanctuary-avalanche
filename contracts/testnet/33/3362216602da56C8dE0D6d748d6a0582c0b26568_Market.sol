...
contract Media is IMedia, ERC721Burnable, ReentrancyGuard {
     ...
     */
    constructor(address marketContractAddr) public ERC721("Glacier", "GLC") {
    ...
    function BidsbyTokenId(uint256 tokenId)
        public
        view
        returns (address[] memory)
    {
        return IMarket(marketContract).BidsbyTokenId(tokenId);
    }
    ...
    function endMarketing(uint256 tokenId)
        public
        nonReentrant
        onlyApprovedOrOwner(msg.sender, tokenId)
    {
        IMarket(marketContract).endMarketing(tokenId);
    }
...
interface IMarket {
    ...
    function endMarketing(uint256 tokenId) external;
    ...
    function BidsbyTokenId(uint256 tokenId) external view returns (address[] memory);
}
...
contract Market is IMarket {
    ...
    mapping(uint256 => address[]) private tokenBids;
        ...
        onlyMediaCaller
    {
        ...
        require(_tokenAsks[tokenId].amount == 0, "Auction not end");
    ...
    function setBid(
        ...
        tokenBids[tokenId].push(bid.bidder);
        ...
        onlyMediaCaller
    {
        ...
        removeBidArraybyAddress(tokenId, bidder);
    }

    function removeBidArraybyAddress(uint256 tokenId, address bidder) internal {
        uint256 index = 1000000000;
        for (uint256 i = 0; i < tokenBids[tokenId].length; i++)
            if (bidder == tokenBids[tokenId][i]) {
                index = i;
                break;
            }
        if (index == 1000000000) return;

        for (uint256 i = index; i < tokenBids[tokenId].length - 1; i++) {
            tokenBids[tokenId][i] = tokenBids[tokenId][i + 1];
        }
        delete tokenBids[tokenId][tokenBids[tokenId].length - 1];
    ...
    function endMarketing(uint256 tokenId) external override onlyMediaCaller {
        address maxaddress;
        uint256 maxPrice = 0;
        for (uint256 i = 0; i < tokenBids[tokenId].length; i++)
            if (
                maxPrice < _tokenBidders[tokenId][tokenBids[tokenId][i]].amount
            ) {
                maxPrice = _tokenBidders[tokenId][tokenBids[tokenId][i]].amount;
                maxaddress = tokenBids[tokenId][i];
            }
        if (maxaddress == address(0)) return;
        Bid memory bid = _tokenBidders[tokenId][maxaddress];
        require(bid.amount > 0, "Market: cannot accept bid of 0");
        require(
            isValidBid(tokenId, bid.amount),
            "Market: Bid invalid for share splitting"
