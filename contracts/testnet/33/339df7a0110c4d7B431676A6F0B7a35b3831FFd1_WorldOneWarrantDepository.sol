...
contract WorldOneWarrantDepository is Ownable {
    ...
    event WarrantPriceChanged(uint256 indexed price);
    event InitWarrantLot(WarrantLot terms);
    event LogSetFactory(address factory);
    ...
    address public dexFactory; // Factory address to get pair address
    ...
    uint8 public constant conversionRate = 1;
    ...
    function deposit(
        ...
        uint256 _amount = _warrantAmount.mul(nativePrice).div(
            10 ** WorldOne.decimals()
        );
        ...
        treasury.deposit(
            _warrantAmount.mul(10 ** WorldOne.decimals()),
            address(principle),
            fee
        );
        ...
        _depositorWarrantIds[_depositor].length == 0
            ? warrantId_
            : warrantId_ += _depositorWarrantIds[_depositor][lastIndex];
        warrantInfo[_depositor][warrantId_] = Warrant({
            payout: warrantInfo[_depositor][warrantId_].payout.add(
                _warrantAmount
            ),
    ...
    function updateWarrantLot(
        ...
        if (warrantLotIndex > 0) {
            ...
            require(
                currentWarrantLot().consumed !=
                    currentWarrantLot().totalCapacity &&
                    _totalCapacity.mul(10 ** WorldOne.decimals()) >
                    currentWarrantLot().consumed,
                "Warrant lot already in progress"
            );
        }
    ...
    function redeem(
        ...
        require(canAvailableClaim(_recipient, warrantId_), "Cannot redeem");
        delete warrantInfo[_recipient][warrantId_]; // delete user info
        for (uint i = 0; i < _depositorWarrantIds[_recipient].length; i++) {
            // delete user warrant id
            if (_depositorWarrantIds[_recipient][i] == warrantId_) {
                while (i < _depositorWarrantIds[_recipient].length - 1) {
                    _depositorWarrantIds[_recipient][i] = _depositorWarrantIds[
                        _recipient
                    ][i + 1];
                    i++;
                }
    ...
    function allCost() public view returns (uint256) {
        return
            remainingAvailable().mul(warrantPrice()).div(
                10 ** WorldOne.decimals()
            );
    }
    ...
    function payoutFor(uint256 _value) public view returns (uint256) {
        uint256 valueInWei = _value.mul(1e18).div(1e9);
        return (valueInWei.mul(1e9)).div(warrantPrice());
    }
    ...
    function canAvailableClaim(
        ...
        return
            block.timestamp >=
            info.purchasedAt.add32(warrantLots[info.warrantLotID].vestingTerm);
    }
     ...
     *  @return uint
     */
    function getLatestDepositingWarrantId(
        address _depositor
