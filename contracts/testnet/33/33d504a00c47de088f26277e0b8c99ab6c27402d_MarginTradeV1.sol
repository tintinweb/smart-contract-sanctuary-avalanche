...
contract MarginTradeV1 is Ownable {
    ...
    function createPositionandTrade(
        ...
        path[0] = tokenAddress;
        path[1] = targetToken;
        if (tokenAddress == address(0)) path[0] = uniswapV2Router.WAVAX();
        ...
        returns (uint256, uint256)
    {
        ...
        if (path[0] == uniswapV2Router.WAVAX()) {
            // Trade and get asset out
            result = uniswapV2Router.swapExactAVAXForTokens{value: tradeAmount}(
                0,
                path,
                address(this),
                deadline
            );
        } else if (path[1] == uniswapV2Router.WAVAX()) {
            // Approve token in
            IERC20(path[0]).approve(address(uniswapV2Router), tradeAmount);
            // Trade and get asset out
            result = uniswapV2Router.swapExactTokensForAVAX(
                tradeAmount,
                0,
                path,
                address(this),
    ...
    function closePosition(uint256 _positionID) public payable {
        ...
        if (path[1] == address(0)) path[1] = uniswapV2Router.WAVAX();
        // Store trade result
