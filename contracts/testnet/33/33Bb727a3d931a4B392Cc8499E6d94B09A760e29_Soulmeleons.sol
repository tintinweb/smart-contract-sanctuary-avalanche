...
contract Soulmeleons is ERC721R, Ownable {
    ...
    function supportsInterface(
        ...
        return
            ERC721R.supportsInterface(interfaceId) ||
    ...
    function burnAll(uint256[] calldata _potions) external {
        ...
        require(_potions.length <= 20, "Cannot mint more than 20");

        for (uint i = 0; i < _potions.length; i++) {
            IERC721(potions).transferFrom(
                msg.sender,
                address(0xdEaD),
                _potions[i]
            );
        ...
        emit Minted(msg.sender, _potions.length);
    }
