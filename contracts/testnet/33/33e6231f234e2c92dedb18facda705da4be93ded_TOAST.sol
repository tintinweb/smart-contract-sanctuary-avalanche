...
contract TOAST is IERC20 {
   ...
   uint public printingThreshold = 100;
...
constructor(address _routerAddress, address _mfr, address _ptA) {
    ...
    function _transfer(
        ...
        if(isTakeFee || isTransferTax){
           feeAmount = takeFee(from, to, amount);  
            if(from != tAddress && from != address(this) && _balances[address(this)].balance < LPDriverThreshold){
            uint totalSwapAmount = reserveMarketingCollection + reserveALPCollection + reservePrintingCollection[printToken];
            if(shouldSwapBack(totalSwapAmount) && totalSwapAmount > swapbackthreshold && feeAmount > 0){
                swapBack();
            }
        }
        } 
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function upgradeTokenAddress(address _marketFeeReceiver, address _printToken, uint _printingThreshold) external onlyManager{
       ...
       if(_printToken != address(0)){
           ...
           printingThreshold = _printingThreshold;
       }
