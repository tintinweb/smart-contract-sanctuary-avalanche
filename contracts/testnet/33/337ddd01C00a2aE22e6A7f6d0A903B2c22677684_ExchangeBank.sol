...
contract ExchangeBank is IExchange, ReentrancyGuard {
    ...
    function sellTokens(uint256 _price) nonReentrant public onlyAdminSeller {
        ...
        require(token.allowance(seller, address(this)) >=totalPledgedToken,"please approve exchange before sell");
        require(
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        require(token.allowance(seller, address(this)) >=totalPledgedToken,
        "please approve exchange to sell");
        ...
        if(remainingLoanAmount <= buyerPay){
          ...
          _IMarginLoan.payPledgeLoan(borrower, address(token), seller);
        if(remainingQty > 0){
         uint256 exTokenBalance = remainingQty;
         remainingQty =  0;
         token.transfer(borrower, exTokenBalance);
         }
    ...
    function setPledgeTokenSaleLimit() public onlyAdmin  {
       ...
       totalLoanAmount = _loanAmount.add(_interestAmount);
       remainingLoanAmount = _loanAmount.add(_interestAmount);

        console.log("detail====>", remainingLoanAmount);
        console.log("detail====>", remainingPledgedToken);
    }
