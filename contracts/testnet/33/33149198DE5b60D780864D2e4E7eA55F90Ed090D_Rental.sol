...
contract Rental is Withdrawable {
    ...
    modifier isOwnerOf(address game, uint256 tokenId) {
        require(_isOwnerOf(game, tokenId), REVERT_NOT_A_OWNER_NFTS);
        _;
    }
    ...
    function rent(uint256 orderId) external isNotRenting(orderId) {
        ...
        IERC4907(rental.nftAddress).setUser(rental.nftId, account, uint64(block.timestamp + rental.period));
    ...
    function _setListRental(
        ...
        require(_rentalInfo[orderId].renter == address(0) || _rentalInfo[orderId].renter == _msgSender(), REVERT_NOT_A_OWNER_NFTS);
        require(period > 0, REVERT_INVALID_PERIOD);
        _rentalInfo[orderId] = RentalInfo(nftAddress, tokenId, _msgSender(), address(0), paymentToken, price, period);
    ...
    function listRental(
        ...
        uint256 price,
        uint256 period
    ) external isOwnerOf(nftAddress, orderId) {
