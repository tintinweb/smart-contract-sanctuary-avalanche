    ...
    IPack
{
    ...
    function setQuackAddress(address _quackAddr) external onlyRoleWithSwitch(minterRole) { 
    ...
    function openPack(uint256 _packId, uint256 _amountToOpen) external returns (Token[] memory) {
        ...
        if (quackIDsRemaining.length > 0) {
            ...
            require(targetQuack < quackIDsRemaining.length);
            quackIDsRemaining[targetQuack] = quackIDsRemaining[quackIDsRemaining.length-1];
            quackIDsRemaining.pop();
            quacksAddress.safeTransferFrom(address(this), msg.sender, targetQuack);
        }
