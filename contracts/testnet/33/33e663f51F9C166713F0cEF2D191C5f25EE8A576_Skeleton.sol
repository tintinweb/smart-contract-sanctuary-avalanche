...
contract Skeleton is ERC721Enumerable, Controllable, Pausable, Restricted {
    ...
    event onOrderPlaced(address indexed buyer, uint8 amount, bool stake);
    event onOrderSettled(address indexed recipient, uint256 indexed tokenId, bool stake);
     ...
     */
    function mint(uint8 amount, bool stake) external payable noReentrency whenNotPaused {
        ...
        emit onOrderPlaced(_msgSender(), amount, stake);
    ...
    function core_mint(Order memory order) internal returns(uint256){
        ...
        emit onOrderSettled(recipient, ordersSettled, order.stake);
    ...
    function setconstInAdventurers(uint8 _costInAdventurers) external onlyController {
        costInAdventurers = _costInAdventurers;
    }

    function setconstInHunters(uint8 _costInHunters) external onlyController {
