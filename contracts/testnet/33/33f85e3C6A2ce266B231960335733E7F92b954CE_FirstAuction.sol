...
contract FirstAuction is Ownable {
    string public temporal = "HELLO WORLD FROM HARDHAT VERIFY VERIFICATION!";
    ...
    function redeem() public {        
        ...
        uint256 totalSupplymLaunchToken = IERC20(mlaunchToken).balanceOf(address(this));
        require(totalSupplymLaunchToken >= redeemAmount, "Not enough balance for redeem");
        InvestorInfo storage investor = investorInfoMap[msg.sender];
        require(!investor.claimed, "already claimed");
        require(
            IERC20(mlaunchToken).transfer(
                msg.sender,
                redeemAmount
            ),
            "transfer failed"
    ...
    function setmLaunchToken(address _mlaunchToken) public onlyOwner {
        mlaunchToken = _mlaunchToken;
    }

    function depositmLaunchtoken(uint256 amount) public onlyOwner {
        require(
            IERC20(mlaunchToken).transferFrom(msg.sender, address(this), amount),
            "transfer failed"
        );
    }

    // withdraw in case some tokens were not redeemed
    function withdrawmLaunchtoken(uint256 amount) public onlyOwner {
        require(
            IERC20(mlaunchToken).transfer(msg.sender, amount),
            "transfer failed"
        );
    }
    ...
    function enableRedeem() public onlyOwner { 
        require(mlaunchToken != address(0), "launch token not set");
        redeemEnabled = true;
