...
contract ConsumableItem is ERC1155, Ownable {
    ...
    // Consumable Items prices in USD (format: $4.99 = 499 * 10^16)
    mapping(uint256 => uint256) public pricesUSD;

    // amount of Consumable Items minted individually (addtress => item's ID => amount)
    mapping(address => mapping(uint256 => uint256)) public mintedByPlayer;

    // if Consumable Item used within the game (addtress => item's ID => used or not)
    ...
    function addItem() external onlyOwner {
        types.push(types.length);
    }
