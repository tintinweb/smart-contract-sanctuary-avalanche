...
contract Registry is IRegistry, AccessControlledAndUpgradeable {
  ...
  function registerPoolMarketContract(
    ...
    IMarketCommon.PoolInfo[] memory launchPools
  ) external adminOnly {
    ...
    require(
      IMarketExtended(address(marketContract)).initializePools(
        IMarketExtended.InitializePoolsParams(
          launchPools,
  ...
  */
  ...
  function updateSystemStateMulti(uint32[] calldata marketIndexes) external override {
    ...
    for (uint256 i = 0; i < length; i++) {
      IMarketTieredLeverage(separateMarketContracts[marketIndexes[i]]).updateSystemState();
    }
  }
  ...
  function executeOutstandingNextPriceSettlementsUserMulti(
    ...
    uint256 length = marketIndexes.length;
    for (uint256 i = 0; i < length; i++) {
      IMarketTieredLeverage(separateMarketContracts[marketIndexes[i]])
        .executeOutstandingEpochSettlementsUser(user);
    }
  }
