/**
 *Submitted for verification at testnet.snowtrace.io on 2022-11-25
*/
...
/// @title Cosmo Gold for Loot holders!
/// @author Will Papper <https://twitter.com/WillPapper>
/// @notice This contract mints Cosmo Gold for Loot holders and provides
/// administrative functions to the Loot DAO. It allows:
/// * Loot holders to claim Cosmo Gold
/// * A DAO to set seasons for new opportunities to claim Cosmo Gold
/// * A DAO to mint Cosmo Gold for use within the Loot ecosystem
/// @custom:unaudited This contract has not been audited. Use at your own risk.
contract CosmoGold is Context, Ownable, ERC20 {
    ...
    // Give out 10,000 Cosmo Gold for every Loot Bag that a user holds
    uint256 public CosmoGoldPerTokenId = 10000 * (10**decimals());
    ...
    */
    ...
    // tokenIdEnd of 1800 is based on the following lines in the Loot contract:
    /**
    ...
    */
    uint256 public tokenIdEnd = 1800;
    ...
    constructor() Ownable() ERC20("Cosmo Gold", "XAUc") {
    ...
    /// @notice Claim Cosmo Gold for a given Loot ID
    ...
    /// @notice Claim Cosmo Gold for all tokens owned by the sender
    /// @notice This function will run out of gas if you have too much loot! If
    /// this is a concern, you should use claimRangeForOwner and claim Cosmo
    /// Gold in batches.
    ...
    /// @notice Claim Cosmo Gold for all tokens owned by the sender within a
    /// given range
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that Cosmo Gold have not already been claimed this season
        // for a given tokenId
        require(
        ...
        // Mark that Cosmo Gold has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send Cosmo Gold to the owner of the token ID
        _mint(tokenOwner, CosmoGoldPerTokenId);
    }
    ...
    /// @notice Allows the DAO to set a season for new Cosmo Gold claims
    /// @param season_ The season to use for claiming Loot
    function daoSetSeason(uint256 season_) public onlyOwner {
        season = season_;
    }

    /// @notice Allows the DAO to set the amount of Cosmo Gold that is
    /// claimed per token ID
    /// @param CosmoGoldDisplayValue The amount of Loot a user can claim.
    ...
    /// 100 * 10^18.
    function daoSetCosmoGoldPerTokenId(uint256 CosmoGoldDisplayValue)
        public
        onlyOwner
    {
        CosmoGoldPerTokenId = CosmoGoldDisplayValue * (10**decimals());
    }

    /// @notice Allows the DAO to set the season and Cosmo Gold per token ID
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Cosmo Gold than others
    /// @param season_ The season to use for claiming loot
    /// @param CosmoGoldDisplayValue The amount of Loot a user can claim.
    /// This should be input as the display value, not in raw decimals. If you
    /// want to mint 100 Loot, you should enter "100" rather than the value of
    ...
    /// CosmoGold variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndCosmoGoldPerTokenID(
        uint256 season_,
        uint256 CosmoGoldDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetCosmoGoldPerTokenId(CosmoGoldDisplayValue);
    }
}