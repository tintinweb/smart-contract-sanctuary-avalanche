...
contract BridgesOfFate is Ownable {
    ...
    function _overFlow(bytes32 _playerId,uint256 jumpAtStage,bool _jumpSide) internal returns(bool)  {
        ...
        emit StageTileInComment(_playerId);
    ...
    event StageTileInComment(bytes32 player);
    event comment5(uint256 _rn,bytes32 player);
    event Missing(uint256 _RN);
    ...
    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  {
        ...
        if(totalNFTOnTile[GameStatusInitialized[latestGameId].stageNumber][_jumpSides] < playerIds.length){
            uint256 _range;
            if(totalNFTOnTile[PlayerItem[playerIds[0]].stage + 1][_jumpSides] > 0){
                _range = totalNFTOnTile[PlayerItem[playerIds[0]].stage + 1][_jumpSides] ;
            }else{
                _range = 3;
        ...
        for (uint256 i = 0; i < playerIds.length; i++) {
            ...
            for (uint j = 0; j < playerIndex.length; j++) {
                ...
                if(playerIndex[j] > 0 && (i == playerIndex[j]) ){
                    ...
                    emit StageTileInComment(playerIds[playerIndex[j]]);
        ...
        for (uint256 i = 0; i < playerIndex.length; i++) {
           if(playerIndex[i] > 0){
                ...
                emit comment5(playerIndex[i] ,playerIds[playerIndex[i]]);
            }
