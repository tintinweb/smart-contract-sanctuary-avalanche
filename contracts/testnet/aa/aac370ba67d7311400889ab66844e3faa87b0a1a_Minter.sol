...
contract Minter is Ownable {
     ...
     *  @return price_ uint
     */
    function waceoPriceInWAVAX() public view returns ( uint256 price_, uint256 _wavaxValue, uint256 _waceoValue ) {
          _wavaxValue = IERC20(Wavax).balanceOf(WAVAX_WACEO_LP).div(10** IERC20(Wavax).decimals()).mul( 10** IERC20(Waceo).decimals());
          _waceoValue = IERC20(Waceo).balanceOf(WAVAX_WACEO_LP);
        price_ = _wavaxValue.div(_waceoValue).mul( 10** IERC20(Waceo).decimals());
    }
     ...
     *  @return price_ uint
     */
    function wavaxPriceInUSD() public view returns ( uint256 price_ , uint256 _wavaxValue, uint256 _stableValue) {
          _wavaxValue = IERC20(Wavax).balanceOf(WAVAX_STABLE_LP).div(10** IERC20(Wavax).decimals()).mul( 10** IERC20(Waceo).decimals());
          _stableValue = IERC20(Stable).balanceOf(WAVAX_STABLE_LP).div(10** IERC20(Stable).decimals()).mul( 10** IERC20(Waceo).decimals());
        price_ = _stableValue.div(_wavaxValue).mul( 10** IERC20(Waceo).decimals());
    }
