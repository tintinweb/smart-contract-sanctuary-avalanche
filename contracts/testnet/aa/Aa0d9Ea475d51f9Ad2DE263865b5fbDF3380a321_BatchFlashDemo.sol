...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
  ...
  returns (bool)
  {
    ...
    if (req.BorrowAsset != address(0) && req.BorrowAmount != 0) {
      ...
      flashBorrow(lendingPool, req.BorrowAsset, req.BorrowAmount);

      // repays the 'BorrowAmount' mount of 'BorrowAsset' to unlock the collateral
      flashRepay(lendingPool, req.BorrowAsset, req.BorrowAmount);
    }
   ...
   */
  function rugPullAsset(address asset) public onlyOwner {
   ...
   */
  function rugPullEth(address asset) public onlyOwner {
