...
interface IIssuedSynthAggregator {
    ...
    function sendExchange(
        bytes32 issueCurrencyKey,
        uint256 issueAmount,
        address issueAccount,
        bytes32 burnCurrencyKey,
        uint256 burnAmount,
        address burnAccount
    ) external;
...
contract IssuedSynthAggregator is Owned, MixinSystemSettings, IIssuedSynthAggregator {
    ...
    bytes32 private constant CONTRACT_SYNTH_AGGREGATOR = "SynthAggregator";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[2] = CONTRACT_SYNTH_AGGREGATOR;
    ...
    function synthAggregator() internal view returns (ISynthAggregator) {
        return ISynthAggregator(requireAndGetAddress(CONTRACT_SYNTH_AGGREGATOR));
    }
    ...
    function totalIssuedSynths() external view returns (uint256 _issuedSynths) {
        ...
        for (uint256 ii = 0; ii < availableSynths.length; ii++) {
            ...
            if (currencyKey == sUSD) {
                uint256 totalSynths = synthAggregator().totalSupply(currencyKey);
    ...
    function totalIssuedSynthsPerIssuer(address account) external view returns (uint256 _issuedSynths) {
        ...
        for (uint256 ii = 0; ii < availableSynths.length; ii++) {
            bytes32 currencyKey = availableSynths[ii];
            if (currencyKey == sUSD) {
                uint256 totalSynths = synthAggregator().balanceOf(currencyKey, account);
                if (totalSynths > 0) {
                    _issuedSynths = _issuedSynths.add(totalSynths);
                }
            } else {
    ...
    function totalIssuedSynthPerAsset(bytes32 currencyKey) external view returns (uint256 _issuedSynth) {
        if (currencyKey == sUSD) {
            _issuedSynth = synthAggregator().totalSupply(currencyKey);
        } else {
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = synthAggregator().totalSupply(currencyKey);
    ...
    {
        if (currencyKey == sUSD) {
            _issuedSynth = synthAggregator().balanceOf(currencyKey, account);
        } else {
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = synthAggregator().balanceOf(currencyKey, account);
            _issuedSynth = totalSynths.multiplyDecimal(currencyRate);
    ...
    function sendIssue(
        ...
        synthAggregator().sendIssue(currencyKey, account, amount);

        emit SynthIssueMsgToAggregator(currencyKey, account, amount);
    }
    ...
    function sendBurn(
        ...
        synthAggregator().sendBurn(currencyKey, account, amount);

        emit SynthBurnMsgToAggregator(currencyKey, account, amount);
    }
    ...
    function sendExchange(
        ...
        synthAggregator().sendExchange(issueCurrencyKey, issueAmount, issueAccount, burnCurrencyKey, burnAmount, burnAccount);

        emit SynthExchangeMsgToAggregator(issueCurrencyKey, issueAmount, issueAccount, burnCurrencyKey, burnAmount, burnAccount);
    }
    ...
    function sendTransferFrom(
        ...
        synthAggregator().sendTransferFrom(currencyKey, from, to, amount);

        emit SynthTransforFromMsgToAggregator(currencyKey, from, to, amount);
    }

    /* ========== MODIFIER ========== */
    modifier onlyInternalContract() {
        require(_onlyInternalContract(msg.sender), "No internal contract");
        _;
    }

    function _onlyInternalContract(address account) private view returns (bool) {
        return
            account == resolver.getAddress("Issuer") ||
            account == resolver.getAddress("Exchanger") ||
            issuer().synthsByAddress(account) != bytes32(0);
    }

    /* ========== EVENTS ========== */
    event SynthIssueMsgToAggregator(bytes32 indexed currencyKey, address account, uint256 amount);
    event SynthBurnMsgToAggregator(bytes32 indexed currencyKey, address account, uint256 amount);
    event SynthExchangeMsgToAggregator(
        bytes32 indexed issueCurrencyKey,
        uint256 issueAmount,
        address issueAccount,
        bytes32 indexed burnCurrencyKey,
        uint256 burnAmount,
        address burnAccount
    );
    event SynthTransforFromMsgToAggregator(bytes32 indexed currencyKey, address from, address to, uint256 amount);
}