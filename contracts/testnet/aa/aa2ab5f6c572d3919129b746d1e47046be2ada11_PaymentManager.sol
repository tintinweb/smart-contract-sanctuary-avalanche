...
contract PaymentManager is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /**
     * @notice Add a platform to use critical functions like 'payout'
     */
    ...
    function viewWhitelistedPlatforms() external view onlyOwner returns (address[] memory) {
    ...
    function setCommissionPercentage(uint32 _commissionPercentage) external onlyOwner {
    ...
    function payout(
        ...
        uint256 commission = _getPortionOfBid(_price, commissionPercentage);
        if (commission > 0) {
        ...
        if (IERC721Upgradeable(_nftContractAddress).supportsInterface(type(IRoyalty).interfaceId)) {
            LibRoyalty.Part[] memory calculatedParts = IRoyalty(_nftContractAddress).multiRoyaltyInfo(_tokenId, _price);
            ...
            for (uint i = 0; i < calculatedParts.length; i++) {
                if (INFTCollectible(_nftContractAddress).owner() != _seller) {
                    require(remainder >= calculatedParts[i].value, "royalty amount cannot be higher than original price");
                    remainder -= calculatedParts[i].value;
            ...
            if (royaltyReceiver != _seller && royaltyReceiver != address(0x0)) {
        ...
        if (_shareholders.length > 0) {
            ...
            for (uint i = 0; i < _shareholders.length; i++) {
                if (_shareholders[i].account != address(0x0)) {
                    uint256 share = _getPortionOfBid(sellerTotalShare, _shareholders[i].value);
                    require(remainder >= share, "royalty amount cannot be higher than original price");
                    remainder -= share;
