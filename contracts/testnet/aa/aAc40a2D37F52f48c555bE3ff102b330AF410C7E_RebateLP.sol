...
contract RebateLP is Operator, HasBlacklist, CanPause, HasPOL, HasFactory, HasRouter {
    ...
    mapping (address => mapping(address => mapping(uint256 => uint256))) public userAmountInDays;
    mapping (address => mapping(uint256 => uint256)) amountInDays;
    ...
    constructor(address mainToken, address sideToken, address rewardToken, 
        uint256 _rewardPerLP, uint256 _startDay) {
    ...
    function bond(address token, uint256 amount) external onlyAsset(token) onlyOpen notInBlackList(msg.sender) {
        ...
        if (block.timestamp >= startDay && block.timestamp <= endDay) {
            amountInDays[token][startDay] = amountInDays[token][startDay] + amount;
            userAmountInDays[token][msg.sender][startDay] = userAmountInDays[token][msg.sender][startDay] + amount;
        }
        
        require(amountInDays[token][startDay] <= maxInDays, "RebateLP: over max amount in days");
        require(userAmountInDays[token][msg.sender][startDay] <= asset.maxUserAmountInDays, "RebateLP: over max user amount in days");
    ...
    function _claimRewards(address account) internal {
        ...
        uint256 claimable = schedule.rewardAmount * duration / schedule.period;
        if (block.timestamp >= schedule.end) {
            claimable = schedule.rewardAmount - schedule.rewardClaimed;
        }
    ...
    function claimableRewardToken(address account) external view returns (uint256) {
        VestingSchedule memory schedule = vesting[account];
