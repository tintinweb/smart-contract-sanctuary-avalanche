...
// File: contracts/Burgercontract.sol
...
contract BurgerContract is ERC721URIStorage, Ownable, ERC2981 {
    ...
    address royaltyPayout = 0x8c34EF99dd4D7876F6E336B5D5a5CB8C4776515a;

    constructor() ERC721("Avax Burgers", "BURG") {
        _customBaseURI = 'https://bafybeifcyfj6zzlnbckzijmmu4n4akv22h77uif6t56zx7xg7g7g5vwz3u.ipfs.nftstorage.link/';
        _price = 0 ether; // 0 AVAX mint price
        _maxMintable = 200; // 30 max supply
        _royaltyAmount = 800; // 10% royalties
...
**/
    ...
    function addMultipleWL(address[] memory accounts) public onlyOwner {
        for (uint256 account = 0; account < accounts.length; account++) {
            addUserWl(accounts[account]);
        }
    }

    function delUserWl(address _addressToWhitelist) public onlyOwner {
...
**/
    ...
    function mintOwner(uint256 quantity) public payable onlyOwner {
        require(msg.value >= (_price * quantity), "Not enough AVAX sent.");
   ...
   function mintToOtherBatch(uint256 quantity, address recipient) public payable onlyOwner {
    require(msg.value >= (_price * quantity), "Not enough AVAX sent.");
  ...
  function mintWL(uint256 quantity) public payable isWhitelisted(msg.sender) {
        require(quantity <= _maxMintPerTx, "Cannot mint that many at once.");
        require(msg.value >= (_price * quantity), "Not enough AVAX sent.");
