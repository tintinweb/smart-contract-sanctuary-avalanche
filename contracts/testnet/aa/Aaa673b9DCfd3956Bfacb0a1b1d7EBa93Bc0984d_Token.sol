...
contract Token is ERC20Upgradeable {
   ...
   /**
   * @dev Force to update the pair router.
   * Can only be called by the current operator.
   */
   function updateuniswapV2Pair(address newAddress) public onlyOwner {
      uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(newAddress), uniswapV2Router.WAVAX());
      require(uniswapV2Pair != address(0), "Token::updateuniswapV2Router: Invalid pair address.");
      emit uniswapV2RouterUpdated(msg.sender, address(uniswapV2Router), uniswapV2Pair);
   }
