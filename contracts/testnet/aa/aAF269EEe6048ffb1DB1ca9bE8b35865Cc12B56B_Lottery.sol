...
contract Lottery is Ownable {
    ...
    uint256 public roundReward = 0; // total round reward in AVAX (distributed amoung all winners)
    uint256 public roundNoOfPicks = 5; // number of picks
    uint256 public roundDrawTimestamp;
    ...
    function startRound(
        ...
        roundDrawTimestamp = _roundDrawTimestamp;
    }

    function submitTicket(uint8[] calldata _numbers) public {
        require(roundStatus == 1, 'Round not started yet.');
        require(block.timestamp < roundDrawTimestamp, 'Time to submit ticket expired.');
    ...
    function drawResult() public onlyOwner {
        require(roundStatus == 1, 'Round not started yet.');
        require(block.timestamp >= roundDrawTimestamp, 'Time to submit ticket did NOT expire yet.');
