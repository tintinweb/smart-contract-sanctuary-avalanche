...
import {UsersBalances} from "../../../../soundit/UsersBalances.sol";

/**
 * @notice A struct containing the necessary data to execute collect actions on a publication.
 *
 * @param pricePerSecond The read cost per second associated with this publication.
 */
...
/**
 * @title ChargePerSecondoModule
 * @author Lens Protocol
 *
 * @notice This is a simple ReadModule implementation, inheriting from the IReadModule interface
 *
 * This module works by allowing read data from publications charging the consumers with a price per second.
 */
contract ChargePerSecondModule is ModuleBase, IReadModule, AccessControl {
  bytes32 public constant GOV_ROLE = keccak256("GOV_ROLE");
  bytes32 public constant CHARGER_ROLE = keccak256("CHARGER_ROLE");
  using SafeERC20 for IERC20;

  mapping(uint256 => bool) internal _authorizationByProfile;
  mapping(uint256 => mapping(uint256 => ProfilePublicationData)) internal _dataByPublicationByProfile;
  UsersBalances public usersBalances;

  constructor(
    address hub,
    address governance,
    address _usersBalances,
    address charger
  ) ModuleBase(hub) {
    _setUsersBalances(_usersBalances);
    _setupRole(GOV_ROLE, governance);
    _setupRole(CHARGER_ROLE, charger);
  }

  /**
   * @notice Sets the users balance address. This function
   * can only be called by the governance address.
   *
   * @param newUsersBalances The new users balances address to set.
   */
  function setUsersBalances(address newUsersBalances) external {
    if (!hasRole(GOV_ROLE, msg.sender)) revert Errors.NotGovernance();
    _setUsersBalances(newUsersBalances);
  }

  function _setUsersBalances(address newUsersBalances) internal {
    if (newUsersBalances == address(0)) revert Errors.InitParamsInvalid();

    usersBalances = UsersBalances(newUsersBalances);
    emit Events.UsersBalancesSet(msg.sender, newUsersBalances, block.timestamp);
  }

  /**
   * @notice Sets the charger address. This function
   * can only be called by the governance address.
   *
   * @param newCharger The new charger address to set.
   */
  function setCharger(address newCharger) external {
    if (!hasRole(GOV_ROLE, msg.sender)) revert Errors.NotGovernance();
    _setCharger(newCharger);
  }

  function _setCharger(address newCharger) internal {
    if (newCharger == address(0)) revert Errors.InitParamsInvalid();

    _setupRole(CHARGER_ROLE, newCharger);
    emit Events.ChargerSet(msg.sender, newCharger, block.timestamp);
  }
   ...
   *      uint256 pricePerSecond: The price per second to levy.
   *
  ...
  function initializePublicationReadModule(
    ...
    uint256 pricePerSecond = abi.decode(data, (uint256));

    _dataByPublicationByProfile[profileId][pubId].pricePerSecond = pricePerSecond;
   ...
   * @param authorized Bool indicating whether the module is or not authorized.
   */
  function authorizeModule(uint256 profileId, bool authorized) external {
    ...
    _authorizationByProfile[profileId] = authorized;
  }

  /**
   * @dev Processes a read by:
   *  1. Calculating the total price
   *  2. Sending the tx to move balances to users balances contract.
   */
  function processRead(
    ...
    if (!hasRole(CHARGER_ROLE, msg.sender)) revert Errors.NotCharger();

    (address app, uint256 secondsToCharge) = abi.decode(data, (address, uint256));

    if (!_authorizationByProfile[consumerId]) revert Errors.ModuleNotAuthorized();
    uint256 pricePerSecond = _dataByPublicationByProfile[creatorId][pubId].pricePerSecond;

    uint256 totalPrice = pricePerSecond * secondsToCharge;
    usersBalances.moveBalance(consumerId, totalPrice, creatorId, app);
  }

  /**
   * @dev Processes several reads togetherby:
   *  1. Sending the tx to move balances to users balances contract.
   */
  function processReads(
    uint256[] calldata consumersIds,
    uint256[] calldata creatorsIds,
    uint256[] calldata amounts,
    address app
  ) external {
    if (!hasRole(CHARGER_ROLE, msg.sender)) revert Errors.NotCharger();

    if (consumersIds.length != creatorsIds.length || consumersIds.length != amounts.length)
      revert Errors.IncompleteData();

    for (uint256 i = 0; i < consumersIds.length; i++) {
      if (!_authorizationByProfile[consumersIds[i]])
        revert Errors.ModuleNotAuthorizedFor({consumerId: consumersIds[i]});
      usersBalances.moveBalance(consumersIds[i], amounts[i], creatorsIds[i], app);
    }
  }
   ...
   * initialized with this module.
   *
   * @param profileId The token ID of the profile mapped to the authorization to query.
   *
   * @return ProfilePublicationData The ProfilePublicationData struct mapped to that publication.
   */
  function getAuthorization(uint256 profileId) external view returns (bool) {
    return _authorizationByProfile[profileId];
  }
}
...
contract UsersBalances is ReentrancyGuard, AccessControl, SignerController {
  ...
  IERC20 public token;
  DataTypes.PercentagesData public percentages;
  mapping(address => uint256) public appBalances;
  mapping(uint256 => uint256) public consumersBalances;
  mapping(uint256 => uint256) public creatorsBalances;
  uint256 public protocolBalance;
  uint256 public fansBalance;
  ...
  constructor(
    ...
    DataTypes.PercentagesData memory _percentages
    ...
    _setProtocolFund(_protocolFund);
    _changePercentages(_percentages);
  }
  ...
  function _setProtocolFund(address newProtocolFund) internal {
    ...
    if (newProtocolFund == protocolFund) revert Errors.NoChangeToTheState();

    address prevProtocolFund = protocolFund;
    protocolFund = newProtocolFund;
    emit Events.ProtocolFundSet(msg.sender, prevProtocolFund, newProtocolFund, block.timestamp);
  }

  function changePercentages(DataTypes.PercentagesData memory _percentages) public {
    if (!hasRole(GOV_ROLE, msg.sender)) revert Errors.NotGovernance();
    _changePercentages(_percentages);
  }

  function _changePercentages(DataTypes.PercentagesData memory _percentages) internal {
    require(
      _percentages.creatorPercentage +
        _percentages.protocolPercentage +
        _percentages.appPercentage +
        _percentages.fansPercentage ==
        100,
      "% sum is not 100"
    );
    percentages = _percentages;

    emit Events.PercentagesSet(
      msg.sender,
  ...
  function addBalance(uint256 profileId, uint256 _amount) public payable {
    if (token.allowance(msg.sender, address(this)) < _amount) revert Errors.TokenAllowanceTooLow();
    require(token.transferFrom(msg.sender, address(this), _amount), "Transaction failed");
    consumersBalances[profileId] += _amount;
  ...
  function moveBalance(
    ...
    if (consumersBalances[consumerId] < amount) revert Errors.BalanceInsufficient();

    consumersBalances[consumerId] -= amount;

    uint256 amountToCreator = (amount * percentages.creatorPercentage) / 100;
    uint256 amountToProtocol = (amount * percentages.protocolPercentage) / 100;
    uint256 amountToApp = (amount * percentages.appPercentage) / 100;
    uint256 amountToFans = (amount * percentages.fansPercentage) / 100;

    creatorsBalances[creatorId] += amountToCreator;
    protocolBalance += amountToProtocol;
    appBalances[app] += amountToApp;
    fansBalance += amountToFans;
  ...
  function withdrawConsumerBalance(
    ...
    require(token.transfer(owner, amount), "Tx to consumer failed");
    consumersBalances[consumerId] -= amount;
  ...
  function withdrawCreatorBalance(uint256 creatorId, uint256 amount) public nonReentrant {
    if (creatorsBalances[creatorId] < amount) revert Errors.BalanceInsufficient();

    address owner = IERC721(HUB).ownerOf(creatorId);
    require(token.transfer(owner, amount), "Tx to creator failed");
    creatorsBalances[creatorId] -= amount;
  ...
  function withdrawProtocolBalance(uint256 amount) public nonReentrant {
    ...
    if (protocolBalance < amount) revert Errors.BalanceInsufficient();

    require(token.transfer(protocolFund, amount), "Tx to protocol fund failed");

    emit Events.ProtocolBalanceWithdrawn(protocolFund, amount, block.timestamp);
  }

  function withdrawAppBalance(address app, uint256 amount) public nonReentrant {
    if (appBalances[app] < amount) revert Errors.BalanceInsufficient();

    require(token.transfer(app, amount), "Tx to app fund failed");
  ...
  function withdrawFansBalance(
    ...
    if (fansBalance < amount) revert Errors.BalanceInsufficient();
    _checkSignFansWithdrawal(profileId, amount, _expiration, _nonce, _sig);

    address owner = IERC721(HUB).ownerOf(profileId);
    require(token.transfer(owner, amount), "Tx to fan failed");
    fansBalance -= amount;
