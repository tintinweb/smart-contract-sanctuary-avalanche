...
contract MintControl is Ownable, Pausable {
	 ...
	 * @param nftsToMint uint256 How many nfts to purchase
	 */
	function callForMint(string memory label, uint16 nftsToMint) public whenNotPaused exists(label) {
		...
		require(_checkSupply(label) >= nftsToMint, "Not enough supply remaining.");

		if (nftsToMint > 1){
			for (uint16 i = nftsToMint; i > 0; i--) {
	...
	function _createEntry(
		...
		containers[_containerLabel] = Container({ label: _containerLabel, location: ICFContainer(_location), price: _price, maxNfts: _maxNfts, idCounter: 0 });
	}
