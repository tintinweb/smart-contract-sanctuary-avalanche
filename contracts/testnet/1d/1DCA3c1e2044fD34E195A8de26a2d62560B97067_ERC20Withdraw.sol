...
contract ERC20Withdraw is Withdraw {
    ...
    mapping(address => uint256) public recipients; // mapping from user's address to request id
    ...
    function create(uint256 _amount) external whenNotPaused {
        ...
        require(recipients[_user] == 0, "A request is already active");
        ...
        recipients[_user] = _requestId;

        emit Created(_requestId, tokenAddress, _user, _amount);
    }
    ...
    {
        address _user = msg.sender;
        require(recipients[_user] == _requestId, "Invalid request id");
    ...
    function getActiveRequest(address _user) external view returns(uint256, Request memory) {
        uint256 requestId = recipients[_user];
