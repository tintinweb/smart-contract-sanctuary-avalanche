...
contract Claiming is Initializable, ContextUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event UnlockPeriodChanged(uint256 indexed _periodIndex, uint256 _vestingStartDate, uint256 _tokensPercentage, uint256 _cliffEndDate, uint256 _periodUnit);
    ...
    function updateWallet(
        ...
        _validateProof(_totalAllocation, distributorSetting.claimingWalletsMerkleRoot, _merkleProof);
    ...
    function claim(
        ...
        uint256 totalToClaim = _getAvailableTokens(_totalAllocation);
    ...
    function getAvailableTokens(uint256 _totalAllocation) public view returns (uint256) {
        return _getAvailableTokens(_totalAllocation);
    }
    ...
    function _getAvailableTokens(uint256 _totalAllocation) internal view returns (uint256) {
        uint256 availableAmount = 0;
        uint256 unlocksNumber = distributorSetting.unlocksNumber;
        uint256 totalClaimed = claims[_msgSender()].totalClaimed;
        ...
        if (availableAmount > 10000) availableAmount = 10000;
        return _calculatePercentage(_totalAllocation, availableAmount) - totalClaimed;
    }

    function getTotalClaimedPerWallet(address _wallet) public view returns (uint256) {
        return claims[_wallet].totalClaimed;
    }
}