...
contract nebulaFeeManager is Ownable{
	...
	function INTupdateProtoFutureFees(address _account,uint256 _x) internal {
		uint256 bef = INTgetProtoFutureFees(_account,_x);
		uint256 whole = nebuLib.doMultiple(feePeriod,INTgetProtoNextDue(_account,_x));
		if(nebuLib.isLower(whole,bef)==true && whole>0){
			INTupdateFutureFees(_account,false,bef.sub(whole));
		}else if(nebuLib.isHigher(whole,bef)==true){
			INTupdateFutureFees(_account,true,whole.sub(bef));
		}
	}
