...
contract _OTC_platform   is OwnableUpgradeable{
    ...
    mapping(uint256=>mapping(uint256=>_counterOffer)) public _counterDetails;
    ...
    // Last Countered User
    mapping(uint256=>address) public lastCounteredAddress;
    // Counter IDs
    mapping(uint256 =>uint256)public counterIds;
    // All counter Offers
    mapping(uint256=>uint256[])private _counterIds;
    // is PrivateID
    mapping(uint256=>bool)private isPrivateOffer;
    // accepted IDs
     mapping(uint256=>mapping(uint256=>bool)) public isAccepted;
    // After AcceptedIds
     mapping(address=>mapping(uint256=>uint256[]))public isAcceptedIds;
     // accepted ID
     mapping(uint256=>uint256)public acceptedID;
     //get platform fee
     mapping(address=>mapping(uint256=>uint256)) public getPlatformFee;


     mapping(address=>mapping(uint256=>uint256)) public userCounterID;
     mapping(uint256=>uint256)private isOffered;
   ...
   struct _counterOffer{
    ...
    function cancelOffer(uint256 _otcId)external {
    require(isOffered[_otcId]==0,"Counter Offers exists");
      address _creator=getOffers[_otcId]._creator;
        require(keccak256(bytes(getOffers[_otcId]._status))==keccak256(bytes("Pending")) ,"Not Allowed");
      ...
      uint256 _fee= getPlatformFee[msg.sender][_otcId];
      pendingTaxes[address(token)]-=_fee;
      getPlatformFee[msg.sender][_otcId]=0;
      token.transfer(_creator,_fee);
    }
    ...
    function acceptOffer(uint256 otcId)external{
        ...
        //   require(getOffers[otcId]._Accepter==address(0),"Already Offer Accepted");
          ...
          pendingTaxes[address(token)]+=platformTokenFee;
          getPlatformFee[msg.sender][otcId]=platformTokenFee;
    ...
    function depositFunds(uint256 otcId)external{
          ...
          (address creator,address accept,address offerToken,uint256 offerAmount,address requestToken,uint256 requestAmount, uint256 offerAcceptTime)=(offer._creator,offer._Accepter,offer._offerToken,offer._offerAmount,offer._requestToken,offer._requestAmount, offer._offerAcceptTime);
            require(block.timestamp<=offerAcceptTime+maxTimeToDepositFunds,"Offer Expired");
    ...
    function rejectOffer(uint256 otcId)external {
          ...
          (address creator,address accept, uint256 userID,address offerToken, uint256 offerAmount,address requestToken,uint256  requestAmount, uint256 offerAcceptTime)=(offer._creator,offer._Accepter,offer.userIds,offer._offerToken,offer._offerAmount,offer._requestToken, offer._requestAmount,offer._offerAcceptTime);
            require(block.timestamp<=offerAcceptTime+maxTimeToDepositFunds,"Offer Expired");
          require(creator==msg.sender ||accept==msg.sender,"No Allowed");
          uint256 __fee=getPlatformFee[msg.sender][otcId];
          uint256 _amount;
          ...
          if(creator==msg.sender){
            _amount=((2*__fee)*burnPercentage)/10000;  
            token.transfer(accept,(2*__fee)-_amount);
          }else{
            _amount=((2*__fee)*burnPercentage)/10000;  
            token.transfer(creator,(2*__fee)-_amount);
          ...
          if(isFunded[accept][otcId]){
            ...
            __fee=getPlatformFee[creator][otcId]+getPlatformFee[accept][otcId];
          pendingTaxes[address(token)]-=(__fee);
           token.burn(_amount);
    ...
    function rejectPrivateOffer(uint256 _otcId)external {
        ...
        removeId(_creator,_otcId,true); 
        removeId(msg.sender,_otcId,false);
        uint256 amount=(getPlatformFee[_creator][_otcId]*10)/100;
        token.transfer(msg.sender,amount);
        pendingTaxes[address(token)]-=getPlatformFee[_creator][_otcId];
        token.transfer(_creator,getPlatformFee[_creator][_otcId]-amount);
    ...
    function revokeOffer(uint256 otcId)external{
        ...
        if(msg.sender==creator){
           ...
           uint256 __fee=getPlatformFee[accept][otcId]+getPlatformFee[creator][otcId];
        uint256 _amount= ((__fee)*burnPercentage)/10000;
        pendingTaxes[address(token)]-=__fee;
        require(token.transfer(_user,(__fee)-_amount),"Failed");
        token.burn(_amount);
        }
   ...
   // set Burn fee for not funded offers
    ...
    function cancelNotFundedOffers(uint256 otcId) external onlyOwner{
        ...
        removeId(creator,otcId,true); 
        uint256 __fee=getPlatformFee[accepter][otcId]+getPlatformFee[creator][otcId];
        uint256 burnFee=(__fee*burnFeePercentForNotFunded)/10000;
        uint256 __ammount= __fee-burnFee;
        totalTxTokenFee[address(token)]+=(__ammount);
        pendingTaxes[address(token)]-=__fee;
        token.burn((burnFee));
        emit ClaimFunds(otcId,burnFee);
    }
    ...
    function withdrawFunds(uint256 otcId) public{
          ...
          uint256 __fee=getPlatformFee[creator][otcId]+getPlatformFee[accept][otcId];
          totalTxTokenFee[address(token)]+=__fee;
          pendingTaxes[address(token)]-=__fee;
          delete userOffers[creator][userID];
          delete getOffers[otcId];
          removeId(creator,otcId,false);       
       ...
       function selectCounter(uint256 otcID, uint256 counterId)external{
        require(!isAccepted[otcID][counterId],"Already Accepted");
       require(msg.sender==_counterDetails[otcID][counterId].creator,"Not Allowed");
       address accepter= _counterDetails[otcID][counterId].accepter;
       isAccepted[otcID][counterId]=true;
       acceptedID[otcID]=counterId;
       isAcceptedIds[accepter][otcID].push(counterId);
       getOffers[otcID]._Accepter=accepter;
    }
    ...
    function counterOffer(address offerToken, uint256 offerAmount,address requestToken, uint256 requestAmount,uint256 otcId)external {
        ...
        uint256 _Ids=counterIds[otcId]+1;   

        _counterOffer memory  _counter;
        if(getOffers[otcId]._Accepter!=address(0) || isAccepted[otcId][acceptedID[otcId]]){
            require(creator ==msg.sender ||getOffers[otcId]._Accepter==msg.sender ,"Already Offer Accepted"); 
            if(isAccepted[otcId][acceptedID[otcId]])
                _Ids=acceptedID[otcId];
            else  
                _Ids=otcId;
        }else{
         if(userCounterID[msg.sender][otcId]==0){
             counterIds[otcId]++;  
             userCounterID[msg.sender][otcId]=_Ids;
         } else{
             _Ids=userCounterID[msg.sender][otcId];
         }
        }
        isOffered[otcId]++;
        ...
        address _accepter=_counterDetails[otcId][_Ids].accepter;
        if(msg.sender!=creator){
            _counter.accepter=msg.sender;
        }else{
            _counter.accepter=_accepter;
        }
        lastCounteredAddress[otcId]=msg.sender;
       _counter.creator=creator;
        _counter.offerToken=offerToken;
        _counter.offerAmount=offerAmount;
        _counterDetails[otcId][_Ids]=_counter;
        _counterIds[otcId].push(_Ids);
  ...
  // function to claim all counter offered funds of particular Offer
    function claimOfferFunds(uint256 otcId,uint256 counterId)external{   
           require(_counterDetails[otcId][counterId].accepter==msg.sender,"Not Valid User");
            removeCounterId(otcId,counterId);
            delete _counterDetails[otcId][counterId];
            uint256 __fee =getPlatformFee[msg.sender][otcId];
            pendingTaxes[address(token)]-=__fee;
            delete getPlatformFee[msg.sender][otcId];
            token.transfer(msg.sender,__fee);
    }

    function getAllCounterIds(uint256 _otcId)public view returns(uint256[] memory){
        return _counterIds[_otcId];
    }


    function acceptCounterOffer(uint256 otcID, uint256 _counterId)external {
        require(isCounter[otcID] && lastCounteredAddress[otcID]!=msg.sender,"! Counter offer");
        require( !isOfferAccepted[otcID],"Already Accepted");
        _counterOffer memory _counter=_counterDetails[otcID][_counterId];
        require(isAccepted[otcID][_counterId],"Select Offer");
        require(getOffers[otcID]._creator==msg.sender || _counter.accepter!=address(0) || _counter.accepter==msg.sender,"! Creator");
        delete OpenOffers[otcID];
        removeId(msg.sender,otcID,false);
        uint256 Id=getOffers[otcID].userIds;
        Offer storage  _offer=getOffers[otcID];
        isOfferAccepted[otcID]=true;
        delete _counterDetails[otcID][_counterId];
        removeCounterId(otcID,_counterId);
        _offer._status="Active";
        if( _offer._Accepter==address(0))
        ...
        _offer._offerToken=_counter._requestToken;
        _offer._offerAmount=_counter._requestAmount;
        getOffers[otcID]=_offer;
        userOffers[_counter.creator][Id]=_offer;
    ...
    function rejectCounterOffer(uint256 otcId, uint256 _counterId)external{
        require(isCounter[otcId],"! Counter offer");
        address creator=_counterDetails[otcId][_counterId].creator;
        address accepter=_counterDetails[otcId][_counterId].accepter;
        require(creator==msg.sender  || accepter==msg.sender  ,"! Creator");
        require(keccak256(bytes(getOffers[otcId]._status))==keccak256(bytes("Pending")) ,"Not Allowed");
        require(isPaidFee[msg.sender][otcId],"Not Allowed");
         isOffered[otcId]--;
        delete _counterDetails[otcId][_counterId];
        removeCounterId(otcId,_counterId);
        uint256 _fee=getPlatformFee[msg.sender][otcId];
        pendingTaxes[address(token)]-=_fee;
        isPaidFee[accepter][otcId]=false;
           token.transfer(accepter,_fee);
        emit RejectCounterOffer(otcId, _fee);
    }
    ...
    function counterOfferDetails(uint256 __otcID, uint256 _counterId)public view returns(_counterOffer memory){
        return _counterDetails[__otcID][_counterId];
        
    }
       function removeCounterId(uint256 _otcId,uint256 _counterId) internal {
        uint256[] storage _ids;
        _ids =_counterIds[_otcId];
        for (uint256 i = 0; i < _ids.length; i++) {
            if (_ids[i] == _counterId) {
