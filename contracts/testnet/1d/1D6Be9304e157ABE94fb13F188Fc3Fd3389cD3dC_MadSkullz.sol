    ...
    Pausable
{
    ...
    mapping(address => uint16) internal _freemints;
    uint16 private _totalFreemints = 0;

    // random token id map
    mapping(uint256 => uint256) indexer;
    uint256 indexerLength = MAX_SUPPLY;
    // { 0: 3159, 1: 141, ... }
    mapping(uint256 => uint256) tokenIDMap;
    mapping(uint256 => uint256) takenImages;

    mapping(uint256 => bool) private _reveals;
    bool private _autoRevealed = false;
    ...
    function claim(uint256 amount) public whenNotPaused {
        require(
            step >= ContractSteps.OPEN_HOLDER,
            "MadSkullz: Failed to perform action during current step"
        );
        require(
            amount <= remainingFreemints(),
            "MadSkullz: Can't claim more freemints than you have"
        );
    ...
    function holderMint(uint256 amount) external payable whenNotPaused {
        require(_isHolder(), "MadSkullz: You are not a BabySkullz holder");
        require(
            step >= ContractSteps.OPEN_HOLDER,
            "MadSkullz: Failed to perform action during current step"
        );
    ...
    function whitelistMint(uint256 amount) external payable whenNotPaused {
        require(_isWhitelist(), "MadSkullz: You are not whitelisted");
        require(
            step >= ContractSteps.OPEN_WHITELIST,
            "MadSkullz: Failed to perform action during current step"
        );
        _mint(amount);
    }
    ...
    function publicMint(uint256 amount) external payable whenNotPaused {
        require(
            step >= ContractSteps.OPEN_PUBLIC,
            "MadSkullz: Failed to perform action during current step"
        );
        _mint(amount);
    }
    ...
    function _mint(uint256 amount) internal whenNotPaused {
        ...
        require(msg.value >= totalCost, "MadSkullz: Must send correct price");
        require(
            MAX_SUPPLY - FREEMINT_AMOUNT - totalSupply() - amount > 0,
            "MadSkullz: Not enough MadSkullz left to mint"
        );
        require(
            amount <= MAX_MINTABLE_PER_TX,
            "MadSkullz: Too many mint in a single transaction"
        );
        require(amount > 0, "MadSkullz: Amount can't be zero");
    ...
    // Censé être appelé par le Hulkz
    function safeMint(address to) public whenNotPaused onlyRole(MINTER_ROLE) {
        require(
            MAX_SUPPLY - FREEMINT_AMOUNT - totalSupply() - 1 > 0,
            "MadSkullz: Not enough MadSkullz left to mint"
        );
    ...
    function reveal(uint256 tokenId) external whenNotPaused {
        require(
            _exists(tokenId),
            "MadSkullz: Reveal query for nonexistent token"
        );
        require(
            ownerOf(tokenId) == _msgSender(),
            "MadSkullz: Can't reveal if you are not the owner"
        );
        require(!(_reveals[tokenId]), "MadSkullz: Can't reveal again");

        _reveals[tokenId] = true;
        emit Revealed(
            block.timestamp,
            _msgSender(),
            tokenId,
    ...
    function autoReveal() external onlyRole(ADMIN_ROLE) {
        require(
            ...
            "MadSkullz: Can't auto-reveal if not sold out"
        );

        for (uint256 i = 0; i < totalSupply(); i++) {
            if (!isRevealed(i)) {
                emit Revealed(block.timestamp, _msgSender(), i, tokenIDMap[i]);
            }
        }
    ...
    {
        require(_exists(tokenId), "MadSkullz: URI query for nonexistent token");
        ...
        if (bytes(_baseUri).length > 0) {
            if (isRevealed(tokenId)) {
                ...
                return
                    string(
                        abi.encodePacked(_baseUri, imageID.toString(), ".json")
                    );
            }
    ...
    function isRevealed(uint256 tokenId) public view returns (bool) {
        return _reveals[tokenId] || _autoRevealed;
    }
    ...
    function _isWhitelist() internal view returns (bool) {
    ...
    function setFreemints(
        ...
        require(
            ...
            "MadSkullz: beneficiaries list should equal amounts list"
        );

        for (uint16 i = 0; i < beneficiaries.length; i++) {
            require(
                _totalFreemints + amounts[i] <= FREEMINT_AMOUNT,
                "MadSkullz: can't give more freemints"
            );
