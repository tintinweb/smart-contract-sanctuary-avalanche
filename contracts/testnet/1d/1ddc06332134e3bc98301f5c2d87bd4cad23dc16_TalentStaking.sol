...
// File: contracts/talentstaking1.sol
...
contract TalentStaking is Ownable {
    ...
    //    require(!stakers.contains(_staker), "Staker already added");
       ...
       function getPriceinUSD(uint256 _amt) public returns (uint256){

        return USDT.balanceOf(address(this));
    }
    // uint256 public tsPriceInUsdt = getPriceinUSD;

    function listingFeeRewardToVoter(address _voter) public{

        if(talentInfo[msg.sender].stakeCount == 1){
            USDT.transferFrom(address(this), _voter, 10e6);
        }
        if(talentInfo[msg.sender].stakeCount == 2){
            USDT.transferFrom(address(this), _voter, 15e6);
        }
        if(talentInfo[msg.sender].stakeCount == 3){
            USDT.transferFrom(address(this), _voter, 20e6);
        }
        if(talentInfo[msg.sender].stakeCount == 4){
            USDT.transferFrom(address(this), _voter, 25e6);
        }
        if(talentInfo[msg.sender].stakeCount >= 5){
            USDT.transferFrom(address(this), _voter, 30e6);
        }
    }
   ...
   function setVoterStaking(address _voterStaking) external onlyOwner{
    ...
    function stake(uint256 types, string memory uri, string memory category, string memory subCategory, string memory subChildCategory, string memory uid)public{ // types 1 = 30 days types 2 = 60 days types 3 = 90 days 
      ...
      require (bytes(uid).length == 8);
       ...
       talentInfo[msg.sender].stakeCount = 0;
       talentInfo[msg.sender].stakeCount += 1;
    ...
    function unstake(address _account)public{
       require(!_blacklisted[_account] == true, "You Have Been Blacklisted" );
    ...
    function voting(address _account) public{
        talentInfo[msg.sender].voteCount = 0;
        ...
        listingFeeRewardToVoter(msg.sender);

    }

    // function getTalentData(address _account) public {
    //     return talentInfo[_account];
    // }
    ...
    function updateNextStakeTimestamp(address account)public{
        require(!_blacklisted[account] == true, "You Have Been Blacklisted" );
