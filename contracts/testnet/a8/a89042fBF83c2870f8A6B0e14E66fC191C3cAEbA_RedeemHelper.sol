...
contract RedeemHelper is Ownable {
    ...
    function addBondContracts(address[] memory _contracts) external onlyPolicy() {
        for(uint i=0;i<_contracts.length;i++) {
            addBondContract(_contracts[i]);
        }
    }
    function addBondContract( address _bond ) internal onlyPolicy() {
