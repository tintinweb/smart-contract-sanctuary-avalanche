...
// OpenZeppelin Contracts v4.4.1 (security/ReentrancyGuard.sol)
    ...
    function destroyIdo(uint256 amount) external;

    }

// Ido - $0.000070

contract TalentScoutIDO is ReentrancyGuard, Ownable {
    ...
    uint256 public supply = 15_000_000_000 ether;
    uint256 public distributed;

    uint256 public salePriceUsd = 70000000000000; //$0.000070

    uint256 public idoSupply = 15_000_000_000 * (10**18);
    ...
    uint256 public totalIdoLockedAmount = 0; 
    ...
    /* ONLY OWNER */
        ...
        function destroy() public onlyOwner nonReentrant {
            ...
            require(
                block.timestamp > saleEnd.add(5 minutes),
            ...
            supply = 0;
            talentScout.destroyIdo(remainingSupply);
        ...
        /**
        * Destory the remaining ido supply
        */
    ...
    /* GETTERS */
        ...
        function salePriceAvax() public view returns (uint256) {
            ...
            uint256 tspriceInAvax = (salePriceUsd.mul(10**18)).div(uint256(avaxPriceUsd).mul(10**10));
    ...
    /* EXTERNAL OR PUBLIC */
        ...
        function buy() public payable nonReentrant {
            ...
            require(supply > 0, "Ido ended, everything was sold");
        ...
        /**
            * Release the amount of locked ido amount
        */
        function releaseIdoAmount(uint256 lock_id) public {
            require(
            ...
            totalIdoLockedAmount -= _locks[msg.sender][lock_id].locked;
        ...
        function refundAVAX() external {
            ...
            totalIdoLockedAmount = totalIdoLockedAmount.sub(_totalTsLock[msg.sender]);
    ...
    function saleLock(address _account, uint256 _scoutToLock) private {
        // 15% released after 180 days
        uint256 m6Release = (_scoutToLock.mul(15)).div(100);
        // 15% released after 240 days
        uint256 m8Release = (_scoutToLock.mul(15)).div(100);
        // 15% released after 300 days
        uint256 m10Release = (_scoutToLock.mul(15)).div(100);
        // 15% released after 1 year
        uint256 m12Release = (_scoutToLock.mul(15)).div(100);
        // 15% released after 15 months
        uint256 m15Release = (_scoutToLock.mul(15)).div(100);
        // 15% released after 18 months
        uint256 m18Release = (_scoutToLock.mul(15)).div(100);
        // 10% released after 21 months
        uint256 m21Release = (_scoutToLock.mul(10)).div(100);
        
        insertIdoLock(_account, m6Release, 0 days);
        insertIdoLock(_account, m8Release, 0 days);
        insertIdoLock(_account, m10Release, 0 days);
        insertIdoLock(_account, m12Release, 0 days);
        insertIdoLock(_account, m15Release, 0 days);
        insertIdoLock(_account, m18Release, 0 days);
        insertIdoLock(_account, m21Release, 0 days);
    }
        ...
        */
        function insertIdoLock( address account, uint256 _amount, uint256 _relative_releaseTime ) internal {
            require( totalIdoLockedAmount + _amount <= idoSupply, "Unable to lock the defined amount, cap exceeded" );
