...
contract BlockStarNFTDrop is ERC721, Ownable, DefaultOperatorFilterer,ERC2981,BlockstarService {
    ...
    uint256 public maxWhitelistMint;  
     ...
     constructor(
            ...
            require(_time[2] > block.timestamp && _time[2] > 0 && _time[3] > 0 && _time[2] < _time[3] , "whitelist start time and end time must be greater then current time and start time must be greater then end time");
            require(_isWhitelist &&   _time[0] > block.timestamp && _time[0] > 0 && _time[1] > 0 && _time[0] < _time[1] , "whitelist start time and end time must be greater then current time and start time must be greater then end time");
            require(_isWhitelist &&  _time[0] < _time[2] &&  _time[0] < _time[3] && _time[1] <= _time[2] && _time[1] < _time[3], "public sale start time and end time must be greater then whitelist time.");
