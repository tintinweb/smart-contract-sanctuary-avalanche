...
contract VideoPokerLogic {
  ...
  function dealt(uint256 _randomness) public pure returns (uint, uint256, uint[5] memory, uint[5] memory, uint[5] memory) {
    ...
    (uint[5] memory values, uint[5] memory colors) = getCardDetails(cardValues);
    return (win(cards_), cards_, cardValues, values, colors);

  }

  function replace(uint256 cards_, uint change_, uint256 _oldRandomness, uint256 _newRandomness) public pure returns (uint256, uint256, uint[5] memory, uint[5] memory, uint[5] memory) {
  ...
  function reverse(uint256 cards_, uint change_, uint256 _oldRandomness, uint256 _newRandomness) public pure returns (uint256, uint256, uint[5] memory, uint[5] memory, uint[5] memory) {
    // Rebuild the deck from the current cards
    (,, uint[5] memory oldCards_,,) = dealt(_oldRandomness);
    
    uint[5] memory cardValues;
    cardValues[0] = oldCards_[4];
    cardValues[1] = oldCards_[3];
    cardValues[2] = oldCards_[2];
    cardValues[3] = oldCards_[1];
    cardValues[4] = oldCards_[0];
    // Calculate the deck with the selected cards marked as dealt
    uint deck_ = 16141147358858633216 | // Initialize deck with the initial state
      (1 << (cards_ & 63)) | // Mark the first card as dealt
      (1 << ((cards_ & 4032) >> 6)) | // Mark the second card as dealt
  ...
  function getCardDetails(uint[5] memory _card) public pure returns (uint[5] memory, uint[5] memory) {
