...
contract Characters is ERC721Enumerable, Ownable, Pausable{
    ...
    event tokenMinted(address owner, uint tokenId, bool special);
    ...
    function specialMint(address owner, uint _seed) external {
        ...
        if (traitsData[special_minted + maxPayable].class == 4){juggernaut_minted++;}
        else {ninja_minted++;}

        _safeMint(owner, uint256(special_minted + maxPayable));

        emit tokenMinted(owner, (special_minted + maxPayable), true);
    ...
    function _tokenmint(uint _seed, uint _tokenId) private {  
        ...
        emit tokenMinted(msg.sender, _tokenId , false);
        if (endprice > 0) rouble.burn(_msgSender(), endprice);
        //TODO Withdraw contract money in case its payed with $AVAX 
    }
