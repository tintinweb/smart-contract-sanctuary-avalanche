    ...
    ClampedRandomizer
{
    ...
    uint256 private _maxMintable = 1100;
    ...
    constructor()
        ERC721("TieredMintTest", "TIER")
        ClampedRandomizer(_maxMintable)
    {
        _customBaseURI = "test";
        _price = 500 * 10**15; //price in milli
        _royaltyAmount = 500;
        _maxMintPerTx = 40;
        maxPerWallet = 100;
        tierMints = [0, 10, 5, 2];
        tierPrice = [0, 400, 250, 0];
        paused = true;
        wlTime = 0;
        publicTime = 1;
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(publicMints[msg.sender] <= maxPerWallet, "trying to mint more than the max per wallet");
        ...
        require(quantity <= _maxMintPerTx, "minting more than max per tx");

        for (uint256 i = 0; i < quantity; i++) {
            internalMint(msg.sender);
        }
    }
    function tieredMint(uint256 quantity, uint256 tier) public payable {
        require(wlTier[msg.sender] == tier, "wrong tier or not on wl");
        require(privateSale(), "private sale is not active");
        require(publicMints[msg.sender] + quantity <= tierMints[tier], "minting more than max for this tier");
        require(msg.value >= (tierPrice[tier] * quantity * 10**15), "Not enough AVAX sent");
        publicMints[msg.sender] += quantity;
        require(totalSupply() + quantity <= _maxMintable, "minting more than max supply");
        require(!paused, "minting paused");
    ...
    function internalMint(address to) internal {
        ...
        setRoyaltyReceicver(to, tokenId);
        _safeMint(to, tokenId);
        
    }
    function setRoyaltyReceicver(address receiver, uint256 tokenId) internal {
    ...
    {
        return (royaltyReceiver[_tokenId], ((_salePrice * _royaltyAmount) / 10000));
    }
