...
contract PeonBurner is Ownable2Step, Pausable, IPeonBurner {
    ...
    modifier isUpgradeAllowed() {
        if (upgradeStartTime == 0 || upgradeEndTime == 0) {
            revert PeonBurner__BurnNotInitialized();
        }
        if (block.timestamp > upgradeEndTime) {
            revert PeonBurner__UpgradePeriodEnded();
        }
     ...
     * @param _upgradeEndTime The time when the upgrade ends
     */
    constructor(address _peonToken, address _peonsV1, uint256 _upgradeStartTime, uint256 _upgradeEndTime) {
        if (_peonToken == address(0) || _peonsV1 == address(0)) {
            revert PeonBurner__ZeroAddress();
        }
        if (_upgradeStartTime >= _upgradeEndTime || _upgradeStartTime == 0 || _upgradeEndTime == 0) {
        ...
        upgradeEndTime = _upgradeEndTime;
    }
     ...
     * @dev Only allows for multiple tokens to be minted if they are of the same type
    ...
    function devMint(uint256[] calldata peonv1TokenIds) external onlyOwner {
        if (block.timestamp < upgradeEndTime) revert PeonBurner__UpgradePeriodNotEnded();
        uint256 amount = peonv1TokenIds.length;
        address peonV1Owner;
        PeonTokenType firstPeonTokenType = _checkPeonType(peonv1TokenIds[0]);
        for (uint256 i = 0; i < amount; i++) {
