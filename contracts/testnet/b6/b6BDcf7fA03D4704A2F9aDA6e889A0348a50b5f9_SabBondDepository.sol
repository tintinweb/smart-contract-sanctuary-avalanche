/**
 *Submitted for verification at testnet.snowtrace.io on 2022-02-26
*/
...
contract SabBondDepository is Ownable {
    ...
    uint32 public discountRate = 90; //10% discount in bond price
...
//        require(balanceBefore.add(profit) == Sab.balanceOf(address(this)), "Not enough Sab to cover profit");
    ...
    function withdraw (address _recipient, uint _amount) external returns (uint) {
        ...
        require((uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[1]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[1]).add32(86400)) 
            || (uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[2]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[1]).add32(86400)), 
