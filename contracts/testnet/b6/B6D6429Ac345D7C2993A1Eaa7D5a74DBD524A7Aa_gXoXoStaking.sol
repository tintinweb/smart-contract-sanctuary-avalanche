...
// Root file: contracts/gXoXoStaking.sol
...
// The goal of this farm is to allow a stake gXOXO earn anything model
// In a flip of a traditional farm, this contract only accepts gXOXO as the staking token
// Each new pool added is a new reward token, each with its own start times
// end times, and rewards per second.
contract gXoXoStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 gXoXoStakedAmount; // # of gXOXO allocated to this pool
        ...
        // this address is the owner of the protocol corresponding to the reward token, used for emergency withdraw to them only
        address protocolOwnerAddress;
    }

    IERC20 public immutable gXoXo;
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.gXoXoStakedAmount != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp, pool);
            uint256 reward = multiplier * pool.RewardPerSecond;
            accRewardPerShare += (reward * pool.TokenPrecision) / pool.gXoXoStakedAmount;
        }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        pool.accRewardPerShare += reward * pool.TokenPrecision / pool.gXoXoStakedAmount;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        safeTransfer(gXoXo, address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        pool.gXoXoStakedAmount -= user.amount;
        user.amount = 0;
        user.rewardDebt = 0;

        gXoXo.safeTransfer(address(msg.sender), oldUserAmount);
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(gXoXo), "recoverWrongTokens: Cannot be gXoXo");
