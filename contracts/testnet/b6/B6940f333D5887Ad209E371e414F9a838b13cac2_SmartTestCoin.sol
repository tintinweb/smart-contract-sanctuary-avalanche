...
contract SmartTestCoin is ERC20, Withdrawable {
    ...
    uint256 private _marketingAddressFee = 7;
    uint256 private _gameAddressFee = 60;
    uint256 private _teamAddressFee = 6;
    ...
    constructor(
        ...
        _mint(teamAddress, _teamSupply().sub(_devSupply()));
        _mint(devAddress, _devSupply());

        addToExcludedFromIncomeFee(_coinOwner);
        addToExcludedFromIncomeFee(teamAddress);
        addToExcludedFromIncomeFee(devAddress);
        addToExcludedFromIncomeFee(_investorAddress);

        addToExcludedFromOutcomeFee(_coinOwner);
        addToExcludedFromOutcomeFee(teamAddress);
        addToExcludedFromOutcomeFee(devAddress);
    ...
    function legendaryTotalIncomePerAddress(address address_) public view returns (uint256) {
        return _legendaryTotalIncomePerAddress[address_];
    }

    function epicTotalIncomeTrigger() public view returns (uint256) {
        return _epicTotalIncome;
    }

    function legendaryTotalIncomeTrigger() public view returns (uint256) {
        return _legendaryTotalIncome;
    }
    ...
    function _distributeFee(address sender, uint256 amount) private {
        ...
        _transfer(sender, transitAddress, percent.mul(_poolAddressFee));
        ...
        _transfer(sender, teamAddress, teamFee_.sub(devFee_));
        _transfer(sender, devAddress, devFee_);
    }
    ...
    function _customTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (recipient == address(this)) {
            _distributeFee(sender, amount);
        } else if (excludedFromOutcomeFee[sender]) {
