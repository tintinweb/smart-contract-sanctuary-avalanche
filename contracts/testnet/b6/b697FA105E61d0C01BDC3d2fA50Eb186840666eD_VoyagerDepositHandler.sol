    ...
    PausableUpgradeable
{
    ...
    function iSend(bytes memory packet, bytes calldata requestMetadata, uint256 value) internal {
        // TODO: remove string
        bytes memory requestPacket = abi.encode(string(routerBridge), packet);
        (bool success, ) = gatewayContract.call{ value: value }(
            abi.encodeWithSelector(0x7bed470a, 1, 0, "", ROUTER_CHAIN_ID, requestMetadata, requestPacket)
        );
        require(success);
    }
    ...
    function iReceive(
        ...
        Setter.iReceive(_executionRevertCompleted, _burnList, _reserve, string(routerBridge), requestSender, packet);
        // require(
        //     keccak256(abi.encodePacked(sender)) == keccak256(abi.encodePacked(routerBridge)),
        //     "only Voyager middleware"
        // );
