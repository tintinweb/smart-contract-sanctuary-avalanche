...
/// Generic game where you chose M of 1-N values, can provide specific subgame.
contract Dice is GameBase, GameBet, GameChainlink {
    enum RoundState {
        UNKNOWN,
        LOST,
        UNUSED_TIE,
    ...
    event GameStarted(uint256 indexed gameId, address indexed creator, uint8 subgame, uint8 numRounds, bool[] choices, uint256 betPerRound, uint256 rakePerRound, uint256 payoutPerRound);
    ...
    mapping(uint256 => bool[]) public choicesByGameId;
    ...
    function playGame(
        ...
        BetDetails memory details = validateBet(treasuryAddress.balance, rakeDistributor.getTotalRake(), numRounds);

        uint8 numChoices = uint8(userChoices.length);
        require(numChoices <= 100, "Can pass in at most 100 options");
        require(numChoices >= 2, "Must pass in at least two options.");
        ...
        require(numSelected > 0, "Must choose at least one value");
        require(numSelected < numChoices, "Cannot chose all values");

        fi.payoutPerRound = getPayout(fi.betPerRound, numSelected, numChoices);
    ...
    function getPayout(uint256 betPerRound, uint8 numSelected, uint8 numChoices) public returns (uint256) {
        return betPerRound * numChoices / numSelected;
    }
    ...
    function fulfillRandomWords(uint256 requestId,uint256[] memory randomWords) internal override {
        ...
        bool[] memory values = choicesByGameId[gameId];
        uint8 numRounds = fi.numRounds;
        uint8 numChoices = fi.numChoices;
        uint256 payoutPerRound = fi.payoutPerRound;
        ...
        for (uint i = 0; i < numRounds; ++i) {
            ...
            uint8 selected = uint8(roundRandom % numChoices);
            results[i].selected = selected;

            if (values[selected]) {
                results[i].state = RoundState.WON;
                results[i].sent = payoutPerRound;
                amountToUser += payoutPerRound;
            } else {
