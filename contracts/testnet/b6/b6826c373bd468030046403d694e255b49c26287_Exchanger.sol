...
function bytesToBytes32(bytes memory data) public pure returns (bytes32 result) {
    require(data.length <= 32, "Invalid data length"); // проверяем, что длина данных не превышает 32 байтов
    assembly {
        let mask := sub(exp(256, sub(32, mload(data))), 1) // создаем маску для заполнения недостающих байтов нулями
        result := and(mload(add(data, 32)), mask) // загружаем данные начиная с позиции data+32 и применяем маску для заполнения недостающих байтов нулями
    }
}
  ...
  function redeem(uint256 id, bytes memory key) external {
    ...
    exchange.key = bytesToBytes32(key);
