...
contract Managers{
    ...
    //設定管理員 n個，決策權是nx 2/3
    //新增管理員 、 刪除管理員 、動作發行、 動作表決、 動作只能表決最新的一個
    ...
    function signAction(uint256 _actionId) public isManager returns (bool) {
        console.log("_actionId!=actionId", _actionId, actionId);
        ...
        console.log("x1", actions[_actionId].signatureCount);
        if (actions[_actionId].signatureCount >= (((managerLen * 2) / 3) + 1)) {
            ...
            if (actions[_actionId].cmd == ActionCmd.AddManager) {
                console.log("xx1", actions[_actionId].arg_addr);
                require(
