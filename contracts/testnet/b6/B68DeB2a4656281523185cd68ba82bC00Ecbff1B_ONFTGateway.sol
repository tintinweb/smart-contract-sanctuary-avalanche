...
contract ONFTGateway is IOmniApp {
    ...
    event Minted(address collection, address owner);
     ...
     */
    function setUA(string memory remoteChainName, address remoteUA) external {
        require(msg.sender == _owner);
    ...
    function sendTo(IERC721Metadata collection, uint256 tokenId, string memory dstChainName, uint256 gas, uint256 redirectFee) public payable {
        ...
        // TODO: (Must) verify ownership
        require(_isContract(collAddress), "!EXISTS");
        bytes memory payload = _getPayload(collection, tokenId);

        if (isCopy[collAddress]) {
            IONFTCopy copy = IONFTCopy(collAddress);
            copy.burn(tokenId);
            _send(payload, dstChainName, gas, redirectFee);
    ...
    function omReceive(bytes calldata _payload, address srcUA, string memory srcChain) external override {
        ...
        (address collAddress, uint256 tokenId, string memory tokenURI, address owner, string memory name, string memory symbol) = abi.decode(_payload, (address, uint256, string, address, string, string));

        if (locked[collAddress][tokenId] != address(0)) {
            IERC721 original = IERC721(collAddress);
            delete locked[collAddress][tokenId];
    ...
    function _send(bytes memory payload, string memory dstChainName, uint256 gas, uint256 redirectFee) private {
        omnichainRouter.send{value : msg.value}(dstChainName, remoteChainToUA[dstChainName], payload, gas, msg.sender, redirectFee);
    }

    function _getPayload(IERC721Metadata collection, uint256 tokenId) private view returns (bytes memory) {
        string memory tokenURI = collection.tokenURI(tokenId);

        return abi.encode(address(collection), tokenId, tokenURI, msg.sender, collection.name(), collection.symbol());
    }
...
contract ONFTCopy is IONFTCopy, ERC721URIStorage {
    ...
    function burn(uint256 tokenId) override external {
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId), "ONFTCopy: caller is not owner nor approved");
        _burn(tokenId);
    }
