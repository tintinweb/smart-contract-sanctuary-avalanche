...
contract mintPass is ERC721Enumerable, Ownable {
    ...
    */
    ...
    {   
        string memory payload = string(abi.encodePacked(Strings.toString(quantity)));

        return MerkleProof.verify(proof, GOLDEN_ROOT, _leaf(msg.sender, payload));
    }

    function checkSilverMerkle (bytes32[] calldata proof,uint256 quantity) internal view returns(bool)
    {   
        string memory payload = string(abi.encodePacked(Strings.toString(quantity)));
        return MerkleProof.verify(proof, SILVER_ROOT, _leaf(msg.sender,payload));
    }
