...
// https://ipfs.io/ipfs/QmV1HqRUhLMrknqfDLv6cnyzrPRkvE72CCNETuX6isNJMN?filename=spidy.json
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata { 
    ...
    constructor(string memory name_, string memory symbol_, uint256 tokenNumber_, string memory tokenUrl_) {
        _name = name_;
        _symbol = symbol_;
    ...
    // Staking events //
    event ApproveStakerEvent(address sender, address approvedStakerAddress, uint256 tokenId);
    ...
    // Fractioning events //
    event ApproveFractionerEvent(address sender, address approvedFractionerAddress, uint256 tokenId);
    ...
    modifier onlyApprovedStaker(uint256 tokenId) {
        require(_msgSender() == _stakersApprovals[tokenId], "Not approved staker!");
        _;
    }

    // Modifier to check that the caller is the approved fractioner .
    modifier onlyApprovedFractioner(uint256 tokenId) {
        require(_msgSender() == _fractionersApprovals[tokenId], "Not approved fractioner!");
        _;
    }
    ...
    function approveStaker(address approveStakerAddress, uint256 tokenId) public returns(bool){
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Sender is not owner, approved, or operator");
        ...
        require(_msgSender() != approveStakerAddress, "ERC721: staker approval to owner, approved, or operator");
    ...
    function stakingStartDate(uint256 tokenId) public view virtual returns(uint256){
    ...
    function stakingEndDate(uint256 tokenId) public view virtual returns(uint256){
    ...
    function getApprovedStaker(uint256 tokenId) public view virtual returns (address) {    
    ...
    function renounceApprovedStaker(uint256 tokenId) public returns(bool){
        ...
        // require sender be the owner/approved/operator
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Sender is not the owner, approved, or operator");

        // require approved staker != address(0)
        require(_stakersApprovals[tokenId] != address(0), "No address is approved staker!");
    ...
    function getStakingStartDate(uint256 tokenId) public view returns(uint256){
        return _stakers[tokenId].stakingStartDate;
    }

    // get staked NFT end date
    function getStakingEndDate(uint256 tokenId) public view returns(uint256){
    ...
    function approveFractioner(address approveFractionerAddress, uint256 tokenId) public returns(bool){
        ...
        require(_msgSender() != approveFractionerAddress, "ERC721: fractioner approval to owner, approved, or operator");
    ...
    function fractionerOf(uint256 tokenId) public view virtual returns (address) {
    ...
    function getApprovedFractioner(uint256 tokenId) public view virtual returns (address) {    
        // require tokenId be minted.
        _requireMinted(tokenId);
    ...
    function renounceApprovedFractioner(uint256 tokenId) public returns(bool){
        ...
        require(_fractionersApprovals[tokenId] != address(0), "No address is approved for fractioning!");
    ...
    function getFractionerAddress(uint256 tokenId) public view returns(address){
    ...
    function stakerLock(uint256 tokenId, uint256 endDate) public returns(bool){
        ...
        require(_msgSender() == getApprovedStaker(tokenId), "Sender is not approved as a staker");
    ...
    function stakerUnlock(uint256 tokenId) public returns(bool){
        ...
        if(_stakers[tokenId].stakingEndDate <= block.timestamp){
            ...
            // require sender be the owner, approved, or operator
            require(_isApprovedOrOwner(_msgSender(), tokenId) || _msgSender() == _stakerOf(tokenId), "Sender is not the owner, approved, or operator");

        }else{
            
            // require sender be the staker for the tokenId
            require(_msgSender() == _stakerOf(tokenId), "Sender is not the staker");
    ...
    function fractionerLock(uint256 tokenId) public returns(bool){
        ...
        require(_msgSender() == getApprovedFractioner(tokenId), "Sender is not approved as an fractioner");
    ...
    function fractionerUnlock(uint256 tokenId) public returns(bool){
        ...
        // require fractioned
        require(_fractionerOf(tokenId) != address(0), "TokenId is not fractioned!");

        // require sender be the fractioner for the tokenId
        require(_msgSender() == _fractionerOf(tokenId), "Sender is not the fractioner");
    ...
    function fractionerUnlockAndTransfer(uint256 tokenId, address _transferToAddress) public returns(bool){
    ...
    function _requireNotFractioned(uint256 tokenId) internal view virtual {
        require(_fractionerOf(tokenId) == address(0), "TokenId is locked by fractioner");
    }
    ...
    function _approveStaker(address sender, address approveStakerAddress, uint256 tokenId) internal virtual {
    ...
    function _stakerOf(uint256 tokenId) internal view virtual returns (address) {
        return _stakers[tokenId].stakerAddress;
    }

    function _stakingStartDate(uint256 tokenId) internal view virtual returns(uint256){
        return _stakers[tokenId].stakingStartDate;
    }

    function _stakingEndDate(uint256 tokenId) internal view virtual returns(uint256){
        return _stakers[tokenId].stakingEndDate;
    }

    function _approveFractioner(address sender, address approveFractionerAddress, uint256 tokenId) internal virtual {
    ...
    function _fractionerOf(uint256 tokenId) internal view virtual returns (address) {
        return _fractioners[tokenId].fractionerAddress;
    }
