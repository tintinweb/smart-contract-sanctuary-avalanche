...
contract Gambit {
    ...
    constructor(Match[] memory _matches) {
        ...
        globalV.nextDayPot = 0;
        globalV.finalPot = 0;
        globalV.treasuryPot = 0;
        globalV.foundersClubPot = 0;
        globalV.minBet = 1 ether;
        globalV.initialPot = 0;
    }
    ...
    function viewPots() external view onlyOwner returns (uint, uint, uint, uint, uint){
        require(msg.sender == owner, "Only Owner function");
        return (globalV.nextDayPot, globalV.finalPot, globalV.treasuryPot, globalV.foundersClubPot, globalV.initialPot);
    }
    ...
    function viewMatchDates(string memory _matchId) external view onlyOwner returns (uint, uint){
        require(msg.sender == owner, "Only Owner function");
        return (matches[_matchId].matchDate, matches[_matchId].closingTime);
    }
}