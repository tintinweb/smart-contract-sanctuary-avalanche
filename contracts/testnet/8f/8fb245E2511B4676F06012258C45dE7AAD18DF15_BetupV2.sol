...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function getRewardOf(
        ...
        uint256 _result,
        uint256 _topTokenID
    ) external view returns(RewardType memory reward)
    {
    ...
    function close(
        ...
        for(uint256 i = 0; i < data.ids.length; i++) {
            ...
            if(checkSignatureForClose(
                pairId,
                counter,
                hashCloseData(data, i)) == false) {
                continue;
            }
    ...
    function trystack(
        ...
        IERC20MetadataUpgradeable(chip).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    */
    ...
    function _calcReward(
        PairsCRound memory _pcr,
        PlayerRound memory _player,
        uint256 _result,
        uint256 _maxLevel,
        address _chip
    )
    ...
    function _assembleAmount(
        ...
        if(_ids.length == 1 && _refund  > 0) {
            IERC20MetadataUpgradeable(_chip).safeTransfer(msg.sender, _refund);
            ...
            if(_i == 0) {
                ...
                if(preChip == _chip) {
                    ...
                    _accAmount = _refund;
                    if(_refund  > 0) {
                        IERC20MetadataUpgradeable(_chip).safeTransfer(msg.sender, _preAmount);
                    }
                }
            }
        }
    ...
    function withdraw(
        ...
        for(uint256 i = 0; i < _ids.length; i++) {
            ...
            RewardType memory reward = _calcReward(pcr, player, result, _maxLevel, _pairsConfig[pairId-1].chip);
    ...
    function revokeRole(bytes32 role, address account) public virtual override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(hasRole(role, account) == true, "Not exist");
