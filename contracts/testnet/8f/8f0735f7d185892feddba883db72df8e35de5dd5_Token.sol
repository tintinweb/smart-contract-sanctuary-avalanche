...
contract Token is ERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender != owner() && recipient != owner() && recipient != uniswapV2Pair) { // Exclude owner and Uniswap pair from max per wallet
            uint256 recipientBalance = balanceOf(recipient);
            // If maxPerWallet is not 0, check that recipient balance + amount does not exceed maxPerWallet
            if (maxPerWallet > 0) {
