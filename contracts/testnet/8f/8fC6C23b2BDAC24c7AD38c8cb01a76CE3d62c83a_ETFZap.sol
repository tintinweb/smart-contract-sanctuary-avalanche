...
contract ETFZap is BNum, OperatorUpgradeable {
    ...
    function __ETFZap__init(
        ...
        require(ETFToken_ != zero, "ETFZap: ETFToken is zero address");
        ETFToken = ETFToken_;
    ...
    function setETFToken(address ETFToken_) external onlyOperator {
        ETFToken = ETFToken_;
    }
    ...
    function joinSingle(
        ...
        uint256 ratio = bdiv(poolAmountOut, IERC20Upgradeable(ETFToken).totalSupply());
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            IERC20Upgradeable(tokens[i]).approve(ETFToken, amountsToPool[i]);
        }
        require(
        ...
        IERC20Upgradeable(ETFToken).safeTransfer(msg.sender, poolAmountOut);
    ...
    function exitSingle(
        ...
        uint256 exitFee = bmul(poolAmountIn, IETFToken(ETFToken).exitFee());
        uint256 ratio = bdiv(bsub(poolAmountIn, exitFee), IERC20Upgradeable(ETFToken).totalSupply());
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 usedBalance = IETFToken(ETFToken).getUsedBalance(tokens[i]);
        ...
        IERC20Upgradeable(ETFToken).safeTransferFrom(
            msg.sender,
            address(this),
            poolAmountIn
        );
        ...
        IERC20Upgradeable(ETFToken).approve(ETFToken, poolAmountIn);
