    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(string calldata __baseURI) public initializer {
        __ERC721_init('Chain Colosseum Phoenix Hero', 'HERO');
        __Ownable_init_unchained();
    ...
    function mint(address user, uint256 tokenId) public onlyRole(MINTER_ROLE) returns (uint256) {
        _mint(user, tokenId);
        return tokenId;
    }
    ...
    function getMyHeroIds() public view returns (uint256[] memory) {
        uint256[] memory ids = new uint256[](balanceOf(_msgSender()));
