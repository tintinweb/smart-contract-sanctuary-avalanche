...
contract BridgesOfFate is Ownable {
    ...
    function _randomPushBackBankingGame() internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender))) % 100 == 0 
            ? 1 : uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender))) % 100;
    }
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public GameEndRules PushBackBankingGameEndRules{
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules PushBackBankingGameEndRules{
        ...
        require(_member.feeStatus == true, "Please Pay Entry Fee.");
        // require(PioneerPushBackBankingGameMap[_gameStatus.stageNumber] < 50e9,"Dynamic Game Ended.");
        ...
        if (GameMap[_currentUserStage] <= 0) {
            ...
            PioneerPushBackBankingGameMap[_currentGlobalStage] = _randomPushBackBankingGame();
        ...
        if(_currentUserStage == _gameStatus.stageNumber) {
            if(_checkSide(_gameStatus.stageNumber,_jumpSide) && (PioneerPushBackBankingGameMap[_gameStatus.stageNumber] > 50e9 )){
    ...
    function isGameEnded() external view returns (bool) {
        GameStatus storage _gameStatus = GameStatusInitialized[latestGameId];
        if (PioneerPushBackBankingGameMap[_gameStatus.stageNumber] > 0
            && (_dayDifferance(block.timestamp, _gameStatus.startAt) > _gameStatus.lastUpdationDay)) {
