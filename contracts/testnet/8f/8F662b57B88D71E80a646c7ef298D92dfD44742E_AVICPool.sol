...
contract AVICPool is Ownable, ContractGuard {
    ...
    uint256 public constant runningTime = 17 hours;
    uint256 public TOTAL_AVIC_REWARD = 0;
    uint256 public TOTAL_CHAM_REWARD = 0;
    ...
    uint256 public constant emergencyWithdrawFeePercent = 50; // 50%
    ...
    // WRITE FUNCTIONS
    function deposit(uint256 _amount) external onlyOneBlock {
    ...
    function withdraw() external onlyOneBlock {
    ...
    function distributeReward(uint256 _avicAmount, uint256 _chamAmount) external onlyAdmin {
        require(avicLastRewardTime < poolEndTime && chamLastRewardTime < poolEndTime, "Pool is ended");
        avicToken.safeTransferFrom(msg.sender, address(this), _avicAmount);
        chamToken.safeTransferFrom(msg.sender, address(this), _chamAmount);
        TOTAL_AVIC_REWARD = TOTAL_AVIC_REWARD.add(_avicAmount);
        TOTAL_CHAM_REWARD = TOTAL_CHAM_REWARD.add(_chamAmount);

        uint256 totalAvicReward = avicToken.balanceOf(address(this));
        uint256 totalChamReward = chamToken.balanceOf(address(this));

        avicTokenPerSecond = totalAvicReward.div(poolEndTime.sub(avicLastRewardTime));
        chamTokenPerSecond = totalChamReward.div(poolEndTime.sub(chamLastRewardTime));
