...
contract NFTVault is Ownable, KeeperCompatibleInterface {
    ...
    function withdrawNFT(uint256 nftTokenId) public onlyOwner {
        require(
            nftContract.getFractionalization(nftTokenId).totalSupply == 0,
    ...
    function _fractionalize(uint256 tokenId) private {
        ...
        require(
            !nftContract.getFractionalization(tokenId).isFractionalized,
            "Token ID is already fractionalized"
        );
        uint256 totalSupply = nftContract
            .getFractionalization(tokenId)
            .totalSupply;
        fractionalToken.setClaimAmount(
            nftContract.ownerOf(tokenId),
            totalSupply
        );
        nftContract.getFractionalization(tokenId).isFractionalized = true;
    }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        for (uint256 i = 0; i < nftContract.getTokenIdsCount(); i++) {
            ...
            if (!nftContract.getFractionalization(tokenId).isFractionalized) {
...
interface IFractionalToken {
    ...
    function setClaimAmount(address _claimAddress, uint256 _claimAmount) external;
