...
contract BridgesOfFate is Ownable  {
    ...
    uint256 private constant THERSHOLD = 200; // 5
...
//  
    ...
    function entryFeeSeries(bytes32  _playerId,uint256 _nftId,uint8 _seriesType) public  {
        ...
        if(lastUpdateTimeStamp > 0){
            require(_dayDifferance(block.timestamp, lastUpdateTimeStamp) < 2,"Game Ended !");
            if(GameStatusInitialized[latestGameId].startAt > 0){
                require(_dayDifferance(block.timestamp, GameStatusInitialized[latestGameId].startAt) <= STAGES + THERSHOLD, "Threshold Achieved!");
            }
        }
