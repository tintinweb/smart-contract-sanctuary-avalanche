...
contract LUSDAllocator is MetahubAccessControlled {
    /* ======== DEPENDENCIES ======== */
    ...
    ITreasury public treasury; // Metahub Treasury
    ...
    function withdraw(address token, uint256 amount) external onlyGuardian {
        ...
        if (token == lusdTokenAddress) {
            ...
            treasury.deposit(balance, token, value); // deposit using value as profit so no MHUB is minted
        } else {
    ...
    /**
    Helper method copying MetahubTreasury::_tokenValue(), whose name was 'valueOf()' in v1
