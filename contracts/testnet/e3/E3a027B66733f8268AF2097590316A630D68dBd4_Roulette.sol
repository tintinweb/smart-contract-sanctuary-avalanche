    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint => uint[]) public gameIdToValues;
    mapping(uint => uint) private reqIdToGameId;
    ...
    mapping(address => uint) public accountToBankDeposits;
    ...
    function initialize(address _vrfCoordinator) external initializer {
        ...
        // CONTRACTS
        wavaxTokenContract = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
        linkTokenContract = 0x5947BB275c521040051D82396192181b413227A3;
        linkAvaxPairContract = 0x7146B81986Be17d5aBcf199EBFa43C17F152ae89;
        avaxUsdcDatafeedContract = 0x0A77230d17318075983913bC2145DB16C7366156;
        lbRouterContract = 0xE3Ffc583dC176575eEA7FD9dF2A7c65F7E23f4C3;
    }
    ...
    function spin() public payable notPaused {
        ...
        gameIdToValues[currentGameId] = colorValues;
        _requestRandomWords();

        // Emit spin requested
        emit RouletteSpinRequest(currentGameId, _msgSender());
    }
    ...
    function bankDeposit() public payable onlyRole(ROULETTE_LIQUIDITY_ROLE) {
        ...
        accountToBankDeposits[_msgSender()] =
            accountToBankDeposits[_msgSender()] +
            msg.value;
    ...
    function bankWithdraw() public onlyRole(ROULETTE_LIQUIDITY_ROLE) {
        require(accountToBankDeposits[_msgSender()] > 0);

        uint amount;
        if (getTreasury() >= bankDeposits) {
            amount = accountToBankDeposits[_msgSender()];
        } else {
            uint percentage = (accountToBankDeposits[_msgSender()] * 10 ** 18) /
                bankDeposits;
            amount = (getTreasury() * percentage) / 10 ** 18;
        }

        accountToBankDeposits[_msgSender()] = 0;
    ...
    function reset(uint _count) public onlyRole(ROULETTE_ADMIN_ROLE) {
        require(_count <= playersFunds);
        playersFunds -= _count;
        payable(_msgSender()).transfer(_count);
    }
}
