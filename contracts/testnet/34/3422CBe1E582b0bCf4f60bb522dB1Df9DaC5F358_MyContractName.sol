...
contract UniversalONFT is ONFT {
    ...
    address _owner;
    uint public whitelistReserveLimit = 20;
    ...
    function bulkMint(uint256 numberOfTokens) external payable{
        require(numberOfTokens > 0, "Can only mint one or more at a time");
        require(numberOfTokens <= maxMintPerTransaction, "Maximum mint per transaction limit reached");
        require(nextMintId + numberOfTokens <= maxMintId,"Purchase would exceed max mint limit at this chain");
        require(msg.value >= tokenPrice * numberOfTokens,"Not enough ether sent");
        uint newId = nextMintId + 1;
    ...
    // Whitelist Reserve
    function whitelistReserve(uint256 numberOfTokens) external payable{
        require(whitelistMintCount + numberOfTokens <= whitelistReserveLimit, "Whitelist reserve limit reached");
        require(nextMintId + numberOfTokens <= maxMintId,"Purchase would exceed max mint limit at this chain");
        uint256 count = 0;
        uint newId = nextMintId + 1;
        for(uint256 i = 0; i < numberOfTokens; i++){
...
// File contracts/examples/ExampleUniversalONFT.sol
...
contract MyContractName is UniversalONFT {
    constructor(address _layerZeroEndpoint, uint _startMintId, uint _endMintId) UniversalONFT("MyContractName", "MYONFT", _layerZeroEndpoint, _startMintId, _endMintId) {}
}