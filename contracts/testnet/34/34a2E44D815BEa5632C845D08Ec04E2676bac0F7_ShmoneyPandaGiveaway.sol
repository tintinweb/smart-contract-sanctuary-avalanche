...
contract ShmoneyPandaGiveaway is Ownable, ReentrancyGuard {
    ...
    /// @param numberOfWinners The number of winners to draw for this execution. Allows for drawing a subset of winners in case gas limits are reached
    ///                        when drawing lots of winners.
    function selectRandomWinners(uint256 numberOfWinners) external onlyAdmin {
        require(numberOfWinners >= 1, "Not enough winners");
        require(numberOfWinners <= _maxGiveawayDrawsPerTx, "Too many winners in one transaction");

        GiveawayDetail storage giveaway = giveawayDetails[_giveawayCount];
        require(!giveaway.allWinnersSelected, "All winners already selected");
        ...
        if (currentWinnerCount + numberOfWinners > giveaway.numberOfWinners) {
            numberOfWinners = giveaway.numberOfWinners - currentWinnerCount;
        }

        uint256 giveawayId = giveaway.id;
        ...
        while (winnerCount < numberOfWinners) {
            uint256 tokenIndex = uint256(keccak256(abi.encode(giveaway.randomNumber, winnerCount))) % totalSupply;

            // The same token can't win twice in the same giveaway
            if (_winnerIndices[tokenIndex] != giveawayId) {
                uint256 tokenId = shmoneyPandaTokenContract.tokenByIndex(tokenIndex);
                address winnerAddress = shmoneyPandaTokenContract.ownerOf(tokenId);

                pandaWinners[totalWinnerCount] = PandaWinner(totalWinnerCount, giveawayId, tokenId, winnerAddress);

                _winnerIndices[tokenIndex] = giveawayId;
                totalWinnerCount++;
                winnerCount++;

                _addressClaimCountByGiveawayId[winnerAddress][giveawayId] += 1;
            }
        }
        ...
        if (_currentWinnerCount >= giveaway.numberOfWinners) {
            _currentWinnerCount = 0;
            giveaway.allWinnersSelected = true;
        }
    }
