...
// File soliutils/contracts/[email protected]
...
// File @ndujalabs/attributable/contracts/[email protected]
...
interface IUserSimplified {
  ...
  event Deposited(uint8 tokenType, address user, uint256 amount);
...
// File @ndujalabs/lockable/contracts/[email protected]
...
interface IGamePool is IUserSimplified {
  ...
  event NewDeposit(uint64 depositId, address user);
  /**
  ...
  */
  ...
  function getUserDepositedSeed(address user) external view returns (uint256);

  /**
      @dev returns the number of active deposits
      @return the number of active deposits
    */
  function getStakeAmount(address user, uint8 tokenType) external view returns (uint256);
  ...
  function totalDepositedAmount(address user, uint8 tokenType) external view returns (uint256);
...
// File contracts/V1/GamePool.sol
...
contract GamePool is IGamePool, SignableStakes, Constants, UUPSUpgradableTemplate, IAttributablePlayer {
  ...
  mapping(bytes31 => uint8) private _usedSignatures;
  ...
  // _stakedByTokenId[tokenType][tokenId] = {owner, depositIndex}
  mapping(uint8 => mapping(uint16 => TokenData)) internal _stakedByTokenId;

  // TODO when ready, remove _depositsById and rename _depositsByIdV2 > _depositsById. Right now there are two variable because changing the first one would have broken the upgradeability of the contract
  mapping(uint32 => DepositInfo) private _depositsById;
  mapping(uint64 => DepositInfo) private _depositsByIdV2;
  ...
  function initialize(
    ...
    __Signable_init();
    require(turf.isContract(), "GamePool: turf not a contract");
    require(farm.isContract(), "GamePool: farm not a contract");
    require(seed.isContract(), "GamePool: seed not a contract");
    require(bud.isContract(), "GamePool: bud not a contract");
  ...
  function fixTokenAddresses(
    address turf,
    address farm,
    address seed,
    address bud
  ) external onlyOwner {
    require(block.chainid != 56, "GamePool: function executable only on testnet");
    if (turf != address(0)) {
      require(turf.isContract(), "GamePool: turf not a contract");
      turfToken = IAsset(turf);
    }
  ...
  function stakeAsset(uint8 tokenType, uint16 tokenId) external override {
    //TODO: use a require to check token types
    // it will revert if GamePool not approved to spend the token
    // and/or if GamePool is not an authorized locker
    if (tokenType == TURF) {
      require(!turfToken.isLocked(tokenId), "GamePool: turf already locked");
      conf.turfAmount++;
      turfToken.lock(tokenId);
    } else if (tokenType == FARM) {
      require(!farmToken.isLocked(tokenId), "GamePool: farm already locked");
      conf.farmAmount++;
      farmToken.lock(tokenId);
    } else {
      revert("GamePool: unsupported nft");
    }
  ...
  function unstakeAsset(
    ...
    require(tokenType == TURF || tokenType == FARM, "GamePool: invalid token type");
    ...
    require(
      isSignedByValidator(0, hashUnstake(tokenType, tokenId, stakeIndex, randomNonce), signature0),
      "GamePool: invalid signature0"
    );
    require(
      isSignedByValidator(1, hashUnstake(tokenType, tokenId, stakeIndex, randomNonce), signature1),
    ...
    require(found, "GamePool: asset not found");
    _users[_msgSender()].stakes[tokenType][index].unlockedAt = uint32(block.timestamp);
    if (tokenType == TURF) {
      require(turfToken.isLocked(tokenId), "GamePool: turf not locked");
      conf.turfAmount--;
      turfToken.unlock(tokenId);
    } else {
      // shouldn't we explicitly check for else if tokenType == FARM?
      require(farmToken.isLocked(tokenId), "GamePool: farm not locked");
      conf.farmAmount--;
  ...
  function getStakeAmount(address user, uint8 tokenType) external view override returns (uint256) {
  ...
  function getUserDepositedSeed(address user) external view override returns (uint256) {
    return _users[user].seedAmount;
  }

  function getUserDepositedBud(address user) external view returns (uint256) {
    return _users[user].budAmount;
  }
  ...
  function _saveSignatureAsUsed(bytes memory _signature) internal {
    bytes31 key = bytes31(keccak256(abi.encodePacked(_signature)));
    require(_usedSignatures[key] == 0, "GamePool: signature already used");
  ...
  function depositSeed(
    ...
    require(
      isSignedByValidator(0, hashDeposit(_msgSender(), amount, depositId, randomNonce), signature0),
      "GamePool: invalid signature0"
    );
    _saveSignatureAsUsed(signature0);
  ...
  function depositSeedAndPayOtherUser(
    ...
    _saveSignatureAsUsed(signature0);
    require(recipient != address(0) && recipient != address(this), "GamePool: invalid recipient");
    uint256 percentage = nftTokenType == TURF ? 92 : nftTokenType == FARM ? 95 : 0;
    require(percentage != 0, "GamePool: invalid NFT");
  ...
  function _depositFT(
    ...
    if (tokenType == SEED) {
      _users[user].seedAmount += amount;
    } else {
      _users[user].budAmount += amount;
    }
    Deposit memory deposit = Deposit({tokenType: tokenType, amount: amount, depositedAt: uint32(block.timestamp)});
    _depositsByIdV2[depositId] = DepositInfo({index: uint16(_users[user].deposits.length), user: user});
    _users[user].deposits.push(deposit);
    emit Deposited(tokenType, user, amount);
    // it will revert if spend not approved or if insufficient balance
    if (tokenType == SEED) {
      seedToken.transferFrom(user, address(this), amount);
    } else {
      budToken.transferFrom(user, address(this), amount);
    }
    emit NewDeposit(depositId, user);
  }
  ...
  function totalDepositedAmount(address user, uint8 tokenType) external view override returns (uint256) {
    require(tokenType == SEED || tokenType == BUD, "GamePool: invalid token type");
    if (tokenType == SEED) {
      return _users[user].seedAmount;
    } else {
      return _users[user].budAmount;
  ...
  // TODO: I think this external function should be renamed to numberOfDeposits
  // using "amounts" might be confusing since it's not an enumeration
  ...
  function depositById(uint64 depositId) external view override returns (Deposit memory) {
    DepositInfo memory info = _depositsByIdV2[depositId];
  ...
  // A breach here, i.e., someone getting the private key of the validator
  // would be a total disaster. I think this is not a viable solution, even if
  // just temporarily. The amount of harvestable tokens should derive from
  // parameters on chain that cannot be forced or exploited.
  function harvest(
    uint256 amount,
    uint256 deadline,
    uint256 randomNonce,
    ...
    require(deadline > block.timestamp, "GamePool: harvesting expired");
    require(
      isSignedByValidator(0, hashHarvesting(_msgSender(), amount, deadline, randomNonce), signature0),
      "GamePool: invalid signature0"
    );
    require(
      isSignedByValidator(1, hashHarvesting(_msgSender(), amount, deadline, randomNonce), signature1),
      "GamePool: invalid signature1"
    );
    _saveSignatureAsUsed(signature0);
    _saveSignatureAsUsed(signature1);
    budToken.mint(_msgSender(), amount);
    emit Harvested(_msgSender(), amount);
  }
  ...
  function withdrawFT(
    ...
    require(amount < balance + 1, "GamePool: amount not available");
    if (amount == 0) {
      amount = balance;
    }
  ...
  function updateTurfAttributes(
    ...
    require(
      isSignedByValidator(0, hashTurfAttributes(tokenId, attributes, randomNonce), signature0),
      "GamePool: invalid signature0"
    );
    require(
      isSignedByValidator(1, hashTurfAttributes(tokenId, attributes, randomNonce), signature1),
      "GamePool: invalid signature1"
    );
    _saveSignatureAsUsed(signature0);
    _saveSignatureAsUsed(signature1);
  ...
  // Most likely, we do not need this function because tokens are pre-initialized
  // after the reveal.
  ...
  function updateFarmAttributes(
    ...
    require(
      isSignedByValidator(0, hashFarmAttributes(tokenId, attributes, randomNonce), signature0),
      "GamePool: invalid signature0"
    );
    require(
      isSignedByValidator(1, hashFarmAttributes(tokenId, attributes, randomNonce), signature1),
      "GamePool: invalid signature1"
    );
    _saveSignatureAsUsed(signature0);
    _saveSignatureAsUsed(signature1);
