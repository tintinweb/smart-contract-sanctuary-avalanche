...
contract AuctionManagerCustomToken is Ownable {
        ...
        )
    {
        ...
        highestBid = new uint256[](_auctionIdCounter);
        tokenIds = new uint256[](_auctionIdCounter);
        auctionNFTAddress = new address[](_auctionIdCounter);
        auctionIds = new uint256[](_auctionIdCounter);

        for (uint256 i = 0; i < _auctionIdCounter; i++) { // for each auction
            _auctionState = uint256(getAuctionState(auctions[i].auctionId));
            if(_auctionState == 2 || _auctionState == 3) {
                _auctionOwner = auctions[i].creator; // get the owner of the auction
                _highestBidder = auctions[i].maxBidder;
                _highestBid = auctions[i].maxBid;
                if(msg.sender == _auctionOwner) {
                    highestBid[i] = _highestBid;
                    tokenIds[i] = auctions[i].tokenId; // get the token id
                    auctionNFTAddress[i] = auctions[i].nftAddress;
                    auctionIds[i] = auctions[i].auctionId;
                }
            }
        }
