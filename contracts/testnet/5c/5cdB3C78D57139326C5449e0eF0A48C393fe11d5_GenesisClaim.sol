...
contract GenesisClaim is Ownable, Pausable, Multicall {
	...
	function _purchase(uint256 _nftsToPurchase) internal {
		...
		uint256 purchasable = reservedRemaining(msg.sender);
		uint256 decimals = usdc.decimals();

		if (info.reserved == type(uint256).min || purchasable == 0 || whitelistWindow == false) {
			// TODO --- HERE --- TODO //
			...
			require(_nftsToPurchase <= purchasable, "Exceeds reservation!");
		...
		// } else if (info.price == publicMint){ // ! wrong
