...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 public lastSalvorRewardBalance;
    ...
    */
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (_rewardBalance != lastRewardBalance && _totalVeART != 0) {
            ...
            _accRewardTokenPerShare += (((_accruedReward * ACC_REWARD_PER_SHARE_PRECISION * 8) / _totalVeART) / 10);
        }
    ...
    */
    function pendingSalvorRewards(address _user) external view returns (uint256) {
        ...
        if (_rewardBalance != lastRewardBalance && totalSalvorSupply != 0) {
            uint256 _accruedReward = _rewardBalance - lastRewardBalance;
            _accRewardTokenPerShare += (((_accruedReward * ACC_REWARD_PER_SHARE_PRECISION * 2) / totalSalvorSupply) / 10);
        }
        return ((user.amount * _accRewardTokenPerShare) / ACC_REWARD_PER_SHARE_PRECISION) - user.rewardDebt;
    }

    function pendingSalvorRewardsV2(address _user) external view returns (uint256, uint256, uint256) {
        UserSalvorInfo memory user = salvorUsers[_user];
        uint256 _accRewardTokenPerShare = accSalvorRewardPerShare;
        uint256 _rewardBalance = address(this).balance;
        uint256 _accruedReward = _rewardBalance - lastRewardBalance;
        _accRewardTokenPerShare += (((_accruedReward * ACC_REWARD_PER_SHARE_PRECISION * 2) / totalSalvorSupply) / 10);
        return (_accRewardTokenPerShare, ACC_REWARD_PER_SHARE_PRECISION, _accruedReward);
    }
    ...
    */
    function _updateReward() internal {
        ...
        accRewardPerShare += ((8 * _accruedReward * ACC_REWARD_PER_SHARE_PRECISION / 10) / _totalVeART);
        lastRewardBalance = _rewardBalance;
    }

    function _updateSalvorReward() internal {
        uint256 _rewardBalance = address(this).balance;

        if (_rewardBalance == lastSalvorRewardBalance || totalSalvorSupply == 0) {
            return;
        }

        uint256 _accruedReward = _rewardBalance - lastRewardBalance;
        accSalvorRewardPerShare += ((2 * _accruedReward * ACC_REWARD_PER_SHARE_PRECISION / 10) / totalSalvorSupply);
        lastSalvorRewardBalance = _rewardBalance;
    }
