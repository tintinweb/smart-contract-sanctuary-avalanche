...
contract Marketplace is ReentrancyGuard, Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder, ERC1155Holder {
    ...
    // From ERC721 tokenId to Order (to avoid asset collision)
    mapping(address => mapping(uint256 => Order)) public orderBytokenId;

    // From ERC721 tokenId to Bid (to avoid asset collision)
    ...
    function _createOrder(
        ...
        if (nftType == "721") {
            MockERC721(_nftAddress).safeTransferFrom(msg.sender, address(this), _tokenId);
        } else if (nftType == "1155") {
            MockERC1155(_nftAddress).safeTransferFrom(msg.sender, address(this), _tokenId, _amountOfErc1155, "");
        }
...
contract MockERC1155 is ERC1155URIStorage, INft {
    ...
    constructor(
        ...
        string memory _symbol,
        address _marketplaceAddress
    ) ERC1155(string(abi.encodePacked("", "{id}"))) {
        name = _name;
        symbol = _symbol;
        marketplaceAddress = _marketplaceAddress;
    }
