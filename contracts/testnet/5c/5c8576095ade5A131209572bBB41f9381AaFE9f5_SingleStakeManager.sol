...
/* This contract serves as the main point of contact between any SingleStakingRewards creators and their farm contract.  It contains any function in SingleStakingRewards
that would normally be restricted to the owner and allows access to its functionality as long as the caller is the known owner in the SingleStakeFactory contract. */
...
contract SingleStakeManager is Ownable {
    ...
    // The check used by each function that interacts with the SingleStakingRewards contract. It reads from the owners stored in SingleStakingFactory to determine if the caller is 
    ...
    // Starts the farm emission for the given SingleStakingRewards contract address.  The amount of rewards per rewards token and the duration of the
    // farm emissions must be supplied.  Any reward tokens must be sent to the SingleStakingRewards contract before this function is called.
    function startEmission(address singleStakeAddress, uint256[] memory _rewards, uint256 _duration) external checkOwnership(singleStakeAddress) {
    ...
    function stopEmission(address singleStakeAddress) external checkOwnership(singleStakeAddress) {
    ...
    function recoverLeftoverReward(address singleStakeAddress, address _tokenAddress) external checkOwnership(singleStakeAddress) {
    ...
    function addRewardToken(address singleStakeAddress, address _tokenAddress) external checkOwnership(singleStakeAddress) {
    ...
    function recoverERC20(address singleStakeAddress, address _tokenAddress, uint256 _amount) external checkOwnership(singleStakeAddress) {
    ...
    function setFees(address singleStakeAddress, uint16 _depositFeeBps, uint16[] memory _withdrawalFeesBps, uint32[] memory _withdrawalFeeSchedule) external checkOwnership(singleStakeAddress) {
        ISingleStakingRewards(singleStakeAddress).setFees(_depositFeeBps, _withdrawalFeesBps, _withdrawalFeeSchedule);
    }

    // Withdraw fees collected from deposits/withdrawals in the SingleStakingRewards contract to msg.sender.
    function recoverFees(address singleStakeAddress) external checkOwnership(singleStakeAddress) {
