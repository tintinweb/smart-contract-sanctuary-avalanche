...
contract TokenStakePoolChefLike is ReentrancyGuard, Ownable, Pausable {
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view onlyActive(_id) returns (uint256) {
    return (userInfo[_id][_user].stakedAmount * currentRewardPerStakedToken(_id)
                                                 - userInfo[_id][_user].rewardDebt)
                                                 / SAFE_MULTIPLIER;
  }
  ...
  */
  function _updateUserRewardDebt(uint256 _id, address _user, uint256 _amount) private {
    userInfo[_id][_user].rewardDebt = userInfo[_id][_user].rewardDebt
  ...
  */
  function _updateUserRewardAccrued(uint256 _id, address _user) private {
    if (_user != address(0)) {
      userInfo[_id][_user].rewardDebt = 0;
    }
  }
  ...
  */
  function deposit(uint256 _id, uint256 _amount) external nonReentrant whenNotPaused onlyActive(_id) {
    ...
    userInfo[_id][msg.sender].stakedAmount = userInfo[_id][msg.sender].stakedAmount + _amount;
  ...
  */
    ...
    onlyActive(_id)
  {
    ...
    userInfo[_id][msg.sender].stakedAmount = userInfo[_id][msg.sender].stakedAmount - _amount;
    _updateUserRewardDebt(_id, msg.sender, userInfo[_id][msg.sender].stakedAmount);
  ...
  */
  function claim(uint256 _id) public whenNotPaused onlyActive(_id) {
    ...
    if (rewards > 0) {
      ...
      userInfo[_id][msg.sender].rewardDebt = userInfo[_id][msg.sender].stakedAmount
  ...
  */
  function exit(uint256 _id) external onlyActive(_id) {
    claim(_id);
    withdraw(_id, userInfo[_id][msg.sender].stakedAmount);
  }
