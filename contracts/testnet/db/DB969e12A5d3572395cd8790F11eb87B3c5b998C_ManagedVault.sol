...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    // One mean calendar year has 8765.82 hours. This constant is equivalent to 8765.82 x 60 x 60.
    ...
    // Pending deposits for the network's native currency (eg. ETH for the Ethereum network)
    PendingDeposit.Queue pendingNativeDeposits;
    // A mapping from deposit token address to pending token deposits for whitelisted tokens
    mapping(IERC20 => PendingDeposit.Queue) public pendingTokenDeposits;

    // The price of 1e18 of this contract's token in USD with a given precision such as 1e6.
    // The given precision must be the same for all prices.
    // For instance, if a precision value of 6 is used for a vault, $100 vault price should be set as:
    // 1e2 * 1e6 * 1e18 = 1e26
    uint256 public vaultPrice;

    // The block number set in the last setPrices function call
    ...
    // Replicating OpenZeppelin's Pausable functionality as we cannot add another class dependency due to upgradeable storage patterns.
    bool public paused;

    // Replicating OpenZeppelin's ReentrancyGuard functionality as we cannot add another class dependency due to upgradeable storage patterns.
    // reentrancyGuard is initially 0. We change the parameter between 1 and 2 later for less gas usage.
    uint256 private reentrancyGuard;

    /**
     * @notice Locked vault token amounts that can be only locked and unlocked by the contract.
     *         Used for locking redeemed tokens.
     */
    mapping(address => uint256) public locks;

    // Redemption settings
    uint256 public redemptionFeeBPS;
    IERC20 public nextRedemptionToken; // The token that will be used for settling the next redemption epoch. Must be among the deposit tokens.
    ...
    mapping(uint256 => mapping(address => uint256)) public redemptions; // redemption epoch => redeemer => total redeemed vault token amount
    ...
    uint256 public performanceFeeEntrance; // Unit is a virtual USD amount with the precision used for the prices. $1 would mean 1e24 if 6 price precision is used for prices and 1e18 precision is used for amounts.
     ...
     *         Previously settled redemption epoch proceeds can be withdrawn via withdraw(address account, uint256 redemptionEpochId).
     *
     * @dev Can be called more than once for during the same redemption epoch to override the redemption amount.
    ...
    function redeem(uint256 amount) whenNotPaused external {
        ...
        // _lock and _unlock functions have the necessary require statements for the amounts
        if (oldAmount < amount) {
    ...
    /**
     * @notice Withdraws specified epoch's settlement proceeds (in the form of the token that is set as the redemption token, eg. "USDC") and
     *         burns the redeemed vault tokens.
     *
     * @dev Can be executed for any past epoch at any point in time in which a user redemption was initialized and settled.
     *
    ...
    function withdraw(address account, uint256 redemptionEpochId) whenNotPaused external {
        ...
        require(reentrancyGuard != 2, "ReentrancyGuard: reentrant call");
        reentrancyGuard = 2;
    ...
    /**
     * @notice Sets the vault's token price, native currency price, and all whitelisted token prices for a given blockNumber.
     *         Afterwards, the pending deposits up until blockNumber can be minted.
     *
     * @dev The native currency price must be set regardless of its deposit status. The token prices should be set in the same order as tokenAddresses() *      output.
     ...
     * @param tokenAddresses Array of token addresses to set prices for. The array must be in the right order.
     * @param tokenPricesAtTheBlock Array of token prices.
     */
     ...
     *
     * @dev Removes the pending deposits, and adds vault token balances to depositors.
     *
     * @param maxMints      Maximum number of individual deposits that will be minted. Added to prevent exceeding block gas limit.
     *
    ...
    function mint(uint256 maxMints) external onlyOwner returns (uint256 mintCount) {
        ...
        for (uint256 tokenIndex = 0; tokenIndex < tokensCount;) {
            ...
            while (0 < remainingPendingTokenDeposits && mintCount < maxMints) {
                ...
                // Performance fee logic
                performanceFeeEntrance += vaultTokenAmount * stackTokenPrice;
            }
     ...
     *
     * @dev Since a new epoch is initiated, redeem() calls ensuing this function's execution will be applied for the new redemption epoch.
     *      This effectively means that redemption amounts for the previous epoch are now locked-in, new redeems cannot be added,
     *      existing ones can no longer be modified.
    */
    ...
    /**
     * @notice Calculates and transfers the amount of tokens required to fulfill redemptions for the last redemption epoch. 
     *         Afterwards, redeemed amounts for that epoch are enabled for withdrawal.
     * @dev Prior to execution of this function, the vault owner must verify that the vault owner account is
     *      in possesion of the amount of required redemptionTokens and the vault contract has necessary token
     *      allowance from the owner. Otherwise, the call will revert with TransferFailed()
     *
     * @param redemptionEpochId Redemption epoch id, epoch mustz be in "Pending" state.
    */
    function settleRedemptionEpoch(uint256 redemptionEpochId) external onlyOwner {
        ...
        if (0 < redemptionEpochId) {
            require(redemptionEpochs[redemptionEpochId - 1].state == RedemptionEpochState.Settled);
        }
    ...
    */
    ...
    /**
     * @notice Mints the specified amount of performance fees earned by the vault in the form of new vault tokens
     *
    ...
    */
    ...
    /**
     * @notice Mints the specified amount of management fees earned by the vault in the form of new vault tokens
     *
     * @param to The address to send the newly minted tokens
    ...
    */
    ...
    /**
     * @notice Sets the native currency (eg. ETH for the Ethereum network) whitelist state.
     *
     * @dev {depositNative} function won't work unless this function is set to true.
     *      Emits a {SetNativeDepositState} event if whitelist state changes for native cryptocurrency.
     *
    ...
    /**
     * @notice Sets ERC20 token whitelist state. Only whitelisted tokens are accepted for deposits.
     *
     * @dev If the state is true, the token is allowed for deposits or can be set as nextRedemptionToken. Otherwise depositToken function reverts.
     *      Emits a {SetTokenDepositState} event if whitelist state changes for the token.
     *
     *      Emits a {SetTokenDepositState} event if whitelist state changes for the token.
    ...
    /**
     * @notice Sets the minimum amount of native currency that can be deposited.
     *
    ...
    /**
     * @notice Sets the ERC20 token that will be used to settle redemptions in future epochs
     *
     * @dev Must be present and set to "true" in tokenDepositStates
     *      Emits a {SetTokenMinimumDepositAmount} event.
     *
    ...
    function setPerformanceFeeInitiationTimestamp(uint256 timestamp) external onlyOwner {
        require(performanceFeeInitiationTimestamp != timestamp);

        performanceFeeInitiationTimestamp = timestamp;

        emit SetPerformanceFeeInitiationTimestamp(timestamp);
    }
    ...
    function setPerformanceFeeAccrueWindowSeconds(uint256 durations) external onlyOwner {
        require(performanceFeeAccrueWindowSeconds != durations);

        performanceFeeAccrueWindowSeconds = durations;

        emit SetPerformanceFeeAccrueWindowSeconds(durations);
    }
     ...
     *
     * @dev Only considers deposits at or before setPriceBlockNumber.
    ...
    /**
     * @notice Returns the remaining number of native currency (eg. ETH for the Ethereum network) deposits amount for a given depositor.
     *
    ...
    /**
     * @notice Returns the total remaining deposit amount for the native currency (eg. ETH for the Ethereum network) up to a block number.
     *
    ...
    /**
     * @dev Returns the amount of free vault tokens owned by `account`. The definition of free here is unlocked or unlockable.
     *      If the account `redeem`s tokens, they are locked and these tokens can be unlocked until `rollRedemptionEpoch` is executed. After
     *      `rollRedemptionEpoch`, these tokens can no longer be unlocked so, they are deducted from balanceOf and can only be withdrawn.
     */
    ...
    /**
     * @dev Returns the amount of free vault tokens in circulation. The definition of free here is unlocked or unlockable.
     *      The redeemed tokens in previous redemptionEpochs cannot be unlocked. Therefore, they're not included
     *      in either balanceOf or totalSupply.
     */
    function totalSupply() public view override returns (uint256) {
        return super.totalSupply() + uint(managementFeeAccruedVaultTokenAmount) - withdrawableVaultTokenTotalAmount;
    }

    /**
     * @dev Returns the gross value of vault assets (not accounting for earned fees)
     *      Used in PNL and fee calculations.
    */
    ...
    /**
     * @dev Returns elapsed number of redemption epochs.
    */
    ...
    /**
     * @dev Returns net vault price (accounting for earned fees)
     *      Mints and redeems are executed at this price.
    */
    ...
    /**
     * @dev Calculates and updates accrued management fees based on grossAssetValue(). Executed during the mint() function call.
     *
     */
    function _accrueManagementFee() internal {
        if (0 < managementFeeLastUpdatedTimestamp) {
...
interface IManagedVault {
     ...
     *         function. If the callsucceedes, the state is set as 1 (Settled) and the investors are free to withdraw
     *         their redemptions.
     */
    ...
    /**
     * @notice Redemptions are settled in epochs. Investors can redeem their funds anytime. When the owner calls the
    ...
    event RollRedemptionEpoch(uint256 indexed redemptionEpochId, IERC20 indexed redemptionToken, uint256 redemptionTokenPrice, uint256 netVaultPriceAfterFees, uint256 vaultTokenTotalAmount);

    event SettleRedemptionEpoch(uint256 indexed redemptionEpochId, IERC20 indexed redemptionToken, uint256 totalRedemptionTokenAmount);
    ...
    event SetPerformanceFeeInitiationTimestamp(uint256 timestamp);
    ...
    /**
     * @notice Thrown when accrue period hasn't started yet
     * @param performanceFeePeriodCount The performance fee period for which the fee tried to be accrued
     * @param accrueWindowFirstTimestamp The first allowed timestamp of the accrue period
     * @param currentTimestamp Current timestamp
     */
    error EarlyPerformanceFeeAccrue(uint256 performanceFeePeriodCount, uint256 accrueWindowFirstTimestamp, uint256 currentTimestamp);

    /**
     * @notice Thrown when accrue period has passed
     * @param performanceFeePeriodCount The performance fee period for which the fee tried to be accrued
     * @param accrueWindowLastTimestamp The last allowed timestamp of the accrue period
     * @param currentTimestamp Current timestamp
     */
