...
//TODO: move thie file to upgrade folder???? chicken and Egg
import "../interfaces/IInfinityPool.sol";
import "../interfaces/IWETH.sol";
import "../interfaces/IInfinityToken.sol";
import "../interfaces/IPoolAddressesProvider.sol";
...
import "../libraries/EmergencyWithdrawHelper.sol";
import "../libraries/TransferFundsHelper.sol";
// import "./libraries/LiquidationHelper.sol";
import "../libraries/LiquidationHelper2.sol";
...
contract UpgradeInfinityPoolGood is IERC721Receiver, IInfinityPool, Initializable, ContextUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
	...
	uint256 public constant version = 17;
	...
	function upgradeDummy() external pure returns(uint8)
	{
		return 2;
	}
	...
	function setPause(bool paused) public onlyDefaultAdminRole(msg.sender) {
		paused == true ? super._pause() : super._unpause();
    }

	function setPoolAddressesProvider(IPoolAddressesProvider _poolAddressProvider) public onlyDefaultAdminRole(msg.sender) {
		poolAddressProvider = _poolAddressProvider;
	}
...
/*
	...
	// take tokens
		for(uint i=0;i<tokenTransfers.length;i++){
			...
			if(ERC721Validator.isERC721(tokenTransfers[i].token)){
				require(ERC721Validator.isERC721Owner(tokenTransfers[i].token,address(_msgSender()),tokenAmount), "8");
		...
		if(msg.value>0){
			IWETH(poolAddressProvider.wethAddress()).deposit{value:msg.value}();
			// new array 
			_tt[tokenTransfers.length] = TokenTransfer(poolAddressProvider.wethAddress(),msg.value);
		}
	...
	function requestWithdraw(TokenTransfer[] calldata tokenTransfers) external whenNotPaused override{
		...
		for(uint i=0;i<tokenTransfers.length;i++){
			if(ERC721Validator.isERC721(tokenTransfers[i].token)){
				require(IInfinityToken(poolAddressProvider.infinityTokenAddress()).ifUserTokenExistsERC721(_msgSender(), uint256(uint160(tokenTransfers[i].token)), tokenTransfers[i].amount), "8");
			}else{
				require(IInfinityToken(poolAddressProvider.infinityTokenAddress()).balanceOf(_msgSender(),uint256(uint160(tokenTransfers[i].token)))>=tokenTransfers[i].amount, "7");
	...
	function emergencyWithdraw() whenNotPaused nonReentrant external{
		...
		//require(tokenTransfers.length>0, "1");
		EmergencyWithdrawHelper.emergencyWithdraw(poolAddressProvider);
	 ...
	 */
	function serverTransferAnyFunds(address clientAddress, TokenTransfer[] calldata tokenTransfers) onlyDefaultAdminRole(msg.sender) nonReentrant external {
		TransferFundsHelper.serverTransferFunds(
	 ...
	 * @dev only for ERC20
	 */
	function serverTransferPendingFunds() onlyTimelockRole(msg.sender) nonReentrant whenNotPaused external {
		TransferFundsHelper.serverTransferPendingFunds(pendingTransferFundsAddresses, pendingTransferFundsInfo);
	}

	function resetPendingTransferFundsInfo() onlyDefaultAdminRole(msg.sender) nonReentrant external {
	...
	function serverUpdateBalances(
	...
	) onlyBookkeeperRole(msg.sender) external override {
		...
		for(uint i=0;i<clientAddresses.length;i++){
			IInfinityToken(poolAddressProvider.infinityTokenAddress()).updateBalance(clientAddresses[i],tokenUpdates[i]);
		}
	}

	function serverLend(address _aaveLendingPoolAddress, address _cToken, address _underlyingAsset, uint256 _amount) onlyTreasurerRole(msg.sender) nonReentrant external {
	...
	// ) onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant external override {
	// 	address protocolAddress = liquidationProtocolAddresses[protocolId];
	// 	address swapProtocolAddress = liquidationProtocolAddresses[defaultSwapProtocolId];
	// 	ILiquidationProtocol.LiquidatedAmount[] memory amounts = LiquidationHelper.liquidate(protocolAddress, lparams, swapProtocolAddress);
	...
	function serverLiquidate(
		...
		ILiquidationProtocol.LiquidatedAmount[] memory amounts = LiquidationHelper2.liquidate(
			poolAddressProvider,
			_protocolIds,
			_paths,
			_amountIn,
