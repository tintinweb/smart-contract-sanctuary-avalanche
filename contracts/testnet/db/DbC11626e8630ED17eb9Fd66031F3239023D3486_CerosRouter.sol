...
import "./interfaces/IAvalanchePool.sol";
...
ReentrancyGuardUpgradeable
{
    ...
    IDex private _dex;
    IAvalanchePool private _pool; // default (AvalanchePool)
    // Tokens
    ...
    returns (uint256 value)
    {
        ...
        // let's calculate returned amount of aAVAXc from AvalanchePool
        ...
        if (amount >= minimumStake) { // amount >= minimumStake + relayerFee
            poolAAVAXcAmount = (amount * ratio) / 1e18; // ((amount - relayerFee) * ratio) / 1e18
        }
        // compare poolAAVAXcAmount with dexAAVAXcAmount from Dex
        // if poolAAVAXcAmount >= dexAAVAXcAmount -> stake via AvalanchePool
        // else -> swap on Dex
        uint256 realAmount;
        uint256 profit;
    ...
    returns (uint256 realAmount)
    {
        ...
        _pool.claimCertsFor(recipient, realAmount);
    ...
    {
        realAmount = _vault.withdrawFor(msg.sender, address(this), amount);
        _pool.claimCertsFor(recipient, realAmount); // realAmount -> AVAX
