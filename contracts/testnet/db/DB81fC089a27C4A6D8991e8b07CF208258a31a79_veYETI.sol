...
contract veYETI is IveYETI {
    ...
    /* UserInfo:
        ...
        -yetiStakes[i] is the amount of YETI staked on rewarder i
    ...
    */
    function update(RewarderUpdate[] memory _yetiAdjustments) external {
        ...
        _accumulate(msg.sender, _isStakeIncrease, _stakeAmount);
     ...
     */
    function _accumulate(address _user, bool _isStakeIncrease, uint _amount) internal {
        UserInfo storage userInfo = users[_user];
        ...
        uint256 dt = block.timestamp - userInfo.lastUpdate;
        uint256 veGrowth = userInfo.totalYeti * accumulationRate * dt;
        uint256 preRatioFinalveYeti = userInfo.lastTotalVeYeti + veGrowth;

        // ratio takes into account the penalty if unstaking
        uint256 ratio = _getRatio(_isStakeIncrease, _amount, userInfo.totalYeti);
        userInfo.lastTotalVeYeti = preRatioFinalveYeti * ratio / _1e18;
    }
    ...
    function _allocate(address _user, RewarderUpdate[] memory _yetiAdjustments) internal {
        ...
        for (uint i; i < nAdjustments; i++) {
            ...
            //            emissionsController.userUpdateVe(_user, rewarder, isIncrease, amount);
        }
    }
    ...
    function _handleStaking(UserInfo storage userInfo, bool _isIncreaseStake, uint _amount) internal {
        ...
        require(totalYeti <= _totalYetiSupply, "more Yeti staked in this contract than the supply");
    }
    ...
    /**
     * Calculates the penalty ratio due to unstaking YETI from veYETI.
     * After unstaking, a user's veBalance on a given rewarder is
     * ratio multiplied by the previous balance
     */
    function _getRatio(bool _isStakeIncrease, uint _amount, uint _totalPreviousStake) internal pure returns (uint) {
        if (_isStakeIncrease || _totalPreviousStake == 0) {
            // if you are adding to stake, then no penalty
            return _1e18;
        }
        // otherwise, there is a penalty which is used to calculate the ratio
        uint256 pctUnstaked = _amount * _1e18 / _totalPreviousStake;
        uint256 penalty = pctUnstaked * 2;
        if (penalty > _1e18) {
            return 0;
        }
        return _1e18 - penalty;
    }
