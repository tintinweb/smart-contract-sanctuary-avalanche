...
contract BetUpCore is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function setMintCondition(uint256[] memory _count, IBetStruct.MintCondition[] memory _condition) external onlyRole(PARAMS_MANAGER) {
    ...
    function setUpgradeConditionToLevel(uint8[] memory _levels, IBetStruct.MintCondition[] memory _condition) external onlyRole(PARAMS_MANAGER) {
  ...
  */
    ...
    function decreasePlayerAmount(address _player, uint256 _minusAmount) external onlyRole(RECORD_MANAGER) {
        require(playerRecords[_player].amount >= _minusAmount, "out out amount");
        playerRecords[_player].amount -= _minusAmount;
    }
