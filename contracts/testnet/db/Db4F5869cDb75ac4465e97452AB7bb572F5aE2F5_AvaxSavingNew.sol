...
contract AvaxSavingNew is ReentrancyGuard, Context {
    ...
    function newStake(address referrerID, uint8 plan)
        public
        payable
        nonReentrant
    {
        require(msg.value >= INVEST_MIN_AMOUNT, "min amount 0.1 AVAX");
        require(msg.sender != referrerID, "cannot reffer to self");
        require(plan < 6, "Incorrect plan");
        ...
        //if new Address or top level Id
        if (thisUser.referrer == address(0)) {
            if (users[referrerID].deposits.length == 0) {
                thisUser.referrer = referrerID;
            }
            address uplineAddr = thisUser.referrer;
            for (uint256 i = 0; i < 3; i++) {
                if (uplineAddr != address(0)) {
                    users[uplineAddr].downline[i].push(msg.sender);
                    users[uplineAddr].levels[i] = users[uplineAddr]
                        .levels[i]
                        .add(1);
                    uplineAddr = users[uplineAddr].referrer;
                } else break;
            }
        }

        if (thisUser.referrer != address(0)) {
            address upline = thisUser.referrer;
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(
                        PERCENTS_DIVIDER
