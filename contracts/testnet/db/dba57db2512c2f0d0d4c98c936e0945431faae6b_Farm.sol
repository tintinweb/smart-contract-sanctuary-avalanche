...
contract Farm is Ownable, FarmCore {
    ...
    function withdraw(
        ...
        users[_msgSender()].withdrawals[newWithdrawalId] = Withdrawal(_depositId, _amountFirst, _amountSecond, block.timestamp);
    ...
    function syncCrossChainWithdraw(
        ...
        users[_userAddress].withdrawals[newWithdrawalId] = Withdrawal(_depositId, _amountFirst, _amountSecond, block.timestamp);
        users[_userAddress].withdrawCount += 1;

        emit NewWithdraw(_userAddress, _amountFirst, _amountSecond);
    }
