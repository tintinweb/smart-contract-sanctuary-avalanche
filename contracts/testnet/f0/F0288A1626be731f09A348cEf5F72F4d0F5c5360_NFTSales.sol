...
contract NFTSales is ERC721Min, ReentrancyGuard {
    using Strings for uint256;
    using SafeERC20 for IERC20;

    mapping(address => bool) proxyToApproved; // proxy allowance for interaction with future contract
    string private _contractURI;
    string public baseURI = ""; // METADATA URI aaa
    address public treasuryAddress;
    bool public useBaseUriOnly = false;
    mapping(uint256 => string) public TokenURIMap; // allows assigning individual/unique metada per token
    ...
    uint256 public marketplaceFeePercent; // Fee % multiplied by 100
    ...
    mapping(uint256 => FeeRecipient) public FeeRecipients; // Artist, founder(s) etc. starting from id 1
    ...
    address marketplace_, uint256 marketplaceFee_) ERC721Min(name_, symbol_)
    {
        treasuryAddress = treasury_;
        marketplaceAddress = marketplace_;
        marketplaceFeePercent = marketplaceFee_;
    }
    ...
    function buy(uint8 amount) external payable {
        ...
        require(_owners.length + amount <= maxMint, "EXCEED_MAX_SALE_SUPPLY");
        require(maxPerMint == 0 || amount <= maxPerMint, "EXCEED_MAX_PER_MINT");
        require(maxPerAddress == 0 || balanceOf(_msgSender()) + amount <= maxPerAddress, "EXCEED_MAX_PER_USER");
    ...
    function buyWithToken(uint16 amount) external payable {
        ...
        require(maxPerMint == 0 || amount <= maxPerMint, "EXCEED_MAX_PER_MINT");
        require(maxPerAddress == 0 || balanceOf(_msgSender()) + amount <= maxPerAddress, "EXCEED_MAX_PER_USER");
        splitPaymentWithToken(priceWithToken * amount);
    ...
    function splitPayment(uint256 amount) internal {
        uint256 amountForMarketplace = 0;
        if (marketplaceFeePercent > 0) {
            // Pay fee to marketplaceAddress
            amountForMarketplace = amount * marketplaceFeePercent / 100 / 100;
            (bool sentToMarketplace, ) = marketplaceAddress.call{value: amountForMarketplace}("");
            require(sentToMarketplace, "FAILED_SENDING_FUNDS");
        }

        if (feeRecipientCount > 0) {
            // Pay the recipients
            uint256 amountForRecipients = amount - amountForMarketplace;
            for(uint256 x = 1; x <= feeRecipientCount; x++) {
                uint256 amountForRecipient = amountForRecipients * FeeRecipients[x].basisPoints / totalFeeBasisPoints;
                (bool sent, ) = FeeRecipients[x].recipient.call{value: amountForRecipient}("");
                require(sent, "FAILED_SENDING_FUNDS");
            }
        }
    }

    function splitPaymentWithToken(uint256 amount) internal {
        uint256 amountForMarketplace = 0;
        if (marketplaceFeePercent > 0) {
            // Pay fee to marketplaceAddress
            amountForMarketplace = amount * marketplaceFeePercent / 100 / 100;
            IERC20(paymentToken).transferFrom(msg.sender, marketplaceAddress, amountForMarketplace);
        }
        ...
        if (feeRecipientCount > 0) {
            ...
            for(uint256 x = 1; x <= feeRecipientCount; x++) {
                uint256 amountForRecipient = amountForRecipients * FeeRecipients[x].basisPoints / totalFeeBasisPoints;
                IERC20(paymentToken).transferFrom(msg.sender, FeeRecipients[x].recipient, amountForRecipient);
            }
        } else {
            // Else the payment is sent to the smart contract (can be withdrawn via `withdrawTokens` function)
            IERC20(paymentToken).transferFrom(msg.sender, address(this), amountForRecipients);
        }
    }
    ...
    function withdrawTokens(address _token) external nonReentrant {
        ...
        IERC20(_token).safeTransfer(treasuryAddress, bal);
        emit WithdrawTokens(_msgSender(), bal);
    }
    ...
    function updateConfig(uint16 _maxMint, uint16 _maxPerMint, uint256 _price, 
        ...
        address _paymentToken, uint256 _priceWithToken,
        address _royaltyAddress, uint256 _royaltyFee) external onlyOwner
    {
        maxMint = _maxMint;
    ...
    function tokenURI(uint256 tokenId) external view virtual override returns (string memory) {
        ...
        if (useBaseUriOnly) return baseURI;
    ...
    event WithdrawTokens(address indexed sender, uint256 indexed balance);
...
abstract contract ERC721Min is Context, ERC165, IERC721, IERC721Metadata, IERC2981, ERC165Storage, Ownable {
    ...
    uint256 public royaltyFeePercent; // Fee % multiplied by 100, e.g. 675 = 6.75%
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual 
        override(ERC165, IERC165, ERC165Storage) returns (bool) 
    {
        return
    ...
    function royaltyInfo(uint256, uint256 salePrice) external view override(IERC2981) returns (address receiver, uint256 royaltyAmount) {
        ...
        royaltyAmount = salePrice * royaltyFeePercent / 100 / 100;
    }
    ...
    function setRoyaltyFeePercent(uint256 feePercent) external onlyOwner {
        royaltyFeePercent = feePercent;
    }
}
