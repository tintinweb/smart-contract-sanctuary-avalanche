...
interface IGamePool is IUserSimplified {
  ...
  */
  ...
  function depositSeed(uint256 amount, uint64 depositId) external;

  function depositBud(uint256 amount, uint64 depositId) external;
...
contract GamePool is IGamePool, SignableStakes, Constants, UUPSUpgradableTemplate, IAttributablePlayer {
  ...
  function fixTokenAddresses(address turf, address farm, address seed, address bud) external onlyOwner {
  ...
  function depositSeed(uint256 amount, uint64 depositId) external override {
    _depositFT(SEED, amount, depositId, _msgSender());
  }

  function depositBud(uint256 amount, uint64 depositId) external override {
  ...
  function _depositFT(
    ...
    if (tokenType == SEED) {
      ...
      // NOTE: any token type other than Seed will increment deposit for buds
  ...
  function harvest(
    ...
    require(isSignedByValidator(0, hashHarvesting(amount, deadline, randomNonce), signature0), "GamePool: invalid signature0");
    require(isSignedByValidator(1, hashHarvesting(amount, deadline, randomNonce), signature1), "GamePool: invalid signature1");
    _saveSignatureAsUsed(signature0);
    _saveSignatureAsUsed(signature1);
