...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuard {
    ...
    **/
    ...
    /**
        ...
        where it is claimable by the trove owner who was liquidated
     */
     ...
     */
    function updateTroves(address[] calldata _borrowers, address[] calldata _lowerHints, address[] calldata _upperHints) external override {
        ...
        for (uint256 i; i < lowerHintsLen; ++i) {
            (uint debt, address[] memory tokens, uint[] memory amounts, , , ) = getEntireDebtAndColls(_borrowers[i]);
    ...
    function redeemCollateral(
        ...
        uint _partialRedemptionHintRICR,
        uint _maxIterations
    )
    external
    override
    ...
    /**
        ...
        @return ICR
     */
    function getCurrentICR(address _borrower) public view override returns (uint ICR) {
    ...
    function redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, address[] memory _tokens, uint[] memory _amounts) external override {
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            uint collateralVC = controller.getValueVC(token, amount);
    ...
    function updateTroveCollTMR(address  _borrower, address[] memory _tokens, uint[] memory _amounts) external override {
        _requireCallerIsTMR();
