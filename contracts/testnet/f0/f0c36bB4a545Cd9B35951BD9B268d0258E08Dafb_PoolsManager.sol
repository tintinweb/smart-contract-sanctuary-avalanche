...
contract PoolsManager is Ownable {
    ...
    uint256 public constant rewardPeriodForPools = 60 * 60 * 24;
    ...
    function addPool(
        ...
        rewardToken.safeTransfer(
            address(newPool),
            (stakingPoolDuration * rewardRate) / rewardPeriodForPools
        );
...
contract TimeLockPool is ITimeLockPool {
    ...
    uint256 public constant rewardPeriod = 60 * 60 * 24;
    uint256 public constant minStakingDuration = 10 * 60;
    uint256 public immutable maxStakingDuration; // ~ 1 Year 60*60*24*365
    ...
    constructor(
        ...
        require(
            _maxStakingDuration > minStakingDuration,
            'Need to max time more than min lock duration'
        );
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            ...
            (((((lastTimeRewardApplicable() - lastUpdateTime) * rewardRate) /
                rewardPeriod) * 1e18) / totalFinalSupply);
    }
        ...
        updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            rewardRate = _reward / (stakingPoolDuration / rewardPeriod);
        } else {
            uint256 remainingTime = periodFinish - block.timestamp;
            uint256 leftover = (remainingTime * rewardRate) / rewardPeriod;
            rewardRate = (_reward + leftover) / (stakingPoolDuration / rewardPeriod);
        }
    ...
    function recoverERC20(
        ...
        if (address(depositToken) == address(rewardToken)) {
            require(
                ...
                'Too much tokens required'
            );
        } else {
            require(tokenAddress != address(depositToken), 'Cant recover deposit token');
        }
    ...
    function getMultiplier(uint256 _duration) internal view returns (uint256) {
