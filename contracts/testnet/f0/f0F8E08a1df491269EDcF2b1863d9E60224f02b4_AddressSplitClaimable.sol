    ...
    Initializable
{
    ...
    function claimRewards(address _address) public nonReentrant {
        uint256 totalRewards = balancesOfAddresses[_address];
        if (totalRewards > 0) {
            balancesOfAddresses[_address] = 0;
            payable(msg.sender).transfer(totalRewards);
            emit RewardsPayed(msg.sender, totalRewards);
        }
    }
    ...
    function setAddresses(address[] calldata _addresses)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // set addresses array from input
    ...
    function splitRewards() public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 totalSupply = address(this).balance - payableRewards;
        uint256 amountPerAddress = totalSupply / addresses.length;
        for (uint256 i = 0; i < addresses.length; i++) {
            payableRewards += amountPerAddress;
            balancesOfAddresses[addresses[i]] += amountPerAddress;
        }
        emit RewardsSplit(amountPerAddress);
    }
