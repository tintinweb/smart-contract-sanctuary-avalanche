...
contract PrecogV5 is IPrecogV5 {
    ...
    function _processHandleIncreaseInvestment(address token, uint amount) internal returns (uint _depositAmount){
        ...
        if(precogStorage.getInvestmentsOf(token, msg.sender).length == 0) {
            ...
            if(futureTradingCycle.id >= lastInvestmentOf.idChanged) {
                ...
                if(amount < register.amount) {
                ...
                } else if(amount == register.amount) {
                    ...
                    _depositAmount = amount - register.amount;
                    precogInternal.increaseInvestment(
                        token, 
                        msg.sender, 
                        _depositAmount,
                    ...
                    for(uint i = futureTradingCycle.id + 1; i <= lastInvestmentOf.idChanged; i++) {
                        ...
                        uint newUnit = precogStorage.getTotalUnitsTradingCycle(token, i) + _depositAmount * (tradingCycle.endTime - tradingCycle.startTime);
    ...
    function deposit(address token, uint amount) external override isExistingToken(token) {
        ...
        uint depositAmount = _processHandleIncreaseInvestment(token, actualDepositAmount);
