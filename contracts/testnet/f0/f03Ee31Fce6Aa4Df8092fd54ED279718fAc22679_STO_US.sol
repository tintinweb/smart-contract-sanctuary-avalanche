...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    modifier onlyAdmin() virtual {
        require(whitelist.isAdmin(msg.sender), "STO0");
        _;
    }
    /**
    ...
    //    console.log("_feeStatus------>",_feeStatus);
    //    console.log("amount stable coin before------->",stableCoins);
    //    if( _feeStatus){
    //     uint256 expectedToken= stableCoins/rate();
    //     console.log("expectedToken--->",expectedToken);

    //     uint256 fee = (expectedToken*rate()/100)*_feePercent;
    //     console.log("fee calcaulted ------->",fee);
    //     require(expectedToken*rate() + fee == stableCoins,"Investor do not invest accurate fee amount");
    //     _stableCoin.transfer(_feeAddress, fee);
    //     stableCoins = stableCoins - fee;
    //    }
        // console.log("amount stable coin before------->",stableCoins);
        uint256 targetTokens = stableCoins / _rate;
        //console.log("stableCoins in fallback",stableCoins);
        ...
        //uint256 mirrorRate = ISTO(STONonUSAddress).rate();
        // console.log("mirrorSTO rate",mirrorRate);
        // console.log("_tokenAmount",tokens);
        return true;
    }
    ...
    function _processPurchase(
       ...
       // console.log("in _processPurchase");
...
abstract contract PostGoalCrowdsale is TimedCrowdsale, RefundableCrowdsale {
    ...
    function withdrawAllTokens() public {
        require(goalReached(), "STO21");
       // console.log("here in withdrawAllTokens");
    ...
    function _processPurchase(
        ...
        if (_balances[beneficiary] == 0) {
           ...
           // console.log("here in postgoal in US");
...
abstract contract TimedCrowdsale is Crowdsale {
    ...
    modifier onlyAdmin() override(Crowdsale) virtual{
        require(whitelist.isAdmin(_msgSender()), "STO0");
        _;
    }
    ...
    PostGoalCrowdsale
{
    ...
    {
        require(
            __whitelist.isManager(_msgSender()) ||
                __whitelist.isAdmin(msg.sender),
            "STO1"
        );
        //require(__rate == 1000 ether, "STO2");
       // console.log("__propertyValue % (__rate / 1 ether)",__propertyValue % (__rate / 1 ether));
        require(
    ...
    */
        ...
        override(CappedCrowdsale, TimedCrowdsale, Crowdsale)
    {
        ...
        if (
            (getUserContribution(_beneficiary) + (_weiAmount)) / (1 ether) >
            basicCap()
        ) {
            require(
    ...
    function _processPurchase(
       ...
       if (stage == CrowdsaleStage.ICO)
        {
            //console.log("here in _processPurchase in ICO");
            _deliverTokens(_beneficiary, _tokenAmount, _data);
            uint256 totalTokens = _tokenAmount * (rate()/stoNonUS.rate());
          //  console.log("totalTokens in 1 Non US",totalTokens);
             IERC1400RawERC20(address(stoNonUS.token())).issue(reserveWalletNonUS,totalTokens,"0x00");
        }
        else {
            // console.log("here in _processPurchase in PreICO");
            // console.log("_tokenAmount in _processPurchase in US",_tokenAmount);
             super._processPurchase(_beneficiary, _tokenAmount, _data);
             uint256 totalTokens = _tokenAmount * (rate()/stoNonUS.rate());
             //console.log("totalTokens in 2",totalTokens);
    ...
    /**
     * @dev Get the address of Property token
     */
    ...
    function updateGoal(uint256 newGoal) external returns (uint256) {
        ...
        require(!Address.isContract(_msgSender()),"STO13");
        require((whitelist.isSuperAdmin(_msgSender()) || whitelist.isSubSuperAdmin(_msgSender())),
                 "STO5" );
        require(!goalReached(), "STO14");
        require(!hasClosed(),"STO6");
        require(newGoal > 0, "STO15");
        //require(_securityToken.totalSupply() < newGoal,"STO16");
        require((newGoal != goalPercent), "STO17");
        require(newGoal <= cap(), "STO18");
        emit UpdateGoal(msg.sender,newGoal,goalPercent);
        goalPercent = newGoal;
        uint256 newNonUSGoal = newGoal * rate()/ISTO(STONonUSAddress).rate();
        //console.log("update Goal in US",newGoal);
