...
// File contracts/interfaces/NativeMinterInterface.sol
...
// File contracts/NovusOperations.sol
...
contract NovusOperations is NovusContract, ReentrancyGuard {
    address private constant minterAddr =
        0x0200000000000000000000000000000000000001;
    address private novusDistributionAddr;
    NovusDistribution private novusDistributionContract;
    NativeMinterInterface private minter = NativeMinterInterface(minterAddr);

    constructor(string memory _version, uint _creationDate) 
    NovusContract(_version, _creationDate) {
    }

    function setNovoDistributionContract(address _addr) public onlyAdmin {
      novusDistributionAddr = _addr;
      novusDistributionContract = NovusDistribution(_addr);
    }

    function purchaseNOVO(address to, uint mint_amount, uint fee) public onlyAdmin {
        require(mint_amount > 0, "Invalid Deposited amount");
        require(fee > 0, "Invalid Fees amount");
        string memory dTypeName = "NOVO Minting";
        DistributionType memory _dType = novusDistributionContract.getDistributionTypeByName(dTypeName);

        minter.mintNativeCoin(to, mint_amount);
        minter.mintNativeCoin(novusDistributionAddr, fee);
        novusDistributionContract.addToDistributionBalance(_dType.id, fee);
    }
}