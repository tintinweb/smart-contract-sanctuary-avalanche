...
contract Trainer is ITrainer, Ownable {
    ...
    function luchadoresInTrainer(address trainerOwner)
        public
        view
        returns (uint256[MAX_SPOTS] memory)
    {
        return trainersLuchadores[trainerOwner];
    }

    function trainerWhitelist(address trainerOwner)
        public
        view
        returns (address[MAX_SPOTS] memory)
    {
        return trainersUserWhitelist[trainerOwner];
    }

    function isLuchadorInTrainer(uint256 luchadorId, address trainerOwner)
        public
        view
        returns (bool)
    {
    ...
    function checkAllowed(address trainerOwner, address luchadorOwner)
        public
        view
        returns (bool)
    {
    ...
    function placeLuchador(address trainerOwner, uint256 luchadorId) public {
        require(
            msg.sender == luchador,
            "Only the luchador contract can place luchadores"
        );
        require(
            trainersLuchadorCount[trainerOwner] <
                trainersLuchadorCap[trainerOwner],
            "Not enough luchador cap"
        );
    ...
    function removeLuchador(address trainerOwner, uint256 luchadorId) public {
        require(
            msg.sender == luchador,
            "Only the luchador contract can remove luchadores"
        );
