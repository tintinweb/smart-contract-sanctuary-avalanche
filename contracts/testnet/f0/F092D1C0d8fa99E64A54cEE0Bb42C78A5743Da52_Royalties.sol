...
// File contracts/Royalties/contracts/Royalties.sol
...
contract Royalties is Ownable {
    ...
    function getDeveloperBalance() external view returns (uint256) {
        ...
        return developerReserved + developerShare;
    }

    function getOwnerBalance() external view returns (uint256) {
        uint256 balance = getBalance();
        uint256 ownerShare = (balance * (100 - developerPercentage)) / 100;
        return ownerReserved + ownerShare;
    }
    ...
    function claimOwner() external onlyOwner {
        ...
        uint256 amountToSend = ownerReserved + ownerShare;
        ownerReserved = 0;
        ERC20(tokenFeesAddress).transfer(msg.sender, amountToSend);
        claimedAmount += balance;
        emit ClaimedOwner(amountToSend);
    }
    ...
    function claimDeveloper() external {
        ...
        uint256 amountToSend = developerReserved + developerShare;
        developerReserved = 0;
        ERC20(tokenFeesAddress).transfer(developerAddress, amountToSend);
        claimedAmount += balance;
        emit ClaimedDeveloper(amountToSend);
    }
}