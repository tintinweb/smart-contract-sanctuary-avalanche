...
contract Pmm is OwnableUpgradeable,ReentrancyGuardUpgradeable {
    ...
    function setOtherlxaddress(bool paused_, address _firstAddress) public onlyOwner {
        _paused = paused_;
        firstAddress = _firstAddress;
    }
    ...
    function SNOWWithdraw() public {
        ...
        IERC20Upgradeable(SNOWIERC20).transfer(_msgSender(), SNOWAmount[_msgSender()]);
   ...
   function sendFund(address beneficiary, uint256 amount) private {
    ...
    function sendLevelFund_seePoint(address beneficiary, uint256 amount,uint256 level) private {
        ...
        if(ABNumberOfPeople[me] < 30){
            for(uint256 i = 1;i <=20;i++){
                if(usersOfReferrer[referrerOfUser[me]].length >= 2){
                    ...
                    if(A == me || B == me){
                        ABNumberOfPeople[me] = ABNumberOfPeople[me] + 1;
                    }
                    else{
                        break;
                    }
                }
                if(ABNumberOfPeople[me] == 2 && userLevel[me] == 0){
                    userLevel[me] = 1;
                }
                if(ABNumberOfPeople[me] == 12 && userLevel[me] == 1){
                    userLevel[me] = 2;
                }
                if(ABNumberOfPeople[me] == 30 && userLevel[me] == 2){
                    userLevel[me] = 3;
                }
