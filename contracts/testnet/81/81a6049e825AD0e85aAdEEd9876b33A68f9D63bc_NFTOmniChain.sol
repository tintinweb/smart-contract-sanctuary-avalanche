...
contract NFTOmniChain is ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    
    ILayerZeroEndpoint public layerZeroEndpoint;

    mapping (uint => address) public destinationForChainId;
    uint public lastMessageReceived;
    ...
    function setDestination(uint _chainId, address _contract) external {
        destinationForChainId[_chainId] = _contract;
    }

    function sendToOtherChain(uint16 _chainId, uint _message) public payable {
        ...
        layerZeroEndpoint.send{value: msg.value}(
            ...
            abi.encodePacked(destinationForChainId[_chainId]), // destination address
        ...
        emit MessageSent(_chainId, _message);
    }
    ...
    function lzReceive(
        ...
        uint _message = abi.decode(_payload, (uint));

        lastMessageReceived = _message;
        emit MessageReceived(_srcChainId, fromAddress, _message);
    }
