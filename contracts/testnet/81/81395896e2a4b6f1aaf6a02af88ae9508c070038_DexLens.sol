...
import {Constants} from "lydia-v2-dex/libraries/Constants.sol";
import {PriceHelper} from "lydia-v2-dex/libraries/PriceHelper.sol";
import {ILydiaFactory} from "lydia-v2-dex/interfaces/ILydiaFactory.sol";
import {ILydiaPair} from "lydia-v2-dex/interfaces/ILydiaPair.sol";
import {ILBFactory} from "lydia-v2-dex/interfaces/ILBFactory.sol";
import {ILBPair} from "lydia-v2-dex/interfaces/ILBPair.sol";
import {ILBRouter} from "lydia-v2-dex/interfaces/ILBRouter.sol";
import {Uint256x256Math} from "lydia-v2-dex/libraries/math/Uint256x256Math.sol";
...
contract DexLens is SafeAccessControlEnumerable, IDexLens {
    ...
    ILydiaFactory private immutable _FACTORY_V1;
    ...
    modifier verifyDataFeed(address token, DataFeed calldata dataFeed) {
        ...
        if (collateralAddress == token) revert DexLens__SameTokens();
     ...
     * @param lydiaFactory The address of the v1 factory
     * @param wnative The address of the wnative token
     */
    constructor(ILBFactory lbFactory, ILydiaFactory lydiaFactory, address wnative) {
        // revert if all addresses are zero or if wnative is zero
        if (address(lbFactory) == address(0) && address(lydiaFactory) == address(0) || wnative == address(0)) {
            revert DexLens__ZeroAddress();
        }

        _FACTORY_V1 = lydiaFactory;
    ...
    function initialize(address aggregator) external {
        if (_getDataFeedsLength(_WNATIVE) != 0) revert DexLens__AlreadyInitialized();
     ...
     */
    function getFactoryV1() external view override returns (ILydiaFactory factoryV1) {
        return _FACTORY_V1;
    }
    ...
    {
        if (token == _WNATIVE) revert DexLens__NativeToken();
    ...
    function _setNativeDataFeed(address aggregator) private {
        if (_getDataFeedAt(_WNATIVE, 0).dfAddress == aggregator) revert DexLens__SameDataFeed();
        ...
        if (_getPriceFromChainlink(aggregator) == 0) revert DexLens__InvalidChainLinkPrice();
    ...
    {
        ILydiaPair pair = ILydiaPair(pairAddress);
        ...
        returns (address tokenA, address tokenB)
    {
        if (dfType == DataFeedType.V2_1) {
            ...
            tokenA = ILydiaPair(pair).token0();
            tokenB = ILydiaPair(pair).token1();
        } else {
            revert DexLens__UnknownDataFeedType();
        }
    }
    ...
    function _getNativePriceAnyToken(address token) private view returns (uint256 price) {
        ...
        uint256 totalWeight = totalWeightV2_1 + totalWeightV1;
        return totalWeight == 0 ? 0 : (weightedPriceV2_1 + weightedPriceV1) / totalWeight;
    }
...
/// @title Lydia V1 Factory Interface
/// @notice Interface to interact with Lydia V1 Factory
...
/// @title Lydia V1 Pair Interface
/// @notice Interface to interact with Lydia V1 Pairs
...
import {ILydiaFactory} from "./ILydiaFactory.sol";
import {ILBFactory} from "./ILBFactory.sol";
...
interface ILBRouter {
    ...
    function getV1Factory() external view returns (ILydiaFactory);
...
import {ILydiaFactory} from "lydia-v2-dex/interfaces/ILydiaFactory.sol";
import {ILBFactory} from "lydia-v2-dex/interfaces/ILBFactory.sol";
import {ILBLegacyFactory} from "lydia-v2-dex/interfaces/ILBLegacyFactory.sol";
...
/// @author Lydia Finance
/// @notice The interface needed to interract with the Dex Lens contract
interface IDexLens is ISafeAccessControlEnumerable {
    ...
    function getFactoryV1() external view returns (ILydiaFactory factoryV1);
