...
{
    address public immutable beansManager;
    ...
    constructor(address _beansManager, address _treasury) ERC20("Garden", "SEED") {
        require(
            _beansManager != address(0),
            "Implementations: beansManager is not set"
        );
        beansManager = _beansManager;
        ...
        onlyBeansManager
    {
        ...
        // effectively protecting BeansManager from suspicious addresses
    ...
    function liquidityReward(uint256 amount) external onlyBeansManager {
        require(
    ...
    function _transferTaxOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (!excludedFromFees[sender]) {
            ...
            if (isSEEDLiquidityPool(recipient)) { // if the recipient address is a liquidity pool, apply sell fee
            ...
            } else if (!isSEEDLiquidityPool(sender)) { // apply transfer fee if this isn't a transfer from the pool (buy)
    ...
    // return true if the "_recipient" address is a SEED liquidity pool
    function isSEEDLiquidityPool(address _recipient) private returns (bool) {
