...
contract TestReserves_1 is Ownable{
  ...
  function getTokenPrice() public view returns(uint[3] memory)
   {
    ...
    if (now_val > val.add(val.mul(perc))) {
      uint boo = 1;
    }
    if (now_val < val.sub(val.mul(perc))) {
     uint boo = 2;
    }
    return [res0,res1,boo]; // return amount of token0 needed to buy token1
   }
