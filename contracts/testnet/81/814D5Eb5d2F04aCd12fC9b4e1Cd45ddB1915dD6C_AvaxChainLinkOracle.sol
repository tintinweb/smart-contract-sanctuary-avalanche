...
contract AvaxChainLinkOracle is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event SetAssetsAggregator(address indexed sender, address asset, address aggeregator);

    function setAssetsAggregator(address asset,address aggergator) public onlyOwner {
        _setAssetsAggregator(asset,aggergator);
    }

    function addBRTToken(address BRTToken) external onlyOwner {
        (IERC20Upgradeable _lpToken, , ) = IBRT(BRTToken).poolInfo();
        require(address(_lpToken) != address(0), "N/A");
        lpToken[BRTToken] = IPair(address(_lpToken));
    }
    ...
    function getBRTPrice(address BRTToken) public view returns (bool, uint) {
        IPair _lpToken = lpToken[BRTToken];
        if(address(_lpToken) != address(0)) {
            uint256 BRTTotalSupply = IBRT(BRTToken).totalSupply();
            ( , uint256 lpTokenInPool, ) = IBRT(BRTToken).poolInfo();
    ...
    function initialize() public initializer {
        ...
        _setAssetsAggregator(0x7C9DD6960df20975C1D642e2CB083280547e3bEA, 0x7898AcCC83587C3C55116c5230C17a6Cd9C71bad);//usdt
        _setAssetsAggregator(0x3800955b7A4233A2a4f2344a43362D9126E9FC81, 0x7898AcCC83587C3C55116c5230C17a6Cd9C71bad);//usdc
    }
}
