...
contract BavaCompoundPool_KyberFairV2 is BRTERC20, ReentrancyGuard {
    ...
    function initPool(IERC20 _lpToken, IKyberFairLaunchV2 _stakingContract, IKyberRewardLockerV2 _rewardLocker, uint256 _restakingFarmID, IERC20 _rewardToken, IERC20[] memory _bonusRewardTokens, IRouter _router, uint256 _MIN_TOKENS_TO_REINVEST, uint256 _DEV_FEE_BIPS, uint256 _REINVEST_REWARD_BIPS) external onlyOwner {        
        ...
        poolRestakingInfo.rewardLocker = _rewardLocker;
    ...
    function setAllowancesStaking(uint256 _amount) external onlyOwner {    
        if (address(poolRestakingInfo.stakingContract) != address(0)) {
    ...
    function setAllowancesRouter(uint256 _amount) external onlyOwner {   
        if (address(router) != address(0)) {
            IWAVAX(WAVAX).approve(address(router), _amount);
  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    ...
    function _stakeDepositTokens(uint amount) private {
        require(amount > 0, "#<0");
        poolRestakingInfo.stakingContract.deposit(poolRestakingInfo.restakingFarmID, amount, true);
    }
    ...
    function reinvest() external nonReentrant {
        (uint256 estimatedTotalReward, uint256 estimatedAVAXReward, uint256 rewardBalance, uint256 avaxBalance) = checkLockerReward();
        require((estimatedTotalReward + estimatedAVAXReward + rewardBalance + avaxBalance) >= MIN_TOKENS_TO_REINVEST, "#<MinInvest");
        _reinvest(estimatedTotalReward, estimatedAVAXReward);
    }
    ...
    function checkLockerReward() public view returns (uint, uint, uint, uint) {
        ...
        uint256 totalVesting = _checkLockerReward(address(rewardToken));
        uint256 totalAVAXVesting = _checkLockerReward(address(0));

        return (totalVesting, totalAVAXVesting, rewardBalance, avaxBalance);
    }
    ...
    function setPoolRestakingInfo(IKyberFairLaunchV2 _stakingContract, IKyberRewardLockerV2 _rewardLocker, uint256 _restakingFarmID, IERC20 _rewardToken, IERC20[] memory _bonusRewardTokens, IRouter _router, bool _withUpdate) external onlyOwner {
        require(address(_stakingContract) != address(0) , "0Addr");        
        if (_withUpdate) {
            updatePool();
        }
