...
import "contracts/interface/IPlatformRegistry.sol";
import "contracts/interface/ITransferProxy.sol";
import "contracts/interface/IRoyaltyRegistry.sol";
import "contracts/interface/IExchange.sol";

// TODO: Delete after test code done
  ...
  UUPSUpgradeable
{
  ...
  function __Exchange_init(
    ...
    __EIP712_init("SBINFT Exchange", "2.0");
    __Pausable_init();
  ...
  function _simulateRoyaltyTransfer(
    ...
    (royaltyReceivers, royaltyReceiversCut) = royaltyRegistry.royaltyInfo(
      _asset.token,
      _asset.tokenId,
      _pricePerAsset,
  ...
  function exchange(
    ...
    if (OrderDomain._isOnchainPaymentMode(paymentMode)) {
      ...
      // TODO: make sure to make funtion withdraw extra ETH/ERC20
      if (paymentMode == OrderDomain.NATIVE_PAYMENT_MODE) {
        ...
        uint256 allowanceCount = IERC20(paymentToken)
          .allowance(_buyOrder.payer, address(transferProxy));
    ...
    emit Sale(_buyOrder.saleNounce);
  }
  ...
  function cancel(
    ...
    // TODO: If needed add more data in the event
    emit OrderCancelled(_saleOrder.nounce);
  }
...
import "contracts/library/AuctionDomain.sol";
...
import "contracts/library/OrderDomain.sol";
...
library AuctionDomain {
   ...
   */
  function _isValidBidMode(bytes4 _bidMode)
    internal
    pure
    returns (bool)
  {
  ...
  bytes32 constant BID_STATUS_TYPEHASH =
    keccak256(
      "BidStatus(address currentTopBidder,address payable refundTo,uint256 currentPrice,address[] bidderList,uint256[] bidPriceHistory)"
    );

  bytes32 constant AUCTION_TYPEHASH =
    keccak256(
      "Auction(uint256 auctionId,bytes4 pfKind,address payable creatorAddress,Asset asset,AuctionType auctionType,BidStatus bidStatus,uint256 startPrice,uint256 startTime,uint256 endTime,uint16 pfFeeRate,uint16 externalPfFeeRate,address platformSigner)Asset(bytes4 originKind,address asset,uint256 assetId,uint16 partnerFeeRate,uint8 isSecondarySale)AuctionType(bytes4 bidMode,bytes4 auctionKind,address paymentToken)BidStatus(address currentTopBidder,uint256 currentPrice,address[] bidderList,uint256[] bidPriceHistory)"
    );

  bytes32 constant BID_TYPEHASH =
    keccak256(
      "Bid(uint256 auctionId,uint256 assetId,address bidder,address payable refundTo,uint256 nonce,uint256 price)"
    );
  ...
  function _hashBidStatus(BidStatus calldata _bidstatus)
    internal
    pure
    returns (bytes32)
  {
    return
      keccak256(
        abi.encode(
          BID_STATUS_TYPEHASH,
          _bidstatus.currentTopBidder,
          _bidstatus.currentPrice,
          _bidstatus.bidderList,
          _bidstatus.bidPriceHistory
        )
      );
  }
