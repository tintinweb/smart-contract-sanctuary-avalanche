...
contract Ulys is ERC20Burnable, Operator {
    ...
    event TaxHappened(address recipient, uint256 taxAmount);

    event TransferFromHappened(address recipient);

    event TransferHappened(address recipient);
    ...
    function transferFrom(
        ...
        emit TransferFromHappened(recipient);
        uint256 currentTaxRate = 0;
        bool burnTax = false;

        if (autoCalculateTax) {
            uint256 currentUlysPrice = _getUlysPrice();
            currentTaxRate = _updateTaxRate(currentUlysPrice);
            if (currentUlysPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function _transferWithTax(
        ...
        emit TaxHappened(recipient, taxAmount);
        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(taxAmount, 1, path, taxCollectorAddress, block.timestamp);

        _transfer(sender, taxCollectorAddress, amountAfterTax);
