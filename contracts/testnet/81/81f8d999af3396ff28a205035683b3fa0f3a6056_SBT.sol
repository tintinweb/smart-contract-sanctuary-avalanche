...
error SBT__NotAuthorizedToRevoke();
error SBT__InvalidUserType();
error SBT__CannotTransferSBT();

contract SBT is Owned, ERC721 {
    ...
    modifier validateType(uint8 _type) {
        if (_type < 1 || _type > 3) {
            revert SBT__InvalidUserType();
        }
        _;
    }
    ...
    modifier onlyRevokeAuthorized(uint256 id) {
        if (owner != msg.sender || _ownerOf[id] != msg.sender) {
            revert SBT__NotAuthorizedToRevoke();
        }
        _;
    }
    ...
    /// 2 = Platform
    /// 3 = Product
    mapping(address => uint8) internal _userType;
    ...
    /// @notice method to get the user type of a wallet
    /// @param _wallet the wallet address
    function getWalletType(address _wallet) external view returns (uint8) {
        return _userType[_wallet];
    }
