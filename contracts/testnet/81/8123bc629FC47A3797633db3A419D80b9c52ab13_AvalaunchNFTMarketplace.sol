...
contract AvalaunchNFTMarketplace is IAvalaunchNFTMarketplace, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct Ask {
        ...
        uint256 expirationTimestamp; // Expiration timestamp of an ask
     ...
     * @param _itemId - id of item/nft user wants to buy
     * @param _expirationTimestamp - expiration timestamp of an ask
     */
    function createAsk(uint256 _itemId, uint256 _expirationTimestamp) external payable nonReentrant {
        // Check msg value
        if (msg.value == 0) {
            revert AvalaunchNFTMarketplace__InvalidMessageValue();
        }
        // Check expiration time validity
        if (_expirationTimestamp < 1 hours || _expirationTimestamp > 7 days) {
            revert AvalaunchNFTMarketplace__InvalidAskExpirationTime();
        }
        // Retrieve item
        Item storage item = items[_itemId];
        ...
        if (ask.active == true || block.timestamp <= ask.expirationTimestamp) {
        ...
        ask.value = msg.value;
        // Set expiration time
        ask.expirationTimestamp = block.timestamp + _expirationTimestamp;
        // Emit an event
     ...
     */
    function acceptAsk(uint256 _itemId, address _asker) external nonReentrant {
        Ask storage ask = asks[_itemId][_asker];
        // Check ask activity
        if (!ask.active || block.timestamp > ask.expirationTimestamp) {
            revert AvalaunchNFTMarketplace__AskInactive();
        }
        Item storage item = items[_itemId];
        // Check if item is available to buy
