 ...
 */
contract Vault is Clone, Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, IVault {
    ...
    modifier onlyFactory() {
        if (msg.sender != address(_factory)) revert Vault__OnlyFactory();
        _;
    }
    ...
    function getRange() external view override returns (uint24 low, uint24 upper) {
        ...
        if (address(strategy) != address(0)) {
            (low, upper) = strategy.getRange();
        }
    }

    function getOperators() external view override returns (address defaultOperator, address operator) {
        IStrategy strategy = _strategy;
    ...
    function getPendingFees() external view override returns (uint256 feesX, uint256 feesY) {
        IStrategy strategy = _strategy;

        if (address(strategy) != address(0)) {
            (feesX, feesY) = strategy.getPendingFees();
        }
    }
    ...
    {
        return _previewShares(amountX, amountY);
    }

    function previewWithdraw(address user) external view override returns (uint256 amountX, uint256 amountY) {
        (uint256 balanceX, uint256 balanceY) = _getBalances(_strategy);

        uint256 shares = balanceOf(user);
        uint256 totalShares = totalSupply();

        amountX = (balanceX * shares) / totalShares;
        amountY = (balanceY * shares) / totalShares;
    }
    ...
    function deposit(uint256 amountX, uint256 amountY)
        ...
        returns (uint256 shares, uint256 effectiveX, uint256 effectiveY)
    {
        if (amountX == 0 && amountY == 0) revert Vault__ZeroAmount();

        IStrategy strategy = _strategy;
        if (address(strategy) == address(0)) revert Vault__InvalidStrategy();

        (shares, effectiveX, effectiveY) = _previewShares(amountX, amountY);
    ...
    function withdraw(uint256 shares) external override returns (uint256 amountX, uint256 amountY) {
        ...
        if (address(strategy) == address(0)) {
            (uint256 balanceX, uint256 balanceY) = _getBalances(strategy);
            (amountX, amountY) = (shares * balanceX / totalShares, shares * balanceY / totalShares);

            if (amountX > 0) _tokenX().safeTransfer(msg.sender, amountX);
            if (amountY > 0) _tokenY().safeTransfer(msg.sender, amountY);
        } else {
            (amountX, amountY) = _strategy.withdraw(shares, totalShares, msg.sender);
        }

        emit Withdrawn(msg.sender, amountX, amountY, shares);
    }

    function setStrategy(IStrategy strategy) external override onlyFactory {
        if (
            strategy.getVault() != address(this) || strategy.getPair() != _pair() || strategy.getTokenX() != _tokenX()
                || strategy.getTokenY() != _tokenY()
        ) {
            revert Vault__InvalidStrategy();
        }

        IStrategy currentStrategy = _strategy;

        if (currentStrategy == strategy) revert Vault__SameStrategy();

        if (address(currentStrategy) == address(0)) {
            (uint256 balanceX, uint256 balanceY) = _getBalances(currentStrategy);

            if (balanceX > 0) _tokenX().safeTransfer(address(strategy), balanceX);
            if (balanceY > 0) _tokenY().safeTransfer(address(strategy), balanceY);
        } else {
            IStrategy(currentStrategy).withdraw(1, 1, address(strategy));
        }
    ...
    function pauseVault() external override onlyFactory {
        _strategy.withdraw(1, 1, address(this));
        _setStrategy(IStrategy(address(0)));
    }

    function recoverERC20(address token, address recipient, uint256 amount) external override onlyFactory {
        if (address(_strategy) == address(0) && (token == address(_tokenX()) || token == address(_tokenY()))) {
            revert Vault__InvalidToken();
        }

        IERC20Upgradeable(token).safeTransfer(recipient, amount);
    }

    function _pair() internal pure returns (ILBPair) {
        return ILBPair(_getArgAddress(0));
    }

    function _tokenX() internal pure returns (IERC20Upgradeable) {
        return IERC20Upgradeable(_getArgAddress(20));
    }

    function _tokenY() internal pure returns (IERC20Upgradeable) {
     ...
     */
    function _decimalsX() internal pure returns (uint256 decimalsX) {
        decimalsX = _getArgUint8(60);
    }
     ...
     */
    function _decimalsY() internal pure returns (uint256 decimalsY) {
        decimalsY = _getArgUint8(61);
    }
    ...
    function _dataFeedX() internal pure returns (IAggregatorV3 dataFeedX) {
        dataFeedX = IAggregatorV3(_getArgAddress(62));
    }
    ...
    function _dataFeedY() internal pure returns (IAggregatorV3 dataFeedY) {
        dataFeedY = IAggregatorV3(_getArgAddress(82));
    }

    /**
     * @dev Returns the price of tokenX in tokenY.
     * @return price The price of tokenX in tokenY.
     */
    function _getPrice() internal view returns (uint256 price) {
        (, int256 priceX,,,) = _dataFeedX().latestRoundData();
        (, int256 priceY,,,) = _dataFeedY().latestRoundData();

        if (priceX <= 0 || priceY <= 0) revert Vault__InvalidPrice();

        price = (uint256(priceX) * 10 ** (18 + _decimalsY() - _decimalsX())) / uint256(priceY);
    }

    function _previewShares(uint256 amountX, uint256 amountY)
        internal
        view
        returns (uint256 shares, uint256 amountX_, uint256 amountY_)
    {
        ...
        if (totalShares == 0) return (price * amountX + amountY * PRECISION, amountX, amountY);

        (uint256 totalX, uint256 totalY) = _getBalances(_strategy);

        uint256 totalValueInY = price * totalX + totalY * PRECISION;

        uint256 vaultRatio = totalY * PRECISION / totalValueInY;
        if (vaultRatio > 20e16 && vaultRatio < 80e16) {
            uint256 virtualAmountY = price * amountX / PRECISION;

            if (virtualAmountY > amountY) {
                amountX_ = (amountY_ * PRECISION - 1) / price + 1;
                if (amountX_ > amountX) amountX_ = amountX;
            } else {
                amountY_ = (amountX_ * price - 1) / PRECISION + 1;
                if (amountY_ > amountY) amountY_ = amountY;
            }
        } else {
            amountX_ = amountX;
            amountY_ = amountY;
        ...
        uint256 valueInY = price * amountX_ + amountY_ * PRECISION;
        shares = valueInY.mulDivRoundDown(totalShares, totalValueInY);
    }

    function _getBalances(IStrategy strategy) internal view returns (uint256 amountX, uint256 amountY) {
        if (address(strategy) == address(0)) {
    ...
    function _setStrategy(IStrategy strategy) internal {
        _strategy = strategy;
