...
interface INodeManager {
    ...
    function getAllNodesRewards(address account,uint256[3] memory tier) external view returns (uint256[2] memory);
...
contract beta_main is ERC20, Ownable, PaymentSplitter {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256 treasuryPoolTokens = contractTokenBalance.sub(rewardsPoolTokens).sub(teamTokens);
   ...
   function get(address account) public view returns(uint256[3] memory){
	...
	if (tier[2] != 0) {
        ...
        require(!isBlacklisted[sender],"BLACKLISTED");
        require(sender != teamPool && sender != rewardsPool,"the sender is a team wallet");
        uint256[2] memory rew = nodeManager.getAllNodesRewards(sender,tier);
        uint256 _boost = rew[1];
        uint256 _rewardAmount = rew[0];
        require(_rewardAmount > 0, "your reward amount is not above zero");
        uint get = getEm();
        uint _perc = 100;
        if (tier[1] != 0){
        	_perc = 50;
        }
        uint256 feeAmount = _rewardAmount.mul((cashoutFee.mul(_perc).div(100)));
        uint256 rewardAmount = _rewardAmount.sub(feeAmount);
