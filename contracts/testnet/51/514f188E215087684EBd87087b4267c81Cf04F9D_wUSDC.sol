...
contract wUSDC is ERC20 {
    ...
    function unwrap( uint _amount ) external returns ( uint ) {
        ...
        IERC20( USDC ).transfer( msg.sender, value );
        return value;
    }

    /**
        @notice converts wUSDC amount to USDC
        @param _amount uint
        @return uint
     */
    function wMEMOToMEMO( uint _amount ) public view returns ( uint ) {
        return _amount.mul( IMEMO( USDC ).index() ).div( 10 ** decimals() );
    }
    ...
    function MEMOTowMEMO( uint _amount ) public view returns ( uint ) {
        return _amount.mul( 10 ** decimals() ).div( IMEMO( USDC ).index() );
    }
