...
import {GenericRateLogic} from './GenericRateLogic.sol';
  ...
  AccessControlEnumerable
{
  ...
  string private constant DEPOSIT_SIG =
    'deposit((bytes32,address,address,uint256,uint256,uint256,uint256,uint64,uint64,uint64),bytes,uint256)';

  string private constant WITHDRAW_SIG =
    'withdraw((bytes32,address,address,uint256,uint256,uint256,uint256,uint64,uint64,uint64),address,bytes,uint256,address)';
    ...
    nonReentrant
  {
    ...
    for (uint8 i = 0; i < _creditLines.length; i++) {
      _claimCommission(_creditLines[i], recipient);
    }
  }
    ...
    nonReentrant
  {
    ...
    _claimCommission(_creditLine, recipient);
  }
    ...
    nonReentrant
  {
    ...
    for (uint8 i = 0; i < _creditLines.length; i++) {
      _claimProtocolFees(_creditLines[i], recipient);
    }
  }
    ...
    nonReentrant
  {
    ...
    _claimProtocolFees(_creditLines, recipient);
  }
  ...
  function batchBuyback(
    ...
    for (uint8 i = 0; i < _creditLines.length; i++) {
      ...
      aggregatedCollateral += tokensForBuyBack;
    }
  ...
  function claimBuyBack(
    ...
    emit Buyback(tokensForBuyBack, abi.decode(result, (uint256)), msg.sender);
  }
    ...
    returns (ManagerDataTypes.MigrateReturnValues memory)
  {
    ...
    creditLineStorageManager.refreshInterestRate(msg.sender, currentBalance);
    ...
    returns (uint256 collateralInterest, uint256 buyBackInterest)
  {
    ...
    (collateralInterest, buyBackInterest) = creditLineStorageManager
      .getPendingInterest(currentBalance, _creditLine, _eoaAddress);
  }
    ...
    returns (uint256 amountClaimed)
  {
    ...
    creditLineStorageManager.refreshInterestRate(_creditLine, currentBalance);

    creditLineInfo = creditLineStorageManager.getCreditLineStorage(_creditLine);
    // trigger transfer of funds from _creditLine
    (uint256 interestTokenAmount, ) =
    ...
    creditLineStorageManager.updateCommissionInterest(
      _creditLine,
    ...
    internal
  {
    ...
    creditLineStorageManager.updateProtocolFees(
      _creditLine,
  ...
  function _buyBack(
    ...
    address _creditLine,
    bytes calldata _swapParams
  ) internal returns (uint256 tokensForBuyBack) {
    (
      ManagerDataTypes.CreditLineInfo memory creditLineInfo,
      ManagerDataTypes.LendingInfo memory lendingInfo,
      ILendingCreditLineStorageManager creditLineStorageManager
    ...
    ManagerDataTypes.UserInterestData memory userInterestData =
      _getUserInterestData(_creditLine, msg.sender);
    (uint256 interestTokenAmount, ) =
      collateralToInterestToken(
        _creditLine,
        userInterestData.pendingJrtBuybackInterest
      );

    // trigger transfer of interest token from the creditLine
    interestTokenAmount = ICreditLineLendingTransfer(_creditLine)
      .transferToLendingManager(interestTokenAmount);
    ...
    //update user storage
    creditLineStorageManager.updateBuybackInterest(
      _creditLine,
      msg.sender,
      userInterestData.pendingJrtBuybackInterest
    );

    // // return collateral to use for buyback
    tokensForBuyBack = res.tokensTransferred;
  }
...
library GenericRateLogic {
  ...
  function _pendingInterests(
    ...
    address creditLine,
    address eoaAddress
  ) internal view returns (uint256 deposit, uint256 buyBack) {
    ...
    uint256 newInterestGenerated =
      _calculateNewUserTotalInterestGenerated(
        ...
        newRewards,
        userDeposit,
        newCommissionInterests
      );
    buyBack = newInterestGenerated.wadMul(creditLineInfo.jrtInterestShare);
    deposit = newInterestGenerated - buyBack;
  }

  /**
   * @notice Refreshes the global state and subsequently decreases the user's depositd value.
  ...
  function _withdraw(
    ...
    _refresh(creditLineInfo, totalDeposit, currentBalance);

    (uint256 tokenInterest, uint256 buybackInterest) =
      calculateNewUserInterests(userInterestData, creditLineInfo, userDeposit);
    ...
    userInterestData.pendingJrtBuybackInterest += buybackInterest;
  ...
  /**
   * @notice Refreshes the global state and subsequently increases the user's depositd value.
   * @dev This is an internal call and meant to be called within derivative contracts.
  ...
  function _deposit(
    ...
    newUserDeposit = userDeposit + value + interest;
    newTotalDeposit = totalDeposit + value + interest;
    userInterestData.accDiscountRate += value.wadToRay().rayDiv(
      creditLineInfo.accRate
    );
  }
  ...
  /**
   * @notice Function that calculate the new user's interest split between jrtBuyback and new token depositd
   * @param userInterestData the state of the user
   * @param creditLineInfo the state of the creditLine contract
   * @param userDeposit the total amount depositd for the user
   * @return tokenInterest the amount of new rewards allocated for user's token
   * @return buybackInterest the amount of new rewards allocated for user's jrt buy back
   */
  function calculateNewUserInterests(
    ManagerDataTypes.UserInterestData memory userInterestData,
    ManagerDataTypes.CreditLineInfo memory creditLineInfo,
    uint256 userDeposit
  ) internal view returns (uint256 tokenInterest, uint256 buybackInterest) {
    ...
    buybackInterest = totalInterestGenerated.wadMul(
      creditLineInfo.jrtInterestShare
    );
    tokenInterest = totalInterestGenerated - buybackInterest;
  }
  ...
  function _calculateNewUserTotalInterestGenerated(
    ...
    if (newRewards == 0) {
      ...
      uint256 accRate =
        creditLineInfo.accRate.rayMul(
          WadRayMath.RAY +
            (newRewards - newCommissionInterests).rayDiv(
  ...
  function _refresh(
    ...
    uint256 rewards = currentBalance - creditLineInfo.balanceTracker;

    if (totalDeposit == 0) {
      creditLineInfo.commissionInterest += rewards;
      return;
    }
    uint256 commissionInterests =
      rewards.wadMul(creditLineInfo.commissionInterestShare);
    creditLineInfo.accRate = creditLineInfo.accRate.rayMul(
      WadRayMath.RAY +
        (rewards - commissionInterests).rayDiv(
          creditLineInfo.balanceTracker -
            creditLineInfo.commissionInterest -
            creditLineInfo.protocolFees
        )
    );
    creditLineInfo.commissionInterest += commissionInterests;
  }
}
  ...
  ReentrancyGuard
{
  ...
  function setShares(
    ...
    require(
      _commissionInterestShare <= PreciseUnitMath.PRECISE_UNIT &&
        ...
        _protocolFeesPercentage <= PreciseUnitMath.PRECISE_UNIT,
      'Invalid share'
    );
    ManagerDataTypes.CreditLineInfo storage creditLineInfo =
    ...
    creditLineInfo.protocolFeesPercentage = _protocolFeesPercentage;
  }
    ...
    onlyLendingCreditLineManager
  {
    ...
    GenericRateLogic._refresh(creditLineInfo, totalDeposited, _currentBalance);
  ...
  function updateProtocolFees(address _creditLine, uint256 _amount)
    public
    override
    nonReentrant
    onlyLendingCreditLineManager
  ...
  function updateBuybackInterest(
    ...
    address _user,
    uint256 _amount
  ) public override nonReentrant onlyLendingCreditLineManager {
    ManagerDataTypes.CreditLineInfo storage creditLineInfo =
      creditLineStorage[_creditLine];
    require(creditLineInfo.lendingModuleId != 0x00, 'Bad creditLine');
    ManagerDataTypes.UserInterestData storage userData =
      userInterestData[_creditLine][_user];
    userData.pendingJrtBuybackInterest -= _amount;
    creditLineInfo.balanceTracker -= _amount;
  }
    ...
    returns (uint256 newUserDeposit, uint256 newTotalDeposit)
  {
    ...
    _currentBalance = _currentBalance > creditLineInfo.balanceTracker
      ...
      : creditLineInfo.balanceTracker;
    if (_isDeposit) {
      (newUserDeposit, newTotalDeposit) = _handleDeposit(
        creditLineInfo,
        _user,
        _creditLine,
        _amount,
      ...
      return (newUserDeposit, newTotalDeposit);
    }
    (newUserDeposit, newTotalDeposit) = _handleWithdraw(
      creditLineInfo,
      _user,
      _creditLine,
      _amount,
  ...
  function _handleWithdraw(
    ...
    (newUserDeposit, newTotalDeposit) = GenericRateLogic._withdraw(
      userData,
      creditLineInfo,
      withdrawAmount,
      userDeposited,
  ...
  function _handleDeposit(
    ...
    if (depositAmount >= protocolFees) {
      (newUserDeposit, newTotalDeposit) = GenericRateLogic._deposit(
        userData,
        creditLineInfo,
    ...
    returns (uint256 collateralInterest, uint256 buyBackInterest)
  {
    ...
    ManagerDataTypes.UserInterestData memory userData =
      userInterestData[_creditLine][_eoaAddress];
    uint256 userDeposited = _getUserDepositedAmount(_creditLine, _eoaAddress);
    uint256 totalDeposited = _getTotalDepositedAmount(_creditLine);

    (collateralInterest, buyBackInterest) = GenericRateLogic._pendingInterests(
      userData,
      creditLineInfo,
      userDeposited,
      totalDeposited,
    ...
    )
  {
    ...
    protocolFeesPercentage = creditLineStorage[_creditLine]
