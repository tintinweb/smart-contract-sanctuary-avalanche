...
contract SyntheticPool is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    IUSBLiquidityPool internal USBPool;
    ...
    function initContract(address _reservePool, address _systemCoin, address _USBPool, uint256 _minSystemCoinTxAmount , uint256 _txFee) external onlyOwner{
        ...
        USBPool = IUSBLiquidityPool(_USBPool);
    ...
    */
    function openLimitOrder(uint256 pid, uint8 orderType, uint256 synTokenAmount, uint256 _synTokenLimitPrice) whenNotPaused external {
        ...
        // if(synTokenPriceDecimal == systemCoinDecimal) {
        //     _synTokenPrice = uint256(currentStockPrice);
        // } else if(synTokenPriceDecimal > systemCoinDecimal) {
        //     _synTokenPrice = uint256(currentStockPrice) / (10 ** (synTokenPriceDecimal - systemCoinDecimal));
        // } else if(synTokenPriceDecimal < systemCoinDecimal) {
        //     _synTokenPrice = uint256(currentStockPrice) * (10 ** (systemCoinDecimal - synTokenPriceDecimal));
        // }
    ...
    /**************************************** Internal FUNCTIONS ****************************************/
    function _getPrice(uint256 pid) public view returns (bool,uint256) {
        PoolInfo memory pool = poolInfo[pid];
        ...
        if (address(assetsPrice) != address(0)){
            ...
            uint256 tokenDecimals = decimalsMap;
    ...
    */
    ...
    function setUSBPool(address newUSBPool) external onlyOwner {
        ...
        USBPool = IUSBLiquidityPool(newUSBPool);
    }
