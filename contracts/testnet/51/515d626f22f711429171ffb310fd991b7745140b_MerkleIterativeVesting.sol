...
contract MerkleIterativeVesting is OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param _earlyClaimAvailablePer - claim percentage
    ...
    /**
     * @notice function that initialize merkle user rewards when he intract with contract for the first time
     *
     * @param _user - the user address
     * @param _totalRewards - the total reward amount
     */
    function initMerkleUser(address _user, uint256 _totalRewards)
    internal
    {
    ...
    onlyOwner
    {
        if(!partialFunding){
            ...
            uint256 fundAmount = safeTransferAndGetAmountDiff(address(msg.sender), address(this), _amount);
    ...
    /**
     * @notice function is doing a safe transfer and claculate the actual amount that passed. to avoid deflationary tokens errors
     *
        ...
        nonReentrant
    {
        ...
        uint256 amountEarned;
        uint256 nPortionsVested;
        (amountEarned, nPortionsVested) = _withdrawCalculation(address(msg.sender), _amount, false);
