    ...
    PausableUpgradeable
{
    ...
    function initialize(bytes memory depositArgs) external initializer {
    ...
    function iSend(bytes memory packet, bytes calldata requestMetadata, uint256 value) internal {
        bytes memory requestPacket = abi.encode(routerBridge, packet);
        (bool success, ) = gatewayContract.call{ value: value }(
            abi.encodeWithSelector(0xa84cee01, 1, 0, "", ROUTER_CHAIN_ID, requestMetadata, requestPacket)
        );
        require(success);
    ...
    function checkBurnableAndLock(address token, address user, uint256 amount) internal {
        if (!_burnList[token]) {
            IERC20Upgradeable(token).safeTransferFrom(user, address(_reserve), amount);
        } else {
    ...
    function transferNativeToken(
        ...
        address weth = _WETH;
        IERC20Upgradeable(weth).safeTransfer(address(_reserve), srcTokenAmount);
        ...
        bytes memory packet = getReservePacket(destChainIdBytes, weth, srcTokenAmount, recipient, widgetId);

        iSend(packet, requestMetadata, msg.value.sub(srcTokenAmount));
    }
    ...
    function transferReserveToken(
        ...
        if (_isUsdcBurnableMintable[destChainIdBytes] && srcTokenAddress == _usdc) {
        ...
        bytes memory packet = getReservePacket(destChainIdBytes, srcTokenAddress, srcTokenAmount, recipient, widgetId);

        iSend(packet, requestMetadata, msg.value);
    }
    ...
    function depositReserveToken(
        ...
        iSend(packet, requestMetadata, msg.value.sub(swapDetails.srcStableTokenAmount));
    }
    ...
    function _handleDepositForReserveToken(
        ...
        if (!isSourceNative) {
            ...
            IERC20Upgradeable(_WETH).safeTransfer(address(_reserve), swapDetails.srcStableTokenAmount);
        }
    ...
    function depositNonReserveToken(
        ...
        iSend(packet, requestMetadata, msg.value.sub(swapDetails.srcTokenAmount));
    }
    ...
    function _depositNonReserveToken(
        ...
        if (!isSourceNative) {
            ...
            IERC20Upgradeable(_WETH).safeTransfer(address(_reserve), swapDetails.srcTokenAmount);
        }
    ...
    function handleUsdcBurn(bytes32 destChainIdBytes, uint256 amount) internal {
        ...
        IERC20Upgradeable(usdc).safeTransferFrom(address(_reserve), address(this), amount);
        IERC20Upgradeable(usdc).safeApprove(address(tokenMessenger), amount);
