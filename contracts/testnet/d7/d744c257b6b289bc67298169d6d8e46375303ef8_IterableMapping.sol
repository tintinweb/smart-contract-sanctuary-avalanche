...
contract NodeManager is Ownable, Pausable {
	...
	event NodeCreated(uint256 indexed amount, address indexed account, uint256 indexed blockTime);
	...
	constructor(uint8 _rewardPerNode, uint256 _minPrice) {
	...
	function _uint2str(uint256 _int) private pure returns (string memory _uintAsString) {
		uint256 _i = _int;
		if (_i == 0) {
			return "0";
		}
	...
	function _calculateNodeRewards(uint256 _lastClaimTime, uint256 amount_)
		private
		view
		returns (uint256 rewards)
	{
		...
		return
			((rewardPerDay.mul(10000).div(1440) * (elapsedTime_ / 1 minutes)) / 10000) *
			boostMultiplier;
	}

	function _calculateBoost(uint256 elapsedTime_) internal view returns (uint256) {
	...
	function getNodeReward(address account, uint256 _creationTime) external view returns (uint256) {
		...
		require(nodes.length > 0, "CASHOUT ERROR: You don't have nodes to cash-out");
	...
	) external onlyGuard onlyNodeOwner(account) whenNotPaused {
		require(_creationTime > 0, "NODE: CREATIME must be higher than zero");
		NodeEntity[] storage nodes = _nodesOfUser[account];
		require(nodes.length > 0, "CASHOUT ERROR: You don't have nodes to cash-out");
		NodeEntity storage node = _getNodeWithCreatime(nodes, _creationTime);
		...
		returns (string memory)
	{
		...
		for (uint256 i = 1; i < nodesCount; i++) {
			...
			_creationTimes = string(
				abi.encodePacked(_creationTimes, separator, _uint2str(_node.creationTime))
			);
		}
		...
		returns (string memory)
	{
		...
		for (uint256 i = 1; i < nodesCount; i++) {
			...
			_lastClaimTimes = string(
				abi.encodePacked(_lastClaimTimes, separator, _uint2str(_node.lastClaimTime))
			);
		}
	...
	function getIndexOfKey(address account) external view onlyOwner returns (int256) {
		require(account != address(0), "getIndex: Not allowing NULL account");
		return nodeOwners.getIndexOfKey(account);
	}

	function burn(uint256 index) external onlyOwner {
		require(index < nodeOwners.size(), "BURN: Index out of range");
