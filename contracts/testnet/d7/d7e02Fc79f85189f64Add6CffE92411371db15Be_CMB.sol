...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct Payment {
        ...
        string data;
        uint256 paymentId;
        uint256 amountPerTreatment;
        uint256 numberOfTreatment;
        uint256 paidAmount;
        uint256 numberOfClaim;
        uint256 duration;
        uint256 createdDate;
        uint256 expiredDate;
        Status status;
     ...
     *  When Business Owner is claiming payment             CLAIMING
     *  After Business Owner claims payment                 CLAIMED
     *  After Client cancels payment                        CANCELLED
     */
    enum Status { REQUESTING, PAID, BO_CONFIRMED, CLIENT_CONFIRMED, CLAIMING, CLAIMED, CANCELLED }
    ...
    /**
     *  @notice rightToCancel uint256 is the duration that client can cancel payment after initialing payment
     */
    ...
    event Claiming(uint256 indexed paymentId, address paymentToken, uint256 numberOfClaim);
    ...
    event SetNumberOfTreatment(uint256 oldValue, uint256 newValue);
        ...
        onlyRequestingPayment(paymentId)
    {
        ...
        uint256 oldAmount = payments[paymentId].amountPerTreatment;
        payments[paymentId].amountPerTreatment = newAmount;
        emit SetAmount(oldAmount, newAmount);
    }

    /** 
     *  @notice Set number of treatment by payment ID
     * 
     *  @dev    Only Business Owner can call this function and payment needs to be initialized. 
     * 
     *          Name                    Meaning 
     *  @param  paymentId               ID of payment that needs to be updated 
     *  @param  newValue                New amount that want to be updated 
     *
     *  Emit event {SetAmount}
     */ 
    function setNumberOfTreatment(uint256 paymentId, uint256 newValue)
        external
        onlyValidPayment(paymentId)
        onlyBusinessOwner(paymentId)
        onlyRequestingPayment(paymentId)
    {
        ...
        uint256 oldValue = payments[paymentId].numberOfTreatment;
        payments[paymentId].amountPerTreatment = newValue;
        emit SetNumberOfTreatment(oldValue, newValue);
    }
     ...
     *  @param  amountPerTreatment      Amount per treatment
     *  @param  numberOfTreatment       Number of treatment
     *  @param  createdDate             Created date of payment
        ...
        onlyValidAddress(client)
    {
        ...
        require(
            ...
            "Duration expired date must be greater than 3 days from created date"
        );
        require(amountPerTreatment > 0, "Amount must be greater than 0");
        require(numberOfTreatment > 0, "Number of treatment must be greater than 0");
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "You've paid this payment"
        );
        require(amount > 0, "Do not pay native token");
        require(
            amount >= payment.amountPerTreatment * payment.numberOfTreatment, 
            "You must pay greater than or equal total amount"
        );
        ...
        nonReentrant
    {
        Payment storage payment = payments[paymentId];
        require(
            payment.status == Status.CLIENT_CONFIRMED || payment.status == Status.CLAIMING,
            "Need to be confirmed by client"
        );
        require(payment.numberOfClaim < payment.numberOfTreatment, "You've claimed enough amount");

        payment.numberOfClaim++;
        _claimPayment(paymentId, _msgSender());
        uint256 totalAmount = payment.amountPerTreatment * payment.numberOfTreatment;
        if (payment.numberOfClaim == payment.numberOfTreatment) {
            payment.status = Status.CLAIMED;
            uint256 excessAmount = payment.paidAmount - totalAmount;
            _makePayment(owner(), payment.paymentToken, excessAmount);
            emit Claimed(paymentId, payment.paymentToken, totalAmount);
        } else {
            payment.status = Status.CLAIMING;
            emit Claiming(paymentId, payment.paymentToken, payment.numberOfClaim);
        }
    }
        ...
        nonReentrant
    {
        ...
        if (receiver == payment.bo) {
            require(payment.numberOfClaim < payment.numberOfTreatment, "Business Owner has claimed enough");
            uint256 remainingAmount = (payment.numberOfTreatment - payment.numberOfClaim) * payment.amountPerTreatment;
            uint256 serviceFee = calculateServiceFee(remainingAmount);
            uint256 excessAmount = payment.paidAmount - payment.numberOfTreatment * payment.amountPerTreatment;
            _makePayment(payment.bo, payment.paymentToken, remainingAmount - serviceFee);
            _makePayment(owner(), payment.paymentToken, excessAmount + serviceFee);
            emit ReturnedMoney(paymentId, receiver, remainingAmount - serviceFee);
        } else {
    ...
    function _claimPayment(uint256 _paymentId, address _receiver) private {
        ...
        uint256 serviceFee = calculateServiceFee(payment.amountPerTreatment);
        uint256 claimableAmount = payment.amountPerTreatment - serviceFee;
