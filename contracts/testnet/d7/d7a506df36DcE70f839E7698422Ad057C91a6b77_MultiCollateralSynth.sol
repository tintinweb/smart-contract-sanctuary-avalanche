...
contract MultiCollateralSynth is Synth {
    ...
    function issue(
        ...
        uint256 amount,
        bool isCrossChain
    ) external onlyInternalContracts {
        super._internalIssue(account, amount, isCrossChain);
    }
    ...
    function burn(
        ...
        super._internalBurn(account, amount, isCrossChain);
    }
...
contract Synth is Owned, IERC20, ExternStateToken, MixinResolver, ISynth {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[5] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
    }
    ...
    function transfer(address to, uint256 value) public onlyProxyOrInternal returns (bool) {
        ...
        if (to == address(0)) {
            return _internalBurn(messageSender, value, true);
        }
    ...
    function issue(
        ...
        _internalIssue(account, amount, isCrossChain);
    }
    ...
    function burn(
        ...
        _internalBurn(account, amount, isCrossChain);
    }
    ...
    function _internalIssue(
        ...
        if (isCrossChain) {
            issuedSynthAggregator().sendIssue(currencyKey, account, amount);
        }
    ...
    function _internalBurn(
        ...
        if (isCrossChain) {
            issuedSynthAggregator().sendBurn(currencyKey, account, amount);
        }
    ...
    function _internalTransferFrom(
        ...
        issuedSynthAggregator().sendTransferFrom(currencyKey, from, to, value);
