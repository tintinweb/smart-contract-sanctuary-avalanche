...
contract Liquifier is Ownable {
    ...
    function buyTokens(uint256 amount) public lock {
        address[] memory path = new address[](2);
        path[0] = IRouter(router).WAVAX();
        path[1] = address(token);

        IRouter(router).swapExactAVAXForTokens{value: amount}(0, path, address(this), block.timestamp.add(300));
    }

    function buyTokensAndBurn(uint256 ethAmount) public lock {
        address[] memory path = new address[](2);
        path[0] = IRouter(router).WAVAX();
        path[1] = address(token);

        IRouter(router).swapExactAVAXForTokens{value: ethAmount}(0, path, address(0), block.timestamp.add(300));
