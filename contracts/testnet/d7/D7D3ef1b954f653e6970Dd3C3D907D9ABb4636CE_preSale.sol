...
contract preSale is Auth, Referral
{
    ...
    {
        require((block.timestamp > endTime) && (endTime > 0 ), "Presale is Live!");
        _;
    }

    function convertTokens()public ifPresaleEnded
    {
    ...
    function convertNode()public ifPresaleEnded
    {
    ...
    function purchaseQETHW(uint256 _qethw, uint256 _option, string memory _name)public ifPresale
    {
        if(_option == 0)
        {
            require(bytes(_name).length == 0, "Already Purchased other presale option!");
    ...
    function withdrawFunds(uint256 _amount, address _beneficiary)external onlyOwner
    {
