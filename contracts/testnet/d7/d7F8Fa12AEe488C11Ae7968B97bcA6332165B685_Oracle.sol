...
contract Oracle is Ownable {
        ...
        returns (int256 _price)
    {
        ...
        if (price > 0) {
            ...
            return (int256((uint256(price).mul(1e18) / (10**decimals))));
        }
    ...
    function getPriceFromTraderJoe(address _pair)
        public
        view
        returns (
        ...
        )
    {
        ...
        uint256 price0Cumulative = IJoePair(_pair).price0CumulativeLast();
        uint256 price1Cumulative = IJoePair(_pair).price1CumulativeLast();

        (
            uint112 _reserve0,
            uint112 _reserve1,
            uint32 _blockTimestampLast
        ) = IJoePair(_pair).getReserves();
        if (_blockTimestampLast != blockTimestamp) {
            ...
            price0Cumulative += uint(_reserve1 / _reserve0) * timeElapsed;
            // counterfactual
            price1Cumulative += uint(_reserve0 / _reserve1) * timeElapsed;
        }
