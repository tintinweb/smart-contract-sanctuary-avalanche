...
contract LaunchPad is OwnableUpgradeable, ILaunchPad {
    ...
    function deployCollection(
        ...
        address deployedCollection = address(
            new ERC721A(
                _maxSupply,
                _mintPrice,
                _startTime,
...
contract ERC721A is Ownable, ERC165, IERC721, IERC721Metadata {
    ...
    constructor(
        ...
        require (_startTimestamp > block.timestamp, "startTimestamp before current time");
        require (_endTimestamp > _startTimestamp, "endTimestamp before startTimestamp");
    ...
    function getFeeDistributions() external view returns (RateDistribute[] memory) {
    ...
    function changeMaxTotalSupply(uint256 _maxTotalSupply) external onlyCreator {
        require (totalSupply() <= _maxTotalSupply, "before current supplied amount");
    ...
    function setWhitelist(address[] memory _users, bool _isAdd) external onlyCreator {
    ...
    function _mintNFT(address _recipient, uint256 _quantity) internal {
        ...
        require (
            !whitelistMode || 
            (whitelist[_recipient] || _recipient == owner()), 
            "only whitelist"
        );

        uint256 requireAmount = mintingPrice * _quantity;
        if (!whitelistMode) {   // if user is whitelisted, he can mint NFT before startTime.
