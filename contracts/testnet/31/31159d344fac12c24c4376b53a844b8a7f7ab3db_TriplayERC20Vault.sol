...
abstract contract TriplayVault is EIP712, ReentrancyGuard, Pausable {
    ...
    function fullShutDown() external whenNotFullShutDown {
        require(msg.sender == policy.owner(), "not a policy owner");
        isFullShutDown = true;
        emit FullShutDown(msg.sender);
    }

    function fullResume() external whenFullShutDown {
        require(msg.sender == policy.owner(), "not a policy owner");
...
// File contracts/TriplayERC20Vault.sol
...
contract TriplayERC20Vault is TriplayVault {
    using SafeERC20 for IERC20;

    bytes32 private constant WITHDRAW_TYPE_HASH = keccak256("Withdraw(uint256 requestId,address player,uint256 amount)");
    IERC20 public immutable token;
    mapping(address => uint256) public cumulativeWithdrawal; //cumulativeWithdrawal[player] = uint256
    mapping(address => uint256) public cumulativeDeposit; //cumulativeDeposit[player] = uint256
    mapping(address => IAccountWithdrawal.DailyInfo) public accountsDailyWithdrawal; 

    event Deposit(address indexed player, uint256 amount);
    event Withdraw(address indexed sender, address indexed player, uint256 requestId, uint256 amount, uint256 newBalance);
    event CancelWithdrawal(address indexed player, uint256 requestId, uint256 amount);
    event TransferToTreasury(address indexed sender, uint256 amount);
    event ResetDailyWithdrawal(address indexed sender, address player);
    ...
    function deposit(uint256 amount) external nonReentrant whenNotFullShutDown whenNotPaused {
        require(amount > 0, "amount = 0");
        amount = _transferFrom(amount);
        cumulativeDeposit[msg.sender] += amount;
    ...
    function withdraw(
        ...
        uint256 currentBalance = cumulativeReward + cumulativeDeposit[player] - cumulativeSpend - cumulativeWithdrawal[player];
        ...
        , "merkle proof failed");
        bytes32 paramsHash = hashWithdrawParams(requestId, player, amount);
        IAccountWithdrawal.DailyInfo storage dailyWithdrawal = accountsDailyWithdrawal[player];
        if (dailyWithdrawal.lastTimestamp == 0 || block.timestamp >= dailyWithdrawal.lastTimestamp + 24 hours) {
        ...
        policy.verifyWithdrawERC20(player, dailyWithdrawal, amount, signatures, paramsHash);
        cumulativeWithdrawal[player] += amount;
        requestsStatus[requestId] = RequestStatus.Executed;
        token.safeTransfer(player, amount);
        emit Withdraw(msg.sender, player, requestId, amount, currentBalance - amount);
    }
    ...
    function cancelWithdrawal(
        ...
        require(requestsStatus[requestId] == RequestStatus.Pending, "not a pending request");
        bytes32 paramsHash = hashWithdrawParams(requestId, msg.sender, amount);
        policy.verifyCancelWithdrawal(signatures, paramsHash);
        requestsStatus[requestId] = RequestStatus.Canceled;
        emit CancelWithdrawal(msg.sender, requestId, amount);
    }

    function resetDailyWithdrawal(address player) external {
        require(policy.isOperator(msg.sender), "not an operator");
        delete accountsDailyWithdrawal[player];
        emit ResetDailyWithdrawal(msg.sender, player);
    }

    function transferToTreasury(address treasury, uint256 amount) external whenNotFullShutDown {
        require(amount > 0, "amount = 0");
        policy.verifyTransferToTreasury(msg.sender, treasury);
        token.safeTransfer(treasury, amount);
        emit TransferToTreasury(msg.sender, amount);
    }
    ...
    function _transferFrom(uint amount) internal returns (uint256 balance) {
