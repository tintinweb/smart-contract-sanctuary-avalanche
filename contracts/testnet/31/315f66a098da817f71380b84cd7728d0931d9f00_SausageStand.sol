...
contract SausageStand is Ownable, KindaRandom {
    ...
    function mint(uint256[] calldata condimintTokenIds) external {
        ...
        for (uint256 i = 0; i < quantity; ++i) {
            uint256 randomish = _randomInt() % (totalSupply - currentMinted - i);
    ...
    function _randomInt() private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(currentMinted, block.timestamp, blockhash(block.number))));
    }
}
