...
import "../interfaces/IYieldFarmVault.sol";
...
contract YieldFarmStakePoolManager is Ownable {
  ...
  function lpToken() public view returns (address) {
    return IJoeOracle(joeOracle).lpToken(tokenA, tokenB);
  }

  function lpTokenAmount() public view returns (uint256) {
    (uint256 amount, ,,,,) = ITokenStakePool(stakePool).userPools(stakePoolId, address(this));

    return IJoePair(lpToken()).balanceOf(address(this)) + amount;
  }
  ...
  function assetInfo() public view returns (uint256, uint256) {
    ...
    return (tokenAAssetAmount, tokenBAssetAmount);
  }

  // Get debt info of vault. Debt = Asset - Equity
  function debtInfo() public view returns (uint256, uint256) {
      return (ILendingPool(tokenALendingPool).maxRepay(vault), ILendingPool(tokenBLendingPool).maxRepay(vault));
  }
  ...
  function work(
    ...
    if (_action == ManagerAction.Withdraw) {
      if (_lpAmt > 0) {
        ...
        if (_lpAmt > lpTokenAmount()) {
          _lpAmt = lpTokenAmount();
        }
    ...
    if (_action == ManagerAction.AddLiquidity) {
      // Borrow from lending pools
      _borrow(_borrowTokenAAmt, _borrowTokenBAmt);
      if (_repayTokenAAmt > 0 || _repayTokenBAmt > 0) {
  ...
  function compound() external {
    ITokenStakePool(stakePool).deposit(stakePoolId, 0);
  ...
  function _addLiquidity() internal {
    ...
    if (optimalSwapAmount > 0) {
      ...
      try IJoeRouter02(router).swapExactTokensForTokens(
        optimalSwapAmount,
        0,
    ...
    try IJoeRouter02(router).addLiquidity(
      tokenA,
      tokenB,
      IERC20(tokenA).balanceOf(address(this)),
      IERC20(tokenB).balanceOf(address(this)),
  ...
  function _removeLiquidity(uint256 _lpAmt) internal {
    IJoePair(lpToken()).approve(router, _lpAmt);
    try IJoeRouter02(router).removeLiquidity(
      tokenA,
      tokenB,
      _lpAmt,
      0,
  ...
  function _stake() internal {
    ...
    ITokenStakePool(stakePool).deposit(stakePoolId, IERC20(lpToken()).balanceOf(address(this)));
  }

  function _unstake(uint256 _lpAmt) internal {
    ITokenStakePool(stakePool).withdraw(stakePoolId, _lpAmt);
  }
  ...
  function _swapExcess() internal {
    ...
    if (IYieldFarmVault(vault).getStrategy() == VaultStrategy.Neutral) {
    ...
    if (IYieldFarmVault(vault).getStrategy() == VaultStrategy.Long) {
  ...
  function _swapRewardWithFee() internal {
    ...
    // Transfer 20% fee to treasury
    // TODO: management fee can be changeable
    uint256 fee = IERC20(tokenB).balanceOf(address(this)) * 20 / 100;
    IERC20(tokenB).safeTransfer(treasury, fee);
  }

  function _borrow(uint256 _borrowTokenAAmt, uint256 _borrowTokenBAmt) internal {
    if(_borrowTokenAAmt > 0) {
        ILendingPool(tokenALendingPool).borrow(_borrowTokenAAmt, msg.sender);
      }
    if(_borrowTokenBAmt > 0) {
      ILendingPool(tokenBLendingPool).borrow(_borrowTokenBAmt, msg.sender);
    }
  }
  ...
  function _repay(uint256 _repayTokenAAmt, uint256 _repayTokenBAmt) internal {
    if(_repayTokenAAmt > 0) {
      ...
      ILendingPool(tokenALendingPool).repay(_repayTokenAAmt, msg.sender);
    }
    if(_repayTokenBAmt > 0) {
      IERC20(tokenB).approve(tokenBLendingPool, _repayTokenBAmt);
      ILendingPool(tokenBLendingPool).repay(_repayTokenBAmt, msg.sender);
    }
  }
...
interface IYieldFarmVault {
  function getStrategy() external view returns (VaultStrategy);
}
...
interface ITokenStakePool {
  function userPools(uint256 poolId, address user) external view
    returns (
      uint256,
      uint256,
      uint256,
  ...
  function rewardsEarned(address _user) external view returns (uint256);
  ...
  function updateRewardsDistributionRate(uint256 rate) external;

  function updateUnlockTime(uint256 numberOfSeconds) external;

  function updateRelockTime(uint256 numberOfSeconds) external;
}
