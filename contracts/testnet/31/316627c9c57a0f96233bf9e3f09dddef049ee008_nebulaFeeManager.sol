...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,uint256[] memory times,bool _feeFroze){
		...
		uint256[5] memory ls = [Zero,Zero,Zero,Zero,Zero];
		for(uint i=0;i<5;i++){
			ls[i] = times[i].mul(1 days);
		}
		feeFroze = true;
		feePeriod = ls[0];
		//require(4>5,"asdfasdfdsf");
		gracePeriod = ls[1];
		//require(4>5,"asdfasdfdsf");
		protoLife = ls[2];
		//require(4>5,"asdfasdfdsf");
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		uint256 _collpase= _time.add(protoLife);
		uint256 _nextDue = feePeriod.add(_time);
		uint256 _payable = nebuLib.doMultiple(feePeriod,maxFeePayment);	_protoowners.push(PROTOOWNERS({name:_name,protoCreationDate:INTgetCurrentTime(_account),protoElapsed:Zero,protoCollapseDate:_collpase,protoNextDue:_nextDue,protoDueElapsed:feePeriod,protoFutureFees:Zero,protoIncreasedDecay:Zero,protoFeeFroze:_protoFeeFroze,protoTotalPayable:_payable}));
	...
	function INTcollapse(address _account,uint256 _x) internal{
    	...
    	function updateFeesPaidSpec(address _account,uint _intervals,uint256 _x) internal{
    		for(uint i=0;i<_intervals;i++){
	    		INTreconcileAccountFees(_account);
	    		INTupdateProtoNextDue(_account,_x,feePeriod);
	...
	function INTupdateProtoFeeFroze(address _account,uint256 _x,uint256 _time) internal {
		...
		if(feeFroze == false && INTgetFeeFroze() != Zero){
			INTupdateProtoNextDue(_account,_x,_time.sub(nebuLib.getLower(INTgetFeeFroze(),INTgetProtoCreationDate(_account,_x))));
