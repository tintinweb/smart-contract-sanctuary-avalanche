...
import "./MembersPoolAuthorization.sol";
import "../../interfaces/IMembersPool.sol";
import "../../interfaces/IERC4626.sol";

contract MembersPool is
    Initializable,
    MembersPoolAuthorization,
    ERC20Upgradeable,
    PausableUpgradeable,
    ...
    IERC4626
{
    ...
    uint256 public totalFiatAssets;
    ...
    error DepositCanBeWithdrawnWithRegularFlow();
    error DepositAlreadyScheduledForEmergencyWithdraw();
    ...
    function initialize(
        ...
        require(asset_ != address(0), "asset should be specified");
        require(_treasury != address(0), "treasury should be specified");
        require(maxDeposit_ > 0, "maxDeposit should be grater than 0");
        __MembersPoolAuthorization_init(msg.sender);
        __ERC20_init(RECEIPT_TOKEN_NAME, RECEIPT_TOKEN_SYMBOL);
        _asset = asset_;
        treasury = _treasury;
        depositFeeNominator = _depositFeeNominator;
        emit DepositFeeUpdated(_depositFeeNominator);
        globalWithdrawTimelock = _globalWithdrawTimelock;
        emit GlobalWithdrawTimelockUpdated(_globalWithdrawTimelock);
        withdrawWaitingPeriod = _withdrawWaitingPeriod;
        emit WithdrawWaitingPeriodUpdated(_withdrawWaitingPeriod);
        initialLockPercent = _initialLockPercent;
        emit InitialLockPercentUpdated(_initialLockPercent);
        _maxDeposit = maxDeposit_;
        emit MaxDepositUpdated(maxDeposit_);
        emit MemberPoolInitialized(address(this));
    }

    // *************** INVESTOR ACTIONS ***************************

    function deposit(uint256 assets, address receiver) external override whenNotPaused returns (uint256 shares) {
        return _deposit(assets, receiver, false);
    }

    function managerDeposit(uint256 assets, address receiver) external override whenNotPaused returns (uint256 shares) {
    ...
    function mint(uint256 shares, address receiver) external override whenNotPaused returns (uint256 assets) {
        ...
        assets = _convertToAssets(_deposit(_assets, receiver, false));
    }

    function managerMint(uint256 shares, address receiver) external override whenNotPaused returns (uint256 assets) {
        uint256 _assets = _previewMint(shares);
        assets = _convertToAssets(_deposit(_assets, receiver, true));
    }

    function scheduleRedeem(uint256 shares) external override whenNotPaused {
        _scheduleRedeem(shares);
    }

    function scheduleWithdraw(uint256 assets) external override whenNotPaused {
    ...
    function scheduleEmergencyWithdraw(uint256[] memory depositIds) external override whenNotPaused {
        uint256 totalWithdrawShares = 0;
        uint256 totalWithdrawPenaltyShares = 0;
        EmergencyWithdrawInfo[] memory ewInfos = _getEmergencyWithdrawInfo(msg.sender, depositIds);

        for (uint256 i = 0; i < ewInfos.length; i++) {
            totalWithdrawShares += ewInfos[i].availableToWithdrawShares;
            totalWithdrawPenaltyShares += ewInfos[i].withdrawPenaltyShares;
            DepositInfo storage dInfo = depositInfos[msg.sender][depositIds[i]];
            dInfo.scheduledEmergencyWithdraw = true;
        }
        ...
        emit EmergencyWithdrawScheduled(
            msg.sender,
            totalWithdrawShares,
            totalWithdrawPenaltyShares,
            depositIds,
    ...
    function emergencyWithdraw(address receiver, address owner) external override whenNotPaused nonReentrant {
        EmergencyWithdrawRequest memory ewr = emergencyWithdrawRequests[owner];
        ...
        if (balanceOf(owner) < ewr.withdrawShares + ewr.withdrawPenaltyShares) {
        ...
        _updateEmergencyDepositInfos(owner, ewr.depositIds);
        delete emergencyWithdrawRequests[owner];

        IERC20Upgradeable(_asset).safeTransfer(receiver, assets);
        _transfer(owner, treasury, sharesToTreasury);
        emit EmergencyWithdraw(receiver, sharesToBurn);
    }
    ...
    function updateInvestmentBalance(
        ...
        emit InvestmentBalanceUpdated(qsrp, newBalance, realEventTime);
    }

    function withdrawInvestmentToQSRP(address qsrp, uint256 amount)
        external
        override
        whenNotPaused
        onlyManager
        nonReentrant
    {
        ...
        IERC20Upgradeable(_asset).safeTransfer(qsrp, amount);
    }

    function returnInvestment(address qsrp, uint256 amount) external override whenNotPaused onlyManager nonReentrant {
        ...
        emit InvestmentReturned(qsrp, amount);
        IERC20Upgradeable(_asset).safeTransferFrom(msg.sender, address(this), amount);
    }

    // *************** QSRP related actions ***************************
    function registerQSRP(address qsrp, bool status) external override whenNotPaused onlyQSRPRegistrant {
        _registeredQSRPs[qsrp] = status;
        emit QSRPRegistrationUpdated(qsrp, status);
    }

    // *************** Admin actions ***************************
    function updateWithdrawFee(uint256 newFee) external override whenNotPaused onlyAdmin {
        //todo implement or remove
    }

    function updateTreasuryAddress(address newTreasuryAddress) external override whenNotPaused onlyAdmin {
        require(newTreasuryAddress != address(0), "treasury should be specified");
        treasury = newTreasuryAddress;
        emit TreasuryAddressUpdated(newTreasuryAddress);
    }

    function updateDepositFeeNominator(uint16 _depositFeeNominator) external override whenNotPaused onlyAdmin {
        depositFeeNominator = _depositFeeNominator;
        emit DepositFeeUpdated(_depositFeeNominator);
    }
    ...
    function updateWithdrawWaitingPeriod(uint256 _withdrawWaitingPeriod) external override whenNotPaused onlyAdmin {
        withdrawWaitingPeriod = _withdrawWaitingPeriod;
        emit WithdrawWaitingPeriodUpdated(_withdrawWaitingPeriod);
    }

    function updateInitialLockPercent(uint16 _initialLockPercent) external override whenNotPaused onlyAdmin {
        initialLockPercent = _initialLockPercent;
        emit InitialLockPercentUpdated(_initialLockPercent);
    }

    function updateGlobalWithdrawTimelock(uint256 globalTimelock) external override whenNotPaused onlyAdmin {
        globalWithdrawTimelock = globalTimelock;
        emit GlobalWithdrawTimelockUpdated(globalTimelock);
    }

    function updateMaxDeposit(uint256 maxDeposit_) external override whenNotPaused onlyAdmin {
        _maxDeposit = maxDeposit_;
        emit MaxDepositUpdated(maxDeposit_);
    }
    ...
    function _deposit(
        ...
        address receiver,
        bool _isManager
    ) internal nonReentrant returns (uint256 sharesToMint) {
        if (_userDeposits[receiver] + assets > _maxDeposit) {
        ...
        if (sharesToMint != 0) {
            ...
            uint256 depositIndex = lastDepositIndex[receiver] + 1;
            lastDepositIndex[receiver] = depositIndex;
            ...
            emit DepositFeeCollected(receiver, depositIndex, treasury, depositFee);
            emit TokensMinted(receiver, depositIndex, receiver, sharesToMint);
            ...
            if (_isManager) {
                ...
                IERC20Upgradeable(_asset).safeTransferFrom(msg.sender, address(this), assets - depositFee);
                IERC20Upgradeable(_asset).safeTransferFrom(msg.sender, treasury, depositFee);
            }
        }
    }
    ...
    function _checkAvailableFunds(uint256 amount) internal view {
        if (amount > IERC20Upgradeable(_asset).balanceOf(address(this))) {
            revert InsufficientFundsInPool();
        }
    }

    function _totalAssets() internal view returns (uint256) {
        return totalFiatAssets + totalAssetsInQSRP + IERC20Upgradeable(_asset).balanceOf(address(this));
    }

    function _assetsUnit() internal view returns (uint256) {
        return 10**uint256(ERC20Upgradeable(address(_asset)).decimals());
    }
    ...
    function _getSharesAvailableToWithdraw(address holder) internal view returns (uint256) {
        ...
        for (uint256 i = 1; i <= _lastDepositIndex; i++) {
            DepositInfo memory info = depositInfos[holder][i];
            if (
                info.lockedUntilTimestamp < block.timestamp &&
                !info.emergencyWithdrawn &&
                !info.scheduledEmergencyWithdraw
            ) {
    ...
    function _getEmergencyWithdrawInfo(address investor, uint256[] memory depositIds)
        internal
        view
        returns (EmergencyWithdrawInfo[] memory)
    {
        EmergencyWithdrawInfo[] memory result = new EmergencyWithdrawInfo[](depositIds.length);

        for (uint256 i = 0; i < depositIds.length; i++) {
            DepositInfo memory dInfo = depositInfos[investor][depositIds[i]];

            if (!dInfo.emergencyWithdrawn) {
                if (dInfo.scheduledEmergencyWithdraw) {
                    revert DepositAlreadyScheduledForEmergencyWithdraw();
                }
                if (dInfo.lockedUntilTimestamp < block.timestamp) {
                    revert DepositCanBeWithdrawnWithRegularFlow();
                }
                ...
                // optimized formula to avoid rounding issues
                result[i].currentPercent =
                    ((_LOCK_PERCENT_PRECISION - dInfo.initialLockPercent) *
                        _assetsUnit() *
    ...
    function _redeem(
        ...
        WithdrawRequest memory wr = withdrawRequests[owner];
        if (wr.createdTimestamp == 0) {
            revert NoActiveWithdrawRequests();
        }
        ...
        _checkAvailableFunds(assets);
        _burn(owner, shares);
        _updateDepositInfos(owner, shares);
    ...
    function getEmergencyWithdrawInfo(address investor, uint256[] memory depositIds)
        external
        view
        override
        returns (EmergencyWithdrawInfo[] memory)
    {
        return _getEmergencyWithdrawInfo(investor, depositIds);
    }
...
contract MembersPoolAuthorization is AccessControlEnumerableUpgradeable {
    bytes32 public constant MANAGER_ROLE = keccak256("MembersPool.MANAGER");
    bytes32 public constant QSRP_REGISTRANT_ROLE = keccak256("MembersPool.QSRP_REGISTRANT");

    /// @dev Add `root` to the admin role as a member.
    function __MembersPoolAuthorization_init(address root) public initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, root);
        _setRoleAdmin(MANAGER_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(QSRP_REGISTRANT_ROLE, DEFAULT_ADMIN_ROLE);
    }
    ...
    /// @dev Restricted to members of the QSRPRegistrant role.
    modifier onlyQSRPRegistrant() {
        require(isQSRPRegistrant(msg.sender), "Restricted to managers.");
        _;
    }
    ...
    /// @dev Return `true` if the account belongs to the QSRPRegistrant role.
    function isQSRPRegistrant(address account) public view virtual returns (bool) {
        return hasRole(QSRP_REGISTRANT_ROLE, account);
    }

    /// @dev Add an account to the manager role. Restricted to admins.
    function addManager(address account) external virtual onlyAdmin {
        grantRole(MANAGER_ROLE, account);
    }

    /// @dev Add an account to the QSRPRegistrant role. Restricted to admins.
    function addQSRPRegistrant(address account) external virtual onlyAdmin {
        grantRole(QSRP_REGISTRANT_ROLE, account);
    }
    ...
    function removeManager(address account) external virtual onlyAdmin {
        revokeRole(MANAGER_ROLE, account);
    }

    /// @dev Remove an account from the QSRPRegistrant role. Restricted to admins.
    function removeQSRPRegistrant(address account) external onlyAdmin {
        revokeRole(QSRP_REGISTRANT_ROLE, account);
    }
...
interface IMembersPool {
    // *************** EVENTS ***************************
    event MemberPoolInitialized(address poolAddress);
    event Deposit(
    ...
    event TokensMinted(address indexed depositOwner, uint256 depositIndex, address indexed beneficiary, uint256 amount);
    ...
    event EmergencyWithdrawScheduled(
        ...
        uint256 totalWithdrawPenaltyShares,
        uint256[] depositIndexes,
        uint256 unlockedAt
    );
    event EmergencyWithdraw(address indexed beneficiary, uint256 numberOfShares);
    event InvestmentWithdrawnToQSRP(address indexed requester, address qsrp, uint256 amount);
    event InvestmentBalanceUpdated(address indexed qsrp, uint256 newBalance, uint256 realEventTime);
    event InvestmentReturned(address indexed qsrp, uint256 amount);

    event WithdrawFeeUpdated(uint256 newFee);
    event DepositFeeUpdated(uint256 newDepositFeeNominator);
    event GlobalWithdrawTimelockUpdated(uint256 globalWithdrawTimelock);
    event WithdrawWaitingPeriodUpdated(uint256 withdrawWaitingPeriod);
    event InitialLockPercentUpdated(uint16 initialLockPercent);
    ...
    struct DepositInfo {
        ...
        bool scheduledEmergencyWithdraw;
    }

    struct EmergencyWithdrawInfo {
        uint256 amountPaid; // In USDC
        uint256 sharesReceived; // In receipt tokens
        uint256 createdTimestamp;
        uint256 lockedUntilTimestamp;
    ...
    function emergencyWithdraw(address receiver, address owner) external;
    ...
    function managerDeposit(uint256 assets, address receiver) external returns (uint256 shares);

    function managerMint(uint256 shares, address receiver) external returns (uint256 assets);
