    ...
    ClampedRandomizer
{
    ...
    function play(uint256 quantity) external payable {
        require(!paused, "game is paused");
        require(msg.value == (_price * quantity), "incorrect amount of AVAX sent");
        require(remaining1 > 0 ||remaining2 > 0 ||remaining3 > 0 ||remaining4 > 0 || !avaSharkWon, "all prizes have been caught" );
    ...
    function internalMint(address to, uint256 amount) internal {
        ...
        for (uint256 i = 1; i <= amount; i++) {  
            ...
            if (tokenID <= 5000 && !won[0] && remaining1 > 0) {  //5.85 chance to win the best prize
                contract1.mint(1,to);
                won[0] = true;
                remaining1 -= 1;
            } else if ( 5000 < tokenID && tokenID <= 15000 && remaining2 > 0) { //17.55 chance to win second best
                contract2.mint(1,to);
                won[1] = true;
                remaining2 -= 1;
            } else if ( 15000 < tokenID && tokenID <= 30000 && remaining3 > 0) { // chance to win third best
                contract3.mint(1,to);
                won[2] = true;
                remaining3 -= 1;
            }else if (30000 < tokenID && tokenID <= 30100 && remaining4 > 0){ // chance to win 1 avax
                payable(to).transfer(1 ether);
                won[3] = true;
                remaining4 -= 1;
            }else if(95000 < tokenID && !avaSharkWon){
