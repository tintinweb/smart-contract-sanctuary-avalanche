...
contract TUBasicCastle {
    ...
    */
    ...
    constructor () {
        addNewOwnerChangeLog(0, 0x17a9263CF30a098BA360DB39B4eed8b45f8A25df, 100);
        addNewOwnerChangeLog(0, 0x287c050EaBec9E1BB522F52468b7eAf901007316, 150);
    }
    ...
    function resetPassedLog(uint _tokenId) public {
        uint maxTimestamp;
        cal(_ownersLog[_tokenId].length);
        uint _ownersLength = _ownersLog[_tokenId].length;
        uint offset;
        for (uint i=0; i < _ownersLength; i++) {
            cal(i);
            uint _i = i - offset;
            if (cal(_ownersLog[_tokenId][_i]) <= block.timestamp) {
                if (cal(_ownersLog[_tokenId][_i]) > cal(maxTimestamp)) {
                    baseOwner[_tokenId] = cal2(_ownersTimestampLog[_tokenId][_ownersLog[_tokenId][_i]]);
                    maxTimestamp = cal(_ownersLog[_tokenId][_i]);
                }

                _ownersLog[_tokenId][_i] = cal(_ownersLog[_tokenId][_ownersLog[_tokenId].length-1]);
        ...
        maxTimestamp = 0;
        offset = 0;
        uint _statsLength = _statsLog[_tokenId].length;
        for (uint i=0; i < _statsLength; i++) {
            uint _i = i - offset;
            if (_statsLog[_tokenId][_i] <= block.timestamp) {

                if (_statsLog[_tokenId][_i] > maxTimestamp) {
                    baseStats[_tokenId] = _statsTimestampLog[_tokenId][_statsLog[_tokenId][_i]];
                    maxTimestamp = _statsLog[_tokenId][_i];
                }

                _statsLog[_tokenId][_i] = _statsLog[_tokenId][_statsLog[_tokenId].length-1];
