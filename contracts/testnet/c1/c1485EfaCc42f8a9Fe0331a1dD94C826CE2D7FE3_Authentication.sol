...
contract Authentication is Ownable, IAuthentication {
    ...
    function setMultipleOwnersOfNFTs(OwnershipUpdate[] memory _newOwnerships) external onlyOwner {
        ...
        for (uint i = 0; i < _newOwnerships.length; i++) {
            ...
            transfers[i] = Transfer({from: from, to: _newOwnerships[i].newOwner, chainId: _newOwnerships[i].chainId, projectAddress: _newOwnerships[i].projectAddress, tokenId: _newOwnerships[i].tokenId, metadata: _newOwnerships[i].metadata});
        }
