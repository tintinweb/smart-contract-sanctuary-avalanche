...
contract InfinityToken is IInfinityToken, Initializable, InfinityTokenERC1155, InfinityTokenERC721 {
    ...
    /// @custom:deprecated
    /// @dev use InfinityPool.serverUpdateBalances() instead
    function transfer(
        address from,
        address to,
    	uint[] memory _coinIds, 
    ...
    function _ifUserTokenExistsERC721(
        ...
        exists = _ownersNFT[tokenAddress][tokenId]==account;
    }
    function _depositERC721(
        ...
        require(_ownersNFT[tokenAddress][tokenId]==address(0), "ERC721 already owned by another user");
        address operator = _msgSender();
        _balances[tokenAddress][account].amount += 1; // standard balance update
        _ownersNFT[tokenAddress][tokenId] = account; // save tokenId to user
    ...
    function _withdrawERC721(
        ...
        require(_ownersNFT[tokenAddress][tokenId]==address(account), "ERC721: already belongs to another address");
        _balances[tokenAddress][account].amount -= 1; // standard balance update
        _ownersNFT[tokenAddress][tokenId] = address(0);
...
contract InfinityTokenERC721 {
    ...
    mapping(uint => mapping(uint => address)) internal _ownersNFT;
