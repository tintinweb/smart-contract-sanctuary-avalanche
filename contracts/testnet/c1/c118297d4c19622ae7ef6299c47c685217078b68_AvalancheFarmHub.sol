...
contract AvalancheFarmHub {
	using SafeMath for uint256;
	
	IBEP20 public BUSD = IBEP20(0x3a860CfF7b8F2072Bc9704984D454816ffC6F71d); 
    ...
    uint256[] public PLAN_UNLOCK_TIME = [0 days, 0 days, 0 days, 0 days, 0 days, 0 days];
    uint256[] public PLAN_STAKE_BONUS = [10, 20, 30, 40, 50, 100];

    uint256[] public TOKENS_AIRDROP_PERCENTS = [20000, 20000, 20000, 20000, 20000, 20000];
	
	
	uint256[] public REFERRAL_PERCENTS = [140, 20, 20, 20, 20, 1, 1, 1, 1, 1];
	
	uint256 public constant BNB_PER_TICKET = 1e17; // 0.1 AVAX
    ...
    uint256 public constant STAKE_MIN_AMOUNT = 1e18; //  1 Binance-Peg BUSD Token IN AVAX (BUSD)
	...
	constructor(address payable wallet, address payable _developer, address payable _marketing, IBEP20 BAF_TOKEN_ADDRESS) public {
        ...
        plans.push(Plan(80, 18)); // 1.8% per day for 80 days
        plans.push(Plan(70, 22)); // 2.2% per day for 70 days 
        plans.push(Plan(60, 27)); // 2.7% per day for 60 days
...
function invest(address payable referrer,uint8 plan) public payable {
	...
	function stakeBafTokens(uint256 bafAmount) public {
		...
		require(BAF_TOKEN.balanceOf(msg.sender) >= bafAmount, "Insuff. AFH balance");
	...
	function sellBafTokens(uint256 amountToSell) public { 
		require(BAF_TOKEN.balanceOf(msg.sender) >= amountToSell, "Insuff. AFH bal.");
    ...
    function claimBafTokens() public {
        ...
        require(user.bafTokensToClaim > 0, "Insuff. AFH bal.");
	...
	function stake(uint256 busdAmount) public {
	    require(busdAmount >= STAKE_MIN_AMOUNT, "Min. is 1 BUSD");
	...
	function claimLotteryReward() public {
      ...
      function _buyTickets(address userAddress, uint256 amount) private { // amount - AVAX for purchase
