/**
 *Submitted for verification at testnet.snowtrace.io on 2023-04-15
*/
...
contract Otc is Pausable, Ownable {
    ...
    event BuyEvent(address indexed wallet, uint256 indexed amount);

    constructor(address _tokenAddress, address _stableCoinAddress, uint256 initialPrice) {
        tokenAddress = IERC20(_tokenAddress);
        stableCoinAddress = IERC20(_stableCoinAddress);
        price = initialPrice;
    }

    function list(uint256 amountXeta) external {
        bool res = tokenAddress.transferFrom(msg.sender, address(this), amountXeta);
        require(res, "Error transferring XETA tokens");
        uint256 id = block.timestamp + (uint256(uint160(msg.sender)) * (2 ** 96)); // address is 160 bits, uint256 is 256 bits. Making a 96-bit right shift
        listings[id] = Listing(
            {
                wallet: msg.sender,
                amount: amountXeta,
    ...
    // wallet = 0 means no matter from whom to buy
    function buy(address wallet, uint256 amountXeta) external {
        //require(stableCoinAddress.balanceOf(msg.sender) >= amountXeta * price / (10 ** xetaDecimals), Strings.toString(amountXeta * price / (1 ** xetaDecimals)));
        ...
        while (amountXetaRemains > 0 || currentId != 0) {
            ...
            if (listings[currentId].amount > amountXetaRemains) {
                xetaToTransfer = amountXetaRemains;
                listings[currentId].amount -= amountXetaRemains;
                amountXetaRemains = 0;
            } else {
                xetaToTransfer = listings[currentId].amount;
                amountXetaRemains -= listings[currentId].amount;
        ...
        emit BuyEvent(wallet, amountXeta);
    }
    ...
    function getListings(uint256 initialListingId, uint256 numberOfListings) public view returns (Listing[] memory) {
        ...
        while (numberOfListings > 0 && listings[curListingId].amount > 0) {
    ...
    function countListingsFromId(address wallet, uint256 initialListingId, uint256 maxNumberOfListing) public view returns (uint256) {
        uint256 numberOfListing = 0;
        if (maxNumberOfListing == 0) {
            if (initialListingId == 0 || initialListingId == firstListingId) {
                return listingsNumber;
            }
            numberOfListing = listingsNumber;
        }

        while (numberOfListing < maxNumberOfListing || listings[initialListingId].nextListingId != 0) {
            initialListingId = listings[initialListingId].nextListingId;
    ...
    function removeFromList(uint256 id) external onlyOwner {
        internalRemoveFromList(id);
    }

    function internalRemoveFromList(uint256 id) internal {
        if (listings[id].prevListingId == 0 && listings[id].nextListingId == 0) {
            // The single record
            firstListingId = 0;
            lastListingId = 0;
            return;
        }
        if (listings[id].prevListingId == 0) {
            // The first record
            firstListingId = listings[id].nextListingId;
            listings[firstListingId].prevListingId = 0;
        } else if (listings[id].nextListingId == 0) {
            // The last record
            lastListingId = listings[id].prevListingId;
            listings[lastListingId].nextListingId = 0;
        } else {
            listings[listings[id].prevListingId].nextListingId = listings[id].nextListingId;
            listings[listings[id].nextListingId].prevListingId = listings[id].prevListingId;
        }
