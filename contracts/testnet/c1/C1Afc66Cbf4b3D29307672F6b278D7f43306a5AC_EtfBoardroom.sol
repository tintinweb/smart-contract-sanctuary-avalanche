...
contract EtfBoardroom is ShareWrapperEtf, ContractGuard {
    ...
    uint256 additionalRewardLastRewardTime;
    uint256 accAOEATokenPerShare;
    uint256 additionalRewardLastDevRewardTime = 0;
    uint256 additionalRewardLastDaoRewardTime = 0;
    uint256 public additionalRewardPoolEndTime = 0;
    uint256 public additionalRewardPoolStartTime = 0;
    uint256 public aoeaTokenPerSecondForDev = 0;
    uint256 public aoeaTokenPerSecondForDao = 0;
    ...
    function initialize(
        ...
        additionalRewardLastDevRewardTime = additionalRewardPoolEndTime;
        additionalRewardLastDaoRewardTime = additionalRewardPoolEndTime;

        aoeaTokenPerSecondForDev = treasury.aoeaTokenPerSecondForDev();
        aoeaTokenPerSecondForDao = treasury.aoeaTokenPerSecondForDao();
    ...
    function withdraw(uint256 amount) public override onlyOneBlock memberExists updateReward(msg.sender) {
        ...
        uint256 _devReward = pendingDev(additionalRewardLastDevRewardTime, block.timestamp, msg.sender);
        uint256 _daoReward = pendingDao(additionalRewardLastDaoRewardTime, block.timestamp, msg.sender);
        if (_devReward > 0) {
            safeAdditionalRewardTokenTransfer(msg.sender, _devReward);
            emit RewardPaid(msg.sender, _devReward);
            additionalRewardLastDevRewardTime = block.timestamp;
        }
        if (_daoReward > 0) {
            safeAdditionalRewardTokenTransfer(msg.sender, _daoReward);
            emit RewardPaid(msg.sender, _daoReward);
            additionalRewardLastDaoRewardTime = block.timestamp;
        }
    ...
    function emergencyWithdraw() external onlyOneBlock {
        ...
        members[member].epochTimerStart = treasury.epoch(); // reset timer
    ...
    function pending(address _user) external view returns (uint256) {
        ...
        uint256 devReward = pendingDev(additionalRewardLastDevRewardTime, block.timestamp, _user);
        uint256 daoReward = pendingDao(additionalRewardLastDaoRewardTime, block.timestamp, _user);

        return pendingUser.add(devReward).add(daoReward);
    }

    function pendingDev(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (treasury.isDevWallet(_user)) {
            ...
            if (_toTime >= additionalRewardPoolEndTime) {
                ...
                if (_fromTime <= additionalRewardPoolStartTime) 
                    return additionalRewardPoolEndTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDev).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
                return additionalRewardPoolEndTime.sub(_fromTime).mul(aoeaTokenPerSecondForDev).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
            } else {
                if (_toTime <= additionalRewardPoolStartTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) 
                    return _toTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDev).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
                return _toTime.sub(_fromTime).mul(aoeaTokenPerSecondForDev).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
            }
        }
    ...
    function pendingDao(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (treasury.isDaoWallet(_user)) {
            uint256 additionalRewardTotalAllocPoint = treasury.additionalRewardTotalAllocPoint();
            if (_fromTime >= _toTime) return 0;
            if (_toTime >= additionalRewardPoolEndTime) {
                if (_fromTime >= additionalRewardPoolEndTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) 
                    return additionalRewardPoolEndTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDao).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
                return additionalRewardPoolEndTime.sub(_fromTime).mul(aoeaTokenPerSecondForDao).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
            } else {
                if (_toTime <= additionalRewardPoolStartTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) 
                    return _toTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDao).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
                return _toTime.sub(_fromTime).mul(aoeaTokenPerSecondForDao).mul(additionalRewardAllocPoint).div(additionalRewardTotalAllocPoint);
            }
        }
...
interface ITreasury {
    ...
    function aoeaTokenPerSecondForDev() external view returns (uint256);

    function aoeaTokenPerSecondForDao() external view returns (uint256);
