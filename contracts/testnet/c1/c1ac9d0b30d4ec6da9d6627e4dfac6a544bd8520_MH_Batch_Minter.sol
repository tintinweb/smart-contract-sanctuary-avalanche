...
// File: hc-mini-game/MH_Batch_Minter.sol
...
contract MH_Batch_Minter is Ownable, ReentrancyGuard {

    address public MH_NFT_Contract_Address = address(0);
        
    function setMHNFTContractAddress(address _MH_NFT_Contract_Address) external onlyOwner {
        MH_NFT_Contract_Address = _MH_NFT_Contract_Address;
    }

    function mint(uint256 count, address to_address ) external onlyOwner {
        require(to_address != address(0), "to_address cannot be 0");

        for (uint256 i = 0; i < count; i++)
        {
            IERC721Mintable(MH_NFT_Contract_Address).mint(to_address);
        }
    }
}