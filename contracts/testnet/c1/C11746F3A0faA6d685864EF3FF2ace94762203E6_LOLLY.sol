...
// File contracts/_/interface/ILOLLY.sol
...
contract LOLLY is ILOLLY, ERC20, Ownable {
    ...
    constructor() ERC20("Night Workers's LOLLY", "LOLLY") { }
     ...
     */
    function mint(address to_, uint amount_) override external onlyController {
        _mint(to_, amount_);
    }
     ...
     */
    function burn(address from_, uint amount_) override external onlyController {
        _burn(from_, amount_);
    }

    /**
     * enables an address to mint / burn
     * @param controller_ the address to enable
     */
    function addController(address controller_) external onlyOwner {
        _controllers[controller_] = true;
    }

    /**
     * disable an address to mint / burn
     * @param controller_ the address to disable
     */
    function removeController(address controller_) external onlyOwner {
        _controllers[controller_] = false;
    }

    /**
     * Use ERC20's balanceOf implementation
     */
    function balanceOf(address account_) override(ERC20, ILOLLY) public view returns (uint) {
    ...
    modifier onlyController {
        require(_controllers[_msgSender()], "operation restricted to controllers");
        _;
    }
}