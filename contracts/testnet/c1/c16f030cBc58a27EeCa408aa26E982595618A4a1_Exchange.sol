...
import "./structure/RoyaltyItem.sol";
...
contract Exchange is Ownable, IExchange {
     ...
     * @param collectionAddress collection address
     * @param fromTokenID from token id
     * @param supply supply
    ...
    function offer(
        ...
        require(INFT(collectionAddress).ownerOf(tokenId) == owner, "Owner is not token owner.");
    ...
    function acceptOffer(
        ...
        require(INFT(collectionAddress).ownerOf(tokenId) == msg.sender, "Owner is not token owner.");
        require(IERC20(erc20Address).allowance(buyer, address(this)) >= offerPrice, "Offer price is not approved.");
        require(INFT(collectionAddress).getApproved(tokenId) == address(this), "NFT token is not approved.");
        ...
        for (uint256 i = 0; i < offers[msg.sender][collectionAddress][tokenId].length; i++) {
            if (
                ...
                uint256 royaltyFee = offerPrice.sub(feeAmount).mul(INFT(collectionAddress).getRoyaltyPercent()).div(100);
                uint256 amountToOwner = offerPrice.sub(feeAmount).sub(royaltyFee);

                INFT(collectionAddress).safeTransferFrom(msg.sender, buyer, tokenId);
                ...
                for (uint256 j = 0; j < INFT(collectionAddress).getRoyaltyCnt(); j++) {
                    uint256 rFee = royaltyFee.mul(INFT(collectionAddress).getRoyalty(j).percentage).div(100);
                    IERC20(erc20Address).transferFrom(buyer, INFT(collectionAddress).getRoyalty(j).receiver, rFee);
                }
    ...
    function list(
        ...
        sellOrders[msg.sender][collectionAddress][tokenId].price = price;
        ...
        sellOrders[msg.sender][collectionAddress][tokenId].baseCurrency = baseCurrency;
    ...
    function buyWithStableCoin(
        ...
        uint256 listPrice = sellOrders[owner][collectionAddress][tokenId].price;
        if (sellOrders[owner][collectionAddress][tokenId].baseCurrency == ListBaseCurrency.AVAX) {
            listPrice = erc20Price.getAVAXPrice().mul(listPrice).div(10 ** 18);
        }

        require(
            listPrice.mul(100 * 100 - slippage).div(100 * 100) <= usdtAmount,
            "Price is not in range."
        );

        uint256 feeAmount = msg.value.mul(serviceFee).div(100 * 100);
        uint256 royaltyFee = msg.value.sub(feeAmount).mul(INFT(collectionAddress).getRoyaltyPercent()).div(100);
        uint256 ownerAmount = msg.value.sub(serviceFee).sub(royaltyFee);
        
        INFT(collectionAddress).safeTransferFrom(owner, msg.sender, tokenId);
        payable(owner).transfer(msg.value.sub(ownerAmount));

        for (uint256 j = 0; j < INFT(collectionAddress).getRoyaltyCnt(); j++) {
            uint256 rFee = royaltyFee.mul(INFT(collectionAddress).getRoyalty(j).percentage).div(100);
            payable(INFT(collectionAddress).getRoyalty(j).receiver).transfer(rFee);
        }
    ...
    function buy(
        ...
        uint256 royaltyFee = amount.sub(feeAmount).mul(INFT(collectionAddress).getRoyaltyPercent()).div(100);
        uint256 ownerAmount = amount.sub(serviceFee).sub(royaltyFee);
        ...
        for (uint256 j = 0; j < INFT(collectionAddress).getRoyaltyCnt(); j++) {
            uint256 rFee = royaltyFee.mul(INFT(collectionAddress).getRoyalty(j).percentage).div(100);
            IERC20(erc20Address).transferFrom(msg.sender, INFT(collectionAddress).getRoyalty(j).receiver, rFee);
        }
    ...
    function exchange(
        ...
        for (uint256 i = 0; i < sellOrders[msg.sender][collectionAddress][tokenId].bidOrders.length; i++) {
            if (
                ...
                uint256 royaltyFee = sellOrders[msg.sender][collectionAddress][tokenId].bidOrders[i].bidAmount.sub(feeAmount).mul(INFT(collectionAddress).getRoyaltyPercent()).div(100);
                uint256 amountToOwner = sellOrders[msg.sender][collectionAddress][tokenId].bidOrders[i].bidAmount.sub(feeAmount).sub(royaltyFee);
    ...
    function emitNFTMintEvent(
        ...
        emit NFTMint(msg.sender, fromTokenID, supply, owner);
    }
...
interface INFT is IERC165 {
    ...
    function getRoyalty(uint256 index) external view returns(RoyaltyItem memory);

    function getRoyaltyCnt() external view returns (uint256);

    function getRoyaltyPercent() external view returns (uint256);
}
...
interface IExchange {
  ...
  /**
     ...
     * @param owner owner
     */
    function emitNFTMintEvent(uint256 fromTokenID, uint256 supply, address owner) external;
