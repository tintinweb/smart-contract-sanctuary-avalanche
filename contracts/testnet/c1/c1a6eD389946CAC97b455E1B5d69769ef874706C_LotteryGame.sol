...
{  
    ...
    function setDateTimeAddress(address dateTime_) external onlyOwner {
        if(dateTime_ == address(0)) revert ZeroAddress();
    ...
    function setLotteryToken(address _lotteryToken) external onlyOwner {
        if(_lotteryToken == address(0)) revert ZeroAddress();
    ...
    function setParticipationFee(uint256 _participationFee) public onlyOwner {
        if(_participationFee == 0) revert ZeroAmount();
    ...
    function setGameCallerFeePercents(uint256 _amount) public onlyOwner {
        if(_amount > FIFTY_PERCENT_WITH_PRECISONS) revert InvalidCallerFee();
    ...
    function switchTimelockToDateDuration(uint256 _gameDuration)
        ...
        onlyOwner
    {
        if(!isDateTimeRequired) revert InvalidTimelock();
    ...
    function setWinnersNumber(
        ...
        if(totalPercentsSum != HUNDRED_PERCENT_WITH_PRECISONS) revert IncorrectPercentsSum();
        ...
        nonReentrant
    {
        ...
        if (lastLotteryGame.id > 0) {
            if (isDateTimeRequired) {
                ...
                if(block.timestamp < timestamp + gameDuration) revert GameNotReadyToStart();
            }
        }

        _swapAvaxToLink(address(this));
    ...
    function _isTimelockValid(uint256 gameStartedAt) private view returns (bool) {
