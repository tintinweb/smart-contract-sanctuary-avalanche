...
contract CollectionManager is CollectionEnumerable {
     ...
     * Create Collection
     *
     * @param nftType - 0: ERC721, 1: ERC1155
     * @param name - ex: "First My Collection"
     * @param symbol - ex: "FMC"
     * @param uri - ex: "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/"
     * @param thorContractAddress - address of thorNode Collection
     *
     * returns null
     */
    function createCollection(
        ...
        if (nftType == uint8(NftType.ERC721)) {
            MockERC721 contractAddress = new MockERC721(name, symbol, uri);
        ...
        emit CollectionCreated(collectionId, nftType);
     ...
     * returns null
     */
    function mint(uint256 collectionId, uint256 amountForErc1155)
        external
        onlyOwnerOf(collectionId)
    {
        require(collectionId < collectionIds.current());
        ...
        if (nftCollection.nftType == uint8(NftType.ERC721)) {
            ...
            mockERC1155.mint(msg.sender, amountForErc1155);
        }
    }

    function fetchByCollectionId(uint256 collectionId)
        public
        view
    ...
    function fetchMyCollections() public view returns (NftCollection[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i]._owner == msg.sender) {
                currentIndex += 1;
            }
        }
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i]._owner == msg.sender) {
                items[currentIndex] = idToNftCollection[i];
                currentIndex += 1;
            }
        }
        return items;
    ...
    function fetchCollectionsByOwner(address target)
        public
        view
        returns (NftCollection[] memory)
    {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToNftCollection[i]._owner == target) {
                currentIndex += 1;
            }
        }
     ...
     * update metadata
     */
    function updateName(uint256 collectionId, string memory name)
        external
        onlyOwnerOf(collectionId)
    {
        require(collectionId < collectionIds.current());
    ...
    function updateSymbol(uint256 collectionId, string memory symbol)
        external
        onlyOwnerOf(collectionId)
    {
        require(collectionId < collectionIds.current());
    ...
    function updateTokenUri(uint256 collectionId, string memory tokenuri)
        external
        onlyOwnerOf(collectionId)
    {
        require(collectionId < collectionIds.current());
