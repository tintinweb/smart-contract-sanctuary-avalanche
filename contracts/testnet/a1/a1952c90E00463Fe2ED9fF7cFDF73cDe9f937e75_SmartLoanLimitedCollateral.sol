...
contract Pool is OwnableUpgradeable, ReentrancyGuardUpgradeable, IERC20 {
  ...
  /**
   * Sets the new deposit index contract.
   * Only the owner of the Contract can execute this function.
   * @dev depositIndex_ the address of deposit index
   **/
  function setDepositIndex(CompoundingIndex depositIndex_) external onlyOwner {
    require(AddressUpgradeable.isContract(address(depositIndex_)), "Must be a contract");
    _depositIndex = depositIndex_;
    _updateRates();
  }

  /**
  * Sets the new borrow index contract.
  * Only the owner of the Contract can execute this function.
  * @dev borrowIndex_ the address of borrow index
   **/
  function setBorrowIndex(CompoundingIndex borrowIndex_) external onlyOwner {
    require(AddressUpgradeable.isContract(address(borrowIndex_)), "Must be a contract");
    _borrowIndex = borrowIndex_;
    _updateRates();
  }
...
contract SmartLoanProperties {
  ...
  function getPool() public virtual view returns (Pool) {
    return Pool(0xF1d0410b1A9fB7eC22bb3fE3358F8f02A0966525);
  }
