...
contract ZOAStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address stakingToken, uint256 rewardPerBlock) external initializer {
        __Ownable_init();
        __ReentrancyGuard_init();

        startBlock = 0;
        rewardReserve = new Reserve();
        WithdrawFeeInterval[] memory _withdrawFee = new WithdrawFeeInterval[](4);
        _withdrawFee[0] = WithdrawFeeInterval(5 minutes, 300);
        _withdrawFee[1] = WithdrawFeeInterval(15 minutes, 150);
        _withdrawFee[2] = WithdrawFeeInterval(30 minutes, 100);
        _withdrawFee[3] = WithdrawFeeInterval(90 minutes, 50);
        add(rewardPerBlock, IERC20Upgradeable(stakingToken), IERC20Upgradeable(stakingToken), 0, 0, 0, _withdrawFee);
    ...
    function add(
        ...
        require(address(_stakedToken) != address(0), "ZOAStaking: Invalid Staked token address");
        require(address(_rewardToken) != address(0), "ZOAStaking: Invalid Reward token address");
        require(poolInfo.length <= 1000, "ZOAStaking: Pool Length Full!");
        require(!poolExists[address(_stakedToken)][address(_rewardToken)], "ZOAStaking: Pool Already Exists!");
        require(_depositFeeBP <= 10000, "ZOAStaking: invalid deposit fee basis points");
        require(_harvestInterval <= MAXIMUM_HARVEST_INTERVAL, "ZOAStaking: invalid harvest interval");
    ...
    function setWithdrawFeeInterval(uint256 poolId, uint256 index, WithdrawFeeInterval memory _withdrawFeeInterval) external onlyOwner {
        if(index < withdrawFee[poolId].length)
            withdrawFee[poolId][index] = _withdrawFeeInterval;
        else
            withdrawFee[poolId].push(_withdrawFeeInterval);
    }
    ...
    function depositRewardToken(uint256 poolId, uint256 amount) external nonReentrant {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        require(!paused, "ZOAStaking: Paused!");
        PoolInfo storage pool = poolInfo[_pid];
        require(!pool.lockDeposit, "ZOAStaking: Deposit Locked!");
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        payOrLockupPendingToken(_pid);
        if (_amount > 0) {
            require(_amount >= poolInfo[_pid].minDeposit, "ZOAStaking: Not Enough Required Staking Tokens!");
    ...
    function compound(uint256 _pid) external nonReentrant {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);
                rewardReserve.safeTransfer(pool.rewardToken, address(this), totalRewards);
                rewardDistributions[_pid] = rewardDistributions[_pid].add(totalRewards);

                // re-deposit
                user.amount = user.amount.add(totalRewards);
                pool.stakedAmount = pool.stakedAmount.add(totalRewards);

                user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
                emit Compound(msg.sender, _pid, totalRewards);
            }
        } else {
            revert("ZOAStaking: can't compound");
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "ZOAStaking: withdraw not good");
        updatePool(_pid);
        payOrLockupPendingToken(_pid);
    ...
    function emergencyAdminWithdraw(uint256 _pid) external onlyOwner {
        ...
        require(balanceToWithdraw != 0, "ZOAStaking: Not enough balance to withdraw!");
    ...
    function setStopFarming(bool stopFarming_) external onlyOwner {
        stopFarming = stopFarming_;
    }
}
