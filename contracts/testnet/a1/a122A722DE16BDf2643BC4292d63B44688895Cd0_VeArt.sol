...
// import "../SalvorNFTCollection/ISalvorNFTCollection.sol";
// import "../SalvorNFTCollection/LibStructSalvor.sol";
...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    struct UserSalvorMiniBoostInfo {
        ...
        // every rarity level gives %1
        uint256 totalRarityLevel;
    }

    uint256 rarityLevelMultiplier;

    struct UserInfo {
        uint256 amount; // art staked by user
        uint256 lastRelease; // time of last veArt claim or first deposit if user has not claimed yet
        uint256 rewardDebt;
    ...
    // ISalvorNFTCollection public nftCollection;
    ...
    uint256 private constant ACC_ART_REWARD_PRECISION = 1e18;
    ...
    event RarityLevelMultiplierUpdated(uint256 multiplier);
    event SalvorMiniSet(address salvorMini);
    event GenerationRateUpdated(uint256 rate);
    ...
    function initialize(IERC20Upgradeable _art) public initializer {
        ...
        // set veARTgenerationRate (veArt per sec per art staked) old value: 3888888888888
        _name = "veART";
		_symbol = "VART";
        ...
        // set art
        art = _art;
        ACC_REWARD_PER_SHARE_PRECISION = 1e24; // 25 char
    }
    ...
    function setSalvorMini(address _salvorMini) external onlyOwner {
        salvorMiniCollection = ISalvorMini(_salvorMini);
        emit SalvorMiniSet(_salvorMini);
    }

    function setRarityLevelMultiplier(uint256 _multiplier) external onlyOwner {
        rarityLevelMultiplier = _multiplier;
        emit RarityLevelMultiplierUpdated(_multiplier);
    }
    ...
    /// @param _veARTgenerationRate the new max ratio
    function setGenerationRate(uint256 _veARTgenerationRate) external onlyOwner {
        veARTgenerationRate = _veARTgenerationRate;
        emit GenerationRateUpdated(_veARTgenerationRate);
    }

    /// @notice sets generation rate
    /// @param _rewardArtGenerationRate the new max ratio
    ...
    */
    ...
    function getBoostedGenerationRate(address _addr) external view returns (uint256) {
        uint256 rarityPercentage = userSalvorMiniBoostInfos[_addr].totalRarityLevel.mul(rarityLevelMultiplier).mul(100);
        return veARTgenerationRate.mul(10000 + rarityPercentage).div(10000);
    }

    function getBoostedIncreasePercentage(address _addr) external view returns (uint256) {
        return userSalvorMiniBoostInfos[_addr].totalRarityLevel.mul(rarityLevelMultiplier).mul(100);
    }

    /// @notice deposits art into contract
    /// @param _amount the amount of art to deposit
    function depositART(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        uint256 userArtBalance = art.balanceOf(msg.sender);
        require(userArtBalance >= _amount, "insufficient balance");
    ...
    function burnSalvorMiniToBoostVeART(uint256 _tokenId) external whenNotPaused nonReentrant {
        require(salvorMiniCollection.ownerOf(_tokenId) == msg.sender, "The provided NFT does not belong to the sender");
        require(users[msg.sender].amount > 0, "insufficient art amount");
        ...
        userSalvorMiniBoostInfos[msg.sender].totalBurnedSalvorMiniCount += 1;
        uint256 rarityLevel = salvorMiniCollection.getRarityLevel(_tokenId);
        userSalvorMiniBoostInfos[msg.sender].totalRarityLevel += rarityLevel;

        emit BurnSalvorMini(msg.sender, _tokenId, rarityLevel);
    }

    function withdrawAllART() external nonReentrant whenNotPaused {
        require(users[msg.sender].amount > 0, "amount to withdraw cannot be zero");
        _withdrawART(users[msg.sender].amount);
    }
    ...
    function withdrawART(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        _withdrawART(_amount);
    }

    /// @notice claims accumulated veART
    function harvestVeART() external nonReentrant whenNotPaused {
    ...
    function claimEarnings() external nonReentrant whenNotPaused {
        ...
        uint256 _pending = userVeArtBalance
        .mul(accRewardPerShare)
        .div(ACC_REWARD_PER_SHARE_PRECISION)
        .sub(users[msg.sender].rewardDebt);

        uint256 _pendingArtReward = userVeArtBalance
        .mul(accARTPerShare)
        .div(ACC_ART_REWARD_PRECISION)
        .sub(users[msg.sender].artRewardDebt);

        if (_pending > 0) {
            users[msg.sender].rewardDebt = userVeArtBalance.mul(accRewardPerShare).div(ACC_REWARD_PER_SHARE_PRECISION);
            _claimEarnings(msg.sender, _pending);
            emit ClaimReward(msg.sender, _pending);
        }
        if (_pendingArtReward > 0) {
            users[msg.sender].artRewardDebt = userVeArtBalance.mul(accARTPerShare).div(ACC_ART_REWARD_PRECISION);
            _claimArtEarnings(msg.sender, _pendingArtReward);
            emit ClaimArtReward(msg.sender, _pendingArtReward);
        }
    }
    ...
    function claimableVeART(address _addr) public view returns (uint256) {
        ...
        uint256 rarityPercentage = userSalvorMiniBoostInfos[_addr].totalRarityLevel.mul(rarityLevelMultiplier).mul(100);
        uint256 boostedGenerationRate = veARTgenerationRate.mul(10000 + rarityPercentage).div(10000);

        uint256 pending = _wmul(user.amount, secondsElapsed * boostedGenerationRate);
    ...
    * @notice View function to see pending reward token on frontend
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        uint256 _totalVeArt = this.totalSupply();
        ...
        if (_rewardBalance != lastRewardBalance && _totalVeArt != 0) {
            ...
            _accRewardTokenPerShare = _accRewardTokenPerShare.add(
                _accruedReward.mul(ACC_REWARD_PER_SHARE_PRECISION).div(_totalVeArt)
            );
        }
        uint256 currentBalance = this.balanceOf(_user);
        return currentBalance.mul(_accRewardTokenPerShare).div(ACC_REWARD_PER_SHARE_PRECISION).sub(user.rewardDebt);
    }
    ...
    */
    function pendingArtRewards(address _user) external view returns (uint256) {
        UserInfo memory user = users[_user];
        uint256 _userVeART = this.balanceOf(_user);
        uint256 _totalVeART = this.totalSupply();
        if (_userVeART > 0) {
            uint256 secondsElapsed = block.timestamp.sub(lastRewardTimestamp);
            uint256 artReward = secondsElapsed.mul(rewardArtGenerationRate);
            uint256 _accARTPerShare = accARTPerShare.add((artReward.mul(ACC_ART_REWARD_PRECISION).div(_totalVeART)));
            return _userVeART.mul(_accARTPerShare).div(ACC_ART_REWARD_PRECISION).sub(user.artRewardDebt);
        }
        return 0;
    }

    function _withdrawART(uint256 _amount) internal {
        UserInfo storage user = users[msg.sender];
        // reset last Release timestamp
        users[msg.sender].lastRelease = block.timestamp;

        // update his balance before burning or sending back art
        users[msg.sender].amount -= _amount;
        totalStakedArtAmount -= _amount;
        userSalvorMiniBoostInfos[msg.sender].totalBurnedSalvorMiniCount = 0;
        userSalvorMiniBoostInfos[msg.sender].totalRarityLevel = 0;

        // get user veART balance that must be burned
        uint256 userVeArtBalance = balanceOf(msg.sender);
        ...
        if (userVeArtBalance > 0) {
            ...
            .mul(accARTPerShare)
            .div(ACC_ART_REWARD_PRECISION)
            .sub(user.artRewardDebt);

            user.rewardDebt = 0;
            user.artRewardDebt = 0;
        ...
        _burn(msg.sender, userVeArtBalance);

        // send back the staked art
        art.safeTransfer(msg.sender, _amount);
        emit WithdrawArt(msg.sender, _amount);
    }
    ...
    */
    function _updateArtReward() internal {
        ...
        if (block.timestamp > lastRewardTimestamp && _totalVeART > 0) {
            ...
            uint256 secondsElapsed = block.timestamp.sub(lastRewardTimestamp);
            uint256 artReward = secondsElapsed.mul(rewardArtGenerationRate);
            accARTPerShare = accARTPerShare.add((artReward.mul(ACC_ART_REWARD_PRECISION).div(_totalVeART)));
        }
    ...
    function _depositVeArt(address _user, uint256 _amount) internal {
        ...
        uint256 _previousAmount = this.balanceOf(_user);
        uint256 _newAmount = _previousAmount.add(_amount);
        ...
        user.artRewardDebt = _newAmount.mul(accARTPerShare).div(ACC_REWARD_PER_SHARE_PRECISION);
        ...
        if (_previousAmount != 0) {
            ...
            uint256 _pendingArtReward = _previousAmount
            .mul(accARTPerShare)
            .div(ACC_ART_REWARD_PRECISION)
            .sub(_previousArtRewardDebt);
            if (_pendingArtReward != 0) {
    ...
    * @param _amount The amount to send to `_to`
    */
    function _claimArtEarnings(address _to, uint256 _amount) internal {
        uint256 totalArtBalance = (art.balanceOf(address(this))).sub(totalStakedArtAmount);
        require(totalArtBalance >= _amount, 'insufficient balance');
