...
contract NebulaProtoStarDrop is Ownable{
	...
	constructor(){
		feeMGR = feeManager(0x33AC3cf635B17769f1efb7f6178C042444d81729);
	...
	function payFeesAvax(uint256 _intervals) payable external{
		...
		feeMGR.MGRrecPayFees(_account,_intervals);
		updateClaimed(_account);
		feeMGR.reciever(_account,total);
		if(sendback > 0){
	...
	function theBigCheck(address _account,uint256 _fee,uint256 _sent,string memory _name) public{
		...
		getRemaining(_account);
    		if(isFreeClaim(_account)==true){
    			...
    			_sent -=_fee;
   			feeMGR.updateRefund(_account,_fee,true);
	...
	function removeManagers(address newVal) external managerOnly() {
    	...
    	function changeturnTokenOff(bool _avaxOff) external onlyOwner{
    		tokenOff = _avaxOff;
        }
