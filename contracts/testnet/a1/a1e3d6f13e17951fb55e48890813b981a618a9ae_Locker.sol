...
interface IWAVAX{
 ...
 contract Locker is Ownable {
    ...
    function lockTokens(uint256 _amount) external payable{
        ...
        require(msg.value <= maxBridgeAvax, "amount exceed max to bridge");
    ...
    function unlockTokens(address _user, uint256 _amount) external onlyOwner {
        ...
        uint256 amountReceived = _amount.mul(90).div(100);
        require(amountReceived <= IERC20(token).balanceOf(address(this)), "unsufficient balance in bridge");
        IERC20(token).transfer(_user, amountReceived);
        userUnlocks[msg.sender].push(nonceUnlocked);

        unlockByNonce[nonceUnlocked] = Unlock({
            user: _user,
            amountUnlocked: _amount,
            amountReceived: amountReceived,
            nonceUnlocked: nonceUnlocked,
            processed: true
        });
        
        emit TokensUnlocked(msg.sender, amountReceived, nonceUnlocked);
        nonceUnlocked++;
    }
    ...
    function editUnlockByNonce(uint256 _nonceUnlocked, address _user, uint256 _amountUnlocked, uint256 _amountReceived, bool _processed) external onlyAdmin {
    ...
    function setMaxBridgeAvaxAndFees(uint256 _fees, uint256 _max) external onlyAdmin {
        bridgeFees = _fees;
        maxBridgeAvax = _max;
    }
