...
contract YieldFarmNeutralVault is ERC20, AccessControl, ReentrancyGuard, Pausable {
  ...
  function deposit(uint256 _depositAmt, uint256 _minSharesReceive) external nonReentrant {
    ...
    ERC20(tokenB).safeTransferFrom(msg.sender, address(this), _depositAmt);
    ERC20(tokenB).safeTransfer(manager, _depositAmt);

    // uint256 _depositValue = priceOracle.getAmountsOut(_depositAmt, tokenA, tokenB, lpToken);
    uint256 _equityValue = getEquityValue();

    uint256 _targetLpValue = (_equityValue + _depositAmt)
                             * config.getTargetLeverage()
                             / SAFE_MULTIPLIER;

    uint256 _targetDebtValue = (_equityValue + _depositAmt)
                               * (config.getTargetLeverage() - 1e18)
                               / SAFE_MULTIPLIER;

    (uint256 borrowA, uint256 borrowB, uint256 repayA,  uint256 repayB) = _calculate(_targetDebtValue);

    _execute(0, _depositAmt, _targetLpValue, borrowA, borrowB, repayA, repayB);
  ...
  function _checkAndTransfer(
    ...
    if (_withdrawAmt < _minTokenAReceive) {
      revert InsufficientTokenAReceived(tokenB, _minTokenAReceive, _withdrawAmt);
    }
