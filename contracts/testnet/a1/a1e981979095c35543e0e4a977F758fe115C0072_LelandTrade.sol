...
contract LelandTrade is ERC721Holder, Ownable, ILelandTrade {
    ...
    function setRoot(bytes32 _root) external override onlyOwner {
        merkleRoot = _root;
    }
    ...
    function upgradeCollection(
        ...
        emit CollectionUpgraded(_collections, _duplicateMode);
    }
    ...
    function _checkCollection(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (_duplicateMode) {
                ...
                uint16 cardId = info.cardNo;
                for (uint256 j = i + 1; j < length; j++) {
                    require(
                        cardId != _collections[j].cardNo,
            ...
            IERC721(lelandNFT).safeTransferFrom(msg.sender, DEAD, info.tokenId);
        }
    ...
    function _verifyTokenInfo(
        ...
        bytes32 leaf = keccak256(abi.encode(_tokenId, _rarityId, _cardNo));
