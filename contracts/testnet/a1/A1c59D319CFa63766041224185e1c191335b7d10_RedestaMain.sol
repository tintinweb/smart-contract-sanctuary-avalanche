...
contract RedestaMain  {
    ...
    mapping(address => uint[]) public addrToLikedCommentID;
    mapping(address => uint[]) public addrToDislikedCommentID;
    mapping(address => Profile) public profiles;
    mapping(address => mapping(address => bool)) addrOwnedNFTbool;
    mapping(uint => Projects[]) public categoryToProjects;
    mapping(address => Projects[]) public userToProjects;
    ...
    function addComment(string memory _comment, uint _id) public {
        ...
        idToComments[_id].push(Comments(_id, commentID, msg.sender, _comment, 0, 0, index));
        idToProject[_id].commentCount++;
    }
    ...
    function isLiked(uint _comID) private view returns(bool) {
        for(uint i = 0; i<addrToLikedCommentID[msg.sender].length; i++){
            if(addrToLikedCommentID[msg.sender][i] == _comID){
                return true;
            }
        }
        return false;
    ...
    function isDisliked(uint _comID) private view returns(bool) {
        for(uint i = 0; i<addrToDislikedCommentID[msg.sender].length; i++){
            if(addrToDislikedCommentID[msg.sender][i] == _comID){
                return true;
            }
        }
        return false;
    ...
    function likeComment(uint _comID, uint _projectID, uint _comIndex) public {
        if(isLiked(_comID)){
            revert("You've already liked this post.");
        }
    ...
    function dislikeComment(uint _comID, uint _projectID, uint _comIndex) public {
        if(isDisliked(_comID)){
            revert("You've already disliked this post.");
        }
    ...
    function donateProject(uint _projectID, uint amount) public {
        require(ourToken.balanceOf(msg.sender) >= amount * decimals);
        ourToken.approve(msg.sender, amount * decimals);
        ourToken.transferFrom(msg.sender, address(this), amount * decimals);
        idToProject[_projectID].status.current += ourToken.balanceOf(msg.sender);
        profiles[msg.sender].donateAmount += amount * decimals;
        
        //Miktar threshhold'dan büyük mü ve alınmış nft sayısı maxsupply'dan küçük mü
        //if(amount >= idToProject[_projectID].nftinfo.nftThreshold &&  idToProject[_projectID].nftinfo.owned < idToProject[_projectID].nftinfo.maxSupply)

        //eğer kullanıcı bu projenin nftsine sahipse
        //if(addrOwnedNFTbool[msg.sender][_projectID] == true)


    }

    function getProjectsByCategory(uint _category) public view returns(Projects[] memory){
        return getProjectsByCategory(_category);
    }

    function getUsersProjects(address _address) public view returns(Projects[] memory){
        return userToProjects[_address];
    }
...
contract RedestaToken is ERC20 {
    ...
    function mintRDS(uint amount) public {
        DAIContract.transferFrom(msg.sender, address(this), amount * 10**18);
