    ...
    AccessControlUpgradeable
{
    ...
    function payMaintenanceFee(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            maintFee[_tokenIds[i]].feeDue = maintFee[_tokenIds[i]].feeDue ==
                uint64(0) || maintFee[_tokenIds[i]].feeDue <= uint64(block.timestamp)
                ? uint64((block.timestamp).add(_paidDays[i].mul(3 minutes)))
                : maintFee[_tokenIds[i]].feeDue +
                    uint64(_paidDays[i].mul(3 minutes));
            maintFee[_tokenIds[i]].unClaimedDays += _paidDays[i];
            if (_type == 12) {
                if (
                    !isYieldBoxInArray(
        ...
        returns (uint256 paidDais)
    {
		...
		function getFirstDay(uint256 _tokenIds) public view returns (uint64 timestamp) {
			if (
                (maintFee[_tokenIds].firstDay == uint64(0)) ||
                (maintFee[_tokenIds].unClaimedDays ==
                    Rewards.RewardsClaimed(_tokenIds).claimedDays)
            ) {
                timestamp= uint64(block.timestamp);
            } else {
                uint256 Days = maintFee[_tokenIds].unClaimedDays >
                    Rewards.RewardsClaimed(_tokenIds).claimedDays
                    ? (maintFee[_tokenIds].unClaimedDays -
                        Rewards.RewardsClaimed(_tokenIds).claimedDays).mul(
                            3 minutes
                        )
                    : 0;
                timestamp = uint64(
