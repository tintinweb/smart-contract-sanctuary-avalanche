...
contract SyntheticPoolV2 is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    //     slippageTokenPrice = _synTokenPrice + (_synTokenPrice * pool.slippage / BIPS_DIVISOR);
    //     syntheticTokenMintAmount = systemCoinBuyingAmount * (10 ** synTokenDecimal) / slippageTokenPrice;
    //     require(syntheticTokenMintAmount > 0, "<0");
    //     estSystemCoinAmount = syntheticTokenMintAmount * slippageTokenPrice / (10 ** synTokenDecimal);
    //     _openBuyOrder(pid, msg.sender, estSystemCoinAmount, syntheticTokenMintAmount, slippageTokenPrice);
    //     _mintSynToken(pid, poolOrderLength[pid]-1);
    // }
    ...
    //     _openSellOrder(pid, msg.sender, estSystemCoinAmount, synTokenSellingAmount, slippageTokenPrice);
    //     _burnSynToken(pid, poolOrderLength[pid]-1);
    // }

    function openLimitOrder(uint256 pid, uint8 orderType, uint256 synTokenTradingAmount, uint256 synTokenPrice) whenNotPaused external {
        require(orderType == 0 || orderType == 1, "WrongOT");
        require(synTokenTradingAmount > 0 && synTokenPrice > 0);
        ...
        if(synTokenPriceDecimal == systemCoinDecimal) {
            _synTokenPrice = synTokenPrice;
        } else if(synTokenPriceDecimal > systemCoinDecimal) {
            _synTokenPrice = synTokenPrice / (10 ** (synTokenPriceDecimal - systemCoinDecimal));
        } else {
            _synTokenPrice = synTokenPrice * (10 ** (systemCoinDecimal - synTokenPriceDecimal));
        }
        
        estSystemCoinAmount = (synTokenTradingAmount * _synTokenPrice) / (10 ** (synTokenDecimal + synTokenPriceDecimal - systemCoinDecimal));
        
        if(orderType == 0) {
            _openBuyOrder(pid, msg.sender, estSystemCoinAmount, synTokenTradingAmount, _synTokenPrice);
        } else if(orderType ==1) {
            _openSellOrder(pid, msg.sender, estSystemCoinAmount, synTokenTradingAmount, _synTokenPrice);
        }
    }
    ...
    function _mintSynToken(uint256 pid, uint256 _orderId) internal {
        ...
        emit MintSynToken(pid, msg.sender, order.account, order.systemCoinAmount ,order.synTokenAmount);
    }
    ...
    function _burnSynToken(uint256 pid, uint256 _orderId) internal {
        ...
        emit BurnSynToken(pid, msg.sender, order.account, order.systemCoinAmount, order.synTokenAmount);
    }
