...
contract sMetahub is IsMHUB, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgMHUB public gMHUB; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked MHUB", "sMHUB", 9) ERC20Permit("Staked MHUB") {
    ...
    function setgMHUB(address _gMHUB) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gMHUB) == address(0), "gMHUB:  gMHUB already set");
        require(_gMHUB != address(0), "gMHUB:  gMHUB is not a valid contract");
        gMHUB = IgMHUB(_gMHUB);
    }
    ...
    /**
        @notice increases rMHUB supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sMHUB of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sMHUB
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sMHUB: insufficient balance");
    }
    ...
    // toG converts an sMHUB balance to gMHUB terms. gMHUB is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gMHUB.balanceTo(amount);
    }

    // fromG converts a gMHUB balance to sMHUB terms. sMHUB is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gMHUB.balanceFrom(amount);
    }

    // Staking contract holds excess sMHUB
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gMHUB.balanceFrom(IERC20(address(gMHUB)).totalSupply())).add(
