...
contract CreateProposal {
  ...
  constructor(string memory _title,uint256 _startDate, uint256 _endDate, uint256 _postedDate, string memory _description,string memory _documentURLs, bool _status, address _submittedBy) public {
    prop1 = Proposal(_title,_startDate, _endDate,_postedDate, _description,_documentURLs,_status,_submittedBy);
  }
    ...
    while(i <= whiteLabelAddress.length) {
      if (msg.sender == whiteLabelAddress[i]) {
        if(whiteLabelAddressCreateProposalPermission[whiteLabelAddress[i]]==true) {
          CreateProposal prop = new CreateProposal(_title, _startDate, _endDate, _postedDate, _description,_documentURLs, _status, _submittedBy);
