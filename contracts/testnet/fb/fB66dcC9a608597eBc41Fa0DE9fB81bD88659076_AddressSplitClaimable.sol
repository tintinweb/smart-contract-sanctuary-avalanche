    ...
    Initializable
{
    ...
    // mapping of each address's balance (address, balance)
    mapping(address => uint256) public balancesOfAddresses;

    address[] private addresses;
    uint256[] private amounts;
    uint256 private amountsTotal;
    ...
    function initialize(address[] memory _addresses, uint256[] memory _amounts)
        external
        initializer
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupOwner(msg.sender);
        _setDistrobution(_addresses, _amounts);
    }
    ...
    function claimRewards() public nonReentrant {
        uint256 balance = balancesOfAddresses[msg.sender];
        require(balance > 0, "No rewards to claim");
        payable(msg.sender).transfer(balance);
        balancesOfAddresses[msg.sender] = 0;
    ...
    function _setDistrobution(
        ...
        //update new _amounts from input
    ...
    function _setAddresses(address[] memory _addresses)
        internal
        onlyRole(DEFAULT_ADMIN_ROLE)
    {}
    ...
    function _splitRewards() internal onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(
            ...
            "Addresses and amounts must be set"
        );
        uint256 totalSupply = address(this).balance - payableRewards;
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            uint256 amountPerAddress = rewardPerSingleAmount * amounts[i];
            balancesOfAddresses[addresses[i]] += amountPerAddress;
            payableRewards += amountPerAddress;
        }
        emit RewardsSplit(totalSupply, rewardPerSingleAmount);
    }

    function resetRewardsForToken(address _address)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        balancesOfAddresses[_address] = 0;
    }
