...
contract TheLadder is ERC721AQueryable, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public tokenPrice = 3 ether; // 3 AVAX
    uint256 public allowlistPrice = 2.2 ether; // 2.2 AVAX
    string public baseURIExtended = "ipfs://bafybeih4orj7uotoryxwikcpcndfaafh5lvezoytajtiy4iziwutzhtbbi/";
    string public extension = ".json"; // configurable extension
    bool public saleIsActive = false;
    bool public allowlistActive = false;
    uint96 public tokenRoyalties = 750; // 7% royalty

    address royaltyPayout = 0xB22734474B25dE6F35309B168B637fa03DCaA874; // monkeez deployer

    mapping(address => bool) private _allowlist;

    // CONSTRUCTOR

    constructor() ERC721A("TheLadder", "LDR") {
    ...
    modifier callerIsUser() {
        require(
            ...
            "LADDER: The caller is another contract"
        );
        _;
    }
    ...
    function setAllowlistPrice(uint256 _newPrice) external onlyOwner {
    ...
    function addToAllowlist(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(
                ...
                "LADDER: Can't add the null address"
            );
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(
                addresses[i] != address(0),
                "LADDER: Can't remove the null address"
            );
    ...
    function mintAllowlist(uint256 numberOfTokens)
        external
        payable
        callerIsUser
        nonReentrant
    {
        require(allowlistActive, "LADDER: Allowlist mint is not active");
        require(numberOfTokens > 0, "LADDER: Minimum mint is 1 token");
        require(
            _allowlist[msg.sender],
            "LADDER: You are not on the Monkeez List"
        );
        require(
            totalSupply() + numberOfTokens <= MAX_SUPPLY,
            "LADDER: Purchase would exceed MAX_SUPPLY"
        );
        require(
            numberOfTokens <= MAX_PURCHASABLE,
            "LADDER: Purchase exceeds max allowed"
        );
        require(
            allowlistPrice * numberOfTokens <= msg.value,
            "LADDER: AVAX amount is not sufficient"
        );
    ...
    function mint(uint256 numberOfTokens)
        ...
        nonReentrant
    {
        require(saleIsActive, "LADDER: Sale is not active");
        require(numberOfTokens > 0, "LADDER: Minimum mint is 1 token");
        require(
            numberOfTokens <= MAX_PURCHASABLE,
            "LADDER: Purchase exceeds max allowed"
    ...
    {
        require(_exists(tokenId), "LADDER: Token does not exist");
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "LADDER: Nothing to withdraw");
