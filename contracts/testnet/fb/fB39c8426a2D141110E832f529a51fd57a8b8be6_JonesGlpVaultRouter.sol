...
contract JonesGlpVaultRouter is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, Governable, UUPSUpgradeable {
        ...
        returns (uint256)
    {
        ...
        glpStableVault.transferFrom(msg.sender, address(this), _shares);
    ...
    function _redeemDirectly(uint256 _shares, uint256 _retention) private returns (uint256) {
        ...
        if (stable.balanceOf(address(this)) < remainderStables) {
            revert Errors.NotEnoughStables();
        }
        ...
        if (amountAfterRetention > 0) {
            stable.transfer(msg.sender, amountAfterRetention);
        }

        if (realRetention > 0) {
            stable.transfer(address(glpStableVault), realRetention);
        }
...
interface IJonesGlpLeverageStrategy {
     ...
     */
    function getUnderlyingGlp() external view returns (uint256, uint256, uint256);
