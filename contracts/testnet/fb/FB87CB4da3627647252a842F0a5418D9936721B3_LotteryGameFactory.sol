...
contract Router is ConvertAvax{
    ...
    AutomationRegistryExecutableInterface public immutable keeperRegistry;
    ...
    constructor() ConvertAvax(LINK_TOKEN)  {
        ...
        keeperRegistry = AutomationRegistryExecutableInterface(KEEPERS_REGISTRY);
    }
    ...
    /// @param checkData data passed to the contract when checking for upkeep
    /// @param amount quantity of LINK upkeep is funded with (specified in Juels)
    /// param2 source application sending this request
    /// @param lottery address to perform upkeep on
    function registerAdditionalLottery(
        ...
        linkToken.transferAndCall(
            ...
            abi.encodeWithSelector(
                bytes4(
                    keccak256(
                        "register(string,bytes,address,uint32,address,bytes,bytes,uint96,address)"
                    )
                ),
                name,
                encryptedEmail,
        ...
        if (newNonce == oldNonce + 1) {
            ...
            registeredLotteries[lottery].lotteryRegisteredId = upkeepID;
        } else {
    ...
    function cancelSubscription(address lottery) external {
        (bool success, ) = address(keeperRegistry).call(
            abi.encodeWithSelector(
    ...
    function addFunds(address lottery, uint96 amountKeepers, uint256 amountVRF) external {
        ...
        linkToken.approve(address(keeperRegistry), amount);
...
address constant UPKEEP_REGISTRATION = 0x57A4a13b35d25EE78e084168aBaC5ad360252467;//0xb3532682f7905e06e746314F6b12C1e988B94aDB;
//lottery
