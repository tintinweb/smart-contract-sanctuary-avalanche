...
contract NFTMarketplace {
    ...
    function createCollection(
        ...
        Ownable(address(this)).transferOwnership(msg.sender);
    ...
    function listForSale(
        ...
        require(
            ...
            "not allowed to list for sale this item"
        );
        ...
        _saleItems[nftAddress].push(item);
    }

    function unlistForSale(address nftAddress, uint256 tokenId) external {
        require(
            (IERC721(nftAddress).ownerOf(tokenId) == msg.sender) ||
                nftAddress == msg.sender,
            "not allowed to unlist this item"
        );
        uint256 length = _saleItems[nftAddress].length;
        for (uint256 i = 0; i < length; i++) {
            if (_saleItems[nftAddress][i].tokenId == tokenId) {
                _saleItems[nftAddress][i] = _saleItems[nftAddress][length - 1];
                _saleItems[nftAddress].pop();
                break;
            }
        }
    }

    function getIndexOfItemForSale(address nftAddress, uint256 tokenId)
        private
        view
        returns (uint256)
    {
        uint256 id = 0;
        for (uint256 i = 0; i < _saleItems[nftAddress].length; i++) {
            if (_saleItems[nftAddress][i].tokenId == tokenId) {
                id = i + 1;
                break;
            }
    ...
    function buy(address nftAddress, uint256 tokenId) external payable {
        ...
        uint256 index = getIndexOfItemForSale(nftAddress, tokenId);
        require(index != 0, "cannot find item for sale");
        require(
            msg.value >= _saleItems[nftAddress][index - 1].price,
            "not enough money to buy"
        );
        address currentOwner = IERC721(nftAddress).ownerOf(tokenId);
        IERC721(nftAddress).transferFrom(currentOwner, msg.sender, tokenId);
...
contract NFT is ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint32 public whitelistStartTime = 123;
    uint32 public preSaleStartTime = 456;
    uint32 public publicSaleStartTime = 789;
    ...
    function presaleMint(address _to, uint256 _mintAmount) public payable {
        ...
        require(mintPrice * _mintAmount <= msg.value, "VE: Insufficient AVAX");
    ...
    function setMaxMintSizePerWallet(uint256 _newmaxMintPerWallet)
        external
        onlyOwner
    {
    ...
    function setBaseURIExtension(string memory _newBaseExtension)
        external
        onlyOwner
    {
    ...
    function _beforeTokenTransfer(
        ...
        if (from.isContract()) {
            console.log("this is contract transfer");
            require(
                from == platformContract,
                "Not allowed to buy or sell outside of our platform."
            );
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        IMarketplace(platformContract).unlistForSale(address(this), tokenId);
    }
