...
/// @title A MicroXON contract

/// @notice Use this contract to manage MicroXONs

contract MicroXon is Ownable, Pausable {

    IERC20 public usdcToken;

    address public xetaWallet;
    ...
    mapping(uint8 => uint256) public xonCost;

    mapping(uint8 => uint256) public xonCreationFee;

    mapping(uint8 => uint256) public claimRewardFee;

    mapping(uint8 => uint256) public subscriptionFee;



    uint256 public maxXonsPerUser = 500;

    uint8[] public walletDistribution = [25, 25, 25, 25];



    event ClaimRequestsEvent(address indexed userWallet, bytes24[] nodeIds, uint8[] tokenTypes);
    ...
    event SubscriptionsPaymentEvent(address indexed userWallet, bytes24[] nodeIds, uint8[] tokenTypes, uint8 indexed months);

    event SetXetaWalletEvent(address indexed wallet);

    event SetXonCostEvent(uint8 indexed _type, uint256 indexed cost);

    event SetXonCreationFeeEvent(uint8 indexed _type, uint256 indexed fee);

    event SetSubscriptionFeeEvent(uint8 indexed _type, uint256 indexed fee);

    event SetClaimRewardFeeEvent(uint8 indexed _type, uint256 indexed fee);
    ...
    constructor(address _usdcToken) {
        ...
        xonCreationFee[0] = 15 * (10 ** 6);

        xonCreationFee[1] = 15 * (10 ** 6);



        claimRewardFee[0] = 5 * (10 ** 6);

        claimRewardFee[1] = 5 * (10 ** 6);



        subscriptionFee[0] = 15 * (10 ** 6);

        subscriptionFee[1] = 15 * (10 ** 6);
    ...
    function requestRewards(bytes24[] memory nodeIds, uint8[] memory tokenTypes, uint256 sumClaimRewardFee) external whenNotPaused {

        require(nodeIds.length > 0 && nodeIds.length <= maxXonsPerUser, "Illegal number of XONs");

        require(nodeIds.length == tokenTypes.length, "XON and types array length mismatch");



        uint256 _sumClaimRewardFee = 0;

        for (uint8 i = 0; i < tokenTypes.length; i++) {

            _sumClaimRewardFee += claimRewardFee[tokenTypes[i]];

        }

        require(_sumClaimRewardFee == sumClaimRewardFee, "Invalid claim reward fee");



        if (sumClaimRewardFee > 0) {

            bool res = distributePaymentInUsdc(sumClaimRewardFee);
        ...
        emit ClaimRequestsEvent(msg.sender, nodeIds, tokenTypes);

    }



    /// @notice Use this function to create a XON. A user should pay a create fee and a create cost
    ...
    function mintXon(uint256 amount, uint256 _xonCreationFee, uint8 _type) external whenNotPaused {
        ...
        require(_xonCreationFee == xonCreationFee[_type], "Invalid XON creation fee");

        require(amount > 0, "Illegal amount");

        require(usdcToken.balanceOf(msg.sender) >= (xonCost[_type] + xonCreationFee[_type]) * amount, "Insufficient balance");

        require(usdcToken.allowance(msg.sender, address(this)) >= (xonCost[_type] + xonCreationFee[_type]) * amount, "Insufficient allowance");



        bool res = distributePaymentInUsdc((xonCost[_type] + xonCreationFee[_type]) * amount);
    ...
    function requestSubscriptions(bytes24[] memory nodeIds, uint8[] memory tokenTypes, uint8 months, uint256 sumSubscriptionFee) external whenNotPaused {

        require(nodeIds.length == tokenTypes.length, "XON and types array length mismatch");

        uint256 _sumSubscriptionFee = 0;

        for (uint8 i = 0; i < tokenTypes.length; i++) {

            _sumSubscriptionFee += subscriptionFee[tokenTypes[i]];

        }

        require(_sumSubscriptionFee * months == sumSubscriptionFee, "Invalid subscription fee");

        require(nodeIds.length > 0 && nodeIds.length <= maxXonsPerUser, "Illegal number of nodes");



        if (sumSubscriptionFee > 0) {

            bool res = distributePaymentInUsdc(sumSubscriptionFee);
        ...
        emit SubscriptionsPaymentEvent(msg.sender, nodeIds, tokenTypes, months);
    ...
    /// @notice Sets the XON cost in USDC

    function setXonCost(uint8 _type, uint256 cost) external onlyOwner {

        xonCost[_type] = cost;

        emit SetXonCostEvent(_type, cost);
    ...
    function setXonCreationFee(uint8 _type, uint256 fee) external onlyOwner {

        xonCreationFee[_type] = fee;

        emit SetXonCreationFeeEvent(_type, fee);
    ...
    function setSubscriptionFee(uint8 _type, uint256 cost) external onlyOwner {

        subscriptionFee[_type] = cost;

        emit SetSubscriptionFeeEvent(_type, cost);
    ...
    function setClaimRewardFee(uint8 _type, uint256 fee) external onlyXetaOrOwner {

        claimRewardFee[_type] = fee;

        emit SetClaimRewardFeeEvent(_type, fee);
    ...
    /// @notice Returns XON cost and all fees

    /// @param _type XON type

    function getTypeDetails(uint8 _type) external view returns(uint[4] memory) {

        return [

            xonCost[_type],

            xonCreationFee[_type],

            claimRewardFee[_type],

            subscriptionFee[_type]
