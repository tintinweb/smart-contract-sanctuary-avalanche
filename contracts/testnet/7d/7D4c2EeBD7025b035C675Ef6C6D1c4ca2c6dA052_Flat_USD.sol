           ...
           if(_shouldUpgradePrice()){
            ...
            if(tokenA == address(this)){
              uint  amountOut = router.getAmountOut(_amount, reserve0, reserve1);
               newAmount = (latestUpdatedPrice.mul(reserve1.sub(amountOut))).div(pegDenominator);
            }else{
               uint amountOut = router.getAmountOut(_amount, reserve1, reserve0);
               newAmount = (latestUpdatedPrice.mul(reserve0.sub(amountOut))).div(pegDenominator);       
            }
...
function _buyFixing(uint amount) private {
    ...
    function _upgradePrice() private returns(uint){
        ...
        if(increment){
            ...
            newPrice = latestUpdatedPrice.sub(period.mul(upgradingPerPeriod));
            isOver = newPrice <= targetPrice;
        }
    ...
    function _removeHolder(address _holder) public {
    ...
    function upgradeInflation(uint _inflationRate, uint _periodThreshold, uint _upgradingPeriod) external onlyManager{
        ...
        uint newPrice = pegDenominator.add(newInflationAmount);
        if(newPrice > latestUpdatedPrice){
            uint priceDistance = newPrice.sub(latestUpdatedPrice);
            upgradingPerPeriod = priceDistance.div(pegDivident);
            increment = true;
        }else{
            uint priceDistance = latestUpdatedPrice.sub(newPrice);
            upgradingPerPeriod = priceDistance.div(pegDivident);
            increment = false;
        }
    ...
    function upgradePeg() public {
        ...
        (uint reserve0, uint reserve1, ) = pair.getReserves();
        if(tokenA == address(this)){
             newAmount = (latestUpdatedPrice.mul(reserve1)).div(pegDenominator);
             initialpairBalance = reserve0;
        }else{
             newAmount = (latestUpdatedPrice.mul(reserve0)).div(pegDenominator);
