...
contract GothStake is ERC20("GOTH Bits", "bGOTH"), Ownable, ReentrancyGuard {
    ...
    function mintGOTH () internal 
    {
        ...
        require(_goth.totalSupply() <= _goth.maxSupply().sub(toMint), "mintGOTH: results in max supply overflow");
    ...
    function leave(uint256 _share) public nonReentrant payable
    {
        ...
        //revert("Gate 10");
        uint256 totalShares = totalSupply();
        uint256 toMint = _share.mul(_goth.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        _goth.transfer(msg.sender, toMint);
