...
contract LoadHealersHut is ReentrancyGuard, Ownable {
    ...
    event Heal(
        ...
        string indexed healId
    );
    event Resurrect(
        address indexed buyer,
        uint256 indexed tokenId,
        string indexed ressurectId
    );
    ...
    function heal(
        uint256 healAmount,
        uint256 tokenId,
        string memory healId,
        bytes memory signature,
        string memory nonce
    ) external {
        require(
            matchAddresSigner(
                hashTransaction(msg.sender, healAmount, tokenId, healId, nonce),
                signature
            ),
    ...
    function resurrect(
        uint256 level,
        uint256 tokenId,
        string memory resurrectId,
        bytes memory signature,
        string memory nonce
    ) external {
        require(
            matchAddresSigner(
                hashTransaction(msg.sender, level, tokenId, resurrectId, nonce),
                signature
            ),
