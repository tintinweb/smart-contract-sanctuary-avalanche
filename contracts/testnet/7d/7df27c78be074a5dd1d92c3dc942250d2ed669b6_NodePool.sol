...
contract NodePool is ContractGuard, Initializable {
    ...
    uint256 public constant shareTokenPerSecond = 0.000134766 ether; // 4250 tokens / (365 days * 24h * 60min * 60s)
    uint256 public constant runningTime = 365 days; // 365 days

    uint256 public  constant maxPercentToStake = 21; //21%

    uint256 public constant shareTokenPerSecondForDaoFund = 0.000011891 ether; // 375 tokens / (365 days * 24h * 60min * 60s)
    uint256 public lastDaoFundRewardTime;
    uint256 public constant shareTokenPerSecondForDevFund = 0.000011891 ether; // 375 tokens / (365 days * 24h * 60min * 60s)
    uint256 public lastDevFundRewardTime;
    address[] public usersDeposit;
    mapping(address => bool) private _isDeposit;
    bool public isSendDeadWallet;
    ...
    function __Upgradeable_Init(
        ...
        shareToken = IERC20Upgradeable(_shareToken);
    ...
    function checkPoolDuplicate(IERC20Upgradeable _token, uint256 _lockTime) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            bool isPoolDuplicate = poolInfo[pid].lockTime == _lockTime && poolInfo[pid].token == _token;
    ...
    function add(
        ...
        require(_lockTime > 0, "_lockTime > 0");
        
        checkPoolDuplicate(IERC20Upgradeable(_token), _lockTime);
        massUpdatePools();
    ...
    {   // invalid time range
        ...
        // reward end time greater than pool end time, using pool end time to cacluate
    ...
    function deposit(uint256 _pid, uint256 _amount) external onlyOneBlock {
        ...
        addUserDeposit(_sender);
        uint256 timeDeposit = block.timestamp;
        if (timeDeposit < poolStartTime) {
            timeDeposit = poolStartTime;
        }
        // update share reward
        updatePool(_pid);
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                ...
                user.timeStart = timeDeposit;
                user.lastTimeReward = timeDeposit;
            }
        }
        if (_amount > 0) {
            // transfer token
    ...
    function getMainTokenCirculatingSupply() external view returns (uint256) {
        return mainToken.rebaseSupply();
    }

    function getMaxTokenCanStake() public view returns (uint256) {
        uint256 totalSupply = mainToken.totalSupply();
        return totalSupply.mul(maxPercentToStake).div(3 * 100); // 21% totalSupply / 3 pool
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external onlyOneBlock {
        ...
        updatePool(_pid);
        uint256 timeWithdraw = block.timestamp;
        if (timeWithdraw < poolStartTime) {
            timeWithdraw = poolStartTime;
        }
        ...
        user.lastTimeReward = timeWithdraw;
        user.timeStart = timeWithdraw;
        user.rewardDebt = user.amount.mul(pool.accShareTokenPerShare).div(1e18);
    ...
    function requiredDivisible100(uint256 _amount) internal pure {
        uint256 mod = _amount.mod(100*10**18);
        require(mod == 0, 'Amount must be divisible by 100');
    }

    function addUserDeposit(address account) internal
	{
        if (_isDeposit[account]) return;
		_isDeposit[account] = true;
		usersDeposit.push(account);
	}

    function getUsersDeposit() external view returns (address[] memory)
	{
		return usersDeposit;
	}

    function sendTokenToDeadWallet() external onlyOperator {
        require(!isSendDeadWallet, "only call sendTokenToDeadWallet");
        uint256 _amountToSend = 45000 ether;
        safeShareTokenTransfer(address(0x000000000000000000000000000000000000dEaD), _amountToSend);
        set(0, 1250 ether);
        set(1, 1437.5 ether);
        set(2, 1562.5 ether);
        isSendDeadWallet = true;
    }
}
