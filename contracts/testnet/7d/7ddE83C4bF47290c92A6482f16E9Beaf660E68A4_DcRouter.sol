...
contract DcRouter is  NonBlockingBaseApp, UUPSUpgradeable {
    ...
    *       (https://layerzero.gitbook.io/docs/technical-reference/testnet/testnet-addresses)
    ...
    function initialize(address deCommasTreasurer,
                        ...
                        address currentUSDC,
                        address actionPool,
                        uint16 actionPoolNativeId) public initializer {
        _deCommasTreasurer = deCommasTreasurer;
        lzEndpoint = ILayerZeroEndpointUpgradeable(nativeLZEndpoint);
        _currentUSDCToken = currentUSDC;
        _actionPool = actionPool;
        __Ownable_init();
        __LzAppUpgradeable_init(nativeLZEndpoint);
...
abstract contract NonBlockingBaseApp is NonblockingLzAppUpgradeable{
    ...
    function _setBridge(bytes memory _actionData) internal {
        (address newSgBridge) = abi.decode(_actionData, (address));
    ...
    function _addressToBytes(address _a) internal pure returns (bytes memory b) {
        assembly {
            let m := mload(0x40)
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        _internalLzReceive(_srcChainId, _srcAddress, _nonce, _payload);
    }

    //@notice override this function
    function _internalLzReceive(uint16 _srcChainId,
        bytes memory _srcAddress,
        uint64, /*_nonce*/
