...
_member.feeStatus = true;
 uint256 seriesOneBalance = seriesOne.balanceOf(msg.sender);
        ...
        if (seriesOneBalance > 0 && seriesTwoBalance > 0) {
            require(Fee == msg.value,"Fee aount issue") ;
...
function enterySeriesOne() public  {
        GameMember memory _member = Player[msg.sender];
 uint256 seriesOneBalance = seriesOne.balanceOf(msg.sender);
...
_member.feeStatus = true;
    ...
    */
    function partitionInGame(bool _chooes_side) public after24Hours GameInitialized onlyNftHolder {
        GameMember memory _member = Player[msg.sender];
  ...
  */
    ...
    modifier onlyNftHolder() {
        GameMember memory _member = Player[msg.sender];
        require(_member.feeStatus, "only Nft Holder particpate.");
        _;
    }
}
