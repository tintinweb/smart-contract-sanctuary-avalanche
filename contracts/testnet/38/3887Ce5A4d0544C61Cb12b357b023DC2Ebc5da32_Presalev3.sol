...
contract Presalev3{
    ...
    mapping(address=> uint256) public limitperwalletM;
 ...
 //     uint256 public limitperwallet=827000000000000000000;
  ...
  function unpause() onlyowner whenPaused public {
    ...
    function depostiToTorrent(address pre, address _addr, address _upline, uint256 _amount) internal {
        dc.depositFromPresale(pre, _addr, _upline, _amount);
    }

    address public upline;
    
    function setUpline(address newUpline) public onlyowner {
        upline = newUpline;
    }
    
    function addlimitperwalletM(address addr, uint256 val) onlyowner public returns(bool success) {
            limitperwalletM[addr] = val;
            success = true;
    }
      function addlimitperwalletMs(address[] memory addrs, uint256 val) onlyowner public returns(bool success) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (addlimitperwalletM(addrs[i],val)) {
                success = true;
            }
        }
    }  

// remember to exclude presale
    function Buy(uint256 _amount) public  payable whenNotPaused  {   
        require(limit[msg.sender].add(_amount)<=limitperwalletM[msg.sender],"Limit exceeded");
        ...
        depostiToTorrent(address(this),msg.sender,upline,_amount);
