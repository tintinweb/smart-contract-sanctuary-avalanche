...
contract BridgesOfFate is Ownable {
    ...
    event comment2(uint256 atJumpStage);
    event comment7(bool test);
    event comment8(bool outertest);
    event comment9(uint256 testnum1);
    event comment10(uint256 testnum2);

    function _overFlow(bytes32 _playerId,uint256 jumpAtStage,bool _jumpSide) internal returns(bool)  {
        // GameStatus storage  _gameStatus = GameStatusInitialized[latestGameId];        
        // if(totalNFTOnTile[_gameStatus.stageNumber][_jumpSide] > 0 ){
        //     totalNFTOnTile[_gameStatus.stageNumber][_jumpSide] = totalNFTOnTile[_gameStatus.stageNumber][_jumpSide] - 1;
        // }            
        emit comment2(jumpAtStage);
        if(totalNFTOnTile[jumpAtStage][_jumpSide] <= 0){                
            OverFlowPlayerStatus[_playerId] =  false;
            emit comment(OverFlowPlayerStatus[_playerId]);
        ...
        emit comment8(isSafeSide);
        emit comment9(totalNFTOnTile[jumpAtStage - 1][isSafeSide]);
        emit comment10(TitleCapacityAgainestStage[jumpAtStage - 1]);
    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        // if(_currentUserStage >= 1){
        //     changetileStatic(playerId);
        // }
        ...
        // emit comment(isNarrowingGameEnded);
    }
    ...
    */
    ...
    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  {
        ...
        if(totalNFTOnTile[GameStatusInitialized[latestGameId].stageNumber][_jumpSides] < playerIds.length){
            playerIndex = generateRandomTicketNumbers(playerIds.length,playerIds.length,totalNFTOnTile[GameStatusInitialized[latestGameId].stageNumber][_jumpSides]);
        }
    ...
    event comment3(uint256 globalStage,uint256 localstagecapacity, bool _val);
    event comment4(uint256 message);


    function changetileStatic(bytes32 playerId) public {
        GameItem storage _member = PlayerItem[playerId];
        _member.ishold =  true;
        emit comment3(TitleCapacityAgainestStage[_member.stage] , totalNFTOnTile[_member.stage][_member.lastJumpSide],(TitleCapacityAgainestStage[_member.stage] > totalNFTOnTile[_member.stage][_member.lastJumpSide]));
        if(TitleCapacityAgainestStage[_member.stage] > totalNFTOnTile[_member.stage][_member.lastJumpSide]){
            ...
            emit comment4(totalNFTOnTile[_member.stage][_member.lastJumpSide]);
        }
        PlayerItem[playerId] =  _member;
    }
