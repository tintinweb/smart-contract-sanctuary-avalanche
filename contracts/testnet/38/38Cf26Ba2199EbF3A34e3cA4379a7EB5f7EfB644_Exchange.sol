...
contract Exchange is IExchange, ReentrancyGuard {
    ...
    function buyTokens(uint256 _quantity, address _wallet) nonReentrant public onlyAdminUser {
        ...
        // if ((token.balanceOf(_wallet)).add(_quantity) > (token.basicCap().div(1 ether).div(1000))) {
        //     require(
    ...
    */
    ...
    function acceptCounter(address _buyerWallet) nonReentrant public {
        ...
        if (msg.sender == seller || (_whitelist.isAdmin(msg.sender) && StringUtils.equal(counterOffers[_buyerWallet].role, "buyer"))) {
            ...
            // if (
            //     (token.balanceOf(msg.sender)).add(_quantity) > (token.basicCap().div(1 ether).div(1000))
            // ) {
        ...
        if (banks.length > 0) {
            for (uint256 i = 0; i < banks.length; i++) {
                if (noOfTokens[i] <= quantity && quantity != 0) {
                    ...
                    remainingAmount = totalAmount.sub(loanAmounts[i]);
                    quantity = quantity.sub(noOfTokens[i]);
                    
                    emit Log(totalAmount,remainingAmount,quantity,loanAmounts[i],"first");
                    ...
                    emit Log(transferTokens,totalAmount,loanAmounts[i],totalAmount.sub(transferTokens),"second");

                     
                    // totalAmount = totalAmount.sub(loanAmounts[i]);
                    stableCoin.transferFrom(seller, banks[i], totalAmount.sub(transferTokens));
                    remainingAmount = transferTokens;
                  
                    _IMarginLoan.updateLoan(
                        seller,
                        ids[i],
                        totalAmount.sub(transferTokens),
