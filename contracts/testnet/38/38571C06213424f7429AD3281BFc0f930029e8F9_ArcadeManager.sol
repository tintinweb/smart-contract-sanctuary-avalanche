...
contract ArcadeManager {
    ...
    event ClearReserveBONK(address user, uint time);
    ...
    // Clear all reserve 
    function clearReserveBONK(address _account) external onlyVIP {
        if (_account != address(0)) {
            userBONKToBAMReserve[_account] = 0;
            emit ClearReserveBONK(_account, block.timestamp);
        } else {
            revert("Address is not correct");
        }
    }
