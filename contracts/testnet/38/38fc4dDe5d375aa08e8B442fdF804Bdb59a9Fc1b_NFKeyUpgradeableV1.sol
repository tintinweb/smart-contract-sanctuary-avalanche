...
contract NFKeyUpgradeableV1 is OwnableUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function isZone(address account) public view returns (bool){
        ...
        if (_activeZone >= 5 || (_activeZone >= whitelistUser.zone && whitelistUser.whitelistAddress != address(0))) {
            ...
            if (_activeZone == 1 && !mintedAddressList[account] && (level >= 1 && level <= 10)) return true;
            else if (_activeZone == 2 && !mintedAddressList[account] && (level >= 1 && level <= 3)) return true;
            else if (_activeZone == 3 && level >= 1 && level <= 10) return true;
            else if (_activeZone == 4 && level == 1) return true;
