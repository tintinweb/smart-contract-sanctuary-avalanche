...
contract MilkMarketplace is Ownable {

    uint256 public whitelistCounter;
    uint256 public raffleCounter;
    address public milkAddress;
   ...
   event PurchaseWL (uint256 indexed _id, address indexed _address, string _discord);
   event EnterRaffle (uint256 indexed _id, address indexed _address, uint256 _amount, string _discord);

    constructor() {     
    }

    function enterRaffle(uint256 _id, uint256 _amount, string memory _discord) public {
        require(
        ...
        IERC20(milkAddress).transferFrom(msg.sender, address(this), raffles[_id].price * _amount);
        emit EnterRaffle(_id, msg.sender, _amount, _discord);
    }

    function purchase(uint256 _id, string memory _discord) public {
        require(
       ...
       require(
        ...
        IERC20(milkAddress).transferFrom(msg.sender, address(this), whitelists[_id].price);

        emit PurchaseWL(_id, msg.sender, _discord);
    }

    function addWhitelist(uint256 _amount, uint256 _price, uint256 _duration) external onlyOwner {
    ...
    function addRaffle(uint256 _price, bool _capped, uint256 _duration) external onlyOwner {
    ...
    function setMilkAddress(address _milkAddress) public onlyOwner {
        milkAddress = _milkAddress;
    }
