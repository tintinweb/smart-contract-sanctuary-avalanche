    ...
    ReentrancyGuardUpgradeable
{
        ...
        for (uint256 i = 0; i < YieldBoxRewarded.length; i++) {
            if (amount[i] > 0) {
                ...
                if (
                    ...
                    revert("ERC721 VALIFi: Can't pay Minimal Rewards");
                } else {
     ...
     */
    function minimalRewardPerYK(address _wallet, uint256[] memory _YieldBoxIds) external nonReentrant {
        (
            uint256[] memory amount,
            uint256[] memory rewarded,
            uint256[] memory YieldBoxRewarded,
