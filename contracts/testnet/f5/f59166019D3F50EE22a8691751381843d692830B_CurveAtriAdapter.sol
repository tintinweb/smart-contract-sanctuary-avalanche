...
contract CurveAtriAdapter is CurveBasic {
    constructor(address _adapterManager)
        CurveBasic(_adapterManager, "CurveAtriSwap")
    {}
        ...
        onlyAdapterManager
    {
        ...
        IERC20 tokenTo = IERC20(addresses[1]);
        uint256 balanceBefore = tokenTo.balanceOf(addresses[2]);
        ICurveAtriCrypto router = ICurveAtriCrypto(addresses[3]);
        router.exchange_underlying(
            fromNumber,
            toNumber,
            dx,
            min_dy,
        ...
        uint256 balanceAfter = tokenTo.balanceOf(addresses[2]);
    ...
    function addLiquidity(bytes calldata encodedData)
        ...
        onlyAdapterManager
    {
        /// @param addresses addresses of [dai.e, usdc.e, usdt.e, wbtc.e, weth.e, lp, proxy, router]
        ...
        ) = abi.decode(encodedData, (address[], uint256[5], uint256));
        pullAndApprove(addresses[0], addresses[6], addresses[7], amountsIn[0]);
        pullAndApprove(addresses[1], addresses[6], addresses[7], amountsIn[1]);
        pullAndApprove(addresses[2], addresses[6], addresses[7], amountsIn[2]);
        pullAndApprove(addresses[3], addresses[6], addresses[7], amountsIn[3]);
        pullAndApprove(addresses[4], addresses[6], addresses[7], amountsIn[4]);
        ICurveAtriCrypto router = ICurveAtriCrypto(addresses[7]);

        ICurveLpToken lp = ICurveLpToken(addresses[5]);
        uint256 balanceBefore = lp.balanceOf(addresses[6]);
        router.add_liquidity(amountsIn, minMintAmount, addresses[6]);
        uint256 balanceAfter = lp.balanceOf(addresses[6]);
        ...
        onlyAdapterManager
    {
        ...
        ) = abi.decode(encodedData, (address[], uint256, uint256[5]));
        ICurveAtriCrypto router = ICurveAtriCrypto(addresses[7]);
        pullAndApprove(addresses[5], addresses[6], addresses[7], removeAmount);

        uint256[5] memory balancesBefore;

        balancesBefore[0] = IERC20(addresses[0]).balanceOf(addresses[7]);
        balancesBefore[1] = IERC20(addresses[1]).balanceOf(addresses[7]);
        balancesBefore[2] = IERC20(addresses[2]).balanceOf(addresses[7]);
        balancesBefore[3] = IERC20(addresses[3]).balanceOf(addresses[7]);
        balancesBefore[4] = IERC20(addresses[4]).balanceOf(addresses[7]);

        router.remove_liquidity(removeAmount, minAmounts, addresses[6]);

        uint256[5] memory balancesAfter;

        balancesAfter[0] = IERC20(addresses[0]).balanceOf(addresses[7]);
        balancesAfter[1] = IERC20(addresses[1]).balanceOf(addresses[7]);
        balancesAfter[2] = IERC20(addresses[2]).balanceOf(addresses[7]);
        balancesAfter[3] = IERC20(addresses[3]).balanceOf(addresses[7]);
        balancesAfter[4] = IERC20(addresses[4]).balanceOf(addresses[7]);
        ...
        onlyAdapterManager
    {
        ...
        ICurveAtriCrypto router = ICurveAtriCrypto(addresses[7]);
        pullAndApprove(addresses[5], addresses[6], addresses[7], lpAmount);
        IERC20 tokenGet = IERC20(addresses[tokenNumber]);
        uint256 balanceBefore = IERC20(tokenGet).balanceOf(addresses[6]);
        router.remove_liquidity_one_coin(
            lpAmount,
            tokenNumber,
            minAmount,
            addresses[6]
        );
        uint256 balanceAfter = IERC20(tokenGet).balanceOf(addresses[6]);
