...
/** 
 *  SourceUnit: c:\Users\Administrator\Documents\SystemData\work\01_work\master-savvy\contracts\MasterSavvy.sol
*/
...
interface ISavvyState {
    ...
    /// @notice Gets the address of the savvySwap.
    ///
    /// @return savvySwap The savvySwap address.
    function savvySwap() external view returns (address savvySwap);

    /// @notice Gets the address of the svyRewarder.
    ///
    /// @return svyRewarder The svyRewarder address.
    function svyRewarder() external view returns (address svyRewarder);
...
interface ISavvyEvents {
    ...
    /// @notice Emitted when the savvySwap is updated.
    ///
    /// @param savvySwap The updated address of the savvySwap.
    event SavvySwapUpdated(address savvySwap);
    ...
    /// @notice Emitted when `owner` get svy rewards
    /// @param owner The address of user wanna get rewards
    /// @param rewardsAmount The amount of rewards that owner get
    event ClaimBoostedRewards(address indexed owner, uint256 rewardsAmount);
}
...
interface ISavvyAdminActions {
    ...
    struct InitializationParams {
        ...
        address debtToken;
        // The initial savvySwap or savvySwap buffer.
        address savvySwap;
        // The address of giving rewards to users.
        address svyRewarder;
        // The address of veSvy token contract.
        address veSvyToken;
    ...
    /// @notice Emits a {SavvySwapUpdated} event.
    ...
    /// @notice Set the address of the savvySwap.
    ///
    /// @notice `msg.sender` must be the admin or this call will revert with an {Unauthorized} error.
    ...
    /// @notice Emits a {SavvySwapUpdated} event.
    ///
    /// @param value The address of the savvySwap.
    function setSavvySwap(address value) external;
...
interface ISavvyActions {
    ...
    ///
    /// @notice **_NOTE:_** This function is WHITELISTED.
    /// @notice **_NOTE:_** When depositing, the `MasterSavvy` contract must have **allowance()** to spend funds on behalf of **msg.sender** for at least **amount** of the **underlyingToken** being deposited.  This can be done via the standard `ERC20.approve()` method.
    ///
    /// @notice **Example:**
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 amount = 50000;
    /// @notice MasterSavvy(savvyAddress).depositCollateral(ydai, amount, msg.sender, 1);
    /// @notice ```
    ///
    ...
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 pps = MasterSavvy(savvyAddress).getUnderlyingTokensPerShare(ydai);
    /// @notice uint256 amountUnderlyingTokens = 5000;
    /// @notice MasterSavvy(savvyAddress).withdrawUnderlying(ydai, amountUnderlyingTokens / pps, msg.sender, 1);
    /// @notice ```
    ///
    ...
    /// @notice MasterSavvy(savvyAddress).withdrawUnderlying(msg.sender, ydai, amtUnderlyingTokens / pps, msg.sender, 1);
    /// @notice ```
    ///
    ...
    /// @notice MasterSavvy(savvyAddress).borrowCredit(amtDebt, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice ```
    /// @notice uint256 amtDebt = 5000;
    /// @notice MasterSavvy(savvyAddress).mintFrom(msg.sender, amtDebt, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice MasterSavvy(savvyAddress).burn(amtBurn, msg.sender);
    /// @notice ```
    ///
    /// @param amount    The amount of tokens to burn.
    /// @param recipient The address of the recipient.
    ...
    /// @notice MasterSavvy(savvyAddress).repay(dai, amtRepay, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice MasterSavvy(savvyAddress).liquidate(ydai, amtSharesLiquidate, 1);
    /// @notice ```
    ///
    ...
    /// @notice MasterSavvy(savvyAddress).liquidate(dai, amtSharesLiquidate, 1);
    /// @notice ```
    ///
    ...
    /// @notice This function will be called whenever veSvy balance is changed.
    /// @dev Refresh the userCollateralWeight and sum of them
    /// @param user The address of user that should update collateral wight
    function updateFactor(address user) external;

    /// @notice Claim svy rewards according to amount of borrowed credit
    function claimBoostedRewards() external;
}
...
/// @title  IBoostedSavvy
/// @author Savvy Finance
interface IBoostedSavvy {
   /// @notice Giving rewards according the amounts of borrowed credit
   /// @param user            The address of user that wanna get rewards
   /// @param userColWeight   sqrt([user’s total borrowed credit] x [user’s veSvy balance])
   /// @param totalColWeight  Sum total of all users (userCollateralWeight)
   /// @return                Actual amount of giving rewards
   function onSvyReward(
      address user, 
      uint256 userColWeight,
      uint256 totalColWeight
   ) external returns (uint256);

   /// @notice Update pending rewards when borrowed credit or veSvy balance changed
   /// @param user            The address of user that wanna get rewards
   /// @param userColWeight   sqrt([user’s total borrowed credit] x [user’s veSvy balance])
   /// @param totalColWeight  Sum total of all users (userCollateralWeight)
...
/// @title  ISavvyToken
/// @author Savvy Finance
interface ISavvyToken is IERC20Minimal, IERC20Burnable, IERC20Mintable {
...
/// @title  MasterSavvy
/// @author Savvy Finance
contract MasterSavvy is IMasterSavvy, Initializable, Multicall, Mutex {
    ...
    uint256 public totalUserColWeight;
    ...
    /// @notice The address of veSvy token contract
    address public veSvyToken;
    ...
    /// @inheritdoc ISavvyState
    address public override savvySwap;

    /// @inheritdoc ISavvyState
    address public override svyRewarder;
    ...
    /// @dev User Collateral Weight
    mapping(address => uint256) private _userColWeights;
    ...
    /// @dev using Constructor with intializable no longer supported in the new version of openzeppline contracts
    ...
    function initialize(InitializationParams memory params) external initializer {
        ...
        admin                    = params.admin;
        savvySwap               = params.savvySwap;
        svyRewarder              = params.svyRewarder;
        veSvyToken               = params.veSvyToken;
        ...
        emit SavvySwapUpdated(savvySwap);
    ...
    /// @inheritdoc ISavvyAdminActions
    function setSavvySwap(address value) external override {
        _onlyAdmin();
        _checkArgument(value != address(0));
        savvySwap = value;
        emit SavvySwapUpdated(value);
    }
    ...
    function repayDebt(
        ...
        // Transfer the repaid tokens to the savvySwap.
        TokenUtils.safeTransferFrom(underlyingToken, msg.sender, savvySwap, actualAmount);

        // Inform the savvySwap that it has received tokens.
        IERC20TokenReceiver(savvySwap).onERC20Received(underlyingToken, actualAmount);
    ...
    function liquidateDebt(
        ...
        // Transfer the liquidated tokens to the savvySwap.
        TokenUtils.safeTransfer(underlyingToken, savvySwap, amountUnderlyingTokens);

        // Inform the savvySwap that it has received tokens.
        IERC20TokenReceiver(savvySwap).onERC20Received(underlyingToken, amountUnderlyingTokens);
    ...
    function harvest(address yieldToken, uint256 minimumAmountOut) external override lock {
        ...
        // Transfer the tokens to the fee receiver and savvySwap.
        TokenUtils.safeTransfer(underlyingToken, protocolFeeReceiver, feeAmount);
        TokenUtils.safeTransfer(underlyingToken, savvySwap, distributeAmount);

        // Inform the savvySwap that it has received tokens.
        IERC20TokenReceiver(savvySwap).onERC20Received(underlyingToken, distributeAmount);

        emit Harvest(yieldToken, minimumAmountOut, amountUnderlyingTokens, credit);
    }

    /// @inheritdoc	ISavvyActions
    function claimBoostedRewards() external override {
        address owner = msg.sender;
        Account storage account = _accounts[owner];
        int256 debt = account.debt;

        _checkState(owner != address(0));
        _checkState(debt > 0);
        _checkState(totalUserColWeight > 0);

        uint256 rewardsAmount = IBoostedSavvy(svyRewarder).onSvyReward(owner, _userColWeights[owner], totalUserColWeight);

        emit ClaimBoostedRewards(owner, rewardsAmount);
    }

    /// @inheritdoc	ISavvyActions
    function updateFactor(address user) external override {
        _checkState(msg.sender == veSvyToken);
        _checkState(user != address(0));
        _updateFactor(user);
    }

    /// @dev Update user collateral weight and total of them
    /// @param user The address of user should update collateral weight
    function _updateFactor(
        address user
    ) internal {
        uint256 veBal = IERC20(veSvyToken).balanceOf(user);
        int256 debt = _accounts[user].debt;

        uint256 newColWeight = 0;
        if (veBal > 0 && debt > 0) {
            newColWeight = Math.sqrt(veBal * uint256(debt) / 1e18);
        }

        totalUserColWeight = totalUserColWeight - _userColWeights[user] + newColWeight;
        _userColWeights[user] = newColWeight;

        IBoostedSavvy(svyRewarder).updatePendingRewards(user, newColWeight, totalUserColWeight);
    }
