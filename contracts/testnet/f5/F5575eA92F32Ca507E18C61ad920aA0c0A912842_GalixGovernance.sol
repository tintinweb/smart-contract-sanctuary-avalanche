...
import "./common/interfaces/IRewardDistributionRecipient.sol";
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    address public p2pPool; //NEMO P2P Pool

    EnumerableSetUpgradeable.AddressSet private _stakePools; //Staking Pool
    EnumerableSetUpgradeable.AddressSet private _daoFunds; //DAO Treasury Address
    ...
    event TokenMintP2P(address p2pPool, uint256 amount);
    event ProfitDistribute(address operator, address stakePool, uint256 period, address[] funds, uint256[] amounts);
    ...
    function addStakePool(address _pool) onlyAdmin external {
        _stakePools.add(_pool);
    }

    function removeStakePool(address _pool) onlyAdmin external {
        _stakePools.remove(_pool);
    }

    function addDaoFund(address _dao) onlyAdmin external {
        _daoFunds.add(_dao);
    }

    function removeDaoFund(address _dao) onlyAdmin external {
        _daoFunds.remove(_dao);
    }
    ...
    function tokenMint(
        ...
        address _daoFund,
        uint256 _amount,
        uint256 _daoDivision
    ) onlyTokenOperator external {
        require(_rewardPools.contains(_rewardPool), "_rewardPool invalid");
        require(_daoFunds.contains(_daoFund), "_daoFund invalid");
        uint256 _daoFundAmount = _amount.mul(_daoDivision).div(10000);
        token.mintFrom(_daoFund, _daoFundAmount);
        token.mintFrom(_rewardPool, _amount.sub(_daoFundAmount));
        emit TokenMint(_daoFund, _rewardPool, _daoFundAmount, _amount.sub(_daoFundAmount));
    }

    function tokenMintP2P(uint256 _amount) onlyTokenOperator external {
        token.mintFrom(p2pPool, _amount);
        emit TokenMintP2P(p2pPool, _amount);
    }

    ///////////////////////// STAKE OPERATOR /////////////////////////
    function profitDistribute(
        address _stakePool,
        uint256 _period,
        address[] memory _funds,
        uint256[] memory _amounts
    ) onlyStakeOperator external {
        require(_stakePools.contains(_stakePool), "_stakePool invalid");
        if(_amounts[0] > 0){
            token.transferFrom(_msgSender(), _stakePool, _amounts[0]);
            IRewardDistributionRecipient(_stakePool).notifyRewardAmount(_amounts[0], _period);
        }
        for(uint256 i=0; i<_funds.length; i++){
            if(_amounts[i+1] > 0){
                token.transferFrom(_msgSender(), _funds[i], _amounts[i+1]);
            }
        }
        emit ProfitDistribute(_msgSender(), _stakePool, _period, _funds, _amounts);
    }
