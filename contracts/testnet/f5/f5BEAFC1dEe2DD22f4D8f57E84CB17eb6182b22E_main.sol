...
contract main is ERC20, Ownable {
   ...
   function cashoutAll() external {
        ...
        (uint256 nodes,uint256 time) = nodeManager.getNodesAmount(sender);
        uint256[3] memory Tiers =  _overseer_.getGreensAmount(sender);
        uint256 count;
        uint256 rewards;
        for(uint i=0;i<Tiers.length;i++){
        	for(uint k=0;i<Tiers[i];k++){
        		(uint256 _time ,uint256 _boost) = _overseer_.getCurrGreens(sender,i,k);
        		for(uint j=0;j<5;j++){
        			rewards += nodeManager.getNodesRewards(sender,time,count,_boost,_time);
        			count+=1;
        		}
        	}
        }
        for(uint i=count;i<nodes;i++){
        	rewards += nodeManager.getNodesRewards(sender,time,i,0,0);
        }		
        require(rewards > 0, "you have no rewards to cash out");
        if (Tiers[1] >0){
        	_cashoutFee = myLib.doPercentage(cashoutFee,cashoutPerc);
        }
