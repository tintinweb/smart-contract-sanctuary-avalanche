...
contract Staking is Ownable {
    ...
    function getRewards(uint256 id) public {
        ...
        uint256 timeFactor = (block.timestamp - userStake.lastReward) / rewardPeriod;
    ...
    function ableToUnstake(uint256 id) external view returns (bool) {
        (bool found, UserNode memory userStake, ) = searchUserNode(id, msg.sender);
        require(found, 'User node with id not found.');

        return block.timestamp >= userStake.start + unstakePeriod;
    }
