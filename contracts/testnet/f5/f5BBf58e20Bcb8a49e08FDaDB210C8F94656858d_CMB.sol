...
contract CMB is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     *  @notice Mapping payment ID to excess amount to store excess cash in contract
     */
    mapping(uint256 => uint256) public excessCash;
        ...
        nonReentrant  
    {
        ...
        if (permittedPaymentTokens[payment.paymentToken]) {
            ...
            IERC20(payment.paymentToken).safeTransferFrom(_msgSender(), address(this), amount);
        } else {
            require(
        ...
        nonReentrant 
    {
        ...
        excessCash[paymentId] = payment.paidAmount - payment.amount;

        if (permittedPaymentTokens[payment.paymentToken]) {
            IERC20(payment.paymentToken).safeTransfer(_msgSender(), claimableAmount);
            IERC20(payment.paymentToken).safeTransfer(owner(), serviceFee);
        } else {
        ...
        onlyClient(paymentId) 
    {
        ...
        if (permittedPaymentTokens[payment.paymentToken]) {
            IERC20(payment.paymentToken).safeTransfer(_msgSender(), payment.paidAmount);
        } else {
        ...
        nonReentrant 
    {
        ...
        require(
            ...
            "Can only withdraw within 3 days of initialing payment or within 90 days of void date"
        );
        require(
     ...
     * 
     *          Name             Meaning 
     *  @param  paymentId        ID of payment that owner (CMB) want to withdraw money
     *  @param  fundingReceiver  Address that want to transfer
     */
    function withdrawExcessCash(uint256 paymentId, address fundingReceiver) 
        external 
        onlyOwner 
        onlyValidPayment(paymentId)
        nonReentrant
    {
        ...
        if (permittedPaymentTokens[payment.paymentToken]) {
            IERC20(payment.paymentToken).safeTransfer(fundingReceiver, excessCash[paymentId]);
        } else {
            payable(fundingReceiver).transfer(excessCash[paymentId]);
        }
        emit WithdrawnExcessCash(excessCash[paymentId], payment.paymentToken, fundingReceiver);
        excessCash[paymentId] = 0;
    }
}
