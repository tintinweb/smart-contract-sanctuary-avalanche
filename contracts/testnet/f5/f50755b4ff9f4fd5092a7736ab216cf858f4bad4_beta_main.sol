...
contract beta_main is ERC20, Ownable {
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256 fee = checkTXN(address(this),doPercentage(nodeAmount,rewardsFee));
        emit Transfer(address(this),rewardsPool, fee);
        fee = checkTXN(address(this),doPercentage(nodeAmount,teamPoolFee));
        emit Transfer(address(this),teamPool, fee);
        fee = checkTXN(address(this),doPercentage(nodeAmount,treasuryFee));
        emit Transfer(address(this),treasury, fee);
