...
contract preSale is Auth, Referral
{
    ...
    mapping(address => bool) public isBluewhale;
    mapping(address => uint256) public option2;
    constructor(address _ETHW, address _DAI) Referral(_ETHW)
    {
        DAI = _DAI;
    }

    // 0-option1   1-option2
    function purchaseETHW(uint256 _ethw, uint256 _option, string memory _name)public
    {
        if(_option == 0)
        {
            require((option1[msg.sender] + _ethw) <= 1200 * (10 ** 18), "MAX purchase limit reached!");
            IERC20(DAI).transferFrom(msg.sender, address(this), (_ethw * (9 * (10 ** 17))) / (10 ** 18));
            ethsToken.transfer(msg.sender, _ethw);
            option1[msg.sender] += _ethw;
        }
        else
        {
            if(!isBluewhale[msg.sender])
            {
                // if bluewhale is not purchased...
                IERC20(DAI).transferFrom(msg.sender, address(this), (8 * (10 ** 17)) * (800 * (10 ** 18)));
                // purchasing BlueWhale for msg.sender...
                mintNode( _name, 800 * (10 ** 18), 3);
                isBluewhale[msg.sender] = true;
                purchaseETHW(_ethw - (800 * (10 ** 18)), _option, _name);
            }
            else
            {
                // if bluewhale is already purchased...
                require((option2[msg.sender] + _ethw) <= 400 * (10 ** 18), "Max Limit Reached!");
                IERC20(DAI).transferFrom(msg.sender, address(this), (_ethw * (8 * (10 ** 17))) / (10 ** 18));
                ethsToken.transfer(msg.sender, _ethw);
                option2[msg.sender] += _ethw;
            }
        }
    }
}