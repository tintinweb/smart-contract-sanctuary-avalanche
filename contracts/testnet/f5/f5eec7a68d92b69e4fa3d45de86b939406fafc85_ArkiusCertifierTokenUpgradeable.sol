...
import './NFTs/CertifierTokenUpgradeable.sol';
import './utils/Blacklistable.sol';
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract ArkiusCertifierTokenUpgradeable is Initializable, UUPSUpgradeable, CertifierTokenUpgradeable, Blacklistable {

    event CreateERC1155_v1(address indexed creator, string name, string symbol);

    event UpdateMembership(address membershipNFTAddress);

    IArkiusMembershipToken private membershipToken;

    uint256 constant INVALID = 0;

    /// @dev Contains Id of all certifiers.
    uint256[] private allCertifiers;

    /// @dev mapping from certifierId => index of allCertifiers.
    mapping(uint256 => uint256) private indexAllCertifiers;

    string public constant name = "Arkius Certifier Token";
    ...
    function initialize(string memory          contractURI,
        ...
        __CertifierToken_init(contractURI, tokenURIPrefix, multisigAddress);
    ...
    function mint(string calldata uri) external notBlacklisted {
        ...
        allCertifiers.push(id);
        indexAllCertifiers[id] = allCertifiers.length;
    ...
    function burn(address owner, uint256 value) external notBlacklisted {
        ...
        uint256 certifierId = _certifierIdOf(owner);

        if (certifierId > 0) {
            allCertifiers[indexAllCertifiers[certifierId] - 1] = allCertifiers[allCertifiers.length - 1];
            indexAllCertifiers[allCertifiers[allCertifiers.length - 1]] = indexAllCertifiers[certifierId];
            indexAllCertifiers[certifierId] = 0;
            allCertifiers.pop();
        }
    ...
    function getAllCertifiers() public view returns(uint256[] memory) {
...
import './ERC1155BaseCertifierUpgradeable.sol';
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract CertifierTokenUpgradeable is Initializable, ERC1155BaseCertifierUpgradeable {

    function __CertifierToken_init(string memory contractURI,
                                    string memory tokenURIPrefix,
                                    address       multisigAddress) internal initializer  {

        __ERC1155BaseCertifier_init(contractURI, tokenURIPrefix, multisigAddress);
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
    }

    /**
     *  @dev see {ERC1155BaseCertifier - _mint}
     */
...
import './ERC1155CertifierUpgradeable.sol';

import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract ERC1155BaseCertifierUpgradeable is Initializable, ERC1155Metadata_URIUpgradeable, HasContractURIUpgradeable, ERC1155CertifierUpgradeable {

    /// @dev Contains the mapping from TokenId => user who created that token.
    mapping (uint256 => address) private creators;

    function __ERC1155BaseCertifier_init(string memory contractURI, 
                string memory tokenURIPrefix,
                address       multisigAddress) internal initializer {

            __HasContractURI_init(contractURI);
            __ERC1155Metadata_URI_init(tokenURIPrefix);
            __ERC1155Certifier_init(multisigAddress);
    }
    ...
    */
    function _mint(uint256 id, uint256 supply, string memory uri) internal {
        ...
        require(certifierIDs[_msgSender()] == 0, "CertifierID is already minted for this user");

        creators[id] = _msgSender();
        balances[id][_msgSender()] = supply;
        certifierIDs[_msgSender()] = id;
    ...
    function _burn(address owner, uint256 value) internal {
        ...
        uint256 id = _certifierIdOf(owner);
        require(balances[id][owner] > 0, "Owner does not hold this Token");
        require(value == 1, "Only 1 Token can be burned");
        
        balances[id][owner] = 0;
        certifierIDs[owner] = 0;
...
contract ERC1155CertifierUpgradeable is Initializable, OwnableUpgradeable, IERC1155, ERC165Upgradeable, CommonConstants {
    ...
    mapping(address => uint256) internal certifierIDs;
    ...
    */
    ...
    function __ERC1155Certifier_init(address multisigAddress) internal initializer {
    ...
    */
    ...
    function _certifierIdOf(address owner) internal view returns (uint256) {
        // The balance of any account can be calculated from the Transfer events history.
        // However, since we need to keep the balances to validate transfer request,
        // there is no extra cost to also privide a querry function.
        return certifierIDs[owner];
    }
