...
WalletObserverImplementationPointer
{
    ...
    mapping(address => bool) public _isExcludedFromSmartPabloTax;
    mapping(address => bool) public _isSmartTaxable;
    uint public _smartPabloTax = 20;
    uint public _smartTax = 50;
    address public _burnWallet;

    function updateBurnWallet(address burnWallet)
    public
    onlyOwner
    {
        require(burnWallet != address(0), "ERC20: You must specify an address");
        _burnWallet = burnWallet;
    }

    function updateSmartPabloTax(uint smartPabloTax)
    public
    onlyOwner
    {
        require(smartPabloTax <= 50, "ERC20: Tax must not exceed 20");
        _smartPabloTax = smartPabloTax;
    }

    function updateSmartTax(uint smartTax)
    public
    onlyOwner
    {
        require(smartTax <= 100, "ERC20: Tax must not exceed 50");
        _smartTax = smartTax;
    }

    function excludeFromSmartPabloTax(address account, bool status)
    public
    onlyOwner
    {
        _isExcludedFromSmartPabloTax[account] = status;
    }

    function includeInSmartTax(address account, bool status)
    public
    onlyOwner
    {
        _isSmartTaxable[account] = status;
    }
    ...
    modifier onlyPlanetsManager() {
        ...
        require(
            ...
            "Implementations: Not DealersManager"
        );
        _;
    }

    constructor(address _planetsManager) ERC20("Medellin City", "PABLO") {
        require(
            _planetsManager != address(0),
            "Implementations: DealersManager is not set"
        );
        planetsManager = ERC721(_planetsManager);
        _mint(owner(), 6_000_000_000 * (10**18));
        _burnWallet = _msgSender();
    }
    ...
    function _transfer(
        ...
        // Smart Pablo Tax
        if (!_isExcludedFromSmartPabloTax[from] && planetsManager.balanceOf(from) == 0) {
            amountFee = amountFee + (amount * _smartPabloTax / 100);
        }

        // Smart Tax for Printers and Bots
        if (_isSmartTaxable[from]) {
            amountFee = amountFee + (amount * _smartTax / 100);
        }
        ...
        if (amountFee > 0) {
            super._transfer(from, _burnWallet, amountFee);
        }
    ...
    onlyPlanetsManager
    {
        require(
            ...
            "MedellinCity: Use liquidityReward to reward liquidity"
        );
    ...
    function liquidityReward(uint256 amount) external onlyPlanetsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "MedellinCity: LiquidityPoolManager is not set"
        );
