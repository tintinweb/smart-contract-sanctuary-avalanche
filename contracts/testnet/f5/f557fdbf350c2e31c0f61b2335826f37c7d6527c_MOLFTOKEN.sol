...
contract MOLFTOKEN is Context, IERC20, Ownable {
    ...
    address private _FeeWalletAddress = 0x6c951d61813511E92805204F217295DC52FB634F;
    ...
    string private _name = "MOLF Token";
    string private _symbol = "MOLF";
    uint8 private _decimals = 9;
    uint256 public _taxFee = 0;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _MarketingFee = 10;
    uint256 private _previousMktFee = _MarketingFee;
    ...
    function tokenFromReflection(uint256 rAmount) private view returns(uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,  uint256 tMkt) = _getValues(tAmount);
    ...
    function _takeFeeMkt(uint256 tFeeMkt) private {
        uint256 currentRate =  _getRate();
        uint256 rRnd = tFeeMkt.mul(currentRate);
        _rOwned[_FeeWalletAddress] = _rOwned[_FeeWalletAddress].add(rRnd);
        if(_isExcluded[_FeeWalletAddress])
            _tOwned[_FeeWalletAddress] = _tOwned[_FeeWalletAddress].add(tFeeMkt);
    }
