...
contract MonsterBattle {
    ...
    uint256 public lastFreeEnergyRefill;
    ...
    function _battleMonster(uint256 tokenId, uint256 randomNumberThreshold, uint256 pointsReward) private {
        ...
        if (!hasReceivedFreeEnergy[tokenId]) {
            if (lastFreeEnergyRefill == 0 || block.timestamp >= lastFreeEnergyRefill.add(1 days)) {
                battleEnergy[tokenId] = 5; // Refill free energy to maximum (5 units)
                lastFreeEnergyRefill = block.timestamp;
            } else {
        ...
        if (randomNumber < (randomNumberThreshold * 100)) {
    ...
    function setBattleEnergy(uint256 tokenId, uint256 energy) external onlyOwner {
        require(energy <= 100, "Invalid battle energy"); // Maximum battle energy is 5
