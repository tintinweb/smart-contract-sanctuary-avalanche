...
contract MetroGalaxyMarketplace is
    IMetroGalaxyMarketplace,
    ERC721Holder,
    ERC1155Holder,
    AcceptedToken,
    Pausable,
    ReentrancyGuard
{
    ...
    function list(
        ...
        emit AssetListed(assetAddr, assetId, seller, priceInWei, amount);
    }
    ...
    function delist(address assetAddr, uint256 assetId) external override nonReentrant {
        ...
        bytes32 id = _getAssetId(assetAddr, assetId);
        uint256 assetAmount = listedAssets[id][seller].amount;
        require(assetAmount > 0, "MetroGalaxyMarketplace: asset is not listed");
        delete listedAssets[id][seller];

        // transfer asset to contract
        AcceptedAssets(assetAddr).safeTransferFrom(address(this), seller, assetId, assetAmount);

        emit AssetDelisted(assetAddr, assetId, seller);
    }

    /**
     * @dev Place offer for assets
     * If user want to update price or amount, user need to cancel current offer first
     * Anyone can place an offer
     * @param assetAddr Asset address, should be in list supported address
     * @param assetId Asset id
     * @param priceInWei Price in wei
     * @param amount Asset amount
     ...
     * User can only cancel offer if previously place an offer
     * @param assetAddr Asset address, should be in list supported address
     * @param assetId Asset id
     */
    function cancelOffer(address assetAddr, uint256 assetId) external override nonReentrant {
        ...
        address buyer = msg.sender;
        bytes32 id = _getAssetId(assetAddr, assetId);
        Asset memory asset = offeredAssets[id][buyer];

        require(asset.amount > 0, "MetroGalaxyMarketplace: no asset offer found");
        delete offeredAssets[id][buyer];
        acceptedToken.safeTransfer(buyer, asset.priceInWei * asset.amount);
    ...
    function takeOffer(
        ...
        acceptedToken.safeTransfer(seller, totalPrice - marketFee);
    ...
    function _getMarketFee(uint256 price) private view returns (uint256) {
        return (price * marketFeeInBps) / BPS;
    }
...
interface IMetroGalaxyMarketplace {
    ...
    event AssetDelisted(address indexed assetAddr, uint256 indexed assetId, address seller);
    event AssetBought(
        address indexed assetAddr,
        uint256 indexed assetId,
        address buyer,
