...
contract CollSurplusPool is Ownable, CheckContract, ICollSurplusPool, PoolBase {
    ...
    mapping(address => uint256) internal redemptionBonus;
    uint256 internal totalRedemptionBonus;
    ...
    */
     ...
     * collaterla is sent to the CollSurplusPool and is available to be reclaimed.
     ...
     */
    function accountRedemptionBonus(
        address _account,
        uint256 _amount
    ) external override {
        _requireCallerIsTroveManager();
    ...
    /**
     * @notice Function called by borrower operations which
     * claims the collateral that is owned by a particular trove user.
     * @param _account address
     */
    function claimColl(address _account) external override {
        ...
        if (_collsIsNonZero(claimableColl)) {
            balances[_account].tokens = new address[](0);
            balances[_account].amounts = new uint256[](0); // sets balance of account to 0
            emit CollBalanceUpdated(_account);
        ...
        // sendRedemption bonus also resets _account's redemption bonus to 0
        _sendRedemptionBonus(_account);
    }
