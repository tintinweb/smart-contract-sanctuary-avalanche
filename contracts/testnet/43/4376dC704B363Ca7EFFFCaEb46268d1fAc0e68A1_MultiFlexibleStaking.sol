...
contract MultiFlexibleStaking is Ownable, Pausable {
    ...
    uint256 public constant MIN_STAKING_PERIOD = 60;
    uint256 public constant RECEIVE_PERIOD = 600;
    ...
    function withdraw(uint256 id) public updateReward(id) {
        ...
        uint256 amountToUser = (amount * (100 * ACCURACY - principalPenaltyPercentage)) /
            (100 * ACCURACY);

        // amountToUser != 0 due to the technical design
    ...
    function _claim(uint256 id, uint256 rewardsPenaltyPercentage) internal {
        ...
        uint256 amountToUser = (reward * (100 * ACCURACY - rewardsPenaltyPercentage)) /
            (100 * ACCURACY);
        if (amountToUser != 0) {
            rewardToken.safeTransfer(msg.sender, amountToUser);
        }

        uint256 amountToWallet = (reward * rewardsPenaltyPercentage) / (100 * ACCURACY);
        if (amountToWallet != 0) {
            rewardToken.safeTransfer(penaltyWallet, amountToWallet);
        }
    }
    ...
    function getAPR(uint256 id) external view returns (uint256) {
        ...
        return
            (uint256(rewardRate) * ACCURACY * 365 days * 100) / _tvl; // earned user value in tokens for one second including share (1e18 for accuracy)
    }
