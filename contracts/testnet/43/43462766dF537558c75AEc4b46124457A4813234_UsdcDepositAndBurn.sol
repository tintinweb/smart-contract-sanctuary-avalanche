...
contract UsdcDepositAndBurn is Initializable, UUPSUpgradeable, AccessControlUpgradeable, IUsdcDepositAndBurn {
    ...
    function setDestDetails(DestDetails[] memory destDetails) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint8 i = 0; i < destDetails.length; i++) {
            require(destDetails[i].chainId != 0, "USDC: Chain Id cannot be 0");
            require(destDetails[i].reserveHandlerAddress != address(0), "USDC: Reserve handler cannot be address(0)");
            require(destDetails[i].destCallerAddress != address(0), "USDC: Dest caller cannot be address(0)");
    ...
    function depositAndBurnUsdc(
        ...
        require(destDetails.reserveHandlerAddress != address(0), "USDC: Destination Details not set");
        require(msg.sender == erc20Handler, "USDC: Only ERC20Handler can burn tokens");

        IERC20Upgradeable(usdc).safeTransferFrom(sender, address(this), amount);
        IERC20Upgradeable(usdc).safeApprove(address(circleBridge), amount);
        ...
        uint64 usdcNonce = circleBridge.depositForBurnWithCaller(
            amount,
