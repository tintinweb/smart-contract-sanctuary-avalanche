 ...
 *     NOTICE
 *
 *     The software is licensed under a proprietary license or the GPL v.3.
...
contract FullFeatureToken is ERC20, ERC20Burnable, ERC20Pausable, Ownable {
  ...
  mapping(address => string) public whitelistedDocuments;
  mapping(address => uint256) public votesAsShareholder;
  /// @notice array holding all whitelisted addresses
  address[] public whitelistedAddresses;
  /// @notice array holding all whitelisted addresses KYC Documents
  string[] public whitelistedKYCDocumentUri;
 ...
 /// @notice date set for blocking token transfers
  uint256 public dateBlockUntil;
  /// @notice set of features supported by the token


  /// @notice features of the token
  SharedStruct.ERC20ConfigProps  private configProps;
  /// @notice owner of the contract
  address public immutable initialTokenOwner;
  /// @notice number of decimals of the token
  uint8 private immutable _decimals;
  // Set the blockedUntil timestamp to July 1, 2023
  ...
  event UsersWhitelisted(address[] updatedAddresses);
    /// @notice emitted when a new whitelist Document is set
  event UsersWhitelistedDocumentAdded(address whitelistAddress, string documentUri);
  //@notice emitted when allowed trade date is required
  event blockedUntilDateBlocked(uint256 dateBlockedUntil);
  //@notice emitted when tokens from a lost wallet have been recovered
  event TokensRecovered(address _lostWallet,address _recipient,uint256 balance);
  //@notice emitted when votes are granted to a shareholder
  event votesAsShareholderGranted(address _shareholder, uint256 _votes);
  ...
  error SenderNotWhitelisted(address addr);
   /// @notice raised when a sender address is not whitelisted
  error BlockedUntilNotExpired(address addr);
  ...
  error DocumentUriNotAllowed();
    /// @notice raised when trying to set a whitelist document URI when not allowed
  error WhitelistDocumentUriNotAllowed(address addr);

  error BlockDateUntilNotEnabled();
  ...
  modifier validateTransfer(address sender, address recipient) {
    ...
    if(isDateBlockedAllowed()){
      if(block.timestamp < dateBlockUntil  && sender != initialTokenOwner){
      revert BlockedUntilNotExpired(sender);
    }
    }
  ...
  constructor(
    ...
    uint8 decimalsToSet,
    address tokenOwner,
    SharedStruct.ERC20ConfigProps memory customConfigProps,
    uint256 maxTokenAmount,
    string memory newDocumentUri,
    uint256 _dateBlockUntil
  ) ERC20(name_, symbol_) {
    ...
    dateBlockUntil =_dateBlockUntil;
  ...
  /// @notice method which checks if the token supports Date block
  function isDateBlockedAllowed() public view returns (bool) {
    return configProps._isDateBlocked;
  }
  ...
  function getWhitelistedAddressDocument(address whitelistedAddress) external view returns (string memory getDocumentUri)  {
    getDocumentUri = whitelistedDocuments[whitelistedAddress];
  }
    ...
    whenNotPaused
  {
   ...
   function updateWhitelistDocuments(address whitelistedAddress, string calldata uri)
   external
   onlyOwner
   whenNotPaused
    {
    ...
    if (!whitelist[whitelistedAddress]) {
        revert RecipientNotWhitelisted(whitelistedAddress);
    }

    whitelistedDocuments[whitelistedAddress] = uri;
    emit UsersWhitelistedDocumentAdded(whitelistedAddress,uri);
  ...
  function setBlockdateUntil(uint256 date) public onlyOwner whenNotPaused{
       if(isDateBlockedAllowed()){
        dateBlockUntil = date;
       }
       else{
         revert BlockDateUntilNotEnabled();
       }


  }

  function getBlockDateUntil() external returns(uint256){
        if(isDateBlockedAllowed()){
              emit blockedUntilDateBlocked(dateBlockUntil);
  ...
  function recoverLostTokens(address _lostWallet, address _recipient) public onlyOwner {

    uint256 balance = balanceOf(_lostWallet);
    require(balance > 0, "No balance found in the lost wallet.");

    transferFrom(_lostWallet, _recipient, balance);
    emit TokensRecovered(_lostWallet, _recipient, balance);
}
}