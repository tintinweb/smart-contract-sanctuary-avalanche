...
contract TokenX is IERC20 {
    ...
    event DistributeSwapBack(uint Reflaction_Print, uint WAVAX_liquidity, uint WAVAX_marketing );
     ...
     function swapBack(uint256 _amount, uint256 amountToLiquify) internal swapping {
        uint initialPrintBal = PrintTokenBal(distributorAddress);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WAVAX;
        ...
        uint256 amountAVAXLiquidity = amountAVAX.sub(amountAVAXReflectionPrint.add(amountAVAXMarketing));
         ...
         { try distributor.deposit{value: amountAVAXReflectionPrint}() {} catch {}
          uint printReflactionAmount = PrintTokenBal(distributorAddress) - initialPrintBal;
    ...
    function setPrintToken(address _printToken) external onlyOwner{
    ...
    function setRewardPrintToken(address _tokenAdd, uint _tokenDecimal) external onlyOwner{
        try distributor.setPrintToken(_tokenAdd,_tokenDecimal) {} catch {}
        printToken = _tokenAdd;
    }
...
//0x3Caf97F5Bd858B0D6B8c2135BB7781319d6B91ae owner
