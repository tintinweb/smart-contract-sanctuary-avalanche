...
contract CapitalCall is Context, Ownable, ICapitalCall,ERC1820Client {
    ...
    function _onlyAdmin() private view {
        require(whitelist.isAdmin(_msgSender()) , "Only Admin is allowed to send");
    }
        ...
        override
    {
        ...
        require(stBal > 0,"you are not investor of this property");
        require(block.timestamp >= cc.startDate, "capital call not started");
    ...
    {
        CapitalCallStr storage cc = idToCapitalCall[_id];
        require(whitelist.isOwner(_msgSender()) ||  whitelist.isAdmin(_msgSender()) || cc.stAddress.propertyAccount() == _msgSender(), "Only Owner, Admin or property account are allowed to send");
        require(
        ...
        override
    {
        ...
        require(whitelist.isAdmin(_msgSender()) || cc.stAddress.propertyAccount() == _msgSender(), "Only Owner, Admin or property account are allowed to send");
        require(block.timestamp > cc.endDate, "capital call duration not ended");
        require(_newEndDate > cc.endDate,"new end date should be greater than previous end date");
