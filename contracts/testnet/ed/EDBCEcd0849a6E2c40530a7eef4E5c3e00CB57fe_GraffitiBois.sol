...
// File contracts/GraffitiBois.sol
    ...
    BaseERC2981
{
    ...
    string ipfsHash = "QmZoqB2nJVR1wy9LujPMjq3MEhTtrQTaCNJ9pFJ185fwV5";

    Counters.Counter private _tokenIdTracker;

    uint256 public price = 3.5 * 10e17;
    ...
    modifier onlyMinter() {
        require(
            ...
            "Action requires minter role"
        );

        _;
    }

    constructor() ERC721("Graffity Bois", "GB") Ownable() {
    ...
    function setMax(uint256 newMax) public virtual onlyOwner {
        max = newMax;
    }

    function claim() external payable nonReentrant {
        require(_tokenIdTracker.current() < max, "All assets minted");

        require(msg.value >= price, "Sent amount is lower than price");
    ...
    function airDrop(address[] calldata _address) external onlyMinter {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721, ERC721Enumerable, BaseERC2981, AccessControlEnumerable)
        returns (bool)
    {
        return
            interfaceId == type(IERC721).interfaceId ||
