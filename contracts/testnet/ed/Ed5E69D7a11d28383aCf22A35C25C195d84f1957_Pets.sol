...
contract Pets is ERC721Enumerable, Pausable, Controllable, Restricted {
    ...
    event onOrderPlaced(address indexed buyer, uint256 amount, bool isLimitedMin );
    ...
    function limitedMint(uint8 amount) external payable whenNotPaused onlyEOA noReentrency {
        ...
        if (limitedMintGemPrice > 0 && !freeMint) {
            gem.burn(_msgSender(), amount * limitedMintGemPrice);
        }
    ...
    function associatePetToWallet(uint256 petToken) external onlyEOA noReentrency {
        ...
        require(walletToPetToken[_msgSender()] == 0,  "A pet is already associated to your wallet");
