    ...
    ERC721TokenReceiverUpgradeable
{
    ...
    BitMaps.BitMap private __isUsed;
    ...
    function mint(
        ...
        emit Minted(_msgSender(), uid_, tokenId_, 1);
    }
    ...
    function mintBatch(
        ...
        emit Minted(_msgSender(), uid_, tokenIds[0], quantity_);
    }
    ...
    function __mintBatch(
        ...
        tokenIds = new uint256[](quantity_);
        ...
        for (uint256 i; i < quantity_; ) {
            unchecked {
                ...
                if (mintAmt > 0 && bytes(tokenURIs_[i]).length != 0) _setTokenURI(tokenIds[i], tokenURIs_[i]);
                ++i;
            }
        }
...
interface ICollectible721 {
    ...
    error Collectible721__Duplicate();
...
abstract contract ERC721URIStorageUpgradeable is ERC721Upgradeable {
     ...
     * - `tokenId` must exist.
     */
    function _setTokenURI(uint256 tokenId, string calldata _tokenURI)
        internal
        virtual
    {
        ownerOf(tokenId);
