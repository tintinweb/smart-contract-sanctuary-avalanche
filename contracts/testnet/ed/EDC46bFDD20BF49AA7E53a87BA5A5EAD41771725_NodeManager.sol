...
contract NodeManager is Ownable, Pausable {
    ...
    function getAllNodesRewards(address _account, uint256 k) external view returns (uint256) {
    	NodeEntity[] storage nodes = _nodesOfUser[_account];
    	uint256 lastClaim = nodes[k].lastClaimTime;
    	uint256 rewards = boostLib.calcReward(dailyRewardsPerc,timeStep,block.timestamp, lastClaim,boostMultiplier[0]);
    	return rewards;
   	}
