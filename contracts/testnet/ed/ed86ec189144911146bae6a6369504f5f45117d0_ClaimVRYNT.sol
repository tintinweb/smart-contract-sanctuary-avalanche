...
contract ClaimVRYNT is OwnableUpgradeable {
    IERC20 public _vryntAddress;
    ...
    function initialize(address vryntAddress) external initializer {
        _vryntAddress = IERC20(vryntAddress);
        __Ownable_init();
    }

    function transferVRYNT(address user, uint256 amount) public onlyTransferrer {
        vryntAlloted[user] += amount;
        IERC20(_vryntAddress).transfer(user, amount);
