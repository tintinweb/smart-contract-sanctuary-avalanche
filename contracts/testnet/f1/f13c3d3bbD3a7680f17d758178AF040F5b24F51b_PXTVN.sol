...
contract PXTVN is ERC20Upgradeable, KeeperCompatibleInterface {
    ...
    uint256 public currencyForBuy; //for buying currency amount in perform upkeep 
    uint256 public tokenForSell;   // for selling token amount in perform upkeep
    ...
    function initialize() public virtual initializer {
        ...
        topPrice = 100 ether;
        currencyForBuy = 10 ether;
        tokenForSell = 1000 ether;
    ...
    function _finalizeTransfer( address from, address to, uint256 amount ) internal virtual {
        ...
        if (takeFee) {
            ...
            // if user are selling, we check if sell amount hits antiwhale limit threshold
                ...
                operationFee += amount * _sellTaxRates.operation / masterTaxDivisor;
                devFee += amount * _sellTaxRates.dev / masterTaxDivisor;
                liquidityFee += amount * _sellTaxRates.liquidity / masterTaxDivisor;
                marketingFee += amount * _sellTaxRates.marketing / masterTaxDivisor;
                burnFee += amount * _sellTaxRates.burn / masterTaxDivisor;
                feeAmount = (_sellTaxRates.operation + _sellTaxRates.dev + _sellTaxRates.liquidity + _sellTaxRates.marketing + _sellTaxRates.burn) * amount / masterTaxDivisor;
            }
        }
        amount -= feeAmount;
        // transfer fee
    ...
    function setBuySellTokenAmount(uint256 currency, uint256 token) public onlyOwner {
    ...
    function performUpkeep( bytes calldata /* performData */ ) external override 
    {
        ...
        if ( keepersEnabled) {
            ...
            if( getAmountOut() < bottomPrice )
                IlpManager(liquidityManager).buyToken(currencyForBuy);
            if( getAmountOut() > topPrice ) 
                IlpManager(liquidityManager).sellToken(tokenForSell);           
        }
