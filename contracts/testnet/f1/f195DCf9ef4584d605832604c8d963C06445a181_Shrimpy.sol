    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public AMOUNT; // Amount of Shrimps
    uint256 public PRICE;
    uint256 public MAX_MINT_PER_ADRESS;
    address private devAddress;

    event ShrimpMinted(address minter, uint256 id, string URI);
    ...
    function initialize(
        uint256 mintPrice,
        uint256 amount,
        uint256 amountForOneAdress,
        string memory _uriBase,
        string memory _uriExt
    ) initializer public {
        __ERC721_init("Shrimpy", "SHRIMP");
        ...
        setAmount(amount);
        setMaxMintForOneAdress(amountForOneAdress);
        setURIBase(_uriBase);
        setURIExt(_uriExt);
        setPrice(mintPrice);
        _pause();
    ...
    modifier saleIsOpen() {
        require(_totalSupply() <= AMOUNT, "All Minted");
        if (_msgSender() != owner()) {
    ...
    function setMaxMintForOneAdress(uint256 _newMaxMintPerAdress) public onlyOwner {
        MAX_MINT_PER_ADRESS = _newMaxMintPerAdress;
    }
    ...
    {
        string memory ID = StringsUpgradeable.toString(_id);
    ...
    function safeMint(uint256 amount)
        ...
        saleIsOpen
    {
        require(amount >= 1, "Amount to mint must be more then 1.");
        require(
            balanceOf(msg.sender) < MAX_MINT_PER_ADRESS, 
            "Max mint amount was reached."
        );
        require(
            balanceOf(msg.sender) + amount < MAX_MINT_PER_ADRESS, 
            "Mint a lower number, you overcome the limit on one wallet."
        );
        require(
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            emit ShrimpMinted(_to, tokenId, uri);
        }
    }
    ...
    function withdrawBusiness()
        ...
        nonReentrant
    {
        require(msg.sender == devAddress, "Only the developer can withdraw");
        require(address(this).balance != 0, "Nothing to withdraw");

        payable(devAddress).sendValue( address(this).balance );
    }

    function emergencyChangeDevAddress(address newDevAddress) 
        public 
        onlyOwner 
    {
