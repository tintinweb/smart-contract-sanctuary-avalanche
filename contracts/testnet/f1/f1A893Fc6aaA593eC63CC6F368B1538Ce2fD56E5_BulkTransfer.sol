...
contract BulkTransfer is ERC20, IDapp {
  ...
  /// @param recipients array of addresses of the recipients of tokens on the destination chain.
  /// @param amounts array of amounts of tokens to be transferred cross-chain to each recipient.
  /// @param destChainId chain ID of the destination chain in string.
  /// @param requestMetadata abi-encoded metadata according to source and destination chains.
  function transferBulkCrossChain(
    ...
    require(
      balanceOf(msg.sender) >= getTotalAmount(amounts),
    ...
    _burn(msg.sender, getTotalAmount(amounts));

    bytes memory packet = abi.encode(recipients, amounts);
    bytes memory requestPacket = abi.encode(
      toAddress(bytes(ourContractOnChains[destChainId])),
      packet
    );
