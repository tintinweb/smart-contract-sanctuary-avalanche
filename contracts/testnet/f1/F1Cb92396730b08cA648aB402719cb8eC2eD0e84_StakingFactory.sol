...
contract StakingFactory is RewardsToken {
    ...
    function cloneDeterministic(
        ...
        _initAndEmit(implementation.cloneDeterministic(salt), rewardsToken, stakingToken, current);
        IERC20Mintable(rewardToken).mint(_msgSender(), rewardAmount);
    }

    function _initAndEmit(address instance, address rewardsToken, address stakingToken, uint256 current) private {
        IStakingRewards(instance).initialize(rewardsToken, stakingToken); // khoi tao instance moi
        _clones[current] = instance; // luu vao mapping (để truy xuất các instance bằng index)
        length.increment();
