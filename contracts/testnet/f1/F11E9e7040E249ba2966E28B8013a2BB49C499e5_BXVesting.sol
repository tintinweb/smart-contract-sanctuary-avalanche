...
contract BXVesting is OwnableUpgradeable, PausableUpgradeable {
    ...
    struct Grant {
        ...
        bool lockupStatus; // bool initial percentage vested done or not
    }
    ...
    function calculateGrantClaim(
        ...
        if (elapsedTime >= grantInfo.vestingDuration) {
            ...
            if (grantInfo.lockupDuration > 0 && !grantInfo.lockupStatus) {
                uint256 lockupPeriod = grantInfo.startTime +
                    grantInfo.lockupDuration;
                if (block.timestamp > lockupPeriod) {
                    amountVested = grantInfo
                        .amount
                        .mul(grantInfo.lockupPercentage)
                        .div(10 ** 2);
                    return amountVested.sub(grantInfo.totalClaimed);
                }
            } else {
                amountVested = grantInfo.perSecond.mul(elapsedTime);
                return amountVested.sub(grantInfo.totalClaimed);
            }
        }
    }
    ...
    function claimVestedTokens() external {
        ...
        if (tokenGrant.lockupDuration > 0 && !tokenGrant.lockupStatus) {
            tokenGrant.lockupStatus = true;
        }
