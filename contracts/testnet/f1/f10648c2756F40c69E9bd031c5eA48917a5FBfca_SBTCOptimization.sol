...
import "./BaseERC1155Optimization.sol";
import "./interfaces/ISBTCOptimization.sol";

contract SBTCOptimization is BaseERC1155, ISBTC {
    ...
    function setUnpaidFee(
        ...
        for (uint256 i = 0; i < collections_.length; i++) {
            ...
            if (sbtId_ == 0) sbtId_ = mint(txs_[i].seller);
...
abstract contract BaseERC1155 is ERC1155URIStorage, AccessControlEnumerable, Pausable, ERC1155Supply {
     ...
     * See {ERC1155-_mint}.
     */
    function mint(address account) internal virtual returns (uint256 tokenId) {
        _tokenIds.increment();
