...
contract RebateTreasury is Operator {
    ...
    IUniswapV2Router public JoeRouter;

    mapping (uint256=>uint256) vestingTimeConfig;
    ...
    uint256 pegValue = 101;// DENOMINATOR 100. 1.01
    /*
     * ---------
     * MODIFIERS
     * ---------
    ...
    constructor(address mainToken, address mainTokenOracle, address treasury, address joeRouter) {
        MainToken = IERC20(mainToken);
        MainTokenOracle = IOracle(mainTokenOracle);
        Treasury = ITreasury(treasury);
        JoeRouter = IUniswapV2Router(joeRouter);

        vestingTimeConfig[1] = 10;
        vestingTimeConfig[2] = 20;
        vestingTimeConfig[3] = 30;
        vestingTimeConfig[4] = 40;
        vestingTimeConfig[5] = 50;
    }
    ...
    function bond(address token, uint256 amount, uint256 bondVestingInDays) external onlyAsset(token) {
        // validate bond vesting, min 1 days
        require(shouldOpenBond(), "RebateTreasury: bond is not open");
        require(vestingTimeConfig[bondVestingInDays] > 0, "RebateTreasury: invalid vesting time");
        ...
        schedule.period = schedule.end > newEnd ? schedule.period : bondVesting;
    ...
    // Set MainToken oracle
    function setMainTokenOracle(address oracle) external onlyOwner {
        MainTokenOracle = IOracle(oracle);
    }

    // Set MainToken treasury
    function setTreasury(address treasury) external onlyOwner {
        Treasury = ITreasury(treasury);
    }

    // set schedule address
    function setVestingConfig(uint256 dayKey, uint256 percent) external onlyOwner {// 1-10, 2-20
        vestingTimeConfig[dayKey] = percent;
    }
    ...
    ) external onlyOwner {// not support lp
        assets[token].isAdded = isAdded;
        assets[token].paths = paths;
    }
    ...
    function getMainTokenReturn(address token, uint256 amount, uint256 bondVestingInDays) public view onlyAsset(token) returns (uint256) {
        // uint256 mainTokenPrice = getMainTokenPrice();// 100
        // uint256 tokenPrice = getTokenPrice(token);// 2
        // using route
        Asset memory asset = assets[token];
        uint256 amoutOut = JoeRouter.getAmountsOut(amount, asset.paths)[asset.paths.length - 1];

        uint256 discount = vestingTimeConfig[bondVestingInDays];

        return amoutOut * (100 + discount) / 100;
    }
