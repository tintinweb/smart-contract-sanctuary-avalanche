...
contract NodeManagerV73 is Initializable {
    ...
    event GiftCardPayed(address sender, address token, uint256 amount);
    ...
    function claim() public {
        ...
        uint32 claimFeeRate = rateClaimFee(msg.sender);
        if (dynamicClaimFeeEnabled &&  claimFeeRate > 0) {
    ...
    */
    ...
    function buyGiftCard(
        ...
        } else if (token == uniswapV2Router.WAVAX()) {
            require(msg.value >= amount, "low value");
            payable(address(operationsPoolAddress)).transfer(amount);
        } else {
            _claim();
            require(
                oldRewardsOfUser[msg.sender] >= amount,
        ...
        emit GiftCardPayed(from, token, amount);
    }
}
