...
// error CannotSwitchTeams();

contract WeirdFrensNFT is ERC721, ERC721Enumerable, AccessControl {
  /* ========== STATE VARIABLES ========== */
   ...
   * @param to The address of the user to mint the item to
...
*/
  ...
  function devTestingOnlyCorrupt(
    uint256 tokenId
  ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    Fren storage fren = frens[tokenId];
    fren.isGlitched = true;
  }
  ...
  //     'Only the marketplace contract can transfer tokens'
  //   );
  ...
  function weightedRoll(
    ...
    uint256[] memory cumulativeWeights = new uint256[](50);

    // uint256[] memory levelAdjustedWeights = applyLevelBonuses(weights);

    // // loop through levelAdjustedWeights
    // for (uint256 i = 0; i < levelAdjustedWeights.length; i++) {
    //   console.log('line 1058 levelAdjustedWeights', levelAdjustedWeights[i]);
    // }

    // for (uint256 i = 0; i < levelAdjustedWeights.length; i++) {
    //   cumulativeWeights[i] += levelAdjustedWeights[i];
    // }

    // // loop through cumulativeWeights
    // for (uint256 i = 0; i < cumulativeWeights.length; i++) {
    //   console.log('line 1053 cumulativeWeights', cumulativeWeights[i]);
    // }
   ...
   */
  function getPlayerLevel(address user) external view returns (uint256) {
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    return 'https://weirdfrens.com/api/';
  }
