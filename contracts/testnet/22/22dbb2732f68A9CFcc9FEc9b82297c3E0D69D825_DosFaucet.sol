...
contract DosFaucet is Ownable {
    ...
    function faucet() public payable {
        require(
            ...
            "DOS Faucet: Direct tx is not supported"
        );
        require(
            address(this).balance >= amountPerRequest,
            "DOS Faucet: Insufficient funds"
        );

        payable(_msgSender()).transfer(amountPerRequest);
    }
    ...
    function adminWithdraw(address to) external onlyOwner {
