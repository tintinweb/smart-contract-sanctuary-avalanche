...
contract TeamHunters is ERC721EnumerableV2 {
  ...
  function getBotResult(uint256 teamId) internal returns (uint16 count, uint16 score) {
    Team storage team = teams[teamId];
    count = uint16(TEAM_SIZE >> (team.size * 16));

    uint256 timestamp = block.timestamp;
    require(teams[teamId].startsAt < timestamp, "Invalid state");
    teams[teamId].startsAt = timestamp + BOT_LOCK;

    uint256 botHash = seeder.get(timestamp + count);
  ...
  function bot(uint256 teamId) external {
    address account = msg.sender;

    require(account == tx.origin, "Invalid sender");
    require(ownerOf[teamId] == account, "Invalid owner");

    orb.transferFrom(account, roundToken, BOT_FEE);

    (uint16 count, uint16 score) = getBotResult(teamId);
    botRewards[account] += (score > (count >> 1)) ? ((BOT_FEE * count) / score) : ((BOT_FEE * score) / count);
