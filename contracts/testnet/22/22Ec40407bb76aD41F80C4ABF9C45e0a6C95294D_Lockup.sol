...
contract Lockup is Ownable {
    ...
    constructor (){
        ...
        NFToken = IERC721Metadata(0x2cf27BB321955E502e95E2b59D5310cf0070826c);      // // Node NFT address
    ...
    function unStakeNFT(string memory name) public {
        ...
        (, bool canClaim) = unClaimedReward(name);
        if(canClaim) _claimReward(name, true);
    ...
    function getBoost(int128 duration, uint256 amount) internal view returns (uint8) {
        if (duration < 0 && amount < 500 * 10 ** 9 * 10 ** 18  && amount > 100 * 10 ** 9 * 10 ** 18) return tieA;      // irreversable
