...
contract SuperAdmin is AccessControl, Pausable{
    ...
    constructor(address multiSig,
                address contractsSetter,
                address minter,
                address claimer,
                address manager,
                address royaltySetter,
                address externalProxy,
                address pauser,
                address unpauser,
        ...
        _grantRole(CONTRACTS_ROLE, contractsSetter);
        _grantRole(MINTER_ROLE, minter);
        _grantRole(CLAIMER_ROLE, claimer);
        _grantRole(MANAGER_ROLE, manager);
        _grantRole(ROYALTY_ROLE, royaltySetter);
        _grantRole(EXTERNAL_PROXY_ROLE, externalProxy);
        _grantRole(PAUSER_ROLE, pauser);
        _grantRole(UNPAUSER_ROLE, unpauser);
        _grantRole(MARKETPLACE_MANAGER_ROLE, marketplaceManager);
        _grantRole(FEE_SETTER_ROLE, feeSetter);
        _grantRole(METADATA_UPDATER_ROLE, metadataUpdater);
    }
    ...
    // This function is used for safety purpose if the SuperAdmin contract
    // is redeployed and its address replaced in SuperAdminAddress
    // It should thus never change
    function name() external pure returns (string memory) {
        return 'Farandole SuperAdmin';
    }
}
