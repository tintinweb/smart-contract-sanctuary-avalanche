...
//        require(LevelUtils.getLastIndexBits(snapshot) == 2 ** (256 - _maxLevel), "max Level not match account");
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        ...
        for(uint256 i = 0; i < _amount; i++) {
            ...
            (
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(blockhash(block.number -1), block.timestamp, _tokenId)
                    )
                )
            );
    ...
    function upgradeCard(uint256 _tokenId, uint256 _afterLevel) external {
        ...
        IERC20BurnAndMintUpgradeable(daoTokenAddress).transferFrom(owner, address(this), perSpendAmount);
    ...
    function updateBaseAmount(uint256 _baseUpgrade) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
interface IERC20BurnAndMintUpgradeable {
    ...
    function burnWithAccount(address account, uint256 amount) external;
