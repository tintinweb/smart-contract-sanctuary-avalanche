    ...
    function compileVars(address _account,uint256[3] memory tier) external onlyGuard whenNotPaused returns (uint){
		...
		_total.dailyRewards = Zero;
	    	...
	    	for (i = 0; i < nodes.length; i++) {
			    ...
			    uint rewards = myLib.doPercentage(rewardPerNode,myLib.doPercentage(elapsed,24 hours));
        		    uint boost  = myLib.doPercentage(rewards,rndm_ls[j]);
			    _node.boost = rndm_ls[i];
			    _node.elapsedTime = elapsed;
			    _node.boostTemp = boost - rewards;
			    _node.rewardsTemp = boost;
			    _node.dailyRewards = rewardPerNode;
			    _node.paid = paid;
			    _node.fullPay = full;
