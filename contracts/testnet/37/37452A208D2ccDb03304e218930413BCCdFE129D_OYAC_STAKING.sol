...
contract OYAC_STAKING is Ownable, SignVerify{
    ...
    function lockedSingleReward(uint256 Tid, address user_) public view returns(uint256) {
        ...
        for (uint256 i=0; i<LockedTokenid[user_].length; i++){
            ...
            if(LockedalreadyAwarded[LockedTokenid[user_][_index]] != true &&LockedTokenid[user_][i] == Tid && LockedTokenid[user_][i] > 0) {
    ...
    function unstakelocked(uint256[] memory TokenIds)  external
    {
        ...
        for(uint256 i=0; i<TokenIds.length; i++){
            ...
            LockedUser[msg.sender].lockedAvaialable += lockedSingleReward(TokenIds[i],msg.sender);
