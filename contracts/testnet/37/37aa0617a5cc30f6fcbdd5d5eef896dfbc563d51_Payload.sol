...
contract CreatePayloadOnExecChain is Script {
  function run() public {
    vm.startBroadcast();

    // ----------------- Persist addresses -----------------------------------------------------------------------------
    string memory path = './deployments/avax-deployments.json';
    ...
    bytes32 ipfsHash = 0x0a387fa966f5616423bea53801a843496b1eac5cab5e6bc9426c0958e6496e77;

    IMandateProvider.ExecutionAction[]
      memory actions = new IMandateProvider.ExecutionAction[](1);
    actions[0] = IMandateProvider.ExecutionAction({
      target: address(payload),
      withDelegateCall: true,
      accessLevel: CrossChainMandateUtils.AccessControl.Level_1,
      value: 0,
      signature: 'execute()',
      callData: ''
    });

    uint40 payloadId = IMandateProvider(addresses.mandateProvider)
      .createPayload(actions, ipfsHash);
...
library DeployerHelpers {
  ...
  struct Addresses {
    ...
    address l1VotingStrategy;
    address l2VotingStrategy;
    address rootsWarehouse;
    address votingMachine;
    address executor;
    address mandateProviderImpl;
    address mandateProvider;
  ...
  function decodeJson(string memory path, Vm vm)
    internal
    view
    returns (Addresses memory)
  {
    ...
    Addresses memory addresses = Addresses({
      ...
      clEmergencyOracle: abi.decode(
        persistedJson.parseRaw('.clEmergencyOracle'),
        (address)
      ),
      ccipOnRamp: abi.decode(persistedJson.parseRaw('.ccipOnRamp'), (address)),
      ccipOffRamp: abi.decode(
        persistedJson.parseRaw('.ccipOffRamp'),
        (address)
      ),
      ...
      crossChainManager: abi.decode(
        persistedJson.parseRaw('.crossChainManager'),
        (address)
      ),
      crossChainManagerImpl: abi.decode(
        persistedJson.parseRaw('.crossChainManagerImpl'),
        (address)
      ),
      ccipAdapter: abi.decode(
        persistedJson.parseRaw('.ccipAdapter'),
        (address)
      ),
      ...
      l1VotingStrategy: abi.decode(
        persistedJson.parseRaw('.l1VotingStrategy'),
        (address)
      ),
      l2VotingStrategy: abi.decode(
        persistedJson.parseRaw('.l2VotingStrategy'),
        (address)
      ),
      rootsWarehouse: abi.decode(
        persistedJson.parseRaw('.rootsWarehouse'),
        (address)
      ),
      ...
      mandateProviderImpl: abi.decode(
        persistedJson.parseRaw('.mandateProviderImpl'),
        (address)
      ),
      mandateProvider: abi.decode(
        persistedJson.parseRaw('.mandateProvider'),
        (address)
      ),
      votingPortal: abi.decode(
        persistedJson.parseRaw('.votingPortal'),
        (address)
      ),
      chainId: abi.decode(
        persistedJson.parseRaw('.chainId'),
        (CrossChainUtils.Chains)
      ),
      emergencyRegistry: abi.decode(
        persistedJson.parseRaw('.emergencyRegistry'),
        (address)
      ),
      lzAdapter: abi.decode(persistedJson.parseRaw('.lzAdapter'), (address)),
      lzEndpoint: abi.decode(persistedJson.parseRaw('.lzEndpoint'), (address))
    });
  ...
  function encodeJson(
    ...
    json.serialize('ccipOnRamp', addresses.ccipOnRamp);
    json.serialize('ccipOffRamp', addresses.ccipOffRamp);
    json.serialize('proxyFactory', addresses.proxyFactory);
    json.serialize('proxyAdmin', addresses.proxyAdmin);
    json.serialize('crossChainManager', addresses.crossChainManager);
    json.serialize('crossChainManagerImpl', addresses.crossChainManagerImpl);
    ...
    json.serialize('l1VotingStrategy', addresses.l1VotingStrategy);
    json.serialize('l2VotingStrategy', addresses.l2VotingStrategy);
    json.serialize('rootsWarehouse', addresses.rootsWarehouse);
    json.serialize('votingMachine', addresses.votingMachine);
    json.serialize('executor', addresses.executor);
    json.serialize('mandateProviderImpl', addresses.mandateProviderImpl);
    json.serialize('mandateProvider', addresses.mandateProvider);
    json.serialize('votingPortal', addresses.votingPortal);
    json.serialize('emergencyRegistry', addresses.emergencyRegistry);
    json.serialize('lzAdapter', addresses.lzAdapter);
    json.serialize('lzEndpoint', addresses.lzEndpoint);
    json = json.serialize('chainId', uint8(addresses.chainId));
