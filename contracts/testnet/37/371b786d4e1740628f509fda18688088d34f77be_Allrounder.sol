...
contract Allrounder is IERC20 {
    ...
    //additional Fee
    uint contractReflactionPortion = 500;
   ...
   event TotalDistribution(uint totalFee, uint nativeReflaction, uint liquidityToken, uint tokenForSwap);
    ...
    function _transfer(
        ...
        if(mode == 0 || mode == 1 || mode == 2 || mode == 3 && from == pairAddress){    
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function setPrintToken(address _printerToken, uint _liquidityPercantage) external onlyManager{
