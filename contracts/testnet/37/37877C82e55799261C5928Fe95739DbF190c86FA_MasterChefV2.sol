/**
 *Submitted for verification at testnet.snowtrace.io on 2022-07-10
*/
...
/// @notice The (older) MasterChef contract gives out a constant number of ZAP tokens per block.
/// It is the only address with minting rights for ZAP.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    /// `accZapPerShare` Accumulated ZAPs per share, times 1e12.
    ...
    ///     the ZAP rewards to all the ZSwapSwap products.
    ...
    /// @notice Address of ZAP contract.
    IERC20 public immutable ZAP;

    /// @notice The only address can withdraw all the burn ZAP.
    ...
    ///  @notice 40 zaps per block in MCV1
    uint256 public constant MASTERCHEF_ZAP_PER_BLOCK = 40 * 1e18;
    ...
    uint256 public zapRateToBurn = 643750000000;
    /// @notice ZAP distribute % for regular farm pool
    uint256 public zapRateToRegularFarm = 62847222222;
    /// @notice ZAP distribute % for special pools
    uint256 public zapRateToSpecialFarm = 293402777778;
    ...
    /// @param _MASTER_CHEF The ZSwapSwap MCV1 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of ZAP.
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn ZAP reward from current block in MCV1 pool
    ...
    /// only for ZAP distributions within ZSwapSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
    ...
    /// @notice Harvests ZAP from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    function updateZapRate(
        ...
        require(
            ...
            "MasterChefV2: Zap rate must be greater than 0"
        );
        require(
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough ZAP. If not, harvest from MCV1.
            if (ZAP.balanceOf(address(this)) < _amount) {
