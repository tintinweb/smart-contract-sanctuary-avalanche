...
contract SlideV2 is GameBase, GameBet, GameChainlink {
    ...
    event GameStarted(address indexed creator, uint256 indexed gameId, RoundChoice[] choices, address referrer, uint256 betPerRound, uint256 rakePerRound);
    ...
    function _createGame(
        ...
        for (uint i=0; i<numChoices; i++) {
            ...
            require(maxMark > minMark, "Top end of range must be larger than bottom end of range");
            require(maxMark - minMark >= 10, "Range size must be at least 10");
            uint256 potentialWin = getPayout(minMark, maxMark, details.betPerRound);
            require(potentialWin <= (treasuryAddress.balance / maxBetDivisor), "Gamble less, king");
            minMarks[i] = minMark;
            maxMarks[i] = maxMark;
        }
        ...
        emit GameStarted(msg.sender, fi.gameId, choices, fi.referrer, fi.betPerRound, fi.rakePerRound);
        return fi.gameId;
    }
