...
contract Car is ERC721 {
    ...
    mapping(address => uint64) ownerCarCount;
    mapping(address => uint64[]) ownerToCars;
    ...
    constructor() ERC721("Car Test", "CAR") {
        owner = msg.sender;
        carCount = 0;
        paused = false;
        prices = [30, 60, 130, 240, 325, 360, 435, 490];
        busd = BUSD(0x26a5C8E3D3A8401d22477f1b36bcbc9357DEA009);
        team = msg.sender;
    }
    ...
    function modifyPrices(uint256[] memory _prices) external onlyOwner {
        prices = _prices;
    }

    function mint(uint256 _model, uint256 _level) external notPaused {
        require(prices[_level] > 0, "This is not a valid NFT level");
        busd.transferFrom(msg.sender, team, prices[_level] * 10**18);
    ...
    {
        uint256[] memory _list = new uint256[](_size + 1);
        uint256 _quantity = 0;
        uint64 i;
        for (i = _offset + 1; i < cars.length && _quantity < _size; i++) {
