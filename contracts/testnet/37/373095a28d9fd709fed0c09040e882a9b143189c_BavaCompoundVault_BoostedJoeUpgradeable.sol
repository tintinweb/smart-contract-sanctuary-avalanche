...
contract BavaCompoundVault_BoostedJoeUpgradeable is Initializable, UUPSUpgradeable, BRTERC20Upgradeable {
    ...
    struct VaultInfo {
        ...
        IJoeChef stakingContract;       // Panglin LP Staking contract
    ...
    function initVault(
        ...
        require(address(_stakingContract) != address(0), "0Add");
    ...
    function setAllowances(uint256 _amount) external onlyOwner {
        ...
        //     vaultInfo.lpToken.approve(address(vaultInfo.stakingContract), _amount);
        // }

        if (address(router) != address(0)) {
            WAVAX.approve(address(router), _amount);
            IERC20Upgradeable(IJoePair(address(vaultInfo.lpToken)).token0()).approve(address(router), _amount);
            IERC20Upgradeable(IJoePair(address(vaultInfo.lpToken)).token1()).approve(address(router), _amount);
    ...
    function deposit(uint256 _amount) external {
        ...
        if(vaultInfo.restaking_enabled == true) {
            ...
            // vaultInfo.stakingContract.deposit(vaultInfo.restakingFarmID, restakeAmount);
            _stakeDepositTokens(restakeAmount);
        }
    ...
    function reinvest() external {
        ...
        if(vaultInfo.restaking_enabled == true) {
            // vaultInfo.stakingContract.deposit(vaultInfo.restakingFarmID, liquidity);
    ...
    function _stakeDepositTokens(uint256 _amount) private {
        require(_amount > 0, "MasterChefStrategyV1::_stakeDepositTokens");
        vaultInfo.lpToken.safeTransfer(address(proxy), _amount);
        proxy.deposit(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(vaultInfo.lpToken), _amount);
        proxy.distributeReward(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(0));
    }

    // Withdraw LP token to 3rd party restaking farm
    function _withdrawDepositTokens(uint _amount) internal {
        require(_amount > 0, "#<0");
        // (uint256 depositAmount,) = vaultInfo.stakingContract.userInfo(vaultInfo.restakingFarmID, address(this));
        uint256 depositAmount = _getDepositBalance();

        if (depositAmount > 0) {
            // (uint256 pendingRewardAmount,,,) = vaultInfo.stakingContract.pendingTokens(vaultInfo.restakingFarmID, address(this));
            (uint256 pendingRewardAmount,,) = proxy.pendingRewards(address(vaultInfo.stakingContract), vaultInfo.restakingFarmID);
            if(pendingRewardAmount == 0) {
                // vaultInfo.stakingContract.emergencyWithdraw(vaultInfo.restakingFarmID);
                vaultInfo.lpToken.approve(address(proxy), 0);
                proxy.emergencyWithdraw(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(vaultInfo.lpToken));
            } else if(depositAmount >= _amount) {
                // vaultInfo.stakingContract.withdraw(vaultInfo.restakingFarmID, _amount);
                proxy.withdraw(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(vaultInfo.lpToken), _amount);
            } else {
                // vaultInfo.stakingContract.withdraw(vaultInfo.restakingFarmID, depositAmount);
                proxy.withdraw(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(vaultInfo.lpToken), _amount);
            }
            proxy.distributeReward(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(0));
        }
    ...
    function _getReinvestReward() internal {
        // vaultInfo.stakingContract.deposit(vaultInfo.restakingFarmID, 0);
        proxy.claimReward(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(0));
    }
    ...
    function checkReward() public view returns (uint256) {
        (uint256 pendingJoe, , ) = proxy.pendingRewards(address(vaultInfo.stakingContract), vaultInfo.restakingFarmID);
        return (pendingJoe);
    }

    function _getDepositBalance() internal view returns (uint256 amount) {
        return proxy.poolBalance(address(vaultInfo.stakingContract), vaultInfo.restakingFarmID);
    }
     ...
     */
    function rescueAVAX(address payable _to) external payable onlyOwner {
    ...
    function setVaultInfo(address _stakingPglContract, uint256 _restakingFarmID, address _voterProxy, address _rewardToken, IERC20Upgradeable[] memory _bonusRewardTokens, bool _withUpdate) external onlyOwner {
        require(address(_stakingPglContract) != address(0) , "0Addr");        
        if (_withUpdate) {
            updatePool();
        }
        vaultInfo.stakingContract = IJoeChef(_stakingPglContract);
        vaultInfo.restakingFarmID = _restakingFarmID;
        proxy = IJoeVoterProxy(_voterProxy);
        rewardToken = IERC20Upgradeable(_rewardToken);
        bonusRewardTokens = _bonusRewardTokens;
    ...
    function setBavaMasterFarm(address _BavaMasterFarm, uint256 _bavaPid) external onlyOwner {
    ...
    ****************************************/
    ...
    function _convertWAVAXTokenToDepositToken(uint256 amount) private returns (uint) {
        ...
        path0[1] = IJoePair(address(vaultInfo.lpToken)).token0();
        ...
        path1[0] = path0[0];
        path1[1] = IJoePair(address(vaultInfo.lpToken)).token1();
