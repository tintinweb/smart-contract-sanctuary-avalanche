...
library PoolLib {
    ...
    function distributeInPoolsAtTax(pool storage self, uint256 _amount) public {
        // Calculate treasuy and NEIBR pool shares.
    ...
    ReentrancyGuardUpgradeable
{
    ...
    //************************Pair Variable************************************
    // pair to calculate current liquidity of neighbour
    address public pair;
    ...
    function setPair(address _pair) external onlyOwner {
        require(_pair != address(0), "The Property: Address should not be 0x0");
        pair = _pair;
    }

    //************************Address Variables********************************
    function setNeighbour(address _newAddress) external onlyOwner {
        neighbour = _newAddress;
        pool.setNeighbour(_newAddress);
    }

    // Admin methods
    function setFurniture(address _newAddress) external onlyOwner {
        furniture = _newAddress;
        properties.setFurniture(_newAddress);
    }

    //************************Pool Share Variables*****************************

    function setPool(PoolLib.pool memory _pool) external onlyOwner {
        _pool.precisionValue = 10**5;
        pool = _pool;
    }

    function getPool() external view returns (PoolLib.pool memory) {
        return pool;
    }

    //************************Property Tax Variables**************************
    // Admin methods
    function setTaxDueAllowed(uint256 _newValue) external onlyOwner {
    ...
    function withdraw(uint256 amount) external onlyOwner {
        require(
            ...
            "The Property: Insufficient balance in property."
        );
    ...
    // Method to check if property type exists
    /**
     * @dev Puclic method to check if property type exists.
     * @notice This method let's you check if property tyep exists.
     * @param _propertyTypeIndex Index of Property type to check existance.
     * @return Bool, True if property type exists else false.
     */
     ...
     * @return length of proerty types.
     */
    function getPropertyTypesLength() external view returns (uint256) {
    ...
    function getDailyReward(uint256 _propertyTypeIndex)
        external
        view
        returns (uint256)
    {
        uint256[] memory dailyRewards = propertyTypes
            .get(_propertyTypeIndex)
            .dailyRewards
            .values;
    ...
    function updatePropertyTypeDailyReward(
        uint256 _dailyReward,
        uint256 _propertyTypeIndex
    ) external onlyOwner propertyTypeExists(_propertyTypeIndex) {
    ...
    /**
     * @dev Internal method to mint the property.
     * @param _propertyTypeIndex Property type index
     * @param _name Name of the property
     * @param _tokenURI TokenUri for the token
     ...
     * @notice This method allows you to create new property by paying the main price in Neighbour token
     * @param _propertyTypeIndex Property type index
     * @param _name Name of the property
     * @param _tokenURI TokenUri for the token
     * @param payByReward_  Boolean value if users wants to buy property from reward gained on any particular token
     * @param tokenIds Defualt should be 0 or if wants to pay by reward , tokenIds of the tokens should be passed for which reward should be used for mint.
     **/
    function mint(
        uint256 _propertyTypeIndex,
        string memory _name,
    ...
    /**
     * @dev Public method to get the property.
     * @notice This method allows you to get the property you owned
    ...
    /**
     * @dev Public method to burn the property.
     * @notice This method allows you to burn the property you owned
     * @param tokenId TokenId of the property
     **/
    ...
    /**
     * @dev Public method to get tax amount for a propertyType in Avax.
     * @notice This method allows you to get the tax amount of a property type in AVAX
     * @param _propertyTypeIndex property type index of the property
     **/
    ...
    /**
     * @dev Public method to check if Due tax is cleared.
     * @notice This method allows you to check if your tax for a particular property is cleared.
    ...
    /**
     * @dev Public method to check if property is locked due to insufficient tax payment.
     * @notice This method allows you to check if your property is locked due to defaulter in tax payment.
     * @param tokenId token id of the property
     **/
    ...
    /**
     * @dev Public method to check time till tax is cleared.
     * @notice This method allows you to the time till yout particular tax for a property is cleared.
     * @param tokenId token id of the property
     **/
    ...
    /**
     * @dev Public method to pay tax for a property.
     * @notice This method allows you to pay tax for a particular property.
     * @param tokenIds token ids of the properties
     * @param months No of months for which tax should be payed.
     **/
    function acceptTaxBatch(uint256[] memory tokenIds, uint256[] memory months)
        external
        payable
    {
        uint256 taxAmount = 0;
        uint256[] memory taxCleareds = new uint256[](tokenIds.length);
        require(
            tokenIds.length == months.length,
            "The Property: Lengths must be same."
        );
        for (uint256 index = 0; index < tokenIds.length; index++) {
            require(
            ...
            require(
                !isPropertyLocked(tokenIds[index]),
                "The Property: Property is locked"
            );
            taxAmount +=
                propertyTypes
                    .get(properties.get(tokenIds[index]).propertyTypeIndex)
                    .monthlyTax *
                months[index];

            // update the tax by months.
            properties.acceptTax(tokenIds[index], months[index]);

            taxCleareds[index] = properties
                .get(tokenIds[index])
                .lastTaxDeposited;
        }

        require(
            msg.value >= pool.getDollerInAvax(taxAmount),
            "The Property: Insufficient tax supplied."
        );
        // Share Tax among pools.
        pool.distributeInPoolsAtTax(msg.value);

        emit TaxClearedBatch(
            tokenIds,
            ownerOf(tokenIds[0]),
            taxCleareds,
            msg.value
    ...
    /**
     * @dev Public method to check upcoming due date for tax payment.
     * @notice This method allows you to check upcoming due date for tax payment.
     * @param tokenId token id of the property
     **/
    ...
    /**
     * @dev Public method to check if furniture is allocated to particular property.
     * @notice This method allows you to check if particular furniture is allocated to particular property.
     * @param tokenId token id of the property
     * @param _furnitureId Token id of the furniture
     **/
    ...
    /**
     * @dev Public method to allocated furniture to the property.
     * @notice This method allows you to add furnitur to the property.
     * @param tokenId token id of the property
     * @param _furnitureIds Tokens id of the furniture
     **/
    function allocateFurniture(uint256 tokenId, uint256[] memory _furnitureIds)
        external
        ...
        propertyExists(tokenId)
    {
        ...
        try
            _furniture.allocateToProperty(
                _furnitureIds,
                tokenId,
                ownerOf(tokenId)
            )
        returns (bool) {} catch Error(string memory reason) {
            revert(reason);
        }
        for (uint256 index = 0; index < _furnitureIds.length; index++) {
            properties.allocateFurniture(tokenId, _furnitureIds[index]);
        }

        emit FurnitureAttachedTo(tokenId, _furnitureIds, ownerOf(tokenId));
    }

    /**
     * @dev Public method to deallocated furniture from the property.
     * @notice This method allows you to deallocate the furnitur from the property.
     * @param tokenId token id of the property
     * @param _furnitureIds Tokens id of the furniture
     **/
    function deallocateFurniture(
        uint256 tokenId,
        uint256[] memory _furnitureIds
    ) external proertyOwnerOnly(tokenId) propertyExists(tokenId) {
        // Delloacte the property from furniture contract
        ITheFurniture _furniture = ITheFurniture(furniture);
        try _furniture.deallocateFromProperty(_furnitureIds) returns (
            bool
        ...
        if (
            block.timestamp >= properties.rewardCalculationTime &&
            properties.rewardCalculationTime != 0
        ) {
        ...
        for (uint256 index = 0; index < _furnitureIds.length; index++) {
            ...
            properties.deallocateFurniture(tokenId, _furnitureIds[index]);
        }

        emit FurnitureDetachedFrom(tokenId, _furnitureIds, ownerOf(tokenId));
    }
    ...
    //  Private method to calculate reward.
    /**
     * @dev Private method to calculate reward earned on the property.
     * @param tokenId token id of the property
     **/
    ...
    /**
     * @dev Public method to calculate reward earned on the property.
     * @notice This method allows you to calculate your reward earned on the property.
     * @param tokenId token id of the property
     **/
    ...
    /**
     * @dev Public method to calculate chargable percentage on the claimable reward earned on the property.
     * @notice This method allows you to calculate your chargable percentage on the claimable reward earned on the property.
     * @param tokenId token id of the property
     * @param reward total reward earned
     **/
    function getClaimFeePercentrage(uint256 tokenId, uint256 reward)
        ...
        returns (uint256)
    {
        uint256 _totalSupply = ITheNeighbours(neighbour).balanceOf(pair);
        return
    ...
    /**
     * @dev Public method to calculate chargable fee on the claimable reward earned on the property.
     * @notice This method allows you to calculate your chargable fee on the claimable reward earned on the property.
     * @param tokenId token id of the property
     * @param reward total reward earned
     **/
    ...
    /**
     * @dev Public method to claim reward earned on the property.
     * @notice This method allows you to claim reward earned on the property.
     * @param tokenId token id of the property
     **/
        ...
        nonReentrant
    {
        ...
        (reward, processedTill) = calculateReward(tokenId);
    }

    function claimRewards(uint256[] memory tokenIds) external {
        for (uint256 index = 0; index < tokenIds.length; index++) {
            claimReward(tokenIds[index]);
        }
    }

    // Internal method to calculate required amount to be paid and deduct if from reward.
    /**
     * @dev Internal method to buy property with reward generated on a property.
     * @param tokenIds token ids of the properties
     * @param amount amount to pay to buy property
     **/
    function _payByRewards(uint256[] memory tokenIds, uint256 amount) internal {
        uint256 amountPaid = 0;
        for (uint256 index = 0; index < tokenIds.length; index++) {
            // Get reward calculated till tax is cleared or current time.
            (uint256 reward, uint256 processedTill) = calculateReward(
                tokenIds[index]
            );

            if (amountPaid + reward < amount) {
                // update the reward calculation duration.
                properties.updateReward(tokenIds[index], 0, processedTill);

                // Update the number of amount paid by reward till now.
                amountPaid += reward;
            } else {
                // update the reward calculation duration.
                properties.updateReward(
                    tokenIds[index],
                    reward - (amount - amountPaid),
                    processedTill
                );
        ...
        require(
            amount <= amountPaid,
            "The Property: Insufficient reward to pay the amount"
        );
    }

    /**
     * @dev Public method to buy properties with reward generated on a property.
     * @notice This method allows you to pay for any property by your accumulated reward on a property.
     * @param tokenIds token ids of the properties
     * @param amount amount to pay to buy property
     **/
    function payByReward(
        ...
        if (msg.sender != furniture) {
            return (false, "The Property: Allowed to furniture contract only.");
        }

        for (uint256 index = 0; index < tokenIds.length; index++) {
            if (owner == ownerOf(tokenIds[index]))
                return (
                    false,
                    "The Property: Only Property owner can claim the property's reward"
                );
        }

        // Calculate and deduct the reward.
        _payByRewards(tokenIds, amount);

        emit PaidByReward(tokenIds, ownerOf(tokenIds[0]), msg.sender, amount);
        return (true, "");
    }
    ...
    /**
     * @dev Public method to transfer only property to a address.
     * @notice This method allows you to transfer property from an address to another address.
     * @param from It is the address from which property will be transfered
     * @param to It is the address to which property will be transfered.
     * @param tokenId token id of the property
     **/

    function transferPropertyOnly(
        address from,
        address to,
        uint256 tokenId
    ) external proertyOwnerOnly(tokenId) propertyExists(tokenId) {
        // Claim reward before transferring the NFT
        if (
            block.timestamp >= properties.rewardCalculationTime &&
            properties.rewardCalculationTime != 0
        ) {
    ...
    /**
     * @dev Internal method to transfer property to a address.
     * @param from It is the address from which property will be transfered
     * @param to It is the address to which property will be transfered.
     * @param tokenId token id of the property
     **/
    ...
    function _transfer(
        ...
        // require(
        //     properties.get(tokenId).furnitureIndices.values.length > 0,
        //     "The Property: Can't transfer property without owning furniture."
        // );
        // Claim reward before transferring the NFT
        if (
            block.timestamp >= properties.rewardCalculationTime &&
        ...
        returns (bool data) {} catch Error(string memory reason) {}
    }

    /**
     * @dev Private method to check array is subArray of superArray.
     * @param superArray superArray to check upon
     * @param subArray array ro check
     **/
    ...
    /**
     * @dev Public method to transfer furnitures to new owner if missed due to gas issues.
     * @param tokenId tokenId of the property
     * @param from address of previous owner
     * @param tokenIds array of tokenIds of furnitures.
     **/
    function transferPendingProperties(
        uint256 tokenId,
        address from,
        uint256[] memory tokenIds
    ) external propertyExists(tokenId) {
        require(
            arrayIsSubArray(
                properties.get(tokenId).furnitureIndices.values,
                tokenIds
            ),
            "The Property: TokenIds must be of current property only."
        );
        // Transfer all furnitures.
        ITheFurniture _furniture = ITheFurniture(furniture);
