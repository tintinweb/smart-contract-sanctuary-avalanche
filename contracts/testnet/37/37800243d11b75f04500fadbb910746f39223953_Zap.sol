...
contract Zap is OwnableUpgradeable {
    ...
    uint256 public constant maxSlippageTolerance = 100; // 10%
    ...
    function zapInToken(
        ...
        require(_slippageTolerance <= maxSlippageTolerance, "Slippage out of range");
        ...
        if (_from == token0 || _from == token1) {
            ...
            uint256 otherAmount = _swap(fromToken, sellAmount, other, address(this), _slippageTolerance);
            ...
            if (_from == USDC) {
                ...
                usdcAmount = _swap(_from, amount, USDC, address(this), _slippageTolerance);
            }
            _swapUSDCToLP(_to, usdcAmount, msg.sender, _slippageTolerance);
        }
    }
    ...
    function zapOutToken(address _from, uint amount, address _to, uint256 _slippageTolerance) external {
        ...
        if (token0 != _to) {
            amountA = _swap(token0, amountA, _to, address(this), _slippageTolerance);
        }
        if (token1 != _to) {
            amountB = _swap(token1, amountB, _to, address(this), _slippageTolerance);
        }
    ...
    function _swap(
        ...
        uint256[] memory amountOutEstimates = ROUTER.getAmountsOut(amount, path);
        uint256 amountOutEstimate = amountOutEstimates[amountOutEstimates.length - 1];
        // Slippage tolerance: 0.1% = 1
        uint256 amountOutMin = amountOutEstimate.sub(amountOutEstimate.mul(_slippageTolerance).div(1000));
    ...
    function _swapUSDCToLP(
        ...
        if (token0 == USDC) {
            ...
            token0Amount = _swap(USDC, swapValue, token0, address(this), _slippageTolerance);
        }

        if (token1 == USDC) {
            token1Amount = amount.sub(swapValue);
        } else {
            token1Amount = _swap(USDC, amount.sub(swapValue), token1, address(this), _slippageTolerance);
        }
        if (token0Amount > 0 && token1Amount > 0) {
            _approveTokenIfNeeded(token0);
            _approveTokenIfNeeded(token1);
            (uint256 amountA, uint256 amountB, ) = ROUTER.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, receiver, block.timestamp);
