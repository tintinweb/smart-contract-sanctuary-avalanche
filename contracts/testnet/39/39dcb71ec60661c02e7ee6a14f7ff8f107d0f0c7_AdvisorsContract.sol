...
contract AdvisorsContract {
    ...
    function addWhitelist(address user,uint256 amount) public onlyOwner{
        ...
        require(amountConverted > 10, "min amount to send");
        require(balance > amountConverted,"not enough BFG available");
        
        ownedBFG[user] += amountConverted;
        lockedBFG[user] += amountConverted;
    }

    function getDaysUnlocked(uint8 daysPast, address _receiver) public{
        ...
        //transfer
        if(daysPast == 720){
            itoken.transfer(_receiver,lockedBFG[_receiver]);
            balance-=lockedBFG[_receiver];
            lockedBFG[_receiver] = 0;

            vestingCycles = vestingCycles + daysPast;
            
            emit TransferSent(address(this),_receiver,lockedBFG[_receiver]);
        }
        if(daysPast > 721){
    ...
    function Withdraw() public {
		...
		if(vestingCycles == 0){
			...
			lastUnlockTime = lastUnlockTime + 1 minutes;
            vestingCycles ++;
            return;
		}

        //after 12 months, for 24 Months 0.139% each day linaer (100%)
		if(vestingCycles > 0 && vestingCycles <= 720){
			require(block.timestamp > lastUnlockTime + 1 minutes, "Too early for unlock");
			//need to calculate days
			//uint8 daysPast = uint8((block.timestamp - lastUnlockTime) / 60 / 60 / 24);
			uint8 daysPast = uint8((block.timestamp - lastUnlockTime) / 60);
            require(daysPast > 0, "Too early for unlock");
			getDaysUnlocked(daysPast, msg.sender);
		}
        //send all last vesting
        if(vestingCycles ==  720){
            itoken.transfer(msg.sender,lockedBFG[msg.sender]);
            balance-=lockedBFG[msg.sender];
            lockedBFG[msg.sender] = 0;

            emit TransferSent(address(this),msg.sender,lockedBFG[msg.sender]);
        }
        //send the rest if any after last vesting
		if(vestingCycles > 721){
            require(lockedBFG[msg.sender] > 0, "No more tokens for unlock");
			//transfer
			itoken.transfer(msg.sender,lockedBFG[msg.sender]);
            balance-=lockedBFG[msg.sender];
            lockedBFG[msg.sender] = 0;
