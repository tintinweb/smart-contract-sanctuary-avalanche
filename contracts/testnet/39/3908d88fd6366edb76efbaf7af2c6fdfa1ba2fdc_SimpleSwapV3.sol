...
// File: contracts/SimpleSwapV3.sol
...
contract SimpleSwapV3 {
    ...
    // Stores the balance of each address interacting with the contract
    // For simplicity, We are not using ERC-20 tokens for this project
    mapping(address => uint256) token1Balance;
    mapping(address => uint256) token2Balance;

    // Make sure that the amount is not 0 and that the caller has enough tokens
    modifier validAmountCheck(mapping(address => uint256) 
    storage _balance, uint256 _qty) {
        require(_qty > 0, "Amount cannot be zero!");
        require(_qty <= _balance[msg.sender], "Insufficient amount");
        _;
    }
    
    // Will throw error if liquidity has not been added to the pool
    modifier activePool() {
        require(totalShares > 0, "Zero Liquidity");
        _;
    }

    // This is a faucet to allow a user to request tokes of each type, enabling them 
    // to interact with the AMM contract
    function faucet(uint256 _amountToken1, uint256 _amountToken2) 
    external {
    ...
    external
    view 
    returns(uint256 amountToken1, uint256 amountToken2, uint256 myShare) {
        amountToken1 = token1Balance[msg.sender];
        amountToken2 = token2Balance[msg.sender];
        myShare = shares[msg.sender];
    }
    
    // Returns the total amount of tokens locked in the pool and the total shares 
    // issued corresponding to it
    ...
    // Returns amount of Token1 required when providing liquidity with _amountToken2 
    // quantity of Token2
    ...
    // Returns amount of Token2 required when providing liquidity with _amountToken1 
    // quantity of Token1
    ...
    // Adding new liquidity in the pool x*y=k
    // Returns the amount of share issued for locking given assets
    function provide(uint256 _amountToken1, uint256 _amountToken2) 
    external validAmountCheck(token1Balance, _amountToken1) validAmountCheck(token2Balance, _amountToken2) 
    returns(uint256 share) {
        if(totalShares == 0) { // Genesis liquidity is issued 100 Shares
            share = 100*PRECISION;
        } else{
            // if liquidity has already been provided assure that the correct value for each token is provided.
            // The front end will automatically fill in a token when you are adding a value to one of the fields
        ...
        // reduce the callers tokens and store new amount they have
        ...
        // add the tokens to the pools liquidity
    ...
    external activePool validAmountCheck(shares, _share) 
    returns(uint256 amountToken1, uint256 amountToken2) {
    ...
    // Returns the amount of Token2 that the user will get when swapping a given amount of Token1
    ...
    external activePool validAmountCheck(token1Balance, _amountToken1) 
    returns(uint256 amountToken2) {
    ...
    external activePool validAmountCheck(token2Balance, _amountToken2) 
    returns(uint256 amountToken1) {
