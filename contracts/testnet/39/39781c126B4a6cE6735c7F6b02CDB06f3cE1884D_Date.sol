...
contract Date is Ownable, ERC721 {
  ...
  constructor() ERC721("Date", "DATE") {
    setBaseURI("http://localhost:3009/token/");
  ...
  function mint(
    ...
    address_to_id[msg.sender].push(tokenId);
  }
  ...
  {
    return address_to_id[ownerAddress];
  }
  ...
  function id(
    ...
    return
      (uint256(year) - 1) * 372 + (uint256(month) - 1) * 31 + uint256(day) - 1;
  }
  ...
  function pseudoRNG(
    ...
    return
      uint256(
        keccak256(
          abi.encode(block.timestamp, block.difficulty, year, month, day, title)
        )
      );
  }
}