...
/** 
 *  SourceUnit: c:\Projects\snakecity\blockchain\smart-contracts\contracts\utils\TokenVesting.sol
*/
...
contract TokenVesting is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event PoolCreated(uint256 poolId, uint256 startTime, uint256 cliff, uint256 vestingCliff, uint256 vestingDuration, uint256 tgePercent);
    event PoolRemoved(uint256 poolId);
    event TokenAdded(uint256 poolId, address account, uint256 amount);
    ...
    function createPool(uint256 _poolId, uint256 _startTime, uint256 _cliff, uint256 _vestingCliff, uint256 _vestingDuration, uint256 _tgePercent)
        external
        onlyOwner
    {
        ...
        emit PoolCreated(_poolId, _startTime, _cliff, _vestingCliff, _vestingDuration, _tgePercent);
    }
    ...
    function addToken(uint256 _poolId, address[] memory _accounts, uint256[] memory _amounts)
        external
        nonReentrant
        onlyOwner
        poolExist(_poolId)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit TokenAdded(_poolId, account, amount);
        }
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 amount = getBalance(poolId, msgSender);
    ...
    function getBalance(uint256 _poolId, address _account)
        public
        view
        returns (uint256)
    {
