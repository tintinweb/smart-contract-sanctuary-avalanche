...
contract BondingTreasuryV1 is OwnableUpgradeable, BondingTreasuryStorageV1 {
    ...
    function bond(uint256 _amount) external virtual returns (bool) {
        ...
        EmeraldToken.mintSupply(treasuryAccount, _amount * (10**18));
    ...
    function quatercalulation(uint256 _amount) public {
         uint256 count = 0;
    ...
    function viewUserDiscountedDetails(address _user)
        ...
        for (uint256 i = 0; i < UserBondDetails[_user].length; i++) {
            ...
            if (BondDetailsById[data.bondId].endedAt <= block.timestamp) {
                TotalAmount += data.amount * 10*(10**18);
            }
        }
