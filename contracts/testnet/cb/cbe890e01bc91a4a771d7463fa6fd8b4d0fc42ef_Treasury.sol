...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    address public sncOracle;

    // price
    uint256 public sncPriceOne;
    uint256 public sncPriceCeiling;
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of SNC price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    uint256 public override previousEpochSncPrice;
    uint256 public allocateSeigniorageSalary;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra SNC during debt phase
    ...
    address[] public sncLockedAccounts;
    ...
    event RedeemedBonds(address indexed from, uint256 sncAmount, uint256 bondAmount);
    event BoughtBonds(address indexed from, uint256 sncAmount, uint256 bondAmount);
    ...
    modifier checkEpoch() {
        ...
        epoch_ = epoch_.add(1);
        epochSupplyContractionLeft = (getSncPrice() > sncPriceCeiling) ? 0 : IERC20(snc).totalSupply().mul(maxSupplyContractionPercent).div(10000);
    }

    modifier checkOperator() {
        require(
            IBasisAsset(snc).operator() == address(this) &&
    ...
    function getPegPrice() external override view returns (int256) {
        return IOracle(sncOracle).getPegPrice();
    }

    function getPegPriceUpdated() external override view returns (int256) {
        return IOracle(sncOracle).getPegPriceUpdated();
    }

    // oracle
    function getSncPrice() public override view returns (uint256 sncPrice) {
        try IOracle(sncOracle).consult(snc, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult SNC price from the oracle");
        }
    }

    function getSncUpdatedPrice() public override view returns (uint256 _sncPrice) {
        try IOracle(sncOracle).twap(snc, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult SNC price from the oracle");
        }
    ...
    function getBurnableSncLeft() external view returns (uint256 _burnableSncLeft) {
        uint256 _sncPrice = getSncPrice();
        if (_sncPrice <= sncPriceOne) {
            uint256 _bondMaxSupply = IERC20(snc).totalSupply().mul(maxDebtRatioPercent).div(10000);
            uint256 _bondSupply = IERC20(sbond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnableSnc = _maxMintableBond.mul(getBondDiscountRate()).div(1e18);
                _burnableSncLeft = Math.min(epochSupplyContractionLeft, _maxBurnableSnc);
            }
        }
    }
    ...
    function getRedeemableBonds() external view returns (uint256 _redeemableBonds) {
        uint256 _sncPrice = getSncPrice();
        if (_sncPrice > sncPriceCeiling) {
            uint256 _totalSnc = IERC20(snc).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalSnc.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public override view returns (uint256 _rate) {
        ...
        if (_sncPrice <= sncPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = sncPriceOne.mul(1e18).div(_sncPrice); // to burn 1 SNC
                uint256 _discountAmount = _bondAmount.sub(sncPriceOne).mul(discountPercent).div(10000);
                _rate = sncPriceOne.add(_discountAmount);
    ...
    function getBondPremiumRate() public override view returns (uint256 _rate) {
        ...
        if (_sncPrice > sncPriceCeiling) {
            if (premiumPercent == 0) {
                ...
                uint256 _premiumAmount = _sncPrice.sub(sncPriceOne).mul(premiumPercent).div(10000);
                _rate = sncPriceOne.add(_premiumAmount);
                if (maxDiscountRate > 0 && _rate > maxDiscountRate) {
                    _rate = maxDiscountRate;
                }
            }
    ...
    function getSncLockedBalance() public override view returns (uint256 _lockedBalance) {
        uint256 _length = sncLockedAccounts.length;
        IERC20 _snc = IERC20(snc);
        for (uint256 i = 0; i < _length; i++) {
            _lockedBalance = _lockedBalance.add(_snc.balanceOf(sncLockedAccounts[i]));
        }
    }

    function getSncCirculatingSupply() public override view returns (uint256) {
        return IERC20(snc).totalSupply().sub(getSncLockedBalance());
    }
    ...
    function getNextExpansionRate() public override view returns (uint256 _rate) {
        if (epoch_ < bootstrapEpochs) {// 28 first epochs with 4.5% expansion
            ...
            uint256 _twap = getSncUpdatedPrice();
            if (_twap >= sncPriceCeiling) {
                uint256 _percentage = _twap.sub(sncPriceOne); // 1% = 1e16
    ...
    function getNextExpansionAmount() external override view returns (uint256) {
        ...
        return getSncCirculatingSupply().mul(_rate).div(1e6);
    }
    ...
    function initialize(
        ...
        sncPriceOne = 10**18; // This is to allow a PEG of 1 SNC per DARK
        sncPriceCeiling = sncPriceOne.mul(1001).div(1000);

        maxSupplyExpansionPercent = 200; // Upto 2.0% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for boardroom
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn SNC and mint SBOND)
        maxDebtRatioPercent = 4500; // Upto 45% supply of SBOND to purchase
        ...
        seigniorageSaved = IERC20(snc).balanceOf(address(this));
    ...
    function setGShareRewardPool(address _gshareRewardPool) external onlyOperator {
        gshareRewardPool = _gshareRewardPool;
    }

    function setGShareRewardPoolRates(uint256 _gshareRewardPoolExpansionRate, uint256 _gshareRewardPoolContractionRate) external onlyOperator {
    ...
    function setSncOracle(address _sncOracle) external onlyOperator {
        sncOracle = _sncOracle;
    }

    function setSncPriceCeiling(uint256 _sncPriceCeiling) external onlyOperator {
        require(_sncPriceCeiling >= sncPriceOne && _sncPriceCeiling <= sncPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
        sncPriceCeiling = _sncPriceCeiling;
    }
    ...
    function setNextSupplyTarget(uint256 _target) external onlyOperator {
        require(_target > IERC20(snc).totalSupply(), "too small");
        nextSupplyTarget = _target;
    }

    function setSncLockedAccounts(address[] memory _sncLockedAccounts) external onlyOperator {
        delete sncLockedAccounts;
        uint256 _length = _sncLockedAccounts.length;
        for (uint256 i = 0; i < _length; i++) {
            sncLockedAccounts.push(_sncLockedAccounts[i]);
        }
    }

    /* ========== MUTABLE FUNCTIONS ========== */

    function _updateSncPrice() internal {
        try IOracle(sncOracle).update() {} catch {}
    }

    function buyBonds(uint256 _sncAmount, uint256 targetPrice) external override onlyOneBlock checkOperator nonReentrant {
        require(_sncAmount > 0, "Treasury: cannot purchase bonds with zero amount");

        uint256 sncPrice = getSncPrice();
        require(sncPrice == targetPrice, "Treasury: SNC price moved");
        require(
            sncPrice < sncPriceOne, // price < $1
            "Treasury: sncPrice not eligible for bond purchase"
        );

        require(_sncAmount <= epochSupplyContractionLeft, "Treasury: not enough bond left to purchase");

        uint256 _rate = getBondDiscountRate();
        require(_rate > 0, "Treasury: invalid bond rate");

        address _snc = snc;
        uint256 _bondAmount = _sncAmount.mul(_rate).div(1e18);
        uint256 _sncSupply = IERC20(snc).totalSupply();
        uint256 newBondSupply = IERC20(sbond).totalSupply().add(_bondAmount);
        require(newBondSupply <= _sncSupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(_snc).burnFrom(msg.sender, _sncAmount);
        IBasisAsset(sbond).mint(msg.sender, _bondAmount);

        epochSupplyContractionLeft = epochSupplyContractionLeft.sub(_sncAmount);
        _updateSncPrice();
        if (regulationStats != address(0)) IRegulationStats(regulationStats).addBonded(epoch_, _bondAmount);

        emit BoughtBonds(msg.sender, _sncAmount, _bondAmount);
    }
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external override onlyOneBlock checkOperator nonReentrant {
        ...
        require(sncPrice == targetPrice, "Treasury: SNC price moved");
        require(
            sncPrice > sncPriceCeiling, // price > $1.01
            "Treasury: sncPrice not eligible for bond purchase"
        );
        ...
        uint256 _sncAmount = _bondAmount.mul(_rate).div(1e18);
        require(IERC20(snc).balanceOf(address(this)) >= _sncAmount, "Treasury: treasury has no more budget");

        seigniorageSaved = seigniorageSaved.sub(Math.min(seigniorageSaved, _sncAmount));
        allocateSeigniorageSalary = 1 ether; // 1 SNC salary for calling allocateSeigniorage()

        IBasisAsset(sbond).burnFrom(msg.sender, _bondAmount);
        IERC20(snc).safeTransfer(msg.sender, _sncAmount);

        _updateSncPrice();
        if (regulationStats != address(0)) IRegulationStats(regulationStats).addRedeemed(epoch_, _sncAmount);

        emit RedeemedBonds(msg.sender, _sncAmount, _bondAmount);
    }

    function _sendToBoardroom(uint256 _amount, uint256 _expanded) internal {
        address _snc = snc;
        IBasisAsset(_snc).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(_snc).transfer(daoFund, _daoFundSharedAmount);
        }
        ...
        if (marketingFundSharedPercent > 0) {
            ...
            IERC20(_snc).transfer(marketingFund, _marketingFundSharedAmount);
        }
        ...
        if (insuranceFundSharedPercent > 0) {
            ...
            IERC20(_snc).transfer(insuranceFund, _insuranceFundSharedAmount);
        }

        _amount = _amount.sub(_daoFundSharedAmount).sub(_marketingFundSharedAmount).sub(_insuranceFundSharedAmount);

        IERC20(_snc).safeIncreaseAllowance(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_amount);

        if (regulationStats != address(0)) IRegulationStats(regulationStats).addEpochInfo(epoch_.add(1), previousEpochSncPrice, _expanded,
            _amount, _daoFundSharedAmount, _marketingFundSharedAmount, _insuranceFundSharedAmount);
        emit FundingAdded(epoch_, block.timestamp, previousEpochSncPrice, _expanded,
            _amount, _daoFundSharedAmount, _marketingFundSharedAmount, _insuranceFundSharedAmount);
    }

    function allocateSeigniorage() external onlyOneBlock checkEpoch checkOperator nonReentrant {
        _updateSncPrice();
        previousEpochSncPrice = getSncPrice();
        address _snc = snc;
        uint256 _supply = getSncCirculatingSupply();
        ...
        if (epoch_ < bootstrapEpochs) {
            ...
            if (previousEpochSncPrice > sncPriceCeiling) {
                IBoardroom(boardroom).setWithdrawFee(0);
                // Expansion ($SNC Price > 1 $CRO): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(sbond).totalSupply();
                uint256 _percentage = previousEpochSncPrice.sub(sncPriceOne);
                ...
                if (_savedForBoardroom > 0) {
                    ...
                    // function addEpochInfo(uint256 epochNumber, uint256 twap, uint256 expanded, uint256 boardroomFunding, uint256 daoFunding, uint256 marketingFunding, uint256 insuranceFunding) external;
                    if (regulationStats != address(0)) IRegulationStats(regulationStats).addEpochInfo(epoch_.add(1), previousEpochSncPrice, 0, 0, 0, 0, 0);
                    emit FundingAdded(epoch_.add(1), block.timestamp, previousEpochSncPrice, 0, 0, 0, 0, 0);
                }
                if (_savedForBond > 0) {
                    seigniorageSaved = seigniorageSaved.add(_savedForBond);
                    IBasisAsset(_snc).mint(address(this), _savedForBond);
            ...
            } else if (previousEpochSncPrice < sncPriceOne) {
        ...
        if (allocateSeigniorageSalary > 0) {
            IBasisAsset(_snc).mint(address(msg.sender), allocateSeigniorageSalary);
        }
    }
