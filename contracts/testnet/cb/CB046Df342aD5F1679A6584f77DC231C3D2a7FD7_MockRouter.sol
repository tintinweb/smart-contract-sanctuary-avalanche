...
contract MockRouter {
    ...
    function registerAdditionalLottery(
        ...
        linkToken.transferAndCall(
            ...
            abi.encodeWithSignature(
                "register(string name,bytes encryptedEmail,address upkeepContract,uint32 gasLimit,address adminAddress,bytes checkData,uint96 amount,uint8 source)",
                name,
                encryptedEmail,
                lottery,
                gasLimit,
    ...
    function cancelKeperAdditionalLottery(uint256 id_, address lottery)
        external
    {
        (bool success, bytes memory returnData) = keeperRegistry.call(
            abi.encodeWithSignature("cancelUpkeep(uint256 id))", id_)
        );
