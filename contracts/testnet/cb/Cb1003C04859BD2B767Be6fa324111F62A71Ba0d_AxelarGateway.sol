...
contract AxelarGateway is IAxelarGateway, AdminMultisigBase {
    ...
    function deployToken(bytes calldata params, bytes32) external onlySelf {
        ...
        if (tokenAddress == address(0)) {
            ...
            _setToken(symbol, Token.InternalBurnableFrom);
        } else {
            ...
            _setToken(symbol, Token.External);
        }
    ...
    function burnToken(bytes calldata params, bytes32) external onlySelf {
        ...
        if (_getToken(symbol) == Token.External) {
    ...
    function _burnTokenFrom(
        ...
        if (token == Token.InternalBurnableFrom) {
            burnSuccess = _callERC20Token(
                tokenAddress,
    ...
    function _getToken(string memory symbol) internal view returns (Token) {
        return Token(getUint(_getTokenKey(symbol)));
    }
    ...
    function _setToken(string memory symbol, Token token) internal {
        _setUint(_getTokenKey(symbol), uint256(token));
    }
