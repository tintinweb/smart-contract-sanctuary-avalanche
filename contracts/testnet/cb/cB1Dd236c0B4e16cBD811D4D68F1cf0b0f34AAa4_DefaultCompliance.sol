...
contract DefaultCompliance is ICompliance, AgentRole, ComplianceStorage, Initializable {
     ...
     *  @param _holdReleaseTimestamp the holder limit for the token concerned
     *  emits a `HolderLimitSet` event
     */
    function init(address _token, uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser) external initializer{
        token = IToken(_token);
        bindToken(address(token));
        _tokenLimit = _tokenLimitPerUser;
        _identityRegistry = token.identityRegistry();
        _holdRelease = _holdReleaseTimestamp;
        emit ReleaseTimestampSet(_holdReleaseTimestamp);
    }
