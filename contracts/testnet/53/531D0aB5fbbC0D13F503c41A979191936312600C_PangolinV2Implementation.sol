...
contract PangolinV2Implementation is IDexImplementation {
  ...
  function swapExactInput(bytes calldata info, IDomaniDex.SwapParams memory inputParams)
    external
    override
    returns (IDomaniDex.ReturnValues memory returnValues)
  {
    // insantiate router
    IPangolinRouter pangolinRouter = IPangolinRouter(decodeImplementationData(info));

    // unpack the extraParams
    address[] memory tokenSwapPath = decodeExtraData(inputParams.extraData);

    uint256 lastTokenIndex = tokenSwapPath.length - 1;

    bool isNativeOutput = inputParams.isNative;
  ...
  function swapExactOutput(bytes calldata info, IDomaniDex.SwapParams memory inputParams)
    external
    payable
    override
    returns (IDomaniDex.ReturnValues memory returnValues)
  {
    ...
    bool isNativeInput = inputParams.isNative;
    ...
    if (isNativeInput) {
      returnValues.inputAmount = pangolinRouter.swapAVAXForExactTokens{
        value: inputParams.minOutOrMaxIn
      }(inputParams.exactAmount, tokenSwapPath, address(this), inputParams.expiration)[0];
    } else {
      IERC20(tokenSwapPath[0]).safeIncreaseAllowance(
        address(pangolinRouter),
        inputParams.minOutOrMaxIn
  ...
  function decodeImplementationData(bytes calldata info) internal pure returns (address) {
    return abi.decode(info, (address));
  }
