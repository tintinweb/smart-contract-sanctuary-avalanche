...
contract Academy is FeeCollector, VRFConsumerV2Ownable {
    RawMaterialNFT public immutable RawMaterialNftContract;
    uint256 public constant RESEARCH_TIME = 24 hours;
    IBlueprint public immutable BluePrintsContract;
    ...
    event ResearchStarted(uint256 targetResource, uint256 landTokenId, uint256 slotId);
    event ResearchFinished(address user, uint256 jobId);
    ...
    struct Request {
        address userAddress;
        uint32 requestedBlueprintCategory;
        uint32 level;
        uint32 jobId;
    }
    ...
    constructor(
        ...
        address _bluePrintsNftContract,
        uint64 subscriptionId
    )
        FeeCollector(_smolApaLandAddress, _feeAggregator, _landRegistery, AcademyType)
        VRFConsumerV2Ownable(subscriptionId)
    {
        BluePrintsContract = IBlueprint(_bluePrintsNftContract);
        RawMaterialNftContract = RawMaterialNFT(_rawMaterialNftContract);
    }

    function setOutcomeDictionary(uint256 _targetResource, uint256 _tresholdlevel) external onlyOwner {
        OutcomeDictionary[_targetResource] = _tresholdlevel;
        emit OutcomeDictionarySet(_targetResource, _tresholdlevel);
    }

    function startResearch(uint256 _targetBlueprintCategory, uint256 _landTokenId, uint256 _slotId) external payable {
        ...
        if (OutcomeDictionary[_targetBlueprintCategory] > level) {
            revert LevelTooLow();
        }

        if (OutcomeDictionary[_targetBlueprintCategory] == 0) {
        ...
        RawMaterialNftContract.burnRawMaterial(PAPER, 1, msg.sender);
        uint256 completedTimeStamp = block.timestamp + RESEARCH_TIME;
        slotInfo[_landTokenId][_slotId] = completedTimeStamp; // update the timeslot
        uint256 jobId = _landTokenId * 100 + _slotId;
        JobsDictionary[msg.sender][jobId] = Job(uint32(completedTimeStamp), uint32(_targetBlueprintCategory));
        emit ResearchStarted(_targetBlueprintCategory, _landTokenId, _slotId); // TODO Fees should be inferred from the graph
    }

    function getMaxSlots(uint256 _level) internal pure returns (uint256) {
        if (_level <= 2) {
            return 1;
        } else if (_level <= 4) {
            return 2;
    ...
    function claim(uint256 _landTokenId, uint256 _slotId) external {
        ...
        PlayerRequest[s_requestId] = Request(msg.sender, uint32(job.targetMaterial), uint32(level), uint32(jobId));
    }

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        Request memory vrfRequest = PlayerRequest[requestId];

        uint256 dropChance = getSuccessChances(vrfRequest.requestedBlueprintCategory, vrfRequest.level);

        if (dropChance > (randomWords[0] % 1000)) {
            BluePrintsContract.mintBlueprint(
                vrfRequest.requestedBlueprintCategory, randomWords[0], vrfRequest.userAddress
            );
        }

        delete PlayerRequest[requestId];
        emit ResearchFinished(vrfRequest.userAddress, vrfRequest.jobId);
    }

    function getSuccessChances(uint256 _targetBlueprintCategory, uint256 _level) internal pure returns (uint256) {
        uint256 dropChance;
        if (_targetBlueprintCategory == COMMON_FORMULA || _targetBlueprintCategory == COMMON_SCHEMATIC) {
            dropChance = 200;
        } else if (_targetBlueprintCategory == RARE_FORMULA || _targetBlueprintCategory == RARE_SCHEMATIC) {
            dropChance = 100;
        } else if (_targetBlueprintCategory == LEGENDARY_FORMULA || _targetBlueprintCategory == LEGENDARY_SCHEMATIC) {
            dropChance = 10;
        }
