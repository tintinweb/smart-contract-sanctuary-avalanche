...
contract SmOlLeGeNd is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, Randomizer {
    ...
    uint256 public maxPerPerson = 2;
    uint256 public price = 2 ether;
    address public royaltyAddress = 0x9EE4f72B8244009C07786801b5D33c554568C7f3;
    uint256 public royalty = 800;
    bytes32 internal merkleRoot;
    address[] private devList;

    // MAPPINGS
    mapping(address => uint) public allowlistMinted;
    mapping(address => DevFee) public devFees;

    constructor(address[] memory _devList, uint256[] memory _fees)
        ERC721("SmOlLeGeNd", "SmOlLeGeNd")
        Randomizer(MAX_SUPPLY)
    {
    ...
    function checkAllowlisted(address _minter, bytes32[] calldata _merkleProof)
        public
        view
        returns (bool)
    {
        ...
        return
            ...
            allowlistMinted[_minter] < maxPerPerson;
    }
    ...
    function allowlistMint(uint256 amount, bytes32[] calldata merkleProof) external payable whenNotPaused {
        uint256 supply = totalSupply();
        require(phase == 1, "Not allowlist phase yet");
        require(supply + amount - 1 < MAX_SUPPLY, "Error: TotalSupply reached");
        require(amount > 0 && amount <= maxPerTx, "Error: max per tx limit");
        require(balanceOf(msg.sender) < maxPerPerson, "Error: Max per address limit");
        if (price > 0) require(msg.value == price * amount, "Error: Invalid price");
        require(checkAllowlisted(msg.sender, merkleProof), "Error: Invalid proof");
    ...
    function publicMint(uint256 amount) public payable whenNotPaused {
        uint256 supply = totalSupply();
        require(phase == 2, "Not allowlist phase yet");
        require(supply + amount - 1 < MAX_SUPPLY, "Error: TotalSupply reached");
        require(amount > 0 && amount <= maxPerTx, "Error: max per tx limit");
    ...
    function internalMint(address to) internal {
        uint256 tokenId = _genNonce() + START_ID;
        _safeMint(to, tokenId);
    }
