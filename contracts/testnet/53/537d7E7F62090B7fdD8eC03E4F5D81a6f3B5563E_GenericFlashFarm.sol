...
contract GenericFlashFarm is IFlashStrategy, Ownable, ReentrancyGuard {
    ...
    function depositPrincipal(uint256 _tokenAmount) external override onlyAuthorised returns (uint256) {
        ...
        // Do nothing, principal will stay within strategy

        return _tokenAmount;
    }

    function withdrawYield(uint256 _tokenAmount) private {
        require(_tokenAmount <= yieldBalance, "WITHDRAW TOO HIGH");

        // Perform withdraw action
        // Do nothing, this strategy does not generate yield

        // Deduct from tracked yield balance
        yieldBalance = yieldBalance - _tokenAmount;
    }
    ...
    function withdrawPrincipal(uint256 _tokenAmount) external override onlyAuthorised {
        ...
        // Perform withdraw action
        // Do nothing, this strategy already holds the principal
    ...
    function getYieldBalance() public pure override returns (uint256) {
        // This strategy simply does not generate yield therefore this function should always
        // return 0.
        return 0;
    }
    ...
    function quoteMintFToken(uint256 _tokenAmount, uint256 _duration) external view override returns (uint256) {
        ...
        return (_tokenAmount * (_duration * 31709792000)) / (10**principalDecimals);
    }

    function quoteBurnFToken(uint256 _tokenAmount) public pure override returns (uint256) {
        return 0;
    }
    ...
    function burnFToken(
        ...
        // Here we would normally withdraw yield from the underlying strategy and transfer
        // back to the Flashstake protocol. This is not necessary since yield will always be
        // equal to 0
