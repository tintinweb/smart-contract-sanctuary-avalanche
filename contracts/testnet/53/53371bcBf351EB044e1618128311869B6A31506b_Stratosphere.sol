...
contract Stratosphere is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ILayerZeroReceiver {
    event MintedOnMainChain(uint256 indexed id);

    /// STORAGE
    uint256 s_tokenCounter;
    address public s_authenticationControllerAddress;
    ILayerZeroEndpoint public s_endPoint; //this endpoint should correspond to AVAX chain
    mapping(uint16 => bytes) public s_trustedRemoteLookup;
    ...
    function lzReceive(
        ...
        if (msg.sender != address(s_endPoint)) {
            revert Stratosphere__AccessDenied();
        }
        if (keccak256(_srcAddress) != keccak256(s_trustedRemoteLookup[_srcChainId])) {
            revert Stratosphere__AccessDenied();
        }
        address sender;
        assembly {
            sender := mload(add(_srcAddress, 20))
        }

        /// @notice Check if sender is already a member
        if (balanceOf(sender) != 0) revert Stratosphere__MemberAlreadyExists();

        /// @notice Check if sender is not a secondary account
        if (IAuthenticationController(s_authenticationControllerAddress).associatedTokenOf(sender) != 0)
            revert Stratosphere__AccountAlreadyLinked();
        ...
        _mint(sender, _id); //don't use safeMint because sender will be in other chain
    ...
    function setTrustedRemote(uint16 _srcChainId, bytes memory _srcAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        s_trustedRemoteLookup[_srcChainId] = _srcAddress;
    }
