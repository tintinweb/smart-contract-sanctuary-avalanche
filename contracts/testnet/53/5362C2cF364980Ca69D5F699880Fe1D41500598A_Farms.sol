...
contract Farms is ReentrancyGuard, Ownable, Pausable {
  ...
  address public steady;
  // Contract address for the esSteady token
  address public esSteady;
  ...
  event UpdateEsRewardSplit(uint256 indexed farmId, uint256 _esSteadySplit);
  event UpdateYieldBooster(address indexed caller, uint256 timestamp);
  event Paused(address indexed caller, uint256 timestamp);
  event Unpaused(address indexed caller, uint256 timestamp);

  /* ========== CONSTRUCTOR ========== */

  constructor(address _steady, address _esSteady) {
    require(_steady != address(0), "invalid 0 address");
    require(_esSteady != address(0), "invalid 0 address");

    steady = _steady;
    esSteady = _esSteady;

    IERC20(steady).approve(esSteady, type(uint256).max);
  }
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public returns (uint256) {
  ...
  /**
  * TODO: Returns the reward multiplier of a user's position
  * @return multiplier  Multiplier in 1e18
  */
  function getRewardMultiplier() public returns (uint256) {
  ...
  */
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0) {
      ...
      // TODO: esSteadySplitCheck
      if (farm.esSteadySplit > 0) {
        uint256 esRewards = rewards * farm.esSteadySplit / SAFE_MULTIPLIER;
        uint256 nonesRewards = rewards - esRewards;

        IERC20(steady).safeTransfer(msg.sender, nonesRewards);
        IesREWARD(esSteady).convert(esRewards);
        IERC20(esSteady).safeTransfer(msg.sender, esRewards);
      } else {
  ...
  * @param _esSteadySplit  % split of total rewards to be given in esRewards
  */
  function createFarm(
    bool _active,
  ...
  */
  function depositRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner {
  ...
  * @param _amount  Amount of reward tokens to deposit; in reward token's decimals
  */
  function withdrawRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner {
    Farm storage farm = farms[_id];
  ...
  */
  function updateRewardDistributionRate(uint256 _id, uint256 _rate) external onlyOwner {
  ...
  * @param _id  Unique id of farm
  * @param _esSteadySplit  Rewards split % of total rewards to esRewards
  */
  function updateEsRewardsSplit(uint256 _id, uint256 _esSteadySplit) external onlyOwner {
    require(_esSteadySplit <= MAX_REWARDS_SPLIT, "Reward split greater than maximum");

    farms[_id].esSteadySplit = _esSteadySplit;
    _updateFarm(_id);

    emit UpdateEsRewardSplit(_id, _esSteadySplit);
  }
...
contract $Farms is Farms {
    ...
    constructor(address _steady, address _esSteady) Farms(_steady, _esSteady) {}
