...
// mappings
    ...
    mapping (address => uint256) public sellerBank; // mapping from address to profit
// events
    ...
    function buyItNow(address _auctionAddress) external payable returns (bool)
        {
            ...
            sellerBank[creator] += profit; // add the profit to the seller bank
            amountOwed += profit; // add the profit to the owed amount
            return true;
        }
    ...
    function completeAuction(address _auctionAddress) external onlyOwner returns (bool)
        {
            ...
            dp.transferFrom(msg.sender, address(this), amount);
            ...
            sellerBank[Auction(_auctionAddress).getCreator()] += Auction(_auctionAddress).maxBid() - fee; // add the profit to the seller bank
            return true;
        }


    // withdraw funds from the seller bank to the seller 
    function withdrawFundsSeller(address _auctionAddress) external onlyOwner returns (bool)
        {
            address creator = Auction(_auctionAddress).creator(); // get the creator of the auction
            creator.call{value:sellerBank[creator]}; // send the funds to the creator
            amountOwed -= sellerBank[creator]; // subtract the amount paid from the amount owed
            sellerBank[creator] = 0; // reset the seller bank
            return true;
        }
    // withdraw funds from the Auction manager to the Auction wallet
    function withdrawFundsManager(uint256 _amount) external view onlyOwner returns (bool)
        {
            require((address(this).balance - amountOwed) >= _amount); // Can only remove funds that aren't owed to a creator
            wallet.call{value:(_amount)}; // send the funds to the wallet
            return true;
        }
    // destroy an auction
    function destroyAuction(address _auctionAddress) external onlyOwner returns (bool)
        {
            require(Auction(_auctionAddress).getAuctionState() == Auction.AuctionState.ENDED,"The auction must have ended"); // the auction must be complete
