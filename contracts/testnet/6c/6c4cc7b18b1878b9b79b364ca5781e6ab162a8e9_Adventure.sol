...
contract Adventure is Ownable, Pausable {
    ...
    struct mapTraits {
        ...
        uint   rewardRate;      // ➡️ Chances of returning something                     Example (40% chances of returning)
        uint   failRate;        // ➡️ Reward rate in case he fails to return something   Example (baseReward x failedRate)
        uint   mintRate;        // ➡️ Chances of returning with an rare adventurer       Example (0.3% chances of finding)
        uint   maxTokens;       // ➡️ How many adventurers can be sent per owner         Example (3 adventurers per mapTraits)
        uint   experience;      //
        uint   time;            // ➡️ How long will it take an adventurer to return      Example (3 Hours)
    ...
    event mapCreated            (address owner, uint mapId, mapTraits traits);                
    // ↪ Emits event alerting ➡️ A new mapTraits has been created with ID: "mapID"

    event mapModified           (address owner, uint8 mapId, mapTraits traits);
    ...
    event adventurerSent        (address account, uint tokenId, uint _mapId);
    // ↪ Emits event alerting ➡️ Player sent to the adventure, to mapID

    event adventurerRetrieved    (address account, uint tokenId, uint _mapId, uint amount, bool won);
    ...
    private
    {
        ...
        player.timeleft    = uint32(block.timestamp) + _setTime(_mapId, _tokenId);
    ...
    function toolMap (
        ...
        uint    _rewardRate,    // Whats the chances for a player to find a reward?
        uint    _failRate,      // If he doesnt get the reward how much will he still get rewarded?
        uint    _mintRate,      // Does this map mint any new character?
        uint    _maxTokens,     // How many tokens can a map hold?
        uint    _experience,    // How many adventures does your account need to play this map?
        uint    _time,          // How long will my character take to come back? 
