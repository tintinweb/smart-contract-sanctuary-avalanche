...
contract CampaignContract is Ownable, ReentrancyGuard {
    ...
    uint256 private immutable START_TIMESTAMP;
     ...
     * @param arkiusPayPerAdClick Amount given to user per ad click.
     ...
     * @param destinationUrl      DestinationUrl of the Campaign.
    ...
    function addCampaign(uint256       timestamp,
                         ...
                         uint96        target,
                         uint96        arkiusPayPerAdClick) external onlyAttentionSeeker() {
        ...
        require(arkiusPayPerAdClick  > 0, "arkiusPayPerAdClick must be greater than 0");
    ...
    function fundCampaign(uint256[] memory id, uint256[] memory replenish) external onlyAttentionSeeker nonReentrant {
        ...
        multiwallet.multiwallet(_msgSender(), IMultiwalletContract.RecipientType.attentionSeeker, amount);
    ...
    function withdrawFunds(uint256 campaignID, uint256 amount) external onlyCampaignOwner(campaignID) {
        ...
        multiwallet.unMultiwallet(_msgSender(), IMultiwalletContract.RecipientType.attentionSeeker, amount);
    ...
    function editCampaign(uint id,
        ...
        if (arkiusPayPerAdClick          != 0)   campaigns[id].arkiusPayPerAdClick = arkiusPayPerAdClick;

        campaigns[id].target = target;
        entityContract.editEntity(id, title, content, metadata, _msgSender());

        emit EditCampaign(id, target, arkiusPayPerAdClick, title, content, imageUrl, destinationUrl, metadata);
    }
    ...
    function adServed(uint256 campaignID) external onlyMarketplaceController nonReentrant {
        ...
        uint256 arkiusPayPerAdClick   = campaigns[campaignID].arkiusPayPerAdClick;
        uint256 multiwalletBalance = campaigns[campaignID].multiwalletBalance;

        //multiwalletBalance < arkiusPayPerAdClick condition is already checked in the marketplace contract.
        campaigns[campaignID].adServed = campaigns[campaignID].adServed + 1;
        campaigns[campaignID].multiwalletBalance = multiwalletBalance - arkiusPayPerAdClick;
    }
...
interface IMultiwalletContract{
    ...
    enum RecipientType { member, certifier, attentionSeeker, treasury, devfund, arkiusPBCLicense }


    function multiwalletBalance(RecipientType mType, address mAddress) external returns (uint);

    function multiwallet(address mAddress, RecipientType mtype, uint256 amount) external;

    function deMultiwallet(address mAddress, RecipientType mtype, uint256 amount) external;

    function unMultiwallet(address mAddress, RecipientType mtype, uint256 amount) external;

    function claimRewards() external;

    function multiwalletTransfer(address fromAddress, RecipientType fromType, address toAddress, RecipientType toType, uint256 amount) external ;
