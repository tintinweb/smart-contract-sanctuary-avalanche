...
contract CyberGods_Game is Ownable, Pausable {
    // Mappings
    mapping(uint256 => address) public stakedCyborgsToAddress; // tokenId => owner
    ...
    // Views
    function getAttackPowerFor(address _address) external view returns (uint256) {
        return addressToAttackPower[_address];
    }
    ...
    function unstakeWarriorNFTs(
        ...
        require(
            numberOfStaked[_owner][CYBORGS_INDEX] >= _cyborgsIds.length,
        ...
        for (uint256 i = 0; i < _cyborgsLength; i++) {
            ...
            require(
                stakedCyborgsToAddress[_nftId] == _owner,
                "You don't own this token"
            );
            numberOfStaked[_owner][CYBORGS_INDEX]--;
            delete stakedCyborgsToAddress[_nftId];
