...
contract Traits is Controllable {
    ...
    uint8[5] public skeletonTraitCountForType;
    ...
    function selectSkeletonTraits(uint256 seed) external view returns (uint8 arm, uint8 head, uint8 torso, uint8 legs) {
        ...
        head  = uint8((seed >> 16) % skeletonTraitCountForType[1]);
        torso = uint8((seed >> 32) % skeletonTraitCountForType[2]);
        legs  = uint8((seed >> 48) % skeletonTraitCountForType[3]);
    }
    ...
    function uploadBackgrounds(uint8[] calldata indices, string[] calldata pngs) external onlyController {
        for (uint256 i = 0; i < indices.length;) {
            backgrounds[indices[i]] = pngs[i];
            unchecked{++i;}
        }
    }

    function uploadRangerTraits(
        uint8[] calldata _traitTypes,
        uint8[] calldata _traitIds,
        string[] calldata _names,
        string[] calldata _pngs
    ) external onlyController {
        for (uint256 i; i < _traitTypes.length;) {
            rangerTraitData[_traitTypes[i]][_traitIds[i]] = Trait(_names[i], _pngs[i]);
            if (_traitIds[i] > rangerTraitCountForType[_traitTypes[i]]) {
                rangerTraitCountForType[_traitTypes[i]] = _traitIds[i] + 1;
            }
            unchecked{++i;}
        }
    }
    ...
    function uploadSkeletonTraits(
        ...
        for (uint256 i; i < _traitTypes.length;) {
            skeletonTraitData[_traitTypes[i]][_traitIds[i]] = Trait(_names[i], _pngs[i]);
            if (_traitIds[i] > skeletonTraitCountForType[_traitTypes[i]]) {
                skeletonTraitCountForType[_traitTypes[i]] = _traitIds[i] + 1;
            }
            unchecked{++i;}
        }
    }
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    '<image x="6" y="2" width="64" height="64" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
                    trait.png,
                    '"/>'
                )
            );
    ...
    function drawBackground(string memory png) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<image x="0" y="0" width="75" height="75" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
                    png,
                    '"/>'
                )
            );
    ...
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        ...
        if (s.isRanger) {
            svgString = string(
                abi.encodePacked(
                    drawBackground(backgrounds[s.level-1]),
        ...
        return
            string(
                abi.encodePacked(
                    '<svg id="skeleton" width="100%" height="100%" version="1.1" viewBox="0 0 75 75" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    skeleton.getGeneration(tokenId).toString(),
    ...
    function tokenURI(uint256 tokenId) public view returns(string memory) {
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                s.isRanger ? "Ranger #" : "Skeleton #",
                tokenId.toString(),
                '", "description": "caca", "image": "data:image/svg+xml;base64,', // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< "Yield Hunt - Chapter 2 | Rangers and Skeletons are fighting for a miraculous plant called $GLOW, found in an Underground Forest, hidden under a Temple. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Avalanche blockchain." for real launch
