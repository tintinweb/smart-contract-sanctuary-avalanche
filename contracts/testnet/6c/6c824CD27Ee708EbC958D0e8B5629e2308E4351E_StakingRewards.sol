    ...
    ERC1155Holder
{
    ...
    /* ========== INTERNAL FUNCTIONS ========== */
    function addMaxLock(address vault, uint256 epochEnd, uint256 apr, uint256 reward) internal {
        uint256 duration = epochEnd - block.timestamp;
        uint256 rewardPerToken = apr.mul(duration).div(getYear());
        _maxLock[vault][epochEnd] = reward.div(rewardPerToken);
        emit MaxLock(vault, epochEnd, duration, reward, apr, _maxLock[vault][epochEnd], block.timestamp);
    }
}
