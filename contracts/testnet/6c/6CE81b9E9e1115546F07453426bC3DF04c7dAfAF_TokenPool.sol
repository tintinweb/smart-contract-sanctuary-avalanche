...
import "../openzeppelin/contractsV4.8.1/token/ERC20/utils/SafeERC20.sol";
import "../openzeppelin/contractsV4.8.1/security/Pausable.sol";
...
contract TokenPool is Ownable, Pausable {
    ...
    IERC20 public immutable token; // token token.
    ...
    /**
     * @notice Constructor
     * @param _token: Token token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: token pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(tokenPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Unlock user token funds.
    ...
    /**
     * @notice Deposit funds into the Token Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in TOKEN)
    ...
    /**
     * @notice Withdraw funds from the Token Pool.
    ...
    /**
     * @notice Harvest pending TOKEN tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingToken = masterchefV2.pendingToken(tokenPoolPID, address(this));
        if (pendingToken > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(tokenPoolPID, 0);
    ...
    function setVTokenContract(address _VToken) external onlyAdmin {
        require(_VToken != address(0), "Cannot be zero address");
        VToken = _VToken;
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Token Pool
     */
    ...
    function calculateTotalPendingTokenRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingToken(tokenPoolPID, address(this));
        return amount;
    }
