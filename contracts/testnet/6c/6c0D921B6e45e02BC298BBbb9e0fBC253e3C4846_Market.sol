...
contract Market is LzApp {
  ...
  function _repay(address _for, uint256 _debtValue) internal checkDebtSize returns (uint256 _debtShare) {
    ...
    // 4. Transfer debt from msg.sender to this market.
    ...
    // actually transfer ERC20 token from user
  ...
  function _blockingLzReceive(
    ...
    require(address(uint160(uint256(bytes32(_srcAddress)))) == address(debtMarket), "invalid pair sourceMarket");

    if (_compareStrings("repay", action)) {
      _repay(to, _debtValue);
    }
  ...
  function _isBorrowed(
    ...
    cashier.transferERC20(debt, address(this), _to, _borrowAmount);
  }
