...
contract veYETI {
    ...
    */
    ...
    // rewarders that need to be updated when veYETI chamges
    address[] public updateableRewarders;

    mapping(address => bool) isUpdateableRewarder;

    address constant yetiMultisig = 0xf572455BE31BF54Cc7D0c6D69B60813171bb7b12; // multisig controls adding and removing updateable rewarrders
    ...
    event RewarderStatusUpdate(address _rewarder, bool _updateable);
    ...
    */ 
    function update(RewarderUpdate[] memory _yetiAdjustments) external {
        ...
        // notify all the updateable rewarders about veYETI changes
        _notifyRewarders(msg.sender, updateableRewarders);
    ...
    // returns how much veYETI a user currently has accumulated on a rewarder
    function getUserVeYetiOnRewarder(address _user, address _rewarder) public view returns (uint256) {
    ...
    function _accumulate(address _user, bool _isStakeIncrease) internal {
        ...
        if (_isStakeIncrease) {
            ...
            userInfo.lastTotalVeYeti = getTotalVeYeti(_user);
        } else {
    ...
    function setEmitter(address _emitter) external {
        // require(msg.sender == yetiMultisig, "veYETI: invalid caller"); // TODO uncomment
    ...
    // add a rewarder to the list of updateable rewarders
    function addUpdateableRewarder(address _rewarder) external {
        // require(msg.sender == yetiMultisig, "veYETI: invalid caller"); // TODO uncomment

        isUpdateableRewarder[_rewarder] = true;
        updateableRewarders.push(_rewarder);
        require(updateableRewarders.length < 10, "veYETI: too many rewarders");

        emit RewarderStatusUpdate(_rewarder, true);
    }


    // if a rewarder address is set to be updateable, veYETI
    // will call update() on it when a user stakes or unstakes YETI on it
    function removeUpdateableRewarder(uint _index) external {
        // require(msg.sender == yetiMultisig, "veYETI: invalid caller"); // TODO uncomment
        
        address rewarderToRemove = updateableRewarders[_index];
        updateableRewarders[_index] = updateableRewarders[updateableRewarders.length - 1];
        updateableRewarders.pop();

        isUpdateableRewarder[rewarderToRemove] = false;

        emit RewarderStatusUpdate(rewarderToRemove, false);
    }

    // notify all updateable rewarders of the new veYETI gains
    ...
    function _notifyRewarders(address _user, address[] memory rewarders) internal {
        ...
        for (uint256 i = 0; i < rewarders.length; i++) {
            ...
            if (isUpdateableRewarder[rewarder]) {
