  ...
  ReentrancyGuard
{
  ...
  uint256 public maxMintAmount = 10; // Max # in each mint tx
  uint256 public maxSupply = 1000;

  // Reflective Minting Variables
  uint256 public reflectionBalance; // Total reflection balance
  ...
  function contractURI() public view returns (string memory) {
    ...
    return bytes(currentBaseURI).length > 0
      ? string(abi.encodePacked(currentBaseURI, "collection-metadata", baseExtension))
      : "";
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "mint amount cannot exceed maxMintAmount");
    require(supply + _mintAmount <= maxSupply, "must mint <= to the supply left available to mint");

    require(msg.value == cost * _mintAmount, "must send correct price");

    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, supply + i);
      lastDividendAt[supply + i] = totalDividend;
  ...
  function claimReward(uint256 tokenId) public {
    require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "ChubbyBunnies: Only owner or approved can claim rewards");
  ...
  function splitBalance(uint256 amount) private {
    uint256 reflectionShare = (amount * 50) / 100; // 50% of minting fee is reflected
    uint256 mintingShare1  = (amount - reflectionShare);
  ...
  function _reflectDividend(uint256 amount) internal {
    ...
    // console.log("Reflection Balance: %s", reflectionBalance);
    // console.log("Total Dividend: \t%s", totalDividend);
  }
