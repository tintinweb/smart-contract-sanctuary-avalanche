...
// File contracts/TokensFarms/TokensFarm.sol
...
contract TokensFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC20 public erc20;
    // The total amount of ERC20 that's paid in one epoch.
    ...
    // Flat fee amount to deposit
    uint256 public flatFeeAmountDeposit;
    // Flat fee amount to withdraw
    ...
    // If cool down and warmup has been initialised
    bool public isInitialisedFully;
    // Time when first deposit happened
    uint256 public firstDepositAt;
    // Total tokens currently staked for user
    mapping(address => uint256) public totalActiveStakeAmount;
    // Number of all pending stakes
    uint256 public noOfPendingStakes;
    // Number of all pending withdraws
    uint256 public noOfPendingWithdrawals;
    // Total number of users
    uint256 public totalNoOfUsers;
    ...
    event CoolDownSet(uint256 indexed coolDown);
     ...
     * @param _flatFeeAmountDeposit - flat fee amount
     * @param _flatFeeAmountWithdraw - flat fee amount
        ...
        initializer
    {
        ...
        require(_erc20 != address(0x0), "Reward token address can not be 0x0.");
        ...
        flatFeeAmountDeposit = _flatFeeAmountDeposit;
        flatFeeAmountWithdraw = _flatFeeAmountWithdraw;
        ...
        onlyOwner
    {
        ...
        require(
            _warmup >= 0,
            "Warmup needs to be over or equal to 0"
        );
        require(
            _coolDown >= 0,
            "CoolDown needs to be over or equal to 0"
        );

        warmupPeriod = _warmup;
        coolDownPeriod = _coolDown;
        feeCollector = _feeCollector;
        farmImplementation = _farmImplementation;
        isInitialisedFully = true;
    }
    ...
    function _addParticipant(
        ...
        returns(bool)
    {
        if(totalActiveStakeAmount[user] > 0){
            return false;
        }
    ...
    {
        if(noOfUsers == 1){
            if(amount == totalActiveStakeAmount[user]){
                delete id[user];
                participants.pop();
                noOfUsers--;
        ...
        onlyOwner
    {
        ...
        flatFeeAmountDeposit = _flatFeeAmount;
        emit FlatFeeAmountDepositSet(flatFeeAmountDeposit);
    }
        ...
        onlyOwner
    {
        ...
        flatFeeAmountWithdraw = _flatFeeAmount;
        emit FlatFeeAmountWithdrawSet(flatFeeAmountWithdraw);
    }
        ...
        onlyOwner
    {
        require(
            _coolDown >= 0,
            "_coolDown can not be below 0"
        );

        coolDownPeriod = _coolDown;
        emit CoolDownSet(coolDownPeriod);
    }
        ...
        onlyOwner
    {
        require(
            _warmup >= 0,
            "_warmup can not 0 or below"
        );

        warmupPeriod = _warmup;
        emit WarmupSet(warmupPeriod);
    }
    ...
    {
        require(warmupPeriod != 0, "Warmup is not on");
        require(noOfPendingStakes > 0, "There is no requested stakes yet");
    ...
    function getAllPendingWithdrawals(
        ...
        view
        returns (
            WithdrawRequest[] memory
        )
    {
        require(coolDownPeriod != 0, "Cool down is not on");
        require(withdrawRequest[_user].length > 0, "There is no requested withdraws yet");

        uint256 noOfPendingWithdrawalsForUser = withdrawRequest[_user].length;
        WithdrawRequest[] memory pendingWithdrawals = new WithdrawRequest[](noOfPendingWithdrawalsForUser);
        StakeInfo[] memory stakeOfWithdrawals = new StakeInfo[](noOfPendingWithdrawalsForUser);
        for(uint256 i = 0; i < noOfPendingWithdrawalsForUser; i++){
            pendingWithdrawals[i] = withdrawRequest[_user][i];
            uint256 stakeId = withdrawRequest[_user][i].stakeId;
            stakeOfWithdrawals[i] = stakeInfo[_user][stakeId];
        }
    ...
    function totalPending()
        ...
        returns (uint256)
    {
        if (block.timestamp <= startTime || firstDepositAt == 0 || totalDeposits == 0) {
            return 0;
        }
        ...
        uint256 lastTime = block.timestamp < endTime
            ? block.timestamp
            : endTime;
        return rewardPerSecond.mul(lastTime - firstDepositAt).sub(paidOut);
    }
        ...
        )
    {
        ...
        uint256[] memory warmupExpirations = new uint256[](numberOfStakes);
        ...
        for(uint256 i = 0; i < numberOfStakes; i++){
            ...
            StakeInfo memory stake = stakeInfo[user][i];
            warmupExpirations[i] = stake.warmupExpiration;
        }

        return (deposits, pendingAmounts, depositTime, warmupExpirations);
    }
        ...
        returns (uint256, uint256)
    {
        require(
            isInitialisedFully,
            "Farm is not fully initialised"
        );
        require(
            block.timestamp + warmupPeriod < endTime,
            "Deposit: too late, the farm will be closed when warmup pass"
        );
        require(
            _amount > 0,
        ...
        if(firstDepositAt == 0){
            firstDepositAt = block.timestamp + warmupPeriod;
        }

        if (isFlatFeeAllowed) {
            // Collect flat fee
            require(
                msg.value == flatFeeAmountDeposit,
        ...
        stake.warmupExpiration = block.timestamp + warmupPeriod;
        ...
        totalActiveStakeAmount[msg.sender] = totalActiveStakeAmount[msg.sender].add(stakedAmount);
        ...
        payable
    {
        ...
        _request.warmupExpiration = block.timestamp + warmupPeriod;
        ...
        validateStakeByStakeId(_user, stakeId)
    {
        ...
        require(
            depositRequest[user][requestId].warmupExpiration < block.timestamp,
            "Warmup hasn't passed yet"
        );
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        ...
        require(
            minimalTimeStakeRespected,
            "Can not make withdraw request, minimal time to stake hasn't passed yet"
        );
        require(
        ...
        _request.stakeId = stakeId;
        _request.expirationDate = block.timestamp + coolDownPeriod;
        _request.user = msg.sender;
        ...
        // Update totalRewards
        totalRewards = totalRewards.sub(_request.pendingAmount);
        ...
        noOfPendingWithdrawals++;

        // Emit Withdraw event
        emit Withdraw(msg.sender, stakeId, _amount, _request.pendingAmount);
    }
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        ...
        if(coolDownPeriod != 0){
            ...
            require(
                exists,
                "Request hasn't been made for this stake with this amount"
            );
            require(
                withdrawRequest[msg.sender][requestId].expirationDate < block.timestamp,
                "Cool down hasn't passed yet"
            );
            ...
            WithdrawRequest memory lastRequest = withdrawRequest[msg.sender][lastRequestId];
            ...
            // Emit Withdraw event
            emit Withdraw(msg.sender, stakeId, _amount, pendingAmount);
        }
        else{
            require(
                isInitialisedFully,
        ...
        payable
    {
        ...
        if (isFlatFeeAllowed) {
            // Collect flat fee
            require(
                msg.value == flatFeeAmountWithdraw,
                "Payable amount is less than fee amount."
            );
        ...
        for(uint256 i = 0; i < stakeInfo[msg.sender].length; i++){
            ...
            if (rewardFeePercent > 0) {
                // Collect reward fee
                uint256 feeAmount = pendingAmount.mul(rewardFeePercent).div(100);
        ...
        emit WithdrawRewards(msg.sender, totalAmount);
    }
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        StakeInfo storage stake = stakeInfo[msg.sender][stakeId];
        require(
            stake.isDepositFinalized && stake.amount > 0,
            "Stake was not finalised"
        );
