...
contract CRNpresale {
    ...
    uint8 presale = 1; //TODO: Make it 0 on release
    ...
    constructor(IERC20 _token1, AI _token2, address signer, address _owner) {
        Owner = _owner;
        token1 = _token1;
        token2 = _token2;
    ...
    function checkPresaleStatus() public view returns (uint8){
        return presale;
    }

    function changePresaleStatus(uint8 _status) public OnlyOwners {
        presale = _status;
    }

    function checkLimit(address _who) public view returns (uint256) {
        return buyLimit[_who];
    }
    ...
    function buyTokens(bytes calldata signature) public payable BlacklistCheck {
        require(presale != 0, "Presale has ended.");
        if (presale == 1) {
            require(buyLimit[msg.sender] + msg.value * rate <= 1000 * 10**token2.decimals(), "You can't exceed the buy limit on the first stage of the presale.");
        }
        if (presale == 1 || presale == 2) {
        ...
        require(token1.balanceOf(address(this)) >= _tokenAmount, "There are not enough tokens in the presale wallet.");
...
//    );
    ...
    constructor(address _owner) ERC20("Test Coin", "ABC3") {   //TODO: CHANGE!!! Before deploy
        Owner = _owner;
