...
contract NFTBettingGame is ERC721, Ownable {
    ...
    uint256 public constant TREASURY_FEE_PERCENTAGE = 3;
        ...
        returns (address payable[3] memory)
    {
        ...
        uint256 i;
        uint256 j;
        uint256 randomIndex = random() % tokenIdList.length;
        uint256 poolAmount = pot;

        for (i = 0; i < tokenIdList.length; i++) {
            address bettor = ownerOf(tokenIdList[i]);
            uint256 amount = betAmounts[tokenIdList[i]][bettor];
        ...
        for (i = 0; i < 3; i++) {
            ...
            uint256 reward = (poolAmount * 8) / 10 / 3;
            _transfer(address(this), topWinners[i], topWinnersAmount[i]);
            poolAmount -= reward;
        }

        uint256 treasuryAmount = (pot * 2) / 10;
        treasuryPool += treasuryAmount;
        pot -= treasuryAmount;
        winner = payable(ownerOf(tokenIdList[randomIndex]));
        winner.transfer(pot);
