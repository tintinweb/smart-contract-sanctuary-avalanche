...
import "@thirdweb-dev/contracts/drop/DropERC1155.sol"; // For my collection of Swords
...
contract Mining is ReentrancyGuard, ERC1155Holder {
    ...
    mapping(address => MapValue) public playerSword;
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        require(
            swordNFTCollection.balanceOf(msg.sender, _tokenId) >= 1,
            "You must have 1 sword to stake"
        );
        if (playerSword[msg.sender].isData) {
            swordNFTCollection.safeTransferFrom(
                address(this),
                msg.sender,
                playerSword[msg.sender].value,
                1,
                "returning your old sword"
            );
        }
        ...
        // Update the playerSword mapping
        playerSword[msg.sender].value = _tokenId;
        playerSword[msg.sender].isData = true;
    ...
    function withdraw() external nonReentrant {
        require(
            playerSword[msg.sender].isData,
            "You do not have a sword to withdraw"
        );

        //calculate the rewards theyre owed then pay them
        uint256 reward = calculateRewards(msg.sender);
        rewardsToken.transfer(msg.sender, reward);

        //send the sword to the player
        swordNFTCollection.safeTransferFrom(
            address(this),
            msg.sender,
            playerSword[msg.sender].value,
        ...
        //update the playerSword mapping
        playerSword[msg.sender].isData = false;
    ...
    // If playerLastUpdate or playerSword is not set, then the player has no rewards.
    function calculateRewards(
        address _player
    ) public view returns (uint256 _rewards) {
        // If playerLastUpdate or playerPickaxe is not set, then the player has no rewards.
        if (!playerLastUpdate[_player].isData || !playerSword[_player].isData) {
            return 0;
        }
