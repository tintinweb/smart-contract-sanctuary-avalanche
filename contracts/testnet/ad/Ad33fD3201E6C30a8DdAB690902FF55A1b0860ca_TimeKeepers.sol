    ...
    UUPSUpgradeable
{
    ...
    function claim(
        ...
        if (totalSupply() > MAX_FREE) {
            require(
                msg.value >= PRICE,
                "Free claim limit reached. Please pay 0.025 ETH."
            );
        }
        ...
        require(
            bytes(brand).length <= 10,
            "Brand name must be less than 10 characters"
        );
        uint color = (uint(
            keccak256(abi.encodePacked(msg.sender, block.timestamp, "lollmao"))
        ) % 0xFFFFFF) + 1;
        taken[brand] = true;
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "TimeKeeper #',
                        tokenId.toString(),
                        '", "description": "TimeKeepers is a collection of on-chain, accurate clocks. Refresh metadata to get accurate time.", "attributes":[{"trait_type":"timezone", "value": "UTC',
                        timezone,
