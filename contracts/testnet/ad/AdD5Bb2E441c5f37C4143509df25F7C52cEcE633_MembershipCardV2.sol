...
contract MembershipCardV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ERC721AUpgradeable  {
    ...
    function getLastIndexBits(uint256 x) public pure returns(uint256){
        if(x == 0) {
            return 0;
        }
        return x & (~(x-1));
...
//        return LevelUtils.query(_playersSnapshot[_address].snapshot);
        return query(_playersSnapshot[_address].snapshot);
    }
...
//        require(LevelUtils.getLastIndexBits(snapshot) == 2 ** (256 - _maxLevel), "max Level not match account");
        require(getLastIndexBits(snapshot) == 2 ** (256 - _maxLevel), "max Level not match account");
    ...
    function calcLeftMintOf(address _sender) external view returns(uint256) {
        ...
        return _getAvailableAmount(_sender, pr);
    }
    ...
    function createCard(uint256 _amount, uint8[] memory _fixedItems ) external {
        ...
        IBet.PlayerRecord memory pr = IBet(betContract).playerRecords(_sender);
        require(_getAvailableAmount(_sender, pr) > 0, "condition is not met");
        ...
        for(uint256 i = 0; i < _amount; i++) {
            ...
            (
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(_sender, blockhash(block.number -1), block.timestamp, _tokenId)
                    )
                )
            );
  ...
  */
    ...
    function _getAvailableAmount(address _sender,  IBet.PlayerRecord memory _pr) internal view returns(uint256) {

        if(_pr.amount < 30 || _pr.accAmount < 2000*10**6) { return 0; }

        uint256 totalAmount = calTotalMintOf(_pr);
        uint256 mintedAmount = _numberMinted(_sender);

        return totalAmount >= mintLimit ? mintLimit - mintedAmount : totalAmount - mintedAmount;
    }
    ...
    function _afterTokenTransfers(
        ...
        for(uint256 i = 0; i < quantity; i++) {
            ...
            // mint
            if(from == address(0)) {
                uint256 snapshot = _playersSnapshot[to].snapshot;
//                bool hasLevel = LevelUtils.getBit(snapshot, level);
                bool hasLevel = getBit(snapshot, level);
            ...
            // transfer
                ...
                Player storage _fromSnapshot = _playersSnapshot[from];
                Player storage _toSnapshot = _playersSnapshot[to];
...
interface IBet {
    ...
    function rewardCoefficient() external view returns(uint256);
