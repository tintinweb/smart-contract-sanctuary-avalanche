...
contract OfferRouter is IStargateReceiver, Ownable {
    ...
    mapping(address => Offer[]) public collectionsOffers; // IERC721 => Offer[]
    ...
    function fulfillOffer(OfferFill memory _fulfillment) external payable {
        ...
        if (_fulfillment.dstChainId == chainId) {
            ...
            this._fullFill(_fulfillment, true);
        ...
        bytes memory data;
        {
            data = abi.encode(0, _fulfillment);
        ...
        stargateRouter.swap{value : msg.value}(
            ...
            _fulfillment.amount, // minimum 100% - no slippage - assuming stables for now
    ...
    function sgReceive(
        ...
        (uint256 _amountOutMin, OfferFill memory _fulfillment) = abi.decode(payload, (uint256, OfferFill));

        if (_token != _fulfillment.currency || _amount < _fulfillment.amount) {
            // Emit refund
        ...
        try this._fullFill(_fulfillment, false) {
    ...
    function _fullFill(OfferFill memory _fulfillment, bool isLocal) external {
        if (isLocal == false) {
        ...
        if (isLocal) {
            require(currency.allowance(_fulfillment.fulfiller, address(this)) >= offer.price, "!allowance");
            currency.transferFrom(_fulfillment.fulfiller, offer.offerer, offer.price);
        } else {
            currency.transfer(offer.offerer, offer.price);
        }
