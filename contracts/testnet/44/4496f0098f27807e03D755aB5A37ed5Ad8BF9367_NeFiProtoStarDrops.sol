...
contract NeFiProtoStarDrops is Ownable{
	...
	function sendFeeTokenSpec(uint256 _intervals, uint256 _x) payable external{
    	...
    	function payFeeAvaxSpec(uint256 _intervals,uint _x) payable external{
    		...
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(_intervals,msg.value,74000000000000000);
		//verification
	...
	function sendFeeToken(uint256 _intervals) payable external{
    		...
    		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		address _account = msg.sender;
    		uint256 _intervals = nebuLib.getLower(getTotalPayable(_account),_intervals);
    		require(_intervals >0, "either you opted not to pay any fees at this time or have none left to pay");
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(_intervals,feeTok.balanceOf(_account),tokFee);
		//verification
	...
	function protoFeesAvaxSpec(uint256 _intervals,uint256 _x) payable external{
		...
		require(_intervals >0, "Doesnt Look Like you opted to pay any fees at this time");
		 ...
		 //calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(nebuLib.getLower(getTotalPayable(_account),_intervals),msg.value,74000000000000000);
		//verification
		require(_nIntervals != 0 && needed > 0,"you do not hold enough to cover this claim");
		require(_nIntervals <= _intervals,"you do not hold enough to cover this claim");
	        //bookKeeping
