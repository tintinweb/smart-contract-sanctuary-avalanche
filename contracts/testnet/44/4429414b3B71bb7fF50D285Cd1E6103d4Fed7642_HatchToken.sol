...
import "./HatchFactory.sol";

contract HatchToken is ERC1155, Ownable, ReentrancyGuard {
    HatchRegistry public registry;
    HatchFactory public factory;
    string public name;
    string public symbol;
    uint256 private hatchBalance;
    address hatchFactory;

    mapping(uint256 => uint256) public trackSalesCount;
    ...
    constructor(
        ...
        address _hatchFactory,
        HatchFactory _factory,
        HatchRegistry _registry
    ) ERC1155("") {
        name = _name;
        symbol = _symbol;
        hatchFactory = _hatchFactory;
        factory = _factory;
        registry = _registry;
    }
    ...
    function mintBatch(
        ...
        require(registry.getAreSalesActive(), "Sale isn't active");
        uint256 totalCost = 0;
        for (uint256 i = 0; i < _trackIds.length; ++i) {
            require(
                factory.getExistingTrackId(_trackIds[i]),
                "This track id doesn't exist"
            );
            totalCost += factory.getTrack(_trackIds[i]).trackCost * _amounts[i];
        }
        require(
            (msg.value / 10000) * 10000 >= totalCost,
            "Wrong amount of ETH sent"
        );
        for (uint256 i = 0; i < _trackIds.length; ++i) {
            uint256 trackCost = factory.getTrack(_trackIds[i]).trackCost;
            uint256 trackNetCost = trackCost -
                ((trackCost * registry.getHatchMintingFee()) / 10000);
            for (
                uint256 j = 0;
                j < factory.getTrack(_trackIds[i]).artistsWallets.length;
                ++j
            ) {
                uint256 artistsShare = (trackNetCost *
                    factory.getTrack(_trackIds[i]).artistsShares[j]) / 10000;
                artistBalance[
                    factory.getTrack(_trackIds[i]).artistsWallets[j]
                ] += artistsShare;
            }
            trackSalesCount[_trackIds[i]] += _amounts[i];
        }
        hatchBalance += (totalCost * registry.getHatchMintingFee()) / 10000;
        _mintBatch(_to, _trackIds, _amounts, "");
    }

    function getTrackSalesCount(uint256 _id) public view returns (uint256) {
        return trackSalesCount[_id];
    }
    ...
    function withdrawForHatch() public onlyOwner nonReentrant {
        ...
        sendEth(registry.getHatchWallet(), hatchBalance);
        hatchBalance = 0;
    }
}
