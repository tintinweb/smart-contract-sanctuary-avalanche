...
import "./common/interfaces/IGalixERC20HotWallet.sol";
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    modifier onlyStakeOperator() {
        require(hasRole(STAKE_OPERATOR_ROLE, _msgSender()), "Not Minter");
        _;
    }
    ...
    function addMinter(address _account) onlyAdmin external {
        grantRole(MINTER_ROLE, _account);
    }

    function removeMinter(address _account) onlyAdmin external {
        revokeRole(MINTER_ROLE, _account);
    }

    ///////////////////////// MINTER /////////////////////////
    function mintToken(uint256 _amount, uint256 _daoDivision) onlyMinter external {
        uint256 _daoFundAmount = _amount.mul(10000).div(_daoDivision);
        token.mintFrom(daoFund, _daoFundAmount);
        token.mintFrom(gameRewardPool, _amount.sub(_daoFundAmount));
    }

    function stakeDistribute(uint256 _amount) onlyStakeOperator external {
        token.transferFrom(_msgSender(), galixStakePool, _amount);
    }
...
/**
 * @title GALIX Hotwallet Interface
 * @author GALIX Inc
 */

interface IGalixERC20HotWallet {
    ...
    function lpDecimals() external view returns (uint8);
