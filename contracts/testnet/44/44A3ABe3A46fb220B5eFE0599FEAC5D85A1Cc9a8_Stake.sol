...
contract Stake is Ownable {
    ...
    constructor(RewardToken rewardToken_, StakeNFT stakeNFT_, uint256 rewardPerBlock_) {
    ...
    function depositNFT(uint256 depositAmount_) external {
        ...
        uint256[] memory _userCollections = stakeNFT.getUserCollections(msg.sender);
        ...
        for(uint i = 0 ; i < depositAmount_ ; i++) {
          ...
          require(stakeNFT.getApproved(_tokenId) == address(this), "not approved"); // check nfts is approved.
          stakeNFT.safeTransferFrom(msg.sender, address(this), _tokenId); // transfer nft to stake contract.
        }
    ...
    function claimReward() external {
        ...
        rewardToken.transfer(msg.sender, rewardOfUser(msg.sender));
    }
...
contract StakeNFT is ERC721, Ownable {
    ...
    constructor(string memory baseURI_) ERC721("Stake NFT", "SN") {
        baseURI = baseURI_;
    }
    ...
    function amountOfUser(address user_) public view returns (uint256) {
        uint256 _amount = userCollections[user_].length;
        return _amount;
    }
}
