  ...
  */
    ...
    function calcIncentiveBonus(uint64 _startTime, uint64 _endTime, uint256 _amount) internal view returns(uint256 bonus) {
        ...
        if(current < _startTime + span / 6) {
            bonus = rewardRatio[0] * _amount / 10;
    ...
    function setRewardRatio(uint256[] memory _ratio) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateRewardRatio(uint256 _index, uint256 _ratio) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_ratio > 0, "invalid _ratio");
