...
contract AxelarGateway is IAxelarGateway, AdminMultisigBase {
    ...
    function deployToken(bytes calldata params, bytes32) external onlySelf {
        ...
        if (tokenAddress == address(0)) {
            ...
            _setTokenType(symbol, Types.InternalBurnableFrom);
        } else {
            ...
            _setTokenType(symbol, Types.External);
        }
    ...
    function burnToken(bytes calldata params, bytes32) external onlySelf {
        ...
        if (_getTokenType(symbol) == Types.External) {
    ...
    function _burnTokenFrom(
        ...
        Types tokenType = _getTokenType(symbol);
        bool burnSuccess;

        if (tokenType == Types.External) {
        ...
        if (tokenType == Types.InternalBurnableFrom) {
            burnSuccess = _callERC20Token(
                tokenAddress,
    ...
    function _getTokenType(string memory symbol) internal view returns (Types) {
        return Types(getUint(_getTokenTypeKey(symbol)));
    }
    ...
    function _setTokenType(string memory symbol, Types tokenType) internal {
