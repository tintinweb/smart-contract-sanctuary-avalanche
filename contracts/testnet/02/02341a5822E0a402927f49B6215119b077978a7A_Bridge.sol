...
contract Bridge is Ownable {
    ...
    event BridgeInitiated(uint256 indexed bridgeId, address indexed user, address indexed token, uint256 amount, string targetChain);
    event BridgeCompleted(uint256 indexed bridgeId, address indexed user, address indexed token, uint256 amount, string targetChain);
    event BridgeFailed(uint256 indexed bridgeId, address indexed user, address indexed token, uint256 amount, string targetChain);
    ...
    function initiateBridge(address _token, uint256 _amount, string memory _targetChain) external {
        require(isSupportedToken(_token), "Bridge: token is not supported");
        require(_amount > 0, "Bridge: amount must be greater than 0");
        require(bytes(_targetChain).length > 0, "Bridge: target chain cannot be empty");
        ...
        bridgeRequests[bridgeId] = BridgeRequest(msg.sender, _token, _amount, _targetChain, BridgeStatus.Initiated);

        emit BridgeInitiated(bridgeId, msg.sender, _token, _amount, _targetChain);
