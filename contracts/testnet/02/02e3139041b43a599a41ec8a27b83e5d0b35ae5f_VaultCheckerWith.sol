 ...
 */ 
 contract VaultCheckerWith {
  ...
  uint256 public ratePreWith;
  uint256 public totPreCollVaultLP;
  uint256 public totPreCollVaultRemaining;
...
function withdrawCheck(uint256 amount, int256 newPrice) external {
   actSupply = vault.totalSupply();
   lpSupply = vault.balanceOf(msg.sender);
   remainingSupply = actSupply- lpSupply;
   ratePreWith = vault.getRate();
   totPreCollVaultLP = lpSupply * ratePreWith / (10 ** (18 + (18 - tokenDecimals)));
   totPreCollVaultRemaining = remainingSupply * ratePreWith / (10 ** (18 + (18 - tokenDecimals)));
   ...
   collateralOut = vault.withdraw(amount, msg.sender);
   fee = totPreCollVaultLP - collateralOut;
   ratePostWith = vault.getRate();
   totPostCollPost = remainingSupply * ratePostWith / (10 ** (18 + (18 - tokenDecimals)));
   ...
   totPostUpdtCollPost = remainingSupply * ratePostUpdtDep / (10 ** (18 + (18 - tokenDecimals)));
   pos = pool.positionLPInfo(address(vault));
