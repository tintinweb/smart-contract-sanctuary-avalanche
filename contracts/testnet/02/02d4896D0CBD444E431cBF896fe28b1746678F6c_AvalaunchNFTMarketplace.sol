...
contract AvalaunchNFTMarketplace is IAvalaunchNFTMarketplace, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param _minTimeExtensionPerBid - time for which auction is extended when someone makes an offer
     */
    function initialize(
        address _owner,
        ...
        _setMinTimeExtensionPerBid(_minTimeExtensionPerBid);
    }
     ...
     * @dev for more information look at _setMinTimeExtensionPerBid
     */
    function setMinTimeExtensionPerBid(uint256 _minTimeExtensionPerBid) external onlyOwner {
        _setMinTimeExtensionPerBid(_minTimeExtensionPerBid);
    }
     ...
     * @param _minTimeExtensionPerBid - time for which auction is extended when someone makes an offer
     */
    function _setMinTimeExtensionPerBid(uint256 _minTimeExtensionPerBid) private {
        if (_minTimeExtensionPerBid > 1 days) {
            revert AvalaunchNFTMarketplace__InvalidBiddingParameters();
        }
        minTimeExtensionPerBid = _minTimeExtensionPerBid;
        emit AuctionParametersSet(_minTimeExtensionPerBid);
    }
     ...
     */
    function addAuctionItem(address itemAddress, uint256 tokenId, uint256 startingPrice, uint256 timeExtensionPerBid) external {
        // Check price
        if (startingPrice == 0) {
            revert AvalaunchNFTMarketplace__ZeroValue();
        }
        // Create item
        auctionItems.push(
            AuctionItem(itemAddress, tokenId, payable(msg.sender), payable(address(0)), startingPrice, 0, timeExtensionPerBid, 0, true)
        );
        // Transfer item from user to marketplace
        IERC721Upgradeable(itemAddress).transferFrom(msg.sender, address(this), tokenId);
        // Emit event
    ...
    function claimAuctionItem(uint256 _itemId) external nonReentrant onlyExistingAuction(_itemId) {
        ...
        if (msg.sender != item.bidder && msg.sender != item.seller) {
            revert AvalaunchNFTMarketplace__InvalidCaller();
        }
        // Mark item as unavailable
        item.available = false;
