...
contract NodeManagerV1 is Initializable {
  ...
  function initialize(address[] memory addresses) public initializer {
    ...
    addTier("basic", 10 ether, 1 ether, 60, 0.001 ether);
  ...
  function _transferFee(uint256 amount) private {
    ...
    IERC20Upgradeable(tokenAddress).safeTransferFrom(address(msg.sender), address(rewardsPoolAddress), (amount * rewardsPoolFee) / 10000);
    IERC20Upgradeable(tokenAddress).safeTransferFrom(address(msg.sender), address(operationsPoolAddress), (amount * operationsPoolFee) / 10000);
  }
  ...
  function _claim(uint256 exceptAmount) private {
    ...
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == msg.sender && (node.limitedTime + payInterval) > uint32(block.timestamp)) {
          ...
          Tier storage tier = tierArr[node.tierIndex];
          claimableAmount += ((((uint256(block.timestamp - node.claimedTime) * tier.rewardsPerTime) * multiplier) / 1 ether) / tier.claimInterval);
    ...
    uint256 cA = claimableAmount - exceptAmount;
    if (claimFee > 0) {
      cA = cA - (cA * claimFee) / 10000;
      IERC20Upgradeable(tokenAddress).safeTransferFrom(address(operationsPoolAddress),address(msg.sender), ((cA * claimFee) / 10000));
    }
    IERC20Upgradeable(tokenAddress).safeTransferFrom(address(rewardsPoolAddress),address(msg.sender), cA);
  }
  ...
  function upgrade(
    ...
    if (claimableAmount > 0) {
      rewardsOfUser[msg.sender] += claimableAmount;
      rewardsTotal += claimableAmount;
      IERC20Upgradeable(tokenAddress).safeTransferFrom(address(rewardsPoolAddress),address(msg.sender), claimableAmount);
    }
  ...
  function transfer(
    ...
    if (claimableAmount > 0) {
      ...
      rewardsTotal += claimableAmount;
      IERC20Upgradeable(tokenAddress).safeTransferFrom(address(rewardsPoolAddress), address(from), claimableAmount);
    }
  ...
  function sellNode(uint32 indice) public {
    ...
    if (node.owner != address(0)) {
      ...
      countOfTier[tier.name]--;
      countTotal--;
      IERC20Upgradeable(tokenAddress).safeTransferFrom(address(rewardsPoolAddress), address(node.owner), (tier.price / 100) * sellPricePercent);
    }
  }

  function withdraw(address anyToken, address recipient) external onlyOwner() {
    IERC20Upgradeable(anyToken).safeTransfer(recipient, IERC20Upgradeable(anyToken).balanceOf(address(this)));
  }
