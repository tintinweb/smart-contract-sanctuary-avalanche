...
contract FNFT_Market is Ownable, CloneFactory {
    ...
    function putOnSale(
        ...
        require((listInfo[_key].maker == address(0)) && (listInfo[_key].collectionId == address(0)) || ((listInfo[_key].duration > 0) && (listInfo[_key].duration < block.timestamp) && listInfo[_key].duration != 0), "FNFT: alreay listed");
        ...
        listInfo[_key].duration = duration > 0 ? block.timestamp + duration * 1 days : 0;
    ...
    function cancelList (bytes32 _key) external isBlackList {
        ...
        require(!listInfo[_key].state || ((listInfo[_key].duration < block.timestamp) && (listInfo[_key].total > listInfo[_key].current)), "FNFT:booking duration");
    ...
    */
    ...
    function buyTier0 (bytes32 _key, address user, uint256 amount, uint price) private {
        require(listInfo[_key].duration >= block.timestamp && listInfo[_key].taker == user, "FNFT: not available");
    ...
    function sellTier0(bytes32 _key, address taker) external isBlackList returns(uint id) {
        ...
        require(listInfo[_key].duration >= block.timestamp, "not duration");
