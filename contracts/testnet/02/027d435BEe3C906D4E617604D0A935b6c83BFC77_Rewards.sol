    ...
    ReentrancyGuardUpgradeable
{
        ...
        for (uint256 i = 0; i < _yieldKey.length; i++) {
            ...
            if (_percentageDonations > 0) {
                bool success_treasury = _token.transferFrom(
                    ...
                    total.mul(mulDiv(_percentageDonations, 1, 100))
                );
                ...
                success_treasury = _token.transferFrom(
                    ...
                    _msgSender(),
                    total.mul(
                        mulDiv(uint256(100).sub(_percentageDonations), 1, 100)
                    )
                );
     ...
     */
    function compoundOfRewards(address _wallet) external whenNotPaused nonReentrant {
        (
            bool status,
            uint256 totalAmount,
            uint256 compoundAmount
        ) = VaLiFiHelpers.CompoundOfRewardsParameters(_wallet);
		require(_msgSender() == _wallet, "Only the Owner can compound of rewards");
