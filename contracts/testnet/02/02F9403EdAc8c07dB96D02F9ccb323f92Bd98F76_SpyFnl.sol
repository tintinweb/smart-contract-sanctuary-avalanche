...
contract SpyFnl is ERC721URIStorage, Ownable, ERC721Burnable {
    ...
    uint256 public _price_Public = 0.002 ether;
    ...
    uint256 public _maxMintPerWallet = 2;
    ...
    constructor(
        ...
        address royaltyAddr
    ) ERC721("SpyFnl Collection", "SpyFnl") {
    ...
    function splitFunds(uint256 mintValue) private {
        uint256 amountA = (mintValue * 75) / 100;
        uint256 amountB = (mintValue * 25) / 100;
    ...
    function _mintForAirdrop(address recipient) public onlyOwner {
        ...
        require(newItemId <= _maxMintable, "Mint Supply Reached");

        _mint(recipient, newItemId);
    }

    function airdropQuantity(address recipient, uint256 quantity) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) {
            _mintForAirdrop(recipient);
        }
    }
