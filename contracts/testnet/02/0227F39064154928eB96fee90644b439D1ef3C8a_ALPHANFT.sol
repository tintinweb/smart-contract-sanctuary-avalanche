    ...
    ReentrancyGuard 
{
    ...
    constructor() ERC721("Alpha Shares", "AlphaShares") {
    ...
    *   @dev safely mint ONLY the maximum number of each type of NFT
    ...
    {
        totalApex.increment();
        return createToken(_to, nftTypes.APEX);
    }
        ...
        returns (uint)  
    {
        ...
        return createToken(_to, nftTypes.RONIN);
    }
        ...
        returns (uint)  
    {
        ...
        return createToken(_to, nftTypes.ALPHA);
    }
        ...
        returns (uint)  
    {
        ...
        return createToken(_to, nftTypes.BASE);
    }
    ...
    * @dev setBaseURI - to be set after contract is deployed and metadata is ready
    ...
    function getNFTTAttrbutesByTokenId(
        uint256 tokenId
        )
        external
        view
        _tokenExists(tokenId)
        returns(AlphaNFTAttributes memory)
    {
        return alphaNodes[tokenId];
    }
    ...
    * @dev toggle earning profit - will be toggled off during auction
    ...
    function getIsTokenEarningById(
        uint256 tokenId
        )
        external
        view
        returns(bool)
    {
        return alphaNodes[tokenId].isEarning;
    }   

    function getNFTTypeByTokenId(
        uint256 tokenId
        )
        external
        view
    ...
    *   @dev Marketplace only function, auto sets to not earning if isForSale is true
    ...
    function setIsForSale(
        ...
        _tokenExists(tokenId) 
    {
        bool previousMarketStatus = alphaNodes[tokenId].isForSale;
        // set is earning to the inverse of isForSale
        alphaNodes[tokenId].isForSale = isForSale;
        emit boolAttributeChanged(tokenId, "isForSale", previousMarketStatus, alphaNodes[tokenId].isForSale);
    }
