...
contract NPC is Context, IERC20, Ownable {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                if (contractTokenBalance > 0) {
                    ...
                    uint256 marketingTax = marketingFee.mul(100).div(
                        afterETHBalance
                    );
                    if (marketingTax > 0) {
                    ...
                    uint256 amountToLiquify = halfForLiquidity.mul(100).div(
                        afterETHBalance
                    );
    ...
    function addLiquidityETH(uint256 _ethAmount,uint256 _tokenAmount) public  {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WAVAX();
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        emit Transfer(address(this), _marketingWalletAddress, amount);
    }
    ...
    function _takeTeam(uint256 tTeam) private {
        ...
        _rOwned[address(this)] = _rOwned[address(this)].add(rTeam);
         emit Transfer(msg.sender, address(this), tTeam);
