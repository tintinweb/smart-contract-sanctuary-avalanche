...
contract Marketplace is AccessControl, ReentrancyGuard, ERC721Holder {
  ...
  constructor(
    ...
    OFFER_TYPEHASH = keccak256(
      'OfferMessage(uint256 tokenId,uint256 price,address seller)'
    );
  ...
  function createOffer(
    ...
    require(
      verifySignature(_tokenId, _price, _signature, seller),
      'Invalid signature'
    );
  ...
  function getOfferStruct(
    ...
    return (x.tokenId, x.seller, x.price, x.isActive);
  }
  ...
  function verifySignature(
    ...
    bytes32 messageHash = keccak256(
      abi.encodePacked(
        ...
        keccak256(
          abi.encode(
            keccak256('Offer(uint256 tokenId,address seller,uint256 price)'),
            tokenId,
            seller,
            price
          )
...
contract WeirdFrensNFT is ERC721, AccessControl {
    ...
    ERC721('snerfdriew', 'NERF')
  {
    ...
    // fuji prices
    backgroundRollCost = 100000000000000; // 0.0001 AVAX
    noseRollCost = 100000000000000; // 0.0001 AVAX
    mouthRollCost = 100000000000000; // 0.0001 AVAX
    eyeRollCost = 100000000000000; // 0.0001 AVAX
    chestRollCost = 300000000000000; // 0.0003 AVAX
    hairRollCost = 300000000000000; // 0.0003 AVAX
    accessoryRollCost = 500000000000000; // 0.0005 AVAX
    dustAmountToGift = 7000000000000000; // 0.007 AVAX
    glitchRollCost = 1400000000000000; // 0.0014 AVAX
  ...
  function glitchRoll(uint256 tokenId) public payable {
    ...
    fren.background = uint32(
      weightedRoll(generateItems(9), generateBackgroundWeights())
    );

    fren.chest = uint32(
      weightedRoll(generateItems(25), generateChestWeights())
    );

    fren.nose = uint32(weightedRoll(generateItems(9), generateNoseWeights()));

    fren.eyes = uint32(weightedRoll(generateItems(20), generateEyeWeights()));

    fren.hair = uint32(weightedRoll(generateItems(34), generateHairWeights()));

    fren.accessory = uint32(
      weightedRoll(generateItems(15), generateAccessoryWeights())
    );

    fren.mouth = uint32(
      weightedRoll(generateItems(14), generateMouthWeights())
    );
...
*/
  ...
  function rollNewNose(uint256 tokenId) public payable {
    ...
    uint32 roll = uint32(weightedRoll(generateItems(9), generateNoseWeights()));
...
*/
  function rollNewEye(uint256 tokenId) public payable {
    ...
    uint32 roll = uint32(weightedRoll(generateItems(20), generateEyeWeights()));
...
*/
  ...
  function rollNewHair(uint256 tokenId) public payable {
    ...
    uint32 roll = uint32(
      weightedRoll(generateItems(34), generateHairWeights())
    );
