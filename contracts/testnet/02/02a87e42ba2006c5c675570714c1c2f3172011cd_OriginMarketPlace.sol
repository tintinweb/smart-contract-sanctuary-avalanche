    ...
    Ownable
{
    ...
    function acceptOffer(uint256 _nonce, address _token) external nonReentrant {
        ...
        bool isTransferSuccess = _customTransfer(
            ...
            idToListedToken[currentOffer.listingID].tokenId
        );
        ...
        if (
            idToListedToken[currentOffer.listingID].RoyaltyPercentage > 0 &&
            idToListedToken[currentOffer.listingID].RoyaltyAddress != address(0)
        ) {
            fee =
                (currentOffer.bidPrice *
                    idToListedToken[currentOffer.listingID].RoyaltyPercentage) /
                ROYALTY_MAX;
            if (fee > 0) {
                userReceipt += fee;
                bool isRoyaltyTxSuccess = token.transferFrom(
                    payable(currentOffer.offerer),
                    payable(
                        idToListedToken[currentOffer.listingID].RoyaltyAddress
                    ),
                    fee
                );
