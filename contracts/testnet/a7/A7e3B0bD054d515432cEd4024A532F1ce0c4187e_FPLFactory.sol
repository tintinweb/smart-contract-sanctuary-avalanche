...
import "../interfaces/IPoolMechanics.sol";
import "./FPLPoolProxy.sol";

contract FPLFactory is AccessControl {
    event FPLPoolCreated(
        // address fplPoolProxyAddr
    ...
    event NewStakingTokenWasSet(
        address indexed newStakingToken,
        address indexed oldStakingToken
    );

    event NewRewardTokenWasSet(
        address indexed newRewardToken,
        address indexed oldRewardToken
    );

    event NewFPLPoolImplWasSet(
        address indexed newPoolImplemnetation,
        address indexed oldPoolImplemnetation
    );
    ...
    event NewPoolMechanicsWasSet(
        address indexed newPoolMechanics,
        address indexed oldPoolMechanics
    );

    error NotADifferentImpl();
    error DoesNotSupportIFPLPool();
    error FactoryIsNotAllowedToCreatePool();

    bytes32 public OWNER_ROLE = keccak256("OWNER_ROLE");
    bytes32 public OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    // ISparkWorldToken public sparkWorldToken;
    IRewardManager public rewardManager;
    IPoolMechanics public poolMechanicsContract;

    address public fplPoolImpl;
    address public superAdmin;
    address public feeWallet;

    /**
     * @param _owner project owner’s wallet that is used to change operator in case of operator's
     *        private key exposure. This address's private key must not be on a server.
     */
    constructor(
        IRewardManager _rewardManager,
        IPoolMechanics _poolMechanincs,
        address _fplPoolImpl,
        address _owner,
        address _superAdmin,
        address _feeWallet
    ) {
        ...
        poolMechanicsContract = _poolMechanincs;
        feeWallet = _feeWallet;
        if (
            IFPLPool(_fplPoolImpl).supportsInterface(type(IFPLPool).interfaceId)
        ) {
            address oldFPLPoolImpl = fplPoolImpl;
            fplPoolImpl = _fplPoolImpl;
            emit NewFPLPoolImplWasSet(_fplPoolImpl, oldFPLPoolImpl);
        } else revert DoesNotSupportIFPLPool();
        superAdmin = _superAdmin;
    }

    function createFPLPoolProxy(
        ...
        // uint256 oracleDecimal,
        // uint256 minimumBalance
        uint256[7] memory _variables
    ) external onlyRole(OPERATOR_ROLE) returns (address) {
        if (
            !IRewardManager(rewardManager).hasRole(OPERATOR_ROLE, address(this))
        ) revert FactoryIsNotAllowedToCreatePool();

        FPLPoolProxy fplPoolProxy = new FPLPoolProxy();
        address fplPoolProxyAddr = address(fplPoolProxy);

        fplPoolProxy.upgradeTo(fplPoolImpl);

        fplPoolProxy.initialize(
            // sparkWorldToken,
            rewardManager,
        ...
        uint256 stakingTokenDecimals = IERC20MetadataUpgradeable(_stakingToken)
            .decimals();
        uint256 rewardTokenDecimals = IERC20MetadataUpgradeable(_rewardToken)
            .decimals();
        string memory stakingTokenSymbol = IERC20MetadataUpgradeable(
            _stakingToken
        ).symbol();
        string memory rewardTokenSymbol = IERC20MetadataUpgradeable(
            _rewardToken
        ).symbol();
        ...
        IRewardManager(rewardManager).addPool(fplPoolProxyAddr);

        return fplPoolProxyAddr;
    }

    function setNewFPLPoolImplementationAddr(address _ImpAdr)
        external
        onlyRole(OPERATOR_ROLE)
    {
        if (fplPoolImpl == _ImpAdr) revert NotADifferentImpl();
        if (IFPLPool(_ImpAdr).supportsInterface(type(IFPLPool).interfaceId)) {
            address oldFPLPoolImpl = fplPoolImpl;
            fplPoolImpl = _ImpAdr;
            emit NewFPLPoolImplWasSet(_ImpAdr, oldFPLPoolImpl);
        } else revert DoesNotSupportIFPLPool();
    }
    ...
    function setNewPoolMechanics(IPoolMechanics _poolMechanics)
        external
        onlyRole(OPERATOR_ROLE)
    {
        address oldPoolMechanics = address(poolMechanicsContract);
        poolMechanicsContract = _poolMechanics;

        emit NewPoolMechanicsWasSet(address(_poolMechanics), oldPoolMechanics);
    }
}
...
contract FPLPoolProxy is FPLPoolStorageStructure {
    event FPLPoolImplUpgraded(address indexed newImpl, address indexed oldImpl);

    error NotADifferentImpl();
    error LaunchDateCanNotBeInThePast();
    error NotASupportedNFTCollection();

    constructor() {
        _setupRole(SUPER_ADMIN_ROLE, _msgSender());
    }

    function upgradeTo(address _newFPLPoolImplementation)
        external
        onlyRole(SUPER_ADMIN_ROLE)
    {
        if (FPLPoolImplementation == _newFPLPoolImplementation)
            revert NotADifferentImpl();
        _setFPLPoolImplementation(_newFPLPoolImplementation);
    }

    function initialize(
        // ISparkWorldToken _sparkWorldToken,
        IRewardManager _rewardManager,
        IPoolMechanics _poolMechanincs,
        address _stakingToken,
        address _rewardToken,
        ...
        if (_variables[0] < block.timestamp)
            revert LaunchDateCanNotBeInThePast();

        if (
            INFTCollection(_nftCollection).supportsInterface(
                type(INFTCollection).interfaceId
            ) ||
            INFTBank(_nftCollection).supportsInterface(
                type(INFTBank).interfaceId
            )
        ) {
            nftCollectionContract = _nftCollection;
            if (
                INFTBank(_nftCollection).supportsInterface(
                    type(INFTBank).interfaceId
                )
            ) isExternalCollection = true;
        } else revert NotASupportedNFTCollection();
        ...
        rewardAmount = _variables[3];
        minimumStakeAmount = _variables[4];
        minimumBalance = _variables[6];

        if (oracleContract == address(0)) oracleDecimal = _variables[5];
        else oracleDecimal = PriceConsumer.getDecimals(oracleContract);
    ...
    fallback() external payable {
        address opr = FPLPoolImplementation;
    ...
    function _setFPLPoolImplementation(address _newFPLPoolImpl) internal {
        address oldImpl = FPLPoolImplementation;
        FPLPoolImplementation = _newFPLPoolImpl;
        emit FPLPoolImplUpgraded(_newFPLPoolImpl, oldImpl);
    }
}
...
import "../interfaces/IPoolMechanics.sol";
import "../interfaces/IFPLPool.sol";
...
contract FPLPoolStorageStructure is AccessControlUpgradeable {
    address public FPLPoolImplementation;
    ...
    uint256 public priceOnMaturity;
    uint256 public oracleDecimal;
    uint256 public stakeAPY;
    uint256 public minimumBalance;

    address public oracleContract;
    address public nftCollectionContract;
    ...
    bool public isExternalCollection;
    ...
    mapping(address => IFPLPool.StakeWithPrediction) public stakes;
    ...
    {
        return
            interfaceId == type(IFPLPool).interfaceId ||
...
interface IFPLPool is IERC165Upgradeable {
    ...
    event PoolMatured();
    event PoolDeleted();
    event MaturingPriceUpdated(uint256 priceOnMaturity);
    event UsersUnfreezed(address[] users);
    event EmergencyModeIsOn();
    event EmergencyModeIsOff();

    error NotChangingActivationStatus();
    error PoolIsNotActive();
    error CanNotStakeBeforeLaunchDate();
    error CanNotStakeAfterLockDate();
    error PoolIsLocked();
    error PoolIsNotMatured();
    error AmountCanNotBeLessThanMinimum();
    error AlreadyStaked();
    error WrongNumberOfPredictions();
    error CanNotLockPoolBeforeLockTime();
    error CanNotSetPriceBeforeMaturityTime();
    error CanNotSetWinnersBeforeMaturityTime();
    error CanNotEndPoolBeforeMaturityTime();
    error WinnersNotSetYet();
    error WrongPercentages(uint256 requested, uint256 remaining);
    error TotealPercentagesMustBe100(uint256 requested);
    error PercentagesMustBeDecremental();
    error AlreadyUnstaked();
    error CallerIsNotAStaker();
    error NotEnoughFundsInRewardManager();
    error CanNotWithdrawStakingTokens();
    error UserDoesNotHaveEnoughNativeTokenBalance();

    function setActivationStatus(bool _activationStatus) external;

    function getTier(uint256 _amount) external view returns (uint256 tier);
    ...
    function stake(uint256 _amount, uint256[] memory _pricePredictions)
        external;

    function getEntryFee(uint256 _tier) external view returns (uint256);

    function lockPool() external;

    function setOracleToZero() external;

    function updateMaturingPrice(uint256 _price) external;

    function setWinners(
        address[] calldata _winners,
        uint256[] calldata _percentages,
        uint256[] calldata _bestPredictions,
        address[] calldata _deletedWinners,
        bool lastSlot,
        bool deletePreviousWinners
    ) external;

    function getCurrentPercentages() external view returns (uint256 percentage);

    function endPool() external;

    function unstake() external;

    function getPrize(address user) external view returns (uint256 reward);

    function getStakingReturn(address user)
        external
        view
        returns (uint256 stakingReturn);

    function deletePool() external;

    function userStaked(address user) external view returns (bool);

    function getStakers() external view returns (address[] memory);

    function getWinners() external view returns (address[] memory);

    function getUsersPredictions(address user)
        external
        view
        returns (uint256[] memory);

    function getAdditionalRewards(address user)
        external
        view
        returns (uint256 additionalRewards);
    ...
    function setEmergencyModeTo(bool emergencyMode) external;
}
...
interface INFTCollection is IERC165Upgradeable {
    ...
    error RequestedPublicModeMintCountExceedsTotal();
