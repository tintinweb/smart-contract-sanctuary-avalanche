...
contract TalentScoutPresale is ReentrancyGuard, Ownable {
    ...
    /* EXTERNAL OR PUBLIC */
        ...
        function buy() public payable nonReentrant {
            ...
            _totalTsLock[msg.sender] = _totalTsLock[msg.sender].add(tokensToBuy).div(1e18); // add to user total ts lock
        ...
        */
        function releasePresaleAmount(uint256 lock_id) public {
            ...
            uint256 precisionloss = _locks[msg.sender][lock_id].locked.mod(1e18).div(1e10);
            // uint256 tokensforprecisionloss = precisionloss.mod(1);
            // uint256 tokensToRelease = (_locks[msg.sender][lock_id].locked).sub(precisionloss);
            // // uint256 tokensReleased = distributed.add(tokensToRelease);

            uint precisionlossdiff = precisionloss.div(1e7); 

            if(precisionlossdiff >= 5){
                distributed = distributed.add(_locks[msg.sender][lock_id].locked);
                uint256 tokensToRelease = _locks[msg.sender][lock_id].locked.div(1e18).add(1);
                talentScout.transferFrom(tsAddress, msg.sender, tokensToRelease);
                
            }
            else{
                distributed = distributed.add(_locks[msg.sender][lock_id].locked);
                uint256 tokensToRelease = _locks[msg.sender][lock_id].locked.div(1e18);
                talentScout.transferFrom(tsAddress, msg.sender, tokensToRelease);
            }
        ...
        function refundAVAX() external {
            ...
            totalPresaleLockedAmount = totalPresaleLockedAmount.sub((_totalTsLock[msg.sender]).mul(1e18));
