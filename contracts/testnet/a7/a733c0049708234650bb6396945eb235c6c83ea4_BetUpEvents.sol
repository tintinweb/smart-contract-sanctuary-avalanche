    ...
    */
    ...
    function withdrawToVault(uint256[] memory pairIDs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint256 i = 0; i < pairIDs.length; i++) {
            ...
            uint256 others = (innerPair.total - optionsVolume[keccak256(abi.encodePacked(pairID, innerPair.resultID))].amount);
    ...
    function _getReward(
        ...
        if(innerPair.resultID == INVALID_ID) {
            ...
            if(innerPair.resultID == _resultID) {
                uint256 _profit = (9 * _player.amountWithBonus *(innerPair.total - op)) / (10 *
                        optionsVolume[keccak256(abi.encodePacked(_pairID,_resultID))].amountWithBonus);
                refund = _player.amount +  _profit;
                reward = winPer * _player.amount  /(10 ** _chipDecimal);
            }else {
                if(innerPair.total - op == 0) {
                    refund = _player.amount;
                }else {
            ...
            block.timestamp > pair.endStackAt && (innerPair.total - op == 0)
        )  {
            refund = _player.amount;
        }
