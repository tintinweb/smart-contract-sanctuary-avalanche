...
contract AvaxSavingNew is ReentrancyGuard, Context {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < level0.length; i++) {
            ...
            level0SC = getUserSCFromDeposit(level0[i]);
        }
        for (uint256 i = 0; i < level1.length; i++) {
            ...
            level1SC = getUserSCFromDeposit(level0[i]);
        ...
        for (uint256 i = 0; i < level2.length; i++) {
            ...
            level2SC = getUserSCFromDeposit(level0[i]);
    ...
    function getUserSCFromDeposit(address userAddress)
        public
        view
        returns (uint256 sc)
    {
        ...
        for (uint256 j = 0; j < depositCount; j++) {
            (, , , uint256 percent, uint256 amount, , , ) = getUserDepositInfo(
                userAddress,
                j
            );
            sc = sc.add(amount.mul(percent).div(PERCENTS_DIVIDER));
        }
    }
