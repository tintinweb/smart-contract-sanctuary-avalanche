...
contract AttributesPets is Controllable {
    ...
    struct Attributes {
        ...
        uint8 rarity;
        uint8 boostName;
        // 0 none | 1 ranger boost | 2 wallet boost | 3 both
        uint8 boostType; 
    }
    ...
    string[] boostNames = [
        ...
        "Stubborn Grower",    // 15 Global production speed + increase % chance to get all $GLOW stolen at unstake
        "Ultimate Slug",      // 16 All nft time to unstake
        "Ultimate Staker"     // 17 Increases global production speed according to NFTs staked number
    ];
    ...
    function getPetBoostType(uint8 petId) external view returns (uint8) {
        return attributes[petId].boostType;
    } 
    ...
    function uploadAttributes(
        ...
        uint8[] memory _boostTypes
    ) external onlyController {
        for (uint256 i; i < _petIds.length;) {
            while (attributes.length < _petIds[i]) {
                attributes.push();
            }
            if (attributes.length == _petIds[i]) {
                attributes.push(Attributes(_names[i], _pngs[i], _rarities[i], _boostNames[i], _boostTypes[i]));
            } else {
                attributes[_petIds[i]] = Attributes(_names[i], _pngs[i], _rarities[i], _boostNames[i], _boostTypes[i]);
            }
            unchecked{++i;}
        }
    }
    ...
    function uploadBoosts(
        ...
        for (uint256 i; i < _uploadedBoosts.length;) {
    ...
    function draw(uint8 attributeId) internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<image x="2" y="2" width="32" height="32" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
    ...
    function drawSVG(uint8 attributeId) public view returns (string memory) {
        ...
        return
            string(
                abi.encodePacked(
                    '<svg id="yh_pet" width="100%" height="100%" version="1.1" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
        ...
        returns (string memory)
    {
        string memory metadata = string(
            abi.encodePacked(
                '{"name": "pipi #', // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< "Yield Hunt Pet" for real launch
                tokenId.toString(),
                '", "description": "pipi", "image": "data:image/svg+xml;base64,', // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< "Yield Hunt Pets | Equip pets to your Rangers to help them achieve their tasks." for real launch
