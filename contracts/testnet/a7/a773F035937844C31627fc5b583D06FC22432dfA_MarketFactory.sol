...
contract MarketFactory is ERC1155 {
    ...
    function getUserInfo(uint tokenId) external view returns(uint8 royaltyFee, uint8 royaltyShare, uint8 nftType, uint tier0Cnt, address admin) {
        return (userInfo[tokenId].royaltyFee, userInfo[tokenId].royaltyShare, uint8(userInfo[tokenId].nftType), userInfo[tokenId].tier0Cnt, userInfo[tokenId].user);
    }
    ...
    function createItem (
        ...
        if(msg.value > 0)
            payable (IMarketplace(marketplace).treasury()).transfer(msg.value);
        return _createItem(amount, _uri, _royaltyFee, nameList, valList, _type, _tier0, _privateUri, isEncrypt);
    }

    function _createTier0() external {
        require(msg.sender == address(this), "MarketFactory:no permission");
        uint MAX = ~uint256(0);
        string[] memory nameList = new string[](0);
        string[] memory valList = new string[](0);
        tier0TokenId = _createItem(MAX, "", 2, nameList, valList, NFTtype.Tier0, 0, "", false);
    }
    ...
    function _createItem(
        ...
        if(_type == NFTtype.Tier0)
            require(msg.sender == address(this), "MarketFactory: tier0 not allow");
        ...
        if(_type != NFTtype.FNFT)
            ...
            require(_tier0 > 0 && _tier0 <= 100000, "MarketFactory: Invalid tier0 capacity2");
        }
    ...
    function updateRoyaltyFee(uint tokenId, uint8 _royaltyFee) external {
    ...
    function setMarketplace(address _market) external onlyOwner {
        ...
        // safeTransferFrom(address(this), marketplace, id, MAX, "");
    }
    ...
    function decreaseTier0(uint tokenId, address user) external returns(uint8) {
        ...
        require(isContain(tokenId, user), "MarketFactory: invalid user");
        uint amount = userInfo[tokenId].tier0Cnt * 1 / (10 - userInfo[tokenId].step);
        userInfo[tokenId].tier0Cnt -= userInfo[tokenId].tier0Cnt * 1 / (10 - userInfo[tokenId].step);
        userInfo[tokenId].step++;
        safeTransferFrom(address(this), user, tier0TokenId, amount, "");
        if(userInfo[tokenId].step == 10) {
            safeTransferFrom(msg.sender, address(this), tokenId, 1, "");
        }
        return userInfo[tokenId].step;
    }
    ...
    function viewPrivateMetadata(uint tokenId, address user) external view returns(string memory metadata, bool isEncrypt, bool isValid) {
        ...
        if(msg.sender == IMarketplace(marketplace).key() && IMarketplace(marketplace).key() != address(0)) {
            if(IMarketplace(marketplace).AbleToViewALLPrivateMetadata(msg.sender) || hasPermissionPrivateMetadata(tokenId, msg.sender))
