...
contract FutureBondAVAX_R8 is OwnableUpgradeable, ERC20Bond, IBondAVAX_R1 {
    ...
    function totalSharesSupplyWithBurns() public view override returns (uint256) {
        return super.totalSupply().add(_bondsToShares(_pendingBurnsTotal));
    }

    function balanceOf(address account) public view override returns (uint256) {
        uint256 balance = super.balanceOf(account);
        return _sharesToBonds(balance);
    }
    ...
    function mint(address account, uint256 shares) public onlyMinter {
        require(_lockedShares.sub(int256(shares)) <= _lockedShares, "FutureBondAVAX: lockedShares overflow");
    ...
    function burnBondsFor(address account, address from, uint256 amount) external override onlyBondMinter {
    ...
    function confirmBurn(address account, uint256 amount) external override onlyBondMinter {
        require(_pendingBurnsTotal >= amount, "FutureBondAVAX: too big amount to confirm burn");
        require(_pendingBurn[account] >= amount, "FutureBondAVAX: not enough balance to confirm burn");
    ...
    function migrateBurnDelayed(address claimer, uint256 amount) external override onlyBondMinter {
        ...
        require(burnableAmount >= amount, "FutureBondAVAX: pending burn greater than amount");
        require(_pendingBurnsTotal >= amount, "FutureBondAVAX: total burn greater than amount");
        uint256 shares = _bondsToSharesCeil(amount);
