...
contract Staking is Ownable {
    ...
    function stake(uint amount, bool locked) external {
      ...
      for(uint i; i < rewards.length; i++) {
        rewards[i].onStake(msg.sender, user.weight, newWeight);
      }
    ...
    function unstake(uint amount) external {
      ...
      // Notify all pools registered
      for(uint i; i < rewards.length; i++) {
        rewards[i].onUnstake(msg.sender, user.weight, user.staked);
      }
...
interface RewardStrategy {
    function onStake(address _investor, uint oldWeight, uint newWeight) external;
    function onUnstake(address _investor, uint oldWeight, uint newWeight) external;
}
