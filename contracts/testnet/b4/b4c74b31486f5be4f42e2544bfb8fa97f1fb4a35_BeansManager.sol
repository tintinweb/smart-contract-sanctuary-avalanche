    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    CountersUpgradeable.Counter private _beanCounter;
    mapping(uint256 => BeanEntity) private _beans;
    ...
    function initialize() external initializer {
        // Initialize contract
        __ERC721_init("Magic Beans", "BEAN");
        __Ownable_init();
    ...
    modifier onlyBeanOwner() {
        ...
        require(
            isOwnerOfBeans(sender),
            "Factions: No Bean owned by this account"
        );
        require(
            !liquidityPoolManager.isFeeReceiver(sender),
            "Factions: Fee receivers cannot own Bean"
        );
        _;
    }

    modifier checkPermissions(uint256 _beanId) {
        address sender = _msgSender();
        require(beanExists(_beanId), "Factions: This bean doesn't exist");
        require(
            isOwnerOfBean(sender, _beanId),
            "Factions: You do not control this Bean"
        );
        _;
    }
        ...
        returns (string memory)
    {
        ...
        //_setTokenURI(newBeanId, tokenURI);

        return ERC721URIStorageUpgradeable.tokenURI(tokenId);
    }

    function createBeanWithTokens(
        string memory beanName,
        uint256 beanValue
    ) external nonReentrant whenNotPaused gardenSet returns (uint256) {
        address sender = _msgSender();

        require(
            bytes(beanName).length > 4 && bytes(beanName).length < 32,
            "Factions: Incorrect name length. Must be between 4 to 32"
        );
        require(
            beanValue >= creationMinPrice,
            "Factions: Bean value set below minimum"
        );
        require(
            isNameAvailable(sender, beanName),
        ...
        garden.accountBurn(sender, beanValue);
        ...
        _beanCounter.increment();

        uint256 newBeanId = _beanCounter.current();
        uint256 currentTime = block.timestamp;

        // Add this to the TVL
        totalValueLocked += beanValue;
        logTier(tierLevel[0], int256(beanValue));

        // Add Bean
        _beans[newBeanId] = BeanEntity({
            id: newBeanId,
            name: beanName,
        ...
        // Assign the Bean to this account
        _mint(sender, newBeanId);

        emit Create(sender, newBeanId, beanValue);

        return newBeanId;
    }

    function cashoutReward(uint256 _beanId)
        external
        nonReentrant
        onlyBeanOwner
        checkPermissions(_beanId)
        whenNotPaused
        gardenSet
    {
        address account = _msgSender();
        uint256 reward = _getBeanCashoutRewards(_beanId);
        _cashoutReward(reward);

        emit Cashout(account, _beanId, reward);
    }

    function renameBean(uint256 _beanId, string memory beanName)
        external
        nonReentrant
        onlyBeanOwner
        checkPermissions(_beanId)
        ...
        gardenSet
    {
        address account = _msgSender();
        require(
            bytes(beanName).length > 1 && bytes(beanName).length < 32,
            "Beans: Incorrect name length, must be between 2 to 31"
        );
        BeanEntity storage bean = _beans[_beanId];
        require(bean.beanValue > 0, "Error: Bean is empty");
        (uint256 newBeanValue, uint256 feeAmount) = getProcessingFee(
            bean.beanValue,
            processingFee[3]
        );
        logTier(bean.rewardMult, -int256(feeAmount));
        burnedFromRenaming += feeAmount;
        bean.beanValue = newBeanValue;
        string memory previousName = bean.name;
        bean.name = beanName;
        emit Rename(account, previousName, beanName);
    }
        ...
        gardenSet
    {
        ...
        uint256[] memory beansOwned = getBeanIdsOf(account);
        for (uint256 i = 0; i < beansOwned.length; i++) {
            rewardsTotal += _getBeanCashoutRewards(beansOwned[i]);
        }
        _cashoutReward(rewardsTotal);

        emit CashoutAll(account, beansOwned, rewardsTotal);
    }

    function compoundReward(uint256 _beanId)
        external
        nonReentrant
        onlyBeanOwner
        checkPermissions(_beanId)
        ...
        gardenSet
    {
        ...
        (
            uint256 amountToCompound,
            uint256 feeAmount
        ) = _getBeanCompoundRewards(_beanId);
        require(
            amountToCompound > 0,
        ...
        emit Compound(account, _beanId, amountToCompound);
    }
        ...
        gardenSet
    {
        ...
        uint256[] memory beansOwned = getBeanIdsOf(account);
        uint256[] memory beansAffected = new uint256[](beansOwned.length);

        for (uint256 i = 0; i < beansOwned.length; i++) {
            (
                uint256 amountToCompound,
                uint256 feeAmount
            ) = _getBeanCompoundRewards(beansOwned[i]);
            if (amountToCompound > 0) {
                beansAffected[i] = beansOwned[i];
                feesAmount += feeAmount;
                amountsToCompound += amountToCompound;
            } else {
                delete beansAffected[i];
            }
        }
        ...
        emit CompoundAll(account, beansAffected, amountsToCompound);
    }

    // Private reward functions

    function _getBeanCashoutRewards(uint256 _beanId)
        private
        returns (uint256)
    {
        BeanEntity storage bean = _beans[_beanId];

        if (!isProcessable(bean)) {
            return 0;
        }

        uint256 reward = calculateReward(bean);
        bean.totalClaimed += reward;

        if (bean.rewardMult != tierLevel[0]) {
            logTier(bean.rewardMult, -int256(bean.beanValue));
            logTier(tierLevel[0], int256(bean.beanValue));
        }

        bean.rewardMult = tierLevel[0];
        bean.lastProcessingTimestamp = block.timestamp;
        return reward;
    }

    function _getBeanCompoundRewards(uint256 _beanId)
        private
        returns (uint256, uint256)
    {
        ...
        if (reward > 0) {
            ...
            bean.lastProcessingTimestamp = block.timestamp;
            bean.beanValue += amountToCompound;
            bean.rewardMult += increaseMultiplier(bean.rewardMult);
    ...
    function isProcessable(BeanEntity memory bean)
        private
        view
        returns (bool)
    {
        return
            block.timestamp >= bean.lastProcessingTimestamp + compoundDelay;
    }

    function calculateReward(BeanEntity memory bean)
        private
        view
        returns (uint256)
    {
        return
            _calculateRewardsFromValue(
                bean.beanValue,
                bean.rewardMult,
                block.timestamp - bean.lastProcessingTimestamp,
                rewardPerDay
            );
    }

    function rewardPerDayFor(BeanEntity memory bean)
        private
        view
        returns (uint256)
    {
    ...
    function _calculateRewardsFromValue(
        ...
        return (rewardsMultiplicated * _beanValue) / 100000;
    }

    function beanExists(uint256 _beanId) private view returns (bool) {
        require(_beanId > 0, "Factions: Id must be higher than zero");
        BeanEntity memory bean = _beans[_beanId];
        if (bean.exists) {
            return true;
        }
        return false;
    ...
    function calculateEstimatedRewardPerDay(address _account) external view returns (uint rewardsTotal) {
        uint256[] memory beansOwned = getBeanIdsOf(_account);
        for (uint256 i = 0; i < beansOwned.length; i++) {
            rewardsTotal += rewardPerDayFor(_beans[beansOwned[i]]);
        }
    }
    ...
    function calculateTotalPendingReward(address _account) external view returns (uint rewardsTotal) {
        uint256[] memory beansOwned = getBeanIdsOf(_account);
        for (uint256 i = 0; i < beansOwned.length; i++) {
            rewardsTotal += calculateReward(_beans[beansOwned[i]]);
        }
    }
    ...
    function isNameAvailable(address account, string memory beanName)
        public
        view
        returns (bool)
    {
        uint256[] memory beansOwned = getBeanIdsOf(account);
        for (uint256 i = 0; i < beansOwned.length; i++) {
            BeanEntity memory bean = _beans[beansOwned[i]];
            if (keccak256(bytes(bean.name)) == keccak256(bytes(beanName))) {
                return false;
            }
        }
        return true;
    ...
    function isOwnerOfBeans(address account) public view returns (bool) {
        return balanceOf(account) > 0;
    }

    function isOwnerOfBean(address account, uint256 _beanId)
        public
        view
        returns (bool)
    {
        uint256[] memory beanIdsOf = getBeanIdsOf(account);
        for (uint256 i = 0; i < beanIdsOf.length; i++) {
            if (beanIdsOf[i] == _beanId) {
                return true;
            }
        }
        return false;
    ...
    function getBeanIdsOf(address account)
        public
        view
        returns (uint256[] memory)
    {
        uint256 numberOfBeans = balanceOf(account);
        uint256[] memory beanIds = new uint256[](numberOfBeans);
        for (uint256 i = 0; i < numberOfBeans; i++) {
            uint256 beanId = tokenOfOwnerByIndex(account, i);
            require(
                beanExists(beanId),
                "Factions: This bean doesn't exist"
            );
            beanIds[i] = beanId;
        }
        return beanIds;
    ...
    function getBeansByIds(uint256[] memory _beanIds)
        external
        view
        returns (BeanInfoEntity[] memory)
    {
        BeanInfoEntity[] memory beansInfo = new BeanInfoEntity[](
            _beanIds.length
        );
        for (uint256 i = 0; i < _beanIds.length; i++) {
            uint256 beanId = _beanIds[i];
            BeanEntity memory bean = _beans[beanId];
            beansInfo[i] = BeanInfoEntity(
                bean,
                beanId,
                calculateReward(bean),
                rewardPerDayFor(bean),
                compoundDelay
            );
        }
        return beansInfo;
    ...
    function changeNodePrice(uint256 _creationMinPrice, uint256 _creationMaxPrice) public onlyOwner {
        require(
            ...
            "Factions: Price to create a Bean must be above 0"
        );
