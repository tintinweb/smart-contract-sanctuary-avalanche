...
import {IRouter} from "../interfaces/traderjoe/IRouter.sol";
...
contract AaveVault is Vault {
    ...
    constructor(
        ...
        IRouter _router,
        IERC20 _native
    ) Vault(_asset, name, symbol, recipientInfo, keeper) {
    ...
    function _harvest() internal override returns (uint256) {
        ...
        for (uint256 i = 0; i < rewardsListLength; i++) {
            IERC20 rewardToken = IERC20(rewardsList[i]);
            uint256 rewards = rewardToken.balanceOf(address(this));
            if (address(rewardToken) != address(asset) && rewards > 0) {
                rewardToken.safeApprove(address(router), rewards);

                address[] memory rewardToAssetPath = new address[](2);
                if (address(rewardToken) == address(native)) {
                    rewardToAssetPath[0] = address(rewardToken);
                    rewardToAssetPath[1] = address(asset);
                } else {
                    rewardToAssetPath = new address[](3);
                    rewardToAssetPath[0] = address(rewardToken);
                    rewardToAssetPath[1] = address(native);
                    rewardToAssetPath[2] = address(asset);
                }

                router.swapExactTokensForTokens(rewards, 0, rewardToAssetPath, address(this), block.timestamp);
            }
        }
    ...
    function _deposit(address, uint256) internal override {
        ...
        if (currentAssets > 0) {
            asset.safeApprove(address(lendingPool), currentAssets);
...
contract Vault is ERC20, ReentrancyGuard, AccessControl, Pausable {
    ...
    uint256 public constant MIN_UPDATE_RATES_PERIOD = 1800;
    uint256 public constant RATE_PRECISION = 1e18;
    Rate[] public rates;

    Info[] public historyInfo;

    int256 public totalEarnings = 0;

    uint256 public constant MAX_RECIPIENT_WEIGHT = 10000;
    ...
    event UpdateHistoryInfo(int256 ratePerSec, uint256 totalAssets, uint256 totalSupply, uint256 timestamp);

    event UpdateRate(uint256 rate, uint256 totalAssets, uint256 totalSupply, uint256 timestamp);
    ...
    constructor(
        ...
        string memory name,
        string memory symbol,
        RecipientInfo memory _recipientInfo,
        address _keeper
    ) ERC20(name, symbol) {
        require(_recipientInfo.recipients.length == _recipientInfo.recipientWeights.length, "length error");
        require(
            _recipientInfo.recipientWeights.sum() + _recipientInfo.harvesterWeight <= MAX_RECIPIENT_WEIGHT,
            "weights error"
        );
        asset = _asset;
    ...
    function ratePerSec() public view returns (int256) {
        uint256 _length = ratesLength();
        if (_length < 2) return 0;

        Rate memory rate1 = rates[_length - 1];
        Rate memory rate2 = rates[_length - 2];

        if (rate1.timestamp == rate2.timestamp) return 0;

        return (int256(rate1.rate) - int256(rate2.rate)) / int256(rate1.timestamp - rate2.timestamp);
    }

    function ratesLength() public view returns (uint256) {
        return rates.length;
    }

    function getHistoryInfo(uint256 startTimestamp) public view returns (Info[] memory) {
        Info[] memory _historyInfo = new Info[](2);

        uint256 _historyInfoLength = 0;
        uint256 _length = historyInfo.length;

        if (_length < 2) {
            return new Info[](0);
        }

        for (int256 i = int256(_length - 1); i >= 0; i--) {
            if (historyInfo[uint256(i)].timestamp >= startTimestamp) {
                _historyInfoLength++;
            }
        }

        if (_historyInfoLength < 2) {
            _historyInfo[0] = historyInfo[_length - 2];
            _historyInfo[1] = historyInfo[_length - 1];
            return _historyInfo;
        }

        _historyInfo = new Info[](_historyInfoLength);
        int256 _historyInfoCount = int256(_historyInfoLength - 1);

        for (int256 i = int256(_length - 1); i >= 0; i--) {
            if ((historyInfo[uint256(i)].timestamp >= startTimestamp)) {
                _historyInfo[uint256(_historyInfoCount)] = historyInfo[uint256(i)];
                _historyInfoCount--;
            }

            if (_historyInfoCount < 0) {
                break;
            }
        }
    ...
    function historyInfoLength() public view returns (uint256) {
        return historyInfo.length;
    }

    function updateTotalEarnings() internal {
        uint256 _length = historyInfoLength();
        if (historyInfo.length == 0) return;

        uint256 _beforeTotalAssets = historyInfo[_length - 1].totalAssets;
        uint256 _totalAssets = totalAssets();
        if (_totalAssets >= _beforeTotalAssets) {
            totalEarnings += int256(_totalAssets - _beforeTotalAssets);
        } else {
            totalEarnings -= int256(_beforeTotalAssets - _totalAssets);
        }
    }

    function updateHistoryInfo() internal {
        int256 _ratePerSec = ratePerSec();
        uint256 _totalAssets = totalAssets();
        uint256 _totalSupply = totalSupply();
        uint256 _timestamp = block.timestamp;

        historyInfo.push(Info(_ratePerSec, _totalAssets, _totalSupply, _timestamp));

        emit UpdateHistoryInfo(_ratePerSec, _totalAssets, _totalSupply, _timestamp);
    }
    ...
    function deposit(uint256 assets, address receiver) external nonReentrant whenNotPaused returns (uint256) {
        ...
        if (totalSupply() == 0) shouldUpdateRates = true;

        emit UpdateRate(convertToAssets(RATE_PRECISION), totalAssets(), totalSupply(), block.timestamp);
        ...
        if (shouldUpdateRates) {
            rates.push(Rate(convertToAssets(RATE_PRECISION), block.timestamp));
        }
        updateHistoryInfo();
    ...
    function withdraw(uint256 assets, address receiver, address owner) external nonReentrant returns (uint256) {
        ...
        uint256 _rate = convertToAssets(RATE_PRECISION);
        ...
        if (totalSupply() == 0) rates.push(Rate(_rate, block.timestamp));
        updateHistoryInfo();
    ...
    function _harvest(address caller) internal returns (uint256) {
        ...
        address[] memory recipients = recipientInfo.recipients;
        uint256[] memory recipientWeights = recipientInfo.recipientWeights;
        uint256 harvesterWeight = recipientInfo.harvesterWeight;
        uint256 transferAssets = 0;
        uint256 length = recipients.length;
        for (uint256 i = 0; i < length; ++i) {
            transferAssets = (harvestAssets * recipientWeights[i]) / MAX_RECIPIENT_WEIGHT;
            if (transferAssets > 0) {
                asset.safeTransfer(recipients[i], transferAssets);
            }
        }
        transferAssets = (harvestAssets * harvesterWeight) / MAX_RECIPIENT_WEIGHT;
        if (transferAssets > 0) {
            asset.safeTransfer(caller, transferAssets);
        }

        uint256 _length = ratesLength();
        if (_length > 0 && (block.timestamp - rates[_length - 1].timestamp) > MIN_UPDATE_RATES_PERIOD) {
            rates.push(Rate(convertToAssets(RATE_PRECISION), block.timestamp));
        }
        updateTotalEarnings();
        updateHistoryInfo();
