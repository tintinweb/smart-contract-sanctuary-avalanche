...
contract GamePool  is ERC1155Holder, ERC721Holder,Multicall,Context{
    ...
    event batchWithdrawGameProbeEvent(address player,uint256[] ids,uint256[] amounts,uint256 timeStamp);
    event WithdrawGameProbeEvent(address player,uint256 id,uint256 amount,uint256 timeStamp);
   ...
   */
    ...
    */
    function batchWithdrawGameProbe(address player,uint256[] memory ids,uint256[] memory amounts,bytes memory signature,uint256 currentTimeStamp) external{
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,amounts[0],signature,currentTimeStamp,"ERC1155_game")==true,"You don't get the proof right");
       IERC1155(erc1155GameAddress).safeBatchTransferFrom(address(this),player,ids,amounts,"0x");  
         proofMapping[signature] = 1;
       emit batchWithdrawGameProbeEvent(player,ids,amounts,block.timestamp);
    }
    ...
    * @param: ids: the collection to retrieve the id . amounts: The number of ids to retrieve
    */
    function WithdrawGameProbe(address player,uint256  id,uint256  amount,bytes memory signature,uint256 currentTimeStamp) external{
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,amount,signature,currentTimeStamp,"ERC1155_game")==true,"You don't get the proof right");
       IERC1155(erc1155GameAddress).safeTransferFrom(address(this),player,id,amount,"0x"); 
         proofMapping[signature] = 1;
       emit WithdrawGameProbeEvent(player,id,amount,block.timestamp);
    }
    ...
    */
    function withdrawRole(address player,uint256 tokenId,bytes memory signature,uint256 currentTimeStamp) external{
      require(proofMapping[signature] == 0,"Proof has expired");
      require(Proof.checkPermissions(signer,tokenId,signature,currentTimeStamp,"ERC721_Role")==true,"You don't get the proof right");
