...
contract AuctionSystem is Ownable {
    ...
    function bid(uint256 _auctionId, uint256 _newBid) external {
        ...
        if (
            auctions[_auctionId].exist[msg.sender] &&
            auctions[_auctionId].claimed[msg.sender]
        ) {
            ...
            auctions[_auctionId].claimed[msg.sender] = false;
        } else {
            require(
                !auctions[_auctionId].exist[msg.sender],
                "You already placed a bid. Now try to update your bid"
            ...
            auctions[_auctionId].bidderAddress[
                auctions[_auctionId].bidCount
            ] = msg.sender;
    ...
    function updateBid(uint256 _auctionId, uint256 _updateValue) external {
        require(_auctionId < auctionNonce, "Invalid auction index");
        require(
            auctions[_auctionId].exist[msg.sender],
            "You already placed a bid. Now try to update your bid"
        ...
        auctions[_auctionId].lastBidPrice += (_updateValue -
    ...
    function userExist(address _user, uint256 _auctionId)
        public
        view
        returns (bool)
    {
        return auctions[_auctionId].exist[_user];
    }
    function userClaimed(address _user, uint256 _auctionId)
        public
        view
        returns (bool)
    {
        return auctions[_auctionId].claimed[_user];
    }
        ...
        return
            (auctions[_auctionId].bidCount > 0)
                ? auctions[_auctionId].lastBidPrice +
                    auctions[_auctionId].bidIncrementer
                : auctions[_auctionId].lastBidPrice;
    }
        ...
        returns (uint256)
    {
        ...
        return ((auctions[_auctionId].lastBidPrice +
            auctions[_auctionId].bidIncrementer) -
            auctions[_auctionId].biddingAmounts[
                auctions[_auctionId].bidPosition[_user]
            ]);
    ...
    function claimFunds(uint256 _auctionId) external {
        ...
        auctions[_auctionId].claimed[msg.sender] = true;
        auctions[_auctionId].biddingAmounts[
            auctions[_auctionId].bidPosition[msg.sender]
        ] = 0;
    }
