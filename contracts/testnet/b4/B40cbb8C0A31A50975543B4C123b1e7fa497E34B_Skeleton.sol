...
contract Skeleton is ERC721Enumerable, Pausable, Restricted {
    ...
    function generate(Order memory order, uint256 seed) internal returns (uint256) {
        SklRgr memory t = selectBody(order, seed);
        seed = renewSeed(seed);
    ...
    function selectRecipient(Order memory order, uint256 seed) internal view returns(address) {
        if (seed % 100 < order.stolenProbability) return order.buyer;
        address thief = magicForest.randomSkeletonOwner(seed >> 16);
        if (thief == address(0x0)) return order.buyer;
        return thief;
    }

    function randomBody(Order memory order, uint256 seed) internal pure returns(bool){
        return seed % 100 >= order.skeletonProbability;
    }

    function selectBody(Order memory order, uint256 seed) internal view returns (SklRgr memory t) {
        t.isRanger = randomBody(order, seed);
