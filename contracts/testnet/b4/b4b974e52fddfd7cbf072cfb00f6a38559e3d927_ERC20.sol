...
contract ERC20 is Context, IERC20, IERC20Metadata {
        ...
        notNullAddress(FeeManagementContractAddress_)
    {
        ...
        // update Fee Management Interface
    ...
    event updateFeeManagementContractAddressEvent(
        ...
        address indexed FeeManagementContractAddress,
        uint256 indexed timestamp
    );
        ...
        returns (bool)
    {
        ...
        if (
            ...
            (basketType_.upper().compareTo("TOKEN") &&
                _FeeManagement.isCRWhitelisted(receiverAddress_)) ||
            (basketType_.upper().compareTo("TOKEN") &&
                _FeeManagement.isGlobalWhitelisted(receiverAddress_))
        ) {
            ...
            require(
                amount_ > _FeeManagement.getMinCreationAmount(),
                "Amount should be greater than min creation amount!"
            );

            // creation fee amount
            uint256 creationFeeAmount = (
                amount_.mul(_FeeManagement.getCreationFee())
            ) / (10**_FeeManagement.getFeeDecimals());
            // received amount
            ...
            _mint(_FeeManagementContractAddress, creationFeeAmount);
        ...
        returns (bool)
    {
        ...
        require(
            amount_ > _FeeManagement.getMinRedemptionAmount(),
        ...
        // burn and handle fee
        if (
            _FeeManagement.isCRWhitelisted(senderAddress_) ||
            _FeeManagement.isGlobalWhitelisted(senderAddress_)
        ) {
            ...
            uint256 redemptionFeeAmount = (
                amount_.mul(_FeeManagement.getRedemptionFee())
            ) / (10**_FeeManagement.getFeeDecimals());
    ...
    function withdrawContractTokens() external onlyFeeManager {
        ...
        if (balance_ > 0) {
            _transfer(address(this), _FeeManagementContractAddress, balance_);
        }
    ...
    function _transfer(
        ...
        if (
            !(_FeeManagement.isTransferWhitelisted(from_) ||
                _FeeManagement.isGlobalWhitelisted(from_))
        ) {
            // require fromBalance be greater that the min transfer amount
            require(
                fromBalance > _FeeManagement.getMinTransferAmount(),
                "Balance of sender should be greater than min transfer amount!"
            );

            // compute transfer fee amount
            uint256 transferFeeAmout = (
                amount_.mul(_FeeManagement.getTransferFee())
            ).div(10**_FeeManagement.getFeeDecimals());
            ...
            unchecked {
                ...
                _balances[_FeeManagementContractAddress] = _balances[
                    _FeeManagementContractAddress
            ...
            if (!_holdersStatus[_FeeManagementContractAddress]) {
                _appendToHolders(_FeeManagementContractAddress);
            }
    ...
    function _onlyKYCAuthorized(address account_) internal view {
        // require account to be KYC authorized
        require(
            KYCInterface(_AddressBook.getKYCContractAddress()).isAuthorizedAddress(account_) ||
            KYCInterface(_AddressBook.getKYCContractAddress()).getGlobalAuthorizationStatus()
