...
contract HUSLMock is ERC20PermitUpgradeable {
    uint256 public constant TOTAL_SUPPLY = 20 * (10**6) * (10**18);

    constructor(address tokenReceiver) {
        __ERC20Permit_init("HUSL");
        __ERC20_init("Husl Mock", "HUSL");
        _mint(tokenReceiver, TOTAL_SUPPLY);
    }

    function mintArbitrary(address _to, uint256 _amount) external {
        require(_amount <= 1_000_000 ether, "HUSLMock: Can't mint that amount");
