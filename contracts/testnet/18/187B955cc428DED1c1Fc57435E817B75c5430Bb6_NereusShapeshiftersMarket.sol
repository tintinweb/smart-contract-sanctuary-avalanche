...
contract NereusShapeshiftersMarket is
 ...
 INereusShapeshiftersMarket {
    ...
    mapping(uint256 => mapping(address => uint256)) private _salesWaveTokenPrices;
    ...
    modifier isSalesAllowed(uint256 amount) {
        ...
        require(
            _saleWaves[_currentSaleWave].totalWaveAllocation >= _saleWaves[_currentSaleWave].wavePurchased + amount,
    ...
    * Returns current sales wave information
    ...
    /**
    * Returns current sale wave price for specified token
     */
    function getCurrentSaleWaveTokenPrice(address token) external view returns(uint256) {
        return _salesWaveTokenPrices[_currentSaleWave][token];
    }

    /**
    * Allows to insert wave prices into sales wave
     */
    function addSalesWaveTokenPrices(uint256 salesWaveId, SaleWaveTokenPrice[] calldata wavePrices) internal onlyOwner {
    ...
    * Allows to add new sale wave
     */
    function addSaleWave(SaleWaveTokenPrice[] calldata wavePrices, uint256 claimAllowance, uint256 waveAllocation) external onlyOwner returns (uint256) {
        require(claimAllowance > 0, "NereusShapeshiftersMarket: claim allowance should be more than 0");
        require(waveAllocation > 0, "NereusShapeshiftersMarket: total wave allocation should be more than 0");

        uint256 saleWaveId = _saleWavesCounter;
    
        addSalesWaveTokenPrices(saleWaveId, wavePrices);
    
        _saleWaves[saleWaveId] = SaleWave(false, "", claimAllowance, waveAllocation, 0, true);
        _saleWavesCounter++;
    ...
    * Allows to add new sale wave
    ...
    function addWhitelistableSaleWave(
        ...
        _saleWaves[saleWaveId] = SaleWave(true, merkleRoot, claimAllowance, waveAllocation, 0, true);
        _saleWavesCounter++;
    ...
    * Allows to update whitelistable wave whitelist
     */
    function updateWaveWhitelist(uint256 saleWaveId, bytes32 merkleRoot) external {
        require(_saleWaves[saleWaveId].isWhitelistable, "NereusShapeshiftersMarket: selected wave is not whitelistable");
    ...
    * Gets token at specified index
    ...
    /**
    * Returns amount for address NFTs to purchase
     */
    function getUserRemainingWaveAllowance(address userWallet) external view returns (uint256) {
        return _saleWavesClaimed[_currentSaleWave][userWallet];
    }

    /**
    * Is user whitelisted in current sales wave
     */
    function isUserInWaveWhitelist(address userWallet, bytes32[] calldata _merkleProof) external view returns (bool) {
        if(!_saleWaves[_currentSaleWave].isWhitelistable) {
            return true;
        }

        bytes32 leaf = keccak256(abi.encodePacked(userWallet));
        return MerkleProof.verify(_merkleProof, _saleWaves[_currentSaleWave].merkleRoot, leaf);
    }
