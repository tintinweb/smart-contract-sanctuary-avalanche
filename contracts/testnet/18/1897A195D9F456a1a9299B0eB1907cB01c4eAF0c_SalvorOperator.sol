...
contract SalvorOperator is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    // TODO rarityScore 0 ise işlemler hata vermeli
    struct Salvor {
        uint32 level;
        uint32 rarityScore;
    }
    ...
    */
    ...
    // level -> rarityScore -> rarityLevel -> rate
    mapping(uint32 => mapping(uint32 => mapping(uint32 => uint96))) miniRates;

    // rarityScore -> level -> salvorPower
    mapping(uint32 => mapping(uint32 => uint256)) salvorPowers;
    // rarityScore -> level -> price
    mapping(uint32 => mapping(uint32 => uint256)) levelPrices;
    ...
    event StakeSalvor(uint256 indexed tokenId, uint256 indexed requestId, bool isRebreed);
    event BreedMini(uint256 indexed tokenId, uint256 indexed miniTokenId, uint256 rarityLevel, uint96 rate, bool isMinted);
    ...
    * @dev pause contract, restricting certain operations
    ...
    function setVeARTCommissionPercentage(uint96 _companyWalletPercentage) external onlyOwner {
        companyWalletPercentage = _companyWalletPercentage;
        emit SetCompanyWalletPercentage(_companyWalletPercentage);
    }

    function setDropRates(uint32 _level, uint32 rarityScore, uint32[] calldata rarityLevels, uint96[] calldata rates) external onlyOwner {
        require(rates.length == 3, "Invalid input length");
        uint96 total;
        for (uint256 i=0; i<3; i++) {
            total += rates[i];
            miniRates[_level][rarityScore][rarityLevels[i]] = rates[i];
        }
        require(total <= 100, "Invalid drop rate total");
    }
    ...
    function setVeArtContract(address _veArtContract) external onlyOwner {
        veArtAddress = _veArtContract;
    }
    ...
    function setSalvorAddress(address _salvorAddress) external onlyOwner {
        salvorAddress = _salvorAddress;
    }
    ...
    /**
    * @dev Returns the Salvor struct for the given token ID.
    * @param _tokenId ID of the token to get Salvor information for.
    * @return Salvor struct containing level, knowledge, and rarityLevel attributes.
    */
    ...
    function stake(uint256[] calldata _tokenIds) nonReentrant external payable whenNotPaused {
        uint32 len = uint32(_tokenIds.length);
        require(len <= 100, "Too many tokens to breed at once");
        ...
        for (uint32 i = 0; i<len; i++) {
            ...
            emit StakeSalvor(_tokenId, requestId, false);
    ...
    function breedMini(uint256[] calldata _tokenIds) nonReentrant whenNotPaused external {
        ...
        for (uint256 i; i<len; i++) {
            ...
            if (rarityLevel > 0) {
                ...
                emit BreedMini(_tokenId, generatedMiniTokenId, rarityLevel, randomWord, true);
            } else {
                emit BreedMini(_tokenId, 0, 0, randomWord, false);
            }
    ...
    function rebreed(uint256[] calldata _tokenIds) nonReentrant external payable whenNotPaused {
        uint256 len = _tokenIds.length;
        uint256 counter;
        uint256 requestId = ++requestIds;
        emit RndRequest(requestId, len);
        for (uint256 i; i<len; i++) {
            ...
            emit StakeSalvor(_tokenId, requestId, true);
            breeds[_tokenId].startedAt = block.timestamp;
            breeds[_tokenId].endedAt = block.timestamp + minBreedingDuration;
            breeds[_tokenId].requestId = requestId;
    ...
    function upgradeSalvor(uint256 _tokenId, uint32 amount) nonReentrant external payable whenNotPaused {
    ...
    function callbackRandomWord(uint256 _requestId, uint256[] calldata _randomWords) external {
        ...
        for (uint256 i; i<_randomWords.length; i++) {
            vrfRequests[_requestId][i].randomWord = _randomWords[i];
