...
contract AuctionMarketplace is Initializable, ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct Auction {
        ...
        uint128 highestBid; // keep the highest bid for every successful make bid action
        address highestBidder; // keep the highest bidder for every successful make bid action
    ...
    * it will be extended by defaultAuctionBidPeriod minutes.
    */
    ...
    */
    ...
    // events
    event AuctionSettled(address indexed collection, uint256 indexed tokenId, address indexed highestBidder, uint256 highestBid, bool isBuyNow);
    ...
    */
    modifier minPriceDoesNotExceedLimit(uint128 _buyNowPrice, uint128 _minPrice) {
        require(_buyNowPrice == 0 ||  (_buyNowPrice * (10000 + defaultBidIncreasePercentage) / 10000) >=_minPrice, "buyNowPrice must be greater than or equal to %defaultBidIncreasePercentage percent more than minimumPrice");
        _;
    }
