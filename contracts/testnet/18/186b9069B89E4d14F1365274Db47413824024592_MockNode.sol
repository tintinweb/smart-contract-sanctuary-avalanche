...
contract MockNode is CreateNFTNode {
    uint256 public fakeBlockTimeStamp;

    constructor(string memory _name,
        string memory _symbol,
        string memory _initBaseURI, string memory _initContractURI, uint256[] memory _initRewardRates, address _initDappTokenAddress, address _initNftTokenFarm, address _priceFeed, uint256 dollarFee, uint256[] memory _initPriceNodes, uint256 _initMintRate) public CreateNFTNode(_name,
        _symbol,
        _initBaseURI, _initContractURI, _initRewardRates, _initDappTokenAddress, _initNftTokenFarm, _priceFeed, dollarFee, _initPriceNodes, _initMintRate){}
    ...
    function getTime() public virtual
...
contract CreateNFTNode is ERC721Enumerable, Ownable {
    ...
    address public nftTokenFarmAddress;
    ...
    constructor (string memory _name,
        string memory _symbol,
        string memory _initBaseURI, string memory _initContractURI, uint256[] memory _initRewardRates, address _initDappTokenAddress, address _initNftTokenFarm, address _priceFeed, uint256 dollarFee, uint256[] memory _initPriceNodes, uint256 _initMintRate) public ERC721 (_name, _symbol){
        setBaseURI(_initBaseURI);
        setContractURI(_initContractURI);
        setRewardRates(_initRewardRates);
        setDappTokenAddress(_initDappTokenAddress);
        setNftTokenFarmAddress(_initNftTokenFarm);
        setPriceFeed(_priceFeed);
        setFee(dollarFee);
        setPriceNodes(_initPriceNodes);
        setMintRate(_initMintRate);
        locked = true;
        //setLocked(_initLocked);
    }
    ...
    function getNodeToken(uint256 tokenId) public view returns (uint256, uint256, uint256, bool)
    {
        return (nodeToken[tokenId].rewardTimestamp, nodeToken[tokenId].feeTimestamp, nodeToken[tokenId].tierNumber, nodeToken[tokenId].monthlyFeePaid);
    ...
    function setDappTokenAddress(address dappTokenAddress) public onlyOwner
    {
        dappToken = IERC20(dappTokenAddress);
    }

    function setNftTokenFarmAddress(address _nftTokenFarm) public onlyOwner
    {
        nftTokenFarmAddress = _nftTokenFarm;
    }
    ...
    function mint(uint256 tokenId) public virtual payable {
        ...
        //TODO: checkTime
        (bool doesExist,,) = TokenFarm(nftTokenFarmAddress).getStakedToken(msg.sender, tokenId);
        if (doesExist == true) {
            if (nftMapping[msg.sender][tokenId].firstMintOver == false)
            {
                numberOfNodes = 1;
            }
            else {
            ...
            {
                _safeMint(msg.sender, counter);
                nodeToken[counter] = Node(getTime(), getTime(), tierNumber, false);
                counter = counter + 1;
            }
    ...
    {require(!paused, "Mint paused!");
        ...
        if (tier == 1)
        {
            require(IERC20(dappToken).balanceOf(msg.sender) >= priceNodes[0], "Not enough money!");
            IERC20(dappToken).transferFrom(msg.sender, owner(), priceNodes[0]);
        }
        else if (tier == 2)
        {
            require(IERC20(dappToken).balanceOf(msg.sender) >= priceNodes[1], "Not enough money!");
            IERC20(dappToken).transferFrom(msg.sender, owner(), priceNodes[1]);
        }
        else if (tier == 3)
        {
            require(IERC20(dappToken).balanceOf(msg.sender) >= priceNodes[2], "Not enough money!");
            IERC20(dappToken).transferFrom(msg.sender, owner(), priceNodes[2]);
        }
        _safeMint(msg.sender, counter);
        nodeToken[counter] = Node(getTime(), getTime(), tier, false);
        counter = counter + 1;
    ...
    {
        require(nodeToken[_tokenId].monthlyFeePaid == false, "You already paid!");
    ...
    //TODO:do it
    function checkIfMonthOver(uint256 _tokenId) private returns (bool)
    {
    ...
    //TODO: frontend calculate
    function calculateClaimFee(uint256 _tokenId) public view returns (uint256)
    {
    ...
    {
        //TODO: paid the fee
        //require(nodeToken[tokenId].monthlyFeePaid == true && checkIfMonthOver(_tokenId) == false, "Fee not paid");
        require(balanceOf(msg.sender) > 0, "Needs to have some nodes");
        uint256 rewardRate;
        if (nodeToken[nodeTokenId].tierNumber == 1)
        {
            rewardRate = rewardRates[0];
        }
        else if (nodeToken[nodeTokenId].tierNumber == 2)
        {
        ...
        uint256 wholeReward = calcTimeElapsedReward(nodeTokenId) * rewardRate * calculateClaimFee(nodeTokenId) / 100;
        nodeToken[nodeTokenId].rewardTimestamp = getTime();
        require(wholeReward > 0);
        dappToken.transfer(msg.sender, wholeReward);
    }


    function calcTimeElapsedReward(uint256 _tokenId) public view returns (uint256)
    {
        //uint256 elapsedTime = block.timestamp - nodeToken[_tokenId].rewardTimestamp;
    ...
    {
        uint256 elapsedTime;
        (,uint256 startTimestamp, uint256 nrOfMonths) = TokenFarm(nftTokenFarmAddress).getStakedToken(msg.sender, _tokenId);
        if (nftMapping[_staker][_tokenId].lastUpdate == 0)
        {
        ...
        if (elapsedTime > startTimestamp + nrOfMonths * 2628000)
        {
            elapsedTime = startTimestamp + nrOfMonths * 2628000;
        }
...
contract TokenFarm is Ownable {
    ...
    mapping(address => mapping(uint256 => StakedToken)) public stakedToken;
    ...
    constructor(address _initNftAddress) public {
        setNftAddress(_initNftAddress);
    }
    ...
    {
        nft_address = _newNftAddress;

    }

    function getStakedToken(address _staker, uint256 _tokenId) public view returns (bool, uint256, uint256) {
        return (stakedToken[_staker][_tokenId].doesExist, stakedToken[_staker][_tokenId].startTimestamp, stakedToken[_staker][_tokenId].nrOfMonths);
    }


    function stakeTokens(uint256 _tokenId, address _token, uint256 nrOfMonths) public
    {
        require(stakedToken[msg.sender][_tokenId].doesExist == false, "Already exists!");
        require(nrOfMonths == 1 || nrOfMonths == 3 || nrOfMonths == 6 || nrOfMonths == 12, "You can't stake other duration");
        ...
        StakedToken memory token = StakedToken(getTime(), nrOfMonths, true);
        //staker.tokens.push(token);
        stakedToken[msg.sender][_tokenId] = token;
    }

    //unstake tokens
    function unstakeTokens(uint256 _tokenId, address _token) public {
        require(stakedToken[msg.sender][_tokenId].doesExist == true, "You didn't stake that token!");
        require(checkIfUnstakePossible(msg.sender, _tokenId) == true, "You can't unstake yet!");
        CreateNFT(_token).transferFrom(address(this), msg.sender, _tokenId);
        delete (stakedToken[msg.sender][_tokenId]);
        //TODO: delete other
    }


    function checkIfUnstakePossible(address _staker, uint256 _tokenId) public view returns (bool)
    {
        if (getTime() >= stakedToken[_staker][_tokenId].startTimestamp + stakedToken[_staker][_tokenId].nrOfMonths * 2628000)
        {
            return true;
        }
