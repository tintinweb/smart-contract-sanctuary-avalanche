...
contract GoldenToken is Ownable, ERC721 {
    ...
    uint256 public publicTicketSaleStartDate;
    ...
    constructor(string memory name_, string memory symbol_, address _apaAddress, address _apaRarityAddress)
        ERC721(name_, symbol_)
    {
        APA_CONTRACT = ERC721(_apaAddress);
        publicTicketSaleStartDate = 1670707358;
    ...
    modifier burnAllowed {
      require(block.timestamp < publicTicketSaleStartDate);
      _;
    }
    ...
    function setPublicTicketSaleTime(uint256 _startTime) external onlyOwner {
        publicTicketSaleStartDate = _startTime;
    }
    ...
    function getGoldenTokenWithApaOnChainRarity(uint256 tokenId) external burnAllowed{
        ...
        emit GoldenTokenClaimed(tokenId,rarity,nextTokenId - 1);
    }

    function getGoldenTokenWithAvax() external payable{
        if (block.timestamp < publicTicketSaleStartDate) {
            revert PublicSaleHasntStarted();
        }

        if (msg.value < publicTicketSaleCost) {
            revert CostsNotCovered();
        }

        _mint(msg.sender, nextTokenId);
        tokenRarity[nextTokenId++] = 5; // joker

        if (nextTokenId >= COLLECTION_SIZE) {
            revert TicketsMintedOut();
        }
        emit GoldenTokenClaimed(0,5,nextTokenId - 1);
    }


    function getGoldenTokenWithApaOnChainRarityMultiple(uint256[] calldata tokenIds)
