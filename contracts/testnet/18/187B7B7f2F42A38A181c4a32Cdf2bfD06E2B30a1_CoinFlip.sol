...
contract CoinFlip is AccessControlEnumerable, GameManager, VRFManager {
    ...
    mapping(address => uint256) public playerBetBalance;
    ...
    function startGame(Outcome _guess) external payable {
        ...
        playerBetBalance[msg.sender] += msg.value;
            ...
            if (
                ...
                require(
                    payable(gameData[_gameId].player).send(
                        playerBetBalance[gameData[_gameId].player] * 2
                    )
                );

                playerBetBalance[gameData[_gameId].player] = 0;

                houseHold -=
                    playerBetBalance[gameData[_gameId].player] *
                    RESERVE_RATIO;

                houseBalance += RESERVE_RATIO;
...
contract GameManager {
    ...
    function removePlayerFromGame(address _player) internal virtual {
        // return funds to house and player?
        // release all funds to house?
