...
contract DiamondDao is ERC721Enumerable, Ownable {
  ...
  uint256 public costPS = 1 ether;
  uint256 public MaxCollection = 9999;

  constructor(string memory _initBaseURI,string memory _initNotRevealedUri)
    ERC721("Diamond Dao", "DD") { setBaseURI(_initBaseURI); setNotRevealedURI(_initNotRevealedUri); }
...
function setCostePS(uint256 _newCostPS) public onlyOwner {
  ...
  function withdraw() public payable onlyOwner {
    (bool dd, ) = payable(owner()).call{value: address(this).balance}("");
    require(dd);
  }
}