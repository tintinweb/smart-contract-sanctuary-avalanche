...
contract NeFiNFT is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    function returnNums(uint256 _id) internal returns(uint256,uint256,string memory){
    	uint256 _k = _id-1;
    	return (NFTids[_k],_k,uri_ls[_k]);
    }
    ...
    {	
    	...
    	for(uint i=0;i<_ids.length;i++){
    		(uint256 _id,uint256 _k,string memory uri) = returnNums(_ids[i]);
    		recTotes(_id,_amounts[i]);
    		_setURI(uri);
    		_mint(_to,_id,_amounts[i],data);
    	}
