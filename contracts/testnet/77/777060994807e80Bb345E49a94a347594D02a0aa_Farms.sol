...
contract Farms is ReentrancyGuard, Ownable, Pausable {
  ...
  struct Position {
    ...
    // Block timestamp which this position was last updated at
    uint256 lastUpdatedAt;
  }
  ...
  * @param _lastUpdatedAt  Last updated timestamp of position
  * @return currentRewardPerStakedToken Current reward per staked token
  */
  function currentRewardPerStakedToken(uint256 _id, uint256 _lastUpdatedAt) private view returns (uint256) {
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view returns (uint256) {
    ...
    if (position.stakedAmount <= 0 || currentRewardPerStakedToken(_id, position.lastUpdatedAt) <= 0) return 0;
  ...
  */
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0) {
      ...
      position.rewardsDebt = position.stakedAmount * currentRewardPerStakedToken(_id, position.lastUpdatedAt)
