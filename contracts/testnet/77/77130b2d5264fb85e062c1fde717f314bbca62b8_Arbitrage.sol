...
contract Arbitrage{
    ...
    function useCalcul(uint256 minSwapIn, uint256 maxSwapIn, uint256 stepSwapIn, uint256 minSwapOut, uint256 maxSwapOut, uint256 stepSwapout) external {
        InfoResult memory infor = calcul(minSwapIn, maxSwapIn, stepSwapIn, minSwapOut, maxSwapOut, stepSwapout);
        if(infor.maxSwapAvalancheUSDTIn > 10){
            testBool = !testBool;
        }
    }

    function calcul(uint256 minSwapIn, uint256 maxSwapIn, uint256 stepSwapIn, uint256 minSwapOut, uint256 maxSwapOut, uint256 stepSwapout) public view returns (InfoResult memory){
