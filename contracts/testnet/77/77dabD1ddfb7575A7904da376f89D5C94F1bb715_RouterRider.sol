    ...
    UUPSUpgradeable
{
    ...
    */
    ...
    function RideCrossChain(
        ...
        uint8 ackType;
        // string memory asmAdrr;
        assembly {
            ackType := mload(add(_requestMetadata, 49))
            // asmAdrr := mload(add(_requestMetadata, 49))
        }

        require(
            ackType == 3,
            //TODO: && keccak256(bytes(asmAdrr)) != keccak256(abi.encodePacked(""))
            "invalid asm or ack"
        );
    ...
    function iReceive(
        ...
        address receiver = convertStringToAddress(receiverStr);
