    ...
    ERC1155BurnableUpgradeable
{
    ...
    function orderPlace(
        ...
        if (
            ...
            require(
                _userSubscrption[msg.sender].subEndPeriod.add(
    ...
    function saleWithToken(
       ...
       require(
            _userSubscrption[from].subEndPeriod.add(
                unsubscripetionLimit.mul(1 days)
            ) <
                block.timestamp ||
                from == owner(),
        ...
        if (_unblockBalances[ids[0]][msg.sender] == 0) {
            ...
            _userSubscrption[msg.sender] = _subUser;
        }
        else{
            _userSubscrption[msg.sender].subStartPeriod = block.timestamp;
            _userSubscrption[msg.sender].subEndPeriod = block.timestamp.add(_subscribeNFT[ids[0]].subPeriod.mul(1 days));
        }
        _unblockBalances[ids[0]][from] = _unblockBalances[ids[0]][from].sub(
            ids[2]
        );
        delete _userSubscrption[from];
    }
        ...
        payable
    {
        ...
        if (_unblockBalances[ids[0]][msg.sender] == 0) {
            ...
            _userSubscrption[msg.sender].subStartPeriod = 0;
            _userSubscrption[msg.sender].subEndPeriod = block.timestamp;
        }
        _unblockBalances[ids[0]][from] = _unblockBalances[ids[0]][from].sub(
            ids[2]
        );
    ...
    function acceptBId(
        ...
        require(
            ids[2] == 1 &&(_unblockBalances[ids[0]][bidaddr] == 0 ||
                    _userSubscrption[bidaddr].subEndPeriod.add(
                        unsubscripetionLimit.mul(1 days)
                    ) <
                    block.timestamp),
            "Already You have this NFT or Only Purchase One NFT"
        ...
        if (_unblockBalances[ids[0]][bidaddr] == 0) {
            ...
            _subUser.subPrice = _subscribeNFT[ids[0]].subPrice;
            _subUser.subEndPeriod = block.timestamp;
            _userSubscrption[bidaddr] = _subUser;
        }
        else{
            _userSubscrption[bidaddr].subStartPeriod = 0;
            _userSubscrption[bidaddr].subEndPeriod = block.timestamp;
        }
        _unblockBalances[ids[0]][msg.sender] = _unblockBalances[ids[0]][msg.sender].sub(
            ids[2]
        );
        delete _userSubscrption[msg.sender];
    }
    ...
    function nftSubscription(uint256 tokenId) public payable {
        ...
        _userSubscrption[msg.sender].tokenId = tokenId;
        _userSubscrption[msg.sender].subStartPeriod = block.timestamp;
        _userSubscrption[msg.sender].subEndPeriod = block.timestamp.add(
