...
contract NodeManagerV2 is Initializable {
  ...
  function initialize(address[] memory addresses) public initializer {
    ...
    maxMonthValue = 3;
    oldNodeManager = NODERewardManagement(0x05c88F67fa0711b3a76ada2B6f0A2D3a54Fc775c);
  ...
  function updateTier(
    ...
    require(keccak256(abi.encodePacked((tierName))) == keccak256(abi.encodePacked((name))), "New tier's name is incorrect");
    require(price > 0, "Tier's price has to be positive");
    require(rewardsPerTime > 0, "Tier's rewards has to be positive");
    ...
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == msg.sender) {
          ...
          amount += ((((uint256(block.timestamp - node.claimedTime) * tier.rewardsPerTime) * multiplier) / 1 ether) / tier.claimInterval) + node.leftover;
        }
      }
    }
    return amount;
    ...
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == msg.sender && tierIndexFrom - 1 == node.tierIndex) {
          ...
          uint256 multiplier = getBoostRate(msg.sender, node.claimedTime, block.timestamp);
          uint256 claimed = ((uint256(block.timestamp - node.claimedTime) * tierFrom.rewardsPerTime) / tierFrom.claimInterval) + node.leftover;
    ...
    for (uint32 i = 0; i < nodeIndiceFrom.length; i++) {
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == from && tierIndex - 1 == node.tierIndex) {
          ...
          uint256 claimed = ((uint256(block.timestamp - node.claimedTime) * tier.rewardsPerTime) / tier.claimInterval) + node.leftover;
          claimableAmount += (claimed * multiplier) / (10**18);
          node.claimedTime = uint32(block.timestamp);
          countTransfer++;
      ...
      for (uint32 i = 0; i < nodeIndice.length; i++) {
        ...
        if (nodeIndex > 0) {
          ...
          if (node.owner == msg.sender) {
            ...
            fee += getAmountToPay((tier.price * 3000) / 10000);
          }
        }
      }
    } else {
  ...
  function migrateRewardsFromOldVersion() public {
    ...
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      ...
      if (nodeIndex > 0) {
        Node storage node = nodesTotal[nodeIndex - 1];
        if (node.owner == msg.sender) {
          node.leftover = oldRewardsOfUser[msg.sender];
          i = uint32(nodeIndice.length);
        }
      }
    }
  ...
  function getAmountToPay(uint256 _amount) public view returns (uint256) {
