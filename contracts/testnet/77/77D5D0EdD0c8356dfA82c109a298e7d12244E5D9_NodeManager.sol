...
contract NodeManager is Initializable {
  ...
  // function getNodeById(uint32 id) public returns (Node memory) {
  //   require(!availableFunctions["getNodeById"], "Disabled");
  //   for (uint32 i = 0; i < nodesOfUser[msg.sender].length; i++) {
  //     if (nodesOfUser[msg.sender][i].id == id) {
  //       return nodesTotal[i];
  //     }
  //   }
  // }
  ...
  function sellNode(uint32 indice) public {
    ...
    if (node.owner == msg.sender) {
      ...
      countOfTier[tier.name]--;
      countTotal--;
      IERC20Upgradeable(tokenAddress).transferFrom(address(rewardsPoolAddress), address(msg.sender), tier.price.div(50).mul(15));
    }
