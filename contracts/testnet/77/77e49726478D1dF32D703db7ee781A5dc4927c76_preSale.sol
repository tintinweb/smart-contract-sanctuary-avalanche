...
contract preSale is Auth, Referral
{
    ...
    function purchaseQETHW(uint256 _qethw, uint256 _option, string memory _name)public
    {
        if(_option == 0)
        {
            ...
            ethsToken.transfer(msg.sender, _qethw);
            option1[msg.sender] += _qethw;
        }
        else
        {
            // user has not purchased node...
            if(!isBluewhale[msg.sender])
            {
                // if bluewhale is not purchased...
                uint256 _tokens = (8 * (10 ** 17)) * (800 * (10 ** 18)) / (10 ** 18);
                IERC20(DAI).transferFrom(msg.sender, address(this), _tokens);
                iReferral(QETHW).burn(_tokens);
                // purchasing BlueWhale for msg.sender...
                presaleNode[msg.sender] = _name;
                isBluewhale[msg.sender] = true;
                // if user has 1000 tokens and select option2...
                uint256 _remaining = _qethw - (800 * (10 ** 18));
                if(_remaining > 0)
                {
                    require(_remaining + option2[msg.sender] <= 400 * (10 ** 18));
                    IERC20(DAI).transferFrom(msg.sender, address(this), _remaining);
                    ethsToken.transfer(msg.sender, _qethw);
                    option2[msg.sender] += _qethw;
                }
            }
            else
            {
                // if bluewhale is already purchased...
                require((option2[msg.sender] + _qethw) <= 400 * (10 ** 18), "Max Limit Reached!");
