...
contract EVICTreasury is ContractGuard {
        ...
        if (epoch > 0) {
            ...
            if (enabledRebase) {
                ...
                if (rebaseStarted && previousEpochMainPrice < mainTokenPriceOne) {
                    ...
                    rebaseStarted = false;
                    // twap <= 0.8 WETH.e => rebase
                    // 10 consecutive epoch has twap < 1 WETH.e => rebase
    ...
    function setWithdrawLockupEpoch(uint256 _value) external onlyOperator {
        IBoardroom(boardroom).setWithdrawLockupEpoch(_value);
    }

    function setRewardLockupEpoch(uint256 _value) external onlyOperator {
        IBoardroom(boardroom).setRewardLockupEpoch(_value);
    }
