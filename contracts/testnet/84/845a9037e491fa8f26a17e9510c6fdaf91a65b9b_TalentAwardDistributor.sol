...
// File: contracts/talentAward.sol
...
interface ITalentStaking{
    ...
    function stakerVoteCount(address _staker) external view returns (uint256);
}

contract TalentAwardDistributor is ITalentAwardDistributor {
  ...
  constructor(
    ...
    function distributeTalentAward() internal {
        // Get all stakers
        address[] memory stakers = talentStaking.getstakers();

        require(stakers.length > 0,"No stakers found");
        // Distribute award
        for (uint256 i = 0; i < stakers.length; i++) {
            address staker = stakers[i];
            uint256 stakerAward = talentStaking.stakerVoteCount(staker) * awardPerVote; 
            if (stakerAward > 0) {
                // Send award to staker
                talentScout.transferFrom(
                    address(this),
                    staker,
                    stakerAward
                );
    ...
    function depositRewardAmount(uint256 _amount) external override onlyScout {
        ...
        totalAward = totalAward.add(amount);
        uint256 totalVotes = talentStaking.totalVotes();
        awardPerVote = totalAward.div(totalVotes);
