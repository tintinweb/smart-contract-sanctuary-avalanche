...
contract TraderJoeBaseProxy is AccessControlEnumerable, Pausable {
    ...
    /// @notice The amount of earned reward in network native coins (AVAX)
    /// @return balance
    function getContractAvaxBalance() public view returns (uint256) {
        return address(this).balance;
    }

    /// @notice The amount of earned reward given ERC20 coins
    /// @param _tokenAddress addresss of ERC20 token
    /// @return balance
    function getContractTokenBalance(IERC20 _tokenAddress) public view returns (uint256) {
        // IERC20Token = _tokenAddress;
        return IERC20(_tokenAddress).balanceOf(address(this));

        // return IERC20Token.balanceOf(_msgSender());
    }
    ...
    function getTokenProfit(IERC20 _tokenAddress) public view returns (uint256) {
        return IERC20(_tokenAddress).balanceOf(address(this));
    }
    ...
    function withdrowTokenFee(IERC20 _tokenAddress, address _to) public {
        require(hasRole(OWNER_ROLE, _msgSender()), "Must have admin role to set fee");
    ...
    function swapTokensForExactTokens(
        ...
        IERC20(_tokenPath[0]).approve(address(traderJoe), maxApproval);
        traderJoe.swapTokensForExactTokens(_amountOut, _amountInMax, _pairBinSteps, _tokenPath, _to, _deadline);
    }
    ...
    function swapTokensForExactAVAX(
        ...
        require(!paused(), "Token swap is not allowed while the contract is on pause");
        IERC20(token).safeTransferFrom(msg.sender, address(this), _amountInMax);
        IERC20[] memory path = new IERC20[](2);
        path[0] = token;
        path[1] = wavax;
        /// change to token path from function params. Because may be cases where path includes 3 or more params
        IERC20(token).approve(address(traderJoe), maxApproval);
        // add fee functionality
        traderJoe.swapTokensForExactAVAX(_amountAVAXOut, _amountInMax, _pairBinSteps, path, payable(_to), _deadline);
    }
    ...
    function swapExactAVAXForTokens(
        ...
        traderJoe.swapExactAVAXForTokens{value: sumMinusFee}(amountOutMin, _pairBinSteps, path, msg.sender, block.timestamp);
    }
