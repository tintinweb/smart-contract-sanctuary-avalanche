...
contract Bifrost is Ownable, PaymentSplitter {
    ...
    IJoeRouter02 public traderJoeV2Router =IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    address public distributionPool =0x843B7C183165ab513d7C5b443d8eD7e5169De0e4;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    IMasterOfCoin masterOfCoin =IMasterOfCoin(0x48cC0E6C5df9242aF624DE342b05182af0c49153);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner{
        require(pair != traderJoeV2Pair,"The TraderJoe pair cannot be removed from automatedMarketMakerPairs");
    ...
    function createNodeWithTokens(string memory name, string memory tierName) external {
        require(bytes(name).length > 2 && bytes(name).length < 32,"NODE CREATION: Name size invalid");
        ...
        require(thor.balanceOf(sender) >= nodePrice,"NODE CREATION: Balance too low for creation.");
    ...
    function compoundInto(string memory tierName, string memory nodeName)external{
        ...
        for (uint256 i = 0; i < allTiers.length; i++) {
            ...
            NODERewardManagement nodeRewardManager = tier.manager;
            uint256 nodeNumber = nodeRewardManager._getNodeNumberOf(sender);
    ...
    function cashoutAll(string memory tierName) public verifyFeeStatus(tierName){
        ...
        NodeTier memory tier = tiers[tierName];
        NODERewardManagement nodeRewardManager = tier.manager;
        cashoutFee = _calculateAverageTaxFee(sender, tier);
    ...
    function claimReward(uint256 createdAt,string memory nodeName,string memory tierName) public {
        ...
        uint256 deltaLastClaimTime = block.timestamp - nodeRewardManager._getNodeLastClaimTime(sender, createdAt);
    ...
    function _calculateAverageTaxFee(address sender, NodeTier memory tier) internal view returns (uint256 Fee){
        NODERewardManagement nodeRewardManager = tier.manager;
        uint256[] memory lastClaimTimes = nodeRewardManager._getNodeLastClaimTime(sender);
        uint256 currentTime = block.timestamp;
        bool[] memory validate = new bool[](5);
        for (uint256 i = 0 ; i < lastClaimTimes.length ; i++ ){
            for (uint256 j = 0; j < cashoutFeeDue.length; j++) {
                if(currentTime - lastClaimTimes[i] <= cashoutFeeDue[j]){
                    validate[j] = true; 
                    break;
                }else if(currentTime - lastClaimTimes[i] > cashoutFeeDue[cashoutFeeDue.length - 1]){
                    validate[4] = true; 
                    break;
                }
            }
        }
        Fee = nodeRewardManager._checkValidateAndCalculateFee(validate, cashoutFeeTax, tier.cashOutFee);        
    }
}
