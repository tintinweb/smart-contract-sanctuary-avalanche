...
contract GenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accGenesisPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.057870370 ether; // 10000 bshare / (2 days * 24h * 60min * 60s)
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accGenesisPerShare = pool.accGenesisPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accGenesisPerShare = accGenesisPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accGenesisPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accGenesisPerShare = pool.accGenesisPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accGenesisPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeGenesisTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accGenesisPerShare).div(1e18);
    ...
    function safeGenesisTransfer(address _to, uint256 _amount) internal {
