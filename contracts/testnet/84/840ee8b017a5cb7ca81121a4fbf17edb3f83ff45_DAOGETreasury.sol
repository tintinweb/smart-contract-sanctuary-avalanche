...
contract DAOGETreasury is Ownable {
    ...
    address public immutable DAOGE;
    ...
    address public sDAOGE;
    uint public sDAOGEQueue; // Delays change to sDAOGE address
    ...
    constructor (
        ...
        require( _DAOGE != address(0) );
        DAOGE = _DAOGE;

        isReserveToken[ _DOGE ] = true;
        reserveTokens.push( _DOGE );
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( DAOGE ).mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDAOGEERC20( DAOGE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sDAOGE ).balanceOf( msg.sender ); // Can only borrow against sDAOGE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DAOGE
        @param _amount uint
     */
    function repayDebtWithOHM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDAOGEERC20( DAOGE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, DAOGE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( DAOGE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( DAOGE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DAOGE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DAOGE decimals
            value_ = _amount.mul( 10 ** IERC20( DAOGE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
            ...
            sDAOGEQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
