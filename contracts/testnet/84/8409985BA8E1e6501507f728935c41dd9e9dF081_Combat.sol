...
contract Combat is Database {
    ...
    function start(uint256 _playerId, uint256 _tierLevel) public {
        ...
        // generate mob
        tier = mapTier[_tierLevel];
        uint256 mobId = randMod(3) + 3 * _tierLevel;
        startFight(getFighter(_playerId), getMob(mobId));
    }
    ...
    function p1AttackPhase(Fighter memory _p1, Fighter memory _p2) public {
    ...
    function p2AttackPhase(Fighter memory _p1, Fighter memory _p2) public {
    ...
    function endFight(Fighter memory _player) public {
        modifyPlayerState(_player);
        // reset elements ?
        // delete p1;
        // delete p2;
        // delete player;
    ...
    function modifyPlayerState(Fighter memory _fighter) public {
        addXp(_fighter, 50);
        _fighter.cd = block.timestamp + tier.cd;
        setFighter(_fighter);
