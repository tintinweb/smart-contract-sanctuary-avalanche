/**
 *Submitted for verification at testnet.snowtrace.io on 2023-05-28
*/
...
contract MiningToken is ERC20 {
    ...
    constructor(
        ...
        miningAmount = _miningAmount * (10**uint256(_decimals));
    ...
    function mineTokens() external {
        require(balanceOf(msg.sender) >= miningAmount, "Insufficient balance");
        require(block.timestamp >= lastMiningTime[msg.sender] + miningInterval, "You can mine once per interval");

        _mint(msg.sender, miningAmount);
        lastMiningTime[msg.sender] = block.timestamp;
        emit TokensMined(msg.sender, miningAmount);
    }
}