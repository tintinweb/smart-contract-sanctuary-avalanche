...
contract BridgesOfFate is OwnableUpgradeable {
    ...
    event ParticipateOfPlayerInGame(
        bytes32 playerId,
        uint256 globalStage,
        uint256 jumpAtStage
    );
    ...
    function _pushBackBankingGameEndRules() internal view returns (bool) {
        if (
            (pioneerPushBackGameMap[
                gameStatusInitialized[_latestGameId].stageNumber
            ] > 0) &&
            (pioneerPushBackGameMap[
                gameStatusInitialized[_latestGameId].stageNumber
            ] < _pioneerPushBackNumberProbility) &&
            (_dayDifferance(
                block.timestamp,
    ...
    function _distributionReward() internal {
        ...
        require(
            (gameStatusInitialized[_latestGameId].startAt > 0 &&
                block.timestamp >=
                gameStatusInitialized[_latestGameId].startAt),
            "Game Start After Intialized Time."
        );
        ...
        if (gameStatusInitialized[_latestGameId].isDistribution) {
            ...
            if (gameWinners.length > 0) {
                for (uint i = 0; i < gameWinners.length; i++) {
                    winnerbalances[gameWinners[i]] =
                        (((_winnerRewarsdsPercent * treasuryBalance())) / 100) /
                        (gameWinners.length);
                }
            }
        }
    ...
    function _checkSide(
        ...
        if (
            (userSide == false && stage__randomNumber < 50) ||
            (userSide == true && stage__randomNumber >= 50)
        ) {
            return true;
        } else {
            return false;
    ...
    function entryFeeSeries(
        ...
        uint256 _nftId,
        uint8 _seriesType
    ) public PushBackBankingGameEndRules {
        require(
        ...
        if (_member.userWalletAddress != address(0)) {
            ...
            require(
                _checkSide(_member.stage, _member.lastJumpSide) == false,
                "Already In Game"
            );
            require(
    ...
    function buyBackInFee(
        bytes32 playerId
    ) public GameEndRules PushBackBankingGameEndRules {
    ...
    function switchSide(
        bytes32 playerId
    ) external GameEndRules PushBackBankingGameEndRules {
        GameItem storage _member = playerItem[playerId];
    ...
    function generateRandomNumbersForGame(
        uint256 _length
    ...
    function participateInGame(
        ...
        if (_randomNumber <= 0) {
            ...
            gameRandomNumber[_currentGlobalStage] = 55;
            // ------ END -----
            ...
            _seed = (_seed - ((_seed / 100) * 100));
            pioneerPushBackGameMap[_currentGlobalStage] = 85;
            // ------ END -----
        ...
        if (
            (_currentUserStage == _gameStatus.stageNumber) &&
            (pioneerPushBackGameMap[_gameStatus.stageNumber] <
                _pioneerPushBackNumberProbility)
        ) {
    ...
    function pushBack(
        ...
        require(
            _memberfrom.stage == _gameStatus.stageNumber,
            "Player is not Pioneer."
        );
        require(
            _memberfrom.isPushBack == true,
            "Already push againest this Nft"
        );
        require(
            _memberfrom.stage > _memberto.stage,
            "Stage must be greater to traget nft."
        );
    ...
    function isGameEnded() external view returns (bool) {
        GameStatus storage _gameStatus = gameStatusInitialized[_latestGameId];
        if (
            (pioneerPushBackGameMap[_gameStatus.stageNumber] > 0) &&
            (_dayDifferance(block.timestamp, _gameStatus.startAt) >
                _gameStatus.lastUpdationDay)
        ) {
