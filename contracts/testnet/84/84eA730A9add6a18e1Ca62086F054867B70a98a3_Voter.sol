...
///    via MasterPlatypus.notifyRewardAmount(address _lpToken, uint256 _amount)
...
contract Voter is Initializable, SafeOwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address[] public lpTokens; // all LP tokens
    ...
    mapping(address => uint256) public weights; // lpToken => weight, equals to sum of votes for a LP token
    mapping(address => mapping(address => uint256)) public votes; // user address => lpToken => votes
    mapping(address => LpTokenInfo) internal infos; // lpToken => LpTokenInfo

    // bribe related storage
    mapping(address => address) public bribes; // lpToken => bribe rewarder

    event UpdateVote(address user, address lpToken, uint256 amount);
    event DistributeReward(address lpToken, uint256 amount);
    ...
    function initialize(
        ...
        require(_ptp != address(0), 'vePtp address cannot be zero');
        require(_vePtp != address(0), 'vePtp address cannot be zero');
    ...
    function _checkGaugeExist(address _lpToken) internal view {
    ...
    function pause(address _lpToken) external onlyOwner {
    ...
    function resume(address _lpToken) external onlyOwner {
    ...
    function setGauge(address _lpToken, address _gauge) external onlyOwner {
        require(_gauge != address(0));
        _checkGaugeExist(_lpToken);
    ...
    function setBribe(address _lpToken, address _bribe) external onlyOwner {
        _checkGaugeExist(_lpToken);
        ...
        returns (uint256[] memory bribeRewards)
    {
        ...
        IVe(vePtp).vote(msg.sender, voteDelta);
    }

    /// @notice Claim bribes for LP tokens
    /// @dev This function looks safe from re-entrancy attack
    function claimBribes(address[] calldata _lpTokens) external returns (uint256[] memory bribeRewards) {
    ...
    /// @dev This function looks safe from re-entrancy attack
    /// The gauge should pull tokens from this contract at notifyRewardAmount
    function distribute(address _lpToken) external {
        // PTP emitted for un-whitelist lpTokens are blackholed
        // whitelist indicates the gauge exist, _checkGaugeExist(lpToken) is skipped
        // Also, don't distribute PTP if the contract is paused
        if (infos[_lpToken].whitelist == false || paused()) return;
        ...
        if (_claimable > 0) {
            ...
            IERC20(ptp).approve(infos[_lpToken].gauge, _claimable);
    ...
    function _updateFor(address _lpToken) internal {
    ...
    function emergencyPtpWithdraw() external onlyOwner {
        ...
        IERC20(ptp).transfer(address(msg.sender), IERC20(ptp).balanceOf(address(this)));
    }
