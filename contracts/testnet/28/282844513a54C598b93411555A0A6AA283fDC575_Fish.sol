...
contract Captain is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, 
PausableUpgradeable, ERC721BurnableUpgradeable, EIP712Upgradeable  {
  ...
  mapping(address => bool) public claimed;
    ...
    string public baseURI = "ipfs://QmRAjs3kUhSGd3YySAnwoakZDCUHuTiubu2b2F4j4X1rW4/"; // IPFS URI WILL BE SET AFTER ALL TOKENS SOLD OUT
    ...
    mapping(address =>  mapping(uint256 => CaptainMetadata)) public CaptainMetadaByAddresses;
    mapping(address =>  CaptainMetadata[] ) public CaptainMetadaByAddresses2;
...
/*
    function _baseURI() internal pure override returns (string memory) {
        return "https://arweave.net/klgsynaLg1GfBONET2W7jogH7RoUMzeYVPOxPCWwmHM";
    } */
    ...
    function tokensOfOwner(address _tokenOwner)
        external
        view
        returns (uint256[] memory)
      {
        ...
        if (tokenCount == 0) {
          ...
          for (index = 0; index < tokenCount; index++) {
            result[index] = tokenOfOwnerByIndex(_tokenOwner, index);
          }
          return result;
        }
      }
     ...
     function publicMint(uint256 numberOfToken) public payable {
        ...
        require(numberOfToken > 0 && numberOfToken <= maxTokenPurchase,"Can only mint one or 12 tokens at a time");
        require(msg.sender != address(0), "not first address"); 
        require(totalSupply().add(numberOfToken) <= maxSupply,"Purchase would exceed max supply of tokens");
        require( msg.value >= tokenPrice.mul(numberOfToken),"Ether value sent is not correct");
        ...
        for (uint256 i = 0; i < numberOfToken; i++) {
            ...
            CaptainMetadata memory newCaptainMetadata = CaptainMetadata(
               ...
               CaptainMetadaByAddresses[msg.sender][newItemId] = newCaptainMetadata;
               CaptainMetadaByAddresses2[msg.sender].push(newCaptainMetadata);
       ...
       function presaleMint(uint256 numberOfToken,bytes32[] calldata merkleProof,bytes32 merkleRoot ) public payable {
        ...
        require(
          totalSupply().add(numberOfToken) <= maxSupply,
          "Purchase would exceed max supply of token"
        );
        require(
          numberOfToken > 0 && numberOfToken <= presaleMaxMint,
          "Cannot purchase this many tokens"
        );

        require(
          _presaleListClaimed[msg.sender].add(numberOfToken) <= presaleMaxMint,
...
function airdropTokens(uint256 numberOfToken, address toAddress) external  {
      ...
      function buyToken(uint256 _tokenId) public payable {
        ...
        require(tokenOwner != address(0),"address(0) problem");
        // the one who wants to buy the token should not be the token's owner
        require(tokenOwner != msg.sender,"msg.sender problem");
        // get that token from all ChickenRun mapping and create a memory of it defined as (struct => ChickenRun)
        CaptainMetadata memory captain = allCaptainMetadata[_tokenId];
        // price sent in to buy should be equal to or more than the token's price
        require(msg.value >= captain.price,"msg.value >= captain.price problem");
        // token should be for sale
        require(captain.forSale,"forSale is false");
        ...
        captain.currentOwner = msg.sender;
        allCaptainMetadata[_tokenId] = captain;
        ...
        returns (string memory)
    {
        ...
        if (isRevealed == false) {
          return
           "ipfs://Qmd4MVVFmq6cZVWUofqUmHsaioLWDuYxs7KjvuHJyUHQcK/hidden.json";
           // "https://arweave.net/sgPDWrP9Aqw1nJCC-KZcnVfJcdgzqgtfzROoUHi7oAA/hidden.jpg";
        }
...
contract Fish is ERC20 {
    ...
    uint256 public VEFISH_PER_DAY_PER_TON = 400000000000000000;

    //uint256 public FISH_PER_DAY_PER_TON = 250000000000000000;

    // how much fish it costs to skip the cooldown
    uint256 public COOLDOWN_BASE = 100 ether; // base 100
    // how much additional fish it costs to skip the cooldown per ton
    uint256 public COOLDOWN_BASE_FACTOR = 100 ether; // additional 100 per ton
    // how long to wait before skip cooldown can be re-invoked
    uint256 public COOLDOWN_CD_IN_SECS = 86400; // additional 100 per ton
    ...
    mapping(address => uint24) public totalTonByAddresses;
    ...
    constructor(address _captainContract,address _contractOwner) ERC20(TOKEN_NAME, TOKEN_SYMBOL) {
    ...
    function _stake(uint256 tid) internal {
        ...
        // get calc'd values...
        (, , , , , , , uint256 ton ,) = x.allCaptainMetadata(tid);
        // if lastSkippedTs is 0 its mean it never have a last skip timestamp
        StakedCaptainObj memory c = stakedCaptain[tid];
        ...
        if (stakedCaptain[tid].ton == 0) {
            ...
            totalTon += uint24(ton);
            totalTonByAddresses[msg.sender] +=  uint24(c.ton) ;
            x.setStake(tid,true);
            // let ppl know!
     ...
     */
    function claimableViewForFish(uint256 tokenId) public view returns (uint256) {
        StakedCaptainObj memory c = stakedCaptain[tokenId];
        if (c.ton > 0) {
        ...
        function claimableViewForVeFish(uint256 tokenId) public view returns (uint256) {
        StakedCaptainObj memory c = stakedCaptain[tokenId];
        if (c.ton > 0) {
            uint256 fishPerDay = ((VEFISH_PER_DAY_PER_TON * (c.ton / 100)) +
                BASE_HOLDER_FISH);
            uint256 deltaSeconds = block.timestamp - c.sinceTs;
            return deltaSeconds * (fishPerDay / 86400);
        } else {
    ...
    function myStakedCaptain() public view returns (uint256[] memory) {
        Captain x = Captain(CAPTAIN_CONTRACT);
        uint256 captainCount = x.balanceOf(msg.sender);
    ...
    function myClaimableView() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < cnt; i++) {
            ...
            if (captain.ton > 0) {
                uint256 claimable = claimableViewForFish(tokenId);
                if (claimable > 0) {
    ...
    function _claimFish(uint256[] calldata tokenIds) internal {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (captain.ton > 0) {
                uint256 claimableFish = claimableViewForFish(tokenIds[i]);
                if (claimableFish > 0) {
        ...
        function _claimVeFish(uint256[] calldata tokenIds) internal {
        Captain x = Captain(CAPTAIN_CONTRACT);
        uint256 totalClaimableVeFish = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(x.ownerOf(tokenIds[i]) == msg.sender, "NOT OWNER");
            StakedCaptainObj memory captain = stakedCaptain[tokenIds[i]];
            // we only care about chikn that have been staked (i.e. kg > 0) ...
            if (captain.ton > 0) {
                uint256 claimableVeFish = claimableViewForVeFish(tokenIds[i]);
                if (claimableVeFish > 0) {
                    totalClaimableVeFish = totalClaimableVeFish + claimableVeFish;
                    // reset since, for the next calc...
                    captain.sinceTs = uint32(block.timestamp);
                    stakedCaptain[tokenIds[i]] = captain;
                }
        ...
        if (totalClaimableVeFish > 0) {
            _mint(msg.sender, totalClaimableVeFish);
            emit Minted(msg.sender, totalClaimableVeFish);
        }
    }
    ...
    function claimFish(uint256[] calldata tokenIds) external {
      ...
      function claimVeFish(uint256[] calldata tokenIds) external {
        _claimVeFish(tokenIds);
    }
    ...
    function _unstake(uint256 tokenId) internal {
        ...
        if (c.ton > 0) {
            // update snapshot values...
            totalTon -= uint24(c.ton);
            totalTonByAddresses[msg.sender] -=  uint24(c.ton) ;
            totalStakedCaptain -= 1;

            c.ton = 0;
            stakedCaptain[tokenId] = c;

             x.setStake(tokenId,false);
    ...
    function getYourTonTest()  external {

        totalTonByAddresses[msg.sender] +=  1 ;
    }
     ...
     */
     function levelUpCaptain(uint256 tid) external {
        ...
        require(c.eatenAmount >= feedLevelingRate(c.ton), "MORE FOOD REQD");
        ...
        c.ton = c.ton + 100;
        c.eatenAmount = 0;
        c.cooldownTs = uint32(block.timestamp + cooldownRate(c.ton));
        stakedCaptain[tid] = c;

        // need to increase overall size
        totalTon += uint24(100);
        totalTonByAddresses[msg.sender] +=  uint24(100) ;
    ...
    /**
     * Internal: burns the given amount of fish from the wallet.
     */
    function _burnFish(address sender, uint256 fishAmount) internal {
        // NOTE do we need to check this before burn?
        require(balanceOf(sender) >= fishAmount, "NOT ENOUGH EGG for Burn");
        _burn(sender, fishAmount);
        emit Burned(sender, fishAmount);
    }

    /**
     * Burns the given amount of fish from the sender's wallet.
     */
    ...
    /**
     * Skips the "levelUp" cooling down period, in return for burning Fish.
     */
     function skipCoolingOff(uint256 tokenId, uint256 fishAmount) external {
        StakedCaptainObj memory captain = stakedCaptain[tokenId];
        require(captain.ton != 0, "NOT STAKED");

        uint32 ts = uint32(block.timestamp);

        // check: enough fish in wallet to pay
        uint256 walletBalance = balanceOf(msg.sender);
        require( walletBalance >= fishAmount, "NOT ENOUGH EGG IN WALLET");

        // check: provided egg amount is enough to skip this level
        require(fishAmount >= checkSkipCoolingOffAmt(captain.ton), "NOT ENOUGH EGG TO SKIP");
    ...
    function airdropToExistingHolder(
        ...
        Captain instance = Captain(CAPTAIN_CONTRACT);
        ...
        
    {
        ...
        for (uint256 i = from; i <= to; i++) {
            ...
            // we only care about chikn that have been staked (i.e. kg > 0) ...
            if (captain.ton > 0) {
                _mint(currentOwner, claimableViewForFish(i));
                captain.sinceTs = uint32(block.timestamp);
