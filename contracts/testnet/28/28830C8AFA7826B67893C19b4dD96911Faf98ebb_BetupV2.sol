...
//                pcr.fTotal += _amount;
    ...
    function withdraw(
        ...
        require(_ids.length == _counter.length && _ids.length == _result.length, "Invalid length");
...
//        uint256 fallAmount,
    ...
    public view returns(IBetStruct.RewardType memory reward)
    {
        ...
        uint256 snapshot =  IMembershipCard(membershipAddress).getSnapshot(_account);
        if(_maxLevel == 0) {
            require(snapshot == 0, "Invalid _maxLevel");
        }else {
            require(snapshot == 2 ** (256 - _maxLevel), "Invalid _maxLevel");
        }

//        uint256 _maxLevel = IMembershipCard(membershipAddress).getMaxLevel(_account);
