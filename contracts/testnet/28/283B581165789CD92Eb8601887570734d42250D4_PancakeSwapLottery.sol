...
import "../interface/IRandomNumberGenerator.sol";
import "../interface/IPancakeSwapLottery.sol";
...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    // TODO: change this line to 4 hours - 5 minutes;
    ...
    mapping(address => LotteryAndTicket[]) public userLotteryHistory;

    function getUserHistory(address _userAddress, uint256 _index)
        external
        view
        returns (
            uint256 _mapLength,
    ...
    {
        require(_index >= 0, "_index should be >=0");
        _mapLength = userLotteryHistory[_userAddress].length;
        _lotteryId = userLotteryHistory[_userAddress][_index].lotteryId;
        _ticketId = userLotteryHistory[_userAddress][_index].ticketId;
        return (_mapLength, _lotteryId, _ticketId);
    }
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            //User Ticket history stored here
            userLotteryHistory[msg.sender].push(
                LotteryAndTicket({
        ...
        nonReentrant
    {
        ...
        // CHANGED FROM  randomGenerator.getRandomNumber(uint256(keccak256(abi.encodePacked(_lotteryId, currentTicketId))));
