...
contract AvaxStake is ReentrancyGuard {
    ...
    event StakeRemoved(address indexed _staker);
    event RxgEarned(address indexed _staker, uint256 _amount);
    event Payout(address indexed _staker, uint256 _amount);
    // modifiers
    modifier onlyStaker() {
        require(
    ...
    function autoEmissions(address _staker) internal returns (bool) {
        //in wei
        uint256 multiplier = payChart[stakes[_staker].currentDay] * 10;
        uint256 userStaked = stakes[_staker].amount;
        uint256 _payout = ((userStaked * multiplier) / 10000); // reward based on user staked amount
        uint256 payout = ((_payout / 72) * 10); // convert to RXG
        stakes[_staker].currentDay++;
        stakes[_staker].earnings += payout;
        emit RxgEarned(_staker, payout);
        return true;
    }
    ...
    function activate() public onlyActivator {
        ...
        lastIncrement = time;
        for (uint256 i = 0; i < stakers.length; i++) {
            if (time >= stakes[stakers[i]].timestamp + 86400) {
                // 15 second days for testing
    ...
    function getPayChart() external view returns (uint256[] memory ) {
        return payChart;
    }
