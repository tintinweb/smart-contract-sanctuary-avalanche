...
import {IDiamondLoupe} from "./vendors/interfaces/IDiamondLoupe.sol";
import {IDiamondCut} from "./vendors/interfaces/IDiamondCut.sol";
import {IERC173} from "./vendors/interfaces/IERC173.sol";
...
contract NGInitialize is WithStorage {
    ...
    function init() external onlyAdmin {
        ...
        // Whitelist should be enabled and only used for the star program currently
        ws().enabled = true;
        
        gameConstants().NFT_MAX_WHITELIST_TIER = 5000;
         
        gameConstants().NFT_SALE_AMOUNT_WL = 100000000000000000;
        
        gameConstants().MAX_ACCOUNT_NFT_MINTED = 3;

        gameConstants().BASE_CASH_PER_HOUR = 50;
        gameConstants().BASE_WOOD_PER_HOUR = 10;
        gameConstants().BASE_STONE_PER_HOUR = 8;
        gameConstants().BASE_STEEL_PER_HOUR = 6;
        gameConstants().BASE_OIL_PER_HOUR = 5;

        gameConstants().STAR_CASH_PER_HOUR = 50;
        gameConstants().STAR_WOOD_PER_HOUR = 10;
        gameConstants().STAR_STONE_PER_HOUR = 8;
        gameConstants().STAR_STEEL_PER_HOUR = 6;
        gameConstants().STAR_OIL_PER_HOUR = 5;

        gameConstants().CASH_FACTOR = 1045550;
        gameConstants().WOOD_FACTOR = 2613875;
        gameConstants().STONE_FACTOR = 3485167;
        gameConstants().STEEL_FACTOR = 4182200;
        gameConstants().OIL_FACTOR = 5227750;

        gameConstants().GOLD_TO_TIME = 40;

        gameConstants().UPGRADE_MAX_LEVEL_PERCENTAGE_CHANGE = 10;
        gameConstants().UPGRADE_MAX_STARTING_HOURS = 12;

        gameConstants().CASINO_TICKET_GOLD_COST = 50;
        gameConstants().CASINO_LOTTERY_PAYOUT = 110;

        gameConstants().ACCOUNTING_TAX_FACTOR = 300;

        gameConstants().BANK_VAULT_REWARD_PER_SECONDS = 83;

        gameConstants().TROOP_LEVEL_FACTOR = 2;
        // 3 hours
        gameConstants().TROOP_TRAIN_TIME_SECONDS = 60 * 60 * 3;
        

        // Hero's Stars
        gameConstants().HERO_STARS_WHITELIST[0] = HeroStars(
            0,      // Start Range
            1000,   // End Range
            40,     // 1 Star Chance
            20,     // 2 Star Chance
        ...
        gameConstants().HERO_STARS_WHITELIST[1] = HeroStars(
            1001,    // Start Range
            2500,   // End Range
            30,     // 1 Star Chance
            15,     // 2 Star Chance
        ...
        gameConstants().HERO_STARS_WHITELIST[2] = HeroStars(
            2501,   // Start Range
            3500,   // End Range
            15,     // 1 Star Chance
            10,     // 2 Star Chance
        ...
        gameConstants().HERO_STARS_WHITELIST[3] = HeroStars(
            3501,   // Start Range
            5000,   // End Range
            15,     // 1 Star Chance
            8,     // 2 Star Chance
        ...
        // Spinner Wheel prices
        MSSpinWheelRewards[8] memory prices;
        // Gold X2
        prices[0] = MSSpinWheelRewards(
            0,
            2,
            3,
            false,
        ...
        for (uint i = 0; i < prices.length; i++) {
            // console.log("price indexg", i); 
            for (uint j = 0; j < prices[i].percentChance; j++) {
    
                // console.log("rewardIndex", rewardIndex); 
                gameConstants().SPIN_WHEEL_REWARD[rewardIndex] = prices[i];
                rewardIndex ++;
            }
        }

        if (gs().currentDayOfEvents == 0) {
            gs().currentDayOfEvents = block.timestamp / 86400;
        }
    }
