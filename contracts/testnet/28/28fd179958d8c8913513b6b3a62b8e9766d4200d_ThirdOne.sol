...
interface IflatLaunchPeg {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool); //transfer existing nft to  burn it
...
interface IupgradeTokens {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool); //transfer upgradeToken nft to burn it
    function isApprovedForAll(address, address) external view returns(bool); //check approval for transfer
}
...
contract ThirdOne is Ownable {
    ...
    constructor () {
        ...
        superSafe = 0xc7952F9e71F8866FBb5D57B0cBdA3eCefEAB8F60;//chnage to a gnosis wallet, holding the 3250 supers we pre-minted
        botsCanMint = false; //false means that No smart contract bots can mint or upgrade a Super
        setTypeFVials(27,127,227,275,375,475);//some dummy test data, this should only be set after shuffle reveal
    }
    ...
    function _burnVial(uint _vialID, uint _chadID) internal {//approve transfer of vial first in dapp
        IflatLaunchPeg(vialContract).transferFrom(msg.sender, address(this), _vialID);//burn the upgradeToken by sending it to the zero address
