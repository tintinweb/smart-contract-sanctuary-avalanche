...
contract ProjectXCompetition {
    ...
    address public token = 0x89CCF36af143029c1E19409dF32cDcC3254bb4Cb;
    ...
    function create(uint32 countTotal, int256 priceForGuest, int256 priceForMember, uint32 maxPerPerson) public forSponser returns (uint256) {
    ...
    function update(uint256 id, uint32 countTotal, int256 priceForGuest, int256 priceForMember, uint32 maxPerPerson) public forSponser {
    ...
    function start(uint256 id, uint endTime) public forSponser {
    ...
    function buy(uint256 id, uint32 count) public {
        ...
        uint256 price = uint256(competition.priceForGuest>-1?competition.priceForMember:competition.priceForGuest) * count;
        if(count >= 10)
    ...
    function creditBalance(address account) public view returns (uint256) {
        ...
        if(member.timeUntil < block.timestamp)
            return 0;
        return creditsPerMonth * ((block.timestamp - member.timeStart - 1) / 30 days + 1) + member.creditsPlus - member.creditsMinus;
    }
