...
contract Royalty is Withdrawable {
    ...
    function withdrawWavax() public {
        ...
        _widthdrawWavax(marketingAddress, percent.mul(_marketingFee));
        if (secondStageActive) {
            _widthdrawWavax(treasureAddress, percent.mul(_treasureFee));
        }
        _widthdrawWavax(transitAddress, address(this).balance);
    }
    ...
    receive() external payable {
        ...
        if (balance > 0) {
            ...
            _widthdraw(marketingAddress, percent.mul(_marketingFee));
            if (secondStageActive) {
                _widthdraw(treasureAddress, percent.mul(_treasureFee));
            }
            _widthdraw(transitAddress, address(this).balance);
        }
    }
}
