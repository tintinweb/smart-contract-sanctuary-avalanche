...
contract NFTHoodie is NFTBase {
    constructor(
        address rewardManagerAddress_,
        string memory baseUri_,
        uint256 priceUsd_,
        address feedAddress_
    ) NFTBase("GENESiS HOODIE 11/22", "PHY11", rewardManagerAddress_, baseUri_, priceUsd_, feedAddress_) {}
}
...
abstract contract NFTBase is ERC721A, ERC721AQueryable, Ownable, PriceManager {
    uint256 public startDate; // Usd price with to decimals
    string baseUri = "";
    ...
    constructor(
        ...
        uint256 priceUsd_,
        address feedAddress_
    ) ERC721A(name_, symbol_) PriceManager(priceUsd_, feedAddress_) {
        MAX_SUPPLY = 2500;
        baseUri = baseUri_;

        // 1.11.2022
        startDate = 1668124800;

        rewardManager = IRewardMananger(rewardManagerAddress_);
    }

    function freeMint(uint256 numberOfMints) external payable {
        if (msg.sender != tx.origin) revert OnlyEOAAllowed();

        // TODO: Add Date check

        unchecked {
            if (numberOfMints + _totalMinted() > MAX_SUPPLY) revert MintLimit();
            _safeMint(msg.sender, numberOfMints);
        }
    }

    function upgrade(OrderItem[] memory orderItems) public payable {
        console.log(msg.value);
        console.log(getTotalPrice(address(0x0), orderItems.length));
        // 1% Slippage
        ...
        // TODO: add payout address
        payable(owner()).transfer(msg.value - totalReward);
    }

    function upgradeToken(OrderItem[] memory orderItems, address token) public {
        ...
        // TODO: add payout address
        IERC20(token).transfer(owner(), totalPrice - totalReward);
    }

    function _upgradeNFT(OrderItem memory item) internal {
        if (item.upgradeId == 0) {
            _safeMint(msg.sender, 1);
            nftById[_totalMinted()] = item.nft;
        } else {
            if (ownerOf(item.upgradeId) != msg.sender) revert NotOwner();
...
abstract contract PriceManager is Ownable {
    ...
    function calcPrice(uint256 _priceUsd, uint8 decimals) public view returns (uint256) {
    ...
    function getTotalPrice(address token, uint256 amount) public view returns (uint256 total) {
        if (token == address(0x0)) {
            total = ((calcPrice(priceUsd, 18) * amount) / 10e12) * 10e12; // Round Price
        } else if (_isStablecoin[token]) {
            total = (calcPrice(priceUsd, IERC20(token).decimals()) * amount);
        } else {
    ...
    function getTokenPrice(address token, uint256 price) public onlyOwner {
        _upgradePrice[token] = price;
    }
}
