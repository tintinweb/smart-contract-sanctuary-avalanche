        ...
        if (len > 0) {
            for (uint256 i = 0; i < len;) {
                if (loans[i] <= totalAmount) {
                    ...
                    emit Log(totalAmount,remainingAmount,len,loanAmounts[i],"first");
                    
                    stableCoin.transferFrom(seller, banks[i], loanAmounts[i]);
                    _IMarginLoan.updateLoan(seller, ids[i], 0, 1);
                    totalAmount  = remainingAmount;
                } else if (loans[i] > totalAmount) {
               
                    uint256 remainingLoan = loanAmounts[i] - totalAmount;
                    emit Log(remainingLoan,totalAmount,loanAmounts[i],len,"second");
