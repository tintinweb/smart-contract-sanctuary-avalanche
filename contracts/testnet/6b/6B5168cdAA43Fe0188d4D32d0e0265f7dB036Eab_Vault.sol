...
contract Vault is ERC20, ReentrancyGuard, Pausable {
  ...
  struct WorkStruct {
    uint256 canBorrowValue;
    uint256 canRepayValue;
    uint256 targetLpValue;
    uint256 borrowA;
    uint256 borrowB;
    uint256 repayA;
    uint256 repayB;
  ...
  mapping (address => WorkStruct) public workstructs;
  ...
  function getAssetValue() public view returns (uint256) {
    ...
    ) = IManager(manager).assetInfo(address(this));
    return _tokenAAsset + _tokenBAsset;
  }

  function getDebtValue() public view returns (uint256) {
    (,, uint256 _tokenADebt, uint256 _tokenBDebt) = IManager(manager).debtInfo(address(this));
    return _tokenADebt + _tokenBDebt;
  }

  function getEquityValue() public view returns (uint256) {
    (uint256 totalEquityValue) = IManager(manager).equityInfo(address(this));
    return totalEquityValue;
  }

  function getAssetAmt() public view returns (uint256, uint256) {
    (uint256 _tokenAAssetAmt, uint256 _tokenBAssetAmt,,) = IManager(manager).assetInfo(address(this));

    return (_tokenAAssetAmt, _tokenBAssetAmt);
  }

  function getDebtAmt() public view returns (uint256, uint256) {
    (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt,,) = IManager(manager).debtInfo(address(this));

    return (_tokenADebtAmt, _tokenBDebtAmt);
  }

  function getEquityAmt() public view returns (uint256, uint256) {
      (uint256 _tokenAAssetAmt, uint256 _tokenBAssetAmt,,) = IManager(manager).assetInfo(address(this));
      (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt,,) = IManager(manager).debtInfo(address(this));

      return (_tokenAAssetAmt - _tokenADebtAmt, _tokenBAssetAmt - _tokenBDebtAmt);
  }

  function getSvTokenValue() public view returns (uint256) {
    return getEquityValue() / totalSupply();
  }
  ...
  function getDelta() public view returns (uint256) {
    (uint256 _tokenAAmt,,,) = IManager(manager).assetInfo(address(this));

    (uint256 _tokenADebtAmt,,,) = IManager(manager).debtInfo(address(this));

    return (_tokenAAmt - _tokenADebtAmt) / _tokenAAmt;
  }
  ...
  function deposit(uint256 _depositAmt, uint256 _minSharesReceive) external nonReentrant {
    IManager(manager).compound(address(this));
    ...
    _checkAndMint(_depositAmt, _minSharesReceive, _equityValue);
  }
  ...
  function _calculate(uint256 _targetDebtValue) public view returns (uint256,
   uint256, uint256, uint256) {
    ...
    (,,uint256 _tokenADebt,
    uint256 _tokenBDebt) = IManager(manager).debtInfo(address(this));
  ...
  function _execute(uint256 _depositOrWithdraw,
    ...
    workstructs[address(this)].canBorrowValue = canBorrowValue;
    workstructs[address(this)].canRepayValue = canRepayValue;
    workstructs[address(this)].targetLpValue = _targetLpValue;
    workstructs[address(this)].borrowA = _borrowA;
    workstructs[address(this)].borrowB = _borrowB;
    workstructs[address(this)].repayA = _repayA;
    workstructs[address(this)].repayB = _repayB;
  ...
  function _moreCalc(uint256 canBorrowValue,
    ...
    uint256 repayB
        ...
        if (borrowA > 0 && borrowB > 0) {
          _borrowA = borrowA / ((borrowA + borrowB) * canBorrowValue);
          _borrowB = borrowB / ((borrowA + borrowB) * canBorrowValue);
  ...
  function _checkAndTransfer(
    ...
    ERC20(tokenA).safeTransfer(msg.sender, ERC20(tokenA).balanceOf(address(this)));
    ERC20(tokenB).safeTransfer(msg.sender, ERC20(tokenB).balanceOf(address(this)));
