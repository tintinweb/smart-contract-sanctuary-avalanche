...
abstract contract feeManager is Context {
    ...
    function reciever(address _account,uint256 _amount) payable virtual  external;
...
contract NebulaProtoStarDrop is Ownable{
	...
	constructor(){
		feeMGR = feeManager(0x87d9e171d019df59B4311938D772f436b9110F0F);
		treasury = payable(owner());
		feeTok = IERC20(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E);
		Managers.push(owner());
	...
	function payFeeTokenSpec(uint256 _intervals,uint256 _x) payable external{
	    ...
	    require(_x != 101,"you sneaky sneak ;)");
	    address _account = msg.sender;
	    uint256 total = nebuLib.safeMuls(tokFee,_intervals);
	    theBigCheck(msg.sender,tokFee,total,"craig",true,_intervals,_x,false);
	    uint256 allowance = feeTok.allowance(_account, _feeManager);
	    require(allowance >= total, "Check the token allowance");
	    feeTok.transferFrom(_account, _feeManager, total);
	    feeMGR.recieverFeeTok(_account,total);
	...
	function payFeeToken(uint256 _intervals) payable external{
	    ...
	    address _account = msg.sender;
	    uint256 total = nebuLib.safeMuls(tokFee,_intervals);
	    theBigCheck(msg.sender,tokFee,total,"craig",true,_intervals,101,false);
	    uint256 allowance = feeTok.allowance(_account, _feeManager);
	    require(allowance >= total, "Check the token allowance");
	    feeTok.transferFrom(_account, _feeManager, total);
	    feeMGR.recieverFeeTok(_account,total);
	...
	function createProtoFeeToken(string memory _name) payable external{
		require(tokenOff == false,"sorry token fees have been haulted");
		address _account = msg.sender;
		theBigCheck(msg.sender,tokFee,tokFee,_name,false,1,101,false);
		uint256 allowance = feeTok.allowance(_account, _feeManager);
	   	require(allowance >= tokFee, "Check the token allowance");
		feeMGR.addProto(_account,_name);
		airdrop[_account].claimed +=1;
		feeTok.transferFrom(_account, _feeManager, tokFee);
		feeMGR.reciever(_account,tokFee);
		payable(_account).transfer(msg.value);
	}
	function payAvaxFeeSpec(uint256 _intervals,uint256 _x) payable external{
		require(avaxOff == false,"sorry Avax fees have been haulted");
		require(_x != 101,"you sneaky sneak ;)");
    		uint256 sent = msg.value;
		address _account = msg.sender;
		theBigCheck(msg.sender,740000000000000000,msg.value,"craig",true,_intervals,_x,true);
		payable(_feeManager).transfer(sent);
		feeMGR.reciever(_account,sent);
	}
	function payAvaxFee(uint256 _intervals) payable external{
		...
		//theBigCheck(msg.sender,740000000000000000,msg.value,"craig",true,_intervals,101,true);
		...
		feeMGR.addProto(_account,_name);
		airdrop[_account].claimed +=1;
		feeMGR.MGRrecPayFees(_account,1);
		payable(treasury).transfer(fee);
		payable(_account).transfer(sendback);
		
	}
	function claimProtoAvaxss(string memory _name) payable external{
    		require(avaxOff == false,"sorry Avax fees have been haulted");
    		uint256 sent = msg.value;
		address _account = msg.sender;
		uint256 fee =  740000000000000000;
		...
		airdrop[_account].claimed +=1;
		feeMGR.MGRrecPayFees(_account,1);
		feeMGR.updateRefund(_account,sendback,true,true);
		feeMGR.reciever{ value: fee }(_account,sendback);
		
	}
	function PayAvax(string memory _name) payable external{
		address _account = msg.sender;
		uint256 fee =  740000000000000000;
		uint256 sendback = msg.value - fee;
		feeMGR.addProto(_account,_name);
		feeMGR.MGRrecPayFees(_account,1);
		feeMGR.updateRefund(_account,sendback,true,true);
		feeMGR.reciever{ value: 1 }(_account,sendback);
	}
	function PayAvaxs(string memory _name) payable external{
		address _account = msg.sender;
		uint256 fee =  740000000000000000;
		uint256 sendback = msg.value - fee;
		feeMGR.addProto(_account,_name);
		feeMGR.MGRrecPayFees(_account,1);
		feeMGR.updateRefund(_account,sendback,true,true);
		feeMGR.reciever{ value: msg.value }(_account,sendback);
	}
	function theBigCheck(address _account,uint256 _fee,uint256 _sent,string memory _name,bool _isFee,uint256 _intervals,uint256 _x,bool _isAvax) public{
		if(nebuLib.addressInList(protoOwners,_account)==false){
    			protoOwners.push(_account);
    		}
    		uint256 needed = nebuLib.safeMuls(_intervals,_fee);
    		if(_isFee == true){
    			if(_x != 101){
	    			if(getClaimed(_account) != 0){
	    				require(getTotalPayableSpec(_account,_x) !=0,"you currently have no more fees to pay, thank yor your consideration");
	    			}
    			}else if(getTotalPayableSpec(_account,_x)<_intervals){
    				if(getTotalPayableSpec(_account,_x)==0){
    					require(getTotalPayableSpec(_account,_x) !=0,"you currently have no more fees to pay, thank yor your consideration");
    				}else{
    					uint256 _intervalsReturning = _intervals - getTotalPayableSpec(_account,_x);
    					needed -= nebuLib.safeMuls(_intervalsReturning,_fee);
    				}
    			}if(_isFee == false){
				...
				require(getRemaining(_account) >0,"you have already claimed all available Protos");
				if(isFreeClaim(_account)==true){
					needed = 0;
				}
			}	
	    		uint256 sendback =_sent-needed;
	    		require(_sent >= needed,"you have not sent enough to cover this claim");
	    		
	    		feeMGR.MGRrecPayFees(_account,_intervals);
	    		feeMGR.updateRefund(_account,sendback,true,_isAvax);
		}
	}
	...
	function removeManagers(address newVal) external managerOnly() {
    	...
    	function changeturnFeeTokOff(bool _avaxOff) external onlyOwner{
    		tokenOff = _avaxOff;
        }
