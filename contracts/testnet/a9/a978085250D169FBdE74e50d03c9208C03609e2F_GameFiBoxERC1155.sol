                ...
                for (uint256 j = 0; j < _rewardSetByBox[tokenId][i].reiterations; j++) {
                    ...
                    for (uint256 l = 0; l < actualizedRewardSet.probabilities.length; l++) {
                        if (randomValue >= lowerBoundary && randomValue < lowerBoundary + actualizedRewardSet.probabilities[l]) {
                            ...
                            emit Reward(_msgSender(), tokenId, k, actualizedRewardSet, block.timestamp);
...
interface IGameFiBoxERC1155 is ITokenTypes, IGameFiTokenERC1155 {
    ...
    event Reward(address indexed target, uint256 indexed tokenId, uint256 indexed index, RewardSet gotReward, uint256 timestamp);
