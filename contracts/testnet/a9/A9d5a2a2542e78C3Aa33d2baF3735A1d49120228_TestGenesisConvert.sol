...
contract LimitMetadata is Ownable{
    ...
    function isValidMeta(uint256 nftType, uint256 rarity, uint256 level, uint256 basicPerformance) public view returns(bool){
        bool isValid = true;
    ...
    function _configUnboxLimit(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel, uint256[] memory rarities, uint256[] memory minValues, uint256[] memory maxValues) internal{
    ...
    function configUnboxLimit(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel, uint256[] memory rarities, uint256[] memory minValues, uint256[] memory maxValues) public onlyOwner{
        _configUnboxLimit(_maxType, _maxRarity, _maxLevel, rarities, minValues, maxValues);
    }

    function getConfigUnboxLimit(uint256 rarity) public view returns(uint256, uint256, uint256, Limit memory){
        return (maxType, maxRarity, maxLevel, basicPerformances[rarity]);
    }

    function _configTypeAndRarityAndLevel(uint256 _maxType, uint256 _maxRarity, uint256 _maxLevel) internal{
        maxType = _maxType;
...
contract TestGenesisConvert is ERC4907, ERC721Burnable, BusinessAddresses, Signer, TransferFee, ReentrancyGuard, Withdrawable, LimitMetadata {
    ...
    function safeMint(address to, address creator, Metadata memory meta) private returns(uint256) {
        require(isValidMeta(meta.typeNFT, meta.rarity, 1, meta.basicPerformance),"TestGenesisConvert: Invalid meta data");
