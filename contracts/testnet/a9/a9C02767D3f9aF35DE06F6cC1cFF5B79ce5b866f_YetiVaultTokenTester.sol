...
/**
 * Tester contract for wrapping and unwrapping vault tokens in the Yeti Finance borrowing protocol.
 * Users need to approve this contract to spend their underlying. 
 */

contract YetiVaultTokenTester is IYetiVaultToken, ERC20 {
    IERC20 public underlying;
    uint256 public underlyingDecimal;

    constructor (
        string memory name,
        string memory symbol,
        address _underlying,
        uint256 _underlyingDecimal
    ) public payable ERC20(name, symbol) {
        underlying = IERC20(_underlying);
        underlyingDecimal = _underlyingDecimal;
    }

    function deposit(uint256 _amt) external override returns (uint256 receiptTokens) {
        return depositFor(msg.sender, msg.sender, _amt);
    }

    function depositFor(address _borrower, address _recipient, uint256 _amt) public override returns (uint256 receiptTokens) {
        receiptTokens = _amt * receiptPerUnderlying() / 1e18;
        _mint(_recipient, receiptTokens);
        underlying.transferFrom(_borrower, address(this), _amt);
    }

    function redeem(address _to, uint256 _amt) external override returns (uint256 underlyingTokens) {
        underlyingTokens = _amt * underlyingPerReceipt() / 1e18;
        _burn(msg.sender, _amt);
        underlying.transfer(_to, underlyingTokens);
    }
