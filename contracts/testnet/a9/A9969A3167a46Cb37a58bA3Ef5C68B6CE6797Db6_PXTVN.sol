...
contract PXTVN is ERC20Upgradeable, KeeperCompatibleInterface {
    // STRINGS
    ...
    function initialize() public virtual initializer {
        ...
        _isExcludedFromFee[liquidityManager] = true;

        _buyTaxRates = BuyFees({operation: 500, dev: 200, liquidity: 200, marketing: 0, burn: 100});
        _sellTaxRates = SellFees({operation: 200, dev: 300, liquidity: 100, marketing: 300, burn: 100});
        ...
        LOCK = 0x000000000000000000000000000000000000CcCc;
        ...
        topPrice = 11 ether;
        dfDecimals = 1e12;
        setRewarRatePerDay(0, 100);
        setRewarRatePerDay(1, 200);
        setRewarRatePerDay(2, 300);
        setRewarRatePerDay(3, 400);
        setRewarRatePerDay(4, 500);
        setRewarRatePerDay(5, 1000);

        antiWhaleUsdtLimit1 = 5000 ether;
        antiWhaleUsdtLimit2 = 7500 ether;
        antiWhaleUsdtLimit3 = 10000 ether;
        ...
        address currentRouter = 0xd7f655E3376cE2D7A2b08fF01Eb3B1023191A901; // 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
        // address currentRouter = 0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9; // localhost
    ...
    function setMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        // require(merkleRoot == bytes32(0), "GweiDao: Merkle root already set");
    ...
    function lock(uint256 _amount) public {
        ...
        if(lockedAmount[msg.sender]>0){
            uint256 rewardAmount  = calcRewards(msg.sender, lockedAmount[msg.sender]);
            super._transfer(LOCK, msg.sender, rewardAmount);
        }
        lockedTime[msg.sender] = uint32(block.timestamp);
    ...
    function unlock(uint256 _amount) public {
        require(lockedAmount[msg.sender] > 0, "Locked amount must be greater than zero");
        lockedAmount[msg.sender] -= _amount;
        uint256 rewardAmount  = calcRewards(msg.sender, _amount);
        super._transfer(LOCK, msg.sender, _amount + rewardAmount);
        emit Unlocked(msg.sender, _amount);
    }

    function calcRewards(address _account, uint256 _amount) public view returns(uint256){
        uint256 rewardAmountPerDay = 0;
        uint256 lockedDuration = 0;
        if(nftAddress!=address(0x0)){
            if(INft(nftAddress).userRarity(_account, 0) > 0){
                rewardAmountPerDay = rewardRatePerDay[1] * _amount / 10000;
            }else if(INft(nftAddress).userRarity(_account, 1) > 0){
                rewardAmountPerDay = rewardRatePerDay[2] * _amount / 10000;
            }else if(INft(nftAddress).userRarity(_account, 2) > 0){
                rewardAmountPerDay = rewardRatePerDay[3] * _amount / 10000;
            }else if(INft(nftAddress).userRarity(_account, 3) > 0){
                rewardAmountPerDay = rewardRatePerDay[4] * _amount / 10000;
            }else if(INft(nftAddress).userRarity(_account, 4) > 0){
                rewardAmountPerDay = rewardRatePerDay[5] * _amount / 10000;
            } else 
                rewardAmountPerDay = _amount * rewardRatePerDay[0] / 10000;
        }else
            rewardAmountPerDay = _amount * rewardRatePerDay[0] / 10000;
        lockedDuration = (uint32(block.timestamp) - lockedTime[_account]) / 86400;
        return rewardAmountPerDay * lockedDuration;
    }
    ...
    function _finalizeTransfer( address from, address to, uint256 amount ) internal virtual {
        ...
        if (takeFee) {
            ...
            // if user are selling, we check if sell amount hits antiwhale limit threshold
                ...
                operationFee += amount * _sellTaxRates.operation * dynamicTaxRate / 10 / masterTaxDivisor;
                devFee += amount * _sellTaxRates.dev * dynamicTaxRate / 10 / masterTaxDivisor;
                liquidityFee += amount * _sellTaxRates.liquidity * dynamicTaxRate / 10 / masterTaxDivisor;
                marketingFee += amount * _sellTaxRates.marketing * dynamicTaxRate / 10 / masterTaxDivisor;
                burnFee += amount * _sellTaxRates.burn * dynamicTaxRate / 10 / masterTaxDivisor;
                feeAmount = (_sellTaxRates.operation + _sellTaxRates.dev + _sellTaxRates.liquidity + _sellTaxRates.marketing + _sellTaxRates.burn) * amount * dynamicTaxRate / 10 / masterTaxDivisor;
    ...
    function setRewarRatePerDay(uint8 _rarityId, uint256 _rate) public onlyOwner{
    ...
    function getAmountOut() internal view returns (uint256) 
    {
        if (address(dexRouter) == address(0)) return 0;
        address[] memory path = new address[](2);
        path[0] = address(this);
