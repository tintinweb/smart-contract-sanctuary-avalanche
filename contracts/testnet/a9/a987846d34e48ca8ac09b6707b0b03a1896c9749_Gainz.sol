...
contract RedPillSanta is ERC721, ERC721Enumerable, Authorizable, ReentrancyGuard, IERC2981 {
    ...
    uint256 public TRANSFER_COOLDOWN_PERIOD = 3 minutes;
...
contract Gainz is ERC20, Authorizable, ReentrancyGuard {
 ...
 function viewAllEarnedGainz() public view returns (uint256) {
        RedPillSanta santaContract = RedPillSanta(SANTA_CONTRACT);
        uint256 cnt = santaContract.balanceOf(msg.sender);
        require(cnt>0, "Sender has no Santas!");
        uint256 totalGainzEarned = 0;
        for (uint256 i = 0; i < cnt; i++) {
            uint256 tokenId = santaContract.tokenOfOwnerByIndex(msg.sender, i);
            StakedSantaObj memory santa = stakedSantas[tokenId];
            if (santa.strength > 0) { // To be sure that santa is staked
                uint256 earnedAmount = viewEarnedGainz(tokenId);
                if (earnedAmount > 0) {
