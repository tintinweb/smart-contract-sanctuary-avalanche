...
contract BridgesOfFate is Ownable {
    ...
    uint256 private constant TURN_PERIOD = 360; //Now we are use 5 minute //86400; // 24 HOURS
    ...
    mapping(uint256 => uint256) private gameRandom;
    mapping(uint256 => uint256) public pioneerPushBackGameMap; //private
    ...
    function _pushBackBankingGameEndRules() internal view returns (bool) {
       if((pioneerPushBackGameMap[gameStatusInitialized[_latestGameId].stageNumber] > 0) && 
        ...
        {
            require(_isPioneerPushBackEnd == true ,"Dynamic Game Ended.");
        }
        return true;
    }
    ...
    //     uint256 _packedNumber =  uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender)));
    ...
    // function _randomDynamicGame(uint256 _range) internal view returns (uint256) {
    //     uint256 _packedNumber =  uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender)));
    //     uint256 _number =  uint256ToArray(_packedNumber,_range);
    //     return _number; 
    // }
    ...
    function _checkSide(uint256 stageNumber, bool userSide) internal view returns (bool) {
        uint256 stage__randomNumber = gameRandom[stageNumber]; 
    ...
    function buyBackInFee(bytes32 playerId) public  GameEndRules PushBackBankingGameEndRules {
        ...
        _member.lastJumpSide = gameRandom[_member.stage] >= 50;
    ...
    function bulkbBuyBackInFee(bytes32[] calldata _playerId) external {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules PushBackBankingGameEndRules{
        ...
        require(_member.feeStatus == true, "Please Pay Entry Fee.");
        // require(pioneerPushBackGameMap[_gameStatus.stageNumber] < 50,"Dynamic Game Ended.");
        ...
        if (_member.stage != 0) {
            require((_member.lastJumpSide == true && gameRandom[_member.stage] >= 50) || (_member.lastJumpSide == false && gameRandom[_member.stage] < 50), "You are Failed" );
        }

        uint256 _currentUserStage =  _member.stage + 1;
        if (gameRandom[_currentUserStage] <= 0) {
            ...
            gameRandom[_currentGlobalStage] = _packedRandomNumber[0];
            _gameStatus.stageNumber = _currentGlobalStage;
            _gameStatus.lastUpdationDay = currentDay;
            // pioneerPushBackGameMap[_currentGlobalStage] = _randomDynamicGame(100);
    ...
    function pushBack(bytes32 playerfrom,bytes32 playerto) external GameEndRules PushBackBankingGameEndRules  {
        ...
        _memberto.lastJumpSide = gameRandom[_memberto.stage] >= 50;

        //  if (GameMap[gameStatusInitialized[_latestGameId].stageNumber - 1] >= 50) {
        ...
        lastUpdateTimeStamp    =  block.timestamp;
        // playerItem[playerto] = _memberto;
        // playerItem[playerfrom] = _memberfrom;
    } 
    ...
    //         if (GameMap[_playerFromStage] <= 0) {
    //             uint256 _currentGlobalStage =  _gameStatus.stageNumber + 1;
    //             uint8[2] memory _packedRandomNumber = generateRandomNumbersForGame(100,2);
    //             // GameMap[_currentGlobalStage] = _random(100) * 1;
    //             GameMap[_currentGlobalStage] = _packedRandomNumber[0];
    //             _gameStatus.stageNumber = _currentGlobalStage;
    //             _gameStatus.lastUpdationDay = currentDay;
    //             // pioneerPushBackGameMap[_currentGlobalStage] = _randomDynamicGame(100);
    //             pioneerPushBackGameMap[_currentGlobalStage] = _packedRandomNumber[1];
