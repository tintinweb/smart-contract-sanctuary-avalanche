...
contract VideoPokerLogic {
  ...
  function reverseCards(uint256 cards) internal pure returns (uint8[5] memory) {
  ...
  function reverseCardsB(uint256 cards) internal pure returns (uint8[5] memory) {
    uint8[5] memory cardValues;
  ...
  function getCardDetails(uint cards) public pure returns (uint8[5] memory values, uint8[5] memory suits) {
  for (uint i = 0; i < 5; i++) {
    uint card = (cards >> (6 * i)) & 63; // Extraire la valeur de la carte
    uint8 value = uint8(card % 13 + 2); // Convertir l'index de la carte en valeur de carte (de 2 à 14)
    uint8 suit = uint8(card / 13); // Déterminer la couleur de la carte en divisant l'index de la carte par 13
    values[i] = value;
    suits[i] = suit;
  }
  return (values, suits);
