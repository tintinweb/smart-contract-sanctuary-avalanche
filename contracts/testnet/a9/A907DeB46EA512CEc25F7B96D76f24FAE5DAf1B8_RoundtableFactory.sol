...
contract RoundtableFactory is Multicall, Owned {
	...
	function deployTable(
		...
		address[2] memory initialExtensions = [address(pfpStaker), fundraiseExtension];

		roundtableDAO.init{value: msg.value}(name_, symbol_, voters_, initialExtensions, govSettings_);
