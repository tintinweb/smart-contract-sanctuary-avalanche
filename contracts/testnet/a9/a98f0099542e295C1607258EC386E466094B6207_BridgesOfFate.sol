...
contract BridgesOfFate is Ownable {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        bool alreadyJumped = _member.startAt != 0 || _member.lastJumpTime != 0;
        require(alreadyJumped || currentDay >= _member.day, "Already Jumped");
        ...
        bool jumpSuccessful = totalNFTOnTile[_currentUserStage][_jumpSide] > 0;
        if (!jumpSuccessful) {                
        ...
        bool isSafeSidePervious = GameMap[_currentUserStage - 1] < 50e9 ? false : true;

        if(_currentUserStage >= 2  && TitleCapacityAgainestStage[_currentUserStage - 1] >= totalNFTOnTile[_currentUserStage - 1][isSafeSidePervious]){
            totalNFTOnTile[_currentUserStage - 1][isSafeSidePervious]++;
        }  
        ...
        _member.startAt = _member.lastJumpTime = lastUpdateTimeStamp = block.timestamp;
        // _member.lastJumpTime = block.timestamp;
