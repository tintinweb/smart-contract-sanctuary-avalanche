...
import "../interfaces/IBVICToken.sol";
...
contract BVICGenesis is ContractGuard, Ownable {
    ...
    // Total reward phase 2: 1.5 * 75% * 3/4 = 0.84375
    uint256 public constant TOTAL_REWARD_POOL_4_NEXT_PHASE = 0.5484375 ether; // 65% of (75% of 1.5 * 3/4) BVIC-BTC.b
    uint256 public constant TOTAL_REWARD_POOL_3_NEXT_PHASE = 0.084375 ether; // 10% of (75% of 1.5 * 3/4) CHAM-USDC
    uint256 public constant TOTAL_REWARD_POOL_2_NEXT_PHASE = 0.084375 ether; // 10% of (75% of 1.5 * 3/4) EVIC-WETH.e
    uint256 public constant TOTAL_REWARD_POOL_1_NEXT_PHASE = 0.084375 ether; // 10% of (75% of 1.5 * 3/4) AVIC-USDC
    uint256 public constant TOTAL_REWARD_POOL_0_NEXT_PHASE = 0.0421875 ether; // 5% of (75% of 1.5 * 3/4) BTC.b

    uint256 public constant runningTime = 2 days;
    uint256 public constant TOTAL_USER_REWARD = 1.125 ether; // 75% of 1.5 Token
    uint256 public constant TOTAL_POL_REWARD = 0.225 ether; // 15% of 1.5 Token
    uint256 public constant TOTAL_DAO_REWARD = 0.15 ether; // 10% of 1.5 Token
    ...
    constructor(address _token, uint256 _poolStartTime) {
        ...
        bvicTokenPerSecondForUser = TOTAL_USER_REWARD.div(runningTime); // 75% of 1.5  Token / (2days * 24h * 60min * 60s)
        bvicTokenPerSecondForPol = TOTAL_POL_REWARD.div(runningTime); // 15% of 1.5  Token / (2days * 24h * 60min * 60s)
        bvicTokenPerSecondForDao = TOTAL_DAO_REWARD.div(runningTime); // 10% of 1.5  Token / (2days * 24h * 60min * 60s)
    ...
    function pendingPol(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (IBVICToken(bvicToken).isPolWallet(_user)) {
            if (_fromTime >= _toTime) return 0;
            if (_toTime >= poolEndTime) {
                if (_fromTime >= poolEndTime) return 0;
    ...
    function pendingDao(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (IBVICToken(bvicToken).isDaoFund(_user)) {
            if (_fromTime >= _toTime) return 0;
            if (_toTime >= poolEndTime) {
                if (_fromTime >= poolEndTime) return 0;
...
interface IBVICToken is IERC20 {
