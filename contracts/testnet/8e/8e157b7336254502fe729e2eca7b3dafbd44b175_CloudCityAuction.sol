...
// File: contracts/interfaces/ICloudCityAuction.sol
...
// File: contracts/CloudCityAuction.sol
 ...
 */
contract CloudCityAuction is ICloudCityAuction, ReentrancyGuard {
    ...
    // pulled out of Auction struct
    address public tokenContract;
    address payable private curator;
    uint8 public curatorFeePercentage;
    // If set to 0x0, the auction will be run in ETH
    address public auctionCurrency;

    // A mapping of all of the auctions currently running.
    mapping(uint256 => ICloudCityAuction.Auction) public auctions;
    ...
    constructor(
        ...
        tokenContract = 0x7BD485e8F7913259aBB7eA5e6e8398c53b223A79;
        ...
        curator = payable(0xCc879Ab4DE63FC7Be6aAca522285D6F5d816278e);
    ...
    function createAuction(
        ...
        // require(IERC165(_tokenContract).supportsInterface(interfaceId), "tokenContract does not support ERC721 interface");
        ...
        emit AuctionCreated(auctionId, tokenId, 
            duration, auctions[auctionId].reservePrice, auctions[auctionId].tokenOwner
        );
    ...
    function setAuctionApproval(uint256 auctionId, bool approved) external override auctionExists(auctionId) {
        require(msg.sender == curator, "Must be auction curator");
        require(auctions[auctionId].firstBidTime == 0, "Auction has already started");
        _approveAuction(auctionId, approved);
    }

    // ignores auctionId, sets it for the entire auction
    function setAuctionReservePrice(uint256 auctionId, uint256 _reservePrice) external override auctionExists(auctionId) {
        require(msg.sender == curator || msg.sender == auctions[auctionId].tokenOwner, "Must be auction curator or token owner");
        require(auctions[auctionId].firstBidTime == 0, "Auction has already started");
        auctions[auctionId].reservePrice = _reservePrice;

        emit AuctionReservePriceUpdated(auctionId, auctions[auctionId].tokenId, tokenContract, auctions[auctionId].reservePrice);
        // auctions[auctionId].tokenContract
    }

    function setCurator(address payable _curator) external {
        require(msg.sender == curator, "Must be auction curator");
        curator = _curator;
    }
     ...
     */
    function createBid(uint256 auctionId, uint256 amount) external payable override auctionExists(auctionId) nonReentrant {
        ...
        if (auctions[auctionId].firstBidTime == 0) {
            ...
            _handleOutgoingBid(lastBidder, auctions[auctionId].amount, auctionCurrency);
        }

        _handleIncomingBid(amount, auctionCurrency);
        ...
        if (extended) {
            emit AuctionDurationExtended(auctionId, auctions[auctionId].tokenId, tokenContract, auctions[auctionId].duration);
        }
    }
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        address currency = auctionCurrency == address(0) ? wethAddress : auctionCurrency;
        ...
        try
            IERC721(tokenContract).safeTransferFrom(address(this), auctions[auctionId].bidder, auctions[auctionId].tokenId)
        {} catch {
            _handleOutgoingBid(auctions[auctionId].bidder, auctions[auctionId].amount, auctionCurrency);
        ...
        if (curator != address(0)) {
            ...
            _handleOutgoingBid(curator, curatorFee, auctionCurrency);
        }
        _handleOutgoingBid(auctions[auctionId].tokenOwner, tokenOwnerProfit, auctionCurrency);
    ...
    function cancelAuction(uint256 auctionId) external override nonReentrant auctionExists(auctionId) {
        require(
            auctions[auctionId].tokenOwner == msg.sender || curator == msg.sender,
    ...
    function _cancelAuction(uint256 auctionId) internal {
        ...
        IERC721(tokenContract).safeTransferFrom(address(this), auctions[auctionId].tokenOwner, auctions[auctionId].tokenId);

        emit AuctionCanceled(auctionId, auctions[auctionId].tokenId, tokenContract, auctions[auctionId].tokenOwner);
        delete auctions[auctionId];
    }

    function _approveAuction(uint256 auctionId, bool approved) internal {
        auctions[auctionId].approved = approved;
        emit AuctionApprovalUpdated(auctionId, auctions[auctionId].tokenId, tokenContract, approved);
    }

    function _exists(uint256 auctionId) internal view returns (bool) {
        return auctions[auctionId].tokenId != 0; //address(0);
    }
