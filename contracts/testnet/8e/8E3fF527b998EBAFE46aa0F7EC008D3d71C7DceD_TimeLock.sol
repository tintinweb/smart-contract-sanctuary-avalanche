...
contract TimeLock is Ownable {
    ...
    uint256 public constant BATCH_PERIOD = 60 * 5;
    ...
    constructor(address _token) {
        token = _token;
        adr.push(0x6c7059c5D4e9395a4000Dd77dBaC4654d7E3e165);
        adr.push(0x6FBBDfB19cbC84A20701a6860Ad487B3F34c0cE3);
        adr.push(0x7291030263771b40731D6Bc6b352358D23F5737F);
        for (uint i = 0; i < adr.length; i++) {
            lockedBalances[adr[i]].lockedAmount = 1000 * 1000000;
            lockedBalances[adr[i]].releaseBatches = 10;
            lockedBalances[adr[i]].lockTimestamp = block.timestamp.add(180);
        }
    }
    ...
    function modifyAddress(
        ...
        for (uint i = 0; i < adr.length; i++) {
            if (adr[i] == _privious){
                ...
                if (count == 0){
                    adr[i] = update;
                    lockedBalances[update].lockTimestamp = lockedBalances[_privious].lockTimestamp;
                    lockedBalances[update].lockedAmount = lockedBalances[_privious].lockedAmount;
                    lockedBalances[update].releaseBatches = lockedBalances[_privious].releaseBatches;
                    lockedBalances[update].withdrawedAmount = lockedBalances[_privious].withdrawedAmount;
                    delete lockedBalances[_privious];
                    return ;
                }
            }
        }
