...
contract KryptorenaBattle is VRFConsumerBaseV2, Ownable {
    ...
    event ObtainedRng(uint256);
    event ObtainedIntHpDifference(int256);
    event ObtainedUintHpDifference(uint256);
    event ObtainedRngAtk(uint256);
    event ObtainedDefValue(uint256);
    event ObtainedWinner(address);

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        console.log("FULFILLING RANDOM WORD");
        uint256 randomNumber = randomWords[0];
        emit ObtainedRng(randomNumber);

        address winner = s_requestIdToSender[requestId];
        uint256 id = currentMatch[msg.sender].battleId;
        Battle storage round = battles[id];
        int256 int_HP_DIFFERENCE = absoluteValue(round.player1HP, round.player2HP);
        emit ObtainedIntHpDifference(int_HP_DIFFERENCE);
        uint256 s_HP_DIFFERENCE = uint256(int_HP_DIFFERENCE);
        emit ObtainedUintHpDifference(s_HP_DIFFERENCE);

        uint256 rngAttackValue = randomNumber % s_HP_DIFFERENCE;
        emit ObtainedRngAtk(rngAttackValue);
        uint256 rngDefenseValue = s_HP_DIFFERENCE - rngAttackValue;
        emit ObtainedDefValue(rngDefenseValue);
