...
import "@openzeppelin/contracts/math/SafeMath.sol";
import "../erc-library/contracts/IERC20.sol";
import "../erc-library/contracts/SafeERC20.sol";

import "./NftMintingStation.sol";

/** @title NftFactoryV3.
 * @notice It is a contract for users to mint 'starter NFTs'.
 */
contract NftFactoryV3 is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    NftMintingStation public nftMintingStation;
    ...
    // Number of TOKENs a user needs to pay to acquire a token
    ...
    uint8 private constant numberNftIds = 10;

    // number of previous series (i.e. different visuals)
    uint8 private constant previousNumberNftIds = 5;
     ...
     */
    constructor(
        NftMintingStation _nftMintingStation,
        IERC20 _tokenToken,
        uint256 _tokenPrice,
    ...
    /**
     * @dev Mint NFTs from the NftMintingStation contract.
    ...
    function mintNFT(uint8 _nftId) external {
        ...
        // Send TOKEN tokens to this contract
        tokenToken.safeTransferFrom(senderAddress, address(this), tokenPrice);

        string memory tokenURI = nftIdURIs[_nftId];

        uint256 tokenId = nftMintingStation.mintCollectible(senderAddress, tokenURI, _nftId);

        emit NftMint(senderAddress, tokenId, _nftId);
    }

    /**
     * @dev It transfers the TOKEN tokens back to the chef address.
     ...
     */
    function setNftJson(
        string calldata _nftId5Json,
        string calldata _nftId6Json,
        string calldata _nftId7Json,
        string calldata _nftId8Json,
        string calldata _nftId9Json
    ) external onlyOwner {
        nftIdURIs[5] = string(abi.encodePacked(ipfsHash, _nftId5Json));
        nftIdURIs[6] = string(abi.encodePacked(ipfsHash, _nftId6Json));
        nftIdURIs[7] = string(abi.encodePacked(ipfsHash, _nftId7Json));
        nftIdURIs[8] = string(abi.encodePacked(ipfsHash, _nftId8Json));
        nftIdURIs[9] = string(abi.encodePacked(ipfsHash, _nftId9Json));
    }
...
import "./PantokenNfts.sol";

/** @title NftMintingStation.
 * @dev This contract allows different factories to mint
 * Pantoken Collectibles/Nfts.
 */
contract NftMintingStation is AccessControl {
    PantokenNfts public pantokenNfts;
    ...
    constructor(PantokenNfts _pantokenNfts) public {
        pantokenNfts = _pantokenNfts;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Mint NFTs from the PantokenNfts contract.
     * Users can specify what nftId they want to mint. Users can claim once.
    ...
    function mintCollectible(
        ...
        uint256 tokenId = pantokenNfts.mint(_tokenReceiver, _tokenURI, _nftId);
        return tokenId;
    }
    ...
    function setNftName(uint8 _nftId, string calldata _nftName) external onlyOwner {
        pantokenNfts.setNftName(_nftId, _nftName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        pantokenNfts.transferOwnership(_newOwner);
    }
}
...
/** @title PantokenNfts.
 * @notice It is the contracts for PantokenSwap NFTs.
 */
contract PantokenNfts is ERC721, Ownable {
    ...
    constructor(string memory _baseURI) public ERC721("Pantoken Nfts", "PN") {
