...
abstract contract Context {
 ...
 contract SignVerify {
  ...
  contract Berry_Club is ERC721A,Pausable,Ownable,SignVerify{

    uint256 public  PUBLIC_PRICE = 0.0001 ether;
    uint256 public PRE_SALE_PRICE = 1 ether;
    ...
    constructor() ERC721A("Berry_Club", "BRC ",500,10000) {
      InitMinting();
    }
    ...
    function claim_NFT(uint256 _count, uint256 _nonce, bytes memory signature) external payable
    {
      require(preSaleStarted, "MINT_NOT_STARTED");
      require(msg.value >= PRE_SALE_PRICE*_count,"INVALID BNB SENT");
      require(totalSupply() + _count <= collectionSize, "MAX_SUPPLY_REACHED");
    ...
    require(totalSupply() + _count <= collectionSize, "MAX_SUPPLY_REACHED");
    require(msg.value >=(_count * PUBLIC_PRICE), "Invalid_BNB_Sent");
    _safeMint(msg.sender, _count);
    ...
    function ADD_SIGNER(address signer_) external onlyOwner{
      signer = signer_;
    }
