...
// File: LiquidityAndStakingV1.sol
...
contract EmeraldERC721 is Ownable, ERC721{
    using Strings for uint256; 
    ...
    constructor() ERC721("EMERALD NFT", "EM NFT"){
        tokenCounter = 0;
    }

    function Mint(uint256 number) public returns(bool){
        for(uint256 i = 0; i < number; i++){
            tokenCounter += 1;
            _safeMint(msg.sender, tokenCounter);
            
            emit MintToken(msg.sender, tokenCounter);
        }
        return true;
    }
        ...
        onlyOwner
    {
     ...
     function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return uriConcate(beforeUri, tokenId, afterUri);
    }
