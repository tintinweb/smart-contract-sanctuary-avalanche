...
contract NodeManagerV2 is Initializable {
  ...
  function initialize(address[] memory addresses) public initializer {
    ...
    maxMonthValue = 3;
    oldNodeManager = INodeReward(0x0fE753E28292f04975f73CE7F92206B045EEC812);// mainnet 0x05c88F67fa0711b3a76ada2B6f0A2D3a54Fc775c
  ...
  function updateTier(
    ...
    require(tierId > 0, "Old tier's name is incorrect");
    require(keccak256(bytes(tierName)) != keccak256(bytes(name)), "New tier's name is incorrect");
    require(price > 0, "Tier's price has to be positive");
    require(rewardsPerTime > 0, "Tier's rewards has to be positive");
  ...
  function removeTier(string memory tierName) virtual public onlyOwner {
  ...
  function _claim() private {
    ...
    if(claimableAmount > 0) {
      ...
      oldRewardsOfUser[msg.sender] += claimableAmount;
    }
    for(uint32 i = 0;i<count;i++) {
  ...
  function compound(
    ...
    uint256 amount = _create(msg.sender, tierName, count);
    _claim();
    require(oldRewardsOfUser[msg.sender] >= amount, "Insufficient claimable tokens to compound.");
  ...
  function claim() public {
    ...
    _transferFee(oldRewardsOfUser[msg.sender]);
  ...
  function pay(uint8 count, uint256[] memory selected) public payable {
    require(count > 0 && count <= maxMonthValue, "Invalid number of months");
    uint256 fee = 0;
      ...
      for (uint32 i = 0; i < nodeIndice.length; i++) {
        ...
        if (nodeIndex > 0) {
          ...
          if (node.owner == msg.sender) {
            ...
            uint8 pM = uint8((node.limitedTime - block.timestamp ) / payInterval);
            require(pM <= 3, "Cannot pay more than 3 months");
            fee += getAmountManteinance((tier.price * 3000) / 10000);
          }
        }
      }
    } else {
  ...
  function migrateNodesFromOldVersion() public {
    require(!userMigrated[msg.sender], "Wallet nodes have been migrated");
    uint256 nodeNumber = uint256(oldNodeManager._getNodeNumberOf(msg.sender));

    if (nodeNumber != 0 && !userMigrated[msg.sender]) {
      userMigrated[msg.sender] = true;
      _create(msg.sender,"basic", uint32(nodeNumber));
    }
  }

  function migrateRewardsFromOldVersion() public {
    require(userMigrated[msg.sender], "Wallet nodes havent been migrated");
    require(oldRewardsOfUser[msg.sender] == 0, "Wallet reward have been migrated");
    oldRewardsOfUser[msg.sender] = oldNodeManager._getRewardAmountOf(msg.sender);
  }

  function getAmountManteinance(uint256 _amount) public view returns (uint256) {
    if(address(uniswapV2Router)==address(0)) return 0;
    address[] memory path = new address[](2);
    path[0] = address(tokenAddress);
    path[1] = uniswapV2Router.WAVAX();
    uint256[] memory amountsOut = uniswapV2Router.getAmountsOut(_amount, path);
    return amountsOut[1];
  }
  ...
  function _iterate(address _account, uint8 _tierId, uint32 _count) private view returns (uint256, uint32, uint256[] memory) {
    ...
    address nodeOwner;
    uint8 nodeTierIndex;
    uint32 nodeClaimedTime;
    uint32 count = 0;
    for (uint32 i = 0; i < nodeIndice.length; i++) {
      uint256 nodeIndex = nodeIndice[i] - 1;
      if (nodeIndex > 0) {
        nodeOwner = nodesTotal[nodeIndex].owner;
        nodeTierIndex = nodesTotal[nodeIndex].tierIndex;
        nodeClaimedTime = nodesTotal[nodeIndex].claimedTime;
        
        if (_tierId!=0 && nodeTierIndex!=_tierId-1) continue;
        if (nodeOwner == _account) {
          uint256 multiplier = getBoostRate(nodeOwner, nodeClaimedTime, block.timestamp);          
          Tier storage tier = tierArr[nodeTierIndex];
          claimableAmount = uint256(block.timestamp - nodeClaimedTime)
            * tier.rewardsPerTime
            * multiplier
            / 1 ether
            / tier.claimInterval
            + claimableAmount;
          nodeIndiceResult[count] = i;
          count ++;
