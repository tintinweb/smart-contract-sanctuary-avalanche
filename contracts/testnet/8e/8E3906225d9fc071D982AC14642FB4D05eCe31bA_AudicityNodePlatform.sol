...
contract AudicityNodePlatform is OwnableUpgradeable {
    ...
    function payNodeFee(uint256 nodeId, uint256 advancedPayMonths) external payable
    {
        ...
        uint8 digits = _numDigits(int(usdPayAmount));
        uint256 avaxPayAmount = (usdPayAmount * (10 ** (17-digits))) / getAvaxUsdLatestPrice();
        uint8 avaxPayAmountDigits = _numDigits(int(avaxPayAmount));
        uint256 avaxAmount = avaxPayAmount * (10 ** (18 - avaxPayAmountDigits));
        require(msg.value >= avaxAmount, "Wrong payment");
        uint256 returnAmt = msg.value - avaxAmount;
        treasuryAddress.transfer(avaxAmount);
        if (returnAmt > 0) {
    ...
    function claimSpecificNodeDistribution(uint256 nodeId) public {
       IERC20Upgradeable(communityTokenAddress).transferFrom(distributionPoolAddress, msg.sender, _claimNodeDistribution(nodeId, msg.sender));
    }
    ...
    function claimAllDistributions() public {
        ...
        for (uint256 index = 0; index < nodeIds.length; index++) {
            ...
            distribution += _claimNodeDistribution(nodeIds[index], user);
        }
    ...
    function compoundDistributions(uint256 nodeAmount) public {
        ...
        if (distribution < compoundRequiredPayment) revert();
        distribution -= compoundRequiredPayment;
        _purchaseNode(nodeAmount, true);
        IERC20Upgradeable(communityTokenAddress).transfer(user, distribution);
    }

    function convertPreSaleNodes(address userAddress, uint256 nodeAmount) external onlyOwner
    {
