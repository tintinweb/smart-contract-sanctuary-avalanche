...
import "./implementations/KingsManagerImplementationPointerUpgradeable.sol";
...
contract WalletObserverUpgradeable is
    ...
    KingsManagerImplementationPointerUpgradeable
{
    ...
    function changeGardenImplementation(address newImplementation)
        public
        virtual
        override(GardenImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeGardenImplementation(newImplementation);
        excludeFromObserver(garden.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(kingsManager),
            "WalletObserver: Cannot send directly tokens to kingsManager"
        );
        require(
            to != address(liquidityPoolManager),
            "WalletObserver: Cannot send directly tokens to liquidityPoolManager"
        );
        require(
            to != address(garden),
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= garden.balanceOf(to) + amount,
                "WalletObserver: Cannot transfer to this wallet, it would exceed the limit per wallet."
            );

            // Revert if receiving wallet exceed daily limit
            require(
                getRemainingTransfersIn(to) >= 0,
                "WalletObserver: Cannot transfer to this wallet for this timeframe, it would exceed the limit per timeframe."
            );
        }
        ...
        if (!isExcludedFromObserver(from)) {
            ...
            if (isSellOrLiquidityAdd) {
                require(
                    ...
                    "WalletObserver: Cannot sell from this wallet for this timeframe, it would exceed the limit per timeframe."
                );
            } else {
                require(
                    getRemainingTransfersOut(from) >= 0,
                    "WalletObserver: Cannot transfer out from this wallet for this timeframe, it would exceed the limit per timeframe."
                );
            }
        }
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (garden.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return garden.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the garden team to know if
...
import "../../contracts/interfaces/IKingsManager.sol";

abstract contract KingsManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IKingsManager internal kingsManager;

    event UpdateKingsManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyKingsManager() {
        require(
            address(kingsManager) != address(0),
            "Implementations: KingsManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(kingsManager),
            "Implementations: Not KingsManager"
        );
        _;
    }

    function getKingsManagerImplementation() public view returns (address) {
        return address(kingsManager);
    }

    function changeKingsManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(kingsManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "KingsManager: You can only set 0x0 or a contract address as a new implementation"
        );
        kingsManager = IKingsManager(newImplementation);
        emit UpdateKingsManager(oldImplementation, newImplementation);
    }
