...
contract CertificateController is CertificateControllerMock {
  ...
  constructor(address _certificateSigner, bool activated)  CertificateControllerMock(_certificateSigner, activated) {}
...
interface IMarginLoan {
    ...
    // event PayLoan(address user, uint256 id,uint256 _amount, address _bank, LoanStatus status);
   ...
   function getRemainingLoanLimit( address _user,address _tokenAddress, uint256 _loanPercentage) view external returns ( uint256);
    // function payLoan(address _bank, uint256 _amount, uint256 _loanId) external  returns (uint256);
...
contract ERC1400Dividends {
    ...
    function distributeDividends(
      ...
      uint256 totalSupply
        ...
        //uint256 _totalSupply = _IERC1400raw.totalSupply();
        ...
        for (i = 0; i < investors.length; i++) {
          ...
          if(value>=1){
                ...
                ) = _marginLoan.getTotalLoanOfToken(investors[i], address(this));

                for (j = 0; j < banks.length; j++) {
                   
                    IERC20(_token).transferFrom( //transferDividends
                        msg.sender,
                        banks[j],
                        interestRates[j]
                    );
                    emit Dist2(interestRates[j], "Bank interest");
                    emit DstributeDividends(
                        address(_IERC1400raw), //token Address
                        msg.sender, // from
                        investors[i], //To
                        interestRates[j], //No of amounts
                        "Dividends transfered"
                    );
                    userValue = userValue.sub(interestRates[j]);
                }
                if(userValue >= 1){
                    IERC20(_token).transferFrom(msg.sender, investors[i], userValue); //transferDividends
                    emit DstributeDividends(
                     ...
                     emit Dist(userValue,investors[i], "User amount");
...
abstract contract ERC1400RawIssuable is ERC1400Raw, MinterRole {
    ...
    function issue(
        ...
        uint256 value,
        bytes calldata data
    ) external isValidCertificate(data) onlyAdmin returns (bool) {
