...
contract MCrossBrdigeNFT is IAxelarExecutable, Ownable, IMCrossBridgeNFT {
    ...
    function addMockNFTAddress(address _address) public onlyOwner {
        mockNFt = _address;
    }
    ...
    function _execute(
        ...
        address sender;
        uint256 tokenId;
        (sender, tokenId) = abi.decode(payload, (address, uint256));

        IMockCollection(mockNFt).mint(sender, tokenId);

        // if (
        //     keccak256(bytes(SourceChainName)) != keccak256(bytes(sourceChain))
        // ) {
        //     IMockCollection(mockNFt).mint(sender, tokenId);
        // }
    }
