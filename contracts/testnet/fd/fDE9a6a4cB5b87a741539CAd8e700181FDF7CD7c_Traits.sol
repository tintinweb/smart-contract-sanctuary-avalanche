...
contract Traits is Controllable {
    ...
    function uploadRangerTraits(
        ...
        for (uint256 i; i < _traitTypes.length; ++i) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    '<image x="5" y="2" width="64" height="64" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
                    trait.png,
                    '"/>'
                )
            );
