...
contract Eatereum is IEatereum, ERC20, Pausable, Ownable, ERC20Permit, Lockable, MinterRole, BusinessAddresses {
    constructor() ERC20("Eatereum", "EAT") ERC20Permit("Eatereum") {
        _setMinterStatus(0x57EF6F871888e2d294470A0061292675F6dC309c, true);
    }
    ...
    function burn(bytes32 burnId_, uint256 amount_) external {
        uint256 burnId = burnId_.toUint256();
        address sender = _msgSender();
        if (_inBusinessList(sender)) revert BusinessAddresses__NotAuthorized();
        if (_isUsed.get(burnId)) revert IToken__AlreadyBurnt();
        _isUsed.setTo(burnId, true);
        _burn(sender, amount_);
        emit Burn(burnId_, amount_);
    }
...
interface IEatereum {
    error IToken__AlreadyBurnt();

    event Burn(bytes32 burnId, uint256 amount);
