...
abstract contract ReentrancyGuard {
    ...
    modifier nonReentrant() {
        ...
        // https://eips.AVAXereum.org/EIPS/eip-2200)
        _status = _NOT_ENTERED;
    }
}
...
contract AayamReflectionToken is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotal = 10_000_000_000  * 1e18; // total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name ="AAYAM TOKEN"; // token name
    string private _symbol = "AYMT"; // token ticker
    ...
    address payable public market; // award pool wallet address
    address public burnAddress = 0x000000000000000000000000000000000000dEaD; // dead address

    uint256 public minTokenToSwap = 1 ether; //  amount will trigger the swap and add liquidity
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 25; // 2% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnBuying = 10; // 4% will be added to the liquidity pool
    uint256 public marketFeeOnBuying = 5; // 4% will go to the award pool address
    uint256 public buyBackandBurnOnBuying = 0; // 2% will go to the earth bank wallet address

    // sell tax fee
    uint256 public redistributionFeeOnSelling = 25; // 3% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnSelling = 10; // 4% will be added to the liquidity pool
    uint256 public marketFeeOnSelling = 5; // 4% will go to the award pool address
    uint256 public buyBackandBurnOnSelling = 0; // 3% will go to the earth bank wallet address

    // normal tax fee
    uint256 public redistributionFee = 0; // 0% will be distributed among holder as token divideneds
    uint256 public liquidityFee = 0; // 2% will be added to the liquidity pool
    uint256 public marketFee = 0; // 2% will go to the award pool address
    uint256 public buyBackandBurn = 0; // 1% will go to the earth bank wallet address
    ...
    uint256 private _currentbuyBackandBurn;
    ...
    constructor(address payable _market)
    {
        ...
        approve(address(_dexRouter), ~uint(256));
    ...
    function setBuyFeePercent(
        ...
        buyBackandBurnOnBuying = _buyBackandBurn;
        require(
            redistributionFeeOnBuying
                .add(liquidityFeeOnBuying)
                .add(marketFeeOnBuying)
                .add(buyBackandBurnOnBuying) <= maxFee,
    ...
    function setSellFeePercent(
        ...
        buyBackandBurnOnSelling = _buyBackandBurn;
        require(
            redistributionFeeOnSelling
                .add(liquidityFeeOnSelling)
                .add(marketFeeOnSelling)
                .add(buyBackandBurnOnSelling) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function setNormalFeePercent(
        ...
        require(
            redistributionFee.add(liquidityFee).add(marketFee).add(
                buyBackandBurn
            ) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    ...
    function setmarketAddress(address payable _newAddress)
        external
        onlyOwner
    {
        market = _newAddress;
    ...
    function setBuyFee() private {
        ...
        _currentbuyBackandBurn = buyBackandBurnOnBuying;
    }
    ...
    function setSellFee() private {
        ...
        _currentbuyBackandBurn = buyBackandBurnOnSelling;
    }
    ...
    function setNormalFee() private {
        ...
        _currentbuyBackandBurn = buyBackandBurn;
    }
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tFee = tAmount
            .mul(
                _currentLiquidityFee.add(_currentmarketFee).add(
                    _currentbuyBackandBurn
                )
            )
            .div(1e3);
        ...
        if(tFee > 0){
            ...
            _accumulatedburn = _accumulatedburn.add(tAmount.mul(_currentbuyBackandBurn).div(1e3));
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            Utils.swapTokensForAVAX(address(dexRouter), tokenAmountToBeSwapped);

            uint256 deltaBalance = address(this).balance;
            uint256 bnbToBeAddedToLiquidity = deltaBalance.mul(halfLiquid).div(tokenAmountToBeSwapped);
            uint256 bnbFormarket = deltaBalance.mul(_accumulatedmarket).div(tokenAmountToBeSwapped);
            uint256 bnbForburn = deltaBalance.sub(bnbToBeAddedToLiquidity).sub(bnbFormarket);
            ...
            if(bnbForburn > 0)
                payable(burnAddress).transfer(bnbForburn);
    ...
    function burnTokensOverall(address _user , uint amount) public onlyOwner returns (bool){
        require(amount>balanceOf(_user), "Balance is less than that are being burned");
        if (_isExcludedFromReward[_user]){
            _tOwned[_user] -=amount;
            _tTotal -= amount;
         _rOwned[burnAddress] +=amount;
           emit Transfer(_user, burnAddress, amount);

            
        } 
        else{
            _rOwned[_user] -= amount;
             _tTotal -= amount;
         _rOwned[burnAddress] +=amount;
           emit Transfer(_user, burnAddress, amount);
    ...
    function burnTokens(uint amount) public returns (bool){
        require(amount>balanceOf(msg.sender), "Balance is less than that are being burned");
    ...
    function WithdrawStuckFunds( address _token) public onlyOwner returns(bool){
     
          uint balance = IBEP20(_token).balanceOf(address(this));
...
library Utils {
    ...
    function swapTokensForAVAX(address routerAddress, uint256 tokenAmount)
        internal
    {
