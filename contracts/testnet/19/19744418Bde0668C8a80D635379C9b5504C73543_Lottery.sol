...
contract Lottery is Ownable, Pausable, PullPayment, ReentrancyGuard {
	...
	uint256 public dividentsFee = 1000; //10%
	...
	uint256 private _dividentsDeposits; //dividents deposit storage
	mapping(address => uint256) public withdrawedDividentsTime; //keeps track when dividents were taken

	bool public isDividentsWithdrawalActive = false;
	uint256 public dividentsInterval = 30 days;
	...
	function delBuildNumber6() internal pure {} //etherscan trick
		...
		nonReentrant
	{
		...
		if (affiliateAddress != address(0)) {
			...
			//if the user didn't set an affiliate, transfer the ammount to nonAffiliateWallet
			payable(nonAffiliateWallet).transfer(affiliateReward);
		}

		uint256 insuranceBuyBackAmount = _calcPercentage(ticketPrice, insuranceBuyBackFee); //15%
		payable(insuranceBuyBackWallet).transfer(insuranceBuyBackAmount);
		...
		currentRound.endTime = currentRound.endTime + newTicketStep[phase]; //increase the endtime with the ticket step
		currentRound.leader = _msgSender();

		emit TicketBought(_msgSender());
	}

	function withdrawDividents() external notContract whenNotPaused nonReentrant {
		require(isDividentsWithdrawalActive, "not active");
		require(
			withdrawedDividentsTime[_msgSender()] + dividentsInterval < block.timestamp,
			"already claimed in interval"
		);
		uint256 dividents = getDividentsForAddress(_msgSender());
		withdrawedDividentsTime[_msgSender()] = block.timestamp;
		_asyncTransfer(_msgSender(), dividents);
	}

	//calculates how much dividents an address has
	function getDividentsForAddress(address addr) public view returns (uint256) {
		return (boughtTickets[addr] * _dividentsDeposits) / totalTicketsBought;
	}
	...
	function setCurrentPhase() internal {
		if (ticketsBoughtThisRound > phaseMaxTickets[0] && phase < 2) {
			phase = 2;
			rounds[currentRoundIndex].endTime = block.timestamp + roundMaxTime[phase]; //set one time for each case...
		}
		if (ticketsBoughtThisRound > phaseMaxTickets[1] && phase < 3) {
			phase = 3;
			rounds[currentRoundIndex].endTime = block.timestamp + roundMaxTime[phase];
		}
	...
	function getWinnerEarnings() public view returns (uint256) {
		return _calcPercentage(address(this).balance - _dividentsDeposits, prizeFee);
	}
	...
	function setRoundMaxTime(
		uint256 newRoundTimePhase1,
		uint256 newRoundTimePhase2,
		uint256 newRoundTimePhase3,
		uint256 newRoundTimePhase4,
		uint256 newRoundTimePhase5,
		uint256 newRoundTimePhase6
	) public onlyOwner {
		roundMaxTime[1] = newRoundTimePhase1;
		roundMaxTime[2] = newRoundTimePhase2;
		roundMaxTime[3] = newRoundTimePhase3;
		roundMaxTime[4] = newRoundTimePhase4;
		roundMaxTime[5] = newRoundTimePhase5;
		roundMaxTime[6] = newRoundTimePhase6;
	}
	...
	function setPhaseMaxTickets(
		...
		require(maxTicketsPhase1 < maxTicketsPhase2, "incorrect values");
		require(maxTicketsPhase2 < maxTicketsPhase3, "incorrect values");
		require(maxTicketsPhase3 < maxTicketsPhase4, "incorrect values");
		require(maxTicketsPhase4 < maxTicketsPhase5, "incorrect values");
		require(maxTicketsPhase5 < maxTicketsPhase6, "incorrect values");
	...
	// enable/disable claiming for dividents
	function setDividentsSettings(bool isActive, uint256 intervalInSeconds) public onlyOwner {
		isDividentsWithdrawalActive = isActive;
		dividentsInterval = intervalInSeconds;
	}
	...
	// withdrawal to owner of some funds (used for testing or stuck dividents)
	...
	function distributeEarnings() internal {
		uint256 dividentsAmount = _calcPercentage(address(this).balance, dividentsFee); //10%
		_dividentsDeposits = _dividentsDeposits + dividentsAmount;

		//substract the dividents deposits
		uint256 balanceToDistribute = address(this).balance;
		balanceToDistribute = balanceToDistribute - _dividentsDeposits;

		uint256 winnerAmount = _calcPercentage(balanceToDistribute, prizeFee); //50%

		//announce winner
		Round memory currentRound = rounds[currentRoundIndex];
		emit NewWinner(currentRound.leader, winnerAmount, currentRoundIndex);

		//transfer winnings to multisig
		payable(multisigWallet).transfer(winnerAmount);

		//everything else, rollback (including thet dividents deposits)
	}
