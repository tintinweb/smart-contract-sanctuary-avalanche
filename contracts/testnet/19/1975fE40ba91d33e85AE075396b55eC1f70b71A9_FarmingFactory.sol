...
contract Farming is OwnableUpgradeable {
    ...
    function _safeTransfer(IERC20Upgradeable token_, address to_, uint256 amount_) internal {
...
contract FarmingFactory is Ownable {
     ...
     */
    function countRewardAmount(uint256 start_, uint256 end_, address farmingAddress) external view returns (uint256) {
        uint256 reward = distributionV2.countRewardAmount(start_, end_);
        uint farmingReward = reward.mul(farmingAllocationPoint[farmingAddress]).div(totalFarmingAllocationPoint);
