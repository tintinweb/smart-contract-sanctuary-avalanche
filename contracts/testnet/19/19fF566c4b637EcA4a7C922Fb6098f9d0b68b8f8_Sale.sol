...
contract Sale is SaleToken, Ownable {
    ...
    constructor(
        ...
        require(farmLensV2Address_ != address(0), "Sale::constructor: cannot set to zero address");
    ...
    function initialize(
        ...
        require(isInitialized == false, "Sale::initialize: Sale was already initialized");
    ...
    function claimToken(uint256 periodId_) external onlyInitialized {
        ...
        require(block.timestamp >= _distributionPeriods[periodId_].timestamp, "Sale::claimToken: Claim date is not arrived");
    ...
    function availableAllocationInDepositToken(address depositAddress, address participant) external view returns (uint256) {
        ...
        return addressAvailableAllocationInSellingToken.mul(tokenPriceInUSD).div(10**sellingTokenDecimals).mul(1e18).div(depositTokenPriceInUSD).div(10**depositTokenDecimalsDiff);
    }
    ...
    function _permitLotteryAllocation(
        ...
        if (_signatory == address(0) || signatory != _signatory || block.timestamp > deadline || participant != msg.sender) {
            return false;
        }
