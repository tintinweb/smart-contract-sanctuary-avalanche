...
contract Treasury is Ownable { 
    ...
    address private coinFlip;
    mapping(uint16 => address) private activeCoins;

    constructor(address _coinFlip) {   
        coinFlip = _coinFlip;
        activeCoins[0] = address(0);
    }

    function activateToken(uint16 id, address _address) external { 
        require(msg.sender == coinFlip, "noAuth");
        activeCoins[id] = _address;
    }
    ...
    function sendPayout(address gamer, uint256 amount, uint16 id) external {
        ...
        if(id == 0) { 
            ...
            address tokenAddress = activeCoins[id];
            if (tokenAddress != address(0)) {
                IERC20 token = IERC20(tokenAddress);
                if(token.balanceOf(tokenAddress) >= amount) { 
                    token.transfer(gamer, amount);
                }
            }
        }
    }

    function getBalance(uint16 id) external view returns (uint256) {
        if(id == 0) { 
    ...
    function setCoinFlip(address _flip) external onlyOwner {
         coinFlip = _flip;
    }
