...
contract BridgesOfFate is Ownable  {
    ...
    */
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules {
        ...
        emit ParticipateOfPlayerInGame(playerId,_gameStatus.stageNumber,_currentUserStage,GameMap[_currentUserStage],_checkSide(GameMap[_currentUserStage],_jumpSide),_jumpSide);
        PlayerItem[playerId] = _member;
        GameStatusInitialized[latestGameId] = _gameStatus;
   
    }
    event comment (uint256 _randomNumber,bool Greater,bool lesser,uint256 stage);
    function _checkSide(uint256 stageNumber, bool userSide) internal returns (bool)
    {
        uint256 stage_randomNumber = GameMap[stageNumber]; 
        bool _greaterCheck = userSide == true && stage_randomNumber >= 50e9;
        bool _lessCheck = userSide == false && stage_randomNumber < 50e9;
        if (_greaterCheck || _lessCheck) {
            return true;
        }else{
            return false;
        }
        emit comment(stage_randomNumber,_greaterCheck,_lessCheck,stageNumber); 
    }
