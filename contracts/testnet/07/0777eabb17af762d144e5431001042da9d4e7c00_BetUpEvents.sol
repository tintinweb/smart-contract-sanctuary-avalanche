...
contract BetUpEvents is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function close(uint256 _pairID,  uint8 _resultID) external {
    ...
    function stack(uint256 _pairID, uint8 _resultID, uint256 _amount) external {
        Pair memory pair = pairs[_pairID];
    ...
    /*
       ...
       T = total volume
   */
    function claim(uint256 _pairID, uint8 _resultID, uint256 _maxLevel) external {
        Pair memory pair = pairs[_pairID];
        if (pair.optionsQty == 0) revert InvalidPairID(_pairID);
        if(pair.paused) revert PairPaused(_pairID);

        require(block.timestamp > pair.resolutionAt, "ongoing");
        ...
        require(IBetUpSheep(betUpSheep).checkMaxLevel(_maxLevel, sender), "Invalid _maxLevel");
        ...
        if(block.timestamp < deadlineAt && pair.resultID > 0 ) {

            if(pair.resultID == _resultID) {
                uint256 op = optionsVolume[keccak256(abi.encodePacked(_pairID,_resultID))];

                assembly {
                    let _profit := div(mul(mul(9, playerAmount),sub(totalAmount, op)), mul(10, op))
                    refund := add(playerAmount, _profit)
                    reward := div(mul(winPer, playerAmount), exp(10,18))
                }
            }else {
                assembly {
                    reward := div(mul(losePer, playerAmount), exp(10,18))
                }
            }

        }else if(block.timestamp > deadlineAt && pair.resultID == 0) {
            assembly {
                refund := playerAmount
            }
        }
    ...
    function withdrawFee(uint256 _pairID) external {
        Pair memory pair = pairs[_pairID];
    ...
    function setCreateFee(uint256 _createFee ) external onlyRole(GOVERNANCE_ROLE) {
...
//    function setResolverStatus(bytes32 _resolverID) external onlyRole(DISTRIBUTOR) {
//        pairsResolver[_resolverID].claimed = true;
//    }

    function setResultID(uint256 _pairID, uint8 _resultID) external  onlyRole(DEFAULT_ADMIN_ROLE) {
        Pair memory pair = pairs[_pairID];
        require(pair.optionsQty > 0 && block.timestamp > pair.resolutionAt, "Invalid pair");
        pairs[_pairID].resultID = _resultID;
        emit CloseEvent(_pairID, _resultID);
    }
    ...
    function pausedPair(uint256 _pairID) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pairs[_pairID].paused == false, "paused");
        pairs[_pairID].paused = true;
    }
    ...
    function addChips(address[] memory _chips) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for(uint256 i = 0; i < _chips.length; i++) {
            require(validChips[_chips[i]] == 0, "duplicate");
            validChips[_chips[i]] = 1;
        }

        emit AddChipsEvent(_chips);
    }

    function setResolveDeadlineSecond(uint256 _spam) external onlyRole(DEFAULT_ADMIN_ROLE) {
        resolveDeadlineSecond =_spam ;
    }

    function invalidChip(address _chip, uint8 _status) external onlyRole(DEFAULT_ADMIN_ROLE) {
