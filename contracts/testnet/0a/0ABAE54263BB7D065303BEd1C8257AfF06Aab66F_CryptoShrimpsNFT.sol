...
contract CryptoShrimpsNFT is ERC721Enumerable, IERC2981, Ownable, ReentrancyGuard {
  ...
  string public baseURI = "NOT REVEALED YET!";
   ...
   */
   ...
   function ownerMint(uint256 numTokens) external onlyOwner { // Owner mints for free
       require(numTokens > 0, "Need to mint a positive number of tokens");
       require(totalSupply() + numTokens <= TOKEN_LIMIT, "Not enough tokens available to mint");
  ...
  function mintTokens(uint256 numTokens) external payable nonReentrant {
    ...
    if (saleState == 2) {
      // Open sale
      require(totalSupply() + numTokens <= TOKEN_LIMIT, "Cryptoshrimps has sold out");
    } else {
        // presale
