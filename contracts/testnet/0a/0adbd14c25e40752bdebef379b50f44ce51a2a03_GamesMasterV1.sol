...
contract GamesMasterV1 is Ownable, ReentrancyGuard {
    ...
    function gothBalance () view external returns (uint256)
    {
        return _players[msg.sender].gothBalance;
    }

    function addGoth (uint256 amount) external nonReentrant
    {
        ...
        _players[msg.sender].gothBalance.add(amount);
        emit AddGoth(msg.sender, amount);
    }

    function removeGoth (uint256 amount) external nonReentrant
    {
        Player memory player = _players[msg.sender];
        require(player.gothBalance >= amount, "removeGoth: remove amount too much");
        _gothToken.transfer(msg.sender, amount);
        _players[msg.sender].gothBalance.sub(amount);
    ...
    {
        Player memory player = _players[msg.sender];
        require(player.gothBalance >= _powerUpPrice, "buyPowerUp: not enough GOTH in game wallet");
        require(msg.value > _serviceFee, "buyPowerUp: not enough avax for service fee");

        _players[msg.sender].gothBalance.sub(_powerUpPrice);
