...
contract Smelter is Constants, RefinementBase, VRFConsumerV2Ownable {
    mapping(uint256 => Request) PlayerRequest;

    GearNFT immutable GearNft;
    mapping(uint256 => bytes32) recycleOutput;

    event Claimed(address user, uint256 jobId);
    event RecyclingStarted(uint256 tokenId, uint256 landTokenId, uint256 slotId);
    event RecyclingFinalized(uint256 jobId);
    ...
    constructor(
        ...
        RefinementBase(_smolApaLandAddress, _feeAggregator, _rawMaterialNftContract, _landRegistery, SmelterType)
        VRFConsumerV2Ownable(subscriptionId)
    {
        GearNft = GearNFT(_gearNft);
    }
    ...
    function claim(uint256 _landTokenId, uint256 _slotId) external override {
        ...
        if (job.gearType != 0) {
            finalizeRecycling(job.gearType);
            emit RecyclingFinalized(jobId);
        } else if (level != 10) {
    ...
    function begincRecycling(uint256 _gearNFTId, uint256 _landTokenId, uint256 _slotId) external payable {
        (uint256 level, uint256 landType) = LandRegistryContract.LandInfoRegistry(_landTokenId);
        if (landType != LandType) {
            revert WrongLand();
        }

        if (GearNft.ownerOf(_gearNFTId) != msg.sender) {
        ...
        uint256 jobId = _landTokenId * 100 + _slotId;
        if (JobsDictionary[msg.sender][jobId].completedTimeStamp > block.timestamp) {
            revert TooSoon();
        }
        handleFees(_landTokenId);
        ...
        bytes32 gearInfo = GearNft.GearInfo(_gearNFTId);
        uint16 gearType = uint16(gearInfo[1] | (bytes2(gearInfo[2]) >> 8));
        
        JobsDictionary[msg.sender][jobId] = Job(uint32(completedTimeStamp), uint32(_gearNFTId), gearType);
        GearNft.burnGear(_gearNFTId);
        emit RecyclingStarted(_gearNFTId, _landTokenId, _slotId);
    }

    function finalizeRecycling(uint256 _gearType) internal {
        bytes32 recycleInfo = GearNft.RecycleInfo(_gearType);
        (uint256[] memory types, uint256[] memory amounts) = getMaterialsFromRecipe(recycleInfo);
        RawMaterialNftContract.batchMintRawMaterial(types, amounts, msg.sender);
    }
}
