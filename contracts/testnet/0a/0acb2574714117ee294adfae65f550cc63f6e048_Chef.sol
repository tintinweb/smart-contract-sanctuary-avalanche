...
// File: contracts/deps/libs/Strings.sol
...
// File: contracts/deps/libs/Address.sol
...
// File: contracts/deps/ERC721Pausable.sol
...
// View
    function isTiki(uint tokenId) external view returns(bool) {
        require(revealed, "!reveal");
        return(_isTiki[tokenId]);
    }
    function numTikis() external view returns(uint256) {
        require(revealed, "!reveal");
        return _numTikis;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        if(!revealed) {
        ...
        else if(_isTiki[tokenId]) {
            uint index;
            for(uint i; i < tikiIndex.length; i++) {
            ...
            uint index;
            for(uint i; i < humanIndex.length; i++) {
    ...
    function setURIs(string calldata _human, string calldata _tiki) external onlyOwner {
        humanURI = _human;
        tikiURI = _tiki;
    }
    // --------------------------------------------------------PUBLIC--------------------------------------------------------
    ...
    function mint(uint qty) external payable {
        ...
        require(qty + totalSupply() <= MAX_SUPPLY, "maxSupply"); 
        userMinted[msg.sender] += uint96(qty);
        // For tests only
    ...
    function retrieveRequest(bytes32 requestId) internal view returns(address user, uint qty) {
        ...
        qty = mintParams[requestId].mintQty;
    }
        ...
        do {
            ...
            for(uint i; i < 16; i++) {
                ...
                if(!_exists(tokenId)) {
                    ...
                    // Reentrancy point here, check concequences. 
                    _safeMint(to, tokenId);
                    ++currentSupply;
                    ++minted;
                    if(minted == qty) {
