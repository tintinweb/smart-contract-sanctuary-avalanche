...
contract DAOMemberStorage is IDAOMemberStorage, Initializable, AbstractDependant {
    ...
    function checkPermission(
        ...
        return _permissionManager.hasPermission(member_, DAO_MEMBER_STORAGE_RESOURCE, permission_);
    }

    function addMember(address member_) external override onlyCreatePermission {
        _members.add(member_);

        string memory role_ = string.concat("MemberOf", targetPanel);
        _permissionManager.grantRoles(member_, role_.asArray());
    }

    function addMembers(address[] calldata members_) external override onlyCreatePermission {
        for (uint256 i = 0; i < members_.length; i++) {
            _members.add(members_[i]);

            string memory role_ = string.concat("MemberOf", targetPanel);
            _permissionManager.grantRoles(members_[i], role_.asArray());
        }
    }
    ...
    function removeMember(address member_) external override onlyDeletePermission {
        ...
        string memory role_ = string.concat("MemberOf", targetPanel);
        _permissionManager.revokeRoles(member_, role_.asArray());
    }
    ...
    function _requirePermission(string memory permission_) internal view {
        require(
            _permissionManager.hasPermission(msg.sender, DAO_MEMBER_STORAGE_RESOURCE, permission_),
            "DAOMemberStorage: access denied"
        );
    }
}
