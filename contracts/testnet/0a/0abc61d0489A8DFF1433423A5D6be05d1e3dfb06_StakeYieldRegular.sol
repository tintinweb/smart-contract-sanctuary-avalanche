...
interface IYKRegular is IValiFiYK {
...
interface IMaintFee {
    ...
    function setResetMaintenanceFee(uint256 _yieldKey, uint256 _yieldKeyType) external;
}

contract StakeYieldRegular is
    ...
    PausableUpgradeable
{
    ...
    // Instance of ValiFi Yield Key Regular
    IYKRegular public valifiYKR;
    IMaintFee public maintFee;
    // Mapping of Stake Yield Keys Regular
    ...
    /**
     * @dev Mapping for storage Yield Keys Regular Staked
    ...
    function initialize(
        address yielKeyRegular,
        string memory _initURI
    ) public initializer {
        __ERC721_init_unchained("Staking Yield Key Regular", "STKReg");
        __Ownable_init();
        __Pausable_init();
        valifiYKR = IYKRegular(yielKeyRegular);
        setBaseURI(_initURI);
    }
     ...
     * @param wallet address of Yield Key Regular
     * @param _tokenId Token Id of Yield Key Regular
     */
    ...
    /**
     * @dev This method to Staked Yield Key Regular
     * @param wallet address of Yield Key Regular
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                valifiYKR.ownerOf(tokenIds[i]) == _msgSender(),
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            valifiYKR.safeTransferFrom(wallet, address(this), tokenIds[i]);
            ...
            stakedYieldKeys[wallet][tokenIds[i]].endstake.push(uint64(0));
						_capacityAmount[i] = valifiYKR.capacityAmount(tokenIds[i]);
        }
        emit StakedYieldKey(wallet, tokenIds, _capacityAmount);
    }

    /**
     * @dev This method to unStaked Yield Key Regular
     * @param wallet address of Yield Key Regular
     * @param tokenIds Array of Token Id of Yield Key to Staked
     */
        ...
        whenNotPaused
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _burn(tokenIds[i]);
            valifiYKR.safeTransferFrom(address(this), wallet, tokenIds[i]);
            // Reset Maintenance Fee
            maintFee.setResetMaintenanceFee(tokenIds[i], valifiYKR.capacityAmount(tokenIds[i]));
						_capacityAmount[i] = valifiYKR.capacityAmount(tokenIds[i]);
        }
    ...
    /**
     * @dev Get Historical of Staked Yield Key Regular
     * @param wallet address of Yield Key Regular
     * @param tokenId Token Id of Yield Key Regular
     */
    ...
    /**
     * @dev Verify if the address is owner of one or several Yield-Keys Regular
     * @param _wallet Wallet address of the owner
     * @return tokenIds Array of token Ids of Yield-Keys Regular owned by the wallet
     */
    ...
    /**
     * @dev Verify is the address is owner of one or several Yield-Keys Regular
     * @param _owner address to owner of the tokens
     * @return stakeTokenIds tokenIds Array of token Ids of Yield-Keys Regular who the _owner address is the owner
     */
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !paused(),
            "ERC721 VALIFi: can't staked or transfer any YieldKey Regular while paused"
        );
        if (from != address(0)) {
            require(
