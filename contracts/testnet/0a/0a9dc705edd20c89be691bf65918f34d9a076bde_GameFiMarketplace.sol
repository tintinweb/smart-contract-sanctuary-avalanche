    ...
    ERC721HolderUpgradeable
{
    ...
    modifier onlyAdmin() {
        IGameFiCore(_gameFiCore).isAdmin(msg.sender);
        _;
    }

    function initialize(address gameFiCore, address settlementToken, uint256 newTradeFeePercentage) external override initializer {
        require(gameFiCore.isContract(), "GameFiMarketplace: gameficore be a contract");
        require(settlementToken.isContract(), "GameFiMarketplace: token must be a contract");
    ...
    function createOrder(
        ...
        require(nftStandart != NFTStandart.NULL, "GameFiMarketplace: invalid nft standart");
        require(tokenContract != address(0), "GameFiMarketplace: zero contract address");
        if (nftStandart == NFTStandart.ERC721) {
            require(_whitelistErc721.contains(tokenContract), "GameFiMarketplace: nft not registered");
            require(tokenAmount == 1, "GameFiMarketplace: wrong token amount");
        } else {
            require(_whitelistErc1155.contains(tokenContract), "GameFiMarketplace: nft not registered");
            require(tokenAmount >= 1, "GameFiMarketplace: wrong token amount");
        }
        require(mainSettlementToken != address(0), "GameFiMarketplace: zero trade token address");
        require(_settlementTokens.contains(mainSettlementToken), "GameFiMarketplace: settlement token not registered");
        require(otherSettlementTokens.length == 0, "GameFiMarketplace: only main settlement token");
        require(orderPrice > 0.0001 ether, "GameFiMarketplace: wrong order price");
    ...
    function cancelOrder(uint256 orderId) external override nonReentrant returns (bool success) {
        require(orderId < _totalOrders.current(), "GameFiMarketplace: order does not exist");

        Order memory order = _orders[orderId];

        require(order.status == OrderStatus.OPEN, "GameFiMarketplace: only for open orders");
        require(order.seller == msg.sender, "GameFiMarketplace: sender is not the seller");
        require(order.buyer == address(0), "GameFiMarketplace: order already completed");
    ...
    function executeOrder(uint256 orderId) external override nonReentrant returns (bool success) {
        ...
        require(order.status == OrderStatus.OPEN, "GameFiMarketplace: only for open orders");
        require(order.seller != msg.sender, "GameFiMarketplace: not for seller");
        require(order.buyer == address(0), "GameFiMarketplace: order already completed");
        ...
        IERC20Upgradeable(order.mainSettlementToken).safeTransferFrom(msg.sender, address(this), order.orderPrice);
        ...
        emit ExecuteOrder(msg.sender, orderId, fee, block.timestamp);
    ...
    function setTradeFeePercentage(uint256 newTradeFeePercentage)
        external
        override
        nonReentrant
        onlyAdmin
    ...
    function addSettlementToken(address erc20) external override nonReentrant onlyAdmin returns (bool success) {
        require(erc20.isContract(), "GameFiMarketplace: token must be a contract");
        _settlementTokens.add(erc20);
        return (true);
    }

    function removeSettlementToken(address erc20) external override nonReentrant onlyAdmin returns (bool success) {
    ...
    function addToWhitelistErc721(address erc721) external override nonReentrant onlyAdmin returns (bool success) {
        _whitelistErc721.add(erc721);
        return (true);
    }

    function addToWhitelistErc1155(address erc1155) external override nonReentrant onlyAdmin returns (bool success) {
        _whitelistErc1155.add(erc1155);
        return (true);
    }

    function removeFromWhitelistErc721(address erc721) external override nonReentrant onlyAdmin returns (bool success) {
        _whitelistErc721.remove(erc721);
        return (true);
    }

    function removeFromWhitelistErc1155(address erc1155)
        external
        override
        nonReentrant
        onlyAdmin
    ...
    function _setTradeFeePercentage(uint256 newTradeFeePercentage) internal {
        require(newTradeFeePercentage >= 0 && newTradeFeePercentage < 100_00, "GameFiMarketplace: wrong percent value");
...
interface IGameFiMarketplace {
    ...
    event CancelOrder(address indexed user, uint256 indexed orderId, uint256 timestamp);
    event ExecuteOrder(address indexed user, uint256 indexed orderId, uint256 fee, uint256 timestamp);
    event WithdrawERC20(address indexed admin, address indexed tokenContract, uint256 amount, uint256 timestamp);

    function initialize(address gameFiCore, address settlementToken, uint256 newTradeFeePercentage) external;
    ...
    function setTradeFeePercentage(uint256 newTradeFeePercentage) external returns (bool success);
