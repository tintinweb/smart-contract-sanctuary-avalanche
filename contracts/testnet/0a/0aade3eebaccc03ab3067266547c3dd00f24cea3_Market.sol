...
interface IMarketExtendedCore is IMarketCommon {
  ...
  function changeMarketFundingRateMultiplier(uint256) external;
}
...
contract MarketTieredLeverageLogic is AccessControlledAndUpgradeableModifiers, IMarketCommon, IMarketTieredLeverageCore, MarketStorage {
  ...
  function _packPoolId(IMarketCommon.PoolType poolType, uint8 poolTier) internal pure virtual returns (uint8) {
  ...
  struct ValueChangeAndFunding {
    ...
    IMarketCommon.PoolType underBalancedSide;
  ...
  function _getValueChangeAndFundingTranche(
    ...
    uint256 floatPoolLiquidity = pools[IMarketCommon.PoolType.FLOAT][0].value;
    ...
    if (effectiveValueShort > effectiveValueLong) {
      rebalanceParams.fundingAmount = -int256(_calculateFundingAmount(fundingRateMultiplier_e18, effectiveValueShort, effectiveValueLong));
      rebalanceParams.valueChange =
        ...
        1e18;
      rebalanceParams.underBalancedSide = IMarketCommon.PoolType.LONG;
    } else {
      rebalanceParams.fundingAmount = int256(_calculateFundingAmount(fundingRateMultiplier_e18, effectiveValueLong, effectiveValueShort));
      rebalanceParams.valueChange =
        (priceMovement * int256(effectiveValueShort + ((uint256(-rebalanceParams.floatPoolLeverage) * floatPoolLiquidity) / 1e18))) /
        1e18;
      rebalanceParams.underBalancedSide = IMarketCommon.PoolType.SHORT;
    }
  }
  ...
  function _rebalancePoolsAndExecuteBatchedActionsTranche(
    ...
    totalEffectiveLiquidityPoolType[uint8(rebalanceParams.underBalancedSide)] += uint128(
      (uint256(pools[IMarketCommon.PoolType.FLOAT][0].value) * SignedMath.abs(int256(rebalanceParams.floatPoolLeverage))) / 1e18
    );
    ...
    for (uint8 poolType = uint8(IMarketCommon.PoolType.SHORT); poolType <= uint8(IMarketCommon.PoolType.FLOAT); poolType++) {
      for (uint8 poolTier = 0; poolTier < _numberOfPoolsOfType[poolType]; poolTier++) {
        ...
        if (IMarketCommon.PoolType(poolType) != IMarketCommon.PoolType.FLOAT) {
          poolValue += (((poolValue * poolFixedConfig.leverage) *
            (rebalanceParams.valueChange -
              (IMarketCommon.PoolType(poolType) != rebalanceParams.underBalancedSide ? rebalanceParams.fundingAmount : int256(0)))) /
          ...
          poolValue +=
            ((poolValue * rebalanceParams.floatPoolLeverage * rebalanceParams.valueChange) /
              (int256(uint256(totalEffectiveLiquidityPoolType[uint256(rebalanceParams.underBalancedSide)]) * 1e18))) +
            int256(SignedMath.abs(rebalanceParams.fundingAmount));
        }
        ...
        poolStates[currentPoolStateIndex] = PoolState({
          poolId: _packPoolId(IMarketCommon.PoolType(poolType), poolTier),
  ...
  function updateSystemStateUsingValidatedOracleRoundIds(uint80[] memory oracleRoundIdsToExecute) external virtual checkMarketNotDeprecated {
    ...
    for (uint256 i = 0; i < numberOfEpochsToExecute; i++) {
      ...
      (totalEffectiveLiquidityPoolType, poolStates) = _rebalancePoolsAndExecuteBatchedActionsTranche(
        // TODO: optimize reads on `epochInfo.latestExecutedEpochIndex`
      ...
      emit SystemUpdateInfo(epochInfo.latestExecutedEpochIndex + 1 + uint32(i), uint256(epochPrices[i]), poolStates);
    }
    ...
    checkMarketNotDeprecated
  {
    ...
    emit Deposit(_packPoolId(poolType, uint8(poolTier)), amount, user, currentEpoch);
  }
  ...
  function _redeem(
    ...
    emit Redeem(_packPoolId(poolType, uint8(poolTier)), uint256(amount), user, currentEpoch);
  }
  ...
  function settlePoolUserMints(
    ...
    */
    ...
    if (userAction.correspondingEpoch != 0 && userAction.correspondingEpoch <= epochInfo.latestExecutedEpochIndex) {
      ...
      emit ExecuteEpochSettlementMintUser(_packPoolId(poolType, uint8(poolTier)), user, epochInfo.latestExecutedEpochIndex);
    }
  }
  ...
  function settlePoolUserRedeems(
    ...
    if (userAction.amount > 0 && userAction.correspondingEpoch <= epochInfo.latestExecutedEpochIndex) {
      ...
      emit ExecuteEpochSettlementRedeemUser(_packPoolId(poolType, uint8(poolTier)), user, epochInfo.latestExecutedEpochIndex);
    }
  }
...
contract MarketExtendedCore is AccessControlledAndUpgradeableModifiers, MarketStorage, IMarketExtendedCore {
  ...
  function initializePools(IMarketExtended.InitializePoolsParams memory params) external override initializer returns (bool initializationSuccess) {
  ...
  function changeMarketFundingRateMultiplier(uint256 _fundingRateMultiplier_e18) external adminOnly {
    require(_fundingRateMultiplier_e18 <= 5e19, "funding rate must be <= 5000%");
