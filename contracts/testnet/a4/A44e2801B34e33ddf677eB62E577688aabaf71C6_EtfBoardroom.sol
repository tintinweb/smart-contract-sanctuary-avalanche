...
contract EtfBoardroom is ShareWrapperEtf, ContractGuard {
    ...
    function initialize(
        ...
        additionalRewardLastDevRewardTime = treasury.additionalRewardLastDevRewardTime();
        additionalRewardLastDaoRewardTime = treasury.additionalRewardLastDaoRewardTime();
    ...
    function pendingDev(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (treasury.isDevWallet(_user)) {
            ...
            if (_toTime >= additionalRewardPoolEndTime) {
                ...
                if (_fromTime <= additionalRewardPoolStartTime) return additionalRewardPoolEndTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDev);
                return additionalRewardPoolEndTime.sub(_fromTime).mul(aoeaTokenPerSecondForDev);
            } else {
                if (_toTime <= additionalRewardPoolStartTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) return _toTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDev);
                return _toTime.sub(_fromTime).mul(aoeaTokenPerSecondForDev);
            }
        }
    ...
    function pendingDao(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (treasury.isDaoWallet(_user)) {
            ...
            if (_toTime >= additionalRewardPoolEndTime) {
                if (_fromTime >= additionalRewardPoolEndTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) return additionalRewardPoolEndTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDao);
                return additionalRewardPoolEndTime.sub(_fromTime).mul(aoeaTokenPerSecondForDao);
            } else {
                if (_toTime <= additionalRewardPoolStartTime) return 0;
                if (_fromTime <= additionalRewardPoolStartTime) return _toTime.sub(additionalRewardPoolStartTime).mul(aoeaTokenPerSecondForDao);
                return _toTime.sub(_fromTime).mul(aoeaTokenPerSecondForDao);
            }
        }
...
interface ITreasury {
    ...
    function additionalRewardLastDevRewardTime() external view returns (uint256);

    function additionalRewardLastDaoRewardTime() external view returns (uint256);
