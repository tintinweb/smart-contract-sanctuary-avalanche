...
contract anotherone is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	address private aiswalletAddress;
	...
	constructor(string memory _initUnrevealedURI, address _initaiswalletAddress) ERC721("anotherone", "ANO") {
		paused = true;
		revealed = false;
		allowListings = true;
		...
		aiswalletAddress = _initaiswalletAddress;
	...
	function mint(uint256 amount) public payable nonReentrant {
		...
		uint256 proceeds_percent = proceeds * 100 / 10000;

		(bool success1, ) = payable(aiswalletAddress).call{ value: proceeds_percent * 700 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to aiswallet wallet!");

		(bool success2, ) = payable(owner()).call{ value: proceeds_percent * 300 }("");
	...
	function withdraw() public onlyOwner {
		...
		uint256 balance_fraction = address(this).balance * 100/100000;

		(bool success1, ) = payable(aiswalletAddress).call{ value: balance_fraction * 400 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to aiswallet wallet!");
	...
	function fulfillListing(uint256 tokenID) public payable nonReentrant {
		...
		(bool success1, ) = payable(listing.seller).call{ value: msg.value - royaltiesValue }("");
		require(success1, "AVAX Transaction: Failed to transfer funds");

		(bool success2, ) = payable(owner()).call{ value: royaltiesValue }("");
		require(success2, "AVAX Transaction: Failed to transfer funds to owner address");
	}
    ...
    function setWithdrawAddresses(address newaiswalletAddress) public onlyOwner {
		aiswalletAddress = newaiswalletAddress;
	}
	...
	function withdrawWavax() public onlyOwner { 
		...
		wavaxContract.transfer(aiswalletAddress, balance_fraction * 500);
		wavaxContract.transfer(owner(), balance_fraction * 500);
    }
	...
	receive() external payable {
		...
		(bool success1, ) = payable(aiswalletAddress).call{ value: fraction * 500 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to aiswallet wallet!");

		(bool success2, ) = payable(owner()).call{ value: fraction * 500 }("");
		require(success2, "AVAX Transaction: Failed to transfer funds to owner wallet!");
