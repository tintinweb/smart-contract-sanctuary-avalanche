...
contract BondingTreasuryV2 is OwnableUpgradeable, BondingTreasuryStorageV2 {
    ...
    function withdraw(uint256 _bondId) external virtual returns (bool) {
        ...
        EmeraldToken.mintSupply(msg.sender, BondDetailsById[_bondId].amount);
    ...
    function withdrawAll() external virtual returns (bool) {
        ...
        EmeraldToken.mintSupply(msg.sender, tempAmt);
    ...
    function checkTreasuryValueInUSD() external view returns(uint256 TreasuryPriceInUSD){
        uint256 price = uint256(getEmToUSDPrice());

        TreasuryPriceInUSD = (price * EmeraldToken.balanceOf(treasuryAccount)) / (10**18);
    }
