...
contract Dodo is ERC721, Ownable {
    string public baseURI;
    uint256 public nextTokenID;
    uint256 public MAX_MINT_PER_ADDRESS = 5;
    uint256 public constant TOTAL_SUPPLY = 10000;
    mapping(address => uint256) public mintCount;
    mapping(uint256 => uint256) private shuffleMap;
    ...
    function mint(address _receiver, uint256 mintNumber) external {
        if (mintCount[msg.sender] >= MAX_MINT_PER_ADDRESS)
            revert ExceededMintCount();

        if ((mintCount[msg.sender] + mintNumber) >= MAX_MINT_PER_ADDRESS)
            revert ExceededMintCount();

        uint256 seed = enoughRandom();
        uint256 tokenID = 0;
        uint256 currentIndex = TOTAL_SUPPLY - nextTokenID;
        for (uint256 i = 0; i < mintNumber; i++) {
            seed = seed >> 1;
            uint256 index = seed % currentIndex;

            if (shuffleMap[index] == 0) {
                tokenID = index;
            } else if (shuffleMap[index] != 0) {
                tokenID = shuffleMap[index];
            }
            shuffleMap[index] = currentIndex;
            unchecked {
                nextTokenID++;
