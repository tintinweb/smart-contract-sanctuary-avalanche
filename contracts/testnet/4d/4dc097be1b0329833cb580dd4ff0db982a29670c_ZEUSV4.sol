...
contract NODERewardManagement {
    ...
    function createNode(
        ...
        uint32 nodeType,
        uint256 total
    ) public onlySentry returns(string memory){}
...
contract ZEUSV4 is Ownable, PaymentSplitter {
    ...
    function _getLastNodeId(address sender, uint32 nodeType) view internal returns(string memory){
        NODERewardManagement nodeRewardManager = getNodeRewardManager(nodeType);
         string memory lastcreationTime = nodeRewardManager.getLastCreationTime(sender);
        string memory userId = nodeRewardManager.getNodeId(sender);
