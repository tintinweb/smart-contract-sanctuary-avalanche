...
contract GothStake is ERC20("GOTH Bits", "bGOTH"), Ownable, ReentrancyGuard {
    ...
    // Minting
    uint256 private mintRateConstant = 317097919837640000000;
    uint256 private startTimeStamp;
    uint256 private lastMintTimeStamp;
    uint256 private decayRate = 31705770450;

    mapping(address => uint256) public lastStakeShare;

    //EVENTS
    event ChangeGothOwner (address indexed newOwner);
    ...
    {
        require(newFeeMod <= 1e9, "setEnterFeeMod: enter fee mod too high");
    ...
    {
        require(newLeaveFee <= 5e16, "setLeaveFee: leave fee too high");
    ...
    {
        require(newMintRate <= 5e20, "setMintRate: mint rate too high");
    ...
    function getLastStakeShare (address account) public view returns (uint256)
    {
        return lastStakeShare[account];
    }

    function calculateMintRate () public view returns (uint256)
    {
        uint256 secondsElapsed = block.timestamp.sub(startTimeStamp);      
        return mintRateConstant.sub(decayRate.mul(secondsElapsed));
    }

    function mintGOTH () internal 
    {
        uint256 rate = calculateMintRate();
        uint256 secondsElapsed = block.timestamp.sub(lastMintTimeStamp);
        uint256 toMint = rate.mul(secondsElapsed);

        if (goth.totalSupply() <= goth.maxSupply().sub(toMint))
        {
            uint256 poolPercent = 1000 - treasuryPercent - teamPercent;

            uint256 toPool = toMint.mul(poolPercent).div(1000);
            uint256 toTreasury = toMint.mul(treasuryPercent).div(1000);
            uint256 toTeam = toMint.mul(teamPercent).div(1000);

            goth.mint(address(this), toPool);
            goth.mint(treasuryAddress, toTreasury);
            goth.mint(teamAddress, toTeam);
    ...
    function enter(uint256 amount) public nonReentrant payable
    {
        ...
        {
            uint256 toMint = amount.mul(totalShares).div(totalGoth);
        ...
        lastStakeShare[msg.sender] = share.mul(goth.balanceOf(address(this))).div(totalSupply()); 
    ...
    function leave(uint256 _share) public nonReentrant payable
    {
        ...
        {
            uint256 totalShares = totalSupply();
            uint256 toSend = _share.mul(goth.balanceOf(address(this))).div(totalShares);
            
            _burn(msg.sender, _share);
            goth.transfer(msg.sender, toSend);

            uint256 share = balanceOf(msg.sender);
            uint256 currentStake = share.mul(goth.balanceOf(address(this))).div(totalShares); 
            lastStakeShare[msg.sender] = currentStake;

            emit Leave(msg.sender, _share);
        }
    }

    function gather (address account) internal 
    {
        uint256 totalShares = totalSupply();
        uint256 share = balanceOf(account);
        uint256 currentStake = share.mul(goth.balanceOf(address(this))).div(totalShares); 
        uint256 toGather = currentStake.sub(lastStakeShare[account]);
        goth.transfer(account, toGather);
        lastStakeShare[account] = currentStake.add(toGather);
        emit Gather(account, toGather);
    }
}