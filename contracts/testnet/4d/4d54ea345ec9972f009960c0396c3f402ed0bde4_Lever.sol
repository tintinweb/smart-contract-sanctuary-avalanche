...
contract Router is Ownable {
    ...
    event RouteSet(address _fromToken, address _toToken, Node[] path);
    event Swap(address caller, address _startingTokenAddress, address _endingTokenAddress, uint256 _amount, uint256 _minSwapAmount, uint256 actualOut);
    ...
    uint256 internal constant feeCompliment = 997;
    ...
    struct Node {
        ...
        address tokenIn;
        address tokenOut;
        uint256 _misc; //Extra info for curve pools
        int128 _in;
        int128 _out;
    }
    ...
    function setJoeRouter(address _traderJoeRouter) onlyOwner public {
        traderJoeRouter = _traderJoeRouter;
    }
    function setAAVE(address _aaveLendingPool) onlyOwner public {
        aaveLendingPool = _aaveLendingPool;
    }

    function setApprovals(address _token, address _who, uint256 _amount) onlyOwner external {
        IERC20(_token).approve(_who, _amount);
    }

    function setRoute(address _fromToken, address _toToken, Node[] calldata _path) onlyOwner external {
    ...
    //////////////////////////////////////////////////////////////////////////////////
    function swapJoePair(address _pair, address _tokenIn, uint256 amountIn) internal returns (address, uint256) {
        ...
        if (_tokenIn < _tokenOut) {
            // TokenIn=token0
            (amount0Out, amount1Out)=(uint256(0),getAmountOut(amountIn, reserve0, reserve1));
        } else {
            // TokenIn=token1
            (amount0Out, amount1Out)=(getAmountOut(amountIn, reserve1, reserve0), uint256(0));
        }
        IJoePair(_pair).swap(
            amount0Out, amount1Out, address(this), new bytes(0)
        );
        return (_tokenOut, amount0Out!=0 ? amount0Out : amount1Out);
    }
    ...
    function getAmountIn(uint256 amountOut, uint256 reserveIn, uint256 reserveOut) internal pure returns (uint256 amountIn) {
        ...
        uint256 denominator = (reserveOut-amountOut)*feeCompliment;
    ...
    function _getAmtToSwap(uint256 r0, uint256 totalX) internal pure returns (uint256) {
        ...
        // Plug in the uniswap k=xy equation in the above equality and you will get the following:
    ...
    function _getAmountPairOut(
        ...
        amountOut = min((_amountIn*_totalSupply)/(_reserveIn + _amountIn), (_amountIn2*_totalSupply)/(_reserveOut - _amountIn2));
    }
    ...
    function _getAmountPairIn(
        ...
        uint256 amountInWithFee = amt0*feeCompliment;
        uint256 numerator = amountInWithFee*_reserveOut;
        uint256 denominator = (_reserveIn*FEE_DENOMINATOR) + amountInWithFee;
        amountOut = numerator/denominator;
    ...
    function swapLPToken(address _token, address _pair, uint256 _amountIn, bool LPIn) internal returns (address, uint256) {
        address token0 = IJoePair(_pair).token0();
        address token1 = IJoePair(_pair).token1();
        if (LPIn) {            
            IJoeRouter(traderJoeRouter).removeLiquidity(token0, token1, _amountIn, 0, 0, address(this), block.timestamp + 10 seconds);
            if (token0 == _token) {
                swapJoePair(_pair, token1, IERC20(token1).balanceOf(address(this)));
            } else if (token1 == _token) {
                swapJoePair(_pair, token0, IERC20(token0).balanceOf(address(this)));
            } else {
                revert("_tokenOut is not a token in the pair");
            }
            return (_token, IERC20(_token).balanceOf(address(this)));
        } else {
            (uint112 r0, uint112 r1, uint32 _last) = IJoePair(_pair).getReserves();
            if (token0 == _token) {
                swapJoePair(_pair, _token, _getAmtToSwap(r0, _amountIn));
                IJoeRouter(traderJoeRouter).addLiquidity(token0, token1, IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this)), 0, 0, address(this), block.timestamp + 10 seconds);
            } else if (token1 == _token) {
                swapJoePair(_pair, _token, _getAmtToSwap(r1, _amountIn));
                IJoeRouter(traderJoeRouter).addLiquidity(token0, token1, IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this)), 0, 0, address(this), block.timestamp + 10 seconds);
            } else {
                revert("_tokenOut is not a token in the pair");
            }
            return (_pair, IERC20(_pair).balanceOf(address(this)));
        }
    }
    ...
    function swapCurve2(address _tokenIn, address _tokenOut, address _curvePool, uint256 _amount, uint256 _misc, int128 _in, int128 _out) internal returns (address tokenOut, uint256 amountOut) {
        tokenOut = _tokenOut;
        if (_misc == 1) {
            // Plain pool
            if (_out == -1) {
                ...
                amountOut =  IPlainPool(_curvePool).remove_liquidity_one_coin(_amount, _out, 0);
            } else {
                amountOut = IPlainPool(_curvePool).exchange(_in, _out, _amount, 0);
            }
        } else if (_misc == 2) {
            ...
            if (_out == -1) {
                uint256[2] memory _amounts;
                _amounts[uint256(int256(_in))] = _amount;
                amountOut = ILendingPool(_curvePool).add_liquidity(_amounts, 0, true);
            } else {
                amountOut = ILendingPool(_curvePool).exchange_underlying(_in, _out, _amount, 0);
            }
        }
    }
    //////////////////////////////////////////////////////////////////////////////////
    ...
    function swapCurve3(address _tokenIn, address _tokenOut, address _curvePool, uint256 _amount, uint256 _misc, int128 _in, int128 _out) internal returns (address tokenOut, uint256 amountOut) {
        tokenOut = _tokenOut;
        if (_misc == 1) {
            // Plain pool
            if (_out == -1) {
    ...
    function swapCurve4(address _tokenIn, address _tokenOut, address _curvePool, uint256 _amount, uint256 _misc, int128 _in, int128 _out) internal returns (address tokenOut, uint256 amountOut) {
        tokenOut = _tokenOut;
        if (_misc == 1) {
            // Plain pool
            if (_out == -1) {
    ...
    function swapAAVEToken(address _token, uint256 _amount, bool AaveIn) internal returns (address, uint256) {
        if (AaveIn) {
            // Swap Aave for _token
            _amount = IAAVE(aaveLendingPool).withdraw(_token, _amount, address(this));
            return (_token, _amount);
        } else {
            // Swap _token for Aave
            IAAVE(aaveLendingPool).deposit(_token, _amount, address(this), 0);
            return (address(0), _amount);
        }
    }
    ...
    function swapCOMPToken(address _tokenIn, address _cToken, uint256 _amount) internal returns (address, uint256) {
        if (_tokenIn == _cToken) {
            // Swap ctoken for _token
            ICOMP(_cToken).redeem(_amount);
            return (ICOMP(_cToken).underlying(), IERC20(ICOMP(_cToken).underlying()).balanceOf(address(this)));
        } else {
            // Swap _token for ctoken
            ICOMP(_cToken).mint(_amount);
            return (_cToken, _amount);
        }
    }
    ...
    function swap(
        ...
        uint256 initialOutAmount = IERC20(_endingTokenAddress).balanceOf(address(this));
        ...
        for (uint256 i; i < path.length; i++) {
            if (path[i].nodeType==1) {
                // Is traderjoe
                (, _amount) = swapJoePair(path[i].addy, path[i].tokenIn, _amount);
            } else if (path[i].nodeType==2) {
                // Is jlp
                if (path[i].tokenIn == path[i].addy) {
                    (, _amount) = swapLPToken(path[i].tokenOut, path[i].addy, _amount, true);
                } else {
                    (, _amount) = swapLPToken(path[i].tokenIn, path[i].addy, _amount, false);
                }
            } else if (path[i].nodeType==3) {
                // Is curve pool
                (, _amount) = swapCurve2(path[i].tokenIn, path[i].tokenOut, path[i].addy, _amount, path[i]._misc, path[i]._in, path[i]._out);
            } else if (path[i].nodeType==4) {
                // Is curve pool
                (, _amount) = swapCurve3(path[i].tokenIn, path[i].tokenOut, path[i].addy, _amount, path[i]._misc, path[i]._in, path[i]._out);
            } else if (path[i].nodeType==5) {
                // Is curve pool
                (, _amount) = swapCurve4(path[i].tokenIn, path[i].tokenOut, path[i].addy, _amount, path[i]._misc, path[i]._in, path[i]._out);
            } else if (path[i].nodeType==6) {
                // Is aToken
                (, _amount) = swapAAVEToken(path[i].tokenIn == path[i].addy ? path[i].tokenOut : path[i].tokenIn, _amount, path[i].tokenIn == path[i].addy);
            } else if (path[i].nodeType==7) {
                // Is cToken
                (, _amount) = swapCOMPToken(path[i].tokenIn, path[i].addy, _amount);
            } else {
                revert("Unknown node type");
            }
        }
        uint256 outAmount = IERC20(_endingTokenAddress).balanceOf(address(this))-initialOutAmount;
        require(
            outAmount >= _minSwapAmount,
            "Did not receive enough tokens to account for slippage"
        );
        emit Swap(msg.sender, _startingTokenAddress, _endingTokenAddress, amtIn, _minSwapAmount, outAmount);
        return outAmount;
    }
}
...
contract Lever is Router {
    ...
    function route(
        ...
        _amountOut = swap(_startingTokenAddress, _endingTokenAddress, _amount, _minSwapAmount);
        IERC20(_endingTokenAddress).transfer(to, _amountOut);
    }
    ...
    function unRoute(
        ...
        // IERC20(_startingTokenAddress).transferFrom(_fromUser, address(this), _amount);
        _amountOut = swap(_startingTokenAddress, _endingTokenAddress, _amount, _minSwapAmount);
        IERC20(_endingTokenAddress).transfer(msg.sender, _amountOut);
    }
