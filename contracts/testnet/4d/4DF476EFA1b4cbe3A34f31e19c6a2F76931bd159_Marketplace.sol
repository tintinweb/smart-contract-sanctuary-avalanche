...
contract Marketplace is Ownable {
  ...
  mapping(ERC721A => NFTStructure[]) public itemsOnMarket;
  mapping(address => uint256) public royaltyFees;
  mapping(ERC721A => mapping(uint256 => bool)) public deployedCollectionItems;

  event DeployedItemForSale(ERC721A collection, uint256 id, uint256 tokenId, address minter, uint256 price);
  event ItemSold(ERC721A collection, uint256 id, uint256 tokenId, address minter, address buyer, uint256 price);
  event UpdateItem(ERC721A collection, uint256 id, uint256 tokenId, bool isForSale, uint256 price);
  event AddCollection(ERC721A collection);
  event RemoveCollection(ERC721A collection);

  constructor(ERC20 paymentToken_, ERC721A collection_) {
    collections.push(collection_);
    paymentToken = paymentToken_;
  }

  modifier CollectionExists(ERC721A collection_){
    bool _isReg;
  ...
  function buyItem(ERC721A collection_, uint256 id_)
    ...
    external {
      ...
      paymentToken.transferFrom(address(msg.sender), address(_ownerOfItem), _price);
  ...
  function updateItem(ERC721A collection_, uint256 id_, bool isForSale_, uint256 newPrice_)
    ItemExists(collection_, id_)
    public {
      uint256 _tokenId = itemsOnMarket[collection_][id_].tokenId;
  ...
  function amountItemsForSale(ERC721A collection_) external view returns(uint256) {
    return itemsOnMarket[collection_].length;
  }

  function addCollection(ERC721A collection_)
    onlyOwner
    external {
      bool _isReg;
      for(uint i = 0 ; i < collections.length ; i++) {
  ...
  function removeCollection(ERC721A collection_) public onlyOwner {
