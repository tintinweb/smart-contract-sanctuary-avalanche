...
contract WalrusFarm is Ownable, IERC20 {
    using SafeMath for uint256;

    // name and symbol for tokenized contract
    string private constant _name = "WSHARE LP Farm";
    string private constant _symbol = "WSHARE FARM";
    ...
    function stake(uint256 amount) external {
        ...
        userInfo[msg.sender].unlockTimestamp = block.timestamp + lockTime;
        userInfo[msg.sender].totalExcluded = getCumulativeDividends(
            userInfo[msg.sender].amount
        );
    ...
    function timeUntilUnlock(address user) public view returns (uint256) {
        return
            userInfo[user].unlockTimestamp < block.timestamp
                ? 0
                : userInfo[user].unlockTimestamp - block.timestamp;
    }
