...
contract MultiCollateralSynthLightChain is Synth {
    ...
    constructor(
        ...
        uint256 _totalSupply,
        address _resolver
    ) Synth(_tokenState, _tokenName, _tokenSymbol, _owner, _currencyKey, _totalSupply, _resolver) {}
...
contract Synth is Owned, ExternStateToken, MixinResolver {
    ...
    constructor(
        ...
        uint256 _totalSupply,
        address _resolver
    ) ExternStateToken(_tokenState, _tokenName, _tokenSymbol, _totalSupply, DECIMALS, _owner) MixinResolver(_resolver) {
    ...
    function resolverAddressesRequired() public view virtual override returns (bytes32[] memory addresses) {
        ...
        addresses[3] = CONTRACT_SYNTHR_BRIDGE;
    }
    ...
    function _isInternalContract(address account) internal view virtual returns (bool) {
        return account == address(exchanger()) || account == address(issuer());
    }
