...
// File: contracts/GalixERC20HotWallet.sol
...
/**
 * @title GALIX ERC20 HotWallet
 * @author GALIX Inc
 */
contract GalixERC20HotWallet is
    ContextUpgradeable,
    OwnableUpgradeable,
    ...
    EIP712Upgradeable
{
    ...
    function initialize(
        ...
        __EIP712_init_unchained("GalixERC20HotWalletEIP712", "1.0.0");
    ...
    function _transfer(address _account, address _to, uint256 _amount) internal {
        uint256 _balance = token.balanceOf(_account);
        require(_balance >= _amount, "_transfer balance not enough");
    ...
    function _withdrawHash(address account, uint256 amount, uint256 deadline)
    ...
    //lp view
    function lpWithdraw(uint256 _amount) public {
        require(_amount > 0, "charge amount should be more than 0");
        require(_msgSender() == liquidityProviderAddress, "you are not liquidity Provider");
        _transfer(address(this), liquidityProviderAddress, _amount);
    }
    ...
    function withdraw(
        ...
        require(block.timestamp <= deadline, "withdraw: Expired transaction");
        require(_verify(_withdrawHash(account, amount, deadline), signature), "withdraw: Invalid signature");
        nonces[account]++;
    ...
    function lpDecimals() public view virtual returns (uint8) {
