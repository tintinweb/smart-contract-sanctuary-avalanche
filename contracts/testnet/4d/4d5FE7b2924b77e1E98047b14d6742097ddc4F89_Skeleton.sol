...
contract Skeleton is ERC721Enumerable, Pausable, Restricted {
    ...
    constructor(address _glow, address _traits) ERC721("caca", "caca") { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ("Yield Hunt - Chapter 2", "???") for real launch 
        setGlow(_glow);
        setTraits(_traits);
        ...
        notBlacklisted
    {
        ...
        for (uint256 i; i < tokenIds.length;) {
            ...
            if (advhtr.isAdventurer) {
                unchecked{++nbAdventurerSent;}
            } else {
                unchecked{++nbHunterSent;}
            }
            unchecked{++i;}
        }
    ...
    function settleOrders(uint8 amount)
        ...
        notBlacklisted
    {
        _manageOrders(amount, 0, address(0x0));
    }
        ...
        notBlacklisted
    {
        ...
        for (uint256 i; i < tokenIds.length;) {
            ...
            if (ownerOf(tokenIds[i]) == address(magicForest) && magicForest.getStaker(tokenIds[i]) == _msgSender()) {
    ...
    function _manageOrders(uint256 amountToSettle, uint256 amountToPlace, address buyer) internal{
        ...
        if (amountToPlace > 0) {
            ...
            emit onOrderPlaced(buyer, amountToPlace);
        }
    }
    ...
    function core_mint(Order memory order) internal returns (uint256 seed) {
        ...
        if (tokenTraits[ordersSettled].isRanger) {
            unchecked{rangerMinted++;}
            if (recipient != order.buyer) {
                unchecked{rangerStolen++;}
            }
        } else {
            unchecked{skeletonMinted++;}
            if (recipient != order.buyer) {
                unchecked{skeletonStolen++;}
            }
        }
