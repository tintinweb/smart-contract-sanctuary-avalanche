...
contract CertificateController {
    ...
    modifier isValidCertificate(bytes memory data) {
        if (_certificateControllerActivated && !_iwhitelist.isSuperAdmin(msg.sender)) {
    ...
    //  event Log(bytes4,uint256,address,address,bytes,address);
    /**
    ...
    function _checkCert(bytes memory data) internal view returns (bool) {
        ...
        if (v == 27 || v == 28) {
            ...
            // emit Log(funcId,uint256(amount),to,_signer,msg.data,msg.sender);
    ...
    function toggleCertificateController(bool _isActive) public onlyAKRUAdmin {
        _certificateControllerActivated = _isActive;
    }
...
contract ERC1820Client {
    ...
    function setInterfaceImplementation(string memory _interfaceLabel, address _implementation) internal {
        console.log("Set Implementor is calling");
        bytes32 interfaceHash = keccak256(abi.encodePacked(_interfaceLabel));
        console.log(address(this));
        console.log("address(this), interfaceHash, _implementation", _implementation);
        console.log("interfaceHash", _interfaceLabel);
...
contract ERC1400RawERC20 is IERC20, ERC1400RawIssuable {
 ...
 /**
   ...
   function updateFromExchange(address _investor, uint256 _balance) public onlyAdmin returns(bool) {
    ...
    function addPropertyOwnerReserveDevTokens(uint256 _propertyOwnerReserveDevTokens) public onlyAdmin {
        propertyOwnerReserveDevTokens = _propertyOwnerReserveDevTokens;
    }
