...
contract Game {
    ...
    constructor(
        ...
        //        ceoAddress1=address(0x4E52A9498e3C2d48E12160558C7b9Ef45966CfA8);
        //        ceoAddress2=address(0x608551c8436CbaA3Bc98C7Eb74e21072275a5712);
        tgc = _token;
        ceoAddress1 = _ceo1;
        ceoAddress2 = _ceo2;
    }

    function changeCeoAddress(address _ceo1, address _ceo2) external {
    ...
    function harvestFishes(address ref) public {
        ...
        catchFishes[msg.sender] = SafeMath.add(
            catchFishes[msg.sender],
            newPrinters
        );
        claimedMoneys[msg.sender] = 0;
        lastClaim[msg.sender] = now;

        claimedMoneys[referrals[msg.sender]] = SafeMath.add(
            claimedMoneys[referrals[msg.sender]],
            SafeMath.div(printerUsed, _refferal)
        );
    ...
    function buyFisherman(address ref, uint256 amount) public {
        ...
        uint256 fishermanBought = calculatePrinterBuy(
            amount,
            SafeMath.sub(balance, amount)
        );
        fishermanBought = SafeMath.sub(
            fishermanBought,
            devFee(fishermanBought, devamount)
        );
        ...
        ERC20(tgc).transfer(ceoAddress1, fee2);
        ERC20(tgc).transfer(ceoAddress2, fee2);
        claimedMoneys[msg.sender] = SafeMath.add(
            claimedMoneys[msg.sender],
            fishermanBought
        );
        harvestFishes(ref);
    }
    ...
    function calculateMoneyClaim(uint256 printers)
        public
        view
        returns (uint256)
    {
        return
            calculateTrade(
                printers,
                marketFishes,
                ERC20(tgc).balanceOf(address(this))
            );
    }

    function calculatePrinterBuy(uint256 eth, uint256 contractBalance)
        public
        view
        returns (uint256)
    {
    ...
    function calculatePrinterBuySimple(uint256 eth)
        public
        view
        returns (uint256)
    {
    ...
    function devFee(uint256 amount, uint256 devamount2)
        public
        pure
        returns (uint256)
    {
    ...
    function getMyFish() public view returns (uint256) {
        return
            SafeMath.add(
                claimedMoneys[msg.sender],
                getFishesSinceLastCatch(msg.sender)
            );
    }

    function getFishesSinceLastCatch(address adr)
        public
        view
        returns (uint256)
    {
        uint256 secondsPassed = min(
            FISH_TO_CATCH_1,
            SafeMath.sub(now, lastClaim[adr])
        );
