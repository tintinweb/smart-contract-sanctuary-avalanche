...
import "../superToken/lendingAaveToken/lendingAaveErc20.sol";
import "../modules/debugger.sol";
// Debug version contract.
//
// This contract is used for debugging lendingAaveErc20. Please see lendingAaveErc20.
contract lendingAaveErc20Debug is lendingAaveErc20,debugger {
    ...
    address payable _swapHelper,address payable _feePool,address leverageFactory)
        lendingAaveErc20(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_feePool,leverageFactory){
...
abstract contract superTokenInterface{
    ...
    function getRate()external virtual view returns(uint256);
...
abstract contract superAaveToken is baseSuperToken {
    ...
    address payable _swapHelper,address payable _feePool)
        ...
        //_setReward(0,0,false,0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7,1e15);//wavax
    }
    ...
    function compound() override external{
        ...
        claimReward();
        aaveSuply(feeRate[compoundFeeID]);
    }
...
pragma solidity ^0.7.0;
import "./superAaveToken.sol";
// superToken is the coolest vault in town. You come in with some token, and leave with more! The longer you stay, the more token you get.
//
// This contract handles swapping to and from superToken.
contract superAaveErc20 is superAaveToken {
    using SafeERC20 for IERC20;
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
    address payable _swapHelper,address payable _feePool)
        superAaveToken(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_feePool) {
    ...
    function deposit(address account,uint256 _amount)internal virtual override returns(uint256){
        ...
        return aaveSuply(feeRate[enterFeeID]);
    }
...
pragma solidity ^0.7.0;
import "../superAaveToken/superAaveErc20.sol";
...
contract lendingAaveErc20 is superAaveErc20,lendingSuperToken {
    using SafeMath for uint256;
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
    address payable _swapHelper,address payable _feePool,address leverageFactory)
        superAaveErc20(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_feePool) 
        lendingSuperToken(leverageFactory) {

    }
    function availableBalance() public virtual override(superAaveToken,superTokenInterface) view returns (uint256){
        return stakeToken.balanceOf(address(this));
    }
    function totalUnderlying()public virtual override(superAaveToken,superTokenInterface) view returns (uint256){
...
//
// This contract handles swapping to and from superToken.
abstract contract baseSuperToken is timeLockSetting,superTokenInterface,ERC20,proxyOwner,ReentrancyGuard{
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
...
//    uint256 external feeRate = 2e17;    //1e4
//    uint256 external enterFee = 1e15;
//    uint256 external flashFeeRate = 1e16;
    ...
    function getRate()external virtual override view returns(uint256){
    ...
    function acceptSwapHelper() external onlyOrigin {
        ...
        for (uint256 i=0;i<len;i++){
            rewardInfo memory info = rewardInfos[i];
            if(!info.bClosed){
                if(info.rewardToken != address(0) && IERC20(info.rewardToken).allowance(address(this), address(swapHelper)) == 0){
                    SafeERC20.safeApprove(IERC20(info.rewardToken), address(swapHelper), uint(-1));
                }
            }
        }
    }
