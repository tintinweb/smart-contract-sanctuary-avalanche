    ...
    Ownable
{
    ...
    address private _serviceWallet;
    ...
    function createToken(
        ...
        address[] calldata royaltyWallets
    ) public returns (uint) {
        ...
        createListedToken(currentTokenId, royaltyWallets);
    ...
    function listItem(
        ...
        if (!createdByMarketpalce) {
            ...
            idToListedToken[tokenId].RoyaltyWallets = royaltyWallets;
        ...
        nonReentrant
    {
        ...
        idToListedToken[tokenId].seller = address(msg.sender);
        ...
        for (uint i = 0; i < _offeringIds.current(); i++) {
            if (
                offerersData[i + 1].nftAddress == nftAddress &&
                offerersData[i + 1].tokenId == tokenId
            ) {
                uint currentId = i + 1;
                CreateOffering storage currentItem = offerersData[currentId];
                Address.sendValue(
                    payable(offeringRegistry[currentItem.generatedABI].offerer),
                    offeringRegistry[currentItem.generatedABI].bidPrice
                );
        ...
        if (serviceFee > 0 && _serviceWallet != address(0)) {
            ...
            idToListedToken[tokenId].RoyaltyPercentage += fee;
            Address.sendValue(payable(_serviceWallet), fee);
        }

        // split royalty
        if (idToListedToken[tokenId].RoyaltyPercentage > 0 && idToListedToken[tokenId].RoyaltyWallets.length > 1) {
            for (
                uint8 i = 0;
                i < idToListedToken[tokenId].RoyaltyWallets.length;
                i++
            ) {
                fee =
                    msg.value * idToListedToken[tokenId].RoyaltyPercentage /
                    ROYALTY_MAX;
                if (fee > 0) {
                    idToListedToken[tokenId].RoyaltyPercentage += fee;
                    Address.sendValue(
                        payable(idToListedToken[tokenId].RoyaltyWallets[i]),
                        fee
                    );
                }
            }
            ...
            Address.sendValue(
                payable(idToListedToken[tokenId].seller),
                idToListedToken[tokenId].RoyaltyPercentage
            );
        } else {
           idToListedToken[tokenId].RoyaltyPercentage =
                msg.value -
    ...
    function getUserOfferings(address _userAddress)
        public
        view
    ...
    function cancelOffer(uint _nonce) external nonReentrant {
        ...
        payable(offerersData[_nonce].offerer).transfer(
            offerersData[_nonce].bidPrice
        );
    ...
    function acceptOffer(uint _nonce) external nonReentrant {
        ...
        if (nft.supportsInterface(0xd9b67a26) == true) {
            ...
            idToListedToken[currentOffer.tokenId].seller = address(msg.sender);
