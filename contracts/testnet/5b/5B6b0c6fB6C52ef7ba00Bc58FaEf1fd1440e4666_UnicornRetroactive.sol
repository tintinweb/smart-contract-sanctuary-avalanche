...
abstract contract ERC721WithPermitUpgradable is ERC721Upgradeable, IERC4494 {
    /// @dev value is equal to keccak256("RoleManager_v1")
    ...
    function _buildDigest(address spender, uint256 tokenId, uint256 nonce, uint256 deadline) public view returns (bytes32) {
...
error AirdropReceived__Already();

abstract contract AirdropReceivedUpgradeable {
    using Helper for address;
    using BitMaps for BitMaps.BitMap;

    BitMaps.BitMap private _isReceived;

    function isReceived(address account) external view returns (bool) {
        return _isReceived.get(account.toUint256());
    }

    function _notReceived(address user_) internal virtual {
        require(!_isReceived.get(user_.toUint256()), "UnicornRetroactive: already received");
    }

    function _setReceived(address account_, bool status_) internal {
        _isReceived.setTo(account_.toUint256(), status_);
    }
...
import { ERC721WithPermitUpgradable } from "./extensions/ERC721WithPermitUpgradable.sol";
import { AirdropReceivedUpgradeable } from "./internal-upgradeable/AirdropReceivedUpgradeable.sol";
import { Helper } from "./libraries/Helper.sol";
import { BitMaps } from "./libraries/BitMaps.sol";

contract UnicornRetroactive is
    Initializable,
    ERC721Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function safeMint() external {
        address sender = _msgSender();
        _notReceived(sender);
        _setReceived(sender, true);
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId > _limit, "UnicornRetroactive: exceed limit");
    ...
    function setLimit(uint256 limit_) external onlyRole(OPERATOR_ROLE) {
        _limit = limit_;
    }
