...
contract PeonToken is ONFT721Upgradeable, ERC2981Upgradeable, PausableUpgradeable, IPeonToken {
    error RemoveLaunchpadFunctionalityTODO();
    ...
    /**
     * Checks if mint was initialized to avoid minting token ID 0, with mintRegular
     */
    modifier isMintInitialized() {
        if (saleId == 0) revert PeonToken__NotInitialized();
        _;
    }
     ...
     */
    function mintDiamond(address minter, uint256 amount) external onlyBurner isMintInitialized {
     ...
     * @param amount Amount of tokens that should be minted
     */
    function mintGolden(address minter, uint256 amount) external onlyBurner isMintInitialized {
     ...
     * @param amount Amount of tokens that should be minted
     */
    function mintRegular(address minter, uint256 amount) external onlyBurner isMintInitialized {
    ...
    function publicSaleMint(uint256 _quantity) external payable isEOA whenNotPaused atPhase(Phase.PublicSale) {
        revert RemoveLaunchpadFunctionalityTODO();
