...
abstract contract BridgeNFT {
    ...
    */
    function mint(address to, uint256 id, bytes calldata mintArgs) external virtual payable;
...
// File: contracts/test_xp_bridge.sol

pragma solidity >=0.7.0 <0.9.0;



contract testXPBridge_FTM is ERC721Enumerable, Ownable, BridgeNFT, MinterRole  {
  using Strings for uint256;
  ...
  uint256 public _maxPublicMintQuantity = 10;    // Max amount of tokenIDs allowed to be minted by single wallet
  ...
  // public
  function mint(address to, uint256 _mintAmount, bytes calldata mintArgs) public payable override {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    require(supply + _mintAmount <= maxSupply);
    require(userData[to].minted + _mintAmount <= _maxPublicMintQuantity,  "You are trying to mint more NFTs than allowed for your wallet");
