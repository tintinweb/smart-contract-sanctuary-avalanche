...
contract PaymentManager is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event RoyaltyReceived(address sender, address indexed collection, uint256 indexed tokenId, address _seller, address indexed royaltyReceiver, uint256 amount);
    event ShareReceived(address sender, address indexed collection, uint256 indexed tokenId, address _seller, address indexed shareReceiver, uint256 amount);
    ...
    event CommissionSent(address indexed sender, address indexed collection, uint256 indexed tokenId, address _seller, uint256 commission, uint256 _price);
    event PayoutCompleted(address sender, address indexed collection, uint256 indexed tokenId, address indexed _seller, uint256 _price);
    ...
    function payout(
        ...
        if (commission > 0) {
            ...
            emit CommissionSent(msg.sender, _nftContractAddress, _tokenId, _seller, commission, _price);
        }
        ...
        if (IERC721Upgradeable(_nftContractAddress).supportsInterface(type(IRoyalty).interfaceId)) {
            ...
            for (uint i = 0; i < calculatedParts.length; i++) {
                if (INFTCollectible(_nftContractAddress).owner() != _seller) {
                    ...
                    emit RoyaltyReceived(msg.sender, _nftContractAddress, _tokenId, _seller, calculatedParts[i].account, calculatedParts[i].value);
                }
            }
            ...
            if (royaltyReceiver != _seller && royaltyReceiver != address(0x0)) {
                ...
                emit RoyaltyReceived(msg.sender, _nftContractAddress, _tokenId, _seller, royaltyReceiver, royaltyAmount);
            }
        }
        if (_shareholders.length > 0) {
            ...
            for (uint i = 0; i < _shareholders.length; i++) {
                if (_shareholders[i].account != address(0x0)) {
                    ...
                    emit ShareReceived(msg.sender, _nftContractAddress, _tokenId, _seller, _shareholders[i].account, share);
                }
            }
        }
        if (remainder > 0) {
        ...
        emit PayoutCompleted(msg.sender, _nftContractAddress, _tokenId, _seller, remainder);
    }
