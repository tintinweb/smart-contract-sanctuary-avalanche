...
contract Registryv3 is IRegistry, AccessControl, ReentrancyGuard {
    ...
    mapping(bytes32 => bool) public _allowedCurrencies;
    ...
    mapping(address => mapping(address => mapping(bytes32 => uint256))) private _royalties;
    ...
    constructor(address _factory, address _priceFeed, address _usdc) {
        factory = _factory;
        priceFeed = _priceFeed;
        usdc = _usdc;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(EVENT_CREATOR_ROLE, msg.sender);
        _allowedCurrencies[bytes32('AVAX')] = true;
        _allowedCurrencies[bytes32("USDC")] = true;
    }
    ...
    function royaltyCollectedForBatch(address collector, address[] memory eventAddresses, bytes32 currency) external view returns (uint256[] memory) {
        require(_allowedCurrencies[currency], 'invalid currency');
        require(hasRole(EVENT_CREATOR_ROLE, msg.sender), 'Caller is not authorized');
        uint256[] memory result = new uint256[](eventAddresses.length);
        for (uint256 i = 0; i < eventAddresses.length; i++) {
            result[i] = (_royalties[collector][eventAddresses[i]][currency]);
        }
        return result;
    }

    function royaltyCollectedFor(address collector, address eventAddress, bytes32 currency) external view returns (uint256) {
        require(_allowedCurrencies[currency], 'invalid currency');
        return _royalties[collector][eventAddress][currency];
    }

    function royaltyCollected(address eventAddress, bytes32 currency) external view returns (uint256) {
        require(_allowedCurrencies[currency], 'invalid currency');
        return _royalties[msg.sender][eventAddress][currency];
    }
    ...
    function withdrawRoyaltyFor(
        ...
        require(amount > 0 && _royalties[collector][eventAddress][currency] > 0 && _royalties[collector][eventAddress][currency] >= amount, 'Invalid amount');
        
        if (currency == bytes32('AVAX')) {
            payable(collector).transfer(amount);
        } else if (currency == bytes32('USDC')) {
            IERC20(usdc).transfer(collector, amount);
        }
        _royalties[collector][eventAddress][currency] = _royalties[collector][eventAddress][currency].sub(amount);
        emit RoyaltyWithdrew(eventAddress, collector, amount);
    }

    function withdrawRoyalty(address eventAddress, uint256 amount, bytes32 currency) external nonReentrant {
        require(_allowedCurrencies[currency], 'invalid currency');
        require(amount > 0 && _royalties[msg.sender][eventAddress][currency] > 0 && _royalties[msg.sender][eventAddress][currency] >= amount, 'Invalid amount');
        ...
        _royalties[msg.sender][eventAddress][currency] = _royalties[msg.sender][eventAddress][currency].sub(amount);
        emit RoyaltyWithdrew(eventAddress, msg.sender, amount);
    }

    function withdraw(uint256 amount, bytes32 currency) external nonReentrant {
        require(hasRole(EVENT_CREATOR_ROLE, msg.sender), 'Caller is not authorized');
    ...
    function createEvent(
        ...
        require(endTimestamp > block.timestamp, 'Event can not be in the past');
    ...
    function buyTickets(address eventAddress, uint256[] memory ticketTypes, bytes32 currency) external payable nonReentrant {
        require(_allowedCurrencies[currency], 'invalid currency');
        ...
        for (uint256 i = 0; i < ticketTypes.length; i++) {
            uint256 subPrice = IPriceFeed(priceFeed).getPriceInToken(_prices[eventAddress][ticketTypes[i]], currency);
        ...
        if (currency == bytes32('USDC')) {
            require(IERC20(usdc).transferFrom(msg.sender, address(this), price), 'Incorrect payment');
        }
        ...
        for (uint256 i = 0; i < royaltyCollectors.length; i++) {
            ...
            uint256 currentRoyalty = _royalties[royaltyCollectors[i].collector][eventAddress][currency];
            _royalties[royaltyCollectors[i].collector][eventAddress][currency] = currentRoyalty.add(buyingFee);
            totalFee = totalFee.add(buyingFee);
        }

        //set rest of the royalties to the NFTTiX
        _royalties[address(this)][eventAddress][currency] = _royalties[address(this)][eventAddress][currency].add(price.sub(totalFee));
    ...
    function giftTicket(address recipient, address eventAddress, uint256[] memory ticketTypes, bytes32 currency) external payable nonReentrant {
        require(_allowedCurrencies[currency], 'invalid currency');
        require(eventAddress != address(0), 'invalid eventAddress.');
        require(ticketTypes.length > 0, 'ticketTypes cant be zero.');
    ...
    function fillOrder(OrderInterface.Order memory order, bytes memory signature, bytes32 currency) external payable nonReentrant {
        require(_allowedCurrencies[currency], 'invalid currency');
        ...
        uint256 price;
        if (currency == bytes32('USDC')) {
            price = IPriceFeed(priceFeed).getPriceInToken(order.price, currency);
            require(IERC20(usdc).transferFrom(msg.sender, address(this), price), 'Incorrect payment.');
        } 
        ...
        if (currency == bytes32('USDC')) {
            _fillOrderUsdc(order, price);
        }

        if (currency == bytes32('AVAX')) {
            _fillOrderEth(order, msg.value, msg.sender, price);
        }
        eventContract.transferFrom(ticketOwner, msg.sender, order.tokenId);
    }

    function _fillOrderEth(OrderInterface.Order memory order, uint256 amount, address sender, uint256 price) internal {
        // returns change
        if (amount > price) {
            payable(sender).transfer(amount.sub(price));
        }
        ...
        for (uint256 i = 0; i < royaltyCollectors.length; i++) {
            uint256 orderFee = price.mul(royaltyCollectors[i].royaltyFee).div(10000);
            uint256 currentRoyalty = _royalties[royaltyCollectors[i].collector][order.eventAddress][bytes32('AVAX')];
            _royalties[royaltyCollectors[i].collector][order.eventAddress][bytes32('AVAX')] = currentRoyalty.add(orderFee);
            totalFee = totalFee.add(orderFee);
        }

        // pay (price-fee) to maker
        payable(order.maker).transfer(price.sub(totalFee));
    }

    function _fillOrderUsdc(OrderInterface.Order memory order, uint256 price) internal {
        uint256 totalFee = 0;
        // split royalties to collectors
        RoyaltyCollector[] memory royaltyCollectors = getResellRoyaltyCollectors[order.eventAddress];
        ...
        for (uint256 i = 0; i < royaltyCollectors.length; i++) {
            uint256 orderFee = price.mul(royaltyCollectors[i].royaltyFee).div(10000);
            uint256 currentRoyalty = _royalties[royaltyCollectors[i].collector][order.eventAddress][bytes32('USDC')];
            _royalties[royaltyCollectors[i].collector][order.eventAddress][bytes32('USDC')] = currentRoyalty.add(orderFee);
            totalFee = totalFee.add(orderFee);
        }
        IERC20(usdc).transfer(order.maker, price.sub(totalFee));
    }
...
interface IPriceFeed {
    function getPriceInToken(uint256 _usdPrice, bytes32 _currency) external view returns (uint256);
}
