...
contract Vault is Ownable{ 
    ...
    function add(
        uint256 _lockDuration, // in seconds
        uint256 _additionnalReward, // dditionnal percentage of reward : if additionnal reward is 10% then put 10;
        uint256 _taxesPerEpoch // must be given *1000
    ) public onlyOwner {
        
        require(_taxesPerEpoch>0,"Vault: TaxesPerEpoch must be positive");
        ...
        require((_lockDuration.div(14400)).mul(_taxesPerEpoch).div(1000) < 30,"Vault: taxes not < 30");
        require((_lockDuration.div(14400).mul(_taxesPerEpoch).div(1000)!=0),"Vault: Problem with decimals");
        require(_lockDuration>14399,"Vault locktime must be equal or longer than an epoch");
    ...
    function stake(uint256 amount, uint256 vaultNumber) public {
        ...
        require(rebaseToken.balanceOf(msg.sender)>=amount,"You don't have enough money");
        ...
        userInfo[vaultNumber][msg.sender].withdrawTime = block.timestamp.add(vaultInfo[vaultNumber].lockDuration); 
        rebaseToken.burnFrom(msg.sender, amount);
        userInfo[vaultNumber][msg.sender].amountStaked = amount; 

        emit Staked(msg.sender, amount, vaultNumber); 
    }

    function withdraw(uint256 vaultNumber) public { 
        require(vaultNumber<vaultInfo.length, "Vault: Doesn't exist");
        require(vaultInfo[vaultNumber].isInitialized, "Vault: not initialized yet");
        require(userInfo[vaultNumber][msg.sender].amountStaked>0,"Vault: You don't have money in this vault"); 

        uint256 toWithdraw = userInfo[vaultNumber][msg.sender].amountStaked; 
        uint256 _withdrawTime = (userInfo[vaultNumber][msg.sender].withdrawTime);
        userInfo[vaultNumber][msg.sender].amountStaked = 0; 
        
        if(_withdrawTime > block.timestamp){    
            uint256 nbEarlyEpoch =  (( _withdrawTime.sub(block.timestamp) ).div(14400)).add(1);
            uint256 totalTax = (nbEarlyEpoch.mul(vaultInfo[vaultNumber].taxesPerEpoch));
            uint256 toUser = (toWithdraw.mul(uint256(100000).sub(totalTax))).div(100000);
            uint256 toTaxCollector = toWithdraw.sub(toUser); //calcul
            rebaseToken.mint(msg.sender,toUser);
            rebaseToken.mint(_walletTaxCollector,toTaxCollector);
        }   

        else{
            uint256 reward = toWithdraw;
            for(uint256 i = 0; i<(vaultInfo[vaultNumber].lockDuration.div(14400));i++){
                reward = reward + reward.mul(rebaseToken.getRewardYield()).div(rebaseToken.getRewardYieldDenominator());
            }
            reward = reward.mul(uint256(100).add(vaultInfo[vaultNumber].additionnalReward)).div(100);
            if (reward > 0) {
        ...
        userInfo[vaultNumber][msg.sender].withdrawTime = 0;
    }
}
