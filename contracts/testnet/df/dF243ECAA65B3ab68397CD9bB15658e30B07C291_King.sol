...
contract King is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the day 2-5 KING-WETH LP -> KING pool
    uint256 public constant INITIAL_KING_POOL_DISTRIBUTION = 10 ether;
    ...
    /**
     * @notice Constructs the KING ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("KING", "KING") {
        // Mints 1 KING to contract creator for initial pool setup
    ...
    function _getKingPrice() internal view returns (uint256 _kingPrice) {
        try IOracle(kingOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("King: failed to fetch KING price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _KingPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_KingPrice >= taxTiersTwaps[tierId]) {
    ...
    function setKingOracle(address _kingOracle) public onlyOperatorOrTaxOffice {
        require(_kingOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints KING to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of KING to mint to
...
//        bool burnTax = false;
        ...
        // if (autoCalculateTax) {
        //     uint256 currentKingPrice = _getKingPrice();
        //     currentTaxRate = _updateTaxRate(currentKingPrice);
        //     if (currentKingPrice < burnThreshold) {
        //         burnTax = true;
        //     }
        // }
    ...
    */
    ...
    //     balances[recipient] += amountAfterTax;
    //     emit Transfer(sender, taxCollectorAddress, taxAmount);
    ...
    function distributeReward(
        ...
        require(_kingPool != address(0), "!_kingPool");
        rewardPoolDistributed = true;
        _mint(_kingPool, INITIAL_KING_POOL_DISTRIBUTION);
    }
