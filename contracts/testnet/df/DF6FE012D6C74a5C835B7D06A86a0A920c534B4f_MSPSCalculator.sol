...
contract MSPSCalculator is Ownable {
  ...
  PlatinumManager  platinum;
  InfiniteManager  infinite;
  MetaManager  meta;
  ...
  event CalculateMPSP(TArray input);
  ...
  function updateParameters(uint64 _legendary,
    ...
    uint64 _meta1, uint64 _meta2, uint64 _meta3, uint64 _infinite, uint64 _platinum, uint64 _precision
    ) onlyOwner external {
      parameters.legendary = _legendary;
      parameters.epic = _epic;
      parameters.rare = _rare;
      parameters.common = _common;
      parameters.meta1 = _meta1;
      parameters.meta2 = _meta2;
      parameters.meta3 = _meta3;
  ...
  function calculateMPSP(address from, uint[] memory nfts) external view returns (uint256) {
    uint256 total = 0;
    total = calculateNodesMPSP( from);

    Tier memory counterstMSPS = tMPSPAllTiers(nfts);

    uint totalLegendary = calculateTotalPerTier(nfts, tiers.legendary );
    if(totalLegendary >0){
      total = total + mPSPSingles(tiers.legendary, totalLegendary);
      total = total + (parameters.legendary*parameters.legendary*counterstMSPS.legendary)/parameters.precision;
    }

    uint totalEpic = calculateTotalPerTier(nfts, tiers.epic );
    if (totalEpic >0){
      total = total + mPSPSingles(tiers.epic, totalEpic);
      total = total + (parameters.epic*parameters.epic*counterstMSPS.epic)/parameters.precision;
    }

    uint totalRare = calculateTotalPerTier(nfts, tiers.rare );
    if (totalRare >0){
      total = total + mPSPSingles(tiers.rare, totalRare);
      total = total + (parameters.rare*parameters.rare*counterstMSPS.rare)/parameters.precision;
    }

    uint totalCommon = calculateTotalPerTier(nfts, tiers.common );
    if (totalCommon >0){
      total = total + mPSPSingles(tiers.common, totalCommon);
      total = total + (parameters.common*parameters.common*counterstMSPS.common)/parameters.precision;
    }
  ...
  function calculateNodesMPSP(address from) internal view returns (uint) {
    uint total = 0;
    ...
    if (metas == 1){
      total = total + parameters.meta1 ;
    } else if (metas == 2){
      total = total + parameters.meta2 ;
    }  else if (metas == 3){
      total = total + parameters.meta3 ;
    }
    return total;
  }


  function tMPSPAllTiers(uint[] memory nfts) internal view returns (Tier memory) {
  ...
  function calculateAllCombinations(uint size, TArray memory nfts ) private view returns (uint, Tier memory,  TArray memory) {
    ...
    function calculateTotalPairs(uint[] memory nfts, uint targetTier) internal view returns (uint256) {
      uint pairs = 0;
  ...
  function calculateTotalThirds(uint[] memory nfts, uint targetTier) internal view returns (uint256) {
    uint thirds = 0;
    uint size = nfts.length;
    uint[] memory possibles= new uint[](size) ;
    uint[] memory counter= new uint[](size) ;
  ...
  function calculateTotalPokers(uint[] memory nfts, uint targetTier) internal view returns (uint256) {
    uint pokers = 0;
    uint size = nfts.length;
    uint[] memory possibles= new uint[](size) ;
    uint[] memory counter= new uint[](size) ;
  ...
  function mPSPSingles(uint targetTier, uint singles) internal view returns (uint256) {
  ...
  function mPSPPair(uint targetTier) internal view returns (uint256) {
     if ( targetTier == tiers.legendary ){
  ...
  function mPSPThirds(uint targetTier) internal view returns (uint256) {
     if ( targetTier == tiers.legendary ){
  ...
  function mPSPPokers(uint targetTier) internal view returns (uint256) {
     if ( targetTier == tiers.legendary ){
