...
contract NodeManager is Ownable, Pausable {
    ...
    function compileVars() public view returns (uint256) {
	...
	for (uint256 j = 0; j < nodes.length; j++) {
		...
		uint256 lastClaim = nodes[j].lastClaimTime;
		uint256 elapsed = time - lastClaim;
        	uint256[3] memory rew = boostLib.calcReward(dailyRewardsPerc,timeStep,time,lastClaim,boostMultiplier[0]);
