...
contract BetupV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    private view returns(IBetStruct.RewardType memory reward)
    {
        ...
        if(_maxLevel == 0) {
            require(
                IMembershipCard(membershipAddress).playersSnapshot(_account) == 0,
                "Invalid _maxLevel");
        }else {
            require(
                IMembershipCard(membershipAddress).playersSnapshot(_account) == 2 ** (256 - _maxLevel),
                "Invalid _maxLevel");
        }
