...
contract StakeSTK is Pausable, Ownable {
    ...
    constructor(SuperToken _tokenAddress) {
        require(address(_tokenAddress) != address(0), "Token Address cannot be address 0");
        superToken = _tokenAddress;
        totalStakers = 0;
    }

    function transferToken(address to,uint256 amount) external onlyOwner {
        require(superToken.transfer(to, amount), "Token transfer failed!");
    }
        ...
        superToken.transfer(_msgSender(), stakeAmount);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(superToken.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");

        superToken.transferFrom(_msgSender(), address(this), stakeAmount);
        totalStakers++;
