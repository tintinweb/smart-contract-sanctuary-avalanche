...
// File contracts/util/EnumerableUint16Set.sol
...
// File contracts/BaseAggregator.sol
...
abstract contract BaseAggregator is Ownable {
    ...
    uint256 public constant LZ_FEE = 0.2 ether;
 ...
 * @author Gonçalo Sá <gonc[email protected]>
 *
...
// File contracts/LiquidatorRewardsAggregator.sol

pragma solidity ^0.8.0;

/**
 * This smart contract is for brdging and aggregating liquidation rewards;
 * There's one LiquidatorRewardsAggregator for each collateral asset
 * This LZ UA can be said the aggregated LiquidatorRewards.
 */
contract LiquidatorRewardsAggregator is BaseAggregator, NonblockingLzApp {
    using BytesLib for bytes;
    using EnumerableSet for EnumerableSet.Uint16Set;

    uint256 public constant PT_NOTIFY_REWARD = 1;
    uint256 public constant PT_UPDATE_ENTRY = 2;
    ...
    event NotifyReward(uint16 indexed srcChainId, uint256 accumulatedPerShare, uint256 increasedShare);
    event UpdateEntry(uint16 indexed srcChainId, address indexed account);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        ...
        if (packetType == PT_NOTIFY_REWARD) {
            (, uint256 increasedShare) = abi.decode(_payload, (uint16, uint256));
            _updateAccumulatedShare(increasedShare);
            emit NotifyReward(_srcChainId, accumulatedRewardsPerShare, increasedShare);
        } else if (packetType == PT_UPDATE_ENTRY) {
             (, uint256 debtShare, bytes memory accountBytes) = abi.decode(_payload, (uint16, uint256, bytes));
             address account = accountBytes.toAddress(0);
            _updateEntry(debtShare, account);
            emit UpdateEntry(_srcChainId, account);
        } else {
            revert("LRA: unknow packet type");
        }
    }

    function _updateAccumulatedShare(uint256 _increasedShare) private {
        accumulatedRewardsPerShare += _increasedShare;
    }

    function sendNotifyReward(uint256 increasedShare) external onlySrcContract {
        _updateAccumulatedShare(increasedShare);

        bytes memory lzPayload = abi.encode(PT_NOTIFY_REWARD, increasedShare);
    ...
    function _updateEntry(uint256 _debtShare, address _account) private {
    ...
    function sendUpdateEntry(uint256 debtShare, address account) external onlySrcContract {
        _updateEntry(debtShare, account);
        
        bytes memory lzPayload = abi.encode(PT_UPDATE_ENTRY, debtShare, account);
     ...
     */
    function earned(address account, uint256 debtShare) public view returns (uint256) {
        ...
        //         .add(entry.claimable);
        return
            debtShare * (accumulatedRewardsPerShare - entry.entryAccumulatedRewards) / (10 ** 27) + entry.claimable;
    }
}