...
contract ArableCollateral is Ownable {
    ...
    function _liquidateCollateral(address user, address beneficiary, uint liqAmount) 
        external onlyAddress("ARABLE_LIQUIDATION") {
            ...
            // Note: this case happens when a user's debt * liquidationPenalty is bigger than collateral - but for good protocol maintenance, these positions should be removed
    ...
    function _calculateSingleCollateralValue(address user, address token) internal view returns (uint) {
        ...
        return _userCollateralBalance(user, token) * tokenPrice / 1 ether;
    }
...
interface IArableLiquidation {
    ...
    function calculateLiquidationRate(address user) external view returns (uint);
