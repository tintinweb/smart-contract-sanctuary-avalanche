...
contract Staking is Ownable, Pausable {
    ...
    returns (bool isDefine)
    {
        if (getPowerNft(tokenId) >= 1000 && getPowerNft(tokenId) <= 10000) return true;
        else return false;
    }
...
contract Seam is ERC721Enumerable, Ownable, RoyaltiesImpl, Pausable {
    ...
    uint256 public paidTokens = 500;
    // Price mint in TRSE
    uint public priceToBuyTRSR = 1000;
    ...
    function mintGiveAway(address to, uint mintAmount) public payable whenNotPaused {
        ...
        for (uint256 i = 1; i <= mintAmount; i++) {
            ...
            // royalties fixed 5%
            setRoyalties(i,royaltiesAddressPayable,500);
        }
    }
