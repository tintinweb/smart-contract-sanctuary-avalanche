...
interface Gears__IERC20 is IERC20 {
    function burnFrom(address account, uint256 amount) external;
}

contract Gears is ERC1155, Pausable, Ownable, AccessControl {
    ...
    event Upgraded(uint256 indexed fromId, uint256 indexed toId);
    ...
    function setURI(string memory value) public onlyRole(URI_SETTER_ROLE) {
        _setURI(value);
    }
    ...
    function _isValidSignature(
        ...
        bytes32 hash = keccak256(abi.encode(paymentToken, price, fromId, toId, timestamp));
    ...
    function upgrade(
        ...
        require(_isValidSignature(paymentToken, price, fromId, toId, timestamp, serverSignature));

        // Perform the payment by burning the tokens.
        // Reverts if insufficient tokens or insufficient tokens approved.
        Gears__IERC20(paymentToken).burnFrom(msg.sender, price);

        // Burns the original gear.
        _burn(msg.sender, fromId, 1);
        // Mints an upgraded gear.
        _mint(msg.sender, toId, 1, '');

        // Emits the event for the server to listen to.
        emit Upgraded(fromId, toId);
    }

    function getUpgradeFailureReason(
        address paymentToken,
        uint256 price,
        uint16 fromId,
        uint16 toId,
        ...
        if (!_isTimestampFresh(timestamp)) {
            return 'Gears: timestamp expired.';
        }

        // Check that the `serverSignature` is valid.
        if (!_isValidSignature(paymentToken, price, fromId, toId, timestamp, serverSignature)) {
            return 'Gears: invalid signature.';
        }

        // Check that buyer has enough balance.
        if (IERC20(paymentToken).balanceOf(msg.sender) < price) {
            return 'Gears: insufficient payment tokens.';
        }

        // Check that buyer has enough allowance.
        if (IERC20(paymentToken).allowance(msg.sender, address(this)) < price) {
            return 'Gears: insufficient payment tokens approved.';
        }

        // Check that the buyer has at least a copy of the gear to burn.
        if (balanceOf(msg.sender, fromId) == 0) {
            return 'Gears: insufficient gear.';
        }

        return 'Gears: upgrade failed.';
    }
