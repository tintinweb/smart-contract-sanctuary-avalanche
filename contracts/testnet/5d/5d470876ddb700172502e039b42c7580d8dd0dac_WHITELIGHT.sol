...
library Address {
  ...
  function sendValue(address payable recipient, uint256 amount) internal {
    require(address(this).balance >= amount, "WHITE Address: insufficient balance");

    // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
    (bool success, ) = recipient.call{value: amount}("");
    require(success, "WHITE Address: unable to send value, recipient may have reverted");
  }
  ...
  function functionCall(address target, bytes memory data) internal returns (bytes memory) {
    return functionCall(target, data, "WHITE Address: low-level call failed");
  }
  ...
  function functionCallWithValue(
    ...
    return functionCallWithValue(target, data, value, "WHITE Address: low-level call with value failed");
  }
  ...
  function functionCallWithValue(
    ...
    require(address(this).balance >= value, "WHITE Address: insufficient balance for call");
  ...
  function _functionCallWithValue(
    ...
    require(isContract(target), "WHITE Address: call to non-contract");
...
contract LockOwnable is Ownable {
  ...
  function unlock() public virtual {
    require(_previousOwner == msg.sender, "WHITE: You don't have permission to unlock");
    require(block.timestamp > _lockTime, "WHITE: Contract is still locked");
...
contract WHITELIGHT is Context, IBEP20, LockOwnable {
  ...
  // maintenance  0x82Ec350eeA9741B3B47CC00b29F9B503076ee3D8
  // marketing    0x2b6dE129a667EBCACAb97e58b7027F17E72e7ea9

  address payable public _feeManager = payable(0x82Ec350eeA9741B3B47CC00b29F9B503076ee3D8); // WHITE feeManager address
  address payable public _liquidityManager = payable(0x2b6dE129a667EBCACAb97e58b7027F17E72e7ea9); // WHITE liquidity address
  ...
  uint256 public _buyFeeReducedEndTime = 0 minutes;

  uint256 public _sellFee = 1000; // 10%
  uint256 public _sellFeeIncreaseEndTime = 0 minutes;
  ...
  event BuyFeeSet(uint256 duration, uint256 buyFee);
  event SellFeeSet(uint256 duration, uint256 sellFee);
  ...
  function mint(address account, uint256 amount) external {
    require(msg.sender == _feeManager, "WHITE: can't mint");
    require(account != address(0), "WHITE: mint to the zero address");
    ...
    if (deductedMintFee > 0) {
      _balances[_liquidityManager] = _balances[_liquidityManager].add(deductedMintFee);
      emit Transfer(address(0), _liquidityManager, deductedMintFee);
    }
  }
  ...
  function _burn(address account, uint256 amount) internal virtual {
    ...
    if (deductedBurnFee > 0) {
      _balances[_liquidityManager] = _balances[_liquidityManager].add(deductedBurnFee);
      emit Transfer(account, _liquidityManager, deductedBurnFee);
    }
  }
  ...
  function setMintFee(uint256 mintFee) external onlyOwner {
    require(mintFee <= 2500, "WHITE: mint fee should be be between 0% - 25% (0 - 2500)");
    _mintFee = mintFee;
    emit MintFeeSet(mintFee);
  }

  function setBurnFee(uint256 burnFee) external onlyOwner {
    require(burnFee <= 2500, "WHITE: burn fee should be between 0% - 25% (0 - 2500)");
    _burnFee = burnFee;
  ...
  function setTransactionFee(uint256 fee) external onlyOwner {
    require(fee <= 1250, "WHITE: Transaction fee should be between 0% - 12.5% (0 - 1250)");
    require(
      block.timestamp > _buyFeeReducedEndTime || fee >= _buyFee,
      "WHITE: Transaction fee should be between (buyFee)% - 12.5% (buyFee - 1250)"
    );
    require(
      block.timestamp > _sellFeeIncreaseEndTime || fee <= _sellFee,
      "WHITE: Transaction fee should be between 0% - (sellFee)% (0 - sellFee)"
    );
    _transactionFee = fee;
   ...
   * @dev set's WHITE liquidity provider address
   */
  function setLiquidityManager(address payable liquidityManager) external onlyOwner {
    _isExcludedFromFee[_liquidityManager] = false;
    _liquidityManager = liquidityManager;
    _isExcludedFromFee[_liquidityManager] = true;
    emit LiquidityManagerSet(liquidityManager);
  }
  ...
  function setBuyFee(uint256 duration, uint256 newBuyFee) external onlyOwner {
    ...
    _buyFee = newBuyFee;
    _buyFeeReducedEndTime = block.timestamp.add(duration * 1 minutes);
    emit BuyFeeSet(duration, newBuyFee);
  }
  ...
  function setSellFee(uint256 duration, uint256 newSellFee) external onlyOwner {
    require(
      newSellFee > _transactionFee && newSellFee <= 2500,
      "WHITE: Increased sell fee should be between (transfer fee)% - 25%"
    );
    _sellFee = newSellFee;
    _sellFeeIncreaseEndTime = block.timestamp.add(duration * 1 minutes);
    emit SellFeeSet(duration, newSellFee);
  }
  ...
  function _transfer(
    ...
    if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
      fee = 0;
    } else if (_isPair[from] && block.timestamp <= _buyFeeReducedEndTime) {
      fee = _buyFee;
    } else if (_isPair[to] && block.timestamp <= _sellFeeIncreaseEndTime) {
      fee = _sellFee;
    }
