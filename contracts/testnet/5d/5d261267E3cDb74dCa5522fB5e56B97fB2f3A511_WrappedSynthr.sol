...
contract BaseWrappedSynthr is IERC20, ExternWrappedStateToken, MixinResolver, ISynthetix {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[7] = CONTRACT_SYNTHRBRIDGE;
    }
    ...
    function burnSynths(uint256 amount) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynths(messageSender, amount);
        synthrBridge().sendBurn(messageSender, sUSD, synthAmount, debtShare);
    }

    function burnSynthsOnBehalf(address burnForAddress, uint256 amount) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsOnBehalf(burnForAddress, messageSender, amount);
        synthrBridge().sendBurn(burnForAddress, sUSD, synthAmount, debtShare);
    }

    function burnSynthsToTarget() external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsToTarget(messageSender);
        synthrBridge().sendBurn(messageSender, sUSD, synthAmount, debtShare);
    }

    function burnSynthsToTargetOnBehalf(address burnForAddress) external issuanceActive optionalProxy {
        (uint256 synthAmount, uint256 debtShare) = issuer().burnSynthsToTargetOnBehalf(burnForAddress, messageSender);
        synthrBridge().sendBurn(burnForAddress, sUSD, synthAmount, debtShare);
    }
    ...
    function liquidateDelinquentAccount(
        ...
        if (totalRedeemed > 0) {
            ...
            _burn(account, totalRedeemed, collateralAmount, collateralKey);
    ...
    function liquidateSelf(bytes32 collateralKey) external systemActive optionalProxy returns (bool) {
        ...
        _burn(messageSender, totalRedeemed, collateralAmount, collateralKey);
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function _issueSynths(address from, address to, bytes32 _collateralKey, uint256 _collateralAmount) internal {
        ...
        bool isSucceed = _mint(to, synthrToMint, _collateralAmount, _collateralKey);
    ...
    function withdrawCollateral(bytes32 _collateralKey, uint256 _collateralAmount) external issuanceActive optionalProxy {
        ...
        bool isSucceed = _burn(messageSender, synthrToBurn, _collateralAmount, _collateralKey);
