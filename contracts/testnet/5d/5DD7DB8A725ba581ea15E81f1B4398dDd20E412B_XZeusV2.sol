...
contract XZeusV2 is Ownable, VRFConsumerBaseV2, KeeperCompatibleInterface {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    //TODO: for stakerId handling
    using Counters for Counters.Counter;

    Counters.Counter private _stakerIdPointer;
    ...
    // ZEUS tokens created per block. [new]
    uint256 public rewardPerBlockNew;

    // reward active timestamp
    uint256 public rewardActiveTimestamp;
    ...
    //TODO: receipt infos (receiptId => receipt info)
    ...
    constructor(
        ...
        uint256 _rewardActiveTimestamp,
        ...
        rewardActiveTimestamp = _rewardActiveTimestamp;
         ...
         * Aggregator: AVAX/USD
         * Address: 0x0A77230d17318075983913bC2145DB16C7366156 / 0x5498BB86BC934c8D34FDA08E81D444153d0D06aD
         */
     ...
     * @param _user  user address
     */
    function getUserReceiptIds(uint256 _pid, address _user)
        external
        view
     ...
     * @param _receiptId  receipt id
     */
    function withdraw(uint256 _receiptId) public {
     ...
     * @param _receiptId  receipt id
     */
    function emergencyWithdraw(uint256 _receiptId) public {
        ReceiptInfo storage receipt = receiptInfo[_receiptId];
        require(receipt.user == msg.sender, "Staker address is not matched!");
        ...
        emit EmergencyWithdraw(msg.sender, receipt.poolId, receipt.amount);
    }
        ...
        override
    {
        ...
        uint256 receiptId = randomRequestUserInfo.receiptId;
        uint256 poolId = receiptInfo[receiptId].poolId;

        PoolInfo storage pool = poolInfo[poolId];

        updatePool(poolId);

        uint256 pending = receiptInfo[receiptId]
            .amount
            .mul(pool.accZeusPerShare)
            .div(PRECISION_FACTOR)
            .mul(10**rewardDecimal)
            .div(1e18)
            .sub(receiptInfo[receiptId].rewardDebt);
        ...
        // withdraw staking token to stakers
        pool.stakingToken.safeTransfer(
            requester,
            receiptInfo[receiptId].amount
        );

        // update storage
        pool.totalShares -= receiptInfo[receiptId].amount;
        delete receiptInfo[receiptId];

        emit Withdraw(msg.sender, poolId, receiptInfo[receiptId].amount);
    }
    ...
    function performUpkeep(
        ...
        require(
            block.timestamp > rewardActiveTimestamp + 604800,
    ...
    /**
     * @notice Update receipt rewardDebt
     * @param _pid  pool id
     * @param _user  user address
     */
    function updateReceiptDebt(uint256 _pid, address _user) internal {
        uint256[] memory receiptIds = userInfo[_pid][_user];
        PoolInfo memory pool = poolInfo[_pid];

        for (uint256 i = 0; i < receiptIds.length; i++) {
            ReceiptInfo storage receipt = receiptInfo[receiptIds[i]];
            if (receipt.amount > 0) {
