...
contract DaoSwapper {
    ...
    constructor() {
        PangolinRouter = IPangolinRouter(PANGOLIN_ROUTER_ADRESSE);
    ...
    function swapExactAVAXForTokens() public payable {
        ...
        PangolinRouter.swapExactAVAXForTokens{ value: msg.value }(amountOutMin, getPathForAvaxForshermix(), address(msg.sender), deadline);
    ...
    function addLiquidityAVAX() public payable {
        
        uint amountTokenMin = getLatestPrice();
        uint deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
        PangolinRouter.addLiquidityAVAX{value: msg.value}(shermix, amountTokenMin, 0, 0, address(msg.sender), deadline);
    }
