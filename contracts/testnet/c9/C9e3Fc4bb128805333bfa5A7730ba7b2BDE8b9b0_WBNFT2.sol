...
contract WBNFT2 is ERC721URIStorage, ERC721Enumerable, ERC721Burnable, Ownable {
  ...
  uint256 private _royaltiesPercentage = 35;

  Counters.Counter private _tokenIdCount = Counters.Counter({_value: 20000});

  constructor() ERC721("WyndBlast NFT Next Generation", "WBNFTNG") {}

  modifier onlyMinter() {
    require(
      _minters[_msgSender()] || _msgSender() == owner(),
      "Only Minter"
    );
    _;
  }
  ...
  /**
   * @notice Get type of token
   * @param _tokenId Token id
   * @return TokenType
   */
  function typeOf(uint256 _tokenId) public view returns (string memory) {
    require(_exists(_tokenId), "Nonexistent token");

    TokenType _tokenType = _tokenTypes[_tokenId];

    if (_tokenType == TokenType.Wynd) {
      return "WYND";
    } else if (_tokenType == TokenType.Rider) {
      return "RIDER";
    } else if (_tokenType == TokenType.Equipment) {
      return "EQUIPMENT";
    } else {
      return "UNKNOWN";
    }
  ...
  function mint(
    ...
    emit TokenMinted(_to, _tokenType, tokenId, _tokenURI, _msgSender());
    return tokenId;
  }
  ...
  /**
   * @notice Add minter
   * @param _minter Minter address can be a contract address or wallet address
   */
  ...
  {
    require(_exists(_tokenId), "Nonexistent token");
    uint256 _royalties = (_salePrice * _royaltiesPercentage) / 1000;
    return (_royaltiesReceiver, _royalties);
  }

  /**
   * @notice Changes the royalties' percentage (3.5% = 35)
   * @param percentage uint356 (3.5% = 35)
   */
  function setRoyaltiesReceiver(uint256 percentage) external onlyOwner {
    _royaltiesPercentage = percentage;
  }
}
