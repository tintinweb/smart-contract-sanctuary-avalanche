...
contract Minter is Ownable {
    ...
    function waceoPriceInUSD() public view returns ( uint256 price_ ) {
        uint256 waceoInWAVAX = waceoPriceInWAVAX();
        uint256 wavaxInUSD =  wavaxPriceInUSD();
        price_ = waceoInWAVAX.mul(wavaxInUSD);
    }
    ...
    function waceoPriceInWAVAX() public view returns ( uint256 price_ ) {
        uint256 wavax_value = IERC20(Wavax).balanceOf(WAVAX_WACEO_LP);
        uint256 waceo_value = IERC20(Waceo).balanceOf(WAVAX_WACEO_LP);
        price_ = wavax_value.div(waceo_value);
    }
    ...
    function wavaxPriceInUSD() public view returns ( uint256 price_ ) {
        uint256 wavax_value = IERC20(Wavax).balanceOf(WAVAX_STABLE_LP);
        uint256 usdc_value = IERC20(Waceo).balanceOf(WAVAX_STABLE_LP);
        price_ = usdc_value.div(wavax_value);
    }
}