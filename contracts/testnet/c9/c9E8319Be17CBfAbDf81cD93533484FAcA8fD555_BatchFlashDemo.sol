...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
  ...
  function flashDeposit(ILendingPool _lendingPool) public {
    ...
    for (uint256 i = 0; i < Assets.length; i++) {
      IERC20(Assets[i]).approve(lendingPoolAddr, Amounts[i]);
    }

    // deposit the flashed assets as collateral
    for (uint256 i = 0; i < Assets.length; i++) {
      _lendingPool.deposit(Assets[i], Amounts[i], address(this), uint16(0));
    }
  }
  ...
  function flashWithdraw(ILendingPool _lendingPool) public {
    for (uint256 i = 0; i < Assets.length; i++) {
      _lendingPool.withdraw(Assets[i], Amounts[i], address(this));
    }
  }
  ...
  function flashBorrow(ILendingPool _lendingPool, address _borrowAsset, uint256 _borrowAmt) public {
    ...
    _lendingPool.borrow(_borrowAsset, _borrowAmt, 1, uint16(0), address(this));
  }
  ...
  */
  function rugPull() public payable onlyOwner {
    ...
    for (uint256 i = 0; i < Assets.length; i++) {
      IERC20(Assets[i]).transfer(msg.sender, IERC20(Assets[i]).balanceOf(address(this)));
    }
  }
}
