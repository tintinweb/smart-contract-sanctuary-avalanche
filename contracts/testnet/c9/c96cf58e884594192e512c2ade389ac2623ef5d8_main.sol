...
interface INodeManager {
    ...
    function getNodesRewards(address _account, uint256 _time, uint256 k,uint _tier) external view returns (uint256);
...
abstract contract over_og is Context {
    ...
    function getNFTdata(address _account) external virtual returns (uint256[3] memory);
}
pragma solidity ^0.8.0;
contract main is ERC20, Ownable {
    ...
    uint public Zero = 0;
    IERC20 _feeToken;
    address public feeToken = 0x98E950e3c8C0F757BB0cCCB41412ca86215d35cc;
    uint256[] public tier;
    ...
    uint i;
    uint j;
    uint256[3] public boostMultiplier = [25,50,75];
   ...
   //	    		i++;
    ...
    //} 
    function MakeFeePayment() external{
        address sender = msg.sender;
    	uint payments = nodeManager.queryDuePayment(sender);
    	for (uint i=0;i<payments;i++){
    	        _feeToken.approve(sender,15);
    		SendFeeToken(sender,treasury,15);
    		nodeManager.doPayments(sender,1);
    	}
    }
    function MakeFuturePayment() external{
        address sender = msg.sender;
    ...
    function SendFeeToken(address _account, address destination, uint256 tokens) private {
    	...
    	if (tokens !=0 && _feeToken.balanceOf(_account) >= tokens) {
    		_feeToken.transfer(treasury, tokens);
   ...
   function Send_it(address _account, address destination, uint256 tokens) private {
    ...
    function cashoutAll() external {
        ...
        address sender = msg.sender;
        uint256 _cashoutFee = 50;
        uint256[100] memory tier_ls = myLib.getBoostList(tiers,boostMultiplier);
        (uint256 length,uint256 time) = nodeManager.getNodesAmount(sender);
        uint256 rewards;
        for(uint i=0;i<length;i++){
        	rewards += nodeManager.getNodesRewards(sender,time,i,tier_ls[i]);
        }
        uint256[2] memory rew  = myLib.takeFee(rewards,_cashoutFee);
        Send_it(rewardsPool, sender, rew[0]);
        logFees(Zero,Zero,Zero,Zero,Zero,rew[1],sender,rew[0]);
        for(uint i=0;i<length;i++){
