...
contract BridgesOfFate is Ownable {
    ...
    constructor(IERC20 _wrappedEther,address _commuinty) {
        ...
        _pioneerPushBackNumberProbility = 30;
    }
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) external  GameEndRules PushBackBankingGameEndRules{
        GameItem storage _member = PlayerItem[playerId];
        GameStatus storage _gameStatus = GameStatusInitialized[latestGameId];
        uint256 currentDay = _dayDifferance(block.timestamp,_gameStatus.startAt);
        
        require(_member.feeStatus == true, "Please Pay Entry Fee.");
        // require(PioneerPushBackGameMap[_gameStatus.stageNumber] < 50e9,"Dynamic Game Ended.");
        ...
        if(_currentUserStage == _gameStatus.stageNumber) {
            if(_checkSide(_gameStatus.stageNumber,_jumpSide) && (PioneerPushBackGameMap[_gameStatus.stageNumber] > 50e9 )){
    ...
    function pushBack(bytes32 playerfrom,bytes32 playerto) external GameEndRules PushBackBankingGameEndRules  {
        ...
        _memberto.lastJumpSide = GameMap[_memberto.stage] >= 50e9;

        //  if (GameMap[GameStatusInitialized[latestGameId].stageNumber - 1] >= 50e9) {
