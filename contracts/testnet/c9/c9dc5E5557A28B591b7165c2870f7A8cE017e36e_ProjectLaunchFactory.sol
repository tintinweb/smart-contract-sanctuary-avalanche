...
import "./Sale_interfaces.sol";

contract ProjectLaunchFactory is Sale_interfaces {
  ...
  modifier enoughFee(uint _option) {
    require(msg.value >= launchFee[_option], "8");
    _;
  }
  ...
  function createSale(
    ...
    uint256 _options
  ) external payable enoughFee(_options) returns (address) {
    refundExcessiveFee(_options);

    require(iBlacklist(blacklistContract).getBlacklist(_address,_contract)==0,'1');
    require((_sale_details[1]>block.timestamp), "2");

    _sale_details[4] = launchFee[_options];
    _sale_details[5] = iCoupon(couponContract).getCouponDiscount(_address);
    _seed_details[8] = round1Mult[_options];
    _presale_details[8] = round2Mult[_options];
    _community_details[8] = round3Mult[_options];
    ...
    if (iCoupon(couponContract).getCouponActive(_address) == 1) {
      payable(feeTo).transfer(launchFee[_options] * ((100-iCoupon(couponContract).getCouponDiscount(_address)) /100));
    } else {
      payable(feeTo).transfer(launchFee[_options]);
    }

//this is new
    if (_options >= 4) {
      require(_seed_details[6] == 0, "3");
      if (_presale_details[6] > 0 && _community_details[6] > 0) {
        require(_presale_details[6] >= block.timestamp && _presale_details[7] > _presale_details[6] && _community_details[6] >= _presale_details[7] && _community_details[7] > _community_details[6], "4");
      } else if (_presale_details[6] > 0 && _community_details[6] == 0) {
        require(_presale_details[6] >= block.timestamp && _presale_details[7] > _presale_details[6] && _community_details[6] == 0, "5");
      } else if (_presale_details[6] == 0 && _community_details[6] > 0) {
        require(_presale_details[6] == 0 && _community_details[6] >= block.timestamp && _community_details[7] > _community_details[6], "6");
      }
    } else if (_options <= 3 && _options > 0) {
      require(_seed_details[6] >= block.timestamp && _seed_details[7] > _seed_details[6] && _presale_details[6] >= _seed_details[7] && _presale_details[7] > _presale_details[6] && _community_details[6] >= _presale_details[7] && _community_details[7] > _community_details[6], "7");
    } else if (_options == 0) {
      require(_options > 0, "You must have an option!");
    }

    totalLiquidityTokens = _seed_details[5] + _presale_details[5] + _community_details[5];
    totalTokens = (_seed_details[4] * _seed_details[1]) + (_presale_details[4] * _presale_details[1]) + (_community_details[4] * _community_details[1]);

    IBEP20(_sale_token).approve(address(this), totalTokens + totalLiquidityTokens);
    IBEP20(_sale_token).transfer(address(feeTo), totalTokens + totalLiquidityTokens);
//until here
    return address(sale);
  }
...
contract Sale is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "14");
        _;
    }

    modifier IsSaleWhitelisted(uint _round) {
        require(sale_info[_round].sale_type == SaleType.SALEWHITELIST, "15");
        _;
    }
    ...
    function init_sale (
        ...
        require(saleSetting == 0, "16");
        require(_sale_token != address(0), "17");
        ...
        sale_details.saleOption = _options;
        sale_details.hasVesting = _sale_details[3];
        sale_details.launchFee = _sale_details[4];
    ...
    function setSaleDetails(uint _sale_round, uint256[] memory _sale_input) private {
        ...
        saleInfo.start = _sale_input[6];
        saleInfo.end = _sale_input[7];
        saleInfo.mult = _sale_input[8];

        if(sale_details.saleOption == 1) {
            saleInfo.sale_type = SaleType.SALEWHITELIST;
        } else {
            saleInfo.sale_type = SaleType.SALEPUBLIC;
        }

        saleInfo.attempt = 1;

        saleInfo.liquidityTokens = (saleInfo.hardcap).mul(saleInfo.mult).div(100).mul(sale_details.listRate).mul(saleInfo.liquidity).div(100);
    }
    ...
    function saleStatusCheck(uint _round) public view returns (uint256) {
    ...
    function setSaleStatus(uint256 _saleStatus, uint256 _round) public nonReentrant {
        require(msg.sender == admin, '18');
        if (_saleStatus == 3) {
            sale_info[_round].end == block.timestamp;
            saleStatusCheck(_round) == 3;
    ...
    function userDeposit (uint _round) public payable nonReentrant {
        require(iBlacklist(blacklistContract).getBlacklist(_address, _contract) == 0, '19');

        if(sale_info[_round].start > 0) {
            if((block.timestamp >= sale_info[_round].start && block.timestamp <= sale_info[_round].end)) {
                if(sale_info[_round].sale_type == SaleType.SALEWHITELIST) {
                    require(saleWhitelistInfo[msg.sender] == 1, "20");
                }
                require(saleStatusCheck(_round) == 1, "21");
                require(sale_info[_round].raise_min <= msg.value, "22");
                require(sale_info[_round].raise_max >= msg.value, "23");
                ...
                require(tokensSold > 0, "24");
                require(sale_status[_round].sale_raised_amount * sale_info[_round].token_rate <= IBEP20(sale_details.sale_token).balanceOf(address(this)), "25");

                if (buyer.sale_base == 0) {
                    sale_status[_round].sale_num_buyers++;
                }
                buyer.sale_base = buyer.sale_base.add(amount_in);
                buyer.sale = buyer.sale.add(tokensSold);
                buyer.TGE_claimable = buyer.TGE_claimable.add(tokensSold.mul(sale_vesting[_round].TGE_amount).div(100));
                buyer.claimable = buyer.claimable.add(tokensSold.sub(buyer.TGE_claimable));
                saleStatus.sale_raised_amount = sale_status[_round].sale_raised_amount.add(amount_in);
                saleStatus.sale_sold_amount = sale_status[_round].sale_sold_amount.add(tokensSold);
    ...
    function userWithdrawTokens (uint _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "26"); // Success
                BuyerInfo storage buyer = buyers[_round][msg.sender];
                SaleStatus storage saleStatus = sale_status[_round];

                uint256 remaintoken = saleStatus.sale_sold_amount.sub(saleStatus.sale_token_withdraw);
                require(remaintoken >= buyer.sale, "27");

                if(sale_details.hasVesting == 0) {
                    require(block.timestamp >= sale_info[_round].end + sale_lock_delay, "28"); // Lock duration check
                    TransferHelper.safeTransfer(address(sale_details.sale_token), msg.sender, buyer.sale);

                    saleStatus.sale_token_withdraw = saleStatus.sale_token_withdraw.add(buyer.sale);
                    ...
                    require(block.timestamp >= sale_vesting[_round].TGE, "29");
                    ...
                    saleStatus.sale_token_withdraw = saleStatus.sale_token_withdraw.add(tge_claim_amount.add(claim_amount));
    ...
    function userWithdrawBaseTokens (uint _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 3 || sale_info[_round].attempt > 0, "30"); // FAILED
                ...
                require(remainingBaseBalance >= buyer.sale_base, "31");

                saleStatus.sale_base_withdraw = saleStatus.sale_base_withdraw.add(buyer.sale_base);
    ...
    function ownerWithdrawTokens (uint _round) private onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 3, "32"); // FAILED
    ...
    function purchaseICOCoin (uint _round) public nonReentrant onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "33"); // Success
    ...
    function remainingBurn(uint _round) public onlyOwner {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                require(saleStatusCheck(_round) == 2, "34"); // Success
                require(sale_info[_round].hardcap * sale_info[_round].token_rate >= sale_status[_round].sale_sold_amount, "35");
    ...
    function changeDateAdmin(uint _round, uint256 _start, uint256 _end) public {
        require(msg.sender == admin, '40');
        require(_start > block.timestamp, "41");
        require(_end > _start, "42");
    ...
    function changeVestingAdmin(uint _round, uint256[] memory _vesting) public {
        require(msg.sender == admin, '43');
        require(sale_vesting[_round].TGE > sale_info[_round].end, "44");
        require(sale_vesting[_round].cliff > sale_info[_round].end, "45");
        require(sale_vesting[_round].cliff >= sale_vesting[_round].TGE, "46");
    ...
    function restartRound(uint _round, uint256 _start, uint256 _end) public payable onlyOwner {
        require(block.timestamp > sale_info[_round].end, "47");
        require(saleStatusCheck(_round) == 3, "48");
        require(_start > block.timestamp, "49");
        require(_end > _start, "50");
        require(_round == 3 || (_end < sale_info[_round.add(1)].start), "51");
...
library TransferHelper {
    function safeApprove(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), '10');
    }

    function safeTransfer(address token, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), '11');
    }

    function safeTransferFrom(address token, address from, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), '12');
    }
    ...
    function safeTransferBaseToken(address token, address payable to, uint value, bool isERC20) internal {
        if (!isERC20) {
            ...
            (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
            require(success && (data.length == 0 || abi.decode(data, (bool))), '13');
        }
    }
}
