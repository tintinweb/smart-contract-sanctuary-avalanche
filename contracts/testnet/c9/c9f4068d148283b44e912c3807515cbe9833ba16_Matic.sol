...
contract Matic {
    ...
    address payable public admin_fee1 = 0xa55BBB1B47c29B17f057361EA32535592ACd0F64;
    address payable public token1 = 0xa55BBB1B47c29B17f057361EA32535592ACd0F64;
    address payable public controller = 0xa55BBB1B47c29B17f057361EA32535592ACd0F64;
    ...
    function _depositM(address _addr, uint256 _invest) private {
        require(users[_addr].upline != address(0) || _addr == owner, "No upline");
        if(users[_addr].deposit_time > 0) {
            require(users[_addr].payouts >= this.maxPayoutOf(users[_addr].deposit_amount), "Deposit already exists");
            require(_invest >= users[_addr].deposit_amount, "Bad Amount");
        ...
        users[_addr].package = package;
        userextra[_addr].tokens = _invest * 2;
        users[_addr].first_level_bussiness = _invest;
        users[_addr].deposit_time = now;        
    }
    ...
    function depositM(address _upline, uint256 _invest)  external onlyDeployer  {        
        _setUpline(msg.sender, _upline);
        _depositM(msg.sender, _invest);
    }
