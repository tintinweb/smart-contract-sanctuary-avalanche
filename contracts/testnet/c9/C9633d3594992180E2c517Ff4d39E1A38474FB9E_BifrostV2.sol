...
contract BifrostV2 is Ownable, PaymentSplitter {
    ...
    function claimRewardAll() public{
        ...
        for (uint256 i = 0; i < allTiers.length; i++) {
            ...
            uint256 nodeReward = nodeRewardManager.cashOutAllReward(sender, currentTime);
            cashoutFee += nodeRewardManager._getTierTaxFee(sender, currentTime, cashoutFeeDue, cashoutFeeTax);           
            rewardAmount += nodeReward;
            count++;
        }
        require(rewardAmount > 0, "Enough reward to cash out");
        if(count == 0) count = 1;
...
contract NODERewardManagement {
        ...
        returns (uint256)
    {
        ...
        if(_verifyFeeStatus(nodeIndex)) return 0;
        ...
        if (lastClaim == 0) {
            ...
            uint256 _claims = (endTime.sub(lastClaim)).div(claimTime);
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < nodesIndex.length; i++) {
            if (keccak256(bytes(_nodeList[i].name)) == keccak256(bytes(nodeName))) {
                return false;
            }
        }
        return true;
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 feeTax = cashOutFee;
        for(uint256 i = 0; i < cashoutFeeDue.length; i++){
            if(endTime - node.lastClaimTime <= cashoutFeeDue[i]){
                feeTax = cashoutFeeTax[i];
                return (feeTax, i);
            }
        }
        return (feeTax, cashoutFeeDue.length);
    }
        ...
        returns (uint256)
    {
        ...
        for(uint256 i = 0; i < nodesIndex.length; i++){
            ...
            totalfeeTax = totalfeeTax.add(feeTax);
            countNode++;
            validate[index] = true; 
        }
