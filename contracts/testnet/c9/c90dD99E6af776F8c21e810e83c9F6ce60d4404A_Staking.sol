...
import "./Ownable.sol";
import "./Pausable.sol";
import "./TreasureNode/TRSRNodeManagerUpgradeable.sol";

contract Staking is Ownable, Pausable {

    uint256 public constant minimumToExitPirateAndMarine = 10; // remettre 2 days
    uint256 public constant minimumToExitCorsair = 10; // remettre 1 day
    uint256 public minimumToClaim = 10; // 43200
    uint256 public totalTokenEarned = 0;
    uint public brideMarine = 20;
    uint public brideCorsair = 20;
    uint public maximumTokenStaking = 9950000000 ether;
    uint public burnPirate = 0;
    uint public burnMarine = 0;
    // Daily reward based on level
    uint256[] public rewards = [0,500,1000,1500,2000,2500,250,750];
    ...
    TRSR trsr;
    MintV2 seam;
    TrsrNodesManagerUpgradeable node;
    uint8[] typeSeam;
    uint8[] powerSeam;
    ...
    whenNotPaused
    {
        ...
        require(node.balanceOf(msg.sender) > 0, "You don't have an node TreasureHunt");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (account != address(seam)) {
                require(seam.ownerOf(tokenIds[i]) == account, "Ain't yo token");
    ...
    function setNode(TrsrNodesManagerUpgradeable _newNode)
    public
    onlyOwner
    {
        node = _newNode;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    TRSR trsr;
    // reference to the $PC8 contract
    PC8 pc8;
    // reference to the swapper contract
    SwapperToken swap;
    ...
    modifier checkPermissions(uint256 _trsrnodeId) {
        address sender = _msgSender();
        require(trsrnodeExists(_trsrnodeId),
    ...
    function initialize() external initializer
    {
        ...
        __Pausable_init();
        setBaseUri("ipfs://QmcgF1PEqc9M6B7KR8E71VMf4zqx4fi8yxB6GcqmooHASC/");
        changeNodeMinPrice(42000);
    ...
    function tokenURI(uint256 tokenId) override(ERC721Upgradeable)
    public view
    returns (string memory)
    {
        ...
        string memory linkImage = string (abi.encodePacked(baseURI , uint2str(getLevel(trsrnode.rewardMult))));
        string memory attributes = string(abi.encodePacked("\"attributes\":[{\"trait_type\":\"Locked amount","\",\"value\":\"",uint2str((trsrnode.trsrnodeValue)),"\"},{\"trait_type\":\"Tiers","\",\"value\":\"",uint2str(getLevel(trsrnode.rewardMult)),"\"}]"));
    ...
    function createTrsrNodeWithTokens(string memory trsrnodeName, uint256 trsrnodeValue)
    public payable whenNotPaused verifyName(trsrnodeName)
    returns (uint256)
    {
        address sender = _msgSender();
        require(
        ...
        require(
            msg.value >= fee,
            "Trsrnodes: You don't have enough to pay the fees"
        );
        require(
    ...
    whenNotPaused
    {
        ...
        TrsrNodeEntity storage trsrnode = _trsrnodes[_trsrnodeId];
        require(
            isProcessable(trsrnode),
            "Trsrnodes: Time limit for level up too short"
        );
    ...
    external payable nonReentrant onlyTrsrNodeOwner checkPermissions(_trsrnodeId) whenNotPaused
    {
        require(
            msg.value >= fee,
            "Trsrnodes: You don't have enough to pay the fees"
    ...
    function cashoutAll()
    external payable nonReentrant onlyTrsrNodeOwner whenNotPaused
    {
        address account = _msgSender();
        uint256 rewardsTotal = 0;
        uint256[] memory trsrnodesOwned = getTrsrNodeIdsOf(account);
        require(
            msg.value >= trsrnodesOwned.length * fee,
            "Trsrnodes: You don't have enough to pay the fees"
        );
    ...
    function compoundReward(uint256 _trsrnodeId)
        public payable onlyTrsrNodeOwner checkPermissions(_trsrnodeId) whenNotPaused
    {
        require(
            msg.value >= fee,
            "Trsrnodes: You don't have enough to pay the fees"
    ...
    function compoundAll() external payable nonReentrant onlyTrsrNodeOwner whenNotPaused {
        ...
        require(
            msg.value >= fee * trsrnodesOwned.length,
            "Trsrnodes: You don't have enough to pay the fees"
        );
        ...
        require(
            amountsToCompound > 0,
            "Trsrnodes: No rewards to compound"
        );
        ...
        returns (uint256)
    {
        ...
        require(
            isProcessable(trsrnode),
            "Time limit for cashout too short"
        );
    ...
    function _cashoutReward(uint256 amountToReward)
    private
    {
        require(
            amountToReward > 0,
    ...
    function getTieredRevenues(uint256 mult)
    private view
    returns (uint256) {
        // 1% is 11574
        if (mult >= tierLevel[4]) {
    ...
    function _calculateRewardsFromValue(uint256 _trsrnodeValue, uint256 _rewardMult, uint256 _timeRewards)
    ...
    function trsrnodeExists(uint256 _trsrnodeId)
    private view returns (bool)
    {
        require(
            _trsrnodeId > 0,
            "Trsrnodes: Id must be higher than zero"
        );
    ...
    function isOwnerOfTrsrNodes(address account)
    public view
    returns (bool)
    {
    ...
    function supportsInterface(bytes4 interfaceId)
        public view virtual override(ERC721Upgradeable, ERC721EnumerableUpgradeable)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
