...
contract beta_main is ERC20, Ownable, PaymentSplitter {
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        require(nodeAmount > (nodeAmount.div(denom)*rewardsFee).add(nodeAmount.div(denom)*teamFee).add(nodeAmount.div(denom)*treasuryFee), "fees are too high");
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(address(this),teamPool, nodeAmount.div(denom)*teamFee);
        super._transfer(address(this),rewardsPool, nodeAmount.div(denom)*rewardsFee);
        super._transfer(address(this),treasuryPool, nodeAmount.div(denom)*treasuryFee);
   ...
   function get(address account) public view returns(uint256[3] memory){
	...
	if (tier[2] != 0) {
        ...
        uint get = getEm();
        uint256 feeAmount = rewardAmount.div(denom)*cashoutFee;
