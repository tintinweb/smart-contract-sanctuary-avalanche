...
contract EthernaalNFT is ERC721{
    ...
    //mapping tokenId to asset data
    mapping(uint => AssetData) public assets;
    uint public lazySupply;
    //mapping lazy tokenId to asset data
    mapping(uint => AssetData) public lazyAssets;
    ...
    */
    ...
    function setLazyPrice(uint256 lazyTokenId, uint256 price, uint256 naalPrice) public {
        ...
        require(lazyAssets[lazyTokenId].tokenId == lazyTokenId, "Token ID does not exist");
        require(lazyAssets[lazyTokenId].creator == _msgSender(), "Only creator of the token can set the price");
    ...
    function startLazyMint(
        ...
        lazyAssets[lazyTokenId].isForSale = true;
        return lazyTokenId;
    }
    ...
    function buyLazyTokenWithNaal(uint _lazyTokenId) external{
        ...
        address tokenCreatorAddress = lazyAssets[_lazyTokenId].creator;
    ...
    function _validateBeforeBuy(uint _lazyTokenId, bool payWithNaal, uint payingValue) internal view {
        ...
        require(lazyAssets[_lazyTokenId].creator != address(0), "assets not found or already minted");
        require(lazyAssets[_lazyTokenId].isForSale, "Token is not for sale");
    ...
    function _setAssetData(
        ...
        AssetData memory entry = lazyAssets[lazyTokenId];
        require(entry.creator == address(0), "token already created");
        entry = AssetData(lazyTokenId, creator, title,
            artistName, tokenUri,
            false, nftType, "");
        lazyAssets[entry.tokenId] = entry;
        emit LazyToken(creator, entry.tokenId, tokenUri);
    ...
    function _finishMinting(
        ...
        assets[prefilled.tokenId] = prefilled;

        delete _lazyPrices[lazyTokenId];
        delete lazyAssets[lazyTokenId];
    ...
    */
    function setForSale(uint256 _tokenId, uint256 price, uint naalPrice) public virtual onlyHodlerOf(_tokenId) {
        ...
        require(price > 0, "Price must be greater than 0");
        require(naalPrice > 0, "NAAL Price should be greater than 0");
        require(assets[_tokenId].tokenId == _tokenId, "Token ID does not exist");
        _salePrices[_tokenId].price = price;
        _salePrices[_tokenId].naalPrice = naalPrice;
        assets[_tokenId].isForSale = true;
    ...
    */
    ...
    function _finishSelling(address buyer, bool payWithNaal,
        ...
        require(assets[tokenId].isForSale, "token is not for sale");
        ...
        address tokenCreatorAddress = assets[tokenId].creator;
