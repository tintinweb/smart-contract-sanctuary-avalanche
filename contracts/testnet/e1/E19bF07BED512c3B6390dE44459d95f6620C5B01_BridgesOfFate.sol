...
contract BridgesOfFate is Ownable {
    ...
    mapping(uint256 => uint256) public NarrowingGameMap;
    ...
    bool private isNarrowingGameEnded;
    mapping(bytes32 => bool) private OverFlowPlayerStatus; // change
    // Againest Stage Number and Side set the number of nft's
    mapping(uint256 => mapping(bool => uint256)) public totalNFTOnTitle; //  change 
    ...
    modifier NarrowingGameEndRules() {
        if((_narrowingGameMap  < _NarrowingNumberProbility) && (_dayDifferance(block.timestamp,GameStatusInitialized[latestGameId].startAt) > GameStatusInitialized[latestGameId].lastUpdationDay))
        {
            require(isNarrowingGameEnded == true ,"Hybird Game Ended.");
        }
        _;
    }

    // function getAllNarrowingNumber() external view returns (uint256[] memory,bool) {
    ...
    function _randomFortitleCapacity(uint256 _range) internal view returns (uint) {
    ...
    function generateRandomTicketNumbers(uint256 _lotteryCount,uint256 _range,uint256 _length) internal view returns (uint8[6] memory) {
    ...
    */
    ...
    function _overFlow(bytes32 _playerId,bool _jumpSide) internal  {
        GameStatus memory  _gameStatus = GameStatusInitialized[latestGameId];        
        totalNFTOnTitle[_gameStatus.stageNumber][_jumpSide] = totalNFTOnTitle[_gameStatus.stageNumber][_jumpSide] - 1;

        if(totalNFTOnTitle[_gameStatus.stageNumber][_jumpSide] <= 0){
            OverFlowPlayerStatus[_playerId] =  false;
        }
    }
   
    function calculateBuyBackIn() internal view returns (uint256) {
    ...
    function _distributionReward() internal {
        ...
        // require(GameStatusInitialized[latestGameId].stageNumber == STAGES," Distribution time should not start before reaching final stage.");
            if(GameStatusInitialized[latestGameId].isDistribution){
                ...
                _totalRewardPerUser  = (((_winnerRewarsdsPercent * treasuryBalance())) / 100) / (gameWinners.length);
            }
    }
    ...
    function isDrop(bytes32 _playerID) external view returns (bool) {
        GameStatus storage _gameStatus = GameStatusInitialized[latestGameId];
            if (_dayDifferance(block.timestamp, _gameStatus.startAt) > _gameStatus.lastUpdationDay) {
                OverFlowPlayerStatus[_playerID];
            } 
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if (GameMap[_currentStage] <= 0) {
            ...
            uint256 _titleCapacity =  _randomFortitleCapacity(11 - _globalStageNumber);
            totalNFTOnTitle[_globalStageNumber][true] =  _titleCapacity;
            totalNFTOnTitle[_globalStageNumber][false] =  _titleCapacity;
        ...
        if(_gameStatus.stageNumber == _currentStage){
            if((_narrowingGameMap < _NarrowingNumberProbility ) &&  (_checkSide(_gameStatus.stageNumber,_jumpSide))){
    ...
    function LateBuyInFee(bytes32  _playerId,uint256 _nftId, uint8 seriesType) external GameEndRules NarrowingGameEndRules{
        ...
        _overFlow(playerId,_member.lastJumpSide);
    ...
    /*
        Still in working
        Working on the Random Selction.
    */
    function bulkParticipateInGame(bool _jumpSides, bytes32[] memory playerIds) external  {
        //Select index of NFT's Arrays 
        if(totalNFTOnTitle[GameStatusInitialized[latestGameId].stageNumber][_jumpSides] < playerIds.length){
            playerIndex = generateRandomTicketNumbers(playerIds.length,playerIds.length,totalNFTOnTitle[GameStatusInitialized[latestGameId].stageNumber][_jumpSides]);
        }
        ...
        for (uint256 i = 0; i < playerIds.length; i++) {
            if(PlayerItem[playerIds[0]].stage == PlayerItem[playerIds[i]].stage){
                    ...
                    if((playerIndex[i] > 0)&& (playerIndex[i] == i)){
                         OverFlowPlayerStatus[playerIds[i]] =  true;
                    }else{
    ...
    function switchSide(bytes32 playerId) external  GameEndRules NarrowingGameEndRules{
        ...
        PlayerItem[playerId] = _member;
        _overFlow(playerId,PlayerItem[playerId].lastJumpSide);
        lastUpdateTimeStamp = block.timestamp;
    }
