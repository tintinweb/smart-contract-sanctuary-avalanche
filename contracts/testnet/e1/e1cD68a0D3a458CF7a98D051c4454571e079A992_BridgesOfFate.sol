...
contract BridgesOfFate is Ownable  {
    ...
    uint256 private constant TURN_PERIOD = 600;//10 min // 5 minutes // 3 hours 10800
    ...
    modifier GameEndRules() {
        ...
        require(_dayDifferance(block.timestamp, _gameStatus.startAt) <= (STAGES + THERSHOLD) - 1, "Threshold Achieved!");
        _;
    }
    ...
    function changeCommunityOwnerWA(address[2] calldata _communityOwnerWA) public onlyOwner {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules
    {
        ...
        require(_member.stage != STAGES, "Reached maximum");
        if (GameMap[_member.stage + 1] <= 0 && _gameStatus.stageNumber < STAGES ) {
    ...
    function _distributionReward() internal {
        ...
        require(_gameStatus.stageNumber == STAGES," Distribution time should not start before reaching final stage.");
