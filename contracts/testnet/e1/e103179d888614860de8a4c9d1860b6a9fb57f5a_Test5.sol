...
contract Test5 is IERC20, Auth {
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmountBuy = amount.mul(buyBurnFee).div(feeDenominator);
        uint256 burnAmountSell = amount.mul(sellBurnFee).div(feeDenominator);
        uint256 burnAmountTransfer = amount.mul(transferBurnFee).div(feeDenominator);
        ...
        if (isBuy){
            ...
            _balances[DEAD] = _balances[DEAD].add(burnAmountTransfer);
            emit Transfer(sender, DEAD, burnAmountTransfer);
            ...
            return amount.sub(feeAmount).sub(burnAmountTransfer);
        }
    }
    ...
    function setBuyFees(      
        ...
        require(totalBuyFee < feeDenominator / 100 * 15);       
    }
    ...
    function setSellFees(    
        ...
        require(totalSellFee < feeDenominator / 100 * 20);
    }
    ...
    function setTransFees(     
        ...
        require(totalTransferFee < feeDenominator / 100 * 5);
    }
