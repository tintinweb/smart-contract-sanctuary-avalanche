...
contract metapepRaffle is VRFConsumerBase {
    ...
    constructor(uint256 _ticketCost) 
        VRFConsumerBase(
            0x2eD832Ba664535e5886b75D64C46EB9a228C2610, // VRF Coordinator
    ...
    function endLottery() public onlyAdmin {
        ...
        // Request for a random number from Chainlink VRF
        getRandomNumber();
    }
    ...
    function pickWinner() public onlyAdmin {
        ...
        uint256 index = randomResult % playerSelector.length; //Get a random index
        address winner = playerSelector[index]; //Choose the winner

        // Transfer the NFT to the winner
        ERC721Base(nftContract).transferFrom(admin, winner, tokenId);

        // Reset state variables
        playerSelector = new address[](0); //Reset the playerSelector array
        players = new address[](0); //Reset the players array
        nftContract = address(0); //Reset the NFT contract address
        tokenId = 0; //Reset the NFT token ID
        totalEntries = 0; //Reset the total entries

        //Reset the entryCounts mapping
        for (uint256 i = 0; i < players.length; i++) {
            entryCounts[players[i]] = 0;
        }

        emit Winner(winner); //Emit the event that a winner has been chosen

        // Set randomResultReady back to false
