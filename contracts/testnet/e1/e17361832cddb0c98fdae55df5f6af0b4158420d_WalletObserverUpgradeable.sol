...
import "./implementations/BeansManagerImplementationPointerUpgradeable.sol";
...
contract WalletObserverUpgradeable is
    ...
    BeansManagerImplementationPointerUpgradeable
{
        ...
        returns (bool)
    {
        ...
        require(
            to != address(beansManager),
            "WalletObserver: Cannot send directly tokens to beansManager"
        );
        require(
...
import "../../contracts/interfaces/IBeansManager.sol";

abstract contract BeansManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IBeansManager internal beansManager;

    event UpdateBeansManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyBeansManager() {
        require(
            address(beansManager) != address(0),
            "Implementations: BeansManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(beansManager),
            "Implementations: Not BeansManager"
        );
        _;
    }

    function getBeansManagerImplementation() public view returns (address) {
        return address(beansManager);
    }

    function changeBeansManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(beansManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "BeansManager: You can only set 0x0 or a contract address as a new implementation"
        );
        beansManager = IBeansManager(newImplementation);
        emit UpdateBeansManager(oldImplementation, newImplementation);
    }
