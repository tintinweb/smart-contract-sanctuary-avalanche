...
contract AvaxSavingNew {
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    ...
                    // users[upline].downline.push(Downline())
    ...
    function claimReward() public {
        ...
        //new
        // uint256 overridingAmount = getUserStakeReward(msg.sender);
    ...
    function getStakingRewardCommission(address userAddress)
        public
        view
            ...
            if (users[userAddress].downline[i].length > 0) {
                for (
                    ...
                    totalAmount = totalAmount.add(
                        getUserDividends(users[userAddress].downline[i][k])
                    );
                }
            }
        }
