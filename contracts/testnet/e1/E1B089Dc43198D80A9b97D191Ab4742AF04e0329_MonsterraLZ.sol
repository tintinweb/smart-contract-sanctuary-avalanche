    ...
    NonblockingLzAppUpgradeable
{
    ...
    mapping(bytes32 => ContractInfo) public contractSupportMapping;
    ...
    function setGasAmount(uint256 _gasAmount) public virtual onlyOwner {
    ...
    function bridgeNft(
        ...
        bytes32 keyMapping = _getContractSymbolHash(_srcToken);
        require(
            contractSupportMapping[keyMapping].status &&
                contractSupportMapping[keyMapping].isERC721,
            "Contract-not-supported"
        );
        require(
            IERC721Monsterra(_srcToken).ownerOf(_tokenId) == _msgSender(),
            "Insufficient-token-balance"
        );

        (uint256 estimatedNativeFee, ) = estimateFee(
            _dstChainId,
            _srcToken,
            _tokenId
        );
        require(
            msg.value >= estimatedNativeFee,
            "Insufficient-fee-destination"
        );

        IERC721Monsterra(_srcToken).burn(_tokenId);
        bytes memory payload = abi.encode(keyMapping, _tokenId, _msgSender());
        _lzSend(
            _dstChainId,
            payload,
            payable(msg.sender),
    ...
    function bridgeFt(
        ...
        require(
            IERC20Monsterra(_srcToken).balanceOf(_msgSender()) >= _amount,
            "Insufficient-token-balance"
        );
        ...
        IERC20Monsterra(_srcToken).transferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        IERC20Monsterra(_srcToken).burn(_amount);

        bytes memory payload = abi.encode(keyMapping, _amount, _msgSender());
        _lzSend(
            _dstChainId,
            payload,
            payable(msg.sender),
    ...
    function _nonblockingLzReceive(
        ...
        (bytes32 keyMapping, uint256 data, address to) = abi.decode(
            _payload,
        ...
        if (contractSupportMapping[keyMapping].isERC721) {
            IERC721Monsterra(contractSupportMapping[keyMapping].token).mint(
                to,
                data,
                "",
                0 // Type: not use
            ...
            IERC20Monsterra(contractSupportMapping[keyMapping].token).mint(
                to,
                data
            );
        }
    ...
    function estimateFee(
        ...
        bytes32 keyMapping = _getContractSymbolHash(_srcToken);
        bytes memory payload = abi.encode(keyMapping, _data, _msgSender());
        return
            lzEndpoint.estimateFees(
                ...
                payload,
                false,
                _getAdapterParams() // _adapterParams
            );
    }
    ...
    function addTokenSupport(
        ...
        bytes32 keyMapping = _getContractSymbolHash(_srcToken);
        require(!contractSupportMapping[keyMapping].status, "Contract-added");
        ContractInfo memory contractInfo;
        contractInfo.token = _srcToken;
        contractInfo.isERC721 = _isERC721;
        contractInfo.status = true;
        contractSupportMapping[keyMapping] = contractInfo;
    }

    function removeTokenSupport(address _srcToken) external onlyOwner {
        bytes32 keyMapping = _getContractSymbolHash(_srcToken);
        contractSupportMapping[keyMapping].status = false;
    }

    function isTokenSupport(
        address _srcToken
    ) external view returns (bool, bool, bytes32) {
        bytes32 keyMapping = _getContractSymbolHash(_srcToken);
        return (
            contractSupportMapping[keyMapping].status,
            contractSupportMapping[keyMapping].isERC721,
    ...
    function _getContractSymbolHash(
        address _token
    ) internal view returns (bytes32) {
        // get the symbol of the token
        bytes memory _baseBytes = bytes(ITokenMonsterra(_token).symbol());
        // upper case the Symbol of token if need
        for (uint i = 0; i < _baseBytes.length; i++) {
            if (_baseBytes[i] >= 0x61 && _baseBytes[i] <= 0x7A) {
                _baseBytes[i] = bytes1(uint8(_baseBytes[i]) - 32);
            }
        }
        return keccak256(abi.encodePacked(_baseBytes));
    }

    function _getAdapterParams() internal view returns (bytes memory) {
        uint16 version_ = 1;
        return abi.encodePacked(version_, gasAmount);
    }
}
...
interface IERC721Monsterra is IERC721Upgradeable{
    ...
    function ownerOf(uint256 tokenId) external view returns (address owner);
    function mint(address _to, uint256 _tokenId, string calldata _data, uint8 _type) external;
}
