...
abstract contract BaseParaSwapAdapter is FlashLoanReceiverBase, Ownable {
  ...
  function _pullATokenAndWithdraw(
    ...
    // withdraw reserve
    require(
      LENDING_POOL.withdraw_v2(reserve, amount, address(this), address(0), 0) == amount,
...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  function _pullAToken(
    ...
    // withdraw reserve
    LENDING_POOL.withdraw_v2(reserve, amount, address(this), address(0), 0);
  }
...
contract ParaSwapLiquiditySwapAdapter is BaseParaSwapSellAdapter, ReentrancyGuard {
  ...
  function swapAndDeposit(
    ...
    LENDING_POOL.deposit_v2(address(assetToSwapTo), amountReceived, msg.sender, 0);
  }
  ...
  function _swapLiquidity (
    ...
    assetToSwapTo.safeApprove(address(LENDING_POOL), 0);
    assetToSwapTo.safeApprove(address(LENDING_POOL), amountReceived);
    LENDING_POOL.deposit_v2(address(assetToSwapTo), amountReceived, initiator, 0);
...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 0; vars.i < assetToSwapFromList.length; vars.i++) {
      ...
      LENDING_POOL.deposit_v2(assetToSwapToList[vars.i], vars.receivedAmount, msg.sender, 0);
    }
  }
  ...
  function _swapLiquidity(
    ...
    LENDING_POOL.deposit_v2(assetTo, vars.receivedAmount, initiator, 0);
...
contract UniswapRepayAdapter is BaseUniswapAdapter {
  ...
  function swapAndRepay(
    ...
    LENDING_POOL.repay_v2(debtAsset, amountToRepay, debtRateMode, msg.sender, address(0));
  }
  ...
  function _swapAndRepay(
    ...
    LENDING_POOL.repay_v2(debtAsset, amount, rateMode, initiator, address(0));
...
interface ILendingPool {
  ...
  event TransferBorrowedFunds(address to, address aToken, uint256 amount);
  event TransferWithdrawedFunds(address to, address aToken, uint256 amount);
  ...
  function mintDepositATokens(address onBehalfOf, address asset, uint256 amount) external;
  ...
  function withdraw_v2(
    ...
    uint256 amount,
    address to,
    address assetAnotherChain,
    uint16 _dstChainId
  ) external payable returns (uint256);

  function transferWithdrawedFunds(address to, address asset, uint256 amount) external;
  ...
  function transferBorrowedFunds(address to, address aToken, uint256 amount) external;
  ...
  function setIsRootChain(bool val) external;
...
interface ILayerZeroMessenger {
    ...
    function initialize(address _lendingPool, uint16 _rootChainId) external;

    // check if message received
    function getReceivedMessage() external view returns (bytes memory);
    ...
    function repay(
      uint16 _dstChainId,
      RepayFund memory repayFund,
      address refundAddress
    ) external payable;

    function encodePayload(address onBehalfOf, address asset, uint256 amount) external view returns (bytes memory);

    function estimateFees(
      uint16 _dstChainId,
      bool _userApplication,
...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function depositETH(
    ...
    ILendingPool(lendingPool).deposit_v2(address(WETH), msg.value, onBehalfOf, referralCode);
  }
  ...
  function withdrawETH(
    ...
    ILendingPool(lendingPool).withdraw_v2(address(WETH), amountToWithdraw, address(this), address(0), 0);
  ...
  function repayETH(
    ...
    ILendingPool(lendingPool).repay_v2(address(WETH), msg.value, rateMode, onBehalfOf, address(0));
  ...
  function borrowETH(
    ...
    ILendingPool(lendingPool).borrow_v2(
      address(WETH),
      amount,
      interesRateMode,
      referralCode,
...
contract LayerZeroDemo is NonblockingLzApp {
    event ReceiveMsg(
        uint16 _srcChainId,
        bytes _payload
    );
    ...
    BorrowedFund[] public borrowedFunds;

    constructor(address _lzEndpoint) public NonblockingLzApp(_lzEndpoint) {}

    function initialize(address _lendingPool) public {
      lendingPool = LendingPool(_lendingPool);
    }

    // check if message received
    function getReceivedMessage() public view returns (bytes memory) {
      return receivedMessage;
    }

    // check the borrow request
    function getBorrowedFunds() public view returns (BorrowedFund[] memory) {
    ...
    function borrow(
      ...
      borrowedFunds.push(BorrowedFund(onBehalfOf, asset, amount));
      bytes memory payload = abi.encode(BorrowedFund(onBehalfOf, asset, amount));
      _lzSend(_dstChainId, payload, payable(refundAddress), address(this), bytes(""), msg.value);
      emit SendMsg(_dstChainId, payload);
    }

    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _from, uint64, bytes memory _payload) internal override {
        (BorrowedFund memory borrowedFund) = abi.decode(_payload, (BorrowedFund));
        lendingPool.transferBorrowedFunds(borrowedFund.receiver, borrowedFund.asset, borrowedFund.amount);
        receivedMessage = _payload;
        emit ReceiveMsg(_srcChainId, _payload);
    }

    function encodePayload(address onBehalfOf, address asset, uint256 amount) external view returns (bytes memory) {
      return abi.encode(BorrowedFund(onBehalfOf, asset, amount));
    }

    function estimateFees(uint16 _dstChainId, bool _userApplication, bytes calldata _adapterParams, bytes calldata payload) public view returns (uint nativeFee, uint zroFee) {
        return lzEndpoint.estimateFees(_dstChainId, address(this), payload, _userApplication, _adapterParams);
    }
}
...
pragma experimental ABIEncoderV2;
import {NonblockingLzApp} from './NonblockingLzApp.sol';
import {ILayerZeroMessenger} from '../../interfaces/layerzero/ILayerZeroMessenger.sol';
import {LendingPool} from '../lendingpool/LendingPool.sol';


contract LayerZeroMessenger is ILayerZeroMessenger, NonblockingLzApp {
    // packet type
    uint8 public constant PT_DEPOSIT = 0;
    uint8 public constant PT_WITHDRAW = 1;
    uint8 public constant PT_BORROW = 2;
    uint8 public constant PT_REPAY = 3;
    ...
    function initialize(address _lendingPool, uint16 _rootChainId) public override {
      lendingPool = LendingPool(_lendingPool);
      rootChainId = _rootChainId;
    }

    function getRootChainId() public view returns (uint16) {
      return rootChainId;
    }

    // check if message received
    function getReceivedMessage() public view override returns (bytes memory) {
      return receivedMessage;
    }
    ...
    function deposit(
      ...
      bytes memory payload = abi.encode(PT_DEPOSIT, Fund(onBehalfOf, asset, amount));
      _lzSend(rootChainId, payload, payable(refundAddress), address(this), bytes(""), msg.value);
      emit SendMsg(rootChainId, payload);
    }
    ...
    function borrow(
      ...
      bytes memory payload = abi.encode(PT_BORROW, Fund(onBehalfOf, asset, amount));
      _lzSend(_dstChainId, payload, payable(refundAddress), address(this), bytes(""), msg.value);
      emit SendMsg(_dstChainId, payload);
    }
    ...
    function withdraw(
      ...
      bytes memory payload = abi.encode(PT_WITHDRAW, Fund(onBehalfOf, asset, amount));
      _lzSend(_dstChainId, payload, payable(refundAddress), address(this), bytes(""), msg.value);
      emit SendMsg(_dstChainId, payload);
    }
    ...
    function repay(
      ...
      bytes memory payload = abi.encode(PT_REPAY, repayFund);
      _lzSend(_dstChainId, payload, payable(refundAddress), address(this), bytes(""), msg.value);
      emit SendMsg(_dstChainId, payload);
    }
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _from, uint64, bytes memory _payload) internal override {
        ...
        if (packetType == PT_DEPOSIT) {
          (uint16 packetType, Fund memory depositReceived) = abi.decode(_payload, (uint16, Fund));
          lendingPool.mintDepositATokens(depositReceived.receiver, depositReceived.asset, depositReceived.amount);
          receivedMessage = _payload;
          emit ReceiveMsg(_srcChainId, _payload);
        } else if (packetType == PT_BORROW) {
          (uint16 packetType, Fund memory borrowFund) = abi.decode(_payload, (uint16, Fund));
          lendingPool.transferBorrowedFunds(borrowFund.receiver, borrowFund.asset, borrowFund.amount);
          receivedMessage = _payload;
          emit ReceiveMsg(_srcChainId, _payload);
        } else if (packetType == PT_WITHDRAW) {
          (uint16 packetType, Fund memory withdrawFund) = abi.decode(_payload, (uint16, Fund));
          lendingPool.transferWithdrawedFunds(withdrawFund.receiver, withdrawFund.asset, withdrawFund.amount);
          receivedMessage = _payload;
          emit ReceiveMsg(_srcChainId, _payload);
        } else if (packetType == PT_REPAY) {
          (uint16 packetType, RepayFund memory repayFund) = abi.decode(_payload, (uint16, RepayFund));
          lendingPool.burnDebtTokens(repayFund.receiver, repayFund.asset, repayFund.amount, repayFund.rateMode);
          receivedMessage = _payload;
          emit ReceiveMsg(_srcChainId, _payload);
        } else {
            revert("LayerZeroMessenger: unknown packet type");
        }
    }

    function encodePayload(address onBehalfOf, address asset, uint256 amount) external override view returns (bytes memory) {
      return abi.encode(Fund(onBehalfOf, asset, amount));
    }
    ...
    function estimateFees(
      ...
      bytes calldata _adapterParams,
      bytes calldata payload
    ) public override view returns (uint nativeFee, uint zroFee) {
        return lzEndpoint.estimateFees(_dstChainId, address(this), payload, _userApplication, _adapterParams);
    }
}
...
contract LZEndpointMock is ILayerZeroEndpoint {
    ...
    constructor(uint16 _chainId) public {
    ...
    function estimateFees(uint16 _dstChainId, address _userApplication, bytes memory _payload, bool _payInZRO, bytes memory _adapterParams) override public view returns (uint nativeFee, uint zroFee) {
        bytes memory adapterParams = _adapterParams.length > 0 ? _adapterParams : defaultAdapterParams;
...
abstract contract NonblockingLzApp is LzApp {
    ...
    constructor(address _endpoint) LzApp(_endpoint) public {}
...
import {LayerZeroMessenger} from '../layerzerodemo/LayerZeroMessenger.sol';
import {ILayerZeroMessenger} from '../../interfaces/layerzero/ILayerZeroMessenger.sol';
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  LayerZeroMessenger layerZeroMessenger;
  ...
  modifier onlyLayerZeroMessenger() {
    _onlyLayerZeroMessenger();
    _;
  }
  ...
  function _onlyRootChain() internal view {
    require(_isRootChain, Errors.LP_NOT_ROOT_CHAIN);
  }

  function _onlyLayerZeroMessenger() internal view {
    require(msg.sender == address(layerZeroMessenger));
  }
  ...
  function setLayerZeroMessengerAddress(address _layerZeroMessenger) public {
    layerZeroMessenger = LayerZeroMessenger(_layerZeroMessenger);
  }
  ...
  function deposit_v2(
    ...
    if (_isRootChain) {
      mintDepositATokens(onBehalfOf, asset, amount);
      emit Deposit(asset, msg.sender, onBehalfOf, amount, referralCode);
    } else {
      layerZeroMessenger.deposit.value(msg.value)(onBehalfOf, asset, amount, msg.sender);
    }
  }

  function mintDepositATokens(
    address onBehalfOf,
    address asset,
    uint256 amount
  ...
  function withdraw_v2(
    ...
    address assetAnotherChain,
    uint16 _dstChainId
  ) external payable override whenNotPaused onlyRootChain returns (uint256) {
    DataTypes.ReserveData storage reserve = _reserves[asset];
    ...
    if (_dstChainId == 0 || _dstChainId == layerZeroMessenger.getRootChainId()) {
      IAToken(aToken).burn(msg.sender, to, amountToWithdraw, reserve.liquidityIndex);
    } else {
      layerZeroMessenger.withdraw.value(msg.value)(
        _dstChainId,
        to,
        assetAnotherChain,
        amountToWithdraw,
  ...
  function transferWithdrawedFunds(
    address to,
    address asset,
    uint256 amount
  ) external override onlyLayerZeroMessenger {
    emit TransferWithdrawedFunds(to, asset, amount);
    DataTypes.ReserveData storage reserve = _reserves[asset];
    IAToken(reserve.aTokenAddress).transferUnderlyingTo(to, amount);
  }
  ...
  function borrow_v2(
    ...
    address assetAnotherChain,
    uint16 _dstChainId
  ) external payable override whenNotPaused onlyRootChain {
    DataTypes.ReserveData storage reserve = _reserves[asset];

    _executeBorrowV2(
      ExecuteBorrowParamsV2(
        asset,
        msg.sender,
        onBehalfOf,
        amount,
  ...
  function _executeBorrowV2(ExecuteBorrowParamsV2 memory vars) internal {
    ...
    if (vars._dstChainId == 0 || vars._dstChainId == layerZeroMessenger.getRootChainId()) {
      if (vars.releaseUnderlying) {
        IAToken(vars.aTokenAddress).transferUnderlyingTo(vars.user, vars.amount);
      }
    } else {
      layerZeroMessenger.borrow.value(msg.value)(
        vars._dstChainId,
        vars.onBehalfOf,
        vars.assetAnotherChain,
        vars.amount,
        vars.user
      );
    }
  ...
  function transferBorrowedFunds(
    address to,
    address asset,
    uint256 amount
  ) external override onlyLayerZeroMessenger {
    emit TransferBorrowedFunds(to, asset, amount);
    DataTypes.ReserveData storage reserve = _reserves[asset];
    IAToken(reserve.aTokenAddress).transferUnderlyingTo(to, amount);
  }
  ...
  // first validate repay and get the paybackAmount
  // function validate_repay_v2(
  //   address asset,
  //   uint256 amount,
  //   uint256 rateMode,
  ...
  function repay_v2(
    ...
    if (_isRootChain) {
      burnDebtTokens(asset, onBehalfOf, paybackAmount, rateMode);
    } else {
      layerZeroMessenger.repay.value(msg.value)(
        layerZeroMessenger.getRootChainId(),
        ILayerZeroMessenger.RepayFund(onBehalfOf, assetAnotherChain, paybackAmount, rateMode),
        msg.sender
      );
    }
    emit Repay(asset, onBehalfOf, msg.sender, paybackAmount);
  ...
  function setIsRootChain(bool val) external override onlyLendingPoolConfigurator {
    _isRootChain = val;
  }

  struct ExecuteBorrowParamsV2 {
    address asset;
    address user;
    address onBehalfOf;
    uint256 amount;
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
  ...
  function setIsRootChain(bool val) external onlyEmergencyAdmin {
    pool.setIsRootChain(val);
  }
...
library Errors {
  ...
  string public constant LP_NOT_ROOT_CHAIN = '81';
