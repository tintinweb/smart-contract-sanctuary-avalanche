...
contract VoyagerUtils is AccessControl, ILiquidityPool {
    ...
    constructor(address WETH) AccessControl() {
        _WETH = WETH;
    ...
    function stakeNative() external payable override {
        ...
        address _weth = _WETH;
        require(msg.value > 0, "insufficient native tokens provided");
        IWETH(_weth).deposit{ value: _amount }();
        require(IWETH(_weth).transfer(address(_reserve), _amount));
        _reserve.stakeETH(msg.sender, _weth, _amount);
    ...
    /// @param amount Amount that needs to be unstaked.
    /// @param isNative if the token being unstaked is native token for the chain
    function unstake(address tokenAddress, uint256 amount, bool isNative) public override {
        if (isNative) {
            _reserve.unstakeETH(msg.sender, tokenAddress, amount, _WETH);
            return;
        }
    ...
    function setReserve(address reserveAddress) public virtual onlyRole(RESOURCE_SETTER) {
