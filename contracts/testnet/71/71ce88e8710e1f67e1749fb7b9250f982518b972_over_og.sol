  ...
  function getTokenPrice() public view returns(uint[7] memory)
   {
    ...
    uint[7] memory ret = [res0,res1,boo,now_val,low_val,high_val,i];
    return ret; // return amount of token0 needed to buy token1
   }
   function process() public onlyAuthorized whenNotPaused returns (uint[2] memory) {
    uint[7] memory tok_range = getTokenPrice();
