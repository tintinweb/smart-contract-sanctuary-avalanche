...
contract GNFT is Context, AccessControl, ERC721 {
...
contract GToken is Context, ERC20 {
    constructor() public ERC20("GToken", "GT1") {
        _mint(_msgSender(), 10000 * (10**uint256(decimals())));
    }
}

// File: contracts\GMarketplace.sol
contract GMarketplace {
    GToken private _token;
    GNFT private _gnft;

    address private _organiser;

    constructor(GToken token, GNFT gnft) public {
        _token = token;
        _gnft = gnft;
        _organiser = _gnft.getOrganiser();
    }
    ...
    function purchaseTicket() public {
        ...
        _token.transferFrom(buyer, _organiser, _gnft.getTicketPrice());

        _gnft.transferTicket(buyer);
    }

    // Purchase ticket from the secondary market hosted by organiser
    function secondaryPurchase(uint256 ticketId) public {
        address seller = _gnft.ownerOf(ticketId);
        address buyer = msg.sender;
        uint256 sellingPrice = _gnft.getSellingPrice(ticketId);
        ...
        _gnft.secondaryTransferTicket(buyer, ticketId);

        emit Purchase(buyer, seller, ticketId);
    }
}

// File: contracts\GTicketsFactory.sol
contract GTicketsFactory is Ownable {
    struct GEvent {
        string eventName;
        string eventSymbol;
        uint256 ticketPrice;
    ...
    mapping(address => GEvent) private activeEventsMapping;
    ...
    function createNewEvent(
        ...
        GMarketplace newMarketplace =
            new GMarketplace(token, newEvent);

        address newEventAddress = address(newEvent);

        activeEvents.push(newEventAddress);
        activeEventsMapping[newEventAddress] = GEvent({
            eventName: eventName,
