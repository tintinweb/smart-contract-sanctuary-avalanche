...
contract Vault is ERC20, Router, IYetiVaultToken, ReentrancyGuard {
    using SafeTransferLib for IERC20;

    // Min swap to rid of edge cases with untracked rewards for small deposits. 
    uint256 constant public MIN_SWAP = 1e5;
    ...
    uint256 public immutable underlyingDecimal; //decimal of underlying token
    ERC20 public immutable underlying; // the underlying token
    ...
    function pushRewardToken(address _token) public onlyOwner {
        require(address(_token) != address(0), "Cannot push 0 address");
    ...
    function deposit(uint256 _amt) public nonReentrant returns (uint256 receiptTokens) {
        require(_amt > 0, "Cannot deposit 0 tokens");
        ...
        emit Deposit(msg.sender, msg.sender, _amt, receiptTokens);
    }
    ...
    function depositFor(address _borrower, uint256 _amt) external nonReentrant override returns (uint256 receiptTokens) {
        require(msg.sender == BOpsAddress, "Only BOps can call this function");
        require(_amt > 0, "Cannot deposit 0 tokens");
        // Reinvest if it has been a while since last reinvest
        if (block.timestamp > lastReinvestTime + maxReinvestStale) {
            _compound();
    ...
    function withdraw(uint256 _amt) public nonReentrant returns (uint256 underlyingTokens) {
        require(_amt > 0, "Cannot withdraw 0 tokens");
        if (block.timestamp > lastReinvestTime + maxReinvestStale) {
            _compound();
        }
    ...
    function emergencyWithdraw(uint256 _amt)
        public nonReentrant
    ...
    function withdrawFor(
        ...
        address _from,
        address _to
    ) public nonReentrant override returns (uint256 underlyingTokens) {
        require(_amt > 0, "Cannot withdraw 0 tokens");
        if (block.timestamp > lastReinvestTime + maxReinvestStale) {
            _compound();
        }
    ...
    function withdrawForWithPermit(
        uint256 _amt,
        address _from,
        address _to,
        uint256 _value,
        ...
        return withdrawFor(_amt, _from, _to);
    }
