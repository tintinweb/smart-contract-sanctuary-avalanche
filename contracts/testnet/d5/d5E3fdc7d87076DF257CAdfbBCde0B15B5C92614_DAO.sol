...
contract DAO {
    ...
    function getPollings() public view returns (Polling[] memory) {
        Polling[] memory pollings = new Polling[](totalPolling);
        for (uint256 i = 0; i < totalPolling; i++) {
            pollings[i] = s_pollingRegistry[i];
        }
    ...
    function updatePollings() public {
        for (uint256 i = 0; i < totalPolling; i++) {
            ...
            if (
                polling.pollingState == PollingState.Open &&
                block.timestamp - polling.pollingStartTime >= polling.interval
            ) {
