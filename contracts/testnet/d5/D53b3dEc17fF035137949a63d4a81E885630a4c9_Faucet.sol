...
contract Faucet is Ownable, Withdrawable {
  ...
  function requestTokens() public {
    // toolsContract.externalMint(msg.sender, 1, 1, 999);
    ...
    itemsContract.externalMint(msg.sender, 7, 10);
    itemsContract.externalMint(msg.sender, 8, 30);
    itemsContract.externalMint(msg.sender, 9, 20);
    itemsContract.externalMint(msg.sender, 10, 10);

    goldContract.externalMint(msg.sender, 1000);
    console.log("request tokens ok");
...
contract Tools is ERC721, Ownable, Withdrawable, ExternalActor {
  Utils.BonusType BonusType;
  Utils.BonusType[] public bonusTypes;
  ...
  struct Tool {
    ...
    uint256 speed;
    uint256 sockets;
    Utils.Bonuses bonuses;
  }

  mapping(uint256 => ToolType) public _toolTypes;
  mapping(uint256 => Tool) public _toolDetails;

  event Minted(address indexed _owner, bytes32 indexed _requestId, uint256 _tokenId, Tool _tool);
  ...
  constructor(string memory name, string memory symbol) ERC721(name, symbol) {
    ...
    bonusTypes.push(Utils.BonusType("Stone", 1, 75, 40));
    bonusTypes.push(Utils.BonusType("Iron", 2, 50, 20));
    bonusTypes.push(Utils.BonusType("Mithril", 3, 25, 10));
    bonusTypes.push(Utils.BonusType("Ruby", 4, 10, 10));
    bonusTypes.push(Utils.BonusType("Emerald", 5, 10, 10));
    bonusTypes.push(Utils.BonusType("Sapphire", 6, 10, 10));
    bonusTypes.push(Utils.BonusType("Diamond", 7, 10, 10));
  }
  ...
  function externalMint(
    ...
    for (uint256 i = 1; i <= amount; i++) {
      console.log("minting toolType:");
      console.logUint(toolType);
      mintTool(requester, requestId, toolType, randomBase, i);
    }
  }
  ...
  function mintTool(
    ...
    uint256 rarity = Utils.getWeightedRarity(randomBase, saltBase + 1 + nextId);
    uint256 speed = Utils.getWeightedSpeed(randomBase, saltBase + 2 + nextId, rarity, toolType);
    // console.log("nextId:");
    // console.log(nextId);
    // console.log("saltBase:");
    // console.log(saltBase);
    // console.log("rarity:");
    // console.log(rarity);
    ...
    _toolDetails[nextId] = Tool(
      ...
      rarity,
      speed,
      Utils.getWeightedSockets(randomBase, saltBase + 3 + nextId, rarity),
      Utils.getWeightedBonuses(randomBase, saltBase + 4 + nextId, rarity, bonusTypes)
    );

    _safeMint(requester, nextId);
    emit Minted(requester, requestId, nextId, _toolDetails[nextId]);
    nextId++;
  }
  ...
  function getMetadata(uint256 tokenId) public view returns (string memory) {
    string memory first = string(
      abi.encodePacked(
        ...
        '", "type": "',
        String.toString(_toolDetails[tokenId].toolType),
        '", "rarity": "',
        String.toString(_toolDetails[tokenId].rarity),
        '", "speed": "',
        String.toString(_toolDetails[tokenId].speed),
        '", "sockets": "',
        String.toString(_toolDetails[tokenId].sockets),
        '", "background_color" : "101922", "image": "ipfs://',
    ...
    string memory second = string(
      abi.encodePacked(
        ...
        '", "ruby": "',
        String.toString(_toolDetails[tokenId].bonuses.ruby),
        '", "emerald": "',
        String.toString(_toolDetails[tokenId].bonuses.emerald),
        '", "sapphire": "',
        String.toString(_toolDetails[tokenId].bonuses.sapphire),
        '", "diamond": "',
        String.toString(_toolDetails[tokenId].bonuses.diamond),
        '"}}'
      )
    );

    // if (_toolDetails[tokenId].bonuses.stone > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"stone": "',
    //       String.toString(_toolDetails[tokenId].bonuses.stone),
    //       _toolDetails[tokenId].bonuses.iron > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.iron > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"iron": "',
    //       String.toString(_toolDetails[tokenId].bonuses.iron),
    //       _toolDetails[tokenId].bonuses.mithril > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.mithril > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"mithril": "',
    //       String.toString(_toolDetails[tokenId].bonuses.mithril),
    //       _toolDetails[tokenId].bonuses.gold > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.gold > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"gold": "',
    //       String.toString(_toolDetails[tokenId].bonuses.gold),
    //       _toolDetails[tokenId].bonuses.ruby > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.ruby > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"ruby": "',
    //       String.toString(_toolDetails[tokenId].bonuses.ruby),
    //       _toolDetails[tokenId].bonuses.emerald > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.emerald > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"emerald": "',
    //       String.toString(_toolDetails[tokenId].bonuses.emerald),
    //       _toolDetails[tokenId].bonuses.sapphire > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.sapphire > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"sapphire": "',
    //       String.toString(_toolDetails[tokenId].bonuses.sapphire),
    //       _toolDetails[tokenId].bonuses.diamond > 0 ? '",' : ""
    //     )
    //   );
    // }
    // if (_toolDetails[tokenId].bonuses.diamond > 0) {
    //   second = string(
    //     abi.encodePacked(
    //       second,
    //       '"diamond": "',
    ...
    // for (uint256 i = 0; i < _toolDetails[tokenId].bonuses.length; i++) {
    //   if (i == 0) {
    //     second = string(abi.encodePacked(second, '"'));
    //   } else {
    //     second = string(abi.encodePacked(second, '", "'));
    //   }
    ...
    //     abi.encodePacked(
    //       second,
    //       _toolDetails[tokenId].bonuses[i].name,
    //       '": "',
    //       String.toString(_toolDetails[tokenId].bonuses[i].bonus)
    //     )
    //   );

    //   if (i == _toolDetails[tokenId].bonuses.length - 1) {
    //     second = string(abi.encodePacked(second, '"}}'));
    //   }
    // }
...
library Utils {
  ...
  function getRandomSafe(uint256 randomNumber, uint256 salt) internal pure returns (uint256) {
  ...
  function getRandomSafe(
    ...
    return (uint256(keccak256(abi.encode(randomNumber, salt))) % max) + 1;
  }

  function getWeightedRarity(uint256 randomNumber, uint256 salt) internal pure returns (uint256) {
    uint256 rarity = getRandomSafe(randomNumber, salt);
  ...
  function getWeightedSpeed(
    uint256 randomNumber,
    uint256 salt,
    uint256 rarity,
    uint256 toolType
  ) internal pure returns (uint256) {
    return ((getRandomSafe(randomNumber, salt, 10000) * rarity * toolType) / 8) / 100;
  }

  function getWeightedSockets(
    uint256 randomNumber,
    uint256 salt,
    uint256 rarity
  ) internal pure returns (uint256) {
    uint256 sockets = getRandomSafe(randomNumber, salt);
  ...
  function getPoolRange(BonusType[] memory bonusTypes) internal pure returns (uint256) {
    uint256 poolRange = 0;

    for (uint256 i = 0; i < bonusTypes.length; i++) {
      poolRange += bonusTypes[i].weight;
    }
  ...
  function getWeightedRoll(
    BonusType[] memory bonusTypes,
    uint256 randomNumber,
    uint256 salt
  ) internal pure returns (uint256 rolledIndex) {
    uint256 poolRange = getPoolRange(bonusTypes);
    uint256 roll = getRandomSafe(randomNumber, salt, poolRange);
    ...
    for (uint256 i = 0; i < bonusTypes.length; i++) {
      ...
      currentWeight += bonusTypes[i].weight;
    }

    return rollIndex;
  }

  function getWeightedBonuses(
    uint256 randomNumber,
    uint256 salt,
    uint256 rarity,
    BonusType[] memory bonusTypes
  ) internal view returns (Bonuses memory) {
    uint256 maxBonuses = rarity;
    Bonuses memory bonuses = Bonuses(0, 0, 0, 0, 0, 0, 0, 0);
    Utils.BonusType[] memory bonusTypesMod = bonusTypes;

    for (uint256 i = 0; i < maxBonuses; i++) {
      uint256 rolledIndex = Utils.getWeightedRoll(
        bonusTypesMod,
        randomNumber,
        salt + i + 1 + block.number
      );

      uint256 bonusRoll = Utils.getRandomSafe(
        randomNumber,
        salt + i + 2 + block.number,
        bonusTypesMod[rolledIndex].maxBonus
      );

      if (rolledIndex == 0) {
        bonuses.stone = bonusRoll;
      } else if (rolledIndex == 1) {
        bonuses.iron = bonusRoll;
      } else if (rolledIndex == 2) {
        bonuses.mithril = bonusRoll;
      } else if (rolledIndex == 3) {
        bonuses.gold = bonusRoll;
      } else if (rolledIndex == 4) {
        bonuses.ruby = bonusRoll;
      } else if (rolledIndex == 5) {
        bonuses.emerald = bonusRoll;
      } else if (rolledIndex == 6) {
        bonuses.sapphire = bonusRoll;
      } else if (rolledIndex == 7) {
        bonuses.diamond = bonusRoll;
      }
    }
