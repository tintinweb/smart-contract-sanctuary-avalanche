...
interface BSNT {
    function smallNodeBalanceOf(address address_) external view returns (uint256);

    function getSmallNodeInfo(uint256 id) external view returns (uint256, uint256, bool, bool, uint256, uint256);
}
...
contract SharkMigrate {
    ...
    Counters.Counter private _smallNodesCounter;
    ...
    function countNodes() public view returns (uint256){
        return _smallNodesCounter.current();
    }
    ...
    function transferNodes() public {
        uint256 count = _oldGameContract.smallNodeBalanceOf(msg.sender);

        (uint256 lastClaim_, uint256 createdAt, bool burned, bool active, uint256 id, uint256 rewards) = _oldGameContract.getSmallNodeInfo(1);
        _lastClaim = createdAt;
    }
}
