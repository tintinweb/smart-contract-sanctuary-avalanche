...
contract TestV4 is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC2981, ClampedRandomizer {
    constructor() 
        ERC721("testnet", "testv4") ClampedRandomizer(maxSupply) {
    ...
    mapping(address => bool) public allowListed; //used for redrobes
    //mapping(address => uint) private whiteListedCount;
    mapping(address => bool) public whiteListed;
    //mapping(address => uint) private teamcount;
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        require(block.timestamp >= whitelistTime, "Its not time to mint Pyros yet");
        ...
        if (block.timestamp < publicTime) //white list mint 
        {
            require(whiteListed[msg.sender] ==true, "Silly monkey Pyros are for WL only right now");
            if (price > 0) 
                {
                    require(msg.value >= wlprice * amount * 0.01 ether, "Send more money plz ser");
                }
        ...
        for (uint i=1; i<=amount;i++) {    
            ...
            if (supply < mintpointThreshold)// example if current supply is 0 and threshold is 250, you are first minter and get 250points,
            {
                ...
                points[msg.sender] +=int((mintpointThreshold-supply+1-i)/10); 
            }
        ...
        {
                points[msg.sender] += redRobesSupporterBonus;
        }
    }
    ...
    function setWLPrice(uint _price) public onlyOwner {
        wlprice=_price;
    }  
    ...
    function sumbitPokerHand(uint256 [] memory _tokenIds) public returns(uint [] memory hand, uint firstMatch, uint secondMatch, bool flush, bool straight, bool royalFlush,uint jokers) {

        require(_tokenIds.length <=5 ,"Too Many cards submitted");
        require(_tokenIds.length >1 ,"Too Few cards submitted");
        ...
        sortedsuits=sort_array(sortedsuits);
        
        //Check for duplicates
        for (uint i= 0; i < _tokenIds.length -1 ; i++){
                uint j=i+1;
                if(sortedvalues[i]!=15){
        ...
        for (uint i= 0; i < _tokenIds.length -1 ; i++){
            //for (uint j= i+1; i <= _tokenIds.length -1 ; j++){
            uint j=i+1;
            if(sortedvalues[i]!=15){
                if (firstMatch == 0){
        ...
        if (secondMatch>firstMatch){
            secondMatch=secondMatch+jokers;
        }
        else{
            if(firstMatch==0){
                firstMatch++;
            }
            firstMatch=firstMatch+jokers;
        ...
        if (_tokenIds.length == 5){    
            ...
            for (uint i= 0; i < _tokenIds.length; i++){
                ...
                if(sortedsuits[0]==sortedsuits[i]){
            ...
            uint jokersRemaining=jokers;
            //check for straight
            for (uint i= 0; i < _tokenIds.length-1; i++){
                uint j=i+1;
                //if(sortedvalues[i]==15){//account for jokers
                //    if(i==4){
                        //straight
                ///        straight=true;
                //        break;
                ...
                if(sortedvalues[4]==14){//account for ace being low and high
                    if((i==0)&&(sortedvalues[1]==2)){
                        continue;    
                    }
                    if((i==3)&&(sortedvalues[3]==5)){
                        //straight
                        straight=true;
                        break;
                    }
                ...
                if(sortedvalues[i]+1==sortedvalues[j]){
                   if(i==3){
                       //straight
                        straight=true;
                        break;
            ...
            if((sortedvalues[0]==10) && (straight && flush)){
                royalFlush=true;
            }    
        }
        updateParticipants(msg.sender);
        if (royalFlush==true){points[msg.sender] +=100;}//Royal Flush
        else if(straight==true && flush==true){points[msg.sender] +=90;}//Straight flush
        else if(firstMatch==4){points[msg.sender] +=80;}//4 of a kind 
        else if((firstMatch==3 && secondMatch==2)||(firstMatch==2 && secondMatch==3)){points[msg.sender] +=70;}//full house
        else if(flush==true){points[msg.sender] +=60;} //flush
        else if(straight==true){points[msg.sender] +=50;}//straight
        else if(firstMatch==3){points[msg.sender] +=40;}// 3 of a kind
        else if(firstMatch==2 && secondMatch==2){points[msg.sender] +=30;}//two pair
        else if(firstMatch==2){points[msg.sender] +=10;}//pair
        else {revert("no valid hand detected");}
        //burn(_tokenIds);
        return (sortedvalues, firstMatch, secondMatch, flush, straight, royalFlush, jokers);
