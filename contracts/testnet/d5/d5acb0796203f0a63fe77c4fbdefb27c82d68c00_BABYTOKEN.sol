...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    function setNewTracker (address _NewTracker) external onlyOwner {
                dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(_NewTracker))
...
function setInitialFees () external onlyOwner {
     tokenRewardsFee = 3;
        liquidityFee = 1;
        marketingFee = 1;
}
    function setInitialConfiguration () external onlyOwner {
    ...
    function setRouterandPair(address _NewRouter) external onlyOwner {
