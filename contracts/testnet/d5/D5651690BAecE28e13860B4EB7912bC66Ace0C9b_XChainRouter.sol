...
contract XChainRouter is IAny2EVMMessageReceiver, IERC165, OwnableUpgradeable {
    error InvalidConfig();
    ...
    address public ovToken;

    function initialize(address router, address _ovToken) public initializer {
    ...
    function disableChain(uint64 chainId) external onlyOwner {
    ...
    function ccipReceive(
        ...
        address userAddress = abi.decode(message.data, (address));
        require(userAddress != address(0), "Invalid sender address");

        if (
            message.tokenAmounts.length > 0 &&
            message.tokenAmounts[0].amount > 0 &&
            message.tokenAmounts[0].token == ovToken
        ) {
            IERC20(message.tokenAmounts[0].token).transfer(
                userAddress,
    ...
    function sendTokens(
    ...
    ) external payable validChain(destChainId) {
        require(
            tokenAmounts.length == 1 && tokenAmounts[0].token == ovToken,
            "Invalid token"
        );
        ...
        uint256 fee = i_router.getFee(destChainId, message);
        require(msg.value >= fee, "Insufficient fee");

        bytes32 messageId = i_router.ccipSend{value: fee}(destChainId, message);
