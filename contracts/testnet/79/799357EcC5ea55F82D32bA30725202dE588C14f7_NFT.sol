  ...
  Pausable
{
    ...
    whenNotPaused
  {
    ...
    if (minted < PAID_TOKENS) {
      ...
      require(balanceOf(_msgSender()) + staking.getTokensOf(_msgSender()).length + amount <= 10, "Don't sell your house to buy our NFT please");
    } else {
      require(msg.value == 0);
    }
    ...
    uint256 sponsorTokenCount = balanceOf(sponsor) +  staking.getTokensOf(_msgSender()).length;
...
contract Staking is Ownable, IERC721Receiver, Pausable {
  ...
  function claimManyFromStaking(uint[] calldata tokenIds, bool unstake)
    ...
    _updateEarnings
  {
    require(msg.sender == tx.origin || tx.origin == address(this) || _msgSender() == address(this), "Only Externally Owned Account");
    require(canClaim, "Claim is not currently possible");
    ...
    returns (uint256 owed)
  {
    ...
    require(stake.owner == _msgSender() || _msgSender() == address(this), "Not your properties");
    require(
