...
contract MintV2 is ERC721Enumerable, Ownable, RoyaltiesImpl, Pausable {
    using Strings for uint256;
    using Counters for Counters.Counter;
    TRSR public trsr;
    TreasureHuntGame public treasureHuntGame;
    ...
    uint public priceToBuyTRSR = 40000; // 40 000 $trsr
    uint public nftSwappable = 2500;
    uint public compensation = 5000; // 5000 $trsr
    uint[] public tokenIdSwap;
    ...
    constructor(address _trsr, address _old, string memory _baseUri, address _royalties, uint _startTime) ERC721("SeaMan", "SEAM")
    {
        setBaseURI(_baseUri);
        setAddressRoyalties(_royalties);
        setStartTime(_startTime);
        trsr = TRSR(_trsr);
        treasureHuntGame = TreasureHuntGame(_old);
    ...
    whenNotPaused
    {
        uint256 supply = nftSwappable + _tokenIdCounter.current();
        ...
        require(supply + mintAmount <= maxSupply, "Oh no all Seam is sold");

        if (msg.sender != owner()) {
            require(msg.value >= mintAmount * fees, "Insufficient amount to pay tax");
            require(trsr.balanceOf(msg.sender) >= (priceToBuyTRSR * mintAmount) * 10**18, "Sorry you do not have sufficient funds");
        }

        for (uint256 i = 1; i <= mintAmount; i++) {
            uint256 tokenIdMint = nftSwappable + _tokenIdCounter.current();
            if (msg.sender != owner())
                trsr.burn(msg.sender, priceToBuyTRSR * 10**18);
            _safeMint(to,tokenIdMint);
            // royalties fixed 5%
            setRoyalties(tokenIdMint, royaltiesAddressPayable,500);
            _tokenIdCounter.increment();
        }
    }

    function swapSeam(uint[] memory tokenIds)
    public
    {
        require(tokenIds.length > 0 , "Select at least one Seam");
        require(treasureHuntGame.balanceOf(msg.sender) > 0, "You do not have any nft in your wallet");
        uint256 totalCompensation = 0;
        for(uint8 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < nftSwappable, "Unallocated Seam");
            require(_findElement(tokenIds[i], msg.sender), "I don't have your Seam");
            treasureHuntGame.burn(tokenIds[i]);
            _mint(msg.sender, tokenIds[i]);
            _tokenIdCounter.increment();
            setRoyalties(tokenIds[i], royaltiesAddressPayable, 500);
            tokenIdSwap.push(tokenIds[i]);
            nftSwappable -= 1;
            totalCompensation += compensation;
        }
        if (compensation > 0)
            trsr.mint(msg.sender, totalCompensation * 10**18);
    }


    function swapSeamTeam(uint[] memory tokenIds, address owner)
    public
    onlyOwner
    {
        require(tokenIds.length > 0 , "Select at least one Seam");
        ...
        for(uint8 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < nftSwappable, "Unallocated Seam");
            require(_findElement(tokenIds[i], owner), "I don't have your Seam");
            treasureHuntGame.burn(tokenIds[i]);
    ...
    function _findElement(uint _tokenId, address owner)
    internal
    returns (bool)
    {
        uint[] memory tokenIdsOld = treasureHuntGame.walletOfOwner(owner);
        for(uint i=0;i < tokenIdsOld.length; i++) {
            if (tokenIdsOld[i] == _tokenId) {
                return true;
            }
        }
        return false;
    ...
    external
    {
        require(controllers[msg.sender], "Only controllers can burn Seam");
        _burn(tokenId);
    }
    ...
    function setPriceToBuyTRSR (uint _newPrice)
    public
    onlyOwner
    {
        priceToBuyTRSR = _newPrice;
    }

    function setPriceCompensation (uint _newPrice)
    public
    onlyOwner
    {
        compensation = _newPrice;
    }
