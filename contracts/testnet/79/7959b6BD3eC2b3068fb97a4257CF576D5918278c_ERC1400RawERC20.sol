 ...
 /**
   ...
   function updateFromExchange(address _investor, uint256 _balance) public onlyAdmin returns(bool) {
    ...
    function bulkMint(address[] calldata to,uint256[] calldata amount,bytes calldata cert) external{
      ...
      function addPropertyOwnersShares(uint256[] calldata _share,address[] calldata _owners) 
        external
        onlyAdmin{
 
        require(
            _share.length == _owners.length,
            "invalid shares array"
        );
        uint256 total;
        ...
        require(total == 10000,"percentages sum should be 100");
        for(uint256 i; i < length; ){
            ownerToShares[_owners[i]] =  _share[i];
            _propertyOwners.push(_owners[i]);
            unchecked {
              i++;
            }
        }
