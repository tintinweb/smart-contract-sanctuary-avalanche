...
contract HopeExchange is Ownable {
    ...
    /**
     * @dev Deposit Token B and receive Hope in return
     */
    function deposit(address variant, uint256 amount) public payable {
        ...
        uint256 vestingSchedule = tokenBVariants[variant].vestingSchedule;
        ...
        // Calculate amount of Hope to be credited to user
        ...
        uint256 tokenBAmount = amount.mul(10**hopeDecimals).div(
            10**tokenBDecimals
        );
        uint256 hopeReceived = tokenBAmount.mul(exchangeRate).div(
            10**hopeDecimals
        );
        ...
        // Credit user's balance with Hope
        balances[msg.sender] = UserBalances({
            balance: balances[msg.sender].balance.add(hopeReceived),
            vestingSchedule: vestingSchedule,
            vestingStartTime: block.timestamp,
            vestingUpdateTime: block.timestamp
        });
    ...
    function claim() public {
        ...
        uint256 vestedAmount = getVestedAmount(sender);
        require(vestedAmount > 0, "No vested amount to claim");

        balances[msg.sender].balance = balances[msg.sender].balance.sub(
            vestedAmount
        );
        balances[msg.sender].vestingUpdateTime = block.timestamp;

        IERC20(Hope).safeTransfer(sender, vestedAmount);
        emit Claim(sender, vestedAmount);
    }
        ...
        returns (uint256 vestedAmount)
    {
        ...
        uint256 userBalance = balances[user].balance;

        if (vestingSchedule > 0 && userBalance > 0) {
            uint256 vestingStartTime = balances[user].vestingUpdateTime;
    ...
    function addTokenBVariant(
        ...
        require(
            tokenB != address(Hope),
            "Token B variant cannot be the same as Hope."
        );
        require(
        ...
        require(
            exchangeRate > 1 * (10**tokenBDecimals),
    ...
    // Allows the owner to update the deposit fee.
