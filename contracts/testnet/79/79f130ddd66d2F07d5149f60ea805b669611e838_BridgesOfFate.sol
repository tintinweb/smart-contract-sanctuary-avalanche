...
contract BridgesOfFate is Ownable {
    ...
    //     uint256 _packedNumber = uint256(keccak256(abi.encodePacked(block.timestamp, address(this), msg.sender)));
    //     uint256 _number = uint256ToArray(_packedNumber, _range);
    //     _number = _number + 1;
    //     return (_number * 1e9);
    // }
    ...
    uint256 private _packedNumber = 0;

    function generateRandomNumbersForGame(uint256 _range,uint256 _length) internal  returns (uint8[2] memory) {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            while (!readyToAdd && retry <= maxRetry) {
                _packedNumber = uint256(keccak256(abi.encodePacked(msg.sender,block.timestamp,i,retry)));
    ...
    function generateRandomTicketNumbers(uint256 _range,uint256 _length) public view returns (uint8[] memory) {
    ...
    function _overFlow(bytes32 _playerId,uint256 jumpAtStage,bool _jumpSide) internal returns (bool) {
        ...
        bool isSafeSide = GameMap[jumpAtStage - 1] >= 50;
    ...
    function buyBackInFee(
        ...
        _member.lastJumpSide = GameMap[_member.stage] >= 50;
    ...
    function participateInGame(bool _jumpSide,bytes32 playerId) public GameEndRules NarrowingGameEndRules {
        ...
        if (GameMap[_currentUserStage] <= 0) {
            ...
            GameMap[_globalStageNumber] = _packedRandomNumber[0];
            _gameStatus.stageNumber = _globalStageNumber;
            _gameStatus.lastUpdationDay = currentDay;
            // uint256 _packedNumberNarrowingGameMap = uint256(keccak256(abi.encodePacked(block.timestamp, address(this), msg.sender))) % 100;
            _narrowingGameMap = _packedRandomNumber[1];
...
//        }
    ...
    function _selectPlayerAndSetCapacityOnTile(uint256 _currentPlayerStage,uint256 currentDay,bool _jumpSide) internal {
        ...
        if (_currentPlayerStage > 0) {
            ...
            if (_selectJumpedSidePlayersId.length > _titleCapacityAgainestStage) {
                ...
                _randomSelectPlayerIndex = generateRandomTicketNumbers(_selectUnholdPlayerId.length - 1,_titleCapacityAgainestStage);
    ...
    function _generateTitleCapacity(uint256 _stageNo) internal returns (uint256) {
        if (_stageNo > 1) {
            ...
            bool previousJumpSide = GameMap[_stageNo] >= 50; // Side Stage-#-1
