...
contract SmartVault {
   ...
   * @param token - ERC20 token address.
   * @param unstakeAmount: Pass -1 to fully close debt position, otherwise Amount to be repaid with a flashloan
   * @param paybackAmount: Pass -1 to fully close debt position, otherwise Amount to be repaid with a flashloan
   * @param _vault: The vault address where the debt position exist.
   */
  function manualPayback(
    IERC20 token, 
    uint256 unstakeAmount,
    int256 paybackAmount,
    address _vault
  ) external noReentrant {
    unstakeTokens(token, unstakeAmount);
    IVault(_vault).payback(paybackAmount);
  }
}