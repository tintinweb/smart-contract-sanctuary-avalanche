...
contract arb is Ownable{
    ...
    function swap(address[] memory path , address pair, uint256 amount, uint256 amount_outMin, uint256 reserve0Min, uint256 reserve1Min, uint256 amoutCalculated) external onlyOwner() {
        ...
        if(getBalanceOfToken(path[0]) < amount){
            ...
            if(reserve0 < reserve0Min || reserve1 < reserve1Min){
                ...
                (uint256 amount0Out, uint256 amount1Out) = path[0] == token0 ? (uint(0), amoutCalculated) : (amoutCalculated, uint(0));
