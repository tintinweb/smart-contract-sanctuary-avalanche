...
interface ITaxProcessor {
    function distributeBondProceeds(uint _amount) external;
}
...
contract OlympusBondDepository is Ownable {
    ...
    address public immutable taxProcessor; // processes bond fees
    ...
    struct Terms {
        ...
        uint minimumPriceUSD; // min price in USD of the bond
    ...
    constructor (
        ...
        DAO = _DAO;
        require( _taxProcessor != address(0) );
    ...
    function initializeBondTerms(
        ...
        terms = Terms ({
            ...
            minimumPriceUSD: _minimumPriceUSD,
    ...
    enum PARAMETER { VESTING, PAYOUT, FEE, DEBT, MINIMUM_PRICE, DEPOSITS_ENABLED }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            ...
            terms.minimumPriceUSD = _input;
        } else if ( _parameter == PARAMETER.DEPOSITS_ENABLED ) { // 5
    ...
    function deposit(
        ...
        if ( fee != 0 ) {
            IERC20( OHM ).safeTransfer( taxProcessor, fee);
    ...
    function bondPrice() public view returns ( uint price_ ) {
      ...
      uint priceUSD = bondPriceInUSD(price_);
      if ( priceUSD < terms.minimumPriceUSD ) {
          price_ = price_.mul(terms.minimumPriceUSD).div(priceUSD);
      }
    }
     ...
     *  @return price_ uint
     */
    function bondPriceInUSD(uint _bondPrice) public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = _bondPrice.mul( IBondCalculator( bondCalculator ).markdown( principle ) ).div( 100 );
        } else {
            price_ = _bondPrice.mul( 10 ** IERC20( principle ).decimals() ).div( 100 );
        }
    }
