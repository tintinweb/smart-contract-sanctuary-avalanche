...
contract ZKL is ERC20Capped, ERC20Permit, Ownable, IZKL {
    ...
    bool public constant IS_MINT_CHAIN = false;
    uint256 public constant CAP = 1000000000 * 1e18;

    struct BridgeInfo {
        address bridge;
        bool enableBridgeTo; // We can disable bridge when `bridge` is compromised
    }
    ...
    event UpdateBridge(uint256 indexed bridgeIndex, bool enableBridgeTo);
    ...
    constructor() ERC20("ZKLINK", "ZKL") ERC20Capped(CAP) ERC20Permit("ZKLINK") {}
    ...
    function updateBridge(uint256 index, bool enableBridgeTo) external onlyOwner {
        require(index < bridges.length, "Invalid index");
        BridgeInfo memory info = bridges[index];
        if (info.enableBridgeTo != enableBridgeTo) {
            info.enableBridgeTo = enableBridgeTo;
            bridges[index] = info;
            emit UpdateBridge(index, enableBridgeTo);
        }
    }
    ...
    function bridgeTo(address spender, address from, uint16 dstChainId, bytes memory to, uint256 amount, uint64 nonce) external override {
        ...
        BridgeInfo memory info = bridges[index];
        require(info.bridge == bridge, "Require bridge");
        require(info.enableBridgeTo, "Bridge to disabled");
    ...
    function bridgeFrom(uint16 srcChainId, address receiver, uint256 amount, uint64 nonce) external override {
        ...
        BridgeInfo memory info = bridges[index];
        require(info.bridge == bridge, "Require bridge");
        // always enable bridge from the src chain even though `enableBridgeTo` is disabled at the src chain
        // this may happen when upgrade bridge contract to a new version
        // but user's bridge tx of old version has not been submitted to this dst chain yet
