...
contract beta_main is ERC20, Ownable, PaymentSplitter {
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256 teamTokens = nodeAmount.mul(teamPoolFee).div(100);
        uint256 rewardsPoolTokens = nodeAmount.mul(rewardsFee).div(100);
        uint256 treasuryTokens = nodeAmount.mul(treasuryFee).div(100);
        super._transfer(address(this),teamPool,teamTokens);
        super._transfer(address(this),rewardsPool,rewardsPoolTokens);
        super._transfer(address(this),treasury,treasuryTokens);
        super._transfer(sender, address(this), nodeAmount.sub(treasuryTokens).sub(rewardsPoolTokens).sub(teamTokens));
