...
contract DonationDrive is ERC721 {
    ...
    bytes32 private _randomSecret;

    uint16 public _boxesLeft;
    uint8 MAX_POP_PER_CALL = 10;

    string public _baseUri;
    IERC20 public _noSec;
    address public _owner;
    bool public _isPromotionLive;

    // Stores mapping w.r.t price in BNB => chances of winning
    mapping(uint256 => RewardBoxChances[]) rewardBoxChances;
    // actual amount of boxes users will have
    ...
    // _baseUri + uint8 -> Image of the NFT
    mapping(uint256 => uint8) public nftIdRarity;
    mapping(uint256 => uint8) public nftIdVariation;
    ...
    constructor(
        address __owner,
        address __noSec,
        uint16 _boxesAvailable,
        string memory __baseUri,
        string memory __secret
    ) ERC721("NOSEC DonationDrive", "NDD") {
        _noSec = IERC20(__noSec);
        _owner = __owner;
        _baseUri = __baseUri;
        _randomSecret = keccak256(abi.encodePacked(__secret));
        _boxesLeft = _boxesAvailable;
    }

    function fund(uint256 _amount) public onlyOwner whenPromotionNotLive {
        _noSec.transferFrom(msg.sender, address(this), _amount);
    }

    // Andrew: Add funds in emergency case
    ...
    function random() private returns (uint16) {
        ...
        return uint16(randomHash % 10000);
    }
    ...
    function donate() public payable whenPromotionIsLive {
        ...
        uint16 _boxBalanceChange = chances[winningIndex].boxRewardsPerRoll;
        require(_boxesLeft > _boxBalanceChange, "Not Enough boxes left");

        RewardBoxChances memory chance = rewardBoxChances[donation][
            winningIndex
        ];
        _boxesLeft -= _boxBalanceChange;
        prizeBoxesState[chance.rewardType].minted += _boxBalanceChange;
        rewardBoxBalances[msg.sender][chance.rewardType] += _boxBalanceChange;
        payable(address(this)).transfer(donation);
    }

    function popRewardBox(
        DonationRewardBoxType _rewardBoxType
    ) public whenPromotionIsLive {
        require(
        ...
        if (chance.rewardsTokens) {
            _noSec.transfer(msg.sender, chance.tokenRewards);
        }
    }
    ...
    function rewardBoxBalanceOf(
        DonationRewardBoxType _type
    ) public view returns (uint256) {
        return rewardBoxBalances[msg.sender][_type];
    }
    ...
    function tokenURI(
        ...
        return string(abi.encodePacked(baseUrl, rarity, bytes("/"), variation));
    }

    function extractFunds(uint256 _amount) public payable onlyOwner {
