...
contract XZeus is Ownable {
    ...
    struct PenaltyInfo {
        ...
        uint256 penalty; // Penalty percent
    }
    ...
    function getPenalty(uint256 _amount) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < penaltyInfo.length; i++) {
            if (_amount <= penaltyInfo[i].criteria)
                return penaltyInfo[i].penalty;
        }

        return penaltyInfo[penaltyInfo.length - 1].penalty;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && stakingTokenSupply != 0) {
            ...
            accZeusPerShare = accZeusPerShare.add(
                zeusReward.mul(1e12).div(stakingTokenSupply)
            );
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 feeAmount = (_amount * getPenalty(_amount)) / 10000;
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = user.amount.mul(pool.accZeusPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            rewardToken.safeTransferFrom(
