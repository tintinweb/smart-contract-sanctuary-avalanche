...
contract SolarisV2 is ERC20 {
   ...
   address contractAddress;
	...
	address operationPoolAddress;
    address tresuryPoolAddress;
    ...
    constructor() ERC20("SOLARIS", "SOLV2") {
        //__ERC20_init("TEST TOKEN", "TTT");
        _mint(msg.sender, 1000000e18);
        
        owner = msg.sender;
        operationPoolAddress = 0xdD25C9bec0925aF0Ee1A83fF10Ee477b8Cb76E62;
        tresuryPoolAddress = 0xa2755Af8DA746E40Cc93aa1bF103a8031E74DD22;
        transferTaxRate = 0;
        operationPoolFee = 0;
        ...
        _isFeeExcluded[contractAddress] = true;
        ...
        //IJoeRouter02 _dexRouter = IJoeRouter02(0x2D99ABD9008Dc933ff5c0CD271B88309593aB921); //pangolin fuji
        //dexRouter = _dexRouter;
 
        address currentRouter = 0x2D99ABD9008Dc933ff5c0CD271B88309593aB921;

        dexRouter = IJoeRouter02(currentRouter); // fuji pangoling
        //lpPair = IFactoryV2(dexRouter.factory()).createPair(dexRouter.WAVAX(), contractAddress);
        //lpPairs[lpPair] = true;
	...
	function setRatios(uint16 _operationRatio, uint16 _treasuryRatio) external onlyOwner {
    ...
    function contractMustSwap(address from, address to, uint256 contractTokenBalance) internal view returns (bool) {
        return contractTokenBalance >= swapThreshold &&
        !inSwap &&
        from != lpPair &&
        this.balanceOf(lpPair) > 0 &&
        !_isFeeExcluded[to] &&
        !_isFeeExcluded[from] &&
        contractSwapEnabled;
    }
    ...
    */
    ...
    function getPairAddressV2(address add1, address add2) public view returns (address) {
        ...
        return IUniswapV2Factory(dexRouter.factory()).getPair(add1, add2);
    }
   ...
   /**
    ...
    function createPair() public onlyOwner() {
        lpPair = IUniswapV2Factory(dexRouter.factory()).createPair(dexRouter.WAVAX(), contractAddress);
        lpPairs[lpPair] = true;
    }

    function createPairv2(address add1) public onlyOwner() {
        lpPair = IUniswapV2Factory(dexRouter.factory()).createPair(dexRouter.WAVAX(), add1);
        lpPairs[lpPair] = true;
    }
    ...
    function updateLimitsEnabled(bool newValue) external onlyOwner {
        limitsEnabled = newValue;
    }
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal {
        if (sniperProtection){
        ...
        uint256 contractTokenBalance = this.balanceOf(contractAddress);
        ...
        // SWAP
        if (contractMustSwap(from, to, contractTokenBalance)) {
        ...
        if (takeFee) {
            ...
            if (feeAmount > 0) {
                ...
                _transfer(from, contractAddress, feeAmount);
            }
        }
    ...
    function calculateWhaleFee(uint256 amount) public view returns (uint256) {
        ...
        uint256 avaxAmount = getOutEstimatedTokensForTokens(contractAddress, swapTokenAddress, amount);
        uint256 liquidityRatioAmount = getOutEstimatedTokensForTokens(contractAddress, swapTokenAddress, getReserves()[0]);

        // if amount in busd exceeded the % setted as whale, calc the estimated fee
        if (avaxAmount >= ((liquidityRatioAmount * whaleFeePercent) / staticVals.masterTaxDivisor)) {
            // mod of busd amount sold and whale amount
            uint256 modAmount = avaxAmount % ((liquidityRatioAmount * whaleFeePercent) / staticVals.masterTaxDivisor);
    ...
    function calcBuySellTransferFee(address from, address to, uint256 amount) internal view returns (uint256) {
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (super.allowance(contractAddress, address(dexRouter)) != type(uint256).max) {
            super.approve(address(dexRouter), type(uint256).max);
        }
        
        // swap
        address[] memory path = getPathForTokensToTokens(contractAddress, swapTokenAddress);
        _approve(contractAddress, address(dexRouter), numTokensToSwap);
    ...
    function sendToPools() internal {
        ...
        if (newBalance > 0) {
            ...
            uint256 treasuryAmount = (newBalance * _ratios.treasuryRatio) / (_ratios.total);
            if (treasuryAmount > 0) {
                payable(tresuryPoolAddress).transfer(treasuryAmount);
                emit TransferedToPool(tresuryPoolAddress, treasuryAmount);
            }

            uint256 operationAmount = (newBalance * _ratios.operationRatio) / (_ratios.total);
            if (operationAmount > 0) {
                payable(operationPoolAddress).transfer(operationAmount);
                emit TransferedToPool(operationPoolAddress, operationAmount);
            }
    ...
    function _updateliquidityRatioAdd() external onlyOwner() {
        _liqAddBlock = block.number;
        _liquidityRatioHolders[msg.sender] = true;
        hasLiqBeenAdded = true;
        contractSwapEnabled = true;
    }
    ...
    */
    ...
    function swapTokensForAVAX(uint256 tokenAmount) private {
        ...
        _approve(contractAddress, address(dexRouter), tokenAmount);
    ...
    //     dexRouter = _dexRouter;
    //     //_approve(contractAddress, address(dexRouter), balanceOf(contractAddress));
    //     uniswapV2Pair = IUniswapV2Factory(_dexRouter.factory()).createPair(contractAddress, _dexRouter.WAVAX());
    //     require(balanceOf(contractAddress) > 0, "Must have Tokens on contract to launch");
    //     require(address(this).balance > 0, "Must have AVAX on contract to launch");
    //     addLiquidity(balanceOf(contractAddress), address(this).balance);
    //     //enableTrading();
    ...
    function withdrawStuckTokens(uint256 amount) public onlyOwner {
        require(this.balanceOf(contractAddress) > 0, "Contract balance is zero");
        if (amount > this.balanceOf(contractAddress)) {
            amount = this.balanceOf(contractAddress);
        }

        super._transfer(contractAddress, msg.sender, amount);
    }
}
