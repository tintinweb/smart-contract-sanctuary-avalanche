...
contract TestUpgradeable is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC20BurnableUpgradeable {
   ...
   function withdraw(uint256 amount) public onlyOwner {
      require(IERC20Upgradeable(address(proxyAddress)).balanceOf(address(proxyAddress)) >= amount, "Withdraw: Insufficent balance.");
      require(proxyAddress != address(0), "Call withdraw to transfer from the zero address.");
      IERC20Upgradeable(address(proxyAddress)).transfer(address(msg.sender), amount);
   }
}
