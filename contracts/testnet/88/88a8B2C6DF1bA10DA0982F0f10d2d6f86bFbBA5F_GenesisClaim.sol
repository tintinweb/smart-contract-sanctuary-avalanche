...
contract GenesisClaim is Ownable, Pausable, Multicall {
	 ...
	 */
	uint256 public maximumTotalAllocation;
	...
	function purchase(uint256 _nftsToPurchase) external whenNotPaused {
		// uint256 supply_query = totalPurchased.add(_nftsToPurchase);
		// require(totalPurchased.add(_nftsToPurchase) <= maximumTotalAllocation, "Exceeds maximum supply!");
	...
	function _purchase(uint256 _nftsToPurchase) internal {
		...
		if (purchasable == 0) {
			...
			require(purchasable >= _nftsToPurchase, 'Purchase more than total allocation.');
			...
			if (info.credit > 0){
				if (requested_total > info.credit) {
					/// @notice Had 4*350 credit
					/// @notice Asked for 6 NFT
					/// @notice 6 NFT * publicMint = request total
					/// @notice Request total - 4*350 credit / publicMint should be less than 2

					// All nfts after credited nfts revert to public price
					require(requested_total.sub(info.credit).div(publicMint) <= 2, "Remaining NFTs for purchase after credit applied cannot exceed 2!");
					// Zero out credit 
	...
	/**
	 * @notice Max remaining for Whitelist Mint
	...
	*/
	function setPublicMint(uint256 _price) external onlyOwner {
