...
interface KYCMultiSigInterface {
    ...
    function createGlobalAuthorizationProposal(bool status_, uint256 expiration_) external;

    // approve global authorization proposal
    function approveGlobalAuthorizationProposal(uint256 ProposalIndex_) external;
    ...
    function IsGlobalAuthorizationProposalApprovers(uint256 ProposalIndex_, address account_)
        external
        view
        returns (bool);
...
contract KYCMultiSig is KYCMultiSigInterface {
    ...
    mapping(uint256 => GlobalAuthorizationProposal) private _GlobalAuthorizationProposals;

    // kyc manager proposal approvers: proposal index => address => status
    mapping(uint256 => mapping(address => bool)) private _GlobalAuthorizationProposalApprovers;
        ...
        onlyGreaterThanZero(expiration_)
    {
        ...
        // create proposal
        _GlobalAuthorizationProposals[_ProposalIndex] = GlobalAuthorizationProposal({
            ID: _ProposalIndex,
        ...
        // approve the porposal by KYC manager sender
        _GlobalAuthorizationProposalApprovers[_ProposalIndex][msg.sender] = true;
        ...
        if (_Signatories.length == 1) {
            ...
            // UDPATE EXECUTED TIME STAMP
            _GlobalAuthorizationProposals[_ProposalIndex].EXECUTEDTIMESTAMP = block
                .timestamp;
    ...
    {
        // proposal info
        GlobalAuthorizationProposal storage proposal = _GlobalAuthorizationProposals[ProposalIndex_];
        ...
        require(
            !(proposal.ISEXECUTED ||
                proposal.ISREVOKED ||
                proposal.EXPIRATION > block.timestamp ||
                _GlobalAuthorizationProposalApprovers[ProposalIndex_][msg.sender]),
            "KYC Multi-Sig: Proposal should not be executed, expired, revoked, or approved by sender!"
        );

        // update proposal approved by kyc manager sender
        _GlobalAuthorizationProposalApprovers[ProposalIndex_][msg.sender] = true;
