...
contract clawMachine is ConfirmedOwner, VRFConsumerBaseV2 {
        ...
        ConfirmedOwner(msg.sender)
    {
        ...
        _price = 10 * 10**15; //price in milli
        paused = false;
    }

    event win(uint prize, address user);
    ...
    {
        require(requests[requestIds[currentRequestIndex]].exists, "request not found");
        RequestStatus memory request = requests[requestIds[currentRequestIndex]];
        randomWord = request.randomWords[currentRngIndex];
        if (currentRngIndex < numWords-1) {
             currentRngIndex++;
        }else{
            currentRngIndex = 0;
            require(requestIds.length - 1 > currentRequestIndex, "no more randomness");
            currentRequestIndex++;
    ...
    function minted3() public view returns (uint256) {
        return contract3.totalSupply();
    }
    ...
    function withdrawERC20(address addy) external onlyOwner {
        IERC20(addy).transferFrom(
            address(this),
            msg.sender,
            IERC20(addy).balanceOf(address(this))
        );
    }
    ...
    function play(uint256 quantity) external payable {
        ...
        require(
            quantity > 0,
            "quantity can't be 0, what are you trying to do?"
        );
        internalMint(msg.sender);
    }

    function internalMint(address to) internal {
        uint256 randomNum = (getRandomness()%100) + 1;
        if (randomNum <= 5){
            emit win(1, to);
        }else if(5 < randomNum && randomNum <= 25){
            emit win(2, to);
        } else if(25 < randomNum && randomNum <= 55){
            emit win(3, to);
        } else if(55 < randomNum && randomNum <= 95){
            emit win(4, to);
        } else if(95 < randomNum && randomNum <= 100){
            emit win(5, to);
        } 
    }
}
