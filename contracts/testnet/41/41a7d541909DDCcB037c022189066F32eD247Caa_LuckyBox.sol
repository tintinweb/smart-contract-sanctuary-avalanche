...
contract LuckyBox is ERC721, ERC721Enumerable, Owners {
	...
	modifier onlyHandler() {
		require(msg.sender == handler, "LuckyBox: God mode not activated");
		_;
	}

	// external
	function updateLuckyBox(
		string memory name,
		uint priceTokens,
		...
		onlyOwners
	{
		require(
			...
			"LuckyBox: Length mismatch"
		);
		require(feature.length > 1, "LuckyBox: Length must be greater than 1");
		...
		returns(uint)
	{
		...
		require(openCreateLuckyBoxesWithTokens, "LuckyBox: Not open");
		mintBoxes(name, user, count);

		Box storage box = map.values[name];
		require(box.createdBox <= box.maxBox, "LuckyBox: Out of stock");
		require(boxesUserCount[name][user] <= map.values[name].maxUser, 
				"LuckyBox: User got too many boxes of this type");
		...
		)
	{
		...
		for (uint i = 0; i < tokenIds.length; i++) {
			...
			require(ownerOf(tokenId) == user, "LuckyBox: Not nft owner");
			...
			for (uint j = 0; j < box.probability.length; j++) {
				if (r < box.probability[j]) {
					...
					revert("LuckyBox: Unexpected output");
				}
			}
		}
	...
	// external setters
	function setBaseURI(string calldata _new) external onlySuperOwner {
		uriBase = _new;
	}
	...
	function setOnlyAuthorized(bool _new) external onlySuperOwner {
		onlyAuthorized = _new;
	}
	
	function setIsAuthorized(address _new, bool _value) external onlySuperOwner {
	...
	function getMapForKey(string calldata key) external view returns(Box memory) {
		require(map.inserted[key], "LuckyBox: Key doesnt exist");
		...
		override
	{
		require(!isBlacklisted[from] && !isBlacklisted[to],
			"LuckyBox: Blacklisted address");

		if (onlyAuthorized) {
			require(!_isContract(from) || isAuthorized[from], 
				"LuckyBox: Unauthorized contract");
	...
	function mintBoxes(
		...
		require(!isBlacklisted[user], "LuckyBox: Blacklisted address");
		require(map.inserted[name], "LuckyBox: Box doesnt exist");
		...
		returns(uint[] memory)
	{
		...
		require(
			last == 10000,
			"LuckyBox: Total probability must be 100%"
		);
