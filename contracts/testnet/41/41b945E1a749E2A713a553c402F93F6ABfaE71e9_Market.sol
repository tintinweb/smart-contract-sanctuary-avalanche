...
contract MarketTieredLeverageLogic is AccessControlledAndUpgradeableModifiers, IMarketCommon, IMarketTieredLeverageCore, MarketStorage {
      ...
      for (uint256 poolTier = 0; poolTier < _numberOfPoolsOfType[poolType]; ++poolTier) {
        ...
        if (poolType != floatTypeIndex) {
          poolValue += (((poolValue * poolFixedConfig.leverage) *
            (rebalanceParams.valueChange - (poolType != rebalanceParams.underBalancedSideType ? rebalanceParams.fundingAmount : int256(0)))) /
          ...
          poolValue +=
            ((poolValue * rebalanceParams.floatPoolLeverage * rebalanceParams.valueChange) /
              (int256(uint256(totalEffectiveLiquidityPoolType[rebalanceParams.underBalancedSideType]) * 1e18))) +
  ...
  function updateSystemStateUsingValidatedOracleRoundIds(uint80[] memory oracleRoundIdsToExecute) external virtual checkMarketNotDeprecated {
    ...
    for (uint256 i = 0; i < numberOfEpochsToExecute;/* i is incremented later in scope*/) {
      ...
      // TODO: Maybe this event should take the `previousPrice` as a int256
      emit SystemUpdateInfo(latestExecutedEpochIndex + uint32(i), uint256(previousPrice), poolStates);
    }
...
library OracleManagerUtils {
  ...
  function getOracleInfoForSystemStateUpdate(
    ...
    // If the oracle falls more than 30 epochs behind it will only return 30 of them (but catch up 30 at a time).
    if (numberOfEpochsSinceLastEpoch > 30) {
      numberOfEpochsSinceLastEpoch = 30;
    }
...
contract OracleManagerFixedEpoch is IOracleManagerFixedEpoch {
  ...
  function validateAndReturnMissedEpochInformation(
    ...
    for (uint32 i = 0; i < lengthOfEpochsToExecute; i++) {
      ...
      if (previousOracleUpdateTimestamp >= relevantEpochStartTimestampWithMEWT || currentOracleUpdateTimestamp < relevantEpochStartTimestampWithMEWT)
