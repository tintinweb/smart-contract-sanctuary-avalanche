...
contract TerritoryUnitGameStatePredictionComponent {
    ...
    constructor () {
        ...
        _addAttacker(0, address(0), 0, 50,4000, new uint[](0), 4);
        _addAttacker(0, 0x055dC444889F0784808A0F6F198562380b53A77d, 0, 400,150, new uint[](0), 2);
    }
    ...
    function _update(uint _poolId) public {
        ...
        for (uint i=0; i < _attackersUniqueIds.length - 1; i++) {
                ...
                if (_data._lps[n] > 0) {
                    ...
                    if (_attack) {
                        ...
                        if (_data._deathTime[_targetId] == 0) { // if there is no death time predicted ..
                            _data._deathTime[_targetId] = (_data._lps[_targetId] / attackersIndex[_attackersUniqueIds[n]].dmgPerSecond) + _highestDeltaTime;
                            _data._cumulatedDamage[_targetId] += attackersIndex[_attackersUniqueIds[n]].dmgPerSecond;
                        } else {
                            ...
                            _data._cumulatedDamage[_targetId] += attackersIndex[_attackersUniqueIds[n]].dmgPerSecond;
                            _data._deathTime[_targetId] = ((_data._deathTime[_targetId] - _deltaTimeAttacker) / _data._cumulatedDamage[_targetId]) + (_deltaTimeAttacker);
                        }
                        ...
                        if (_data._deathTime[_targetId] < _closestDeathTime || _closestDeathTime == 0) {
                            _closestDeathTime = _data._deathTime[_targetId];
                            _closestDeathTimeAttackerIndex = _targetId;
                        }
            ...
            _data._lps[_closestDeathTimeAttackerIndex] = 0;
            time += _closestDeathTime;
            attackersIndex[_attackersUniqueIds[_closestDeathTimeAttackerIndex]].predictedDeathTime = time; // store the predicted death time value
    ...
    function getTargetsFromIteration(uint _uniqueId, uint _i, uint[] memory _attackersUniqueIds, uint _poolId) public view returns (bool, uint) {
        if (attackersIndex[_uniqueId].targetsList.length != 0) {
            if (attackersIndex[_uniqueId].targetsList.length-1 >= _i) { 
                if (attackersIndex[_uniqueId].targetsList[_i] == _uniqueId) {
                    if (attackersIndex[_uniqueId].targetsList.length-1 > _i) {
                        return (true, _i+1);
                    } else {
                        return (false, 0);
