...
contract BridgesOfFate is Ownable {
    ...
    function participateInGame(bool _jumpSide, bytes32 playerId) public  GameEndRules NarrowingGameEndRules{
        ...
        if(totalNFTOnTile[_currentUserStage][_jumpSide] <= 0){                
            /**
             * Capacity Full Now randomlly select NFT'S 
             * First check the Capacity At Jumped side.
             * 
            */
            ...
            if(_currentUserStage  > 0){
                   ...
                   emit CurrentStage1122(_currentUserStage); 
                   //Check total Capacit of current tile.
                   bytes32[] memory PlayerIDAtSpecifyStage  =  getStagesData(_currentUserStage);
                    //Succesfull side on current stage or tile
                    ...
                    // Randomlly Select NFT's ANY three maxiumn or minmmun two NFT"S
                        ...
                        if(TitleCapacityAgainestStage[_currentUserStage] > 0){
                            totalNFTOnTile[_currentUserStage][previousJumpSide] = TitleCapacityAgainestStage[_currentUserStage] - _selectSuccessSidePlayerIdList.length;
                        }   
                    }
            }
