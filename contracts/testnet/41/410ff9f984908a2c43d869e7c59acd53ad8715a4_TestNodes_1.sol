...
contract TestNodes_1 is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;

    address public joePair;
    address public joeRouterAddress = 0x688d21b0B8Dc35971AF58cFF1F7Bf65639937860; // TraderJoe Router 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
   ...
   modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "NEBULA::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "NEBULA::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
