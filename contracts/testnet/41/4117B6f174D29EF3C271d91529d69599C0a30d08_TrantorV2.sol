...
contract TrantorV2 is ERC20Upgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            ...
            if(taxAmount>0){
                uint256 operatorionFeeAmount = taxAmount.mul(operationPoolFee).div(100);
                super._transfer(from, operator, operatorionFeeAmount);

                uint256 liquidityAmount = taxAmount.mul(liquidityPoolFee).div(100);
                super._transfer(from, address(this), liquidityAmount);

                super._transfer(from, to, amount.sub(operatorionFeeAmount.add(liquidityAmount)));
            }else
                super._transfer(from, to, amount);
