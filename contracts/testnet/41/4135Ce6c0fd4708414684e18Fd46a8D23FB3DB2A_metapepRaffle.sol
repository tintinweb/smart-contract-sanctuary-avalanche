...
contract metapepRaffle is VRFConsumerBase {
    ...
    function changeTicketCost(uint256 _newCost) public onlyAdmin {
        ticketCost = _newCost; //Change the ticket cost
        emit TicketCostChanged(_newCost); //Emit the event that the ticket cost has been changed
    }

    function withdrawBalance() public onlyAdmin {
        uint256 balance = address(this).balance;
        payable(admin).transfer(balance); //Withdraw the contract balance to the admin address
