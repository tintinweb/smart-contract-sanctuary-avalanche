...
contract FeeManager is SafeOwnable, IFeeManager {
    ...
    uint256 internal _verrifiedRound;
    ...
    modifier onlyVerifiedComponentOperator(address component) {
        ...
        uint256 round = _component.verrifiedRound;
    ...
    function isVerifiedComponent(address component) external view override returns (bool) {
        return _components[component].verrifiedRound > 0;
    }
    ...
    /**
     * @notice Returns the balances of the fee bank for each token.
     * @param tokens The tokens to get the balances for.
     * @return The balances of this contract for each `tokens`.
     */
    function getBalances(IERC20[] calldata tokens) external view override returns (uint256[] memory) {
        uint256[] memory balances = new uint256[](tokens.length);

        address feeBank = address(_FEE_BANK);

        bytes memory balanceOfData = abi.encodeWithSelector(IERC20.balanceOf.selector, feeBank);

        for (uint256 i; i < tokens.length;) {
            (bool success, bytes memory data) = address(tokens[i]).staticcall(balanceOfData);

            if (success) balances[i] = abi.decode(data, (uint256));
    ...
    function verifyComponent(address component) external override onlyOwner {
        ...
        uint256 verrifiedRound = ++_verrifiedRound;
        _component.verrifiedRound = verrifiedRound;

        emit ComponentVerified(component, verrifiedRound);
    }
    ...
    function unverifyComponent(address component) external override onlyOwner {
        ...
        if (_component.verrifiedRound == 0) revert FeeManager__ComponentNotVerified();

        _component.verrifiedRound = 0;
...
interface IFeeManager is ISafeOwnable {
    ...
    function getBalances(IERC20[] calldata tokens) external view returns (uint256[] memory);
