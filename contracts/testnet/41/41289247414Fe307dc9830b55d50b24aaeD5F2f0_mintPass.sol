...
// File: contracts/Dwarf/mintPass.sol
...
contract mintPass is ERC721Enumerable, Ownable {
    ...
    string GOLD_BASE_URI = "goldUri";
    string SILVER_BASE_URI = "silverUri";
    ...
    */
    ...
    onlyAccounts
    {
        ...
        if (checkGoldenMerkle(proof)){
            checkGold = true;
        }else if(checkSilverMerkle(proof)){
            checkGold = false;
        }else{
    ...
    function checkGoldenMerkle (bytes32[] calldata proof) internal view returns(bool)
    {   
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        return MerkleProof.verify(proof, GOLDEN_ROOT, leaf);
    }

    function checkSilverMerkle (bytes32[] calldata proof) internal view returns(bool)
    {   
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        return MerkleProof.verify(proof, SILVER_ROOT, leaf);
    }
