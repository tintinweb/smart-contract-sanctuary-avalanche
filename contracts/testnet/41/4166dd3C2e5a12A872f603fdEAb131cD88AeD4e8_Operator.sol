...
interface WithdrawalInterface {
    /**
    * @notice transfer XFRENS held by the contract belonging to msg.sender to
    * another address
    * @param recipient is the address to send the XFRENS to
    * @param amount is the amount of XFRENS to send
    */
    function withdraw(address recipient, uint256 amount) external;

    /**
    * @notice query the available amount of XFRENS to withdraw by msg.sender
    */
...
abstract contract XFrensTokenReceiver {
    /**
    * @notice Called when XFRENS is sent to the contract via `transferAndCall`
    * @dev The data payload's first 2 words will be overwritten by the `sender` and `amount`
    * values to ensure correctness. Calls oracleRequest.
    * @param sender Address of the sender
    * @param amount Amount of XFRENS sent (specified in wei)
    ...
    */
    function onTokenTransfer(
        ...
        uint256 amount,
        bytes memory data
    ) public validateFromXFRENS permittedFunctionsForXFRENS(data) {
        assembly {
        // solhint-disable-next-line avoid-low-level-calls
    ...
    */
    ...
    /**
    * @dev Reverts if not sent from the XFRENS token
    */
    modifier validateFromXFRENS() {
        require(msg.sender == getFrensProtocolToken(), "Must use XFRENS token");
        _;
    }
    ...
    */
    modifier permittedFunctionsForXFRENS(bytes memory data) {
        bytes4 funcSelector;
        assembly {
            // solhint-disable-next-line avoid-low-level-calls
 ...
 */
contract Operator is AuthorizedReceiver, ConfirmedOwner, XFrensTokenReceiver, OperatorInterface, WithdrawalInterface {
    ...
    XFrensTokenInterface internal immutable xfrensToken;
    ...
    /**
    * @notice Deploy with the address of the XFRENS token
    * @dev Sets the XFrensToken address for the imported XFrensTokenInterface
    * @param xfrens The address of the XFRENS token
    * @param owner The address of the owner
    */
    constructor(address xfrens, address owner) ConfirmedOwner(owner) {
        xfrensToken = XFrensTokenInterface(xfrens); // external but already deployed and unalterable
    }
    ...
    */
    function oracleRequest(
        ...
        uint256 dataVersion,
        bytes calldata data
    ) external override validateFromXFRENS {
    ...
    */
    ...
    /**
    * @notice Allows the node operator to withdraw earned XFRENS to a given address
    * @dev The owner of the contract can be another wallet and does not have to be a Chainlink node
    * @param recipient The address to send the XFRENS token to
    ...
    */
    ...
    {
        assert(xfrensToken.transfer(recipient, amount));
    }

    /**
    * @notice Displays the amount of XFRENS that is available for the node operator to withdraw
    * @dev We use `ONE_FOR_CONSISTENT_GAS_COST` in place of 0 in storage
    * @return The amount of withdrawable XFRENS on the contract
    */
    function withdrawable() external view override(OracleInterface, WithdrawalInterface) returns (uint256) {
        //return _fundsAvailable();
        return xfrensToken.balanceOf(address(this)).sub(ONE_FOR_CONSISTENT_GAS_COST);
    }
    ...
    */
    function ownerForward(address to, bytes calldata data) external onlyOwner validateNotToXFRENS(to) {
    ...
    /**
    * @notice Interact with other XFrensTokenReceiver contracts by calling transferAndCall
    ...
    */
    function ownerTransferAndCall(
        ...
        return xfrensToken.transferAndCall(to, value, data);
    }
    ...
    * Will transfer the XFRENS sent for the request back to the recipient address.
    ...
    */
    function cancelOracleRequest(
        ...
        xfrensToken.transfer(msg.sender, payment);
    }
    ...
    */
    ...
    /**
    * @notice Returns the address of the XFRENS token
    ...
    */
    function _verifyAndProcessOracleRequest(
        ...
        uint256 nonce,
        uint256 dataVersion
    ) private validateNotToXFRENS(callbackAddress) returns (bytes32 requestId, uint256 expiration) {
    ...
    */
    ...
    /**
    * @notice Returns the XFRENS available in this contract, not locked in escrow
    * @return uint256 XFRENS tokens available
    */
    function _fundsAvailable() private view returns (uint256) {
        uint256 inEscrow = s_tokensInEscrow.sub(ONE_FOR_CONSISTENT_GAS_COST);
        return xfrensToken.balanceOf(address(this)).sub(inEscrow);
    }
    ...
    */
    ...
    /**
    * @dev Reverts if the callback address is the XFRENS token
    * @param to The callback address
    */
    modifier validateNotToXFRENS(address to) {
        require(to != address(xfrensToken), "Cannot call to XFRENS");
        _;
    }
