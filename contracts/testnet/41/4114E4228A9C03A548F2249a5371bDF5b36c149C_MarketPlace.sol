...
contract MarketPlace { // ---> Transfering And Approving NFT Tokens Via MarketPlace <---
    ...
    function createERC721Contract(string memory _name, string memory _symbol, string memory _desc) external {
        ...
        bytes memory byteCode = IERC721ByteCodeGenerator(erc721Gen).generate(_name, _symbol, _desc, msg.sender);
        address contractAddr;
        assembly {
