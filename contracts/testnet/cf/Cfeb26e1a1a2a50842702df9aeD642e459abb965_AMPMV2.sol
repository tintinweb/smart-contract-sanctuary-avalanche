...
// File: contracts/AMPM-testnet/AmPmPoolV2.sol
...
contract AMPMV2 is Ownable {
    ...
    address public commissionWallet = 0xE873Dbb18194e032D78a59E0035948366812CA99;
    ...
    function claimableView() public view returns (uint256) {
        ...
        for (uint i = 0; i < user.deposits.length; i++) {
            ...
            uint unallocated = share * needPay / 1e18;
            if (unallocated > needPay) {
                unallocated = needPay;
            }
            if (deposit.allocated > deposit.payout) {
                totalToClaim = deposit.allocated - deposit.payout + unallocated;
       ...
       function unallocatedView(uint256 userid) public view returns (uint256) {
        ...
        for (uint i = 0; i < user.deposits.length; i++) {
            Deposit storage deposit = deposits[user.deposits[i]];
            uint needPay = deposit.amount * payMultiplier - deposit.allocated;
            unallocated = share * needPay / 1e18;
            if (unallocated > needPay) {
                unallocated = needPay;
            }
    ...
    function claim() external {
        ...
        uint toInclude = (totalDeposit - deposits[deposits.length - 1].amount);
        
        uint toExclude = (totalDeposit - deposits[user.depositID].amount);

        uint totalRequired = (toInclude - toExclude) * 2;

        uint topay;
       
        if (totalRequired <= user.shareAllocated) {
            topay = 0; } 
            else 
            {

        uint needs =  totalRequired;
        if (needs == 0) {
            topay = 0;
            }  
        else {
        ...
        uint share = user.totalDeposit * 1e18 / needs;

        //uint share = getOneShareAmount();
        //if (share == 0) return;
         for (uint index = 0; index < user.deposits.length; index++) {
            Deposit storage deposit = deposits[user.deposits[index]];
            uint needPay = user.totalDeposit * payMultiplier - user.totalAllocated;
            uint balance = token.balanceOf(address(this));
            if (needPay == 0) continue;
            topay = share * needPay / 1e18;
            //User storage user = users[userids[deposit.account]];
            if (topay <= user.shareAllocated) continue;
            if (topay >= needPay) {
        ...
        require(topayclaim > 0, "No to claim");
        token.safeTransfer(msg.sender, topayclaim);
        emit UserMsg(userids[msg.sender], "Claim", topayclaim);
    }
    ...
    function processDeposit(address referer, uint units) private {
        ...
        user.depositID = deposits.length + 1;
        if (user.referer == address(0)) {
            if (users[userids[referer]].deposits.length > 0 && referer != msg.sender) {
                //If there is no referral, put it under the first account. 
                if (referer == address(0) && msg.sender != firstWallet){
                    referer = firstWallet;
                }
    ...
    function payForBinatePartA() private {
    ...
    function payForBinatePartB() private {
