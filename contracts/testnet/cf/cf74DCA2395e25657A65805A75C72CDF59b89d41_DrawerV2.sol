...
contract DrawerV2 is Ownable {
  ...
  function getLineAndScore(
    ...
    text = string(abi.encodePacked(getLine(base, x, y, score1, winScore), getScore(x + 80, y + 140, score2, winScore)));
  }
  ...
  function getRoundSVG(
    ...
    for (; i < 12; i++) {
      svg = string(
        abi.encodePacked(
          ...
          getLineAndScore(i, x, y, scores[i / 2], scores[i - 8], winScore)
        )
      );
      x += 360;
    }
    ...
    svg = Base64.encode(
      abi.encodePacked(
        ...
        getTeamName(teamNames[i]),
        "</svg>",
        getScore(720, 260, scores[6], winScore),
        STYLE_ROUND
      )
    );
  }
