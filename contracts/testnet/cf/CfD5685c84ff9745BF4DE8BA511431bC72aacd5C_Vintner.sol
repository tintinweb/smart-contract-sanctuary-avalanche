...
contract Vintner is ERC721, Ownable, Pausable {
    ...
    mapping(address => uint8) userPublicSaleNFTCountMap; // Up to 5
     ...
     * @dev GEN0 minting
     */
    function mintVintnerWithAVAX(uint8 qty) external payable whenNotPaused {
        require(mintingStartedAVAX(), "cannot mint right now");
        require(qty > 0 && qty <= 20, "quantity must be between 1 and 20");
        require(
            userPublicSaleNFTCountMap[msg.sender] + qty > 0 &&
                userPublicSaleNFTCountMap[msg.sender] + qty <= 20,
            "can't mint over 20"
        );
        require(
        ...
        userPublicSaleNFTCountMap[msg.sender] += qty;
        vintnersMintedWithAVAX += qty;
