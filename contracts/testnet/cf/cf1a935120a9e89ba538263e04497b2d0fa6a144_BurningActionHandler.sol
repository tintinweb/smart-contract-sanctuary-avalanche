...
import {ActionHandler} from "./messaging/interfaces/ActionHandler.sol";
import {GasAllocationViewer} from "./messaging/interfaces/GasAllocationViewer.sol";

import {BurningQueue} from "./BurningQueue.sol";
import {TradingModule} from "./TradingModule.sol";
import {TradingModuleDeployer} from "./TradingModuleDeployer.sol";
import {BlockingApp} from "./messaging/BlockingApp.sol";
import {MessageSubject} from "./messaging/MessageSubject.sol";
import {RedeemRouter} from "./RedeemRouter.sol";
import {RemoteSubIndexFactory} from "./SubIndexFactory.sol";
...
contract TradingModule is TradingModuleErrors, Initializable {
    ...
    function trade(
        ...
        uint256 valueToSend = t.value > address(this).balance ? address(this).balance : t.value;
...
contract BlockingApp is OmniNonblockingAppErrors, NonblockingLzApp, BridgingExecutor {
    ...
    {
        return BridgingLogicClient(bridgingLogic).generateTargets(payload, BlockingApp.send.selector);
    }

    function _nonblockingLzReceive(uint16 _srcChainid, bytes memory, uint64, bytes memory _payload)
        internal
        virtual
        override
    {}
...
contract RedeemRouter {
    ...
    address public immutable messageRouter;
    address public immutable stargateReserveAsset;

    constructor(address _messageRouter, address _stargateReserveAsset) {
        messageRouter = _messageRouter;
        stargateReserveAsset = _stargateReserveAsset;
    }
            ...
            MessageLib.Payload({
                body: abi.encode(
                    ...
                    IndexInfo({
                        ...
                        reserveAsset: stargateReserveAsset,
                        receiver: msg.sender,
...
library LzGasEstimation {
    ...
    function estimateGasForMessageWithSgCallback(
        ...
        uint256 amountToAirdrop = sgCallbackMessageFee * LZ_PRICE_RATIO_MULTIPLIER / dstPriceRatio * 120 / 100;
...
library LzChainInfoLib {
    ...
    uint16 internal constant OPTIMISM_GOERLI_LZ = 10_132;
