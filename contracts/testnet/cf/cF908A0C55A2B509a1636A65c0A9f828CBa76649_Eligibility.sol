...
contract Eligibility is AccessControl, IEligibility {
    ...
    /**
     * @notice Set multiple results at once.
     * @param accounts The validated account addresses.
     * @param batch account The KYC results.
     */
    function setBatchResults(address[] memory accounts, Result[] memory batch) external onlyRole(WRITER) {
        require(accounts.length > 0, "Eligibility: accounts has size 0");

        for (uint256 i = 0; i < accounts.length; i++) {
            results[accounts[i]] = batch[i];
            emit Validation(accounts[i], batch[i]);
        }
    }
}
