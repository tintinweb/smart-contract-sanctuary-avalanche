...
contract CloversNFT is ERC721, Ownable {
    ...
    uint256 public MaxOfEachRarity;
    bool public paused = false;
    bool public onlyWhitelisted = false;
    bool public CapForEachRarity = false;
    ...
    CounterOfRarityMinted[] public addressMintedBalance;
        ...
        
    {
        ...
        CounterOfRarityMinted storage ClovCount = addressMintedBalance[uint256(uint160(_msgSender()))];
        ...
        if (onlyWhitelisted != false) {
            ...
            require( ClovCount.Total + _mintAmount <= MaxNFTMintableForWhithelistedAddresses,"max NFT per address exceeded");
        }
        ...
        if(CapForEachRarity != false){
            if ( keccak256(abi.encodePacked((_ClovesType))) == keccak256(abi.encodePacked(("Platinum"))) ){
            require( ClovCount.Platinum + _mintAmount <= MaxOfEachRarity,"max NFT per address exceeded");}
            if ( keccak256(abi.encodePacked((_ClovesType))) == keccak256(abi.encodePacked(("Gold"))) ){
            require( ClovCount.Gold + _mintAmount <= MaxOfEachRarity,"max NFT per address exceeded");}
            if ( keccak256(abi.encodePacked((_ClovesType))) == keccak256(abi.encodePacked(("Silver"))) ){
            require( ClovCount.Silver + _mintAmount <= MaxOfEachRarity,"max NFT per address exceeded");}
        }
    ...
    returns (uint256[] memory)
  {
    ...
    function resetCountOfRarityMinted() public onlyOwner {
        for (uint256 i = 0; i < addressMintedBalance.length; i++){
            delete addressMintedBalance[i];
        }
    }
    ...
    function setMaxOfEachRarity(uint256 _newMaxOfEachRarity) public onlyOwner {
        MaxOfEachRarity = _newMaxOfEachRarity;
    }
