...
RevokableDefaultOperatorFilterer
{
    ...
    event Claimed(uint256 indexed ticket, uint256 indexed comic, address indexed sender, uint256 timestamp);
    ...
    function maxSupply() public view returns (uint256){
        return IERC721A(ticket).totalSupply();
    }

    function mintToken(uint256[] memory ticketId) external payable{
        require(isMintable == true, "is not mintable");
        require(isSale == true, "is not for sale");
        require(msg.value == (ticketId.length.mul(salePrice)), "wrong ether value");
        require(totalSupply() +  ticketId.length <= maxSupply(), "tokens supply reached limit");
        
        uint256 startId = totalSupply()+1;
        for(uint i=0; i< ticketId.length; i++){
            require(ticketId[i] > 0 && ticketId[i] <= maxSupply(), "invalid token id");
            require(isClaimed[ticketId[i]] == 0, "token id already claimed");
            require(IERC721A(ticket).ownerOf(ticketId[i]) == msg.sender, "only owner of token id can claim");
            isClaimed[ticketId[i]] = startId;

            emit Claimed(ticketId[i], startId, msg.sender, block.timestamp);
            startId +=1;
        }
        _mintNFT(msg.sender,ticketId.length);

        emit Minted(ticketId.length, 0, msg.sender, 1, msg.sender, block.timestamp);
    }
