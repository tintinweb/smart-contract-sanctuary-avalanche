...
contract VeArt is ERC721HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    /**
    * @notice allows the whitelisted contracts.
    */
    ...
    */
    function getBoostedGenerationRate(address _addr) external view returns (uint256) {
        uint256 rarityPercentage = userSalvorMiniBoostInfos[_addr].totalRarityLevel * rarityLevelMultiplier * 100;
        return (veARTgenerationRate * (10000 + rarityPercentage)) / 10000;
    }
    ...
    */
    function getBoostedIncreasePercentage(address _addr) external view returns (uint256) {
        return userSalvorMiniBoostInfos[_addr].totalRarityLevel * rarityLevelMultiplier * 100;
    }
    ...
    */
    function burnSalvorMiniToBoostVeART(uint256 _tokenId) external whenNotPaused nonReentrant {
        ...
        UserSalvorMiniBoostInfo memory userSalvorMiniBoostInfo = userSalvorMiniBoostInfos[msg.sender];
        salvorMiniCollection.burn(_tokenId);

        userSalvorMiniBoostInfo.totalBurnedSalvorMiniCount += 1;
        uint256 rarityLevel = salvorMiniCollection.getRarityLevel(_tokenId);
        userSalvorMiniBoostInfo.totalRarityLevel += rarityLevel;
        userSalvorMiniBoostInfos[msg.sender] = userSalvorMiniBoostInfo;
    ...
    */
    function claimEarnings(address _receiver) external nonReentrant whenNotPaused {
        ...
        if (_pendingARTReward > 0 || failedBalance > 0) {
    ...
    */
    function claimableVeART(address _addr) public view returns (uint256) {
        ...
        uint256 rarityPercentage = userSalvorMiniBoostInfos[_addr].totalRarityLevel * rarityLevelMultiplier * 100;
        uint256 boostedGenerationRate = (veARTgenerationRate * (10000 + rarityPercentage)) / 10000;
    ...
    */
    ...
    function _withdrawART(uint256 _amount) internal {
        ...
        users[msg.sender].amount = user.amount - _amount;
        ...
        if (userVeARTBalance > 0) {
            ...
            users[msg.sender].artRewardDebt = 0;
