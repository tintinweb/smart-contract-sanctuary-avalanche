...
// File contracts/ResolverRegistryV1.sol
...
contract ResolverRegistryV1 {
  ...
  function _canWrite(uint256 name) internal view {
    ...
    require(owner == msg.sender, "ResolverRegistry: not owner");
    require(!domain.isSuspended(name), "ResolverRegistry: domain suspended");
    uint256 expiresAt = domain.getDomainExpiry(name);
    require(expiresAt > block.timestamp, "ResolverRegistry: domain expired");
  }
