...
contract UnstakeChefAndTools is Ownable {

    Chef public chef;
    Upgrade public upgrade;
    Pizzeria public pizzeria;
    Pizza public pizza;
    ...
    constructor(Chef _chef, Upgrade _upgrade, Pizza _pizza, address _pizzeriaAddress) {
        ...
        pizzeria = Pizzeria(_pizzeriaAddress);
    }
    ...
    function unstakeUpgradesToOwner(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            ...
            if(staked){
                pizza.mint(owner, 30*1e18);
                upgrade.transferFrom(pizzeriaAddress, owner, i);
            }
        }
    }

    function unstakeChefsToOwner(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
    ...
    function unstakeRestingChefsToOwner(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            (address owner, , , bool staked) = pizzeria.restingChefs(i);
            if(staked){
                chef.transferFrom(pizzeriaAddress, owner, i);
            }
        }
    ...
    function unstakeStakedRestingPizza(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            ...
            if(staked){
                ...
                claimedByChef = claimedByChef*80/100 + 30*1e18; //claiming will be fixed at 80% + 30 pizza per chef to make up for gas fees
    ...
    function mintPizzaToOwner(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            (address owner, , , bool staked) = pizzeria.stakedChefs(i);
            if(staked){
                require(!pizzaClaimed[i], 'pizza already claimed');
                ...
                uint256[] memory claimedByManyChef = pizzeria.getPizzaAccruedForManyChefs(inputData);
                uint256 claimedByChef = claimedByManyChef[0];
                claimedByChef = claimedByChef*80/100 + 30*1e18; //claiming will be fixed at 80%
                pizza.mint(owner, claimedByChef);
            }
        }
    }
