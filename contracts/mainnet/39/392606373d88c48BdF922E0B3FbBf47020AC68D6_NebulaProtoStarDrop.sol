...
contract NebulaProtoStarDrop is Ownable{
	...
	function createProtoFeeTokr(string memory _name) payable external{
    		address _account = msg.sender;
		    checkDrops(_account);
	...
	function createProtoAvaxr(string memory _name) payable external {
    			address _account = msg.sender;		
		 	(uint256 retVal,uint256 len,uint256 _fee) = checkSend(_account,over.getFee(),One,over.getFee());
			uint256 sent = msg.value;
			uint256 fee = over.getFee();
		    	require(sent >= fee,"you have not sent enough to pay a fee");
		    	treasury.transfer(fee);
	...
	function createProtoFeeToks(string memory _name) payable external{
    		address _account = msg.sender;
	...
	function createProtoAvaxs(string memory _name) payable external {
    		address _account = msg.sender;		
		if(createProtoInt(_account,_name) ==false){
			uint256 sent = msg.value;
			uint256 fee = over.getFee();
