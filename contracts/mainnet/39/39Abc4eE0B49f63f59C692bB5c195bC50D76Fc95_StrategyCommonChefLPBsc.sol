...
contract StratManager is Ownable, Pausable {
     ...
     * @param _unirouter *router used for lp*
     * @param _unirouter2 router to use for swaps
    ...
    function setUnirouter(address _unirouter) external onlyOwner {
        ...
        /**
     * @dev Updates router that will be used for swaps.
     * @param _unirouter2 new unirouter address.
     */
    function setUnirouter2(address _unirouter2) external onlyOwner {
        unirouter2 = _unirouter2;
    }
...
abstract contract FeeManager is StratManager {
    uint constant public STRATEGIST_FEE = 445;
    uint constant public MAX_FEE = 1000;
    uint constant public MAX_CALL_FEE = 110;
...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _unirouter2, _vault, _beefyFeeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        IUniswapRouterETH(unirouter2).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
        ...
        IUniswapRouterETH(unirouter2).swapExactTokensForTokens(toBurn, 0, pegPath, address(this), now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapRouterETH(unirouter2).swapExactTokensForTokens(outputHalf, 0, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter2).swapExactTokensForTokens(outputHalf, 0, outputToLp1Route, address(this), now);
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            try IUniswapRouterETH(unirouter2).getAmountsOut(outputBal, outputToNativeRoute)
    ...
    function _giveAllowances() internal {
        ...
        IERC20(output).safeApprove(unirouter2, uint256(-1));
        IERC20(native).safeApprove(unirouter2, uint256(-1));

        IERC20(lpToken0).safeApprove(unirouter2, 0);
        IERC20(lpToken0).safeApprove(unirouter2, uint256(-1));

        IERC20(lpToken1).safeApprove(unirouter2, 0);
        IERC20(lpToken1).safeApprove(unirouter2, uint256(-1));
    ...
    function _removeAllowances() internal {
        ...
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
        IERC20(output).safeApprove(unirouter2, 0);
        IERC20(lpToken0).safeApprove(unirouter2, 0);
        IERC20(lpToken1).safeApprove(unirouter2, 0);
    }
...
contract GasThrottler {
    ...
    address public gasprice = address(0xd6e9D176E189831e2cD5cC405f824Cc2BC00C3BD);
