...
interface IInfiniteManager is IERC721 {
    function getNameOf(uint256 _id) external view returns (string memory);
    function getMintOf(uint256 _id) external view returns (uint64);
    function getInfinitesOf(address _account) external view returns (uint256[] memory);
}

contract MetaManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    ...
    IAscendMembershipManager public manager;
    IInfiniteManager public infiniteManager;

    bool public transferIsEnabled = true;

    constructor(address _ASCEND_TOKEN, address _ascendMembership, address _infiniteManager)  {
        ASCEND = IERC20(_ASCEND_TOKEN);
        manager = IAscendMembershipManager(_ascendMembership);
        infiniteManager = IInfiniteManager(_infiniteManager);
    }
    ...
    function _addMeta(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createMeta(_id, _name, _mint, _claim, _metadata, _to);
    }

    function setManager (IAscendMembershipManager _manager) external onlyOwner {
        manager = IAscendMembershipManager(_manager);
    }

    function setInfiniteManager (IInfiniteManager _infiniteManager) external onlyOwner {
        infiniteManager = IInfiniteManager(_infiniteManager);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "Meta Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory ascendNodes, uint64[] memory userInfiniteNodes) public {
        ...
        require(manager.balanceOf(sender) >= nodeAscendMembershipPrice, "HELPER: Ascend Membership balance too low for creation.");
        require(infiniteManager.balanceOf(sender) >= nodeInfinitePrice, "HELPER: Infinite Membership balance too low for creation.");
        ...
        for (uint256 i = 0; i < nodeAscendMembershipPrice; i++) {
            manager.transferFrom(_msgSender(), address(0), ascendNodes[i]);
        }
        for (uint256 i = 0; i < nodeInfinitePrice; i++) {
            infiniteManager.transferFrom(_msgSender(), address(0), userInfiniteNodes[i]);
        }
        ASCEND.transferFrom(_msgSender(), address(manager),  nodePrice);
    ...
    if (transferIsEnabled == true){
            ...
            require (_msgSender() == upgradeManager, "Meta Transfers are not allowed");
            _transfer(from, to, tokenId);
            require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
        }
        _transfer(from, to, tokenId);
