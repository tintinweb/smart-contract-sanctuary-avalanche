...
contract BonsaiV1 is ERC721Upgradeable {
    ...
    uint256 public constant seedHoldersMaxMint = 100;
    ...
    event SaleToggle(uint256 bonsaiNumber, bool isForSale, uint256 price);
    event PurchaseEvent(uint256 bonsaiNumber, address from, address to, uint256 price);

    event LevelUp(uint256 bonsaiNumber, uint256 newCm);
    event NameChange(uint256 bonsaiNumber, string name);
    ...
    // add bonsai naming service
    mapping(uint256 => string) public bonsainame;

    function changeName(uint256 _tokenId, string memory name)
        external
        onlyAuthorized
    {
        bonsainame[_tokenId] = name;
