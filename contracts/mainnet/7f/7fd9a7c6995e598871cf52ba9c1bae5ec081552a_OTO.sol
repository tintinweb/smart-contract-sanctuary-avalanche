...
contract OTO is ERC20Detailed, Ownable {
    ...
    string public _name = "OTO Protocol";
    string public _symbol = "OTO";
    ...
    uint256 public otoVaultFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(otoVaultFundFee).add(
            firePitFee
        );
    ...
    constructor() ERC20Detailed("OTO Protocol", "OTO", uint8(DECIMALS)) Ownable() {
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountAVAXToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(otoVaultFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(otoVaultReceiver).call{
            value: amountAVAXToTreasuryAndSIF.mul(otoVaultFundFee).div(
                treasuryFee.add(otoVaultFundFee)
            ),
            gas: 30000
        }("");
