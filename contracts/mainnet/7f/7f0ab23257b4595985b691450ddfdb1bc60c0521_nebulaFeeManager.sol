...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,bool _feeFroze){
		...
		feeFroze = _feeFroze;
		nebuMath = nebulaMath(_addresses[4]);
	...
	function payFeeTokenSpec(address _account, uint256 _amount,uint256 _intervals, uint256 _x) payable external onlyGuard(){
    	    uint256 _intervals = nebuLib.getLower(INTgetProtoTotalPayable(_account,_x),_intervals);
    	    require(_intervals >0, "either you opted not to pay any fees at this time or have none left to pay");
    	    //calculation
            (uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(_intervals,feeTok.balanceOf(_account),tokFee);
	...
	function payFeeToken(address _account,uint256 _amount,uint256 _intervals) payable external onlyGuard(){
    	    uint256 _intervals = nebuLib.getLower(INTgetTotalPayable(_account),_intervals);
	...
	function payFeeAvax(address _account,uint256 _amount,uint256 _intervals) payable external onlyGuard(){
    		...
    		require(_intervals >0, "Doesnt Look Like you opted to pay any fees at this time");
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = nebuMath.EXTdoAllMultiple(_intervals,msg.value,over.getFee());
		//verification
	...
	function checkAvaxSend(address _account,uint256 _sent,uint256 _intervals) internal returns (uint256,uint256,uint256){
		...
		(uint256 unusedIntervals,uint256 _intervals,uint256 newFee,uint256 balanceReturn) = nebuMath.EXTdoAllMultiple(_intervals,_sent,over.getFee());
	...
	function INTupdateProtoTotalPayable(address _account,uint256 _x) internal {
		...
		if(next<feePeriod){
			...
			(protoowners[_account][_x].protoFutureFees,rem) = nebuMath.EXTgetAllMultiple(feePeriod,protoowners[_account][_x].protoDueElapsed);
			...
			(protoowners[_account][_x].protoTotalPayable ,rem)= nebuMath.EXTgetAllMultiple(feePeriod,nebuLib.getLower(futureElapsed,totalPayableElapsed));
		}
	}
	...
	function pauseFees(bool _x) external onlyOwner {
        ...
        function changeMath(address _account) external onlyOwner(){
    		_nebulaMath = _account;
        }
