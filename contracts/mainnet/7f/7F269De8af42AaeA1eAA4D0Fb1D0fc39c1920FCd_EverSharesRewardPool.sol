...
contract EverSharesRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accEverSharesPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "EverSharesRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "EverSharesRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accEverSharesPerShare = pool.accEverSharesPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accEverSharesPerShare = accEverSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accEverSharesPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accEverSharesPerShare = pool.accEverSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accEverSharesPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeEverSharesTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accEverSharesPerShare).div(1e18);
    ...
    function safeEverSharesTransfer(address _to, uint256 _amount) internal {
