...
import './TraderJoeV2SwapRouter.sol';
import './V3SwapRouter.sol';
import './base/ApproveAndCall.sol';
import './base/MulticallExtended.sol';

/// @title Uniswap V2 and V3 Swap Router with trader joe init code hash for v2 support
contract TraderJoeSwapRouter02 is ISwapRouter02, TraderJoeV2SwapRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    constructor(
        address _factoryV2,
        address factoryV3,
...
import './libraries/TraderJoeV2Library.sol';

/// @title Uniswap V2 Swap Router
/// @notice Router for stateless execution of swaps against Uniswap V2
abstract contract TraderJoeV2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    using LowGasSafeMath for uint256;
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = TraderJoeV2Library.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(TraderJoeV2Library.pairFor(factoryV2, input, output));
            uint256 amountInput;
            ...
            // scope to avoid stack too deep errors
            {
                ...
                amountOutput = TraderJoeV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) =
                input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
            address to = i < path.length - 2 ? TraderJoeV2Library.pairFor(factoryV2, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        pay(
            ...
            TraderJoeV2Library.pairFor(factoryV2, path[0], path[1]),
            amountIn
        );
    ...
    function swapTokensForExactTokens(
        ...
        amountIn = TraderJoeV2Library.getAmountsIn(factoryV2, amountOut, path)[0];
        require(amountIn <= amountInMax, 'Too much requested');

        pay(path[0], msg.sender, TraderJoeV2Library.pairFor(factoryV2, path[0], path[1]), amountIn);
