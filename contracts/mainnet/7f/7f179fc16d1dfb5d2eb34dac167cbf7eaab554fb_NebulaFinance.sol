...
pragma solidity ^0.8.0;
contract NebulaFinance is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address[] memory pools,uint256[] memory fees,uint256 Supply)ERC20("NebulaFinance", "NeFi") {
        ...
        _pools = [rewardsPool,treasury,teamPool,OverSeerProtocol,NeFiPair,address(this)];
    }
//LogFees-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//NodeInfo-------------------------------------------------------------------------------------------------------------------
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        uint256[4] memory _fees = [NeFiLib.sendPercentage(nodeAmount,treasuryFee),NeFiLib.sendPercentage(nodeAmount,treasuryFee),NeFiLib.sendPercentage(nodeAmount,teamPoolFee),nodeAmount];
...
//updatePairInfo-------------------------------------------------------------------------------------------------------------
    function updateNeFiRouterAddress(address newAddress) external onlyOwner {
        ...
        address _NeFiPair = IUniswapV2Factory(NeFiRouter.factory()).createPair(address(this),_NeFiRouter.WETH());
        NeFiPair = _NeFiPair;
        NeFiRouterAddress = newAddress;
    }
    function updateNeFiPair(address payable newVal) external onlyOwner {
