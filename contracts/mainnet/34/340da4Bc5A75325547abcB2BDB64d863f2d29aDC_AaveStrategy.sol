...
contract AaveStrategy is IAaveStrategy, UUPSUpgradeable {
        ...
        returns (uint256 amountOut)
    {
        ...
        if (address(path[0]) == address(0x0)) {
            ...
            if (_getChainID() == 43114) {
                path[0] = uniswapV2Router.WAVAX();
                require(
                    address(this).balance >= amountIn,
                    "AaveStrategy::swap:inssuficient amountIn"
                );
            ...
            require(
                ...
                "AaveStrategy::swap:insufficient amountIn"
            );
            require(
                IERC20(path[0]).approve(address(uniswapV2Router), amountIn),
                "AaveStrategy::swap:router approval failed"
            );
    ...
    function marginShort(bytes memory _data) public override onlyActionPool {
        ...
        require(
            ...
            ).balanceOf(address(aaveVault)) >= amountOut,
    ...
    function _authorizeUpgrade(address) internal override onlyActionPool {}
