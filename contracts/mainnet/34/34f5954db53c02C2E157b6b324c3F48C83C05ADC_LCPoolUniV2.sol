...
contract LCPoolUniV2 is Ownable {
  ...
  function deposit(
    ...
    require(msg.sender == info.account || managers[msg.sender], "LC pool: no access");
    ...
    dpvar[3] = _distributeFee(info.basketId, (info.token==address(0)?WETH:info.token), info.amount, 1);
  ...
  function withdraw(
    ...
    // tokenId
    // outAmount
    uint256[] memory wvar = new uint256[](7);
    ...
    (wvar[3], wvar[4]) = _withdraw(info, wvar[5], mtoken[0], paths, minAmounts[0]);
    ILCPoolUniV2Ledger(ledger).updateInfo(info.account, wvar[3], info.basketId, info.amount, wvar[0], wvar[2], wvar[1], false);
    ...
    if (wvar[4] > 0) {
      if (isCoin) {
        ...
        (bool success, ) = payable(info.account).call{value: wvar[4]}("");
        require(success, "LC pool: Failed receipt");
      }
      else {
        IERC20(info.token).safeTransfer(info.account, wvar[4]);
      }
    }
  ...
  function _reinvest(
    ...
    if (claimReward) {
      (rvar[6], rvar[7]) = ILCPoolUniV2Ledger(ledger).getSingleReward(info.account, info.meta, info.basketId, rvar[1]);
    }
  ...
  function _distributeFee(uint256 basketId, address token, uint256 amount, uint256 mode) internal returns(uint256) {
    ...
    if (fvar[3] > 0) {
      ...
      for (uint256 i=0; i<len; i++) {
        ...
        if (feeAmount > 0 && fvar[2] >= feeAmount) {
          IERC20(token).safeTransfer(feeAccount, feeAmount);
          emit LcFee(token, feeAccount, feeAmount);
          fvar[2] -= feeAmount;
        }
      }
    }
...
interface ILCPoolUniV2Ledger {
  ...
  function getSingleReward(address acc, uint256 poolId, uint256 basketId, uint256 currentReward)
