...
contract DiamondbankTreasury is Ownable {
    ...
    address public sDb;
    uint public sDBQueue; // Delays change to sDB address
    ...
    constructor (
        ...
        // isLiquidityToken[ _DBMIM ] = true;
        // liquidityTokens.push( _DBMIM );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for DB
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint DB needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Db ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn DB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDBERC20( Db ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sDb ).balanceOf( msg.sender ); // Can only borrow against sDB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DB
        @param _amount uint
     */
    function repayDebtWithDB( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDBERC20( Db ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Db, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Db ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Db ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DB decimals
            value_ = _amount.mul( 10 ** IERC20( Db ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SDB ) { // 9
            sDBQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
