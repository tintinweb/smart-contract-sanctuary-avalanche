...
contract Dice is Ownable, ReentrancyGuard, Pausable {
  ...
  function gameResult(address account, uint256 randomRound) public view returns (uint256 random) {
  ...
  function isWinner(address account) public view returns (bool) {
    ...
    return gameResult(account, game_.randomRound) == uint256(game_.side);
  }
  ...
  constructor(
    ...
    configure(_minAmount, _maxAmount, _multiplyer, _token, _randomizer);
    operator = _operator;
  }
  ...
  function buy(uint256 amount, DiceSide side) external whenNotPaused returns (bool) {
    ...
    _resolveGame(game_);
    return _buy(game_, caller, amount, side);
  }

  function withdraw(bool autobuy, uint256 amount, DiceSide side) external nonReentrant returns (bool) {
    address caller = msg.sender;
  ...
  function _buy(Game storage game_, address caller, uint256 amount, DiceSide side) private returns (bool) {
    require(side > DiceSide.EMPTY, "invalid dice side");
    ...
    fee += amount;
    rewards -= rewards_;
    emit Bought(caller, side, game_.randomRound, amount, rewards_);
    return true;
  }
