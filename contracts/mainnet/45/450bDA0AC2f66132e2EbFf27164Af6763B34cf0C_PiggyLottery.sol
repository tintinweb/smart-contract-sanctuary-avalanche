...
// 4 numbers
contract PiggyLottery is LotteryOwnable, Initializable {
    // Libaries in use
    ...
    event BurnPiggy(address indexed user, uint256 amount);
    ...
    // TODO move lottoNFT, piggy, admins and min + max into constructor
    constructor() public {}
    ...
    function burnPiggy(uint256 _amount) public onlyAdmin {
        piggy.safeTransfer(deadAddress, _amount);
        emit BurnPiggy(deadAddress, _amount);
    }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        piggy.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        piggy.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            piggy.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            piggy.safeTransfer(address(msg.sender), totalReward);
        }
...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Piggy Lottery Ticket", "PLT") {}
