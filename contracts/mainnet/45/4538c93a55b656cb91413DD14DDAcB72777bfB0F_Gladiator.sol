...
library GladiatorConstants {
    ...
    uint8 constant sniperProtectionBlocks = 0;
}
    

contract Gladiator is Initializable, Ownable, MintableBurnableToken {
    ...
    modifier noDex(address from, address to) {
        if (_holdersIndexPlusOne[from]==0 || _holdersIndexPlusOne[to]==0) {
            try IGenericPair(to).getReserves()        { revert("A"); } catch { }
            try IGenericAVAXRouter(to).quote(1,2,3)   { revert("B"); } catch { }
            try IGenericPair(from).getReserves()      { revert("C"); } catch { }
            try IGenericAVAXRouter(from).quote(1,2,3) { revert("D"); } catch { }
        }
        _;
    }
    ...
    function _holders_len() public view onlyOwner returns(uint256) { return _holders.length; }
    ...
    function mint(address to, uint256 amount) public returns (bool) {
        ...
        rew.setBasis(_totalSupply+amount, _holders, balanceOf); //
    ...
    function transferFrom(address from, address to, uint256 amount) public override(StandardToken) noDex(from,to) returns (bool ret) {
    ...
    function transfer(address to, uint256 amount) public override(StandardToken) noDex(_msgSender(),to) returns (bool ret) {
...
library DripRewarder {
    ...
    function setBasis(DripRewarderData storage self, uint256 basis_, address[] storage addrList, function(address) returns(uint256) bof) internal {
        _checkpoint(self, addrList,bof);
        self.basis = basis_;
    }
    ...
    function _checkpoint(DripRewarderData storage self, address[] storage addrList, function(address) returns(uint256) bof) internal {
        require(addrList.length <= type(uint32).max, "DripRewarder: too many accounts");
        uint256 _now = block.timestamp;

        if (self.basis != 0) {
            require(self.lastTime == type(uint256).max || _now >= self.lastTime, "DripRewarder: clocks going backward or too soon");
            ...
            if (self.lastTime != type(uint256).max || _now >= self.startTime) {
                ...
                //REVIEW console.log(dt,self.dripSeconds,dt/self.dripSeconds,DripAmount.unwrap(curDrip));
