...
contract CoinFlip is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    address private linkTokenAddress = 0x5947BB275c521040051D82396192181b413227A3;
    bytes32 private keyHash = 0x89630569c9567e43c4fe7b1633258df9f2531b62f2352fa721cf3162ee4ecb46;
    address s_owner;
    ...
    function _checkFlip(uint256 flipId, uint256 _randomNumber) internal {
        ...
        if (isWin) {
            // BURN BY WIN TAX
            _recordBurnTax(c.totalGain * (c.winTax / 1000));

            // TF REWARD
            _tokenContract.transfer(msg.sender, c.reward);
        } else {
            // BURN BY LOSE TAX
            _recordBurnTax(c.bet * (c.loseBurnTax / 1000));

            // TF REST TO TREASURY
            _tokenContract.transfer(
                address(TREASURY_ADDRESS),
                (c.bet * (1000 - c.loseBurnTax)) / 1000
            );
        }
    ...
    function setLoseBurnTax(uint256 _loseBurnTax) external onlyOwner {
    ...
    function setTreasuryAddress(address _treasuryAddress) external onlyOwner {
        TREASURY_ADDRESS = address(_treasuryAddress);
    }

    function setBurnerAddress(address _burnerAddress) external onlyOwner {
        BURNER_ADDRESS = address(_burnerAddress);
    }
