...
contract AstroToken is ERC20Detailed {
    ...
    address public futureEcosystemReceiver = 0xBAB5461eB9E3A9C5D8df2a44823852aFC989fEfD;
    address public burnReceiver = 0xaC13f6517d7841A0499533453b944e2f91AC2B4c;

    IDEXRouter public router;
    address[] public pairs;
    address public pair;
    address public avaxAddress = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    ...
    */
    ...
    constructor(address _router, address _usdcAddress) ERC20Detailed("100 Days Ventures", "devASTRO", uint8(DECIMALS)) {
        owner = msg.sender;
        TOTAL_GONS = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
        router = IDEXRouter(_router);
        usdcAddress = IERC20(_usdcAddress);

        pairs.push(IDEXFactory(router.factory()).createPair(address(this), router.WAVAX()));


        _allowedFragments[address(this)][address(router)] = ~uint256(0);
        _allowedFragments[address(this)][pairs[0]] = ~uint256(0);
        _allowedFragments[address(this)][address(this)] = ~uint256(0);

        setAutomatedMarketMakerPair(pairs[0], true);
    ...
    function setPair(address _pairAddress) external onlyOwner{
        ...
        _allowedFragments[address(this)][_pairAddress] = ~uint256(0);

        setAutomatedMarketMakerPair(_pairAddress, true);
    }
