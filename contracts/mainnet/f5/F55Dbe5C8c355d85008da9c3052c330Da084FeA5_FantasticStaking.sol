...
import "../interfaces/IYTokenFund.sol";
...
contract FantasticStaking is ReentrancyGuard, Ownable {
    ...
    IYTokenFund public stakingTokenFund;
    address[] public rewardTokens;
    mapping(address => Reward) public rewardData;

    // Duration that rewards are streamed over
    uint256 public constant rewardsDuration = 60 * 15; // 86400 * 7; // 1 week

    // Duration of lock/earned penalty period
    uint256 public constant lockDuration = rewardsDuration * 4; // 8; // 8 weeks
    ...
    constructor(
        ...
        address _stakingTokenFund,
        address[] memory _minters
    ) Ownable() {
        stakingToken = IYToken(_stakingToken);
        stakingTokenFund = IYTokenFund(_stakingTokenFund);
        stakingTokenFund.setRewarder(address(this));
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        ...
        stakingTokenFund.transfer(address(this), amount);
