...
contract NebulaProtoStarDrop is Ownable{
	...
	function transferFutureDrops(uint256 _x) external returns(uint256,uint256,uint256,uint256){
		address _account = protoMGR.getProtoAddress(_x);
		return (airdrop[_account].feesPaid,airdrop[_account].claimed,airdrop[_account].transfered,airdrop[_account].dropped);
	...
	function updateManagers(address newVal) external onlyOwner {
    	...
    	function updateProtoManager(address newVal) internal onlyOwner {
