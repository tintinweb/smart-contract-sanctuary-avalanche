    ...
    ERC721Burnable
{
    ...
    address public royaltyAddress = 0x16F6F9406b556C9C32daC0AF5763692604CAdBb5;
    ...
    uint256 public maxPerPerson = 250;
    uint256 public price = .01 ether;
    uint256 public burnBase = 5;
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("AVAX Check", "AVAXC") {     
    ...
    function _baseURI() internal view override returns (string memory) {
       ...
       function _burnBase() internal view returns (uint256) {
        return burnBase;
    }
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        function burnToMint(uint256[] memory tokenIds) public {
        //require that the range of tokens in the index are between 1 and the burnCap number
        require(burnPaused == false, "Burning is Paused");
        require(tokenIds.length % burnBase == 0, "Must be a multiple of base number");

        uint256 quantity = tokenIds.length / burnBase;
    ...
    function updateburnPaused(bool _burnPaused) external onlyOwner {
    ...
    function setBurnBase(uint256 newBase) external onlyOwner {
        burnBase = newBase;
    }
