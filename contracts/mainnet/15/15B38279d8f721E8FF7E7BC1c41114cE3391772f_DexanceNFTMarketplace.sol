...
import "./interfaces/IDexanceNFTTreasuryPool.sol";
import "./interfaces/IDexanceNFTMarketplace.sol";

contract DexanceNFTMarketplace is IDexanceNFTMarketplace, ERC721, IERC721Pausable{
    ...
    address public dxnTreasuryPool;

    constructor() ERC721("Dexance NFT Marketplace", "DXNM") {
        tokenId = 1;
    ...
    function setPool(address pool) public onlyOwner(){
        ...
        if(dxnTreasuryPool != address(0)){
            IDexanceNFTTreasuryPool(dxnTreasuryPool).migratePool(pool);
        }
        dxnTreasuryPool = pool;
    ...
    function burnByLaunchpad(address account,uint256 _tokenId) external override tokenNotFound(_tokenId) onlyLaunchpad(){
        require(_rootTokens[_tokenId], "Bad root");
    ...
    function burn(uint256 _tokenId) external override tokenNotFound(_tokenId) isNotRootToken(_tokenId) isNotFragments(_tokenId)  {
        require(ownerOf(_tokenId) == msg.sender, "Bad owner of token");
        _burn(msg.sender, _tokenId);
    }

    function setDefaultAmountEarn(address _user, address _tokenAddress) external override{
        require(msg.sender == dxnTreasuryPool, "Bad Pool");
    ...
    function mint(address _to,address _token, string memory _uri, uint256 _royalty, bool _isRoot) public override onlyEOA {
        require(_token == address(0) || _token.isContract(), "Not a contract address");
    ...
    function mintFragment(address _to,uint256 _rootTokenId) public override onlyLaunchpad(){
        require(tokenAddress[_rootTokenId] == address(0) || tokenAddress[_rootTokenId].isContract(), "Not a contract address");
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isNotRootToken(_tokenId) onlyEOA { 
        ...
        if(_subOwnerFee > 0 && flag){
            IDexanceNFTTreasuryPool(dxnTreasuryPool).subOwnerFeeBalance{value: _subOwnerFee}();
        }
        if(_subOwnerFee > 0 && !flag){
...
interface IDexanceNFTTreasuryPool {
