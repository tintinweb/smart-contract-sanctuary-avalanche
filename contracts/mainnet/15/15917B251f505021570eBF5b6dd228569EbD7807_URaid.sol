...
import "./interfaces/IURaidOld.sol";
...
contract URaid is IURaid, Ownable, ReentrancyGuard, Pausable {
  ...
  // No BLOOD is paid back for "blocking" a spot in the Raid and then leaving
  ...
  function decideRaid(uint16 raidId, uint256 seed) external whenNotPaused onlyAdmin {
    ...
    require(participantsM.length > 1, "Raid: Nobody to fight with");
    // At this point, an NFT may have lost a level and thus it needed to be removed BUT we cannot expect maxParticipants to be met anymore at this point!
    // require(participantsM.length >= raidM.maxParticipants, "Raid: Raid is not filled up yet"); // DO NOT use this line
    ...
    for (uint16 i = 0; i < participantsM.length; i++) {
      ...
      if (participantsM[i].tokenId == winnerTokenId) {
        ...
        if (bloodRewardNew > 0) uBlood.mint(tokenOwner, bloodRewardNew);
        if (goldRewardNew > 0) uGold.mint(tokenOwner, goldRewardNew);
  ...
  function removeFromRaid(uint16 raidId, uint256 tokenId) external onlyOwner {
    Raid memory raidM = raids[raidId];

    _removeFromRaidUnsafe(raidId, tokenId);

    address tokenOwner = uGame.getOwnerOfFYToken(tokenId);
    // Give back the enter fee
    uBlood.mint(tokenOwner, raidM.bloodToEnter + 25_000 ether);
  }

  function removeAllFromRaid(uint16 raidId) external onlyOwner {
    Raid memory raidM = raids[raidId];
    RaidParticipant[] memory participantsM = raidParticipants[raidId];

    for (uint16 i = 0; i < participantsM.length; i++) {
      uint256 tokenId = participantsM[i].tokenId;
      // Remove tokenId from Raid (storage)
      _removeFromRaidUnsafe(raidId, tokenId);

      address tokenOwner = uGame.getOwnerOfFYToken(tokenId);
      // Give back the enter fee (storage)
      uBlood.mint(tokenOwner, raidM.bloodToEnter + 25_000 ether);
    }
  }

  // Execute this only once as it does not do any checks thus it will pay back the BLOOD at every execution
  function payBackBloodForAllActiveRaids(address raidContract) external onlyOwner {
    IURaidOld raidContractInstance = IURaidOld(raidContract);

    IURaidOld.Raid[] memory activeRaids = raidContractInstance.getActiveRaids();

    for (uint256 r = 0; r < activeRaids.length; r++) {
      uint16 raidId = activeRaids[r].id;
      IURaidOld.Raid memory raidM = raidContractInstance.raids(raidId);

      uint256 participantsSize = raidContractInstance.getRaidParticipantsSize(raidId);

      for (uint16 i = 0; i < participantsSize; i++) {
        IURaidOld.RaidParticipant memory participantM = raidContractInstance.raidParticipants(raidId, i);

        address tokenOwner = uGame.getOwnerOfFYToken(participantM.tokenId);
        // Give back the enter fee
        uBlood.mint(tokenOwner, raidM.bloodToEnter + 25_000 ether);
      }
    }
  ...
  function setBloodTaxCollected(uint256 taxCollected) external onlyOwner {
    bloodTaxCollected = taxCollected;
  }
...
interface IURaidOld {
  ...
  function raids(uint256 raidId) external returns(Raid memory);
  function getActiveRaids() external view returns(Raid[] memory);
  function getRaidParticipantsSize(uint16 raidId) external view returns (uint256);
  function raidParticipants(uint16 raidId, uint256 index) external returns(RaidParticipant memory);
