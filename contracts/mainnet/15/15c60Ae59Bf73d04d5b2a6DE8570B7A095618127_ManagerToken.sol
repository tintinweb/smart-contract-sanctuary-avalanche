...
contract ManagersNFT is ERC721Enumerable {
    ...
    uint256 public GUEST_PRICE = 8000 ether;
    IERC20 guestToken;
    ...
    constructor(
        ...
        _owner = msg.sender;
        baseURL = _baseURL;
        guestToken = IERC20(_guestTokenAddress);
    }

    // Make this internal
    function mint(uint256 numberOfMints, address owner) public {
        require(saleIsActive, "Sale is not active");
        require(
            totalSupply() + numberOfMints <= MAX_NFT_SUPPLY,
            "All managers are minted"
        );
    ...
    function mintWithGuest(uint256 amount) public {
        require(
            amount > 0 && amount <= 10,
            "Purchase exceed max limit per transaction"
        );
        require(
            guestToken.transferFrom(
                msg.sender,
    ...
    function freeMint(
        ...
        require(
            freeRedeemed[msg.sender] + numberOfMints <= totalGiven,
            "Exceeded nubmer of mints"
        );
    ...
    function setGUESTPRICE(uint256 price) public onlyOwner {
        GUEST_PRICE = price;
    }

    function setContractAddress(address addr) public onlyOwner {
        guestToken = IERC20(addr);
    }
...
// File: contracts/ManagerToken.sol

pragma solidity ^0.8.0;

contract ManagerToken is ERC20("Manager", "MNGR") {
    using SafeMath for uint256;
    mapping(address => bool) public whitelisted;

    uint256 public BASE_RATE = 10 ether;
    uint256 public MAX_SUPPLY = 28000000 ether;
    uint256 public START = 0;
    address private _owner;

    mapping(uint256 => uint256) public lastUpdate;
    ManagersNFT NFTContract;

    constructor(address _contractAddress) {
        START = block.timestamp;
        _owner = msg.sender;
        whitelistUser(_owner);
        NFTContract = ManagersNFT(_contractAddress);
    }
    ...
    function getTotalClaimable(address _user) public view returns (uint256) {
        ...
        uint256[] memory ids = NFTContract.walletOfOwner(_user);
        for (uint256 i = 0; i < ids.length; i++) {
            pending += BASE_RATE
                .mul(time.sub(max(lastUpdate[ids[i]], START)))
                .div(86400);
        }
        return pending;
    }
    ...
    function getReward(address _from) public {
        uint256 count = getTotalClaimable(_from);
        setClaimed(_from);
        mint(_from, count);
    }

    function extmint(address _to, uint256 _amount) external {
    ...
    function setContractAddress(address addr) public onlyOwner {
        NFTContract = ManagersNFT(addr);
    }

    function setBaseRate(uint256 rate) public onlyOwner {
        BASE_RATE = rate;
    }
