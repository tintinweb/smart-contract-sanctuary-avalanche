...
// File contracts/BtcbAvaxSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract BtcbAvaxSoliSnek  is Ownable, Pausable {
    ...
    address public constant btcb = address(0x152b9d0FdC40C096757F570A51E494bd4b943E50);
    address public want;
    ...
    ISoliSnekRouter.Routes[] public snekToBtcbPath;
    ...
    constructor(
        ...
        ISoliSnekRouter.Routes[] memory _snekToBtcbPath,
        ...
        for (uint i; i < _snekToBtcbPath.length; ++i) {
            snekToBtcbPath.push(_snekToBtcbPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToBtcbPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(btcb).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wavax, btcb, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(btcb).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(btcb).safeApprove(router, type(uint).max);
    }
