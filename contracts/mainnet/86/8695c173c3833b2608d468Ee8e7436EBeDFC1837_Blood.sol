...
contract Blood is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 21000 ether;
    ...
    /**
     * @notice Constructs the BLOOD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("blood.finance", "BLOOD") {
        // Mints 1 BLOOD to contract creator for initial pool setup
    ...
    function _getBloodPrice() internal view returns (uint256 _bloodPrice) {
        try IOracle(bloodOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Blood: failed to fetch BLOOD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bloodPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_bloodPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBloodOracle(address _bloodOracle) public onlyOperatorOrTaxOffice {
        require(_bloodOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BLOOD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BLOOD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBloodPrice = _getBloodPrice();
            currentTaxRate = _updateTaxRate(currentBloodPrice);
            if (currentBloodPrice < burnThreshold) {
                burnTax = true;
            }
        }
