...
// File: contracts/ASTRO/interface/IJoeRouter02.sol
...
// File: contracts/ASTRO/interface/IAstroToken.sol
...
// File: contracts/ASTRO/DashboardFactory.sol
...
contract DashboardFactory is Ownable {
    ...
    uint256 public CLAIM_PERIOD                             = 60 * 60 * 24 * 7;                 // Seconds for one day
    uint256 public CLAIM_CAP                                = 5 * 10 ** 3 * 10 ** 6;            // $5000 as a claim cap
    ...
    function claimASTRO() external {
        require(userClaimTimes[msg.sender] + CLAIM_PERIOD < block.timestamp, "User can't claim several time per one day.");
        
        uint256 userBalance = _astroAddress.balanceOf(msg.sender);
        uint256 claimAmount = userBalance / 100;

        uint256 claimUSDC = getUSDCFromASTRO(claimAmount);
        if (claimUSDC > CLAIM_CAP) {
            claimAmount = getASTROFromUSDC(CLAIM_CAP);
        }
    ...
    function getASTROFromUSDC(uint256 _amount) public view returns (uint256) {
        address[] memory path = new address[](3);
        path[0] = address(_usdcAddress);
        path[1] = _joe02Router.WAVAX();
        path[2] = address(_astroAddress);

        uint256[] memory price_out = _joe02Router.getAmountsIn(_amount, path);
        return price_out[2];
    }
    ...
    function setClaimCap(uint256 _cap) external onlyOwner {
        CLAIM_CAP = _cap;
    }
