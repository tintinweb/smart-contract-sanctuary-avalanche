...
contract KSElasticLMHelper is IKSElasticLMHelper, KSAdmin {
  ...
  function getCurrentUnclaimedReward(
    ...
    (
      ,
      ,
      IKSElasticLMV2.PhaseInfo memory phase,
      uint256 fLiq,
      ,
      uint256[] memory sumRewardPerLiquidity,
      uint32 lastTouchedTime
  ...
  function getEligibleRanges(
    ...
    (address poolAddr, IKSElasticLMV2.RangeInfo[] memory rangesInfo, , , , , ) = farm.getFarm(fId);
      ...
      if (rMintQty != 0) {
        ...
        unchecked {
          if (governmentFeeUnits != 0) {
            uint256 rGovtQty = (rMintQty * governmentFeeUnits) / MathConstants.FEE_UNITS;
            rMintQty -= rGovtQty;
          }
          feeGrowthGlobal += FullMath.mulDivFloor(rMintQty, MathConstants.TWO_POW_96, baseL);
        }
      }
    }
    unchecked {
