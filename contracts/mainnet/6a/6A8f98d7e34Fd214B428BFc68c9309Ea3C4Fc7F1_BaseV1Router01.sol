...
contract BaseV1Router01 {
    ...
    receive() external payable {
        assert(msg.sender == address(wavax)); // only accept ETH via fallback from the WETH contract
    }
    ...
    function addLiquidityAVAX(
        ...
        address pair = pairFor(token, address(wavax), stable);
    ...
    function removeLiquidityAVAX(
        ...
        address to,
        uint deadline
    ) public ensure(deadline) returns (uint amountToken, uint amountAVAX) {
    ...
    function removeLiquidityAVAXWithPermit(
        ...
        uint value = approveMax ? type(uint).max : liquidity;
        IBaseV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountAVAX) = removeLiquidityAVAX(token, stable, liquidity, amountTokenMin, amountAVAXMin, to, deadline);
    }
    ...
    {
        require(routes[0].from == address(wavax), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(msg.value, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        wavax.deposit{value: amounts[0]}();
        assert(wavax.transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
    }
    ...
    returns (uint[] memory amounts)
    {
        require(routes[routes.length - 1].to == address(wavax), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(amountIn, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
