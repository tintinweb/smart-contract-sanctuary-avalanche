...
/** @dev This is core contract for making token sale process:
  ...
  */   
    ...
    struct saleRecord {
        ...
        uint256 minUserStakeAmount; // min possible amount of stake per user
     ...
     * @notice dates array:
     * @notice 0-approveTimestampStart
     * @notice 1-approveTimestampStop
     * @notice 2-saleTimestampStart
     * @notice 3-saleTimestampStop
     * @notice 4-claimTimestampStart
     * @notice 5-claimTimestampStop
     * @param dates dates array
     * @param maximumParticipants maximum count of wallets (unique) allowed for the sale
     * @param minStakeAmount min amount of stake per user
     * @param maxStakeAmount max amount of stake per user
     * @param stakeContract stake contract
     */
    ...
    function createSale(
        ...
        sales[totalSales] = saleRecord({
            ...
            staking: ISwapStakingContract(stakeContract),
            dates: dates,
            tokenPrice: tokenPrice,
            minUserStakeAmount: minStakeAmount,
            maxUserStakeAmount: maxStakeAmount,  //MAX user TAKE
    ...
    function withdraw(uint256 saleID) external {
        ...
        require(block.timestamp >= sales[saleID].dates[4], "Sale::withdraw: only from CLAIM stage");
    ...
    function registerStake(uint256 saleID, bytes32 _msgForSign, bytes memory _signature) external {
        ...
        require(curStake >= sales[saleID].minUserStakeAmount, "Sale::registerStake: too little stake for register");
    ...
    function getSalestage(uint256 saleId) public view virtual returns (stages) {
        ...
        if (
            ...
            block.timestamp < sales[saleId].dates[0] ||
            block.timestamp > sales[saleId].dates[5]
        ) {
            return (stages.UNKNOWN);
        } else if (block.timestamp >= sales[saleId].dates[0] && block.timestamp <= sales[saleId].dates[1]) {
            return (stages.APPROVE);
        } else if (block.timestamp >= sales[saleId].dates[2] && block.timestamp <= sales[saleId].dates[3]) {
            return (stages.SALE);
        } else if (block.timestamp >= sales[saleId].dates[4] && block.timestamp <= sales[saleId].dates[5]) {
            return (stages.CLAIM);
        } else {  
            return stages.UNKNOWN;
        }
    ...
    {
        return (
            sales[saleID].minUserStakeAmount, 
