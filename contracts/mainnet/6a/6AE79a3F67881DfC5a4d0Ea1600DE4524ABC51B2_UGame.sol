...
contract UGame is IUGame, Ownable, ReentrancyGuard, Pausable {
  ...
  function getBloodGoldSwapParameters(address addr) public view returns(uint256, uint256) {
    ...
    if (totalGoldBalance < 1 ether) {
      goldBloodCost = GOLD_BLOOD_MINT_COST;
    } else {
      goldBloodCost = GOLD_BLOOD_MINT_COST + (GOLD_BLOOD_MINT_COST * totalGoldBalance / 1 ether) * 1.5 ether / 1 ether;
    }
