...
contract PLPPriceFeed is IPriceFeed {
    using UQ112x112 for uint224;
    using SafeMathJoe for uint256;
    IPriceFeed public AVAXPriceFeed;
    JoePair public PLPAVAX;
    address public PLPToken;
    constructor(address LPToken, address _PLPToken, address _AVAXPriceFeed) public {
        PLPAVAX = JoePair(LPToken);
        PLPToken = _PLPToken;
        AVAXPriceFeed = IPriceFeed(_AVAXPriceFeed);
    }

    function fetchPrice_v() view external override returns (uint256) {
        address t0 = PLPAVAX.token0();
        address t1 = PLPAVAX.token1();
        (uint112 r0, uint112 r1, ) = PLPAVAX.getReserves();
