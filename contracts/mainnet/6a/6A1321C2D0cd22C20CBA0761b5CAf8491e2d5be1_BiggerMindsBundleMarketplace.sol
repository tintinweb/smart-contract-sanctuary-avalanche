...
contract BiggerMindsBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IBiggerMindsTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "BiggerMindsMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "BiggerMindsMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IBiggerMindsMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IBiggerMindsMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            IBiggerMindsMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
