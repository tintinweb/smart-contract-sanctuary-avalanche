...
contract StabilityPool is PoolBase, ReentrancyGuardUpgradeable, IStabilityPool {
    ...
    event PendingCollGainsUpdated(address indexed _depositor, address[] _collaterals, uint256[] _amounts);
    event YUSDFromRewardSwap(address indexed _depositor, uint256 _YUSDAmount);
    ...
    {
        require(_yusdMinAmountTotal != 0, "SP: Nonzero swap");
        ...
        emit YUSDFromRewardSwap(msg.sender, amountFromSwap);
        if (_reinvest) {
        ...
        returns (uint256 totalYUSD)
    {
        ...
        uint256 swapAssetsLen = _swapAssets.length;
        require(swapAssetsLen <= assetsLen, "SP:Swap length mismatch");
        ...
        if (swapAssetsLen == 0) {
            for (uint256 i; i < assetsLen; ++i) {
                ...
                delete pendingCollGains[msg.sender];
                // Should emit empty
                emit PendingCollGainsUpdated(msg.sender, pendingCollGains[msg.sender].tokens, pendingCollGains[msg.sender].amounts);
            } 
        } else {
            ...
            for (; i < assetsLen && j < swapAssetsLen; ++i) {
                uint256 amount = _amounts[i];
                address asset = _rewardAssets[i];
                if (amount == 0) {
                    continue;
            ...
            emit PendingCollGainsUpdated(msg.sender, tempPendingCollGains.tokens, tempPendingCollGains.amounts);
        }
        require(
