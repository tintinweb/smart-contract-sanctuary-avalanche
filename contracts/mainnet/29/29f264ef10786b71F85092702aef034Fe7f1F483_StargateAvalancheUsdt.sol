...
contract StargateAvalancheUsdt is Defii {
    using SafeERC20 for IERC20;

    IERC20 constant STG = IERC20(0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590);
    IERC20 constant USDT = IERC20(0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7);
    IERC20 constant SUSDT = IERC20(0x29e38769f23701A2e4A8Ef0492e19dA4604Be62c);

    IRouter constant StargateRouter =
        IRouter(0x45A01E4e04F14f7A4a6702c74187c5F6222033cd);
    ILPStaking constant LPStaking =
        ILPStaking(0x8731d54E9D02c286767d56ac03e8037C07e01e98);

    uint16 constant ROUTER_POOL_ID = 2;
    uint256 constant STAKING_PID = 1;

    function _enter() internal override {
        uint256 usdtBalance = USDT.balanceOf(address(this));
        USDT.safeApprove(address(StargateRouter), usdtBalance);
        StargateRouter.addLiquidity(ROUTER_POOL_ID, usdtBalance, address(this));

        uint256 lpAmount = SUSDT.balanceOf(address(this));
        SUSDT.safeApprove(address(LPStaking), lpAmount);
        LPStaking.deposit(STAKING_PID, lpAmount);
    }

    function _exit() internal override {
        uint256 lpAmount = LPStaking.userInfo(STAKING_PID, address(this)).amount;
        LPStaking.withdraw(STAKING_PID, lpAmount);
        StargateRouter.instantRedeemLocal(ROUTER_POOL_ID, lpAmount, address(this));
        _claimIncentive(STG);
    }

    function hasAllocation() public view override returns (bool) {
        return LPStaking.userInfo(STAKING_PID, address(this)).amount > 0;
    }

    function _harvest() internal override {
        LPStaking.withdraw(STAKING_PID, 0);
        _claimIncentive(STG);
    }
