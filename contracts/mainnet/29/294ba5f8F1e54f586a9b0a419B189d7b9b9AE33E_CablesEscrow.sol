...
// File contracts/enums/ExecutionFacility.sol
...
// File contracts/enums/OrderStatus.sol
...
// File contracts/enums/OrderType.sol
...
// File contracts/enums/TradeType.sol
...
// File contracts/types/Order.sol
...
// File contracts/CablesEscrow.sol
...
contract CablesEscrow is Ownable, ReentrancyGuard {
    ...
    uint constant FEE_MULTIPLIER = 100 * (10 ** 3);
    uint public feePortion = 40; // 0.04%
    ...
    **/
    function setFeePortion(uint _feePortion) external onlyOwner {
        ...
        require(_feePortion <= MAX_FEE, "fee exceeds max");
    ...
    **/
    function cancelOrder(uint _escrowId) public nonReentrant {
        ...
        if (escrow.order.orderType == OrderType.Sell) {
            transferToUser(escrow.order.baseCurrency, escrow.user, escrow.balance);
        } else {
            transferToUser(escrow.order.quoteCurrency, escrow.user, escrow.balance);
        }
    ...
    **/
    function exchangeAssets(
    ...
    ) private returns (uint filled, uint price) {
        ...
        if (_sellEscrow.balance >= _buyEscrow.order.baseAmount) {
            ...
            if (_buyEscrow.order.quoteAmount == 0) {
                ...
                quoteAmount = _buyEscrow.order.quoteAmount;
            }

            _sellEscrow.balance -= filled;
            _sellEscrow.order.baseAmount -= filled;
            ...
            if (_sellEscrow.order.quoteAmount == 0) {
                quoteAmount = calcQuoteAmount(
                    filled,
                    baseCurrency,
                    price
                ...
                quoteAmount = _sellEscrow.order.quoteAmount;
            }

            _sellEscrow.balance = 0;
            _sellEscrow.order.baseAmount = 0;
            _buyEscrow.balance -= quoteAmount;
            _buyEscrow.order.baseAmount -= filled;
        }

        if (!isExecutionFacility(_buyEscrow.user)) {
            buyEscrowUserFeeAmount = filled * feePortion / FEE_MULTIPLIER;
        }
        transferToUser(baseCurrency, _buyEscrow.user, filled - buyEscrowUserFeeAmount);

        if (!isExecutionFacility(_sellEscrow.user)) {
            sellEscrowUserFeeAmount = quoteAmount * feePortion / FEE_MULTIPLIER;
        }
        transferToUser(quoteCurrency, _sellEscrow.user, quoteAmount - sellEscrowUserFeeAmount);

        if (buyEscrowUserFeeAmount != 0) {
            transferToUser(baseCurrency, feeReceiver, buyEscrowUserFeeAmount);
        }

        if (sellEscrowUserFeeAmount != 0) {
    ...
    **/
    function _fill(
        ...
        require(
            makerEscrow.order.orderType != takerEscrow.order.orderType,
            "Order types not suitable"
        );

        uint quantityFilled;
        uint price;
        if (makerEscrow.order.orderType == OrderType.Buy) {
            (quantityFilled, price) = exchangeAssets(makerEscrow, takerEscrow, OrderType.Buy);
        } else {
            (quantityFilled, price) = exchangeAssets(takerEscrow, makerEscrow, OrderType.Sell);
        }

        if (makerEscrow.balance == 0) {
            closeOrder(_makerEscrowId,  price, quantityFilled, false);
        } else {
            updateStatus(
                _makerEscrowId,
                price,
        ...
        if (takerEscrow.balance == 0) {
            closeOrder(_takerEscrowId,  price, quantityFilled, false);
        } else {
            updateStatus(
                _takerEscrowId,
                price,
    ...
    **/
    function gasCompensation(
        ...
        if (makerEscrow.balance == 0) {
            compensation = (_gasAmountLeft - gasleft() + 24145) * tx.gasprice;
            ...
            compensation = (_gasAmountLeft - gasleft() + 24002) * tx.gasprice;
