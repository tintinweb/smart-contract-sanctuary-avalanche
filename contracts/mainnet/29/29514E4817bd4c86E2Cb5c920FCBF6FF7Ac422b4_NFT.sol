...
contract NFT is INFT, ERC721Enumerable, IERC2981, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor() ERC721("The Joker", "The Joker") {
        // _pause();
    }
    ...
    uint256 public override MAX_GEN0_TOKENS = 12000;
    // number of tokens have been minted so far
    uint16 public override tokensMinted;
    
    /** PRIVATE VARS */
    mapping(address => bool) private _admins;
    // available ranks for Villain (has to be sorted ASC)
    uint8[4] private _VillainRanks = [5, 6, 7, 8];
    mapping(uint256 => HeroVillain) private _tokenTraits;
    ...
    uint256 private _royaltyPermille = 200;
    address royaltyAddress= address(0x953666239e8e3Cc4128B8f47F5F3492775537671);
    /** MODIFIERS */
    ...
    function mint(address recipient, bool isGen0) external override whenNotPaused  nonReentrant onlyAdmin {
    ...
    function setTraitLevel(uint256 tokenId, uint16 level) external override whenNotPaused onlyAdmin {
        HeroVillain memory traits = _tokenTraits[tokenId];
        traits.level = level;
    ...
    function getTokenTraits(uint256 tokenId) external view override onlyAdmin returns (HeroVillain memory) {
        return _tokenTraits[tokenId];
    }

    function revealTokenId(uint16 tokenId, uint256 seed) external override  onlyAdmin {
        _writeTokenTraits(tokenId, seed);
        emit TokenRevealed(_msgSender(), tokenId);
    }

    /** PRIVATE FUNCTIONS */
    function _writeTokenTraits(uint256 tokenId, uint256 seed) private  returns (HeroVillain memory traits) {
        ...
        // Hero or Villain
        traits.isHero = (seed & 0xFFFF) % 20 != 0;

        // Rank
        seed >>= 16;
        uint8 rankIndex = uint8(seed & (_VillainRanks.length - 1)); // 0-3
        traits.rank = _VillainRanks[rankIndex]; // [5, 6, 7, 8];
        traits.lastRankUpgradeTime = block.timestamp;
    ...
    function getVillainRanks() external view override onlyAdmin returns(uint8[4] memory) {
        return _VillainRanks;
    }
    ...
    function isHero(uint256 tokenId) external view override  returns (bool) {
        require(_exists(tokenId), "NFT: Token does not exist");
        HeroVillain memory traits = _tokenTraits[tokenId];
        require(traits.isRevealed, "NFT: Token is not yet revealed");
        
        return traits.isHero;
    }

    function tokenURI(uint256 tokenId) public view override  returns (string memory) {
        string memory jsonString;

        HeroVillain memory traits = uGame.getHVTokenTraits(tokenId);
        if (_exists(tokenId) && traits.isRevealed) {
            jsonString = string(abi.encodePacked(
                jsonString,
                Strings.toString(tokenId),",",
                Strings.toString((traits.isHero)? 1 : 0),",",
    ...
    function updateMaxGen0Token(uint _amount) external onlyOwner {
        MAX_GEN0_TOKENS = _amount;
    }

    function updateSupply(uint _amount) external onlyOwner {
        MAX_TOKENS = _amount;
    }
}
