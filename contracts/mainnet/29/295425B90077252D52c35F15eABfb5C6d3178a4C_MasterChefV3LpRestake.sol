...
contract MasterChefLpRestake is Automate {
  ...
  function run(
    ...
    Swap memory swap0,
    Swap memory swap1
  ) external bill(gasFee, "BscApeSwapMasterChefLpRestake") {
    IMasterChef _staking = staking; // gas optimization
    require(_staking.pendingCake(pool, address(this)) > 0, "MasterChefLpRestake::run: no earned");
 ...
 * @notice Use with LP token only.
 */
contract MasterChefV3LpRestake is Automate {
  using ERC20Tools for IERC20;
  ...
  function init(
    ...
    require(
      !_initialized || address(staking) == _staking,
      "MasterChefV3LpRestake::init: reinitialize staking address forbidden"
    );
    staking = IMasterChefV3(_staking);
    require(
      !_initialized || liquidityRouter == _liquidityRouter,
      "MasterChefV3LpRestake::init: reinitialize liquidity router address forbidden"
    );
    liquidityRouter = _liquidityRouter;
    require(!_initialized || pool == _pool, "MasterChefV3LpRestake::init: reinitialize pool index forbidden");
    pool = _pool;
    slippage = _slippage;
    deadline = _deadline;

    if (!_initialized) {
      IMasterChefV3.PoolInfo memory poolInfo = staking.poolInfo(pool);
      stakingToken = IERC20(poolInfo.lpToken);
      rewardToken = IERC20(staking.JOE());
    }
  }
  ...
  function refund() external onlyOwner {
    IMasterChefV3 _staking = staking; // gas optimisation
    address __owner = owner(); // gas optimisation
    IMasterChefV3.UserInfo memory userInfo = _staking.userInfo(pool, address(this));
    _staking.withdraw(pool, userInfo.amount);
    stakingToken.transfer(__owner, stakingToken.balanceOf(address(this)));
    rewardToken.transfer(__owner, rewardToken.balanceOf(address(this)));
  }
  ...
  function run(
    ...
    Swap memory swap0,
    Swap memory swap1
  ) external bill(gasFee, "AvaxTraderjoeMasterChefV3LpRestake") {
    IMasterChefV3 _staking = staking; // gas optimization
    (uint256 earned, , , ) = _staking.pendingTokens(pool, address(this));
    require(earned > 0, "MasterChefV3LpRestake::run: no earned");
