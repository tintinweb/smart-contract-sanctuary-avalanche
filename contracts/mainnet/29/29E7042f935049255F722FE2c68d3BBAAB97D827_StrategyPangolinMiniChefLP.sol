...
contract StrategyPangolinMiniChefLP is Ownable, Pausable, GasThrottler{
    ...
    function chargeFees() internal {
        if (rewardToOutputRoute.length != 0) {
            for (uint i=0; i < rewardToOutputRoute.length; i++) {
    ...
    function rewardsAvailable() public view returns (uint256, uint256[] memory) {
        ...
        uint256[] memory secondaryRewards = new uint256[](0);
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeBal = 0;
        uint256[] memory amountOut = IUniswapRouterETH(unirouter).getAmountsOut(pngReward, outputToNativeRoute);
        nativeBal = amountOut[amountOut.length -1];

        if (rewardToOutputRoute.length != 0) {
            for (uint i=0; i < rewardToOutputRoute.length; i++) {
                uint256[] memory initialAmountOut = IUniswapRouterETH(unirouter).getAmountsOut(secondaryRewards[i], rewardToOutputRoute[i]);
                uint256 outputBal = initialAmountOut[initialAmountOut.length - 1];
                uint256[] memory finalAmountOut = IUniswapRouterETH(unirouter).getAmountsOut(outputBal, outputToNativeRoute);
                nativeBal += finalAmountOut[finalAmountOut.length - 1];
            }
        }

        (uint256 totalFee, uint256 baseFee) = IFeeTierStrate(feeStrate).getTotalFee();
        return nativeBal.mul(totalFee).div(baseFee);
    }
...
interface IPangolinRewarder {
    function pendingTokens(uint256 _pid, address _user, uint256 _rewardAmount) external view returns (address[] memory tokens, uint256[] memory amounts);
}
