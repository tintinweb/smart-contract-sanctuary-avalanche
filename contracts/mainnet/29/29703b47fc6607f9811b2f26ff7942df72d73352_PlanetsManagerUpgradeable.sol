    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        __ERC721_init("Universe Ecosystem", "FORK");
        __Ownable_init();
        ...
        changeCompoundDelay(60); // 1 minute
        changeTierSystem(
    ...
    function addPresaleAddressesAndPlanet(
        address[] memory _presales,
        uint256[] memory _planetValues
    ) external nonReentrant whenNotPaused universeSet onlyOwner {
        for (uint i = 0; i < _presales.length; i++) {
            presales[_presales[i]] = _planetValues[i];
        }
    }
    ...
    function changeTierSystem(
        ...
        require(
            _tierMax.length == 6,
            "Planets: newTierMax length has to be 6"
        );
