...
contract eXpanzProject is Initializable {
    ...
    function _swapBack(uint r1) public returns (uint) {
        ...
        // SENDAVAX(_stabilizer, usdcAmount.mul(stabilizerFee).div(totalFee));
        // SENDAVAX(_treasury, usdcAmount.mul(treasuryFee).div(totalFee));
    ...
    function _swapTokensForUsdc(uint256 tokenAmount) internal swapping {
        ...
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664);
        path[2] = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
        path[3] = address(0xc7198437980c041c805A1EDcbA50c1Ce5db95118);
        path[4] = address(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664);

        // _approve(address(this), _uniswapV2Router, tokenAmount);

        // make the swap
        IJoeRouter02(_uniswapV2Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
    ...
    function _swapUsdcForTokens(uint256 usdcAmount, address to) internal swapping {
        if (usdcAmount == 0) { // no AVAX. skip
            return;
        }

        address[] memory path = new address[](2);
        path[0] = address(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664);
        path[1] = address(this);
