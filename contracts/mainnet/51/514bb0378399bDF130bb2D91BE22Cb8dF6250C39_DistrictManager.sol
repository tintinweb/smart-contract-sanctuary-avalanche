...
contract DistrictManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    DistrictManager public DISTRICTS_V1 = DistrictManager(0x660f476F39249B848572273265370B14cF739308);
    ...
    function totalNodesCreated() view external returns (uint) {
        return nodeCounter + DISTRICTS_V1.totalNodesCreated();
    }
    ...
    function migrate() external {
        ...
        uint256 len = DISTRICTS_V1.balanceOf(msg.sender);
        uint256[] memory nodeIds = DISTRICTS_V1.getDistrictsOf(msg.sender);
        for (uint16 i = 0; i < len; i++) {
            (string memory _name, string memory _metadata, uint256 _id, uint64 _mint, uint64 _claim) = (DISTRICTS_V1.getDistricts(nodeIds[i]).name, DISTRICTS_V1.getDistricts(nodeIds[i]).metadata, DISTRICTS_V1.getDistricts(nodeIds[i]).id, DISTRICTS_V1.getDistricts(nodeIds[i]).mint, DISTRICTS_V1.getDistricts(nodeIds[i]).claim);
            _nodes[_id] = District({
                name: _name,
                mint: _mint,
                claim: _claim,
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter + DISTRICTS_V1.totalNodesCreated();
