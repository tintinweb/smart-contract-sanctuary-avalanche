...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    {
        return
            _getTierLevel(_upline) > _downlineDepth && isNetPositive(_upline);
    }
    ...
    /**
     * @dev - Calculates total credits and debits for a given address
     * @param _user - User address for which we want to calculate credits and debits
     * @return _credits uint256, _debits uint256 - Returns the total credits and debits for a given address
     */
    function creditsAndDebits(address _user)
        public
        view
        noZeroAddress(_user)
        returns (uint256 _credits, uint256 _debits)
    {
        _credits = airdrops[_user].airdropsReceived + users[_user].depositsNCTR;
        _debits = users[_user].payouts;
    }

    /**
     * @dev - Check if the address is net positive
     * @param _user - User we want to check if it's net positive
     * @return bool - Returns true if the address is net positive, false otherwise
     */
    function isNetPositive(address _user)
        public
        view
        noZeroAddress(_user)
        returns (bool)
    {
        ...
        (uint256 credits, uint256 debits) = creditsAndDebits(_user);
        ...
        onlyAmountGreaterThanZero(_amountUSDCe)
    {
        ...
        if (_getRewardEligibility(_upline, 0)) {
            ...
            if (
                users[_upline].referralCount >= MIN_NUM_OF_REF_FOR_TEAM_WALLET
            ) {
    ...
    /**
     * @dev - Claim from Bloom Treasury, calculate taxes
     * @param _claimAmount - NCTR amount to be claimed from Bloom Referral
     */
    function claim(uint256 _claimAmount)
        external
        onlyBloomReferralNode
        onlyAmountGreaterThanZero(_claimAmount)
    {
        require(
            users[msg.sender].depositsNCTR +
                airdrops[msg.sender].airdropsReceived >=
                _claimAmount + users[msg.sender].payouts,
            "Can't claim more than you have!"
        );

        uint256 treasuryBalance = nectarToken.balanceOf(address(treasury));
        if (treasuryBalance < _claimAmount) {
            uint256 differenceToMint = _claimAmount - treasuryBalance;
            nectarToken.mintNectar(address(treasury), differenceToMint);
        }

        (uint256 realizedClaimNCTR, ) = _calculateTax(_claimAmount, claimTax);
        ...
        totalWithdraw += realizedClaimNCTR;
        users[msg.sender].payouts += realizedClaimNCTR;

        emit Claim(msg.sender, realizedClaimNCTR);
    }

    /**
     * @dev - Airdrop to any address and save airdrop to Treasury. Update _to's deposit section
     * @param _to - Address to which airdrop would go
     * @param _amount - Amount to airdrop to _to address
     */
    ...
    {
        require(
            users[_to].upline != address(0) || _to == owner(),
            "Can't airdrop to someone that's not in the Bloom Referral system!"
        );

        // Transfer NCTR tokens to Treasury address
        require(
            nectarToken.transferFrom(msg.sender, address(treasury), _amount),
            "NCTR token transfer failed!"
        );

        // Update _to's stats
        users[_to].depositsNCTR += _amount;
        airdrops[_to].airdropsReceived += _amount;

        // Update user's stats
        airdrops[msg.sender].airdropsGiven += _amount;
        ...
        emit AirdropNCTR(
            ...
            _to,
            _amount,
            airdrops[msg.sender].lastAirdropTime
        );
    }
}
