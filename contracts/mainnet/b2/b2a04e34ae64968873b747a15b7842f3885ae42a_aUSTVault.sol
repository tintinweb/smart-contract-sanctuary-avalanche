...
contract aUSTVault is Vault {
    ...
    ISwapFacility public swapper = ISwapFacility(address(1)); //Just set to non null value
    IERC20 public aUST;
    ...
    function setSwapper(address _swapper) public onlyOwner {
        aUST.approve(address(swapper), 0); //Revoke approval for old swapper
    ...
    function underlyingPerReceipt() public override view returns (uint256) {
        ...
        uint256 _USTAmt = _getUSTaUST() * aUST.balanceOf(address(this));
        _USTAmt = _USTAmt * 100000 / 100001; // Steals 0.1 bps to account for chainlink error and ensures 100% solvency
        _USTAmt = max(_USTAmt, lastUSTBalance * 1e18); // This accounts for multiple deposits done between receiving aust from bridge
        return _USTAmt / totalSupply; //No need to scale by 1e18 since chainlink pricefeed already scales
    }
