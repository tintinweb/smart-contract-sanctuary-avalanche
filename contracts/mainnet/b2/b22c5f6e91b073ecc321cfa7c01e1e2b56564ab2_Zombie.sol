...
interface COTCSerum {
    ...
    function burnSerumForAddress(uint256 burnTokenId) external;
    ...
    Pausable
{
    ...
    constructor(address cotcAddress) ERC721("Potluck Labs Super Serums", "PLSS") ClampedRandomizer(maxSupply) {
        baseURI = "https://potlucklabs.mypinata.cloud/ipfs/QmbhZbDZLPW4mGpLvGu8yB6Kt3nNAepA5EiNXteL4nYmL7/";
        cotcs = cotcAddress;
    }
    ...
    function mutate(uint256 serumId) public whenNotPaused {
        require(COTCSerum(cotcs).balanceOf(msg.sender) > 0, "Must own at least one of this serum type to mutate");        
        require(COTCSerum(cotcs).ownerOf(serumId) == msg.sender,"You dont have this serum");

        COTCSerum(cotcs).safeTransferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD) , serumId);
    ...
    function internalMint(address to) internal {
        require(totalSupply() < maxSupply, "All Mutants have been minted!");
