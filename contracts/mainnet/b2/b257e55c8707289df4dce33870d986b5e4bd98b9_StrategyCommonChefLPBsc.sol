...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    function addLiquidity() internal {
        ...
        uint256 outputfull = IERC20(output).balanceOf(address(this));         
        address[] memory joePath = new address[](4); 
        address[] memory swapsiclePath = new address[](2);
        joePath[0] = outputToLp1Route[0];
        joePath[1] = outputToLp1Route[1];
        joePath[2] = outputToLp1Route[2];
        joePath[3] = outputToLp1Route[3];

        swapsiclePath[0] = outputToLp1Route[3];
        swapsiclePath[1] = outputToLp1Route[4];
        ...
        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter2).swapExactTokensForTokens(outputHalf, 0, joePath, address(this), now);
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputfull, 0, swapsiclePath, address(this), now);
        }
