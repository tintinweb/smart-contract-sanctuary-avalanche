...
// File contracts/WmemoWavaxxGlacier.sol


pragma solidity 0.8.17;
contract WmemoWavaxGlacier  is Ownable, Pausable {
    ...
    address public constant wmemo = address(0x0da67235dD5787D67955420C84ca1cEcd4E5Bb3b);
    address public want;
    ...
    IGlacierRouter.Routes[] public glcrToWmemoPath;
    ...
    constructor(
        ...
        IGlacierRouter.Routes[] memory _glcrToWmemoPath,
        ...
        for (uint i; i < _glcrToWmemoPath.length; ++i) {
            glcrToWmemoPath.push(_glcrToWmemoPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IGlacierRouter(router).swapExactTokensForTokens(glcrHalf, 0, glcrToWmemoPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(wmemo).balanceOf(address(this));

        IGlacierRouter(router).addLiquidity(wavax, wmemo, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(wmemo).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(wmemo).safeApprove(router, type(uint).max);
    }
