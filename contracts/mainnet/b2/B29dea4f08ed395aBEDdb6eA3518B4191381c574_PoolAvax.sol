...
import '../interfaces/IPriceOracleGetterV3.sol';
...
contract PoolAvax is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, Core, IPool {
    ...
    IPriceOracleGetterV3 private _priceOracle;
    ...
    function setPriceOracle(address priceOracle) external onlyOwner {
        ...
        _priceOracle = IPriceOracleGetterV3(priceOracle);
    }
    ...
    function _quoteIdealToAmount(
        ...
        (uint256 fromPrice, uint256 fromPriceFeedDecimals) = _priceOracle.getAssetPrice(fromAsset.underlyingToken());
        (uint256 toPrice, uint256 toPriceFeedDecimals) = _priceOracle.getAssetPrice(toAsset.underlyingToken());

        require(fromPriceFeedDecimals < 19 && toPriceFeedDecimals < 19, 'INVALID_DECIMALS');

        // handle different decimal points in the feeds
        fromPrice = fromPrice * 10**(18 - fromPriceFeedDecimals);
        toPrice = toPrice * 10**(18 - toPriceFeedDecimals);

        idealToAmount =
            ((fromAmount * (10**toAsset.decimals())) * (fromPrice).wdiv(toPrice)) /
...
interface IPriceOracleGetterV3 {
