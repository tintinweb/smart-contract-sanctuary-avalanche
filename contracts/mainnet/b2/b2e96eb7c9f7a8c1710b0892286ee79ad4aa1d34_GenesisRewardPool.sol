...
contract GenesisRewardPool is ContractGuard, OwnableUpgradeable {
    ...
    uint256 public constant AIRDROP_FUND_REWARDS = 6750 ether;
    uint256 public constant mainTokenPerSecondForAirdrop = 0.0260416 ether; // 6750 Token / (72h * 60min * 60s)
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return pendingUser.add(airdropReward);
    }
    ...
    function pendingAirdrop(uint256 _fromTime, uint256 _toTime, address _user) internal view returns (uint256) {
        if (isAirdrop(_user)) {
            ...
            if (_toTime >= poolEndTime) {
                if (_fromTime >= poolEndTime) return 0;
                if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(mainTokenPerSecondForAirdrop);
                return poolEndTime.sub(_fromTime).mul(mainTokenPerSecondForAirdrop);
            } else {
                if (_toTime <= poolStartTime) return 0;
                if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(mainTokenPerSecondForAirdrop);
                return _toTime.sub(_fromTime).mul(mainTokenPerSecondForAirdrop);
            }
        }
