...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    function removeFromArray(Action[] storage array,uint256 index) private returns(Action[] storage) {
    ...
    function getLastActions(address user) external view returns(Action[] memory) {
        return users[user].lastActions;
    }
    function getRealLastActions(address user) public view returns(uint256 countCompound, uint256 countClaim) {
    ...
    function claim() external onlyBloomReferralNode {
        ...
        require(getPendingReward(msg.sender) < 2000000000000000000000, "Call Dev Team");
 ...
 (uint256 countCompound, uint256 countClaim) = getRealLastActions(msg.sender);
        uint256 _claimTax;
