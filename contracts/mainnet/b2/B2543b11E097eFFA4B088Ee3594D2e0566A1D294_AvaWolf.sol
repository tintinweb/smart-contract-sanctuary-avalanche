...
contract AvaWolf is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    //change that
    uint8 public maxLiqFee = 2;                // Transaction fee % that would be used to add liquidity (0-10)
    ...
    uint8 public maxWalletFee = 3;             // Transaction fee % that would be sent to a marketing or charity wallet (0-10)
    //change that
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        pcsV2Pair = IUniswapV2Factory(_pcsV2Router.factory())
            .createPair(address(this), _pcsV2Router.WAVAX());
    ...
    function swapTokensForAVAX(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = pcsV2Router.WAVAX();

        _approve(address(this), address(pcsV2Router), tokenAmount);

       
        pcsV2Router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            tokenAmount,
            0, 
            path,
            address(this),
    ...
    function swapAVAXForTokens(uint256 amount) private {
        address[] memory path = new address[](2);
        path[0] = pcsV2Router.WAVAX();
        path[1] = address(this);

      
        pcsV2Router.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, 
            path,
            dead, 
    ...
    function addLiquidity(uint256 tokenAmount, uint256 AVAXAmount) private {
        ...
        pcsV2Router.addLiquidityAVAX{value: AVAXAmount}(
            address(this),
            tokenAmount,
            0, 
            0, 
