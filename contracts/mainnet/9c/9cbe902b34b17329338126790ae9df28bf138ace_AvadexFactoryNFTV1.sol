...
contract AvadexNFTs is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Map the number of tokens per avexId
    mapping(uint8 => uint256) public bunnyCount;

    // Map the number of tokens burnt per avexId
    mapping(uint8 => uint256) public bunnyBurnCount;

    // Used for generating the tokenId of new NFT minted
    Counters.Counter private _tokenIds;

    // Map the avexId for each tokenId
    mapping(uint256 => uint8) private bunnyIds;

    // Map the avexName for a tokenId
    mapping(uint8 => string) private bunnyNames;

    constructor(string memory _baseURI) public ERC721("Avadex NFTs", "ANFTs") {
        _setBaseURI(_baseURI);
    }

    /**
     * @dev Get avexId for a specific tokenId.
     */
    ...
    /**
     * @dev Get the associated avexName for a specific avexId.
     */
    ...
    /**
     * @dev Get the associated avexName for a unique tokenId.
     */
    ...
    /**
     * @dev Set a unique name for each avexid. It is supposed to be called once.
     */
...
contract AvadexMintingStation is AccessControl {
    AvadexNFTs public pancakeBunnies;
    ...
    constructor(AvadexNFTs _pancakeBunnies) public {
        pancakeBunnies = _pancakeBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev Mint NFTs from the AvadexNFTs contract.
     */
...
contract AvadexFactoryNFTV1 is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    AvadexMintingStation public bunnyMintingStation;
    ...
    // Number of AVEXs a user needs to pay to acquire a token
    ...
    constructor(
        AvadexMintingStation _bunnyMintingStation,
        IBEP20 _cakeToken,
        uint256 _tokenPrice,
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        // Send AVEX tokens to this contract
    ...
    /**
     * @dev It transfers the AVEX tokens back to the chef address.
