...
// File contracts/interfaces/common/IUniswapRouter.sol
...
// File contracts/interfaces/traderjoe/IMasterchef.sol
...
contract StratManager is Ownable, Pausable {
    ...
    */
    ...
    address public steakHutFeeRecipient;
     ...
     * @param _steakHutFeeRecipient address where to send SteakHut's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        address _unirouter,
        address _vault,
        address _steakHutFeeRecipient
    ){
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        steakHutFeeRecipient = _steakHutFeeRecipient;
    }
    ...
    /**
     * @dev Updates SteakHut fee recipient.
     * @param _steakHutFeeRecipient new SteakHut fee recipient address.
     */
    function setSteakHutFeeRecipient(address _steakHutFeeRecipient) external onlyOwner {
        steakHutFeeRecipient = _steakHutFeeRecipient;
    }
...
// File contracts/strategies/TraderJoe/StrategyTraderJoeDualNonNativeLP.sol
...
contract StrategyTraderJoeDualNonNativeLP is StratManager, FeeManager {
    ...
    event ChargedFees(uint256 callFees, uint256 steakHutFees, uint256 strategistFees);


    struct routes{
        address[] _outputToNativeRoute;
        address[] _secondOutputToNativeRoute;
        address[] _nativeToLp0Route;
        address[] _nativeToLp1Route;
    }
    ...
    constructor(
        ...
        address _vault
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _steakHutFeeRecipient){
        want = _want;
        poolId = _poolId;
        chef = _chef;

        output = _routes._outputToNativeRoute[0];
        native = _routes._outputToNativeRoute[_routes._outputToNativeRoute.length - 1];
        outputToNativeRoute = _routes._outputToNativeRoute;

        secondOutput = _routes._secondOutputToNativeRoute[0];
        secondOutputToNativeRoute = _routes._secondOutputToNativeRoute;

        // setup lp routing
        lpToken0 = IUniswapV2Pair(want).token0();
        require(_routes._nativeToLp0Route[0] == native, "nativeToLp0Route[0] != native");
        require(_routes._nativeToLp0Route[_routes._nativeToLp0Route.length - 1] == lpToken0, "nativeToLp0Route[last] != lpToken0");
        nativeToLp0Route = _routes._nativeToLp0Route;

        lpToken1 = IUniswapV2Pair(want).token1();
        require(_routes._nativeToLp1Route[0] == native, "nativeToLp1Route[0] != native");
        require(_routes._nativeToLp1Route[_routes._nativeToLp1Route.length - 1] == lpToken1, "nativeToLp1Route[last] != lpToken1");
        nativeToLp1Route = _routes._nativeToLp1Route;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 steakHutFeeAmount = nativeBal.mul(steakHutFee).div(MAX_FEE);
        IERC20(native).safeTransfer(steakHutFeeRecipient, steakHutFeeAmount);

        uint256 strategistFee = nativeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(native).safeTransfer(strategist, strategistFee);

        emit ChargedFees(callFeeAmount, steakHutFeeAmount, strategistFee);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).safeApprove(chef, uint256(MAX_INT));
        IERC20(output).safeApprove(unirouter, uint256(MAX_INT));
        IERC20(secondOutput).safeApprove(unirouter, uint256(MAX_INT));
        IERC20(native).safeApprove(unirouter, uint256(MAX_INT));

        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, uint256(MAX_INT));

        IERC20(lpToken1).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, uint256(MAX_INT));
    }
