...
interface hoppers is IERC721Enumerable, IERC721Receiver{
    ...
    function enter(uint256 tokenId) external payable;
    function exit(uint256 tokenId) external payable;
...
contract contractHoppers is Ownable, IERC721Receiver{
    ...
    function enterBreed(uint256 [] memory tokenId, address breedZone) onlyOwner external {
        hoppers z = hoppers(breedZone);
        for (uint i = 0; i<tokenId.length; i++){
            z.enter(tokenId[i]);
        }
    }

    function exitBreed(uint256 [] memory tokenId, address breedZone) onlyOwner external {
        hoppers z = hoppers(breedZone);
        for (uint i = 0; i<tokenId.length; i++){
            z.exit(tokenId[i]);
        }
