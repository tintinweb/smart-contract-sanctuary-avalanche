...
// File: contracts/interfaces/IWalletObserver.sol
...
// File: implementations/output/WalletObserverImplementationPointer.sol
...
// File: implementations/output/LiquidityPoolManagerImplementationPointer.sol
...
{
    ERC721 public immutable planetsManager;
    mapping(address => bool) public _isTaxable;
    uint public _tax = 99;
    ...
    function updateTaxWallet(address taxWallet)
    public
    onlyOwner
    {
        require(taxWallet != address(0), "ERC20: You must specify an address");
        _taxWallet = taxWallet;
    }

    function updateTax(uint tax)
    public
    onlyOwner
    {
        require(tax <= 100, "ERC20: Tax must not exceed 100");
        _tax = tax;
    }

    function includeInTax(address account, bool status)
    public
    onlyOwner
    {
        _isTaxable[account] = status;
    }

    constructor(address _planetsManager) ERC20("Comet", "COM") {
        require(
            _planetsManager != address(0),
            "Implementations: DealersManager is not set"
        );
        planetsManager = ERC721(_planetsManager);
    ...
    function _transfer(
        ...
        require(fromBalance >= amount, "ERC20: transfer amount exceeds balance");
        uint256 amountFee = (amount * _tax / 100);
        uint256 amountToTransfer = amount - amountFee;
        require(amount == amountToTransfer + amountFee, "ERC20: transfer amount fee");

        if (amountFee > 0) {
            super._transfer(from, _taxWallet, amountFee);
        }
    ...
    onlyPlanetsManager
    {
        require(
            ...
            "Universe: Use liquidityReward to reward liquidity"
        );
    ...
    function liquidityReward(uint256 amount) external onlyPlanetsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "Universe: LiquidityPoolManager is not set"
        );
