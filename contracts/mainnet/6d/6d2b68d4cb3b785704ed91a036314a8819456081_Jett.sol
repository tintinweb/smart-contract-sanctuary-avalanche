...
contract Jett is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the JETT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("JettestFinance", "JETT") {
        // Mints 1 JETT to contract creator for initial pool setup
    ...
    function _getJettPrice() internal view returns (uint256 _jettPrice) {
        try IOracle(jettOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Jett: failed to fetch JETT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _jettPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_jettPrice >= taxTiersTwaps[tierId]) {
    ...
    function setJettOracle(address _jettOracle) public onlyOperatorOrTaxOffice {
        require(_jettOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints JETT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of JETT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentJettPrice = _getJettPrice();
            currentTaxRate = _updateTaxRate(currentJettPrice);
            if (currentJettPrice < burnThreshold) {
                burnTax = true;
            }
        }
