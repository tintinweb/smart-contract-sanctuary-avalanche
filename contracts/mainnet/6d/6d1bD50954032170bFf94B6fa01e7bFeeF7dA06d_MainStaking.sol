...
contract MainStaking is Initializable, OwnableUpgradeable {
    ...
    function vote(
        ...
        for (uint256 i; i < length; i++) {
            ...
            address rewarder = _rewarders[i];
            rewardTokens[i] = address(IBribe(voter.bribes(_lpVote[i])).rewardToken());
            uint256 feeAmount;
            if (rewardAmount > 0) {
                ...
                _approveTokenIfNeeded(rewardTokens[i], rewarder, rewardAmount);
                IBaseRewardPool(rewarder).queueNewRewards(rewardAmount, rewardTokens[i]);
            }
