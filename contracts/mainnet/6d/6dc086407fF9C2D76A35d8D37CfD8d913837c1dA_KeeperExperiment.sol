...
contract KeeperExperiment is AccessControlledAndUpgradeable {
  ...
  mapping(uint32 => uint256) public batchPaymentTokenValueThreshold;
  ...
  function updateBatchPaymentTokenValueThreshold(
    uint32 marketIndex,
    uint256 _batchPaymentTokenValueThreshold
  ) external onlyRole(ADMIN_ROLE) {
    // No validation - we are admin ;)
    batchPaymentTokenValueThreshold[marketIndex] = _batchPaymentTokenValueThreshold;
  }
  ...
  function arePendingActionsOverThreshold(uint32 marketIndex) public view returns (bool) {
    return
      getPendingNextPriceActionsPaymentTokenValue(marketIndex) >=
      batchPaymentTokenValueThreshold[marketIndex];
  }
  ...
  function shouldUpdateOracle(uint32 marketIndex) public view returns (bool) {
    ...
    if (arePendingActionsOverThreshold(marketIndex)) {
      return true;
    }
  }
    ...
    )
  {
    ...
    return (
      ...
      arePendingActionsOverThreshold(marketIndex)
    );
  }
    ...
    )
  {
    (
      ...
      bool priceChangeIsOverThreshold,
      int256 priceChangePercent_1e18,
      bool arePendingActionsOverThreshold
    ) = shouldUpdateOracleNotViewBreakdown(marketIndex);
    return (
      wasLastUpdateOlderThanThreshold,
      priceChangeIsOverThreshold,
      priceChangePercent_1e18,
      arePendingActionsOverThreshold,
  ...
  // TODO: can delete this function, no need to pass in valueThreshold, it is stored in this contract
  // Used by Gelato resolver
