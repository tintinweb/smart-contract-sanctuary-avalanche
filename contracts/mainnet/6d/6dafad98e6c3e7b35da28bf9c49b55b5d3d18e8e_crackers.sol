...
Cracker Bob - Arrogant   
*/


contract crackers is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter public _tokenIds;
    IERC721[] public nft; 
    uint256 public _price = 1 ether; //avax
    uint256 public _price_Public = 1 ether; //avax
    uint256 public _maxMintable = 100;
    string private _customBaseURI;
    uint256 public _royaltyAmount = 800; //8%
    uint256 public _maxMintPerTx = 3;
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x85E6cC88; //10
    bool public _mintActive = false;
    address private _royaltyAddress;
    address private walletFundsA;
    address private walletFundsB;
    address private walletFundsC;
    address private walletFundsD;

           // WL //
    uint256 public _price_WL = .90 ether; 
    ...
    constructor(
        ...
        address _walletFundsC,
        address _walletFundsD,
        address royaltyAddr
    ) ERC721("Cracker Bob", "CRKRS") {
        walletFundsA = _walletFundsA;
        walletFundsB = _walletFundsB;
        walletFundsC = _walletFundsC;
        walletFundsD = _walletFundsD;
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (isWL) {
            require(whiteListed[msg.sender] == true, "Your wallet is not white listed.");
        }
    ...
    function splitFunds(uint256 mintValue) private {
        uint256 amountA = (mintValue * 50) / 100;
        uint256 amountB = (mintValue * 20) / 100;
        uint256 amountC = (mintValue * 20) / 100;
        uint256 amountD = (mintValue * 10) / 100;
        payable(walletFundsA).transfer(amountA);
        payable(walletFundsB).transfer(amountB);
        payable(walletFundsC).transfer(amountC);
        payable(walletFundsD).transfer(amountD);
    }

    function setNFTsTransferWallets(address _walletFundsA, address _walletFundsB ,address _walletFundsC,address _walletFundsD)
        external
        onlyOwner
    {
        walletFundsA = _walletFundsA;
