...
ChainlinkVRFConsumerUpgradeable, Blacklistable {
    ...
    modifier gated() {
        require(block.timestamp >= startTime || hasRole(ADMIN_ROLE, _msgSender()), "Mint has not started");
        _;
    }
    ...
    function buy(uint256 count, Method method, uint256[] memory tickets) public whenNotPaused gated {
        ...
        for (uint256 i = 0; i < discounts.length; i++) {
            ...
            totalCost += discounts[i] * price / 100;
        }
    ...
    function setAttributeWeights(uint256 gender, uint256[] memory attributes, uint256[][] memory weights) public onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < attributes.length; i++) {
            uint256[] storage attrWeights = _weights[gender][attributes[i]];
            for (uint256 j = 0; j < weights[i].length; j++) {
                attrWeights[j] = weights[i][j];
            }
        }
    }
