...
contract WAVAXWEED is Initializable, OwnableUpgradeable, ReentrancyGuard {
    ...
    IERC20 private WAVAXinterface;
    address public WAVAX;
    bool public init;
    bool public alreadyInvested;
    ...
    function initialize() public initializer {
        __Ownable_init();
        WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7; /* Avalanche: Wrapped AVAX */
        WAVAXinterface = IERC20(WAVAX);
        min = 0.1 ether; // 0.1 WAVAX
        max = 100000 ether; // 100k WAVAX
        roi = 25;
        fee = 3;
        withdraw_fee = 3;
        ref_fee = 15;
        dev = payable(0x0a0b0C2F5040c6C06F06950E7185889329C3DDE3);
    }
    ...
    mapping(uint256 => address) public userRank;

    function start() public onlyOwner {
        init = true;
    }

    function deposit(address _ref, uint256 _amount) public nonReentrant {
        require(
            _amount >= min && _amount <= max,
            "Amount must be between 0.1 and 100000 WAVAX"
        );
        ...
        if (!checkAlready()) {
            ...
            WAVAXinterface.transferFrom(msg.sender, dev, total_fee);
            WAVAXinterface.transferFrom(
                msg.sender,
                address(this),
                total_contract
            );
            userRank[usersCount] = msg.sender;
            usersCount++;
        } else {
            uint256 ref_fee_add = refFee(_amount);
            if (_ref != address(0) && _ref != msg.sender) {
    ...
    function withdrawal() public nonReentrant {
        ...
        WAVAXinterface.transfer(msg.sender, totalAmountToWithdraw);
        WAVAXinterface.transfer(dev, wFee);
    ...
    function claimDailyRewards() public nonReentrant {
        require(
    ...
    function unStake() external nonReentrant {
        ...
        WAVAXinterface.transfer(msg.sender, UnstakeValueCore);
    ...
    function Ref_Withdraw() external nonReentrant {
        uint256 value = refferal[msg.sender].reward;

        WAVAXinterface.transfer(msg.sender, value);
    ...
    function getBalance() public view returns (uint256) {
        return WAVAXinterface.balanceOf(address(this));
    }

    function getUserBalance(address _user) public view returns (uint256) {
        return WAVAXinterface.balanceOf(_user);
    }
}