...
interface INativeToken {
    ...
    function NATIVEPerSecond() external view returns (uint256);
...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accNATIVEPerShare` (and `lastRewardBlockTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlockTime; // Last block number that NATIVE distribution occurs.
        uint256 accNATIVEPerShare; // Accumulated NATIVE per share, times 1e12. See below.
        address strat; // Strategy address that will auto compound want tokens
    }

    // Token address
    address public constant NATIVE = 0x14bf0e443380c197dd7019c455F7d5d92c230e41; // NativeToken address
    ...
    function add(
        ...
        uint256 startTime = INativeToken(NATIVE).startTime();
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardBlockTime && sharesTotal != 0) {
            uint256 multiplier = INativeToken(NATIVE).getMultiplier(pool.lastRewardBlockTime, block.timestamp);
            uint256 NATIVEReward =
                multiplier.mul(INativeToken(NATIVE).NATIVEPerSecond())
                          .mul(pool.allocPoint)
                          .div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 NATIVEReward =
            multiplier.mul(INativeToken(NATIVE).NATIVEPerSecond()).mul(pool.allocPoint).div(
                totalAllocPoint
            );

        INativeToken(NATIVE).mint(NATIVEReward);
