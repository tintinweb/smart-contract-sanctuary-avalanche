...
interface IPizzeriaMafia {
    function getTotalPPM(address player) external view returns (uint256);
}

interface IPizzeriaProgressionMafia {
    function getMafiaModifier(address owner) external view returns (uint256);
}

contract MafiaBossV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    struct Mafioso {
        ...
        uint256 currentHp;
        uint256 penalty;
        uint256 noBribePenalty;
        uint256 bribeMultiplier;
        uint256 bribeAmount;
        uint256 bribeBonusFactor;
        uint256 bribeBonusReduction;
        uint256 bribeBonusMaxReduction;
    ...
    Pizza pizza;
    IPizzeriaMafia public pizzeria;
    IPizzeriaProgressionMafia public pizzeriaProgression;
    uint256 graceHours;
    ...
    function initialize(Pizza _pizza, address _pizzeria, address _pizzeriaProgression) public initializer {
        pizza = _pizza;
        pizzeria = IPizzeriaMafia(_pizzeria);
        pizzeriaProgression = IPizzeriaProgressionMafia(_pizzeriaProgression);
        graceHours = 12;
    ...
    function setGradeHours(uint256 _graceHours) external onlyOwner {
        graceHours = _graceHours;
    }
    function setPizzeria(address _pizzeria) external onlyOwner {
        pizzeria = IPizzeriaMafia(_pizzeria);
    }
    ...
    function createMafioso(uint256 _totalHp, uint256 _penalty, uint256 _noBribePenalty, uint256 _bribeAmount) external onlyOwner {
        require(_penalty < 80, 'Invalid penalty');
        require(_noBribePenalty < 80, 'Invalid penalty');
        ...
        mafioso.noBribePenalty = _noBribePenalty;
        mafioso.bribeBonusFactor = 3;
        mafioso.bribeBonusReduction = 5;
        mafioso.bribeBonusMaxReduction = 30;
        mafioso.startTimestamp = block.timestamp;
    ...
    function updateNoBribePenalty(uint256 _newPenalty) external onlyOwner {
        require(_newPenalty < 80, 'Invalid penalty');
        Mafioso storage mafioso = mafiosos[currentMafiosoId];
        mafioso.noBribePenalty = _newPenalty;
    }

    function updateBribeBonusFactor(uint256 _bribeBonusFactor) external onlyOwner {
        require(_bribeBonusFactor > 0, 'Invalid bribe bonus factor');
        Mafioso storage mafioso = mafiosos[currentMafiosoId];
        mafioso.bribeBonusFactor = _bribeBonusFactor;
    }

    function updateBribeBonusReduction(uint256 _bribeBonusReduction) external onlyOwner {
        Mafioso storage mafioso = mafiosos[currentMafiosoId];
        require(_bribeBonusReduction > 0 && _bribeBonusReduction < mafioso.bribeBonusMaxReduction, 'Invalid bribeBonusReduction');
        mafioso.bribeBonusReduction = _bribeBonusReduction;
    }

    function updateBribeBonusMaxReduction(uint256 _bribeBonusMaxReduction) external onlyOwner {
        Mafioso storage mafioso = mafiosos[currentMafiosoId];
        require(_bribeBonusMaxReduction > 0 && _bribeBonusMaxReduction < mafioso.penalty, 'Invalid bribeBonusMaxReduction');
        mafioso.bribeBonusMaxReduction = _bribeBonusMaxReduction;
    }
    ...
    function bribeMafioso(uint256 _nrBribes) public {
        ...
        if(playerIndex[currentMafiosoId][owner] == 0){
            ...
            player.owner = owner;
            player.bribes += _nrBribes;
            player.ppm = pizzeria.getTotalPPM(owner);
        } else {
        ...
        if (mafioso.currentHp == 0) {
            mafioso.endTimestamp = block.timestamp;
            emit MafiosoGone(currentMafiosoId);
        }
    }

    function bribeGracePeriod(uint256 _nrBribes) public {
        address owner = msg.sender;
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        uint256 _amountPizza = _nrBribes * mafioso.bribeAmount;
        require(mafioso.currentHp == 0, 'There is a Mafia boss active');
        require(pizza.balanceOf(owner) >= _amountPizza, "not enough PIZZA");
        uint256 gracePeriod = mafioso.endTimestamp + graceHours*60*60;
        require(block.timestamp <= gracePeriod, 'Outside of the grace period');
    ...
    // Views
    function getPlayerPenalty(address _player) public view returns (uint256) {
        Player memory player = participantPlayers[currentMafiosoId][playerIndex[currentMafiosoId][_player]];
        Mafioso memory mafioso = mafiosos[currentMafiosoId];

        uint256 mafiaSkillModifier = pizzeriaProgression.getMafiaModifier(_player);

        uint256 tax;
        uint256 penalty;

        if(mafioso.currentHp > 0){
            penalty = mafioso.penalty;

            if(player.ppm > 0 && player.bribes > player.ppm){
                uint256 bribeBonusFactor = 1 + player.ppm/mafioso.bribeBonusFactor;
                uint256 penaltyReduction = mafioso.bribeBonusReduction * (player.bribes - player.ppm)/bribeBonusFactor;
                if(penaltyReduction > mafioso.bribeBonusMaxReduction){
                    penaltyReduction = mafioso.bribeBonusMaxReduction;
                }
                if(penalty <= penaltyReduction){
                    penalty = 0;
                } else {
                    penalty = penalty - penaltyReduction;
                }
            }
        }
        else if(player.ppm == 0 || player.bribes < player.ppm){
                penalty = mafioso.noBribePenalty;
        }

        if(penalty > mafiaSkillModifier){
            tax = penalty - mafiaSkillModifier;
        }
    ...
    function inGracePeriod() public view returns (bool) {
        ...
        return block.timestamp < gracePeriod;
    }
    ...
    function mafiaCurrentNoBribePenalty() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.noBribePenalty;
    }

    function mafiaBribeBonusFactor() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.bribeBonusFactor;
    }

    function mafiaBribeBonusReduction() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.bribeBonusReduction;
    }

    function mafiaBribeBonusMaxReduction() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.bribeBonusMaxReduction;
    }

    function mafiaStartTime() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.startTimestamp;
    }

    function mafiaEndTime() public view returns (uint256) {
        Mafioso memory mafioso = mafiosos[currentMafiosoId];
        return mafioso.endTimestamp;
    }
