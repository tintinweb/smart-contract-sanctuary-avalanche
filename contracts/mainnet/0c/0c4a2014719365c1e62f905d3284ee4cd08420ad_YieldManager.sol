...
contract YieldManager is IYieldManager, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable, Rescuable, Modifier {
    ...
    modifier onlyIfEmptyModule() {
        require(_getModulesBalance() <= minHarvestThreshold, "YieldManager: module not empty");
        _;
    }
     ...
     * @dev     Total allocation should be 100%, should be set after a addModule operation
    ...
    function finishPanic() onlyOwner external whenPaused {
        aum = _deposit(IERC20Upgradeable(baseToken).balanceOf(address(this)));
        _unpause();
    }
    ...
    /**
     * @notice  Set a new Base Token
     * @param   newBaseToken  Address of the new Base Token
     */
     ...
     * @param   _amount  Amount of tokens requested for withdrawal execution
     ...
     */
    function _deposit(uint _amount) private returns (uint) {
        uint amountForThatModule = 0;
        uint depositedAmount = 0;
        uint balanceBefore = _getModulesBalance();
        for (uint256 i = 0; i < numberOfModules; i += 1) {
        ...
        uint balanceAfter = _getModulesBalance();
        return balanceAfter - balanceBefore;
    }

    /**
     * @notice  Pulls the funds from the all the modules according to the allocation
     * @dev     Receivers needs to be specified in case of Async withdrawal
     * @param   _requestedAmount  Total amount of Base token to be pulled out from the Modules
     ...
     */
    function _withdraw(uint _requestedAmount, address _receiver) private returns (uint, uint) {
        ...
        for (uint i = 0; i < numberOfModules; i += 1) {
            ...
            (uint instantWithdrawn, uint pendingAsyncWithdraw) = yieldOptions[i].module.withdraw{value : fee}(withdrawAmountForThatModule, _receiver);
