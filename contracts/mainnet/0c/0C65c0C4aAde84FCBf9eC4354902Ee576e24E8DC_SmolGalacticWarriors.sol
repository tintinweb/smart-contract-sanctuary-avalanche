...
contract SmolGalacticWarriors is ERC721URIStorage, Ownable, KindaRandom {
  ...
  constructor(address userBase_) ERC721("Smol Galactic Warriors", "SGW") KindaRandom(222) {
    _customBaseURI = "https://campfire.mypinata.cloud/ipfs/Qmf8pJR7rcnuoXfMcrw515As8fBrp62eT8WS5amGNCyhpx/";
    _maxMintable = 222; // 222 max supply
    _royaltyAmount = 850; // 8.5% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 1;
    _startTime = 1661196600;
  ...
  function onlyWhitelist() public view returns (bool) {
    return block.timestamp < (_startTime + 1800); // 30 minutes for WL
  }
  ...
  function mint(uint256 quantity) public payable {
    ...
    if (onlyWhitelist()) {
      require(isWhitelisted(msg.sender), "Public mint not active.");
    }

    payable(_artist).transfer(msg.value); // should not be a value, but just in case
  ...
  function _privateMint(address recipient) private {
    ...
    _setTokenURI(newItemId, string(abi.encodePacked(newItemId.toString(), ".json")));
  }
