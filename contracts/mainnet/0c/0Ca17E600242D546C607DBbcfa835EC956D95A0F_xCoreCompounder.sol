...
contract xCoreCompounder is Initializable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    function _compoundIfNeeded() internal {
        uint256 pendingReward = IBaseRewardPool(rewarder).earned(address(this), VTX);
        if (maxRewardPending > 0 && pendingReward > maxRewardPending) {
            compound();
        }
    }
    ...
    function _harvestRewarder() internal {
        (, , , , rewarder, , ) = masterchef.addressToPoolInfo(address(XTOKEN));
    ...
    function _compound(bool safeSwap) internal {
        ...
        emit Harvested(WAVAX, wavaxBalance - _lvtxFee - _callerFee - _protocolFee);
        ...
        if (_vtxLvtxFee > 0) {
            _approveTokenIfNeeded(VTX, lvtxRewarder, _lvtxFee);
            IBaseRewardPool(lvtxRewarder).donateRewards(_lvtxFee, VTX);
        }
