...
contract PactOfTheScribble is ERC721URIStorage, Ownable, KindaRandom {
  ...
  mapping (address => uint256) private _mintsRemaining;

  constructor() ERC721("Pact of the Scribble", "PACTS") KindaRandom(500) {
    _customBaseURI = "https://nfts.bpi.network/scribbles/";
    _maxMintable = 500; // 500 max supply
    _royaltyAmount = 1000; // 10% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 1;

    _startTime = 1664416800;
    
    _artist = 0x728B158Cefede30973123c8034BbdBF29D2e23Df;

    _scribble = 0xcf50b23A09Fb51E0262A609a0FeE723639F705a2;

    _privateMint(_scribble);
  }
  ...
  function setScribble(address scribble_) public onlyOwner {
    _scribble = scribble_;
  }

  function addMints(address[] memory accounts) public onlyOwner {
    for (uint256 index = 0; index < accounts.length; index++) {
      address account = accounts[index]; 
      _mintsRemaining[account] += 1;
    }
  }

  function mintsRemaining(address account) public view returns (uint256) {
    return _mintsRemaining[account];
  }
  ...
  function mint(uint256 quantity) public payable {
    ...
    payable(_artist).transfer(msg.value); // this should not do anything

    for(uint i = 0; i < quantity; i++) {
      require(_mintsRemaining[msg.sender] > 0, "No more mints remaining.");
      _mintsRemaining[msg.sender] -= 1;
      _privateMint(msg.sender);
    }
  }

  function artistMint(uint256 quantity) public {
    require(msg.sender == _scribble); // onlyScribble
