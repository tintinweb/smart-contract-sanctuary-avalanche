...
// File contracts/distribution/GSharesRewardPool.sol
// License-Identifier: MIT
...
contract GenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accGSharesPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.011574074 ether; // 59500 bshare / (370 days * 24h * 60min * 60s)
    uint256 public runningTime = 2 days; // 370 days
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "GSharesRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "GSharesRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accGSharesPerShare = pool.accGSharesPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accGSharesPerShare = accGSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accGSharesPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accGSharesPerShare = pool.accGSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accGSharesPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeGSharesTransfer(_sender, _pending);
                emit RewardPaid(_sender, _pending);
            }
        }
        if (_amount > 0) {
            pool.token.safeTransferFrom(_sender, address(this), _amount);
            uint256 depositDebt = _amount.mul(100).div(10000); // 1%
            user.amount = user.amount.add(_amount.sub(depositDebt));
            pool.token.safeTransfer(daoFundAddress, depositDebt);
            
            //user.amount = user.amount.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accGSharesPerShare).div(1e18);
    ...
    function safeGSharesTransfer(address _to, uint256 _amount) internal {
