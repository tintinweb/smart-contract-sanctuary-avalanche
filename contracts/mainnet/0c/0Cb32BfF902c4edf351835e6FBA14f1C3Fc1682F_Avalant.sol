...
contract Avalant is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    // VERSION 1.4
    uint public currentMaxStage;
    uint public antGPerStageSesame;
    ...
    function dig(uint _tokenId) public {
        ...
        if (allAvalants[_tokenId].colonyStage > currentMaxStage) {
            currentMaxStage = allAvalants[_tokenId].colonyStage;
        }
    ...
    function digSesame(uint _tokenId, uint _stage) public {
        require(ownerOf(_tokenId) == msg.sender, "Not ur ant");
        require(_stage < currentMaxStage - 2, "Cannot go this far");
        AntBosses b = AntBosses(BOSSES_CONTRACT);
        AntGold ag = AntGold(ANTG_CONTRACT);
        Suga s = Suga(SUGA_CONTRACT);
        require(b.isBossAliveAtStage(allAvalants[_tokenId].colonyStage) == false, "Blocked by boss");
        ...
        uint currentStage = allAvalants[_tokenId].colonyStage;
        uint stagesToDig = _stage - currentStage;
        uint sugarToDig;

        for (uint i = _stage - 1; i >= currentStage; i--) {
            sugarToDig += (i**2 * 20) * 1e17;
        }
        AvalantItems items = AvalantItems(ITEMS_CONTRACT);
        if (items.doesAntHaveArmor(_tokenId)) {
            sugarToDig = 10;
        } else {
            sugarToDig *= 13;
        }
        ...
        // Sesame is also 2k ANTG by stages by default
        uint antgToDig = stagesToDig * antGPerStageSesame * 1e18;

        ag.burn(msg.sender, antgToDig);

        allAvalants[_tokenId].colonyStage = _stage;

        // Digging into stage 2 will require 2 hours of resting
        // Helmet reduce cooldown by 80%
        uint multiplyTotalCooldown = 10;
        if (items.doesAntHaveHelmet(_tokenId)) {
            multiplyTotalCooldown = 2;
        }
        allAvalants[_tokenId].restUntil = block.timestamp + 60*60*(_stage) * multiplyTotalCooldown / 10;
        emit AntDug(_tokenId, allAvalants[_tokenId].colonyStage);
    }

    function digSesameMultipleAnts(uint[] calldata _tokenIds, uint _stage) public {
        for (uint i = 0; i < _tokenIds.length; i++) {
            digSesame(_tokenIds[i], _stage);
        }
    }

    function resetFatigue(uint _antId) external {
        require(msg.sender == BOSSES_CONTRACT, "Only callable by bosses contract");
        allAvalants[_antId].restUntil = block.timestamp;
    }
    ...
    function temporarySetCurrentMaxStage(uint _currentMaxStage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        currentMaxStage = _currentMaxStage;
    }

    function setAntGPerStageSesame(uint _antGPerStageSesame) external onlyRole(DEFAULT_ADMIN_ROLE) {
        antGPerStageSesame = _antGPerStageSesame;
    }
    // </AdminStuff>
        ...
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable)
    {
        ...
        if (address(ITEMS_CONTRACT) != address(0) && address(from) != address(0)) {
            AvalantItems ai = AvalantItems(ITEMS_CONTRACT);
            ai.unequippedAnt(tokenId);
        }
...
contract AntGold is Initializable, ERC20Upgradeable, AccessControlUpgradeable {
    ...
    // VERSION 1.2
    uint public HOURS_COLONY_STAGE_RATIO;
    uint public START_FATIGUE_DATE;
    ...
    function timeAllowedOnAStage(uint _colonyStage) public view returns (uint) {
        if (_colonyStage == 1) return 0; // Since there is no restUntil on stage 1..

        uint ratio = HOURS_COLONY_STAGE_RATIO > 0 ? HOURS_COLONY_STAGE_RATIO : 3000000000; // No fatigue by default..
        // divide by 10 because ratio is 20 for 2 times hours.
        uint hoursAllowed = _colonyStage * ratio / 10;

        return hoursAllowed * 60 * 60;
    }

    function getDateMaxForAnt(uint _colonyStage, uint _restUntil) public view returns (uint) {
        uint initDate = START_FATIGUE_DATE > 0 ? START_FATIGUE_DATE : 2650276004; // No fatigue by default.
        uint dateToUse = initDate > _restUntil ? initDate : _restUntil;
        uint dateMaxForGenerationOfAntGold = dateToUse + timeAllowedOnAStage(_colonyStage);

        return dateMaxForGenerationOfAntGold;
    }

    function claimableView(uint256 tokenId) public view returns (uint) {
        Avalant a = Avalant(AVALANT_CONTRACT);
        (,,uint colonyStage,,uint restUntil) = a.allAvalants(tokenId);
        ...
        if (antStaked[tokenId] == false) {
            ...
            // Fatigue mecanism
            // max date becomes the date of end of cooldown + timeAllowedOnAStage
            uint maxDate = getDateMaxForAnt(colonyStage, restUntil);
            uint dateToUse = block.timestamp > maxDate ? maxDate : block.timestamp;
            if (dateToUse <= antLastClaim[tokenId]) return 0;
            uint deltaSeconds = dateToUse - antLastClaim[tokenId];
    ...
    function transfer(address recipient, uint256 amount) public virtual override(ERC20Upgradeable) returns (bool) {
        ...
        if (recipient == TRADER_JOE_ROUTER && msg.sender != LP_ADDRESS) {
            taxAmount = amount / 100 * SELL_TAX_PERCENT;
        }
    ...
    function setHoursFatigueRatio(uint _ratio) public onlyRole(DEFAULT_ADMIN_ROLE) {
        HOURS_COLONY_STAGE_RATIO = _ratio;
    }

    function setStartFatigueDate(uint _date) public onlyRole(DEFAULT_ADMIN_ROLE) {
        // 1650276004 is 18th april 12pm
        START_FATIGUE_DATE = _date;
    }
    ...
    function setBossesContract(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) {
        BOSSES_CONTRACT = _address;
    }
    // </AdminStuff>
}
...
contract AntBosses is Initializable, AccessControlUpgradeable {
    ...
    function hitBoss(uint _damage, uint _antId, uint _bossId) public {
        Boss storage boss = bosses[_bossId];
        Avalant a = Avalant(AVALANT_CONTRACT);
    ...
    function hitBossBatch(uint _damage, uint[] calldata _antIds, uint _bossId) external {
        for (uint i = 0; i < _antIds.length; i++) {
            hitBoss(_damage, _antIds[i], _bossId);
        }
    }
...
contract AvalantItems is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    // To fix issue afterwards sadly
    function unequipBatch(uint[] calldata _antIds) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i = 0; i < _antIds.length; i++) {
            unequippedAnt(_antIds[i]);
        }
    }
    // </AdminStuff>

    // Called by Avalant Contract on transfer
    function unequippedAnt(uint _antId) public {
        require(hasRole(ANT_CONTRACTS_ROLE, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not authorized");
