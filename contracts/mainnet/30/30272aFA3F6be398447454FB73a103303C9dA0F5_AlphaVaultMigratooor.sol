...
contract AlphaVaultMigratooor is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function migrate() external {
        ...
        IERC20Upgradeable(oldVault).transferFrom(msg.sender, address(this), IERC20Upgradeable(oldVault).balanceOf(msg.sender));

        // Withdraw from v1
        IAlphaVault(oldVault).withdraw(IERC20Upgradeable(oldVault).balanceOf(address(this)));

        // Deposit in v2
        IERC20Upgradeable(token).approve(newVault, IERC20Upgradeable(token).balanceOf(address(this)));
        IAlphaVault(newVault).deposit(IERC20Upgradeable(token).balanceOf(address(this)));

        // Transfer IOU balance of v2
        IERC20Upgradeable(newVault).transfer(msg.sender, IERC20Upgradeable(newVault).balanceOf(address(this)));
    }
}
