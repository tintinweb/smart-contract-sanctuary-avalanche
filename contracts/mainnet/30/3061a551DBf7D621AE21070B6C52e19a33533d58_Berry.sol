...
contract Berry is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the BERRY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BerryFund", "BERRY") {
        // Mints 1 BERRY to contract creator for initial pool setup
    ...
    function _getBerryPrice() internal view returns (uint256 _berryPrice) {
        try IOracle(berryOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Berry: failed to fetch BERRY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _berryPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_berryPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBerryOracle(address _berryOracle) public onlyOperatorOrTaxOffice {
        require(_berryOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BERRY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BERRY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBerryPrice = _getBerryPrice();
            currentTaxRate = _updateTaxRate(currentBerryPrice);
            if (currentBerryPrice < burnThreshold) {
                burnTax = true;
            }
        }
