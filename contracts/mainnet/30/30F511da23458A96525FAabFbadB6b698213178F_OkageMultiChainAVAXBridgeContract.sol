/**
 *Submitted for verification at snowtrace.io on 2023-02-07
*/
...
contract OkageMultiChainAVAXBridgeContract is Ownable {
    ...
    uint256 private _bridgeFee = 0;
    uint256 constant MAX_BRIDGE_FEES = 10;
    bool public _isBridgingPaused = false;

    address payable public system = payable(0x7634c6c019541a590C72f2d5f705FF3622B78773);
    address public governor = address(0x7634c6c019541a590C72f2d5f705FF3622B78773);
    address public bridgeFeesAddress = address(0x7634c6c019541a590C72f2d5f705FF3622B78773);
    ...
    constructor(address _okage) {
        //   initializing processed fees
        // chainID : AVAX mainnet => 43114
        _processFees[1] = 0.001 ether;
    ...
    function updateOkageContract(address _okage) external onlyOwner {
        okage = IERC20(_okage);
    }
    ...
    /// @notice Transfers `amount` Okage initializes a bridging transaction to the target chain.
    /// @param _targetChain The target chain to which the wrapped asset will be minted
    /// @param _amount The amount of Okage to bridge
    function bridge(
        ...
        okage.safeTransferFrom(_msgSender(), address(this), _amount);
    ...
    function bridgeBack(
        ...
        if(serviceFee > 0) {
            okage.safeTransfer(bridgeFeesAddress, serviceFee);
        }
        okage.safeTransfer(_receiver, transferAmount);
