...
contract xWalrusPrizePool is Ownable, VRFConsumerBaseV2 {
    ...
    function getTickets(address token, uint256 numTickets) external {
        ...
        if (token == walrus) {
            unchecked { totalWalrus += received; }
        } else if (token == wbond) {
            unchecked { totalWBond += received; }
        } else {
    ...
    function balanceOf() public view returns (uint256) {
        return IERC20(xWalrus).balanceOf(address(this));
    }
