...
contract GordionStrongbox_v3_AVAX {
    /**
    Events
     */
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public lastDividendAt;
    ...
    function splitDividend() external onlyOwners {
        ...
        reflectDividend(claimableTotal);
        claimableTotal = 0;
    }
    ...
    function claimRewards() external {
        ...
        for (uint i = 0; i < count; i++) {
            ...
            lastDividendAt[counter][tokenId] = totalDividend;
        }
        rewardPool -= balance;
    ...
    {
        _reflectionBalance = totalDividend - lastDividendAt[counter][tokenId];
    }

    function reflectDividend(uint256 amount) private {
        reflectionBalance = reflectionBalance + amount;
        rewardPool += amount;
        totalDividend = totalDividend + (amount / (nft.totalSupply() - 2919));
    }
    ...
    function resetRewards() external onlyOwners {
        claimableTotal = 0;
        reflectionBalance = 0;
        rewardPool = 0;
        totalDividend = 0;
