...
contract XYFormulaCurve is ICurve, CurveErrorCodes {
    ...
    function getBuyInfo(
        ...
        uint256 totalTokenSupply,
        uint32 reserveRatio
    ) external pure returns (CurveErrorCodes.Error error, uint256 inputValue) {
    ...
    function getSellInfo(
        ...
        uint256 totalTokenSupply,
        uint32 reserveRatio
    ) external pure returns (CurveErrorCodes.Error error, uint256 outputValue) {
        if (reserveRatio > MAX_WEIGHT) {
            return (CurveErrorCodes.Error.INVALID_RESERVE_RATIO, 0);
        }
    ...
    UUPSUpgradeable
{
    ...
    /**
    @dev Utility function get tokens from an IERC721Enumerable.
     */
...
import {Ownable} from "lib/openzeppelin-contracts/contracts/access/Ownable.sol";
import {RegisteredPairParams} from "./IPotionPairRegistry.sol";

/// @author 10xdegen.
// A Simple implementation of IPotionPairRegistry
// Allows on-chain indexing, whitelist, etc for PotionPairFactory.
contract PotionPairRegistry is Ownable {
    address public factory;

    mapping(address => RegisteredPairParams) public registeredPairs;
...
// A Simple implementation of IPotionPairRegistry
// Allows on-chain indexing, whitelist, etc for PotionPairFactory.
contract CauldronNFT is Ownable {
    address public factory;
    ...
    function registerPair(RegisteredPairParams calldata params)
        ...
        returns (bool)
    {
        if (registeredPairs[params.nft].pair != address(0)) {
            return false;
        }
        registeredPairs[params.nft] = params;
        return true;
    }

    // resets a pair to the zero address to be reused.
    // admin only.
    function unregister(address nft) external onlyOwner {
