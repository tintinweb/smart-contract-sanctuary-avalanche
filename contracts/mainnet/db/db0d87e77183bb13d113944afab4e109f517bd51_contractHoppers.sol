...
interface hoppers is IERC721Enumerable, IERC721Receiver{
    ...
    function getHopperAndGauge(uint256 tokenId) external view returns (uint64 [] memory hopper, uint256 hopperGauge, uint256 gaugeLimit);
...
contract contractHoppers is Ownable, IERC721Receiver{
    ...
    function rebirth(uint256 [] memory tokenIds) onlyOwner external {
        for (uint i=0; i<tokenIds.length; i++) {
            h.rebirth(tokenIds[i]);
        }
    }
    ...
    function checkAndLevelUp(uint256 [] memory tokenIds, uint64 nbLvl, bool useOwnRewards) onlyOwner external {
        for(uint i=0; i<tokenIds.length; i++) {
            hoppers z = hoppers(h.ownerOf(tokenIds[i]));
            z.exit(tokenIds[i]);
            z.getHopperAndGauge(tokenIds[i]);
        }
    }

    function getGauge(uint256 tokenId, address zone) public view returns (uint256 limit) {
        hoppers z = hoppers(zone);
        (,,uint256 gaugeLimit) = z.getHopperAndGauge(tokenId);
        return gaugeLimit;
    }
