...
contract AVAXHOUR {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 0.10 ether; 
    uint256 constant public HOUR_LIMIT = 50 ether; // only 50 AVAX per Hour
	...
	function reinvest(uint8 plan) public {
		User storage user = users[msg.sender];
		require(user.deposits.length < MAX_INVESTS, " max 500 depsoits");
