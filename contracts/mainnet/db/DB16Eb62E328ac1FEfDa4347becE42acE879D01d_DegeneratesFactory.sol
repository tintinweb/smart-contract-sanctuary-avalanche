...
contract DegeneratesFactory is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    BunnyMintingStation public bunnyMintingStation;

    IERC20 public tripToken;

    // starting block
    uint256 public startBlockNumber;

    // Number of TRIPs a user needs to pay to acquire a token
    ...
    uint8 private constant previousNumberBunnyIds = 0;
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        require(!hasClaimed[senderAddress], "Has already claimed");
        ...
        // Send TRIP tokens to this contract
        tripToken.safeTransferFrom(senderAddress, address(this), tokenPrice);
    ...
    /**
     * @dev It transfers the TRIP tokens back to the chef address.
    ...
    function setBunnyJson(
        string calldata _bunnyId0Json,
        string calldata _bunnyId1Json,
        string calldata _bunnyId2Json,
        string calldata _bunnyId3Json,
        string calldata _bunnyId4Json
    ) external onlyOwner {
        bunnyIdURIs[0] = string(abi.encodePacked(ipfsHash, _bunnyId0Json));
        bunnyIdURIs[1] = string(abi.encodePacked(ipfsHash, _bunnyId1Json));
        bunnyIdURIs[2] = string(abi.encodePacked(ipfsHash, _bunnyId2Json));
        bunnyIdURIs[3] = string(abi.encodePacked(ipfsHash, _bunnyId3Json));
        bunnyIdURIs[4] = string(abi.encodePacked(ipfsHash, _bunnyId4Json));
    }
...
contract BunnyMintingStation is AccessControl {
    TrueDegenerates public trueDegenerates;
    ...
    constructor(TrueDegenerates _trueDegenerates) public {
        trueDegenerates = _trueDegenerates;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev Mint NFTs from the TrueDegenerates contract.
     */
    function mintCollectible(
        ...
        uint256 tokenId =
            trueDegenerates.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
    ...
    function setBunnyName(uint8 _bunnyId, string calldata _bunnyName)
        ...
        onlyOwner
    {
        trueDegenerates.setBunnyName(_bunnyId, _bunnyName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        trueDegenerates.transferOwnership(_newOwner);
    }
}
...
contract TrueDegenerates is ERC721, Ownable {
    ...
    constructor(string memory _baseURI) public ERC721("True Degenerates", "TD") {
