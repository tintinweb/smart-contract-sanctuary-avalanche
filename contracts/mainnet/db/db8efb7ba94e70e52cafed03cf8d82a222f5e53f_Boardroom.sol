...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    function earned(address member) public view returns (uint256) {
        ...
        return balanceOf(member).mul(latestRPS.sub(storedRPS)).div(1e17).add(members[member].rewardEarned); //17: 0.1AVAX peg
    }
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 nextRPS = prevRPS.add(amount.mul(1e17).div(totalSupply())); //0.1 AVAX peg
