...
contract Liquidator is ILiquidator, Multicall, Manager {
    ...
    constructor(address _lendingPoolAddressProvider, address _augustusSwapper, address _tokenTransferProxy) {
        lendingPoolAddressProvider = _lendingPoolAddressProvider;
        updateSwapParams(_augustusSwapper, _tokenTransferProxy);
    }
    ...
    function swap(bytes calldata data, address srcToken, uint256 amount) override payable public onlyManager {

        _provideAllowanceIfNotEnough(srcToken, amount);
        (bool success, bytes memory result) = augustusSwapper.call{value: msg.value}(data);
        if (!success) {
    ...
    function updateSwapParams (address _augustusSwapper, address _tokenTransferProxy) public onlyOwner {
        augustusSwapper = _augustusSwapper;
        tokenTransferProxy = _tokenTransferProxy;
    }


    function _provideAllowanceIfNotEnough(address token, uint256 amount) private {
    ...
    function withdraw(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
        ...
        (bool success, bytes memory data) = tokenAddress.call(abi.encodeWithSelector(IERC20.transfer.selector, recipient, amount));
...
interface ILiquidator {
  ...
  function swap(bytes calldata data, address srcToken, uint256 amount) external payable;
