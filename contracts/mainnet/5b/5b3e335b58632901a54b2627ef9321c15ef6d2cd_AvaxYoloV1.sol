...
contract AvaxYoloV1 {
	...
	uint256 constant public MAX_WITHDRAW_AMOUNT_PER_DAY = 500 ether;
	...
	uint256 constant public DWTIME_STEP = 1 days;
    ...
    struct DailyDepositsWithdrawals {
        uint256 time;
        uint256 amount;
    }
    DailyDepositsWithdrawals[] public dailyWithdrawals;

	uint256 public lastWithdrawStartUNIX = 0;

	uint256 public startWithdrawalUNIX = 5 days;

	uint256 public last24HrWithdrawAmounts;
	...
	function withdraw() public {
		...
		if((block.timestamp.sub(startUNIX)) >= startWithdrawalUNIX){
            if(lastWithdrawStartUNIX == 0 || ((block.timestamp.sub(lastWithdrawStartUNIX)) >= DWTIME_STEP)){
            	if(lastWithdrawStartUNIX != 0){
                	dailyWithdrawals.push(DailyDepositsWithdrawals(block.timestamp, last24HrWithdrawAmounts));
            	}

		    lastWithdrawStartUNIX = block.timestamp;
            last24HrWithdrawAmounts = 0;
            }

        require((last24HrWithdrawAmounts.add(totalAmount)) <= MAX_WITHDRAW_AMOUNT_PER_DAY, "Daily max withdraw amount threshold reached");
        }
        ...
        last24HrWithdrawAmounts = last24HrWithdrawAmounts.add(totalAmount); 
	...
	function getWithdrawStatus() public view returns (bool isWithdrawEnabled) {
       if (startUNIX < block.timestamp && block.timestamp.sub(startUNIX) >= startWithdrawalUNIX && last24HrWithdrawAmounts < MAX_WITHDRAW_AMOUNT_PER_DAY) {
