...
contract contractHoppers is Ownable, IERC721Receiver{
    ...
    function levelUpHoppersFromZone(uint256 amountFly, uint256 [] memory tokenIds, uint64 nbLvl) external {
        ...
        hoppers z = hoppers(0x85e66216fB0e80F87b54eb39a415c3bbD40E37f9);
        for (uint i=0; i<tokenIds.length; i++){
            h.safeTransferFrom(msg.sender, address(this), tokenIds[i]);
            for (uint j=0; j<nbLvl; j++){
                z.levelUp(tokenIds[i], false);
            }
            h.safeTransferFrom(address(this), msg.sender, tokenIds[i]);
        }
