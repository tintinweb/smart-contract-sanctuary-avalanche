...
// File: contracts/interfaces/IUserSwapFee.sol
...
contract KalmarTokenPriviledge is Partnership {
    ...
    IUserSwapFee swapFee;

    event UpdateUserSwapFee(address userSwapFeeAddr);

    function updateUserStakingPrice(address _userSwapFeeAddr) public onlyOwner
    {
        swapFee = IUserSwapFee(_userSwapFeeAddr);
        emit UpdateUserSwapFee(_userSwapFeeAddr);
    }

    function userSwapFee(address user) public view returns (uint256 fee)
    {
        fee = IUserSwapFee(swapFee).userFeeValue(user);
        return fee;
    }

}

contract Kalmyswap is KalmarTokenPriviledge, WhitelistFeeOnTransfer, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    */
        ...
        returns(uint256)
    {
        ...
        // checking fee
        uint256 fee = userSwapFee(msg.sender);
