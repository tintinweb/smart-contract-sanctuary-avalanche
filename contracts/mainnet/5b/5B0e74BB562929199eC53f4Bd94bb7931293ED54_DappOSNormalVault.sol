...
import "../interfaces/IPayDB.sol";
import "../interfaces/IMainVault.sol";
import "./QuotaLimited.sol";

contract DappOSNormalVault is QuotaLimited {
    bytes32 public constant ROLE_SYNC = keccak256("ROLE_SYNC");
    bytes32 public constant ROLE_EXECUTE = ROLE_REQUEST;
    bytes32 public constant ROLE_PAUSE_NODE = keccak256("ROLE_PAUSE_NODE");
    mapping(address => bool) paused;
    mapping(address => bool) withdrawWhitelist;

    address payDB;

    event SetTransferWhiteList(address indexed to, bool indexed isWhitelisted);

    constructor(address _payDB, address _mainVault) QuotaLimited(_mainVault) {
        setPayDB(_payDB);
    }

    function setPayDB(address _payDB) public onlyRole(DEFAULT_ADMIN_ROLE) {
        payDB = _payDB;
    }
    ...
    function pauseNode(
        address node,
        bool isPaused
    ) public onlyRole(ROLE_PAUSE_NODE) {
    ...
    function cancelOrder(
        ...
        // Check sync auth
        {
            bytes32 digest = keccak256(abi.encode(owner, receiver, cparams));
            address signer = ECDSA.recover(digest, signature);
            require(hasRole(ROLE_SYNC, signer) && !paused[signer], "ERR_AUTH");
        }

        // Check exec auth
        require(
            !paused[msg.sender] && hasRole(ROLE_EXECUTE, msg.sender),
            "ERR_PausedOrAuth"
        );
        ...
        for (uint256 i = 0; i < cparams.length; i++) {
            QuotaLimited.request(cparams[i].tokenIn, cparams[i].amountIn);
        }
    ...
    function executeOrder(
        ...
        // Check sync auth
        {
            bytes32 digest = keccak256(
                abi.encode(eparams, keccak256(vwExeParam.data))
            );
            address signer = ECDSA.recover(digest, signature);
            require(hasRole(ROLE_SYNC, signer) && !paused[signer], "ERR_AUTH");
        }
        ...
        for (uint256 i = 0; i < eparams.length; i++) {
            QuotaLimited.request(eparams[i].tokenOut, eparams[i].amountOut);
        }
    ...
    function setTransferWhiteList(
        ...
        withdrawWhitelist[account] = enabled;

        emit SetTransferWhiteList(account, enabled);
    }
    ...
    function transferOut(
        ...
        require(withdrawWhitelist[account], "E1");

        TransferHelper.safeTransfer2(token, account, amount);
    }
...
contract QuotaLimited is VaultPlugIn, ReentrancyGuard {
    // Quota per interval
    mapping(address => uint256) quotaInterval;
    mapping(address => uint256) quotaAmount;
    mapping(address => uint256) lastSlot;
    mapping(address => uint256) lastSlotRequested;
    mapping(address => uint256[]) requestTimestamps;

    // Quota per block
    mapping(address => uint256) blockQuotaAmount;
    mapping(address => uint256) lastBlock;
    mapping(address => uint256) lastBlockRequested;

    constructor(address _mainVault) VaultPlugIn(_mainVault) {}

    function request(
        address tokenAddress,
        uint256 tokenAmount
    ) internal override onlyRole(ROLE_REQUEST) nonReentrant {
        uint256 tokenValue = IMainVault(mainVault).calculateTokenValue(
            block.chainid,
            tokenAddress,
            tokenAmount
        );

        require(_checkQuota(msg.sender, tokenValue), "E15");
        VaultPlugIn.request(tokenAddress, tokenAmount);
        _updateQuota(msg.sender, tokenValue);
    }

    function setQuota(
        address[] calldata botAddress,
        uint256[] calldata _quotaInterval,
        uint256[] calldata _quotaAmount,
        uint256[] calldata _blockQuotaAmount
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < botAddress.length; i++) {
            quotaAmount[botAddress[i]] = _quotaAmount[i];
            quotaInterval[botAddress[i]] = _quotaInterval[i];
            blockQuotaAmount[botAddress[i]] = _blockQuotaAmount[i];
        }
    }
    ...
    function _checkQuota(
        ...
        // 1. Check block quota
        uint256 blockQuota = blockQuotaAmount[botAddress];
        uint256 blockRequest = tokenValue;
        if (block.number == lastBlock[botAddress]) {
            blockRequest += lastBlockRequested[botAddress];
        }
        if (blockRequest > blockQuota) return false;

        // 2. Check interval quota
        // Protect against divide-by-zero
        if (quotaInterval[botAddress] == 0) {
            return false;
        }

        uint256 currentSlot = block.timestamp / quotaInterval[botAddress];
        uint256 quota = quotaAmount[botAddress];
        if (currentSlot == lastSlot[botAddress]) {
            // Add to tokenValue instead of subtract from quota
            // to avoid integer underflow
            tokenValue += lastSlotRequested[botAddress];
        }

        return tokenValue <= quota;
    }

    function _updateQuota(address botAddress, uint256 tokenValue) internal {
        // 1. Update block quota info
        if (block.number == lastBlock[botAddress]) {
            lastBlockRequested[botAddress] += tokenValue;
        } else {
            lastBlock[botAddress] = block.number;
            lastBlockRequested[botAddress] = tokenValue;
        }

        // 2. Update interval quota info
        uint256 currentSlot = block.timestamp / quotaInterval[botAddress];

        if (currentSlot == lastSlot[botAddress]) {
            lastSlotRequested[botAddress] += tokenValue;
        } else {
            lastSlot[botAddress] = currentSlot;
            lastSlotRequested[botAddress] = tokenValue;
        }
    }
}
