...
/**
 * @title AvalanchePredictionV3
 */
contract AvalanchePredictionV3 is Ownable, Pausable, ReentrancyGuard {
  ...
  address public treasuryAddress = owner(); // address of the treasury
  ...
  modifier onlyAdmin() {
    require(msg.sender == adminAddress || msg.sender == owner(), "Not admin");
    _;
  }
  ...
  /**
   * @notice Force start the next round n, lock price for round n-1, end round n-2
   * @dev Callable by operator
   */
  function forceExecuteRound() external whenNotPaused onlyAdminOrOperator {
    require(
      genesisStartOnce && genesisLockOnce,
      "Can run after Start and Lock"
    );
    ...
    // CurrentEpoch refers to previous round (n-1)
    _forceLockRound(currentEpoch, currentRoundId, currentPrice);
    _forceEndRound(currentEpoch - 1, currentRoundId, currentPrice);
    _calculateRewards(currentEpoch - 1);
   ...
   */
  function claim(uint256 amount) external nonReentrant onlyAdmin {
    require(amount < treasuryAmount, "TreasuryAmount is not enough");
    treasuryAmount = treasuryAmount - amount;
    _safeTransferAVAX(treasuryAddress, amount);
  ...
  function claimTreasury() external nonReentrant onlyAdmin {
    ...
    treasuryAmount = 0;
    _safeTransferAVAX(treasuryAddress, currentTreasuryAmount);
  ...
  function _calculateRewards(uint256 epoch) internal {
    ...
    if (round.closePrice > round.lockPrice) {
      ...
      if (rewardBaseCalAmount == 0) {
        treasuryAmt += round.bearAmount - (round.bearAmount * treasuryFee) / 10000;
      }
    ...
    // Bear wins
      ...
      treasuryAmt = (round.totalAmount * treasuryFee) / 10000;
      if (rewardBaseCalAmount == 0) {
        treasuryAmt += round.bullAmount - (round.bullAmount * treasuryFee) / 10000;
      }
      rewardAmount = round.totalAmount - treasuryAmt;
    }
    // House wins
  ...
  /**
   * @notice Force End round
   * @param epoch: epoch
   * @param roundId: roundId
   * @param price: price of the round
   */
  function _forceEndRound(
    uint256 epoch,
    uint256 roundId,
    int256 price
  ) internal {
  ...
  /**
   * @notice Force Lock round
   * @param epoch: epoch
   * @param roundId: roundId
   * @param price: price of the round
   */
  function _forceLockRound(
    uint256 epoch,
    uint256 roundId,
    int256 price
  ) internal {
