...
contract MAKER is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 36h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 60000 ether;
    ...
    uint256 public burnThreshold = 1.10e15;
    ...
    /**
     * @notice Constructs the MAKER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MAKER", "MAKER") {
        // Mints 1 MAKER to contract creator for initial pool setup
    ...
    function _getMakerPrice() internal view returns (uint256 _makerPrice) {
        try IOracle(makerOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Maker: failed to fetch MAKER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _makerPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_makerPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMakerOracle(address _makerOracle) public onlyOperatorOrTaxOffice {
        require(_makerOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MAKER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MAKER to mint to
    ...
    function transferFrom(
        ...
        if (!autoCalculateTax || excludedAddresses[sender]) {
            _transfer(sender, recipient, amount);
        } else {
