...
library PoolHelperFactoryLib {
    ...
    {
        PoolHelper pool = new PoolHelper(_pid, _stakingToken, _depositToken, _mainStaking, _masterVtx, _rewarder, _xptp);
        return address(pool);
    }
}
...
contract MainStaking is Initializable, OwnableUpgradeable {
    ...
    function withdrawPTP() external onlyOwner {
        uint256 stakedPTP = IPTPStaking(staking_ptp).getStakedPtp(address(this));
        IPTPStaking(staking_ptp).withdraw(stakedPTP);
        IERC20(ptp).safeTransfer(msg.sender, IERC20(ptp).balanceOf(address(this)));
    }
