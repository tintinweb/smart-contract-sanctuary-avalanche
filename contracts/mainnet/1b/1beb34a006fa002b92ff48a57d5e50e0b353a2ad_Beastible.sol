...
contract Beastible is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    using Strings for uint256;

    uint256 public MAX_MINTABLE = 151; // 151 total for this launchpad
    uint256 public constant MAX_PER_CLAIM = 2;
    uint256 public constant MAX_CLAIM_PER_WALLET = 2;
    uint256 public STARTING_NFT_PRICE = 3 ether; // 3 avax for this launchpad
    uint256 public constant ROYALTY_VALUE = 500; // 100 = 1%, 200 = 2% etc...  5% for this launchpad
    uint256 public constant MIN_PRICE = 0 ether;
    uint256 public constant DURATION_STEP = 2 minutes; // 2min for this launchpad
    uint public immutable discountRate = 0.2 ether;  // 0.2 avax for this launchpad
    uint public currentSale = 1;
    uint public startAt;
    ...
    event Claim(uint256 indexed _id, uint256 indexed _price);

    mapping(address => mapping(uint => uint)) public publicMintedAmount;
...
//
    function claim(uint256 n) public payable {
        ...
        require(publicMintedAmount[msg.sender][currentSale] < MAX_CLAIM_PER_WALLET, "exceeds the public minting limit");
        require(n + publicMintedAmount[msg.sender][currentSale] <= MAX_CLAIM_PER_WALLET, "you can't claim that much");
        ...
        publicMintedAmount[msg.sender][currentSale] += n;
...
//////
    ...
    function setMaxMintable(uint256 _maxMintable) external onlyOwner {
        MAX_MINTABLE = _maxMintable;
    }

    function setCurrentSale(uint256 _currentSale) external onlyOwner {
...
//
    ...
    function getPublicPrice() public view returns (uint) {
        ...
        uint countTenMinutes = timeElapsed / DURATION_STEP; // 600sec = 10minutes
        uint discountFinal = countTenMinutes * discountRate;
        uint finalResult;
    ...
    function getPublicMintedAmount(address _user, uint256 _sale) public view returns(uint256){
            return publicMintedAmount[_user][_sale];
    }
