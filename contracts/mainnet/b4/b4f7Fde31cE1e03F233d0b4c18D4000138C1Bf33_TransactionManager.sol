...
contract TransactionManager is TransactionOwner, ReentrancyGuard {
  ...
  function createNewTransaction(
    ...
    require(uint8(TransactionType.TokenToToken) >= transactionType);
    ...
    require(transactions[transactionKey].isExist != true);

    uint256 pendingTime = defaultPendingTime;

    if (proposedPendingTime != 0) {
      require(proposedPendingTime >= minPendingTime);
      require(proposedPendingTime <= maxPendingTime);
  ...
  function depositUser0(
    ...
    require(transactions[transactionKey].isExist);

    Transaction memory transaction = transactions[transactionKey];

    require(transaction.user0 == _msgSender());
    require(block.timestamp < transaction.expiresAt);
    require(transaction.user0Status == Status.Empty);

    transactions[transactionKey].user0Status = Status.Deposited;

    if (transaction.transactionType == TransactionType.ETHToToken) {
      require(msg.value >= transaction.token0Amount);
    }
  ...
  function depositUser1(
    ...
    require(transaction.user1 == _msgSender());
    require(transaction.user0 == user0);
    require(block.timestamp < transaction.expiresAt);
    require(transaction.user1Status == Status.Empty);

    transactions[transactionKey].user1Status = Status.Deposited;

    if (transaction.transactionType == TransactionType.TokenToETH) {
      require(msg.value >= transaction.token1Amount);
    }
