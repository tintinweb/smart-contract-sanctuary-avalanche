...
contract PriceGetter is Pausable, AccessControl, IPriceGetter {
    ...
    /// @inheritdoc IPriceGetter
    function addPriceFeed(
        bytes32 _priceFeedName,
        address _priceOracleAddress
    ) public onlyRole(OPERATOR_ROLE) returns (bool success) {
        // require(_priceFeedName.length >= 7, "Minimum length not accomplished");
    ...
    /// @inheritdoc IPriceGetter
    function getPrice(bytes32 _priceFeedName) public view returns (uint256) {
        if (!isPriceFeedExists(_priceFeedName)) revert("PriceFeedNotExists");
