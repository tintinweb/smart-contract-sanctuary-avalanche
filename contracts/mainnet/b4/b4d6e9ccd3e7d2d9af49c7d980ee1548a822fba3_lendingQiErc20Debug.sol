...
// File: contracts/interfaces/IBenqiCompound.sol
...
abstract contract superTokenInterface{
    ...
    function availableBalance() public virtual view returns (uint256);
...
//    uint256 public flashFeeRate = 1e16;
    ...
    event SetFeeRate(address indexed from,uint256 index,uint256 _feeRate);
    ...
    function approveRewardToken(address _rewardToken)internal {
        if(_rewardToken == address(0)){
            if(IERC20(address(WAVAX)).allowance(address(this), address(swapHelper)) == 0){
                SafeERC20.safeApprove(IERC20(address(WAVAX)), address(swapHelper), uint(-1));
            }
        }else if(IERC20(_rewardToken).allowance(address(this), address(swapHelper)) == 0){
    ...
    function _setReward(uint256 index,uint8 _reward,bool _bClosed,address _rewardToken,uint256 _sellLimit) internal virtual{
        ...
        if(index <rewardInfos.length){
            ...
            if(!_bClosed){
                approveRewardToken(_rewardToken);
            }else{
                if(_rewardToken == address(0)){
                    SafeERC20.safeApprove(IERC20(address(WAVAX)), address(swapHelper), 0);
                }else{
...
// File: contracts/superToken/superQiToken/superQiToken.sol
...
//
// This contract handles swapping to and from superToken.
abstract contract superQiToken is baseSuperToken{
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
        ...
        compounder = IBenqiCompound(ICErc20(_stakeToken).comptroller());
        _setReward(0,0,false,0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5,1e17);//qi
        _setReward(1,1,false,address(0),1e15);//avax
    }
    function availableBalance() public virtual override view returns (uint256){
        return totalUnderlying();
    }
    function totalUnderlying()public virtual override view returns (uint256){
        uint256 exchangeRate = ICErc20(address(stakeToken)).exchangeRateStored();
        return exchangeRate.mul(stakeToken.balanceOf(address(this)))/calDecimals;
    }
...
// File: contracts/superToken/superQiToken/superQiErc20.sol
...
contract superQiErc20 is superQiToken {
    ...
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
        address payable _swapHelper,address payable _FeePool)
            superQiToken(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_FeePool) {
        underlying = IERC20(ICErc20(_stakeToken).underlying());
        SafeERC20.safeApprove(IERC20(underlying), _stakeToken, uint(-1));
    }
    ...
    function withdraw(address account,uint256 _amount)internal virtual override returns(uint256){
        ICErc20(address(stakeToken)).redeemUnderlying(_amount);
    ...
    function qiSupply(uint256 _fee) internal returns (uint256){
        ...
        if (balance>0){
            ...
            ICErc20(address(stakeToken)).mint(balance);
            return balance;
        }
        return 0;
    }
    ...
    function lendTo(address account,uint256 amount) internal virtual override {
        ICErc20(address(stakeToken)).redeemUnderlying(amount);
...
// File: contracts/modules/interestEngine_hash.sol
...
// File: contracts/superToken/lendingSuperToken.sol
...
//
// This contract handles swapping to and from superToken.
abstract contract lendingSuperToken is superTokenInterface,interestEngine_hash{
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    uint64 internal defaultRate = 5e14;
    address public ownerLeverageFactory;
        // --- Auth ---
    mapping (address => uint256) public authorizedAccounts;
    event Borrow(address indexed from,bytes32 indexed account,address indexed token,uint256 reply);
    event Repay(address indexed from,bytes32 indexed account,address indexed token,uint256 amount);
...
//        authorizedAccounts[leverageFactory] = 1;
    ...
    **/
    ...
    function poolInterest()public view returns (uint256){
        ...
        return (_totalSupply.sub(balance)).mul(interestRate)/_totalSupply;
    }
    */
    function setInterestRate(int256 _interestRate,uint256 rateInterval)external isFactory{
        _setInterestInfo(_interestRate,rateInterval,1e30,0);
    }
    /*
    function interestInflation(uint64 balanceLeftRate, uint64 inflation) external isFactory{
        uint256 _totalSupply = totalUnderlying();
        if(_totalSupply > 0){
            uint256 balance = availableBalance();
            if(balance<_totalSupply.mul(balanceLeftRate)/calDecimals){
                interestRate = interestRate*inflation/1e18;
            }else{
...
// File: contracts/superToken/lendingQiToken/lendingQiErc20.sol
...
contract lendingQiErc20 is superQiErc20,lendingSuperToken {
    ...
    function availableBalance() public virtual override(superQiToken,superTokenInterface) view returns (uint256){
        uint256 exchangeRate = ICErc20(address(stakeToken)).exchangeRateStored();
        return exchangeRate.mul(stakeToken.balanceOf(address(this)))/calDecimals;
    }
    function totalUnderlying()public virtual override(superQiToken,superTokenInterface) view returns (uint256){
        return availableBalance().add(totalAssetAmount());
    }
}

// File: contracts/modules/debugger.sol
...
// File: contracts/debug/lendingQiErc20Debug.sol
...
//
// This contract handles swapping to and from superToken.
contract lendingQiErc20Debug is lendingQiErc20,debugger {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        lendingQiErc20(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_FeePool,leverageFactory) {
