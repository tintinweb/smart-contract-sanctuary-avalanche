 ...
 */
contract AaveVaultStage is BaseBuildingBlock, Guarded {
     ...
     */
    function setUserEMode(uint8 categoryId) external auth {
     ...
     * @param collateralAsset Asset address
     */
    function setCollateralAsset(address collateralAsset) external auth {
        aaveLendingPool.setUserUseReserveAsCollateral(collateralAsset, true);
    }

    /**
     * @notice _deposits into contract to allow open a position on AAVE
     * @param depositor, baseAsset, amount
     */
    function deposit(
        address baseAsset,
        uint256 amount,
        address depositor
    ) external payable auth whenNotPaused {
        if (baseAsset == address(0x0)) {
            require(
                msg.value == amount,
                "AAVE BB: message value mismatch amount"
            );
        } else {
            require(
                ...
                "AAVE BB: Desposit transfer failed"
            );
        }
     ...
     * @param baseAsset  asset address, amount to deposit
     */
    function openPosition(address baseAsset, uint256 amount)
        external
        auth
        whenNotPaused
    {
        if (baseAsset == address(0x0)) {
            ...
            require(
                amount <= IERC20(baseAsset).balanceOf(address(this)),
     ...
     * @param baseAsset baseAsset address,amount to withdraw
     * @dev to withdraw amount on native currency
     */
    function closePosition(address baseAsset, uint256 amount)
        external
        auth
        whenNotPaused
    {
     ...
     * @param baseAsset : baseAsset & amount to withdraw, recipient to withdraw from
     */
    function withdrawOnAddress(
        address baseAsset,
        uint256 amount,
        address recipient
     ...
     * @dev If borrow Eth, borrowed amount will be sent to msg.sender
     */
    function borrow(
        address borrowAsset,
        uint256 amount,
     ...
     * @param baseAsset asset to repay with
     * @param amount to repay
     * @param borrowRate the type of borrow debt. Stable: 1, Variable: 2
     * @dev Repays debt on behalf of this contract
     */
    function repay(
        address baseAsset,
        uint256 amount,
    ...
    {
        (rewardsList, claimedAmounts) = rewardsController
            .claimAllRewardsOnBehalf(assets, user, to);
    }

    /**
     * @notice Withdraw native asset on this contract
     * @param amount to withdraw
     */
    function withdrawNativeAsset(uint256 amount)
        external
        payable
        auth
        whenNotPaused
        ...
        )
    {
        (
            ...
            ltv,
            healthFactor
        ) = aaveLendingPool.getUserAccountData(user);
    }
}
/* solhint-enable */
