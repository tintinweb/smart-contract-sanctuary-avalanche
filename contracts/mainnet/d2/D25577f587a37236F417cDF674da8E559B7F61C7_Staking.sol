/**
 *Submitted for verification at snowtrace.io on 2022-07-30
*/
...
abstract contract GlobalsAndUtils {
    ...
    mapping(uint256 => mapping(uint256 => address)) internal ticketCollection;
    mapping(uint256 => mapping(uint256 => uint256)) public ticketNFTID;
...
/**
     ...
     */
    function claimStakingReward(uint256 campaignId)  external {
     ...
     */
    function startLootBox(uint256 startTime, uint256 endTime, uint256 rewardCount, address[] memory _awardCollections, uint256 tokenPerBox) external onlyOwner campaignEnded {
        uint256 totalNfts = 0;
        ...
        latestCampaignId += 1;
        campaigns[latestCampaignId] = Campaign(rewardCount, startTime, endTime, tokenPerBox);
    ...
    function addNftTokenIds(uint256 _campaignId, address _collectionAddress, uint256[] memory _nftTokenIds) external onlyOwner {
        ...
        for (uint256 m; m < _nftTokenIds.length; m++) {
            if(IERC721(_collectionAddress).ownerOf(_nftTokenIds[m]) != address(this)){
                revert("no in contract ownership");
            }
    ...
    function _unStake(uint256 _stakeId) private {
        ...
        unStakes[msg.sender][_stakeId] = UnStake(stakes[msg.sender][_stakeId].stakedAt,block.timestamp,stakes[msg.sender][_stakeId].stakedAmount);
     ...
     */
     ...
     function claimAllStakingRewards() public {
        uint256 count;
         for (uint256 index = 1; index <= latestCampaignId; index++) {
             Campaign memory tempCampaing = campaigns[index];
             if(tempCampaing.endTime < block.timestamp){
                 (uint256[] memory _userWinningIds,,,,uint256 claimed,) = getRewardClaimable(index);
                 if(claimed ==0 && _userWinningIds.length > 0){
                     _rewardWinner(msg.sender,index);
                     count++;
                 }
             }
         }
         require(count > 0, "no reward to be claimed");
     }
     ...
     */
     ...
     function isClaimAllRewards() public view returns(bool) {
        uint256 count;
         for (uint256 index = 1; index <= latestCampaignId; index++) {
             Campaign memory tempCampaing = campaigns[index];
             if(tempCampaing.endTime < block.timestamp){
    ...
    function _getUserStakedTime(Stake memory usrStake) private view returns (uint256 unixStakedTime) {
        ...
        unixStakedTime =  block.timestamp.sub(usrStake.stakedAt > campaigns[latestCampaignId].startTime ? usrStake.stakedAt : campaigns[latestCampaignId].startTime);
    }
    ...
    function _rewardWinner(address _winnerAddress, uint256 campaignId) private {
       (uint256[] memory _winningTicketIds,,,,,)= getRewardClaimable(campaignId);
       ...
       uint256 tokenToMint = campaigns[campaignId].token;
    //    uint256[] storage _winningTicketIds = winningTicketIds[campaignId];
        for (uint256 j = 0; j < _winningTicketIds.length; j++) {
           ...
           if(winningTicketId >= rangeStart && winningTicketId <= rangeEnd){
               if(rewardsReceived[_winnerAddress][campaignId] >= 5){
                   xanaToken.mint(msg.sender,  tokenToMint.mul(rewardsReceived[_winnerAddress][campaignId]));
                    return;
                }
                rewardsToken = IERC721(ticketCollection[campaignId][winningTicketId]);
                if(rewardsToken.ownerOf(ticketNFTID[campaignId][winningTicketId]) == address(this)){
    ...
    function _getRandomNumbers(uint256 randomSeed, uint256 num, uint256 ranEnd) private pure returns (uint256[] memory ranValues) {
        ...
        for (uint256 i = 0; i < num; i++) {
            ranValues[i] = uint256(keccak256(abi.encode(randomSeed, i))) % ranEnd + 1;
        }
        return ranValues;
    }
     ...
     * @param receiver Address of the user to transfer the nft to
     */
    function withdrawPenaltyAmount(uint256 _amount, address receiver) external onlyOwner {
        require(_amount <= penaltyAccumulated, "not enough tokens in contract");
    ...
    function getRewardClaimable(uint256 campaignId) public view  returns(uint256[] memory, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (getXTicektedIDs(campaignId),campaigns[campaignId].token, campaigns[campaignId].startTime, campaigns[campaignId].endTime, rewardsReceived[msg.sender][campaignId], totalUserXTickets[msg.sender][campaignId]);
    }
     ...
     * @dev ADMIN METHOD: Method to update staking token address
     */
    function updateXanaPointToken(address xanaPoint) external onlyOwner {
        require(xanaPoint != address(0), "invalid address");
        xanaToken = IERC20(xanaPoint);
    }
    ...
    function claimTaxBonus() external nonReentrant  {
        ...
        penaltyHistories[msg.sender][totalPenaltiesClaimed] = penaltyHistory(block.timestamp, totalStakedAmount, userStakedAmount[msg.sender], penaltyAccumulated, stakerPenaltyBonus);
