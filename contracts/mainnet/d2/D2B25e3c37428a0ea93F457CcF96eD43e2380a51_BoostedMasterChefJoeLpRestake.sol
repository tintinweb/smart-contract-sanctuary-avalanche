 ...
 */
contract BoostedMasterChefJoeLpRestake is Automate {
  ...
  IBoostedMasterChefJoe public staking;
  ...
  function init(
    ...
    require(
      ...
      "BoostedMasterChefJoeLpRestake::init: reinitialize staking address forbidden"
    );
    staking = IBoostedMasterChefJoe(_staking);
    require(
      !_initialized || liquidityRouter == _liquidityRouter,
      "BoostedMasterChefJoeLpRestake::init: reinitialize liquidity router address forbidden"
    );
    liquidityRouter = _liquidityRouter;
    require(!_initialized || pool == _pool, "BoostedMasterChefJoeLpRestake::init: reinitialize pool index forbidden");
    pool = _pool;
    slippage = _slippage;
    deadline = _deadline;

    if (!_initialized) {
      IBoostedMasterChefJoe.PoolInfo memory poolInfo = staking.poolInfo(pool);
  ...
  function refund() external onlyOwner {
    IBoostedMasterChefJoe _staking = staking; // gas optimisation
    address __owner = owner(); // gas optimisation
    IBoostedMasterChefJoe.UserInfo memory userInfo = _staking.userInfo(pool, address(this));
  ...
  function run(
    ...
    Swap memory swap0,
    Swap memory swap1
  ) external bill(gasFee, "AvaxTraderjoeBoostedMasterChefJoeLpRestake") {
    IBoostedMasterChefJoe _staking = staking; // gas optimization
    (uint256 earned, , , ) = _staking.pendingTokens(pool, address(this));
    require(earned > 0, "BoostedMasterChefJoeLpRestake::run: no earned");
