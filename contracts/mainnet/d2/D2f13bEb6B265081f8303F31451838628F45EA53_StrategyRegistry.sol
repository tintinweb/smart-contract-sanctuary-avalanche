    ...
    NonBlockingNonUpgradableBaseApp
{
    ...
    modifier onlyFabric() {
        require(
            msg.sender == address(this) || msg.sender == _nativeFabric,
            "Registry:only from fabric"
        );
        _;
    }
    ...
    function setNativeFabric(address _fabric) external onlyOwner {
        _nativeFabric = _fabric;

        emit NativeFabricChanged(_nativeFabric);
    }

    function getNativeFabric() external view returns (address) {
        return _nativeFabric;
    }
...
interface IStrategyRegistry {
    ...
    event NativeFabricChanged(address newFabric);
    ...
    function setNativeFabric(address fabric) external;
