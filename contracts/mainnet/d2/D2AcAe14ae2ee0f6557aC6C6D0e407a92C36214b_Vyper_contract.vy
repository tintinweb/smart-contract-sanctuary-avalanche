...
AVAX_REWARDS: constant(address) = 0x929EC64c34a17401F460460D4B9390518E5B473e
...
def _claim_rewards():
    ...
    if reward_receiver != ZERO_ADDRESS:
        raw_call(
            AVAX_REWARDS,
            concat(
                method_id("claimAllRewards(address[],address)"),
                convert(32 * 2, bytes32),
                convert(reward_receiver, bytes32),
                convert(3, bytes32),
        ...
        for i in range(N_COINS):
            ...
            if amount != 0:
                ...
                raw_call(
                    AAVE_LENDING_POOL,
                    concat(
                        method_id("supply(address,uint256,address,uint16)"),
