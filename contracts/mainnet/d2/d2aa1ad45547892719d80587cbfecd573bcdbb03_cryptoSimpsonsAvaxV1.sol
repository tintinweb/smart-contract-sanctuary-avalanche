 ...
 *   - Minimal deposit: 0.1AVAX, no maximal limit
 *   - withdraw AT THE END 
 *
 *   [AFFILIATE PROGRAM]
 *
...
contract cryptoSimpsonsAvaxV1 {
	...
	constructor(address payable marketing, uint256 startDate)  {
		require(!isContract(marketing));
		require(startDate > 0);
        marketingWallet = marketing;
        
		startUNIX = startDate; // 1649358000 
        
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(3, 290)); // 3 days
        plans.push(Plan(6, 173));// 6 days
        plans.push(Plan(9, 145)); // 9 days
        plans.push(Plan(12, 125)); // 12 days
	}
