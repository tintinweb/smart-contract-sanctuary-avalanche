...
/// @notice RockPaperScissors contract using a global treasury and multiple hands
contract RockPaperScissors is GameBase, GameBet, GameChainlink {
    enum RoundChoice {
        ROCK,   // 0
        PAPER,  // 1
        SISSORS // 2
    ...
    /// @notice even if a user ties all plays, we keep this much of the rake for chainlink costs
    uint256 public rakeMinimum = .01 ether;
    ...
    function _createGame(
        ...
        emit GameStarted(msg.sender, fi.gameId, fi.choices, fi.referrer, fi.betPerRound, fi.rakePerRound);
        return fi.gameId;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        RoundInfo[] memory results = new RoundInfo[](fi.choices.length);
        uint256 betToUser = 0;
        uint256 rakeToUser = 0;
        uint256 rakeToKeep = 0;

        for (uint i=0; i<fi.choices.length; i++) {
            uint256 roundRandom = uint256(keccak256(abi.encode(randomWords[0], i)));
            results[i].selected = RoundChoice(roundRandom % 3);
            if (fi.choices[i] == results[i].selected) {
                results[i].state = RoundState.TIE;
                results[i].sent = fi.betPerRound + fi.rakePerRound;
                betToUser += fi.betPerRound;
                rakeToUser += fi.rakePerRound;
            } else if ((uint(fi.choices[i]) + 1) % 3 == uint(results[i].selected)) {
                results[i].state = RoundState.LOST;
                rakeToKeep += fi.rakePerRound;
            } else {
                results[i].state = RoundState.WON;
                results[i].sent = fi.betPerRound * 2;
                betToUser += fi.betPerRound * 2;
                rakeToKeep += fi.rakePerRound;
            }
        }

        if (rakeToKeep < rakeMinimum) {
            /// Reduce the rake refund by the extra we need to keep.
            uint256 rakeDelta = rakeMinimum - rakeToKeep;
            results[0].sent -= rakeDelta;
            rakeToUser -= rakeDelta;
            rakeToKeep += rakeDelta;
        }

        uint256 amountToUser = betToUser + rakeToUser;
        if (amountToUser > 0) {
            treasury.sendPayout(fi.creator, amountToUser);
        }
        rakeDistributor.distributeRake{value:rakeToKeep}();
