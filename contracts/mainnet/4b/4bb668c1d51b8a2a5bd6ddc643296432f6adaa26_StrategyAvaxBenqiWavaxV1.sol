...
abstract contract StrategyBase {
    ...
    // Dex
    address public univ2Router2 = 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106;//pangolin exchange

    //Sushi
    address public sushiRouter = 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106;//pangolin exchange
...
interface IComptroller {
    ...
    function annexAccrued(address) external view returns (uint256);
    ...
    // Claim all the Venus accrued by holder in all markets
    function claimVenus(address holder) external;

    // Claim all the COMP accrued by holder in specific markets
    function claimComp(address holder, address[] calldata cTokens) external;
    
    // Claim all the Venus accrued by holder in specific markets
    function claimVenus(address holder, address[] calldata vTokens) external;

    // Claim all the Annex accrued by holder in specific markets
    function claimAnnex(address holder, address[] calldata aTokens) external;
...
contract StrategyAvaxBenqiWavaxV1 is StrategyBase, Exponential {
    //want token WAVAX
    address public constant comptroller = 0x486Af39519B4Dc9a7fCcd318217352830E8AD9b4; //done
    address public constant qiAVAX = 0x5C0401e81Bc07Ca70fAD469b451682c0d747Ef1c; //done   qiAVAX
    address public constant BENQI = 0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5; //reward 
    ...
    public StrategyBase(WAVAX, _governance, _strategist, _controller, _timelock)
    {
        // Enter qiAVAX Market
        address[] memory ctokens = new address[](1);
        ctokens[0] = qiAVAX;
        IComptroller(comptroller).enterMarkets(ctokens);
        IERC20(WAVAX).safeApprove(qiAVAX, uint256(-1));
        benignCallers[address(this)] = true;
        
        IERC20(want).safeApprove(WAVAX, uint256(-1));
    }

    receive() external payable {
        //allow contract receive AVAX
    }
    ...
    function getName() external override pure returns (string memory) {
        return "StrategyAvaxBenqiWavaxV1";
    }

    function getSuppliedView() public view returns (uint256) {
        (, uint256 cTokenBal, , uint256 exchangeRate) = ICToken(qiAVAX).getAccountSnapshot(address(this));
        (, uint256 bal) = mulScalarTruncate(Exp({mantissa: exchangeRate}), cTokenBal);
        return bal;
    }

    function getBorrowedView() public view returns (uint256) {
        return ICToken(qiAVAX).borrowBalanceStored(address(this));
    }
    ...
    function getLeveragedSupplyTarget(uint256 supplyBalance, uint256 _leverage) pure public returns (uint256){
        return supplyBalance.mul(_leverage).div(1e18);
    }
    ...
    function _getSuppliedUnleveraged(uint256 _supplied) internal returns (uint256) {
        uint256 borrowed = getBorrowed();
        return _supplied.sub(borrowed);
    }

    function getSupplied() public returns (uint256) {
        return ICToken(qiAVAX).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public returns (uint256) {
        return ICToken(qiAVAX).borrowBalanceCurrent(address(this));
    }
    ...
    function getBorrowable() public returns (uint256) {
        ...
        uint256 _amt = supplied.mul(colFactor).div(1e18);
        return _amt > borrowed? (_amt.sub(borrowed)).mul(9999).div(10000) : 0;
    }
    ...
    function leverageToMax() public {
        ...
        if (curLeverage < maxLeverage){
            ...
            uint256 idealSupply = getLeveragedSupplyTarget(unleveragedSupply, maxLeverage);
            leverageUntil(idealSupply, maxLeverage, unleveragedSupply);
        }
    }
    ...
    // 1. Borrow out <X> AVAX
    // 2. Supply <X> AVAX
    function leverageUntil(uint256 _supplyAmount, uint256 _maxLeverage, uint256 _unleveragedSupply) public onlyBenignCallers {

        require(_supplyAmount >= _unleveragedSupply && _supplyAmount <= _unleveragedSupply.mul(_maxLeverage).div(1e18), "!leverage");
        ...
        while (supplied < _supplyAmount) {
            ...
            if (_borrowAndSupply > 0){
                ICEther(qiAVAX).borrow(_borrowAndSupply);
                WETH(WAVAX).deposit{value: _borrowAndSupply}();
                deposit();
    ...
    // 1. Redeem <x> AVAX
    // 2. Repay <x> AVAX
    function deleverageUntil(uint256 _supplyAmount) public onlyBenignCallers {
        uint256 supplied = getSupplied();
        uint256 unleveragedSupply = _getSuppliedUnleveraged(supplied);
        ...
        do {
            ...
            if (_redeemAndRepay > 0){
                require(ICEther(qiAVAX).redeemUnderlying(_redeemAndRepay) == 0, "!redeemInDeleverageUntil");
                ICEther(qiAVAX).repayBorrow{value:_redeemAndRepay}();
    ...
    function harvest() public override onlyBenignCallers {
        ...
        // claim AVAX if any
        uint256 _avaxBal = address(this).balance;
        IComptroller(comptroller).claimReward(1, address(this), qiTokens);
        uint256 _avaxClaimed = address(this).balance.sub(_avaxBal);
        if (_avaxClaimed > 0){
            WETH(WAVAX).deposit{value:_avaxClaimed}();
        }
        
        // claim BENQI
        IComptroller(comptroller).claimReward(0, address(this), qiTokens);
        uint256 _rewards = IERC20(BENQI).balanceOf(address(this));
        if (_rewards > 0) {
            _swapUniswap(BENQI, want, _rewards);
        }

        uint256 _reinvest = balanceOfWant().sub(_wantBal);
        uint256 _buybackAmount = _reinvest.mul(performanceFee).div(performanceMax);

        if (buybackEnabled == true && _buybackAmount > 0) {
            buybackAndNotify(want, _buybackAmount);
            _reinvest = _reinvest.sub(_buybackAmount); 
        }
    ...
    function deposit() public override{
        ...
        if (_want > 0) {
            WETH(WAVAX).withdraw(_want);
            ICEther(qiAVAX).mint{value: _want}();
            require(IERC20(qiAVAX).balanceOf(address(this)) > 0, "!deposit");
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256){
        ...
        require(ICToken(qiAVAX).getCash() >= _redeem, "!cash-liquidity");
        ...
        // Redeems underlying
        require(ICToken(qiAVAX).redeemUnderlying(_redeem) == 0, "!redeemInWithdrawSome");
        WETH(WAVAX).deposit{value:_redeem}();

        // -- CoinFabrik: calculate tokens redeemed --
        uint256 _reedemed = balanceOfWant();
        _reedemed = _reedemed.sub(_balance);

        return _reedemed > _amount? _amount : _reedemed;
    }

    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        require(qiAVAX != address(_asset), "!cToken");
