...
contract MainStakingContractAVAX is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < stables.length; i++) {
            token.push(stables[i]);
        }
        // Set swappers
        setSwappers(stables, swappers);
        // Pool
        poolInfo = PoolInfo({
    ...
    // Set swapper
    function setSwappers(address[] memory _tokens, ISwapper[] memory _swappers) public onlyOwner {
        require(_tokens.length == _swappers.length, "bad length");
        for (uint256 i = 0; i < _tokens.length; i++) {
            require(address(_swappers[i].BUSD()) == address(BUSD), "wrong BUSD");
            require(address(_swappers[i].token()) == address(_tokens[i]), "wrong TOKEN");
            swappers[_tokens[i]] = _swappers[i];
            IERC20(BUSD).approve(address(_swappers[i]), 2**256- 1);
            IERC20(_tokens[i]).approve(address(_swappers[i]), 2**256- 1);
        }
    }
    ...
    function getAndUnifyStables(uint256[5] memory amounts) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            if (amounts[i] > 0) {
                ...
                totalWantAmount = totalWantAmount.add(swappers[token[i]].toBUSD(amounts[i]));
            }
        }
    ...
    function sendStablesToUser(uint256 busdAmount, uint256[5] memory amounts, uint256 wantAmtTotal) internal {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            if (amount > 0) {
                uint256 swappedAmt = swappers[token[i]].fromBUSD(amount);
