...
contract PangolinYieldFarmManager is Ownable {
  ...
  // Router contract
  IPangolinRouter02 public router;
  // Vault contract
  IPangolinYieldFarmVault public immutable vault;
  ...
  // PANGOLIN AVAX USDC LP token
  IERC20 public immutable lpToken;
  // LP staking reward token contract
  IERC20 public immutable rewardToken;
  // Staking pool contract
  IMinichefV2 public immutable stakePool;
  // Pangolin Oracle contract
  IPangolinOracle public immutable pangolinOracle;
  ...
  event UpdateRouter(IPangolinRouter02 indexed router);
  ...
  /**
    ...
    * @param _lpToken Pangolin AVAX USDC LP token
    ...
    * @param _pangolinOracle Pangolin Oracle contract
  */
  constructor(
    IPangolinYieldFarmVault _vault,
    IERC20 _rewardToken,
    IERC20 _lpToken,
    ILendingPool _tokenALendingPool,
    ILendingPool _tokenBLendingPool,
    IPangolinRouter02 _router,
    IMinichefV2 _stakePool,
    uint256 _stakePoolId,
    IPangolinOracle _pangolinOracle
  ) {
    ...
    require(address(_pangolinOracle) != address(0), "Invalid address");
    ...
    pangolinOracle = _pangolinOracle;
  ...
  */
  function assetInfo() public view returns (uint256, uint256) {
    (uint256 tokenAAssetAmt, uint256 tokenBAssetAmt) = pangolinOracle.getLpTokenReserves(
      lpTokenAmt(),
      address(tokenA),
      address(tokenB),
      address(lpToken)
  ...
  */
  function compound() external {
    stakePool.harvest(stakePoolId, address(this));
  ...
  */
  function _addLiquidity() internal {
    (uint256 reserveA, uint256 reserveB) = pangolinOracle.getLpTokenReserves(
      lpToken.totalSupply(),
      address(tokenA),
      address(tokenB),
      address(lpToken)
  ...
  */
  function _unstake(uint256 _lpAmt) internal {
    stakePool.withdraw(stakePoolId, _lpAmt, address(this));
  }
  ...
  */
  function _swapForRepay(uint256 _repayTokenAAmt, uint256 _repayTokenBAmt) internal {
    ...
    swapAmountIn = pangolinOracle.getAmountsIn(
      swapAmountOut,
  ...
  */
  function updateRouter(IPangolinRouter02 _router) external onlyOwner {
    require(address(_router) != address(0), "Invalid address");
