/**
 *Submitted for verification at snowtrace.io on 2022-11-11
*/
  ...
  OwnableUpgradeable
{
    ...
    notZeroAddress(_msgSender())
  {
    ...
    if (project.publicsaleMaxPurchase > 0) {
      require(
        _publicsaleAmount[project.id][buyer] + amount <= project.publicsaleMaxPurchase,
        "AMOUNT_OVER_LIMITATION"
      );
    }
    uint256 remainPresale = project.presaleAmount - project.presaleSold;
    uint256 totalPublicSaleAmount = project.publicsaleAmount + remainPresale;
    require(
      project.publicsaleSold + amount <= totalPublicSaleAmount,
      "AMOUNT_INVALID"
    );
