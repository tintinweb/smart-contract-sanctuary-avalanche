...
contract YetiController is OwnableUpgradeable, IYetiController, CheckContract, LiquityBase {
    ...
    // Interest rate update variables
    // Address of collateral => interest rate per unit time (day by default)
    ...
    event BorrowingFeeFloorChanged(uint256 _newBorrowingFeeFloor);

    event InterestRateChanged(address _collateral, uint256 _newInterestRate);

    event InterestTimeWindowChanged(uint256 _newInterestTimeWindow);
    ...
    function addCollateral(
        ...
        troveManager.interestInitCollateral(_collateral);
    ...
    /** 
     * @notice Changes interest time window on trove manager to the new value. 
     */
    function changeInterestTimeWindow(uint256 _newInterestTimeWindow) external override {
        _requireThreeDayTimelock();
        require(_newInterestTimeWindow >= 86400, "longer than one day");
        troveManager.changeInterestTimeWindow(_newInterestTimeWindow);
        emit InterestTimeWindowChanged(_newInterestTimeWindow);
    }

    function setTroveManager(address _tm) external override {
        _requireThreeDayTimelock();
        // Can only call once
    ...
    function setInterestRates(address[] calldata _colls, uint256[] calldata _newRates) external override {
        _requireThreeDayTimelock();
        uint256 collsLen = _colls.length;
        _requireInputLengthChange(collsLen == _newRates.length);
        troveManager.tickInterest();
        for (uint256 i; i < collsLen; ++i) {
            require(_newRates[i] <= 1e17, "Reasonable new interest");
            interestRates[_colls[i]] = _newRates[i];
            emit InterestRateChanged(_colls[i], _newRates[i]);
        }
    }

    function setBorrowingFeeFloor(uint256 _newBorrowingFeeFloor) external override {
        _requireThreeDayTimelock();
        require(_newBorrowingFeeFloor <= 1e17, "Reasonable new borrowing fee floor");
        borrowingFeeFloor = _newBorrowingFeeFloor;
        emit BorrowingFeeFloorChanged(_newBorrowingFeeFloor);
    }
    ...
    function getInterestRates(address[] calldata _colls) external view override returns (uint256[] memory rates) { 
        uint256 collsLen = _colls.length;
        rates = new uint256[](collsLen);
        for (uint256 i; i < collsLen; ++i) {
            rates[i] = interestRates[_colls[i]];
        }
    }
