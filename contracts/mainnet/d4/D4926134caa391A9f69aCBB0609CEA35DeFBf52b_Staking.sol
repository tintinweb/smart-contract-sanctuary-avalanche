...
contract Staking is Ownable {
	...
	constructor(address _token, address _sToken, address _minter, uint256 _rewardsRate) {
		...
		require(_minter != address(0), "Staking: Minter cannot be address zero");
		minter = _minter;
	...
	function mintRewards() private {
		if (rewardsRate > 0) {
			IMinter(minter).mintRewards(
				0, IERC20(token).totalSupply() * rewardsRate / 1000000
			);
			adjust();
		}
	}
	...
	function setRewardsRate(uint256 _rate) external onlyOwner {
	...
	function setEpochDuration(uint32 dur) external onlyOwner {
		epoch.duration = dur;
	}
}
