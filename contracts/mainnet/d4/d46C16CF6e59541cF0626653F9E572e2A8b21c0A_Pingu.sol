...
contract Pingu is Context, IERC20, Ownable {
    ...
    string private _name = "PenguInu";
    string private _symbol = "PINGU";
    uint8 private _decimals = 9;
    bool public market_active;
    mapping (address => bool) public premarket_user;

    address payable public marketingWalletAddress = payable(0x43193AFE9924F5E700A656A33c4f8252A797724b);
    address payable public devWalletAddress = payable(0x852FeC08A47aaF28efe6E16401327Cd8ee0a5a71);
    address payable public teamWalletAddress =  payable(0x1c8bb6281f2384F2930fC9BCe186e75Ef0956781);
    address payable public uWalletAddress =  payable(0x8e3c4021838c896C7955cC94DEEC35e7F4524c3b);
    ...
    uint256 _marketingShare = 55;
    uint256 _devShare = 10;
    ...
    constructor () {
        IDEXRouter _uniswapV2Router = IDEXRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4); // avax 
        uniswapPair = IDEXFactory(_uniswapV2Router.factory())
            .createPair(address(this),0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);  // wavax
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyDevFee).add(_buyUFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellDevFee).add(_sellUFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_devShare).add(_teamShare).add(_uShare);
    ...
    function setSelTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newDevTax, uint256 newTeamFee) external onlyOwner() {
        ...
        _sellUFee = newTeamFee;

        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellDevFee).add(_sellUFee);
    }
    
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newDevShare, uint256 newTeamShare, uint256 newUShare) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBDev).sub(amountBNBTeam).sub(amountBNBu);
        ...
        if(amountBNBu > 0)
            transferToAddressETH(uWalletAddress, amountBNBTeam);
