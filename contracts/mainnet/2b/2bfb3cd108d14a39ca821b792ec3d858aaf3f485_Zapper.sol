 ...
 * a SLOT/ETH or STOMB/ETH liquidity pair. And breaks a liquidity pair to single assets
 *
 */
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
	...
	constructor(
		...
		IERC20(_STOMB).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_SLOT).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_ETH).safeApprove(address(ROUTER), type(uint256).max);

		//approve to breakLP
		IERC20(_STOMBAVAX).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_SLOTAVAX).safeApprove(address(ROUTER), type(uint256).max);

		// set route pairs for our tokens
		// routePairAddresses[_SLOT] = _WAVAX;
		routePairAddresses[_STOMB] = _ETH;
	}
	...
	function zapTokenToLP(
		...
		// SLOT, STOMB, ETH to create SLOT-ETH or STOMB-ETH will hit this if
	...
	function _swapAVAXToLP(
		...
		if (token0 != WAVAX && token1 != WAVAX) {
			...
			ROUTER.addLiquidityETH{value: token0 == WAVAX ? swapValue : amount.sub(swapValue)}(
				other, 
	 ...
	 * Not very useful for our current use case as bolt input currencies will only be ETH, WAVAX, STOMB, SLOT
	...
	function _swap(
		...
		if (intermediate != address(0) && (_from == WAVAX || _to == WAVAX)) {
			// E.g. [WAVAX, ETH, SLOT/STOMB] or [SLOT/STOMB, ETH, WAVAX]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// E.g. [ETH, SLOT/STOMB] or [SLOT/STOMB, ETH]
			path = new address[](2);
			path[0] = _from;
			path[1] = _to;
		} else if (
			...
			// E.g. [STOMB, ETH, SLOT] or [SLOT, ETH, STOMB]
			path = new address[](3);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = _to;
			...
			// [SLOT/SLOT, ETH, WAVAX, xRoute, xToken]
			path = new address[](5);
			path[0] = _from;
			...
			// E.g. [SLOT/STOMB, ETH, WAVAX, xTokenWithWAVAXLiquidity]
			path = new address[](4);
			path[0] = _from;
			path[1] = intermediate;
			path[2] = WAVAX;
			...
			// E.g. [xTokenWithWAVAXLiquidity, WAVAX, ETH, SLOT/STOMB]
			path = new address[](4);
			path[0] = _from;
			path[1] = WAVAX;
			path[2] = intermediate;
