...
contract CastleManager is Ownable, AidOwnable, IERC721, IERC721Metadata {
    ...
    uint256 public nodeChessPrice = 20;
    ...
    function updatenodeChessPrice (uint256 _nodeChessPrice) external onlyOwner {
        nodeChessPrice = _nodeChessPrice;
    }
    ...
    function setChess(IERC20 _CHESS) external onlyOwner {
        CHESSES = IERC20(_CHESS);
    }
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) public {
        ...
        uint256 nodePrice = nodeChessPrice * 10 ** 18;
        require(nodePrice > 0, "error");
        require(CHESSES.balanceOf(sender) >= nodePrice, "AID: Chess balance too low for creation.");
        ...
        CHESSES.transferFrom(_msgSender(), address(manager),  nodePrice);
