...
interface IERC1155Receiver is IERC165 {
     ...
     * @param values An array containing amounts of each token being transferred (order and length must match ids array)     * @param data Additional data with no specified format
...
// File contracts/interfaces/INodeERC1155.sol
...
// File contracts/interfaces/ICorkToken.sol
...
// File contracts/interfaces/ISwapCork.sol
...
contract NodeERC1155 is INodeERC1155, ERC1155, Ownable, ReentrancyGuard {
    ...
    bool public isTradeActive = true;
    ...
    uint256 public claimFeePercentage = 10;
    ...
    function mint(uint256 _nodeType, string calldata _uri) public {
        ...
        if(isPresaleActive){
            require(
                isPresaleActive && corkToken.balanceOf(msg.sender) >= collection[_nodeType].price / 2,
                "receiver's balance is less than node price"
            );
        } else {
            require(
                !isPresaleActive && corkToken.balanceOf(msg.sender) >= collection[_nodeType].price,
                "receiver's balance is less than node price"
            );
        }
    ...
    function mintTo(address to, uint256 _nodeType, uint amount, string calldata _uri) public onlyOwner{
        for (uint i = 0; i < amount; i++){
            ...
            require(
                getOwnedNodeCountByType(to, _nodeType) <
                    collection[_nodeType].purchaseLimit,
                "minted nodes exceed amount limit"
            );
            uint256 _id = _getNextTokenID();
    ...
    function bailOutMint(
        ...
        if (wastedCork <= snowballRewardCork) {
            if (wastedCork < snowballRewardCork)
                nodeState[id].remainClaimedAmounts =
                    snowballRewardCork -
                    wastedCork;
        }
    ...
    function claimPartial(uint256 amountMax, uint256 id) external payable nonReentrant {
        require(ownedNodes[msg.sender].length > 0, "No have a node");
        require(nodeState[id].purchaser == msg.sender, "only puchaser can claim");
    ...
    function calculateMainAmount(
        ...
        if (_lastedMainDays > _roiTime) {
            ...
            return ((_firstRun * _roiTime) + lastedTrueYieldDays * _trueYield) + (_lastClaimDate - lastedTrueYieldDays) * _firstRun; // ROI + true yield
        } else {
            return _lastClaimDate * _firstRun;
        }
    }
