...
contract AVAXYOLO {
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(5, 220)); // 22% per day for 5 days
        plans.push(Plan(10, 130)); // 13% per day for 10 days
        plans.push(Plan(15, 100)); // 10% per day for 15 days
		plans.push(Plan(5, 185)); // 18.5% per day for 5 days (at the end, compounding)
        plans.push(Plan(10, 120)); // 12% per day for 10 days (at the end, compounding)
        plans.push(Plan(15, 100)); // 10% per day for 15 days (at the end, compounding)
	}
	...
	function getInfo() public view returns (address, address) {
        return (commissionWallet, developerWallet);
    }
