...
contract Lottery is Ownable, Pausable {
    ...
    address public tokenAddress = 0xcF2A2A5C47595E1f78C7e314A590D0d5895FF952;
    IERC20 token;

    address public marketingAddress = 0x73566f6F41D93E479f8aDAA364b096fAF3B0781f;
    ...
    function enter(uint256 tickets) external whenNotPaused {
        ...
        token.transferFrom(msg.sender, address(this), tickets * 10**18);
    }
    ...
    function drawLottery(uint256[] memory _randomWords) public onlyOwner {
        ...
        if (players.length > 0) {
            ...
            uint256 marketingAmount = totalAmount - winnerAmount * numWords;
            
            for (uint256 index; index < numWords; index++) {
                uint256 i = _randomWords[index] % players.length;
                lotteryWinners[lotteryID][i] = players[i];
    ...
    function setNumWords(uint32 _numWords) public onlyOwner {
        require(_numWords != 0, "can't be 0!");
        numWords = _numWords;
        winnerShare = winnerShare / numWords;
    }

    function setMaxTicketsPerPlayer(uint256 _maxTicketsPerPlayer) public onlyOwner {
        require(_maxTicketsPerPlayer != 0, "can't be 0!");
