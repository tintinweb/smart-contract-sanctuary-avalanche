...
abstract contract NebulaProtoStarDrop is Context{
	...
	function giveStructVar(address _account)external view returns(uint256,uint256,uint256){
		return (airdrop[_account].dropped,airdrop[_account].claimed,airdrop[_account].transfered);
	}
}
...
contract dropget{
	...
	function getStructVar(address _account)public view returns(uint256,uint256,uint256){
			return drops.giveStructVar(_account);
	}
}