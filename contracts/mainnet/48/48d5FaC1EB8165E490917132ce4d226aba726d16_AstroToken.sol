...
contract AstroToken is ERC20Detailed {
    ...
    uint256 public nextRebase = 1651579200; //block.timestamp + 604800;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 1 * 10**6 * 10**DECIMALS; //10**9
    ...
    */
    ...
    constructor(address _router, address _usdcAddress) ERC20Detailed("100 Days Ventures", "betaASTRO", uint8(DECIMALS)) {
        owner = msg.sender;
        TOTAL_GONS = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
        router = IDEXRouter(_router);
        usdcAddress = IERC20(_usdcAddress);

        releaseBlock = 1651579200; //block.timestamp;
