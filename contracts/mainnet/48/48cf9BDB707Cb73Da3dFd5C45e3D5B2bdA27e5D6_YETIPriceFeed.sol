...
contract YETIPriceFeed is IPriceFeed {
    using UQ112x112 for uint224;
    using SafeMathJoe for uint256;
    IPriceFeed public AVAXPriceFeed;
    JoePair public YETIAVAX;
    address public YETIToken;
    constructor(address LPToken, address _YETIToken, address _AVAXPriceFeed) public {
        YETIAVAX = JoePair(LPToken);
        YETIToken = _YETIToken;
        AVAXPriceFeed = IPriceFeed(_AVAXPriceFeed);
    }

    function fetchPrice_v() view external override returns (uint256) {
        address t0 = YETIAVAX.token0();
        address t1 = YETIAVAX.token1();
        (uint112 r0, uint112 r1, ) = YETIAVAX.getReserves();
        uint256 AVAXPrice = AVAXPriceFeed.fetchPrice_v();
        if (t0 == YETIToken) {
            return uint256(r1).mul(10 ** 36).div(uint256(r0)).div(AVAXPrice);
        } else {
             return uint256(r0).mul(10 ** 36).div(uint256(r1)).div(AVAXPrice);
        }
