...
contract nebulaFeeManager is Ownable{
    ...
    function payFeeToken(uint256 _intervals) payable external {
	    ...
	    feeTok.transferFrom(msg.sender, address(this), tokFee);
	    payable(msg.sender).transfer(tokFee);
    ...
    function FeeToken(uint256 _intervals) payable external {
	    ...
	    for(uint i = 0;i<_intervals;i++) {
	    	if(_intervals > 0 && returnBalance >= tokFee){
	    		feeTok.approve(treasury, tokFee);
	    		feeTok.approve(address(this), tokFee);
	    		feeTok.transferFrom(address(this),treasury,tokFee);
