  ...
  Authorizable
{
 ...
 uint256 low_perc = 30;
  ...
  function getStablePrice() internal view returns(uint256)
   {
  ...
  function adds(address pair_add, bool tok) internal view returns(address) {
  ...
  function decs(address pair_add) internal view returns(uint256) {
         IERC20 swap_token = IERC20(pair_add);
         uint256 _dec =  swap_token.decimals();
         return _dec;
     }
  function find_dir(address ad)internal view returns(uint[4] memory) {
