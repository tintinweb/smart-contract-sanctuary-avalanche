...
contract MadVeggies is
    ...
    uint256 public mintedWithJuice;
    uint256 public JuiceLimit = 100;
    ...
    address link_token_contract = 0x5947BB275c521040051D82396192181b413227A3;
    address private _owner;
    ...
    function setIndexer(uint256 _new) external onlyOwner {
      ...
      for (uint i = 0; i < amount; i++) {
          _mint(msg.sender, enoughRandom());
          _setTokenRoyalty(enoughRandom(), _owner, contractRoyalties);
          minter[enoughRandom()] = msg.sender;
          lastDividendAt[enoughRandom()] = totalDividend;
        ...
        function mintWithjuice(uint amount) public {
            ...
            require(mintedWithJuice <= JuiceLimit, "max amount to be minted with $JUICE has been reached");
    ...
    function airdrop(address to, uint amount) external onlyOwner {
        ...
        require(_tokenIdTracker.current() + amount <= _maxSupply, "Not enough avax mad veggies left to mint amount.");     
         for(uint i=0; i < amount; i++){
         _mint(to , enoughRandom());
         _setTokenRoyalty(enoughRandom(), to, contractRoyalties);
         minter[enoughRandom()] = to;
         lastDividendAt[enoughRandom()] = totalDividend;
         _tokenIdTracker.increment();
        }
    }
    ...
    function setJuicelimit(uint256 _newLimit) public onlyOwner {
        JuiceLimit = _newLimit;
    }
