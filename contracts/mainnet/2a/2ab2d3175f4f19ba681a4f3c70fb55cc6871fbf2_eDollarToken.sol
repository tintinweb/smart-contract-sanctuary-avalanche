...
contract eDollarToken is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 eDOLLAR-WAVAX LP -> eDollar pool
    uint256 public constant INITIAL_eDOLLAR_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the eDollar ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("eDollar Token", "eDOLLAR") {
        // Mints 1 eDOLLAR to contract creator for initial pool setup
    ...
    function _geteDollarPrice() internal view returns (uint256 _eDollarPrice) {
        try IOracle(eDollarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("eDollar: failed to fetch eDollar price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _eDollarPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_eDollarPrice >= taxTiersTwaps[tierId]) {
    ...
    function seteDollarOracle(address _eDollarOracle) public onlyOperatorOrTaxOffice {
        require(_eDollarOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints eDOLLAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of eDOLLAR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currenteDollarPrice = _geteDollarPrice();
            currentTaxRate = _updateTaxRate(currenteDollarPrice);
            if (currenteDollarPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _eDollarPool
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_eDollarPool != address(0), "!_eDollarPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_eDollarPool, INITIAL_eDOLLAR_POOL_DISTRIBUTION);
    }
