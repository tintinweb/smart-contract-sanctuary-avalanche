...
// File contracts/StakingDoubleRewardsSuper.sol
...
contract StakingDoubleRewardsSuper is Ownable {
    ...
    function stake(uint256 amount) external onlyApprovedContractOrEOA updateReward(msg.sender) {
        ...
        if (newShares > 0) {
            ...
            if (isSuper) {
                for (uint256 i; i < rewardSuperTokens.length; ++i) {
    ...
    function _claimRewards() internal {
        ...
        if (updateAndClaim && totalShares > 0) {
            ...
            uint256[] memory superBalancesBefore;
            uint256[] memory balanceDiffSuper;
            ...
            if (isSuper) {
                for (uint256 i; i < rewardSuperTokens.length; ++i) {
                    balanceDiffSuper[i] = _checkBalance(rewardSuperTokens[i]) - superBalancesBefore[i];
                    if (balanceDiffSuper[i] > 0) {
                        increaseRewardTokensPerShare(rewardSuperTokens[i], (balanceDiffSuper[i] * ACC_PRECISION) / totalShares);
                    }
                }                
            }  
        }
    ...
    function pendingRewards(address user) public view returns (uint256, uint256, uint256[] memory) {
        if(isSuper){
            return (earned(user), pendingRewardPng(user), pendingRewardSuper(user));
        }else{
            return (earned(user), pendingRewardPng(user), pendingRewardSuper(user));
        }
    }

    function pendingRewardSuper(address user) public view returns (uint256[] memory) {
        (, uint256[] memory pendingSuper) = rewarder.pendingTokens(0, user, pendingRewardPng(user));
        return pendingSuper;
    }
