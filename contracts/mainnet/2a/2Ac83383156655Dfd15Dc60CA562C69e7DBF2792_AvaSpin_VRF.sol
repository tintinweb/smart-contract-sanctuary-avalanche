...
contract AvaSpin_VRF is Ownable, VRFConsumerBaseV2 {
    ...
    function fulfillRandomWords(
        ...
        s_randomWords[requestId] = (randomWords[0] % 40);
    }
	...
	uint spin = s_randomWords[requestID[msg.sender]];
        ...
        if (selectedMultiplier[msg.sender] == 2 && spin < 18) {
        ...
        } else if (selectedMultiplier[msg.sender] == 3 && spin >= 18 && spin < 28) {
            uint pay = realPlayed * 3;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 5 && spin >= 28 && spin < 34) {
            uint pay = realPlayed * 5;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 10 && spin >= 34 && spin < 37) {
            uint pay = realPlayed * 10;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 20 && spin >= 37 && spin < 39) {
            uint pay = realPlayed * 20;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 25 && spin == 39) {
