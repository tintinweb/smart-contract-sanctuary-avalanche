...
import "./Token.sol";
import "./contractByMiph.sol";

// lol hi feel free to read and learn from my bad/copied code 
// check my twitter too @miphguy


contract pcatstaking is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable {
    ...
    uint public RATE_PER_DAY = 20 * 10**18;
    uint public MAX_CAP = 200 * 10**18;
    string public baseURI = "ipfs://bafybeig42nwbjik5usozt3p7ebc6n6vksqs6nqas43awqxwvjmkqleszvy/";
    ...
    constructor(address _pixcat, address _token) ERC721("Staked PixCat", "SPCAT") {
        pixcat = pixCat(_pixcat);
        token = red(_token);
    }
    ...
    {
        require (user == pixcat.ownerOf(tokenId), "User must be owner.");
        pixcat.transferFrom(user, address(this), tokenId);
    ...
    function stakeAll() external whenNotPaused{
        depositSelected(pixcat.tokenIdsOfUser(msg.sender));
    }

    function depositSelected(uint256[] memory tokenIds) public whenNotPaused {
        uint length = tokenIds.length;
        require(balanceOf(msg.sender) + length < 999, "Cannot stake more.");
    ...
    function withdraw(uint256 tokenId, address user) internal
    {
        ...
        claim(user, tokenId);
        pixcat.transferFrom(address(this), user, tokenId);
        burn(tokenId);
        emit Withdraw(user, tokenId);
    }


    function withdrawSelected(uint256[] memory tokenIds) public whenNotPaused {
        uint length = tokenIds.length;
        for (uint i = 0; i < length; i++){
    ...
    function withdrawAll() public whenNotPaused{
        withdrawSelected(tokenIdsOfUser(msg.sender));
    }
...
contract red is ERC20, ERC20Burnable, Ownable, Authorizable {
...
contract pixCat is ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, ClampedRandomizer{
  uint256 private _price;
  uint256 private _wlPrice;
  uint256 private _maxMintable = 555;
  ...
  constructor() ERC721("PixCat", "PCAT") ClampedRandomizer(_maxMintable){
    _customBaseURI = 'ipfs://bafybeiarama4riues5ap4quexldmlpph4ttsv53qvaagp4hjzdz3ni66ye/';
    _price = 200 * 1000000000000000;  //price in milli 
    _wlPrice = 150 * 1000000000000000;  //price in milli 
    _royaltyAmount = 500;
    _maxMintPerTx = 40;
    maxPerWallet = 555;
    paused = true;
    payoutAddress1 = 0x8654FE925701a6cb66C1cBA2786eF4d3a68ad6fE;
    payoutAddress2 = 0x56101AFA0E51cC3CC7E006539598A73Ad5c581d3;
    royaltyAddress = 0x56101AFA0E51cC3CC7E006539598A73Ad5c581d3;
  }

  mapping(address => uint256) publicMints;
  mapping(address => uint256) whitelistMints;
  function addUser(address[] calldata _addressToWhitelist, uint256 amount) external onlyOwner {
    for(uint256 i=0;i< _addressToWhitelist.length ;i++){
    whitelistMints[_addressToWhitelist[i]] = amount;
  }
  }
  function wlMintsAllowed(address addy) public view returns (uint){
