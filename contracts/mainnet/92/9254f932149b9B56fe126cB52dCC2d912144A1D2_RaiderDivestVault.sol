 ...
 */
contract RaiderDivestVault is ADivestVault {
    ...
    )
  {
    ...
    uint256 userLPTokens = cumulativeDeposits[user] -
      cumulativeWithdrawals[user];
    (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();

    uint256 ts = pair.totalSupply();

    uint256 userToken0Balance = (reserve0.mul(userLPTokens)).div(ts).div(2);
    uint256 userToken1Balance = (reserve1.mul(userLPTokens)).div(ts).div(2);

    return (userLPTokens, userToken0Balance, userToken1Balance);
  }
}
