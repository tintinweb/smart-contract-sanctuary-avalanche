...
contract Membership is Context {
    ...
    event Renew(uint256 indexed member, uint256 indexed plan, uint256 renewed);
    ...
    function _afterMemberBuySubscribe(
        ...
        if(subs.length > 0) {
            for(uint32 subid = 0;  subid < subs.length; subid++) {
                ...
                if(plan == subPlan) {
                     _updateSubscription(member, plan, subs[subid], subEnd);
                } else {
                    _newSubscription(service, plan, member);
                }
            } 
        } else {
            _newSubscription(service, plan, member);
    ...
    function _newSubscription(
        ...
        uint32 newSub = _createSubscriptionStructure(plan);
        _linkActiveSub(service, member, newSub);

        emit Subscribe(member, plan, getTimestamp());
    }
    ...
    function _updateSubscription(
        ...
        uint256 planDuration = getPlanDuration(plan);
        subscriptions[subscription].start = getTimestamp();
        subscriptions[subscription].end = getTimestamp() <= end ? end : getTimestamp() + (planDuration * 1 days);

        emit Renew(member, plan, getTimestamp());
    }
    ...
    function _createSubscriptionStructure(
        ...
        uint256 end = getTimestamp() + (getPlanDuration(plan) * 1 days);
        subscriptions[newId] = Subscriptions(plan, getTimestamp(), end);
        subscriptionsID++;
    ...
    {
        uint32[] memory servicesInGuild = getGuildsServices(guild);
        if(servicesInGuild.length > 0) {
            for(uint32 sid = 0; sid < servicesInGuild.length; sid++) {
                _deleteService(guild, servicesInGuild[sid]);
            }
        }
        _deleteGuildStructure(guild);
    }
    ...
    function _deleteService(
        ...
        if(plansInService.length > 0) {
            for(uint32 pid = 0; pid < plansInService.length; pid++) {
                _deletePlan(plansInService[pid]);
            }
        }
        _deleteLinkedPlans(service);
            ...
            for(uint32 mid = 0; mid < membersFromGuildService.length; mid++) {
                if(_getMemberState(membersFromGuildService[mid])) {
                    ...
                    if(membersSubscriptions.length > 0) {
                        ...
                        _deleteLinkedActiveSubs(service, membersFromGuildService[mid]);
                    }
                }
            }
            _deleteLinkedGuildMembers(service);
            _deleteLinkedMemberAccounts(guild, service);
        }

        uint32[] memory servicesInGuild = getGuildsServices(guild);
        if(servicesInGuild.length > 0) {
            for(uint32 sid = 0; sid < servicesInGuild.length; sid++) {
                if(servicesInGuild[sid] == service) {
                    _deleteLinkedService(guild, sid);
                }
            }
        }
        ...
        if(eventsInService.length > 0) {
            for(uint32 eid = 0; eid < eventsInService.length; eid++) {
                if(eventsInService[eid] != 0 && _getEventState(eventsInService[eid])) {
                    _deleteEventStructure(eventsInService[eid]);
                    _deleteEventsAttendies(service, eventsInService[eid]);
                } 
            }
        }
        _deleteEventsFromService(service);

        _deleteServiceStructure(service);
    }
    ...
    function _deleteLinkedServices(
        uint32 guild
    ) private {
        delete guildServices[guild]; 
    }

    function _deleteLinkedService(
        uint32 guild,
        uint32 row
    ) private {
        delete guildServices[guild][row]; 
    }
    ...
    function _deleteGuildStructure(
        uint32 guild
    ) private {
        _setGuildState(guild, false);
        delete guilds[guild];
    }

    function _deleteServiceStructure(
        uint32 service
    ) private {
        _setServiceState(service, false);
