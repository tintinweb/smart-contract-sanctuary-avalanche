...
contract SmartFarmooor is ISmartFarmooor, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable, Rescuable, Modifier, RoleManagement {
    ...
    function initialize(string memory _name, string memory _symbol, address _feeManager, address _baseToken, uint256 _minHarvestThreshold, uint16 _performanceFee, uint256 _cap, address _manager, address _admin, uint256 _minAmount) public initializer {
        ...
        // _setAutomationRules(_automationRules);
    }
     ...
     */
    function removeModule(uint _moduleId) external onlyRole(DEFAULT_ADMIN_ROLE) whenPaused onlyIfEmptyModule {
     ...
     */
    function setModuleAllocation(uint[] memory _allocation) external onlyRoleOrOpenRole(MANAGER_ROLE) whenPaused onlyIfEmptyModule {
     ...
     */
    function panic() external payable onlyRoleOrOpenRole(PANICOOOR_ROLE) {
        _pause();
     ...
     */
    function finishPanic() external onlyRoleOrOpenRole(MANAGER_ROLE) whenPaused {
     ...
     */
    function setMinHarvestThreshold(uint256 newMinHarvestThreshold) external onlyRoleOrOpenRole(MANAGER_ROLE) {
