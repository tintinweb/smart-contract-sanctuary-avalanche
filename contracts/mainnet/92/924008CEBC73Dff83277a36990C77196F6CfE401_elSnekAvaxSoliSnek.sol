...
// File contracts/elSnekAvaxSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract elSnekAvaxSoliSnek  is Ownable, Pausable {
    ...
    address public constant elsnek = address(0x63D73d848b7F5142F11974188E9Fe3F5dE28f88c);
    address public want;
    ...
    ISoliSnekRouter.Routes[] public snekToElsnekPath;
    ...
    constructor(
        ...
        ISoliSnekRouter.Routes[] memory _snekToElsnekPath,
        ...
        for (uint i; i < _snekToElsnekPath.length; ++i) {
            snekToElsnekPath.push(_snekToElsnekPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToElsnekPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(elsnek).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wavax, elsnek, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(elsnek).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(elsnek).safeApprove(router, type(uint).max);
    }
