...
contract BT is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0xE54F4a39e326D910fbd81069a7B58D3f439cdD54;
    ...
    // Min amount to liquify. (default 1 SPECTA)
    ...
    // The operator can call all functions except of minting SpectaToken
    address private _operator;

    // Max holding rate in basis point. (5% default)
    uint16 public maxHoldingRate = 500;
    // Address that are identified as botters with holding of more than 1%.
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "SPECTA::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    modifier blackList(address sender, address recipent) {
        if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
            require(_includeToBlackList[sender] == false,"SPECTA::blackList: You have been blacklisted as a bot (SENDER)");
            require(_includeToBlackList[recipent] == false,"SPECTA::blackList: You have been blacklisted as a bot (RECIPENT)");
        }
        _;
    }

    /**
     * @notice Constructs the SpectaToken contract.
     */
    constructor() public BEP20("BT", "BT") {
    ...
    /// @dev overrides transfer function to meet tokenomics of SPECTA
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) blackList(sender, recipient) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(amount == sendAmount + taxAmount, "SPECTA::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "SPECTA::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "SPECTA::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "SPECTA::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SPECTA::transferOperator: new operator is the zero address");
