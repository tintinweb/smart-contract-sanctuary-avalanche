...
contract Parmesan is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  string private baseURI = "https://cheesyverse.s3.filebase.com/parmesan-json/";
  string private uriSuffix = ".json";

  event MakingParmesan(
      uint256 amount,
      address indexed caller
  );

  constructor() ERC721("Parmesan", "PARM") {
  ...
  function mint(address to, uint amount) internal {
    ...
    function claimCheese() public nonReentrant {
        ...
        require(supply.current() < 5000, "All the Parmesan has been minted!");
  ...
  function claimableView(address account) public view returns(uint) {
    ...
    function cheeseTime(address account) public view returns(uint) {
        if (fromagerLevel[account] >= LEVEL_REQ) {
            ...
            return PARM_PRODUCE_TIME - (timeModifier * MODIFIER);
        }
        
        return PARM_PRODUCE_TIME;
  ...
  function makeCheese(uint _amount) public nonReentrant {
        ...
        emit MakingParmesan(_amount, msg.sender);
    }
