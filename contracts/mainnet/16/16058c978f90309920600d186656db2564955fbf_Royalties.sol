...
contract Royalties is Ownable {
    ...
    event CommunityClaimed(address owner, uint256 tokenID);
    ...
    function claimCommunity(address collectionAddress, uint256 tokenID) public {
        emit CommunityClaimed(collectionAddress, tokenID);                
    }
    ...
    function claimCommunityBatch(
        ...
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            claimCommunity(collectionAddress, tokenIDs[i]);
        }
    }
