...
// File contracts/interfaces/ISoliSnekGauge.sol



pragma solidity 0.8.17;

interface ISoliSnekGauge {
...
// File contracts/interfaces/ISoliSnekRouter.sol



pragma solidity 0.8.17;

interface ISoliSnekRouter {
...
// File contracts/fBombwEtheSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract fBombwEtheSoliSnek  is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wavax = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
    address public constant snek = address(0xeeee99b35Eb6aF5E7d76dd846DbE4bcc0c60cA1d);
    address public constant fbomb = address(0x5C09A9cE08C4B332Ef1CC5f7caDB1158C32767Ce);
    address public constant wethe = address(0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB);
    address public want;
    ...
    //Routes
    ISoliSnekRouter.Routes[] public snekToAvaxPath;
    ISoliSnekRouter.Routes[] public snekToFbombPath;
    ISoliSnekRouter.Routes[] public snekToEthPath;
    ISoliSnekRouter.Routes[] public feeTokenPath;
    ISoliSnekRouter.Routes[] public customPath;
    ...
    constructor(
        ...
        address _feeToken,
        ISoliSnekRouter.Routes[] memory _snekToAvaxPath,
        ISoliSnekRouter.Routes[] memory _snekToFbombPath,
        ISoliSnekRouter.Routes[] memory _snekToEthPath,
        ISoliSnekRouter.Routes[] memory _feeTokenPath
    ) {
        ...
        for (uint i; i < _snekToAvaxPath.length; ++i) {
            snekToAvaxPath.push(_snekToAvaxPath[i]);
        }

        for (uint i; i < _snekToFbombPath.length; ++i) {
            snekToFbombPath.push(_snekToFbombPath[i]);
        }

        for (uint i; i < _snekToEthPath.length; ++i) {
            snekToEthPath.push(_snekToEthPath[i]);
        }
    ...
    function _deposit() internal whenNotPaused {
        ...
        ISoliSnekGauge(gauge).deposit(wantBal, 0);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISoliSnekGauge(gauge).withdraw(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        ISoliSnekGauge(gauge).getReward(address(this), rewardTokens);
        uint256 outputBal = IERC20(snek).balanceOf(address(this));
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(snek).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        if(feeToken != snek){
        ISoliSnekRouter(router).swapExactTokensForTokens(toFee, 0, feeTokenPath, address(this), block.timestamp);
        }
    ...
    function addLiquidity() internal {
        uint256 snekHalf = IERC20(snek).balanceOf(address(this)) / 2;

        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToEthPath, address(this), block.timestamp);
        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToFbombPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wethe).balanceOf(address(this));
        uint256 t2Bal = IERC20(fbomb).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wethe, fbomb, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 wrappedOut;
        if (outputBal > 0) {
            (wrappedOut,) = ISoliSnekRouter(router).getAmountOut(outputBal, snek, wavax);
        }
    ...
    function rewardBalance() public view returns (uint256) {
        return ISoliSnekGauge(gauge).earned(snek, address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        return ISoliSnekGauge(gauge).balanceOf(address(this));
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        _harvest(msg.sender);
        ISoliSnekGauge(gauge).withdraw(balanceOfPool());
    ...
    function makeCustomTxn(address [][] memory _tokens, bool[] calldata _stable) external onlyAdmin {
        for (uint i; i < _tokens.length; ++i) {
            customPath.push(ISoliSnekRouter.Routes({
                from: _tokens[i][0],
                to: _tokens[i][1],
                stable: _stable[i]
            }));
        ...
        ISoliSnekRouter(router).swapExactTokensForTokens(bal, 0, customPath, address(this), block.timestamp + 600);
   ...
   function setFeeToken(address _feeToken, ISoliSnekRouter.Routes[] memory _feeTokenPath) external onlyAdmin {
    feeToken = _feeToken;
    delete feeTokenPath;
