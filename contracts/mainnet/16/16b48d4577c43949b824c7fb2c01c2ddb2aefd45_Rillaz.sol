...
// Rillaz by xrpant - Modified from Campfire's Leopard Queens contract
...
contract Rillaz is ERC721, Ownable, ReentrancyGuard, KindaRandom {
    ...
    bool private _apeListOnly;

    mapping (address => uint8) addressMinted;
    mapping (address => uint8) apeList;

    IERC721 _apes;
    
    constructor() ERC721("Rillaz", "RILLA") KindaRandom(50) {
        _customBaseURI = "ipfs://bafybeifhlfdr2afeykmxoj3qzo2i4b6x7cybufxurhr3gbc6zrvjjsufsy/";
    ...
    function flipApeListOnly() public onlyOwner {
        _apeListOnly = !_apeListOnly;
    }
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        if (msg.sender != owner()) {
            ...
            require(_apes.balanceOf(msg.sender) > 0, "You are not an Ape!");

            if (_apeListOnly) {
                require(apeList[msg.sender] == 1, "You aren't ApeListed!");
            }
        }
    ...
    function addApeList(address[] calldata _addresses) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            apeList[_addresses[i]] = 1;
        }
    }
