...
/**
 *Submitted for verification at AVAXgonscan.com on 2021-08-30
*/
...
contract AVAXBridge is Ownable{
   ...
   event BridgedAVAXtoBSC ( uint256 _request, address indexed _user, uint256 _amount, uint256 _blocknumber );
    ...
    function BridgeMMAC ( uint256 _mmac ) public  payable  {
       ...
       deposits += _mmac;
       bridgeRequests[AVAXtoBSCBridgeRequestCount].user =  msg.sender;
       bridgeRequests[AVAXtoBSCBridgeRequestCount].balance = _mmac;
       bridgeRequests[AVAXtoBSCBridgeRequestCount].blockNumber = block.number;
       
       emit BridgedAVAXtoBSC ( AVAXtoBSCBridgeRequestCount, msg.sender, _mmac, block.number );
    ...
    function transferBridgedMMAC ( uint256 bridgeRequest, address _address, uint256 _amount ) public onlyBridge {
        ...
        bscBridgeRequests[ BSCtoAVAXBridgeRequestCount ] = true;
        IERC20 _token = IERC20 ( MMACADDRESS ) ;
        _token.transfer ( _address , _amount );
        deposits -= _amount;
        emit BridgedBSCtoAVAX ( BSCtoAVAXBridgeRequestCount, _address, _amount, block.number );
    }
