...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    function claimAlocation() external onlyBloomReferralNode {
        ...
        uint256 TVL = 550000000000;

        uint256 Stables = 65000000000;

        uint256 getPercentage = (Stables / TVL) * 100;
        ...
        if (movedBloomBoxes[msg.sender].length > 0) {
            for (uint256 i = 0; i < movedBloomBoxes[msg.sender].length; i++) {
                ...
                userAlocation += bloomifyMove.instantAmount;
        ...
        uint256 calc = alocationCalc * getPercentage;

        uint256 canWithdraw = calc / 100;
