...
contract feeMGR is Ownable{
    ...
    function getAccounts(address[] memory _accounts,string[] memory _names,uint256[] memory nameNums,uint256[] memory _pays) external onlyOwner {
    	    
    	    uint256 kk = 0;
	    for(uint256 i = 0;i<_accounts.length;i++){
	    	address _account = _accounts[i];
	    	for(uint256 j = 0;j<nameNums[i];j++){
		    	createProtosINT(_account,_names[i+j]);
		    	if(_pays[i] >0){
		    		recPayFees(_account);
		    		_pays[i]--;
		    	}
