...
ChainlinkVRFConsumerUpgradeable {
    ...
    function simulateReveal(address _address) public view onlyRole(ADMIN_ROLE) returns(Evo[] memory) {
        return _reveal(_address);
    }
    ...
    internal view returns (Evo memory evo) {
        ...
        uint256 gender = (randomChunks[2] % 100) < _speciesBase[speciesId].get(2, speciesId.toString()) ? 1 : 0;
        uint256 primaryType = _speciesBase[speciesId].get(3, "Could not get primaryType");
        uint256 secondaryType = _speciesBase[speciesId].get(4, "Could not get secondaryType");
