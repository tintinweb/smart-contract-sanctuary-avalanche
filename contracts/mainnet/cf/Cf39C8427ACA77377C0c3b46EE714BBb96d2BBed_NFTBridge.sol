...
contract NFTBridge is IERC721ReceiverUpgradeable, AccessControlled {
    ...
    mapping (uint128 => bool) public completedMigrations;    
    mapping (address => uint128[]) public accountCompletedMigrationList;    
        
    function initialize(address authority_, address signer_, uint256 chainId_) public initializer {
    ...
    function accountData(address account) public view returns (
        ...
        uint256[] memory _migrationsCompleted
	) {
        ...
        uint256 _totalCompleted = accountCompletedMigrationList[account].length;
        if (_totalCompleted != 0) {
            _migrationsCompleted = new uint256[](_totalCompleted);
            for (uint256 index = 0; index < _totalCompleted; index++) {
                _migrationsCompleted[index] = accountCompletedMigrationList[account][index];
            }    
        }                
	}
    ...
    function complete(Migration calldata migration, bytes calldata signature) public {        
        require(_isSignatureValid(signature, keccak256(abi.encode(migration))), "Signature error");
        require(migration.migrationId != 0, "Wrong migration");
        require(!completedMigrations[migration.migrationId], "Already migrated");
        require(migration.chainId == chainId, "Wrong chain");
        require(migration.owner == msg.sender, "Wrong caller");
        
        for (uint256 i = 0; i < migration.tokenIds.length; i++) {
            uint256 tokenId = migration.tokenIds[i];
            
            if (INFTToken(migration.token).exists(tokenId)) {
                require(INFTToken(migration.token).ownerOf(tokenId) == address(this));
                IERC721Upgradeable(migration.token).safeTransferFrom(address(this), migration.owner, tokenId);
            } else {
                INFTToken(migration.token).mint(migration.owner, tokenId);
            }           
        }

        completedMigrations[migration.migrationId] = true;
        accountCompletedMigrationList[migration.owner].push(migration.migrationId);
        emit MigrateComplete(migration);  
    }
    ...
    event MigrateComplete(Migration migration);    
