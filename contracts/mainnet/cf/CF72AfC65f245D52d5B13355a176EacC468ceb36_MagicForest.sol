...
contract MagicForest is IERC721Receiver, Pausable, Restricted {
    ...
    uint256[6] public claimingFeeValues= [0, 0.01 ether, 0.02 ether, 0.03 ether, 0.04 ether, 0.05 ether];
     ...
     */
    function _stakeRanger(address account, uint16 tokenId) internal {
    ...
    /**
     * Realize $GLOW earnings and optionally unstake tokens
    ...
    function _claimRanger(
        ...
        if (totalGlowEarned  < MAXIMUM_GLOBAL_GLOW) lastRangerClaimTimestamp = block.timestamp;
        totalGlowEarned += owed;
    ...
    function beforeUpgradeLevel(uint16 tokenId) external {
        ...
        AttributesPets.Boost memory rangerBoost = pets.getRangerBoost(tokenId);
        if (isRanger) {
            stakes[tokenId].unclaimed = _calculateRangerRewards(
                level,
                stake.lastValue,
                walletBoost,
                rangerBoost,
    ...
    function _updateSkeletonAlpha(uint16 tokenId, uint8 oldVirtualAlpha, uint8 newVirtualAlpha) internal {
        ...
        stakes[tokenId].unclaimed = _calculateSkeletonRewards(
            oldVirtualAlpha,
            stakes[tokenId].lastValue,
            stakes[tokenId].unclaimed
        );
        stakes[tokenId].lastValue = glowPerAlpha;        
    }
    ...
    function calculateRewards(uint16 tokenId) external view returns (uint256) {
        ...
        if (isRanger) {
            ...
            return _calculateSkeletonRewards(
                _getVirtualAlpha(alphaIndex, level, walletBoost.alphaAugmentation),
                stake.lastValue,
                stake.unclaimed
            );
    ...
    function getClaimingFee() public view returns (uint256) {
        if (sessionBegining == 0) return claimingFeeValues[2];
            ...
            uint256 claimNumber;
            if (block.timestamp - sessionBegining > 2 * sessionDuration) {
                claimNumber = 0;
            } else if (block.timestamp - sessionBegining > sessionDuration) {
                claimNumber = sessionClaimNumber;
            } else {
                claimNumber = lastSessionClaimNumber;
            }
            for (uint256 i; i < 5;) {
                if (claimNumber < claimingFeeTresholds[i]) return claimingFeeValues[i];
                unchecked{++i;}
            }
    ...
    function _calculateRangerRewards(
        ...
        // Unclaimed rewards (due to level up)
        uint256 owed = unclaimed;
        {
            uint256 start = lastClaim;
            uint256 middle = lastWalletAssociation;
            uint256 end = totalGlowEarned < MAXIMUM_GLOBAL_GLOW ? block.timestamp : lastRangerClaimTimestamp;
            
            if (start > end) start = end;
            if (middle < start) middle = start;
            else if (middle > end) middle = end;
            
            // Rewards since the wallet pet was equipped
            owed += ((middle - start) * dailyGlowRateWithoutWalletBoost) / 1 days; 
            // Rewards until the wallet pet was equipped
            owed += ((end - middle) * dailyGlowRateWithWalletBoost) / 1 days;
        }

        // Limit adventurer wage based on their level (limited inventory)
        uint256 maxGlow = _inventoryLimit(level);
        return owed > maxGlow ? maxGlow : owed;
    }

    function _calculateSkeletonRewards(
        uint8 virtualAlpha,
        uint256 lastGlowPerAlpha,
        uint256 unclaimed)
    internal view returns (uint256) {
        return unclaimed + virtualAlpha * (glowPerAlpha - lastGlowPerAlpha);
    }
    ...
    function _getClaimTaxPercentage(uint8 claimTaxReduction) internal view returns (uint8) {
    ...
    function _getMinimumToExit(
        ...
        return (MINIMUM_TO_EXIT * (100 + uint256(unstakeCooldownAugmentation) + uint256(globalUnstakeCooldownAugmentation))) / 100;
    }
    ...
    function _updateClaimingSession(uint32 claimingTimestamp) internal {
        if (claimingTimestamp - sessionBegining > sessionDuration) {
            if (claimingTimestamp - sessionBegining > 2 * sessionDuration) {
                lastSessionClaimNumber = (sessionBegining == 0) ? claimingFeeTresholds[1] : 0;
            } else {
    ...
    function setDailyGlowRate(uint256 _dailyGlowRate) external onlyController {
        DAILY_GLOW_RATE = _dailyGlowRate;
    }

    function setMinimumToExit(uint256 _minimumToExit) external onlyController {
        MINIMUM_TO_EXIT = _minimumToExit;
    }

    function setGlowClaimTaxPercentage(uint8 _glowClaimTaxPercentage) external onlyController {
        GLOW_CLAIM_TAX_PERCENTAGE = _glowClaimTaxPercentage;
    }

    function setClaimingFees(
        uint256[] calldata values,
        uint256[] calldata tresholds,
        uint32 duration
