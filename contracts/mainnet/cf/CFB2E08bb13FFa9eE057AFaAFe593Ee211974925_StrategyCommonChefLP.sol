...
/*     // performance fees
    function chargeFees(address callFeeRecipient) internal {
        ...
        address[] memory nativeToBurnTokenRoute = new address [] (3); //array with path address native to -> burnToken
        nativeToBurnTokenRoute[0] = native; //fudge specific due to nature of peg
        nativeToBurnTokenRoute[1] = lpToken0; //dai / native to peg address
        nativeToBurnTokenRoute[2] = burnToken; //burn token address
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(burnBal, 0, nativeToBurnTokenRoute, address(this), now); //purchase burnToken and send back to contract



        uint256 burnAmount = IERC20(burnToken).balanceOf(address(this)); //the amount of token purchased for the burn
        burnToken.call(abi.encodeWithSignature("burn(uint256)", burnAmount));
        ...
        address[] memory routeToBurn = new address[](2);
        routeToBurn[0] = lpToken1;
        routeToBurn[1] = lpToken0;

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(burnBal, 0, routeToBurn, address(this), now);
    ...
    function _giveAllowances() internal {
        ...
        /* IERC20(burnToken).safeApprove(address(this), uint256(-1)); */

       /*  IERC20(burnToken).safeApprove(unirouter, 0);
        IERC20(burnToken).safeApprove(unirouter, uint256(-1)); */
