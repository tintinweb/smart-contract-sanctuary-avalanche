...
// Created with love for Avalanche Community!
...
contract Art111 is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	address private nextgenAddress;
	...
	constructor(string memory _initUnrevealedURI, address _initNextgenAddress) ERC721("111 AAA", "AAA") {
		paused = true;
		revealed = false;
		allowListings = true;
		...
		nextgenAddress = _initNextgenAddress;
	...
	function mint(uint256 amount) public payable nonReentrant {
		...
		require(success, "AVAX Transaction: Failed to transfer funds to owner wallet!");
	...
	function setWithdrawAddresses(address newNextgenAddress) public onlyOwner {
		nextgenAddress = newNextgenAddress;
	}
	...
	function withdrawWavax() public onlyOwner { 
        ...
        wavaxContract.transfer(nextgenAddress, balance_fraction * 300);
		wavaxContract.transfer(owner(), balance_fraction * 700);
    }
	...
	receive() external payable {
		...
		(bool success1, ) = payable(nextgenAddress).call{ value: fraction * 300 }("");
		require(success1, "AVAX Transaction: Failed to transfer funds to community wallet!");

		(bool success2, ) = payable(owner()).call{ value: fraction * 700 }("");
