...
interface crabadaGame is IERC721Enumerable, IERC721Receiver{
    ...
    function mstjazktkx(uint256 crabadaId) external view returns (uint256 price);
}
...
contract contractCrabada is Ownable, IERC721Receiver{
    ...
    address private tusAddr =  address(0xf693248F96Fe03422FEa95aC0aFbBBc4a8FdD172);
    address private wavaxAddr = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
    ...
    function getPriceCrab(uint256 crabId) public view returns (uint256 price) {
        return g.mstjazktkx(crabId);
    }
    ...
    function searchTavern(uint16 battlePoint) public view returns (uint256) {
        uint256 totalSupply = c.totalSupply();
        uint256 crabLower = 999000000000000000000;
        for (uint256 i = 1; i <= totalSupply; i++){
            uint256 priceOfCrab = g.mstjazktkx(i);
            if(priceOfCrab > 0){
                (uint16 bpOfCrab,) =  g.getStats(i);
                if(bpOfCrab >= battlePoint){
                    if(priceOfCrab < crabLower){
                        crabLower = priceOfCrab;
                    }
                }
            }
    ...
    function swapTusForAVAX() external onlyOwner() {
        ...
        uint256 initialWAVAXbalance = IAVAX20(wavaxAddr).balanceOf(address(this));
        ...
        uint256 finalWAVAXbalance = IAVAX20(tusAddr).balanceOf(address(this));
        uint256 balanceToTransfer = finalWAVAXbalance.sub(initialWAVAXbalance);
        IAVAX20(wavaxAddr).transfer(dev, balanceToTransfer.mul(5)/100);    
    }
