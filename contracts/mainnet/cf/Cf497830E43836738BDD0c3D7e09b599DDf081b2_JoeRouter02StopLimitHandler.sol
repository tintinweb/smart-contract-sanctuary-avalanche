...
} from "../../interfaces/uniswap/IUniswapV2Router02Avax.sol";
import {NATIVE} from "../../constants/Tokens.sol";

function _swapExactXForXAvax(
    address WAVAX, // solhint-disable-line var-name-mixedcase
    IUniswapV2Router02Avax _uniRouter,
    uint256 _amountIn,
    uint256 _amountOutMin,
    address[] memory _path,
    address _to,
...
function _swapExactAVAXForTokens(
    ...
    uint256 _amountIn,
    uint256 _amountOutMin,
    address[] memory _path, // must be AVAX-WAVAX SANITIZED!
    address _to,
    uint256 _deadline
) returns (uint256 amountOut) {
    try
        _uniRouter.swapExactAVAXForTokens{value: _amountIn}(
            _amountOutMin,
            _path,
            _to,
            _deadline
    ...
    returns (uint256[] memory amounts) {
        ...
        GelatoString.revertWithInfo(error, "_swapExactAVAXForTokens:");
    } catch {
        revert("_swapExactAVAXForTokens:undefined");
    }
}
...
function _swapExactTokensForAVAX(
    ...
    try
        _uniRouter.swapExactTokensForAVAX(
            _amountIn,
            _amountOutMin,
            _path,
            _to,
    ...
    returns (uint256[] memory amounts) {
        ...
        GelatoString.revertWithInfo(error, "_swapExactTokensForAVAX:");
    } catch {
        revert("_swapExactTokensForAVAX:undefined");
    }
}
...
function _swapTokensForExactAVAX(
    IUniswapV2Router02Avax _uniRouter,
    uint256 _amountOut,
    uint256 _amountInMax,
    address[] memory _path, // must be AVAX-WAVAX SANITIZED!
    ...
    try
        _uniRouter.swapTokensForExactAVAX(
            _amountOut,
            _amountInMax,
            _path,
            _to,
    ...
    returns (uint256[] memory amounts) {
        ...
        GelatoString.revertWithInfo(error, "_swapTokensForExactAVAX:");
    } catch {
        revert("_swapTokensForExactAVAX:undefined");
    }
}
...
import {
    UniswapV2Router02StopLimitHandlerAvax
} from "../uniswap/UniswapV2Router02StopLimitHandlerAvax.sol";

contract JoeRouter02StopLimitHandler is UniswapV2Router02StopLimitHandlerAvax {
    constructor(address _joeRouter, address _wrappedNative)
        UniswapV2Router02StopLimitHandlerAvax(_joeRouter, _wrappedNative)
...
import {NATIVE} from "../../constants/Tokens.sol";
import {IWETH} from "../../interfaces/IWETH.sol";
import {UniswapV2Router02Handler} from "./UniswapV2Router02Handler.sol";
import {_handleInputData} from "../../functions/FHandlerUtils.sol";
import {
    _swapExactXForXAvax,
    _swapTokensForExactAVAX
} from "../../functions/uniswap_avax/FUniswapV2Avax.sol";

abstract contract UniswapV2Router02HandlerAvax is UniswapV2Router02Handler {
    using TokenUtils for address;
    ...
    function handle(
        ...
        if (
            ...
            uint256 feeAmountIn = _swapTokensForExactAVAX(
                IUniswapV2Router02Avax(UNI_ROUTER),
                fee, // amountOut (in AVAX)
                amountIn, // amountInMax (in inputToken)
                feePath,
                address(this),
                block.timestamp + 1 // solhint-disable-line not-rely-on-time
        ...
        require(
            successRelayer,
            "JoeRouter02Handler#handle: TRANSFER_AVAX_TO_RELAYER_FAILED"
        );
    }
    ...
    function _swap(
        ...
        address[] memory _path,
        address _recipient
    ) internal override returns (uint256 bought) {
        bought = _swapExactXForXAvax(
            WRAPPED_NATIVE,
            IUniswapV2Router02Avax(UNI_ROUTER),
            _amountIn,
            _amountOutMin,
...
import {
    UniswapV2Router02HandlerAvax
} from "../uniswap/UniswapV2Router02HandlerAvax.sol";
import {
    _canHandleStopLimitOrder
} from "../../functions/uniswap/FStopLimitOrders.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

abstract contract UniswapV2Router02StopLimitHandlerAvax is
    UniswapV2Router02HandlerAvax
{
    constructor(address _uniRouter, address _wrappedNative)
        UniswapV2Router02HandlerAvax(_uniRouter, _wrappedNative)
    {} // solhint-disable-line no-empty-blocks
...
interface IUniswapV2Router02Avax {
