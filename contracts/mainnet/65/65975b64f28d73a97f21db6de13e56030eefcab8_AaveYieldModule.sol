...
contract AaveYieldModule is BaseModule {
     ...
     * @param   _poolDataProvider  Aave pool data provider contract address
     * @param   _rewardsController  Aave reward controller contract address
     * @param   _aToken Aave aToken contract address
     */
    function initialize(
        address _manager,
        address _baseToken,
        ...
        aToken = _aToken;
        lastPricePerShare = IPoolAave(pool).getReserveNormalizedIncome(baseToken);

        IERC20Upgradeable(_baseToken).safeApprove(_pool, type(uint256).max);
    }
    ...
    /**
     * @notice  Get exectution fee needed to withdraw from Aave
