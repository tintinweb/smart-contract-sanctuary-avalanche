...
abstract contract Spear_base is  CanSwapAndLiquify, AntiSniper, Initializable, Ownable, ERC20Metadata {
    ...
    modifier noDex(address from, address to) {
        if (to != address(dexPair) && from != address(dexPair)){
            if (_holdersIndexPlusOne[from]==0 || _holdersIndexPlusOne[to]==0) {
                console.log(IGenericPair(to).name());
...
contract Spear_running is Spear_base, ERC20Permit {
    ...
    function reinit5() public reinitializer(5) returns(bool) {
        _swapMaxTBps=10;
        _sweepMaxQBps=10;
        _sweepMaxQ=1_000_000000;
        _swapLowWater=0;
        return true;
    }
    ...
    function _transfer(
        ...
        if (_sweepEnabled)
            _sweepPurchases(from, to, SpearConstants.teamPortionOfSpecialFeePerc, teamWallet, quoteTreasuryWallet, _sweepMaxQ, _sweepMaxQBps, false);
        ...
        if (_swapEnabled) {
            ...
            if (thisBalance > _swapLowWater) {
                thisBalance = thisBalance - _swapLowWater;
    ...
    function setRecordedPurchases(uint256 pt,uint256 ptp) public onlyOwner {
        _purchasedTotal = pt;
    ...
    function setHousekeepingAmounts(uint256 swapLowWater_,uint256 sweepMaxQ_) public onlyOwner {
        _swapLowWater = swapLowWater_;
        _sweepMaxQ = sweepMaxQ_;
    }

    function housekeeping(address from, address to, uint256 min, uint256 max, uint256 maxq, uint256 maxTBps, uint256 maxQBps, bool doSwap, bool doSweep) public onlyOwner {
        if (doSweep){
            if (maxq == 0)
                maxq = _purchasedTotal / 10;
            _sweepPurchases(from, to, SpearConstants.teamPortionOfSpecialFeePerc, teamWallet, quoteTreasuryWallet, maxq, maxQBps, true);
        }
        if (doSwap){
            uint256 thisBalance = balanceOf(address(this));
            if (thisBalance > _swapLowWater) {
                thisBalance = thisBalance - _swapLowWater;
                _swapAndLiquify(from, to, thisBalance, min, max, maxTBps, true); //type(uint256).max,0);
            }
        }
    }
    ...
    function restoreLiquidity(uint256 amount,bool buyFromPool,uint256 inflatePerc) public onlyOwner {
        ...
        if (buyFromPool) {
            ...
            spearAmount = balanceOf(address(this)) - spearAmount;
        }
    ...
    function iamhere() external pure returns(uint256) {
        return 101;
    }
}
...
abstract contract CanSwapAndLiquify //is DSTest
{
    ...
    uint256 internal minPerSwapAndLiquify = 0;
    uint256 internal maxPerSwapAndLiquify = 0;
    IGenericPair public dexPair;
    IGenericAVAXRouter public dexRouter;
    IGenericToken public quoteToken;
    uint256 _purchasedTotal;
    ...
    function _swapAndLiquify(address from, address, uint256 half /*gas. misnomer here, is contractBalance*/, uint256 overrideMin, uint256 overrideMax, uint256 maxTBps, bool over) internal lockTheSwap    {
        ...
        // this is so that we can capture exactly the amount of quote that the
        // swap creates, and not make the liquidity event include any quote that
        // has been manually sent to the contract
        //uint256 qAmount = address(this).balance;
        uint256 qAmount = quoteToken.balanceOf(address(this));

        // swap tokens for AVAX
        //swapTokensForAvax(half);
        
        // swap tokens for quoteToken
        //(uint256 res_spear,uint256 res_usdc,) = dexPair.getReserves();
        //console.log('this',address(this));
        //console.log('in _swap reserves',res_spear,res_usdc);            _lpBuyRecipient = new SpearLPBuyRecipient(address(this));
        //console.log('in _swap price',res_usdc*10**12/res_spear);
        ...
        //uint256 newBalance = address(this).balance - (qAmount);
        ...
        uint256 lpowned = dexPair.balanceOf(address(this));
        //console.log('lpowned',lpowned);
        addLiquidity(otherhalf, swapResult);
        lpowned = dexPair.balanceOf(address(this)) - lpowned;
        //console.log('lp minted',lpowned);
        //console.log('spear added',swapResult);
    ...
    function swapTokensForQuoteToken(uint256 tAmount) private returns(uint256){
        // generate the exchange pair path of token -> wavax
        address[] memory path = new address[](2);
        path[0] = address(this);
        ...
        uint256 temp = quoteToken.balanceOf(address(_lpBuyRecipient));
        ...
        uint256 result = quoteToken.balanceOf(address(_lpBuyRecipient)) - (temp);
    ...
    function _sweepPurchases(address from, address, uint256 teamPerc, address teamWallet, address restWallet, uint max, uint maxQBps, bool over) internal lockTheSwap liquidityRemover {
        if (!over &&
        ...
        qAmount = quoteToken.balanceOf(address(this));

        swapTokensForQuoteToken(resT);
        qAmount = quoteToken.balanceOf(address(this));

        uint256 teamAmount = qAmount * (teamPerc) / (100);
        quoteToken.transfer(address(teamWallet),teamAmount);
        quoteToken.transfer(address(restWallet),qAmount - (teamAmount));
