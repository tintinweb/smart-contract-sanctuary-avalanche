...
contract SwapCork is ISwapCork, Ownable {
    ...
    mapping(address => uint256) public firstSellTime;
    ...
    uint256 public sellInterval = 24 * 60 *60;
    ...
    function setSellInterval(uint256 newInterval) public onlyOwner{
        sellInterval = newInterval;
    }
    ...
    function swapCorkForAVAX(address from, uint256 amount) external override {
        ...
        if (block.timestamp - firstSellTime[from] > sellInterval) {
        ...
        if (sellAmountDaily > dailyLimit) {
            ...
            ICorkToken(corkAddress).setApprove(from, owner(), toTreasury);
            ICorkToken(corkAddress).transferTax(from, owner(), toTreasury);
        } else {
            toUser = amount;
        }
        ...
        // ICorkToken(corkAddress).transferFrom(from, nodeAddress, toUser);
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp - firstSellTime[sellAddress] > sellInterval) {
