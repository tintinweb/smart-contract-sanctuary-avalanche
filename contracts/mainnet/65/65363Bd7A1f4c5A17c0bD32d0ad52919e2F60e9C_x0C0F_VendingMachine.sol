...
// File contracts/C0FVendingMachine.sol
...
contract x0C0F_VendingMachine is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string baseURI;

    uint256 costs = 0.15 ether;
    uint256 minted = 0;

    x0C0F_Embassy nftContract;

    constructor() ERC721('C0F Vending Machine', 'C0FVM') {
        setBaseURI("https://raw.githubusercontent.com/shadowkite/council-of-frogs/master/json/116.json");
        _mint(msg.sender, 1);
    }

    function setNftContract(address _ca) public onlyOwner {
        nftContract = x0C0F_Embassy(_ca);
    }

    function setCosts(uint256 _costs) public onlyOwner {
        costs = _costs;
    }

    function setMinted(uint256 _minted) public onlyOwner {
        minted = _minted;
    }

    /** Public **/
    function mint(uint256 _amount) public payable {
        require(msg.value >= _amount * costs);
        require(nftContract.balanceOf(address(this)) >= _amount);
