...
contract UnderpegNFTPoolNGv5 is Controllable, IERC721Receiver, MyPausable {
    ...
    uint256[5] public predatorLoadIndex2;
    bool[5] public predatorLoaded2;

    UnderpegNFTPoolNGv5 public oldPool;
    UnderpegNFTPoolNGv5 public rlyoldPool;
    UnderpegNFTPoolNGv5 public rlyoldPool2;
    UnderpegNFTPoolNGv5 public rlyoldPool3;
    ...
    constructor(
        ...
        address _oldpool,
        address _rlyoldpool,
        address _rlyoldpool2,
        address _rlyoldpool3
    ) {
        ...
        oldPool = UnderpegNFTPoolNGv5(_oldpool);
        rlyoldPool = UnderpegNFTPoolNGv5(_rlyoldpool);
        rlyoldPool2 = UnderpegNFTPoolNGv5(_rlyoldpool2);
        rlyoldPool3 = UnderpegNFTPoolNGv5(_rlyoldpool3);
    ...
    function loadPredatorTokens(uint256 alphaIndex)  external onlyOwner {
        ...
        for (; gasleft() > 300000 ; i++) {
            try rlyoldPool.pack(alphaIndexToAlphaLUT[alphaIndex], predatorLoadIndex[alphaIndex] + i) returns (address owner, uint80 stakedAt, uint256 tokenId, uint256 mintPrice, uint256 premium, uint256 claimed) {
                alreadyStaked[tokenId] = true;
                packIndices[tokenId] = pack[alphaIndexToAlphaLUT[alphaIndex]].length;
                pack[alphaIndexToAlphaLUT[alphaIndex]].push(Stake(owner, stakedAt, tokenId, mintPrice, premium, claimed));
                IERC721Metadata(preyPredator).transferFrom(address(rlyoldPool2), address(this), tokenId);
    ...
    function loadPredatorTokens2(uint256 alphaIndex)  external onlyOwner {
        require(canMigrate, "Migration no longer possible");
        ...
        require(!predatorLoaded2[alphaIndex], "This predator group has already been imported");
        uint256 i;
        for (; gasleft() > 300000 ; i++) {
            try rlyoldPool2.pack(alphaIndexToAlphaLUT[alphaIndex], predatorLoadIndex2[alphaIndex] + i) returns (address owner, uint80 stakedAt, uint256 tokenId, uint256 mintPrice, uint256 premium, uint256 claimed) {
                alreadyStaked[tokenId] = true;
                packIndices[tokenId] = pack[alphaIndexToAlphaLUT[alphaIndex]].length;
                pack[alphaIndexToAlphaLUT[alphaIndex]].push(Stake(owner, stakedAt, tokenId, mintPrice, premium, claimed));
                IERC721Metadata(preyPredator).transferFrom(address(rlyoldPool3), address(this), tokenId);
                emit TokenStaked(owner, tokenId, mintPrice, premium, false);
                if (claimed > 0) emit Claimed(owner, tokenId, claimed);
            } catch { 
                predatorLoaded2[alphaIndex] = true;
                break;
            }
        }
        predatorLoadIndex2[alphaIndex] += i;
    }
    ...
    function closeMigration() external {
        ...
        require(predatorLoaded[3], "Alpha 3 not yet imported");
        require(predatorLoaded[4], "Alpha 4 not yet imported");
        require(predatorLoaded2[0], "Alpha2 0 not yet imported");
        require(predatorLoaded2[1], "Alpha2 1 not yet imported");
        require(predatorLoaded2[2], "Alpha2 2 not yet imported");
        require(predatorLoaded2[3], "Alpha2 3 not yet imported");
        require(predatorLoaded2[4], "Alpha2 4 not yet imported");
        canMigrate = false;
    }
