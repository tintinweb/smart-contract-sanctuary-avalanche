...
contract TroveManagerRedemptions is TroveManagerBase, ITroveManagerRedemptions {
    ...
    function redeemCollateralSingle(
        ...
        if (singleRedemption.troveDebt == YUSD_GAS_COMPENSATION) {
            ...
            troveManager.removeStakeAndCloseTrove(_target, 4);
            _redeemCloseTrove(
                contractsCache,
                _target,
    ...
    function _redeemCollateralFromTrove(
        ...
        if (newDebt == YUSD_GAS_COMPENSATION) {
            // No debt left in the Trove (except for the liquidation reserve), therefore the trove gets closed
            troveManager.removeStakeAndCloseTrove(_borrower, 4);
            _redeemCloseTrove(
                contractsCache,
                _borrower,
                YUSD_GAS_COMPENSATION,
    ...
    /**
     * @notice Works as the valid burner, since the correct access control was not given to YUSDToken
     *  and it is an immutable contract. Any new source of YUSD will burn YUSD through TMR. 
     */
    function burn(address _account, uint256 _amount) external override {
        require(controller.isValidAdditionalMinter(msg.sender), "TMR: Burn not valid minter");
        yusdTokenContract.burn(_account, _amount);
    }
}
