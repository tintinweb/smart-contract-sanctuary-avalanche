...
interface ITemple {
    function randomNinjaOwner(uint256 seed) external returns (address);
...
contract HakuNFT is ERC721Enumerable, Ownable {
    ...
    uint16 public phase;
    uint16 public ninjaStolen;
    uint16 public monkStolen;
    uint16 public ninjaMinted;

    uint16[] private _availableTokens;

    mapping(uint16 => bool) private _isNinja;
    ...
    IHakuCoin public hakuCoin;
    IRandomNumGenerator randomGen;

    address private constant fundWallet = 0xa355A0BFe235b9a1EA304A9dAFeafA113f7d2CAa;

    event TokenStolen(address owner, uint16 tokenId, address thief);

    constructor() ERC721("Hakuswap NFT", "HakuMonk") {
        _safeMint(msg.sender, 0);

        // Set default price for each phase
        phasePrice[1] = 1000 ether;
        phasePrice[2] = 1250 ether;
        phasePrice[3] = 1500 ether;
    }
    ...
    function isNinja(uint16 id) public view returns (bool) {
        return _isNinja[id];
    }
    ...
    function setNinjas(uint16[] calldata ids, bool value) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isNinja[ids[i]] = value;
        }
    }

    function setContracts(ITemple _temple, IHakuCoin _hakuCoin, IRandomNumGenerator _randomGen) external onlyOwner {
        temple = _temple;
        hakuCoin = _hakuCoin;
    ...
    function addAvailableTokens(uint16 _from, uint16 _to) external onlyOwner {
        require(!saleIsEnabled, "Sale is live");
    ...
    function reserveHakuNFT(address to, uint amount) external onlyOwner {
    ...
    function mint(uint amount, bool stake) external payable {
        require(tx.origin == msg.sender, "Only EOA");
        require(saleIsEnabled || privateSaleIsEnabled, "Sale must be enabled");
        if (privateSaleIsEnabled) {
            require(whitelist[msg.sender], "No whitelisted");
        }
        ...
        if (phase == 0) {
            ...
            if (totalPennyCost > 0) {
                hakuCoin.transferFrom(msg.sender, address(this), totalPennyCost);
            }
        }
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (recipient != msg.sender) {
                isNinja(tokenId) ? ninjaStolen += 1 : monkStolen += 1;
            ...
            if (!stake || recipient != msg.sender) {
                ...
                _safeMint(address(temple), tokenId);
                tokenIds[i] = tokenId;
            }
        }

        if (stake) {
            temple.addTokensToStake(msg.sender, tokenIds);
        }
    }
    ...
    function withdraw() external onlyOwner {
        ...
        uint coinBalance = hakuCoin.balanceOf(address(this));
        if (coinBalance > 0) {
            hakuCoin.transfer(fundWallet, coinBalance);
        }
    }
    ...
    function _selectRecipient(uint seed) private returns (address) {
        ...
        if (randomGen.getRandomNumber(totalSupply() + seed, 100) >= 10) {
            return msg.sender; // 90%
        }

        address thief = temple.randomNinjaOwner(totalSupply() + seed);
