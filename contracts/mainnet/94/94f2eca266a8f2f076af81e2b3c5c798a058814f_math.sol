  ...
  Authorizable
{
  ...
  uint256[3] public nftCost;
  nft_stake public NFT_CONT;
  bool stableOn = true;
  uint256[] public boostMultiplier;
  constructor(address[] memory swaps,uint256[] memory _boosts,uint256[] memory _nftCost){
	  swap_add = swaps[0];
	  ...
	  for(uint i=0;i<_nftCost.length;i++){
	  	nftCost[i] = _nftCost[0]*(10**18);
	  }
  }
 ...
 function getStakedAmount(address _account,uint256 _id) external returns(uint256){
  	if (NFT_CONT.isStaked(_account) == false){
  		return  Zero;
  	}
...
function updateStableOn(bool newVal) external onlyOwner {
 ...
 function updateNftCost(uint256[] memory _vals) external onlyOwner {
 	for(uint i=0;i<_vals.length;i++){
	  	nftCost[i] = _vals[0]*(10**18);
	  }
 }
