...
interface loveMonster is IERC721Enumerable, IERC721Receiver{
    function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256 id);
    function publicMultiMint(uint256 quantity) external payable;
}

contract minterNFT is Ownable, IERC721Receiver{
    uint256 private _totalMinted;
    loveMonster lm = loveMonster(0x07E741cf962Ef140cbA9B15B7E6AF7F31F3eD04D);
    ...
    function numberMintedView() external view returns (uint256){
        return numberMinted;
    }

    function tokenOwnerIndex(address owner, uint256 index) public view returns (uint256){
        return lm.tokenOfOwnerByIndex(owner, index);
    }
    ...
    function transferBack(uint256 id) onlyOwner external {
        lm.transferFrom(address(this), msg.sender, id);
    }
    function safeTransferBack(uint256 id) onlyOwner external {
        lm.safeTransferFrom(address(this), msg.sender, id);
    }


    function buy(uint256 number) external payable {
        uint256 toMint = number;
        lm.publicMultiMint{value:toMint*66*10**17}(toMint);
        payable(msg.sender).transfer(address(this).balance);
        uint256 tokenId = lm.tokenOfOwnerByIndex(address(this), numberMinted);
        if(tokenId < 5000){
            revert("sub 5000");
        }
        else{
