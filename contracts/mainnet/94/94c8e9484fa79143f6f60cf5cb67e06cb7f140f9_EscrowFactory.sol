...
contract EscrowFactory is Owned(tx.origin), ERC721TokenReceiver {
    ...
    function moveAssets() external returns (address payable) {
        require(rewardRouter != address(0), "REWARD_ROUTER_NOT_SET");
        require(escrowController != address(0), "ESCROW_CONTROLLER_NOT_SET");
        address escrow = this.getEscrow(msg.sender);
        require(RewardRouterV2(rewardRouter).pendingReceivers(msg.sender) == escrow, "NO_SIGNALXFER_TO_ESCROW");
        require(this.isEscrowDeployed(msg.sender), "ESCROW_NOT_DEPLOYED");
        Escrow(payable(escrow)).acceptTransferIn();
        uint256 nftId = Escrow(payable(escrow)).getTokenId();
