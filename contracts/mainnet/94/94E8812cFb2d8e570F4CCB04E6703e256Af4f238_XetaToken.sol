/**
 *Submitted for verification at snowtrace.io on 2022-11-10
*/
...
contract XetaToken is ERC20, Pausable, Ownable {
    ...
    event CreatingXonEvent(address indexed userWallet, uint256 indexed amount);

    event CreatingXonFromXpreEvent(address indexed userWallet, uint256 indexed amount);
    ...
    function mintXon(uint256 amount, uint256 _xonCreationFee) external whenNotPaused {
        ...
        emit CreatingXonEvent(msg.sender, amount);

    }



    /// @notice Use this function to create a XON from XPRE. A user should pay a create fee

    /// @dev An external service should listen to the CreatingXonFromXpreEvent

    function mintXonFromXpre(uint256 amount, uint256 _xonCreationFee) external whenNotPaused {
        ...
        emit CreatingXonFromXpreEvent(msg.sender, amount);
    ...
    function withdrawToken(address _tokenContract, address _recepient, uint256 _amount) external onlyOwner {
        ...
        tokenContract.transfer(_recepient, _amount);
