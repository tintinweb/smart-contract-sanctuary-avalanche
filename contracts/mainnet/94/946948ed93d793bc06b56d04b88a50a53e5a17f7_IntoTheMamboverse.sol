...
contract IntoTheMamboverse is Ownable, ERC721  {
    ...
    uint256 public MAX_SUPPLY                   = 95;
        
    bool    public saleIsActive                 = false;
    bool    public whitelistSaleIsActive        = false;
    bool    public publicSaleIsActive           = false;
    
    uint256  public rebate = .01 ether;
    ...
    string private _baseTokenURI = "ipfs://QmQc9dEhEi5yu8h6pBToqRTDUiupbJsjz3vxVSHjV46viA/";
    ...
    function flipWhitelistSale() public onlyOwner {
        whitelistSaleIsActive = !saleIsActive;
    }

    function flipPublicSale() public onlyOwner {
        publicSaleIsActive = !saleIsActive;
    }
    ...
    function airdrop(address[] calldata _to) public onlyOwner {
        require(supply.current() + _to.length <= MAX_SUPPLY, "Amount would exceed supply!");
    ...
    function signatureMint(bytes memory signature) external payable {
        require(saleIsActive, "Sale is not active yet.");
        require(!_hasMinted[msg.sender], "Already minted!");

        // Make sure signed by our backend first
        bytes32 hashedSender = keccak256(abi.encodePacked(msg.sender));
        address signer = hashedSender.toEthSignedMessageHash().recover(signature);
    ...
    function whitelistMint() external payable {
        require(whitelisted[msg.sender], "You're not on the WL!");
        require(whitelistSaleIsActive, "WL not open yet!");
        require(!_hasMinted[msg.sender], "Already minted!");
        _hasMinted[msg.sender] = true;
        _internalMint(msg.sender);
