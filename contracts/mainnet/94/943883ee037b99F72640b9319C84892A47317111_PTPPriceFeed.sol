...
contract PTPPriceFeed is IPriceFeed {
    using UQ112x112 for uint224;
    using SafeMathJoe for uint256;
    IPriceFeed public AVAXPriceFeed;
    JoePair public PTPAVAX;
    address public PTPToken;
    constructor(address LPToken, address _PTPToken, address _AVAXPriceFeed) public {
        PTPAVAX = JoePair(LPToken);
        PTPToken = _PTPToken;
        AVAXPriceFeed = IPriceFeed(_AVAXPriceFeed);
    }

    function fetchPrice_v() view external override returns (uint256) {
        address t0 = PTPAVAX.token0();
        address t1 = PTPAVAX.token1();
        (uint112 r0, uint112 r1, ) = PTPAVAX.getReserves();
        uint256 AVAXPrice = AVAXPriceFeed.fetchPrice_v();
        if (t0 == PTPToken) {
            return uint256(r1).mul(10 ** 18).div(uint256(r0)).mul(AVAXPrice).div(10 ** 18);
        } else {
             return uint256(r0).mul(10 ** 18).div(uint256(r1)).div(AVAXPrice).div(10 ** 18);
        }
