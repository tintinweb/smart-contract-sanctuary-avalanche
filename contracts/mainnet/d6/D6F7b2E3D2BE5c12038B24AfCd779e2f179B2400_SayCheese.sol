...
contract SayCheese is ERC721  {
    ...
    constructor()
        ERC721("SayCheese By KimiNaki", "CHESEE")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 108) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeieiapmcr2vfe6xbtn7rmu3osmeqwlrkhvixn7tfdx2q6tmkz3i4uy/", Strings.toString(tokenId), ".json")
            );
    }
