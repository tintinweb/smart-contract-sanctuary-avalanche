...
contract neadStake is Initializable, AccessControlEnumerableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function deposit(address account, uint amount) external whenNotPaused updateReward(account) {
    ...
    function withdraw(address account, uint amount) external whenNotPaused updateReward(account) {
        require(amount > 0, "Can't stake 0!");
        require(msg.sender == elmo, "!elmo");
    ...
    function notifyRewardAmount(address token, uint256 amount) external onlyRole(NOTIFIER_ROLE) updateReward(address(0)) {
        require(amount > 0);
