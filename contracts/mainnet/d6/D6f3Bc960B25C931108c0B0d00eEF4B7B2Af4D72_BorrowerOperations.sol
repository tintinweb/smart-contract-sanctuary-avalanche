...
contract BorrowerOperations is LiquityBase, IBorrowerOperations, ReentrancyGuardUpgradeable {
     ...
     * Commented out for contract sizing purposes.  
     */
    ...
    function _openTroveInternal(
        ...
        _changeTroveDebt(contractsCache.troveManager, vars.compositeDebt, true);
    ...
    function _closeTrove(
        ...
        contractsCache.troveManager.removeStakeAndCloseTrove(msg.sender, 2);
    ...
    /**
     * @notice Triggers YUSD Interest fee, only callable by trove manager.
     * @param _YUSDInterest Pending interest to be minted
     */
    function mintYUSDInterestFee(uint256 _YUSDInterest) external override {
        require(msg.sender == address(troveManager), "8");
        address yetiTreasury = controller.getYetiFinanceTreasury();
        _yusdTokenMint(yusdToken, yetiTreasury, _YUSDInterest);
    }
    ...
    function _requireValidMaxFeePercentage(uint256 _maxFeePercentage, bool _isRecoveryMode)
        ...
        view
    {
        // Alwawys require max fee to be less than 100%, and if not in recovery mode then max fee must be greater than the current fee floor
        if (
            _maxFeePercentage > DECIMAL_PRECISION ||
            (!_isRecoveryMode && _maxFeePercentage < controller.getBorrowingFeeFloor())
        ) {
            revert("27");
        }
    }
     ...
     */
    function _changeTroveDebt(ITroveManager _troveManager, uint256 _amount, bool _isDebtIncrease)
        internal
        returns (uint256)
    {
        return _troveManager.changeTroveDebt(msg.sender, _amount, _isDebtIncrease);
    }
