...
import "../interfaces/IFantasticChef.sol";
...
/*
    FantasticZap is a ZapperFi's simplified version of zapper contract which will:
    1. use ETH to swap to target token
    2. make LP between ETH and target token
    3. add into FantasticChef farm
*/
contract FantasticZapTraderJoe is Ownable, ReentrancyGuard {
    ...
    ZapInfo[] public zaps;
    IFantasticChef public fantasticChef;
    IJoeRouter02 public uniRouter;

    address public constant wavaxAddress = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7; // WAVAX

    constructor(address _fantasticChef, address _uniRouter) {
        require(_fantasticChef != address(0), "Zap::constructor: Invalid address");
        require(_uniRouter != address(0), "Zap::constructor: Invalid address");
        fantasticChef = IFantasticChef(_fantasticChef);
        uniRouter = IJoeRouter02(_uniRouter);
    }
    ...
    function zap(
        ...
        address _lp = address(fantasticChef.lpToken(_info.pid));
        ...
        (uint256 _tokenAmtUsedInLp, uint256 _ethAmtUsedInLp, uint256 _liquidity) = uniRouter
            .addLiquidityAVAX{value: _ethAmtToAddLP}(
            address(_info.token),
            _tokenAmtToAddLP,
            1,
            1,
        ...
        approveToken(_lp, address(fantasticChef), _liquidity);
        fantasticChef.deposit(_info.pid, _liquidity, msg.sender);
    ...
    function swap(
        ...
        address _token0 = IJoePair(_lp).token0();
        (uint256 _res0, uint256 _res1, ) = IJoePair(_lp).getReserves();
        ...
        returns (uint256 _tokenReceived)
    {
        ...
        _path[0] = wavaxAddress;
        _path[1] = _toToken;

        _tokenReceived = uniRouter.swapExactAVAXForTokens{value: _ethAmt}(
            1,
            _path,
            address(this),
            block.timestamp
