...
contract AaveVault is BaseBuildingBlock, IAaveVault {
    ...
    function initialize(bytes memory _data) public initializer {
        ...
        require(
            ...
            "AaveVault::intialize: address zero"
        );
        require(
        ...
        require(
            address(wavaxVariableDebtTokenAddress) != address(0),
            "AaveVault::intialize: address zero"
        );
        require(
            nativeId > 0,
            "AaveVault::intialize: id must be grester than 0"
        );
        ...
        wavaxVariableDebtToken = wavaxVariableDebtTokenAddress;
    }

    modifier onlyStrategy() {
        require(msg.sender == _aaveStrategy, "AaveVault::onlyStrategy");
        _;
    }
        ...
        onlyStrategy
    {
        if (_asset == address(0x0)) {
            (bool sent, ) = address(_aaveStrategy).call{value: _amount}("");
            require(sent, "AaveVault::transferToStrategy: native transfer");
        } else {
            require(
                IERC20(_asset).transfer(_aaveStrategy, _amount),
                "AaveVault::transferToStrategy: ERC20 transfer"
            );
        }
    }
     ...
     */
    function setAaveStrategy(bytes memory _data) public override onlySelf {
        _aaveStrategy = _bytesToAddress(_data);
    }
     ...
     */
    function setUserEMode(bytes memory _data) public override onlySelf {
     ...
     */
    function setCollateralAsset(bytes memory _data) public override onlySelf {
     ...
     */
    function openPosition(bytes memory _data) public override onlySelf {
        ...
        if (address(baseAsset) == address(0x0)) {
            require(
                ...
                "AaveVault::openPosition: insufficient base asset balance"
            );
     ...
     */
    function borrow(bytes memory _data) public override onlySelf {
        (
            IERC20 borrowAsset,
            uint256 amount,
            uint16 borrowRate,
     ...
     */
    function repay(bytes memory _data) public payable override onlySelf {
        ...
        if (address(asset) == address(0x0)) {
            require(
                msg.value == amount,
                "AaveVault::repay: mismatch of msg.value and amount"
            );
