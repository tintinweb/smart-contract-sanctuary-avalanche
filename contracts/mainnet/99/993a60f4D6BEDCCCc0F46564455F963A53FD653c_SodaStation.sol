...
contract SodaStation is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function getSodaAccruedForSPD(address _owner, uint256 _spd) internal view returns (uint256) {
        ...
        uint256 delta = block.timestamp - startTimeStamp[_owner];

        return stationSoda[_owner] + YIELDSPS * _spd * delta;
    }
    ...
    function getTotalSPD(address _owner) public view returns (uint256) {
        ...
        return totalSPD * (100 + getBoostSkillModifier(_owner)) *  (100 + getTotalMasterChefBonus(_owner)) / 10000;
    ...
    function createCupForPlayer(uint256 _cupType, uint256[] calldata _upgradeIds) external {
        ...
        require(_cupType <= getSizeSkillModifier(owner), "invalid cup type");

        _updateState(owner);

        CupType memory cupType = cupTypes[_cupType];
        uint256 upgradeLength = _upgradeIds.length;
        uint256 transactionCost = cupType.lpCost * upgradeLength;
    ...
    */
    function scrapCups(uint256[] calldata _cupIds) external {
        ...
        require (_getLevel(owner) < stationLevels.length, "already at max level");
    ...
    */
    ...
    function repairCups(uint256[] calldata _cupIds) external {
        ...
        for (uint256 i = 0; i < cupsLength; i++) {
            ...
            uint256 repairCost = cupType.lpCost + cup.repairs * cupType.lpCost / 4;  
    ...
    function _claimSoda(address _owner) internal {
        ...
        uint256 totalClaimed = getSodaAccruedForSPD(_owner, totalSPD * (100 + getBoostSkillModifier(_owner)) *  (100 + getTotalMasterChefBonus(_owner)) / 10000);
