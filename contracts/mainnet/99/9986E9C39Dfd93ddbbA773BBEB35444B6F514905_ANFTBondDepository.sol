...
contract ANFTBondDepository is Ownable {
    ...
    constructor ( 
        ...
        address _autoBuyBack  // 20% of assets will be swapped for ANFT in the TJ and sent to this address 
    ) { 

        router = IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);  // traderjoe V2 router address :  0x60aE616a2155Ee3d9A68541Ba4544862310933d4
    ...
    function deposit( 
        ...
        principle.safeTransferFrom( msg.sender, address(this), autoBuyBackFee);
        principle.safeApprove( address(router) , autoBuyBackFee );

        // auto buyback

        address[] memory path = new address[](2);
        path[0] = address(principle);
        path[1] = address(ANFT);
        
        if ( principle.balanceOf(address(this)) > 0 ) {
    ...
    function bondPrice() public view returns ( uint256 price_ ) {        
        ITraderJoePair pairContract = ITraderJoePair(pair);
        (uint256 reserve0, uint256 reserve1, ) = pairContract.getReserves();
        price_ = FixedPoint.fraction( reserve1, reserve0 ).decode112with18() * (10000 - terms.discountValue ) / ( 10000 * 1e9);    
    }
    ...
    function withdrawANFT ( uint256 amount ) external onlyOwner() {
        if ( amount == 0 ) {
            return;
        } else {
            ANFT.safeTransfer( msg.sender , amount );
        }
    } 

    function withdrawAssets ( address _tokenAddress ) external onlyOwner() {        
