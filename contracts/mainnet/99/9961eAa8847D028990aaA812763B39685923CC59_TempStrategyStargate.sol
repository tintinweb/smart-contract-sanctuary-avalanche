...
contract TempStrategyStargate is ReentrancyGuard, Ownable {
    ...
    uint256 baseTokenSellingPrice = 120;
    ...
    function _redeemLp() internal {
        ...
        if (isInstantRedeemLp == true) {
            ...
            (bool success, ) = address(stargateRouterAddress).call{ value: gasForTriggerStop }(abi.encodeWithSelector(
                bytes4(keccak256("redeemLocal(uint16,uint256,uint256,address,uint256,bytes,(uint256,uint256,bytes))")), 12, liquidityPoolId, liquidityPoolId, payable(address(this)), stakedLpBalance, abi.encodePacked(address(this)), abi.encode(0, 0, abi.encodePacked(address(this)))
            ));
