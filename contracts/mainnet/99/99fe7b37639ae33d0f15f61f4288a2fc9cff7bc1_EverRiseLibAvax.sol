...
library EverRiseAddressNumberLibAvax {
    ...
    bytes private constant token0Selector =
        abi.encodeWithSelector(IJoePair.token0.selector);
    bytes private constant token1Selector =
        abi.encodeWithSelector(IJoePair.token1.selector);
...
library EverRiseLibAvax {
    ...
    function swapETHForTokensNoFee(
        ...
        // make the swap
        uniswapV2Router.swapExactAVAXForTokens{
            value: amount
        }(
...
abstract contract EverRiseWallet is Context, IERC2612, IEverRiseWallet, IERC20Metadata {
    using EverRiseAddressNumberLibAvax for address;
...
abstract contract EverRiseConfigurable is EverRiseRoles, EverRiseWallet, IEverRise {
    using EverRiseAddressNumberLibAvax for uint256;
    ...
    function setRouterAddress(address newAddress) external onlyOwner {
        ...
        IJoeRouter02 _uniswapV2Router = IJoeRouter02(newAddress); 
        uniswapV2Pair = IJoeFactory(_uniswapV2Router.factory()).getPair(
            address(this),
            _uniswapV2Router.WAVAX()
        );
        if (uniswapV2Pair == address(0)) {
            uniswapV2Pair = IJoeFactory(_uniswapV2Router.factory())
...
contract EverRiseAvax is EverRiseConfigurable, IEverDrop {
    using EverRiseAddressNumberLibAvax for address;
    using EverRiseAddressNumberLibAvax for uint256;
    ...
    function _buyBackTokens(uint256 amount) private lockTheSwap returns (bool boughtBack) {
        ...
        if (amount > 0) {
            ...
            EverRiseLibAvax.swapETHForTokensNoFee(uniswapV2Router, address(stakeToken), amount);
    ...
    function _convertTokens(uint256 tokenAmount) private lockTheSwap {
        ...
        EverRiseLibAvax.swapTokensForEth(uniswapV2Router, tokenAmount);
