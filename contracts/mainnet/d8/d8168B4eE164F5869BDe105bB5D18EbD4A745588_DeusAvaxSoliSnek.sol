...
// File contracts/DeusAvaxSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract DeusAvaxSoliSnek  is Ownable, Pausable {
    ...
    ISoliSnekRouter.Routes[] public snekToDeusPath;
    ...
    constructor(
        ...
        ISoliSnekRouter.Routes[] memory _snekToDeusPath,
        ...
        for (uint i; i < _snekToDeusPath.length; ++i) {
            snekToDeusPath.push(_snekToDeusPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToDeusPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(deus).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wavax, deus, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
