...
contract InfiniteNodes is ERC20, Ownable, PaymentSplitter {
    ...
    mapping (address => bool) public _boosterUsed;
    ...
    function cashoutAll() external {
        ...
        if (swapLiquifyEnabled) {
            uint256 feeAmount;
            uint256 last_claim = _lastClaim[sender];
            if(NFTboosterActivated){
                bool booster_first_used = _boosterUsed[sender];
                if( (block.timestamp - last_claim) >= 24 hours || !booster_first_used){
