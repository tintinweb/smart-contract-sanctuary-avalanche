    ...
    ReentrancyGuardUpgradeable
{
        ...
        onlyExistingId(_tokenId)
    {
         ...
         uint setPrice = 50 * MULTIPLIER;

           if (_tokenId > 1) {
                    setPrice =  200 * MULTIPLIER;         
             }

        bool canMintTier = _ownsPrevious(_to, _tokenId);

        require(canMintTier, "Address don't own previous tier");
    ...
    function _beforeTokenTransfer(address _sender) private {
        ...
        function _ownsPrevious( address _to, uint256 _tokenId)
        private
        view
        noZeroAddress(_to)
        returns (bool)
    {
        bool canMintTier = false;
        uint256 previous = _tokenId -1;
        if(_tokenId > 1){
          if(balanceOf(_to, previous) > 0) {
                    canMintTier = true; }
                 }else{
                 canMintTier= true;
                 }
