...
contract PenguinEmperor is OwnableInitialized {
    ...
    //amount in seconds that playtime lasted
    uint32 public gameDuration;
    //amount of players that stole the crown
    uint32 public amountOfParticipants;
    ...
    function stealCrown(uint256 amount, address paidByToken) external canBetWithToken(paidByToken) stealCrownCheck() returns (bool bidFailed) {
        ...
        if (playerDB[msg.sender].hasStolenTheCrown == false) {
            playerDB[msg.sender].hasStolenTheCrown == true;
