...
contract CapitalCall is Context, Ownable, ICapitalCall,ERC1820Client {
    ...
    string internal ERC20_INTERFACE_VALIDATOR = "ERC20Token";
    ...
    mapping(address => address) public propertyToPropertyAccount;
    mapping(uint256 => uint256) public idTowithdrawnAmount;
    mapping(uint256 => mapping(address => uint256)) private addrToAmount; // invester address to amount invested
    ...
    function _checkGoalReached(uint256 _id) private view {
        CapitalCallStr memory cc = idToCapitalCall[_id];
    ...
    // ---------------------external functions----------------------
    /**
    * @dev distribution the tokens according to the investments
    * @param _id capital call identity
    * @param investor address of investor
    * @param certificate unique certificate to issue token(s)
    */
        ...
        checkGoalReached(_id)
    {
        ...
        uint256 invested = addrToAmount[_id][investor];
        uint256 token = invested / cc.rate;

        require(token != 0,"CapitalCall: try to transfer zero tokens");
        
        addrToAmount[_id][investor] = 0;
     ...
     /**
    * @dev getAllowedInvestmentAmount get Allowed Investment Amount of investors
    * @param investor address of investor
    */
    ...
    /**
    * @dev getCapitalCall get CapitalCall details
    * @param _id capital call identity
    */
        ...
        ) 
    {
      ...
      /**
    * @dev initiateCapitalCall initiate CapitalCall of a property
    * @param _stAddress address of the property token
    * @param _proposal description of the capital call proposal
    * @param _startDate start date of the capital call
    * @param _endDate end date of the capital call
    * @param _publicEndDate end date of the capital call while public end
    * @param _goal capital call raised amount 
    * @param _minGoal minimum goal amount for withdraw from capital call
    * @param _rate rate of the token to invest in capital call
    */
    ...
    /**
    * @dev investorDeposit invester will invest in the capital call
    * @param _id identity of the capital call
    * @param _amount amount to invest in the capital call from the invester
    ...
    */
        ...
        onlyWhitelistedUser
    {
        ...
        require(
            _amount <= akusd.allowance(_msgSender(), address(this)),
            "CapitalCall: Approve AKUSD Before Invest"
        );
        require(
            akusd.balanceOf(_msgSender()) >= _amount,
            "CapitalCall:Unsufficient AKUSD balance"
        );
        require(block.timestamp >= cc.startDate, "CapitalCall: Capital call not started");
        ...
        uint256 alreadyInvested = addrToAmount[_id][_msgSender()];
        ...
        if (addrToAmount[_id][_msgSender()] == 0) {
            cc.investors.push(_msgSender());
        }

        cc.amountRaised += _amount;
        addrToAmount[_id][_msgSender()] += _amount;
    ...
    /**
    * @dev investForPublic invester will invest in the capital call while in public sale
    * @param _id identity of the capital call
    * @param _amount amount to invest in the capital call from the invester
    ...
    */
    ...
    /**
    * @dev isGoalReached check  if the goal is reach for the given amount of capital call
    * @param _id identity of the capital call
    ...
    */
    function isGoalReached(uint256 _id) external view override returns(bool) {
     ...
     /**
    * @dev setAkUsdAddress update the akusd address  of capital call
    * @param _akusd new address of akusd(stablecoin address)
    ...
    */
 ...
 
    {
     ...
     /**
    * @dev setWhiteListAddress update the whitelist address of capital call
    * @param _whitelist new address of whitelist
    ...
    */
...

    {
     ...
     /**
    * @dev withdrawRaisedAmount withdraw raised amount of capital call
    * @param _id identity of the capital call
    ...
    */
        ...
        override
    {
        ...
        require(propertyAccount != address(0),"Capital Call: property account address can not be zero");
        
        uint256 withdrawnAmount = idTowithdrawnAmount[_id]; 
        uint256 leftoverAmount = cc.amountRaised - withdrawnAmount;
        
        require(leftoverAmount != 0,"Capital call: No Amount to Withdraw");

        idTowithdrawnAmount[_id] += leftoverAmount;
     ...
     /**
    * @dev setPropertyAccount set new property account address of capital call
    * @param _stAddress address of the property token
    * @param _propertyAccount address of the property account
    */
        ...
        onlyAdmin    
    {
      ...
      /**
    * @dev extendTime extend the time of capital call to invest in public
    * @param _id identity of the capital call
    * @param _newEndDate new end date of the capital call
    */
        ...
        override
    {
     ...
     /**
    * @dev extendEndTime extend the time of capital call to invest of the token holders
    * @param _id identity of the capital call
    * @param _newEndDate new end date of the capital call
    */
        ...
        
    {
     ...
     /**
    * @dev update interface hash for erc1400
    * @param _interfaceHash interface hash for erc1400
    */
    function updateInterfaceHash(string calldata _interfaceHash) external override onlyAdmin {
        ERC20_INTERFACE_VALIDATOR =  _interfaceHash;
    }
}
...
interface ICapitalCall {
    ...
    function updateInterfaceHash(string calldata _interfaceHash) external ;
