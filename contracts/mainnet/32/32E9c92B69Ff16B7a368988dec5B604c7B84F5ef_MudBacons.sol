...
contract MudBacons {
    ...
    uint256 public WALLET_DEPOSIT_LIMIT = 25 * 1e18; /** 25 AVAX  **/
    ...
    constructor(address payable _dao, address payable _dev, address payable _mkt) {
		require(isContract(_dao) && !isContract(_dev) && !isContract(_mkt));
        owner = msg.sender;
        dao = _dao;
        dev = _dev;
        mkt = _mkt;
    ...
    function getBaconsYield(uint256 amount) public view returns(uint256,uint256) {
        uint256 baconsAmount = calculateBaconBuy(amount , getBalance().add(amount).sub(amount));
