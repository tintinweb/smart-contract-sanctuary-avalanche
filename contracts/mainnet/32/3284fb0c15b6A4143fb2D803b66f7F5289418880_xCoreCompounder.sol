...
/// @title Compounder
/// @author Vector Team
contract xCoreCompounder is Initializable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    function __xCoreCompounder_init_(
        string memory _name,
    ...
    function updateDepositsInfo() external onlyOwner {
        (, , , , rewarder, poolHelper, ) = masterchef.addressToPoolInfo(address(XTOKEN));
    }
    ...
    function totalDeposits() public view returns (uint256) {
        return masterchef.depositInfo(address(XTOKEN), address(this));
    }
        ...
        returns (address[] memory tokens, uint256[] memory amounts)
    {
        ...
        (, , , , address rewarder, , ) = masterchef.addressToPoolInfo(address(XTOKEN));
    ...
    function _depositLp(uint256 _amount) internal {
        _approveTokenIfNeeded(address(XTOKEN), poolHelper, _amount);
    ...
    function migrateAllUserDepositsFromManual() external {
        uint256 amount = masterchef.depositInfo(address(XTOKEN), msg.sender);
    ...
    function convertAndDeposit(uint256 amount) external {
        _compoundIfNeeded();
        TOKEN.safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(address(TOKEN), tokenConvertor, amount);
        uint256 amountToDeposit = IxTokenConvertor(tokenConvertor).deposit(amount);
    ...
    function _withdraw(uint256 _amount) internal {
        masterchef.withdraw(address(XTOKEN), _amount);
    }
    ...
    function _addLiquidityAndDepositLp() internal {
        ...
        if (amount > 0) {
            _swapWavaxToToken(address(TOKEN), amount);
            _convertAllTOKEN();
            _depositLp(XTOKEN.balanceOf(address(this)));
        }
    }

    /// @notice Convert all token to xtoken using the smart convertor
    function _convertAllTOKEN() internal {
        uint256 amount = TOKEN.balanceOf(address(this));
        _approveTokenIfNeeded(address(TOKEN), tokenConvertor, amount);
        IxTokenConvertor(tokenConvertor).deposit(amount);
    }

    function _harvestPool() internal {
        masterchef.deposit(address(XTOKEN), 0);
    }
    ...
    function _swapWavaxToToken(address token, uint256 balance) internal {
        if (token != WAVAX) {
            ...
            ISwapHelper(swapHelper).safeSwapTokensForToken(
                tokens,
                amounts,
                token,
                address(this),
    ...
    function convertDust() public {
        _safeSwapToWavax(address(TOKEN), TOKEN.balanceOf(address(this)));
    }
