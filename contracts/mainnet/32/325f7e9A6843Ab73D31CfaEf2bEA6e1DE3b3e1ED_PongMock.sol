...
contract PongMock is NonblockingLzApp {
    ...
    event LZReceiveInfo(uint nativeFee);
    ...
    function _nonblockingLzReceive(
        ...
        bytes memory callbackAdapterParams = abi.encodePacked(uint16(1), fixedCallbackHomeFee);
        ...
        emit LZReceiveInfo(nativeFee);

        _lzSend(_srcChainId, callbackPayload, payable(address(this)), address(0x0), callbackAdapterParams, nativeFee);
    }

    function setFixedCallbackHomeFee(uint _fixedCallbackHomeFee) external {
        fixedCallbackHomeFee = _fixedCallbackHomeFee;
    }
