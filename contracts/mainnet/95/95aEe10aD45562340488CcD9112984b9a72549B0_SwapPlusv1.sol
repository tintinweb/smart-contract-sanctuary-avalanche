...
contract SwapPlusv1 is Ownable {
  ...
  constructor(
    ...
    routers["TradeJoe"] = 0xb4315e873dBcf96Ffd0acd8EA43f689D8c20fB30;
  ...
  function _swap(swapLine memory line, uint256 amount) internal returns(uint256) {
    ...
    for (uint256 x=0; x<swLen; x++) {
      ...
      if (_compareStrings(line.swaps[x].platform, "TradeJoe")) {
        uint256[] memory pairBinSteps = new uint256[](1);
        pairBinSteps[0] = line.swaps[x].meta[0];
        ILBRouter.Version[] memory versions = new ILBRouter.Version[](1);
        versions[0] = ILBRouter.Version(line.swaps[x].meta[1]);
        address[] memory path = new address[](2);
        path[0] = line.swaps[x].tokenIn;
        path[1] = line.swaps[x].tokenOut;
        ILBRouter.Path memory pm = ILBRouter.Path({
          pairBinSteps: pairBinSteps,
          versions: versions,
          tokenPath: path
        });
        outAmount = ILBRouter(routers["TradeJoe"]).swapExactTokensForTokens(
