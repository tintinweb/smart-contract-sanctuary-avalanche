...
contract SafeAvax is Context, IERC20, Ownable {
    ...
    string private constant _name = "SafeAvax"; // 
    string private constant _symbol = "SafeAvax"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 private _marketingFee = 3;
    uint256 private _liquidityFee = 5;
    address private  _marketingWallet = 0x83b3d869BC9428e369CE1A0740095f0459573Ce1;
    ...
    event SwapAndLiquify(uint256 tokensSwapped, uint256 AVAXReceived, uint256 tokensIntoLiquidity);
    ...
    constructor () {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WAVAX());
    ...
    function swapAndSendToFee (uint256 tokens) private {
        uint256 AVAXToSend = swapTokensForAVAX(tokens);
        
        if (AVAXToSend > 0)
            payable(_marketingWallet).transfer(AVAXToSend);
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 newBalance = swapTokensForAVAX(half);
    ...
    function swapTokensForAVAX(uint256 tokenAmount) private returns (uint256) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 AVAXAmount) private {
        ...
        // add the liquidity
        (,uint256 AVAXFromLiquidity,) = uniswapV2Router.addLiquidityAVAX {value: AVAXAmount} (
            address(this),
            tokenAmount,
        ...
        if (AVAXAmount - AVAXFromLiquidity > 0)
            payable(_marketingWallet).sendValue (AVAXAmount - AVAXFromLiquidity);
    }
}