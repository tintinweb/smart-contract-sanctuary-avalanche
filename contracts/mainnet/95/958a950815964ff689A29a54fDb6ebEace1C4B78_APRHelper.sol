...
contract APRHelper is Initializable, OwnableUpgradeable {
    ...
    function getAdditionalRewardsInUSDPerYear(address lp) public view returns (uint256 USDPerYear) {
        uint256 _pid = IMasterPlatypusv4(masterPlatypus).getPoolId(lp);
    ...
    function getAPRforPTPPoolInAdditionalReward(address lp) public view returns (uint256 APR) {
        APR = getAdditionalRewardsInUSDPerYear(lp) / getTVL(lp) / 10**8;
    }
    ...
    {
        APR =
            (getAdditionalRewardsInUSDPerYear(lp) * feeAmount) /
            FEE_DENOMINATOR /
            getTVLForLocker() /
            10**8;
    }
    ...
    function getXPTPTVL() public view returns(uint256 usdValue) {
        ...
        usdValue = (sizeOfPool *
            getTokenPricePairedWithAvax(ptp) *
            getRatio(xPTP, ptp, 8))/ 10**(8 + ERC20(xPTP).decimals());
    }
        ...
        returns (uint256 bribesPerYear)
    {
        ...
        if (bribe != address(0)) {
            ...
            uint256 vePTPVote = (coef * IBribeManager(bribeManager).totalVotes()) /
                ACC_TOKEN_PRECISION;
            uint256 tokenReward = 3600 * 24 * 365 * (IBribe(bribe).tokenPerSec());
            uint256 accTokenPerShare = ((tokenReward * (ACC_TOKEN_PRECISION)) / totalShares);
        ...
        returns (uint256 bribesPerYear)
    {
        uint256 totalShares = IVoter(voter).weights(lp);
        address bribe = IVoter(voter).bribes(lp);
        uint256 veptpAmount = (IBribeManager(bribeManager).veptpPerLockedVtx() * vtxAmount) /
            FEE_DENOMINATOR;
        if (bribe != address(0)) {
            uint256 tokenReward = 3600 * 24 * 365 * (IBribe(bribe).tokenPerSec());
            uint256 accTokenPerShare = ((tokenReward * (ACC_TOKEN_PRECISION)) / totalShares);
    ...
    // function getProjectedAPRForBribes(address lp) public view returns (uint256 APR) {
    //     uint256 totalShares = IVoter(voter).weights(lp);
    //     address bribe = IVoter(voter).bribes(lp);
    //     if (bribe != address(0)) {
    //         uint256 bribesPerYear = getBribesPerAmount(lp, 10**18);
