...
contract GasThrottler {
    ...
    address public gasprice = address(0x28D43539776d1698c85b68193e1FB5Bae7bF41E2); // avax
...
// File: contracts/BIFI/strategies/Common/FroyoStrategyLP.sol
...
contract FroyoStrategyLP is StratManager, FeeManager {
    ...
    // Routes
    address[] public nativeToBurnRoute;
    ...
    constructor(
      //  uint256 _burnRate, //RATE OF REWARDS BURNED
        ...
        address[] memory _nativeToBurnRoute
    ...
    // BURN FEE / TAX LOGIC IN CONSTRUCTOR //
        ...
        // setup lp routing
        lpToken0 = IUniswapV2Pair(want).token0(); //LP 0
        ...
        require(_nativeToBurnRoute[0] == native, "_nativeToBurnRoute[0] != native"); // route to purchase burn token regardless of LP
        burnToken = _nativeToBurnRoute[_nativeToBurnRoute.length - 1]; //get burn token
        nativeToBurnRoute = _nativeToBurnRoute; //get burn route
        _giveAllowances();
    }

     /* ============= Taxation + Burn  ============= */
    function setBurnRoute(address[] memory _nativeToBurnRoute )
        public
        onlyOwner
    {
        nativeToBurnRoute = _nativeToBurnRoute;
    }

    function setBurnRate(uint256 _burnRate) public onlyOwner {      // Change the burnrate up to 10%
        require(_burnRate < 1000, "burnrate must be less than  100%");
        burnRate = _burnRate;
    }
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(60).div(1000); //6% FEE
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);

        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        uint256 toBurn = nativeBal.div(2); //use fee and set aside enough according to burn rate

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toBurn, 0, nativeToBurnRoute, address(this), now); // purchase burn token with router
        uint256 pegTokenBal = IERC20(burnToken).balanceOf(address(this)); // balance of burn token in contract
        burnToken.call(abi.encodeWithSignature("burn(uint256)", pegTokenBal)); //burn token using ERC20BURNABLE _burn
    ...
    function _giveAllowances() internal {
        ...
        IERC20(burnToken).safeApprove(address(this), uint256(-1)); //approve strategy with allowance for burnToken

        IERC20(burnToken).safeApprove(unirouter, 0); //approve burn token to router with 0 allowance
        IERC20(burnToken).safeApprove(unirouter, uint256(-1)); // approve burn token to router with max value

        IERC20(native).safeApprove(unirouter, 0); //approve network native to to router with 0 allowance
        IERC20(native).safeApprove(unirouter, uint256(-1)); // approve network native to router with max value
    ...
    function _removeAllowances() internal {
        ...
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
        IERC20(burnToken).safeApprove(address(this), 0);
        IERC20(burnToken).safeApprove(unirouter, 0);
    }
...
// File: contracts/BIFI/strategies/Common/StrategyCommonChefLPFROYO.sol
...
contract StrategyCommonChefLPFROYO is FroyoStrategyLP, GasThrottler {
