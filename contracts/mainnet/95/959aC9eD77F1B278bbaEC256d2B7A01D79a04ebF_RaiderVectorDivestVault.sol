...
import "../../interfaces/IDivestmentStrategy.sol";
import "../../interfaces/IUniswapRouterETH.sol";
import "../../interfaces/IRaiderZapper.sol";
import "../../common/ADivestVault2.sol";
 ...
 */
contract RaiderVectorDivestVault is ADivestVault2 {
  ...
  ) ADivestVault2(_uniRouter, _token, _name, _symbol, _approvalDelay) {}

  /* solhint-enable */

  /**
   * @dev It calculates number of LP otkens a user has deposited.
   * 1. LP Tokens = Deposits - Withdrawals
   * 2. (_reserve0, _reserve1) = UniswapPair.getReserves()
   * 3. Get reserves for LP token value.
   */
  function userBalance(address user) external view returns (uint256 balance) {
    return cumulativeDeposits[user] - cumulativeWithdrawals[user];
  }
}
...
import "./IStrategy2.sol";

interface IDivestmentStrategy is IStrategy2 {
  function getDivestToken() external view returns (address);
 ...
 * The yield optimizing strategy itself is implemented in a separate 'Strategy.sol' contract.
 */
abstract contract ADivestVault2 is ERC20, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;
   ...
   * Never invest in a Raider Vault with tokens you don't trust.
   * Never invest in a Raider Vault with tokens whose rules for minting you don’t agree with.
   * Ensure the accuracy of the contracts for the tokens in the Raider Vault.
   * Ensure the accuracy of the contracts for the Raider Vault and Strategy you are depositing in.
   * Check our documentation regularly for additional disclaimers and security assessments.
   * ...and of course: DO YOUR OWN RESEARCH!!!
   *
   * By accepting these terms, you agree that Raider or any parties
  ...
  mapping(address => uint256) public cumulativeDivestments;

  event NewStratCandidate(address implementation);
  event UpgradeStrat(address implementation);
  event TermsAccepted(address user);

  event DepositsIncremented(
  ...
  function initialize(address _strategy) public onlyOwner returns (bool) {
    ...
    // TODO @cstoneham clean this line up - HACK HACK
  ...
  function balance() public view returns (uint256) {
    return
      token.balanceOf(address(this)).add(IStrategy2(strategy).totalLPBalance());
  }
  ...
  function deposit(uint256 _amount) public nonReentrant {
    ...
    // Before deposit, payout the user and update info
    distributeDivestedTokens();
  ...
  function withdraw(uint256 _shares) public nonReentrant {
    ...
    require(totalSupply() > 0, "there must be a non-zero number of tokens");

    // Before withdrawal, payout the user and update info
    distributeDivestedTokens();
    ...
    if (b < r) {
      ...
      IStrategy2(strategy).withdraw(_withdraw);
      uint256 _after = token.balanceOf(address(this));
  ...
  /**
   * Distribute divested tokens and emit a distribution event
   */
  function distributeDivestedTokens() public {
    IERC20 divestToken = IERC20(IDivestmentStrategy(strategy).getDivestToken());
    uint256 withdrawn = IDivestmentStrategy(strategy).withdrawDivestedTokens(
      msg.sender
    );

    if (withdrawn > 0) {
      divestToken.safeTransfer(msg.sender, withdrawn);
      incrementDivestments(withdrawn);
    }
  }
  ...
  function upgradeStrat() public onlyOwner {
    ...
    IStrategy2(strategy).retireStrat();
  ...
  function incrementDeposits(uint256 _amount) internal returns (bool) {
    uint256 initial = cumulativeDeposits[msg.sender];
    uint256 newTotal = initial.add(_amount);
  ...
  function incrementWithdrawals(uint256 _amount) internal returns (bool) {
    uint256 initial = cumulativeWithdrawals[msg.sender];
    uint256 newTotal = initial.add(_amount);
  ...
  function incrementDivestments(uint256 _amount) internal returns (bool) {
    uint256 initial = cumulativeDivestments[msg.sender];
    uint256 newTotal = initial.add(_amount);
    cumulativeDivestments[msg.sender] = newTotal;
    emit DivestmentsIncremented(msg.sender, _amount, newTotal);
    return true;
  }
}
...
interface IStrategy2 {
  ...
  //update Call Fee
  function updateCallFee(uint256 _callFee) external returns (bool);
