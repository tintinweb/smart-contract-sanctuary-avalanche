...
contract AvaxtarsTokenStaking is Ownable {
    ...
    function transferFromLayer2(uint _amount, uint _nonce, address tokenAddress, bytes memory _signature) external
    {
        ...
        emit TransferFromLayer2(msg.sender, tokenAddress, _nonce, _amount);
    }

    function approve(address tokenAddress, uint256 _amount) public onlyOwner
    {
        IERC20 token = IERC20(tokenAddress);
    ...
    function deposit(address tokenAddress, uint _amount) public
    {
        ...
        emit TransferToLayer2(msg.sender, tokenAddress, _amount);
    }
    ...
    event TransferFromLayer2(address indexed to, address tokenAddress, uint256 nonce, uint256 amount);
    event TransferToLayer2(address indexed to, address tokenAddress, uint256 amount);
}