...
ERC721URITokenJSON, CosmicAttributeStorageUpgradeable, Blacklistable, TokenConstants, IStandardERC721  {
    ...
    function tokenURIJSON(uint256 tokenId) public view virtual override returns(string memory) {
        ...
        for (uint256 i = 0; i < 11; i++) {
            ...
            attributes[i] = TokenMetadata.Attribute(getSkillName(0, i), '', getSkillName(1000 + i + baseSkillNameId, baseSkills[i]), false);
        }
        ...
        for (uint256 i = 0; i < 12; i++) {
            attributes[i + 13] = TokenMetadata.Attribute(getSkillName(1, i), '', getSkill(tokenId, 1, i).toString(), true);
        }

        attributes[25] = TokenMetadata.Attribute("Rarity Bonus", 'boost_percentage', baseSkills.getRarityBonus().toString(), true);
