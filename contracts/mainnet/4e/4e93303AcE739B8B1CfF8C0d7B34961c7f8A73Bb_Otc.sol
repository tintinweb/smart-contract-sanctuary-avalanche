...
contract Otc is Pausable, Ownable {
    ...
    constructor(
        ...
        appendListing(listingWallets, initialAmounts, createdAts);
    }

    /// @notice Appends listings by the owner to the end of list
    /// @dev Constructor doesn't allow to deploy SC with a huge number of listings
    ...
    function cancelListing(uint256 listingId) external whenNotPaused {
        ...
        require(listings[listingId].remainingAmount > 0, "There is no remaining amount");
    ...
    /// @dev Does not emit ListEvent. Increases listingsNumber
