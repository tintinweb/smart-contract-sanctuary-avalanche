...
contract xJoeConvertor is Ownable {
    ...
    /// @notice returns the ratio between xJoe and Joe. 10000= 1:1 rate
    function getxJoeRatio() public view returns (uint256 xJoeRatio) {
        (uint256 r0, uint256 r1, ) = IJoePair(pair).getReserves();
        if (IJoePair(pair).token0() == xJoe) {
            xJoeRatio = (r1 * DENOMINATOR) / r0;
        } else {
            xJoeRatio = (r0 * DENOMINATOR) / r1;
        }
    }

    /// @notice estimates the amount of xJoe recieved if bought on traderJoe
    function estimateOutAmount(uint256 amountSold) public view returns (uint256 amountOut) {
        (uint256 r0, uint256 r1, ) = IJoePair(pair).getReserves();
        if (IJoePair(pair).token1() == xJoe) {
    ...
    function maxAmountToBuy() public view returns (uint256) {
        (uint256 r0, uint256 r1, ) = IJoePair(pair).getReserves();
        uint256 xjoeReserves = (IJoePair(pair).token0() == xJoe ? r0 : r1);
        uint256 joeReserves = (IJoePair(pair).token0() == xJoe ? r1 : r0);
        if (joeReserves > xjoeReserves) {
            return 0;
        } else {
            return ((xjoeReserves - joeReserves) * deltaPercentage) / DENOMINATOR;
        }
    }
    ...
    /// @dev if xJoeRation < buyThreshold, will buy in open market a % of the deposit
    ...
    {
        uint256 xJoeRatio = getxJoeRatio();
        minimumEstimatedTotal = _amount;
        if (xJoeRatio <= buyThreshold && buyThreshold < DENOMINATOR) {
    ...
    function _depositFor(uint256 _amount, address _for) internal {
        IERC20(joe).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 xJoeRatio = getxJoeRatio();
        uint256 amountToConvert = _amount;
        if (xJoeRatio <= buyThreshold) {
            ...
            if (amountToBuy > 0) {
                uint256 estimatedOutAmount = estimateOutAmount(amountToBuy);
                if (amountToBuy < estimatedOutAmount) {
                    _approveTokenIfNeeded(joe, router, amountToBuy);
                    address[] memory path = new address[](2);
                    path[0] = joe;
                    path[1] = xJoe;
                    uint256 obtainedJoeAmount = IJoeRouter01(router).swapExactTokensForTokens(
                        amountToBuy,
                    ...
                    require(obtainedJoeAmount > amountToBuy, "Transaction is disadvantageous");
                    amountToConvert -= amountToBuy;
                }
            }
        }
        _approveTokenIfNeeded(joe, xJoe, amountToConvert);
        // IERC20(joe).approve(xJoe, amountToConvert);
        IxJoe(xJoe).depositFor(amountToConvert, _for);
    }
