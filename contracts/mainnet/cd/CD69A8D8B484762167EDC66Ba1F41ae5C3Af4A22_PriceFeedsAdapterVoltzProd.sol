...
contract PriceFeedsAdapterVoltzProd is PriceFeedsAdapterWithoutRounds {
  ...
  function getDataFeedIndex(bytes32 dataFeedId) public view override virtual returns (uint256) {
    if(dataFeedId == SOFR_ID) {
      return 0;
    } else if (dataFeedId == SOFR_EFFECTIVE_DATE_ID) {
      return 1;
    } else if (dataFeedId == SOFRAI_ID) {
      return 2;
    } else if (dataFeedId == SOFRAI_EFFECTIVE_DATE_ID) {
      return 3;
    }
  ...
  function requireAuthorisedUpdater(address updater) public view override virtual {
    if (updater != 0xB0ADD54EA5efebd82c579a5bfD609FdAF4066F7A) {
