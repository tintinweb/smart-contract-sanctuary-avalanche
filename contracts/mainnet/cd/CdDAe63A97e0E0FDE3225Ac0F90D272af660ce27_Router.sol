...
contract Router {
	...
	mapping(address => uint256) private poolRewardsShare; // currency (eth, usdc, etc.) => bps
	mapping(address => uint256) private poolShare; // currency (eth, usdc, etc.) => bps
	mapping(address => uint256) private snowShare; // currency => bps

	mapping(address => address) poolRewards; // currency => contract
	mapping(address => address) snowRewards; // currency => contract
	...
	function getPoolRewardsShare(address currency) external view returns(uint256) {
		return poolRewardsShare[currency];
	}
	...
	function setPoolRewardsShare(address currency, uint256 share) external onlyOwner {
		poolRewardsShare[currency] = share;
	}

	function setSnowShare(address currency, uint256 share) external onlyOwner {
		snowShare[currency] = share;
	}
	...
	function setSnowRewards(address currency, address _contract) external onlyOwner {
		snowRewards[currency] = _contract;
	}
