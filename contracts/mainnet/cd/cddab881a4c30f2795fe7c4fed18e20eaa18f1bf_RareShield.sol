...
contract RareShield is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier onlyHolder(uint _tokenId) {
    require(msg.sender == ownerOf(_tokenId), 'only holder.');
    _;
  }
  ...
  // [.√.] mints NFT to sender //
  function mint() external payable mintCompliance() {
  ...
  function mint(address _receiver) external payable mintCompliance() {
    require(!paused, 'contract paused.');
  ...
  function mintEmail(address _receiver, string memory _email) external payable mintCompliance() {
    require(!paused, 'contract paused.');

    _safeMint(_receiver, 1);

    // sets: email associated with token //
    setEmail(totalSupply(), _email);
  }
  ...
  function setDev(address _devAddress) external {
    require(msg.sender == devAddress, 'onlyDev');
  ...
  // [.√.] sends funds to the appropriate addresses //
  function withdraw() external nonReentrant {
    require(msg.sender == devAddress || msg.sender == owner(), 'only dev || owner.');
    // sends 10% to dev //
  ...
  function readEmail(uint _tokenId) public view returns (string memory) {
    return holderInfo[_tokenId].email;
  }
}
