...
contract Distributor is Ownable, IDistributor
{
  ...
  constructor(address[] memory accounts, uint256[] memory points) public
  {
    ...
    {
      allocations.push(Allocation({ account : accounts[a], points : points[a] }));
  ...
  {
    allocations.push(Allocation({ account: account, points: points }));
  ...
  function call(address payable _to, uint256 _value, bytes calldata _data) external payable onlyOwner returns (bytes memory)
  {
    ...
    require(success, "Distributor: External call failed");
