...
contract NoName is ERC721URIStorage, Ownable, KindaRandom {
  ...
  constructor() ERC721("No Name", "NONAME") KindaRandom(32) {
    _customBaseURI = "https://nfts.bpi.network/no_name/";
    _maxMintable = 32; // 32 max supply
    _royaltyAmount = 500; // 5% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 2;
    _price = 5 ether;
    _startTime = 1664906400;
    
    _artist =  0x0595aFf51982ac681414297C7Bc41A394694a80C;
  ...
  function hasEarlyAccess(address account) public view returns (bool) {
    return _earlyAccess[account];
  }

  function onlyEarlyAccess() public view returns (bool) {
    return block.timestamp < (_startTime + 600); // 10 minutes for early access
  }
