...
contract wsFORTSwapper is ISwapper {
    using BoringMath for uint256;
    ...
    IUniswapV2Pair constant FORT_MIM = IUniswapV2Pair(0x3E5F198B46F3dE52761b02d4aC8ef4cECeAc22D6);
    IERC20 public constant POLE = IERC20(0x65069e550C5526c029DC9135eDD02F6683859Ac1);
    IERC20 public constant SFORT = IERC20(0x6B8FB769d1957F2c29aBc9d1bEB95851cce775D8);
    IWSFORT public constant WSFORT = IWSFORT(0xF3823A2504b5C5907fDd0f708efa0198bE4D837C);
    IStakingManager public constant STAKING_MANAGER = IStakingManager(0x4D8ba74820e2d6EaD2Ea154586CB7dfbA8A691aa);
    IFORT public constant FORT = IFORT(0xf6d46849DB378AE01D93732585BEc2C4480D1fD5);
    ...
    constructor() public {
        ...
        SFORT.approve(address(STAKING_MANAGER), type(uint256).max);
    }
    ...
    function swap(
...

        {
            ...
            amountSecond = WSFORT.unwrap(amountFirst);
        }

        STAKING_MANAGER.unstake(amountSecond, false);

        FORT.transfer(address(FORT_MIM), amountSecond);

        {
            (address token0, ) = UniswapV2Library.sortTokens(address(MIM), address(FORT));

            (uint256 reserve0, uint256 reserve1, ) = FORT_MIM.getReserves();

            (reserve0, reserve1) = address(FORT) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);

            amountThird = getAmountOut(amountSecond, reserve0, reserve1);

            (uint256 amount0Out, uint256 amount1Out) = address(FORT) == token0
                ? (uint256(0), amountThird)
                : (amountThird, uint256(0));

            FORT_MIM.swap(amount0Out, amount1Out, address(MIM_USDT), new bytes(0));
        }
...
contract wMEMOV2Swapper is ISwapper {
    using BoringMath for uint256;

    IBentoBoxV1 public constant BentoBox = IBentoBoxV1(0xBBe7bF1c422eFBb5B2cB7a91A6f0AA7CdE86C1d3);
    CurvePool public constant Curve3POOL = CurvePool(0x001E3BA199B4FF4B5B6e97aCD96daFC0E2e4156e);
    IUniswapV2Pair constant USDCe_MIM = IUniswapV2Pair(0x50141C21e4e861d4b2cbEb825b9a2B5E5e09A186);
    ...
    constructor() public {
        USDCe.approve(address(Curve3POOL), type(uint256).max);
        POLE.approve(address(Curve3POOL), type(uint256).max);
    }
    ...
    function swap(
        ...
        {
            (uint256 amountFirst,) = BentoBox.withdraw(fromToken, address(this), address(wMEMO_MIM), 0, shareFrom);
            ...
            wMEMO_MIM.swap(amount0Out, amount1Out, address(USDCe_MIM), new bytes(0));
        }

        {
            (address token0, ) = UniswapV2Library.sortTokens(address(MIM), address(USDCe));

            (uint256 reserve0, uint256 reserve1, ) = USDCe_MIM.getReserves();
            ...
            USDCe_MIM.swap(amount0Out, amount1Out, address(this), new bytes(0));
        }

        {
            amountTo = Curve3POOL.exchange_underlying(address(POLE3POOL), 2, 0, amountThird, 0, address(BentoBox));
        }
...
contract SMELTSwapper is ISwapper {
    ...
    function swap(
...

        {
            ...
            uint256 oldBalanceOf = MELT.balanceOf(address(this));

            STAKING_MANAGER.withdraw(amountFirst);

            uint256 nowBalanceOf = MELT.balanceOf(address(this));

            amountSecond = nowBalanceOf.sub(oldBalanceOf);
        }
...
contract PTPWAVAXLPSwapper is ISwapperGeneric {
    ...
    IBentoBoxV1 public constant BentoBox = IBentoBoxV1(0x640CEE7Fa90c00948b8e390127b4EE01C5396B9a);
    CurvePool public constant Curve3POOL = CurvePool(0x001E3BA199B4FF4B5B6e97aCD96daFC0E2e4156e);
    IUniswapV2Pair constant USDT_WAVAX = IUniswapV2Pair(0xeD8CBD9F0cE3C6986b22002F03c6475CEb7a6256);
...
contract JOEUSDCLPSwapper is ISwapperGeneric {
    using BoringMath for uint256;

    IBentoBoxV1 public constant BentoBox = IBentoBoxV1(0x17dD4518309d9BE182605f44ab5b936Ee4efc084);
    CurvePool public constant Curve3POOL = CurvePool(0x001E3BA199B4FF4B5B6e97aCD96daFC0E2e4156e);
    IUniswapV2Pair constant USDC_JOE = IUniswapV2Pair(0x3bc40d4307cD946157447CD55d70ee7495bA6140);
    IUniswapV2Pair constant USDC_USDCe = IUniswapV2Pair(0x2A8A315e82F85D1f0658C5D66A452Bbdd9356783);
    IERC20 public constant POLE = IERC20(0x65069e550C5526c029DC9135eDD02F6683859Ac1);
    ...
    function swap(
...

        {
            ...
            USDC_JOE.transfer(address(USDC_JOE), amountFirst);
        }

        {
            (uint256 amount0, uint256 amount1) = USDC_JOE.burn(address(this));
            
            (address token0, ) = UniswapV2Library.sortTokens(address(JOE), address(USDC));

            (uint256 reserve0, uint256 reserve1, ) = USDC_JOE.getReserves();

            (reserve0, reserve1) = address(JOE) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);

            uint256 amountToken = address(JOE) == token0 ? amount0 : amount1;

            amountSecond = getAmountOut(amountToken, reserve0, reserve1);

            JOE.transfer(address(USDC_JOE), amountToken);

            (uint256 amount0Out, uint256 amount1Out) = address(JOE) == token0
                    ? (uint256(0), amountSecond)
                    : (amountSecond, uint256(0));

            USDC_JOE.swap(amount0Out, amount1Out, address(this), new bytes(0));
        }

        {
            (address token0, ) = UniswapV2Library.sortTokens(address(USDCe), address(USDC));

            (uint256 reserve0, uint256 reserve1, ) = USDC_USDCe.getReserves();

            (reserve0, reserve1) = address(USDC) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);

            amountThird = USDC.balanceOf(address(this));

            amountFourth = getAmountOut(amountThird, reserve0, reserve1);

            USDC.transfer(address(USDC_USDCe), amountThird);

            (uint256 amount0Out, uint256 amount1Out) = address(USDC) == token0
                    ? (uint256(0), amountFourth)
                    : (amountFourth, uint256(0));

            USDC_USDCe.swap(amount0Out, amount1Out, address(this), new bytes(0));
        }

        {
            amountFifth = USDCe.balanceOf(address(this));

            amountTo = Curve3POOL.exchange_underlying(address(POLE3POOL), 2, 0, amountFifth, 0, address(BentoBox));
        }
...
contract wsFORTLevSwapper {
    ...
    IWSFORT public constant WSFORT = IWSFORT(0xF3823A2504b5C5907fDd0f708efa0198bE4D837C);
    IStakingManager public constant STAKING_MANAGER = IStakingManager(0x4D8ba74820e2d6EaD2Ea154586CB7dfbA8A691aa);
    IERC20 public constant FORT = IERC20(0xf6d46849DB378AE01D93732585BEc2C4480D1fD5);
    IERC20 public constant MIM = IERC20(0x130966628846BFd36ff31a822705796e8cb8C18D);
    ...
    constructor() public {
        POLE.approve(address(Curve3POOL), type(uint256).max);
        FORT.approve(address(STAKING_MANAGER), type(uint256).max);
        SFORT.approve(address(WSFORT), type(uint256).max);
    }
    ...
    function swap(
...

        {
            ...
            MIM_USDT.swap(amount0Out, amount1Out, address(FORT_MIM), new bytes(0));
        }
        
        {
            ...
            (uint256 amount0Out, uint256 amount1Out) = address(MIM) == token0
                    ? (uint256(0), amountFinal)
                    : (amountFinal, uint256(0));
            FORT_MIM.swap(amount0Out, amount1Out, address(this), new bytes(0));
        }
        ...
        amountTo = WSFORT.wrap(amountFinal);

        WSFORT.transfer(address(BentoBox), amountTo);

        (, shareReturned) = BentoBox.deposit(WSFORT, address(BentoBox), recipient, amountTo, 0);
        extraShare = shareReturned.sub(shareToMin);
    }
}
...
contract wMEMOLevV2Swapper{
    ...
    function swap(
...

        {
            ...
            amountSecond = Curve3POOL.exchange_underlying(address(POLE3POOL), 0, 2, amountFirst, 0, address(USDCe_MIM));
...

        {
            ...
            (reserve0, reserve1) = address(USDCe) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
            
            amountThird = getAmountOut(amountSecond, reserve0, reserve1);

            (uint256 amount0Out, uint256 amount1Out) = address(USDCe) == token0
                    ? (uint256(0), amountThird)
                    : (amountThird, uint256(0));

            USDCe_MIM.swap(amount0Out, amount1Out, address(wMEMO_MIM), new bytes(0));
        }

        {
            (address token0, ) = UniswapV2Library.sortTokens(address(MIM), address(wMEMO));
...
contract SMELTLevSwapper {
    ...
    function swap(
        ...
        uint256 oldBalanceOf = SMELT.balanceOf(address(this));

        STAKING_MANAGER.deposit(amountFinal);

        uint256 nowBalanceOf = SMELT.balanceOf(address(this));

        amountTo = nowBalanceOf.sub(oldBalanceOf);
...
contract JOEUSDCLPLevSwapper{
    ...
    constructor() public {
        POLE.approve(address(Curve3POOL), type(uint256).max);
        USDC_JOE.approve(address(BentoBox), type(uint256).max);
    ...
    function swap(
...

        {
            ...
            amountSecond = Curve3POOL.exchange_underlying(address(POLE3POOL), 0, 2, amountFirst, 0, address(USDC_USDCe));
        }

        {
            (address token0, ) = UniswapV2Library.sortTokens(address(USDC), address(USDCe));
...

        {
            ...
            uint256 amountFrom =  USDC.balanceOf(address(this));

            uint256 mimSwapInAmount = _calculateSwapInAmount(reserve0, amountFrom);
            
            USDC.transfer(address(USDC_JOE), mimSwapInAmount);
            ...
            uint256 oldBalance = USDC_JOE.balanceOf(address(this));
            ...
            uint256 newBalance = USDC_JOE.balanceOf(address(this));

            amountTo = newBalance.sub(oldBalance);
        }

        (, shareReturned) = BentoBox.deposit(IERC20(address(USDC_JOE)), address(this), recipient, amountTo, 0);
        extraShare = shareReturned.sub(shareToMin);
    }
}
...
interface JLPStrategy {
    function withdrawAll(address _strategyToken) external;
}
...
/// @dev This contract allows contract calls to any contract (except BentoBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract CauldronV2Strategys is BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    using RebaseLibrary for Rebase;
    using BoringERC20 for IERC20;
    ...
    // Immutables (for MasterContract and all clones)
    IBentoBoxV1 public immutable bentoBox;
    CauldronV2Strategys public immutable masterContract;
    IERC20 public immutable POLE;
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        uint256 amountCollateral = bentoBox.toAmount(collateral, share, true);
        if(address(bentoBox.strategy(collateral)) != address(0) && amountCollateral > nowBalance){
            JLPStrategy(address(bentoBox.strategy(collateral))).withdrawAll(address(collateral));
            needHarvest = true;
        }
    ...
    function cook(
        ...
        if (needHarvest) {
            bentoBox.harvest(collateral, true, 999999999999999999999999000000000000000000);
            needHarvest = false;
        }
    }
    ...
    function liquidate(
        ...
        uint256 nowBalance= IERC20(collateral).balanceOf(address(bentoBox));
        uint256 amountCollateral = bentoBox.toAmount(collateral, allCollateralShare, true);
        if(address(bentoBox.strategy(collateral)) != address(0) && amountCollateral > nowBalance){
            JLPStrategy(address(bentoBox.strategy(collateral))).withdrawAll(address(collateral));
            needHarvest = true;
        }
...
contract NORTHOracle is IOracle {
    using FixedPoint for *;
    using BoringMath for uint256;
    uint256 public constant PERIOD = 10 minutes;

    IAggregator public constant USDT_USD = IAggregator(0xEBE676ee90Fe1112671f19b6B7459bC678B67e8a);
    IUniswapV2Pair public constant NORTH_USDT = IUniswapV2Pair(0x332f909F6bBa8c1B124202F9FEC5d515c7635500);
    address public constant NORTH = 0xD0755413bfE2e08dB6bE72761cdD56d77d4B60f1;
    ...
    PairInfo public pairInfo;
    function _get(uint32 blockTimestamp) public view returns (uint256) {
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = NORTH_USDT.getReserves();

        uint256 priceCumulative = address(NORTH) == NORTH_USDT.token0() ? NORTH_USDT.price0CumulativeLast() : NORTH_USDT.price1CumulativeLast();

        (reserve0, reserve1) = address(NORTH) == NORTH_USDT.token0() ? (reserve1, reserve0) : (reserve0, reserve1);

        priceCumulative += uint256(FixedPoint.fraction(reserve0*1e12, reserve1)._x) * (blockTimestamp - blockTimestampLast);
    ...
    function get(bytes calldata data) external override returns (bool, uint256) {
        ...
        pairInfo.priceAverage = uint144(1e53 / (uint256(1e18).mul(uint256(FixedPoint
            .uq112x112(uint224((priceCumulative - pairInfo.priceCumulativeLast) / timeElapsed))
            .mul(1e18)
            .decode144())).mul(uint256(USDT_USD.latestAnswer())) / 1e9));
        pairInfo.blockTimestampLast = blockTimestamp;
        pairInfo.priceCumulativeLast = priceCumulative;
    ...
    /// @inheritdoc IOracle
    function peekSpot(bytes calldata data) external view override returns (uint256 rate) {
        (uint256 reserve0, uint256 reserve1, ) = NORTH_USDT.getReserves();

        (reserve0, reserve1) = address(NORTH) == NORTH_USDT.token0() ? (reserve1, reserve0) : (reserve0, reserve1);

        rate = 1e53 / (uint256(1e18).mul(reserve0.mul(1e18).mul(1e12) / reserve1).mul(uint256(USDT_USD.latestAnswer())) / 1e9);
    }
...
contract JOEOracle is IOracle {
    ...
    IAggregator public constant JOE_USD = IAggregator(0x02D35d3a8aC3e1626d3eE09A78Dd87286F5E8e3a);
    
    // Calculates the lastest exchange rate
    // Uses both divide and multiply only for tokens not supported directly by Chainlink, for example MKR/USD
    function _get() internal view returns (uint256) {
        return 1e26 / uint256(JOE_USD.latestAnswer());
    }
...
contract CauldronV2Strategy is BoringOwnable, IMasterContract {
    ...
    bool public liquidateManagerOpen;
    mapping(address => bool) public liquidateManager;
    uint64 public oldTargetPercentage = 0;
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        uint256 amountCollateral = bentoBox.toAmount(collateral, share, true);
        if(address(bentoBox.strategy(collateral))!= address(0) && amountCollateral > nowBalance){
            (, uint64 oldPercentage, ) = bentoBox.strategyData(collateral);
            oldTargetPercentage = oldPercentage;
            bentoBox.setStrategyTargetPercentage(collateral, 0);
            bentoBox.harvest(collateral, true, 999999999999999999999999000000000000000000);
        }
    ...
    function cook(
        ...
        if (oldTargetPercentage > 0) {
            bentoBox.setStrategyTargetPercentage(collateral, oldTargetPercentage);
            bentoBox.harvest(collateral, true, 999999999999999999999999000000000000000000);
            oldTargetPercentage = 0;
        }
    }
    ...
    function liquidate(
        ...
        if(address(bentoBox.strategy(collateral))!= address(0)){
            (, uint64 oldPercentage, ) = bentoBox.strategyData(collateral);
            oldTargetPercentage = oldPercentage;
            bentoBox.setStrategyTargetPercentage(collateral, 0);
            bentoBox.harvest(collateral, true, 999999999999999999999999000000000000000000);
        ...
        uint64 oldPercentage;
        if(address(bentoBox.strategy(collateral)) != address(0)){
            (, oldPercentage, ) = bentoBox.strategyData(collateral);
            bentoBox.setStrategyTargetPercentage(collateral, 0);
            bentoBox.harvest(collateral, true, 999999999999999999999999000000000000000000);
        }
