...
contract BFGMysteryBox is ERC721A, Ownable {
    ...
    {
        require(
        msg.value == _quantity * neededAvax,
        "Invalid payment !"
        );
        _;
    }
    ...
    //only Owner
    function changeMaxPrivate(uint256 _value) public onlyOwner {
    ...
    function mintAVAX(uint256 _quantity,string memory _code) public payable costs(_quantity){
        //check the quantity sent
        require(_quantity > 0,"Selected quantity of mint boxes must be greater than 0.");
        ...
        if(!startPublic)
        {
            require(newSupply <= MAX_PRIVATE, "Max supply for private round reached");
            //check if valid code
            require(
                (giveaway_address[_code] != address(0)) ||
                    (affiliate_codeAddress[_code] != address(0)),
        ...
        _modifyState(msg.sender, _quantity, _code);
        _mintNFT(msg.sender, _quantity);
    }

    function mintUSDT(uint256 amount, uint256 _quantity,string memory _code) public{
    ...
    function _modifyState(address player, uint256 quantity_, string memory _code) internal{
