...
contract MasterChef is Ownable {
    ...
    uint256 public tokenWeight1 = 80;
    uint256 public tokenWeight2 = 20;
    uint256 public BLOCKS_PER_YEAR = 42561;
    ...
    function add(
        ...
        uint256 lastRewardBlock = block.number > startBlock
            ...
            : startBlock;
        if (_isNFT) {
            totalAllocPointNft = totalAllocPointNft.add(_allocPoint);
        } else {
    ...
    function _setMigrator(IMigratorChef _migrator) public {
        require(msg.sender == owner() || msg.sender == devaddr, "!dev addr");
        migrator = _migrator;
    }

    // Migrate lp token to another lp contract. Can be called by anyone. We trust that migrator contract is good.
    function _migrate(uint256 _pid) public {
    ...
    function getMultiplier(
        ...
        if (_to <= bonusEndBlock) {
            if (_isNFT) {
                return _to.sub(_from).mul(100 - tokenWeight1).div(100);
            } else {
                return _to.sub(_from).mul(tokenWeight1).div(100);
            }
        } else if (_from >= bonusEndBlock) {
            if (_isNFT) {
                return _to.sub(_from).mul(100 - tokenWeight2).div(100);
            } else {
                return _to.sub(_from).mul(tokenWeight2).div(100);
            }
        } else {
            if (_isNFT) {
                return
                    bonusEndBlock
                        .sub(_from)
                        .mul(100 - tokenWeight1)
                        .div(100)
                        .add(
                            _to.sub(bonusEndBlock).mul(100 - tokenWeight2).div(
                                100
                            )
                        );
            } else {
                return
                    bonusEndBlock.sub(_from).mul(tokenWeight1).div(100).add(
                        _to.sub(bonusEndBlock).mul(tokenWeight2).div(100)
                    );
            }
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sushiReward = multiplier.mul(sushiPerBlock).mul(
                pool.allocPoint
            );
            if (pool.isNFT) {
                sushiReward = sushiReward.div(totalAllocPointNft);
            } else {
                sushiReward = sushiReward.div(totalAllocPoint);
            }
            accSushiPerShare = accSushiPerShare.add(
                sushiReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accSushiPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (user.amount > 0)
    ...
    function getYield(uint256 _pid) public view returns (uint256 yield) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 baseYield = sushiPerBlock
            .mul(BLOCKS_PER_YEAR)
            .mul(pool.allocPoint)
            .div(100);

        if (pool.isNFT && block.number <= bonusEndBlock) {
            return baseYield.mul(100 - tokenWeight1).div(totalAllocPointNft);
        }
        if (pool.isNFT && block.number > bonusEndBlock) {
            return baseYield.mul(100 - tokenWeight2).div(totalAllocPointNft);
        }
        if (!pool.isNFT && block.number <= bonusEndBlock) {
            return baseYield.mul(tokenWeight1).div(totalAllocPoint);
        }
        if (!pool.isNFT && block.number > bonusEndBlock) {
            return baseYield.mul(tokenWeight2).div(totalAllocPoint);
        }
    }
