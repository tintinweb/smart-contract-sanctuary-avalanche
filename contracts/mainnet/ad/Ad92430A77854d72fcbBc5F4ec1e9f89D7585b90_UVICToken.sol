...
contract UVICToken is ERC20, Operator {
    using SafeMath for uint256;
	
	uint256 public constant INITIAL_FRAGMENTS_SUPPLY = 200592 ether;
    ...
    constructor(address _polWallet, address _daoFundAddress, address _usdceAddress, address _router) ERC20("USDC.e Victory", "UVIC") {
		require(_polWallet != address(0), "!_polWallet");
		require(_daoFundAddress != address(0), "!_daoFundAddress");
		require(_usdceAddress != address(0), "!_usdc.eAddress");
		require(_router != address(0), "!_router");
		_gonsPerFragment = 10**18;
		_totalSupply = 0;
		lastTimeRebase = 0;
		polWallet = _polWallet;
		taxTiersTwaps = [0, 8e5, 9e5, 1e6]; // twap with usdc.e
		...
		address dexPair = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _usdceAddress);
        setMarketLpPairs(dexPair, true);
		// Mints 200592 tokens to DAO fund for swap AVIC
        _mint(_daoFundAddress, INITIAL_FRAGMENTS_SUPPLY);
    }
