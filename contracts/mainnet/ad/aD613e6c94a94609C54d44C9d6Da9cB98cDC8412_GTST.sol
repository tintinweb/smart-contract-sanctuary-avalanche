...
contract GTSTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    function _approve(
        ...
        require(false, "GTST: No approvals allowed");
    }
    ...
    function _transfer(
        ...
        require(false, "GTST: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GTST: withdrawDividend disabled. Use the 'claim' function on the main GTST contract."
        );
    }
...
contract GTST is ERC20, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    string private constant _name = "GTST";
    string private constant _symbol = "GTST";
    uint8 private constant _decimals = 18;
    uint256 private constant _tTotal = 1e12 * 10**18;


    // pangolin avax fuji: 0x2D99ABD9008Dc933ff5c0CD271B88309593aB921
    // traderjoe router avax mainnet: 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
    // traderjoe router avax testnet: 0x5db0735cf88F85E78ed742215090c465979B5006
    // uniswap eth mainnet: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    ...
    uint256 private minimumMyBagsToken = 1 * 10**18;
    ...
    uint256 public sellPercentageOfHolding = 20; // how much per interval can be sold w/o penalty tax
    uint256 public minutesIntervalPerSell = 7200 minutes; // 120h = 5d
    ...
    GTSTDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = new GTSTDividendTracker();
    ...
    function openTrading(uint256 _launchTime, uint256 _minutesForPrivateSale)
        ...
        onlyOwner
    {
        require(!tradingOpen, "GTST: Trading is already open");
        require(_launchTime > block.timestamp, "GTST: Invalid timestamp");
    ...
    function _claim(address payable _account, bool _reinvest) private {
        ...
        require(
            ...
            "GTST: Claimer has no withdrawable dividend"
        );
        uint256 ethAmount;
        uint256 tokenAmount;
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                ...
                "GTST: Private and public sale is not open"
            );
            if (
                privateSaleTimestamp <= block.timestamp &&
        ...
        if (!automatedMarketMakerPairs[_to] && !isExludeFromWalletLimit[_to]) {
            ...
            require(
                ...
                "GTST: Wallet balance limit reached"
            );
        }
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool _value) private {
        require(
            ...
            "GTST: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[_pair] = _value;
    ...
    function distributeTokensEth(uint256 _tokenAmount) private {
        ...
        uint256 totalEth = swapTokensForAVAX(tokensForSwap);
    ...
    function swapTokensForAVAX(uint256 _tokenAmount) private returns (uint256) {
    ...
    function addLiquidity(uint256 _tokenAmount, uint256 _ethAmount) private {
        _approve(address(this), address(uniswapV2Router), _tokenAmount);
        uniswapV2Router.addLiquidityAVAX{value: _ethAmount}(
            address(this),
            _tokenAmount,
            0,
            0,
    ...
    function swapAndSendDividends(uint256 _tokenAmount) private {
        uint256 dividends = swapTokensForAVAX(_tokenAmount);
