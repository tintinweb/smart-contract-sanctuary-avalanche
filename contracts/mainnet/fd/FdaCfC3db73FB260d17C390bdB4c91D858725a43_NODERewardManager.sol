...
contract NODERewardManager is PaymentSplitter {
	...
	function createNodeWithTokens(string memory nodeTypeName, uint256 count) public {
		...
		uint256 nodePrice = getNodeTypePrice(nodeTypeName) * count;
		require(
	...
	function getNodeTypesAllAt(uint256 start, uint256 end) public view returns(uint256[][] memory) {
		...
		for (uint256 i = start; i < end; i++) {
			uint256[] memory cur = new uint256[](11);
			nt = _nodeTypes.getValueAtIndex(i);
			cur[0] = nt.nodePrice;
			cur[1] = nt.claimTime;
			cur[2] = nt.rewardAmount;
			cur[3] = nt.claimTaxBeforeTime;
			cur[4] = nt.count;
			cur[5] = nt.max;
			cur[6] = nt.earlyClaimTax;
			cur[7] = nt.maxLevelUpGlobal;
			cur[8] = nt.maxLevelUpUser;
			cur[9] = nt.maxCreationPendingGlobal;
			cur[10] = nt.maxCreationPendingUser;
			all[i - start] = cur;
		}
		return all;
	}
	...
	function getNodeTypePrice(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 0);
	}

	function getNodeTypeClaimTime(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 1);
	}

	function getNodeTypeRewardAmount(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 2);
	}

	function getNodeTypeClaimTaxBeforeTime(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 3);
	}
	
	function getNodeTypeCount(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 4);
	}
	
	function getNodeTypeMax(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 5);
	}
	
	function getNodeTypeEarlyClaimTax(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 6);
	}
	
	function getNodeTypeMaxLevelUpGlobal(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 7);
	}

	function getNodeTypeMaxLevelUpUser(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 8);
	}
	
	function getNodeTypeMaxCreationPendingGlobal(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 9);
	}

	function getNodeTypeMaxCreationPendingUser(string memory nodeTypeName) public view returns(uint256) {
		return getNodeTypeNameData(nodeTypeName, 10);
	}
	...
	function updateGateKeeper(address _new)
        external onlySentry
    {
        _gateKeeper = _new;
    }
