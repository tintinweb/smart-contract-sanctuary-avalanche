...
contract StakingRewardsNFT is ReentrancyGuard, Ownable {
    ...
    function stake(uint256[] calldata tokenId) external nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < tokenId.length; i++) {
            ...
            addToOwnerTokens(msg.sender, tokenId[i]);
    ...
    function withdraw(uint256[] memory tokenId) public nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < tokenId.length; i++) {
            ...
            removeFromOwnerTokens(msg.sender, tokenId[i]);
            _balances[msg.sender] -= 1;
            delete _stakings[tokenId[i]];
        }
    ...
    function addToOwnerTokens(address owner, uint256 tokenId) private {
        uint256 length = _balances[owner];
        _ownedTokens[owner][length] = tokenId;
        _ownedTokensIndex[tokenId] = length;
    }

    function removeFromOwnerTokens(address owner, uint256 tokenId) private {
        uint256 lastTokenIndex = _balances[owner] - 1;
