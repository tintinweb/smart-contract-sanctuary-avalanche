...
import "./interfaces/IBalanceOfAstro.sol";
import "./interfaces/IDEXPair.sol";
import "./interfaces/IAstroToken.sol";
import "./interfaces/IAstroSettings.sol";
...
contract AstroToken is ERC20Upgradeable, OwnableUpgradeable, IAstroToken {
  ...
  uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 5616000508158465660602786996;
  ...
  IAstroSettings public settings;
  ...
  address[] public astroGamesContracts;
  ...
  mapping(address => bool) public astroGamesCheck;
  ...
  function init() public initializer {
    __Ownable_init();
    __ERC20_init('100Days Ventures', 'ASTRO');
    ...
    maxInvestRemovablePerPeriod = 2000000 * 10 ** 18;
    ...
    isTotalFeeExempt[astroSwapper] = true;
  ...
  function _transferFrom(
    ...
    // We should limit how much a user can deposit in astro games
    if(astroGamesCheck[recipient] && isGameDepositLimited) {
      InvestorInfo storage investor = investorInfoMap[sender];
    ...
    if(astroGamesCheck[recipient] || astroGamesCheck[sender]) {
  ...
  //this function iterates through all other contracts that are being part of the Astro ecosystem
  //we add a new contract like wASTRO or sASTRO, whales could technically abuse this
  ...
  function getBalanceContracts(address sender) public view returns (uint256) {
    ...
    for (uint256 i = 0; i < subContracts.length; i++) {
      userTotal += (IBalanceOfAstro(subContracts[i]).balanceOfAstro(sender));
    }
    for (uint256 i = 0; i < astroGamesContracts.length; i++) {
      userTotal += (IERC20(astroGamesContracts[i]).balanceOf(sender));
    }
  ...
  function getCirculatingSupply() external view returns (uint256) {
    return
    (TOTAL_GONS - _gonBalances[DEAD] - _gonBalances[ZERO] - _gonBalances[ONE] - _gonBalances[treasuryReceiver]) /
    gonsPerFragment;
  }
  ...
  returns (uint256)
  {
    ...
    uint256 userTotal = getUserTotalOnDifferentContractsAstro(_address);
  ...
  function getUserTotalOnDifferentContractsAstro(address sender)
  public
  view
  returns (uint256)
  {
  ...
  function takeGameFee(
    ...
    require(astroGamesCheck[sender] || astroGamesCheck[recipient], "Non astro game transfer");

    IAstroSettings.GameFees memory fees = settings.currentGameFees();

    uint256 feeGonAmount = (originalGonAmount * (fees.stakeFee)) / (FEE_DENOMINATOR);
    uint256 feeAmount = feeGonAmount / gonsPerFragment;

    if (astroSwapper != address(0x0)) {
      _gonBalances[astroSwapper] += feeGonAmount;
      emit Transfer(sender, astroSwapper, feeAmount);
    } else {
  ...
  function takeFee(
    ...
    if(automatedMarketMakerPairs[sender]) {
      IAstroSettings.BuyFees memory buyFees = settings.currentBuyFees();
      _realFee = isBuyFeeExempt[sender] ? 0 : buyFees.totalFees;

      if (isWall) {
        _realFee = _realFee / (wallDivisor);
      }

    // Sell order
    } else if(automatedMarketMakerPairs[recipient]) {
      IAstroSettings.SellFees memory sellFees = settings.currentSellFees();
    ...
    // Transfer order
      ...
      IAstroSettings.TransferFees memory fees = settings.currentTransferFees();
      _realFee = fees.totalFees;
    }
    ...
    // NOTE: take a share of all buy / sell fees for astro game prize pool
    uint256 feeAstroGamePoolGonAmount = 0;
    uint256 feeAstroGamePoolAmount = 0;

    if(astroGamePool != address(0)) {
      IAstroSettings.GameFees memory gameFees = settings.currentGameFees();
      feeAstroGamePoolGonAmount = feeGonAmount * gameFees.depositLimit / FEE_DENOMINATOR;
      feeAstroGamePoolAmount = feeAstroGamePoolGonAmount / gonsPerFragment;

      _gonBalances[astroGamePool] += feeAstroGamePoolGonAmount;
      emit Transfer(sender, astroGamePool, feeAstroGamePoolAmount);

      feeGonAmount -= feeAstroGamePoolGonAmount;
    ...
    return gonAmount - (feeGonAmount + feeAstroGamePoolGonAmount);
  }
  ...
  function setAstroSettings(address _settings) external onlyOwner {
    require(_settings != address(0x0), "Zero settings");
    settings = IAstroSettings(_settings);
  }
  ...
  function addAstroGamesAddies(address _astroGamesAddy, bool _value)
  external
  onlyOwner
  {
    require(
      astroGamesCheck[_astroGamesAddy] != _value,
      'Value already set'
    );
    ...
    if (_value) {
      astroGamesContracts.push(_astroGamesAddy);
    } else {
      for (uint256 i = 0; i < astroGamesContracts.length; i++) {
        if (astroGamesContracts[i] == _astroGamesAddy) {
          astroGamesContracts[i] = astroGamesContracts[
          astroGamesContracts.length - 1
          ];
          astroGamesContracts.pop();
          break;
        }
      }
    }

    emit SetAstroGamesAddresses(_astroGamesAddy, _value);
  }
  ...
  function setFeeReceivers(
    ...
    address _astroSwapper,
    address _astroGamePool
  ) external onlyOwner {
    liquidityReceiver = _liquidityReceiver;
    treasuryReceiver = _treasuryReceiver;
    riskFreeValueReceiver = _riskFreeValueReceiver;
    galaxyBondReceiver = _galaxyBondReceiver;
    astroSwapper = _astroSwapper;
    astroGamePool = _astroGamePool;
  }
...
interface IBalanceOfAstro {
  function balanceOfAstro(address _address) external view returns (uint256);
}
...
interface IAstroToken {
  ...
  event SetAstroGamesAddresses(address indexed pair, bool indexed value);
