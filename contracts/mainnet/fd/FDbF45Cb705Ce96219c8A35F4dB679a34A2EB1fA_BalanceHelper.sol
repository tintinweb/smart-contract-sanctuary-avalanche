...
contract BalanceHelper is Ownable {
    ...
    address public USP = 0xdaCDe03d7Ab4D81fEDdc3a20fAA89aBAc9072CE2;
    address public ankrETH = 0x12D8CE035c5DE3Ce39B1fDD4C1d5a745EAbA3b8C;
    ...
    function getTokenPricePairedWithAvax(address token) public view returns (uint256 tokenPrice) {
        ...
        if (token == ankrETH) {
            return ((IyyAvax(ankrETH).ratio()) / getTokenPricePairedWithAvax(ETH));
        }
        ...
        returns (uint256 allowanceAmount)
    {
        ...
        (, , , , , address helper) = IMainStakingJoe(mainStaking).getPoolInfo(pool);
