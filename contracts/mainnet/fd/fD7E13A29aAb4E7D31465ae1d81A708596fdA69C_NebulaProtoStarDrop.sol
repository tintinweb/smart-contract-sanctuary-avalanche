...
contract NebulaProtoStarDrop is Ownable{
	...
	function createProtoFeeToken(string memory _name) payable external{
		...
		address _account = msg.sender;
		uint256 _send = msg.value;
		uint256 _fee = over.getFee();
		uint256 _total = _fee.mul(_intervals);
		AvaxCheck(_account,_total);
		treasury.transfer(_fee);
		for(uint i=0;i<_intervals;i++){
			updateFees(_account);
		}
		payable(_account).transfer(_send.sub(_total));
	}
	function createProtoAvax(string memory _name) payable external{
		address _account = msg.sender;
		uint256 _send = msg.value;
		uint256 _fee = over.getFee();
		AvaxCheck(_account,_fee);
		uint256 _amount = _send.sub(_fee);
		treasury.transfer(_fee);
		payable(_account).transfer(_send.sub(_fee));
		updateCreateProto(_account,_name);
	}
	function AvaxCheck(address _account,uint256 _amount) internal {
		tallyAccount(_account);
		checkDrops(_account);
		checkAvaxBalanceOfUser(_account,_amount);
		require(msg.value >= _amount,"you have not sent enough to pay a fee");
	}
	   ...
	   require(allowance >= _total, "Check the token allowance");
    	...
    	function updateCreateProto(address _account,string memory _name) internal {
    		if(airdrop[_account].feesPaid > airdrop[_account].claimed){
			updateFees(_account);
		}
