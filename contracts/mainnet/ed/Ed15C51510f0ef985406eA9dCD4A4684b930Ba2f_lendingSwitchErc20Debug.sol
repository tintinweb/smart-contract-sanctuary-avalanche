...
abstract contract superTokenInterface is ERC20{
    ...
    function checkRewardToken(address rewardToken)internal virtual;
    function onDeposit(address account,uint256 _amount)internal virtual returns(uint256);
    ...
    function onGetRate() internal virtual view returns(uint256);
    function getTotalUnderlying() internal virtual view returns (uint256);
    ...
    function setTokenInfo(string memory _prefixName,string memory _prefixSympol)internal{
        ...
        string memory tokenName_ = string(abi.encodePacked(_prefixName,midName,underlying.name()));
        string memory symble_ = string(abi.encodePacked(_prefixSympol,midSymbol,underlying.symbol()));
        setErc20Info(tokenName_,symble_,underlying.decimals());
    }
    function availableBalance() external view returns (uint256){
        return getAvailableBalance();
    }
    function totalUnderlying() internal virtual view returns (uint256){
        return getTotalUnderlying();
    }
...
contract superSwitchErc20 is baseSuperToken,superAaveErc20Impl,superQiErc20Impl{    
    ...
    function checkRewardToken(address rewardToken)internal virtual override(superAaveTokenImpl,superQiTokenImpl,superTokenInterface){ 
        if (lendingSwitch == 0){
            superAaveTokenImpl.checkRewardToken(rewardToken);
        }else{
            superQiTokenImpl.checkRewardToken(rewardToken);
        }
    }
    ...
    function getTotalUnderlying() internal virtual override(superAaveTokenImpl,superQiTokenImpl,superTokenInterface) view returns (uint256){
        if (lendingSwitch == 0){
            return superAaveTokenImpl.getTotalUnderlying();
        }else{
            return superQiTokenImpl.getTotalUnderlying();
        }
    }
    function onDeposit(address account,uint256 _amount)internal virtual override(superAaveErc20Impl,superQiErc20Impl,superTokenInterface) returns(uint256){
        if (lendingSwitch == 0){
            return superAaveErc20Impl.onDeposit(account,_amount);
        }else{
            return superQiErc20Impl.onDeposit(account,_amount);
        }
    }
    function onWithdraw(address account,uint256 _amount)internal virtual override(superAaveErc20Impl,superQiErc20Impl,superTokenInterface) returns(uint256){
        if (lendingSwitch == 0){
            return superAaveErc20Impl.onWithdraw(account,_amount);
        }else{
    ...
    function lendTo(address account,uint256 amount) internal virtual override(superAaveTokenImpl,superQiErc20Impl,superTokenInterface){
        if (lendingSwitch == 0){
            superAaveTokenImpl.lendTo(account,amount);
        }else{
            superQiErc20Impl.lendTo(account,amount);
        }
    }
    function repayFrom(address account,uint256 amount) internal virtual override(superAaveTokenImpl,superQiErc20Impl,superTokenInterface){
        if (lendingSwitch == 0){
            superAaveTokenImpl.repayFrom(account,amount);
        }else{
            superQiErc20Impl.repayFrom(account,amount);
        }
    }
}
...
abstract contract superQiTokenImpl is superTokenInterface{
    ...
    constructor(address _lendingToken){
        ...
        _setReward(benqiRewardInfos,0,0,false,0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5,1e17);//qi
        _setReward(benqiRewardInfos,1,1,false,address(0),1e15);//avax
    }
    ...
    function getTotalUnderlying() internal virtual override view returns (uint256){
        uint256 exchangeRate = ICErc20(address(qiToken)).exchangeRateStored();
        return exchangeRate.mul(qiToken.balanceOf(address(this)))/calDecimals;
    }
    function checkRewardToken(address rewardToken)internal virtual override{ 
        require(rewardToken != address(qiToken), "reward token error!");
    }
...
abstract contract superQiErc20Impl is superQiTokenImpl {
    ...
    constructor(address _lendingToken) superQiTokenImpl(_lendingToken){
        underlying = IERC20(ICErc20(address(qiToken)).underlying());
        SafeERC20.safeApprove(underlying, address(qiToken), uint(-1));
    }
    function onDeposit(address account,uint256 _amount)internal virtual override returns(uint256){
        underlying.safeTransferFrom(account, address(this), _amount);
        return qiSupply(feeRate[enterFeeID]);
    }
    function onWithdraw(address account,uint256 _amount)internal virtual override returns(uint256){
        ...
        underlying.safeTransfer(account, _amount);
        return _amount;
    }
...
abstract contract superAaveTokenImpl is superTokenInterface {
    ...
    constructor(address _lendingToken){
        ...
        _setReward(aaveRewardInfos,0,0,false,0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7,1e15);//wavax
    }
    function getAvailableBalance() internal virtual view override returns (uint256){
        return aavaToken.balanceOf(address(this));
    }
    function getTotalUnderlying() internal virtual view override returns (uint256){
        return aavaToken.balanceOf(address(this));
    }
    function checkRewardToken(address rewardToken)internal virtual override{ 
        require(rewardToken != address(aavaToken), "reward token error!");
    }
    ...
    function aaveWithdraw()internal{
        aavePool.withdraw(address(underlying), aavaToken.balanceOf(address(this)), address(this));
    }
...
abstract contract superAaveErc20Impl is superAaveTokenImpl {
    ...
    constructor(address _lendingToken) superAaveTokenImpl(_lendingToken){
        underlying = IERC20(IATokenV3(address(aavaToken)).UNDERLYING_ASSET_ADDRESS());
        underlying.safeApprove(address(aavePool), uint(-1));
    }
    function onDeposit(address account,uint256 _amount)internal virtual override returns(uint256){
        underlying.safeTransferFrom(account, address(this), _amount);
        return aaveSupply(feeRate[enterFeeID]);
    }
    function onWithdraw(address account,uint256 _amount)internal virtual override returns(uint256){
...
contract lendingSwitchErc20 is superSwitchErc20,lendingSuperToken {
    ...
    function getTotalUnderlying() internal virtual override(superSwitchErc20,superTokenInterface) view returns (uint256){
...
//        authorizedAccounts[leverageFactory] = 1;
    ...
    **/
    ...
    function borrow(bytes32 account,uint256 amount) external isAuthorized returns(uint256) {
        ...
        lendTo(msg.sender,amount);
        emit Borrow(msg.sender,account, address(underlying),amount);
        return amount;
    }
    function repay(bytes32 account,uint256 amount) external payable isAuthorized {
        ...
        if(amount>_repayDebt){
            ...
            if (fee>0){
                underlying.safeTransferFrom(msg.sender, feePool, fee);
            }
            amount = amount.sub(fee);
        }
        repayFrom(msg.sender,amount);
        emit Repay(msg.sender,account,address(underlying),amount);
    }
...
abstract contract baseSuperToken is timeLockSetting,superTokenInterface,proxyOwner,ReentrancyGuard{
    ...
    event Enter(address indexed sender,uint256 amountIn,uint256 amountOut);
    event Leave(address indexed sender,uint256 amountIn,uint256 amountOut);
    ...
    constructor(address multiSignature,address origin0,address origin1,
        ...
        feeRate[enterFeeID] = 1e15;
        feeRate[flashFeeID] = 1e14;
    }
    function onGetRate() internal virtual override view returns(uint256){
        // Gets the amount of superToken in existence
        uint256 totalShares = totalSupply();
        // Calculates the amount of stakeToken the superToken is worth
        if (totalShares>0){
            return getTotalUnderlying().mul(calDecimals)/totalShares;
        }
        return calDecimals;
    }
    ...
    function enter(uint256 _amount) external payable nonReentrant{
        ...
        uint256 totaStake = getTotalUnderlying();
        // Gets the amount of superToken in existence
        uint256 totalShares = totalSupply();
        _amount = onDeposit(msg.sender,_amount);
    ...
    function leave(uint256 _share) external nonReentrant{
        ...
        uint256 _amount = onWithdraw(msg.sender, what);
...
// Contract owner can withdraw everything from debug version contract.
...
abstract contract baseInterestEngineHash{
    ...
    function getAssetBalance(bytes32 account)public virtual view returns(uint256){
