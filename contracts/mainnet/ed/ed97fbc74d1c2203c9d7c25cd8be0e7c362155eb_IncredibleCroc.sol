...
contract IncredibleCroc is ERC721 {
    ...
    constructor()
        ERC721("Incredible Croc", "AvaxCroc")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 144) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihxrkipd3ns4luiktbcqyssraiw4vm5a3q3rpntnag6tyyas7bmoa/", Strings.toString(tokenId), ".json")
            );
    }
