...
contract CashTestToken is ERC20, Ownable {
    ...
    constructor() ERC20("CashTestToken", "CASH") { }

    /// @dev generate tokens and distributes to testing account
    function mintFor(address _acc, uint256 _amount) external onlyOwner {
        _mint(_acc, _amount * Constant.TOKEN_SCALE);
    }

    /// @dev generate tokens and distributes to list of testing account
    function mintForList(address[] memory _accs, uint256 _amount) external onlyOwner {
        require(_accs.length > 0, "Invalid minted accounts");
        require(_amount > 0, "Invalid minted amount");

        uint256 mintedAmount = _amount * Constant.TOKEN_SCALE;
