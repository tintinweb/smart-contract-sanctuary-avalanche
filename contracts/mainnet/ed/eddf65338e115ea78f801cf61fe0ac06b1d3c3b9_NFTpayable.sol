...
abstract contract prevNFTpayable is Context {
    ...
    function PROTOaccountData(address _account, uint256 _x) external virtual returns(uint256,uint256,uint256,uint256,uint256,bool,bool,bool);
}
...
contract NFTpayable is Ownable {
    ...
    function createProto(address _account) external {
    	...
    	protos.push(PROTOstars({
    	    nextDue:block.timestamp.add(gracePeriod),
    	    feeFroze:froze,
    ...
    function collapseProto(address _account, uint256 _x) internal {
    	...
    	stars.push(DEADStars({
    	    ...
    	    quantumShift:proto.quantumShift,
    	    insolvent:proto.insolvent,
    	    collapsed:proto.collapsed
    	    }));
    	}
    function queryFees(address _account, bool _x) internal returns (uint256) {
    	PROTOstars[] storage protos = protostars[_account];
    	uint256 low;
    	uint256 lowTime;
    	uint256 total;
    	...
    	for(uint i = 0;i<protos.length;i++) {
    		...
    		if(proto.collapsed == false && proto.quantumShift == false){
	    		uint256 paid = proto.nextDue - block.timestamp;
	    		uint256 remainingLife = proto.collapseDate - block.timestamp;
	    		total = remainingLife.div(gracePeriod);
	    		proto.protoElapsed = block.timestamp - proto.creationTime;
	    		if (remainingLife <= 0 || proto.protoElapsed >= protoLife || proto.collapseDate <= block.timestamp){
	    			proto.collapsed = true;
	    		}else if((block.timestamp - proto.nextDue) > gracePeriod){
	    			proto.quantumShift = true;
	    		} else if((paid) > lowTime && (remainingLife - paid) >= feePeriod && proto.collapsed == false && proto.collapsed == false) {
	    			low = i;
	    			lowTime = paid;
	    			owed.feesOwed += 1;
	    			uint256 futPayments = maxFeePayment - paid;
	    			if ( futPayments > feePeriod && remainingLife > feePeriod){
	    				uint256 fictRemaining = remainingLife;
	    				for(uint j = 0;j< futPayments.div(feePeriod);j++){
	    					if (fictRemaining >0){
	    						owed.futureFees += 1;
	    						fictRemaining -=feePeriod;
	    					}
	    				}
	    			}
			} 
	...
	if (lowTime != 0 && _x == true){
		PROTOstars storage proto = protos[low];
		proto.nextDue += feePeriod;
		proto.collapseDate = block.timestamp + (protoLife - proto.protoElapsed);
		owed.feesPaid += 1;
		total -= 1;
    ...
    function payFees(address _account) payable external {
        uint256 _total = queryFees(_account,false);
        require(_total > 0, "you dont owe any fees");
        uint256 avaxPrice = _overseer.getEm();
    	uint256 fee = uint256(15*(10**18)).div(avaxPrice);
    	uint256 ogVal = msg.value;
    	uint256 balance = getBalanceOf(_account);
    	uint256 intervals = ogVal.div(fee);
    	uint256 total = maxFeePayment.mul(100);
    	
    	require(balance >= fee && intervals > 0,"you do not have enough AVAX to pay the fee");
	for(uint i=0;i<intervals;i++){
		if(ogVal >= fee && total > 0 && intervals >0){ 
	    		total = queryFees(_account,true);
	    		_transfer(treasury, fee);
	    		ogVal -= fee;
	    		intervals -=1;
	    	}else{
	    		total = queryFees(_account,false);
	    	}
	}
    }   		
   ...
   function queryERC20Balance(address _account,address _token) internal view returns (uint) {
    	    	...
    	    	for(uint i=0;i<accts;i++){
    	    		...
    	    		if(isInList(_account,PROTOtransfered) == false){
	    	    		...
	    	    		for(uint j=0;j<P_stars;j++){
		    	    		(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e,bool f,bool g,bool h) = _prev.PROTOaccountData(_account,j);
    ...
    function PROTOaccountData(address _account, uint256 _x) external onlyGuard onlyManager(msg.sender) returns(uint256,uint256,uint256,uint256,uint256,bool,bool,bool){
    		PROTOstars[] storage stars = protostars[_account];
    		PROTOstars storage star = stars[_x];
    		return (star.nextDue,star.feeFroze,star.creationTime, star.protoElapsed,star.collapseDate,star.quantumShift,star.insolvent,star.collapsed);
    	}
