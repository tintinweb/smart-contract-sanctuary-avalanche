...
contract MuOracle{
    ...
    constructor(){
       ...
       avax_usdc = LP(0xf4003F4efBE8691B60249E6afbD307aBE7758adb);
...
//avax pool trader joes
       function internal_mu_avax_traderjoes_pool_info() internal view returns(address LPpool, address token0, address token1, uint112 _reserve0,uint112 _reserve1, uint256 muAVAX, uint256 muAVAXusd, uint32 _blockTimestampLast){
        ...
        return (0xdAD81eabB09Be931b305b326E0839Ad9434A7707, _token0, _token1, __reserve0, __reserve1, muAVAX, ((muAVAX/10**9) * (internal_get_avax_usd_price()/10**9)), __blockTimestampLast);
    }
    function get_mu_avax_traderjoes_pool_info() public view returns (address LPpool, address avax_token0, address mu_token1, uint112 avax_reserve0,uint112 mu_reserve1, uint256 muAVAX, uint256 muAVAXusd, uint32 _blockTimestampLast){
        return internal_mu_avax_traderjoes_pool_info();
    }

    
    //avax pool sushi swap
       function internal_mu_avax_sushiswap_pool_info() internal view returns(address LPpool, address token0, address token1, uint112 _reserve0,uint112 _reserve1, uint256 muAVAX, uint32 _blockTimestampLast){
        (uint112 __reserve0, uint112 __reserve1, uint32 __blockTimestampLast) = mu_avax_sushiswap.getReserves();
        address _token0 = mu_avax_sushiswap.token0();
        address _token1 = mu_avax_sushiswap.token1();
        uint256 avax = uint256(__reserve0);
        uint256 mu = uint256(__reserve1);
        avax = avax * 10**18;
        uint256 muAVAX = avax/mu;
        return (0xBF12Ca918c7e8Cb260F480e74fdb99F5E0DE5E8B, _token0, _token1, __reserve0, __reserve1, muAVAX, __blockTimestampLast);
    }
    function get_mu_avax_sushiswap_pool_info() public view returns (address LPpool, address avax_token0, address mu_token1, uint112 avax_reserve0,uint112 mu_reserve1, uint256 muAVAX, uint32 _blockTimestampLast){
        return internal_mu_avax_sushiswap_pool_info();
    }
...
//in this function mu is token0
    ...
    //helper function to get avax usdc price
    
//in this function mu is token0

    function internal_avax_usdc_pool_info() internal view returns(address LPpool, address token0, address token1, uint112 _reserve0,uint112 _reserve1, uint256 avaxUSDC, uint32 _blockTimestampLast){
        (uint112 __reserve0, uint112 __reserve1, uint32 __blockTimestampLast) = avax_usdc.getReserves();
        address _token0 = avax_usdc.token0();
        address _token1 = avax_usdc.token1();
        uint256 avax = uint256(__reserve0);
        uint256 usdc = uint256(__reserve1);
        usdc = usdc * 10**30;
        uint256 avaxUSDC = usdc/avax;
        return (0xf4003F4efBE8691B60249E6afbD307aBE7758adb, _token0, _token1, __reserve0, __reserve1, avaxUSDC, __blockTimestampLast);
    }
    function get_avax_usdc_pool_info() public view returns (address LPpool, address avax_token0, address usdc_token1, uint112 _reserve0,uint112 _reserve1, uint256 avaxUSDC, uint32 _blockTimestampLast){
        return internal_avax_usdc_pool_info();
    }
    function internal_get_avax_usd_price()internal view returns(uint256){
        (uint112 __reserve0, uint112 __reserve1, uint32 __blockTimestampLast) = avax_usdc.getReserves();
        address _token0 = avax_usdc.token0();
        address _token1 = avax_usdc.token1();
        uint256 avax = uint256(__reserve0);
