...
contract Test1001 is Ownable {
    ...
    address public pairToken; // address of the token pairing to POPS token
    ...
    function deposit(uint256 amount) external {
        ...
        // Swap 100% staking token(USDC) in pairToken (USD+)
        _swapInMarket(stakeToken, pairToken, amount);
        // Then 50% as POP token -> buy in Market.
        _swapInMarket(pairToken, popsToken, amount / 2);
        ...
        )
    {
        ...
        IERC20(pairToken).approve(sicleRouter02, sTokenAmount);
    ...
    function _swapInMarket(address token0, address token1, uint256 amount) internal {
        uint256 deadline = block.timestamp + 5 minutes;
        uint256 amountOut;
        ...
        if (token1 == popsToken) {
            // Swap to POPS
            amountOut = _getPopsForLP(amount) * (10000 - slippage) / 10000;
        } else {
            // Swap to Stablecoin
            amountOut = amount * (10000 - slippage) / 10000;   
        }
        console.log(amount, amountOut);
        IERC20(token0).approve(sicleRouter02, amount);
        IERC20(token1).approve(sicleRouter02, amountOut);
