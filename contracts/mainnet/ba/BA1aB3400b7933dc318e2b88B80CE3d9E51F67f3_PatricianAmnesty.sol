...
contract PatricianAmnesty is MyPausable {
    PreyPredator public preyPredator;

    event AmnestyGranted(uint256 tokenId);

    constructor(address _preyPredator) {
        require( _preyPredator != address(0), "NFTTrainer: nft zero address");
        preyPredator = PreyPredator(_preyPredator);
    }

    function bathAmnesty(uint256[] memory _tokenIds) external onlyOwner whenNotPaused {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _amnesty(_tokenIds[i]);
        }
    }

    function _amnesty(uint256 _tokenId) internal {
        PreyPredator.PreyPredator memory traits =  preyPredator.getTokenTraits(_tokenId);
        
        require(traits.generation > 0 && traits.generation < 22, "NFTAmnesty: this generation is not eligible for amnesty");
        
        traits.generation = 24;
        
        emit AmnestyGranted(_tokenId);
