...
contract StargateManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @notice Checks the sender is the vault or the owner.
    modifier onlyVaultOrOwner {
        require(msg.sender == vault || msg.sender == owner(), "Not allowed");
        _;
    }
    ...
    function _stargateSyncRedeem() internal {
        ...
        IERC20Upgradeable(token).transfer(vault, IERC20Upgradeable(token).balanceOf(address(this)));
    }
    ...
    function withdrawToken() external onlyVaultOrOwner returns (uint256) {
