...
contract DefaultPool is IDefaultPool, PoolBase2 {
    ...
    /**
     * @notice used for logging extra collateral sent to this contract from external sources, only available
     * for tokens which are already owned by the contract.
     */
    function logExtraCollateral(address _collateral) external {
        // Require that this collateral is already owned by defaultPool
        uint256 poolCollLen = poolColl.tokens.length;
        for (uint i = 0; i < poolCollLen; i++) {
            if (poolColl.tokens[i] == _collateral) {
                // Update balance to match up with collateral balance
                uint balance = IERC20(_collateral).balanceOf(address(this));
                require(balance != 0, "DP:Collateral balance 0 update");
                poolColl.amounts[i] = balance;
                emit DefaultPoolBalanceUpdated(_collateral, balance);
                return;
            }
        }
        revert("DP:Collateral not owned");
    }
}
