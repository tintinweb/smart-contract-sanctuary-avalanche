...
/**
 * @title Staked Legendary YieldKey Contract / NFT Smart Contract ERC721
 * @custom:a ValiFi
 */
contract StakedLegendaryYieldKey is
    ...
    PausableUpgradeable
{
    ...
    // Instance of ValiFi Legendary YieldKey
    IValiFiYK public valifiYK;
    IMaintFee public maintFee;
    // Mapping of Staked Legendary YieldKeys
    ...
    /**
     * @dev Mapping for storing Staked Legendary YieldKeys
    ...
    /**
     * @dev Event when a Legendary YieldKey is Staked
     * @param user Address of the Legendary YieldKey NFT holder
     * @param tokenIds Token Ids of the Legendary YieldKeys
     */
    event StakedYieldKey(address indexed user, uint256[] tokenIds);

    /**
     * @dev Event when a Legendary YieldKey is UnStaked
     * @param user Address of the Legendary YieldKey NFT holder
     * @param tokenIds Token Ids of the Legendary YieldKeys
     */
    event UnStakedYieldKey(address indexed user, uint256[] tokenIds);

    function initialize(address legendaryYieldKey, string memory _initURI)
        public
        initializer
    {
        __ERC721_init_unchained("Staked Legendary YieldKey", "LYKSTK");
        __Ownable_init();
        __Pausable_init();
        valifiYK = IValiFiYK(legendaryYieldKey);
        setBaseURI(_initURI);
    }
    ...
    /**
     * @dev Checks if the YieldKey is Staked
     * @param wallet address of Legendary YieldKey holder
     * @param _tokenId Token Id of Legendary YieldKey
     */
    ...
    /**
     * @dev Allows to stake Legendary YieldKeys
     * @param wallet address of Legendary YieldKey holder
     * @param tokenIds Array of Token Ids of YieldKeys to be Staked
     */
        ...
        whenNotPaused
    {
        require(
            wallet == _msgSender(),
            "ERC721 ValiFi: Only the owner can stake YieldKey"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                valifiYK.ownerOf(tokenIds[i]) == _msgSender(),
                "ERC721 ValiFi: Sender is not owner of YieldKey"
            );
            require(
                !isStaked(wallet, tokenIds[i]),
                "ERC721 ValiFi: YieldKey is already staked"
            );
        }
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // Transfer the Token YieldKey from the wallet to the contract
            _safeMint(wallet, tokenIds[i]);
            valifiYK.safeTransferFrom(wallet, address(this), tokenIds[i]);
            // add the tokenId to the stakedYieldKeys mapping
    ...
    /**
     * @dev Allows to unStake Legendary YieldKeys
     * @param wallet address of Legendary YieldKey holder
     * @param tokenIds Array of Token Ids of YieldKeys to be unStaked
     */
        ...
        whenNotPaused
    {
        require(
            wallet == _msgSender(),
            "ERC721 ValiFi: Only the owner can unstake YieldKey"
        );
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            require(
                isStaked(wallet, tokenIds[i]),
                "ERC721 ValiFi: YieldKey is not staked"
            );
        }
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                ...
                "ERC721 ValiFi: YieldKey unstaking failed"
            );
            ...
            // Transfer from the Smart contract to the wallet of the holder
            _burn(tokenIds[i]);
            valifiYK.safeTransferFrom(address(this), wallet, tokenIds[i]);
            // Reset Maintenance Fee
            maintFee.setResetMaintenanceFee(tokenIds[i], 12);
        }
        emit UnStakedYieldKey(wallet, tokenIds);
    }

    /**
     * @dev Get History of Staked Legendary YieldKey
     * @param wallet address of Legendary YieldKey
     * @param tokenId Token Id of Legendary YieldKey
     */
    function getStakedYieldKeyHistory(address wallet, uint256 tokenId)
        external
        view
    ...
    /**
     * @dev Get Legendary YieldKey Tokens owned by a wallet address
     * @param _wallet Wallet address of the owner
     * @return tokenIds Array of token Ids of Legendary YieldKeys owned by the wallet
     */
    ...
    /**
     * @dev Get Staked Legendary YieldKey Tokens owned by a wallet address
     * @param _owner address of tokens holder
     * @return stakedTokenIds tokenIds Array of Staked Legendary YieldKeys owned by wallet address
     */
    function tokenStaking(address _owner)
        public
        view
        returns (uint256[] memory stakedTokenIds)
    {
        uint256 index;
        uint256[] memory _tokenIds = tokenHoldings(_owner);
        if (_tokenIds.length == 0) {
            return stakedTokenIds;
        }
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (isStaked(_owner, _tokenIds[i])) {
                index++;
            }
        }
        if (index == 0) {
        ...
        stakedTokenIds = new uint256[](index);
        index = 0;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (isStaked(_owner, _tokenIds[i])) {
                stakedTokenIds[index] = _tokenIds[i];
                index++;
            }
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !paused(),
            "ERC721 ValiFi: can't stake/unstake or transfer any Legendary YieldKey while paused"
        );
        if (from != address(0)) {
            require(
                !isStaked(from, _tokenId),
                "ERC721 ValiFi: can't transfer staked YieldKey"
            );
        }
