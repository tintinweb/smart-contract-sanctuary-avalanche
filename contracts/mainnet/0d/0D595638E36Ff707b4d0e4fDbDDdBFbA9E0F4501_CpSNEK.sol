...
import "./CpSNEKSolidStaker.sol";

contract CpSNEK is CpSNEKSolidStaker {
    ...
    event RewardsHarvested(uint256 _rewardSNEKBal, uint256 _rewardChamSNEKBal);
    ...
    function initialize(
        ...
        CpSNEKSolidStaker.init(_name, _symbol, _proxy, _manager[0], _manager[1], _manager[2], _manager[3], _configurator);
    ...
    function harvest() public {
        ...
        uint256 rewardCpSNEKBal = balanceOf(address(this));
        _chargeFees(rewardBal, rewardCpSNEKBal);
    }
    ...
    function _getVotedReward(uint256 _tokenId, address[] memory tokenVotes) internal {
        if (_tokenId > 0) {
            for (uint i; i < tokenVotes.length; i++) {
                ...
                proxy.getFeeReward(_tokenId, tokenVotes[i]);
            }
        }
    }

    function _chargeFees(uint256 _rewardSNEKBal, uint256 _rewardCpSNEKBal) internal {
        uint256 feePercent = configurator.getFee();
        address coFeeRecipient = configurator.coFeeRecipient();
        if (_rewardSNEKBal > 0) {
            uint256 feeBal = (_rewardSNEKBal * feePercent) / MAX_RATE;
            if (feeBal > 0) {
            ...
            IERC20Upgradeable(want).safeTransfer(daoWallet, _rewardSNEKBal - feeBal);
        }

        if (_rewardCpSNEKBal > 0) {
            uint256 feeBal = (_rewardCpSNEKBal * feePercent) / MAX_RATE;
            if (feeBal > 0) {
                IERC20Upgradeable(address(this)).safeTransfer(coFeeRecipient, feeBal);
                emit ChargedFees(0, feeBal, 0);
            }

            IERC20Upgradeable(address(this)).safeTransfer(daoWallet, _rewardCpSNEKBal - feeBal);
        }

        emit RewardsHarvested(_rewardSNEKBal, _rewardCpSNEKBal);
    }
...
import "../interfaces/ISnekProxy.sol";
import "../interfaces/ISnekVe.sol";
import "../interfaces/IVoter.sol";
import "../interfaces/ISolidlyRouter.sol";
import "../interfaces/ISolidlyFactory.sol";
import "../interfaces/IPairFactory.sol";
import "../interfaces/ICpSNEKConfigurator.sol";

contract CpSNEKSolidStaker is ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Addresses used
    ISnekProxy public proxy;
    IERC20Upgradeable public want;
    IERC20Upgradeable public native;
    ISnekVe public ve;
    IVoter public solidVoter;
    ISolidlyRouter public router;
    ICpSNEKConfigurator public configurator;

    uint256 public constant MAX = 10000; // 100%
    uint256 public constant MAX_RATE = 1e18;
    // Vote weight decays linearly over time. Lock time cannot be more than `MAX_LOCK` (1 years).
    ...
    modifier onlyManager() {
        require(
            ...
            "CpSNEKSolidStaker: MANAGER_ONLY"
        );
        _;
    }

    modifier onlyVoter() {
        require(msg.sender == voter, "CpSNEKSolidStaker: VOTER_ONLY");
        _;
    }
    ...
    function init(
        ...
        proxy = ISnekProxy(_proxy);
        want = IERC20Upgradeable(proxy.SNEK());
        ve = ISnekVe(proxy.ve());
        solidVoter = IVoter(proxy.solidVoter());
        router = ISolidlyRouter(proxy.router());
        configurator = ICpSNEKConfigurator(_configurator);
    ...
    function depositVe(uint256 _tokenId) external nonReentrant whenNotPaused {
        require(!configurator.isPausedDepositVe(), "CpSNEKSolidStaker: PAUSED");
        uint256 tokenId = proxy.mainTokenId();
        require(tokenId > 0, "CpSNEKSolidStaker: NOT_ASSIGNED");
        uint256 currentPeg = getCurrentPeg();
        require(currentPeg >= configurator.maxPeg(), "CpSNEKSolidStaker: NOT_MINT_WITH_UNDER_PEG");
        increaseUnlockTime();
    ...
    function _deposit(uint256 _amount) internal nonReentrant whenNotPaused {
        require(!configurator.isPausedDeposit(), "CpSNEKSolidStaker: PAUSED");
        uint256 tokenId = proxy.mainTokenId();
        require(tokenId > 0, "CpSNEKSolidStaker: NOT_ASSIGNED");
        increaseUnlockTime();
        ...
        require(pairAddress != address(0), "CpSNEKSolidStaker: LP_INVALID");
    ...
    function createMainLock(
        ...
        require(_amount > 0, "CpSNEKSolidStaker: ZERO_AMOUNT");
    ...
    function release() external onlyManager {
        ...
        require(endTime <= block.timestamp, "CpSNEKSolidStaker: LOCKED");
        proxy.release();
...
interface ISnekProxy {
    ...
    function SNEK() external returns (address);
    ...
    function vote(uint256 _tokenId, address[] calldata poolVote, uint256[] calldata weights) external;
    ...
    function getFeeReward(uint256 _tokenId, address _lp) external;
