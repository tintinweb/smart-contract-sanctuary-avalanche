    ...
    */
    ...
    function leechIn(
        ...
        if (baseToken.allowance(msg.sender, address(this)) < amount)
            revert InsufficientAllowance();
        if (address(baseToken) != baseToToken0[0]) revert WrongPath();
    ...
    function setRouter(address _router) external onlyOwner {
        if (_router == address(0)) revert WrongArgument();
    ...
    function _swapOut(address lpAddr) private {
        ...
        if (amount0 < MIN_AMOUNT) revert InsufficientAmountA();
        if (amount1 < MIN_AMOUNT) revert InsufficientAmountB();
    }
