...
//
// JUPITER PROTOCOL COPYRIGHT (C) 2022 
...
contract Jupiter is ERC20Detailed, Ownable {
    ...
    string public _name = "Jupiter";
    string public _symbol = "JUPITER";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        399 * 10**3 * 10**DECIMALS;

    uint256 public liquidityFee = 40;
    uint256 public treasuryFee = 25;
    uint256 public jupiterInsuranceFundFee = 50;
    uint256 public sellFee = 40;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(jupiterInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public jupiterInsuranceFundReceiver;
    ...
    uint256 private constant MAX_SUPPLY = 399 * 10**7 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("Jupiter", "JUPITER", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x0e5EF4184E897C3aC50A4C3e3Cc3294c486A2B5c;
        treasuryReceiver = 0x0e5EF4184E897C3aC50A4C3e3Cc3294c486A2B5c; 
        jupiterInsuranceFundReceiver = 0x0e5EF4184E897C3aC50A4C3e3Cc3294c486A2B5c;
        firePit = 0x0e5EF4184E897C3aC50A4C3e3Cc3294c486A2B5c;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(jupiterInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountAVAXToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(jupiterInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(jupiterInsuranceFundReceiver).call{
            value: amountAVAXToTreasuryAndSIF.mul(jupiterInsuranceFundFee).div(
                treasuryFee.add(jupiterInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Jupiter token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WAVAX();
        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _jupiterInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        jupiterInsuranceFundReceiver = _jupiterInsuranceFundReceiver;
        firePit = _firePit;
    }
