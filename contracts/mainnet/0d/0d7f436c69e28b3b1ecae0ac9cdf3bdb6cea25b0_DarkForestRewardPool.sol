...
interface PlanetsManagerUpgradeable {
    ...
    function getPlanet(uint256) external view  returns (PlanetEntity memory);
...
contract DarkForestRewardPool is Ownable {
        ...
        onlyOwner 
    {
        if (block.timestamp > lastRewardTime + rewardPeriod)  {
            ...
            uint256 CurIndex = planetsManager.getCurrentIndex();
            for (uint256 i = 0; i < CurIndex; i++) {

                PlanetsManagerUpgradeable.PlanetEntity memory planet = planetsManager.getPlanet(i);
                if (!planet.exists) return;
                address account = planetsManager.ownerOf(planet.id);
                
                if (planet.rewardMult > tRewardMults[0] && IERC20(rewardToken).balanceOf(account) > 0) {
