...
contract Vault is IVault, AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // This function should be removed in the next release as it is meant to be run only once on mainnet
    function syncFeesCalledOnlyOnce() external onlyRole(DEFAULT_ADMIN_ROLE) {
        // sync feeInRho with the actual rhoToken balance
        vault.feeInRho = IRhoToken(vault.config.rhoToken()).balanceOf(address(this));
    }
