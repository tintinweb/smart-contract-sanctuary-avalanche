...
contract TriePay is Ownable, ERC223ContractInterface {
    ...
    constructor(uint256 _feeRate, uint256 _chainId, address _routerAddress, address[] memory _tokens, address[] memory _stableTokens) {
        feeRate = _feeRate;
        chainId = _chainId;
        ...
        for (uint i = 0; i < _stableTokens.length; ++i) {
            routerTokens.add(_stableTokens[i]);
        }
    }
