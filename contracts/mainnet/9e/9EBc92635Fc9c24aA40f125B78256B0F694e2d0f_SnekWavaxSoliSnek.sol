...
// File contracts/SnekWavaxSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract SnekWavaxSoliSnek  is Ownable, Pausable {
    ...
    function addLiquidity() internal {
        uint256 glcrHalf = IERC20(snek).balanceOf(address(this)) / 2;

        ISoliSnekRouter(router).swapExactTokensForTokens(glcrHalf, 0, snekToAvaxPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(snek).balanceOf(address(this));
        uint256 t2Bal = IERC20(wavax).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(snek, wavax, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
