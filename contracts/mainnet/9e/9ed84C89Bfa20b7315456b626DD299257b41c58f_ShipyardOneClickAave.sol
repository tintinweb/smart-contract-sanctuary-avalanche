...
import "../interfaces/company/IStrategyAave.sol";
import "../libraries/SafeUniswapRouter.sol";
import "../managers/SlippageManager.sol";
import "../utils/AddressUtils.sol";

contract ShipyardOneClickAave is Ownable, SlippageManager, ReentrancyGuard {
    ...
    function deposit(address _shipyardVaultAddress, address _depositTokenAddress, uint256 _amountInDepositToken) external nonReentrant {
        ...
        IStrategyAave strategy = IStrategyAave(shipyardVault.strategy());

        address wantTokenAddress = (address)(strategy.want());

        require(_depositTokenAddress == wantTokenAddress || _depositTokenAddress == usdcAddress, 'Invalid deposit token address');
        ...
        if (_depositTokenAddress == wantTokenAddress) {
            // If it's the want, don't need to do any additional logic.
            _approveTokenIfNeeded(wantTokenAddress, _shipyardVaultAddress);
            shipyardVault.deposit(_amountInDepositToken);
            return;
        }

        // Swap USDC into the want
        address[] memory paths;
        paths[0] = usdcAddress;
        paths[1] = wantTokenAddress;
        ...
        uint256 wantTokenBalance = IERC20(wantTokenAddress).balanceOf(address(this));
        shipyardVault.deposit(wantTokenBalance);
    ...
    function withdraw(address _shipyardVaultAddress, address _requestedTokenAddress, uint256 _withdrawAmountInShipToken) external nonReentrant {
        ...
        require(_requestedTokenAddress == wantTokenAddress || _requestedTokenAddress == usdcAddress, 'Invalid deposit token address');
        ...
        if (_requestedTokenAddress == wantTokenAddress) {
            IERC20(wantTokenAddress).safeTransfer(msg.sender, wantTokenBalance);
            return;
        }

        address[] memory paths;
        paths[0] = wantTokenAddress;
        paths[1] = usdcAddress;

        address unirouter = strategy.unirouter();
        _approveTokenIfNeeded(wantTokenAddress, unirouter);
        IUniswapRouterETH(unirouter).safeSwapExactTokensForTokens(slippage, wantTokenBalance, paths, address(this), block.timestamp);
