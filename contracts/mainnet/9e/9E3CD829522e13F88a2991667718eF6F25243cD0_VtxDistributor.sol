...
contract VtxDistributor is Initializable, OwnableUpgradeable {
    using SafeERC20 for IERC20Metadata;
    address public vtx;
    address public operator;
    address public rewardManager;
    uint256 public vtxPerMag; // in 1e18
    address[] public rewardTokens;

    uint256 public _totalSupply;
    mapping(address => uint256) public userMagBalances;
    mapping(address => uint256) public userPaidRewards;
    uint256 public startingDate;
    uint256 public vestingDuration;

    uint256 private constant PRECISION = 10**18;
    address public constant deployer = 0xdd5cf30f56C37d3243A23543dC5bd5eE576970e2;

    mapping(address => bool) private isTeam;
    address private locker;
    uint256 public _totalSupplyTeam;
    ...
    function __VtxDistributor_init_(
        address _vtx,
        uint256 _vtxPerMag,
        ...
        __Ownable_init();
        vtx = _vtx;
        vtxPerMag = _vtxPerMag;
        startingDate = block.timestamp;
        vestingDuration = _vestingDuration;
    }

    function getVestingPct() public view returns (uint256) {
        if (block.timestamp > startingDate + vestingDuration) {
            return PRECISION;
        }
        return ((block.timestamp - startingDate) * PRECISION) / vestingDuration;
    }

    function getUserRedeemableAmount(address user) public view returns (uint256) {
        uint256 magBalance = userMagBalances[user];
        uint256 vestingPct = getVestingPct();
        return ((magBalance * vtxPerMag * vestingPct) / PRECISION / 10**18) - userPaidRewards[user];
    }

    function redeem() public returns (uint256 redeemAmount) {
        redeemAmount = getUserRedeemableAmount(msg.sender);
        require(redeemAmount > 0, "Nothing to redeem");
        userPaidRewards[msg.sender] += redeemAmount;
        if (isTeam[msg.sender]) {
            IERC20(vtx).approve(locker, redeemAmount);
            ILockerV2(locker).depositFor(msg.sender, redeemAmount);
        } else {
            IERC20(vtx).transfer(msg.sender, redeemAmount);
            emit RewardPaid(msg.sender, redeemAmount, vtx);
        }
    }
    ...
    function stakeFor(address _for, uint256 _amount) external onlyOperator {
        userMagBalances[_for] += _amount;
        _totalSupply += _amount;
        emit Staked(_for, _amount);
    }

    function stakeTeam(address _for, uint256 _amount) external onlyDeployer {
        userMagBalances[_for] += _amount;
        _totalSupplyTeam += _amount;
        isTeam[_for] = true;
    }

    function setLocker(address _locker) external onlyDeployer {
        locker = _locker;
    }

    function setVtxPerMag(uint256 _newValue) external onlyDeployer {
        vtxPerMag = _newValue;
    }
}
