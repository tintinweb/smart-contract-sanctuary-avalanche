...
interface IBiggerMindsAddressRegistry {
    function minds() external view returns (address);
...
interface IBiggerMindsBundleMarketplace {
...
interface IBiggerMindsNFTFactory {
...
interface IBiggerMindsPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract BiggerMindsMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IBiggerMindsAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IBiggerMindsBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IBiggerMindsAuction auction = IBiggerMindsAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isBiggerMindsNFT(_nftAddress), "invalid nft address");
    ...
    function _isBiggerMindsNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.minds() == _nftAddress ||
            IBiggerMindsNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IBiggerMindsNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IBiggerMindsNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IBiggerMindsNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IBiggerMindsPriceFeed priceFeed = IBiggerMindsPriceFeed(
    ...
    /**
     @notice Update BiggerMindsAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IBiggerMindsAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IBiggerMindsTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
