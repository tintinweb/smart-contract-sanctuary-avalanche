...
interface IZap {
    function add_liquidity(uint[5] memory amounts, uint amountOutMin) external;
...
contract PowerBombAvaxCurveTri is PowerBombAvaxCurve {
    ...
    IAv3Pool constant av3Pool = IAv3Pool(0x7f90122BF0700F9E7e1F688fe926940E8839F353);
    IERC20Upgradeable constant crvUSDBTCETH = IERC20Upgradeable(0x1daB6560494B04473A0BE3E7D83CF3Fdf3a51828);

    IZap public zap;

    function initialize(
        IPool _pool, IGauge _gauge, IZap _zap, address _treasury, address _bot, IERC20Upgradeable _rewardToken
        ...
        gauge = _gauge;
        zap = _zap;
        lpToken = IERC20Upgradeable(zap.token());
        ...
        WBTC.safeApprove(address(zap), type(uint).max);
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor, uint amountOutMin) internal override nonReentrant whenNotPaused {
        require(token == USDT || token == USDC || token == DAI || token == WBTC || token == WETH || token == lpToken, "Invalid token");
        ...
        if (token != lpToken) {
            ...
            else if (token == WETH) amounts[4] = amount;
            zap.add_liquidity(amounts, amountOutMin);
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint amountOutMin) external override nonReentrant {
        require(token == USDT || token == USDC || token == DAI || token == WBTC || token == WETH || token == lpToken, "Invalid token");
        ...
        if (token != lpToken) {
            ...
            zap.remove_liquidity_one_coin(amountOutLpToken, i, amountOutMin);
    ...
    function _harvest(bool isDeposit) internal virtual override {
    ...
    function getPricePerFullShareInUSD() public override view returns (uint) {
        ...
        uint totalAv3CRVInUSD = pool.balances(0) * av3Pool.get_virtual_price() / 1e18;
        uint totalavWBTCInUSD = pool.balances(1) * pool.price_oracle(0) / 1e8;
        uint totalavWETHInUSD = pool.balances(2) * pool.price_oracle(1) / 1e18;
        uint totalAssetsInUSD = totalAv3CRVInUSD + totalavWBTCInUSD + totalavWETHInUSD;
        // Calculate price per full share
        return totalAssetsInUSD * 1e6 / crvUSDBTCETH.totalSupply();
    }

    // For calculate slippage, index 0 = BTC, 1 = ETH, 2 = USD, 18 decimals
    function getPriceOracle(uint index) external view returns (uint) {
        if (index != 2) {
            return pool.price_oracle(index);
        } else {
            return av3Pool.get_virtual_price();
        }
    }
...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    uint private ibRewardTokenBaseAmt; // Depreciated
