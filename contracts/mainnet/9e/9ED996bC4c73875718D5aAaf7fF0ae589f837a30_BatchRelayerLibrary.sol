...
// File @rumble-finance/solidity-utils/contracts/misc/[email protected]
...
/**
 * @dev Interface for WAVAX9.
 * See https://github.com/gnosis/canonical-wavax/blob/0dd1ea3e295eef916d0c6223ec63141137d22d67/contracts/WAVAX9.sol
 */
...
// File @rumble-finance/vault/contracts/interfaces/[email protected]
...
/**
 * @dev This is an empty interface used to represent either ERC20-conforming token contracts or AVAX (using the zero
...
interface IVault is IVaultPartial, ISignaturesValidator, ITemporarilyPausable {
    ...
    //    means that Rumble governance must approve each individual contract to act as a relayer for the intended
    //    functions.
    ...
    /**
     * @dev Data for `manageUserBalance` operations, which include the possibility for AVAX to be sent and received
     without manual WAVAX wrapping or unwrapping.
     */
    ...
    //
    // AVAX can be used by passing the AVAX sentinel value as the asset and forwarding AVAX in the call:
    // it will be wrapped and deposited as WAVAX. Any AVAX amount remaining will be sent back to the caller
    ...
    //
    // AVAX can be used by passing the AVAX sentinel value as the asset. This will deduct WAVAX instead,
    // unwrap it and send it to the recipient as AVAX.
    //
    // Emits an `InternalBalanceChanged` event.
    //
    //
    ...
    //
    // Reverts if the AVAX sentinel value is passed.
    //
    // Emits an `InternalBalanceChanged` event.
    //
    //
     ...
     *
     * Because Internal Balance works exclusively with ERC20 tokens, AVAX deposits and withdrawals will use the WAVAX
     * address.
     */
    ...
    // constant product one popularized by Rumble V1. Swap costs are smaller compared to general Pools, and are
     ...
     * these maximums.
     *
     * If joining a Pool that holds WAVAX, it is possible to send AVAX directly: the Vault will do the wrapping.
     * To enable this mechanism, the IAsset sentinel value (the zero address) must be passed in the `assets` array
     * instead of the WAVAX address. Note that it is not possible to combine AVAX and WAVAX in the same join.
     * Any excess AVAX will be sent back to the caller (not the sender, which is important for relayers).
     *
     * `assets` must have the same length and order as the array returned by `getPoolTokens`. This prevents issues when
     * interacting with Pools that register and deregister tokens frequently. If sending AVAX however, the array must be
     * sorted *before* replacing the WAVAX address with the AVAX sentinel value (the zero address),
     ...
     * be made for the difference between the requested amount and Internal Balance (if any). Note that AVAX cannot be
     ...
     *
     * If exiting a Pool that holds WAVAX, it is possible to receive AVAX directly: the Vault will do the unwrapping. To
     * enable this mechanism, the IAsset sentinel value (the zero address) must be passed in the `assets` array instead
     * of the WAVAX address. Note that it is not possible to combine AVAX and WAVAX in the same exit.
     *
     * `assets` must have the same length and order as the array returned by `getPoolTokens`. This prevents issues when
     * interacting with Pools that register and deregister tokens frequently. If receiving AVAX however, the array must
     * be sorted *before* replacing the WAVAX address with the AVAX sentinel value (the zero address), which means the
     * final `assets` array might not be sorted. Pools with no registered tokens cannot be exited.
     *
     * If `toInternalBalance` is true, the tokens will be deposited to `recipient`'s Internal Balance. Otherwise,
     * an ERC20 transfer will be performed. Note that AVAX cannot be deposited to Internal Balance: attempting to
    ...
    //
    // If interacting with Pools that hold WAVAX, it is possible to both send and receive AVAX directly: the Vault will
    // do the wrapping and unwrapping. To enable this mechanism, the IAsset sentinel value (the zero address) must be
    // passed in the `assets` array instead of the WAVAX address. Note that it is possible to combine AVAX and WAVAX in
    // the same swap. Any excess AVAX will be sent back to the caller (not the sender, which is relevant for relayers).
    //
     ...
     * the `kind` value.
     *
     * `assetIn` and `assetOut` are either token addresses, or the IAsset sentinel value for AVAX (the zero address).
     * Note that Pools never interact with AVAX directly: it will be wrapped to or unwrapped from WAVAX by the Vault.
     *
     ...
     *
     * Returns an array with the net Vault asset balance deltas. Positive amounts represent tokens (or AVAX) sent to the
     * Vault, and negative amounts represent tokens (or AVAX) sent by the Vault. Each delta corresponds to the asset at
     ...
     * or the IAsset sentinel value for AVAX (the zero address). Each entry in the `swaps` array specifies tokens in and
     * out by referencing an index in `assets`. Note that Pools never interact with AVAX directly: it will be wrapped to
     * or unwrapped from WAVAX by the Vault.
     *
     ...
     * `assets` array passed to that function, and AVAX assets are converted to WAVAX.
     *
     ...
     *
     * Note that AVAX cannot be deposited to or withdrawn from Internal Balance: attempting to do so will trigger a
     * revert.
     */
     ...
     * AVAX) the Vault would take from the sender (if positive) or send to the recipient (if negative). The arguments it
    ...
    /**
     * @dev Returns the Vault's WAVAX instance.
     */
...
library Errors {
    ...
    uint256 internal constant INVALID_AVAX_INTERNAL_BALANCE = 514;
    uint256 internal constant INVALID_POST_LOAN_BALANCE = 515;
    uint256 internal constant INSUFFICIENT_AVAX = 516;
    uint256 internal constant UNALLOCATED_AVAX = 517;
    uint256 internal constant AVAX_TRANSFER = 518;
    uint256 internal constant CANNOT_USE_AVAX_SENTINEL = 519;
...
// File contracts/interfaces/IRumbleRelayer.sol
...
// File contracts/relayer/RumbleRelayer.sol
 ...
 * e.g. sweeping any leftover AVAX which should have been refunded to the user.
 *
 * NOTE: Only the entrypoint contract should be whitelisted by Rumble governance as a relayer and so the Vault
 ...
 */
contract RumbleRelayer is IRumbleRelayer, ReentrancyGuard {
    ...
    receive() external payable {
        // Accept AVAX transfers only coming from the Vault. This is expected to happen due to a swap/exit/withdrawal
        // with AVAX as an output should the relayer be listed as the recipient. This may also happen when
        // joining a pool, performing a swap or managing a user's balance does not use the full AVAX value provided.
        // Any excess AVAX value will be refunded back to this contract and forwarded back to the original sender.
        _require(msg.sender == address(_vault), Errors.AVAX_TRANSFER);
    }
    ...
    function _refundAVAX() private {
        uint256 remainingAvax = address(this).balance;
        if (remainingAvax > 0) {
            msg.sender.sendValue(remainingAvax);
        }
    }
}


// File @rumble-finance/vault/contracts/[email protected]
...
abstract contract AssetHelpers {
    ...
    // Sentinel value used to indicate WAVAX with wrapping/unwrapping semantics. The zero address is a good choice for
    // multiple reasons: it is cheap to pass as a calldata argument, it is a known invalid token and non-contract, and
    // it is an address Pools cannot register as a token.
    address private constant _AVAX = address(0);

    constructor(IWAVAX wavax) {
        _wavax = wavax;
    }

    // solhint-disable-next-line func-name-mixedcase
    function _WAVAX() internal view returns (IWAVAX) {
        return _wavax;
    }

    /**
     * @dev Returns true if `asset` is the sentinel value that represents AVAX.
     */
    function _isAVAX(IAsset asset) internal pure returns (bool) {
        return address(asset) == _AVAX;
    }

    /**
     * @dev Translates `asset` into an equivalent IERC20 token address.
     * If `asset` represents AVAX, it will be translated to the WAVAX contract.
     */
    function _translateToIERC20(IAsset asset) internal view returns (IERC20) {
        return _isAVAX(asset) ? _WAVAX() : _asIERC20(asset);
    }
     ...
     * This function should only be called on `asset` if `_isAVAX` previously returned false for it,
     * that is, if `asset` is guaranteed not to be the AVAX sentinel value.
     */
...
abstract contract IBaseRelayerLibrary is AssetHelpers {
    constructor(IWAVAX wavax) AssetHelpers(wavax) {
        // solhint-disable-previous-line no-empty-blocks
    }
 ...
 * @dev
 * Relayers are formed out of a system of two contracts:
 *  - A `RumbleRelayer` contract which acts as a single point of entry into the system through a multicall function
...
contract BaseRelayerLibrary is IBaseRelayerLibrary {
    ...
    IVault private immutable _vault;
    IRumbleRelayer private immutable _entrypoint;

    constructor(IVault vault) IBaseRelayerLibrary(vault.WAVAX()) {
        _vault = vault;
        _entrypoint = new RumbleRelayer(vault, address(this));
    }
    ...
    function getEntrypoint() public view returns (IRumbleRelayer) {
        return _entrypoint;
    }
    ...
    // solhint-disable-next-line var-name-mixedcase
    bytes32 private immutable _TEMP_STORAGE_SUFFIX = keccak256("rumble.base-relayer-library");
 ...
 * @dev All functions must be payable so that it can be called as part of a multicall involving AVAX
 */
...
// File contracts/interfaces/IstAVAX.sol
...
// File contracts/interfaces/IwstAVAX.sol
...
// solhint-disable-next-line max-line-length
// Based on https://github.com/lidofinance/lido-dao/blob/2b46615a11dee77d4d22066f942f6c6afab9b87a/contracts/0.6.12/WstAVAX.sol

/**
 * @title StAVAX token wrapper with static balances.
 * @dev It's an ERC20 token that represents the account's share of the total
 * supply of stAVAX tokens. WstAVAX token's balance only changes on transfers,
 * unlike StAVAX that is also changed when oracles report staking rewards and
 * penalties. It's a "power user" token for DeFi protocols which don't
 * support rebasable tokens.
 *
 * The contract is also a trustless wrapper that accepts stAVAX tokens and mints
 * wstAVAX in return. Then the user unwraps, the contract burns user's wstAVAX
 * and sends user locked stAVAX in return.
 *
 * The contract provides the staking shortcut: user can send AVAX with regular
 * transfer and get wstAVAX in return. The contract will send AVAX to Lido submit
 * method, staking it and wrapping the received stAVAX.
 *
 */
interface IwstAVAX is IERC20 {
    function stAVAX() external returns (IstAVAX);

    /**
     * @notice Exchanges stAVAX to wstAVAX
     * @param _stAVAXAmount amount of stAVAX to wrap in exchange for wstAVAX
     * @dev Requirements:
     *  - `_stAVAXAmount` must be non-zero
     *  - msg.sender must approve at least `_stAVAXAmount` stAVAX to this
     *    contract.
     *  - msg.sender must have at least `_stAVAXAmount` of stAVAX.
     * User should first approve _stAVAXAmount to the WstAVAX contract
     * @return Amount of wstAVAX user receives after wrap
     */
    function wrap(uint256 _stAVAXAmount) external returns (uint256);

    /**
     * @notice Exchanges wstAVAX to stAVAX
     * @param _wstAVAXAmount amount of wstAVAX to uwrap in exchange for stAVAX
     * @dev Requirements:
     *  - `_wstAVAXAmount` must be non-zero
     *  - msg.sender must have at least `_wstAVAXAmount` wstAVAX.
     * @return Amount of stAVAX user receives after unwrap
     */
    function unwrap(uint256 _wstAVAXAmount) external returns (uint256);

    /**
     * @notice Get amount of wstAVAX for a given amount of stAVAX
     * @param _stAVAXAmount amount of stAVAX
     * @return Amount of wstAVAX for a given stAVAX amount
     */
    function getWstAVAXByStAVAX(uint256 _stAVAXAmount) external view returns (uint256);

    /**
     * @notice Get amount of stAVAX for a given amount of wstAVAX
     * @param _wstAVAXAmount amount of wstAVAX
     * @return Amount of stAVAX for a given wstAVAX amount
     */
    function getStAVAXByWstAVAX(uint256 _wstAVAXAmount) external view returns (uint256);

    /**
     * @notice Get amount of wstAVAX for a one stAVAX
     * @return Amount of stAVAX for 1 wstAVAX
     */
    function stAvaxPerToken() external view returns (uint256);

    /**
     * @notice Get amount of stAVAX for a one wstAVAX
     * @return Amount of wstAVAX for a 1 stAVAX
     */
    function tokensPerStAvax() external view returns (uint256);
}
...
/**
 * @title LidoWrapping
 * @notice Allows users to wrap and unwrap stAVAX as one of
 * @dev All functions must be payable so that it can be called as part of a multicall involving AVAX
 */
abstract contract LidoWrapping is IBaseRelayerLibrary {
    using Address for address payable;

    IstAVAX private immutable _stAVAX;
    IwstAVAX private immutable _wstAVAX;

    /**
     * @dev The zero address may be passed as wstAVAX to safely disable this module
     * @param wstAVAX - the address of the Lido's wrapped stAVAX contract
     */
    constructor(IERC20 wstAVAX) {
        // Safely disable stAVAX wrapping if no address has been passed for wstAVAX
        _stAVAX = wstAVAX != IERC20(0) ? IwstAVAX(address(wstAVAX)).stAVAX() : IstAVAX(0);
        _wstAVAX = IwstAVAX(address(wstAVAX));
    }
    ...
    function wrapStAVAX(
        ...
        if (sender != address(this)) {
            require(sender == msg.sender, "Incorrect sender");
            _pullToken(sender, _stAVAX, amount);
        }

        _stAVAX.approve(address(_wstAVAX), amount);
        uint256 result = IwstAVAX(_wstAVAX).wrap(amount);

        if (recipient != address(this)) {
            _wstAVAX.transfer(recipient, result);
        }
    ...
    function unwrapWstAVAX(
        ...
        if (sender != address(this)) {
            require(sender == msg.sender, "Incorrect sender");
            _pullToken(sender, _wstAVAX, amount);
        }

        // No approval is needed here as wstAVAX is burned directly from the relayer's account
        uint256 result = _wstAVAX.unwrap(amount);

        if (recipient != address(this)) {
            _stAVAX.transfer(recipient, result);
        }
    ...
    function stakeAVAX(
        ...
        uint256 result = _stAVAX.submit{ value: amount }(address(this));
    ...
    function stakeAVAXAndWrap(
        ...
        // As the wstAVAX contract doesn't return how much wstAVAX is minted we must query this separately.
        uint256 result = _wstAVAX.getWstAVAXByStAVAX(amount);

        // The fallback function on the wstAVAX contract automatically stakes and wraps any AVAX which is sent to it.
        // We can then safely just send the AVAX and just have to ensure that the call doesn't revert.
        //
        // This would be dangerous should `_wstAVAX` be set to the zero address, however in this scenario
        // this function would have already reverted on when calling `getWstAVAXByStAVAX`, preventing loss of funds.
        payable(address(_wstAVAX)).sendValue(amount);
...
// File @rumble-finance/solidity-utils/contracts/math/[email protected]
...
// File @rumble-finance/pool-weighted/contracts/[email protected]
...
/**
 * @title VaultActions
 * @notice Allows users to call the core functions on the Rumble Vault (swaps/joins/exits/balance management)
 * @dev
 ...
 *
 * All functions must be payable so that it can be called as part of a multicall involving AVAX
 */
abstract contract VaultActions is IBaseRelayerLibrary {
    ...
    function joinPool(
        ...
        // The output of a join is expected to be balance in the Pool's token contract, typically known as KPT (Rumble
    ...
    function exitPool(
        ...
        for (uint256 i = 0; i < outputReferences.length; i++) {
            ...
            if (request.toInternalBalance) {
                ...
                initialRecipientBalances[i] = _isAVAX(asset)
                    ? recipient.balance
        ...
        if (request.toInternalBalance) {
            ...
            for (uint256 i = 0; i < outputReferences.length; i++) {
                IAsset asset = request.assets[outputReferences[i].index];
                finalRecipientTokenBalances[i] = _isAVAX(asset)
                    ? recipient.balance
                    : _asIERC20(asset).balanceOf(recipient);
            }
        }
...
/**
 * @title VaultPermit
 * @notice Allows users to approve the Rumble Vault to use their tokens using permit (where supported)
 * @dev All functions must be payable so that it can be called as part of a multicall involving AVAX
 */
...
contract BatchRelayerLibrary is BaseRelayerLibrary, AaveWrapping, LidoWrapping, VaultActions, VaultPermit {
    constructor(IVault vault, IERC20 wstAVAX) BaseRelayerLibrary(vault) LidoWrapping(wstAVAX) {
        // solhint-disable-previous-line no-empty-blocks
    }
}