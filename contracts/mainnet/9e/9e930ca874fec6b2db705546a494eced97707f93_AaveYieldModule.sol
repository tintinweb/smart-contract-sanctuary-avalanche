...
import "../interface/aave/IPoolDataProvider.sol";
...
contract AaveYieldModule is BaseModule {
    ...
    function initialize(
        ...
        _setPool(_pool);
        _setPoolDataProvider(_poolDataProvider);
    ...
    function setPoolDataProvider(address _poolDataProvider) external onlyOwner {
        _setPoolDataProvider(_poolDataProvider);
    }
    ...
    returns (uint256 instant, uint256 pending)
    {
        ...
        uint256 withdrawnAmount = IPoolAave(pool).withdraw(baseToken, amount, address(this));
    ...
    function getBalance() public view returns (uint256) {
        (uint256 balance, , , , , , , ,) =  IPoolDataProvider(poolDataProvider).getUserReserveData(
            baseToken,
            address(this)
        );
        return balance;
    ...
    function _lpProfit(uint256 aum) internal {
        ...
        if(protocolBalance > aum) {
            ...
            IPoolAave(pool).withdraw(baseToken, expectedLpProfit, address(this));
        }
    }
    ...
    function _setPool(address _pool) internal {
        require(
            _pool != address(0),
            "Aave: cannot be the zero address"
        );
        require(baseToken != address(0), "Aave: baseToken not initialized");
        pool = _pool;
        uint256 allowance = IERC20Upgradeable(baseToken).allowance(address(this), _pool);
        if (allowance == 0) {
            IERC20Upgradeable(baseToken).safeApprove(_pool, type(uint256).max);
        } else {
    ...
    function _setPoolDataProvider(address _poolDataProvider) internal {
        require(_poolDataProvider != address(0), "Aave: cannot be the zero address");
        poolDataProvider = _poolDataProvider;
    }

    function _setRewardsController(address _rewardsController) internal {
        require(_rewardsController != address(0), "Aave: cannot be the zero address");
        rewardsController = _rewardsController;
    }

    function _setAToken(address _aToken) internal {
        require(_aToken != address(0), "Aave: cannot be the zero address");
        aToken = _aToken;
    }
}
