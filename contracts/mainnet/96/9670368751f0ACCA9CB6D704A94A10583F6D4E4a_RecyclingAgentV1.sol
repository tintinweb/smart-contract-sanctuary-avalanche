...
import "./ContractRegistryInterface.sol";
import "./PricingOracleInterface.sol";
import "./RainbowTableInterface.sol";
import "./Domain.sol";
import "./NamespaceInterface.sol";

contract RecyclingAgentV1 is AccessControl {
  ...
  event RegistrationPremiumSet(uint256[] premiumPricePoints);
  ...
  function setRegistrationPremiumDetails(uint256[] calldata premiumPricePoints) external onlyRole(MANAGER_ROLE) {
    // each premiumPricePoint should be smaller than the previous
    for (uint256 i = 0; i < premiumPricePoints.length; i += 1) {
      if (i > 0) {
        require(premiumPricePoints[i] < premiumPricePoints[i-1], "RecyclingAgentV1: premiumPricePoint[i] must be smaller than premiumPricePoint[i-1]");
      }
    }

    _premiumPricePoints = premiumPricePoints;
    emit RegistrationPremiumSet(premiumPricePoints);
  }

  function getRegistrationPremium(uint256 t, uint256 expiry, uint256 gracePeriodLength, uint256 recyclePeriodLength) public view returns (uint256) {
    uint256 _premiumStartTime = expiry + gracePeriodLength;
    uint256 _premiumEndTime = _premiumStartTime + recyclePeriodLength;
  ...
  function _getRegistrationLength(uint256 currExpiry, uint256 numYears) internal view returns (uint256) {
    ...
    require(expiry <= maxExpiry, "RecyclingAgentV1: registration too far in future");
    return expiry;
  }
  ...
  function _registerName(
    ...
    require(quantity > 0, "RecyclingAgentV1: invalid quantity");
    domain.recycle(
      msg.sender, // to
      _namespaceId, // namespace
      name, // name id
      _getRegistrationLength(expiry, quantity)
    );
  }
  ...
  function _transferToTreasury(uint256 total) internal {
    ...
    require(sent, "RecyclingAgentV1: payment not sent");
  }

  function _getPricingForName(
    uint256 name,
    uint256 quantity,
    uint256 expiry,
    bytes calldata pricingProof,
    uint256 gracePeriodLength, 
    uint256 recyclePeriodLength,
    PricingOracleInterface _pricingOracle
  ) internal returns (uint256) {
    uint256 price;
    (price, /* priceCentsUsd */) = _pricingOracle.getPriceForName(name, pricingProof);
    return price * quantity + getRegistrationPremium(block.timestamp, expiry, gracePeriodLength, recyclePeriodLength);
  }

  function _getPricingForNames(
    ...
    NamespaceInterface namespace = NamespaceInterface(_contractRegistry.get('Namespace'));
    uint256 avaxNs = 4272832630669137235923015693490068373911885005413996126751674003559469537065;
    uint256 recyclePeriodLength = namespace.getRecyclePeriodLength(avaxNs); 
    uint256 gracePeriodLength = namespace.getGracePeriodLength(avaxNs);
    uint256 total = 0;
    uint256 i;

    // get pricing for names
    for (i = 0; i < names.length; i += 1) {
      total += _getPricingForName(
        names[i], 
        quantities[i], 
        expiries[i], 
        pricingProofs[i],
  ...
  function register(
    ...
    require(_enabled, "RecyclingAgentV1: registration disabled");
    require(names.length == pricingProofs.length, "RecyclingAgentV1: proof length mismatch");
    require(names.length == quantities.length, "RecyclingAgentV1: quantities length mismatch");

    Domain _domain = Domain(_contractRegistry.get('Domain'));
    uint256[] memory expiries = new uint256[](names.length);
    uint256 i;

    for (i = 0; i < names.length; i += 1) {
      expiries[i] = _domain.getDomainExpiry(names[i]);
    }

    uint256 total = _getPricingForNames(
      names,
      quantities,
      expiries,
      pricingProofs
    ...
    // collect payment 
    require(msg.value >= total, "RecyclingAgentV1: insufficient payment");
    ...
    for (i = 0; i < names.length; i += 1) {
      _registerName(names[i], quantities[i], _domain, expiries[i]);
    }
