...
abstract contract SupportingSwap is FeeStore, IRYZRSwapRouter {
    ...
    {
        require(path.length == 2, "RYZRSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        require(path[0] == WAVAX, "RYZRSwapRouter: INVALID_PATH");

        uint avax = msg.value;
        address pair = RYZRSwapLibrary.pairFor(factory, path[0], path[1]);
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (avax, adminFeeDeduct) = RYZRSwapLibrary.adminFeeCalculation(avax, adminFee);
        ...
        assert(IWAVAX(WAVAX).transfer(pair, amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactAVAX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
    external
    ...
    {
        require(path.length == 2, "RYZRSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        require(path[path.length - 1] == WAVAX, "RYZRSwapRouter: INVALID_PATH");
        ...
        uint amountAVAXOut = amounts[amounts.length - 1];
        if(path[1] == pairFeeAddress[pair]){
            (amountAVAXOut,adminFeeDeduct) = RYZRSwapLibrary.adminFeeCalculation(amountAVAXOut,adminFee);
        }
        IWAVAX(WAVAX).withdraw(amountAVAXOut);
        TransferHelper.safeTransferAVAX(to, amountAVAXOut);
    }
    function swapExactTokensForAVAX(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
    external
    virtual
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[0] == pairFeeAddress[pair]){
            ...
            require(amounts[0] <= msg.value, "RYZRSwapRouter: EXCESSIVE_INPUT_AMOUNT");
            IWAVAX(WAVAX).deposit{value: amounts[0]}();
            assert(IWAVAX(WAVAX).transfer(RYZRSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        }

        _swap(amounts, path, to);
        // refund dust avax, if any
        uint bal = amounts[0].add(adminFeeDeduct);
        if (msg.value > bal) TransferHelper.safeTransferAVAX(msg.sender, msg.value - bal);
    }
    ...
    ensure(deadline)
    {
        ...
        assert(IWAVAX(WAVAX).transfer(pair, amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        if(path[1] == pairFeeAddress[pair]){
            (amountOutMin,adminFeeDeduct) = RYZRSwapLibrary.adminFeeCalculation(amountOutMin,adminFee);
...
contract RYZRSwapRouter is SupportingSwap {
    ...
    constructor(address _factory, address _WAVAX, address _BUSD, uint256 _adminFee, address _adminFeeAddress, address _adminFeeSetter) {
        factory = _factory;
        WAVAX = _WAVAX;
        BUSD = _BUSD;
    ...
    function addLiquidityAVAX(
        ...
        address to,
        uint deadline
    ) external virtual override payable ensure(deadline) returns (uint amountAVAX, uint amountToken, uint liquidity) {
        (amountAVAX, amountToken) = _addLiquidity(
            WAVAX,
            token,
            msg.value,
            amountTokenDesired,
        ...
        address pair = RYZRSwapLibrary.pairFor(factory, token, WAVAX);
