...
contract CirculateUSDC  is Ownable, ReentrancyGuard{
    ...
    uint256 public totalDepositLimit = 1e12;
    ...
    address public AutoStartOperator = 0x1AA0CfC410b8eBc193AcDE13aA166946a0b952c4;
    address public AutoEndOperator = 0x1AA0CfC410b8eBc193AcDE13aA166946a0b952c4;
    address private devWallet = 0x5695Ef5f2E997B2e142B38837132a6c3Ddc463b7;
    address public USDCContract = 0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E; 
    ...
    function changeUSDCContractAddress(address _usdcContract) public onlyOwner{
        require(_usdcContract!=address(0),"Please provide a valid address");
    ...
    function deposit(uint256 _amount, address _referrer) external {
        ...
        IERC20(USDCContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(USDCContract).safeTransfer(devWallet,depositFee);
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(USDCContract).safeTransfer(msg.sender,amountToSend);
        totalReward = totalReward.add(amountToSend);

        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(USDCContract).safeTransfer(investors[msg.sender].referrer,(amountToSend*ferralfee).div(percentRate));
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(USDCContract).safeTransfer(investors[msg.sender].referrer,(claimableReward*ferralfee).div(percentRate));
    ...
    function getTotalAssets(address _otherToken) public view returns (uint256) {
        ...
        uint256 otherAssets = ( otherBalance * ISwapHelper(SwapHelper).getSwapOut( _otherToken, USDCContract, 1e6) ).div(1e6);
        uint256 mainAssets = IERC20(USDCContract).balanceOf(address(this));
    ...
    function getBorrowableAmount(address _toToken, address _trader) public view returns(uint256 _amount){
        (uint256 MaxStartMLimit,,, uint256 MaxLoanLimit, uint256 MaxTotalTradingLimit,,,uint256 LoanDivCollateral) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_toToken );
    ...
    function startTrading(address _trader, uint256 _borrowAmount, address _swappedToken) public {
        ...
        (,, uint256 MinCollateralLimit,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
        ...
        IERC20(USDCContract).safeApprove(SwapHelper, _borrowAmount);
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( USDCContract,_swappedToken, _borrowAmount);
    ...
    function getTotalProfit(address _token) public view returns(uint256) {
        (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract, _token );
    ...
    function withdrawCollateralFunds(uint256 _amount) external nonReentrant {
        ...
        debtors[msg.sender].withdrawableAmount -= _amount;
        debtors[msg.sender].collateralAmount -= _amount;
        IERC20(USDCContract).safeTransfer(msg.sender,_amount);
    }
    function getSwapBackAmount(address _trader) public view returns(uint256){
        if(debtors[_trader].tradingState){
            ...
            return ISwapHelper(SwapHelper).getSwapOut( _swappedToken, USDCContract, debtors[_trader].swappedAmount);
        }
        else
            return 0;
    }
    function getLoanInterestAmount(address _trader)public view returns (uint256){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
    ...
    function getUserBenefit(address _trader) public view returns(int256 _withdrawableAmount){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
            uint256 swapOutAmount = ISwapHelper(SwapHelper).getSwapOut( _swappedToken, USDCContract, debtors[_trader].swappedAmount);
    ...
    function endTrading( address _trader ) public {
        ...
        (,uint256 MaxEndMLimit,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
        ...
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( _swappedToken, USDCContract, debtors[_trader].swappedAmount);
    ...
    function IsNeededLiquidation(address _trader) public view returns (bool) {
        address _swappedToken = debtors[_trader].swappedToken;
        (,,,,,uint256 DailyLoanInterestRate,uint256 LiquidationLimitRate,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
        if( debtors[_trader].tradingState == false )
            return false;
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut(debtors[_trader].swappedToken , USDCContract, debtors[_trader].swappedAmount );
    ...
    function setAutoStartTrading( uint256 _borrowAmount, address _swappedToken, uint256 swappedDownLimit, uint256 swappedUpLimit ) external {
        ...
        require( _borrowAmount <= _borrowableAmount, "Over full borrowable amount limit" );
        (,, uint256 MinCollateralLimit,,,,,) = ISwapHelper(SwapHelper).TradingInfo( USDCContract,_swappedToken );
    ...
    function checkAutoStarting(address _trader) public view returns(bool){
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut( USDCContract, autoStarters[_trader].swappedToken, autoStarters[_trader].debtAmount );
