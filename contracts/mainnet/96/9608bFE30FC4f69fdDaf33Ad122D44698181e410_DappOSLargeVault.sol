...
contract LZAdapter is AccessControl, ILayerZeroReceiver {
    ...
    function _sendToLayerZero(
        ...
        (uint256 layerZeroNativeFee, ) = ILayerZeroEndpoint(endpoint)
            .estimateFees(
        ...
        layerZeroNativeFee *= 2;

        ILayerZeroEndpoint(endpoint).send{value: layerZeroNativeFee}(
            chainIdToLzId[dstChainId],
            abi.encodePacked(vaultAddress[dstChainId], address(this)),
            abi.encodePacked(digest),
            payable(address(this)), // Set tx.origin as the receiver of remaining gas
            address(this),
            ""
        );
