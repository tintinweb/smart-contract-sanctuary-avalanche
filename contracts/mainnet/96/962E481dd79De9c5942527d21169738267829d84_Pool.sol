...
contract Pool is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_yTokenFund != address(0), "Pool::initialize: invalidAddress");
        xToken = IXToken(_xToken);
        yToken = IYToken(_yToken);
        yTokenFund = IYTokenFund(_yTokenFund);
        xToken.setMinter(address(this));
        yTokenFund.setPool(address(this));
    }
    ...
    function mint(uint256 _minXTokenOut) external payable nonReentrant {
        ...
        if (_fee > 0) {
            WethUtils.transfer(reserve, _fee);
        }
    ...
    function collect() external nonReentrant {
        ...
        if (_sendYToken) {
            yTokenFund.transfer(_sender, _yTokenAmount);
        }
    ...
    function reduceExcessCollateral(uint256 _amount) external onlyOwner {
        ...
        if (exceeded && _excessWethBal > 0) {
            ...
            require(address(reserve) != address(0), "Pool::reduceExcessCollateral: invalid address");
            WethUtils.transfer(address(reserve), _amount);
        }
    }
    ...
    /// @notice Set the address of Reserve
    /// @param _reserve address of Reserve contract
    function setReserve(address _reserve) external {
        require(reserve == address(0), "Pool::setReserve: not allowed");
        reserve = _reserve;
    }
