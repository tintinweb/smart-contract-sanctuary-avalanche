...
contract APRHelper is Initializable, OwnableUpgradeable {
    ...
    function getUSDCaprForzJoe() public view returns (uint256) {
        uint256 usdcAmount = 375302448 * 10**8;
        uint256 timePeriod = 3600 * 24;
        uint256 numberOfCyles = (3600 * 24 * 365) / timePeriod;
        (, , uint256 sizeOfPool, ) = IMasterChefVTX(masterChief).getPoolInfo(
        ...
        uint256 zJoeTVL = (sizeOfPool *
            ratio *
            getTokenPricePairedWithAvax(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd)) /
            (10**(18 + precision));
        return (usdcAmount * numberOfCyles * FEE_DENOMINATOR) / zJoeTVL;
    }

    function getUSDCaprForLVTX() public view returns (uint256) {
        uint256 usdcAmountZjoe = 375302448 * 10**8;
        (, , uint256 sizeOfPool, ) = IMasterChefVTX(masterChief).getPoolInfo(
            0x769bfeb9fAacD6Eb2746979a8dD0b7e9920aC2A4
        );
        uint256 ratio = ILockerV2(locker).getZjoeStaked() * (10 **precision) / sizeOfPool; 
        usdcAmountZjoe = usdcAmountZjoe * ratio / (10 **precision);
        uint256 usdcAmount = 41700272 * 10**8 + usdcAmountZjoe;
        uint256 timePeriod = 3600 * 24;
        uint256 numberOfCyles = (3600 * 24 * 365) / timePeriod;
        return (usdcAmount * numberOfCyles * FEE_DENOMINATOR) / getTVLForLocker();
    }

    function getATHaprForLVTX(uint256 priceATH) public view returns (uint256) {
        uint256 athAmount = 500000;
        return ((priceATH * athAmount * 12) * 10**(precision) * 100) / getTVLForLocker();
    }

    function getxHUMaprForLVTX(uint256 priceHUM, uint256 ratio) public view returns (uint256) {
        uint256 humAmount = 250000;
        return (priceHUM * ratio * humAmount * 12 * 100) / getTVLForLocker();
    }

    function getxHUMaprForxPTP(uint256 priceHUM, uint256 ratio) public view returns (uint256) {
        uint256 humAmount = 250000;
        uint256 xPTPTotalValue = (ERC20(xPTP).totalSupply() *
            getTokenPricePairedWithAvax(ptp) *
            getRatio(xPTP, ptp, 8)) / 10**(8 + ERC20(xPTP).decimals());
        return (priceHUM * ratio * humAmount * 12 * 100) / xPTPTotalValue;
    }
}
...
interface ILockerV2 {
    ...
    function getZjoeStaked() external view returns (uint256);
