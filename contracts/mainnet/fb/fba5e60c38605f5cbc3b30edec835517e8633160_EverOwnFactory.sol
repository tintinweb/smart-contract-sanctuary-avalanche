...
contract EverOwnFactory is Context, Ownable {
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4); //mainnet
    ...
    function unlockLiquidity(address payable cAddress, address payable eoAddress) public onlyOwner {
        address liquidityPair = IUniswapV2Factory(uniswapV2Router.factory())
            .getPair(cAddress, uniswapV2Router.WAVAX());
        EverOwn(eoAddress).unlockLiquidity(liquidityPair);
    }

    function unlockLiquidityByLPPair(address pairAddress, address payable eoAddress) public onlyOwner {
        EverOwn(eoAddress).unlockLiquidity(pairAddress);
    }
