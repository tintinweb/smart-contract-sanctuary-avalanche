...
/*----------------------------------------------------*/
pragma solidity ^0.8.11;
contract AdventureDAO is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function viewGame(uint cardID) public view returns(
        ...
        situations[0] = action_text[c.textID];
        for(i=1; i<10; i++){
            c = cards[c.parent];
    ...
    function serverSoftwareIsReady() public {
        require(msg.sender == pineapples && HEAVY_TEXT);
        HEAVY_TEXT = false;
    }
