...
contract VaultResolverVRC20 is DSMath {
    ...
    function deposit(
        ...
        if (setId > 0) {
            setUint(setId, sub(vault.balanceOf(address(this)), sharesBefore));
        }
        emit VaultDeposit(address(vault), address(erc20), realAmt);
    }
    ...
    function redeem(
        ...
        uint256 wantReceived = sub(IERC20(underlying).balanceOf(address(this)), balanceBefore);

        emit VaultWithdraw(address(vault), underlying, wantReceived);
