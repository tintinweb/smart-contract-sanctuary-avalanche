...
contract MasterChef is Ownable {
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        } else if (_from <= bonusEndTime && _to >= bonusEndTime && _to <= bonusEndTime1
        ) {
            return bonusEndTime.sub(_from).mul(BONUS_MULTIPLIER).add(_to.sub(bonusEndTime));
        } else if (_from >= bonusEndTime && _to <= bonusEndTime1) {
            return _to.sub(_from);
        } else if (_from >= bonusEndTime1) {
            return _to.sub(_from).div(10);
        } else {
            return bonusEndTime.sub(_from).mul(BONUS_MULTIPLIER).add(bonusEndTime1.sub(bonusEndTime)).add(
            	_to.sub(bonusEndTime1).div(10)
            );
        }
    }
    ...
    function formatDuration(uint256 sec_) public pure returns (uint256) {
        ...
        else if (sec_ <= 357 days) return 357 days;
        else if (sec_ <= 364 days) return 364 days;
        else return 364 days;
    }
    function getTimeMultiplex(uint256 sec_) public pure returns (uint256) {
        if (sec_ == 0) return 100;
        else if (sec_ <= 7 days) return 100;
        else if (sec_ <= 14 days) return 100;
        else if (sec_ <= 357 days) return 100;
        else if (sec_ <= 364 days) return 100;
        else return 100;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _durationSec, address _for) public payable {
        if (_for == address(0)) _for = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_for];
        require(!pool.isNFT, "!is NFT: not good");
        updatePool(_pid);
        _harvest(pool, user, _for);
        if (msg.value > 0) {
        ...
        if(_amount > 0) {
            ...
            if (user.fundBy == address(0) && _for != address(0)) user.fundBy = msg.sender;
        }
        user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
        emit Deposit(_for, _pid, _amount, 0);
    }

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _amount, address _for) public payable {
        if (_for == address(0)) _for = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_for];
        require(!pool.isNFT, "!is NFT: not good");
        ...
        if (user.fundBy == address(0)) {
            require(msg.sender == _for, "withdraw: not fund");
        } else {
            require(msg.sender == user.fundBy || msg.sender == _for, "withdraw: not fund");
        }
        ...
        if(_amount > 0) {
            ...
            if (user.amount == 0) user.fundBy = address(0);
        ...
        user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
        emit Withdraw(_for, _pid, _amount, buybackAmount);
    }
    ...
    function withdrawEth(address _to, uint256 _amount, bool _isWeth) internal {
        if (_isWeth) {
            ...
            IWETHelper(WETH).withdraw(_amount);
