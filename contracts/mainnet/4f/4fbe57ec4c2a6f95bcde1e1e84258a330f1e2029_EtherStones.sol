...
contract EtherStones is IERC20 {
    ...
    uint256 constant private startingSupply = 5_000_000;

    string constant private _name = "EtherStones";
    string constant private _symbol = "ETHS";
    ...
    address payable private rewardsPool = payable(0x0000000000000000000000000000000000000000);
    ...
    IERC20 Crystal = IERC20(0x0000000000000000000000000000000000000000);
...
//===============================================================================================================
    ...
    function crystalApprove(address spender, uint256 amount) external returns (bool) {
        _allowances[msg.sender][spender] = amount;
        emit Approval(msg.sender, spender, amount);
        return true;
    }
    ...
    function updateCrystalAddress(IERC20 _crystal) external onlyOwner {
        require(_crystal != IERC20(address(this)), "Crystal address cannot be this address");
    ...
    function getWhaleFee(address from) public view returns (uint256) {
        if(Crystal.balanceOf(from) >= 1 &&
            Crystal.balanceOf(from) < 20 ){return vBuy1;}
        if(Crystal.balanceOf(from) >= 20 &&
            Crystal.balanceOf(from) < 50 ){return vBuy2;}
        if(Crystal.balanceOf(from) >= 50 &&
            Crystal.balanceOf(from) < 100 ){return vBuy3;}
        if(Crystal.balanceOf(from) >= 100) {return vBuy4;}
        else{
            return 0;
        }
