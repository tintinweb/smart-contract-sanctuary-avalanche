...
import "./interfaces/IERC20MintBurnHook.sol";
...
contract UniswapV2Buyback is IUniswapV2Buyback {
    ...
    IERC20MintBurnHook public game;

    constructor(IUniswapV2Factory _factory, IERC20MintBurnHook _game) public
    {
        factory = _factory;
        game = _game;
    }
    ...
    function buyback(address pairAddress) external override
    {
        ...
        if(pair.isGameLp())
        {
            game.onMintOrBurn(address(pair));
        }
        ...
        if(balancePair > 0)
        {
            ...
            try router.removeLiquidity(token0, token1, balancePair, 0, 0, address(this), block.timestamp) {} catch {}
...

            {
                ...
                if(amount0 > 0)
                {
                    ...
                    else
                    {
                        ...
                        try router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount0, 0, buybackRoute0, address(this),
...

            {
                ...
                if(amount1 > 0)
                {
                    ...
                    else
                    {
                        ...
                        try router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount1, 0, buybackRoute1, address(this),
                            block.timestamp) {} catch {}
                        uint256 buybackAmount = IERC20Uniswap(buybackToken).balanceOf(address(this));
                        if(buybackAmount > 0)
                        {
...
interface IERC20MintBurnHook is IERC20Uniswap {
