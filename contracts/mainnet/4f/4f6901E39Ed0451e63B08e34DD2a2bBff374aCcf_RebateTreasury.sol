...
contract RebateTreasury is Ownable {
    ...
    constructor(address santa, address santaOracle, address treasury) {
        Santa = IERC20(santa);
        SantaOracle = IOracle(santaOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Santa at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 santaAmount = getSantaReturn(token, amount);
        require(santaAmount <= Santa.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient santa balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + santaAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += santaAmount;
    }

    // Claim available Santa rewards from bonding
    ...
    function setSanta(address santa) external onlyOwner {
        Santa = IERC20(santa);
    }

    // Set Santa oracle

    function setSantaOracle(address oracle) external onlyOwner {
        SantaOracle = IOracle(oracle);
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getSantaPrice() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Santa.transfer(account, claimable);
    }
    ...
    // Calculate Santa return of bonding amount of token

    function getSantaReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 santaPrice = getSantaPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / santaPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 santaPrice = getSantaPrice();
        if (santaPrice < 1e17) return 0; // related to 0.1Avax

        uint256 santaPremium = santaPrice * DENOMINATOR / 1e17 - DENOMINATOR; // related to 0.1Avax
        if (santaPremium < bondThreshold) return 0;
        if (santaPremium <= secondaryThreshold) {
            return (santaPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (santaPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get SANTA price from Oracle

    function getSantaPrice() public view returns (uint256) {
        return SantaOracle.consult(address(Santa), 1e18);
    }
    ...
    // Get claimable vested Santa for account

    function claimableSanta(address account) external view returns (uint256) {
