...
/// @title Slide!
/// @notice Slide contract using a global treasury and multiple hands
contract SlideV2 is GameBase, GameBet, GameChainlink {
    struct RoundChoice {
        uint8 minMark;
        uint8 maxMark;
    }
    ...
    struct GameInfo {
        /// Solidity does not like arrays of structs copied between memory/storage.
        uint8[] minMarks;
        uint8[] maxMarks;
        GameState state;
        address creator;
    ...
    event GameStarted(address indexed creator, uint256 indexed gameId, uint8 subgame, RoundChoice[] choices, address referrer, uint256 betPerRound, uint256 rakePerRound);
    event RequestedGameWinner(uint256 indexed requestId, uint256 indexed gameToClose, uint8 subgame);
    ...
    function _createGame(
        ...
        BetDetails memory details = validateBet(
            treasuryAddress.balance, rakeDistributor.getTotalRake(), choices.length);

        uint numChoices = choices.length;
        uint8[] memory minMarks = new uint8[](numChoices);
        uint8[] memory maxMarks = new uint8[](numChoices);
        for (uint i=0; i<numChoices; i++) {
            uint8 minMark = choices[i].minMark;
            uint8 maxMark = choices[i].maxMark;
            require(minMark >= 1, "Range must be at least 1");
            require(maxMark <= 100, "Range must be at most 100");
            require(maxMark >= minMark, "Top end of range cannot be lower than bottom end of range");
            // require(maxMark - minMark >= 10, "Range size must be at least 10");
            require(maxMark - minMark < 95, "Range size cannot be more than 95");
            // We don't want to expose the treasury to multiple high-multiplier items
            // but we also don't need to be as strict for low-occurrance events. Blending
            // these two is a compromise for that.
            uint256 betAvg = (details.bet + details.betPerRound) / 2;
            uint256 potentialProfit = getPayout(minMark, maxMark, betAvg) - betAvg;
            require(potentialProfit <= (treasuryAddress.balance / maxBetDivisor), "Gamble less, king");
            minMarks[i] = minMark;
            maxMarks[i] = maxMark;
        }
        ...
        fi.minMarks = minMarks;
        fi.maxMarks = maxMarks;
        ...
        emit RequestedGameWinner(requestId, fi.gameId, fi.subgame);
        ...
        emit GameStarted(msg.sender, fi.gameId, fi.subgame, choices, fi.referrer, fi.betPerRound, fi.rakePerRound);
        return fi.gameId;
    }
    ...
    function playGame(
        ...
        _createGame(subgame, choices, address(0));
    }
    /// @notice creates a new game
    function playReferredGame(
        ...
        _createGame(subgame, choices, referrer);
    }

    function getPayout(uint8 minMark, uint8 maxMark, uint256 gambleAmount) internal pure returns(uint256){
        return gambleAmount * 100 / (maxMark - minMark + 1);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        RoundInfo[] memory results = new RoundInfo[](fi.minMarks.length);
        uint256 amountToUser = 0;

        for (uint i=0; i<fi.minMarks.length; i++) {
            uint256 roundRandom = uint256(keccak256(abi.encode(randomWords[0], i)));
            uint8 mark = uint8(roundRandom % 100 + 1);
            results[i].selected = mark;

            uint8 minMark = fi.minMarks[i];
            uint8 maxMark = fi.maxMarks[i];
            uint256 payout = getPayout(minMark, maxMark, fi.betPerRound);
    ...
    function forceRequestWinner(uint256 gameId) external onlyOwner {
        ...
        emit RequestedGameWinner(requestId, gameId, fi.subgame);
    }
