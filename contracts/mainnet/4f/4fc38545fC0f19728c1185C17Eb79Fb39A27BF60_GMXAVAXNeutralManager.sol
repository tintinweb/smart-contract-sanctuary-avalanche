...
contract GMXAVAXNeutralManager is Ownable {
  ...
  // WAVAX lending pool contract
  ILendingPool public immutable lendingPoolWAVAX;
  ...
  /**
    ...
    * @param _lendingPoolWAVAX WAVAX lending pool contract
  ...
  */
  constructor(
    ...
    ILendingPool _lendingPoolWAVAX,
    ...
    vault = _vault;
    lendingPoolWAVAX = _lendingPoolWAVAX;
    ...
    IERC20(address(WAVAX)).approve(address(lendingPoolWAVAX), type(uint256).max);
  ...
  */
  function debtAmts() public view returns (uint256, uint256, uint256, uint256) {
    return (
      lendingPoolWAVAX.maxRepay(address(this)),
  ...
  */
  function debtAmt(address _token) public view returns (uint256) {
    ...
    } else if (_token == WAVAX) {
      return lendingPoolWAVAX.maxRepay(address(this));
    } else {
  ...
  */
  function _removeLiquidity(WorkData calldata _data) internal returns (uint256) {
    uint256 glpAmtInForWAVAX;
    uint256 glpAmtInForWETH;
    uint256 glpAmtInForBTCb;

    // remove lp receive enough WAVAX for repay
    if (_data.repayWAVAXAmt > 0) {
      glpAmtInForWAVAX = gmxOracle.getGlpAmountIn(_data.repayWAVAXAmt, GLP, WAVAX);
    ...
    if (_data.repayBTCbAmt > 0) {
      glpAmtInForBTCb = gmxOracle.getGlpAmountIn(_data.repayBTCbAmt, GLP, BTCb);
    ...
    if (_data.token == USDC) {
      rewardRouter.unstakeAndRedeemGlp(
        USDC,
        _data.lpAmt - glpAmtInForWAVAX - glpAmtInForWETH - glpAmtInForBTCb,
        0,
        address(this)
      );
      return 0;
      ...
      return (_data.lpAmt - glpAmtInForWAVAX - glpAmtInForWETH - glpAmtInForBTCb - glpAmtInForUSDC);
    }
  }
  ...
  */
  function _borrow(WorkData calldata _data) internal {
    if (_data.borrowWAVAXAmt > 0) {
      lendingPoolWAVAX.borrow(_data.borrowWAVAXAmt);
    }
  ...
  */
  function _repay(WorkData calldata _data) internal {
    if (_data.repayWAVAXAmt > 0) {
      lendingPoolWAVAX.repay(_data.repayWAVAXAmt);
    }
    if (_data.repayWETHAmt > 0) {
