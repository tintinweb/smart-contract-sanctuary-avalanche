...
contract AvaxWhalesV2 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, IERC721Receiver {
    bytes4 internal constant MAGIC_ON_ERC721_RECEIVED = 0x150b7a02;
    using Strings for uint256;

    uint256 currentId = 3333;
    ...
    ERC721 constant previous = ERC721(0xC60a0Dff601911d6Aff569d289D17e2dA38d5b46);

    constructor() ERC721("Avax Whales", "AXWHALESv2") {

    }

    function onERC721Received(address , address from, uint256 tokenId, bytes calldata ) external override returns(bytes4) {
        require(previous.ownerOf(tokenId) == address(this) && !_exists(tokenId), "Invalid NFT");
        _safeMint(from, tokenId);
        previous.safeTransferFrom(address(this), burnAddress, tokenId);
   ...
   function mint(uint256[] calldata _ids, address _newOwner) external onlyOwner {
       for (uint256 i = 0; i <_ids.length; i++) {
            require(!_exists(_ids[i]) && _ids[i] < currentId, "Whale already minted or invalid ID");     
            _safeMint(_newOwner, _ids[i]);
       }
    }  
