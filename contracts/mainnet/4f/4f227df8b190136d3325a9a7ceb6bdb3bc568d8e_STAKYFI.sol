...
contract STAKYFI is ERC20WithVotingAndPermit("STAKYFI", "STF"), Ownable {
    uint256 public constant YEAR_TWO_CAP = 10e24; //10 million tokens
    uint256 public immutable END_OF_YEAR_TWO;
    constructor() {
        END_OF_YEAR_TWO = block.timestamp + 730 days;
    }
    function mint(address account, uint256 amount) external onlyOwner {
        if(block.timestamp < END_OF_YEAR_TWO) {
            require((totalSupply() + amount) <= YEAR_TWO_CAP, "mint would exceed cap.");
        }
