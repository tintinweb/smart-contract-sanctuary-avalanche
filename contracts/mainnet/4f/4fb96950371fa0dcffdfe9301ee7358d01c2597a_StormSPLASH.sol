...
// File contracts/stormSPLASH.sol
...
interface ITap {
    function refundTax(uint256 _amount) external;
}

contract StormSPLASH is IERC20, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    // token data
    string private constant _name = "Storm.Money SPLASH";
    string private constant _symbol = "stormSPLASH";
    ...
    uint256 public sellFee = 95000; // 5% redeem fee
    uint256 public transferFee = 95000; // 5% transfer fee
    uint256 public cashedOutFee = 94000; // 6% cashed out fee
    ...
    */
    ...
    function _mintWithBacking(
        ...
        uint256 taxAmount = numUnderlying.mul(10).div(100);
        theTap.refundTax(taxAmount);
    ...
    function _purchaseShelterUnderlyingWithUnderlying(
        ...
        if (address(underlying) == router.WAVAX()) {
            path = new address[](2);
            path[0] = router.WAVAX();
            path[1] = address(shelterUnderlying);
    ...
    */
    ...
    /** Set TheTap Address */
    function setTheTap(address _newTapAddress) external onlyOwner {
        require(_newTapAddress != address(0), "Zero Address");
        theTap = ITap(_newTapAddress);
    }

    /** Mint Tokens to Buyer */
    receive() external payable {
        _mintWithNative(msg.sender, 12, msg.value);
        _checkGarbageCollector(address(this));
        _checkGarbageCollector(DEAD);
    }
