...
contract VaultOracle is Ownable, IPriceFeed {
  using SafeMath for uint;

  IPriceFeed underlyingFeed;
  IVault vault;
  uint256 wad;
  
  function setParam(address _underlyingFeed, address _vault) external onlyOwner {
    vault = IVault(_vault);
    underlyingFeed = IPriceFeed(_underlyingFeed);
    wad = vault.underlyingDecimal();
    wad = 10 ** wad;
  }
