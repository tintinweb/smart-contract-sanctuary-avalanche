...
contract TombGenesisRewardPool {
    ...
    // MAINNET
    uint256 public tombPerSecond = 0.01157 ether; // 25000 TOMB / (48h * 60min * 60s)
    ...
    constructor(
        address _tomb,
        address _daoFund, //tax dao address
        uint256 _poolStartTime
    ) public {
        require(block.timestamp < _poolStartTime, "late");
        if (_tomb != address(0)) tomb = IERC20(_tomb);
        if (_daoFund != address(0)) daoFundAddress = _daoFund; //tax dao address
