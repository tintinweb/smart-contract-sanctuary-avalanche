...
// File: contracts/superToken/AAVELendingToken/AaveLendingErc20.sol
...
//
// This contract handles swapping to and from superToken.
contract AaveLendingErc20 is AaveSuperErc20,lendingSuperToken {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        AaveSuperErc20(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_FeePool) 
        lendingSuperToken(leverageFactory) {

    }
    function availableBalance() public virtual override(AaveSuperToken,superTokenInterface) view returns (uint256){
        return stakeToken.balanceOf(address(this));
    }
    function totalUnderlying()public virtual override(AaveSuperToken,superTokenInterface) view returns (uint256){
...
// File: contracts/debug/AaveLendingErc20Debug.sol
...
//
// This contract handles swapping to and from superToken.
contract AaveLendingErc20Debug is AaveLendingErc20,debugger {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        AaveLendingErc20(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_FeePool,leverageFactory){
