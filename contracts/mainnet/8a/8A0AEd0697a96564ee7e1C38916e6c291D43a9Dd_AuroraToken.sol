...
contract AuroraToken {
  string public constant name = "Aurora";
  string public constant symbol = "AURORA";
  uint8 public constant decimals = 18;
  uint256 private constant scale = 2**64;
  ...
  function transferAndCall(
    ...
    if (_size > 0) {
      require(
        CallableContract(_to).tokenCallback(msg.sender, _transferred, _data)
      );
    }
    return true;
  }
  ...
  function _transfer(
    ...
    require(balanceOf(_from) >= _tokens, "Insufficient balance");
    info.users[_from].balance -= _tokens;
    uint256 _burnedAmount = (_tokens * transferTax) / 100;

    //check the minimum supply percentage and if buring is disabled
    if (
      totalSupply() - _burnedAmount < (1e26 * 10) / 100 ||
      isBurningDisabled(_from)
    ) {
      _burnedAmount = 0;
    }
  ...
  function _unstake(uint256 _amount) internal {
    ...
    uint256 _burnedAmount = (_amount * (transferTax / 2)) / 100;
    info.scaledPayoutPerToken += (_burnedAmount * scale) / info.totalStaked;
    info.totalStaked -= _amount;
  ...
  function infoFor(address _user)
    public
    view
    returns (
