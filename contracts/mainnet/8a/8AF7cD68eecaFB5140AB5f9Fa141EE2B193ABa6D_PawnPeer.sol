...
interface IERC721Shipper {
    function transferToken(address collection, address from, address to, uint256 tokenId) external;
}
...
library DataTypes {
    ...
    function hashListing(Listing memory order) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            keccak256("Listing(address maker,address collection,uint256 tokenId,uint256 askPrice,uint32 maxLockTime,uint32 expiresAt,uint8 dailyInterest,uint32 nonce)"),
            order.maker,
            order.collection,
            order.tokenId,
            order.askPrice,
    ...
    function hashOffer(Offer memory offer) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            keccak256("Offer(address taker,address collection,uint256 tokenId,uint256 bidPrice,uint32 expiresAt,uint8 collectionOffer,uint32 maxLockTime,uint8 dailyInterest,uint32 nonce)"),
            offer.taker,
            offer.collection,
            offer.tokenId,
            offer.bidPrice,
...
import {TokenIdentifier} from "./TokenIdentifier.sol";
import {IFeeManager} from "./interfaces/IFeeManager.sol";
import {SignatureVerifier} from "./SignatureVerifier.sol";
import {IERC721Shipper} from "./interfaces/IERC721Shipper.sol";
...
contract PawnPeer is SignatureVerifier, TokenIdentifier, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    address public erc721ShipperAddress;

    uint256 public constant percentageMultiplier = 100;

    uint32 public maxAllowedLockTime;
    uint8 public minDailyInterest;
    uint8 public maxDailyInterest;

    mapping(address => uint32) public userMinNonce;
    mapping(address => mapping(uint32 => uint8)) private nonceExecutedOrCancelled;

    uint32 public nextDealId = 1;
    mapping(uint32 => DataTypes.Deal) public idToDeal;
    ...
    event NewDeal(uint32 deealId);
    event DealClosed(uint32 deealId);
    event CancelAllOrdersAndOffers(address user, uint32 minNonce);
    event CancelSingleOrderOrOffer(address user, uint32 nonce);
    event CancelMultipleOrdersAndOffers(address user, uint32[] nonceList);
    ...
    constructor(address _wavaxAddress, address _paymentReceiver) {
        ...
        maxAllowedLockTime = 90 days;
    }

    // --------- OWNER FUNCTIONS ---------
    function setERC721ShipperAddress(address _adr) external onlyOwner {
        erc721ShipperAddress = _adr;
    }

    function setMaxLockTime(uint32 _maxLockTime) external onlyOwner {
        maxAllowedLockTime = _maxLockTime;
    }

    function setFeeManagerAddress(address _adr) external onlyOwner {
        feeManagerAddress = _adr;
    }

    // 0 - 10000
    function setDailyInterest(uint8 _min, uint8 _max) external onlyOwner {
    ...
    function acceptDeal(DataTypes.Listing calldata listing) external nonReentrant whenNotPaused payable {
        ...
        IERC721Shipper(erc721ShipperAddress).transferToken(listing.collection, listing.maker, address(this), listing.tokenId);
        createDeal(listing.collection, listing.tokenId,
    ...
    function acceptMultipleDeals(DataTypes.Listing[] calldata listings) external nonReentrant whenNotPaused payable {
        ...
        for (uint256 i = 0; i < listings.length; i += 1) {
            ...
            validateListingAndRevert(listing);
            nonceExecutedOrCancelled[listing.maker][listing.nonce] = 1;
            IERC721Shipper(erc721ShipperAddress)
                .transferToken(listing.collection, listing.maker, address(this), listing.tokenId);
            createDeal(listing.collection, listing.tokenId,
                listing.maxLockTime, listing.maker, msg.sender, listing.askPrice, listing.dailyInterest);
            transferDealFunds(listing.collection, listing.maker, listing.askPrice, transferFromAddress);
        }
    ...
    function acceptOffer(DataTypes.Offer calldata offer, uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        if (offer.collectionOffer == 0) {
            tokenId = offer.tokenId;
        }
        if (!isOwnerValid(msg.sender, offer.collection, tokenId)) {
            revert Unauthorized();
        }
        validateOfferAndRevert(offer);
        nonceExecutedOrCancelled[offer.taker][offer.nonce] = 1;
        IERC721Shipper(erc721ShipperAddress)
            .transferToken(offer.collection, msg.sender, address(this), tokenId);
        createDeal(offer.collection, tokenId,
        ...
        emit CancelSingleOrderOrOffer(offer.taker, offer.nonce);
    }

    function createDeal(address collection, uint256 tokenId, uint32 maxLockTime,
        address maker, address taker, uint256 dealPrice, uint8 dailyInterest) internal {
        uint32 expiresAt = maxLockTime + uint32(block.timestamp);
        unchecked {
            expiresAt = maxLockTime + uint32(block.timestamp);
        }
        idToDeal[nextDealId] = DataTypes.Deal({
            ...
            tokenId: tokenId,
            dealPrice: dealPrice,
            acceptedTs: uint32(block.timestamp),
            expiresTs: expiresAt,
            dailyInterest: dailyInterest,
            closed: 0
        });
        emit NewDeal(nextDealId);
        unchecked {
            nextDealId += 1;
        }
    }
    ...
    function transferDealFunds(address collection, address maker, uint256 dealPrice, address transferFundsFrom) internal {
        (uint32 platformFee, uint32 platformDiscount, uint8 feeType, address secondary) = IFeeManager(feeManagerAddress).getFeeDetails(collection, maker);
        uint256 platformAmount = (dealPrice * platformFee) / (100 * percentageMultiplier);
        uint256 platformDiscountAmount = 0;
        if (feeType == 2) { // 2 means fee sharing
            platformDiscountAmount = (dealPrice * platformDiscount) / (100 * percentageMultiplier);
        }
        uint256 makerAmount = dealPrice - (platformAmount + platformDiscountAmount);
        IERC20 wavax = IERC20(wavaxAddress);
        wavax.safeTransferFrom(transferFundsFrom, maker, makerAmount);
        wavax.safeTransferFrom(transferFundsFrom, paymentReceiver, platformAmount);
        if (feeType == 2 && platformDiscountAmount > 0) {
            wavax.safeTransferFrom(transferFundsFrom, secondary, platformDiscountAmount);
        }
    }

    function getCloseNowPrice(uint32 dealId) external view returns(uint256) {
        DataTypes.Deal memory deal = idToDeal[dealId];
        if (deal.acceptedTs == 0 || deal.closed == 1) {
            return 0;
        }
    ...
    function closeDeal(uint32 dealId) external nonReentrant payable {
        ...
        if (deal.expiresTs < block.timestamp) {
            deal.closed = 1;
            transferNFTFromContract(deal.collection, deal.tokenId, deal.taker);
        } else {
            // Payment
            ...
            IERC20(wavaxAddress).safeTransferFrom(transferFromAddress, deal.taker, totalPayment);
            transferNFTFromContract(deal.collection, deal.tokenId, deal.maker);
        }
        emit DealClosed(dealId);
    }

    function forceCloseDeal(uint32 dealId) external nonReentrant onlyOwner {
        DataTypes.Deal storage deal = idToDeal[dealId];
        deal.closed = 1;
        transferNFTFromContract(deal.collection, deal.tokenId, deal.taker);
        emit DealClosed(dealId);
    ...
    function isOwnerValid(address owner, address collection, uint256 tokenId) internal view returns(bool) {
    ...
    function transferNFTFromContract(address collection, uint256 tokenId, address to) internal {
        if (isERC721(collection)) {
    ...
    function validateListingAndRevert(DataTypes.Listing calldata listing) public view {
        if (listing.dailyInterest < minDailyInterest || listing.dailyInterest > maxDailyInterest || listing.askPrice == 0 || listing.maxLockTime > maxAllowedLockTime) {
            revert OutOfBounds();
        }
    ...
    function validateOfferAndRevert(DataTypes.Offer calldata offer) public view {
        if (offer.dailyInterest < minDailyInterest || offer.dailyInterest > maxDailyInterest || offer.bidPrice == 0 || offer.maxLockTime > maxAllowedLockTime) {
            revert OutOfBounds();
        }
    ...
    function cancelAllOrdersAndOffers(uint32 minNonce) external nonReentrant whenNotPaused {
        if (minNonce < userMinNonce[msg.sender] || minNonce > userMinNonce[msg.sender] + 50000) {
            revert OutOfBounds();
        }
    ...
    function cancelMultipleOrdersAndOffers(uint32[] calldata nonceList) external nonReentrant whenNotPaused {
...
contract TokenIdentifier is ITokenIdentifier, IERC165 {
    using ERC165Checker for address;

    bytes4 public constant IID_ITOKEN_IDENTIFIER = type(ITokenIdentifier).interfaceId;
    ...
    function getTokenType(address nftAddress) public view returns (uint8) {
        if (isERC721(nftAddress)) {
            return 1;
        } else if (isERC1155(nftAddress)) {
            return 2;
        }
        return 3;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == IID_ITOKEN_IDENTIFIER || interfaceId == IID_IERC165;
    }
}