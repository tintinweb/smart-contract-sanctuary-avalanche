...
contract MillionLegends is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function flattenCost_1Time() public{
        require( flat>0 && msg.sender == pineapples);
        flat -=1;
        cardCost = 0;
    }
    ...
    function newPath(uint[] memory cardIDs, string[] memory actions, string[] memory situations) public payable returns(uint otID){
        ...
        uint[] memory UINTs = new uint[](4);//dodging bullets
        ...
        if(cardCost>0){
            ...
            minecart(UINTs[3]);
            require( $MM.transferFrom(msg.sender, THIS, cardCost*UINTs[0]) );
        }
        
        otID = ORACLE.fileRequestTicket{value: msg.value}(1, true);
    ...
    uint public tallest;
    mapping( uint => uint) head;
    mapping( uint => uint) tail;
    mapping( uint => mapping( uint => uint)) rank_order_ofEdge;

    function resetMinecart() internal{
        if(tail[tallest] == head[tallest] && tallest>0){
            tallest -= 1;
        }
        uint card = rank_order_ofEdge[tallest][tail[tallest]];
        if(cards[card].lead == tallest){
            cart = card;
            cards[card].lead = 0;
            tail[tallest] += 1;
        }else{
            cart = cardCount-1; // shatter fall
            // As the cart falls,
            // random branches light up
        }
        
    }

    function pushRank(uint cardID, uint height, uint parentID) internal{
        // take care of removing parent from downlines competing to be next
        uint order;
        if(height>0){
            order = cards[parentID].rankPosition;
            uint _head = head[height];
            
            rank_order_ofEdge[height][order] = rank_order_ofEdge[height][ _head ];
            if(_head > tail[height]){
                head[height] -= 1;
            }
        
            cards[parentID].lead = cards[parentID].lead;
        }

        height += 1;

        cards[cardID].lead = height;
        ...
        order = head[height];
        rank_order_ofEdge[height][order] = cardID;
        cards[cardID].rankPosition = order;
        head[height] += 1;
    }
    ...
    function uriRequest(uint cardID, string memory URI) public payable{
        ...
        uint otID = ORACLE.fileRequestTicket{value: msg.value }(1, true);
        if( !isOwner ){
            require( $MM.transferFrom(msg.sender, THIS, cardCost) );
    ...
    function mintUniqueTokenTo(
        ...
        pushRank(cardCount, cards[parent].lead, parent);
...
/*
The money tree. The number contributed in MM is expected to be smaller than the number contributed in AVAX eventually. But it could be near or larger since MM still has to grow. So what's being passed up in the beginning is vaulted up the tree in form of small amounts.
