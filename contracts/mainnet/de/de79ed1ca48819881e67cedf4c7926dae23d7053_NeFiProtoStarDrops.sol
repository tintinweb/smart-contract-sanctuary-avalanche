...
contract NeFiProtoStarDrops is Ownable{
	...
	ProtoManager public OLDprotoMGR;
	dropManager public dropMGR;
	dropManager2 public dropMGR2;
	...
	constructor(address[] memory addresses){
		...
		_feeToken = addresses[2];
		protoMGR = ProtoManager(addresses[3]);
	...
	function getallElse(uint _x, uint _y) external onlyOwner()  {
		...
		for(j= _x;j < _y ; j++ ){
			...
			if(NeFiLib.addressInList(gElse,_account) ==false){
				airdrop[_account].claimed = protoMGR.getProtoStarsLength(_account);
				getRemaining(_account);
				_k = airdrop[_account].claimed;
				for(i = 0;i < _k ; i++){
					(string memory name,uint256 b,uint256 c,uint256 d,uint256 e,uint256 f,uint256 g,uint256 h,uint256 i) = protoMGR.protoAccountData(_account,i);
