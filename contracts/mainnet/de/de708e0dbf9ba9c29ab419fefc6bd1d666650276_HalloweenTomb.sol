...
contract HalloweenTomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 12000 ether;
    // Initial distribution for the day 2-5 HTOMB-WFTM LP -> HTOMB pool
    uint256 public constant INITIAL_HTOMB_POOL_DISTRIBUTION = 0 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HalloweenTomb", "HTOMB") {
    ...
    function _updateTaxRate(uint256 _htombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_htombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHtombOracle(address _htombOracle) public onlyOperatorOrTaxOffice {
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHtombPrice = _getHtombPrice();
            currentTaxRate = _updateTaxRate(currentHtombPrice);
            if (currentHtombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        _mint(_htombPool, INITIAL_HTOMB_POOL_DISTRIBUTION);
