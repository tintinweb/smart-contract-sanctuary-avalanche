...
contract OpLong is OpCommon {
    ...
    function onWithoutFlashLoan(bytes memory data) internal {
        uint8 operation;
        bytes memory arguments;
    ...
    function handleOpenLong(bytes memory arguments) internal {
        ...
        if (amountFlashLoan == 0) {} else {
            flashLoanFee = IERC3156FlashLender(lender).flashFee(
                leverageToken,
                amountFlashLoan
            );
        }
