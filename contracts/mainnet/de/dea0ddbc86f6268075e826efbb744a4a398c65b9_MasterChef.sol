...
contract MasterChef is Ownable {
    ...
    /// @notice Address of reward (SUPREME) contract.
    ...
    modifier onlyFunder() {
        require(msg.sender == owner() || funder[msg.sender] == true, "MasterChef: caller is not a funder");
        _;
    }
    ...
    function fundRewards(uint256 funding, uint256 duration) external onlyFunder {
        require(funding > 0, "MasterChef: funding cannot be zero");

        REWARD.safeTransferFrom(msg.sender, address(this), funding);

        if (block.timestamp >= rewardsExpiration) {
            require(duration > 0, "MasterChef: reward duration cannot be zero");
            massUpdateAllPools();
    ...
    function extendRewardsViaFunding(uint256 funding, uint256 minExtension) external {
        require(funding > 0, "MasterChef: funding amount cannot be zero");

        uint256 extensionDuration = funding / rewardPerSecond;
        require(extensionDuration >= minExtension, "MasterChef: insufficient extension limit");
    ...
    function extendRewardsViaDuration(uint256 extension, uint256 maxFunding) external {
        require(extension > 0, "MasterChef: extension duration cannot be zero");

        uint256 fundingRequired = rewardPerSecond.mul(extension);
        require(fundingRequired <= maxFunding, "MasterChef: insufficient funding limit");
