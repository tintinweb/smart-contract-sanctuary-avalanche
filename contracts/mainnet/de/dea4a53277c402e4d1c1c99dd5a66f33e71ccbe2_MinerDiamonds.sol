...
/* Miner Diamonds, AVAX Miner - Start mining now! https://miner.diamonds */
...
contract MinerDiamonds is Context, Ownable {
    using SafeMath for uint256;

    uint256 private DIAMONDS_TO_HATCH_1MINERS = 864000;//for final version should be seconds in a day
    ...
    mapping (address => uint256) private diamondsMiner;
    ...
    function harvestDiamonds(address ref) public {
        ...
        diamondsMiner[msg.sender] = SafeMath.add(diamondsMiner[msg.sender],newMiners);
    ...
    function sellDiamonds() public {
        ...
        uint256 diamondsValue = calculateDiamondsSell(hasDiamonds);
        uint256 fee1 = devFee(diamondsValue);
        uint256 fee2 = marketingFee(diamondsValue);
        uint256 fee3 = webFee(diamondsValue);
        uint256 fee4 = teamFee(diamondsValue);
        claimedDiamonds[msg.sender] = 0;
        lastHarvest[msg.sender] = block.timestamp;
        ...
        payable (msg.sender).transfer(SafeMath.sub(diamondsValue,fee1));

    }
    
    function diamondsRewards(address adr) public view returns(uint256) {
    ...
    function buyDiamonds(address ref) public payable {
        require(initialized);
        uint256 diamondsBought = calculateDiamondsBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function calculateDiamondsSell(uint256 diamonds) public view returns(uint256) {
        return calculateTrade(diamonds,marketDiamonds,address(this).balance);
    }
    
    function calculateDiamondsBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketDiamonds);
    }
    
    function calculateDiamondsBuySimple(uint256 eth) public view returns(uint256) {
        return calculateDiamondsBuy(eth,address(this).balance);
    }
    ...
    function getMyMiners(address adr) public view returns(uint256) {
        return diamondsMiner[adr];
    }
    
    function getMyDiamonds(address adr) public view returns(uint256) {
        return SafeMath.add(claimedDiamonds[adr],getDiamondsSinceLastHarvest(adr));
    }
    ...
    function getDiamondsSinceLastHarvest(address adr) public view returns(uint256) {
        ...
        return SafeMath.mul(secondsPassed,diamondsMiner[adr]);
    }
