/**
 *Submitted for verification at snowtrace.io on 2022-10-04
*/
...
//        outputToLp1SymbolRoute = _getSymbolRoute(outputToLp1Route);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(rewardPool, uint256(type(uint256).max));
        IERC20(output).safeApprove(unirouter, uint256(type(uint256).max));

        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, uint256(type(uint256).max));

        IERC20(lpToken1).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, uint256(type(uint256).max));
    }
