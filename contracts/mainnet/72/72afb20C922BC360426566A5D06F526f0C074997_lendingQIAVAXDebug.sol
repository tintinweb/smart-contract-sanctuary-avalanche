...
// File: contracts/superToken/benqiSuperToken/superQiToken.sol
...
// File: contracts/superToken/benqiSuperToken/superQiAvax.sol
...
contract superQiAvax is superQiToken {
    ...
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _wavax,
        address payable _swapHelper,address payable _FeePool)
            superQiToken(multiSignature,origin0,origin1,_stakeToken,_swapHelper,_FeePool) {
        underlying = IERC20(_wavax);
        wavax = IWAVAX(_wavax);
    }
    function deposit(address,uint256)internal virtual override returns(uint256){
    ...
    function withdraw(address account,uint256 _amount)internal virtual override returns(uint256){
        uint256 amount = ICEther(address(stakeToken)).redeemUnderlying(_amount);
    ...
    function qiSupply(uint256 _fee) internal returns (uint256){
        ...
        if(balance > 0){
            ...
            ICEther(address(stakeToken)).mint{value:balance}();
            return balance;
        }
        return 0;
    }
    ...
    function lendTo(address account,uint256 amount) internal virtual override {
        amount = ICEther(address(stakeToken)).redeemUnderlying(amount);
        wavax.deposit{value:amount}();
        IERC20(address(wavax)).safeTransfer(account, amount);
    }
    function repayFrom(address account,uint256 amount) internal virtual override{
        IERC20(address(wavax)).safeTransferFrom(account, address(this), amount);
        wavax.withdraw(amount);
        ICEther(address(stakeToken)).mint{value:amount}();
    }
}
...
// File: contracts/superToken/benqiLendingToken/lendingQIAVAX.sol
...
//
// This contract handles swapping to and from superToken.
contract lendingQIAVAX is superQiAvax,lendingSuperToken {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _wavax,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        superQiAvax(multiSignature,origin0,origin1,_stakeToken,_wavax,_swapHelper,_FeePool)
...
// File: contracts/debug/lendingQIAVAXDebug.sol
...
//
// This contract handles swapping to and from superToken.
contract lendingQIAVAXDebug is lendingQIAVAX,debugger {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    // Define the qiToken token contract
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _wavax,
    address payable _swapHelper,address payable _FeePool,address leverageFactory)
        lendingQIAVAX(multiSignature,origin0,origin1,_stakeToken,_wavax,_swapHelper,_FeePool,leverageFactory){
