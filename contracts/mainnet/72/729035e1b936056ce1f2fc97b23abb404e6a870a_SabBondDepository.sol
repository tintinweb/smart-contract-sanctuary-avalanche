...
interface ITreasury {
    function deposit( uint _amount, address _token, uint _value, uint _profit ) external;
...
contract SabBondDepository is Ownable {
    ...
    IWETH9 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Sab when receives principle
    ...
    uint32 public discountRate = 85; //5% discount in bond price
    ...
    function deposit( 
        ...
        value = value.mul( assetPrice() ) / 10**priceFeed.decimals();
    ...
    function withdraw (address _recipient, uint _amount) external returns (uint) {
        ...
        require((uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[1]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[1]).add32(86400)) 
            || (uint32(block.timestamp) >= info.lastTime.add32(withdrawTerm[2]) && uint32(block.timestamp) <= info.lastTime.add32(withdrawTerm[2]).add32(86400)), 
    ...
    function setWithdrawFee (uint32 _withdrawfee) external onlyPolicy() {
        require(_withdrawfee >= 0 && _withdrawfee <= 100, 'WithdrawFee must less than 100%');
        withdrawFee = _withdrawfee;
    }

    function setDiscountRate (uint32 _discountrate) external onlyPolicy() {
