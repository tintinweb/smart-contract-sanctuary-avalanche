...
contract ZapVTX is Ownable {
    ...
    /* ========== External Functions ========== */
    function previewAmount(address _from, uint256 amount)
        external
        view
        returns (uint256)
    {
        address[] memory path = _findRouteToAvax(_from);
        uint256[] memory amounts = ROUTER.getAmountsOut(amount, path);
        IJoePair pair = IJoePair(LP1);
        address token0 = pair.token0();
        address token1 = pair.token1();

        uint256[] memory amounts0 = ROUTER.getAmountsOut(
            amounts[amounts.length - 1] / 2,
            _findRouteFromAvax(token0)
        );
        uint256[] memory amounts1 = ROUTER.getAmountsOut(
            amount - (amounts[amounts.length - 1] / 2),
            _findRouteFromAvax(token1)
        );
        uint256 amount0 = amounts0[amounts0.length - 1];
        uint256 amount1 = amounts1[amounts1.length - 1];
        (uint256 reserve0, uint256 reserve1, ) = PAIR.getReserves();
        uint256 amount1Optimal = quote(amount0, reserve0, reserve1);
        if (amount1Optimal <= amount1) {
            require(amount1Optimal >= 0, "JoeRouter: INSUFFICIENT_B_AMOUNT");
            ...
            require(amount0Optimal >= 0, "JoeRouter: INSUFFICIENT_A_AMOUNT");
            (amount0, amount1) = (amount0Optimal, amount1);
        }
        uint256 _liquidityFee = estimateLiquidityFee(reserve0, reserve1);
        uint256 _totalSupply = PAIR.totalSupply() + _liquidityFee; // gas savings, must be defined here since totalSupply can update in _mintFee
        return
            Math.min(
    ...
    function migrateZap(uint256 amount) external {
        IERC20(LP0).safeTransferFrom(msg.sender, address(this), amount);
        IERC20(LP0).approve(address(ROUTER), amount);
        ROUTER.removeLiquidity(WAVAX, VTX, amount, 0, 0, address(this), block.timestamp+2000);
        uint256 vtxAmount = IERC20(VTX).balanceOf(address(this));
        IERC20(VTX).approve(newVtx, vtxAmount);
        IVTXnew(newVtx).deposit(vtxAmount);
        zapInToken(newVtx, vtxAmount);
    ...
    //     _swapAVAXToLP(LP1, msg.value, msg.sender);
    // }
