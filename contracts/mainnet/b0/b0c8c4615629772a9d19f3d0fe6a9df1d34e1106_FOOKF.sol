...
// Dependency file: contracts/baby/FOOKFDividendTracker.sol
...
contract FOOKFDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FOOKF contract."
        );
    }
...
// Root file: contracts/baby/FOOKF.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/FOOKFDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract FOOKF is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    FOOKFDividendTracker public dividendTracker;

    address public rewardToken;

    uint256 public swapTokensAtAmount;

    uint256 public maxAmountPerTrx;
    ...
    constructor(
        ...
        address dividendTrackerAddresss,
        uint256 minimumTokenBalanceForDividends_
        ...
        maxAmountPerTrx = totalSupply_.div(100); // 1%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = FOOKFDividendTracker(
            payable(Clones.clone(dividendTrackerAddresss))
        );
        dividendTracker.initialize(
            rewardToken,
            string(abi.encodePacked(name_, '_DIVIDEND_TRACKER')),
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "FOOKF: The dividend tracker already has that address"
        );

        FOOKFDividendTracker newDividendTracker = FOOKFDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "FOOKF: The new dividend tracker must be owned by the FOOKF token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "FOOKF: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "FOOKF: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setMaxAmountPerTrx(uint256 value) external onlyOwner {
        maxAmountPerTrx = value;
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "FOOKF: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "FOOKF: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "FOOKF: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "FOOKF: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        require(isPairTx && (amount > maxAmountPerTrx), "FOOKF: amount bigger as allowed");
        ...
        if ( contractTokenBalance > maxAmountPerTrx) {
            contractTokenBalance = maxAmountPerTrx;
        }
        ...
        // if pair is not involved, don't take fees
        if (
