...
contract PowerBombAvaxCurveSAVAX is PowerBombAvaxCurve {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for IWAVAX;
    using SafeERC20Upgradeable for ISAVAX;

    ISAVAX constant sAVAX = ISAVAX(0x2b2C81e08f1Af8835a78Bb2A90AE924ACE0eA4bE);

    function initialize(
        IPool _pool, IGauge _gauge, address _treasury, address _bot
        ...
        rewardToken = IERC20Upgradeable(address(sAVAX));
        ...
        USDT.safeApprove(address(_pool), type(uint).max);
        USDC.safeApprove(address(_pool), type(uint).max);
        DAI.safeApprove(address(_pool), type(uint).max);
        lpToken.safeApprove(address(_pool), type(uint).max);
        lpToken.safeApprove(address(_gauge), type(uint).max);
    ...
    function _harvest(bool) internal override {
        ...
        if (WAVAXAmt > minSwapAmt) {
            ...
            // Withdraw WAVAX to AVAX
            WAVAX.withdraw(WAVAXAmt);

            // Stake AVAX into Benqi
            uint rewardTokenAmt = sAVAX.submit{value: address(this).balance}();
            ...
            accRewardPerlpToken += (rewardTokenAmt * 1e36 / gauge.balanceOf(address(this)));
