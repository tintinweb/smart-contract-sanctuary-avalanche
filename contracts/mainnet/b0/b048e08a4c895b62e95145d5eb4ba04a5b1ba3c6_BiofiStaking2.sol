...
contract BiofiStaking2 is Ownable {
    ...
    function readPrincipalInterest(address staker, uint256 activityCount) external view onlyOwner returns (uint256 principal, uint256 interest) {
        Stake memory stake = userStake[staker];
    ...
    function closeStake(bytes [] memory activityProofs) external nonReentrant {
        ...
        Staker memory staker = stakerIds[msg.sender];
        if(staker.exists) {
            stakers[staker.id] = stakers[stakers.length-1];
            stakers.pop();
        }
