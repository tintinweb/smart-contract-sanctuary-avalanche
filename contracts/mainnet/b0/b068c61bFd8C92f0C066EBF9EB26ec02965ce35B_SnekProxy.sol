...
import "../interfaces/ISnekVoter.sol";
import "../interfaces/IVeDist.sol";
import "../interfaces/ISnekMinter.sol";
import "../interfaces/ISolidlyRouter.sol";
import "../interfaces/ISnekGauge.sol";

contract SnekProxy is OwnableUpgradeable {
    ...
    event SetCpSNEK(address oldValue, address newValue);
    event SetLPDepositor(address oldValue, address newValue);
    event SetAddresses(address _cpSNEK, address lpDepositor);
    event AddedGauge(address bribe, address[] bribeTokens, address[] rewardTokens);
    event AddedRewardToken(address token);
    event NewAddressesCommitted(address cpSNEK, address lpDepositor, uint256 newAddressDeadline);

    function initialize(IERC20Upgradeable _SNEK, ISnekVoter _solidVoter, ISolidlyRouter _router) public initializer {
        __Ownable_init();
        SNEK = _SNEK;
        solidVoter = _solidVoter;
        ve = ISnekVe(solidVoter._ve());
        router = _router;

        ISnekMinter _minter = ISnekMinter(_solidVoter.minter());
        veDist = IVeDist(_minter._rewards_distributor());

        SNEK.safeApprove(address(ve), type(uint256).max);
    }

    modifier onlyCpSNEK() {
        require(msg.sender == cpSNEK, "Proxy: FORBIDDEN");
        _;
    }
    ...
    function setCpSNEK(address _cpSNEK) external onlyOwner {
        require(address(cpSNEK) == address(0), "Proxy: ALREADY_SET");
        emit SetCpSNEK(cpSNEK, _cpSNEK);
        cpSNEK = _cpSNEK;
    }
    ...
    function createMainLock(uint256 _amount, uint256 _lock_duration) external onlyCpSNEK returns (uint256) {
        require(mainTokenId == 0, "CpSNEKSolidStaker: ASSIGNED");
    ...
    function increaseAmount(uint256 _amount) external onlyCpSNEK {
        ve.increase_amount(mainTokenId, _amount);
    }

    function increaseUnlockTime() external onlyCpSNEK {
        ve.increase_unlock_time(mainTokenId, MAX_LOCK);
    }

    function resetVote(uint256 _tokenId) external onlyCpSNEK {
        solidVoter.reset(_tokenId);
    }

    function release() external onlyCpSNEK {
        uint256 before = SNEK.balanceOf(address(this));
        ve.withdraw(mainTokenId);
        uint256 amount = SNEK.balanceOf(address(this)) - before;
        if (amount > 0) SNEK.safeTransfer(cpSNEK, amount);
        mainTokenId = 0;
    }
    ...
    function pause() external onlyCpSNEK {
        SNEK.safeApprove(address(ve), 0);
    }

    function unpause() external onlyCpSNEK {
        SNEK.safeApprove(address(ve), type(uint256).max);
    }
    ...
    function deposit(address _token, uint256 _amount) external onlyLPDepositor {
        ...
        ISnekGauge(gauge).deposit(_amount, mainTokenId);
    }
    ...
    function withdraw(
        ...
        address gauge = solidVoter.gauges(_token);
        ISnekGauge(gauge).withdraw(_amount);
        IERC20Upgradeable(_token).transfer(_receiver, _amount);
    }

    function claimVeEmissions() external onlyCpSNEK returns (uint256) {
    ...
    function totalDeposited(address _token) external view returns (uint) {
        address gauge = solidVoter.gauges(_token);
        return ISnekGauge(gauge).balanceOf(address(this));
    }
    ...
    function totalLiquidityOfGauge(
        ...
        address gauge = solidVoter.gauges(_token);
        return ISnekGauge(gauge).totalSupply();
    }
    ...
    function addGauge(
        ...
        gauges[_lp] = Gauges(
            solidVoter.feeDistributers(gauge),
            _bribeTokens,
            _rewardTokens
        );
        lpInitialized[_lp] = true;
        emit AddedGauge(solidVoter.feeDistributers(gauge), _bribeTokens, _rewardTokens);
    }
    ...
    function addRewardToken(
        ...
        require(_rewardToken != address(SNEK), "Proxy: ROUTE_FROM_IS_SNEK");
        require(
            _route[_route.length - 1].to == address(SNEK),
    ...
    function getBribeReward(uint256 _tokenId, address _lp) external onlyCpSNEK {
        Gauges memory _gauges = gauges[_lp];
        address[] memory _bribes = new address[](1);
        _bribes[0] = _gauges.feeDistributor;
        address[][] memory _tokens = new address[][](1);
        _tokens[0] = _gauges.bribeTokens;
        solidVoter.claimBribes(_bribes, _tokens, _tokenId);
        ...
        for (uint256 i; i < _gauges.bribeTokens.length; ++i) {
            ...
            if (tokenBal > 0) {
                if (bribeToken == address(SNEK) || bribeToken == address(cpSNEK)) {
                    IERC20Upgradeable(bribeToken).safeTransfer(cpSNEK, tokenBal);
                } else {
    ...
    function getFeeReward(uint256 _tokenId, address _lp) external onlyCpSNEK {
       Gauges memory _gauges = gauges[_lp];
        address[] memory _bribes = new address[](1);
        _bribes[0] = _gauges.feeDistributor;
        address[][] memory _tokens = new address[][](1);
        _tokens[0] = _gauges.bribeTokens;
        solidVoter.claimFees(_bribes, _tokens, _tokenId);

        for (uint256 i; i < _gauges.bribeTokens.length; ++i) {
            address feeToken = _gauges.bribeTokens[i];
            uint256 tokenBal = IERC20Upgradeable(feeToken).balanceOf(address(this));
            if (tokenBal > 0) {
                if (feeToken == address(SNEK) || feeToken == address(cpSNEK)) {
                    IERC20Upgradeable(feeToken).safeTransfer(cpSNEK, tokenBal);
                } else {
                    router.swapExactTokensForTokens(
                        tokenBal,
                        0,
    ...
    function claimableReward(address _lp) external view returns (uint256) {
        ...
        for (uint256 i; i < _gauges.rewardTokens.length; ++i) {
            ...
            uint256 reward = ISnekGauge(gauge).earned(rewardToken, address(this));
            if (reward > 0) {
                if (rewardToken != address(SNEK)) {
                    uint256 rewardSNEK = router.getAmountsOut(reward, routes[rewardToken])[routes[rewardToken].length];
                    totalReward = totalReward + rewardSNEK;
                } else {
    ...
    function getReward(address _lp) external onlyLPDepositor {
        Gauges memory _gauges = gauges[_lp];
        address gauge = solidVoter.gauges(_lp);
        ISnekGauge(gauge).getReward(address(this), _gauges.rewardTokens);
        ...
        for (uint256 i; i < _gauges.rewardTokens.length; ++i) {
            ...
            if (tokenBal > 0) {
                ...
                } else SNEK.safeTransfer(lpDepositor, tokenBal);
            }
        }
    }
    ...
    */
    function setPendingAddresses(
        ...
        pendingCpSNEK = _cpSNEK;
        newAddressDeadline = block.timestamp + 86400 * 7;

        emit NewAddressesCommitted(_cpSNEK, _lpDepositor, newAddressDeadline);
    }
