...
// Unnamed Branding by The Spot Devs
...
contract UnnamedBranding is ERC721URIStorage, IERC721Receiver, ReentrancyGuard, Ownable {
	...
	address private unnamedAddress = 0x6BDAd2A83a8e70F459786a96a0a9159574685c0e;
	address private spotAddress = 0x0C6945E825fc3c80F0a1eA1d3E24d6854F7460d8; 

    IERC721 uNFT = IERC721(unnamedAddress);
	...
	constructor() ERC721("Unnamed Branded", "BRAND") {
	...
	function mint(uint256 unnamedId, string memory uri) public payable nonReentrant {
		require(!paused, "Minting is paused");
		if (spot.balanceOf(msg.sender) == 0) {
			require(msg.value >= cost, "Insufficient funds");
		}

		_mintLoop(unnamedId, uri, _msgSender());
	}

    function _mintLoop(uint256 unnamedId, string memory uri, address to) internal {
        uNFT.safeTransferFrom(to, address(this), unnamedId);
    ...
    function onERC721Received(address, address, uint256, bytes calldata) external view override returns (bytes4) {
        require(msg.sender == unnamedAddress, "Only Unnamed!");        
