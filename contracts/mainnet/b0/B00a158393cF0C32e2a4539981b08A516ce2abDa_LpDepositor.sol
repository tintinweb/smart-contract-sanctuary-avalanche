...
contract LpDepositor is OwnableUpgradeable {
    ...
    function deposit(address _user, address _token, uint256 _amount) external {
        ...
        IERC20Upgradeable(_token).safeTransferFrom(msg.sender, address(proxy), _amount);
    ...
    function claim(address _receiver, address[] memory _tokens) public {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            ...
            uint256 reward = FLDX.balanceOf(address(this)) - before;
        ...
        if (unclaimedReward > 0) {
            FLDX.safeTransfer(_receiver, unclaimedReward);
        }
