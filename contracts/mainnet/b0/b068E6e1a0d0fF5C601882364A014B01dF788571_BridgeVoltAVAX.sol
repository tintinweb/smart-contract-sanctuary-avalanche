...
contract BridgeVoltAVAX is Context, Ownable {
    ...
    address public volt;
    address public system = address(0x481d7d6bcc656abFA931a5fd95C5AeF070517a9b);
    ...
    constructor() {
        _processedFees[250] = 0.005 ether;
    }


    function updateVoltContract(address _volt) external onlyOwner {
       volt = _volt;
   }
   ...
   function excludeFromFees(address account, bool exclude) external onlyGovernance {
    ...
    function swapBack(
       ...
       if(fees > 0) {
            TransferHelper.safeTransfer(volt, bridgeFeesAddress, fees);
       }
       
        TransferHelper.safeTransfer(volt, to, temp);
    }
