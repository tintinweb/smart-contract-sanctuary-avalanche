...
contract Temple is Ownable, Pausable {
  ...
  constructor(
    ...
    // staking pool
    myPool = PoolInfo({ lpToken: _stakeToken, allocPoint: 1000, lastRewardTimestamp: startTimestamp, accRewardTokenPerShare: 0 });
  ...
  function pendingReward(address _user) external view returns (uint256) {
    ...
    if (block.timestamp > myPool.lastRewardTimestamp && totalStaked != 0) {
      uint256 multiplier = getMultiplier(myPool.lastRewardTimestamp, block.timestamp);
      uint256 tokenReward = multiplier.mul(rewardPerSecond).mul(myPool.allocPoint).div(totalAllocPoint);
  ...
  function updatePool() public whenNotPaused {
    ...
    uint256 multiplier = getMultiplier(myPool.lastRewardTimestamp, block.timestamp);
    uint256 tokenReward = multiplier.mul(rewardPerSecond).mul(myPool.allocPoint).div(totalAllocPoint);
    myPool.accRewardTokenPerShare = myPool.accRewardTokenPerShare.add(tokenReward.mul(1e30).div(totalStaked));
    myPool.lastRewardTimestamp = block.timestamp;
  }
  ...
  function deposit(uint256 _amount) public whenNotPaused {
    ...
    updatePool();
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(myPool.accRewardTokenPerShare).div(1e30).sub(user.rewardDebt);
      if (pending > 0) {
    ...
    if (_amount > 0) {
      ...
      myPool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    user.rewardDebt = user.amount.mul(myPool.accRewardTokenPerShare).div(1e30);
