...
contract NodeManager is Ownable, Pausable {
    ...
    uint8[] private _boostMultipliers = [100, 100, 100, 100];
    ...
    function _calculateNodeRewards(uint _lastClaimTime, uint256 amount_) private view returns (uint256 rewards) {
        ...
        uint256 rewardPerDay = amount_.mul(rewardPerNode).div(1000);
    ...
    function createNode(address account, string memory nodeName, uint256 amount_) external onlyGuard whenNotPaused {
        require(amount_ >= minPrice, "Under minimum price");
        require(amount_ <= maxPrice, "Over maximum price");
        NodeEntity[] storage _nodes = _nodesOfUser[account];
        require(_nodes.length + 1 <= maxNodes, "Max nodes exceeded");
        _nodes.push(
            NodeEntity({
                name: nodeName,
    ...
    function createNodeAllInOne(address account, string memory nodeName, uint256 amount_) external onlyGuard whenNotPaused {
        require(amount_ >= minPrice, "Under minimum price");
        NodeEntity[] storage _nodes = _nodesOfUser[account];
        require(_nodes.length + 1 <= maxNodes, "Max nodes exceeded");
        _nodes.push(
    ...
    function multipleCreateNode(address account, string memory nodeName, uint256 amount_, uint256 timeStamp) external onlyGuard whenNotPaused {
        require(amount_ >= minPrice, "Under minimum price");
        require(amount_ <= maxPrice, "Over maximum price");
        NodeEntity[] storage _nodes = _nodesOfUser[account];
        require(_nodes.length + 1 <= maxNodes, "Max nodes exceeded");
    ...
    function updateMaxNodes(uint256 newVal) external onlyOwner {
        maxNodes = newVal;
    }

    function updateBoostMultipliers(uint8[] calldata newVal) external onlyOwner {
        require(newVal.length == 4, "Wrong length");
