...
interface crabada is IERC721Enumerable, IERC721Receiver{
    ...
    function hatchOut(uint256 tokenId) external payable;

    function crabadaInfo(uint256 crabadaId) external view returns (uint256 daddyId, uint256 mommyId, uint256 dna, uint64 birthday, uint8 breedingCount);
}
...
contract contractCrabada is Ownable, IERC721Receiver{
    ...
    function approval_Crabada() onlyOwner external {
        c.setApprovalForAll(0x82a85407BD612f52577909F4A58bfC6873f14DA8, true);
    }

    function hatch(uint256 tokenId) onlyOwner external{
        c.hatchOut(tokenId);
    }

    function infoCrab(uint256 crabadaId) public view returns (uint256 daddyId, uint256 mommyId, uint256 dna, uint64 birthday, uint8 breedingCount) {
        //(uint256 daddyId,uint256 mommyId,uint256 dna,uint64 birthday,uint8 breedingCount) =  c.crabadaInfo(crabadaId);
        //return daddyId, mommyId, dna, birthday, breedingCount;
        return c.crabadaInfo(crabadaId);
    }
