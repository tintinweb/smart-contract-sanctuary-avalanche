...
contract DropsPriceOracleAvalanche is Admin, Initializable, IDropsPriceOracle {
  ...
  function _getTraderJoeUSDC(uint256 usdAmount) internal view returns (uint256 weiAmount) {
    ...
    (uint128 amountIn, uint128 amountOutLeft, uint128 fee) = TraderJoeRouter.getSwapIn(TraderJoeUsdcPool, uint128(usdAmount), true);
  ...
  function _getTraderJoeUSDT(uint256 usdAmount) internal view returns (uint256 weiAmount) {
    // add decimal places for amount IF decimals are above 6!
    // usdAmount = usdAmount * (10**(18 - 6));
    (uint128 amountIn, uint128 amountOutLeft, uint128 fee) = TraderJoeRouter.getSwapIn(TraderJoeUsdtPool, uint128(usdAmount), true);
    weiAmount = amountIn + fee;
  }
}
...
interface ILBRouter {
  function getSwapIn(ILBPair LBPair, uint128 amountOut, bool swapForY) external view returns (uint128 amountIn, uint128 amountOutLeft, uint128 fee);
}
