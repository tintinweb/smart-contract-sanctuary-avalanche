...
contract xJoeConvertor is Ownable {
    ...
    event xJoeObtained(address user, uint256 depositedJoe, uint256 obtainedxJoe);
        ...
        returns (uint256 obtainedxJoeAmount)
    {
        ...
        if (xJoeRatio <= buyThreshold) {
            ...
            if (amountToBuy > 0) {
                ...
                if (amountToBuy < estimatedOutAmount) {
                    ...
                    path[0] = joe;
                    path[1] = xJoe;
                    obtainedxJoeAmount = IJoeRouter01(router).swapExactTokensForTokens(
                        amountToBuy,
                    ...
                    require(obtainedxJoeAmount > amountToBuy, "Transaction is disadvantageous");
        ...
        obtainedxJoeAmount += amountToConvert;
        IxJoe(xJoe).depositFor(amountToConvert, _for);
        emit xJoeObtained(_for, _amount, obtainedxJoeAmount);
    }
    ...
    function deposit(uint256 _amount) external returns (uint256 obtainedxJoeAmount) {
        obtainedxJoeAmount = _depositFor(_amount, msg.sender);
    }
    ...
    function depositFor(uint256 _amount, address _for)
        ...
        returns (uint256 obtainedxJoeAmount)
    {
        obtainedxJoeAmount = _depositFor(_amount, _for);
    }
}
