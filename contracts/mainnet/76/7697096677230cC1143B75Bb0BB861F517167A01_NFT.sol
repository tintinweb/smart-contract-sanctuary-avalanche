...
contract NFT is Ownable, Manageable, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  function getNextAmountStaked(uint256 tokenId) public view tokenExists(tokenId) returns (uint256) {
    return getTokenTierIndex(tokenId) - 1 == 0 ? 0 : tiers[getTokenTierIndex(tokenId) - 1];
  }
  ...
  function _mint(address to, uint256 tokenId) internal {
    ...
    require(balanceOf(to) == 0, "ERC721: limited to one token");
  ...
  function mint(address to, uint256 value) public onlyManager {
  ...
  function mint(address to, uint256 value, uint256 mintTimestamp) public onlyOwner {
    uint256 id = totalSupply() + 1;
    _safeMint(to, id);
    nftINfo[id] = NFTInfo({
      value: value,
  ...
  function updateValue(uint256 id, uint256 value) public onlyManager {
    nftINfo[id].value += value;
  }
