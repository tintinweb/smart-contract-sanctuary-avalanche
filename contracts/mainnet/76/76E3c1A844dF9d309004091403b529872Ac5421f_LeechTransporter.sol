 ...
 *
 * @title Leech Protocol transporter contract.
 * @notice The transporter is a layer between leech protocol and external cross-chain services.
...
contract LeechTransporter is OwnableUpgradeable, ILeechTransporter {
    ...
    /// @dev Define current cross-chain service in use.
    enum Bridge {
        MULTICHAIN_V6,
        STARGATE
    }
    ...
    /// @notice Current cross-chain service in use.
    Bridge public activeBridge;

    /// @notice We implements protocol base token in each chain for the crosschain interactions.
    mapping(uint256 => address) public chainIdToBaseToken;

    ///============= Stargate variable section =================
    /// @notice Stargate router address.
    IStargate public stargate;

    /// @notice Returns stargate chain id from EVM chain id.
    mapping(uint256 => uint16) public getStargateChainId;

    /// @notice Return id of the stargate token pool.
    mapping(address => uint256) public getStargatePoolIdFromToken;

    /// ============ Multichain variable section ================
    ///@notice We can support 3 version of multichain router instances
    ...
    function bridgeOut(
        ...
        // Check active crosschain service and call internal funcion
     ...
     * @param _stargate Address of the stargate router
     */
    ...
    /**
     * @notice Set stargate internal chain id
     * @param _chainId EVM chain ID
     * @param _stargateId Stargate chain ID
     */
    ...
    /**
     * @notice Set stargate internal pool id
    ...
    /**
     * @notice Set leech protocol base token for each chain
     * @param _chainId EVM chain ID
     * @param _token Network base token address (address from this network)
     */
    ...
    /**
     * @notice Set leech protocol base token for each chain
     * @param _bridgedToken Token address which should be sent
     * @param _bridgedAmount Token amount
     * @param _destinationChainId EVM chain ID
     * @param _destAddress Address of the receiver on the other side
     */
    function _bridgeOutMultichain(
        ...
        // Get current router ID for the bridged token
        uint256 routerId = destinationTokenToRouterId[_bridgedToken];
        // Define the multichain version and send cross-chain TX.
        if (routerId == 1) {
            ...
            // Convert address to bytes for the V7 router.
    ...
    function _bridgeOutStargate(
        ...
        // Prepare lzTxObj struct for stargate router.
        ...
        // Check if all needed pools are existed.
        ...
        // Perform crosschain TX using swap function on the stargate router.
    ...
    /**
     * @notice Emergency withdraw for stuck tokens or ether.
