...
/* All minting MUST be called from the BRAINRewards contract */

contract BRAINNFTUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ...
    NFTInfoUpgradeable
{
    ...
    mapping(uint256 => string) public brainsToNames;
    mapping(string => bool) public names;

    error NameIsNotUnique(string name);
    ...
    function initialize(string memory _name, string memory _symbol)
        ...
        initializer
    {
        __ERC721BaseUpgradeable_init(_name, _symbol);
        OwnableUpgradeable.__Ownable_init();
    }

    modifier isBRAINOwner(address account) {
        if (balanceOf(account) == 0) revert NotBRAINOwner();
        _;
    }
    ...
    function batchMint(
        ...
        uint256 amount
    ) external onlyProxy returns (uint256[] memory) {
        uint256[] memory nftIDs = new uint256[](amount);
        for (uint256 x; x < amount; x++) {
            string memory thisName = _names[x];
            _validateName(thisName);
            _mint(receiver);
            uint256 id = _owners.length - 1;
            nftIDs[x] = id;
            brainsToNames[id] = thisName;
            names[thisName] = true;
        }
        return nftIDs;
    }
    ...
    {
        string[] memory brainNames = new string[](tokenIDs.length);
        for (uint256 x = 0; x < tokenIDs.length; x++) {
            brainNames[x] = brainsToNames[tokenIDs[x]];
        }
        return brainNames;
    }

    function isValidName(string memory value) external view returns (bool) {
        return (!names[value] &&
            bytes(value).length > 3 &&
            bytes(value).length < 33);
    }

    function renameBRAIN(
        address account,
        uint256 tokenID,
        string calldata value
    ) external onlyProxy {
        if (!_exists(tokenID)) revert NonexistentToken(tokenID);
        if (_owners[tokenID] != account) revert NotOwner(tokenID);
        _validateName(value);
        string memory oldName = brainsToNames[tokenID];
        if (bytes(oldName).length > 0) {
            names[oldName] = false;
        }
        names[value] = true;
        brainsToNames[tokenID] = value;
        emit RenameBRAIN(account, tokenID, value);
    }
    ...
    function setNFTNumericInfo(
        ...
        _setNFTNumericInfo(tokenID, propertyNum, value);
    }
    ...
    function setNFTStringInfo(
        ...
        _setNFTStringInfo(tokenID, propertyNum, value);
    }
        ...
        returns (bool)
    {
        ...
        return (spender == owner ||
            ...
            proxyToApproved[spender]);
    }

    function _validateName(string memory name) private view {
        if (names[name]) revert NameIsNotUnique(name);
        if (bytes(name).length < 4) revert NameTooShort(name);
        if (bytes(name).length > 32) revert NameTooLong(name);
    }
}
