...
/// @title XJoe
/// @author Vector Team
/// @notice xJoe is a token minted when 1 joe is staked in the Vector protocol
contract xJoe is ERC20 {
    ...
    constructor(address _mainContract, address _joe) ERC20("zJoe", "zJoe") {
        mainContract = _mainContract;
        joe = _joe;
    }

    /// @notice deposit Joe in vector protocol and get vJoe at a 1:1 rate
    /// @param _amount the amount of Joe
    function depositFor(uint256 _amount, address _for) external {
        IERC20(joe).safeTransferFrom(msg.sender, mainContract, _amount);
        IMainStakingJoe(mainContract).stakeJoe(_amount);
        _mint(_for, _amount);
        emit XJoeMinted(_for, _amount);
    }

    function depositWithoutTransferFor(uint256 _amount, address _for) external {
        require(msg.sender == mainContract, "Only MainStaking");
        IMainStakingJoe(mainContract).stakeJoe(_amount);
        _mint(_for, _amount);
        emit XJoeMinted(_for, _amount);
    }
