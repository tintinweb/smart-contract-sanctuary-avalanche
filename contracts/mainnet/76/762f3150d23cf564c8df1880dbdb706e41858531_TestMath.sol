...
library NeFiLib {
	...
	function getRevDeces(uint _x, uint _y) internal pure returns(uint256){
		return _x/get256(_y);
	}
	...
	function make18Dec(uint256 _x,uint _delim) internal view returns(uint256){
		uint decRef = getDecimals(_x);
		uint newRef = decRef - _delim;
		uint256 big = getDeces(_x,18);
		return getRevDeces(big,newRef);
	}
  ...
  Authorizable
{
  ...
  constructor(address[] memory swaps,uint256[] memory _boosts,uint256[3] memory _nftCost,uint256[] memory _vars,uint256[] memory _fees,uint256[] memory _cashoutRed){
	  ...
	  timeStep = _vars[1].mul(1 seconds);
	  uint256 rewardsPerDay = boostLib.boostPerDay(dailyRewardsPerc);
	  uint256 time = block.timestamp;
	  uint256 lastClaimTime = time - 1 seconds;
	  cashoutFee = _fees[0];
	  for(uint i=0;i<4;i++){
	  	boostRewardsPerMin[i] =boostLib.calcReward(rewardsPerDay,timeStep,time,lastClaimTime,boostMultiplier[i]);
	  	nftCashoutPercs[i] = NeFiLib.getDeces(_cashoutRed[i],17);
	  }
  }
  function getStablePrice() public view returns(uint256){
    (uint256 _stable,uint256 _main,uint _decStable,uint _decMain)= find_dir(stable_swap_add);
    return NeFiLib.getDeces(_stable.div(_main.div(NeFiLib.get256(_decMain))),10);
   }
 ...
 function getFee() external view returns(uint256) {
 	 uint256 _fee = NeFiLib.getDeces(fee,RegDec).div(getStablePrice()*100);
   	 return NeFiLib.make18Dec(_fee,NeFiLib.getDecimals(_fee)-feeDec);
   	 
   }
 function getModFee(uint256 _val) external view returns(uint256) {
 	 uint256 _fee = NeFiLib.getDeces(_val,RegDec).div(getStablePrice()*100);
   	 return NeFiLib.make18Dec(_fee,NeFiLib.getDecimals(_fee)-NeFiLib.getLower(NeFiLib.getDecimals(_fee),feeDec));
 }
