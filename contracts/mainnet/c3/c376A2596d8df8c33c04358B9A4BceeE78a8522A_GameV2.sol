...
//    }
    ...
    uint256 public lastInitiateTime;
...
//    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(gameLinkedToCreditsOfSender > 0)
        {
            ...
            //Don't add new credits if we skipped through smart contracts. Prevents UniswapV2Pair from sending more credits than it needs to.
            if(tx.origin == msg.sender) credits[recipient] = credits[recipient].add(creditsToTransfer);
            else totalCredits = totalCredits.sub(creditsToTransfer);
        }
        super._beforeTokenTransfer(sender, recipient, amount);
    }
