...
contract NebulaProtoStarDrop is Ownable{
	...
	function payFee(uint256 _intervals) payable external {
		...
		require(feeTok.balanceOf(_account) >= tokenFee.mul(_intervals),"you have not sent enough to pay the amount of fees you are seeking to pay");
	...
	function createProto(string memory _name) payable external {
		...
		require(feeMGR.simpleQuery(_account) > 0,"doesnt look like you owe any fees, you're either maxed out, or i glitched :0");
		require(feeMGR.simpleQuery(_account) >= 1,"looks like youre attempting to overpay, less intervals next time please");
		require(feeTok.balanceOf(_account) >= tokenFee,"you have not sent enough to pay the amount of fees you are seeking to pay");
		feeTok.transferFrom(_account, treasury, tokenFee);
		feeMGR.MGRrecPayFees(_account);
	...
	function createProto(string memory _name,address _account) external managerOnly{
		address _account = msg.sender;
		if(nebuLib.addressInList(protoOwners,_account) == false){
			protoOwners.push(_account);
		}
	...
	function transferOldDrops(uint _x, uint _y) internal {
