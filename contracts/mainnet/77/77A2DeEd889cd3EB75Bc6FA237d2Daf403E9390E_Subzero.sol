...
// File contracts/Subzero.sol
// License-Identifier: MIT

pragma solidity 0.6.12;

contract Subzero is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Supply used to launch in LP
    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 2200000 ether;
    ...
    /**
     * @notice Constructs the SUBZERO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SUBZERO", "SUBZERO") {
        // Mints 1 SUBZERO to contract creator for initial pool setup
    ...
    function _getSubzeroPrice() internal view returns (uint256 _subzeroPrice) {
        try IOracle(subzeroOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Subzero: failed to fetch SUBZERO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _subzeroPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_subzeroPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSubzeroOracle(address _subzeroOracle) public onlyOperatorOrTaxOffice {
        require(_subzeroOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SUBZERO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SUBZERO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSubzeroPrice = _getSubzeroPrice();
            currentTaxRate = _updateTaxRate(currentSubzeroPrice);
            if (currentSubzeroPrice < burnThreshold) {
                burnTax = true;
            }
        }
