// Telegram: https://t.me/OmniInu
// Website: https://omniinu.com
...
contract OmniChainToken is ERC20, NonblockingReceiver, ILayerZeroUserApplicationConfig {
	...
	address public marketingWallet = payable(0x5145267A2764494869a0c5A2d6D531cF734843dc);
	
	UniswapV2Router private uniswapV2Router = UniswapV2Router(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
	//New Features
    ...
    constructor(string memory name_, string memory symbol_, address _layerZeroEndpoint) ERC20(name_, symbol_){
		...
		swapTokensAtAmount = fixedSupply * 5 / 10000; //0.05% of supply
	...
	function setSwapEnabled(bool newVal) external onlyOwner {
		swapEnabled = newVal;
	}
	function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	    require(newAmount <= fixedSupply * 5 / 100, "Swap amount cannot be higher than 5% total supply.");
    ...
    function _transfer(
		...
		if (
			...
			if (tradingActive && block.number <= tradingBlock + 3 && automatedMarketMakerPairs[from] && to != address(uniswapV2Router) && to != address(this)) {  
    ...
    function swapBack() private {
		...
		uint256 swapAmount = swapTokensAtAmount;
	...
	function manualSendToken(address token) external onlyOwner {
		...
		tokenInstance.transfer(marketingWallet, balance);
	}
