...
interface IMiniChefV2  {
    function pendingStargate(uint256 _pid, address _user) external view returns (uint256 pending);
...
interface IStargateRouter {
    function instantRedeemLocal(uint16 _srcPoolId, uint256 _amountLP, address _to) external;//withdraw
    function addLiquidity(uint256 _poolId, uint256 _amountLD, address _to) external;
}

contract STGUSDCStrategy is ERC20, AccessControl, IStrategy {
  ...
  IERC20 public rewardToken1;//stg
  IERC20 public poolToken;//usdc lp 
  IMiniChefV2 public stakingContract;
  IStargateRouter public poolContract;
  uint16 public poolId;
  uint256 public pid;

  uint public MIN_TOKENS_TO_REINVEST = 10000;
  uint public REINVEST_REWARD_BIPS = 100;//1%
  uint public ADMIN_FEE_BIPS = 400;//4%
  uint public WITHDRAW_FEE_BIPS = 0;//0%
  uint constant private BIPS_DIVISOR = 10000;

  bool public REQUIRE_REINVEST_AFTER_DEPOSIT = true;
  uint public MIN_TOKENS_TO_REINVEST_AFTER_DEPOSIT = 200;
  ...
  event UpdateRequireReinvestAfterDeposit(bool newValue);
  event UpdateMinTokensToReinvestAfterDeposit(uint oldValue, uint newValue);
  ...
  constructor(
    ...
    depositToken = IERC20(0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7);//avax usdt token
    rewardToken1 = IERC20(0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590);//stargate token
    poolContract = IStargateRouter(0x45A01E4e04F14f7A4a6702c74187c5F6222033cd);//stargate pool
    poolId = 2;//stargate usdt fantom pool id
    poolToken = IERC20( 0x29e38769f23701A2e4A8Ef0492e19dA4604Be62c);//USD Tether-LP
    stakingContract = IMiniChefV2(0x8731d54E9D02c286767d56ac03e8037C07e01e98);//stg avax staking
    router = IRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);//traderjoe 
    pid = 1;//usdt-avax-stg farm
  ...
  function setAllowances() public onlyAdmin {
    depositToken.approve(address(poolContract), depositToken.totalSupply());
    poolToken.approve(address(stakingContract), poolToken.totalSupply());
  ...
  function _deposit(uint amount) internal {
    ...
    amount = _poolDepositTokens(amount);
    _stakeDepositTokens(amount);
    _mint(msg.sender, getSharesForDepositTokens(amount));
    totalDeposits = totalDeposits + amount;  
    if (REQUIRE_REINVEST_AFTER_DEPOSIT) {
      uint unusedReward = rewardToken1.balanceOf(address(this));
      if (unusedReward >= MIN_TOKENS_TO_REINVEST_AFTER_DEPOSIT) {
        _reinvest(unusedReward);
      }
    }
    emit Deposit(msg.sender, amount);
  }

  function _poolDepositTokens(uint amount) internal returns(uint256){
      poolContract.addLiquidity(poolId, amount, address(this));
  ...
  function _withdrawPoolTokens() internal {
      uint256 amount = poolToken.balanceOf(address(this));
      poolContract.instantRedeemLocal(poolId, amount, address(this));
  }
  ...
  function checkReward() public view returns (uint) {
    uint pendingReward = stakingContract.pendingStargate(pid, address(this));
  ...
  function _reinvest(uint amount) internal {
    ...
    uint tokenAmount = _convertRewardTokensToDepositTokens((amount - adminFee) - reinvestFee);
    uint poolTokens = _poolDepositTokens(tokenAmount);
    _stakeDepositTokens(poolTokens);
  ...
  function _convertRewardTokensToDepositTokens(uint amount) private returns (uint) {
    require(amount > 0, "::_convertRewardTokensToDepositTokens");

    // swap stg->wftm->usdc
    address[] memory path = new address[](3);
    path[0] = address(rewardToken1);
    path[1] = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);//wavax
    path[2] = address(depositToken);

    uint amountOutToken0 = amount;
    uint[] memory amountsOutToken0 = router.getAmountsOut(amount, path);
    amountOutToken0 = amountsOutToken0[amountsOutToken0.length - 1];
    router.swapExactTokensForTokens(amount, amountOutToken0, path, address(this), block.timestamp);
  ...
  /**
   * @notice Toggle requirement to reinvest after deposit
   */
  function updateRequireReinvestAfterDeposit() external onlyAdmin {
    REQUIRE_REINVEST_AFTER_DEPOSIT = !REQUIRE_REINVEST_AFTER_DEPOSIT;
    emit UpdateRequireReinvestAfterDeposit(REQUIRE_REINVEST_AFTER_DEPOSIT);
  }

  /**
   * @notice Update reinvest minimum threshold after a deposit
   * @param newValue min threshold in wei
   */
  function updateMinTokensToReinvestAfterDeposit(uint newValue) external onlyAdmin {
    emit UpdateMinTokensToReinvestAfterDeposit(MIN_TOKENS_TO_REINVEST_AFTER_DEPOSIT, newValue);
    MIN_TOKENS_TO_REINVEST_AFTER_DEPOSIT = newValue;
  }
