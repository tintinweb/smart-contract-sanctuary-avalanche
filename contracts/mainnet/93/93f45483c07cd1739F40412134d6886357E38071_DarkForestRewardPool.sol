...
contract DarkForestRewardPool is Ownable {
    ...
    uint256[][] public multipleArray = [[74400, 74434, 74469, 75094, 75126, 75158],                 // 1.0002316, 1.0002318
                                [74536, 74604, 74671, 75858, 75916, 75974],
                                [74803, 74934, 75062, 77217, 77318, 77417],
                                [75062, 75252, 75438, 78402, 78534, 78665]];

    uint256 public lastRewardTime;
    uint256 public rewardPeriod;
    uint256 public baseMult = 74262;
    ...
    function DistributeRewards(
        uint256[] memory planetIds, 
        uint256[] memory planetRwMults, 
        uint256[] memory planetValues,
        ...
        onlyOwner 
    {
        if (block.timestamp > lastRewardTime + rewardPeriod)  
        {
            ...
            for (uint256 i = 0; i < size; i++) 
            {
                ...
                if (rewardInfo[accounts[i]] == 0)                         
                    rewardAccounts[len++] = accounts[i];

                if (rewardInfo[accounts[i]] < multipleArray[lockingLevel][threashold])
                    rewardInfo[accounts[i]] = multipleArray[lockingLevel][threashold];
