...
contract AssetManagementHomoraAdapter {
    ...
    function getAssetAmount(address asset) public view returns (uint256 assetAmount, uint256 assetCapital, uint256 assetInterest) {
        ...
        assetAmount = _mulScalarTruncate(ibBalance, exchangeRate);
        assetCapital = assetInfo[asset].capital;
        //console.log(assetAmount, assetCapital);
    ...
    function deposit(uint256 amount) external {
        ...
        IDetailedERC20(usdc).safeTransferFrom(msg.sender, address(this), amount);
        //console.log("Vault USDC balance before depositing:", IDetailedERC20(usdc).balanceOf(address(this)));
    ...
    function allocate(AllocInputs memory input) external {
        ...
        if(fromAssetDifference.sub(toAssetDifference) > 0) {
            toInfo.txCosts = toInfo.txCosts.add(fromAssetDifference.sub(toAssetDifference));
        }
    ...
    function claimInterest(address asset, address[] memory path) external {
        ...
        // Swap interest amount for USDC.
        _approveRouterAndExecuteSwap(asset, path, interestAmount);
    ...
    function _approveRouterAndExecuteSwap(address fromAsset, address[] memory path, uint256 fromAmount) internal returns (uint256) {
        ...
        // From allocate()
        /*
        bool check = IDetailedERC20(fromAsset).approve(JoeRouter, amountSwap); 
        require(check, "Error: Check reverted");

        uint256[] memory amounts = IJoeRouter(JoeRouter).getAmountsOut(amountSwap, path);
        uint256 minAmountAccepted = amounts[amounts.length - 1].mul(95).div(100);

        uint256[] memory _amounts = IJoeRouter(JoeRouter).swapExactTokensForTokens(amountSwap, minAmountAccepted, path, address(this), block.timestamp);
        require(_amounts[_amounts.length - 1] >= minAmountAccepted, "Error: Slippage exceeded");
        */

        // From claimInterest()
        /*
        bool check = IDetailedERC20(asset).approve(JoeRouter, interestAmount);
        require(check, "Error: Check reverted");

        uint256[] memory amounts = IJoeRouter(JoeRouter).getAmountsOut(interestAmount, path);
        uint256 minAmountAccepted = amounts[amounts.length - 1].mul(95).div(100);

        uint256[] memory swapAmounts = IJoeRouter(JoeRouter).swapExactTokensForTokens(interestAmount, minAmountAccepted, path, address(this), block.timestamp);
        require(swapAmounts[swapAmounts.length - 1] >= minAmountAccepted, "Error: Slippage exceeded");
        */
    }
