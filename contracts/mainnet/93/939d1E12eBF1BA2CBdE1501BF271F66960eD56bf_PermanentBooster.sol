...
contract PermanentBooster is ERC721Enumerable, Ownable {
    ...
    function mint(address recipient) external onlyMinter returns (uint) {
        tokenCount++;
        _mint(recipient, tokenCount);
        return tokenCount;
    }

    function multiMint(uint256 num) external onlyMinter {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        require(from == address(0) || to == address(0) || whitelisted[from] || whitelisted[to], "Not allowed");
