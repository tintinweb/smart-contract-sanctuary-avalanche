...
contract THeHexagon is ERC721Enumerable, Ownable, Pausable {
    // NFT URL
    string baseTokenURI;
    string baseExtension;

    // Wallet and Token address
    address public admin =  0x8cB60A2Cc0DaeD224c23A443e5c50A70bd3bB6A4; //50 %
    address public dev = 0x2F20D2cafaa1692e401791Be811700fb56f0930B; //25%
    address public market = 0x19983b1b05Eb4aa0d5d0d35b5E3239C9E7D1e402; // 15%
    address public manager =0x7ecC2E9f9Fc8C1dCb2Ed69F8b18DF74b02cfFDfE; //10%

    address public constant honeyContract = 0x7F492Ba74ba0Cf9AD6E11fAA29a5d4797Ec3891F;
    ...
    constructor() ERC721("TheHexagon", "THXG") {}
    ...
    function withdrawFunds() external onlyOwner {
        ...
        if (withdrawCNT == 1) require(minted > 999, "Withdraw is not available. 1k NFT should be minted at least");
        if (withdrawCNT == 2) require(minted > 2999, "Withdraw is not available. 3k NFT should be minted at least");
        if (withdrawCNT == 3) require(minted == 9999, "Withdraw is not available. All NFT should be minted");

        uint256 balance;
        balance = address(this).balance;

        payable(admin).transfer(balance * 50 / 100);
        payable(dev).transfer(balance * 25 / 100);
        payable(market).transfer(balance * 15 / 100);
        payable(manager).transfer(balance * 10 / 100);
    } 
}