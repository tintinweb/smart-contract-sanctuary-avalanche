...
contract NFTSales is Ownable, ERC721Min {
    ...
    address constant private DEAD_ADRESS = 0x000000000000000000000000000000000000dEaD;
    ...
    string public _tokenBaseURI = ""; // SET TO THE METADATA URI
    ...
    mapping(address => uint256) public purchaseCountPerAddress;
    ...
    function addFeeRecipient(address recipient, uint256 basisPoints)
        ...
        onlyOwner
    {
        require(feeRecipientCount < 11, "TOO_MANY_RECIPIENTS");
        feeRecipientCount++;
    ...
    function airdrop(address[] calldata receivers, uint256[] memory amounts)
        external
        onlyOwner
    {
    ...
    function buyWithToken(uint16 amount) external {
        ...
        require(maxMint >= purchaseCount + amount, "EXCEED_MAX_SALE_SUPPLY");
        require(amount <= maxPerMint, "EXCEED_MAX_PER_MINT");
        require(
            maxPerAddress == 0 ||
                maxPerAddress >= purchaseCountPerAddress[_msgSender()] + amount,
            "EXCEED_MAX_PER_ADDRESS"
        );
        ...
        IERC20(pairedToken).transferFrom(
            _msgSender(),
            DEAD_ADRESS,
            pairedTokenPerNFT * amount
        );
        purchaseCount += amount;
        purchaseCountPerAddress[_msgSender()] += amount;
        purchases[_msgSender()].push(
            Purchase({
                amount: amount,
    ...
    function claim() external {
        Purchase[] storage purchase = purchases[_msgSender()];
        require(purchase.length > 0, "NO_PURCHASES");
        uint256 claimable;
        for (uint256 x; x < purchase.length; x++) {
            if (
                purchase[x].claimed ||
                purchase[x].purchaseTime + lockTime > block.timestamp
            ) continue;
            claimable += purchase[x].amount;
            purchase[x].claimed = true;
        }
    ...
    function editFeeRecipient(
        ...
        require(id > 0 && id <= feeRecipientCount, "INVALID_ID");
        totalFeeBasisPoints =
            totalFeeBasisPoints -
    ...
    function setMaxMint(uint16 maxMint_) external onlyOwner {
        require(!saleActive, "SALE_ACTIVE");
        maxMint = maxMint_;
    }
    ...
    function updatePairedToken(address _pairedToken, uint256 _pairedTokenPerNFT)
        external
        onlyOwner
    {
        pairedToken = _pairedToken;
