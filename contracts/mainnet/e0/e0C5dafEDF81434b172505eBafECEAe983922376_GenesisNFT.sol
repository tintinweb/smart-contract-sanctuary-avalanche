...
import { IERC721Burnable } from "./base/interfaces/IERC721Burnable.sol";
...
import { IERC721Permit, ERC721Permit } from "./base/token/ERC721/extensions/ERC721Permit.sol";
import { IGenesisNFTBox } from "./interfaces/IGenesisNFTBox.sol";
import { IGenesisNFT } from "./interfaces/IGenesisNFT.sol";
import { Withdrawable } from "./internal/Withdrawable.sol";
import { Signable } from "./internal/Signable.sol";
import { Verifier } from "./internal/Verifier.sol";
import { ReentrancyGuard } from "./utils/ReentrancyGuard.sol";
import { Helper } from "./libraries/Helper.sol";

contract GenesisNFT is ERC721Permit, ERC721Rentable, ERC721URIStorage, IGenesisNFT, Withdrawable, ReentrancyGuard, Verifier {
    ...
    ) payable ERC721Rentable(name_, symbol_) Signable(type(GenesisNFT).name, "1") {
        _configGenesis(genesisNFT_);
        _idCounter = 1;
    }

    function setMinterVerifier(address newVerifier_) external onlyOwner {
        _setMinterVerifier(newVerifier_);
    }

    function setConvertVerifier(address newVerifier_) external onlyOwner {
        _setConvertVerifier(newVerifier_);
    }

    function configGenesis(address newGenesis_) external onlyOwner {
        _configGenesis(newGenesis_);
    }
    ...
    function convert(
        ...
        address genesisNFT = _genesisNFT.toAddress();
        _convert(genesisNFT, tokenId_, meta_, v_, r_, s_);
    }
    ...
    function convertWithPermit(
        ...
        address genesisNFT = _genesisNFT.toAddress();
        IERC721Permit(genesisNFT).permit(address(this), tokenId_, deadline_, v_, r_, s_);
        _convert(genesisNFT, tokenId_, meta_, vv_, vr_, vs_);
    }
    ...
    function batchMintWithSignature(
        ...
        _verifyRaw(_minterVerifier, keccak256(abi.encodePacked(sender, abi.encode(meta_), deadline_)), v_, r_, s_);
        _batchMint(sender, meta_);
    }

    function batchMint(address user_, Metadata[] calldata meta_) external onlyOwner {
        _batchMint(user_, meta_);
    }
    ...
    function _convert(
        ...
        address sender = _msgSender();
        IERC721(genesisNFT_).transferFrom(sender, address(this), tokenId_);
        IERC721Burnable(genesisNFT_).burn(tokenId_);

        (, uint256 typeNFT) = IGenesisNFTBox(genesisNFT_).metadata(tokenId_);
        (, , , uint256 unboxQuantity) = IGenesisNFTBox(genesisNFT_).getTypeNFT(typeNFT);
        if (unboxQuantity != meta_.length) revert GenesisNFT__LengthMismatch();

        _verifyRaw(_convertVerifier, keccak256(abi.encodePacked(sender, tokenId_, abi.encode(meta_))), v_, r_, s_);
        _batchMint(sender, meta_);
    }

    function _batchMint(address user_, Metadata[] calldata meta_) internal {
        uint256 length = meta_.length;
        uint256 tokenId = _idCounter;
        for (uint256 i = 0; i < length; ) {
            _metadata[tokenId] = Metadata(meta_[i].creator, meta_[i].typeNFT, meta_[i].rarity);
            unchecked {
        ...
        _idCounter = tokenId;
        emit Registered(user_, tokenId, length);
    }

    function _configGenesis(address newGenesis) internal {
        _genesisNFT = newGenesis.toUint256();
    }

    function genesis() external view returns (address) {
        return _genesisNFT.toAddress();
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721Rentable) {
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function __notExpired(uint256 deadline_) private view {
        if (block.timestamp > deadline_) revert GenesisNFT__Expired();
    }
}
...
abstract contract ERC721Permit is ERC721, IERC721Permit, Signable {
    ...
    bytes32 private constant _PERMIT_TYPEHASH = 0x49ecf333e5b8c95c40fdafc95c1ad136e8914a8fb55e9dc8bb01eaa83a2df9ad;
    ...
    function permit(
        ...
        _verify(owner, keccak256(abi.encode(_PERMIT_TYPEHASH, spender_, tokenId_, _useNonce(tokenId_), deadline_)), v_, r_, s_);
...
interface IGenesisNFTBox {
    error GenesisNFTBox__InvalidType();
    ...
    function getTypeNFT(uint256 typeNFT)
        external
        view
        returns (
...
interface IGenesisNFT {
    ...
    event Registered(address indexed user, uint256 indexed tokenId, uint256 quantity);
}
...
abstract contract Signable is EIP712, ISignable {
    ...
    function nonce(uint256 tokenId_) external view override returns (uint256) {
    ...
    function _verify(
        ...
        _checkVerifier(verifier_, _hashTypedDataV4(structHash_), v_, r_, s_);
    }
    ...
    function _verifyRaw(
        ...
        _checkVerifier(verifier_, messageHash_.toEthSignedMessageHash(), v_, r_, s_);
    }
    ...
    function _checkVerifier(
        ...
        if (digest_.recover(v_, r_, s_) != verifier_) revert Signable__InvalidSignature();
    }
    ...
    function _checkVerifier(
        ...
        if (digest_.recover(signature_) != verifier_) revert Signable__InvalidSignature();
    }

    function _useNonce(uint256 tokenId) internal returns (uint256) {
        unchecked {
...
abstract contract Verifier is IVerifier, Context {
    address internal _minterVerifier;
    address internal _convertVerifier;

    function minterVerifier() external view override returns (address) {
        return _minterVerifier;
    }

    function convertVerifier() external view override returns (address) {
        return _convertVerifier;
    }

    function _setMinterVerifier(address newVerifier) internal {
        if (newVerifier == address(0)) revert Verifier__ZeroAddress();
        emit VerifierTransferred(_minterVerifier, newVerifier);
        _minterVerifier = newVerifier;
    }

    function _setConvertVerifier(address newVerifier) internal {
        if (newVerifier == address(0)) revert Verifier__ZeroAddress();
        emit VerifierTransferred(_convertVerifier, newVerifier);
        _convertVerifier = newVerifier;
    }
}
...
interface IVerifier {
    error Verifier__InvalidSignature();
    error Verifier__ZeroAddress();
    error Verifier__NotAuthorized();

    event VerifierTransferred(address verifier, address newVerifier);

    function minterVerifier() external view returns (address);

    function convertVerifier() external view returns (address);
}