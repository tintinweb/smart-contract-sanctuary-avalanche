 ...
 *  @author CygnusDAO
 *  @notice Initializes Borrow Arm
 *  @notice Passes name, symbol and decimals to CygnusTerminal for the CygDAI Token
...
contract CygnusBorrowControl is ICygnusBorrowControl, CygnusTerminal("Cygnus: Borrow", "CygnusDAI", 18) {
    ...
    function setBaseRate(uint256 newBaseRate) external override cygnusAdmin nonReentrant {
        ...
        // Old base rate
        uint256 oldBaseRatePerYear = baseRatePerYear;

        // Update base rate
        baseRatePerYear = newBaseRate;

        /// @custom:event NewBaseRate
        emit NewBaseRate(oldBaseRatePerYear, newBaseRate);
    }
 ...
 *  @author CygnusDAO
 *  @notice Constructs the interest rate model used and updates the internal `per-second` rates, these are set
 *          as internal since we have the easier to read annual rates as public on CygnusBorrowControl.sol
 */
contract CygnusBorrowInterest is ICygnusBorrowInterest, CygnusBorrowControl {
    ...
    // Used to accrue interest per second in cygnusborrowtracker
...
contract CygnusBorrow is ICygnusBorrow, CygnusBorrowTracker {
    ...
    function mintReservesInternal(uint256 _exchangeRate, uint256 _totalSupply) internal returns (uint256) {
        ...
        if (_exchangeRate > _exchangeRateLast) {
            ...
            // Else mint new reserves and update the exchange rate
            else {
                // Get reserves
...
contract CygnusAlbireo is ICygnusAlbireo, Context {
    ...
    function deployAlbireo(
        ...
        borrowParameters = BorrowParameters({
            ...
            vegaTokenManager: vegaTokenManager
        });
