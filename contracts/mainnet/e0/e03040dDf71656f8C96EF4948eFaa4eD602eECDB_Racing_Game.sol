...
contract Racing_Game is Ownable {
    ...
    function getEpochAddressListLength(uint256 epochId) external view returns (uint256) {
        return epochAddressList[epochId].length;
    }

    function getEpochRaceIdsLength(uint256 epochId) external view returns (uint256) {
        return epochRaceIds[epochId].length;
    }
