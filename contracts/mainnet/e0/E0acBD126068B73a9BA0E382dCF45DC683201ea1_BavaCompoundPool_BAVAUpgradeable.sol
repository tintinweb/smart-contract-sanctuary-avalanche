...
contract BavaCompoundPool_BAVAUpgradeable is
    Initializable,
    UUPSUpgradeable,
    BRTERC20Upgradeable
{
    ...
    VaultInfo public vaultInfo; // Info of each pool.

    // Info of each user.
    struct UserInfo {
        uint256 receiptAmount; // user receipt tokens.
        uint256 rewardDebt; // Reward debt. See explanation below.
        uint256 blockdelta; // time passed since last deposit
    ...
    struct VaultInfo {
        ...
        IMiniChef stakingContract; // Panglin LP Staking contract
    ...
    function setAllowances(uint256 _amount) external onlyOwner {
        ...
        if (address(router) != address(0)) {
            ...
            IERC20(IPair(address(vaultInfo.lpToken)).token0()).approve(
                address(router),
                _amount
            );
            IERC20(IPair(address(vaultInfo.lpToken)).token1()).approve(
                address(router),
                _amount
            );
    ...
    function deposit(uint256 _amount) public {
        ...
        uint256 poolReceiptAmount = getSharesForDepositTokens(_amount);
        vaultInfo.depositAmount += _amount;

        uint256 userReceiptAmount = poolReceiptAmount -
            ((poolReceiptAmount * userDepFee) / 10000);
        uint256 devrReceiptAmount = poolReceiptAmount - userReceiptAmount;
        ...
        if (vaultInfo.restaking_enabled == true) {
            ...
            vaultInfo.stakingContract.deposit(
                vaultInfo.restakingFarmID,
                restakeAmount,
                address(this)
            );
        }
    ...
    function emergencyWithdraw() public {
        ...
        require(vaultInfo.depositAmount >= depositTokenAmount, "#>Bal"); //  pool.lpToken.balanceOf(address(this))
        _withdrawDepositTokens(depositTokenAmount);
    ...
    function _withdrawDepositTokens(uint256 amount) private {
        ...
        (uint256 depositAmount, ) = vaultInfo.stakingContract.userInfo(
            vaultInfo.restakingFarmID,
            address(this)
        );

        if (depositAmount > 0) {
            uint256 pendingRewardAmount = vaultInfo
                .stakingContract
                .pendingReward(vaultInfo.restakingFarmID, address(this));
            if (pendingRewardAmount == 0) {
                vaultInfo.stakingContract.emergencyWithdraw(
                    vaultInfo.restakingFarmID,
                    address(this)
                );
            } else if (depositAmount >= amount) {
                vaultInfo.stakingContract.withdraw(
                    vaultInfo.restakingFarmID,
                    amount,
                    address(this)
                );
    ...
    function _getReinvestReward() private {
        uint256 rewardBalBefore = Bava.balanceOf(address(this));
        uint256 pendingRewardAmount = vaultInfo.stakingContract.pendingReward(
            vaultInfo.restakingFarmID,
            address(this)
        );
        if (pendingRewardAmount > 0) {
            vaultInfo.stakingContract.harvest(
                vaultInfo.restakingFarmID,
                address(this)
            );
        }
    ...
    function _reinvest() private returns (uint256) {
        _getReinvestReward();
        uint256 wavaxAmount = _convertRewardIntoWAVAX();
        uint256 liquidity = _convertWAVAXTokenToDepositToken(wavaxAmount);
        if (liquidity > 0) {
    ...
    function _convertRewardIntoWAVAX() private returns (uint256) {
        ...
        uint256 reinvestFee = (avaxAmount * (REINVEST_REWARD_BIPS)) /
            (BIPS_DIVISOR);
        if (reinvestFee > 0) {
    ...
    function _convertWAVAXTokenToDepositToken(uint256 amount)
        private
        returns (uint256)
    {
        require(amount > 0, "#<0");
