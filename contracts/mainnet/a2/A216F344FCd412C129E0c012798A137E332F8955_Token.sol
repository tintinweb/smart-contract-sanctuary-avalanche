...
import "../../lib0.8/upgrable/ERC20.sol";
...
contract Token is ERC20, Ownable {
    ...
    function initialize(string memory name_, string memory symbol_, address _airdrop) external initializer {
        ...
        teamLockToken = new TokenVesting(msg.sender, block.timestamp + 180 days, 0 days, 300 days);
        adLockToken = new TokenVesting(msg.sender, block.timestamp + 90 days, 0 days, 150 days);

        __mint(msg.sender, 1 * 1e8 * 1e18);
        __mint(_airdrop, 1 * 1e8 * 1e18);
        __mint(address(teamLockToken), 0.5 * 1e8 * 1e18);
        __mint(address(adLockToken), 0.5 * 1e8 * 1e18);
    }

    function mintToFarm(address _farm) external onlyOwner {
        require(!doOnce, "only can mint once");
        doOnce = true;
        __mint(_farm, 7 * 1e8 * 1e18);
    }
