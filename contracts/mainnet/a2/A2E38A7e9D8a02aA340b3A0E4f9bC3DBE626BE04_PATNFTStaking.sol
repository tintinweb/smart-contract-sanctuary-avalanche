...
contract PATNFTStaking is Ownable, Pausable, ERC721Holder  {
    IRewardToken public PATContract;
    IERC721 public nft;
    address nft_address;
    ...
    uint256 public stakingRewards = 1e18;
    ...
    constructor(address _nft_address, IRewardToken _PAT_token) {
        nft_address = _nft_address;
        nft = IERC721(_nft_address);
        PATContract = _PAT_token;
    }
    ...
    function _stake(uint256 _tokenId) internal {
        ...
        require(stakedTokens[tokenId] == false, "Tokens Already Staked");
    ...
    function calculateReward(uint256[] memory _tokenIds)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (
                ...
                uint256 stakedTime = (
                    (block.timestamp - uint256(stakedNFT.tokenStakingCoolDown))
                ) / stakingTime;

                reward += stakingRewards * stakedTime;
            }
        }
        return reward;
    }
    ...
    function _updateReward(uint256[] memory _tokenIds) internal {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (
                ...
                stakedNFT.balance += stakingRewards * stakedTime;
    ...
    function claimReward(uint256[] memory _tokenIds) public whenNotPaused {
        ...
        if (reward > 0) {
            PATContract.mint(msg.sender, reward);
    ...
    function setStakingTime(uint256 _time) public onlyOwner {
        stakingTime = _time;
    }

    function setStakingTimePerDay_OrHour(bool _isPerDay) public onlyOwner {
        if(_isPerDay)
            stakingTime = 1 days;
        else 
            stakingTime = 1 hours;
    ...
    function setNFTContract(address _nft_address) public onlyOwner {
        nft = IERC721(_nft_address);
    }

    function setPATToken(address _token_address) public onlyOwner {
        PATContract = IRewardToken(_token_address);
    }
