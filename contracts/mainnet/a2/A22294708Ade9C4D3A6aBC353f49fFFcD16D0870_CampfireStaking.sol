...
contract CampfireStaking is Ownable, ReentrancyGuard {
  ...
  Counters.Counter private _totalNftsStaked;

  uint256 private _totalBlockEntries;
  address private _wavaxAddress;

  mapping (address => Counters.Counter) private _totalNftsStakedForAccount;
  ...
  function stakeSet(address _contractAddress, uint256[] memory _tokenIds) public nonReentrant {
    for (uint256 index = 0; index < _tokenIds.length; index++) {
      ...
      _stake(_contractAddress, _tokenId);
    }
  }
  ...
  function _stake(address _contractAddress, uint256 _tokenId) private {
    ...
    if (_totalNftsStakedForAccount[msg.sender].current() > 0) {
      uint256 avaxRewards = availableAVAXRewards(msg.sender);
      uint256 wavaxRewards = availableWAVAXRewards(msg.sender);

      _totalBlockEntries -= _blockEntries[msg.sender];
    ...
    _totalNftsStaked.increment();
    _totalNftsStakedForAccount[msg.sender].increment();

    uint256 blockEntry = block.number * _totalNftsStakedForAccount[msg.sender].current();
    _blockEntries[msg.sender] = blockEntry;
    _totalBlockEntries += blockEntry;
  ...
  function totalStaked() public view returns (uint256) {
    return _totalNftsStaked.current();
  }

  function totalStakedFor(address staker) public view returns (uint256) {
    return _totalNftsStakedForAccount[staker].current();
  }
  ...
  function unstake(address _contractAddress, uint256 _tokenId) public nonReentrant {
    ...
    _totalNftsStaked.decrement();
    _totalNftsStakedForAccount[msg.sender].decrement();
  ...
  function unclaimedBlocks() public view returns (uint256) {
    uint256 totalUnclaimedBlocks = (block.number * _totalNftsStaked.current()) - _totalBlockEntries;
    return totalUnclaimedBlocks;
  }

  function claim() public nonReentrant {
    require(_totalNftsStakedForAccount[msg.sender].current() > 0, "Campfire Staking: You are not staked.");
    uint256 avaxRewards = availableAVAXRewards(msg.sender);
    uint256 wavaxRewards = availableWAVAXRewards(msg.sender);
  ...
  function availableAVAXRewards(address account) public view returns (uint256) {
    ...
    return _rewardsCalc(account, available);
  }
  ...
  function _rewardsCalc(address account, uint256 available) private view returns (uint256) {
      uint256 totalUnclaimedBlocks = (block.number * _totalNftsStaked.current()) - _totalBlockEntries;

     if (totalUnclaimedBlocks > 0) {
      uint256 accountNftsStaked = _totalNftsStakedForAccount[account].current();

      uint256 accountUnclaimedBlocks = (block.number * accountNftsStaked) - (_blockEntries[account]);
