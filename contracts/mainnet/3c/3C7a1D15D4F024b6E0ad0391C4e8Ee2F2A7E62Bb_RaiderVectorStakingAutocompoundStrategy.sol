...
import "./AAutocompoundStrategy.sol";
 ...
 */
contract RaiderVectorStakingAutocompoundStrategy is AAutocompoundStrategy {
   ...
   * {rewardTokenRoutes} - Route we take to get from {rewardToken} into {depositToken}.
   */
  mapping(address => address[]) public rewardTokenRoutes;
  ...
  {
    poolContract = IVectorPoolHelper(_masterChef);

    // Extract tokens to routes
    extractRewardTokensToToken(rewardTokens, depositToken, rewardTokenRoutes);
  }

  /**
   * @dev Swaps {rewardToken} for {lpToken0}, {lpToken1} & {wftm} using SpookySwap.
   */
  function addLiquidity(address rewardToken) internal override {
  ...
  function retireStrat() external override {
    ...
    IERC20(depositToken).transfer(vault, pairBal);
  }
 ...
 * Expect the amount of LP tokens you have to grow over time while you have assets deposit
 */
abstract contract AAutocompoundStrategy is Ownable, Pausable {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;
  ...
  function extractRewardTokensToToken(
    address[] memory _rewardTokens,
    address toToken,
    mapping(address => address[]) storage routesMapping
  ) internal {
    for (uint256 i = 0; i < _rewardTokens.length; i++) {
      address rewardToken = _rewardTokens[i];

      if (toToken == WNATIVE) {
        routesMapping[rewardToken] = [rewardToken, WNATIVE];
      } else if (toToken != rewardToken) {
        routesMapping[rewardToken] = [rewardToken, WNATIVE, toToken];
      }
    }
  }
  ...
  function withdraw(uint256 _amount) external {
    ...
    if (tokenBalance < _amount) {
      poolWithdraw(_amount.sub(tokenBalance));
      tokenBalance = IERC20(depositToken).balanceOf(address(this));
    }

    if (tokenBalance > _amount) {
      tokenBalance = _amount;
    }
    uint256 withdrawFee = tokenBalance.mul(securityFee).div(PERCENT_DIVISOR);
    IERC20(depositToken).safeTransfer(vault, tokenBalance.sub(withdrawFee));
  }
   ...
   */
  function addLiquidityAll() internal {
    for (uint256 i = 0; i < rewardTokens.length; i++) {
      address rewardToken = rewardTokens[i];
      addLiquidity(rewardToken);
    }
  }
  ...
  function addLiquidity(address rewardToken) internal virtual;
