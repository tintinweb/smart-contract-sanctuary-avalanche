/**
 *Submitted for verification at snowtrace.io on 2022-09-06
*/
...
abstract contract prevfeeMGR is Context{
    ...
    function getPrev(address _account,uint256 _x) view external virtual returns (string memory,uint256,uint256,uint256,uint256,bool,bool,bool,bool);
}
...
contract feeMGR is Ownable{
    ...
    function transfer(address _account,uint256 _x,uint c, uint k) external onlyOwner {
    uint len = prevFee.getAccountsLength();
	for(uint i = c;i<k;i++){
		
    		Accounts.push(protoMgr.getProtoAddress(i));
    		if(nebuLib.addressInList(transfered,Accounts[i]) == false){
	    		for(uint j = 0;j<protoMgr.getProtoStarsLength(Accounts[i]);j++){
	    			totalFees[address(this)].protos++;
	    			totalFees[Accounts[i]].protos++;
	    			totalFees[Accounts[i]].feesOwed +=1;
	    			recPayFees(Accounts[i]);
	    			addProto(Accounts[i],j);
	    		}
	    		transfered.push(Accounts[i]);
	    	}
	 }
    }

    function addProto(address _account,uint256 _x) internal{
	(string memory _name,uint256 collapseDate,uint256 nextDue,uint256 futureFees,uint256 feeFroze,bool owed,bool full,bool insolvent,bool imploded) = prevFee.getPrev(_account,_x);	
    	protoOwners[_account].push(PROTOOWNERS({
	    	name:_name,
