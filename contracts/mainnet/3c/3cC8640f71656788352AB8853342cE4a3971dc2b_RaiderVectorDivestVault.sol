...
abstract contract ADivestVault2 is ERC20, Ownable, ReentrancyGuard {
  ...
  // Emergency mode flag, once the contract is in emergency mode only withdraws are allowed.
  ...
  function deposit(uint256 _amount) public nonReentrant {
    ...
    require(!isEmergencyMode, "cannot deposit in emergency mode");
  ...
  function withdraw(uint256 _shares) public nonReentrant {
    ...
    // Before withdrawal, payout the user and update info, skip this if emergency mode.
    if (!isEmergencyMode) {
      distributeDivestedTokens();
    }
