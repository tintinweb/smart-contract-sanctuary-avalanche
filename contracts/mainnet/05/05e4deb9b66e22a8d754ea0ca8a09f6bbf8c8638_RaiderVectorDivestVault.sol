...
contract RaiderVectorDivestVault is ADivestVault {
  ...
  // TODO @cstoneham - replace zapin
  function zapIn() external payable override {
    // 1. Swap native to stake token
    IRaiderZapper(raiderZapper).swapFromNative(
      address(token),
      msg.sender,
      uniRouter
    );
...
abstract contract ADivestVault is ERC20, Ownable, ReentrancyGuard {
  ...
  function zapIn() external payable virtual;
