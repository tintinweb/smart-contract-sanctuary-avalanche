...
contract UWLHub {
    ...
    event WLImagePathUpdated(uint256 indexed id, string newImagePath);
    event WLUrlUpdated(uint256 indexed id, string newUrl);
    ...
    function setWhiteListUrl(
        uint256 id, 
        string memory newUrl
    ) external onlyOwnerOrManager onlyExistingWhiteList(id) {
        WhiteList storage wl = whitelists[id];
        wl.url = newUrl;
        emit WLUrlUpdated(id, newUrl);
    }

    function setWhiteListImagePath(
        uint256 id, 
        string memory newImagePath
    ) external onlyOwnerOrManager onlyExistingWhiteList(id) {
        WhiteList storage wl = whitelists[id];
        wl.imagePath = newImagePath;
        emit WLImagePathUpdated(id, newImagePath);
    }
