...
contract VoltTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, VOLTs }
    ...
    uint public VOLTsQueue; // Delays change to VOLTs address
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IVOLTERC20( VOLT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( VOLTs ).balanceOf( msg.sender ); // Can only borrow against VOLTs held
    ...
    function repayDebtWithVOLT( uint _amount ) external {
        ...
        IVOLTERC20( VOLT ).burnFrom( msg.sender, _amount );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.VOLTs ) { // 9
            VOLTsQueue = block.number.add( blocksNeededForQueue );
        } else return false;
