...
contract PendingFeesContract {
    ...
    function getIdsWithFees(address LBPair, address user, uint256[] memory ids)
        external
        view
        returns (uint256[] memory idsWithFees, uint256[] memory amountsX, uint256[] memory amountsY)
    {
...
library PendingFees {
        ...
        returns (uint256[] memory idsWithFees, uint256[] memory amountsX, uint256[] memory amountsY)
    {
        ...
        (uint256 cachedX, uint256 cachedY) = getCachedFees(LBPair, user);
        ...
        for (uint256 i; i < ids.length; ++i) {
            ...
            if (amountX + amountY > cachedX + cachedY) {
                idsWithFees[j] = ids[i];
                amountsX[j] = amountX;
                amountsY[j] = amountY;
