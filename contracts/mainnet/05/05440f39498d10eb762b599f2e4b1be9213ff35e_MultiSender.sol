...
contract MultiSender {
    ...
    function sendTokens(IERC20 token, address[] calldata recipients, uint256[] calldata values) external {
        uint256 total = 0;
        for (uint256 i = 0; i < recipients.length; i++) {
            total += values[i];
        }
        require(token.transferFrom(msg.sender, address(this), total), "TRANSFER FROM FAILED");
...
contract MultiSend is Script, Test {
    bytes32 constant salt = keccak256("[email protected]");
    // https://github.com/ZeframLou/create3-factory
    ICREATE3 constant CREATE3 = ICREATE3(0x9fBB3DF7C40Da2e5A0dE984fFE2CCB7C47cd0ABf);
    
    function run() public {
        address[] memory recipients = vm.envAddress("RECIPIENTS", ",");

        // Top up the address to a min balance
        // To avoid typos this value is hard coded down below, update
        // the values accordingly
        uint256 minBalance = getAmountForChain();
        uint256[] memory values = new uint256[](recipients.length);
        uint256 total = 0;
        for (uint256 i = 0; i < recipients.length; i++) {
            uint256 balance = recipients[i].balance;
            if (balance < minBalance) {
                uint256 value = minBalance - balance;
                values[i] = value;
                total += value;
            }
        }
        ...
        if (total == 0) {
            emit log_string("Not required to send");
            return;
        }

        // Deterministic deploy for the address sending the funds
        MultiSender multiSender = MultiSender(CREATE3.getDeployed(address(msg.sender), salt));
        vm.startBroadcast();
        if (address(multiSender).codehash == bytes32(0)) {
            // deploy deterministically for this address
            address deployed = CREATE3.deploy(salt, type(MultiSender).creationCode);
            emit log_named_address("Deployed MultiSend at", address(deployed));
        }
        multiSender.sendEth{value: total}(recipients, values);
        vm.stopBroadcast();
    }

    // TODO remove when Foundry stops deploying libraries
    ...
    function getAmountForChain() internal view returns (uint256) {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
