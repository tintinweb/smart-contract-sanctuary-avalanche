...
contract YobiSamurais is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    using Strings for uint256;

    uint256 public constant MAX_MINTABLE = 1375;  //1375
    uint256 public constant MAX_RESERVED = 375;  //375
    uint256 public constant MAX_PER_CLAIM = 10; //10
    uint256 public constant MAX_WL_CLAIM = 250; //250
    ...
    uint256 public airDrop_Wallets_Count = 0;
    bool public airDroped = false;
    ...
    // tokenID mapping for random mint
    ...
    bool public isRandomMint = false;
    
    address royaltyRecipient;
    address NFTsTransferWallet;
    string nftName = "Yobi Samurais";
    ...
    function mintWL(uint256 n, bytes32[] calldata _proof) public payable {
        require(earlyMint, "Mint for WL is not open yet.");
        require(n + minted  <= MAX_WL_CLAIM, "Not enough left to mint of White List, decrease the number of minting.");
    ...
    function mintPublic(uint256 n) public payable {
    ...
    function airDropNFTsTo(
        ...
        airDrop_Wallets_Count += addresses.length;
        for (uint256 i = 0; i < addresses.length; i++) {
            minted += 1;
            randomMint(addresses[i], minted);
            emit Claim(minted);
        }
    }
    ...
    function randomMint(address receiver, uint256 nextTokenIndex) internal {
        if(isRandomMint){
            ...
            assert(takenImages[nextTokenIndex] == 0);
            takenImages[nextTokenIndex] = 1;
            tokenIDMap[nextTokenIndex] = nextTokenIndex;
        }
        _safeMint(receiver, nextTokenIndex);
    }

    function setRandomMint(bool _isRandomMint) onlyOwner public{
        isRandomMint = _isRandomMint;
    }
