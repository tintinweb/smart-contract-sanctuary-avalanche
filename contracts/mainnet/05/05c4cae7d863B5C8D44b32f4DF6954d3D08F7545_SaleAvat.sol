...
contract SaleAvat is Ownable, ReentrancyGuard {
    ...
    constructor (address[] memory depositTokens_, uint[] memory rates_, address withdrawToken_, address signatory_) {
     ...
     * param startSale_ End date for sale
     * param endSale_ End date for sale
     * param precision_ Precision for percent of distribution period
     */
    function createDistributionPeriod(DistributionPeriod[] memory distributionPeriods_, uint startSale_, uint endSale_) external onlyOwner {
        ...
        require(startSale < endSale, "SaleAvat::createDistributionPeriod: Start of sale should be lesser then End of sale");
        require(endSale < distributionPeriods_[0].timestamp, "SaleAvat::createDistributionPeriod: End of sale should be lesser then fist claim period");
    ...
    */
    function buyToken(address addressTransferFrom, uint amountTransferFrom, uint valueAllocation, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        bool accept = permitBySig(msg.sender, valueAllocation, nonce, deadline, v, r, s);
        ...
        require(addressAllocationSpent[address(msg.sender)] <= valueAllocation, "SaleAvat::buyToken: Allocation spent");

        _depositTokens[addressTransferFrom].safeTransferFrom(address(msg.sender), address(this), amountTransferFrom);
        ...
        addressFundsCollected[addressTransferFrom][address(msg.sender)] = addressFundsCollected[addressTransferFrom][address(msg.sender)].add(amountTransferFrom);
    }
     ...
     * @param s Half of the ECDSA signature pair
     */
    function permitBySig(address participant, uint rawAmount, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) internal returns (bool) {
        uint96 amount;
    ...
    * @param ownerAddress_ Address owner
    */
    function collectToken(address token_, uint amount_, address ownerAddress_) external onlyOwner {
        address payable addressPayable = payable(ownerAddress_);

        _depositTokens[token_].safeTransfer(addressPayable, amount_);
    }
    ...
    * @param ownerAddress_ Address owner
    */
    function leftover(uint amount_, address ownerAddress_) external onlyOwner {
        address payable addressPayable = payable(ownerAddress_);

        _withdrawToken.safeTransfer(addressPayable, amount_);
    }
    ...
    */
    function updateTokenRates(address[] memory depositTokens_, uint[] memory rates_) external onlyOwner {
    ...
    **/
     ...
     * @param s Half of the ECDSA signature pair
     */
    function checkSig(address participant, uint rawAmount, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) external view returns (bool) {
        uint96 amount;
