...
contract WhatisJoe is ERC721  {
    ...
    constructor()
        ERC721("What is Joe ?", "WIJ")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 223) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigwmvavozcfgwqyymxglue72n56x3ogaogcdoviro7nwdztwgfkiu/", Strings.toString(tokenId), ".json")
            );
    }
