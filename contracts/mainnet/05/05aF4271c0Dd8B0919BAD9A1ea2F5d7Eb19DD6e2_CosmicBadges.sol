...
ERC1155URITokenJSON, ERC1155Soulbound {
    ...
    function reallocate(uint256 id, address[] memory to, uint256[] memory amounts) public onlyRole(MINTER_ROLE) {
        for (uint256 i = 0; i < to.length; i++) {
            _burn(to[i], uint256(0), amounts[i]);
            _mint(to[i], id, amounts[i], '');
        }
    }
    function uri(uint256 tokenId) public view virtual override(ERC1155Upgradeable, ERC1155URITokenJSON) returns(string memory) {
        ...
        return _makeJSON(tokenId, name, description, attributes);
    }
