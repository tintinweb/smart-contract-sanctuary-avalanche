...
interface IZap {
    ...
    function swapToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external;
    function swapToNative(address _from, uint amount, address routerAddr, address _recipient, uint _minOut) external;
    function zapIn(address _to, address routerAddr, address _recipient, uint _minOut) external payable;
    function zapInToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external;
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient, uint _minOut) external;
    function zapOut(address _from, uint amount, address routerAddr, address _recipient, uint _minOut) external;
    function zapOutToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external;
}

// File: Zap.sol

contract SishiZapAvax is Ownable, IZap {
    ...
    mapping(address => mapping(address => address)) public tokenBridgeForRouter;
    mapping(address => bool) public isFeeOnTransfer;

    // mapping (address => bool) public useNativeRouter;

    constructor(address _WNATIVE) Ownable() public {
       WNATIVE = _WNATIVE;
    }
    ...
    function zapInToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external override {
        _approveTokenIfNeeded(_from, routerAddr);
        uint before = IERC20(_to).balanceOf(_recipient);
        ...
        require(IERC20(_to).balanceOf(_recipient) >= before.add(_minOut), "Insufficient Output");
    }
    ...
    function zapIn(address _to, address routerAddr, address _recipient, uint _minOut) external payable override {
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient, uint _minOut) external override {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        ...
        IUniswapV2Pair pairTo = IUniswapV2Pair(
            IUniswapV2Factory(IJoeRouter02(_toRouter).factory())
                .getPair(pair.token0(),pair.token1())
        ); 

        uint before = pairTo.balanceOf(_recipient);

        IJoeRouter02(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);

        require(pairTo.balanceOf(_recipient) >= before.add(_minOut), "Insufficient Output");

    }

    function zapOut(address _from, uint amount, address routerAddr, address _recipient, uint _minOut) external override {
        ...
        uint before = address(_recipient).balance;
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            if(isFeeOnTransfer[token]){
                uint amtTokenBefore = IERC20(token).balanceOf(address(this));
                (amtAVAX) = IJoeRouter02(routerAddr).removeLiquidityAVAXSupportingFeeOnTransferTokens(token, amount, 0, 0, address(this), block.timestamp);
                // swap with msg.sender as recipient, so they already get the Native
                _swapTokenForNative(token, IERC20(token).balanceOf(address(this)).sub(amtTokenBefore), _recipient, routerAddr);
            ...
            uint amt0;
            uint amt1;
            if(isFeeOnTransfer[token0] || isFeeOnTransfer[token1]){
                uint amt0Before = IERC20(token0).balanceOf(address(this));
                uint amt1Before = IERC20(token1).balanceOf(address(this));

                // convert both for Native with msg.sender as recipient
                IJoeRouter02(routerAddr).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
                
                amt0 = IERC20(token0).balanceOf(address(this)).sub(amt0Before);
                amt1 = IERC20(token1).balanceOf(address(this)).sub(amt1Before);
            } else {
                // convert both for Native with msg.sender as recipient
        ...
        require(address(_recipient).balance >= before.add(_minOut), "Insufficient Output");
    }

    function zapOutToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external override {
        ...
        if(isFeeOnTransfer[token0] || isFeeOnTransfer[token1]){
            ...
            uint amt0 = IERC20(token0).balanceOf(address(this)).sub(amt0Before);
            uint amt1 = IERC20(token1).balanceOf(address(this)).sub(amt1Before);
    ...
    function swapToken(address _from, uint amount, address _to, address routerAddr, address _recipient, uint _minOut) external override {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
        uint before = IERC20(_to).balanceOf(_recipient);
    ...
    function swapToNative(address _from, uint amount, address routerAddr, address _recipient, uint _minOut) external override {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
        uint before = address(_recipient).balance;
    ...
    function _swapNativeForToken(address token, uint value, address recipient, address routerAddr) private returns (uint) {
        ...
        uint tokenBefore = IERC20(token).balanceOf(address(this));

        if (isFeeOnTransfer[token]) {
            router.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value : value}(0, path, recipient, block.timestamp);
        }else {
            router.swapExactAVAXForTokens{value : value}(0, path, recipient, block.timestamp);
        }

        uint tokenAfter = IERC20(token).balanceOf(address(this));
        return tokenAfter.sub(tokenBefore);
    }
    ...
    function _swapTokenForNative(address token, uint amount, address recipient, address routerAddr) private returns (uint) {
        ...
        if (isFeeOnTransfer[token]) {
            ...
            router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp);
    ...
    function _swap(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (isFeeOnTransfer[_from] || isFeeOnTransfer[_to]) {
            uint before = IERC20(_to).balanceOf(address(this));
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp);
            return IERC20(_to).balanceOf(address(this)).sub(before);
        } else {
