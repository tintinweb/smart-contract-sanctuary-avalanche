...
contract SteakHutVoter is Ownable, AccessControl { 
    ...
    address public BJMC_ADDRESS = 0x4483f0b6e2F5486D06958C20f8C39A7aBe87bf8F;
    address public VeJoeStaking_ADDRESS = 0x25D85E17dD9e544F6E9F8D44F99602dbF5a97341;
    address public JOE = address(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd);
    address public VeJOE = address(0x3cabf341943Bc8466245e4d6F1ae0f8D071a1456);
    ...
    function _deposit(IERC20 _lpToken, uint256 _pid, uint256 _amount) public {
        IBoostedMasterChefJoe BJMC = IBoostedMasterChefJoe(BJMC_ADDRESS);        

        //Send LP token to BMCJ
        _lpToken.approve(address(BJMC), _amount);
        //deposit to BJMC    
        BJMC.deposit(_pid, _amount);
        _lpToken.approve(address(BJMC), 0);
    ...
    function _withdraw(IERC20 _lpToken, uint256 _pid, uint256 _amount) public {
        require(hasRole(MASTERCHEF_ROLE, msg.sender), "Caller is not masterchef");

        IBoostedMasterChefJoe BJMC = IBoostedMasterChefJoe(BJMC_ADDRESS);

        //Return LP token from BMCJ
        BJMC.withdraw(_pid, _amount);
    ...
    /// @notice Claims veJOE Rewards
    /// @param _amount amount of Joe to Claim
    /// @notice highly sensitive limited to only SteakHut Master Chef
    /// @notice this is to be limited to only masterchef
    function _claimJOE(uint256 _amount) public {
        require(hasRole(MASTERCHEF_ROLE, msg.sender), "Caller is not masterchef");

        IERC20 JoeToken = IERC20(JOE);
        require(JoeToken.balanceOf(address(this)) >= _amount, 'Voter: Reject, Not Enough Joe; Wait.');
        JoeToken.transfer(msg.sender, _amount);
    ...
    /// @notice highly sensitive limited to only SteakHut Master Chef
    function _claimBonusToken(uint256 _amount, address _tokenAddress) public {
        require(hasRole(MASTERCHEF_ROLE, msg.sender), "Caller is not masterchef");
    ...
    function _stakeJOE(uint256 _amount) public {
        require(hasRole(ZAPPER_ROLE, msg.sender), "Caller is not Zapper");

        IERC20 JoeToken = IERC20(JOE);
        IVeJoeStaking VeJoeStaking = IVeJoeStaking(VeJoeStaking_ADDRESS);

        JoeToken.approve(address(VeJoeStaking_ADDRESS), _amount);
    ...
    /// @notice highly sensitive withdraws spare tokens from voter 
    /// @param _tokenAddress token address to remove to multisig owner
    /// Only used in the event of an upgrade to remove JOE or bonus tokens from contract 
    function withdrawInvestablePool(address _tokenAddress) public onlyOwner {

        uint256 amount = IERC20(_tokenAddress).balanceOf(address(this));
        
        (bool sent) =  IERC20(_tokenAddress).transfer(msg.sender, amount);
        require(sent, "Failed to transfer Tokens from Contract to MultiSig");
    }
