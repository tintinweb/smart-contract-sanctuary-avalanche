...
contract MainStakingJoe is Initializable, OwnableUpgradeable {
    ...
    event JoeClaimed(uint256 amount);
    ...
    function setFee(uint256 index, uint256 value) external onlyOwner {
        ...
        require(
            fee.minValue <= value && value <= fee.maxValue,
            "Value not in range"
        );
    ...
    function harvest(address token, bool isUser) public {
        ...
        IMasterJoe(masterJoe).deposit(poolInfo.pid, 0); //stock pour la pool chaque reward, vue ?
    ...
    function sendRewards(
        ...
        for (uint256 i = 0; i < feeInfos.length; i++) {
            ...
            if (feeInfo.isActive) {
                ...
                if (!feeInfo.isJoe) {
                    _approveTokenIfNeeded(joe, xJoe, feeAmount);
                    IxJoe(xJoe).depositFor(feeAmount, address(this));
                    rewardToken = xJoe;
                }
                if (!feeInfo.isAddress) {
                    _approveTokenIfNeeded(rewardToken, feeInfo.to, feeAmount);
                    IBaseRewardPool(feeInfo.to).donateRewards(
                        feeAmount,
                        rewardToken
                    );
                } else {
    ...
    /// @param _token the address of the token to send
    /// @param _rewarder the rewarder that will get the rewards
    function donateTokenRewards(address _token, address _rewarder)
        external
        onlyOwner
    {
        // require(_token != joe, "not authorized");
    ...
    function stakeJoe(uint256 amount) public {
        if (amount > 0) {
    ...
    /// @notice stake all the Joe balance of the contract
    function stakeAllJoe() public {
        stakeJoe(IERC20(joe).balanceOf(address(this)));
    }

    /// @notice Claim the pending veJoe
    function claimVeJoe() public {
        uint256 amount = getStakedJoe();
        if (amount > 0) {
            IJoeStaking(stakingJoe).claim();
        }
        emit JoeClaimed(amount);
    }
...
contract PoolHelperJoe {
    ...
    address public mainStakingJoe;
    address public rewarder;
    address public wavax = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;

    uint256 public pid;
    bool public isWavaxPool;
    ...
    function withdraw(uint256 amount) external _harvest {
        _withdraw(amount);
