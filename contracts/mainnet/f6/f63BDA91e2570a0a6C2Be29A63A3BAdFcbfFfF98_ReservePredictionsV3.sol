...
contract ReservePredictionsV3 {
  ...
  * @param _oracle Sets the address of the oracle used used as pricefeed. Avalanche AVAX/USD price oracle = 0x0a77230d17318075983913bc2145db16c7366156.
  ...
  */
  constructor(address _oracle, uint _duration, address _operator, uint _miniumvalue, address _dummytoken) {
    ...
    uint _price = getLatestPrice(); //returns latest AVAX/USD in the following format: 40345000000 (8 decimals)
  ...
  */
  function placeBet(Option _option, uint256 amount) external {
    ...
    m.users[msg.sender].comission = m.users[msg.sender].comission.add(_commissionStake);
    ...
    m.amountStaked[msg.sender] = m.amountStaked[msg.sender] +_predictionStake;

    if(calculateAllWinnings(msg.sender) == 0){
        users[msg.sender].markettostart = marketCount;
    }
  ...
  */
  function restartMarket() external onlyOperator {
    ...
    uint _pricenew = getLatestPrice(); //returns latest AVAX/USD in the following format: 40345000000 (8 decimals)
 ...
 /*
  * @dev Allows the calling user address to calculate is winnings.
  */
  function calculateAllWinnings(address user) public view returns (uint winnings) { 
    uint winnings;
    for(uint i= users[user].markettostart; i<marketCount; i++){
      Market storage m = markets[i];
      uint winningBet = m.users[user].amountStaked[m.winningOption];
      winnings = winnings + calculateWinnings(i, user);

      if(winningBet != 0 && winnings == winningBet) {
        winnings = winningBet.add(m.users[user].comission);
      }
      if(m.totalBets[m.winningOption] == 0 && m.amountStaked[user] != 0 && winningBet == 0){
          winnings = winnings + m.amountStaked[user];
      }
    }
    return winnings;
  }
  ...
  /*
  * @dev Allows the calling user address to withdraw all his/her winnings of all actual markets.
  * @param _marketId The id of the prediction market instance on which this function is called.
  */
  function withdrawAllWinnings() external {
    for(uint i= users[msg.sender].markettostart; i<marketCount; i++){
      ...
      if(m.totalBets[m.winningOption] == 0 && m.amountStaked[msg.sender] != 0 && winningBet == 0){
          winnings = winnings + m.amountStaked[msg.sender];
          winnings = winnings.add(m.users[msg.sender].comission);
          commissionAmount = commissionAmount.sub(m.users[msg.sender].comission);
      }
    ...
    IERC20(dummytoken).transferFrom(address(this), address(msg.sender), users[msg.sender].totalAmountToClaim);
    users[msg.sender].totalAmountToClaim = 0;
  }

  /**
  * @dev Gets the latest/current asset price (e.g. AVAX/USD) from the configured pricefeed oracle.
  * @return latestPrice Returns the latest/current asset price (e.g. AVAX/USD) from the configured pricefeed oracle.
  */
  ...
  /*
  * @dev Gets the historical asset price (e.g. AVAX/USD) at prediction market screettlement time from the configured pricefeed oracle.
  * @param _expireTime The prediction market closed timestamp, for which an historical asset price should be returned.
  * @return closePrice Returns the historical asset price (e.g. AVAX/USD) at prediction market closed time from the configured pricefeed oracle.
  ...
  */
  ...
  /**
  * @dev Updates the address of the pricefeed oracle (e.g. the ChainLink AVAX/USD pricefeed oracle).
