...
interface PlanetsManagerUpgradeable {
    ...
    function getPlanets() external view  returns (PlanetEntity[] memory);
...
contract DarkForestRewardPool is Ownable {
    ...
    mapping(address => uint256) rewardInfo;
        ...
        onlyOwner 
    {
        if (block.timestamp > lastRewardTime + rewardPeriod)  {
            address[] storage rewardAccounts;

            PlanetsManagerUpgradeable.PlanetEntity[] memory planets = planetsManager.getPlanets();
            for (uint256 i = 0; i < planets.length; i++) {
                PlanetsManagerUpgradeable.PlanetEntity memory planet = planets[i];
                if (planet.exists && planet.rewardMult > tRewardMults[0]) {
                    uint256 threashold = GetThreashold(planet.rewardMult);
                    uint256 lockingLevel = GetLockingLevel(planet.planetValue);
                    address owner = planetsManager.ownerOf(planet.id);
                    if (rewardInfo[owner] == 0)                         
                        rewardAccounts.push(owner);

                    if (rewardInfo[owner] < multipleArray[lockingLevel][threashold])
                        rewardInfo[owner] = multipleArray[lockingLevel][threashold];
                }
            }
