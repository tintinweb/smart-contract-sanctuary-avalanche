...
contract TournamentNative is Ownable, ITournament {
    ...
    function _getTicketPrice() internal view returns (uint256) {
      ...
      function _getCorrectedTicketPrice(uint256 amount) internal view returns (uint256) {
        return ((_getPotSize() - (amount * POT_SHARE / 10000) - (amount * TEAMSHARE / 10000)) * TICKET_SIZE) / 10000;
    }
    ...
    function buy() external payable {
        ...
        Address.sendValue(payable(owner()), ticketPrice * (10000 - POT_SHARE) / 10000);
        finalPotSize = _getPotSize();
        
        address[] memory path = new address[](2);
        path[0] = WAVAX;
        path[1] = address(grave);
        IDEXRouter(router).swapExactAVAXForTokens{value: msg.value}(
            0,
            path,
            address(msg.sender),
            block.timestamp
    ...
    function claimWinner() external {
        ...
        Address.sendValue(payable(winner), finalPotSize);
    }
