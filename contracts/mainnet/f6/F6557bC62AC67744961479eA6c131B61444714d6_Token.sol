...
contract Token is Ownable, ERC20{
    TokenVesting public marketingLockToken;
    TokenVesting public communityLockToken;
    TokenVesting public teamLockToken;
    bool doOnce;
    uint256 public constant MAX_SUPPLY = 300 * 1e6 * 1e18;

    constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_,symbol_){  

        marketingLockToken = new TokenVesting(msg.sender, block.timestamp, 0 days, 300 days);
        ...
        __mint(msg.sender, 30 * 1e6 * 1e18);
        __mint(address(marketingLockToken), 30 * 1e6 * 1e18);
        __mint(address(communityLockToken), 24 * 1e6 * 1e18);
        __mint(address(teamLockToken), 21 * 1e6 * 1e18);
