...
// File: contracts/LEAFPresale.sol

contract LeafPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale LEAF, 0.000000048 USDC
	uint256 TokenPerUSDC = 166 * 10 **18; 
	// Amount of USDC received in presale
	uint256 usdcReceived = 0;

    uint256 HardCap = 50000 * 10 ** 6;
	...
	function getLEAFTokensLeft() external view returns (uint256) {
    ...
    function buy(uint256 _amount) public nonReentrant {
		...
		LEAF.safeTransfer(msg.sender, tokens);
	...
	function withdrawUnsoldLEAF() external onlyOwner {
		uint256 amount = LEAF.balanceOf(address(this)) - totalTokensSold;
		LEAF.safeTransfer(msg.sender, amount);
	}

    function AllBurnLEAF() external onlyOwner {
		LEAF.safeTransfer(burnAddress, LEAF.balanceOf(address(this)));
	}
}