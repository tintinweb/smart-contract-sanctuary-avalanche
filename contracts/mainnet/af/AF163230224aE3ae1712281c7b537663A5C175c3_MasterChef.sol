...
contract MasterChef is Ownable {
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (_to <= bonusEndTime) {
            ...
            return bonusEndTime.sub(_from).mul(BONUS_MULTIPLIER).add(
    ...
    function getTimeMultiplex(uint256 sec_) public pure returns (uint256) {
        ...
        else if (sec_ <= 7 days) return 102;
        else if (sec_ <= 14 days) return 104;
        else if (sec_ <= 357 days) return 202;
        else if (sec_ <= 364 days) return 204;
        else return 204;
    }
