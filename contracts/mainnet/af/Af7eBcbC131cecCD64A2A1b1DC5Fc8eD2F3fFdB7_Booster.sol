    ...
    ReentrancyGuardUpgradeable
{
    ...
    function listExtraRewards(uint256 _pid) external view returns (ExtraRewardsPool[] memory) {
    ...
    function _depositFor(
        ...
        (uint256 ptpClaimed, uint256[] memory extraRewardsClaimed) = depositorProxy
            .deposit(
    ...
    function _distributeIncentives(uint256 amount) private returns (uint256) {
        ...
        if (_ecdLockedIncentive != 0) {
            ...
            if ((block.timestamp - veEcdLastPaid) > 1 days) {
                veEcdLastPaid = block.timestamp;
                ptp.approve(veEcdRewardPool, queuedVeEcdRewards);
        ...
        emit IncentivesDistributed(0, _platform, 0, _ecdLockedIncentive);

        return amount - _ecdLockedIncentive - _platform;
    }
    ...
    function _distributeRewards(
        ...
        ExtraRewardsPool[] memory extraRewardsPoolsCache = extraRewardsPools[_pid];
        if (
            extraRewardAmounts.length != 0
        ) {
            ...
            if(extraRewardAmounts.length != extraRewardsPoolsCache.length) {
                ...
                for(uint256 _rewardId = extraRewardsPoolsCache.length; _rewardId < extraRewardAmounts.length ; ++_rewardId) {
    ...
    function _addExtraRewards(uint256 _pid, address rewarder, uint256 _rewardId) internal {
        IRewarder.PoolInfo memory poolInfo = IRewarder(rewarder).poolInfos(_rewardId);
        ...
        extraRewardsPools[_pid].push(ExtraRewardsPool({
            token: rewardToken,
    ...
    function migrateLegacyExtraRewardPools() external onlyOwner {
        for(uint256 i; i < existingPools.length; ++i) {
            ExtraRewardsPool memory extraPool = legacyExtraRewardsPools[existingPools[i]];
...
interface IRewarder {
    ...
    function poolInfos(uint256 _pid) external view returns (PoolInfo memory);
}
