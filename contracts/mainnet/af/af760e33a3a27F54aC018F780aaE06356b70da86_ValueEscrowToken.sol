...
contract Ownership is IOwnership
{
	...
	function _total(uint256 _when) internal returns (uint256 _value)
	{
		Entry storage _entry = cache_[address(0)][_when];
		if (_entry.cached) return _entry.value;
		_value = IERC20Historical(token_).totalSupply(_when);
		_entry.cached = true;
		_entry.value = _value;
		return _value;
	}
	...
	function _local(address _account, uint256 _when) internal returns (uint256 _value)
	{
		Entry storage _entry = cache_[_account][_when];
		if (_entry.cached) return _entry.value;
		_value = IERC20Historical(token_).balanceOf(_account, _when);
		_entry.cached = true;
		_entry.value = _value;
		return _value;
	}
...
contract CumulativeOwnership is IOwnership
{
	address private immutable token_;
	...
	function totalOwnership(uint256 _when) external view override returns (uint256 _totalOwnership)
	{
		return _totalv(_when) - _totalv(_when - period_);
	}

	function totalOwnershipPersist(uint256 _when) external override returns (uint256 _totalOwnership)
	{
		if (_when >= block.timestamp) return _totalv(_when) - _totalv(_when - period_);
		return _total(_when) - _total(_when - period_);
	}

	function localOwnership(address _account, uint256 _when) external view override returns (uint256 _localOwnership)
	{
		return _localv(_account, _when) - _localv(_account, _when - period_);
	}

	function localOwnershipPersist(address _account, uint256 _when) external override returns (uint256 _localOwnership)
	{
		if (_when >= block.timestamp) return _localv(_account, _when) - _localv(_account, _when - period_);
		return _local(_account, _when) - _local(_account, _when - period_);
	}
	...
	function _totalv(uint256 _when) internal view returns (uint256 _value)
	{
		Entry storage _entry = cache_[address(0)][_when];
		if (_entry.cached) return _entry.value;
		return IERC20HistoricalCumulative(token_).cumulativeTotalSupply(_when);
	}
	...
	function _total(uint256 _when) internal returns (uint256 _value)
	{
		Entry storage _entry = cache_[address(0)][_when];
		if (_entry.cached) return _entry.value;
		_value = IERC20HistoricalCumulative(token_).cumulativeTotalSupply(_when);
		_entry.cached = true;
		_entry.value = _value;
		return _value;
	}
	...
	function _localv(address _account, uint256 _when) internal view returns (uint256 _value)
	{
		Entry storage _entry = cache_[_account][_when];
		if (_entry.cached) return _entry.value;
		return IERC20HistoricalCumulative(token_).cumulativeBalanceOf(_account, _when);
	}
	...
	function _local(address _account, uint256 _when) internal returns (uint256 _value)
	{
		Entry storage _entry = cache_[_account][_when];
		if (_entry.cached) return _entry.value;
		_value = IERC20HistoricalCumulative(token_).cumulativeBalanceOf(_account, _when);
		_entry.cached = true;
		_entry.value = _value;
		return _value;
	}
...
contract RewardDistributor is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	{
		address _escrowOwnership = _escrowCumulative ? address(new CumulativeOwnership(_escrowToken, CLAIM_BASIS)) : address(new Ownership(_escrowToken));
		address _boostOwnership = address(0);
		if (_boostToken != address(0)) {
			_boostOwnership = _boostCumulative ? address(new CumulativeOwnership(_boostToken, CLAIM_BASIS)) : address(new Ownership(_boostToken));
		}
	...
	function _calculateAccruedClaim(address _account, uint256 _firstPeriod, uint256 _lastPeriod) internal view returns (uint256 _amount, uint256 _excess)
	{
		...
		if (boostToken == address(0)) {
			for (uint256 _period = _firstPeriod; _period < _lastPeriod; _period += CLAIM_BASIS) {
				uint256 _local = IOwnership(escrowOwnership_).localOwnership(_account, _period);
				if (_local == 0) continue;
				uint256 _total = IOwnership(escrowOwnership_).totalOwnership(_period);
			...
			for (uint256 _period = _firstPeriod; _period < _lastPeriod; _period += CLAIM_BASIS) {
				uint256 _local = IOwnership(escrowOwnership_).localOwnership(_account, _period);
				uint256 _boostLocal = IOwnership(boostOwnership_).localOwnership(_account, _period);
				if (_local == 0 && _boostLocal == 0) continue;
				uint256 _total = IOwnership(escrowOwnership_).totalOwnership(_period);
				if (_total == 0) continue;
				uint256 _boostTotal = IOwnership(boostOwnership_).totalOwnership(_period);
	...
	function _calculateAccruedClaimPersist(address _account, uint256 _firstPeriod, uint256 _lastPeriod) internal returns (uint256 _amount, uint256 _excess)
	{
		...
		if (boostToken == address(0)) {
			for (uint256 _period = _firstPeriod; _period < _lastPeriod; _period += CLAIM_BASIS) {
				uint256 _local = IOwnership(escrowOwnership_).localOwnershipPersist(_account, _period);
				if (_local == 0) continue;
				uint256 _total = IOwnership(escrowOwnership_).totalOwnershipPersist(_period);
				if (_total == 0) continue;
				_amount += rewardPerPeriod_[_period] * _local / _total;
			}
		} else {
			for (uint256 _period = _firstPeriod; _period < _lastPeriod; _period += CLAIM_BASIS) {
				uint256 _local = IOwnership(escrowOwnership_).localOwnershipPersist(_account, _period);
				uint256 _boostLocal = IOwnership(boostOwnership_).localOwnershipPersist(_account, _period);
				if (_local == 0 && _boostLocal == 0) continue;
				uint256 _total = IOwnership(escrowOwnership_).totalOwnershipPersist(_period);
				if (_total == 0) continue;
				uint256 _boostTotal = IOwnership(boostOwnership_).totalOwnershipPersist(_period);
				if (_boostTotal == 0) continue;
				uint256 _normalizedTotal = 10 * _total * _boostTotal;
				uint256 _isolatedLocal = 10 * _local * _boostTotal;
				uint256 _normalizedLocal = 4 * _local * _boostTotal + 6 * _total * _boostLocal;
