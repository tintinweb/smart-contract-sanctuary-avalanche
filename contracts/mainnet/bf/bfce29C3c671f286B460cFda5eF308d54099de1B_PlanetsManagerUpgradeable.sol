    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    modifier onlyRewardPool() {
        ...
        require(
            ...
            "Planets: Caller is not rewardPool!"
        );
        _;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 len = 0;
        for (uint256 i = 1; i <= _planetCounter.current(); i++) 
        {
            address account = ownerOf(_planets[i].id);
            if (_planets[i].exists 
                && IERC20(universe).balanceOf(account) > 0 
                && _planets[i].rewardMult > 105000)
            {
                Ids[len++] = _planets[i].id;
            }
    ...
    function getPlanetRewardMults() 
        public 
        view 
        returns (uint256[] memory)
    {
        uint256 size = totalSupply();
        uint256[] memory rwMults = new uint256[](size);
        uint256 len = 0;
        for (uint256 i = 1; i <= _planetCounter.current(); i++)
        {
            address account = ownerOf(_planets[i].id);
            if (_planets[i].exists 
                ...
                && _planets[i].rewardMult > 105000)
            {
                rwMults[len++] = _planets[i].rewardMult;
            }
        }
        return rwMults;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 1; i <= _planetCounter.current(); i++)
        {
            ...
            if (_planets[i].exists 
                ...
                && _planets[i].rewardMult > 105000)
            {
                values[len++] = _planets[i].planetValue;
            }
        }
        return values;
    }
