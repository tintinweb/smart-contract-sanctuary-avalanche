...
contract RangoWormhole is IRangoWormhole, BaseInterchainContract {
    ...
    function wormholeSwap(
        ...
        require(router != ETH, "Wormhole router address not set");
        ...
        if (_wormholeRequest._bridgeType == WormholeBridgeType.TRANSFER_WITH_MESSAGE){
            ...
            if (_fromToken == ETH) {
                IWormholeRouter(router).wrapAndTransferETHWithPayload{value: msg.value}(
                ...
                IWormholeRouter(router).transferTokensWithPayload(
            ...
            if (_fromToken == ETH) {
                IWormholeRouter(router).wrapAndTransferETH{value: msg.value}(
                    _wormholeRequest._recipientChain,
                    _wormholeRequest._targetAddress,
                ...
                IWormholeRouter(router).transferTokens(
                    _wormholeRequest._fromAddress,
                    _wormholeRequest._finalInput,
                    _wormholeRequest._recipientChain,
                    _wormholeRequest._targetAddress,
    ...
    function wormholeReceiver(
        ...
        address wormhole_token_bridge_address = address(wormholeRouter);
        ITokenBridge core_bridge = ITokenBridge(wormhole_token_bridge_address);
        BridgeStructs.TransferWithPayload memory transfer = core_bridge.parseTransferWithPayload(payload);
        ...
        (address receivedToken, uint dstAmount, OperationStatus status) = handleDestinationMessage(
                address(uint160(uint256(transfer.fromAddress))),
                transfer.amount,
                m
        );
    ...
    ) external whenNotPaused nonReentrant
    {
        address wormhole_token_bridge_address = address(wormholeRouter);
        ITokenBridge core_bridge = ITokenBridge(wormhole_token_bridge_address);
        core_bridge.completeTransferWithPayload(vaas);
    }
