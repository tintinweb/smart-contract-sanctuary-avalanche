...
import "./ISlizGauge.sol";
import "./IGasPrice.sol";
import "./FeeManager.sol";
import "./ILiveSolidly.sol";

contract KolStrategySolidlyV2 is Initializable, UUPSUpgradeable, FeeManager {
    ...
    ILiveSolidly public constant liveToken = ILiveSolidly(0x14835EB5816701784C21507B7A0FF70905A6f5ca);
    bool public boosted;
    ...
    function initialize() public initializer {
        ...
        keeper = 0xbB8707d7365DD9fA686a7EE8C178d69768c4C4C5;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            if (boosted) {
                liveToken.depositForStrategy(address(gauge), wantBal);
            } else {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            if (boosted) {
                liveToken.withdrawForStrategy(address(gauge), _amount - wantBal);
            } else {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        tokens[0] = output;
        if (boosted) {
            liveToken.getRewardsForStrategy(address(gauge), tokens);
        } else {
    ...
    function balanceOfPool() public view returns (uint256) {
        if (boosted)
            return liveToken.balanceOfStrategy(address(gauge));
        else
    ...
    function rewardsAvailable() public view returns (uint256) {
        if (boosted){
            return liveToken.earnedOfStrategy(address(gauge), output);
        }
        else
    ...
    function panic() public onlyManager {
        pause();
        if (boosted) {
            liveToken.withdrawAllForStrategy(address(gauge));
        } else {
            gauge.withdrawAll();
        }
    ...
    function set_Boosted(bool _value) external onlyOwner {
        require(balanceOf()==0,"cannot change");
        boosted = _value;
        if (boosted) {
            want.approve(address(liveToken), type(uint).max);
        }
    }
    ...
    function updateV2() external onlyOwner {
        gasprice = IGasPrice(0x4e69F60b2CADd3F421d3520801eE40a26E1D450E);
    }
