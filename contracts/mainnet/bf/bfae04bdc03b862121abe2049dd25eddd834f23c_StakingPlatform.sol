...
// File: contracts/IStakingPlatform.sol
...
// File: contracts/StakingPlatform.sol
...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    mapping(address => uint) public _userStakeStartTime;
    mapping(address => uint) private _userLastClaimTime;
    ...
    function deposit(uint amount) external override {
        ...
        if (_userStakeStartTime[_msgSender()] == 0) {
            _userStakeStartTime[_msgSender()] = block.timestamp;
        }

        if(_userLastClaimTime[_msgSender()] == 0) {
            _userLastClaimTime[_msgSender()] = block.timestamp;
        }
    ...
    function withdraw(uint amount) external override {
        require(
            block.timestamp >= (_userStakeStartTime[_msgSender()] + 365 days),
    ...
    function withdrawAll() external override {
        ...
        _userStakeStartTime[_msgSender()] = 0;
    ...
    function _claimRewards() private {
        require((block.timestamp - _userLastClaimTime[_msgSender()]) > 1 days, "Can Claim only After Every 24hrs");
        _updateRewards();
