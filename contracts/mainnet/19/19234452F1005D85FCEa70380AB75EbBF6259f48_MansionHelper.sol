...
contract MansionHelper is Ownable, Manageable {
    MansionManager public manager;
    IERC20 public PLAYMATES;
    DistrictHelper public districtHelper;
    DistrictHelper public citiesHelper;
    ...
    address public TREASURY = 0xE63fb42e89cD67d93bcb6DC5E9388B347E3174be;
    ...
    constructor(address _manager, address _PLAYMATES, address _districtHelper, address _citiesHelper, address _DISTRICT)  {
        manager = MansionManager(_manager);
        DISTRICT = IERC20(_DISTRICT);
        districtHelper = DistrictHelper(_districtHelper);
        citiesHelper = DistrictHelper(_citiesHelper);
        PLAYMATES = IERC20(_PLAYMATES);
    ...
    function setNewCitiesHelper(address _cities) external onlyOwner {
        citiesHelper = DistrictHelper(_cities);
    }
   ...
   function setNodeRatios(uint16 _poolFee, uint16 _liquidityFee) external onlyOwner {
    ...
    function createNodesWithRewards(string memory name, uint64[] calldata _nodes, uint256 amount, bool useBalance) public {
        ...
        else require(useBalance && rewardAmount + PLAYMATES.balanceOf(sender) >= nodePrice * amount,"HELPER: You don't have enough reward to cash out");
        ...
        if (rewardAmount > nodePrice * amount) {
            ...
            uint256 newFeeAmount = remainingRewards.mul(newTax).div(100);
        ...
        } else if (useBalance && rewardAmount + PLAYMATES.balanceOf(sender) >= nodePrice * amount) {
            uint256 balanceToTransfer = nodePrice * amount - rewardAmount;
            PLAYMATES.transferFrom(_msgSender(), address(this),  balanceToTransfer);
            if ((PLAYMATES.balanceOf(address(this)) > swapThreshold)) {
                uint256 contractTokenBalance = PLAYMATES.balanceOf(address(this));
                contractSwap(contractTokenBalance);
            }
    ...
    function claimUtility(uint64[] calldata _nodes, address whereTo, uint256 neededAmount, address excessAmountReceiver) onlyManager public {
        uint256 rewardAmount = 0;
        for (uint256 i = 0; i < _nodes.length; i++) {
            rewardAmount = rewardAmount + manager.claim(excessAmountReceiver, _nodes[i]);
        }
        require(rewardAmount > 0,"HELPER: You don't have enough reward to cash out");
        uint256 remainingRewards = rewardAmount - neededAmount;
        pool.pay(excessAmountReceiver, remainingRewards);
        pool.pay(whereTo, neededAmount);
    }

    function claimMansionsAndDistricts(uint64[] calldata _mansions, uint64[] calldata _districts) public {
        claimAll(_mansions);
        claimDistricts(_districts);
    }
    ...
    function claimCities(uint64[] calldata _nodes) public {
        address sender = _msgSender();
        require(sender != address(0), "HELPER: creation from the zero address");
        require(blacklist[sender] == false, "HELPER: You are blacklisted");
        uint256 rewardAmount = 0;
        for (uint256 i = 0; i < _nodes.length; i++) {
            rewardAmount = rewardAmount + citiesHelper.claim(_msgSender(), _nodes[i]);
        }
        require(rewardAmount > 0,"HELPER: You don't have enough reward to cash out");
        uint256 feeAmount = rewardAmount.mul(getClaimFee(sender)).div(100);
        require(feeAmount > 0, "Helper: Error");
