...
contract SnowsightManager
{
    ...
    constructor()
    {
        ...
        tierSettings[uint(paymentTierTypes.STANDARD)].ratePerSecond = 2604166666666; 
        tierSettings[uint(paymentTierTypes.PRIORITY)].ratePerSecond = 9645061728395;
    ...
    function pay(uint tier) external payable onlyValidTier(tier)
    {
        ...
        else
        {
            ...
            emit Debug(tierSettings[tier].ratePerSecond);
            payments[msg.sender][tier].endTimestamp = block.timestamp + (msg.value / tierSettings[tier].ratePerSecond);
        }
