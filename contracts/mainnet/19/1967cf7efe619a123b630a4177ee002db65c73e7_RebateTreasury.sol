...
contract RebateTreasury is Ownable {
    ...
    constructor(address _dai, address globa, address globaOracle, address treasury) {
        DAI = _dai;
        Globa = IERC20(globa);
        GlobaOracle = IOracle(globaOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Globa at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 globaAmount = getGlobaReturn(token, amount);
        require(globaAmount <= Globa.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient globa balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + globaAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += globaAmount;
    }

    // Claim available Globa rewards from bonding
    ...
    function setGloba(address globa) external onlyOwner {
        Globa = IERC20(globa);
    }

    // Set Globa oracle

    function setGlobaOracle(address oracle) external onlyOwner {
        GlobaOracle = IOracle(oracle);
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getGlobaPrice() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Globa.transfer(account, claimable);
    }
    ...
    // Calculate Globa return of bonding amount of token

    function getGlobaReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 globaPrice = getGlobaPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / globaPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 globaPrice = getGlobaPrice();
        if (globaPrice < 1e18) return 0; 

        uint256 globaPremium = globaPrice * DENOMINATOR / 1e18 - DENOMINATOR; 
        if (globaPremium < bondThreshold) return 0;
        if (globaPremium <= secondaryThreshold) {
            return (globaPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (globaPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get GLOBA price from Oracle

    function getGlobaPrice() public view returns (uint256) {
        return GlobaOracle.consult(address(Globa), 1e18);
    }
    ...
    // Get claimable vested Globa for account

    function claimableGloba(address account) external view returns (uint256) {
