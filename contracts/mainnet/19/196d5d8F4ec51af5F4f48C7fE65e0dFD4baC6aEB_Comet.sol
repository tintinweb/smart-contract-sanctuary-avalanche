...
{
    address public immutable cometsManager;

    modifier onlyCometsManager() {
        address sender = _msgSender();
        require(
            sender == address(cometsManager),
            "Implementations: Not CometsManager"
        );
        _;
    }

    constructor(address _cometsManager) ERC20("Asteroid", "AST") {
        require(
            _cometsManager != address(0),
            "Implementations: CometsManager is not set"
        );
        cometsManager = _cometsManager;
        ...
        onlyCometsManager
    {
        ...
        // effectively protecting CometsManager from suspicious addresses
        ...
        onlyCometsManager
    {
        require(
            ...
            "Comet: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyCometsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "Comet: LiquidityPoolManager is not set"
        );
