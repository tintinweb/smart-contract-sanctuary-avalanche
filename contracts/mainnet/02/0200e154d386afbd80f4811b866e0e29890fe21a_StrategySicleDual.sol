...
// File contracts/BIFI/interfaces/traderjoe/IMasterChef.sol
...
// File contracts/BIFI/strategies/TraderJoe/StrategySicleDual.sol
...
contract StrategySicleDual is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused payable {
        ...
        if (wantBal > 0) {
            IPopsMasterChef(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external payable {
        ...
        if (wantBal < _amount) {
            IPopsMasterChef(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal {
        IPopsMasterChef(chef).deposit(poolId, 0);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = IPopsMasterChef(chef).userInfo(poolId, address(this));
        return _amount;
    }

    function rewardsAvailable() public view returns (uint256) {
        (uint256 outputBal) = IPopsMasterChef(chef).pendingShare(poolId, address(this));
        return outputBal;
    }
    ...
    function retireStrat() external {
        ...
        IPopsMasterChef(chef).emergencyWithdraw(poolId);
