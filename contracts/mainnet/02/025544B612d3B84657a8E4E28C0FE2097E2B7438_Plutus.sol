...
contract NODERewardManagement {
    ...
    function getNodesOfUser(address account)
        external
        view
...
contract Plutus is ERC20, Ownable, PaymentSplitter {
    ...
    uint256[2] public sellTax= [2, 2];
    uint256 public launchDate;
    uint256 public limitDuration = 7 * 86400;
    uint256 public limitAmount = 10 ether;
    ...
    function userExceedsLimit(address account, uint256 amount) public view returns (bool) {
        if (block.timestamp < (limitDuration + launchDate)) {
            require(
                nodeRewardManager._isNodeOwner(account),
                "CONSTR: Not a node owner"
            );
        }
        uint256 userWithdrawal = usersWithdrawals[account];
        return userWithdrawal.add(amount) > limitAmount;
    }

    function checkWithdrawable(address account) public view returns (uint256) {
        uint256 withdrawn = usersWithdrawals[account];
        return limitAmount.sub(withdrawn);
    }

    function updateLimitDuration(uint256 time) external onlyOwner {
        limitDuration = time;
    }
     ...
     function _transfer(
        ...
        uint256 fee = amount.div(sellTax[1]).mul(sellTax[0]);
        uint256 taxedAmount = amount - fee;
        super._transfer(from, address(this), fee);
        super._transfer(from, to, taxedAmount);
    }

    function updateSellTax(uint256[2] memory values) external onlyOwner {
        require(values.length == 2, "2 values required");
        sellTax = values;
    }
