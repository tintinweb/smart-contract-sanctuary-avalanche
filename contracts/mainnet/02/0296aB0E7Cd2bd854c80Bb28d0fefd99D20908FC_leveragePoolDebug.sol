...
// File: contracts/interface/IWAVAX.sol
...
// File: contracts/interface/ISwapHelper.sol
...
// File: contracts/leveragePool/leverageData.sol
...
abstract contract leverageData is ImportOracle,Halt,ReentrancyGuard{
    ...
    event Liquidate(address indexed sender,address indexed account,address lending,uint256 amount,address underlying,uint256 _payback);
        
}

// File: contracts/leveragePool/leveragePool.sol
...
contract leveragePool is leverageData {
    ...
    function getUnderlingLeft(address account) external view returns (uint256){
        ...
        (uint256 underlyingPrice,uint256 loadPrice) = getPrices();
        uint256 allUnderlying = userVault[userID].mul(underlyingPrice);
        uint256 loadUSD = loan.mul(loadPrice).mul(collateralRate)/calDecimals;
    ...
    **/
    ...
    function getPrices()internal view returns(uint256,uint256){
        ...
        (bool tol1,uint256 loadPrice) = oracle.getPriceInfo(address(lendingToken));
         require(tol0 && tol1,"Oracle price is abnormal!");
         return (underlyingPrice,loadPrice);
    }
    function canLiquidate(address account) external view returns (bool){
        ...
        (uint256 underlyingPrice,uint256 loadPrice) = getPrices();
        uint256 allUnderlying = userVault[userID].mul(underlyingPrice);
        return loan.mul(loadPrice).mul(collateralRate)/calDecimals>allUnderlying;
    }
    ...
    function liquidateTest(uint256 allUnderlying,uint256 loan,uint256 amount) 
        ...
        (uint256 underlyingPrice,uint256 loadPrice) = getPrices();
        bool bLiquidate = loan.mul(loadPrice).mul(collateralRate)/calDecimals<allUnderlying.mul(underlyingPrice);
        if (bLiquidate){
            uint256 penalty;
            uint256 repayLoan;
            (penalty,repayLoan,amount) = liquidateAmount(amount,loan);
            amount = amount.mul(loadPrice).mul(calDecimals.add(liquidationReward))/underlyingPrice/calDecimals;
    ...
    function liquidate(address account,uint256 amount) notZeroAddress(account) nonReentrant external{
        bytes32 userID = getUserVaultID(account);
        ...
        emit Liquidate(msg.sender,account,address(lendingToken),penalty.add(repayLoan),underlying,_payback);  
    }
...
// File: contracts/debug/leveragePoolDebug.sol
