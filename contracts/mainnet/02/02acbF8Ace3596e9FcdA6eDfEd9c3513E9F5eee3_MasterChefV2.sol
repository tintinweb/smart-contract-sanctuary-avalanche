...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accPopsPerShare) - user.rewardDebt
    ///
    ...
    ///     POPS rewards each pool gets.
    /// `accPopsPerShare` Accumulated POPS per share, times 1e12.
    ...
    ///     the POPS rewards to all the Swapsicle products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accPopsPerShare;
    ...
    /// @notice Address of POPS token.
    IERC20 public immutable POPS;
    ...
    uint256 public totalPopsPerBlock = 1e17;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accPopsPerShare);
    ...
    event UpdateTotalPopsPerBlock(uint256 amount);

    /// @param _POPS The POPS token contract address.
    constructor(IERC20 _POPS) {
        POPS = _POPS;
    }
    ...
    /// "Special" pools are only for POPS distributions within Swapsicle products.
    ...
    function add(
        ...
        // stake POPS token will cause staked token and reward token mixed up,
    ...
    /// @notice Update the given pool's POPS allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending POPS rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingPops(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 amountPerShare = pool.accPopsPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);
            uint256 rewards = multiplier.mul(popsPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
        ...
        return boostedAmount.mul(amountPerShare).div(ACC_REWARDS_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update pops reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of POPS per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function popsPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = totalPopsPerBlock.mul(regularFarmsRate).div(REWARDS_RATE_TOTAL_PRECISION);
        } else {
            amount = totalPopsPerBlock.mul(specialFarmsRate).div(REWARDS_RATE_TOTAL_PRECISION);
        }
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                uint256 popsRewards = multiplier.mul(popsPerBlock(pool.isRegular))
                    .mul(pool.allocPoint).div(totalAllocPoint);
                pool.accPopsPerShare = pool.accPopsPerShare.add((popsRewards.mul(ACC_REWARDS_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accPopsPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = _calculateDebt(user.amount, multiplier, pool.accPopsPerShare);
        poolInfo[_pid] = pool;
    ...
    /// @param _amountPerShare PoolInfo accPopsPerShare
    ...
    /// @notice Update totalPopsPerBlock (amount of POPS to distribute as rewards).
    /// @param _totalPopsPerBlock The new total amount of POPS per block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateTotalPopsPerBlock(
      uint256 _totalPopsPerBlock,
      bool _withUpdate
    ) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        ...
        totalPopsPerBlock = _totalPopsPerBlock;
        emit UpdateTotalPopsPerBlock(_totalPopsPerBlock);
    }
    ...
    function updateBoostMultiplier(
        ...
        user.rewardDebt = _calculateDebt(user.amount, _newMultiplier, pool.accPopsPerShare);
    ...
    /// @notice Settles & distributes the pending POPS rewards for a given user.
    /// @param _user The user address.
    /// @param _pid The pool id.
    ...
    function _settlePendingRewards(
        ...
        // SafeTransfer POPS
        _safeTransfer(_user, popsToSend);

    }

    /// @notice Safe Transfer POPS.
    /// @param _to The receiver address.
    /// @param _amount The amount of POPS to transfer.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = POPS.balanceOf(address(this));
            // Check whether MCV2 has enough POPS. If not, fail with an error.
            require(balance >= _amount, "_safeTransfer: Insufficient");
            POPS.safeTransfer(_to, _amount);
        }
    }
}