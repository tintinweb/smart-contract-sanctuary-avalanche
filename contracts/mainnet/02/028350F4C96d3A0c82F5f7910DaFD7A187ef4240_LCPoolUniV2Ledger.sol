...
contract LCPoolUniV2Ledger is Ownable {
  ...
  function updateInfo(address acc, uint256 tId, uint256 bId, uint256 liquidity, uint256 reward, uint256 rewardAfter, uint256 exLp, bool increase) public onlyManager {
    ...
    if (poolInfoAll[tId].length > 0) {
      ...
      ivar[2] -= prevRTR.prevReward;
      ivar[5] = prevRTR.rtr;
    }
    ivar[5] += ivar[0] > 0 ? ivar[2] * MULTIPLIER / ivar[0] : 0;
    
    (ivar[3], ivar[4]) = getSingleReward(acc, tId, bId, reward);
    ...
    RewardTVLRate memory tmpRTR = RewardTVLRate({
      ...
      tvl: increase ? ivar[0] + liquidity + ivar[3] : ivar[0] - liquidity,
      rtr: ivar[5],
    ...
    if (increase) {
      userInfo[acc][tId][bId].amount += liquidity + ivar[3];
  ...
  function getSingleReward(address acc, uint256 tId, uint256 bId, uint256 currentReward) public view returns(uint256, uint256) {
    ...
    if (jvar[2] > 0) {
      ...
      uint256 index = t0;
      while (index < tn) {
        jvar[3] = jvar[2] * (poolInfoAll[tId][index].rtr - poolInfoAll[tId][t0].rtr) / MULTIPLIER;
      ...
      jvar[1] += poolInfoAll[tId][tn-1].tvl > 0 ? jvar[2] * (currentReward - poolInfoAll[tId][tn-1].prevReward) / poolInfoAll[tId][tn-1].tvl : 0;
    }
    ...
    returns(uint256[] memory, uint256[] memory)
  {
    ...
    for (uint256 x = 0; x < poolId.length; x ++) {
      ...
      for (uint256 y = 0; y < bLen; y ++) {
        (extraLp[x*bLen + y], reward[x*bLen + y]) = getSingleReward(account, poolId[x], basketIds[y], currentReward);
      }
    }
