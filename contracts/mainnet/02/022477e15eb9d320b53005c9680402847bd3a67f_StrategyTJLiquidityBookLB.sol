...
contract StrategyTJLiquidityBookLB is StratManager, FeeManager {
        ...
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        for (uint256 i; i < liquidityMinted.length; i++) {
            ...
            /// @dev ->> (check rounding err below)
            currentLiquidities[_id] = liquidities[i] + currentLiquidities[_id];
        }
    ...
    function collectRewards(
        ...
        for (uint256 i; i < liquidities.length; ++i) {
            ...
            ///@dev this needs to be checked for overflow
            ...
            console.log(feeGrowthInside[i], "<- fee growth");
