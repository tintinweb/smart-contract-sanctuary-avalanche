...
contract ProtocolUniswapV2 is DSMath, ProtocolWETH {
    ...
    event PathViaEth(address token0, address token1, address token2);
    ...
    function buyToken(
        ...
        address[] memory paths = getPathsViaETH(
            address(_buyAddr),
            address(_sellAddr)
        );
    ...
    function sellToken(
        ...
        if (paths.length > 2) {
            emit PathViaEth(paths[0], paths[1], paths[2]);
        }
    ...
    {
        uint256[] memory amts = IUniswapV2Router02(router).getAmountsOut(
            sellAmt,
            paths
        );
        buyAmt = amts[1];
    ...
    {
        uint256[] memory amts = IUniswapV2Router02(router).getAmountsIn(
            buyAmt,
            paths
        );
        sellAmt = amts[0];
    ...
    function checkPair(address[] memory paths) internal view {
        if (paths.length == 2) {
            ...
            pair = IUniswapV2Factory(IUniswapV2Router02(router).factory())
                .getPair(paths[1], paths[2]);
            require(pair != address(0), "No-exchange-address");
        }
    }

    function getPathsViaETH(address buyAddr, address sellAddr)
        internal
        view
        returns (address[] memory paths)
    {
        if (sellAddr == wethAddr || buyAddr == wethAddr) {
            paths = new address[](2);
            ...
            paths = new address[](3);
            paths[0] = address(sellAddr);
            paths[1] = address(wethAddr);
            paths[2] = address(buyAddr);
        }
    }
        ...
        )
    {
        ...
        address exchangeAddr = IUniswapV2Factory(
            IUniswapV2Router02(router).factory()
