...
contract OnisRetribution is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer {
    ...
    // CONSTANTS
    uint public constant MAX_SUPPLY = 445;
    uint public constant START_ID = 444;

    string public baseURI;
    // VARIABLES
    bool public whitelistedOnly;
    bool public rewardMintEnabled;
    uint public rewardMintThreshold = 3;
    uint public maxSupply = MAX_SUPPLY;
    uint public maxPerTx = 5;
    uint public maxPerPerson = 889;
    uint public price = 500000000000000000;
    address public royaltyAddress = 0x584612eC4C63c1B58DB8b1FF48B50961FE94DC42;
    ...
    constructor(
        ...
        uint[] memory _fees,
        address _lzEndpoint
    ) ERC721("Onis Retribution", "ONIS") ClampedRandomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            tempCount += 1;
            if (rewardMintEnabled && tempCount % rewardMintThreshold == 0) rewardUser();
    ...
    function rewardUser() internal {
        require(rewarderContract != address(0), "Error: Invalid rewarder contract");
        IRewarder rewarder = IRewarder(rewarderContract);
        rewarder.rewardUser(1, msg.sender);
    }
    ...
    function setRewarderContract(address _rewarderContract) external onlyOwner {
        if(_rewarderContract == address(0)) rewardMintEnabled = false;
        else rewardMintEnabled = true;
        rewarderContract = _rewarderContract;
    }

    function setRewardMintThreshold(uint _rewardMintThreshold) external onlyOwner {
        rewardMintThreshold = _rewardMintThreshold;
    }
...
pragma solidity ^0.8.10;
interface IRewarder {
    function rewardUser(uint256 quantity, address to) external;
}
