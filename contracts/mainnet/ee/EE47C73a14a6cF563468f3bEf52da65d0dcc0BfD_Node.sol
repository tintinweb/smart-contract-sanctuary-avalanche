...
contract Node {
    ...
    uint256 public nodePrice = 0.8 ether;
    struct User {
    ...
    uint256 public rewardPerSec = nodePrice.mul(dailyRewardP).div(1000).div(24 * 3600);
    ...
    uint256 public claimFeeBelow = 30;
    uint256 public claimFeeAbove = 10;


    constructor(uint256 _startTime, address _gldm, address _oracle) public {
        gldm = IERC20(_gldm);
        oracle = _oracle;
    ...
    function setDailyRewardPercent( uint256 _p) external onlyDev {
        dailyRewardP = _p;
        rewardPerSec = nodePrice.mul(dailyRewardP).div(1000).div(24 * 3600);
    }
    ...
    function setTierValue(uint256 _nodePrice) external onlyDev {
    ...
    function setNodes(address _user, uint256 _node) external onlyDev {
        nodes[_user] = _node;
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        if ( isWhitelist )
            require(whitelist[_sender], "Not in whitelist");
        require(totalNodes < nodesLimit, "Node count exceeds");
        ...
        uint256 tierPrice = nodePrice.mul(numNodes);
        
        require(gldm.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(gldm.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        ...
        nodes[_sender] = nodes[_sender].add(numNodes);
        totalNodes = totalNodes.add(numNodes);
    ...
    function compound() public {
      ...
      require(rewardsPending >= nodePrice, "Not enough to compound");  
      uint256 numPossible = rewardsPending.div(nodePrice);
      claim();
    ...
    function getGLDMPrice() public view returns (uint256 grapePrice) {
        try IOracle(oracle).consult(address(gldm), 1e18) returns (uint144 price) {
