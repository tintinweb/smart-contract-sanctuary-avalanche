...
import "./PizzeriaRobotV4.sol";

interface IPizzeriaProgressionV4_14 {
...
contract PizzeriaV5_11 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    // Constants
    ...
    IPizzeriaProgressionV4_14 public pizzeriaProgression;
    IMafia_14 public mafia;
    ...
    function initialize(Chef _chef, Upgrade _upgrade, Pizza _pizza, Soda _soda, address _freezerAddress, address _pizzeriaProgression) public initializer {
        ...
        pizzeriaProgression = IPizzeriaProgressionV4_14(_pizzeriaProgression);
    ...
    function getFatiguePerMinuteWithModifier(address _owner) public view returns (uint256) {
        ...
        uint256 levelBonus;
        uint256 level = pizzeriaProgression.getLevel(_owner);
        if(level >= 21 && level <= 25){
            levelBonus += (level - 20) * 3;
        }
        else if(level > 25){
            levelBonus += 15 + (level - 25) * 5;
        }
        return fatiguePerMinute[_owner]*fatigueSkillModifier*(100 - levelBonus)*(100-pizzeriaRobot.getNuclearCoreBonus(_owner))/1000000;
    }
    ...
    function getTotalPPM(address _owner) public view returns (uint256) {
        return totalPPM[_owner]- 2*_getMasterChefNumber(_owner) + pizzeriaRobot.getRobotRPD(_owner);
    }
    ...
    function resetFatigue() public {
        ...
        uint256 costToReset = getTotalPPM(_owner) * sodaResetCost * pizzeriaProgression.getMasterChefSkillModifier(_owner, masterChefNumber) * (100 + masterChefNumber * masterChefMultiplicationBonus) * (100 + pizzeriaRobot.getProductionBonus(_owner))/1000000;

        require(soda.balanceOf(_owner) >= costToReset, "not enough SODA");

        soda.transferFrom(address(_owner), treasuryWallet, costToReset);
    ...
    function _taxUnstake(address _owner) internal { 
    ...
    function unstakeChefsAndUpgrades(uint256[] calldata _chefIds, uint256[] calldata _upgradeIds) public {
        ...
        require(numberOfStaked[owner][0] + numberOfStaked[owner][1] + pizzeriaRobot.getRobotSlots(owner) - _chefIds.length >= ownedUpgradeStakesBalance[owner] - _upgradeIds.length, "Needs at least chef for each tool");
    ...
    function stakeMany(uint256[] calldata _chefIds, uint256[] calldata _upgradeIds) public {
        ...
        // Check 1:1 correspondency between chef and upgrade
        require(chefsAfterStaking + pizzeriaRobot.getRobotSlots(owner) >= ownedUpgradeStakesBalance[owner] + _upgradeIds.length, "Needs at least chef for each tool");
    ...
    function energyDrink(uint256 _qty) public {
        address _owner = msg.sender;
        require(_qty > 0 && _qty <= 1 + pizzeriaRobot.getEnergyDrinkBonus(_owner), "invalid qty");
        uint256 masterChefNumber = _getMasterChefNumber(_owner);

        uint256 ppm = getTotalPPM(_owner);
        uint256 costToReset = _qty * ppm * energyDrinkCost;
        costToReset = costToReset * pizzeriaProgression.getMasterChefSkillModifier(_owner, masterChefNumber) * (100 + masterChefNumber * masterChefMultiplicationBonus) * (100 + pizzeriaRobot.getProductionBonus(_owner))/1000000;
        costToReset = costToReset * (100 - pizzeriaRobot.getEnergyDrinkCostReduction(_owner))/100;
        require(pizza.balanceOf(_owner) >= costToReset, "not enough PIZZA");

        pizza.transferFrom(address(_owner), treasuryWallet, costToReset);

        _updateState(_owner);

        endTimeEnergyBoost[_owner] = block.timestamp + 24*60*60*_qty;
        energyBoostForOwner[_owner] = ppm * (energyDrinkModifier + pizzeriaProgression.getPizzaStorage(_owner));
    }
    ...
    function roniAccruedCalculation(uint256 _startTime, uint256 _endTime, uint256 _ppm, uint256 _fatigue, uint256 _fatiguePerMinute, uint256 _yieldPPS, uint256 _energyDrinkBonus, uint256 _endTimeForBoost, uint256 _energyDrinkBonusOld) public pure returns (uint256) {
        ...
        if(_endTimeForBoost > _startTime) {
            uint256 _deltaTime2;
            uint256 baseBonus = _ppm * _energyDrinkBonusOld;
            if(_energyDrinkBonus > baseBonus || _energyDrinkBonus == 0){
                _energyDrinkBonus = baseBonus;
            }
            if(_endTimeForBoost > _endTime) {
                _deltaTime2 = _endTime - _startTime;
            } else {
                _deltaTime2 = _endTimeForBoost - _startTime;
            }
            uint256 a2 = _deltaTime2 * _energyDrinkBonus * _yieldPPS * (MAX_FATIGUE - _fatigue) / (100 * MAX_FATIGUE);
            uint256 b2 = _deltaTime2 * _deltaTime2 * _energyDrinkBonus * _yieldPPS * _fatiguePerMinute / (2 * 60 * 100 * MAX_FATIGUE);
            result += (a2 - b2);
        }
    ...
    function getRoniAccrued(address _owner) public view returns (uint256) {
        ...
        uint256 energyDrinkBonus = energyBoostForOwner[_owner];
        uint256 energyDrinkBonusOld = energyDrinkModifier + pizzeriaProgression.getPizzaStorage(_owner);
        uint256 endTimeForBoost = endTimeEnergyBoost[_owner];

        uint256 fatiguePerMinuteWithMod = getFatiguePerMinuteWithModifier(_owner);

        uint256 newPizzaAmount = roniAccruedCalculation(startTimeForClaiming, endTimestamp, ppm, fatigueLastUpdate, fatiguePerMinuteWithMod, roniPPS, energyDrinkBonus, endTimeForBoost, energyDrinkBonusOld);

        newPizzaAmount = newPizzaAmount * pizzeriaProgression.getMasterChefSkillModifier(_owner, masterChefNumber) * (100 + masterChefNumber * masterChefMultiplicationBonus);
        newPizzaAmount = newPizzaAmount * (100 + pizzeriaRobot.getProductionBonus(_owner))/1000000;
        return pizzeriaRoni[_owner] + newPizzaAmount;
    }

    function _claimRoni(address _owner) internal {
        require(numberOfStaked[_owner][0] + numberOfStaked[_owner][1] + pizzeriaRobot.getRobotSlots(_owner) >= ownedUpgradeStakesBalance[_owner], "Invalid number of chefs");
    ...
    PizzeriaRobotV4 public pizzeriaRobot;

    function unstakeRobots(uint256[] calldata _robotIds, uint256[] calldata _assistantsIds) external {
        address owner = msg.sender;
        require(pizzeriaRobot.getRobotSlots(owner) >= _robotIds.length, "Invalid number of robots");
        require(numberOfStaked[owner][0] + numberOfStaked[owner][1] + pizzeriaRobot.getRobotSlots(owner) - _robotIds.length >= ownedUpgradeStakesBalance[owner], "Needs at least chef for each tool");
        _taxUnstake(owner);
        pizzeriaRobot.unstakeRobots(owner, _robotIds, _assistantsIds);
        _updatefatiguePerMinute(owner);
    }

    function stakeRobots(uint256[] calldata _robotIds, uint256[] calldata _assistantsIds) external {
        address owner = msg.sender;
        _updateState(owner);
        pizzeriaRobot.stakeRobots(owner, _robotIds, _assistantsIds);
        _updatefatiguePerMinute(owner);
    }

    mapping(address => uint256) public energyBoostForOwner; // address => bonus for energy drink
...
import "./PizzaFactorySkillsV3.sol";

contract PizzeriaRobotV4 is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable
{
    ...
    mapping(uint256 => StakedRobotInfo) public stakedRobot; // tokenId => owner
    ...
    // Counting for bonus
    mapping(address => uint256[8]) public numberOfStaked; // address => [number of iron chefs, number of ultra iron chefs, number of nuclear cores, number of extra arms, number of energy drink, number of high quality parts, number of small pepperoni robots, number of big pepperoni robots]
    mapping(address => uint256) public totalRPD; // address => RPD
    ...
    PizzaFactorySkillsV3 public factorySkills;

    function initialize(PizzaRobot _pizzaRobot, address _pizzeriaAddress, address _treasuryWallet) public initializer {
    ...
    function getNuclearCoreBonus(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = numberOfStaked[_owner][2]*8;
        if(totalBonus > 80){
            totalBonus = 80;
        }
    }

    function getRobotSlots(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = numberOfStaked[_owner][0] + numberOfStaked[_owner][1] + 2*numberOfStaked[_owner][3];
    }

    function getProductionBonus(address _owner) public view returns (uint256 totalBonus) {
        if(numberOfStaked[_owner][5] <= 10){
            totalBonus = 2*numberOfStaked[_owner][5] + 3*numberOfStaked[_owner][0] + 8*numberOfStaked[_owner][1];
        } else {
            totalBonus = 20 + 3*numberOfStaked[_owner][0] + 8*numberOfStaked[_owner][1];
        }
    }

    function getEnergyDrinkBonus(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = numberOfStaked[_owner][4];
    }

    function getEnergyDrinkCostReduction(address _owner) public view returns (uint256 totalBonus) {
        if(factorySkills.hasSkill(_owner,25)){
            totalBonus = numberOfStaked[_owner][4]*3;
        } 
    }

    function getRobotRPD(address _owner) public view returns (uint256 totalSPD) {
        return totalRPD[_owner];
    }

    function _unstakeRobots(address owner, uint256[] calldata _robotIds, uint256[] calldata _assistantsIds) internal {
        uint256 robotLength = _robotIds.length;
        uint256 assistantsLength = _assistantsIds.length;

        for (uint256 i = 0; i < robotLength; i++) {
            uint256 robotId = _robotIds[i];
            uint256 rtype = stakedRobot[robotId].robotType;
            uint256 utype = stakedRobot[robotId].robotUpgradeType;

            require(stakedRobot[robotId].owner == owner, "You don't own this token");
            require(rtype == 1 || rtype == 2, "Invalid robot type");
            
            totalRPD[owner]-=2;

            if(rtype == 1){
                numberOfStaked[owner][0]--; // iron chefs
            }
            if(rtype == 2){
                numberOfStaked[owner][1]--; // ultra iron chefs
            }
            if(utype == 1 || utype == 3){
            numberOfStaked[owner][2]--; //nuclear cores
            }
            if(utype == 2 || utype == 3){
                numberOfStaked[owner][3]--; //extra arms
            }
          
            _removeRobotFromPizzeria(robotId, owner);

            pizzaRobot.transferFrom(address(this), owner, robotId);
        }
        for (uint256 i = 0; i < assistantsLength; i++) {
            uint256 robotId = _assistantsIds[i];
            uint256 rtype = stakedRobot[robotId].robotType;
            uint256 utype = stakedRobot[robotId].robotUpgradeType;

            require(stakedRobot[robotId].owner == owner, "You don't own this token");
            require(rtype == 3, "Invalid robot type");

            if(utype == 0 || utype == 1 || utype == 2 || utype == 3){
                totalRPD[owner]-=1; // small
                numberOfStaked[owner][6]--;
            }
            if(utype == 4 || utype == 5 || utype == 6 || utype == 7){
                totalRPD[owner]-=4; // big
                numberOfStaked[owner][7]--;
            }

            if(utype == 1 || utype == 3 || utype == 5 || utype == 7){
                numberOfStaked[owner][4]--; // energy drink
            }
            if(utype == 2 || utype == 3 || utype == 6 || utype == 7){
                numberOfStaked[owner][5]--; // high quality parts
            }
    ...
    function unstakeRobots(address owner, uint256[] calldata _robotIds, uint256[] calldata _assistantsIds) public {
        require(_msgSender() == pizzeriaAddress, "msgsender does not have permission");
        _unstakeRobots(owner, _robotIds,  _assistantsIds);
    }

    // Need to approve chef and robot first
    function stakeRobots(address owner, uint256[] calldata _robotIds, uint256[] calldata _assistantsIds) public {
        require(_msgSender() == pizzeriaAddress, "msgsender does not have permission");
        ...
        uint256 maxAssistants = 10;
        if(factorySkills.hasSkill(owner,26)){
            maxAssistants = 15;
        }
        if(factorySkills.hasSkill(owner,27)){
            maxAssistants = 20;
        }

        require(numberOfStaked[owner][6]+numberOfStaked[owner][7]+assistantsLength<=maxAssistants, 'invalid number of assistants');

        for (uint256 i = 0; i < robotLength; i++) {
            uint256 robotId = _robotIds[i];
            uint256 rtype = pizzaRobot.getType(robotId);
            uint256 utype = pizzaRobot.getUpgradeType(robotId);

            require(
                pizzaRobot.ownerOf(robotId) == owner,
            ...
            _addRobotToPizzeria(robotId, owner, rtype, utype, pizzaRobot.getOriginalChefId(robotId));
        ...
        for (uint256 i = 0; i < assistantsLength; i++) {
            ...
            _addRobotToPizzeria(robotId, owner, rtype, utype, 0);

            pizzaRobot.transferFrom(owner, address(this), robotId);
        }
    }

    function _addRobotToPizzeria(uint256 _tokenId, address _owner, uint256 robotType, uint256 upgradeType, uint256 originalChefId) internal {
        stakedRobot[_tokenId] = StakedRobotInfo(_owner,_tokenId, robotType, upgradeType, originalChefId);
    ...
    function _removeRobotFromPizzeria(uint256 _tokenId, address _owner) internal
    {
        delete stakedRobot[_tokenId];
    ...
    function batchedRobotsOfOwner(
        ...
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (StakedRobotInfoV2[] memory) {
        if (_offset >= ownedRobotstakesBalance[_owner]) {
            return new StakedRobotInfoV2[](0);
        }
        ...
        StakedRobotInfoV2[] memory outputs = new StakedRobotInfoV2[](outputSize);

        for (uint256 i = 0; i < outputSize; i++) {
            uint256 robotId = ownedRobotStakes[_owner][_offset + i];
            outputs[i] = StakedRobotInfoV2({
                owner: _owner,
                robotId: robotId,
                ...
                name: pizzaRobot.getName(robotId)
            });
        }
    ...
    function openCloseFactory() external onlyOwner {
        isOpen = !isOpen;
    }

    function setPizzaFactorySkills(PizzaFactorySkillsV3 _factorySkills) external onlyOwner {
        factorySkills = _factorySkills;
    }
...
contract PizzaFactorySkillsV3 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    // Events
    event skillBought(address owner, uint256 skillId);
    ...
    function learnSkill(address owner, uint256 _skillId) public {
        ...
        emit skillBought(owner, _skillId);
    }
    ...
    function updatePreRequisite(uint256 _skillId, uint256 _prerequisite1, uint256 _prerequisite2) external onlyOwner {
        require(_skillId > 0 && skillTree[_skillId].skillId == _skillId, "Invalid Skill");
        Skill storage skill = skillTree[_skillId];
        skill.prerequisite1 = _prerequisite1;
        skill.prerequisite2 = _prerequisite2;
    }
