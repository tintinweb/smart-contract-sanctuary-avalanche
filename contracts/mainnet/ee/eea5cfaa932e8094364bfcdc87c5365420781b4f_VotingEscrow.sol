...
bool private initialized; // Flag for initializer



function initialize(address token_addr, address art_proxy) public {
...
modifier nonreentrant() {
    ...
    string constant public name = "veGLACIER";
    string constant public symbol = "veGLCR";
    ...
    mapping(uint => uint) public partnerTokenInitialLockTime; // user -> Point[user_epoch]
    ...
    uint internal constant MAXTIME = 12 * 7 * 86400;
    int128 internal constant iMAXTIME = 12 * 7 * 86400;
    ...
    if (_isPartnerLock) {
      ...
      require(unlock_time <= block.timestamp + MAXTIME, 'Voting lock can be 12 weeks max');
    }
    ...
    function create_lock_for(uint _value, uint _lock_duration, address _to) external nonreentrant returns (uint) {
      ...
      function createLockForPartner(uint _value, uint _lock_duration, address _to)
  external nonreentrant returns (uint) {
    ...
    function increaseunlock_time(uint _tokenId, uint _lock_duration) external nonreentrant {
    LockedBalance memory _locked = locked[_tokenId];
    ...
    function merge(uint _from, uint _to) external {
        ...
        require(isPartnerToken[_from] == isPartnerToken[_to], "Cannot merge different types of NFTs");
