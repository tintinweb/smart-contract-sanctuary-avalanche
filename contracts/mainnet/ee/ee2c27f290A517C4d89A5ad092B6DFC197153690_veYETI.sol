...
contract veYETI is IveYETI {
    ...
    */
    ...
    /// @notice userLockedYetiAmount maps user address to the amount of YETI locked by collateralGate
    mapping (address => uint256) userLockedYetiAmount;
    ...
    event UserVeYetiBurned(address _user, address _rewarder, uint256 _amount);
    event UserLockedYetiAmountChanged(uint256 _newYetiLock);
    ...
    /// @notice set collateral gate with upgradeAndCall
    function setCollateralGate(ICollateralGate _collateralGate) external {
        require(address(collateralGate) == address(0));
        collateralGate = _collateralGate;
    }
    ...
    /** 
     * @notice Can use update() to:
     *   stake or unstake more YETI overall and/or
     *   reallocate current YETI to be staked on different rewarders
     *   Also updates the userLockedYetiAmount, in the case of an increase or decrease in lock 
     *   from the collateralGate contract. 
     */
    function update(RewarderUpdate[] memory _yetiAdjustments) external {
        ...
        uint256 userLockedYeti = collateralGate.updateUserLockedYeti(msg.sender);

        // Update amount of YETI that is put into the collateral gate, and therefore will not 
        // be earning veYETI continuously. Whether this is increased or decreased, it will update
        // here, and will be subsequently subtracted from all calculations of veYETI per rewarder
        userLockedYetiAmount[msg.sender] = userLockedYeti;
        emit UserLockedYetiAmountChanged(userLockedYeti);

        // confirms that this update does not remove locked Yeti from the collateral gate rewarder
        require(userInfo.yetiStakes[address(collateralGate)] >= userLockedYeti, "Update removes locked YETI");
    ...
    /**
     * @notice Will firstly lower user's lastTotalVeYeti by _veYETI burn.
     *   Then will confirm that there's enough Yeti locked in veYETI on collateral gate.
     *   Does necessary updates, most importantly update userLockedYetiAmount
     * @param _user address to check the YETI balances for
     * @param _veYETIBurn amount of veYETI to burn
     * @param _newYetiLock amount of YETI that is locked by collateralGate, total
     */
    function lockBurnFromCollateralGate(address _user, uint256 _veYETIBurn, uint256 _newYetiLock) external {
        require(msg.sender == address(collateralGate), "Only callable by Collateral Gate");
        emitter.updateUserRewards(_user);

        UserInfo storage userInfo = users[_user];

        require(userInfo.yetiStakes[address(collateralGate)] >= _newYetiLock, "not enough yeti available to lock");

        // update user's lastTotalVeYeti
        _accumulate(_user, true);

        // veYETI burned proportionally across all rewarders
        userInfo.lastTotalVeYeti -= _veYETIBurn;
        ...
        // be earning veYETI continuously. Whether this is increased or decreased, it will update
        // here, and will be subsequently subtracted from all calculations of veYETI per rewarder
        userLockedYetiAmount[_user] = _newYetiLock;
        emit UserLockedYetiAmountChanged(_newYetiLock);

        // notify all the updatable rewarders about veYETI changes
        _notifyRewarders(_user, updatableRewarders);

        emit UserVeYetiBurned(_user, msg.sender, _veYETIBurn);
    }

    /**
     * @notice Does necessary updates, most importantly update userLockedYetiAmount
     * @param _user address to check the YETI balances for
     * @param _newYetiLock amount of YETI that is locked by collateralGate, total
     */
    ...
    function getVeYetiOnRewarder(address _user, address _rewarder) external view override returns (uint256) {
        ...
        // If collateral gate is the actual rewarder, we want to offset by the correct amount based on what is locked
        if (_rewarder == address(collateralGate)) {
            return currentVeYeti * (userInfo.yetiStakes[_rewarder] - userLockedYetiAmount[_user]) / (userInfo.totalYeti - userLockedYetiAmount[_user]);    
        }
        return currentVeYeti * userInfo.yetiStakes[_rewarder] / (userInfo.totalYeti - userLockedYetiAmount[_user]);
    }
    ...
    function getTotalVeYeti(address _user) public view returns (uint256) {
        ...
        uint256 veGrowth = (userInfo.totalYeti - userLockedYetiAmount[_user]) * accumulationRate * dt;
...
//        require(msg.sender == contractController, "veYETI: invalid caller"); todo do not redeploy the contract with this
    ...
    function _notifyRewarders(address _user, address[] memory rewarders) internal {
        ...
        for (uint256 i = 0; i < rewarders.length; i++) {
            ...
            if (isUpdatableRewarder[rewarder]) {
                ...
                if (userInfo.totalYeti > 0) {
                    veYetiAmount = currentVeYeti * userInfo.yetiStakes[rewarder] / (userInfo.totalYeti - userLockedYetiAmount[_user]);
                }
