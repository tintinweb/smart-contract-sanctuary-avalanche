...
interface INodeManager {
    ...
    function getNodeNumberOf(address account)external returns(uint256);
...
contract nodePresale is OwnerWithdrawable {
    using SafeMath for uint256;
    ...
    uint256 public maxPresaleNodesPerInvader;
    ...
    reflections private refPool;
    address public wlToken;

    mapping(address => bool) public isInvader;
    mapping(address => uint256) public nodesPerInvader;
    mapping(address => bool) public returnBuyer;
    constructor(address _nodeManager, address _refPool){
    ...
    function updateWlToken(address _token)external onlyOwner{
        wlToken = _token;
    }

    function setPresaleParams(
        uint256 _maxPresaleNodesPerInvader,
        ...
        maxPresaleNodesPerInvader = _maxPresaleNodesPerInvader;
    ...
    function addPresaleInvaders(address[] memory _address)external onlyOwner{
        require(_address.length > 0, "Wrong length");
        for(uint256 i = 0; i < _address.length; i++){
            require(_address[i] != address(0), "Wrong address");
            isInvader[_address[i]] = true;
        }
    }
    function buyNodes(address _token, uint256 _amount, string memory _nodeName)external payable saleDuration{
        address sender = _msgSender();
        require(
                bytes(_nodeName).length > 3 && bytes(_nodeName).length < 32,
                "PreSale: Invalid Node name"
            );
        require(isInvader[sender],"PreSale: buyer is not an Invader");
        if(isInvader[sender])
            require((nodeManager.getNodeNumberOf(sender)+1) <= maxPresaleNodesPerInvader, "PreSale: Max presale nodes per Invader reached");
        require(
                sender != teamPool && sender != rewardsPool,
                "PreSale: teamPool and rewardsPool cannot participate"
            );
        if(_token != address(0)){
            require(_token == wlToken, "PreSale: Wrong token");
            require( _amount >= rate1, "PreSale: Not enough USDC.e");
            IERC20Metadata(_token).safeTransferFrom(sender, address(this), _amount);
        }
        else
        {
            require(msg.value >= rate2, "PreSale: Not enough AVAX");
        }
        totalNodesSold++;
        nodesPerInvader[sender]++;

        nodeManager.createNode(sender, _nodeName, nodeValue);
