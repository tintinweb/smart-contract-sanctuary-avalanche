...
contract FIEFStoreTransactorSigner is Signable {
    using BuyLib for BuyLib.Buy;

    address public transactor;

    constructor(address transactorAddress) {
        transactor = transactorAddress;
    }

    function getInfo(address msgSender, BuyLib.Buy calldata buy)
        external
        view
        returns (address signer, uint256 chainId)
    {
        chainId = _getChainId();
        return (buy.getSigner(msgSender, transactor, chainId), chainId);
    }
}
