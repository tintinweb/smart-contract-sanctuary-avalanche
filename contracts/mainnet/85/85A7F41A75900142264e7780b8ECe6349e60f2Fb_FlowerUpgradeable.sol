...
contract FlowerUpgradeable is OwnableUpgradeable {
        ...
        onlyAmountGreaterThanZero(_amountUSDCe)
    {
        ...
        nectarToken.swapUsdcForToken(amountUSDCe, amountUSDCe / 2);
        uint256 amountNCTR = amountUSDCe *
            (nectarToken.balanceOf(pairAddress) /
                USDCeToken.balanceOf(pairAddress));
        ...
        onlyAmountGreaterThanZero(_compoundAmount)
    {
        ...
        for (; downlineDepth < NUM_OF_TIERS; downlineDepth++) {
            ...
            if (
                ...
                nectarToken.swapTokenForUsdc(forSwapNCTR, forSwapNCTR / 2);
                uint256 forUplineWalletUSDCe = forSwapNCTR *
                    (USDCeToken.balanceOf(pairAddress) /
                        nectarToken.balanceOf(pairAddress));
                require(
                    USDCeToken.transfer(
