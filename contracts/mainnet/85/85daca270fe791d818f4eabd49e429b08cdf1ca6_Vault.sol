...
contract Vault is Ownable {
  ...
  function stake(uint256 amount, bool isAdding) external {
    ...
    if (stakers[msg.sender].amount == 0 || isAdding) {
      ...
      require(amountAfterFees.add(stakers[msg.sender].amount) >= minStake * 1**18, "STAKE: Below min amount");
      require(amountAfterFees.add(stakers[msg.sender].amount) <= maxStake * 1**18, string(abi.encodePacked(amountAfterFees.add(stakers[msg.sender].amount))));
