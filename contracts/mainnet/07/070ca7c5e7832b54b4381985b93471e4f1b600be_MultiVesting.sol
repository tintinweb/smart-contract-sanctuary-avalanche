/**
 *Submitted for verification at snowtrace.io on 2023-04-20
*/
...
contract MultiVesting is AccessControl {
  ...
  struct Vesting {
    ...
    uint256 stepsAmount; // Amount of Steps
  }
  ...
  /// @param _stepsAmount - amount of steps
  function addVesting(
    address _beneficiary,
    uint256 _amount,
    uint256 _startedAt,
    uint256 _stepsAmount
  ...
  /// @notice Creates vestings for beneficiary
  /// @param _beneficiary - addresses of beneficiary.
  /// @param _amount - amounts of tokens to allocate
  /// @param _startedAt - timestamps (in seconds) when the allocation should start
  /// @param _stepsAmount - amounts of steps
  function bulkAddVesting(
    address[] calldata _beneficiary,
    uint256[] calldata _amount,
    uint256[] calldata _startedAt,
    uint256[] calldata _stepsAmount,
    uint256 _from,
    uint256 _to
  ) external {
    require(
    ...
    require(_from < _beneficiary.length && _to <= _beneficiary.length, "INDEX_OUT_OF_RANGE");
  ...
  function withdrawUnallocatedFunds(address _receiver) external onlyRole(OPERATOR_ROLE) {
  ...
  function getAvailableAmountAtTimestamp(
    ...
    uint256 periodsPassed = _timestamp.sub(vesting.startedAt).div(30 days); // We say that a month is always 30 days
