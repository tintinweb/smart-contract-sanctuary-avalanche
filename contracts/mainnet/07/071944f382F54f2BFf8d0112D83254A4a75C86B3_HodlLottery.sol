...
contract HodlLottery is Ownable, ReentrancyGuard {
    ...
    function getLotteryHistory(address _addr, LOTTERY_TYPE _lotteryType) external view returns (Lottery_History[] memory) {
        uint256 _index = 0;
        Lottery_History[] memory retInfo;
        Lottery_Info storage lottoInfo;
        if (_lotteryType == LOTTERY_TYPE.LOTTERY_WEEKLY) {
            ...
            for (uint256 i = 0; i < weeklyLotteryCounter; i ++) {
                ...
                if (allWeeklyLotteryInfos[i].members[winnerID] == _addr) {
                    retInfo[_index] = Lottery_History({addr: _addr, timestamp: allBiDailyLotteryInfos[i].lotteryTimeStamp, winnerPrize: allBiDailyLotteryInfos[i].winnerPrize});
                    _index ++;
                }
            }
        }
    ...
    function getAllLotteryWinners(LOTTERY_TYPE _lotteryType) external view returns (Lottery_History[] memory) {
        ...
        if (_lotteryType == LOTTERY_TYPE.LOTTERY_WEEKLY) {
            ...
            for (uint256 i = 0; i < weeklyLotteryCounter; i ++) {
                uint16 winnerID = allWeeklyLotteryInfos[i].winnerID;
                retInfo[_index] = Lottery_History({addr: allWeeklyLotteryInfos[i].members[winnerID], timestamp: allBiDailyLotteryInfos[i].lotteryTimeStamp, winnerPrize: allBiDailyLotteryInfos[i].winnerPrize});
                _index ++;
            }
        }
        else {
            ...
            for (uint256 i = 0; i < weeklyLotteryCounter; i ++) {
                uint16 winnerID = allBiDailyLotteryInfos[i].winnerID;
                retInfo[_index] = Lottery_History({addr: allBiDailyLotteryInfos[i].members[winnerID], timestamp: allBiDailyLotteryInfos[i].lotteryTimeStamp, winnerPrize: allBiDailyLotteryInfos[i].winnerPrize});
                _index ++;
            }
        }
