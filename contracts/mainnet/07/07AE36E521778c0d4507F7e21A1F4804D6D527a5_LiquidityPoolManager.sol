    ...
    UniverseImplementationPointer
{
    ...
    address[] public feeAddresses = [
        address(0x2775D1174C73aD1AC158b9FB9B2571a1a214f110), // RFV (40%)
        address(0x4e7329B45BB5f919616c099A652FCF1313c0a7B8), // Dev (15%)
        address(0x8F5E20fFAA83ca221b3d60f5F2E2d2C719e6E1b2) // Marketing (15%)
    ];
    uint8[] public feePercentages = [40, 15, 15];
        ...
        returns (bool)
    {
        ...
        if (
            ...
            for (uint256 i = 0; i < feePercentages.length; i++) {
                totalFees += feePercentages[i];
            }

            uint256 initialRightBalance = rightSide.balanceOf(address(this));
            swapLeftSideForRightSide(swapTokensToLiquidityThreshold / totalFees / 100);
            ...
            for (uint256 i = 0; i < feeAddresses.length; i++) {
                if ((feeAddresses.length - 1) == i) {
                    ...
                    uint256 calculatedFee = totalRight * feePercentages[i] / totalFees;
                    totalRightRemaining -= calculatedFee;
                    sendRightTokensTo(feeAddresses[i], calculatedFee);
                }
            }

            // Add liquidity to LPM
            swapAndLiquify(swapTokensToLiquidityThreshold / (100 - totalFees) / 100);
    ...
    function swapAndLiquify(uint256 tokens) private returns (uint256) {
        ...
        uint256 newRightBalance = rightSide.balanceOf(address(this)) - initialRightBalance;
