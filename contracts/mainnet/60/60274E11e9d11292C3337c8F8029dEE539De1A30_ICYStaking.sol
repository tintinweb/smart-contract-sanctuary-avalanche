...
contract ICYStaking is Ownable {
    ...
    constructor(IBEP20 _stakeToken, IBEP20 _weth) {
    ...
    function stake(uint amount) public {
        ...
        require(stakeToken.balanceOf(msg.sender) >= amount, "Insufficient token balance to stake");

        IICY ICY = IICY(address(stakeToken));
        uint256 liquidityFee = ICY.liquidityFee();
        uint256 treasuryFee = ICY.treasuryFee();
        uint256 reflectionFee = ICY.reflectionFee();
        uint256 marketingFee = ICY.marketingFee();
        uint256 feeDenominator = ICY.feeDenominator();
    ...
    function numOfDaysSinceStartedUnstake(address account) internal view returns(uint) {
