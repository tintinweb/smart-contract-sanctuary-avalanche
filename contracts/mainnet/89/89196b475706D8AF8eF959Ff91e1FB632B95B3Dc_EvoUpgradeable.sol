...
ChainlinkVRFConsumerUpgradeable {
    ...
    function _reveal(uint256[] memory ids) internal {
        PendingHatch storage pending = _pendingHatches[_msgSender()];
        require(pending.requestId > 0, "No pending hatch in progress");
        require(pending.words.length > 0, "Results still pending");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            // generation
            _attributes[ids[i]].set(3, 0);
            // breed count
            _attributes[ids[i]].set(6, 999);
            // experience
            _attributes[ids[i]].set(7, 0);
            uint256[] memory randomChunks = chunkRandom(pending.words[i]);
            speciesRoll(ids[i], randomChunks, minRarity, speciesIds, totalRarity);
            geneRoll(ids[i], treated[i], randomChunks);
