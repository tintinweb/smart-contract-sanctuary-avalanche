...
interface ICryptoPuffiesBestFriends {
...
interface IOldIgloo {
    function userTotalStakedPuffies(address _user) external view returns(uint256);
}
...
contract PuffiesIgloo is Ownable {
    ...
    IERC721 public immutable cryptoPuffies; //the CryptoPuffies contract
    ICryptoPuffiesBestFriends public immutable cryptoPuffiesBestFriends; //the CryptoPuffiesBestFriends contract
    ...
    uint256 public BEST_FRIEND_PAIR_BONUS = 3;
    address public latestUniqueDepositor; //latest person to deposit a puffy for the first time
    address public topStakerAddress;
    uint256 public topStakerAmount;
    ...
    mapping(address => uint256) public pairsStaked; //how many pairs each user has staked
    mapping(address => uint256) public puffiesPairBonuses; //keeps track of number of points each user has, specifically from pair bonuses
    mapping(uint256 => bool) public puffyEverDeposited; //tracks if a puffy has ever been deposited

    event LogOnReward(address indexed user,  uint256 amount);
    event RewardRateUpdated(uint256 oldRate, uint256 newRate);

    constructor(address _rewardToken, IERC721 _cryptoPuffies, ICryptoPuffiesBestFriends _cryptoPuffiesBestFriends, uint256 _tokensPerSecond, uint256 _rewardStartTimestamp) {
        require(_rewardStartTimestamp > block.timestamp, "rewards must start in future");
        rewardToken = _rewardToken;
        cryptoPuffies = _cryptoPuffies;
        cryptoPuffiesBestFriends = _cryptoPuffiesBestFriends;
    ...
    function hasNotMigrated(address user) public view returns(bool) {
        if (IOldIgloo(0x2cE75C9a6464a2471685AE9F12300Fe29e5eB838).userTotalStakedPuffies(user) > 0) {
            return true;
        } else {
            return false;
        }
    ...
    //returns topStakerAddress as well as the tokenId of the latest Puffy they deposited
    function luckyPenguin() public view returns(address, uint256) {
        uint256 puffiesStaked = userTotalStakedPuffies(topStakerAddress);
        uint256 latestStakedPuffyId;
        if (puffiesStaked > 0) {
            latestStakedPuffyId = userStakedPuffiesAtIndex(topStakerAddress, puffiesStaked - 1);
        }
        return (topStakerAddress, latestStakedPuffyId);
    }

    //returns latestUniqueDepositor as well as the tokenId of the latest Puffy they deposited
    function latestStaker() public view returns(address, uint256) {
        uint256 puffiesStaked = userTotalStakedPuffies(latestUniqueDepositor);
        uint256 latestStakedPuffyId;
        if (puffiesStaked > 0) {
            latestStakedPuffyId = userStakedPuffiesAtIndex(latestUniqueDepositor, puffiesStaked - 1);
        }
        return (latestUniqueDepositor, latestStakedPuffyId);
    }
    ...
    function userEarningPerSecond(address _user) public view returns(uint256) {
        UserInfo storage user = userInfo[_user];
        if (distributionTimeRemaining() == 0 || totalShares == 0) {
            return 0;
        } else {
            return tokensPerSecond * user.amount / totalShares;
        }
    }
    ...
    function increaseTotalRewardAmount(uint256 increaseAmount) external payable {
        if (rewardToken == AVAX) {
            require(msg.value == increaseAmount, "not enough AVAX sent");
        } else {
            IERC20(rewardToken).transferFrom(msg.sender, address(this), increaseAmount);
        }
        totalRewardAmount += increaseAmount;
    }
    ...
    function deposit(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint16 bestFriend = cryptoPuffiesBestFriends.bestFriend(tokenIds[i]);
            amountUpdate++;
            //if msg.sender already has the puffy's best friend deposited
            if (puffiesStakedBy[bestFriend] == msg.sender) {
                uint256 toAdd = ((BEST_FRIEND_PAIR_BONUS - 1) * 2);
                amountUpdate += toAdd;
                pairsStaked[msg.sender] += 1;
                puffiesPairBonuses[msg.sender] += toAdd;
            }
            //if puffy is deposited for the first time
            if (!puffyEverDeposited[tokenIds[i]]) {
                puffyEverDeposited[tokenIds[i]] = true;
                if (latestUniqueDepositor != msg.sender) {
                    latestUniqueDepositor = msg.sender;
                }
            }
        }
        ...
        //if depositor is now the top staker
        if (user.amount > topStakerAmount) {
            topStakerAddress = msg.sender;
            topStakerAmount = user.amount;
        }
        if (pending > 0) {
    ...
    function withdraw(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            amountUpdate++;
            //if msg.sender already has the puffy's best friend deposited
            if (puffiesStakedBy[bestFriend] == msg.sender && pairsStaked[msg.sender] > 0) {
                uint256 toDeduct = puffiesPairBonuses[msg.sender] / pairsStaked[msg.sender];
                amountUpdate += toDeduct;
                pairsStaked[msg.sender] -= 1;
                puffiesPairBonuses[msg.sender] -= toDeduct;
            }
        }
        user.amount -= amountUpdate;
        totalShares -= amountUpdate;
    ...
    function updateBestFriendMultiplier(uint256 newBestFriendMultiplier) external onlyOwner {
        require(newBestFriendMultiplier > 0, "must be > 0");
        BEST_FRIEND_PAIR_BONUS = newBestFriendMultiplier;
    }
