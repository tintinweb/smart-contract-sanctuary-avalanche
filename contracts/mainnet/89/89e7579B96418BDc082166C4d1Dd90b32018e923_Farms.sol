...
contract Farms is ReentrancyGuard, Ownable, Pausable {
  ...
  */
  function boost(uint256 _id, address _user, uint256 _amount) external nonReentrant whenNotPaused onlyYieldBooster {
    require(farms[_id].active, "Farm is not active");
    require(_user != address(0), "Invalid zero address");
    require(_amount > 0, "Cannot boost 0");

    _updateFarm(_id);
    _updateUserRewardDebt(_id, _user, _amount);
    ...
    IERC20(esSteady).safeTransferFrom(_user, address(this), _amount);
  ...
  */
  function unboost(uint256 _id, address _user, uint256 _amount) external nonReentrant whenNotPaused onlyYieldBooster {
    require(farms[_id].active, "Farm is not active");
    require(_user != address(0), "Invalid zero address");
    require(_amount > 0, "Cannot boost 0");
    ...
    position.pointsAmount = position.pointsAmount - _amount;
    farm.totalPoints = farm.totalPoints - _amount;
    IERC20(esSteady).safeTransfer(_user, _amount);
