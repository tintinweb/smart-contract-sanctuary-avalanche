...
contract WLRS is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("walrus.finane", "WLRS") {
    ...
    function _getWlrsPrice() internal view returns (uint256 _wlrsPrice) {
        try IOracle(wlrsOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("WLRS: failed to fetch WLRS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _wlrsPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_wlrsPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWlrsOracle(address _wlrsOracle) public onlyOperatorOrTaxOffice {
        require(_wlrsOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWlrsPrice = _getWlrsPrice();
            currentTaxRate = _updateTaxRate(currentWlrsPrice);
            if (currentWlrsPrice < burnThreshold) {
                burnTax = true;
            }
        }
