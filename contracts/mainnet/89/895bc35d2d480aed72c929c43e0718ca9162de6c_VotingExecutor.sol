...
// File contracts/upgrade/FsOwnable.sol
...
// File contracts/upgrade/FsBase.sol
...
// File contracts/voting/interfaces/IVotingExecutor.sol


pragma solidity ^0.8.7;

/// @title An IVotingExecutor executes a sequence of calls atomically.
interface IVotingExecutor {
    /// @notice Executes a vote for a set of given targets and given calldata
    ///         See description in the title of the contract for details
    /// @param _targets An array of targets to call
    /// @param _callData An array of call data (same length as _targets) to sent to each target
    function executeVote(address[] memory _targets, bytes[] memory _callData) external;
}


// File contracts/voting/VotingExecutor.sol
...
/// @title The VotingExecutor executes a vote atomically.
///        A vote can consist of multiple calls to different contracts.
///        A vote is only allowed to pass if none of the calls have reverted.
///        In case one call has reverted all other calls need to revert.
contract VotingExecutor is FsBase, FsAdmin, IVotingExecutor {
    ...
    //slither-disable-next-line unused-state
    uint256[998] private _____contractGap;
    ...
    /// @param _targets An array of targets to call
    /// @param _callData An array of call data (same length as _targets) to sent to each target
    function executeVote(address[] memory _targets, bytes[] memory _callData)
        external
        override
        canExecuteVote
    {
        require(_targets.length == _callData.length, "Array mismatch");

        for (uint256 i = 0; i < _targets.length; i++) {
            //slither-disable-next-line unused-return
            Address.functionCall(_targets[i], _callData[i]);
        }
    }

    /// @dev Transfers ownership of the contract to a new account (`newOwner`).
    /// Can only be called by the owner, admin or VotingExecutor.
    function transferOwnership(address newOwner) external override {
        //
        require(
            msg.sender == address(this) || msg.sender == owner() || msg.sender == admin,
            "Only voting"
        );
        _transferOwnership(newOwner);
    }

    /// @dev Prevents calling from any address except the admin address or the owner (voting system)
    modifier canExecuteVote() {
        require(msg.sender == owner() || msg.sender == admin, "access");
        _;
    }
}