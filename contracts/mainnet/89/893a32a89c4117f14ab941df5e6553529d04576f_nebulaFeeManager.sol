...
contract nebulaFeeManager is Ownable{
    ...
    function payFeeToken(uint256 _intervals) payable external {
	    ...
	    for(uint i = 0;i<_intervals;i++) {
	    	if(_intervals > 0 && returnBalance >= tokFee){
	    		feeTok.transfer(treasury, tokFee);
			returnBalance -= tokFee;
			count++;
		}
	}
    ...
    function approveFee(uint256 _intervals) external {
    	...
    	queryFeeToken(_account,totTok);
    	feeTok.approve(_account, totTok);
    }
    ...
    function queryFees(address _account) internal {
		...
		for(i = 0;i<protos.length;i++) {
			...
			if(proto.imploded != true){
				...
				uint256 paidTill = paidTime.add(time);
				uint256 lifeLeftUnpaidCollapsed = proto.collapseDate.sub(paidTill);
				...
				acctTot.futureFees += unPaidTimePeriods;
				acctTot.totalPayable += unPaidTimePeriods;
				acctTots.feesOwed += acctTot.feesOwed;
			    	acctTots.protos += acctTot.protos;
			    	acctTots.futureFees += acctTot.futureFees;
			    	acctTots.totalPayable += acctTot.totalPayable;
