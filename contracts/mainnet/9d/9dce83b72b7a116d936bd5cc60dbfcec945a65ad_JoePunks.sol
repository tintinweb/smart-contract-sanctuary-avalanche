...
/// @title Joe Punks
contract JoePunks is ERC721Metadata {
    ...
    uint256 private _giveawayMax = 400; 

    uint256 private _totalSupply;
    uint256[1001] private _pendingIDs;
    ...
    function mint(uint256 counts_)
        external payable {
        require(pendingCount > 0, "Joe Punks: All minted");
        require(counts_ > 0, "Joe Punks: Counts cannot be zero"); 
        require(totalSupply().add(counts_) <= MAX_NFT_SUPPLY,
            "Joe Punks: sale already ended"); 
        require(!paused, "Joe Punks: The contract is paused"); 
    ...
    function _randomMint(address to_)
        ...
        require(to_ != address(0), "Joe Punks: zero address");

        require(totalSupply() < MAX_NFT_SUPPLY,
            "Joe Punks: max supply reached");
