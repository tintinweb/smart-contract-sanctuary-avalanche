...
// File contracts/anycall/access/PausableControl.sol
...
// File contracts/anycall/access/AdminControl.sol
...
// File contracts/anycall/access/PausableControlWithAdmin.sol
...
// File contracts/anycall/AnyswapTokenAnycallClient.sol
...
abstract contract AnycallClientBase is PausableControlWithAdmin {
    ...
    modifier onlyCallProxy() {
        require(msg.sender == callProxy || msg.sender == IAnycallV6Proxy(callProxy).executor(), "AnycallClient: not authorized");
        _;
    }
    ...
    receive() external payable {
        require(msg.sender == callProxy || msg.sender == IAnycallV6Proxy(callProxy).executor(), "AnycallClient: receive from forbidden sender");
    }
...
contract AnyswapTokenAnycallClient is AnycallClientBase {
        ...
        returns (bool success, bytes memory result)
    {
        ...
        if (_underlying != address(0) && (IERC20(_underlying).balanceOf(address(this)) >= amount)) {
            IERC20(_underlying).safeTransferFrom(address(this), receiver, amount);
        } else {
