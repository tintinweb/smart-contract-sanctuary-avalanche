...
interface IPizzeriaProgressionV4_11 {
...
contract PizzeriaV5_8 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    // Constants
    ...
    IPizzeriaProgressionV4_11 public pizzeriaProgression;
    IMafia_11 public mafia;
    ...
    function initialize(Chef _chef, Upgrade _upgrade, Pizza _pizza, Soda _soda, address _freezerAddress, address _pizzeriaProgression) public initializer {
        ...
        pizzeriaProgression = IPizzeriaProgressionV4_11(_pizzeriaProgression);
    ...
    function resetFatigue() public {
        ...
        uint256 costToReset = getTotalPPM(_owner) * sodaResetCost;
    ...
    function energyDrink() public {
        ...
        uint256 costToReset = getTotalPPM(_owner) * energyDrinkCost * pizzeriaProgression.getMasterChefSkillModifier(_owner, masterChefNumber) * (100 + masterChefNumber * masterChefMultiplicationBonus)/10000;
    ...
    function roniAccruedCalculation(uint256 _startTime, uint256 _endTime, uint256 _ppm, uint256 _fatigue, uint256 _fatiguePerMinute, uint256 _yieldPPS, uint256 _energyDrinkBonus, uint256 _endTimeForBoost) public pure returns (uint256) {
        ...
        uint256 ppmyield = _ppm * _yieldPPS;

        if(_endTimeForBoost <= _startTime) {
            uint256 _deltaTime = _endTime - _startTime;
            uint256 a = _deltaTime * ppmyield * (MAX_FATIGUE - _fatigue) / ( MAX_FATIGUE);
            uint256 b = _deltaTime * _deltaTime * ppmyield * _fatiguePerMinute / (2 * 60 * MAX_FATIGUE);
            if(a > b){
                return (a - b);
            }
        }
        ...
        if(_endTimeForBoost < _endTime && _endTimeForBoost > _startTime) {
            ...
            uint256 a1 = _deltaTime1 * ppmyield * (MAX_FATIGUE - _fatigue) / ( MAX_FATIGUE);
            uint256 b1 = _deltaTime1 * _deltaTime1 * ppmyield * _fatiguePerMinute / (2 * 60 * MAX_FATIGUE);

            uint256 _deltaTime2 = _endTime - _endTimeForBoost;
            uint256 a2 = _deltaTime2 * ppmyield * (MAX_FATIGUE - _fatigue) / ( MAX_FATIGUE);
            uint256 b2 = _deltaTime2 * _deltaTime2 * ppmyield * _fatiguePerMinute / (2 * 60 * MAX_FATIGUE);
            uint256 c2 = _deltaTime1 * _deltaTime2 * ppmyield * _fatiguePerMinute / (60 * MAX_FATIGUE);

            if(a1 > b1 && a2 > b2 ){
                return a2 + (a1 - b1)*(100 + _energyDrinkBonus)/100 - b2 - c2;
            }
        }
    ...
    function fixRestingState(address _owner) external onlyOwner {
        uint256 totalPPMFixed;
        uint256 numberChefFixed;
        uint256 numberMasterChefFixed;

        uint256 totalChefs = ownedChefStakesBalance[_owner];
        for (uint256 i = 0; i < totalChefs; i++) {
            uint256 chefId = ownedChefStakes[_owner][i];
            if(chef.getType(chefId) == chef.MASTER_CHEF_TYPE()){
                numberMasterChefFixed++;
            } else {
                numberChefFixed++;
            }

            totalPPMFixed += chef.getYield(chefId);
            
        }

        uint256 totalTools = ownedUpgradeStakesBalance[_owner];
        for (uint256 i = 0; i < totalTools; i++) {
            uint256 toolId = ownedUpgradeStakes[_owner][i];

            totalPPMFixed += upgrade.getYield(toolId);
        }

        numberOfStaked[_owner][1] = numberMasterChefFixed;
        numberOfStaked[_owner][0] = numberChefFixed;
        totalPPM[_owner] = totalPPMFixed;
    }
