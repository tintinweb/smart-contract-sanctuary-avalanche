...
abstract contract Keepers is Ownable {
    mapping(address=>bool) private _keeper;

    event ToggleKeeper(address indexed keepr, bool indexed status);

    constructor () {
    }

    /**
     * @dev Returns the keeper status of the address.
     */
    function keeper(address _address) public view virtual returns (bool) {
        return _keeper[_address];
    }

    /**
     * @dev Throws if called by not a keeper.
     */
    modifier onlyKeeper() {
        require(keeper(_msgSender()), "Keepers: caller is not a keeper");
        _;
    }

    /**
     * @dev Converts keeper into not keeper or not keeper into keeper
     */
    function toggleKeeper(address _address) public virtual onlyOwner {
        bool _newStatus = !keeper(_address);
        emit ToggleKeeper(_address, _newStatus);
        _keeper[_address] = _newStatus;
    }
}
...
contract AvolidlyLpStaker is Keepers {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        require(_startTimeOffset.length == _rewardsPerSecond.length, "wrong parameters");
    ...
    // Update the given pool's AVLD allocation point. Can only be called by the owner.
    ...
    // View function to see pending reward tokens on frontend.
    function totalClaimableReward(address _user)
        external
        view
        returns (uint256)
    {
        ...
        for(uint i; i < poolInfo.length; ++i)
            rewardAmount += claimableReward(i, _user);
        return rewardAmount;
    }
    ...
    // Just in case transaction is so big that It doesn't fit into a block.
    function updateEmissionsNoUpdate() external onlyOwner {
        uint256 length = emissionSchedule.length;
        if (startTime > 0 && length > 0) {
            EmissionPoint memory e = emissionSchedule[length-1];
            if (block.timestamp.sub(startTime) > e.startTimeOffset) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            rewardMinter.mint(owner(), pending/10);
        }
    ...
    function claim(uint256[] calldata _pids) public {
        _updateEmissions();
        uint256 pending;
    ...
    function claimAll() external{
        ...
        uint pending;
        for(uint i; i<poolInfo.length; ++i)
            if(claimableReward(i, msg.sender) > 0){
                _updatePool(i);
    ...
    function claimSwapFeesFor(uint _pid, address _to) external onlyKeeper{
        IBasePair lpt = IBasePair(address(poolInfo[_pid].lpToken));
        IERC20 token0 = IERC20(lpt.token0());
        IERC20 token1 = IERC20(lpt.token1());
        uint balB40 = token0.balanceOf(address(this));
        uint balB41 = token1.balanceOf(address(this));
        lpt.claimFees();
        uint earn0 = token0.balanceOf(address(this))-balB40;
        uint earn1 = token1.balanceOf(address(this))-balB41;
        token0.transfer(_to, earn0);
        token1.transfer(_to, earn1);
    }
}