...
contract StepAppStaking is ERC20VotesCompUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev StepAppDistributor address
    ...
    function initialize(
        ...
        require(address(_stakingToken) != address(0), "StepAppStaking: staking token is the zero address");
        require(_penaltyDays <= PENALTY_DAYS_LIMIT, "StepAppStaking: penalty days exceeds limit");
    ...
    function stakeFor(address _user, uint128 _amount) external nonReentrant {
        require(msg.sender == distributor, "StepAppStaking: only distributor can stake");
    ...
    function unstake(uint256 _stakeIndex) external nonReentrant {
        require(block.timestamp < 1664625600, "StepAppStaking: migrated to Step Network");
        ...
        emit Unstaked(msg.sender, _stakeIndex, stakeRef.amount, uint128(penaltyAmount), uint128(totalSupply()), stakeRef.shares, totalShares);
    }
    ...
    /**
     * @notice Set a StepAppDistributor address
     * @param _distributor StepAppDistributor address
     */
    ...
    function _stake(address _user, uint128 _amount) internal {
        ...
        require(_amount >= minAmount, "StepAppStaking: invalid amount");
        ...
        emit Staked(_user, stakeIndex, _amount, uint48(block.timestamp), penaltyDays, penaltyBP, uint128(totalSupply()), shares, totalShares);
    }
