...
contract Locker is Ownable, ReentrancyGuard {
    ...
    IPangolinRouter02 public pangolinRouter;
    
    constructor() public {
        pangolinRouter = IPangolinRouter02(0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106);
        addBaseToken(pangolinRouter.WAVAX());
    }
    ...
    function createLock(address pair, address baseToken, uint amount, uint unlockTimestamp) external noContractsAllowed nonReentrant payable {
        ...
        uint estimatedAmountOut = pangolinRouter.getAmountsOut(msg.value, path)[1];
        uint amountOutMin = estimatedAmountOut.mul(ONE_HUNDRED_X_100.sub(SLIPPAGE_TOLERANCE_X_100)).div(ONE_HUNDRED_X_100);
        
        pangolinRouter.swapExactAVAXForTokens{value: msg.value}(amountOutMin, path, address(this), block.timestamp);
    ...
    function getMinLockCreationFeeInWei(address pair, address baseToken, uint amount) public view returns (uint) {
        ...
        if (baseToken == pangolinRouter.WAVAX()) return feeBaseToken;
        
        address[] memory path = new address[](2);
        
        path[0] = baseToken;
        path[1] = pangolinRouter.WAVAX();
        uint ethAmount = pangolinRouter.getAmountsOut(feeBaseToken, path)[1];
        return ethAmount;
    }
}