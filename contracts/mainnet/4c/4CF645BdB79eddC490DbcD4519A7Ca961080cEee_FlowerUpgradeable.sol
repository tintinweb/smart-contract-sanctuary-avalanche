...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    uint256[] private _moveToBloomify;
    ...
    function _compoundRewards() private {
        ...
        if (movedBloomBoxes[msg.sender].length > 0) {
            for (uint256 i = 0; i < movedBloomBoxes[msg.sender].length; i++) {
                ...
                if (bloomifyMove.movingDay < 11) {
                    movedNCTRFromBloomBox[msg.sender] += bloomifyMove
                    .instantAmount;
                    bloomifyMove.movingDay += 1;
                    bloomifyMove.alreadyMoved += bloomifyMove.instantAmount / 10;
                } else {
                    movedNCTRFromBloomBox[msg.sender] += bloomifyMove
                    .rewardAmount;
                    bloomifyMove.movingDay += 1;
                    bloomifyMove.alreadyMoved += bloomifyMove.rewardAmount;
                }
            }
        }
