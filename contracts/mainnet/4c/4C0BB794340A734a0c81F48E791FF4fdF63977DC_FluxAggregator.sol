...
contract FluxAggregator is AggregatorV2V3Interface, Owned {
  ...
  event ClimateLinkUpdated(string indexed _stationIcosLink);
   ...
   * @param _stationIcosLink Link from ICOS service meaning where we got the data from
   */
  constructor(
    address _link,
    ...
    string memory _stationIcosLink
  ) public {
    ...
    stationIcosLink = _stationIcosLink;
    rounds[0].updatedAt = uint64(block.timestamp.sub(uint256(_timeout)));
  }

  function updateClimeteLink(address _oracle, string calldata _stationIcosLink) external {
    require(oracles[_oracle].admin == msg.sender, "only callable by admin");
    stationIcosLink = _stationIcosLink;
    emit ClimateLinkUpdated(_stationIcosLink);
  }
