...
contract GmxVault is BaseBuildingBlock {
        ...
        returns (uint256 amountPayed)
    {
        ...
        if (address(tokenOut) == address(0x0)) {
            ...
            require(
                tokenOut.transfer(msg.sender, amountPayed),
                "GMX BB: Transfer to ActionPool failed"
            );
        }
     ...
     *   @dev access restricted to only self base buildingblock call
     */
    function claimRewards(bytes memory _data) external onlySelf returns (bool) {
        (
            bool shouldClaimGmx,
            bool shouldStakeGmx,
     ...
     */
    function usdgAmounts(IERC20 _token) public view returns (uint256) {
        return gmxVault.usdgAmounts(address(_token));
    }
    ...
    function getGlpPrice(bool _maximize) public view returns (uint256) {
        return glpManager.getAum(_maximize) / glpToken.totalSupply();
    }

    /**
     * @notice retrieve the reserved amount of token in vault
     */
    function getReservedAmounts(IERC20 _asset) public view returns (uint256) {
        return gmxVault.reservedAmounts(address(_asset));
    }

    function getGLPBalance() public view returns (uint256) {
        return glpTrackerToken.balanceOf(address(this));
    }
    ...
    function getPoolSize(IERC20 _asset) public view returns (uint256) {
        return gmxVault.poolAmounts(address(_asset));
    }
    ...
    function getAssetBalance(IERC20 _asset) public view returns (uint256) {
...
abstract contract BaseBuildingBlock is NonBlockingBaseApp {
    ...
    event DepositBatchReceived(uint256 amount, uint256 batchSize);
    ...
    // @dev Should be called when users deposits transferred from router to BB
    // payload contains array of users who send deposits and their deposit amounts respectively
    function receiveDeposit(bytes calldata _payload) public onlySelf {
        uint256 total = 0;
        (address[] memory users, uint256[] memory amounts) = abi.decode(
            _payload,
            (address[], uint256[])
        );
        ...
        for (uint256 i = 0; i < users.length; i++) {
            userPosition[users[i]].deposit += amounts[i];
            if (tvl == 0) {
                // special case when BB is empty. Calculate shares as 1:1 deposit
                userPosition[users[i]].shares += amounts[i];
            } else {
                // amount / tvl is the share of the one token. Multiply it by amount to get total shares
                userPosition[users[i]].shares +=
                    (amounts[i] * amounts[i]) /
                    tvl;
            }
        }

        emit DepositBatchReceived(total, users.length);
    }
