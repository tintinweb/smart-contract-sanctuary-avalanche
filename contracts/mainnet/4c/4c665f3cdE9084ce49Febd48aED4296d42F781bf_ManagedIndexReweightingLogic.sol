...
contract ManagedIndexReweightingLogic is IndexLayout, IManagedIndexReweightingLogic, ERC165 {
    ...
    function reweight(address[] calldata _updatedAssets, uint8[] calldata _updatedWeights) external override {
        ...
        require(reserveToken != address(0), "ManagedIndex: RESERVE");

        if (!assets.contains(reserveToken) && inactiveAssets.add(reserveToken)) {
            // lazy creation of vToken for WETH
            IvTokenFactory(vTokenFactory).createdVTokenOf(reserveToken);

            emit UpdateAnatomy(reserveToken, 0);
        }
        ...
        for (uint i; i < inactiveAssetsCount; ) {
            ...
            } else if (inactiveAsset != reserveToken) {
