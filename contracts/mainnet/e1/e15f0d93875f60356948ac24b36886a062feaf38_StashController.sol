...
// Dependency file: contracts/Stash.sol
...
// Root file: contracts/StashController.sol

pragma solidity ^0.8.13;

// import "contracts/Stash.sol";
// import "@openzeppelin/contracts/access/Ownable.sol";

contract StashController is Ownable {
    Stash public immutable stash;

    uint256 private constant MAX_SELL_FEE = 70;

    constructor(Stash _stash) {
        stash = _stash;
    }

    function setBasicRebaseRate(uint256 value) external onlyOwner {
        stash.setBasicRebaseRate(value);
    }

    function withdrawAllToTreasury() external onlyOwner {
        stash.withdrawAllToTreasury();
    }

    function setAutoRebase(bool _flag) external onlyOwner {
        stash.setAutoRebase(_flag);
    }

    function setAutoAddLiquidity(bool _flag) external onlyOwner {
        stash.setAutoAddLiquidity(_flag);
    }

    function setAutoSwapback(bool flag) external onlyOwner {
        stash.setAutoSwapback(flag);
    }
    ...
    function setFeeReceivers(
        ...
        stash.setFeeReceivers(
            _autoLiquidityReceiver,
            _treasuryReceiver,
            _stashInsuranceFundReceiver
        );
    }

    function setFees(uint256 _sellFee) external onlyOwner {
        require(_sellFee <= MAX_SELL_FEE, "max amount: 70");

        stash.setFees(
            stash.liquidityFee(),
            stash.treasuryFee(),
            stash.stashInsuranceFundFee(),
            _sellFee,
            stash.burnRate(),
            stash.feeDenominator()
        );
    }

    function setWhitelist(address _addr) external onlyOwner {
        stash.setWhitelist(_addr);
    }

    function setBotBlacklist(address _botAddress, bool _flag) external onlyOwner {
        stash.setBotBlacklist(_botAddress, _flag);
    }

    function setLP(address _address) external onlyOwner {
        stash.setLP(_address);
    }
    ...
    function rescueToken(address tokenAddress, uint256 amount) external onlyOwner {
        stash.rescueToken(tokenAddress, amount);
