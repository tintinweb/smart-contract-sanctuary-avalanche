...
contract StrategyTJLiquidityBookLB is StratManager, FeeManager {
    ...
    /// @notice where strategy currently has pending fee's to harvest
    EnumerableSet.UintSet private _pendingFeeBins;
    ...
    /// -----------------------------------------------------------
    /// Track Pending Rewards Functions
    /// -----------------------------------------------------------

    /// @notice Returns the number of pending fee bins ready to harvest
    /// @return The number of bins where fee's are pending of strategy
    function pendingFeesNumber() public view returns (uint256) {
        return _pendingFeeBins.length();
    }

    /// @notice returns all of the bins where rewards are ready to harvest
    /// @return pendingFeeBins currently ready to harvest
    function strategyPendingFeeBins()
        public
        view
        returns (uint256[] memory pendingFeeBins)
    {
        pendingFeeBins = new uint256[](_pendingFeeBins.length());
        for (uint256 i; i < _pendingFeeBins.length(); i++) {
            pendingFeeBins[i] = _pendingFeeBins.at(i);
        }
    }

    ///@notice resets the pending bins array after a harvest
    function resetPendingFeeBins() external {
        //limit to only vault
        require(msg.sender == vault, "Strategy: !vault");

        for (uint256 i; i < _pendingFeeBins.length(); i++) {
            _pendingFeeBins.remove(_pendingFeeBins.at(i));
        }
    }
    ...
    function _collectRewards() internal returns (uint256 feesX, uint256 feesY) {
        ...
        console.log("calculate fee growth");
        ...
        for (uint256 i; i < liquidities.length; ++i) {
            ...
            if (liquidities[i] >= currentLiquidities[_id]) {
                ...
                //add the ID's to the pending fees set
                _pendingFeeBins.add(_id);
            } else {
                _feesL = 0;
            }
    ...
    /// @notice entrypoint for owner to remove liquidity from protocol ignoring rewards
    /// @param denominator proportion of liquidity to remove
    /// @return amountX amount of tokenX removed from liquidity
    /// @return amountY amount of tokenY removed from liquidity
    ...
    function executeRebalance(
        ...
        //we should harvest pending rewards as part of keepers checks to minimise gas usage
