...
contract ChefManager is Initializable, OwnableUpgradeable, PausableUpgradeable, IERC1271Upgradeable {
     ...
     */
    function whitelistStrategy(address _joeChef, address _strategy) external onlyManager {
     ...
     */
    function blacklistStrategy(address _joeChef, address _strategy) external onlyManager {
        IERC20Upgradeable _want = IJoeStrategy(_strategy).want();
        uint256 _pid = IJoeStrategy(_strategy).pid();
        _want.safeApprove(_joeChef, 0);
