...
contract LPPriceOracle {
    ...
    function priceOfLP(address LP) public view returns (uint256) {
        ...
        address backing0 = fetchBackingStable(token0);
        address backing1 = fetchBackingStable(token1);

        // fetch prices of tokens
        uint256 price0 = priceOf(token0, backing0);
        uint256 price1 = priceOf(token1, backing1);
