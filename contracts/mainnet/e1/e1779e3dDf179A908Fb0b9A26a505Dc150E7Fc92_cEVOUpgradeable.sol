...
ERC20PermitUpgradeable, ERC20BurnableUpgradeable, OldTokenConstants {
    ...
    function pendingOf(address _address) public view returns(uint256) {
        ...
        if ((s.balance + locked) > 0) {
            ...
            uint256 pendingPerSecond = (s.amount + locked) / (s.duration > 0 ? s.duration : 365 days);
            uint256 pending = block.timestamp - ((s.startTime > 0 ? s.startTime : 1681860918) * pendingPerSecond);
            if (claimed < pending) {
