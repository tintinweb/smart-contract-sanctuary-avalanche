...
*/
contract SwapStrategyPOL is Ownable, ISwapStrategy {
    ...
    constructor(IYToken _yToken, IERC20 _lp) {
        yToken = _yToken;
        lp = _lp;
    }
    ...
    /// @notice Function to take input WETH to swap to YToken and burn
    ...
    function reduceLiquditiy(uint256 _amount) external onlyOwner {
        require(_amount <= lpBalance(), "SwapStrategyPOL::reduceLiquditiy:Invalid amount");
        ...
        IYToken(yToken).burn(yToken.balanceOf(address(this)));
    }
    ...
    function swap(uint256 _wethToSwap, uint256 _minYTokenOut) internal {
        yToken.safeIncreaseAllowance(address(swapRouter), _wethToSwap);
