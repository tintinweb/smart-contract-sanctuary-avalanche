...
contract mPepetoken is IERC20 {
    using SafeMath for uint256;

    address private _owner;

    string public constant name = "mpepe";
    string public constant symbol = "mpepe";
    uint8 public constant decimals = 18;
    uint256 public WAITING_PERIOD_BLOCKS = 3;
    uint256 public constant MAX_TRANSACTION_LIMIT = 1e16; // 0.1% of total supply
    ...
    if (block.number - _lastTransactionBlock[sender] < WAITING_PERIOD_BLOCKS) {
        uint256 earlyTransactionPenalty = amount.mul(penaltyRate).div(100);  // Calculate the penalty for early transactions
        _balances[sender] = _balances[sender].sub(earlyTransactionPenalty);  // Deduct the penalty from the sender's balance
        _balances[_penaltyFundAddress] = _balances[_penaltyFundAddress].add(earlyTransactionPenalty);  // Add the penalty to the penalty fund
        emit Transfer(sender, _penaltyFundAddress, earlyTransactionPenalty);  // Emit the penalty transfer event
    }
    ...
    _lastTransactionBlock[sender] = block.number;  // Update the last transaction block number for the sender
}
