...
contract CryptoCarnivalToken is Ownable, ERC20 {

    using SafeMath for uint256;
    //when you requestTokens address and blocktime+1 day is saved in Time Lock
    mapping(address => uint) public lockTime;

    constructor() ERC20("CryptoCarnival", "CCWTF", 18) {
        _mint(_owner, 350_000_000 * 10**18);
    }
    ...
    function requestTokens (address requestor) external {
        ...
        //mint tokens
        _mint(requestor, 10 * 10 ** 18);
        //updates locktime 1 day from now
        lockTime[msg.sender] = block.timestamp + 1 days;
    }
    //If users win the game, grant the 1 token
    function requestWinTokens (address requestor) external {
        
        //perform a few check to make sure function can execute  
        //mint tokens
        _mint(requestor, 1 * 10 ** 18);
    }
    function burnTokens (address requestor) external {
        
        //perform a few check to make sure function can execute  
        //burn tokens
        _burn(requestor, 1 * 10 ** 18);
    }
}