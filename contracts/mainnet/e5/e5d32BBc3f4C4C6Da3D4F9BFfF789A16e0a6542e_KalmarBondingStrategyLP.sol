...
contract KalmarBondingStrategyLP is ReentrancyGuard, Ownable, Pausable {
    ...
    function _usdTokenPrice() internal view returns (uint256) {
      IChainlinkAggregator source = IChainlinkAggregator(chainlink);
    ...
    function _kalmPrice() internal view returns (uint256) {
        ...
        uint256 resOther = otherReserve*(10**18);

        // amount of
        uint256 otherPERkalm = ((1e18*resOther)/kalmReserve)/1e18;
        uint256 kalmPrice = (otherPERkalm*_usdTokenPrice())/1e18;
    ...
    function _getLpPrice() internal view returns(uint256)
    {
      ...
      uint256 totalOtherPrice = (otherReserve * _usdTokenPrice());
