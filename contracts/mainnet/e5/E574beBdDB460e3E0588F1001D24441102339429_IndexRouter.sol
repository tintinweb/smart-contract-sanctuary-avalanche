...
contract IndexRouter is IIndexRouter, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Man and min allowed diffs between amounts of constituents in base (2%)
    uint internal constant MIN_DEVIATION = 9_800;
    uint internal constant MAX_DEVIATION = 10_200;
    ...
    function initialize(
        ...
        require(_WETH != address(0) && _helper != address(0), "IndexRouter: ZERO");
    ...
    function mintSwapIndexAmount(MintSwapParams calldata _params) external view override returns (uint _amount) {
        ...
        for (uint i; i < assets.length; ++i) {
            amountInBase += _params.quotes[i].buyAssetMinAmount.mulDiv(
                FixedPoint112.Q112,
                priceOracle.lastAssetPerBaseInUQ(assets[i])
            );
        }

        uint8 _indexDecimals = IERC20Metadata(_params.index).decimals();
        if (IERC20(_params.index).totalSupply() != 0) {
            (uint lastEvaluation, ) = IIndexHelper(helper).totalEvaluation(_params.index);

            _amount =
                (priceOracle.convertToIndex(amountInBase, _indexDecimals) * IERC20(_params.index).totalSupply()) /
                priceOracle.convertToIndex(lastEvaluation, _indexDecimals);
        } else {
            _amount = priceOracle.convertToIndex(amountInBase, _indexDecimals) - IndexLibrary.INITIAL_QUANTITY;
        }
    ...
    function burnTokensAmount(address _index, uint _amount) external view override returns (uint[] memory _amounts) {
        (address[] memory _assets, ) = IIndex(_index).anatomy();
        address[] memory _inactiveAssets = IIndex(_index).inactiveAnatomy();
        _amounts = new uint[](_assets.length + _inactiveAssets.length);

        _amount -=
            (_amount * IFeePool(IIndexRegistry(registry).feePool()).burningFeeInBPOf(_index)) /
            BP.DECIMAL_FACTOR;

        uint nativeReserveAmount;
        uint burnAmount;
        {
            uint valueInAsset;
            {
                (uint totalEvaluation, ) = IIndexHelper(helper).totalEvaluation(_index);
                uint valueInBase = _amount.mulDiv(totalEvaluation, IERC20(_index).totalSupply());

                IPhuturePriceOracle oracle = IPhuturePriceOracle(IIndexRegistry(registry).priceOracle());
                valueInAsset = valueInBase.mulDiv(oracle.lastAssetPerBaseInUQ(WETH), FixedPoint112.Q112);
            }

            address vToken = IvTokenFactory(IIndex(_index).vTokenFactory()).vTokenOf(WETH);
            (uint assetAmount, ) = IvTokenV2(vToken).burnReserveInfo(valueInAsset);

            burnAmount = _amount.mulDiv(assetAmount, valueInAsset);
            _amount -= burnAmount;
            nativeReserveAmount = assetAmount;
        }

        uint totalAssetCount = _assets.length + _inactiveAssets.length;
        for (uint i; i < totalAssetCount; ) {
            address asset = i < _assets.length ? _assets[i] : _inactiveAssets[i - _assets.length];
            IvToken vToken = IvToken(IvTokenFactory(IIndex(_index).vTokenFactory()).vTokenOf(asset));

            uint indexAssetBalance;
            if (asset == WETH) {
                uint balance = vToken.balanceOf(_index);
                _amounts[i] += nativeReserveAmount;

                indexAssetBalance = vToken.assetBalanceForShares(balance - burnAmount);
            } else {
                indexAssetBalance = vToken.assetBalanceOf(_index);
            }
        ...
        returns (uint)
    {
        ...
        uint assetCount = _assets.length;
        for (uint i; i < assetCount; ) {
            address asset = _assets[i];
            address vToken = vTokenFactory.createdVTokenOf(_assets[i]);
            uint amount = oracle.refreshedAssetPerBaseInUQ(asset).amountInAsset(_weights[i], _params.amountInBase);

            IERC20(asset).safeTransferFrom(msg.sender, vToken, amount);

            IvTokenV2(vToken).mint(amount, _params.index);
        ...
        returns (uint[] memory _amounts)
    {
        ...
        uint assetCount = _assets.length;
        address[] memory inactiveAssets = IIndex(_params.index).inactiveAnatomy();
        uint totalAssetCount = assetCount + inactiveAssets.length;
        uint[] memory balancesBefore = new uint[](totalAssetCount);
        for (uint i; i < totalAssetCount; ++i) {
            address asset = i < assetCount ? _assets[i] : inactiveAssets[i - assetCount];
            balancesBefore[i] = IERC20(asset).balanceOf(_params.recipient);
        }

        IIndex(_params.index).burn(_params.recipient);

        _amounts = new uint[](totalAssetCount);
        for (uint i; i < totalAssetCount; ++i) {
            address asset = i < assetCount ? _assets[i] : inactiveAssets[i - assetCount];
    ...
    function _mint(
        ...
        uint inputBalanceBefore = IERC20(_inputToken).balanceOf(address(this));
        (address[] memory assets, uint8[] memory weights) = index.anatomy();

        uint firstTotalInvestedAmountInBase;

        for (uint i; i < assets.length; ) {
            address asset = assets[i];

            require(asset == _quotes[i].asset, "IndexRouter: INVALID");

            // if one of the assets is inputToken we transfer it directly to the vault
            uint amount = asset == _inputToken
                ? _quotes[i].buyAssetMinAmount
                : _swap(_inputToken, _amountInInputToken, _quotes[i]);

            {
                uint consituentAmountInBase = amount.mulDiv(
                    FixedPoint112.Q112,
                    priceOracle.refreshedAssetPerBaseInUQ(asset)
                );

                if (i == 0) {
                    firstTotalInvestedAmountInBase = (consituentAmountInBase * IndexLibrary.MAX_WEIGHT) / weights[i];
                } else {
                    uint totalInvestedAmountInBase = (consituentAmountInBase * IndexLibrary.MAX_WEIGHT) / weights[i];
                    uint totalInvestmentDeviation = (firstTotalInvestedAmountInBase * BP.DECIMAL_FACTOR) /
                        totalInvestedAmountInBase;

                    require(
                        totalInvestmentDeviation >= MIN_DEVIATION && totalInvestmentDeviation <= MAX_DEVIATION,
                        "IndexRouter: DEVIATION"
                    );
                }
            }

            address vToken = vTokenFactory.createdVTokenOf(asset);

            IERC20(asset).safeTransfer(vToken, amount);
            IvTokenV2(vToken).mint(amount, address(index));
        ...
        require(
            inputBalanceBefore - IERC20(_inputToken).balanceOf(address(this)) == _amountInInputToken,
    ...
    function _burn(BurnSwapParams calldata _params) internal {
        ...
        IPriceOracle priceOracle = IPhuturePriceOracle(IIndexRegistry(registry).priceOracle());
        for (uint i; i < totalAssetCount; ) {
            IERC20 inputAsset = IERC20(i < assetCount ? assets[i] : inactiveAssets[i - assetCount]);
    ...
    /// @notice Swaps input asset to constituent
    /// @param _input Address of input asset
    /// @param _inputAmount Amount of input asset
    /// @param _quote Qoute for swap
    function _swap(
        address _input,
        uint _inputAmount,
        MintQuoteParams calldata _quote
    ) internal returns (uint _outputAmount) {
        require(
            IAccessControl(registry).hasRole(EXCHANGE_TARGET_ROLE, _quote.swapTarget),
            "IndexRouter: INVALID_TARGET"
        );

        uint assetBalanceBefore = IERC20(_quote.asset).balanceOf(address(this));

        _safeApprove(_input, _quote.swapTarget, _inputAmount);
        _fillQuote(_quote.swapTarget, _quote.assetQuote);

        _outputAmount = IERC20(_quote.asset).balanceOf(address(this)) - assetBalanceBefore;

        require(_outputAmount >= _quote.buyAssetMinAmount, "IndexRouter: UNDERBOUGHT_ASSET");
    }
