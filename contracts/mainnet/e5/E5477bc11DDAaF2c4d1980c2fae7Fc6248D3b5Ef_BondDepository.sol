...
contract BondDepository is AccessControlled, ReentrancyGuardUpgradeable {
	...
	struct Terms {
		uint256 price; // price of token in asset (100 tokens = 100 assets)
	...
	function setTerms(
		...
		terms = Terms({ price: price_, minAmount: minAmount_, maxAmount: maxAmount_, capacity: capacity_, duration: duration_, maxBonds: maxBonds_ });
		if (!inited) {
			inited = true;			
	...
	//
	function whiteListAccounts(address[] memory accounts_, uint256[] memory amounts_) external onlyOperator {
		require(accounts_.length == amounts_.length, "==");
		for (uint256 i = 0; i < accounts_.length; i++) {
			whiteList[accounts_[i]] = amounts_[i];
		}
		emit WhiteListed(accounts_, amounts_);
	}

	//
	function whiteListEnable(bool state_) external onlyOperator {
		whiteListEnabled = state_;
		emit WhiteListEnable(state_);
	}
	...
	function payoutFor(uint256 value_) public view returns (uint256 _payout) {
		_payout = (value_ * (10**dlc.decimals())) / treasury.tokenValue(address(asset), terms.price);		
	}
	...
	function buy(
		uint256 amount_		
	) external onlyInited nonReentrant returns (uint256 _payout) {
		...
		if (whiteListEnabled) {
			require(whiteList[msg.sender] >= _payout, "White listed amount reached");
			whiteList[msg.sender] -= _payout;
		} 
		...
		emit Buy(msg.sender, amount_, terms.price, _payout, terms.duration);
	}
	...
	event WhiteListed(address[] accounts, uint256[] amounts);
	event WhiteListEnable(bool state);
...
contract DLC is IDLC, ERC20Upgradeable, AccessControlled {
    ...
    function newMethod() view public returns(uint8){
