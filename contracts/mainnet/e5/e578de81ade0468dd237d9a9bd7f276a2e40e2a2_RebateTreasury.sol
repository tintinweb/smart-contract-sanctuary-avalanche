...
contract RebateTreasury is Ownable {
    ...
    address public constant BUSD = 0xfA1DCF8369fF71AFe0fc2b57e1d103Ab931766c7;
    ...
    constructor(address tornado, address tornadoOracle, address treasury) {
        Tornado = IERC20(tornado);
        TornadoOracle = IOracle(tornadoOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Tornado at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 tornadoAmount = getTornadoReturn(token, amount);
        require(tornadoAmount <= Tornado.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient tornado balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + tornadoAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += tornadoAmount;
    }

    // Claim available Tornado rewards from bonding
    ...
    function setTornado(address tornado) external onlyOwner {
        Tornado = IERC20(tornado);
    }

    // Set Tornado oracle

    function setTornadoOracle(address oracle) external onlyOwner {
        TornadoOracle = IOracle(oracle);
    }

    // Set Tornado treasury
    ...
    function _claimVested(address account) internal {
        ...
        Tornado.transfer(account, claimable);
    }
    ...
    // Calculate Tornado return of bonding amount of token

    function getTornadoReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 tornadoPrice = getTornadoPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / tornadoPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 tornadoPrice = getTornadoPrice();
        if (tornadoPrice < 1e18) return 0;

        uint256 tornadoPremium = tornadoPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (tornadoPremium < bondThreshold) return 0;
        if (tornadoPremium <= secondaryThreshold) {
            return (tornadoPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (tornadoPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get REBEL price from Oracle

    function getTornadoPrice() public view returns (uint256) {
        return TornadoOracle.consult(address(Tornado), 1e18);
    }
    ...
    // Get claimable vested Tornado for account

    function claimableTornado(address account) external view returns (uint256) {
