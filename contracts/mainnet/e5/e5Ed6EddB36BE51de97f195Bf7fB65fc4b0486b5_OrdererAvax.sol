...
contract OrdererAvax is IOrderer, IOrdererV2, UUPSUpgradeable, ERC165Upgradeable {
    ...
    function externalSwap(ExternalSwapV2 calldata _info) external override onlyRole(ORDERING_EXECUTOR_ROLE) {
        ...
        if (
            ...
            uint boughtAmount;
            {
                soldAmount = sellBalanceBefore - IERC20(_details.sellAsset).balanceOf(address(this));
                uint soldAmountInBase = soldAmount.mulDiv(FixedPoint112.Q112, sellAssetPerBase);

                // checks diff between input and swap amounts
                require(
                    sellAmount.mulDiv(FixedPoint112.Q112, sellAssetPerBase) - soldAmountInBase <= MIN_SWAP_AMOUNT,
                    "OrdererAvax: AMOUNT"
                );

                boughtAmount = IERC20(_details.buyAsset).balanceOf(address(this));
            ...
            IvTokenV2(address(_details.sellVToken)).utilizeReserve(soldAmount);

            uint248 _buyShares = uint248(
                Math.min(
                    IvTokenV2(address(_details.buyVToken)).mintFor(boughtAmount, _info.account),
            ...
            if (change > 0) {
                ...
                IvTokenV2(address(_details.sellVToken)).mintFor(change, _info.account);
            }
