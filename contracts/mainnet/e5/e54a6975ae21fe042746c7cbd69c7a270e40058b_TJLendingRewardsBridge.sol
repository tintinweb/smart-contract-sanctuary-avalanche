...
// File: contracts/prize/TraderJoeLending/TJLendingRewardsBridge.sol
...
contract TJRewardDistributor {
    function claimReward(uint8 rewardType, address payable holder) public {}
}

contract TJLendingRewardsBridge is ReentrancyGuard {
    using LowGasSafeMath for uint256;

    IExchangeBridge private exchangeBridge;
    TJRewardDistributor private tjRewardsDistributor;
    IERC20 private joeToken;

    constructor(
        address _exchangeBridgeContract,
        address _tjRewardsDistributorContract,
        address _joeToken
    ) {
        exchangeBridge = IExchangeBridge(_exchangeBridgeContract);
        tjRewardsDistributor = TJRewardDistributor(_tjRewardsDistributorContract);
        joeToken = IERC20(_joeToken);
    }

    //must be called with delegated call
    function claimRewardsInTargetToken(address targetToken) external nonReentrant returns (uint256) {

        uint256 joeBalanceBefore = joeToken.balanceOf(address(this));
        tjRewardsDistributor.claimReward(0, payable(address(this)));
        uint256 joeBalanceAfter = joeToken.balanceOf(address(this));
        uint256 joeTotalToconvert = joeBalanceAfter.sub(joeBalanceBefore);
        joeToken.transfer(address(exchangeBridge), joeTotalToconvert);

        IERC20 targetTokenERC20 = IERC20(targetToken);
        uint256 targetTokenBalanceBefore = targetTokenERC20.balanceOf(address(this));
        exchangeBridge.exchangeExactTokensForTokens(joeTotalToconvert, address(joeToken), targetToken);
