...
// File contracts/fee-collector/PGLConverter.sol
...
contract PGLConverter is AccessControl {
    ...
    function _pullLiquidity(address pair, uint256 balance) private returns (uint256 amount0, uint256 amount1) {
        IERC20(pair).safeTransferFrom(msg.sender, pair, balance);
    ...
    /// @notice Converts a list of liquidity pairs into an ERC20 token
    /// @param liquidityPairs - list of all the pairs to convert
    /// @param outputToken - the token into which all liquidity will be swapped
    function convert(IPangolinPair[] memory liquidityPairs, address outputToken, address to) external onlyRole(ADMIN) {
        for (uint256 i; i < liquidityPairs.length; ++i) {
            IPangolinPair liquidityPair = liquidityPairs[i];
            uint256 pglBalance = liquidityPair.balanceOf(msg.sender);
            if (pglBalance > 0) {
        ...
        IERC20(outputToken).safeTransfer(to, finalBalance);
    }
