...
contract MushMons is ReentrancyGuard, ERC20 {
    ...
    address public swapDEX = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;  // polygon: quickswap route
    ...
    // user address -> exposure token -> stablecoin vault share
    mapping(address => mapping(address => uint256)) public stablecoinVaultShares;
    ...
    // exposure token address -> rebalancing path from stablecoin to exposure token
    mapping(address => address[]) public rebalancingPaths;
    
    // exposure token address -> rebalancing path from exposure token to stablecoin
    mapping(address => address[]) public rebalancingPathsFromExposure;
    ...
    function amountOfStableCoin(address _user, address _exposure) public view returns (uint256) {
        return amountOfTokenForShare(_user, address(token), mintedShares[_user][_exposure], _exposure);
    }
    ...
    function amountOfExposureToken(address _user, address _exposure) public view returns (uint256) {
        return amountOfTokenForShare(_user, _exposure, mintedShares[_user][_exposure], _exposure);
    }
    
    // get token withdrawable for given _user and _exposure token and _share and _exposure
    function amountOfTokenForShare(address _user, address _token, uint256 _share, address _exposure) public view returns (uint256) {
        address _tokenVault = yieldVaults[_user][_token];
        uint256 _shareTotal = mintedShares[_user][_exposure];
        
        require(_tokenVault != address(0), "!invalidVault");
        require(_share <= _shareTotal, "!invalidShare");
        
        uint256 _yieldShare = _token == address(token)? stablecoinVaultShares[_user][_exposure] : yieldVaultShares[_user][_tokenVault];
    ...
    function setRebalancingPath(address _exposure, address[] calldata _path) public onlyGovernance{
        require(_path.length >= 2, "!invalidPath");
        rebalancingPaths[_exposure] = _path;
    }

    function setRebalancingPathFromExposure(address _exposure, address[] calldata _path) public onlyGovernance{
        require(_path.length >= 2, "!invalidPath");
        rebalancingPathsFromExposure[_exposure] = _path;
    }
    ...
    function addExposureToken(address _token) public {
        ...
        path[1] = _token;
        rebalancingPaths[_token] = path;
        
        address[] memory pathFromExposure = new address[](2);
        pathFromExposure[0] = _token;
        pathFromExposure[1] = address(token);
        rebalancingPathsFromExposure[_token] = pathFromExposure;
    }
    ...
    function initMushMon(address _exposureToken, 
        ...
        if (yieldVaults[msg.sender][address(token)] != _tokenVault){
            changeYieldFarms(address(token), _tokenVault);
        }
        if (yieldVaults[msg.sender][_exposureToken] != _exposureVault){
    ...
    function changeYieldFarms(address _token, address _vault) public {
        ...
        require(yieldVaultShares[msg.sender][_tokenVault] <= 0, '!withdrawAllFirstBeforeChange');
    ...
    function deposit(uint256 _amount, address _exposure, uint256 _exposureAmount) public nonReentrant {
        ...
        if (tr.increase){
            _yieldFarmingDeposit(msg.sender, address(token), yieldVaults[msg.sender][address(token)], tr.amount, _exposure);
            _yieldFarmingDeposit(msg.sender, _exposure, yieldVaults[msg.sender][_exposure], delta, _exposure);
        } else{
            _yieldFarmingDeposit(msg.sender, address(token), yieldVaults[msg.sender][address(token)], delta, _exposure);
            _yieldFarmingDeposit(msg.sender, _exposure, yieldVaults[msg.sender][_exposure], tr.amount, _exposure);
        }
    ...
    function rebalance(address _user, address _exposure, bytes calldata _extraRebalancingData) public nonReentrant {
        ...
        if (tr.increase){
            ...
            _yieldFarmingWithdraw(_user, address(token), yieldVaults[_user][address(token)], tr.amount, _exposure);
            ...
            _yieldFarmingWithdraw(_user, _exposure, yieldVaults[_user][_exposure], tr.amount, _exposure);
        ...
        if (delta > 0){
            if (tr.increase){
                _yieldFarmingDeposit(_user, _exposure, yieldVaults[_user][_exposure], delta, _exposure);
            } else{
                _yieldFarmingDeposit(_user, address(token), yieldVaults[_user][address(token)], delta, _exposure);
            }
        }
    }
    ...
    function withdraw(uint256 _shares, address _exposure, uint256 _withdrawType) public nonReentrant {
        ...
        uint256 _stablecoinAmt = amountOfTokenForShare(msg.sender, address(token), _shares, _exposure);
        uint256 _exposureAmt = amountOfTokenForShare(msg.sender, _exposure, _shares, _exposure);
        ...
        if (_stablecoinAmt > 0) {
            require(yieldVaults[msg.sender][address(token)] != address(0), '!invalidYieldVault');
            _yieldFarmingWithdraw(msg.sender, address(token), yieldVaults[msg.sender][address(token)], _stablecoinAmt, _exposure);
        }
        
        if (_exposureAmt > 0) {
            require(yieldVaults[msg.sender][_exposure] != address(0), '!invalidYieldVaultExposure');
            _yieldFarmingWithdraw(msg.sender, _exposure, yieldVaults[msg.sender][_exposure], _exposureAmt, _exposure);
        }
    ...
    function _yieldFarmingDeposit(address _user, address _token, address _tokenVault, uint256 _amount, address _exposure) internal returns(uint256){
        ...
        if (_amount > 0){
            ...
            if (_token == address(token)){
                stablecoinVaultShares[_user][_exposure] = stablecoinVaultShares[_user][_exposure].add(_vaultShare);
            }
        }
        return _vaultShare;
    }
    ...
    function _yieldFarmingWithdrawShare(address _user, address _token, address _tokenVault, uint256 _share, address _exposure) internal returns(uint256){
        require(_tokenVault != address(0) && validYieldFarms[_token][_tokenVault], '!noYieldVault');
        uint256 _actual;
        if (_share > 0){
            ...
            if (_token == address(token)){
                stablecoinVaultShares[_user][_exposure] = stablecoinVaultShares[_user][_exposure].sub(_share);
            }
    ...
    function _yieldFarmingWithdraw(address _user, address _token, address _tokenVault, uint256 _want, address _exposure) internal returns(uint256){
        uint256 _vaultShare = _convertToMToken(_tokenVault, _want);
        return _yieldFarmingWithdrawShare(_user, _token, _tokenVault, _vaultShare, _exposure);
    }
    ...
    function _swapInDex(address _from, address _to, uint256 _amountIn, uint256 _markPrice, bool _fromExposure) internal returns(uint256 amountOutput){
        ...
        address[] memory path = _fromExposure? rebalancingPathsFromExposure[_from] : rebalancingPaths[_to];
