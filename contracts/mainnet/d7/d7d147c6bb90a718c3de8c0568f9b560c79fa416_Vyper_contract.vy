...
AVWETH: constant(address) = 0x53f7c5869a859F0AeC3D334ee8B4Cf01E3492f21
...
def __init__(_receiver: address, _owner: address, _tricrypto: address):
    ...
    ERC20(AVWETH).approve(_tricrypto, MAX_UINT256)
...
def burn(_coin: address) -> bool:
    ...
    if _coin != AVWETH:
        ...
        RegistrySwap(registry_swap).exchange_with_best_rate(_coin, AVWETH, amount, 0)
        amount = ERC20(AVWETH).balanceOf(self)

    # AVWETH -> av3CRV
    CryptoSwap(self.tricrypto).exchange(2, 0, amount, 0)
