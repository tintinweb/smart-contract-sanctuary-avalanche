...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x9e76dbab956f3F844a169772D0c4249e31390181)
    ];
    ...
    function initialize(
        ...
        gravePriceOne = 10**17; // This is to allow a PEG of 1 GRAVE per wAVAX
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochGravePrice > gravePriceCeiling) {
                ...
                uint256 _mse = _calculateMaxSupplyExpansionPercent(graveSupply).mul(1e13);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForBoardroom = graveSupply.mul(_percentage).div(1e17);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = graveSupply.mul(_percentage).div(1e17);
