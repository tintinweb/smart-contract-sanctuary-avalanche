 ...
 */
 contract Presale is Owned{
    ...
    uint public immutable RATE_DECIMALS = 10 ** 12;

    uint256 public minUsdcLimit = 150 * (10 ** 6);
    uint256 public maxUsdcLimit = 4500 * (10 ** 6);
    ...
    function buy(uint _amount) public {
        ...
        usdc.safeTransferFrom(msg.sender, wallet, _amount);

        // calculate pToken amount to be created
        uint256 pTokenAmount = _amount.div(rate).mul(RATE_DECIMALS);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        // uint totalpTokenToClaim = preBuys[_account].usdcAmount.div(rate).mul(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
        uint totalpTokenToClaim = preBuys[_account].usdcAmount.div(rate).mul(RATE_DECIMALS);
    ...
    function claim() public {
        ...
        // uint totalpTokenToClaim = preBuys[msg.sender].usdcAmount.div(rate).mul(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
        uint totalpTokenToClaim = preBuys[msg.sender].usdcAmount.div(rate).mul(RATE_DECIMALS);
