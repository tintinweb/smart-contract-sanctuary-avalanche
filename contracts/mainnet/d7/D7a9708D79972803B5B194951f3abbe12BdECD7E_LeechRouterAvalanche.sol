...
contract LeechRouterAvalanche is LeechRouterBase {
    ...
    function initialize(
        ...
        // Init main pathes.
        // USDC > USDT
        routes[IERC20(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E)][
        ...
        // USDT > USDC
        routes[IERC20(0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7)][
    ...
    function _swap(
        ...
        // Get path
        address[] memory path = routes[tokenIn][tokenOut];
        // Prepare array for bins
        uint256[] memory pairBinSteps = new uint256[](path.length - 1);
        // Get bins
        pairBinSteps = _getBins(path);
        // Swap
        swapedAmount = ILBRouter(uniV2).swapExactTokensForTokens(
            amount,
            0,
            pairBinSteps,
            path,
    ...
    /**
     * @dev Receive data from JoeTrader V2 for swap.
     * @param path Path to tokenOut.
     * @return bins Data from JoeTrader.
     */
    function _getBins(
        address[] memory path
    ) private view returns (uint256[] memory bins) {
        address factory = ILBRouter(uniV2).factory();
    ...
    ILeechRouter
{
    ...
    /// @notice For services (EOA or multi-sig).
    ...
    modifier isActivePool(uint16 poolId) {
        if (poolIdtoActiveStrategy[poolId].chainId == 0)
    ...
    modifier checkDepositAmount(IERC20 depositToken, uint256 amount) {
        if (amount < depositMinAmount[address(depositToken)] || amount == 0)
            revert AmountTooLow();
        _;
    }
    ...
    function initialize(
        ...
        // Set initial params
        (baseToken, uniV2, chainId, treasury, signer) = (
            _baseToken,
            _uniV2Address,
            _chainId,
            _treasury,
     ...
     * @param maxBlockNumber Max block for the signature expiring.
     * @param data Additional pool data.
     * @dev Path parameter can have two options:
     * tokenInToToken0 if current chain is active strategy
     * tokenInToBase if cerrent chain isn't active and bridging reqiured
     */
    function deposit(
    ...
    {
        // Get active strategy
        Strategy memory activeStrat = poolIdtoActiveStrategy[poolId];
        // Check crosschain
        if (chainId != activeStrat.chainId) revert CrosschainError();
        ...
        uint256 deposited = IStrategyMasterchefFarmV2(
            activeStrat.strategyAddress
        ).deposit(msg.sender, depositToken, data);
        // Notify watchers
        emit Deposited(
            msg.sender,
            poolId,
        ...
        checkDepositAmount(depositToken, amount)
    {
        ...
        transporter.bridgeOut{value: msg.value}(
            ...
            amount,
            activeStrat.chainId,
            chainIdToRouter[activeStrat.chainId].routerAddress
        );
        // Notify watchers
        emit BaseBridged(
            msg.sender,
        ...
        checkWhitelist(signature, maxBlockNumber)
    {
        ...
        if (chainId == activeStrat.chainId) {
            // if current chain is active, withdraw from strategy
            uint256 amount = IStrategyMasterchefFarmV2(
                activeStrat.strategyAddress
            ).withdraw(msg.sender, shares, tokenOut, data);
            ...
            address payable _to = payable(getSigner());
        ...
        zeroAddr(user)
    {
        ...
        uint256 tokenOutAmount = IStrategyMasterchefFarmV2(
            activeStrat.strategyAddress
        ).withdraw(user, shares, tokeOut, data);
        ...
        if (targetChainId == chainId) {
            ...
            baseToken.safeTransfer(user, tokenOutAmount);
            emit WithdrawCompleted(
                user,
                poolId,
                activeStrat.id,
            ...
            baseToken.safeTransfer(address(transporter), tokenOutAmount);
    ...
    function initMigration(
        ...
        payable
        onlyRole(ADMIN_ROLE)
        zeroAddr(_strategy.strategyAddress)
    {
        ...
        IStrategyMasterchefFarmV2(_currentStrategy.strategyAddress).migrate(
            slippage
        );
        ...
        if (_strategy.chainId == chainId) {
            ...
            IStrategyMasterchefFarmV2(_strategy.strategyAddress).deposit(
            ...
            IERC20 bridgedToken = IERC20(path[path.length - 1]);

            bridgedToken.safeTransfer(address(transporter), withdrawAmount);

            if (baseToken != bridgedToken) {
                withdrawAmount = _swap(withdrawAmount, bridgedToken, baseToken);
            }
    ...
    function finalizeCrosschainMigration(
        ...
        Strategy calldata _strategy,
        uint256 baseAmount
    ) external onlyRole(ADMIN_ROLE) zeroAddr(_strategy.strategyAddress) {
        require(_strategy.poolId == poolId, "wrong parameters");
    ...
    function setStrategy(
        ...
        require(_strategy.withdrawalFee <= FEE_MAX, "withdrawalFee <= FEE_MAX");
        poolIdtoActiveStrategy[_strategy.poolId] = _strategy;
    }
    ...
    ///@notice Should be used for bridging commision withdrawing
    function rescueNative(address payable _to) external onlyRole(ADMIN_ROLE) {
    ...
    function quotePotentialWithdraw(
        ...
        address stratAddress = poolIdtoActiveStrategy[poolId].strategyAddress;
        amountOut = IStrategyMasterchefFarmV2(stratAddress)
    ...
    function userAllocation(
        ...
        address stratAddress = poolIdtoActiveStrategy[poolId].strategyAddress;
        allocation = IStrategyMasterchefFarmV2(stratAddress).allocationOf(user);
    }
    ...
    function totalAllocation(
        ...
        address stratAddress = poolIdtoActiveStrategy[poolId].strategyAddress;
        allocation = IStrategyMasterchefFarmV2(stratAddress).totalAllocation();
    }
