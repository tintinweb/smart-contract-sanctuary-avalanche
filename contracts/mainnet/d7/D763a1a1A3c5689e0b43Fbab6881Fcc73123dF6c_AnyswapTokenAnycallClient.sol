...
abstract contract AnycallClientBase is PausableControlWithAdmin {
    ...
    modifier onlyCallProxy() {
        require(msg.sender == IAnycallV6Proxy(callProxy).executor(), "AnycallClient: not authorized");
        _;
    }
    ...
    receive() external payable {
        require(msg.sender == IAnycallV6Proxy(callProxy).executor(), "AnycallClient: receive from forbidden sender");
    }
