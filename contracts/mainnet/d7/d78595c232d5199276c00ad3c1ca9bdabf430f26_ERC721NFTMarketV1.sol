...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
     ...
     * @param _WAVAXAddress: WAVAX address
    ...
    constructor(
        ...
        require(_WAVAXAddress != address(0), "Operations: WAVAX address cannot be zero");
    ...
    /**
     * @notice Buy token with AVAX by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     */
    function buyTokenUsingAVAX(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap AVAX
        IWETH(WAVAX).deposit{value: msg.value}();

        _buyToken(_collection, _tokenId, msg.value, true);
    }

    /**
     * @notice Buy token with WAVAX by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingWAVAX(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(WAVAX).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(WAVAX).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != WAVAX, "Operations: Cannot recover WAVAX");
     ...
     * @param _withAVAX: whether the token is bought with AVAX (true) or WAVAX (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
        ...
        // Transfer WAVAX
        IERC20(WAVAX).safeTransfer(askOrder.seller, netPrice);
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withAVAX);
    }
