...
contract BladesXGlory is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties, RandomRequest, VRFConsumerBaseV2{
    using Strings for uint256;

    uint256 public constant MAX_MINTABLE = 60; //  60 total for this launchpad
    uint256 public constant MAX_PER_CLAIM = 10;
    uint256 public constant ROYALTY_VALUE = 690; // 100 = 1%, 200 = 2% etc...
    uint256 public constant MAX_EARLY_CLAIM = 5;
    uint256 public constant MAX_PUBLIC_CLAIM = 5;
    uint256 public constant NFT_EARLY_PRICE = 1 ether; // 1 avax
    uint256 public constant NFT_PRICE = 1 ether; // 1 avax
...
//
    ...
    function fulfillRandomWords(uint requestId, uint[] memory randomWords) internal override {
        for (uint i = 0; i < randomWords.length; i++) {
            ...
            uint newTokenId = _randomize(randomNum) + mintIndexStart;
            address ownerAddress = s_requestIdToSender[requestId];
            _safeMint(ownerAddress, newTokenId);
            _setTokenRoyalty(newTokenId, royaltyRecipient, ROYALTY_VALUE);

            emit Claim(newTokenId);
        }
...
/////////////////////////////////////////////////////////////
    ...
    constructor(
        ...
        bytes32 gasLine,
        uint64 subId
    ) ERC721(nftName, nftSymbol) Ownable() RandomRequest(MAX_MINTABLE) VRFConsumerBaseV2(vrfCoordinator){
...
//
    function earlyClaim(uint256 n, bytes32[] calldata _proof) public payable{
        ...
        uint requestId = getRandomAvailableToken(uint32(n));
...
//
    function claim(uint256 n) public payable {
        ...
        require(n > 0, "Number need to be higher than 0");
        require(n <= MAX_PER_CLAIM, "you can't claim that much at ounce");
        require(publicMintedAmount[msg.sender] < MAX_PUBLIC_CLAIM, "exceeds the public minting limit");
        require(n + publicMintedAmount[msg.sender] <= MAX_PUBLIC_CLAIM, "you can't claim that much");
        require(
...
//
    ...
    function premintRandom(uint256 n, address adr) public onlyOwner {
        ...
        uint requestId = getRandomAvailableToken(uint32(n));
        s_requestIdToSender[requestId] = adr;
...
//
    ...
    function set_i_callBackGasLimit(uint32 newLimit) external onlyOwner{
...
contract RandomRequest {
    ...
    function _randomize(uint256 rand) internal virtual returns (uint256) {
        uint256 scope = _scopeCap - _scopeIndex;
        uint256 swap;
        uint256 result;

        uint256 i = rand % scope;
