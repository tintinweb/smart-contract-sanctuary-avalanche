...
contract vToken is IvToken, IvTokenV2, Initializable, ReentrancyGuardUpgradeable, ERC165Upgradeable {
    ...
    /// @notice Role granted for router contracts
    bytes32 internal immutable ROUTER_ROLE = keccak256("ROUTER_ROLE");
    ...
    function setController(address _vaultController) external override onlyRole(RESERVE_MANAGER_ROLE) {
        ...
        if (controller != address(0)) {
            _updateDeposited(0, IVaultController(controller).withdraw());
        }
    ...
    function withdraw() external override onlyRole(RESERVE_MANAGER_ROLE) {
        ...
        require(controller != address(0), "vToken: ZERO");
    ...
    function deposit() external override onlyRole(ORACLE_ROLE) {
        ...
        uint withdrawn = IVaultController(controller).withdraw();
        if (withdrawn > 0) {
            _updateDeposited(0, withdrawn);
        }

        uint amount = (IERC20(asset).balanceOf(address(this)) * IVaultController(controller).depositRate()) /
            BP.DECIMAL_FACTOR;

        IERC20(asset).safeTransfer(controller, amount);

        IVaultController(controller).deposit();
        _updateDeposited(amount, 0);
    }
    ...
    /// @inheritdoc IvTokenV2
    function mint(uint amount, address index)
        external
        override
        nonReentrant
    ...
    /// @inheritdoc IvTokenV2
    function mintFor(uint amount, address recipient)
        external
        override
        nonReentrant
        onlyRole(ORDERER_ROLE)
        returns (uint)
    {
        return _mint(amount, recipient);
    }
    ...
    /// @inheritdoc IvTokenV2
    function increaseReserve(uint value) external override nonReentrant onlyRole(INDEX_ROLE) {
        reserve += Math.min(value, assetBalanceOf(msg.sender));
    }

    /// @inheritdoc IvTokenV2
    function decreaseReserve(uint value) external override nonReentrant onlyRole(INDEX_ROLE) {
        reserve -= Math.min(value, reserve);
    }

    /// @inheritdoc IvTokenV2
    function utilizeReserve(uint value) external override nonReentrant onlyRole(ORDERER_ROLE) {
        uint currentReserve = reserve;
        if (value > 0 && currentReserve > 0) {
            reserve -= Math.min(value, currentReserve);
        }
    }
    ...
    function virtualTotalAssetSupply() external view override returns (uint) {
        if (vaultController == address(0)) {
            return _NAV.assetBalance();
        }

        return _NAV.assetBalance() + IVaultController(vaultController).expectedWithdrawableAmount();
    }
    ...
    /// @inheritdoc IvTokenV2
    function assetBalances() external view override returns (uint128 newBalance, uint128 lastBalance) {
        return _NAV.assetBalances();
    }
    ...
    /// @inheritdoc IvToken
    function totalAssetSupply() external view override returns (uint) {
        return _NAV.assetBalance();
    }
    ...
    function assetDataOf(address _account, uint _shares) external view override returns (AssetData memory) {
        ...
        return
            AssetData({ maxShares: _shares, amountInAsset: _NAV.assetBalanceForShares(_shares, _NAV.assetBalance()) });
    }

    /// @inheritdoc IvToken
    function assetBalanceForShares(uint _shares) external view override returns (uint) {
        return _NAV.assetBalanceForShares(_shares, _NAV.assetBalance());
    }

    /// @inheritdoc IvTokenV2
    function assetsToShares(uint amount) external view override returns (uint) {
        return _NAV.assetsToShares(amount);
    }

    /// @inheritdoc IvTokenV2
    function burnReserveInfo(uint totalAmount) external view override returns (uint amount, uint shares) {
        uint availableReserve = reserve;

        // checks that reserve isn't empty
        if (availableReserve > 0) {
            // calculates amount of asset and shares to burn
            amount = Math.min(availableReserve, totalAmount);
            shares = _NAV.assetsToShares(amount);
        }
    }
        ...
        returns (uint newShares, uint oldShares)
    {
        ...
        if (_totalSupply > 0) {
            uint _assetBalance = _NAV.assetBalance();
            uint availableAssets = (oldShares * _assetBalance) / _totalSupply;
    ...
    /// @inheritdoc IvToken
    function assetBalanceOf(address _account) public view override returns (uint) {
        return _NAV.assetBalanceForShares(_NAV.balanceOf[_account], _NAV.assetBalance());
    }

    /// @inheritdoc IvToken
    function currentDepositedPercentageInBP() public view override returns (uint) {
        uint total = _NAV.assetBalance();

        return total == 0 ? 0 : (deposited * BP.DECIMAL_FACTOR) / total;
    }

    /// @inheritdoc ERC165Upgradeable
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return
            _interfaceId == type(IvTokenV2).interfaceId ||
            _interfaceId == type(IvToken).interfaceId ||
    ...
    function _mint(uint _amount, address _recipient) internal returns (uint shares) {
        shares = _NAV.mint(_amount, _recipient);
        _NAV.sync(_NAV.assetBalance() + _amount);
    }
    ...
    function _burn(address _recipient) internal returns (uint amount) {
        amount = _NAV.burn();
        _NAV.sync(_NAV.assetBalance() - _transferAsset(_recipient, amount));
    }
    ...
    function _transferAsset(address _recipient, uint _amount) internal returns (uint amount) {
        address controller = vaultController;
        uint balance = _NAV.assetBalance() - deposited;

        if (balance < _amount && controller != address(0)) {
            _updateDeposited(0, IVaultController(controller).withdraw());

            balance = _NAV.assetBalance();
        }

        amount = Math.min(_amount, balance);
        IERC20(asset).safeTransfer(_recipient, amount);
    }

    /// @notice Updates deposited value
    function _updateDeposited(uint _deposited, uint _withdrawn) internal {
        if (_deposited == 0 && _withdrawn > 0) {
            _NAV.sync(_NAV.assetBalance() - deposited + _withdrawn);
        }

        deposited = _deposited;
        emit UpdateDeposit(msg.sender, _deposited);
    }
...
library NAV {
    ...
    /// @dev packedAssetBalances128x2 contains current and previous asset balances
    struct Data {
        uint packedAssetBalances128x2;
        uint totalSupply;
    ...
    /// @param _amount Amount of asset to mint shares for
    ...
    function mint(
        ...
        if (_totalSupply != 0) {
            shares = (_amount * _totalSupply) / assetBalance(self);
        } else {
            shares = _amount - INITIAL_QUANTITY;
    ...
    /// @param self Data structure reference
    function burn(Data storage self) internal returns (uint amount) {
        uint value = self.balanceOf[address(this)];
        amount = (value * assetBalance(self)) / self.totalSupply;
    ...
    function sync(Data storage self, uint _newBalance) internal {
        require(_newBalance <= type(uint128).max, "NAV: OVERFLOW");

        self.packedAssetBalances128x2 = (self.packedAssetBalances128x2 << 128) | _newBalance;
    }
    ...
    function assetsToShares(Data storage self, uint amount) internal view returns (uint shares) {
        uint totalAssets = assetBalance(self);

        shares = totalAssets == 0 ? 0 : (amount * self.totalSupply) / totalAssets;
    }
    ...
    function assetBalanceForShares(
        ...
        return totalSupply == 0 ? 0 : (_shares * _balance) / totalSupply;
    }

    /// @notice Returns current asset balance
    /// @param self Data structure reference
    function assetBalance(Data storage self) internal view returns (uint128) {
        return uint128(self.packedAssetBalances128x2);
    }

    /// @notice Returns last asset balance
    /// @param self Data structure reference
    function lastAssetBalance(Data storage self) internal view returns (uint128) {
        return uint128(self.packedAssetBalances128x2 >> 128);
    }

    /// @notice Returns current and last asset balances
    /// @param self Data structure reference
    /// @return newBalance Current asset balance
    /// @return lastBalance Last asset balance
    function assetBalances(Data storage self) internal view returns (uint128 newBalance, uint128 lastBalance) {
        uint packedBalances = self.packedAssetBalances128x2;
        return (uint128(packedBalances), uint128(packedBalances >> 128));
    }
