...
/// @title Compounder
/// @author Vector Team
contract CompounderPTP is Initializable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    IAPRHelper public aprHelper;

    //END OF STORAGE v3

    bool public harvestBeforeWithdraw;
    ...
    function setHarvestBeforeWithdraw(bool _state) external onlyOwner {
        harvestBeforeWithdraw = _state;
    }
    ...
    function addRewardToken(address rewardToken) external onlyOwner {
        if (!isRewardToken[rewardToken]) {
            ...
            if (rewardToken == address(depositToken)) {
                harvestBeforeWithdraw = true;
            }
        }
    }
    ...
    function deleteRewardToken(uint256 index) external onlyOwner {
        if (isRewardToken[rewardTokens[index]]) {
            ...
            if (rewardTokens[index] == address(depositToken)) {
                harvestBeforeWithdraw = false;
            }
            rewardTokens.pop();
        }
    }
    ...
    function updatePoolHelper() external {
        (, , , , , , , , poolHelper) = mainStaking.getPoolInfo(assetToken);
    }
    ...
    /// @notice Set the apr helper which gives the harvestable rewards
    /// @param _aprHelper address of the avax helper
    function setAPRHelper(address _aprHelper) external onlyOwner {
        aprHelper = IAPRHelper(_aprHelper);
    }
    ...
    function totalDeposits() public view returns (uint256) {
        return IPoolHelperV4(poolHelper).balance(address(this));
    }
    ...
    {
        (, , , address lp, , , , address rewarder, ) = mainStaking.getPoolInfo(assetToken);
        (tokens, amounts, ) = aprHelper.getPTPHarvestableRewards(lp, address(this));
        uint256 length = tokens.length;
        for (uint256 i; i < length; i++) {
            if (tokens[i] != address(0)) {
                amounts[i] +=
                    IBaseRewardPool(rewarder).earned(address(this), tokens[i]) +
    ...
    function _depositLp(uint256 _amount) internal {
        _approveTokenIfNeeded(address(depositToken), address(mainStaking), _amount);
        IPoolHelperV4(poolHelper).deposit(_amount);
    }
    ...
    function migrateAllUserDepositsFromManual() external {
        uint256 amount = IPoolHelperV4(poolHelper).balance(msg.sender);
    ...
    function _migrateFromManual(uint256 amount) internal {
        ...
        IPoolHelperV4(poolHelper).swapToCompoundFor(msg.sender, amount);
    ...
    function _migrateToManual(uint256 amount) internal {
        ...
        if (balanceOf(msg.sender) > sharesToBurn || sharesToBurn == 0) {
            sharesToBurn += 1;
        }
        ...
        IPoolHelperV4(poolHelper).swapBackToManualFor(msg.sender, amount);
    }
    ...
    /// @notice Deposits AVAX in the pool if WAVAX is the correct deposit token
    function depositAvax() external payable {
        _compoundIfNeeded();
        require(address(depositToken) == WAVAX, "AVAX deposit not allowed");
    ...
    function _compoundIfNeeded() internal {
        (, , , , , address stakeToken, , address rewarder, address _helper) = mainStaking
            .getPoolInfo(assetToken);
        poolHelper = _helper;
        if (
            (IERC20(PTP).balanceOf(address(this)) +
                IBaseRewardPool(rewarder).earned(address(this), PTP)) > maxRewardPending
        ) {
            _compound();
        }
    }
    ...
    function _withdraw(uint256 _amount, uint256 _minAmount)
        internal
        returns (uint256 withdrawnAmount)
    {
        uint256 shares = getSharesForDepositTokens(_amount);
        if (balanceOf(msg.sender) > shares || shares == 0) {
            shares += 1;
        }
        depositTracking[msg.sender] -=
        ...
        IPoolHelperV4(poolHelper).withdraw(_amount, _minAmount);
        withdrawnAmount = depositToken.balanceOf(address(this)) - previousBalance;
    }
    ...
    function withdraw(uint256 amount, uint256 minAmount) external {
        if (harvestBeforeWithdraw) {
            _harvest();
        }
        uint256 withdrawnAmount = _withdraw(amount, minAmount);
    ...
    function withdrawAvax(uint256 amount, uint256 minAmount) external {
        require(address(depositToken) == WAVAX, "AVAX deposit not allowed");
        uint256 withdrawnAmount = _withdraw(amount, minAmount);
        _approveTokenIfNeeded(WAVAX, avaxHelper, withdrawnAmount);
        IAvaxHelper(avaxHelper).withdrawTo(msg.sender, withdrawnAmount);
        emit Withdraw(msg.sender, amount);
    }
    ...
    function _addLiquidityAndDepositLp() internal {
        ...
        if (amount > 0) {
            _swapWavaxToToken(address(depositToken), amount);
    ...
    function _harvest() internal {
        IPoolHelperV4(poolHelper).getReward();
    }
    ...
    function convertDust() public {
        _safeSwapToWavax(address(depositToken), depositToken.balanceOf(address(this)));
    }
}
...
interface IPoolHelperV4 {
    ...
    function depositAVAXFor(address _for) external payable;
