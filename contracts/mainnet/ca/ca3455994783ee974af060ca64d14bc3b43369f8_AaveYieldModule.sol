...
contract AaveYieldModule is BaseModule {
    ...
    */
    function getMaxWithdrawableAmount() public view returns (uint256) {
        (uint totalSupply, uint totalDebt) = _collectAavePoolData();
        uint256 liquidLiquidity = _getAvailableLiquidity(totalSupply, totalDebt);
        if (liquidLiquidity >= getBalance())
            return getBalance();
        return liquidLiquidity;
    }
