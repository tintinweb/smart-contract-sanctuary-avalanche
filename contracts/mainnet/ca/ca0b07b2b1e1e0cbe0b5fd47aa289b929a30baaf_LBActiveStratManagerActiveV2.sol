    ...
    AutomationCompatibleInterface
{
    ...
    //if the strategy should rebalance and attempt to evenly distribute the liquidity
    //sort of like a swith
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        //get the centre of the active bins
        uint256 binLength = activeBins.length;
        uint256 centreBin = binLength / 2;
        uint256 minCentreBin = activeBins[centreBin - 5];
        uint256 maxCentreBin = activeBins[centreBin + 5];
        ...
        //if the ratio is skewed rebalance to try and get ratio back to where it should be
        if (
            (activeId <= minCentreBin && isTokenYWeighted) ||
            (activeId >= maxCentreBin && isTokenXWeighted)
        ) {
            //conditions here
            upkeepNeeded = true;
        }
    ...
    function performUpkeep(bytes calldata) external override {
        ...
        uint256 binLength = activeBins.length;
        uint256 centreBin = binLength / 2;
        uint256 minCentreBin = activeBins[centreBin - 3];
        uint256 maxCentreBin = activeBins[centreBin + 3];
        ...
        if (activeId <= minBin + binOffset || activeId >= maxBin - binOffset) {
            //the liquidty upon this rebalance will be skewed so turn on trigger switch
