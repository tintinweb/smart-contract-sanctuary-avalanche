...
contract AvaxBalloons is ERC721ABurnable, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(string memory defaultBaseURI, address payableAddress) ERC721A("AVAX Balloons", "BALLOON") {
        _defaultBaseURI = defaultBaseURI;
        _payableAddress = payable(payableAddress);
    }
    ...
    function give(address to, uint256 amount) external onlyOwner whenNotPaused {
        require(_totalMinted() + amount <= maxSupply, "Max supply reached");
        _safeMint(to, amount);
    }

    function withdraw() external onlyOwner nonReentrant {
        (bool success, ) = _payableAddress.call{value: address(this).balance}("");
    ...
    function setPayableAddress(address newAddress) external onlyOwner {
        _payableAddress = payable(newAddress);
    }
