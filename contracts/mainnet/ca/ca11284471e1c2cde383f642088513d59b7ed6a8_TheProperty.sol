    ...
    ReentrancyGuardUpgradeable
{
    ...
    function termintatePresale() external onlyOwner {
        presaleStart = 0;
        presaleEnd = 0;
    ...
    /**
     * @dev Public method to Presalemint the property.
     ...
     * @param tokenId Defualt should be 0 or if wants to pay by reward , tokenId of the token should be passed whom reward should be used for mint.
     **/
    function mint(
        uint256 _propertyTypeIndex,
        string memory _name,
    ...
    {
        require(!isPropertyLocked(tokenId), "The Property: Property is locked");
        require(
            msg.value >=
                getTaxInAVAX(properties.get(tokenId).propertyTypeIndex) *
                    months,
   ...
   /**
    ...
    function initialize() public initializer {
        ...
        uint256[3] memory _prices = [
            uint256(4 ether), // Condo   4 $NEIBR
            7 ether, // House   7 $NEIBR
            10 ether // Mansion 10 $NEIBR
        ];

        // Daily rewards in $NEIBR
        uint256[3] memory _dailyRewards = [
            uint256(0.03 ether), // Condo 0.03 $NEIBR
            0.06 ether, // House  0.06 $NEIBR
            0.1 ether // Mansion 0.1 $NEIBR
        ];

        // Max daily rewards in $NEIBR
        uint256[3] memory _maxDailyRewards = [
            uint256(0.04 ether), // Condo Max (with home decor) 0.04 $NEIBR
            0.08 ether, // House Max (with home decor) 0.8 $NEIBR
            0.13 ether // Mansion Max (with home decor) 0.13 $NEIBR
        ];
