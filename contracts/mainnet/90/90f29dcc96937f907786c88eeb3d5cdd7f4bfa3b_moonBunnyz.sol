...
contract moonBunnyz is ERC721URIStorage, Ownable {
  ...
  uint256 private _maxPrivatesale;
  uint256 private _maxPerWallet;
  bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
  bool private _privateSale;
  bool private _mintActive;

  constructor() ERC721("MoonBunnyz", "MNBNZ") {
    _customBaseURI = 'ipfs://bafybeiezxnzzsshvj6pgcdgomybtjz2wj7sb66q4nfuuag5u7ugcvhozzu/';
    _price = 0 ether; //price 
    _maxMintable = 100; // max mintable 
    _royaltyAmount = 1000; //royalty percentage (750 = 7.5%)
    _maxMintPerTx = 1; //max per transaction
    _maxPrivatesale = 50;
    _maxPerWallet = 1;
    _mintActive = false; //to activate minting set to true
    _privateSale = false; //to allow only whitelisted users set to true
    _currentToken =  _tokenIds.current();
  }
  
  modifier isWhitelisted(address _address) {
    if(_privateSale && _currentToken < _maxPrivatesale){require(whitelistedAddresses[_address], "You need to be whitelisted");}
  _;
  }
  ...
  function mint(uint256 quantity) public payable isWhitelisted(msg.sender) {
