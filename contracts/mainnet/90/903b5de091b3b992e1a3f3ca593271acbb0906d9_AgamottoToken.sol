...
contract NODERewardManagement {
    ...
    function _cashoutNodeReward(address account, uint256 _creationTime) external returns (uint256) {
        ...
        uint256 rewardNode = (block.timestamp.sub(node.lastClaimTime)).mul(rewardPerNode);
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            rewardCount += (block.timestamp.sub(nodes[i].lastClaimTime)).mul(rewardPerNode);
        }
...
contract AgamottoToken is ERC20, Ownable {
    ...
    function cashoutAll() public {
        ...
        for(uint256 id = 0; id < 3; id ++) {
            ...
            rewardAmount = rewardAmount.add(nodeRewardManager[id]._getRewardAmountOf(sender).mul(100 - cashoutFee[id]).div(100));
            feeAmount = feeAmount.add(nodeRewardManager[id]._getRewardAmountOf(sender).mul(cashoutFee[id]).div(100));
        }
    ...
    function cashoutReward(uint256 id, uint256 blocktime) public {
        address sender = _msgSender();
        require(sender != address(0), "AgamottoToken:  creation from the zero address");
        require(!_isBlacklisted[sender], "Cashout: Blacklisted address");
        require(
            sender != futurUsePool && sender != distributionPool,
        ...
        require(getNodeNumberOf(sender, id) > 0, "no node owner");

        uint256 rewardAmount = nodeRewardManager[id]._getRewardAmountOf(sender, blocktime);
