...
library SelectiveLiquidity {
    ...
    function selectiveDeposit (
        ...
        require(_minComponents < components_, "Toroid/under-minimum-components");
...
library Components {
    ...
    function _transfer(ComponentStorage.Component storage component, address sender, address recipient, uint256 amount) private {
        ...
        component.balances[sender] = sub(component.balances[sender], amount, "Toroid/insufficient-balance");
        component.balances[recipient] = add(component.balances[recipient], amount, "Toroid/transfer-overflow");
...
library Swaps {
    ...
    function getOriginAndTarget (
        ...
        require(o_.addr != address(0), "Toroid/origin-not-supported");
        require(t_.addr != address(0), "Toroid/target-not-supported");
...
library ComponentMath {
    ...
    function enforceSwapInvariant (
        ...
        require(0 < _diff || _diff >= MAX_DIFF, "Toroid/swap-invariant-violation");
    ...
    function enforceLiquidityInvariant (
        ...
        require(0 < _diff || _diff >= MAX_DIFF, "Toroid/liquidity-invariant-violation");
        ...
        for (uint i = 0; i < _length; i++) {
            ...
            if (_nBals[i] > _nIdeal) {
                ...
                if (_nBals[i] > _nHalt){
                    ...
                    if (_oBals[i] < _oHalt) revert("Toroid/upper-halt");
                    if (_nBals[i] - _nHalt > _oBals[i] - _oHalt) revert("Toroid/upper-halt");
                ...
                if (_nBals[i] < _nHalt){
                    ...
                    if (_oBals[i] > _oHalt) revert("Toroid/lower-halt");
                    if (_nHalt - _nBals[i] > _oHalt - _oBals[i]) revert("Toroid/lower-halt");
...
library Orchestrator {
    ...
    function setParams (
        ...
        require(0 < _alpha && _alpha < 1e18, "Toroid/parameter-invalid-alpha");

        require(0 <= _beta && _beta < _alpha, "Toroid/parameter-invalid-beta");

        require(_feeAtHalt <= .5e18, "Toroid/parameter-invalid-max");

        require(0 <= _epsilon && _epsilon <= .02e18, "Toroid/parameter-invalid-epsilon");

        require(0 <= _lambda && _lambda <= 1e18, "Toroid/parameter-invalid-lambda");

        require(0 <= _sigma && _sigma <= .5e18, "Toroid/parameter-invalid-sigma");

        require(_protocol != address(0), "Toroid/parameter-invalid-protocol");
        ...
        require(_omega >= _psi, "Toroid/parameters-increase-fee");
    ...
    function includeAsset (
        ...
        require(_numeraire != address(0), "Toroid/numeraire-cannot-be-zeroth-adress");

        require(_numeraireAssim != address(0), "Toroid/numeraire-assimilator-cannot-be-zeroth-adress");

        require(_reserve != address(0), "Toroid/reserve-cannot-be-zeroth-adress");

        require(_reserveAssim != address(0), "Toroid/reserve-assimilator-cannot-be-zeroth-adress");

        require(_weight < 1e18, "Toroid/weight-must-be-less-than-one");
    ...
    function includeAssimilator (
        ...
        require(_derivative != address(0), "Toroid/derivative-cannot-be-zeroth-address");

        require(_numeraire != address(0), "Toroid/numeraire-cannot-be-zeroth-address");

        require(_reserve != address(0), "Toroid/numeraire-cannot-be-zeroth-address");

        require(_assimilator != address(0), "Toroid/assimilator-cannot-be-zeroth-address");
