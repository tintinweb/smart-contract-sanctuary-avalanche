...
contract TestIceVault is Ownable {
    ...
    address public immutable sicleFactory; // address to sicleFactory to create pair token LP
    ...
    uint256 public stakeLength; // the number of days for
        ...
        notZeroAddress(_pairAddress)
    {
        ...
        rewardPerBlock = (_rewardPerDay * 1e12) / blocksPerDay;
        rewardPerDay = _rewardPerDay;
        endBlock = block.number + blocksPerDay * (_stakeLengthInDays * 1 days);
        sicleFactory = _sicleFactory;
    ...
    function getTotalPOPsForLP() external view returns (uint256) {
        return _getPopsForLP(totalStaked);
    }

    function getUnassignedRewards() external view returns (uint256) {
        return _getUnassignedRewards();
    }
    ...
    function getRewardPerDay() external view returns (uint256) {
        return (rewardPerBlock * blocksPerDay) / 1e12;
    }
        ...
        )
    {
        ...
        return (userInfo.amount, userInfo.rewardDebt, userInfo.lastRewardBlock);
    }

    function isCurrentPopSTokenAboveAvg5d() external view returns (bool) {
        (uint256 popsRatio,) = _getTokenRatio();
        return popsRatio > popsTokenRatioAvgLast5d;
    }
    ...
    function deposit(uint256 amount, uint256 slippage) external {
        ...
        // add user to array, when it's the first time, to track the unassigned rewards
        if (userInfo.lastRewardBlock == 0)
            _users.push(_msgSender());

        // 100% amount -> 50% as tokenERC and 50% as POP token
        // Transfer 100% deposit to the contract
        IERC20(stakeToken).transferFrom(
            _msgSender(),
            address(this),
            amount
        );
        // Then 50% as POP token -> buy in Market.
        _swapInMarket(amount / 2, slippage);
    ...
    // force withdraw by admin; user receives LP, but no reward
    function emergencyWithdrawByAdmin(address user)
        external
        returns (
            uint256,
            uint256,
        ...
        )
    {
        ...
        uint256 fees = popsReserve > popsTokenRatioAvgLast5d
            ? (userAmount * withdrawalFee) / 10000
            : 0;
        accumulatedFees += fees;
        uint256 sTokenAmount = (userAmount - fees) / 2;
        uint256 popsTokenAmount = _getPopsForLP(sTokenAmount);

        IERC20(stakeToken).approve(sicleRouter02, sTokenAmount);
        IERC20(popsToken).approve(sicleRouter02, popsTokenAmount);
        uint256 deadline = block.timestamp + 5 minutes;
        (uint256 amountPopsToken, uint256 amountStakeToken, uint256 liquidity) = ISicleRouter02(
            sicleRouter02
        ).addLiquidity(
                popsToken,
                stakeToken,
                popsTokenAmount,
                sTokenAmount,
                1, // amountAMin set low to ensure completion
                1, // amountBMin set low to ensure completion
        ...
        stakingNotStarted
    {
        ...
        rewardPerBlock = (rewardPerDay * 1e12) / blocksPerDay;
    }
    ...
    function setPopsTokenRatioAvgLast5d(uint256 amount) external onlyOwner {
        popsTokenRatioAvgLast5d = amount;
    }
    ...
    function setRewardPerDay(uint256 value)
        ...
        stakingNotStarted
    {
        rewardPerBlock = (value * 1e12) / blocksPerDay;
    }
    ...
    function setStakingToken(address value) external onlyOwner notZeroAddress(value) stakingNotStarted {
        stakeToken = value;
    }
        ...
        )
    {
        ...
        (uint256 popsReserve, uint256 stakeReserve) = _getTokenRatio();
        uint256 fees = popsReserve > popsTokenRatioAvgLast5d
            ? (_amount * withdrawalFee) / 10000
            : 0;
        uint256 sTokenAmount = (_amount - fees) / 2;
        // uint256 popsTokenAmount = sTokenAmount * popsReserve / stakeReserve;
        uint256 popsTokenAmount = _getPopsForLP(sTokenAmount);
    ...
    function withdrawExcessPopsToTreasury()
        ...
        returns (uint256)
    {
        uint256 popsBalance = IERC20(popsToken).balanceOf(address(this));
        uint256 popsRequiredForLP = _getPopsForLP(totalStaked / 2);

        if (popsBalance < popsRequiredForLP) return 0; // There is no excess of POPS

        uint256 excessPOPS = popsBalance - popsRequiredForLP;
        IERC20(popsToken).transferFrom(address(this), treasury, excessPOPS);
        emit WithdrawExcessPOPS(popsToken, excessPOPS);
        return excessPOPS;
    }

    // get unassigned rewards only when the staking has finished
    // large number of users will cause out of gas
        ...
        returns (uint256)
    {
        ...
        uint256 unAssignedRewards = _getUnassignedRewards();
        if (unAssignedRewards == 0) return 0;

        // users can not claim anymore
        address[] memory users = _users;
        for (uint x; x < users.length; x++) {
            userInfos[users[x]].lastRewardBlock = block.number;
        }
        ...
        emit WithdrawUnassignedRewards(stakeToken, unAssignedRewards);
        return unAssignedRewards;
    }
    ...
    function _claim() private {
        ...
        if (pending > 0) {
            ...
            if (bal - pending < 0) revert InsufficientRewards();
    ...
    function _getTokenRatio() private view returns (uint256, uint256) {
        (uint112 _reserve0, uint112 _reserve1,) = ISiclePair(pairAddress).getReserves();
        if (isToken0Pops) {
    ...
    function _getUnassignedRewards()
        private
        view
        returns (uint256 totalUnassignedRewards)
    {
        // this will fail with more than a few hundred users
        address[] memory users = _users;
        for (uint x; x < users.length; x++) {
            totalUnassignedRewards += _getPending(users[x]);
        }
    }

    function _swapInMarket(uint256 amount, uint256 slippage) internal {
        uint256 deadline = block.timestamp + 5 minutes;
        address[] memory path = new address[](2);
        path[0] = stakeToken;
        path[1] = popsToken;

        // allow slippage by getting 95% of expected amount
        uint256 popsAmount = _getPopsForLP(amount) * slippage / 100;

        IERC20(stakeToken).approve(sicleRouter02, amount);
        IERC20(popsToken).approve(sicleRouter02, popsAmount);
