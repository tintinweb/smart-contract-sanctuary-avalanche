...
/**
GorillaNodes BANANABOX NFT
https://gorillanodes.finance
*/
...
contract BananaBox is ERC721("BANANABOX", "BBNFT"), Ownable {
    ...
    uint256 public avaxPrice = 1 * 10**18; // 1 AVAX
    uint256 public bananaPrice = 10 * 10**18; // 10 BANANA

    uint256 public MAX_TOKENS = 2000; // maximum supply
    string public baseURI = "https://gateway.pinata.cloud/ipfs/QmXYVyxojph1R1vqhTKoCmv6CEQpndqP2QPxtkLmaa1Gpe/"; //METADATA
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (_msgSender() != owner()) { 
                ...
                require(msg.value >= avaxPrice * _mintAmount, "You need AVAX.");
                require((banana.balanceOf(_msgSender())) >= (_mintAmount * bananaPrice),"You need BANANA"); 
        }
        
        //BANANA split payment
        _transferFrom(banana, _msgSender(), address(this),_mintAmount * bananaPrice);
    ...
    function withdrawBananaTokens() onlyOwner external  {
        uint256 balance = banana.balanceOf(address(this));
        banana.transfer(owner(), balance);
    }


    function generateGiveaway(uint256 _amount) onlyOwner external {
    ...
    function setAvaxPrice(uint256 _price) public onlyOwner {
        avaxPrice = _price;
    }

    function setBananaPrice(uint256 _price) public onlyOwner {
        bananaPrice = _price;
    }
    ...
    function tokenContract(address _tokencontract) external onlyOwner() {
        banana = IERC20(_tokencontract);
    }
