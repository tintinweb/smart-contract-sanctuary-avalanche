...
contract HentaiTreasury is Ownable {
    ...
    address public sHentai;
    uint public sHentaiQueue; // Delays change to sHentai address
    ...
    //    isLiquidityToken[ _HentaiDAI ] = true;
    //    liquidityTokens.push( _HentaiDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for Hentai
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Hentai needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Hentai ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Hentai for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHentai( Hentai ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHentai ).balanceOf( msg.sender ); // Can only borrow against sHentai held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Hentai
        @param _amount uint
     */
    function repayDebtWithHentai( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHentai( Hentai ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Hentai, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Hentai ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Hentai ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Hentai valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Hentai decimals
            value_ = _amount.mul( 10 ** IERC20( Hentai ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sHentai ) { // 9
            sHentaiQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
