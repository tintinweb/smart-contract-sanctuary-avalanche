...
import { LibAaveV3Pool } from "./LibAaveV3Pool.sol";
import { LibUniswapV2Pair } from "./LibUniswapV2Pair.sol";
import { VaultBase } from "./VaultBase.sol";

contract PhantomTT3Vault is VaultBase
{
	using SafeERC20 for IERC20;
	using LibAaveV3Pool for LibAaveV3Pool.Self;
	using LibUniswapV2Pair for LibUniswapV2Pair.Self;

	LibAaveV3Pool.Self public lendingPool;
	LibUniswapV2Pair.Self public rewardPool;

	address public flashLoanReceiver;

	function decimals() public view override returns (uint8 _decimals)
	{
		return IERC20Metadata(lendingPool.asset).decimals();
	}

	function _initialize(bytes memory _data) internal override
	{
		uint8 _categoryId;
		(lendingPool, rewardPool, _categoryId, flashLoanReceiver) = abi.decode(_data, (LibAaveV3Pool.Self, LibUniswapV2Pair.Self, uint8, address));
		require(lendingPool.rewardAssets.length == 1, "invalid reward");
		address _ua = lendingPool.underlyingAsset;
		address _ra = lendingPool.rewardAssets[0];
		address _rt0 = rewardPool.token0;
		address _rt1 = rewardPool.token1;
		require(_ra == _rt0 && _ua == _rt1 || _ra == _rt1 && _ua == _rt0, "invalid route");
		lendingPool._setup(_categoryId);
		lendingPool._approve(type(uint256).max);
	}

	function _afterEmergencyDeclared() internal override
	{
		lendingPool._emergencyWithdraw();
		lendingPool._approve(0);
	}

	function tokens() external view override returns (address[] memory _tokens)
	{
		_tokens = new address[](2);
		_tokens[0] = lendingPool.underlyingAsset;
		_tokens[1] = lendingPool.asset;
		return _tokens;
	}

	function totalReserve(address _token) public view override returns (uint256 _totalReserve)
	{
		require(_token == lendingPool.asset || _token == lendingPool.underlyingAsset, "invalid token");
		return emergencyMode ? IERC20(lendingPool.underlyingAsset).balanceOf(address(this)) : lendingPool._balanceOf(address(this));
	}
	...
	{
		require(_token == lendingPool.asset || _token == lendingPool.underlyingAsset, "invalid token");
		uint256 _totalReserve = totalReserve(lendingPool.asset);
		uint256 _totalSupply = totalSupply();
		IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
		_shares = _calcSharesFromAmount(_totalReserve, _totalSupply, _amount);
		require(_shares >= _minShares, "high slippage");
		if (_token == lendingPool.underlyingAsset) {
			lendingPool._deposit(_amount);
		}
	...
	function withdraw(address _token, uint256 _shares, uint256 _minAmount) external override nonReentrant returns (uint256 _amount)
	{
		...
		if (emergencyMode) {
			if (_token == lendingPool.asset) {
				lendingPool._deposit(_amount);
			}
		} else {
			_makeLeverageRoom(_amount);
			if (_token == lendingPool.underlyingAsset) {
				lendingPool._withdraw(_amount);
			}
		}
	...
	{
		if (!emergencyMode) {
			lendingPool._harvest();
		}
		address _rewardAsset = lendingPool.rewardAssets[0];
		uint256 _rewardAmount = IERC20(_rewardAsset).balanceOf(address(this));
		if (_rewardAmount == 0) {
		...
		uint256 _totalSupply = totalSupply();
		uint256 _amount;
		if (_rewardAsset == rewardPool.token0) {
			_amount = rewardPool._swap0(_rewardAmount, address(this));
		}
		else
		if (_rewardAsset == rewardPool.token1) {
			_amount = rewardPool._swap1(_rewardAmount, address(this));
		}
		else {
			revert("panic");
		}
	...
	function adjustLeverage(uint256 _ltv) external onlyWhitelisted nonEmergency nonReentrant
	{
		(uint256 _supply, uint256 _borrow) = lendingPool._supplyAndBorrowOf(address(this));
		uint256 _balance = _supply - _borrow;
		uint256 _newBorrow = (_balance * _ltv) / (10000 - _ltv);
		if (_newBorrow > _borrow) {
			lendingPool._borrow(_newBorrow - _borrow, flashLoanReceiver);
		}
		else
		if (_newBorrow < _borrow) {
			lendingPool._repay(_borrow - _newBorrow);
		}
	}

	function _makeLeverageRoom(uint256 _amount) internal
	{
		(uint256 _supply, uint256 _borrow) = lendingPool._supplyAndBorrowOf(address(this));
		if (_borrow == 0) return;
		uint256 _ltv = _borrow * 10000 / _supply;
		uint256 _balance = _supply - _borrow;
		uint256 _newBalance = _balance - _amount;
		uint256 _newBorrow = (_newBalance * _ltv) / (10000 - _ltv);
		lendingPool._repay(_borrow - _newBorrow);
	}
}
...
import { IAaveV3AToken } from "./IAaveV3AToken.sol";
import { IAaveV3Pool } from "./IAaveV3Pool.sol";
import { IAaveV3RewardsController } from "./IAaveV3RewardsController.sol";

library LibAaveV3Pool
{
	using SafeERC20 for IERC20;
	...
	uint16 constant REFERRAL_CODE_NONE = 0;

	uint256 constant INTEREST_RATE_MODE_VARIABLE = 2;

	function _new(address _asset, address _debtAsset) internal view returns (Self memory _self)
	{
		address _pool = IAaveV3AToken(_asset).POOL();
		address _underlyingAsset = IAaveV3AToken(_asset).UNDERLYING_ASSET_ADDRESS();
		address _incentives = IAaveV3AToken(_asset).getIncentivesController();
		address[] memory _rewardAssets = IAaveV3RewardsController(_incentives).getRewardsByAsset(_asset);
		return Self({ pool: _pool, asset: _asset, debtAsset: _debtAsset, underlyingAsset: _underlyingAsset, incentives: _incentives, rewardAssets: _rewardAssets });
	}

	function _balanceOf(Self storage _self, address _account) internal view returns (uint256 _balance)
	{
		return IERC20(_self.asset).balanceOf(_account) - IERC20(_self.debtAsset).balanceOf(_account);
	}

	function _supplyAndBorrowOf(Self storage _self, address _account) internal view returns (uint256 _supplyBalance, uint256 _borrowBalance)
	{
		return (IERC20(_self.asset).balanceOf(_account), IERC20(_self.debtAsset).balanceOf(_account));
	}

	function _setup(Self storage _self, uint8 _categoryId) internal
	{
		IAaveV3Pool(_self.pool).setUserEMode(_categoryId);
	}

	function _approve(Self storage _self, uint256 _amount) internal
	{
		IERC20(_self.underlyingAsset).safeApprove(_self.pool, _amount);
	}

	function _deposit(Self storage _self, uint256 _amount) internal
	{
		IAaveV3Pool(_self.pool).supply(_self.underlyingAsset, _amount, address(this), REFERRAL_CODE_NONE);
	}

	function _harvest(Self storage _self) internal
	{
		address[] memory _assets = new address[](2);
		_assets[0] = _self.asset;
		_assets[1] = _self.debtAsset;
		IAaveV3RewardsController(_self.incentives).claimRewards(_assets, type(uint256).max, address(this), _self.rewardAssets[0]);
	}

	function _withdraw(Self storage _self, uint256 _amount) internal
	{
		IAaveV3Pool(_self.pool).withdraw(_self.underlyingAsset, _amount, address(this));
	}

	function _emergencyWithdraw(Self storage _self) internal
	{
		IAaveV3Pool(_self.pool).repayWithATokens(_self.underlyingAsset, type(uint).max, INTEREST_RATE_MODE_VARIABLE);
		IAaveV3Pool(_self.pool).withdraw(_self.underlyingAsset, type(uint).max, address(this));
	}

	function _borrow(Self storage _self, uint256 _amount, address _receiver) internal
	{
		IAaveV3Pool(_self.pool).setUserUseReserveAsCollateral(_self.underlyingAsset, true);
		address[] memory _assets = new address[](1);
		_assets[0] = _self.underlyingAsset;
		uint256[] memory _amounts = new uint256[](1);
		_amounts[0] = _amount;
		uint256[] memory _interestRateModes = new uint256[](1);
		_interestRateModes[0] = INTEREST_RATE_MODE_VARIABLE;
		IAaveV3Pool(_self.pool).flashLoan(_receiver, _assets, _amounts, _interestRateModes, address(this), new bytes(0), REFERRAL_CODE_NONE);
	}

	function _repay(Self storage _self, uint256 _amount) internal
	{
		IAaveV3Pool(_self.pool).repayWithATokens(_self.underlyingAsset, _amount, INTEREST_RATE_MODE_VARIABLE);		
	}
}
...
import { EmergencyControl } from "./EmergencyControl.sol";
...
interface IAaveV3AToken is IERC20
{
	...
	function UNDERLYING_ASSET_ADDRESS() external view returns (address _UNDERLYING_ASSET_ADDRESS);
	function getIncentivesController() external view returns (address _incentivesController);
}
...
interface IAaveV3Pool
{
	...
	function withdraw(address _asset, uint256 _amount, address _to) external returns (uint256 _withdrawnAmount);
...
{
	function getRewardsByAsset(address _asset) external view returns (address[] memory _rewards);

	function claimRewards(address[] calldata _assets, uint256 _amount, address _to, address _reward) external returns (uint256 _rewardAmount);
}
