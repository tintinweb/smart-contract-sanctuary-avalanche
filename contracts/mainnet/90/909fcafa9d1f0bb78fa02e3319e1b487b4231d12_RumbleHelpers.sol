...
// File @rumble-finance/vault/contracts/balances/[email protected]
 ...
 * prevent stack-too-deep issues. This is compounded by the fact that Rumble contracts typically are memory-intensive,
...
// File @rumble-finance/pool-utils/contracts/interfaces/[email protected]
 ...
 * Adds the {permit} method, which can be used to change an account's ERC20 allowance (see {IERC20-allowance}) by
 * presenting a message signed by the account. By not relying on `{IERC20-approve}`, the token holder account doesn't
 * need to send a transaction, and thus is not required to hold Avax at all.
 *
 * _Available since v3.4._
 */
...
// File @rumble-finance/pool-utils/contracts/[email protected]
 ...
 */
contract RumblePoolToken is ERC20Permit {
 ...
 */
abstract contract BasePool is IBasePool, BasePoolAuthorization, RumblePoolToken, TemporarilyPausable {
    ...
    constructor(
        ...
        RumblePoolToken(name, symbol, vault)
    ...
    function _getAuthorizer() internal view override returns (IAuthorizer) {
        // Access control management is delegated to the Vault's Authorizer. This lets Rumble Governance manage which
...
// File contracts/RumbleHelpers.sol
...
/**
 * @dev This contract simply builds on top of the Rumble architecture to provide useful helpers to users.
 ...
 */
contract RumbleHelpers is AssetHelpers {
    using Math for uint256;
    ...
    constructor(IVault _vault) AssetHelpers(_vault.WAVAX()) {
        vault = _vault;
    }
