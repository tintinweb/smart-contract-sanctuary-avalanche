...
contract Presale is Context, Ownable, ReentrancyGuard {
    ...
    function starPresale(
        ...
        require(treasury != address(0), "PRESALE: TREASURY ZERO ADDRESS");
        _treasury = treasury;

        _endPresaleTime = block.timestamp + 3 days;
    }

    function presaleEnd() public onlyOwner {
        _endPresaleTime = block.timestamp;
    }

    function endPresale() external onlyOwner {
        require(block.timestamp >= _endPresaleTime, "PRESALE: NOT FINISHED YET");
        _startPresale = !_startPresale;
    }

    function setReclaim() external onlyOwner {
        require(block.timestamp >= _endPresaleTime, "PRESALE: NOT FINISHED YET");
        _reclaimable = !_reclaimable;
    }

    function buyTokens(uint256 _invest) public {
        require(_startPresale, "PRESALE: NOT STARTED YET");
        ...
        require(baseAmount * 10e18 <= _presaleToken.balanceOf(address(this)), "PRESALE: INSUFFICIENT BALANCE");

        uint256 claimableAmount = baseAmount * 10e18;

        _investToken.transferFrom(msg.sender, _treasury, _invest * 100e18);
        _presaleToken.transfer(msg.sender, claimableAmount);
        ...
        for (uint256 i = 0; i < baseAmount; i ++) {
    ...
    function burnToken() public onlyOwner {
        require(!_startPresale, "PRESALE: NOT FINISHED YET");
