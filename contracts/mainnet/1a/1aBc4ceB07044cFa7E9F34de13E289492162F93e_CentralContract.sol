...
contract CentralContract is Ownable, MessageApp {
	...
	//IntentUser value "0" deployment of smartwallet,  "1" mean transfer, swap, approve etc
	event IntentUser(
		uint8 intentType,
		address from,
		address to,
	...
	function sendUserIntent(
		...
		emit IntentUser(intentType, from, to, token, dstChainId, amount, value, data);
	}
	...
	// called by admin on destination chain to withdraw tokens user
	function withdrawTokensUser(address _user, address _token, uint256 _amount) external onlyOwner {
		balances[_user][_token] -= _amount;
		IERC20(_token).safeTransfer(msg.sender, _amount);
	}

	// called by admin on destination chain to withdraw token amount, which belong only to central contract
	function withdrawTokensContract(address _token, uint256 _amount) external onlyOwner {
		IERC20(_token).safeTransfer(msg.sender, _amount);
	}
}
