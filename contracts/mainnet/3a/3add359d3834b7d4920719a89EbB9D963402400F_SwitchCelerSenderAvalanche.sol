...
contract SwitchCelerSenderAvalanche is SwitchAvalanche {
    ...
    ) SwitchAvalanche(_switchViewAddress, _switchEventAddress, _paraswapProxy, _augustusSwapper)
        public
    {
        ...
        returns (bytes32 transferId)
    {
        ...
        if (transferArgs.srcSwap.srcToken == transferArgs.srcSwap.dstToken) {
            ...
            if (transferArgs.paraswapUsageStatus == uint(DataTypes.ParaswapUsageStatus.None)) {
                (returnAmount, ) = _swapBeforeCeler(transferArgs);
            } else if (transferArgs.paraswapUsageStatus == uint(DataTypes.ParaswapUsageStatus.OnSrcChain)) {
                returnAmount = _swapFromParaswap(transferArgs);
            }
        }
        ...
        returns (uint256 returnAmount)
    {
        ...
        returnAmount = _swapInternalWithParaSwap(IERC20(transferArgs.srcSwap.srcToken), IERC20(transferArgs.srcSwap.dstToken), transferArgs.amount, transferArgs.srcParaswapData);
    }
