...
contract ThirstTrapWaifus is ERC721  {
    ...
    constructor()
        ERC721("Thirst Trap Waifus", "Waifus")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 300) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeickndcaqwmekqhh6fi45smzls7fnvlzwykyljvdvxrbes4ee77mt4/", Strings.toString(tokenId), ".json")
            );
    }
