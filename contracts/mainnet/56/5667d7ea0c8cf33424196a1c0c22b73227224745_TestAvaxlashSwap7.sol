...
contract TestAvaxlashSwap7 is IPangolinCallee {
  // Uniswap V2 router
  ...
  function testFlashSwap(address _tokenBorrow, uint _amount, address _arbtoken, string memory _option) external {
    ...
    bytes memory data = abi.encode(_tokenBorrow, _amount, _arbtoken, _option);
  ...
  function pangolinCall(
    ...
    (address tokenBorrow, uint amount, address arbtoken, string memory option) = abi.decode(_data, (address, uint, address, string));
    ...
    if (keccak256(abi.encodePacked(option)) == keccak256(abi.encodePacked("joeFirst"))) {
        swap("pangolin", tokenBorrow, arbtoken, amount);
        uint amount2 = IPangolinERC20(arbtoken).balanceOf(address(this));
        swap("joe", arbtoken, tokenBorrow, amount2); 
    } else {
        swap("joe", tokenBorrow, arbtoken, amount);
        uint amount2 = IPangolinERC20(arbtoken).balanceOf(address(this));
        swap("pangolin", arbtoken, tokenBorrow, amount2);
    }
