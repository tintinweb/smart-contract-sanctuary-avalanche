...
interface IDeltaPrimePool {
    ...
    /**
     * @notice Deposit an _amount of underlying asset into the pool
     **/
    ...
    /**
    * @notice Amount of want supplied to the pool
    **/
    function totalSupply() external view returns (uint256);

    /**
    * @notice Amount of want borrowed by Credit accounts
    **/
...
abstract contract BaseModule is IYieldModule, UUPSUpgradeable, OwnableUpgradeable, Rescuable {
    ...
    /// @notice The smart farmooor of the module
    ...
    modifier onlyVault() {
        require(msg.sender == vault, "BaseModule: only smart farmooor");
        _;
    }
     ...
     * @param   _want  Asset contract address
    ...
    /**
     * @notice  Set the Smart Farmooor of the Module
     * @param   _smartFarmooor  Address of the Smart Farmooor
     */
    ...
    /**
     * @notice  Set the want of the Module
     * @param   _want  Address of the want contract
     */
    function _setBaseToken(address _want) private {
        require(
            _want != address(0),
            "BaseModule: cannot be the zero address"
        );
...
import "../interface/deltaprime/IDeltaPrimePool.sol";

/**
 * @author  HedgeFarm team
 * @title   DeltaPrime Yield module
 * @dev     The DeltaPrime module is responsible for interacting with the Delta Prime deposit pools
 * @notice  Upgradability is needed because DeltaPrime protocol is built with Proxy - it's implementation could be updated
 */

contract DeltaPrimeYieldModule is BaseModule {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @dev A constant used for calculating shares
    uint256 private constant IOU_DECIMALS_EXP = 1e18;

    /// @notice The DeltaPrime pool contract
    address public pool;
    ...
    **/
    ...
    function initialize(
        ...
        _initializeBase(_smartFarmooor, _manager, _want, _executionFee, _dex, _rewards, _name, _wrappedNative);

        pool = _pool;
        lastUpdatedBalance = 0;
        IERC20Upgradeable(_want).safeApprove(_pool, type(uint256).max);
    }

    /** manager **/

    /**
     * @notice  Deposit Base token into DeltaPrime - provide liquidity
     * @param   amount  Amount of Base token to be deposited
     */
    function deposit(uint256 amount) external onlyVault {
        require(amount > 0, "DeltaPrime: deposit amount cannot be zero");
        IERC20Upgradeable(want).safeTransferFrom(msg.sender, address(this), amount);
        lastUpdatedBalance += amount;
        IDeltaPrimePool(pool).deposit(amount);
        emit Deposit(want, amount);
    }

    /**
     * @notice  Withdraw want from DeltaPrime
     * @dev     Amount gets converted to shares for withdrawal
     * @param   amount  Amount of want to be withdrawn from DeltaPrime
     * @param   receiver  Receiver of the funds
     * @return  withdrawnAmount  Amount of want received
     */
    ...
    {
        require(amount > 0, "DeltaPrime: amount cannot be zero");
        require(msg.value == 0, "DeltaPrime: msg.value must be zero");
        uint256 balBefore = IERC20Upgradeable(want).balanceOf(address(this));
        IDeltaPrimePool(pool).withdraw(amount);
        uint256 balAfter = IERC20Upgradeable(want).balanceOf(address(this));
        withdrawnAmount = balAfter - balBefore;
        ...
        IERC20Upgradeable(want).safeTransfer(receiver, withdrawnAmount);
        emit Withdraw(want, withdrawnAmount);

        return withdrawnAmount;
    }

    /**
     * @notice  Harvest the rewards from DeltaPrime
    ...
    {
        _lpProfit();
        uint256 totalProfit = IERC20Upgradeable(want).balanceOf(address(this));
        if (totalProfit != 0) {
            IERC20Upgradeable(want).safeTransfer(receiver, totalProfit);
        }
        emit Harvest(want, totalProfit);
        return totalProfit;
    }
    ...
    */
    function getMaxWithdrawableAmount() public view returns (uint256) {
        (uint totalSupply, uint totalDebt) = _collectDeltaPrimePoolBalance();
        uint256 availableLiquidity = _getAvailableLiquidity(totalSupply, totalDebt);
        if (availableLiquidity >= getBalance())
            return getBalance();
        return availableLiquidity;
    }

    /**
     * @notice  Get current the underlying balance on DeltaPrime
     * @dev     Returns an amount in Base token
     * @return  uint256  Amount of base token
     */
    function getBalance() public view returns (uint256) {
        return IDeltaPrimePool(pool).balanceOf(address(this));
    }
    ...
    /**
     * @notice  Get execution fee needed to withdraw from DeltaPrime
    ...
    function _lpProfit() private {
        uint256 currentTotalHoldings = getBalance();
        require(currentTotalHoldings >= lastUpdatedBalance, "DeltaPrime: module not profitable");
        uint256 aumDelta = currentTotalHoldings - lastUpdatedBalance;
        if (aumDelta > 0) {
            IDeltaPrimePool(pool).withdraw(aumDelta);
        }
    }

    /**
     * @notice  DeltaPrime lp token
     * @dev     overridden for pool address which is DeltaPrime lp token
     * @return  pool address
     */
    ...
    /**
    * @notice  Returns want balance of the DeltaPrime Pool - the available liquidity
    */
    function _collectDeltaPrimePoolBalance() private view returns (uint256, uint256) {
        uint256 totalSupply = IDeltaPrimePool(pool).totalSupply();
        uint256 totalBorrowed = IDeltaPrimePool(pool).totalBorrowed();
        return (totalSupply, totalBorrowed);
    }

    /**
    * @notice  Get available liquidity on the Pool
    * @dev     Returns 0 when all the liquidity has been borrowed
    */
    function _getAvailableLiquidity(uint totalSupply, uint totalDebt) private view returns (uint256) {
