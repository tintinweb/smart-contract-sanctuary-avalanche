...
abstract contract YToken is ERC20Burnable {
    ...
    modifier onlyMinters() {
        require(minters[msg.sender], "YToken::onlyMinters: Only minters allowed");
        _;
    }
    ...
    function maxTotalSupply() internal virtual returns (uint256);

    function genesisSupply() internal virtual returns (uint256);
    ...
    function setRewarder(address _rewarder) external returns (bool) {
        require(rewarder == address(0), "YToken::setRewarder: NOT_ALLOWED");
        rewarder = _rewarder;
        minters[_rewarder] = true;
        return true;
    }
    ...
    {
        require(treasuryFund == address(0), "YToken::setTreasuryFund: NOT_ALLOWED");
        treasuryFund = _treasuryFund;
        minters[_treasuryFund] = true;
    ...
    {
        require(devFund == address(0), "YToken::setDevFund: NOT_ALLOWED");
        devFund = _devFund;
        minters[_devFund] = true;
        return devAllocation();
    }

    function setPool(address _pool) external returns (bool) {
        require(pool == address(0), "YToken::setPool: NOT_ALLOWED");
        pool = _pool;
        minters[_pool] = true;
        return true;
    }
    ...
    function mint(address _to, uint256 _value) external onlyMinters {
        require(maxTotalSupply() >= totalSupply() + _value, "YToken::mint: > MAX_TOTAL_SUPPLY");
        _mint(_to, _value);
    }
}
...
contract Snowy is YToken {
    ...
    constructor(string memory _name, string memory _symbol) YToken(_name, _symbol) {}
    ...
    function genesisSupply() internal pure override returns (uint256) {
