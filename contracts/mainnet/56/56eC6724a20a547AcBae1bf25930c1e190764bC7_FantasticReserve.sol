...
==================================================================== */
contract FantasticReserve is Ownable {
    ...
    /// @notice Return ERC-20 balance of FantasticReserve
    /// @param _token Address of the ERC-20 token
    /// @return Balance of the FantasticReserve
    ...
    function requestFund(address _token, uint256 _amount) external {
        require(strategies[msg.sender], "FantasticReserve::requestFund: Only strategies can request fund");
        require(_amount <= balanceOf(_token), "FantasticReserve::requestFund: Request more fund than balance");
    ...
    function addStrategy(address _strategy) external onlyOwner {
        require(_strategy != address(0), "FantasticReserve::addStrategy: invalid address");
        require(!strategies[_strategy], "FantasticReserve::addStrategy: strategy was previously added");
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        require(strategies[_strategy], "FantasticReserve::removeStrategy: strategy not found");
    ...
    function allocateFee(address _token, uint256 _amount) external onlyOwner {
        require(address(staking) != address(0), "FantasticReserve::allocateFee:Fee distributor not set");
        require(_amount > 0, "FantasticReserve::allocateFee: invalid amount");
