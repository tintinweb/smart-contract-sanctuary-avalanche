...
contract PlatypusHelper is Ownable {
    ...
    function getLPPriceAndDecimals(address lp) public view returns (uint256 inUSD, uint256 tokenDecimals) {
        address token = IAPRHelper(aprHelper).pool2token(lp);
    ...
    function getUserInfo(address user) public view returns(UserPoolInfo[] memory usersInfo){
        uint256 length = IAPRHelper(aprHelper).getLengthPtpPools();
        address mainStaking = IAPRHelper(aprHelper).mainStaking();
        ...
        for (uint256 i; i < length; ++i) {
            ...
            address asset = IAPRHelper(aprHelper).lp2asset(pool);
            (, , , , , , ,address rewarder, ) = IMainStaking(mainStaking).getPoolInfo(
                asset
            );
            ...
            (userInfo.tokenRewards, userInfo.rewards, userInfo.rewardsUSD) = getPTPHarvestableRewards(pool, user);
            (userInfo.tokenHarvestable, userInfo.harvestable, userInfo.harvestableUSD) = getPTPHarvestableRewards(pool, user);
