...
contract Comptroller is ComptrollerVXStorage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function setRewardSpeedInternal(uint8 rewardType, QiToken qiToken, uint newSupplyRewardSpeed, uint newBorrowRewardSpeed) internal {
        ...
        uint currentBorrowRewardSpeed = _getBorrowRewardOverride(rewardType, address(qiToken)); // TODO: Remove in next upgrade
    ...
    function updateRewardBorrowIndex(uint8 rewardType, address qiToken, Exp memory marketBorrowIndex) internal {
        ...
        uint borrowSpeed = _getBorrowRewardOverride(rewardType, qiToken); // TODO: Remove in next upgrade
        uint blockTimestamp = getBlockTimestamp();
   ...
   /**
    ...
    /**
     * @dev Temporarily overriding borrow speed to return the same value as supply speed
     * @return Borrow speed
     */
    function _getBorrowRewardOverride(uint8 rewardType, address qiToken) public view returns (uint256) {
        // Return borrow speed if it has value
        if (borrowRewardSpeeds[rewardType][qiToken] != 0) {
            return borrowRewardSpeeds[rewardType][qiToken];
        }

        // if borrow speed is not set, return the same value as supply speed
        return supplyRewardSpeeds[rewardType][qiToken];
    }
