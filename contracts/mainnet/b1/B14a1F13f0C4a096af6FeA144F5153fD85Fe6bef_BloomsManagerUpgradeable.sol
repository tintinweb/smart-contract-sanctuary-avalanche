    ...
    NectarImplementationPointerUpgradeable
{
    ...
    function movingChoice(
        ...
        if (bloomifyMigration.shouldMoveToBloomify == 2) {
            ...
            bloomifyMigration.instantAmount =
                bloomifyMigration.instantAmount /
                2;
        ...
        onlyApprovedOrOwnerOfBloom(_bloomId)
    {
        ...
        uint256 claimFee = 900;

        uint256 maxAmount = thisShouldTransfer(
            bloomifyMigration.bloomValue,
            claimFee
        );

        uint256 amountToReward = calculateEmergancyFee(
            bloomifyMigration.rewardAmount,
            claimFee
        );

        require(bloomifyMigration.movingDay < 101, "day limit reached");
        require(bloomifyMigration.alreadyMoved < maxAmount, "Limit Reached");
    ...
    function calculateEmergancyFee(uint256 _amount, uint256 _fee)
        internal
        pure
        returns (uint256)
    {
    ...
    function thisShouldTransfer(uint256 _amount, uint256 _fee)
        internal
        pure
        returns (uint256)
    {
