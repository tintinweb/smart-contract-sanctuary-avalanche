...
contract UGame is IUGame, Ownable, ReentrancyGuard, Pausable {
  ...
  function revealManyFYTokenTraits(uint16[] memory tokenIds) external whenNotPaused nonReentrant onlyEOA {
    for (uint16 i = 0; i < tokenIds.length; i++) {
      uint16 tokenId = tokenIds[i];
      require(uNFT.ownerOf(tokenId) == _msgSender(), "NFT: You don't own one of these tokens");
