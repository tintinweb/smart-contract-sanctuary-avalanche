...
contract LeechRouterAvalanche is LeechRouterBase {
     ...
     * @param _validator Whitelist validator.
    ...
    ILeechRouter
{
    ...
    modifier checkWhitelist(bytes memory signature, uint256 maxBlockNumber) {
        if (whitelistEnabled) {
            ...
            if (msgHash.toEthSignedMessageHash().recover(signature) != validator)
                revert BadSignature();
        }
        _;
    }
    ...
    function initialize(
        ...
        // Set initial params
        (baseToken, uniV2, chainId, treasury, validator, finalizer) = (
            _baseToken,
            _uniV2Address,
            _chainId,
            _treasury,
    ...
    /**
     * @notice Validator setter. Only multisig role.
     * @param _validator New validator.
     */
    function setValidator(
        address _validator
    ) external onlyRole(ADMIN_ROLE) zeroAddr(_validator) {
