...
contract Strategy is BaseTokenizedStrategy {
    ...
    address public openflowSdk;

    constructor(address _asset, string memory _name, uint16 _stakingID, address _openflowSdk) BaseTokenizedStrategy(_asset, _name) {
        ...
        ERC20(reward).safeApprove(address(_openflowSdk), type(uint256).max);
        openflowSdk = _openflowSdk;
    }
    ...
    function _claimAndSellRewards() internal {
        ...
        IOpenflow(openflowSdk).swap(address(reward), asset); //@note Sell rewards via Openflow
    }
...
contract StrategyFactory {
    ...
    constructor(address _asset, string memory _name, uint16 _stakingID, address _openflowSdk) {
        newStrategy(_asset, _name, _stakingID, _openflowSdk, msg.sender, msg.sender, msg.sender);
    }
    ...
    function newStrategy(
        ...
        IStrategy _newStrategy = IStrategy(
            address(new Strategy(_asset, _name, _stakingID, _openflowSdk))
        );
