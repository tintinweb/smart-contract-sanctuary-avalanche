...
contract SimpleTokenSwap is Ownable{
    ...
    // Wrapped Fantom
    IWETH public WETH = IWETH(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        ...
        payable // Must attach ETH equal to the `value` field from the API response.
    {
        ...
        TransferHelper.safeTransfer(address(buyToken), fundingWallet, _fee);

        TransferHelper.safeTransfer(address(buyToken),_msgSender(), boughtAmount.sub(_fee));
