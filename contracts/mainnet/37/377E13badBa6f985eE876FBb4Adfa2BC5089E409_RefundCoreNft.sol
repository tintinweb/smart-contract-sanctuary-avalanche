...
contract RefundCoreNft is Ownable {
    //Deploy todo: go to wavax contract address and approve 667 wavax to be used by this contract
    uint private totalWavaxToRefund = 667000000000000000000;

    uint seedRefundAmount = 689505950000000000; //0.578~
    uint saplingRefundAmount = 1379011900000000000; //1.379~
    uint treeRefundAmount = 3447529750000000000; //3.448~
    ...
    function loadEligibleSaplingNfts(uint[] memory ids) public onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            ...
            for (uint j = 0; j < eligibleSaplingNfts.length; j++) {
                if (eligibleSaplingNfts[j] == ids[i]) {
                    //No need to add this one!
                    alreadyExists = true;
                    break;
                }
