...
contract Olympic is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for LP1
    uint256 public constant INITIAL_OLYMPIC_POOL_DISTRIBUTION = 800 ether;
    ...
    /**
     * @notice Constructs the OLYMPIC ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress)
        public
        ERC20("OLYMPIC", "OLYMPIC")
    {
        // Mints 1 OLYMPIC to contract creator for initial pool setup
    ...
    /**
     * @notice Operator mints OLYMPIC to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of OLYMPIC to mint to
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        // if sender excluded, no tax
        if (excludedAddresses[_msgSender()]) {
     ...
     */
    function distributeReward(address _olympicPool) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_olympicPool != address(0), "!_olympicPool");
        rewardPoolDistributed = true;
        _mint(_olympicPool, INITIAL_OLYMPIC_POOL_DISTRIBUTION);
    }
