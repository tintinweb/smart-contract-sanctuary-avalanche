...
/**
 * @title CHRO Pre Sales
 */
contract CHROPreSalesV5 is OwnableUpgradeable {
  ...
  IERC20Upgradeable public chroContract;
  IERC20Upgradeable public usdContract;
  ...
  uint256 public constant MAX_ALLOCATION = 7500000000000000000000000;
  ...
  function buyTokens(uint256 _usdAmount) public started {
    ...
    if (!_whitelists[msg.sender]) {
      _whitelists[msg.sender] = true;
      _allocations[msg.sender] = maximumPurchase;
    }

    require(usdAmount <= _allocations[msg.sender], "Max USD allocation exceeded");
    ...
    SafeERC20Upgradeable.safeTransferFrom(usdContract, msg.sender, multisigAddress, usdAmount);
    _addBalance(msg.sender, chroAmount);

    tokensSold += chroAmount;

    /// subtract user allocation
    _allocations[msg.sender] -= _usdAmount;
  ...
  function setUsdTokenAddress(address _usdTokenAddress) public onlyOwner {
    ...
    usdContract = IERC20Upgradeable(_usdTokenAddress);
  }
  ...
  function changeTokenAddress(address _tokenAddress) public onlyOwner {
    chroContract = IERC20Upgradeable(_tokenAddress);
  }
  ...
  function addWhitelist(address _user) public onlyOwner {
    require(!_whitelists[_user], "User exists");
    _whitelists[_user] = true;
    _allocations[_user] = maximumPurchase;

    emit WhitelistAdded(_user, maximumPurchase);
  }
   ...
   */
  function batchAddWhitelist(address[] memory _users) public onlyOwner {
    require(_users.length > 0, "Invalid inputs");

    for (uint256 i = 0; i < _users.length; i++) {
      if (!_whitelists[_users[i]]) {
        _whitelists[_users[i]] = true;
        _allocations[_users[i]] = maximumPurchase;

        emit WhitelistAdded(_users[i], maximumPurchase);
      }
    }
  }
  ...
  function emergencyTransfer(uint256 _amount) public onlyOwner {
    ...
    SafeERC20Upgradeable.safeTransfer(chroContract, owner(), _amount);
  ...
  function release() public {
    uint256 lockTimestamp = uint256(block.timestamp);
    require(lockTimestamp > start(), "Can't claim CHRO");
    Account storage account = _accounts[msg.sender];

    uint256 toRelease = vestedAmount(msg.sender, lockTimestamp).sub(account.released);

    require(toRelease > 0, "CHRO has been claimed");
    
    if (toRelease > 0) {
      SafeERC20Upgradeable.safeTransfer(chroContract, msg.sender, toRelease);
      account.released += toRelease;
      account.balance -= toRelease;
