...
contract IndexNAV {
    ...
    function getNAV(IIndex _index) external view returns (uint _nav) {
        (address[] memory assets, ) = _index.anatomy();
        IvTokenFactory vTokenFactory = IvTokenFactory(_index.vTokenFactory()); 
        IIndexRegistry registry = IIndexRegistry(_index.registry());
        IPriceOracle priceOracle = IPriceOracle(registry.priceOracle());
        for (uint i; i < assets.length; ++i) {
          uint assetValue = IvToken(vTokenFactory.vTokenOf(assets[i])).totalAssetSupply();
