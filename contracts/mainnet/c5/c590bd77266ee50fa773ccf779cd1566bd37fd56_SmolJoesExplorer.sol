...
import "./interfaces/ISmolJoeSeeder.sol";
import "./interfaces/ISmolJoes.sol";

contract SmolJoesExplorer {
    ISmolJoes public smolJoes =
        ISmolJoes(0xB449701A5ebB1D660CB1D206A94f151F5a544a81);

    function isHundred(address _owner) public view returns (bool) {
        if (smolJoes.balanceOf(_owner) == 0) return false;

        return tokenOfOwnerByIndex(_owner, 0) < 100;
    }

    function isLuminary(address _owner) public view returns (bool) {
        if (smolJoes.balanceOf(_owner) == 0) return false;

        uint256[] memory tokens = tokensOfOwner(_owner);
        for (uint256 i; i < tokens.length; i++) {
            if (tokens[i] < 200 && tokens[i] >= 100) {
                return true;
            }
        }
        return false;
    ...
    function isSmol(address _owner) public view returns (bool) {
        if (smolJoes.balanceOf(_owner) == 0) return false;
    ...
    function isHouse(address _owner, uint256 _houseId) public view returns (bool) {
        if (smolJoes.balanceOf(_owner) == 0) return false;

        uint256[] memory tokens = tokensOfOwner(_owner);
        for (uint256 i; i < tokens.length; i++) {
            ISmolJoeSeeder.Seed memory joeSeed = smolJoes.getTokenSeed(tokens[i]);
            if (uint256(joeSeed.brotherhood) == _houseId) {
                return true;
            }
        }
        return false;
    ...
    function tokensOfOwner(
        ...
        unchecked {
            ...
            uint256 tokenIdsLength = smolJoes.balanceOf(_owner);
            uint256[] memory tokenIds = new uint256[](tokenIdsLength);
            address ownership;
            for (uint256 i = 0; tokenIdsIdx != tokenIdsLength; ++i) {
                try smolJoes.ownerOf(i) returns (address _ownership) {
