...
interface ITraderJoeXYZFactory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface ITraderJoeXYZRouter {
...
contract BabyBezos is Context, ERC20, Ownable {
    using SafeMath for uint256;

    ITraderJoeXYZRouter private _traderJoeXYZRouter;
    ...
    uint256 private constant _tSupply = 1e8 * (10**18);
    ...
    address payable private _mktgWallet = payable(0x1739FE834ED271502bC65E83B911FE25424f4091);
    address payable private _devWallet = payable(0x1739FE834ED271502bC65E83B911FE25424f4091);
    address payable private _liqWallet = payable(0xDAc2aD78389bFaC7DaC086A4aC9A4dD01eC287cc);

    address private _traderJoeXYZPair;
    address constant private DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant private ZERO = 0x0000000000000000000000000000000000000000;
    
    constructor() ERC20("Baby Bezos", "BEZOS") {
        _traderJoeXYZRouter = ITraderJoeXYZRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        _approve(address(this), address(_traderJoeXYZRouter), _tSupply);
        _traderJoeXYZPair = ITraderJoeXYZFactory(_traderJoeXYZRouter.factory()).createPair(address(this), _traderJoeXYZRouter.WAVAX());
        IERC20(_traderJoeXYZPair).approve(address(_traderJoeXYZRouter), type(uint).max);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            ...
            if (from == _traderJoeXYZPair && to != address(_traderJoeXYZRouter) && !_excludedMaxTx[to]) {
            ...
            if (to == _traderJoeXYZPair && from != address(_traderJoeXYZRouter) && !_excludedMaxTx[from]) {
    ...
    function _swapBack(uint256 contractBalance) internal {
        ...
        uint256 avaxForMktg = avaxBalance.mul(_tokensForMktg).div(totalTokensToSwap);
        uint256 avaxForDev = avaxBalance.mul(_tokensForDev).div(totalTokensToSwap);
        uint256 avaxForLiq = avaxBalance.sub(avaxForMktg).sub(avaxForDev);
    ...
    function swapTokensForAVAX(uint256 tokenAmount) internal {
        ...
        path[0] = address(this);
        path[1] = _traderJoeXYZRouter.WAVAX();
        _approve(address(this), address(_traderJoeXYZRouter), tokenAmount);
        _traderJoeXYZRouter.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 avaxAmount) internal {
        _approve(address(this), address(_traderJoeXYZRouter), tokenAmount);
        _traderJoeXYZRouter.addLiquidityAVAX{value: avaxAmount}(
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function bots(address[] memory accounts, bool bl) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
        	if((accounts[i] != _traderJoeXYZPair) && (accounts[i] != address(this)) && (accounts[i] != address(_traderJoeXYZRouter))) _isBot[accounts[i]] = bl;
        }
    }
