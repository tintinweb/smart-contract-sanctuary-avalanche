...
contract SoulAura is Ownable {
    ...
    function bondPower(address member) public view returns (uint raw, uint formatted) {
        ...
        uint THREE = bond.pendingSoul(3, member); // AVAX-USDC
        uint FOUR = bond.pendingSoul(4, member); // AVAX-BTC
        uint FIVE = bond.pendingSoul(5, member); // AVAX-ETH
        
        uint avaxBondPower = THREE + FOUR + FIVE;

        uint stableBondPower = bond.pendingSoul(6, member); // USDC-DAI
