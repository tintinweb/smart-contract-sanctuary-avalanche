...
ChainlinkVRFConsumerUpgradeable {
    ...
    EnumerableSetUpgradeable.UintSet private _broken1;
    ...
    function reveal() public {
        ...
        for (uint256 i = 0; i < _speciesIds.length; i++) {
            if (_speciesBase[_speciesIds[i]].get(0) == 0) {
                continue;
            }
            minRarity[i] = totalRarity;
            totalRarity += _speciesBase[_speciesIds[i]].get(1);
        }

        for (uint256 i = 0; i < pending.ids.length; i++) {
            // generation
            _attributes[pending.ids[i]].set(3, 0);
            // breed count
            _attributes[pending.ids[i]].set(6, 999);
            // experience
            _attributes[pending.ids[i]].set(7, 0);
    ...
    function speciesRoll(
        ...
        for (uint256 i = 0; i < minRarity.length; i++) {
            ...
            _attributes[tokenId].set(0, _speciesIds[i]);
            break;
        }
    }
    ...
    function geneRoll(
        ...
        _attributes[tokenId].set(1, visualRarity);
        // gender
        _attributes[tokenId].set(2, randomChunks[2] % 100 < _speciesBase[speciesId].get(2) ? 1 : 0);

        // primary type
        _attributes[tokenId].set(4, _speciesBase[speciesId].get(3));
        // secondary type
        _attributes[tokenId].set(5, _speciesBase[speciesId].get(4));

        // nature
        _attributes[tokenId].set(8, randomChunks[3] % 21);

        // stats
        for (uint256 i = 0; i < 5; i++) {
            _attributes[tokenId].set(i + 9, randomChunks[i + 4] % 50);
        }
    }
    ...
    function getExtraAttributes(uint256 tokenId) internal view returns (string memory) {
        string memory primaryType = _attributeStrings[4][_attributes[tokenId].get(4)];
        string memory secondaryType = _attributeStrings[5][_attributes[tokenId].get(5)];
    ...
    function getBattleAttributes(uint256 tokenId) internal view returns (string memory) {
        string memory nature = _attributeStrings[8][_attributes[tokenId].get(8)];
    ...
    function enableSpecies(uint256 speciesIds) public onlyRole(ADMIN_ROLE) {
        _speciesIds[0] = speciesIds;
        //require(speciesIds.length > 0, "no species defined");
        //for (uint256 i = 0; i < speciesIds.length; i++) {
        //    uint256 speciesId = speciesIds[i];
        //    require(_speciesIds.add(speciesId), "error adding id");
        //    require(_speciesBase[speciesId].set(0, 1), "error adding id");
        //}
    }
        ...
        for (uint256 i = 0; i < speciesIds.length; i++) {
            ...
            for (uint256 j = 0; j < baseAttributes.length; j++) {
                for (uint256 k = 0; k < baseAttributes[j].length; k++) {
                    _speciesBase[speciesId].set(j, baseAttributes[j][k]);
                }
            }
        }
    }
