...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SDREX }
    ...
    address public sDREX;
    uint public sDREXQueue; // Delays change to sDREX address
    ...
    /**
        @notice allow approved address to deposit an asset for DREX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint DREX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( DREX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn DREX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDREXERC20( DREX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sDREX ).balanceOf( msg.sender ); // Can only borrow against DREX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DREX
        @param _amount uint
     */
    function repayDebtWithDREX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDREXERC20( DREX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, DREX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( DREX ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( DREX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DREX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DREX decimals
            value_ = _amount.mul( 10 ** IERC20( DREX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SDREX ) { // 9
            sDREXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
