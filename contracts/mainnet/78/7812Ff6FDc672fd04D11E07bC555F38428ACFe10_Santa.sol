...
contract Santa is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 SANTA-WFTM LP -> SANTA pool
    uint256 public constant INITIAL_SANTA_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SANTA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SANTA Token", "SANTA") {
        // Mints 1 SANTA to contract creator for initial pool setup
    ...
    function _getSantaPrice() internal view returns (uint256 _santaPrice) {
        try IOracle(santaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Santa: failed to fetch SANTA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _santaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_santaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSantaOracle(address _santaOracle) public onlyOperatorOrTaxOffice {
        require(_santaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SANTA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SANTA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSantaPrice = _getSantaPrice();
            currentTaxRate = _updateTaxRate(currentSantaPrice);
            if (currentSantaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_santaPool != address(0), "!_santaPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_santaPool, INITIAL_SANTA_POOL_DISTRIBUTION);
