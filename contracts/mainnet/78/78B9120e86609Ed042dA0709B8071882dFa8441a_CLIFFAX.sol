 ...
 * Contract Code
 */
contract CLIFFAX is IBEP20, Ownable {
    ...
    // Basic Contract Info
    string constant _name = "CLIFFAX";
    string constant _symbol = "CLFX";
    ...
    address public marketingWallet = 0x5DF58DFF688CA2117af001B875A0138C853A7dbC; // 
    address public teamWallet = 0x5DF58DFF688CA2117af001B875A0138C853A7dbC;  //  
    address public gameWallet = 0x5DF58DFF688CA2117af001B875A0138C853A7dbC;     //   
    ...
    uint256 public _maxWalletSize = rSupply.div(100).mul(2);
    bool public sameBlockActive; 

    // DEX
    IDEXRouter public router;
    address public _pancakeRouterAddress=0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    //Pancake Mainnet: 0x60aE616a2155Ee3d9A68541Ba4544862310933d4 || Pancake Testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    address public pair;
...
// Swap and distribution  
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalAVAXFee = totalFees.sub(liquidity.div(2));
        
        uint256 AVAXLiquidity = amountAVAX.mul(liquidity).div(totalAVAXFee).div(2);
        uint256 AVAXGame = amountAVAX.mul(game).div(totalAVAXFee);
        uint256 AVAXMarketing = amountAVAX.mul(marketing).div(totalAVAXFee);
        uint256 AVAXTeam = amountAVAX.mul(team).div(totalAVAXFee);

        if(AVAXMarketing > 0) {payable(marketingWallet).transfer(AVAXMarketing);}
        if(AVAXGame > 0) {payable(gameWallet).transfer(AVAXGame);}
        if(AVAXTeam > 0) {payable(teamWallet).transfer(AVAXTeam);}
