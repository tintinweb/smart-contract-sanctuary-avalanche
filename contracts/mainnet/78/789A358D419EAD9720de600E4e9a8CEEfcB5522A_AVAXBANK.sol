...
contract AVAXBANK {
	...
	uint256 public INVEST_MIN_AMOUNT = 0.10 ether; 
    uint256 constant public DAY_LIMIT = 1000 ether; // only 1000 AVAX per day
	uint256 constant public MAX_INVESTS = 500; 
	...
	function withdraw() public {
        ...
        if(dLimit[cDay()][msg.sender] + totalAmount > DAY_LIMIT){
            user.bonus = dLimit[cDay()][msg.sender].add(totalAmount).sub(DAY_LIMIT);
            totalAmount = DAY_LIMIT.sub(dLimit[cDay()][msg.sender]);
        }
        ...
        require(totalAmount  < limit, "you must invest more than 50 avax to withdraw");
    ...
    function setting(uint256 w_limit) public project {
                             limit = w_limit ;
                             }
}
