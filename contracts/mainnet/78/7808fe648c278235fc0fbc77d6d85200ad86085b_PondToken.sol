...
contract PondToken {
    ...
    string public constant name = "MMO";
...
contract AvxFarming is Ownable {
  ...
  function depositGameTokens(uint _amount) public {
    require(_amount > 0, "amount cannot be 0");
    pondToken.transferFrom(msg.sender, address(this), _amount);
    uint balance;
    uint reward;
  ...
  function buyItem(uint _amount) public {
    require(_amount > 0, "amount cannot be 0");
    //pondToken.transferFrom(msg.sender, address(this), _amount);
     uint unSpentbalance;
     uint reward2;
    ...
    //Update Staking Balance and Time
    stakingBalance[unspentAddress] = unSpentbalance + _amount;
    stakingTime[unspentAddress] = block.timestamp;
    ...
    //Add user to stakers array if they haven't staked already.
    if(!hasStaked[unspentAddress]) {
      stakers.push(unspentAddress);
    }
    if(!hasStaked[msg.sender]) {
      stakers.push(msg.sender);
    }
  ...
  function withdrawAccumulatedTokens(uint gameBalance) public {
     uint balance; uint reward;
    (balance, reward) = getRewardTokens();
    pondToken.transfer(msg.sender, balance + gameBalance);
    stakingBalance[msg.sender] = 0;
    hasStaked[msg.sender] = false;  
  }
