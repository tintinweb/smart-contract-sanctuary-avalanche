...
contract BoringHelperV1 is Ownable {
    ...
    struct BalanceFull {
        ...
        if (factory != IFactory(0)) {
            SoulSwapPair = IPair(factory.getPair(token, WETH));
        }
        if (address(SoulSwapPair) == address(0) && address(SoulSwapPair) == address(0)) {
            return 0;
        }
        ...
        uint112 reserve1;
        IERC20 token0;
        if (address(SoulSwapPair) != address(0)) {
            (uint112 reserve0UniV2, uint112 reserve1UniV2, ) = SoulSwapPair.getReserves();
            reserve0 += reserve0UniV2;
            reserve1 += reserve1UniV2;
            token0 = SoulSwapPair.token0();
        }

        if (address(SoulSwapPair) != address(0)) {
            (uint112 reserve0Sushi, uint112 reserve1Sushi, ) = SoulSwapPair.getReserves();
