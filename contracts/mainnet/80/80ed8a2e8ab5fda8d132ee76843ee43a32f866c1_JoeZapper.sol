...
// File contracts/protocol/JoeZapper.sol
...
/// @notice Key Function for Zapping Joe Tokens to hJOE. 
/// Zaps JOE Tokens into hJOE and sends the users JOE to the Voter Contract for staking
/// Blackholes JOE Tokens and Stakes for veJOE (Voter)

contract JoeZapper is Ownable, ERC20 { 

    ///Steakhut Voter
    SteakHutVoter public voter; 
    
    /// @notice is Zapper Protocol Enabled
    bool zapperEnabled;

    /// @notice Joe Contract Address
    IERC20 public constant JOE = IERC20(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd);

    event hJOEMinted(address indexed user, uint256 amount);
    event ZapperEnabled(bool isEnabled);
    event SetVoterProxy(address voterProxy);
    ...
    /**
     * @notice Deposits JOE and mints hJOE at 1:1 ratio
     * @notice Deposits JOE to the voter contract that will stake them on veJoeStaking
     * @param _amount to deposit
     */
    function deposit(uint256 _amount) external {
        require(zapperEnabled, "JoeZapper:deposits disabled");
        require(JOE.transferFrom(msg.sender, address(voter), _amount), "JoeZapper:transfer failed");
        
        _mint(msg.sender, _amount); 
        voter._stakeJOE(_amount);

        emit hJOEMinted(msg.sender, _amount);      
    }

    /**
     * @notice Enable/Disable the JoeZapper
     * @param _isEnabled to Bool
     */
    function setZapperStatus(bool _isEnabled) external onlyOwner {
        zapperEnabled = _isEnabled;
        emit ZapperEnabled(_isEnabled);
    }

    /**
     * @notice check if the Zapper is taking deposits
     */
    function isZapperEnabled() external view returns(bool) {
        return zapperEnabled;
    }

    /**
    * @notice Update proxy address
    * @dev Very sensitive, restricted to owner
    * @param _voterAddress new address
    */
    function setVoterProxy(SteakHutVoter _voterAddress) external onlyOwner {
        voter = _voterAddress;
        emit SetVoterProxy(address(_voterAddress));
    }
