...
interface WNATIVE {
    ...
    function approve(address _address, uint amount) external returns (bool);
    function transfer(address _address, uint amount) external returns (bool);
}
...
DividendPayingTokenOptionalInterface
{
    ...
    returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = WNATIVE(rewardToken).transfer(
                user,
                _withdrawableDividend
            );
...
contract CNSTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
contract CNSTOKEN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CNSTOKENDividendTracker public dividendTracker;
    ...
    // CONSTRUCTOR
    //address[3] memory addrs, // reward token address, router address, dividendTracker address
    //uint256[2] memory feeSettings, // rewards, liquidity
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address[3] memory addrs, uint256[2] memory feeSettings, uint256 minimumTokenBalanceForDividends_) payable ERC20(name_, symbol_) {
        rewardToken = addrs[0];

        tokenRewardsFee = feeSettings[0];
        liquidityFee = feeSettings[1];
        totalFees = tokenRewardsFee.add(liquidityFee);
        require(totalFees <= 40, "Total fee is over 40%");
        ...
        dividendTracker = CNSTOKENDividendTracker(
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(
            pair != uniswapV2Pair,
            "CNSTOKEN: The pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "CNSTOKEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "CNSTOKEN: The dividend tracker already has that address"
        );

        CNSTOKENDividendTracker newDividendTracker = CNSTOKENDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "CNSTOKEN: The new dividend tracker must be owned by the CNSTOKEN token contract"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "CNSTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "CNSTOKEN: Cannot update gasForProcessing to same value"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "CNSTOKEN: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "CNSTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        //get wFUSE
        WNATIVE(rewardToken).approve(address(this), bal);
        WNATIVE(rewardToken).deposit{value : bal}();

        uint256 dividends =  WNATIVE(rewardToken).balanceOf(address(this));
        bool success = WNATIVE(rewardToken).transfer(
    ...
    function addLiquidity(uint256 tokenAmount, uint256 baseAmount) private {
        ...
        // add the liquidity
        uniswapV2Router.addLiquidityAVAX{value : baseAmount}(
            address(this),
            tokenAmount,
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // if from is one of the pair addresses, remove fee for buys
