 ...
 */
 contract Presale is Owned{
    ...
    uint public immutable RATE_DECIMALS = 10 ** 6;

    uint256 public minUsdcLimit = 300 * (10 ** 6);
    uint256 public maxUsdcLimit = 600 * (10 ** 6);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _usdc, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
    ...
    function buy(uint _amount) public {
        ...
        require(_amount >= minUsdcLimit, "You need to sell at least some min amount");
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].usdcAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].usdcAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
