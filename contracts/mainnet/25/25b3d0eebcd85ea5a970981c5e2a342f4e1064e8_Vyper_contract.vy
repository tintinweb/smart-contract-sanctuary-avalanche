...
@payable
@external
def add_liquidity(_amounts: uint256[N_UL_COINS], _min_mint_amount: uint256, use_eth: bool = False, _receiver: address = msg.sender):
    if not use_eth:
        assert msg.value == 0
...
@payable
@external
def exchange_underlying(i: uint256, j: uint256, _dx: uint256, _min_dy: uint256, use_eth: bool = False, _receiver: address = msg.sender):
    if not use_eth:
        assert msg.value == 0
...
@external
def remove_liquidity(_amount: uint256, _min_amounts: uint256[N_UL_COINS], use_eth: bool = False, _receiver: address = msg.sender):
...
@external
def remove_liquidity_one_coin(_token_amount: uint256, i: uint256, _min_amount: uint256, use_eth: bool = False, _receiver: address = msg.sender):
