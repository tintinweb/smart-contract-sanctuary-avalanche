...
contract VaultOwned is Ownable {
    ...
    function getCaller(uint256 _index) public view returns (address) {
        require(_index <= getCallerLength() - 1, "VaultOwned: index out of bounds");
    ...
    */
    modifier onlyVault() {
        require(isCaller(msg.sender), "LendToken: not the caller");
        _;
    }
...
contract LandToken is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    uint256 public constant maxSupply = 3e26;

    constructor() ERC20("AVAX Land", "LAND", 18) {
    }

    function mint(address account_, uint256 amount_) external onlyVault() {
        require(_totalSupply.add(amount_) <= maxSupply,"LnadToken: exceeds the maxSupply");
