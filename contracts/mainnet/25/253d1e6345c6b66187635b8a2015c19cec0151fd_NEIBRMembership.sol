...
// File contracts/NEIBRMembership.sol
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function changePrice(uint256 _membershipTypeIndex, uint256 _price)
        external
        onlyOwner
    {
        membershipTypes.array[_membershipTypeIndex].price = _price;
    }
    ...
    function mint(
        ...
        IERC20Upgradeable _neighbour = IERC20Upgradeable(neighbour);
        ...
        memberships[tokenCounter] = membership(
            _membershipTypeIndex,
            block.timestamp,
    ...
    function claim(
        ...
        membership memory _membership = memberships[tokenId];

        require(
            msg.sender == ownerOf(tokenId),
            "NEIBRMembership: Membership owner only"
        );

        require(
            _membership.boughtAt != 0,
        ...
        uint256 rewardDays = (claimTill - _membership.lastRewardClaimed) /
            1 days;

        uint256 reward = rewardDays *
            membershipTypes.get(_membership.membershipTypeIndex).dailyReward;

        IERC20Upgradeable(neighbour).transfer(msg.sender, reward);

        memberships[tokenId].lastRewardClaimed = claimTill;
...
function initialize() public initializer {
        __ERC721_init("Neighbour Membership", "NEIBRM");
