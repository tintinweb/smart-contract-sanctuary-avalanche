...
contract GMXPerpetualDEXLongVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  ...
  // ALPHA TEST: Mapping of whitelisted users
  ...
  */
  function deposit(uint256 _amt, bool /*_isNative*/) external payable nonReentrant whenNotPaused {
    ...
    token.safeTransfer(address(manager), _amt);

    uint256 sharesToUser = _deposit(_amt, reader.tokenValue(address(token), _amt));
  ...
  */
  function withdraw(uint256 _shareAmt, bool _isNative) external nonReentrant whenNotPaused {
    ...
    _withdraw(address(token), withdrawAmt, _isNative);
  ...
  /**
    * Emergency withdraw function, enabled only when vault is paused
    * @param _shareAmt Amount of svTokens to burn; expressed in 1e18
  */
  ...
  /**
    * Get manager to compound rewards from staked tokens
  */
  ...
  */
  function _deposit(uint256 _amt, uint256 _depositValue) internal returns (uint256) {
    ...
    require(equityBefore + _depositValue <= capacityValue, "Exceeded capacity");

     uint256 borrowTokenAmt = _depositValue
      * ((vaultConfig.targetLeverage - (1 * SAFE_MULTIPLIER)) / SAFE_MULTIPLIER )
      * SAFE_MULTIPLIER
      / priceOracle.consult(address(token))
      / reader.to18ConversionFactor(address(token));
  ...
  */
  ...
  /**
    * ALPHA TEST: Recover ERC20 tokens from this contract
    * and pause all deposits and normal withdrawals
  */
...
contract $GMXPerpetualDEXLongVault is GMXPerpetualDEXLongVault {
    ...
    constructor(string memory _name, string memory _symbol, IERC20 _token, IChainLinkOracle _priceOracle, GMXPerpetualDEXLongVault.VaultConfig memory _vaultConfig, uint256 _capacityValue, uint256 _mgmtFeePerSecond, uint256 _perfFee, address _treasury) GMXPerpetualDEXLongVault(_name, _symbol, _token, _priceOracle, _vaultConfig, _capacityValue, _mgmtFeePerSecond, _perfFee, _treasury) {}
    ...
    function $_withdraw(address _token,uint256 _withdrawAmt,bool arg2) external {
        super._withdraw(_token,_withdrawAmt,arg2);
    }
