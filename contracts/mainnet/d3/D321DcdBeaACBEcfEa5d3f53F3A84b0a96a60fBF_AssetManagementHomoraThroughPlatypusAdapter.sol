...
contract AssetManagementHomoraThroughPlatypusAdapter {
    ...
    constructor(address _usdc, address _ibUSDC, address _usdcIronBank, address _PlatypusPool, address _caller, address _manager) public {
        ...
        _addAssetAndEnterMarkets(_usdc, _ibUSDC, _usdcIronBank, _PlatypusPool, IDetailedERC20(_usdc).decimals());
    }
    ...
    function addAsset(address asset, address ibAsset, address assetIronBank) external {
        ...
        _addAssetAndEnterMarkets(asset, ibAsset, assetIronBank, PlatypusPool, decimals);
    }
    ...
    function allocate(AllocInputs memory input) external {
        ...
        // If fromAssetDifference - toAssetDifference > 0, it should be registered within toInfo.txCosts.
        if(fromAssetDifference > toAssetDifference) {
    ...
    function _addAssetAndEnterMarkets(address _asset, address _ibAsset, address _ironBankAsset, address _dexPool, uint256 _decimals) internal {
        bool check = _assets.add(_asset);
        require(check, "Error: Check reverted");

        assetInfo[_asset] = AssetInfo(_ibAsset, _ironBankAsset, _decimals, 0, 0);
        
        bool check2 = IDetailedERC20(_asset).approve(_ibAsset, type(uint256).max);
        require(check2, "Error: Check reverted");

        bool check3 = IDetailedERC20(_asset).approve(_dexPool, type(uint256).max); 
        require(check3, "Error: Check reverted");
