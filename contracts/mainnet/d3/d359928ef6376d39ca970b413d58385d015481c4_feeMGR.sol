...
contract feeMGR is Ownable{
    ...
    function MGRcreateProto(address[]  memory _accounts,string[] memory _names, uint256[] memory _nameNums,uint256[] memory _pays) external managerOnly(){
    	for(i=0;i<_accounts.length;i++){
    		address _account = _accounts[i];
    		for(j=0;j<_nameNums[i];j++){
    			INTcreateProtos(_account,_names[i+j]);
    			if(_pays[i]!=0){
    				recPayFees(_account);
    				_pays[i]--;
    			}
    ...
    function createProtos(address _account,string memory _name) external onlyGuard(){
    	INTcreateProtos(_account,_name);
    }
    	    
    function INTcreateProtos(address _account,string memory _name) internal {
    ...
    function approveFee(address _account,uint256 _intervals) internal {
        uint256 totTok = tokFee.mul(_intervals);
    	feeTok.approve(_account, totTok);
    }
    function queryFeeToken(address _account,uint256 _intervals) internal {
        queryFees(_account);
        uint256 _fee = tokFee.mul(_intervals);
        approveFee(_account,_intervals);
        TOTALFEES storage acctTot = totalFees[_account];
        uint256 allowance = feeTok.allowance(_account, address(this));
        require(allowance >= tokFee, "Check the token allowance");
        uint256 allowance2 = feeTok.allowance(_account, treasury);
        require(allowance2 >= tokFee, "Check the token allowance");
        require(acctTot.totalPayable > 0,"you dont owe any fees");
        
    	require(feeTok.balanceOf(_account) <= _fee,"you do not hold enough FeeTokens to pay the amount of fees that you have selected");
    }
    function payAaxFee(uint256 _intervals, address _account) payable external onlyGuard() {
        queryFees(_account);
        TOTALFEES storage acctTot = totalFees[_account];
    ...
    function queryFees(address _account) internal {
    	...
    	tots.spareFees -= acctTot.spareFees;
	acctTot.protos = Zero;
	...
	tots.protos += acctTot.protos;
    	...
    	tots.futureFees += acctTot.futureFees;
    	tots.totalPayable += acctTot.totalPayable;
    	acctTot.spareFees = acctTot.protos -acctTot.feesOwed;
    	tots.spareFees +=acctTot.spareFees;
    }
