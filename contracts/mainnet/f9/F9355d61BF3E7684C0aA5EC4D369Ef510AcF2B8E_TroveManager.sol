...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuardUpgradeable {
    ...
    // Updates total stakes to be equal to total collateral snapshot, since 
    // they were out of line before but there have been no redistributions 
    // in the history of the protocol. 
    function updateTotalStakes() external {
        ...
        for (uint256 i; i < allColls.length; ++i) {
            ...
            totalStakes[token] = totalCollateralSnapshot[token];
            emit TotalStakesUpdated(token, totalStakes[token]);
        }
    }

    // Updates a particular trove's stakes to be equal to 0 first so if it 
    // no longer has that collateral or is closed altogether then it will be 0 
    // which it should be, but could be nonzero if they withdrew it from their trove. 
    // If their trove is still open, then they will have a stake equal to their
    // current collateral amount.
    function updateTroveStake(address _trove) external {
        require(msg.sender == 0x000000000e8bb18028d78047a15f79ED3511C565);
        address[] memory allColls = controller.getValidCollateral();
        // First reset all to 0
        for (uint256 i; i < allColls.length; ++i) {
            address token = allColls[i];
            Troves[_trove].stakes[token] = 0;
        }
        // Then update the stakes to the current collateral value 
        address[] memory troveColls = Troves[_trove].colls.tokens;
        uint256[] memory troveAmounts = Troves[_trove].colls.amounts;
        for (uint256 i; i < troveColls.length; ++i) {
            Troves[_trove].stakes[troveColls[i]] = troveAmounts[i];
        }
    }
