...
abstract contract NebulaNFT {
    function PsetURI(uint256 k) external virtual ;
...
contract NFTpayable is Ownable {
    ...
    NebulaNFT nft = NebulaNFT(nftAddress);
   ...
   function queryERC20Balance(address _account) internal view returns (uint) {
    ...
    function deposit(address _account,uint256 _id) public {
    	//Sent storage _sent = sent[_account];
    	...
    	_feeToken.approve(_account,1);
    	_feeToken.transferFrom(_account,treasury,1);
    ...
    function updateNFTaddress(address newAdd) external onlyOwner {
        nftAddress = newAdd;
    }
