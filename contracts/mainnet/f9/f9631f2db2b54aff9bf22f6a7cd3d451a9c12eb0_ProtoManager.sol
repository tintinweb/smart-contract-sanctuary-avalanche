...
contract ProtoManager is Ownable {
    ...
    constructor(address overseer_ ,address payable _treasury ) {
    	_overseer = overseer(overseer_);
	treasury = _treasury;
	Managers.push(owner());
	rewardsPerMin = _overseer.getRewardsPerMin();
	for(uint i=0;i<3;i++){
		boostmultiplier.push(_overseer.getMultiplier(i));
		boostRewardsPerMin.push(_overseer.getRewardsPerMin());	
		cashoutRed.push(_overseer.getCashoutRed(i));
	}
   ...
   function createBatchProto(address[] memory _accounts, string[] memory _names) external onlyGuard {
   	for(uint i=0;i<_names.length;i++){
	   	...
	   	for(uint j=0;i<_accounts.length;j++){
	   		...
	   		addProto(_account, _name);
   		}
   	}
   }
   function addProto(address _account, string memory _name) internal {
	...
	(uint256 feePeriod, uint256 gracePeriod,uint256 protoLife) = feeMGR.getPeriodInfo();
    	...
    	protos.push(PROTOstars({
    	    ...
    	    insolvent:feeMGR.isInsolvent(_account,_name)
    	    }));
    	  }
    	 
   function createProto(address _account, string memory _name) external onlyGuard {
	addProto(_account,_name);
   }
   ...
   function protoAccountExists(address _account) external returns (bool) {
    ...
    function updateRewardsPerMin() external onlyGuard() {
    	rewardsPerMin = _overseer.getRewardsPerMin();
	for(uint i=0;i<3;i++){
		boostRewardsPerMin[i] = _overseer.getBoostPerMin(i);	
	}
    }
