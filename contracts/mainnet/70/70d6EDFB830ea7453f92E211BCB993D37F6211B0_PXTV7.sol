...
contract PXTV7 is ERC20Upgradeable, IBridgeToken {
    // STRINGS
    ...
    function initialize() public virtual initializer {
        ...
        usdtAddress = 0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E; // 0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E
        ...
        address currentRouter = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4; // 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
        ...
        excludeFromFee(w1, true);
        excludeFromFee(w2, true);
        decimalMultiplier = 1;
    }
    ...
    function calcBuySellTransferFee(
        ...
        // add this fee percent to sell fee percent
            ...
            totalFeePercent += calculateWhaleFeePercent(amount, currentNetwork);
        }
        // TRANSFER
        else {
    ...
    function _beforeTransferCheck(
        ...
        if (createNodeBeforeSellCheck && automatedMarketMakerPairs[to]) {
            ...
            */
            require(
                nodeManager.countOfUser(from) > 0,
                "Insufficient Node count!"
            );
    ...
    function updateDataFeedsAddress(address addr) public virtual onlyOwner {
        priceFeed = AggregatorV3Interface(addr);
    }

    function updateUsdtAddress(address addr) public virtual onlyOwner {
        usdtAddress = addr;
    }

    function calculateWhaleFeePercent(uint256 amount, string memory network)
        public
        view
        virtual
        returns (uint256)
    {
        ...
        if (antiWhaleUsdtLimit1 > 0) {
            uint256 usdtAmount = getAmountOutUSD(amount, network);
        ...
        returns (uint256)
    {
        ...
        if (availableChainlinkNetworks[network]) {
            ...
            if (ERC20(usdtAddress).decimals() != decimals()) {
    ...
    function enableAvailableChainlinkNetworks(string memory network, bool value)
        public
        virtual
        onlyOwner
    {
    ...
    function updateDecimalMultiplier(uint256 val) public virtual onlyOwner {
        decimalMultiplier = val;
    }
}
