...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x54B953B2467575F44D958fD07dFbA0A359F3CB77), // GraveGenesisPool
        address(0x168944972765272F529294874002d34bEE8B8DCD) // new GraveRewardPool
    ];

    // core components
    address public grave;
    address public bbond;
    address public graveshare;
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(grave).operator() == address(this) &&
                ...
                IBasisAsset(graveshare).operator() == address(this) &&
    ...
    function getBurnableGraveLeft() public view returns (uint256 _burnableGraveLeft) {
        ...
        if (_gravePrice <= gravePriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableGrave = _maxMintableBond.mul(_gravePrice).div(1e16);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_gravePrice > gravePriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalGrave.mul(1e16).div(_rate);
            }
        }
    }
    ...
    function initialize(
        ...
        grave = _grave;
        bbond = _bbond;
        graveshare = _graveshare;
        graveOracle = _graveOracle;
        boardroom = _boardroom;
        startTime = _startTime;

        gravePriceOne = 10**16; // This is to allow a PEG of 100 GRAVE per wBNB
    ...
    function buyBonds(uint256 _graveAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _graveAmount.mul(_rate).div(1e16);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _graveAmount = _bondAmount.mul(_rate).div(1e16);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        IERC20(grave).safeApprove(boardroom, 0);
        IERC20(grave).safeApprove(boardroom, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochGravePrice > gravePriceCeiling) {
                // Expansion ($GRAVE Price > 1 $ETH): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForBoardroom = graveSupply.mul(_percentage).div(1e16);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = graveSupply.mul(_percentage).div(1e16);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(graveshare), "share");
