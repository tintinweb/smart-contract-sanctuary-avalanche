...
import "./Soda.sol";
import "./Pizza.sol";
import "./SodaStationRobotV5.sol";

contract SodaStationV12 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function getSodaAccruedForSPD(address _owner, uint256 _spd, uint256 _spdPenalty) internal view returns (uint256) {
        ...
        if(endTimeForBoost > startTimeOwner) {
            uint256 endBonus;
            uint256 boost = energyBoostForOwner[_owner];
            uint256 baseBonus = _spdPenalty * energyDrinkModifier;
            if(boost > baseBonus){
                boost = baseBonus;
            }
            ...
            if(endTimeForBoost > endTime) {
                ...
                endBonus = endTimeForBoost;
            }
            result += (endBonus - startTimeOwner)* boost * bonus / 10000000000;
        }
    ...
    function _payCup(address owner, CupType memory cupType, uint256 length) internal {
        uint256 transactionCost = cupType.lpCost * length * (100 - getSizeSkillModifier(owner))/100;
    ...
    function createCupForPlayer(uint256 _cupType, uint256[] calldata _upgradeIds) external {
        ...
        _payCup(owner, cupType, upgradeLength);
    ...
    */
    function _updateLevel(address owner, uint256 amount) internal {
    ...
    */
    ...
    function repairCups(uint256[] calldata _cupIds) external {
        ...
        for (uint256 i = 0; i < cupsLength; i++) {
            ...
            uint256 repairCost = (cupType.lpCost + cup.repairs * cupType.lpCost / 4)* _repairDiscount(owner)/1000000;  
    ...
    function _repairDiscount(address owner) public view returns (uint256) {
        return (100 - getSizeSkillModifier(owner))* (100 - getRepairSkillModifier(owner)) * (100 - sodaStationRobot.getNuclearCoreBonus(owner));
    }

    function repairCupsWithScraps(uint256[] calldata _cupIds) external {
        address owner = msg.sender;
        _updateState(owner);
        ...
        for (uint256 i = 0; i < cupsLength; i++) {
            ...
            StationCups storage cup = stationCups[cupId];
            require(cup.owner == owner, "You don't own this cup");
            uint256 aux_cost = 2 * (1 + cupTypes[cup.cupType].tier) * (4 + cup.repairs)*_repairDiscount(owner)/4000000;
            if(aux_cost == 0){
                aux_cost = 1;
            }
            cost += aux_cost;
        ...
        require(scrapsDeposited[owner] >= 100 + cost, "you don't have enough scraps");
        scrapsDeposited[owner] -= cost;
    }
    ...
    function batchedCupsOfOwner(
        ...
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (StationCupInfoV2[] memory) {
        if (_offset >= ownedCupsBalance[_owner]) {
            return new StationCupInfoV2[](0);
        }
        ...
        StationCupInfoV2[] memory outputs = new StationCupInfoV2[](outputSize);
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            uint256 repairCost = (cupType.lpCost + cup.repairs * cupType.lpCost / 4)* _repairDiscount(_owner) /1000000;
            uint256 repairCostScrap = 2 * (1 + cupType.tier) * (4 + cup.repairs)* _repairDiscount(_owner) /4000000;
            if(repairCostScrap == 0 ){
                repairCostScrap = 1;
            }

            outputs[i] = StationCupInfoV2({
                cupId: cupId,
                cupType: cup.cupType,
                yield: cupType.yield,
                claims: cup.claims,
                ...
                repairCostScrap: repairCostScrap
            });
        }
    ...
    function energyDrink(uint256 _qty) public {
        ...
        costToReset = costToReset * (100 - sodaStationRobot.getEnergyDrinkCostReduction(_owner))/100;
    ...
    SodaStationRobotV5 public sodaStationRobot;
    ...
    function scrapAssistants(uint256[] calldata _assistantsIds) external {
        address owner = msg.sender;
        _updateState(owner);
        uint256 amount = sodaStationRobot.scrapAssistants(owner, _assistantsIds);
        _updateLevel(owner, amount);
    }

    struct StationCupInfoV2 {
        uint256 cupId;
        uint256 cupType;
        uint256 yield;
        uint256 claims;
...
contract SodaStationRobotV5 is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable
{
    ...
    event assistantScraped(address owner, uint256 assistantId);


    struct StakedRobotChefInfoV2 {
        address owner;
        uint256 robotId;
        uint256 robotType;
        uint256 robotUpgradeType;
    ...
    function getProductionBonus(address _owner) public view returns (uint256 totalBonus) {
        if(numberOfStaked[_owner][5] <= 10){
            ...
            totalBonus = 20 + 5*numberOfStaked[_owner][0] + 15*numberOfStaked[_owner][1];
        }
    }
    ...
    function stakeAssistants(address owner, uint256[] calldata _assistantsIds) public {
        ...
        if(factorySkills.hasSkill(owner,28)){
            maxAssistants = 15;
        }
        if(factorySkills.hasSkill(owner,29)){
            maxAssistants = 20;
        }

        require(assistantLength + ownedAssistantStakesBalance[owner]<= maxAssistants, "invalid number of assistants");
    ...
    function scrapAssistants(address owner, uint256[] calldata _assistantsIds) external returns (uint256 amount) {
         require(_msgSender() == sodaStationAddress, "msgsender does not have permission");
         require(factorySkills.hasSkill(owner,30), "you don't have the skill for that");
        ...
        for (uint256 i = 0; i < assistantLength; i++) {
            ...
            pizzaRobot.transferFrom(owner, treasuryWallet, assistantsId);
            emit assistantScraped(owner, assistantsId);
        }
    }
    ...
    function batchedRobotChefsOfOwner(
        ...
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (StakedRobotChefInfoV2[] memory) {
        if (_offset >= ownedChefsStakesBalance[_owner]) {
            return new StakedRobotChefInfoV2[](0);
        }
        ...
        StakedRobotChefInfoV2[] memory outputs = new StakedRobotChefInfoV2[](outputSize);

        for (uint256 i = 0; i < outputSize; i++) {
            uint256 robotId = ownedChefsStakes[_owner][_offset + i];
            outputs[i] = StakedRobotChefInfoV2({
                owner: _owner,
                robotId: robotId,
