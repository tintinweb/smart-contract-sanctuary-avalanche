    ...
    ERC2981
{
    ...
    constructor(string memory uri) ERC721("NFDNA", "NFDNA") {
    ...
    function batchMintToAccounts(
        address[] calldata accounts,
        uint256[] calldata ids
    ) external onlyRole(MINTER_ROLE) {
        require(
            accounts.length == ids.length,
            "Provided arrays must be same length"
        );
    ...
    function batchMintToAccount(address to, uint256[] calldata ids)
        external
        onlyRole(MINTER_ROLE)
    {
