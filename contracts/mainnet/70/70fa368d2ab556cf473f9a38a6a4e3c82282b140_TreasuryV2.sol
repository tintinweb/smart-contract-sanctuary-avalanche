...
interface ITreasury {
    ...
    function balanceOfDLC() external view returns (uint256);
    function feeDistributorWithdraw(uint256 amount_) external;
}

contract TreasuryV2 is ITreasury, ReentrancyGuardUpgradeable, AccessControlled {
    ...
    function deposit(
        ...
        if (value_ >= 100 && amount_ >= 100 && feeDistributor != address(0)) {
            if (devTeamFeePercent != 0) {
                ...
                IERC20Upgradeable(dlc).approve(feeDistributor, devTeamFee);
            ...
            if (liqFeePercent != 0) {                
                ...
                IERC20Upgradeable(token_).approve(feeDistributor, liqFee);
    ...
    /**
     * @notice allow fee distributor to withdraw DLC required for adding to liquidity
     * @param amount_ uint256
     */
    function feeDistributorWithdraw(uint256 amount_) external {
        require(msg.sender == feeDistributor, notApproved); 
        require(balanceOfDLC() >= amount_, notAccepted); 
        IERC20Upgradeable(dlc).safeTransfer(msg.sender, amount_);        
    }
