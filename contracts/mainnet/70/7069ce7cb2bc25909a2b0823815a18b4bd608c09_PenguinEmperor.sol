...
contract PenguinEmperor is OwnableInitialized {
    ...
    function stealCrownAndPoison(uint256 amount, address paidByToken) external canBetWithToken(paidByToken) stealCrownCheck()  returns (bool bidFailed) {
        ...
        bidFailed = _stealCrown(totalAmount, emperor, emperorStatus, callerStatus);
        return bidFailed;
    }
    ...
    function castHailStorm() external {
        ...
        uint256 sentAmt = _transferToNestFrom(msg.sender, TOKEN_TO_BUY_POTIONS, FROST_POTION_PRICE);
        totalSentToNest += sentAmt;
    ...
    function castPerplex() external {
        ...
        uint256 sentAmt = _transferToNestFrom(msg.sender, TOKEN_TO_BUY_POTIONS, PERPLEX_POTION_PRICE);
        totalSentToNest += sentAmt;
    ...
    function castBanana() external {
        ...
        uint256 sentAmt = _transferToNestFrom(msg.sender, TOKEN_TO_BUY_POTIONS, BANANA_POTION_PRICE);
        totalSentToNest += sentAmt;
    ...
    function castVigor() external {
        ...
        uint256 sentAmt = _transferToNestFrom(msg.sender, TOKEN_TO_BUY_POTIONS, VIGOR_POTION_PRICE);
        totalSentToNest += sentAmt;
    ...
    function castToxin() external {
        ...
        uint256 sentAmt = _transferToNestFrom(msg.sender, TOKEN_TO_BUY_POTIONS, TOXIN_POTION_PRICE);
        totalSentToNest += sentAmt;
    ...
    function _transferToNestFrom(address _from, address token, uint256 amount) internal returns(uint256){
        if(token == NEST) {
            ...
            // Token is AVAX
            payable(NEST_ALLOCATOR).transfer(amount);
            emit TransferToNestFrom(_from, amount);
            return amount;
        } else {
