...
contract New_router4 is ERC20, ERC20Burnable, Ownable {
    ...
    constructor()
        ERC20("New router4", "NRT4") 
    {
        address supplyRecipient = 0x163c7B8729198e9CCB912F8e29c0dAF9e1367CaC;
        
        updateSwapThreshold(5 * (10 ** decimals()));

        marketingAddressSetup(0x059Da6A5aDb2e07946c8A0e7bb80cC63d62de67d);
        marketingFeesSetup(200, 200, 200);

        charityAddressSetup(0x059Da6A5aDb2e07946c8A0e7bb80cC63d62de67d);
        charityFeesSetup(200, 0, 0);

        autoBurnFeesSetup(200, 0, 0);

        lpTokensReceiverSetup(0x0000000000000000000000000000000000000000);
        liquidityFeesSetup(0, 300, 0);

        excludeFromFees(supplyRecipient, true);
        excludeFromFees(address(this), true); 

        _updateRouterV2(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);

        _mint(supplyRecipient, 6969 * (10 ** decimals()));
    ...
    function _swapTokensForCoin(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = routerV2.WAVAX();

        _approve(address(this), address(routerV2), tokenAmount);

        routerV2.swapExactTokensForAVAXSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp);
    }
    ...
    function marketingFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        ...
        totalFees[0] = 0 + marketingFees[0] + charityFees[0] + autoBurnFees[0] + liquidityFees[0];
        totalFees[1] = 0 + marketingFees[1] + charityFees[1] + autoBurnFees[1] + liquidityFees[1];
        totalFees[2] = 0 + marketingFees[2] + charityFees[2] + autoBurnFees[2] + liquidityFees[2];
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 coinAmount) private returns (uint, uint, uint) {
        ...
        return routerV2.addLiquidityAVAX{value: coinAmount}(address(this), tokenAmount, 0, 0, lpTokensReceiver, block.timestamp);
    }
    ...
    function _transfer(
        ...
        bool canSwap = 0 + _marketingPending + _charityPending + _liquidityPending >= swapThreshold;
        
        if (!_swapping && !AMMPairs[from] && canSwap) {
            _swapping = true;
            
            if (false || _marketingPending > 0 || _charityPending > 0) {
                uint256 token2Swap = 0 + _marketingPending + _charityPending;
                bool success = false;
    ...
    function _updateRouterV2(address router) private {
        routerV2 = IJoeRouter02(router);
        pairV2 = IJoeFactory(routerV2.factory()).createPair(address(this), routerV2.WAVAX());
