...
import {OhStrategy} from "@ohfinance/core/contracts/strategy/OhStrategy.sol";
...
contract OhYetiStrategy is OhStrategy, OhYetiStrategyStorage, OhYetiHelper, IStrategy {
    ...
    /// @param compounder_ The YETI Compounder used by all the YETI strategies
    ...
    function initializeYetiStrategy(
        ...
        initializeYetiStorage(compounder_, index_);

        IERC20(underlying_).approve(compounder_, type(uint256).max);
    }
    ...
    function _compound() internal {
        ...
        IYetiCompounder(compounder).claim(index());
    ...
    function _deposit() internal {
        ...
        if (amount > 0) {
            ...
            IYetiCompounder(compounder).addLiquidity(underlying(), index(), amount, 1);
            // stake all received in the YETI LP Farm Pool
            IYetiCompounder(compounder).stake();
        }
    }
    ...
    function _withdraw(address recipient, uint256 amount) internal returns (uint256) {
        ...
        IYetiCompounder(compounder).removeLiquidity(underlying, address(this), index(), unstakeAmount, minAmount);
...
import {IStrategyBase} from "../interfaces/strategy/IStrategyBase.sol";
...
import {IYetiStrategyStorage} from "./interfaces/IYetiStrategyStorage.sol";
...
contract OhYetiStrategyStorage is Initializable, OhUpgradeable, IYetiStrategyStorage {
    ...
    function initializeYetiStorage(address yetiCompounder_, uint256 index_) internal initializer {
...
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/Initializable.sol";
import {IStrategyStorage} from "../interfaces/strategy/IStrategyStorage.sol";
import {OhUpgradeable} from "../proxy/OhUpgradeable.sol";
