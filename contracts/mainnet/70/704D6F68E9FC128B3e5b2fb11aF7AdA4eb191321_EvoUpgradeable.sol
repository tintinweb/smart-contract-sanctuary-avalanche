...
ChainlinkVRFConsumerUpgradeable {
    ...
    function chainlinkCheck(address _address) public view onlyRole(ADMIN_ROLE) returns (uint256[]memory, uint256[]memory, uint256) {
        PendingHatch storage pending = _pendingHatches[_address];
        return (pending.ids, pending.words, pending.requestId);
    }

    function startHatching(uint256[] memory tokenIds) public {
    ...
    function simulateReveal(address _address) public view onlyRole(ADMIN_ROLE) returns(PendingHatch memory, Evo[] memory evos) {
        return (_pendingHatches[_address], _reveal(_address));
    }

    function reveal() public {
        Evo[] memory evos = _reveal(_msgSender());
        for (uint256 i = 0; i < evos.length; i++) {
            _attributes[evos[i].tokenId].set(0, evos[i].species);
            _attributes[evos[i].tokenId].set(1, evos[i].stats.rarity);
            _attributes[evos[i].tokenId].set(2, evos[i].stats.gender);
            _attributes[evos[i].tokenId].set(3, evos[i].generation);
            _attributes[evos[i].tokenId].set(4, evos[i].stats.primaryType);
            _attributes[evos[i].tokenId].set(5, evos[i].stats.secondaryType);
            _attributes[evos[i].tokenId].set(6, evos[i].summons.total);
            _attributes[evos[i].tokenId].set(7, evos[i].experience);
            _attributes[evos[i].tokenId].set(8, evos[i].stats.nature);
            _attributes[evos[i].tokenId].set(9, evos[i].battle.attack);
            _attributes[evos[i].tokenId].set(10, evos[i].battle.defense);
            _attributes[evos[i].tokenId].set(11, evos[i].battle.special);
            _attributes[evos[i].tokenId].set(12, evos[i].battle.resistance);
            _attributes[evos[i].tokenId].set(13, evos[i].battle.speed);
            _safeMint(_msgSender(), evos[i].tokenId);
        }
    ...
    function _reveal(address _address) internal view returns(Evo[] memory evo) {
        PendingHatch storage pending = _pendingHatches[_address];
        ...
        evo = new Evo[](pending.ids.length);
        for (uint256 i = 0; i < pending.ids.length; i++) {
            ...
            uint256 speciesId = speciesRoll(randomChunks, minRarity, speciesIds, totalRarity);
    ...
    internal view returns (Evo memory evo) {
        ...
        evo = Evo({
         ...
         generation: 0,
         experience: 0,
         summons: Summons(999, 999),
         stats: Stats(gender, rarity, primaryType, secondaryType, nature),
         battle: BattleStats(50, battle[0], battle[1], battle[2], battle[3], battle[4])
        });
    }
    ...
    function getSpeciesBaseAttributes(uint256 speciesId) public view returns(uint256[] memory) {
        ...
        for (uint256 i = 0; i < 5; i++) {
            baseAttributes[i] = _speciesBase[speciesId].get(i);
        }
    ...
    function setBaseAttributes(uint256 speciesId, uint256[] memory _baseAttributes) public onlyRole(ADMIN_ROLE) {
        _speciesIds.add(speciesId);
        for (uint256 i = 0; i < _baseAttributes.length; i++) {
            _speciesBase[speciesId].set(i, _baseAttributes[i]);
        }
    }
    function batchSetBaseAttributes(uint256[] memory speciesIds, uint256[][] memory _baseAttributes) public onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < speciesIds.length; i++) {
            setBaseAttributes(speciesIds[i], _baseAttributes[i]);
        }
    }
    ...
    {
        require(tokenId > 50 || from == address(0) || to == address(0), "Staff Evo are non-transferable");
