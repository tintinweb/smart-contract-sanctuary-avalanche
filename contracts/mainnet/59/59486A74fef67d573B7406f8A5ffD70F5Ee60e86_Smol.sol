...
contract Smol is ERC721Enumerable, Ownable, ReentrancyGuard, ERC2981 {
    uint256 private a;
    uint256 constant public MAX_APES = 100;
    bytes32 private merkleRoot;
    mapping(address => bool) public minted;
    string private _baseTokenURI;

    constructor(address receiver, uint96 feeNumerator)
        ERC721("Smol Apes", "APE")
    {
        _setDefaultRoyalty(receiver, feeNumerator);
    }

    function ape(uint256 b, bytes32[] calldata _merkleProof) public {
        require(
            tx.origin == msg.sender,
            "I don't deal with contracts!"
        );

        require (b == 8888, "wrong human");
        require(a == 1, "a is not greater than 0");
        
        require(totalSupply() < MAX_APES, "no more apes, I'm sorry");
        require(!minted[msg.sender], "you already minted an ape");
