...
contract LittleScurries is Ownable, ERC721, NonblockingReceiver {

    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 8;
    uint256 MAX_MINT_AVALANCHE = 9;

    uint gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint) ERC721("LittleScurries", "ls") { 
        _owner = msg.sender;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 3, "LS: Max 2 NFTs per transaction");
        require(nextTokenId + numTokens <= MAX_MINT_AVALANCHE, "LS: Mint exceeds supply");
    ...
    function traverseChains(uint16 _chainId, uint tokenId) public payable {
        ...
        require(msg.value >= messageFee, "LS: msg.value not enough to cover messageFee. Send gas for message fees");
    ...
    function withdraw(uint amt) external onlyOwner {
        ...
        require(sent, "LS: Failed to withdraw Ether");
    }
