...
contract Medal is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 MEDAL-WETH LP -> MEDAL pool
    // uint256 public constant INITIAL_MEDAL_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MEDAL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MEDAL", "MEDAL") {
        // Mints 1 MEDAL to contract creator for initial pool setup
    ...
    function _getMedalPrice() internal view returns (uint256 _medalPrice) {
        try IOracle(medalOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Medal: failed to fetch MEDAL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _medalPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_medalPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMedalOracle(address _medalOracle) public onlyOperatorOrTaxOffice {
        require(_medalOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MEDAL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MEDAL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMedalPrice = _getMedalPrice();
            currentTaxRate = _updateTaxRate(currentMedalPrice);
            if (currentMedalPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        // _mint(_cometPool, INITIAL_MEDAL_POOL_DISTRIBUTION);
