...
contract SharesDistributor is Administrable {
    ...
    constructor(address[] memory accounts, uint256[] memory points) {
        Zshare = IERC20(0x1d35812E66DFeE0926802FE2143b7BCF159faf76);
    ...
    function distribute() external onlyAdmin {
        uint256 balance = Zshare.balanceOf(address(this));
        for (uint256 a = 0; a < allocations.length; a ++) {
            Zshare.transfer(allocations[a].account, balance * allocations[a].points / totalPoints);
        }
    }

    function setZshare(address share) external onlyOwner {
        Zshare = IERC20(share);
    }
