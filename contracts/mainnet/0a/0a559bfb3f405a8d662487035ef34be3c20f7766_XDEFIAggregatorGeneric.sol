...
/**
 *Submitted for verification at snowtrace.io on 2022-11-09
*/
...
contract XDEFITokenTransferProxy is Ownable {
    ...
    function transferTokens(address token, address from, address to, uint256 amount) external onlyOwner {
...
// File contracts/XDEFIAggregatorGeneric.sol
...
contract XDEFIAggregatorGeneric is XDEFIAggregator {
    ...
    constructor(address _tokenTransferProxy, address[] memory whitelist_) XDEFIAggregator(_tokenTransferProxy) {
    ...
    function swapIn(
        ...
        if (!isWhitelisted[router]) revert ContractNotWhitelisted(router);

        tokenTransferProxy.transferTokens(token, msg.sender, address(this), amount);
        // Approve the contract that will pull token.
