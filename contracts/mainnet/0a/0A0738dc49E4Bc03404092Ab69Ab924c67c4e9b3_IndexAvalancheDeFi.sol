    ...
    IIndexStrategy
{
    ...
    using SafeERC20Upgradeable for IIndexToken;
    ...
    modifier whenNotReachedEquityValuationLimit() {
        ...
        if (equityValuation() > equityValuationLimit) {
    ...
    function __IndexStrategyUpgradeable_init(
        ...
        indexToken = initParams.indexToken;
        ...
        returns (uint256 amountTokenSpent)
    {
        ...
        if (token != WETH) {
            (uint256 amountTokenTotal, address bestRouter) = _getAmountInMin(
                routers[token],
                amountWETHTotal,
                token,
                WETH
            ...
            if (amountTokenSpent != amountTokenTotal) {
        ...
        returns (uint256 amountToken)
    {
        ...
        if (token != WETH) {
            (uint256 amountOutMax, address bestRouter) = _getAmountOutMax(
                routers[token],
                amountWETH,
                WETH,
                token
            ...
            if (amountToken != amountOutMax) {
                console.log("1");
                console.log(amountToken);
                console.log(amountOutMax);
                revert Errors.Index_WrongSwapAmount();
            }
        } else {
    ...
    function equityValuation() public view virtual returns (uint256);
    ...
    function _getAmountIndexFromWETH(uint256 amountWETHTotal)
        internal
        view
        returns (uint256 amountIndex, uint256 amountWETHSpent)
    {
        (
            ...
            ,
            uint256[] memory amountComponentUnits
        ) = _getMintingDataForExactIndex(Constants.PRECISION);

        amountIndex = type(uint256).max;

        for (uint256 i = 0; i < components.length; i++) {
            uint256 amountWETH = (amountWETHTotal * amountWETHUnits[i]) /
                amountWETHTotalUnit;
            ...
            amountIndex = MathUpgradeable.min(
                amountIndex,
                (amountComponent * Constants.DECIMALS) / amountComponentUnits[i]
            );
        ...
        returns (uint256 amountWETH)
    {
        for (uint256 i = 0; i < components.length; i++) {
            ...
            (uint256 amountOutMax, ) = _getAmountOutMax(
                routers[components[i]],
                amountComponent,
                components[i],
                WETH
...
contract IndexAvalancheDeFi is UUPSUpgradeable, IndexStrategyUpgradeable {
    IChainlinkAggregatorV3 public wethPriceFeed;
    ...
    function initialize(
        IndexStrategyInitParams calldata initParams,
        IChainlinkAggregatorV3 _wethPriceFeed
    ...
    function equityValuation() public view override returns (uint256) {
        ...
        if (priceWETH < 0) {
            revert Errors.Index_NegativePrice();
        }

        return (amountWETH * uint256(priceWETH)) / Constants.DECIMALS;
    }
...
interface IIndexStrategy {
    ...
    function equityValuation() external view returns (uint256);
