...
contract Foggy is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 FOGGY-WAVAX LP -> FOGGY pool
    // uint256 public constant INITIAL_FOGGY_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the FOGGY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Foggy Token", "FOGGY") {
        // Mints 1 FOGGY to contract creator for initial pool setup
    ...
    function _getFoggyPrice() internal view returns (uint256 _foggyPrice) {
        try IOracle(foggyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Foggy: failed to fetch FOGGY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _foggyPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_foggyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFoggyOracle(address _foggyOracle) public onlyOperatorOrTaxOffice {
        require(_foggyOracle != address(0), "oracle address cannot be 0 address");
        foggyOracle = _foggyOracle;
    }
    ...
    /**
     * @notice Operator mints FOGGY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FOGGY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFoggyPrice = _getFoggyPrice();
            currentTaxRate = _updateTaxRate(currentFoggyPrice);
            if (currentFoggyPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_foggyPool != address(0), "!_foggyPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_foggyPool, INITIAL_FOGGY_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForFoggySellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of Foggy
