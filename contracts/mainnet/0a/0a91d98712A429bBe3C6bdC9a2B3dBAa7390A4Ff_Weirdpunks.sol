...
contract Weirdpunks is ERC721  {
    ...
    constructor()
        ERC721("Weird Punks", "WPNKS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 152) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiewss7eo5zaxhvqqy3qbmdenrptxknthb72r6hnxxtt6kvoblnbcq/", Strings.toString(tokenId), ".json")
            );
    }
