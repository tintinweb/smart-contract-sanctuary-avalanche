...
// F*cking Avax Collection of BoredChoured here my bro'
...
// File: contracts/BoredChoured.sol
...
contract BoredChoured is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, Randomizer {
    ...
    // MODIFIERS
    modifier onlyDevs() {
        require(devFees[msg.sender].percent > 0, "Dev Only: caller is not the Boss");
        _;
    }
    ...
    constructor(
        ...
        uint[] memory _fees,
        address _lzEndpoint
    ) ERC721("BoredChoured", "BrdChrd") Randomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        require(supply + amount - 1 < maxSupply, "Error: f*cking SOLD OUT");
        require(amount > 0 && amount <= maxPerTx, "Error: Only 1 per Tx my lord ! ");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: Max 5 per Wallet my whale !");
        if (price > 0) require(msg.value == price * amount, "Error: You need more money Bro !");
        if (whitelistedOnly) require(whiteListed[msg.sender] >= amount, "Error: You're Not a Bored Whitelisted ");
