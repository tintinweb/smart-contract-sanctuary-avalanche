...
contract NeFiProtoStarDrops is Ownable{
	...
	function sendFeeTokenSpec(uint256 _intervals, uint256 _x) payable whiteOnly() external{
    	...
    	function payFeeAvaxSpec(uint256 _intervals,uint _x) payable  whiteOnly() external{
    		...
    		//calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(_intervals,msg.value,74000000000000000);
		//verification
	...
	function protoFeesAvaxSpec(uint256 _intervals,uint256 _x) payable whiteOnly() external{
		...
		require(_intervals >0, "Doesnt Look Like you opted to pay any fees at this time");
		 ...
		 //calculation
		(uint256 remainingIntervals,uint256 _nIntervals,uint256 needed,uint256 balanceRemainder) = NeFiLib.doAllMultiple(NeFiLib.getLower(getTotalPayable(_account),_intervals),msg.value,74000000000000000);
		//verification
		require(_nIntervals != 0 && needed > 0,"you do not hold enough to cover this claim");
		require(_nIntervals <= _intervals,"you do not hold enough to cover this claim");
	        //bookKeeping
