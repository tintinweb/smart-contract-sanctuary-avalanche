...
abstract contract overseer is Context {
   function getModFee(uint256 _value) external virtual returns(uint256);
...
contract NftMint is Ownable {
    ...
    function mint(uint256 _id) payable external {
    	...
    	uint256 _price = over.getModFee(nft_cost[num]);
    ...
    function MGRmint(uint256[] memory _ids,address[] memory _accounts,bool[] memory _sends,bool[] memory _records,bool[] memory _fullOverrrides) external managerOnly {
