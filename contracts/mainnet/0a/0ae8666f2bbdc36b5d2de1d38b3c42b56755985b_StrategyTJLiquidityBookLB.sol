...
contract StrategyTJLiquidityBookLB is StratManager, FeeManager {
    ...
    /// @notice mapping of current liquidities per bin
        ...
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        */
        ...
        //add the liquidities to the liquidites mapping
        _updateLiquidities();
    ...
    function _giveAllowances() public {
    ...
    function _removeAllowances() public {
    ...
    function _updateLiquidities() internal {
        ...
        for (uint256 i; i < liquidities.length; ++i) {
            //update the mapping
            currentLiquidities[activeBinIds[i]] = liquidities[i];
        }
    }
    ...
    function collectRewards(
        ...
        console.log("COLLECT REWARDS LBStrategy.sol");
        ...
        for (uint256 i; i < liquidities.length; ++i) {
            ...
            if (liquidities[i] >= currentLiquidities[_id]) {
                _feesL = liquidities[i] - currentLiquidities[_id];
            } else {
                _feesL = 0;
            }
            ...
            console.log(_feesL, _feesX, _feesY);
            ...
            console.log(feeGrowthInside[i], "<- fee growth inside");
        }
        ...
        if (feesX > 0 || feesY > 0) {
            console.log("transfer fees to vault");
    ...
    /// @notice gets the current amounts and liquidities of strategies position.
    /// @return amountsX amount of tokenX
    /// @return amountsY amount of tokenX
    /// @return liquidities of positions
