...
contract NFTBalanceHelper is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function addM(address[] memory token) public onlyOwner {
        for (uint256 idx = 0; idx < token.length; idx++) {
            _tokens.add(token[idx]);
        }
    }
    ...
    function removeM(address[] memory token) public onlyOwner {
        for (uint256 idx = 0; idx < token.length; idx++) {
            _tokens.remove(token[idx]);
        }
    }
    ...
    function balanceOf(address wallet) public view returns (uint256) {
        ...
        for (uint256 idx = 0; idx < len; idx++) {
            balance += IERC721(_tokens.at(idx)).balanceOf(wallet);
        }
