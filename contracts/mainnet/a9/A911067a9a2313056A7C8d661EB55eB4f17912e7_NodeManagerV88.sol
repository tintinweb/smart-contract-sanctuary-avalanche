...
contract NodeManagerV88 is Initializable, KeeperCompatibleInterface {
    ...
    uint256 public giftUsdtFee;
    uint256 public giftPxtFee;
    uint256 public giftUPXTFee;
    uint256 public giftAvaxFee;
    ...
    function transferUnclaimed(
        ...
        uint32 claimFeeRate = dynamicClaimFeeEnabled
            ? (rateClaimFee(_account) * compTaxRate) / 100
    ...
    function rateClaimFee(address account) public view returns (uint32) {
        if (lastTimestampClaim[account] == 0) return 100;
    ...
    */
    ...
    function getUnpaidNodes(uint32 _start, uint32 _count)
        public
        view
    ...
    function buyGiftCard(
        ...
        if (
            ...
            IERC20(token).transferFrom(
                address(msg.sender),
                address(operationsPoolAddress),
                amount + ((amount * giftUsdtFee) / 10000)
            );
        } else if (
            token == address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7)
        ) {
            require(msg.value >= ((amount * giftAvaxFee) / 10000), "low value");
            ...
            IERC20(token).transferFrom(
                address(rewardsPoolAddress),
                address(operationsPoolAddress),
                amount + ((amount * giftUPXTFee) / 10000)
            );
        }
    ...
    function burnUnPaidNodes(uint32 _start, uint32 _count) public onlyOwner {
        uint32[] memory unpaidNodesList = getUnpaidNodes(_start, _count);
    ...
    function setGiftUsdtFee(uint256 val) public onlyOwner {
        giftUsdtFee = val;
    }
}
