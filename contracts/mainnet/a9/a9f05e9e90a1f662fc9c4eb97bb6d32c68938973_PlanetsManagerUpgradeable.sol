...
LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    uint256[3] public burnedFromBail;
    ...
    function mintPlanet(
        ...
        uint256 planetValue
    ) payable external nonReentrant whenNotPaused universeSet returns (uint256) {
        ...
        require(msg.value / 10 ** 18 == planetValue / mintPerAvax, "Dealers: must send correct price");
        burnedFromBail[0] += planetValue;
        burnedFromBail[1] += msg.value;
        burnedFromBail[2] += 1;

        return _createPlanetWithTokens(planetName, planetValue, true);
    }
    ...
    function createPlanetWithTokens(
        ...
        return _createPlanetWithTokens(planetName, planetValue, false);
    }
    ...
    )
    {
        ...
        if (planet.rewardMult != tierLevel[0]) {
            ...
            if (planet.planetValue < 0) {
                ...
                logTier(tierLevel[0], -int256(reward));
            }
        }

        if (planet.planetValue < 0) {
            reward = oldValue;
            planet.planetValue = 0;
        }

        (uint256 amountToReward, uint256 takeAsFee) = getProcessingFee(reward, 0);
        (, uint256 burnFromFee) = getProcessingFee(reward, 0);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 reward = calculateReward(planet);
        if (reward > 0) {
            (uint256 amountToCompound, uint256 feeAmount) = getProcessingFee(reward, 0);
            ...
            // Max is 300%
            if (planet.rewardMult > 300000) {
                planet.rewardMult = 300000;
            }
    ...
    returns (PlanetInfoEntity[] memory)
    {
        ...
        for (uint256 i = 0; i < _planetIds.length; i++) {
            ...
            (uint256 amountToReward, ) = getProcessingFee(pendingRewardsGross, 0);
            (uint256 amountToRewardDaily, ) = getProcessingFee(rewardsPerDayGross, 0);
    ...
    whenNotPaused
    {
        ...
        PlanetEntity memory planet = _planets[tokenId];
        uint256 initialPlanetValue = planet.planetValue;
        planet.planetValue = initialPlanetValue * 9 / 10;
        logTier(planet.rewardMult, -int256(initialPlanetValue * 1 / 10));
    }
    ...
    function migrateToV2(uint256 value) external nonReentrant universeSet {
        IUniverse oldUniverse = IUniverse(0x68d19e3Ff9319470575b9CA78Fa39538a3C1245d);

        if (msg.sender == address(0xd261f23aD71bd9b15CfE010d6f37d647F70Da869)) {
            oldUniverse.accountReward(address(0xd261f23aD71bd9b15CfE010d6f37d647F70Da869), value);
        } else {
            require(value <= oldUniverse.balanceOf(msg.sender), "Dealers: not enough $PABLO V1 to migrate");
            uint256 newValue = value * 11 / 10;
            require(newValue <= universe.balanceOf(address(this)), "Dealers: not enough $PABLO V2 to migrate");
            oldUniverse.transferFrom(msg.sender, address(this), value);
            universe.transfer(msg.sender, newValue);
        }
    }
}