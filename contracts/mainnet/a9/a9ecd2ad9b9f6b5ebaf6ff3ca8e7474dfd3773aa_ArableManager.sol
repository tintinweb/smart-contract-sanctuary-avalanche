...
contract ArableManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function setSynthsDisabled(address[] calldata synthAddresses, bool[] calldata disables) external onlyOwner {
        require(synthAddresses.length == disables.length, "Invalid");
        for (uint256 index = 0; index < synthAddresses.length; index++) {
            isSynthDisabled[synthAddresses[index]] = disables[index];
        }
    }
