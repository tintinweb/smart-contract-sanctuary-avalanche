...
interface ISamuraiGG {
    ...
    function getTypeNFT(
        ...
        external
        view
        returns (address paymentToken, uint256 price, uint256 limit, uint256 unboxQuantity);
}
...
abstract contract Lockable {
    ...
    function _notLocked(address sender_, address from_, address to_) internal view virtual {
        if (
            _isLocked.get(sender_.toUint256()) ||
            _isLocked.get(from_.toUint256()) ||
            _isLocked.get(to_.toUint256())
...
import { Lockable } from "./internal-upgradeable/Lockable.sol";
import { PrimarySale } from "./internal-upgradeable/PrimarySale.sol";
import { NonFungibleType } from "./internal-upgradeable/NonFungibleType.sol";
import { ChainLinkPriceOracle } from "./internal-upgradeable/ChainLinkPriceOracle.sol";
import { Helper } from "./libraries/Helper.sol";
    ...
    ERC721WithPermitUpgradable
{
    ...
    modifier onlyUpgrader() {
        if (_msgSender() != upgrader) revert NFT__Unauthorized();
        _;
    }
    ...
    function initialize(string calldata name_, string calldata symbol_) external initializer {
        ...
        _setType(1, address(0), 1 ether, 100, 1);
        _setType(2, address(0), 0.5 ether, 100, 2);
        _setType(3, address(0), 0.1 ether, 100, 3);
        _setType(3, address(0), 0.5 ether, 15, 3);
        _setPriceFeeds(address(0), 0x0A77230d17318075983913bC2145DB16C7366156); // Avax
        _idCounter = 1;
    }
    ...
    function setUpgrader(address account_) external onlyOwner {
        upgrader = account_;
    }
    ...
    function buy(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
...

        {
            ...
            if (typeInfo.paymentToken == address(0)) {
                refund = value - paymentAmount; // will throw underflow error if value < paymentAmount -> skip refund
                if (refund > 0.1 ether) {
                    CurrencyTransferLib.safeTransferNativeToken(sender, refund);
                    CurrencyTransferLib.safeTransferNativeToken(_recipient, paymentAmount);
                } else {
    ...
    function ownerMint(
        ...
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, totalMint, _sold[typeNFT_], address(0), 0);
        _batchMint(recipient_, typeNFT_, tokenId, totalMint);
    }
    ...
    function metadata(
        uint256 tokenId_
    ) external view override returns (address owner_, uint256 typeId_) {
