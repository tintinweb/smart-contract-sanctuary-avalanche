...
// File contracts/SubWavaxGlacier.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract SubWavaxGlacier  is Ownable, Pausable {
    ...
    address public constant sub = address(0xA0105D7fc6190598523f568001A71164341b0A22);
    address public want;
    ...
    IGlacierRouter.Routes[] public glcrToSubPath;
    ...
    constructor(
        ...
        IGlacierRouter.Routes[] memory _glcrToSubPath,
        ...
        for (uint i; i < _glcrToSubPath.length; ++i) {
            glcrToSubPath.push(_glcrToSubPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IGlacierRouter(router).swapExactTokensForTokens(glcrHalf, 0, glcrToSubPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(sub).balanceOf(address(this));

        IGlacierRouter(router).addLiquidity(wavax, sub, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(sub).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(sub).safeApprove(router, type(uint).max);
    }
