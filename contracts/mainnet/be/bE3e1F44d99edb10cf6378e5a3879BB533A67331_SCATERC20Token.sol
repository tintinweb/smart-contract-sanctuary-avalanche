...
contract SCATERC20Token is ERC20Permit, VaultOwned {
    ...
    event UpdateAntiBotCoolDownDuration( uint256 newDuration );
    ...
    /**
     * @dev Update the antiBotTriggererMap
     * Can only be called by the current operator.
     */
    ...
    /**
     * @dev Update the anti-bot cool-down duration
     * Can only be called by the current operator.
     */
    function updateAntiBotCoolDownDuration( uint256 newDuration ) external onlyOperator {
      require( newDuration < 3600, "anti-bot cooldown too long" );
      antiBotCoolDown = newDuration;

      emit UpdateAntiBotCoolDownDuration( antiBotCoolDown );
    }
