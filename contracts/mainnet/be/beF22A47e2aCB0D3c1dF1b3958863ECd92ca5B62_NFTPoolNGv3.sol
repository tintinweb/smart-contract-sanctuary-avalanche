...
contract NFTPoolNGv3 is Controllable, IERC721Receiver, MyPausable {
    ...
    constructor(
        ...
        oldPool = NFTPoolNGv3(_oldPool);
        oldPool2 = NFTPoolNGv3(_oldPool2);
    ...
    function closeMigration() external onlyOwner {
        address(migrationContract).delegatecall(abi.encodeWithSignature("closeMigration()"));
    }
    ...
    function _addPreyToBarn(
        ...
        IPreyPredator.PreyPredator memory traits = _getTraits(tokenId);
        require(traits.generation < 1 || traits.generation > 22, "Cant stake this generation");
        if (isForGshare) {
    ...
    function _addPredatorToPack(
        ...
        IPreyPredator.PreyPredator memory traits = _getTraits(tokenId);
        require(traits.generation < 1 || traits.generation > 22, "Cant stake this generation");
        uint256 alpha = _alphaForPredator(traits);
        if (isForGshare) {
    ...
    function _switchPool(uint256 tokenId) internal {
        _ensureGen0(_getTraits(tokenId));
    ...
    function _switchPreyInBarn(address account, uint256 tokenId) internal {
        ...
        require(barn[barnIndices[tokenId]].owner == account, "Not your NFT");
        if (isForGshare) {
            totalPreyStakedForGshare -= 1;
            userPool[account].gladshare.preyStaked -= 1;
            totalPreyStakedForGlad += 1;
    ...
    function _switchPredatorInPack(address account, uint256 tokenId) internal {
        bool isForGshare = stakedForGshare[tokenId];
        uint256 alpha = _alphaForPredator(_getTraits(tokenId));
        require(pack[alpha][packIndices[tokenId]].owner == account, "Not your NFT");
        if (isForGshare) {
            totalAlphaStakedForGshare -= alpha; // Portion of earnings mapped
            totalPredatorsStakedForGshare -= 1;
            userPool[account].gladshare.predatorsStaked -= 1;
    ...
    function _ensureGen0(IPreyPredator.PreyPredator memory t) internal pure {
        require(t.generation == 0, "Only gen0 NFTs are for GLAD");
    }
    function _getTraits(uint256 tokenId) internal view returns (IPreyPredator.PreyPredator memory t) {
        return preyPredator.getTokenTraits(tokenId);
    }
    /** CLAIMING / UNSTAKING */
        ...
        updateEarnings(msg.sender)
    {
        ...
        uint256 currentEpoch = treasury.epoch();
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // address tmp = preyPredator.ownerOf(tokenIds[i]);
            //require(
            //    tmp == address(this) || tmp == address(oldPool2) || tmp == address(oldPool),
    ...
    function sendBackToken(uint256 tokenId, address to) internal {
        if (address(this) == preyPredator.ownerOf(tokenId)) {
            preyPredator.safeTransferFrom(address(this), to, tokenId, "");
        } else if (address(oldPool2) == preyPredator.ownerOf(tokenId)) {
            preyPredator.safeTransferFrom(address(oldPool2), to, tokenId, "");
        } else if (address(oldPool) == preyPredator.ownerOf(tokenId)) {
            preyPredator.safeTransferFrom(address(oldPool), to, tokenId, "");
        } else {
            revert("NFT not in this pool");
        }
    }
    // ** ----------- * //
    ...
    function _rescue(uint256[] calldata tokenIds, bool sendToOwner) internal {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (isPrey(tokenId)) {
                ...
                sendBackToken(tokenId, sendToOwner ? owner() : stake.owner);

                emit TokenClaimed(tokenId, true);
            } else {
                alpha = _alphaForPredator(_getTraits(tokenId));
    ...
    function isPrey(uint256 tokenId) public view returns (bool prey) {
        IPreyPredator.PreyPredator memory iPreyPredator = _getTraits(tokenId);
     ...
     */
    function _alphaForPredator(IPreyPredator.PreyPredator memory t)
        internal
        view
        returns (uint256)
    {
