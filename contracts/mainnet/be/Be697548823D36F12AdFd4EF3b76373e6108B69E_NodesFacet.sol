...
import "../libraries/LNodeController.sol";
import "../interfaces/INodesFacet.sol";

/// @title NodesFacet
/// @author mejiasd3v, mektigboy
/// @notice Facet in charge of displaying information about the nodes
/// @dev Utilizes 'AppStorage', 'LERC721' and 'LNodeController'
contract NodesFacet is INodesFacet {
    ///////////////////
    /// APP STORAGE ///
    ///////////////////
    ...
    function getAllNodes(address account) public view returns (Node[] memory) {
        uint256 accountBalance = LERC721.balanceOf(s, account);
        if (accountBalance == 0) revert NodesFacet_NotEnoughBalance();

        Node[] memory nodes = new Node[](accountBalance);

        for (uint256 i = 0; i < accountBalance; ++i) {
            uint256 tokenId = s.ownedTokens[account][i];

            Node memory node = s.nodeByTokenId[tokenId];
    ...
    /// @notice Get node rewards
    /// @param _id Token ID
    function getNodeRewards(uint256 _id) external view returns (uint256) {
        if (!LERC721.exists(s, _id)) revert LERC721__InvalidToken();

        Node memory node = s.nodeByTokenId[_id];

        uint256 rewards = LRewards.pendingRewards(s, node);
    ...
    function getNodesRewards(uint256[] memory _ids) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            if (!LERC721.exists(s, _ids[i])) revert LERC721__InvalidToken();

            Node memory node = s.nodeByTokenId[_ids[i]];

            rewards += LRewards.pendingRewards(s, node);
        }

        return rewards;
    }

    /// @notice Get all the rewards of a node
    /// @param _account User account
    function getAllNodesRewards(address _account) external view returns (uint256) {
        Node[] memory nodes = getAllNodes(_account);
    ...
    /// @notice Get all the amount of a node
    /// @param _account User account
    function getAllNodesAmount(address _account) public view returns (uint256) {
        Node[] memory nodes = getAllNodes(_account);

        if (nodes.length == 0) revert NodesFacet_NoNodesFound();

        uint256 personalTVL = 0;

        for (uint256 i = 0; i < nodes.length; ++i) {
            Node memory node = nodes[i];

            personalTVL += node.amount;
        }
    ...
    function getNodesCount(address account) external view returns (uint256) {
        return LERC721.balanceOf(s, account);
    }
...
library LFees {
    ...
    function enforceCompoundFee(AppStorage storage s, uint256 _amount) internal returns (uint256) {
        uint256 compoundFee = (_amount * 15) / 100;
