...
//and vice versa with votr contract
...
interface IeggBurnPartner { //Hen Solo contract to burn Chikn $egg
  function burnEgg(string calldata _name, uint256 _amount) external; //burn egg through egg burn partner contract
  function subscribe(string calldata _name, address _callbackContractAddr) external; //subscibe contestant burnID
}
...
contract BeautyPageantVoting is Ownable, ReentrancyGuard 
{
...
/* commented out these test functions
  ...
  function testEggBurnWei(string memory _CandidateID, uint _testBurnTotal) public onlyOwner { //can only burn egg from the smart contract wallet or let dapp do it, can't burn from a user wallet using a contract directly since egg burn contract looks at msg.sender
    ...
    IeggBurnPartner(EggBurnPartner).burnEgg(_CandidateID,_testBurnTotal);//then we burn from our own contracts wallet since it checks msg.sender
  }
*/
  ...
  function subscribeBurnIDNOOP (uint _numberedID, string memory _contestantNewIDString) public onlyOwner {//set a number of 1-32 for candidate
    ...
    IeggBurnPartner(EggBurnPartner).subscribe(_contestantNewIDString, noopAddress);//subscribe the name ID to egg burn partner contract with noop address of 0x00..001
  }
...
////main vote code:
  ...
  public timeNotStarted timeIsOver nonReentrant returns(bool) {
    ...
    IeggBurnPartner(EggBurnPartner).burnEgg(_candidateIDName, EggVotesToBurnNowWei);//then we burn from our own contracts wallet
...
/*
  function testMintVotePostrMsgSender(uint _ourNumberedContestantID) public onlyOwner{ //nft test mint
    IvotePostrs(votePostrsAddress).voterMint(msg.sender, _ourNumberedContestantID);
  }
*/
