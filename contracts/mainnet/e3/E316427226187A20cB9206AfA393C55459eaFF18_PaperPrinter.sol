...
contract Paper is ERC20, ERC20Burnable, Ownable {
    ...
    address public constant router = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4; // JOE Router
    ...
    constructor (address _to, uint256 _initialSupply, uint256 _tokenCap) ERC20("Paper", "PAPER") {
        ...
        _excludedFromAntiWhale[router] = true;
...
contract PaperPrinter is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. PAPERs to distribute per second.
        uint256 lastRewardTime;  // Last timestamp that PAPERs distribution occurs.
        uint256 accPaperPerShare;   // Accumulated PAPERs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;       // Total amount deposited into the pool.
        bool isNative;            // Determines if the pool is a native token or non-native one.
    }
    ...
    // PAPER tokens created per timestamp.
    uint256 public paperPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when PAPER distribution starts.
    uint256 public startTime;
    // Maximum PAPER Per Second
    ...
    event UpdateEmissionRate(address indexed user, uint256 paperPerSecond);
    ...
    event paperPrintError(bytes reason);
    ...
    constructor(
        ...
        require(_devaddr != address(0), "Dev address cannot be the zero address.");
        require(_feeAddress != address(0), "Fee address cannot be the zero address.");
        require(_paperPerSecond < MAX_EMISSION_RATE, "Emission rate cannot be more than the maximum.");
    ...
    function initializePools(address _lpAddress) external onlyOwner {
        require(poolInfo.length == 0, "Pools are already existing");

        add(200, IERC20(_lpAddress), 0, true, false);   // PAPER LP
        add(175, IERC20(paper), 0, true, false);        // PAPER
        add(15, IERC20(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7), 190, false, false);           // wAVAX pool
        add(15, IERC20(0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB), 190, false, false);           // wETH.e pool
        add(15, IERC20(0x50b7545627a5162F82A992c33b87aDc75187B218), 190, false, false);           // wBTC.e pool
        add(15, IERC20(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664), 190, false, false);           // USDC.e pool
        add(15, IERC20(0xc7198437980c041c805A1EDcbA50c1Ce5db95118), 190, false, false);           // USDT.e pool
        add(15, IERC20(0xd586E7F844cEa2F87f50152665BCbc2C279D8d70), 190, false, false);           // DAI.e pool
        add(5, IERC20(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd), 190, false, false);            // JOE pool
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _isNative, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        // valid ERC20 token
    ...
    // Update the given pool's PAPER allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending PAPERs on frontend.
    function pendingPaper(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 paperReward = multiplier * paperPerSecond * pool.allocPoint / totalAllocPoint;
            uint256 devReward = paperReward / 10;
            uint256 totalRewards = paper.totalSupply() + devReward + paperReward;

            if (totalRewards > paper.tokenCap()) {
                paperReward = paper.tokenCap() - paper.totalSupply();
            }     
            
            accPaperPerShare = accPaperPerShare + paperReward * 1e18 / pool.lpSupply;
        }
        return user.amount * accPaperPerShare / 1e18 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= paper.tokenCap()) {
            // mint as normal as not at max supply
            paper.mint(devaddr, devReward);
            paper.mint(address(this), paperReward);
        } else {
            // mint the difference only to MC, update paperReward
            paperReward = paper.tokenCap() - paper.totalSupply();
            paper.mint(address(this), paperReward);
        }

        if (paperReward != 0) {
            // only calculate and update if paperReward is non 0
            pool.accPaperPerShare = pool.accPaperPerShare + paperReward * 1e18 / pool.lpSupply;
        }

        pool.lastRewardTime = block.timestamp;
    }
    
    // Deposit LP tokens to MasterChef for PAPER allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accPaperPerShare / 1e18 - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount * pool.accPaperPerShare / 1e18;
    ...
    // Safe Treat transfer function, just in case if rounding error causes pool to not have enough PAPERs.
    function safePaperTransfer(address _to, uint256 _amount) internal {
        uint256 paperBal = paper.balanceOf(address(this));
        bool transferSuccess = false;
        
        if (_amount > paperBal) {
            transferSuccess = paper.transfer(_to, paperBal);
        } else {
            transferSuccess = paper.transfer(_to, _amount);
        }
        
        require(transferSuccess, "safePaperTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _paperPerSecond) external onlyOwner {
        require(_paperPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: INCORRECT INPUT");
        massUpdatePools();
        
        paperPerSecond = _paperPerSecond;
       
        emit UpdateEmissionRate(msg.sender, _paperPerSecond);
    }

    // Only update before start of farm
    function updateStartTime(uint256 _newStartTime) external onlyOwner {
        require(block.timestamp < startTime, "updateStartTime: FARM ALREADY STARTED");
        require(block.timestamp < _newStartTime, "updateStartTime: INCORRECT INPUT");
