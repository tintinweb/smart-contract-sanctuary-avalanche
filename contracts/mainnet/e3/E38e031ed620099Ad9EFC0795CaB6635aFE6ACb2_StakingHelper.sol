...
contract StakingHelper {
    ...
    IERC20 public immutable BigToken;

    constructor (address _staking, address _Big) {
        require(_staking != address(0));
        require(_Big != address(0));
        staking = IStaking(_staking);
        BigToken = IERC20(_Big);
    }

    function stake(uint256 _amount, address recipient) external {
        BigToken.transferFrom(msg.sender, address(this), _amount);
        BigToken.approve(address(staking), _amount);
