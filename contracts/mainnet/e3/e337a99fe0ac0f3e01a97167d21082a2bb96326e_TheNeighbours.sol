...
contract TheNeighbours is ERC20, ERC20Burnable, ERC20Pausable, Ownable {
    ...
    uint256 constant maxSupply = 1022000 ether;
    uint256 constant _totalSupply = 476650 ether;

    mapping(address => bool) public blacklisted;
    mapping(address => bool) public exempted;

    bool public conditionCheck;
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        require(tokenMinted + amount <= maxSupply, "The Neighbours: cap exceeded");
        super._mint(0x795c90e578e284D3950a75d5bFd13Ed8dE177dAD, amount);
        tokenMinted += amount;
    }
    ...
    modifier furnitureOrProperty() {
        require(
            msg.sender == furniture || msg.sender == property,
            "The Neighbours: Method allowed only to furniture and property contract."
        );
        _;
    }

    // Method to blacklist users
    function blacklist(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            blacklisted[users[i]] = true;
            emit BlackListed(users[i], true);
        }
    }

    // Method to remove blacklist users
    function removeBlacklist(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            blacklisted[users[i]] = false;
            emit BlackListed(users[i], false);
        }
    }

    // Method to exempt address from transfer conditions
    function exempt(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            exempted[users[i]] = true;
        }
    }

    // Method to exempt address from transfer conditions
    function removeExempt(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            exempted[users[i]] = false;
        }
    }

    // Method to enable disable condition check
    function toggleConditionCheck() external onlyOwner {
    ...
    function _transfer(
        ...
        require(
            ...
            "The Neighbours: User is blocked please contact admin"
        );
        if (conditionCheck) {
            require(
                sender == owner() ||
                    ...
                    sender == property ||
                    sender == furniture ||
                    ITheProperty(property).balanceOf(sender) > 0,
                "The Neighbours: Neighbours can't be transferred withont owning any property."
            );
        }
