...
abstract contract Migratable is ERC20, ERC20Burnable, Supervised {
    ...
    function migrate(uint256 oldAmount) public virtual {
        uint256 newAmount = _premigrate(oldAmount);
        _mint(msg.sender, newAmount);
    }

    /** preimport amount from old contract w/o minting new tokens */
    function _premigrate(uint256 oldAmount) internal returns (uint256) {
...
abstract contract SovMigratable is Migratable, SovMigratableSupervised {
    ...
    /** import amount from old contract */
    function migrate(uint256 oldAmount) public override {
        uint256 newAmount = _premigrate(oldAmount);
        _moe.transferFrom(msg.sender, (address)(this), oldAmount);
        _mint(msg.sender, newAmount);
    }
