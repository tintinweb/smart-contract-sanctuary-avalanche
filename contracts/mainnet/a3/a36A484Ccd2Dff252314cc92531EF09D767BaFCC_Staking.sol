...
interface IAEPNFT is IERC721Upgradeable {
    enum TIER {
        HUMAN,
        ZOMBIE,
        VAMPIRE
...
/// @title TheEgoVerse Custodial Staking
/// @author Ace (@vikings_ace)
/// @notice This contract supports custodial NFT staking with dynamic reward rate updates
contract Staking is
    ...
    UUPSUpgradeable
{
    ...
    function getRewards(uint _id) public view returns (uint) {
        ...
        (, IAEPNFT.TIER tokenTier) = getTokenTierIndex(_id);
        ...
        if (collected == 0) {
            collected +=
                ...
                rate[currentTime][uint(tokenTier)];
        } else {
            collected +=
    ...
    function getTokenTierIndex(
        uint256 _id
    ) public view returns (uint256 tokenIndex, IAEPNFT.TIER tokenTier) {
    ...
    function updateRewards(uint[3] calldata _newRate) external onlyOwner {
        rate.push(_newRate);
        time.push(block.timestamp);
    }
