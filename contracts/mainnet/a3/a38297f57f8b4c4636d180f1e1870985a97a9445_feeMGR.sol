...
contract feeMGR is Ownable{
    ...
    function getAccounts(uint _x, uint _y) internal {
    ...
    function getProtoNumbers(uint _x, uint _y) internal {
    	address _account;
    ...
    function doAccountData(uint _x, uint _y) internal{
    	address _account;
    	for(uint i = _x;i<_y;i++){
    		_account = Accounts[i];
    		getAccountData(_account);
    	}
    }	
     function addProtos(uint _x, uint _y) internal {
    	address _account;
    	for(uint i = _x;i<_y;i++){
    		TOTALFEES storage _fees = totalFees[_account];
    		for(uint j = 0;i<_fees.protos;j++){
    			(string memory _name,uint256 creationTime,uint256 lastClaimTime,uint256 protoElapsed,uint256 rewards,uint256 boost,uint256 protoLife,uint256 lifeDecrease,uint256 collapseDate) = protoMgr.protoAccountData(_account,j);		
    ...
    function getAccountData(address _account) internal {
    	address _account;
    	for(uint i = 0;i<Accounts.length;i++){
    		_account = Accounts[i];
    		PROTOOWNERS[] storage stars = protoOwners[_account];
	    	for(uint j = 0;j<stars.length;i++){
	    		PROTOOWNERS storage star = stars[j];
