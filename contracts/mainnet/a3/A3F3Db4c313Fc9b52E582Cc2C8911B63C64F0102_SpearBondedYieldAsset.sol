...
library SpearBYAConstants {
    uint256 constant mintLiquidityPerc           =  48;
    uint256 constant mintSpearTreasuryPerc       =  50;
    uint256 constant mintAllianceRefererPerc     =   1;
    uint256 constant mintAllianceNewAccountPerc  =   1;
    uint256 constant mintTotalPerc               = 100;

    uint256 constant mCheckSum               = mintLiquidityPerc+mintSpearTreasuryPerc+mintAllianceRefererPerc+mintAllianceNewAccountPerc;
    uint256 constant mCheckSumShouldBe			 = mintTotalPerc;

    uint256 constant spearRewardsPerPeriod   = 134_154_688;
    uint256 constant spearRewardsPeriodSec   = 24 hours;
    
    uint8 constant sniperProtectionBlocks = 0;
}

interface IStartedOn {
    function startedOn() external returns(uint256);
}

contract SpearBondedYieldAsset is ERC20Permit, AntiSniper, Initializable, Ownable, MintableBurnableToken {
    ...
    WhitelistTokenWallet spearTreasuryWallet;
    WhitelistTokenWallet spearRewardWallet;
    uint256 private _spearRewardPerSec;
    ...
    function initialize(string memory name_, string memory symbol_,
                        ...
                        WhitelistTokenWallet spearRewardWallet_,
                        uint256 rewardStartDelay
                        ) public initializer {
        require(address(spearToken_) != address(0),
                "Invalid address supplied");

        assert(SpearBYAConstants.mCheckSum == SpearBYAConstants.mCheckSumShouldBe);
        ...
        spearRewardWallet = spearRewardWallet_;

        _spearRewardPerSec = (SpearBYAConstants.spearRewardsPerPeriod*(10**spearToken.decimals()))/SpearBYAConstants.spearRewardsPeriodSec;

        if (rewardStartDelay < type(uint256).max)
            rewardStartDelay = rewardStartDelay + IStartedOn(address(spearToken)).startedOn();
        rew.initialize(0,StartTime.wrap(rewardStartDelay),DripRate.wrap(_spearRewardPerSec),DripAmount.wrap(type(uint256).max),1 seconds); //can't set this properly because of launch timing...
        
        targetSupply = (spearToken.totalSupply()*105_000)/100_000; //REVIEW
        
        setSniperBlockEnd(block.number+SpearBYAConstants.sniperProtectionBlocks);
        setSniperProtection(false);
        _enableMinting(true);
    }

    function spearRewardPerSec() public view onlyOwner returns(uint256) {
    ...
    function mint(address to, uint256 amount, address referer) public returns (bool) {
        require(amount > 0 && _totalSupply + (amount) <= targetSupply);
        
        address sender = _msgSender();
        require(referer != sender && referer != to && to == sender, "Cannot be your own referer and BYA must go to minter");

        uint256 temp = amount * (SpearBYAConstants.mintLiquidityPerc) / (SpearBYAConstants.mintTotalPerc);
        spearToken.transferFrom(sender, address(spearToken), temp);
            
        if (referer == address(0)){
            temp = amount - (temp);
            spearToken.transferFrom(sender, address(spearTreasuryWallet), temp);
            
        } else {
            uint256 liq = temp;
            //Transfer less because there is a referer
            temp = amount * (SpearBYAConstants.mintAllianceRefererPerc) / (SpearBYAConstants.mintTotalPerc);
            spearToken.transferFrom(sender, referer, temp);
            

            if(SpearBYAConstants.mintAllianceNewAccountPerc != SpearBYAConstants.mintAllianceRefererPerc)
                temp = temp + (amount * (SpearBYAConstants.mintAllianceNewAccountPerc) / (SpearBYAConstants.mintTotalPerc));
            else
                temp = temp * (2);
            
            spearToken.transferFrom(sender, address(spearTreasuryWallet), amount - (liq) - (temp));
        }

        bool newAccount = balanceOf(to) == 0; //REVIEW this gets more complicated once BYAs can move around
        ...
        //do this after setBasis so the new account is not checkpointed by setBasis
        if (newAccount)
            _holders.push(to);
    ...
    function rewardBalance(address a) public view returns(uint256) {
        return rew.balanceOf(a,balanceOf(a),Remainder.wrap(spearToken.balanceOf(address(spearRewardWallet))));
    }

    function claimReward(address to) public returns(uint256) {
        address sender = _msgSender();
        if (to == address(0))
            to = sender;
        uint256 amount = rew.claim(sender,balanceOf(sender),Remainder.wrap(spearToken.balanceOf(address(spearRewardWallet))));
        if (amount > 0) {
            //This will always succeed because of Remainder param above
            spearRewardWallet.transferTokenTo(spearToken,to,amount);
        }
        emit DripRewarder.RewardClaimed(sender,to,amount);
        return amount;
    }
    
    function transferFrom(address from, address to, uint256) public override(StandardToken) noSniper(from,to) returns (bool ret) {
        //require(from == address(this) || from == owner());
    ...
    function transfer(address to, uint256) public override(StandardToken) noSniper(_msgSender(),to) returns (bool ret) {
        //require(msg.sender == owner() || _msgSender() == address(this));
