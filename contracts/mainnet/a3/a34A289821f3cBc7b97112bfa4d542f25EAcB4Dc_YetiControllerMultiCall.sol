...
contract YetiControllerMultiCall is Ownable {
    ...
    function getYetiControllerData() external view 
        returns (address[] memory whitelistedCollaterals
                , address[] memory vaultTokens
                , address[] memory underlyingTokens
                , uint256[] memory underlyingDecimals
                , uint256[] memory underlyingPerReceptRatios
                , uint256[] memory receiptPerUnderlyingRatios
                , uint256[] memory prices
                , uint256[] memory underlyingPrices
                , uint256[] memory tvls) {
        
        whitelistedCollaterals = yetiController.getValidCollateral();

        uint totalLen = whitelistedCollaterals.length + additionalVaults.length;

        vaultTokens = new address[](totalLen);
        underlyingTokens = new address[](totalLen);
        underlyingDecimals = new uint256[](totalLen);
        underlyingPerReceptRatios = new uint256[](totalLen);
        receiptPerUnderlyingRatios = new uint256[](totalLen);
        prices = new uint256[](totalLen);
        underlyingPrices = new uint256[](totalLen);
        tvls = new uint256[](totalLen);
        ...
        for (uint i = 0; i < whitelistedCollaterals.length; i++) {
            ...
            uint256 price = yetiController.getPrice(whitelistedCollaterals[i]);

            prices[i] = price;

            if (yetiController.isWrapped(collateral)) {
                IVault vaultToken = IVault(collateral);

                vaultTokens[i] = collateral;
                underlyingTokens[i] = vaultToken.underlying();

                underlyingDecimals[i] = vaultToken.underlyingDecimal();
                underlyingPerReceptRatios[i] = vaultToken.underlyingPerReceipt();
                receiptPerUnderlyingRatios[i] = vaultToken.receiptPerUnderlying();

                underlyingPrices[i] = vaultToken.receiptPerUnderlying().mul(price).div(10 ** (36 - vaultToken.underlyingDecimal()));

                tvls[i] = vaultToken.totalSupply();
            } else {
                IERC20 token = IERC20(collateral);

                underlyingDecimals[i] = token.decimals();

                underlyingPerReceptRatios[i] = 10 ** 18;
                receiptPerUnderlyingRatios[i] = 10 ** 18;
                underlyingPrices[i] = price;

                tvls[i] = 0;
            }
        }
        for (uint i = 0; i < additionalVaults.length; i++) {
            uint index = whitelistedCollaterals.length + i;

            address vault = additionalVaults[i];

            uint256 price = additionalVaultPriceFeeds[i].fetchPrice_v();

            prices[index] = price;

            IVault vaultToken = IVault(vault);

            vaultTokens[index] = vault;
            underlyingTokens[index] = vaultToken.underlying();

            underlyingDecimals[index] = vaultToken.underlyingDecimal();
            underlyingPerReceptRatios[index] = vaultToken.underlyingPerReceipt();
            receiptPerUnderlyingRatios[index] = vaultToken.receiptPerUnderlying();

            underlyingPrices[index] = vaultToken.receiptPerUnderlying().mul(price).div(10 ** (36 - vaultToken.underlyingDecimal()));
            tvls[index] = vaultToken.totalSupply();
