...
// File: contracts/SwiftTokenV2.sol

pragma solidity 0.6.12;

contract SwiftTokenV2 is ERC20("SwiftTokenV2", "SwiftV2"), Ownable {
    uint256 constant MAX_CAP_SUPPLY = 10_000_000 ether;
    uint256 constant BASIS_POINT = 10_000;
    uint256 constant TRANSFER_TAX = 1000; //basis points out of 10000 ex 500 = 5%
    Whitelist public whitelist;

    modifier isWhitelistedContract() {
        require(
            checkWhitelist(msg.sender) == true,
            "Isn't a whitelisted address"
        );
        _;
    }
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            totalSupply() < MAX_CAP_SUPPLY.add(_amount),
            "Exceeds max supply"
        );
        _mint(_to, _amount);
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        require(amount > 0, "Transfer Amount can't be 0");

        if (checkWhitelist(msg.sender) == true) {
            _transfer(_msgSender(), recipient, amount);
            return true;
        }
        ...
        taxAmount = amount.div(BASIS_POINT).mul(TRANSFER_TAX);
    ...
    function burn(address from, uint256 amount) external returns (bool) {
        require(from == _msgSender(), "Caller need to be owner of tokens");
        _burn(from, amount);
