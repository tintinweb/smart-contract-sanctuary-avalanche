...
abstract contract feeManager is Context {
    ...
    function reciever(address _account,uint256 _amount,bool _isAvax) payable virtual  external;
    function recieverFeeTok(address _account,uint256 _amount,bool _isAvax) payable virtual external;
...
contract NebulaProtoStarDrop is Ownable{
	...
	address public _feeManager = 0x9851ACd275cD2174530afDD5bfD394D94Fe51a75;
	address oldDrop;
	address Guard;
	...
	function payFeeTokenSpec(uint256 _intervals,uint256 _x) payable external{
	    ...
	    feeMGR.recieverFeeTok(_account,total,false);
	...
	function createProtoFeeToken(string memory _name) payable external{
		...
		feeMGR.reciever(_account,tokFee,false);
		payable(_account).transfer(msg.value);
	}
	function payAvaxFeeSpec(uint256 _intervals,uint256 _x) payable external{
		...
		feeMGR.reciever(_account,sent,true);
	}
	function payAvaxFee(uint256 _intervals) payable external{
    	...
    	function claimProtoAvax(string memory _name) payable external{
    		...
    		uint256 sent = msg.value;
		address _account = msg.sender;
		theBigCheck(msg.sender,740000000000000000,msg.value,_name,false,1,101,true);
		feeMGR.addProto(_account,_name);
		airdrop[_account].claimed +=1;
		payable(_feeManager).transfer(sent);
		feeMGR.reciever(_account,sent,true);
