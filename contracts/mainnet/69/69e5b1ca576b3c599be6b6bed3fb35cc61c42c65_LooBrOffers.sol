...
contract ERC721BidStorage {
    ...
    // Check if ERC20 token is whitelisted
    mapping(address => bool) isERC20Whitelisted;
...
// File contracts/LooBrOffers.sol
...
contract LooBrOffers is Ownable, ERC721BidStorage {
     ...
     * @param _owner - owner
     */
    constructor(address _owner, uint256 _feeRate, address _revenueAddress) {
        // Set owner
    ...
    /**
     * @dev Modifier for checking if provided ERC20 contract is whitelisted, fails if not.
     * @param _erc20ContractAddress - address of the ERC20 token
     */
    modifier checkERC20Whitelist(address _erc20ContractAddress) {
        require(
            isERC20Whitelisted[_erc20ContractAddress],
            "LooBr: Contract address isn't whitelisted"
        );
        _;
    }

    /**
     * @dev Whitelist or Blacklist ERC20 contract.
     * @param _erc20ContractAddress - address of the ERC20 token
     * @param _toggler - bool set true to whitelist and false to blacklist ERC20 Token
     */
    function toggleContractWhitelist(
        address _erc20ContractAddress,
        bool _toggler
    ) public onlyOwner {
        require(
            _erc20ContractAddress != address(0),
            "LooBr: Invalid wallet address"
        );
        isERC20Whitelisted[_erc20ContractAddress] = _toggler;
    }
    ...
    ) public checkERC20Whitelist(_erc20Address) returns (bytes32) {
        return
            _placeBid(
                _tokenAddress,
                _tokenId,
    ...
    // function _acceptBid(address _tokenAddress, uint256 _tokenId, address bidder) public {
    //     require(_bidderHasABid(_tokenAddress, _tokenId, bidder), "Bid not found");
     ...
     * @param _bidId Additional data with no specified format
     */
    function acceptBid(
        address _tokenAddress,
        uint256 _tokenId,
        ...
        require(
            ...
            bid.id == _bidId && bid.expiresAt >= block.timestamp,
            "ERC721Bid#onERC721Received: INVALID_BID"
        );
        address bidder = bid.bidder;
        uint256 price = bid.price;
        address _erc20Address = bid.erc20Address;
        ...
        require(
            ERC20Interface(_erc20Address).transferFrom(
                bidder,
                msg.sender,
                priceAfterFee
            ),
        ...
        require(
            ...
            "ERC721Bid#onERC721Received:: TRANSFER_AMOUNT_TO_MARKETPLACE_FAILED"
        );
    ...
    //     uint256 fee = (price * feeRate) / FEE_RATE_BASE;
    //     uint256 revenue = fee;
    //     ERC20TOKENADDRESS.safeTransferFrom(mrToken, msg.sender, mrConfig.getRevenueAddress(), revenue);
    ...
    function _cancelBid(
        ...
        if (lastBidIndex != _bidIndex) {
            ...
            Bid storage lastBid = bidsByToken[_tokenAddress][_tokenId][
                lastBidIndex
            ];
    ...
    function getBidByToken(
        ...
        uint256 _tokenId,
        uint256 _index
    ) public view returns (bytes32, address, uint256, uint256) {
     ...
     */
    function _requireBidderBalance(
        address _bidder,
        uint256 _amount,
        address _erc20Address
    ) internal view {
        require(
            ERC20Interface(_erc20Address).balanceOf(_bidder) >= _amount,
            "ERC721Bid#_requireBidderBalance: INSUFFICIENT_FUNDS"
        );
        require(
            ERC20Interface(_erc20Address).allowance(_bidder, address(this)) >=
                _amount,
