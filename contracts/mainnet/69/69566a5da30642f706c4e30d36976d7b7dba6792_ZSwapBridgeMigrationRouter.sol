...
// File: contracts/zswap-periphery/libraries/SafeMath.sol
...
// File: contracts/zswap-periphery/libraries/ZSwapLibrary.sol
...
library ZSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'bc1fb831e0a2f7e7e40bd2551015adb58e0aaea592b56b4a889bb589903909e9' // init code hash
            ))));
    }
...
// File: contracts/zswap-periphery/libraries/Roles.sol
...
// File: contracts/zswap-lib/libraries/TransferHelper.sol
...
// File: contracts/zswap-periphery/interfaces/IBridgeToken.sol


pragma solidity >=0.5.0;


interface IBridgeToken is IZSwapERC20 {
    function swap(address token, uint256 amount) external;
    function swapSupply(address token) external view returns (uint256);
}
// File: contracts/zswap-periphery/ZSwapBridgeMigrationRouter.sol
...
contract ZSwapBridgeMigrationRouter {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'ZSwapBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'ZSwapBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "ZSwapBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "ZSwapBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "ZSwapBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "ZSwapBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IZSwapERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IZSwapPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = ZSwapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = ZSwapLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IZSwapPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IZSwapPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "ZSwapBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "ZSwapBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "ZSwapBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IZSwapPair(pairA).token0() == IZSwapPair(pairB).token0() ||
            IZSwapPair(pairA).token0() == IZSwapPair(pairB).token1() ||
            IZSwapPair(pairA).token1() == IZSwapPair(pairB).token0() ||
            IZSwapPair(pairA).token1() == IZSwapPair(pairB).token1(),
            "ZSwapBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "ZSwapBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "ZSwapBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IZSwapPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IZSwapPair(liquidityPairFrom).token0();
        if (
            IZSwapPair(liquidityPairFrom).token0() == IZSwapPair(liquidityPairTo).token0() ||
            IZSwapPair(liquidityPairFrom).token0() == IZSwapPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IZSwapPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "ZSwapBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IZSwapPair(liquidityPairTo).token0() ||
            newTokenAddress == IZSwapPair(liquidityPairTo).token1(),
            "ZSwapBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IZSwapPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IZSwapPair(liquidityPairFrom).token0() != IZSwapPair(liquidityPairTo).token0() &&
            IZSwapPair(liquidityPairFrom).token1() != IZSwapPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IZSwapPair(liquidityPairTo).token0(), IZSwapPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IZSwapPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IZSwapPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IZSwapPair(pairAddress).getReserves();
        uint totalSupply = IZSwapPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "ZSwapBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "ZSwapBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IZSwapPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = ZSwapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
