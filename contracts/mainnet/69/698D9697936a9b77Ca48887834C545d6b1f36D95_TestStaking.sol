...
contract TestStaking is Ownable, AccessControl {
    ...
    uint256 public slippage; // slippage. 500 = 5%
    ...
    bytes32 public constant AVG_UPDATER_ROLE = keccak256("AVG_UPDATER_ROLE");
        ...
        notZeroAddress(_pairAddress)
    {
        ...
        stakeLength = _stakeLengthInDays;
    ...
    function getRewardPerDay() external view returns (uint256) {
        return (rewardPerBlock * blocksPerDay);
    }
    ...
    function isCurrentPopSTokenAboveAvg5d() external view returns (bool) {
        (uint256 popsReserve0, uint256 stakeReserve1) = _getTokenRatio();
        uint256 ratio = popsReserve0 / stakeReserve1;
        return ratio > popsTokenRatioAvgLast5d;
    }
        ...
        )
    {
       UserInfo storage userInfo = userInfos[user];
        ...
        uint256 ratio = popsReserve / stakeReserve;
        uint256 fees = ratio > popsTokenRatioAvgLast5d
    ...
    function setPopsTokenRatioAvgLast5d(uint256 amount) external {
        if (msg.sender != owner() && !hasRole(AVG_UPDATER_ROLE, msg.sender)) revert Unauthorized();
    ...
    function setRewardPerDay(uint256 value)
        ...
        stakingNotStarted
    {
        rewardPerBlock = value / blocksPerDay;
    }

    function setSlippage(uint256 value) external onlyOwner {
        slippage = value;
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(stakeToken).transfer(treasury, unAssignedRewards);
    ...
    function _getPending(address _user) private view returns (uint256) {
        ...
        if (userAmount > 0 && block.number > userInfo.lastRewardBlock) {
            ...
            return (userAmount * rewardPerBlock * multiplier);
        }
        return 0;
    }
    ...
    function _swapInMarket(uint256 amount) internal {
        uint256 deadline = block.timestamp + 5 minutes;
        ...
        uint256 popsAmount = _getPopsForLP(amount) * (10000 - slippage) / 10000;
