...
contract NFTTrainer is MyPausable {
    PreyPredator public preyPredator;
    Glad public glad;
    address public dao;

    mapping(uint256 => bool) allowedGenerations;

    uint256[256] public cost;
    uint256[256] public costTo;

    uint256 public initIdx;

    event GenerationAllowed(uint256 generation, bool allowed);

    constructor(address _preyPredator, address _glad) {
        require( _preyPredator != address(0), "NFTTrainer: nft zero address");
        require( _glad != address(0), "NFTTrainer: glad zero address");

        glad = Glad(_glad);
        preyPredator = PreyPredator(_preyPredator);
        allowedGenerations[0] = true;
        allowedGenerations[23] = true;
        allowedGenerations[24] = true;
        cost[0] = 0;
        costTo[0] = 0;
        cost[1] = 1e17;
        costTo[1] = 1e17;
    ...
    function setInitialCosts(uint256 _count) public onlyOwner {
        uint256 i;
        for ( i = initIdx; i < 256 && _count > 0; i ++ ){
            cost[i] = cost[i-1] * 100 / 99;
            costTo[i] = costTo[i-1] + cost[i];
            _count--;
        }
        initIdx = i;
    }

    function setCostTo(uint256[] memory indexes, uint256[] memory values) public onlyOwner {
        uint256 len = values.length;
        require(len == indexes.length, "NFTTrainer: input wrong");
        for ( uint256 i = 0 ; i < len ; i ++) {
            require(indexes[i] != 0 || values[i] == 0, "NFTTrainer: starting value can only be 0");
            require(indexes[i] != 0 || values[i] == 0, "NFTTrainer: starting value can only be 0");
            costTo[indexes[i]] = values[i];
        }
        // recalculate costs
        for ( uint256 i = 1 ; i < 256 ; i ++) {
            require(costTo[i] > costTo[i-1], "NFTTrainer: costTo must be ascending");
            cost[i] = costTo[i] - costTo[i-1];
        }
    }


    function setCost(uint256[] memory indexes, uint256[] memory values) public onlyOwner {
        uint256 len = values.length;
        require(len == indexes.length, "NFTTrainer: input wrong");
        for ( uint256 i = 0 ; i < len ; i ++) {
            require(indexes[i] != 0  && values[i] != 0 || indexes[i] == 0  && values[i] == 0, "NFTTrainer: starting value can only be 0, other values cant be 0");
            cost[indexes[i]] = values[i];
        }
        // recalculate costTo-s
        for ( uint256 i = 1 ; i < 256 ; i ++) {
            costTo[i] = cost[i] + costTo[i-1];
        }
    }
    ...
    function setGenerationAllowed(uint256 _generation, bool _allowed) public onlyOwner {
        allowedGenerations[_generation] = _allowed;
        emit GenerationAllowed(_generation, _allowed);
    }

    function costToUpgrade(uint256 current, uint256 to) public view returns(uint256) {
        return costTo[to] - costTo[current];
    }

    function setPreyPredator(address _preyPredator) external onlyOwner {
        preyPredator = PreyPredator(_preyPredator);
    }

    function train(uint256 _tokenId, uint8 agility, uint8 charisma, uint8 damage, uint8 defense, uint8 dexterity, uint8 health, uint8 intelligence, uint8 luck, uint8 strength) external whenNotPaused {
        PreyPredator.PreyPredator memory traits =  preyPredator.getTokenTraits(_tokenId);
        require(allowedGenerations[traits.generation], "NFTTrainer: this generation is not allowed to train");        
        uint256 maxLevel = 75;
        if (traits.generation == 0) {
            if (traits.isPrey) maxLevel = 200;
            else if (traits.alphaIndex == 0) maxLevel = 100;
            else if (traits.alphaIndex == 1) maxLevel = 150;
            else if (traits.alphaIndex == 2) maxLevel = 200;
            else if (traits.alphaIndex == 3) maxLevel = 250;
            else if (traits.alphaIndex == 4) maxLevel = 255;
        }
        require(agility <= maxLevel, "NFTTrainer: agility is higher than the max your character can achieve");
        require(charisma <= maxLevel, "NFTTrainer: charisma is higher than the max your character can achieve");
        require(damage <= maxLevel, "NFTTrainer: damage is higher than the max your character can achieve");
        require(defense <= maxLevel, "NFTTrainer: defense is higher than the max your character can achieve");
        require(dexterity <= maxLevel, "NFTTrainer: dexterity is higher than the max your character can achieve");
        require(health <= maxLevel, "NFTTrainer: health is higher than the max your character can achieve");
        require(intelligence <= maxLevel, "NFTTrainer: intelligence is higher than the max your character can achieve");
        require(luck <= maxLevel, "NFTTrainer: agility is higher than the max your character can achieve");
        require(strength <= maxLevel, "NFTTrainer: strength is higher than the max your character can achieve");
        require(traits.agility <= agility, "NFTTrainer: agility should be higher than current one");
        require(traits.charisma <= charisma, "NFTTrainer: charisma should be higher than current one");
        require(traits.damage <= damage, "NFTTrainer: damage should be higher than current one");
        require(traits.defense <= defense, "NFTTrainer: defense should be higher than current one");
        require(traits.dexterity <= dexterity, "NFTTrainer: dexterity should be higher than current one");
        require(traits.health <= health, "NFTTrainer: health should be higher than current one");
        require(traits.intelligence <= intelligence, "NFTTrainer: intelligence should be higher than current one");
        require(traits.luck <= luck, "NFTTrainer: luck should be higher than current one");
        require(traits.strength <= strength, "NFTTrainer: strength should be higher than current one");

        uint256 val = 0;
        val += costTo[agility] - costTo[traits.agility];
        val += costTo[charisma] - costTo[traits.charisma];
        val += costTo[damage] - costTo[traits.damage];
        val += costTo[defense] - costTo[traits.defense];
        val += costTo[dexterity] - costTo[traits.dexterity];
        val += costTo[health] - costTo[traits.health];
        val += costTo[intelligence] - costTo[traits.intelligence];
        val += costTo[luck] - costTo[traits.luck];
        val += costTo[strength] - costTo[traits.strength];

        if (dao == address(0)) {
            glad.burnFrom(msg.sender, val);
        } else {
            glad.transferFrom(msg.sender, dao, val);
        }

        traits.agility = agility;
        traits.charisma = charisma;
        traits.damage= damage;
        ...
        preyPredator.updateTokenTraits(_tokenId, traits);
    }
}
