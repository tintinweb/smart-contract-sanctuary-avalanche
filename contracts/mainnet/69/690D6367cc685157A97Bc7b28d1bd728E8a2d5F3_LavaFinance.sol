...
contract LavaFinance is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event MicroRewardUpdated(uint amount);
    ...
    event PermanentNFTUpdated(address indexed bronze, address indexed silver, address indexed gold);
    event ClaimCooldownUpdated(uint cooldown);
    event MaxClaimIntervalUpdated(uint maxInterval);
    event CompoundClaimWeightUpdated(uint compoundWeight);
    event MaxNodeLavaUpdated(uint amount);
    event FusionParametersUpdated(uint fees, bool indexed isActive);
    event PLavaPriceUpdated(uint pLavaPrice);
    ...
    mapping(address => uint) public pendingMicroRewards;
    ...
    uint public pLavaPrice;

    // decay parameters
    uint public decayedRewardFactor;    // 4 decimals. 4000 = 40%
    uint public decayedFusionWeight;    // 4 decimals. 5000 = 50%

    uint public compoundClaimWeight;    // 4 decimals. 5000 = 50%

    address public fusionNFT;
    uint public fusionFees; // 6 decimals
    bool public fusionActive;
    uint private constant lavaDecimals = 18;
    uint private constant TIER_LIMIT = 10;
    ...
    function setMaxClaimInterval(uint _maxClaimInterval) external onlyOwner {
        maxClaimInterval = _maxClaimInterval;
        emit MaxClaimIntervalUpdated(maxClaimInterval);
    }

    function setCompoundClaimWeight(uint _compoundClaimWeight) external onlyOwner {
    ...
    function setPLavaPrice(uint _pLavaPrice) external onlyOwner {
    ...
    function setDecayParameters(uint _decayedRewardFactor, uint _decayedFusionWeight) external onlyOwner {
        require(_decayedRewardFactor <= 10000, "Reward cannot be more than 100%");
        require(_decayedFusionWeight <= 10000, "Fusion weight cannot be more than 100%");
    ...
    function setNodeMintPrice(uint[] memory tokenIds, uint _mintPrice) external onlyOwner {
    ...
    function mint(uint tier, address token, uint numNodes, string memory name) external nonReentrant returns (uint cost) {
        ...
        if (token == lpPairAddress && lpPairRatio > 0) {
            ...
            if(token == lavaToken || token == pLavaToken ) {
                ...
                require(IERC20(token).transferFrom(msg.sender, address(this), cost), "Payment error");
            } else {
        ...
        if (token == lavaToken || token == pLavaToken) {
            ...
            require(IERC20(lavaToken).transfer(lpWallet, (lavaAmount * lpRatio) / 1e4), "Payment error");
        }
    }

    function _mintNode(address user, uint tier, string memory name, bool isPlava) internal {
        _mintNodeNoGLava(user, tier, name, isPlava);
        GLAVA(gLavaToken).mint(user, tiers[tier].cost);
    }

    function _mintNodeNoGLava(address user, uint tier, string memory name, bool isPlava) internal whenNotPaused {
        uint nodePrice = tiers[tier].cost;
        require(userNodeAmount[user] + nodePrice <= maxNodeLavaAllowed, "Limit reached");
        // Using 1 usdce = 1$. Good enough for ROI stuff
        uint mintPrice = getNodePriceInToken(usdce, nodePrice);
        if (isPlava && pLavaPrice > 0) {
            uint mintPricePlava = pLavaPrice * nodePrice / 1e18;
    ...
    function addMicroNode(address token, uint amount) external nonReentrant {
        ...
        require(IERC20(token).transferFrom(msg.sender, address(this), amount), "Payment error");
    ...
    function payMaintenanceFees(uint[] memory tokenIds, uint[] memory numMonths) external nonReentrant returns (uint amount) {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (nodeIsDecayed(nodeData[tokenId])) {
                maintenanceAmount = maintenanceAmount * decayedRewardFactor / 1e4;
            } else {
    ...
    function claim(uint[] memory tokenIds, uint compoundPercent, bool claimMicro, address booster) external nonReentrant {
        uint amountToClaim = setClaimAmount(msg.sender, tokenIds, compoundPercent, claimMicro, booster);
        require(amountToClaim > 0, "Nothing To Claim");

        if (!claimMicro && compoundPercent > 0) {
            pendingMicroRewards[msg.sender] = getMicroReward(msg.sender);
        } else {
        ...
        if (amountToClaim > 0) {
            require(IERC20(lavaToken).transfer(msg.sender, amountToClaim), "Payment error");
            emit NodeRewardsClaimed(msg.sender, amountToClaim);
        }
        updateOracle();
    }

    function setClaimAmount(address user, uint[] memory tokenIds, uint compoundPercent, bool claimMicro, address booster) internal returns (uint amount) {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            (uint tokenAmount, uint claimAmount, uint claimTimestamp) = getTokenClaim(user, tokenId, compoundPercent, true);
            amount += (tokenAmount * bonus) / 100;
            if (claimTimestamp > 0) {
                nodeData[tokenId].lastClaim = claimTimestamp;
                nodeData[tokenId].amountClaimed += (claimAmount * bonus) / 100;
            }
        }
        if (claimMicro && microNodes[user] > 0) {
            amount += (getMicroReward(user) * bonus) / 100;
            lastMicroClaim[user] = block.timestamp;
        }
    }
    ...
    function fuseNodes(uint[] memory tokenIds, uint newTier, string memory name) external nonReentrant {
        ...
        for (uint i=0; i < tokenIds.length; i++) {
            ...
            uint nodeWeight = nodeIsDecayed(nodeData[tokenId]) ? decayedFusionWeight : 1e4;
            currentNodeCost += (tiers[nodeData[tokenId].tier].cost * nodeWeight) / 1e4;
        }
    ...
    function adminWithdraw(address token) external onlyOwner {
        require(IERC20(token).transfer(msg.sender, IERC20(token).balanceOf(address(this))), "Payment error");
    }
    ...
    function getNodePriceInToken(address token, uint nodePrice) public view returns (uint) {
        uint nodePriceInUsdce = oracle.consult(lavaToken, nodePrice);
        if (token == usdce) {
    ...
    function getClaimAmount(address user, uint[] memory tokenIds, uint compoundPercent, bool claimMicro, address booster) external view returns (uint amount) {
        (uint bonus, ) = getBoosterBonus(user, booster);

        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            (uint tokenAmount, , ) = getTokenClaim(user, tokenId, compoundPercent, false);
            amount += (tokenAmount * bonus) / 100;
        }
        if (claimMicro && microNodes[user] > 0) {
            amount += (getMicroReward(user) * bonus) / 100;
    ...
    function getTokenClaim(address user, uint tokenId, uint compoundPercent, bool useCooldown) public view returns (uint amount, uint claimAmount, uint claimTimestamp) {
        ...
        if (!useCooldown || block.timestamp - nd.lastClaim > claimCooldown) {
            ...
            uint claimInterval = claimTimestamp - nd.lastClaim;
            if (claimInterval > maxClaimInterval) {
                claimInterval = maxClaimInterval;
            }
            uint salesTaxFactor = 1e4 - ILavaParameters(lavaToken).salesTax();
            uint compoundFactor = 1e4 - (compoundPercent * (1e4 - compoundClaimWeight) / 1e4);
            // Reduce calculations if node is already decayed
            if (nodeIsDecayed(nd)) {
                amount = (claimInterval * tiers[nd.tier].reward * decayedRewardFactor) / (1e4 * 1 days);
                uint claimAmountLava = (amount * compoundFactor * salesTaxFactor) / 1e8;
                claimAmount = getNodePriceInToken(usdce, claimAmountLava);
            } else {
                amount = (claimInterval * tiers[nd.tier].reward) / 1 days;
                uint claimAmountLava = (amount * compoundFactor * salesTaxFactor) / 1e8;
                claimAmount = getNodePriceInToken(usdce, claimAmountLava);
                if (claimAmount + nd.amountClaimed > nd.mintPrice) {
                    // 4 decimals
                    uint decayPortion = (1e4 * (claimAmount + nd.amountClaimed - nd.mintPrice) ) / claimAmount;
                    amount = getDecaySplitAmount(amount, decayPortion);
                    claimAmount = getDecaySplitAmount(claimAmount, decayPortion);
                }
            }
        }
    }

    function getDecaySplitAmount(uint amount, uint decayPortion) internal view returns (uint) {
        return (amount * (1e4 - decayPortion) / 1e4) + (amount * decayPortion * decayedRewardFactor / 1e8);
    }

    function getMicroReward(address user) internal view returns (uint) {
        return (microNodes[user] * (block.timestamp - lastMicroClaim[user]) * microReward) / (10**lavaDecimals * 1 days) + pendingMicroRewards[user];
    }
    ...
    function getUserNodes(address user) external view returns (uint[] memory nodeIds) {
    ...
    function transferNodeByOwner(address from, address to, uint tokenId) external onlyOwner {
