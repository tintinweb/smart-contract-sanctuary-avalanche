...
library LMyChance {
    ...
    function claimInternal(uint256 total, address charity, uint256 _percentage) internal returns(uint256 withdrawlAmount, uint256 _totalFees, uint256 totalToCharity) {
        ...
        payable (msg.sender).transfer((withdrawalAmount - totalToCharity));
        payable (charity).transfer(totalToCharity);
