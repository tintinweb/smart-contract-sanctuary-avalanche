...
contract FunnyCops is ERC721 {
    ...
    constructor()
        ERC721("Funny Cops", "FunnyCops")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 78) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeibjfxglmbbvwcku7pfl57xa6v2laaq7efhwy6tffjoyd5mxij7vfe/", Strings.toString(tokenId), ".json")
            );
    }
