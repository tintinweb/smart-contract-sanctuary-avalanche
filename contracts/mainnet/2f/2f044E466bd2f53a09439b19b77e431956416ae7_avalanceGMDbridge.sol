/**
 *Submitted for verification at snowtrace.io on 2023-01-30
*/
...
// File: contracts/crosschain/avalanceGMDbridge.sol
...
contract avalanceGMDbridge is Ownable, NonblockingLzApp{
    ...
    function _nonblockingLzReceive(
        ...
        if(message.id == 0){
            GMD.mint(message.to, message.amount);
        }
        if(message.id == 1){
            esGMD.mint(message.to, message.amount);
        }
    ...
    function bridgeTokens(uint16 dstChainId, uint amount, uint id) external payable {
        ...
        if(id == 0){
            GMD.transferFrom(sender, owner(), feeAmount);
            GMD.burn(sender, amount);
        }
        if(id == 1){
            esGMD.transferFrom(sender, owner(), feeAmount);
            esGMD.burn(sender, amount);
        }
