...
contract CMB is PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	uint256 public constant CANCELABLE_DURATION = 2 minutes;

	/**s
	 *  @dev The extend duration that BO can withdraw when payment is expired
	 */
	// uint256 public constant BO_EXTEND_WITHDRAW_DURATION = 1 days;
	uint256 public constant BO_EXTEND_WITHDRAW_DURATION = 2 minutes;

	/**
	 *  @dev The extend duration that client can withdraw when payment is expired
	 */
	uint256 public clientExtendWithdrawDuration;
	...
	event SetClientExtendWithdrawDuration(uint256 oldDuration, uint256 newDuration);
	...
	modifier onlyBusinessOwner(uint256 _paymentId) {
		require(_msgSender() == payments[_paymentId].bo, "Caller is not the BO of this payment");
		_;
	}
	...
	function initialize(address _owner) public initializer {
		...
		// clientExtendWithdrawDuration = 30 days;
		clientExtendWithdrawDuration = 2 minutes;
	...
	/**
	 *  @dev    Set client extend withdraw duration
	 *
	 *  @notice Only owner can call this function.
	 *
	 *          Name            Meaning
	 *  @param  _duration       New duration want to update
	 *
	 *  Emit event {SetClientExtendWithdrawDuration}
	 */
	function setClientExtendWithdrawDuration(uint256 _duration) external whenNotPaused onlyOwner {
		require(_duration > 0, "Invalid duration");

		uint256 oldDuration = serviceFeePercent;
		clientExtendWithdrawDuration = _duration;
		emit SetClientExtendWithdrawDuration(oldDuration, _duration);
	}
	...
	function requestPayment(
		...
		require(_msgSender() != _client, "Business Owner and Client can not be same");
		...
		require(_numberOfInstallment > 0, "Number of installments must be greater than 0");
		...
		onlyBusinessOwner(_paymentId)
	{
		...
		if (payment.lastTreatmentId > 0) {
			TreatmentStatus treatmentStatus = treatments[_paymentId][payment.lastTreatmentId].status;
			require(
		...
		emit ConfirmedProvideServices(_paymentId, payment.lastTreatmentId, payment.status, treatment.status);
	}
		...
		onlyClient(_paymentId)
	{
		...
		Treatment storage treatment = treatments[_paymentId][payment.lastTreatmentId];
		require(treatment.status == TreatmentStatus.BO_CONFIRMED, "Business owner hasn't confirmed treatment yet");

		treatment.status = TreatmentStatus.CLIENT_CONFIRMED;
		payment.pendingTreatmentIds.push(payment.lastTreatmentId);

		emit ConfirmedToRelease(_paymentId, payment.lastTreatmentId, treatment.status);
	}
		...
		nonReentrant
	{
		...
		for (uint256 i = 0; i < payment.pendingTreatmentIds.length; ++i) {
			...
			treatments[_paymentId][treatmentId].status = TreatmentStatus.CLAIMED;
		}

		uint256 claimableTotalAmount = payment.pendingTreatmentIds.length * payment.amount;
		...
		uint256[] memory pendingTreatmentIdsBefore = payment.pendingTreatmentIds;
		payment.pendingTreatmentIds = new uint256[](0);

		// Calculate fee and transfer tokens
		uint256 serviceFee = calculateServiceFee(claimableTotalAmount, payment.serviceFeePercent);
	...
	function cancelPayment(uint256 _paymentId) external whenNotPaused onlyValidPayment(_paymentId) nonReentrant {
		...
		if (paymentStatusBefore == PaymentStatus.PAID) {
			...
			_withdraw(payment.client, payment.paymentToken, withdrableAmount);
		}

		emit Canceled(payment.client, _paymentId, payment.amount, payment.paymentToken, payment.status);
	}
	...
	function calculateServiceFee(uint256 _amount, uint256 _serviceFeePercent) public pure returns (uint256) {
		return (_amount * _serviceFeePercent) / (10**WEIGHT_DECIMAL);
	}
	 ...
	 */
	function getPendingTreatmentIds(uint256 _paymentId) external view returns (uint256[] memory) {
