...
contract Managers{
    ...
    function newAction(
        ...
        if (cmd == ActionCmd.AddManager) {
            require(managers[arg_addr] == 0, "Address already manager");
        } else if (cmd == ActionCmd.DelManager) {
            require(managers[arg_addr] == 1, "Address not manager");
        }
        actionId = actionId + 1;
    ...
    function signAction(uint256 _actionId) public isManager returns (bool) {
        require(_actionId == actionId, "Not current action");
        require(actions[_actionId].done == false, "Action expired");
        require(
        ...
        if (actions[_actionId].signatureCount >= (((managerLen * 2) / 3) + 1)) {
            ...
            if (actions[_actionId].cmd == ActionCmd.AddManager) {
                ...
                return true;
            } else if (actions[_actionId].cmd == ActionCmd.MintToken) {
                annuToken.mint(actions[_actionId].arg_addr,actions[_actionId].arg_num);
                return true;
            } else {
                revert("Error cmd name");
            }
        } else {
            return false;
        }
