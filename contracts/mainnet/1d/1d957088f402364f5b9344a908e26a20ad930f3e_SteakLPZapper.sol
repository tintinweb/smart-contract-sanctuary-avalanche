...
// File contracts/libraries/IJoePair.sol
...
// File contracts/libraries/IJoeRouter01.sol
...
// File contracts/libraries/IJoeRouter02.sol
...
// File contracts/libraries/IWAVAX.sol
...
// File contracts/utilities/LPZapper.sol
...
/*
 * SteakHut
 * MIT License; modified from PancakeBunny / Trader Joe
 *
 */
...
/// @title LPZapper
/// @author SteakHut Finance Team
/// @notice This contract is utilized to zap avax / other nominated tokens into JLP tokens.
contract SteakLPZapper is Ownable {
    ...
    function zapInToken(
        ...
        _approveTokenIfNeeded(_from, address(ROUTER), amount);

        if (isLP(_to)) {
            IPair pair = IPair(_to);
    ...
    function _swapAVAXToLP(
        ...
        if (!isLP(lp)) {
            ...
            if (token0 == WAVAX || token1 == WAVAX) {
                ...
                _approveTokenIfNeeded(token, address(ROUTER), amount);
                ...
                _approveTokenIfNeeded(token0, address(ROUTER), amount);
                _approveTokenIfNeeded(token1, address(ROUTER), amount);
