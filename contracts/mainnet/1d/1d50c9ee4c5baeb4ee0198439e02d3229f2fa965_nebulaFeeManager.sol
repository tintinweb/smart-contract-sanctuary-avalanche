...
contract nebulaFeeManager is Ownable{
    ...
    function findFromName(address _account, string memory _name) internal returns(uint256){
    	    	PROTOOWNERS[] storage protos = protoOwners[_account];
    	    	for(i = 0;i<protos.length;i++) {
    			PROTOOWNERS storage proto = protos[i];
  ...
  function mgrTotQuery() internal {
    ...
    function allQuery() external managerOnly(){
    	mgrTotQuery();
    }
