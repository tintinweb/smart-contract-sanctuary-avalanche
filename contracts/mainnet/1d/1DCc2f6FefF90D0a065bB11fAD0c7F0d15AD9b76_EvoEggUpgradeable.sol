...
AccessControlEnumerableUpgradeable, TokenConstants, ERC721BlacklistUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable) returns (string memory) {
        string memory imageURI = string(abi.encodePacked(imageBaseURI, (tokenId % 4).toString(), '.webm'));
    ...
    function checkTreated(uint256[] memory tokenIds) public returns(bool[] memory) {
    ...
    function staffMint(address[] memory recipients, uint256[] memory tokenIds) public onlyRole(ADMIN_ROLE) {
        _tokenIdCounter._value = 50;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] <= 50, "Invalid tokenId");
