...
contract SPePe is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 amount) public payable nonReentrant {
    ...
    require(amount <= MaxperWallet, "max mint amount per tx exceeded");
    require(totalSupply() + amount <= maxSupply, "We Soldout");
    require(_numberMinted(_msgSenderERC721A()) + amount <= MaxperWallet, "Max NFT Per Wallet exceeded");
...
/// @dev free mint
    function freemint(uint256 amount) public nonReentrant {
    require(!paused, "oops contract is paused");
    require(_numberMinted(_msgSenderERC721A()) + amount <= MaxperWalletFree, "Max NFT Per Wallet exceeded");
    require(amount <= MaxperWalletFree, "max mint per Tx exceeded");
    require(totalSupply() + amount <= FreeSupply, "Whitelist MaxSupply exceeded");
