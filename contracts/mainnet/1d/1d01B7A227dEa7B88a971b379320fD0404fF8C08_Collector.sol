...
contract Collector is Ownable {
    ...
    error MintPassNotExist(uint256 tokenId);
    error MintPassNotYours(uint256 tokenId);
    error MintPassNotApproved(uint256 tokenId);
    ...
    function setDestinationContract(address destinationContract_) public onlyOwner {
        if (destinationContract != address(0)) revert DestinationContractAlreadySet();
    ...
    function _collectBox(uint256[] calldata tokenIds_) internal {
        ...
        bool isApprovedForAll = mintPass.isApprovedForAll(msg.sender, address(this));

        // check order, owners and collected status
        uint256 last = 750;
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (collected[tokenIds_[i]] != address(0) || tokenIds_[i] >= 750) revert MintPassNotExist(tokenIds_[i]);
            if (last != 750 && last >= tokenIds_[i]) revert UnorderedTokenIds();
            last = tokenIds_[i];

            // check if the caller is the owner of the collectable box
            if (mintPass.ownerOf(tokenIds_[i]) != msg.sender) revert MintPassNotYours(tokenIds_[i]);
            if (!isApprovedForAll && mintPass.getApproved(tokenIds_[i]) != address(this))
                revert MintPassNotApproved(tokenIds_[i]);
        }
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            mintPass.transferFrom(msg.sender, address(this), tokenIds_[i]);
        }
    }
    ...
    function _lzSend(
        ...
        _lzEndpoint.send{value: msg.value}(
            destinationChainId,
            abi.encodePacked(destinationContract, address(this)),
            _payload,
            _refundAddress,
            address(0),
            _txParam
