...
contract BavaCompoundVault_BoostedJoeUpgradeable is Initializable, UUPSUpgradeable, BRTERC20Upgradeable {
    ...
    function setAllowances(uint256 _amount) external onlyOwner {
        if (address(router) != address(0)) {
            ...
            for (uint i; i < rewardLength; i++) {
                if (address(bonusRewardTokens[i]) != address(0)) {
    ...
    function _stakeDepositTokens(uint256 _amount) private {
        ...
        proxy.distributeReward(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(bonusRewardTokens[0]));
    }
    ...
    function _getReinvestReward() internal {
        proxy.claimReward(vaultInfo.restakingFarmID, address(vaultInfo.stakingContract), address(bonusRewardTokens[0]));
    }
	...
	function setStage(uint[] memory _blockStarts, uint[] memory _blockEnds) public onlyAuthorized {
