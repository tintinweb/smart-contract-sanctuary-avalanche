...
contract LendingManager is ILendingManager, Ownable, Pausable, Modifier {
    using SafeERC20 for IERC20;

    // Interfaces
    address private core;
    ILendingModule private module;
    ...
    constructor(address _module, address _suppliedAsset, address _borrowedAsset, address[] memory _assets, address[] memory _priceFeeds) {
    ...
    function setCore(address _core) external onlyOwner onlyNotZeroAddress(_core) onlyIfEmptyModule whenPaused {
        core = _core;
    }
    ...
    function setModule(address _module) external onlyOwner onlyNotZeroAddress(_module) onlyIfEmptyModule whenPaused {
        module = ILendingModule(_module);
    }
    ...
    function addPriceFeeds(address[] calldata _assets, address[] calldata _priceFeeds) external onlyOwner whenPaused {
        require(_assets.length == _priceFeeds.length, "not same length");
        for(uint256 i = 0; i < _assets.length; i++) {
            priceFeeds[_assets[i]] = _priceFeeds[i];
        }
    ...
    function removePriceFeeds(address[] calldata _assets) external onlyOwner whenPaused {
        for(uint256 i = 0; i < _assets.length; i++) {
    ...
    function supply(uint256 amount) external onlyCore whenNotPaused {
        IERC20(suppliedAsset).transferFrom(core, address(module), amount);
        module.supply(amount);
    }

    function withdraw(uint256 amount) external onlyCore whenNotPaused {
        module.withdraw(amount);
        IERC20(suppliedAsset).transferFrom(address(module), core, amount);
    }

    function borrowMax() external onlyCore whenNotPaused returns (uint256) {
