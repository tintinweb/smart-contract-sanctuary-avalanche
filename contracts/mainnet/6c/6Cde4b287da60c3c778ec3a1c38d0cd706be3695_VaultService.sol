...
contract PoolService is Ownable {
    ...
    event SwapTokensForExactTokens(address indexed sender, uint256 amountOut, uint256 amountInMax, address[] path, uint256 deadline, string router, uint256 amountInFact);
    event SwapExactTokensForTokens(address indexed sender, uint256 amountIn, uint256 amountOutMin, address[] path, uint256 deadline, string router, uint256 amountOutFact);
    ...
    function swapTokensForExactTokens(
        ...
        _swap(false, amountInMax, amountOut, path, deadline, router);
    }
    ...
    function swapExactTokensForTokens(
        ...
        _swap(true, amountIn, amountOutMin, path, deadline, router);
    }

    function _swap(
        bool exactTokensForTokens,
        uint256 amountIn,
        uint256 amountOut,
        address[] calldata path,
        uint256 deadline,
        ...
        if (exactTokensForTokens) {
            emit SwapExactTokensForTokens(msg.sender, amountIn, amountOut, path, deadline, router, amountOutGot);
        } else {
            emit SwapTokensForExactTokens(msg.sender, amountOut, amountIn, path, deadline, router, amountInSpent);
        }
    }
    ...
    function calcPositionBalance(address holder) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokensCount; i++) {
            (address addr, uint256 amount) = _PositionRepository.getTokenById(address(this), holder, i);
            if (amount > 0) {
