...
contract CunoroBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Cunoro) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Cunoro), "not a Cunoro lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Cunoro.decimals() ) ).div( getTotalValue( _pair ) );
    }
}