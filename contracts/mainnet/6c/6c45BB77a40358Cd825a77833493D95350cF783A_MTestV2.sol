...
contract MTestV2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        _getReferralAmount(nodeRewardManager.nodePrice());
        // validator
        uint256 processFee;
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        uint256 referralAmount = _getReferralAmount(rewardAmount);
        //validator
        uint256 processFee = _getRewardProcessFee(rewardAmount);
    ...
    function _getReferralAmount(uint256 amount) private returns(uint256 referralAmount) {
        if (referralRateForNodeRewards > 0) {
            address referral = _getReferral(_msgSender());
            referralAmount = amount * referralRateForNodeRewards / 10000;
            if (referralAmount > 0) {
                ReferralIncome[referral] += referralAmount;
    ...
    function _getRewardProcessFee(uint256 rewardAmount) private returns(uint256 processFee) {
