...
contract Zap is Ownable {
    ...
    address private WNATIVE = 0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd;
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external {
        ...
        IJoeRouter01(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);
    }
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtETH;
            (amtToken, amtETH) = IJoeRouter01(routerAddr).removeLiquidityAVAX(token, amount, 0, 0, address(this), block.timestamp);
            ...
            uint amt0;
            uint amt1;
            (amt0, amt1) = IJoeRouter01(routerAddr).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
    ...
    function _swapTokenToLP(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) { // check if we already have one of the assets
            ...
            uint liquidity;
            ( , , liquidity) = IJoeRouter01(routerAddr).addLiquidity(_from, other, amount.sub(sellAmount), otherAmount, 0, 0, recipient, block.timestamp);
            return liquidity;
        } else {
            // go through native token for highest liquidity
