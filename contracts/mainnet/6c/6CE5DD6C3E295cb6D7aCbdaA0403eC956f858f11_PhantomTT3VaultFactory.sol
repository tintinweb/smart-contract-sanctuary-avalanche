...
import { PhantomTT3Vault } from "./PhantomTT3Vault.sol";
import { VaultFactoryBase } from "./VaultFactoryBase.sol";

contract PhantomTT3VaultFactory is VaultFactoryBase
{
	function _newVault() internal override returns (address _vault)
	{
		return address(new PhantomTT3Vault());
	}
}
...
contract PhantomTT3Vault is VaultBase
{
	...
	LibAaveV3Pool.Self private lendingPool;
	LibUniswapV2Pair.Self private rewardPool;

	function _initialize(bytes memory _data) internal override
	{
		(lendingPool, rewardPool) = abi.decode(_data, (LibAaveV3Pool.Self, LibUniswapV2Pair.Self));
...
library LibAaveV3Pool
{
	...
	function _new(address _asset) internal view returns (Self memory _self)
	{
		...
		return Self({ pool: _pool, asset: _asset, underlyingAsset: _underlyingAsset, incentives: _incentives, rewardAssets: _rewardAssets });
	}

	function _balanceOf(Self storage _self, address _account) internal view returns (uint256 _balance)
	{
		return IERC20(_self.asset).balanceOf(_account);
	}
	...
	{
		IAaveV3Pool(_self.pool).supply(_self.underlyingAsset, _amount, address(this), 0);
	}
...
abstract contract VaultBase is Initializable, Ownable, ReentrancyGuard, ERC20, EmergencyControl, Whitelistable, IVault
{
	...
	{
		return _shares * (_totalReserve + 1) / (_totalSupply + 1);
	}
