...
contract Shields is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function mint() external payable mintCompliance() mintPriceCompliance() nonReentrant {
    require(!paused, 'contract paused.');

    _safeMint(_msgSender(), 1);
  }

  function mint(address _receiver) external payable mintCompliance() mintPriceCompliance() nonReentrant {
    require(!paused, 'contract paused.');

    _safeMint(_receiver, 1);
  }

  function mintForAddress(address _receiver) external mintCompliance() onlyOwner nonReentrant {
    _safeMint(_receiver, 1);
  }
  ...
  function withdraw() external onlyOwner nonReentrant {
    // sends 10% to contract creator //
    (bool hs, ) = payable(0xFd63Bf84471Bc55DD9A83fdFA293CCBD27e1F4C8).call{value: address(this).balance * 10 / 100}('');
    require(hs);
    // sends remainder (90%) to owner //
