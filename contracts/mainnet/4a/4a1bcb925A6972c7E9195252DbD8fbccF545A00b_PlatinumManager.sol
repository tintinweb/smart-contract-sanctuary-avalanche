...
interface IPlatinumManager is IERC721 {
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) external;
    function createNodeWithRewards(string memory _name, uint64[] memory userNodes) external;
}

interface PlatinumManagerV1 {
...
interface AmsHelper {
  ...
  function claimRewardsForCompoundPlatinum(address sender, uint256 nodePrice ) external returns (uint256);
}

interface PlatinumHelperV1 {
...
interface IAscendMembershipManager {
    ...
    function infinite_membership() external view returns (IERC721 );
    function balanceOf(address owner) external view returns (uint256);
    function transferFrom(address from, address to, uint256 tokenId) external;
}

contract PlatinumManager is Ownable, IERC721, IERC721Metadata, IPlatinumManager {
    ...
    mapping(uint256 => Platinum) private _nodes;
    ...
    uint256 public nodeAscendTokenPrice = 250;
    uint256 public nodeAscendMembershipPrice = 25;
    uint256 private nodeCounter = 1;
    uint public maxPlatinumWallet = 1;
    bool public transferIsEnabled = true;
    bool enableClaims = true; //upgradeable
    ...
    PlatinumManagerV1 public platinumManagerV1;
    PlatinumHelperV1 public platinumHelperV1;
    ...
    BoostersParameters public boostersParameters = BoostersParameters({
        presaleBooster: 15937, // 15%
        founderL1Booster: 10625, // 10%
        founderL2Booster: 15937, // 15%
        founderL3Booster: 21250, // 20%
        infiniteBooster: 0,
        metaBooster: 0
    });
    ...
    event CreatePlatinum(address from, uint id);
    event RemovePlatinum(address from, uint id);
    event ClaimPlatinum(address account, uint rewardNode);
    event CreatePlatinumWithRewards(address sender, uint rewardAmount);

    constructor(address _ASCEND, address _amsManager, address _amsHelper) {
        amsManager = IAscendMembershipManager(_amsManager);
    ...
    function setPlatinumManagerV1(address _platinumManagerV1 ) external onlyOwner {
        platinumManagerV1 = PlatinumManagerV1(_platinumManagerV1);
    }

    function setPlatinumHelperV1(address _platinumHelperV1 ) external onlyOwner {
        platinumHelperV1 = PlatinumHelperV1(_platinumHelperV1);
    }
    ...
    function setBoosters(uint256 _founderL1Booster, uint256 _founderL2Booster,
      uint256 _founderL3Booster, uint256 _presaleBooster, uint256 _infiniteBooster,
    ...
    function changeEnableClaims(bool _newVal) onlyOwner external {
    ...
    function updatePlatinum(uint256 id, string calldata metadata) external {
        Platinum storage platinum = _nodes[id];
        platinum.metadata = metadata;
    }

    function getPlatinums(uint256 _id) public view onlyIfExists(_id) returns (Platinum memory) {
        return _nodes[_id];
    }
    ...
    function getPlatinumsOf(address _account) public view override returns (uint256[] memory) {
        return _bags[_account];
    }
    function updateTokenPrice (uint256 _nodeAscendTokenPrice) external onlyOwner {
        nodeAscendTokenPrice = _nodeAscendTokenPrice;
    }
    function updateAMSPrice (uint256 _nodeAscendMembershipPrice) external onlyOwner {
    ...
    function _addPlatinum(string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        uint256 nodeId = nodeCounter;
        _createPlatinum(nodeId, _name, _mint, _claim, _metadata, _to);
        nodeCounter += 1;
    }
    ...
    function setAmsManager(IAscendMembershipManager _amsManager) external onlyOwner {
        amsManager = IAscendMembershipManager(_amsManager);
    }
    function setMaxPlatinumWallet(uint256 _max) external onlyOwner {
        maxPlatinumWallet = _max;
    }
    function setAscendMembership(IERC20 _ASCEND) external onlyOwner {
        ASCEND = IERC20(_ASCEND);
    }
    ...
    function getAddressRewards(address account) external override view returns (uint) {
        uint256 rewardAmount = 0;
        uint256[] memory userMemberships = getPlatinumsOf(account);
        uint interval = 0;
        for (uint256 i = 0; i < userMemberships.length; i++) {
            Platinum memory _node = _nodes[userMemberships[i]];
    ...
    function getReward(address from) public view returns(uint256) {
        uint rewardNode = rewardsParameters.reward + getUserAdditionalRewardsPlatinum(from) + getUserBooster(from);
        return rewardNode;
    }


    function getUserAdditionalRewardsPlatinum(address from) public view returns (uint256) {
      if(amsManager.meta_membership().balanceOf(from) > 0){
        return boostersParameters.metaBooster;
      } else if(amsManager.infinite_membership().balanceOf(from) > 0){
    ...
    function claim(address account, uint256 _id) external onlyIfExists(_id) returns (uint) {
      ...
      Platinum memory _node = _nodes[_id];
      ...
      require(rewardNode >= 1, "You don't have enough reward");
      _node.claim = _node.claim + (interval*rewardsParameters.claimTime);   // uint64(block.timestamp);
      _nodes[_id] = _node;
      emit ClaimPlatinum(account, rewardNode);
      return rewardNode;
    }

    function _createPlatinum(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to) internal {
        require(!_exists(_id), "MANAGER: Platinum already exist");
        _nodes[_id] = Platinum({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function _remove(uint256 _id, address _account) internal {
        ...
        _ownerNodes.pop();
        emit RemovePlatinum(_account, _id);
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        Platinum memory _node = _nodes[uint64(tokenId)];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == address(0) || to == deadAddress, "Platinum Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter;
        _createPlatinum(nodeId, nodeName, uint64(block.timestamp), uint64(block.timestamp), "", account);
        nodeCounter += 1;
    }
    ...
    function migrateNodes(uint64[] memory userNodes) public  {
      ...
      require(userNodes.length + balanceOf(sender) <= maxPlatinumWallet, "HELPER: Exceeds max platinum per wallet");
      uint64[] memory _claims = new uint64[](userNodes.length);
      for (uint256 i = 0; i < userNodes.length; i++) {
           require(sender ==  platinumManagerV1.ownerOf(userNodes[i]), "You are not the owner of the Node" );
           uint64 nodeclaim = platinumHelperV1.seeNodeClaim(userNodes[i]);
           if  (nodeclaim == 0){
             nodeclaim =  platinumManagerV1.getMintOf(userNodes[i]);
           }
           _claims[i] = nodeclaim;
           platinumManagerV1.transferFrom(_msgSender(), address(deadAddress), userNodes[i] );
      }
      migratePlatinums(_claims, sender);
    }

    function migratePlatinums( uint64[] memory _claims, address _to) internal {
        for (uint256 i = 0; i < _claims.length; i++) {
            uint256 nodeId = nodeCounter;
            _createPlatinum(nodeId, "Platinum", _claims[i],  _claims[i], "", _to);
            nodeCounter += 1;
        }
    }

    function createNodeWithRewards(string memory _name, uint64[] memory userNodes) public override {
          ...
          require(balanceOf(sender) + 1 <= maxPlatinumWallet, "HELPER: Exceeds max wallet amount");
          require(amsManager.balanceOf(sender) >= nodeAscendMembershipPrice, "HELPER: Ascend Membership balance too low for creation.");

          uint256 rewardAmount = amsHelper.claimRewardsForCompoundPlatinum( sender, nodePrice );
          ...
          for (uint256 i = 0; i < nodeAscendMembershipPrice; i++) {
              amsManager.transferFrom(_msgSender(), address(deadAddress), userNodes[i]);
          }
          emit CreatePlatinumWithRewards( sender,  rewardAmount);
          createNode(sender, _name);
    }


    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) public override {
        require(bytes(_name).length > 0 && bytes(_name).length < 33, "HELPER: name size is invalid");
        address sender = _msgSender();
        require(sender != address(0), "HELPER:  Creation from the zero address");
        uint256 nodePrice = nodeAscendTokenPrice * 10 ** 18;
    ...
    function renamePlatinum(uint64 id, string memory newName) external {
    ...
    function transferFrom(address from, address to,uint256 tokenId) external override {
        ...
        if (transferIsEnabled == true){
            ...
            require (to == address(0) || to == address(deadAddress), "Platinum Transfers are not allowed");
            _transfer(from, to, tokenId);
        }
    }
