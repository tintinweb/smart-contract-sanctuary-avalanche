...
import "./Signable.sol";
import "./libs/GenericBuyV2Lib.sol";

/**
    @dev This contract will be used as a Avatar Transactor in the Fief Guild Marketplace
        to buy any NFT of the Avatar series.
    @dev It supports multiple payment tokens such as Fief.
 */
contract FIEFAvatarStoreTransactor is AccessControlEnumerable, Signable, Pausable {
    using SafeERC20 for IERC20;
    using GenericBuyV2Lib for GenericBuyV2Lib.GenericBuyV2;
    ...
    // Avatar address => Avatar config
    mapping(address => Config) internal avatarConfig;
    ...
    function buyTokens(GenericBuyV2Lib.GenericBuyV2 calldata buy) external whenNotPaused {
        Config storage config = avatarConfig[buy.nft];
        require(config.exist, "!nft");
        require(config.maxTokensToBuy >= buy.quantity, "!max_tokens_to_buy");
        ...
        require(
            config.maxPerWallet >= IERC721(buy.nft).balanceOf(msg.sender) + buy.quantity,
            "too_much_tokens"
        );
        ...
        uint256[] memory tokenIds = _trackSells(msg.sender, buy.nft, buy.purchaseId, buy.quantity);
    ...
    function configNft(
        ...
        Config storage config = avatarConfig[nft];
        config.exist = exist;
        config.latestTokenId = latestTokenId;
        config.maxPerWallet = maxPerWallet;
        config.maxTokensToBuy = maxTokensToBuy;

        emit NftConfigured(nft, maxPerWallet, latestTokenId, maxTokensToBuy, exist);
    }
    ...
    function getConfig(address nft)
        external
        view
        returns (
            bool exist,
    ...
    {
        Config memory config = avatarConfig[nft];
        exist = config.exist;
        latestTokenId = config.latestTokenId;
        maxPerWallet = config.maxPerWallet;
        maxTokensToBuy = config.maxTokensToBuy;
    }
    ...
    function _trackSells(
        ...
        uint256 startTokenId = config.latestTokenId;
