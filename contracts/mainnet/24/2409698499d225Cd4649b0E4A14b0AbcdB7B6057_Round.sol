...
contract Round is ERC721EnumerableV2 {
  ...
  function getReward(uint256 roundId, uint256 timestamp) internal view returns (uint256 rewards) {
    rewards = ((timestamp - lastClaimed[roundId]) * roundSizes[roundId] * EMISSION) / 1 days;
  }

  function getBoostedRewards(address account, uint256 rewards) internal view returns (uint256) {
  ...
  function getRewards(address account) external view returns (uint256 rewards) {
    ...
    rewards = getBoostedRewards(account, rewards);
  }
  ...
  function claimRewards(address account) external returns (uint256 rewards) {
    ...
    orb.transfer(account, getBoostedRewards(account, rewards));
  }
