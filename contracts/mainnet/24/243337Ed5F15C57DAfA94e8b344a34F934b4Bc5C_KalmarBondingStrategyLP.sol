...
// File: contracts/KalmarBondingStrategyLP.sol
...
contract KalmarBondingStrategyLP is ReentrancyGuard, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    address public constant lp = 0x49A426861FaE6c61F156e98467C4167de6D555c8;
    ...
    function _usdTokenPrice() internal view returns (uint256) {
      uint256 price = IChainlinkAggregator(chainlink).latestAnswer().mul(1e10);
      return price;
    }
    ...
    function _kalmPrice() internal view returns (uint256) {
        ...
        uint256 decimalsOther = (10**(IERC20Detailed(other).decimals()));
        // amount of
        uint256 otherPERkalm = (decimalsOther*otherReserve)/kalmReserve;
        uint256 kalmPrice = (otherPERkalm*_usdTokenPrice())/decimalsOther;
    ...
    function _getLpPrice() internal view returns(uint256)
    {
      ...
      uint256 totalOtherPrice = ((otherReserve*1e18)/(10**(IERC20Detailed(other).decimals()))) * _usdTokenPrice();
     ...
     */
    function setBurnAddress(address _burnAddr) external onlyAdmin {
