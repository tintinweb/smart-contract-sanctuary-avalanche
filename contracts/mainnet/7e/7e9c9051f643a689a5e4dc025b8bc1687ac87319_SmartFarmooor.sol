...
import "./interface/ISmartFarmooor.sol";
import "../common/library/Modifier.sol";
import "../common/Rescuable.sol";

/**
 * @author  HedgeFarm Team
 * @title   Smart Farmoor Strategy
 * @dev     The Smart Farmer is composable and integrable into other Yielding strategy
 * @notice  Upgradability is needed because the Smart Farmer is built on top of protocols that have Upgradable Smart contracts
 */

contract SmartFarmooor is ISmartFarmooor, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable, Rescuable, Modifier {
    ...
    /// @notice The address allowed to trigger regular maintenance actions
    address public manager;
    /// @notice The address receiving the performance fee
    address public feeManager;
    /// @notice The asset used by the Smart Farmoor
    ...
    event Withdraw(address indexed user, uint shares);
    ...
    modifier onlyIfEmptyModule() {
        require(getModulesBalance() <= minHarvestThreshold, "SmartFarmooor: module not empty");
        _;
    }
     ...
     */
    function initialize(string memory _name, string memory _symbol, address _feeManager, address _baseToken, uint256 _minHarvestThreshold, uint16 _performanceFee, uint256 _cap, address _manager, uint256 _minAmount) public initializer {
    ...
    /**
     * @notice  Makes sure only the owner can upgrade
     ...
     */
    function addModule(IYieldModule _module) external onlyOwner onlyNotZeroAddress(address(_module)) onlyIfEmptyModule whenPaused {
        require(_module.baseToken() == baseToken, "SmartFarmooor: not compatible module");
     ...
     */
    function removeModule(uint _moduleId) external onlyOwner whenPaused onlyIfEmptyModule {
     ...
     */
    function setModuleAllocation(uint[] memory _allocation) external onlyOwnerOrManager whenPaused onlyIfEmptyModule {
        ...
        require(totalAllocation == MAX_BPS, "SmartFarmooor: total allocation is wrong");
    }
     ...
     */
    function panic() external payable onlyOwnerOrManager {
        _pause();
        balanceSnapshot = getModulesBalance();
        uint executionFee = getExecutionFee(totalSupply());
        require(msg.value >= executionFee, "SmartFarmooor: msg.value to small for withdraw execution");
     ...
     */
    function finishPanic() external onlyOwnerOrManager whenPaused {
     ...
     * @param   newMinHarvestThreshold  .
     */
    function setMinHarvestThreshold(uint256 newMinHarvestThreshold) external onlyOwner {
    ...
    */
    function rescueToken(address token) external onlyOwner {
        require(token != baseToken, "SmartFarmooor: can't pull out base tokens");
        _rescueToken(token);
    }
    ...
    */
    ...
    function deposit(uint256 _amount) external nonReentrant whenNotPaused {
        require(_amount >= minAmount, "SmartFarmooor: amount too small");
        require(_amount + getModulesBalance() <= cap, "SmartFarmooor: Cap reached");
    ...
    function withdraw(uint256 _shares) external payable whenNotPaused nonReentrant returns (uint256, uint256) {
        require(_shares > 0, "SmartFarmooor: withdraw can't be 0");
        require(_shares <= balanceOf(msg.sender), "SmartFarmooor: not enough shares");
    ...
    function _getUsersShare(uint256 _amount) private returns (uint256) {
        if (totalSupply() == 0) {
            ...
            uint256 userShare = (_amount * totalSupply()) / getModulesBalance();
            require(userShare > 0, "SmartFarmooor: userShare = 0");
            return userShare;
        }
    }
    ...
    function _setManager(address newManager) internal {
        require(newManager != address(0), "SmartFarmooor: manager can't be the zero address");
        manager = newManager;
    }
    ...
    */
    function _setMinAmount(uint256 newMinAmount) internal {
    ...
    function _setPerformanceFee(uint16 newPerformanceFee) internal {
        require(
            ...
            "SmartFarmooor: performanceFee fee must be less than 20%"
        );
    ...
    function performUpkeep(bytes calldata performData) external {
        if (keccak256(performData) == HARVEST_PROFIT) {
            ...
            require(profit * MAX_BPS / (MAX_BPS - performanceFee) > minHarvestThreshold, "SmartFarmooor: not enough to harvest");
            return;
        }
        revert("Unknown task");
    }
