...
contract BPSSC is owned {
    ...
    function USDtoBP( uint256 value) public returns (bool success) {
    	IERC20 b = IERC20(bpay);
    	IERC20 u = IERC20(usd);
    	require(online);
    	u.transferFrom(msg.sender,mainWallet,value*price/(10**38));
    	b.transferFrom(mainWallet,msg.sender,value);
    	emit Sale(msg.sender,value);
        return true;
    }
    ...
    function tokenUSD() view public returns (address){
        return usd;
    }
    ...
    function changeUSD(address newUSD) onlyOwner public returns (bool success) {
        usd = newUSD;
        return true;
    }
