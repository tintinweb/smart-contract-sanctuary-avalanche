...
import { PyramidInfoEntity } from "./PyramidsManager.sol";
...
contract PresaleSwap is Ownable, OwnerRecovery, PyramidsManagerPointer {
  address private constant DEAD_ADDRESS =
    0x000000000000000000000000000000000000dEaD;

  IERC20 public newPrmd;
  IERC20 public oldPrmd;
  IPyramidsManager private oldPyramidsManager;
  ...
  function swap(bytes32[] calldata _merkleProof) external {
    require(
      !isClaimed(msg.sender),
      "You have already swapped your allocation."
    );
    claimed[msg.sender] = true;

    uint256[] memory pyramidsOwned = oldPyramidsManager.getPyramidIdsOf(
      msg.sender
    );

    PyramidInfoEntity[] memory pyramids = oldPyramidsManager.getPyramidsByIds(
      pyramidsOwned
    );

    // Calculate amount of PRMD locked in pyramid NFTs
    uint256 pyramidBalance = 0;
    for (uint256 i = 0; i < pyramids.length; i++) {
      pyramidBalance += pyramids[i].pyramid.pyramidValue;
      if (isWhitelisted(msg.sender, _merkleProof)) {
        pyramidBalance += pyramids[i].pendingRewardsGross;
      }
    }

    uint256 oldBalance = oldPrmd.balanceOf(msg.sender);
    // Send old tokens to dead address
    try oldPrmd.transferFrom(msg.sender, DEAD_ADDRESS, oldBalance) {} catch {}
    // Send new tokens to user
    require(
      newPrmd.transfer(msg.sender, oldBalance + pyramidBalance),
      "PRMD transfer to user failed"
    );

    for (uint256 i = 0; i < pyramids.length; i++) {
      // Burn old pyramid NFT
      oldPyramidsManager.transferFrom(msg.sender, DEAD_ADDRESS, pyramids[i].id);
      uint256 value = pyramids[i].pyramid.pyramidValue;
      if (isWhitelisted(msg.sender, _merkleProof)) {
        value += pyramids[i].pendingRewardsGross;
      }

      (, uint256 tier) = pyramidsManager.getTier(
        pyramids[i].pyramid.rewardMult
      );

      pyramidsManager.whitelistCreatePyramidWithTokens(
        pyramids[i].pyramid.name,
        value,
        msg.sender,
        tier
      );
  ...
  function setOldPyramidsManager(IPyramidsManager manager) external onlyOwner {
    require(
      address(manager) != address(0),
      "Pyramid: PyramidsManager is not set"
    );
    oldPyramidsManager = manager;
  }

  function setOldPyramid(IPyramid _prmd) external onlyOwner {
    require(
      address(_prmd) != address(0),
      "PresaleSwap: Old Pyramid is not set"
    );
    oldPrmd = _prmd;
  }
  ...
  function adminMint(
    ...
    require(newPrmd.transfer(_address, _amount), "PRMD transfer failed");
    pyramidsManager.whitelistCreatePyramidWithTokens(
      _name,
      _amount, // 0% unlocked
      _address,
