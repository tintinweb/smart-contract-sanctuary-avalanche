...
AccessControlEnumerableUpgradeable, TokenConstants, ERC721BlacklistUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mintLocked() public nonReentrant whenNotPaused {
        require(lockedMintsRemaining[_msgSender()] > 0, "No locked mints remaining");
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable) returns (string memory) {
        string memory imageURI = string(abi.encodePacked(imageBaseURI, (tokenId % 4).toString()));
        string memory animationURI = string(abi.encodePacked(imageBaseURI, (tokenId % 4).toString(), '.webm'));
        string memory treated = _treated[tokenId] ? 'true' : 'false';
        ...
        bytes memory staffEggAttribute = abi.encodePacked(
            '{',
                '"trait_type": "staff", ',
                '"value": true',
            '}, '
        ...
        bytes memory dataURIAttributes = abi.encodePacked(
            '"attributes": [',
                '{',
                    ...
                    '"value": 0',
                '}, ',
                tokenId <= 50 ? string(staffEggAttribute) : "",
                '{',
                    '"trait_type": "treated", ',
                    '"value": ',  treated,
                '}'
    ...
    */
    ...
    {
        require(tokenId > 50, "Staff eggs are non-transferable");
