...
pragma solidity >=0.7.0 <0.8.0;
import "../superAaveTokenImpl/superAaveAVAXImpl.sol";
import "../superQiTokenImpl/superQiAvaxImpl.sol";
...
contract superSwitchAVAX is baseSuperToken,superAaveAVAXImpl,superQiAvaxImpl{    
    ...
    constructor(address multiSignature,address origin0,address origin1,address _aavaToken,address _qiToken,
        address payable _swapHelper,address payable _feePool,uint8 _lendingSwitch)
            baseSuperToken(multiSignature,origin0,origin1,_swapHelper,_feePool) superAaveAVAXImpl(_aavaToken) superQiAvaxImpl(_qiToken){
    ...
    function onDeposit(address account,uint256 _amount)internal virtual override(superAaveTokenImpl,superQiAvaxImpl,superTokenInterface) returns(uint256){
        if (lendingSwitch == 0){
            return superAaveTokenImpl.onDeposit(account,_amount);
        }else{
            return superQiAvaxImpl.onDeposit(account,_amount);
        }
    }
    function onWithdraw(address account,uint256 _amount)internal virtual override(superAaveTokenImpl,superQiAvaxImpl,superTokenInterface) returns(uint256){
        if (lendingSwitch == 0){
            return superAaveTokenImpl.onWithdraw(account,_amount);
        }else{
            return superQiAvaxImpl.onWithdraw(account,_amount);
        }
    }
    function onCompound() internal virtual override(superAaveTokenImpl,superQiAvaxImpl,superTokenInterface){
        if (lendingSwitch == 0){
            superAaveTokenImpl.onCompound();
        }else{
            superQiAvaxImpl.onCompound();
        }
    }
}
...
//
// This contract handles swapping to and from superToken.
abstract contract superAaveAVAXImpl is superAaveTokenImpl {
    using SafeERC20 for IERC20;
...
abstract contract baseSuperToken is timeLockSetting,superTokenInterface,proxyOwner,ReentrancyGuard{
    ...
    function convertToAssets(uint256 _shareNum) internal view returns(uint256){
    ...
    function _deposit(address from,uint256 _amount, address receiver) internal returns (uint256){
        ...
        // Gets the amount of superToken in existence
        uint256 totalShares = totalSupply();
        _amount = onDeposit(from,_amount);
    ...
    function withdrawETH(uint256 _value,address receiver,address owner) external payable AVAXUnderlying nonReentrant returns (uint256) {
        uint256 _share = convertToShares(_value);
    ...
    function redeemETH(uint256 shares,address receiver,address owner) external payable AVAXUnderlying nonReentrant returns (uint256) {
        uint256 _value = convertToAssets(shares);
    ...
    function flashLoan(
        ...
        onDeposit(address(receiver),amount + fee);
