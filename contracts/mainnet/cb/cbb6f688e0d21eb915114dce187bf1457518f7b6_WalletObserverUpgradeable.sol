    ...
    PlanetsManagerImplementationPointerUpgradeable
{
        ...
        returns (bool)
    {
        ...
        if (!isExcludedFromObserver(from)) {
            ...
            // Ensure last buy isn't 10 blocks ago
            require(
                block.number > _lastBuyOf[from] + 30 || _lastBuyOf[from] == 0,
                "WalletObserverUpgradeable: You must either be an arbitrage or front-running bot!"
            );
        }

        if (!isExcludedFromObserver(tx.origin)) {
            if (isBuy) {
                _lastBuyOf[tx.origin] = block.number;
            } else {
                require(
                    block.number > _lastBuyOf[tx.origin] + 30 ||
