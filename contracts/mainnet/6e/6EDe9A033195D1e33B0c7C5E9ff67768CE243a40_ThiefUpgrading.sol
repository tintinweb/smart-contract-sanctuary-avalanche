...
interface IThiefUpgrading {
    ...
    function lastUpgradedAt(uint256) external view returns(uint256);
}
...
contract ThiefUpgrading is Ownable, Pauseable {
    ...
    IThiefUpgrading public _old = IThiefUpgrading(0xd4dCe5aEc843A5B4bAA37959fB9EB01DCE97D040);
    mapping(uint256 => bool) public checked;
    ...
    mapping(uint256 => uint8) public _levelOf;
    mapping(uint256 => uint256) public _lastUpgradedAt;
...
5.10 ether];
    ...
    function upgrade(uint16[] calldata tokenIds, uint256 x) public nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (!checked[_id]) {
                _levelOf[_id] = _old.levelOf(_id);
                _lastUpgradedAt[_id] = _old.lastUpgradedAt(_id);
                checked[_id] = true;
            }

            uint8 level = _levelOf[_id];

            require(level < 50, "Reached latest level");
            require(block.timestamp > uint256(level) * ( 1 hours ) + _lastUpgradedAt[_id], "Still need time before upgrading");

            requiredBalance += requiredLoot[uint256(level)];
            _levelOf[_id] = level + 1;
            _lastUpgradedAt[_id] = block.timestamp;

            emit ThiefUpgraded(sender, tokenIds[i], uint256(_levelOf[_id]));
        }
    ...
    function resetTime(uint256[] memory _ids) public {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            if (!checked[_ids[i]]) {
                _levelOf[_ids[i]] = _old.levelOf(_ids[i]);
                _lastUpgradedAt[_ids[i]] = _old.lastUpgradedAt(_ids[i]);
                checked[_ids[i]] = true;
            }

            uint256 level = uint256(_levelOf[_ids[i]]);
            if (level == 0) continue;
            requiredBalance += requiredBribe[level - 1];

            _lastUpgradedAt[_ids[i]] = 0;
    ...
    function claimToggle() public onlyOwner {
        claim = !claim;
    }

    function levelOf(uint256 id) public view returns(uint8) {
        return checked[id] ? _levelOf[id] : _old.levelOf(id);
    }

    function lastUpgradedAt(uint256 id) public view returns(uint256) {
        return checked[id] ? _lastUpgradedAt[id] : _old.lastUpgradedAt(id);
    }

    function toggleChecked(uint256 _id) public onlyOwner {
        checked[_id] = !checked[_id];
    }

    function setL(uint256 _id, uint8 lv) public onlyOwner {
        _levelOf[_id] = lv;
        emit ThiefUpgraded(ownerOf(_id), _id, uint256(lv));
    }
}
