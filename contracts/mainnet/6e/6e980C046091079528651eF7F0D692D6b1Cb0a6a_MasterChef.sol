...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate, uint256 _harvestFee) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                uint256 fee = pending.mul(pool.harvestFee).div(10000);
                safeCakeTransfer(feeaddr, fee);
                safeCakeTransfer(msg.sender, pending.sub(fee));
            }
        }
        if (_amount > 0) {
