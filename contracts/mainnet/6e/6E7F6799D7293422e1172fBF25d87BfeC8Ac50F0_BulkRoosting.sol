...
import "./EggV2.sol";
import "./RoostrV2.sol";
import "./RoostingRoostrV2.sol";
import "hardhat/console.sol";

contract BulkRoosting is Authorizable {
    using SafeMath for uint256;

    address public EGG_CONTRACT;
    address public ROOSTR_CONTRACT;
    address public ROOSTINGROOSTR_CONTRACT;

    mapping(address => uint256) public userCredit;

    event DepositEgg(address indexed user, uint256 amount);
    event WithdrawEgg(address indexed user, uint256 amount);

    constructor(
        address _roostrContract,
        address _eggContract,
        address _roostingRoostrV2Contract
    ) {
        EGG_CONTRACT = _eggContract;
        ROOSTR_CONTRACT = _roostrContract;
        ROOSTINGROOSTR_CONTRACT = _roostingRoostrV2Contract;
    }
    ...
    function deposit(uint256 amount) external {
        ...
        bool success = eggContract.transferFrom(
            msg.sender,
            address(this),
            amount
        );
        require(success, "Unable to transfer eggs");
        userCredit[msg.sender] += amount;
        emit DepositEgg(msg.sender, amount);
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        EggV2 eggContract = EggV2(EGG_CONTRACT);
        bool success = eggContract.transfer(msg.sender, amount);
        require(success, "Unable to transfer eggs");
        userCredit[msg.sender] -= amount;
        emit WithdrawEgg(msg.sender, amount);
    }

    function calculateRequiredAmount(uint256 tokenId)
        public
        view
        returns (uint256)
    {
        RoostrRootingV2 y = RoostrRootingV2(ROOSTINGROOSTR_CONTRACT);
        (uint24 kg, , , , uint48 eatenAmount, ) = y.stakedRoostr(tokenId);
        require(kg > 0, "NOT STAKED");
        uint24 eggBurnCap = y.EGG_BURN_CAP();
        uint48 requiredAmount = 0;
        if (kg > eggBurnCap) {
            requiredAmount = uint48(eggBurnCap) - eatenAmount;
        } else {
            requiredAmount = uint48(kg) - eatenAmount;
        }
        return uint256(requiredAmount) * 1e18;
    }

    function calculateTotalRequiredAmount(uint256[] calldata tokenIds)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 requiredAmount = calculateRequiredAmount(tokenIds[i]);
    ...
    /**
     * Feed all roostr owned by the message sender to next level, user will only send staked roostr
     */
    function feedAllRoostr(uint256[] calldata tokenIds) external {
        RoostrRootingV2 y = RoostrRootingV2(ROOSTINGROOSTR_CONTRACT);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 requiredAmount = calculateRequiredAmount(tokenIds[i]);
            if (requiredAmount > 0) {
                require(
                    userCredit[msg.sender] >= requiredAmount,
                    "NOT ENOUGH EGG"
                );
                y.feedRoostr(tokenIds[i], requiredAmount);
                userCredit[msg.sender] -= requiredAmount;
            }
        }
    }
    ...
    /**
     * Level up all staked roostr
     */
    function levelUpAllRoostr(uint256[] calldata tokenIds) external {
        RoostrRootingV2 y = RoostrRootingV2(ROOSTINGROOSTR_CONTRACT);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            y.levelUpRoostr(tokenIds[i]);
        }
    }

    function feedAllRoostrAndLevelup(uint256[] calldata tokenIds) external {
        RoostrRootingV2 y = RoostrRootingV2(ROOSTINGROOSTR_CONTRACT);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 requiredAmount = calculateRequiredAmount(tokenIds[i]);
            if (requiredAmount > 0) {
            ...
            // if is ready to be level up then level up, if not, just skip it
            ( , , , , , uint32 cooldownTs) = y.stakedRoostr(tokenIds[i]);
            if (block.timestamp >= cooldownTs) {
                y.levelUpRoostr(tokenIds[i]);
            }
        }
    }
...
// IMPOTANT!!!!! DO NOT USE THIS IS MAINET
// NOTE: this is use for testing, cut down version due to file size
...
contract RoostrV2 is ERC721Upgradeable {
    address private _owner;
    address private _royaltiesAddr;
    uint256 public royaltyPercentage;
    uint24 public constant maxSupply = 12000;
    uint24 public publicMintCounter;
    uint24 public wlMintCounter;
    ...
    mapping(uint256 => Item) public collectible;
    ...
    event LevelUp(uint256 _tokenId, uint256 _newKg, uint256 ts);

    // modifiers
    mapping(address => bool) public authorized;

    modifier onlyAuthorized() {
        require(authorized[msg.sender] || msg.sender == _owner, "NA");
        _;
    }
    modifier onlyOwner() {
    ...
    function initialize(
        ...
        bool _openForPublic,
        uint24 _pubMintCounter,
        uint24 _wlMintCounter
    ) public initializer {
        __ERC721_init("Roostr", "Roostr");
        royaltyPercentage = _royaltyPercentage;
        _owner = _msgSender();
        _royaltiesAddr = _royaltyReceiver;
        mintFee = _mintFeeAmount;
        baseURL = _baseURL;
        openForPublic = _openForPublic;
        publicMintCounter = _pubMintCounter;
        wlMintCounter = _wlMintCounter;
        maxMint = 2;
    }

    function _mintinternal(uint256 numberOfToken, address to) internal {
        for (uint256 i = 1; i <= numberOfToken; i++) {
            unchecked {
                publicMintCounter++;
            }
            _safeMint(to, publicMintCounter);
            Item memory newCollectibleItem = Item(
                uint24(publicMintCounter),
                0, // numOfTransfer
                1, // kg
                to, //mintedBy
                to, //currOwner
            ...
            collectible[publicMintCounter] = newCollectibleItem;
        }
    }

    // mint with avax
    // function mint(uint256 amt) public payable {
    //     require(openForPublic == true, "NO1");
    //     require(msg.sender != address(0));
    //     require(publicMintCounter + amt <= maxSupply, "SUP");
    //     require(amt > 0, "MIN");
    //     require(amt <= maxMint, "MAX");
    //     uint256 price = mintFee * amt;
    //     require(msg.value >= price, "FEE");
    //     payable(_royaltiesAddr).transfer(msg.value);
    //     _mintinternal(amt, msg.sender);
    // }
    ...
    function setPriceForSale(
        ...
        require(_exists(_tokenId), "NE");
        address tokenOwner = ownerOf(_tokenId);
        require(tokenOwner == msg.sender, "NO");
        Item memory roostr = collectible[_tokenId];
        roostr.price = _newPrice;
        roostr.forSale = isForSale;
        collectible[_tokenId] = roostr;
        emit SaleToggle(_tokenId, isForSale, _newPrice, block.timestamp);
    }

    function buyToken(uint256 _tokenId) public payable {
        address tokenOwner = ownerOf(_tokenId);
        require(tokenOwner != address(0), "AD0");
        require(tokenOwner != msg.sender, "OWNED");
        Item memory roostr = collectible[_tokenId];
        require(msg.value >= roostr.price, "FEE");
        // token should be for sale
        require(roostr.forSale, "SALE");
        ...
        payable(tokenOwner).transfer(payOwnerAmount);
        roostr.prePrice = roostr.price;
        collectible[_tokenId] = roostr;
    ...
    function _beforeTokenTransfer(
        ...
        Item memory roostr = collectible[tokenId];
        roostr.currOwner = to;
        roostr.numOfTransfer += 1;
        roostr.forSale = false;
        collectible[tokenId] = roostr;
        if (from == address(0)) {
    ...
    function setKg(uint256 _tokenId, uint256 _newKg) external onlyAuthorized {
        Item memory roostr = collectible[_tokenId];
        roostr.kg = uint24(_newKg);
        collectible[_tokenId] = roostr;
        emit LevelUp(_tokenId, _newKg, block.timestamp);
    }
    ...
    // // authorized function
    function mintWL(address to, uint256 tokenId) external onlyAuthorized {
        require(_exists(tokenId) == false, "MINTED");
        require(tokenId <= 4000, "OUT OF RANGE");
        unchecked {
            wlMintCounter++;
        }
        ...
        collectible[tokenId] = newCollectibleItem;
    }

    // allow airdrop token to address
    function airdrop(uint256 numberOfToken, address toAddress)
        external
        onlyOwner
    {
        require(numberOfToken + publicMintCounter <= maxSupply, "MAX");
        _mintinternal(numberOfToken, toAddress);
    }
}
...
contract RoostrRootingV2 is Authorizable {
    using SafeMath for uint256;
    address public ROOSTR_CONTRACT;
    address public FERT_CONTRACT;
    address public EGG_CONTRACT;
    address public FEED_CONTRACT; // use for skipping digestion time

    uint256 public LEVELING_BASE = 1 * 1e18; // 1 EGG / KG (Once hit 36kg it stayed at 36 EGG)
    uint24 public EGG_BURN_CAP = 36;
    uint256 public DIGESTION_TIME_BASE = 3600; // roostr digestion period increase 1 hr per kg
    uint256 public SKIP_DIGESTION_BASE = 1000 * 1e18; // 1000 feed per kg
    uint256 public FERT_EMIT_BASE = 6 * 1e18; // 6 FERT EMIT per day base
    uint256 public FERT_EMIT_PER_KG = 2 * 1e18; // increase by 2 per level of kg, start from 2kg onwards
    uint256 public ONE_DAY = 86400;
    bool public openForPublic = false;
    bool public thirdPartyUpdates = false; // if enabled, 3rd parties can [levelup / skipdigest / feed] but not [claimfert / stake / unstake].

    // minimum 1 EGG is required / day in order for FERT to keep producing.
    // the level up also count as egg being use.

    /**
     * Stores staked roostr fields
     */
    struct StakedRoostrObj {
        uint24 kg;
        // when to calculate FERT from (max 20/02/36812, 11:36:16)
        uint32 sinceTs;
        //  last Egg feed time, use for stopping Fert generation
        uint32 sinceEggFeedTs;
        // for the skipCooldown's cooldown (max 20/02/36812, 11:36:16)
        uint32 lastSkippedTs;
        // how much this roostr has been fed (in whole numbers)
        uint48 eatenAmount;
        // cooldown time until level up is allow (per kg)  -- same as digestion time
        uint32 cooldownTs;
    }

    // redundant struct - can't be packed? (max totalKg = 167,772,160,000)
    uint40 public totalkg;
    uint16 public totalStakedRoostr;

    StakedRoostrObj[120001] public stakedRoostr;

    // Events
    event FertMinted(address owner, uint256 fertAmt, uint256 ts);
    event Staked(uint256 tid, uint256 ts);
    event UnStaked(uint256 tid, uint256 ts);
    ...
    constructor(
        address _roostrContract,
        address _fertContract,
        address _eggContract,
        address _feedContract,
        bool _openForPublic
    ) {
        ROOSTR_CONTRACT = _roostrContract;
        FERT_CONTRACT = _fertContract;
        EGG_CONTRACT = _eggContract;
        FEED_CONTRACT = _feedContract;
        openForPublic = _openForPublic;
    }

    // "READ" Functions

    // how long it take before we can level up in sec
    function digestTimeRate(uint24 kg) public view returns (uint32) {
        // return in seconds
        return uint32((kg) * DIGESTION_TIME_BASE);
    }

    // how many EGG is require to level up based on current kg
    function eggFeedLevelingRate(uint24 kg) public view returns (uint256) {
        if (kg >= EGG_BURN_CAP) {
            return LEVELING_BASE * EGG_BURN_CAP; // max out at 36 egg burn
        }
        return LEVELING_BASE * kg;
    }

    // how many FEED is require to skip the digestion time
    function skipDigestionTimeRate(uint24 kg) public view returns (uint256) {
        return kg * SKIP_DIGESTION_BASE;
    }

    // Staking Functions

    // stake roostr, get all detail for roostr
    function _stake(uint256 tid) internal {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        // verify user is the owner of the roostr...
        require(x.ownerOf(tid) == msg.sender, "NOT OWNER");
        (, , uint24 kg, , , , , ) = x.collectible(tid);
        StakedRoostrObj memory c = stakedRoostr[tid];
        uint32 ts = uint32(block.timestamp);
        if (stakedRoostr[tid].kg == 0) {
            // create staked roostr...
            stakedRoostr[tid] = StakedRoostrObj(
                kg,
                ts, //since
                c.sinceEggFeedTs > 0 ? c.sinceEggFeedTs : ts, // since egg feed ts
                c.lastSkippedTs > 0
                    ? c.lastSkippedTs // last skip time
                    : uint32(ts - ONE_DAY), // can only skip once a day
                c.eatenAmount, // eaten amount (use previous eaten value or 0)
                ts + digestTimeRate(kg) // cool down ts
            );
            totalStakedRoostr += 1;
            totalkg += uint24(kg);
            emit Staked(tid, block.timestamp);
    ...
    /**
     * Calculates the amount of fert that is claimable from a Roostr.
     */
    function claimableView(uint256 tokenId) public view returns (uint256) {
        StakedRoostrObj memory c = stakedRoostr[tokenId];
        if (c.kg == 0) return 0;

        // calculate delta - since = start counting the delta from this timestamp
        if(c.sinceTs > c.sinceEggFeedTs && c.sinceTs - c.sinceEggFeedTs > 86400){
            //if the last claim is more than 1 day from feed, return 0
            return 0;
        }
        uint256 since = max(c.sinceEggFeedTs, c.sinceTs);
        uint256 deltaSeconds = 0;
        if (since < block.timestamp) {
            deltaSeconds = min(86400, block.timestamp - since);
        }
        if (deltaSeconds == 0) return 0;

        uint256 fertPerDay = FERT_EMIT_BASE + (FERT_EMIT_PER_KG * (c.kg - 1));
        return deltaSeconds * fertPerDay / ONE_DAY;
    }

    /**
     * Get all MY staked roostr id
     */

    function myStakedRoostr() public view returns (uint256[] memory) {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        uint256 roostrCount = x.balanceOf(msg.sender);
        uint256[] memory tokenIds = new uint256[](roostrCount);
        uint256 counter = 0;
        for (uint256 i = 0; i < roostrCount; i++) {
    ...
    /**
     * Calculates the TOTAL amount of fert that is claimable from ALL Roostr.
     */
    ...
    /**
     * Claims ferts from the provided Roostr.
     * (MUST BE OWNER)
     */
    function _claimFerts(uint256[] calldata tokenIds) internal {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        Fert f = Fert(FERT_CONTRACT);
        uint256 totalClaimable = 0;
        uint32 ts = uint32(block.timestamp);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(x.ownerOf(tokenIds[i]) == msg.sender, "NOT OWNER");
            StakedRoostrObj memory c = stakedRoostr[tokenIds[i]];
            // we only care about chikn that have been staked (i.e. kg > 0) ...
            if (c.kg > 0) {
                uint256 claimableFert = claimableView(tokenIds[i]);
                if (claimableFert > 0) {
                    totalClaimable = totalClaimable + claimableFert;
                    // reset since, for the next calc...
                    c.sinceTs = ts;
                    // c.sinceEggFeedTs = ts; // reset eggfeed -- should not extend the time
                    stakedRoostr[tokenIds[i]] = c;
                }
            }
        }
        if (totalClaimable > 0) {
            f.mint(msg.sender, totalClaimable); // mint fert
            emit FertMinted(msg.sender, totalClaimable, ts);
        }
    }
     ...
     * Claims Ferts from the provided roostr.
     */
    function claimFert(uint256[] calldata tokenIds) external {
        _claimFerts(tokenIds);
    }
    ...
    /**
     * Unstakes MULTIPLE roostr AND claims the eggs.
     */
    function withdrawAllRoostrAndClaim(uint256[] calldata tids) external {
        _claimFerts(tids);
        _unstakeMultiple(tids);
    }

    /**
     * Gets the remaining digest cooldown in seconds for a Roostr.
     */
    function remainingDigestCooldownSeconds(uint256 tokenId) public view returns (uint256) {
        StakedRoostrObj memory c = stakedRoostr[tokenId];
        if (c.cooldownTs > block.timestamp) {
            return c.cooldownTs - block.timestamp;
        }
        return 0;
    }

    /**
     * Public : all the feeding, skip digest, levelup
     * (THIRDPARTY)
     */
    function levelUpRoostr(uint256 tid) external {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        // if we don't allow third party updates, then verify the user is the owner of the roostr...
        if (!thirdPartyUpdates) {
            require(x.ownerOf(tid) == msg.sender, "NOT OWNER");
        }
        StakedRoostrObj memory c = stakedRoostr[tid];
        require(c.kg > 0, "NOT STAKED");
        require(
            c.eatenAmount >= uint48(eggFeedLevelingRate(c.kg) / 1e18),
            "MORE FOOD REQD"
        );
        // check: cooldown has passed...
        require(block.timestamp >= c.cooldownTs, "COOLDOWN NOT MET");
        // NOTE allow people other than the owner to levelup someone's roostr
        // require(x.ownerOf(tid) == msg.sender, "NOT OWNER");
        uint32 ts = uint32(block.timestamp);
        // we only care about chikn that have been staked (i.e. kg > 0) ...

        _internalClaimFert(tid);
        c.sinceTs = ts;
        // increase kg, reset eaten to 0, update next feed level and cooldown time
        c.kg = c.kg + 1;
        c.eatenAmount = 0;
        c.cooldownTs = ts + digestTimeRate(c.kg);
        // c.sinceEggFeedTs = ts; // should not extend the time
        stakedRoostr[tid] = c;
    ...
    /**
     * Skips the "levelUp" digest time period, in return for burning FEED.
     * (THIRDPARTY)
     */
    function skipDigestTime(uint256 tokenId, uint256 feedAmt) external {
        // if we don't allow third party updates, then verify the user is the owner of the roostr...
        if (!thirdPartyUpdates) {
            RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
            require(x.ownerOf(tokenId) == msg.sender, "NOT OWNER");
        ...
        require(c.kg > 0, "NOT STAKED");
        uint32 ts = uint32(block.timestamp);
        FeedV2 feed = FeedV2(FEED_CONTRACT);
        require(
            feedAmt >= skipDigestionTimeRate(c.kg),
            "NOT ENOUGH FEED TO SKIP"
        );
        require(c.lastSkippedTs + ONE_DAY <= ts, "Only 1 skip per day");
        // check: enough egg in wallet to pay
        uint256 walletBalance = feed.balanceOf(msg.sender);
        require(walletBalance >= feedAmt, "NOT ENOUGH FEED IN WALLET");
        // burn the FEED - send to 0x00dEaD address
        bool success = feed.transferFrom(
            msg.sender,
    ...
    /**
     * Claim the outstanding FERT on a staked Roostr.
     */
    function _internalClaimFert(uint256 tokenId) internal {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        uint256 claimableFert = claimableView(tokenId);
        if (claimableFert > 0) {
            Fert f = Fert(FERT_CONTRACT);
            // reset since, for the next calc...
            // NOTE c.sinceTs = ts; - disabled - must be updated outside function!
            // NOTE ensure FERT goes to the token owner only!
            address owner = x.ownerOf(tokenId);
            f.mint(owner, claimableFert); // mint fert
            emit FertMinted(owner, claimableFert, block.timestamp);
        }
    }

    /**
     * Feeds EGG to a Roostr.
     * (THIRDPARTY)
     */
    function feedRoostr(uint256 tokenId, uint256 feedAmount) public {
        // if we don't allow third party updates, then verify the user is the owner of the roostr...
        if (!thirdPartyUpdates) {
            RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
            require(x.ownerOf(tokenId) == msg.sender, "NOT OWNER");
        ...
        require(c.kg > 0, "NOT STAKED");
        require(feedAmount > 0, "NOTHING TO FEED");
        require(feedAmount >= 1e18, "MIN 1 EGG");
        EggV2 egg = EggV2(EGG_CONTRACT);
        uint256 walletBalance = egg.balanceOf(msg.sender);
        uint32 ts = uint32(block.timestamp);
        require(walletBalance >= feedAmount, "NOT ENOUGH EGG IN WALLET");

        _internalClaimFert(tokenId);
        c.sinceTs = ts;
        c.sinceEggFeedTs = ts;
        egg.burnEggs(msg.sender, feedAmount);
        // update the block time as well as claimable
        c.eatenAmount = uint48(feedAmount / 1e18) + c.eatenAmount;
        stakedRoostr[tokenId] = c;
    }

    /**
     * Feeds all Roostr owned by the message sender.
     */
    function feedAllRoostrMinimumEgg() external {
        RoostrV2 x = RoostrV2(ROOSTR_CONTRACT);
        uint256 cnt = x.balanceOf(msg.sender);
        for (uint256 i = 0; i < cnt; i++) {
            uint256 tokenId = x.tokenOfOwnerByIndex(msg.sender, i);
     ...
     */
    function config(
        uint256 a, // leveling base - 1e18
        uint256 b, // digestion time base = 3600
        uint256 c, // skip digestion base = 1e21
        uint256 d, // fert emit base = 6e18
        uint256 e, // fert emit per kg = 2e18
        uint256 f, // one day 86400
        bool g, // open for public = false
        uint24 h, // egg burn cap = 36
        bool i // third party updates = false
    ) external onlyOwner {
        LEVELING_BASE = a;
        DIGESTION_TIME_BASE = b;
        SKIP_DIGESTION_BASE = c;
        FERT_EMIT_BASE = d;
        FERT_EMIT_PER_KG = e;
        ONE_DAY = f;
        openForPublic = g;
...
contract FeedV2 is ERC20, Authorizable {
    ...
    // egg event
    event MintedEgg(address owner, uint256 numberOfFeed); // not used
    event BurnedEgg(address owner, uint256 numberOfEggs); // not used
