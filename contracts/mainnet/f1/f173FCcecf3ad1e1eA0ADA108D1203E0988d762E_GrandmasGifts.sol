...
contract GrandmasGifts is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string public baseURI = "https://gma-meta.smolrun.repl.co/api/uri/";
    ...
    function mint(uint256 amount) public payable nonReentrant {
        ...
        if (msg.sender != owner()) {
            if (onlyAllowList) {
                require(allowList[msg.sender] == 1 ||
                        _soul.balanceOf(msg.sender) > 0, "You aren't on Allow List!");
            }    
    ...
    function updateSmolrun(address _newAddress) public onlyOwner {
        _smolrun = _newAddress;
    }

    function updateSociety(address _newAddress) public onlyOwner {
        _society = _newAddress;
    }
    ...
    function claimAVAX() public {
        ...
        payable(_society).transfer(address(this).balance);
    }
    ...
    function claimWAVAX() public {
        ...
        if (balance > 0) {
            ...
            wavax.transfer(_society, wavax.balanceOf(address(this)));
        }
    }
