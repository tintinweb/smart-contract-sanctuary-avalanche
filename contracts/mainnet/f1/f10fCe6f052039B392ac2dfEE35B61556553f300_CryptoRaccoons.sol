...
contract CryptoRaccoons is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, IERC2981, ERC165Storage {
  ...
  constructor() ERC721("Crypto Raccoons", "CR") {
    mintCount = 0;
    price = 0.5 * 10**18;
    supply = 30;
    royaltyFee = 7;
  ...
  function mint() external payable {
    ...
    require(msg.value == price, "You are not sending the necessary amount");
    _doMint(1, msg.sender, MintType.PUBLIC);
    payable(paymentAddress).transfer(msg.value);
  }

  function adminMint(uint256 quantity, address to) external onlyOwner {
    _doMint(quantity, to, MintType.ADMIN);
  }
  ...
  function changeMintingStatus(bool value) external onlyOwner {
    mintingEnabled = value;
    emit BoolPropertyChange("mintingEnabled", mintingEnabled);
  }

  function changeSupply(uint256 value) external onlyOwner {
    require(value >= mintCount, "supply must be >= mintCount");
    supply = value;
    emit UintPropertyChange("supply", value);
  }

  function changePrice(uint256 value) external onlyOwner {
    price = value;
    emit UintPropertyChange("price", value);
  }

  function changeRoyaltyFee(uint256 value) external onlyOwner {
    royaltyFee = value;
    emit UintPropertyChange("royaltyFee", value);
  }

  function changeBaseURI(string calldata value) external onlyOwner {
    baseURI = value;
    emit UpdateBaseURI(value);
  }

  function changeTokenURI(uint256 id, string calldata dotJson) external onlyOwner {
