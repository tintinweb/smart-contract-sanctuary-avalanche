...
interface IStrategyYak {
   ...
   function deposit(address user, address[] memory pathTokenInToToken0, bytes memory data) external returns (uint256);
    ...
    function withdraw(address user, uint256 _amountLP, address[] memory token0toTokenOut, address[] memory _unused, bytes memory data) external returns (uint256);
...
contract StrategyYak is Ownable, Pausable, IStrategyYak {
    ...
    ///@notice Address of JoeTrader Router V2
    IJoeTraderRouter public joeV1Router;
    ...
    constructor(address _controller, address _router, address _lbRouter, address _joeV1Router) {
        controller = _controller;
        router = _router;
        lbRouter =  ILBRouter(_lbRouter);
        joeV1Router = IJoeTraderRouter(_joeV1Router);
        IERC20(WANT).safeApprove(YAK, type(uint256).max);
        IERC20(WANT).safeApprove(address(lbRouter), type(uint256).max);
    }
    ...
    function deposit(
        ...
        if (pathTokenInToWant[0] != WANT) {
            ...
            _approveTokenIfNeeded(pathTokenInToWant[0], address(lbRouter));
        ...
        uint256 balanceBefore = IERC20(YAK).balanceOf(address(this));
       
        IYak(YAK).deposit(wantBal);

        uint256 amountMinted = IERC20(YAK).balanceOf(address(this)) - balanceBefore;

        //Check if not autocompound or migration
        if (user != address(this)) {
            if (totalAllocation == 0) {
                userAllocation[user] += amountMinted;
                totalAllocation += amountMinted;
                isActive = true;
            } else {
                uint256 shares = amountMinted * totalAllocation / balanceBefore;
...
/**
    ...
    function withdraw(
        ...
        uint256 amountYakToken = IERC20(YAK).balanceOf(address(this)) * shares / totalAllocation; 

        IYak(YAK).withdraw(amountYakToken);
        ...
        emit Withdrawn(shares, tokenOutAmount);
    }

    function withdrawAll() external {
        uint256 amountAll = IERC20(YAK).balanceOf(address(this));
        withdraw(address(this), amountAll, pathWantToBase, zeroArray, "");
    }
    ...
    function completeMigration(address[] memory users, uint256[] memory allocations, bool finalized) external onlyOwnerOrController {
        ...
        totalAllocation = IERC20(YAK).balanceOf(address(this));
     ...
     * @param _unused Path to output token.
     */
    function quotePotentialWithdraw(uint256 shares, address[] calldata token0toTokenOut, address[] calldata _unused) public view returns(uint256 amountOut) {
        _unused;

        uint256 strBalance = IERC20(YAK).balanceOf(address(this));
        uint256 usrBalance = strBalance * shares / totalAllocation;
        
        uint256 amountWant = IYak(YAK).getDepositTokensForShares(usrBalance);

        amountOut = 0;

        if (token0toTokenOut.length > 1) {
            amountOut += joeV1Router.getAmountsOut(amountWant, token0toTokenOut)[token0toTokenOut.length - 1];
        } else {
    ...
    function _getBins(address[] memory path) private view returns (uint256[] memory bins) {
        address factory = ILBRouter(lbRouter).factory();
