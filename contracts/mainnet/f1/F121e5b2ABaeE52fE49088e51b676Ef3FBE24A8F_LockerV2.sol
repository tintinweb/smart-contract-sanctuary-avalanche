...
contract LockerV2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function setZjoe(address _zjoe) external onlyOwner {
        zjoe = _zjoe;
    }

    function addZjoeRewards(address _rewardToken) external onlyOwner {
        require(_rewardToken != address(0) && _rewardToken != address(VTX), "Bad input");
        zjoeRewards.push(_rewardToken);
    }
    ...
    function stakeZjoe(uint256 _amount) public {
        IERC20(zjoe).approve(masterchief, _amount);
        IMasterChefVTX(masterchief).deposit(zjoe, _amount);
        _handleZjoeRewards();
    }

    function sendAndStakeZjoe(uint256 _amount) external {
        IERC20(zjoe).safeTransferFrom(msg.sender, address(this), _amount);
        stakeZjoe(_amount);
    }
    ...
    function _harvestZjoe() internal whenNotPaused {
        IMasterChefVTX(masterchief).deposit(zjoe, 0);
        _handleZjoeRewards();
    }

    function _handleZjoeRewards() internal whenNotPaused {
        uint256 length = zjoeRewards.length;
        for (uint256 i; i < length; i++) {
            address token = zjoeRewards[i];
            _queueRewards(token, IERC20(token).balanceOf(address(this)));
        }
    }
    ...
    function getZjoeStaked() external view returns (uint256) {
        return IMasterChefVTX(masterchief).depositInfo(zjoe, address(this));
    }
