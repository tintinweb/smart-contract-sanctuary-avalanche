...
contract UpgradableStakingPie is
    Ownable,
    ...
    Initializable
{
    ...
    ITokenERC721 public PUMPSKIN_CONTRACT;
    IBMITokenERC20 public PIE_CONTRACT;
    IBMITokenERC20 public POTION_CONTRACT;

    uint256 public MAX_POTION_SUPPLY;
    uint256 public BOOSTER_MULTIPLIER;
    uint256 public POTION_FARMING_FACTOR;
    uint256 public POTION_SWAP_FACTOR;
    uint256 public PIE_DEPOSITE_BOOSTER;
    uint256 public PIE_DEPOSITE_BOOSTER_DECIMALS;
    ...
    function initialize(
        ...
        uint256 max_potion_supply,
        uint256 booster_multiplier,
        uint256 potion_farming_factor,
        uint256 potion_swap_factor,
        uint256 pie_deposite_booster,
        uint256 pie_deposite_booster_decimals
        ...
        // uint256 public PIE_DEPOSITE_BOOSTER = 1; // swap pie for potion ratio
        // uint256 public PIE_DEPOSITE_BOOSTER_DECIMALS = 4; // swap pie for potion ratio

        MAX_POTION_SUPPLY = max_potion_supply;
        BOOSTER_MULTIPLIER = booster_multiplier;
        POTION_FARMING_FACTOR = potion_farming_factor;
        POTION_SWAP_FACTOR = potion_swap_factor;
        PIE_DEPOSITE_BOOSTER = pie_deposite_booster;
        PIE_DEPOSITE_BOOSTER_DECIMALS = pie_deposite_booster_decimals;

        PUMPSKIN_CONTRACT = ITokenERC721(_pumpskinContract);
        PIE_CONTRACT = IBMITokenERC20(_pieContract);
        POTION_CONTRACT = IBMITokenERC20(_potionContract);
    }
    ...
    function staking(uint256 amount) external {
        ...
        uint256 boost = PIE_DEPOSITE_BOOSTER / PIE_DEPOSITE_BOOSTER_DECIMALS;
        uint256 bonusOnDeposite = amount * boost;
        uint256 fullAmount = amount + bonusOnDeposite;
        ...
        if (existingPie.amount > 0) {
            ...
            emit Minted(msg.sender, fullAmount);
            _upsertPieStaking(msg.sender, existingPie.amount + fullAmount);
        } else {
            // no pie staked just update staking
            _upsertPieStaking(msg.sender, fullAmount);
        }
        PIE_CONTRACT.burnFrom(msg.sender, amount);
        emit StakedPie(msg.sender, fullAmount);
    }
    ...
    function withdrawAllPieAndClaimpotion() external {
        ...
        if (pie.amount > 0) {
            ...
            IBMITokenERC20 pieContract = PIE_CONTRACT;
    ...
    function swapPieForPotion(uint256 pieAmt) external {
        ...
        IBMITokenERC20 pieContract = IBMITokenERC20(PIE_CONTRACT);
     ...
     */
    function updateBoosterMultiplier(
        uint256 _value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function updatepotionSwapFactor(
        uint256 _value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function updateMaxpotionSupply(
        uint256 _value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        MAX_POTION_SUPPLY = _value;
    }

    function updatePieDepositeBooster(
        uint256 _value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        PIE_DEPOSITE_BOOSTER = _value;
    }
    ...
    function updateBoosterDecimal(
        uint256 _value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        PIE_DEPOSITE_BOOSTER_DECIMALS = _value;
    }
}