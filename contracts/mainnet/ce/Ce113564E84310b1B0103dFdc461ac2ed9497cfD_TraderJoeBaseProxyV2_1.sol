...
interface ITraderJoeV2_1 {
    ...
    /// @param path The path of the pairs and versions
...
contract TraderJoeBaseProxyV2_1 is AccessControlEnumerable, Pausable {
    ...
    constructor(address _traderJoe, IERC20 _wavax) {
        ...
        traderJoe = ITraderJoeV2_1(_traderJoe);
        wavax = _wavax;
    }
    ...
    /// @param path path contain 3 values pairBinSteps: [], versions : [], tokenPath: [] You can get this values from TraderJoeSDK
    /// @param _to The address of the recipient
    /// @param _deadline The deadline of the tx
    function swapExactTokensForTokens(
        uint256 _amountIn,
        uint256 _amountOutMin,
        ITraderJoeV2_1.Path memory path,
        address _to,
        uint256 _deadline
    ) external {
        require(!paused(), "Token swap is not allowed while the contract is on pause");
        IERC20(path.tokenPath[0]).safeTransferFrom(msg.sender, address(this), _amountIn);
        IERC20(path.tokenPath[0]).approve(address(traderJoe), maxApproval);
        uint256 sumMinusFee = _amountIn - ((_amountIn * fee) / 10000);
        traderJoe.swapExactTokensForTokens(sumMinusFee, _amountOutMin, path, _to, _deadline);
    }
    ...
    function swapTokensForExactTokens(
        ...
        require(!paused(), "Token swap is not allowed while the contract is on pause");
        uint256 tokenBalanceBeforeTransferUsersMoney = getContractTokenBalance(path.tokenPath[0]);
        IERC20(path.tokenPath[0]).safeTransferFrom(msg.sender, address(this), _amountInMax);
        IERC20(path.tokenPath[0]).approve(address(traderJoe), maxApproval);
        ...
        traderJoe.swapTokensForExactTokens(_amountOut, sumToTraderJoe, path, _to, _deadline);
        uint256 tokenBalanceAfterSwap = getContractTokenBalance(path.tokenPath[0]);
        ...
        IERC20(path.tokenPath[0]).transfer(msg.sender, notSpent); // need security check
    }
    ...
    function swapExactTokensForNATIVE(
        ...
        IERC20(path.tokenPath[0]).approve(address(traderJoe), maxApproval);
        uint256 sumMinusFee = _amountIn - ((_amountIn * fee) / 10000);
        traderJoe.swapExactTokensForNATIVE(sumMinusFee, _amountOutMinAVAX, path, payable(_to), _deadline);
    }
    ...
    function swapTokensForExactNATIVE(
        ...
        uint256 feeFromFullSumm = (_amountInMax * fee) / 10000;
        uint256 sumToTraderJoe = _amountInMax - feeFromFullSumm;
        IERC20(path.tokenPath[0]).approve(address(traderJoe), sumToTraderJoe);

        traderJoe.swapTokensForExactNATIVE(_amountAVAXOut, sumToTraderJoe, path, payable(_to), _deadline);
        uint256 tokenBalanceAfterSwap = getContractTokenBalance(path.tokenPath[0]);
        uint256 spentPlusEveryFinFee = sumToTraderJoe - (tokenBalanceAfterSwap - tokenBalanceBeforeTransferUsersMoney - feeFromFullSumm);
        uint256 notSpent = sumToTraderJoe - spentPlusEveryFinFee;
        IERC20(path.tokenPath[0]).transfer(msg.sender, notSpent);
    }
    ...
    function swapExactNATIVEForTokens(
        ...
        traderJoe.swapExactNATIVEForTokens{ value: sumMinusFee }(_amountOutMin, path, _to, _deadline);
    }
    ...
    function swapNATIVEForExactTokens(
        ...
        require(!paused(), "Token swap is not allowed while the contract is on pause");
        if (path.tokenPath[0] != IERC20(wavax)) revert("Acept only wavax");
        uint256 sumMinusFee = msg.value - ((msg.value * fee) / 10000);
        traderJoe.swapNATIVEForExactTokens{ value: sumMinusFee }(_amountOut, path, _to, _deadline);
