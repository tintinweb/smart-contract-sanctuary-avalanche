...
contract WandInvestments is ReentrancyGuard, Ownable {
    ...
    function setCirculatingSupplyXDaysToPrevious(uint256 dInArray) private {
        if (!setCircSupplyToPreviousDay[dInArray] && dInArray > 0) {
            setCircSupplyToPreviousDay[dInArray] = true;
            circulatingSupplyXDays[dInArray] = circulatingSupplyXDays[dInArray - 1];
        }
    }
    ...
    function getCircSupplyXDays() public view returns (uint256) {
        ...
        return circulatingSupplyXDays[daySinceLaunched - numdays];
    }

    function getBTONBackingPrice() public view returns (uint256) {
        // TODO(KAMA): Backing price if total supply is 0?
    ...
    function calcSPTRData() private {
        ...
        // TODO(KAMA): Backing price if totalSupply is 0?
    ...
    function addStable(string memory ticker, address addr, uint256 dec) public onlyOwner {
