...
contract PenguinEmperor is OwnableInitialized {
    ...
    //address[4] memory _addressParameters = [TOKEN_TO_PLAY, NEST_ALLOCATOR, PENGUIN_DATABASE, owner, TOKEN_TO_BUY_POTIONS]
    ...
    constructor (
        ...
        address[5] memory _addressParameters,
        ...
        require(_addressParameters[0] != address(0) && _addressParameters[1] != address(0)
            && _addressParameters[2] != address(0) && _addressParameters[3] != address(0)
            && _addressParameters[4] != address(0), 
            "bad address input");
        ...
        for(i=0; i<_SUPPORTED_BETTING_TOKENS.length; i++){
    ...
    function stealCrownAndPoison(uint256 amount, address paidByToken) external canBetWithToken(paidByToken) stealCrownCheck()  returns (bool bidFailed) {
        //copy current emperor to memory for savings on repeatedly checking storage slot
        PlayerAndBid memory emperor = currentEmperorAndBid;
        ...
        uint256 totalAmount = amount + currentPoisonCost;
        // calculate paid amount and make transfer from player
        uint256 paidAmount = totalAmount * exchangeRateOfPlayingToken(paidByToken) / 1e18;
        IERC20(paidByToken).transferFrom(msg.sender, address(this), paidAmount);
        //convert paid token to TOKEN_TO_PLAY token (result will be ~totalAmount)
        _convertToPlayingToken(paidByToken, paidAmount);
        ...
        totalNestPendingDistribution += ((currentPoisonCost * poisonSplitToNest) / MAX_BIPS);
