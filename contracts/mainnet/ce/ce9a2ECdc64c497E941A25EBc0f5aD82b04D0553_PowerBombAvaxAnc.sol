...
contract PowerBombAvaxAnc is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    IERC20Upgradeable constant UST = IERC20Upgradeable(0xb599c3590F42f8F995ECfa0f85D2980B76862fc1);
    IERC20Upgradeable constant WAVAX = IERC20Upgradeable(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
    IERC20Upgradeable constant lpToken = IERC20Upgradeable(0xaB9A04808167C170A9EC4f8a87a0cD781ebcd55e); // aUST
    IERC20Upgradeable public rewardToken;

    IPool constant pool = IPool(0x95aE712C309D33de0250Edd0C2d7Cb1ceAFD4550); // Anchor
    ...
    struct User {
        ...
        uint rewardStartAt;
        uint depositTime;
        uint pendingWithdrawAmt;
    }
    mapping(address => User) public userInfo;
    mapping(address => uint) internal depositedBlock;

    // Newly added variable after deployed
    uint public depositFeePerc;
    uint public withdrawFeePerc;
    ...
    event SetDepositFeePerc(uint oldDepositFeePerc, uint newDepositFeePerc);
    ...
    function initialize(IERC20Upgradeable _rewardToken, address _treasury, address _admin, address _helper) external virtual initializer {
        __Ownable_init();
        ...
        lpToken.safeApprove(helper, type(uint).max);
    ...
    /// @notice A complete withdrawal consists of 3 function:
    /// @notice withdraw -> executeWithdraw(helper SC) -> repayWithdraw(helper SC)
    /// @notice This function initialize a withdrawal and bot receive event emitted by this
    /// @notice Call by depositor 
    /// @dev Third parameter is reserved for slippage
    function withdraw(IERC20Upgradeable token, uint amount, uint) external virtual nonReentrant {
        ...
        // 1% fee if withdraw within 1 week
        // Deactivate this function first for testing purpose
        // if (user.depositTime + 604800 < block.timestamp) {
        //     amount -= amount * 100 / 10000;
        // }

        user.pendingWithdrawAmt += amount;
        emit Withdraw(address(token), amount, msg.sender);
    }

    /// @notice There is no "harvest" function in Anchor, rewards is accumulated in UST
    /// @notice Admin calculate extra UST and redeem from Anchor SC
    function initializeHarvest(uint aUSTAmtToRedeem) external virtual {
    ...
    function harvest() public virtual {
        ...
        if (USTAmt != 0) {
            uint WAVAXAmt = swap2(address(UST), address(WAVAX), USTAmt, 0);
            ...
            if (unclaimedRewardsAmt > 1e17) { // Approximately $9.0 @ 6 Apr 2022
            ...
            // Deposit reward token into Geist to get interest bearing gToken
            ...
            emit Harvest(USTAmt, rewardTokenAmt, fee);
        }
    }
    ...
    function setFeePerc(uint _depositFeePerc, uint _withdrawFeePerc, uint _yieldFeePerc) external virtual onlyOwner {
        require(
            _depositFeePerc <= 1000 &&
            _withdrawFeePerc <= 1000 &&
            _yieldFeePerc <= 2000,
            "Invalid fee percentage"
        );

        if (depositFeePerc != _depositFeePerc) {
            uint oldDepositFeePerc = depositFeePerc;
            depositFeePerc = _depositFeePerc;
            emit SetDepositFeePerc(oldDepositFeePerc, _depositFeePerc);
        }
        if (withdrawFeePerc != _withdrawFeePerc) {
            uint oldWithdrawFeePerc = withdrawFeePerc;
            withdrawFeePerc = _withdrawFeePerc;
            emit SetWithdrawFeePerc(oldWithdrawFeePerc, _withdrawFeePerc);
        }
        if (yieldFeePerc != _yieldFeePerc) {
    ...
    function resetPendingWithdrawAmt(address account) external virtual {
        require(msg.sender == helper, "Only helper");
        userInfo[account].pendingWithdrawAmt = 0;
    }
