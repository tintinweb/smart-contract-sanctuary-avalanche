...
//For Worm and Feed Rewards 0x85fdca3741395b7f9faa70bf7cc40f4ac89127ff
interface IFarmlandLPV3 {

    function burnFert(uint256 farmlandID, uint256 fertAmt) external;

    function TVLLP() external view returns (uint256 lockedValue);

    function baseCapacity() external view returns (uint256 capacity);

    function claimableView(address publicAddress) external view returns (uint256 amountToClaim);

    function checkActiveFarm() external view returns (bool);
    
    function fertilityMultiplier(uint256 farmlandId) external view returns (uint256 multiplier);

    function lpStaked(address publicAddress) external view returns (uint256 amountStaked);

    function activateFarm(uint256 farmlandID) external;

    function deactivateFarm(uint256 farmlandID) external;
    ...
    function withdraw(uint256 amtOfLP) external;

    function staking(uint256 amtOfLP) external;
...
interface IRoostrNFT {
    ...
    // map id to ChickenRun obj
    function allRoostrRun(uint key) external view returns(RoostrRun memory);
...
interface IRoostrRoosting {   
    ...
    // map id to ChickenRun obj
    function allRoostrRun(uint key) external view returns(StakedRoostrObj memory);

    function EGG_BURN_CAP() external view returns (uint24);

    function myStakedRoostr() external view returns (uint256[] memory);
    ...
    function approveFeedToSkip() external;

    function skipDigestTime(uint256 tid, uint256 feedAmt) external;  

    function SKIP_DIGESTION_BASE() external view returns(uint256);

    function FERT_EMIT_PER_KG() external view returns(uint256); 

    function LEVELING_BASE() external view returns(uint256);

    function DIGESTION_TIME_BASE() external view returns(uint256);

    function FERT_EMIT_BASE() external view returns(uint256);

    function ONE_DAY() external view returns(uint256);

    function eggFeedLevelingRate() external view returns(uint256);

    function skipDigestionTimeRate(uint256 kg) external view returns (uint256);

    function digestTimeRate() external view returns (uint256);

    function levelUpRoostr(uint256 tid) external;

    function totalKg() external returns (uint256);

    function totalStakedRoostr() external returns(uint256);

    function feedRoostr(uint256 tid, uint256 amt) external;

    function feedAllRoostrMinimumEgg() external;

    function claimFert(uint256[] calldata tids) external; 

    function remainingDigestCooldownSeconds(uint256 tid) external view returns (uint256);
...
contract ChiknKeeperV2 is Ownable, IERC721Receiver {
    ...
    //Set to true to allow community care for chikns
    ...
    function baseHolderEggs() public view returns (uint256) {
        ...
        //Deposit chikns to allow use of interface functions below
        //Any one can deposit but once they do this the chikn is in control of the contract owners.
