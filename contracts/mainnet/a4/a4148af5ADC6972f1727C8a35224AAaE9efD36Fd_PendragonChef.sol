...
contract PendragonChef is Ownable {
    using SafeERC20 for IERC20;

    address public PLATFORM_ADDRESS = 0x19A566ea45A3b9fA033101F0487187aBBa01903F;
    ...
    IPendragonStaking public rewardMinter;
    ...
    function deposit(
		...
		uint256 amountRealized = amount;
		
            if (pool.protocolFeeBP > 0) {
                uint256 protocolFee = (amount * pool.protocolFeeBP) / 10000;
                lpToken[pid].safeTransfer(PLATFORM_ADDRESS, protocolFee);
                amountRealized = amount - protocolFee;
            }

        // Effects
        user.amount = user.amount + amountRealized;
        user.rewardDebt += int256((amountRealized * pool.accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function withdraw(
        ...
        lpToken[pid].safeTransfer(to, amountRealized);
	...
	require(!paused);
        ...
        emit Withdraw(msg.sender, pid, amountRealized, to);
        emit Harvest(msg.sender, pid, _pendingReward);
    }
    ...
    function emergencyWithdraw(uint256 pid, address to) public {
        ...
        lpToken[pid].safeTransfer(to, amountRealized);
        emit EmergencyWithdraw(msg.sender, pid, amountRealized, to);
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lastRewardTime: block.timestamp, accRewardPerShare: 0, protocolFeeBP: _protocolFeeBP}));
        require(poolInfo.length <= MAX_NUM_OF_POOLS, "PendragonChef::add: > MAX_NUM_OF_POOLS");
	...
	require(_protocolFeeBP <= 1000);
	    ...
	    poolInfo[_pid].protocolFeeBP = _protocolFeeBP;
        if (overwrite) {
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) public onlyOwner {
        require(_rewardPerSecond <= MAX_REWARD_PER_SECOND, "PendragonChef::setRewardPerSecond: > MAX_REWARD_PER_SECOND");
        massUpdatePools();
    ...
    function setRewardMinter(IPendragonStaking _rewardMinter) external onlyOwner{
        require(address(_rewardMinter).code.length > 0 && paused, "PendragonChef::setRewardMinter: Cannot be an EOA");
        rewardMinter = _rewardMinter;
    }

    function setPlatformAddress(address _platformAddress) external onlyOwner {
        require(_platformAddress != address(0) && paused, "cannot be Zero Address");
   ...
   function enableFarming() external onlyOwner {
        ...
        uint256 len = poolInfo.length;
        for (uint256 i = 0; i < len; ++i) {
        poolInfo[i].accRewardPerShare = 0;
        poolInfo[i].lastRewardTime = block.timestamp;
        }
    }

    function resetAccumulatedRewards(uint256 _pid) external onlyOwner {
        poolInfo[_pid].accRewardPerShare = 0;
        poolInfo[_pid].lastRewardTime = block.timestamp;
    }
