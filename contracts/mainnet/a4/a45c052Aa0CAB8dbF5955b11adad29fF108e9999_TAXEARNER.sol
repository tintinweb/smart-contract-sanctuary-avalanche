...
telegram : https://t.me/TAXEARNER

 
Rules:
 
If you make the biggest buy (in tokens) you become the TAXEARNER, and collect 5% fees (in AVAX) the same way marketing does. 
If you sell any tokens at all at any point you can never be the TAXEARNER. 
If someone beats your record, they become the TAXEARNER. 
If you sell while the TAXEARNER, the marketing wallet becomes TAXEARNER and the record is reset to 0, this way no one can buy a ton and then sell and keep collecting tax fees.
The TAXEARNER is set via biggest buy in tokens.
...
The point is to reward big buys. There are no punishments other than disallowing sellers to be the TAXEARNER.
*/
...
contract TAXEARNER is IBEP20, Auth {
    using SafeMath for uint256;
 
    string constant _name = "TAXEARNER";
    string constant _symbol = "TAXEARNER";
    ...
    uint256 public _walletMax = 10000; 
    ...
    uint256 public TaxCollectorFee = 5;
    ...
    constructor () Auth(msg.sender) {      
        ...
        autoLiquidityReceiver = 0xAc53dBb4Fac9bCa23F2a6D36e7Dab3CCA19d55f7; //LP receiver
        marketingWallet = 0xAc53dBb4Fac9bCa23F2a6D36e7Dab3CCA19d55f7;  //marketing wallet
        taxCollector = 0xAc53dBb4Fac9bCa23F2a6D36e7Dab3CCA19d55f7;  //tax collector wallet
        totalFee = liquidityFee.add(marketingFee).add(TaxCollectorFee);
	...
	function checkTxLimit(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender != owner
            ...
            if (taxCollector == sender){
                taxCollector = marketingWallet;
                _record = 0;
            }
            hasSold[sender] = true;
        }
    ...
    function setMaxWallet(uint256 _maxWallet) public authorized {
        _walletMax = _maxWallet;
    }
    ...
    function swapBack() internal lockTheSwap {  
        ...
        uint256 amountAVAXTaxCollector = amountAVAX.mul(TaxCollectorFee).div(totalAVAXFee);
        ...
        (bool tmpSuccess2,) = payable(taxCollector).call{value: amountAVAXTaxCollector, gas: 30000}("");
