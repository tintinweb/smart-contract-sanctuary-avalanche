...
contract StakingDoubleRewardsSuper is Ownable {
    ...
    //for test
    function withdrawadmin(uint256 pid, uint256 lp) external onlyOwner {
        pangolinMiniChef.withdraw(pid, lp, msg.sender);
    }
        ...
        if (isSuper) {
            for (uint256 i; i < rewardSuperTokens.length; i++) {
                ...
                if (userPendingRewardsSuper > 0) {
                    ...
                    //emit Harvest(msg.sender, userPendingRewardsSuper);
                }
            }
        }
    }
