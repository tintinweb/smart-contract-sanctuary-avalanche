...
contract AstroToken is ERC20Detailed {
    ...
    */
    ...
    uint256[4] public riskFeeValueFee   = [150, 600, 1000, 800];
    uint256[4] public ecosystemFee      = [375, 600, 750, 1200];
    uint256[4] public burnFee           = [0, 300, 750, 2000];
    ...
    constructor(address _router/*0x10ED43C718714eb63d5aA57B78B54704E256024E*/) ERC20Detailed("Astro Token", "ASTRO", uint8(DECIMALS)) {
        owner = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint8 whaleRatio = uint8(balanceOf(sender) * 100 / getCirculatingSupply());
        ...
        uint256 gonAmountReceived = shouldTakeFee(sender, recipient) ? takeFee(sender, recipient, gonAmount, whaleRatio) : gonAmount;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToEcosystem = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV).sub(amountToTreasury);
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount, uint8 ratio) internal returns (uint256) {
        uint256 _realFee = totalFee[1];
        if(automatedMarketMakerPairs[recipient] && ratio >= 1) {
            if (justBusinessList[msg.sender] == true) _realFee = totalFee[2];
            else _realFee = totalFee[3];
        }
        if(automatedMarketMakerPairs[sender]) {
            _realFee = totalFee[0];
            if (ratio >= 1) {
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _futureEcosystemReceiver) external onlyOwner {
