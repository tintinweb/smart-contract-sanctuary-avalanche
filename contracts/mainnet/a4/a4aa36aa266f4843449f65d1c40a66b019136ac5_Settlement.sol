...
contract Settlement is OrderManager, Signing {
    ...
    bytes32 public constant _PAYLOAD_TYPE_HASH =
        keccak256(
            "Payload(address fromToken,address toToken,uint256 fromAmount,uint256 toAmount,address sender,address recipient,uint32 validFrom,uint32 validTo,address driver,uint8 scheme,Condition condition,Condition[] conditions)Condition(address target,bytes data)"
        );
