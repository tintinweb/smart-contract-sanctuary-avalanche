...
contract Fed is Managed, ManaCooldown, LiteMath, ReentrancyGuard {
    ...
    uint64 public levelUpManaCost = 20;
    uint64 public claimManaCost = 60;
...
contract InterestUtils {
    ...
    function fv(uint256 v, uint256 t, int128 dr, uint256 c) external pure returns (uint256) {
        return uint256(ABDKMath64x64.mulu(
            ABDKMath64x64.pow(
                ABDKMath64x64.add(
                    0x10000000000000000,
                    ABDKMath64x64.div(
                        dr,
                        ABDKMath64x64.divu(c, t)
                    )
                ), c),
                v
        ));
  ...
  function pow (int128 x, uint n) public pure returns (int128 r) {
    ...
    function compound(uint256 principle, int128 rate, int128 periods) external pure returns(uint256){
        uint256 result = ABDKMath64x64.mulu(
                            ABDKMath64x64.pow (
                                ABDKMath64x64.add (
                                0x10000000000000000,
                                rate),
                                ABDKMath64x64.toUInt (periods)), principle);
        return result;
    }
