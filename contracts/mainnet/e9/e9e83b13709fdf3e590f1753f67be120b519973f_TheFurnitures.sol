    ...
    OwnableUpgradeable
{
     ...
     * @param propertyIds property tokenIds of property whose reward will be used to buy furniture if payByReward is true or else 0
     */
    function mint(
        string memory _tokenURI,
        uint256 _furnitureCatogoryIndex,
        ...
        )
    {
        ...
        if (payByReward_) {
            ...
            // Pay the price by reward from property.
            try _property.payByReward(propertyIds, msg.sender, price) returns (
                bool paid,
    ...
    function airDrop(
        ...
        uint256[] memory _furnitureCatogoryIndices,
        uint256[] memory _furnitureCollectionIndices,
        address[] memory users
    ) external onlyOwner {
        // Check for details length
        require(
            _tokenURIs.length == _furnitureCatogoryIndices.length &&
                _tokenURIs.length == _furnitureCollectionIndices.length &&
                _tokenURIs.length == users.length,
            "The Furniture: Length of tokenURIs and categories and collections must be same"
        );

        for (uint256 index = 0; index < _tokenURIs.length; index++) {
            // Chrck for furniture category and collection
            require(
                _furnitureCatogoryIndices[index] < furnitureCatogories.length,
                "The Furnitures: Furniture category doesn't exists."
            );
            require(
                _furnitureCollectionIndices[index] <
                    furnitureCatogoryCollections[
                        _furnitureCatogoryIndices[index]
                    ].length,
                "The Furnitures: Furniture category doesn't exists."
            );
        ...
        returns (uint256, uint256)
    {
        furnitureCollection
            ...
            ][furnitures[tokenId].furnitureCollectionIndex];
     ...
     * @param tokenIds of the furnitures.
    ...
    function allocateToProperty(
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            require(
                ownerOf(tokenIds[index]) == propertyOwner,
                "The Furniture: Only can be added by same property owner "
            );
            // Allocate furniture to proerty.
            furnitures[tokenIds[index]].propertyId = _propertyId;
        }
        return true;
    }
     ...
     * @param tokenIds of the furniture.
     */
        ...
        returns (bool)
    {
        for (uint256 index = 0; index < tokenIds.length; index++) {
            // Dellocate property.
            furnitures[tokenIds[index]].propertyId = 0;
        }
        return true;
    }
    ...
    function initialize() public {
        ...
        require(!upgradeFlag, "The Furniture: Implemntation already upgraded.");
        ...
        for (uint256 index = 0; index < _categories.length; index++) {
            for (uint256 j = 0; j < _subcategories.length; j++) {
                furnitureCatogoryCollections[index][j].price = prices[j];
                furnitureCatogoryCollections[index][j].dailyRewards[0] = dailyRewards[j];
                furnitureCatogoryCollections[index][j].timestamps[0] = block.timestamp;
            }
        }
