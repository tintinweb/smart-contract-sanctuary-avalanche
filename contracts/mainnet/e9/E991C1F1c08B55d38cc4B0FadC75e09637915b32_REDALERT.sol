...
contract REDALERT is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 15000 ether;
    // Initial distribution for the day 2-5 REDALERT-WFTM LP -> REDALERT pool
    uint256 public constant INITIAL_REDALERT_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the REDALERT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("REDALERT", "REDALERT") {
        // Mints 1 REDALERT to contract creator for initial pool setup
    ...
    function _getRedAlertPrice() internal view returns (uint256 _REDALERTPrice) {
        try IOracle(redalertOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("REDALERT: failed to fetch REDALERT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _REDALERTPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_REDALERTPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRedAlertOracle(address _REDALERTOracle) public onlyOperatorOrTaxOffice {
        require(_REDALERTOracle != address(0), "oracle address cannot be 0 address");
        redalertOracle = _REDALERTOracle;
    }
    ...
    /**
     * @notice Operator mints REDALERT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of REDALERT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentREDALERTPrice = _getRedAlertPrice();
            currentTaxRate = _updateTaxRate(currentREDALERTPrice);
            if (currentREDALERTPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _redalertPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_redalertPool != address(0), "!_redalertPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_redalertPool, INITIAL_REDALERT_POOL_DISTRIBUTION);
