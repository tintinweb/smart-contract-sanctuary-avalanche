...
contract NodeManagerV71 is Initializable {
    ...
    function rateClaimFee(address account) public view returns (uint32) {
        uint32 elapsed = 0;
        bytes4 methodLastTimeSell = bytes4(
            keccak256(bytes("lastTimestampSell(address)"))
        );
        (bool success, bytes memory data) = tokenAddress.staticcall(
            abi.encodeWithSelector(methodLastTimeSell, account)
        );
        if (success) {
            uint32 lastTimestampSell = uint32(abi.decode(data, (uint32)));
            if (lastTimestampSell == 0) return claimFee;
            elapsed = uint32(block.timestamp) - lastTimestampSell;
        }
