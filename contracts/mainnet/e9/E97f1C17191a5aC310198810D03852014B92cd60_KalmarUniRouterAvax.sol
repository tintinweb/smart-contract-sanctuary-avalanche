...
// File: contracts/interfaces/IUniswapRouterAvax.sol
...
// File: contracts/routes/KalmarUniRouterAvax.sol
...
contract KalmarUniRouterAvax is IKalmarTradingRoute, Ownable, WhitelistedRole, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapRouterAvax public routers;
    ...
    constructor(
        address _kalmswap,
        IUniswapRouterAvax _routers,
        IERC20[] memory _correspondentTokens,
        IERC20 _wETH
    ) public {
        kalmarswap = IKalmar(_kalmswap);
        routers = _routers;
        ...
        returns(uint256 _destAmount)
    {
        ...
        for (uint256 i = 1; i < path.length.sub(1); i++) {
          ...
          if (src == etherERC20) {
              uint256[] memory amounts = routers.swapExactAVAXForTokens.value(srcAmount)(
                  amountOutMin,
                  path,
                  msg.sender,
                  deadline
              ...
              routers.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
                  srcAmount,
                  amountOutMin,
                  path,
                  msg.sender,
