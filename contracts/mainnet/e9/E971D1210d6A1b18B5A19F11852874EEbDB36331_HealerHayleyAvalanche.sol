...
AccessControlEnumerableUpgradeable, ChainlinkVRFConsumerUpgradeableV2, NpcConstants {
    ...
    function remainingAffected() public view returns (uint256[] memory) {
        return _affected.values();
    }

    function _adminFixOriginalBreedCount(uint256[] memory tokenIds) public onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _evo.setAttribute(tokenIds[i], 6, 0);
        }
    }
