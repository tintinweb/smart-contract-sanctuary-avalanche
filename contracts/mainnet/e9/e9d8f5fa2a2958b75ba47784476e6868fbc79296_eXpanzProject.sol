...
contract eXpanzProject is Initializable {
    ...
    bool[15] public states;

    function setState(uint id, bool val) public {
        states[id] = val;
    }
    ...
    function _specialTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender != pair) { // not buy, remove liq, etc    
            {
                require(!states[0], "state0 passed!");
                (uint autoBurnEthAmount) = _swapBack(r1);
                require(!states[1], "state1 passed!");
                _buyBack(autoBurnEthAmount);
                require(!states[2], "state2 passed!");
            }
        }
        ...
        if (sender != pair) { // not buy, remove liq, etc 
            require(!states[3], "state3 passed!");   
          _addBigLiquidity(r1);
          require(!states[4], "state4 passed!");
    ...
    function _swapBack(uint r1) internal returns (uint) {
        ...
        require(!states[8], "state8 passed!");
