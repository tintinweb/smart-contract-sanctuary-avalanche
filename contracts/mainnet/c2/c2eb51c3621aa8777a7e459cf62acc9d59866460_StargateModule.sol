...
contract StargateModule is BaseModule {
    ...
    function harvest() override external onlyManager {
        _harvest();
    }
    ...
    function withdrawAsync(address receiver, uint256 amount) override external payable virtual onlyManager returns (uint256) {
        return _withdrawAsync(receiver, amount);
    }

    function withdrawSync(address receiver, uint256 amount) override external onlyManager returns (uint256) {
        return _withdrawSync(receiver, amount);
    }

    function collect(address receiver, uint256 amount) override external onlyManager returns(uint256) {
...
abstract contract BaseModule is IYieldModule, Ownable, Modifier {
    ...
    function panic() override external view onlyOwnerOrManger {
