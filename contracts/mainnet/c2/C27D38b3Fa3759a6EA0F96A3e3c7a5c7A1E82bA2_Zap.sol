...
contract Zap is Ownable {
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtDAI;
            (amtToken, amtDAI) = IUniswapV2Router01(routerAddr).removeLiquidityETH(token, amount, 0, 0, address(this), block.timestamp);
            ...
            TransferHelper.safeTransferETH(_recipient, amtDAI);
        } else {
