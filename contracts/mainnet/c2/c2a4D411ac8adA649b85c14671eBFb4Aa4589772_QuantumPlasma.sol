...
/* QuantumPlasma contract */
contract QuantumPlasma is ERC20Burnable, Operator
{
    ...
    {
        require(taxOffice == msg.sender, "Warning: Caller is not the tax office");
        _;
    }

    modifier onlyOperatorOrTaxOffice()
    {
        require(isOperator() || taxOffice == msg.sender, "Warning: Caller is not the operator or the tax office");
        _;
    }

    /**
     * @notice Constructs the PLASMA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Quantum Plasma", "PLASMA")
    {
        /* Mints 1 PLASMA to contract creator for initial pool setup */
        require(_taxRate < 10000, "Warning: Tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "Warning: Tax collector address must be non-zero address");
    ...
    {
        require(_index >= 0, "Warning: Index has to be higher than 0");
        require(_index < getTaxTiersTwapsCount(), "Warning: Index has to lower than count of tax tiers");
    ...
    {
        require(_index >= 0, "Warning: Index has to be higher than 0");
        require(_index < getTaxTiersRatesCount(), "Warning: Index has to lower than count of tax tiers");
    ...
    function _getPlasmaPrice() internal view returns (uint256 _plasmaPrice)
    {
        try IOracle(plasmaOracle).consult(address(this), 1e18) returns (uint144 _price)
        {
            return uint256(_price);
        }
        catch
        {
            revert("Plasma: Failed to fetch PLASMA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _plasmaPrice) internal returns (uint256)
    {
        if (autoCalculateTax)
        {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId)
            {
                if (_plasmaPrice >= taxTiersTwaps[tierId])
                {
                    require(taxTiersRates[tierId] < 10000, "Warning: Tax equal or bigger to 100%");
    ...
    function setPlasmaOracle(address _plasmaOracle) public onlyOperatorOrTaxOffice
    {
        require(_plasmaOracle != address(0), "Warning: Oracle address cannot be 0 address");
        plasmaOracle = _plasmaOracle;
    }

    function setTaxOffice(address _taxOffice) public onlyOperatorOrTaxOffice
    {
        require(_taxOffice != address(0), "Warning: Tax office address cannot be 0 address");
    ...
    {
        require(!autoCalculateTax, "Warning: Auto calculate tax cannot be enabled");
        require(_taxRate < 10000, "Warning: Tax equal or bigger to 100%");
    ...
    {
        require(!excludedAddresses[_address], "Warning: Address can't be excluded");
    ...
    {
        require(excludedAddresses[_address], "Warning: Address can't be included");
    ...
    /**
     * @notice Operator mints PLASMA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PLASMA to mint to
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool)
    {
        ...
        if (autoCalculateTax)
        {
            uint256 currentPlasmaPrice = _getPlasmaPrice();
            currentTaxRate = _updateTaxRate(currentPlasmaPrice);

            if (currentPlasmaPrice < burnThreshold)
            {
                burnTax = true;
            }
        }
    ...
    external onlyOperator {
        ...
        require(!rewardPoolDistributed, "Warning: Only can distribute once");
        require(_genesisPool != address(0), "Warning: !_genesisPool");
