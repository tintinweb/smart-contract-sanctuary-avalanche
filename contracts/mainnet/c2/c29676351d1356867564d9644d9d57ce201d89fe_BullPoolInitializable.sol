...
contract BullPoolInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // The address of the bull pool factory
    address public BULL_POOL_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == BULL_POOL_FACTORY, "Not factory");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 bullReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
            accTokenPerShare.add(bullReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 bullReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(bullReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
...
// File: contracts/BullPoolFactory.sol
...
contract BullPoolFactory is Ownable {
    event NewBullPoolContract(address indexed bullPool);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new bull jungle contract
     */
    function deployPool(
        IERC20 _stakedToken,
        IERC20 _rewardToken,
        ...
        bytes memory bytecode = type(BullPoolInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address bullPoolAddress;

        assembly {
            bullPoolAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        BullPoolInitializable(bullPoolAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewBullPoolContract(bullPoolAddress);
    }
}