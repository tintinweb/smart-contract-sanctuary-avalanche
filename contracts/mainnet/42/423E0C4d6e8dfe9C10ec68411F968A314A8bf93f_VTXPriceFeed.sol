...
contract VTXPriceFeed is IPriceFeed {
    using UQ112x112 for uint224;
    using SafeMathJoe for uint256;
    IPriceFeed public AVAXPriceFeed;
    JoePair public VTXAVAX;
    address public VTXToken;
    constructor(address LPToken, address _VTXToken, address _AVAXPriceFeed) public {
        VTXAVAX = JoePair(LPToken);
        VTXToken = _VTXToken;
        AVAXPriceFeed = IPriceFeed(_AVAXPriceFeed);
    }

    function fetchPrice_v() view external override returns (uint256) {
        address t0 = VTXAVAX.token0();
        address t1 = VTXAVAX.token1();
        (uint112 r0, uint112 r1, ) = VTXAVAX.getReserves();
