 ...
 */
 contract Presale is Owned{
    ...
    // address of wavax stablecoin
    ERC20 public wavax;
    // buy rate
    uint public rate;
    // amount of wei raised
    uint public wavaxRaised;
    ...
    uint256 public minWavaxLimit = 300 * (10 ** 18);
    uint256 public maxWavaxLimit = 600 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _wavax, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
        require(_wallet != address(0));
        require(address(_wavax) != address(0));
    ...
    // allows buyers to put their wavax to get some pToken once the presale will closes
    function buy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(isList == false || whiteListed[msg.sender] == true);
        require(_amount >= minWavaxLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + _amount <= maxWavaxLimit);
        ...
        // update state
        wavaxRaised = wavaxRaised.add(_amount);
        ...
        preBuys[msg.sender].wavaxAmount = preBuys[msg.sender].wavaxAmount.add(_amount);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].wavaxAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].wavaxAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    // allows operator wallet to get the wavax deposited in the contract
    function retreiveWAVAX() public {
        require(msg.sender == wallet);
        require(!isPresaleOpen, "Presale is not over yet");
        wavax.safeTransfer(wallet, wavax.balanceOf(address(this)));
    }
