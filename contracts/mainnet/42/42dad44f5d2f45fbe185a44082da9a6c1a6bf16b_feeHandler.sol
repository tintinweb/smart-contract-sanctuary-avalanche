    ...
    UUPSUpgradeable
{
    ...
    event ClaimBribe(address indexed token, address indexed caller, uint amount, uint bounty);
    ...
    function claimBribes(address pool, address[] calldata tokens) internal {
        ...
        feeDist.getReward(tokenID, tokens);
    }

    /// @notice swaps bribes to weth, or locks to neadSnek if the reward token is snek and notifies multiRewards
    function processBribes(address pool, address[] calldata tokens) public onlyRole(HARVESTER_ROLE) {
        claimBribes(pool, tokens);
            ...
            for (uint i; i < len; ++i) {
                ...
                if (bal > 0) {
                    ...
                    if (tokens[i] == snek) {
                        ...
                        emit ClaimBribe(veDepositor, msg.sender, bal-fee, fee);
                        ...
                        emit ClaimBribe(swapTo, msg.sender, amountOut-fee, fee);
                    }
                }
            }
        }
    ...
    function batchProcessBribes(address[] calldata pools, address[][] calldata tokens) external {
        for (uint i; i < pools.length; ) {
            processBribes(pools[i], tokens[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function processPerformanceFees(address token) public onlyRole(HARVESTER_ROLE) {
        booster.poke(token);
