...
/// @dev Utilizes 'IERC20', 'AppStorage', 'LAuthorizable', 'LNodeController', 'LPausable'
contract VaporControllerFacet {
    ...
    function claim(uint256[] memory _ids) external payable {
        ...
        uint256 rewards = LNodeController.claim(s, _ids);
        IERC20(s.vpnd).transfer(msg.sender, rewards);
    }
    ...
    function createNode(string memory _name, uint256 _amount) external {
        ...
        bool isStratosphereMember = IStratosphere(s.stratosphere).tokenIdOf(account) != 0;
    ...
    function compound(uint256[] memory _ids) external {
        LPausable.enforceIsUnpaused(s);

        (uint256 rewards, uint256 fees) = LNodeController.compound(s, _ids);
        IERC20(s.vpnd).transfer(s.rewardsPool, rewards);
        IERC20(s.vpnd).transfer(s.treasury, fees);
    }
...
library LNodeController {
    ...
    function compound(AppStorage storage s, uint256 _id) internal {
        ...
        if (pending != 0) {
            ...
            node.lastRewardUpdate = s.txCounter;
            node.rewardPaid += pending;
            IERC20(s.vpnd).transfer(s.rewardsPool, pendingAfterFee); // transfer from diamond back to rewards pool when compound oki
