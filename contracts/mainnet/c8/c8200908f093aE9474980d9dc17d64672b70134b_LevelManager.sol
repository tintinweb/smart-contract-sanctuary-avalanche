...
// File contracts/levels/IStakingLockable.sol
...
// File contracts/levels/ILevelManager.sol
...
// File contracts/levels/WithLevels.sol
...
// File contracts/levels/WithPools.sol
...
abstract contract WithPools is Adminable, ILevelManager {
    ...
    mapping(address => address) public poolPair;
    mapping(address => uint256) public poolPrice;

    event PoolEnabled(address indexed pool, bool status);
    event PoolMultiplierSet(address indexed pool, uint256 multiplier);
    event PoolPairSet(address indexed pool, address pair);
    event PoolPriceSet(address indexed pool, uint256 price);
    ...
    function setPoolPair(address pool, address pair) public onlyOwnerOrAdmin {
        require(poolExists(pool), "LevelManager: Pool does not exist");
        poolPair[pool] = pair;
        emit PoolPairSet(pool, pair);
    }

    function setPoolPrice(address pool, uint256 price) public onlyOwnerOrAdmin {
        require(poolExists(pool), "LevelManager: Pool does not exist");
        poolPrice[pool] = price;
        emit PoolPriceSet(pool, price);
    }

    // price in USD wei
    function getPoolPrice(address pool) public view override returns (uint256) {
        return poolPrice[pool];
    }
...
// File contracts/levels/LevelManager.sol
...
contract LevelManager is Adminable, ILevelManager, WithLevels, WithPools {
    ...
    mapping(address => ForceLevel) public forceLevel;
        ...
        returns (Tier memory, uint256)
    {
        ...
        uint256 amount;
        address pool;
        ForceLevel memory flevel = forceLevel[account];
        if (flevel.idx > 0) {
            tier = tiers[flevel.idx];
            pool = address(pools[flevel.poolIdx]);
            amount = tier.minAmount;
        } else {
            (amount, pool) = getUserAmountAndPool(account);
            tier = getTierForAmount(amount);
        }
        return (
            tier,
            pool != address(0x0)
                ? getUserMaxBuy(tier, IStakingLockableExternal(pool), account)
                : 0
        );
    }

    // this returns max user buy in usd.
    function getUserMaxBuy(
        Tier memory tier,
        IStakingLockableExternal pool,
        address account
    ) internal view returns (uint256) {
        uint256 amount = pool.getLockedAmount(account);
        uint256 price = poolPrice[address(pool)];
        amount = amount.mul(tier.multiplier).div(100e18);
        return amount.mul(price) / 1e18;
    }
    ...
    function getUserAmountAndPool(address account)
        public
        view
        returns (uint256, address)
    {
        uint256 totalAmount = 0;
        address userpool = address(0);
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (poolEnabled[poolAddr]) {
                try pool.getLockedAmount(account) returns (uint256 amount) {
                    uint256 nAmount = (amount * multiplier) /
                        DEFAULT_MULTIPLIER;
                    if (nAmount > totalAmount) {
                        totalAmount = nAmount;
                        userpool = poolAddr;
                    }
                    continue;
        ...
        return (totalAmount, address(userpool));
    }
    ...
    function addIDO(address account, string memory _dataUri)
        external
        onlyRole(ADDER_ROLE)
    {
        require(account != address(0), "IDO cannot be zero address");
        isIDO[account] = true;
        idoInfos.push(IDOInfo(account, _dataUri));
    }

    function updateIDOUri(uint256 index, string memory _dataUri)
        external
        onlyRole(ADDER_ROLE)
    {
        require(index < idoInfos.length, "Index out of bounds");
        idoInfos[index].dataUri = _dataUri;
    }
    ...
    function setAccountLevel(
        ...
        forceLevel[account] = ForceLevel(levelIdx, poolIdx);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            ForceLevel memory flevel = forceLevel[addr];
            addresses[i] = addr;
            tiersIds[i] = tiers[flevel.idx].id;
            weights[i] = tiers[flevel.idx].multiplier;
        }
