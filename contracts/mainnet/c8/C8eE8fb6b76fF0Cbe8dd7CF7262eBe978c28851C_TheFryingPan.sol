...
contract TheFryingPan is Ownable, Pausable, IERC721Receiver, ReentrancyGuard {
	...
	uint256 public DAILY_GREASE_RATE = 10000 ether;

	// bacon must have 5 days worth of $GREASE to unstake or else it's too cold
	uint256 public MINIMUM_TO_EXIT = 5 days;
	...
	function delBuildNumber41() internal pure {} //etherscan trick
	...
	function stake(uint256[] calldata tokenIds) external notContract whenNotPaused nonReentrant {
		...
		if ((stakedBaconCounterByAddress[msg.sender] + noOfBacons > stakingLevelPerAddress[msg.sender].maxBacons) ||
			(stakedCopCounterByAddress[msg.sender] + noOfCops > stakingLevelPerAddress[msg.sender].maxCops)) {
	 ...
	 */
	function claim(uint256[] calldata tokenIds, bool unstake) external notContract _updateEarnings whenNotPaused nonReentrant {
		uint256 owed = 0;
		for (uint256 i = 0; i < tokenIds.length; i++) {
			if (!isCop(tokenIds[i])) {
				//Bacon pay Bacon tax
	 ...
	 */
	function upgradeHideoutStakingLevel() external whenNotPaused nonReentrant {
		// check if stakingLevelPerAddress is set. If not, set to default values
		if (!isStakingLevelPerAddressSet[msg.sender]) {
			stakingLevelPerAddress[msg.sender] = StakingLevel({
				hideoutLevel: 0, // default
		...
		StakingLevel memory currStakingLevel = stakingLevelPerAddress[msg.sender];

		// get next upgrade requirements
		uint256 upgradeCost = (200 * currStakingLevel.hideoutLevel++) + 200; // 200(n)+200
		upgradeCost = upgradeCost * 10 ** 18;

		// burn token and level up
		egg.burn(msg.sender, upgradeCost);

		stakingLevelPerAddress[msg.sender].hideoutLevel = stakingLevelPerAddress[msg.sender].hideoutLevel + 1;
	 ...
	 */
	function upgradePoliceStationStakingLevel() external whenNotPaused nonReentrant {
		// check if stakingLevelPerAddress is set. If not, set to default values
		if (!isStakingLevelPerAddressSet[msg.sender]) {
			stakingLevelPerAddress[msg.sender] = StakingLevel({
				hideoutLevel: 0, // default
		...
		stakingLevelPerAddress[msg.sender].policeStationLevel = stakingLevelPerAddress[msg.sender].policeStationLevel + 1;
	...
	function currentStakingLevel(address user) external view returns (uint256 hideoutLevel, uint256 policeStationLevel, uint256 maxBacons, uint256 maxCops) {
		...
		return (stakingLevel.hideoutLevel, stakingLevel.policeStationLevel, stakingLevel.maxBacons, stakingLevel.maxCops);
	}
	...
	/** UTILS */
	function _ceil(uint a, uint m) internal pure returns (uint256 r) {
    	return (a + m - 1) / m * m;
	}
