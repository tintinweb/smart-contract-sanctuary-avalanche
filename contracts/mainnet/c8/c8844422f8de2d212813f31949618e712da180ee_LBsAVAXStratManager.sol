...
interface IStakedAvax {
    function getSharesByPooledAvax(
        uint avaxAmount
...
/// @title LBsAVAXStrategyManager
/// @author SteakHut Finance
/// @notice contract to enable rebalances of the underlying strategy using existing parameters
/// @notice this strategy will rebalance after hitting triggers to the NAV of sAVAX
/// @notice the strategy will harvest each time period and also look to place earn call if within particular range
////https://toolkit.abdk.consulting/math#convert-number
contract LBsAVAXStratManager is Ownable, AutomationCompatibleInterface {
    using Math512Bits for uint256;
    using SafeERC20 for IERC20;

    address public immutable strategyAddress;
    IStakedAvax constant sAVAX =
        IStakedAvax(0x2b2C81e08f1Af8835a78Bb2A90AE924ACE0eA4bE);
    ...
    //underlying bin step of the lbPair
    uint256 public immutable binStep;

    //offset from the nav bin that enables deploying liquidity in;
    uint256 public binOffset;
    //offset from the NAV of the underlying asset i.e around the nav;
    //once activeBin = storedNavBin + navOffset rebalance
    //navOffset should be > bin Offset
    uint256 public navOffset;
    //store the navBin
    uint256 public storedNavBin;
    ...
    /// @notice Updates navOffset
    /// @param _navOffset new nav offset.
    function setNavOffset(uint256 _navOffset) external onlyOwner {
        require(_navOffset >= 0, "Manager: Nav offset too small");
        navOffset = _navOffset;
    }

    /// @notice Manually Updates storedNavBin
    /// @param _storedNavBin new storedNavBin
    function setStoredNavBin(uint256 _storedNavBin) external onlyOwner {
        require(_storedNavBin >= 0, "Manager: Nav Bin too small");
        storedNavBin = _storedNavBin;
    }
    ...
    /// @notice gets the current NAV bin from sAVAX contract
    /// @return id binId of the NAV bin
    function getNAVBin() public view returns (uint256 id) {
        uint256 navPrice = sAVAX.getPooledAvaxByShares(1e18);

        //getIdFromPrice require 128x128 so need to convert
        uint256 price = uint256(navPrice).shiftDivRoundDown(128, 1e18);
        uint24 _id = BinHelper.getIdFromPrice(price, binStep);
    ...
    /// @notice executes a earn of the underlying strategy
    function _earn() internal {
        ILBStrategy strategy = ILBStrategy(strategyAddress);
        strategy.earn();
    }

    /// @notice executes a rebalance of the underlying strategy with the same parameters
    ...
    /// @param callFeeRecipient address to be credited as gasCaller
    ...
    /// @notice 1. If period has been exceeded harvest the strategy only
    /// @notice 2. If activeId is within range of storedNavBin + binOffset earn
    /// @notice 3. If the storedNAVBin is deviating by navOffset we should rebalance
    /// @notice 4. ^^ we also protect against a rebalance if the price is not within a range
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        //get the current NAVBin of sAVAX
        uint256 currentNAVBinId = uint256(getNAVBin());

        // if the active bin is within the storedNavBin + binOffset
        // we can place an earn call
        if (activeId == storedNavBin - binOffset) {
            //we are able to deposit liquidity by placing an earn call
            //however, we should also check if there is sufficient capital to deploy.
            if (
                strategy.getBalanceX() >= minAmount ||
                strategy.getBalanceX() >= minAmount
            ) {
                upkeepNeeded = true;
            }
        ...
        //if the current currentNAVBinId is coming close to the storedNavBin we should rebalance and store the new navBin
        //we dont want to rebalance if the nav is falling.
        if (currentNAVBinId >= storedNavBin + navOffset) {
            //check price is within the new NavBin + BinOffset
            if (activeId == currentNAVBinId - binOffset) {
                //we should now trigger a rebalance
                //we also need to store the new _navBinId
                upkeepNeeded = true;
            }
        }
        ...
        performData; //silence unused param
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        //get the current NAV bin of sAVAX
        uint256 _NAVBinId = uint256(getNAVBin());
        ...
        if (activeId == storedNavBin - binOffset) {
            //we are able to deposit liquidity by placing an earn call
            //we should also check if there is sufficient capital to deploy.
            if (
                strategy.getBalanceX() >= minAmount ||
                strategy.getBalanceX() >= minAmount
            ) {
        ...
        //if the _navBinId is coming close to the storedNavBin we should rebalance and store the new navBin
        //we dont want to rebalance if the nav is falling so ignore this case
        if (_NAVBinId >= storedNavBin + navOffset) {
            //check price is within the new NavBin - binOffset (discount)
            if (activeId == _NAVBinId - binOffset) {
                //we should now trigger a rebalance
                _executeRebalance();
                //we also need to store the new _navBinId
                storedNavBin = _NAVBinId;
            }
        }
        performData; //silence unused param
    }
