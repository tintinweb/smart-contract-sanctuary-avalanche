...
contract Unknown is ERC20, Ownable, PaymentSplitter {
    ...
    uint16 public selTax = 45;
    uint256 public add_reward = 0;
    uint256 public nodePrices = 10000000000000000000;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Unknown::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "Unknown::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
    ...
    function updatenodePrices(uint256 newVal) external onlyOwner {
        nodePrices = newVal;
    }
    ...
    function updatemaxBalanceAmountRate(uint16 _maxBalanceAmountRate) external onlyOwner {
        require(_maxBalanceAmountRate <= 10000, "Unknown::updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxBalanceAmountRate >= 200, "Unknown::updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "Unknown::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 100, "Unknown::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate.");
    ...
    function _transfer(
        ...
        if (sellTaxActivated && to == joePair) {
            uint256 taxAmount = amount * selTax/100;
    ...
    function createNodeWithTokens(string memory name) external {
        ...
        super._transfer(sender, address(this), nodePrices);
        nodeManager.createNode(sender, name, nodePrices);
    }

    function createNodeWithAirdropAdmin(string memory name, address destination,uint256 count) external onlyOwner{
        require(
            bytes(name).length > 3 && bytes(name).length < 32,
            "NC:1"
        );
        ...
        nodeManager.createNode(destination, name, count * (10 * 10**18));
    }
     ...
     function multipleNodeAirdrop(address[] calldata addresses) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            ...
            nodeManager.createNode(addresses[i], "Unknown-AIRDROP", 10 * 10**18);
        }
    } 
    ...
    function setSelTax (uint16 value) external onlyOwner {
        require(value <= 1000);
        selTax = value;
    }
