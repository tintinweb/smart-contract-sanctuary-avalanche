...
contract Burrata is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint public BURRATA_PRODUCE_TIME = 86400;
  uint public LEVEL_REQ = 5;
  uint public MODIFIER = 3600;
  ...
  string private baseURI = "https://cheesyverse.s3.filebase.com/burrata-json/";
  ...
  constructor() ERC721("Burrata", "BURR") {
  ...
  function mint(address to, uint amount) internal {
    ...
    function claimCheese() public nonReentrant {
        ...
        require(supply.current() < 5000, "All the Burrata has been minted!");
  ...
  function claimableView(address account) public view returns(uint) {
    ...
    function cheeseTime(address account) public view returns(uint) {
        if (fromagerLevel[account] >= LEVEL_REQ) {
            ...
            return BURRATA_PRODUCE_TIME - (timeModifier * MODIFIER);
        }
        
        return BURRATA_PRODUCE_TIME;
  ...
  function makeCheese(uint _amount) public nonReentrant {
        ...
        emit MakingBurrata(_amount, msg.sender);
    }
  ...
  function setMilkTime(uint _newValue) public onlyOwner {
      BURRATA_PRODUCE_TIME = _newValue;
  }
