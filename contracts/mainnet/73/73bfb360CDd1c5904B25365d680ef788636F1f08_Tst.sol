    ...
    WalletObserverImplementationPointer
{
    ...
    constructor(address _playersManager,address _treasury) ERC20("TSTV2", "TSTV2") { // to change 
        require(
     ...
     function setFeesAmount(uint _sellFeesAmount) public onlyOwner {
        require(_sellFeesAmount <= 500, "fees too high");
    ...
    function liquidityReward(uint256 amount) external onlyPlayersManager {
        require(
            ...
            "Ball: LiquidityPoolManager is not set"
        );
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
              if (address(walletObserver) != address(0)) {
            walletObserver.beforeTokenTransfer(_msgSender(), sender, recipient, amount);
        }
