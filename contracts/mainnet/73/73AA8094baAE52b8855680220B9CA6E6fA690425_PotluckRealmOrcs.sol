...
// File: contracts/OrkAIrdrop.sol

//SPDX-License-Identifier: MIT
pragma solidity ^0.8.7;

contract PotluckRealmOrcs is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, Pausable {
  ...
  constructor() ERC721 ("Avax Realm Orcs", "AXRO") {
    baseURI = "ipfs://QmS6AvUxUhM6g8EdAwEpyJzUgxZQaFqVvz2tTeTY8zukvb/";
    _pause();
  }
  ...
  function _baseURI() internal view override returns (string memory) {
    ...
    function mintWhitelisted(uint8 amount) public whenNotPaused {
  ...
  function addWhitelistAddresses(address[] memory _addresses, uint256 numberOfMints) external onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      require(_addresses[i] != address(0), "Address cannot be 0.");
      require(whitelistedAddress[_addresses[i]] == 0, "Balance must be 0.");
      whitelistedAddress[_addresses[i]] = numberOfMints;
    }
  }
    ...
    function removeWhiteList(address[] memory addressList) public onlyOwner {
      ...
      whitelistedAddress[addressList[i]] = 0;
    }

  function orcsOwned(address _owner) external view returns (uint256[] memory) {
  ...
  function internalMint(address to) internal {
    require(totalSupply() < maxSupply, "All Orcs have been minted!");
