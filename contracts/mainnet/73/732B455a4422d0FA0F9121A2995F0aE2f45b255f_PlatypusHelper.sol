...
contract PlatypusHelper is Ownable {
    ...
    function getTVL(address lp) public view returns (uint256 TVLinUSD) {
        ...
        if (TVLinUSD == 0){
            TVLinUSD = (amount * getTokenPricePairedWithStable(token,USDC)) / (10**ERC20(lp).decimals());
        }
    }
...
interface IBalanceHelper {
    ...
    function getTokenPricePairedWithStable(address token, address stable) external view returns (uint256 tokenPrice);
