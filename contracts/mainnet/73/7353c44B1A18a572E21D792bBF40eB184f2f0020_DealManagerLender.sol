...
contract DealManagerLender is AccessControl, ReentrancyGuard {
    ...
    /// @dev mapping asset address with offer id
    mapping(address => uint256[]) public assetOffers;
    ...
    /// @dev mapping offer id -> preferred collaterals
    mapping(uint256 => address[]) public offerPreferredCollaterals;
    ...
    event OfferCreated(uint256 indexed _id, address indexed _maker, address indexed _asset, uint256 _assetAmount);
    event OfferAskInfoUpdated(
        ...
        uint256 _askAssetRatio
    );
    event OfferPreferredCollateralsUpdated(uint256 indexed _id, address[] _preferredCollaterals);
    ...
    /**
     * @notice Create offer
     * @param _asset: An address of asset
     ...
     * @param _askAssetRatio: Ask asset ratio
     * @param _preferredCollaterals: Preferred collaterals
     */
    function createOffer(
        ...
        uint256 _askDuration,
        uint256 _askAssetRatio,
        address[] memory _preferredCollaterals
    ) external nonReentrant {
        require(whitelistedAssets.contains(_asset), "CreateOffer: asset is not allowed");

        // Asset balance before transfer
        uint256 _before = IERC20(_asset).balanceOf(address(this));

        // Transfer asset to contract
        IERC20(_asset).safeTransferFrom(address(msg.sender), address(this), _assetAmount);

        // Deflationary tokens check
        _assetAmount = IERC20(_asset).balanceOf(address(this)).sub(_before);
        ...
        assetOffers[_asset].push(_offerId);
        ...
        offerAskInfos[_offerId] = OfferAskInfo({
            ...
            askAssetRatio: _askAssetRatio
        });

        // Set preferred collaterals
        offerPreferredCollaterals[_offerId] = _preferredCollaterals;

        // Add offer to storage before moving collateral
        offers[_offerId] = _offer;

        // Emit OfferCreated event
        emit OfferCreated(_offerId, address(msg.sender), _asset, _assetAmount);
    }
    ...
    function updateOfferAskInfo(
        ...
        // Set offer ask asset ratio
        _info.askAssetRatio = _askAssetRatio;

        emit OfferAskInfoUpdated(_offerId, _askTitle, _askDescription, _askIntRateBP, _askDuration, _askAssetRatio);
    }

    /**
     * @notice Set offer's preferred collaterals
     * @param _offerId: Offer ID
     * @param _preferredCollaterals: Preferred collaterals
     */
    function setPreferredCollaterals(uint256 _offerId, address[] memory _preferredCollaterals) external nonReentrant {
        require(_offerId < totalOffersCount, "SetPreferredCollaterals: offer not found");
        Offer storage _offer = offers[_offerId];
        require(address(msg.sender) == _offer.maker, "SetPreferredCollaterals: only maker is allowed to update");
        require(_offer.status == OfferStatus.Pending, "SetPreferredCollaterals: offer is either closed or canceled");

        // Set new offer preferred collaterals
        offerPreferredCollaterals[_offerId] = _preferredCollaterals;

        emit OfferPreferredCollateralsUpdated(_offerId, _preferredCollaterals);
    }
    ...
    function cancelOffer(uint256 _offerId) external nonReentrant {
        ...
        // Transfer asset back to offer's maker
        IERC20(_offer.asset).safeTransfer(address(_offer.maker), _offer.assetAmount);
     ...
     * @param _offerId: Offer ID
     * @param _collateral: An address of collateral to lend
    ...
    function offerBid(
        ...
        require(whitelistedCollaterals.contains(_collateral), "OfferBid: collateral is not allowed");
        Offer storage _offer = offers[_offerId];
        ...
        /// Transfer asset to contract
        IERC20(_collateral).safeTransferFrom(address(msg.sender), address(this), _amount);

        // Deflationary tokens check
        _amount = IERC20(_collateral).balanceOf(address(this)).sub(_before);
    ...
    function setBid(
        ...
        if (_amount > _bid.amount) {
            // Transfer additional bid collateral from bidder
            uint256 _incAmount = _amount.sub(_bid.amount);

            // Collateral balance before transfer
            uint256 _before = IERC20(_bid.collateral).balanceOf(address(this));

            /// Transfer collateral to contract
            IERC20(_bid.collateral).safeTransferFrom(address(msg.sender), address(this), _incAmount);

            // Deflationary tokens check
            _incAmount = IERC20(_bid.collateral).balanceOf(address(this)).sub(_before);
            ...
            // Transfer reducted bid collateral back to bidder
            uint256 _reducAmount = _bid.amount.sub(_amount);
            IERC20(_bid.collateral).safeTransfer(address(msg.sender), _reducAmount);
    ...
    function setBidInfo(
        ...
        if (_useVault) {
            (address _token, ) = IVaultController(vaultController).vaultInfo(_vaultId);
            require(_bid.collateral == _token, "CreateOffer: vault token mismatch");
        }
        ...
        _bid.useVault = _useVault;
        _bid.vaultId = _vaultId;

        emit OfferBidInfoSet(_offerId, _bid.id, _duration, _intRateBP, _intProRated, _allowLiquidator, _useVault, _vaultId);
    }
    ...
    function acceptBid(
        ...
        IERC20(_offer.asset).safeApprove(address(feedLoan), 0);
        IERC20(_offer.asset).safeApprove(address(feedLoan), _offer.assetAmount);
        IERC20(_bid.collateral).safeApprove(address(feedLoan), 0);
        IERC20(_bid.collateral).safeApprove(address(feedLoan), _bid.amount);
    ...
    function cancelBid(uint256 _offerId, uint256 _bidId) external nonReentrant {
        ...
        IERC20(_bid.collateral).safeTransfer(address(msg.sender), _bid.amount);
    ...
    /**
     * @notice View list of offers by asset
    ...
    function viewOffersByAsset(
        ...
        uint256 _length = _size;
        uint256 _offersLength = assetOffers[_asset].length;
        if (_length > _offersLength - _cursor) {
            _length = _offersLength - _cursor;
        }
        ...
        Offer[] memory _values = new Offer[](_length);
        for (uint256 i = 0; i < _length; i++) {
            uint256 _offerId = assetOffers[_asset][_cursor + i];
    ...
    /**
     * @notice Count total number of offers by asset
     * @param _asset: asset address
     */
    function offersCountByAsset(address _asset) external view returns (uint256) {
        return assetOffers[_asset].length;
    }

    /**
     * @notice Count offer preferred collaterals
     * @param _offerId: Offer ID
     */
    function offerPreferredCollateralsCount(uint256 _offerId) external view returns (uint256) {
        return offerPreferredCollaterals[_offerId].length;
    }
