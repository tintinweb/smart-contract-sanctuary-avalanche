...
WalletObserverImplementationPointer
{
    ...
    function updateBuyFee(uint256 buyFee)
    public
    onlyOwner
    {
        require(buyFee <= 30, "ERC20: Liquidity fee must not exceed 30");
        _buyFee = buyFee;
    }

    function updateSellFee(uint256 sellFee)
    public
    onlyOwner
    {
        require(sellFee <= 30, "ERC20: Sell fee must not exceed 30");
        _sellFee = sellFee;
    }
    ...
    function _transfer(
        ...
        if (shouldTakeFee(from, to)) {
            if (_pairs[to]) {
                amountFee = amount * _sellFee / 100;
            } else {
                amountFee = amount * _buyFee / 100;
            }
        }
        ...
        if (amountFee > 0) {
            super._transfer(from, address(liquidityPoolManager), amountFee);
        }
    ...
    function setLiquidityPair(address _address, bool value) external onlyOwner {
        _pairs[_address] = value;
    }

    function shouldTakeFee(address from, address to) internal view returns (bool) {
        return (_pairs[from] || _pairs[to]) && !_isFeeExempt[from];
    }

    function setFeeExempt(address _address, bool value) external onlyOwner{
