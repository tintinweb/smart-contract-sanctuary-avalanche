...
contract Vesting is Ownable {
    ...
    function flipProxyState(address proxyAddress) external onlyAdmin {
    ...
    function insertBeneficiaries(address[] calldata users, uint256[] calldata lastClaimDate,
        uint256[] calldata vestingAmount, uint256[] calldata start, uint256[] calldata claimedBalance)
        external onlyOwner 
    {
        require(users.length == lastClaimDate.length && users.length == vestingAmount.length &&
            users.length == start.length && users.length == claimedBalance.length, "INCONSISTENT_ARRAYS");
        for(uint256 x; x < users.length; x++) {
            Beneficiaries[users[x]] = Beneficiary({
                lastClaimDate: lastClaimDate[x],
                vestingAmount: vestingAmount[x],
                start: start[x]
            });       
            beneficiaryCount++;
            BeneficiaryWallets[beneficiaryCount] = users[x];       
            ClaimedBalance[users[x]] = claimedBalance[x]; 
            emit InsertBeneficiary(users[x], lastClaimDate[x], vestingAmount[x], start[x], claimedBalance[x]);
        }
    }    
    ...
    event InsertBeneficiary(address indexed user, uint256 indexed lastClaimedDate, uint256 indexed vestingAmount,
        uint256 start, uint256 claimedBalance);    
}
