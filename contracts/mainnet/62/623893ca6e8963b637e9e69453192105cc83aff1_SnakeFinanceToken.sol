...
contract SnakeFinanceToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 SNAKE-WAVAX LP -> Snake pool
    uint256 public constant INITIAL_SNAKE_POOL_DISTRIBUTION = 140000 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Snake Finance Token", "SNAKE") {
        // Mints 1 SNAKE to contract creator for initial pool setup
    ...
    function _getSnakePrice() internal view returns (uint256 _SnakePrice) {
        try IOracle(SnakeOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function _updateTaxRate(uint256 _SnakePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_SnakePrice >= taxTiersTwaps[tierId]) {
    ...
    function setSnakeOracle(address _SnakeOracle) public onlyOperatorOrTaxOffice {
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSnakePrice = _getSnakePrice();
            currentTaxRate = _updateTaxRate(currentSnakePrice);
            if (currentSnakePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_SnakePool != address(0), "!_SnakePool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_SnakePool, INITIAL_SNAKE_POOL_DISTRIBUTION);
    }
