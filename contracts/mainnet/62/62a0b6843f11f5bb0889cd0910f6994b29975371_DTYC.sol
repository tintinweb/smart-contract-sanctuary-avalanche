/**
 *Submitted for verification at snowtrace.io on 2023-01-03
*/
...
contract DTYC is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public maxSupply = 750;

    constructor() ERC721A("SuperJordan", "SuperJordan Club",69,6969)
    {
        baseURI = "ipfs://QmVQkZCSBJSwi1vF64kaNapDwtjA8UQnL5u8rND6DHQ372/";
        MintStartEnabled = true;
    }

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Trump Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "Trump Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Trump Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Trump Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Trump Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
