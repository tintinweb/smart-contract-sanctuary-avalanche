  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    ...
    ****************************************/
    ...
    function _liquidateCollateral(address userAccount, uint256 amount) private {
        ...
        (uint amountA, uint amountB) = router.removeLiquidity(IPair(address(poolInfo.lpToken)).token0(), IPair(address(poolInfo.lpToken)).token1(), depositTokenAmount, amount0, amount1, address(this), block.timestamp+300);
    ...
    function _convertTokentoUSDCE(uint amount, uint token) private returns (uint256) {
        ...
        if (oriToken == address(USDCE)) {
            ...
            uint[] memory amountAs = router.swapExactTokensForTokens(amount, amountOutToken0, path, address(this), block.timestamp+300);
