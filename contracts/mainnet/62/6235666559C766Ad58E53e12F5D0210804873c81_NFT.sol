...
// File: contracts/Fridxs.sol
pragma solidity >=0.6.0 <0.8.0;


/**
 * @title Fridxs contract
...
contract NFT is ERC721, Ownable {
    ...
    uint256 private _reserved = 40;

    uint256 public price_wl;

    uint256 public price_public;
    ...
    constructor( string memory _tokenName, string memory _tokenNameCode, uint16 _maxTokens, uint256 _mintPrice_wl, uint256 _mintPrice_public, address _project_wallet, uint256 _maxPurchase)
     ERC721(  _tokenName, _tokenNameCode) {
         MAX_NFT =  _maxTokens;
         t1 = _project_wallet;
         price_wl =  _mintPrice_wl;
         price_public =_mintPrice_public;
    ...
    function setMaxNFTS(uint16 _maxTokens) public onlyOwner {
       MAX_NFT =  _maxTokens;
    }
    ...
    function reserveNFTs(uint8 numberOfTokens) public onlyOwner {
    ...
    function mint(uint8 numberOfTokens) public onlyEOA {
      require(numberOfTokens <= maxPurchase, "Max NFTs exceded");
      require(totalSupply().add(numberOfTokens) <= MAX_NFT, "Purchase would exceed max supply");
      require(presaleIsActive || saleIsActive, "Presale and Public sale are not active yet");
      if (!saleIsActive){
          require(whitelistedAddresses[msg.sender] == true, "Not whitelisted");
          //delete(whitelistedAddresses[msg.sender]);
      }
      MIM.safeTransferFrom( msg.sender, address(this), getNFTPrice().mul(numberOfTokens) );
    ...
    /**
     * @dev Gets current Fridx Price,  staggered floor price increase
     */
