...
contract LuckyDaoNFT is ERC721A, IERC2981Royalties, Ownable {
  ...
  uint256 public publicMintPrice = 0.33 ether ; // 0.33 Avax
  ...
  uint256 public WLPerWallet = 10; // mint limit per wallet for special sale
  uint256 public maxMintAmount = 5; 
  uint256 public WLmaxMintAmount = 2;  
  uint256 private reservedNFT;  // reserved NFT  

  uint256 public mintStage = 0 ; // 0-pause || 1-special mint ||2-public mint
 ...
 /***************************  MINTS *************************/
  ...
  function mint(uint256 numTokens) public payable { // public mint 
    if(numTokens > 0) {   
  ...
  function SpecialMint(uint256 numTokens) external payable onlyOrigin onlyWhitelisted { // wl & special mint
    if(numTokens > 0){
      if (mintStage == 1){
        require(totalSupply() + numTokens <= maxSupply - teamReserved_airdrop, "Mint Amount Exceeds Total Allowed Mints");
