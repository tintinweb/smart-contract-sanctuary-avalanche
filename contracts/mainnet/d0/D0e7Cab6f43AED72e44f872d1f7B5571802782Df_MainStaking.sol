...
contract MainStaking is Initializable, OwnableUpgradeable {
    ...
    /// @notice set the percentPTPStored address
    /// @param _percentPTPStored the percent to be set aside during staking
    function setPercentPTPStored(uint256 _percentPTPStored) external onlyOwner {
        require(_percentPTPStored < MAX_FEE, "Above max percent");
        percentPTPStored = _percentPTPStored;
    }

    /// @notice set the storage ptp address
    /// @param _storagePTP the storage ptp address
    function setStoragePTP(address _storagePTP) external onlyOwner {
        storagePTP = _storagePTP;
    }
    ...
    function setFee(uint256 index, uint256 value) external onlyOwner {
        ...
        require(fee.min_value <= value && value <= fee.max_value, "Value not in range");
    ...
    function sendRewards(
        ...
        if (!(computeAPR == address(0))) {
            Pool storage poolInfo = pools[token];
            IComputeAPR(computeAPR).computeAPRStable(token, ptp, afterFee, poolInfo.size);
    ...
    /// @notice withdraw from a PTP pool, Enables withdrawing liquidity from an asset using LP from a different asset in the same aggregate
    /// @dev Only a PoolHelper can call this function
    /// @param token the address of the pool token from which to withdraw
    /// @param _amount the initial amount of tokens to withdraw
    /// @param minAmount The minimal amount the user accepts because of slippage
    ...
    function withdrawWithDifferentAssetForOverCoveredToken(
        ...
        IERC20(poolInfo.lpAddress).approve(staking, lpAmount);
        IMasterPlatypus(masterPlatypus).withdraw(poolInfo.pid, lpAmount);
        uint256 beforeWithdraw = IERC20(overCoveredToken).balanceOf(address(this));
        IStableStaking(staking).withdrawFromOtherAsset(
            token,
            overCoveredToken,
            lpAmount,
            minAmount,
        ...
        IERC20(overCoveredToken).safeTransfer(
            sender,
            IERC20(overCoveredToken).balanceOf(address(this)) - beforeWithdraw
        );
        emit NewWithdraw(sender, token, _amount);
    }
