...
import "../interfaces/benqi/IBenqiInterestRateModel.sol";
import "../interfaces/ITokenExchange.sol";
import "../interfaces/IPriceOracle.sol";
import "./BaseRhoStrategy.sol";

contract BenqiStrategy is BaseRhoStrategy {
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    /// @dev reference to QiToken
    IQIToken public qiToken;

    /// @dev reference to comptroller
    IBenqiComptroller public benqiComptroller;

    /// @dev reference to Bonus Tokens: Benqi & AVAX
    IERC20MetadataUpgradeable public qi;

    /// @dev BenqiUSD price oracle contract
    IPriceOracle public qiUsdPriceOracle;
    IPriceOracle public avaxUsdPriceOracle;
    ...
    function initialize(
        ...
        address _qiTokenAddr,
        address _iBenqiComptrollerAddr,
        address _tokenExchange,
        address _qiAddr,
        address _qiUsdPriceOracleAddr,
        address _avaxUsdPriceOracleAddr,
        uint256 _lockDurationInBlock,
        uint256 _blocksPerYear
    ) external initializer {
        require(_qiTokenAddr != address(0), "qiToken address is 0");
        require(_iBenqiComptrollerAddr != address(0), "Benqi Comptroller address is 0");
        require(_tokenExchange != address(0), "Token exchange address is 0");
        require(_qiAddr != address(0), "Benqi address is 0");
        require(_qiUsdPriceOracleAddr != address(0), "BenqiUSD price oracle address is 0");
        require(_avaxUsdPriceOracleAddr != address(0), "AvaxUSD price oracle address is 0");

        BaseRhoStrategy.__initialize(_underlyingAddr, _lockDurationInBlock);
        qiToken = IQIToken(_qiTokenAddr);
        benqiComptroller = IBenqiComptroller(_iBenqiComptrollerAddr);
        qi = IERC20MetadataUpgradeable(_qiAddr);
        qiUsdPriceOracle = IPriceOracle(_qiUsdPriceOracleAddr); //off chain source to get the price
        avaxUsdPriceOracle = IPriceOracle(_avaxUsdPriceOracleAddr);
    ...
    function supplyRate() external view override returns (uint256) {
        return (qiToken.supplyRatePerTimestamp() * 365 days);
    }

    function supplyRatePerBlock() external view override returns (uint256) {
        return (qiToken.supplyRatePerTimestamp() * 365 days) / BLOCK_PER_YEAR;
    }

    function balanceOfUnderlying() public view override returns (uint256) {
        return (qiToken.balanceOf(address(this)) * qiToken.exchangeRateStored()) / 1e18;
    }
    ...
    function deployInternal(uint256 _amount) internal override {
        underlying.safeIncreaseAllowance(address(qiToken), _amount);
        assert(qiToken.mint(_amount) == 0);
    }

    function _qiPerTimeStampPerUnderlying(uint256 newSupply) internal view returns (uint256) {
        return
            (benqiComptroller.supplyRewardSpeeds(0, address(qiToken)) *
                qiUsdPriceOracle.price(address(qi)) *
                10**(qiToken.decimals() + underlying.decimals() - qiUsdPriceOracle.decimals())) / newSupply;
    }

    function _avaxPerTimeStampPerUnderlying(uint256 newSupply) internal view returns (uint256) {
        return
            (benqiComptroller.supplyRewardSpeeds(1, address(qiToken)) *
                avaxUsdPriceOracle.price(WAVAX_ADDR) *
                10**(18 + underlying.decimals() - avaxUsdPriceOracle.decimals())) / newSupply;
    }

    function _bonusRatePerTimeStamp(uint256 delta, bool isPositive) internal view returns (uint256) {
        uint256 newSupply = isPositive
            ? qiToken.exchangeRateStored() * qiToken.totalSupply() + delta
            : qiToken.exchangeRateStored() * qiToken.totalSupply() - delta;
        return (_qiPerTimeStampPerUnderlying(newSupply) + _avaxPerTimeStampPerUnderlying(newSupply));
    }

    function bonusRatePerTimeStamp(uint256 delta, bool isPositive) external view returns (uint256) {
        return _bonusRatePerTimeStamp(delta, isPositive);
    }
    ...
    function bonusSupplyRate() external view override returns (uint256) {
        return
            (((benqiComptroller.supplyRewardSpeeds(0, address(qiToken)) *
                qiUsdPriceOracle.price(address(qi)) *
                10**(qiToken.decimals() + underlying.decimals() - qiUsdPriceOracle.decimals())) +
                (benqiComptroller.supplyRewardSpeeds(1, address(qiToken)) *
                    avaxUsdPriceOracle.price(WAVAX_ADDR) *
                    10**(18 + underlying.decimals() - avaxUsdPriceOracle.decimals()))) * 365 days) /
            (qiToken.exchangeRateStored() * qiToken.totalSupply());
    }

    function bonusRatePerBlock() external view override returns (uint256) {
        return (_bonusRatePerTimeStamp(0, true) * 365 days) / BLOCK_PER_YEAR;
    }

    function effectiveSupplyRate() external view override returns (uint256) {
        return (this.supplyRate() + this.bonusSupplyRate());
    }

    function effectiveSupplyRate(uint256 delta, bool isPositive) external view override returns (uint256) {
        return
            (IBenqiInterestRateModel(qiToken.interestRateModel()).getSupplyRate(
                isPositive ? qiToken.getCash() + delta : qiToken.getCash() - delta,
                qiToken.totalBorrows(),
                qiToken.totalReserves(),
                qiToken.reserveFactorMantissa()
            ) + _bonusRatePerTimeStamp(delta, isPositive)) * 365 days;
    }

    function withdrawUnderlyingInternal(uint256 _amount) internal override {
        uint256 errorcode = qiToken.redeemUnderlying(_amount);
        string memory message = "BenqiStrategy: Fail to withdraw from Benqi";
        if (errorcode == 14) {
            message = "BenqiStrategy: Benqi has not enough cash";
        }
    ...
    function withdrawAllCashAvailableInternal() internal override {
        ...
        if (balance > withdrawable) {
            ...
            require(qiToken.redeemUnderlying(withdrawable) == 0, "BenqiStrategy: Fail to withdraw from Benqi");
            underlying.safeTransfer(_msgSender(), withdrawable);
            return;
        }
        require(qiToken.redeem(qiToken.balanceOf(address(this))) == 0, "BenqiStrategy: Fail to withdraw from Benqi");
    ...
    function collectRewardTokens(uint256 rewardCollectThreshold) external override onlyRole(VAULT_ROLE) nonReentrant {
        if (_shouldCollectBenqi(rewardCollectThreshold)) {
            benqiComptroller.claimReward(0, address(this)); // claimBenqi
            // sell Benqi at TokenExchange for underlying
            qi.safeIncreaseAllowance(address(tokenExchange), qi.balanceOf(address(this)));
            tokenExchange.sellExactInput(qi, underlying, _msgSender(), qi.balanceOf(address(this)));
        }

        if (_shouldCollectAvax(rewardCollectThreshold)) {
            address payable holder = payable(address(this));
            benqiComptroller.claimReward(1, holder); // claimAvax
            tokenExchange.sellExactNativeTokens{value: address(this).balance}(underlying, address(holder));
        }
    }

    function bonusTokens() external view override returns (address[] memory) {
        address[] memory _addr = new address[](2);
        _addr[0] = address(qi);
        _addr[1] = WAVAX_ADDR;
        return _addr;
    }

    function bonusTokensAccrued() external view override returns (uint256[] memory) {
        uint256[] memory _bal = new uint256[](2);
        _bal[0] = benqiComptroller.rewardAccrued(0, address(this));
        _bal[1] = benqiComptroller.rewardAccrued(1, address(this));
        return _bal;
    }

    function shouldCollectBenqi(uint256 rewardCollectThreshold) external view returns (bool) {
        return _shouldCollectBenqi(rewardCollectThreshold);
    }

    function shouldCollectAvax(uint256 rewardCollectThreshold) external view returns (bool) {
        return _shouldCollectAvax(rewardCollectThreshold);
    }

    function _shouldCollectBenqi(uint256 rewardCollectThreshold) internal view returns (bool) {
        uint256 qiPrice = qiUsdPriceOracle.price(address(qi));
        uint8 qiPriceDec = qiUsdPriceOracle.decimals();
        uint256 qiRewards = benqiComptroller.rewardAccrued(0, address(this));
        uint256 qiQty = qi.balanceOf(address(this)) + qiRewards;
        uint256 minUnderlyingQty = (qiQty * qiPrice * 10**underlying.decimals()) / 10**(qi.decimals() + qiPriceDec);
        return minUnderlyingQty > rewardCollectThreshold;
    }

    function _shouldCollectAvax(uint256 rewardCollectThreshold) internal view returns (bool) {
        uint256 avaxPrice = avaxUsdPriceOracle.price(WAVAX_ADDR);
        uint8 avaxPriceDec = avaxUsdPriceOracle.decimals();
        uint256 avaxRewards = benqiComptroller.rewardAccrued(1, address(this));
        uint256 avaxQty = address(this).balance + avaxRewards;
        uint256 minUnderlyingQty = (avaxQty * avaxPrice * 10**underlying.decimals()) / 10**(18 + avaxPriceDec); //18 is "avax.decimails"
        return minUnderlyingQty > rewardCollectThreshold;
    }

    function shouldCollectReward(uint256 rewardCollectThreshold) external view override returns (bool) {
        return (_shouldCollectBenqi(rewardCollectThreshold) || _shouldCollectAvax(rewardCollectThreshold));
    }
    ...
    function sweepERC20Token(address token, address to) external override onlyRole(SWEEPER_ROLE) {
        ...
        require(token != address(qiToken), "!safe");
        require(token != address(qi), "!safe");
        require(token != WAVAX_ADDR, "!safe");
        super._sweepERC20Token(token, to);
    }

    function getCash() internal view override returns (uint256) {
        return qiToken.getCash();
    }
    ...
    receive() external payable {} //for receieving avax
}
...
interface IBenqiComptroller {
    ...
    function supplyRewardSpeeds(uint8 tokenType, address qiToken) external view returns (uint256);

    function rewardSupplyState(uint8 tokenType, address holder) external view returns (uint256);

    function rewardAccrued(uint8 tokenType, address holder) external view returns (uint256);

    function rewardQi() external view returns (uint8);

    function rewardAvax() external view returns (uint8);

    function oracle() external view returns (address); //qiOracle address
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function bonusTokensAccrued() external view virtual override returns (uint256[] memory) {
