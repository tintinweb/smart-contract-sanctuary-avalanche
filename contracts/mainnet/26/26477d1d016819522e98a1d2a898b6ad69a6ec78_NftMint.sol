...
abstract contract overseer is Context {
   ...
   function getModFee(uint256 _val) external virtual returns(uint256);
   function getNftPrice(uint _val) external virtual returns(uint256);
...
contract NftMint is Ownable {
    ...
    constructor(address[] memory addresses,address payable _treasury, uint256[] memory supplies,uint256 _limit) {
    ...
    function mint(uint256 _id) payable external {
    	...
    	uint256 _price = over.getNftPrice(num);
    ...
    function updateOverseer(address _address) external onlyOwner {
        _overseer = _address;
    	over = overseer(_overseer);
    }
