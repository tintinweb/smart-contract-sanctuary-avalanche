...
// File: gladiator-finance-contracts/contracts/glad_core/roman_nft/InitialNFTPool.sol
...
contract InitialNFTPool is IPool, Controllable, IERC721Receiver, Pausable {
    ...
    event TokenStaked(address indexed owner, uint256 indexed tokenId, bool isPrey);
    event TokenClaimed(uint256 indexed tokenId, bool isPrey);
    event BarnRewardClaimed(address indexed user, uint256 amount, uint256 tax);
    event PackRewardClaimed(address indexed user, uint256 amount);
    event UserStakeChange(
    ...
    event PoolStakeChange(
        uint256 totalPreyStaked,
        uint256 totalPredatorsStaked,
    ...
    uint80 public lastTimestamp;

    // amount of total Reward due for each prey point staked
    uint256 public totalRewardPerPrey;

    // amount of total Reward due for each alpha point staked
    uint256 public totalRewardPerAlpha;

    uint256 public constant PERIOD = 86400 * 3; // (3 days)
    uint256 public REWARDS_PER_PERIOD = 1 * 10**18; // 1 GLADSHARE

    function setStakingParams(uint256 _rewardrate) external onlyOwner {
        REWARDS_PER_PERIOD = _rewardrate;
    }
    ...
    // number of Prey staked in the Barn
    ...
    uint80 public stakingEnd;

    // deadline for claiming
    uint80 public claimDeadline;
     ...
     */
    function addManyToPool(uint64[] calldata tokenIds)
        external
        whenNotPaused
        beforeDeadline
        updateEarnings(msg.sender)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (isPrey(tokenIds[i])) _addPreyToBarn(msg.sender, tokenIds[i]);
            else _addPredatorToPack(msg.sender, tokenIds[i]);
        }
    }
     ...
     */
    function _addPreyToBarn(address account, uint256 tokenId) internal {
        totalPreyStaked += 1;
        userPool[account].preyStaked += 1;
        barnIndices[tokenId] = barn.length;
        barn.push(Stake({owner: account, tokenId: uint64(tokenId)}));

        emit TokenStaked(account, tokenId, true);
    }
    ...
    function claimManyFromPool(uint64[] calldata tokenIds)
        public
        whenNotPaused
        beforeDeadline
        updateEarnings(msg.sender)
    {
        require(tx.origin == msg.sender, "Only EOA");
        require(stakingEnd <= block.timestamp, "Cannot unstake yet");
    ...
    function _claimPreyFromBarn(uint256 tokenId) internal {
        ...
        totalPreyStaked -= 1;
        userPool[_msgSender()].preyStaked -= 1;
    ...
    function _claimPredatorFromPack(uint256 tokenId) internal {
        ...
        totalPredatorsStaked -= 1;
        userPool[_msgSender()].predatorsStaked -= 1;
    ...
    function _rescue(uint256[] calldata tokenIds, bool sendToOwner) internal {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (isPrey(tokenId)) {
                ...
                totalPreyStaked -= 1;
                userPool[stake.owner].preyStaked -= 1;
                ...
                totalPredatorsStaked -= 1;
                userPool[stake.owner].predatorsStaked -= 1;
                userPool[stake.owner].stakedAlpha -= alpha;
                totalAlphaStaked -= alpha; // Remove Alpha from total staked
     ...
     */
    function _payPackTax(uint256 amount) internal {
        if (totalAlphaStaked == 0) {
            // if there's no staked predators
            unaccountedRewards += amount; // keep track of $REWARD to predators
            return;
        }
        // makes sure to include any unaccounted $REWARD
        totalRewardPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
    ...
    modifier updateEarnings(address _user) {
        totalRewardPerPrey = getRewardPerPrey();
        if (totalPreyStaked != 0) lastTimestamp = uint80(block.timestamp);
        totalRewardPerAlpha = getRewardPerAlpha();
        if (totalAlphaStaked != 0) {
            unaccountedRewards = 0;
        }

        //update claimable prey reward
        userPool[_user].claimableBarnReward +=
            (totalRewardPerPrey - userPool[_user].lastRewardPerPrey) *
            userPool[_user].preyStaked;
        userPool[_user].claimablePackReward +=
            (totalRewardPerAlpha - userPool[_user].lastRewardPerAlpha) *
            userPool[_user].stakedAlpha;
        userPool[_user].lastRewardPerPrey = totalRewardPerPrey;
        userPool[_user].lastRewardPerAlpha = totalRewardPerAlpha;
        ...
        emit UserStakeChange(
            ...
            userPool[_user].preyStaked,
            userPool[_user].predatorsStaked,
            userPool[_user].lastRewardPerPrey,
            userPool[_user].claimableBarnReward,
            userPool[_user].claimablePackReward,
            userPool[_user].stakedAlpha,
            userPool[_user].lastRewardPerAlpha
        );
        emit PoolStakeChange(totalPreyStaked, totalPredatorsStaked, totalAlphaStaked);
    }

    /* REWARD calculation */

    function getRewardPerPrey() public view returns (uint256 amount) {
        if (block.timestamp <= stakingStart) return totalRewardPerPrey;
        if (lastTimestamp >= stakingEnd) return totalRewardPerPrey;
        if (totalPreyStaked == 0) return totalRewardPerPrey;
        uint80 periodstart = stakingStart > lastTimestamp
            ? stakingStart
            : lastTimestamp;
        uint80 periodend = uint80(
            stakingEnd < block.timestamp ? stakingEnd : block.timestamp
        );
        if (periodend <= periodstart) return totalRewardPerPrey;
        return
            totalRewardPerPrey +
            ((periodend - periodstart) * REWARDS_PER_PERIOD) /
            PERIOD /
            totalPreyStaked;
    }

    function getRewardPerAlpha() public view returns (uint256 amount) {
        if (totalAlphaStaked != 0) {
            // makes sure to include any unaccounted $REWARD
            return
                totalRewardPerAlpha +
    ...
    function _claimBarnReward() internal
    {
        address _user = msg.sender;
        uint256 claimable = userPool[_user].claimableBarnReward;
        if (claimable > 0) {
            userPool[_user].claimableBarnReward = 0;
            uint256 tax = (claimable * REWARD_CLAIM_TAX_PERCENTAGE) / 100;
            _payPackTax(tax);
            rewardToken.mint(_user, claimable - tax);
            emit BarnRewardClaimed(msg.sender, claimable - tax, tax);
        }
    }
    ...
    function _claimPackReward() internal
    {
        address _user = msg.sender;
        uint256 claimable = userPool[_user].claimablePackReward;
        userPool[_user].claimablePackReward = 0;
        if (claimable > 0) {
            rewardToken.mint(_user, claimable);
            emit PackRewardClaimed(msg.sender, claimable);
        }
    }
    ...
    function setRescueEnabled() external onlyOwner {
        ...
        stakingEnd = uint80(block.timestamp);
        rescueEnabled = true;
    }
    ...
    function getClaimableBarnReward(address _user)
        external
        view
        returns (uint256 amount)
    {
        if (block.timestamp > claimDeadline) return 0;
        uint256 tmp = getRewardPerPrey();
        return
            userPool[_user].claimableBarnReward +
            (tmp - userPool[_user].lastRewardPerPrey) *
            userPool[_user].preyStaked;
    }

    function getClaimablePackReward(address _user)
        external
        view
        returns (uint256 amount)
    {
    ...
    function _randomPreyStake(uint256 seed)
        ...
        returns (Stake memory)
    {
        if (totalPreyStaked == 0) return Stake(0, address(0));
        // choose a value from Barn
        return barn[(seed & 0xFFFFFFFFFFFFFFFF) % totalPreyStaked];
    }
