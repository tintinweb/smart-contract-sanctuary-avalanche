...
contract GordionStrongboxERC20 {
    ...
    function splitDividend(uint tokenIndex) external onlyOwners {
        ...
        uint teamShare = (claimableTotal[tokenIndex] - claimableNFTReward - burnAmount) / 4;
        uint developmentShare = teamShare * wShare[tokenIndex] / 10000;
        _token.transfer(vl,developmentShare);
        _token.transfer(dev,developmentShare);
        uint lastShare = teamShare - (developmentShare * 2);
