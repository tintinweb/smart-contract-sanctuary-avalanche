...
contract superSwitchAVAX is baseSuperToken,superAaveAVAXImpl,superQiAvaxImpl{    
    ...
    function onDeposit(address account,uint256 _amount,uint64 _fee)internal virtual override(superAaveTokenImpl,superQiAvaxImpl,superTokenInterface) returns(uint256){
        if (lendingSwitch == 0){
            return superAaveTokenImpl.onDeposit(account,_amount,_fee);
        }else{
            return superQiAvaxImpl.onDeposit(account,_amount,_fee);
        }
    }
