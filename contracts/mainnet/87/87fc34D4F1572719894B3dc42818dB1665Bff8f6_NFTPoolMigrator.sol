...
contract NFTPoolMigrator is Controllable, IERC721Receiver, MyPausable {
    ...
    function loadPreyTokenOnwers()  external onlyOwner {
        ...
        for (; gasleft() > 300000 && preyLoadIndex + i < cnt; i++) {
            (uint256 tokenId, address owner) = oldPool.barn(i + preyLoadIndex);
    ...
    function loadPredatorTokenOnwers(uint256 alphaIndex)  external onlyOwner {
        ...
        for (; gasleft() > 300000 ; i++) {
            try oldPool.pack(alphaIndexToAlphaLUT[alphaIndex], predatorLoadIndex[alphaIndex] + i) returns (uint256 tokenId, address owner) {
                tokenOwners[tokenId] = owner;
            } catch {
    ...
    function migrateNfts()  external onlyOwner {
        ...
        for (; gasleft() > 300000 && migrationIndex + i < cnt;) {
            i++;
            ...
            if (!userMigrated[stakeOwner]) {
                ...
                (NFTPoolNG.UserStake memory glad, NFTPoolNG.UserStake memory gladshare) = oldPool.userPool(stakeOwner);
