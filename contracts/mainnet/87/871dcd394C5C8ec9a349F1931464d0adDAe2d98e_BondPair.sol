...
interface IPair {
	function getReserves() external returns (
		uint112 _reserve0,
		uint112 _reserve1,
...
contract BondPair is Ownable {
	...
	constructor(
		...
		require(rates[1] >= 0 && rates[1] < 10000, "Bond: maxPayoutRate must between 0 and 10000");
		require(rates[2] > 0 && rates[2] < 10000, "Bond: cashoutMintRate must between 0 and 10000");
		require(rates[3] > 0 && rates[3] < 10000, "Bond: cashinMintRate must between 0 and 10000");
		rewardRate = rates[0];
		maxPayoutRate = rates[1];
		cashoutMintRate = rates[2];
		cashinMintRate = rates[3];
		
		require(_swapAmountOut != 0, "Bond: SwapAmountOut cannot be zero");
	...
	function deposit(uint256 _amount, uint256 _minPayout, address _depositor) external {
		require(_depositor != address(0), "Bond: Depositor cannot de address zero");

		(uint256 amountEvol, uint256 amountStable) = _getAmounts(_amount);

		uint256 payout = (amountEvol * 2) * (10000 + rewardRate) / 10000;
		require(payout >= _minPayout && payout >= 1000000000, "Bond: Payout is too small");
		require(payout <= maxPayout(), "Bond: Payout is too large");

		uint256 additionalEvol = (amountEvol * 2) * (10000 + cashinMintRate) / 10000 - (amountEvol * 2);
		...
		totalInvested += amountStable * 2;
		totalRemainingRewards += payout;
		IMinter(minter).mintRewards(amountStable * 2, payout + additionalEvol);
		IERC20(pair).safeTransferFrom(msg.sender, liquidityHandler, _amount);
	...
	function _getAmounts(uint256 _amount) internal returns (uint256, uint256) {
		(uint256 reserve0, uint256 reserve1,) = IPair(pair).getReserves();

		uint256 reserveEvol;
		uint256 reserveStable;
		if (IPair(pair).token0() == evol) {
			reserveEvol = reserve0;
			reserveStable = reserve1;
		} else {
			reserveEvol = reserve1;
			reserveStable = reserve0;
		}

		uint256 totalSupply = IPair(pair).totalSupply();
		uint256 amountEvol = _amount * reserveEvol / totalSupply;
		uint256 amountStable = _amount * reserveStable / totalSupply;
		require(amountEvol > 0 && amountStable > 0, "Bond: Amount too low");
		return (amountEvol, amountStable);
	}
	...
	function stakeOrSend(address _recipient, bool _toStake, uint256 _amount) private {
		...
		if (!_toStake) {
			IERC20(evol).transfer(_recipient, _amount);
		} else {
	...
	function setCashinMintRate(uint256 _rate) external onlyOwner {
		require(_rate > 0 && _rate < 10000, "Bond: CashinMintRate must between 0 and 10000");
		cashinMintRate = _rate;
	}
