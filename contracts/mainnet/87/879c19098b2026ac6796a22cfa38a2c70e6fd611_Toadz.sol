...
// File contracts/toadz/IERC2981Royalties.sol
...
// File contracts/toadz/ERC2981PerTokenRoyalties.sol
...
// File contracts/toadz/Toadz.sol
...
// To use console.log do not specify a network when running script.
// import "hardhat/console.sol";

contract Toadz is ERC721, ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    uint256 public constant maxTokens = 6969;
    uint256 public constant maxMintsPerTx = 10;
    uint256 public tokenPrice = 690000000000000000; //0.69 ether
    uint256 public startingBlock = 999999999;
    string private _baseURIextended;
    string public provenance;
    uint256 public nextTokenId = 1;
    bool public devMintLocked = false;
    uint256[] public remainingBuckets;
    uint256[] public remainingTokenIdsInBucket;
    ...
    function mint(uint256 quantity) external payable {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            if (remainingTokenIdsInBucket.length == 0) {
                uint256 randomBucketIndex = _getRandomBucketIndex();
                bucket = remainingBuckets[randomBucketIndex];
                uint256 endingIndex = bucket == 69 ? 70 : 101;
                for (uint256 j = 1; j < endingIndex; j++) {
                    remainingTokenIdsInBucket.push((bucket * 100) + j);
                }
                _removeBucket(randomBucketIndex);
            }
            uint256 randomTokenIdIndex = _getRandomTokenIdIndex();
            uint256 tokenIdToMint = remainingTokenIdsInBucket[
                randomTokenIdIndex
            ];
            _safeMint(msg.sender, tokenIdToMint);
            _setTokenRoyalty(tokenIdToMint, owner(), contractRoyalties);
            _removeTokenId(randomTokenIdIndex);
            _storeMintedTokenId(tokenIdToMint);
            nextTokenId++;
        }
    }
    ...
    function _removeBucket(uint256 index) internal {
        require(index < remainingBuckets.length);
        remainingBuckets[index] = remainingBuckets[remainingBuckets.length - 1];
        remainingBuckets.pop();
    }

    function _removeTokenId(uint256 index) internal {
        require(index < remainingTokenIdsInBucket.length);
        remainingTokenIdsInBucket[index] = remainingTokenIdsInBucket[
            remainingTokenIdsInBucket.length - 1
        ];
        remainingTokenIdsInBucket.pop();
    }

    function _storeMintedTokenId(uint256 index) internal {
        mintedTokenIds.push(index);
    }
    ...
    function _getRandomBucketIndex() internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
            ...
            ) % remainingBuckets.length;
    }

    function _getRandomTokenIdIndex() internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
                        block.difficulty,
                        block.timestamp,
                        remainingTokenIdsInBucket
                    )
                )
            ) % remainingTokenIdsInBucket.length;
    }
