    ...
    TemporarilyPausable
{
    ...
    constructor(IAuthorizer authorizer)
        ...
        SignaturesValidator("Rumble Vault")
    {
...
abstract contract FlashLoans is Fees, ReentrancyGuard, Authentication, TemporarilyPausable {
    ...
    function flashLoan(
        ...
        uint256[] memory amounts,
        bytes memory userData
    ) external override nonReentrant authenticate whenNotPaused {
        InputHelpers.ensureInputLengthMatch(tokens.length, amounts.length);
...
abstract contract AssetTransfersHandler is AssetHelpers {
     ...
     *
     * If `asset` is AVAX, `fromInternalBalance` must be false (as AVAX cannot be held as internal balance),
     * and the funds will be wrapped into WAVAX.
     *
     * WARNING: this function does not check that the contract caller has actually supplied any AVAX - it is up to the
     * caller of this function to check that this is true to prevent the Vault from using its own AVAX (though the Vault
    ...
    function _receiveAsset(
        ...
        if (_isAVAX(asset)) {
            _require(!fromInternalBalance, Errors.INVALID_AVAX_INTERNAL_BALANCE);

            // The AVAX amount to receive is deposited into the WAVAX contract, which will in turn mint WAVAX for
            // the Vault at a 1:1 ratio.

            // A check for this condition is also introduced by the compiler, but this one provides a revert reason.
            // Note we're checking for the Vault's total balance, *not* AVAX sent in this transaction.
            _require(address(this).balance >= amount, Errors.INSUFFICIENT_AVAX);
     ...
     *
     * If `asset` is AVAX, `toInternalBalance` must be false (as AVAX cannot be held as internal balance), and the funds
     * are instead sent directly after unwrapping WAVAX.
     */
    function _sendAsset(
        IAsset asset,
        uint256 amount,
        ...
        if (_isAVAX(asset)) {
            // Sending AVAX is not as involved as receiving it: the only special behavior is it cannot be
            // deposited to Internal Balance.
            _require(!toInternalBalance, Errors.INVALID_AVAX_INTERNAL_BALANCE);

            // First, the Vault withdraws deposited AVAX from the WAVAX contract, by burning the same amount of WAVAX
            // from the Vault. This receipt will be handled by the Vault's `receive`.
            _WAVAX().withdraw(amount);

            // Then, the withdrawn AVAX is sent to the recipient.
    ...
    /**
     * @dev Returns excess AVAX back to the contract caller, assuming `amountUsed` has been spent. Reverts
     * if the caller sent less AVAX than `amountUsed`.
     *
     * Because the caller might not know exactly how much AVAX a Vault action will require, they may send extra.
     * Note that this excess value is returned *to the contract caller* (msg.sender). If caller and e.g. swap sender are
     * not the same (because the caller is a relayer for the sender), then it is up to the caller to manage this
     * returned AVAX.
     */
    function _handleRemainingAvax(uint256 amountUsed) internal {
        _require(msg.value >= amountUsed, Errors.INSUFFICIENT_AVAX);
    ...
    /**
     * @dev Enables the Vault to receive AVAX.
     * This is required for it to be able to unwrap WAVAX, which sends AVAX to the caller.
     *
     * Any AVAX sent to the Vault outside of the WAVAX unwrapping mechanism would be forever locked inside the Vault, so
     * we prevent that from happening. Other mechanisms used to send AVAX to the Vault (such as being the recipient of
     * an AVAX swap, Pool exit or withdrawal, contract self-destruction, or receiving the block mining reward) will
     * result in locked funds, but are not otherwise a security or soundness issue. This check only exists as an attempt
     * to prevent user error.
     */
    receive() external payable {
        _require(msg.sender == address(_WAVAX()), Errors.AVAX_TRANSFER);
    }
...
abstract contract UserBalance is ReentrancyGuard, AssetTransfersHandler, VaultAuthorization {
    ...
    function manageUserBalance(UserBalanceOp[] memory ops) external payable override nonReentrant {
        // We need to track how much of the received AVAX was used and wrapped into WAVAX to return any excess.
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            if (kind == UserBalanceOpKind.WITHDRAW_INTERNAL) {
                ...
                if (kind == UserBalanceOpKind.DEPOSIT_INTERNAL) {
                    ...
                    // Keep track of all AVAX wrapped into WAVAX as part of a deposit.
                    if (_isAVAX(asset)) {
                        ethWrapped = ethWrapped.add(amount);
                    }
                } else {
                    // Transfers don't support AVAX.
                    _require(!_isAVAX(asset), Errors.CANNOT_USE_AVAX_SENTINEL);
                    IERC20 token = _asIERC20(asset);
        ...
        // Handle any remaining AVAX.
        _handleRemainingAvax(ethWrapped);
    }
...
abstract contract PoolBalances is Fees, ReentrancyGuard, PoolTokens, UserBalance {
    ...
    function _processJoinPoolTransfers(
        ...
        for (uint256 i = 0; i < change.assets.length; ++i) {
            ...
            if (_isAVAX(asset)) {
                wrappedAvax = wrappedAvax.add(amountIn);
            }
        ...
        // Handle any used and remaining AVAX.
        _handleRemainingAvax(wrappedAvax);
    }
...
abstract contract Swaps is ReentrancyGuard, PoolBalances {
        ...
        returns (uint256 amountCalculated)
    {
        ...
        // If the asset in is AVAX, then `amountIn` AVAX was wrapped into WAVAX.
        _handleRemainingAvax(_isAVAX(singleSwap.assetIn) ? amountIn : 0);
    }
        ...
        returns (int256[] memory assetDeltas)
    {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            if (delta > 0) {
                ...
                if (_isAVAX(asset)) {
                    wrappedAvax = wrappedAvax.add(toReceive);
                }
            } else if (delta < 0) {
...
/**
 * @dev The `Vault` is Rumble's core contract. A single instance of it exists for the entire network, and it is the
...
contract Vault is VaultAuthorization, FlashLoans, Swaps {
    ...
    // solhint-disable-next-line func-name-mixedcase
    function WAVAX() external view override returns (IWAVAX) {
        return _WAVAX();
    }
}