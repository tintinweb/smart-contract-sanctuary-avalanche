...
contract Pets is ERC721Enumerable, Pausable, Restricted {
    ...
    uint256[5] private generationsPriceAVAX = [0.5 ether, 0.4 ether, 0.3 ether, 0.2 ether, 0.1 ether];
    ...
    function withdrawToken(address _token) external onlyController {
        IERC20(_token).transfer(devAddress, (getTokenBalance(_token) * 5) / 100);
        IERC20(_token).transfer(adminAddress, getTokenBalance(_token));
    }

    function withdrawToken(address _token, uint256 _amount) external onlyController {
        uint256 devAmount = (_amount * 5) / 100;
        IERC20(_token).transfer(devAddress, devAmount);
        IERC20(_token).transfer(adminAddress, _amount - devAmount);
    }
