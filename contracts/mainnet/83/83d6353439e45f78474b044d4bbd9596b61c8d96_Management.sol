...
contract Management is IManagement {
    ...
    function setNodeLimit(uint256 _limit) external onlyManager {
        nodeLimit = _limit;
    }

    function setClaimInterval(uint256 _hours) external onlyManager {
        claimHours = _hours;
    }
    ...
    function cashoutRewardArray(address account, uint256[] memory _indexArray) external onlyManager returns(uint256) {
        require(isNodeOwner(account), "MANAGEMENT: CASHOUT NO NODE OWNER");

        uint256 totalRewards = 0;

        for (uint256 i = 0; i < _indexArray.length; i ++) {
            uint256 _index = _indexArray[i];
    ...
    function cashoutAllReward(address account) external onlyManager returns(uint256) {
        ...
        for (uint256 i = nodeLength - 1; i >= 0; i --) {
    ...
    function compoundNode(address account, uint256 amount) external onlyManager {
        ...
        for (uint256 i = nodeLength; i >= 0; i --) {
            node = nodes[i];
