...
contract TheTapV3 is UUPSUpgradeable, OwnableUpgradeable {
    ...
    uint256 public payoutRateInterval;
    uint256 public minPayoutRate;
    uint256 public maxPayoutRate;
    uint256 public maxPayoutPeriod;
    uint256 public minPayoutWait;
    ...
    function setPayoutRateInterval(uint256 _payoutRateInterval)
        external
        onlyOwner
    {
        payoutRateInterval = _payoutRateInterval;
    }

    function setMinPayoutRate(uint256 _minPayoutRate) external onlyOwner {
        minPayoutRate = _minPayoutRate;
    }

    function setMaxPayoutRate(uint256 _maxPayoutRate) external onlyOwner {
        maxPayoutRate = _maxPayoutRate;
    }

    function setMaxPayoutPeriod(uint256 _maxPayoutPeriod) external onlyOwner {
        maxPayoutPeriod = _maxPayoutPeriod;
    }

    function setMinPayoutWait(uint256 _minPayoutWait) external onlyOwner {
        minPayoutWait = _minPayoutWait;
    }
    ...
    function claim() external noBlackList isNotPaused {
        ...
        address _addr = msg.sender;
        require(
            block.timestamp > users[_addr].deposit_time + minPayoutWait,
    ...
    */
        ...
        )
    {
        ...
        if (users[_addr].payouts < max_payout) {
            uint256 timeElasped = SafeMath.min(
                block.timestamp.safeSub(users[_addr].deposit_time),
                2 days
            );

            uint256 currentPayoutRate = getCurrentPayoutRate(_addr);
            //Using 1e18 we capture all significant digits when calculating available divs
            share = users[_addr]
                .deposits
                .mul(currentPayoutRate * 1e18)
                .div(10000e18)
                .div(24 hours); //divide the profit by payout rate and seconds in the day

            payout = share * timeElasped;
    ...
    function getCurrentPayoutRate(address _addr) public view returns (uint256) {
        if (
            users[_addr].currentPayoutRate == 0 ||
            (users[_addr].deposits * 360) / 100 > max_payout_cap
        ) {
            return maxPayoutRate;
        }
        return users[_addr].currentPayoutRate;
    }

    function decreasePayoutRate(address _addr) internal {
        if (users[_addr].currentPayoutRate == 0) {
            users[_addr].currentPayoutRate = maxPayoutRate;
        }
        users[_addr].currentPayoutRate -= payoutRateInterval;
        if (users[_addr].currentPayoutRate < minPayoutRate) {
            users[_addr].currentPayoutRate = minPayoutRate;
        }
    }

    function increasePayoutRate(address _addr) internal {
        if (users[_addr].currentPayoutRate == 0) {
            users[_addr].currentPayoutRate = maxPayoutRate;
        }
        users[_addr].currentPayoutRate += payoutRateInterval;
        if (users[_addr].currentPayoutRate > maxPayoutRate) {
            users[_addr].currentPayoutRate = maxPayoutRate;
        }
    }
