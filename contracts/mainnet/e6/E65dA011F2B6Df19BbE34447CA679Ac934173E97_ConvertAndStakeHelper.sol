...
contract ConvertAndStakeHelper is Ownable {
    ...
    function setConvertor(address _convertor) external onlyOwner {
        convertor = _convertor;
    }
    ...
    function depositFor(address _for, uint256 _amount) external {
        ...
        _approveTokenIfNeeded(depositToken, convertor, _amount);
        IConvertor(convertor).depositFor(_amount, address(this));
        uint256 amountConverted = IERC20(stakingToken).balanceOf(address(this));
        _approveTokenIfNeeded(stakingToken, helper, amountConverted);
        ISimpleHelper(helper).depositFor(amountConverted, _for);
    }
}
