...
contract NodeManagerV7 is Initializable {
    ...
    function setManager(address add, bool value) public onlyOwner {
        managers[add] = value;
    }
    ...
    function addTier(
        ...
        require(rewardsPerTime > 0, "Rewards has to be positive");
        require(claimInterval > 0, "Claim interval has to be positive");
        tierArr.push(
            Tier({
    ...
    function updateTier(
        ...
        require(tierId > 0, "Old name is incorrect");
        require(
    ...
    function removeTier(string memory tierName) public virtual onlyOwner {
        require(tierMap[tierName] > 0, "Already removed");
        tierMap[tierName] = 0;
        tierTotal--;
    }
    ...
    function _transferFee(uint256 amount) private {
        require(amount != 0, "Transfer amount can't zero!");
    ...
    function compound(string memory tierName, uint32 count) public {
        ...
        require(
            ...
            "Insufficient claimable tokens to compound."
        );
    ...
    function claim() public {
        ...
        if (claimFee > 0) {
            ...
            uint256 feeAmount = (oldRewardsOfUser[msg.sender] * claimFee) /
                10000;
    ...
    function transfer(
        ...
        require(
            (countOfUser[to] + count) <= maxCountOfUser,
        ...
        if (selected.length == 0) {
            ...
            for (uint32 i = 0; i < selected.length; i++) {
                ...
                if (node.owner == msg.sender) {
                    ...
                    require(pM <= 3, "Cannot pay more than 3 months");
                    fee += getAmountOut(((tier.price * mantPercent) / 10000) * count);
                }
            }
        }
    ...
    function migrateNodesFromOldVersion() public {
        require(!userMigrated[msg.sender], "Nodes have been migrated");
    ...
    function migrateRewardsFromOldVersion() public {
        require(userMigrated[msg.sender], "Nodes havent been migrated");
        require(
            !rewardMigrated[msg.sender],
            "Reward have been migrated"
        );
    ...
    function setARM(address[] memory add) public onlyManager {
        for (uint8 i = 0; i < add.length; i++) {
            rewardMigrated[add[i]] = true;
        }
    }
}
