...
contract RHYMEBondDepository is Ownable {
    ...
    IERC20 public immutable RHYME; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints RHYME when receives principle
    ...
    uint32 public lastDecay; // reference RHYME for debt decay
    ...
    struct Bond {
        uint payout; // RHYME remaining to be paid
    ...
    struct Adjust {
        ...
        uint32 lastTime; // RHYME when last adjustment made
    }
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 RHYME ( underflow protection )
        ...
        uint balanceBefore = RHYME.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) RHYME
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            RHYME.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == RHYME.balanceOf(address(this)), "Not enough RHYME to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            RHYME.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                RHYME.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                RHYME.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return RHYME.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to RHYME supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = RHYME.totalSupply();
    ...
    /**
     *  @notice calculate amount of RHYME available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or RHYME) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != RHYME, "NAT" );
