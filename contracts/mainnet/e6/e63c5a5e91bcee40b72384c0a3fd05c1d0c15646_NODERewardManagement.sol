...
contract NODERewardManagement {
    ...
    modifier onlySentry() {
        require(msg.sender == token || msg.sender == gateKeeper, "Just no");
        _;
    }
    ...
    // in case someone sends funds to the nodemgmr
...
contract MAGIAFINANCE is ERC20, Ownable, PaymentSplitter {
    ...
    bool private nodeRequired = false;

    uint256 private snipeBlockAmt;
    uint256 private _openTradingBlock = 0;
    uint256 private maxTx = 1;

    uint256 public sellTax = 200; //20% at launch on sells - lowering every 24h
    ...
    function _transfer(
        ...
        if ( to == address(uniswapV2Pair) && (from != address(this) && from != owner())){
            require(nodeRequired, "NodeRequirement disabled skip tax.");
    ...
    function addNodeValue(uint256 blocktime) public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool && sender != devPool,
            "CSHT: treasury, dev and rewardsPool cannot compound nodes"
        );
        uint256 rewardAmount = nodeRewardManager._getRewardAmountOf(
            sender,
            blocktime
    ...
    function addAllNodeValue() public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool && sender != devPool,
            "MANIA CSHT: treasury, dev and rewardsPool cannot cashout rewards"
        );
        uint256 rewardAmount = nodeRewardManager._getRewardAmountOf(sender);
        require(
            rewardAmount > 0,
    ...
    function setNodeRequired(bool _bool) external onlyOwner() {
        nodeRequired = _bool;
    }
