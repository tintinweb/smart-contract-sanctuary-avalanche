...
contract Growing is ReentrancyGuard, ERC1155Holder {
    ...
    constructor(
        DropERC1155 toolContractAddress,
        TokenERC20 ganjaContractAddress
    ) {
        toolNftCollection = toolContractAddress;
        rewardsToken = ganjaContractAddress;
    }
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        require(
            ...
            "You must have at least 1 Pot & Watering Bucket to start growing!"
        );

        if (playertool[msg.sender].isData) {
            toolNftCollection.safeTransferFrom(
                address(this),
                msg.sender,
                playertool[msg.sender].value,
                1,
                "Returning your old Watering Bucket"
            );
        }
    ...
    function withdraw() external nonReentrant {
        require(
            playertool[msg.sender].isData,
            "You do not have a Watering Bucket to withdraw."
        );
