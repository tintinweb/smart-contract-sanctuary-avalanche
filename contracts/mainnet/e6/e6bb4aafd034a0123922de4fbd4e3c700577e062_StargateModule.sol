...
contract StargateModule is BaseModule {
    ...
    uint16 public redeemFromChainId = 110;           // use Arbitrum as default
    ...
    event WithdrawSync(address receiver, uint256 amountBaseToken, uint256 amountLpToken, address callingUser);
    event WithdrawAsync(address receiver, uint256 amountBaseToken, uint256 amountLpToken, address callingUser);
    ...
    function setMinAmountToPayForCrossChainCall(uint256 newAmount) onlyOwner external {
    ...
    function harvestAndWithdrawPreview() override external view returns (uint256) {
        ...
        if (pendingStg <= stgSellThreshold) {
            ...
            // no additional harvest will be done return what we have (if any)
        }
    ...
    function withdrawAsync(address receiver, uint256 amount, address user) override external payable virtual onlyOwnerOrManager returns (uint256) {
        return _withdrawAsync(receiver, amount, user);
    }

    function withdrawSync(address receiver, uint256 amount, address user) override external onlyOwnerOrManager returns (uint256) {
        return _withdrawSync(receiver, amount, user);
    }
    ...
    function _withdrawSync(address receiver, uint256 amount, address user) internal returns(uint256) {
        require(isSyncWithdrawalPossible(amount), "StargateModule: not enough deltaCredits for sync withdrawal");
        (uint256 expectedBaseTokenAmount, uint256 lpTokenAmount) = _withdraw(amount, address(0x0), syncRedeem);
        emit WithdrawSync(receiver, expectedBaseTokenAmount, lpTokenAmount, user);
        return expectedBaseTokenAmount;
    }

    function _withdrawAsync(address receiver, uint256 amount, address user) internal returns (uint256) {
        (uint256 expectedBaseTokenAmount, uint256 lpTokenAmount) = _withdraw(amount, user, asyncRedeem);
        emit WithdrawAsync(receiver, expectedBaseTokenAmount, lpTokenAmount, user);
        return expectedBaseTokenAmount;
    }

    function _withdraw(uint256 amount, address user, function (uint256, address) internal redeem) internal returns (uint256, uint256) {
        ...
        redeem(lpTokenAmount, user);
        uint256 expectedAmount = pool.amountLPtoLD(lpTokenAmount);
        return (expectedAmount, lpTokenAmount);
    }


    // second argument not user but added in order to syncRedeem and asyncRedeem having the same function signature
    function syncRedeem(uint256 lpAmount, address __) internal {
        stargateRouter.instantRedeemLocal(routerPoolId, lpAmount, address(this));
    }

    function asyncRedeem(uint256 lpAmount, address user) internal {
        ...
        stargateRouter.redeemLocal{value : minAmountToPayForCrossChainCall}(
            ...
            payable(address(user)), // refund extra native gas to this address
...
interface IYieldModule {
    ...
    function withdrawAsync(address receiver, uint256 amount, address user) external payable returns(uint256);
    function withdrawSync(address receiver, uint256 amount, address user) external returns(uint256);
