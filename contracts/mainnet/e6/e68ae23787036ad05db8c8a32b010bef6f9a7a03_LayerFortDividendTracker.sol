...
contract LayerFort is ERC20, Ownable {
   ...
   LayerFortDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = AVAXRewardsFee + marketingFee + teamFee + liquidityFee;
    
    address public marketingWallet = 0x951A0a0cc4b589FdCC5e9e9db90435D00739640f;
    address public teamWallet = 0x95C03f91a2b196341f135cC93154D1538Aa68b9c;
    address public utilityWallet = 0x36B6E88Ae6f0cf4c3B0E629C9b1D0AE044877036;
    ...
    constructor()  ERC20("LayerFort", "LAYER") {
        
    	dividendTracker = new LayerFortDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LayerFort: The dividend tracker already has that address");

        LayerFortDividendTracker newDividendTracker = LayerFortDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LayerFort: The new dividend tracker must be owned by the LayerFort token contract");
    ...
    function updaterouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "LayerFort: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LayerFort: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address newPair, bool value) public onlyOwner {
        require(newPair != pair, "LayerFort: The pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "LayerFort: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LayerFort: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LayerFort: Cannot update gasForProcessing to same value");
    ...
    function setFees(uint256 _rewards, uint256 _marketing, uint256 _team, uint256 _utility, uint256 _liquidity) external onlyOwner{
        AVAXRewardsFee = _rewards;
        marketingFee = _marketing;
        teamFee = _team;
        utilityFee = _utility;
        liquidityFee = _liquidity;
        totalFees = _rewards+_marketing+ _team +_liquidity;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 AVAXToAddLiquidityWith = unitBalance * liquidityFee;
        ...
        if(teamAmt > 0) payable(teamWallet).sendValue(teamAmt);
...
contract LayerFortDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor()  DividendPayingToken("LayerFort_Dividend_Tracker", "LayerFort_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**9);
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "LayerFort_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LayerFort contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LayerFort_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LayerFort_Dividend_Tracker: Cannot update claimWait to same value");
