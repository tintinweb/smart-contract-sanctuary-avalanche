...
contract AstroToken is ERC20Detailed {
    ...
    */
    ...
    constructor(address _router, address _usdcAddress) ERC20Detailed("100 Days Ventures", "preASTRO", uint8(DECIMALS)) {
        ...
        //pairUSDC = IDEXFactory(router.factory()).createPair(address(this), address(usdcAddress));
        ...
        _allowedFragments[address(this)][address(usdcAddress)] = type(uint256).max;
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        uint256 initialBalance = usdcAddress.balanceOf(address(this));

        _swapTokensForUSDC(half, address(this));

        uint256 newBalance = usdcAddress.balanceOf(address(this)) - (initialBalance);
    ...
    function swapBack() internal swapping {
        ...
        uint256 balanceBefore = usdcAddress.balanceOf(address(this));
        ...
        uint256 amountStable = usdcAddress.balanceOf(address(this)) - balanceBefore;

        uint256 totalAVAXFee = ((realTotalFee) -
        (liquidityFee[0] + liquidityFee[1] + liquidityFee[2] + liquidityFee[3]) /
        (2));

        uint256 amountStableLiquidity = (amountStable * (liquidityFee[0] + liquidityFee[1] + liquidityFee[2] + liquidityFee[3])) /
        (totalAVAXFee) /
        (2);

        uint256 amountToRFV = amountStable * (riskFeeValueFee[0] + riskFeeValueFee[1] + riskFeeValueFee[2] + riskFeeValueFee[3]) / (realTotalFee);
        uint256 amountToTreasury = amountStable * (treasuryFee[0] + treasuryFee[1] + treasuryFee[2] + treasuryFee[3]) / (realTotalFee);
        uint256 amountToOperation = amountStable * (operationsFee[0] + operationsFee[1] + operationsFee[2] + operationsFee[3]) / (realTotalFee);
        uint256 amountToxASTRO = amountStable * (xASTROFee[0] + xASTROFee[1] + xASTROFee[2] + xASTROFee[3]) / (realTotalFee);
        uint256 amountToEcosystem = amountStable * (ecosystemFee[0] + ecosystemFee[1] + ecosystemFee[2] + ecosystemFee[3]) / (realTotalFee);


        usdcAddress.transfer(riskFreeValueReceiver, amountToRFV);
        usdcAddress.transfer(treasuryReceiver, amountToTreasury);
        usdcAddress.transfer(operationsReceiver, amountToOperation);
        usdcAddress.transfer(xASTROReceiver, amountToxASTRO);
        usdcAddress.transfer(futureEcosystemReceiver, amountToEcosystem);
