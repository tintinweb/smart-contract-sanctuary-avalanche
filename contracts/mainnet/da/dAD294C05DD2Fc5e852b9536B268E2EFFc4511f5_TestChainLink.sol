...
contract TestChainLink is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, VRFConsumerBaseV2 {
    ...
    uint256 constant MAX_SUPPLY = 18;
    ...
    uint256[MAX_SUPPLY] private _availableTokens;
    uint256 private _numAvailableTokens = MAX_SUPPLY;

    using SafeMath for uint;
    using Address for address;

    address public royaltyAddress = 0xa7b237bC08F6F128C7Bcd21a0fe55a3949b7ea9f;
    ...
    constructor(
        ...
        bytes32 gasLine,
        uint64 subId
    ) ERC721("Obsession", "OBSS") VRFConsumerBaseV2(_vrfConsumer) {
    ...
    function splitFees(uint sentAmount) internal {
     ...
     function getRandomAvailableToken(uint32 howMany) public returns (uint256) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        uint256 requestId = getRandomAvailableToken(uint32(amount));
        ...
        override
    {
        for(uint i = 0; i < randomWords.length; i++){
            uint256 randomNum = randomWords[i];
            uint256 randomIndex = randomNum % _numAvailableTokens;
            uint256 newTokenId = useAvailableTokenAtIndex(randomIndex);
    ...
    function safeMint(address to) public onlyOwner {
        uint256 requestId = getRandomAvailableToken(1);
