...
contract Lottery is Owners {
	...
	struct Draw {
		...
		address[] freeParticipants; // address owning free tickets
	...
	uint public maxFreeTickets;

	mapping(uint => mapping(address => uint)) public userNbTickets; // user nb of tickets
	mapping(uint => mapping(address => uint)) public userNbFreeTickets; // user nb of free tickets
	mapping(address => bool) public isBlacklisted;


	constructor(address _handler, uint _maxFreeTickets) {
		handler = _handler;
		maxFreeTickets = _maxFreeTickets;
	}
		...
		notBlacklisted(user)
	{
		...
		require(userNbFreeTickets[i][user] + count <= maxFreeTickets, "Lottery: Too many free tickets requested");

		userNbFreeTickets[i][user] += count;

		_storeFreeParticipant(draw, user, count);
	...
	function _storeFreeParticipant(
		Draw storage draw,
		address user, 
		uint count
	) 
		internal 
	{
		...
		for (uint j = 0; j < count; j++)
			draw.freeParticipants.push(user);
	}
	...
	function _drawExecute(
		...
		internal 
	{
		uint freeOrNot = _generatePseudoRandom(1000);
		uint iWin;
		address winner;

		if (freeOrNot%42 != 0 && size != 0) { // draw normal participant
			iWin = _generatePseudoRandom(size);
			winner = draw.participants[iWin];
			draw.participants[iWin] = draw.participants[size - 1];
			draw.participants.pop();
		} else { // free participant
			iWin = _generatePseudoRandom(freeSize);
			winner = draw.freeParticipants[iWin];
			draw.freeParticipants[iWin] = draw.freeParticipants[freeSize - 1];
			draw.freeParticipants.pop();
		}
	...
	function setmaxFreeTickets(uint _maxFreeTickets) external onlyOwners {
		maxFreeTickets = _maxFreeTickets;
	}
	...
	function getFreeParticipantsSize(uint i) external view returns (uint) {
		return draws[i].freeParticipants.length;
	}
	...
	function getFreeParticipantsBetweenIndexes(
		uint i,
		uint iStart,
		uint iEnd
	)
	...
	{
		address[] memory res = new address[](iEnd - iStart);
		address[] storage freeParticipants = draws[i].freeParticipants;
		
		for (uint j = iStart; j < iEnd; j++)
			res[j - iStart] = freeParticipants[j];
