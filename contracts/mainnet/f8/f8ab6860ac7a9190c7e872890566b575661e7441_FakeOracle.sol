...
contract Settlement is OrderManager, Signing {
    ...
    bytes32 public constant _PAYLOAD_TYPE_HASH =
        keccak256(
            "Payload(address fromToken,address toToken,uint256 fromAmount,uint256 toAmount,address sender,address recipient,uint32 validFrom,uint32 validTo,address driver,uint8 scheme,Condition condition,Interaction[] preHooks,Interaction[] postHooks)Condition(address target,bytes data)Interaction(address target,bytes data,uint256 value)"
        );

    bytes32 constant _CONDITION_TYPE_HASH =
        keccak256("Condition(address target,bytes data)");

    bytes32 constant _INTERACTION_TYPE_HASH =
        keccak256("Interaction(address target,bytes data,uint256 value)");
    ...
    function executeOrder(ISettlement.Order memory order) public {
        ...
        _execute(signatory, order.payload.preHooks);
        ...
        _execute(signatory, order.payload.postHooks);
    ...
    function verify(
        ...
        checkCondition(order.payload.condition);
    ...
    function buildDigest(
        ...
        bytes memory dataToHash = abi.encode(
            _PAYLOAD_TYPE_HASH,
            _payload.fromToken,
            _payload.toToken,
            _payload.fromAmount,
        ...
        bytes32 structHash = keccak256(
            abi.encodePacked(
                dataToHash,
                hash(_payload.condition),
                hash(_payload.preHooks),
                hash(_payload.postHooks)
            )
        );
    ...
    function hash(
        ...
        return
            keccak256(
                abi.encode(
                    _CONDITION_TYPE_HASH,
                    condition.target,
                    keccak256(bytes(condition.data))
                )
            );
    }

    function hash(
        ISettlement.Interaction memory interaction
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    _INTERACTION_TYPE_HASH,
                    interaction.target,
                    keccak256(bytes(interaction.data)),
                    interaction.value
                )
            );
    }
    ...
    function hash(
        ISettlement.Condition[] memory conditions
    ) internal pure returns (bytes32) {
        bytes32[] memory hashes = new bytes32[](conditions.length);
        for (uint256 i = 0; i < conditions.length; i++) {
            hashes[i] = hash(conditions[i]);
        }
    ...
    function hash(
        ...
        bytes32[] memory hashes = new bytes32[](interactions.length);
        for (uint256 i = 0; i < interactions.length; i++) {
            hashes[i] = hash(interactions[i]);
        }
        return keccak256(abi.encodePacked(hashes));
    }
}
...
contract OpenflowSdk is OrderDelegator {
    ...
    function initialize(address settlement, address _manager) external {
        _initialize(settlement, _manager);
    }
    ...
    function _submitOrder(
        ...
        require(
            ...
            "Only sender or manager can swap"
        );
        if (payload.recipient == address(0)) {
            payload.recipient = msg.sender;
        }
        ...
        payload.preHooks = _appendTransferToPreswapHooks(
            payload.sender,
            payload.preHooks,
            payload.fromToken,
            payload.fromAmount
...
contract OpenflowFactory {
    ...
    // Emit event when new instance is created
    event InstanceCreated(address instanceAddress, address managerAddress);
    ...
    /// @notice Generate an SDK instance with custom manager.
    ...
    function newSdkInstance(
        ...
        sdk.initialize(settlement, _manager);
        emit InstanceCreated(address(sdk), _manager);
    }
...
interface IOpenflowSdk {
    ...
    function initialize(address settlement, address manager) external;
...
contract SdkStorage {
    ...
    function _initialize(address settlement, address _manager) internal {
        ...
        options.auctionDuration = 60 * 1000 * 60 * 10;
