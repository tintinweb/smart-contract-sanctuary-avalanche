...
contract StrategyCurveAaveNew {
    ...
    address public gauge = address(0x4620D46b4db7fB04a01A75fFed228Bc027C9A899); // new gauge

    address public constant joeRouter =
        address(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);

    Mintr public constant liquidityGaugeFactory =
        Mintr(0xabC000d88f23Bb45525E447528DBF656A9D55bf5);
    ...
    constructor(address _controller, address[] memory _crvToUsdcPath) {
        ...
        internal
    {
        ...
        uint256[] memory amounts = IUniswapRouter(joeRouter).getAmountsOut(
            _amount,
            path
        );
        ...
        require(IERC20(path[path.length - 1]).balanceOf(address(this)) >= minAmount, "Wrong amount received");
    }
    ...
    function harvest() public {
        ...
        // Claim CRV reward
        liquidityGaugeFactory.mint(gauge);
        uint256 _crv = IERC20(crv).balanceOf(address(this));

        if (_crv > 0) {
            _swapTraderJoeWithPath(crvToUsdcPath, _crv);
            _zap();
        }
    }

    function harvestExtra(
        address[] calldata _pathToUsdc, 
        uint256 _amount
    ) external {
        require(
            msg.sender == strategist || msg.sender == governance,
        ...
        // Claim extra tokens reward if any exist
        if (ICurveGauge(gauge).reward_tokens(0) != address(0)) {
            ICurveGauge(gauge).claim_rewards();
            _swapTraderJoeWithPath(_pathToUsdc, _amount);
            _zap();
        }
    }

    function _zap() internal {
        // add liquidity on curve in usdc
        uint256 _usdc = IERC20(usdc).balanceOf(address(this));
        if (_usdc > 0) {
            IERC20(usdc).approve(pool, _usdc);
            ICurvePool(pool).add_liquidity([0, _usdc, 0], 0, true);
        }
        // send fee and deposit the LPs left into the related gauge
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
    ...
    function setCrvToUsdcPath(address[] memory _crvToUsdcPath) external onlyGovernance {
        crvToUsdcPath = _crvToUsdcPath;
    }
}