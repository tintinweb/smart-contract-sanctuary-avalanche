...
contract TShare is ERC20Burnable, Operator {
    ...
    // TOTAL MAX SUPPLY = 70,000 straw
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 55000 ether;
    uint256 public constant COMMUNITY_FUND_POOL_ALLOCATION = 10000 ether; // nft pool allocation
    ...
    bool public NFTPoolDistributed = false;

    constructor(uint256 _startTime, address _devFund)
        public
        ERC20("Sundae Finance", "STRAW")
    {
        _mint(msg.sender, 1 ether); // mint 1 STRAW Share for initial pools deployment
    ...
    /**
     * @notice distribute to NFT pool (only once) new function
     */
    function distributeNFTReward(address _communityFund) external onlyOperator {
        require(!NFTPoolDistributed, "only can distribute once");
        require(_communityFund != address(0), "!_communityFund");
        NFTPoolDistributed = true;
        _mint(_communityFund, COMMUNITY_FUND_POOL_ALLOCATION);
    }
