...
contract AVAcadoClub is ERC721, Ownable {
    using Strings for uint256;
    ...
    // URI
    string public uriPrefix = "ipfs://QmWUdqMhGQgMu5Ug9sQvyHdH5i93npXNR286qp1p4557mD/";
    string public uriSuffix = ".json";
    
    // Cost
    uint256 public presaleCost = 0.5 ether;
    uint256 public whitelistsaleCost = 0.7 ether;
    ...
    // Constructor
    constructor(bytes32 _whitelistsaleMerkleTreeRoot) ERC721("AVAcadoClub", "AVAC") {
        whitelistsaleMerkleTreeRoot = _whitelistsaleMerkleTreeRoot;
    }
    ...
    function presaleMint(uint256 _mintAmount) public payable presaleMintCompliance(_mintAmount) {
        ...
        require(presale == true && whitelistsale == false, "MSG: Presale is not live yet");
    ...
    function whitelistsaleMint(uint256 _mintAmount, bytes32[] memory _merkleTreeProof) public payable whitelistsaleMintCompliance(_mintAmount, _merkleTreeProof) {
        require(!paused, "MSG: The contract is paused");
        require(presale == false && whitelistsale == true, "MSG: Whitelistsale is not live yet");

        require(msg.value >= whitelistsaleCost * _mintAmount, "MSG: Insufficient funds");
        
        _whitelistsaleMintLoop(msg.sender, _mintAmount);
    }

    // Functions: Whitelistsale Mint Helpers
    modifier whitelistsaleMintCompliance(uint256 _mintAmount, bytes32[] memory _merkleTreeProof) {
        ...
        require(ownerMintedCount + _mintAmount <= nftWhitelistsalePerAddressLimit, "MSG: Max NFT per address exceeded for whitelistsale");

        require(supply.current() + _mintAmount <= maxSupplyLimit, "MSG: Max supply exceeded");
        _;
    }

    function _whitelistsaleMintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            supply.increment();
    ...
    function publicsaleMint(uint256 _mintAmount) public payable publicsaleMintCompliance(_mintAmount) {
        require(!paused, "MSG: The contract is paused");
        require(presale == false && whitelistsale == false, "MSG: Publicsale is not live yet");
    ...
    function withdraw() public payable onlyOwner {
        (bool os, ) = payable(0x4dfD1bE1E10cD8EC104f332e721253686080f638).call{value: address(this).balance}("");
        require(os);
    }
}