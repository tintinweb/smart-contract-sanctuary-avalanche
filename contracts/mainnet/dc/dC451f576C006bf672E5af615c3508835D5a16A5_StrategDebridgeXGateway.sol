...
contract StrategDebridgeXGateway is Ownable, IStrategXGateway {
    ...
    constructor(
        ...
        address pool = ICurvePoolToken(lptoken).minter();
    ...
    function _change3CRV(bool _in, address _token) internal {
        uint256 coinsIdx;
        address[] memory underlyingCoins = _3CRV.underlying_coins();
        for (uint256 i = 0; i < underlyingCoins.length; i++) {
            if(underlyingCoins[i] == _token) {
                coinsIdx = i;
            }
        }
        if(_in) {
            uint256 amount = IERC20(_token).balanceOf(address(this));
            uint256[] memory uamounts = new uint256[](underlyingCoins.length);
            for (uint128 i = 0; i < underlyingCoins.length; i++) {
                if(underlyingCoins[i] == _token) {
                    uamounts[i] = amount;
                } else {
                    uamounts[i] = 0;
            ...
            IERC20(_token).approve(address(_3CRV), amount);
    ...
    function _receiveXChainMigration(
        ...
        uint256 assetBal = IERC20(_asset).balanceOf(_asset);
