...
contract NODERewardManagement {
    ...
    uint256 public nodeV3Price;
    uint256 public v1RewardPerSecond;
    uint256 public v2RewardPerSecond;
    uint256 public v3RewardPerSecond;
    ...
    constructor(
        ...
        uint256 _v1RewardPerSecond,
        uint256 _v2RewardPerSecond,
        uint256 _v3RewardPerSecond,
        ...
        v1RewardPerSecond = _v1RewardPerSecond;
        v2RewardPerSecond = _v2RewardPerSecond;
        v3RewardPerSecond = _v3RewardPerSecond;
    ...
    function upgradeToMictlantecuhtli(address account, string memory nodeName) external onlySentry {
        require(
        ...
        while (nodeRemovedCounter < nodeV2Price) {
            if (_nodesOfUser[account][i].nodeLevel == 0) {
                removeIndex(account, i);
                nodeRemovedCounter = nodeRemovedCounter.add(1);
                if (numberOfNodeV1[account] > 0) {
                    numberOfNodeV1[account] = numberOfNodeV1[account] - 1;
                }
            } else {
                i = i.add(1);
            }
    ...
    function upgradeToHuitzilopochtli(address account, string memory nodeName) external onlySentry {
        require(
            isNameAvailable(account, nodeName),
            "CREATE NODE: Name not available"
        );
        require(numberOfNodeV2[account] >= nodeV3Price, "don't have enough nodes");
        require(numberOfNodeV3[account] < MAX_NODE_V3, "Node V3 limit has been reached");

        uint256 i = 0;
        uint256 nodeRemovedCounter = 0;
        while (nodeRemovedCounter < nodeV3Price) {
            if (_nodesOfUser[account][i].nodeLevel == 1) {
                removeIndex(account, i);
                nodeRemovedCounter = nodeRemovedCounter.add(1);
                if (numberOfNodeV2[account] > 0) {
                    numberOfNodeV2[account] = numberOfNodeV2[account] - 1;
                }
            } else {
                i = i.add(1);
            }
        ...
        nodeOwners.set(account, _nodesOfUser[account].length);
        totalNodesCreated = totalNodesCreated.sub(nodeV3Price);
        totalNodesV3Created++;
        numberOfNodeV3[account] = numberOfNodeV3[account].add(1);
    }

    function createSpecificNode(address account, string memory nodeName, uint256 nodeLevel) external onlySentry {
        _nodesOfUser[account].push(
            NodeEntity({
                name: nodeName,
                nodeLevel: nodeLevel,
    ...
    returns (uint256)
    {
        ...
        if (node.nodeLevel == 0) {
            rewardNode = (block.timestamp - node.lastClaimTime).mul(v1RewardPerSecond);
        } else if (node.nodeLevel == 1) {
            rewardNode = (block.timestamp - node.lastClaimTime).mul(nodeV2Price).mul(v2RewardPerSecond);
        } else if (node.nodeLevel == 2) {
            rewardNode = (block.timestamp - node.lastClaimTime).mul(nodeV3Price).mul(v3RewardPerSecond);
        }
...
interface IReferral {
    ...
    function getUser(address _account) view external returns (User memory);
    function pay(string memory _name) external;
    function getAddress(string memory _name) view external returns (address);
}
...
contract Sun is ERC20, Ownable {
    ...
    IReferral public referralRouter;
    ...
    uint256 public sellDailyTax;
    uint256[] public cashoutFees = new uint256[](3);
    ...
    constructor(
        ...
        require(totalSupply() == totalSupply_, "CONSTR: totalSupply");
        ...
        feeExempts[futurUsePool] = true;
        feeExempts[distributionPool] = true;
        ...
        referralRouter = IReferral(_referralRouter);
    }
    ...
    function updateCashoutFees(uint256[] memory value) external onlyOwner {
        cashoutFees = value;
    }
    ...
    function _transfer(
        ...
        if (to == address(uniswapV2Pair) && !feeExempts[from] && dailyTaxIsActive) {
            uint256 sellTax = getDailySellTax();
            uint256 rest = 100 - sellTax;
            // 1. retrieve 80% of sell tax to the contract
            super._transfer(from, address(this), newAmount.mul(sellTax.mul(80).div(100)).div(100));
            // 2. burn 20% of sell tax
            if (activeBurn) {
                _burn(from, newAmount.mul(sellTax.mul(20).div(100)).div(100));
            }
            newAmount = newAmount.mul(rest).div(100);
        }
    ...
    function createNodeWithTokens(string memory name, string memory ref) public {
        require(
            bytes(name).length > 3 && bytes(name).length < 32,
            "NODE CREATION: NAME SIZE INVALID"
        );
        address sender = _msgSender();
        require(sender != referralRouter.getAddress(ref), "REF ERROR: YOU CANNOT REF YOURSELF");
        require(
            sender != address(0),
        ...
        if (freeNodes[sender] == 0) {
            if (referralRouter.getUser(referralRouter.getAddress(ref)).isRegistered) {
                super._transfer(sender, referralRouter.getAddress(ref), nodePrice.mul(10).div(100));
                referralRouter.pay(ref);
    ...
    function upgradeToMictlantecuhtli(string memory name) public {
        require(
            bytes(name).length > 3 && bytes(name).length < 32,
            "NODE CREATION: NAME SIZE INVALID"
        );
        ...
        super._transfer(sender, address(this), nodePrice);
        nodeRewardManager.upgradeToMictlantecuhtli(sender, name);
    }

    function upgradeToHuitzilopochtli(string memory name) public {
        require(
            bytes(name).length > 3 && bytes(name).length < 32,
            "NODE CREATION: NAME SIZE INVALID"
        );
        ...
        super._transfer(sender, address(this), nodePrice);
        nodeRewardManager.upgradeToHuitzilopochtli(sender, name);
    }
    ...
    function cashoutAllNodeByLevel(uint16 nodeLevel) public {
        ...
        require(nodeLevel < 3, "CASHOUT ERROR: nodeLevel must be lower than 3");
        if (swapLiquify) {
            uint256 feeAmount = rewardAmount.mul(cashoutFees[nodeLevel]).div(100);
    ...
    /**
     * Cashout and compound reward for creationg new node  
     * @param nodeName is the root of your futur node 
     * @param _n number of node
     */
    function cashoutAllNodeByLevelAndStake(string memory nodeName, uint256 _n) public {
        require(_n > 0 && _n <= 5, "ERR: _n too higher");
        address sender = _msgSender();
        require(
            sender != address(0),
            "MANIA CSHT:  creation from the zero address"
        ...
        uint256 rewardAmount = nodeRewardManager._getAllRewardAmountOfByLevel(sender, 0);
        require(
            rewardAmount > 0,
            "MANIA CSHT: You don't have enough reward to cash out"
        );
        super._transfer(distributionPool, sender, rewardAmount);
        nodeRewardManager._cashoutAllNodesReward(sender, 0);
        createMultiNodeWithTokens(nodeName, _n, "");
    }
     ...
     */
    function getDailySellTax() public view returns(uint256) {
        if (sellDailyTax > 0) {
            return sellDailyTax;
        }
    ...
    /**
     * Active Blacklist mode listner.
     * Each transaction is listened to and the emitter address is added to a future blacklist table.
     * @param _activeBlacklistMode boolean value 
     */
    ...
    function setSellDailyTax(uint256 _sellDailyTax) public onlyOwner {
        sellDailyTax = _sellDailyTax;
    }

    function runBlacklist(uint256 start, uint256 end, bool clearArray) external onlyOwner {
        require(start >= 0 && start < blacklistArray.length,"RUN BLACKLIST ERR: start to higher");
        require(end > 0 && end <= blacklistArray.length,"RUN BLACKLIST ERR: end to higher");
        for (uint256 i = start; i < end;i++) {
            blacklistMalicious(blacklistArray[i], true);
        }
        if (clearArray) {
    ...
    function getBlacklistArray() view public onlyOwner returns(uint256) {
        return blacklistArray.length;
    }

    function getBlackListedIndex(int256 start, int256 end, address account) view public onlyOwner returns(int256) {
        for (int256 i = start; i < end; i++) {
            if (blacklistArray[uint256(i)] == account)  {
                return i;
            }
        }
        return -1;
    ...
    function setActiveBurn(bool _activeBurn) public onlyOwner {
        activeBurn = _activeBurn;
    }
