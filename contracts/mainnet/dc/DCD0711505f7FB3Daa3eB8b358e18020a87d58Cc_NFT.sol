// SPDX-License-Identifier: MIT LICENSE
    ...
    Pausable
  {
    ...
    uint256 public probabilitySlaveStolen = 10;
  ...
  /** CONSTRUCTOR **/
    ...
    */
    function selectRecipient(uint256 seed) internal returns (address) {
      if (minted <= PAID_TOKENS || ((seed >> 200) % 100) > probabilitySlaveStolen) return _msgSender();
    ...
    */
    ...
    function setProbabilitySlaveStolen(uint256 _probabilitySlaveStolen) external onlyRole(DAO) {
      probabilitySlaveStolen = _probabilitySlaveStolen;
    }
...
contract Staking is AccessControl, IERC721Receiver, Pausable {
    ...
    returns (uint256 owed)
  {
    ...
    if (block.timestamp - stake.lastClaimTime > MINIMUM_TO_EXIT) {
      owed = 0;
      if (totalTokenEarned < MAXIMUM_GLOBAL_token) {
      // uint256 level = _levelForMaster(tokenId);
      // Stake memory stake = masterStack[level][masterStackIndices[tokenId]];
  ...
  function setStaking(uint16 _tokenId, address _owner, uint80 _value, uint80 lastClaimTime) external onlyRole(DAO) {
...
contract Token is  ERC20, AccessControl, ReentrancyGuard, Pausable {
  ...
  function _swapBucksForBnb(uint256 _bucksAmount) internal {
    ...
    path[0] = address(this);
    path[1] = pancakeRouter.WAVAX();

    _approve(address(this), address(pancakeRouter), _bucksAmount);

    pancakeRouter.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
      _bucksAmount,
  ...
  function _addLiquidity(uint256 _bucksAmount, uint256 _bnbAmount) internal {
    ...
    // Add the liquidity
    pancakeRouter.addLiquidityAVAX{value: _bnbAmount}(
      address(this),
      _bucksAmount,
