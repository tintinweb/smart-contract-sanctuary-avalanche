...
contract APRHelper is Initializable, OwnableUpgradeable {
    ...
    function getMultipleAPRforLockerlsInAdditionalReward(uint256 feeAmount, address[] calldata lps, address[] calldata inputRewardTokens)
        public
        view
        returns (uint256[] memory APRs, address[] memory rewardTokens)
    {
        uint256 length = lps.length;
        APRs = new uint256[](inputRewardTokens.length);
        rewardTokens = new address[](inputRewardTokens.length);
        for (uint256 i; i < length; ++i) {
            (uint256[] memory APR, address[] memory rewardToken) = getAPRforLockerInAdditionalReward(lps[i], feeAmount);
    ...
    function getAdditionalRewardsInUSDPerYear(address lp) public view returns (uint256[] memory USDPerYear, address[] memory rewardTokens) {
        uint256 _pid = IMasterPlatypusv4(masterPlatypus).getPoolId(lp);
    ...
    function getAPRforPTPPoolInAdditionalReward(address lp) public view returns (uint256 APR) {
        (uint256[] memory APRs, address[] memory tempRewardToken) = getAPRforPTPPoolInAdditionalRewardArray(lp);
    ...
    function getAPRforPTPPoolInAdditionalRewardArray(address lp) public view returns (uint256[] memory APR, address[] memory rewardToken) {
        (uint256[] memory USDPerYear, address[] memory tempRewardToken) = getAdditionalRewardsInUSDPerYear(lp);
