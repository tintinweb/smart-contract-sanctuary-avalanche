...
/*
 * This contract is used to distribute dividends to users that allocated xSteak here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xSteak allocators during the current cycle
 *
    ...
    IDividendsV2
{
    ...
    address public immutable xSteakToken; // xSteakToken contract
    ...
    constructor(address xSteakToken_, uint256 startTime_) {
        require(xSteakToken_ != address(0), "zero address");
        xSteakToken = xSteakToken_;
    ...
    modifier xSteakTokenOnly() {
        require(
            msg.sender == xSteakToken,
            "xSteakTokenOnly: caller should be xSteakToken"
        );
        _;
    }
    ...
    function allocate(
        ...
        uint256 amount,
        bytes calldata /*data*/
    ) external override nonReentrant xSteakTokenOnly {
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xSteak is allocated or initial distribution has not started yet
        if (
            totalAllocation == 0 ||
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXSteakAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXSteakAllocation.mul(accDividendsPerShare).div(1e18).sub(
                user.rewardDebt
            )
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXSteakAllocation.mul(accDividendsPerShare).div(
            1e18
        );
