...
contract airDrop is Ownable{
    ...
    function airdropNodes(address _address, string[] memory _names, uint256 _nodes)public onlyOwner{
        require(_address != address(0), "Airdrop: Invalid Address");
        require(_names.length == _nodes, "Airdrop: Invalid Names length");
        // for(uint256 i = 0; i < _names.length; i+= 1){

            for(uint256 j = 0; j < _nodes; j+= 1){
                nodeManager.createNode(_address, _names[j], nodeValue);
                refPool.addNodeForReflections(_address);
            }
    }
