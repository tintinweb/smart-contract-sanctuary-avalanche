...
contract  DarkForest  is Ownable,RoyaltiesAddon,ERC2981
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY =5000;
    uint public _BATCHSUPPLY = 1500;

    uint public maxQuantity = 10;
    uint256 public price = 5 ether;

    uint private tokenId=501;
    uint private tokenIdAirdrop=1;

    uint public _revealId = 0;
  ...
  uint public status = 0; //0-pause 1-Whitelist 2-Public
    ...
    constructor() ERC721("Dark Forest", "DF")  {
    _uriBeforeRevel = "https://gateway.pinata.cloud/ipfs/QmXFSY4FCpWD9BpxaR46F9Y6cgU7siCSMLbDuYiKkQRGCr";
    _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmYzcDnqsTzeJmrfwVkpTeUZdfh551hu95VNk6VddZ8mty";
    }
  ...
  function setBaseURI(string memory _baseURI) external onlyOwner {
    ...
    function setBatchSupplyLimit(uint _supply) external onlyOwner() {
        _BATCHSUPPLY = _supply;
    }
    function setMaxQtPerWallet(uint256 _quantity) public onlyOwner {
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
        ...
        require(tokenId+chosenAmount-1<=_BATCHSUPPLY ,"Quantity must be lesser then Batch Supply");
    ...
    function tokensOfOwner(address _owner) public view returns (uint256[] memory)
    {
     ...
     function airDrop(address a, uint amount)public onlyOwner{
      ...
      function airDropFirst500(address a, uint amount)public onlyOwner{
        require(tokenIdAirdrop + amount <= 500, "reached max supply");
        for (uint i = 0; i < amount; i++) {
            _safeMint(a, tokenIdAirdrop);
            tokenIdAirdrop++;
        }
    }
   ...
   function setStatus(uint256 s)external onlyOwner{
    ...
    uint256 public itemPriceWhiteList = 3.5 ether;
    ...
    function purchaseWhiteListTokens(uint256 _howMany, bytes32[] calldata _proof) external payable {
        ...
        require(totalSupply()+_howMany<=_TOTALSUPPLY,"Quantity must be lesser then MaxSupply");
        require(tokenId+_howMany-1<=_BATCHSUPPLY,"Quantity must be lesser then Whitelist MaxSupply");
