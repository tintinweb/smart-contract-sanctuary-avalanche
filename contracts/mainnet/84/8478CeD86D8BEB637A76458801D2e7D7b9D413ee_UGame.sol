...
contract UGame is IUGame, Ownable, ReentrancyGuard, Pausable {
  ...
  uint256 public MAX_GEN0_TOKENS = 12_000;
  ...
  /** MINTING FUNCTIONS */
  /** 
   * Mint Fighter & Yakuza NFTs with AVAX.
  ...
  function _getOwnerOfFYToken(uint256 tokenId) public view returns(address ownerOf) {
    ...
    if (traits.isFighter) { // Fighter
      ...
      if (ringsInWallet < RING_DIMINISHING_FROM) {
        ...
        for (uint256 i = 1; i <= remainingRings; i++) {
          // Now add the rest of the ring rewards
          uint256 reduceBy = RING_DAILY_BLOOD_RATE * i * 5/100;
          if (RING_DAILY_BLOOD_RATE >= reduceBy) {
            rewardsFromRings += traits.level * (RING_DAILY_BLOOD_RATE - reduceBy);
          } else {
            break;
          }
        }
