...
contract LightsDAO is ERC20Detailed, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 119150881 * 10**(DECIMALS-2);
    uint256 private constant PRESALE_AMOUNT = 89150881 * 10**(DECIMALS - 2);

    uint256 public constant liquidityFee = 5;
    uint256 public constant Treasury = 5;
    uint256 public constant sellFee = 5;
    uint256 public totalFee =
        liquidityFee.add(Treasury);
    ...
    uint256 public _rebaseLock;
    uint256 public rebaseRate = 354778;
    ...
    uint256 public MAX_SUPPLY = 4 * 10**9 * 10**DECIMALS;
    ...
    constructor(address _presale, address _treasury) ERC20Detailed("LightsDAO", "LIGHTSDAO", uint8(DECIMALS)) {
        router = IDEXRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4); //Joe 0x60aE616a2155Ee3d9A68541Ba4544862310933d4
        ...
        autoLiquidityReceiver = TreasuryReceiver;
        ...
        _isFeeExempt[_presale] = true;

        _basicTransfer(TreasuryReceiver, _presale, PRESALE_AMOUNT);

        _lastRebasedTime = block.timestamp + 1 days;
        _rebaseLock = block.timestamp + 2 days;

        _transferOwnership(TreasuryReceiver);
        emit Transfer(address(0x0), TreasuryReceiver, _totalSupply);
    }

    function updateBlacklist(address _botAddress, bool _flag) public onlyOwner{
        require(isContract(_botAddress), "Only contract address");
        blacklist[_botAddress] = _flag;
    }

    function setRebaseAndMaxSupply(uint256 _rebaseRate, uint256 _maxSupply) external onlyOwner {
        require(block.timestamp > _rebaseLock, "Locked");
        require(_rebaseRate <= rebaseRate, "Rebase Rate cannot be increased");
        require(_maxSupply >= _totalSupply, "Max supply cannot be lower than total supply");
        rebaseRate = _rebaseRate;
        MAX_SUPPLY = _maxSupply;
        _rebaseLock = block.timestamp + 1 days;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountAVAXLiquidity = amountAVAX
            ...
            .div(totalAVAXFee)
            .div(2);
        uint256 amountAVAXTreasury = amountAVAX.mul(Treasury).div(
            totalAVAXFee
        );

        safeTransferAVAX(TreasuryReceiver, amountAVAXTreasury);
