...
interface IXswapFarm {
    ...
    function swappers(address _token) external view returns (address);

    function uniRouterAddress(uint256 _index) external returns (address);

    function getTokenId(address _token) external returns (uint256);

    function getBUSDToTokenPath(uint256 _index) external returns (address[] memory);
    function getTokenToBUSDPath(uint256 _index) external returns (address[] memory);

    function ELLIPSIS_3EP_POOL() external view returns (address);
...
interface ISwapper {
    function fromBUSD(uint256 _amount) external returns (uint256);
    function toBUSD(uint256 _amount) external returns (uint256);
    function BUSD() external returns (address);
    function token() external returns (address);
}

contract StrategyPangolinLPAVAX is Ownable, ReentrancyGuard {
    ...
    event Withdraw(uint256 _wantAmt, uint256 wantAmtToRemove, uint256 busdAmt);
    ...
    constructor(
        ...
        WETH = IXRouter02(uniRouterAddress).WAVAX();
    ...
    function BUSDToWant(uint256 _wantAmt) internal returns (uint256) {
        ...
        IERC20(BUSD).safeTransferFrom(address(msg.sender), address(this), _wantAmt);
        ...
        // Swap to token0
        ISwapper swapper0 = ISwapper(IXswapFarm(nativeFarmAddress).swappers(token0Address));
        IERC20(BUSD).safeIncreaseAllowance(address(swapper0), _wantAmt.div(2));
        swapper0.fromBUSD(_wantAmt.div(2));

        // Swap to token1
        ISwapper swapper1 = ISwapper(IXswapFarm(nativeFarmAddress).swappers(token1Address));
        IERC20(BUSD).safeIncreaseAllowance(address(swapper1), _wantAmt.div(2));
        swapper1.fromBUSD(_wantAmt.div(2));
    ...
    function wantToBUSD(uint256 _wantAmt) internal returns (uint256) {
        ...
        // Withdraw from farm
        IXswapFarm(farmContractAddress).withdraw(pid, _wantAmt, address(this));
        ...
        IERC20(wantAddress).safeIncreaseAllowance(uniRouterAddress, _wantAmt);
        ...
        // Swap token0 to BUSD
        ISwapper swapper0 = ISwapper(IXswapFarm(nativeFarmAddress).swappers(token0Address));
        IERC20(token0Address).safeIncreaseAllowance(address(swapper0), token0Amt);
        swapper0.toBUSD(token0Amt);

        // Swap token1 to BUSD
        ISwapper swapper1 = ISwapper(IXswapFarm(nativeFarmAddress).swappers(token1Address));
        IERC20(token1Address).safeIncreaseAllowance(address(swapper1), token1Amt);
        swapper1.toBUSD(token1Amt);
    ...
    function _farm() internal {
        ...
        if (wantAmt > 0) {
            ...
            IXswapFarm(farmContractAddress).deposit(pid, wantAmt, address(this));
        }
    }
    ...
    function withdraw(uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        // Event
        emit Withdraw(_wantAmt, wantAmtToRemove, busdAmt);
    ...
    function earn() public nonReentrant {
        ...
        // Harvest farm tokens
        IXswapFarm(farmContractAddress).withdraw(pid, 0, address(this));
