...
contract BunnyToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 BUNNY-WAVAX LP -> Bunny pool
    uint256 public constant INITIAL_BUNNY_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the Bunny ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Bunny Token", "BUNNY") {
        // Mints 1 BUNNY to contract creator for initial pool setup
    ...
    function _getBunnyPrice() internal view returns (uint256 _BunnyPrice) {
        try IOracle(BunnyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bunny: failed to fetch Bunny price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _BunnyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_BunnyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBunnyOracle(address _BunnyOracle) public onlyOperatorOrTaxOffice {
        require(_BunnyOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BUNNY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BUNNY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBunnyPrice = _getBunnyPrice();
            currentTaxRate = _updateTaxRate(currentBunnyPrice);
            if (currentBunnyPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_BunnyPool != address(0), "!_BunnyPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_BunnyPool, INITIAL_BUNNY_POOL_DISTRIBUTION);
    }
