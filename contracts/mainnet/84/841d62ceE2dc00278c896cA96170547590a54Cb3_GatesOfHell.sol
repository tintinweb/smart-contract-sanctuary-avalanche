...
{  
  ...
  {
    require(_exists(tokenId), "Gates of Hell: Unexistant Gate to Hell...");
...
contract GatesOfHell is ERC721OnChainMetadata, Ownable, ERC2981 {
    ...
    constructor() ERC721OnChainMetadata("Gates of Hell", "GOHELL") {
        _addValue(
            _contractMetadata,
            key_contract_name,
            abi.encode("Gates of Hell")
        );
        _addValue(
            _contractMetadata,
            key_contract_description,
            abi.encode("Ready to connect with Hell?")
        );
        _setValue(
            1,
            key_token_name,
            abi.encode(
                string.concat("Gate to Hell #", Strings.toString(1))
            )
        );
        _setValue(
            1,
            ...
            abi.encode(
                "Want to be possessed by the Devil himself? Mint this no-roadmap collection and give me your soul..."
            )
        );
        _setValue(
            1,
    ...
    modifier isOpen() {
        require(block.timestamp > 1659027600); // 5pm utc
        _;
    }
    ...
    function mint(uint256 quantity) public payable isOpen {
        ...
        require(
            msg.value == 0.0666 ether,
            "Gates of Hell: Please send the correct value"
        );
        require(
            !peopleInHell[_msgSender()],
            "Gates of Hell: You have minted an nft already."
        );
        _setValue(
            tokenId,
            key_token_name,
            abi.encode(
                string.concat("Gate to Hell #", Strings.toString(tokenId))
            )
        );
        _setValue(
            tokenId,
        ...
        public
    {
        require(
            ...
            "Gates of Hell: You are not the owner of this NFT"
        );
