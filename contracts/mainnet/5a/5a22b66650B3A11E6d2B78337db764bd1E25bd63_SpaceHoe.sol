...
// 0x78483B2Adf05EdF3210E90d911A5D1E3ab57D15c
...
contract SpaceHoe is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public constant HOE_PRICE_WHITELIST = 1 ether;
    uint256 public constant HOE_PRICE_AVAX = 1.2 ether;

    uint256 public constant WHITELIST_HOES = 2000; 

    uint256 public constant NUM_HOES = 6969;

    // metadata URI
    string public BASE_URI;

    // mint tracking
    uint256 public hoesMintedWithAVAX;
    uint256 public hoesMintedWhitelist;
    uint256 public hoesMinted = 0;
    ...
    event onHoeCreated(uint256 tokenId);

    constructor(address payable _DAO) ERC721("Space Hoes", "SPACE-HOE") {
        DAO = _DAO;
    }
    ...
    function _createHoe(address to, uint256 tokenId) internal {
        require (hoesMinted <= NUM_HOES, "cannot mint anymore hoes");
        _safeMint(to, tokenId);

        emit onHoeCreated(tokenId);
    }

    function _createHoes(uint256 qty, address to) internal {
        for (uint256 i = 0; i < qty; i++) {
            hoesMinted += 1;
            _createHoe(to, hoesMinted);
        }
    }
    /**
     * @dev Whitelist minting
     * We implement a hard limit on the whitelist hoes.
     */
    function mintWhitelist(uint256 qty) external payable whenNotPaused {
        ...
        require(whitelist[_msgSender()]==true, "this address does not have permission");

        // check more advanced requirements
        require((hoesMintedWhitelist + qty) <= WHITELIST_HOES, "you can't mint that many right now");

        // check price
        require(msg.value >= HOE_PRICE_WHITELIST * qty, "not enough AVAX");

        hoesMintedWhitelist += qty;
        whitelistClaimed[_msgSender()] += qty;

        // mint hoes
        _createHoes(qty, _msgSender());
    }

    /**
     * @dev minting
     */
    function mintHoeWithAVAX(uint256 qty) external payable whenNotPaused {
        require (mintingStartedAVAX(), "cannot mint right now");
        require ((hoesMintedWithAVAX + qty) <= (NUM_HOES - hoesMintedWhitelist), "you can't mint that many right now");

        // calculate the transaction cost
        uint256 transactionCost = HOE_PRICE_AVAX * qty;
        require (msg.value >= transactionCost, "not enough AVAX");

        hoesMintedWithAVAX += qty;

        // mint hoes
        _createHoes(qty, _msgSender());
    }

    // Returns information for multiples hoes
    function batchedHoesOfOwner(address _owner, uint256 _offset, uint256 _maxSize) public view returns (HoeInfo[] memory) {
        if (_offset >= balanceOf(_owner)) {
            return new HoeInfo[](0);
        }
        ...
        HoeInfo[] memory hoes = new HoeInfo[](outputSize);
