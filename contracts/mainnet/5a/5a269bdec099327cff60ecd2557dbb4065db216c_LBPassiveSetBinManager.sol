...
contract LBPassiveSetBinManager is Ownable, AutomationCompatibleInterface {
    ...
    /// @notice should be set to the chainlink automation register
    /// @param _keeper new keeper address.
    function setKeeper(address _keeper) external onlyManager {
        require(address(_keeper) != address(0), "Manager: Address 0");
        keeper = _keeper;
    }

    /// @notice Updates fee recipient for gas reimbursement
    /// @param _gasCaller address.
    function setGasCaller(address _gasCaller) external onlyManager {
    ...
    function setMinimumAmount(uint256 _minAmount) external onlyManager {
