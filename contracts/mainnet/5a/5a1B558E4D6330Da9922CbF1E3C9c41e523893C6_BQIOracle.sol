...
contract BQIOracle is Ownable, IPriceFeed {
  ...
  function setParam(IPriceFeed _base, address _deposit, uint _underlyingDecimals) external onlyOwner {
    base = _base;
    deposit = _deposit;
  ...
  function fetchPrice_v() external override view returns (uint) {
    return IQIToken(deposit).exchangeRateStored().mul(base.fetchPrice_v()).div(wad);
  }
