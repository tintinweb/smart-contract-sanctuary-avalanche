...
contract arb is Ownable{
    ...
    function swap_eyqxmvatdl(address[] memory path , address pairA, address pairB, uint256 amount, uint256 amount_outMin) external onlyOwner() {      
        ...
        assert(IWAVAX(path[0]).transfer(pairB, amount));
        ...
        token0 = IUniswapV2Pair(pairB).token0();
        token1 = IUniswapV2Pair(pairB).token1();
        reserveIn = path[1] == token0 ? reserve0 : reserve1;
        reserveOut = path[2] == token1 ? reserve1 : reserve0;
        
        uint256 amountOutFinal = calculate(amountOut, reserveIn, reserveOut);

        (amount0Out, amount1Out) = path[1] == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
        
        assert(IWAVAX(path[1]).transfer(pairB, amountOut));
        IUniswapV2Pair(pairB).swap(amount0Out, amount1Out, address(this), new bytes(0));
        
        require(amountOutFinal >= amount_outMin);
