...
contract SpecialCrew is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC2981, ClampedRandomizer {
    constructor() 
        ERC721("Special Crew", "SC") ClampedRandomizer(maxSupply) {
    ...
    uint public maxPerNonTeam = 2;
    uint256 public price = 0.00 ether;
    uint public whitelistTime= 1671822000; // Fri Dec 23 2022 19:00:00 GMT+0000
    uint public publicTime= 1671825600; //Fri Dec 23 2022 20:00:00 GMT+0000
    address public royaltyAddress=0x1e6fEBcC2Daa1A4EFC37c3a034668b597512c8C5;
    uint96 public royalty=969;
    string public baseURI = "";
    string public unrevealedURI = "ipfs://QmXLp8gJ4r7L1uxNNnQrDScdE7VFx3Epmjio6MFC88TG92/hidden.json";
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        //mint start
        require(block.timestamp >= whitelistTime, "Its not time to go Special Crew yet");
        
        //supply limits
        uint supply = totalSupply();
        require(supply + amount <= maxSupply, "No more Special Crew left for you");

        //transaction limits
        require(amount > 0,"you want to mint more than just air right?");
        require(amount <= maxPerTx, "Thats too many Special Crews at once");

        //mint limits
        require(whiteListedCount[msg.sender] < maxPerNonTeam, "You can have only 1 Special Crews");
        

        
        if (price > 0) 
        {
            require(msg.value >= price * amount, "Send more money plz ser");
        }
        ...
        {
            require(whiteListed[msg.sender] ==true, "Silly Special Crew are for WL only right now");
        ...
        if (block.timestamp >= publicTime)
        {
            require(publicCount[msg.sender] < maxPerNonTeam, "You can have only 1 Special Crewd");   
        }
    ...
    {
        uint supply = totalSupply();
        require(supply + 1 <= maxSupply, "No more Special Crew left for you");
        uint256 tokenId=_genClampedNonce()+1;//+1 since we dont want to deal with token id 0
        _isMinted[tokenId]=true;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(tokenId.toString(), ".json")));
    ...
    function setMaxPerNonTeam(uint _maxPerNonTeam) public onlyOwner {
    maxPerNonTeam = _maxPerNonTeam;
    }
