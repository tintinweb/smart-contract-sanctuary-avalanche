...
contract Vault is VaultBase, AssetSpenderRole, MultichainRouterRole {
    ...
    {
        require(
            !_forVariableToken || variableTokenEnabled,
            "token-not-enabled"
        );
    ...
    function cleanup(address _tokenAddress, uint256 _tokenAmount) external onlyManager {
        require(
            _tokenAddress != asset, // Vault asset cleanup is not allowed
            "vault-asset"
        );
