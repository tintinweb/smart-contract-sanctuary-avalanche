...
contract TournamentERC20 is Ownable, ReentrancyGuard, ITournament {
    ...
    constructor (address _wl, address _token, uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _gladShare, uint256 _burnShare, bool _nonBurnable, address _treasury) {
    ...
    function buy() external payable nonReentrant {
        ...
        if (BURN_SHARE >  0) {
            if (nonBurnable) {
                rewardToken.transfer(address(0), transferSize * BURN_SHARE / 10000);
            } else {
