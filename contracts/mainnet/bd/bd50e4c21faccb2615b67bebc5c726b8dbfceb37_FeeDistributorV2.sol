...
import { IJoePair } from "./traderjoe/IJoePair.sol";
import { IJoeFactory } from "./traderjoe/IJoeFactory.sol";
import { ITreasury } from "./TreasuryV2.sol"; 

interface IFeeDistributor {
	function notify(address token, uint256 amount) external;    
}

contract FeeDistributorV2 is IFeeDistributor, AccessControlled {     
    using SafeERC20Upgradeable for IERC20Upgradeable;
    
    address public dlc;
    address public treasury;
    address public router;
    struct addToLiquidityData {
        bool state;
        uint256 percent;
    }
    mapping(address => addToLiquidityData) public addToLiquidityTokens;
    ...
    function init(address dlc_, address treasury_, address router_) external onlyOperator {
        dlc = dlc_;
        treasury = treasury_;
        router = router_; 
	}

    function setAddToLiquidityToken (
        address token_,	
        bool state_,
        uint256 percent_
    ) external onlyOperator {
        require(percent_ <= 100, '!');
        addToLiquidityTokens[token_] = addToLiquidityData(state_, percent_);
    }
    ...
    function notify(address token_, uint256 amount_) public {
        ...
        if (addToLiquidityTokens[token_].state) {
            uint256 tokenAmountToAdd = amount_ * addToLiquidityTokens[token_].percent / 100;
            uint256 tokenAmountToSwap;
            
            address factory = IJoeRouter02(router).factory();            
            address pair = IJoeFactory(factory).getPair(address(dlc), token_);

            (uint256 reserve0, uint256 reserve1,) = IJoePair(pair).getReserves();
            (uint256 reserveA, uint256 reserveB) = token_ == IJoePair(pair).token0() ? (reserve0, reserve1) : (reserve1, reserve0);
                        
            uint256 dlcAmountToAdd = IJoeRouter02(router).quote(tokenAmountToAdd, reserveA, reserveB);
            
            IERC20Upgradeable(token_).approve(router, amount_);
            
            if (ITreasury(treasury).balanceOfDLC() >= dlcAmountToAdd) {

                ITreasury(treasury).feeDistributorWithdraw(dlcAmountToAdd);                
                
                IERC20Upgradeable(dlc).approve(router, dlcAmountToAdd);
            
                IJoeRouter02(router).addLiquidity(
                    token_, // address tokenA Asset
                    dlc, // address tokenB DLC
                    tokenAmountToAdd, // amountADesired
                    dlcAmountToAdd, // amountBDesired
                    tokenAmountToAdd, // amountAMin
                    dlcAmountToAdd, // amountBMin
                    address(this), // to
                    block.timestamp // deadline
                );

                tokenAmountToSwap = amount_ - tokenAmountToAdd;            
            } else {
                tokenAmountToSwap = amount_;
            }
            ...
            if (tokenAmountToSwap != 0) {                
                ...
                IJoeRouter02(router).swapExactTokensForTokens(
                    tokenAmountToSwap, // uint256 amountIn
