...
contract Token is ERC20, AccessControl, ReentrancyGuard, Pausable {
    ...
    uint256 public minimumSafeFeeBalanceToSwap = 100e18; // TOKEN balance required to perform a swap
    uint256 public minimumLiquidityFeeBalanceToSwap = 150e18; // TOKEN balance required to add liquidity
    uint256 public minimumBNBRewardsBalanceToSwap = 200e18; // TOKEN balance required to add liquidity
    bool public swapEnabled = true;

    // Buying and selling fees
    uint256 public buyingFee = 0; // (/1000)
    uint256 public sellingFeeClaimed = 400; // (/1000)
    uint256 public sellingFeeNonClaimed = 400; // (/1000)
    ...
    uint256 public safeFeeBalance = 0; // TOKEN balance accumulated from fee safe fees
    uint256 public liquidityFeeBalance = 0; // TOKEN balance accumulated from liquidity fees
    uint256 public BNBRewardsFeeBalance = 0; // TOKEN balance accumulated from liquidity fees
        ...
        ERC20("ServerTech", "STK")
    {
        ...
        pancakeTokenBnbPair = IUniswapV2Factory(pancakeRouter.factory())
            .createPair(address(this), pancakeRouter.WAVAX());
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "You do not have enough TOKEN to sell/send them."
        );
        super._beforeTokenTransfer(_from, _to, _amount);
    }

    // Transfers TOKEN from _from to _to, collects relevant fees, and performs a swap if needed
    function _transfer(
        address _from,
        address _to,
        uint256 _amount
        ...
        if (!_exemptFromFees[_from] && !_exemptFromFees[_to]) {
            ...
            if (fees > 0) {
                // Send the TOKEN tokens to the contract
                super._transfer(_from, address(this), fees);

                // Keep track of the TOKEN tokens that were sent
        ...
        if (swapEnabled) {
            ...
            if (
                ...
                // Perform the swap
                _swapTokenForBnb(BNBRewardsFeeBalance);
    ...
    function _swapSafeFeeBalance() internal {
        require(
            safeFeeBalance > minimumSafeFeeBalanceToSwap,
            "Not enough TOKEN tokens to swap for safe fee"
        );

        uint256 oldBalance = address(this).balance;

        // Swap
        _swapTokenForBnb(safeFeeBalance);
    ...
    // Mint new TOKEN tokens to the given address
    function mintDAO(address _to, uint256 _amount) public onlyRole(DAO) {
        _mint(_to, _amount);
    }

    // Burns TOKEN tokens from a given address
    ...
    function grantDEFAULT_ADMIN_ROLE(address _defaultAdminRole)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        grantRole(DEFAULT_ADMIN_ROLE, _defaultAdminRole);
    }

    function revokeDEFAULT_ADMIN_ROLE(address _defaultAdminRole)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        revokeRole(DEFAULT_ADMIN_ROLE, _defaultAdminRole);
    }

    function grantRoleDAO(address _dao) external onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(DAO, _dao);
    }

    function revokeRoleDAO(address _daoToRevoke)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    {
        require(
            _pair != pancakeTokenBnbPair,
            "The TOKEN / BNB pair cannot be removed from automatedMarketMakerPairs"
        );
