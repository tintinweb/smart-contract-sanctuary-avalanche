...
abstract contract GlobalsAndUtils {
    ...
    uint256 internal constant CLAIM_X_TICKET_DURATION = 3 minutes;
...
contract Staking is GlobalsAndUtils, Ownable, ReentrancyGuard, IERC721Receiver {
       ...
       uint256 rangeEnd = userXTicketRange[_winnerAddress][campaignId].end;
        ...
        for (uint256 j = 0; j < _winningTicketIds.length; j++) {
            uint256 winningTicketId = _winningTicketIds[j];
                ...
                if(!ticketIdUsed[campaignId][winningTicketId]){
     ...
     */
    function _pickWinnersWithArray(uint256[] memory winningTickets) public onlyOwner {
    ...
    function _getRandomNumbers(uint256 ranEnd, uint256 start, uint256 end) internal {
        for (uint256 i = start; i < end; i++) {
            winningTicketIds[latestCampaignId].push(uint256(keccak256(abi.encode(randomSeed, i, block.timestamp))) % ranEnd + 1);
        }
    }
