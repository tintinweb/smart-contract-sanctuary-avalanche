...
contract Refrigerator is ERC20("Staked Soda", "sSODA"), Ownable {
    ...
    Soda public soda;
    uint256 public frozenSoda; // SODA pending withdrawal
    ...
    function sodaBalance() public view returns (uint256 balance) {
        return soda.balanceOf(address(this)) - frozenSoda;
    }

    function _unstakeOutput(uint256 _share) internal view returns (uint256 output) {
        uint256 totalShares = totalSupply();
        return _share.mul(sodaBalance()).div(totalShares);
    }
    ...
    function stake(uint256 _amount) external {
        ...
        uint256 totalShares = totalSupply();
        // If no sSODA exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || sodaBalance() == 0) {
            _mint(_msgSender(), _amount);
        } else {
            uint256 share = _amount.mul(totalShares).div(sodaBalance());
            _mint(_msgSender(), share);
        }

        soda.safeTransferFrom(msg.sender, address(this), _amount);
    }

    function quickUnstake(uint256 _share) external {
        // QUICK_UNSTAKE_CONTRIBUTION_PERCENT of the claimable SODA will remain in the refrigerator
        ...
        // QUICK_UNSTAKE_BURN_PERCENT of the claimable SODA is burned
        uint256 amountSpoiled = unstakeOutput.mul(QUICK_UNSTAKE_BURN_PERCENT).div(100);

        // burn staker's share
        _burn(msg.sender, _share);

        soda.safeTransfer(DEAD_ADDRESS, amountSpoiled);
        soda.safeTransfer(msg.sender, output);
    }

    /**
     * @dev _share argument specified in sSODA
     */
    function prepareDelayedUnstake(uint256 _share) external {
        // calculate output and burn staker's share
        uint256 output = _unstakeOutput(_share); //converts sSoda values to soda
    ...
    /**
     * @dev argument specified in SODA, not sSODA
     */
    function claimDelayedUnstake(uint256 _amount) external {
        require(block.timestamp >= unlockTimestamps[_msgSender()], "SODA not yet unlocked");
        ...
        // transfer claim
        soda.safeTransfer(msg.sender, _amount);
    }
