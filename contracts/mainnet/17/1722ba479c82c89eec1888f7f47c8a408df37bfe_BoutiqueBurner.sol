...
// File: contracts/applications/BoutiqueBurner.sol

pragma solidity ^0.8.0;



contract BoutiqueBurner is Ownable {
    ...
    constructor(address _boutique) {
        setMetadata(_boutique);
    }

    function setMetadata(address _boutique) public onlyOwner {
        boutique = _boutique;
    }

    function setTiers(uint256 amount, bool enabled) public onlyOwner {
        tiers[amount] = enabled;
    }

    function burn(uint256[] memory tokenIds) public {
        uint256 amount = tokenIds.length;
        require(tiers[amount], "invalid number of tokens");

        for (uint256 i = 0; i < amount; i++) {
            INft(boutique).transferFrom(
                msg.sender,
                address(0x000000000000000000000000000000000000dEaD),
                tokenIds[i]
            );
            burnedBy[tokenIds[i]] = address(msg.sender);
            burns[msg.sender] += 1;
        }
    }
}