...
contract BavaCompoundPool_UVRTUpgradeable is
    Initializable,
    UUPSUpgradeable,
    BRTERC20Upgradeable
{
    ...
    function withdraw(address _user, uint256 _amount) public onlyWhitelisted {
        ...
        if (_amount > 0) {
            ...
            vaultInfo.lpToken.safeTransfer(address(msg.sender), _amount);
            
            emit Withdraw(_user, bavaPid, _amount);
        }
    }
    ...
    // lock 95% of reward
    function _harvest(address account) private returns(uint256) {
