...
contract StargateModule is BaseModule {
    ...
    uint256 stgSellThreshold = 200 * 10**18; //threshold to harvest in Stg  // TODO: check if the price is added to any oracle data feed, other wise set the threshold with just token amount
    ...
    // TODO: when we have keeper role than it should be both keeper and owner allowed
    function setStgSellThreshold(uint256 newThreshold) onlyOwner external {
        stgSellThreshold = newThreshold;
    }

    function deposit(uint256 amountToken) override external onlyManager {
        IERC20(baseToken).safeTransferFrom(manager, address(this), amountToken);
        _deposit(amountToken);
    }

    function harvestAndWithdraw(address receiver) override external onlyManager returns(uint256) {
        _harvest();
    ...
    function getYieldInfo() override external view returns(uint256) {
        return currentHarvest;
    }

    function withdraw(address receiver, uint256 amount) override external onlyManager returns(uint256) { 
       return _withdraw(receiver, amount);     
    }

    function withdrawAllToManager() override public onlyManager { 
        _withdraw(manager, getBaseTokenAmountFromProtocol());
    }

    function getBaseTokenAmountFromProtocol() override public returns(uint256) {
    ...
    function getLpAmount() external returns (uint256) {
       return _getLpAmount();
    }
    ...
    function _harvest() internal {
        ...
        // TODO: how we can get STG price -- waiting for answer on Discord 
        // if not use formula : (as they do in alpaca)
        // uint256 lpTokenPrice = vault.totalToken()).div(vault.totalSupply(); // total token is number given stable deposited, totalSupply() is supply of lpToken 
        // uint256 underlyingTokenAmount = lpTokenAmount.mul(lpTokenPrice);
    ...
    function _deposit(uint256 amountToken) internal {
        if(lpStaking.pendingStargate(lpStakingPoolId, address(this)) > 0) {
            _harvest();
        }
    ...
    function _getLpAmount() internal returns (uint256) {
...
abstract contract BaseModule is IYieldModule {
    using SafeERC20 for IERC20;
    ...
    modifier onlyManager() {
        require(manager != address(0), "manager not initialized");
    ...
    function initManager(address _manager) override external onlyOwnerOrManger {
        require(manager == address(0), "Init only once");
        manager = _manager;
        IERC20(baseToken).safeApprove(_manager, UINT_MAX); // manager need full access to funds
    }

    function switchManager(address _manager) override external onlyOwnerOrManger {
        require(manager != address(0), "Must be initialized");
        manager = _manager;
        IERC20(baseToken).safeApprove(_manager, UINT_MAX); // manager need full access to funds
    }

    function activate() override external onlyOwnerOrManger {
        active = true;
    }

    function deactivate() override public onlyOwnerOrManger {
        active = false;
    }

    function withdrawAllToManager() virtual override public onlyOwnerOrManger {
        require(true, "must implement");
    }
...
interface IYieldModule {
    ...
    function initManager(address manager) external;
    function switchManager(address _manager) external;
    function baseTokenAddress() external returns(address);
    function harvestAndWithdraw(address receiver) external returns(uint256);
    function getYieldInfo() external returns(uint256);
     ...
     */
    function getBaseTokenAmountFromProtocol() external returns(uint256); 
