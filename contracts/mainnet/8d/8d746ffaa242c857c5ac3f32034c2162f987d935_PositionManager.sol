...
import "./RebalancerUpgradeable.sol";
import "../interfaces/IWETH.sol";

contract PositionManager is RebalancerUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // address => bitmap holding used withdrawal nonces
    mapping(address => BitMapsUpgradeable.BitMap) internal usedNonces;
    ...
    function initialize() external initializer {
        __EIP712_init("PositionManager", "1");
        __ReentrancyGuard_init();

        __RebalancerUpgradeable_init();
    }
    ...
    function depositAndWrapNativeToken(
        ...
        uint256 balanceBefore = IERC20Upgradeable(wethAddress).balanceOf(address(this));

        _validateSig(msg.sender, wethAddress, amount, nonce, deadline, signature, SigAction.Deposit);

        IWETH(wethAddress).deposit{value: amount}();

        uint256 balanceAfter = IERC20Upgradeable(wethAddress).balanceOf(address(this));
    ...
    function depositInPositionManager(
        ...
        require(IERC20Upgradeable(asset).allowance(msg.sender, address(this)) >= amount, "insufficient allowance");
    ...
    function _validateSig(
        ...
        require(!BitMapsUpgradeable.get(usedNonces[_beneficiary], _nonce), "Invalid nonce");
        ...
        BitMapsUpgradeable.set(usedNonces[_beneficiary], _nonce);
    }
...
abstract contract RebalancerUpgradeable is Initializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function __RebalancerUpgradeable_init() internal initializer {
        ...
        assetsListHaveSameLength(tokens, amounts)
    {
        ...
        for (uint256 i = 0; i < length; ) {
            IERC20Upgradeable(tokens[i]).safeTransfer(rebalanceManager, amounts[i]);
        ...
        assetsListHaveSameLength(tokens, amounts)
    {
        ...
        for (uint256 i = 0; i < length; ) {
            IERC20Upgradeable(tokens[i]).safeTransferFrom(rebalanceManager, address(this), amounts[i]);
