...
contract Trading {
	...
	struct Product {
		...
		uint64 volatilityFactor; // Use smaller number for more volatile tokens
	}
	...
	function addProduct(bytes32 productId, Product memory _product) external onlyOwner {
		...
		products[productId] = Product({
			...
			volatilityFactor: _product.volatilityFactor
		});
	...
	function updateProduct(bytes32 productId, Product memory _product) external onlyOwner {
		...
		product.volatilityFactor = _product.volatilityFactor;
	...
	function submitOrder(
		...
		uint256 oracleIndex = IOracle(oracle).addOrder(msg.sender, productId, currency, isLong);
		orders[key] = Order({
			isClose: false,
			size: uint64(size),
	...
	function cancelOrder(
		...
		delete orders[key];
		IOracle(oracle).cancelOrder(productId, order.oracleIndex);
	...
	function settleOrder(
		...
		if (order.isClose) {
				...
				if (pnl < 0) {
					{
						...
						_transferOut(currency, treasury, positivePnl);
						ITreasury(treasury).notifyFeeReceived(currency, positivePnl * 10**(18-UNIT_DECIMALS));
			...
			uint256 priceWithSlippage = _getPriceWithSlippage(price, currency, order.size, product.volatilityFactor, isLong);

			Position storage position = positions[key];

			uint256 averagePrice = (uint256(position.size) * uint256(position.price) + uint256(order.size) * uint256(priceWithSlippage)) / (uint256(position.size) + uint256(order.size));
	...
	function liquidatePosition(
		...
		if (pnl <= -1 * int256(threshold)) {
			...
			_transferOut(currency, treasury, position.margin);
			ITreasury(treasury).notifyFeeReceived(currency, position.margin * 10**(18-UNIT_DECIMALS));
	...
	function _getPriceWithSlippage(
		...
		address currency,
		uint256 size,
		uint64 volatilityFactor,
		bool isLong
	) internal view returns (uint256) {
		address pool = IRouter(router).getPool(currency);
		uint256 poolSize = size + ((IPool(pool).totalSupply() - IPool(pool).openInterest()) / 10**(18 - UNIT_DECIMALS)); // 8 decimals
		uint256 slippage = price*((10**6*size/poolSize)**volatilityFactor);
		if (isLong) {
			return price + slippage/(10**6)**volatilityFactor;
		} else {
			return price - slippage/(10**6)**volatilityFactor;
		}
	}
...
interface IOracle {
	function addOrder(address user, bytes32 productId, address currency, bool isLong) external returns(uint256 index);

    function cancelOrder(bytes32 productId, uint256 index) external;
}
