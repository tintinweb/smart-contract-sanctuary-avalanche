...
contract GelatoCompound is Ownable {
    ...
    address public callFeeRecipient = 0xB0Dd96118594903baCb2746eB228550C10B2b1DA;
    ...
    function checker() external view returns (bool canExec, bytes memory execPayload) {
        for (uint256 i = 0; i < vaults.length(); i++) {
            ...
            canExec = block.timestamp >= strat.lastHarvest() + delay;

            if (canExec) {
                execPayload = abi.encodeWithSelector(IStrategy.harvestWithCallFeeRecipient.selector, callFeeRecipient);
                break;
            }
        }
    }
