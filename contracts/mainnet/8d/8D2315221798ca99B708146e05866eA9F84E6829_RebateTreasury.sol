...
contract RebateTreasury is Ownable {
    ...
    function getXombReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        ...
        uint256 bondPremium = geXbondPremium();
    ...
    function geXbondPremium() public view returns (uint256) {
        uint256 xombPrice = getXombPrice();
        if (xombPrice < 1e18) return 0;

        uint256 xombPremium = xombPrice * DENOMINATOR / 1e18 - DENOMINATOR;
