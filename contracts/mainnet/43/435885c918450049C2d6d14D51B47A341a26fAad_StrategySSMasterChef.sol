...
abstract contract FeeManager is StratManager {
    ...
    uint256 public sideProfitFee = 0; // amount of rewardToken swapped to stable
...
abstract contract StrategySSBase is StratManager, FeeManager {
    ...
    // performance fees
    // TODO: make this more efficient by not swaping twice
    function chargeFees() internal {
        uint256 toStrat = ((rewardToken.balanceOf(address(this)) * strategistFee) / MAXFEE);
        if (toStrat > 0)
        {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(toStrat, 0, rewardToProfitRoute, address(this), block.timestamp);
            sideProfitToken.safeTransfer(strategist, sideProfitToken.balanceOf(address(this)));
        }

        if (sideProfitFee > 0) {
            uint256 toProfit = FullMath.mulDiv(rewardToken.balanceOf(address(this)), sideProfitFee, MAXFEE); // Take sideProfitFee% of rewardToken
            if (toProfit > 0)
            {
                IUniswapRouterETH(unirouter).swapExactTokensForTokens(toProfit, 0, rewardToProfitRoute, address(this), block.timestamp); // swap to profit
                sideProfitToken.safeTransfer(vault, sideProfitToken.balanceOf(address(this))); // deposit sideProfitToken into vault
            }
        }
    }
