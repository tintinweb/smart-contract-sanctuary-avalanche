...
contract Hero is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the HERO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Hero", "HERO") {
        // Mints 1 HERO to contract creator for initial pool setup
    ...
    function _getHeroPrice() internal view returns (uint256 _heroPrice) {
        try IOracle(heroOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Hero: failed to fetch HERO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _heroPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_heroPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHeroOracle(address _heroOracle) public onlyOperatorOrTaxOffice {
        require(_heroOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HERO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HERO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHeroPrice = _getHeroPrice();
            currentTaxRate = _updateTaxRate(currentHeroPrice);
            if (currentHeroPrice < burnThreshold) {
                burnTax = true;
            }
        }
