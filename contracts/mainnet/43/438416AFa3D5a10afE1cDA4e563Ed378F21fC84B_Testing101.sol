// File: interfaces/IPangolinFactory.sol
    ...
    */
    ...
    contract Testing101 is ERC20, Ownable, PaymentSplitter {
        ...
        uint256 public refReceived;
        uint256 public refPerNode;
        ...
        modifier antiWhale(address sender, address recipient, uint256 amount) {
            if (maxTransferAmount() > 0) {
                if (
                    ...
                    require(amount <= maxTransferAmount(), "SmartNodesss::antiWhale: Transfer amount exceeds the maxTransferAmount");
                    if (sender == joePair) {
                        require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "SmartNodesss::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                    }
                }
            }
            _;
        ...
        modifier antiSniper(address from, address to, address callee){
            ...
            // if (from != owner() && from != address(this) && from != rewardsPool
            // && to != owner() && to != address(this) && to != rewardsPool){
            //     if(callee != joePair && callee != address(joeRouter))
                    require(!(size1 > 0 || size2 > 0),"Sniper Detected");
                // else
                //     require(!(size2 > 0),"Sniper Detected");
            ...
            PaymentSplitter(payees, shares)
        {
            ...
            _excludedFromAntiSniper[msg.sender] = true;
        ...
        */
        ...
        function _transfer(
            ...
            if (from != owner() && from != rewardsPool && from != address(this) && to != address(this) && to != rewardsPool && to != owner()) {
        ...
        function _transferWithTax(bool isSell, bool isTransfer, address from, address to, uint256 amount) internal {
            ...
            if(isSell && !isExcludedfromTax[from]) {
                ...
                // taxReceived = taxReceived.add(taxAmt);
            }
        ...
        function _distributeTax(uint256 taxAmount, address from) internal {
            ...
            if (
                ...
                refReceived = refReceived.add(taxAmount.sub(rewardsPoolTokens).sub(treasuryRewardstoSwap).sub(teamTokens));
        ...
        function updateTaxes(uint256[] memory taxes, uint256[] memory taxPercents, uint256 taxLim1, uint256 taxLim2)external onlyOwner{
        ...
        function setExcludedFromTax(address _user)public onlyOwner{
            isExcludedfromTax[_user] = true;
        }
        ...
        function createNodeWithTokens(string memory name, uint256 amount_) external notPaused{
            ...
            require(
                amount_ >= nodeValue, "NC: Not enough SMN tokens"
            );
            require(
            ...
            if (
                ...
                // console.log("contract token", balanceOf(address(this)));
        ...
        function updateCashoutDitribution(uint256 _rewardsPoolCut, uint256 _teamPoolCut)public onlyOwner{
        ...
        // function distributeReflections()external onlyOwner{
            
        //     uint256 tNodes = nodeManager.getTotalNodes();
        //     refPerNode = (refReceived.sub(refClaimed)).div(tNodes);
        //     refClaimed = refReceived;
        //     uint256 nodesPerUser;
        //     for(uint256 i = 0; i < noders.length; i++)
        //     {
                
        //         nodesPerUser = nodeManager.getNodeNumberOf(noders[i]);
        //         userReceipts[noders[i]].userRefClaimed = (userReceipts[noders[i]].userRefClaimed.add(refPerNode.mul(nodesPerUser))); 
        //     }
        ...
        // function cashoutReflections()external{
        ...
        //         "CASHOUT:7"
        //     );
        //     uint256 refForClaim = userReceipts[sender].userRefClaimed;
        //     require(
        //         refForClaim > 0,
        //         "CASHOUT:8"
        //     );
        ...
        //         uint256 feeAmount;
        //         if (cashoutFee > 0) {
        //             feeAmount = refForClaim.mul(cashoutFee).div(100);
        //             swapAndSendToFee(rewardsPool, feeAmount);
        //         }
        //         refForClaim -= feeAmount;
        //     }
        //     super._transfer(address(this), sender, refForClaim);
        //     userReceipts[sender].userRefClaimed = 0;
