...
contract Zombie is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the ZOMBIE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("zombie.finance", "ZOMBIE") {
        // Mints 1 ZOMBIE to contract creator for initial pool setup
    ...
    function _getZombiePrice() internal view returns (uint256 _zombiePrice) {
        try IOracle(zombieOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Zombie: failed to fetch ZOMBIE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _zombiePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_zombiePrice >= taxTiersTwaps[tierId]) {
    ...
    function setZombieOracle(address _zombieOracle) public onlyOperatorOrTaxOffice {
        require(_zombieOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentZombiePrice = _getZombiePrice();
            currentTaxRate = _updateTaxRate(currentZombiePrice);
            if (currentZombiePrice < burnThreshold) {
                burnTax = true;
            }
        }
