...
contract Globa is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the 3days GLOBA-WETH LP -> GLOBA pool
    uint256 public constant INITIAL_GLOBA_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the GLOBA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GLOBA Token", "GLOBA") {
        // Mints 1 GLOBA to contract creator for initial pool setup
    ...
    function _getGlobaPrice() internal view returns (uint256 _globaPrice) {
        try IOracle(globaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Globa: failed to fetch GLOBA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _globaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_globaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGlobaOracle(address _globaOracle) public onlyOperatorOrTaxOffice {
        require(_globaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GLOBA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GLOBA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGlobaPrice = _getGlobaPrice();
            currentTaxRate = _updateTaxRate(currentGlobaPrice);
            if (currentGlobaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_globaPool != address(0), "!_globaPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_globaPool, INITIAL_GLOBA_POOL_DISTRIBUTION);
