...
contract StrategyCommonSolidlyRewardPoolVaraLP is StratManagerUpgradeable, DynamicFeeManager {
  ...
  function deposit() public whenNotPaused {
    ...
    if (wantBal > 0) {
      IGauge(rewardPool).deposit(wantBal, 0);
  ...
  function withdraw(uint256 _amount) external {
    ...
    if (wantBal < _amount) {
      IGauge(rewardPool).withdraw(_amount - wantBal);
  ...
  function _harvest(address callFeeRecipient) internal whenNotPaused {
    IGauge(rewardPool).getReward(address(this), rewards);
  ...
  function rewardsAvailable() public view returns (uint256) {
    return IGauge(rewardPool).earned(output, address(this));
  }
  ...
  function retireStrat() external {
    ...
    IGauge(rewardPool).withdraw(balanceOfPool());
