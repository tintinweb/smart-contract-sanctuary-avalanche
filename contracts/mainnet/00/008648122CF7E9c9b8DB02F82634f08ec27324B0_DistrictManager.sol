...
interface IMansionManager is IERC721 {
    ...
    function getMansionsOf(address _account) external view returns (uint256[] memory);
...
interface IMansionsHelper {
  ...
  function dexRouter() external returns (IRouter02);
  function _claimRatios () external view returns (ClaimRatios memory);
  function _newTaxRatios () external view returns (NewTaxRatios memory);
...
contract DistrictManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    DistrictManager public DISTRICTS_V3 = DistrictManager(0x9aDA04Af0B1c989A188F0063358f2d0530327386);
    IERC20 public PLAYMATES = IERC20(0x490bf3ABcAb1fB5c88533d850F2a8d6D38298465);
    IMansionManager public manager = IMansionManager(0xc4a25F823582d9ccf5cf8C8BF5338073e7a51676);
    IMansionsHelper public MANSIONSHEPLER = IMansionsHelper(0x3e8eD0e33684a00F0091114dd95f3Eb05ac5beD5);
    address public REWARDS_POOL = 0x4A558eCb43B909deB29a1d36c42EA288E09865cd;
    ...
    mapping(uint256 => District) private _nodes;
    ...
    uint256 public nodeMansionPrice = 20;
    uint256 private nodeCounter = 1;
    uint public maxWallet = 10;

    bool public transferIsEnabled = false;

    ITaxManager public taxManager = ITaxManager(0xab1d5e40E18ce4b53a0A292f20b12800Ce84d517);
    ...
    function totalNodesCreated() view external returns (uint) {
        return nodeCounter + DISTRICTS_V3.totalNodesCreated();
    }
    ...
    function updateDistrict(uint256 id, string calldata metadata) external {
        District storage district = _nodes[id];
        district.metadata = metadata;
    }

    function getDistricts(uint256 _id) public view onlyIfExists(_id) returns (District memory) {
        return _nodes[_id];
    }
    ...
    function getDistrictsOf(address _account) public view returns (uint256[] memory) {
    ...
    function updateNodeMansionsPrice (uint256 _nodeMansionPrice) external onlyOwner {
    ...
    function _addDistrict(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createDistrict(_id, _name, _mint, _claim, _metadata, _to);
    }

    function setManager (IMansionManager _manager) external onlyOwner {
        manager = IMansionManager(_manager);
    }
    ...
    function setTaxManager(address val) external onlyOwner {
      taxManager = ITaxManager(val);
    }

    function setMansionsHelper(address val) external onlyOwner {
      MANSIONSHEPLER = IMansionsHelper(val);
    }
    ...
    function migrate() external {
        require(migrated[msg.sender] == false, "MIGRATE: Already migrated");
        uint256 len = DISTRICTS_V3.balanceOf(msg.sender);
        uint256[] memory nodeIds = DISTRICTS_V3.getDistrictsOf(msg.sender);
        for (uint16 i = 0; i < len; i++) {
            (string memory _name, string memory _metadata, uint256 _id, uint64 _mint, uint64 _claim) = (DISTRICTS_V3.getDistricts(nodeIds[i]).name, DISTRICTS_V3.getDistricts(nodeIds[i]).metadata, DISTRICTS_V3.getDistricts(nodeIds[i]).id, DISTRICTS_V3.getDistricts(nodeIds[i]).mint, DISTRICTS_V3.getDistricts(nodeIds[i]).claim);
            _nodes[_id] = District({
                name: _name,
                mint: _mint,
                claim: _claim,
            ...
            _bags[msg.sender].push(_id);
            emit Transfer(address(0), msg.sender, _id);
        }
        migrated[msg.sender] = true;
    }

    function _createDistrict(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to) internal {
        require(!_exists(_id), "MANAGER: District already exist");
        _nodes[_id] = District({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        District memory _node = _nodes[uint64(tokenId)];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "District Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter + DISTRICTS_V3.totalNodesCreated();
        _createDistrict(nodeId, nodeName, uint64(block.timestamp), uint64(block.timestamp), "", account);
        nodeCounter += 1;
    }
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) public {
        ...
        require(manager.balanceOf(sender) >= nodeMansionPrice, "HELPER: Mansion balance too low for creation.");
        for (uint256 i = 0; i < nodeMansionPrice; i++) {
            manager.safeTransferFrom(_msgSender(), address(0), userNodes[i], "");
        }
        PLAYMATES.transferFrom(_msgSender(), REWARDS_POOL,  nodePrice);
        createNode(sender, _name);        
    }

    function compoundAndCreateNode(string memory _name, uint64[] memory userNodes, bool useBalance) public {
        require(bytes(_name).length > 0 && bytes(_name).length < 33, "HELPER: name size is invalid");
        address sender = _msgSender();
        require(sender != address(0), "HELPER:  Creation from the zero address");
        uint256 nodePrice = nodePlaymatePrice * 10 ** 18;
        require(nodePrice > 0, "error");
        uint256 addressRewards = manager.getAddressRewards(sender);
        uint256 availableRewards = addressRewards + addressRewards * manager.getUserMultiplier(sender) / 1000;
        if (!useBalance) require(availableRewards >= nodePrice, "HELPER: Playmate balance too low for creation.");
        else require(availableRewards + PLAYMATES.balanceOf(sender) >= nodePrice, "HELPER: Playmate balance too low for creation.");
        require(balanceOf(sender) + 1 <= maxWallet, "HELPER: Exceeds max wallet amount");
        require(manager.balanceOf(sender) >= nodeMansionPrice, "HELPER: Mansion balance too low for creation.");
        if (availableRewards >= nodePrice) {
          uint256 remainingRewards = availableRewards - nodePrice;
          MANSIONSHEPLER.claimUtility(userNodes, REWARDS_POOL, nodePrice, address(taxManager), sender);
          taxManager.execute(remainingRewards, sender);
        } else {
          require(useBalance, "HELPER: Invalid action");
          PLAYMATES.transferFrom(sender, REWARDS_POOL, nodePrice - availableRewards);
          MANSIONSHEPLER.claimUtility(userNodes, REWARDS_POOL, availableRewards - 1, address(this), sender);
        }
        for (uint256 i = 0; i < nodeMansionPrice; i++) {
          manager.safeTransferFrom(_msgSender(), address(0), userNodes[i], "");
        }
    ...
    function renameDistrict(uint64 id, string memory newName) external {
