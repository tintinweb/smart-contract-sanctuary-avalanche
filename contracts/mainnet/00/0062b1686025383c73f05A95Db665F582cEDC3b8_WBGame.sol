  ...
  ReentrancyGuardUpgradeable
{
  ...
  function claimReward() public nonReentrant {
    ...
    require(amount <= _totalReward, "Insufficent reward balance");
    require(
   ...
   */
  function claimableStatus() public view returns (uint256[2] memory) {
    return [
      _addressCHROReward[_msgSender()],
      _addressClaimedTime[_msgSender()]
    ];
  }
  ...
  /**
   * @notice See address Claimable Status
   * @param _address address to be looked
   * @return array [amount, last claimed time]
   */
  function viewClaimableStatus(address _address)
    external
    view
    onlyOwner
  ...
  /**
   * @notice View total nest
   */
  function viewTotalNest() external view returns (int8) {
    return _addressToNestAmount[_msgSender()];
  }

  /**
   * @notice Buy Nest
   * @dev Further processes will be done by listening event
   */
  function buyNest() public {
    uint256 price;

    // TODO: GET NEST PRICE FROM API
    price = 150000000000000000000;

    require(
      _tokenContract.balanceOf(_msgSender()) >= price,
      "Not enough tokens"
    );
    require(
      _tokenContract.allowance(_msgSender(), address(this)) >= price,
      "Not enough allowance"
    );

    _tokenContract.transferFrom(_msgSender(), address(this), price);
    _addressToNestAmount[_msgSender()] += 1;
    emit NestBought(_msgSender(), price);
  }

  function breed(uint256[] memory _parents, string memory _tokenURI) public {
    // NEED AVAILABLE NEST
    require(
      _addressToNestAmount[_msgSender()] + 1 >= 1,
      "No available Nest"
    );
