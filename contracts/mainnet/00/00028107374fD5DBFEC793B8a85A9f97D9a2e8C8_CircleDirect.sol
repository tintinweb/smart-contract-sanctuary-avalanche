...
import "./interfaces/IpAVAX_Direct.sol";
...
contract CircleDirect is Owned, TokensRecoverable, ReentrancyGuard 
{
    ...
    IPAVAX public immutable pAVAX;
    IPi public immutable pi;
    IpAVAX_Direct public immutable pAVAXDirect;
    ...
    IUniswapV2Router02 private uniswapV2Router = IUniswapV2Router02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    IUniswapV2Factory private uniswapV2Factory = IUniswapV2Factory(0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10);
    
    event SlippageSet(uint slippage);

    constructor(IPAVAX _pAVAX, IPi _pi, IpAVAX_Direct _pAVAX_Direct, IPiTransferGate _transferGate, IERC31337 _pAVAX_Liquidity, IVault _vaultContract, IERC1155 _ERC1155Token)
    {
        pAVAX = _pAVAX;
        pAVAXDirect = _pAVAX_Direct;
        transferGate = _transferGate;
        CircleNFT = _pAVAX_Liquidity;
        pi = _pi;
        vaultContract = _vaultContract;
        ERC1155Token = _ERC1155Token;

        LPAddress = uniswapV2Factory.getPair(address(_pAVAX), address(_pi));

        _pAVAX.approve(address(_pAVAX_Direct), uint256(-1));
        _pi.approve(address(_pAVAX_Direct), uint256(-1));

        _pAVAX.approve(address(_transferGate), uint256(-1));
        _pi.approve(address(_transferGate), uint256(-1));

        _pAVAX.approve(address(uniswapV2Router), uint256(-1));
    ...
    {
        require (msg.sender == address(pAVAX));
    }
    ...
    function estimateBuyLPFromBNB(uint256 _bnb_amount) external view returns(uint256){
        ...
        uint256 piAmount = pAVAXDirect.estimateBuy(bnbToBuy);
    ...
    function estimateBuyLPFromPi(uint256 _pi_amount) external view returns(uint256){
        ...
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 
        uint256 bnbAmount = pAVAXDirect.estimateSell(piToBuy);
    ...
    function easyBuySmallCircle() external payable nonReentrant
    {
        ...
        pAVAX.deposit{ value: tBNB }();

        uint256 piAmt = pAVAXDirect.easyBuy{ value: tBNB }();
        address LPaddress = uniswapV2Factory.getPair(address(pi), address(pAVAX));

        uint256 prevLPBalance = IERC20(LPaddress).balanceOf(address(this));

        (, ,  uint256 LPtokens) =transferGate.safeAddLiquidity(uniswapV2Router, pAVAX, tBNB, piAmt);
    ...
    //  pAVAX => small Circle
    function easyBuyFromPAVAX(uint256 pAVAXAmt) public nonReentrant returns (uint256)
    {

        uint256 prevPAVAXAmount = pAVAX.balanceOf(address(this));
        uint256 prevpiAmount = pi.balanceOf(address(this));

        pAVAX.transferFrom(msg.sender,address(this),pAVAXAmt);

        //swap half pAVAX to pi    
        uint256 pAVAXForBuy = pAVAXAmt.div(2);

        uint256 piAmt = pAVAXDirect.easyBuyFromPAVAX(pAVAXForBuy);

        address LPaddress = uniswapV2Factory.getPair(address(pi), address(pAVAX));
        uint256 prevLPBalance = IERC20(LPaddress).balanceOf(address(this));

        (, ,  uint256 LPtokens) =transferGate.safeAddLiquidity(uniswapV2Router, IERC20(pAVAX), pAVAXForBuy, piAmt);
        ...
        // any residue sent back to buyer/seller
        uint256 currpiAmount = pi.balanceOf(address(this)); 
        uint256 currPAVAXAmount = pAVAX.balanceOf(address(this));
        uint256 currLPBalance = IERC20(LPaddress).balanceOf(address(this));
        ...
        if(currPAVAXAmount>prevPAVAXAmount)
            pAVAX.transfer(msg.sender,currPAVAXAmount.sub(prevPAVAXAmount));
    ...
    function easyBuyFromPi(uint256 piAmt) external nonReentrant
    {
        ...
        uint256 pAVAXAmt = pAVAXDirect.easySellToPAVAX(piForBuy);

        address LPaddress = uniswapV2Factory.getPair(address(pi), address(pAVAX));
        uint256 prevLPBalance = IERC20(LPaddress).balanceOf(address(this));

        (, ,  uint256 LPtokens) =transferGate.safeAddLiquidity(uniswapV2Router, IERC20(pAVAX), pAVAXAmt, piForBuy);
    ...
    function easySellSmallCircleToPi(address _LPToken, uint256 sCircleValue) external nonReentrant
    {
        ...
        uniswapV2Router.removeLiquidity(address(pAVAX), address(pi), claimed, 0, 0, address(this), block.timestamp);
     
        
        uint256 currpAVAXAmount = pAVAX.balanceOf(address(this));
        pAVAXDirect.easyBuyFromPAVAX(currpAVAXAmount.sub(prevpAVAXAmount));
    ...
    //  CircleNFT => pAVAX
    function easySellSmallCircleToPAVAX(address _LPToken, uint256 sCircleValue) external nonReentrant
    {
        uint256 prevpAVAXAmount = pAVAX.balanceOf(address(this));
        uint256 prevPiAmount = pi.balanceOf(address(this));
        ...
        uint256 currpiAmount = pi.balanceOf(address(this)); 
        if(currpiAmount>prevPiAmount)
            pAVAXDirect.easySellToPAVAX(currpiAmount.sub(prevPiAmount));
    ...
    function easySellSmallCircleToBNB(address _LPToken, uint256 sCircleValue) external nonReentrant
    {
        ...
        uint256 currpAVAXAmount = pAVAX.balanceOf(address(this));
        uint256 pAVAXAmt = currpAVAXAmount.sub(prevpAVAXAmount);

        uint remAmount = pAVAXAmt;
        if(!pAVAX.isIgnored(msg.sender)){
            uint feeAmount= pAVAXAmt.mul(pAVAX.FEE()).div(100000);
            remAmount = pAVAXAmt.sub(feeAmount);
            pAVAX.transfer(pAVAX.FEE_ADDRESS(), feeAmount);
        }

        pAVAX.withdraw(remAmount);
        ...
        if(pAVAX.balanceOf(address(this))>prevpAVAXAmount)
            pAVAX.transfer(msg.sender,pAVAX.balanceOf(address(this)).sub(prevpAVAXAmount));
...
interface IpAVAX_Direct 
{
    ...
    function easyBuyFromPAVAX(uint256 piBNBIn) external  returns (uint256 PiAmount);

    function easySell(uint256 PiAmountIn) external returns (uint256 piBNBAmount);
    function easySellToPAVAX(uint256 PiAmountIn) external returns (uint256 piBNBAmount);

    function buyFromPAVAX(uint256 piBNBIn, uint256 dMagicOutMin) external returns (uint256 PiAmount);
