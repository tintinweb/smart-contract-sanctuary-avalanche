...
interface IBalanceOfHypercube {
    function balanceOfHypercube(address _address) external view returns (uint256);
}
...
contract HypercubeToken is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 4109576037277;
    ...
    address[] public HypercubeGamesContracts;
    address[] public partyArray;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(address => bool) public subContractCheck;
    mapping(address => bool) public HypercubeGamesCheck;
    ...
    0x111111119915E89E93366Cd2C78B87c05C649404;
    address public treasuryReceiver =
    0x2222222227C5e8962ca7E2E4838358A60d98a738;
    address public riskFreeValueReceiver =
    0x333333337bb836273418e329eE443bA7da4C8225;
    address public stableCoin = 0xd586E7F844cEa2F87f50152665BCbc2C279D8d70;
    ...
    constructor() ERC20Detailed('Hypercube Money', 'Hypercube', uint8(DECIMALS)) {
    ...
    function getUserTotalOnDifferentContractsHypercube(address sender)
    public
    view
    returns (uint256)
    {
        ...
        uint256 balanceOfAllHypercubeGamesContracts;
        ...
        balanceOfAllHypercubeGamesContracts = getBalanceOfAllHypercubeGamesContracts(
            sender
        );

        userTotal.add(balanceOfAllSubContracts).add(
            balanceOfAllHypercubeGamesContracts
        );
        return userTotal;
    }

    //this function iterates through all other contracts that are being part of the Hypercube ecosystem
    //we add a new contract like wHypercube or sHypercube, whales could technically abuse this
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < subContracts.length; i++) {
            userTotal += IBalanceOfHypercube(subContracts[i]).balanceOfHypercube(
                sender
            );
        }
    ...
    //get Hypercube Games Tickets Count For Tax
    function getBalanceOfAllHypercubeGamesContracts(address sender)
    public
    view
    returns (uint256)
    {
        uint256 HypercubeGamesTotal;

        for (uint256 i = 0; i < HypercubeGamesContracts.length; i++) {
            HypercubeGamesTotal += IPublicBalance(HypercubeGamesContracts[i])
            .balanceOf(sender);
        }

        return HypercubeGamesTotal;
    }
    ...
    returns (uint256)
    {
        ...
        uint256 userTotal = getUserTotalOnDifferentContractsHypercube(_address);
    ...
    function updateRebaseIndex(uint256 epoch) private {
        ...
        //update Index similarly to OHM, so a wrapped token created is possible (wHypercube)
    ...
    //Add Hypercube Games Contracts
    function addHypercubeGamesAddies(address _HypercubeGamesAddy, bool _value)
    public onlyOwner
    {
        require(
            HypercubeGamesCheck[_HypercubeGamesAddy] != _value,
            'e12'
        );
        ...
        if (_value) {
            HypercubeGamesContracts.push(_HypercubeGamesAddy);
        } else {
            require(HypercubeGamesContracts.length > 1, 'e13');
            for (uint256 i = 0; i < HypercubeGamesContracts.length; i++) {
                if (HypercubeGamesContracts[i] == _HypercubeGamesAddy) {
                    HypercubeGamesContracts[i] = HypercubeGamesContracts[
                    HypercubeGamesContracts.length - 1
                    ];
                    HypercubeGamesContracts.pop();
                    break;
                }
            }
        }

        emit SetHypercubeGamesAddresses(_HypercubeGamesAddy, _value);
    }
    ...
    function setIsLiquidityInAvax(bool _value) external onlyOwner {
        require(isLiquidityInAvax != _value, 'e36');
        isLiquidityInAvax = _value;
        emit SetIsLiquidityInAvax(_value, block.timestamp);
    }
    ...
    event SetHypercubeGamesAddresses(address indexed pair, bool indexed value);
}