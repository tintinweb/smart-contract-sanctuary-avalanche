  ...
  Authorizable
{
 ...
 uint256 low_perc = 30;
  ...
  function getStablePrice() public returns (uint256)
   {
   ...
   function avaxPrice() external returns(uint256) {
   	return getStablePrice();
   }
   function feeGen(uint256 _fee) external returns(uint256) {
	uint256 _fee = (10000*fee).div(getStablePrice()*100);
   	return _fee*(10**16);
   }
  // calculate price based on pair reserves
  function adds(address pair_add, bool tok) internal returns(address) {
  ...
  function decs(address pair_add) internal returns(uint8) {
         IERC20 swap_token = IERC20(pair_add);
         uint8 _dec =  swap_token.decimals();
         return _dec;
     }
  function find_dir(address ad) internal returns(uint[4] memory) {
  ...
  function getTokenPrice(address swap_add) external returns(uint256) {
