...
// KryptonCoin with Governance.
contract KryptonCoin is BEP20('Krypton Coin', 'KR') {
...
// MasterChef is the master of Krypton. He can make Krypton and he is a fair guy.
//
...
contract KryptonMasterChefL3 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accKryptonPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accKryptonPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accKryptonPerShare;  // Accumulated KRs per share, times 1e18. See below.
        uint16 depositFeeBP;        // Deposit fee in basis points
        uint256 harvestInterval;    // Harvest interval in seconds
    }

    // The KR TOKEN!
    KryptonCoin public immutable krypton;
    // Dev address.
    address public devAddress;
    // KR tokens created per second.
    uint256 public kryptonPerSecond;
    // Deposit Fee address
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed kryptonPerSecond);
    event RewardLockedUp(address indexed user, uint256 indexed pid, uint256 amountLockedUp);
    event StartTimeChanged(uint256 oldStartTime, uint256 newStartTime);

    constructor(
        KryptonCoin _krypton,
        address _devAddress,
        address _feeAddress,
        uint256 _startTime
    ) public {
        ...
        kryptonPerSecond = INITIAL_EMISSION_RATE;
    ...
    function pendingKrypton(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accKryptonPerShare = pool.accKryptonPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 kryptonReward = multiplier.mul(kryptonPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accKryptonPerShare = accKryptonPerShare.add(kryptonReward.mul(1e18).div(lpSupply));
        }

        uint256 pending = user.amount.mul(accKryptonPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Kryptons's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (krypton.totalSupply() >= MAX_SUPPLY) {
            kryptonReward = 0;
        } else if (krypton.totalSupply().add(kryptonReward.mul(11).div(10)) >= MAX_SUPPLY) {
            kryptonReward = (MAX_SUPPLY.sub(krypton.totalSupply()).mul(10).div(11));
        }

        if (kryptonReward > 0) {
            krypton.mint(devAddress, kryptonReward.div(10));
            krypton.mint(address(this), kryptonReward);
            pool.accKryptonPerShare = pool.accKryptonPerShare.add(kryptonReward.mul(1e18).div(lpSupply)); 
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingKrypton(_pid);
        ...
        user.rewardDebt = user.amount.mul(pool.accKryptonPerShare).div(1e18);
    ...
    // Pay or lockup pending krypton.
    function payOrLockupPendingKrypton(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accKryptonPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestInterval(_pid);
                // send rewards
                safeKryptonTransfer(msg.sender, rewardsToDistribute);
            }
        } else if (pending > 0) {
    ...
    // Safe krypton transfer function, just in case if rounding error causes pool to not have enough KRs.
    function safeKryptonTransfer(address _to, uint256 _amount) internal {
        uint256 kryptonBal = krypton.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > kryptonBal) {
            transferSuccess = krypton.transfer(_to, kryptonBal);
        } else {
            transferSuccess = krypton.transfer(_to, _amount);
        }
        require(transferSuccess, "safeKryptonTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _kryptonPerSecond) external onlyOwner {
        require (_kryptonPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: value higher than maximum");
        massUpdatePools();
        kryptonPerSecond = _kryptonPerSecond;
        emit UpdateEmissionRate(msg.sender, _kryptonPerSecond);
    }
