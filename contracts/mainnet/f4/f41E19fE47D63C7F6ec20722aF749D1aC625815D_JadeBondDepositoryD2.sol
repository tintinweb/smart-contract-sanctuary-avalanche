...
contract JadeBondDepositoryD2 is BaseBond {
    ...
    uint256[4] public discountRates;
    mapping(address => uint256)[4] public bondableTotalPerYearPerToken;
    mapping(address => uint256)[4] public bondedTotalPerYearPerToken;
    uint256[4] public bondableTotalPerUser;

    uint256 public totalJadeBonded;
    uint256 public totalJadeBondable;
    ...
    constructor(
        ...
        uint256 _startDate,
        uint256 _totalJadeBondable
    ) BaseBond(_Jade, _treasury, _startDate, _amountsTracker) {
        totalJadeBondable = _totalJadeBondable;
    }

    function setTotalJadeBondable(uint256 total) external onlyOwner {
        totalJadeBondable = total;
    }

    /* ======== USER FUNCTIONS ======== */

    function registerDiscounts(uint256[] calldata _discounts) external onlyOwner {
        for (uint256 i; i < 4; i++) {
            discountRates[i] = _discounts[i];
        }
    }

    function registerCapsByYear(address token, uint256[] calldata _bondableTotalPerYear)
        external
        onlyOwner
    {
        for (uint256 i; i < 4; i++) {
            bondableTotalPerYearPerToken[i][token] = _bondableTotalPerYear[i];
        }
    }

    function registerCapsByUser(uint256[] calldata _bondableTotalPerUser) external onlyOwner {
        for (uint256 i; i < 4; i++) {
            bondableTotalPerUser[i] = _bondableTotalPerUser[i];
        }
    }

    function applyDiscount(uint256 payout, uint256 _yearNb) public view returns (uint256) {
        return (payout * percentagePrecision) / discountRates[_yearNb];
    }

    function getDiscountRates(address user) public view returns (uint256[4] memory _discounts) {
        for (uint256 year; year < 4; year++) {
            _discounts[year] = discountRates[year];
        }
    }
    ...
    function getRemainingDiscountCaps(address user) public view returns (uint256[4] memory _caps) {
        for (uint256 year; year < 4; year++) {
            _caps[year] = bondableTotalPerUser[year] - bondsBought[year][user].payout;
        }
    }

    function getRemainingTotalDiscountCaps(address token)
        public
        view
        returns (uint256[4] memory _caps)
    {
        for (uint256 year; year < 4; year++) {
            _caps[year] =
                bondableTotalPerYearPerToken[year][token] -
                bondedTotalPerYearPerToken[year][token];
        }
    }
    ...
    {
        for (uint256 year; year < 4; year++) {
            _caps[year] = bondableTotalPerYearPerToken[year][token];
        }
    }
    ...
    function deposit(
        ...
        (bool isUserAllowed, , , ) = IDiamondAccounting(amountsTracker).getUserInfo(msg.sender);
        require(isUserAllowed, "User uneligible for this bond");
        ...
        uint256 discountedPayout = applyDiscount(payout, _yearNb);
        ...
        totalJadeBonded += discountedPayout;
        bondedTotalPerYear[_yearNb] += discountedPayout;
        require(totalJadeBondable >= totalJadeBonded, "Bond is full");
        bondedTotalPerYearPerToken[_yearNb][_token] += discountedPayout;
        require(
            bondedTotalPerYearPerToken[_yearNb][_token] <=
                bondableTotalPerYearPerToken[_yearNb][_token],
            "Bond is full for this year and token"
        );

        // depositor info is stored
        bondsBought[_yearNb][msg.sender] = Bond({
            payout: bondsBought[_yearNb][msg.sender].payout + discountedPayout,
            payoutHonored: 0 // May need a second look
        });

        require(
            bondsBought[_yearNb][msg.sender].payout <= bondableTotalPerUser[_yearNb],
            "Max capacity reached for this user and year"
        );
