...
contract BPS is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 150,000 BPS
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 130000 ether;
    uint256 public constant COMMUNITY_FUND_POOL_ALLOCATION = 9960 ether; // 40 additional BPD minted at creation for liquidity
    ...
    constructor(uint256 _startTime, address _communityFund, address _devFund1, address _devFund2, address _devFund3 ) public ERC20("BPS", "BPS") {
        _mint(msg.sender, 40 ether); // mint 40 BPS for initial pools deployment
    ...
    function claimRewards() external onlyOwner {
        ...
        if (_pending > 0 && devFund1 != address(0) && devFund2 != address(0) && devFund3 != address(0)) {
            _mint(devFund1, _pending.div(3));
            _mint(devFund2, _pending.div(3));
            _mint(devFund3, _pending.div(3));
