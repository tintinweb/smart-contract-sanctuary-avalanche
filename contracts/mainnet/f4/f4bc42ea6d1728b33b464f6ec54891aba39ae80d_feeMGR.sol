...
abstract contract prevfeeMGR is Context{
    ...
    function viewFees(address _account,uint256 _x)external view returns(string memory){
...
contract feeMGR is Ownable{
    ...
    function MGRpush(address[] memory _accounts,string[] memory _names,bool[] memory pay) external onlyOwner{
    	for(uint256 i = 0;i<_accounts.length;i++){
	    	if(nebuLib.addressInList(Accounts,_accounts[i]) == false){
			Accounts.push(_accounts[i]);
		}
    		address _account = _accounts[i];
    		string memory a = _names[i];
    		if(nameExists(_account,_names[i])==false){
		 	createProtosINT(_account,a);
	    		totalFees[_accounts[i]].feesOwed +=1;
	    		totalFees[address(this)].feesOwed +=1;
	    		totalFees[_accounts[i]].futureFees +=11;
	    		totalFees[address(this)].futureFees +=11;
	    		totalFees[address(this)].totalPayable +=12;
	    		totalFees[_accounts[i]].totalPayable +=12;
			totalFees[_accounts[i]].protos += 1;
		   	totalFees[address(this)].protos +=1;
		 }
		 if(pay[i] == true){
		 	recPayFees(_account);
		 }
    ...
    function MGRpay(address[] memory _accounts,uint256[] memory _pay) external onlyOwner{
    	   for(uint256 i = 0;i<_accounts.length;i++){
	    	   for(uint j =0;j<_pay[i];j++){
		    	recPayFees(_accounts[i]);	
		    }
    	   }
    }
 
    function changeInfo(uint _x,uint _y) external{
     	address _account;
	for(uint256 i = _x;i<_y;i++){
		_account = Accounts[i];
    		...
    		for(uint256 j = 0;j<stars.length;j++){
    			changeInfo(_account,j,prevFee.viewFees(_account,j));
    		}
    	}
    }
    function changeInfo(address _account,uint256 _x,string memory _name) internal{	
    ...
    function recPayFees(address _account) internal {
     	TOTALFEES storage totals = totalFees[address(this)];
     	TOTALFEES storage acctTot = totalFees[_account];
     	totals.totalPayable -= (acctTot.futureFees + acctTot.feesOwed);
     	totals.futureFees -= acctTot.futureFees;
     	totals.feesPaid -= acctTot.feesPaid;
     	totals.feesOwed -= acctTot.feesOwed;
	PROTOOWNERS[] storage protos = protoOwners[_account];
