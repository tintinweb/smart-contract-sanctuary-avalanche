...
contract Node {
    ...
    uint256[] public tierAmounts = [1 ether, 1000 ether, 10000 ether];
    struct User {
    ...
    uint256 public rewardPerSec = tierAmounts[0].mul(dailyRewardP).div(1000).div(24 * 3600);
    ...
    function setDailyRewardPercent( uint256 _p) external onlyDev {
        dailyRewardP = _p;
        rewardPerSec = tierAmounts[0].mul(dailyRewardP).div(1000).div(24 * 3600);
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(totalNodes[nodeTier] < nodesLimit, "Node count exceeds");
