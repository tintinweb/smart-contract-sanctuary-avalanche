...
contract pAVAXDirect is Owned, TokensRecoverable, ReentrancyGuard
{
    using SafeMath for uint256;

    IPAVAX immutable pAVAX;
    ...
    constructor(address payable _pAVAX, address payable _pi)
    {
        pAVAX = IPAVAX(_pAVAX);
        pi = IGatedERC20(_pi);

        IPAVAX(_pAVAX).approve(address(uniswapV2Router), uint256(-1));
    ...
    function estimateBuy(uint256 pAVAXAmountIn) public view returns (uint256 PiAmount)
    {
        address[] memory path = new address[](2);
        path[0] = address(pAVAX);
        path[1] = address(pi);
        (uint256[] memory amounts) = JoeLibrary.getAmountsOut(address(uniswapV2Factory), pAVAXAmountIn, path);
        return amounts[1];
    }
    ...
    function estimateSell(uint256 PiAmountIn) public view returns (uint256 ethAmount)
    {
        address[] memory path = new address[](2);
        path[0] = address(pi);
        path[1] = address(pAVAX);
        (uint256[] memory amounts) = JoeLibrary.getAmountsOut(address(uniswapV2Factory), PiAmountIn, path);
        return amounts[1];
    }
    ...
    function easyBuy() public payable returns (uint256 PiAmount)
    {
     ...
     function easyBuyFromPAVAX(uint256 pAVAXIn) public returns (uint256 PiAmount)
    {
        uint slippageFactor=(SafeMath.sub(100000,slippage)).div(1000); // 100 - slippage => will return like 98000/1000 = 98 for default
        return buyFromPAVAX(pAVAXIn, (estimateBuy(pAVAXIn).mul(slippageFactor).div(100)));
    }

    function easySell(uint256 PiAmountIn) public returns (uint256 pAVAXAmount)
    {
        uint slippageFactor=(SafeMath.sub(100000,slippage)).div(1000); // 100 - slippage => will return like 98000/1000 = 98 for default
        return sell(PiAmountIn, estimateSell(PiAmountIn).mul(slippageFactor).div(100));
    }

    function easySellToPAVAX(uint256 PiAmountIn) public returns (uint256 pAVAXAmount)
    {
        uint slippageFactor=(SafeMath.sub(100000,slippage)).div(1000); // 100 - slippage => will return like 98000/1000 = 98 for default
        return sellForPAVAX(PiAmountIn, estimateSell(PiAmountIn).mul(slippageFactor).div(100));
    }
    ...
    function buyFromPAVAX(uint256 pAVAXIn, uint256 piOutMin) public nonReentrant returns (uint256 PiAmount)
    {

        uint256 piPrev=pi.balanceOf(address(this));

        pAVAX.transferFrom(msg.sender,address(this),pAVAXIn);
        
        address[] memory path = new address[](2);
        path[0] = address(pAVAX);
        path[1] = address(pi);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(pAVAXIn, piOutMin, path, address(this), block.timestamp);
        uint256 piCurr=pi.balanceOf(address(this));
    ...
    function sell(uint256 PiAmountIn, uint256 pAVAXOutMin) public nonReentrant returns (uint256 bnbAmount)
    {
        ...
        path[0] = address(pi);
        path[1] = address(pAVAX);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(PiAmountIn, pAVAXOutMin, path, address(this), block.timestamp);
        uint256 currPAVAXAmount = pAVAX.balanceOf(address(this));

        uint256 pAVAXAmount = currPAVAXAmount.sub(prevpAVAXAmount);
    
        // will be applied only if BNB payout is happening 
        //else IGNORED_ADDRESSES in pAVAX will handle
        if(!pAVAX.isIgnored(msg.sender)){

            uint feePAVAX = pAVAX.FEE();
            address feeAddress = pAVAX.FEE_ADDRESS();

            uint feeAmount= pAVAXAmount.mul(feePAVAX).div(100000);
            uint remAmount = pAVAXAmount.sub(feeAmount);
            pAVAX.transfer(feeAddress, feeAmount);
            pAVAX.withdraw(remAmount);
            msg.sender.transfer(remAmount);
            return remAmount;
        }
        else{
            pAVAX.withdraw(pAVAXAmount);
            msg.sender.transfer(pAVAXAmount);
            return pAVAXAmount;
        }
    }


    function sellForPAVAX(uint256 PiAmountIn, uint256 pAVAXOutMin) public nonReentrant returns (uint256 pAVAXAmount)
    {
        require (PiAmountIn > 0, "Nothing to sell");
        IPiTransferGate gate = IPiTransferGate(address(pi.transferGate()));
        uint256 prevpAVAXAmount = pAVAX.balanceOf(address(this));
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(PiAmountIn, pAVAXOutMin, path, address(this), block.timestamp);
        uint256 currPAVAXAmount = pAVAX.balanceOf(address(this));
        pAVAXAmount = currPAVAXAmount.sub(prevpAVAXAmount);
        pAVAX.transfer(msg.sender, pAVAXAmount);
