...
// File contracts/fBombGlcrGlacier.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract fBombGlcrGlacier  is Ownable, Pausable {
    ...
    IGlacierRouter.Routes[] public glcrToFbombPath;
    ...
    constructor(
        ...
        for (uint i; i < _glcrToFbombPath.length; ++i) {
            glcrToFbombPath.push(_glcrToFbombPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IGlacierRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(glcrHalf, 0, glcrToFbombPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(glcr).balanceOf(address(this));
        uint256 t2Bal = IERC20(fbomb).balanceOf(address(this));

        IGlacierRouter(router).addLiquidity(glcr, fbomb, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
