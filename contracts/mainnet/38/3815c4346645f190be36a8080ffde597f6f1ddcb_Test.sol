...
contract Test is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	constructor(string memory _initUnrevealedURI, address _initArtistAddress, address _initCommunityAddress) ERC721("Travelling Frog", "TF") {
		paused = true;
		revealed = true;
		allowListings = true;
		...
		wavaxAddr = 0x7b16DeC105055164F0e56115d823bCb3879Aa90E;
		TFAddr = 0x7b16DeC105055164F0e56115d823bCb3879Aa90E;
	...
	function airDropTFOwner(uint256 _lastToken) public onlyOwner {
		require(nextTFAirdrop <= _lastToken, "Token already airdropped!");
		require(_lastToken <= 7777, "Last token must be 7777 or less.");

		IERC721 TFContract = IERC721(TFAddr);

		for (uint256 i = nextTFAirdrop; i <= _lastToken; i++) {

			supply.increment();

			uint256 tokenID = supply.current();

			_safeMint(TFContract.ownerOf(i), tokenID);

			reservedSupply -= 1;

		}

        nextTFAirdrop = _lastToken + 1;
	}
    ...
    function setNextTFAirDrop(uint256 newAmount) public onlyOwner {
		nextTFAirdrop = newAmount;
	}
    ...
    function setTFAddr (address _contract) public onlyOwner {
		TFAddr = _contract;
	}
