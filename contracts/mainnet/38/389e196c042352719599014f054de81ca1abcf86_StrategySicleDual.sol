...
interface IPopsMasterChef {
    ...
    function userInfo(uint256 _pid, address _user) external view returns (uint256, uint256);
    function emergencyWithdraw(uint256 _pid) external;
    function pendingPops(uint256 _pid, address _user) external view returns (uint256, address, string memory, uint256);
}
...
contract StrategySicleDual is StratManager, FeeManager {
    ...
    function rewardsAvailable() public view returns (uint256, uint256) {
        (uint256 outputBal,,,uint256 secondBal) = IPopsMasterChef(chef).pendingPops(poolId, address(this));
