...
contract AvaxFlip is Ownable, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    bytes32 constant keyHash = 0x89630569c9567e43c4fe7b1633258df9f2531b62f2352fa721cf3162ee4ecb46;
    ...
    uint16 public winningRate = 0;
    address public TreasuryWallet = 0x0A9DB2740552869f6348e3dA5579aaa42909BC00;
    uint256 public NumOfWins = 0;
    ...
    constructor(uint64 _subscriptionId) payable initCosts(0.01 ether) VRFConsumerBaseV2(vrfCoordinator) {
    ...
    modifier initCosts(uint256 initCost) {
        require(msg.value >= initCost, "Contract needs some AVAX.");
        _;
    }

    modifier betConditions() {
        require(msg.value >= minBet, "Insufficient amount, please increase your bet!");
    ...
    function flip(uint256 _betChoice) public payable betConditions nonReentrant {
    ...
    function withdrawPlayerBalance() external nonReentrant {
        ...
        require(playersByAddress[msg.sender].balance > 0, "You don't have any funds to withdraw.");
        ...
        payable(msg.sender).transfer((amount*winningRate)/100);
    ...
    function captureFees(uint256 amountTo) public onlyOwner {
    (bool hs, ) = payable(TreasuryWallet).call{value: amountTo}("");
    require(hs);
    contractBalance -= amountTo;
  }

    function changeMinBet(uint256 value) public onlyOwner {
        minBet = value;
    }

    function changeMaxBet(uint256 value) public onlyOwner {
        maxBet = value;
    }

    function changeWinRate(uint16 value) public onlyOwner {
        winningRate = value;
    }

    function changeTreasuryWallet(address wallet) public onlyOwner {
