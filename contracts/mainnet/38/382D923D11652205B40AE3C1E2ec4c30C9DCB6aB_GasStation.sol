...
contract GasStation is Ownable, FeePayerGuard, LegacyTxSupport {
...
abstract contract LegacyTxSupport is EIP712Library {
    using SafeMath for uint256;
    ...
    int public constant TX_VERSION = 0;

    bytes32 public constant TX_REQUEST_TYPEHASH = keccak256("TxRequest(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes data,uint256 deadline,uint256 gasPrice)");
    ...
    function _calculateCharge(uint256 _gasUsed, uint256 _txRelayFeePercent, TxRequest calldata _tx) internal pure returns (uint256, uint256) {
        uint256 feeForAllGas = _gasUsed.mul(_tx.gasPrice);
