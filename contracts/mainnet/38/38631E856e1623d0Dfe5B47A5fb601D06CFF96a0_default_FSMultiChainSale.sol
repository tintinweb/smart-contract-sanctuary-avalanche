...
contract default_FSMultiChainSale is AccessControl {
    ...
    **/
    ...
    /**
    * @dev Set max items on INO buy per user
    */
    function setMaxItemsPerUser(uint256 _maxItemsPerUser) public isAdmin {
        maxItemsPerUser = _maxItemsPerUser;
    }
    ...
    function getPaymentForEntireAllocation(address _buyer) public view returns (uint256 payTokensAmount) {
        ...
        if (isINO == false) {
            ...
            if (isFCFS == false) {
                ...
                return calculateMaxPaymentINOToken(maxItemsPerUser);
            }
        }
    }
    ...
    * Calculates how much Payment tokens you need to spend to acquire the exact INO allocation
    **/
    function calculateInoTokensBought(uint256 _amountInPayToken) public view returns (uint256 maxPaymentTokens) {

        // price per item is 100.5 * 10_000 = 1_005_000

        _amountInPayToken = _amountInPayToken.mul(idoTokenPriceMultiplier);  // 100_000_000 * 10_000 = 1_000_000_000_000

        uint256 _divideByRatio = itemPriceRatio.mul(10 ** uint256(payToken.decimals())); // 500_000 * 100_000 = 50_000_000_000

        uint256 _inoToGet = _amountInPayToken.div(_divideByRatio); // 1_000_000_000_000 / 50_000_000_000 =
    ...
    function buy(uint256 _amountInPayToken) external isFundingOpenAndRunning isFundingNotCanceledByAdmin isLotteryPlayedAndAllocationCalculated onlyWinners isNotSoldOut {
        ...
        if (isINO == false) {
            if (isFCFS == false) {
                ...
                if (inOneTransaction) {
                    require(calculateMaxPaymentToken(maxItemsPerUser) >= _amountInPayToken, "You need to buy the entire allocation in one transaction");
                } else {
                    require(_amountInPayToken <= calculateMaxPaymentToken(maxItemsPerUser), "You cannot buy more tokens than is allowed according to your lottery allocation calculation");
                    require(spendPayTokensMapping[msg.sender].add(_amountInPayToken) <= calculateMaxPaymentToken(maxItemsPerUser), "You cannot buy more tokens than is allowed according to your lottery allocation calculation");
                }
            }
        } else {
            if (isFCFS == false) {
                if (inOneTransaction) {
                    ...
                    require(calculateInoTokensBought(_amountInPayToken) <= lotteryContract.getWinnerTokenAllocation(msg.sender), "You cannot buy more than is allowed according to the INO");
                }
            } else {
                if (inOneTransaction) {
                    require(calculateMaxPaymentINOToken(maxItemsPerUser) >= _amountInPayToken, "You do not have enough payment tokens");
                } else {
                    require(calculateInoTokensBought(_amountInPayToken) <= maxItemsPerUser, "You cannot buy more than is allowed according to the INO");
                }
            }
        }
        ...
        if (isINO == false) {
            ...
            _idoTokensToBuy = calculateInoTokensBought(_amountInPayToken);
        }
