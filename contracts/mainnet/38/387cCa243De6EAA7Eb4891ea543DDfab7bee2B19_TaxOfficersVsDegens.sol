...
contract TaxOfficersVsDegens is ITaxOfficersVsDegens, ERC721Enumerable, Ownable, Pauseable {
    ...
    //multisig wallet
    address public multisigWallet = 0xfDc5922022bcc0D4F30EB2b5De9BB6e4a6cef383;
    ...
    // mapping used to giveaways freemint
    ...
    function mint(uint256 amount, bool stake) external payable nonReentrant whenNotPaused {
        ...
        if (minted < PAID_TOKENS) {
            if (freeMintAddresses[_msgSender()] > 0) {
                require(amount <= freeMintAddresses[_msgSender()], "Invalid free mint amount");
                require(msg.value == 0);
                freeMintAddresses[_msgSender()] -= amount;
            } else {
        ...
        if (totalFiatCost > 0 && freeMintAddresses[_msgSender()] == 0) fiat.burn(_msgSender(), totalFiatCost);
   ...
   */
    function mintCost(uint256 tokenId) public view returns (uint256) {
        ...
        if (tokenId <= 10000 + PAID_TOKENS) return 20000 ether;
        if (tokenId <= 30000 + PAID_TOKENS) return 40000 ether;
        return 80000 ether;
    }
   ...
   */
    function selectTraits(uint256 seed) internal view returns (OfficersDegens memory t) {
        ...
        t.hats = selectTrait(uint16(seed & 0xFFFF), 3  );
            ...
            seed >>= 16;
        t.taxBody = selectTrait(uint16(seed & 0xFFFF), 4 );
            seed >>= 16;
        t.shoes = selectTrait(uint16(seed & 0xFFFF), 5  );
            seed >>= 16;
        t.bottom = selectTrait(uint16(seed & 0xFFFF), 6  );
            seed >>= 16;
        t.hand = selectTrait(uint16(seed & 0xFFFF), 7  );
            seed >>= 16;
        t.top = selectTrait(uint16(seed & 0xFFFF), 8  );
          
            seed >>= 16;
        t.alphaIndex = selectTrait(uint16(seed & 0xFFFF), 9  );
        }
   ...
   */
    ...
    // add free mint addresses
    function addFreeMint(address[] memory _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            freeMintAddresses[_addresses[i]] += 1;
        }
    }

    // remove free mint addresses
    function delFreeMint(address[] memory _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            freeMintAddresses[_addresses[i]] = 0;
        }
    }
