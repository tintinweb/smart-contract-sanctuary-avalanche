...
contract PlatypusHelper is Ownable {
    ...
    function getTVL(address lp) public view returns (uint256 TVLinUSD) {
        ...
        if (TVLinUSD == 0) {
            TVLinUSD =
                (amount * getTokenPricePairedWithStable(token, USDC)) /
        ...
        returns (uint256 inUSD, uint256 tokenDecimals)
    {
        ...
        if (inUSD == 0) {
            inUSD = getTokenPricePairedWithStable(token, USDC);
        }
    ...
    function getLPsPrice(address[] calldata lps) public view returns (uint256[] memory inUSD) {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            if (inUSD[i] == 0) {
                inUSD[i] = getTokenPricePairedWithStable(token, USDC);
            }
        }
    }
        ...
        returns (uint256[] memory USDPerYear, address[] memory rewardTokens)
    {
        ...
        if (rewarder != address(0)) {
            ...
            for (uint256 i; i < length; ++i) {
                ...
                USDPerYear[i] += 10**(18-IERC20Metadata(rewardToken).decimals()) * (((userInfo.amount *
                    userInfo.accTokenPerShare +
                    userInfo.accTokenPerFactorShare *
    ...
    {
        address balanceHelper = IAPRHelper(aprHelper).balanceHelper();
        tokenPrice = IBalanceHelper(balanceHelper).getTokenPricePairedWithStable(token, stable);
    }
        ...
        )
    {
        ...
        for (uint256 i; i < bonusTokenAddresses.length; ++i) {
            rewardTokens[i + 1] = bonusTokenAddresses[i] == address(0)
                ? wavax
        ...
        returns (UserPoolInfo[] memory usersInfo)
    {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            (, , helpStack.depositToken, , , , , helpStack.rewarder, ) = IMainStaking(
                helpStack.mainStaking
            ).getPoolInfo(helpStack.asset);
            userInfo.balance = IBaseRewardPool(helpStack.rewarder).balanceOf(user);
            if (userInfo.balance > 0) {
                ...
                (
                    ...
                    userInfo.rewards,
                    userInfo.rewardsUSD
                ) = getPTPClaimableRewards(pool, inputRewardTokens, user);
                (
            ...
            userInfo.approvalAmount = IERC20(helpStack.depositToken).allowance(
                user,
                helpStack.mainStaking
            );
...
interface IBalanceHelper {
    ...
    function axlUSDC() external view returns (address);
