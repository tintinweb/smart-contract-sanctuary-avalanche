...
ERC721URITokenJSON, CosmicAttributeStorageUpgradeable, Blacklistable  {
    ...
    function tokenURIJSON(uint256 tokenId) public view virtual returns(string memory) {
        ...
        attributes[0] = Attribute("Profession", '', getSkillName(1, skillId), false);
        attributes[1] = Attribute("Durability", '', durability, false);
        attributes[2] = Attribute("Rarity", '', rarityString(rarity), false);
        uint256 imgId = (skillId * 15) + uint256(rarity);
        return _makeJSON(tokenId, name, getString(tokenId, 0), imgId, 'Cosmic Tools for Cosmic Universe', attributes);
    }
    ...
    function bridgeContract() external pure returns(address) {
        return address(0);
    }
...
abstract contract ERC721URITokenJSON is Initializable, ERC721Upgradeable {
    ...
    function _makeJSON(
        ...
        uint256 customImageId,
        string memory description,
        Attribute[] memory attributes
    ) internal view returns(string memory) {
        string memory metadataJSON = _makeMetadata(tokenId, name, customName, customImageId, description);
    ...
    function _makeMetadata(
        ...
        string memory id = customImageId == type(uint256).max ? tokenId.toString() : customImageId.toString();
        string memory imageURI = string(abi.encodePacked(imageBaseURI, id));
...
interface IBridgeableERC721 is IERC165Upgradeable, IERC721Upgradeable {
