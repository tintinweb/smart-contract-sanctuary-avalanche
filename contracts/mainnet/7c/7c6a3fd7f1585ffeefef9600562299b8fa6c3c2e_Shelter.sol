...
contract Shelter is IERC20, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    // token data
    string private constant _name = "Shelter";
    string private constant _symbol = "SHELTER";
    ...
    // Swap Path From Wrapped Native -> Underlying
    address[] path;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _admins.length; i++) {
            canTransactPreLaunch[_admins[i]] = true;
            max_holdings_exempt[_admins[i]] = true;
        }

        // Swap Path For Wrapped Native -> Underlying
        path = new address[](2);
        path[0] = router.WAVAX();
    ...
    /**
        ...
        It will then mint tokens to `recipient` based on the number of underlying coins received
    ...
    */
    ...
    function _mintWithNative(
        ...
        uint256 received = _purchaseUnderlying(slippage, nativeAmount);
    ...
    /** Price Of Token in Underlying With 18 Points Of Precision */
    ...
    function registerUserToBuyPreLaunch(
    ...
    function withdrawNonUnderlyingToken(IERC20 token) external onlyOwner {
        require(
    ...
    /** 
        Sells Tokens On Behalf Of Other User
            Requirements:
                User MUST have more than the max_holdings quantity in Underlying
                Can only redeem as much as they have excess in Underlying
     */
    function sellDownAccountToMaximumHoldings(
        address account
    ) external nonReentrant onlyOwner {
        require(account != address(0), "Zero Address");
        require(
            !max_holdings_exempt[account],
            "Account Is Max Holdings Exempt"
        );
        ...
        uint256 amtToSellUnderlying = valueOfHoldings.sub(max_holdings);

        // convert to Token
        uint256 tokensToSell = amtToSellUnderlying.mul(precision).div(
            _calculatePrice()
        );

        // sell excess tokens
        if (tokensToSell > 0) {
            _sell(account, tokensToSell, account);
        }
    }
    ...
    */
    ...
    /** Excludes Account From Max Holdings */
    function setMaxHoldingsExempt(
        address account,
        bool isHoldingsExempt
    ) external onlyOwner {
        require(account != address(0), "Zero Address");
