...
/// @title Flip!
/// @author 0xSmitty
/// @notice A contract letting users flip a coin to double up
contract Flip is VRFConsumerBaseV2, Ownable {
    ...
    event FlipStarted(address indexed creator, CoinSide indexed choice, uint256 indexed flipId, address referrer, uint256 gambleAmount);
    event WinnerPicked(CoinSide indexed winningChoice, uint256 indexed flipId, uint256 gambleAmount);
    ...
    /// @notice maximum flip amount is treasury divided by this
    ...
    function _createFlip(
        ...
        require(gambleAmount <= (address(this).balance / maxGambleDivisor), "Gamble less, king");
        require(gambleAmount % flipMultiple == 0, "Bad price fidelity");
        currentFlipId++;
        ...
        fi.gambleAmount = gambleAmount;
        flipById[currentFlipId] = fi;
        ...
        emit RequestedFlipWinner(requestId, currentFlipId);
        flipByRequestId[requestId] = currentFlipId;
    ...
    function flip(
        ...
        uint256 gambleAmount = _createFlip(choice, address(0));
        emit FlipStarted(msg.sender, choice, currentFlipId, address(0), gambleAmount);
    }
    /// @notice creates a new flip
    function referredFlip(
        ...
        uint256 gambleAmount = _createFlip(choice, address(0));
        emit FlipStarted(msg.sender, choice, currentFlipId, referrer, gambleAmount);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        CoinSide flipResult = randomWords[0] % 2 == 0 ? CoinSide.HEADS : CoinSide.TAILS;
        if( flipResult == fi.choice ) {
            (bool success, ) = payable(fi.creator).call{value: fi.gambleAmount*2}("");
            require(success);
        }
        fi.state = FlipState.CLOSED;
        emit WinnerPicked(flipResult, flipId, fi.gambleAmount);
    }
