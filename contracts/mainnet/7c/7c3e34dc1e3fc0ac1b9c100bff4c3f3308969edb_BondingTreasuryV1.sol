...
contract BondingTreasuryV1 is OwnableUpgradeable, BondingTreasuryStorageV1 {
    ...
    function initialize() public initializer {
        __Ownable_init();
        EmeraldToken = IERC20(0x97E7044ab98B7891546485f5f66EDcc2F58be106);
        WrappedAvax = IERC20(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
        WrappedUsdt = IERC20(0xc7198437980c041c805A1EDcbA50c1Ce5db95118);

        bondCounter = 0;
        quarterBondCounter = 0;
        quarterCount = 2;

        bondTimeline = 7 days;
        bondMaxAmount = 100 * (10**18);
        discountPercent = 10;
        ...
        treasuryAccount = 0x2d5472959BAB47B4D0e290bB30f7B868C35F9421;
    }

    /**
     * @dev user can get Em price w.r.t. AVAX.
     *
     * @param _amount Avax amount.
     *
     * Returns
     * - Em price.
     */
    ...
    /**
     * @dev user can get AVAX price w.r.t. EM token.
     *
     * @param _amount EM amount.
     *
     * Returns
     * - AVAX price.
     */
    ...
    /**
     * @dev user can get USD price w.r.t. EM token.
     *
     * Returns
     * - AVAX price.
     */
    ...
    /**
     * @dev user can get USD price w.r.t. AVAX token.
     *
     * Returns
     * - AVAX price.
     */
    ...
    /**
     * @dev user can get USD price w.r.t. USDT.e token.
     *
     * Returns
     * - AVAX price.
     */
    ...
    /**
     * @dev owner can update max bond value and its timeline.
     *
     * @param _maxAmt max bond amount for bonding.
     * @param _timeline bond duration period
     *
     * Requirements:
    ...
    function updateBondDetails(uint256 _maxAmt, uint256 _timeline)
        external
        virtual
        onlyOwner
    {
     ...
     * @dev owner can update max bond value and its timeline.
     *
     * @param _em_contract EM contract address
     * @param _usdt_contract USDT.e contract address
     * @param _avax_contract AVAX contract address
     *
     * Requirements:
     * - msg.sender must be owner of contract.
     *
    ...
    function updateERC20Address(
        address _em_contract,
        address _usdt_contract,
        address _avax_contract
    ) external virtual onlyOwner {
        EmeraldToken = IERC20(_em_contract);
        WrappedUsdt = IERC20(_usdt_contract);
        WrappedAvax = IERC20(_avax_contract);
        delete pair1;
        delete pair2;

        pair1.push(_avax_contract);
        pair1.push(_em_contract);

        pair2.push(_em_contract);
        pair2.push(_avax_contract);
    }

    /**
     * @dev owner can update tresuary account address.
     *
     * @param _account tresuary address
     *
     * Requirements:
     * - msg.sender must be owner of contract.
     *
    ...
    /**
     * @dev owner can update quarter details by defining time stamp
     * and set discount percent for reward.
     *
     * @param _timestamp tresuary address
     * @param _margin discounter percent
     *
     * Requirements:
     * - msg.sender must be owner of contract.
     *
    ...
    function updateQuarterDetails(
        uint256[] calldata _timestamp,
        uint256 _margin
    ) external virtual onlyOwner {
        delete quarterTimestamp;
        for (uint256 i = 0; i < _timestamp.length; i++) {
            quarterTimestamp.push(_timestamp[i]);
        }
        discountPercent = _margin;
    }

    /**
     * @dev updated the bond supply for quarter(optional)
     *
     * Requirements:
     * - msg.sender must be owner of contract.
     *
    ...
    /**
     * @dev user can create the AVAX/EM bond.
     * User can pay in AVAX with discounted price and withdraw after bond duration is over.
     *
     * @param _amount Amount of EM to be bond.
     *
     * Returns
     * - boolean.
     *
     * Emits a {CreateBond} event.
     */

    function bond(uint256 _amount) external payable virtual returns (bool) {
        if (block.timestamp < quarterTimestamp[0]) {
            revert("BondingTreasuryV1: Bonding is not started yet");
        }
        require(
        ...
        for (uint256 i = 0; i < quarterTimestamp.length; i++) {
            if (block.timestamp > quarterTimestamp[4]) {
                revert("BondingTreasuryV1: quarterly details not updated");
            }
            if (
                block.timestamp >= quarterTimestamp[i] &&
                block.timestamp <= quarterTimestamp[i + 1]
            ) {
                count = i + 1;
                if (quarterCount != count) {
                    quarterCount = count;
                    quarterBondCounter = 0;
                    bondSupply = feeCalculation(1, EmeraldToken.totalSupply());
                    bondSupplyCounter = 0;
    ...
    /**
     * @dev user can withdraw his/her bonf by using bond ID
     *
     * @param _bondId bond Id.
     *
     * Returns
     * - boolean.
     *
    ...
    /**
     * @dev user can wthdraw all his/her bond reward at one time.
     *
     * Emits a {WithdrawAll} event.
     */
    ...
    /**
     * @dev user can view its total discounted details.
     *
    ...
    /**
     * @dev user can view treasury balance in USD
     *
     * Returns
     * - Tresuary balance in USD.
     */
    ...
    /**
     * @dev view all bond details by user address
     *
     * @param _user user address.
     *
     * Returns
     * - Array of User Bond details.
     */
    ...
    /**
     * @dev Can withdra2 all EM token in smart contract.
     *
     * Requirements:
     * - msg.sender must be owner of contract.
     *
    ...
    /**
     * @dev used for calulation of discound percent and bond supply.
     */
...
contract BondingTreasuryStorageV1 is OwnableUpgradeable {
    // chainlink address for USDTto USD
    AggregatorV3Interface internal UsdtUsdPriceFeed;

    // chainlink address for AVAX to USD
    AggregatorV3Interface internal AvaxUsdPriceFeed;

    // Router address for pair address
    IRouter internal router;

    // temporary varibles for storing data
    address[] pair1;
    address[] pair2;
    uint256[] tempId;
    ...
    // USDT.e token Address
    IERC20 public WrappedUsdt;

    // bond details structure
    struct BondDetails {
        uint256 bondId;
        address user;
        uint256 amount;
    ...
    // bond details by address
    mapping(address => BondDetails[]) public UserBondDetails;

    // treasuryAccount account address
    address public treasuryAccount;

    // array of quarter timestamp
    uint256[] public quarterTimestamp;

    // bond supply for eavh quater
    uint256 public bondSupply;

    // bond counter for quater
    uint256 public quarterBondCounter;

    // count for quater
    uint256 public quarterCount;

    // discount percent for bond reward
    uint256 public discountPercent;

    // how much bond supply is used
    uint256 public bondSupplyCounter;

    // Bond counter
    uint256 public bondCounter;

    // time duration for bonding
    uint256 public bondTimeline; // 7 days

    // max bond value for each transaction
