...
contract Firn {
    ...
    IRouter router; // router deployed at 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106
    ERC20 immutable token = new ERC20("Firn Token", "FIRN"); // only use this to buy from below.
    ...
    function administrate(address _owner, uint32 _fee, address _transfer, address _withdrawal, address _router) external {
    ...
    function deposit(bytes32 Y, bytes32[2] calldata signature) external payable {
        ...
        pending[Y][0] = pending[Y][0].add(Utils.g().mul(credit)); // convert to uint256?
    ...
    function withdraw(bytes32[N] calldata Y, bytes32[N] calldata C, bytes32 D, bytes32 u, uint32 epoch, uint32 amount, uint32 tip, bytes calldata proof, address destination, bytes calldata data) external {
        ...
        statement.u = Utils.decompress(u);
        statement.fee = tip + (amount >> fee); // implicit conversion to uint256
        ...
        // buy and burn `uint256(amount >> fee) * 1e16` worth of FIRN token from pangolin
        address[] memory path = new address[](2); // is this actually cheaper than storage?
        path[0] = router.WAVAX(); // could probably save gas by putting this into storage once within administrate.
        path[1] = address(token);
        uint256[] memory amounts = router.swapExactAVAXForTokens{value: uint256(amount >> fee) * 1e16}(0, path, address(this), block.timestamp);
        token.burn(amounts[1]); // immediately burn the FIRN we got.
