 ...
 */
contract CHROPrivateSalesDev is Vesting {
  ...
  /// token price in wei. 1 $CHRO = $0.04
  uint256 public tokenPrice = 4 ether / 100;

  /// minimum purchase in USD
  uint256 public minimumPurchase = 500 ether;
  ...
  /**
   * @notice Approve the amount of USD
   * @param _usdAmount USD amount
   */
  function approve(uint256 _usdAmount) public {
    require(usdContract.balanceOf(msg.sender) >= _usdAmount, "Insufficent USD");
    SafeERC20.safeApprove(usdContract, address(this), _usdAmount);
  }
   ...
   */
  function buyTokens(uint256 _usdAmount) public payable started {
    uint256 usdAmount = _usdAmount;
    uint256 chroAmount = _usdAmount / tokenPrice;
...
contract Vesting is Ownable {
   ...
   * @param _timestamp Timestamp formatted with seconds
   */
  function release(uint256 _timestamp) public {
    require(_isReleasable(_timestamp), "Can't release CHRO");
    Account storage account = _accounts[msg.sender];

    uint256 _vestedAmount = _vestingSchedule(account.balance.add(account.released), uint256(_timestamp)).sub(account.released);
  ...
  function _vestingSchedule(
    ...
    if (_timestamp < start()) {
      ...
      return _allocationTotal.mul(start()).div(duration());
    }
  }
