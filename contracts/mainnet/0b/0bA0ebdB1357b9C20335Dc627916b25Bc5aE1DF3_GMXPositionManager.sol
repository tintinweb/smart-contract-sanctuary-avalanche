...
contract GMXPositionManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function positionsSize() external view returns (uint256) {
        ...
        for (uint8 i = 0; i < IGMXVault(gmxVault).allWhitelistedTokensLength(); i++) {
            ...
            (uint256 longSizeDelta,,,,,,,) = IGMXVault(gmxVault).getPosition(address(this), tradingToken, tradingToken, true);
            (uint256 shortSizeDelta,,,,,,,) = IGMXVault(gmxVault).getPosition(address(this), token, tradingToken, false);
            size += longSizeDelta;
            size += shortSizeDelta;
        }
