...
contract Plinko is GameBase, GameBet, GameChainlink {
    ...
    event GameStarted(address indexed creator, uint256 indexed gameId, uint8 roundCount, uint8 bucketCount, address referrer, uint256 betPerRound, uint256 rakePerRound);
    ...
    /// @notice number of buckets to the multipliers for the buckets
    mapping(uint8 => uint256[]) public bucketMultipliers;

    /// @notice number of buckets to the cumulative probabilities for the buckets
    mapping(uint8 => uint256[]) public bucketProbabilities;

    /// @dev amount to divide the multipliers by to get to the percent
    uint256 private divisor = 1000000;
    ...
    constructor(
        ...
        // Need to be higher to ensure we cover chainlink fees.
        minBet = 1 ether;
        // More than the standard number of bets, here.
        maxConsecutiveRounds = 100;

        bucketMultipliers[7] = [1000, 100, 70, 40, 70, 100, 1000];
        bucketProbabilities[7] = [15955, 169917, 345960, 654040, 830083, 984045, 1000000];
        bucketMultipliers[8] = [1500, 250, 70, 40, 40, 70, 250, 1500];
        bucketProbabilities[8] = [8553, 69696, 226169, 499997, 773831, 930304, 991447, 1000000];
        bucketMultipliers[9] = [2050, 400, 90, 60, 40, 60, 90, 400, 2050];
        bucketProbabilities[9] = [5201, 29646, 159398, 354027, 645973, 840602, 970354, 994799, 1000000];
        bucketMultipliers[10] = [4500, 800, 90, 60, 40, 40, 60, 90, 800, 4500];
        bucketProbabilities[10] = [2319, 17515, 119090, 271453, 499998, 728547, 880910, 982485, 997681, 1000000];
        bucketMultipliers[11] = [4700, 800, 200, 90, 60, 40, 60, 90, 200, 800, 4700];
        bucketProbabilities[11] = [2117, 7971, 44564, 166108, 355465, 644535, 833892, 955436, 992029, 997883, 1000000];
        bucketMultipliers[12] = [6400, 1700, 400, 90, 60, 40, 40, 60, 90, 400, 1700, 6400];
        bucketProbabilities[12] = [1228, 5526, 23543, 123849, 274308, 499997, 725692, 876151, 976457, 994474, 998772, 1000000];
        bucketMultipliers[13] = [7000, 1600, 300, 200, 90, 60, 40, 60, 90, 200, 300, 1600, 7000];
        bucketProbabilities[13] = [1113, 3885, 21282, 36598, 156659, 351451, 648549, 843341, 963402, 978718, 996115, 998887, 1000000];
    }
    ...
    function _createGame(
        ...
        require(bucketMultipliers[bucketCount].length > 0, "Use an expected configuration, pleb");
        require(bucketProbabilities[bucketCount].length > 0, "Use an expected configuration, pleb");
        uint256 maxPayout = details.betPerRound * bucketMultipliers[bucketCount][0] / 100;
        ...
        emit GameStarted(msg.sender, fi.gameId, roundCount, bucketCount, fi.referrer, fi.betPerRound, fi.rakePerRound);
        return fi.gameId;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        uint8 roundCount = fi.roundCount;
        uint8 bucketCount = fi.bucketCount;

        uint8[] memory results = new uint8[](roundCount);
        ...
        uint256[] memory multipliers = bucketMultipliers[bucketCount];
        uint256[] memory probabilities = bucketProbabilities[bucketCount];

        for (uint i=0; i<roundCount; i++) {
            uint256 roundRandom = uint256(keccak256(abi.encode(randomWords[0], i)));
            roundRandom  >>= 20;
            uint256 roundValue = roundRandom % divisor;

            for (uint8 j=0; j<bucketCount; j++) {
