...
contract Treasury is Ownable {
    ...
    mapping(address => uint256) public MinterQueue; // Delays changes to mapping.
    ...
    function deposit(
        ...
        require(
            ...
            "Treasury:NOT_ACCEPTED_AS_TREASURY_TOKEN"
        );
        ...
        if (isReserveToken[_token]) {
            require(
                ...
                "Treasury:NOT_APPROVED_AS_RESERVE_DEPOSITOR"
            );
        } else {
            require(
                isLiquidityDepositor[msg.sender],
                "Treasury:NOT_APPROVED_AS_LIQUIDITY_DEPOSITOR"
            );
        }
    ...
    function withdraw(uint256 _amount, address _token) external {
        require(isReserveToken[_token], "Treasury:NOT_ACCEPTED_AS_RESERVE_TOKEN");
        require(
            isReserveSpender[msg.sender] == true,
            "Treasury:NOT_APPROVED_AS_RESERVE_SPENDER"
        );

        uint256 value = tokenValue(_token, _amount);
        IERC20Burnable(token).burnFrom(msg.sender, value);
    ...
    function manage(address _token, uint256 _amount) external {
        if (isLiquidityToken[_token]) {
            require(
                ...
                "Treasury:NOT_APPROVED_AS_LIQUIDITY_MANAGER"
            );
        } else {
            require(
                isReserveManager[msg.sender],
                "Treasury:NOT_APPROVED_AS_RESERVE_MANAGER"
            );
        }

        uint256 value = tokenValue(_token, _amount);
        require(value <= excessReserves(), "Treasury:INSUFFICIENT_EXCESS_RESERVES");
    ...
    function mint(address _recipient, uint256 _amount) external {
        require(isMinter[msg.sender], "Treasury:NOT_APPROVED_AS_MINTER");

        require(_amount <= excessReserves(), "Treasury:INSUFFICIENT_EXCESS_RESERVES");
    ...
    function excessReserves() public view returns (uint256) {
        ...
        if(totalReserves > totalSupply){
            return totalReserves.sub(totalSupply);
        }else{
            return 0;
        }
    }
        ...
        returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20Metadata(token).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.MINTER) {
            // 7
            MinterQueue[_address] = block.number.add(blocksNeededForQueue);
        } else return false;
    ...
    function toggle(
        ...
        if (_managing == MANAGING.RESERVEDEPOSITOR) {
            ...
            // 7
            if (requirements(MinterQueue, isMinter, _address)) {
                MinterQueue[_address] = 0;
                if (!listContains(minters, _address)) {
                    minters.push(_address);
                }
            }
            result = !isMinter[_address];
            isMinter[_address] = result;
        } else return false;
    ...
    function requirements(
        ...
        if (!status_[_address]) {
            require(queue_[_address] != 0, "Treasury:MUST_QUEUE");
            require(queue_[_address] <= block.number, "Treasury:QUEUE_NOT_EXPIRED");
            return true;
        }
        return false;
    }
