    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct Payment {
        ...
        uint256       serviceFeePercent;        // Payment's pending claimable amount
     ...
     */
    uint256 public constant CANCELABLE_DURATION = 3 days;

    /**s
     *  @dev The extend duration that client can withdraw when payment is expired
     */
    uint256 public constant CLIENT_EXTEND_WITHDRAW_DURATION = 30 days;

    /**s
     *  @dev The extend duration that BO can withdraw when payment is expired
     */
    uint256 public constant BO_EXTEND_WITHDRAW_DURATION = 3 days;
    ...
    event SetGasLimitUnit(
        uint256 indexed oldGasLimitUnit,
        uint256 indexed newGasTransferLimit
    );
    ...
    modifier onlyBusinessOwner(uint256 _paymentId) {
        require(
            _msgSender() == payments[_paymentId].bo,
            "Caller is not the BO of this payment"
        );
        _;
    }
    ...
    modifier onlyClient(uint256 _paymentId) {
        require(
            _msgSender() == payments[_paymentId].client,
            "Caller is not the Client of this payment"
        );
        _;
    }
    ...
    modifier onlyValidPayment(uint256 _paymentId) {
        require(
            _paymentId > 0 && _paymentId <= lastPaymentId,
            "Invalid payment id"
        );
        require(
            payments[_paymentId].status != PaymentStatus.CANCELED,
            "Payment is canceled"
        );
        _;
    }
    ...
    modifier onlyRequestingPayment(uint256 _paymentId) {
        require(
            payments[_paymentId].status == PaymentStatus.REQUESTING,
            "Payment isn't requesting"
        );
        _;
    }
    ...
    modifier onlyNonExpiredPayment(uint256 _paymentId) {
        require(
            block.timestamp <= payments[_paymentId].expiredDate,
            "Payment is expired"
        );
        _;
    }
    ...
    function initialize(address _owner) public initializer {
        ...
        gasLimitUnit = 2300;
        serviceFeePercent = (15 * (10**WEIGHT_DECIMAL)) / 1000;
     ...
     */
    function setGasLimitUnit(uint256 _gasLimitUnit)
        external
        whenNotPaused
        onlyOwner
    {
     ...
     */
    function setPermittedToken(address _paymentToken, bool _allowed)
        external
        whenNotPaused
        onlyOwner
    {
     ...
     */
    function setServiceFeePercent(uint256 _percent)
        external
        whenNotPaused
        onlyOwner
    {
     ...
     *  @param  _data                   Encrypt sensitive data
     *  @param  _amount                 Total amount of payment
     *  @param  _numberOfInstallment    Array of amount per installment of payment
    ...
    function requestPayment(
        ...
        string memory _data,
        uint256 _amount,
        uint256 _numberOfInstallment,
        uint256 _expiredDate
    ) external whenNotPaused onlyValidAddress(_client) {
        require(
            _msgSender() != _client,
            "Business Owner and Client can not be same"
        );
        require(
            permittedPaymentTokens[_paymentToken] == true ||
                _paymentToken == address(0),
            "Invalid payment token"
        );
        require(_amount > 0, "Amount per installment must be greater than 0");
        require(
            _numberOfInstallment > 0,
            "Number of installments must be greater than 0"
        );

        uint256 currentTime = block.timestamp;
        require(
            _expiredDate > currentTime + CANCELABLE_DURATION,
        ...
        payment.numberOfInstallment = _numberOfInstallment;
        payment.createdDate = currentTime;
        payment.expiredDate = _expiredDate;
        payment.serviceFeePercent = serviceFeePercent;
     ...
     *  @param  _newClient  New client address that want to be updated
     *
     *  Emit event {SetClient}
     */
    function setClient(uint256 _paymentId, address _newClient)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyRequestingPayment(_paymentId)
        onlyNonExpiredPayment(_paymentId)
        onlyBusinessOwner(_paymentId)
        onlyValidAddress(_newClient)
    {
        address oldClient = payments[_paymentId].client;
        payments[_paymentId].client = _newClient;
        emit SetClient(oldClient, _newClient);
    }
     ...
     *          Name        Meaning
     *  @param  _paymentId  ID of payment that needs to be updated
     *  @param  _newData    New encrypted data that want to be updated
     *
     *  Emit event {SetData}
     */
    function setData(uint256 _paymentId, string memory _newData)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyRequestingPayment(_paymentId)
        ...
        onlyBusinessOwner(_paymentId)
    {
        require(bytes(_newData).length > 0, "Data must be not empty");

        string memory oldData = payments[_paymentId].data;
        payments[_paymentId].data = _newData;
        emit SetData(oldData, _newData);
    }
     ...
     *          Name        Meaning
     *  @param  _paymentId  ID of payment that needs to be updated
     *  @param  _newAmount  New amount that want to be updated
     *
     *  Emit event {SetAmount}
     */
    function setAmount(uint256 _paymentId, uint256 _newAmount)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyRequestingPayment(_paymentId)
        ...
        onlyBusinessOwner(_paymentId)
    {
        ...
        uint256 oldAmount = payments[_paymentId].amount;
        payments[_paymentId].amount = _newAmount;
        emit SetAmount(oldAmount, _newAmount);
    }
     ...
     *  Emit event {Paid}
     */
    function pay(uint256 _paymentId, uint256 _amount)
        external
        payable
        whenNotPaused
        onlyValidPayment(_paymentId)
        ...
        nonReentrant
    {
        ...
        require(
            _amount == payment.amount * payment.numberOfInstallment,
        ...
        if (permittedPaymentTokens[payment.paymentToken]) {
            ...
            IERC20Upgradeable(payment.paymentToken).safeTransferFrom(
                _msgSender(),
                address(this),
                _amount
            );
     ...
     */
    function confirmProvideServices(uint256 _paymentId)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyNonExpiredPayment(_paymentId)
        onlyBusinessOwner(_paymentId)
    {
        ...
        require(
            payment.lastTreatmentId < payment.numberOfInstallment,
            "Nothing to confirm"
        );

        if (payment.lastTreatmentId > 0) {
            TreatmentStatus treatmentStatus = treatments[_paymentId][
                payment.lastTreatmentId
            ].status;
            require(
                treatmentStatus == TreatmentStatus.CLIENT_CONFIRMED ||
                    treatmentStatus == TreatmentStatus.CANCELED ||
                    treatmentStatus == TreatmentStatus.CLAIMED,
                "Client hasn't confirmed previous treatment yet"
            );
        } else {
            // When process the first treatment
            payment.status = PaymentStatus.PROCESSING;
        }
        ...
        treatments[_paymentId][payment.lastTreatmentId] = treatment;

        emit ConfirmedProvideServices(
            _paymentId,
            payment.lastTreatmentId,
            payment.status,
            treatment.status
     ...
     */
    function confirmToRelease(uint256 _paymentId)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyNonExpiredPayment(_paymentId)
        onlyClient(_paymentId)
    {
        ...
        Treatment storage treatment = treatments[_paymentId][
            payment.lastTreatmentId
        ];
        require(
            treatment.status == TreatmentStatus.BO_CONFIRMED,
            "Business owner hasn't confirmed treatment yet"
        );

        treatment.status = TreatmentStatus.CLIENT_CONFIRMED;
        payment.pendingTreatmentIds.push(payment.lastTreatmentId);

        emit ConfirmedToRelease(
            _paymentId,
            payment.lastTreatmentId,
            treatment.status
        );
     ...
     *
     *          Name          Meaning
     *  @param  _paymentId    ID of payment want to claim all treatments
     *
     *  Emit event {Claimed}
     */
    function claim(uint256 _paymentId)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyBusinessOwner(_paymentId)
        nonReentrant
    {
        ...
        require(
            block.timestamp <=
                payment.expiredDate + BO_EXTEND_WITHDRAW_DURATION,
            "Claim time has expired"
        );
        require(
            payment.status == PaymentStatus.PROCESSING,
            "Payment hasn't been processing yet"
        );
        ...
        for (uint256 i = 0; i < payment.pendingTreatmentIds.length; ++i) {
            ...
            treatments[_paymentId][treatmentId].status = TreatmentStatus
                .CLAIMED;
        }

        uint256 claimableTotalAmount = payment.pendingTreatmentIds.length *
            payment.amount;
        ...
        uint256[] memory pendingTreatmentIdsBefore = payment
            .pendingTreatmentIds;
        ...
        nonReentrant
    {
        ...
        if (paymentStatusBefore == PaymentStatus.PAID) {
            uint256 withdrableAmount = payment.amount *
                payment.numberOfInstallment;
     ...
     */
    function clientWithdrawExpiredPayment(uint256 _paymentId)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyClient(_paymentId)
        nonReentrant
    {
        ...
        uint256 currentTime = block.timestamp;
        uint256 boClaimEndTime = payment.expiredDate +
            BO_EXTEND_WITHDRAW_DURATION;
        require(
            currentTime > boClaimEndTime &&
                currentTime <= boClaimEndTime + CLIENT_EXTEND_WITHDRAW_DURATION,
        ...
        for (
            uint256 i = payment.finishedTreatmentsCount + 1;
            i <= payment.lastTreatmentId;
            i++
        ) {
            treatments[_paymentId][i].status = TreatmentStatus.CANCELED;
            canceledTreatmentIds[i - payment.finishedTreatmentsCount - 1] = i;
        }

        uint256 claimable = payment.amount *
            (payment.numberOfInstallment - payment.finishedTreatmentsCount);
        payment.status = PaymentStatus.FINISHED;
     ...
     */
    function ownerWithdrawExpiredPayment(uint256 _paymentId)
        external
        whenNotPaused
        onlyValidPayment(_paymentId)
        onlyOwner
        nonReentrant
    {
        ...
        require(
            block.timestamp >
                payment.expiredDate +
                    BO_EXTEND_WITHDRAW_DURATION +
                    CLIENT_EXTEND_WITHDRAW_DURATION,
            "Not in withdraw time"
        );
     ...
     *
     *          Name                Meaning
     *  @param  _paymentId          ID of payment that Owner (CMB) want to handle
     *  @param  _treatmentIds       ID of treatments that want to judge
     *  @param  _isCancel           true -> Client win -> cancel this treatment; false -> BO win -> force client to confirm this treatment
     *
     *  Emit event {Judged}
     */
    function judge(
        uint256 _paymentId,
        uint256[] memory _treatmentIds,
        bool _isCancel
    )
        external
        whenNotPaused
        ...
        nonReentrant
    {
        require(_treatmentIds.length > 0, "Invalid treatment ids");
        ...
        for (uint256 i = 0; i < _treatmentIds.length; i++) {
            uint256 treatmentId = _treatmentIds[i];
            Treatment storage treatment = treatments[_paymentId][treatmentId];
            require(
                treatment.status == TreatmentStatus.BO_CONFIRMED,
                "Treatment hasn't confirm yet"
            );
        ...
        emit Judged(_paymentId, _treatmentIds, _isCancel, payment.status);
    }
     ...
     */
    function calculateServiceFee(uint256 _amount, uint256 _serviceFeePercent)
        public
        pure
        returns (uint256)
    {
        return (_amount * _serviceFeePercent) / ((10**WEIGHT_DECIMAL) * 100);
    }

    /**
     *  @dev    Get pending treatment ids that waiting for BO claim from payment by payment id
     *
     *          Name                Meaning
     *  @param  _paymentId          Payment id that treatment ids are belong to
     *
     *  @return Pending treatment ids that waiting for BO claim
     */
    function getPendingTreatmentIds(uint256 _paymentId)
        external
        view
        returns (uint256[] memory)
    {
        return payments[_paymentId].pendingTreatmentIds;
    }
