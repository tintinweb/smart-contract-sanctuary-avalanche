...
contract MNCDAO is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MemeNodeCapitalDAO";
    string constant _symbol = "MNCDAO";
    ...
    address public autoLiquidityReceiver=0xCe8927eD61A94Bf2A0FC651aE6B7bF93aD4eD7E9;
    address public opsFeeReceiver=0xCE88df3C5c3DeB95a31297e9C6eacf99FFBF7366;
    ...
    constructor (
        address _dexRouter, address _opsAddress, address _wAVAX
    ) Auth(msg.sender) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountAVAXOps = amountAVAX.mul(opsFee).div(totalAVAXFee);

        try distributor.deposit{value: amountAVAXReflection}() {} catch {}
        payable(opsFeeReceiver).transfer(amountAVAXOps);
    ...
    function Retr() external onlyOwner {
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _opsFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        opsFee = _opsFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_opsFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/4);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _opsFeeReceiver) external authorized {
