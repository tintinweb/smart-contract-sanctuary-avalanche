...
░█▄▄▄ ░█──▀█ ░█▄▄▄ ░█─░█ ░█▄▄█ ──░█── 　 ░█──▀█ ░█▄▄▄█ ░█▄▄▀ ░█▄▄▄ ░█▄▄▄█
    ...
    https://twitter.com/ENRGYnodes

    https://energynodes.finance/#
...
contract EnergyNodes is ERC20, Ownable, PaymentSplitter {
   ...
   modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "ENRGYNODES::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "ENRGYNODES::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
    ...
    function getBlock() public {
        userLastBlockBuyTransactions[msg.sender] = block.number;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "NEBULA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 0, "NEBULA::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate.");
    ...
    function updatemaxBalanceAmountRate(uint16 _maxBalanceAmountRate) external onlyOwner {
        require(_maxBalanceAmountRate <= 10000, "NEBULA::updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxBalanceAmountRate >= 0, "NEBULA::updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate.");
