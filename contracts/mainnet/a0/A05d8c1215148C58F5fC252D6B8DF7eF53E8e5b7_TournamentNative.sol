...
interface ITournament {
    ...
    function pgClaimed(address user) external returns (bool); // returns true if the given user has already claimed his/her share in the prize as a pglad owner
    ...
    function ROUND() external returns (uint256); // time until first earning
    ...
    function GLAD_SHARE() external returns (uint256); // 10000th of ticketprice
...
contract TournamentNative is Ownable, ITournament {
    ...
    uint256 public TICKET_SIZE = 500; // 10000th of pot
    uint256 public POT_SHARE = 5100; // 10000th
    uint256 public GLAD_SHARE = 1000; // 10000th
    Whitelist public wl;
    ...
    function buy() external payable {
        ...
        if (lastTs != 0) {
            ...
            if (delta_t > ROUND) {
                reward = (ticketPrice * (block.timestamp - lastTs)) / 3600;
            } 
            Address.sendValue(payable(winner), reward);
        }
        ...
        Address.sendValue(payable(owner()), ticketPrice * (10000 - POT_SHARE - GLAD_SHARE) / 10000);
        winner = msg.sender;
    }
