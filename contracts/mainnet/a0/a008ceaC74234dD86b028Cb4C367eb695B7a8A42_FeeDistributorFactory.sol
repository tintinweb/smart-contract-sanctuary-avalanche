...
contract FeeDistributor is Initializable, IFeeDistributor {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) public veWithdrawnTokenAmountByPeriod;
    ...
    function _getReward(uint256 period, uint256 tokenId, address token, address receiver) internal {
        if (totalVeShareByPeriod[period] != 0) {
            uint256 _reward = (tokenTotalSupplyByPeriod[period][token] * veShareByPeriod[period][tokenId]) /
    ...
    function _getAllRewards(uint256 tokenId, address[] memory tokens, address receiver) internal {
        uint256 currentPeriod = getPeriod();
        uint256 lastClaim;
        for (uint256 i = 0; i < tokens.length; ++i) {
            lastClaim = Math.max(lastClaimByToken[tokens[i]][tokenId], firstPeriod);
    ...
    function getPeriodReward(uint256 period, uint256 tokenId, address token) external lock {
    ...
    function earned(address token, uint256 tokenId) external view returns (uint256 reward) {
        uint256 currentPeriod = getPeriod();
        uint256 lastClaim = Math.max(lastClaimByToken[token][tokenId], firstPeriod);
        for (uint256 period = lastClaim; period <= currentPeriod; period += WEEK) {
            if (totalVeShareByPeriod[period] != 0) {
                reward +=
                    (tokenTotalSupplyByPeriod[period][token] * veShareByPeriod[period][tokenId]) /
                    totalVeShareByPeriod[period];

                reward -= veWithdrawnTokenAmountByPeriod[period][tokenId][token];
            }
        }
    }
