...
This is the official contract of the TheFinalCountdown token. This token
...
contract TheFinalCountdown is Context, IERC20, Ownable {
    ...
    uint256 private constant AVAX_DECIMALS = 18;
    ...
    // We don't add to liquidity unless we have at least 1 TFC token
    uint256 private constant LIQ_SWAP_THRESH = 10**_decimals;

    // PCS takes 0.25% fee on all txs
    uint256 private constant ROUTER_FEE = 30;
    ...
    uint256 private constant JACKPOT_MINBUY_MIN = 5 * 10**(AVAX_DECIMALS - 2);
    uint256 private constant JACKPOT_MINBUY_MAX = 5 * 10**(AVAX_DECIMALS - 1);
    ...
    string private constant _name = "TheFinalCountdown";
    ...
    // Buys > 0.1 AVAX will be eligible for the jackpot
    uint256 public jackpotMinBuy = 1 * 10**(AVAX_DECIMALS - 1);
    // Jackpot time span is initially set to 10 mins
    uint256 public jackpotTimespan = 10 * 60;
    // Jackpot hard limit, AVAX value
    uint256 public jackpotHardLimit = 250 * 10**(AVAX_DECIMALS);
    ...
    event DevFeesCollected(uint256 avaxCollected);
    event MarketingFeesCollected(uint256 avaxCollected);
    ...
    event JackpotFund(uint256 avaxSent, uint256 tokenAmount);
    ...
    constructor(address cOwner) Ownable(cOwner) {
        ...
        uniswapV2Router = IUniswapV2Router02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    function setJackpotFeatures(
        ...
        require(
            ...
            "Jackpot min buy needs to be between 0.05 and 0.5 AVAX"
        );
    ...
    function fundJackpot(uint256 tokenAmount) external payable onlyAuthorized {
        ...
        _pendingJackpotBalance = _pendingJackpotBalance.add(avaxSent);
        if (tokenAmount > 0) {
            transferBasic(msg.sender, address(this), tokenAmount);
            _jackpotTokens = _jackpotTokens.add(tokenAmount);
        }

        emit JackpotFund(avaxSent, tokenAmount);
    }
    ...
    function usdEquivalent(uint256 avaxAmount) public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WAVAX();
        path[1] = USDT;

        return uniswapV2Router.getAmountsOut(avaxAmount, path)[1];
    }
        ...
        )
    {
        ...
        // Either 0 or 1+ TFC to prevent PCS errors on liq swap
    ...
    function jackpotBuyerShareAmount() public view returns (uint256, uint256) {
        uint256 avax = _pendingJackpotBalance
            .mul(jackpotCashout)
            .div(MAX_PCT)
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 tokensForAvaxExchange = toBeSwapped.sub(pureLiqTokens);

        uint256 initialBalance = address(this).balance;
        swapTokensForAvax(tokensForAvaxExchange);

        // How many AVAXs did we gain after this conversion?
        uint256 gainedAvax = address(this).balance.sub(initialBalance);

        // Calculate the amount of AVAX that's assigned to the marketing wallet
        uint256 balanceToMarketing = gainedAvax.mul(marketingTokens).div(
            tokensForAvaxExchange
        );
        _pendingMarketingBalance += balanceToMarketing;

        // Same for dev
        uint256 balanceToDev = gainedAvax.mul(devTokens).div(
            tokensForAvaxExchange
        );
        _pendingDevBalance += balanceToDev;

        // Same for Jackpot
        uint256 balanceToJackpot = gainedAvax.mul(jackpotTokens).div(
            tokensForAvaxExchange
        );
        _pendingJackpotBalance += balanceToJackpot;

        uint256 remainingAvax = gainedAvax
            .sub(balanceToMarketing)
            .sub(balanceToDev)
            .sub(balanceToJackpot);

        if (liqTokens >= LIQ_SWAP_THRESH) {
            // The leftover AVAXs are purely for liquidity here
            ...
            uint256 prevAvaxBalance = address(this).balance;
            addLiquidity(pureLiqTokens, remainingAvax);
            uint256 usedAvaxs = prevAvaxBalance.sub(address(this).balance);
            ...
            // Reallocate the unused AVAXs to the pending marketing wallet balance
            if (usedAvaxs < remainingAvax) {
                _pendingMarketingBalance += remainingAvax.sub(usedAvaxs);
            }

            emit SwapAndLiquify(tokensForAvaxExchange, usedAvaxs, usedTokens);
        } else {
            // We could have some dust, so we'll just add it to the pending marketing wallet balance
            _pendingMarketingBalance += remainingAvax;

            emit SwapAndLiquify(tokensForAvaxExchange, 0, 0);
        }
    }
