...
contract Lottery is Ownable, ReentrancyGuard {
    ...
    INeonMonsters public tokenAddr;
    address payable[] private _participants;
    address payable public winner;

    State public state;

    mapping(address => uint256) public isParticipated;
    ...
    error LotteryNotActiveError();
    error UnexpectedError();
    error NotMinterError();
    error InsufficientBalanceError();
    error ParticipatedError();

    constructor(INeonMonsters tokenAddr_) {
        tokenAddr = INeonMonsters(tokenAddr_);
    }

    function start() external payable onlyOwner {
        if (state != State.NOT_ACTIVE) revert LotteryActiveError();
    ...
    function participate() external nonReentrant {
        if (state != State.ACTIVE) revert LotteryNotActiveError();
        if (!INeonMonsters(tokenAddr).isMinter(_msgSender()))
            revert NotMinterError();
        if (isParticipated[_msgSender()] > 0) revert ParticipatedError();

        isParticipated[_msgSender()] = 1;
        _participants.push(payable(_msgSender()));
    }

    function finalize(uint256 salt) external onlyOwner {
        if (state != State.ACTIVE) revert LotteryNotActiveError();
        ...
        uint256 indexOf = _psuedoRandom(salt) % _participants.length;

        winner = _participants[indexOf];

        emit Winner(winner);

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = winner.call{ value: prize }("");
        if (!success) revert UnexpectedError();
    }
    ...
    function _psuedoRandom(uint256 salt) private view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
