...
contract SShareSwapper is Operator {
    ...
    address public santaJoeLpPair;
    address public sshareJoeLpPair;
    ...
    constructor(
            ...
            address _santaJoeLpPair,
            address _sshareJoeLpPair,
            address _daoAddress
    ) public {
        santa = IERC20(_santa);
        sbond = IERC20(_sbond);
        ...
        santaJoeLpPair = _santaJoeLpPair;
        sshareJoeLpPair = _sshareJoeLpPair;
    ...
    modifier isSwappable() {
        ...
        require(santa.totalSupply() >= 60 ether, "ChipSwapMechanismV2.isSwappable(): Insufficient supply.");
        _;
    }

    function estimateAmountOfSShare(uint256 _sbondAmount) external view returns (uint256) {
        uint256 sshareAmountPerSanta = getSShareAmountPerSanta();
        return _sbondAmount.mul(sshareAmountPerSanta).div(1e18);
    }
    ...
    function swapSbondToSShare(uint256 _sbondAmount) external {
        ...
        uint256 sshareAmountPerSanta = getSShareAmountPerSanta();
        uint256 sshareAmount = _sbondAmount.mul(sshareAmountPerSanta).div(1e18);
    ...
    function getSantaPrice() public view returns (uint256) {
        return IERC20(wavaxAddress).balanceOf(santaJoeLpPair)
            .mul(1e18)
	    .div(santa.balanceOf(santaJoeLpPair));
    }

    function getSSharePrice() public view returns (uint256) {
        return IERC20(wavaxAddress).balanceOf(sshareJoeLpPair)
            .mul(1e18)
            .div(sshare.balanceOf(sshareJoeLpPair));
    }

    function getSShareAmountPerSanta() public view returns (uint256) {
        uint256 santaPrice = IERC20(wavaxAddress).balanceOf(santaJoeLpPair)
            .mul(1e18)
	    .div(santa.balanceOf(santaJoeLpPair));

        uint256 ssharePrice =
            IERC20(wavaxAddress).balanceOf(sshareJoeLpPair)
	    .mul(1e18)
            .div(sshare.balanceOf(sshareJoeLpPair));
            

        return santaPrice.mul(1e18).div(ssharePrice);
    }
