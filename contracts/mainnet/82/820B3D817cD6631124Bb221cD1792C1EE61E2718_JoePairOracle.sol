...
contract JoePairOracle is Ownable {
    using FixedPoint for *;

    uint256 public PERIOD = 60; // 1-minute TWAP (Time-Weighted Average Price)
    uint256 private constant MAXIMUM_PERIOD = 3600 * 48; // 48 hours
    uint256 private constant MINIMUM_PERIOD = 60; // 1 minute
    uint256 private constant LENIENCY = 3600 * 12; // 12 hours

    IJoePair public immutable pair;
    ...
    constructor(address pairAddress) {
        IJoePair _pair = IJoePair(pairAddress);
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
    ...
    function spot(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        address _token0 = joePair.token0();
        address _token1 = joePair.token1();
        require(_token0 == token || _token1 == token, "Invalid pair");
        (uint256 _reserve0, uint256 _reserve1, ) = joePair.getReserves();
        ...
        )
    {
        ...
        IJoePair joePair = IJoePair(_pair);
        price0Cumulative = joePair.price0CumulativeLast();
        price1Cumulative = joePair.price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 _blockTimestampLast) = joePair.getReserves();
        unchecked {
