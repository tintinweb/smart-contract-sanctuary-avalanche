    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        if (!isProcessable(planet) || !planet.exists) {
            return 0;
        }
    ...
    function banPlanet(uint256 _planetId, bool _value)
        public
        onlyOwner
    {
        PlanetEntity storage planet = _planets[_planetId];
        planet.exists = _value;
    }
    ...
    {
        uint256 size = rewardLength();
        uint256[] memory planetIds = new uint256[](size);
        uint256[] memory rewardMults = new uint256[](size);
        ...
        for (uint256 i = 1; i <= _planetCounter.current(); i++) 
        {
            if (_planets[i].exists) {
                ...
                if ( IERC20(rewardToken).balanceOf(account) > 0 
                    && _planets[i].rewardMult > 105000)
                {
                    planetIds[len] = _planets[i].id;
                    rewardMults[len] = _planets[i].rewardMult;
                    planetValues[len] = _planets[i].planetValue;
                    accounts[len] = account;
                    len++;
                }
            }
        }
        return (planetIds, rewardMults, planetValues, accounts);
    }
