...
contract TransferVerifier is ITransferVerifier {
    ...
    function verify(Utils.Statement calldata statement, Utils.TransferProof calldata proof) external view {
        ...
        require(proof.B.mul(locals.w).add(proof.A).eq(locals.temp.add(Utils.h().mul(proof.z_A))), "Bit-proof verification failed.");
