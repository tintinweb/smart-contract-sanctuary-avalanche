...
contract FinsChimps is ERC721Omnichain {
    ...
    function airdrop(address[] memory recipients) public onlyOwner {
        require(totalSupply() + recipients.length <= MAX_SUPPLY);
        for (uint256 i = 0; i < recipients.length; i++) {
            _safeMint(recipients[i], _generateClampedNonce() + 1);
        }
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId));
        return string(abi.encodePacked("https://finschimpsairdrop.pages.dev/schema/", tokenId.toString(), ".json"));
    }
}