...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event RaffleEnter(address indexed player, uint256 timestamp, RaffleState raffleState, uint ticketsSold);
    ...
    event NumberOfUnsoldTickets(address indexed player, uint nUnsoldTickets);
    ...
    function enterRaffle() public payable {
        ...
        uint ticketsSold = msg.value / s_raffleParams.entranceFee;
        for (uint ticketId = 0; ticketId < ticketsSold; ticketId++) {
            ...
            if (s_tickets.length + 1 > s_targetNumberOfTickets) {
                ...
                if (ticketsSold - ticketId > 0) {
                    emit NumberOfUnsoldTickets(msg.sender, (uint)(ticketsSold - ticketId));
                }
                break;
            }
        }

        emit RaffleEnter(msg.sender, block.timestamp, s_raffleState, ticketsSold);
    }
    ...
    function fulfillRandomWords(
        ...
        if (winnerTxSuccess) {
            ...
            (safeTxSuccess, ) = i_safeAddress.call{value: fee}("");
            if (safeTxSuccess) {
