...
contract TeamHunters is ERC721EnumerableV2 {
  ...
  // Team Create
  uint256 public constant CREATE_ORB = 20_000_000_000; // 20k $orb
  uint256 public constant CREATE_LOCK = 2 hours;

  // PVP
  uint256 public constant PVP_FEE = 10_000_000_000; // 10k $orb
  uint256 public constant ROUND_LOCK = 60 seconds;

  // BOT
  uint256 public constant BOT_FEE = 15_000_000_000; // 15k $orb
  uint256 public constant BOT_LOCK = 30 minutes;
  ...
  event BotResult(uint256 teamId, uint16 count, uint16 score);
  ...
  mapping(address => uint256) public botRewards; // user -> rewards
  mapping(address => uint256) public botAttends; // user -> lastAttend
  ...
  function registerTeam(
    ...
    orb.transferFrom(account, roundToken, CREATE_ORB); // 20k $orb
    ...
    ticker++;
    teams[ticker] = Team(name, size, hunters, block.timestamp + CREATE_LOCK);
  ...
  function getTeam(uint256 teamId) external view returns (Team memory team) {
    team = teams[teamId];
  }
  ...
  function enterRound(uint16 size, uint256[] calldata teamIds) external {
    ...
    uint256 teamCount = teamIds.length;
    uint16 count = uint16(TEAM_SIZE >> (size * 16));
    orb.transferFrom(account, roundToken, PVP_FEE * count * teamCount);

    uint256[] memory seedHashes = seeder.gets(block.timestamp, teamCount);

    for (uint256 i = 0; i < teamCount; i++) {
      uint256 teamId = teamIds[i];
      require(ownerOf[teamId] == account, "Invalid owner");
      require(teams[teamId].startsAt < block.timestamp, "Invalid lock");
      ...
      if (currentRound == 0 || rounds[currentRound].roundEndedAt > 0) {
        ...
        enterRoundInternal(roundTicker, teamId, seedHashes[i]);
      } else {
        require(teams[teamId].size == rounds[currentRound].size, "Invalid size");
        enterRoundInternal(currentRound, teamId, seedHashes[i]);
      }
    }
  }
  ...
  function getResult(
    ...
    for (uint16 i = 0; i < count; i++) {
      dataA = data.getData(teamA.hunters[i]);
      dataB = data.getData(teamB.hunters[i]);

      specialA = uint16(dataA >> (16 * 7)) % 3;
      specialB = uint16(dataB >> (16 * 7)) % 3;

      if (specialA == specialB) {
        isMaleA = uint16(dataA) > 0;
        isMaleB = uint16(dataB) > 0;
        if (isMaleA == isMaleB) {
          // Same gender -> random
          // 0, 2: teamB wins
          // 1, 3: teamA wins
    ...
    )
  {
    ...
    roundEndedAt = round.roundEndedAt;
  }

  function revealRound(uint256 roundId) external onlyOwner {
    require(rounds[roundId].roundEndedAt > 0, "Invalid round");
    rounds[roundId].roundEndedAt = block.timestamp;
  }

  function getBotResult(uint256 teamId, uint256 botHash) internal view returns (uint16 count, uint16 score) {
    Team storage team = teams[teamId];
    count = uint16(TEAM_SIZE >> (team.size * 16));

    uint16 matchHash = uint16(botHash);
    botHash = botHash >> 16;
    ...
    for (uint16 i = 0; i < count; i++) {
      dataA = data.getData(team.hunters[i]);

      specialA = uint16(dataA >> (16 * 7)) % 3;
      specialB = uint16(botHash) % 3;
      botHash = botHash >> 15;

      if (specialA == specialB) {
        isMaleA = uint16(dataA) > 0;
        isMaleB = (botHash & 0x1) > 0;
        botHash = botHash >> 1;
        if (isMaleA == isMaleB) {
          // Same gender -> random
          // 0, 2: teamB wins
  ...
  function bot(uint256[] calldata teamIds) external {
    address account = msg.sender;
    require(account == tx.origin, "Invalid sender");

    uint256 teamCount = teamIds.length;
    orb.transferFrom(account, roundToken, BOT_FEE * teamCount);

    uint256 timestamp = block.timestamp;
    require(botAttends[account] < timestamp, "Invalid time");
    botAttends[account] = timestamp + BOT_LOCK;

    uint256 rewards = botRewards[account];
    uint256[] memory botHashes = seeder.gets(timestamp, teamCount);

    for (uint256 i = 0; i < teamCount; i++) {
      uint256 teamId = teamIds[i];
      require(ownerOf[teamId] == account, "Invalid owner");

      (uint16 count, uint16 score) = getBotResult(teamId, botHashes[i]);
      rewards += (score > (count >> 1)) ? ((BOT_FEE * count) / score) : ((BOT_FEE * score) / count);
      emit BotResult(teamId, count, score);
    }

    botRewards[account] = rewards;
  }
