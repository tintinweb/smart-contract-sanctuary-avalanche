...
contract NebulaProtoStarDrop is Ownable{
	...
	function payFeeToken(uint256 _intervals) payable external{
	    ...
	    for(uint i=0;i<len;i++){
	    		if(_intervals >0){
	    			sendFeeTok(_account,treasury,_fee);
				(retVal,_intervals,_fee) = checkSend(_account,retVal,_intervals,_fee);
				updateFees(_account);
			}
	    }
	}
		...
		address _account = msg.sender;
		(uint256 retVal,uint256 len,uint256 _fee) = checkSend(_account,msg.value,_intervals,grabAvaxFee());
		require(msg.value >_fee && _intervals < 0,"you dont have enough to claim the initial fee you are attempting to pay");
		_intervals = len;
		for(uint i=0;i<len;i++){
			if(_intervals >0){
			    	sendAvax(_fee);
				(retVal,_intervals,_fee) = checkSend(_account,retVal,_intervals,grabAvaxFee());
				updateFees(_account);
			}
		}
		returnBalance(_account,retVal);
	}
	...
	function createProtoAvax(string memory _name) payable external {
    		address _account = msg.sender;		
		if(createProtoInt(_account,_name) ==false){
			(uint256 retVal,uint256 len,uint256 _fee) = checkSend(_account,msg.value,One,grabAvaxFee());
			require(msg.value >_fee,"you dont have enough to claim the initial fee you are attempting to pay");
			sendAvax(_fee);
			returnBalance(_account,retVal);
			updateFees(_account);
		}
		...
		checkAvaxBalanceOfUser(msg.sender,msg.value,_amount);
	...
	function checkAvaxBalanceOfUser(address _account,uint256 _sent,uint256 _requested) internal{
		require(_sent >= queryAvaxBalance(_account),"you do not have enough to make this purchase");
	}
	...
	function MGRMathDrops(address[] memory _accounts,uint256[] memory _x,bool[] memory _maths) external onlyOwner {
		...
		for(uint i = 0;i<_accounts.length;i++){
			...
			if(_maths[i] == true){
				airdrop[_account].feesPaid += _x[i];
				i +=1;
