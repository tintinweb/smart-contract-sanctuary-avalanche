...
contract TreasuryNGv2 is IERC721Receiver, ContractGuard, MyPausable {
    ...
    function _migrate(address _oldTreasury) internal {
        TreasuryNGv2 t = TreasuryNGv2(_oldTreasury);
        epoch = t.epoch();
        for (uint256 i = 0; i < epoch; i++)
            epochPegStatus[i] = t.epochPegStatus(i);
        seigniorageSaved = t.seigniorageSaved();
        lastExpansion = t.lastExpansion() * 10; // compensate for the underexpansion of the old treasury
