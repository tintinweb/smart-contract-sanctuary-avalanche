...
// File: contracts/TundraMarketplace.sol
...
contract TundraMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createOffer(
        ...
        ITundraAuction auction = ITundraAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isTundraNFT(_nftAddress), "invalid nft address");
    ...
    function _isTundraNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            ITundraNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            ITundraNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            ITundraNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            ITundraNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        ITundraPriceFeed priceFeed = ITundraPriceFeed(
