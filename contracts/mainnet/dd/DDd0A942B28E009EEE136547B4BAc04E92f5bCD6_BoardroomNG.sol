...
contract ShareWrapper {
    ...
    bool public memberMigrationFinished;
    ...
    function migrateStake(address usr, uint256 amount) public virtual {
        require(!memberMigrationFinished, "Migration closed");
        _totalSupply = _totalSupply.add(amount);
        _balances[usr] = _balances[usr].add(amount);
    }
...
contract BoardroomNG is ShareWrapper, ContractGuard, MyPausable {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        withdrawLockupEpochs = 6; // Lock for 6 epochs before release withdraw
        rewardLockupEpochs = 6; // Lock for 6 epochs before release claimReward

        initialized = true;
        operator = msg.sender;
        oldBoardroom = _oldBoardroom;
        _migrateHistory();
        _pause();
        emit Initialized(msg.sender, block.number);
    }

    function _migrateHistory() internal {
        BoardroomNG b = BoardroomNG(oldBoardroom);
        for (uint256 i = 0; i < treasury.epoch(); i++) {
            try b.boardroomHistory(i) returns (uint256 time, uint256 rewardReceived, uint256 rewardPerShare) {

                boardroomHistory.push(BoardroomSnapshot({time: time, rewardReceived: rewardReceived, rewardPerShare: rewardPerShare}));
            } catch {
                break;
            }
        }
    ...
    function migrateMembers(address[] calldata addrs, bool lastBatch) external onlyOwner {
        BoardroomNG b = BoardroomNG(oldBoardroom);
        for (uint256 i = 0; i < addrs.length; i++) {
            (uint256 lastSnapshotIndex, uint256 rewardEarned, uint256 epochTimerStart) = b.members(addrs[i]);
            members[addrs[i]].lastSnapshotIndex = lastSnapshotIndex;
            members[addrs[i]].rewardEarned = rewardEarned;
            members[addrs[i]].epochTimerStart = epochTimerStart;
            migrateStake(addrs[i], b.balanceOf(addrs[i]));
        }
        memberMigrationFinished = lastBatch;
    }
    ...
    function stake(uint256 amount) public override onlyOneBlock whenNotPaused updateReward(msg.sender) {
    ...
    function migrateStake(address usr, uint256 amount) public override onlyOwner {
        if (amount > 0 && balanceOf(usr) == 0) {
            super.migrateStake(usr, amount);
            emit Staked(usr, amount);    
        }
    }

    function withdraw(uint256 amount) public override onlyOneBlock whenNotPaused memberExists updateReward(msg.sender) {
    ...
    function claimReward() public updateReward(msg.sender) whenNotPaused {
        ...
        if (reward > 0) {
            ...
            glad.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        glad.safeTransferFrom(msg.sender, address(this), amount);
