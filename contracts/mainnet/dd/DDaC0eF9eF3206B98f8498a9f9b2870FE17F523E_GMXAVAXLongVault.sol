...
import "../../../../interfaces/vaults/gmx/v2/avax/IGMXAVAXLongManager.sol";
import "../../../../interfaces/vaults/gmx/v2/avax/IGMXAVAXLongReader.sol";
import "../../../../interfaces/oracles/IChainlinkOracle.sol";
import "../../../../interfaces/tokens/IWETH.sol";
import "../../../../interfaces/tokens/IStakedGLP.sol";
import "../../../../enum/ManagerAction.sol";

contract GMXAVAXLongVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  // Manager contract
  IGMXAVAXLongManager public manager;
  // Reader contract
  IGMXAVAXLongReader public reader;
  // Vault config struct
  ...
  */
  ...
  function deposit(address _token, uint256 _amt, uint256 _minSharesAmt) external nonReentrant whenNotPaused {
    ...
    require(
      _token == USDC || _token == sGLP || _token == WETH || _token == BTCb || _token == WAVAX,
      "Invalid token"
    );
  ...
  function depositNative(uint256 _amt, uint256 _minSharesAmt) payable external nonReentrant whenNotPaused {
    ...
    uint256 glpPrice = reader.glpPrice(false);
    uint256 depositValue = reader.tokenValue(WAVAX, _amt);
    uint256 equityBefore = reader.assetValueWithPrice(glpPrice) - reader.debtValue();

    IWETH(WAVAX).deposit{ value: msg.value }();
    IERC20(WAVAX).safeTransfer(address(manager), _amt);

    _deposit(WAVAX, _amt, _minSharesAmt, glpPrice, depositValue, equityBefore);
  }
  ...
  */
  function withdrawNative(uint256 _shareAmt, uint256 _minWithdrawAmt) external nonReentrant whenNotPaused() {
    _burnAndWork(WAVAX, _shareAmt);

    uint256 withdrawAmt = IERC20(WAVAX).balanceOf(address(this));
    require(withdrawAmt >= _minWithdrawAmt, "Assets received less than minimum");

    IWETH(WAVAX).withdraw(withdrawAmt);
  ...
  */
  ...
  function _deposit(
    ...
    IGMXAVAXLongManager.WorkData memory data = IGMXAVAXLongManager.WorkData(
      {
        token: _token,
        lpAmt: 0,
  ...
  */
  function updateAddress(uint256 _type, address _addr) external onlyOwner {
    if (_type == 1) {
      manager = IGMXAVAXLongManager(_addr);
    }

    if (_type == 2) {
      reader = IGMXAVAXLongReader(_addr);
    }
  ...
  */
  receive() external payable {
    require(msg.sender == WAVAX, "fallback function error");
  }
}