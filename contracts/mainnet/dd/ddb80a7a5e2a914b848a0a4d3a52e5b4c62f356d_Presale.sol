 ...
 */
 contract Presale is Owned{
    ...
    // finished payout for referral
    mapping (address => uint) public paidRefferal;
    ...
    uint256 public minBusdLimit = 500 * (10 ** 18);
    uint256 public maxBusdLimit = 3500 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _busd, ERC20 _pToken) {
        require(_rate > 0);
    ...
    function setMinUSDLimit(uint256 amount) external onlyOwner {
        minBusdLimit = amount;    
    }
    ...
    function setMaxUSDLimit(uint256 amount) external onlyOwner {
        maxBusdLimit = amount;    
    }

    // allows buyers to put their busd to get some pToken once the presale will closes
    function buy(uint _amount, address _referrer) public {
        require(isPresaleOpen, "Presale is not open yet");
        ...
        payReferralCommission(msg.sender, _amount, _referrer);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].busdAmount.mul(rate).div(RATE_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].busdAmount.mul(rate).div(RATE_DECIMALS);
        ...
        require(pToken.balanceOf(address(this)) >= pTokenToClaim, "There is not enough token on contract");
    ...
    function payReferralCommission(address _user, uint256 _pending, address _referrer) internal {
        if (referralCommissionRate > 0) {
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (_referrer != address(0) && commissionAmount > 0 && busd.balanceOf(address(this)) >= commissionAmount) {
                busd.safeTransfer(_referrer, commissionAmount);
                paidRefferal[_referrer] = paidRefferal[_referrer].add(commissionAmount);
