...
contract NodeERC1155 is INodeERC1155, ERC1155, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _nodeType, uint256 _amount, string calldata _uri) public {
        require(
        ...
        require(
            getOwnedNodeCountByType(msg.sender, _nodeType) + _amount <
                collection[_nodeType].purchaseLimit,
        ...
        if(isPresaleActive){
            require(
                isPresaleActive && corkToken.balanceOf(msg.sender) >= collection[_nodeType].price * _amount / 2,
                "receiver's balance is less than node price"
            );
        } else {
            require(
                !isPresaleActive && corkToken.balanceOf(msg.sender) >= collection[_nodeType].price * _amount,
                "receiver's balance is less than node price"
            );
        }
        ...
        _mint(msg.sender, _nodeType, _amount, "");
    }
