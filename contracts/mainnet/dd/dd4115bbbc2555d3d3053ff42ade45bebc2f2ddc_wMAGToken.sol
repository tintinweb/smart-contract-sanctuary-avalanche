...
// File: contracts/lib/OwnableMulti.sol
...
interface ISMAG {
    function rebase(uint256 MAGProfit_, uint256 epoch_)
        external
        returns (uint256);
...
contract wMAGToken is ERC20 {
    ...
    constructor(address _MAG, address _sMAG, address _bMAG, address _staking)
        ERC20("Wrapped MAG", "wMAG", 9)
    {
        MAG = _MAG;
        sMAG = _sMAG;
        bMAG = _bMAG;
    ...
    /**
     * @notice pull index from sMAG token
     */
    function index() public view returns (uint256) {
        return ISMAG(sMAG).index();
    }

    /**
        @notice converts gMAG amount to MAG
        @param _amount uint
        @return uint
     */
    function convertWMagToMag(uint256 _amount) public view returns (uint256) {
        return _amount.mul(index()).div(10**decimals());
    }

    /**
        @notice converts MAG amount to gMAG
        @param _amount uint
        @return uint
     */
    function convertMagToWMag(uint256 _amount) public view returns (uint256) {
    ...
    function stakeDepositedTokens(uint256 _amount) internal {
        ...
        IMagStaking(staking).stake(_amount, address(this));
        IMagStaking(staking).claim(address(this));
    }

    function unstakeTokens(uint256 _amount) internal {
        IERC20(sMAG).approve(staking, _amount);
        IMagStaking(staking).unstake(_amount, true);
    }

    /**
        @notice converts MAG to gMAG
        @param _amount uint
     */
    function deposit(uint256 _amount) public {
        ERC20(MAG).transferFrom(msg.sender, address(this), _amount);
        stakeDepositedTokens(_amount);
        uint256 wMagAmount = convertMagToWMag(_amount);
        _mint(msg.sender, wMagAmount);
    }

    /**
        @notice converts wMAG to MAG
        @param _amount uint
     */
    function withdraw(uint256 _amount) public {
        _burn(msg.sender, _amount);
        uint256 MagAmount = convertWMagToMag(_amount);
        unstakeTokens(MagAmount);
        ERC20(MAG).transfer(msg.sender, MagAmount);
    }

    /**
        @notice refill contract if needed
        @param _amount uint
     */
    function refill(uint256 _amount) public {
        ERC20(MAG).transferFrom(msg.sender, address(this), _amount);
        stakeDepositedTokens(_amount);
    }
