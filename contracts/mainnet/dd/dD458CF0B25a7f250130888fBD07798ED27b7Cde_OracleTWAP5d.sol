...
contract OracleTWAP5d {
    ...
    function update() external {
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = _currentCumulativePrices(address(pair));
        ...
        returns (uint256 amountOut) 
    {
        if (token == token0) {
            ...
            amountOut = uint256(UQ112x112.decode(uint224(price0Average * amountIn)));
        } else {
            require(token == token1, 'Oracle: INVALID_TOKEN');
            // Token Out => div by 1e18 to put the decimals
            amountOut = uint256(UQ112x112.decode(uint224(price1Average * amountIn)));
        }
    }
        ...
        returns (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) 
    {
        ...
        (uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast) = siclePair.getReserves();

        if (blockTimestamp > _blockTimestampLast) {
            // Storing cumulative Price Data on-Chain
            uint32 timeElapsed = blockTimestamp - _blockTimestampLast;
            price0Cumulative += uint256(UQ112x112.encode(_reserve1) / _reserve0 * 1e12) * timeElapsed; // *1e12 -> POPS and USDC have different decimals 1e18/1e6
            price1Cumulative += uint256(UQ112x112.encode(_reserve0) / _reserve1) * timeElapsed; 
        }
    }
