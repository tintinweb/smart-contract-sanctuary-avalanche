...
// File: contracts/interfaces/IStakingRewardsV3.sol
...
interface IStakingRewardsV3 {
    ...
    function migrateToLpStaking(uint256 amount) external;
...
// File: contracts/KalmStakingV3.sol
...
contract KalmarStakingV3 is IStakingRewardsV3 , ReentrancyGuard, Initializable {
    using SafeMath for uint256;
    ...
    IKalmarLpStaking public constant kalmarLpStaking = IKalmarLpStaking(0x472358A032051F42cfDD4aF2D5aa3eEc9E3c289F);
    ...
    function deposit(uint256 amount) external override nonReentrant updateReward(msg.sender)
     ...
     isNotEmergencyStop
     {
        ...
        token.mint(msg.sender, amounAfterFee);

        emit Staked(msg.sender, amount);
    }

    function depositWhitelist(address _for, uint256 _amount) external override nonReentrant updateReward(_for)
        onlyWhitelistStake
        isNotEmergencyStop
    {
        ...
        // deposit amount include fee
        uint256 amounAfterFee =  _amount;
        _totalSupply = _totalSupply.add(amounAfterFee);
        _balances[_for] = _balances[_for].add(amounAfterFee);
        token.mint(_for, amounAfterFee);
    ...
    function migrateToLpStaking(uint256 amount) external override nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
        ...
        token.burn(msg.sender, amount);
        stakingToken.safeApprove(address(kalmarLpStaking), uint256(-1));
        kalmarLpStaking.depositWhitelist(address(stakingToken), msg.sender, amount, false);
        stakingToken.safeApprove(address(kalmarLpStaking), uint256(0));
        emit Withdrawn(msg.sender, amount);
    }
