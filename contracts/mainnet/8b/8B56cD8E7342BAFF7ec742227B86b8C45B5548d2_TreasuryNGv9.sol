...
import "./glad_core/roman_nft/NFTMintRegister.sol";
import "./glad_core/Glad.sol";


contract TreasuryNGv9 is IERC721Receiver, ContractGuard, MyPausable {
    ...
    NFTMintRegister public mintRegister;

    bool public gen0LimitEnabled;
    ...
    function initialize(
        ...
        mintRegister = NFTMintRegister(_mintRegister);
    ...
    function _migrate(address _oldTreasury) internal {
        TreasuryNGv9 t = TreasuryNGv9(_oldTreasury);
        epoch = t.epoch();
        ...
        underpegMintedPerEpoch[epoch] = t.underpegMintedPerEpoch(epoch);
    }
    ...
    function setGen0LimitEnabled(bool enabled) external onlyOperator {
        gen0LimitEnabled = enabled;
    }
    ...
    function mintedPerAddress(address a) external view returns (uint256) {
        return mintRegister.genMintsPerAddress(1, a);
    }
        ...
        whenNotPaused
    {
        ...
        uint256 maxMint =  mintRegister.genMintsPerAddress(0, msg.sender);
        if (maxMint > 0) {
            if (maxMint > 1) {
                maxMint *= maxMint;
                maxMint /= 2;
        ...
        // 1 is standard underpeg type, not gen1
        uint256 mints =  mintRegister.genMintsPerAddress(1, msg.sender);
        require(!gen0LimitEnabled || mints <= maxMint, "Treasury: maximum amount of Patricians mintable per gen0 NFT reached for this address");
        require(block.timestamp > lastMintTime[msg.sender] +  BUY_DELAY, "Treasury: Wait until cooldown time between your Patrician mints");
        ...
        // 1 is standard underpeg type, nothing to do with actual generation
        mintRegister.addMint(1, msg.sender, _amount);
    ...
    function getMintPrice(uint256 amount) public view returns (uint256) {
        return PreyPredator(nft).getMintPrice(amount);
    }
    ...
    function mintGen0(uint256 amount) external payable whenNotPaused {
        ...
        require(
            PreyPredator(nft).mintedPerGen(0) + amount <= PreyPredator(nft).GEN0_TOKENS(),
            "Mint less, there are no this many gen0 tokens left"
        );

        uint256 mintCostEther = PreyPredator(nft).getMintPrice(amount);
        ...
        IPreyPredator(nft).mintGeneric(msg.sender, amount, 0, 0, true);

        // send back excess value
        if (msg.value > mintCostEther) {
            Address.sendValue(payable(msg.sender), msg.value - mintCostEther);
        }

        // send 25% to dao address, 75% to team address
        if (address(daoFund) != address(0) && address(teamFund) != address(0)) {
            Address.sendValue(payable(daoFund), address(this).balance * 25 / 100);
            Address.sendValue(payable(teamFund), address(this).balance);
        }

        mintRegister.addMint(0, msg.sender, amount);
...
contract NFTMintRegister is Controllable {
    ...
    function setInitialized(uint256 generation, bool init) external onlyOwner {
        initialized[generation] = init;
    }
