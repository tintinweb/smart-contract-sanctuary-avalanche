...
import "./traderjoe/JoeRouter02.sol";
import "./traderjoe/JoePair.sol";

contract spies is ERC721, Ownable {
    ...
    JoePair public joePair;

    bool public revealed;
    IERC20 public token;

    constructor() public ERC721("Spies", "SPY") {
        maxSupply = 750;
        maxPerWallet = 20;
        maxPerTx = 10;
        price = 0.75 ether;

        token = IERC20(0xefb0475757652c9dd7B8bE2C6e0d99fB05950aB1);
        joePair = JoePair(0xB1DAf32eb92BC5F6329BfCB44f6881FeF19641C0);
        joeRouter = JoeRouter02(payable(0x60aE616a2155Ee3d9A68541Ba4544862310933d4));
        privateSaleTime = 1674925200;
        publicSaleTime = 1674925200;
        unrevealedURI = "ipfs://bafybeib5t266oaji7jw6q4mzgjfx3fme3yt26ncteztu6idhwiilkxiova/0.json";
        uriExtension = "";
    }
    ...
    function getTokenPrice() public view returns (uint256) {
        ...
        uint256 tokenPrice = (reserve1 * price) / reserve0;
        return tokenPrice;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (revealed) {
            return string(abi.encodePacked(_baseURI(), tokenId.toString(), uriExtension));
        } else {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        // ERC2981
        else return super.supportsInterface(interfaceId);
    }
    ...
    function privateSale() public view returns (bool) {
        return block.timestamp >= privateSaleTime && block.timestamp < publicSaleTime;
    }
    ...
    function mint(uint256 amount) public payable {
        ...
        require(balanceOf(msg.sender) + amount <= maxPerWallet, "Max per wallet exceeded");
        require(msg.value >= price * amount, "Insufficient funds");
        for (uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, totalSupply() + 1);
            if (totalSupply() == maxSupply / 2) withdrawAll();
        }
        if (totalSupply() > maxSupply / 2) _addLP();
    }

    function mintWithToken(uint256 amount) public {
        require(publicSale(), "Sale not started");
        require(amount <= maxPerTx, "Max per tx exceeded");
        require(totalSupply() + amount <= maxSupply, "Max supply exceeded");
        require(balanceOf(msg.sender) + amount <= maxPerWallet, "Max per wallet exceeded");
        token.transferFrom(msg.sender, address(this), getTokenPrice() * amount);
        for (uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, totalSupply() + 1);
            if (totalSupply() == maxSupply / 2) withdrawAll();
        }
    ...
    function airdrop(address[] memory to, uint256[] memory amounts) public onlyOwner {
        for (uint256 i = 0; i < to.length; i++) {
            require(totalSupply() + amounts[i] <= maxSupply, "Max supply exceeded");
    ...
    function setWLPrice(uint256 wlPrice_) public onlyOwner {
        wlPrice = wlPrice_;
    }
    ...
    function _addLP() internal {
        ...
        if (tokenBalance > 0 && avaxBalance > 0) {
            token.approve(address(joeRouter), tokenBalance);
