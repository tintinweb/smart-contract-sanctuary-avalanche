...
contract CirculateWAVAX  is Ownable, ReentrancyGuard{
    ...
    address public WAVAXContract = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7; 

    struct DepositStruct{
        address investor;
        uint256 depositAmount;
        uint256 depositAt; // deposit timestamp
        uint256 claimedAmount; // claimed wavax amount
    ...
    function changeWAVAXContractAddress(address _wavaxContract) public onlyOwner{
        require(_wavaxContract!=address(0),"Please provide a valid address");
    ...
    function deposit(uint256 _amount, address _referrer) external {
        ...
        require(_amount > 0, "you can deposit more than 0 wavax");
        require( getBalance() + _amount <=  totalDepositLimit, "Overflow Deposit Limit" );

        IERC20(WAVAXContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(WAVAXContract).safeTransfer(devWallet,depositFee);
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(WAVAXContract).safeTransfer(msg.sender,amountToSend);
        totalReward = totalReward.add(amountToSend);

        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(WAVAXContract).safeTransfer(investors[msg.sender].referrer,(amountToSend*ferralfee).div(percentRate));
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(WAVAXContract).safeTransfer(investors[msg.sender].referrer,(claimableReward*ferralfee).div(percentRate));
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(WAVAXContract).balanceOf(address(this));
    }
    ...
    function getTotalAssets(address _otherToken) public view returns (uint256) {
        ...
        uint256 otherAssets = ( otherBalance * ISwapHelper(SwapHelper).getSwapOut( _otherToken, WAVAXContract, 1e6) ).div(1e6);
        uint256 mainAssets = IERC20(WAVAXContract).balanceOf(address(this));
    ...
    function getBorrowableAmount(address _toToken, address _trader) public view returns(uint256 _amount){
        (uint256 MaxStartMLimit,,, uint256 MaxLoanLimit, uint256 MaxTotalTradingLimit,,,uint256 LoanDivCollateral) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_toToken );
    ...
    function startTrading(address _trader, uint256 _borrowAmount, address _swappedToken) public {
        ...
        (,, uint256 MinCollateralLimit,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
        ...
        IERC20(WAVAXContract).safeApprove(SwapHelper, _borrowAmount);
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( WAVAXContract,_swappedToken, _borrowAmount);
    ...
    function getTotalProfit(address _token) public view returns(uint256) {
        (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract, _token );
    ...
    function withdrawCollateralFunds(uint256 _amount) external nonReentrant {
        ...
        debtors[msg.sender].withdrawableAmount -= _amount;
        debtors[msg.sender].collateralAmount -= _amount;
        IERC20(WAVAXContract).safeTransfer(msg.sender,_amount);
    }
    function getSwapBackAmount(address _trader) public view returns(uint256){
        if(debtors[_trader].tradingState){
            ...
            return ISwapHelper(SwapHelper).getSwapOut( _swappedToken, WAVAXContract, debtors[_trader].swappedAmount);
        }
        else
            return 0;
    }
    function getLoanInterestAmount(address _trader)public view returns (uint256){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
    ...
    function getUserBenefit(address _trader) public view returns(int256 _withdrawableAmount){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
            uint256 swapOutAmount = ISwapHelper(SwapHelper).getSwapOut( _swappedToken, WAVAXContract, debtors[_trader].swappedAmount);
    ...
    function endTrading( address _trader ) public {
        ...
        (,uint256 MaxEndMLimit,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
        ...
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( _swappedToken, WAVAXContract, debtors[_trader].swappedAmount);
    ...
    function IsNeededLiquidation(address _trader) public view returns (bool) {
        address _swappedToken = debtors[_trader].swappedToken;
        (,,,,,uint256 DailyLoanInterestRate,uint256 LiquidationLimitRate,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
        if( debtors[_trader].tradingState == false )
            return false;
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut(debtors[_trader].swappedToken , WAVAXContract, debtors[_trader].swappedAmount );
    ...
    function setAutoStartTrading( uint256 _borrowAmount, address _swappedToken, uint256 swappedDownLimit, uint256 swappedUpLimit ) external {
        ...
        require( _borrowAmount <= _borrowableAmount, "Over full borrowable amount limit" );
        (,, uint256 MinCollateralLimit,,,,,) = ISwapHelper(SwapHelper).TradingInfo( WAVAXContract,_swappedToken );
    ...
    function checkAutoStarting(address _trader) public view returns(bool){
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut( WAVAXContract, autoStarters[_trader].swappedToken, autoStarters[_trader].debtAmount );
