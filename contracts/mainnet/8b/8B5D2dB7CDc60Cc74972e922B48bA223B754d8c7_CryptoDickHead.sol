...
contract CryptoDickHead is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    ...
    string baseUri = "ipfs://QmW48o7E8scGxLK6nkZPW9M8dJNkra2mCvDaEZyC2LGuaJ/";
    ...
    function setRegister(address _register) external onlyOwner  {
        ...
        register = Register(payable(_register));
    }

    function setEarlyMinters(address[] calldata addresses, uint256 allowedToMint) external onlyOwner {
    ...
    function isEarlyMinter(address _whitelistedAddress) public view returns(uint256) {
        uint256 userIsWhitelisted = earlyMintAddresses[_whitelistedAddress];
        return userIsWhitelisted;
    }

    function setFreeMinters(address[] calldata addresses, uint256 allowedToMint) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            freeMintAddresses[addresses[i]] = allowedToMint;
        }
    }

    function removeFreeUser(address _addressToWhitelist) external onlyOwner {
        freeMintAddresses[_addressToWhitelist] = 0;
    }

    function isFreeMinter(address _whitelistedAddress) public view returns(uint256) {
        uint256 userIsWhitelisted = freeMintAddresses[_whitelistedAddress];
        return userIsWhitelisted;
    }
    ...
    function earlyClaim(uint256 n) public payable {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            _setTokenRoyalty(minted, registerAddress, ROYALTY_VALUE);
    ...
    function freeClaim(uint256 n) public {
        require(canClaim, "It's not possible to claim just yet.");
        require(freeMintAddresses[msg.sender] > 0, "You are not whitelisted");
        require(n + minted <= MAX_MINTABLE, "Not enough left to mint.");
        require(n > 0, "Number need to be higher than 0");
        require(n <= MAX_PER_CLAIM, "Max per claim is 10");

        freeMintAddresses[msg.sender] = 0;
        randomMint(_msgSender(), minted);
        _setTokenRoyalty(minted, registerAddress, ROYALTY_VALUE);
...
contract Register is ReentrancyGuard {
    ...
    // compute amount of royalties claimable by sender
    function holderClaimable() public view returns (uint256) {
         uint256 balance = token.balanceOf(msg.sender);
         require(balance > 0, "You don't own any nft");

        uint256 tokenPart = (amountReceived * 1000 ) / 5555000; // 5% holders
        uint256 total = 0;

         for (uint256 i=0; i < balance; i++) {
             uint256 tokenId = token.tokenOfOwnerByIndex(msg.sender, i);
             uint256 claimable = tokenPart - claimed[tokenId];
             total += claimable;
         }
         return total;
    }
    ...
    // compute amount of royalties claimable by sender
    function holderClaim() external nonReentrant {
         uint256 balance = token.balanceOf(msg.sender);
         require(balance > 0, "You don't own any nft");
        ...
        uint256 total = 0;
         ...
         require(total > 0, "No royalties to claim");
