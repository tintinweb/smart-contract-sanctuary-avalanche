...
contract GasStation is Ownable {
    ...
    IERC677 public LINK_ERC677 =
        IERC677(0x5947BB275c521040051D82396192181b413227A3);

    /* Wrapped AVAX Token */
    IWERC20 public WAVAX_ERC20 =
        IWERC20(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
    ...
    function _wrapMATIC(uint256 amount) internal returns (uint256) {
        WAVAX_ERC20.deposit{value: amount}();
        uint256 amountWrapped = WAVAX_ERC20.balanceOf(address(this));
        return amountWrapped;
    }

    function _swapToLINK(uint256 amountIn) internal returns (uint256) {
        WAVAX_ERC20.approve(address(router), amountIn);
        address[] memory path;
        path = new address[](2);
        path[0] = address(WAVAX_ERC20);
        path[1] = address(LINK_ERC677);
