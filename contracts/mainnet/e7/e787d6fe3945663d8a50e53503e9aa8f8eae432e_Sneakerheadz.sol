...
// Sneakerheadz by High Society - contract by xrpant
...
// File: contracts/Sneakerheadz.sol
...
contract Sneakerheadz is ERC721, Ownable, ReentrancyGuard, KindaRandom {
    ...
    address private _cm;
    address private _dub;
    address private _dev;

    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    bool public _mintActive;
    bool public _allowListOnly;

    mapping (address => uint8) public allowList;

    IERC20 _wavax;

    constructor() ERC721("Sneakerheadz", "SNKZ") KindaRandom(101) {
        _customBaseURI = "ipfs://bafybeiekzfouuihljk4py3eq43welogo64adspjzc3yh75cbhyigagimxm/";
        ...
        _maxMintPerTx = 3;
        _price = 0.2 ether;
        _cm = 0x49b18DfD1ED0e79d20A5Ad02b3FE197D85D17fFB;
        _dub = 0x5108a7b6485e9D87da8E1fe24E8127C961b6E317;
        _dev = 0xc03B9483B53c5b000Fa073D3C4549E0aEE6e2E8e;
    ...
    function flipAllowListOnly() public onlyOwner {
        _allowListOnly = !_allowListOnly;
    }
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        if (msg.sender != owner()) {
            ...
            require(quantity <= _maxMintPerTx, "Cannot mint that many.");
            require(msg.value == (quantity * _price), "Incorrect amount of AVAX sent.");

            if (_allowListOnly) {
                require(allowList[msg.sender] == 1, "You aren't AllowListed!");
            }
        }

        uint256 fifteenPercent = msg.value * 1500 / 10000;

        payable(_dev).transfer(fifteenPercent);
        payable(_dub).transfer(fifteenPercent);
        payable(_cm).transfer(msg.value - (fifteenPercent * 2));
    ...
    function updateCM(address _newAddress) public onlyOwner {
        _cm = _newAddress;
    }

    function updateDub(address _newAddress) public onlyOwner {
        _dub = _newAddress;
    }

    function updatePrice(uint256 _newAmount) public onlyOwner {
        _price = _newAmount;
    }

    function updateMaxPerTx(uint256 _newAmount) public onlyOwner {
        _maxMintPerTx = _newAmount;
    }
    ...
    function royaltyInfo(uint256, uint256 value) external view returns(address, uint256) {
        return (address(this) , value * _royaltyAmount / 10000);
    } 

    // Royalty functions from Leopard Queens contract
    function claimAVAX() public {
        uint256 balance = address(this).balance;
        uint256 fifteenPercent = balance * 1500 / 10000;

        payable(_dev).transfer(fifteenPercent);
        payable(_dub).transfer(fifteenPercent);
        payable(_cm).transfer(address(this).balance);
    }

    function claimWAVAX() public {
        uint256 balance = _wavax.balanceOf(address(this));
        if (balance > 0) {
            uint256 fifteenPercent = balance * 1500 / 10000;
            _wavax.transfer(_dev, fifteenPercent);
            _wavax.transfer(_dub, fifteenPercent);
            _wavax.transfer(_cm, _wavax.balanceOf(address(this)));
        }
    }
