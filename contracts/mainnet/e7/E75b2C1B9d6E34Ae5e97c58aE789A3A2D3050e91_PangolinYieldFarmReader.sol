...
contract PangolinYieldFarmReader {
  /* ========== STATE VARIABLES ========== */
  ...
  */
  ...
  /**
    * Returns the total value of token A & token B assets held by the manager; asset = debt + equity
    * @return assetValue   Value of total assets in 1e18
  */
  function assetValue() public view returns (uint256) {
    (uint256 _tokenAAssetAmt, uint256 _tokenBAssetAmt) = IPangolinYieldFarmManager(manager).assetInfo();

    return tokenValue(tokenA(), _tokenAAssetAmt) + tokenValue(tokenB(), _tokenBAssetAmt);
  }
  ...
  */
  function debtValue() public view returns (uint256, uint256) {
    (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt) = IPangolinYieldFarmManager(manager).debtInfo();
    return (tokenValue(tokenA(), _tokenADebtAmt), tokenValue(tokenB(), _tokenBDebtAmt));
  }

  /**
    * Returns the value of token A & token B equity held by the manager; equity = asset - debt
  ...
  */
  function equityValue() public view returns (uint256) {
    ...
    return (
      tokenValue(tokenA(), _tokenAAssetAmt) +
      tokenValue(tokenB(), _tokenBAssetAmt) -
      tokenValue(tokenA(), _tokenADebtAmt) -
      tokenValue(tokenB(), _tokenBDebtAmt)
    );
  }

  /**
    * Returns the amt of token A & token B assets held by manager
    * @return assetAmt   Amt of token A and token B asset in asset's decimals
  */
  ...
  /**
    ...
    * @return debtAmt   Amt of token A and token B debt in asset's decimals
  */
  ...
  */
  function lpAmt() public view returns (uint256) {
    return IPangolinYieldFarmManager(manager).lpTokenAmt();
  }
  ...
  */
  ...
  /**
    * Current delta: token A equity value / vault's equityValue; possible to be negative value
    * @return delta  Current delta a % in 1e18
  */
  function delta() public view returns (int) {
    (uint256 _tokenAAmt,) = IPangolinYieldFarmManager(manager).assetInfo();
    (uint256 _tokenADebtAmt,) = IPangolinYieldFarmManager(manager).debtInfo();

    if (_tokenAAmt == 0 && _tokenADebtAmt == 0) return 1;

    return (int(_tokenAAmt) - int(_tokenADebtAmt))
            * int(to18ConversionFactor(tokenA()))
            * int(priceOracle.consult(tokenA()))
            / int(equityValue());
  }

  /**
    * Debt ratio: token A and token B debt value / total asset value
  ...
  */
  ...
  /**
    ...
    * @param _amt Amount of token to convert to value; expressed in 1e18
    @ @return tokenValue tokenA value
  */
...
import "../../../contracts/vaults/pangolin/PangolinYieldFarmReader.sol";

contract $PangolinYieldFarmReader is PangolinYieldFarmReader {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _vault, address _manager, IChainLinkOracle _priceOracle) PangolinYieldFarmReader(_vault, _manager, _priceOracle) {}
