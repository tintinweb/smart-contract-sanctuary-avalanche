    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mint(
        ...
        if (payByReward_) {
            for (uint256 index = 0; index < tokenIds.length; index++) {
                require(msg.sender == ownerOf(tokenIds[index]));
            }
            _payByRewards(
                tokenIds,
        ...
        payable
    {
        ...
        require(tokenIds.length == months.length, "The Property:");
        for (uint256 index = 0; index < tokenIds.length; index++) {
            require(
            ...
            taxAmount +=
                properties.get(tokenIds[index]).propertyTypeIndex *
                months[index];
        ...
        require(
            msg.value >= getTaxInAVAX(taxAmount),
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            if (amountPaid + reward < amount) {
                ...
                properties.updateReward(
                    tokenIds[index],
                    amount - (amountPaid + reward),
                    processedTill
                );

                // Update the number of amount paid by reward till now.
                amountPaid += (amount - (amountPaid + reward));
    ...
    function payByReward(
        ...
        require(
            ...
            "The Property: Allowed to furniture contract only."
        );
