...
contract LockingChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct UserLocking {
        ...
        uint256 govPower; // equal to gXOXO amount
    ...
    // Total XOXO pending payout for all users
    // Debts increases when users lock XOXO
    // Debts decrease when users claim XOXO
    ...
    function endLock() external nonReentrant {
        ...
        // minimum gov power by minimum term
        uint256 minimumGovPower = estimateGovPower(0, user.principal);
        require(user.govPower >= minimumGovPower, "something wrong");

        // burn gXOXO
        gXoXo.burn(msg.sender, user.govPower);
