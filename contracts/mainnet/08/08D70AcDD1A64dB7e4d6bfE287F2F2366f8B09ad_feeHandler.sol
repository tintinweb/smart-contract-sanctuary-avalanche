    ...
    UUPSUpgradeable
{
    ...
    function processBribes(address pool, address[] calldata tokens, address to) public {
    ...
    function batchProcessBribes(address[] calldata pools, address[][] calldata tokens, address to) external {
        for (uint i; i < pools.length; ) {
            processBribes(pools[i], tokens[i], to);
            unchecked {
                ++i;
            }
        }
