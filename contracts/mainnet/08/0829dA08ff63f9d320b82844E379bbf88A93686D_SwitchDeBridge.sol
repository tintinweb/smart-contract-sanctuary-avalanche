...
contract SwitchDeBridge is Switch {
        ...
        nonReentrant
    {
        ...
        if (transferArgs.srcSwap.srcToken == transferArgs.srcSwap.dstToken) {
            ...
            if (transferArgs.useParaswap) {
                returnAmount = _swapFromParaswap(transferArgs, transferArgs.srcParaswapData, amountAfterFee);
            } else {
                returnAmount = _swapFromAggregator(transferArgs, amountAfterFee);
            }
        }

        require(returnAmount > 0, 'failed swapping token from dex');
        IERC20(transferArgs.srcSwap.dstToken).universalApprove(deBridgeGate, amountAfterFee);
        IDeBridgeGate(deBridgeGate).send{ value: transferArgs.nativeFee }(
