...
// File contracts/UsdcAvaxSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract UsdcAvaxSoliSnek  is Ownable, Pausable {
    ...
    ISoliSnekRouter.Routes[] public snekToUsdcPath;
    ...
    constructor(
        ...
        ISoliSnekRouter.Routes[] memory _snekToUsdcPath,
        ...
        for (uint i; i < _snekToUsdcPath.length; ++i) {
            snekToUsdcPath.push(_snekToUsdcPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        ISoliSnekRouter(router).swapExactTokensForTokens(snekHalf, 0, snekToUsdcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wavax).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdc).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wavax, usdc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
