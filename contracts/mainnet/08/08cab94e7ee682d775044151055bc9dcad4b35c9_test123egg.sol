...
contract test123egg is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	constructor(string memory _initUnrevealedURI, address _initArtistAddress, address _initCommunityAddress) ERC721("test123egg", "T1E") {
		paused = true;
		revealed = false;
		allowListings = true;
...
function mint(uint256 amount, uint256 _pid) public payable nonReentrant {
	...
	function withdrawWavax() public onlyOwner { 
     ...
     function withdrawOtherTokens(uint256 _pid) public payable onlyOwner() {
            TokenInfo storage tokens = AllowedCrypto[_pid];
            IERC20 paytoken;
            paytoken = tokens.paytoken;
