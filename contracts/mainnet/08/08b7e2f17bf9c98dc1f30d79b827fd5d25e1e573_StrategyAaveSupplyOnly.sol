...
// File contracts/strategies/Aave/StrategyAaveSupplyOnly.sol
...
contract StrategyAaveSupplyOnly is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 nativeFeeBal = IERC20(native).balanceOf(address(this)).mul(40).div(1000);

        uint256 callFeeAmount = nativeFeeBal.mul(callFee).div(MAX_FEE);
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 steakHutFeeAmount = nativeFeeBal.mul(steakHutFee).div(MAX_FEE);
    ...
    function callReward() public view returns (uint256) {
        return rewardsAvailable().mul(40).div(1000).mul(callFee).div(MAX_FEE);
    }
