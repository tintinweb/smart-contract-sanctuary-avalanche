...
contract TheTapV3e is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        total_users = 1;
        deposit_bracket_size = 10000e18; // 5% increase whale tax per 10000 tokens... 10 below cuts it at 50% since 5 * 10
    ...
    function removeFromBlackList(
        address[] memory blackListAddress
    ) external onlyOwner {
    ...
    function removeFromAdmins(address adminAddress) external onlyOwner {
        admins[adminAddress] = false;
    }

    function addToAdmins(address adminAddress) external onlyOwner {
    ...
    function setPayoutRateInterval(
        uint256 _payoutRateInterval
    ) external onlyOwner {
    ...
    function refundTax(uint256 _amount) external onlyAdmins {
    ...
    function deposit(
        address _upline,
        uint256 _amount
    ) external noBlackList isNotPaused {
        address _addr = msg.sender;
    ...
    function claim() external noBlackList isNotPaused {
        ...
        address _addr = msg.sender;
        require(
            block.timestamp > users[_addr].lastClaimOrRoll + minPayoutWait,
    ...
    function roll() external noBlackList isNotPaused {
        //Checkin for custody management.  If a user rolls for themselves they are active
        address _addr = msg.sender;
        if (users[_addr].currentPayoutRate < 200) {
            require(
                block.timestamp > users[_addr].lastClaimOrRoll + minPayoutWait,
                "You need to wait more time"
            );
    ...
    */
        ...
        )
    {
        ...
        if (users[_addr].payouts < max_payout) {
            uint256 timeElapsed = block.timestamp.safeSub(
            ...
            payout = share * timeElapsed;
            payout += users[_addr].accumulatedDiv;
            uint256 maxPayout = share *
                SafeMath.min(
                    block.timestamp.safeSub(custody[_addr].last_checkin),
                    maxPayoutPeriod
                );
    ...
    function decreasePayoutRate(address _addr) internal {
        ...
        users[_addr].lastClaimOrRoll = block.timestamp;
    }
    ...
    function airdrop(
        address _to,
        uint256 _amount
    ) public noBlackList isNotPaused {
        address _addr = msg.sender;
