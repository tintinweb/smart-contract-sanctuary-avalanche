...
contract Firn {
    ...
    function withdraw(bytes32[N] calldata Y, bytes32[N] calldata C, bytes32 D, bytes32 u, uint32 epoch, uint32 amount, uint32 tip, bytes calldata proof, address destination, bytes calldata data) external {
        ...
        if (burn > 0) { // buy and burn `uint256(burn) * 1e16` worth of FIRN token from router
            ...
            uint256[] memory amounts = router.swapExactAVAXForTokens{value: uint256(burn) * 1e16}(0, path, address(this), block.timestamp);
