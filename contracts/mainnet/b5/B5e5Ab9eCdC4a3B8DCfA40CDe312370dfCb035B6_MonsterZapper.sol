...
// File: contracts/interfaces/IWrappedFtm.sol
...
// File: contracts/interfaces/IHyperswapRouter.sol
...
contract MonsterZapper is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    // @WNATIVE - base token we use as a native for swaps when there is no liquidity pair to swap 
    ...
    modifier whitelist(address route) {
        require(whitelistedRouter[route], "route not allowed");
        _;
    }
    ...
    function zapInToken(address _in, uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountLp) external whitelist(routerAddr) {
       ...
       require (lpAmount >= minAmountLp, "lpAmt < minAmount quoted");
    }
    // @_in - Token we want to throw in
    // @amount - amount of our _in
    // @out - address of LP we are going to get
    ...
    function estimateZapInToken(address _in, address out, address router, uint256 amount) public view whitelist(router) returns (uint256, uint256) {
        ...
        if (_in == IUniswapV2Pair(out).token0() || _in == IUniswapV2Pair(out).token1()) {
            ...
            // go through WNATIVE token, that's not in our LP, for highest liquidity
            uint256 nativeAmount = _in == WNATIVE ? amount : _estimateSwap(_in, amount, WNATIVE, router);
            return estimateNativeZapIn(out, router, nativeAmount);
        }
    }

    function estimateNativeZapIn(address LP, address router, uint256 amount) public view whitelist(router) returns (uint256, uint256) {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint256 tokenAmount = _estimateSwap(WNATIVE, zapAmount, token, router);
            if (token0 == WNATIVE) {
            ...
            uint256 amountToken0 = _estimateSwap(WNATIVE, zapAmount, token0, router);
            uint256 amountToken1 = _estimateSwap(WNATIVE, zapAmount, token1, router);
      ...
      // from an LP token to WNATIVE through specified router
    ...
    function zapOutToNative(address _in, uint256 amount, address routerAddr, address recipient, uint256 minAmountNative) external whitelist(routerAddr) {
        ...
        _approveTokenIfNeeded(pair._token1, routerAddr);
         ...
         if (pair._token0 == WNATIVE || pair._token1 == WNATIVE) {
            // if so, we only need to swap one, figure out which and how much
            args._token = pair._token0 != WNATIVE ? pair._token0 : pair._token1;
           
           // half of FTM is sent to recepient already after removeLiqETH
            (pair._amountToken0, pair._amtETH) = IUniswapV2Router(routerAddr).removeLiquidityETH(args._token, args._amount, 0, 0, address(this), block.timestamp);
            // swap the other half of tokens to WNATIVE and send to this contract
           uint256 halfOfEth =  _swapTokenForNative(args._token, pair._amountToken0, address(this), args._routerAddr);
           // unWrap WNATIVE into NATIVE(ftm in this case)
           IWrappedFtm(WNATIVE).withdraw(halfOfEth);
            require(halfOfEth.add(pair._amtETH) >= minAmountNative, "WNATIVE amt < minAmountNative");
            // send other half of NATIVE
            TransferHelper.safeTransferETH(args._recipient, pair._amtETH.add(halfOfEth));
        } else {

        (pair._amountToken0, pair._amountToken1) = IUniswapV2Router(routerAddr).removeLiquidity(pair._token0, pair._token1, args._amount, 0, 0, address(this), block.timestamp);
            ...
            IWrappedFtm(WNATIVE).withdraw(pair._amountToken0);
            IWrappedFtm(WNATIVE).withdraw(pair._amountToken1);
        
        require (pair._amountToken0.add(pair._amountToken1) >= minAmountNative, "WNATIVE amt < minAmountNative");
        TransferHelper.safeTransferETH(args._recipient, pair._amountToken0.add(pair._amountToken1));
   ...
   // @in - LP we want to throw in
    ...
    function zapOutToToken(address _in, uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountToken) whitelist(routerAddr) external {
        ...
        if (pair._token0 != args._out) {
            if(pair._token0 == WNATIVE) {
                pair._amountToken0 = _swapNativeForToken(args._out, pair._amountToken0, address(this), args._routerAddr);
            } else {
                pair._amountToken0 = this._swap(pair._token0, pair._amountToken0, args._out, address(this), args._routerAddr);
            }
        }
        if (pair._token1 != args._out) {
            if (pair._token1 == WNATIVE) {
                pair._amountToken1 = _swapNativeForToken(args._out, pair._amountToken1, address(this), args._routerAddr);
            } else {
                pair._amountToken1 = this._swap(pair._token1, pair._amountToken1, args._out, address(this), args._routerAddr);
            }
        }
    ...
    function estimateZapOutToken(address _fromLp, address _to, address _router, uint256 minAmountToken0, uint256 minAmountToken1 ) public view whitelist(_router) returns (uint256) {
        ...
        if(token0 == WNATIVE) {
            ...
            if(_to == token1) {
                ...
                uint256 halfAmountof_to = _estimateSwapNativeForToken(_to, minAmountToken0, _router);
            ...
            if (_to == token0) {
                ...
                uint256 otherhalfAmountof_to = _estimateSwapNativeForToken(_to, minAmountToken1, _router);
                return halfAmountof_to.add(otherhalfAmountof_to);
            }
        }
    }
    ...
    function _swap(address _in, uint256 amount, address out, address recipient, address routerAddr) external whitelist(routerAddr) returns (uint256) {
        ...
        if (fromBridge != address(0) && toBridge != address(0)) {
            ...
            path[2] = toBridge;
            path[3] = out;
        } else if (_in == WNATIVE || out == WNATIVE) {
            path = new address[](2);
            path[0] = _in;
            path[1] = out;
        } else {
    ...
    function _estimateSwapNativeForToken(address token, uint256 amount, address routerAddr) public view whitelist(routerAddr) returns (uint256) {
        address[] memory path;
        IUniswapV2Router router = IUniswapV2Router(routerAddr);
    ...
    function _swapTokenToLP(address _in, uint256 amount, address out, address recipient, address routerAddr) private returns (uint256) {
        ...
        if (args._in == IUniswapV2Pair(args._out).token0() || args._in == IUniswapV2Pair(args._out).token1()) {
            ...
            args._otherAmt = this._swap(args._in, args._swapAmt, args._token, address(this), args._routerAddr);
            ...
            _dustDistribution(args._in, args._token, args._recipient);
            return pair._liqTokenAmt;
        } else {
            // go through WNATIVE token for highest liquidity
    ...
    // @amount - amount of our WNATIVE token
    // @out - LP we want to get
    ...
    function _dustDistribution(address token0, address token1, address recipient) private {
        uint256 token0_Dust = IERC20(token0).balanceOf(address(this));
        uint256 token1_Dust = IERC20(token1).balanceOf(address(this));

        if (token0_Dust > 0) {
            IERC20(token0).safeTransfer(recipient, token0_Dust);
        } 
        if (token1_Dust > 0) {
            IERC20(token1).safeTransfer(recipient, token1_Dust);
        } 
    ...
    function _swapNativeToEqualTokensAndProvide(address token0, address token1, uint256 amount, address routerAddress, address recipient) private returns (uint256) {
        ...
        if (pair._token0 == WNATIVE) {
            ...
            _dustDistribution( pair._token0, pair._token1, args._recipient);
            return pair._liqTokenAmt;
        } else  if (pair._token1 == WNATIVE) { 
            _approveTokenIfNeeded( pair._token0, args._routerAddr);
            _approveTokenIfNeeded( pair._token1, args._routerAddr);
            ...
            _dustDistribution( pair._token1, pair._token0, args._recipient);
            return pair._liqTokenAmt;
        } else {
            _approveTokenIfNeeded( pair._token0, args._routerAddr);
            _approveTokenIfNeeded( pair._token1, args._routerAddr);
            uint256 amountToken0 = _swapNativeForToken(pair._token0,  args._swapAmt, address(this), args._routerAddr);
            uint256 amountToken1 = _swapNativeForToken(pair._token1,  args._amount.sub(args._swapAmt), address(this), args._routerAddr);
            
            (pair._amountToken0, pair._amountToken1, pair._liqTokenAmt) = router.addLiquidity(pair._token0, pair._token1, amountToken0, amountToken1, 0, 0, args._recipient, block.timestamp);
            _dustDistribution( pair._token1, pair._token0, args._recipient);
            return pair._liqTokenAmt;
    ...
    function addWhitelistedRouter(address router) external onlyOwner {
        whitelistedRouter[router] = true;
    }

    function removeWhitelistedRouter(address router) external onlyOwner {
        whitelistedRouter[router] = false;
    }
}