...
import "../interfaces/swaps/pangolin/IPangolinFactory.sol";
import "../interfaces/swaps/pangolin/IPangolinRouter02.sol";
import "../interfaces/oracles/IChainLinkOracle.sol";


contract PangolinOracle {
  ...
  */
  function lpToken(
    ...
    return IPangolinFactory(factory).getPair(_tokenA, _tokenB);
  }
  ...
  */
  function getAmountsOut(
    ...
    require(
      _tokenA == IPangolinPair(_pair).token0() || _tokenA == IPangolinPair(_pair).token1(),
      "invalid token in pool"
    );
    require(
      _tokenB == IPangolinPair(_pair).token0() || _tokenB == IPangolinPair(_pair).token1(),
      "invalid token in pool"
    );
    ...
    return IPangolinRouter02(router).getAmountsOut(_amountIn, path)[1];
  }
  ...
  */
  function getAmountsIn(
    ...
    return IPangolinRouter02(router).getAmountsIn(_amountOut, path)[0];
  }
  ...
  */
  function getLpTokenReserves(
    ...
    (uint256 reserve0, uint256 reserve1, ) = IPangolinPair(_pair).getReserves();
    uint256 totalSupply = IPangolinPair(_pair).totalSupply();

    if (_tokenA == IPangolinPair(_pair).token0() && _tokenB == IPangolinPair(_pair).token1()) {
      reserveA = reserve0;
      reserveB = reserve1;
    } else {
      reserveA = reserve1;
  ...
  */
  function getLpTokenAmount(
    ...
    uint256 lpTokenValue = getLpTokenValue(
      IPangolinPair(_pair).totalSupply(),
      _tokenA,
      _tokenB,
      _pair
    );

    uint256 lpTokenAmount = _value * IPangolinPair(_pair).totalSupply() / lpTokenValue;
...
import "../../../../contracts/interfaces/swaps/pangolin/IPangolinFactory.sol";

abstract contract $IPangolinFactory is IPangolinFactory {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../contracts/interfaces/tokens/IERC20.sol";
...
import "../../contracts/oracles/PangolinOracle.sol";

contract $PangolinOracle is PangolinOracle {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _factory, address _router, address _chainlinkOracle) PangolinOracle(_factory, _router, _chainlinkOracle) {}
