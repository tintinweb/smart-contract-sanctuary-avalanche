...
contract BYOB is ERC721URIStorage, Ownable {
  ...
  constructor(address signer_, address userBase_) ERC721("Not BYOB", "NOTBYOB") {
    _customBaseURI = 'https://nfts.bpi.network/byob/';
    _maxMintable = 24; // 24 max supply
    _royaltyAmount = 500; // 5% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 1;
    _signer = signer_;
    _userBaseAddress = userBase_;
  }
  ...
  function signatureMint(uint256 quantity, bytes32 _salt, bytes memory _signature) public payable {

    UserBase userBase = UserBase(_userBaseAddress);
    require(keccak256(abi.encodePacked((userBase.usernameFor(msg.sender)))) != keccak256(abi.encodePacked((""))), "Must have username set.");

    bytes32 hash = keccak256(abi.encodePacked(msg.sender, address(this), _salt));
