...
contract SnakeGenesisRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SNAKE to distribute.
        uint256 lastRewardTime; // Last time that SNAKE distribution occurs.
    ...
    // TESTNET
    // uint256 public snakePerSecond = 3.0555555 ether; // 11000 SNAKE / (1h * 60min * 60s)
    // uint256 public runningTime = 24 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    uint256 public snakePerSecond = 0.115740741 ether; // 20000 SNAKE / (1h * 60min * 60s * 2)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SnakeGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SnakeGenesisPool: existing pool?");
        }
    }
    ...
    function safeSnakeTransfer(address _to, uint256 _amount) internal {
        uint256 _snakeBalance = snake.balanceOf(address(this));
        if (_snakeBalance > 0) {
            if (_amount > _snakeBalance) {
                snake.safeTransfer(_to, _snakeBalance);
            } else {
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (SNAKE or lps) if less than 90 days after pool ends
