...
contract MTest is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => AMMPair) public ammPairs;
    struct AMMPair {
        bool isActive;
        bool useSwapExactTokensForAVAXSupportingFeeOnTransferTokens;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        //sell
        if (!isExcludedFromFee[from] &&  ammPairs[to].isActive) {
        ...
        //buy
        if (!isExcludedFromFee[to] &&  ammPairs[from].isActive) {
            fee = buyFee;
    ...
    /*** referrals ***/
    // 100 = 1.00%
    function setReferralConfig(uint256 _referralRateForBuySell, uint256 _referralRateForNodeCreation, 
        uint256 _referralRateForNodeRewards, uint256 _minTokensForReferral, uint256 _minNodesForReferral)
        external onlyOwner 
    {
        ...
        maxBuyFee = maxBuyFee - referralRateForBuySell + _referralRateForBuySell;
        maxSellFee = maxSellFee - referralRateForBuySell + _referralRateForBuySell;
    ...
    function setAutomatedMarketMakerPair(address pair, bool isActive, bool _useSwapExactTokensForAVAXSupportingFeeOnTransferTokens) external onlyOwner {
        AMMPair storage thisPair =  ammPairs[pair];
        require(thisPair.isActive != isActive, "AMM_ALREADY_SET");
        thisPair.isActive = isActive;
        thisPair.useSwapExactTokensForAVAXSupportingFeeOnTransferTokens = _useSwapExactTokensForAVAXSupportingFeeOnTransferTokens;
        emit SetAutomatedMarketMakerPair(pair, isActive, _useSwapExactTokensForAVAXSupportingFeeOnTransferTokens);
    }
    ...
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed isActive, bool useSwapExactTokensForAVAXSupportingFeeOnTransferTokens);
