...
contract rigelInstantJoe is Context {
    ...
    function swapTokensForETH(
        ...
        require(IERC20(path[0]).transferFrom(_user, address(this), _amountIn));
        smartSwap.swapExactTokensForAVAX(
            _amountIn,
            outputAmount[1],
            path,
            _user,
    ...
    function SwapETHForTokens(uint256 _id, address _user, uint256 amount) external {
        ...
        _userData.time = block.timestamp;
        if(_userData.path[0] == smartSwap.WAVAX()) {
            smartSwap.swapExactAVAXForTokens{value: amount}(
                outputAmount[1],
                _userData.path,
                _user,
                block.timestamp + 1200
    ...
    function withdrawWAVAX(address _user, uint256 _amount) external onlyOwner {
