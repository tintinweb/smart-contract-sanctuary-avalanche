...
contract SwapHelper is Initializable, OwnableUpgradeable {
        ...
        if (customRouteLength > 0) {
            ...
            for (uint256 i; i < customRouteLength; i++) {
                ...
                if (poolOrRouter == address(ROUTER) || poolOrRouter == address(PNG_ROUTER)) {
                    _amount = _previewSwapOnAMM(inToken, _amount);
                } else {
            ...
            if (routeToAvaxOnPlatypus[token] != address(0)) {
                ...
                avaxAmount = _previewSwapOnAMM(token, amount);
            }
        }
    }
    ...
    function _previewSwapOnAMM(address tokenIn, uint256 amount)
        internal
        view
        returns (uint256 amountOut)
    {
        if (tokenIn == WAVAX) return amount;
        ...
        if (token == WAVAX) {
            ...
            if (customRouteLength > 0) {
                ...
                for (uint256 i; i < customRouteLength; i++) {
                    ...
                    if (poolOrRouter == address(ROUTER) || poolOrRouter == address(PNG_ROUTER)) {
                        _amount = _swapOnAMM(inToken, _amount, _minAmountReceived, _receiver);
                    } else {
                ...
                if (routeToAvaxOnPlatypus[token] != address(0)) {
                    ...
                    wavaxAmount = _swapOnAMM(token, amount, minAmountReceived, receiver);
                }
            }
        }
    }
