...
contract TraderJoeYieldFarmVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  ...
  event UpdateManager(ITraderJoeYieldFarmManager _manager);
  event UpdateReader(ITraderJoeYieldFarmReader _reader);
  ...
  */
  function pendingMgmtFee() public view returns (uint256) {
    ...
    return (totalSupply() * mgmtFeePerSecond * secondsFromLastCollection)
      / SAFE_MULTIPLIER;
  }
  ...
  */
  function deposit(uint256 _amt, uint256 _minSharesAmt) external payable
    ...
    if (msg.value > 0) {
      ...
      IWAVAX(address(depositToken)).deposit{ value: msg.value }();
  ...
  */
  ...
  /**
    ...
    * @param _borrowTokenAAmt Amount of borrow token to borrow
    * @param _borrowTokenBAmt Amount of borrow token to borrow
    * @param _repayTokenAAmt Amount of borrow token to repay
    * @param _repayTokenBAmt Amount of borrow token to repay
  */
  function updateVaultConfig(
    ...
    uint256 _repayTokenBAmt) external onlyOwner {
    vaultConfig = _vaultConfig;

    rebalance(_action, _lpAmt, _borrowTokenAAmt, _borrowTokenBAmt, _repayTokenAAmt, _repayTokenBAmt);
