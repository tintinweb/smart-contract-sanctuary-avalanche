...
contract Zap is Ownable {
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external {
        ...
        IJoeRouter02(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);
    }
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtAVAX;
            (amtToken, amtAVAX) = IJoeRouter02(routerAddr).removeLiquidityAVAX(token, amount, 0, 0, address(this), block.timestamp);
            ...
            uint amt0;
            uint amt1;
            (amt0, amt1) = IJoeRouter02(routerAddr).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
    ...
    function _swapTokenToLP(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) { // check if we already have one of the assets
            ...
            uint liquidity;
            ( , , liquidity) = IJoeRouter02(routerAddr).addLiquidity(_from, other, amount.sub(sellAmount), otherAmount, 0, 0, recipient, block.timestamp);
            return liquidity;
        } else {
            // go through native token for highest liquidity
