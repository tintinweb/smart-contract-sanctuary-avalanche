...
contract HOEPublicPresale is ReentrancyGuard {
    ...
    function setWhitelistPresale( bool _f) external onlyOwner {
        isWhitelistPresale = _f;
    }
	...
	function buy(uint256 _amount) public nonReentrant {
		...
		uint256 tokens = _amount.div(USDCperToken).mul(10**11);
	...
	function getHOETokensLeft() external view returns (uint256) {
		return HOE.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some HOE tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed HOE tokens");
		require(HOE.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough HOE tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		HOE.transfer(msg.sender, callerTokensUnclaimed);
		emit TokenClaim(msg.sender, callerTokensUnclaimed);
	}

    function setHOE(address _hoe) external onlyOwner {
        require(_hoe != address(0));
        HOE = IBEP20(_hoe);
    }
	...
	function withdrawUnsoldHOE() external onlyOwner {
		uint256 amount = HOE.balanceOf(address(this)) - totalTokensSold;
		HOE.transfer(msg.sender, amount);
	}

	function withdrawAllHOE() external onlyOwner {
		HOE.transfer(msg.sender, HOE.balanceOf(address(this)));
	}
