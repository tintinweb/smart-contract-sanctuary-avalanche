...
contract MINDv2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // referral
        if (isBuyOrSell) {
                        transferAmount -= _processReferralFeeWithReferral(from, referral, 
                amount, referralRateForBuySell);
        } 
    ...
    // 100 = 1.00%
    function setProcessFeeConfig(uint256 _rewardProcessFee) external onlyOwner {
    ...
    function setBrainsNFTContract(address value) external onlyOwner {
        require(value != address(0), "ADDRESS_IS_ZERO");
        brainsContract = value;
    }
    ...
    function updateReferralIncome(address referral, uint256 referralAmount) external onlyApproved {
        ReferralIncome[referral] += referralAmount;
    }

    // ensures referral fulfills requirements to receove referral inocme
    function _getReferral(address referral) private view returns(address result) {
        result = referral == address(0)  || balanceOf(referral) < minTokensForReferral
            || IERC721Upgradeable(brainsContract).balanceOf(referral) < minNodesForReferral
            ? address(this) 
            : referral;
    } 
    ...
    function _processReferralFee(address user, uint256 amount, uint256 rate) private returns(uint256 referralAmount) {
        ...
        referral = _getReferral(referral);
        referralAmount = amount * rate / 10000;
        if (referralAmount > 0) {
            ReferralIncome[referral] += referralAmount;
            super._transfer(user, referral, referralAmount);
        }
    } 

    function _processReferralFeeWithReferral(address user, address referral, uint256 amount, uint256 rate) private returns(uint256 referralAmount) {
        if (rate == 0) return 0;
        referral = _getReferral(referral);
        referralAmount = amount * rate / 10000;
        if (referralAmount > 0) {
    ...
    function _processRewardProcessFee(uint256 rewardAmount) private returns(uint256 processFee) {
        if (rewardProcessFee > 0 && rewardProcessFeeRecipient != address(0)) {
            ...
            if (processFee > 0) { 
                ...
                super._transfer(_msgSender(), rewardProcessFeeRecipient, processFee);
            }
        }        
    }
    ...
    modifier onlyApproved() {
        require(proxyToApproved[_msgSender()] == true || _msgSender() == owner(), "ONLY_APPROVED");
        _;
    }       
    ...
    modifier onlyRecoveryAdmin() {
        require(_msgSender() == recoveryAdmin, "LERC20_NOT_RECOVERY_ADMIN");
        _;
    }
    ...
    function setLosslessController(address _controller) public onlyOwner {
        require(_controller != address(0), "LERC20_CONTROLLER_IS_ZERO");
        require(_controller != address(lossless), "LERC20_SAME_ADDRESS");
    ...
    function setLosslessAdmin(address newAdmin) external onlyOwner {
        require(newAdmin != admin, "LERC20_SAME_ADDRESS");
        admin = newAdmin;
    }
    ...
    function acceptRecoveryAdminOwnership(bytes memory key) external {
        require(_msgSender() == recoveryAdminCandidate, "LERC20_NOT_CANDIDATE");
        require(keccak256(key) == recoveryAdminKeyHash, "LERC20_INVALID_KEY");
    ...
    function transferOutBlacklistedFunds(address[] calldata from) external {
        require(_msgSender() == address(lossless), "LERC20_NOT_LOSSLESS_CONTRACT");
        for(uint256 i; i < from.length;i++) {
            address fromAddress = from[i];
            _transfer(fromAddress, address(lossless), balanceOf(fromAddress));
        }
    }
    ...
    */
    function proposeLosslessTurnOff() external onlyRecoveryAdmin {
        require(losslessTurnOffTimestamp == 0, "LERC20_ALREADY_PROPOSED");
        require(isLosslessOn, "LERC20_ALREADY_OFF");
    ...
    */
    function executeLosslessTurnOff() external onlyRecoveryAdmin {
        require(losslessTurnOffTimestamp != 0, "LERC20_NOT_PROPOSED");
        require(losslessTurnOffTimestamp <= block.timestamp, "LERC20_TIMELOCK_IN_PROGRESS");
        isLosslessOn = false;
    ...
    */
    function executeLosslessTurnOn() external onlyRecoveryAdmin {
        require(!isLosslessOn, "LERC20_ALREADY_ON");
        losslessTurnOffTimestamp = 0;
        isLosslessOn = true;
    }
}
