...
contract DegenSwapProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberTripToReactivate;
    uint256 public numberTripToRegister;
    uint256 public numberTripToUpdate;
    ...
    constructor(
        IBEP20 _tripToken,
        uint256 _numberTripToReactivate,
        uint256 _numberTripToRegister,
        uint256 _numberTripToUpdate
    ) public {
        tripToken = _tripToken;
        numberTripToReactivate = _numberTripToReactivate;
        numberTripToRegister = _numberTripToRegister;
        numberTripToUpdate = _numberTripToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends TRIP to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer TRIP tokens to this contract
        tripToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberTripToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer TRIP token to this address
        tripToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberTripToUpdate
    ...
    /**
     * @dev Claim TRIP to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        tripToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of TRIP to register
     * Callable only by owner admins.
     */
    function updateNumberTrip(
        uint256 _newNumberTripToReactivate,
        uint256 _newNumberTripToRegister,
        uint256 _newNumberTripToUpdate
    ) external onlyOwner {
        numberTripToReactivate = _newNumberTripToReactivate;
        numberTripToRegister = _newNumberTripToRegister;
        numberTripToUpdate = _newNumberTripToUpdate;
    }
...
contract ClaimBackTrip is Ownable {
    using SafeBEP20 for IBEP20;

    IBEP20 public tripToken;
    DegenSwapProfile degenSwapProfile;

    uint256 public numberTrip;
    ...
    constructor(
        IBEP20 _tripToken,
        address _degenSwapProfileAddress,
        uint256 _numberTrip,
        uint256 _thresholdUser
    ) public {
        tripToken = _tripToken;
        degenSwapProfile = DegenSwapProfile(_degenSwapProfileAddress);
        numberTrip = _numberTrip;
        thresholdUser = _thresholdUser;
    }

    function getTripBack() external {
        // 1. Check if she has registered
        require(degenSwapProfile.hasRegistered(_msgSender()), "not active");

        // 2. Check if she has claimed
        require(!hasClaimed[_msgSender()], "has claimed TRIP");

        // 3. Check if she is active
        uint256 userId;
        (userId, , , , , ) = degenSwapProfile.getUserProfile(_msgSender());
        ...
        // Transfer TRIP tokens from this contract
        tripToken.safeTransfer(_msgSender(), numberTrip);
    }
    ...
    function canClaim(address _userAddress) external view returns (bool) {
        if (!degenSwapProfile.hasRegistered(_userAddress)) {
            return false;
        } else if (hasClaimed[_userAddress]) {
            return false;
        } else {
            uint256 userId;
            (userId, , , , , ) = degenSwapProfile.getUserProfile(_userAddress);
