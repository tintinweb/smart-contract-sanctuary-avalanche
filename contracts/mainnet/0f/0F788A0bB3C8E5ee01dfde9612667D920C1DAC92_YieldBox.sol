...
interface IRewards {
    function getRewardsExtension() external view returns (address);
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier claimer(address _claimer) {
        if (
            ...
            revert("Not elegible to claim YieldBoxes");
        }
        _;
    }
    ...
    function create(address _currency) external nonReentrant whenNotPaused {
        craftYB(_msgSender(), _currency);
    }

    function createFor(address _user, address _currency)
        external
        nonReentrant
        whenNotPaused
    {
        require(
            _msgSender() == RewardsExtension,
            "Caller is not the rewards extension contract"
        );
        craftYB(_user, _currency);
    }

    function craftYB(address _user, address _currency) internal {
        require(
            TOTAL_SUPPLY >= totalSupply().add(uint256(1)),
            "Unable to mint YieldBox, maximum supply reached"
        );
        require(isValidToken(_currency), "Token currency not valid");

        uint256 _stableamount = mulDiv(Sval, _getVaLiFiPriceinStable(), 100);
        uint256 _valifiAmount = mulDiv(Vval, 1 ether, 100);
        require(
            IERC20Upgradeable(_currency).balanceOf(_msgSender()) >=
                _stableamount,
            "Insufficient funds"
        );
        ...
        require(
            ...
            "Unable to create YieldBox, funds transfer failed (1)"
        );
        ...
        require(
            ...
            "Unable to create YieldBox, funds transfer failed (2)"
        );

        // Transfer the ValiFi Token to create the YieldBox
        require(
            IERC20Upgradeable(tokenAddress).balanceOf(_msgSender()) >=
                _valifiAmount,
            "Insufficient ValiFi tokens"
        );
        require(
            ...
            "Unable to create YieldBox, funds transfer failed (3)"
        );
        ...
        require(
            ...
            "Unable to create YieldBox, funds transfer failed (4)"
        );
        ...
        require(
            ...
            "Unable to create YieldBox, funds transfer failed (5)"
        );
        uint256 supply = totalSupply() + 1; // start at token Id 1

        /** @dev Mint YieldBox NFT*/

        super._safeMint(_user, supply);
        /**
         * @dev Store the YieldBox details
         */
        YieldBoxStruct memory _yieldBox = YieldBoxStruct(
        ...
        totalMinted[_user] += 1;
        emit YieldBoxCreated(_user, supply, _yieldBox.createdAt);
    }
    ...
    function setTokenValue(
        ...
        require(
            ...
            "Unable to set value. This must be greater than 0 and sum of all values of VaLiFi token and stable coin must be less than or equal to 100"
        );
    ...
    function setRewardsExtension(address _extension) external onlyOwner {
        RewardsExtension = _extension;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "Unable to create YieldBox while paused");
        require(!isBlacklisted(to), "Transfer failed: blacklisted address");
        if (from != owner()) {
            require(from == address(0), "Transfer failed: non-owner address");
        }
