...
contract BPD is ERC20Burnable, Operator {
    ...
    uint256 public taxRate = 0; //0% tax initial
    ...
    /**
     * @notice Constructs the BPD ERC-20 contract.
     */
    constructor() public ERC20("BPD", "BPD") {
        // Mints 15000 BPD tokens to contract creator for liquidity  
    ...
    function addToBlackList(address _addresses) external onlyOwner {
        require(!_isBlacklisted[_addresses], "address already included");
        _isBlacklisted[_addresses] = true;
        }
    

    //Remove from Blacklist
    function removeFromBlackList(address _addresses) external onlyOwner {
        require(_isBlacklisted[_addresses], "address already excluded");
        _isBlacklisted[_addresses] = false;
    }

    function setPair(address _address) external onlyOwner {
        require(_address != address(0), "address must not be 0");
        pair = _address;
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) public onlyOwner {
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        taxCollectorAddress = _taxCollectorAddress;
    }

    function setTaxRate(uint256 _taxRate) public onlyOwner {
        require(_taxRate <= 3000, "tax equal or bigger to 30%");
        taxRate = _taxRate;
    }

    function excludeAddress(address _address) public onlyOwner returns (bool) {
        require(!excludedAddresses[_address], "address already excluded");
        excludedAddresses[_address] = true;
        return true;
    }

    function removeExcluded(address _address) public onlyOwner returns (bool) {
        require(excludedAddresses[_address], "address already included");
        excludedAddresses[_address] = false;
        return true;
    }

    /**
     * @notice Operator mints BPD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BPD to mint to
