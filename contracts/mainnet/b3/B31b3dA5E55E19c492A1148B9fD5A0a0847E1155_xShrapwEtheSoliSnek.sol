...
// File contracts/xShrapwEtheSoliSnek.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract xShrapwEtheSoliSnek  is Ownable, Pausable {
    ...
    ISoliSnekRouter.Routes[] public avaxToXshrapPath;
    ISoliSnekRouter.Routes[] public avaxToEthPath;
    ...
    constructor(
        ...
        ISoliSnekRouter.Routes[] memory _avaxToXshrapPath,
        ISoliSnekRouter.Routes[] memory _avaxToEthPath
    ) {
        ...
        for (uint i; i < _avaxToXshrapPath.length; ++i) {
            avaxToXshrapPath.push(_avaxToXshrapPath[i]);
        }

        for (uint i; i < _avaxToEthPath.length; ++i) {
            avaxToEthPath.push(_avaxToEthPath[i]);
        }
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(snek).balanceOf(address(this));

        ISoliSnekRouter(router).swapExactTokensForTokens(toFee, 0, snekToAvaxPath, address(this), block.timestamp);

        uint256 feeBal = IERC20(wavax).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = feeBal * CALL_FEE / FEE_DIVISOR;
        IERC20(wavax).safeTransfer(caller, callFee);

        uint256 recipientFee = feeBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(wavax).safeTransfer(grimFeeRecipient, recipientFee);
        ...
        IERC20(wavax).safeTransfer(strategist, stratFee);

    }

    /** @dev Converts reward to both sides of the LP token and builds the liquidity pair */
    function addLiquidity() internal {
        uint256 avaxHalf = IERC20(wavax).balanceOf(address(this)) / 2;

        ISoliSnekRouter(router).swapExactTokensForTokens(avaxHalf, 0, avaxToEthPath, address(this), block.timestamp);
        ISoliSnekRouter(router).swapExactTokensForTokens(avaxHalf, 0, avaxToXshrapPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wethe).balanceOf(address(this));
        uint256 t2Bal = IERC20(xshrap).balanceOf(address(this));

        ISoliSnekRouter(router).addLiquidity(wethe, xshrap, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
