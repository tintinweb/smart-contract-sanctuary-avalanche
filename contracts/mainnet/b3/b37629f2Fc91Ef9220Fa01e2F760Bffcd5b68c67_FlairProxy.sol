...
import "../interfaces/IFlairGauge.sol";
import "../interfaces/ISolidlyGauge.sol";
import "../interfaces/IPairFactory.sol";

contract FlairProxy is OwnableUpgradeable {
    ...
    event SetCpFLDX(address oldValue, address newValue);
    event SetLPDepositor(address oldValue, address newValue);
    event SetAddresses(address _cpFLDX, address lpDepositor);
    ...
    event NewAddressesCommitted(address cpFLDX, address lpDepositor, uint256 newAddressDeadline);

    function initialize(IERC20Upgradeable _FLDX, IVoter _solidVoter, ISolidlyRouter _router) public initializer {
        __Ownable_init();
        FLDX = _FLDX;
        ...
        FLDX.safeApprove(address(ve), type(uint256).max);
    }

    modifier onlyCpFLDX() {
        require(msg.sender == cpFLDX, "Proxy: FORBIDDEN");
        _;
    }
    ...
    function setCpFLDX(address _cpFLDX) external onlyOwner {
        require(address(cpFLDX) == address(0), "Proxy: ALREADY_SET");
        emit SetCpFLDX(cpFLDX, _cpFLDX);
        cpFLDX = _cpFLDX;
    }
    ...
    function createMainLock(uint256 _amount, uint256 _lock_duration) external onlyCpFLDX returns (uint256) {
        require(mainTokenId == 0, "CpFLDXSolidStaker: ASSIGNED");
        mainTokenId = ve.createLock(_amount, _lock_duration);
        return mainTokenId;
    }

    function createVe1(uint256 _amount, uint256 _lock_duration) external onlyCpFLDX returns (uint256) {
        require(ve1TokenId == 0, "CpFLDXSolidStaker: ASSIGNED");
        ve1TokenId = ve.createLock(_amount, _lock_duration);
        return ve1TokenId;
    }

    function createVe2(uint256 _amount, uint256 _lock_duration) external onlyCpFLDX returns (uint256) {
        require(ve2TokenId == 0, "CpFLDXSolidStaker: ASSIGNED");
        ve2TokenId = ve.createLock(_amount, _lock_duration);
        return ve2TokenId;
    }

    function merge(uint256 _from, uint256 _to) external {
        require(_to == mainTokenId || _to == ve1TokenId || _to == ve2TokenId, "Proxy: TO_TOKEN_INVALID");
        require(_from != mainTokenId && _from != ve1TokenId && _from != ve2TokenId, "Proxy: NOT_MERGE_BUSINESS_TOKEN");
        require(ve.ownerOf(_from) == address(this), "Proxy: OWNER_IS_NOT_PROXY");
        ve.merge(_from, _to);
    }

    function mergeVe1() external onlyOwner {
        ve.merge(ve1TokenId, mainTokenId);
        ve1TokenId = 0;
    }

    function mergeVe2() external onlyOwner {
        ve.merge(ve2TokenId, mainTokenId);
        ve2TokenId = 0;
    }

    function getVeTokenAvailable() external view returns (uint256) {
        uint256 tokenId = 0;
        if (!ve.voted(ve1TokenId)) tokenId = ve1TokenId;
        if (!ve.voted(ve2TokenId)) {
            if (tokenId > 0) {
                (uint256 _amountVe1,) = ve.locked(ve1TokenId);
                (uint256 _amountVe2,) = ve.locked(ve2TokenId);
                if (_amountVe2 > _amountVe1) tokenId = ve2TokenId;
            } else {
                tokenId = ve2TokenId;
            }
        }
    ...
    function increaseAmount(uint256 _amount) external onlyCpFLDX {
        ve.increaseAmount(mainTokenId, _amount);
    }

    function increaseUnlockTime() external onlyCpFLDX {
        ve.increaseUnlockTime(mainTokenId, MAX_LOCK);
    }

    function resetVote(uint256 _tokenId) external onlyCpFLDX {
        solidVoter.reset(_tokenId);
    }

    function release() external onlyCpFLDX {
        uint256 before = FLDX.balanceOf(address(this));
        ve.withdraw(mainTokenId);
        uint256 amount = FLDX.balanceOf(address(this)) - before;
        if (amount > 0) FLDX.safeTransfer(cpFLDX, amount);
        mainTokenId = 0;
    }
    ...
    function pause() external onlyCpFLDX {
        FLDX.safeApprove(address(ve), 0);
    }

    function unpause() external onlyCpFLDX {
        FLDX.safeApprove(address(ve), type(uint256).max);
    }
    ...
    function deposit(address _token, uint256 _amount) external onlyLPDepositor {
        ...
        IFlairGauge(gauge).deposit(_amount, mainTokenId);
    }
    ...
    function withdraw(
        ...
        address gauge = solidVoter.gauges(_token);
        IFlairGauge(gauge).withdraw(_amount);
        IERC20Upgradeable(_token).transfer(_receiver, _amount);
    }

    function claimVeEmissions() external onlyCpFLDX returns (uint256) {
        uint256 reward = veDist.claim(mainTokenId);
        if (ve1TokenId > 0) {
            reward = reward + veDist.claim(ve1TokenId);
        }
        if (ve2TokenId > 0) {
            reward = reward + veDist.claim(ve2TokenId);
        }
    ...
    function totalDeposited(address _token) external view returns (uint) {
        address gauge = solidVoter.gauges(_token);
        return IFlairGauge(gauge).balanceOf(address(this));
    }
    ...
    function totalLiquidityOfGauge(
        ...
        address gauge = solidVoter.gauges(_token);
        return IFlairGauge(gauge).totalSupply();
    }
    ...
    function addRewardToken(
        ...
        require(_rewardToken != address(FLDX), "Proxy: ROUTE_FROM_IS_FLDX");
        require(
            _route[_route.length - 1].to == address(FLDX),
            "Proxy: ROUTE_TO_NOT_FLDX"
        );
    ...
    function getBribeReward(uint256 _tokenId, address _lp) external onlyCpFLDX {
        ...
        for (uint256 i; i < _gauges.bribeTokens.length; ++i) {
            ...
            if (tokenBal > 0) {
                if (bribeToken == address(FLDX) || bribeToken == address(cpFLDX)) {
                    IERC20Upgradeable(bribeToken).safeTransfer(cpFLDX, tokenBal);
                } else {
    ...
    function getTradingFeeReward(address _lp) external onlyCpFLDX {
        Gauges memory _gauges = gauges[_lp];
        ...
        for (uint256 i; i < _gauges.feeTokens.length; ++i) {
            ...
            if (tokenBal > 0) {
                if (feeToken == address(FLDX) || feeToken == address(cpFLDX)) {
                    IERC20Upgradeable(feeToken).safeTransfer(cpFLDX, tokenBal);
                } else {
                    router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        tokenBal,
                        0,
    ...
    function claimableReward(address _lp) external view returns (uint256) {
        ...
        for (uint256 i; i < _gauges.rewardTokens.length; ++i) {
            ...
            uint256 reward = IFlairGauge(gauge).earned(rewardToken, address(this));
            if (reward > 0) {
                if (rewardToken != address(FLDX)) {
                    uint256 rewardFLDX = router.getAmountsOut(reward, routes[rewardToken])[routes[rewardToken].length];
                    totalReward = totalReward + rewardFLDX;
                } else {
    ...
    function getReward(address _lp) external onlyLPDepositor {
        Gauges memory _gauges = gauges[_lp];
        address gauge = solidVoter.gauges(_lp);
        IFlairGauge(gauge).getReward(address(this), _gauges.rewardTokens);
        ...
        for (uint256 i; i < _gauges.rewardTokens.length; ++i) {
            ...
            if (tokenBal > 0) {
                ...
                } else FLDX.safeTransfer(lpDepositor, tokenBal);
            }
        }
    }
    ...
    */
    function setPendingAddresses(
        ...
        pendingCpFLDX = _cpFLDX;
        newAddressDeadline = block.timestamp + 86400 * 7;

        emit NewAddressesCommitted(_cpFLDX, _lpDepositor, newAddressDeadline);
    }
