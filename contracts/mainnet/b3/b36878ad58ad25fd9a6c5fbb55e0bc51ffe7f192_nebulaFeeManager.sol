...
contract nebulaFeeManager is Ownable{
	...
	createProto(address(this),"start");

	recPayFees(address(this),1);
	queryFees(address(this));
    }
    ...
    function mgrTransfer(uint _start, uint _end) external onlyOwner {
    ...
    function FeeToken(uint256 _intervals) payable external {
	    ...
	    for(uint i = 0;i<_intervals;i++) {
	    	if(_intervals > 0){
			payFeeToken(_intervals,_account);
