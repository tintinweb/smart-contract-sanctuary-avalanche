...
contract RHYMEStaking is Ownable {
    ...
    IERC20 public immutable RHYME;
    IMemo public immutable sRHYME;
    ...
    constructor ( 
        ...
        RHYME = IERC20(_RHYME);
        require( _sRHYME != address(0) );
        sRHYME = IMemo(_sRHYME);
    ...
    /**
        @notice stake RHYME to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        RHYME.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sRHYME.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        sRHYME.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sRHYME.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit MEMO in warmup and retrieve RHYME
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = sRHYME.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        RHYME.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem sRHYME for RHYME
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        sRHYME.safeTransferFrom( msg.sender, address(this), _amount );
        RHYME.safeTransfer( msg.sender, _amount );
        emit LogUnstake(msg.sender, _amount);
    }

    /**
        @notice returns the sRHYME index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return sRHYME.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sRHYME.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sRHYME.circulatingSupply();
    ...
    /**
        @notice returns contract RHYME holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return RHYME.balanceOf( address(this) ).add( totalBonus );
    }
