...
contract HoneyRoyalJelly is Ownable, ReentrancyGuard {
     ...
     */
    function updateStartAndEndTimestamp(uint256 _startTimestamp, uint256 _bonusEndTimestamp) external onlyOwner {
        require(block.timestamp < startTimestamp, "Pool has started");
        require(_startTimestamp < _bonusEndTimestamp, "New startTimestamp must be lower than new endTimestamp");
    ...
    function getWithdrawalFeeBP(address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        if (block.timestamp < user.nextWithdrawalUntil &&
