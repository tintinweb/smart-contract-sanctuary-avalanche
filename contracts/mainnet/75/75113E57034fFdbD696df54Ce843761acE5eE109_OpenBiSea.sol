...
//        _auctionContractFeeMultiplier = auctionContractFeeMultiplier;
    ...
    uint256 private initialPriceInt = 55956229793583686000;
    // 88800000000000000 - 0.0888 BNB one token, 607578947368421000 - MATIC one token, 14697931034482762 - METIS,
    // 14145132743363000 - Aurora(ETH), 55956229793583686000 - Cronos(CRO), KAVA - 7668711656441718000 (KAVA)
    function getInitialPriceInt() public view returns (uint256)  {
        return initialPriceInt;
    }

    function setInitialPriceInt(uint256 _initialPriceInt) public onlyGPartner  {
...
//    uint256 private _tokensaleTotalAmount = (10 ** uint256(18)).mul(8000);// 8000 obs
    ...
    function purchaseTokens() public payable returns (uint256) {
        require(msg.value > 1000000000000000, "TokensalePool: minimal purchase 0.001");
