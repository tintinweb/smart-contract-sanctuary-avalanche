...
// This contract utilizes IJoePair.sol, IJoeFactory.sol and JoeLibrary.sol from the Trader Joe protocol found here
// https://github.com/traderjoe-xyz/joe-core
//
// This contract will swap all AVAX at this contract address to GOTH using the Trader Joe protocol and send it the 
...
contract GothBuy is Ownable
{
    ...
    event GothBuyBack (uint256 avaxOut, uint256 gothBought);
    ...
    function swap(address _fromToken, address _toToken, uint256 _amountIn, address _to, uint256 _slippage) internal returns (uint256 amountOut) 
    {
        ...
        uint256 amountInput = IERC20(_fromToken).balanceOf(address(pair)).sub(reserve1); // calculate amount that was transferred, this accounts for transfer taxes

        amountOut = JoeLibrary.getAmountOut(amountInput, reserve1, reserve0);

        {
            ...
            require(
                JoeLibrary.getAmountOut(amountOut, reserve0, reserve1) >=
        ...
        feeToDate += _amountIn;
        gothBought += amountOut;
        emit GothBuyBack(_amountIn, amountOut);
    }
}