...
contract Brewery is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    /// @notice The early claim period
    uint256 public earlyClaimPeriod;

    /// @notice The early claim penalty
    ...
    modifier isBreweryOwner(uint256 _tokenId) {
        require(
            ownerOf(_tokenId) == _msgSender(),
            "Must be owner of this BREWERY"
        );
        _;
    }
    ...
    function _mint(address _to, string memory _name) internal {
        ...
        breweryStats[tokenId] = BreweryStats({
            ...
            enabled: true,
            xp: 0,
            productionRatePerSecondMultiplier: 1e4,
            fermentationPeriodMultiplier: 1e4,
            experienceMultiplier: 1e4,
            totalYield: 0,
    ...
    function _compound(uint256 _amount) internal {
        ...
        for (uint256 i = 0; i < _amount; ++i) {
            _mint(msg.sender, "");
            IClassManager(settings.classManager()).addReputation(msg.sender, settings.reputationForMead());
        }
        ...
        _claim(takenMead - totalCost, msg.sender);
    }
     ...
     */
    function getYield(uint256 _tokenId) external view returns(uint256) {
     ...
     */
    function getYieldFromTier(uint256 _tier) external view returns(uint256) {
        return yields[_tier];
    }
    ...
    function claimAll() public {
        ...
        for (uint256 index = 0; index < balance; index = index + 1) {
            _claimImpl(tokenOfOwnerByIndex(msg.sender, index), msg.sender);
        }
    ...
    function getBrewersTax(address brewer) public view returns (uint256) {
        return calculateSellTax(brewer);
    }

    /**
     * @notice Linearly interpolates between the brewers base tax, and a early claim penalty amount
     */
    function calculateSellTax(address brewer) public view returns (uint256) {
        uint256 class = IClassManager(settings.classManager()).getClass(brewer);
        uint256 baseTax = settings.classTaxes(class);

        uint256 timepointWhenPenaltyEnds = globalLastClaimedAt[brewer] + earlyClaimPeriod;

        // If time isn't elapsed past the last time claimed, then tax 90%
        if (block.timestamp <= globalLastClaimedAt[brewer]) {
            return earlyClaimPenalty;
        }

        // If time has elapsed past the timepoint when the penalty ends for this user, then use the base tax for the user
        if (block.timestamp >= timepointWhenPenaltyEnds) {
            return baseTax;
        }

        // Otherwise, interpolate
        // X is time  (x0 is lastTimeClaimed    x1 is timeWhenPenaltyEnds)
        // Y is tax   (y0 is earlyClaimPenalty  y1 is baseTax)
        // We are trying to find Y
        return (earlyClaimPenalty * (timepointWhenPenaltyEnds - block.timestamp) + baseTax * (block.timestamp - globalLastClaimedAt[brewer])) / (timepointWhenPenaltyEnds - globalLastClaimedAt[brewer]);
    }
    ...
    function _calculateReputation() internal {
        ...
        IClassManager(settings.classManager()).addReputation(msg.sender, (block.timestamp - fermentationEnd) * settings.reputationForClaimPerDay() / 86400);
    }

    /**
     * @notice Handles the specific claiming of MEAD and distributing it to the rewards pool and the treasury
     */
    function _claim(uint256 amount, address to) internal returns (uint256 claimedAmount) {
     ...
     */
    function _claimImpl(uint256 _tokenId, address account) internal returns (uint256 claimedAmount) {
        // require(ownerOf(_tokenId) == account, "Must be owner of this BREWERY");
        ...
        if (totalRewards > 0) {
            ...
            claimedAmount = _claim(totalRewards, account);
    ...
    function claim(uint256 _tokenId) public {
        claimTo(_tokenId, msg.sender);
        globalLastClaimedAt[msg.sender] = block.timestamp;
    }

    function claimTo(uint256 _tokenId, address account) public isBreweryOwner(_tokenId) returns (uint256 claimedAmount) {
        return _claimImpl(_tokenId, account);
    }

    function autoSell() external {
        uint256 meadAmount = balanceOf(msg.sender);
        claimAll();
        meadAmount = balanceOf(msg.sender) - meadAmount;
        ...
        if (meadAmount > 0) {
            ...
            IERC20Upgradeable(settings.mead()).approve(
                address(settings.dexRouter()),
                meadAmount
            );
            IERC20Upgradeable usdc = IERC20Upgradeable(settings.usdc());
            ...
            usdcAmount = usdc.balanceOf(address(this)) - usdcAmount;
            // Transfer USDC to treasury and user
            uint256 treasuryAmount = (usdcAmount * settings.autoSellFee()) / 1e4;
            usdcAmount = usdcAmount - treasuryAmount;

            usdc.safeTransfer(msg.sender, usdcAmount);
            usdc.safeTransfer(settings.tavernsKeep(), treasuryAmount);
        }
    }
    ...
    function setEarlyClaimPeriod(uint256 period) external onlyRole(DEFAULT_ADMIN_ROLE) {
        earlyClaimPeriod = period;
    }

    function setEarlyClaimPenalty(uint256 penalty) external onlyRole(DEFAULT_ADMIN_ROLE) {
        earlyClaimPenalty = penalty;
    }
...
contract TavernSettings is Initializable, OwnableUpgradeable {
    ...
    /// @notice The fee to apply on MEAD autosell
    uint256 public autoSellFee;
    ...
    function setAutoSellFee(uint256 _amount) external onlyOwner {
        autoSellFee = _amount;
    }
}
...
interface IClassManager {
    ...
    function addReputation(address _account, uint256 _amount) external;
}
