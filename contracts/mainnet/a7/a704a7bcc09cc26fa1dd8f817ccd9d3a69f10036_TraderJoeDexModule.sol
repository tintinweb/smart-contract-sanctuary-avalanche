...
contract TraderJoeDexModule is IDex, Ownable {
    ...
    // This interface can be used for both ERC20 and the native token. Use the wrapped native token to get a preview of a native token swap.
    ...
    function swapNative(address _in, address _out, address _to) external payable returns (uint[] memory amounts) {
        if (swapPreview(msg.value, _in, _out) > 0)
            return router.swapExactAVAXForTokens{value : msg.value}(AMOUNT_OUT_MIN, routes[_in][_out], _to, block.timestamp + DEADLINE_DELTA);
    }
    ...
    function swapNativeSlippageMin(uint256 _amountOutMin, address _in, address _out, address _to) external payable returns (uint[] memory amounts) {
        if (swapPreview(msg.value, _in, _out) > 0)
            return router.swapExactAVAXForTokens{value : msg.value}(_amountOutMin, routes[_in][_out], _to, block.timestamp + DEADLINE_DELTA);
    }

    // NOTE: This function can be used to add and update routes
    function setRoutes(address[][] memory _routes) public onlyOwner {
        for (uint8 i = 0; i < _routes.length; i++) {
            ...
            if (allowance == 0) {
                ...
                IERC20(_routes[i][0]).safeApprove(address(router), type(uint256).max - allowance);
            }
        }
    }
