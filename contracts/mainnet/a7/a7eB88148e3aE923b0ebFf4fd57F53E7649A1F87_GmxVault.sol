...
import "./BaseBuildingBlock.sol";
  ...
  */
contract GmxVault is OwnableUpgradeable, BaseBuildingBlock {
    ...
    address public glpTrackerToken;
    ...
    event BuyingEvent(address token,uint256 amount,uint256 glpAmountReceived);
    event SellingEvent(address receiver, uint256 glpAmount,address tokenOut);

    function initialize(bytes memory _data) public initializer {
        (
        IVault gmxVaultAddress,
        IRewardRouterV2 rewardRouterAddress
        ) = abi.decode(_data,(IVault,IRewardRouterV2));

        require(address(gmxVaultAddress) != address(0), "GMX BB: zero address");
        require(address(rewardRouterAddress) != address(0),"GMX BB: zero address");
        __Ownable_init();
        ...
        glpTrackerToken = rewardRouter.stakedGlpTracker();
    }
    ...
    **/
    function buyGLP(bytes memory _data) external returns(uint256 glpBoughtAmount){
        (address token, uint256 amount, uint256 minUsdg, uint256 minGlp) = abi.decode(_data,(address,uint256,uint256,uint256));
        require(token != address(0),"GMX BB: Invalid asset");
        require(amount > 0,"GMX BB: Zero amoun");
        if (token == address(0x0)) {
                require(address(this).balance >= amount,"GMX BB: Bridge or deposit native currency");
                glpBoughtAmount = rewardRouter.mintAndStakeGlpETH{value: amount}(minUsdg, minGlp);
            }else{
                require(amount > 0,"GMX BB: Zero amount");

                // check for balance to buy with
                require(IERC20(token).balanceOf(address(this)) >= amount,"GMX BB: Bridge or deposit assets.");

                // approve to void contracts is necessary
                IERC20(token).approve(address(rewardRouter), amount);
                IERC20(token).approve(address(glpManager), amount);

                // get GLP balance after buying
                uint256 glpBalanceBefore = IERC20(glpTrackerToken).balanceOf(address(this));
                // // buy Glp
                glpBoughtAmount = rewardRouter.mintAndStakeGlp(
                    token, // the token to buy GLP with
                ...
                uint256 glpBalanceAfter = IERC20(glpTrackerToken).balanceOf(address(this));

                // get glp balance change after buy
                uint256 balanceChange = glpBalanceAfter - glpBalanceBefore;

                // check if contract balance reflects the buying
                require(balanceChange >= glpBoughtAmount, "GMX BB: Glp buying failed. ");
            }
    ...
    * */
    function sellGLP(bytes memory _data) external returns (uint256 amountPayed){
        (address tokenOut, uint256 glpAmount, uint256 minOut) = abi.decode(_data,(address, uint256, uint256));
       if(tokenOut == address(0x0)){
            amountPayed = rewardRouter.unstakeAndRedeemGlpETH(glpAmount, minOut, payable(address(this)));
       }else{
            // get contract balance before selling
            uint256 tokenOutBalanceBefore = IERC20(tokenOut).balanceOf(address(this));

            // sell glp
            amountPayed = rewardRouter.unstakeAndRedeemGlp(tokenOut, glpAmount, minOut,address(this));

            // get contract balance after selling
            uint256 tokenOutBalanceAfter = IERC20(tokenOut).balanceOf(address(this));

            // get balance change
            uint256 balanceChange = tokenOutBalanceAfter - tokenOutBalanceBefore;

            // check if vault balance reflects the sale
            require(balanceChange >=amountPayed, "GMX BB: Glp buying failed. ");

            // transfer to Action pool
            require(IERC20(tokenOut).transfer(msg.sender,amountPayed),"GMX BB: Transfer to ActionPool failed");
       }
    ...
    function getTvl() public view returns(uint256){
        uint256 glpPrice = glpManager.getAum(true) / IERC20(glpToken).totalSupply();
        uint256 fsGlpAmount = IERC20(glpTrackerToken).balanceOf(address(this));
    ...
    */
    ...
    function getGlpPrice(bool _maximize) public view returns(uint256){
        return glpManager.getAum(_maximize) / IERC20(glpToken).totalSupply();
    }
    ...
    function getGLPBalance() public view returns(uint256){
        return IERC20(glpTrackerToken).balanceOf(address(this));
    }
    ...
    /**
    * @notice Get Asset under management
    */
    function getAum(bool _maximise) public view returns (uint256) {
        return glpManager.getAum(_maximise);
    }

    /**
    * @notice Get Assets under management
    */
    function getAums() public view returns (uint256[] memory) {
        return glpManager.getAums();
    }
    ...
    */
    function getAumInUsdg(bool _maximise) public view returns (uint256) {
         return glpManager.getAumInUsdg(_maximise);
    }
...
abstract contract BaseBuildingBlock is SmartLZBase {
    ...
    function lzReceive(
        ...
        (uint16 flag, bytes memory actionData) = abi.decode(
            payload,
            (uint16, bytes)
        );
        if (flag == 1) {
            ...
            // action
            (bool success, ) = address(this).call(actionData);
    ...
    function approve(
        bytes memory _data /* onlySelf() */
    ) public {
        (address baseAsset, address spender, uint256 amount) = abi.decode(
            _data,
    ...
    function backTokensToNative(
        bytes memory _data /* onlySelf() */
    ) public {
        (address _token, uint256 amount) = abi.decode(
            _data,
            (address, uint256)
        );
...
abstract contract SmartLZBase is
    OwnableUpgradeable,
    ILayerZeroReceiver,
    ILayerZeroUserApplicationConfig
{
    ...
    // todo only ActionPool
    function setNativeLZEndpoint(
        address _endpoint /*  onlySelf() */
    ) public {
    ...
    function _bridge(bytes memory _actionData) internal {
        (
            ...
            address destinationStableToken
                ...
                _actionData,
                (address, uint256, uint16, address, address)
            );

        IERC20(nativeStableToken).approve(_sgBridge, stableAmount);
        ISgBridge(_sgBridge).bridge{value: address(this).balance}(
            nativeStableToken,
            stableAmount,
            receiverLZId,
            receiverAddress,
    ...
    function forceResumeReceive(uint16 _srcChainId, bytes calldata _srcAddress)
        ...
        override
    {
        ILayerZeroEndpoint(_nativeLZEndpoint).forceResumeReceive(
            _srcChainId,
            _srcAddress
        );
    }
    ...
    function getReceiveVersion() external view returns (uint16) {
        return
            ILayerZeroEndpoint(_nativeLZEndpoint).getReceiveVersion(
                address(this)
            );
    }
    ...
    function getConfig(
        ...
        return
            ILayerZeroEndpoint(_nativeLZEndpoint).getConfig(
                ILayerZeroEndpoint(_nativeLZEndpoint).getSendVersion(
                    address(this)
                ),
                _chainId,
                address(this),
