    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    uint256 public constant rewardPerDay = 46299;
    ...
    function mintPlanet(
        ...
        require(msg.value == planetValue / mintPerAvax * 10 ** 18, "Dealers: must send correct price");
    ...
    function _createPlanetWithTokens(
        ...
        if (!mint && presale == 0) {
            ...
            // Burn the tokens used to mint the NFT
            universe.accountBurn(address(this), planetValue);
        }
    ...
    returns (uint256)
    {
        uint256 multiplier = tierSlope[0];

        for (uint256 i = 5; i > 0; i--) {
            if (prevMult >= tierLevel[i]) {
                multiplier = tierSlope[i];
                break;
            }
        }

        // Check max reached
        uint256 newMult = prevMult + multiplier;

        for (uint256 i = 5; i > 0; i--) {
            if (newMult >= tierLevel[i] && prevMult < tierLevel[i]) {
                if (currentTierMax[i] > tierMax[i] / 100 * totalSupply()) {
                    return 0;
                }
