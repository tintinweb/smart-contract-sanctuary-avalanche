...
// File contracts/staking/contracts/BSWToken.sol
...
// Biswap token for Polygon chain.
...
// File contracts/staking/contracts/MasterChef.sol
...
contract MasterChef is Ownable {
    ...
    //MCB-01 lp tokens mapping
    mapping(address => bool) public lpTokensMap;
    ...
    //MCB-02
    event SetMigrator(IMigratorChef newMigrator);
    event UpdateBswPerBlock(uint256 newBswPerBlock);
    ...
    function add( uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate ) public onlyOwner {
        //MCB-01 lp tokens mapping
        require(lpTokensMap[address(_lpToken)] == false, 'The token was added earlier');
        lpTokensMap[address(_lpToken)] = true;
    ...
    //MCB-03
    function pendingBSW(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function emergencyWithdraw(uint256 _pid) public validatePoolByPid(_pid){
        ...
        if(_pid == 0){
            depositedBsw = depositedBsw.sub(user.amount);
        }
        user.amount = 0;
        user.rewardDebt = 0;
    }
    ...
    function updateBswPerBlock(uint256 newAmount) public onlyOwner {
        ...
        //MCB-02
        emit UpdateBswPerBlock(newAmount);
    }

    function setStakingPercent(uint256 _stakingPercent) public onlyOwner{
        require(_stakingPercent >= 1 && _stakingPercent <= 1000000, 'Wrong staking percent value');
        stakingPercent = _stakingPercent;
    }

    function setDevPercent(uint256 _devPercent) public onlyOwner{
        require(_devPercent >= 1 && _devPercent <= 1000000, 'Wrong staking percent value');
        devPercent = _devPercent;
    }

    function setRefPercent(uint256 _refPercent) public onlyOwner{
        require(_refPercent >= 1 && _refPercent <= 1000000, 'Wrong staking percent value');
        refPercent = _refPercent;
    }

    function setSafuPercent(uint256 _safuPercent) public onlyOwner{
        require(_safuPercent >= 1 && _safuPercent <= 1000000, 'Wrong staking percent value');
        safuPercent = _safuPercent;
    }
