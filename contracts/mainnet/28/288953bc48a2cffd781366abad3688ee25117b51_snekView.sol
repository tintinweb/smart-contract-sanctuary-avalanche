...
import "./interfaces/SoliSnek/IMinter.sol";
    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice returns snek address
     */
    function snekAddress() public view returns (address ram) {
        ram = ve.token();
    }
    ...
    /**
     * @notice returns SoliSnek core contract addresses
     */
    ...
    /**
     * @notice returns all SoliSnek pool addresses
     */
    ...
    /**
     * @notice returns all SoliSnek pools that have active gauges
     */
    ...
    /**
     * @notice returns current fee rate of a SoliSnek pool
     * @param pool pool address to check
     */
    ...
    /**
     * @notice returns useful information for all SoliSnek pools
     */
    ...
    function stakingPositionOf(
        ...
        userGaugeTokenData[] memory _userRewards = new userGaugeTokenData[](
            len
        );
    ...
    function bribesPositionsOf(
        address user,
        address[] memory pools
    ) public view returns (userBribeData[] memory rewardsData) {
    ...
    /**
     * @notice returns gauge reward data for a SoliSnek pool
    ...
    function poolsPositionsOf(
    ...
    ) public view returns (PositionPool[] memory _poolsPositionsOf) {
        uint len = allActivePools().length;
        _poolsPositionsOf = new PositionPool[](len);

        uint i;
        for (uint256 poolIndex; poolIndex < len; ++poolIndex) {
            address poolAddress = voter.pools(poolIndex);
            uint256 balanceOf = IPair(poolAddress).balanceOf(accountAddress);
            if (balanceOf > 0) {
                _poolsPositionsOf[i].id = poolAddress;
                _poolsPositionsOf[i].balanceOf = balanceOf;
                ++i;
            }
        }
        ...
        assembly {
            mstore(add(encodedPositions, 0x40), i)
        }
    ...
    function userInfo(
        address user
    ) public view returns (userData memory _userInfo) {
