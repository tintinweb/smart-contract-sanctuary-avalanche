...
contract ActionPoolDcRouter is NonblockingLzApp, IActionPoolDcRouter {
    ...
    */
    function processDeposits(
        ...
        if (_receiverId == _nativeChainId) {
            (bool success, bytes memory response) = _bytesToAddress(
                _receiverAddress
    ...
    function initNewBB(
        ...
        if (_fabricID == _nativeChainId) {
            ...
            emit BBInited(_strategyId, _implementation, _fabricID);
        } else {
            bytes memory lzEndpointParams = abi.encodePacked(
                uint16(2), // lzSend version
                _gasForDstLzReceive,
    ...
    function _performAction(
        ...
        if (_receiverId == _nativeChainId) {
            (bool success, bytes memory response) = _bytesToAddress(
                _receiverAddress
            ).call{value: msg.value}(_payloadToEndpoint);
            if (!success) {
                revert(_getRevertMsg(response));
            }
