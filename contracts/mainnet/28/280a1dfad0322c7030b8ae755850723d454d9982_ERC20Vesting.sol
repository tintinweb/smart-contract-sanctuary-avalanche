...
contract ERC20Vesting is IERC20, IERC20Vesting, ReentrancyGuard{
	...
	*/
	...
	function revoke(address beneficiary, uint256 ID) external nonReentrant override returns(uint256){
		...
		// drop pending tokens
		uint256 toBeDropped = vesting.amount - _granted[vesting.ID];
		_onPreTokenTransfer(vesting.grantor, toBeDropped);


		// set status
		vesting.status = Status.REVOKED;

		emit VestingRevoked(vesting.beneficiary, vesting.ID, tobeTransfered);
	...
	function abandon(uint256 ID) external nonReentrant override returns(uint256){
		...
		// emit event
		emit VestingAbandoned(vesting.beneficiary, vesting.ID, tobeTransfered);
	...
	*/
	 ...
	 */
	function decimals() external view returns (uint8) {
		return IERC20Specs(_token).decimals();
	}
	...
	function totalSupply() external view override returns (uint256){
		return IERC20Specs(_token).totalSupply();
	}
	...
	function balanceOf(address account) external view override returns (uint256){
		...
		uint256 balance = 0;
		for (i = 0; i < ids.length; i++) {
			if(_vesting[account][ids[i]].status == Status.ACTIVE){
