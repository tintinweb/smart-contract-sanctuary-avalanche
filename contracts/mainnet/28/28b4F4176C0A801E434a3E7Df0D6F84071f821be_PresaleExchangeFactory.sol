...
// File: contracts/ASTRO/PresaleExchangeFactory.sol
...
contract PresaleExchangeFactory is Ownable {
    IAstroToken _pAstroAddress;
    IERC20 _usdcAddress;
    ...
    constructor(address _pAstro) {
        _pAstroAddress = IAstroToken(_pAstro);
    }

    function airdrop(TokenInfo[] memory _infos) external onlyOwner {
        for (uint256 i = 0; i < _infos.length; i ++) {
            // transfer pASTRO token to user
            _pAstroAddress.transfer(_infos[i].holder, _infos[i].amount);
        }
    }
