...
import {ILydiaFactory} from "./interfaces/ILydiaFactory.sol";
 ...
 * @notice Main contract to interact with to swap and manage liquidity on Lydia V2 exchange.
 */
contract LBRouter is ILBRouter {
    using TokenHelper for IERC20;
    using TokenHelper for IWNATIVE;
    using LydiaLibrary for uint256;
    using PackedUint128Math for bytes32;

    ILBFactory private immutable _factory;
    ILydiaFactory private immutable _factoryV1;
    ...
    /**
     * @notice Constructor
     * @param factory Address of Lydia V2.1 factory
     * @param factoryV1 Address of Lydia V1 factory
     * @param wnative Address of WNATIVE
     */
    constructor(ILBFactory factory, ILydiaFactory factoryV1, IWNATIVE wnative) {
        _factory = factory;
     ...
     */
    function getV1Factory() external view override returns (ILydiaFactory factoryV1) {
        return _factoryV1;
    }
        ...
        )
    {
        ILBPair lbPair = ILBPair(
            _getLBPairInformation(liquidityParameters.tokenX, liquidityParameters.tokenY, liquidityParameters.binStep)
        );
    ...
    function removeLiquidityNATIVE(
        ...
        ILBPair lbPair = ILBPair(_getLBPairInformation(token, IERC20(wnative), binStep));
     ...
     * @param versions The list of versions (V1 or V2_1)
    ...
    function _getAmountsIn(
        ...
        for (uint256 i = pairs.length; i != 0; i--) {
            ...
            if (version == Version.V1) {
                (uint256 reserveIn, uint256 reserveOut,) = ILydiaPair(pair).getReserves();
    ...
    function _swapExactTokensForTokens(
        ...
        unchecked {
            for (uint256 i; i < pairs.length; ++i) {
                ...
                if (version == Version.V1) {
                    (uint256 reserve0, uint256 reserve1,) = ILydiaPair(pair).getReserves();

                    if (token < tokenNext) {
                        amountOut = amountOut.getAmountOut(reserve0, reserve1);
                        ILydiaPair(pair).swap(0, amountOut, recipient, "");
                    } else {
                        amountOut = amountOut.getAmountOut(reserve1, reserve0);
                        ILydiaPair(pair).swap(amountOut, 0, recipient, "");
                    }
                } else {
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        unchecked {
            for (uint256 i; i < pairs.length; ++i) {
                ...
                if (version == Version.V1) {
                    (uint256 _reserve0, uint256 _reserve1,) = ILydiaPair(pair).getReserves();
                    if (token < tokenNext) {
    ...
    /**
     * @notice Helper function to return the address of the pair (v1 or v2_1, according to `binStep`)
     * @dev Revert if the pair is not created yet
     * @param tokenX The address of the tokenX
     * @param tokenY The address of the tokenY
     * @param binStep The bin step of the LBPair
...
/**
 * @title Liquidity Book Lydia Library Helper Library
 * @author Lydia Finance
 * @notice Helper contract used for Lydia V1 related calculations
 */
library LydiaLibrary {
    error LydiaLibrary__AddressZero();
    error LydiaLibrary__IdenticalAddresses();
    error LydiaLibrary__InsufficientAmount();
    error LydiaLibrary__InsufficientLiquidity();

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        if (tokenA == tokenB) revert LydiaLibrary__IdenticalAddresses();
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        if (token0 == address(0)) revert LydiaLibrary__AddressZero();
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        if (amountA == 0) revert LydiaLibrary__InsufficientAmount();
        if (reserveA == 0 || reserveB == 0) revert LydiaLibrary__InsufficientLiquidity();
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut)
        ...
        returns (uint256 amountOut)
    {
        if (amountIn == 0) revert LydiaLibrary__InsufficientAmount();
        if (reserveIn == 0 || reserveOut == 0) revert LydiaLibrary__InsufficientLiquidity();
    ...
    {
        if (amountOut == 0) revert LydiaLibrary__InsufficientAmount();
        if (reserveIn == 0 || reserveOut == 0) revert LydiaLibrary__InsufficientLiquidity();
