...
contract CpFLDXConfigurator is OwnableUpgradeable {
    ...
    uint256 public veMintRatio;
    bool public isAutoIncreaseLock;

    uint256 public maxPegMintFLDX;
    uint256 public maxPegMintVeFLDX;
    ...
    event SetVeMintRatio(uint256 oldValue, uint256 newValue);
    event SetMaxPegMintFLDX(uint256 oldValue, uint256 newValue);
    event SetMaxPegMintVeFLDX(uint256 oldValue, uint256 newValue);
    ...
    function initialize(address _coFeeConfig, address _coFeeRecipient) public initializer {
        ...
        maxPegMintFLDX = 1e18;
        maxPegMintVeFLDX = 1e18;

        excluded.push(deadWallet);
    }

    function adjustVeMintRatio(uint256 _ratio) external onlyOwner {
        // validation from 1.0 -> 1.5 veFldx to 1.0 cpFLDX
        require(_ratio >= 1e18 && _ratio <= 1.5e18, "CpFLDXConfigurator: OUT_OF_RANGE");
        emit SetVeMintRatio(veMintRatio, _ratio);
        veMintRatio = _ratio;
    }
    ...
    function setMaxPegMintFLDX(uint256 _value) external onlyOwner {
        // validation from 0-1
        require(_value <= MAX_RATE, "CpFLDXConfigurator: VALUE_OUT_OF_RANGE");
        emit SetMaxPegMintFLDX(maxPegMintFLDX, _value);
        maxPegMintFLDX = _value;
    }

    function setMaxPegMintVeFLDX(uint256 _value) external onlyOwner {
        // validation from 0-1
        require(_value <= MAX_RATE, "CpFLDXConfigurator: VALUE_OUT_OF_RANGE");
        emit SetMaxPegMintVeFLDX(maxPegMintVeFLDX, _value);
        maxPegMintVeFLDX = _value;
    }
