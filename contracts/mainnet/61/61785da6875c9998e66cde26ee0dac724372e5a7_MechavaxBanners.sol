...
Mechavax Banners - Arrogant   
*/


contract MechavaxBanners is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter public _tokenIds;
    IERC721[] public nft; 
    IERC721Enumerable[] private enumerableNFt;
    uint256 public _price = 0 ether; //avax
    uint256 public _price_Public = 0 ether; //avax
    uint256 public _maxMintable = 251;
    ...
    mapping(uint256 => bool) private listOfUsedTokenIDs;

    constructor(
        string memory customBaseURI,
        address _walletFundsA,
        address royaltyAddr
    ) ERC721("Mechavax Banners", "BNRS") {
    ...
    function setNFT(address[] memory _addressList) external onlyOwner {
        ...
        for (uint256 i = 0; i < _addressList.length; i++) {
            ...
            enumerableNFt.push(IERC721Enumerable(_addressList[i]));
        }
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        for (uint256 i = 0; i < nft.length; i++) {
            uint256 userBalance = nft[i].balanceOf(msg.sender);
            for(uint256 j=0; j < userBalance ; j++) {
                uint256 tokenID = enumerableNFt[i].tokenOfOwnerByIndex(msg.sender, j);
                if (!listOfUsedTokenIDs[tokenID]) { 
                    listOfUsedTokenIDs[tokenID] = true;
    ...
    function splitFunds(uint256 mintValue) private {
        uint256 amountA = (mintValue * 100) / 100;
        payable(walletFundsA).transfer(amountA);
      
    }

    function setNFTsTransferWallets(address _walletFundsA)
        external
        onlyOwner
    {
        walletFundsA = _walletFundsA;
