...
contract YellowSnowballs is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using Counters for Counters.Counter;

    string public baseURI = "ipfs://bafybeianesxqc7p34cgjsonzelyz5rugb555etdtdtv7uxugrivntou3zi/";
    string public baseExtension = ".json";

    uint256 public cost = .05 ether;
    uint256 public maxMintable = 5000;
    uint256 public minted = 0;

    address private proceedsWallet;

    mapping(uint256 => uint8) tokenUnlocked;
    mapping(address => uint8) addressClaimed;

    bool public _mintActive = false;
    bool public _transActive = false;

    Counters.Counter private supply;

    constructor() ERC721("Yellow Snowballs", "YELLOWSNOWBALLS") {
        proceedsWallet = 0xC92bf61da670bD8A436e8418e03c75c03DB3FA95;
    }
    ...
    function mint(address _to, uint256 _amount) public payable nonReentrant {
        ...
        (bool success, ) = payable(proceedsWallet).call{value: msg.value}("");
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0
                ? string(
                    abi.encodePacked(baseURI, token.toString(), baseExtension)
                )
                : "";
    }
	...
	uint256 tokenId
        ...
        if (from != address(0)) {
            require(_transActive, "Transfer is not active.");
        }
