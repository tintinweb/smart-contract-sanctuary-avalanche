...
contract PaintHuffingDegenerates2 is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer {
    ...
    // CONSTANTS
    uint public constant MAX_SUPPLY = 1110;
    ...
    uint public maxPerPerson = 1110;
    uint public price = 450000000000000000;
    uint public whiteListPrice = 400000000000000000;
    address public royaltyAddress = 0xEE6A8eF30A2E3AdBdd3F0b884ed44812db260970;
    ...
    constructor(
        ...
        uint[] memory _fees,
        address _lzEndpoint
    ) ERC721("Paint Huffing Degenerates 2", "PHDT") ClampedRandomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        if (whitelistedOnly) {
            if (price > 0) require(msg.value == whiteListPrice * amount, "Error: invalid price");
            require(isWhitelisted[msg.sender] >= amount, "Error: you are not whitelisted or amount is higher than limit");
        } else if (price > 0) require(msg.value == price * amount, "Error: invalid price");

        for (uint i = 0; i < amount; ++i) {
            internalMint(msg.sender);
        }

        if (whitelistedOnly) isWhitelisted[msg.sender] -= amount;
    ...
    function airdropsToken(address[] memory _addr, uint amount) public onlyOwner {
        for (uint i = 0; i < _addr.length; i++) {
            airdropTokenInternal(amount, _addr[i]);
        }
    }

    function airdropTokenInternal(uint amount, address _addr) internal {
        for (uint i = 0; i < amount; i++) {
            internalMint(_addr);
        }
    }
