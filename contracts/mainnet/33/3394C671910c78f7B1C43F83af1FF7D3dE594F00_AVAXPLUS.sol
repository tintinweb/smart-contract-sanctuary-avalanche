...
contract AVAXPLUS {
	...
	event NewDeposit(address indexed user, uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish);
    ...
    function invest(address payable referrer,uint256 _days) public payable {
        _invest(referrer, msg.sender, msg.value, _days);
           
    }


	function _invest(address payable referrer, address payable sender, uint256 value, uint256 _days) private {
		...
		(uint256 percent, uint256 profit, uint256 finish) = getResult(value, _days);
		
		user.deposits.push(Deposit( percent, value, profit, block.timestamp, finish));

		totalStaked = totalStaked.add(value);
        totalDeposits = totalDeposits.add(1);
		
		emit NewDeposit(sender, percent, value, profit, block.timestamp, finish);
	}
	...
	function getPercent(uint256 _days) public view returns (uint256) {
	...
	function getResult(uint256 deposit,uint256 _days) public view returns (uint256 percent, uint256 profit, uint256 finish) {
		percent = getPercent(_days);
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns( uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
