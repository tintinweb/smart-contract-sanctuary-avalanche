...
interface hoppersNFT is IERC721Enumerable, IERC721Receiver{
...
interface hoppersZone is IERC721Enumerable, IERC721Receiver{
...
contract contractHoppers is Ownable, IERC721Receiver{
    ...
    hoppersNFT h = hoppersNFT(0x4245a1bD84eB5f3EBc115c2Edf57E50667F98b0b);
    ...
    function levelUpHoppersFromZone(uint256 tokenId, uint64 nbLvl, address zone, bool useOwnRewards) onlyOwner external {
        hoppersZone z = hoppersZone(zone);
        for (uint i=0; i<nbLvl; i++){
            z.levelUp(tokenId, useOwnRewards);
        }
    }

    function levelUpMultiHoppersFromZone(uint256 [] memory tokenId, uint64 nbLvl, address zone, bool useOwnRewards) onlyOwner external {
        hoppersZone z = hoppersZone(zone);
