...
contract ClaimAll is Ownable, ReentrancyGuard {
    ...
    constructor (address _reward_token, address[] memory _pools, address[] memory _farms){
        month_reward_token = _reward_token;
        pools = _pools;
        farms = _farms;

        setAllMonth(block.timestamp);
    }

    function setPools(address[] memory _pools) external onlyOwner {
        pools = _pools;
    }
    function setFarms(address[] memory _farms) external onlyOwner {
        farms = _farms;
    }
    ...
    function setRewardToken(address _rewardToken) public onlyOwner{
        month_reward_token = _rewardToken;
    }
