...
contract NebulaProtoStarDrop is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury){
		...
		(uint256 _intervals,uint256 total,uint256 returnBalance) = checkSend(owner(),tokFee,One,tokFee);//owner(),tokFee,One,tokFee//address _account,uint256 _sent,uint256 _intervals,uint256 _fee//_intervals,_sent,_fee//
	    	checkFeeTokenBalanceOfUser(owner(),total);
	}
	function payFeeToken(uint256 _intervals) payable external{
	    address _account = msg.sender;
	    (uint256 _intervals,uint256 total,uint256 returnBalance) = checkSend(_account,tokFee,_intervals,tokFee);
	...
	function checkFeeTokenBalanceOfUser(address _account,uint256 _total) internal{
		...
		require(tokFee < queryFeeTokBalance(_account),"you have n");
