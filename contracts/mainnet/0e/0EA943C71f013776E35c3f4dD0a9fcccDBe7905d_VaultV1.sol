...
contract VaultV1 is IERC4626, Pausable, ReentrancyGuard, AccessControl {
    ...
    function redeem(
        ...
        if (shares == 0) shares = _shares[msg.sender];
        return _redeem(shares, receiver, msg.sender);
    }

    function redeem(uint256 shares, address receiver) external returns (uint256) {
        return redeem(shares, receiver, address(0));
    }
    ...
    function revokeRole(
        ...
        require(
            _shares[account] == 0,
            "VaultV1: Account with remaining shares cannot be dewhitelisted"
        );
        _revokeRole(role, account);
    }
    ...
    function _vaultBalance() internal view returns (uint256 totalSize) {
        ...
        for (uint256 i = 0; i < withdrawalQueue.length; i++) {
            try IStrategyConfig(withdrawalQueue[i]).strategyBalance() returns (
                uint256 strategyBalance
            ) {
                totalSize += strategyBalance;
            } catch {}
        }
    }
}