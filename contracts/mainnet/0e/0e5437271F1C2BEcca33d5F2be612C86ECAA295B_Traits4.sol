...
contract Traits4 is Ownable, ITraits {
    ...
    constructor(ITraits _old) public {
        old = _old;
    }
   ...
   */
    ...
    function moveData(uint8 id, uint8 startFrom) public onlyOwner {
        uint8 till = old.traitCountForType(id);

        for(uint8 i = startFrom; i < till; i++) {
            Trait memory _t;

            (_t.name, _t.png) = old.traitData(id, i);
            traitData[id][i] = _t;
        }
    }
}
