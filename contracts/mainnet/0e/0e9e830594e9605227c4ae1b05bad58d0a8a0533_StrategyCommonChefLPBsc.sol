...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    function addLiquidity() internal {
        ...
        address[] memory sicklePath = new address[](2);
        sicklePath[0] = outputToLp1Route[3];
        sicklePath[1] = outputToLp1Route[4];
        uint256 halfYUSDbal;

        if (lpToken0 != output) {
            //Convert all to YUSD, split after, then create liquidity
            IUniswapRouterETH(unirouter2).swapExactTokensForTokens(outputFull, 0, outputToLp0Route, address(this), now);
            halfYUSDbal = IERC20(sicklePath[0]).balanceOf(address(this)).div(2);
        }

        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(halfYUSDbal, 0, sicklePath, address(this), now);
        }
