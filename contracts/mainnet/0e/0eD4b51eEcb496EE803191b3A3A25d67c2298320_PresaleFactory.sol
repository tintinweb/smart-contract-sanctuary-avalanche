...
// File: contracts/MAGIC/interface/IJoeRouter02.sol
...
// File: contracts/MAGIC/interface/IMagicToken.sol
...
// File: contracts/MAGIC/PresaleFactory.sol
...
contract PresaleFactory is Ownable {
    IMagicToken _pMagicAddress;
    IERC20 _usdcAddress;
    IJoeRouter02 public _joe02Router;

    // min/max cap
    uint256 public minCapUSDC                                       = 200 * 10 ** 6;
    uint256 public maxCapUSDC                                       = 3000 * 10 ** 6;
    uint256 public pTokenPrice_USDC                                 = 3 * 10 ** 4;
    ...
    address payable presaleOwnerAddress                             = payable(0x62878e0FE7C4e1c37EE10F6C9D8cf625FF013311);

    mapping (address => uint256) private _userPaidUSDC;

    constructor(address _router, address _pMagic, address _usdc) {
        _joe02Router = IJoeRouter02(_router);
        _pMagicAddress = IMagicToken(_pMagic);
    ...
    function buyTokensByUSDC(uint256 _amountPrice) external {
        ...
        // transfer pMAGIC token to user
        _pMagicAddress.transfer(msg.sender, tokenAmount);
    ...
    function withdrawAll() external onlyOwner{
        uint256 balance = _pMagicAddress.balanceOf(address(this));
        if(balance > 0) {
            _pMagicAddress.transfer(msg.sender, balance);
        }
    ...
    event WithdrawAll(address addr, uint256 magic);
