...
contract NebulaProtoStarDrop is Ownable{
	...
	function transferOldDrops(address _account,uint _x,uint _y) external managerOnly(){
		ProtoManager prev = ProtoManager(_account);
		uint256 length = prev.getProtoAccountsLength();
		if(_y>length){
		_y = length;
		}
		for(uint i=_x;i<_y;i++){
			address _account = prev.getProtoAddress(uint256(i));
			if(nebuLib.addressInList(transfered,_account) == false){
				DROPS storage drop = airdrop[_account];
    				drop.claimed += prev.getProtoStarsLength(prev.getProtoAddress(uint256(i)));
    				drop.dropped += prev.getProtoStarsLength(prev.getProtoAddress(uint256(i)));
