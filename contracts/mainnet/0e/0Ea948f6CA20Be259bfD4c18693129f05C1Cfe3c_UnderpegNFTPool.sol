...
contract UnderpegNFTPool is Controllable, IERC721Receiver, Pausable {
    ...
    uint256 public predatorPremiumBase = 1.5 ether;
    ...
    modifier checkGeneration(uint256 tokenId) {
        require(
            IPreyPredator(preyPredator).getTokenTraits(tokenId).generation > 0,
            "not underpeg nft"
        );
        _;
    }
    ...
    function stake(uint256 tokenId)
        external
        whenNotPaused
        checkGeneration(tokenId)
    {
        IERC721Metadata(preyPredator).tokenURI(tokenId);
        ...
        uint256 mintPrice = IPreyPredator(preyPredator).mintedPrice(tokenId);
        uint256 premium = _calcPremium(tokenId);
    ...
    function unstake(uint256 tokenId) external whenNotPaused {
        ...
        require(epoch > _stake.stakedAt + lockPeriod, "unstake: nft locked");
    ...
    function _calcPremium(uint256 tokenId) internal view returns (uint256) {
        IPreyPredator.PreyPredator memory t = IPreyPredator(preyPredator)
            .getTokenTraits(tokenId);
        if (t.isPrey) {
            return
                (IPreyPredator(preyPredator).mintedPrice(tokenId) *
                    preyPremiumMultiplier) / 100;
        } else {
            return predatorPremiumBase + 0.2 ether * t.alphaIndex;
        }
    }
    ...
    function setPredatorPremiumBase(uint256 _predatorPremiumBase)
        external
        onlyOwner
    {
        predatorPremiumBase = _predatorPremiumBase;
    }
