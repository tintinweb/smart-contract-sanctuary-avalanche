...
import "./interface/gmx/IGMXOrderBook.sol";
...
contract GMXPositionManager is OwnableUpgradeable, UUPSUpgradeable {
    /// @notice The token that can be deposited or withdrawn in the vault.
    address public token;
    /// @notice The AlphaVault address.
    address public vault;
    /// @notice The manager can manage the stop loss and take profits.
    ...
    /// @notice The order book of GMX to store take profits/stop losses.
    address public gmxOrderBook;
    /// @notice Chainlink oracle address for {token}/USD
    mapping(address => address) public oracles;

    /// @notice The slippage we tolerate.
    ...
    /// @notice Initializes a new GMXPositionManager.
    /// @param _token The token that can be deposited or withdrawn in the vault.
    /// @param _vault The address of the AlphaVault.
    /// @param _manager The address that can set stop losses or take profits.
    function initialize(address _token, address _vault, address _manager) external initializer {
        __Ownable_init();
        ...
        gmxVault = address(0x9ab2De34A33fB459b538c43f251eB825645e8595);
        gmxRouter = address(0x5F719c2F1095F7B9fc68a68e35B51194f4b6abe8);
        gmxPositionRouter = address(0xffF6D276Bc37c61A23f06410Dce4A400f66420f8);
        gmxOrderBook = address(0x4296e307f108B2f583FF2F7B7270ee7831574Ae5);
    ...
    function openPosition(address indexToken, uint256 tokenAmount, bool isLong) external payable onlyVault {
        require(msg.value == IGMXPositionRouter(gmxPositionRouter).minExecutionFee(), "Wrong value");
        require(tokenAmount > 10 * 1e6, "Min amount not met");

        IERC20Upgradeable(token).transferFrom(vault, address(this), tokenAmount);
        IERC20Upgradeable(token).approve(gmxRouter, tokenAmount);
        ...
        if (isLong) {
            ...
            path[0] = token;
            path[1] = indexToken;
            acceptablePrice = uint256(price) * (MAX_BPS + slippage) / (MAX_BPS) * 1e22; // GMX uses 30 bps tolerance

            IGMXPositionRouter(gmxPositionRouter).createIncreasePosition{value: msg.value}(path, indexToken, tokenAmount, 0, positionSize, isLong, acceptablePrice, 20000000000000000, 0x0, address(0));
        } else {
    ...
    /// @param indexToken The address of the token that is longed or shorted.
    /// @param tokenAmount The amount of collateral of {token} to close.
    /// @param isLong If we long or if we short.
    function closePosition(address indexToken, uint256 tokenAmount, bool isLong) external payable onlyVault {
        require(msg.value == IGMXPositionRouter(gmxPositionRouter).minExecutionFee(), "Wrong value");
        ...
        if (isLong) {
            ...
            IGMXPositionRouter(gmxPositionRouter).createDecreasePosition{value: msg.value}(path, indexToken, 0, tokenAmount, isLong, vault, acceptablePrice, 0, 20000000000000000, false, address(0));
        } else {
            address[] memory path = new address[](1);
            path[0] = token;
            acceptablePrice = uint256(price) * (MAX_BPS + slippage) / MAX_BPS * 1e22; // GMX uses 30 bps tolerance
    ...
    function addConditionalOrder(address _indexToken, uint256 _sizeDelta, bool _isLong, uint256 _triggerPrice) external payable onlyManagerOwner {
        require(msg.value > IGMXOrderBook(gmxOrderBook).minExecutionFee(), "Wrong value");

        if (_isLong) {
            IGMXOrderBook(gmxOrderBook).createDecreaseOrder{value: msg.value}(_indexToken, _sizeDelta, _indexToken, 0, _isLong, _triggerPrice, false);
        } else {
            IGMXOrderBook(gmxOrderBook).createDecreaseOrder{value: msg.value}(_indexToken, _sizeDelta, token, 0, _isLong, _triggerPrice, false);
        }
    }

    function removeConditionalOrder(uint256 _orderIndex) external onlyManagerOwner {
        IGMXOrderBook(gmxOrderBook).cancelDecreaseOrder(_orderIndex);
    }

    /// @notice Returns the amount of {token} currently in long/short positions.
    function positionsSize() external view returns (uint256) {
        uint256 size = 0;
        for (uint8 i = 0; i < IGMXVault(gmxVault).allWhitelistedTokensLength(); i++) {
            address tradingToken = IGMXVault(gmxVault).allWhitelistedTokens(i);
            (,uint256 longCollateral,,,,,,) = IGMXVault(gmxVault).getPosition(address(this), tradingToken, tradingToken, true);
            (,uint256 shortCollateral,,,,,,) = IGMXVault(gmxVault).getPosition(address(this), token, tradingToken, false);
            size += longCollateral;
            size += shortCollateral;
        }

        return size;
    }

    /// @notice Helper function to recover tokens on this contract, for example in case of GMX revert. They will be sent back to the vault.
    function withdrawToken() external onlyVault returns (uint256) {
        uint256 balance = IERC20Upgradeable(token).balanceOf(address(this));
        IERC20Upgradeable(token).transfer(vault, balance);
        return balance;
    }
    ...
    function rescue(address _rescueToken, address _recipient) external onlyOwner {
        ...
        IERC20Upgradeable(_rescueToken).transfer(_recipient, IERC20Upgradeable(_rescueToken).balanceOf(address(this)));
    }
    ...
    /// @notice Sets new addresses and parameters for GMX.
    /// @param _gmxVault The address of the GMX vault.
    /// @param _gmxRouter The address of the router to approve plugins.
    /// @param _gmxPositionRouter The address of the router to interact with positions.
    /// @param _slippage The tolerated slippage when opening or closing a position (in MAX_BPS).
    function setGMXParameters(address _gmxVault, address _gmxRouter, address _gmxPositionRouter, uint256 _slippage) external onlyOwner {
        gmxVault = _gmxVault;
    ...
    /// @param _token The address of the token monitored by the oracle.
...
interface IGMXOrderBook {
    ...
    function createDecreaseOrder(address _indexToken, uint256 _sizeDelta, address _collateralToken, uint256 _collateralDelta, bool _isLong, uint256 _triggerPrice, bool _triggerAboveThreshold) external payable;
...
interface IGMXVault {
    ...
    function allWhitelistedTokens(uint256 input) external view returns (address);
