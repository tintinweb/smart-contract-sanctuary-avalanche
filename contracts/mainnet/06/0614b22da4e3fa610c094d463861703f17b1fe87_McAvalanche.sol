...
McAvalache - $McAVAX - Did you want AVAX with that? FK yeah you do - and get your moon on - LFG.
...
TG: https://t.me/McAvalanchePortal
...
Sell Tax Will Be Higher During Launch.

Marketing: 4%
Rewards: 4%
Liquidity: 4%

DO NOT BUY BEFORE THE CONTRACT IS PASTED IN THE TELEGRAM GROUP OR YOU WILL BE BLACKLISTED FROM TRADING
...
contract McAvalanche is IERC20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'McAvalache';
    string private constant _symbol = '$McAVAX';
    ...
    //DevWallets
    address public Development=payable(0x7d7290f436D7C46FDe599b1efEf19aD057580b33);
    ...
    address private _pangolinPairAddress; 
    IPangolinRouter02 private  _pangolinRouter;
    ...
    constructor () {
        ...
        // Pangolin Router
        _pangolinRouter = IPangolinRouter02(PangolinRouter);
        //Creates a Pangolin Pair
        _pangolinPairAddress = IPangolinFactory(_pangolinRouter.factory()).createPair(address(this), _pangolinRouter.WAVAX());
        ...
        //excludes Pangolin Router, pair, contract and burn address from staking
        _excludedFromStaking.add(address(_pangolinRouter));
        _excludedFromStaking.add(_pangolinPairAddress);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        //transfers between PangolinRouter and PangolinPair are tax and lock free
        address pangolinRouter=address(_pangolinRouter);
        bool isLiquidityTransfer = ((sender == _pangolinPairAddress && recipient == pangolinRouter) 
        || (recipient == _pangolinPairAddress && sender == pangolinRouter));

        //differentiate between buy/sell/transfer to apply different taxes/restrictions
        bool isBuy=sender==_pangolinPairAddress|| sender == pangolinRouter;
        bool isSell=recipient==_pangolinPairAddress|| recipient == pangolinRouter;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            //withdraws AVAX when sending less or equal to 1 Token
        ...
        //Swapping AutoLP and MarketingAVAX is only possible if sender is not pangolin pair, 
        ...
        if((sender!=_pangolinPairAddress)&&(!manualConversion)&&(!_isSwappingContractModifier)&&isSell)
            _swapContractToken();
    ...
    function _calculateFee(uint256 amount, uint8 tax, uint8 taxPercent) private pure returns (uint256) {
     ...
     //AVAX Autostake/////////////////////////////////////////////////////////////////////////////////////////
       //Autostake uses the balances of each holder to redistribute auto generated AVAX.
    //Each transaction _addToken and _removeToken gets called for the transaction amount
    //WithdrawAVAX can be used for any holder to withdraw AVAX at any time, like true Staking,
    ...
    //Contract, pangolin and burnAddress are excluded, other addresses like CEX
    ...
    //adds Token to balances, adds new AVAX to the toBePaid mapping and resets staking
    ...
    function _distributeStake(uint256 AVAXamount) private {
        // Deduct marketing Tax
        uint256 marketingSplit = (AVAXamount * marketingShare) / 100;
        uint256 amount = AVAXamount - marketingSplit;
    ...
    function claimAVAX(address addr) private{
        ...
        uint256 amount;
        if(isExcludedFromStaking(addr)){
            //if excluded just withdraw remaining toBePaid AVAX
        ...
        path[0] = _pangolinRouter.WAVAX(); //Avanalche Network - Wrapped AVAX
        path[1] = 0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98; //BUSD Rewards

        _pangolinRouter.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value: amount}(
        0,
        path,
        addr,
        block.timestamp);
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        uint256 distributeAVAX=(address(this).balance - initialAVAXBalance);
        //distributes remaining AVAX between stakers and Marketing
        _distributeStake(distributeAVAX);
    }
    //swaps tokens on the contract for AVAX
    function _swapTokenForAVAX(uint256 amount) private {
        _approve(address(this), address(_pangolinRouter), amount);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _pangolinRouter.WAVAX();

        _pangolinRouter.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            amount,
            0,
            path,
            address(this),
    ...
    function _addLiquidity(uint256 tokenamount, uint256 avaxamount) private {
        ...
        _approve(address(this), address(_pangolinRouter), tokenamount);
        _pangolinRouter.addLiquidityAVAX{value: avaxamount}(
            address(this),
            tokenamount,
            0,
            0,
    ...
    function TeamWithdrawALLMarketingAVAX() public onlyOwner{
        uint256 amount=marketingBalance;
        marketingBalance=0;
        payable(Development).transfer((amount*25) / 100);
        payable(Marketing).transfer((amount*75) / 100);
    } 
    function TeamWithdrawXMarketingAVAX(uint256 amount) public onlyOwner{
    ...
    function TeamSwitchManualAVAXConversion(bool manual) public onlyOwner{
    ...
    function TeamCreateLPandAVAX() public onlyOwner{
    _swapContractToken();
    }
    ...
    function TeamReleaseLiquidity() public onlyOwner {
        ...
        IPangolinERC20 liquidityToken = IPangolinERC20(_liquidityTokenAddress);
    ...
    function TeamRemoveLiquidity(bool addToStaking) public onlyOwner{
        ...
        liquidityToken.approve(address(_pangolinRouter),amount);
        //Removes Liquidity and either distributes liquidity AVAX to stakers, or 
        // adds them to marketing Balance
        //Token will be converted
        //to Liquidity and Staking AVAX again
        uint256 initialAVAXBalance = address(this).balance;
        _pangolinRouter.removeLiquidityAVAXSupportingFeeOnTransferTokens(
            address(this),
            amount,
            0,
            0,
        ...
        uint256 newAVAXBalance = address(this).balance-initialAVAXBalance;
        if(addToStaking){
            _distributeStake(newAVAXBalance);
        }
        else{
            marketingBalance+=newAVAXBalance;
        }

    }
    //Releases all remaining AVAX on the contract wallet, so AVAX wont be burned
    function TeamRemoveRemainingAVAX() public onlyOwner{
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
