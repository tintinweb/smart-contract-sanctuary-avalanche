...
contract xPTP is ERC20 {
    using SafeERC20 for IERC20;
    address public immutable mainContract;
    address public immutable ptp;


    event XptpMinted(address indexed user, uint256 amount);

    constructor(address _mainContract, address _ptp) ERC20("xPTP", "xPTP") {
        mainContract = _mainContract;
        ptp = _ptp;
    }
    /// @notice deposit PTP in vector protocol and get xPTP at a 1:1 rate
    /// @param _amount the amount of PTP
    function deposit(uint256 _amount) external {
        IERC20(ptp).safeTransferFrom(msg.sender, mainContract, _amount);
        IMainStaking(mainContract).stakePTP(_amount);
        _mint(msg.sender, _amount);
        emit XptpMinted(msg.sender, _amount);
    }
}
