...
import "./Chef.sol";
import "./Upgrade.sol";
import "./PizzeriaV2.sol";
import "./Pizza.sol";


contract UnstakeChefAndToolsV2 is Ownable {

    Chef public chef;
    Upgrade public upgrade;
    PizzeriaV2 public pizzeria;
    Pizza public pizza;

    address public pizzeriaAddress;

    uint256 public additionalPizza = 100*1e18;

    mapping(uint256 => bool) public pizzaClaimed; // chefId => pizza was claimed?


    constructor(Chef _chef, Upgrade _upgrade, Pizza _pizza, address _pizzeriaAddress) {
        chef = _chef;
        upgrade = _upgrade;
        pizza = _pizza;
        pizzeriaAddress = _pizzeriaAddress;
        pizzeria = PizzeriaV2(_pizzeriaAddress);
    }

    function unstakeUpgradesToOwner(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            (address owner, , bool staked) = pizzeria.stakedUpgrades(i);
            if(staked){
                pizza.mint(owner, additionalPizza);
                upgrade.transferFrom(pizzeriaAddress, owner, i);
            }
        }
    }

    function unstakeStakedRestingPizza(uint256 _minId, uint256 _maxId) external onlyOwner {
        for (uint256 i = _minId; i <= _maxId; i++) {
            (address owner, , , bool staked) = pizzeria.stakedChefs(i);
            if(staked){
                require(!pizzaClaimed[i], 'pizza already claimed');
                pizzaClaimed[i] = true;
                uint256[] memory inputData = new uint256[](1);
                inputData[0] = i;
                uint256[] memory claimedByManyChef = pizzeria.getPizzaAccruedForManyChefs(inputData);
                uint256 claimedByChef = claimedByManyChef[0];
                claimedByChef = claimedByChef*80/100 + additionalPizza; //claiming will be fixed at 80% + additionalPizza pizza per chef to make up for gas fees
                pizza.mint(owner, claimedByChef);

                chef.transferFrom(pizzeriaAddress, owner, i);
            } else {
                (address owner2, , , bool staked2) = pizzeria.restingChefs(i);
                if(staked2){
                    pizza.mint(owner2, additionalPizza);
                    chef.transferFrom(pizzeriaAddress, owner2, i);
                }
            }
        }
    }
