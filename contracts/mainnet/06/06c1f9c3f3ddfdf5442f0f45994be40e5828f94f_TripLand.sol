...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠙⠛⠛⠛⠋⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀  
TRIPLAND - BNB/AVAX/MATIC/CRO/FTM Multi-Chain Miner
*/
...
contract TripLand is Context, Ownable {
    ...
    function sellShrooms() public {
        ...
        recAdd.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(shroomValue,fee));
    }
    
    function shroomRewards(address adr) public view returns(uint256) {
        uint256 hasShrooms = getMyShrooms(adr);
        uint256 shroomValue = 0;
        if(hasShrooms > 0){
          shroomValue = calculateShroomSell(hasShrooms);
        }
        return shroomValue;
    }
    
    function buyShrooms(address ref) public payable {
        require(initialized);
        uint256 shroomsBought = calculateShroomBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function calculateShroomSell(uint256 shrooms) public view returns(uint256) {
        return calculateTrade(shrooms,marketShrooms,address(this).balance);
    }
    ...
    function calculateShroomBuySimple(uint256 eth) public view returns(uint256) {
        return calculateShroomBuy(eth,address(this).balance);
    }
