...
contract PublicSale is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) capByEpochs;
    ...
    constructor(
        ...
        capByEpochs[0] = 1000 * 10**ERC20(investToken).decimals(); // 1–15min: 1000$ max contribution
        capByEpochs[1] = 3000 * 10**ERC20(investToken).decimals(); // 16–30min: 3000$ max contribution
        capByEpochs[2] = 6000 * 10**ERC20(investToken).decimals(); // 31–45min: 6000$ max contribution
        capByEpochs[3] = 10000 * 10**ERC20(investToken).decimals(); // 46–60min: 10 000$ max contribution
    }
    ...
    function currentCap() public view returns (uint256) {
        ...
        uint256 cap = capByEpochs[epoch];

        //return maxCap if no cap defined in capByEpochs
        if (epoch == 0) {
            return capByEpochs[0];
        } else if (cap == 0 || cap > maxCap) {
            return maxCap;
        } else {
            return cap;
        }
