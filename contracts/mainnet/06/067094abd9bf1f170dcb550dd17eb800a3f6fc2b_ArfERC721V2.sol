...
contract ArfERC721V2 is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    Mintpeg public ConstructAddress;
    ...
    /**
     * @dev Emitted when user burns tokens
     */
    event TokenBurned(
        address PreviousOwner,
        uint256[] TokenIds,
    ...
    function updateARCPoolDetails(
        ...
        for (uint256 i = 0; i < _pool_id.length; i++) {
            ...
            FactoryPools[_epoch_id][_pool_id[i]]
                .totalQuantityARC = _quantity_arc[i];
     ...
     * @param _construct_address construct wallet address
     *
     * Requirements:
     * - msg.sender must be owner address of this contract.
     *
    ...
    function updateWallets(
        ...
        ConstructAddress = Mintpeg(_construct_address);
    ...
    function mintInternalARC(uint256 epoch_id) external nonReentrant {
        ...
        if (epoch_id == 1) {
            ...
            emit ARC_Internal_Minted(msg.sender, tokenId, 1);
        }
        ...
        if (epoch_id == 2) {
            ...
            emit ARC_Internal_Minted(msg.sender, tokenId, 2);
        }
    ...
    function _beforeTokenTransfer(
        ...
        require(!isBlackListed[from] && !isBlackListed[to], "ArfERC721: User is blacklisted");
    }
    ...
    function burnTokens(uint256[] calldata _tokenIds) external virtual {
        emit TokenBurned(ownerOf(_tokenIds[0]), _tokenIds, msg.sender);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                msg.sender == ownerOf(_tokenIds[i]) ||
                    msg.sender == owner(),
                "ArfERC721: Invalid owner"
            );
            _burn(_tokenIds[i]);
        }
    }
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "ArfERC721: URI query for nonexistent token"
        );
        return
            string(
                abi.encodePacked(
