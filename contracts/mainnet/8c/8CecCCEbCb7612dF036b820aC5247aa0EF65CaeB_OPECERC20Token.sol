...
contract OPECERC20Token is ERC20Permit, Ownable {
    using SafeMath for uint256;
    ...
    event SetBlackList(address indexed account, bool isBlacklisted);

    event treasuryWalletUpdated(address indexed newWallet, address indexed oldWallet);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor(address _treasuryWallet, address _dai) 
    ERC20("Opulence", "OPEC", 18) 
    ERC20Permit("OPEC") 
    {
        dai = _dai;
        ...
        uniswapV2Pair = IJoeFactory(_uniswapV2Router.factory()).createPair(address(this), dai);
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);

        initialSupply = 1000000*1e18;
        maxTransactionAmount = initialSupply * 5 / 10000; // 0.05% maxTransactionAmountTxn
    ...
    function mint(address account, uint256 amount) external onlyOwner {
        ...
        require(totalAmount + amount <= 5000000 * 1e18, "OPEC: TOTAL SUPPLY INCORRECT");
    ...
    function _transfer(
        ...
        require(!blacklists[from] && !blacklists[to], "OPEC: blacklist address");
        ...
        if(takeFee && feeEnable){
            // on sell

            if (automatedMarketMakerPairs[to] && enableTime != 0 && block.timestamp <= enableTime + 30 minutes){
                fees = amount.mul(70).div(100);
                tokensForTreasury += fees;
                super._transfer(from, address(this), fees);
            }

            if (automatedMarketMakerPairs[to] && enableTime != 0 && block.timestamp > enableTime + 30 minutes && block.timestamp <= enableTime + 30 minutes + 5 hours){
                uint256 passedHours = (block.timestamp - enableTime - 30 minutes) / 1 hours + 1;
                fees = amount.mul(70 - 5 * passedHours).div(100);
                tokensForTreasury += fees;
                super._transfer(from, address(this), fees);
            }

            if (automatedMarketMakerPairs[to] && enableTime != 0 && block.timestamp > enableTime + 30 minutes + 5 hours) {
                uint256 passedDates = (block.timestamp - enableTime - 30 minutes - 5 hours) / 1 days + 1;
                uint256 feePercent = 0;
                if (45 > 10 * passedDates) {
                    feePercent = 45 - 10 * passedDates;
                }
    ...
    function swapBack() private {
        ...
        swapTokensForTokens(amountToSwap);
    }
}