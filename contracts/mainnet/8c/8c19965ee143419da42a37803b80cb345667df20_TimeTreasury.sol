...
contract TimeTreasury is Ownable {
	...
	/**
        @notice returns excess token rate
        @return uint
     */
    function excessRate() public view returns ( uint ) {
        return excessrate;
    }

    /**
        @notice sets excess token rate
     */
    function setexcessRate(uint _erate) external onlyManager() {
		excessrate = _erate;
	}
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Time ).totalSupply().sub( totalDebt ) ).add( 1 ).mul( excessrate );
    }
