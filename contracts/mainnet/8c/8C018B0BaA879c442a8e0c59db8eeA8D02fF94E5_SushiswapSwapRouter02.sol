...
import './SushiswapV2SwapRouter.sol';
import './V3SwapRouter.sol';
import './base/ApproveAndCall.sol';
import './base/MulticallExtended.sol';

/// @title Uniswap V2 and V3 Swap Router
contract SushiswapSwapRouter02 is ISwapRouter02, SushiswapV2SwapRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    constructor(
        address _factoryV2,
        address factoryV3,
...
abstract contract SushiswapV2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    using LowGasSafeMath for uint256;
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair pair = IUniswapV2Pair(SushiswapV2Library.pairFor(factoryV2, input, output));
            uint256 amountInput;
            ...
            address to = i < path.length - 2 ? SushiswapV2Library.pairFor(factoryV2, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        pay(
            ...
            SushiswapV2Library.pairFor(factoryV2, path[0], path[1]),
            amountIn
        );
    ...
    function swapTokensForExactTokens(
        ...
        amountIn = SushiswapV2Library.getAmountsIn(factoryV2, amountOut, path)[0];
        require(amountIn <= amountInMax, 'Too much requested');

        pay(path[0], msg.sender, SushiswapV2Library.pairFor(factoryV2, path[0], path[1]), amountIn);
