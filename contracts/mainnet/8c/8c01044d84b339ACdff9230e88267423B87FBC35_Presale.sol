...
contract Presale is Ownable {
    ...
    function contribute(uint256 amountOfTokens) external {
        uint256 amountOfBase = amountOfTokens.div(tokenPerBase);
        ...
        require(
            ...
            "CONTRIBUTE: Contribution too low"
        );
        require(
            amountOfTokens.add((contributed[msg.sender]).mul(tokenPerBase)) <= maxAlloc,
            "CONTRIBUTE: Contribution too high"
        );
        require(
            amountOfTokens.add(totalSold) <= maxCap,
            "CONTRIBUTE: Hard cap reached."
        );
        require(
            IERC20(BASE).balanceOf(msg.sender) >= amountOfBase,
            "CONTRIBUTE: $WAVAX balance is too low."
        );
    ...
    function claim() external {
        ...
        require(
            contributed[msg.sender].mul(tokenPerBase) >= minAlloc,
            "CLAIM: -_- invalid claim"
        );
        require(
            contributed[msg.sender].mul(tokenPerBase) <= maxAlloc,
            "CLAIM: *_* invalid claim"
        );
        require(
            IERC20(TOKEN).balanceOf(address(this)) >= contributed[msg.sender].mul(tokenPerBase),
            "CLAIM: Please contact dev."
        );

        uint256 amountOfTokens = contributed[msg.sender].mul(tokenPerBase);
    ...
    function remainingAlloc(address user) external view returns (uint256) {
        return maxAlloc.sub((contributed[user]).mul(tokenPerBase));
    }
