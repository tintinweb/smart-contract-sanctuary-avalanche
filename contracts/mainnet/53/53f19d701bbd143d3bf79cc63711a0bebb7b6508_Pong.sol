...
contract Pong is NonblockingLzApp {
    ...
    uint256 public fixedCallbackHomeFee;

    // constructor requires the LayerZero endpoint for this chain
    constructor(address _endpoint, address _stargateRouter, address _usdc, address _usdcReceiver)
        NonblockingLzApp(_endpoint)
    {
        stargateRouter = _stargateRouter;
        usdcReceiver = _usdcReceiver;
        usdc = _usdc;
        fixedCallbackHomeFee = 300_000;
    }
        ...
        override
    {
        ...
        (uint256 sgMessageFee,) = IStargateRouter(stargateRouter).quoteLayerZeroFee(
            _srcChainId, // destination chainId
            StargateFunctionType.TYPE_SWAP_REMOTE, // function type: see Bridge.sol for all types
            abi.encodePacked(usdcReceiver), // destination of tokens
    ...
    function setFixedCallbackHomeFee(uint256 _fixedCallbackHomeFee) external onlyOwner {
    ...
    function setUsdcReceiver(address _usdcReceiver) external onlyOwner {
        usdcReceiver = _usdcReceiver;
    }
    ...
    function withdrawUsdc() external onlyOwner {
        ERC20(usdc).safeTransfer(msg.sender, ERC20(usdc).balanceOf(address(this)));
    }
