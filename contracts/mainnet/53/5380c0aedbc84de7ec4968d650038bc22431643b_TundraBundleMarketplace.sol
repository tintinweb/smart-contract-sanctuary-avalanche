...
// File: contracts/TundraBundleMarketplace.sol
...
contract TundraBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    ITundraTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "TundraMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "TundraMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            ITundraMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            ITundraMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            ITundraMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
