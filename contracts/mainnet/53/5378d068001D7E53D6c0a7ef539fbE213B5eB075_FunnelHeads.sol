...
contract FunnelHeads is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    using Strings for uint256;

    uint256 public constant MAX_MINTABLE = 5555; // 5555 total
    uint256 public constant NFT_PRICE = 1 ether; // 1 avax for this launchpad
...
//
    function claim(uint256 n) public payable {
        ...
        require(n > 0, "Number need to be higher than 0");
        require(n <= 2, "Max claim per address is 2");
        require((mainMintCount[msg.sender] + n ) <= 2, "Max claim per address is 2");
        require(n <= MAX_PER_CLAIM, "Max per claim is 10");
        require(
...
//
    ...
    function setEarlyMinters(address addresses, uint256 allowedToMint) external onlyOwner {
            earlyMintAddresses[addresses] = allowedToMint;
    }
