...
import "/OpenZeppelin/[email protected]/contracts/token/ERC721/utils/ERC721Holder.sol";
...
contract Marketplace is ERC1155Holder, ERC721Holder, Ownable {
    ...
    event New721Listing(address indexed seller, uint256 indexed listingId);
    event New1155Listing(address indexed seller, uint256 indexed listingId);
    ...
    constructor(IERC20 craft_, IEventLogger eventLogger_) {
        craft = craft_;
    ...
    function list721(IERC721 token, uint256 tokenId, uint256 price) external {
        ...
        emit New721Listing(msg.sender, listingId);
        eventLogger.reportMarketplaceListingCreated(listingId);
    }

    function list1155(IERC1155 token, uint256 tokenId, uint256 amount, uint256 price) external {
        require(price > 0, "price cannot be zero");
        ...
        emit New1155Listing(msg.sender, listingId);
        eventLogger.reportMarketplaceListingCreated(listingId);
    }
    ...
    function cancelSale(uint256 listingId) external {
        ...
        eventLogger.reportMarketplaceListingClosed(listingId);
    }
    ...
    function buyListing(uint256 listingId) external {
        ...
        craft.safeTransferFrom(msg.sender, address(this), listing.price);
        craft.safeTransfer(listing.seller, listing.price * (1e6 - feePercentage) / 1e6);
