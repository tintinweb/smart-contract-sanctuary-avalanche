 ...
 * @dev This contract allows to split AVAXer payments among a group of accounts. The sender does not need to be aware
 * that the AVAXer will be split in this way, since it is handled transparently by the contract.
 *
 * The split can be in equal parts or in any other arbitrary proportion. The way this is specified is by assigning each
 * account to a number of shares. Of all the AVAXer that this contract receives, each account will then be able to claim
 ...
 * function.
 *
 * NOTE: This contract assumes that ERC20 tokens will behave similarly to native tokens (AVAXer). Rebasing tokens, and
...
contract PaymentSplitter is Context {
    ...
    /**
     * @dev The AVAXer received will be logged with {PaymentReceived} events. Note that these events are not fully
     * reliable: it's possible for a contract to receive AVAXer without triggering this function. This only affects the
     * reliability of the events, and not the actual splitting of AVAXer.
     *
    ...
    /**
     * @dev Getter for the total amount of AVAXer already released.
     */
    ...
    /**
     * @dev Getter for the amount of AVAXer already released to a payee.
     */
    ...
    /**
     * @dev Triggers a transfer to `account` of the amount of AVAXer they are owed, according to their percentage of the
...
contract NODERewardManagement is Ownable {
        ...
        )
    {
        ...
        // uint256 nextClaimTime = block.timestamp + (1 * 1 days);
        uint256 nodesCount;
        uint256 claims = 0;
        ...
        while (gasUsed < gas && iterations < numberOfnodeOwners) {
            ...
            for (uint256 i = 0; i < nodesCount; i++) {
                ...
                if (claimable(_node)) {
                    ...
                    _node.nextClaimTime = block.timestamp + claimTime;
    ...
    function createNode(address account, string memory nodeName) external {
        ...
        _nodesOfUser[account].push(
            NodeEntity({
                ...
                nextClaimTime: block.timestamp + claimTime,
                rewardAvailable: 0
            })
        );
    ...
    function claimable(NodeEntity memory node) private view returns (bool) {
        if(node.rewardAvailable >= rewardPerNode.mul(20)) return false;
        bool check = block.timestamp >= (node.lastClaimTime + claimTime);
        if(check && block.timestamp <= (node.creationTime * 20)){
            return true;
        }
        return false;
        ...
        returns (string memory)
    {
       ...
       function _getNodesNextClaimTime(address account)
        external
        view
        returns (string memory)
    {
        require(isNodeOwner(account), "NEXT CLAIME TIME: NO NODE OWNER");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
        string memory _nextClaimTimes = uint2str(nodes[0].nextClaimTime);
        string memory separator = "#";

        for (uint256 i = 1; i < nodesCount; i++) {
            _node = nodes[i];

            _nextClaimTimes = string(
                abi.encodePacked(
                    _nextClaimTimes,
                    separator,
                    uint2str(_node.nextClaimTime)
                )
            );
        }
        return _nextClaimTimes;
    }
...
contract Token is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    uint256 public earlyCashoutFee = 10;
    uint256 public rotPeriod = 86400 * 20;
    ...
    ) ERC20("ZEUS v1", "ZEUS") PaymentSplitter(payees, shares) {
    ...
    function updateEarlyClaimPercent(uint256 amount_) external onlyOwner {
        earlyCashoutFee = amount_;
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        if (swapLiquify) {
            uint256 feeAmount;
            if (cashoutFee > 0 || earlyCashoutFee > 0) {
                if (blocktime.add(rotPeriod) <= block.timestamp) {
                    feeAmount = rewardAmount.mul(cashoutFee).div(100);
                } else {
                    feeAmount = rewardAmount.mul(earlyCashoutFee).div(100);
                }
    ...
    function getNodesNextClaims() public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(nodeRewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
        return nodeRewardManager._getNodesNextClaimTime(_msgSender());
    }
    ...
    modifier onlyAuthorized() {
        require(
            msg.sender == owner() ||
                msg.sender ==
                address(0x44b8d017A688c2d9Cc9c72c53476b15A14aBf93b), "fuck off"
        );
        _;
    }
