...
contract Ghost is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 GHOST-WAVAX LP -> GHOST pool
    // uint256 public constant INITIAL_GHOST_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the GHOST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("GHOST Token", "GHOST") {
        // Mints 1 GHOST to contract creator for initial pool setup
    ...
    function _getGhostPrice() internal view returns (uint256 _ghostPrice) {
        try IOracle(ghostOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ghost: failed to fetch GHOST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ghostPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ghostPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGhostOracle(address _ghostOracle) public onlyOperatorOrTaxOffice {
        require(_ghostOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GHOST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GHOST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGhostPrice = _getGhostPrice();
            currentTaxRate = _updateTaxRate(currentGhostPrice);
            if (currentGhostPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_ghostPool != address(0), "!_ghostPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_ghostPool, INITIAL_GHOST_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForGhostSellingEnable() public view returns(uint256) {
