...
contract BEP20 is Context {
  ...
  constructor() {
    ...
    IJoeRouter02 _joeRouterRouter = IJoeRouter02(_pancakeRouter);
    WAVAX = _joeRouterRouter.WAVAX();
    uniswapV2Pair = IJoeFactory(_joeRouterRouter.factory()).createPair(
      address(this),
      WAVAX
    );
  }
...
contract Coin is BEP20, IERC20Joe {
  mapping(address => uint256) private _tOwned;
  mapping(address => mapping(address => uint256)) private _allowances;

  uint256 private constant MAX = ~uint256(0);

  string public name =/*TOKENNAME*/"BabySpell"/*TOKENNAME*/;
  string public symbol =/*TOKENSYMBOL*/"BabySpell"/*TOKENSYMBOL*/;
