...
contract SpookyTreasury is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;

    mapping(address => bool) public strategies;
    address[] public strategiesArray;
    ISpookyStaking public staking;

    constructor(ISpookyStaking _staking) {
        staking = _staking;
    }

    // ========== PUBLIC VIEW FUNCTIONS ============

    /// @notice Return ERC-20 balance of SpookyTreasury
    /// @param _token Address of the ERC-20 token
    /// @return Balance of the SpookyTreasury
    ...
    function requestFund(address _token, uint256 _amount) external {
        require(
            ...
            "SpookyTreasury::requestFund: Only strategies can request fund"
        );
        require(
            _amount <= balanceOf(_token),
            "SpookyTreasury::requestFund: Request more fund than balance"
        );
    ...
    function addStrategy(address _strategy) external onlyOwner {
        require(_strategy != address(0), "SpookyTreasury::addStrategy: invalid address");
        require(
            !strategies[_strategy],
            "SpookyTreasury::addStrategy: strategy was previously added"
        );
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        require(strategies[_strategy], "SpookyTreasury::removeStrategy: strategy not found");
    ...
    function allocateFee(address _token, uint256 _amount) external onlyOwner {
        require(
            ...
            "SpookyTreasury::allocateFee:Fee distributor not set"
        );
        require(_amount > 0, "SpookyTreasury::allocateFee: invalid amount");
