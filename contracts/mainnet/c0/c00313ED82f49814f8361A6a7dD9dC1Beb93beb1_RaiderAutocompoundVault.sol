...
import "../interfaces/IStrategyAutocompound.sol";
import "../interfaces/IUniswapRouterETH.sol";
import "../interfaces/IRaiderZapper.sol";

contract RaiderAutocompoundVault is ERC20, Ownable, ReentrancyGuard {
  ...
  function balance() public view returns (uint256) {
    return
      token.balanceOf(address(this)).add(
        IStrategyAutocompound(strategy).balanceOf()
      );
  }
  ...
  function earn() public {
    ...
    IStrategyAutocompound(strategy).deposit();
  }
  ...
  function withdraw(uint256 _shares) public nonReentrant {
    ...
    if (b < r) {
      ...
      IStrategyAutocompound(strategy).withdraw(_withdraw);
      uint256 _after = token.balanceOf(address(this));
  ...
  function upgradeStrat() public onlyOwner {
    ...
    IStrategyAutocompound(strategy).retireStrat();
...
interface IStrategyAutocompound {
