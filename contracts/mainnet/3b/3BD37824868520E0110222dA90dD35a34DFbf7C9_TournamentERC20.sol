...
// File: gladiator-finance-contracts/contracts/TournamentERC20.sol
...
contract TournamentERC20 is Ownable, ITournament {
    ...
    constructor (address _wl, address _token, uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _gladShare, uint256 _burnShare) {
    ...
    function buy() external payable {
        ...
        require(oldBalance + ticketPrice == rewardToken.balanceOf(address(this)), "Transfer of ticket price failed");
        gladPotSize += ticketPrice * GLAD_SHARE / 10000;
        if (BURN_SHARE >  0) {
            rewardToken.burn(ticketPrice * BURN_SHARE / 10000);
        }
        rewardToken.transfer(owner(), ticketPrice * (10000 - POT_SHARE - GLAD_SHARE - BURN_SHARE) / 10000);
        winner = msg.sender;
    }
