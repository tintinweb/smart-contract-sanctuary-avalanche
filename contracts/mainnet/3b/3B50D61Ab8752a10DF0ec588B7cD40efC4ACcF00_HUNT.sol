...
import "./type/ERC721URIStorage.sol";
import "./library/Ownable.sol";
import "./library/Counters.sol";
import "./library/Strings.sol";
import "./library/Base64.sol";

contract HUNT is Ownable, ERC721URIStorage {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds;

    mapping(address => bool) public poolList;
    mapping(uint => HuntMetadata) public NftMeta;
    struct HuntMetadata {
        address pool;
        string name;
        string imageURI;
    ...
    constructor()
    ERC721("Hunt NFT", "HUNT") {}

    function managePool(address[] calldata pools, bool flag) external onlyOwner {
        require(pools.length > 0);

        for(uint i = 0; i < pools.length; i++) {
            poolList[pools[i]] = flag;
        }
    }

    function updateURI(uint tokenId, string memory metaURI) external onlyOwner {
    ...
    function createToken(
        ...
        uint tokenPercent,
        address receiver
    ) external returns(uint newItemId) {
        require(poolList[msg.sender], "Not authorized");
        ...
        NftMeta[newItemId] = HuntMetadata({
            pool: msg.sender,
            name: tokenName,
            imageURI: imageURI,
    ...
    function getTokens() external view returns(uint[] memory) {
        ...
        if(tokenCount == 0) {
            ...
            for (uint nft = 1; nft <= currentId; nft++) {
                if (ERC721.ownerOf(nft) == msg.sender) {
                    result[resultIndex] = nft;
                    resultIndex++;
                }
            }
