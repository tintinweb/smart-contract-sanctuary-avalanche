...
contract BeeTradeOrderbook {
    ...
    function depositAVAX(uint256 _amount) external payable {
        require(msg.value > 0, "Beetrade: Please Deposit Right Amount");
    ...
    function depositToken(address _token, uint256 _amount) external {
        require(_amount > 0, "Beetrade: Amount Can't Be Zero");
        ...
        if(decimals < 18){
            uint8 difference = 18 - decimals;
            tokenAmount = _amount / (10 ** difference);
        } else if(decimals > 18){
            uint8 difference = decimals - 18;
            tokenAmount = _amount * (10 ** difference);
        } 
