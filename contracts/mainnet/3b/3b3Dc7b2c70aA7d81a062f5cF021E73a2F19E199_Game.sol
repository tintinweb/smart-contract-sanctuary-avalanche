...
contract Game is IGame, Ownable, ReentrancyGuard, Pausable {
    ...
    event ManyPowerUpsMinted(address indexed owner, uint256 indexed amount);
    event ManyPotionsMinted(address indexed owner, uint256 indexed amount);
    event ExcaliburMinted(address indexed owner, uint256 indexed amount);

    /** PUBLIC VARS */
    uint256 public GEN0_PRICE = 2500000000000000000;
    ...
    uint256 public DAILY_WORLD_RATE = 400 ether;
    uint256 public DAILY_WORLD_PER_LEVEL = 800 ether;

    uint256 public POWERUP_DIMINISHING_FROM = 11;
    uint256 public POWERUP_DAILY_WORLD_RATE = 10 ether;
    uint256 public POWERUP_WORLD_MINT_COST = 20_000 ether;
    uint256 public POWERUP_MIN_WORLD_MINT_COST = 20_000 ether;
    uint256 public POWERUP_LAST_MINT_TIME = block.timestamp;

    uint256 public LEVEL_DOWN_AFTER_DAYS = 4 days;
    uint256 public POTION_WORLD_MINT_COST = 100_000 ether;
    uint256 public POTION_LEVEL_DOWN_INCREASE_DAYS = 1 days;
    uint256 public POTION_MIN_WORLD_MINT_COST = 100_000 ether;
    uint256 public POTION_LAST_MINT_TIME = block.timestamp;
    ...
    function mintPowerUp(uint256 amount)
        external
        whenNotPaused
        nonReentrant
        onlyEOA
    {
        uint256 totalCost = amount * getPowerUpMintCost();
        ...
        // Increase the price after mint
        POWERUP_WORLD_MINT_COST =
            getPowerUpMintCost() +
            ((POWERUP_WORLD_MINT_COST * MINT_COST_INCREASE_PERCENT) / 100);

        POWERUP_LAST_MINT_TIME = block.timestamp;
        emit ManyPowerUpsMinted(_msgSender(), amount);
    }

    function getPowerUpMintCost() public view returns (uint256 newCost) {
        uint256 intervalDiff = (block.timestamp - POWERUP_LAST_MINT_TIME) /
            MINT_COST_REDUCE_INTERVAL;
        uint256 reduceBy = ((POWERUP_WORLD_MINT_COST * MINT_COST_REDUCE_PERCENT) /
            100) * intervalDiff;

        if (POWERUP_WORLD_MINT_COST > reduceBy) {
            newCost = POWERUP_WORLD_MINT_COST - reduceBy;
        } else {
            newCost = 0;
        }

        if (newCost < POWERUP_MIN_WORLD_MINT_COST)
            newCost = POWERUP_MIN_WORLD_MINT_COST;

        return newCost;
    }

    function mintPotion(uint256 amount)
        external
        whenNotPaused
        nonReentrant
        onlyEOA
    {
        uint256 totalCost = amount * getPotionMintCost();
        ...
        // Increase the price after mint
        POTION_WORLD_MINT_COST =
            getPotionMintCost() +
            ((POTION_WORLD_MINT_COST * MINT_COST_INCREASE_PERCENT) / 100);

        POTION_LAST_MINT_TIME = block.timestamp;
        emit ManyPotionsMinted(_msgSender(), amount);
    }

    function getPotionMintCost() public view returns (uint256 newCost) {
        uint256 intervalDiff = (block.timestamp - POTION_LAST_MINT_TIME) /
            MINT_COST_REDUCE_INTERVAL;
        uint256 reduceBy = ((POTION_WORLD_MINT_COST *
            MINT_COST_REDUCE_PERCENT) / 100) * intervalDiff;

        if (POTION_WORLD_MINT_COST > reduceBy) {
            newCost = POTION_WORLD_MINT_COST - reduceBy;
        } else {
            newCost = 0;
        }

        if (newCost < POTION_MIN_WORLD_MINT_COST)
            newCost = POTION_MIN_WORLD_MINT_COST;
        ...
        returns (INFT.HeroVillain memory)
    {
        ...
        uint256 potionsInWallet = getBalanceOfActivePotions(ownerOfToken);
        // Potions increase your level down days, thus your level goes down slower
        uint256 LEVEL_DOWN_AFTER_DAYS_NEW = (potionsInWallet *
            POTION_LEVEL_DOWN_INCREASE_DAYS) + LEVEL_DOWN_AFTER_DAYS;
    ...
    // Get the number of Potions that are "active", meaning they have an effect on the game
    function getBalanceOfActivePotions(address owner)
        public
        view
        returns (uint256)
    {
        ...
        returns (uint256 owed)
    {
        ...
        if (traits.isHero) {
            ...
            // The rewards from PowerUps with diminishing returns after POWERUP_DIMINISHING_FROM
            uint256 powerUpsInWallet = getBalanceOfActivePowerUps(ownerOfToken);
            uint256 rewardsFromPowerUps = 0;

            if (powerUpsInWallet < POWERUP_DIMINISHING_FROM) {
                rewardsFromPowerUps =
                    traits.level *
                    powerUpsInWallet *
                    POWERUP_DAILY_WORLD_RATE;
            } else if (powerUpsInWallet >= POWERUP_DIMINISHING_FROM) {
                // Normal rewards until X-1 powerUps
                rewardsFromPowerUps =
                    traits.level *
                    (POWERUP_DIMINISHING_FROM - 1) *
                    POWERUP_DAILY_WORLD_RATE;

                uint256 lastPowerUpRewards = POWERUP_DAILY_WORLD_RATE;
                uint256 remainingPowerUps = powerUpsInWallet -
                    (POWERUP_DIMINISHING_FROM - 1);
                for (uint256 i = 1; i <= remainingPowerUps; i++) {
                    lastPowerUpRewards = (lastPowerUpRewards * 95) / 100;
                    rewardsFromPowerUps += traits.level * lastPowerUpRewards;
                }
            }

            owed +=
                ((block.timestamp - myStake.stakeTimestamp) *
                    (rewardsFromLevels + rewardsFromPowerUps)) /
                1 days;
        } else {
            // Villain
    ...
    // Get the number of PowerUps that are "active", meaning they have an effect on the game
    function getBalanceOfActivePowerUps(address owner)
        public
        view
        returns (uint256)
    {
        ...
        returns (uint256 price)
    {
        ...
        if (level == 2) return 1400 ether;
        if (level == 3) return 1900 ether;
        if (level == 4) return 2960 ether;
        if (level == 5) return 3170 ether;
        if (level == 6) return 4520 ether;
        if (level == 7) return 5320 ether;
        if (level == 8) return 6110 ether;
        if (level == 9) return 6910 ether;
        if (level == 10) return 7710 ether;
        if (level == 11) return 8510 ether;
        if (level == 12) return 9310 ether;
        if (level == 13) return 10110 ether;
        if (level == 14) return 10900 ether;
        if (level == 15) return 11700 ether;
        if (level == 16) return 12500 ether;
        if (level == 17) return 13300 ether;
        if (level == 18) return 14100 ether;
        if (level == 19) return 14900 ether;
        if (level == 20) return 15690 ether;
        if (level == 21) return 16490 ether;
        if (level == 22) return 17290 ether;
        if (level == 23) return 18090 ether;
        if (level == 24) return 18890 ether;
        if (level == 25) return 19860 ether;
        if (level > 25) return 20000 ether;
    ...
    function setContracts(
        ...
        uPotion = IPotion(_uPowerUp);
        uPowerUp = IPowerUp(_uPotion);
    ...
    function setPowerUpDiminishingFrom(uint256 number) external onlyOwner {
        POWERUP_DIMINISHING_FROM = number;
    }

    function setPowerUpWorldMintCost(uint256 number) external onlyOwner {
        POWERUP_WORLD_MINT_COST = number;
    }

    function setPowerUpDailyWorldRate(uint256 number) external onlyOwner {
        POWERUP_DAILY_WORLD_RATE = number;
    }

    function setPowerUpMinWorldMintCost(uint256 number) external onlyOwner {
        POWERUP_MIN_WORLD_MINT_COST = number;
    }

    function setPotionWorldMintCost(uint256 number) external onlyOwner {
        POTION_WORLD_MINT_COST = number;
    }

    function setPotionLevelDownIncreaseDays(uint256 number) external onlyOwner {
        POTION_LEVEL_DOWN_INCREASE_DAYS = number;
    }

    function setPotionMinWorldMintCost(uint256 number) external onlyOwner {
        POTION_MIN_WORLD_MINT_COST = number;
    }
