...
/* used for implementing EIP 4494 */
library ECDSA {
    enum RecoverError {
        NoError,
        InvalidSignature,
...
/* interface implementing EIP 4494 see https://github.com/dievardump/erc721-with-permits */
...
abstract contract ERC721WithPermit is IERC721WithPermit, ERC721Enumerable {
...
pragma solidity ^0.8.0;
contract BasicNFT is ERC721WithPermit, Ownable {
    using Strings for uint256;

    /* queries oracleToken for balance of oracleUser for help with pseudorandom number generation */
    /* oracleToken should be something like wavax or usdc and oracleUser should be an LP token contract */

    /* baseURI should have a trailing forward-slash */
    string public baseURI;
    string public baseExtension = ".json";
    
    /* max supply cannot be changed after construction */
    ...
    constructor(
		...
		uint _maxSupply
		) ERC721(_name, _symbol) ERC721WithPermit(_DOMAIN_SEPARATOR) {
    ...
    function devmint(address[] memory recipients) external onlyOwner {
	uint length = recipients.length;
	uint supply = totalSupply();
	require(supply + length <= maxSupply, "insufficient supply");
	for (uint256 i = 0; i < length; ++i) {
	    _mint(recipients[i],i+supply);
	}
    }
    ...
    /* virtual means it can be overridden while override means it is overriding an inherited definition of tokenURI */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
	require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");        
