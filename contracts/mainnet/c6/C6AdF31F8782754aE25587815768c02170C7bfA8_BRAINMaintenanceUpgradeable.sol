...
import "./utils/NFTInfoUpgradeable.sol";
...
contract BRAINMaintenanceUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ...
    WithdrawableUpgradeable
{
    ...
    mapping(uint256 => BRAINInfo) public brainsToFees;
    ...
    error InvalidBRAINID(uint256 brainID);
    error InvalidAccountForBRAINID(uint256 brainID);
    error NotApproved();
    error NotFeeMultiplier(uint256 payment);

    event PayMaintenanceFees(
        address indexed account,
        uint256[] indexed brainIDs,
        uint256[] indexed intervals
    );
    ...
    modifier onlyApproved() {
        if (_msgSender() != owner() && !proxyToApproved[_msgSender()]) {
            ...
            console.log("approved:", proxyToApproved[_msgSender()]);
            revert NotApproved();
        }
        _;
    }
    ...
    {
        uint256[] memory intervalsActive = new uint256[](brainIDs.length);
        uint256[] memory intervalsPaid = new uint256[](brainIDs.length);
        for (uint256 x; x < brainIDs.length; x++) {
            BRAINInfo memory brain = brainsToFees[brainIDs[x]];
            if (brain.creationTimestamp == 0)
                revert InvalidBRAINID(brainIDs[x]);
            intervalsActive[x] = _intervalsActive(brain.creationTimestamp);
            intervalsPaid[x] = brain.intervalsPaid;
        }
        return (intervalsActive, intervalsPaid);
    }
    ...
    {
        for (uint256 x; x < brainIDs.length; x++) {
            brainsToFees[brainIDs[x]] = BRAINInfo({
    ...
    function payMaintenanceFees(
        ...
        for (uint256 x; x < brainIDs.length; x++) {
            BRAINInfo storage brain = brainsToFees[brainIDs[x]];
            if (brain.creationTimestamp == 0)
                revert InvalidBRAINID(brainIDs[x]);
            if (brainNFTContract.ownerOf(brainIDs[x]) != account)
                revert InvalidAccountForBRAINID(brainIDs[x]);
            if (!withinGracePeriod(brainIDs[x])) {
                brainRewardsContract.setBRAINRewardToZero(brainIDs[x]);
            }
            brain.intervalsPaid += intervals[x];
        }
        emit PayMaintenanceFees(account, brainIDs, intervals);
    }
    ...
    function setInterval(uint256 seconds_) external onlyApproved {
        if (seconds_ < 86400) revert IntervalTooShort();
        interval = seconds_;
    }
    ...
    function withinGracePeriod(uint256 brainID) public view returns (bool) {
        BRAINInfo storage brain = brainsToFees[brainID];
        uint256 activeTime = block.timestamp - brain.creationTimestamp;
        uint256 paidTime = brain.intervalsPaid > 0
            ? brain.intervalsPaid * interval + (interval * 3) / 2
            : 0;
        if (paidTime >= activeTime) return true;
        return activeTime - paidTime <= (interval * 3) / 2;
    }

    function _intervalsActive(uint256 creationTimestamp)
        private
        view
        returns (uint256)
    {
        return (block.timestamp - creationTimestamp) / interval;
    }
}
...
abstract contract NFTInfoUpgradeable is Initializable {
    ...
    mapping(uint256 => bool) public NFTIDHasNFTInfo;

    error OutOfBounds();

    function __NFTInfoUpgradeable_init() internal onlyInitializing {
        __NFTInfoUpgradeable_init_unchained();
    }

    function __NFTInfoUpgradeable_init_unchained() internal onlyInitializing {}

    // caller should provide security
    function _setNFTInfo(
        ...
        nftInfo.propertyNum1 = propertyNum1;
        nftInfo.propertyNum2 = propertyNum2;
        nftInfo.propertyNum3 = propertyNum3;
        nftInfo.propertyString1 = propertyString1;
        nftInfo.propertyString2 = propertyString2;
        NFTIDHasNFTInfo[tokenID] = true;
    }

    function _setNFTNumericInfo(
        uint256 tokenID,
        uint256 propertyNum,
        uint256 value
    ) internal virtual {
        if (propertyNum > 3) revert OutOfBounds();
        NFTInfo storage nftInfo = NFTInfos[tokenID];
        if (propertyNum == 1) {
            nftInfo.propertyNum1 = value;
        } else if (propertyNum == 2) {
            nftInfo.propertyNum2 = value;
        } else {
            nftInfo.propertyNum3 = value;
        }
    }

    function _setNFTStringInfo(
        uint256 tokenID,
        uint256 propertyNum,
        string memory value
    ) internal {
        if (propertyNum > 2) revert OutOfBounds();
        NFTInfo storage nftInfo = NFTInfos[tokenID];
        if (propertyNum == 1) {
            nftInfo.propertyString1 = value;
        } else {
            nftInfo.propertyString2 = value;
        }
    }
    ...
    function getNFTInfos(uint[] memory tokenIDs) external view returns (NFTInfo[] memory) {
        NFTInfo[] memory nftInfos = new NFTInfo[](tokenIDs.length);
        for(uint256 x; x < tokenIDs.length; x++) {
            nftInfos[x] = NFTInfos[tokenIDs[x]];
        }
        return nftInfos;
    }    
}
