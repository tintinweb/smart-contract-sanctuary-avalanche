...
contract Villain is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the VILLAIN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Villain", "VILLAIN") {
        // Mints 1 VILLAIN to contract creator for initial pool setup
    ...
    function _getVillainPrice() internal view returns (uint256 _villainPrice) {
        try IOracle(villainOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Villain: failed to fetch VILLAIN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _villainPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_villainPrice >= taxTiersTwaps[tierId]) {
    ...
    function setVillainOracle(address _villainOracle) public onlyOperatorOrTaxOffice {
        require(_villainOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints VILLAIN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of VILLAIN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentVillainPrice = _getVillainPrice();
            currentTaxRate = _updateTaxRate(currentVillainPrice);
            if (currentVillainPrice < burnThreshold) {
                burnTax = true;
            }
        }
