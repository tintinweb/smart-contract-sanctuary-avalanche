...
contract StakedLPToken0212 is IUniswapV2Pair, PoolToken {
	...
	function _getReward() internal returns (uint256) {
		...
		if (rewarder != address(0)) {
			(address[] memory rewardTokens,) = IRewarder(rewarder).pendingTokens(pid, address(this), 0);
			IMasterChef0212(masterChef).harvest(pid, address(this));
			for (uint i = 0; i < rewardTokens.length; i++) {
				uint256 myBalance = rewardTokens[i].myBalance();
				if (myBalance > 0) {
					// Convert rewardTokens[i] to rewardsToken
					if (rewardTokens[i] == rewardsToken) continue;
					if (rewardTokens[i] != WETH) swapExactTokensForTokens(rewardTokens[i], WETH, myBalance);
...
contract StakedLPTokenFactory0212 {
	address public router;
	address public masterChef;
	address public rewardsToken;
	address public WETH;
