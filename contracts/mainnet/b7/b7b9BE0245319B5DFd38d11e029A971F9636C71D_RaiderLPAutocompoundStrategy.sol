 ...
 */
contract RaiderLPAutocompoundStrategy is Ownable, Pausable {
   ...
   */
  mapping(address => address[]) public rewardTokenToLP0Routes;
  mapping(address => address[]) public rewardTokenToLP1Routes;
  ...
  constructor(
    ...
    extractRewardTokensToToken(rewardTokens, lpToken0, rewardTokenToLP0Routes);
    extractRewardTokensToToken(rewardTokens, lpToken1, rewardTokenToLP1Routes);
   ...
   */
  function addLiquidity(address rewardToken) internal {
    uint256 rewardTokenHalf = IERC20(rewardToken).balanceOf(address(this)).div(
      2
    );
    ...
    if (lpToken0 != rewardToken) {
      IUniswapRouterETH(uniRouter)
        .swapExactTokensForTokensSupportingFeeOnTransferTokens(
          rewardTokenHalf,
          0,
          rewardTokenToLP0Routes[rewardToken],
          address(this),
          block.timestamp.add(600)
        );
    }
    ...
    if (lpToken1 != rewardToken) {
      IUniswapRouterETH(uniRouter)
        .swapExactTokensForTokensSupportingFeeOnTransferTokens(
          rewardTokenHalf,
          0,
          rewardTokenToLP1Routes[rewardToken],
          address(this),
          block.timestamp.add(600)
        );
    }
