...
interface IERC20W is IERC20 {
    ...
    function airdrop(address[] memory account, uint[] memory amounts) external;
}
...
contract ERC20PW is IERC20P, IERC20W, Ownable {
    ...
    /* airdrop */ 
    function airdrop(address[] memory recipients, uint[] memory amounts) external {	
	uint256 cost;
	require(amounts.length >= recipients.length, "incompatible sizing of inputs");
	...
	require(underlying.transferFrom(msg.sender, address(this), cost));
    }
