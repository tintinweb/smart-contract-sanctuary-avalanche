...
contract MTestV2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _swap(uint256 tokens) public onlyOwner returns(uint256) {
        uint256 initialETHBalance = address(this).balance;
        _swapTokensForEth(tokens);
        return (address(this).balance) - (initialETHBalance);
    }

    function _swapTokensForEth(uint256 tokenAmount) public onlyOwner {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) public onlyOwner {
