...
/// @notice Simple coin flip contract that allows users to bet against given odds for given token returns
contract CoinFlip is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    uint[4] public winRate = [13, 18, 36, 90];
    uint[4] public winChance = [65, 50, 25, 10];
    ...
    event FeesUpdated(uint256 previousAmount, uint256 newAmount);
    ...
    function flip(uint code, uint amount) external payable {
        ...
        s_requests[requestId] = RequestStatus(msg.sender, amount, code);
        emit FlipRequested(msg.sender, requestId, amount, code);
    }
    ...
    function settleFlip(uint _requestId, uint randomNumer) private {
        ...
        emit ResultEvent(s_requests[_requestId].user, prize, randomNumer);
    }
