...
interface IVault {
  function amountStaked (address user, uint256 cycle) external returns (uint256);
  function stakedDuringCycle (address user, uint256 cycle) external returns (bool);
}
...
contract Vault is Ownable {
  ...
  function migrate() external {
      ...
      require(IVault(TRUST_V1).amountStaked(msg.sender, 1) > 0, "MIGRATE: You were not staking.");
      require(IVault(TRUST_V1).stakedDuringCycle(msg.sender, 1) == true, "MIGRATE: You were not staking");
      ...
      stakers[msg.sender] = Stake({
        ...
        lastClaimCycle: 1,
        unstakeCycle: 0,
        amount: IVault(TRUST_V1).amountStaked(msg.sender, 1),
        totalRewards: 0
      });
