...
contract Pool is Ownable {
    ...
    function pay(address _to, uint _amount) external onlyOwner returns (bool) {
        return CHESS.transfer(_to, _amount);
    }
}
...
interface PawnManager is IERC721 {
...
contract PawnAid is Ownable {
    PawnManager public manager;
    IERC20 public CHESS;
    CastleAid public castleAid;
    ...
    address payable public marketingWallet = payable(0x348C1c65b2eAf5FcD9c784Aa4c3e13BbAA9B2Ca9);
    address public currentRouter;
    address public TREASURY = 0x32239690806a2caf30CeaF30a7d9dB641c72cA17;
    ...
    IERC20 public CASTLE = IERC20(0x09cb9F1f174ca829d67557B0F8508EF5b9430C83);
    ...
    constructor(address _manager, address _CHESS, address _castle)  {
        manager = PawnManager(_manager);
        castleAid = CastleAid(_castle);
        CHESS = IERC20(_CHESS);
        pool = new Pool(_CHESS);
    ...
    function setNewUpgrade(IERC20 _CASTLE) external onlyOwner {
        CASTLE = IERC20(_CASTLE);
    }

    function setNewCastleAid(address _castle) external onlyOwner {
        castleAid = CastleAid(_castle);
    }

    function approveTokenOnRouter() external onlyOwner{
        CHESS.approve(currentRouter, type(uint256).max);
    }

    function updatePoolAddress(address _pool) external onlyOwner {
        pool.pay(address(owner()), CHESS.balanceOf(address(pool)));
        pool = new Pool(_pool);
    }

    function updateManager(PawnManager _newManager) external onlyOwner {
        manager = PawnManager(_newManager);
    }
    ...
    function updateMaxPawnTX(uint256 _maxPawnTX) external onlyOwner {
        maxPawnTX = _maxPawnTX;
    }
   ...
   function setNodeRatios(uint16 _poolFee, uint16 _liquidityFee) external onlyOwner {
    ...
    function tokenApprovals() external onlyOwner {
        CHESS.approve(address(dexRouter), 2000000 * 10^18);
    }
    ...
    function contractSwap(uint256 numTokensToSwap) internal {
        ...
        if(amountToRewardsPool > 0) {
            CHESS.transfer(address(pool), amountToRewardsPool);
        }
    ...
    function createNodeWithTokens(string memory name) public {
        require(bytes(name).length > 0 && bytes(name).length < 33, "AID: name size is invalid");
        ...
        require(CHESS.balanceOf(sender) >= nodePrice, "AID: Balance too low for creation.");
        require(manager.balanceOf(sender) + 1 < maxWallet, "AID: Exceeds max wallet amount");
        CHESS.transferFrom(_msgSender(), address(this),  nodePrice);
        manager.createNode(sender, name);        
    }

    function createMultipleNodeWithTokens(string memory name, uint amount) public {
        require(amount <= maxPawnTX, "AID: Exceeds max transaction amount");
        require(bytes(name).length > 0 && bytes(name).length < 33, "AID: name size is invalid");
        address sender = _msgSender();
        require(sender != address(0), "AID:  Creation from the zero address");
        uint256 nodePrice = manager.price(sender) * 10 ** 18;
        require(CHESS.balanceOf(sender) >= nodePrice * amount, "AID: Balance too low for creation.");
        require(manager.balanceOf(sender) + amount < maxWallet, "AID: Exceeds max wallet amount");
        CHESS.transferFrom(_msgSender(), address(this),  nodePrice * amount);

        for (uint256 i = 0; i < amount; i++) {
            manager.createNode(sender, name);   
        }

        if ((CHESS.balanceOf(address(this)) > swapThreshold)) {
            uint256 contractTokenBalance = CHESS.balanceOf(address(this));
    ...
    function createMultipleNodeWithTokensAndName(string[] memory names, uint amount) public {
        require(amount <= maxPawnTX, "AID: Exceeds max transaction amount");
        require(names.length == amount, "AID: You need to provide exactly matching names");
        address sender = _msgSender();
        require(sender != address(0), "AID:  creation from the zero address");
        uint256 nodePrice = manager.price(sender) * 10 ** 18;
        require(CHESS.balanceOf(sender) >= nodePrice * amount, "AID: Balance too low for creation.");
    ...
    function createNodesWithRewards(string memory name, uint64[] calldata _nodes, uint256 amount, bool useBalance) public {
        ...
        address sender = _msgSender();
        require(sender != address(0), "AID: creation from the zero address");
        require(blacklist[sender] == false, "AID: You are blacklisted");
        ...
        uint256 nodePrice = manager.price(sender) * 10 ** 18;
        if (!useBalance) require(rewardAmount >= nodePrice * amount,"AID: You don't have enough reward to cash out");
        else require(useBalance && rewardAmount + CHESS.balanceOf(sender) >= nodePrice * amount,"AID: You don't have enough reward to cash out");
        ...
        if (rewardAmount > nodePrice * amount) {
            ...
            uint256 feeAmount = remainingRewards.mul(getClaimFee(sender)).div(100);
        ...
        } else if (useBalance && rewardAmount + CHESS.balanceOf(sender) >= nodePrice * amount) {
            uint256 balanceToTransfer = nodePrice * amount - rewardAmount;
            CHESS.transferFrom(_msgSender(), address(this),  balanceToTransfer);
            if ((CHESS.balanceOf(address(this)) > swapThreshold)) {
                uint256 contractTokenBalance = CHESS.balanceOf(address(this));
                contractSwap(contractTokenBalance);
            }
    ...
    function getClaimFee (address sender) public view returns (uint256) {
        uint256 claimFee;
        if(CASTLE.balanceOf(sender) >= baseFee) {
            return 0;
        }        
        else {        
            claimFee = baseFee.sub(CASTLE.balanceOf(sender));
            return claimFee;
        }
    }
    ...
    function claimCastles(uint64[] calldata _nodes) public {
        address sender = _msgSender();
        require(sender != address(0), "AID: creation from the zero address");
        require(blacklist[sender] == false, "AID: You are blacklisted");
        uint256 rewardAmount = 0;
        for (uint256 i = 0; i < _nodes.length; i++) {
            rewardAmount = rewardAmount + castleAid.claim(_msgSender(), _nodes[i]);
        }
        require(rewardAmount > 0,"AID: You don't have enough reward to cash out");
        uint256 feeAmount = rewardAmount.mul(getClaimFee(sender)).div(100);
        require(feeAmount > 0, "Aid: Error");
