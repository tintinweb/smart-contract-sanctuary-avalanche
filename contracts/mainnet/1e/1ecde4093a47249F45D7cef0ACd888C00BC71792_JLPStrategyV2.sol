...
contract LPStrategy is BaseStrategy {
    ...
    address public feeCollector;
    address public feeRewardCollector = 0x851E2d054b0761f39D7be904325d3c0a6dd32983;
...
contract JLPStrategy is BaseStrategy {
    ...
    function swapToLP(uint256 amountOutMin) public onlyExecutor returns (uint256 amountOut) {
        ...
        if (balanceOfWAVAX >= minWAVAX) {
            ...
            _swapTokens(pairInputToken, path[0]);
...
contract JLPStrategyV2 is BaseStrategy {
    using SafeERC20 for IERC20;

    event LpMinted(uint256 total, uint256 feeRewardAmount, uint256 feeAmount, uint256 feeSpeedAmount);
    ...
    ISushiSwap private immutable router;
    IMasterChefV3 private immutable masterchef;
    uint256 private immutable pid;
    ICauldronV2 public cauldronV2;
    ...
    address private immutable usdtJOEUSDCLPOracle = 0x114297945bE62115BEA387eF6aB1b7747886BAD7;
    address private immutable usdtNORTHOracle = 0x7cffdBFAB3a65BCCb2FB8ef3E4b6eB09ec2554e4;
    address private immutable usdtWAVAXOracle = 0x9AC9CBFA47D13fa2bA21F5aAb4CAD68A44300AC7;
    ...
    function swapToLP(uint256 amountOutMin) public onlyExecutor returns (uint256 amountOut) {
        ...
        if (balanceOfWAVAX >= minWAVAX) {
            uint256 amountOutNorth = _swapToNorth((balanceOfWAVAX * (FEE + FEEReward)) / 1000);
            ...
            uint256 tokenInAmount = (IERC20(path[0]).balanceOf(address(this)) * FEERepeat) / (1000 - FEE - FEEReward);
            ...
            _swapTokens(path[0], path[1]);

            uint256 feeSpeedAmount = IERC20(path[1]).balanceOf(address(this));
            if (feeSpeedAmount > 0) {
                veJoeStaking.deposit(feeSpeedAmount);
            }

            emit LpMinted(amountOut, feeRewardAmount, feeAmount, feeSpeedAmount);
        }
    }
    ...
    function boost(uint256 amount) public onlyExecutor {
        veJoeStaking.deposit(amount);
    }

    function unboost(uint256 amount) public onlyExecutor {
        veJoeStaking.withdraw(amount);
    }

    function boostClaim() public onlyExecutor {
        veJoeStaking.claim();
    }

    function firstHarvest() public onlyExecutor {
        IERC20(rewardToken).safeApprove(address(veJoeStaking), 0);
        IERC20(rewardToken).safeApprove(address(veJoeStaking), type(uint256).max);
        uint256 feeSpeedAmount = IERC20(rewardToken).balanceOf(address(this));
        if (feeSpeedAmount > 0) {
            veJoeStaking.deposit(feeSpeedAmount);
        }
        uint256 max = 999999999999999999999999000000000000000000;
    ...
    function baseApr() public view returns (uint256 joeDay, uint256 ratio) {
        uint256 joeToUSD = (1e36 / IOracle(usdtJOEOracle).peekSpot(new bytes(0)));
        uint256 lpToUSD = (1e36 / IOracle(usdtJOEUSDCLPOracle).peekSpot(new bytes(0)));

        uint256 allocPoint = masterchef.poolInfo(pid).allocPoint;
        uint256 veJoeShareBp = masterchef.poolInfo(pid).veJoeShareBp;
        uint256 totalFactor = masterchef.poolInfo(pid).totalFactor;
        uint256 totalLpSupply = masterchef.poolInfo(pid).totalLpSupply;
        ...
        (uint256 amount, , uint256 factor) = masterchef.userInfo(pid, address(this));
    ...
    function apr() public view returns (uint256 finalApr, uint256 speedApr) {
        (uint256 joeDay, uint256 ratio) = baseApr();
        uint256 veJoeShareBp = masterchef.poolInfo(pid).veJoeShareBp;
        uint64 percent = IBentoBoxMinimal(bentoBox).strategyData(IMasterChefV3(address(masterchef)).poolInfo(pid).lpToken).targetPercentage;
        finalApr = (joeDay * percent) / 100;
    ...
    function transferReward(address _newStrategy) public onlyOwner {
        if (IERC20(rewardToken).balanceOf(address(this)) > 0) {
            _swapTokens(rewardToken, pairInputToken);
        }
        uint256 balanceOfWAVAX = IERC20(pairInputToken).balanceOf(address(this));
        if (balanceOfWAVAX > 0) {
            IERC20(pairInputToken).safeTransfer(_newStrategy, balanceOfWAVAX);
        }
        (uint256 balanceOfStake, , , ) = veJoeStaking.userInfos(address(this));
        if (balanceOfStake > 0) {
            veJoeStaking.withdraw(balanceOfStake);
            uint256 balanceOfrewardToken = IERC20(rewardToken).balanceOf(address(this));
            if (balanceOfrewardToken > 0) {
                IERC20(rewardToken).safeTransfer(address(_newStrategy), balanceOfrewardToken);
            }
        }
    }
