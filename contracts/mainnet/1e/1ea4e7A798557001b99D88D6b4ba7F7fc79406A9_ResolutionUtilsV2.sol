...
interface ResolverInterface {
  ...
  function resolve(uint256 datasetId, uint256 hash, string memory key) external view returns (string memory data);
}
...
interface ResolverRegistryInterface {
  function get(uint256 name, uint256 hash) external view returns (address resolver, uint256 datasetId);
}
...
import "../ContractRegistryInterface.sol";
import "../reverse-resolvers/EVMReverseResolverInterface.sol";
import "../RainbowTableInterface.sol";
import "../ReverseResolverRegistryInterface.sol";
import "../ResolverRegistryInterface.sol";
import "../NameHashInterface.sol";
import "../ResolverInterface.sol";
import "../Domain.sol";


contract ResolutionUtilsV2 {
  ...
  function reverseResolveEVMToName(address addy) external view returns (string memory preimage) {
    ReverseResolverRegistryInterface registry = ReverseResolverRegistryInterface(contractRegistry.get('ReverseResolverRegistry'));
    EVMReverseResolverInterface reverseResolver = EVMReverseResolverInterface(registry.getResolver(3));
    RainbowTableInterface rainbowTable = RainbowTableInterface(contractRegistry.get('RainbowTable'));
    NameHashInterface nameHash = NameHashInterface(contractRegistry.get('NameHash'));
    try reverseResolver.get(addy) returns (uint256 /* name */, uint256 hash) {
      try rainbowTable.lookup(hash) returns (uint256[] memory _preimage) {
        return nameHash.inputSignalsToString(_preimage);
      } catch {
        // do nothing
      }
    } catch {
  ...
  function getRootName(string calldata name) internal pure returns (bool set, string memory rootName) {
    bytes1 period = 0x2e;
    uint periodCount = 0;
    uint lastPeriodIndex = 0;
    uint secondLastPeriodIndex = 0;
    ...
    // we verify the character set first. this is primarily to
    // prevent integrators from forgetting to lowercase domain
    // names, which may lead to failures which are difficult to
    // diagnose.
    for (uint i = 0; i < nameBytes.length; i += 1) {
      if (nameBytes[i] == period) {
        periodCount += 1;
        secondLastPeriodIndex = lastPeriodIndex;
        require(i != 0, "ResolutionUtilsV2: Name contains empty label");
        require(i - lastPeriodIndex > 1, "ResolutionUtilsV2: Name contains empty label");
        lastPeriodIndex = i;
      }

      if (periodCount < 1) {
        currentChar = uint8(nameBytes[i]);
        isNum = (currentChar >= 48 && currentChar <= 57);
        isLetter = (currentChar >= 97 && currentChar <= 122);
        isHyphen = currentChar == 45;
        if (i == 0 || i == nameBytes.length - 1) {
          require(isNum || isLetter, "ResolutionUtilsV2: Invalid character set");
        } else {
          require(isNum || isLetter || isHyphen, "ResolutionUtilsV2: Invalid character set");
        }
      }
    }

    // if we have more than one period then we have a
    // subdomain.
    if (periodCount > 1) {
      rootName = name[secondLastPeriodIndex+1:];
      set = true;
    } else {
      rootName = name;
      set = false;
  ...
  function getResolutionDetails(string calldata name) internal view returns (
    uint256 hash, 
    ...
    NameHashInterface nameHash = NameHashInterface(contractRegistry.get('NameHash'));
    ResolverRegistryInterface resolverRegistry = ResolverRegistryInterface(contractRegistry.get('ResolverRegistry'));
    Domain domain = Domain(contractRegistry.get('Domain'));
    hash = nameHash.hashString(name);
    uint256 rootHash;
    (bool set, string memory rootName) = getRootName(name);
    if (!set) {
      rootHash = hash;
    } else {
      rootHash = nameHash.hashString(rootName);
    }

    if (
      domain.getDomainExpiry(rootHash) <= block.timestamp
      || domain.isSuspended(rootHash)
    ) {
      isSuccess = false;
    } else {
      try resolverRegistry.get(rootHash, hash) returns (address _resolverAddress, uint256 _datasetId) {
        isSuccess = true;
  ...
  function resolve(string calldata name, string memory key) external view returns (string memory value) {
    (uint256 hash, address resolverAddress, uint256 datasetId, bool isSuccess) = getResolutionDetails(name);
    if (!isSuccess) {
      return '';
    }
    ResolverInterface resolver = ResolverInterface(resolverAddress);
    try resolver.resolve(datasetId, hash, key) returns (string memory _value) {
      value = _value;
    } catch {
      value = '';
    }
  ...
  function resolveStandard(string calldata name, uint256 key) external view returns (string memory value) {
    (uint256 hash, address resolverAddress, uint256 datasetId, bool isSuccess) = getResolutionDetails(name);
    if (!isSuccess) {
      return '';
    }
    ResolverInterface resolver = ResolverInterface(resolverAddress);
    try resolver.resolveStandard(datasetId, hash, key) returns (string memory _value) {
      value = _value;
    } catch {
      value = '';
    }
