...
contract Treasury {
	...
	function notifyFeeReceived(
		...
		address snowRewards = IRouter(router).getSnowRewards(currency);

		// Send poolShare to pool-currency rewards contract
		uint256 poolRewardShare = IRouter(router).getPoolRewardsShare(currency) * amount / 10**4;
		_transferOut(currency, poolRewards, poolRewardShare);
		IRewards(poolRewards).notifyRewardReceived(poolRewardShare);

		uint256 poolShare = IRouter(router).getPoolShare(currency) * amount / 10**4;
		_transferOut(currency, pool, poolShare);

		if (snowRewards != address(0)) {
			// Send snowPoolShare to snow-currency rewards contract
			uint256 snowRewardShare = IRouter(router).getSnowShare(currency) * amount / 10**4;
			_transferOut(currency, snowRewards, snowRewardShare);
			IRewards(snowRewards).notifyRewardReceived(snowRewardShare);
		}
