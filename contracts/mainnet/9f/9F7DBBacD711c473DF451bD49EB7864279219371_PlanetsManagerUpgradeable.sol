    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 1; i <= _planetCounter.current(); i++)
            if (_planets[i].exists) Ids[len++] = _planets[i].id;
        return Ids;
    }
    ...
    function getPlanetRewardMults() 
        ...
        returns (uint256[] memory)
    {
        uint256[] memory rwMults;
        uint256 len = 0;
        for (uint256 i = 1; i <= _planetCounter.current(); i++)
            if (_planets[i].exists) rwMults[len++] = _planets[i].rewardMult;
        return rwMults;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 len = 0;
        for (uint256 i = 1; i <= _planetCounter.current(); i++)
            if (_planets[i].exists) values[len++] = _planets[i].planetValue;
        return values;
    }
