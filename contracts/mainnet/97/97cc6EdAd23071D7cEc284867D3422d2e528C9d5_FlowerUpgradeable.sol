...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    function initialize(
        ...
        // Router
        router = IMiniRouter(_routerAddress);
        ...
        compoundUplinePercNCTR = 45;
        compoundUplinePercUSDCe = 5;
     ...
     * @param _amountToSwap - Half the deposited amount of tokens
    ...
    ) private noZeroAddress(_tokenA) noZeroAddress(_tokenB) returns (uint256) {
    ...
    /**
     * @dev - Updates users APR. If user is team wallet or it has upline that is team wallet it gets 1% APR and 0.5% otherwise
     * @param _user - User which APR needs to be updated
     */
    function _updateAPR(address _user) private {
        if (
            userDownlines[_user].length >= MIN_NUM_OF_REF_FOR_TEAM_WALLET ||
            userDownlines[users[_user].upline].length >=
    ...
    function getDepositedValue(address _user)
        ...
        returns (uint256)
    {
        return airdrops[_user].airdropsReceived + users[_user].depositsNCTR;
    }

    /**
     * @dev - Set user's USDC.e wallet address
     * @param _walletUSDCe - Address of the wallet to where USDC.e token will be sent
     */
    function setUSDCeWallet(address _walletUSDCe)
        external
        noZeroAddress(_walletUSDCe)
    {
        users[msg.sender].walletUSDCe = _walletUSDCe;
    }
        ...
        onlyAmountGreaterThanZero(_amountUSDCe)
    {
        ...
        USDCeToken.approve(address(router), amountUSDCe);
        uint256 amountNCTR = _routerSwap(
            address(USDCeToken),
            address(nectarToken),
            amountUSDCe
        );
        ...
        users[msg.sender].depositsNCTR += realizedDepositNCTR;
        users[msg.sender].depositsUSDCe += realizedDepositUSDCe;
        users[msg.sender].lastDepositTime = block.timestamp;

        emit Deposit(msg.sender, realizedDepositNCTR, address(nectarToken));
        emit Deposit(msg.sender, realizedDepositUSDCe, address(USDCeToken));

        // Update stats
        totalDepositedNCTR += realizedDepositNCTR;
        totalDepositedUSDCe += realizedDepositUSDCe;

        // Reward an upline if it's eligible
        if (_getRewardEligibility(_upline, 0)) {
            // Check if _upline is Team wallet. If true, give 25% of the _upline's reward to downline
            if (
                userDownlines[_upline].length >= MIN_NUM_OF_REF_FOR_TEAM_WALLET
            ) {
                ...
                uplineRewardNCTR -= downlineRewardNCTR;
                emit TeamReward(
                    _upline,
                    msg.sender,
                    downlineRewardNCTR,
        ...
        nectarToken.burnNectar(address(this), burnAmountNCTR);
     ...
     * @param _compoundAmount - NCTR amount to be compounded
     */
    function compoundRewards(uint256 _compoundAmount)
        external
        onlyBloomReferralNode
        onlyAmountGreaterThanZero(_compoundAmount)
    {
        require(
            users[msg.sender].depositsNCTR >= _compoundAmount,
        ...
        for (; downlineDepth < NUM_OF_TIERS; downlineDepth++) {
            ...
            if (
                ...
                nectarToken.approve(address(router), forSwapNCTR);
                uint256 forUplineWalletUSDCe = _routerSwap(
                    address(nectarToken),
                    address(USDCeToken),
                    forSwapNCTR
                );
    ...
    function claim() external onlyBloomReferralNode {
        require(
            getDepositedValue(msg.sender) + airdrops[msg.sender].airdropsGiven >
                users[msg.sender].dailyClaimAmount + users[msg.sender].payouts,
        ...
        require(
            users[msg.sender].payouts + users[msg.sender].dailyClaimAmount <=
                maxClaim,
        ...
        if (treasuryBalance < users[msg.sender].dailyClaimAmount) {
            uint256 differenceToMint = users[msg.sender].dailyClaimAmount -
                treasuryBalance;
            nectarToken.mintNectar(address(treasury), differenceToMint);
        }

        (uint256 realizedClaimNCTR, ) = _calculateTax(
            users[msg.sender].dailyClaimAmount,
            claimTax
        );
        ...
        totalWithdraw += users[msg.sender].dailyClaimAmount;
        users[msg.sender].payouts += users[msg.sender].dailyClaimAmount;

        emit Claim(msg.sender, users[msg.sender].dailyClaimAmount);
