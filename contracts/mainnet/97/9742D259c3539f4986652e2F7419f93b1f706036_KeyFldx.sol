...
interface ISolidlyBoostedStrategy {
...
contract KeyFldx is KolSolidlyManager, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    function initialize() public initializer {
        ...
        __ERC20_init_unchained("keyFLDX KeyofLife.fi", "keyFLDX");
        __SolidlyStaker_init(
            0x3a0bA70ca11617F3Ec06c0d99185095477AFF4d4, // _solidVoter
            0x869326C3B88b46df37e94316234e599eC5950A83, // _veDist
            ...
            0x107D2b7C619202D994a4d044c762Dd6F8e0c5326  // want = FLDX
        );
    }
    ...
    function depositForStrategyByOwner(address _gauge, uint256 _amount) external onlyOwner {
        ...
        IERC20Upgradeable _underlying = ISolidlyBoostedStrategy(_strategy).want();
        _underlying.safeTransferFrom(msg.sender, address(this), _amount);
    }

    function depositForStrategyByOwner2(address _gauge, uint256 _amount) external onlyOwner {
        address _strategy = whitelistedStrategy[_gauge];
        IERC20Upgradeable _underlying = ISolidlyBoostedStrategy(_strategy).want();
        _underlying.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function depositForStrategy(address _gauge, uint256 _amount) external onlyWhitelist(_gauge) {
        IERC20Upgradeable _underlying = ISolidlyBoostedStrategy(msg.sender).want();
        _underlying.safeTransferFrom(msg.sender, address(this), _amount);
        IGauge(_gauge).deposit(_amount, veTokenId);
    }
    ...
    function getRewardsForStrategy(address _gauge, address[] calldata tokens) external onlyWhitelist(_gauge) {
    ...
    function balanceOfSolidlyTokenInVe() public view returns (uint256 sliz) {
    ...
    function whitelistStrategy(address _strategy) external onlyManager {
        IERC20Upgradeable _want = ISolidlyBoostedStrategy(_strategy).want();
        address _gauge = ISolidlyBoostedStrategy(_strategy).gauge();
