...
contract Nft is Context, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function mintFor(address[] memory userAddresses) public onlyOwner {
        for (uint256 i = 0; i < userAddresses.length; i++) {
            _mint(userAddresses[i], currentTokenId.current() + 1);
            currentTokenId.increment();
        }
    }

    function mint(uint256 amount) public payable virtual onlyMintingEnabled {
        require(
    ...
    function setFreeMintsAddresses(
        address[] memory userAddresses,
        uint256 count
    ) public onlyOwner {
        for (uint256 i = 0; i < userAddresses.length; i++) {
            freeMints[userAddresses[i]] = count;
        }
    }

    function airdrop(uint256[] memory tokenIds, address[] memory userAddresses)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < userAddresses.length; i++) {
            this.transferFrom(msg.sender, userAddresses[i], tokenIds[i]);
        }
    }
