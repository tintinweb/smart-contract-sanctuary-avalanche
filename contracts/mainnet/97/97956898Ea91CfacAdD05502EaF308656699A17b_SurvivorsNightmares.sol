...
contract SurvivorsNightmares is ERC721  {
    ...
    constructor()
        ERC721("Survivors: Nightmares", "SUNT")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 77) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeia5433kzq43quvghy72n32i5dl2v4lqu3camg4jgwwk5sku37lgom/", Strings.toString(tokenId), ".json")
            );
    }
