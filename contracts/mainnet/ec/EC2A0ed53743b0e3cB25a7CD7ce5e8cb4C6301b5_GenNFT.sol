...
contract GenNFT is ERC721Permit, ERC721Rentable, ERC721URIStorage, IGenesisConvert, Withdrawable, ReentrancyGuard, Verifier {
    ...
    ///@dev value is equal to keccak256("Mint(address user,address creator,Metadata[] meta,uint256 deadline)Metadata(address creator,uint48 typeNFT,uint48 rarity)")
    bytes32 private constant _MINT_TYPE_HASH = 0xbc3d5d827e8ffc3db1efd1fb097711708e2f6b6c5adbb2f9c15b9bda0a116641;
    ...
    ) payable ERC721Rentable(name_, symbol_) Signable(type(GenNFT).name, "1") {
    ...
    function _convert(
        ...
        (, uint256 typeNFT) = IGenesisNFT(genesisNFT_).metadata(tokenId_);
        (, , , uint256 unboxQuantity) = IGenesisNFT(genesisNFT_).getTypeNFT(typeNFT);
