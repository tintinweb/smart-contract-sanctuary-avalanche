...
contract traderjoeSwap is proxyOwner,timeLockSetting {
    ...
    function swapToken_exactOut(address token0,address token1,uint256 amountMaxIn,uint256 amountOut,address to) public returns (uint256){
        ...
        (bool next,uint256 amountIn) = tokensInput(token0,token1,amountsIn[0],to);
        if(!next){
            return amountIn;
        }
        uint256 amount = swapTokenToToken(token0,token1,amountIn,amountOut,to);
    ...
    function getPriceImpact(address token0,address token1,uint256 amountIn)external view returns (uint256){
        IUniswapV2Factory factory = IUniswapV2Factory(swapRouter.factory());
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(token0,token1));
        (uint256 reserve0, uint256 reserve1,) = pair.getReserves();
        uint256 reserveIn = pair.token0() == token0 ? reserve0 : reserve1;
        uint256 amountInWithFee = amountIn * 997e15;
        return amountInWithFee / (reserveIn.add(amountInWithFee/1e18));
    }
