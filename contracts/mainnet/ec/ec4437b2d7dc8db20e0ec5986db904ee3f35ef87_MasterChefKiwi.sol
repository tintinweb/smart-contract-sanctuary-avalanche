...
contract KiwiToken is ERC20("KiwiToken", "KIWI"), Ownable {
    /// @notice Total number of tokens
    uint256 public maxSupply = 500_000_000e18; // 500 million Kiwi

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterKiwi).
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            totalSupply().add(_amount) <= maxSupply,
            "KIWI::mint: cannot exceed max supply"
        );
        _mint(_to, _amount);
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "KIWI::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "KIWI::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "KIWI::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "KIWI::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// File: contracts/MasterChefKiwiV2.sol
...
interface IRewarder {
    ...
    function onKiwiReward(address user, uint256 newLpAmount) external;
...
// MasterChefKiwi is a boss. He says "go f your blocks lego boy, I'm gonna use timestamp instead".
...
contract MasterChefKiwi is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accKiwiPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. KIWIs to distribute per second.
        uint256 lastRewardTimestamp; // Last timestamp that KIWIs distribution occurs.
    ...
    // KIWI tokens created per second.
    uint256 public kiwiPerSec;
    ...
    // The timestamp when KIWI mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 _kiwiPerSec);
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 lpPercent = 1000 -
                ...
                treasuryPercent -
                investorPercent;
            uint256 kiwiReward = multiplier
            .mul(kiwiPerSec)
            .mul(pool.allocPoint)
            .div(totalAllocPoint)
            .mul(lpPercent)
            .div(1000);
            accKiwiPerShare = accKiwiPerShare.add(
                kiwiReward.mul(1e12).div(lpSupply)
            );
        }
        pendingKiwi = user.amount.mul(accKiwiPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 kiwiReward = multiplier.mul(kiwiPerSec).mul(pool.allocPoint).div(
            totalAllocPoint
        );
        uint256 lpPercent = 1000 -
            devPercent -
            treasuryPercent -
            investorPercent;
        kiwi.mint(devAddr, kiwiReward.mul(devPercent).div(1000));
        kiwi.mint(treasuryAddr, kiwiReward.mul(treasuryPercent).div(1000));
        kiwi.mint(investorAddr, kiwiReward.mul(investorPercent).div(1000));
        kiwi.mint(address(this), kiwiReward.mul(lpPercent).div(1000));
        pool.accKiwiPerShare = pool.accKiwiPerShare.add(
            kiwiReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000)
        );
        pool.lastRewardTimestamp = block.timestamp;
        emit UpdatePool(
            _pid,
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (address(rewarder) != address(0)) {
            rewarder.onKiwiReward(msg.sender, user.amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // Harvest KIWI
        uint256 pending = user.amount.mul(pool.accKiwiPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeKiwiTransfer(msg.sender, pending);
        emit Harvest(msg.sender, _pid, pending);
    ...
    function updateEmissionRate(uint256 _kiwiPerSec) public onlyOwner {
        massUpdatePools();
        kiwiPerSec = _kiwiPerSec;
        emit UpdateEmissionRate(msg.sender, _kiwiPerSec);
    }
}