...
contract ParaSwapLiquiditySwapAdapter is BaseParaSwapSellAdapter, ReentrancyGuard {
  ...
  function swapAndDeposit(
    ...
    IERC20WithPermit viToken =
      IERC20WithPermit(_getReserveData(address(assetToSwapFrom)).viTokenAddress);
    ...
    )
  {
    ...
    for (vars.i = 0; vars.i < reservesCount; vars.i++) {
      ...
      if (userConfig.isBorrowing(vars.i)) {
        vars.compoundedBorrowBalance = IERC20(currentReserve.stableVdTokenAddress).balanceOf(
          user
        );
...
library ValidationLogic {
  ...
  function validateRebalanceStableBorrowRate(
    ...
    uint256 totalDebt = stableVdToken
      .totalSupply()
      .add(variableVdToken.totalSupply())
      .wadToRay();
...
contract ViniumProtocolDataProvider {
    ...
    )
  {
    ...
    stableRateLastUpdated = IStableVdToken(reserve.stableVdTokenAddress).getUserLastUpdated(
      user
    );
    ...
    )
  {
    ...
    return (
      ...
      reserve.variableVdTokenAddress
    );
  }
}
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      if (user != address(0)) {
        ...
        if (userConfig.isBorrowing(i)) {
          ...
          userReservesData[i].principalStableDebt = IStableVdToken(
            reserveData.stableVdTokenAddress
          ).principalBalanceOf(user);
          if (userReservesData[i].principalStableDebt != 0) {
            userReservesData[i].stableBorrowRate = IStableVdToken(
              reserveData.stableVdTokenAddress
            ).getUserStableRate(user);
            userReservesData[i].stableBorrowLastUpdateTimestamp = IStableVdToken(
              reserveData.stableVdTokenAddress
...
contract UniswapRepayAdapter is BaseUniswapAdapter {
  ...
  function swapAndRepay(
    ...
    address vdToken = DataTypes.InterestRateMode(debtRateMode) ==
