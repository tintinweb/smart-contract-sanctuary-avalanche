...
// File contracts/distribution/SkullGenesisRewardPool.sol
...
contract SkullGenesisRewardPool {
    using SafeMath for uint256;
    ...
    // TESTNET
    //uint256 public skullPerSecond = 0.66667 ether; // 2400 SKULL / (1h * 60min * 60s)
    //uint256 public runningTime = 1 hours; // 1 hours
    //uint256 public constant TOTAL_REWARDS = 2400 ether;
    // END TESTNET

    // MAINNET
    uint256 public skullPerSecond = 0.30092 ether; // 26000 SKULL / (24h * 60min * 60s)
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accSkullPerShare: 0, isStarted: _isStarted, depositFeeBP: _depositFeeBP}));
        if (_isStarted) {
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (SKULL or lps) if less than 90 days after pool ends
            require(_token != skull, "skull");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
