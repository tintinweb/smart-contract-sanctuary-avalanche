/**
 *Submitted for verification at snowtrace.io on 2022-05-30
*/
...
// File: contracts/NCTRPresale.sol

contract NectarPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale NCTR, 0.000000048 USDC
	uint256 TokenPerUSDC = 1 * 10 **18; 
	// Amount of USDC received in presale
	uint256 usdcReceived = 0;

    uint256 HardCap = 500000 * 10 ** 6;

    uint256 public mincontributionLimit = 100 * 10 ** 6;
    uint256 public maxcontributionLimit = 250 * 10 ** 6;
	...
	function getNCTRTokensLeft() external view returns (uint256) {
		return NCTR.balanceOf(address(this));
	}
        ...
        returns (uint256)
    {
        ...
        return maxcontributionLimit.sub(user.amount);
    }
    ...
    function buy(uint256 _amount) public nonReentrant {
        ...
        require(
            Phasewhitelisted[msg.sender] == true || isPublic,
        ...
        require( maxcontributionLimit >= user.amount.add(_amount), "new amount above user limit");
        user.amount = user.amount.add(_amount);
		uint256 tokens = _amount.mul(TokenPerUSDC).div( 10 ** 6);   

		USDC.safeTransferFrom(msg.sender, address(this), _amount);
		usdcReceived = usdcReceived.add(_amount);

		NCTR.safeTransfer(msg.sender, tokens);
	...
	function withdrawUnsoldNCTR() external onlyOwner {
		uint256 amount = NCTR.balanceOf(address(this)) - totalTokensSold;
		NCTR.safeTransfer(msg.sender, amount);
	}

    function AllBurnNCTR() external onlyOwner {
		NCTR.safeTransfer(burnAddress, NCTR.balanceOf(address(this)));
	}
}