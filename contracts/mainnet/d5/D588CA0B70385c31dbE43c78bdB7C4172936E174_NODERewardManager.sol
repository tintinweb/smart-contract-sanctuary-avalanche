...
contract NODERewardManager is PaymentSplitter {
	...
	function addNodeType(
        ...
        _nodeTypes.set(nodeTypeName, IterableNodeTypeMapping.NodeType({
                ...
                nodePrice: nodePrice * 10**18,
                claimTime: claimTime,
                rewardAmount: rewardAmount * 10**18,
	...
	function _calculateNodeReward(NodeEntity memory node) private view returns(uint256) {
		...
		if (nt.rewardAmount * (block.timestamp - node.creationTime) / nt.claimTime < nt.nodePrice) {
			rewards = rewards * (100 - earlyClaimFee) / 100;
		}
		return rewards;
	}
	...
	function levelUp(string[] memory nodeTypeNames, string memory target) public {
		...
		uint256 targetPrice = _nodeTypes.get(target).nodePrice;
	...
	function getNodeTypeFullDataString(string memory nodeTypeName) public view returns (string memory) {
		IterableNodeTypeMapping.NodeType memory nt;
		...
		nt = _nodeTypes.get(nodeTypeName);
		result = string(abi.encodePacked(nt.nodeTypeName, separator, _uint2str(nt.nodePrice)));
		...
		result = string(abi.encodePacked(result, separator, _uint2str(nt.count)));
		result = string(abi.encodePacked(result, separator, _uint2str(nt.max)));
		return result;
	}

	function getNodeTypesIndexes(uint256 indexStart, uint256 indexEnd) 
	public view returns (string memory) {
		...
		uint256 start = indexStart;
		uint256 end = indexEnd;
		uint256 size = _nodeTypes.size();
		if (start >= size) {
			return "";
		} else if (end > size) {
			end = size;
	...
	function blacklistMalicious(address account, bool value) external onlySentry {
	...
	function updateEarlyClaimFee(uint256 value)
        external onlySentry
    {
        earlyClaimFee = value;
    }
