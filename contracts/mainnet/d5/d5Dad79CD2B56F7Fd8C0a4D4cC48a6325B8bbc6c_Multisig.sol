...
interface IMultisig {
    ...
    event ProposalCreated(uint256 timestamp, uint256 indexed id, ProposalType proposalType, address indexed proposer);
...
contract Multisig is IMultisig {
    ...
    uint32 constant DEV_PROPOSAL_DELAY = 120 seconds;
    uint32 constant TEAM_PROPOSAL_DELAY = 3 days;
    uint32 constant COMMUNITY_PROPOSAL_DELAY = 7 days;

    uint32 constant TYPE1_DELAY = 24 hours;
    uint32 constant TYPE2_DELAY = 72 hours;
    uint32 constant TYPE3_DELAY = 1 weeks;
    uint32 constant TYPE4_DELAY = 4 weeks;
    uint32 constant EXPIRATION_DELAY = 5 weeks;

    uint32 constant DEV_KICK_GRACE_DELAY = 180 days;
    uint32 constant TEAM_KICK_GRACE_DELAY = 120 days;
    uint32 constant COMMUNITY_KICK_GRACE_DELAY = 7 days;

    uint256 constant PROPOSAL_PRICE = 1 ether / 10;

    mapping (address => uint256) pingTime;
    ...
    function addRemoveDevMemberProposal(address _a) external payable onlyWithRole {
        ...
        require(pingTime[_a] + DEV_KICK_GRACE_DELAY < block.timestamp, "Dev is active");
        _ping(msg.sender);
    ...
    function addRemoveTeamMemberProposal(address _a) external payable onlyWithRole {
        ...
        require(pingTime[_a] + TEAM_KICK_GRACE_DELAY < block.timestamp, "Team member is active");
        _ping(msg.sender);
    ...
    function addRemoveCommunityMemberProposal(address _a) external payable onlyWithRole {
        ...
        require(pingTime[_a] + COMMUNITY_KICK_GRACE_DELAY < block.timestamp, "Community member is active");
        _ping(msg.sender);
    ...
    function addGenericBatchedProposal(address[] memory _target, string[] memory _fun, uint256[] memory _value, bytes[] memory _data) external payable onlyWithRole {
        for (uint256 i = 0; i < _target.length; i++) {
            require(_target[i] != address(this), "Don't call the multisig like that");
            for (uint256 j = 0; j < bytes(_fun[i]).length; j++) {
                require(bytes(_fun[i])[j] != 0x20, "No spaces in fun please");
            }
        }
        _ping(msg.sender);
        _addBatchedProposal(_target, _fun, _value, _data);
    }
    ...
    function executeOrRevert(uint256 _id) external onlySelf {
        string memory reason;
        for (uint256 i = 0; i < proposals[_id].target.length; i++){
            (bool success, bytes memory retData) = proposals[_id].target[i].call{value: proposals[_id].value[i]}(abi.encodePacked(proposals[_id].signature[i], proposals[_id].data[i]));
            reason = string(abi.encodePacked(reason,"|"));
            if (!success && retData.length >= 68) {
                assembly {
                    // Slice the sighash.
                    retData := add(retData, 0x04)
                }
                reason = string(abi.encodePacked(reason,abi.decode(retData, (string))));
            }
            if (!success) revert(reason);
        }
    }
    ...
    function _removeDev(address _a) internal {
        ...
        require(pingTime[_a] + COMMUNITY_KICK_GRACE_DELAY < block.timestamp, "Dev is active");
        if (devs[_a]) {
            devs[_a] = false;
    ...
    function _addProposal(address _target, string memory _fun, uint256 _value, bytes memory _data) internal {
        ...
        target[0] = _target;
        string[] memory fun = new string[](1);
        fun[0] = _fun;
        uint256[] memory value = new uint256[](1);
        value[0] = _value;
        bytes[] memory data = new bytes[](1);
        data[0] = _data;
        _addBatchedProposal(target, fun, value, data);
    }

    function _addBatchedProposal(address[] memory _target, string[] memory _fun, uint256[] memory _value, bytes[] memory _data) internal {
        require(devs[msg.sender] || team[msg.sender] || community[msg.sender], "Only members can do this");
        require(_target.length == _fun.length && _fun.length == _value.length && _value.length == _data.length, "Params must be the same length");
        proposalCount++;
        Proposal memory tmp;
        ...
        tmp.signature = new bytes4[](_target.length);
        tmp.data = _data;
        ...
        for (uint256 i = 0; i < _fun.length; i++) {
            require(bytes(_fun[i]).length == 0 || bytes(_fun[i]).length >=3, "provide a valid function specification");
            if (bytes(_fun[i]).length != 0) {
                tmp.signature[i] = (bytes4(keccak256(bytes(_fun[i]))));
            } else {
                tmp.signature[i] = "";
            }
        }
        ...
        emit ProposalCreated(uint64(block.timestamp), proposalCount, tmp.proposalType, msg.sender);
    }
    ...
    function _execute(uint256 _id) internal {
        ...
        (bool success, bytes memory retData) = address(this).call{value: 0}(abi.encodeWithSignature("executeOrRevert(uint256)", _id));
