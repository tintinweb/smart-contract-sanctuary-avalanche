...
contract OrderPool is Ownable {
    ...
    address public constant USDt_org = 0xBA2a995Bd4ab9e605454cCEf88169352cd5F75A6;
    ...
    struct Order {
        ...
        uint256 status; //number of execution, 1-open, ~2-executed
    }
    Order[] public orders;
    ...
    function _setAllowances() internal {
        IERC20(USDC).safeApprove(YAK_Router, type(uint256).max);
        IERC20(USDCe).safeApprove(YAK_Router, type(uint256).max);
        IERC20(USDt_org).safeApprove(YAK_Router, type(uint256).max);
        IERC20(DAIe).safeApprove(YAK_Router, type(uint256).max);
        IERC20(WAVAX).safeApprove(YAK_Router, type(uint256).max);
    }

    function setAllowance(address _token) public onlyOwner {
        IERC20(_token).safeApprove(YAK_Router, type(uint256).max);
    }
    ...
    function executeOrder_test(
        ...
        ApexRouter.Trade memory trade = ApexRouter.Trade(amountIn, 0, path, adapters);
    ...
    function swap(
        ...
        ApexRouter.Trade memory trade = ApexRouter.Trade(amountIn, amountOut, path, adapters);
        IRouter(YAK_Router).swapNoSplit(trade, address(this), 0);
    }
    ...
    function callRaw(
        ...
        for (uint256 i = start; i <= end; i += step) {
            uint256 amountout = getBestAmountOut(i, token, token, steps);
            if (amountout > i && (amountout - i) > bestProfit) {
                bestProfit = amountout - i;
                bestAmountIn = i;
            }
        }
        require(bestProfit >= expectedProfit, "Not enough profit");
        swap(bestAmountIn, bestAmountIn + expectedProfit, token, token, steps);
    }
    ...
    function executeOrder(
        ...
        ApexRouter.Trade memory trade = ApexRouter.Trade(
            amountIn,
            orders[index].amountOut, // return should be greater than expected
            path,
            adapters
        );
        IRouter(YAK_Router).swapNoSplit(trade, address(this), 0);
