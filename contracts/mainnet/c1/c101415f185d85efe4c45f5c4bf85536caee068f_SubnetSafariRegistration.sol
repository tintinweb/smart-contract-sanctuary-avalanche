...
contract SubnetSafariRegistration is  Ownable {
    ...
    event PlayerTeamChanged(address player, uint team);
    ...
    function setTeamAdmin(address player, uint team) external onlyOwner {
        uint currentTeam = teams[player];
        teamSizes[currentTeam]-=1;
        teamSizes[team] +=1;
        teams[player] = team;
        emit PlayerTeamChanged(player,team);
    }
    ...
    function register() payable external {
        ...
        uint team = (rng % 4) + 1; // 1,2,3 or 4
        team = balanceTeams(team); // returns another team if its getting unbalanced
        teams[msg.sender] = team;
        emit NewPlayer(msg.sender, team);
    }

    function balanceTeams(uint team) internal returns (uint){
        teamSizes[5]+=1; // increase total size by 1
        uint size = teamSizes[5]; // get total size
        uint appropriateSize =  size /4; // integer division will be 0,1,2...
        if(teamSizes[team] > appropriateSize)  { // if current team size is bigger than the normal size
            uint lastTeam = team + 4; // get the last team, if current team is 2 it will be 6
            for (uint256 i = team; i < lastTeam; ++i) { // start from 2 and go until 6, index will be one higher and moduloed so (3, 4, ,1 , 2)
                uint index = (i % 4) + 1;
