...
import "../../interfaces/traderjoe/IVeJoeStaker.sol";
import "../Common/StratFeeManager.sol";
import "../../utils/GasFeeThrottler.sol";

contract StrategyTraderJoeBoostedLP is StratFeeManager, GasFeeThrottler {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IVeJoeStaker(boostStaker).withdraw(chef, poolId, _amount - wantBal);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (secondOutputToNativeRoute.length != 0) {
            ...
            if (secondBal > 0) {
                IUniswapRouterETH(unirouter).swapExactTokensForTokens(secondBal, 0, secondOutputToNativeRoute, address(this), block.timestamp);
            }
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        if (_outputBal > 0) {
            ...
            _nativeBal = _nativeBal + _amountOut[_amountOut.length -1];
        }
        ...
        if (_secondBal > 0 && outputToNativeRoute.length > 0) {
            ...
            _nativeBal = _nativeBal + _secondAmountOut[_secondAmountOut.length -1];
        }
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(boostStaker, type(uint256).max);
        ...
        if (secondOutputToNativeRoute.length > 0) {
            IERC20(secondOutputToNativeRoute[0]).safeApprove(unirouter, type(uint256).max);
        }
    }
