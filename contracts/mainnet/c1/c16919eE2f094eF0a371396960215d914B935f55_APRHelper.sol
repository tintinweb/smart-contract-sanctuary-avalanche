...
contract APRHelper is Initializable, OwnableUpgradeable {
    ...
    {
        uint256 TVL = getTVL(lp);
        TVL = (TVL == 0) ? 10**8 : TVL;
        (uint256 pendingBasePtp, uint256 pendingBoostedPtp) = getPTPperYearForVector(lp);
        baseAPR = (pendingBasePtp * getTokenPricePairedWithAvax(ptp)) / TVL / 10**8;
        boostedAPR = (pendingBoostedPtp * getTokenPricePairedWithAvax(ptp)) / TVL / 10**8;
    }
