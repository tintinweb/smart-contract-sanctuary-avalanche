...
contract FitfiPool is Ownable {
    ...
    uint256 public constant MAX_TEN_APY     = 600; // 25%
    ...
    function claimSomeShares (uint256 _amount) public {
        require(treasury != address(0), "Treasury is not zero address");
        require(treasury == msg.sender, "This contract can be called only by treasury");
        require(_amount > 0, "Nothing Claim");
        uint256 balance = fitfiToken.balanceOf(address(this));
        require(balance > _amount, "Balance not enough");
        fitfiToken.safeTransfer(msg.sender, _amount);
        totalShares -= _amount;
    }
    ...
    function calculatorReward(address _user) private view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        
        uint256 totalReward_ = 0;
        if (user.lockStartTime != 0) {
            for(uint256 index = 0; index < user.depositInfo.length; index++) {
                DepositInfo memory depositInfo_ = user.depositInfo[index];
                if (depositInfo_.isClaimed == false) {
                    uint256 lockedDuration_ = 0;
                    if (block.timestamp >= depositInfo_.depositedTime + MAX_TEN_PERIOD) {
                        lockedDuration_ = block.timestamp - depositInfo_.depositedTime;
                    }
                    uint256 availableReward_ = depositInfo_.depositAmount * MAX_TEN_APY / (100 * MAX_LOCK_DURATION) * lockedDuration_;
                    totalReward_ += availableReward_;
                }
            }
        }
    ...
    function getRewardAmount () public view returns(uint256) {
        ...
        if (user.lockStartTime != 0) {
            for(uint256 index = 0; index < user.depositInfo.length; index++) {
                DepositInfo memory depositInfo_ = user.depositInfo[index];
                if (depositInfo_.isClaimed == false) {
                    uint256 lockedDuration_ = block.timestamp - depositInfo_.depositedTime;
                    uint256 availableReward_ = depositInfo_.depositAmount * MAX_TEN_APY / (100 * MAX_LOCK_DURATION) * lockedDuration_;
                    totalReward_ += availableReward_;
                }
            }
    ...
    function checkRewardAvailable() public view returns (uint256){
    ...
    function claimReward() public whenNotPaused {
        ...
        if (totalShares < rewardAmount_) {
            paused = true;
            revert("Contract Balance Not Enough.");
        }
        ...
        UserInfo storage user = userInfo[msg.sender];
        for(uint256 index = 0; index < user.depositInfo.length; index++) {
            user.depositInfo[index].isClaimed = true;
        }
    }
    ...
    function withdraw() public whenNotPaused {
        ...
        uint256 withdrawAmount_ = calculatorWithdraw(msg.sender);
        uint256 rewardAmount_ = calculatorReward(msg.sender);
        uint256 amount_ = withdrawAmount_ + rewardAmount_;

        if (totalShares < amount_) {
            paused = true;
            revert("Contract Balance Not Enough.");
        }        
        ...
        totalShares -= amount_;
        delete user.depositInfo;
        user.depositAmount = 0;
        user.lockStartTime = 0;
    }

    function checkWithdrawAvailable() public view returns (bool){
        if (calculatorWithdraw(msg.sender) < userInfo[msg.sender].depositAmount) {
            return false;
        } else {
            return true;
        }
    ...
    function calculatorWithdraw(address _user) private view returns(uint256) {
        UserInfo storage user = userInfo[_user];
        ...
        if (user.lockStartTime != 0) {
            for(uint256 index = 0; index < user.depositInfo.length; index++) {
                ...
                uint256 availableWithdraw_ = depositInfo_.depositAmount * (100 - penultyPercent_) / 100;
                totalWithdraw_ += availableWithdraw_;
            }
        }
