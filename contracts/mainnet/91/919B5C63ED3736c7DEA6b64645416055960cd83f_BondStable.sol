...
contract BondStable is Ownable {
	...
	function deposit(uint256 _amount, uint256 _minPayout, address _depositor) external {
		...
		uint256 priceInEvol = _getPriceInEvol(_amount);
		...
		uint256 contractStableBalance = IERC20(stable).balanceOf(address(this));
		if (swapping && !lock && contractStableBalance > swapAmountIn) {
			lock = true;

			uint256 toLiquify = contractStableBalance * liquifyRate / 10000;

			if (liquify) {
				(, uint256 excessStable) = swapAndLiquify(toLiquify);
				IERC20(stable).safeTransfer(payee, contractStableBalance - toLiquify + excessStable);
			} else {
				swapAndBurn(toLiquify);
				IERC20(stable).safeTransfer(payee, contractStableBalance - toLiquify);
			}
		...
		IMinter(minter).mintRewards(_amount, payout);
		IERC20(stable).safeTransferFrom(msg.sender, address(this), _amount);
	...
	function _getPriceInEvol(uint256 _amount) internal view returns (uint256) {
		address[] memory path = new address[](2);
		path[0] = stable;
		path[1] = evol;
		return IRouter(router).getAmountsOut(_amount, path)[1];
	}
