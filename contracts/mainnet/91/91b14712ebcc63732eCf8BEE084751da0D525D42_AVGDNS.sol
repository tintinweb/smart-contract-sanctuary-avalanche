...
contract AVGDNS is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("AVGDNS", "AVGDNS") {}
    ...
    function purchaseWithERC20(address _payTokenAddress, uint256 _numberOfTokens) external nonReentrant {
        ...
        require(isWhitelisted(msg.sender) || publicCosts.contains(_payTokenAddress), "Address is not whitelisted");
        require(publicCosts.contains(_payTokenAddress), "Token is not supported");

        TokenInfo memory tokenInfo = payTokens[_payTokenAddress];
        uint256 cost = isWhitelisted(msg.sender) ? tokenInfo.ERC20whitelistCost : tokenInfo.ERC20publicCost;
        require(tokenInfo.ERC20paytoken.allowance(msg.sender, address(this)) >= SafeMath.mul(cost, _numberOfTokens), "Token allowance not set");
 
        if (cost > 0) {
            uint256 totalCost = SafeMath.mul(cost, _numberOfTokens);
            uint256 royaltyAmount = SafeMath.div(SafeMath.mul(totalCost, tokenInfo.ERC20royaltyPercentage), 100);
            uint256 ownerAmount = SafeMath.sub(totalCost, royaltyAmount);

            SafeERC20.safeTransferFrom(tokenInfo.ERC20paytoken, msg.sender, address(this), ownerAmount);
            if (royaltyAmount > 0) {
                SafeERC20.safeTransferFrom(tokenInfo.ERC20paytoken, msg.sender, tokenInfo.ERC20royaltyWallet, royaltyAmount);
            }
