...
// File contracts/utilities/AllocationVestor.sol
 ...
 * adapted for use for SteakHut Finance by 0xWagyu
 */

contract AllocationVester is Ownable {
    ...
    struct Member {
        ...
        uint256 totalAllocation; //totalAllocation granted to user
        uint256 harvested; //total tokens harvested
    }
    ...
    function harvest() external {
        ...
        member.harvested += uint256(amount);
     ...
     * @dev should ensure previous vesting period has completed.
     */
    function setAllocations(
        ...
        for (uint256 i; i < length; ++i) {
            ...
            if (allocation != 0) {
                ...
                member.rate = rate;
                member.lastUpdate = uint64(block.timestamp);
                member.totalAllocation += allocation;
