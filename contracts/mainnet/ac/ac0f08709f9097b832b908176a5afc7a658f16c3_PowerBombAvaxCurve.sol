...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _harvest(bool isDeposit) internal virtual {
        ...
        // console.log(CRV.balanceOf(address(this))); // 1.133506921369962820
    ...
    function switchGauge(address _gauge) external onlyOwner {
        gauge.claim_rewards();
        uint lpTokenAmt = gauge.balanceOf(address(this));
        gauge.withdraw(lpTokenAmt);
        gauge = IGauge(_gauge);
        lpToken.approve(_gauge, type(uint).max);
        gauge.deposit(lpTokenAmt);
    }
