...
import "./interfaces/IReserveTokenManager.sol";
...
contract IndexRegistry is IIndexRegistry, IReserveTokenManager, NameRegistry {
    ...
    /// @inheritdoc IReserveTokenViewer
    mapping(address => address) public override reserveTokenOf;
    ...
    /// @inheritdoc IReserveTokenManager
    function setReserveToken(address reserveToken, address index) external override onlyRole(INDEX_MANAGER_ROLE) {
        require(hasRole(ASSET_ROLE, reserveToken), "IndexRegistry: INVALID");

        reserveTokenOf[index] = reserveToken;
    }
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return
            _interfaceId == type(IReserveTokenManager).interfaceId ||
            _interfaceId == type(IIndexRegistry).interfaceId ||
    ...
    function _authorizeUpgrade(address _newImpl) internal view virtual override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _newImpl.supportsInterface(type(IReserveTokenManager).interfaceId) &&
                _newImpl.supportsInterface(type(IIndexRegistry).interfaceId),
