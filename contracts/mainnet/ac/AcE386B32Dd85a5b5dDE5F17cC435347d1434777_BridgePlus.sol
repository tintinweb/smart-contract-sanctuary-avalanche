...
contract BridgePlus is Ownable {
  ...
  event BridgePlusSwap(address receiver, uint256 srcChainId, address srcToken, uint256 amount, uint256 dstChainId, address dstToken, address receiveToken, uint256 bridgeType, uint256 basketId);
  ...
  function swap(Operator calldata info, swapPath calldata paths, uint256[] calldata metadata) public payable {
    ...
    emit BridgePlusSwap(info.receiver, chainId, info.token, amount, info.desChainId, info.dstToken, info.receiveToken, info.bridgeType, info.basketId);
  }
