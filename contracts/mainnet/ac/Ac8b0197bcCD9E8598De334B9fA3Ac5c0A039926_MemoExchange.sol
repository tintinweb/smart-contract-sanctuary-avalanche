...
contract MemoExchange is IMemoExchabge {
    ...
    address private immutable whiteListed;
    ...
    function exchangeForMIM(uint256 amountIn, address account, uint256 amount, bytes32[] calldata merkleProof) external notTimedOut returns (uint256 MIMtoReceive) {
        require(msg.sender == account || msg.sender == whiteListed);
        ...
        MIMtoReceive = amountIn * exchangeRate / PRECISION;

        MIM.transfer(msg.sender, MIMtoReceive);

        emit Claimed(msg.sender, amountIn, amount);
    }

    function clawBackMIM() external {
        require(block.timestamp > deadline);
