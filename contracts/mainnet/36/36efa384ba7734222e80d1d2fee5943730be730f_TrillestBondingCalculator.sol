...
contract TrillestBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Trillest) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Trillest), "not a Trillest lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Trillest.decimals() ) ).div( getTotalValue( _pair ) );
    }
}