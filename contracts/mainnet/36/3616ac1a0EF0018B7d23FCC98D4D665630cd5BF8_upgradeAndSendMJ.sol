...
contract upgradeAndSendMJ is Ownable {

  MinerJoe public minerjoe;


  constructor (MinerJoe _minerjoe) {
    minerjoe = _minerjoe;
  }


  function upgradeAndSend(uint256[] calldata tokenIDs, address[] calldata addressList, uint256 targetLevel) external onlyOwner {
    require(tokenIDs.length == addressList.length, "upgradeAndSend: mismatch array sizes");
    for (uint256 i = 0; i < addressList.length; ++i) {
      minerjoe.setLevel(tokenIDs[i], targetLevel);
      minerjoe.transferFrom(msg.sender, addressList[i], tokenIDs[i]);
    }
  }
}