...
/// @notice funds are only put to work once the set bin is active and there are idle funds in strategy
/// @notice we also check that required time limit has passed as to not waste gas and harvest, however anyone can manually call
contract LBPassiveSetBinManager is Ownable, AutomationCompatibleInterface {
    address public keeper;
    address public strategyAddress;
    address public gasCaller = 0xD316d2AA04d71Fd1E332437b0Cb5d261FD2F99AE;
    uint256 targetBin = 8388608;

    //gas saving measures
    uint256 public lastTimestamp;
    uint256 public period = 21600; //6 hours

    constructor(address _keeper, address _strategyAddress) {
        keeper = _keeper;
    ...
    /// @notice Updates address of passive keeper
    /// @notice typically should be chainlink or gelato bot
    /// @param _keeper new keeper address.
    function setKeeper(address _keeper) external onlyManager {
        keeper = _keeper;
    }

    /// @notice Updates underlying keeper of strategy
    /// @notice typically should be chainlink or gelato bot
    /// @param strategy address.
    /// @param _newKeeper new keeper address.
    function setStrategyKeeper(
        address strategy,
        address _newKeeper
    ) external onlyManager {
        ILBStrategy(strategy).setKeeper(_newKeeper);
    }

    /// @notice Updates the minimum period
    ...
    /// @notice external point of call to update params and execute a rebalance of the strategy
    ...
    /// @param _strategyAddress address of the strategy to rebalance
    function updateParametersRebalance(
        int256[] memory _deltaIds,
        uint256[] memory _distributionX,
        uint256[] memory _distributionY,
        uint256 _idSlippage,
        ...
        //allow the strategy to update the parameter and rebalances
        ILBStrategy(_strategyAddress).executeRebalance(
            _deltaIds,
            _distributionX,
            _distributionY,
            _idSlippage
    ...
    {
        checkData; // dummy reference to silence unused parameter warning
        upkeepNeeded = false;
        ILBStrategy strategy = ILBStrategy(strategyAddress);
        ...
        // if we have hit the target bin and there is free funds to deploy
        if (
            (activeId == targetBin && strategy.getBalanceX() > 1e6) &&
            (activeId == targetBin && strategy.getBalanceY() > 1e6)
        ) {
            upkeepNeeded = true;
        }

        //require an upkeep if there has been minimum time between
        //harvest every x hours regardless
        if (block.timestamp > lastTimestamp + period) {
            upkeepNeeded = true;
        }

        //performUpkeep by passing the encoded strategy address
        performData = abi.encodePacked(strategyAddress);
    }

    //others can call this to put potentially unused funds to work
    //but we only call if after each minimum period.
    function performUpkeep(bytes calldata performData) external override {
        performData; //silence warnings
        //perform upkeep logic
        //decode the address of the strategy
        ILBStrategy strategy = ILBStrategy(strategyAddress);
        ...
        // place an earn
        if (
            (activeId == targetBin && strategy.getBalanceX() > 1e4) &&
            (activeId == targetBin && strategy.getBalanceY() > 1e4)
        ) {
            _earn();
        }
    }
    ...
    /// @notice executes an of the associated strategy
