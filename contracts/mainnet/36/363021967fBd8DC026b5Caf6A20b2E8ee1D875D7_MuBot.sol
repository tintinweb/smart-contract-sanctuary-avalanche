...
contract MuBot is Ownable{
    ...
    function exactIn(address router, address _tokenIn, address _tokenOut, uint256 amountIn, uint256 amountOutMin) external returns (uint256[] memory amounts){
        ...
        return Router(router).swapExactTokensForTokens( amountIn,  amountOutMin, path, address(this), deadline);
    }

    function exactout(address router, address _tokenIn, address _tokenOut, uint256 amountOut,uint256 amountInMax) external returns (uint256[] memory amounts){
            uint256 deadline = block.timestamp + 300;
            address[] memory path;
		    path = new address[](2);
		    path[0] = _tokenIn;
		    path[1] = _tokenOut;
            IERC20(_tokenIn).approve(router, amountInMax);
            return Router(router).swapTokensForExactTokens( amountOut,  amountInMax,path,address(this), deadline);
    }
    ...
    function trade(address _router1,  address _token1, address _token2, uint256 _amount) external onlyOwner{
