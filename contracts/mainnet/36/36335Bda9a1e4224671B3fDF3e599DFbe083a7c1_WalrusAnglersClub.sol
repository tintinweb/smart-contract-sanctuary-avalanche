...
contract WalrusAnglersClub is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Walrus Anglers Club";
    ...
    // Mint Token
    IERC20 public constant mintToken = IERC20(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664);

    // Team Receive Wallet
    address public teamWallet = 0xc197f340d1e57bDE2630FA687456f0B3e55861F6;

    // Swap Router
    IUniswapV2Router02 public constant router = IUniswapV2Router02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    constructor() {
        path = new address[](2);
        path[0] = address(mintToken);
        path[1] = 0xe6d1aFea0B76C8f51024683DD27FA446dDAF34B6;
    }
    ...
    */
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        if (_totalSupply == breakpoint0 || _totalSupply == breakpoint1 || _totalSupply == breakpoint2) {
            mintingEnabled = false;
        }
    ...
    function _distribute() internal {
        ...
        // send half of the usdc.e to team wallet
        uint forTeam = mintToken.balanceOf(address(this)) / 2;
        if (forTeam > 0) {
            mintToken.transfer(
                teamWallet,
                forTeam
        ...
        // swap rest of USDC to walrus for team wallet
        uint remainder = mintToken.balanceOf(address(this));
        if (remainder > 0) {
            mintToken.approve(address(router), remainder);
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                remainder, 0, path, teamWallet, block.timestamp + 100
            );
        }
    }
