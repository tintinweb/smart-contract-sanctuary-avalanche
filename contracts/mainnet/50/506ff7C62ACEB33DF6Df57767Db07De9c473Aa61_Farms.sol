...
contract Farms is ReentrancyGuard, Ownable, Pausable {
  ...
  uint256 public constant MIN_MAX_MULTIPLIER = 1 * 1e18; // 1x
  ...
  */
  ...
  constructor(address _steady, address _esSteady, address _tokenManager) {
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view returns (uint256) {
    ...
    return (position.stakedAmount * currentRewardPerStakedToken(_id)
            * getRewardMultiplier(_id, _user) // TODO: mock multiplier for now
  ...
  */
  function getRewardMultiplier(uint256 _id, address _user) public view returns (uint256) {
    ...
    uint256 multiplier = (position.pointsAmount * farm.totalStaked / SAFE_MULTIPLIER
                         * farm.maxMultiplier / SAFE_MULTIPLIER)
                         / (position.stakedAmount * farm.totalPoints);

    if (multiplier < (1 * SAFE_MULTIPLIER)) {
      return 1 * SAFE_MULTIPLIER;
    } else if (multiplier > farm.maxMultiplier) {
      return farm.maxMultiplier;
    } else {
      return multiplier;
    }
  ...
  */
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0) {
      ...
      position.rewardDebt = position.stakedAmount * currentRewardPerStakedToken(_id)
                            * getRewardMultiplier(_id, msg.sender) // TODO: mock multiplier for now
                            / SAFE_MULTIPLIER; // TODO: mock multiplier for now
  ...
  /**
  * Withdraw all staked tokens in a position and claim all rewards
  * @param _id  Unique if of farm
  */
  function exit(uint256 _id) public nonReentrant whenNotPaused {
    Position memory position = positions[_id][msg.sender];

    claim(_id);
    withdraw(_id, position.stakedAmount);
  }
  ...
  * Callable only from the YieldBooster contract
  * @param _id  Unique id of farm
  * @param _user  Address of user boosting
  * @param _amount  Amount of esSteady points to boost this position
  */
  function boost(uint256 _id, address _user, uint256 _amount) external nonReentrant whenNotPaused {
    require(farms[_id].active, "Farm is not active");
    ...
    IERC20(esSteady).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  * Callable only from the YieldBooster contract
  * @param _id  Unique id of farm
  * @param _user  Address of user unboosting
  * @param _amount  Amount of esSteady points to unboost this position
  */
  function unboost(uint256 _id, address _user, uint256 _amount) external nonReentrant whenNotPaused {
    require(farms[_id].active, "Farm is not active");
    require(_user != address(0), "Invalid zero address");
    require(_amount > 0, "Cannot boost 0");
    ...
    position.pointsAmount = position.pointsAmount - _amount;
    farm.totalPoints = farm.totalPoints - _amount;
    IERC20(esSteady).safeTransfer(msg.sender, _amount);
  ...
  */
  function withdrawRewardTokens(uint256 _id, uint256 _amount) external nonReentrant onlyOwner {
    ...
    require(
      farm.totalReward > 0, "Cannot withdraw when farm has no reward tokens deposited"
    );
    require(
      _amount <= farm.totalReward,
  ...
  */
  function updateYieldBooster(address _yieldBooster) external onlyOwner {
    require(_yieldBooster != address(0), "invalid zero address");
