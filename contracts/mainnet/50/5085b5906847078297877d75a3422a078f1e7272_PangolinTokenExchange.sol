...
import {TokenExchangeLibrary} from "./TokenExchangeLibrary.sol";
import "../interfaces/IPriceOracle.sol";
import "../interfaces/ITokenExchange.sol";

contract PangolinTokenExchange is ITokenExchange, AccessControlEnumerableUpgradeable {
    ...
    function initialize(address router_) external initializer {
        ...
        router = IPangolinRouter(router_);
    }

    /**
     * @dev set a token pair recipient be traded on Uniswap V2 (or equivalent)
    ...
    function sellExactNativeTokens(IERC20MetadataUpgradeable outputToken, address recipient)
        ...
        returns (uint256)
    {
        IERC20MetadataUpgradeable inputToken = IERC20MetadataUpgradeable(router.WAVAX());
        TokenExchangeLibrary.validateSellExactNativeTokenParams(
            outputToken,
            isRegistered[inputToken][outputToken],
            recipient,
            msg.value
        );
        ...
        uint256 amountOutMinimum = TokenExchangeLibrary.calculateAmountOutMinimum(
            tp.priceOracle,
            inputToken,
            outputToken,
            msg.value,
        ...
        emit SellToken(address(inputToken), address(outputToken), msg.value, output);
        return output;
    }
    ...
    function sellExactInput(
        ...
        // check
        TokenExchangeLibrary.validateSellExactInputParams(
            inputToken,
            outputToken,
            isRegistered[inputToken][outputToken],
            recipient,
...
library TokenExchangeLibrary {
    function calculateAmountOutMinimum(
        IPriceOracle _priceOracle,
        IERC20MetadataUpgradeable _inputToken,
        IERC20MetadataUpgradeable _outputToken,
        uint256 _amountIn,
        uint64 _slippage
    ) internal view returns (uint256) {
        uint256 price = _priceOracle.price(address(_inputToken));
        uint8 dec = _priceOracle.decimals();

        // _amountIn in max 18, price in max 18, _outputToken in max 18, _slippage in 18
        // if all multiplication is done before division, overflow is possible
        // To avoid overflow, calculate exactAmount first
        uint256 exactAmount = (_amountIn * price * 10**_outputToken.decimals()) / 10**(_inputToken.decimals() + dec);
        // the error of amountOutMinimum will be ±1 wei since (1e18 - _slippage)) / 1e18 < 1 wei
        // the accuracy of is not vitally important as it is just specifying a limit
        uint256 amountOutMinimum = (exactAmount * (1e18 - _slippage)) / 1e18;

        return amountOutMinimum;
    }

    function validateSellExactNativeTokenParams(
        IERC20MetadataUpgradeable _outputToken,
        bool _isRegistered,
        address _recipient,
        uint256 _amountIn
    ) internal pure {
        require(address(_outputToken) != address(0), "outputToken is address(0)");
        require(_isRegistered, "unregistered token pair");
        require(_recipient != address(0), "recipient is address(0)");
        require(_amountIn > 0, "msg.value must be > 0");
    }

    function validateSellExactInputParams(
        IERC20MetadataUpgradeable _inputToken,
        IERC20MetadataUpgradeable _outputToken,
        bool _isRegistered,
        address _recipient,
        ...
        require(address(_inputToken) != address(0), "inputToken is address(0)");
        require(address(_outputToken) != address(0), "outputToken is address(0)");
        require(_isRegistered, "unregistered token pair");
        require(_recipient != address(0), "recipient is address(0)");
