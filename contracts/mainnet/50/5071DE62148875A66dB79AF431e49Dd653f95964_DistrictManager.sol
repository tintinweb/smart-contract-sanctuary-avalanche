...
contract DistrictManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    ...
    function compoundAndCreateNode(string memory _name, uint64[] memory userNodes, bool useBalance) public {
        ...
        uint256 nodePrice = nodePlaymatePrice * 10 ** 18;
        require(nodePrice > 0, "error");
        uint256 availableRewards = manager.getAddressRewards(sender) + manager.getAddressRewards(sender) * manager.getUserMultiplier(sender) / 1000;
        if (!useBalance) require(manager.getAddressRewards(sender) >= nodePrice, "HELPER: Playmate balance too low for creation.");
        ...
        if (availableRewards >= nodePrice) {
          ...
          MANSIONSHEPLER.claimUtility(userNodes, REWARDS_POOL, availableRewards, address(this), sender);
        }
        createNode(sender, _name);
    }
