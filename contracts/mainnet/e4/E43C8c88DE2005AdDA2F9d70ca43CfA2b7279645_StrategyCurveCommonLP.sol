...
contract StrategyCurveCommonLP is StratManager, FeeManager, GasThrottler {
    ...
    function chargeFees() internal {
        ...
        uint256 nativeFeeBal = IERC20(native).balanceOf(address(this)).mul(100).div(1000);

        uint256 callFeeAmount = nativeFeeBal.mul(callFee).div(MAX_FEE);
        if (callFeeAmount > 0)
            IERC20(native).safeTransfer(tx.origin, callFeeAmount);

        uint256 customerFeeAmount = nativeFeeBal.mul(customerFee).div(MAX_FEE);
        IERC20(native).safeTransfer(additionalCustomerRecipient, customerFeeAmount);

        uint256 LiquidCFeeAmount = nativeFeeBal.mul(LiquidCFee).div(MAX_FEE);
