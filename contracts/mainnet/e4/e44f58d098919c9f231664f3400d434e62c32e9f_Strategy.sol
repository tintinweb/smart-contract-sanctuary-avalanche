/**
 *Submitted for verification at snowtrace.io on 2023-07-06
*/

/**
 *Submitted for verification at snowtrace.io on 2023-07-02
*/
...
abstract contract BaseTokenizedStrategy {
    ...
    address public constant tokenizedStrategyAddress =
        0x03bD8E1d20D0474A4E44ffF85D651ce0aEC5b81c;
...
contract Strategy is BaseTokenizedStrategy {
    ...
    ILPStaking public constant lpStaker = ILPStaking(0x8731d54E9D02c286767d56ac03e8037C07e01e98);
    IStargateRouter public constant stargateRouter = IStargateRouter(0x45A01E4e04F14f7A4a6702c74187c5F6222033cd);
    IOpenflow public openflowSdk;
    IPool public pool;
    uint256 public poolID; // @note Main Pool ID
    uint256 public stakingID; // @note Pool ID for LPStaking (S*USDC: 0, S*USDT: 1, S*FRAX: 2)
    ERC20 public reward;
    ERC20 public lpToken;

    constructor(address _asset, string memory _name, uint16 _stakingID, address _openflowInstance) BaseTokenizedStrategy(_asset, _name) {
        ...
        require(_asset == address(pool.token()), "already initialized");
        poolID = pool.poolId();
        ...
        ERC20(reward).safeApprove(address(_openflowInstance), type(uint256).max);
        openflowSdk = IOpenflow(_openflowInstance);
    }
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        lpStaker.withdraw(stakingID, _lpAmount); //@note Unstake
        stargateRouter.instantRedeemLocal(uint16(poolID), _lpAmount, address(this)); //@note Withdraw
    }
    ...
    function _claimAndSellRewards() internal {
        _stakeLP(0); //@note Claim rewards
        openflowSdk.swap(); //@note Sell rewards via OpenFlow
    }
    ...
    /*//////////////////////////////////////////////////////////////
                    STARGATE SPECIFIC HELPER FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    ...
    function _ldToLp(uint256 _amountLD) internal returns (uint256) {
        ...
        return (_amountLD * pool.totalSupply()) / (_totalLiquidity);
    }
    ...
    function availableWithdrawLimit() public view returns (uint256) {
        //@note Stargate allows for partial fill of withdraw request, if liquidity is not sufficient we must revert
        return pool.deltaCredit() + TokenizedStrategy.totalIdle();
    }

    function valueOfLPTokens() public view returns (uint256) {
        uint256 _totalLPTokenBalance = lpToken.balanceOf(address(this)) + lpStaker.userInfo(stakingID, address(this)).amount;
        return pool.amountLPtoLD(_totalLPTokenBalance);
    }
