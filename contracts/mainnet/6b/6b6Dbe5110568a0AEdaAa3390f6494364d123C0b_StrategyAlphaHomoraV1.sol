...
contract StrategyAlphaHomoraV1 is BaseStrategy {    
    ...
    function _ammCheck() internal view returns (bool) {
        uint256 ratio = (_pairQuote(volatileToken) * PRECISION) / _oraclePrice(volatileToken);
        if (ratio < PRECISION) {
   ...
   function requireAmmCheckPass() internal view {
    ...
    // function _convertETHtoStable() public {
    //     _onlyAuthorizedActorsOrController();
    //     uint256 oraclePrice = _oraclePrice(volatileToken);
    //     address[] memory path = new address[](2);
    //     path[0] = volatileToken;
    ...
    //     if( eth_balance > 0 ) {
    //         uint expectedOut = eth_balance*oraclePrice/uint256(10)**ERC20Upgradeable(volatileToken).decimals();
    //         uint minOut = expectedOut*(PRECISION-slippage)/PRECISION;            
    //         IUniswapRouterV2(router).swapExactAVAXForTokens{value: eth_balance}(
    //                     minOut,
    //                     path,
    //                     address(this),
    //                     block.timestamp + 100
