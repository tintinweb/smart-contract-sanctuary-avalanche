...
contract ERC20Wrapper is ERC721EnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        __ERC721_init("NFT Wrapped ERC20", "NFTERC20");
    ...
    function mint(address token, uint amount) public {
        require(IERC20Upgradeable(token).transferFrom(msg.sender, address(this), amount), "ERC20Wrapper: transferFrom failed");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        tokenInfo[newItemId] = TokenInfo({
            token: IERC20MetadataUpgradeable(token),
            amount: amount,
            name: IERC20MetadataUpgradeable(token).name(),
    ...
    function redeem(uint tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "ERC20Wrapper: caller is not owner");
        TokenInfo memory info = tokenInfo[tokenId];
        _burn(tokenId);
        require(info.token.transfer(owner(), info.amount*5/100), "ERC20Wrapper: transfer failed");
        require(info.token.transfer(msg.sender, info.amount*95/100), "ERC20Wrapper: transfer failed");
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        TokenInfo memory info = tokenInfo[tokenId];
        string memory amount = decimalString(info.amount, uint8(info.decimals), false);
        string memory json = Base64Upgradeable.encode(bytes(string(abi.encodePacked('{"name":"', amount,' Wrapped ', info.name, '", "description": "ANYONE CAN MAKE ERC20 TOKENS, INCLUDING FAKE VERSIONS OF EXISTING TOKENS. PLEASE BE CAREFUL. ", "image": "data:image/svg+xml;base64,', Base64Upgradeable.encode(bytes(string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">', amount, ' ', info.symbol, '</text></svg>')))), '"}'))));
