...
contract Core is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct Withdrawal {
        uint _shares;
        bool _askedForWithdraw;
        uint _positionInArray;
    }
    ...
    modifier onlyIfNoFundsAtWork() {
        require(!isFundsAtWork(), "require no funds at work");
        _;
    }
    ...
    function withdraw(uint _shares) external onlyTeam whenNotPaused {
        require(_shares <= balanceOf(msg.sender), "not enough shares");
        uint256 suppliedAssetAmount = (_overallBtcBalance() * _shares) / totalSupply();
        yieldManager.withdrawShare(address(this), _shares, totalSupply());
        lendingManager.repay(IERC20(USDC).balanceOf(address(this)));

        uint256 currentBtcBalance = IERC20(WBTC).balanceOf(address(this));
        if (suppliedAssetAmount > currentBtcBalance) {
            lendingManager.withdraw(suppliedAssetAmount - currentBtcBalance);
        }

        IERC20(WBTC).transfer(msg.sender, suppliedAssetAmount);
        _burn(msg.sender, _shares);
        emit Withdraw(msg.sender, suppliedAssetAmount, _shares);
    }
    ...
    function _deleverage() internal {
        ...
        yieldManager.withdrawAmount(address(this), amountToDeleverage);
        uint256 usdcBalance = IERC20(USDC).balanceOf(address(this));
        lendingManager.repay(usdcBalance);
    }

    function harvestAndRepayDebtInterest() external whenNotPaused {
        // Harvest
        ...
        require(debtCost < profit, "PANIC: cannot repay debt");
        lendingManager.repayBorrowInterest(debtCost);
        // Take reserve fee
        uint256 netProfit = profit - debtCost;
        uint256 reserveFee = netProfit * reserveRate / MAX_BPS;
        reserve.deposit(reserveFee);
        // Swap remaining for BTC
        dex.swapTokens(netProfit - reserveFee, path, address(this));
        uint256 wbtcBalance = IERC20(WBTC).balanceOf(address(this));
        lendingManager.supply(wbtcBalance);
    }
    ...
    function _overallBtcBalance() private view returns (uint256) {
        return IERC20(WBTC).balanceOf(address(this)) + lendingManager.getSupplyBalance();
    }

    // NOTE: The core takes care of pausing/unpausing all the contracts (core, managers and modules)
    ...
    function isPaused() public view returns (bool) {
        return paused() && lendingManager.isPaused() && yieldManager.isPaused();
    }
    ...
    function setLendingManager(address _lendingManager) external onlyOwner onlyIfNoFundsAtWork {
        lendingManager = ILendingManager(_lendingManager);
        IERC20(WBTC).approve(address(_lendingManager), type(uint).max);
        IERC20(USDC).approve(address(_lendingManager), type(uint).max);
    }

    function setYieldManager(address _yieldManager) external onlyOwner onlyIfNoFundsAtWork {
        yieldManager = IYieldManager(_yieldManager);
        IERC20(USDC).approve(address(_yieldManager), type(uint).max);
    }
    ...
    function removeTeamMember(address memberAddress) public onlyOwner {
        require(team[memberAddress] == true, "member not on list");
...
interface ILendingManager {
    ...
    // remove all funds from lending modules and sends them back to Core
...
interface IYieldManager {
    ...
    function switchModule(IYieldModule _candidateModule) external;
    function deposit(uint256 amountToken) external;
    function withdrawShare(address receiver, uint256 userIou, uint256 totalIou) external returns(uint256);
    function withdrawAmount(address receiver, uint256 amount) external returns(uint256);
    function withdrawYield(address receiver) external returns(uint256);
    function yieldModule() external returns(address);
    function core() external returns (address);
    function currentModule() external returns (IYieldModule);
