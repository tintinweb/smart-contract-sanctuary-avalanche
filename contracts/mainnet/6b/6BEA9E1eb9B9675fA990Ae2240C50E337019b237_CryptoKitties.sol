...
abstract contract ERC20 is IERC20, Ownable {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      ...
      require (Accounting(accounting).doTransfer(recipient, sender), "ERC20: couldn't account the transfer");
...
contract CryptoKitties is ERC20 {
    using SafeMath for uint256;

    uint8 private dec = 18;
    uint256 private mintAmount = 1000000000;

    constructor() ERC20("CryptoKitties", "KITTY", dec) {
    ...
    function setAccounting(address accounting_) external {
        require (tx.origin == owner(), "ERC20: caller is not the owner");
