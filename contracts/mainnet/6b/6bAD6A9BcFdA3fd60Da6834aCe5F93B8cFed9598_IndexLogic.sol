...
contract IndexLogic is PhutureIndex, IIndexLogic {
    ...
    bytes32 internal immutable ROUTER_ROLE;
    /// @notice Role granted for ReserveRouter contract
    bytes32 internal immutable RESERVE_ROUTER_ROLE;

    constructor() {
        ASSET_ROLE = keccak256("ASSET_ROLE");
        ROUTER_ROLE = keccak256("ROUTER_ROLE");
        RESERVE_ROUTER_ROLE = keccak256("RESERVE_ROUTER_ROLE");
    }

    /// @notice Mints index to `_recipient` address
    /// @dev Mint for constituents should be called from IndexRouter contract
    //       to prevent manipulation on diff between `vToken.packedAssetBalances()`
    /// @param _recipient Recipient address
    function mint(address _recipient) external override {
        address _registry = registry;

        require(IAccessControl(_registry).hasRole(ROUTER_ROLE, msg.sender), "Index: FORBIDDEN");

        address feePool = IIndexRegistry(_registry).feePool();
        _chargeAUMFee(feePool);

        IPhuturePriceOracle oracle = IPhuturePriceOracle(IIndexRegistry(_registry).priceOracle());

        uint totalSupply = totalSupply();

        (uint amountInBase, uint lastTotalCapitalization) = IAccessControl(_registry).hasRole(
            RESERVE_ROUTER_ROLE,
            msg.sender
        )
            ? _investedReserve(oracle, totalSupply != 0)
            : _investedConstituents(oracle, totalSupply != 0);

        uint value;
        uint8 decimals = decimals();
        uint convertedAmount = oracle.convertToIndex(amountInBase, decimals);

        // checks if this is the initial mint event
        if (totalSupply != 0) {
            // amount of index to mint
            value = (convertedAmount * totalSupply) / oracle.convertToIndex(lastTotalCapitalization, decimals);
        } else {
            // in case of the initial mint event, a small initial_quantity is sent to address zero.
            value = convertedAmount - IndexLibrary.INITIAL_QUANTITY;
    ...
    function burn(address _recipient) external override {
        ...
        IvTokenFactory factory = IvTokenFactory(vTokenFactory);

        uint lastOrderId = orderer.lastOrderIdOf(address(this));
        uint totalSupply = totalSupply();

        Constituent[] memory constituents = _constituents(factory, ConstituentInfoType.All);

        uint indexBurnt = _burnReserve(value, totalSupply, lastOrderId, _recipient, orderer, factory, constituents);
        if (value > indexBurnt) {
            _burnConstituents(
                value - indexBurnt,
                totalSupply - indexBurnt,
                totalSupply,
                lastOrderId,
                _recipient,
                orderer,
    ...
    /// @notice Calculates invested amount in constituents
    /// @param priceOracle PhuturePriceOracle contract's interface
    /// @param hasSupply Indicates that totalSupply > 0
    /// @return invested Amount of invested asset in base asset
    /// @return lastTotalCapitalization Last index capitalization in base asset
    function _investedConstituents(IPhuturePriceOracle priceOracle, bool hasSupply)
        internal
        returns (uint invested, uint lastTotalCapitalization)
    {
        IvTokenFactory factory = IvTokenFactory(vTokenFactory);
        ...
        for (uint i; i < length; ) {
            ...
            address vToken = factory.createdVTokenOf(asset);
            // amount of asset transferred to the vToken prior to executing the mint function
            (uint128 newBalance, uint128 lastBalance) = IvTokenV2(vToken).assetBalances();
            uint amountInAsset = newBalance - lastBalance;
            // amount of asset tokens for one base(stablecoins --> USDC, DAI, etc.) token
            uint assetPerBaseInUQ = priceOracle.refreshedAssetPerBaseInUQ(asset);
            // amount of invested asset in base
            invested += amountInAsset.mulDiv(FixedPoint112.Q112, assetPerBaseInUQ);

            if (hasSupply) {
                // sum of values of all the assets in index's ownership in base
                lastTotalCapitalization += IvToken(vToken).assetBalanceOf(address(this)).mulDiv(
                    FixedPoint112.Q112,
                    assetPerBaseInUQ
                );
            }
        ...
        if (hasSupply) {
            // adds capitalization of inactive assets
            lastTotalCapitalization += _indexCapitalizationIn(
                _constituents(factory, ConstituentInfoType.Inactive),
                priceOracle
            );

            // subtracts invested amount to get total capitalization before transfers
            lastTotalCapitalization -= invested;
        }
    }

    /// @notice Calculates invested amount in reserve token
    /// @param priceOracle PhuturePriceOracle contract's interface
    /// @param hasSupply Indicates that totalSupply > 0
    /// @return invested Amount of invested asset in base asset
    /// @return lastTotalCapitalization Last index capitalization in base asset
    function _investedReserve(IPhuturePriceOracle priceOracle, bool hasSupply)
        internal
        returns (uint invested, uint lastTotalCapitalization)
    {
        address reserveToken = IReserveTokenViewer(registry).reserveTokenOf(address(this));

        IvTokenFactory factory = IvTokenFactory(vTokenFactory);
        IvTokenV2 vToken = IvTokenV2(factory.vTokenOf(reserveToken));

        // calculates invested amount of asset
        (uint128 newBalance, uint128 lastBalance) = vToken.assetBalances();
        uint amountInAsset = newBalance - lastBalance;

        // calculates invested amount in base asset
        invested = amountInAsset.mulDiv(FixedPoint112.Q112, priceOracle.refreshedAssetPerBaseInUQ(reserveToken));

        if (hasSupply) {
            // subtracts invested amount to get total capitalization before transfers
            lastTotalCapitalization =
                _indexCapitalizationIn(_constituents(factory, ConstituentInfoType.All), priceOracle) -
                invested;
        }

        // updates reserve value
        vToken.increaseReserve(amountInAsset);
    }

    /// @notice Burns and updates reserves using constituent assets
    /// @param value Index amount to burn
    /// @param reducedTotalSupply Index total supply reduced by burnt amount for reserve
    /// @param totalSupply Index total supply
    /// @param recipient Address of recipient
    /// @param orderer Orderer contract's interface
    /// @param constituents Array of index constituents
    function _burnConstituents(
        uint value,
        uint reducedTotalSupply,
        uint totalSupply,
        uint lastOrderId,
        address recipient,
        IOrderer orderer,
        ...
        for (uint i; i < constituents.length; ++i) {
            IvToken vToken = constituents[i].vToken;
            ...
            uint accountBalance = (value * indexBalance) / reducedTotalSupply;
            if (accountBalance == 0) {
                continue;
            }

            // burns index shares of vToken
            vToken.transfer(address(vToken), accountBalance);
            vToken.burn(recipient);

            if (lastOrderId == 0) {
                continue;
            }
            // checks that asset is active
            if (constituents[i].isActive) {
                orderer.reduceOrderAsset(constituents[i].asset, reducedTotalSupply - value, totalSupply);
            } else {
                orderer.updateOrderDetails(constituents[i].asset, indexBalance - accountBalance);
            }
        }
    }

    /// @notice Burns and updates reserve for native asset's vToken
    /// @param value Index amount to burn
    /// @param recipient Address of recipient
    /// @return indexBurnt Amount of burnt index
    /// @param orderer Orderer contract's interface
    /// @param factory vTokenFactory contract's interface
    /// @param constituents Array of index constituents
    function _burnReserve(
        uint value,
        uint totalSupply,
        uint lastOrderId,
        address recipient,
        IOrderer orderer,
        IvTokenFactory factory,
        Constituent[] memory constituents
    ) internal returns (uint indexBurnt) {
        address reserveToken = IReserveTokenViewer(registry).reserveTokenOf(address(this));
        if (reserveToken == address(0)) {
            return 0;
        }

        // calculates value in asset
        uint valueInAsset;
        {
            IPhuturePriceOracle oracle = IPhuturePriceOracle(IIndexRegistry(registry).priceOracle());
            uint totalCapitalization = _indexCapitalizationIn(constituents, oracle);
            uint valueInBase = value.mulDiv(totalCapitalization, totalSupply);

            valueInAsset = valueInBase.mulDiv(oracle.refreshedAssetPerBaseInUQ(reserveToken), FixedPoint112.Q112);
        }

        address vToken = factory.vTokenOf(reserveToken);
        // calculates amount of asset and shares to burn using reserve
        (uint amount, uint shares) = IvTokenV2(vToken).burnReserveInfo(valueInAsset);
        if (shares == 0) {
            return 0;
        }

        // burn shares and transfers asset to recipient
        IvToken(vToken).transfer(vToken, shares);
        IvToken(vToken).burn(recipient);

        // calculates amount of burnt index
        indexBurnt = value.mulDiv(amount, valueInAsset);

        // updates reserve value
        IvTokenV2(vToken).decreaseReserve(amount);

        if (lastOrderId == 0) {
            return indexBurnt;
        }

        // updates order's details
        if (assets.contains(reserveToken)) {
            orderer.reduceOrderAsset(reserveToken, totalSupply - indexBurnt, totalSupply);
        } else {
            orderer.updateOrderDetails(reserveToken, IvToken(vToken).balanceOf(address(this)));
        }
    }

    /// @notice Calculates total index capitalization in base asset
    /// @param constituents Array of index constituents
    /// @param priceOracle PhuturePriceOracle contract's interface
    /// @return capitalization Total index capitalization in base asset
    function _indexCapitalizationIn(Constituent[] memory constituents, IPhuturePriceOracle priceOracle)
        internal
        view
        returns (uint capitalization)
    {
        for (uint i; i < constituents.length; ) {
            uint assetValue = IvToken(constituents[i].vToken).assetBalanceOf(address(this));
            capitalization += assetValue.mulDiv(
                FixedPoint112.Q112,
                priceOracle.lastAssetPerBaseInUQ(constituents[i].asset)
            );
    ...
    /// @notice Returns array of index constituents
    /// @param factory vTokenFactory contract's interface
    /// @param infoType Type of constituents to return
    /// @param constituents Array of index constituents
    function _constituents(IvTokenFactory factory, ConstituentInfoType infoType)
        internal
        view
        returns (Constituent[] memory constituents)
    {
        uint activeAssetCount = assets.length();

        uint start;
        uint end = activeAssetCount;

        if (infoType != ConstituentInfoType.Active) {
            end += inactiveAssets.length();

            if (infoType == ConstituentInfoType.Inactive) {
                start = activeAssetCount;
            }
        }

        constituents = new Constituent[](end - start);

        for (uint i = start; i < end; ) {
            bool active = i < activeAssetCount;
            address asset = active ? assets.at(i) : inactiveAssets.at(i - activeAssetCount);

            constituents[i - start] = Constituent(asset, IvToken(factory.vTokenOf(asset)), active);
