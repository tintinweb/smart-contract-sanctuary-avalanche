...
// File contracts/distribution/ZSharesRewardPool.sol
// License-Identifier: MIT
...
contract ZSharesRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accZSharesPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00142694 ether; // 45,000 zshare / (365 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "ZSharesRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "ZSharesRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accZSharesPerShare = pool.accZSharesPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accZSharesPerShare = accZSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accZSharesPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accZSharesPerShare = pool.accZSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accZSharesPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeZSharesTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accZSharesPerShare).div(1e18);
    ...
    function safeZSharesTransfer(address _to, uint256 _amount) internal {
