    ...
    ClampedRandomizer
{
     ...
     */
    function airdrop(uint256 maxIterationCount) public onlyOwner onlySnapshotCompleted overridePause {
        uint256 count = SNAPSHOT.length();
        require(currentSnapshotPosition < count, "ERC721: airdrop completed");

        uint256 completed = 0;
        for (currentSnapshotPosition; currentSnapshotPosition < count; currentSnapshotPosition++) {
            address owner = SNAPSHOT.holders(currentSnapshotPosition);

            // only airdrop if the current holder is not a contract and is not
            // the null address which prevents us from minting to a place where
            // it is unrecoverable
            if (!owner.isContract() && owner != address(0)) {
                _mintNFT(owner);
                completed += 1;
            }

            if (completed == maxIterationCount) {
                break; // limit the batches to the count due to gas constraints
            }
        }

        if (completed != 0) {
            currentSnapshotPosition += 1; // if we minted, we need to move ahead one so our next round doesn't mess up

            emit AirdropSent(completed);
        }
    }
    ...
    /**
     * @dev sets the contract that we will use for snapshotting for airdrop recipients
     */
    function setSnapshotContract(IERC721Enumerable ERC721Contract) public onlyOwner onlySnapshotNotSet whenPaused {
        SNAPSHOT = IERC721Snapshot(BASE_SNAPSHOT.clone());
        SNAPSHOT.initialize(ERC721Contract);
    }
     ...
     */
    function snapshot(uint256 maxIterationCount) public onlyOwner whenPaused returns (bool) {
        require(address(SNAPSHOT) != address(0));
        SNAPSHOT.snapshot(maxIterationCount);
        return SNAPSHOT.completed();
    }

    /**
     * @dev returns if the snapshot has been completed
     */
    function snapshotCompleted() public view returns (bool) {
        require(address(SNAPSHOT) != address(0));
        return SNAPSHOT.completed();
    }
