...
contract PegPool is PegPoolView, WalrusPegToken, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        IERC20Upgradeable(WLRS).safeApprove(_router, type(uint256).max);
        IERC20Upgradeable(WLRS).safeApprove(pairAddress, type(uint256).max);
    ...
    /// @dev We know that WLRS in token0 in the pair so this token order works without checks
    function _addWithdrawLiquidity(uint256 _depositTokenToUser) internal {
        ...
        uint256 quotedWLRSAmount = router.quote(depositTokenIn, reserve1, reserve0);

        // WLRS is token0/A in current pair
    ...
    function harvestRewards() external onlyOwner {
        IERC20Upgradeable token = IERC20Upgradeable(WSHARE);
...
interface IBoardroomTreasury {
    ...
    function getWlrsUpdatedPrice() external view returns (uint256);
}
...
contract WalrusPegToken is ERC20Upgradeable {
    function __WalrusPegToken__init() internal initializer {
        // Base contracts require manual initialization with upgrade systems
        __ERC20_init("Walrus Peg Token", "WPG");
    }
}
...
contract PegPoolState is OwnableUpgradeable {
    ...
    address public constant WLRS = 0x395908aeb53d33A9B8ac35e148E9805D34A555D3;

    address public constant WSHARE = 0xe6d1aFea0B76C8f51024683DD27FA446dDAF34B6;
