...
contract AvalanchePool_R5 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool_R1 {
    ...
    function stakeAndClaimCerts() override external payable {
        _stake(false);
        IBondAVAX(_fAvaxContract).unlockSharesFor(msg.sender, msg.value);
    }
    ...
    function _stake(bool isRebasing) internal nonReentrant {
        ...
        IBondAVAX(_fAvaxContract).mintBonds(msg.sender, msg.value);
        /* emit events */
    ...
    function claimCerts(uint256 amount) override external nonReentrant {
        uint256 bonds = IBondAVAX(_fAvaxContract).sharesToBalance(amount);
        IBondAVAX(_fAvaxContract).lockSharesFor(msg.sender, amount);
        _claim(bonds, false);
    }

    function _claim(uint256 amount, bool isRebasing) internal {
        ...
        IBondAVAX(_fAvaxContract).lockForDelayedBurn(msg.sender, amount);
        emit AvaxClaimPending(msg.sender, amount);
        emit AvaxClaimPendingV2(msg.sender, amount, isRebasing);
    }
    ...
    function serveClaims(address payable residueAddress, uint256 minThreshold) public onlyOperator payable {
        ...
        for (i = _pendingAvaxClaimGap; i < _pendingClaimers.length; i++) {
            ...
            if (!result) {
                ...
                amounts[j] = amount;
                j++;
                IBondAVAX(_fAvaxContract).commitDelayedBurn(claimer, amount);
            }
