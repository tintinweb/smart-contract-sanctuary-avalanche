...
*     ______  _______         _____         _____     ____       _____               _____   ____  _____   ______         _____  _____   ______         _____        ______   
*    |      \/       \    ___|\    \    ___|\    \   |    |  ___|\    \         ____|\    \ |    ||\    \ |\     \    ___|\    \|\    \ |\     \    ___|\    \   ___|\     \  
*   /          /\     \  /    /\    \  /    /\    \  |    | /    /\    \       |    | \    \|    | \\    \| \     \  /    /\    \\\    \| \     \  /    /\    \ |     \     \ 
*  /     /\   / /\     ||    |  |    ||    |  |____| |    ||    |  |    |      |    |______/|    |  \|    \  \     ||    |  |    |\|    \  \     ||    |  |    ||     ,_____/|
* /     /\ \_/ / /    /||    |__|    ||    |    ____ |    ||    |__|    |      |    |----'\ |    |   |     \  |    ||    |__|    | |     \  |    ||    |  |____||     \--'\_|/
*|     |  \|_|/ /    / ||    .--.    ||    |   |    ||    ||    .--.    |      |    |_____/ |    |   |      \ |    ||    .--.    | |      \ |    ||    |   ____ |     /___/|  
*|     |       |    |  ||    |  |    ||    |   |_,  ||    ||    |  |    |      |    |       |    |   |    |\ \|    ||    |  |    | |    |\ \|    ||    |  |    ||     \____|\ 
*|\____\       |____|  /|____|  |____||\ ___\___/  /||____||____|  |____|      |____|       |____|   |____||\_____/||____|  |____| |____||\_____/||\ ___\/    /||____ '     /|
*| |    |      |    | / |    |  |    || |   /____ / ||    ||    |  |    |      |    |       |    |   |    |/ \|   |||    |  |    | |    |/ \|   ||| |   /____/ ||    /_____/ |
* \|____|      |____|/  |____|  |____| \|___|    | / |____||____|  |____|      |____|       |____|   |____|   |___|/|____|  |____| |____|   |___|/ \|___|    | /|____|     | /
*    \(          )/       \(      )/     \( |____|/    \(    \(      )/          )/           \(       \(       )/    \(      )/     \(       )/     \( |____|/   \( |_____|/ 
*     '          '         '      '       '   )/        '     '      '           '             '        '       '      '      '       '       '       '   )/       '    )/    
*                                             '                                                                                                           '             '     
 ...
 *
 *    Web:      https://magia.finance/
 *    Telegram: https://t.me/magiafinance
 *    Discord:  https://discord.gg/dtStEnaBY7
 *    Twitter:  https://twitter.com/magiafinance
 *
 *    LinkTree: https://linktr.ee/MagiaNodes
 * 
 *    Created by the MooniFly.eth - thanks to DevTheApe.
 */
...
contract NODERewardManagement {
    ...
    modifier onlySentry() {
        require(msg.sender == token || msg.sender == gateKeeper, "Who dis?");
        _;
    }
    ...
    // in case someone sends funds directly to the contract
...
contract MAGIAFINANCE is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    uint256 public sellTax = 2000; //20% at launch on sells - lowering every 24h
    ...
    constructor(
        ...
        address uniV2Router,
        uint256 snipeBlkAmt
    ) ERC20("MAGIA.FINANCE", "MAGIA") PaymentSplitter(payees, shares) {
        ...
        require(treasuryPool != address(0) && distributionPool != address(0) && devPool != address(0), "FUTUR, DEV & REWARD ADDRESS CANNOT BE ZERO");
    ...
    function _transfer(
        ...
        if ( to == address(uniswapV2Router) && (from != address(this) && from != owner())){
            require(nodeRewardManager._isNodeOwner(from), "You need to own a node to be able to sell");
    ...
    function createNodeWithTokens(string memory name, uint256  _initValue) public {
        ...
        if (
            ...
            swapAndSendToFee(treasuryPool, futurTokens);
    ...
    function setnodeRequired(bool value) public onlyOwner {
        nodeRequired = value;
    }
    ...
    function transferToTreasury(address _tokenAddr, uint _amount) public onlyOwner {
        IERC20(_tokenAddr).transfer(treasuryPool, _amount);
    }
