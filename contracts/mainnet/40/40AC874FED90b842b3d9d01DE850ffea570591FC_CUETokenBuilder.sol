...
contract CUELauncherToken is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        uint256[4] memory __data,        // [refPer, run_tokenomics, max_tx, max_hold]
        ...
        _maxHoldAmount = __data[3];
        _maxTransAmount = __data[2];
        _minTokensForLiquidity = __data[1];
    ...
    function isExcluded(address account) public view returns (bool) { return _isExcluded[account]; }
    ...
    function restoreTax() private { _totalBuyTax = _previousRefBuyTax; _totalSellTax = _previousRefSellTax; }
    ...
    function _isBuy(address _sender) private view returns (bool) { return _sender == uniswapV2Pair; }

    function _isSell(address _to) private view returns (bool) { return _to == uniswapV2Pair; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_tradingOpen && !_isExcluded[sender] && !_isExcluded[recipient]) { revert("Trading is not yet enabled."); }

        if(!_isExcluded[sender]) { require(amount <= _maxTransAmount, "Transfer amount exceeds the maxTransAmount."); }
        ...
        if(_isExcluded[sender] || _isExcluded[recipient]) { removeTax(); }

        //remove tax if not a buy or sell
        if(!_isBuy(sender) && !_isSell(recipient)) { removeTax(); }
        ...
        if(_isExcluded[sender] || _isExcluded[recipient]) { restoreTax(); }

        if(!_isBuy(sender) && !_isSell(recipient)) { restoreTax(); }
    }
    ...
    function _setParentContract(address new_contract) public onlyOwner() { parent_contract = IERC20(new_contract); }

    function _setMarketingWallet(address payable wallet) public onlyOwner() { _MarketingWallet = wallet; }

    function _setDevWallet(address payable wallet) public onlyOwner() { _DevWallet = wallet; }
    ...
    function _setTotalBuyTax(uint256 totalBuyTax) external onlyOwner() {
        ...
        _totalBuyTax = totalBuyTax;_previousRefBuyTax = totalBuyTax;
    }

    function _setTotalSellTax(uint256 totalSellTax) external onlyOwner() {
        require(totalSellTax <= _maxSellTax, "Sell tax too high");

        _totalSellTax = totalSellTax;_previousRefSellTax = totalSellTax;
    }

    function _setRefPer(uint256 refPer) external onlyOwner() { _refPer = refPer; }
    ...
    function _setMinTokensForLiquidity(uint256 minTokensForLiquidity) external onlyOwner() { _minTokensForLiquidity = minTokensForLiquidity; }

    function _setLockLiquiditiesEnabled(bool lockLiquiditiesEnabled) external onlyOwner() { _lockLiquiditiesEnabled = lockLiquiditiesEnabled; }

    function _setAutoTaxEnabled(bool AutoTaxEnabled) external onlyOwner() { _autoTaxEnabled = AutoTaxEnabled; }
    ...
    function _doMarketing(uint256 amount) private { Address.sendValue(_MarketingWallet, amount); }

    function _doDev(uint256 amount) private { Address.sendValue(_DevWallet, amount); }

    function _doBurn(uint256 amount) private { _transfer(address(this), 0x000000000000000000000000000000000000dEaD, amount);emit Transfer(address(this), 0x000000000000000000000000000000000000dEaD, amount); }

    function _doBuyBack(uint256 tokenAmount) private { _swapTokensForTokens(tokenAmount, address(this)); }
...
contract CUETokenBuilder is Context, Ownable {
    ...
    IERC20 public usd_token = IERC20(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E);
    ...
    uint256 public builder_price_wei = 300000000000000000000;
    ...
    function buildToken(
        ...
        uint256[6] memory _buy_data,    // [buy_tax, tax_auto_liq, tax_dev, tax_burn, tax_marketing, buy_back]
        uint256[6] memory _sell_data,   // [sell_tax, tax_auto_liq, tax_dev, tax_burn, tax_marketing, buy_back]
        uint256[4] memory _data,        // [refPer, run_tokenomics, max_tx, max_hold]
        address _parent_contract,
        address _router
    ) external payable {

        uint256 discount = isAddressDiscountWhitelisted(msg.sender);

        uint256 new_price = builder_price_wei;

        if(discount > 0) {
            new_price = new_price.sub(new_price.div(100).mul(discount));
        }

        require(usd_token.balanceOf(msg.sender) >= new_price, "Fee not met");

        if(new_price > 0) {
            usd_token.transferFrom(msg.sender, 0x6DE425bfeB2435e1c1d32bD7ec686eC60C680645, new_price);
        }
    ...
    function getMyPrice(address _wallet) public view returns (uint256) {
        
        uint256 new_price = builder_price_wei;

        uint256 discount = isAddressDiscountWhitelisted(_wallet);
