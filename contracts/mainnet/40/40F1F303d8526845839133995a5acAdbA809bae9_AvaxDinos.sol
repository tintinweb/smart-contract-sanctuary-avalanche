...
contract AvaxDinos is ERC721 {
    ...
    constructor()
        ERC721("Avax Dinos", "AVDN")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 30) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiaxw46uxcp5pntjsov4a6iuhza7hihrp3gp37gjm7z3az7fqzbbzi/", Strings.toString(tokenId), ".json")
            );
    }
