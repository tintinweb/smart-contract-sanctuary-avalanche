...
/*
 ...
 __       ________     ______  __  __     ___   __    ________   ___   ___   ______   ______      
/_/\     /_______/\   /_____/\/_/\/_/\   /__/\ /__/\ /_______/\ /___/\/__/\ /_____/\ /_____/\     
\:\ \    \::: _  \ \  \:::__\/\ \ \ \ \  \::\_\\  \ \\::: _  \ \\::.\ \\ \ \\::::_\/_\:::_ \ \    
 \:\ \    \::(_)  \ \    /: /  \:\_\ \ \  \:. `-\  \ \\::(_)  \ \\:: \/_) \ \\:\/___/\\:\ \ \ \   
  \:\ \____\:: __  \ \  /::/___ \::::_\/   \:. _    \ \\:: __  \ \\:. __  ( ( \::___\/_\:\ \ \ \  
   \:\/___/\\:.\ \  \ \/_:/____/\ \::\ \    \. \`-\  \ \\:.\ \  \ \\: \ )  \ \ \:\____/\\:\/.:| | 
    \_____\/ \__\/\__\/\_______\/  \__\/     \__\/ \__\/ \__\/\__\/ \__\/\__\/  \_____\/ \____/_/ 
...
contract Randomizer {
    uint256 private _sizeIndex = 0; //Clamping cache for random TokenID generation in the anti-sniping algo
    uint256 private immutable _sizeCap ; //Size of initial randomized number pool & max generated value (zero indexed)
    mapping(uint256 => uint256) _swappedIDs; //TokenID cache for random TokenID generation in the anti-sniping algo

    constructor(uint256 sizeCap) {
        _sizeCap = sizeCap;
    }

    function _genNonce() internal virtual returns (uint256) {
        uint256 size = _sizeCap - _sizeIndex;
        uint256 swap;
        uint256 result;

        uint256 i = randomNumber() % size;

        //Setup the value to swap in for the selected number
        if (_swappedIDs[size - 1] == 0) {
            swap = size - 1;
        } else {
            swap = _swappedIDs[size - 1];
        }
...
contract LazyNaked is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, Randomizer {
    ...
    // MODIFIERS
    modifier onlyDevs() {
        require(devFees[msg.sender].percent > 0, "Dev Only: caller is not the LazyBoss");
        _;
    }
    ...
    uint public constant START_ID = 1;
    ...
    uint public maxPerPerson = 30;
    uint public price = 100000000000000000;
    address public royaltyAddress = 0xe6638Ed2d1a581B7d5A4aEd6752e3cCeAB14D2aF;
    uint public royalty = 1000;
    ...
    ) ERC721("LazyNaked", "LN") Randomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        require(supply + amount - 1 < maxSupply, "Error:  TotalSupply reached");
        require(amount > 0 && amount <= maxPerTx, "Error:  Only 10 per Tx");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: Max 30 per Wallet");
        if (price > 0) require(msg.value == price * amount, "Error: Go work for more money bro !");
        if (whitelistedOnly) require(whiteListed[msg.sender] >= amount, "Error: No whitelist here ");

        for (uint i = 0; i < amount; i++) {
            internalMint(msg.sender);
            if (whitelistedOnly) whiteListed[msg.sender] -= 1;
        }
        if (price > 0) splitFees(msg.value);
    }
    ...
    function internalMint(address to) internal {
        uint tokenId = _genNonce() + START_ID;
        _safeMint(to, tokenId);
    }
