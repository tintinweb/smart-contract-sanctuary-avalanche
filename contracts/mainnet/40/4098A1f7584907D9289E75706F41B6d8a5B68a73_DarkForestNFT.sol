...
contract DarkForestNFT is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    mapping(uint256 => uint256) public tierOfNfts;
    uint256 public tokenId;
    string BASE_URI;

    function initialize() public initializer {
        __ERC721_init("Dark Forest", "DF");
        __Ownable_init();
    }

    function mintTo(address[] memory accounts, uint256[] memory tier)
        external
        onlyOwner
    {
    ...
    function getNftTier(uint256 tokenID) public view returns (uint256 tier) {
        return tierOfNfts[tokenID];
    }

    function getHigherTier(address account)
        public
        view
        returns (uint256 _highestTier)
    {
    ...
    function getListofToken(address account)
        public
        view
