...
// File contracts/MadVeggies.sol


pragma solidity ^0.8.0;

contract MadVeggies is
    Context,
    ERC721Enumerable,
    ERC721URIStorage,
    Ownable,
    ...
    uint256 public _price = 1.6 ether;
    uint256 public  invitorsfraction = 0.17 ether;
    uint256 public _max = 1515;
    ...
    mapping(address => uint256) public InvitorsBalance;
    mapping (address => bool) public alreadyClaimed;
    mapping (address => bool) public IsInvitor;
    ...
    function setInvitorsFraction(uint256 _amount) public onlyOwner{
        invitorsfraction= _amount;
    }
        

    function SetBalanceInvitors(address[] calldata wallets, uint256[] calldata amounts) public onlyOwner{
        
        for (uint i = 0; i < wallets.length; i++) {
        InvitorsBalance[wallets[i]]= invitorsfraction * amounts[i];
        alreadyClaimed[wallets[i]] = false;
        IsInvitor[wallets[i]] = true;   
        }

    }
          
    function InvitorsClaim() public {

        require (IsInvitor[msg.sender], "User isn't in the invitors list.");
        require (!alreadyClaimed[msg.sender], "User already claimed.");
        uint balance = InvitorsBalance[msg.sender];
    ...
    function mint(uint amount) public payable {
        ...
        require(_tokenIdTracker.current() + amount <= _max, "Max Mint amount been reached.");
        require(amount <= maxTx, "max mint amount per session excceded.");
        ...
        for(uint i=0; i < amount; i++){
            ...
            _setTokenRoyalty(_tokenIdTracker.current(), msg.sender, contractRoyalties);
    ...
    function airdrop(address to, uint amount) external onlyOwner {
        ...
        require(_tokenIdTracker.current() + amount <= _max, "Not enough avax mad veggies left to mint amount.");     
         for(uint i=0; i < amount; i++){
         _mint(to , _tokenIdTracker.current());
         _setTokenRoyalty(_tokenIdTracker.current(), to, contractRoyalties);
         minter[_tokenIdTracker.current()] = to;
         lastDividendAt[_tokenIdTracker.current()] = totalDividend;
         _tokenIdTracker.increment();
        }
    }
    ...
    function setMax(uint256 amount) public onlyOwner {  
        _max = amount;
    }

    function setTxRate(uint256 amount) public onlyOwner {
        maxTx = amount;
    }

    function setContractRoyalties(uint256 _contractRoyalties) public onlyOwner {
        contractRoyalties = _contractRoyalties;
    }
