/**
 *Submitted for verification at snowtrace.io on 2023-05-01
*/
...
struct StrategyData {
    ...
    uint128 balance; // the balance of the strategy that GniLamp thinks is in there
}
...
// File: interfaces/IRatioCLPool.sol



pragma solidity 0.8.10;

interface IRatioCLPool {
...
// File: interfaces/IKamiswapV3Pool.sol



pragma solidity 0.8.10;

interface IKamiswapV3Pool {
  function token0() external returns (address);
  function token1() external returns (address);
...
/// @title A route processor for the Kami Aggregator
/// version 2.1
contract RouteProcessor3 is Ownable {
  ...
  IGniLampMinimal public immutable gniLamp;
  ...
  constructor(address _gniLamp, address[] memory priviledgedUserList) {
    gniLamp = IGniLampMinimal(_gniLamp);
  ...
  function processRouteInternal(
    ...
    while (stream.isNotEmpty()) {
      ...
      else if (commandCode == 5) processInsideGni(stream);
  ...
  /// @notice Processes Gni tokens 
  /// @notice Call swap for all pools that swap from this token
  /// @param stream Streamed process program
  function processInsideGni(uint256 stream) private {
    address token = stream.readAddress();
    uint8 num = stream.readUint8();

    uint256 amountTotal = gniLamp.balanceOf(token, address(this));
    unchecked {
      if (amountTotal > 0) amountTotal -= 1;     // slot undrain protection
      for (uint256 i = 0; i < num; ++i) {
        uint16 share = stream.readUint16();
  ...
  function swap(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    ...
    if (poolType == 0) swapKami(stream, from, tokenIn, amountIn);
    else if (poolType == 1) swapKamiV3(stream, from, tokenIn, amountIn);
    else if (poolType == 2) wrapNative(stream, from, tokenIn, amountIn);
    else if (poolType == 3) gniBridge(stream, from, tokenIn, amountIn);
    else if (poolType == 4) swapRatio(stream, from, tokenIn, amountIn);
  ...
  /// @notice Bridge/unbridge tokens to/from Gni
  /// @param stream [direction, recipient]
  /// @param from Where to take liquidity for swap
  /// @param tokenIn Input token
  /// @param amountIn Amount of tokenIn to take for swap
  function gniBridge(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    uint8 direction = stream.readUint8();
    address to = stream.readAddress();

    if (direction > 0) {  // outside to Gni
      // deposit to arbitrary recipient is possible only from address(gniLamp)
      if (amountIn != 0) {
        if (from == address(this)) IERC20(tokenIn).safeTransfer(address(gniLamp), amountIn);
        else IERC20(tokenIn).safeTransferFrom(from, address(gniLamp), amountIn);
      } else {
        // tokens already are at address(gniLamp)
        amountIn = IERC20(tokenIn).balanceOf(address(gniLamp)) +
        gniLamp.strategyData(tokenIn).balance -
        gniLamp.totals(tokenIn).elastic;
      }
      gniLamp.deposit(tokenIn, address(gniLamp), to, amountIn, 0);
    } else { // Gni to outside
      if (amountIn > 0) {
        gniLamp.transfer(tokenIn, from, address(this), amountIn);
      } else amountIn = gniLamp.balanceOf(tokenIn, address(this));
      gniLamp.withdraw(tokenIn, address(this), to, 0, amountIn);
    }
  }

  /// @notice Kamiswap pool swap
  /// @param stream [pool, direction, recipient]
  /// @param from Where to take liquidity for swap
  /// @param tokenIn Input token
  /// @param amountIn Amount of tokenIn to take for swap
  function swapKami(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    address pool = stream.readAddress();
    uint8 direction = stream.readUint8();
    address to = stream.readAddress();

    (uint256 r0, uint256 r1, ) = IKamiswapPair(pool).getReserves();
    ...
    IKamiswapPair(pool).swap(amount0Out, amount1Out, to, new bytes(0));
  }

  /// @notice Ratio pool swap
  /// @param stream [pool, swapData]
  /// @param from Where to take liquidity for swap
  /// @param tokenIn Input token
  /// @param amountIn Amount of tokenIn to take for swap
  function swapRatio(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    address pool = stream.readAddress();
    bytes memory swapData = stream.readBytes();

    if (amountIn != 0) {
      gniLamp.transfer(tokenIn, from, pool, amountIn);
    }
    
    IPool(pool).swap(swapData);
  }

  /// @notice KamiswapV3 pool swap
  /// @param stream [pool, direction, recipient]
  /// @param from Where to take liquidity for swap
  /// @param tokenIn Input token
  /// @param amountIn Amount of tokenIn to take for swap
  function swapKamiV3(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    address pool = stream.readAddress();
    bool zeroForOne = stream.readUint8() > 0;
    address recipient = stream.readAddress();

    if (from != address(this)) {
      require(from == msg.sender, 'swapKamiV3: unexpected from address');
      IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), uint256(amountIn));
    }

    lastCalledPool = pool;
    IKamiswapV3Pool(pool).swap(
      recipient,
      zeroForOne,
      int256(amountIn),
    ...
    require(lastCalledPool == IMPOSSIBLE_POOL_ADDRESS, 'RouteProcessor.swapKamiV3: unexpected'); // Just to be sure
  }

  /// @notice Called to `msg.sender` after executing a swap via IKamiswapV3Pool#swap.
  /// @dev In the implementation you must pay the pool tokens owed for the swap.
  /// The caller of this method must be checked to be a KamiswapV3Pool deployed by the canonical KamiswapV3Factory.
  /// amount0Delta and amount1Delta can both be 0 if no tokens were swapped.
  /// @param amount0Delta The amount of token0 that was sent (negative) or must be received (positive) by the pool by
  /// the end of the swap. If positive, the callback must send that amount of token0 to the pool.
  /// @param amount1Delta The amount of token1 that was sent (negative) or must be received (positive) by the pool by
  /// the end of the swap. If positive, the callback must send that amount of token1 to the pool.
  /// @param data Any data passed through by the caller via the IKamiswapV3PoolActions#swap call
  function kamiswapV3SwapCallback(
    int256 amount0Delta,
    int256 amount1Delta,
    bytes calldata data
  ) external {
    require(msg.sender == lastCalledPool, 'RouteProcessor.kamiswapV3SwapCallback: call from unknown source');
    lastCalledPool = IMPOSSIBLE_POOL_ADDRESS;
    (address tokenIn) = abi.decode(data, (address));
    int256 amount = amount0Delta > 0 ? amount0Delta : amount1Delta;
    require(amount > 0, 'RouteProcessor.kamiswapV3SwapCallback: not positive amount');
