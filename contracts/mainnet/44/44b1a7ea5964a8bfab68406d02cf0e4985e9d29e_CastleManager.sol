...
interface IPawnManager is IERC721 {    
    ...
    function getPawnsOf(address _account) external view returns (uint256[] memory); 
}

contract AidOwnable is Context {
     ...
     * @dev Returns the address of the current owner.
     */
    function aidContract() public view returns (address) {
        return _contract;
    }
...
contract CastleManager is Ownable, AidOwnable, IERC721, IERC721Metadata {
    CastleManager public CASTLES_V1 = CastleManager(0x2B25880655326d2030FeBF76a19D99e8c2F9dD97);
    IERC20 public CHESSES = IERC20(0xB81c7ee77E0D567F601CA897F7AB893E4b872576);
    IPawnManager public manager = IPawnManager(0x3E32deFbBE3b1C10774037D8311f2aEf71b805cA);
    address public REWARDS_POOL = 0x1b5838E4bfd5b1187eb98bfcf0FF20dCcB5484fe;
    ...
    mapping(uint256 => Castle) private _nodes;
    ...
    uint256 public nodeChessgamePrice = 20;
    uint256 public nodePawnPrice = 20;
    ...
    function totalNodesCreated() view external returns (uint) {
        return nodeCounter + CASTLES_V1.totalNodesCreated();
    }
    ...
    function updateCastle(uint256 id, string calldata metadata) external {
        Castle storage castle = _nodes[id];
        castle.metadata = metadata;
    }

    function getCastles(uint256 _id) public view onlyIfExists(_id) returns (Castle memory) {
        return _nodes[_id];
    }
    ...
    function getCastlesOf(address _account) public view returns (uint256[] memory) {
        return _bags[_account];
    }

    function updatenodeChessgamePrice (uint256 _nodeChessgamePrice) external onlyOwner {
        nodeChessgamePrice = _nodeChessgamePrice;
    }
    ...
    function _addCastle(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createCastle(_id, _name, _mint, _claim, _metadata, _to);
    }

    function setManager (IPawnManager _manager) external onlyOwner {
        manager = IPawnManager(_manager);
    }

    function setChessgames(IERC20 _CHESSES) external onlyOwner {
        CHESSES = IERC20(_CHESSES);
    }
    ...
    function migrate() external {
        ...
        uint256 len = CASTLES_V1.balanceOf(msg.sender);
        uint256[] memory nodeIds = CASTLES_V1.getCastlesOf(msg.sender);
        for (uint16 i = 0; i < len; i++) {
            (string memory _name, string memory _metadata, uint256 _id, uint64 _mint, uint64 _claim) = (CASTLES_V1.getCastles(nodeIds[i]).name, CASTLES_V1.getCastles(nodeIds[i]).metadata, CASTLES_V1.getCastles(nodeIds[i]).id, CASTLES_V1.getCastles(nodeIds[i]).mint, CASTLES_V1.getCastles(nodeIds[i]).claim);
            _nodes[_id] = Castle({
                name: _name,
                mint: _mint,
                claim: _claim,
    ...
    function _createCastle(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to) internal {
        require(!_exists(_id), "MANAGER: Castle already exist");
        _nodes[_id] = Castle({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        Castle memory _node = _nodes[uint64(tokenId)];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "Castle Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter + CASTLES_V1.totalNodesCreated();
        _createCastle(nodeId, nodeName, uint64(block.timestamp), uint64(block.timestamp), "", account);
        nodeCounter += 1;
    }

    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) public {
        require(bytes(_name).length > 0 && bytes(_name).length < 33, "AID: name size is invalid");
        address sender = _msgSender();
        require(sender != address(0), "AID:  Creation from the zero address");
        uint256 nodePrice = nodeChessgamePrice * 10 ** 18;
        require(nodePrice > 0, "error");
        require(CHESSES.balanceOf(sender) >= nodePrice, "AID: Chessgame balance too low for creation.");
        require(balanceOf(sender) + 1 < maxWallet, "AID: Exceeds max wallet amount");
        require(manager.balanceOf(sender) >= nodePawnPrice, "AID: Pawn balance too low for creation.");
        for (uint256 i = 0; i < nodePawnPrice; i++) {
            manager.safeTransferFrom(_msgSender(), address(0), userNodes[i], "");
        }
        CHESSES.transferFrom(_msgSender(), REWARDS_POOL,  nodePrice);
        createNode(sender, _name);        
    }

    function renameCastle(uint64 id, string memory newName) external {
    ...
    if (transferIsEnabled == true){
            ...
            require (_msgSender() == upgradeManager, "Castle Transfers are not allowed");
