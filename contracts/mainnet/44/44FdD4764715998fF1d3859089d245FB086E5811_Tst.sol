    ...
    WalletObserverImplementationPointer
{
        ...
        onlyPlayersManager
    {
        require(
            ...
            "TST: Use liquidityReward to reward liquidity"
        );
    ...
    function liquidityReward(uint256 amount) external onlyPlayersManager {
        require(
            address(liquidityPoolManager) != address(0),
            "TST: LiquidityPoolManager is not set"
        );
    ...
    function _transferTaxOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (!excludedFromFees[sender]) {
            ...
            if (isTSTLiquidityPool(recipient)) { // if the recipient address is a liquidity pool, apply sell fee
                uint _fees = (amount * sellFeesAmount) / 1000;
                _transferAmount = amount - _fees;
                super._burn(sender, _fees);
                super._mint(treasury, _fees); // transfer fee to treasury address
            }else {
    ...
    function isTSTLiquidityPool(address _recipient) private returns (bool) {
