...
ERC20PermitUpgradeable, ERC20BurnableUpgradeable, OldTokenConstants {
    ...
    mapping (address => Disbursement) private selfDisbursement;
    ...
    function mint(address _address, uint256 amount) public onlyRole(MINTER_ROLE) {
        uint256 locked = lockedOf[_address];
        lockedOf[_address] = 0;
    ...
    function claimPending() public whenNotPaused {
        ...
        if (s.balance > 0) {
            ...
            uint256 pending = ((block.timestamp - s.startTime) * pendingPerSecond);
            if (claimed < pending) {
                uint256 tPending = (pending - claimed) <= s.balance ? (pending - claimed) : s.balance;
                s.balance -= tPending;
    ...
    function pendingOf(address _address) public view returns(uint256) {
        ...
        uint256 locked = lockedOf[_address];
        if ((s.balance + locked) > 0) {
            uint256 claimed = (s.amount + locked) - (s.balance + locked);
            uint256 pendingPerSecond = (s.amount + locked) / s.duration;
            uint256 pending = ((block.timestamp - s.startTime) * pendingPerSecond);
            if (claimed < pending) {
                totalPending += (pending - claimed) <= (s.balance + locked) ? (pending - claimed) : (s.balance + locked);
            }
        }
        return totalPending;
    }
    ...
    function _transferAllDisbursements(address from, address to) internal {
        ...
        selfDisbursement[to].amount += d.amount + lockedFrom + lockedOf[to];
        selfDisbursement[to].balance += d.balance + lockedFrom + lockedOf[to];
        lockedOf[to] = 0;
...
// SPDX-License-Identifier: MIT
// Based on OpenZeppelin Contracts (last updated v4.5.0) (token/ERC20/extensions/ERC20Burnable.sol)
