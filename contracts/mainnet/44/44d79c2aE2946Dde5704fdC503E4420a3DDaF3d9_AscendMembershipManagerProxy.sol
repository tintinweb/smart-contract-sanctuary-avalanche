...
contract AscendMembershipManagerProxy is Ownable, IAscendMembershipManager {
    ...
    constructor( address _ascendMembership, address _ASCEND,
        address _treasuryWallet,
        address _liquidityWallet, address _rewardsPool)  {
    ...
    function updatePoolAddress(address _rewardsPool) external onlyOwner {
        rewardsPool = _rewardsPool;
    }
    ...
    function updateNodeRatios(uint16 _poolFee, uint16 _liquidityFee,uint16 _treasuryFee) external onlyOwner {
    ...
    function contractSwap(uint256 numTokensToSwap) internal {
        ...
        if(amountToRewardsPool > 0) {
            ASCEND.transfer(rewardsPool, amountToRewardsPool);
        }
    ...
    function sentASNDToWallets() external onlyOwner {
