...
interface INft {
    function balanceOf(address, uint256) external view returns(uint32);
}
library MerkleProof {
    function verify(
...
contract HALON is ERC20Upgradeable, KeeperCompatibleInterface {
    ...
    mapping(uint8 => uint256) public rewardExtraRate;
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    ...
    function initialize() public virtual initializer {
        ...
        setRewardRatePerDay(1, 1500);
        setRewardRatePerDay(2, 1500);
        setRewardRatePerDay(3, 1500);
        setRewardRatePerDay(4, 1500);
        setRewardRatePerDay(5, 1500);

        setRewardExtraRate(0, 0); //0.1%
        setRewardExtraRate(1, 10); //0.16%
        setRewardExtraRate(2, 16); 
        setRewardExtraRate(3, 25);
        setRewardExtraRate(4, 40);
    ...
    function claimAutoStakingReward() public {
        uint256 extra = rewardsAccumulated(msg.sender);
    ...
    function rewardsAccumulated(address _account) public view returns(uint256){
        uint256 power = 0;
        uint256 one = 1 ether;
    ...
    function unlock(uint32 _index) public {
        ...
        uint256 power = 0;
        uint256 one = 1 ether;
        (uint256 rewardAmountPerDay, uint256 rewardExtraPerDay) = calcRewards(msg.sender);
        uint256 multiplier = one * (rewardAmountPerDay + (duration - 6) * rewardExtraPerDay) / rewardYieldDenominator;
    ...
    function calcRewards(address _account) public view returns(uint256, uint256){
        
        uint256 rewardAmountPerDay = rewardRatePerDay[0];
        uint256 rewardExtraPerDay = rewardExtraRate[0];
        if(nftAddress!=address(0x0)){
            if(INft(nftAddress).balanceOf(_account, 0) > 0){
                rewardAmountPerDay = rewardRatePerDay[1];
                rewardExtraPerDay = rewardExtraRate[1];
            }
            if(INft(nftAddress).balanceOf(_account, 1) > 0){
                rewardAmountPerDay = rewardRatePerDay[2];
                rewardExtraPerDay = rewardExtraRate[2];
            }
            if(INft(nftAddress).balanceOf(_account, 2) > 0){
                rewardAmountPerDay = rewardRatePerDay[3];
                rewardExtraPerDay = rewardExtraRate[3];
            }
            if(INft(nftAddress).balanceOf(_account, 3) > 0){
                rewardAmountPerDay = rewardRatePerDay[4];
                rewardExtraPerDay = rewardExtraRate[4];
            }
            if(INft(nftAddress).balanceOf(_account, 4) > 0){
                rewardAmountPerDay = rewardRatePerDay[5];
                rewardExtraPerDay = rewardExtraRate[5];
            }
        }
        
        return (rewardAmountPerDay, rewardExtraPerDay);
    }
    ...
    function setRewardExtraRate(uint8 _rarityId, uint256 _rate) public onlyOwner{
        require(_rarityId <= 5, "NFT rarityId must be less than 5");
        require(_rate < 1000, "Cant greater than 100% ");
        rewardExtraRate[_rarityId] = _rate;
    }
