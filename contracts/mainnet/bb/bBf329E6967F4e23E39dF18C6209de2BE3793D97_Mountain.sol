...
contract Mountain is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the Mountain ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Mountain", "Mountain") {
        // Mints 1 Mountain to contract creator for initial pool setup
    ...
    function _getMountainPrice() internal view returns (uint256 _MountainPrice) {
        try IOracle(MountainOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Mountain: failed to fetch Mountain price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _MountainPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_MountainPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMountainOracle(address _MountainOracle) public onlyOperatorOrTaxOffice {
        require(_MountainOracle != address(0), "oracle address cannot be 0 address");
        MountainOracle = _MountainOracle;
    }
    ...
    /**
     * @notice Operator mints Mountain to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Mountain to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMountainPrice = _getMountainPrice();
            currentTaxRate = _updateTaxRate(currentMountainPrice);
            if (currentMountainPrice < burnThreshold) {
                burnTax = true;
            }
        }
