...
/// @title Xptp
/// @author Vector Team
/// @notice Xptp is a token minted when 1 ptp is staked in the Vector protocol
contract xPTPDepositor is Ownable {
    ...
    function maxAmountForSlippage(uint256 slippage) public view returns(uint256){
        (uint256 r0, uint256 r1, ) = IJoePair(pair).getReserves();
        return (IJoePair(pair).token0() == xptp ? r1 : r0) * slippage/(DENOMINATOR-slippage);
    ...
    function deposit(uint256 _amount) external {
        ...
        if (xPTPRatio <= buyThreshold && buyThreshold < DENOMINATOR) {
            uint256 amountToBuy = (amountToConvert * buyPercent) / DENOMINATOR;
            uint256 maxAmountToRespectSlippage = maxAmountForSlippage(slippagePct);
            if (amountToBuy > maxAmountToRespectSlippage){
                amountToBuy = maxAmountToRespectSlippage;
            }
            if (amountToBuy < estimateOutAmount(amountToBuy)) {
                IERC20(ptp).approve(router, amountToBuy);
                address[] memory path = new address[](2);
                path[0] = ptp;
                path[1] = xptp;
                uint256 obtainedPtpAmount = IJoeRouter01(router)
        ...
        IERC20(xptp).safeTransfer(msg.sender, amountToConvert);
    }
}
