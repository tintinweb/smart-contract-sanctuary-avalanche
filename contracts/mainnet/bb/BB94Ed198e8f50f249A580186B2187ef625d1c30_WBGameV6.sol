    ...
    ReentrancyGuardUpgradeable
{
    ...
    address public aCollection;
    address public bCollection;

    event TransferRewardCHRO(address caller, address receiver, uint256 amount);

    function initialize() public initializer {}

    function setACollection(address aCollection_) external onlyOwner {
        _aCollection = IERC721(aCollection_);
        aCollection = address(aCollection_);
    }

    function setBCollection(address bCollection_) external onlyOwner {
        _bCollection = IERC721b(bCollection_);
        bCollection = bCollection_;
    }
    ...
    /**
     * @notice Transfer CHRO reward to user
     * @param _address Address to transfer
     * @param _amount Amount to transfer (in wei)
     */
    function transferRewardCHRO(address _address, uint256 _amount)
        public
        onlyExecutor
    {
        _tokenContract.transfer(_address, _amount);

        emit TransferRewardCHRO(_msgSender(), _address, _amount);
    }
    ...
    function breed(
        ...
        uint256 childTokenId;
        if (_holdEnum == 2) {
            childTokenId = _bCollection.mint(
                _msgSender(),
