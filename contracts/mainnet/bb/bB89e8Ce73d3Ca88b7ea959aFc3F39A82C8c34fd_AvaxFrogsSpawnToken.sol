...
contract AvaxFrogsSpawnToken is ERC20Permit, Ownable {
    using SafeMath for uint256;
    ...
    address public rewardsContract = address(0);
    address public pair;
    address public treasuryWallet = 0xCdaaE81Cd554DDf647031cf2787a8AfB37c6e4aE;

    uint256 public sellFeePercentage = 100000; // 100000 == 10%
    uint256 public burnAmount = 0;
    uint256 public buildTimestamp = 0;
    uint256 public constant MAX = ~uint256(0);

    mapping(address => bool) private allowedTraders;

    bool public tradingEnabled = false;

    constructor() ERC20("Avax Frogs Spawn", "SPAWN", 9) {
        // set TJoe Router
        ...
        // Create the LP and set address for pair
        pair = IFactory(factoryAddress).createPair(MIM, address(this));

        // owner pays no fees
        isFeeExempt[msg.sender] = true;

        // contract pays no fees
        isFeeExempt[address(this)] = true;

        // time stamp for creation
        buildTimestamp = block.timestamp;

        // set allowance for owner to max
        _allowances[msg.sender][address(routerAddress)] = MAX;

        // mint 3,000,000
        _mint(msg.sender, 3000000 * 10**9);
    }

    function addTrader(address _trader) external onlyOwner {
        allowedTraders[_trader] = true;
    }
    ...
    function setRewardContract(address contract_) external onlyOwner {
        rewardsContract = contract_;
    }
    ...
    function _transferFrom(
        ...
        require(canTrade(sender), "Error: Trading not enabled yet");
        ...
        if (shouldTakeFee(sender)) {
            ...
            totalFee = (isSell)
                ? amount.mul(sellFeePercentage).div(1000000)
                : totalFee;    

            uint256 treasuryFee = totalFee.div(2);
            uint256 rewardsFee = totalFee.div(2);

            if (totalFee > 0) {
                // Update the balances
                _basicTransfer(sender, address(treasuryWallet), treasuryFee);
                _basicTransfer(sender, address(rewardsContract), rewardsFee);

                // Adjust the amount to send
                amount = amount.sub(totalFee);
            }

            // transfer the adjusted balance
    ...
    function canTrade(address sender) internal view returns (bool) {
        return (tradingEnabled) ? true : (allowedTraders[sender]);
    }

    function setSellFee(uint256 _input) external onlyOwner {
