...
import "../interfaces/tokens/IGM.sol";
import "../interfaces/tokens/IesGM.sol";
import "../interfaces/tokens/ITokenManager.sol";
import "../interfaces/tokens/IesGMUsage.sol";

/*
 * esGM is Steadefi's escrowed governance token obtainable by converting GM to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to GM through a vesting process
 * This contract is made to receive esGM deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract TokenManager is Ownable, ReentrancyGuard, ITokenManager {
  using Address for address;
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  IGM public immutable GM; // GM token to convert to/from
  IesGM public immutable esGM; // esGM token to convert to/from
  ...
  uint256 public minRedeemDuration = 1 minutes; // 60s
  uint256 public maxRedeemDuration = 5 minutes; // 300s

  /* ========== STRUCTS ========== */

  struct EsGMBalance {
    uint256 allocatedAmount; // Amount of esGM allocated to a Usage
    uint256 redeemingAmount; // Total amount of esGM currently being redeemed
  }

  struct RedeemInfo {
    uint256 GMAmount; // GM amount to receive when vesting has ended
    uint256 esGMAmount; // esGM amount to redeem
    uint256 endTime;
  }
  ...
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating esGM

  uint256 public constant MAX_FIXED_RATIO = 100; // 100%

  /* ========== MAPPINGS ========== */

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate esGM
  mapping(address => mapping(address => uint256)) public override usageAllocations; // Active esGM allocations to usages
  mapping(address => EsGMBalance) public esGMBalances; // User's esGM balances
  ...
  event Redeem(address indexed userAddress, uint256 esGMAmount, uint256 GMAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 esGMAmount, uint256 GMAmount);
  event CancelRedeem(address indexed userAddress, uint256 esGMAmount);
  ...
  constructor(IGM _GM, IesGM _esGM) {
    GM = _GM;
    esGM = _esGM;
  }
  ...
  /**
   * Returns user's esGM balances
   */
  function getEsGMBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    EsGMBalance storage balance = esGMBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /**
   * returns redeemable GM for "amount" of esGM vested for "duration" seconds
   */
  function getGMByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 GMAmount, uint256 esGMAmount, uint256 endTime) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.GMAmount, _redeem.esGMAmount, _redeem.endTime);
  }

  /**
   * returns approved esGM to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * returns allocated esGM from "userAddress" to "usageAddress"
   */
  ...
  /**
   * Convert caller's "amount" of GM to esGM
   */
  ...
  /**
   * Convert caller's "amount" of GM to esGM to "to" address
   */
  ...
  /**
   * Approves "usage" address to get allocations up to "amount" of esGM from msg.sender
   */
  function approveUsage(IesGMUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * Initiates redeem process (esGM to GM)
   */
  function redeem(uint256 esGMAmount, uint256 duration) external nonReentrant {
    require(esGMAmount > 0, "redeem: esGMAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    IERC20(address(esGM)).safeTransferFrom(msg.sender, address(this), esGMAmount);
    EsGMBalance storage balance = esGMBalances[msg.sender];

    // get corresponding GM amount
    uint256 GMAmount = getGMByVestingDuration(esGMAmount, duration);
    emit Redeem(msg.sender, esGMAmount, GMAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount + (esGMAmount);

      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(GMAmount, esGMAmount, _currentBlockTimestamp() + (duration)));
    } else {
      // immediately redeem for GM
      _finalizeRedeem(msg.sender, esGMAmount, GMAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount - (_redeem.esGMAmount);
    _finalizeRedeem(msg.sender, _redeem.esGMAmount, _redeem.GMAmount);
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming esGM available again
    balance.redeemingAmount = balance.redeemingAmount - (_redeem.esGMAmount);
    IERC20(address(esGM)).safeTransfer(msg.sender, _redeem.esGMAmount);

    emit CancelRedeem(msg.sender, _redeem.esGMAmount);
  ...
  /**
   * Allocates caller's "amount" of available esGM to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates esGM to usageContract
    IesGMUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * Allocates "amount" of available esGM from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required esGM esGM from "userAddress"
   */
  ...
  /**
   * Deallocates caller's "amount" of available esGM from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate esGM into usageContract
    IesGMUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * Deallocates "amount" of allocated esGM belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate esGM from itself
   */
  ...
  /**
   * Convert caller's "amount" of GM into esGM to "to"
   */
  function _convert(uint256 amount, address to) internal {
    ...
    IERC20(address(GM)).safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * Finalizes the redeeming process for "userAddress" by transferring him "GMAmount" and removing "esGMAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * GM excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 esGMAmount, uint256 GMAmount) internal {
    uint256 GMExcess = esGMAmount - (GMAmount);

    // sends due GM tokens
    IERC20(address(GM)).safeTransfer(userAddress, GMAmount);

    // burns GM excess if any
    // TODO relook into burn mechanism and token approval/flows
    GM.burn(address(this), GMExcess);
    esGM.burn(address(this), esGMAmount);

    emit FinalizeRedeem(userAddress, esGMAmount, GMAmount);
  }

  /**
   * Allocates "userAddress" user's "amount" of available esGM to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedEsGM = usageApprovals[userAddress][usageAddress];
    require(approvedEsGM >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedEsGM - (amount);
    ...
    // adjust user's esGM balances
    balance.allocatedAmount = balance.allocatedAmount + (amount);
    IERC20(address(esGM)).safeTransferFrom(userAddress, address(this), amount);

    emit Allocate(userAddress, usageAddress, amount);
  }

  /**
   * Deallocates "amount" of available esGM to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated esGM to this usage to deallocate
    ...
    EsGMBalance storage balance = esGMBalances[userAddress];
    balance.allocatedAmount = balance.allocatedAmount - (amount);
    IERC20(address(esGM)).safeTransfer(userAddress, amount - (deallocationFeeAmount));
    // burn corresponding GM and esGM
    // TODO relook into burn mechanism and token approval/flows
    GM.burn(address(this), deallocationFeeAmount);
    esGM.burn(address(this), deallocationFeeAmount);
