...
contract StrategStepAaveV3Deposit is StrategStep {
    ...
    function oracleExit(IStratStep.OracleResponse memory _before, bytes memory _parameters) external view returns (OracleResponse memory) {
        ...
        if(amountToWithdraw == 0) {
            amountToWithdraw = IERC20(parameters.aToken).balanceOf(before.vault);
            before = _addTokenAmount(parameters.aToken, amountToWithdraw, before);
        }

        IStratStep.OracleResponse memory _after = _removeTokenAmount(parameters.aToken, amountToWithdraw, before);
