/**
 *Submitted for verification at snowtrace.io on 2023-06-05
*/
...
contract FarmProRata is Ownable {
    ...
    address public constant trustedDepositTokenAddress = 0x66EeCC97203704d9E2Db4A431cb0e9cE92539D5a; // uniswap pair address
    ...
    address public constant trustedStakingContractAddress = 0x6a258Bd17456e057A7c6102177EC2f9d64D5F9e4;
    ...
    // Amount of tokens
    uint public constant disburseAmount = 48000000e18;
    ...
    uint public MAGIC_NUMBER = 2025125628140614;
    ...
    function doSwap(address fromToken, address toToken, uint fromTokenAmount, uint amountOutMin, uint deadline) 
        ...
        if (fromToken == uniswapRouterV2.WAVAX() || toToken == uniswapRouterV2.WAVAX()) {
            path = new address[](2);
            path[0] = fromToken;
            path[1] = toToken;
        } else {
    ...
    function updateAccount(
        ...
        if (pendingDivsEth > 0) {
            ...
            if (claimAsToken == address(0) || claimAsToken == uniswapRouterV2.WAVAX()) {
                IERC20(uniswapRouterV2.WAVAX()).safeTransfer(account, pendingDivsEth);
            } else {
                require(trustedClaimableTokens[claimAsToken], "cannot claim as this token!");
                
                IERC20(uniswapRouterV2.WAVAX()).safeApprove(address(uniswapRouterV2), 0);
                IERC20(uniswapRouterV2.WAVAX()).safeApprove(address(uniswapRouterV2), pendingDivsEth);
                address[] memory path = new address[](2);
                path[0] = uniswapRouterV2.WAVAX();
                path[1] = claimAsToken;
    ...
    function deposit(
        ...
        uint _80Percent = amountAfterFee.mul(80e2).div(100e2);
        uint _20Percent = amountAfterFee.sub(_80Percent);

        uint amountToDepositByContract = doSwap(depositToken, trustedPlatformTokenAddress, _20Percent, /*_amountOutMin_25Percent*/minAmounts[0], _deadline);

        IERC20(trustedPlatformTokenAddress).safeTransferFrom(address(this), address(trustedStakingContractAddress), amountToDepositByContract);

        uint half = _80Percent.div(2);
        uint otherHalf = _80Percent.sub(half);
    ...
    function attemptSwap(uint _amountOutMin_attemptSwap, uint _deadline) private {
        ...
        uint oldWethBalance = IERC20(uniswapRouterV2.WAVAX()).balanceOf(address(this));
        
        uniswapRouterV2.swapExactTokensForTokens(_tokensToBeSwapped, _amountOutMin_attemptSwap, SWAP_PATH, address(this), _deadline);
    
        uint newWethBalance = IERC20(uniswapRouterV2.WAVAX()).balanceOf(address(this));
