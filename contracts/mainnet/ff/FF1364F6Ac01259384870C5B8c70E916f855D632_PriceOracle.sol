...
contract PriceOracle is Epoch {
    ...
    constructor(IUniswapV2Pair _pair, uint256 _period) public Epoch(_period, block.timestamp, 0) {
        pair = _pair;
        token0 = pair.token0();
        token1 = pair.token1();
    ...
    function update() external checkEpoch {
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(
            address(pair)
        );
