    ...
    RoleAware
{
        ...
        external
    {
        ...
        address sourceLendingAddress = trancheIdService().viewTrancheContractByID(trancheId);
        require(
        ...
        uint256 yield = stable.balanceOf(address(this));
        if (yield > posMeta.debt) {
            stable.mint(sourceLending.ownerOf(trancheId), yield);
        }
        uint256 newTrancheId = lending2.mintDepositAndBorrow(
            posMeta.token,
            targetStrategy,
            collateralValue,
            posMeta.debt > yield ? (995 * (posMeta.debt - stable.balanceOf(address(this)))) / 1000 : 0,
            address(this)
        );
