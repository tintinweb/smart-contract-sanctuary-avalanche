...
contract nebulaFeeManager is Ownable{
	...
	constructor(address[] memory _addresses,address payable _treasury,uint256[] memory times,bool _feeFroze){
		...
		uint256[6] memory ls = [feePeriod,gracePeriod,protoLife,maxFeePayment,maxPayPeriods,protoLife];
		for(uint i=0;i<5;i++){
			ls[i] = times[i];
		}
		feeFroze = _feeFroze;
	...
	function reconcileAccountFees(address _account) external onlyOwner(){
	...
	function addProto(address _account,string memory _name) external onlyGuard(){
		...
		_protoowners.push(PROTOOWNERS({name:_name,protoCreationDate:INTgetCurrentTime(_account),protoElapsed:Zero,protoCollapseDate:INTgetCurrentTime(_account).add(protoLife),protoNextDue:feePeriod,protoDueElapsed:Zero,protoFutureFees:11,protoIncreasedDecay:Zero,protoFeeFroze:_protoFeeFroze,protoTotalPayable:maxPayPeriods.sub(One)}));
