...
contract TheCollectionQ12023 is ERC721, ERC721Enumerable, AccessControl, ERC2981, EIP712, ERC721Votes  {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address public ADMIN = 0x53877dF168057a513108039D3D8c4dFDf6b0aBD1;
    address public DAO = 0x5E069c8E72b7A0b3F18cB262d2cbc7461d2B0270;
    address public owner = 0x53877dF168057a513108039D3D8c4dFDf6b0aBD1;

    string public revealedUri;

    uint96 public tokenRoyalties = 690; // 6.9% royalty
    address public royaltyPayout;

    bool public mintOpen = true;
    uint256 public startTime = 1681153200;
    ...
    constructor() ERC721("The Community Collection: Q1 2023", "CCQ123") EIP712("The Community Collection: Q1 2023", "1") {
        _grantRole(DEFAULT_ADMIN_ROLE, ADMIN);
        _tokenIdCounter.increment();

        _setDefaultRoyalty(DAO, tokenRoyalties);
        royaltyPayout = DAO;
        
    }

    function mint(uint256 amount) public payable callerIsUser{
        require (mintOpen == true, "mint closed");
        require (block.timestamp > startTime, "too early");
        require (!(msg.value < amount*price), "send more redcoin");
        mintedAmount[msg.sender] += amount;
        require(!(mintedAmount[msg.sender] > 20), "already minted 20");
        require(!(_tokenIdCounter.current() + amount > maxSupply + 1), "Sold Out");
   ...
   // METADATA
    ...
    function setRevealed(string memory _revealed) public onlyRole(DEFAULT_ADMIN_ROLE) {
        revealedUri = _revealed;
    }
    ...
    function setTokenRoyalties(uint96 _royalties) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenRoyalties = _royalties;
        _setDefaultRoyalty(royaltyPayout, tokenRoyalties);
    }

    function setRoyaltyPayoutAddress(address _payoutAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        royaltyPayout = _payoutAddress;
        _setDefaultRoyalty(royaltyPayout, tokenRoyalties);
    }


    function rescueAvax() payable external onlyRole(DEFAULT_ADMIN_ROLE){ 
    ...
    function openSale() external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintOpen = true;
    }

    function closeSale() external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintOpen = false;
    }
