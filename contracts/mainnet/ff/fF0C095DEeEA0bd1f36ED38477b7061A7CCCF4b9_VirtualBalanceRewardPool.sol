...
/**
 * @notice contract in charge of aditional rewards distribution.
 * no token can be staked into it, the staking mechanism is based on the token staked into a RewardPool.
 */
    ...
    UUPSUpgradeable
{
    ...
    event ExtraRewardAdded(uint256 reward);
    ...
    function initialize(
        ...
        assert(address(deposits) == address(0x0));
        deposits = IRewardPool(deposit_);
        ...
        returns (bool)
    {
        ...
        uint256 depositTotalSupply = deposits.totalSupply();

        if (depositTotalSupply == 0) {
        ...
        emit ExtraRewardAdded(_rewards);
