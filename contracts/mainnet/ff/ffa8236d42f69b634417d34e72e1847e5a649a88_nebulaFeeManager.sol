...
contract nebulaFeeManager is Ownable{
    ...
    function transferAllBatch(uint start,bool _plus) external onlyOwner(){
    	uint length = getPrevAccountsLength();
    	transferAll(start,length,_plus);
    }
    function transferAllSpec(uint _start,uint _intervals,bool _plus) external onlyOwner(){
    	transferAll(_start,_intervals,_plus);
    }
function transferAll(uint _start,uint _intervals,bool _plus) internal{
    ...
    function plus(address _account) internal {
    		recPayFees(_account);
