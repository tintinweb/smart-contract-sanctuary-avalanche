...
contract SDG is ERC721 {
    ...
    constructor()
        ERC721("Sadgirlsnft", "SDG")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 115) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiaikz35njt25giyhdjhkvzfxjcykddvohpzcgndcexyw5ttptm4k4/", Strings.toString(tokenId), ".json")
            );
    }
