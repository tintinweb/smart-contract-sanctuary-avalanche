...
contract MyChance is Ownable, VRFConsumerBaseV2, Pausable, KeeperCompatibleInterface {
    ...
    IPrizeBond prizeBond = IPrizeBond(0x99843eD5966967c831662e6Ad64aC2Fbf90Dff64);
    uint256[] prizeBonds;
    ...
    constructor() VRFConsumerBaseV2(vrfCoordinator) {
        ...
        lastDrawn = block.timestamp - TIME_FOR_NEXT_DRAW;
    ...
    function drawSpecialLottery(uint256 _drawDate) public whenNotPaused {
     ...
     function claim(uint256 _tokenId, uint256 _percentage) public {
        ...
        uint256 totalWinnerDAI;
        uint256 totalWinnerUSDT;
        uint256 totalWinnerUSDC;
        ...
        if (totalDAI > 0) {
            totalToCharityDAI = totalDAI * 20 / 100;
            totalFeesDAI = totalDAI * 45 / 100;
            totalWinnerDAI = totalDAI * 35 / 100;
        ...
        if (totalUSDT > 0) {
            totalToCharityUSDT = totalUSDT * 20 / 100;
            totalFeesUSDT = totalUSDT * 45 / 100;
            totalWinnerUSDT = totalUSDT * 35 / 100;
        ...
        if (totalUSDC > 0) {
            totalToCharityUSDC = totalUSDC * 20 / 100;
            totalFeesUSDC = totalUSDC * 45 / 100;
            totalWinnerUSDC = totalUSDC * 35 / 100;
        ...
        emit AssetsClaimed(msg.sender, _tokenId, totalWinnerDAI, totalToCharityDAI, totalFeesDAI, totalWinnerUSDT, totalToCharityUSDT, totalFeesUSDT, totalWinnerUSDC, totalToCharityUSDC, totalFeesUSDC);
    }
    ...
    function mintPrizeBond(IPrizeBond.Assets _assetType, uint weight) public whenNotPaused {
        ...
        if (_assetType == IPrizeBond.Assets.DAI) {
            ...
            lendingPool.supply(daiToken, cost, address(this), 0);
            ...
            lendingPool.supply(usdcToken, cost, address(this), 0);
            ...
            lendingPool.supply(usdtToken, cost, address(this), 0);
    ...
    function _addSpecialLottery(uint256 _drawDate, IPrizeBond.Assets _assetType, uint256 _total, string memory _description) public onlyOwner whenNotPaused {
        ...
        lendingPool.supply(token, _total, address(this), 0);
    ...
    function _updateFeeCollector(address _feeCollector) public onlyOwner {
    ...
    function _claimFees() public {
        ...
        if (daiFees > 0) {
            lendingPool.withdraw(daiToken, daiFees, feeCollector);
            pendingDAI-=daiFees;
        }

        if (usdtFees > 0) {
            lendingPool.withdraw(usdtToken, usdtFees, feeCollector);
            pendingUSDT-=usdtFees;
        }

        if (usdcFees > 0) {
            lendingPool.withdraw(usdcToken, usdcFees, feeCollector);
