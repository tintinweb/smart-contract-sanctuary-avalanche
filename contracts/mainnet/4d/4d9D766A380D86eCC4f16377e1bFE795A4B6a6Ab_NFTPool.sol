...
// File contracts/glad_core/roman_nft/IPool.sol
...
contract NFTPool is Controllable, IERC721Receiver, Pausable {
    using SafeERC20 for IERC20;
    ...
    bool public gshareRewardEnabled = true;
    ...
    event TokenStaked(address indexed owner, uint256 indexed tokenId, bool isForGshare, bool isPrey);
    event TokenClaimed(uint256 indexed tokenId, bool isPrey);
    event BarnRewardClaimed(address indexed user, bool isForGshare, uint256 amount, uint256 tax);
    event PackRewardClaimed(address indexed user, bool isForGshare, uint256 amount);
    event UserStakeChange(address user, UserStake gladshare, UserStake glad);
    ...
    ) external whenNotPaused beforeDeadline updateEarnings(msg.sender) {
    ...
    function getGshareRewardPerPrey() public view returns (uint256 amount) {
        ...
        if (!gshareRewardEnabled) return totalGshareRewardPerPrey;
    ...
    function setGladshareRewardEnabled(bool _enabled) external onlyOwner {
        gshareRewardEnabled = _enabled;
    }
     ...
     */
    function _alphaForPredator(uint256 tokenId) internal view returns (uint256) {
        IPreyPredator.PreyPredator memory iPreyPredator = preyPredator
            .getTokenTraits(tokenId);
