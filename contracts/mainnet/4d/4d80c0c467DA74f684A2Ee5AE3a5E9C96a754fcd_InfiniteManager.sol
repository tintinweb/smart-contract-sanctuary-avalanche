...
contract InfiniteManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    ...
    address public metaManager = 0x000000000000000000000000000000000000dEaD;
    ...
    IPlatinumManager public platinumManager;


    constructor(address _ASCEND_TOKEN, address _ascendMembership, address _platinumMembership)  {
        ASCEND = IERC20(_ASCEND_TOKEN);
        manager = IAscendMembershipManager(_ascendMembership);
        platinumManager = IPlatinumManager(_platinumMembership);
    }
    ...
    function updateMetaManager (address _metaManager) external onlyOwner {
    ...
    function _addInfinite(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createInfinite(_id, _name, _mint, _claim, _metadata, _to);
    }
    ...
    function setPlatinumManager (IPlatinumManager _platinumManager) external onlyOwner {
        platinumManager = IPlatinumManager(_platinumManager);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "Infinite Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory ascendNodes, uint64[] memory userPlatinumNodes) public {
        ...
        require(platinumManager.balanceOf(sender) >= nodePlatinumPrice, "HELPER: Platinum Membership balance too low for creation.");
        ...
        for (uint256 i = 0; i < nodePlatinumPrice; i++) {
            platinumManager.transferFrom(_msgSender(), address(0), userPlatinumNodes[i]);
        }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override {
        ...
        if (transferIsEnabled == true){
            ...
            require(_msgSender() == metaManager, "ERC721: transfer to non ERC721Receiver implementer");
            _safeTransfer(from, to, tokenId, _data);
        }
    }
    ...
    if (transferIsEnabled == true){
            ...
            require (_msgSender() == metaManager, "Infinite Transfers are not allowed");
            _transfer(from, to, tokenId);
            require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
        }
        _transfer(from, to, tokenId);
