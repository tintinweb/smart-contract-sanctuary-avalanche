...
// File contracts/ArturrXDelta.sol
...
contract ArturrXDelta is ERC721, ERC721URIStorage, Ownable, RoyaltiesV2Impl {
    ...
    function setSuffixURI(string memory _newSuffix) public onlyOwner {
        suffix = _newSuffix;
    }

    function suffixURI() public view returns (string memory) {
        return suffix;
    }
    ...
    function safeMint(address to) public onlyOwner {
        require(isSaleActive, "Please, activate sale");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < getMintLimit(), "Limit has been reached");
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }

    function setActiveSale(bool active) public onlyOwner {
        isSaleActive = active;
    }
    ...
    function _suffixURI() internal view returns (string memory) {
        return suffix;
    }
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(_baseURI(), tokenId.toString(), suffixURI())
            );
    }
