...
contract DistrictManager is Ownable, HelperOwnable, IERC721, IERC721Metadata {
    DistrictManager public DISTRICTS_V2 = DistrictManager(0xEb92b856a9Fb9F7eA6530eaB40F56C6dA616eC7d);
    ...
    function totalNodesCreated() view external returns (uint) {
        return nodeCounter + DISTRICTS_V2.totalNodesCreated();
    }
    ...
    function migrate() external {
        ...
        uint256 len = DISTRICTS_V2.balanceOf(msg.sender);
        uint256[] memory nodeIds = DISTRICTS_V2.getDistrictsOf(msg.sender);
        for (uint16 i = 0; i < len; i++) {
            (string memory _name, string memory _metadata, uint256 _id, uint64 _mint, uint64 _claim) = (DISTRICTS_V2.getDistricts(nodeIds[i]).name, DISTRICTS_V2.getDistricts(nodeIds[i]).metadata, DISTRICTS_V2.getDistricts(nodeIds[i]).id, DISTRICTS_V2.getDistricts(nodeIds[i]).mint, DISTRICTS_V2.getDistricts(nodeIds[i]).claim);
            _nodes[_id] = District({
                name: _name,
                mint: _mint,
                claim: _claim,
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter + DISTRICTS_V2.totalNodesCreated();
