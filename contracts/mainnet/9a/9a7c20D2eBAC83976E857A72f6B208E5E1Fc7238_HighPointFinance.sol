...
contract HighPointFinance is ERC20Detailed, Ownable, MinterRole {
    ...
    //Solidity response begin
    //Remove unused feesOnNormalTransfers, isBurnEnabled, and isLiquidityInAvax variables for finding #7
    //The finding is asking for the variables to be labelled constant, but they're unused anyways, so better to remove them entirely
    //Solidity response end
    ...
    //Solidity response edit begin
    //Removed the unused sellFeeLiquidityAdded variable for finding #5
    //Solidity response edit end
    uint256 public sellFeeTreasuryAdded = 7;
    uint256 public sellMaintenanceFee = 3;
    //Solidity response edit begin
    //Changed variable to constant for finding #7
    uint256 public constant sellLaunchFeeAdded = 30;
    //Solidity response edit end
    ...
    uint256 public rewardYield = 34218;
    ...
    //Solidity response edit begin
    //Change variables to constant for finding #7
    ...
    //Solidity response edit begin
    //Changed the function from public to external for finding #6
    ...
    function coreRebase(uint256 epoch, int256 supplyDelta) private returns (uint256) {
        //Solidity response edit begin
        //Added "|| msg.sender != pair" in the if-condition for finding #1
        if (supplyDelta == 0 || msg.sender != pair) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToMaintenance = contractTokenBalance
        .mul(maintenanceFee.mul(2).add(sellMaintenanceFee))
        .div(realTotalFee);

        //Solidity response edit begin
        //Added .sub(amountToMaintenance) for finding #3
        uint256 amountToTreasury = contractTokenBalance
        .sub(amountToLiquify)
        .sub(amountToRFV)
        .sub(amountToMaintenance);
        //Solidity response edit end
        ...
        if (amountToMaintenance > 0) {
            _swapTokensForStableCoin(amountToMaintenance, maintenanceValueReceiver);
        }
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        //Solidity response edit begin
        //Removed if-else conditions for finding #4
    ...
    function setFees(
        ...
        require(
            ...
            _maintenanceFee <= MAX_FEE_RATE &&
            _sellFeeTreasuryAdded <= MAX_FEE_RATE &&
            _sellMaintenanceFee <= MAX_FEE_RATE &&
