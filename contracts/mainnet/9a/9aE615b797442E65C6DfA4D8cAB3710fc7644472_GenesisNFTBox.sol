...
contract GenesisNFTBox is ERC721Burnable, ERC721Rentable, ERC721URIStorage, ERC721Permit, IGenesisNFTBox, Beneficiary, Creator, NonFungibleType, Withdrawable, ReentrancyGuard {
    using Helper for uint256;
    using Helper for address;

    ///@dev value is equal to keccak256("GenesisNFTBox_v1")
    bytes32 public constant VERSION = 0x6928f9a5f035bc89775846504fd0d5ac61ba3c7a03f8456bc4a553700fce32ce;
    ...
    constructor(
        ...
        address creator_
    ) payable ERC721Rentable(name_, symbol_) Signable(type(GenesisNFTBox).name, version_) {
        _setBeneficiary(beneficiary_);
        _setCreator(creator_);
        _setType(1, 0x0000000000000000000000000000000000000000, 100000000000000, 1_000_000, 1);
        _setType(2, 0x0000000000000000000000000000000000000000, 200000000000000, 1_000_000, 1);
        _idCounter = 1;
    }
    ...
    function buyNFT(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
        ...
        if (total == 0) revert GenesisNFTBox__InvalidType();
