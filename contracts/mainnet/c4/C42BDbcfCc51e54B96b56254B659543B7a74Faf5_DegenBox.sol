...
interface IStrategy {
    ...
    function harvest(address strategyToken, uint256 balance, address sender) external returns (int256 amountAdded);
    ...
    function strategyPool(address strategyToken) external view returns (address cauldron, address oracle, uint256 pid);
}
...
contract DegenBox is MasterContractManager, BoringBatchable {
    ...
    uint256 private constant STRATEGY_DELAY = 10 seconds;
    ...
    function setStrategyTargetPercentage(IERC20 token, uint64 targetPercentage_) public {
        if(address(strategy[token]) != address(0)){
            (address cauldron, , ) = IStrategy(address(strategy[token])).strategyPool(address(token));
            require(msg.sender == owner || msg.sender == cauldron, "setStrategyTargetPercentage: caller != owner");
        }else{
            require(msg.sender == owner, "setStrategyTargetPercentage: caller != owner");
        }
        // Checks
    ...
    function setStrategy(IERC20 token, IStrategy newStrategy) public onlyOwner {
        ...
        if (data.strategyStartDate == 0 || pending != newStrategy) {
            ...
            if (address(strategy[token]) != address(0)) {
                int256 balanceChange = strategy[token].exit(address(token), data.balance);
                // Effects
                if (balanceChange > 0) {
    ...
    function harvest(
        ...
        int256 balanceChange = _strategy.harvest(address(token), data.balance, msg.sender);
        ...
        if (balance) {
            ...
            if (data.balance < targetBalance) {
                ...
                _strategy.skim(address(token), amountOut);
                ...
                uint256 actualAmountIn = _strategy.withdraw(address(token), amountIn);
