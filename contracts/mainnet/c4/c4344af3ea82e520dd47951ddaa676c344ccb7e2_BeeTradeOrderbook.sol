...
contract BeeTradeOrderbook {
    ...
    function depositToken(address _token, uint256 _amount) external {
        ...
        // handle tokens that have different decimals
        uint8 decimals = IERC20(_token).decimals();
        uint256 tokenAmount = _amount * (10 ** decimals);
        uint256 contractAmount = _amount * (10 ** 18);

        require(IERC20(_token).transferFrom(msg.sender, address(this), tokenAmount), "Beetrade: Transfer Failed");
        tokensBalances[_token][msg.sender].available += contractAmount;
        emit Deposit(_token, msg.sender, contractAmount);
    }
    ...
    function withdrawToken(address _token, uint256 _amount) external {
        ...
        require(tokensBalances[_token][msg.sender].available >= contractAmount, "Beetrade: Insufficient Balance");
        tokensBalances[_token][msg.sender].available -= contractAmount;
        IERC20(_token).transfer(msg.sender, tokenAmount);
        emit Withdraw(_token, msg.sender, contractAmount);
    }
