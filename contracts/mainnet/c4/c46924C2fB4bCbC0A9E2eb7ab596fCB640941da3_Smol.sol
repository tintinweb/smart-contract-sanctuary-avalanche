...
contract Smol is ERC721Enumerable, Ownable, ReentrancyGuard, ERC2981 {
    uint256 private a;
    uint256 constant public MAX_TURDS = 100;
    uint256 constant public SHIDS = 26;
    uint256 private shids;
    mapping(address => bool) public minted;
    mapping(address => bool) public shidlisted;
    string private _baseTokenURI;

    constructor(address receiver, uint96 feeNumerator)
        ERC721("Smol Turds", "TURD")
    {
        _setDefaultRoyalty(receiver, feeNumerator);
    }

    function turd(uint256 b) public {
        require(
            tx.origin == msg.sender,
            "I don't deal with contracts!"
        );
        require (b == 1337, "you are not the shit");
        require(a == 1, "a is not greater than 0");
        
        require(totalSupply() < MAX_TURDS - SHIDS, "no more turds, I'm sorry");
        require(!minted[msg.sender], "you already minted an turds");
        _mint(msg.sender, totalSupply());
        minted[msg.sender] = true;
    }

    function smol(uint256 _stage) public onlyOwner {
        a = _stage;
    }

    function shid() public {
        require(shidlisted[msg.sender], "you already minted a shid or are not shidlisted");
        require(shids < SHIDS, "max shids minted");
        require(a == 2, "shidlist minting hasn't started");
        shidlisted[msg.sender] = false;
        _mint(msg.sender, totalSupply());
        ++shids;
    }

    function addToShidlist(address[] memory _shidlisted) external onlyOwner {
        for (uint i; i < _shidlisted.length; i++)
        {
            shidlisted[_shidlisted[i]] = true;
        }
    }
