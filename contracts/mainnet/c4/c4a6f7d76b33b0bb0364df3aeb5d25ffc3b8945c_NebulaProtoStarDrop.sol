...
abstract contract feeManager is Context {
    ...
    function getFee(address _account) external virtual view returns(TOTALFEES memory){
    	TOTALFEES storage totals =  totalFees[_account];
    	return totals;
    	}
    function isInsolvent(address _account,string memory _name) external virtual view returns(bool);
    function simpleQuery(address _account) external virtual view returns(uint256);
...
contract NebulaProtoStarDrop is Ownable{
	...
	mapping(address => TOTALFES) public totalFees;
	mapping(address => DROPS) public airdrop;
	...
	constructor(address[] memory _addresses,address payable _treasury){
		...
		treasury = _treasury;
		Managers.push(owner());
		transferOldDropsINT(uint256(0),uint256(2));
	...
	function transferOldDropsINT(uint256  _x, uint256 _y) internal {
		uint256 length = protoMGR.getProtoAccountsLength();
		for(uint256 i=_x;i<_y;i++){
			address _account =protoMGR.getProtoAddress(uint256(i));
			if(nebuLib.addressInList(transfered,_account) == false){
				uint256 stars = protoMGR.getProtoStarsLength(_account);
    				airdrop[_account].claimed += stars;
    				airdrop[_account].dropped += stars;
    				airdrop[_account].transfered += stars;
    				airdrop[_account].feesPaid += stars;
	...
	function addFee(address[] memory _account,uint256[] memory _x) external onlyOwner() {
		for(uint i = 0;i<_account.length;i++){
			if(totalFees[_account[i]].protos == 0){
				airdrop[_account[i]].feesPaid += _x[i];
			}
		}
	}
	
	function transferOldDrops(uint256 _x, uint256 _y) external onlyOwner {
		transferOldDropsINT(_x,_y);
	}
