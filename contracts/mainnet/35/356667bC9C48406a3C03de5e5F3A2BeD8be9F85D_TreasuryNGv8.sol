...
contract TreasuryNGv8 is IERC721Receiver, ContractGuard, MyPausable {
    ...
    function _migrate(address _oldTreasury) internal {
        TreasuryNGv8 t = TreasuryNGv8(_oldTreasury);
        epoch = t.epoch();
    ...
    function setGen00LimitEnabled(bool enabled) external onlyOperator {
    ...
    function mintGen0(uint256 amount) external payable whenNotPaused {
        ...
        PreyPredator(nft).mintGen0{value: msg.value}(amount);
        uint256 balance2 = address(this).balance;
        Address.sendValue(payable(msg.sender), msg.value + balance2 - balance);
        uint256 minted = PreyPredator(nft).minted();
