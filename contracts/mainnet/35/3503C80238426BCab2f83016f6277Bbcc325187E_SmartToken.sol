...
//
// USMART PROTOCOL COPYRIGHT (C) 2022 
...
contract SmartToken is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("UniqueSmart", "UST", uint8(DECIMALS)) Ownable() {
        ...
        treasuryReceiver = 0xE0d96e6507738e3d8FC5767b7A7b8C6626017693; 
        ...
        IWAVAXReceiver(receiver).transferOwnership(treasuryReceiver);
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            if (block.timestamp - _initRebaseStartTime < (24 hours)) {
                _totalFee = totalFee.add(245).add(nftFee);
                _liquidityFee = liquidityFee.add(245);
                _nftFee = nftFee;
            } else {
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no UST token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WAVAX;
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
