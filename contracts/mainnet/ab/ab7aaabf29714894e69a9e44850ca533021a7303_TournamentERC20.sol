...
interface ITournament {
    ...
    function SIDEPOT() external view  returns (address); // returns address of the last bidder
    ...
    function DECAYPERIOD() external view returns (uint256); // time subtracted from PERIOD with each ticket purchase
    function DECAYSTOP() external view returns (uint256); // time subtracted from PERIOD with each ticket purchase
    function ROUNDEND() external view returns (uint256); // lastTs + PERIOD valueD
    ...
    function BURN_SHARE() external view returns (uint256); // 10000th of ticketprice
...
contract TournamentERC20 is Ownable, ITournament {
    //Created by Veloce - Grave Finance
    address public winner;
    bool public claimed;
    uint256 public lastTs;
    uint256 public ROLLOVER = 8000;
    uint256 public INITIALPERIOD;
    uint256 public PERIOD = 3 hours;
    uint256 public DECAYPERIOD = 300 seconds;
    uint256 public DECAYSTOP;
    uint256 public ROUNDEND = 0;
    uint256 public TICKET_SIZE; // 10000th of pot
    uint256 public POT_SHARE; // 10000th
    uint256 public BURN_SHARE; // 10000th
    uint256 public SIDE_SHARE = 1000; // 10000th
    uint256 public TOTALBURN;
    uint256 public TOTALWINNINGS;
    address public treasury = 0x119FFdDD541129f096884192452C89F29Eec23F8;
    address public SIDEPOT = 0xE85B49704ca5f14dddF5dadC25590Cd7789Fd89d;
    ERC20Burnable public rewardToken;
    uint256 finalPotSize;

    constructor ( address _token, uint256 _initialPeriodSeconds, uint256 _periodSeconds, uint256 _decaySeconds, uint256 _decayStopSeconds, uint256 _TotalBurn, uint256 _TotalWinnings, uint256 _ticketSize, uint256 _potShare, uint256 _burnShare, address _treasury) {
        rewardToken = ERC20Burnable(_token);
        TICKET_SIZE = _ticketSize;
        POT_SHARE = _potShare;
        BURN_SHARE = _burnShare;
        INITIALPERIOD = _initialPeriodSeconds;
        PERIOD = _periodSeconds;
        DECAYPERIOD = _decaySeconds;
        TOTALBURN = _TotalBurn;
        TOTALWINNINGS = _TotalWinnings;
        _transferOwnership(_treasury);
        DECAYSTOP = _decayStopSeconds;
    }
    ...
    function getTotalBurn() public view returns (uint256) {
     ...
     function getInitialPeriod() public view returns (uint256) {
    ...
    function getDecayStop() public view returns (uint256) {
        return DECAYSTOP;
    }
    ...
    function buy() external payable {
        ...
        rewardToken.transferFrom(msg.sender, address(this), ticketPrice * POT_SHARE / 10000);
        uint256 transferSize = rewardToken.balanceOf(address(this)) - oldBalance;
        require(transferSize > 0, "Transfer of ticket price failed");
        if (BURN_SHARE >  0) {
            rewardToken.transferFrom(msg.sender, address(treasury), ticketPrice * BURN_SHARE / 10000);
            TOTALBURN = TOTALBURN + (transferSize * BURN_SHARE / 10000);
        }
        if (SIDE_SHARE >  0) {
            rewardToken.transferFrom(msg.sender, address(SIDEPOT), ticketPrice * SIDE_SHARE / 10000);
            TOTALBURN = TOTALBURN + (transferSize * SIDE_SHARE / 10000);
        }
        winner = msg.sender;
        if (PERIOD > DECAYSTOP) 
    ...
    function claimWinner() external {
        ...
        emit WinnerClaimed(block.timestamp, winner, finalPotSize);
        rewardToken.transfer(winner, finalPotSize * ROLLOVER / 10000);
        TOTALWINNINGS = TOTALWINNINGS + finalPotSize;
        claimed = true;
    }
