...
contract RumbleBunnies is ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties, RandomRequest {
    using Strings for uint256;

    uint256 public constant MAX_MINTABLE = 100; //  100 total for this launchpad
    uint256 public constant MAX_PER_CLAIM = 10;
    uint256 public constant ROYALTY_VALUE = 750; // 100 = 1%, 200 = 2% etc...
    uint256 public constant MAX_EARLY_CLAIM = 1;
    uint256 public constant MAX_EARLY_CLAIM_TWO = 1;
    uint256 public constant NFT_EARLY_PRICE = 1 ether; //
    ...
    uint public WLTwoStartAt;    

    bytes32 public merkleRoot;
    bytes32 public merkleRoot_two;
    mapping(address => uint256) earlyMintAddresses;
    mapping(address => uint256) earlyMintAddresses_two;
    ...
    constructor(
        ...
        merkleRoot_two = _merkleRoot_two;
        startAt = _startAt;
        WLstartAt = _WLstartAt;
        WLTwoStartAt = _WLTwoStartAt;
    }
...
//
    ...
    function earlyClaim(uint256 n, bytes32[] calldata _proof) public payable{
        ...
        bytes32 root;
        if(block.timestamp < WLTwoStartAt){
        ...
        else if(block.timestamp >= WLTwoStartAt){
            require(earlyMintAddresses_two[msg.sender] <= MAX_EARLY_CLAIM_TWO, "You can't early claim anymore");
            require(n + earlyMintAddresses_two[msg.sender] <= MAX_EARLY_CLAIM_TWO, "you can't early claim that much");
            root = merkleRoot_two;
        }
        ...
        if(block.timestamp < WLTwoStartAt){
            ...
            earlyMintAddresses_two[msg.sender] += n;
        }
...
//
    ...
    function setMerkleRootTwo(bytes32 _root) external onlyOwner {
            merkleRoot_two = _root;
    }
    ...
    function setWLTwostartAt(uint _WLTwoStartAt) external onlyOwner {
            WLTwoStartAt = _WLTwoStartAt;
    }
...
//
        ...
        returns (uint256)
    {
        ...
        if(block.timestamp < WLTwoStartAt){
            userMinted = earlyMintAddresses[_whitelistedAddress];
        }

        else if(block.timestamp >= WLTwoStartAt){
            userMinted = earlyMintAddresses_two[_whitelistedAddress];
        }
