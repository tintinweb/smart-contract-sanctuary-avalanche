...
contract Phoenix is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 51000 ether;
    // Initial distribution for the day 2-5 PHOENIX-WETH LP -> PHOENIX pool
    // uint256 public constant INITIAL_PHOENIX_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the PHOENIX ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("phoenix.finance", "PHOENIX") {
        // Mints 1 PHOENIX to contract creator for initial pool setup
    ...
    function _getPhoenixPrice() internal view returns (uint256 _phoenixPrice) {
        try IOracle(phoenixOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Phoenix: failed to fetch PHOENIX price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _phoenixPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_phoenixPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPhoenixOracle(address _phoenixOracle) public onlyOperatorOrTaxOffice {
        require(_phoenixOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PHOENIX to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PHOENIX to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPhoenixPrice = _getPhoenixPrice();
            currentTaxRate = _updateTaxRate(currentPhoenixPrice);
            if (currentPhoenixPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _phoenixPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_phoenixPool != address(0), "!_phoenixPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_phoenixPool, INITIAL_PHOENIX_POOL_DISTRIBUTION);
