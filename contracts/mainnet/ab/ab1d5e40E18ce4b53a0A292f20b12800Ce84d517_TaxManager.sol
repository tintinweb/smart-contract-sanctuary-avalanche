...
contract TaxManager is Ownable, Manageable {
  ...
  function approveTokensOnRouter() public onlyOwner {
    PLAYMATES.approve(ROUTER, type(uint256).max);
  }
  ...
  function execute(uint256 remainingRewards, address receiver) external onlyManager {
    ...
    if (newTax > 0) {
      uint256 amountToRewardsPool = (newFeeAmount * _newTaxRatios.rewards) / _newTaxRatios.total;
      PLAYMATES.transfer(POOL, amountToRewardsPool);
    ...
    PLAYMATES.transfer(receiver, excessRewards);
  }
