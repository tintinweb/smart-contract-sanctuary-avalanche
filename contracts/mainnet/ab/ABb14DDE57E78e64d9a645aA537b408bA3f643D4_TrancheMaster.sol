...
contract StrategyWonderland is ReentrancyGuard, Ownable, CoreRef {
    ...
    function setInputTokenToBaseTokenPath(address[] calldata _inputTokenToBaseTokenPath) public onlyTimelock {
        inputTokenToBaseTokenPath = _inputTokenToBaseTokenPath;
        ...
        for (uint i = 0; i < tranches.length; i++) {
            ...
            if (u.isAuto) {
                ...
                if (active && !inv.rebalanced) {
                    ...
                    inv.rebalanced = true;
                    inv.principal = valid;
                    u.balance = u.balance.add(principal.sub(valid));
                    tranches[i].autoPrincipal = tranches[i].autoPrincipal.sub(principal.sub(valid));
                } else {
                ...
                if (inv.cycle < cycle) {
                    ...
                    inv.principal = valid;
                    u.balance = u.balance.add(principal.sub(valid));
                    tranches[i].principal = tranches[i].principal.sub(principal.sub(valid));
                    IMasterWTF(staker).updateStake(i, account, inv.principal);
                }
            }
            inv.cycle = cycle;
        ...
        for (uint i = 0; i < tranches.length; i++) {
            ...
            if (u.isAuto) {
                for (uint j = inv.cycle; j < cycle; j++) {
                    ...
                    capital = valid.mul(snapshot.rate).div(PercentageScale);
                    principal = principal.sub(valid).add(capital);
                }
                if (active && !rebalanced) {
                    valid = principal.mul(tranches[i].validPercent).div(PercentageScale);
                    invested = invested.add(valid);
                    balance = balance.add(principal.sub(valid));
                } else {
