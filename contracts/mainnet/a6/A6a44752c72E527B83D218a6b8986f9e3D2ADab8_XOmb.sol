...
contract XOmb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 XOMB-WAVAX LP -> XOMB pool
    uint256 public constant INITIAL_XOMB_POOL_DISTRIBUTION = 75000 ether;
    ...
    /**
     * @notice Constructs the XOMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("XOMB Token", "XOMB") {
        // Mints 1 XOMB to contract creator for initial pool setup
    ...
    function _getXombPrice() internal view returns (uint256 _xombPrice) {
        try IOracle(xombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Xomb: failed to fetch XOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _xombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_xombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setXombOracle(address _xombOracle) public onlyOperatorOrTaxOffice {
        require(_xombOracle != address(0), "oracle address cannot be 0 address");
        xombOracle = _xombOracle;
    }
    ...
    /**
     * @notice Operator mints XOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of XOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentXombPrice = _getXombPrice();
            currentTaxRate = _updateTaxRate(currentXombPrice);
            if (currentXombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_xombPool != address(0), "!_xombPool");
        //require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_xombPool, INITIAL_XOMB_POOL_DISTRIBUTION);
