...
interface ICitiesManager {
  ...
  function getCities(uint256 _id) external view returns (City memory);
}
...
contract CitiestHelper is Ownable {
  IERC20 public PLAYMATES;
  IMansionManager public MANSION;
  ICitiesManager public CITIES;

  bool enableClaims = false;

  uint256 public claimTime = 1;
  uint256 public reward = 16320 * 10;
  uint256 public precision = 1410065408;

  uint256 public releaseTime = 0;

  mapping(uint256 => ICitiesManager.City) private _nodes;

  constructor(address _mansion, address _playmates, address _city)  {
      PLAYMATES = IERC20(_playmates);
      MANSION = IMansionManager(_mansion);
      CITIES = ICitiesManager(_city);
  }

  modifier onlyIfExists(uint256 _id) {
    require(CITIES.ownerOf(_id) != address(0), "ERC721: operator query for nonexistent token");
    _;
  }
  ...
  function claim(address account, uint256 _id) external onlyIfExists(_id) returns (uint) {
    ...
    require(CITIES.ownerOf(_id) == account, "MANAGER: You are not the owner");
    ICitiesManager.City memory _node = CITIES.getCities(_id);
  ...
  function _changeCities(address _city) onlyOwner external {
      CITIES = ICitiesManager(_city);
  }
}