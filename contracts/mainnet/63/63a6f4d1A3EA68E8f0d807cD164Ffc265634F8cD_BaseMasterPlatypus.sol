...
/// - Removed vePTP boost
contract BaseMasterPlatypus is
    Initializable,
    ...
    IBaseMasterPlatypus
{
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accPtpPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 baseAllocPoint; // How many allocation points assigned to this pool. PTPs to distribute per second.
    ...
    IBaseMasterPlatypus newMasterPlatypus;
    ...
    function setNewMasterPlatypus(IBaseMasterPlatypus _newMasterPlatypus) external onlyOwner {
    ...
    function add(
        ...
        // add _baseAllocPoint to total alloc points
        totalBaseAllocPoint = totalBaseAllocPoint + _baseAllocPoint;
    ...
    function set(
        ...
        massUpdatePools();
        totalBaseAllocPoint = totalBaseAllocPoint - poolInfo[_pid].baseAllocPoint + _baseAllocPoint;
        poolInfo[_pid].baseAllocPoint = _baseAllocPoint;
        if (overwrite) {
            poolInfo[_pid].rewarder = _rewarder;
        }
        emit Set(_pid, _baseAllocPoint, overwrite ? _rewarder : poolInfo[_pid].rewarder, overwrite);
    }
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 ptpReward = (secondsElapsed * ptpPerSec * pool.baseAllocPoint) / totalBaseAllocPoint;
            accPtpPerShare += (ptpReward * 1e12) / lpSupply;
        }
        pendingPtp = ((user.amount * accPtpPerShare) / 1e12) - user.rewardDebt;
    ...
    function _updatePool(uint256 _pid) private {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            pool.accPtpPerShare += (ptpReward * 1e12) / lpSupply;
    ...
    function depositFor(
        ...
        if (user.amount > 0) {
            // Harvest PTP
            uint256 pending = ((user.amount * pool.accPtpPerShare) / 1e12) - user.rewardDebt;
        ...
        // update reward debt
        user.rewardDebt = (user.amount * pool.accPtpPerShare) / 1e12;
        ...
        returns (uint256, uint256)
    {
        ...
        if (user.amount > 0) {
            // Harvest PTP
            pending = ((user.amount * pool.accPtpPerShare) / 1e12) - user.rewardDebt;
...
/**
 * @dev Interface of the BaseMasterPlatypus
 */
