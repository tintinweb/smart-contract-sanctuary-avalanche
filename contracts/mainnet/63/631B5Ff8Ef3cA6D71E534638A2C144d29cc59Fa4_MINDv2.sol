...
contract MINDv2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _manualSwapAndLiquify() private {
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 _totalFeeBasisPointsToSwap = totalFeeBasisPointsToSwap;
        uint256 _totalFeeBasisPoints = totalFeeBasisPoints;
        uint256 liquidityAmount = contractTokenBalance * liquidityPoolFee / 10000;
        uint256 liquidityAmountToSwap = liquidityAmount / 2;
        uint256 remainder = contractTokenBalance - liquidityAmount;
        uint256 remainderToSwap = _totalFeeBasisPoints > 0
            ? remainder * _totalFeeBasisPointsToSwap / _totalFeeBasisPoints
            : 0;
        uint256 totalAmountToSwap = liquidityAmountToSwap + remainderToSwap;
        ...
        uint256 totalFeeBasisPointsNotToSwap = _totalFeeBasisPoints - _totalFeeBasisPointsToSwap;
        _sendToFeeRecipients(_totalFeeBasisPointsToSwap, remainderAVAX, remainder, totalFeeBasisPointsNotToSwap);
        emit ManualSwapAndLiquify(_msgSender(), contractTokenBalance);
    }   

    function _sendToFeeRecipients(uint256 _totalFeeBasisPointsToSwap, uint256 _remainderAVAX, uint256 _remainder, uint256 _totalFeeBasisPointsNotToSwap) private {
        uint256 _feeRecipientCount = feeRecipientCount;
        uint256 remainderAVAXBalance = _remainderAVAX;
        uint256 remainderBalance = _remainder;
        for(uint256 x; x < _feeRecipientCount; x++) {
            FeeRecipient memory feeRecipient = FeeRecipients[x];
            if (feeRecipient.basisPoints == 0) continue;
            if (feeRecipient.sellToNative) {
                uint256 amount = _totalFeeBasisPointsToSwap > 0
                    ? _remainderAVAX * feeRecipient.basisPoints / _totalFeeBasisPointsToSwap
                    : 0;
                ...
                uint256 amount = _totalFeeBasisPointsNotToSwap > 0
                    ? _remainder * feeRecipient.basisPoints / _totalFeeBasisPointsNotToSwap
                    : 0;
                if (amount == 0) continue;
                amount = amount > remainderBalance ? remainderBalance : amount;
                super._transfer(address(this),feeRecipient.recipient, amount);
