...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address tokenReward, uint256 _minimumTokenBalanceForDividends) DividendPayingToken(tokenReward, "_Dividend_Tracker", "_Dividend_Tracker") {
        claimWait = 3600;
...
contract DxDividendToken is ERC20, Ownable {
    ...
    constructor(address tokenOwner, address _rewardToken, string memory name_, string memory symbol_, uint8 decimals_, uint256 _totalSupply, uint8 _tokenRewardsFee, uint8 _liquidityFee, address _router, address _basePair) ERC20(name_, symbol_) {
        ...
        //_minimumTokenBalanceForDividends = _totalSupply /100000;

        //string memory tracker = string.concat(symbol_,"_Dividend_Tracker");

        dividendTracker = new DividendTracker(rewardToken, (_totalSupply /100000));
...
*/
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && from != owner() && to != owner() && liquidityFee > 0) {
            swapping = true;
            ...
            if (rewardToken == basePair) {
                swapAndSendDividendsNative(sellTokens);
            } else {
    ...
    function swapAndSendDividendsNative(uint256 tokens) private {
        uint256 initialBalance = address(this).balance;
