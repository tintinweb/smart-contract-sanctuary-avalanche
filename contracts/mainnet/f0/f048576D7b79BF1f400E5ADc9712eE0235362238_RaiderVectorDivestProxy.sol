...
import "../../common/ADivestStrategy.sol";
import "../../interfaces/IVectorProxyHelper.sol";
 ...
 */
contract RaiderVectorDivestProxy is ADivestStrategy {
  ...
  IVectorProxyHelper public poolContract;
  ...
  constructor(
    ...
    address _poolManager // This is the proxy contract itself
  )
    ADivestStrategy(
      _uniRouter,
      _WNATIVE,
  ...
  {
    poolContract = IVectorProxyHelper(_poolManager);
  }

  function poolDeposit(uint256 _amount) internal override {
    poolContract.deposit(depositToken, _amount);
  }

  function poolWithdraw(uint256 _amount) internal override {
    poolContract.withdraw(depositToken, _amount);
  }

  function getPoolReward() internal override {
    address[] memory lps = new address[](1);
    lps[0] = depositToken;
    poolContract.multiclaim(lps, address(this));
  }
  ...
  function poolBalance(address _address)
    ...
    returns (uint256)
  {
    return poolContract.depositInfo(depositToken, _address);
  }
}
 ...
 * Expect the amount of divestment token to grow over time.
 */
abstract contract ADivestStrategy is ERC20, Ownable, Pausable {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;
  ...
  // TODO @cstoneham - Write a test for this
  ...
  uint256 private _totalDistributed; // the total amount of rewardTokens distributed, used to calc each distribution
  ...
  function withdraw(uint256 _amount) external {
    ...
    // Note: for simplicity, NEVER remove a user from the total holders
    // their entitlements just drop.
  }
  ...
  function withdrawDivestedTokens(address _account) external returns (uint256) {
    ...
    // Harvest and settle up with IOUs
    harvest();

    // Get balance of IOU tokens
    ...
    // Transfer to vault
    IERC20(divestToken).safeTransfer(vault, amount);
   ...
   */
  function deposit(address _sender) public whenNotPaused {
    require(
    ...
    // Harvest and distribute IOUs before changing the split
    harvest();

    // If new user, update the mapping - must be after harvest + distribute
    // bool isNewUser = true;
    // for (uint256 i = 0; i < _totalHolders; i++) {
    //   if (_holders[i] == _sender) {
    //     isNewUser = false;
    //   }
    // }
    // if (isNewUser) {
    //   _holders[_totalHolders] = _sender;
    //   _totalHolders += 1;
    // }
  }
  ...
  /**
   * @dev gets appropriate distribution of divested yield for
   * a given user.
   */
  function getDistributionAmount(address _address)
    internal
    view
    returns (Distribution memory)
  {
    uint256 vaultLPShares = IERC20(vault).balanceOf(_address);
    uint256 totalLPShares = IERC20(vault).totalSupply();

    // Return 0 if the user or vault has no LP tokens
    if (totalLPShares == 0 || vaultLPShares == 0) {
      return Distribution(_address, 0);
    }

    uint256 divestTokenBalanceSinceLastDistribution = _totalDivested -
      _totalDistributed;

    // Get the appropriate amount of tokens this user is owed
    uint256 entitledDivestTokenBalance = (
      divestTokenBalanceSinceLastDistribution.mul(vaultLPShares)
    ).div(totalLPShares);

    return Distribution(_address, entitledDivestTokenBalance);
  }

  /**
   * Mint's IOUs in the currency of the divest token for the end
   * users and emit's events describing the divestment for each user.
   *
   * NOTE: this is all done in the *divestment* currency. In many cases
   * this will be USDC which has only 6 decimals rather than 18.
   */
  function distributeRewardTokens() internal {
    Distribution[] memory distributions = new Distribution[](_totalHolders);

    // Calculate appropriate distributions for each user
    for (uint256 i = 0; i < _totalHolders; i++) {
      address currAddress = _holders[i];
      distributions[i] = getDistributionAmount(currAddress);
    }

    // Distribute the IOUs
    for (uint256 i = 0; i < distributions.length; i++) {
      Distribution memory distribution = distributions[i];
      _mint(distribution.user, distribution.amount);
    ...
    // update _totalDistributed
    _totalDistributed = _totalDivested;
  }
