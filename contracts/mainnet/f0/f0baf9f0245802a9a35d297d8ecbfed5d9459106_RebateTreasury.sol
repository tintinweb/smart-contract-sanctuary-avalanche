...
contract RebateTreasury is Ownable {
    ...
    constructor(address xomb, address xombOracle, address treasury) {
        Xomb = IERC20(xomb);
        XombOracle = IOracle(xombOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Xomb at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 xombAmount = getXombReturn(token, amount);
        require(xombAmount <= Xomb.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient xomb balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + xombAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += xombAmount;
    }

    // Claim available Xomb rewards from bonding
    ...
    function setXomb(address xomb) external onlyOwner {
        Xomb = IERC20(xomb);
    }

    // Set Xomb oracle

    function setXombOracle(address oracle) external onlyOwner {
        XombOracle = IOracle(oracle);
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getXombPrice() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Xomb.transfer(account, claimable);
    }
    ...
    // Calculate Xomb return of bonding amount of token

    function getXombReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 xombPrice = getXombPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / xombPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 xombPrice = getXombPrice();
        if (xombPrice < 1e17) return 0; // related to 0.1Avax

        uint256 xombPremium = xombPrice * DENOMINATOR / 1e17 - DENOMINATOR; // related to 0.1Avax
        if (xombPremium < bondThreshold) return 0;
        if (xombPremium <= secondaryThreshold) {
            return (xombPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (xombPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get XOMB price from Oracle

    function getXombPrice() public view returns (uint256) {
        return XombOracle.consult(address(Xomb), 1e18);
    }
    ...
    // Get claimable vested Xomb for account

    function claimableXomb(address account) external view returns (uint256) {
