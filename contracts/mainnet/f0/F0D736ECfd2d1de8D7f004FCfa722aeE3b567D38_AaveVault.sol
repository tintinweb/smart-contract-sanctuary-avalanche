...
contract AaveVault is BaseBuildingBlock {
    ...
    event SwapEvent(address tokenOut, uint256 amountOut);
     ...
     */
    function closePosition(bytes memory _data) external onlySelf() {
     ...
     **/
    function claimAllRewards(bytes memory _data) external onlySelf {
    ...
    function swap(bytes memory _data) external returns (uint256 amountOut) {
        ...
        emit SwapEvent(path[path.length - 1], amountOut);
    }
