...
ERC721URITokenJSON, CosmicAttributeStorageUpgradeable, Blacklistable, TokenConstants  {
    ...
    function tokenURIJSON(uint256 tokenId) public view virtual override returns(string memory) {
        ...
        uint256 count = 0;
        // base (11)
        uint256 baseSkillNameId = gender == 0 ? 11 : 22;
        for (uint256 i = 0; i < 11; i++) {
            attributes[i] = TokenMetadata.Attribute(getSkillName(0, i), '', getSkillName(0, baseSkillNameId + getSkill(tokenId, 0, i)), false);
            count++;
        }
        // Elven Adventures / Expeditions (3)
        for (uint256 i = 0; i < 3; i++) {
            attributes[i + count] = TokenMetadata.Attribute(getSkillName(3, i), '', getSkill(tokenId, 3, i) == 0 ? 'False' : 'True', false);
            count++;
        }
        // Professions (12)
        for (uint256 i = 0; i < 12; i++) {
            attributes[i + count] = TokenMetadata.Attribute(getSkillName(1, i), 'number', getSkill(tokenId, 1, i).toString(), false);
            count++;
        }
