...
contract AvaDice_VRF is Ownable, VRFConsumerBaseV2 {
    event GameResult(
        address player,
        uint playedAmount,
        uint wonAmount,
    ...
    mapping(address => uint[]) public selectedRoll;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        s_randomWords[requestId] = (randomWords[0] % 6) + 1;
    }
    ...
    function playDice(
        ...
        require(0 < dice1 && dice1 < 7, "Dice should be in 1-6");
        require(0 < dice2 && dice2 < 7, "Dice should be in 1-6");
        require(0 < dice3 && dice3 < 7, "Dice should be in 1-6");
    ...
    function getResult() external {
        ...
        uint randomWord = s_randomWords[requestID[msg.sender]];
        uint houseEdge = houseEdgeCalculator(msg.sender);
        uint amount = payedAmount[msg.sender];
        if (
            selectedRoll[msg.sender][0] == randomWord ||
            selectedRoll[msg.sender][1] == randomWord ||
            selectedRoll[msg.sender][2] == randomWord
        ) {
            uint pay = ((amount * 1000) / (houseEdge + 1000)) * 2;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, randomWord);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else {
            emit GameResult(msg.sender, amount, 0, false, randomWord);
