    ...
    UUPSUpgradeable
{
    ...
    function getReward(address[] calldata pools) external {
        for (uint i; i < pools.length; ) {
            address _pool = tokenForPool[pools[i]];
            INeadPool(_pool).getReward(msg.sender);
            unchecked {
                ++i;
            }
        }
    ...
    function claimRewards(address[] calldata pools) external {
        for (uint i; i < pools.length; ) {
            address _pool = tokenForPool[pools[i]];
            INeadPool(_pool).claimRewards(msg.sender);
            unchecked {
                ++i;
            }
        }
