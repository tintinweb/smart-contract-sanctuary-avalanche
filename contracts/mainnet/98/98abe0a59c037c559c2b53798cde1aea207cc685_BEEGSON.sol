...
contract BEEGSON is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC2981, ClampedRandomizer {
    constructor() 
        ERC721("BEEG SON", "BS") ClampedRandomizer(maxSupply) {
    ...
    uint public whitelistTime= 1671386400; // Sun Dec 18 2022 18:00:00 GMT+0000
    uint public publicTime= 1671390000; //Sun Dec 18 2022 19:00:00 GMT+0000
    address public royaltyAddress=0x1e6fEBcC2Daa1A4EFC37c3a034668b597512c8C5;
    uint96 public royalty=969;
    string public baseURI = "";
    string public unrevealedURI = "ipfs://QmVLtoo9LHkeLqbNrN6itcNQfi2w6XhKYvTiP3gwc2bC6w/hidden.json";
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        //mint start
        require(block.timestamp >= whitelistTime, "Its not time to go Beeg Son yet");
        
        //supply limits
        uint supply = totalSupply();
        require(supply + amount <= maxSupply, "No more Beeg Son left for you");

        //transaction limits
        require(amount > 0,"you want to mint more than just air right?");
        require(amount <= maxPerTx, "Thats too many Beeg Sons at once");

        //mint limits
        require(whiteListedCount[msg.sender] < maxPerNonTeam, "You can have only 2 Beeg Sons");
        ...
        {
            require(whiteListed[msg.sender] ==true, "Silly Beeg Son are for WL only right now");
        ...
        if (block.timestamp >= publicTime)
        {
            require(publicCount[msg.sender] < maxPerNonTeam, "You can have only 2 Beeg Sond");   
        }
    ...
    {
        uint supply = totalSupply();
        require(supply + 1 <= maxSupply, "No more Beeg Son left for you");
        uint256 tokenId=_genClampedNonce()+1;//+1 since we dont want to deal with token id 0
        _isMinted[tokenId]=true;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(tokenId.toString(), ".json")));
