...
contract StrategyTJLiquidityBookLB is StratManager, FeeManager {
    ...
    event AddLiquidity(address user, uint256 amountX, uint256 amountY, uint256 liquidity);

    event RemoveLiquidity(address user, uint256 amountX, uint256 amountY);

    event Rebalance(uint256 amountXBefore, uint256 amountYBefore, uint256 amountXAfter, uint256 amountYAfter);
    ...
    constructor(
        ...
        require(_strategyParams.deltaIds.length < MAX_BINS, "Strategy: Too many bins");

        //check parameters are equal in length
        require(
            _strategyParams.deltaIds.length == _strategyParams.distributionX.length
                && _strategyParams.distributionX.length == _strategyParams.distributionY.length,
    ...
    function strategyPositionAtIndex(uint256 _index) public view returns (uint256) {
    ...
    function strategyActiveBins() public view returns (uint256[] memory activeBins) {
    ...
    function checkProposedBinLength(int256[] memory proposedDeltas, uint256 activeId) public view returns (uint256) {
        uint256 newIdCount;
    ...
    function _addLiquidity(uint256 amountX, uint256 amountY, uint256 amountXMin, uint256 amountYMin)
        internal
        whenNotPaused
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        require(checkProposedBinLength(deltaIds, activeId) < MAX_BINS, "Strategy: Requires Rebalance");
        ...
        (,,,, depositIds, liquidityMinted) = ILBRouter(joeRouter).addLiquidity(liquidityParameters);
    ...
    function _removeLiquidity(uint256 denominator) internal returns (uint256 amountX, uint256 amountY) {
        ...
        for (uint256 i; i < activeBinIds.length; i++) {
            ...
            uint256 LBTokenAmount = (PRECISION * lbToken.balanceOf(address(this), activeBinIds[i])) / (denominator);
        ...
        (amountX, amountY) = ILBRouter(payable(joeRouter)).removeLiquidity(
            tokenX, tokenY, binStep, 0, 0, activeBinIds, amounts, address(this), block.timestamp
        );
    ...
    function _updateLiquidities() internal {
        ...
        (,, uint256[] memory liquidities) =
            LiquidityAmounts.getAmountsAndLiquiditiesOf(address(this), activeBinIds, address(lbPair));
    ...
    function _swap(uint256 amountIn, bool _swapForY) internal returns (uint256 amountOutReal) {
        ...
        amountOutReal = ILBRouter(joeRouter).swapExactTokensForTokens(amountIn, 0, path, address(this), block.timestamp);
    }
    ...
    function _collectRewards() internal returns (uint256 feesX, uint256 feesY) {
        ...
        //get a snapshot of the most recent liquidities
        (uint256[] memory amountsX, uint256[] memory amountsY, uint256[] memory liquidities) =
            LiquidityAmounts.getAmountsAndLiquiditiesOf(address(this), activeBinIds, address(lbPair));
        ...
        for (uint256 i; i < liquidities.length; ++i) {
            ...
            uint256 _feesX = _feesL.mulDivRoundDown(amountsX[i], liquidities[i]);
            uint256 _feesY = _feesL.mulDivRoundDown(amountsY[i], liquidities[i]);
            ...
            feeGrowthInside[i] = performanceFee.mulDivRoundDown(_amount, MAX_FEE);
        }
        ...
        if (feesX > 0 || feesY > 0) {
            ...
            lbToken.batchTransferFrom(
                address(this),
                feeRecipient, //sends to SH fee collector wallet
                activeBinIds,
                feeGrowthInside
            );
        }
        ...
        emit CollectRewards(address(0), lastHarvest, amountX - feesX, amountY - feesY, amountX, amountY);
    }
    ...
    function harvest() external virtual returns (uint256 amountXReceived, uint256 amountYReceived) {
    ...
    function earn() external onlyManager returns (uint256 amountX, uint256 amountY) {
        ...
        (amountX, amountY) = _calculateAmountsPerformSwap(deltaIds, balanceX, balanceY);

        //use the funds in the strategy to add liquidty.
        if (amountX > 0 || amountY > 0) {
            (uint256 _amountX, uint256 _amountY) = computeReserveAmounts(amountX, amountY);
    ...
    function removeLiquidity(uint256 denominator) external returns (uint256 amountX, uint256 amountY) {
    ...
    function computeReserveAmounts(uint256 _amountX, uint256 _amountY)
        internal
        view
    ...
    function getTotalAmounts() public view returns (uint256 totalX, uint256 totalY) {
        ...
        (totalX, totalY) = LiquidityAmounts.getAmountsOf(address(this), strategyActiveBins(), address(lbPair));
    ...
    function binHasXLiquidity(int256[] memory _deltaIds) public pure returns (bool hasXLiquidity) {
    ...
    function binHasYLiquidity(int256[] memory _deltaIds) public pure returns (bool hasYLiquidity) {
        for (uint256 i; i < _deltaIds.length; i++) {
            if (_deltaIds[i] <= 0) {
                hasYLiquidity = true;
                break;
    ...
    /// @return amountX amount of tokenX removed from liquidity
    /// @return amountY amount of tokenY removed from liquidity
    function removeLiquidityIgnoreRewards(uint256 denominator)
        external
        onlyOwner
        returns (uint256 amountX, uint256 amountY)
    {
    ...
    function retireStrat() external {
        ...
        //add currently active tokens supplied as liquidity
        (uint256 totalX, uint256 totalY) =
            LiquidityAmounts.getAmountsOf(address(this), strategyActiveBins(), address(lbPair));
    ...
    function updateLiquidityReserve(uint256 _liquidityReserve) external onlyOwner {
        require(_liquidityReserve < 100, "LBStrategy: liquidityReserve shall be less than 100");
    ...
    function executeRebalance() external onlyManager returns (uint256 amountX, uint256 amountY) {
        (amountX, amountY) = executeRebalance(deltaIds, distributionX, distributionY, idSlippage);
    }
    ...
    function executeRebalance(
        ...
        require(_deltaIds.length < MAX_BINS, "Strategy: Bins shall be limited to <MAX_BINS");

        //check parameters are equal in length
        require(
            _deltaIds.length == _distributionX.length && _distributionX.length == _distributionY.length,
            "Strategy: Unbalanced params"
        );
        ...
        if (amountXBefore > 0 || amountYBefore > 0) {
            (amountX, amountY) = _calculateAmountsPerformSwap(
                _deltaIds, tokenX.balanceOf(address(this)), tokenY.balanceOf(address(this))
            );
        }
    ...
    function _calculateAmountsPerformSwap(int256[] memory _deltaIds, uint256 _amountX, uint256 _amountY)
        internal
        returns (uint256 amountX, uint256 amountY)
    {
