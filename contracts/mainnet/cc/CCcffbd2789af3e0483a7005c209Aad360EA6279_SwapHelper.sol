...
contract SwapHelper is Initializable, OwnableUpgradeable {
        ...
        returns (uint256 avaxAmount)
    {
        if (routeToAvaxOnPlatypus[token] != address(0)) {
            (avaxAmount, ) = IPlatypusPool(routeToAvaxOnPlatypus[token]).quotePotentialSwap(
                token,
                WAVAX,
                amount
            );
            ...
            address[] memory path = findPathToAvax(token);
            IJoeRouter02 router = getRouter(token);
            uint256[] memory amounts = router.getAmountsOut(amount, path);
            avaxAmount = amounts[amounts.length - 1];
        }
    }
    ...
    function _swapTokenForAVAX(
        ...
        if (token == WAVAX) {
            ...
            IAvaxHelper(avaxHelper).withdrawTo(receiver, amount);
            avaxAmount = amount;
        } else {
            if (routeToAvaxOnPlatypus[token] != address(0)) {
                address platypusPool = routeToAvaxOnPlatypus[token];
                _approveTokenIfNeeded(token, platypusPool);
                (avaxAmount, ) = IPlatypusPool(platypusPool).swapToETH(
                    token,
                    amount,
                    minAmountReceived,
                    payable(receiver),
    ...
    function _swapTokenForWAVAX(
        ...
        if (token == WAVAX) {
            ...
            if (routeToAvaxOnPlatypus[token] != address(0)) {
                address platypusPool = routeToAvaxOnPlatypus[token];
                _approveTokenIfNeeded(token, platypusPool);
                (wavaxAmount, ) = IPlatypusPool(platypusPool).swap(
                    token,
                    WAVAX,
                    amount,
                    minAmountReceived,
                ...
                wavaxAmount = amounts[amounts.length - 1];
            }
        }
    }
    ...
    function _swapWAVAXForToken(
        ...
        address[] memory path = findPathFromAvax(token);
        IJoeRouter02 router = getRouter(token);
        _approveTokenIfNeeded(WAVAX, address(router));
        uint256[] memory amounts = router.swapExactTokensForTokens(
            amount,
            minAmountReceived,
            path,
