...
interface IMasterChefVTX {
    ...
    function pendingTokens( address _lp, address _user, address _token)
    external view 
    returns(
        uint256 pendingVTX,
    ...
    function depositInfo( address _lp, address _user) external view returns(uint256 availableAmount);
}

// 
contract StrategyVtxV1 is StrategyBase {
    // Want Token
    address constant public VTX = 0x5817D4F0b62A59b17f75207DA1848C2cE75e7AF4;
    address constant public PTP = 0x22d4002028f537599bE9f666d1c4Fa138522f9c8;
    address constant public xPTP = 0x060556209E507d30f2167a101bFC6D256Ed2f3e1;

    address constant public vtxMasterChef = 0x423D0FE33031aA4456a17b150804aA57fc157d97;
    ...
    public
    StrategyBase( VTX, _governance, _strategist, _controller, _timelock){
        IERC20(VTX).safeApprove(vtxMasterChef, uint256(-1));
        univ2Router2 = 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106; //pangolin
        sushiRouter = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4; //trader joe
    }
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        uint256 _wavaxBefore = IERC20(WAVAX).balanceOf(address(this));
        //cover VTX to WAVAX
        address[] memory path = new address[](2);
        path[0] = VTX;
        path[1] = WAVAX;
        _swapUniswapWithDetailConfig(VTX, WAVAX, _lpAmount, 0, path, sushiRouter);

        uint256 _wavax = IERC20(WAVAX).balanceOf(address(this));
        return (WAVAX, _wavax.sub(_wavaxBefore));
    }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        // Collects VTX/xPTP tokens
        IMasterChefVTX(vtxMasterChef).withdraw(want, 0);

        uint256 _xPTPBal = IERC20(xPTP).balanceOf(address(this));
        if(_xPTPBal > 0){
            ...
            path[2] = WAVAX;
            path[3] = VTX;
            _swapUniswapWithDetailConfig(xPTP, VTX, _xPTPBal, 0, path, sushiRouter);
        }


        // We want to get back VTX tokens
        uint256 _want = balanceOfWant();
        uint256 _buybackLpAmount = _want.mul(performanceFee).div(performanceMax);
        if(_want > 0 && buybackEnabled == true && _buybackLpAmount > 0){
            (address _buybackPrinciple, uint256 _buybackAmount) = _convertWantToBuyback(_buybackLpAmount);
            buybackAndNotify(_buybackPrinciple, _buybackAmount);
            _want = _want.sub(_buybackLpAmount);
        }
    ...
    function getName() external override pure returns (string memory) {
        return "StrategyVTXV1";
    }

    function balanceOfPool() public override view returns (uint256) {
        uint256 amount = IMasterChefVTX(vtxMasterChef).depositInfo(want, address(this));
        return amount;
    }

    function getHarvestableVTX() external view returns (uint256) {
        (uint256 pendingVTX,,,) = IMasterChefVTX(vtxMasterChef).pendingTokens(want, address(this), want);
        return pendingVTX;
    }

    function getHarvestableXPTP() external view returns (uint256) {
        (,,,uint256 pendingBonusTokens) = IMasterChefVTX(vtxMasterChef).pendingTokens(want, address(this), xPTP);
        return pendingBonusTokens;
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            IMasterChefVTX(vtxMasterChef).deposit(want, _want);
        }
    }
    ...
    returns (uint256)
    {
        IMasterChefVTX(vtxMasterChef).withdraw(want, _amount);
        return _amount;
    }

    function withdrawEmergency() external onlyBenevolent {        
        uint256 _before = balanceOfWant();
        uint256 _all = balanceOfPool();
        IMasterChefVTX(vtxMasterChef).emergencyWithdraw(want);
        uint256 _after = balanceOfWant();
        require(_after.sub(_before) >= _all, '!emergencyWithdraw');

        IERC20(want).safeTransfer(IController(controller).vaults(address(want)), _after);
    }
}