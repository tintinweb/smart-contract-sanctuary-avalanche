...
// File contracts/KalmarLpStakingV2.sol
...
interface IKalmarStakingRewardConfig {
...
contract KalmarLpStakingV2 is ReentrancyGuard, Initializable {
    ...
    IKalmarStakingRewardConfig public constant rewardConfig = IKalmarStakingRewardConfig(0x9D43B2C18c79c6e3236ED905d18F4De7E11b7e57);
    ...
    function _getRewardData(address _token) internal view returns (uint256 accRewardPerShare, uint256 rewardsPerSecond) {
        ...
        uint256 start = 1658754000;
        uint256 currentWeek = (block.timestamp - start) / 604800;

        if (lpSupply == 0) {
            return (0, rewardConfig.getRewardsPerSecond(_token,currentWeek));
        }
        ...
        if (rewardWeek < currentWeek) {
            while (rewardWeek < currentWeek) {
                ...
                rewardWeek += 1;
                rewardsPerSecond = rewardConfig.getRewardsPerSecond(_token,rewardWeek);
    ...
    /**
        @notice Withdraw LP and remove tokens from the contract
        @dev Also updates the caller's current boost
        @param _token LP token address to withdraw.
        @param _amount Amount of tokens to withdraw.
        @param _claimRewards If true, also claim rewards earned on the token.
    ...
    function withdrawRemoveLp(
        ...
        _safeRemoveLiquidity(_token, _amount, token0, token1);
        uint256 token0Aft = IERC20(token0).balanceOf(address(this)) - token0Bf;
        uint256 token1Aft = IERC20(token1).balanceOf(address(this)) - token1Bf;
    ...
    function _safeRemoveLiquidity(address _token, uint256 _lpAmt, address _token0, address _token1) internal virtual {
        ...
        // remove lp
        if (lpAmt > 0) {
            IERC20(_token).approve(0x10ED43C718714eb63d5aA57B78B54704E256024E, _lpAmt);
            IPancakeRouter02(address(0x10ED43C718714eb63d5aA57B78B54704E256024E)).removeLiquidity(
                token0,
                token1,
                lpAmt,
                0,
            ...
            IERC20(_token).approve(0x10ED43C718714eb63d5aA57B78B54704E256024E, 0);
        }
    }
    ...
    function claimAndStake(address _user, address[] calldata _tokens) external returns (uint256) {
        if (msg.sender != _user) {
            require(!blockThirdPartyActions[_user], "Cannot claim on behalf of this account");
        }
        ...
        _mintRewards(address(this), pending);
        _depositByClaim(address(0x4BA0057f784858a48fe351445C672FF2a3d43515),_user,pending);
        return pending;
    }
