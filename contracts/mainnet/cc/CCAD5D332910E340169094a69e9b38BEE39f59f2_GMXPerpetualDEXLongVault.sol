...
import "../../../../contracts/interfaces/vaults/gmx/IGMXPerpetualDEXLongReader.sol";

abstract contract $IGMXPerpetualDEXLongReader is IGMXPerpetualDEXLongReader {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../contracts/vaults/gmx/GMXPerpetualDEXLongVault.sol";

contract $GMXPerpetualDEXLongVault is GMXPerpetualDEXLongVault {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    event return$_deposit(uint256 ret0);

    constructor(string memory _name, string memory _symbol, IERC20 _token, IChainlinkOracle _chainlinkOracle, GMXPerpetualDEXLongVault.VaultConfig memory _vaultConfig, uint256 _maxCapacity, uint256 _mgmtFeePerSecond, uint256 _perfFee, address _treasury) GMXPerpetualDEXLongVault(_name, _symbol, _token, _chainlinkOracle, _vaultConfig, _maxCapacity, _mgmtFeePerSecond, _perfFee, _treasury) {}

    function $_deposit(uint256 _amt,uint256 _minSharesAmt) external returns (uint256 ret0) {
        (ret0) = super._deposit(_amt,_minSharesAmt);
    ...
    function $_withdraw(address _token,uint256 _withdrawAmt) external {
        super._withdraw(_token,_withdrawAmt);
    }
