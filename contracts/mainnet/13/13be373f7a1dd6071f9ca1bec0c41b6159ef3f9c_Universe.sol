    ...
    WalletObserverImplementationPointer
{
    ...
    mapping(address => bool) public _isExcludedFromSmartSmallTax;
    mapping(address => bool) public _isSmartTaxable;
    uint public _smartSmallTax = 20;
    ...
    function updateSmartSmallTax(uint smartSmallTax)
    public
    onlyOwner
    {
        require(smartSmallTax <= 20, "ERC20: Tax must not exceed 20");
        _smartSmallTax = smartSmallTax;
    }
    ...
    onlyOwner
    {
        require(smartTax <= 50, "ERC20: Tax must not exceed 50");
        _smartTax = smartTax;
    }

    function excludeFromSmartSmallTax(address account, bool status)
    public
    onlyOwner
    {
        _isExcludedFromSmartSmallTax[account] = status;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromSmartSmallTax[from] && planetsManager.balanceOf(from) > 0) {
            amountFee = amountFee + (amount * _smartSmallTax / 100);
        }
        ...
        if (amountFee > 0 && _burnWallet != address(0)) {
