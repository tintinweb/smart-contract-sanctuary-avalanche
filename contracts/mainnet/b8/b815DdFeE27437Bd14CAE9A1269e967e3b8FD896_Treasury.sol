...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xD2641C5e2Bb94ec5A9CdCAdDF3fd979b32AfF659) // WlrsGenesisPool
    ];
    ...
    function initialize(
        ...
        maxExpansionTiers = [400, 300, 200, 150, 125, 100];
    ...
    function setMinExpansion(uint256 _value) external onlyOperator returns (bool) {
        require(_value >= 0 && _value <= 1000, "_value: out of range"); // [0%, 10%]
        minExpansion = _value;
        return true;
    }

    function setMultiplifier(uint256 _value) external onlyOperator returns (bool) {
        multiplier = _value;
        return true;
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochWlrsPrice > wlrsPriceCeiling) {
                ...
                uint256 _mse = _calculateMaxSupplyExpansionPercent(wlrsSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if ((minExpansion != 0) && (minExpansion > _percentage)) { // if min expansion threshold enabled then set lower limit
                    _percentage = minExpansion;
                }
                if (multiplier != 0) {
                    _percentage = multiplier;
                }
