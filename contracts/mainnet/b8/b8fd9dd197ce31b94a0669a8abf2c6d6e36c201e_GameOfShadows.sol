...
contract GameOfShadows is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.1 ether;
    uint256 public maxMints = 69;
    ...
    constructor()
        ERC721("Game Of Shadows", "GOS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 461) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeihpq4ghjfglhbnd54jhbjv24lsl474ore3twj7pcgmvhrq52yngcy/", Strings.toString(tokenId), ".json")
            );
    }
