...
interface IDex {
    ...
    function swapNative(address _in, address _out, address _to) external payable returns (uint[] memory amounts);
    function swapNativeSlippageMin(uint256 _amountOutMin, address _in, address _out, address _to) external payable returns (uint[] memory amounts);
...
interface ComptrollerInterface {
    ...
    function checkMembership(address account, QiTokenInterface qiToken) external view returns (bool);
     ...
     */
    function claimReward(uint8 rewardType, address payable[] memory holders, QiTokenInterface[] memory qiTokens, bool borrowers, bool suppliers) external payable;
}
...
abstract contract BaseModule is IYieldModule, UUPSUpgradeable, OwnableUpgradeable, Rescuable {
    ...
    function _initializeBase(
        ...
        _setExectutionFee(_executionFee);
    ...
    function setExectutionFee(uint256 _executionFee) external onlyOwner {
        _setExectutionFee(_executionFee);
    }
    ...
    function _setDex(address _dex) internal {
        require(
            _dex != address(0),
    ...
    function _setExectutionFee(uint256 _executionFee) internal {
        require(
            _executionFee <= 5 * 1e17,
            "BaseModule: execution fee must be less than 0.5 AVAX"
        );
        executionFee = _executionFee;
    }

    function _setRewards(address[] memory _rewards) internal {
...
import "../interface/benqi/ComptrollerInterface.sol";
import "../interface/benqi/QiTokenInterface.sol";

contract BenqiYieldModule is BaseModule {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function initialize(
        ...
        _initializeBase(_manager, _baseToken, _executionFee, _dex, _rewards);
        
        _setComptroller(_comptroller);
        _setQiToken(_qiToken);
        _enterMarket();
    }
    ...
    function setQiToken(address _qiToken) external onlyOwner {
        _setQiToken(_qiToken);
    }
    ...
    function deposit(uint256 amount) external onlyOwnerOrManager {
        require(amount > 0, "Benqi: deposit amount cannot be zero");
        IERC20Upgradeable(baseToken).safeTransferFrom(msg.sender, address(this), amount);
        uint256 error = QiErc20Interface(qiToken).mint(amount);
        require(error == 0, "Benqi: deposit error");
    ...
    {
        require(amount > 0, "Benqi: withdraw amount cannot be zero");
        require(msg.value == 0, "Benqi: msg.value must be zero");
        uint256 baseTokensBalanceBefore = IERC20Upgradeable(baseToken).balanceOf(
            address(this)
        );
        uint256 error = QiErc20Interface(qiToken).redeemUnderlying(amount);
        require(error == 0, "Benqi: withdraw error");
        uint256 baseTokensBalanceAfter = IERC20Upgradeable(baseToken).balanceOf(
            address(this)
        );
        uint256 withdrawnAmount = baseTokensBalanceAfter -
            baseTokensBalanceBefore;
    ...
    function harvest(uint256 aum, address receiver)
    external
    onlyOwnerOrManager
    returns (uint256)
    {
        uint256 lpProfit = _lpProfit(aum);
        uint256 rewardsProfit = _rewardsProfit();
        uint256 totalProfit = lpProfit + rewardsProfit;
    ...
    function getBalance() public returns (uint256) {
        return QiTokenInterface(qiToken).balanceOfUnderlying(address(this));
    }
    ...
    function _lpProfit(uint256 aum) internal returns (uint256) {
        uint256 protocolBalance = getBalance();
        if(protocolBalance > aum) {
            uint256 expectedLpProfit = protocolBalance - aum;
            uint256 baseTokensBalanceBefore = IERC20Upgradeable(baseToken).balanceOf(address(this));
            uint256 error = QiErc20Interface(qiToken).redeemUnderlying(expectedLpProfit);
            require(error == 0, "Benqi: withdraw error");
            uint256 baseTokensBalanceAfter = IERC20Upgradeable(baseToken).balanceOf(address(this));
            return baseTokensBalanceAfter - baseTokensBalanceBefore;
        }

        return 0;
    }

    function _rewardsProfit() internal returns (uint256) {
        QiTokenInterface[] memory market = new QiTokenInterface[](1);
        market[0] = QiTokenInterface(qiToken);
        ...
        uint256 qiProfit = baseTokensBalanceAfter - baseTokensBalanceBefore;
        ...
        baseTokensBalanceBefore = IERC20Upgradeable(baseToken).balanceOf(address(this));
        IDex(dex).swapNative{value: rewardBalance}(
            rewards[1],
            baseToken,
            address(this)
        );
        baseTokensBalanceAfter = IERC20Upgradeable(baseToken).balanceOf(address(this));
        uint256 avaxProfit = baseTokensBalanceAfter - baseTokensBalanceBefore;

        return qiProfit + avaxProfit;
    }
    ...
    function _setComptroller(address _comptroller) internal {
        require(
            ...
            "Benqi: cannot be the zero address"
        );
        comptroller = _comptroller;
    }

    function _setQiToken(address _qiToken) internal {
        require(_qiToken != address(0), "Benqi: cannot be the zero address");
        require(baseToken != address(0), "Benqi: baseToken not initialized");
        qiToken = _qiToken;
        uint256 allowance = IERC20Upgradeable(baseToken).allowance(address(this), _qiToken);
        if (allowance == 0) {
            IERC20Upgradeable(baseToken).safeApprove(_qiToken, type(uint256).max);
        } else {
    ...
    function _enterMarket() internal {
        require(
            ...
            "Benqi: comptroller not initialized"
        );
        require(qiToken != address(0), "Benqi: qiToken not initialized");
        address[] memory market = new address[](1);
        market[0] = qiToken;
        uint256[] memory errors = ComptrollerInterface(comptroller)
            .enterMarkets(market);
        require(errors[0] == 0, "Benqi: failed to enter market");
    }
