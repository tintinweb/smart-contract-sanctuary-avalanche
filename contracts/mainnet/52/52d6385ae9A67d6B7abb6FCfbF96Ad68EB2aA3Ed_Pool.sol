...
contract Pool is Ownable, ReentrancyGuard {
    ...
    uint256 public constant REDEMPTION_FEE_MAX = 10000; // 1.0%
    uint256 public mintingFee = 5000; // 6 decimals of precision
    uint256 public constant MINTING_FEE_MAX = 10000; // 1.0%
    ...
    function setOracle(IMasterOracle _oracle) external onlyOwner {
        require(address(_oracle).code.length > 0, "Pool::setOracle: EOA not allowed");
        oracle = _oracle;
    ...
    function setYTokenSlippage(uint256 _slippage) external onlyOwner {
        require(_slippage <= 500000, "Pool::setYTokenSlippage: yTokenSlippage cannot be more than 50%");
    ...
    function setTreasury(address _treasury) external {
        require(_treasury.code.length > 0, "Pool::setTreasury: EOA not allowed");
        treasury = _treasury;
