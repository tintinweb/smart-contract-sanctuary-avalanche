...
contract TraderJoeBaseProxy is AccessControlEnumerable, Pausable {
    ...
    uint256 public fee = 100; // 100 = 1%
    uint256 public dollarFee = 100; // 100 = $1
    bool public feeInDollars = true;
    uint256 private maxApproval = 115792089237316195423570985008687907853269984665640564039457584007913129639935;
    address internal usdtToken = 0xc7198437980c041c805A1EDcbA50c1Ce5db95118;
    IERC20 public wavax;
    ...
    constructor(address _traderJoe, IERC20 _wavax, address oneInchAggregator, address _usdt) {
        aggregator = priceAggregator(oneInchAggregator);
    ...
    function callTokenPrice(address _token1, address _usdtToken) public view returns (uint256) {
        // console.log("price");
        // uint256 price = aggregator.getRate(_token1, _usdtToken, true);
        // console.log(price);
        // console.log("price");

        return aggregator.getRate(_token1, _usdtToken, true);
    }

    //55539347322
    //55000000000000000

    function calculateDollarFee(address _erc20Token, uint256 amountIn) public returns (uint256) {
        uint price = callTokenPrice(_erc20Token, usdtToken);
        uint256 decimals = IER(_erc20Token).decimals();
        uint priceDecimals = 18 - (decimals - 6);
        ...
        console.log(priceDecimals);
        console.log("priceDecimals");
        uint256 oneDolarInTokens;
        oneDolarInTokens = (10 ** 18) / (price);
        uint amountMinusDollarFee = amountIn - oneDolarInTokens;
        ...
        console.log(price);
        console.log("price");
        console.log("oneDolarInTokens");
        console.log(oneDolarInTokens);
        console.log("oneDolarInTokens");
        console.log("amountMinusDollarFee");
        console.log(amountMinusDollarFee);
        console.log("amountMinusDollarFee");
    }
    ...
    function setFeeInDollars(uint256 _newFee) public {
    ...
    /// @notice return the current fee in dollars
    /// @dev 1% = 100 untis. fee can not be bigger than 10%
    /// @return dollarFee
    function getDollarFee() public view returns (uint256) {
        return dollarFee;
    }
    ...
    function swapTokensForExactTokens(
        ...
        uint256 tokenBalanceBeforeSwapUSDT = getContractTokenBalance(_tokenPath[_pairBinSteps.length]);
        ...
        uint256 tokenBalanceAfterSwapUSDT = getContractTokenBalance(_tokenPath[_pairBinSteps.length]);
