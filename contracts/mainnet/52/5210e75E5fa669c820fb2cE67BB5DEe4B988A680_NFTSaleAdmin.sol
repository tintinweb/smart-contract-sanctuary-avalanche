...
contract NFTSaleAdmin is NFTSaleLib {
    ...
    constructor(
        ...
        require(externalStorage != address(0), "externalStorage is the zero address");
    }

    function addCurrency(string memory symbol, address currencyAddress) public onlyRole(MANAGER_ROLE) whenNotPaused
    {
        require(currencyAddress != address(0));
        _externalStorage.setAddress(keccak256(abi.encodePacked("currency", symbol)), currencyAddress);
    }

    function removeCurrency(string memory symbol) public onlyRole(MANAGER_ROLE) whenNotPaused
    {
        _externalStorage.deleteAddress(keccak256(abi.encodePacked("currency", symbol)));
    }

    function setFeeWallet(address payable feeWallet) public onlyRole(MANAGER_ROLE) whenNotPaused 
    {
        require(feeWallet != address(0), "wallet is the zero address");
        _externalStorage.setAddress(keccak256(abi.encodePacked("feeWallet")), feeWallet);
    }

    function allowNativeCurrency(bool value) public onlyRole(MANAGER_ROLE) whenNotPaused
    {
        _externalStorage.setBool(keccak256("allowNativeCurrency"), value);
    }
    ...
    function setFees(
        ...
        uint16 maxOwnerFeeRate
    ) public onlyRole(MANAGER_ROLE) whenNotPaused  {
        if (special==address(0))
        {   
            _externalStorage.setUint16(keccak256("firstSaleBuyerFeeRate"), firstSaleBuyerFeeRate);
            _externalStorage.setUint16(keccak256("secondSaleBuyerFeeRate"), secondSaleBuyerFeeRate);
            _externalStorage.setUint16(keccak256("firstSaleSellerFeeRate"), firstSaleSellerFeeRate);
            _externalStorage.setUint16(keccak256("secondSaleSellerFeeRate"), secondSaleSellerFeeRate);
            _externalStorage.setUint16(keccak256("maxOwnerFeeRate"), maxOwnerFeeRate);
        } else {
            uint256 blockNumber = block.number;
            _externalStorage.appendUIntArr(keccak256(abi.encodePacked("specialBlocks", special)), blockNumber);
            if (firstSaleBuyerFeeRate>0)
                _externalStorage.setUint16(keccak256(abi.encodePacked("firstSaleBuyerFeeRate",special, blockNumber)), firstSaleBuyerFeeRate);

            if (secondSaleBuyerFeeRate>0)
                _externalStorage.setUint16(keccak256(abi.encodePacked("secondSaleBuyerFeeRate",special, blockNumber)), secondSaleBuyerFeeRate);

            if (firstSaleSellerFeeRate>0)
                _externalStorage.setUint16(keccak256(abi.encodePacked("firstSaleSellerFeeRate",special, blockNumber)), firstSaleSellerFeeRate);

            if (secondSaleSellerFeeRate>0)
                _externalStorage.setUint16(keccak256(abi.encodePacked("secondSaleSellerFeeRate",special, blockNumber)), secondSaleSellerFeeRate);

            if (maxOwnerFeeRate>0)
                _externalStorage.setUint16(keccak256(abi.encodePacked("maxOwnerFeeRate",special, blockNumber)), maxOwnerFeeRate);
        }
    }
}
...
abstract contract NFTSaleLib is Pausable, ERC2771Context, AccessControl {
    using SafeERC20 for IERC20;
    bytes32 constant NATIVE_CURRENCY = keccak256(bytes("AVAX"));
