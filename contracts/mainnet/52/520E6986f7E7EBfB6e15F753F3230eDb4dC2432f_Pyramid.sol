...
contract Pyramid is ColorToken{
	...
	// Slope
    uint256 constant internal tokenPriceInitial_ = 0.0000000000000001 ether;
    uint256 constant internal tokenPriceIncremental_ = 0.00000000000000001 ether;

	// Typical values that we have to declare.
	string constant public name = "Color Clocks";
	string constant public symbol = "RGB";
	...
	//new new
	ERC20 $TIME = ERC20(0xb54f16fB19478766A268F172C9480f8da1a7c9C3);
	...
	function reinvestEarnings(uint amountFromEarnings) public returns(uint,uint){
		...
		if (value_ < 0.000000000000001 ether)
			revert();
	...
	function buy(address addr, uint _red, uint _green, uint _blue, uint msgValue) public returns(uint createdBonds){
		...
		require( $TIME.transferFrom(msg.sender,THIS,msgValue) );

		// Any transaction of less than 1 szabo is likely to be worth less than the gas used to send it.
		if ( msgValue < 0.000000000000001 ether )
			revert();

		// Calculate the fee
		uint fee = fluxFee(msgValue);

		// The amount of Ether used to purchase new bonds for the caller.
		uint numEther = msgValue;
		...
		emit Buy( addr, msgValue, createdBonds, resolveFee);
		return createdBonds;
	}
	...
	function sell(uint256 amount) internal returns(uint eth, uint resolves, uint initialInput){
		...
		$TIME.transfer(sender, numEthers);
