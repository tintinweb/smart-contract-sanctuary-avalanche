    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => ExtraRewardsPool) private legacyExtraRewardsPools;
    ...
    uint256 public queuedVeEcdRewards;
    uint256 public veEcdLastPaid;
    uint256 public queuedPlatformFee;
    mapping(uint256 => ExtraRewardsPool[]) public extraRewardsPools;
    address public ecdPtp;
    address public ptpDepositor;
    uint256 public queuedEcdPtpRewards;
    uint256 public ecdPtpLastPaid;
    ...
    function listExtraRewards(uint256 _pid)
        external
        view
        returns (ExtraRewardsPool[] memory)
    {
        return extraRewardsPools[_pid];
    }
    ...
    function setEcdPtp(address _ecdPtp) external onlyOwner {
        ecdPtp = _ecdPtp;
    }

    function setPtpDepositor(address _ptpDepositor) external onlyOwner {
        ptpDepositor = _ptpDepositor;
    }
    ...
    function _depositFor(
        ...
        uint256 _amount,
        address _account,
        bool __depositToPlatypus,
        uint256 _deadline
    ) internal {
        PoolInfo storage pool = pools[_pid];
        require(address(pool.lpToken) != address(0x0), "zero");
        require(pool.shutdown == false, "shutdown");

        if (__depositToPlatypus) {
            _amount = _depositToPlatypus(pool, _amount, _deadline);
        } else {
        ...
        migrate(pool, _account);
        IRewardPool(pool.rewardPool).stake(_account, _amount);
    }
    ...
    function _withdraw(
        ...
        (
            uint256 ptpClaimed,
            uint256[] memory extraRewardsClaimed
        ) = depositorProxy.withdraw(address(masterPlatypus), _pid, _amount);
    ...
    function claimRewards(uint256[] calldata _pids)
        external
        nonReentrant
        returns (uint256)
    {
        ...
        if (totalClaimed != 0) {
            ...
            uint256 _callIncentive = (totalClaimed * earmarkIncentive) /
                FEE_DENOMINATOR;
            IERC20(ptp).safeTransfer(msg.sender, _callIncentive);
            amountAfterFees -= _callIncentive;
    ...
    function _distributeIncentives(uint256 amount) private returns (uint256) {
        ...
        uint256 _platform = (amount * platformFee) / FEE_DENOMINATOR;
        if (_platform > 0) {
            queuedPlatformFee += _platform;
        }

        //send stakers's share of ptp to reward contract

        if (_ecdLockedIncentive != 0) {
            queuedVeEcdRewards += _ecdLockedIncentive;
            if (
                (block.timestamp - veEcdLastPaid) > 1 days &&
                veEcdRewardPool != address(0)
            ) {
                veEcdLastPaid = block.timestamp;
                ptp.approve(ptpDepositor, queuedVeEcdRewards);

                uint256 balBefore = IERC20(ecdPtp).balanceOf(address(this));
                IPTPDepositor(ptpDepositor).deposit(
                    queuedVeEcdRewards,
                    false,
                    address(0)
                );
                queuedVeEcdRewards = 0;
                uint256 toTransfer = IERC20(ecdPtp).balanceOf(address(this)) -
                    balBefore;

                IERC20(ecdPtp).approve(veEcdRewardPool, toTransfer);
                IVeEcdRewardsPool(veEcdRewardPool).queueNewRewards(toTransfer);
            }
        }

        if (_ecdPtpIncentive != 0) {
            queuedEcdPtpRewards += _ecdPtpIncentive;
            if ((block.timestamp - ecdPtpLastPaid) > 1 days) {
                ecdPtpLastPaid = block.timestamp;
                ptp.approve(ecdPtpRewardPool, queuedEcdPtpRewards);
                // sends PTP to reward
                IVeEcdRewardsPool(ecdPtpRewardPool).queueNewRewards(
                    queuedEcdPtpRewards
                );
                queuedEcdPtpRewards = 0;
            }
        }
        ...
        return amount - _ecdLockedIncentive - _platform - _ecdPtpIncentive;
    }

    function _distributeRewards(
        uint256 _pid,
        uint256 amount,
        uint256[] memory extraRewardAmounts
    ) internal {
        if (amount != 0) {
            address rewardPool = pools[_pid].rewardPool;
            IRewardPool(rewardPool).queueNewRewards(amount);
        }
        ExtraRewardsPool[] memory extraRewardsPoolsCache = extraRewardsPools[
            _pid
        ];
        if (extraRewardAmounts.length != 0) {
            // make sure all extra rewards pools exists
            if (extraRewardAmounts.length != extraRewardsPoolsCache.length) {
                // create missing pools
                IMasterPlatypus.PoolInfo memory poolInfo = masterPlatypus
                    .poolInfo(_pid);
                for (
                    uint256 _rewardId = extraRewardsPoolsCache.length;
                    _rewardId < extraRewardAmounts.length;
                    ++_rewardId
                ) {
                    _addExtraRewards(_pid, poolInfo.rewarder, _rewardId);
                }
                extraRewardsPoolsCache = extraRewardsPools[_pid]; // Read again after updates
            }
            for (uint256 i; i < extraRewardAmounts.length; ++i) {
                IERC20(extraRewardsPoolsCache[i].token).safeTransferFrom(
                    address(depositorProxy),
                    address(this),
                    extraRewardAmounts[i]
                );
                IVirtualBalanceRewardPool(
                    extraRewardsPoolsCache[i].virtualBalanceRewardPool
                ).queueNewRewards(extraRewardAmounts[i]);
            }
        }
        emit RewardsDistributed(_pid, amount, extraRewardAmounts);
    }
    ...
    function addNewPool(uint256 _pid) external onlyOwner {
        ...
        pools[_pid] = PoolInfo({
            ...
            shutdown: false,
            legacyRewardPool: address(0x0)
        });
        existingPools.push(_pid);
        depositorProxy.increaseAllowance(poolInfo.lpToken);

        IERC20(ptp).approve(newRewardPool, type(uint256).max);
    ...
    function _addExtraRewards(
        uint256 _pid,
        address rewarder,
        uint256 _rewardId
    ) internal {
        IRewarder.PoolInfo memory poolInfo = IRewarder(rewarder).poolInfo(
            _rewardId
        );
        address rewardToken = poolInfo.rewardToken;
        address additionalRewards = rewardFactory.createTokenRewards(
            rewardToken,
            pools[_pid].rewardPool,
            false
        );
        IERC20(rewardToken).approve(additionalRewards, type(uint256).max);

        extraRewardsPools[_pid].push(
            ExtraRewardsPool({
                token: rewardToken,
    ...
    function pullPlatformFee() public {
        require(msg.sender == treasury, "!auth");
        ptp.transfer(treasury, queuedPlatformFee);
        queuedPlatformFee = 0;
    }

    function migrate(PoolInfo memory _pool, address _account) internal {
        if (_pool.legacyRewardPool != address(0x0)) {
            uint256 balance = IRewardPool(_pool.legacyRewardPool).balanceOf(
                _account
            );
            if (balance != 0) {
                IRewardPool(_pool.legacyRewardPool).unStake(
                    _account,
                    balance,
                    true
                );
                IRewardPool(_pool.rewardPool).migrateStake(_account, balance);
            }
        }
    }

    function multiClaim(uint256[] calldata _pids, address _account) external {
        for (uint256 i = 0; i < _pids.length; ++i) {
            PoolInfo memory pool = pools[_pids[i]];
            migrate(pool, _account);

            IRewardPool(pool.rewardPool).getReward(_account, true);
        }
    }

    function migrateLegacyExtraRewardPools() external onlyOwner {
        for (uint256 i; i < existingPools.length; ++i) {
            ExtraRewardsPool memory extraPool = legacyExtraRewardsPools[
                existingPools[i]
            ];
            if (extraPool.virtualBalanceRewardPool != address(0)) {
                extraRewardsPools[existingPools[i]].push(extraPool);
            }
        }
    }
}
