...
contract WerewolfDividendTracker is IDividendDistributor {
...
contract Werewolf is Ownable, IERC20 {
    using SafeMath for uint256;
    ...
    string _name = "Werewolf";
    string _symbol = "$Werewolf";
    ...
    address treasuryWallet = address(0x2AEa2eC9fc908CB18E0b44996134c31ee6B9D531); //change this
    address liquidityWallet;

    ITraderJoeRouter public router; //trader joe
    address pair; 

    WerewolfDividendTracker public dividendTracker;
    ...
    constructor () {
        ...
        dividendTracker = new WerewolfDividendTracker();
		...
		setBuyFees(5, 0, 0, 0);
		setSellFees(0, 12, 2, 0);

        isReferred[owner_] = true;
    ...
    function takeReferralFees(address recipient,uint256 amount) internal returns(uint) {
        if (excludeFee[recipient]) 
            return amount;
        uint256 referralTokens = buyFees.referralFee * amount / feeDenominator;
        if(_balances[referrer[recipient]] > minSupplyForReferralReward) {
            _balances[referrer[recipient]] = _balances[referrer[recipient]].add(referralTokens);
            totalRewards[referrer[recipient]] = totalRewards[referrer[recipient]].add(referralTokens);
            if(_balances[recipient] <= 1e9){
            totalReferrals[referrer[recipient]] = totalReferrals[referrer[recipient]].add(1);
            }
            emit ReferralBonus(referrer[recipient],recipient,referralTokens);
        } else {
             _balances[owner()] = _balances[owner()].add(referralTokens);
            emit ReferralBonus(owner(),recipient,referralTokens);
        }
    ...
    function setBuyFees(uint256 _reflectionFee, uint256 _treasuryFee, uint256 _liquidityFee, uint256 _referralFee) public onlyOwner {
		buyFees = FeeSet({
			reflectionFee: _reflectionFee,
			treasuryFee: _treasuryFee,
			liquidityFee: _liquidityFee,
            referralFee: _referralFee,
			totalFee: _reflectionFee + _treasuryFee + _liquidityFee + _referralFee
		});
	}

	function setSellFees(uint256 _reflectionFee, uint256 _treasuryFee, uint256 _liquidityFee, uint256 _referralFee) public onlyOwner {
		sellFees = FeeSet({
			reflectionFee: _reflectionFee,
			treasuryFee: _treasuryFee,
			liquidityFee: _liquidityFee,
