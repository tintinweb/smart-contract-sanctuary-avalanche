...
contract NebulaProtoStarDrop is Ownable{
	...
	address Guard;
	address oldDrop = 0x1e042948c57F937546BAac445C4DBf4698AfD35B;
	bool avaxOff = false;
	bool tokenOff = false;
	...
	function payFeeToken(uint256 _intervals) payable external{
	    ...
	    theBigCheck(_account,total,total,"crrrt");
	...
	function createProtoFeeToken(string memory _name) payable external{
		...
		theBigCheck(_account,tokFee,sent,_name);
		uint256 allowance = feeTok.allowance(_account, treasury);
	...
	function payFeesAvax(uint256 _intervals) payable external{
		...
		address _account = msg.sender;
		if(avaxOff==true){
			payable(_account).transfer(sent);
			sent = 0;
		}
		uint256 fee =over.getFee();
		uint256 total = nebuLib.safeMuls(fee,_intervals);
		theBigCheck(_account,total,sent,"crrrt");
		...
		uint256 fee =over.getFee();
		theBigCheck(_account,fee,sent,_name);
	...
	function removeManagers(address newVal) external managerOnly() {
    	...
    	function turnTokenOff(bool _avaxOff) external onlyOwner{
    		tokenOff = _avaxOff;
        }
    	function turnAvaxOff(bool _avaxOff) external onlyOwner{
    		avaxOff = _avaxOff;
        }
        ...
        function changeOverseer(address _overseer) external  onlyOwner(){
