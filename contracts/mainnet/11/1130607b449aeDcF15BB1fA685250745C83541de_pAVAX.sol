...
abstract contract ERC31337 is WrappedERC20, IERC31337BNB
{
    ...
    function sweepFloor(address to) public override returns (uint256 amountSwept)
    {
        ...
        amountSwept = floorCalculator.calculateSubFloorPAVAX(wrappedToken, this);
...
contract pAVAX is ERC31337, IWETH
{
    ...
    constructor (address wethAddress)
        ERC31337(IWETH(wethAddress), "pAVAX", "pAVAX")
    {
   ...
   // 100 axbnb => 100 bnb is deposited to wbnb contract by this AXBNB contract => 100 AXBNB
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        require(sender != address(0), "pAVAX: transfer from the zero address");
        require(recipient != address(0), "pAVAX: transfer to the zero address");

        _beforeTokenTransfer(sender, recipient, amount);

        // cut no FEE
        if(IGNORED_ADDRESSES[recipient]){
            _balanceOf[sender] = _balanceOf[sender].sub(amount, "pAVAX: transfer amount exceeds balance");
            ...
            uint256 pAVAXFee = amount.mul(FEE).div(100000); 
            uint256 remAmount = amount.sub(pAVAXFee);

            if(pAVAXFee>0){
                _balanceOf[FEE_ADDRESS] = _balanceOf[FEE_ADDRESS].add(pAVAXFee);
                emit Transfer(sender, FEE_ADDRESS, pAVAXFee);
            }
