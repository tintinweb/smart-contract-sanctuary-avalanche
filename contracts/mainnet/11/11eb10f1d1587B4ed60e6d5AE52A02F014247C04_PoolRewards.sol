...
import "../interfaces/IVPoolRewardsV1.sol";
import "../interfaces/IThorPool.sol";
...
contract PoolRewards is Initializable, IPoolRewards, ReentrancyGuardUpgradeable, PoolRewardsStorage {
     ...
     */
    function initialize(address _pool, address[] memory _rewardTokens, address _notifier) public initializer {
    ...
    modifier onlyAuthorized() {
        require(msg.sender == notifier, "not-authorized");
        _;
    }
    ...
    //     _notifyRewardAmount(_rewardTokens, _rewardAmounts, _rewardDurations, IThorPool(pool).totalShares());
    // }
    ...
    function notifyRewardAmount(
        ...
        _notifyRewardAmount(_rewardToken, _rewardAmount, _rewardDuration, IThorPool(pool).totalShares());
    }
    ...
    function claimReward(address _account) external virtual override nonReentrant {
        uint256 _totalSupply = IThorPool(pool).totalShares();
        uint256 _balance = IThorPool(pool).getShares(_account);
        ...
        for (uint256 i = 0; i < _len; i++) {
            ...
            if (_reward != 0 && _reward <= IERC20Upgradeable(_rewardToken).balanceOf(address(this))) {
    ...
    function _claimReward(
        ...
        // Transfer reward
        IERC20Upgradeable(_rewardToken).safeTransfer(_account, _reward);
    }
    ...
    /// @notice Allows owner to update the notifier address
    /// @param _newNotifier address
    function updateNotifier(address _newNotifier) external onlyAuthorized {
        notifier = _newNotifier;
    }


    /// @notice Allows owner to withdraw any ERC20 tokens collected in the contract
    /// @dev Partial withdrawals permitted
    /// @param _token address
    /// @param _amount Being withdrawn
    function withdrawBalance(address _token, uint256 _amount) external onlyAuthorized {
        IERC20Upgradeable token = IERC20Upgradeable(_token);
        require(token.balanceOf(address(this)) >= _amount, "withdraw");
...
interface IThorPool {
    ...
    function getInvestment(address _user) external view returns (uint256);
