...
interface IGMXAVAXNeutralManager {
  struct WorkData {
    address token;
    uint256 lpAmt;
    uint256 borrowWAVAXAmt;
    uint256 borrowWETHAmt;
    uint256 borrowBTCbAmt;
    uint256 borrowUSDCAmt;
    uint256 repayWAVAXAmt;
  ...
  function lendingPoolWAVAX() external view returns (address);
...
interface IGMXAVAXNeutralVault {
  struct VaultConfig {
...
import "../../../../interfaces/vaults/gmx/v2/avax/IGMXAVAXNeutralVault.sol";
import "../../../../interfaces/vaults/gmx/v2/avax/IGMXAVAXNeutralManager.sol";
...
contract GMXAVAXNeutralReader {
  using SafeCast for uint256;

  /* ========== STATE VARIABLES ========== */

  // Vault's address
  IGMXAVAXNeutralVault public immutable vault;
  // Vault's manager address
  IGMXAVAXNeutralManager public immutable manager;
  // Chainlink oracle
  ...
  uint256 public constant BTCb_DECIMALS = 8;
  ...
  */
  constructor(
    IGMXAVAXNeutralVault _vault,
    IGMXAVAXNeutralManager _manager,
  ...
  */
  function _minOf4(uint256 a, uint256 b, uint256 c, uint256 d) internal pure returns (uint256) {
    uint256 min = a;
    if (b < min) min = b;
    if (c < min) min = c;
    if (d < min) min = d;
  ...
  */
  function debtValues() public view returns (uint256) {
    uint256 debtWAVAX = manager.debtAmt(WAVAX);
    uint256 debtWETH = manager.debtAmt(WETH);
    uint256 debtBTCb = manager.debtAmt(BTCb);
    uint256 debtUSDC = manager.debtAmt(USDC);

    return tokenValue(WAVAX, debtWAVAX)
      + tokenValue(WETH, debtWETH)
      + tokenValue(BTCb, debtBTCb)
  ...
  */
  ...
  /**
    * Returns the amt of token debt held by manager (WETH, BTCb, USDC)
  ...
  */
  function delta(address _token) public view returns (int256) {
    ...
    for (uint256 i = 0; i < tokenAddresses.length; i ++) {
      // Store the values of BTCb and WBTC so we can add them up after
      // Normalize for BTCb and WBTC having only 8 decimals
      // We do not care about USDC's delta so we do not worry about it
      if (tokenAddresses[i] == BTCb) BTCbAmt = tokenAmts[i] / 10**(18 - BTCb_DECIMALS);
      if (tokenAddresses[i] == WBTC) WBTCAmt = tokenAmts[i] / 10**(18 - BTCb_DECIMALS);

      // For every other asset besides BTC
      if (tokenAddresses[i] == _token) tokenAmt = tokenAmts[i];
    }

    // Since we utilize only BTCb, we combine WBTC and BTCb values together
    // if not, tokenAmt will still be the other non-BTC amounts
    if (_token == BTCb || _token == WBTC) tokenAmt = BTCbAmt + WBTCAmt;
  ...
  */
  ...
  /**
    ...
    * @return tokenAddress array of whitelisted tokens
  ...
  */
  function additionalCapacity() public view returns (uint256) {
    IGMXAVAXNeutralVault.VaultConfig memory _vaultConfig = vault.vaultConfig();

    address lendingPoolWAVAX = manager.lendingPoolWAVAX();
    address lendingPoolWETH = manager.lendingPoolWETH();
    address lendingPoolBTCb = manager.lendingPoolBTCb();
    address lendingPoolUSDC = manager.lendingPoolUSDC();

    uint256 additionalCapacityWAVAX = tokenValue(address(WAVAX), ILendingPool(lendingPoolWAVAX).totalAvailableSupply())
      * SAFE_MULTIPLIER
      / (currentTokenWeight(WAVAX)
    ...
    uint256 additionalCapacityBTCb = tokenValue(address(BTCb), ILendingPool(lendingPoolBTCb).totalAvailableSupply())
      * SAFE_MULTIPLIER
      / (currentTokenWeight(BTCb)
      * (_vaultConfig.targetLeverage)
      / SAFE_MULTIPLIER);

    uint256 additionalCapacityUSDC = tokenValue(address(USDC), ILendingPool(lendingPoolUSDC).totalAvailableSupply())
      * SAFE_MULTIPLIER
      / ((1e18 - currentTokenWeight(WAVAX) - currentTokenWeight(WETH) - currentTokenWeight(WBTC) - 0.33e18)
      * (_vaultConfig.targetLeverage)
      / SAFE_MULTIPLIER);

    return _minOf4(additionalCapacityWAVAX, additionalCapacityWETH, additionalCapacityBTCb, additionalCapacityUSDC);
  }
