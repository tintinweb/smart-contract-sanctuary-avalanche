...
contract UnderpegNFTPoolNG is Controllable, IERC721Receiver, MyPausable {
    ...
    uint256 public generationMax = 23;
    ...
    modifier checkGeneration(uint256 tokenId) {
        uint256 g = IPreyPredator(preyPredator).getTokenTraits(tokenId).generation;
        require(
            g <= generationMax && g >= generationMin || generationWhitelist[g],
            "not eligible nft"
        );
        _;
    }
