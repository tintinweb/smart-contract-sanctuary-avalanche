...
contract Roulette is Ownable, ReentrancyGuard {
    ...
    function provideTokensToHouse(address token_, uint256 amount_, uint256 riskPerc_) external nonReentrant noUnallowedTokens(token_) {
        ...
        uint256 beforeTransfer = IERC20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
        IERC20(token_).transferFrom(msg.sender, houseAddress, amount_);
        uint256 afterTransfer = IERC20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
        ...
        noUnbetableTokens(token_)
    {
        ...
        require(IERC20(token_).allowance(msg.sender, address(this)) >= totalBet, "Roulette::gamble: Allowance too low");
        ...
        require(
            totalBet <= IERC20(token_).balanceOf(msg.sender),
        ...
        if (amount_[randomNumber] > 0) {
            ...
            // If the win does not get a net reward to the player cause his total bet was higher, providers get a bump to their share, based on their riskable amount.
                ...
                transferTax = IERC20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
                IERC20(token_).transferFrom(msg.sender, houseAddress, (totalBet + txFeeAmount) - (winAmount - winFeeAmount));
                transferTax = IERC20(token_).balanceOf(houseAddress) - transferTax; // Support for transfer-tax tokens.
            }
            ...
            if (gambleToken != address(0) && priceCalc != address(0)) {
                IERC20(gambleToken).mint(msg.sender, feeRefundInGamble(token_, winFeeAmount + txFeeAmount));
            }
            ...
            transferTax = IERC20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
            IERC20(token_).transferFrom(msg.sender, houseAddress, totalBet);
            transferTax = IERC20(token_).balanceOf(houseAddress) - transferTax; // Support for transfer-tax tokens.

            IERC20(token_).transferFrom(msg.sender, ustFundAddress, txFeeAmount);

            // Fees refund in GAMBLE.
            if (gambleToken != address(0) && priceCalc != address(0)) {
                IERC20(gambleToken).mint(msg.sender, feeRefundInGamble(token_, txFeeAmount));
            }
    ...
    function getAllowedTokenAddressFromSymbol(string memory symbol_) external view returns (address allowedToken) {
        for (uint i = 0; i < tokens.length; i++) {
            if (keccak256(bytes(IERC20(tokens[i]).symbol())) == keccak256(bytes(symbol_))) {
                return tokens[i];
            }
        }
        return address(0);
