...
contract LPStrategy is BaseStrategy {
    ...
    ICauldronV2 public cauldronV2 = ICauldronV2(0x88d5225D5ccBD7372095C81Ccff466d71A9df9Ba);
    ...
    */
    ...
    function swapToLP(uint256 amountOutMin) public onlyExecutor returns (uint256 amountOut) {
        ...
        if (balanceOfWAVAX >= 1000000000000000) {
    ...
    function setFEEReward(uint256 _amount) external onlyOwner {
        FEEReward = _amount;
    }
    ...
    function fitstHarvest() public onlyExecutor {
    ...
    function autoHarvest() public onlyExecutor {
        ...
        uint256 maxChangeAmount = (maxBalance * 10) / 100;
    ...
    function userPending(address _user) external view returns (uint256) {
        ...
        uint256 valForUsd = ((pendingJoe * 1e36) / IOracle(usdt_joe_Oracle).peekSpot(new bytes(0))) +
            ((pendingBonusToken * 1e36) / IOracle(usdt_ptp_Oracle).peekSpot(new bytes(0)));
        uint256 lpAmt = valForUsd / (1e36 / IOracle(usdt_wavax_ptp_lp_Oracle).peekSpot(new bytes(0)));
        return (lpAmt * ICauldronV2(cauldronV2).userCollateralShare(_user)) / ICauldronV2(cauldronV2).totalCollateralShare();
    }

    function apy() external view returns (uint256) {
        (uint256 pendingJoe, , , uint256 pendingBonusToken) = IMasterChefV3(address(masterchef)).pendingTokens(pid, address(this));
        (uint256 amount, ) = IMasterChefV3(address(masterchef)).userInfo(pid, address(this));

        // uint256 lastRewardTimestamp = IMasterChefV3(address(masterchef)).poolInfo(pid).lastRewardTimestamp;
        uint256 allocPoint = IMasterChefV3(address(masterchef)).poolInfo(pid).allocPoint;
        uint256 totalAllocPoint = IMasterChefV3(address(masterchef)).totalAllocPoint();
        uint256 joePerSec = IMasterChefV3(address(masterchef)).joePerSec();

        // uint256 multiplier = block.timestamp - (lastRewardTimestamp);
        IERC20 lpToken = IMasterChefV3(address(masterchef)).poolInfo(pid).lpToken;
        uint256 joeRewardPerSec = (joePerSec * allocPoint) / totalAllocPoint;
        uint256 lpSupply = lpToken.balanceOf(address(masterchef));

        uint256 _joeRewardPerSec = (joeRewardPerSec * amount) / lpSupply;

        uint256 _ptpRewardPerSec = (pendingBonusToken * _joeRewardPerSec) / pendingJoe;

        uint256 valForUsd = (_joeRewardPerSec * (1e36 / IOracle(usdt_joe_Oracle).peekSpot(new bytes(0)))) +
            (_ptpRewardPerSec * (1e36 / IOracle(usdt_ptp_Oracle).peekSpot(new bytes(0))));
        uint256 lpForUsd = amount * (1e36 / IOracle(usdt_wavax_ptp_lp_Oracle).peekSpot(new bytes(0)));

        return ((valForUsd * (3600 * 24 * 365)) * 1e18) / lpForUsd;
    }
}
