...
contract CaesarShiba is ERC20 {
    ...
    address public chestAddress = 0xb5A80DB14B045ff930aea9ee807009ed226C4F03;
    address payable public marketingAddress = payable(0x7e1F3165a051919570C29b6D7Ce0117Ea68c7218);
    ...
    constructor() ERC20("MagicToken", "MTKN") {
        _owner = payable(msg.sender);

        address uniswapV2RouterAddress = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    ...
    function launch(string memory newName, string memory newSymbol) public onlyOwner{
        changeName(newName, newSymbol);
        setPresaleActivation(false);
    }

    function _addLiquidity(uint amountTokenDesired, uint amountAVAX) private
    {
        _approve(address(this), address(uniswapV2Router), amountTokenDesired);
        uniswapV2Router.addLiquidityAVAX{value: amountAVAX}(address(this), amountTokenDesired, 0, 0, _owner, block.timestamp);
    }

    function swapTokensForAVAX(uint amountToken) private
    {
        // Step 1 : approve
        _approve(address(this), address(uniswapV2Router), amountToken);

        // Step 2 : swapExactTokensForAVAX
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WAVAX();

        uniswapV2Router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(amountToken, 0, path, address(this), block.timestamp + 1 minutes);
    }
    ...
    function _swapAndLiquify() internal lockTheSwap
    {
        ...
        if (contractBalance > minSwapAmount || _manualSwap)
        {
            ...
            swapTokensForAVAX(marketingTokens.add(liquidityTokensHalf));
            uint amountAVAXToLiquefy = address(this).balance.mul(liquidityTokensHalf).div(marketingTokens.add(liquidityTokensHalf));
            _addLiquidity(liquidityTokensOtherHalf, amountAVAXToLiquefy);
    ...
    function addLiquidityInit(uint amountTokenDesired) public payable onlyOwner
    {
        ...
        _approve(address(this), address(uniswapV2Router), amountTokenDesired);
        uniswapV2Router.addLiquidityAVAX{value: msg.value}(address(this), amountTokenDesired, 0, 0, msg.sender, block.timestamp);
    }

    function retrieveAVAXFromContract() public onlyOwner {
        (bool sent,) = _owner.call{value: address(this).balance}("");
        require(sent, "Failed to send Ether");
    }

    function getAVAXBalance(address account) public view onlyOwner returns (uint) {
