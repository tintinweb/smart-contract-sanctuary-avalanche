...
contract PowerBombAvaxJoeStake is PowerBombAvaxJoe {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for IPair;
    using SafeERC20Upgradeable for IWAVAX;

    IsJOE constant sJOE = IsJOE(0x1a731B2299E22FbAC282E7094EdA41046343Cb51);
    IERC20Upgradeable constant USDC = IERC20Upgradeable(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E);

    function initialize(
        IMasterChef _masterChef, uint _poolId,
        IERC20Upgradeable _rewardToken, IERC20Upgradeable _extraRewardToken,
        address _treasury, address _bot
        ...
        rewardToken.safeApprove(address(sJOE), type(uint).max);
        ...
        if (address(token0) != address(USDC) && address(token1) != address(USDC)) {
            USDC.safeApprove(address(router), type(uint).max);
        }
        if (
            address(extraRewardToken) != address(0) && 
            address(token0) != address(extraRewardToken) &&
            address(token1) != address(extraRewardToken)
        ) {
            extraRewardToken.safeApprove(address(router), type(uint).max);
        }
    }

    function harvest() public override {
        // Record balance of WAVAX and USDC before harvest from farm
        uint WAVAXBal = WAVAX.balanceOf(address(this));
        ...
        // Collect USDC from sJOE farm
        sJOE.withdraw(0);
        USDCAmt = USDC.balanceOf(address(this)) - USDCAmt;

        uint estimatedWAVAX;
        // Get estimated JOE in WAVAX
        if (JOEAmt > 1e15) { // To prevent not enough input error
            estimatedWAVAX += router.getAmountsOut(JOEAmt, getPath(address(JOE), address(WAVAX)))[1];
        }
        // Get estimated USDC in WAVAX
        if (USDCAmt > 1e3) { // To prevent not enough input error
            estimatedWAVAX += router.getAmountsOut(USDCAmt, getPath(address(USDC), address(WAVAX)))[1];
        }
        // Get estimated extra reward token in WAVAX if any (extra WAVAX reward not count as extra reward token)
        if (address(extraRewardToken) != address(0)) {
            uint extraRewardTokenAmt = extraRewardToken.balanceOf(address(this));
            if (extraRewardTokenAmt > 1e15) { // To prevent not enough input error
                estimatedWAVAX += router.getAmountsOut(extraRewardTokenAmt, getPath(address(extraRewardToken), address(WAVAX)))[1];
            }
        }

        uint WAVAXAmt = WAVAX.balanceOf(address(this)) - WAVAXBal; // If extra reward is WAVAX
        estimatedWAVAX += WAVAXAmt;
        if (estimatedWAVAX > 25e16) { // 0.25 WAVAX
            // Swap JOE to WAVAX
            if (JOEAmt > 1e15) WAVAXAmt += swap2(address(JOE), address(WAVAX), JOEAmt, 0);

            // Swap USDC to WAVAX
            if (USDCAmt > 1e3) WAVAXAmt += swap2(address(USDC), address(WAVAX), USDCAmt, 0);

            // Swap WAVAX to JOE
            uint rewardTokenAmt = swap2(address(WAVAX), address(JOE), WAVAXAmt, 0);
            ...
            // Deposit JOE to sJOE farm
            sJOE.deposit(rewardTokenAmt);
            ...
            emit Harvest(WAVAXAmt, rewardTokenAmt, fee, currentPool);
        }
    }
    ...
    function claimReward(address account) public override {
        ...
        if (user.lpTokenBalance > 0) {
            ...
            if (rewardTokenAmt > 0) {
                ...
                // Withdraw JOE from sJOE
                sJOE.withdraw(rewardTokenAmt);
...
contract PowerBombAvaxJoe is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    mapping(address => uint) private userAccReward; // Depreciated
    ...
    // Newly added variable after deploy
    IERC20Upgradeable public extraRewardToken;
    ...
    event Harvest(uint indexed harvestedfarmToken, uint swappedRewardTokenAfterFee, uint fee, uint currentPool);
    event ClaimReward(address indexed receiver, uint claimedIbRewardTokenAfterFee, uint indexed rewardToken);
    event SetTreasury(address oldTreasury, address newTreasury);
    event SetBot(address oldBot, address newBot);
    event SetHelper(address oldHelper, address newHelper);
    ...
    modifier onlyOwnerOrHelper {
        require(msg.sender == owner() || msg.sender == helper, "Only owner or helper");
        _;
    }
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint amountOutMin) external virtual {
        uint amountOutToken = _withdraw(token, amountOutLpToken, amountOutMin);
        token.safeTransfer(msg.sender, amountOutToken);
    }

    function withdrawAVAX(IERC20Upgradeable token, uint amountOutLpToken, uint amountOutMin) external virtual {
    ...
    function harvest() public virtual {
        ...
        if (ibRewardTokenAmt > lastIbRewardTokenAmt) {
            ...
            uint accruedAmt = ibRewardTokenAmt - lastIbRewardTokenAmt;
        ...
        // Get balance of extraRewardToken in AVAX if any
        uint _WAVAXAmt;
        uint extraRewardTokenAmt;
        if (address(extraRewardToken) != address(0)) {
            extraRewardTokenAmt = extraRewardToken.balanceOf(address(this));
            _WAVAXAmt += router.getAmountsOut(extraRewardTokenAmt, getPath(address(extraRewardToken), address(WAVAX)))[1];
        }

        uint JOEAmt = JOE.balanceOf(address(this));
        uint WAVAXAmt = WAVAX.balanceOf(address(this)); // If any
        _WAVAXAmt = router.getAmountsOut(JOEAmt, getPath(address(JOE), address(WAVAX)))[1] + WAVAXAmt;
        if (_WAVAXAmt > 25e16) { // 0.25 WAVAX
            // Swap JOE to WAVAX
            WAVAXAmt += swap2(address(JOE), address(WAVAX), JOEAmt, 0);

            // Swap extraRewardToken to WAVAX if any
            if (extraRewardTokenAmt > 1e15) { // To prevent not enough input error
                WAVAXAmt += swap2(address(extraRewardToken), address(WAVAX), extraRewardTokenAmt, 0);
            }
            ...
            if (unclaimedRewardsAmt > 1e16) {
                WAVAXAmt += incentivesController.claimRewards(assets, unclaimedRewardsAmt, address(this));
            }
    ...
    function setTreasury(address _treasury) external onlyOwnerOrHelper {
    ...
    function setBot(address _bot) external onlyOwnerOrHelper {
        address oldBot = bot;
        bot = _bot;
    ...
    function setHelper(address _helper) external onlyOwnerOrHelper {
        address oldHelper = helper;
        helper = _helper;

        emit SetHelper(oldHelper, _helper);
    }

    function setYieldFeePerc(uint _yieldFeePerc) external onlyOwnerOrHelper {
    ...
    function setTVLMaxLimit(uint _tvlMaxLimit) external onlyOwnerOrHelper {
    ...
    function setExtraRewardToken(IERC20Upgradeable _extraRewardToken) external onlyOwner {
        extraRewardToken = _extraRewardToken;
    }

    function pauseContract() external onlyOwnerOrHelper {
        _pause();
    }

    function unpauseContract() external onlyOwnerOrHelper {
        _unpause();
    }
    ...
    function getPoolPendingReward() external virtual view returns (uint pendingRewards, uint pendingBonus) {
