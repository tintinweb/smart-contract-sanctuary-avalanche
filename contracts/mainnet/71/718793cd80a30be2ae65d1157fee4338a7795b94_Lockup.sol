...
contract Lockup is Ownable {
    ...
    uint256 public minInterval = 4 hours; // rewards is accumulated every 4 hours
    ...
    function claimReward(string memory name) public {
        ...
        uint256 reward = _calculateReward(name);
        require(stakingToken.balanceOf(address(this)) >= totalStaked + reward, "Insufficent pool balance");
        bytes32 key = _string2byte32(name);
    ...
    function isClaimable(string memory name) public view returns(bool) {
        ...
        // if((block.timestamp - lastblock) / rewardClaimInterval * 1 seconds > 0) return true;
        else return false;
    }
