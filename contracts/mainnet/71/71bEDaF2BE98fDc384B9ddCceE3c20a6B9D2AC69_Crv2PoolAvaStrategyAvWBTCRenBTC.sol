...
contract Crv2PoolAvaStrategyAvWBTCRenBTC is CrvPoolStrategyBase {
    ...
    // Withdraw all fund and send to pool. Curve strategy need some fixes so lets withdrawAll and remove curve strategy
    function withdrawAll() external onlyGovernor {
        uint256 _totalDebt = IVesperPool(pool).totalDebtOf(address(this));
        _claimRewardsAndConvertTo(address(collateralToken));
        _unstakeLp(IERC20(crvGauge).balanceOf(address(this)));
        _withdrawAsFromCrvPool(IERC20(crvLp).balanceOf(address(this)), 1, collIdx);
        uint256 _balanceHere = collateralToken.balanceOf(address(this));
        uint256 _profit;
        uint256 _loss;
        uint256 _payback;
        if (_balanceHere > _totalDebt) {
            _payback = _totalDebt;
            _profit = _balanceHere - _totalDebt;
        } else {
            _payback = _balanceHere;
            _loss = _totalDebt - _balanceHere;
        }
        IVesperPool(pool).reportEarning(_profit, _loss, _payback);
    }
