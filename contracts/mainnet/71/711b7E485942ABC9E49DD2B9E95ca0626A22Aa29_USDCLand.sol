...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    string private _name = "USDCLand";
    string private _symbol = "USEED";
...
contract USDCLand is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address internal constant avariceTeam_addr = 0x6674393a1314D9037345efFe38a5c6bd90632f81; // 
    ...
    function launch() public onlyOwner(){
		...
		LAUNCH_TIME = block.timestamp.sub(5 minutes);
        launched = true;
    } 
    ...
    function flushFirstDayLobbyEntry() external onlyOwner() nonReentrant {
        ...
        IERC20(stableCoin).safeTransfer(avariceTeam_addr,(lobbyEntry[1] * 20) /100);
    }

    /* Flushed lottery pool*/ 
    function flushLottyPool() external onlyOwner() nonReentrant {
        IERC20( stableCoin ).safeTransfer(address(avariceTeam_addr), lottery_Pool);
    }
    ...
    function _clcDay() public view returns (uint256) {
        return (block.timestamp - LAUNCH_TIME) / (5 minutes);
    }
    ...
    function _updateDaily() public {
        ...
        if (currentDay == 1){
            for(uint256 _day = 3 ; _day <= 4 ; _day++){
                dayBNBPool[_day] += (lobbyEntry[currentDay] * 725 ) / (2 * 1000);
            }
        }else if (currentDay < dividendsPoolCapDays) {
            for(uint256 _day = currentDay + 1 ; _day <= currentDay * 2 ; _day++){
                dayBNBPool[_day] += (lobbyEntry[currentDay] * 925 ) / (currentDay * 1000);
            }

        } else {
            for(uint256 _day = currentDay + 1 ; _day <= currentDay + dividendsPoolCapDays ; _day++){
                dayBNBPool[_day] += (lobbyEntry[currentDay] * 925 ) / (dividendsPoolCapDays * 1000);
            }
        }
    ...
    function _sendDevShare() internal nonReentrant {
        ...
        IERC20(stableCoin).safeTransfer(avariceTeam_addr,avariceTeamPercentage);
    ...
    function _sendMarketingShare() internal nonReentrant {
        ...
        IERC20(stableCoin).safeTransfer(marketing_addr,(lobbyEntry[currentDay - 1] * DM_marketing_percentage) /100);
    }
    ...
    function _buyLobbyBuybackShare() internal nonReentrant {
        ...
        IERC20(stableCoin).safeTransfer(buyBack_addr,(lobbyEntry[currentDay - 1] * DM_buyBack_percentage) /100);
    }
     ...
     */
    function EnterLobby(address referrerAddr, uint256 rawAmount) external {
        require(rawAmount > 0, "ERR: Amount required");
        _updateDaily();
        require(currentDay > 0);

        IERC20( stableCoin ).transferFrom( msg.sender, address(this), rawAmount);
    ...
    function _clcTokenValue (address _address, uint256 _Day) public view returns (uint256) {
        ...
        if(_Day != 0 && _Day < currentDay) {
            _tokenVlaue = (tokenForDay(_Day) * mapMemberLobby[_address][_Day].memberLobbyValue)/lobbyEntry[_Day]; 
        }else{
            _tokenVlaue = 0;
        }
    ...
    function EndStake(uint256 stakeId) external nonReentrant {
        ...
        IERC20( stableCoin ).safeTransfer( msg.sender, profit );
    ...
    */
    ...
    function checkForLotteryWinner() internal nonReentrant {
        if (lottery_topBuy_today > lottery_topBuy_latest) {
            ...
            if (currentDay >= 7) {
                IERC20(stableCoin).safeTransfer(address(lottery_topBuyer_today), (lottery_Pool * 30 /100));                
