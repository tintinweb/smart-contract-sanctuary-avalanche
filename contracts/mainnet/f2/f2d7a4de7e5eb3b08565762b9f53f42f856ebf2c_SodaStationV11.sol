...
import "../Soda.sol";
import "../Pizza.sol";
import "./SodaStationRobotV3.sol";

contract SodaStationV11 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function getSodaAccrued(address _owner) public view returns (uint256) {
        (uint256 spd, uint256 sdpPenalty) = getTotalSPD(_owner);
        return stationSoda[_owner] + getSodaAccruedForSPD(_owner, spd, sdpPenalty);
    }

    function getSodaAccruedForSPD(address _owner, uint256 _spd, uint256 _spdPenalty) internal view returns (uint256) {
        if(ownedCupsBalance[_owner] > getNumberSkillModifier(_owner) + sodaStationRobot.getExtraCups(_owner)){
            return 0;
        }
        if(numberOfStaked[_owner][0]+numberOfStaked[_owner][1]+ sodaStationRobot.getRobotSlots(_owner) != ownedCupsBalance[_owner]){
            return 0;
        }
        ...
        uint256 bonus = YIELDSPS * (100 + getBoostSkillModifier(_owner)) *  (100 + getTotalMasterChefBonus(_owner)) * (100 + sodaStationRobot.getProductionBonus(_owner));

        uint256 result = (endTime - startTimeOwner) * _spdPenalty * bonus / 100000000;

        if(endTimeForBoost > startTimeOwner) {
            uint256 _energy = energyBoostForOwner[_owner];
            if(durPenStartTime > startTimeOwner) {
                _energy = _spd * energyDrinkModifier;
            }

            if(endTimeForBoost > endTime) {
                result += (endTime - startTimeOwner)* _energy * bonus / 10000000000;
            }
            else {
                result += (endTimeForBoost - startTimeOwner)* _energy * bonus / 10000000000;
            }
        }

        if(durPenStartTime > startTimeOwner && _spd > _spdPenalty) {
            result += (durPenStartTime - startTimeOwner)* (_spd - _spdPenalty) * bonus / 100000000;
        }

        return result;
    }

    //Now this function returns SPD times 100
    function getTotalSPD(address _owner) public view returns (uint256 totalSPD, uint256 totalSPDPenalty) {
        ...
        for (uint256 i = 0; i < ownedCupsBalance[_owner]; i++) {
            ...
            uint256 totalDurability = cupTypes[cup.cupType].durability + durabilityBonus;
            if(cup.claims <  totalDurability){
                totalSPD += 100*cupTypes[cup.cupType].yield;
                totalSPDPenalty += 100*cupTypes[cup.cupType].yield * (totalDurability - cup.claims) / totalDurability;
            }
        }
        totalSPDPenalty += sodaStationRobot.getAssistantSPD(_owner, durabilityBonus);
    }
    ...
    function createCupForPlayer(uint256 _cupType, uint256[] calldata _upgradeIds) external {
        ...
        cupType.lpToken.safeTransferFrom(address(owner), treasuryWallet, transactionCost);

        require(ownedCupsBalance[owner] + upgradeLength <= getNumberSkillModifier(owner) + sodaStationRobot.getExtraCups(owner), "invalid number of cups");
    ...
    */
    ...
    function repairCups(uint256[] calldata _cupIds) external {
        ...
        uint256 discount = (100 - getSizeSkillModifier(owner))* (100 - getRepairSkillModifier(owner)) * (100 - sodaStationRobot.getNuclearCoreBonus(owner));
        ...
        for (uint256 i = 0; i < cupsLength; i++) {
            ...
            uint256 repairCost = (cupType.lpCost + cup.repairs * cupType.lpCost / 4)* discount/1000000;  
            cupType.lpToken.safeTransferFrom(address(owner), treasuryWallet, repairCost);
    ...
    function _claimSoda(address _owner) internal {
        uint256 totalSPD;
        uint256 totalSPDPenalty;
        uint256 durabilityBonus = getDurabilitySkillModifier(_owner);
        for (uint256 i = 0; i < ownedCupsBalance[_owner]; i++) {
            StationCups storage cup = stationCups[ownedCups[_owner][i]];
            uint256 totalDurability = cupTypes[cup.cupType].durability + durabilityBonus;
            if(cup.claims <  totalDurability){
                totalSPD += 100*cupTypes[cup.cupType].yield;
                totalSPDPenalty += 100*cupTypes[cup.cupType].yield * (totalDurability - cup.claims) / totalDurability;
        ...
        totalSPDPenalty += sodaStationRobot.claimAndReturnSPDAssistant(_owner, durabilityBonus);

        uint256 totalClaimed = stationSoda[_owner] + getSodaAccruedForSPD(_owner, totalSPD, totalSPDPenalty);
    ...
    function batchedCupsOfOwner(
        ...
        uint256 discount = (100 - getSizeSkillModifier(_owner))* (100 - getRepairSkillModifier(_owner)) * (100 - sodaStationRobot.getNuclearCoreBonus(_owner));
    ...
    function energyDrink(uint256 _qty) public {
        ...
        require(_qty > 0 && _qty <= 1 + sodaStationRobot.getEnergyDrinkBonus(_owner), "invalid qty");
        (, uint256 sdpPenalty) = getTotalSPD(_owner);
        uint256 costToReset =  _qty * sdpPenalty * energyDrinkCost * (100 + getBoostSkillModifier(_owner)) *  (100 + getTotalMasterChefBonus(_owner)) * (100 + sodaStationRobot.getProductionBonus(_owner)) / 100000000;
        ...
        energyBoostForOwner[_owner] = sdpPenalty * energyDrinkModifier;
    }

    // Scrap directly from tools
    // Need to approve Tool and LP first
    function scrapFromTool(uint256 _cupType, uint256[] calldata _upgradeIds) external {
        address owner = msg.sender;
        require(_cupType < currentLevelIndex, "invalid cup type");
        ...
        emit receivedSkillPoints(owner, upgradeLength, levelAfter - levelBefore);

    }

    address public treasuryWallet;

    uint256 public durPenStartTime;

    SodaStationRobotV3 public sodaStationRobot;

    function repairAssistants(uint256[] calldata _assistantIds) external {
        address owner = msg.sender;
        _updateState(owner);
        sodaStationRobot.repairAssistants(owner, _assistantIds, getRepairSkillModifier(owner));
    }

    function unstakeRobots(uint256[] calldata _robotIds) external {
        address owner = msg.sender;
        _updateState(owner);
        sodaStationRobot.unstakeRobots(owner, _robotIds);
    }

    function stakeRobots(uint256[] calldata _robotIds) external {
        address owner = msg.sender;
        _updateState(owner);
        sodaStationRobot.stakeRobots(owner, _robotIds);
    }

    function unstakeAssistants(uint256[] calldata _assistantsIds) external {
        address owner = msg.sender;
        _updateState(owner);
        sodaStationRobot.unstakeAssistants(owner, _assistantsIds, getRepairSkillModifier(owner));
    }

    function stakeAssistants(uint256[] calldata _assistantsIds) external {
        address owner = msg.sender;
        _updateState(owner);
        sodaStationRobot.stakeAssistants(owner, _assistantsIds);
    }
...
contract SodaStationRobotV3 is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable
{
    ...
    // Staking Assistants
    struct StakedAssistantInfo {
        address owner;
        uint256 assistantId;
        uint256 assistantType;
        uint256 assistantUpgradeType;
        uint256 yieldTimeTwo;
        uint256 claims;
    }

    mapping(uint256 => StakedAssistantInfo) public stakedAssistants; // tokenId => owner
    mapping(address => mapping(uint256 => uint256)) public ownedAssistantsStakes; // (address, index) => tokenid
    mapping(uint256 => uint256) public ownedAssistantsStakesIndex; // tokenId => index in its owner's stake list
    mapping(address => uint256) public ownedAssistantStakesBalance; // address => stake count

    // Staking Robot Chefs
    struct StakedRobotChefInfo {
        address owner;
        uint256 robotId;
        uint256 robotType;
    ...
    mapping(uint256 => StakedRobotChefInfo) public stakedChefs; // tokenId => owner
    mapping(address => mapping(uint256 => uint256)) public ownedChefsStakes; // (address, index) => tokenid
    mapping(uint256 => uint256) public ownedChefsStakesIndex; // tokenId => index in its owner's stake list
    mapping(address => uint256) public ownedChefsStakesBalance; // address => stake count

    // Counting for bonus
    mapping(address => uint256[6]) public numberOfStaked; // address => [number of iron chefs, number of ultra iron chefs, number of nuclear cores, number of extra arms, number of energy drink, number of high quality parts]


    // Var
    bool public isOpen; // returns if the factory is open
    PizzaRobot public pizzaRobot;
    address public sodaStationAddress;
    address public treasuryWallet;
    IERC20 public lpToken;

    //Events
    event assistantRepaired(address owner, uint256 assistantId);


    function initialize(PizzaRobot _pizzaRobot, address _sodaStationAddress, address _treasuryWallet, IERC20 _lpToken) public initializer {
        pizzaRobot = _pizzaRobot;
        sodaStationAddress = _sodaStationAddress;
    ...
    function getNuclearCoreBonus(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = numberOfStaked[_owner][2]*10;
        if(totalBonus > 50){
            totalBonus = 50;
        }
    }
    ...
    function getExtraCups(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = 2*numberOfStaked[_owner][3];
    }

    function getProductionBonus(address _owner) public view returns (uint256 totalBonus) {
        totalBonus = 2*numberOfStaked[_owner][5] + 5*numberOfStaked[_owner][0] + 15*numberOfStaked[_owner][1];
    }
    ...
    //Now this function returns SPD times 100
    function getAssistantSPD(address _owner, uint256 _durabilityModifier) public view returns (uint256 totalSPD) {
        uint256 totalDurability = 2 + _durabilityModifier; //2 durability for the robots
        for (uint256 i = 0; i < ownedAssistantStakesBalance[_owner]; i++) {
            StakedAssistantInfo memory assistant = stakedAssistants[ownedAssistantsStakes[_owner][i]];
            if(assistant.claims <  totalDurability){
                totalSPD += 100*assistant.yieldTimeTwo * (totalDurability - assistant.claims) / (2 * totalDurability);
            }
        }
    }

    function claimAndReturnSPDAssistant(address _owner, uint256 _durabilityModifier) public returns (uint256 totalSPD){
        require(_msgSender() == sodaStationAddress, "msgsender does not have permission");
        uint256 totalDurability = 2 + _durabilityModifier; //2 durability for the robots
        for (uint256 i = 0; i < ownedAssistantStakesBalance[_owner]; i++) {
            StakedAssistantInfo storage assistant = stakedAssistants[ownedAssistantsStakes[_owner][i]];
            if(assistant.claims <  totalDurability){
                totalSPD += 100*assistant.yieldTimeTwo * (totalDurability - assistant.claims) / (2 * totalDurability);
                assistant.claims++;
            }
    ...
    function unstakeAssistants(address owner, uint256[] calldata _assistantsIds, uint256 _repairSkillModifier) public {
        require(_msgSender() == sodaStationAddress, "msgsender does not have permission");

        uint256 assistantLength = _assistantsIds.length;
        uint256 needRepair;
        for (uint256 i = 0; i < assistantLength; i++) {
            uint256 assistantsId = _assistantsIds[i];
            uint256 rtype = stakedAssistants[assistantsId].assistantType;
            uint256 utype = stakedAssistants[assistantsId].assistantUpgradeType;

            require(stakedAssistants[assistantsId].owner == owner, "You don't own this token");
            require(rtype == 4, "Invalid robot type");
            ...
            if(stakedAssistants[assistantsId].claims > 0){
                needRepair++;
            }

            _removeAssistantFromSodaStation(assistantsId, owner);

            pizzaRobot.transferFrom(address(this), owner, assistantsId);
        }

        //repair needed
        if(needRepair > 0){
            uint256 repairCost = needRepair * 30 * 1e18 *(100 - _repairSkillModifier) * (100 - getNuclearCoreBonus(owner))/10000;
            lpToken.safeTransferFrom(address(owner), treasuryWallet, repairCost);
        }
    }

    // Need to approve chef and robot first
    function stakeAssistants(address owner, uint256[] calldata _assistantsIds) public {
        require(_msgSender() == sodaStationAddress, "msgsender does not have permission");
        require(isOpen, "The game has not started");

        uint256 assistantLength = _assistantsIds.length;
        require(assistantLength + ownedAssistantStakesBalance[owner]<= 10, "invalid number of assistants");

        for (uint256 i = 0; i < assistantLength; i++) {
            uint256 assistantsId = _assistantsIds[i];
            uint256 rtype = pizzaRobot.getType(assistantsId);
            uint256 utype = pizzaRobot.getUpgradeType(assistantsId);
            require(rtype == 4, "Invalid robot type");
            require(
                pizzaRobot.ownerOf(assistantsId) == owner,
            ...
            if(utype == 0 || utype == 1 || utype == 2 || utype == 3){
                yieldTimeTwo = 1; //will divide by 2 later
            }
            if(utype == 4 || utype == 5 || utype == 6 || utype == 7){
                yieldTimeTwo = 4; //will divide by 2 later
            }

            _addAssistantToSodaStation(assistantsId, owner, rtype, utype, yieldTimeTwo);

            pizzaRobot.transferFrom(owner, address(this), assistantsId);
        }
    }

    function unstakeRobots(address owner, uint256[] calldata _robotIds) public {
        require(_msgSender() == sodaStationAddress, "msgsender does not have permission");

        uint256 robotLength = _robotIds.length;
        for (uint256 i = 0; i < robotLength; i++) {
            uint256 robotId = _robotIds[i];
            uint256 rtype = stakedChefs[robotId].robotType;
            uint256 utype = stakedChefs[robotId].robotUpgradeType;

            require(stakedChefs[robotId].owner == owner, "You don't own this token");
            ...
            _removeRobotFromSodaStation(robotId, owner);
    ...
    // Need to approve chef and robot first
    function stakeRobots(address owner, uint256[] calldata _robotIds) public {
        require(_msgSender() == sodaStationAddress, "msgsender does not have permission");
        require(isOpen, "The game has not started");
        ...
        for (uint256 i = 0; i < robotLength; i++) {
            ...
            _addRobotToSodaStation(robotId, owner, rtype, utype, pizzaRobot.getOriginalChefId(robotId));

            pizzaRobot.transferFrom(owner, address(this), robotId);
        }
    }

    function _addAssistantToSodaStation(uint256 _tokenId, address _owner, uint256 robotType, uint256 upgradeType, uint256 yieldTimeTwo) internal {
        stakedAssistants[_tokenId] = StakedAssistantInfo(_owner,_tokenId, robotType, upgradeType, yieldTimeTwo, 0);
        uint256 length = ownedAssistantStakesBalance[_owner];
        ownedAssistantsStakes[_owner][length] = _tokenId;
        ownedAssistantsStakesIndex[_tokenId] = length;
        ownedAssistantStakesBalance[_owner]++;
    }

    function _removeAssistantFromSodaStation(uint256 _tokenId, address _owner) internal
    {
        delete stakedAssistants[_tokenId];

        uint256 lastTokenIndex = ownedAssistantStakesBalance[_owner] - 1;
        uint256 tokenIndex = ownedAssistantsStakesIndex[_tokenId];

        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = ownedAssistantsStakes[_owner][lastTokenIndex];

            ownedAssistantsStakes[_owner][tokenIndex] = lastTokenId;
            ownedAssistantsStakesIndex[lastTokenId] = tokenIndex;
        }

        delete ownedAssistantsStakesIndex[_tokenId];
        delete ownedAssistantsStakes[_owner][lastTokenIndex];
        ownedAssistantStakesBalance[_owner]--;
    }

    function _addRobotToSodaStation(uint256 _tokenId, address _owner, uint256 robotType, uint256 upgradeType, uint256 originalChefId) internal {
        stakedChefs[_tokenId] = StakedRobotChefInfo(_owner,_tokenId, robotType, upgradeType, originalChefId);
        uint256 length = ownedChefsStakesBalance[_owner];
        ownedChefsStakes[_owner][length] = _tokenId;
        ownedChefsStakesIndex[_tokenId] = length;
        ownedChefsStakesBalance[_owner]++;
    }

    function _removeRobotFromSodaStation(uint256 _tokenId, address _owner) internal
    {
        delete stakedChefs[_tokenId];

        uint256 lastTokenIndex = ownedChefsStakesBalance[_owner] - 1;
        uint256 tokenIndex = ownedChefsStakesIndex[_tokenId];

        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = ownedChefsStakes[_owner][lastTokenIndex];

            ownedChefsStakes[_owner][tokenIndex] = lastTokenId;
            ownedChefsStakesIndex[lastTokenId] = tokenIndex;
        }

        delete ownedChefsStakesIndex[_tokenId];
        delete ownedChefsStakes[_owner][lastTokenIndex];
        ownedChefsStakesBalance[_owner]--;
    }

    function repairAssistants(address owner, uint256[] calldata _assistantIds, uint256 _repairSkillModifier) external {
         require(_msgSender() == sodaStationAddress, "msgsender does not have permission");

        uint256 discount = (100 - _repairSkillModifier) * (100 - getNuclearCoreBonus(owner));
        uint256 repairCost;
        uint256 assistantsLength = _assistantIds.length;
        for (uint256 i = 0; i < assistantsLength; i++) {
            uint256 assistantId = _assistantIds[i];
            StakedAssistantInfo storage assistant = stakedAssistants[assistantId];
            require(assistant.owner == owner, "You don't own this assistant");
            repairCost += 30 * 1e18 *discount/10000;  

            assistant.claims = 0;

            emit assistantRepaired(owner, assistantId);
        }
        lpToken.safeTransferFrom(address(owner), treasuryWallet, repairCost);

    }

    function batchedAssistantsOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (StakedAssistantInfo[] memory) {
        if (_offset >= ownedAssistantStakesBalance[_owner]) {
            return new StakedAssistantInfo[](0);
        }

        uint256 outputSize = _maxSize;
        if (_offset + _maxSize >= ownedAssistantStakesBalance[_owner]) {
            outputSize = ownedAssistantStakesBalance[_owner] - _offset;
        }
        StakedAssistantInfo[] memory outputs = new StakedAssistantInfo[](outputSize);

        for (uint256 i = 0; i < outputSize; i++) {
            outputs[i] = stakedAssistants[ownedAssistantsStakes[_owner][_offset + i]];
        }

        return outputs;
    }

    function batchedRobotChefsOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (StakedRobotChefInfo[] memory) {
        if (_offset >= ownedChefsStakesBalance[_owner]) {
            return new StakedRobotChefInfo[](0);
        }

        uint256 outputSize = _maxSize;
        if (_offset + _maxSize >= ownedChefsStakesBalance[_owner]) {
            outputSize = ownedChefsStakesBalance[_owner] - _offset;
        }
        StakedRobotChefInfo[] memory outputs = new StakedRobotChefInfo[](outputSize);

        for (uint256 i = 0; i < outputSize; i++) {
            outputs[i] = stakedChefs[ownedChefsStakes[_owner][_offset + i]];
        }
