...
import "./IACryptoSVaultFactoryVault.sol";
...
contract ACryptoSVaultFactory is Initializable, UUPSUpgradeable {
    ...
    function getFactoryDeployData(IBeacon beacon)
        external
        view
        returns (
            IACryptoSVaultFactoryVault.FactoryDeployData[]
                memory factoryDeployData
        )
    {
        factoryDeployData = IACryptoSVaultFactoryVault(beacon.implementation())
            .getFactoryDeployData();

        for (uint256 i = 0; i < factoryDeployData.length; i++) {
            factoryDeployData[i].deployed = proxyById[
                abi.encode(beacon, factoryDeployData[i].data)
            ];
        }
    }
...
interface IACryptoSVaultFactoryVault {
    struct FactoryDeployData {
        address deployed;
        bytes data;
        address token;
        string name;
    ...
    function getFactoryDeployData()
        external
        view
        returns (FactoryDeployData[] memory);
}
