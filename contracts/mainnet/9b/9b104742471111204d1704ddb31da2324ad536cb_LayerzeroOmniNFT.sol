...
contract LayerzeroOmniNFT is Ownable, ERC721, NonblockingLzApp {
    uint256 public counter;
    uint256 public currentTokenId;
    uint256 public immutable MAX_ID;
    uint16 public constant DEST_CHAIN_ID = 102; // Replace with your desired destination chain ID
    uint256 public constant START_TOKEN_ID = 100; // Fixed start token ID
    ...
    constructor(address _endpoint) ERC721('LayerzeroOmniNFT', 'LZONFT') NonblockingLzApp(_endpoint) {
        currentTokenId = START_TOKEN_ID;
    ...
    function estimateFees(uint256 tokenId) external view returns (uint256) {
        bytes memory payload = abi.encode(msg.sender, tokenId);
        uint16 version = 1;
        uint256 gasForLzReceive = 350000;
        bytes memory adapterParams = abi.encodePacked(version, gasForLzReceive);
