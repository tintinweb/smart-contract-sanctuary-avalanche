...
contract DoubleRewarder is OwnableUpgradeable {
    ...
    function setSupported(address _rewardToken) external onlyOwner {
        supportedRewardToken[_rewardToken] = true;
    }

    function setLastRewardTimestamp(address _rewardToken, uint256 _timestamp)
        external
        onlyOwner
    {
        pools[_rewardToken].lastRewardTimestamp = _timestamp;
    }
    ...
    function updatePool(address _rewardToken, uint256 _lpSupply) internal {
        ...
        if (pool.lastRewardTimestamp == 0)
            if (pool.rewardPerSecond > 0) {
                if (
                    block.timestamp > pool.lastRewardTimestamp && _lpSupply > 0
                ) {
                    uint256 timeElapsed = block.timestamp -
                        pool.lastRewardTimestamp;
