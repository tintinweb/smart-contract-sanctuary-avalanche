...
contract APRHelper is Initializable, OwnableUpgradeable {
    ...
    function getJoeLPPrice(address lp) public view returns (uint256 inUSD) {
        ...
        token1Price = token1Price == 0
            ? token1 > token0
                ? token0Price * getRatio(token1, token0, 8)
                : token0Price / getRatio(token1, token0, 8)
            : token1Price;
        token0Price = token0Price == 0
            ? token0 > token1
                ? token1Price * getRatio(token0, token1, 8)
                : token1Price / getRatio(token0, token1, 8)
            : token0Price;
        uint256 totalSupply = IJoePair(lp).totalSupply();
        uint256 token0Amount = IERC20(token0).balanceOf(lp);
        uint256 token1Amount = IERC20(token1).balanceOf(lp);
        inUSD = ((token0Amount * token0Price) + (token1Amount * token1Price)) / totalSupply;
    }
