...
contract CampfireCollectorsClub is ERC721URIStorage, Ownable, KindaRandom {
  ...
  address private _userBaseAddress;

  address private _registry;
  mapping (address => bool) private _locallyBlocked;
  mapping (address => bool) private _explicitlyAllowed;

  constructor() ERC721("Campfire Collector's Club", "CCC") KindaRandom(500) {
    _customBaseURI = "https://nfts.bpi.network/ccc/";
    _userBaseAddress = 0x5adCD28C08Fdc5a913982391cebD866b27C717D4;
    _maxMintable = 500; // 500 max supply
    _royaltyAmount = 1000; // 10% royalties
    _mintActive = false; // start paused
    _maxMintPerTx = 5;
    _price = 5 ether;
    _startTime = 1684544400;
    
    _registry = 0x4058af9B14884B75ac685Db4b6633ca83e3B9D73;

    _artist =  0x9FfA64955eBbecA59e68401afA848a81669F2d0b;
    _earlyAccessDuration = 1800; // 30 minutes
  ...
  function secondsUntilPublicMint() public view returns (uint256) {
    if (onlyEarlyAccess()) {
  ...
  function hasEarlyAccess(address account) public view returns (bool) {
    UserBase userBase = UserBase(_userBaseAddress);
    return keccak256(abi.encodePacked((userBase.usernameFor(account)))) != keccak256(abi.encodePacked(("")));
  }
  ...
  function mint(uint256 quantity) public payable {
    ...
    if (onlyEarlyAccess()) {
      require(hasEarlyAccess(msg.sender), "Must have username set.");
    }
