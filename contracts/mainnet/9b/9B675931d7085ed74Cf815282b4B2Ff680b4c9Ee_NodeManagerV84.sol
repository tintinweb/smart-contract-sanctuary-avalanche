...
interface ICreditNFT {
    function useCredits(address, uint32, uint32) external;
}

contract NodeManagerV84 is Initializable, KeeperCompatibleInterface {
    ...
    function claimable(address _account) external view returns (uint256) {
        ...
        return claimableAmount + oldRewardsOfUser[_account];
    }
    ...
    function transferUnclaimed(address _account, address _to, uint256 _amount) external onlyBuyer {
        _claim(_account);
        uint32 claimFeeRate = dynamicClaimFeeEnabled ? rateClaimFee(msg.sender) * compTaxRate / 100 : claimFee * compTaxRate / 100;
    ...
    function claim() public {
        ...
        uint32 claimFeeRate = dynamicClaimFeeEnabled ? rateClaimFee(msg.sender) : claimFee;
        if (claimFeeRate > 0) {
    ...
    */
    ...
    function payWithNFT(uint8 count, uint256[] memory selected, bool useNFT) public payable {
        require(
        ...
        if (selected.length == 0) {
            if(useNFT) creditNFT.useCredits(msg.sender, uint32(countOfUser[msg.sender]), uint32(count));
            uint256[] storage nodeIndice = nodesOfUser[msg.sender];
            for (uint32 i = 0; i < nodeIndice.length; i++) {
                uint256 nodeIndex = nodeIndice[i];
                if (nodeIndex > 0) {
            ...
            if(useNFT) creditNFT.useCredits(msg.sender, uint32(selected.length), uint32(count));
