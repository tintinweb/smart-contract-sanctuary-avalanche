...
contract StrategyTraderJoeDualLP is CompoundFeeManager, CompoundStrat {
    ...
    function withdraw(uint256 _amount) external override {
        ...
        if (tx.origin != owner() && !paused()) {
            uint256 withdrawalFeeAmount = wantBal.mul(withdrawalFee).div(MAX_WITHDRAWAL_FEE);
...
abstract contract CompoundFeeManager is CompoundStratManager {
    ...
    // Max value for fees
    uint256 public constant WITHDRAWAL_FEE_CAP = 250; // 2.5%
    ...
    // Initial fee values
    uint256 public withdrawalFee = 150; // 1.5%
