...
contract Metaverse is ERC721 {
    ...
    constructor()
        ERC721("Metaverse Ape Yacht Avax Club (Avalanche)", "Metaverse")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 3200) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeidryg3bpbz3ihf37qockfyveoxsszlqewnnsh3ybrivj3dfq3y7ua/", Strings.toString(tokenId), ".json")
            );
    }
