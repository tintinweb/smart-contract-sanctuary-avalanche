...
contract nebulaFeeManager is Ownable{
    ...
    constructor(address[] memory addresses, address payable _treasury, uint256[] memory _fees){
       for(uint i = 0;i<addresses.length;i++){
    	...
    	maxPayPeriods = _fees[4];
    	maxFeePayment = feePeriod.mul(maxPayPeriods);
    	Managers.push(owner());
	for(uint i = 1;i<3;i++){
		Managers.push(addresses[i]);
	}
	uint length = getPrevAccountsLength();
	for(uint i = 0;i<length;i++){
		address oldAct = getPrevActAddress(i);
		AccountsOld.push(oldAct);
		protoLengthOld.push(getPrevProtoLength(oldAct));
		Accounts.push(oldAct);
    ...
    function transferAllBatch(uint start) external onlyOwner(){
    	uint length = getPrevAccountsLength();
    	transferAll(start,length);
    }
    function transferAllSpec(uint _start,uint _intervals) external onlyOwner(){
    	transferAll(_start,_intervals);
    }
    
function transferAll(uint _start,uint _intervals) internal{
	for(uint j=_start;j<_intervals;j++){
		address oldAct = AccountsOld[j];
		uint256 length = protoLengthOld[j];
		if(nebuLib.addressInList(transfered,oldAct) == false){
		uint256 ii = Zero;
			for(uint i=0;i<length;i++){
				
				(string memory name,uint256 collapseDate,uint256 nextDue,uint256 futureFees,uint256 feeFroze,bool owed,bool full,bool insolvent,bool imploded) = getPrevprotoOwners(oldAct,ii);	
				createProto(oldAct,name);
				PROTOOWNERS[] storage protos = protoOwners[oldAct];
			    	PROTOOWNERS storage proto = protos[ii];
				    	proto.name = name;
				    	proto.collapseDate = collapseDate;
				    	proto.nextDue = nextDue;
				    	proto.futureFees = futureFees;
				    	proto.feeFroze = feeFroze;
			    		proto.owed = owed;
			    		proto.full = full;
			    		proto.insolvent = insolvent;
			    		proto.imploded = imploded;

			    	transfered.push(oldAct);
				ii++;
			}
			TOTALFEES storage acctTot = totalFees[oldAct];
			acctTot.feesPaid +=length;
		}
	}
    }

    function plus() external onlyOwner(){
    		for(uint j=0;j<transfered.length;j++){
	    		recPayFees(transfered[j]);
	    		TOTALFEES storage acctTot = totalFees[transfered[j]];
			TOTALFEES storage acctTots = totalFees[address(this)];
			acctTots.totalPayable += acctTot.totalPayable;
			acctTots.feesOwed += acctTot.feesOwed;
			acctTots.futureFees += acctTot.futureFees;
			acctTots.feesPaid += acctTot.feesPaid; 
		}
    }
    ...
    function payFeeToken(uint256 _intervals) payable external {
        ...
        queryFees(_account);
        TOTALFEES storage acctTot = totalFees[_account];
        uint256 sent = feeTok.balanceOf(_account);
        uint256 fee = 15*(10**6);
    	...
    	for(uint i = 0;i<_intervals;i++) {
    		if(_intervals > 0 && sent >= fee){
    			feeTok.approve(_account, fee);
    			feeTok.transfer(treasury, fee);
			recPayFees(_account);
    ...
    function queryFees(address _account) internal {
    	if(nebuLib.addressInList(Accounts,_account) == true){
	    	TOTALFEES storage acctTot = totalFees[_account];
	    	TOTALFEES storage acctTots = totalFees[address(this)];
	    	acctTots.protos -= acctTot.protos;
	    	acctTots.feesOwed -= acctTot.feesOwed;
	    	acctTots.futureFees -= acctTot.futureFees;
	    	acctTots.totalPayable -= acctTot.totalPayable;
		acctTot.protos = Zero;
   ...
   function totalReturn(address _account) internal returns (uint256 ,uint256 ,uint256 ,uint256 ,uint256 ,uint256){
   	queryFees(_account);
    	TOTALFEES storage accTot = totalFees[_account];
    	return (accTot.totalPayable,accTot.protos,accTot.feesOwed,accTot.futureFees,accTot.feesPaid,accTot.collapsed);
    }
  function zeroMgrTotal() internal {
  	TOTALFEES storage acctTot = totalFees[address(this)];
  	acctTot.totalPayable = Zero;
    	acctTot.protos = Zero;
	acctTot.feesOwed = Zero;
	acctTot.futureFees = Zero;
	acctTot.feesPaid = Zero; 
	acctTot.collapsed = Zero;
  }
  function updatemgrTots(uint256 _totalPayable,uint256 _protos,uint256 _feesOwed,uint256 _futureFees,uint256 _feesPaid,uint256 _collapsed) internal{
  	
  }
  function mgrTotQuery() internal {
  	zeroMgrTotal();
  	TOTALFEES storage acctTot = totalFees[address(this)];
  	for(i=0;i<Accounts.length;i++){
  		(uint256 _totalPayable,uint256 _protos,uint256 _feesOwed,uint256 _futureFees,uint256 _feesPaid,uint256 _collapsed) =totalReturn(Accounts[i]);
	  	acctTot.totalPayable += _totalPayable;
		acctTot.protos += _protos;
		acctTot.feesOwed += _feesOwed;
		acctTot.futureFees += _futureFees;
		acctTot.feesPaid += _feesPaid; 
		acctTot.collapsed += _collapsed;	
  	}
   }
   function mgrTotalQuery() external  managerOnly(){
    	mgrTotQuery();
    }
