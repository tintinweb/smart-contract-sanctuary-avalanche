...
contract BEARS is ERC721 {
    ...
    constructor()
        ERC721("CC0 BEARS", "BEARS")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 139) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiddx5mkcyo4p6nwhua2b7mlkfld4bts5elbzc7sgqdx5sdt5albxe/", Strings.toString(tokenId), ".json")
            );
    }
