...
contract MansionManager is Ownable, HelperOwnable, IERC721, IERC721Metadata, ITestManager, FoundersNFT {
    ...
    IERC721 FOUNDER = IERC721(0x000000000000000000000000000000000000dEaD);
	...
	uint userMultiplier = getUserMultiplier(account);
        if(rewardNode > 0 && userMultiplier > 0 ){
            rewardNode = rewardNode * userMultiplier;
    ...
    function getUserMultiplier(address from) public view returns (uint256) {
         return (getFoundersMultiplier(from).add(getDistrictMultiplier(from)).add(getCityMultiplier(from))).div(1000);
    }
