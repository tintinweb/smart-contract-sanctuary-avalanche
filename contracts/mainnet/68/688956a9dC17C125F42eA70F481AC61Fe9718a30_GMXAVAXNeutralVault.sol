...
interface IGMXAVAXNeutralReader {
...
import "../../../../interfaces/vaults/gmx/v2/avax/IGMXAVAXNeutralReader.sol";
import "../../../../interfaces/oracles/IChainlinkOracle.sol";
import "../../../../interfaces/tokens/IStakedGLP.sol";
import "../../../../enum/ManagerAction.sol";

contract GMXAVAXNeutralVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  // Manager contract
  IGMXAVAXNeutralManager public manager;
  // Reader contract
  IGMXAVAXNeutralReader public reader;
  // Vault config struct
  ...
  */
  ...
  function _deposit(
    ...
    uint256 borrowWAVAXValue = totalValue
      * reader.currentTokenWeight(WAVAX)
      / SAFE_MULTIPLIER;
    ...
    uint256 borrowBTCbValue = totalValue
      * (reader.currentTokenWeight(BTCb) + reader.currentTokenWeight(WBTC)) // GMX on ARB has BTCb and WBTC
      / SAFE_MULTIPLIER;

    uint256 borrowUSDCValue = totalValue - _depositValue - borrowWAVAXValue - borrowWETHValue - borrowBTCbValue;

    IGMXAVAXNeutralManager.WorkData memory data = IGMXAVAXNeutralManager.WorkData(
      {
        token: _token,
        lpAmt: 0,
        borrowWAVAXAmt: borrowWAVAXValue * SAFE_MULTIPLIER / reader.tokenValue(WAVAX, 1e18),
        borrowWETHAmt: borrowWETHValue * SAFE_MULTIPLIER / reader.tokenValue(WETH, 1e18),
        borrowBTCbAmt: borrowBTCbValue * BTCb_DECIMALS / reader.tokenValue(BTCb, BTCb_DECIMALS),
  ...
  */
  function _burnAndWork(address _token, uint256 _shareAmt) internal {
    ...
    (uint256 debtWAVAX, uint256 debtWETH, uint256 debtBTCb, uint256 debtUSDC) = manager.debtAmts();
    ...
    IGMXAVAXNeutralManager.WorkData memory data = IGMXAVAXNeutralManager.WorkData(
      {
        ...
        borrowBTCbAmt: 0,
        borrowUSDCAmt: 0,
        repayWAVAXAmt: shareRatio * debtWAVAX / SAFE_MULTIPLIER,
  ...
  */
  function updateAddress(uint256 _type, address _addr) external onlyOwner {
    if (_type == 1) {
      manager = IGMXAVAXNeutralManager(_addr);
    }

    if (_type == 2) {
      reader = IGMXAVAXNeutralReader(_addr);
    }
