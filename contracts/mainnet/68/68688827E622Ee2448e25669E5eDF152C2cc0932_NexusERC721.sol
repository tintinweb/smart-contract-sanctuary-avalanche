    ...
    Pausable
{
    ...
    bool public useAuthTransfers = true;
    ...
    event TransferAuthUpdated(address account, bool value);
        ...
        ERC721("Nexus Ecosystem", "NEXUS")
    {
        ...
        canTransfer[developer] = true;
        canTransfer[treasury] = true;
    ...
    function setUseAuthTransfers(bool value) external onlyOwner {
        useAuthTransfers = value;
    }

    function setCanTransfer(address account, bool value) external onlyOwner {
        canTransfer[account] = value;
        emit TransferAuthUpdated(account, value);
    }
    ...
    function removeLastTier() external whenPaused onlyOwner {
        require(
            tiers[tiers.length - 1].count == 0,
            "NexusERC721: tier count != 0"
        );
        ...
        whenValidName(name)
    {
        require(
            ownerOf(tokenId) == _msgSender() ||
                vault.getStakerOf(tokenId) == _msgSender(),
            "NexusERC721: owner and caller differ"
        );
    ...
    function airdrop(
        ...
        require(
            accounts.length == _tiers.length,
            "NexusERC721: array length mismatch"
        );

        for (uint256 i = 0; i < accounts.length; i++) {
            _mintInternal(accounts[i], _tiers[i], name);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && useAuthTransfers) {
            require(
                canTransfer[_msgSender()],
                "NexusERC721: not authorized to transfer"
            );
        }
