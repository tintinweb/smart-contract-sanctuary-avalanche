...
contract TraderJoeYieldFarmVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  ...
  */
  function compound() public {
    ITraderJoeYieldFarmManager(manager).compound();
  ...
  */
  function _deposit(uint256 _amt, uint256 _depositValue) internal returns (uint256) {
    ...
    ITraderJoeYieldFarmManager(manager).work(
  ...
  */
  function _burnAndWork(uint256 _shareAmt) internal {
    ...
    // do calculations
    (uint256 tokenADebtAmt, uint256 tokenBDebtAmt) = ITraderJoeYieldFarmManager(manager).debtInfo();
    uint256 lpAmt = shareRatio * ITraderJoeYieldFarmManager(manager).lpTokenAmt() / SAFE_MULTIPLIER;
  ...
  */
  function emergencyShutDown() external onlyOwner whenNotPaused {
    ...
    uint256 lpAmt = ITraderJoeYieldFarmManager(manager).lpTokenAmt();
    (uint256 tokenADebtAmt, uint256 tokenBDebtAmt) = ITraderJoeYieldFarmManager(manager).debtInfo();
