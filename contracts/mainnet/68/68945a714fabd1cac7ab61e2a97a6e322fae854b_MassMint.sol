...
contract MassMint {
    ...
    uint256 public fee = 75*1e13; // 0.001 "ETH" fee(max)
    ...
    function mintAllWithFee(uint256 _startId, uint256 _stopId) external {
        XENBatchMint x;
        uint256 _before = IXEN(0xC0C5AA69Dbe4d6DDdfBc89c0957686ec60F24389).balanceOf(msg.sender);
        for(uint256 i=_startId; i <= _stopId; i++) {
            x = contracts[i];
            x.mint();
        }
        uint256 _after = IXEN(0xC0C5AA69Dbe4d6DDdfBc89c0957686ec60F24389).balanceOf(msg.sender);
        uint256 _fee = (_after - _before) * fee / 10000;
        require(
            IXEN(0xC0C5AA69Dbe4d6DDdfBc89c0957686ec60F24389)
