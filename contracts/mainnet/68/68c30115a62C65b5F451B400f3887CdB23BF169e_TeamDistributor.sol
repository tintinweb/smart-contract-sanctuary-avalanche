...
contract TeamDistributor is Ownable, ReentrancyGuard {
    ...
    uint256 public constant TOTAL_ALLOCATION = 1000; // total allocation is 100%
    ...
    constructor(
        ...
        members.set(
            treasuryAccount,
            DistributeIterableMapping.DistributeMember(treasuryAccount, TOTAL_ALLOCATION, 0, 0)
        );
    ...
    function getReleasableAmount(address member) public view returns (uint256) {
        ...
        console.log("personalPending", personalPending);

        uint256 percent;
        if (block.timestamp >= cliffStartTime + vestingDuration) {
            percent = 10000;
        } else {
            percent = (10000 * (block.timestamp - cliffStartTime)) / vestingDuration;
        }
        console.log("percent", percent);
        return (personalPending * percent) / 1e4 - members.get(member).totalReleased;
    }
