...
import "../lend/LendingManager.sol";
import "../common/interface/IDex.sol";

contract Core is Ownable, ERC20 {

    // TODO: we need to get this addresses from the constructor
    ...
    address[] public path = [USDC, WAVAX, WBTC];
    ...
    constructor(address _lendingManager, address _yieldManager, IDex _dex, string memory _name, string memory _symbol)
    ERC20(_name, _symbol) {
        ...
        IERC20(USDC).approve(address(dex), type(uint).max);
    }

    function deposit(uint _amount) external onlyTeam {
    ...
    function withdraw(uint _shares) external onlyTeam {
        ...
        lendingManager.repay(USDC, IERC20(USDC).balanceOf(address(this)));
    ...
    function enter() external onlyTeam {
        // TODO: make sure we are not sending the Reserve BTC to the LendingManager
        lendingManager.supplyMax();
        uint256 borrowedAmount = _leverage();
        emit Enter(borrowedAmount);
    }

    function rebalance() external {
        // Check if need to leverage or deleverage
    ...
    function _leverage() internal returns (uint256) {
        uint256 borrowedAmount = lendingManager.borrowMax(USDC);
        yieldManager.deposit(borrowedAmount);
        return borrowedAmount;
    }

    function _deleverage() internal {
        uint amountToDelevage = lendingManager.getDeleverageAmount(USDC);
        yieldManager.withdrawAmount(address(this), amountToDelevage);
        lendingManager.deleverage(USDC);
    }

    function harvestAndRepayDebtInterest() external {
        // Harvest
        lendingManager.harvest();
        // TODO: would be nice if withdrawYield can return the amount too
        yieldManager.withdrawYield(address(this));
        uint256 profit = IERC20(USDC).balanceOf(address(this));
        // Repay borrow interest
        uint256 debtCost = lendingManager.getBorrowInterestAmount(USDC);
        require(debtCost < profit, "PANIC: cannot repay debt");
        lendingManager.repayBorrowInterest(USDC);

        // Swap net profit for BTC
        dex.swapTokens(profit - debtCost, path, address(this));
        lendingManager.supplyMax();
    }
    ...
    function _overallBtcBalance() private returns (uint256) {
        ///TODO : Local balance + Lending balance + Reserve balance ? Assuming 1 BTC = 1 aBTC
...
interface IYieldManager {
    ...
    function withdrawYield(address receiver) external;
    ...
    /**
     * Get data about yield generated in the the module. Earnings are in baseToken (USDC, DAI, ..)
     */
    function getYieldInfo() external returns(uint256);
    function initCore(address core) external;


    // TODO: getApy, panic(), activate()
