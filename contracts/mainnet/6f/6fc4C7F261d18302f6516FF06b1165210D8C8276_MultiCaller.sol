...
contract MultiCaller is Initializable, OwnableUpgradeable {
    ...
    function previewMultiHarvest(
        ...
        for (uint256 i; i < length; ++i) {
            if (harvestedContracts[i] == address(bribeManager)) {
                ...
                if (
                    ...
                    amounts[0] += ICompounderJoe(harvestedContracts[i]).previewAvaxAmountForHarvest() *  ICompounderJoe(harvestedContracts[i]).callerFee()/10000;
                }
            }
        }
        ...
        for (uint256 j; j < lengthCore; ++j) {
            ...
            rewardTokens[1] = VTX;
            amounts[0] += avaxAmount * ILVTXAutoCompound(harvestedCoreContracts[j]).callerFee()/10000;
            amounts[1] += vtxAmount * ILVTXAutoCompound(harvestedCoreContracts[j]).callerFee()/10000;
        }
    }
}
