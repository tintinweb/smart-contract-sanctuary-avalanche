...
contract xCircle is IERC20, Ownable {
    ...
    // token data
    string private constant _name = 'xCircle';
    string private constant _symbol = 'XCIRCLE';
    ...
    // Whether Or Not xCircle Mint Rate Reduces As Supply Is Burned
    ...
    function mint(uint256 amountCircle) external {
        ...
        // take circle to mint
        uint256 received = _takeCircle(amountCircle);
        ...
        // increment total circle
        unchecked { 
            totalCircle += received;
        }
    ...
    function redeemRate() public view returns (uint256) {
        if (totalMinted == 0 || totalCircle == 0 || _totalSupply == 0) {
            return 10**18;
        }
        ...
        // totalCircleReceived / averaged minted
        return ( totalCircle * 10**18 ) / denom;
    }
    ...
    function _takeCircle(uint256 amount) internal returns (uint256) {
        // ensure allowance is preserved
        require(
            IERC20(circle).allowance(msg.sender, address(this)) >= amount,
            'Insufficient Allowance'
        );

        // transfer in Circle
        uint before = IERC20(circle).balanceOf(feeRecipient);
        IERC20(circle).transferFrom(msg.sender, feeRecipient, amount);
        uint After = IERC20(circle).balanceOf(feeRecipient);
        require(
            After > before,
            'Zero Received'
        );
