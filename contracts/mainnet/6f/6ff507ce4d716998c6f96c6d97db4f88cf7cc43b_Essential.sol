...
contract Essential is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 12000 ether;
    // Initial distribution for the day 2-5 RAINY-WFTM LP -> RAINY pool
    uint256 public constant INITIAL_RAINY_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the RAINY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ESSENTIAL", "ESSENTIAL") {
        // Mints 1 ESSENTIAL to contract creator for initial pool setup
    ...
    function _getRainyPrice() internal view returns (uint256 _rainyPrice) {
        try IOracle(rainyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rainy: failed to fetch RAINY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rainyPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rainyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRainyOracle(address _rainyOracle) public onlyOperatorOrTaxOffice {
        require(_rainyOracle != address(0), "oracle address cannot be 0 address");
        rainyOracle = _rainyOracle;
    }
    ...
    /**
     * @notice Operator mints RAINY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RAINY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRainyPrice = _getRainyPrice();
            currentTaxRate = _updateTaxRate(currentRainyPrice);
            if (currentRainyPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_rainyPool != address(0), "!_rainyPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_rainyPool, INITIAL_RAINY_POOL_DISTRIBUTION);
