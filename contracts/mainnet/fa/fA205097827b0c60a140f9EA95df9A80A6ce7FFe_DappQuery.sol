...
contract DappQuery is Ownable {
    ...
    struct TempStore {
        ...
        uint256 len;
        strings.slice delim;
        strings.slice nodeNames;
        strings.slice creationTimes;
        strings.slice lastClaimTimes;
    ...
    function getNodes(address account) public view returns (Node[] memory) {
        ...
        for (uint256 i = 0; i < allTiers.length; i++) {
            ts.tier = allTiers[i];
            NODERewardManagement manager = managers[ts.tier];

            if (manager._getNodeNumberOf(account) == 0) {
                continue;
            }

            ts.nodeNames = manager._getNodesNames(account).toSlice();
            ts.creationTimes = manager._getNodesCreationTime(account).toSlice();
            ts.lastClaimTimes = manager
                ._getNodesLastClaimTime(account)
                .toSlice();
            ts.nodesIndexes = manager._getNodesIndex(account);

            uint256 len = ts.nodeNames.count(ts.delim) + 1;
            for (uint256 a = 0; a < len; a++) {
                ...
                uint256 taxedRewards = manager.getRewardAmountOf(
                    ts.nodesIndexes[a]
                );
                string memory nodeId = manager.getNodeId(ts.nodesIndexes[a]);

                nodes[ts.globalIdx++] = Node({
                    tier: ts.tier,
                    nodeId: nodeId,
                    name: ts.nodeNames.split(ts.delim).toString(),
                    idx: ts.nodesIndexes[a],
                    creationTime: stringToUint(
                        ts.creationTimes.split(ts.delim).toString()
                    ),
                    lastClaimTime: stringToUint(
                        ts.lastClaimTimes.split(ts.delim).toString()
                    ),
