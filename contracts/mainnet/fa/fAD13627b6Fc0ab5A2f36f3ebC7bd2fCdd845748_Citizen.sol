...
contract Citizen is ERC721, Ownable {
    ...
    function unfrozeCitizen(address  _to, uint _id) onlyACM payable external returns (uint){
        citizens.push(Citizens(_id, 0, 100, 100, 100, block.timestamp + needsFulfillTimer, false, false, false, false, true));
        _safeMint(_to, _id);
        _triggerTimer(_id);
        return _id;
    }
    ...
    function fulfillNeeds(uint _id) onlyCitizenOwner(_id) public {
        require(citizens[_id].needsFulfillTime <= block.timestamp, "Time!");
        IAnyElectricityToken(electricity).burn(ownerOf(_id), 576 * 10 ** 18);
        IAnyFoodToken(food).burn(ownerOf(_id), 7752 * 10 ** 18);
        IAnyWaterToken(water).burn(ownerOf(_id), 12 * 10 ** 18);
        _triggerTimer(_id);
        citizens[_id].ss += 10;
        citizens[_id].ms += 9;
        citizens[_id].cs += 8;
    ...
    function stopProduceElectricity(uint _id) onlyCitizenOwner(_id) shiftTimer(_id) public {
        require(citizens[_id].ef == true, "Not electricity!");
    ...
    function electricityAllowance(uint _productiveTime) internal pure returns(uint) {
        ...
        if(_productiveTime % 1000 <= 576){return 576 * 10 ** 18;} else{return allowance * 10 ** 18;}
    }
    ...
    function stopProduceFood(uint _id) onlyCitizenOwner(_id) shiftTimer(_id) public {
        require(citizens[_id].ff == true, "Not food!");
        uint productiveTime = block.timestamp - citizens[_id].startTime;
        citizens[_id].ff = false;
        uint daysSpend = productiveTime / 86400;
        citizens[_id].ss += 5 * daysSpend;
    ...
    function foodAllowance(uint _productiveTime) internal pure returns(uint) {
        ...
        if(_productiveTime % 10000 <= 7752){return 7752 * 10 ** 18;} else{return allowance * 10 ** 18;}
    }
    ...
    function stopProduceWater(uint _id) onlyCitizenOwner(_id) shiftTimer(_id) public {
        require(citizens[_id].wf == true, "Not water!");
        uint productiveTime = block.timestamp - citizens[_id].startTime;
    ...
    function waterAllowance(uint _productiveTime) internal pure returns(uint) {
        ...
        if(_productiveTime % 100 <= 12){return 12 * 10 ** 18;} else{return allowance * 10 ** 18;}
    }
    ...
    function stopProduceOre(uint _id) onlyCitizenOwner(_id) shiftTimer(_id) public {
        require(citizens[_id].mf == true, "Not Ore!");
        uint productiveTime = block.timestamp - citizens[_id].startTime;
        uint allowance = (productiveTime % 100) * 10 ** 18;
        citizens[_id].mf = false;
        uint daysSpend = productiveTime / 86400;
        citizens[_id].ss += 4 * daysSpend;
        citizens[_id].ms += 7 * daysSpend;
        citizens[_id].cs += 7 * daysSpend;
        IAnyOreToken(ore).mint(ownerOf(_id), allowance);
    }

    function RD(uint _id) onlyRD external {
        citizens[_id].ss += 8;
        citizens[_id].ms += 8;
        citizens[_id].cs += 10;
    }
    ...
    function setAddresses(address _acm, address _cluster, address _electricity, address _food, address _water, address _ore, address _rd) onlyOwner public returns(bool) {
        acm = _acm;
        cluster = _cluster;
    ...
    modifier shiftTimer(uint _id) {
        require(citizens[_id].startTime + 86400 <= block.timestamp, "Time!");
        _;
    }
    modifier isProducing(uint _id) {
        require( citizens[_id].ef == false && citizens[_id].ff == false && citizens[_id].wf == false && citizens[_id].mf == false, "Already Producing!");
        _;
    }
}
