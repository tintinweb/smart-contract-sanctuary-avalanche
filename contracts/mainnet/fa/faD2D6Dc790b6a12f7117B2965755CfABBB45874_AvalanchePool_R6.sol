...
import "../interfaces/IBondAVAX_R1.sol";
import "../interfaces/IGlobalPool_R2.sol";

contract AvalanchePool_R6 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool_R2 {
    ...
    event ManualClaimExpected(address indexed claimer, uint256 amount, uint256 indexed id);
    ...
    uint256 private _stashedForManualClaim;
    mapping(uint256 => address) private _manualClaimers;
    mapping(uint256 => uint256) private _manualStashed;
    ...
    function stake() override external payable {
        _stake(true);
    }

    function stakeAndClaimBonds() override external payable {
        _stake(true);
    }

    function stakeAndClaimCerts() override external payable {
        _stake(false);
        IBondAVAX_R1(_fAvaxContract).unlockSharesFor(msg.sender, msg.value);
    }

    function _stake(bool isRebasing) internal nonReentrant {
        ...
        IBondAVAX_R1(_fAvaxContract).mintBonds(msg.sender, msg.value);
        /* emit events */
        emit StakePending(msg.sender, msg.value);
        emit StakePendingV2(msg.sender, msg.value, isRebasing);
    }
    ...
    function changeFutureAvaxContract(address futureAvaxContract) public onlyOwner {
        ...
        emit BondContractChanged(futureAvaxContract);
    }

    function changeCertAvaxContract(address certToken) public onlyOwner {
    ...
    function getPendingClaims() public onlyOperator view returns (address[] memory, uint256[] memory) {
        address[] memory addresses = new address[](_pendingClaimers.length - _pendingAvaxClaimGap);
        uint256[] memory amounts = new uint256[](_pendingClaimers.length - _pendingAvaxClaimGap);
        uint256 j = 0;
        for (uint256 i = _pendingAvaxClaimGap; i < _pendingClaimers.length; i++) {
    ...
    function claim(uint256 amount) override external nonReentrant {
        _claim(amount, true, msg.sender);
    }

    function claimBonds(uint256 amount) override external nonReentrant {
        _claim(amount, true, msg.sender);
    }

    function claimCerts(uint256 amount) override external nonReentrant {
        uint256 bonds = IBondAVAX_R1(_fAvaxContract).sharesToBalance(amount);
        IBondAVAX_R1(_fAvaxContract).lockSharesFor(msg.sender, amount);
        _claim(bonds, false, msg.sender);
    }

    function claimCertsFor(address recipient, uint256 amount) override public nonReentrant {
        uint256 bonds = IBondAVAX_R1(_fAvaxContract).sharesToBalance(amount);
        IBondAVAX_R1(_fAvaxContract).lockSharesFor(msg.sender, amount);
        _claim(bonds, false, recipient);
    }

    function _claim(uint256 amount, bool isRebasing, address recipient) internal {
        require(amount > 0, "AvalanchePool: cannot claim zero");
        require(IERC20Upgradeable(_fAvaxContract).balanceOf(msg.sender) >= amount, "Cannot claim more than have on address");
        if (_pendingUserClaims[recipient] == 0) {
            _pendingClaimers.push(recipient);
        }
        _pendingUserClaims[recipient] = _pendingUserClaims[recipient].add(amount);
        IBondAVAX_R1(_fAvaxContract).burnBondsFor(recipient, msg.sender, amount);
        emit AvaxClaimPending(recipient, amount);
        emit AvaxClaimPendingV2(recipient, amount, isRebasing);
    }
    ...
    function serveClaims(address payable residueAddress, uint256 minThreshold) public onlyOperator payable {
        ...
        for (i = _pendingAvaxClaimGap; i < _pendingClaimers.length; i++) {
            ...
            if (!result) {
                _stashedForManualClaim = _stashedForManualClaim.add(amount);
                _manualClaimers[i] = claimer;
                _manualStashed[i] = amount;
                emit ManualClaimExpected(claimer, amount, i);
            } else {
                claimers[j] = claimer;
                amounts[j] = amount;
                j++;
                IBondAVAX_R1(_fAvaxContract).confirmBurn(claimer, amount);
            }
        ...
        uint256 missing = 0;
        for (i = _pendingAvaxClaimGap; i < _pendingClaimers.length; i++) {
            missing = missing.add(_pendingUserClaims[_pendingClaimers[i]]);
        }
    ...
    function manualServeClaim(uint256 id) public nonReentrant {
        uint256 amount = _manualStashed[id];
        address claimer = _manualClaimers[id];

        require(amount > 0, "AvalanchePool: claim amount should be not zero");
        require(claimer != address(0), "AvalanchePool: not available for serve");
        require(address(this).balance >= _stashedForManualClaim, "AvalanchePool: insufficient pool balance");
        ...
        address payable wallet = payable(address(claimer));
        bool result = wallet.send(amount);
        require(result, "AvalanchePool: failed to serve claim to claimer");


        claimers[0] = claimer;
        amounts[0] = amount;
        _stashedForManualClaim = _stashedForManualClaim.sub(amount);

        IBondAVAX_R1(_fAvaxContract).confirmBurn(claimer, amount);

        _manualStashed[id] = 0;
        delete _manualClaimers[id];

        emit ClaimsServed(claimers, amounts, 0);
    }
    ...
    function migrateBurnDelayed() external onlyOwner {
        for (uint256 i = _pendingAvaxClaimGap; i < _pendingClaimers.length; i++) {
            address claimer = _pendingClaimers[i];
            uint256 amount = _pendingUserClaims[claimer];
            if (claimer == address(0) || amount == 0) {
            ...
            IBondAVAX_R1(_fAvaxContract).migrateBurnDelayed(claimer, amount);
        }
        for (uint256 i = 0; i < _pendingAvaxClaimGap; i++) {
            uint256 amount = _manualStashed[i];
            address claimer = _manualClaimers[i];
            if (claimer == address(0) || amount == 0) {
                continue;
            }
            IBondAVAX_R1(_fAvaxContract).migrateBurnDelayed(claimer, amount);
...
interface IGlobalPool_R2 {
    ...
    event MinimumStakeChanged(uint256 amount);

    event PendingClaimGapReset(uint256 gap);

    event PendingGapReset(uint gap);

    event CertContractChanged(address cert);

    event BondContractChanged(address bond);
    ...
    function claimCertsFor(address recipient, uint256 amount) external;
