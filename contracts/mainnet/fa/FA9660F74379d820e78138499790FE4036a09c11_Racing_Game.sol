...
contract Racing_Game is Ownable {
    ...
    uint256 public booster1To = 11000;
    uint8 public booster1Cooldown = 2;

    uint256 public booster2From = 0;
    uint256 public booster2To = 0;
    ...
    function getRaceAddressListLength(uint256 _raceId)
        external
        view
        returns (uint256)
    {
        return raceAddressList[_raceId].length;
    }
    ...
    function signUp(uint256 boosterId, uint16 booster2Threshold) public {
        ...
        /*
        // no need of this since we force start if all slots are filled
        require(
            raceAddressList[raceId].length < maxRacers,
        ...
        */
        ...
        if (booster != 1 && booster1Usage[epochId][msg.sender] > 0) {
            booster1Usage[epochId][msg.sender] -= 1;
        }

        if (booster != 2 && booster2Usage[epochId][msg.sender] > 0) {
            booster2Usage[epochId][msg.sender] -= 1;
        }

        if (
            raceAddressList[raceId].length >= maxRacers
            || (raceAddressList[raceId].length >= minRacers &&
    ...
    function rewardWinnerAndStartNew() public {
        ...
        // return quarter ticket-price (pricePerRace) to 3rd place
        shady.transfer(third, pricePerRace / 4);
