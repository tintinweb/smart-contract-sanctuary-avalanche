...
contract ManualCompound is Initializable, OwnableUpgradeable {
    ...
    event Converted(address indexed user, address indexed token, uint256 amount);
    event Compounded(address indexed user, address indexed token, uint256 amount);
    event Skipped(address indexed user, address indexed token, uint256 amount);
    ...
    function addReward(
        ...
        rewards.push(
            Reward({tokenAddress: _tokenAddress, tokenHelper: _tokenHelper, convertor: _convertor})
        );
    }
        ...
        for (uint256 i; i < rewardTokensLength; i++) {
            ...
            if (isRewardSkippable[_tokenAddress] && compoundType == CompoundType.IGNORE) {
                if (contractBalance > 0) {
                    ...
                    emit Skipped(msg.sender, _tokenAddress, contractBalance);
                }
            } else {
                ...
                if (diffBalance + contractBalance > 0) {
                    ...
                    if (
                        ...
                        if (_convertor != address(0)) {
                            ...
                            emit Converted(msg.sender, _tokenAddress, diffBalance);
                        }
                        if (_helperAddress != address(0)) {
                            ...
                            emit Compounded(msg.sender, _tokenAddress, diffBalance);
                        }
                        require(
