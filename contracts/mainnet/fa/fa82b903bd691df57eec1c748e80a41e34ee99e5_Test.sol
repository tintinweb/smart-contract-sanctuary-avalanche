...
contract Test is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	uint256 public nextAoFsAirdrop;
	...
	constructor(string memory _initUnrevealedURI, address _initArtistAddress, address _initCommunityAddress) ERC721("Army of Frogs", "AoFs") {
		paused = true;
		revealed = false;
		allowListings = true;
		...
		AoFsAddr = 0x7b16DeC105055164F0e56115d823bCb3879Aa90E;
	...
	function airDropAoFsOwner(uint256 _lastToken) public onlyOwner {
		require(nextAoFsAirdrop <= _lastToken, "Token already airdropped!");
		require(_lastToken <= 3500, "Last token must be 3500 or less.");

		IERC721 AoFsContract = IERC721(AoFsAddr);

		for (uint256 i = nextAoFsAirdrop; i <= _lastToken; i++) {

			supply.increment();

			uint256 tokenID = supply.current();

			_safeMint(AoFsContract.ownerOf(i), tokenID);

			reservedSupply -= 1;

		}

        nextAoFsAirdrop = _lastToken + 1;
	}
    ...
    function setNextAoFsAirDrop(uint256 newAmount) public onlyOwner {
		nextAoFsAirdrop = newAmount;
	}
    ...
    function setAoFsAddr (address _contract) public onlyOwner {
		AoFsAddr = _contract;
	}
