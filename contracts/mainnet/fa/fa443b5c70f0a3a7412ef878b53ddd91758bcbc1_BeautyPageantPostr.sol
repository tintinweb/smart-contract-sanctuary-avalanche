...
contract BeautyPageantPostr is ERC721URIStorage, Ownable, Pausable
{
  ...
  uint256 public maxMintable = (10*1000*1000);//no more than this many nfts can be minted (about the current supply of egg tokens
  ...
  constructor() ERC721("Beauty Pageant Postr", "POSTR") {
    baseURI = "ipfs://QmNtSpy4aDx6k5gNcePnbiHpnPZnDaeYbN9MKS4ZrNui8Q/";//add in a starter ipfs location info for the json metadata test 
  }//End of constructor
...
//Added in pausable functions from Openzepplin contract wizard
  ...
  function ownerMintSend(address recipient, uint _numberedContestantID) external onlyOwner{//could also use this to mint special postrs with new "contestant id number" just to get to new json file for uniques or whatever by hand. Or same postr but in gold or different color. Maybe make some for each team member and extras for giveaways. 
      _privateMint(recipient, _numberedContestantID);
  }

  function ownerMintSelf(uint _numberedContestantID) external onlyOwner{
  ...
  function _privateMint(address recipient, uint _numberedContestantID) private { //nft mint hashlips inspired
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    //Shows an I Voted sticker based on the contestant they voted on
  }
