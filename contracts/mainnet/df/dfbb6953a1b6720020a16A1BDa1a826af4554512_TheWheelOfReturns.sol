...
interface Random {
    function randomDays() external view returns (uint256);
    function randomPerc() external view returns (uint256);
}
...
contract TheWheelOfReturns is Ownable {
    ...
    uint256 private constant PRIMARY_BENIFICIARY_PERC = 80; //8%
    uint256 private constant SECONDARY_BENIFICIARY_PERC = 20; //2%
    uint256 private constant MIN_INVESTMENT = 0.1 ether;
    uint256 private constant MIN_WITHDRAW = 0.005 ether;
    ...
    address payable public primaryBenificiaryAddress;
    address payable public secondaryBenificiaryAddress;
    ...
    constructor(address payable _primaryBenificiaryAddress,address payable _secondaryBenificiaryAddress,address _randomContractAddress) {
        require(
            _primaryBenificiaryAddress != address(0),
            "Benificiary address cannot be null"
        );
        require(
            _secondaryBenificiaryAddress != address(0),
            "Benificiary address cannot be null"
        );
        require(isContract(_randomContractAddress),"Not a contract address");
        primaryBenificiaryAddress = _primaryBenificiaryAddress;
        secondaryBenificiaryAddress = _secondaryBenificiaryAddress;
        randomContract = Random(_randomContractAddress);
    }

    function changeRandomContractAddress(address newAddress) public onlyOwner {
        require(newAddress != address(0), "Address cannot be null");
        randomContract = Random(newAddress);
    }

    function changePrimaryBenificiary(address payable newAddress) public onlyOwner {
        require(newAddress != address(0), "Address cannot be null");
        primaryBenificiaryAddress = newAddress;
    }
    
    function changeSecondaryBenificiary(address payable newAddress) public onlyOwner {
        require(newAddress != address(0), "Address cannot be null");
        secondaryBenificiaryAddress = newAddress;
    }
    ...
    function invest(address _ref) public payable {
        require(isSpinOpen, "Cannot spin at the moment");
    ...
    function _invest(
        ...
        require(
            ...
            "Cannot spin more than 100"
        );

        uint256 day = randomContract.randomDays();
        uint256 perc = randomContract.randomPerc().mul(10);
        ...
        if (investor.ref != address(0)) {
            ...
            primaryBenificiaryAddress.transfer(amount);
            totalReferralReward = totalReferralReward.add(amount);
        }
    ...
    function _sendRewardOnInvestment(uint256 _amount) private {
        ...
        uint256 rewardForPrimaryBenificiary = _amount.mul(PRIMARY_BENIFICIARY_PERC).div(
            TOTAL_PERC
        );
        primaryBenificiaryAddress.transfer(rewardForPrimaryBenificiary);
        uint256 rewardForSecondaryBenificiary = _amount.mul(SECONDARY_BENIFICIARY_PERC).div(
            TOTAL_PERC
        );
        secondaryBenificiaryAddress.transfer(rewardForSecondaryBenificiary);
    }
    ...
    function withdraw() public {
        ...
        require(totalAmount >= MIN_WITHDRAW,"Cannot withdraw less than 0.005");
