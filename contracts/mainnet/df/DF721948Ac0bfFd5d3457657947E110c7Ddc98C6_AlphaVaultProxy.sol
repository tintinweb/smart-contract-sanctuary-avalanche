...
contract AlphaVaultProxy is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        swapRouter = address(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        cap = _cap;
        ...
        stgToken = address(0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590);
    ...
    function start() external onlyManagerOwner nonReentrant {
        ...
        _stargateDeposit(IERC20Upgradeable(token).balanceOf(address(this)) * 80 / 100);
    ...
    function harvest(bool autocompound) public onlyManagerOwner {
        ...
        uint256 stgBalance = IERC20Upgradeable(stgToken).balanceOf(address(this));
        IERC20Upgradeable(stgToken).safeApprove(swapRouter, stgBalance);
    ...
    /// @notice Sets new Stargate related addresses and ids.
    /// @param _stargateLpStaking The address of the LP staking contract.
    /// @param _stargateRouter The address of the Stargate router.
    /// @param _stargateLpToken The address of the S* LP token.
    /// @param _stgToken The address of the STG token.
    /// @param _stargateRouterPoolId The id of the S* token in the router contract.
    /// @param _stargateLpStakingPoolId The id of the S* token in the LP staking contract.
    function setStargateAddresses(address _stargateLpStaking, address _stargateRouter, address _stargateLpToken, address _stgToken, uint8 _stargateRouterPoolId, uint8 _stargateLpStakingPoolId) external onlyOwner {
        stargateLpStaking = IStargateLpStaking(_stargateLpStaking);
