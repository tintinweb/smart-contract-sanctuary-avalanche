...
contract RytellWoodElf is ERC721Metadata {
    ...
    function mint(uint256 counts_)
        external payable {
        require(pendingCount > 0, "Rytell Orc: All minted");
        require(counts_ > 0, "Rytell Orc: Counts cannot be zero"); 
        require(totalSupply().add(counts_) <= MAX_NFT_SUPPLY,
            "Rytell Orc: sale already ended"); 
        require(!paused, "Rytell Orc: The contract is paused"); 
    ...
    function _randomMint(address to_)
        ...
        require(to_ != address(0), "Rytell Orc: zero address");

        require(totalSupply() < MAX_NFT_SUPPLY,
            "Rytell Orc: max supply reached");
