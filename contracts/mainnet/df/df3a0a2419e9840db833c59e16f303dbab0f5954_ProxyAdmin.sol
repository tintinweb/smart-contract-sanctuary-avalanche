...
contract TransparentUpgradeableProxy is ERC1967Proxy {
     ...
     */
    uint256 public timelockLength = 0;
    ...
    function submitUpgrade(address newImplementation) external ifAdmin {
        if (timelockLength == 0) {
            _upgradeToAndCall(nextImplementation, bytes(""), false);
        } else {
    ...
    function upgradeTo() external ifAdmin {
        require(nextImplementation != address(0), "Can't upgrade to address 0");
        require(block.timestamp > timelockEndForUpgrade, "Timelock not finished");
        _upgradeToAndCall(nextImplementation, bytes(""), false);
     ...
     */
    function upgradeToAndCall(bytes calldata data)
        external
        payable
        ifAdmin
    {
        require(nextImplementation != address(0), "Can't upgrade to address 0");
        require(block.timestamp > timelockEndForUpgrade, "Timelock not finished");
        _upgradeToAndCall(nextImplementation, data, true);
        nextImplementation = address(0);
