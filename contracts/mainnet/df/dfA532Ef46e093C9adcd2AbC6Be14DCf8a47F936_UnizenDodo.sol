...
contract UnizenDodo {
        ...
        payable
    {
        ...
        uint256 remainingAmount = _generalBalanceOf(fromToken, address(this));
        _generalTransfer(toToken, payable(msg.sender), returnAmount);
        if (remainingAmount > 0) {
            _generalTransfer(fromToken, payable(msg.sender), returnAmount);
        }
    ...
    function _generalApproveMax(
        ...
        if (allowance < amount) {
            ...
            IERC20(token).safeApprove(to, type(uint128).max);
        }
    }
