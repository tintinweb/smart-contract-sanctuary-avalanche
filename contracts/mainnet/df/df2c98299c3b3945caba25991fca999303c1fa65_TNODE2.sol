...
contract TNODE2 is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    uint256 private constant _totalSupply = 1 * 1e6 * 1e18; // 1 million token total supply
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("TestNodes2", "TNODE2") PaymentSplitter(payees, shares) {

        treasuryPool = addresses[1];
        distributionPool = addresses[2];
        liquidityPool = addresses[3];

        require(treasuryPool != address(0) && distributionPool != address(0), "TREASURY & REWARD ADDRESS CANNOT BE ZERO");
        ...
        // launch controls
        maxTransactionAmount = totalSupply() * 10 / 10000; // 0.1% max transaction = 1,000 tokens.
        maxWallet = totalSupply() * 1 / 100; // 1% max wallet = 10,000 tokens.

        // fee exclusions
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[liquidityPool] = true;
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_totalSupply * 1 / 1000) / 1e18, "Cannot set maxTransactionAmount lower than 0.1%");
        maxTransactionAmount = newNum * (10 ** 18);
    }

    function updateMaxWallet(uint256 newNum) external onlyOwner {
        require(newNum >= (_totalSupply * 1 / 10000) / 1e18, "Cannot set maxTransactionAmount lower than 0.1%");
    ...
    // gas limits to prevent sniper buys in first blocks
    ...
    // when sell
            ...
            if (requireNodeToSell) {
                require(nodeRewardManager._isNodeOwner(_msgSender()),"Sell transfer requires node creation");
            }
        }
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        require(
            ...
            "NODE CREATION: treasuryPool and rewardsPool cannot create node"
        );
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool,
            "CSHT: treasuryPool and rewardsPool cannot cashout rewards"
        );
    ...
    function cashoutAll() public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool,
            "MANIA CSHT: treasuryPool and rewardsPool cannot cashout rewards"
        );
    ...
    function changeRequireNodeToSell(bool newVal) private onlyOwner {
        requireNodeToSell = newVal;
    }

    function changeGasPriceLimit(uint256 gas) external onlyOwner {
        require(gas >= 2700000000);
        gasPriceLimit = gas;
    }

    function changeBuyTaxFee(uint256 buyTaxFee) external onlyOwner {
        _buyTaxFee = buyTaxFee;
        require(_buyTaxFee <= 2000,"Must keep buy taxes below 20%");
    }

    function changeSellTaxFee(uint256 sellTaxFee) external onlyOwner {
        _sellTaxFee = sellTaxFee;
        require(_sellTaxFee <= 2000,"Must keep sell taxes below 20%");
    }
