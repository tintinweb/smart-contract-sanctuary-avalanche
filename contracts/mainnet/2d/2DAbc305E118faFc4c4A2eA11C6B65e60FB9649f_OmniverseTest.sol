...
contract OmniverseTest is ERC20Detailed, Ownable, MinterRole {
    ...
    constructor(
        ...
        ERC20Detailed("OmniverseTest", "OMNITEST", uint8(DECIMALS))
    {
    ...
    function setFees(
        uint256 buyPercent,
        uint256 sellPercent,
        uint256 maxTaxReceiverPercent
    ) external onlyOwner {
        require(buyPercent <= MAX_FEE, "exceeded max buy fees");
        require(sellPercent <= MAX_FEE, "exceeded max sell fees");
        require(
            maxTaxReceiverPercent <= MAX_FEE,
            "exceeded max tax receiver fees"
        );

        buyFee = buyPercent;
        sellFee = sellPercent;
        maxTaxReceiverFee = maxTaxReceiverPercent;
    }
    ...
    function mint(address recipient, uint256 amount) external onlyMinter {
        uint256 supply = totalTokenSupply + amount;

        if (supply > MAX_SUPPLY) {
            supply = MAX_SUPPLY;
        }

        amount = supply - totalTokenSupply;
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue)
        ...
        returns (bool)
    {
        if (subtractedValue >= allowedFragments[msg.sender][spender]) {
            allowedFragments[msg.sender][spender] = 0;
        } else {
            allowedFragments[msg.sender][spender] -= subtractedValue;
        }
        emit Approval(
            msg.sender,
            spender,
    ...
    function getSellFee(address addr) private view returns (uint256) {
        ...
        if (getMaxTaxReceiver(addr)) {
            sellPercentage = maxTaxReceiverFee;
        }
