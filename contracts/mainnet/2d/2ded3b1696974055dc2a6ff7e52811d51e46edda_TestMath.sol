...
library boostLib {
    using SafeMath for uint256;
    function boostPer(uint256 _dailyRewardsPerc) internal pure returns(uint256){
            uint256 _one_ = 1;
  ...
  Authorizable
{
  ...
  constructor(address[] memory swaps,uint256[] memory _boosts,uint256[3] memory _nftCost,uint256[] memory _vars,uint256[] memory _fees,uint256[] memory _cashoutRed){
	  ...
	  for(uint i=0;i<4;i++){
	  	boostRewardsPerMin[i] =boostLib.calcReward(boostLib.boostPer(_vars[0]),_vars[1].mul(1 minutes),time,lastClaimTime,boostMultiplier[i]);
