  ...
  */
  function delta(address _token) public view returns (int256) {
    ...
    for (uint256 i = 0; i < tokenAddresses.length; i ++) {
      ...
      if (tokenAddresses[i] == BTCb) BTCbAmt = tokenAmts[i];
      if (tokenAddresses[i] == WBTC) WBTCAmt = tokenAmts[i];
    ...
    if (_token == BTCb || _token == WBTC) tokenDebt = tokenDebt * 10**(18 - BTCb_DECIMALS);
