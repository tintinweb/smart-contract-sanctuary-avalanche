...
contract PowerBombAvaxAncSwap is PowerBombAvaxAnc {
    ...
    function harvest() external override nonReentrant {
        ...
        if (harvestAmt != 0) {
            ...
            // Swap UST to rewardToken through WAVAX
            address[] memory path = new address[](3);
            path[0] = address(UST);
            path[1] = address(WAVAX);
            path[2] = address(rewardToken);
            uint rewardTokenAmt = router.swapExactTokensForTokens(
                harvestAmt, 0, path, address(this), block.timestamp
            )[2];
