    ...
    ReentrancyGuardUpgradeable
{
    ...
    function deposit(
        ...
        if (whitelistEnabled) {
            ...
            bytes32 msgHash = keccak256(
                abi.encode(msg.sender, maxBlockNumber)
            );
            require(
        ...
        if (chainId == activeStrat.chainId) {
            ...
            uint256 deposited = IStrategyMasterchefFarmV2(activeStrat.strategyAddress)
                .deposit(msg.sender, path, "");
    ...
    function withdraw(
       ...
       if (whitelistEnabled) {
        ...
        if (chainId == activeStrat.chainId) {
            ...
            uint256 withdrew = IStrategyMasterchefFarmV2(activeStrat.strategyAddress)
                .withdraw(msg.sender, shares, token0ToTokenOut, token1ToTokenOut, "");
            ...
            emit WithdrawalRequested(msg.sender, poolId, shares, chainId, tokenOut);
        }
    }
    ...
    function placeToFarm(
        ...
        uint256 deposited = IStrategyMasterchefFarmV2(activeStrat.strategyAddress).deposit(
            user,
            pathBaseToToken0,
            ""
        );
    ...
    function initWithdrawal(
        ...
        uint256 tokenOutAmount = IStrategyMasterchefFarmV2(activeStrat.strategyAddress)
    ...
    function initMigration(
        ...
        IStrategyMasterchefFarmV2(_currentStrategy.strategyAddress).withdrawAll();
        ...
        if (_strategy.chainId == chainId) {
            ...
            IStrategyMasterchefFarmV2(_strategy.strategyAddress).deposit(_strategy.strategyAddress, path, "");
        } else {
     ...
     *@param _strategy Tupple of the new strategy.
     */
    function setStrategy(Strategy calldata _strategy) external onlyRole(ADMIN_ROLE) {
    ...
    function quotePotentialWithdraw(
        ...
        address[] calldata token1toTokenOut
            ...
            amountOut = IStrategyMasterchefFarmV2(stratAddress).quotePotentialWithdraw(
                shares,
                token0toTokenOut,
                token1toTokenOut
            );
    ...
    function userAllocation(uint256 poolId, address user) external view returns(uint256 allocation) {
            address stratAddress = poolIdtoActiveStrategy[poolId].strategyAddress;
            allocation = IStrategyMasterchefFarmV2(stratAddress).userAllocation(user);
    }

    function totalAllocation(uint256 poolId) external view returns(uint256 allocation) {
            address stratAddress = poolIdtoActiveStrategy[poolId].strategyAddress;
