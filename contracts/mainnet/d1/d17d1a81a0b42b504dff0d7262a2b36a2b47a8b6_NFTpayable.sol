...
abstract contract prevNFTpayable is Context {
    ...
    function PROTOaccountData(address _account, uint256 _x) external virtual returns(uint256,uint256,uint256,uint256,uint256,uint256,bool,bool);
}
...
contract NFTpayable is Ownable {
    ...
    function payFees(address _account,uint256 _intervals) payable external {
   ...
   function queryERC20Balance(address _account,address _token) internal view returns (uint) {
    	    	...
    	    	for(uint i=0;i<accts;i++){
    	    		...
    	    		if(isInList(_account,PROTOtransfered) == false){
	    	    		...
	    	    		for(uint j=0;j<P_stars;j++){
		    	    		(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e,uint256 f,bool g,bool h) = _prev.PROTOaccountData(_account,j);
    ...
    function PROTOaccountData(address _account, uint256 _x) external onlyGuard onlyManager(msg.sender) returns(uint256,uint256,uint256,uint256,uint256,uint256,bool,bool){
    		PROTOstars[] storage stars = protostars[_account];
    		PROTOstars storage star = stars[_x];
    		return (star.nextDue,star.feeFroze,star.futureFees,star.creationTime,star.protoElapsed,star.collapseDate,star.insolvent,star.owed);
    	}
