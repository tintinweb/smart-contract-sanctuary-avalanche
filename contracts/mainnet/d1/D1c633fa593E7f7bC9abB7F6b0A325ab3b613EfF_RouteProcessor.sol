/**
 *Submitted for verification at snowtrace.io on 2023-03-26
*/
...
contract RouteProcessor {
  ...
  function processRouteInternal(
    ...
    while (stream.isNotEmpty()) {
      ...
      if (commandCode < 20) {
        if (commandCode == 10)
          swapKamiswapPool(stream); // kami/kamiswp pool swap
        ...
        if (commandCode == 20) gniDepositAmountFromGni(stream, tokenIn);
        else if (commandCode == 21) swapTrident(stream);
        else if (commandCode == 23) gniWithdrawShareFromRP(stream, tokenIn);
        else revert('Unknown command code');
      } else {
        if (commandCode == 24) amountInAcc += distributeGniShares(stream, tokenIn);
        else if (commandCode == 25) distributeGniPortions(stream);
        else if (commandCode == 26) gniDepositAllFromGni(stream);
        else if (commandCode == 27) gniWithdrawAllFromRP(stream);
        else revert('Unknown command code');
      }
    }
  ...
  /// @notice Transfers input tokens sent to GniLamp to a pool
  /// @notice Expected to be called for initial liquidity transfer from user to GniLamp, so we know exact amounts
  /// @param stream [Pool, Amount]. Pool into which an amount of tokens will be transferred
  /// @param token Address of the token to transfer
  function gniDepositAmountFromGni(uint256 stream, address token) private {
    address to = stream.readAddress();
    uint256 amount = stream.readUint();
    gniLamp.deposit(token, address(gniLamp), to, amount, 0);
  }

  /// @notice Transfers all available input tokens from GniLamp to a pool
  /// @param stream [Pool, Token]. Pool into which all tokens will be transferred 
  function gniDepositAllFromGni(uint256 stream) private {
    address to = stream.readAddress();
    address token = stream.readAddress();

    uint256 amount = IERC20(token).balanceOf(address(gniLamp)) +
      gniLamp.strategyData(token).balance -
      gniLamp.totals(token).elastic;
    gniLamp.deposit(token, address(gniLamp), to, amount, 0);
  }

  /// @notice Withdraws GniLamp tokens from GniLamp to an address
  /// @param stream [To, Amount]. Destination where an amount of token will be transferred
  /// @param token Token to transfer
  function gniWithdrawShareFromRP(uint256 stream, address token) private {
    address to = stream.readAddress();
    uint256 amount = stream.readUint();
    gniLamp.withdraw(token, address(this), to, amount, 0);
  }

  /// @notice Withdraws all available GniLamp tokens from GniLamp to an address
  /// @param stream [Token, To]. Token which will be transferred to a destination
  function gniWithdrawAllFromRP(uint256 stream) private {
    address token = stream.readAddress();
    address to = stream.readAddress();
    uint256 amount = gniLamp.balanceOf(token, address(this));
    gniLamp.withdraw(token, address(this), to, 0, amount);
  }
  ...
  /// @notice Performs a Kami/Kamiswap pool swap
  /// @param stream [Pool, TokenIn, Direction, To]
  /// @return amountOut Amount of the output token
  function swapKamiswapPool(uint256 stream) private returns (uint256 amountOut) {
    address pool = stream.readAddress();
  ...
  /// @notice Distributes input GniLamp tokens from msg.sender to addresses. Tokens should be approved
  /// @notice Expected to be called for initial liquidity transfer from the user to pools, so we know exact amounts
  ...
  function distributeGniShares(uint256 stream, address token) private returns (uint256 sharesTotal) {
    uint8 num = stream.readUint8();
    sharesTotal = 0;
    for (uint256 i = 0; i < num; ++i) {
      address to = stream.readAddress();
      ...
      sharesTotal += share;
      gniLamp.transfer(token, msg.sender, to, share);
    }
  }
  ...
  /// @notice Distributes GniLamp tokens from RouteProcessor to addresses
  ...
  /// @param stream [Token, ArrayLength, ...[To, ShareAmount][]]. Token to distribute. An array of destinations and token share amounts
  function distributeGniPortions(uint256 stream) private {
    address token = stream.readAddress();
    uint8 num = stream.readUint8();
    ...
    unchecked {
      for (uint256 i = 0; i < num; ++i) {
        ...
        uint256 amount = (amountTotal * share) / 65535;
        amountTotal -= amount;
        gniLamp.transfer(token, address(this), to, amount);
      }
    }
  }
