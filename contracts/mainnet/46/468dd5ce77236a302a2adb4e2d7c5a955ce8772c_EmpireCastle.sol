...
// File: contracts/EmpireCastle.sol
...
contract EmpireCastle is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, Randomizer {
    ...
    // MODIFIERS
    modifier onlyDevs() {
        require(devFees[msg.sender].percent > 0, "Dev Only: caller is not the dev");
        _;
    }
    ...
    // INSTRUCTIONS
    // PUBLIC:   Price : 50000000000000000 (0.05 AVAX)   MaxTx : 10   MaxPerPerson : 100
    ...
    address public royaltyAddress = 0x084de7F40E8239dB8E43d990CecA4F3bf6F4eBC9;
    ...
    constructor(
        ...
        uint[] memory _fees,
        address _lzEndpoint
    ) ERC721("EmpireCastle","Hero") Randomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: Max 100 per Wallet");
        if (price > 0) require(msg.value == price * amount, "Error amount");
        if (whitelistedOnly) require(whiteListed[msg.sender] >= amount, "Error:  Not whitelist ");
