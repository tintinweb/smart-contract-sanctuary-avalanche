...
contract Diligence is Owned(tx.origin) {
    ...
    function swap(uint256 amount, address from, address to, address router) internal returns (uint256)
    {
        ...
        path[0] = from;
        path[1] = to;
        //bytes memory payload = abi.encodeCall(IRouter(router).swapExactTokensForTokens, (amount, 0, path, address(this), block.timestamp));
        ...
        //(bool success, bytes memory result) = address(router).call(payload);
    ...
    function buyAndSell(address token, address with, uint256 amount, address factory) internal onlyOwner returns (uint256)
    {
