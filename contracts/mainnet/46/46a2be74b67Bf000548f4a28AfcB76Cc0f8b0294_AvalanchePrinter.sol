...
contract AvalanchePrinter {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 0.05 ether; // 5 FTM
	uint256[] public REFERRAL_PERCENTS = [60, 30, 10];
	uint256 constant public PROJECT_FEE = 66;
	uint256 constant public DEVELOPER_FEE = 44;
			...
			if (user.checkpoint < user.deposits[i].finish) {
				if (user.deposits[i].plan < 3) {
					...
					if (from < to) {
						...
						uint256 _dividendsWithFee = _dividends.sub(_dividends.mul(WITHDRAW_FEE).div(PERCENTS_DIVIDER));
					...
					if(block.timestamp > user.deposits[i].finish) {
						...
						uint256 _profitWithFee = _profit.sub(_profit.mul(WITHDRAW_FEE).div(PERCENTS_DIVIDER));
