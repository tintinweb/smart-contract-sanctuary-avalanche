    ...
    ReentrancyGuardUpgradeable
{
        ...
        onlyOwner
    {
        ...
        IYieldAdapter(previousStrategy).withdraw(_token, type(uint256).max);
        ...
        onlyEmergencyAdminOrOwner
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            IYieldAdapter(tokenStrategy).withdraw(token, type(uint256).max);
        }
    }
    ...
    function _removeOrder(
        ...
        if (contractBal < depositPlusYield && tokenStrategy != address(0)) {
            ...
            uint256 withdrawAmount = depositPlusYield +
                neededAmountInBuffer -
                contractBal;
            try
                IYieldAdapter(tokenStrategy).withdraw(_token, withdrawAmount)
            {} catch {
                IERC20(_token).safeTransfer(tokenStrategy, contractBal);
                IYieldAdapter(tokenStrategy).deposit(_token, contractBal);
                IYieldAdapter(tokenStrategy).withdraw(_token, type(uint256).max);
            }
        }
    }
}
