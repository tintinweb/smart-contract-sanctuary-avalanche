...
contract RShareRewardPool {
    ...
    uint256 public tSharePerSecond = 0.00156322 ether; // 50000 rshare / (370 days * 24h * 60min * 60s)
    ...
    function safeBShareTransfer(address _to, uint256 _amount) internal {
        uint256 _bshareBal = rshare.balanceOf(address(this));
        if (_bshareBal > 0) {
            if (_amount > _bshareBal) {
                rshare.safeTransfer(_to, _bshareBal);
            } else {
