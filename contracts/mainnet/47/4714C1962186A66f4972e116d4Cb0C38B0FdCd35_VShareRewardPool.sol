...
// File contracts/distribution/VShareRewardPool.sol
...
contract VShareRewardPool {
    ...
    uint256 public vSharePerSecond = 0.00497685 ether; // 64500 vshare / (150 days * 24h * 60min * 60s)
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _vshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (vSHARE or lps) if less than 90 days after pool ends
