...
contract PlatinumManager is Ownable, IERC721, IERC721Metadata, IAscendMembershipManager {
    ...
    constructor(address _AscendManager, address _ASCEND) {
        nodeCounter = 1;
        manager = IAscendMembershipManager(_AscendManager);
    ...
    function _addPlatinum(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createPlatinum(_id, _name, _mint, _claim, _metadata, _to);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "Platinum Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNodeWithTokens(string memory _name, uint64[] memory userNodes) public {
        ...
        for (uint256 i = 0; i < nodeAscendMembershipPrice; i++) {
            manager.transferFrom(_msgSender(), address(0), userNodes[i]);
        }
    ...
    if (transferIsEnabled == true){
            ...
            require (_msgSender() == upgradeManager, "Platinum Transfers are not allowed");
            _transfer(from, to, tokenId);
            require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
        }
        _transfer(from, to, tokenId);
