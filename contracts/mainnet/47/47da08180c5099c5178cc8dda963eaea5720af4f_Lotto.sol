/**
 *Submitted for verification at snowtrace.io on 2022-10-14
*/
...
contract Lotto is ILotto, VRFConsumerBaseV2 {
    ...
    uint32 public constant MONTHLY_WINNERS = 0;
    ...
    // Burn GameFi Amounts
    uint256 public GRAPES_BURNED_PER_TEN_TICKETS = 10 * 10**18;
    ...
    uint256 public LP_Per_Ticket = 500 * 10**18;
    ...
    uint256 public rollOverPercentage = 15;

    // token reward allocations
    uint256 public largestDailyPercent   = 70;
    uint256 public dailyDepositPercent   = 30;
    uint256 public monthlyWinnersPercent = 0;
    uint256 public percentDenominator    = 100;

    // Gas For Lottery Trigger
    uint32 public dailyGas   = 2_500_000;
    uint32 public monthlyGas = 2_500_000;
    ...
    modifier onlyAuth(){
        require(
            msg.sender == IOwnable(Miner).getOwner() || msg.sender == bot,
            'Only Auth Can Call'
        );
        _;
    }
    ...
    function compoundoor(address user, uint256 amount) external override {
        require(
            msg.sender == Miner,
            'Only Miner Can Register Users'
        );
    ...
    function setAuthBot(address _bot) external onlyOwner {
        bot = _bot;
    }
     ...
     */
    function forceNewDay() external onlyAuth {
        _newDay();
    }
    ...
    /**
        ...
        This is a dangerous function, as it changes the timing and frequency of lotto rewards
     */
    function forceNewMonth() external onlyAuth {
        _newMonth();
    }
    ...
    /**
        ...
        This is a dangerous function, as it changes the timing and frequency of lotto rewards
     */
    function forceNewDayAndMonth() external onlyAuth {
        _newDay();
        _newMonth();
    }
    ...
    /**
        Withdraws AVAX That Is Stuck In This Contract
        Contract does not have receive function, so avax can only enter
    ...
    /**
        Sets The Number Of GRAPE To Burn To Create Ten Tickets For The User
        @param burnedPerTen number of GRAPE, cannot be zero
     */
    function setGRAPESBurnedPerTenTickets(uint burnedPerTen) external onlyOwner {
        require(
            burnedPerTen > 0,
            'Cannot Be Zero'
        );

        GRAPES_BURNED_PER_TEN_TICKETS = burnedPerTen;
    }
    ...
    /**
        Burns GRAPE For Tickets
        The amount transferred in depends on GRAPES_BURNED_PER_TEN_TICKETS
        NOTE: Must Have Prior Approval of GRAPE for address(this) before calling
        @param batches - number of batches of ten tickets to mint
     */
    function burnGRAPE(uint batches) external {
        // transfer in GRAPE
        uint received = _transferIn(IERC20(GRAPE), GRAPES_BURNED_PER_TEN_TICKETS * batches);
        require(
            received == GRAPES_BURNED_PER_TEN_TICKETS * batches,
            'Invalid Tokens Received'
        );
        // send GRAPE
        IERC20(GRAPE).transfer(treasury, received);
    ...
    function _binarySearch(uint256 id, uint256 low, uint256 high) internal view returns (address user) {
        ...
        if (low > high) {
            ...
            mid = ( low + high ) / 2;
            if (
                ticketRanges[mid].lower <= id &&
                ticketRanges[mid].upper > id
            ) {
                return ticketRanges[mid].user;
            }
        }

        if ( id > ticketRanges[mid].upper ) {
            return _binarySearch(id, mid + 1, high);
        } else {
            return _binarySearch(id, low, mid - 1);
        }
    }

    function _fetchTicketOwner(uint256 id) internal view returns (address) {
        return _binarySearch(id, 0, currentTicketRangeID - 1);
    }
    ...
    function isNewDay() public view returns (bool) {
        return (block.timestamp - lastDay) >= day;
    }
    ...
    function isNewMonth() public view returns (bool) {
        return (block.timestamp - lastMonth) >= month;
    }

    function timeLeftUntilNewDay() public view returns (uint256) {
        return isNewDay() ? 0 : day - ( block.timestamp - lastDay );
    }

    function timeLeftUntilNewMonth() public view returns (uint256) {
        return isNewMonth() ? 0 : month - ( block.timestamp - lastMonth );
    }
