...
// File: contracts/interfaces/IZSwapRouter01.sol


pragma solidity >=0.6.2;

interface IZSwapRouter01 {
...
// File: contracts/interfaces/IZSwapRouter02.sol


pragma solidity >=0.6.2;


interface IZSwapRouter02 is IZSwapRouter01 {
...
// File: contracts/ZSwapZapV1.sol

pragma solidity ^0.8.4;

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by ZSwapSwap teams.
 */
contract ZSwapZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped AVAX (WAVAX)
    IWETH public WAVAX;

    // ZSwapRouter interface
    IZSwapRouter02 public zswapRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from ZSwapSwap)
    ...
    // Address ZSwapRouter
    address private zswapRouterAddress;

    // Address Wrapped AVAX (WAVAX)
    ...
    // tokenToZap = 0x00 address if AVAX
    event ZapIn(
    ...
    // token0ToZap = 0x00 address if AVAX
    ...
    // tokenToReceive = 0x00 address if AVAX
    event ZapOut(
        address indexed lpToken,
    ...
    /*
     * @notice Fallback for WAVAX
     */
    receive() external payable {
        assert(msg.sender == WAVAXAddress);
    }

    /*
     * @notice Constructor
     * @param _WAVAXAddress: address of the WAVAX contract
     * @param _zswapRouter: address of the ZSwapRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WAVAXAddress,
        address _zswapRouter,
        uint256 _maxZapReverseRatio
    ) {
        WAVAXAddress = _WAVAXAddress;
        WAVAX = IWETH(_WAVAXAddress);
        zswapRouterAddress = _zswapRouter;
        zswapRouter = IZSwapRouter02(_zswapRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap AVAX in a WAVAX pool (e.g. WAVAX/token)
     * @param _lpToken: LP token address (e.g. ZAP/AVAX)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ZAP) to receive in the intermediary swap (e.g. AVAX --> ZAP)
     */
    function zapInAVAX(address _lpToken, uint256 _tokenAmountOutMin) external payable nonReentrant {
        WAVAX.deposit{value: msg.value}();

        // Call zap function
        uint256 lpTokenAmountTransferred = _zapIn(WAVAXAddress, msg.value, _lpToken, _tokenAmountOutMin);
     ...
     * @param _lpToken: LP token address (e.g. ZAP/USDC)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ZAP) in the intermediary swap (e.g. USDC --> ZAP)
     */
    function zapInToken(
        address _tokenToZap,
    ...
    /*
     * @notice Zap 1 token and AVAX, rebalance them to 50-50, before adding them to LP
     * @param _token1ToZap: address of token1 to zap
     * @param _token1AmountIn: amount of token1 to zap
    ...
    /*
     * @notice Zap a LP token out to receive AVAX
     * @param _lpToken: LP token address (e.g. ZAP/WAVAX)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in AVAX/WAVAX) in the intermediary swap (e.g. ZAP --> AVAX)
     */
    function zapOutAVAX(
        address _lpToken,
        ...
        // Call zapOut
        uint256 tokenAmountToTransfer = _zapOut(_lpToken, WAVAXAddress, _tokenAmountOutMin);

        // Unwrap AVAX
        WAVAX.withdraw(tokenAmountToTransfer);

        // Transfer AVAX to the msg.sender
        (bool success, ) = msg.sender.call{value: tokenAmountToTransfer}(new bytes(0));
        require(success, "AVAX: transfer fail");
     ...
     * @param _lpToken: LP token address (e.g. ZAP/USDC)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ZAP or USDC)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ZAP) in the intermediary swap (e.g. USDC --> ZAP)
     */
     ...
     * @dev Use WAVAX for _tokenToZap (if AVAX is the input)
    ...
    {
        address token0 = IZSwapPair(_lpToken).token0();
        address token1 = IZSwapPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IZSwapPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = zswapRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = zswapRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @dev Use WAVAX for _token0ToZap (if AVAX is the input)
    ...
    {
        require(
            _token0ToZap == IZSwapPair(_lpToken).token0() || _token0ToZap == IZSwapPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IZSwapPair(_lpToken).token0() || _token1ToZap == IZSwapPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IZSwapPair(_lpToken).token0()) {
    ...
    /*
     * @notice View the details for single zap
     * @dev Use WAVAX for _tokenToReceive (if AVAX is the asset to be received)
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IZSwapPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IZSwapPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = zswapRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = zswapRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IZSwapPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "ZSwapRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "ZSwapRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        zswapRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), zswapRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(zswapRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = zswapRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = zswapRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = zswapRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = zswapRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = zswapRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
