...
import { IRewardsToken } from "./interfaces/IRewardsToken.sol";
...
contract RewardsToken is IRewardsToken, ERC20, Pausable, Ownable, ERC20Permit, Lockable, MinterRole, BusinessAddresses {
    using ECDSA for bytes32;
    using Helper for bytes32;
    using BitMaps for BitMaps.BitMap;

    constructor() ERC20("Rewards Token", "RET") ERC20Permit("RewardsToken") {}
    ...
    function addMinter(address account_) external override onlyOwner {
        _setMinterStatus(account_, true);
    }

    function removeMinter(address account_) external override onlyOwner {
        _setMinterStatus(account_, false);
    }
    ...
    function burn(
        ...
        if (_isUsed.get(burnId)) revert IRewardsToken__AlreadyBurnt();
    ...
    function __notExpired(uint256 deadline_) private view {
        if (block.timestamp > deadline_) revert IRewardsToken__Expired();
    }
}
...
interface IRewardsToken {
    error IRewardsToken__AlreadyBurnt();
    error IRewardsToken__Expired();
    ...
    function removeMinter(address account_) external;
