...
// File: contracts/interfaces/IJoeFactory.sol
...
// File: contracts/interfaces/IJoeRouter.sol
...
// File: implementations/output/UniverseImplementationPointer.sol
...
// File: contracts/helpers/OwnerRecovery.sol
...
UniverseImplementationPointer
{
    ...
    address(0xB98D8C9487c55C7D1AB5E2e4aadaBf9Cc72d7aeF), // Treasury investments (40%)
    address(0xB98D8C9487c55C7D1AB5E2e4aadaBf9Cc72d7aeF), // Dev (30%)
    address(0xB98D8C9487c55C7D1AB5E2e4aadaBf9Cc72d7aeF) // Marketing (30%)
    ];
    uint8[] public feePercentages = [40, 30, 30];
    ...
    returns (bool)
    {
        ...
        if (
            ...
            swapLeftSideForRightSide(swapTokensToLiquidityThreshold / 2);
            uint256 totalRight = rightSide.balanceOf(address(this)) - initialRightBalance;
            uint256 totalRightRemaining = totalRight;

            for (uint256 i = 0; i < feeAddresses.length; i++) {
                if ((feeAddresses.length - 1) == i) {
                    // Send remaining right tokens to the last address
                    sendRightTokensTo(feeAddresses[i], totalRightRemaining);
                } else {
                    uint256 calculatedFee = (totalRight * feePercentages[i]) / 100;
                    totalRightRemaining -= calculatedFee;
                    sendRightTokensTo(feeAddresses[i], calculatedFee);
                }
            }

            uint256 totalLP = swapAndLiquify(swapTokensToLiquidityThreshold / 2);
    ...
    // Transfer right tokens conveniently
    function sendRightTokensTo(address to, uint256 tokens) private {
        rightSide.transfer(to, tokens);
    }
