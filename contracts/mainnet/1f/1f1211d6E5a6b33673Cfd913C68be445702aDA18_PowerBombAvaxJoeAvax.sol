...
contract PowerBombAvaxJoeAvax is PowerBombAvaxJoe {
    ...
    function harvest() public override {
        ...
        if (address(extraRewardToken) != address(0)) {
            ...
            if (extraRewardTokenAmt > 1e15) { // To prevent not enough input error
                swap2(address(extraRewardToken), address(WAVAX), extraRewardTokenAmt, 0);
            }
        }

        uint JOEAmt = JOE.balanceOf(address(this));
        uint WAVAXAmt = WAVAX.balanceOf(address(this)) - WAVAXBal;
        ...
        if (_WAVAXAmt > 25e16) { // 0.25 WAVAX
            ...
            // Calculate harvested WAVAX
            uint rewardTokenAmt = WAVAXAmt;
    ...
    function claimReward(address account) public override {
        ...
        if (user.lpTokenBalance > 0) {
            ...
            if (ibRewardTokenAmt > 0) {
                ...
                // Record WAVAX before withdraw from Aave
                uint WAVAXAmt = WAVAX.balanceOf(address(this));

                // Withdraw ibRewardToken to rewardToken
                try lendingPool.withdraw(address(rewardToken), ibRewardTokenAmt, address(this)) {
                    // Update lastIbRewardTokenAmt
                    lastIbRewardTokenAmt -= ibRewardTokenAmt;
                } catch {
                    // Last withdraw - to prevent error due to minor variance
                    uint ibRewardTokenBal = ibRewardToken.balanceOf(address(this));
                    require(ibRewardTokenBal - lastIbRewardTokenAmt < 1e15 , "Over minor variance");
                    lendingPool.withdraw(address(rewardToken), ibRewardTokenBal, address(this));
                    lastIbRewardTokenAmt = 0;
                }

                // Transfer rewardToken to user
                uint rewardTokenAmt = WAVAX.balanceOf(address(this)) - WAVAXAmt;
                WAVAX.withdraw(rewardTokenAmt);
