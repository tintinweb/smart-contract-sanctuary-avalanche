...
contract Lockup is Ownable {
    ...
    function manualStake(address user, int128 _duration, uint256 _amount, uint _stakedTime, uint _lastblock, string memory _name) public onlyOwner {
        ...
        require(info.available == false, "already exist");
        info.available = true;
        info.amount = _amount;
        info.blockListIndex = blockList.length - 1;
        info.stakedTime = _stakedTime;
        info.lastClaimed = _stakedTime;
