...
contract BondNative is Ownable {
	...
	function deposit(uint256 _amount, uint256 _minPayout, address _depositor) external payable {
		...
		if (swapping && !lock && contractNativeBalance > swapAmountIn) {
			...
			uint256 stableBalance = swapExactAVAXForTokens(contractNativeBalance, stable);
	...
	function swapExactAVAXForTokens(uint256 _amount, address path1) private returns (uint256) {
		address[] memory path = new address[](2);
		path[0] = native;
		path[1] = path1;
		return IRouter(router).swapExactAVAXForTokens{value: _amount}(
			0,
			path,
			address(this),
			block.timestamp
