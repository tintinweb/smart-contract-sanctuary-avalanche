...
contract PandaToken is ERC20('PandaSwap Token', 'PANDA') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BambooForest).
...
// BambooKnots with Governance.
contract BambooKnots is ERC20('BambooKnots Token', 'BAMBOO') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BambooForest).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safePandaTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy PandaSwap to PandaSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PandaSwap LP tokens.
    // PandaSwap must mint EXACTLY the same amount of PandaSwap LP tokens or
    // else something bad will happen. Traditional PandaSwap does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// BambooForest is the master of Panda. He can make Panda and he is a fair guy.
//
...
contract BambooForest is Ownable {
    using SafeMath for uint256;
    ...
    function updatePool(uint256 _pid) public {
        ...
        panda.mint(address(bamboo), pandaReward);
        pool.accPandaPerShare = pool.accPandaPerShare.add(pandaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to BambooForest for PANDA allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'deposit PANDA by staking');
    ...
    // Withdraw LP tokens from BambooForest.
    function withdraw(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'withdraw PANDA by unstaking');
    ...
    // Stake PANDA tokens to BambooForest
    function enterStaking(uint256 _amount) public {
        ...
        bamboo.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw PANDA tokens from STAKING.
    function leaveStaking(uint256 _amount) public {
        ...
        bamboo.burn(msg.sender, _amount);
    ...
    function safePandaTransfer(address _to, uint256 _amount) internal {
        bamboo.safePandaTransfer(_to, _amount);
    }
