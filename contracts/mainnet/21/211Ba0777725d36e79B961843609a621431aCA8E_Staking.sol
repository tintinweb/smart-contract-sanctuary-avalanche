...
/**
    ...
    function addNftTokenIds(uint256 _campaignId, address _collectionAddress, uint256[] memory _nftTokenIds) external onlyOwner {
        ...
        for (uint256 m; m < _nftTokenIds.length; m++) {
            ...
            NftTokenIds[_campaignId][_collectionAddress].push(_nftTokenIds[m]);
        }
    }
        ...
        if (length > 1) {
            ...
            for (uint256 x = 0; x < length; x++) {
                ...
                if (userStakeIds[msg.sender][x] == _stakeId) {
                    if (length > 1) {
                        userStakeIds[msg.sender][x] = userStakeIds[msg.sender][length.sub(1)];
           ...
           if(winningTicketId >= rangeStart && winningTicketId <= rangeEnd){
               if(rewardsReceived[_winnerAddress][campaignId] >= 5){
                ...
                (address _collection, uint256 _nftId) = getNftId();
                 ...
                 rewardsToken.safeTransferFrom(address(this), _winnerAddress, _nftId, "");
     ...
     * @return ranValues
     */
    function _getRandomNumbers(uint256 num, uint256 ranEnd, uint256 start, uint256 end) public view returns (uint256[] memory ranValues) {
    ...
    function _pickWinners(uint256 start, uint256 end) private {
        ...
        if (totalClaimableTickets == 0) {
         ...
         winningTicketIds[latestCampaignId] = _getRandomNumbers( N_REWARD_WINNER, totalClaimableTickets, start, end);
