...
contract PawnManager is Ownable, AidOwnable, IERC721, IERC721Metadata, IPawnManager, FoundersNFT {
    using Address for address;
    using SafeMath for uint256;
    FoundersNFT public foundersNFT = FoundersNFT(0x9308Ccf203e1c238d6fFE3a955467c1D6ed8f0F5);
    ...
    mapping(uint256 => Pawn) private _nodes;
    ...
    uint256 public castleMultiplier = 15;
    uint256 public bishopMultiplier = 0;

    bool public transferIsEnabled = false;


    IERC721 FOUNDER = IERC721(0x9308Ccf203e1c238d6fFE3a955467c1D6ed8f0F5);
    IERC721 CASTLE = IERC721(0x000000000000000000000000000000000000dEaD);
    IERC721 BISHOP = IERC721(0x000000000000000000000000000000000000dEaD);
    ...
    function createNode(address account, string memory nodeName) onlyContract override external {
        ...
        _createPawn(nodeId, nodeName, uint64(block.timestamp), uint64(block.timestamp), "", account);
        nodeCounter += 1;
    }
    ...
    function updatePawn(uint256 id, string calldata metadata) external {
        Pawn storage pawn = _nodes[id];
        pawn.metadata = metadata;
    }

    function getPawns(uint256 _id) public view onlyIfExists(_id) returns (Pawn memory) {
        return _nodes[_id];
    }

    function getRewardOf(uint256 _id) public view onlyIfExists(_id) returns (uint) {
        Pawn memory _node = _nodes[_id];
        uint interval = (block.timestamp - _node.claim) / claimTime;
    ...
    function getAddressRewards(address account) external view returns (uint) {
        ...
        uint256[] memory userPawns;
        userPawns = getPawnsOf(account);
        for (uint256 i = 0; i < userPawns.length; i++) {
            rewardAmount = rewardAmount + getRewardOf(userPawns[i]);
        }
    ...
    function getPawnsOf(address _account) public view returns (uint256[] memory) {
        return _bags[_account];
    }

    function setMultipliers(uint256 _foundersMultiplier, uint256 _castleMultiplier, uint256 _bishopMultiplier) onlyOwner external {
        foundersMultiplier = _foundersMultiplier;
	    castleMultiplier = _castleMultiplier;
	    bishopMultiplier = _bishopMultiplier;
    }

    function _changeRewardPerPawn(uint64 newReward) onlyOwner external {
        reward = newReward;
    }
    ...
    function _addPawn(uint256 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        _createPawn(_id, _name, _mint, _claim, _metadata, _to);
    }

    function _deletePawn(uint256 _id) onlyOwner external {
    ...
    function _deleteMultiplePawn(uint256[] calldata _ids) onlyOwner external {
    ...
    function getCastleMultiplier(address from) public view returns(uint256){
        if(CASTLE.balanceOf(from) >= 1){ return CASTLE.balanceOf(from).mul(castleMultiplier); 
        }
    ...
    function getBishopMultiplier(address from) public view returns(uint256){
        if(BISHOP.balanceOf(from) >= 1){ 
            return BISHOP.balanceOf(from).mul(bishopMultiplier); 
        }
        else{
            return 0;
        }
    ...
    {   
        ...
        upgradeNodeCount = CASTLE.balanceOf(from).add(BISHOP.balanceOf(from));
    ...
    function getUserMultiplier(address from) public view returns (uint256) {
         return getFoundersMultiplier(from).add(getCastleMultiplier(from)).add(getBishopMultiplier(from));
    }

   function setNewNodes(address _founder, address _castle, address _bishop) external onlyOwner {
        FOUNDER = IERC721(_founder);
        CASTLE = IERC721(_castle);
        BISHOP = IERC721(_bishop);
    }
    ...
    function _createPawn(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to) internal {
        require(!_exists(_id), "MANAGER: Pawn already exist");
        _nodes[_id] = Pawn({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        Pawn memory _node = _nodes[uint64(tokenId)];
    ...
    function renamePawn(uint64 id, string memory newName) external {
    ...
    function transferFrom(address from, address to,uint256 tokenId) external override {
        ...
        if (transferIsEnabled == true){
            ...
            require (to == deadAddress, "Pawn Transfers are not allowed");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override {
        ...
        if (transferIsEnabled == true){
            ...
            require (_msgSender() == upgradeManager, "Pawn Transfers are not allowed");
