...
contract Love_Swap_V1 {
    ...
    function Approve(address tokenAddress) public onlyOwner {
        ERC20(tokenAddress).safeApprove(aRouter,15792089237316195423570985008687907853269984665640564039457584007913129639936);
        ERC20(tokenAddress).safeApprove(bRouter,15792089237316195423570985008687907853269984665640564039457584007913129639936);
    }

    // arouter:ETH->tonken,brouter:tonken->ETH
    function doitForA(uint256 ethAmount,address tokenAddress) public  onlyOwner{
        uint256 ethBefore = address(this).balance;
        address[] memory data = new address[](2);
        data[0] = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
        data[1] = address(tokenAddress);
        ARouter(aRouter).swapExactAVAXForTokens.value(ethAmount)(0,data,address(this),block.timestamp);
        ...
        BRouter(bRouter).swapExactTokensForAVAX(tokenMiddle,1,path,address(this),block.timestamp);
        require(address(this).balance >= ethBefore, "ETH not enough"); 
    }


    // brouter:ETH->tonken,arouter:tonken->ETH
    function doitForB(uint256 ethAmount,address tokenAddress) public  onlyOwner{
        uint256 ethBefore = address(this).balance;
        address[] memory data = new address[](2);
        data[0] = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
        data[1] = address(tokenAddress);
        BRouter(bRouter).swapExactAVAXForTokens.value(ethAmount)(0,data,address(this),block.timestamp);
        uint256 tokenMiddle = ERC20(tokenAddress).balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = address(tokenAddress);
        path[1] = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
        ARouter(aRouter).swapExactTokensForAVAX(tokenMiddle,1,path,address(this),block.timestamp);
        require(address(this).balance >= ethBefore, "ETH not enough"); 
    }
