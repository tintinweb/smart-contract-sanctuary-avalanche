...
contract MansionHelper is Ownable, Manageable {
   ...
   function setNodeRatios(uint16 _poolFee, uint16 _liquidityFee) external onlyOwner {
    ...
    function claimUtility(uint64[] calldata _nodes, address whereTo, uint256 neededAmount, address excessAmountReceiver, address nodeOwner) onlyManager public {
        uint256 rewardAmount = 0;
        for (uint256 i = 0; i < _nodes.length; i++) {
            rewardAmount = rewardAmount + manager.claim(nodeOwner, _nodes[i]);
        }
