...
/*
    ...
    |    | ____ |  | |  | ___.__. \_   _____/______   ____   ____  
    |    |/ __ \|  | |  |<   |  |  |    __) \_  __ \_/ __ \ /    \ 
/\__|    \  ___/|  |_|  |_\___  |  |     \   |  | \/\  ___/|   |  \
\________|\___  >____/____/ ____|  \___  /   |__|    \___  >___|  /
              \/          \/           \/                \/     \/ 
*/
...
contract JellyFren is ERC721URIStorage, Ownable, Pausable
{
  ...
  uint256 public maxMintable = (42);//no more than this many nfts can be minted (about the current supply of egg tokens
  ...
  constructor() ERC721("Jelly Fren", "JELLYFREN") {
    baseURI = "ipfs://QmbHEeX6ohzLEq7Qmi6EPp4HtfsVMMzFbEarz5Z3qM7aod/";//add in a starter ipfs location info for the json metadata test 
  }//End of constructor
...
//Added in pausable functions from Openzepplin contract wizard
  ...
  function AirdropOne(address recipient) external onlyOwner{
    _privateMint(recipient);
  }

  function AidropArray(address[] memory addressList) external onlyOwner{ 
     for (uint i = 0; i < addressList.length; i++) 
       {
       _privateMint(addressList[i]);
       }
  }
  ...
  function _privateMint(address recipient) private { //nft mint hashlips inspired
    require(_tokenIds.current() <= maxMintable, "Too many NFTs have been minted, max mintable amount reached");
    _tokenIds.increment();
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, Strings.toString(tokenId), baseExtension)) : "";//base uri plus token id plus extention is the token uri
  }
