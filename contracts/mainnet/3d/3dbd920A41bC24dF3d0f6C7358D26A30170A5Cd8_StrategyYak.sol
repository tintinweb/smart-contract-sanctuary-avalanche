...
contract StrategyYak is BaseFarmStrategy {
    ...
    function _deposit(
        ...
        if (
            depositToken != USDC && depositToken != USDT && depositToken != IERC20(baseToken)
        ) revert BadToken();
    ...
    function quotePotentialWithdraw(
        ...
        if (address(want) != baseToken) {
            amountOut += joeV1Router.getAmountsOut(amountWant, routes[want][IERC20(baseToken)])[routes[want][IERC20(baseToken)].length - 1];
        } else {
