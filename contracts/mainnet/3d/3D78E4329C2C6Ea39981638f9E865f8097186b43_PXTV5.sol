...
contract PXTV5 is ERC20Upgradeable, IBridgeToken {
    // STRINGS
        ...
        returns (uint256)
    {
        ...
        if (antiWhaleUsdtLimit1 > 0) {
            //uint256 usdtDecimals = ERC20(usdtAddress).decimals();
            uint256 usdtAmount = getAmountOutUSD(amount)[2];

            /*
            if (usdtDecimals != decimals()) {
                usdtAmount = usdtAmount*usdtDecimals;
            }
            */
    ...
    function getAmountOutUSD(uint256 _amount)
        public
        view
