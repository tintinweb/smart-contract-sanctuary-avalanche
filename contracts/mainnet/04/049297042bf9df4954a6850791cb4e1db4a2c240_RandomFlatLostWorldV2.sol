...
// File contracts/storage/VariationLostWorldBaseStorage.sol
...
/**
 * @title VariationLostWorldBaseStorage
 * @author 0xLostArchitect
 *
 * @notice Storage for any VariationLostWorldBase
 */
abstract contract VariationLostWorldBaseStorage {
    /* CONSTANTS */
    /**
     * @dev Address that will select which variation a tokenId belongs to
     */
    IVariationSelector internal _variationSelector;
...
// File contracts/ERC721/LostWorlds/bases/VariationLostWorldBaseV2.sol
...
/**
 * @title VariationLostWorldBaseV2
 * @author 0xLostArchitect
 *
 ...
 */
abstract contract VariationLostWorldBaseV2 is
    VariationLostWorldBaseStorage,
    LostWorldBaseV2
{
    /* Libraries */
     ...
     */
    function __VariationLostWorldBase_init(
        InitializationParams calldata initializationParams_,
        IVariationSelector variationSelector_,
        Variation[] calldata variations_
    ) internal initializer {
        // Call super
        __LostWorldBase_init(initializationParams_);
        _variationSelector = variationSelector_;
    ...
    function _dataURI(uint256 tokenId)
        ...
        returns (bytes memory)
    {
        uint256 variationIndex = _variationSelector.selectVariation(
            _variations,
            tokenId
        );
        string memory data = _variations[variationIndex].data;
        return
            abi.encodePacked(
                '{"tokenId":',
                ...
                bytes(_tokenMintContext[tokenId]).length > 0
                    ...
                    : "",
                bytes(data).length > 0
                    ? string(abi.encodePacked(data, ","))
                    : "",
                '"image":"',
                _imageURI,
                StringsUpgradeable.toString(variationIndex),
                '"}'
            );
    }
}
 ...
 */
abstract contract RandomLostWorldStorage {
    /**
...
// File contracts/ERC721/LostWorlds/RandomFlatLostWorld/RandomFlatLostWorldV2.sol
...
/**
 * @title RandomFlatLostWorldV2
 * @author 0xLostArchitect
 *
 * @notice This contract implements a LostWorld with flat pricing and random minting
 *
 * @dev As this contract inherits from `FlatLostWorldBase`,
 * @dev the only thing needed to be implemented is the id selection mechanism, which is handled by the `AlphaBitmapHolder`
 *
 * Inherits:
 * - RandomLostWorldStorage - Use `storage` pattern to put all structs, state variables, and events elsewhere for upgradeability
 * - FlatLostWorldBase - To implement curved pricing on a LostWorld
 * - VariationLostWorldBaseV2 - To implement curved pricing on a LostWorld

 */
contract RandomFlatLostWorldV2 is
    RandomLostWorldStorage,
    FlatLostWorldBase,
    VariationLostWorldBaseV2
{
    /**
     * @dev Event emitted upon Initialization
     */
    event RandomFlatLostWorldInitialized(
    ...
    /**
     * @dev Initialization function for any RandomFlatLostWorldV2
     *
    ...
    function initialize(
        ...
        // Call super to set initialization params, variations, and base price
        __VariationLostWorldBase_init(
        ...
        emit RandomFlatLostWorldInitialized(
            initializationParams_,
            variationSelector_,
            variations_,
            price_,
    ...
    /**
     * @notice See {FlatLostWorldBase-getCostToMint}
     */
    function getCostToMint(uint256 v)
        public
        view
        virtual
     ...
     * Requirements:
     *
     * - enough tokens to mint given how many desired
     */
    ...
    /**
     * @dev Helper function to get an available ID
     *
     * @param seed - Use some seed to modulo the remaining amount
     * @return uint256 - An available id
     */
