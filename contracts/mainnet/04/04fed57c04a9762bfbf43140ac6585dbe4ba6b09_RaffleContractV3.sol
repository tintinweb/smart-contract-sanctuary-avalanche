...
contract RaffleContractV3 is Ownable, IERC721Receiver, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Address for address;
    using SafeMath for uint256;

    address private USDC;
    address private VALI;
    address private NFT;
    uint256 public withdrawableUSDCPool;
    uint256 public withdrawableVALIPool;
    uint256 public fundingPoolUSDC;
    uint256 public fundingPoolVALI;
    ...
    uint256 private minimumUSDCBalanceToWithdraw = 10000000; // 10 USDC
    uint256 private minimumVALIBalanceToWithdraw = 1000000000000000000; // 1 VALI

    address private addressD;
    address private addressP;
    address private addressT;

    uint128 private DValue = 150;
    uint128 private PValue = 50;
    ...
    function setMinimumUSDCBalanceToWithdraw(uint256 value) external onlyOwner {
        minimumUSDCBalanceToWithdraw = value;
    }

    function setMinimumVALIBalanceToWithdraw(uint256 value) external onlyOwner {
        minimumVALIBalanceToWithdraw = value;
    }
    ...
    function createRaffleVALI(
        ...
        require(
            IERC20(VALI).allowance(owner(), address(this)) >=
                rafflePrizeAmount_,
            "Contract not approved"
        );
        require(
            IERC20(VALI).balanceOf(owner()) >= rafflePrizeAmount_,
            "Not enough ALT tokens"
        );
        IERC20(VALI).transferFrom(owner(), address(this), rafflePrizeAmount_);

        _raffleId.increment();
        raffles[_raffleId.current()] = Raffle(
            _raffleId.current(),
            rafflePrizeAmount_,
            PrizeTypes.AltTokens,
            price_,
            totalTickets_,
            ticketLimit_,
            minimumTickets_,
    ...
    function cancelRaffle(uint256 id) public onlyAdmin {
        require(
            id <= _raffleId.current() && id > 0,
            "Raffle id does not exist"
        );
        ...
        } else if (raffle.prizeType == PrizeTypes.AltTokens) {
            IERC20(VALI).transfer(owner(), raffle.rafflePrize);
        } else {
            IERC721(NFT).safeTransferFrom(
                address(this),
                owner(),
    ...
    function purchaseTicket(uint256 id, uint256 quantity) public {
        ...
        address token = USDC;
        if (raffle.prizeType == PrizeTypes.AltTokens) {
            token = VALI;
        }

        require(
            IERC20(token).allowance(msg.sender, address(this)) >= totalPrice,
            "Contract not approved"
        );
        require(
            IERC20(token).balanceOf(msg.sender) >= totalPrice,
            "Not enough funds"
        );
    ...
    function selectRandomWinner(uint256 id) external onlyAdmin {
        require(
            id <= _raffleId.current() && id > 0,
            "Raffle id does not exist"
        );
        ...
        if ((totalTicketSoldOfId * raffle.price) > raffle.rafflePrize) {
            if (raffle.prizeType == PrizeTypes.AltTokens) {
                withdrawableVALIPool +=
                    (totalTicketSoldOfId * raffle.price) -
                    raffle.rafflePrize;
                fundingPoolVALI += raffle.rafflePrize;
            } else {
                withdrawableUSDCPool +=
                    (totalTicketSoldOfId * raffle.price) -
                    raffle.rafflePrize;
                fundingPoolUSDC += raffle.rafflePrize;
            }
        } else {
            if (raffle.prizeType == PrizeTypes.AltTokens) {
                fundingPoolVALI += (totalTicketSoldOfId * raffle.price);
            } else {
                fundingPoolUSDC += (totalTicketSoldOfId * raffle.price);
            }
        }
    ...
    function selectRandomWinnerForEachActiveRaffles()
        external
        nonReentrant
        onlyAdmin
    {
        ...
        for (uint256 i = 0; i < activeRaffles.length; i++) {
            uint256 id = activeRaffles[i].raffleId;
            if (id <= _raffleId.current() && id > 0) {
                Raffle storage raffle = raffles[id];
                if (
                    raffle.winner == address(0) &&
                    block.timestamp >= raffle.endIn &&
                    totalTicketSold[id] >= raffle.mininumTickets
                ) {
                    uint256 totalTicketSoldOfId = totalTicketSold[id];
                    if (
                        ...
                        if (raffle.prizeType == PrizeTypes.AltTokens) {
                            fundingPoolVALI += (totalTicketSoldOfId *
                                raffle.price);
                        } else {
                            fundingPoolUSDC += (totalTicketSoldOfId *
                                raffle.price);
                        }
                    }
                    uint256 randomNumber = random(totalTicketSoldOfId);
    ...
    function claim(uint256 id) public nonReentrant {
        ...
        if (raffle.prizeType == PrizeTypes.Tokens) {
            ...
            IERC20(VALI).transfer(msg.sender, raffle.rafflePrize);
        } else {
            IERC721(NFT).safeTransferFrom(
                address(this),
                msg.sender,
    ...
    function claimFees(uint256 id) public nonReentrant {
        ...
        IERC20(token).transfer(msg.sender, feesToReturn);
    }
    ...
    function withdrawUSDC() external onlyOwner {
        require(
            (withdrawableUSDCPool + fundingPoolUSDC) > 0,
            "USDC Pool is empty"
        );
        uint256 withdrawAmount = withdrawableUSDCPool + fundingPoolUSDC;
        withdrawableUSDCPool = 0;
        fundingPoolUSDC = 0;
        IERC20(USDC).transfer(owner(), withdrawAmount);
    }

    function withdrawVALI() external onlyOwner {
        require(
            (withdrawableVALIPool + fundingPoolVALI) > 0,
            "VALI Pool is empty"
        );
        uint256 withdrawAmount = withdrawableVALIPool + fundingPoolVALI;
        withdrawableVALIPool = 0;
        fundingPoolVALI = 0;
        IERC20(VALI).transfer(owner(), withdrawAmount);
    }

    function withdrawAndDistribute() external nonReentrant onlyAdmin {
        require(
            (withdrawableUSDCPool +
                withdrawableVALIPool +
                fundingPoolUSDC +
        ...
        require(
            DValue + PValue > 0 && DValue + PValue <= 1000,
            "Incorrect values"
        );
        if (fundingPoolUSDC > 0) {
            uint256 returnFundingAmount = fundingPoolUSDC;
            fundingPoolUSDC = 0;
            IERC20(USDC).transfer(owner(), returnFundingAmount);
        }

        if (fundingPoolVALI > 0) {
            uint256 returnFundingAmount = fundingPoolVALI;
            fundingPoolVALI = 0;
            IERC20(VALI).transfer(owner(), returnFundingAmount);
        }

        if (
            withdrawableUSDCPool > 0 &&
            withdrawableUSDCPool >= minimumUSDCBalanceToWithdraw
        ) {
            uint256 withdrawAmount = withdrawableUSDCPool;
            withdrawableUSDCPool = 0;
            uint256 usdAmountD = withdrawAmount.mul(DValue).div(1000);
            uint256 usdAmountP = withdrawAmount.mul(PValue).div(1000);
            uint256 usdAmountT = withdrawAmount.sub(usdAmountD).sub(usdAmountP);

            IERC20(USDC).transfer(addressD, usdAmountD);
            IERC20(USDC).transfer(addressP, usdAmountP);
            IERC20(USDC).transfer(addressT, usdAmountT);
        }

        if (
            withdrawableVALIPool > 0 &&
            withdrawableVALIPool >= minimumVALIBalanceToWithdraw
        ) {
            uint256 valiAmountT = withdrawableVALIPool;
            withdrawableVALIPool = 0;
