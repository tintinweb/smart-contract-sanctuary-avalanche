...
contract Ownable {
    ...
    modifier onlyRebaser() {
        require(isRebaser() || isOwner(), 'not auth');
        _;
    }
    ...
    function isRebaser() public view returns (bool) {
        return msg.sender == _rebaser;
    }
...
contract Kami is ERC20Detailed, Ownable, MinterRole {
    ...
    ) ERC20Detailed("Kami", "$KAMI", uint8(DECIMALS)) {
    ...
    function simpleRebase() external onlyRebaser {
        if(shouldRebase()){
            _rebase();
        }
    }
