...
contract Head is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 HEAD-WAVAX LP -> HEAD pool
    // uint256 public constant INITIAL_HEAD_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the HEAD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HEAD Token", "HEAD") {
        // Mints 1 HEAD to contract creator for initial pool setup
    ...
    function _getHeadPrice() internal view returns (uint256 _headPrice) {
        try IOracle(headOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Head: failed to fetch HEAD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _headPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_headPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHeadOracle(address _headOracle) public onlyOperatorOrTaxOffice {
        require(_headOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HEAD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HEAD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHeadPrice = _getHeadPrice();
            currentTaxRate = _updateTaxRate(currentHeadPrice);
            if (currentHeadPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_headPool != address(0), "!_headPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_headPool, INITIAL_HEAD_POOL_DISTRIBUTION);
