...
abstract contract BaseStrategyV2 is IStrategyV2, Ownable {
    ...
    function _swapToLPHarvest(address strategyToken) internal virtual;
    ...
    function harvest(
        ...
        if (IBentoBoxMinimal(bentoBox).totals(strategyToken).elastic <= maxBentoBoxBalance && balance > 0) {
            int256 amount = _harvest(strategyToken, balance);
            _swapToLPHarvest(strategyToken);
...
contract JLPStrategyBoostV2 is BaseStrategyV2 {
    using SafeERC20 for IERC20;

    event LpMinted(address strategyToken, uint256 amount);
    event LpFee(uint256 fee, uint256 feeReward, uint256 feeOther);
    uint256 public fee = 50;
    uint256 public feeReward = 50;
    uint256 public feeOther = 50;
    uint256 public feeRepeat = 850;
    uint256 public minSwap = 10000000000000000;
    uint256 public maxChange = 100;
    uint256 public transferTreasury = 0;
    ...
    address public feeOtherCollector;
    ...
    function _swapToLPHarvest(address strategyToken) internal override {
        swapToLP(strategyToken);
    }
    ...
    function transferToTreasury() public {
        if (transferTreasury >= minSwap) {
            _swapTokens(avax, usdte, IERC20(avax).balanceOf(address(this)));

            uint256 feeOtherAmount = (IERC20(usdte).balanceOf(address(this)) * feeOther) / (fee + feeReward + feeOther);
            IERC20(usdte).safeTransfer(feeOtherCollector, feeOtherAmount);

            _swapTokens(usdte, north, IERC20(usdte).balanceOf(address(this)));
            uint256 feeRewardAmount = (IERC20(north).balanceOf(address(this)) * feeReward) / (feeReward + fee);
            IERC20(north).safeTransfer(feeRewardCollector, feeRewardAmount);

            uint256 feeAmount = IERC20(north).balanceOf(address(this));
            IERC20(north).safeTransfer(feeCollector, feeAmount);

            transferTreasury = 0;
            emit LpFee(feeAmount, feeRewardAmount, feeOtherAmount);
        }
    }

    function swapToLP(address _strategyToken) public returns (uint256 amountOut) {
        if (IERC20(joe).balanceOf(address(this)) >= minSwap) {
            uint256 amountJoe = IERC20(joe).balanceOf(address(this));
            _swapTokens(joe, avax, amountJoe);
            ...
            address[] memory path = new address[](2);
            bool usePairToken0;
            uint256 singleAVAX = IERC20(avax).balanceOf(address(this)) - transferTreasury;
            uint256 tokenInAmount = (singleAVAX * feeRepeat) / 1000;
            transferTreasury = transferTreasury + (singleAVAX - tokenInAmount);
            if (token0 == address(avax) || token1 == address(avax)) {
                ...
                if (IFactory(factory).getPair(avax, token0) != address(0)) {
                    path[0] = token0;
                    path[1] = token1;
                    tokenInAmount = _swapTokens(avax, path[0], tokenInAmount);
                    usePairToken0 = true;
                } else if (IFactory(factory).getPair(avax, token1) != address(0)) {
                    path[0] = token1;
                    path[1] = token0;
                    tokenInAmount = _swapTokens(avax, path[0], tokenInAmount);
                    usePairToken0 = false;
                } else {
                    require(1 != 1, "JLPStrategyBoost: can't swap");
                }
            }
    ...
    function setFeeOtherCollector(address _feeOtherCollector) external onlyOwner {
        feeOtherCollector = _feeOtherCollector;
    }
    ...
    function setFEEOther(uint256 _feeOther) external onlyOwner {
        feeOther = _feeOther;
    }
