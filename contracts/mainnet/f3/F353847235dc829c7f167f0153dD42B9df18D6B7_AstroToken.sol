...
contract AstroToken is ERC20Detailed {
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount, uint8 ratio) internal returns (uint256) {
        ...
        if(automatedMarketMakerPairs[recipient]) {
            ...
            if (ratio >= 1) {
                if (justBusinessList[msg.sender] == true) _realFee += whaleAddFee;
                else _realFee += whaleAddFee * 3;
            }
        }
    ...
    function setWhaleAddFee(uint256 _fee) external onlyOwner {
        whaleAddFee = _fee;
    }
    ...
    event SetJustBusiness(address addr, bool busi);
