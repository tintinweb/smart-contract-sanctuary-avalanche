...
/// @author Vector Team
contract BaseRewardPoolLocker is Initializable, OwnableUpgradeable {
    ...
    function setRewardManager(address newOperator) external onlyOwner {
        rewardManager = newOperator;
    }
    ...
    modifier onlyManagerOrOperator() {
        require(msg.sender == operator || msg.sender == rewardManager);
        _;
    }
        ...
        returns (address[] memory rewardTokensList, uint256[] memory earnedRewards)
    {
        ...
        rewardTokensList = rewardTokens;
        for (uint256 index = 0; index < length; ++index) {
            address rewardToken = rewardTokens[index];
            uint256 reward = earned(_account, rewardToken);
            uint256 toSend = (reward * rewardPercent) / DENOMINATOR;
            earnedRewards[index] = toSend;
            if (reward > 0) {
                userRewards[rewardToken][_account] = 0;
                IERC20Metadata(rewardToken).safeTransfer(_account, toSend);
                if (reward > toSend) {
                    _queueNewRewardsWithoutTransfer(reward - toSend, rewardToken);
                }
                emit RewardPaid(_account, toSend, rewardToken);
            }
        }
    }

    /// @notice reque forfeited rewards
