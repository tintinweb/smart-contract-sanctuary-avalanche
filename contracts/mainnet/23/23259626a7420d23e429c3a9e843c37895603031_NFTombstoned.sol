...
// File: NFTombstoned.sol




// NFTombstoned by The Spot Devs
...
contract NFTombstoned is ERC721URIStorage, IERC721Receiver, ReentrancyGuard,  Ownable {
    using Strings for uint256;
    ...
    address private NFTombstoneAddress = 0xe3525413c2a15daec57C92234361934f510356b8;
    ...
    IERC721 NFTombstone = IERC721(NFTombstoneAddress);
    IERC721 spot = IERC721(spotAddress);

    Counters.Counter private supply;

    constructor() ERC721("NFTombstoned", "STONED") {
    }

    mapping(uint256 => string) private _tokenURIs;

	function mint(uint256 nfTombstoneId, string memory uri, address _to) public payable nonReentrant {
        require(!paused, "Minting is paused");	
       	if (spot.balanceOf(msg.sender) == 0) {
		require(msg.value >= cost, "Insufficient funds");
		}
	

		_mintLoop(nfTombstoneId, uri, _msgSender(), _to);
	}

    function _mintLoop(uint256 nfTombstoneId, string memory uri, address to, address _to) internal {
        NFTombstone.safeTransferFrom(to, address(this), nfTombstoneId);
    ...
    function withdrawERC721(address _nftAddress, uint _tokenID) public payable onlyOwner {
     ...
     function onERC721Received(address, address, uint256, bytes calldata) external view override returns (bytes4) {
        require(msg.sender == NFTombstoneAddress, "Only NFTombstones!!");        
