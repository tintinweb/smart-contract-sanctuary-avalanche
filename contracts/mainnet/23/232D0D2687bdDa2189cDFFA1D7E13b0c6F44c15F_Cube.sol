...
// File contracts/library/ERC20Detailed.sol
...
// File contracts/library/IBalanceOfCube.sol
...
interface IBalanceOfCube {
    function balanceOfCube(address _address) external view returns (uint256);
}


// File contracts/library/IJoeFactory.sol
...
// File contracts/library/IJoePair.sol
...
// File contracts/library/IJoeRouter01.sol
...
// File contracts/library/InterfaceLP.sol
...
// File contracts/library/IPublicBalance.sol
...
// File contracts/Cube.sol




contract Cube is ERC20Detailed, Ownable {
    ...
    function initialize( 
        ...
        _gonBalances[_treasuryManager] = TOTAL_GONS;
        ...
        emit Transfer(address(0x0), _treasuryManager, _totalSupply); 
    ...
    function setPairConfig(
         address _router,
        address _stableCoin )  external onlyOwner {
        ...
        address pairStableCoin = IJoeFactory(router.factory()).createPair(
            address(this),
            stableCoin
        );
        ...
        _allowedFragments[address(this)][pair] = type(uint128).max;
        _allowedFragments[address(this)][address(this)] = type(uint128).max;
        _allowedFragments[address(this)][pairStableCoin] = type(uint128).max;

        setAutomatedMarketMakerPair(pair, true);
        setAutomatedMarketMakerPair(pairStableCoin, true); 

        IERC20(stableCoin).approve(address(router), type(uint128).max);
        IERC20(stableCoin).approve(address(pairStableCoin), type(uint128).max);
        IERC20(stableCoin).approve(address(this), type(uint128).max);

        

    }

    constructor() ERC20Detailed("Cube Finance", "Cube", uint8(DECIMALS)) {
    ...
    function getUserTotalOnDifferentContractsCube(address sender)
    public
    view
    returns (uint256)
    {
    ...
    //this function iterates through all other contracts that are being part of the Cube ecosystem
    //we add a new contract like wCube or sCube, whales could technically abuse this
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < subContracts.length; i++) {
            userTotal += IBalanceOfCube(subContracts[i]).balanceOfCube(
                sender
            );
        }
    ...
    function getTokensInLPCirculation() public view returns (uint256) {
        ...
        address token0;
        address token1;
        IJoePair iDexFeeCalculator;
        uint256 LPTotal;

        for (uint256 i = 0; i < _markerPairs.length; i++) {
            iDexFeeCalculator = IJoePair(_markerPairs[i]);
    ...
    )
    {
        ...
        iDexFeeCalculator = IJoePair(_markerPairs[_id]);
        (reserve0, reserve1, blockTimestampLast) = iDexFeeCalculator
        .getReserves();
    ...
    returns (uint256)
    {
        ...
        uint256 userTotal = getUserTotalOnDifferentContractsCube(_address);
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 BUSDAmount) private {
        router.addLiquidityAVAX{value: BUSDAmount}(
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function _swapTokensForBUSD(uint256 tokenAmount, address receiver)
    private
    {
    ...
    function updateRebaseIndex(uint256 epoch) private {
        ...
        //update Index similarly to OHM, so a wrapped token created is possible (wCube)
    ...
    function setIsLiquidityInBUSD(bool _value) external onlyOwner {
        require(isLiquidityInBusd != _value, "Not changed");
        isLiquidityInBusd = _value;
        emit SetIsLiquidityInBUSD(_value, block.timestamp);
    }
