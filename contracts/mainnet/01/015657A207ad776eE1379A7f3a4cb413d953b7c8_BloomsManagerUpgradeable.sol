    ...
    NectarImplementationPointerUpgradeable
{
    ...
    uint256 public numberOfAutocompoundingBlooms;
    ...
    uint256[] private _bloomsTransferChoosed;
    ...
    function movingChoice(uint256 _bloomId, uint256 shouldMove) external {
        BloomEntity storage bloom = blooms[_bloomId];
        ...
        onlyApprovedOrOwnerOfBloom(_bloomId)
    {
        ...
        if (_lockPeriod == 21 days) {
            // Increase reward multiplier by 0.20%
            bloom.rewardMult += 20000;
        } else if (_lockPeriod == 13 days) {
            // Increase reward multiplier by 0.15%
            bloom.rewardMult += 15000;
        } else if (_lockPeriod == 27 days) {
    ...
    function AddNodeToAutoCompound(uint256[] memory Ids) external onlyOwner {
        for (uint256 index = 0; index < Ids.length; index++) {
            _bloomsClaimable.push(Ids[index]);
        }
    }
    ...
    function autoClaim(uint256 _numNodes) external onlyOwner {
        ...
        while (_numNodes > 0) {
            ...
            BloomEntity storage bloomy = blooms[bloomId];

            if (
                _checkTime(
                    bloomId,
                    bloom.lockedUntil,
                    bloom.lastProcessingTimestamp
                )
            ) {
                _removeNodeFromClaimable(lastUpdatedClaimIndexLocal);
                bloomy.owner = bloomNFT.ownerOfBloom(bloomId);
                bloom.owner = bloomNFT.ownerOfBloom(bloomId);
        ...
        onlyApprovedOrOwnerOfBloom(_bloomId)
    {
        ...
        _cashoutReward(
            amountToReward,
            emergencyFee,
            bloomNFT.ownerOfBloom(_bloomId)
        );
    ...
    //     require(block.timestamp >= bloom.lastProcessingTimestamp + 86400);
    ...
    function _cashoutReward(
        ...
        nectar.mintNectar(_to, amountToReward - feeAmount);
    }
        ...
        if (
            ...
            for (uint256 i = 0; i < tierSlope.length; i++) {
                if (_timesCompounded == tierSlope[i]) {
                    if (_prevMult > tierLevel[i]) {
                        uint256 bonus = _prevMult - tierLevel[i - 1];
                        return tierLevel[i] + bonus;
                    }
                    return tierLevel[i];
                }
            }
    ...
    function _checkTime(
        ...
        return
            lockedUntil != 0 &&
            block.timestamp >= lockedUntil + 86400 &&
            block.timestamp >= lastProcessingTimestamp + 86400;
    }
    ...
    function _resetRewardMultiplier(uint256 _bloomId) private {
        ...
        if (bloom.lockPeriod == 14 days) {
            multiplier = 15000;
        } else if (bloom.lockPeriod == 21 days) {
            multiplier = 20000;
        } else if (bloom.lockPeriod == 27 days) {
            multiplier = 25000;
        } else {
            multiplier = 0;
        }
    ...
    function getBloomsClaiming() external view returns (uint256[] memory) {
        return _bloomsClaimable;
    }
