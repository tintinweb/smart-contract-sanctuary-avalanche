...
contract InteropPeriphery is Variables {
    ...
    constructor(
        ...
        list = ListInterface(instaIndex.list());
    ...
    /**
     * @notice non dsa user will deposit ERC20 tokens to this contract
     * @dev we will use the dsaAddr to cast the spell
    ...
    function transfer (
        ...
        bytes4 interopFunc = bytes4(
            keccak256(
                "submitActionERC20(Position,string,uint256,uint256,bytes)"
            )
        );
    ...
    /**
     * @notice non dsa user will deposit ETH to this contract
     * @dev we wrap the ETH and calls deposit func
     * @param targetChainTokenAddress_ The `target` chain token Address
     * @param targetChainId_ The chainID of target chain
     */
