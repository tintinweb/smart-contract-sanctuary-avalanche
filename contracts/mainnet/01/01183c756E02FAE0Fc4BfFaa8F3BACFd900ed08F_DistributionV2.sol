...
contract DistributionV2 is OwnableUpgradeable, PausableUpgradeable, AdminUpgradeable {
    ...
    event MintTokens(address indexed receiver, uint indexed amount);
    ...
    */
    function initialize(
        IMintableOwnableERC20 token_,
        RewardEmission[] calldata rewardEmission_
    )
    initializer
    public
    {
    ...
    */
    function countRewardAmount(uint start_, uint end_) external view returns (uint) {
        ...
        */
        ...
        // Check 0
        if (start_ < rewardEmission[0].start && end_ > rewardEmission[0].start) { startEntry = true; }

        // Check 1
        if (start_ < rewardEmission[0].start && end_ < rewardEmission[0].start) { return 0; }

        // Check 2
        if (rewardEmission[rewardEmission.length - 1].end < start_) { return 0; } 

        for (uint i = 0; i < rewardEmission.length; i++) {
            // Check 2
    ...
    */
    function setPool(address poolAddress_, uint allocationPoint_) external onlyOwner {
    ...
    * @param poolAddress_ Pool contract address
    */
    function _calculatePoolAllocationReward(uint amount_, address poolAddress_) private view returns (uint) {
