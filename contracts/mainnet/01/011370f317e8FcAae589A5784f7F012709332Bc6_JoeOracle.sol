...
contract JoeOracle {
  ...
  constructor(address _factory, address _router, address _chainlinkOracle) {
    factory = _factory;
    router = _router;
  ...
  */
  function lpToken(
    ...
    return IJoeFactory(factory).getPair(_tokenA, _tokenB);
  }
  ...
  */
  ...
  /**
    ...
    * @param _tokenB Token B address
    * @param _pair LP token address
    * @return lpTokenValue Value of respective tokens; expressed in 1e6
  */
  function getLpTokenValue(
    ...
    uint256 priceA = IChainLinkOracle(chainlinkOracle).consult(_tokenA);
    uint256 priceB = IChainLinkOracle(chainlinkOracle).consult(_tokenB);
    uint256 lpFairValue = sqrtK * 2 * Math.sqrt(priceA) / 2**56 * Math.sqrt(priceB) / 2**56; // in 1e12
    uint256 lpTokenValue = _amount * lpFairValue / SAFE_MULTIPLIER / 1e6; // Divide by 1e6 to return in 1e6
  ...
  /**
    ...
    * @param _value Value of LP token, expressed in 1e6
    * @param _tokenA Token A address
    * @param _tokenB Token B address
    * @param _pair LP token address
  ...
  */
  function getLpTokenAmount(
    ...
    uint256 lpTokenAmount = _value * IJoePair(_pair).totalSupply() / lpTokenValue;
