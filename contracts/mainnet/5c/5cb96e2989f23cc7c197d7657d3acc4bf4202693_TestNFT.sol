...
interface ITestNFT {
    error TestNFT__ExceedLimit();
    error TestNFT__ZeroValue();
    error TestNFT__InsufficientBalance();
    ...
    function buy(address account, address paymentToken, uint256 quantity_) external payable;
}
    ...
    ReentrancyGuard
{
    ...
    /**
     * @dev Initializes the TestNFT NFT.
     ...
     */
    function buy(address account_, address paymentToken_, uint256 quantity_) external payable override nonReentrant {
        address sender = _msgSender();
        uint256 amount = _paymentAmount[paymentToken_];
        uint256 tokenId = _idCounter;

        unchecked {
            if (tokenId + quantity_ - 1 > MAX_SUPLLY) revert TestNFT__ExceedLimit();
        }

        uint256 total = amount * quantity_;

        if (total == 0) revert TestNFT__ZeroValue();

        if (paymentToken_ == address(0)) {
            if (msg.value < total) revert TestNFT__InsufficientBalance();
            SafeTransferLib.safeTransferETH(_recipient, total);
        } else {
            SafeTransferLib.safeTransferFrom(paymentToken_, sender, _recipient, total);
        }

        emit BatchMint(account_, tokenId, quantity_);
        _batchMint(account_, tokenId, quantity_);
    }
