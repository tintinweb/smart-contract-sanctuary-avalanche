...
contract PurpleJellyEyes is ERC721URIStorage, Ownable, Pausable
{
  ...
  uint256 public maxMintable = (21);//no more than this many nfts can be minted (about the current supply of egg tokens
  ...
  constructor() ERC721("Purple Jelly Eyes", "PurpleJellyEyes") {
    baseURI = "ipfs://QmSYfD2H85MqAvQ4KLuVgkzAhYmsbAKjSqbqvWVC7ryMja/";//add in ipfs location info for the json metadata test 
  }//End of constructor
...
///////////////////////////////////////NFT Aidrop CODE/////////////////////////////////////////////////////
  function AirdropToShortList16Superjellies() public onlyOwner {
    _privateMint(0x2573143b6be26d9513f989C0f2CA457B29168142); //we mint new token to address
    _privateMint(0x02Ce292223962F2D06a09CF7897e76E941DC8367); //we mint new token to address
    _privateMint(0x33E23501733243F18B3b2a8D1479c19119658908); //we mint new token to address
    _privateMint(0xb052c60A83Bd79170a3ca470e4b1618ebDF28a10); //we mint new token to address
    _privateMint(0x9CD4Ea50A769718a5481A04364AA7774f5ab7D40 ); //we mint new token to address

    _privateMint(0x86269fE8E5704825c3E5dAd4b678c5dB12182152); //we mint new token to address
    _privateMint(0x2B1855f3E0d453faF76c91EA8504239D3bbEDb61); //we mint new token to address
    _privateMint(0x436B10A32fa369bB636896Ef911CEaF05537B2f8); //we mint new token to address
    _privateMint(0xDB3E54ff11623F6e5D80c574cCFb5e3a81bB1AF4); //we mint new token to address
    _privateMint(0xB73D499f70fdc40d875A2D2e73dc133dE910CE32); //we mint new token to address

    _privateMint(0xB7c4FC8B2C2c8e500C1D6d65cAca61F01999990A); //we mint new token to address
    _privateMint(0x79c9aF4FD5De2e1106b031A9ED76Aca2a13cdf2b); //we mint new token to address
    _privateMint(0x5b7d83894777e830F75C2c3a823248D1e8a74787); //we mint new token to address
    _privateMint(0xECab6F71D28E8248b46c87E38ae1B1BFcDab6a9d); //we mint new token to address
    _privateMint(0xf8221Ccb35f0B838748E42209A40Ee937300d90a ); //we mint new token to address

    _privateMint(0xe3115272dDCfE726A63e2D9B963C7eA8314D0d04); //we mint new token to address
  }
