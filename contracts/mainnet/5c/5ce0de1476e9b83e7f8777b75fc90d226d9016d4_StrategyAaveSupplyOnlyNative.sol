...
// File contracts/interfaces/aave/IDataProvider.sol
...
// File contracts/interfaces/aave/IAaveV3Incentives.sol
...
// File contracts/strategies/Aave/StrategyAaveSupplyOnlyNative.sol



pragma solidity ^0.8.0;
contract StrategyAaveSupplyOnlyNative is StratManager, FeeManager {
    ...
    function chargeFees(uint256 wantHarvested, address callFeeRecipient) internal {
        uint256 feeBal = wantHarvested.mul(40).div(1000);

        uint256 callFeeAmount = feeBal.mul(callFee).div(MAX_FEE);
        IERC20(want).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 steakHutFeeAmount = feeBal.mul(steakHutFee).div(MAX_FEE);
        IERC20(want).safeTransfer(steakHutFeeRecipient, steakHutFeeAmount);

        uint256 strategistFeeAmount = feeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(want).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, steakHutFeeAmount, strategistFeeAmount);
    }
    ...
    // want reward amount for calling harvest
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(lendingPool, uint256(MAX_INT));
    }
