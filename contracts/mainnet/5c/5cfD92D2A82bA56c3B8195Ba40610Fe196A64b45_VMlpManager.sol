...
contract VMlpManager is ReentrancyGuard, Governable, IGlpManager {
    ...
    function setCooldownDuration(uint256 _cooldownDuration) override external onlyGov {
        require(_cooldownDuration <= MAX_COOLDOWN_DURATION, "VMlpManager: invalid _cooldownDuration");
        cooldownDuration = _cooldownDuration;
    }
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("VMlpManager: action not enabled"); }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) private returns (uint256) {
        require(_amount > 0, "VMlpManager: invalid _amount");
        ...
        require(usdgAmount >= _minUsdg, "VMlpManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(glpSupply).div(aumInUsdg);
        require(mintAmount >= _minGlp, "VMlpManager: insufficient GLP output");
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        require(_glpAmount > 0, "VMlpManager: invalid _glpAmount");
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "VMlpManager: cooldown duration not yet passed");
        ...
        require(amountOut >= _minOut, "VMlpManager: insufficient output");
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "VMlpManager: forbidden");
    }
}
