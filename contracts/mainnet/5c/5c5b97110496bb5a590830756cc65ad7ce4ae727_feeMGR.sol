...
contract feeMGR is Ownable{
    ...
    function getAccounts(uint _x,uint _y) external {
    	    address _account;
	    for(uint256 i = _x;i<_y;i++){
	    _account = protoMgr.getProtoAddress(i);
		if(nebuLib.addressInList(Accounts,_account) == false){
			...
			_fees.protos = protoMgr.getProtoStarsLength(_account);
	    		...
	    		for(uint256 j = 0;j<_fees.protos;j++){
	    			(string memory _name,uint256 _collapseDate,uint256 _nextDue,uint256 _futureFees) =   prevFee.viewFees(_account,j);
