...
contract AvaxZapper is Ownable {
    using SafeERC20 for IERC20;

    /* ========== CONSTANT VARIABLES ========== */

    address public constant WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    IJoeFactory public constant joeFactory =
    ...
    function routePair(address _address) public view returns (address) {
        ...
        returns (uint256 avaxAmount)
    {
        ...
        for (uint256 i; i < length; i++) {
            if (inTokens[i] != address(0)) {
                avaxAmount += previewAmount(inTokens[i], amounts[i]);
            }
        }
    }
    ...
    function zapInToken(
        ...
        if (amount > 0) {
            ...
            avaxAmount = _swapTokenForAVAX(_from, amount, receiver);
        }
    }
    ...
    function _findRouteToAvax(address token) private view returns (address[] memory) {
        address[] memory path;
