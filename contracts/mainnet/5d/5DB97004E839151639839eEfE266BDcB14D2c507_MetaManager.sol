...
interface AmsHelper {
  ...
  function claimRewardsForCompoundMeta(address sender, uint256 nodePrice ) external returns (uint256);
}

interface MetaHelperV1 {
   function seeNodeClaim(uint256 _id) external view returns (uint64);
}

contract MetaManager is Ownable, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => Meta) private _nodes;
    ...
    uint256 public nodeAscendTokenPrice = 1000;
    uint256 public nodeAscendMembershipPrice = 50;
    uint256 public nodeInfinitePrice = 1;

    uint256 private nodeCounter = 1;
    uint public maxMetaWallet = 3;
    uint public maxMetaGlobal = 250;

    IERC20 public ASCEND;
    IAscendMembershipManager public amsManager;
    AmsHelper public amsHelper;

    MetaManagerV1 public metaManagerV1;
    MetaHelperV1 public metaHelperV1;

    bool public transferIsEnabled = true;

    bool public enableClaims = true; //upgradeable
    ...
    BoostersParameters public boostersParameters = BoostersParameters({
        presaleBooster: 41250, // 15%
        founderL1Booster: 27500, // 10%
        founderL2Booster: 41250, // 15%
        founderL3Booster: 55000, // 20%
        metaBooster: 0
    });


    event CreateMeta(address from, uint id);
    event RemoveMeta(address from, uint id);
    event ClaimMeta(address account, uint rewardNode);
    event CreateMetaWithRewards(address sender, uint256 rewardAmount);

    constructor(address _ASCEND_TOKEN, address _amsManager, address _amsHelper)  {
    ...
    function setMetaManagerV1(address _metaManagerV1 ) external onlyOwner {
        metaManagerV1 = MetaManagerV1(_metaManagerV1);
    }

    function setMetaHelperV1(address _metaHelperV1 ) external onlyOwner {
        metaHelperV1 = MetaHelperV1(_metaHelperV1);
    }
    ...
    function updateMeta(uint256 id, string calldata metadata) external {
        Meta storage meta = _nodes[id];
        meta.metadata = metadata;
    }

    function getMetas(uint256 _id) public view onlyIfExists(_id) returns (Meta memory) {
        return _nodes[_id];
    }
    ...
    function  getMetasOf(address _account) public view returns (uint256[] memory) {
    ...
    function updateInfinitePrice(uint256 _price) external {
        nodeInfinitePrice = _price;
    }
    ...
    function updateAscendPrice (uint256 _nodeAscendPrice) external onlyOwner {
        nodeAscendTokenPrice = _nodeAscendPrice;
    }

    function updateAmsPrice (uint256 _amsPrice) external onlyOwner {
        nodeAscendMembershipPrice = _amsPrice;
    }
    ...
    function _addMeta( string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to) onlyOwner external {
        uint256 nodeId = nodeCounter;
        _createMeta(nodeId, _name, _mint, _claim, _metadata, _to);
        nodeCounter += 1;
    }

    function updateAmsManager (IAscendMembershipManager _amsManager) external onlyOwner {
        amsManager = IAscendMembershipManager(_amsManager);
    }

    function setMaxMetaWallet (uint256 _max) external onlyOwner {
        maxMetaWallet = _max;
    }

    function setmaxMetaGlobal (uint256 _max) external onlyOwner {
        maxMetaGlobal = _max;
    }
    ...
    function getAddressRewards(address account) external view returns (uint) {
          ...
          uint256[] memory userMemberships = getMetasOf(account);
          uint interval = 0;
          for (uint256 i = 0; i < userMemberships.length; i++) {
              Meta memory _node = _nodes[userMemberships[i]];
    ...
    function getReward(address from) public view returns(uint256) {
        uint rewardNode = rewardsParameters.reward + getUserBooster(from);
        return rewardNode;
    }
    ...
    function claim(address account, uint256 _id) external onlyIfExists(_id) returns (uint) {
      ...
      _node.claim =_node.claim + (interval*rewardsParameters.claimTime);
      _nodes[_id] = _node;
      emit ClaimMeta( account, rewardNode);
      return rewardNode;
    }


    function _createMeta(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to) internal {
        require(!_exists(_id), "MANAGER: Meta already exist");
        _nodes[_id] = Meta({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        Meta memory _node = _nodes[uint64(tokenId)];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId ) external override {
        if (transferIsEnabled == true){
            ...
            require (to == address(0) || to == address(deadAddress), "Meta Transfers are not allowed");
            safeTransferFrom(from, to, tokenId, "");
        }
    }
    ...
    function createNode(address account, string memory nodeName) internal {
        ...
        uint256 nodeId = nodeCounter;
        _createMeta(nodeId, nodeName, uint64(block.timestamp), uint64(block.timestamp), "", account);
        nodeCounter += 1;
    }

    function migrateNodes(uint64[] memory userNodes) public {
      address sender = _msgSender();
      require(userNodes.length + balanceOf(sender) <= maxMetaWallet, "HELPER: Exceeds max Meta per wallet");
      uint64[] memory _claims = new uint64[](userNodes.length);
      for (uint256 i = 0; i < userNodes.length; i++) {
           require(sender ==  metaManagerV1.ownerOf(userNodes[i]), "You are not the owner of the Node" );
           uint64 nodeclaim = metaHelperV1.seeNodeClaim(userNodes[i]);
           if  (nodeclaim == 0){
             nodeclaim =  metaManagerV1.getMintOf(userNodes[i]);
           }
           _claims[i] = nodeclaim;
           metaManagerV1.transferFrom(_msgSender(), address(deadAddress), userNodes[i] );
      }
      migrateMetas(_claims, sender);
    }

    function migrateMetas( uint64[] memory _claims, address _to) internal {
        for (uint256 i = 0; i < _claims.length; i++) {
            uint256 nodeId = nodeCounter;
            _createMeta(nodeId, "Meta", _claims[i],  _claims[i], "", _to);
            nodeCounter += 1;
        }
    }

    function createMultipleNodeWithRewards(string memory _name, uint64[] memory ascendNodes, uint64[] memory userInfiniteNodes) public {
        ...
        require(nodeCounter <= maxMetaGlobal, "HELPER: Maximun number of Meta nodes reached");
        ...
        require(balanceOf(sender) + 1 <= maxMetaWallet, "HELPER: Exceeds max wallet amount");
        require(amsManager.balanceOf(sender) >= nodeAscendMembershipPrice, "HELPER: Ascend Membership balance too low for creation.");
        require(amsManager.infinite_membership().balanceOf(sender) >= nodeInfinitePrice, "HELPER: Infinite Membership balance too low for creation.");
        require(ascendNodes.length == nodeAscendMembershipPrice, "HELPER: Ascend Membership Nodes (Invalid amount)");
        require(userInfiniteNodes.length == nodeInfinitePrice, "HELPER: Infinite Membership Nodes (Invalid amount)");

        uint256 rewardAmount = amsHelper.claimRewardsForCompoundMeta( sender, nodePrice );

        for (uint256 i = 0; i < nodeAscendMembershipPrice; i++) {
            amsManager.transferFrom(_msgSender(), address(deadAddress), ascendNodes[i]);
        }
        for (uint256 i = 0; i < nodeInfinitePrice; i++) {
            amsManager.infinite_membership().transferFrom(_msgSender(), address(deadAddress), userInfiniteNodes[i]);
        }
        ...
        emit CreateMetaWithRewards(sender, rewardAmount);
        createNode(sender, _name);
    }

    function createNodeWithTokens(string memory _name, uint64[] memory ascendNodes, uint64[] memory userInfiniteNodes) public {
        require(bytes(_name).length > 0 && bytes(_name).length < 33, "HELPER: name size is invalid");
        address sender = _msgSender();
        require(sender != address(0), "HELPER:  Creation from the zero address");
        uint256 nodePrice = nodeAscendTokenPrice * 10 ** 18;
    ...
    function renameMeta(uint64 id, string memory newName) external {
    ...
    function transferFrom(address from, address to,uint256 tokenId) external override {
        ...
        if (transferIsEnabled == true){
            ...
            require (to == address(0) || to == deadAddress, "Meta Transfers are not allowed");
            _transfer(from, to, tokenId);
        }
    }
