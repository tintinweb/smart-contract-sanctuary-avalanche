...
import "./CompoundV2Module.sol";

/**
 * @author  HedgeFarm team
 * @title   Benqi Yield module
 * @dev     The Benqi module is reponsible for interacting with the Benqi lending protocol
 * @notice  Upgradability is needed because Benqi protocol is built with Proxy - it's implementation could be updated
 */
contract BenqiYieldModule is CompoundV2Module {
     ...
     * @param   _comptroller  Benqi Comptroller contract address
    ...
    function initialize(
        ...
        _initializeCompoundV2(_manager, _baseToken, _executionFee, _dex, _rewards, _comptroller, _cToken);
    }
}
...
contract CompoundV2Module is BaseModule, ExponentialNoError {
    ...
    function _initializeCompoundV2(
        address _manager,
        address _baseToken,
        uint256 _executionFee,
        address _dex,
    ...
    {
        require(amount > 0, "CompoundV2: withdraw amount cannot be zero");
        ...
        uint256 error = CErc20Interface(cToken).redeemUnderlying(amount);
    ...
    // Return the same amount as getBalance but with calculation
    function getBalanceCalculated() public view returns (uint256) {
    ...
    function _lpProfit(uint256 aum) internal {
        uint256 protocolBalance = getBalance();
        if(protocolBalance > aum) {
            uint256 expectedLpProfit = protocolBalance - aum;
            uint256 error = CErc20Interface(cToken).redeemUnderlying(expectedLpProfit);
            require(error == 0, "CompoundV2: withdraw error");
        }
    }

    function _rewardsProfit() internal {
        ...
        // Claim and swap protocol token rewards
        ...
        // Claim and swap native token rewards
        ComptrollerInterface(comptroller).claimReward(
            1,
            holder,
            market,
    ...
    function _setComptroller(address _comptroller) internal {
        require(
            ...
            "CompoundV2: cannot be the zero address"
        );
        comptroller = _comptroller;
    }

    function _setCToken(address _cToken) internal {
        require(_cToken != address(0), "CompoundV2: cannot be the zero address");
        require(baseToken != address(0), "CompoundV2: baseToken not initialized");
        cToken = _cToken;
        uint256 allowance = IERC20Upgradeable(baseToken).allowance(address(this), _cToken);
        if (allowance == 0) {
    ...
    function _enterMarket() internal {
        require(
            ...
            "CompoundV2: comptroller not initialized"
        );
        require(cToken != address(0), "CompoundV2: cToken not initialized");
