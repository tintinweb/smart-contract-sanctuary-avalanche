...
contract Hall is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 HALL-WETH LP -> HALL pool
    // uint256 public constant INITIAL_HALL_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the HALL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HALL", "HALL") {
        // Mints 1 HALL to contract creator for initial pool setup
    ...
    function _getHallPrice() internal view returns (uint256 _hallPrice) {
        try IOracle(hallOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Hall: failed to fetch HALL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _hallPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_hallPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHallOracle(address _hallOracle) public onlyOperatorOrTaxOffice {
        require(_hallOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HALL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HALL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHallPrice = _getHallPrice();
            currentTaxRate = _updateTaxRate(currentHallPrice);
            if (currentHallPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        // _mint(_cometPool, INITIAL_HALL_POOL_DISTRIBUTION);
