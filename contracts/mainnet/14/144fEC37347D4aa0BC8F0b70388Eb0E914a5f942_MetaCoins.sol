...
contract MetaCoins is ERC721URIStorage, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;
    Counters.Counter public _tokenIds;

   
    uint256 public _maxMintable = 50;
    ...
    ///////// WL & Price ////////
    uint256 public _price = 1.25 ether;
    uint256 public _price_WL = 1.25 ether;
    uint256 public _price_Public = 1.5 ether;
    mapping(address => bool) public whiteListed;
    mapping(uint256 => address) public burnedTokens;
    bool public isWL = true;

    //mint per wallet
    bool public isMaxPerWallet = false;
    uint256 public _maxMintPerWallet = 0;
    ...
    function splitFunds(uint256 mintValue) private {
        uint256 amount = (mintValue * 25) / 100;
        payable(walletFundsA).transfer(amount);
        payable(walletFundsB).transfer(amount);
        payable(walletFundsC).transfer(amount);
        payable(walletFundsD).transfer(amount);
    }
    ...
    /////// for WL ///////
    function setWLStatus(bool isWL_) external onlyOwner {
        isWL = isWL_;
    }
