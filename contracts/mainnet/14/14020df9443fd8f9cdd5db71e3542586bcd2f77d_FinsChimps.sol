...
// File contracts/abstracts/LzApp.sol
...
// File contracts/abstracts/NonblockingLzApp.sol
...
// File contracts/abstracts/Cloneable.sol
...
library Random {
    ...
    function randomize(uint256 value) internal view returns (uint256) {
        return _getEntropy(value, 0x0) % value;
    }
    ...
    function randomize(uint256 value, uint256 maximumValue) internal view returns (uint256) {
        return _getEntropy(value, 0x0) % maximumValue;
    }
    ...
    function randomizeWithEntropy(uint256 value, bytes32 entropy) internal view returns (uint256) {
        return _getEntropy(value, entropy) % value;
    }
    ...
    /**
     * @dev Generates simple on-chain entropy that is salted with the supplied entropy
     */
...
// File contracts/abstracts/URIMaskable.sol
...
// File contracts/abstracts/ClampedRandomizer.sol
...
// File contracts/interfaces/IERC721.Omnichain.sol

/**
 * @dev Interface of the ONFT Core standard
 */
interface IERC721Omnichain is IERC165 {
    /**
...
// File contracts/abstracts/ERC721.Omnichain.sol
...
// File contracts/FinsChimps/FinsChimps.sol

contract FinsChimps is ERC721Omnichain {
    using Address for address;
    using Strings for uint256;

    uint256 public MINT_START_TIME = type(uint256).max;
    uint256 constant MAX_SUPPLY = 117;

    modifier isOpen() {
        require(block.timestamp >= MINT_START_TIME);
        _;
    }

    constructor()
        ERC721("Fin's Chimps", "FCHIMP")
        ClampedRandomizer(117)
        LzApp(0xb6319cC6c8c27A8F5dAF0dD3DF91EA35C4720dd7)
    {
        _setDefaultRoyalty(0xE5CE15891e69A80d36A8946e3ad38163414BBF6b, 500);
    }


    function setOpenTime(uint256 time) public onlyOwner {
        MINT_START_TIME = time;
    }

    function mint(uint256 count) public payable isOpen {
        require(count == 1);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked("https://finschimps.pages.dev/schema/", tokenId.toString(), ".json"));
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (!to.isContract() && to != address(0) && to != address(1)) {
            require(balanceOf(to) + 1 <= 1);
        }
        super._beforeTokenTransfer(from, to, tokenId);
    }
}