    ...
    ClampedRandomizer
{
    ...
    uint256 public wlTimeastamp;

    constructor()
        ERC721("Rise of the Machines", "TRMNTR")
        ClampedRandomizer(_maxMintable)
    {
        _customBaseURI = "https://metacardsclub.io/rotm/json/";
        _wlPrice = 0;
        _price = 0.3 ether;
        _royaltyAmount = 500;
        wlTimeastamp = 1677787200;
        publicTimestamp = 1677873600;
        devAddress = 0xc0e12AF01B0bf0a4Cdc2667AbB353B7b3B0b8F42;
        artistAddress = 0xd8936E602E38dfEe5d6466865068b94b1943DEBF;
        royaltyAddress = 0xd8936E602E38dfEe5d6466865068b94b1943DEBF;
    }

    mapping(address => uint256) whitelistMints;

    function addWl(address[] calldata _addressToWhitelist, uint256 amount)
        external
        onlyOwner
    {
    ...
    function setPrice(uint256 priceInMilli) public onlyOwner {
        _price = priceInMilli * 0.001 ether;
    }

    function setWLTime(uint256 timestamp) public onlyOwner {
        wlTimeastamp = timestamp;
    }

    function setPublicTime(uint256 timestamp) public onlyOwner {
    ...
    function setRoyaltyAddress(address royaltyAddress_) public onlyOwner {
        royaltyAddress = royaltyAddress_;
    }

    function mint(uint256 quantity) public payable {
        require(
            totalSupply() + quantity <= _maxMintable,
            "Max 250 NFTs in existence"
        );
        payable(devAddress).transfer((msg.value * 25) / 100);
        payable(artistAddress).transfer((msg.value * 75) / 100);
        if (
            block.timestamp >= wlTimeastamp && block.timestamp < publicTimestamp
        ) {
            require(
                whitelistMints[msg.sender] >= quantity,
                "Not enough mints left for you"
            );
            require(
                msg.value >= _wlPrice * quantity,
                "Not enough AVAX sent for the transaction"
            );
            for (uint256 i = 0; i < quantity; i++) {
                internalMint(msg.sender);
                whitelistMints[msg.sender] -= 1;
            }
        } else if (block.timestamp >= publicTimestamp) {
            require(
    ...
    {
        return string.concat(super.tokenURI(tokenId), _URIExtension);
    }
