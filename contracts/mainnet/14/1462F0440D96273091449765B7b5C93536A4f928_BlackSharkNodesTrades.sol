...
contract BlackSharkNodesTrades {
    ...
    event SmallNodeClaimed(address indexed _address, uint256 _reward, uint256 _id, uint256 claimTime);
    event MiddleNodeClaimed(address indexed _address, uint256 _reward, uint256 _id, uint256 claimTime);
    event BigNodeClaimed(address indexed _address, uint256 _reward, uint256 _id, uint256 claimTime);
    ...
    function getSmallNodeInfo(uint256 id) public view returns (uint256, uint256, bool, bool, uint256, uint256) {
        SmallNode storage node = _smallNodes[msg.sender][id];
        return (node.lastClaim, node.createdAt, node.burned, node.active, node.id, node.rewards);
    }

    function getMiddleNodeInfo(uint256 id) public view returns (uint256, uint256, bool, bool, uint256, uint256) {
        MiddleNode storage node = _middleNodes[msg.sender][id];
        return (node.lastClaim, node.createdAt, node.burned, node.active, node.id, node.rewards);
    }

    function getBigNodeInfo(uint256 id) public view returns (uint256, uint256, bool, bool, uint256, uint256) {
    ...
    function calculateReward(address address_) public returns (uint256) {
        ...
        for (; i < smallNodesBalance; i++) {
            ...
            if (smallNode.active) {
                ...
                reward += localReward;
                emit SmallNodeClaimed(address_, localReward, smallNode.id, block.timestamp);
            }
        }
        ...
        for (; i < middleNodesBalance; i++) {
            ...
            if (middleNode.active) {
                ...
                reward += localReward;
                emit MiddleNodeClaimed(address_, localReward, middleNode.id, block.timestamp);
            }
        }
        ...
        for (; i < bigNodesBalance; i++) {
            ...
            if (bigNode.active) {
                ...
                reward += localReward;
                emit BigNodeClaimed(address_, localReward, bigNode.id, block.timestamp);
            }
        }
