...
contract contractHoppers is Ownable, IERC721Receiver{
    ...
    function getInfos(uint256 tokenId) public view returns (uint32[] memory infosArray){
        (infosArray[0],infosArray[1],infosArray[2],infosArray[3],infosArray[4],infosArray[5],infosArray[6]) = h.hoppers(tokenId);
        
        return infosArray;
        
    }

    function getZoneInfo(address zone) public view returns (uint256 value){
        Ihoppers z = Ihoppers(zone);
        uint256 data = z.totalBaseShare();
        return data;
    }
