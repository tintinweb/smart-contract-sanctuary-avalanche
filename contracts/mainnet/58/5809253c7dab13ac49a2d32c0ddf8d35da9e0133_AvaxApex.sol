...
contract AvaxApex is Dev {
    ...
    uint256 constant public AMOUNT_MAX_INVEST= 40 ether;
    uint256 constant public AMOUNT_MAX_WITHDRAWN = 40 ether;

    uint8 constant public REFERRAL_PERCENTS = 25;
    uint8 constant public REFERRAL_PERCENTS_MAX = 100;
    uint8 constant public PERCENTS_PENALTY = 200;
    ...
    function withdraw() public nonReentrant{
        ...
        user.blocked.times = user.blocked.times.add(1);
    ...
    function getUserInfoBlocked(address user_) public view returns(bool state, uint256 times, uint256 investPenalty, uint256 date) {
    ...
    function definedBLocked(User storage user, uint256 amount) internal {
        if(user.blocked.times > 2){
            user.blocked.state = true;
            user.blocked.investPenalty = amount;
