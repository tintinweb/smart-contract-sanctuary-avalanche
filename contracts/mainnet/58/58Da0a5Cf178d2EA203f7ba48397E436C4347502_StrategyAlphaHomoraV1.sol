...
contract StrategyAlphaHomoraV1 is BaseStrategy {    
    ...
    function balanceOfPool() public view override returns (uint256) {
        ...
        uint256 expectedEquity = balanceOfPoolInt()+balanceStable;         
        address[] memory path = new address[](2);
        path[0] = stableToken;
        path[1] = USDC;
        uint256 convertedToUSDC = address(stableToken)!=address(USDC) ? _routerQuote(expectedEquity, path): expectedEquity;
        return convertedToUSDC;
    }
   ...
   function requireAmmCheckPass() internal view {
    ...
    function _setPnl() internal {
        ...
        int _balOfPool = int(balanceOfPoolInt());
    ...
    // function _pairQuote(address token) public view returns (uint price) {
    //     IUniswapV2Factory factory = IUniswapV2Factory(IUniswapRouterV2(router).factory());
    ...
    //     if (isToken0Stable && address(token) == address(volatileToken)) {
    //         price = r0/(r1/uint256(10)**volatileDecimals);
    //     } else if (!isToken0Stable && r0 > r1 && address(token) == address(volatileToken)) {
    //         price = r1/(r0/uint256(10)**volatileDecimals);
    //     } else if (r1 > r0 && address(token) != address(volatileToken) && address(token) != address(USDC)) {
    //         price = r0/(r1/uint256(10)**volatileDecimals);            
    //     } else if (r0 > r1 && address(token) != address(volatileToken) && address(token) != address(USDC)){
    //         price = r0/(r0)*uint256(10)**((volatileDecimals-stableDecimals)/2);
    //     } else if (r0 > r1 && address(token) == address(USDC)){
    //         price = r0/(r1/uint256(10)**volatileDecimals);
    //     }
    // }
        ...
        returns (uint256 withdrawn)
    {
        ...
        uint256 equity = balanceOfPoolInt();
    ...
    function getPendingRewards() public view returns (uint pendingRewards) {        
        if(isPangolin == true) {
             pendingRewards = getPendingRewardsPangolin();             
        }
        if(isTraderJoe == true) {
             pendingRewards = getPendingRewardsTraderJoe();             
        }        
    }

    function getBankData() internal view returns (address collateralTokenAddress, uint collateralId, uint collateralAmount){
        (, collateralTokenAddress, collateralId, collateralAmount) = IBank(homoraBank).getPositionInfo(positionId);
    }

    function getPendingRewardsPangolin() internal view returns (uint pendingRewards) {
    // query position info from position id
            (address collateralTokenAddress, uint collateralId, uint collateralAmount) = getBankData();
        ...
        returns (uint accRewardPerShare)
    {
      ...
      function getPendingRewardsTraderJoe() internal view returns (uint pendingRewards) {
        // query position info from position id
        (address collateralTokenAddress, uint collateralId, uint collateralAmount) = getBankData();
        ...
        endRewardTokenPerShare += totalSupply > 0 ? (pendingRewardFromChef * PRECISION) / totalSupply : 0;
    ...
    function _swapRewards() internal {
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            uint stableDecimals = ERC20Upgradeable(stableToken).decimals();      
            uint expectedOut = balance*price/uint256(10)**(stableDecimals);
            uint minOut = expectedOut*(PRECISION-slippage)/PRECISION;            
            minOut = minOut/uint256(10)**ERC20Upgradeable(rewards[i]).decimals();
            address[] memory path = new address[](2);
            path[0] = rewards[i];
            path[1] = stableToken;
