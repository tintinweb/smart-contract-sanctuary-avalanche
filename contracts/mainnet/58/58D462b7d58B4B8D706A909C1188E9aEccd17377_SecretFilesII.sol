...
contract SecretFilesII is ERC721Enumerable, Ownable, ERC2981ContractWideRoyalties {
  using Strings for uint256;
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;
  mapping(address => bool) private _owners;
  string public baseURI;
  uint24 maxSupply = 333;
  mapping(address => uint8) private _whiteList;
  uint256 saleStart = 1690761600;
  ...
  function mint() public notOwner(msg.sender) {
    ...
    if(block.timestamp <= saleStart + 10 minutes){
      require(_whiteList[msg.sender] == 1, "Address not white-listed");
    }
    _tokenIds.increment();
  ...
  modifier notOwner(address to){
    require(!_owners[to], "This address already minted a token");
    _;
  }
