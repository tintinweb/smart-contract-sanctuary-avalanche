...
contract PresaleFactory is Ownable {
    ...
    mapping (address => uint256) private _userBoughtTokens;
    ...
    function presale(uint256 _amountToken, uint cType) external payable {
        uint256 currentAmount = _userBoughtTokens[msg.sender];
        require(currentAmount + _amountToken >= minCap && currentAmount + _amountToken <= maxCap, "PresaleFactory: The amount is not allowed for presale.");
        ...
        // add pASTRO amount user bought
        _userBoughtTokens[msg.sender] += _amountToken;
