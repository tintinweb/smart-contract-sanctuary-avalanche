...
abstract contract feeManager is Context {
    ...
    function payFeeTokenSpec(address _account, uint256 _intervals, uint256 _x) payable virtual external;
...
contract NebulaProtoStarDrop is Ownable{
	...
	constructor(address[] memory addresses,address payable _treasury){
		protoMGR = ProtoManager(addresses[0]);
		over = overseer(addresses[1]);
		feeMGR = feeManager(addresses[2]);
		feeTok = IERC20(addresses[3]);
	...
	//Avax Fee pay
    	function payFeeToken(address _account,uint256 _intervals) payable external{
    		...
    		getInList(_account);
    		uint256 needed = nebuLib.safeMuls(tokFee,_intervals);
	    	uint256 allowance = feeTok.allowance(_account, treasury);
		require(allowance >= needed, "Check the token allowance");
	        require(needed <= feeTok.balanceOf(_account),"you do have not hold enough to cover this claim");
    		feeMGR.payFeeToken(_account,_intervals);
    		if(msg.value > 0){
		...
		uint256 needed = nebuLib.safeMuls(fee,_intervals);
    		getInList(_account);
	        require(needed <= sent,"you did not send enough to cover this claim");
	        uint256 balanceRemainder = sent - needed;
    		feeMGR.payFeeAvax{ value: needed }(_account,_intervals,needed);
    		if(balanceRemainder > 0){
			payable(_account).transfer(balanceRemainder);
		}
    	}
	//fee Tok Fee pay
	function ProtoFeeTok(uint256 _intervals) payable external{
	...
	function ProtoAvaxFees(uint256 _intervals) payable external{
		require(tokOff == false, "sorry AVAX Payments are currently disabled, please use USDC");
		address _account = msg.sender;
    		uint256 sent = msg.value;
		uint256 fee = over.getFee();
		uint256 needed = nebuLib.safeMuls(fee,_intervals);
		require(getFeesPaid(_account).add(_intervals)<=nebuLib.safeMuls(getClaimed(_account),11),"you have no more fees to claim");
	        require(needed <= sent,"you did not send enough to cover this claim");
	        getInList(_account);
	        feeMGR.MGRrecPayFees(_account,_intervals);
	        uint256 balanceRemainder = sent - needed;
	...
	function payFeeTokenSpec(address _account, uint256 _intervals, uint256 _x) payable external{
		...
		require(allowance >= needed, "Check the token allowance");
	        require(needed <= feeTok.balanceOf(_account),"you do have not hold enough to cover this claim");
		feeMGR.payFeeTokenSpec(_account,_intervals,_x);
		if(msg.value > 0){
			payable(_account).transfer(msg.value);
		}
	}
		...
		uint256 needed = nebuLib.safeMuls(fee,_intervals);
	        ...
	        uint256 balanceRemainder = sent - needed;
    		feeMGR.payFeeAvaxSpec{ value: needed }(_account,_intervals,needed,_x);
    		if(balanceRemainder > 0){
			payable(_account).transfer(balanceRemainder);
		}
    	}
	function ProtoFeeTokSpec(uint256 _intervals, uint256 _x) payable external{
		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
    		address _account = msg.sender;
		uint256 needed = nebuLib.safeMuls(tokFee,_intervals);
	    	uint256 allowance = feeTok.allowance(_account, treasury);
	...
	//ProtoClaim
		...
		require(allowance >= tokFee, "Check the token allowance");
	        require(tokFee <= feeTok.balanceOf(_account),"you do have not hold enough to cover this claim");
	...
	function createProtoAvax(string memory _name) payable external {
		...
		feeMGR.addProto(_account,_name);
		feeMGR.MGRrecPayFees(_account,1);
		uint256 balanceRemainder = sent - fee;
