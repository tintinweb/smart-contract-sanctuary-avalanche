...
/*----------------------------------------------------*/
pragma solidity ^0.8.11;
contract WonderlandQuest is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function newPath(uint[] memory cardIDs, string[] memory actions, string[] memory situations, uint msgValue) public returns(uint otID){
        ...
        uint[] memory UINTs = new uint[](actions.length);
        UINTs[0] = actions.length; //page count
        (UINTs[1], UINTs[2]) = ORACLE.getFee();
        UINTs[3] = UINTs[1] + UINTs[2];// oracleFee
        if(cardCost + UINTs[3] > 0){
            UINTs[4] = cardCost * UINTs[3]/( cardCost + UINTs[3])/2;//pock
            UINTs[5] = cardCost * cardCost/( cardCost + UINTs[3])/2;//pass
        }
        
        require( $MM.transferFrom(msg.sender,THIS,msgValue) && msgValue >= (UINTs[3])+cardCost*UINTs[0]);
        uint toOracle = msgValue-(cardCost*UINTs[0]/2);
        $MM.approve(address(ORACLE), toOracle);
        otID = ORACLE.fileRequestTicket(1, true, toOracle);
        ...
        for(uint i; i<UINTs[0]; i++){
            ...
            minecart(cardID,cardCost,UINTs[3],UINTs[0]);
    ...
    uint cart; //story position

    function minecart(uint cardID, uint cc, uint rt, uint D) internal{
        uint upwards;
        uint pock;
        uint pass;
        uint parentID;
        ...
        for( i=0; i<3; i+=1){
            ...
            pock = upwards * rt/( cc + rt );
            pass = upwards * cc/( cc + rt );
            parentID = cards[cart].parent;
            pocket[ownerOf(parentID)] += pock;
        ...
        for(i=0; i<(D>5?1:10/D); i+=1){
            upwards = passing[cardID];
            pock = upwards * rt/( cc + rt );
            pass = upwards * cc/( cc + rt );
            parentID = cards[cardID].parent;
            pocket[ownerOf(parentID)] += pock;
            passing[parentID] += pass;
            if(cardID != 0){
                cardID = parentID;
