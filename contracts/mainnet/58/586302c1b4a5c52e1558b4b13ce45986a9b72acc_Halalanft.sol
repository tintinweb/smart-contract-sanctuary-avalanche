  ...
  ReentrancyGuard
{
  ...
  uint256 private constant MAX_PRESALE = 441;
  ...
  constructor(address _usdc, address _feed) ERC721A("Halalanft", "HNFT") {
    ...
    _usdPrice = 199 * 10 ** _priceFeed.decimals(); // 199 USD by default
  ...
  function publicMint(uint256 _amount) external payable {
    ...
    require(_amount < MAX_PUBLIC, "public: cannot minted more than 5");
    require(publicMintingEnabled, "public: minting is not enabled");
    if (totalSupply() < MAX_PRESALE) {
      require(totalSupply() + _amount < MAX_PRESALE, "Mint Exceeds Presale Allowance");
    }
    uint256 price = getCurrentPrice();
    _transferIn(price * _amount);
    _safeMint(msg.sender, _amount);
    if (totalSupply() == MAX_PRESALE - 1) publicMintingEnabled = false;
  }
