...
contract Plutus is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("Plutus Node", "PLUTUS") PaymentSplitter(payees, shares) {
    ...
    function compoundNode(string memory name, uint256 total) public {
        ...
        require(
            walletBalances[sender] >= finalPrice,
            "NODE CREATION: Insufficient Balance"
        );
        walletBalances[sender] -= finalPrice;
        nodeRewardManager.createNode(sender, name, total);
    }
