...
contract MintJellyWL is ERC721URIStorage, Ownable, Pausable, ReentrancyGuard
{
...
//
  constructor() ERC721("don Donut", "donDonut") {//contract name and symbol, can only be set here in the constructor
    allowlistPrice = 1000000000000000000; //  1 AVAX Allowlist mint price in wei https://eth-converter.com/
    salePrice = 2000000000000000000; //  2 AVAX public mint price in wei https://eth-converter.com/
    baseURI = "ipfs://QmSoqc3cj9brEXRrrt9irggpMHsZDLx9nDqH3i9sngHBxX/"; //don Donut add in ipfs location info for the json metadata folder 
    unrevealedURI = "ipfs://Qme1RgYm5hwD5LCUq55b75R4j75PeeW8K7eK318KYuRrQt"; //generic unrevealed json
    maxMintPerTx = 10; //start with a limit of 10 mints per transaction
    royaltyPercent = 500; //starts with 5% royalties
    mintActive = true; //start with mint activated since we have the start time as well
    maxSupply = 99; //no more than this many nfts can be minted total
    allowlistStartTime = 1669687200; //https://www.unixtimestamp.com/ timestamp time Mon Nov 28 2022 21:00:00 GMT-0500 (Eastern Standard Time)
    publicSaleStartTime = 1669689000; //https://www.unixtimestamp.com/ timestamp time Mon Nov 28 2022 21:30:00 GMT-0500 (Eastern Standard Time)

    botsCanMint = false; //false means that No smart contract bots can mint

    devFeePercent = 5000; //50% dev fee, can only be set here in the constructor
  ...
  modifier timeNotStarted{
    require (allowlistStartTime>0, "Mint start time is not yet set");
    require(block.timestamp>allowlistStartTime, "Minting has not started yet.");
    _;
  }
  ...
  function setPublicMintTime(uint _startTime) public onlyOwner {
    publicSaleStartTime = _startTime;
  }

  function setWLMintTime(uint _startTime) public onlyOwner {
  ...
  function setWLMintTimeToNow() public onlyOwner {
      allowlistStartTime = (block.timestamp+1);
  }
...
//Added pause transfer function from Openzepplin contract wizard
  ...
  function setBaseExtention(string memory _newBase) public onlyOwner { //can set to ".json" for pinata style tokenuri links
    baseExtension = _newBase;
  }


  function ownerMintSend(address _recipient) external onlyOwner{//Owner mints free and sends to address 
      _privateMint(_recipient);
  }
  ...
  function timeUntilWLMint() public view returns(uint){
    require (allowlistStartTime>0, "Mint time not yet set");
    if (allowlistStartTime<=block.timestamp){
        return(0);
    } else {
    return(allowlistStartTime-block.timestamp);
    }
  }

  function timeUntilPublicMint() public view returns(uint){
    require (publicSaleStartTime>0, "Mint time not yet set");
    if (publicSaleStartTime<=block.timestamp){
        return(0);
    } else {
  ...
  function setSalePrice(uint _newPrice) public onlyOwner{ //nft
    salePrice = _newPrice;
  }

  function setWLPrice(uint _newPrice) public onlyOwner{ //nft
  ...
  function multiMint(uint256 _quantity) public payable timeNotStarted nonReentrant{ //1.69 Avax is 1690000000000000000 in Wei or 3380000000000000000 to mint two at that price https://eth-converter.com/
    require(mintActive, "Minting is not active.");
    if (totalSupply() + _quantity > maxSupply) {revert ("Launchpeg__MaxSupplyReached()");
    }
    require(_quantity <= maxMintPerTx, "Cannot mint that many at once.");

    uint nowPrice = allowlistPrice;//price changes with allowlist time vs public mint. with timeNotStarted modifier we know that allowlist has started
    if (publicSaleStartTime<=block.timestamp){//if public sale has started use public price
        nowPrice = salePrice;
    } else {
        if (_quantity > allowlist[msg.sender]) {//if public sale not started then check against allowlist
            revert("You tried to mint more than is allowed for your total personal allowlist amount");
        }
    allowlist[msg.sender] -= _quantity;
    }
    
    require(msg.value == (nowPrice * _quantity), "Wrong amount of AVAX sent for mint price * quantity.");
    ...
    emit Mint(msg.sender, _quantity, salePrice, _tokenIds.current());
  }
