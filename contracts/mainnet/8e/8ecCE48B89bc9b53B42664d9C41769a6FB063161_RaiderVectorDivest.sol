...
contract RaiderVectorDivest is ADivestStrategy2 {
  ...
  constructor(
    ...
    address _divestStrategyHelpers
  )
    ADivestStrategy2(
      _uniRouter,
      _WNATIVE,
...
import "../interfaces/IUniswapRouterETH.sol";
import "../interfaces/IDivestStrategyHelpers.sol";
...
abstract contract ADivestStrategy2 is Ownable, Pausable {
  ...
  address public divestStrategyHelpers;
  ...
  DivestStructs.Harvest[] harvests;
  ...
  constructor(
    ...
    divestStrategyHelpers = _divestStrategyHelpers;
  ...
  function setPID(uint8 _poolId) external onlyOwner {
    poolId = _poolId;
  }
  ...
  function recordHarvest(uint256 _harvestAmount) internal {
    uint256 totalSupply = IERC20(vault).totalSupply();
    DivestStructs.Harvest memory harvestDetails = IDivestStrategyHelpers(
      divestStrategyHelpers
    ).getHarvestDetails(_harvestAmount, totalSupply, harvests);

    harvests.push(harvestDetails);
  }
  ...
  function calculatePayout(address _user) public view returns (uint256) {
    ...
    return
      IDivestStrategyHelpers(divestStrategyHelpers).calculatePayout(
        userEntryPositions[_user].isActive,
        numShares,
        userEntryPositions[_user].entryPosition,
        1 ether,
        harvests
      );
  }
...
interface IDivestStrategyHelpers {
