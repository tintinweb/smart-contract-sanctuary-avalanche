...
contract StakingDoubleRewardsJoe is Ownable {
    ...
    IJoeChef public constant masterChefJoeV3 = IJoeChef(0x188bED1968b795d5c9022F6a0bb5931Ac4c18F00); //  MCJoe V3 //0xd6a4F121CA35509aF06A0Be99093d08462f53052 MCJoe V2
    ...
    constructor(
        ...
        stakingToken.safeApprove(address(masterChefJoeV3), MAX_UINT);
    ...
    function stake(uint256 amount) external onlyApprovedContractOrEOA updateReward(msg.sender) {
        ...
        if (amount > 0) {
            masterChefJoeV3.deposit(joePid, amount);
        }
        if (newShares > 0) {
    ...
    function withdraw(uint256 amountShares) external onlyApprovedContractOrEOA updateReward(msg.sender) {
        ...
        if (lpFromShares > 0) {
            masterChefJoeV3.withdraw(joePid, lpFromShares);
    ...
    function _claimRewards() internal {
        ...
        if (updateAndClaim && totalShares > 0) {
            ...
            masterChefJoeV3.deposit(joePid, 0);
    ...
    function checkReward() public view returns (uint256) {
        (uint256 pendingJoe,,,) = masterChefJoeV3.pendingTokens(joePid, address(this));
        return pendingJoe;
    }

    function checkRewards() public view returns (uint256, address, string memory, uint256) {
        (uint256 pendingJoe, address bonusTokenAddress, string memory bonusTokenSymbol, uint256 pendingBonusToken) = masterChefJoeV3.pendingTokens(joePid, address(this));
