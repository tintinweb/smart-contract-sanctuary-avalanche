...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    uint256 private constant MAXSUPPLY = 10000 ether;
...
contract CoupToken is ERC20('COUPv2', 'COUPv2') {

    constructor() {
        _mint(_msgSender(), uint256(333000000000000000000)); // 10 tokens for initial liquidity
        //_mint(address(0x775bA9da22762e6C3f50374D8f5AAeC830848a6E), uint256(30000000000000000000)); // 10 tokens to treasury
    }
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accCoupPerShare;    
        uint16 depositFeeBP;
        uint256 lpSupply;
    }
    
    CoupToken public immutable coup;
    
    uint256 public startTime;
    address public feeAdd;
    address public treasuryAdd;
    
    uint256 private immutable COUPMAXSUPPLY;
    uint256 public coupPerSec = 0.007 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 coupPerSec);

    constructor(CoupToken _coup, uint256 _startTime, address _feeAdd, address _treasuryAdd) {
        coup = _coup;
        startTime = _startTime;
        feeAdd = _feeAdd;
        treasuryAdd = _treasuryAdd;
        COUPMAXSUPPLY  = _coup.maxSupply();
    }
    ...
    function safeCoupTransfer(address _to, uint256 _amount) internal {
        uint256 coupBalance = coup.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > coupBalance) {
            transferSuccess = coup.transfer(_to, coupBalance);
        } else {
            transferSuccess = coup.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTerraTransfer: Transfer failed");
    }
    
    function updateEmissionRate(uint256 _coupPerSec) external onlyOwner {
        require(_coupPerSec >= MIN_PERSEC && _coupPerSec <= MAX_PERSEC, "Bad Emissions per sec");
        massUpdatePools();
        coupPerSec = _coupPerSec;
        emit UpdateEmissionRate(msg.sender, _coupPerSec);
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (coup.totalSupply() >= COUPMAXSUPPLY) { 
            return 0; 
        }
        return _to - _from;
    }

    function pendingCoup(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCoupPerShare = pool.accCoupPerShare;
        ...
        if (block.timestamp > pool.lastRewardperSec && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 coupReward = (multiplier * coupPerSec * pool.allocPoint) / totalAllocPoint;
            accCoupPerShare += coupReward * 1e18 / pool.lpSupply;
        }
        return ((user.amount * accCoupPerShare) / 1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 coupReward = (multiplier * coupPerSec * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = coupReward / 10;
        uint256 CoupCalc = coup.totalSupply() + coupReward + toTreasury;
        
      if (CoupCalc <= COUPMAXSUPPLY) {
            coup.mint(treasuryAdd, toTreasury);
            coup.mint(address(this), coupReward);
        } else {
            coupReward = COUPMAXSUPPLY - coup.totalSupply();
            coup.mint(address(this), coupReward);
        }
        
        pool.accCoupPerShare += coupReward * 1e18 / pool.lpSupply;
        pool.lastRewardperSec = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accCoupPerShare) / 1e18) - user.rewardDebt;
            
            if (pending > 0) {
                safeCoupTransfer(msg.sender, pending);
            }
        }
        ...
        user.rewardDebt = (user.amount * pool.accCoupPerShare) / 1e18;
