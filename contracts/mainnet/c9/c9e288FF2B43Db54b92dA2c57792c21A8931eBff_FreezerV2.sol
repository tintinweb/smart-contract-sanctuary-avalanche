...
contract FreezerV2 is ERC20("Staked Pepperoni", "sRONI"), Ownable {
    ...
    uint256 public frozenPepperoni; // RONI pending withdrawal
    ...
    constructor(Pepperoni _pepperoni) {
        pepperoni = _pepperoni;
    }

    // Views

    function pepperoniBalance() public view returns (uint256 balance) {
        return pepperoni.balanceOf(address(this)) - frozenPepperoni;
    }

    function _unstakeOutput(uint256 _share) internal view returns (uint256 output) {
        uint256 totalShares = totalSupply();
        return _share.mul(pepperoniBalance()).div(totalShares);
    }
    ...
    function stake(uint256 _amount) external {
        ...
        uint256 totalShares = totalSupply();
        // If no sRONI exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || pepperoniBalance() == 0) {
            _mint(_msgSender(), _amount);
        } else {
            uint256 share = _amount.mul(totalShares).div(pepperoniBalance());
            _mint(_msgSender(), share);
        }

        pepperoni.transferToFreezer(_msgSender(), _amount);
    }

    function quickUnstake(uint256 _share) external {
        // QUICK_UNSTAKE_CONTRIBUTION_PERCENT of the claimable RONI will remain in the freezer
        ...
        // QUICK_UNSTAKE_BURN_PERCENT of the claimable RONI is burned
        uint256 amountSpoiled = unstakeOutput.mul(QUICK_UNSTAKE_BURN_PERCENT).div(100);

        // burn staker's share
        _burn(_msgSender(), _share);

        pepperoni.burn(address(this), amountSpoiled);
        pepperoni.safeTransfer(_msgSender(), output);
    }

    /**
     * @dev _share argument specified in sRONI
     */
    function prepareDelayedUnstake(uint256 _share) external {
        // calculate output and burn staker's share
        uint256 output = _unstakeOutput(_share); //converts sRONI values to RONI
        _burn(_msgSender(), _share);
        ...
        frozenPepperoni += output;

        pepperoni.burn(address(this), amountSpoiled);
    }

    /**
     * @dev argument specified in RONI, not sRONI
     */
    function claimDelayedUnstake(uint256 _amount) external {
        require(block.timestamp >= unlockTimestamps[_msgSender()], "RONI not yet unlocked");
        require(_amount <= unlockAmounts[_msgSender()], "insufficient locked balance");

        // deduct from unlocked
        unlockAmounts[_msgSender()] -= _amount;

        frozenPepperoni -= _amount;

        // transfer claim
        pepperoni.safeTransfer(_msgSender(), _amount);
    }
