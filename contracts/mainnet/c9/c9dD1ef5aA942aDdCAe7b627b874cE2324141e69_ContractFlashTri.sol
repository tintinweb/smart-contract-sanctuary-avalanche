...
contract ContractFlashTri {
    ...
    address private constant RUGPULL =
        0x61eCd63e42C27415696e10864d70ecEA4aA11289;
    address private constant QI = 0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5;
    address private constant AVE = 0x78ea17559B3D2CF85a7F9C2C704eda119Db5E6dE;
    address private constant AVME = 0x1ECd47FF4d9598f89721A2866BFEb99505a413Ed;
    address private constant PNG = 0x60781C2586D68229fde47564546784ab3fACA982;
    address private constant ACRE = 0x00EE200Df31b869a321B10400Da10b561F3ee60d;
    ...
    function triangularArbitrage(
        ...
        address dummyToken;
        if (_tokens[0] != WAVAX && _tokens[1] != WAVAX && _tokens[2] != WAVAX) {
            dummyToken = WAVAX;
        } else if (
            _tokens[0] != RUGPULL &&
            _tokens[1] != RUGPULL &&
            ...
            _tokens[0] != AVE && _tokens[1] != AVE && _tokens[2] != AVE
        ) {
            dummyToken = AVE;
        } else if (
            _tokens[0] != AVME && _tokens[1] != AVME && _tokens[2] != AVME
        ) {
            dummyToken = AVME;
        } else if (
            _tokens[0] != PNG && _tokens[1] != PNG && _tokens[2] != PNG
        ) {
            dummyToken = PNG;
        } else if (
            _tokens[0] != ACRE && _tokens[1] != ACRE && _tokens[2] != ACRE
        ) {
            dummyToken = ACRE;
        } else {
            dummyToken = QI;
        ...
        // Passing data triggers panAVECall as this is what constitutes a loan
