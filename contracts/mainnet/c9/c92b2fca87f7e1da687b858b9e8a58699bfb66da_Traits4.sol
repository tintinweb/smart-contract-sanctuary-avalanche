...
contract Traits4 is Ownable, ITraits {
   ...
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        (IPoliceAndThief.ThiefPolice memory s, uint256 level) = upgrading.getTraitsAndLevel(tokenId);
        uint8 shift = s.isThief ? 0 : 10;

        string memory svgString = string(abi.encodePacked(
                s.isThief ? draw(thiefBody) : draw(policeBody),
                s.isThief && level > 0 ? drawTrait(traitData[7][uint8(level)]) : '',
    ...
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        if (s.isThief) {
            traits = string(abi.encodePacked(
                    ...
                    attributeForTypeAndValue("Level", Strings.toString(level)), ','
                ));
        } else {
            traits = string(abi.encodePacked(
...
contract ThiefUpgrading is Ownable, Pauseable {
    ...
    function upgrade(uint256[] calldata tokenIds) public nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 level = levelOf[tokenIds[i]];
    ...
    function getTraitsAndLevel(uint256 tokenId) public view returns (IPoliceAndThief.ThiefPolice memory traits, uint256 level) {
        traits = game.getTokenTraits(tokenId);
...
interface IThiefUpgrading {
    ...
    function getTraitsAndLevel(uint256) external view returns(IPoliceAndThief.ThiefPolice memory, uint256 level);
}
