...
contract BoLStaking is IERC721Receiver, Ownable {
    ...
    struct StakedNFT {
        ...
        uint256 dripAmount; // the token to reward for staking
    ...
    function _nameFinder(string memory _name, uint256 _dLevel) internal returns(uint256){

        if(StringUtils.equal(_name, "Town Hall")) {
        ...
        else if(StringUtils.equal(_name, "Barrack")){
        ...
        else if(StringUtils.equal(_name, "Dragon Pit")){
            if(_dLevel == 0){
                return 500;
        ...
        else if(StringUtils.equal(_name, "Defense Wall")){
            if(_dLevel == 0){
                return 250 ;
        ...
        else if(StringUtils.equal(_name, "Wizard")){
            if(_dLevel == 0){
                return 350;
        ...
        else if(StringUtils.equal(_name, "Archer Tower")){
        ...
        else if(StringUtils.equal(_name, "Workshop") || StringUtils.equal(_name,"Builder")){
    ...
    function stake(
        ...
        uint256 _level = nftContract.getLevel(tokenId);
        uint256 _theDrip = _nameFinder(_structName, _level);

        // set the drip rate based upon the amount released per day divided by the seconds in a day
        uint256 dripRate = (_theDrip * 86400 * 10**13) / 24 hours;
    ...
    function unstake(bytes32 stakeId) public {
        ...
        //this.claimAll(0xdDbbe09E7d867A24d8FF27A67DA04937BE1dF070);
     ...
     *
     */
    function permitRewardToken(address token)
        public
        onlyOwner
    {
        require(
