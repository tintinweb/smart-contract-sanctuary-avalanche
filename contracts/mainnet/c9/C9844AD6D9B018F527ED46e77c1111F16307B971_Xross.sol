/**
 *Submitted for verification at snowtrace.io on 2022-06-24
*/
...
contract Xross is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the XROSS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("XrossChainToken", "XROSS") {
        // Mints 1 XROSS to contract creator for initial pool setup
    ...
    function _getXrossPrice() internal view returns (uint256 _xrossPrice) {
        try IOracle(xrossOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Xross: failed to fetch XROSS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _xrossPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_xrossPrice >= taxTiersTwaps[tierId]) {
    ...
    function setXrossOracle(address _xrossOracle) public onlyOperatorOrTaxOffice {
        require(_xrossOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints XROSS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of XROSS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentXrossPrice = _getXrossPrice();
            currentTaxRate = _updateTaxRate(currentXrossPrice);
            if (currentXrossPrice < burnThreshold) {
                burnTax = true;
            }
        }
