...
contract FlowerUpgradeable is OwnableUpgradeable {
    ...
    ITreasuryUpgradeable private treasury;
    ...
    mapping(address => bool) public alreadyDone;
    ...
    function movingChoice(
        ...
        movedNCTRFromBloomBox[_bloomOwner] += _instantAmount;
        ...
        returns (uint256 countCompound, uint256 countClaim)
    {
        ...
        for (uint256 i = actions.length; i > 0; i--) {
            if (actions.length >= 6) {
                if (countCompound + countClaim < 7) {
                    if (actions[i - 1].isCompound) {
                        countCompound += 1;
                    } else {
                        countClaim += 1;
                    }
    ...
    function calculateExtraFee(uint256 _amount, uint256 _fee)
        internal
        pure
        returns (uint256)
    {
        ...
        return calculateFees / 100;
    }

    function calculateExtra(uint256 _amount, uint256 _fee)
        internal
        pure
        returns (uint256)
    {
        ...
        return calculateFees * 10;
    }

    function claimAlocation(address wallet) external onlyBloomReferralNode {
        require(getDepositedValue(msg.sender) > 0, "User Never Deposited");
        require(alreadyDone[msg.sender] != true, "your Alocation is out!");
        uint256 TVL = 60000000000;

        uint256 Stables = 66000000000;

        uint256 getPercentage = calculateExtra(Stables, TVL);

        uint256 userAlocation = getDepositedValue(wallet) -
            users[wallet].payouts;

        if (movedBloomBoxes[wallet].length > 0) {
            for (uint256 i = 0; i < movedBloomBoxes[wallet].length; i++) {
                MoveToBloomify storage bloomifyMove = movetobloomify[
                    movedBloomBoxes[wallet][i]
                ];
                [movedBloomBoxes[wallet][i]];
                ...
                if (bloomifyMove.instantAmount > bloomifyMove.alreadyMoved) {
                    userAlocation +=
                        bloomifyMove.instantAmount -
                        bloomifyMove.alreadyMoved;
                }
        ...
        uint256 alocationCalc = userAlocation / 1000000000000000000;

        uint256 fees = calculateExtraFee(alocationCalc, getPercentage);

        uint256 feessplited = fees / 13;

        USDCeToken.transfer(msg.sender, feessplited * (10**6));
    }

    function claimAlocation() external onlyBloomReferralNode {
        require(getDepositedValue(msg.sender) > 0, "User Never Deposited");
        require(alreadyDone[msg.sender] != true, "your Alocation is out!");
        uint256 TVL = 60000000000;
        ...
        uint256 userAlocation = getDepositedValue(msg.sender) -
            users[msg.sender].payouts;
        ...
        uint256 feees = calculateExtraFee(alocationCalc, getPercentage);

        USDCeToken.transfer(msg.sender, feees * (10**6));

        users[msg.sender].depositsNCTR = 0;
        users[msg.sender].depositsUSDCe = 0;
        users[msg.sender].payouts = 0;
        movedNCTRFromBloomBox[msg.sender] = 0;
        airdrops[msg.sender].airdropsReceived = 0;
        userCompoundRewards[msg.sender] = 0;
        updateAPR(msg.sender);

        alreadyDone[msg.sender] = true;
    }
    ...
    function withdrawUSDCe(address _to, uint256 _amount) external onlyOwner {
        require(
