...
contract Treasury is ContractGuard {
    ...
    event BoughGbonds(address indexed from, uint256 ghostAmount, uint256 bondAmount);
    ...
    function getBurnableGhostLeft() public view returns (uint256 _burnableGhostLeft) {
        ...
        if (_ghostPrice <= ghostPriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableGhost = _maxMintableBond.mul(_ghostPrice).div(1e17); // related to peg to 0.1avax
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_ghostPrice > ghostPriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalGhost.mul(1e17).div(_rate); // related to peg to 0.1avax
            }
        }
    }
    ...
    function initialize(
        ...
        ghostPriceOne = 10**17; // related to peg to 0.1avax
    ...
    function seGbondDepletionFloorPercent(uint256 _bondDepletionFloorPercent) external onlyOperator {
    ...
    function seGbondSupplyExpansionPercent(uint256 _bondSupplyExpansionPercent) external onlyOperator {
    ...
    function buyBonds(uint256 _ghostAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _ghostAmount.mul(_rate).div(1e17); // related to peg to 0.1avax
        ...
        emit BoughGbonds(msg.sender, _ghostAmount, _bondAmount);
    }
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _ghostAmount = _bondAmount.mul(_rate).div(1e17); // related to peg to 0.1avax
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        IERC20(ghost).safeApprove(masonry, 0);
        IERC20(ghost).safeApprove(masonry, _amount);
    ...
    function _sendToBondTreasury(uint256 _amount) internal {
        uint256 treasuryBalance = IERC20(ghost).balanceOf(bondTreasury);
        ...
        if (_amount > unspent) {
            IBasisAsset(ghost).mint(bondTreasury, _amount.sub(unspent));
        }
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        _sendToBondTreasury(ghostSupply.mul(bondSupplyExpansionPercent).div(10000));
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToMasonry(ghostSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochGhostPrice > ghostPriceCeiling) {
                // Expansion ($GHOST Price > 0.1 $AVAX): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForMasonry = ghostSupply.mul(_percentage).div(1e17); // related to peg to 0.1avax
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = ghostSupply.mul(_percentage).div(1e17); // related to peg to 0.1avax
