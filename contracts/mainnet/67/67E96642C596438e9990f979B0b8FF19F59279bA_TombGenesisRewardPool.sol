...
contract TombGenesisRewardPool {
    ...
    // MAINNET
    uint256 public tombPerSecond = 0.14467 ether; // 25000 TOMB / (48h * 60min * 60s)
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            uint256 depositDebt = _amount.mul(100).div(10000); // tax amount 1%
            user.amount = user.amount.add(_amount.sub(depositDebt));
            pool.token.safeTransfer(daoFundAddress, depositDebt); //send tax to daofund
        }
