    ...
    ILiquidityPool
{
    ...
    function deposit(
        ...
        if (address(swapDetails.srcTokenAddress) == swapDetails.srcStableTokenAddress) {
            ...
            if (_burnList[address(swapDetails.srcStableTokenAddress)]) {
                ...
                // TODO: remove this src stable token amount
                swapDetails.srcStableTokenAddress == _usdc &&
                _isUsdcBurnableMintable[destinationChainID]
            ) {
                // Burn USDC
    ...
    function handleUsdcBurn(
        ...
        uint64 usdcNonce = usdcBurnerContract.depositAndBurnUsdc(destChainId, amount);
    ...
    function handleDepositForReserveToken(
        ...
        if (_burnList[address(swapDetails.srcTokenAddress)]) {
            ...
            if (
                // TODO: remove this src stable token amount
                _usdc == swapDetails.srcStableTokenAddress &&
                _isUsdcBurnableMintable[destinationChainID]
            ) {
                // Burn USDC
...
contract HandlerHelpersUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable, IERCHandler {
    ...
    function resetResource(bytes32 resourceID, address contractAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
