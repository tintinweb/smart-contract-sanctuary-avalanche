...
contract LayerZeroGateway is Pausable, ReentrancyGuard, BalanceManagement, IGateway {
    ...
    uint256 public targetCallGasReserve;
    ...
    event SetTargetCallGasReserve(uint256 gasReserve);
    ...
    event TargetGasFailure(uint256 indexed sourceStandardChainId);
    event TargetClientFailure();
    ...
    function setTargetCallGasReserve(uint256 _gasReserve) external onlyManager {
        targetCallGasReserve = _gasReserve;

        emit SetTargetCallGasReserve(_gasReserve);
    }
        ...
        onlyLayerZeroProxy
    {
        ...
        if (gasLeft < targetCallGasReserve) {
            emit TargetGasFailure(sourceStandardChainId);
        }

        try client.handleExecutionPayload{gas: gasLeft - targetCallGasReserve}(sourceStandardChainId, _payload) {
        } catch {
            emit TargetClientFailure();
        }
    }
