...
*/
contract EvoUpgradeable is Initializable, ERC721Upgradeable, ERC721EnumerableExtendedUpgradeable, PausableUpgradeable,
AccessControlEnumerableUpgradeable, ERC721BurnableUpgradeable, TokenConstants, ERC721BlacklistUpgradeable,
ChainlinkVRFConsumerUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) private _speciesBase;
    // set of configured species
    uint256[] private _speciesIds;

    // Evo Attribute storage
    // map speciesId to attribute id for value of attribute
    mapping(uint256 => mapping(uint256 => uint256)) private _attributes;
    ...
    function startHatching(uint256[] memory tokenIds) public whenNotPaused onlyRole(ADMIN_ROLE) {
        PendingHatch storage pending = _pendingHatches[_msgSender()];
        require(pending.requestId == 0, "Existing hatch in progress");
        require(tokenIds.length <= 500, "Maximum 500 per hatch");

        EGG.hatch(_msgSender(), tokenIds);
    ...
    function reveal() public {
        PendingHatch storage pending = _pendingHatches[_msgSender()];
        require(pending.requestId > 0, "No pending hatch in progress");
        require(pending.words.length > 0, "Results still pending");

        bool[] memory treated = EGG.checkTreated(pending.ids);

        uint256[] memory minRarity = new uint256[](_speciesIds.length);

        uint256 totalRarity = 0;
        for (uint256 i = 0; i < _speciesIds.length; i++) {
            if (_speciesBase[_speciesIds[i]][0] == 0) {
                continue;
            }
            minRarity[i] = totalRarity;
            totalRarity += _speciesBase[_speciesIds[i]][1];
        }

        for (uint256 i = 0; i < pending.ids.length; i++) {
            // generation
            _attributes[pending.ids[i]][3] = 0;
            // breed count
            _attributes[pending.ids[i]][6] = 999;
            // experience
            _attributes[pending.ids[i]][7] = 0;
            uint256[] memory randomChunks = chunkRandom(pending.words[i]);
            speciesRoll(pending.ids[i], randomChunks, minRarity, totalRarity);
            geneRoll(pending.ids[i], treated[i], randomChunks);
    ...
    function speciesRoll(
        ...
        // species
        uint256 speciesResult = randomChunks[0] % totalRarity;
        for (uint256 i = 0; i < minRarity.length; i++) {
            if (speciesResult > minRarity[i]) {
                continue;
            }
            _attributes[tokenId][0] = _speciesIds[i];
            break;
        }
    }
    ...
    function geneRoll(
        ...
        uint256 speciesId = _attributes[tokenId][0];
        // visual rarity
        bool epic = (randomChunks[1] % (treated ? 3000 : 5000)) < (treated ? 9 : 1);
        bool chroma = (randomChunks[1] % (treated ? 3000 : 1000)) < (treated ? 45 : 1);
        uint256 visualRarity = epic ? 2 : (chroma ? 1 : 0);
        _attributes[tokenId][1] = visualRarity;
        // gender
        _attributes[tokenId][2] = randomChunks[2] % 100 < _speciesBase[speciesId][2] ? 1 : 0;

        // primary type
        _attributes[tokenId][4] = _speciesBase[speciesId][3];
        // secondary type
        _attributes[tokenId][5] = _speciesBase[speciesId][4];

        // nature
        _attributes[tokenId][8] = randomChunks[3] % 21;

        // stats
        for (uint256 i = 0; i < 5; i++) {
            _attributes[tokenId][9 + i] = randomChunks[i + 4] % 50;
        }
    }

    function chunkRandom(uint256 random) internal pure returns(uint256[] memory) {
        uint256 basisPoints = 10_000;
        uint256 chunks = 9; // 77 digits constant, max 20 chunks
        uint256[] memory chunked = new uint256[](chunks);
        uint256 number = random;
        for (uint256 i = 0; i < chunks; i++) {
            uint256 chunk = uint256(number % basisPoints);
            number = number / basisPoints;
            chunked[i] = chunk;
        }
        return chunked;
    }
    ...
    function getGeneralAttributes(uint256 tokenId) internal view returns (string memory) {
        string memory species = _attributeStrings[0][_attributes[tokenId][0]];
        string memory rarity = _attributeStrings[1][_attributes[tokenId][1]];
        string memory gender = _attributes[tokenId][2] == 0 ? 'Male' : 'Female';
        uint256 generation = _attributes[tokenId][3];
    ...
    function getExtraAttributes(uint256 tokenId) internal view returns (string memory) {
        string memory primaryType = _attributeStrings[4][_attributes[tokenId][4]];
        string memory secondaryType = _attributeStrings[5][_attributes[tokenId][5]];
        uint256 breedCount = _attributes[tokenId][6];
        uint256 experience = _attributes[tokenId][7];
        return string(abi.encodePacked(
    ...
    function getBattleAttributes(uint256 tokenId) internal view returns (string memory) {
        string memory nature = _attributeStrings[8][_attributes[tokenId][8]];
        uint256 attack = _attributes[tokenId][9];
        uint256 defense = _attributes[tokenId][10];
        uint256 special = _attributes[tokenId][11];
        uint256 specialDefense = _attributes[tokenId][12];
        uint256 speed = _attributes[tokenId][13];
    ...
    function getAllSpeciesIds() public view returns(uint256[] memory) {
        return _speciesIds;
    }

    function getSpeciesBaseAttributes(uint256 speciesId) public view returns(uint256[] memory) {
        uint256[] memory baseAttributes = new uint256[](5);
        for (uint256 i = 0; i < 5; i++) {
            baseAttributes[i] = _speciesBase[speciesId][i];
        }
    ...
    function enableSpecies(uint256[] memory speciesIds) public onlyRole(ADMIN_ROLE) {
        require(speciesIds.length > 0, "no species defined");
        for (uint256 i = 0; i < speciesIds.length; i++) {
            uint256 speciesId = speciesIds[i];
            _speciesIds[i] = speciesId;
            _speciesBase[speciesId][0] = 1;
        }
    }

    function setBaseAttributes(uint256[] memory speciesIds, uint256[][] memory baseAttributes) public onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < speciesIds.length; i++) {
            uint256 speciesId = speciesIds[i];
            _speciesIds[i] = speciesId;
            for (uint256 j = 0; j < baseAttributes.length; j++) {
                for (uint256 k = 0; k < baseAttributes[j].length; k++) {
                    _speciesBase[speciesId][j] = baseAttributes[j][k];
                }
            }
        }
    }
