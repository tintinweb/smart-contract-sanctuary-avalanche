...
import "./lib/Math.sol";
import "./lib/RewardsHelperLibs.sol";
    ...
    RewardsHelperLibs
{
    ...
    address private RewardsExtension;
    /**
     * @dev Event of Reward
    ...
    function pauseReward(bool status) public onlyOwner {
        ...
        IRewardsExtension(RewardsExtension).pauseReward(status);
    }
    ...
    function OwnerIsStaked(
        ...
        return
            IRewardsExtension(RewardsExtension).OwnerIsStaked(
                _yieldKey,
                _type,
                _user
            );
        ...
        )
    {
        ...
        for (uint256 i = 0; i < YieldBoxIds.length; i++) {
            ...
            if (
                ...
                (RewardsClaimed[YieldBoxIds[i]].claimedDays > 0)
            ) {
                rewarded[i] = 0;
                amount[i] = 0;
            } else if (
                ...
                rewarded[i] = (MINIMAL_REWARDS_DAYS.div(1 minutes)) >
                    RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed
                    ? (MINIMAL_REWARDS_DAYS.div(1 minutes)) -
                        RewardsClaimed[YieldBoxIds[i]].freeDaysClaimed
                    : 0;
                amount[i] = mulDiv(
                    rewarded[i],
    ...
    {
        address user = wallet;
        uint256[] memory YieldKeyLeg = StakedYieldKeyLeg.tokenStaking(user);
        uint256[] memory YieldKeyReg = StakedYieldKeyReg.tokenStaking(user);
        if (!((YieldKeyLeg.length == 0) && (YieldKeyReg.length == 0))) {
            uint256 index = getIndexTotalRewards(user);
            ...
            for (uint256 i = 0; i < YieldKeyLeg.length; i++) {
                if (MaintFee.yieldBoxYieldKeyLeg(YieldKeyLeg[i]).length > 0) {
                    (
                        ...
                        YieldBoxRewarded2,
                        time2
                    ) = IRewardsExtension(RewardsExtension).preRewardPerYK(
                        YieldKeyLeg[i],
                        12,
                        user
                    );
            ...
            uint256[] memory YieldBoxMinimalRewards = MaintFee
                .getYieldBoxWithoutYieldKey(user);
        ...
        nonReentrant
    {
        for (uint256 i = 0; i < _yieldKey.length; i++) {
            ...
            require(
                _token.transferFrom(rewardsPool, _msgSender(), total),
            ...
            for (uint256 j = 0; j < YieldBoxRewarded.length; j++) {
                if (amount[j] > 0) {
                    ...
                    setYieldBoxRewardsClaimed(YieldBoxRewarded[j], rewarded[j]);
                }
            }
        }
    }
    ...
    function getRewardsDays(
        ...
        uint256 stakedDays = IRewardsExtension(RewardsExtension).getStakedDays(
            startStaked,
            endStaked,
            YieldBoxIds
        );
    ...
    function setYieldBoxBatchRewards(
        uint256[] memory amount,
        uint256[] memory rewarded,
        uint256[] memory YieldBoxRewarded,
        address user,
        uint256 percentage
    ) external onlyRewardsExtension returns (bool) {
        for (uint256 j = 0; j < YieldBoxRewarded.length; j++) {
            uint256 donationsMade = amount[j].mul(percentage).div(100);
            if (amount[j] > 0) {
                emit RewardsPaid(
                    user,
                    YieldBoxRewarded[j],
                    rewarded[j],
                    amount[j].sub(donationsMade)
                );
    ...
    function setYieldBoxRewardsClaimed(
        uint256 YieldBoxRewarded,
        uint256 rewarded
    ) internal {
        RewardsClaimed[YieldBoxRewarded].freeDaysClaimed = MINIMAL_REWARDS_DAYS
            .div(1 minutes);
        RewardsClaimed[YieldBoxRewarded].claimedDays += rewarded;
        if (
            (RewardsClaimed[YieldBoxRewarded].claimedDays ==
                LIMITS_DAYS.div(1 minutes))
        ) {
            YieldBox.setRewardClaimed(false, true, YieldBoxRewarded);
        } else {
            YieldBox.setRewardClaimed(true, false, YieldBoxRewarded);
        }
    }

    function setYieldBoxWithYieldKeyRewards(
        uint256 amount,
        uint256 rewarded,
        uint256 YieldBoxRewarded,
        uint256 _YieldKey,
        ...
        if (amount > 0 && _YieldKey != 0) {
            emit RewardsPaid(user, YieldBoxRewarded, rewarded, amount);

            setYieldBoxRewardsClaimed(YieldBoxRewarded, rewarded);
        }
        if (amount > 0 && _YieldKey == 0) {
            emit RewardsPaid(user, YieldBoxRewarded, rewarded, amount);

            RewardsClaimed[YieldBoxRewarded].freeDaysClaimed += rewarded;
        }
    ...
    function getFunds(
        ...
        require(
            IERC20Upgradeable(tokenAddress).transferFrom(user, to, _amount),
            "funds transfer to extension Failed"
        );

        return true;
    }

    function setRewardsExtension(address extension) public onlyOwner {
        RewardsExtension = extension;
    }

    function setRouterAddress(address _router) public onlyOwner {
        IRewardsExtension(RewardsExtension).setRouterAddress(_router);
    }

    function getRewardsExtension() external view returns (address) {
        return RewardsExtension;
    }

    modifier onlyRewardsExtension() {
        require(
            _msgSender() == address(RewardsExtension),
...
interface IRewards {
    function REWARDS_PER_DAYS() external view returns (uint256);
    ...
    function getRewardsDays(
        ...
        uint256 YieldBoxIds,
        uint64 time
    ) external view returns (uint256 rewardDays);
...
interface IRewardsExtension {
    function pauseReward(bool status) external;
    ...
    function getStakedDays(
        ...
        uint64[] memory endStaked,
        uint256 YieldBoxIds
    ) external view returns (uint256 amountDays);
...
interface IYieldBox is I_ValiFiNFT {
    ...
    function createFor(address _user, address _currency) external;

    function _getVaLiFiPriceinStable() external view returns (uint256 price);
}
