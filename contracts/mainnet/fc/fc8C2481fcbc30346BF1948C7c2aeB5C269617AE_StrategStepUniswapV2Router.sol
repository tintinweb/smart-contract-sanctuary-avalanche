...
contract StrategStepUniswapV2Router is StrategStep {
    ...
    function exit(bytes calldata _parameters) external {
        ...
        IERC20(parameters.tokenOut).approve(address(parameters.router), amountToSwap);

        address[] memory path = new address[](2 + parameters.intermediateToken.length);

        if(parameters.intermediateToken.length > 0) {
    ...
    function oracleEnter(IStratStep.OracleResponse memory _before, bytes memory _parameters) external view returns (OracleResponse memory) {
        ...
        console.log("amount of token: %s", _findTokenAmount(parameters.tokenIn, before));
        uint256 amountToSwap = _findTokenAmount(parameters.tokenIn, before) * parameters.tokenInPercent / 100;

        if(amountToSwap == 0) {
            amountToSwap = IERC20(parameters.tokenIn).balanceOf(before.vault);
            if(amountToSwap == 0) return before;
            before = _addTokenAmount(parameters.tokenIn, amountToSwap, before);
        }
        ...
        IStratStep.OracleResponse memory _after = _removeTokenAmount(parameters.tokenIn, amountToSwap, before);
    ...
    function oracleExit(IStratStep.OracleResponse memory _before, bytes memory _parameters) external view returns (OracleResponse memory) {
        Parameters memory parameters = abi.decode(_parameters, (Parameters));
        IStratStep.OracleResponse memory before = _before;
        uint256 amountToSwap = _findTokenAmount(parameters.tokenOut, before);

        if(amountToSwap == 0) {
            amountToSwap = IERC20(parameters.tokenOut).balanceOf(before.vault);
            if(amountToSwap == 0) return before;
            before = _addTokenAmount(parameters.tokenOut, amountToSwap, before);
        }
        ...
        IStratStep.OracleResponse memory _after = _removeTokenAmount(parameters.tokenOut, amountToSwap, before);
