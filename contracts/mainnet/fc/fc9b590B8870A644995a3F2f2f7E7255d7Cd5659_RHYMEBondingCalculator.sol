...
contract RHYMEBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(RHYME) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(RHYME), "not a RHYME lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** RHYME.decimals() ) ).div( getTotalValue( _pair ) );
    }
}