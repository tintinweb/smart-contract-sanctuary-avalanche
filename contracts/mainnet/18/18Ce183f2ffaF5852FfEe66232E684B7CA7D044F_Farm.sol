...
contract Farm is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
  ...
  event EquipmentAttached(
    address indexed owner,
    uint256 indexed equipmentId,
    uint256 indexed tokenId,
    uint256 typeIndex
  );
  event EquipmentDetached(
    address indexed owner,
    uint256 indexed equipmentId,
    uint256 indexed tokenId,
    uint256 typeIndex
  );
  event MutantGenerated(address indexed owner, uint256 indexed superMutantId);

  // Reference to the SpaceFarmer NFT contract
  SpaceFarmer private game;
  // Reference to the $JUICE contract for minting $JUICE earnings
  IBurnableToken private juice;

  IEquipment private equipment;
  ...
  // Equipment id => token id
  mapping(uint256 => uint256) public equipmentIdToTokenId;

  // Address of the owner => Equipment type index => how many have been attached
  mapping(address => mapping(uint256 => uint256))
    private equipmentTypeToAmountAttached;

  // Token id => Equipment ids
  mapping(uint256 => uint256[]) private tokenIdToEquipments;

  // Hero => how many are staked in total
  mapping(Hero => uint256) public heroToAmountStaked;

  // Address => total juice produced by that address
  mapping(address => uint256) private addressTojuiceProduced;

  // Token id of Super Mutant => last time the mutant generation lottery was executed for it
  mapping(uint256 => uint256) private mutantGenerationTimestamp;

  // any rewards distributed when no Farmers are staked
  uint256 private unaccountedRewards;
  // amount of $JUICE due for each score point staked
  uint256 public juicePerSpaceFarmer;

  struct JuiceSettings {
    // Rate of JUICE the mutant generates each day
    uint256 mutantRate;
    // Rate of JUICE the super mutant generates each day
    uint256 superMutantRate;
    // The amount of JUICE catched by the Space Farmer on claim
    uint256 claimTax;
    // The amount of JUICE catched by the Space Farmer on unstake
    uint256 unstakeTax;
  }

  JuiceSettings public juiceSettings;

  // Emergency rescue to allow unstaking without any checks but without $JUICE
  bool public rescueEnabled;

  bool public canClaim;

  /**
   * @param _game reference to the SpaceFarmer NFT contract
   * @param _juice reference to the $JUICE token
   * @param _equipment reference to the Equipment contract
   */
  constructor(
    address _game,
    address _juice,
    ...
    require(
      _game != address(0) && _juice != address(0) && _equipment != address(0),
      "Invalid address"
    );
    game = SpaceFarmer(_game);
    juice = IBurnableToken(_juice);
    equipment = IEquipment(_equipment);
    juiceSettings = JuiceSettings({
      // 100 JUICE per day
      mutantRate: 100 ether,
      // 200 JUICE per day
      superMutantRate: 200 ether,
      // 30 percent
      claimTax: 30,
      // 40 percent
      unstakeTax: 40
  ...
  /**
   * Stake the given tokens
   * @param account Address of the staker
   * @param tokenIds Ids of the tokens
   */
  function stakeMany(address account, uint256[] calldata tokenIds)
    external
    whenNotPaused
    nonReentrant
  {
    ...
    require(
      (account == sender && account == tx.origin) || sender == address(game),
      "Not allowed"
    );
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      if (sender != address(game)) {
        ...
        require(game.ownerOf(tokenIds[i]) == sender, "Not owner of tokens");
        game.transferFrom(sender, address(this), tokenIds[i]);
      }
      _stakeSingle(account, tokenIds[i]);
    }
  }

  /**
  * @dev Stake a single token
   */
  function _stakeSingle(address account, uint256 tokenId)
    private
    whenNotPaused
  {
    // Get the hero
    Hero hero = getHero(tokenId);
    stakes[tokenId] = Stake({
      owner: account,
      tokenId: tokenId,
      juicePerSpaceFarmer: juicePerSpaceFarmer,
      timestamp: block.timestamp
    });
    // Keep track of the amount staked for each hero
    heroToAmountStaked[hero] += 1;
    // Map this token to its owner
    ownerToTokenIds[account].push(tokenId);
    if (hero == Hero.SuperMutant) {
      // If it's a super mutant we need to store a separate
      // timestamp to allow to generate the mutant with 3%
      // percent chance for each day (on claim)
      mutantGenerationTimestamp[tokenId] = block.timestamp;
    }
    emit TokenStaked(hero, account, tokenId, juicePerSpaceFarmer);
  }

  /***CLAIMING / UNSTAKING */

  /**
   * Realize $JUICE earnings and optionally unstake tokens 
   * @param tokenIds the ids of the tokens to claim earnings from
   * @param unstake whether or not to unstake ALL of the tokens listed in tokenIds
   */
  function claimMany(uint256[] calldata tokenIds, bool unstake)
    external
    nonReentrant
  {
    ...
    uint256 owed = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      if (getHero(tokenIds[i]) != Hero.SpaceFarmer) {
        owed += _claimMutant(tokenIds[i], unstake);
      } else {
        owed += _claimSpaceFarmer(tokenIds[i], unstake);
      }
    }
    if (owed == 0) {
      return;
    }
    // Keep tracked of how much JUICE each address has ever produced
    addressTojuiceProduced[_msgSender()] += owed;
    juice.mint(_msgSender(), owed);
  }

  function _claimMutant(uint256 tokenId, bool unstake)
    internal
    returns (uint256 owed)
  {
    address sender = _msgSender();
    Stake memory stake = stakes[tokenId];
    require(stake.owner == sender, "Not owner");
    Hero hero = getHero(tokenId);
    // Get the effect of the equipment attached to this token if any
    (uint256 supplementaryJuice, uint256 juiceFactor) = getEquipmentEffects(
      tokenId,
      hero
    );
    // Calculate the daily juice rate according to
    // the type of mutant and the effects of the equipments
    uint256 juiceRate = (
      hero == Hero.SuperMutant
        ? juiceSettings.superMutantRate
        : juiceSettings.mutantRate
    ) *
      juiceFactor +
      supplementaryJuice;
    owed = ((block.timestamp - stake.timestamp) * juiceRate) / 1 days;
    // If it's a Super Mutant, and if we are in mint phase Gen1 with enough
    // mutant left in the supply, we run the lottery (with 3% chance)
    // on whether this claim will generate mutants for each days passed since
    // last time this lottery was run
    if (
      hero == Hero.SuperMutant &&
      game.mintPhase() == MintPhase.Gen1 &&
      game.getSupplyLeft(Hero.Mutant) > 0
    ) {
      // Get the time passed since last time this lottery was run for this token
      uint256 timelapse = block.timestamp - mutantGenerationTimestamp[tokenId];
      // Get how many days passed
      uint256 daysPassed = timelapse / 1 days;
      // If more than one day then we need to compute
      if (daysPassed >= 1) {
        // For each day we pick a random number
        for (uint256 i = 0; i < daysPassed; i++) {
          uint256 randomNumber = random(tokenId ^ i) % 100;
          // Each day is a 3% chance of minting a mutant for a super mutant
          if (randomNumber < 3) {
            // Mint a random mutant
            game.genericMint(1, false, Hero.Mutant, sender);
            emit MutantGenerated(sender, tokenId);
          }
        }
      }
      // Take the remaining hours of the day that have not been consumed
      // into account
      mutantGenerationTimestamp[tokenId] =
        block.timestamp -
        (timelapse - daysPassed * 1 days);
    }
    if (unstake) {
      // Pay the tax to the farm
      _paySpaceFarmerTax((owed * juiceSettings.unstakeTax) / 100);
      owed = (owed * (100 - juiceSettings.unstakeTax)) / 100;
      // Transfer the token back to its owner
      game.transferFrom(address(this), sender, tokenId);
      // Removing any reference to this stake
      delete stakes[tokenId];
      // Update the count of the mutant or super mutants accordingly
      heroToAmountStaked[hero] -= 1;
      // Remove the mapping from owner to this token
      ownerToTokenIds[sender].remove(tokenId);
    } else {
      // Pay the tax to the farm
      _paySpaceFarmerTax((owed * juiceSettings.claimTax) / 100);
      owed = (owed * (100 - juiceSettings.claimTax)) / 100;
      // Reset the stake
      stakes[tokenId] = Stake({
        owner: sender,
        tokenId: tokenId,
        juicePerSpaceFarmer: 0,
        timestamp: block.timestamp
      });
    }
    emit Claim(hero, tokenId, unstake, sender, owed);
  }

  function _claimSpaceFarmer(uint256 tokenId, bool unstake)
    internal
    returns (uint256 owed)
  {
    ...
    // The Space Farmer doesn't have a daily production of JUICE
    // only tax them from the mutants and super mutants
    // But the equipment can add to the daily Space Farmer production
    uint256 bonusJuice = ((block.timestamp - stake.timestamp) *
      supplementaryJuice) / 1 days;
    owed =
      (juicePerSpaceFarmer - stake.juicePerSpaceFarmer) *
      juiceFactor +
      bonusJuice;
    if (unstake) {
      // Transfer the token back to its owner
      game.transferFrom(address(this), _msgSender(), tokenId);
      // Update the count of staked Space Farmer accordingly
      heroToAmountStaked[Hero.SpaceFarmer] -= 1;
      // Removing any reference of this stake
      delete stakes[tokenId];
      // Remove the mapping from the owner to this token
    ...
    emit Claim(Hero.SpaceFarmer, tokenId, unstake, _msgSender(), owed);
  }

  function getEquipmentEffects(uint256 tokenId, Hero hero)
    private
    view
    returns (uint256 juiceAddition, uint256 juiceMultiplier)
  {
    // Start with a multiplier with no effects
    juiceMultiplier = 1;
    // Check if there are any equipment attached to this token
    if (tokenIdToEquipments[tokenId].length > 0) {
      for (uint256 i = 0; i < tokenIdToEquipments[tokenId].length; i++) {
        // Get the id of the equipment
        uint256 equipmentId = tokenIdToEquipments[tokenId][i];
        // Get its type
        uint256 equipmentType = equipment.getTokenType(equipmentId);
        // Get its expiry
        uint256 expiry = equipment.getTokenExpiry(equipmentId);
        // If expired we ignore it
        if (expiry > 0 && block.timestamp >= expiry) {
          // Equipment expired
          continue;
        }
        // Get the properties of the equipment
        Properties memory properties = equipment.getTypeProperties(
          equipmentType
        );
        // If it applies to this hero then we update the effect
        if (properties.hero == hero) {
          // Add the supplementary JUICE
          juiceAddition += properties.juiceAddition;
          // Do not multiply each multiplier with each other
          // to avoid high exponentiation
          // Only one multiplier at a time can be used
          // So we just take the highest one
          if (properties.juiceMultiplier > juiceMultiplier) {
            juiceMultiplier = properties.juiceMultiplier;
          }
        }
      }
    }
  ...
  function attachEquipment(uint256 tokenId, uint256 equipmentId) external {
    require(game.ownerOf(tokenId) == address(this), "Not staked");
    address sender = _msgSender();
    Stake memory stake = stakes[tokenId];
    require(stake.owner == sender, "Not owner");
    require(equipment.ownerOf(equipmentId) == sender, "Not owner of equiment");
    require(equipmentIdToTokenId[equipmentId] != tokenId, "Alreay attached");
    // Get the type
    uint256 equipmentType = equipment.getTokenType(equipmentId);
    // Get the properties
    Properties memory properties = equipment.getTypeProperties(equipmentType);
    // The equipment must apply to this hero
    require(properties.hero == getHero(tokenId), "Cannot attach to this NFT");
    // Check that this address didn't reach its limit
    require(
      equipmentTypeToAmountAttached[sender][equipmentType] <
        properties.limitPerAddress,
      "Too many equipments attached"
    );
    // Update the necessary mappings
    equipmentIdToTokenId[equipmentId] = tokenId;
    tokenIdToEquipments[tokenId].push(equipmentId);
    // Update the count of equipment attached for this address for easier check later
    equipmentTypeToAmountAttached[sender][equipmentType] += 1;
    // Transfer the equipment to this contract (attachment = staking)
    equipment.transferFrom(sender, address(this), equipmentId);
    emit EquipmentAttached(sender, equipmentId, tokenId, equipmentType);
  }

  function detachEquipment(uint256 tokenId, uint256 equipmentId) external {
    require(game.ownerOf(tokenId) == address(this), "Not staked");
    address sender = _msgSender();
    Stake memory stake = stakes[tokenId];
    // Can detach the equipment whether the token is staked or not
    require(
      stake.owner == sender || game.ownerOf(tokenId) == sender,
      "Not owner"
    );
    require(equipmentIdToTokenId[equipmentId] == tokenId, "Not attached");
    // Update the mapping accordingly
    delete equipmentIdToTokenId[equipmentId];
    uint256 equipmentType = equipment.getTokenType(equipmentId);
    equipmentTypeToAmountAttached[sender][equipmentType] -= 1;
    tokenIdToEquipments[tokenId].remove(equipmentId);
    // Transfer the equipment back to its owner
    equipment.transferFrom(address(this), sender, equipmentId);
    emit EquipmentDetached(sender, equipmentId, tokenId, equipmentType);
  }
  ...
  function rescue(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(stake.owner == _msgSender(), "Not owner");
      // Transfer back to its owner
      game.transferFrom(address(this), _msgSender(), tokenId);
      // Remove any reference of this stake
      delete stakes[tokenId];
      // Remove the mapping from the owner to this token
      ownerToTokenIds[_msgSender()].remove(tokenId);
      Hero hero = getHero(tokenId);
      // Update state according to the type of token
      heroToAmountStaked[hero] -= 1;
      emit Claim(hero, tokenId, true, _msgSender(), 0);
    }
  }

  /**
   * add $JUICE to claimable pot for the Space Farmers
   * @param amount $JUICE to add to the pot
   */
  function _paySpaceFarmerTax(uint256 amount) internal {
    if (heroToAmountStaked[Hero.SpaceFarmer] == 0) {
      // if there's no staked Farmers
      unaccountedRewards += amount;
      // keep track of $JUICE due to Farmers
      return;
    }
    // makes sure to include any unaccounted $JUICE
    juicePerSpaceFarmer +=
      (amount + unaccountedRewards) /
      heroToAmountStaked[Hero.SpaceFarmer];
  ...
  /**
   * Get the hero of a token
   * @param tokenId the id of the token to check
   */
  function getHero(uint256 tokenId) public pure returns (Hero hero) {
    if (
      (tokenId > 1000 && tokenId <= 10000) ||
      (tokenId > 11000 && tokenId <= 56000)
    ) {
      return Hero.Mutant;
    } else if (
      (tokenId > 10000 && tokenId <= 11000) ||
      (tokenId > 61000 && tokenId <= 62000)
    ) {
      return Hero.SuperMutant;
    } else {
      return Hero.SpaceFarmer;
    }
  }
      ...
      uint256(
        keccak256(
          abi.encodePacked(
            ...
            block.timestamp,
            seed,
            heroToAmountStaked[Hero.Mutant],
            heroToAmountStaked[Hero.SpaceFarmer]
          )
        )
  ...
  function setJuiceSettings(JuiceSettings memory _juiceSettings)
    external
    onlyOwner
  {
    juiceSettings = _juiceSettings;
  }
  ...
  function getJuiceProducedBy(address addr) external view returns (uint256) {
    return addressTojuiceProduced[addr];
  }

  function getTokenAttachedEquipments(uint256 tokenId)
    external
    view
    returns (uint256[] memory)
  {
    return tokenIdToEquipments[tokenId];
  }
}
...
struct Properties {
  // Value to scale juice daily production by
  uint256 juiceMultiplier;
  // Value to add to juice daily production
  uint256 juiceAddition;
  // How many can be used by a single address
  uint256 limitPerAddress;
  // How long the equipment last
  uint256 duration;
  // Which hero this equipment applies to
  Hero hero;
}

interface IEquipment is IERC721, IERC721Enumerable {
  function safeMint(address to, uint256 typeIndex) external;

  function setTypePrice(uint256 typeIndex, uint256 price) external;

  function setTypeProperties(uint256 typeIndex, Properties memory properties)
    external;

  function getTypePrice(uint256 typeIndex) external view returns (uint256);

  function getTypeProperties(uint256 typeIndex)
    external
    view
  ...
  function getTokenExpiry(uint256 tokenId) external view returns (uint256);
}
