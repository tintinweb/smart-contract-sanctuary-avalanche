...
contract RockGenesisRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ROCK to distribute.
        uint256 depositFee; // Deposit Fee of this pool.
        uint256 lastRewardTime; // Last time that ROCK distribution occurs.
        uint256 accRockPerShare; // Accumulated ROCK per share, times 1e18. See below.
    ...
    // TESTNET
    // uint256 public rockPerSecond = 3.0555555 ether; // 11000 ROCK / (1h * 60min * 60s)
    // uint256 public runningTime = 24 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    uint256 public rockPerSecond = 0.11574 ether; // 20000 ROCK / (24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "RockGenesisPool: caller is not the operator");
        _;
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, depositFee: _depositFee, lastRewardTime: _lastRewardTime, accRockPerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
    ...
    function safeRockTransfer(address _to, uint256 _amount) internal {
        uint256 _rockBalance = rock.balanceOf(address(this));
        if (_rockBalance > 0) {
            if (_amount > _rockBalance) {
                rock.safeTransfer(_to, _rockBalance);
            } else {
