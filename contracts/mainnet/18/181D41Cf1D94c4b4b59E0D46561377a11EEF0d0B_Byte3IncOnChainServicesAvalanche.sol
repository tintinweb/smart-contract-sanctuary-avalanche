...
contract Byte3IncOnChainServicesAvalanche is Byte3IncOnChainServices {
  ...
  // token0 = WAVAX
  // token1 =  USDC
  IUniswapV2Pair constant PangolinUsdcPool = IUniswapV2Pair(0x0e0100Ab771E9288e0Aa97e11557E6654C3a9665);
  ...
  function convertUsdToWei(uint256 usdAmount) external view override returns (uint256 weiAmount) {
    ...
    weiAmount = (_getPangolinUSDC(usdAmount) + _getTraderJoeUSDC(usdAmount)) / 2;
    // add 1% overhead
  ...
  function purchaseFractionToken(
    ...
    usdAmount += serviceFee;
    uint256 amountIn = _swap(address(this), usdAmount);
    // mint FRACT10N for USDC
    uint256 fractionAmount = usdAmount - serviceFee;
    _fractionToken().mint(recipient, fractionAmount);
    // get remainder
    remainder = msg.value - amountIn;
    if (remainder > 0) {
  ...
  function _swap(address recipient, uint256 usdAmount) internal returns (uint256 amountIn) {
    // get pangolin price
    uint256 pangolinWeiIn = _getPangolinUSDC(usdAmount);
    // get traderjoe price
    uint256 traderjoeWeiIn = _getTraderJoeUSDC(usdAmount);
    // get best price
    amountIn = pangolinWeiIn < traderjoeWeiIn ? pangolinWeiIn : traderjoeWeiIn;
    // check that we have enough value
    require(msg.value >= amountIn, "BYTE3: insufficient msg.value");
    // buy USDC with WAVAX
    if (amountIn == pangolinWeiIn) {
      // swap with pangolin
      _pangolinSwap(recipient, amountIn, usdAmount);
    } else {
      // swap with traderjoe
      _traderjoeSwap(recipient, amountIn, usdAmount);
    }
  }

  function _pangolinSwap(address recipient, uint256 nativeAmount, uint256 usdAmount) internal {
    IWETH wavax = IWETH(WAVAX);
    // wrap the native gas token
    wavax.deposit{value: nativeAmount}();
    // transfer to token pool
    wavax.transfer(address(PangolinUsdcPool), nativeAmount);
    // get USDC
    PangolinUsdcPool.swap(0, usdAmount, recipient, "");
  }

  function _traderjoeSwap(address recipient, uint256 nativeAmount, uint256 usdAmount) internal {
    IWETH wavax = IWETH(WAVAX);
    // wrap the native gas token
    wavax.deposit{value: nativeAmount}();
    // transfer to token pool
    wavax.transfer(address(TraderJoeUsdcPool), nativeAmount);
    ERC20 usdc = ERC20(USDC);
    uint256 balance = usdc.balanceOf(address(this));
    // get USDC
    TraderJoeUsdcPool.swap(true, recipient);
    // check that valid amount was supplied
    require(usdc.balanceOf(address(this)) - balance >= usdAmount, "BYTE3: invalid swap amount");
  }

  function _getPangolinUSDC(uint256 usdAmount) internal view returns (uint256 weiAmount) {
    // token0 = WAVAX
    // token1 =  USDC
    (uint112 _reserve0, uint112 _reserve1, ) = PangolinUsdcPool.getReserves();
  ...
  function _getTraderJoeUSDC(uint256 usdAmount) internal view returns (uint256 weiAmount) {
    // tokenX = WAVAX
    // tokenY =  USDC
    (uint128 amountIn, , ) = TraderJoeUsdcPool.getSwapIn(uint128(usdAmount), true);
    weiAmount = amountIn;
  }
}
