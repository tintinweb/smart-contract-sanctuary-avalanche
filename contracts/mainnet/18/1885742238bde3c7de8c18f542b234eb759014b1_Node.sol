...
contract Node {
    ...
    uint256[] public tierAmounts = [1 ether, 500 ether, 5000 ether];
    struct User {
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            WINE.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(WINE.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(WINE.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        WINE.safeTransferFrom(_sender, address(this), tierPrice);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(WINE).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getWineBalancePool();
    ...
    function getWineBalance() public view returns (uint256) {
        return IERC20(WINE).balanceOf(address(this));
    }

     function getWineBalancePool() public view returns (uint256) {
        return getWineBalance().sub(total_rewards).sub(treasury_rewards);
    }
