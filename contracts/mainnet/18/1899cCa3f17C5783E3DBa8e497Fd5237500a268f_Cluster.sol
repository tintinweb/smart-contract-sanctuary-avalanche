...
contract Cluster is ERC721, Ownable {
    ...
    mapping(uint => uint) citizenWithdrawTime;
    mapping(uint => uint) clusterWithdrawTime;
    ...
    function generateCluster(address _to, uint _id) onlyACM payable external returns (uint){
        require(clusters.length < 10000, "Sold out");
        clusterWithdrawTime[_id] = block.timestamp;
        clusters.push(Clusters(_id, 0));
        _safeMint(_to,_id);
        return _id;
    }

    function addCitizen(uint _idCitizen, uint _idCluster) public {
        require(ICitizen(_ctzn)._ownerOf(_idCitizen) == msg.sender, "OO");
        require(_idCluster < clusters.length, "Not exist!");
        require(citizenAdded[_idCitizen] == false, "Already joined!");
        citizenAdded[_idCitizen] = true;
        citizenWithdrawTime[_idCitizen] = block.timestamp;
        citizensOfCluster[_idCluster].push(_idCitizen);
        clusters[_idCluster].population += 1;
        citizenToCluster[_idCitizen] = _idCluster;
    }

    function citizensCluster(uint _idCitizen) external view returns(uint) {
        return citizenToCluster[_idCitizen];
    }

    function getCitizensOfCluster(uint _idCluster) external view returns(uint[] memory) {
        return citizensOfCluster[_idCluster];
    }
    ...
    function daysOfCitizen(uint _idCitizen) public view returns (uint) {
        uint daysOfCtzn = block.timestamp - citizenWithdrawTime[_idCitizen];
        if(daysOfCtzn <= 298 * 86400){return daysOfCtzn;}else{return 298 * 86400;}
    }
    function daysOfCluster(uint _idCluster) public view returns (uint) {
        uint daysOfClstr = block.timestamp - clusterWithdrawTime[_idCluster];
        if(daysOfClstr <= 298 * 86400){return daysOfClstr;}else{return 298 * 86400;}
    }

    function citizenRatio(uint _idCitizen) public view returns (uint) {
    ...
    function citizenAnyTokenAllowance(uint _idCitizen) public view returns (uint) {
        uint cAllowance = 5 * 9 * daysOfCitizen(_idCitizen) * citizenRatio(_idCitizen) * 10**18;
        uint citizenAllowance = cAllowance / 86400;
        return citizenAllowance;
    }
    function clusterOwnerAnyTokenAllowance(uint _idCluster) public view returns (uint) {
        uint cOwnerAllowance = 5 * 9 * daysOfCluster(_idCluster) * clusters[_idCluster].population * 10**18;
        uint clusterOwnerAllowance = cOwnerAllowance / 86400;
        return clusterOwnerAllowance;
    }

    function clusterAnyTokenMint(uint _idCluster) public returns (bool) {
        require(ownerOf(_idCluster) == msg.sender, "OO");
        clusterWithdrawTime[_idCluster] = block.timestamp;
        uint allowance = clusterOwnerAnyTokenAllowance(_idCluster);
        IAnyGameToken(_anyToken).mint(ownerOf(_idCluster), allowance);
        return true;
    }
    function citizenAnyTokenMint(uint _idCitizen) public returns (bool) {
        require(ICitizen(_ctzn)._ownerOf(_idCitizen) == msg.sender, "OO");
        require(citizenAdded[_idCitizen] == true, "Not allowed");
        require(ICitizen(_ctzn).isFulfillNeeded(_idCitizen) == false, "Fulfill First!");
        citizenWithdrawTime[_idCitizen] = block.timestamp;
    ...
    function setAddresses(address _acm, address anyToken, address AnyCitizen) onlyOwner public {
        acm = _acm;
        _anyToken = anyToken;
        _ctzn = AnyCitizen;
    }
    ...
    interface ICitizen {
        ...
        function isFulfillNeeded(uint _id) external view returns (bool);
    }
