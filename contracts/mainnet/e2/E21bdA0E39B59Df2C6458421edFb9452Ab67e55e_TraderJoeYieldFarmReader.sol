...
contract TraderJoeYieldFarmReader {
  ...
  */
  function assetValue() public view returns (uint256) {
    (uint256 _tokenAAssetAmt, uint256 _tokenBAssetAmt) = ITraderJoeYieldFarmManager(manager).assetInfo();
  ...
  */
  function debtValue() public view returns (uint256, uint256) {
    (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt) = ITraderJoeYieldFarmManager(manager).debtInfo();
  ...
  */
  function lpAmt() public view returns (uint256) {
    return ITraderJoeYieldFarmManager(manager).lpTokenAmt();
  }
  ...
  */
  function delta() public view returns (int) {
    (uint256 _tokenAAmt,) = ITraderJoeYieldFarmManager(manager).assetInfo();
    (uint256 _tokenADebtAmt,) = ITraderJoeYieldFarmManager(manager).debtInfo();
...
import "../../../contracts/vaults/trader-joe/TraderJoeYieldFarmReader.sol";

contract $TraderJoeYieldFarmReader is TraderJoeYieldFarmReader {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _vault, address _manager, IChainLinkOracle _priceOracle) TraderJoeYieldFarmReader(_vault, _manager, _priceOracle) {}
