...
contract TraderJoeYieldFarmManager is Ownable {
  ...
  */
  function tokenA() public view returns (address) {
    return ITraderJoeYieldFarmVault(vault).tokenA();
  }
  ...
  */
  function tokenB() public view returns (address) {
    return ITraderJoeYieldFarmVault(vault).tokenB();
  }
  ...
  */
  function lpTokenAmt() public view returns (uint256) {
    (uint256 amt, ) = IMasterChefJoeV3(stakePool).userInfo(stakePoolId, address(this));

    return IJoePair(lpToken()).balanceOf(address(this)) + amt;
  }
  ...
  */
  function assetInfo() public view returns (uint256, uint256) {
    (uint256 tokenAAssetAmt, uint256 tokenBAssetAmt) = IJoeOracle(joeOracle).getLpTokenReserves(
      lpTokenAmt(),
      tokenA(),
      tokenB(),
      lpToken()
  ...
  */
  function _stake() internal {
    ...
    IMasterChefJoeV3(stakePool).deposit(
      stakePoolId,
  ...
  */
  function _swapExcess() internal {
    ...
    if (ITraderJoeYieldFarmVault(vault).strategy() == VaultStrategy.Neutral) {
    ...
    if (ITraderJoeYieldFarmVault(vault).strategy() == VaultStrategy.Long) {
  ...
  */
  function _swapRewardWithFee() internal {
    ...
    uint256 fee = IERC20(tokenB()).balanceOf(address(this))
                  * ITraderJoeYieldFarmVault(vault).perfFee()
                  / SAFE_MULTIPLIER;

    IERC20(tokenB()).safeTransfer(ITraderJoeYieldFarmVault(vault).treasury(), fee);
  }
...
import "../../../contracts/interfaces/oracles/IJoeOracle.sol";

abstract contract $IJoeOracle is IJoeOracle {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/interfaces/staking/trader-joe/IMasterChefJoeV3.sol";

abstract contract $IMasterChefJoeV3 is IMasterChefJoeV3 {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/interfaces/swaps/trader-joe/IJoePair.sol";

abstract contract $IJoePair is IJoePair {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/interfaces/swaps/trader-joe/IJoeRouter01.sol";

abstract contract $IJoeRouter01 is IJoeRouter01 {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/interfaces/swaps/trader-joe/IJoeRouter02.sol";

abstract contract $IJoeRouter02 is IJoeRouter02 {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../../contracts/interfaces/vaults/trader-joe/ITraderJoeYieldFarmVault.sol";

abstract contract $ITraderJoeYieldFarmVault is ITraderJoeYieldFarmVault {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../contracts/vaults/trader-joe/TraderJoeYieldFarmManager.sol";

contract $TraderJoeYieldFarmManager is TraderJoeYieldFarmManager {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";

    constructor(address _vault, address _rewardToken, address _tokenALendingPool, address _tokenBLendingPool, address _router, address _stakePool, uint256 _stakePoolId, address _joeOracle) TraderJoeYieldFarmManager(_vault, _rewardToken, _tokenALendingPool, _tokenBLendingPool, _router, _stakePool, _stakePoolId, _joeOracle) {}
