...
// File contracts/libs/IJoeRouter.sol
...
contract TreasuryUtil is AccessControl {
    ...
    constructor(address _treasuryDAOAddress, address _usdCurrency, address _routerAddress) {
        ...
        WAVAX = IJoeRouter(routerAddress).WAVAX();
    }
    ...
    function _swapTokensForTokens(address from, address to, uint256 amount) internal {
        address[] memory path = new address[](from == WAVAX || to == WAVAX ? 2 : 3);
        if (from == WAVAX || to == WAVAX) {
            path[0] = from;
            path[1] = to;
        } else {
            path[0] = from;
        ...
        IJoeRouter(routerAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
    ...
    function _removeLiquidity(address token, uint256 tokenAmount) internal returns(address, address) {
        ...
        IJoeRouter(routerAddress).removeLiquidity(
            token0,
            token1,
            tokenAmount,
            0,
