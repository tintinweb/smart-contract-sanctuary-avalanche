  ...
  Authorizable
{
  ...
  function getSwapTokensadd(address _pairAdd) public view returns(address,address) {
  	 IUniswapV2Pair _pair = IUniswapV2Pair(_pairAdd);
         return (address(IERC20(_pair.token0())),address(IERC20(_pair.token1())));
  }
  function getSwapTokensDec(address _pairAdd) public view returns(uint256,uint256) {
  	 IUniswapV2Pair _pair = IUniswapV2Pair(_pairAdd);
         return (swapToken0.decimals(),swapToken1.decimals());
  }
  function getSwapTokensRes(address _pairAdd) public view returns(uint256,uint256) {
  	 IUniswapV2Pair _pair = IUniswapV2Pair(_pairAdd);
  ...
  function getSwapTokensInfo(address _pairAdd) public view returns(address,address,uint256,uint256,uint256,uint256) {
  	 IUniswapV2Pair _pair = IUniswapV2Pair(_pairAdd);
  	 (uint256 res0, uint256 res1,) = _pair.getReserves();
         return (address(IERC20(_pair.token0())),address(IERC20(_pair.token1())),swapToken0.decimals(),swapToken1.decimals(),res0,res1);
     }
