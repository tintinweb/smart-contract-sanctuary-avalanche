...
contract ABep20 is AToken, ABep20Interface {
    ...
    function borrowBehalf(address receiver, uint borrowAmount) external returns (uint) {
        return borrowBehalfInternal(receiver, borrowAmount);
    }
...
contract AToken is ATokenInterface, Exponential, TokenErrorReporter {
    ...
    function borrowInternal(uint borrowAmount) internal nonReentrant returns (uint) {
        ...
        return borrowFresh(msg.sender, msg.sender, borrowAmount);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowBehalfInternal(address receiver, uint borrowAmount) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
            return fail(Error(error), FailureInfo.BORROW_ACCRUE_INTEREST_FAILED);
        ...
        // borrowFresh emits borrow-specific logs on errors, so we don't need to
        return borrowFresh(msg.sender, receiver, borrowAmount);
    }
    ...
    /**
      ...
      * @param borrower The account who is borrowing the the amount
      * @param receiver The account where the borrowed amount will be send to
      * @param borrowAmount The amount of the underlying asset to borrow
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowFresh(address payable borrower, address receiver, uint borrowAmount) internal returns (uint) {
         ...
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        doTransferOut(address(uint160(receiver)), borrowAmount);


        /* We emit a Borrow event */
        emit Borrow(borrower, receiver, borrowAmount, vars.accountBorrowsNew, vars.totalBorrowsNew);
...
contract ATokenInterface is ATokenStorage {
     ...
     */
    event Borrow(address borrower, address receiver, uint borrowAmount, uint accountBorrows, uint totalBorrows);
