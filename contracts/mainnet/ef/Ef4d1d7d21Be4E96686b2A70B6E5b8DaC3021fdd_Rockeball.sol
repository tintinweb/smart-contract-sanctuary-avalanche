...
contract Rockeball is ERC721URIStorage, ERC721Burnable, Ownable, Whitelist {
  ...
  uint256 public nftWlLimit = 1;

  bool private _mintActive;
  uint256 public _price;
  uint256 public _priceWL;

  constructor() ERC721("Rockeballs", "RBALL") {
    _customBaseURI = 'https://not-a.real.website/';
    _maxMintable = 491; // 491 max supply
  ...
  function mint(uint256 quantity) public payable {
    ...
    if(whitelist[msg.sender]) {
          ...
          require(ownerMintedCount + quantity <= nftWlLimit, "max NFT per address exceeded");
          require(msg.value >= _priceWL * quantity, "insufficient funds");
          whitelist[msg.sender] = false;
    }
    else {
        require(msg.value >= _price * quantity, "AVAX sent is not correct");
    }
  ...
  function howManyBallByType ( uint _type) external view returns (uint) {
      uint max = balanceOf(msg.sender);
      uint count = 0;

      for (uint i = 0; i < max; i++) {
        if(types[_ownedTokens[msg.sender][i]] == _type)
          count++;
      }
        return count;
  }

  function setTypes(uint[] memory _types, uint from) external onlyOwner {
      for (uint i = 0; i < _types.length; i++) {
          types[from+i] = _types[i];
      }
  }
