...
// File: contracts\DAOSwapAVAX.sol
...
contract DAOHqSwapAVAX is Ownable, ReentrancyGuard, DaoHqRouter {
    using SafeERC20 for IERC20;

    uint256 public fees;

    event AVAXTransfer(address indexed from, address indexed to, uint256 value);
    ...
    function transfer(uint amount, address to) internal {
        ...
        emit AVAXTransfer(address(this), to, amountToTransfer);
    }
    ...
    external nonReentrant returns (uint amountOut) {
        ...
        require(initialBalance + amountOut == address(this).balance, "AVAX was not returned correctly");
    ...
    /**
     * @dev to recieve AVAX from DaoHqRouter when swaping
     */
