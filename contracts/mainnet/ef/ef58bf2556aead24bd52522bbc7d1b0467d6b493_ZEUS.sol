...
contract ZEUS is Ownable, PaymentSplitter {
    ...
    address tokenAddress = 0x4156F18bF7C1ef04248632C66Aa119De152D8f2E;
    ...
    function createNodeWithTokens(
        ...
        uint256 userNodes = nodeRewardManager._getUserNodeCount(sender);
        require(userNodes < 100, "Maximum number of nodes reached");
        require(
            sender != address(0),
    ...
    function cashoutAllExtraBalances() public {
        ...
        uint256 smallestNodePrice = nodeRewardManager.getNodePriceByType(0);
        require(amount < smallestNodePrice, "Cashout unavailable");
        require(
            amount > 0,
            "MANIA CSHT: You don't have enough reward to cash out"
        );
    ...
    function compoundNode(string memory nodeName, uint32 nodeType) external {
        address sender = _msgSender();
        // get the available number of nodes in the account
        uint256 userNodes = nodeRewardManager._getUserNodeCount(sender);
        require(userNodes < 100, "Maximum amount reached for this account");
        require(
            sender != address(0),
            "NODE CREATION:  creation from the zero address"
        );
