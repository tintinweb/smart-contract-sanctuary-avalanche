/**
 *Submitted for verification at snowtrace.io on 2022-06-08
*/
...
contract Rewarder_Mint_RatePerToken is IRewarder, TokenOrAvaxTransfer, Ownable {
    struct UserInfo {
        uint224 amount;
    ...
    uint256 private constant DAY_DIVISOR = REWARD_TOKEN_DIVISOR * SECONDS_PER_DAY;

    // mapping depositToken => rate at which rewardToken is distributed to token depositors, per day, per token
    // e.g. if rewardRatesPerDayPerToken[depositToken] = 7e18, then a depositor of 3e18 'depositToken' earns 21e18 (7e18 * 3e18 / 1e18) rewardTokens per day
    mapping(address => uint256) public rewardRatesPerDayPerToken;
    // token => depositor => UserInfo
    mapping(address => mapping(address => UserInfo)) public userInfo;

    modifier onlyHauntedHouse {
        require(
            msg.sender == hauntedHouse,
            "Only hauntedHouse can call this function."
        );
        _;
    }

    constructor (address _rewardToken, address _hauntedHouse, uint32 _startTime) {
        rewardToken = _rewardToken;
        hauntedHouse = _hauntedHouse;
        startTime = _startTime;
    }

    function pendingTokens(address depositToken, address user)
        ...
        returns (address[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        UserInfo memory _userInfo = userInfo[depositToken][user];
        uint256 pendingReward;
        if (uint32(block.timestamp) > startTime) {
            if (_userInfo.amount != 0) {
                uint32 timeElapsed = uint32(block.timestamp) - _userInfo.lastRewardTimestamp;
                pendingReward = (_userInfo.amount * rewardRatesPerDayPerToken[depositToken] * timeElapsed) / DAY_DIVISOR;
            } else {
                uint32 timeElapsed = uint32(block.timestamp) - startTime;
                (uint256 previousShareAmount, ) = IHauntedHouse(hauntedHouse).userInfo(depositToken, user);
                pendingReward = (previousShareAmount * rewardRatesPerDayPerToken[depositToken] * timeElapsed) / DAY_DIVISOR;
            }           
        }

        _rewardAmounts[0] = pendingReward;
        return (_rewardTokens, _rewardAmounts);
    }

    function setRewardRatePerDayPerToken(address token, uint256 newRewardRate) external onlyOwner {
        rewardRatesPerDayPerToken[token] = newRewardRate;
    }
    ...
    function onZBoofiReward(address depositToken, address caller, address recipient, uint256, uint256 previousShareAmount, uint256 newShareAmount)
        override external onlyHauntedHouse
    {
        UserInfo memory _userInfo = userInfo[depositToken][caller];
        uint256 pendingReward;
        if (uint32(block.timestamp) > startTime) {
            if (_userInfo.amount != 0) {
                uint32 timeElapsed = uint32(block.timestamp) - _userInfo.lastRewardTimestamp;
            ...
            // update timestamp
            _userInfo.lastRewardTimestamp = uint32(block.timestamp);
        } else {
            // update timestamp
            _userInfo.lastRewardTimestamp = startTime;
        }

        //update share amount
        _userInfo.amount = uint224(newShareAmount);
        // write to storage slot
        userInfo[depositToken][caller] = _userInfo;

        if (pendingReward > 0) {
            IMintableToken(rewardToken).mint(recipient, pendingReward);            
        }
    }
}