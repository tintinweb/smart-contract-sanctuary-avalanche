...
contract Node {
    ...
    uint256[] public tierAmounts = [5 ether, 500 ether, 5000 ether];
    struct User {
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            GRAPE.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(GRAPE.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(GRAPE.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        GRAPE.safeTransferFrom(_sender, address(this), tierPrice);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(GRAPE).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getGrapeBalancePool();
    ...
    function getGrapeBalance() public view returns (uint256) {
        return IERC20(GRAPE).balanceOf(address(this));
    }

     function getGrapeBalancePool() public view returns (uint256) {
        return getGrapeBalance().sub(total_rewards).sub(treasury_rewards);
    }
