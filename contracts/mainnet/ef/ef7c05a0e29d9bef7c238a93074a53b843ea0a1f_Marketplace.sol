...
contract Marketplace is IMarketplace, OwnableUpgradeable {
    ...
    uint256 public totalBlacklist;
    mapping(address => bool) public blacklisted;
    mapping(uint256 => address) public blacklistIdx;
    ...
    function _createSingleAsk(
        ...
        asks[address(NFT)][tokenId] = Ask({creator: msg.sender, price: price});
    ...
    function cancelAsks(
        ...
        for (uint256 i = 0; i < NFT.length; i++) {
            ...
            if (asks[NFTAddress][tokenId[i]].creator != msg.sender)
    ...
    function acceptAsk(
        ...
        if (NFT.ownerOf(tokenId) != ask.creator) revert AskCreatorNotOwner();
        ...
        NFT.safeTransferFrom(ask.creator, msg.sender, tokenId);
