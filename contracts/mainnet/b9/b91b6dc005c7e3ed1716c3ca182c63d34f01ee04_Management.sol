...
contract Management is Ownable {
    ...
    // Events
    event CREATE_NODE(address _account, string _name, uint256 _rewardPerDay, uint256 _createTime); // ok
    event AIREDROP_NODE(address _account, string _name, uint256 _rewardPerDay, uint256 _createTime); // ok
    ...
    function createNode(address account, string memory _name, uint256 _rewardPerDay) external onlyManager {
    ...
    function airdropNode(address account, string memory _name, uint256 _rewardPerDay) external onlyManager {
        require(nodeCountOfUser[account] < nodeLimit, "MANAGEMENT: AIREDROP NODE LIMIT ERROR");
    ...
    function calculateAvailableReward(address account) external view onlyManager returns(uint256) {
    ...
    function compoundNode(address account, uint256 amount) external onlyManager returns(uint256) {
    ...
    function getNodeNames(address account) external view onlyManager returns (string memory) {
