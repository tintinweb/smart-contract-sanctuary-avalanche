...
interface IStrategyLegacy {
    ...
    function withdraw(uint256 _amount) external;    // MAXIMUS STAKING POOL ONLY
    ...
    function withdrawableBalanceOf(address account) external view returns (uint);   // MAXIMUS STAKING POOL ONLY (deprecated)
    function profitOf(address account) external view returns (uint _usd, uint _maximus, uint _avax);
    function earned(address account) external view returns (uint);
    function tvl() external view returns (uint);    // in USD
    function apy() external view returns (uint _usd, uint _maximus, uint _avax);
...
interface IStrategyHelper {
    ...
    function profitOf(IMaximusMinter minter, address _flip, uint amount) external view returns (uint _usd, uint _maximus, uint _bnb);
    ...
    function apy(IMaximusMinter minter, uint pid) external view returns(uint _usd, uint _maximus, uint _bnb);
...
// File: contracts/vaults/MaximusPool.sol
...
contract MaximusPool is IStrategyLegacy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeMath for uint256;
    ...
    IERC20 public constant stakingToken = IERC20(0x885d748C00A279B67A7749EC6b03301700dd0455);
    ILydiaRouter private constant LYDIA_ROUTER = ILydiaRouter(0xA52aBE4676dbfd04Df42eF7755F01A3c41f28D27);
    IPriceCalculator public constant priceCalculator = IPriceCalculator(0xeABe3A541058f6894C97Ac11c257C7A3755f1741);

    /* ========== STATE VARIABLES ========== */

    IERC20 public rewardsToken; // maximus/avax flip
    ...
    function initialize() external initializer {
        ...
        IERC20(WAVAX).safeApprove(address(LYDIA_ROUTER), uint(- 1));
        stakingToken.safeApprove(address(LYDIA_ROUTER), uint(- 1));
    }
    ...
    function profitOf(address account) override public view returns (uint _usd, uint _maximus, uint _avax) {
        _usd = 0;
        _maximus = 0;
        (_avax,) = priceCalculator.valueOfAsset(address(rewardsToken), earned(account));
    }

    function tvl() override public view returns (uint) {
        (uint priceInAVAX,) = priceCalculator.valueOfAsset(address(stakingToken), _totalSupply);
        return priceInAVAX;
    }

    function apy() override public view returns (uint _usd, uint _maximus, uint _avax) {
        ...
        (uint maximusPrice,) = priceCalculator.valueOfAsset(address(stakingToken), 1e18);
        (uint flipPrice,) = priceCalculator.valueOfAsset(address(rewardsToken), 1e18);

        _usd = 0;
        _maximus = 0;
        _avax = rewardPerTokenPerSecond.mul(365 days).mul(flipPrice).div(maximusPrice);
    }
    ...
    function _deposit(uint256 amount, address _to) private nonReentrant notPaused updateReward(_to) {
        ...
        emit Staked(_to, amount, msg.sender);
    }
    ...
    function getReward() override public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            reward = _flipToWAVAX(reward);
            IERC20(WAVAX).safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    function _flipToWAVAX(uint amount) private returns (uint reward) {
        (uint rewardMaximus,) = LYDIA_ROUTER.removeLiquidity(
            address(stakingToken), WAVAX,
            amount, 0, 0, address(this), block.timestamp);
        address[] memory path = new address[](2);
        path[0] = address(stakingToken);
        path[1] = WAVAX;
        LYDIA_ROUTER.swapExactTokensForTokens(rewardMaximus, 0, path, address(this), block.timestamp);

        reward = IERC20(WAVAX).balanceOf(address(this));
    }
    ...
    function info(address account) override external view returns(UserInfo memory) {
        ...
        (uint usd, uint maximus, uint avax) = profitOf(account);
        profit.usd = usd;
        profit.maximus = maximus;
        profit.avax = avax;
        ...
        APY memory poolAPY;
        (usd, maximus, avax) = apy();
        poolAPY.usd = usd;
        poolAPY.maximus = maximus;
        poolAPY.avax = avax;
    ...
    function setRewardsToken(IERC20 _rewardsToken) external onlyOwner {
        ...
        _rewardsToken.safeApprove(address(LYDIA_ROUTER), uint(- 1));
    }
    ...
    event Staked(address indexed user, uint256 amount, address from);
