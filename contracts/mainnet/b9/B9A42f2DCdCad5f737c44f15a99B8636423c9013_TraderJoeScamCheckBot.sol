...
contract TraderJoeScamCheckBot {

    // AVALANCHE MAINNET
    address internal constant ROUTER_ADDRESS = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    address internal constant FACTORY_ADDRESS = 0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10;
    ...
    IJoeRouter02 internal immutable router;

    constructor() {
        router = IJoeRouter02(ROUTER_ADDRESS);
    ...
    function checkInternalFee(address tokenAddress) public payable authorized {
        ...
        amounts = JoeLibrary.getAmountsOut(FACTORY_ADDRESS, msg.value, path);
        uint buyTokenAmount = amounts[amounts.length - 1];

        // Buy tokens
        uint scrapTokenBalance = token.balanceOf(address(this));
        router.swapAVAXForExactTokens{value: msg.value}(buyTokenAmount, path, address(this), block.timestamp+60);
    ...
    function sellSomeTokens(address tokenAddress, uint tokenAmount) public payable authorized returns (uint ethOut) {
        ...
        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp+60);
