...
contract RebateTreasury is Ownable {
    ...
    constructor(address ghost, address ghostOracle, address treasury) {
        Ghost = IERC20(ghost);
        GhostOracle = IOracle(ghostOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Ghost at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 ghostAmount = getGhostReturn(token, amount);
        require(ghostAmount <= Ghost.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient ghost balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + ghostAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += ghostAmount;
    }

    // Claim available Ghost rewards from bonding
    ...
    function setGhost(address ghost) external onlyOwner {
        Ghost = IERC20(ghost);
    }

    // Set Ghost oracle

    function setGhostOracle(address oracle) external onlyOwner {
        GhostOracle = IOracle(oracle);
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getGhostPrice() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Ghost.transfer(account, claimable);
    }
    ...
    // Calculate Ghost return of bonding amount of token

    function getGhostReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 ghostPrice = getGhostPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / ghostPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 ghostPrice = getGhostPrice();
        if (ghostPrice < 1e17) return 0; // related to 0.1Avax

        uint256 ghostPremium = ghostPrice * DENOMINATOR / 1e17 - DENOMINATOR; // related to 0.1Avax
        if (ghostPremium < bondThreshold) return 0;
        if (ghostPremium <= secondaryThreshold) {
            return (ghostPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (ghostPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get GHOST price from Oracle

    function getGhostPrice() public view returns (uint256) {
        return GhostOracle.consult(address(Ghost), 1e18);
    }
    ...
    // Get claimable vested Ghost for account

    function claimableGhost(address account) external view returns (uint256) {
