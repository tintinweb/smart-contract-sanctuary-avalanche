...
contract VaporController04 is Pausable, Authorizable, Taxable02 {
  ...
  constructor(address[] memory _addresses, address[] memory _pools) {
    require(_addresses.length == 6, "C_1");
    ...
    joeRouter = IJoeRouter02(_addresses[4]);
    joePair = IJoePair(_addresses[5]);
  ...
  function getClaimTaxQuote(address _account, uint256[] calldata _ids)
    external
    view
    returns (uint256)
  {
...
contract Taxable02 is Ownable {
  ...
  function getClaimQuote(uint256 amount) public view returns (uint256) {
    ...
    path[0] = joePair.token0();
    path[1] = joePair.token1();
    uint256[] memory amounts = joeRouter.getAmountsOut(amount, path);
