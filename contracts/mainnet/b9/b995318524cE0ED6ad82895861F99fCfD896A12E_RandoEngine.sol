...
interface RandoData {
    function requestDice( address _caller, uint8 _type, uint8 _numberofdie, uint8 _diesides ) external returns ( uint256);
    function getResult( uint256 _request ) external returns(uint256);
}
...
contract RandoEngine is Ownable {
    ...
    function requestDice(uint8 _type, uint8 _numberofdie, uint8 _diesides ) public onlyOptionalEngine returns ( uint256) {
        ...
        RandoData _rd = RandoData ( ecosystem.returnAddress("RandoData"));
        uint256 requests = _rd.requestDice ( msg.sender, _type, _numberofdie, _diesides );
        return requests;
    }

    function getResult( uint256 _request ) public returns(uint256){
        RandoData _rd = RandoData ( ecosystem.returnAddress("RandoData")); 
        return _rd.getResult(_request);
    }
