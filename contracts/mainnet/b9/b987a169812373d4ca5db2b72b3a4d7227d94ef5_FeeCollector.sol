...
contract FeeCollector is BaseComponent, IFeeCollector {
    ...
    /**
     * @notice Returns the protocol fees for a given pair.
     * @param lbPair The pair to get the protocol fees for.
     * @return feesX The protocol fees for token X.
     * @return feesY The protocol fees for token Y.
     */
    function getProtocolFees(ILBPair lbPair) external view override returns (uint128 feesX, uint128 feesY) {
        (feesX, feesY) = _getProtocolFees(lbPair);
    }

    /**
     * @notice Returns the protocol fees for a given list of pairs.
     * @param lbPairs The list of pairs to get the protocol fees for.
     * @return feesX The protocol fees for token X.
     * @return feesY The protocol fees for token Y.
     */
    function getBatchProtocolFees(ILBPair[] calldata lbPairs)
        external
        view
        override
        returns (uint128[] memory feesX, uint128[] memory feesY)
    {
        feesX = new uint128[](lbPairs.length);
        feesY = new uint128[](lbPairs.length);

        for (uint256 i; i < lbPairs.length;) {
            (feesX[i], feesY[i]) = _getProtocolFees(lbPairs[i]);
    ...
    /**
     * @dev Returns the protocol fees for a given pair.
     * @param lbPair The pair to get the protocol fees for.
     * @return feesX The protocol fees for token X.
     * @return feesY The protocol fees for token Y.
     */
    function _getProtocolFees(ILBPair lbPair) private view returns (uint128 feesX, uint128 feesY) {
        (,, feesX, feesY) = lbPair.getGlobalFees();
    }
    ...
    function _collectProtocolFees(ILBPair lbPair) private {
        (uint256 amountX, uint256 amountY) = lbPair.collectProtocolFees();

        emit ProtocolFeesCollected(address(lbPair), amountX, amountY);
    }
}
...
interface IFeeCollector is IBaseComponent {
    event ProtocolFeesCollected(address indexed lbPair, uint256 feesX, uint256 feesY);

    function getProtocolFees(ILBPair lbPair) external view returns (uint128 feesX, uint128 feesY);
