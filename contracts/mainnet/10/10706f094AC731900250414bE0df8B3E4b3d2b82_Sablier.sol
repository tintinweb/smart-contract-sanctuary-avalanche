...
contract Sablier is ISablier, ReentrancyGuard, CarefulMath {
    ...
    mapping(uint256 => streamItem) public idToStreamItem;

    event streamItemCreated (
        uint streamId,
        address sender,
        address recipient,
        uint256 deposit
    ...
    function fetchStreamItem() public view returns (streamItem[] memory) {
        uint itemCount = nextStreamId;
        
        uint currentIndex = 0;

        streamItem[] memory items = new streamItem[](nextStreamId -100000);
        for (uint i = 100000; i < itemCount; i++) {
        
            uint currentId = idToStreamItem[i].streamId;
            streamItem storage currentItem = idToStreamItem[currentId];
    ...
    function fetchMyStreams() 
        public view 
        returns (streamItem[] memory) {
        uint itemCount = nextStreamId;
        uint myStreamCount = 0;
        uint currentIndex = 0;
        for (uint i = 100000; i < itemCount; i++) {
      
            if (idToStreamItem[i].sender == msg.sender) {
                myStreamCount += 1;
            }
        }

        streamItem[] memory items = new streamItem[](myStreamCount);
        for (uint i = 100000; i < itemCount; i++) {
            if (idToStreamItem[i].sender == msg.sender) {
                uint currentId = idToStreamItem[i].streamId;
                streamItem storage currentItem = idToStreamItem[currentId];
        ...
        returns (uint256)
    {
        ...
        idToStreamItem[streamId] =  streamItem(
            streamId,
            msg.sender,
            recipient,
            deposit
