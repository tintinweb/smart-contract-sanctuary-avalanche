...
contract FxBridgeLogic is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function addBridgeToken(address _tokenAddr, bytes32 _channelIBC, bool _isOriginated) public onlyOwner returns (bool)  {
        ...
        emit AddBridgeTokenEvent(_tokenAddr, IERC20MetadataUpgradeable(_tokenAddr).name(), IERC20MetadataUpgradeable(_tokenAddr).symbol(), IERC20MetadataUpgradeable(_tokenAddr).decimals(), state_lastEventNonce, _channelIBC);
        return true;
    }
    ...
    ) public nonReentrant {
        {
...

            {
                ...
                for (uint256 i = 0; i < _amounts.length; i++) {
                    ...
                    if (_tokenStatus.isOriginated == true) {
                        IERC20MetadataUpgradeable(_tokenContract).mint(address(this), _amounts[i]);
                    }
                ...
                if (_tokenStatus.isOriginated == true) {
                    IERC20MetadataUpgradeable(_tokenContract).mint(address(this), totalFee);
                }
