...
contract PDRGTreasuryFund is Fund {
    uint256 public ALLOCATION = 3_000_000 ether; // 10%
    uint256 public constant VESTING_DURATION = 180 days;
    uint256 public constant VESTING_START = 1648155600;
    IERC20 PDR;
    /*===================== VIEWS =====================*/

    function allocation() public view override returns (uint256) {
        return ALLOCATION;
    }
    ...
    function addExcess() public onlyOwner {
        uint256 excess = PDR.balanceOf(address(this)) - claimable();
	if(excess > 0){
		ALLOCATION += excess;
	}
    }
    ...
    function withDrawTokens(address tokenAddress) public onlyOwner {
        require(address(PDR) != tokenAddress);
	IERC20 token = IERC20(tokenAddress);
        uint256 tokenAmt = token.balanceOf(address(this));
        require(tokenAmt > 0, 'balance is 0');
        token.transfer(msg.sender, tokenAmt);
    }
    function addpdrg(address _PDR) public onlyOwner {
        require(_PDR != address(0), "Fund::constructor: Invalid address");
        PDR = IERC20(_PDR);
    }
...
abstract contract Fund is Ownable, Initializable {
    ...
    function initialize(address _pdrg) external initializer {
        require(_pdrg != address(0), "Fund::constructor: Invalid address");
        pdrg = _pdrg;
    }
    ...
    function currentBalance() public view returns (uint256) {
        return IERC20(pdrg).balanceOf(address(this));
    }
    ...
    function transfer(address receiver, uint256 amount) external onlyOwner {
        ...
        IERC20(pdrg).safeTransfer(receiver, amount);
    }
