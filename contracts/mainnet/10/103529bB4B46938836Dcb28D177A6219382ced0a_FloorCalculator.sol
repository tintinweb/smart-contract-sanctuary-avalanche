...
contract FloorCalculator is TokensRecoverable
{
    ...
    // add addresses that you have just locked Pi permanently and wont ask for Wizard or pAVAX from pool
    ...
    // wrapped BNB = ? 
    // backing token is pAVAX 
   
    function calculateSubFloorPAVAX(IERC20 wrappedToken, IERC20 backingToken) public view returns (uint256)
    {
        ...
        for(uint i=0;i<uniswapV2Factories.length;i++){
            ...
            address pair = JoeLibrary.pairFor(address(uniswapV2Factories[i]), address(Pi), address(backingToken));
            
            uint256 freePi = Pi.totalSupply().sub(Pi.balanceOf(pair)).sub(ignoredAddressesTotalBalance());

            if (freePi > 0) {
                uint256[] memory amountsOut = JoeLibrary.getAmountsOut(address(uniswapV2Factories[i]), freePi, path);
...
{
    function calculateSubFloorPAVAX(IERC20 wrappedToken, IERC20 backingToken) external view returns (uint256);
