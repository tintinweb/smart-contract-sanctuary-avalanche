...
contract Handler_3_1 is Owners {
	event NewNode(
	...
	function createWithTokens(
		...
		onlyOwners
	{
		swapper.swapCreate(tokenIn, user, price, sponso);
	}
		...
		onlyOwners
	{
		...
		swapper.swapClaim(
			tokenOut, user, rewardsTotal - price, feesTotal);
	}
	...
	function burnBatchNft(address user, uint256[] memory _tokenIds) external {
		require(msg.sender == migration, "Handler: Not allowed to burn");
		IPolarNode(nft).burnBatch(user, _tokenIds);
	}

	function setMigration(address _migration) external onlyOwners {
