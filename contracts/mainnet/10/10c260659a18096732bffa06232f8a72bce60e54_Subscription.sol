...
contract Subscription is ISubscription{
    ...
    constructor(string memory offer , address currency, address thepayor, address thepayee, uint256 interval, uint256 amount){
        MuPrice = IMuPriceOracle(0x5b9438372a6641Efbd9f285ab6931E190Ed841eB);
    ...
    function makePayment() public virtual override{
        if(block.timestamp >= _next_payment_time){
            _MUG_owed = MuPrice.get_cost_In_MUG_USD(_subscriptionAmount);
            if(_payment.balanceOf(payor) < _MUG_owed || _payment.allowance(address(this), payor) < _MUG_owed){
                _payment.transferFrom(payor, payee, _MUG_owed);
                _last_payment_time = _next_payment_time;
