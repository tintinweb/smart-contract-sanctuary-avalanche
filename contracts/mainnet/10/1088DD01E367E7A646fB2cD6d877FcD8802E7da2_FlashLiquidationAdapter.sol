...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  /**
   * @dev Swaps an exact `amountToSwap` of an ETH to another
   * @param assetToSwapTo Destination asset
   * @param amountToSwap Exact amount of ETH to be swapped
   * @param minAmountOut the min amount of `assetToSwapTo` to be received from the swap
   * @return the amount received from the swap
   */
  function _swapExactETHForTokens(address assetToSwapTo, uint256 amountToSwap, uint256 minAmountOut) internal returns (uint256) {
    address[] memory path;
    path = new address[](2);
    path[0] = WETH_ADDRESS;
    path[1] = assetToSwapTo;
    
    uint256[] memory amounts = UNISWAP_ROUTER.swapExactETHForTokens{value: amountToSwap}(
      minAmountOut, 
      path, 
      address(this), 
      block.timestamp
    ...
    emit Swapped(WETH_ADDRESS, assetToSwapTo, amounts[0], amounts[amounts.length - 1]);
...
import {IERC20} from "../dependencies/openzeppelin/contracts/IERC20.sol";
import {IATriCryptoZap} from "./interfaces/IATriCryptoZap.sol";

abstract contract CurveATriCryptoAdapter is BaseUniswapAdapter {
    address internal constant ATRICRYPTO_ZAP =
        0x58e57cA18B7A47112b877E31929798Cd3D703b0f;
    uint internal constant N_UL_COINS = 5;

    constructor() public {}

    function convertCurveLPToAsset(uint amount, address asset) internal returns(uint totalConverted) {
        address[] memory ulCoins = new address[](N_UL_COINS);
        uint[] memory minAmounts = new uint[](N_UL_COINS);
        uint[] memory beforeULTokenBalance = new uint[](N_UL_COINS);
        for (uint i; i < N_UL_COINS; i++) {
            ulCoins[i] = IATriCryptoZap(ATRICRYPTO_ZAP)
                .underlying_coins(i);
            beforeULTokenBalance[i] = IERC20(ulCoins[i]).balanceOf(
                address(this)
            );
            minAmounts[i] = 0;
        }
        IATriCryptoZap(ATRICRYPTO_ZAP).remove_liquidity(
            amount,
            minAmounts,
            address(this)
        );
        totalConverted = 0;
        for (uint i; i < N_UL_COINS; i++) {
            uint diffULTokenBalance = IERC20(ulCoins[i]).balanceOf(
                address(this)
            ) - beforeULTokenBalance[i];

            if (ulCoins[i] == asset) {
                totalConverted += diffULTokenBalance;
            } else {
                totalConverted += _swapExactTokensForTokens(
                    ulCoins[i],
                    asset,
                    diffULTokenBalance,
                    0,
...
import {BaseUniswapAdapter} from "./BaseUniswapAdapter.sol";
import {CurveATriCryptoAdapter} from "./CurveATriCryptoAdapter.sol";
...
import {IATriCryptoZap} from "./interfaces/IATriCryptoZap.sol";
import {BeefyVaultLibrary} from "./libraries/BeefyVaultLibrary.sol";
 ...
 * @author Vinium
 **/
contract FlashLiquidationAdapter is BaseUniswapAdapter, CurveATriCryptoAdapter, GMXAdapter {
    using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
    uint256 internal constant LIQUIDATION_CLOSE_FACTOR_PERCENT = 5000;
    address internal constant CURVE_ATRICRYPTO_LP_TOKEN = 0x1daB6560494B04473A0BE3E7D83CF3Fdf3a51828;
    address internal constant GMX_LP_TOKEN = 0xaE64d55a6f09E4263421737397D1fdFA71896a69;
    address internal constant GMX_STAKED_GLP_TRACKER = 0x9e295B5B976a184B14aD8cd72413aD846C299660;
    ...
    function _liquidateAndSwap(
        ...
        if (collateralAsset != borrowedAsset) {
            ...
            if (BeefyVaultLibrary.supportInterface(collateralAsset)) {
                uint256 remainingTokens;
                uint256 convertedAmount;
                uint256 lpBalance;
                IERC20 lpToken = IBeefyVaultV6(collateralAsset).want();
                IBeefyVaultV6(collateralAsset).withdrawAll();
                if (address(lpToken) == CURVE_ATRICRYPTO_LP_TOKEN) {
                    // atricrypto
                    lpBalance = lpToken.balanceOf(address(this));
                    convertedAmount = convertCurveLPToAsset(lpBalance, borrowedAsset);
                } 
                if (address(lpToken) == GMX_LP_TOKEN) {
                    // glp
                    lpBalance = IERC20(GMX_STAKED_GLP_TRACKER).balanceOf(address(this));
                    convertedAmount = convertGLPToAsset(lpBalance, borrowedAsset);
                }

                remainingTokens = convertedAmount.sub(vars.flashLoanDebt.sub(vars.diffFlashBorrowedBalance));
                if (remainingTokens > 0) {
                  IERC20(borrowedAsset).transfer(initiator, remainingTokens);
                }
            } else {
...
import {IERC20} from "../dependencies/openzeppelin/contracts/IERC20.sol";
import {IGlpRewardRouter} from "./interfaces/IGlpRewardRouter.sol";

abstract contract GMXAdapter is BaseUniswapAdapter {
    address internal constant GLP_REWARD_ROUTER = 0xB70B91CE0771d3f4c81D87660f71Da31d48eB3B3;

    constructor() public {}

    function convertGLPToAsset(uint amount, address asset) internal returns(uint totalConverted) {
        uint256 ethAmountOut = IGlpRewardRouter(GLP_REWARD_ROUTER).unstakeAndRedeemGlpETH(amount, 0, payable(address(this)));
        totalConverted = _swapExactETHForTokens(asset, ethAmountOut, 0);
    }
}
...
interface IATriCryptoZap {
    function underlying_coins(uint256 _index) external view returns(address);
    function remove_liquidity(uint256 _amount, uint256[] memory _min_amounts, address _receiver) external;
}
...
library BeefyVaultLibrary {
    function supportInterface(
        address _addr
    ) external returns (bool _isSupported) {
        bool _success = false;
        bytes memory data = abi.encodeWithSelector(bytes4(keccak256("want()")));
        assembly {
            _success := call(
                5000,
                _addr,
            ...
            _isSupported := _success
        }

        data = abi.encodeWithSelector(bytes4(keccak256("withdrawAll()")));
        assembly {
            _success := call(
                200000,
                _addr,
            ...
            _isSupported := and(_isSupported, _success)
        }
    }
}
