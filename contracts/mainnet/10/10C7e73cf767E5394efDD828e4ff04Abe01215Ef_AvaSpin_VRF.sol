...
contract AvaSpin_VRF is Ownable, VRFConsumerBaseV2 {
    event GameResult(
        address player,
        uint playedAmount,
        uint wonAmount,
    ...
    uint256 public maxLimit = 2.3 ether;
    uint256 public minLimit = 0.1 ether;
    uint256 public publicHouseEdge = 30;
    ...
    mapping(address => uint) public selectedMultiplier;
    mapping(address => uint) public payedAmount;
    mapping(address => uint) public playedBlock;
    mapping(address => uint) public requestID;
    ...
    function fulfillRandomWords(
        ...
        s_randomWords[requestId] = (randomWords[0] % 40) + 1;
    }

    function playSpin(uint16 selection) external payable {
        require(msg.value >= minLimit, "Risk under limit");
        require(msg.value <= maxLimit, "Risk over limit");
        require(!isPaused, "Game is paused");
        require(!isPlaying[msg.sender], "Have already played");
        ...
        require(
            selection == 2 ||
                ...
                selection == 20 ||
                selection == 25,
            "You can't select this multiplier"
        );
        selectedMultiplier[msg.sender] = selection;
        requestID[msg.sender] = requestRandomWords();
    ...
    function getResult() external {
        require(
            (playedBlock[msg.sender] + 3) <= block.number,
            "Please wait for VRF confirmation!"
        );
        require(isPlaying[msg.sender], "You have not played yet");
        isPlaying[msg.sender] = false;
	    uint spin = s_randomWords[requestID[msg.sender]];
        uint houseEdge = houseEdgeCalculator(msg.sender);
        uint amount = payedAmount[msg.sender];
        uint realPlayed = ((amount * 1000) / (houseEdge + 1000));
        
        if (selectedMultiplier[msg.sender] == 2 && spin >= 1 && spin < 19) {
            uint pay = realPlayed * 2;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 3 && spin >= 19 && spin < 29) {
            uint pay = realPlayed * 3;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 5 && spin >= 29 && spin < 35) {
            uint pay = realPlayed * 5;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 10 && spin >= 35 && spin < 38) {
            uint pay = realPlayed * 10;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 20 && spin >= 38 && spin < 40) {
            uint pay = realPlayed * 20;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else if (selectedMultiplier[msg.sender] == 25 && spin == 40) {
            uint pay = realPlayed * 25;
            treasury.executePaymentAVAX(pay, msg.sender);
            emit GameResult(msg.sender, amount, pay, true, spin);
            database.updateStatistics(msg.sender, true, amount, pay);
        } else {
            emit GameResult(msg.sender, amount, 0, false, spin);
