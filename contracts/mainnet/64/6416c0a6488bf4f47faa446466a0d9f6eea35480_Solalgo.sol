...
contract Solalgo is ReentrancyGuard, Context, Ownable {
    ...
    function claim() external nonReentrant{
        ...
        uint256 amount = calculateVesting(msg.sender);
        ...
        returns (uint256)
    {
        ...
        if(block.timestamp > timeToWait){
            uint256 max_payout = this.maxPayoutOf(totalContributions[userAddress]);
            // 10% Monthly for 10 Month to release 100% of remaining token
            amount = (((totalContributions[msg.sender] * 10)/100) * (((block.timestamp + 30 days) - timeToWait) / 30 days)) - claimed[msg.sender];
    ...
    function checkVesting(address addr) external view returns(uint256){
        uint256 tokensBought = calculateVesting(addr);
        return (tokensBought);
    }

    function checkTokenClaimed(address addr) external view returns(uint256){
        uint256 tokensBought = claimed[addr];
        return (tokensBought);
    }
