...
contract HodlLottery is Ownable, ReentrancyGuard {
    ...
    function buyTicket(LOTTERY_TYPE _lotteryType, uint256 _lotteryID, uint16 _numberOfTickets) external payable {
        ...
        lottoInfo.ticketsOfMember[newID] = numTickets * 2;

        emit BuyTicket(msg.sender, _lotteryType, _lotteryID, block.timestamp, newID, numTickets);
    }

    function randomNumberGenerate(uint256 _count) private view returns (uint16) {
        ...
        uint16 randomNum = uint16(randomHash % _count);
        return randomNum;
    }
    ...
    function selectWinner(LOTTERY_TYPE _lotteryType, uint256 _lotteryID) external nonReentrantSelectWinner onlyOwner returns(uint16) {
        ...
        uint16 winnerIDKey = randomNumberGenerate(totalMembers);
        ...
        // select real lottery card number
        uint16 userTicketCount = lottoInfo.ticketsOfMember[lottoInfo.ids[mid]];
        uint16 selectedTicketNo = randomNumberGenerate (userTicketCount);
        winnerIDKey = lottoInfo.ids[mid] + selectedTicketNo;
        if (selectedTicketNo < uint16(userTicketCount / 2)) {        
            // send prize AVAX to winner
            winnerAddress = lottoInfo.members[lottoInfo.ids[mid]];
