...
contract XavaEscrow {
    ...
    /// @param withdrawAmount - total amount to be moved from the user blaance into withdraw escrow
    /// @param balanceDelta - the change between user balance in contract vs in server when calling 
    /// @param direction - the direction the change occurs for this withdrawal (true = user net winning, false = user net losing)
    function initiateWithdrawal(
        ...
        // this is necessary so treasury balance can change for safe accounting
        if(balanceDelta != 0) {
            if(direction) {
            
                balances[who] += balanceDelta;
                balances[owner] -= balanceDelta;
            } else {
                balances[who] -= balanceDelta;
                balances[owner] += balanceDelta;
            }
    ...
    function ownerWithdraw(uint256 amount, address to) external OnlyOwner {
        ...
        uint256 fiftyPercent = (1000 * balances[owner]) / 2000;
