...
contract GovernanceToken is ERC20,ERC1820Client, Ownable{
    string private constant ERC20_INTERFACE_VALIDATOR = "ERC20Token";

    using SafeMath for uint256;
    ITokenismWhitelist whitelist;
    IERC1400RawERC20 ERC1400RawERC20;
    address[] private tokenReceivers;
    

    mapping(address => address) private SecurityTokenOwner;

    constructor(string memory name ,string memory symbol, ITokenismWhitelist _whitelist, IERC1400RawERC20 _ERC1400RawERC20) ERC20(name, symbol) {
    require(Address.isContract(address(_ERC1400RawERC20)),"Not valid contract address");
    require(interfaceAddr(address(_ERC1400RawERC20),ERC20_INTERFACE_VALIDATOR) != address(0),"Not a valid Security Token address");
    whitelist = _whitelist;
    ERC1400RawERC20 = _ERC1400RawERC20;
    }

    event TokenMint(
        address to, 
        IERC1400RawERC20 SecurityToken,
        uint256 amount
    );
    ...
    //mapping(address=>address) private GovernanceTokenMinted;
    modifier onlyAdmin() {
        require(
    ...
    function mintGovernanceToken(address to) external onlyAdmin {
        //require(GovernanceTokenMinted[to] != propertyAddress,"Already assign token");
        require(to != address(0), "Invalid address: cannot mint");
        require(balanceOf(to) == 0, "Governance Token: Token already minted");
        SecurityTokenOwner[to] = address(ERC1400RawERC20); 
        _mint(to, 1);
        tokenReceivers.push(to);
        emit TokenMint(to,ERC1400RawERC20,1);
    }
    function bulkMintGovernanceToken(address[] calldata _to) external onlyAdmin{
        
        uint256 len = _to.length;
        for (uint i = 0; i < len; ){
            
            require(_to[i] != address(0), "Invalid address: cannot mint");
            require(balanceOf(_to[i]) == 0, "Governance Token: Token already minted");

            SecurityTokenOwner[_to[i]] = address(ERC1400RawERC20); 
            _mint(_to[i], 1);

            tokenReceivers.push(_to[i]);

            emit TokenMint(_to[i],ERC1400RawERC20,1);
            unchecked {
                i++;
            }
        }
    }
    function burnGovernanceToken(address account) external onlyAdmin{
       // delete GovernanceTokenMinted[accoun t];
         require(_balance(account) >0, "Not enough Tokens to burn");
         delete SecurityTokenOwner[account];
         _remove(account);
        _burn(account, 1);
        emit TokenBurn(account, 1);
    }
    function _remove(address _account) internal{
        for(uint i= 0; i< tokenReceivers.length; i++){
            if(tokenReceivers[i] == _account){
                 tokenReceivers[i] =tokenReceivers[tokenReceivers.length -1];
                 tokenReceivers.pop();
                 break;
            }
        }
    }
     function _balance(address _investor) internal view returns(uint256){
      return  balanceOf(_investor);
    }
    function ownerOfSecuityToken( address _owner) public view returns(address){

        return SecurityTokenOwner[_owner];
    }

    function getTokenReceivers() public view returns(address[] memory){
