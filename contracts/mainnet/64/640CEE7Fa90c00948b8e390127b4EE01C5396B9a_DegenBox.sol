...
interface IStrategy {
    ...
    function cauldronV2() external returns (address cauldron);
}
...
contract DegenBox is MasterContractManager, BoringBatchable {
    ...
    function setStrategyTargetPercentage(IERC20 token, uint64 targetPercentage_) public {
        if(address(strategy[token]) != address(0)){
            require(msg.sender == owner || msg.sender == IStrategy(strategy[token]).cauldronV2(), "setStrategyTargetPercentage: caller != owner");
        }else{
