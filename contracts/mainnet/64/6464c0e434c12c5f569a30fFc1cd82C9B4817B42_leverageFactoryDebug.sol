...
contract leveragePool is leverageData,timeLockSetting {
    ...
    function leverage(address account, uint256 amount,uint amountLending,uint256 slipRate,uint256 deadLine) ensure(deadLine) external payable {
        ...
        _leverage(getUserVaultID(account),amount,amountLending,slipRate);
    }
    function leverage2(address account, uint256 amount,uint amountLending,uint256 slipRate,uint256 deadLine) ensure(deadLine) external payable {
        amount = getPayableAmount(IERC20(underlying),amount);
        bytes32 vaultID = getUserVaultID(account);
        userVault[vaultID] = userVault[vaultID].add(amount);
        emit AddUnderlying(msg.sender,vaultID,amount);
        _leverage(vaultID,0,amountLending,slipRate);
    }
    ...
    **/
    ...
    function _leverage(bytes32 vaultID, uint256 amount,uint amountLending,uint256 slipRate) internal notHalted nonReentrant {
...
abstract contract leverageData is Halt,ReentrancyGuard{
    ...
    event AddUnderlying(address indexed sender, bytes32 indexed vaultID, uint256 amount);
