...
/// @notice The (older) MasterChef contract gives out a constant number of PANDA tokens per block.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///  @notice 30 pandas per block in MCV1
    uint256 public constant MASTERCHEF_PANDA_PER_BLOCK = 30 * 1e18;
    ...
    /// @param _MASTER_CHEF The PanpandaSwap MCV1 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of PANDA.
    ...
    function pandaPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_PANDA_PER_BLOCK.mul(pandaRateToRegularFarm).div(PANDA_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_PANDA_PER_BLOCK.mul(pandaRateToSpecialFarm).div(PANDA_RATE_TOTAL_PRECISION);
        }
    }
    ...
    function pandaPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_PANDA_PER_BLOCK.mul(pandaRateToBurn).div(PANDA_RATE_TOTAL_PRECISION);
    }
    ...
    /// @notice Harvests PANDA from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    function updatePandaRate(
        ...
        require(
            ...
            "MasterChefV2: Panda rate must be greater than 0"
        );
        require(
