...
contract StrategyAaveSupplyOnly is StratManager, FeeManager {
    ...
    event ChargedFees(uint256 callFees, uint256 dexanceFees, uint256 strategistFees);
    ...
    constructor(
        ...
        address _keeper,
        address _strategist,
        address _dxnFeeRecipient
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _dxnFeeRecipient) public {
        want = _want;
        native = _native;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 dexanceFeeAmount = nativeFeeBal.mul(dexanceFee).div(MAX_FEE);
        IERC20(native).safeTransfer(dxnFeeRecipient, dexanceFeeAmount);

        uint256 strategistFeeAmount = nativeFeeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(native).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, dexanceFeeAmount, strategistFeeAmount);
    }
...
abstract contract FeeManager is StratManager {
    ...
    ///@notice withdrawalFee = 10 / WITHDRAWAL_MAX
    uint public withdrawalFee = 0;

    uint public callFee = 111;
    uint public dexanceFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        dexanceFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
...
contract StratManager is Ownable, Pausable {
    ...
    address public dxnFeeRecipient;
     ...
     * @param _dxnFeeRecipient address where to send Dexance's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        ...
        vault = _vault;
        dxnFeeRecipient = _dxnFeeRecipient;
    }
    ...
    function setKeeper(address _keeper) external onlyManager {
        require(_keeper != address(0),"Bad address");
        keeper = _keeper;
    }
    ...
    function setStrategist(address _strategist) external {
        ...
        require(_strategist != address(0),"Bad address");
        strategist = _strategist;
    }
    ...
    function setUnirouter(address _unirouter) external onlyOwner {
        require(_unirouter != address(0),"Bad address");
        unirouter = _unirouter;
    }
    ...
    /**
     * @dev Updates dexance fee recipient.
     * @param _dxnFeeRecipient new dexance fee recipient address.
     */
    function setDxnFeeRecipient(address _dxnFeeRecipient) external onlyOwner {
        require(_dxnFeeRecipient != address(0),"Bad address");
        dxnFeeRecipient = _dxnFeeRecipient;
    }
