...
/// @title Chainlink V2 Randomness Provider.
/// @author 0xBoots <[email protected]>
/// @notice RandProvider wrapper around Chainlink VRF v2.
contract ChainlinkV2RandProvider is RandProvider, VRFConsumerBaseV2 {
    ...
    /// @param _mechAvax Address of the MechAvax contract.
    /// @param _vrfCoordinator Address of the VRerc298F coordinator.
    /// @param _linkToken Address of the LINK token contract.
    /// @param _chainlinkKeyHash Public key to generate randomness against.
    /// @param _subscriptionId to identify the correct subscription to charge.
    constructor(
        MechAvax _mechAvax,
    ...
    /// @notice Request random bytes from Chainlink VRF. Can only by called by the MechAvax contract.
    function requestRandomBytes() external returns (bytes32 requestId) {
        // The caller must be the MechAvax contract, revert otherwise.
        if (msg.sender != address(mechAvax)) revert NotMechs();

        // The requestRandomness call will revert if we don't have enough LINK to afford the request.
        emit RandomBytesRequested(requestId = bytes32(COORDINATOR.requestRandomWords(
            s_keyHash,
            s_subscriptionId,
    ...
    /// @dev Handles VRF response by calling back into the MechAvax contract.
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        emit RandomBytesReturned(bytes32(requestId), randomWords[0]);

        mechAvax.acceptRandomSeed(bytes32(requestId), randomWords[0]);
    }
}
