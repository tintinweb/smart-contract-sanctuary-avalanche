...
// File: contracts/withdrawer/MultichainWithdrawer.sol
...
contract MultichainWithdrawer is BoringOwnable {
    ...
    IBentoBoxV1 public immutable bentoBox = IBentoBoxV1(0xBBe7bF1c422eFBb5B2cB7a91A6f0AA7CdE86C1d3);
    
    IERC20 public constant POLE = IERC20(0x65069e550C5526c029DC9135eDD02F6683859Ac1);
    IERC20 public constant NORTH = IERC20(0xD0755413bfE2e08dB6bE72761cdD56d77d4B60f1);
    IERC20 public constant sNORTH = IERC20(0xe40EddA6d49d935D03eC8F7FAFd84DE14d99D2a1);

    IUniswapV2Pair constant USDT_NORTH = IUniswapV2Pair(0x332f909F6bBa8c1B124202F9FEC5d515c7635500);
    ...
    function withdraw() public {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (feesEarned <= (bentoBox.toAmount(POLE, bentoBox.balanceOf(POLE, address(bentoBoxCauldronsV2[i])), false))) {
                bentoBoxCauldronsV2[i].withdrawFees();
            }
  
        }

        uint256 mimFromBentoBoxAmt = address(bentoBox) != address(0) ? bentoBox.balanceOf(POLE, address(this)) : 0;

        require(mimFromBentoBoxAmt > 0 ,'The liquidation reward has been issued, and there are no more rewards.');

        bentoBox.withdraw(POLE, address(this), address(this), mimFromBentoBoxAmt, 0);

        uint256 amountFirst = POLE.balanceOf(address(this));

        //POLE >> USDT >> NORTH
        amountSecond = Curve3POOL.exchange_underlying(address(POLE3POOL), 0, 3, amountFirst, 0, address(USDT_NORTH));
        (address token0, ) = sortTokens(address(USDT), address(NORTH));
        (uint256 reserve0, uint256 reserve1, ) = USDT_NORTH.getReserves();
        ...
        (uint256 amount0Out, uint256 amount1Out) = address(USDT) == token0
                    ? (uint256(0), amountTo)
                    : (amountTo, uint256(0));
        USDT_NORTH.swap(amount0Out, amount1Out, address(this), new bytes(0));
        uint256 NORTHAmount = NORTH.balanceOf(address(this));

        IERC20(NORTH).transfer(address(sNORTH), NORTHAmount); //send NORTH to sNORTH
    }

    function withdrawFromBentoBoxes(uint256 amountBentoboxShare) public {
        if (amountBentoboxShare > 0) {
            bentoBox.withdraw(POLE, address(this), address(this), 0, amountBentoboxShare);
        }
    }
