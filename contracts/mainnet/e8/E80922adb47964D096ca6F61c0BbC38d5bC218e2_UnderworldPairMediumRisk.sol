...
// Underworld Lending Medium Risk
// Twitter: @0xBuns
...
// File contracts/UnderworldPair.sol
// License-Identifier: UNLICENSED
// Underworld Lending Medium Risk

/// @title UnderworldPair
/// @dev This contract allows contract calls to any contract (except CoffinBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract UnderworldPairMediumRisk is ERC20, BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    UnderworldPairMediumRisk public immutable masterContract;
    ...
    Rebase public totalAsset; // elastic = CoffinBox shares held by the UnderworldPair, base = Total fractions held by asset suppliers
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Underworld Medium Risk ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    // Settings for the Medium Risk UnderworldPair
    ...
    constructor(ICoffinBox coffinBox_) public {
    ...
    function init(bytes calldata data) public payable override {
        require(address(collateral) == address(0), "UnderworldPair: already initialized");
        (collateral, asset, oracle, oracleData) = abi.decode(data, (IERC20, IERC20, IOracle, bytes));
        require(address(collateral) != address(0), "UnderworldPair: bad pair");
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, false, exchangeRate), "UnderworldPair: user insolvent");
    }
    ...
    function _addTokens(
        ...
        if (skim) {
            require(share <= coffinBox.balanceOf(token, address(this)).sub(total), "UnderworldPair: Skim too much");
        } else {
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        require(_totalAsset.base >= 1000, "Underworld: below minimum");
    ...
    function _call(
        ...
        require(callee != address(coffinBox) && callee != address(this), "UnderworldPair: can't call");

        (bool success, bytes memory returnData) = callee.call{value: value}(callData);
        require(success, "UnderworldPair: call failed");
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                require((!must_update || updated) && rate > minRate && (maxRate == 0 || rate > maxRate), "UnderworldPair: rate not ok");
    ...
    function liquidate(
        ...
        require(allBorrowAmount != 0, "UnderworldPair: all are solvent");
        ...
        if (!open) {
            ...
            require(masterContract.swappers(swapper), "UnderworldPair: Invalid swapper");
