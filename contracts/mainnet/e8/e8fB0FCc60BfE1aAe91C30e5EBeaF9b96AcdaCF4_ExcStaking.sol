...
contract ExcStaking is ReentrancyGuard, Ownable {
    ...
    mapping (uint256 => NFTData) NFTLastClaimed;

    uint256 public totalSupply;
    uint256 public lockedSupply;
    uint256 public NFTrewardPerDay = 50 ether;
    uint256 public NFTrewardPerSecond =  NFTrewardPerDay / 86400;
    ...
    function enableStaking() external onlyOwner{
	    rewardData[address(stakingToken)].lastUpdateTime = block.timestamp;
        rewardData[address(stakingToken)].periodFinish = block.timestamp;
	    paused = false;
    }

    function toggleNFTStaking() external onlyOwner{
    	    nftStaking = !nftStaking;
    }
    
    // Add a new reward token to be distributed to stakers
    function addReward(address _rewardsToken, address _distributor) public onlyOwner {
        require(rewardData[_rewardsToken].lastUpdateTime == 0, "PendragonStaking::addReward: Invalid");
    ...
    function approveRewardDistributor(
        ...
        require(rewardData[_rewardsToken].lastUpdateTime > 0, "PendragonStaking::approveRewardDistributor: Invalid");
    ...
    function checkNFTRewards(uint256 _tokenId) internal view returns (uint256 amountGenerated, bool claimable) {
			uint256 mintTS = NFTLastClaimed[_tokenId].activateTimestamp;
			uint256 claimTS = NFTLastClaimed[_tokenId].lastclaimTS;
			uint256 claimed = NFTLastClaimed[_tokenId].timesClaimed;
			require(mintTS !=0);
			if(claimed == 0){ 
                require(block.timestamp > mintTS);
                amountGenerated = (block.timestamp -  mintTS) * NFTrewardPerSecond;
			    claimable = true;
        	} else {
                amountGenerated = (block.timestamp -  claimTS) * NFTrewardPerSecond;
			    claimable = (claimTS + rewardsDuration) <= block.timestamp;
	      }	    
	}
    ...
    function claimableNFTReward(uint16 _tokenId) public view returns (uint256 amountGenerated, bool claimable) {
			(amountGenerated,claimable) = checkNFTRewards(_tokenId);
	}
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        require(amount > 0, "PendragonStaking::stake: Cannot stake 0");
	    require(!paused);
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        require(minters[msg.sender], "PendragonStaking::mint: Only minters allowed");
	    require(!paused);
        totalSupply = totalSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
	    require(!paused);
        require(amount > 0, "PendragonStaking::withdraw: Cannot withdraw 0");
        Balances storage bal = balances[msg.sender];
        ...
        if (amount <= bal.unlocked) {
            ...
            require(bal.earned >= remaining, "PendragonStaking::withdraw: Insufficient unlocked balance");
            bal.unlocked = 0;
            ...
            for (uint256 i = 0; ; i++) {
                ...
                if (penaltyAmount == 0 && userEarnings[msg.sender][i].unlockTime > block.timestamp) {
                    ...
                    require(bal.earned >= remaining, "PendragonStaking::withdraw: Insufficient balance after penalty");
                    bal.earned = bal.earned.sub(remaining);
                    if (bal.earned == 0) {
    ...
    function claimNFTReward(uint16[] memory _tokenIds) public {
	    require(nftStaking);
        uint256 NFTrewards = 0;
        for (uint i = 0; i < _tokenIds.length; i++) {
            uint16 tokenId = _tokenIds[i];
            require(IPDRGenNFT(genesisNFT).ownerOf(tokenId) == msg.sender,"You are not the owner of this token");
			(uint256 amountGenerated,bool claimable) = checkNFTRewards(tokenId);			
			if(!claimable){
				continue;
				} else {
					NFTLastClaimed[tokenId].timesClaimed++;
					NFTLastClaimed[tokenId].lastclaimTS = block.timestamp;
					NFTLastClaimed[tokenId].totalClaimed += amountGenerated;
					NFTrewards += amountGenerated;
				}
			}
		stakingTokenReserve.transfer(msg.sender, NFTrewards);
    }
	
	function registerNFT(uint16[] memory _tokenIds) public {
		for (uint i = 0; i < _tokenIds.length; i++) {
            uint16 tokenId = _tokenIds[i];
            require(IPDRGenNFT(genesisNFT).ownerOf(tokenId) == msg.sender,"You are not the owner of this token");
			if(NFTLastClaimed[tokenId].activateTimestamp == 0){continue;}
			NFTLastClaimed[tokenId].activateTimestamp = block.timestamp;
			}
    }
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external updateReward(address(0)) {
        require(rewardDistributors[_rewardsToken][msg.sender], "PendragonStaking::notifyRewardAmount: Only reward distributors allowed");
        require(reward > 0, "PendragonStaking::notifyRewardAmount: No reward");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(tokenAddress != address(stakingToken), "PendragonStaking::recoverERC20: Cannot withdraw staking token");
        require(rewardData[tokenAddress].lastUpdateTime == 0, "PendragonStaking::recoverERC20: Cannot withdraw reward token");
    ...
    function addMinter(address _minter) external onlyOwner {
		require(_minter.code.length > 0);
        minters[_minter] = true;
        mintersArray.push(_minter);
    }

    function addGenesisNFT(address _genesis) external onlyOwner {
        require(_genesis.code.length > 0);
        genesisNFT = _genesis;
    }
	
	function setNFTReward(uint256 _reward) external onlyOwner {
		NFTrewardPerDay = _reward;
		NFTrewardPerSecond =  NFTrewardPerDay / 86400;
    }
...
interface IPDRGenNFT is IERC721{
    function mintTimestamp(uint256 _amount) external returns(uint256);
}
