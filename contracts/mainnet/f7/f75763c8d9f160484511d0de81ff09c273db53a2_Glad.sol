...
contract Glad is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 GLAD-WETH LP -> GLAD pool
    // uint256 public constant INITIAL_GLAD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the GLAD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Glad Token", "GLAD") {
        // Mints 1 GLAD to contract creator for initial pool setup
    ...
    function _getGladPrice() internal view returns (uint256 _gladPrice) {
        try IOracle(gladOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Glad: failed to fetch GLAD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _gladPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_gladPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGladOracle(address _gladOracle) public onlyOperatorOrTaxOffice {
        require(_gladOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GLAD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GLAD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGladPrice = _getGladPrice();
            currentTaxRate = _updateTaxRate(currentGladPrice);
            if (currentGladPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_gladPool != address(0), "!_gladPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_gladPool, INITIAL_GLAD_POOL_DISTRIBUTION);
