...
contract ChiknKeeperV2 is Ownable, IERC721Receiver {
    ...
    function checkSkipCoolingOffAmt(uint tid) external view returns (uint256) {
        return EGG_CONTRACT.checkSkipCoolingOffAmt(stakedChickenKG(tid));
    }
...
///HOW IS THIS DETEREMINED?///
    ...
    function chiknRemainingCD(uint256 tid) public view returns(uint) {
        return block.timestamp - stakedChickenCDT(tid);
    }
    ...
    function stakedChickenLastSkipT(uint tid) public view returns(uint) {
        IEggV2.StakedChiknObj memory _stakedChiknObj = IEggV2(0x7761E2338B35bCEB6BdA6ce477EF012bde7aE611).stakedChikn(tid);
        return _stakedChiknObj.lastSkippedTs;
    }
