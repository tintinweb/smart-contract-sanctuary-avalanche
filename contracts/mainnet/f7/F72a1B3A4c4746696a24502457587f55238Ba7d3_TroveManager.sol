...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuardUpgradeable {
    ...
    mapping (address => InterestSnapshot) public interestSnapshots;
        ...
        returns (newColls memory pendingCollRewards)
    {
        ...
        for (uint256 i; i < allCollsLen; ++i) {
            ...
            uint256 dec = _getIERC20Decimals(coll);
    ...
    function removeStakeAndCloseTrove(address _borrower, uint256 _status) external override {
        ...
        uint256 idxLast = TroveOwnersArrayLength.sub(1);
    ...
    function _updateStakeAndTotalStakes(address _borrower) internal {
        address[] memory allColls = Troves[_borrower].colls.tokens;
        uint256[] memory allAmounts = Troves[_borrower].colls.amounts;
        uint256 troveOwnerLen = allColls.length;

        uint256[] memory valuesVC = _getValuesVCIndividual(allColls, allAmounts);
        uint256 totalVC = 0;
        InterestSnapshot storage interestSnapshotUser = interestSnapshots[_borrower];

        for (uint256 i; i < troveOwnerLen; ++i) {
            address token = allColls[i];
            uint256 amount = allAmounts[i];
    ...
    function _computeNewStake(address token, uint256 _coll) internal view returns (uint256 stake) {
        if (totalCollateralSnapshot[token] == 0) {
            ...
            require(totalStakesSnapshot[token] != 0, "TM1");
    ...
    function redistributeDebtAndColl(
        ...
        //         require(absColls.length != 0 && unAllocatedAbsWeight != 1e18, "TM2");
    ...
    /** 
     * @notice Interest rate initialization for all variables and terms. Called once and then removed with a contract upgrade. 
     *  Initializes all L_YUSDInterest terms to 1e18, and initializes the last interest rate update time to be this block timestamp.
     */ 
    // todo 
    function interest_init() external {
        lastInterestRateUpdateTime = block.timestamp;
        address[] memory allColls = controller.getValidCollateral();
        for (uint i; i < allColls.length; ++i) {
            address token = allColls[i];
            L_YUSDInterest[token] = 1e18;
            emit L_YUSDInterestUpdated(token, 1e18);
        }
    }
    ...
    /**
     * @notice Special upgrade function to manually change all interest snapshot terms to 1e18
     * and to set the collateral denominator terms.
     */
    // todo 
    function interestInitTrove(address _borrower) external {
        require(msg.sender == 0xCa9ffF2f9838D326E4e8b4C68FEdc653f60fe1B5);
        address[] memory allColls = Troves[_borrower].colls.tokens;
        for (uint256 i; i < allColls.length; ++i) {
            interestSnapshots[_borrower].interestReward[allColls[i]] = 1e18;
        }
        ...
        returns (uint256)
    {
        ...
        require(redemptionFee < _YUSDRedeemed, "TM7");
    ...
    function decayBaseRateFromBorrowingAndCalculateFee(uint256 _YUSDDebt) external override returns (uint256){
        ...
        require(decayedBaseRate <= DECIMAL_PRECISION, "TM8"); // The baseRate can decay to 0
    ...
    function _requireTroveIsActive(address _borrower) internal view {
        require(Troves[_borrower].status == Status.active, "TM9");
    }

    function _requireMoreThanOneTroveInSystem(uint256 TroveOwnersArrayLength) internal pure {
        require(TroveOwnersArrayLength > 1, "TM10");
    }
