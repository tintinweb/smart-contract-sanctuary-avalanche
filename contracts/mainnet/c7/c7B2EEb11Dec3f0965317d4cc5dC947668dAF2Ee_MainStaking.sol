...
contract MainStaking is Initializable, OwnableUpgradeable {
    ...
    function stakePtpNft(uint256 _tokenId) external onlyOwner {
        IERC721(IPTPStaking(staking_ptp).nft()).approve(staking_ptp, _tokenId);
        IPTPStaking(staking_ptp).stakeNft(_tokenId);
    }

    function unstakePtpNft() external onlyOwner {
        IPTPStaking(staking_ptp).unstakeNft();
    }

    function retrievePtpNft(uint256 _tokenId) external onlyOwner {
        IERC721(IPTPStaking(staking_ptp).nft()).safeTransferFrom(
            address(this),
            msg.sender,
            _tokenId
        );
...
interface IPoolHelper {
    ...
    function earned() external view returns (uint256 vtxAmount, uint256 ptpAmount);
