...
contract Presale is Ownable {
    ...
    // Total USDC sent to vault
    uint256 public sentToVault;

    // Total USDC sent to vault
    uint256 public sentToTreasury;

    // Flag for users to withdraw usdc
    bool public usdcWithdrawable;
    ...
    function deposit(uint256 _amount) external {
        ...
        uint256 toVault = (depositUSDC * minToVault) / 100;

        usdc.transferFrom(msg.sender, vault, toVault);
        usdc.transferFrom(msg.sender, address(this), depositUSDC - toVault);
        sentToVault += toVault;
    ...
    function moveFunds() external onlyOwner {
        require(totalUSDC >= softCap, "Presale Not Succeed Yet");
        uint256 usdcBal = usdc.balanceOf(address(this));
        require(usdcBal > 0, "No USDC To Withdraw");

        uint256 toVault;
        uint256 toTreasury;

        // Success Presale
        toVault = (totalUSDC * vaultRatio) / 100 - sentToVault;
        toTreasury = (totalUSDC * (100 - vaultRatio)) / 100 - sentToTreasury;

        if (toVault > 0) {
            usdc.transfer(vault, toVault);
            sentToVault += toVault;
        }

        if (toTreasury > 0) {
            usdc.transfer(treasury, toTreasury);
            sentToTreasury += toTreasury;
        }

        emit MoveFunds(toVault, toTreasury);
    }

    function emergencyWithdraw() external onlyOwner {
        uint256 usdcBal = usdc.balanceOf(address(this));
        usdc.transfer(vault, usdcBal);
    }
    ...
    function setSoftCap(uint256 _softCap) external onlyOwner {
        require(softCap > 0, "Invalid HardCap");
        softCap = _softCap;
    }

    function setEndTime(uint256 _endTime) external onlyOwner {
        require(_endTime > 0, "Invalid HardCap");
        endTime = _endTime;
    }

    function setClaimTime(uint256 _claimTime) external onlyOwner {
        require(_claimTime > 0, "Invalid HardCap");
