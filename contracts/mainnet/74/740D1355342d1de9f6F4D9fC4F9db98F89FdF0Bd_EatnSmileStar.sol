...
library Bytes32Address {
    ...
    function fillLast12Bytes(address addressValue) internal pure returns (bytes32 value) {
        assembly {
            value := addressValue
        }
    }

    function fromFirst160Bits(uint256 uintValue) internal pure returns (address addr) {
        assembly {
            addr := uintValue
        }
    }
    ...
    function fromLast160Bits(uint256 uintValue) internal pure returns (address addr) {
        assembly {
            addr := shr(0x60, uintValue)
        }
    }

    function fillFirst96Bits(address addressValue) internal pure returns (uint256 value) {
        assembly {
    ...
    Lockable
{
    ...
    constructor() payable Pausable() ERC20("EatnSmile Star", "STAR") ERC20Permit("EatnSmileStar") {
        address operator = 0xe2d540570d2780D5f5F065D5a0D05807826246Ed;
        _grantRole(DEFAULT_ADMIN_ROLE, operator);
        _grantRole(MINTER_ROLE, operator);
        _grantRole(OPERATOR_ROLE, operator);
        _mint(0xad94f9Cb31aD5D8028f455bb3fcb578F0AcDC2Fd, 105_000_000 ether);
    }
