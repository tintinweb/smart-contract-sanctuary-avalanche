...
contract InfiniteNodes is ERC20, Ownable, PaymentSplitter {
    ...
    bool public CashOutFeeRemoval = false;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "InfiniteNodes::antiWhale: Transfer amount exceeds the maxTransferAmount");
                if (sender == joePair) {
                    require(balanceOf(recipient).add(amount) <= maxBalanceAmount(), "InfiniteNodes::antiWhale: Transfer would exceed the maxBalanceAmount of the recipient");
                }
            }
        }
        _;
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        emit Transfer(address(0), _msgSender(), 20000000 * 10**18);
    }
    ...
    function updatemaxBalanceAmountRate(uint16 _maxBalanceAmountRate) external onlyOwner {
        require(_maxBalanceAmountRate <= 10000, "InfiniteNodes::updatemaxBalanceAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxBalanceAmountRate >= 200, "InfiniteNodes::updatemaxBalanceAmountRate: Max transfer amount rate must  exceed the minimum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "InfiniteNodes::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 100, "InfiniteNodes::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate.");
    ...
    function _transfer(
        ...
        //sell
        if (isTradingEnabled && to == joePair && from != address(joeRouter) && ! _isExcludedFromFee[from]) {  
            //remove 5% sell fee
            uint256 new_amount = amount.mul(95).div(100);
            uint256 to_contract = amount.sub(new_amount);
            super._transfer(from, to, new_amount);
            super._transfer(from, address(this), to_contract);      
        }else{
    ...
    function cashoutAll() external {
        ...
        if (swapLiquifyEnabled) {
            ...
            if(NFTboosterActivated){
                //check last claim
                string memory lastuserclaim = nodeManager.getNodesLastClaimTime(sender);
                //convert string to uint
                uint converted_lastclaim = st2num(lastuserclaim);
                if((block.timestamp - converted_lastclaim) >= 24 hours){
                    ...
                    if(numberboosternft > 0){
                        ...
                        if(numberboosternft > usernodes){
                            add_reward = usernodes.mul(25).div(100);
                        }else{
                            add_reward = numberboosternft.mul(25).div(100);
                        }
                        //add decimals
                        add_reward = add_reward * 10**18;

                        //remove cashout fee
                        CashOutFeeRemoval = true;
                    }
                }  
            }
            
            if(!CashOutFeeRemoval){
                if (cashoutFee > 0) {
                    feeAmount = rewardAmount.mul(cashoutFee).div(100);
                    swapAndSendToFee(rewardsPool, feeAmount);
                }
