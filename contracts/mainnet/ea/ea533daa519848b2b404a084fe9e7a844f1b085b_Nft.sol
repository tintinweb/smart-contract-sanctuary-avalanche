...
contract Nft is Context, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    /// @dev The Team Payment Splitter contract address.
    address public paymentSplitterContractAddress;

    /// @dev The Burner contract address.
    address public burnerContractAddress;

    /// @dev Determines if minting is enabled.
    bool public mintingEnabled;

    /// @dev Determines if Whitelist minting is enabled.
    bool public whitelistMintingEnabled;

    /// @dev Map of addresses to the amount of free mints they have.
    mapping(address => uint256) public freeMints;

    /// @dev Map of addresses to if they are Whitelisted.
    mapping(address => bool) public whitelistAddresses;

    /// @dev The NFT constructor.
    /// @param _name the token name.
    /// @param _symbol the token symbol.
    /// @param _baseTokenUri the NFT metadata API endpoint.
    /// @param _mintPrice the mint price.
    /// @param _maxMintSupply the max supply.
    /// @param _paymentSplitterContractAddress the Team Payment Splitter contract address.
    /// @param _burnerContractAddress the Burner contract address.
    constructor(
        string memory _name,
    ...
    /// @dev Modifier that ensures a function is only callable when minting is enabled.
    ...
    function mint(uint256 amount) public payable onlyMintingEnabled {
        require(
            msg.value == mintPrice * amount || owner() == msg.sender,
    ...
    /// @dev Claim free mints.
    /// @param amount the amount of free mints to claim.
    ...
    /// @dev Manually mints an NFT for address. For honararies and 1/1s.
    /// @param recievingAddress the address of the recepient.
    /// @param tokenId the token ID of the new NFT to be minted.
    function mintExtra(address recievingAddress, uint256 tokenId)
        public
        onlyOwner
    {
        require(
            tokenId > mintSupply,
            "Manual mints needs to be outside of supply"
        );
        _mint(recievingAddress, tokenId);
    }

    /// @dev Modifies user's whitelist status.
    /// @param userAddress the address to modify.
    /// @param isWhitelisted if the user should be whitelisted.
    ...
    /// @dev Changes address' free mint count.
    /// @param userAddress the address to modify.
    /// @param amount the amount of free mints to give the address.
    ...
    /// @dev Sends token to the burner address.
    ...
    /// @dev required override for _baseURI.
