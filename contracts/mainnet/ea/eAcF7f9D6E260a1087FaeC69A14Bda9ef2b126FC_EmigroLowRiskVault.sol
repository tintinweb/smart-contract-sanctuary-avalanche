...
contract EmigroLowRiskVault is
    ERC4626,
    Ownable,
    Pausable
{
    ...
    /// @notice The strategy handles all underlying tokens on the vaults behalf
    ...
    /// @notice Get the total number of strategy tokens held by this vault
    /// @return The total amount of strategy tokens the Vault holds.
    function totalHoldings() public view override returns (uint256) {
        return strategy.balanceOf(address(this));
    }

    /// @notice Hook called when redeeming vault tokens
    /// @dev this function is called, then vault tokens are burned
    /// @dev Overrides 4626 equivalent
    function beforeWithdraw(uint256 underlyingAmount)
        internal
        override
        whenNotPaused
        returns(uint)
    ...
    /// @notice Hook called after depositing into the vault
    /// @dev Vault tokens are minted, then this is called
    /// @dev Overrides 4626 equivalent
    function afterDeposit(uint256 underlyingAmount)
        internal
        override
        whenNotPaused
    {
    ...
    function depositIntoStrategy(uint256 underlyingAmount) internal {
        emit StrategyDeposit(msg.sender, address(strategy), underlyingAmount);
    ...
    /// @notice Redeem vault tokens for share of strategy in its underlying tokens
    /// @param vaultShares  Qty of vault shares being redeemed
    /// @return uint256 number of underlying tokens redeemed
    function withdrawFromStrategy(uint256 vaultShares) internal returns(uint256){
        // Decrease totalStrategyHoldings to account for the withdrawal.
        emit StrategyWithdrawal(msg.sender, address(strategy), vaultShares);

        // Withdraw from the strategy and revert if it returns an error code.
        // vault shares / total vault shares x strat tokens held by vault
        uint256 strategySharesToRedeem = vaultShares.fmul(exchangeRate(), baseUnit);
        (uint256 errorCode, uint256 withdrawnUTokens) = LowRiskStrategy(address(strategy)).redeemUnderlying(strategySharesToRedeem);
        require(
            errorCode == 0,
            "REDEEM_FAILED"
        );
    ...
    /// @notice Get the supply rate of the vaults strategy
    /// @dev Needs to be compounded per second for a year to convert to APY
    /// @return uint256 supply rate per time stamp as mantissa
    function supplyRate() public view returns (uint256) {
        return LowRiskStrategy(address(strategy)).supplyRate();
    }

    /// @notice Returns the amount of strategy tokens a vault token can be redeemed for.
    /// @return uint256 amount of strategy tokens
    function exchangeRate() public view returns (uint256) {
        // Get the total supply of rvTokens.
        uint256 vaultTokenSupply = totalSupply;

        // If there are no vaultTokens in circulation, return an exchange rate of 1:1.
        if (vaultTokenSupply == 0) return baseUnit;

        // Calculate the exchange rate by dividing the total strategy tokens held by the vaultToken supply.
        return totalHoldings().fdiv(vaultTokenSupply, baseUnit);
    }

    /// @notice Calculate what each vault token is worth based on the value of its strategies tactics in underlying
    /// @return uint256 the exchange rate from vault token for underlying
    function exchangeRateUnderlying() public view returns (uint256) {
        uint256 vaultTokenSupply = totalSupply;
        if (vaultTokenSupply == 0) return baseUnit;

        //strategies balance of the underlying tokens / total vault tokens
        return strategy.balanceOfUnderlying(address(this)).fdiv(vaultTokenSupply, baseUnit);
    }

    /// @notice Calculates the number of vault shares/tokens to mint
    /// @param underlyingAmount underlying tokens deposited
    /// @return uint256 qty of shares/tokens to mint
    function calculateShares(uint256 underlyingAmount) override public view virtual returns (uint256) {
        uint256 vaultTokenSupply = totalSupply;

        //if this is the first mint we issue a set amount of vault tokens to initialise the vault
        if (vaultTokenSupply == 0) return baseUnit;

        //shares = amount of undelying tokens deposited * vault token supply / total value of the vault in underlying tokens as reported by the strategy
        uint256 shares = underlyingAmount.fmul(vaultTokenSupply, baseUnit).fdiv(strategy.balanceOfUnderlying(address(this)), baseUnit);

        return shares;

        /*
            TODO:
            Currently relies underlying tactics exchangeRateStored, which does not account for
            interest accrued since the last interaction with that tactic (mint, borrow, repay, liquidate).
            To ensure our valuation of the vault is up to date we should instead use exchangeRateCurrent.
            This function consumes gas so should only be used in state altering functions.
            We might also consider balancing the tactics on mint or redeem or both.
        */
    }
    ...
    /*TODO: Add vault admin functions
        setStrategy
        setFee
        collectFee
        sweepTokens
...
abstract contract ERC4626 is ERC20 {
    ...
    event Withdraw(address indexed from, uint256 underlyingAmount);
    ...
    function redeem(uint256 vaultShares) public virtual {
        require(balanceOf[msg.sender]>=vaultShares, "you don't own that many vault shares");
        uint withdrawnUTokens = beforeWithdraw(vaultShares);
        _burn(msg.sender, vaultShares);
        emit Withdraw(msg.sender, vaultShares);
    ...
    function withdrawFrom(
        ...
        redeem(shareAmount = calculateShares(underlyingAmount));
    }
