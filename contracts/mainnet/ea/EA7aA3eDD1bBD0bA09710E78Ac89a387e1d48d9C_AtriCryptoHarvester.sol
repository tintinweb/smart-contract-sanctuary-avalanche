...
contract AtriCryptoHarvester is ERC20 {
    ...
    function getDefaultDepositIndex() internal view returns (uint) {
        address lastTokenInRoute = nativeToDepositRoute[nativeToDepositRoute.length - 1];
    ...
    function harvest() public {
        ...
        if (crvBal > 0) {
            swapCrvToNative(crvBal);
            chargeFees(msg.sender);
            uint natBal = IERC20(native).balanceOf(address(this));
            swapNativeToDeposit(natBal);
    ...
    function swapCrvToNative(uint crvBal) internal {
        IJoeRouter01(joeRouter).swapExactTokensForTokens(crvBal, 0, crvToNativeRoute, address(this), block.timestamp);
    }

    function swapNativeToDeposit(uint natBal) internal {
        IJoeRouter01(joeRouter).swapExactTokensForTokens(natBal, 0, nativeToDepositRoute, address(this), block.timestamp);
    }

    function chargeFees(address sender) internal {
        uint nativeTokenBal = IERC20(native).balanceOf(address(this));

        uint senderFee = nativeTokenBal * 5 / 1000;
        IERC20(native).safeTransfer(sender, senderFee);
    ...
    function _giveAllowances() internal {
        ...
        for (uint i = 0; i < crvToNativeRoute.length; i++) {
            IERC20(crvToNativeRoute[i]).safeApprove(joeRouter, type(uint).max);
        }
    }
}