...
contract Lotto is ILotto, VRFConsumerBaseV2 {
    ...
    address public immutable WINE;

    // Burn GameFi Amounts
    uint256 public GRAPES_BURNED_PER_TEN_TICKETS = 50 * 10**18;
    ...
    uint256 public LP_Per_Ticket = 333 * 10**15;
    ...
    /**
        Sets The Number Of WINE To Burn To Create Ten Tickets For The User
        @param burnedPerTen number of WINE, cannot be zero
     */
    ...
    /**
        Burns WINE For Tickets
        The amount transferred in depends on GRAPES_BURNED_PER_TEN_TICKETS
        NOTE: Must Have Prior Approval of WINE for address(this) before calling
        @param batches - number of batches of ten tickets to mint
     */
    function burnWINE(uint batches) external {
        // transfer in WINE
        uint received = _transferIn(IERC20(WINE), GRAPES_BURNED_PER_TEN_TICKETS * batches);
        require(
            received == GRAPES_BURNED_PER_TEN_TICKETS * batches,
            'Invalid Tokens Received'
        );
        // burn WINE
        IERC20(WINE).transfer(treasury, received);
