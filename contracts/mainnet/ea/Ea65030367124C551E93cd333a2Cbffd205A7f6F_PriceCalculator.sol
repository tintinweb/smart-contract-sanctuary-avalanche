...
import {INeonMonsters, IERC721} from "./INeonMonsters.sol";
import {IPriceCalculator} from "./IPriceCalculator.sol";

contract PriceCalculator is Ownable, IPriceCalculator {
    enum Round {
        NFT,
        DEFI
    }
    ...
    INeonMonsters private _neonMonsters;
    ...
    function init(INeonMonsters neonMonsters, uint256 min) external onlyOwner {
        if (initialized > 0) revert InitializedError();
        initialized++;

        _neonMonsters = INeonMonsters(neonMonsters);

        _erc721Tokens.push(neonMonsters);
        _minHoldingAmounts[address(neonMonsters)] = min;
    }

    function setWhitelist(address[] memory accounts, uint256 discount)
        external
        onlyOwner
    {
        for (uint256 i = 0; accounts.length > i; i++) {
            address account = accounts[i];
            _whitelist[account] = discount;
        }
    }
    ...
    /// @dev Discounts;
    /// Whitelist:
    ///     n% discount for tokens in every segment
    /// Segment 0:
    ///     90% discount for every account minted a monster
    /// Segment 1, 2, 3, 4:
    ///     10% discount for every token
    /// Segment 5, 6, 7, 8:
    ///     10% discount for every token
    function getPrice(
        uint256 segmentNo,
        ...
        if (segmentNo == 0) {
            if (!isMinter && INeonMonsters(_neonMonsters).isMinter(account))
                return price / 10;
        }

        if (_whitelist[account] > 0 && !isMinter) {
            return price - ((price * _whitelist[account]) / 100);
        }

        // NFT Round
        if (segmentNo > 0 && segmentNo < 5)
            return _roundPrice(Round.NFT, price, account);

        // DeFi Round
        if (segmentNo > 4 && segmentNo < 9)
            return _roundPrice(Round.DEFI, price, account);
    ...
    function _roundPrice(
        ...
        if (isEligible) return price - ((price * 10) / 100);
