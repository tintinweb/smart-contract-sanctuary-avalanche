...
import "../../Pool/IOddzStrategyManager.sol";
import "../../IOddzSDK.sol";
import "./IOddzDex.sol";
import "@openzeppelin/contracts/utils/Address.sol";

contract OddzDex is IOddzDex {
    using Address for address;
    IOddzSDK public oddzSDK;

    IOddzStrategyManager public oddzStrategyManager;

    constructor(IOddzSDK _oddzSDK, IOddzStrategyManager _oddzStrategyManager) {
        oddzSDK = _oddzSDK;
        oddzStrategyManager = _oddzStrategyManager;
    }

    function buyOption(
        OptionData[] memory _optionData,
        address _provider,
        bool _isGasless
    ) public override returns (uint256[] memory optionId) {
        optionId = new uint256[](_optionData.length);

        for (uint256 i = 0; i < _optionData.length; i++) {
            if (_isGasless) {
                optionId[i] = oddzSDK.buyWithGasless(
                    _optionData[i]._pair,
                    _optionData[i]._optionModel,
                    _optionData[i]._premiumWithSlippage,
                    _optionData[i]._expiration,
                    _optionData[i]._amount,
                    _optionData[i]._strike,
                    _optionData[i]._optionType,
                    _provider
                );
            } else {
                optionId[i] = oddzSDK.buy(
                    _optionData[i]._pair,
                    _optionData[i]._optionModel,
                    _optionData[i]._premiumWithSlippage,
                    _optionData[i]._expiration,
    ...
    function getPremium(OptionData memory _optionData)
        external
        view
        override
        returns (
        ...
        )
    {
        return
            oddzSDK.getPremium(
                _optionData._pair,
                _optionData._optionModel,
                _optionData._expiration,
                _optionData._amount,
                _optionData._strike,
                _optionData._optionType
            );
    }

    function _addLiquidity(LiquidityData memory _liquidityData) private {
        oddzStrategyManager.addLiquidity(_liquidityData._strategy, _liquidityData._amount, msg.sender);
    }

    function provideLiquidity(LiquidityData[] memory _liquidityData) public override {
        require(_liquidityData.length > 0, "OD: Liquidity data not provided");
        for (uint256 i = 0; i < _liquidityData.length; i++) {
            require(address(_liquidityData[i]._strategy).isContract(), "OD: Strategy address cannot be null or user");

            _addLiquidity(_liquidityData[i]);
        }
    }

    function buyOptionAndProvideLiquidity(
        OptionData[] memory _optionData,
        address _provider,
        LiquidityData[] memory _liquidityData,
        bool _isGasless
    ) external override returns (uint256[] memory optionId) {
        provideLiquidity(_liquidityData);
        optionId = buyOption(_optionData, _provider, _isGasless);
...
interface IOddzStrategyManager {
    ...
    event ChangedStrategy(address indexed _old, address indexed _new, address indexed _user, uint256 _liquidityMoved);
...
import "../../Option/IOddzOption.sol";
import "../../Pool/IOddzWriteStrategy.sol";
...
interface IOddzDex {
    ...
    function buyOption(OptionData[] memory _optionData, address _provider, bool _isGasless) external returns (uint256[] memory optionId);
    ...
    function provideLiquidity(LiquidityData[] memory _liquidityData) external;
    ...
    function buyOptionAndProvideLiquidity(
        ...
        LiquidityData[] memory _liquidityData,
        bool _isGasless
    ) external returns (uint256[] memory optionId);
}
...
interface IOddzWriteStrategy {
    ...
    /**
     * @notice Gets Premium from Strategy
     * @param _provider Address of provider
     */
    function getPremium(address _provider) external view returns (uint256 premium, bool isNegative);
}
