...
contract NeFiProtoStarDrops is Ownable{
	...
	function makegetclaimed(address[] memory addresses) external onlyOwner(){
		for(uint j = 0;j<addresses.length;j++){
			getclaims.push(addresses[j]);
		}
	}
	function getDropped(uint _x, uint _y) external onlyOwner()  {
		if(_y> getclaims.length){
			_y = getclaims.length;
		}
		for(uint j = _x;j<_y;j++){
			address _account = getclaims[j];
			airdrop[_account].dropped = dropMGR.EXTgetdropped(_account);
			airdrop[_account].claimed = protoMGR.getProtoStarsLength(_account);
			getRemaining(_account);
			getFeesPaid(_account);
		}
	}
