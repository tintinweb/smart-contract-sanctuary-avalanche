...
contract ThirdOne is Ownable, ReentrancyGuard {
    ...
    mapping (uint=> uint) public typeFLinkToID;//typeF shuffled ipfs link metadata file number to supers ID
    ...
    //
    constructor () {
        baseURISupers3250N = "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/N/";//bayc stand in uri // set baseURI, doesn't exist yet but will be standard like Chad Doge uri
        baseURISupers3250F = "https://boredapeyachtclub.com/api/mutants/F/";//mayc stand in uri // set baseURI, doesn't exist yet but will be standard like Chad Doge uri
        baseURISupers1750 =  "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/P/";//bayc stand in uri // set baseURI, doesn't exist yet but will be standard like Chad Doge uri
        ...
        setTypeFVials(1001,1002,1003,1004,1005,1006);//test: some dummy test data, this should only be set after shuffle reveal
    }
    ...
    function upgradeChad(uint _vialID, uint _chadID) public nonReentrant{
        ...
        require (IflatLaunchPeg(superContract).isApprovedForAll(superSafe, address(this)), "Dev must approve supers held in gnosis wallet");//user must have approved for transfer
        _burnVial(_vialID, _chadID);//burn vial
    ...
    function _burnVial(uint _vialID, uint _chadID) internal {//approve transfer of vial first in dapp
        ...
        if (vialToF[_vialID] == true) {//if vial was a type F
            fVialsBurned++;
            idToTypeFLink[_chadID] = randomizeNewF();//map supers ID to type F ipfs file number 1-6 randomized
            typeFLinkToBool[idToTypeFLink[_chadID]] = true;//mark F link ID as being used
            typeFLinkToID[idToTypeFLink[_chadID]] = _chadID;//map type F vials 1-6 to the chad ID used on it (beware 0 id overlap)
    ...
    function _chadMint(uint _chadID) internal { //when user burns a vial with a doge then we send them a supers nft from our preapproved multisig wallet
        require (superToMinted[_chadID] == false, "Error, chad has already minted a super with a vial");
        superToMinted[_chadID] = true;
        IflatLaunchPeg(superContract).safeTransferFrom(superSafe, msg.sender, _chadID);
...
//this function is cheaper but you have to know the shuffled and unshuffled ID pairing already
    function superTokenURIWrite (uint _superID, uint _superIdShuffled) public view returns(string memory) {//set supers contract to our url and have API read this after supers have been shuffle revealed. Then need to unchuffle subdomain name, have included function for dapps to use to unshuffle
        require (_superID<5000, "Highest super token allowed is ID# 4999");//doublecheck the 4999 vs 5000 alignment
        require (_superID>=0, "Lowest super token allowed is ID# 0");//doublecheck the 0 vs 1 alignment
        if (_superID<3250){
            return lowerSuperURI(_superID);
    ...
    function lowerSuperURI(uint _superID) private view returns (string memory){
        if (blockUnmintedURI == true){
            require (superToMinted[_superID] == true, "Super ID has not been created with a vial yet");//this also prevents multiple 0 mapping with chadToVIal check below
        }
