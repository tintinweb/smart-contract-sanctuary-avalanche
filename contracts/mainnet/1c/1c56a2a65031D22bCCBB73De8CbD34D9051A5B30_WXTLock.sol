...
import {IWXTLock} from '../interfaces/IWXTLock.sol';
import {INXUSDStaking} from '../interfaces/INXUSDStaking.sol';
import {IncentivizedERC20} from '../protocol/tokenization/IncentivizedERC20.sol';
import {IAaveIncentivesController} from '../interfaces/IAaveIncentivesController.sol';
import "../protocol/libraries/security/ReentrancyGuard.sol";
...
ReentrancyGuard
{
    // Duration that locks are streamed over
    uint256 public constant compressDuration = 7 days;
    using SafeERC20 for IERC20;
    uint256 public immutable lockDuration;
    address internal immutable _underlyingAsset;
    IAaveIncentivesController internal _incentivesController;
    INXUSDStaking internal _NXUSDStaking;
   ...
   */
    constructor(
        ...
        INXUSDStaking NXUSDStaking,
        IAaveIncentivesController incentivesController,
        uint256 _lockDuration,
        uint8 aTokenDecimals,
        string memory aTokenName,
        string memory aTokenSymbol
    ) IncentivizedERC20(aTokenName, aTokenSymbol, aTokenDecimals) public {
        _underlyingAsset = underlyingAsset;
        _incentivesController = incentivesController;
        _NXUSDStaking = NXUSDStaking;
    ...
    function lock(uint256 amount) external override {
        ...
        uint256 unlockTime = block.timestamp.div(compressDuration).mul(compressDuration).add(lockDuration);
        uint256 lockLength = userLocks[msg.sender].length;
        if (lockLength == 0 || userLocks[msg.sender][lockLength-1].unlockTime < unlockTime) {
            userLocks[msg.sender].push(LockedBalance({amount: amount, unlockTime: unlockTime}));
        } else {
            userLocks[msg.sender][lockLength-1].amount = userLocks[msg.sender][lockLength-1].amount.add(amount);
        }
        _mint(msg.sender, amount);
        updateUserNXUSDTiers(msg.sender);
    ...
    function unLock(uint256 amount) external override {
        ...
        uint256 unlockedAmount = withdrawableBalance(msg.sender);
        require(unlockedAmount >= amount, "Don't have enough unlocked tokens");
        _burn(msg.sender, amount);
        LockedBalance[] storage locks = userLocks[msg.sender];
        uint256 remaining = amount;
        for (uint i = 0; i < locks.length; i++) {
            if(remaining >= locks[i].amount){
                remaining = remaining.sub(locks[i].amount);
                delete locks[i];
                continue;
            }
            locks[i].amount = locks[i].amount.sub(remaining);
            break;
        }
        IERC20(_underlyingAsset).safeTransfer(msg.sender, amount);
        updateUserNXUSDTiers(msg.sender);
    ...
    function lockedBalance(address user) view external override returns (uint256) {
        return balanceOf(user);
    }

    function withdrawableBalance(address user) view public override returns (uint256) {
        uint256 userTotalBalance = balanceOf(user);
        if(userTotalBalance == 0) return 0;
        uint256 withdrawableAmount;
        LockedBalance[] storage locks = userLocks[user];
        for (uint i = 0; i < locks.length; i++) {
            if (locks[i].unlockTime > block.timestamp) break;
            withdrawableAmount = withdrawableAmount.add(locks[i].amount);
        }
        return withdrawableAmount;
    }

    function updateUserNXUSDTiers(address user) internal {
        uint256 storedReward = (_NXUSDStaking.userData(user)).storedReward;
        if(storedReward > 0) _NXUSDStaking.updateUserAPYData(user);
    }

    function getUserLocks(address user) external view returns (LockedBalance[] memory) {
...
interface INXUSDStaking {
    ...
    function updateUserAPYData(address user) external;
}
