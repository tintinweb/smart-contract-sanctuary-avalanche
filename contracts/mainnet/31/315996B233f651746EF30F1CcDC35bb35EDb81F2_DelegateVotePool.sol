...
contract DelegateVotePool is Initializable, OwnableUpgradeable {
    ...
    function __DelegateVotePool_init_(address _LVTX, address _bribeManager) external initializer {
        __Ownable_init();
        LVTX = _LVTX;
    ...
    function stakingDecimals() public view returns (uint256) {
        return IERC20Metadata(LVTX).decimals();
    }

    /// @return Returns lenghts of rewards
    function lengthOfRewards() public view returns (uint256) {
    ...
    modifier claim() {
        ...
        _manageRewards(rewardTokensList, earnedRewards, true);
        // handle locker rewards
        (address[] memory rewardLockerTokensList, uint256[] memory earnedLockerRewards) = ILockerV2(
            LVTX
        ).claim();
        _manageRewards(rewardLockerTokensList, earnedLockerRewards, false);
        _;
    }
    ...
    function _manageRewards(
        ...
        for (uint256 index = 0; index < length; ++index) {
            ...
            if (applyFees && fees > 0 && feeCollector != address(0)) {
    ...
    function castVotes() public {
        ...
        (address[] memory feesTokens, uint256[] memory earnedFees) = IBribeManager(bribeManager)
            .castVotes(false);
        _manageRewards(feesTokens, earnedFees, true);
        // handle bribes reward
        (address[] memory rewardTokensList, uint256[] memory earnedRewards) = IBribeManager(
            bribeManager
        ).harvestAllBribes(address(this));
    ...
    /// @param _amount Amount of newly staked tokens by the user on masterchief
    function _depositFor(address _for, uint256 _amount) internal claim updateReward(_for) {
        IERC20Metadata(LVTX).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw(uint256 _amount, bool _claim) public claim updateReward(msg.sender) {
        require(unlockTime < block.timestamp, "Votes are currently locked");
        require(_amount <= _balances[msg.sender], "Not enough to withdraw");
        totalSupply = totalSupply - _amount;
        updateVote();
        _balances[msg.sender] = _balances[msg.sender] - _amount;
        IERC20Metadata(LVTX).safeTransfer(msg.sender, _amount);
    ...
    function getRewardUser() public updateReward(msg.sender) {
        _getReward(msg.sender);
    }

    /// @notice Calculates and sends reward to user
    function harvestAndGetRewards() public {
        withdraw(0, true);
    }
