...
contract SnowyCentralFund is Initializable {
    ...
    modifier onlyFunds() {
        require(funds[msg.sender], "SnowyReserve::onlyFunds: Only allowed funds can withdraw");
        _;
    }

    /* ============ CONSTRUCTORS ========== */

    function initialize(IERC20 _snowy) external initializer {
        snowy = _snowy;
    ...
    function setDaoFund(address _daoFund) external returns (bool) {
        require(daoFund == address(0), "SnowyReserve::setDaoFund: NOT_ALLOWED");
        daoFund = _daoFund;
        funds[_daoFund] = true;
        return true;
    }
    ...
    function setTreasuryFund(address _treasuryFund) external {
        require(treasuryFund == address(0), "SnowyReserve::setTreasuryFund: NOT_ALLOWED");
...
import "./SnowyCentralFund.sol";

contract SnowyDaoFund is Ownable {
    uint256 public constant ALLOCATION = 6_000_000 ether; // 20%
    uint256 public constant VESTING_DURATION = 3 * 365 * 24 * 3600; // 3 years
    uint256 public constant VESTING_START = 1646791200; // 1647007200; // 11th Mar 2022, 2PM UTC

    uint256 public claimed_amount;
    SnowyCentralFund public centralFund;

    // ======== CONSTRUCTOR ==================

    constructor(SnowyCentralFund _centralFund) {
        require(address(_centralFund) != address(0), "SnowyDaoFund::constructor: Invalid address");
        centralFund = _centralFund;
        centralFund.setDaoFund(address(this));
    }
    ...
    function transfer(address _receiver, uint256 _amount) external onlyOwner {
        require(_receiver != address(0), "SnowyDaoFund::transfer: Invalid address");
        require(_amount > 0, "SnowyDaoFund::transfer: invalid amount");
        require(_amount <= claimable(), "SnowyDaoFund::transfer: > vestedAmount");

        claimed_amount = claimed_amount + _amount;
        centralFund.transfer(_receiver, _amount);
    }
}