...
import "../../common/ADivestVault.sol";
 ...
 */
contract RaiderVectorDivestVault is ADivestVault {
...
interface IStrategy {
  ...
  function deposit(address _sender) external;
...
interface IDivestmentStrategy is IStrategy {
 ...
 * The yield optimizing strategy itself is implemented in a separate 'Strategy.sol' contract.
 */
abstract contract ADivestVault is ERC20, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;
  ...
  function balance() public view returns (uint256) {
    return
      token.balanceOf(address(this)).add(IStrategy(strategy).totalLPBalance());
  }
  ...
  function earn() public {
    ...
    IStrategy(strategy).deposit(msg.sender);
  }
  ...
  function withdraw(uint256 _shares) public nonReentrant {
    ...
    // Before withdrawal functionality, take care of divestments
    _withdrawDivestedTokens();
  ...
  /**
   * @dev Function to withdraw all divestedTokens from the strategy.
   * Called by end user directly OR when a user removes all funds.
   */
  function _withdrawDivestedTokens() private {
  ...
  function withdrawDivestedTokens() public nonReentrant {
    _withdrawDivestedTokens();
  }
