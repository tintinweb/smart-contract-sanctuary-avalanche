...
contract VIP is ERC721Upgradeable, AuthorizableNoOperatorUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function canRedeem(uint256 gen, uint256[] memory ids) view external returns (bool) {
        uint len = ids.length;
        for(uint i; i < len; ++i)
        {
            if(!_exists(ids[i].add(gen.mul(100)))) return true;
        }
        return false;
    }
    ...
    function redeem(uint256 gen, uint256[] memory ids, bytes memory signature) external nonReentrant {
        ...
        for(uint i; i < len; ++i)
        {
            if(!_exists(ids[i].add(gen.mul(100)))) __mint(msg.sender, gen, ids[i]);
        }
    }
