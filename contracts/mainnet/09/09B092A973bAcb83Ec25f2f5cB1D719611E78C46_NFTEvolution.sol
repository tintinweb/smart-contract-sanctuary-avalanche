...
contract NFTEvolution is MyPausable {
    ...
    constructor(address _preyPredator, address _glad, address _entropy) {
    ...
    function evolve(uint256 _tokenId, bool _isPrey, uint8 _alphaIndex) external whenNotPaused {
        require(msg.sender == tx.origin, "Only EOA");
        PreyPredator.PreyPredator memory traits =  preyPredator.getTokenTraits(_tokenId);
        require(allowedGenerations[traits.generation], "NFTEvolution: this generation is not allowed to train");
        require(preyPredator.ownerOf(_tokenId) == msg.sender, "NFTEvolution: not your NFT");

        uint256 cost = costToEvolve(traits.generation, traits.isPrey, traits.alphaIndex, _isPrey, _alphaIndex);
        bool resetLevels = false;
    
        if (traits.generation != 0) {
            _checkAllLevelsAtLeast(traits, 75);
            resetLevels = true;
        } else if (traits.isPrey) {
            _checkAllLevelsAtLeast(traits, 200);
            resetLevels = true;
        } else {
            _checkAllLevelsAtLeast(traits, 100 + 50 * traits.alphaIndex);
        }

        if (dao == address(0)) {
            glad.burnFrom(msg.sender, cost);
        } else {
            glad.transferFrom(msg.sender, dao, cost);
        }
        ...
        traits.generation = 0;
        traits.alphaIndex = _alphaIndex;

        if (_isPrey != traits.isPrey) {
            while (true) {
                selectTraits(entropy.random(++cnt), _isPrey, traits);
                try PreyPredator(preyPredator).updateTokenTraits(_tokenId, traits) {
                    break;
                } catch {
                }
            }
        } else {
            PreyPredator(preyPredator).updateTokenTraits(_tokenId, traits);
        }
    }

    function costToEvolve(uint256 generation, bool isPrey, uint256 alphaIndex, bool newIsPrey, uint256 newAlphaIndex) public view returns (uint256 cost) {
        if (generation == 0 && isPrey && !newIsPrey && newAlphaIndex == 0) return 1e18;
        if (generation == 0 && !isPrey && !newIsPrey) {
            if (alphaIndex == 0 && newAlphaIndex == 1) return 1e18;
            if (alphaIndex == 1 && newAlphaIndex == 2) return 1e18;
            if (alphaIndex == 2 && newAlphaIndex == 3) return 1e18;
            if (alphaIndex == 3 && newAlphaIndex == 4) return 1e18;
        }
        if (generation != 0 && isPrey && newIsPrey) return 1e18;
        if (generation != 0 && !isPrey && newIsPrey) return 1e18;
        if (generation != 0 && !isPrey && !newIsPrey) {
            if (alphaIndex == 0 && newAlphaIndex == 0) return 500e18;
            if (alphaIndex == 1 && newAlphaIndex == 1) return 800e18;
            if (alphaIndex == 2 && newAlphaIndex == 2) return 800e18;
            if (alphaIndex == 3 && newAlphaIndex == 3) return 800e18;
            if (alphaIndex == 4 && newAlphaIndex == 4) return 800e18;
        }
        revert("NFTEvolution: Invalid update path");
    }

    function _checkAllLevelsAtLeast(PreyPredator.PreyPredator memory traits, uint256 threshold) internal {
        require(traits.agility >= threshold, "NFTEvolution: agility does not reach the level required for this evolutionary upgrade");
        require(traits.charisma >= threshold, "NFTEvolution: charisma does not reach the level required for this evolutionary upgrade");
        require(traits.damage >= threshold, "NFTEvolution: damage does not reach the level required for this evolutionary upgrade");
        require(traits.defense >= threshold, "NFTEvolution: defense does not reach the level required for this evolutionary upgrade");
        require(traits.dexterity >= threshold, "NFTEvolution: dexterity does not reach the level required for this evolutionary upgrade");
        require(traits.health >= threshold, "NFTEvolution: health does not reach the level required for this evolutionary upgrade");
        require(traits.intelligence >= threshold, "NFTEvolution: intelligence does not reach the level required for this evolutionary upgrade");
        require(traits.luck >= threshold, "NFTEvolution: luck does not reach the level required for this evolutionary upgrade");
        require(traits.strength >= threshold, "NFTEvolution: strength does not reach the level required for this evolutionary upgrade");
    }

    function selectTraits(uint256 seed, bool isPrey, PreyPredator.PreyPredator memory t)
        internal
        view
        returns (PreyPredator.PreyPredator memory)
    {
        t.isPrey = isPrey;
