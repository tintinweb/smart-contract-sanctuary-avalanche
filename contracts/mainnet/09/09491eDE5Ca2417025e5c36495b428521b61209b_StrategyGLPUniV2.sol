...
contract StrategyGLPUniV2 is StratFeeManager, GasFeeThrottler {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IGMXRouter(chef).unstakeAndRedeemGlp(want, wantForGlp(_amount - wantBal), 1, address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        ...
        return glpForWant(glpAmount);
    }

    // it calculates the amount received for swapping GLP to ETH, not taking fees into account
    function glpForWant(uint256 _amount) internal view returns (uint256) {
    ...
    function wantForGlp(uint256 _amount) internal view returns (uint256) {
        uint256 usdgAmount = _amount
            * IGMXVault(gmxVault).getMaxPrice(want)
            / IGMXVault(gmxVault).PRICE_PRECISION();
        usdgAmount = IGMXVault(gmxVault).adjustForDecimals(usdgAmount, want, usdg);
        uint256 glpSupply = IERC20(glp).totalSupply();
        uint256 aumInUsdg = IGLPManager(glpManager).getAumInUsdg(false);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(glpManager, type(uint).max);
        IERC20(native).safeApprove(unirouter, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(glpManager, 0);
