...
contract PoolRewards is Initializable, IPoolRewards, ReentrancyGuardUpgradeable, PoolRewardsStorage {
    ...
    function _claimable(
        ...
        uint256 _rewardPerTokenAvailable = 0;
        if(_rewardPerToken(_rewardToken, _totalSupply) > userRewardPerTokenPaid[_rewardToken][_account]) {
            _rewardPerTokenAvailable =
    ...
    function resetUser(address _user, address _rewardToken, bool _resetRewards) external onlyAuthorized {
        require(_rewardToken != address(0), "incorrect-reward-token");
        require(isRewardToken[_rewardToken], "invalid-reward-token");
        userRewardPerTokenPaid[_rewardToken][_user] = 0;
        if(_resetRewards) {
            rewards[_rewardToken][_user] = 0;
        }
    }


    function resetRewardSetting(
        address _rewardToken,
        uint256 _rewardAmount,
        uint256 _rewardDuration
        ...
        // Update rewards earned so far
        rewardPerTokenStored[_rewardToken] = 0;

        rewardRates[_rewardToken] = _rewardAmount / _rewardDuration;

        rewardPerTokenStored[_rewardToken] = _rewardPerToken(_rewardToken, IThorPool(pool).totalShares());
        ...
        lastUpdateTime[_rewardToken] = block.timestamp;
        periodFinish[_rewardToken] = block.timestamp + _rewardDuration;
        emit RewardReset(_rewardToken, _rewardAmount, _rewardDuration);
    }
    ...
    function withdrawBalance(address _token, uint256 _amount, address _receiver) external onlyAuthorized {
...
interface IPoolRewards {
    ...
    /// reward Reset
    event RewardReset(address indexed rewardToken, uint256 reward, uint256 rewardDuration);
...
interface IThorPool {
    ...
    function setThorPoolRewards(address _poolRewards) external;
}
