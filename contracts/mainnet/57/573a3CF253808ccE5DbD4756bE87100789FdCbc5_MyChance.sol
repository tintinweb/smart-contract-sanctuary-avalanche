...
contract MyChance is Ownable, VRFConsumerBaseV2, Pausable, KeeperCompatibleInterface {
    ...
    mapping(PrizeBond.Assets=>uint256)  platformStakes;

    mapping(uint256 => mapping(PrizeBond.Assets=>uint256)) claimable;

    mapping(PrizeBond.Assets=>uint256)  totalFees;

    mapping(PrizeBond.Assets=>mapping(address=>uint256)) increasedStakes; // It keeps track of the staking of each user 
    PrizeBond prizeBond  = PrizeBond(0x76F36F3E7200ec8beB8b5E707af07D2025da49B9);
    uint256[] prizeBonds;
    ...
    function drawSpecialLottery(uint256 _drawDate) public whenNotPaused {
     ...
     function claim(uint256 _tokenId, uint256 _percentage) public {
        ...
        uint256 totalDAI = claimable[_tokenId][PrizeBond.Assets.DAI];
        claimable[_tokenId][PrizeBond.Assets.DAI] = 0;

        uint256 totalUSDT = claimable[_tokenId][PrizeBond.Assets.USDT];
        claimable[_tokenId][PrizeBond.Assets.USDT] = 0;

        uint256 totalUSDC = claimable[_tokenId][PrizeBond.Assets.USDC];
        claimable[_tokenId][PrizeBond.Assets.USDC] = 0;
        ...
        if (totalDAI > 0) {
            ...
            totalFees[PrizeBond.Assets.DAI] += totalFeesDAI;
        }
        ...
        if (totalUSDT > 0) {
            ...
            totalFees[PrizeBond.Assets.USDT] += totalFeesUSDT;
        }
        ...
        if (totalUSDC > 0) {
            ...
            totalFees[PrizeBond.Assets.USDC] += totalFeesUSDC;
        }
    }

    function mintPrizeBond(PrizeBond.Assets _assetType, uint weight) public whenNotPaused {
        require(weight > 0, "Invalid weight");

        if (_assetType == PrizeBond.Assets.DAI) {
        ...
        else if (_assetType == PrizeBond.Assets.USDC) {
        ...
        else if (_assetType == PrizeBond.Assets.USDT) {
            uint256 cost = PRICE * weight * 1e6;
    ...
    function burnPrizeBond(uint256 _tokenId) public {
        ...
        PrizeBond.Assets assetType = prizeBond.getAssetType(_tokenId);

        prizeBond.safeBurn(_tokenId);

        uint256 weight= prizeBondPositions[_tokenId].weight;

        if (PrizeBond.Assets(assetType) == PrizeBond.Assets.DAI) {
            lendingPool.withdraw(daiToken, PRICE * weight * 1e18, msg.sender);
            totalDAIBonds -= weight;
        } 
        else if (PrizeBond.Assets(assetType) == PrizeBond.Assets.USDC) {
            lendingPool.withdraw(usdcToken, PRICE * weight * 1e6, msg.sender);
            totalUSDCBonds -= weight;
        }
        else if (PrizeBond.Assets(assetType) == PrizeBond.Assets.USDT) {
    ...
    function increaseStake(PrizeBond.Assets _assetType, uint256 _total) public whenNotPaused {
        if (_assetType == PrizeBond.Assets.DAI) {
    ...
    function reduceStake(PrizeBond.Assets _assetType, uint256 _total) public {
    ...
    function _addSpecialLottery(uint256 _drawDate, PrizeBond.Assets _assetType, uint256 _total, string memory _description) public onlyOwner whenNotPaused {
        address token;
    ...
    function _executeDraw(uint256 _random) internal { 
        ...
        if (totalDAI > 0) 
        {
            claimable[tokenId][PrizeBond.Assets.DAI] += totalDAI;
        }

        if (totalUSDT > 0) {
            claimable[tokenId][PrizeBond.Assets.USDT] += totalUSDT;
        }

        if (totalUSDC > 0) {
            claimable[tokenId][PrizeBond.Assets.USDC] += totalUSDC;
        }
    ...
    function _claimFees() public onlyOwner {
        uint256 daiFees = totalFees[PrizeBond.Assets.DAI];
        totalFees[PrizeBond.Assets.DAI] = 0;

        uint256 usdtFees = totalFees[PrizeBond.Assets.USDT];
        totalFees[PrizeBond.Assets.USDT] = 0;

        uint256 usdcFees = totalFees[PrizeBond.Assets.USDC];
        totalFees[PrizeBond.Assets.USDC] = 0;
    ...
    function howMuchToClaim(uint256 _tokenId) public view returns(uint256) {
        return claimable[_tokenId][PrizeBond.Assets.DAI] + claimable[_tokenId][PrizeBond.Assets.USDT] + claimable[_tokenId][PrizeBond.Assets.USDC];
    }
    ...
    function accumulatedDAI() public view returns (uint256) {
        return IERC20(aDaiToken).balanceOf(address(this)) - totalDAIBonds * PRICE * 1e18 - pendingDAI - platformStakes[PrizeBond.Assets.DAI];
    }

    function accumulatedUSDT() public view returns (uint256) {
        return IERC20(aUsdtToken).balanceOf(address(this)) - totalUSDTBonds * PRICE * 1e6 - pendingUSDT - platformStakes[PrizeBond.Assets.USDT];
    }

    function accumulatedUSDC() public view returns (uint256) {
        return IERC20(aUsdcToken).balanceOf(address(this)) - totalUSDCBonds * PRICE * 1e6 - pendingUSDC - platformStakes[PrizeBond.Assets.USDC];
    }

    function getStakedAmount(PrizeBond.Assets _assetType) public view returns (uint256){
