...
contract Rome is ERC20Detailed, Ownable {
    ...
    string public _name = "Rome";
    string public _symbol = "RME";
    ...
    uint256 public OlyInsuranceFundFee = 200;
    uint256 public sellFee = 300;
    uint256 public blackholeFee = 25;
    uint256 public nftFee = 15;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(OlyInsuranceFundFee).add(blackholeFee);
    ...
    address public OlyInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("Rome", "RME", uint8(DECIMALS)) Ownable() {
        ...
        treasuryReceiver = 0x263fa694EB5Fd80b755b20C0AC64fE0dD1601bf6; 
        OlyInsuranceFundReceiver = 0xe43FC5e73694DC35BC977511cbbD0D130ab84E92;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(OlyInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        IERC20(USDC).transfer(treasuryReceiver, amountUSDCToTreasuryAndAIF.mul(treasuryFee).div(treasuryFee.add(OlyInsuranceFundFee)));
        IERC20(USDC).transfer(OlyInsuranceFundReceiver, amountUSDCToTreasuryAndAIF.mul(OlyInsuranceFundFee).div(treasuryFee.add(OlyInsuranceFundFee)));
    }

    function withdrawAllToTreasury() external swapping onlyOwner {

        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Oly token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = USDC;
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _OlyInsuranceFundReceiver,
        address _blackhole,
        address _nftFeeReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        OlyInsuranceFundReceiver = _OlyInsuranceFundReceiver;
