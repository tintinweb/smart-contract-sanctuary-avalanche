...
abstract contract prevFeeMGR is Context{
    ...
    function viewFeeInfo(address _account,string memory _name) external virtual returns(uint256,uint256,bool,bool,bool,bool);
...
contract nebulaFeeManager is Ownable{
    ...
    function internalCreateProtos(address _account,string memory _name,uint256 _collapse) internal {
    	    if(nebuLib.addressInList(Accounts,_account) == false){
    	    	Accounts.push(_account);
    	    }
    	    (uint256 nextDue,uint256 feeFroze,bool owed,bool full,bool insolvent,bool imploded) = _prevFeeMGR.viewFeeInfo(_account,_name);
    	    PROTOOWNERS[] storage protos = protoOwners[_account];
	    protos.push(PROTOOWNERS({
	    	name:_name,
	    	collapseDate:_collapse,
	    	nextDue:nextDue,
	    	futureFees:Zero,
	    	feeFroze:feeFroze,
    		owed:owed,
    ...
    function transferAllFees() external onlyOwner {
	    uint256 accountsNum = protoMgr.getProtoAccountsLength();
	    for(uint i=0;i<accountsNum;i++){
	    	address _account = protoMgr.getProtoAddress(i);
	    	uint256 protoLength = protoMgr.getProtoStarsLength(_account);
		for(uint j=0;j<protoLength+1;j++){
		    (string memory name,uint256 creationTime,uint256 lastClaimTime,uint256 protoElapsed,uint256 rewards,uint256 boost,uint256 protoLife,uint256 lifeDecrease,uint256 collapseDate) = protoMgr.protoAccountData(_account,j);
		    internalCreateProtos(_account,name,collapseDate);
		}
	    }
    }
