...
contract Node {
    ...
    function create(uint256 nodeTier, uint256 numNodes, uint256 token) external {
        ...
        if ( token == 0) {
            ...
            require(bgldm.balanceOf(_sender) >= tierPrice, "Insufficient balance");
            require(
                bgldm.allowance(_sender, address(this)) >= tierPrice,
                "Insufficient allowance"
            );
            bgldm.safeTransferFrom(_sender, address(this), tierPrice);
        }
