    ...
    ReentrancyGuardUpgradeable
{
    ...
    address public nodeCreateProcessFeeRecipient; // stabilator contract
    uint256 public rewardProcessFee; // 100 = 1.00%
    address public rewardProcessFeeRecipient; // stabilator contract
    ...
    function initialize(
        ...
        require(_joeRouter != address(0), "JOE_ROUTER_IS_0");
        require(_treasury != address(0), "TREASURY_IS_0");
        require(
            addresses.length == basisPoints.length &&
                basisPoints.length == sellToNative.length,
            "ARRAY_LENGTH_MISTMATCH"
        );
        require(_sellFee < 2001, "SELLFEE_>2000");
        require(_buyFee < 2001, "BUYFEE_>2000");
        __ERC20_init("MTEST", "MTEST");
        ...
        _mint(treasury, 250000000 ether);

        require(swapAmount > 0, "SWAP_IS_ZERO");
        swapTokensAmount = swapAmount * 1 ether;
    ...
    function _transfer(
        ...
        if (
            ...
            (from == address(uniswapV2Router) || from == address(uniswapV2Pair))
        ) {
            fee = buyFee;
        ...
        if (isBuyOrSell && referralRateForBuySell > 0) {
            referral = referral == address(0) ||
                balanceOf(referral) < minTokensForReferral ||
                nodeRewardManager._getNodeNumberOf(referral) <
                minNodesForReferral
                ? address(this)
                : referral;
    ...
    function setReferralConfig(
        uint256 _referralRateForBuySell,
        uint256 _referralRateForNodeCreation,
        uint256 _referralRateForNodeRewards,
        uint256 _minTokensForReferral,
        uint256 _minNodesForReferral
    ) external onlyOwner {
        require(_referralRateForBuySell < 1001, "VALUE>1000");
        require(_referralRateForNodeCreation < 1001, "VALUE>1000");
        require(_referralRateForNodeRewards < 1001, "VALUE>1000");
        maxBuyFee =
            maxBuyFee -
            referralRateForBuySell +
            _referralRateForBuySell;
        maxSellFee =
            maxSellFee -
            referralRateForBuySell +
            _referralRateForBuySell;
        referralRateForBuySell = _referralRateForBuySell;
        referralRateForNodeCreation = _referralRateForNodeCreation;
        referralRateForNodeRewards = _referralRateForNodeRewards;
        minTokensForReferral = _minTokensForReferral;
        minNodesForReferral = _minNodesForReferral;
    }
    ...
    // 100 = 1.00%
    function setProcessFeeConfig(
        uint256 _nodeCreateProcessFee,
        uint256 _rewardProcessFee
    ) external onlyOwner {
    ...
    function editFeeRecipient(
        ...
        totalFeeBasisPoints =
            ...
            basisPoints;
        totalFeeBasisPointsToSwap -= FeeRecipients[id].sellToNative
            ? FeeRecipients[id].basisPoints
            : 0;
        totalFeeBasisPointsToSwap += sellToNative ? basisPoints : 0;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "CANT_REMOVE_AMM_PAIRS");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ROUTER_HAS_ADDRESS");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IJoeRouter02(newAddress);
        address _uniswapV2Pair = IJoeFactory(uniswapV2Router.factory()).getPair(
            address(this),
            uniswapV2Router.WAVAX()
        );
        if (_uniswapV2Pair == address(0)) {
            uniswapV2Pair = IJoeFactory(uniswapV2Router.factory()).createPair(
                address(this),
                uniswapV2Router.WAVAX()
            );
        } else {
    ...
    function manualSwapAndLiquify() external onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this)) > rewardsPool
            ? balanceOf(address(this)) - rewardsPool
            : 0;
        ...
        uint256 liquidityAmount = (contractTokenBalance * liquidityPoolFee) /
            10000;
        uint256 liquidityAmountToSwap = liquidityAmount / 2;
        uint256 remainder = contractTokenBalance -
            rewardAmount -
            liquidityAmount;
        uint256 remainderToSwap = totalFeeBasisPoints > 0
            ? (remainder * totalFeeBasisPointsToSwap) / totalFeeBasisPoints
            : 0;
        uint256 totalAmountToSwap = rewardAmountToSwap +
            liquidityAmountToSwap +
            remainderToSwap;
        ...
        if (totalAmountToSwap > 0) {
            _addLiquidity(
                liquidityAmountToSwap,
                (receivedAVAX * liquidityAmountToSwap) / totalAmountToSwap
            );
        }
        ...
        uint256 totalFeeBasisPointsNotToSwap = totalFeeBasisPoints -
        ...
        for (uint256 x; x < feeRecipientCount; x++) {
            if (FeeRecipients[x].sellToNative) {
                uint256 amount = totalFeeBasisPointsToSwap > 0
                    ? (remainderAVAX * FeeRecipients[x].basisPoints) /
                        totalFeeBasisPointsToSwap
                    : 0;
                amount = amount > remainderAVAXBalance
                    ? remainderAVAXBalance
                    : amount;
                (bool sent, ) = FeeRecipients[x]
                    .recipient
                    .call{value: amount}("");
                ...
                uint256 amount = totalFeeBasisPointsToSwap > 0
                    ? (remainder * FeeRecipients[x].basisPoints) /
                        totalFeeBasisPointsNotToSwap
                    : 0;
                amount = amount > remainderBalance ? remainderBalance : amount;
                super._transfer(
                    address(this),
                    FeeRecipients[x].recipient,
                    amount
                );
    ...
    function createNodeWithTokens(string memory name) public tradingIsOpen {
        require(
        ...
        require(
            balanceOf(_msgSender()) >= nodeRewardManager.nodePrice(),
        ...
        if (referralRateForNodeCreation > 0) {
            address referral = Referrals[_msgSender()] == address(0) ||
                balanceOf(Referrals[_msgSender()]) < minTokensForReferral ||
                nodeRewardManager._getNodeNumberOf(Referrals[_msgSender()]) <
                minNodesForReferral
                ? address(this)
                : Referrals[_msgSender()];
            referralAmount =
                (nodeRewardManager.nodePrice() * referralRateForNodeCreation) /
                10000;
            if (referralAmount > 0) {
                ReferralIncome[referral] += referralAmount;
        ...
        uint256 processFee;
        if (
            nodeCreateProcessFee > 0 &&
            nodeCreateProcessFeeRecipient != address(0)
        ) {
            processFee =
                (nodeRewardManager.nodePrice() * nodeCreateProcessFee) /
                10000;
            if (processFee > 0) {
                totalProcessFees += processFee;
                IERC20Upgradeable(address(this)).safeTransferFrom(
                    _msgSender(),
                    nodeCreateProcessFeeRecipient,
                    processFee
                );
            }
        }
        ...
        IERC20Upgradeable(address(this)).safeTransferFrom(
            _msgSender(),
            address(this),
            nodeRewardManager.nodePrice() - referralAmount - processFee
        );
        ...
        if (
            ...
            uint256 remainderToSwap = totalFeeBasisPoints > 0
                ? (remainder * totalFeeBasisPointsToSwap) / totalFeeBasisPoints
                : 0;
            uint256 totalAmountToSwap = ((rewardAmount * rwSwap) / 10000) +
                liquidityAmountToSwap +
                remainderToSwap;
            uint256 receivedAVAX = _swap(totalAmountToSwap);
                ...
                if (FeeRecipients[x].sellToNative) {
                    ...
                    uint256 amount = totalFeeBasisPointsNotToSwap > 0
                        ? (remainder * FeeRecipients[x].basisPoints) /
                            totalFeeBasisPointsNotToSwap
                        : 0;
                    amount = amount > remainderBalance
                        ? remainderBalance
                        : amount;
                    super._transfer(
                        address(this),
    ...
    function setNodeCreateProcessFeeRecipient(address value)
        external
        onlyOwner
    {
    ...
    function removeFromRewardsPool(uint256 amount) external onlyApproved {
        require(
            rewardsPool > 0 && rewardsPool >= amount,
    ...
    function cashoutReward(uint256 blocktime) external tradingIsOpen {
        require(!isBlacklisted[_msgSender()], "BLACKLISTED");
        ...
        if (referralRateForNodeRewards > 0) {
            address referral = Referrals[_msgSender()] == address(0) ||
                ...
                : Referrals[_msgSender()];
            referralAmount =
                (rewardAmount * referralRateForNodeRewards) /
                10000;
            if (referralAmount > 0) {
                ReferralIncome[referral] += referralAmount;
        ...
        if (rewardProcessFee > 0 && rewardProcessFeeRecipient != address(0)) {
            ...
            if (processFee > 0) {
                totalProcessFees += processFee;
                IERC20Upgradeable(address(this)).safeTransfer(
                    rewardProcessFeeRecipient,
                    processFee
                );
            }
        }

        uint256 amount = rewardAmount - referralAmount - processFee;
        if (amount > 0 && IERC20Upgradeable(address(this)).balanceOf(address(this)) >= amount) {
            IERC20Upgradeable(address(this)).safeTransfer(
                _msgSender(),
                amount
            );
    ...
    function cashoutAll() public tradingIsOpen {
        require(!isBlacklisted[_msgSender()], "BLACKLISTED");
        ...
        IERC20Upgradeable(address(this)).safeTransfer(
            _msgSender(),
            rewardAmount - referralAmount - processFee
        );
        rewardsPool -= rewardAmount;
        emit CashoutAll(_msgSender(), rewardAmount);
    }

    function compound(uint256 amount) external tradingIsOpen {
        ...
        bytes memory basic = bytes.concat(
            "BRAIN-",
            _toString(block.timestamp),
            _toString(nodeRewardManager._getNodeNumberOf(_msgSender()))
        );
        uint256 nodeCount = nodePrice > 0 ? amount / nodePrice : 0;
    ...
    modifier onlyApproved() {
        require(
            proxyToApproved[_msgSender()] == true || _msgSender() == owner(),
            "onlyProxy"
        );
        _;
    }

    modifier tradingIsOpen() {
        require(isOpen, "TRADING_NOT_OPEN");
        _;
    }
    ...
    event ManualSwapAndLiquify(
        address indexed user,
        uint256 indexed contractTokenBalance
    );
    ...
    event SetNodeCreateProcessFeeRecipient(
        address indexed user,
        address indexed value
    );
    event SetRewardProcessFeeRecipient(
        address indexed user,
        address indexed value
    );
}
