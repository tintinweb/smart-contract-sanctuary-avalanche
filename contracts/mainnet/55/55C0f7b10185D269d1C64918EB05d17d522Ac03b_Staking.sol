...
abstract contract DelegationsAdapter is DelegatorRewardsAdapter {
    ...
    function _stake(bytes32 poolId, address delegator, Currency[] memory currencies) internal {
        ...
        require(
            isPoolOwner(delegator, poolId) || (currencies.length == 1 && currencies[0].erc20 == rewardsToken()),
            "E105"
        );
...
abstract contract DelegatorRewardsAdapter is Finalizer {
    ...
    /**
     * @notice Get total (in-flight and already collected) rewards of an account.
     * @param account account address
     * @param poolIds list of pools to compute in-flight rewards from
     */
    function getTotalRewards(address account, bytes32[] calldata poolIds) external view returns (uint256 rewards) {
        rewards = _unclaimedRewards[account] + computeDelegatorReward(account, poolIds);
...
abstract contract Finalizer is StakingStorage, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function _finalizePool(bytes32 poolId, uint16 performance, uint16 ownerBaseApy) private {
        ...
        ownerStake = MathUpgradeable.min(ownerStake, getConfig(POOL_SATURATION_CAP));
...
abstract contract StakingStorage is DepositsAdapter {
     ...
     */
    function _initializeRewardsToken(IERC20Upgradeable rewardsToken_) internal onlyInitializing {
