...
contract TraderJoeHelper is Ownable {
    ...
    function getTJHarvestableRewards(address user, address lp)
        public
        view
        returns (
            address[] memory rewardTokens,
        ...
        )
    {
        ...
        if (pool.rewarder != address(0)) {
            rewardTokens[1] = IBaseRewardPool(pool.rewarder).rewardToken();
            (uint256 balanceOf, , ) = IBaseRewardPool(pool.rewarder).userInfo(
                helpstack.mainStakingTJ
            );
            amounts[1] =
                (IBaseRewardPool(pool.rewarder).pendingTokens(helpstack.mainStakingTJ) *
                    IBaseRewardPool(helpstack.vectorRewarder).balanceOf(user)) /
                IBaseRewardPool(helpstack.vectorRewarder).totalSupply();
            usdAmounts[1] =
    ...
    //     uint256 length = IAPRHelper(aprHelper).getLengthTraderJoePools();
    ...
    //         (uint256 pid,,,,address rewarder, address helper) = IMainStakingJoe(mainStaking).getPoolInfo(asset);
