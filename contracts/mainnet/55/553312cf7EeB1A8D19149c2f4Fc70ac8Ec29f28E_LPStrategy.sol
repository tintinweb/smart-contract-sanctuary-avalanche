...
contract LPStrategy is BaseStrategy {
    ...
    uint256 public minWAVAX = 100000000000000;
    uint256 public maxChange = 10;

    ISushiSwap private immutable router;
    IMasterChef private immutable masterchef;
    uint256 private immutable pid;
    ICauldronV2 public cauldronV2 = ICauldronV2(0xc75962c0f4c5eE929F8E19ac3229860823D44ebf);

    address public immutable NORTH = 0xD0755413bfE2e08dB6bE72761cdD56d77d4B60f1;
    address public immutable USDT = 0xc7198437980c041c805A1EDcbA50c1Ce5db95118;
    address public immutable PTP = 0x22d4002028f537599bE9f666d1c4Fa138522f9c8;
    address private immutable rewardToken;
    address private immutable pairInputToken;
    bool private immutable usePairToken0;

    address public usdt_joe_Oracle = 0x8C927362AF77a7bbC542645Ff3244618354Fe533;
    address public usdt_ptp_Oracle = 0x1279d70E6DaaEb6ff5924bDF8424953d3f97f1c9;
    address public usdt_wavax_ptp_lp_Oracle = 0x392B8332FB29dddE4f4cA8C5Ee58048a6FD15739;
    address public usdt_north_Oracle = 0x7cffdBFAB3a65BCCb2FB8ef3E4b6eB09ec2554e4;
    address public usdt_avax_Oracle = 0x9AC9CBFA47D13fa2bA21F5aAb4CAD68A44300AC7;
    ...
    function harvestAndSwapToLP() public {
        autoHarvest();
    ...
    function refreshReward() public view returns (uint256 pendingValue, uint256 pendingNorth) {
        uint256 joeToUSD = (1e36 / IOracle(usdt_joe_Oracle).peekSpot(new bytes(0)));
        uint256 ptpToUSD = (1e36 / IOracle(usdt_ptp_Oracle).peekSpot(new bytes(0)));
        uint256 avaxToUSD = (1e36 / IOracle(usdt_avax_Oracle).peekSpot(new bytes(0)));
        uint256 northToUSD = (1e36 / IOracle(usdt_north_Oracle).peekSpot(new bytes(0)));
        ...
        pendingValue = (pendingBonusToken * ptpToUSD) + (pendingJoe * joeToUSD) + (balanceOfLPStrategy * avaxToUSD);
        pendingNorth = (pendingValue * 1e18) / northToUSD / 1e18;
    }

    function apr() public view returns (uint256 finalApr) {
        uint256 joeToUSD = (1e36 / IOracle(usdt_joe_Oracle).peekSpot(new bytes(0)));
        uint256 lpToUSD = (1e36 / IOracle(usdt_wavax_ptp_lp_Oracle).peekSpot(new bytes(0)));
        uint256 ptpToUSD = (1e36 / IOracle(usdt_ptp_Oracle).peekSpot(new bytes(0)));

        uint256 lpSupply = IERC20(IMasterChefV3(address(masterchef)).poolInfo(pid).lpToken).balanceOf(address(masterchef));
        uint256 joeYear = (IMasterChefV3(address(masterchef)).joePerSec() *
            IMasterChefV3(address(masterchef)).poolInfo(pid).allocPoint *
            1e18 *
            31536000) /
            IMasterChefV3(address(masterchef)).totalAllocPoint() /
            lpSupply;
        uint256 joeApr = (joeYear * joeToUSD) / lpToUSD;

        address rewarderToken = IMasterChefV3(address(masterchef)).poolInfo(pid).rewarder;
        uint256 rewarderTokenYear = (IRewarder(rewarderToken).tokenPerSec() * 1e18 * 31536000) / lpSupply;
        uint256 rewarderTokenApr = (rewarderTokenYear * ptpToUSD) / lpToUSD;
        ...
        finalApr = ((joeApr + rewarderTokenApr) * percent) / 100;
    }
}
...
contract RefreshBtn is Ownable {
    ...
    address public north;
    address public lpStrategy;

    address[] public enableStrategy;
    
    constructor(address _north,address[] memory _strategy) {
        north = _north;
        _setEnableStrategy(_strategy);
    }

    function setEnableStrategy(address[] memory _strategy) public onlyOwner returns(bool){
        return _setEnableStrategy(_strategy);
    }

    function _setEnableStrategy(address[] memory _strategy) internal returns(bool){
        for(uint256 i =0;i<_strategy.length;i++){
            enableStrategy.push(_strategy[i]);
        }
        return true;
    }
    ...
    function pending() public view returns(uint256,uint256){
        ...
        for(uint256 i =0;i<enableStrategy.length;i++){
            (uint256 pendingValue, uint256 pendingNorth) = ILPStrategy(enableStrategy[i]).refreshReward();
    ...
    function refresh(bool _type) public returns(uint256) {
        // uint256 lastNorthBalance = IERC20(north).balanceOf(address(this));
        for(uint256 i =0;i<enableStrategy.length;i++){
            ILPStrategy(enableStrategy[i]).harvestAndSwapToLP();
        }
        uint256 nowNorthBalance = IERC20(north).balanceOf(address(this));
        uint256 reward = nowNorthBalance;
        // uint256 reward = nowNorthBalance - lastNorthBalance;
        require(reward > 0 || _type,"Reward is 0");
        if(reward > 0){
