...
contract NODERewardManagement is Ownable {
    ...
    function claimable(NodeEntity memory node) private view returns (bool) {
        bool isDone = node.rewardAvailable >= rewardPerNode.mul(20);
        if(isDone) return false;
        bool check = block.timestamp >= (node.lastClaimTime + (claimTime * 1 days));
        if(check && block.timestamp <= (node.creationTime * 20 days)){
            return true;
        }
        return false;
...
contract Token is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 public rotPeriod = 20 days;
