...
contract ANPMainTest is Auth {
  ...
  event SwapBack(uint256 swapBackAmount);

  /** ======= ERC20 PARAMS ======= */

  string public constant NAME =  "ANPMainTest";    
  string public constant SYMBOL = "ANPMainTest";        
  ...
  // 9% Goes to the reflections for token holders; 
  uint256 reflectionFee = 900;
  // 0% Goes to the marketing team;
  uint256 marketingFee = 0;
  // 2.5% Goes to the dev team; 
  uint256 devFee = 250;
  // 2.5% Goes to the investment fund;
  uint256 investmentFee = 250;
  ...
  uint256 private lastSwapTime = block.timestamp; // setting last swap at contract creation time
  ...
  function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
    ...
    // check if swap back should occur on sell only
    if (isSell) {
        if(shouldSwapBack()){ 
            swapBack();             
        }
  ...
  function shouldSwapBack() internal view returns (bool) {
    return 
      ...
      swapEnabled
        &&
      block.timestamp >= lastSwapTime.add(swapTimeLockLength)
        && 
  ...
  function setSwapTimeLock(uint lengthOfTime) external onlyOwner {
    require(lengthOfTime > 0, "Time lock should be greater than zero");
    swapTimeLockLength = lengthOfTime * 1 minutes;
  }
