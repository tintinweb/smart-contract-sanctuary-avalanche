...
contract RaffleContractV2 is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 private minimumBalanceToWithdraw = 100000000; //$100
    ...
    function setMinimumBalanceToWithdraw(uint256 margin) external onlyOwner {
    ...
    function selectRandomWinner(uint256 id) public onlyAdmin {
        require(
            id <= _raffleId.current() && id > 0,
            "Raffle id does not exist"
        );
        ...
        if ((totalTicketSoldOfId * raffle.price) > raffle.rafflePrize) {
            withdrawablePool +=
                ...
                raffle.rafflePrize;
            fundingPool += raffle.rafflePrize;
        } else {
            fundingPool += (totalTicketSoldOfId * raffle.price);
        }
    ...
    function _withdraw(uint256 amount) private onlyOwner {
    ...
    function withdrawUSDC() external onlyOwner {
        require((withdrawablePool + fundingPool) > 0, "Pool is empty");
        uint256 withdrawAmount = withdrawablePool + fundingPool;
        withdrawablePool = 0;
        fundingPool = 0;
    ...
    function withdrawAndDistribute() external nonReentrant onlyAdmin {
        ...
        if (fundingPool > 0) {
            uint256 returnFundingAmount = fundingPool;
            fundingPool = 0;
            IERC20(USDC).transfer(owner(), returnFundingAmount);
        }

        if (withdrawablePool > 0 && withdrawablePool >= minimumBalanceToWithdraw) {
