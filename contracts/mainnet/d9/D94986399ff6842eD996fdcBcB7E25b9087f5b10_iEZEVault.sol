...
contract iEZEVault is Ownable {
    ...
    uint8[4] internal percentages = [25, 50, 75, 100];
    Timelock[4] internal vaults;
    ...
    constructor(
        ...
        require(_iEZE != address(0), "Zero address: iEZE");
        iEZE = IERC20(_iEZE);

        require(_owner != address(0), "Zero address: owner");
        require(_timestamps.length == 4, "FAILD: incorrect timestamp length");
    ...
    function claim() external onlyOwner {        
        ...
        for (uint8 i = 0; i < vaults.length; i++) {
            if(block.timestamp >= vaults[i].epoch && vaults[i].isClaimed == false) {
                ...
                // safe transfer token
                iEZE.safeTransfer(msg.sender, allowedTokens);
                // call event
    ...
    function getBalance() public view returns (uint) {
        return iEZE.balanceOf( address(this) );
    }

    function getVaults() external view returns (Timelock[4] memory) {
        return vaults;
    }  
