...
abstract contract BeefySingleHarvesterBase is ManageableUpgradeable, PausableUpgradeable {
  ...
  uint256 public constant GELATO_REGISTRY_GAS_OVERHEAD = 170_000; //Compared
  //	normal harvest to a harvest via gelato and
  //	came up with this conservative estimate.
  uint256 public constant BEEFY_HARVESTER_OVERHEAD = 30_000; //Estimated from
  ...
  ITaskTreasury public constant TASK_TREASURY =
    ITaskTreasury(0xe167199A490b04042414aFD71830bBF165f44C09);
    ...
    )
  {
    ...
    //	console.log( "willHarvest", willHarvestVault_);
    //	console.log( "txCostWithPremium",  estimatedTxCost_);
    //	console.log( "callReward", estimatedCallRewards_);
  ...
  function _performUpkeep(
    ...
    require(
      !isDailyHarvest_ ||
        (lastHarvest = uint32(vault_.strategy().lastHarvest())) < block.timestamp - 1 days,
      string(
        abi.encodePacked(
    ...
    )
  {
    ...
    try strategy.harvest(callFeeRecipient) {
      ...
      //		console.log( "Harvest Panic", errorCode);
    } catch (bytes memory) {
      //		console.log( "Default harvest reversion (last catch)");
    }
...
contract BeefySingleHarvesterGelato is BeefySingleHarvesterBase {
  ...
  function initialize() external initializer {
    ...
    maxGasPrice = 45_000_000_000; //45 gwei
    depositFundsThreshold = 1_000_000_000_000_000_000; //1 ether
  }
