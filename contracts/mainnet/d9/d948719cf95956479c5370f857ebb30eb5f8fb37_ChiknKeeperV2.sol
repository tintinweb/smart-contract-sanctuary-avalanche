...
contract ChiknKeeperV2 is Ownable, IERC721Receiver {
    ...
    function baseHolderEggs() public view returns (uint256) {
        return EGG_CONTRACT.BASE_HOLDER_EGGS();
            ...
            //Must be called with 01,02,03,04,05,06,07,08,09,10,11,12...... format.  Have not tested to the extent of 10000
    function allChickenRun(uint tid) public view returns(uint) {
            IChiknNFT.ChickenRun memory _chickenRun = IChiknNFT(0x8cFa8c9E967847c9B40C695D31c45488E708779C).allChickenRun(tid);
            return _chickenRun.kg;
    }

    function stakedChickenKG(uint tid) public view returns(uint) {
        IEggV2.StakedChiknObj memory _stakedChiknObj = IEggV2(0x7761E2338B35bCEB6BdA6ce477EF012bde7aE611).stakedChikn(tid);
        return _stakedChiknObj.kg;
    }

    function stakedChickenCDT(uint tid) public view returns(uint) {
        IEggV2.StakedChiknObj memory _stakedChiknObj = IEggV2(0x7761E2338B35bCEB6BdA6ce477EF012bde7aE611).stakedChikn(tid);
        return _stakedChiknObj.cooldownTs;
    }

    function stakedChickenEat(uint tid) public view returns(uint) {
        IEggV2.StakedChiknObj memory _stakedChiknObj = IEggV2(0x7761E2338B35bCEB6BdA6ce477EF012bde7aE611).stakedChikn(tid);
        return _stakedChiknObj.eatenAmount;
    }

    function stakedChickenLastCD(uint tid) public view returns(uint) {
        IEggV2.StakedChiknObj memory _stakedChiknObj = IEggV2(0x7761E2338B35bCEB6BdA6ce477EF012bde7aE611).stakedChikn(tid);
        return _stakedChiknObj.kg;
    }
    ...
    function _stake(uint256[] calldata tids) internal {
    ...
    function approveFeedToLevel(address to, uint256 amount) external restricted {
       FEED_CONTRACT.approve(to, amount);
    }

    function approveEggToSkip(address to, uint256 amount) external restricted {
       EGG_CONTRACT.approve(to, amount);
    }
    ...
    function transferFrom(address tokenAddress, address to, uint256 amt) external onlyOwner {
        ...
        function withdrawFEED() external onlyOwner {
            FEED_CONTRACT.transferFrom(address(this), msg.sender, FEED_CONTRACT.balanceOf(address(this)));
        }
