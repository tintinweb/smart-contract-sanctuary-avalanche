...
import "../../interfaces/ICompounderPTP.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "../roles/DependsOnFeeRecipient.sol";

/// Compounding strategy using yieldyak
contract YieldYakCompounderStrategy is Strategy2, DependsOnFeeRecipient {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function collectCollateral(
        ...
        ICompounderPTP(yS).deposit(collateralAmount);

        return
            ICompounderPTP(yS).getDepositTokensForShares(
    ...
    function returnCollateral(
        ...
        address yS = yakStrategy[token];
        uint256 receiptAmount = ICompounderPTP(yS).getSharesForDepositTokens(
            targetAmount
        );
        ...
        override
    {
        ...
        require(
            ICompounderPTP(newStrat).depositToken() == token,
    ...
    function _viewTVL(address token) public view override returns (uint256) {
        ...
        return
            ICompounderPTP(strat).getDepositTokensForShares(
    ...
    function harvestPartially(address token) external override nonReentrant {
        // ICompounderPTP(yakStrategy[token]).reinvest();
    }
    ...
    {
        address strat = yakStrategy[token];
        // uint256 reinvestAmount = ICompounderPTP(strat).estimateReinvestReward();
    ...
    function currentMultiple(address token) public view returns (uint256) {
        return
            (1e18 *
                ICompounderPTP(yakStrategy[token]).getDepositTokensForShares(
                    1e18
    ...
    function changeUnderlyingStrat(address token, address newStrat) internal {
        ...
        if (current != address(0)) {
            uint256 balanceBefore = IERC20(token).balanceOf(address(this));
            ICompounderPTP(current).withdraw(
            ...
            ICompounderPTP(newStrat).deposit(balanceDelta);

            startingTokensPerShare[token] =
                (ICompounderPTP(newStrat).getDepositTokensForShares(1e18) *
                    startingTokensPerShare[token]) /
                ICompounderPTP(current).getDepositTokensForShares(1e18);
        } else {
            yakStrategy[token] = newStrat;
            startingTokensPerShare[token] = ICompounderPTP(newStrat)
