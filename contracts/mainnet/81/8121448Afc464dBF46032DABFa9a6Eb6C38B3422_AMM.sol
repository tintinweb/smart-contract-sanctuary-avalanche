...
contract AMM is IAMM, Governable {
    ...
    uint256 public setllementPrice;
    ...
    */
    ...
    {
        if (ammState != AMMState.Active && ammState != AMMState.Settled) return 0;
        ...
        if (ammState == AMMState.Settled) {
            (int256 pnl,,) = _forceRemoveLiquidity(trader);
            position = positions[trader]; // reload the position because it may have changed in _forceRemoveLiquidity
            int notional = setllementPrice.toInt256() * abs(position.size) / 1e18;
            if (position.size > 0) {
                pnl += notional - position.openNotional.toInt256();
                longOpenInterestNotional -= position.size.toUint256();
            } else if (position.size < 0) {
                pnl += position.openNotional.toInt256() - notional;
                shortOpenInterestNotional -= (-position.size).toUint256();
            }
            position.size = 0;
            position.openNotional = 0;
            // in clearing house, the sign of the value returned from here is reversed before sending it in to marginAccount.realizePnL
            return fundingPayment - pnl;
        }
    }

    function getPostSettlementPnl(address trader) external view returns (int pnl) {
        (,int256 unrealizedPnl, int size, uint openNotional) = getNotionalPositionAndUnrealizedPnl(trader);
        if (ammState != AMMState.Settled) return unrealizedPnl;

        int notional = setllementPrice.toInt256() * abs(size) / 1e18;
        if (size > 0) {
            return notional - openNotional.toInt256();
        } else if (size < 0) {
            return openNotional.toInt256() - notional;
        }
    }
    ...
    */
    ...
    {
        return _forceRemoveLiquidity(maker);
    }

    function _forceRemoveLiquidity(address maker)
        internal
        returns (int realizedPnl, uint makerOpenNotional, int makerPosition)
    {
        Maker storage _maker = _makers[maker];
        if (_maker.dToken == 0 && _maker.ignition == 0) return (0, 0, 0);
        if (ammState == AMMState.Active || ammState == AMMState.Settled) {
        ...
        } else if (ammState == AMMState.Ignition) {
    ...
    */
    ...
    function settle() external whenActive onlyGovernance {
        ammState = AMMState.Settled;
        setllementPrice = uint(oracle.getUnderlyingPrice(underlyingAsset));
    }
}
