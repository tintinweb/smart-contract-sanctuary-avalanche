...
contract FractVaultV1 is Ownable, ReentrancyGuard {
    ...
    // Address of layerOne contract.
    address public contractFromLayerOne;
    ...
    // Total deposits of the vault, excluding deployed capital.
    uint256 public uninvestedCapital;
    ...
    /**
     * @notice This event is fired when a layerOneAddress is set.
     * @param account The layerOneAddress.
     */
    event SetLayerOneAddress(address indexed account);
    ...
    // modifier onlyContractFromLayerOne() {
    //     require(msg.sender == contractFromLayerOne, "Only the contract from L1 can call this function.");
    //     _;
    // }

    /**
     * @notice Method for depositing to the vault, without deploying to a strategy.
     * @notice This method will be set to onlyContractFromLayerOne
     * @param layerOneAddress address of contract on L1 that is sending deposit.
     */
    function setL1Address(address layerOneAddress) external onlyOwner {
        require(layerOneAddress != address(0), "Address cannot be a 0 address");
        contractFromLayerOne = layerOneAddress;

        emit SetLayerOneAddress(contractFromLayerOne);
    }
    ...
    function depositToVault(uint256 amount) public onlyOwner nonReentrant {
        ...
        uninvestedCapital = uninvestedCapital + amount;
    ...
    function deployToStrategy(address strategy) external onlyOwner {
        ...
        uninvestedCapital = uninvestedCapital - depositTokenBalance;
    ...
    function withdrawFromStrategy(address strategy) external onlyOwner {
        ...
        uint256 interest = withdrawnAmount - deployedCapital;
        uninvestedCapital = uninvestedCapital + withdrawnAmount;
        deployedCapital = deployedCapital + interest - uninvestedCapital;
    ...
    function withdrawPercentageFromStrategy(address strategy, uint256 withdrawPercentageBips) external onlyOwner {
        ...
        uninvestedCapital = uninvestedCapital + withdrawnAmount;
        deployedCapital = deployedCapital - uninvestedCapital;
    ...
    /**
     * @notice Returns uninvestedCapital + deployedCapital. 
     */
    function totalDeposits() external view returns (uint256) {
        return uninvestedCapital + deployedCapital;
    }

    /**
     * @notice Returns uninvestedCapital.
     */
    function totalUninvestedCapital() external view returns (uint256) {
        return uninvestedCapital;
    }

    /**
     * @notice Returns deployedCapital.
     */
