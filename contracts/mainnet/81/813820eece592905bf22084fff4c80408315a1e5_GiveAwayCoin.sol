...
//  Telegram: https://t.me/GiveAway_Coin_Spot  
// MAX BUY 1 token / can't sell
// More info in the Telegram or https://twitter.com/SecureSpot_ on twitter
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract GiveAwayCoin is ERC20, Ownable  {
    using SafeMath for uint256;
    ...
    constructor() ERC20("GiveAway Coin", "GIVEAWAY") {
        ...
        uint256 _buyDevFee = 0;             // 0% buy tax

        uint256 _sellMarketingFee = 100;
        uint256 _sellLiquidityFee = 0;
        uint256 _sellDevFee = 0;            // 100% sell tax you can't sell
        
        uint256 totalSupply = 2 * 1e2 * 1e18; // 200 tokens
        
        maxTransactionAmount = 1 * 1e0 * 1e18; // 1 token
        maxWallet = totalSupply * 5 / 1000; // 1 token = 0.5% maxWallet
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                        require(amount == maxTransactionAmount, "Buy transfer needs to be 1.");
                        require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded");
                }
                
                //when sell
                else if (automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[from]) {
                        require(amount <= 0, "Sell transfer amount exceeds the maxTransactionAmount.");
                }
