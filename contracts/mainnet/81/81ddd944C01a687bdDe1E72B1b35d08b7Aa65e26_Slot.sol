...
contract Slot is VRFConsumerBaseV2, Ownable, KeeperCompatibleInterface {
    ...
    // mapping(uint8 => uint256) private i_payoutMapping;
    ...
    uint256 private constant PAYOUT_CUSHION = 1719197707736390000;
    ...
    constructor(
        ...
        unchecked {
            ...
            // s_lastNumUsedIdx = startingStoredWords * uint16(NUMBERS_PER_WORD) - 1;
        }
        // i_payoutMapping[0] = 0;
        // i_payoutMapping[1] = PAYOUTS_WEI_1;
        // i_payoutMapping[2] = PAYOUTS_WEI_2;
        // i_payoutMapping[3] = PAYOUTS_WEI_3;
        // i_payoutMapping[4] = PAYOUTS_WEI_4;
        // i_payoutMapping[5] = PAYOUTS_WEI_5;
        // i_payoutMapping[6] = PAYOUTS_WEI_6;
        // i_payoutMapping[7] = PAYOUTS_WEI_7;
        // i_payoutMapping[8] = PAYOUTS_WEI_8;
        // i_payoutMapping[9] = PAYOUTS_WEI_9;
        // i_payoutMapping[10] = PAYOUTS_WEI_10;
        // i_payoutMapping[11] = PAYOUTS_WEI_11;
        // i_payoutMapping[12] = PAYOUTS_WEI_12;
        // i_payoutMapping[13] = PAYOUTS_WEI_13;
    }
    ...
    {
        // bool needsWords = s_words.length <= i_refreshStoredNumbers;
        bool needsWords;
        unchecked {
    ...
    function fulfillRandomWords(uint256, uint256[] memory randomWords) internal override {
        ...
        s_words = numbers;
        // console.log("Fulfilled... length: ", s_words.length);
        unchecked {
    ...
    function evalSpin(
        ...
        uint8 spinStreakStopOne,
        uint8 spinStreakStopTwo
    ) internal pure returns (uint8 winType, uint256 payout) {
        if (spinStreakOne == 0) {
            winType = 0;
            payout = 0;
            ...
            if (spinStreakStopOne == LUCKY_STOP_DIGIT) {
                if (spinStreakOne == 2) {
                    winType = 7;
                    payout = PAYOUTS_WEI_7;
                } else if (spinStreakOne == 3) {
                    winType = 8;
                    payout = PAYOUTS_WEI_8;
                } else if (spinStreakOne == 4) {
                    winType = 9;
                    payout = PAYOUTS_WEI_9;
                } else if (spinStreakOne == 5) {
                    winType = 10;
                    payout = PAYOUTS_WEI_10;
                }
            } else {
                if (spinStreakOne == 2) {
                    winType = 1;
                    payout = PAYOUTS_WEI_1;
                } else if (spinStreakOne == 3) {
                    winType = 2;
                    payout = PAYOUTS_WEI_2;
                } else if (spinStreakOne == 4) {
                    winType = 3;
                    payout = PAYOUTS_WEI_3;
                } else if (spinStreakOne == 5) {
                    winType = 4;
                    payout = PAYOUTS_WEI_4;
                }
            }
        } else {
            ...
            if (
                ...
                winType = 6;
                payout = PAYOUTS_WEI_6;
            } else if (
                spinStreakStopOne == LUCKY_STOP_DIGIT || spinStreakStopTwo == LUCKY_STOP_DIGIT
            ) {
                winType = 11;
                payout = PAYOUTS_WEI_11;
            } else {
                winType = 5;
                payout = PAYOUTS_WEI_5;
            }
        }
        return (winType, payout);
    }
    ...
    function buySpins() public payable {
        ...
        for (uint8 i; i < SPINS_PER_BUY; ) {
            ...
            // uint256 payout;
            uint16 currentIdx;
            // SpinStreakDataNew memory spinStreak = SpinStreakDataNew(0, 0, 0, 99, 99, 99); // this ends up costing slightly more gas.
            ...
            // (winType, payout) = evalSpin(
            //     spinStreak.spinStreakOne,
            //     spinStreak.spinStreakTwo,
            //     spinStreak.spinStreakStopOne,
            //     spinStreak.spinStreakStopTwo
            // );
            if (spinStreakOne == 0) {
                playerWins[i] = 0;
            } else if (spinStreakTwo == 0) {
                if (spinStreakStopOne == LUCKY_STOP_DIGIT) {
                    if (spinStreakOne == 2) {
                        playerWins[i] = 7;
                        unchecked {
                            // playerPayout += i_payoutMapping[7];
                            playerPayout += PAYOUTS_WEI_7;
                        }
                    } else if (spinStreakOne == 3) {
                        playerWins[i] = 8;
                        unchecked {
                            // playerPayout += i_payoutMapping[8];
                            playerPayout += PAYOUTS_WEI_8;
                        }
                    } else if (spinStreakOne == 4) {
                        playerWins[i] = 9;
                        unchecked {
                            // playerPayout += i_payoutMapping[9];
                            playerPayout += PAYOUTS_WEI_9;
                        }
                    } else if (spinStreakOne == 5) {
                        playerWins[i] = 10;
                        unchecked {
                            // playerPayout += i_payoutMapping[10];
                    ...
                    if (spinStreakOne == 2) {
                        playerWins[i] = 1;
                        unchecked {
                            // playerPayout += i_payoutMapping[1];
                            playerPayout += PAYOUTS_WEI_1;
                        }
                    } else if (spinStreakOne == 3) {
                        playerWins[i] = 2;
                        unchecked {
                            // playerPayout += i_payoutMapping[2];
                            playerPayout += PAYOUTS_WEI_2;
                        }
                    } else if (spinStreakOne == 4) {
                        playerWins[i] = 3;
                        unchecked {
                            // playerPayout += i_payoutMapping[3];
                            playerPayout += PAYOUTS_WEI_3;
                        }
                    } else if (spinStreakOne == 5) {
                        playerWins[i] = 4;
                        unchecked {
                            // playerPayout += i_payoutMapping[4];
                ...
                if (
                    ...
                    unchecked {
                        ...
                        // playerPayout += i_payoutMapping[12];
                    }
                } else if (
                    // 2 doge x 3 other
                    (spinStreakStopOne == LUCKY_STOP_DIGIT && spinStreakTwo == 3) ||
                    ...
                    playerWins[i] = 13;
                    unchecked {
                        // playerPayout += i_payoutMapping[13];
                    ...
                    playerWins[i] = 6;
                    unchecked {
                        // playerPayout += i_payoutMapping[6];
                    ...
                    unchecked {
                        ...
                        // playerPayout += i_payoutMapping[11];
                    }
                } else {
                    playerWins[i] = 5;
                    unchecked {
                        playerPayout += PAYOUTS_WEI_5;
                        // playerPayout += i_payoutMapping[5];
                    }
                }
            }
            // playerWins[i] = winType;
        ...
        // for (uint8 i; i < SPINS_PER_BUY * NUM_REELS; ) {
        //     s_words.pop();
        //     unchecked {
        //         i++;
        //     }
