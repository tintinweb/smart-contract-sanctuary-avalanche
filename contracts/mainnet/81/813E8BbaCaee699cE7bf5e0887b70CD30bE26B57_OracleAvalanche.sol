    ...
    IIndexOracle
{
    ...
    function getPrice(
        ...
        if (!includeAmmPrice) {
            if (chainlinkPrice < 0) {
                revert Errors.Oracle_NegativePrice(); // Token price must be nonnegative.
            }
        ...
        if (price < 0) {
            revert Errors.Oracle_TokenNotSupported(token);
        }
    ...
    function _getAmmPrice(address token, bool maximize)
        internal
        view
        returns (int256 price)
    {
