...
PantherNodes.
Websitet: https://panthernodes.finance
Discord: https://discord.gg/p5QmQVTk
Twitter: https://twitter.com/PantherNodes
...
contract PantherNodes is ERC20, Ownable {
    ...
    bool public nftActivated = false;
    ...
    event Compound(address indexed account, uint256 amount, uint256 indexed blockTime);
    ...
    constructor(address[] memory addresses, uint256[] memory fees, uint256 swapAmount, address _boosterNFTAddress)
    ERC20("PantherNodes", "CLAW") {
        ...
        boosterNFT = IERC20(_boosterNFTAddress);

        require(addresses[0] != address(0) && addresses[1] != address(0) && addresses[2] != address(0), "CTR:1");
        teamPool = addresses[0];
        rewardsPool = addresses[1];
        nodeManager = INodeManager(addresses[2]);

        require(joeRouterAddress != address(0), "CTR:2");
        ...
        require(fees[0] != 0 && fees[1] != 0 && fees[2] != 0 && fees[3] != 0, "CTR:3");
        teamPoolFee = fees[0];
        rewardsFee = fees[1];
    ...
    function updatemaxBalanceAmountRate(uint16 _maxBalanceAmountRate) external onlyOwner {
        ...
        require(_maxBalanceAmountRate >= 200, "Max transfer amount rate must  exceed the minimum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        ...
        require(_maxTransferAmountRate >= 100, "Max transfer amount rate must exceed the minimum rate.");
    ...
    function cashoutAll() external {
        address sender = _msgSender();
        require(sender != address(0), "CSH:5");
        require(!isBlacklisted[sender], "BL");
        require(sender != teamPool && sender != rewardsPool, "CSH:7");
        uint256 rewardAmount = nodeManager.getAllNodesRewards(sender);
        require(rewardAmount > 0, "CSH:8");
        if (swapLiquifyEnabled) {
            uint256 feeAmount;
            bool active_NFT;
            if(nftActivated){
                ...
                if(((block.timestamp - last_booster_claim) >= 24 hours)){
                    uint256 numberBoosterNFT = boosterNFT.balanceOf(sender);
    ...
    function excludeFromFee(address[] calldata addrs) public onlyOwner {
        for (uint256 i; i < addrs.length; ++i) {
            _isExcludedFromFee[addrs[i]] = true;
        }
    }

    function setNodeManager(address _addr) external onlyOwner() {
        nodeManager = INodeManager(_addr);
    }

    function setBoosterNFT(address _addr) external onlyOwner() {
        boosterNFT = IERC20(_addr);
    }

    function activateBoosterNFT(bool _status) external onlyOwner() {
        nftActivated = _status;
    }
