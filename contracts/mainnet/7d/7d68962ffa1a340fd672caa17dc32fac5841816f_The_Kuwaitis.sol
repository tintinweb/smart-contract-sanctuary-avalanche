...
contract The_Kuwaitis is ERC721Enumerable, Ownable, IERC2981Royalties {
    using Strings for uint256;
    ...
    uint256 public maxTokens = 529;
    uint256[6226] public remainingTokens;
    uint256 public mintPrice = 0.35 ether;
    string public baseURI = "ipfs://bafybeibdog73dtmaxsylbf25noxvieop77hiatjus3uy7rzn5xtmtffbx4/";
    
    constructor() ERC721("The Kuwaitis", "KWTS") {
        ...
        function mint(uint256 mintAmount) public payable {
            require(!salePaused, "Minting is not active");
            require(mintAmount > 0, "Cannot mint 0 NFTs");
            require(totalSupply() + mintAmount <= maxTokens, "Exceeded maximum token amount!");
            require(msg.value == mintPrice * mintAmount, "Incorrect input price");
			
			payable(owner()).transfer(msg.value);
            
            for (uint256 i = 1; i <= mintAmount; i++) {
                randomizeMinting(msg.sender);
            }
        }

        function randomizeMinting(address sender) internal  {
