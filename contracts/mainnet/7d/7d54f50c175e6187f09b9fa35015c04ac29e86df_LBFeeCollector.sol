...
/// @title SteakHut LB Fee Collector (TJ V2.1)
/// @author SteakHut Finance
/// @notice acts as a wallet to store fees from LB strategies for harvesting later
contract LBFeeCollector is Ownable {
    using SafeERC20 for IERC20;

    // The address of the JOE router
    ...
    constructor(address _joeRouter) {
        require(_joeRouter != address(0), "JR:0");
    ...
    /// @dev fix this here.
    function inCaseLBTokensGetStuck(address _lbToken, uint256[] calldata _id, uint256[] calldata _amounts)
        external
        onlyOwner
    {
        ILBToken(_lbToken).batchTransferFrom(address(this), msg.sender, _id, _amounts);
    }

    /// @notice distributes protocol and strategist fees
    /// @notice to be used in conjunction with a subgraph to track fee bins
    function convertFees(CollectionParams memory _params) external onlyOwner {
        ILBToken lbToken = ILBToken(_params._lbToken);

        //provide required lb token approvals
        lbToken.approveForAll(joeRouter, true);

        uint256[] memory amounts = new uint256[](_params.binsToClaim.length);

        //fetch balance to remove
        for (uint256 i; i < _params.binsToClaim.length; i++) {
            uint256 lbTokenAmount = lbToken.balanceOf(address(this), _params.binsToClaim[i]);
            amounts[i] = lbTokenAmount;
        }

        (uint256 amountX, uint256 amountY) = ILBRouter(joeRouter).removeLiquidity(
            IERC20(_params._tokenX),
            IERC20(_params._tokenY),
            _params._binStep,
            0,
            0,
            _params.binsToClaim,
        ...
        _chargeFees(_params._tokenX, _params._tokenY, amountX, amountY, _params._treasury);
    ...
    function _chargeFees(
        ...
        if (amountXReceived > 0 || amountYReceived > 0) {
            if (amountXReceived > 0) {
                ...
                IERC20(_tokenX).safeTransfer(_feeRecipient, amountXReceived);
            }
            if (amountYReceived > 0) {
                //transfer y to reward receivers
                IERC20(_tokenY).safeTransfer(_feeRecipient, amountYReceived);
            }
            //emit an event
