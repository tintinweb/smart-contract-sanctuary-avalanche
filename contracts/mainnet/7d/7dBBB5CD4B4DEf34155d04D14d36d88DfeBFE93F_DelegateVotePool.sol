...
contract DelegateVotePool is Initializable, OwnableUpgradeable {
    ...
    function updateWeight(address lp, uint256 weight) external onlyOwner {
        ...
        if (!isvotePool[lp]) {
            require(IBribeManager(bribeManager).isPoolActive(lp), "Pool not active");
            isvotePool[lp] = true;
            votePools.push(lp);
        }
    ...
    function deletePool(address lp) external onlyOwner {
        isvotePool[lp] = false;
        ...
        totalWeight = totalWeight - votingWeights[lp];
        votingWeights[lp] = 0;
        if (_getVoteForLp(lp) > 0) {
            IBribeManager(bribeManager).unvote(lp);
        }
        _updateVote();
    }
    ...
    function _getVoteForLp(address lp) internal returns(uint256 vote) {
        vote = IBribeManager(bribeManager).userVoteForPools(address(this), lp);
    }
